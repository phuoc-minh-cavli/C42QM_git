...
#  $Header: //components/rel/core.glue/1.3/tools/cmm/common/mdm9205/kickstart/lsusb.py#1 $
...
if __name__ == '__main__':
    ...
    ShowOnlyAvailableComPorts = True
    portname = None                     # They could do "python lsusb.py -p COM9" so look to see if COM9 exists
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "p:a", ["port=","showall"])
    except getopt.GetoptError, err:
        # print help information and exit:
        print "\n"
        print str(err) # will print something like "option -a not recognized"
        print "\nEXAMPLE: To look for COM9 do this"
        print "\n\tpython lsusb.py -p COM9\n"
        sys.exit(1)
    ...
    for o, a in opts:
        if o in ("-p", "--port"):
            ...
            ShowOnlyAvailableComPorts = False

    for order, port, desc, hwid in sorted(comports(ShowOnlyAvailableComPorts)):
    ...
    if portname is not None:
        ...
        if PortExists == 0:
            filename = '%s_DoesNotExist.txt' % portname
            try:
                fp = open(filename, 'w')
            except:
                print "\nERROR: Can't create the file %s" % filename
                print "This might be because the file is open and locked"
                print "Or because you are running this from a read-only location\n"
                sys.exit()

            try:
                fp.write( "Sorry about your luck :)")
            except:
                print "\nERROR: Can't write to the file %s" % filename
                sys.exit()
            ...
            filename = '%s_exists.txt' % portname
            try:
                fp = open(filename, 'w')
            except:
                print "\nERROR: Can't create the file %s" % filename
            ...
            try:
                fp.write( "%-10s: ID %s %s" % (PortArray[Where], PIDArray[Where], DescArray[Where]) )
            except:
                print "\nERROR: Can't write to the file %s" % filename
                sys.exit()
