...
REM 04/04/2014  AJCheriyan      Added change to detect version of windows
...
IF EXIST %TEMP%\%TEMPFILE% DEL /Q %TEMP%\%TEMPFILE%
...
call python %~dp0..\..\..\build\app\gen_cmm_data3.py ..\..\.. "%TEMP%\%CMMFILENAME%" -paths_only
...
:START_T32_1
IF NOT EXIST %T32SYS%\bin\%OSFLAVORPATH%\t32start.exe (
start %T32SYS%\t32start.exe -runcfgfile %CHIPSET% -runitem "//Root/DAP"
GOTO EXIT
)
:START_T32_2
IF EXIST %T32SYS%\bin\%OSFLAVORPATH%\t32start.exe (
start %T32SYS%\bin\%OSFLAVORPATH%\t32start.exe -runcfgfile %CHIPSET% -runitem "//Root/DAP"
GOTO EXIT
)
)
