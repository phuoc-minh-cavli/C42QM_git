...
-------------------------------------------------------------------------------
 ...
 $Header: //components/rel/build.glue/1.0/lib/meta_lib.py#58 $
 $DateTime: 2021/04/13 12:37:28 $
...
class meta_info:
   ...
   def __init__ (self, logger=None, file_pfn=None, fb_nearest=False, local_image=False):
...
#      txt += XmlPrettyPrint(self.XmlRoot)
   ...
   def clean_file_paths(self, si_root_path_rule=None, lower_case_si_path=True):
         ...
         if (si_root_path_rule is not None and build_name != 'glue' and build_name != 'common'):
            if (si_root_path_rule == 'spf' or si_root_path_rule == 'chipset_sipl'):
               ...
               if si_root_path_rule == 'spf':
                  ...
                  lnx_path += (str(build.find('chipset').text) + '_' + si_name)
                  win_path += (str(build.find('chipset').text) + '_' + si_name)
            elif(si_root_path_rule == 'buildtag'):
               lnx_path += build_name
               win_path += build_name
            elif(si_root_path_rule == 'chipset_role'):
               lnx_path += (str(build.find('chipset').text) + '_' + str(build.find('role').text))
               win_path += (str(build.find('chipset').text) + '_' + str(build.find('role').text))
            #Add trailing slash explicitly to resolve QPST path issue
            if(si_root_path_rule != 'root'):
               lnx_path += '/'
               win_path += '\\'
         if lower_case_si_path:
            lnx_path = lnx_path.lower()
            win_path = win_path.lower()
  ...
  # Get list of workflow steps for re-compilation chnage of given SI build
   ...
   #Get list of dump type
   def get_dump_types(self, binary_type=None, form_factor=None, storage=None):
      dump_type_list=[]
      dump_type_elements = self.XmlRoot.findall('./dump_types/dump_type') 
      if len(dump_type_elements) > 0:
         attr = None
         if (binary_type==None and form_factor==None and  storage==None):
            attr = '!(binary_type)&&!(form_factor)&&!(storage_type)'
         
         if binary_type != None:
            attr = 'binary_type=='+'"'+	binary_type+'"'
         if form_factor != None and attr != None:
            attr = '('+ attr +')'+ '&&' + '(' + 'form_factor==' + '"'+ form_factor + '"' +')' 
         elif 	form_factor != None and attr == None:	 
           attr = 'form_factor==' + '"'+ form_factor + '"' #no need of ( around attribute
         if storage != None and attr != None:
            attr = attr + '&&' + '(' + 'storage_type==' + '"' + storage + '"' +')'   
         elif 	storage != None and attr == None:
            attr = 'storage_type==' + '"'+ storage	+ '"'
         self.log("Attribute :" + str(attr))
            		  
         attr_tree = ae.parse_attr_string(attr)
         for dump_type_element in dump_type_elements:
           if (ae.eval(attr_tree, dump_type_element) == True):
              dump_type_list.append(str(dump_type_element.text))
      #Return dump type list	 
      return dump_type_list		   
...
'''
   ...
   def is_signed(self):
      '''Check if meta binaries are signed'''
      step_elem = self.XmlRoot.findall('./workflow/step[@filter="sign_binary"][@type="exec"]')
      return (len(step_elem) > 0)
...
Attributes, if any, are not returned.'''
   ...
   def __init_fb_obj(self):
      ...
      if self.FINDABLE:
         ...
         self.fb = fb_info_lib.fbInfo(self.on_linux, self.logger, self.fb_nearest, self.local_image)
   ...
   def get_default_flash_config(self):
      product_info = self.XmlRoot.find('product_info')
      default_element = product_info.find ('default')
      default = {}    
      if default_element == None: 
         return default   
      if 'flavor' in default_element.attrib : 
         default['flavor'] = default_element.attrib['flavor']
      if 'storage_type' in default_element.attrib:
         default['storage_type'] =  default_element.attrib['storage_type']
      if 'active_slot' in default_element.attrib:
         default['active_slot'] =  default_element.attrib['active_slot']
      return default
#==============================================================================
# 
