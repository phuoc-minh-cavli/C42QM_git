...
from .v2_header import ROOTCERTHASH_MAX_COUNT
from .v2_header import SERIAL_NUM_MAX_COUNT
...
class DbgpHdrv3(DbgpHdrv2):
    ...
    def unpack(self, data, offset=0):
        ...
        if not self.data_is_none:
            data = ensure_binary(data[offset:])

        # Unpack the base
        fmt = self.get_format()
        if self.data_is_none:
            data = b'\0' * struct.calcsize(fmt)
    ...
    def _pack_rootcerthash_qti_array(self):
        values = []
        for i in range(self.rootcerthash_qti_count):
            values.append(ensure_binary(binascii.unhexlify(self.rootcerthash_qti_array[i])))
        values = values + list([b''] * (ROOTCERTHASH_MAX_COUNT - self.rootcerthash_qti_count))
    ...
    def validate_ranges(self, serial_num_min_count=1):
        ...
        if not (serial_num_min_count <= self.serial_num_count <= SERIAL_NUM_MAX_COUNT):
            raise RuntimeError('Invalid number of serial numbers supplied in config file. ' +
                               '%d - %d' % (serial_num_min_count, SERIAL_NUM_MAX_COUNT) +
                               ' serial numbers supported, %d detected.' % self.serial_num_count)
