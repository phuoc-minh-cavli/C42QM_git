...
class DbgpHdrv1(StructBase):
    ...
    def _pack_base(self):
        values = [ensure_binary(self.magic),
                  int(self.data_size),
                  int(self.version)]
        return struct.pack(self.get_format(), *values)

    def _pack_serial_segment(self):
        values = [int(self.sernum_start), int(self.sernum_end)]
    ...
    def _pack_rootcerthash_count(self):
        values = [int(self.rootcerthash_count)]
    ...
    def validate(self):
        ...
        if ensure_str(self.magic) not in df.DBGPMAG_DESCRIPTION.keys():
            raise RuntimeError(b'Invalid magic in the Debug Policy file: ' + self.magic + b'\n'
                               b'This is not a Debug Policy file.')
