...
from sectools.common.parsegen.dbgp.header.v2_header import SERIAL_NUM_MAX_COUNT
    ...
    def _validate_generation(self, dp_file):
        ...
        if self._config_parser:
            ...
            if dbgpelfparser.dbgpparser.header.rootcerthash_array[:dbgpelfparser.dbgpparser.header.rootcerthash_count] != \
                [ensure_binary(x.lower()) for x in self._config_parser.root.file_properties.root_cert_hash_list.root_cert_hash]:
            ...
            if getattr(dbgpelfparser.dbgpparser.header, 'serial_num_array', None) is not None:
                config_serial_num = self._config_parser.root.file_properties.serial_num_list.serial_num
                config_serial_num_count = len(config_serial_num)
                config_serial_num_padded = \
                    list(config_serial_num) + [0] * (SERIAL_NUM_MAX_COUNT - config_serial_num_count)
                image_serial_num = list(dbgpelfparser.dbgpparser.header.serial_num_array)
                image_serial_num_count = dbgpelfparser.dbgpparser.header.serial_num_count
                if config_serial_num_count != image_serial_num_count:
                    reterr.append('Serial Number Array Size mismatch '
                                  '' + ':- \nDebugpolicy File: ' + str(image_serial_num) +
                                  '' + '\nConfig File: ' + str(config_serial_num_padded))
                elif config_serial_num_count > 0 and image_serial_num_count > 0:
                    if set(image_serial_num[:config_serial_num_count]) != set(config_serial_num):
                        reterr.append('Serial Number Array Content mismatch '
                                      '' + ':- \nDebugpolicy File: ' + str(image_serial_num) +
                                      '' + '\nConfig File: ' + str(config_serial_num_padded))
            ...
            if self._config_parser.root.file_properties.root_cert_hash_qti_list is not None:
                if len(self._config_parser.root.file_properties.root_cert_hash_qti_list.root_cert_hash_qti) == 0:
                    ...
                    for i in range(len(self._config_parser.root.file_properties.root_cert_hash_qti_list.root_cert_hash_qti)):
                        val = ensure_str(list(dbgpelfparser.dbgpparser.header.rootcerthash_qti_array)[i].lower())
                        if  val != self._config_parser.root.file_properties.root_cert_hash_qti_list.root_cert_hash_qti[i].lower():
                            reterr.append('Root Certificate Hash QTI Array mismatch '
                                          '' + ':- \nDebugpolicy File: ' + str(dbgpelfparser.dbgpparser.header.rootcerthash_qti_array) +
                                          '' + '\nConfig File: ' + str(self._config_parser.root.file_properties.root_cert_hash_qti_list.root_cert_hash_qti + list([0] * (4 - len(self._config_parser.root.file_properties.root_cert_hash_qti_list.root_cert_hash_qti)))))
                            break
