...
from sectools.features.fuse_common.parsegen.fb_config.defines import FEC_DATA_56BIT_MASK, \
    FEC_IN_ROW, DIFF_FEC_REG, DIFF_FEC_REG_NAZGUL, DIFF_FEC_REG_VT, KP_FEC_REG, \
    DIFF_FEC_REG_NIC, DIFF_FEC_REG_TW, DIFF_FEC_REG_SAI, DIFF_FEC_KONA_HURACAN_FUSION, DIFF_FEC_REG_CYP, DIFF_FEC_MAKENA
...
class _CUSTKEY1(object):
    ...
    def configure(self, data_model):
        ...
        try:
            ...
            encryption_key = ''
            for idx in range(int(len(encryption_key_old)/2)):
...
class _OEM_IMAGE_ENCR_KEY(object):
    ...
    def configure(self, data_model):
        ...
        try:
            ...
            oem_image_encr_key = ''
            for idx in range(int(len(oem_image_encr_key_old)/2)):
            ...
            if self.chipset in FEC_IN_ROW:
                ...
                # set FEC_EN region bit for Region18 (OEM_IMAGE_ENCR_KEY region)
                set_field_value(data_model, region_id='QFPROM_RAW_FEC_EN', field_id='REGION18_FEC_EN', value=0x1)
                ...
                # set FEC_EN region bit for Region20 (OEM_IMAGE_ENCR_KEY region)
                set_field_value(data_model, region_id='QFPROM_RAW_FEC_EN', field_id='REGION20_FEC_EN', value=0x1)
            elif self.chipset in DIFF_FEC_REG_NIC or self.chipset in DIFF_FEC_REG_SAI or self.chipset in DIFF_FEC_REG_CYP:
                # set FEC_EN region bit for Region21 (OEM_IMAGE_ENCR_KEY region)
                set_field_value(data_model, region_id='QFPROM_RAW_FEC_EN', field_id='REGION21_FEC_EN', value=0x1)
            elif self.chipset in DIFF_FEC_KONA_HURACAN_FUSION:
                # set FEC_EN region bit for Region22 (OEM_IMAGE_ENCR_KEY region)
                set_field_value(data_model, region_id='QFPROM_RAW_FEC_EN', field_id='REGION22_FEC_EN', value=0x1)
            elif self.chipset in DIFF_FEC_MAKENA:
                # set FEC_EN region bit for Region30 (OEM_IMAGE_ENCR_KEY region)
