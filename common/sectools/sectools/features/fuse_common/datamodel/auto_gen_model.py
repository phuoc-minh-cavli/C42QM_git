...
class Cfg_Validations(object):
    ...
    def normalize_value(cls, value, bit_length=None):
        ...
        :param int bit_length: The number of bits to use for justifying the
        ...
        if bit_length is not None:
            bit_length = int(cls.get_bit_length(bit_length) / INT_BIT_LEN)
            retval = (retval[:HEX_START_CHARS_LEN] +
                      retval[HEX_START_CHARS_LEN:].rjust(int(bit_length), '0'))
        return retval
...
class Cfg_Fuseblower(Cfg_Base):
    ...
    def _compare(self, other, dummy=False):
        ...
        all_fuse_addresses = list(set(list(fuse_dict_self.keys()) + list(fuse_dict_other.keys())))
    ...
    def compare(self, other, dm_1_name, dm_2_name, dummy=False):
        ...
        :rtype: bool
        """
        mismatching, missing_self, missing_other = self._compare(other, dummy)
        error_string = self._get_compare_error_string(mismatching,
                                                      missing_self,
                                                      missing_other,
                                                      dm_1_name,
...
class Cfg_Fuse(Cfg_Base):
    ...
    def add_field(self, field):
        ...
        :param :class:`Cfg_Field` field: Field to be added to the list
        """
        assert isinstance(field, Cfg_Field)
        field.set_parent_attributes(self.get_value_by_bits,
                                    self.set_value_by_bits)
