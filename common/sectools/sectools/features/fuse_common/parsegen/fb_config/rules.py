...
def set_field_value(data_model, region_id, field_id, value, ignore_missing_value=False):
    '''
...
def delete_field(data_model, region_id, field_id):
    '''
    helper function to remove a field based on region and field name
    '''
    for region in data_model.module[0].fuse_region:
        if region.id == region_id:
            for fuse in region.fuse:
...
def generate_hash_from_file(filepath, hash_algo='sha256'):
    import hashlib
    import os
    ...
    if hash_algo == 'sha384':
        ...
        raise RuntimeError("Hash algorithm provided is not supported" + hash_algo)
    hasher.update(file_data)
    hash_str = hasher.digest()

    return ensure_str(hash_str, encoding="latin-1")
