...
FUSE_REGION_DICT = \
{
    ...
    'QFPROM_RAW_OEM_SPARE'              : 'OEM_SPARE'
}
...
FUSE_OPERATION_DICT = \
{
    ...
    'BLOW_RANDOM'    : 'BLOW_RANDOM'
}


class SecdatParseGen(BaseDataParseGen):

    def __init__(self, feature_type, data=None, dummy=False):
    ...
    def set_data(self, data=None):
        self._secdat_parsegen = ParseGenSecdat(data, dummy=self.dummy)
        if not self.dummy:
    ...
    def write(self, file_path, version, fuse_version=None, hdr_info=None, dummy=False):
        if fuse_version is not None and not dummy:
