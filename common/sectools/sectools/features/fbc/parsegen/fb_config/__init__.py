...
from sectools.features.fuse_common.defines import XML_NAME_END_SECIMAGE
...
class ConfigDir(object):
    ...
    def _get_cpaths(self, chipset_dir, mode):
        ...
        :rtype: (tuple(str))
        """
        oem, qti, ui, user, secimage = None, None, None, None, None
        chipset_from_dir_name = os.path.basename(chipset_dir)

        for entry in os.listdir(chipset_dir):
            path = c_path.join(chipset_dir, entry)
            if c_path.validate_file(path):
                if entry.endswith(XML_NAME_END_SECIMAGE):
                    try:
                        chipset_from_file = SecImageConfigParser(path).chipset
                    except Exception as e:
                        logger.debug2('Skipping file: ' + entry + '\n'
                                      '    ' + 'Failed to load the file: ' + str(e))
                        continue
                ...
                if chipset_from_file == chipset_from_dir_name:
                    if entry.endswith(defines.XML_NAME_END_OEM[mode]):
                        ...
                        user = path
                    elif entry.endswith(XML_NAME_END_SECIMAGE):
                        secimage = path
                    else:
                        logger.debug2('Skipping file: ' + entry + '\n'
                                      '    ' + 'Name does not match any of the naming convention patters')
        ...
        logger.debug2('Config paths found for chipset_dir: ' + chipset_dir + '\n'
                      ...
                      '    ' + 'qti: ' + str(qti) + '\n'
                      '    ' + 'user: ' + str(user) + '\n'
                      '    ' + 'secimage: ' + str(secimage) + '\n')
        return oem, qti, ui, user, secimage
...
class ConfigParser(BaseDataParseGen):
    ...
    @property
    def elf(self):
        """. Returns an object corresponding elf entry in user xml.
        """
        return None if (self._user_config is None or self._user_config.root is None) else self._user_config.root.elf
    ...
    def update(self, dummy=None):
        ...
        for config in [self._ui_config, self._qti_config, self._oem_config, self._user_config]:
            if config is not None:
                ...
                if config is self._user_config:
                    self.data_model = config.update_data_model(self.data_model, dummy)
                else:
                    self.data_model = config.update_data_model(self.data_model)

    def read(self, oem=None, qti=None, ui=None, user=None, dummy=None):
        # Read the files
        self.reset()
