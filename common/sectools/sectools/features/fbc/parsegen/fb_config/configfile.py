...
class _BaseParseGen(with_metaclass(abc.ABCMeta, CoreConfig)):

    __version__ = '1.0'

    def __init__(self, config_path):
        from . import auto_gen_fb_config
...
class _USERParseGen(_BaseParseGen):
    ...
    def __init__(self, config_path):
        from . import auto_gen_fb_user_config
    ...
    def update_data_model(self, data_model, dummy=None):
        entries = []
        ...
        for entry in entries:
            if entry['ignore'] == False:
                if entry['name'] == 'root_cert_file' or entry['name'] == 'root_cert_sha384_hash_file':
        ...
        for entry in entries:
            if entry['ignore'] == False:
                if entry['name'] == 'root_cert_hash0_file' or entry['name'] == 'root_cert_sha384_hash0_file':
                    dt = datautils.DataHandler(datautils.get_cb_dict(path_basepath=os.path.dirname(self.config_path)))
                    entry['value'] = dt.validate(entry['value'], datautils.path_handler.TypePath())
                    pk_hash_set = True
        ...
        if check_for_pk_hash and not pk_hash_set and not dummy:
