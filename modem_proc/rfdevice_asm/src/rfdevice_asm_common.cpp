...
$Header: //components/rel/rfdevice_asm.mpss/1.30/src/rfdevice_asm_common.cpp#1 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/23/19   dh      Fix RFDEVICE_ASM_REG_INVALID data execution issue
06/19/19   dh      Added ASM sleep call during boot-up sequence
06/06/19   bm      KW error fix for is_internal_asm class variable
05/06/19   dh      Added an API to check for internal_asm_device 
01/18/19   dh     Removed re-programming of INIT scripts during Wakeup()
09/25/18   vp      Added support to get RFFE device register address list
08/2/18    dh      Added support for GNSS mode
07/13/18   dh      Implemented wakeup() API
06/15/18   vp      Featurization of WCDMA, CDMA and TDSCDMA code
...
rfdevice_asm_common::rfdevice_asm_common(rfdevice_physical_device *phy_dev_obj_ptr,
    ...
    parent_physical_device_ptr(NULL)
{
  uint8 i = 0;
  uint8 rxtx = 0;
  uint8 swtcnt = 0;
  uint8 rfmds = 0;
  rfdevice_asm_cfg asm_cfg;
  ...
  if((phy_device_info != NULL) && (logical_device_info != NULL) && (logical_device_info->rf_device_type == RFDEVICE_ASM))
  {
        ...
        for(rfmds = 0; rfmds < RFM_MAX_DEVICES; rfmds ++)
    {
      ...
      asm_cfg.trigger_type   = RFDEVICE_ASM_TRIGGER_NONE;
      asm_cfg.mode           = RFM_LTE_MODE;
      asm_cfg.band.lte_band  = RFCOM_NUM_LTE_BANDS;

      sleep(&asm_cfg, NULL, RFDEVICE_EXECUTE_IMMEDIATE, 0);
...
)
{
  ...
  case RFCOM_GNSS_MODE:
   ...
   if(band.gnss_band > RFCOM_BAND_GNSS_MAX)
   {
      RF_MSG_1( RF_ERROR, "Invalid GNSS band %d", band.gnss_band);
      return FALSE;
   }
   break;
  default:
                                    ...
                                    int16 script_timing)
{
  ...
  if(!rfdevice_asm_param_validity(asm_cfg_p->mode, asm_cfg_p->band, script_buffer, execution_type, script_timing))
  {
    RF_MSG_3(RF_ERROR, "Wakeup ASM invalid Params usid=0x%x asm_cfg_p->mode %d, band %d",
             this->assigned_usid,asm_cfg_p->mode, asm_cfg_p->band.lte_band);
    return status;
  }
  ...
  asm_cfg.req = RFDEVICE_ASM_WAKEUP_DATA;
  /* use port 0 settings because wakeup script is the same for all ports*/
  asm_cfg.port = 0;
  status = asm_data_ptr->settings_data_get(&asm_cfg, &asm_settings);
  ...
  else
  {
    ...
    RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_ERROR, "No Wakeup script support! (mode=%d, band=%d, timing=%d",
                       asm_cfg_p->mode, asm_cfg_p->band.lte_band, script_timing);
  return TRUE;
}
  ...
  if (status == FALSE)
  {
    ...
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,"Failed to wakeup ASM! (mode=%d, band=%d, timing=%d)",
          asm_cfg_p->mode, asm_cfg_p->band.lte_band, script_timing);
  }
    ...
    uint8 *num_trans)
{
  ...
  for (uint8 i=0; i < asm_settings->num_regs; i++)
  {
    ...
    if (RFDEVICE_ASM_REG_INVALID != asm_settings->data[i])
    {
      ...
      rffe_script[script_idx].data = (uint8)asm_settings->data[i];
                                      ...
                                      int32 *data_ptr, uint8 size)
{
  ...
  case RFCOM_GNSS_MODE:
   if (band.gnss_band < RFCOM_BAND_GNSS_MAX)
   {
     this->gnss_band_port[band.gnss_band] = port;
   }
   break;
  default:
    break;
                                    ...
                                    rfcom_band_type_u band, rfdevice_asm_rxtx_enum_type rxtx)
{
  ...
  case RFCOM_GNSS_MODE:
   if (band.gnss_band < RFCOM_BAND_GNSS_MAX)
   {
     port = this->gnss_band_port[band.gnss_band];
   }
   break;
...
/*----------------------------------------------------------------------------*/
/*!
  @brief get rffe registers adrress list for ASM

  @details
  This function is used to update rffe register list for ASM 
*/
void rfdevice_asm_common::update_phy_dev_reg_list(void)
{
  ...
  ========================================================== */
   ...
   {
      RF_MSG(RF_ERROR, "rfdevice_asm_common::update_phy_dev_reg_list(): parent_physical_device_ptr is NULL");
      return;
   }
  ...
  {
    RF_MSG(RF_ERROR, "rfdevice_asm_common::update_phy_dev_reg_list(): rffe_reg_addr_list or reg_addr_occur_table is NULL");
  ...
  /* ======[ Validity Check ]===================================================*/
  if (NULL == this->asm_data_ptr)
  {
    RF_MSG(RF_ERROR, "rfdevice_asm_common::update_phy_dev_reg_list(): NULL data pointer. Returning FALSE.");
  ...
  for (uint16 script_req = 0; script_req < RFDEVICE_ASM_DATA_REQ_INVALID; script_req++)
  {
    asm_cfg.req = (rfdevice_asm_data_req_enum_type)script_req;
    setting_data_get_status = asm_data_ptr->settings_data_get(&asm_cfg, &asm_settings);

    /* Not all logical devices have all of the request types, if the status is False, just perform skip */
    if (setting_data_get_status)
    {
      for (uint16 j = 0; j < asm_settings.num_regs; j++)
      {
        // Skip invalid register address 0xFF and 0xFFFF
        if (asm_settings.addr[j] == 0xFF)
        {
          continue;
        }
        if (parent_physical_device_ptr->reg_addr_list_index >= RFDEVICE_PHY_DEV_REG_LIST_SIZE)
        {
          RF_MSG_1(RF_ERROR, "rfdevice_asm_common::update_phy_dev_reg_list(): "
                   "asm %d too many rffe register addresses. Need larger buffer ",
                   this->rf_device_id);

          parent_physical_device_ptr->reg_addr_dump_status = RFDEVICE_REG_ADDR_ERROR_FATAL;
          return;
        }
        if (asm_settings.addr[j] < RFDEVICE_PHY_DEV_REG_FLAG_LIST_SIZE)
        {
          if (parent_physical_device_ptr->reg_addr_occur_table[asm_settings.addr[j]] == FALSE)
          {
            /* To remove duplicates for address from 0x00 to 0xFF, set the visited address to TRUE */
            parent_physical_device_ptr->reg_addr_occur_table[asm_settings.addr[j]] = TRUE;

            /* Store the current register address to the physical device class */
            parent_physical_device_ptr->rffe_reg_addr_list[parent_physical_device_ptr->reg_addr_list_index++] = asm_settings.addr[j];
          }
        } /*if (asm_settings.addr[j] < RFDEVICE_PHY_DEV_REG_FLAG_LIST_SIZE)*/
...
void rfdevice_asm_common::update_internal_asm_flag()
{
  if(this->mfg_id == 0x217 && this->prd_id == 0xC7)
{
    this->is_internal_asm = TRUE;
  }
}
...
/*!
  @brief
  interface function for check if ASM device is internal
 
  @details
  Checks if ASM device used is internal (IOTFE) or external

  @return
  boolean indicating internal or external device
*/
boolean rfdevice_asm_common::is_internal_asm_device()
{
   return this->is_internal_asm;
}
