...
$Header: //source/qcom/qct/modem/fw/components/fw_lte.mpss/rel/15.2/api/intf_ul.h#1 $
...
typedef struct
{
   ...
   uint16  pusch_scrambler_init_helper; //ML1 to provide FW the computed value of ((j0+j)Nacc mod 10) for CATM
...
/*! @brief structure for holding CATM related parameters from ML1*/
typedef struct 
{
  uint8          starting_rb_k0;    ///< start RB of allocation counted across the entire system bandwidth
  uint8		     num_rb;       		///< no. of RBs allocated
  uint8		     start_rb_nb_idx;  	///< start RB of the 6 RB narrowband region
  uint32         num_repetition;    ///< Repetition for UL only for logging
  boolean        is_repetition_pending;  ///< enabled if there are pending repetitions in UL CATM
  boolean        prach_back_to_back;  // ML1 set to true if PRACH is back to back
  uint8          subrame_num_cqi_rep_start;  // ML1 set to valid subframe num for CSF payload peek
  boolean        partial_overlap_acknak_cqi_rep; //ML1 set this to true if acknak cqi rep overlap detected
  uint8          acknak_dlsf_offset; //ML1 to send SF no. diff between UL bit mask set to DL for which acknak to be sent >=4
}lte_LL1_ul_catm_params_struct;
...
typedef struct 
{
   ...
   lte_LL1_ul_catm_params_struct      catm_params;   ///< catm parameters from ML1
