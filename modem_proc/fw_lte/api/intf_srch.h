...
$Header: //source/qcom/qct/modem/fw/components/fw_lte.mpss/rel/15.2/api/intf_srch.h#1 $
...
typedef enum
{
  LTE_LL1_SRCH_NORMAL_CP = 0,  // Normal CP
  LTE_LL1_SRCH_EXTENDED_CP,    // Extended CP
  LTE_LL1_SRCH_EXTENDED_UNKNOWN,// ML1 fills with this when it doesn't know CP type
} lte_LL1_srch_cp_type_enum;
...
/* @ML1 to indicate FW on merging/combining PSS and SSS metrics across HFs */
typedef enum
{
  LTE_LL1_NB_SRCH_SAVE_LIST_1=0,
  LTE_LL1_NB_SRCH_SAVE_LIST_2,
  LTE_LL1_NB_SRCH_SAVE_LIST_3,
  LTE_LL1_NB_SRCH_MERGE_FINAL,
  LTE_LL1_NB_SRCH_NO_MERGE,
}lte_LL1_catm_nb_srch_save_combine_merge_list_enum;
...
typedef struct
{
  ...
  uint16 max_hf;                          ///<uint16, range: 1 to 256, default:4, Maximum number of half frames to use for
  ...
  lte_LL1_srch_cp_type_enum    target_cell_cp_type;///< target cell ID cp type

  lte_LL1_fscan_mode_enum fscan_mode;      ///<fscan mode 

  boolean modified_srch;                   ///< 1hf_initacq search: flag should be true and max_hf = 1 for 1HF SRCH 
                                           ///< Legacy IA SRCH : flag should be false and max_hf = any btwn 1 to 8
                                           ///< FW will do search for max_hf+1 no. of HFs as FW does DIAG loading
                                           ///< in first HF.
  lte_LL1_nb_processing_type_enum  srch_proc_type; ///< Enum to clarify different Ncell Srch processing type 
  boolean isModeB;                         ///< Enum to differentiate between mode A and mode B
  boolean early_term_en_flag;///< flag to enable early termination for mode A
...
typedef struct {
  ...
  boolean short_srch_window;         /// Boolean to flag short versus long intra-frequency ncell search.  
                                     /// True: Use optimized srch window
                                     /// False: Use the 5ms SRCH window for 1HF and 10ms (non-contiguous) for 2HF. 
                                     /// Default value : False (for legacy SRCH)

  uint8 short_srch_num_sym;          /// short_srch_num_sym is only valid for the optimized (short) intra-frequency SRCH. 
                                     /// In this case search window will be 
                                     /// [start_of_PSS_symbol-short_srch_num_sym*SYM_LEN, end_of_PSS_symbol+short_srch_num_sym*SYM_LEN], 
                                     /// where SYM_LEN = ECP Symbol Length (1920 samples/ 12 symbols = 160).
                                     /// Since symbol duration of NCP < ECP, picking the longer symbol duration for uniform design.
                                     /// Default value : 0 (for legacy SRCH) 
                                     /// Min value = 5
  uint8 merge_combine_type;          /// Is only valid for CatM, it will indicate the FW to merge or combine the PSS or SSS metrics
                                     /// to be combined across multiple 1HF NB srch
  lte_LL1_nb_processing_type_enum  srch_proc_type; ///< Enum to clarify different Ncell Srch processing type 
  uint16 current_hf_num;             /// To help combining the HF results across connected mode gaps.
                                     /// When current_hf_num=0 reset all the pss non coherent buffers.
  boolean isModeB;                   ///< Enum to differentiate between mode A and mode B
...
typedef struct {
  ...
  uint8 total_combined_merged_hfs;        ///<the number of HFs combined/merged only used in connected mode
  lte_LL1_nb_processing_type_enum  srch_proc_type; ///< Enum to clarify different Ncell Srch processing type 
