...
$Header: //source/qcom/qct/modem/fw/components/fw_lte.mpss/rel/15.2/api/intf_dl.h#1 $
...
/* Number of SIBs in CAT-M. TBD - Need confirmation and update */
#define LTE_LL1_DL_MAX_NUM_SI 8
/*===========================================================================
...
typedef enum
{
  ...
  LTE_LL1_DL_CH_MPDCCH,
  LTE_LL1_DL_CH_MPDSCH,
...
typedef enum
{
  ...
  LTE_LL1_CCE_AGG_LVL_16,
  LTE_LL1_CCE_AGG_LVL_24
...
typedef enum
{
  ...
  LTE_LL1_CCE_CM1_SRCH_SPACE,
  LTE_LL1_CCE_CM2_SRCH_SPACE
...
typedef enum
{
  LTE_LL1_DL_HALFDUPLEX_RE_ENABLE = 0,
  LTE_LL1_DL_HALFDUPLEX_DISABLE = 1
}lte_LL1_dl_halfduplex_disable_enum;

typedef enum
{
  LTE_LL1_DL_NBHOP_TUNE_DISABLE = 0,
  LTE_LL1_DL_NBHOP_TUNE_ENABLE = 1
} lte_LL1_dl_nbhop_tune_enum;
...
typedef enum
{
 ...
 LTE_LL1_PAGE_NB_T_OVER_64,
 LTE_LL1_PAGE_NB_T_OVER_128,
 LTE_LL1_PAGE_NB_T_OVER_256,
 NB_MAX
...
typedef enum
{
    LTE_LL1_MPDCCH_MODE_LOCALIZED = 0,
    LTE_LL1_MPDCCH_MODE_DISTRIBUTED,
    LTE_LL1_MPDCCH_MODE_MAX
} lte_LL1_mpdcch_mode_enum;

typedef enum
{
    LTE_LL1_MPDCCH_SRCH_SPACE_TYPE0CSS_OR_UESS = 0,
    LTE_LL1_MPDCCH_SRCH_SPACE_TYPE1CSS,
    LTE_LL1_MPDCCH_SRCH_SPACE_TYPE2CSS,
    LTE_LL1_MPDCCH_SRCH_SPACE_MAX
} lte_LL1_mpdcch_srch_space_enum;

typedef enum
{
    LTE_LL1_CATM_CE_MODE_A = 0,
    LTE_LL1_CATM_CE_MODE_B,
    LTE_LL1_CATM_CE_MODE_MAX
} lte_LL1_catm_ce_mode_enum;
...
typedef struct
{
  uint64                      dl_subfn_bitmap;        ///< 40 bit bitmap
  boolean                     dl_bitmap_present;      ///< indication if dl_subfn_bitmap is present
} lte_LL1_dl_bitmap_info_struct;
// --------------------------------------------------------------------------
//
...
typedef struct
{
  /*! Boolean to indicate if PBCH repetition was detected in Cat M1 mode */
    boolean                          pbch_repetition_detected;
} lte_LL1_dl_pbch_cfg_struct;

typedef struct 
{
    boolean                             is_mpdcch_config;       ///< set to '0' if the params are not to be parsed by FW
    boolean                             search_space_enable;    ///< enable or disable
    boolean                             freq_hop_enabled;       ///< freq hop enabled
    boolean                             resume_type1_ss;        /// resume Type1SS after early termination
    lte_LL1_mpdcch_srch_space_enum      srch_space_type;        ///< type0Common/UESS, Type1Common, type2Common
    lte_LL1_catm_ce_mode_enum           ce_mode;                ///< A/B
    lte_LL1_mpdcch_mode_enum            mpdcch_mode;            ///< local it disctributed
    uint8                               num_prb_mpdcch;         ///< 1-6
    uint8                               combinatorial_index_r;  ///< prb allocation
    uint32                              k_0;                    ///< startsubfn in Q1 format corresponding to chosen search space. for type0/2 this is 'G', for type1 this is PO
    uint32                              r_max;                  ///< repetition param for  chosen search space
    uint32                              n_id_mpdcch;            ///< scrambling seq init
    uint32                              rnti_mask;              ///< RNTIs should agree with search space type
    uint32                              n_nb_idx;               ///< Hop param (starting nb). This is slightly different for P-RNTI (See spec)
    uint32                              n_dl_ch;                ///< hopping param
    uint32                              n_dl_ch_hop;            ///< hopping param
    uint32                              f_dl_hop;               ///< hopping param
    uint16                              c_rnti;                 ///< connected state
    uint16                              sps_c_rnti;             ///< connected state
    uint16                              p_rnti;                 ///< paging 
...
} lte_LL1_dl_mpdcch_dyn_cfg_struct;


typedef struct
{
    uint8                       start_ofdm_sym;         ///< in lieu of CFI
    uint32                      n_scid_mpdcch;          ///< scrambling seq init
    uint8                       prb_set_idx;            ///< 0/1 prb set idx
    uint32                      csi_num_rep_ce_e;       ///< number of CSI repetitions

} lte_LL1_dl_mpdcch_stat_cfg_struct;

typedef struct
{
    lte_LL1_dl_mpdcch_dyn_cfg_struct         mpdcch_dyn_cfg;
    lte_LL1_dl_mpdcch_stat_cfg_struct        mpdcch_stat_cfg;
} lte_LL1_dl_mpdcch_cfg_struct;


typedef struct
{
    lte_LL1_catm_ce_mode_enum   ce_mode;                            ///< A/B
    uint32                      max_num_repetition_ce_mode_A;       ///< pdsch repetition for CE modeA
    uint32                      max_num_repetition_ce_mode_B;       ///< pdsch repetition for CE modeB
    boolean                     freq_hop_enabled;                   ///< freq hop enabled
	boolean                     is_ce_pdsch_ten_processes;          ///< is_ce_pdsch_ten_processes Rel-14 10-HARQ
	boolean                     is_ce_modulation_order_override;     /// modulation override  FR 44939: Rel.14 Cat.M1 VoLTE enhancements: new set of PUSH repetition factors
	boolean                     is_ce_SchedulingEnhancement;        // Indicates whether the UE supports dynamic HARQ-ACK delay for HD-FDD in CE mode A as specified in TS 36.212
	boolean                     is_ce_HARQ_AckBundling;             //Indicates whether the UE supports HARQ-ACK bundling in half duplex FDD in CE mode A, as specified in TS 36.212 [22] and TS 36.213 [23].
	uint8                       HARQ_AckDelayRange;                 // Indicates the range to be used to calculate ack delay from DCI field HARQ-ACK delay when is_ce_SchedulingEnhancement is enabled
    uint32                      n_dl_ch;                            ///< hopping param
    uint32                      n_dl_ch_hop;                        ///< hopping param
    uint32                      f_dl_hop;                           ///< hopping param
    lte_LL1_dl_pdsch_params_struct  pdsch_param;                    ///< common (legacy and Cat-M)
} lte_LL1_dl_mpdsch_cfg_struct;
...
typedef struct {
  ...
  uint16 channel_config_mask; ///< each bit corresponding to one DL_CH_CONFIG_TYPE,
                             ///< refer to lte_LL1_dl_ch_config_type_enum.
                             ///< "1" means corresponding config is valid.
                             ///< 8 bits not sufficent - expand to 16 for CATM1
  ...
  // CAT-M1 related
  lte_LL1_dl_pbch_cfg_struct            pbch_cfg;        //signalled via LTE_LL1_DL_CH_PBCH bit in channel_config_mask
  lte_LL1_dl_mpdsch_cfg_struct          mpdcsh_cfg;        //signalled via LTE_LL1_DL_CH_MPDSCH bit in channel_config_mask
  lte_LL1_dl_mpdcch_stat_cfg_struct     mpdcch_stat_cfg;   //signalled via LTE_LL1_DL_CH_MPDCCH bit in channel_config_mask
  lte_LL1_dl_mpdcch_dyn_cfg_struct      mpdcch_dyn_cfg;    //signalled via LTE_LL1_DL_CH_MPDCCH bit in channel_config_mask
  lte_LL1_dl_bitmap_info_struct         dl_bitmap_cfg;     //required always regardless of LTE_LL1_DL_CH_MPDSCH or LTE_LL1_DL_CH_MPDCCH
...
typedef struct {
  ...
  // CAT-M1 related
  lte_LL1_dl_mpdcch_dyn_cfg_struct      mpdcch_dyn_cfg;
  boolean                               hold_rnti_cnf_cmd; //This flag is introduced to handle certain Corener Scenario when ML1 assumes there is no grant and by the time it gets the abort request from the RRC 
                                                           //and based on it changes the CLOCK from NOM to SVS and results in TDEC stall (as FW might be decoding PDSCH from the grant which came after the RRC connection abort request to ML1)
  
  boolean 								pdsch_early_termination_flag; // This flag is introduced to handle PDSCH early termination command from ML1. When FW receives the RNTI_disable 
																	  // rqst from ML1 with this flag set, FW terminates any further scheduling of PDSCH and waits for completion of 
																	  // already scheduled PDSCH before sending a RNTI change confirmation.
																	  // This is being introduced to synchronize ML1 and FW for stability and robustness.
																	  // It is important that ML1 is aware of the PDSCH acitivity ongoing in FW so that it can
																	  // issue TDEC CLK change to IDLE SVS CLK or sleep without causing any TDEC stall.
																	  // We can further leverage this flag to introduce power saving feature in future.
...
/*! @brief DL half duplex gap request input structure
    @detail dl_disable=1 at start of ul gap, =0 when resuming dl.
  */
typedef struct {
  lte_LL1_req_header_struct req_hdr;
  lte_LL1_dl_halfduplex_disable_enum dl_disable;
} lte_LL1_dl_halfduplex_disable_req_struct;

LTE_LL1_INTF_MSG( dl_halfduplex_disable_req );

/*! @brief NB Hop request input structure
    @detail dl_disable=1 at start of ul gap, =0 when resuming dl.
  */
typedef struct {
  lte_LL1_req_header_struct req_hdr;
  lte_LL1_dl_nbhop_tune_enum hop_to_center;
} lte_LL1_dl_nbhop_tune_req_struct;

LTE_LL1_INTF_MSG( dl_nbhop_tune_req );
...
typedef struct {    
  lte_LL1_req_header_struct          req_hdr;
  boolean cancel_pending_fmd_mode_req; // 1--> to cancel the pending idle_mode_fmd request CNF from FW to ML1
                                       // 0 --> default mode(FW will send the CNF to ML1)
}lte_LL1_idle_mode_doppler_estimate_req_struct;

typedef struct 
{
  msgr_hdr_struct_type               msg_hdr;
  lte_LL1_idle_mode_doppler_estimate_req_struct          msg_payload;
}lte_LL1_idle_mode_doppler_estimate_req_msg_struct;
...
typedef struct { 
  lte_LL1_dl_cnf_enum err_code;
  lte_LL1_carrier_type_enum carrier_type;  ///< Enum for dl carriers in  CA
  boolean  pdsch_early_termination_flag;   // FW will send the ET flag that it received as a part of disable rqst in the CNF msg now
  boolean  hold_rnti_cnf_cmd;              // FW will send the HOLD_RNTI flag that it received as a part of disable rqst in the CNF msg now
  uint32   rnti_mask;                      // FW will send the rnti_mask that it received as a part of disable rqst in the CNF msg now
...
/*! @brief DL half duplex gap confirm input structure */
typedef struct {
  msgr_hdr_struct_type                      msg_hdr;      ///< message router header
} lte_LL1_dl_halfduplex_disable_cnf_msg_struct;

/*! @brief nb hop tune confirm input structure */
typedef struct {
  msgr_hdr_struct_type                      msg_hdr;      ///< message router header
} lte_LL1_dl_nbhop_tune_cnf_msg_struct;

/*! @brief structure for SI info confirmation */
...
/*! @brief Structure for idle mode doppler est. request confirmation */
typedef struct
{
  lte_LL1_cnf_header_struct cnf_hdr;
  uint16 frame_num;
  uint16 sub_frame_num;
  uint32 doppler_estimate; 
} lte_LL1_idle_mode_doppler_est_struct;

/*! @brief Confirmation for idle mode doppler est. request confirmation */
typedef struct
{
  msgr_hdr_struct_type          			  msg_hdr;                                          
  lte_LL1_idle_mode_doppler_est_struct        msg_payload;
}lte_LL1_dl_idle_mode_doppler_est_cnf_msg_struct;
...
typedef struct { 
  ...
  boolean                                   pdsch_skip;
  uint8                                     harq_id;
  uint8                                     harq_ack_resource_offset;
  uint8                                     harqack_Delay;
  boolean                                   bundling_en;
  uint8                                     bundle_id;
  uint8                                     bundled_tb_idx;
  uint8                                     bundle_size;
  ...
  boolean                                   pdcch_ind_new_tx[2];

} lte_LL1_dl_pdcch_ind_info_struct;

typedef struct { 
  boolean   valid ;                     // contents of this struct are valid if set to true.
  uint8     direct_ind_info;            // field from the dci for paging.  values defined in table 6.5-1 of 36.331
  uint16    dci_subframe_repetition;    // repetition level for mpdcch� indicated by the "dci subframe repetition number" field in dci 6-1
  uint16    pdsch_repetition;           // repetition level for pdsch indicated by the "repetition number" field in dci 6-1
  uint16    mpcch_rep_end_frame ;      // end frame of mpdcch repetition.
  uint16    mpcch_rep_end_subframe ;   // end subframe  of mpdcch repetition.
  uint16    pdsch_rep_end_frame ;      // last frame of pdsch repetition.
  uint16    pdsch_rep_end_subframe ;   // last subframe  of pdsch repetition.
  }  lte_LL1_dl_mpdcch_ind_struct ;

  /*! @brief Page MPDCCH decode result */
  typedef enum
  {
    LTE_LL1_TYPE1_SS_INCOMPLETE = 0,
    LTE_LL1_TYPE1_SS_COMPLETE,
  } lte_LL1_type1_ss_decode_status_enum;
...
typedef struct {    
  ...
  lte_LL1_dl_mpdcch_ind_struct              mpdcch_ind;
  lte_LL1_type1_ss_decode_status_enum       type1_ss_status;// mpdcch early termination
} lte_LL1_dl_pdcch_phich_ind_struct;

typedef struct {
lte_LL1_ind_header_struct                 header;   
uint8  num_carriers;
lte_LL1_dl_pdcch_phich_ind_struct   carrier[LTE_LL1_DL_NUM_CARRIERS_2];
...
/*!@brief CATm PDSCH status struct 
*/ 
typedef struct {
  uint8   overall_crc_status;   ///<  1 = This repetition bundle has passed.
  uint16  pdsch_rep_end_frame ;      ///< last frame of pdsch repetition.
  uint8   pdsch_rep_end_subframe ;   ///< last subframe  of pdsch repetition.

  uint16  repetition_counter;     ///<  Repetition counter from 0 to max-1. Note counter can jump when half-duplex.
  uint16  repetition_value;       //< Max repetition for this bundle.
  uint8   discard_status;          ///< TBD
  uint8   nb_idx;                ///< Narrow Band index for this pdsch decode
  boolean bundling_enable;       //Bundling enable/disable 0/1
  uint8   delay_value;           //Delay absolute value , 0 if disabled
  uint8   bundled_ack_nak;       //Bundled ack/nak = 2 if bundling disabled,0 if not end_tb_of_bundle
  boolean end_tb_of_bundle;
}lte_LL1_dl_pdsch_catm_status_struct;
...
typedef struct {
  ...
  lte_LL1_dl_pdsch_catm_status_struct catm_stat; ///< catm side info.
...
{
  msgr_hdr_struct_type                  msg_hdr;
}lte_LL1_dl_ftl_snr_drop_ind_msg_struct;
...
typedef struct 
{
  ...
  boolean disable_rep_decoding;  ///< if TRUE, no PBCH decoding on repetition subframes
...
typedef struct
{
  ...
  boolean rep_decoding_result;  ///< if TRUE, decoding on repetition sf attempted and successful
...
typedef struct
{
  lte_LL1_cnf_header_struct cnf_hdr; /* message header */
} lte_LL1_dl_si_info_update_cnf_struct;
LTE_LL1_INTF_MSG(dl_si_info_update_cnf);
typedef struct
{
  uint8   num_sib1br_rep;                     ///<  No. of SIB1BR repetitions
  uint16  nrSibs; //ML1 parameter
  uint16  si_WindowLength_BR; //ML1 parameter In terms of miliseconds.
  uint16  si_RepetitionPattern;// Ml1 parameter
  uint16  si_Periodicity[LTE_LL1_DL_MAX_NUM_SI];//ML1 parameter
  uint8   start_nb_idx[LTE_LL1_DL_MAX_NUM_SI]; //ML1 parameter first SIBx NarrowBand
  
  //hopping for SIB
  boolean freq_hop_enabled; //ML1 parameter
  uint32  n_dl_ch; //ML1 parameter
  uint32  n_dl_ch_hop; //Ml1 Parameetr
  uint32  f_dl_hop; //ML1 Parameter

} lte_LL1_dl_si_info_struct;

/* DL SIB info update req */
typedef struct
{
  lte_LL1_req_header_struct req_hdr; /* message header */
  lte_LL1_dl_si_info_struct si_info; /* SIB info payload */
} lte_LL1_dl_si_info_update_req_struct;

LTE_LL1_INTF_MSG(dl_si_info_update_req);

typedef struct
{
  uint16  sfn;                
  uint8   subfn;
  boolean cancelPrevDldis;
} lte_LL1_ULtime_s;


/*===========================================================================

            LL1 -> ML1 Function Calls

===========================================================================*/

/*-------------------------------------------------------------------------
 * FUNCTION:  lte_ml1_ll_get_tx_subfn
 *-----------------------------------------------------------------------*/
/*!
  @brief
   Call from PDCCH context.. Call the ML1 Grant Manager function 
   directly in LL1 context to determine if or not a TX has to be scheduled
   for the UL grant received

  @detail

  @return
  SFN and subfn if TX has to be scheduled, else INVALID VAL for both SFN and subfn
*/

/* Note :  lte_l1_cell_systime_s is already defined in ML1 header at /modem/lte/api/lte_l1_types.h */
lte_LL1_ULtime_s lte_ml1_gh_ll_get_tx_subfn(uint64 mpdcch_payload, uint16 payload_sz, lte_LL1_cce_search_space_enum search_space, lte_l1_cell_systime_s pdcch_timing);
