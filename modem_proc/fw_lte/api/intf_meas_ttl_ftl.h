...
$Header: //source/qcom/qct/modem/fw/components/fw_lte.mpss/rel/15.2/api/intf_meas_ttl_ftl.h#1 $
...
*/
#define LTE_LL1_MAX_NCELLS_PER_REQ  8
...
typedef enum {
  ...
  LTE_LL1_TTL_SCELL_CONFIG_EPIR_TTL_TE_THRESHOLD_DISABLE,
                                                          ///< EPIR TTL threshold value check
                                                          /// 0 => Enable the check
                                                          /// 1 => Disable the check
...
/*!
    @brief
    Parameters provided by ML for DRX mode. These values are determistic. 
    They can be determined on receipt of message
*/
typedef struct
{
  ...
  uint8     drx_num_ms;     ///< Number of SF's of SNE used per neighbor cell to compute measurement results
  lte_LL1_meas_tx_mode_e    meas_tx_mode; ///< 0: average over 2 tx; 1 report tx0 2 report max tx0 and tx1
} lte_LL1_meas_ttl_ftl_ncell_drx_struct;
