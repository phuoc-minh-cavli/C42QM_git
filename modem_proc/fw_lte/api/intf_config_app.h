...
$Header: //source/qcom/qct/modem/fw/components/fw_lte.mpss/rel/15.2/api/intf_config_app.h#1 $
...
/* Changes for online DRX aGc */
  ...
  boolean                       cfg_without_retune;//default=FALSE, to be set based on RF tune done by ML1
...
typedef struct
{
  ...
  uint64                 sys_time_resume_elapsed_time;///< elapse time in Ts units for
                                                      ///< use with sys time resume
													  
  lte_LL1_ue_rf_ftm_mode_e  ue_rf_mode; ///< FTM mode indication (for RFCal)
  // NOTE: ML populates this with NOT_FTM=1 ALWAYS apart from NS mode.
  // RFSW implicitly sets FTM=1 by memsetting
  ...
  lte_LL1_ue_sleep_type_e sleep_type;  
  boolean is_cdrx_wakeup; // Flag will be set by ML1 on cdrx wakeup from deep sleep
