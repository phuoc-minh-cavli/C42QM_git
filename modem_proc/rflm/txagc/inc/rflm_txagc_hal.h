...
/*---------------------------------------------------------------------------*/
typedef void
(*rflm_txagc_fixup_rffe_group_fp)
(
  uint32 *des_ptr,
  rflm_txagc_hal_u32_u8_t *src_ptr,
  const rflm_txagc_fixup_data_t *fixup_ptr,
  uint32 num_trans,
  uint8 num_groups,
  uint8 grp_start_index,
  int8 *num_bytes_per_group,
  int8 *num_tokens_per_group,
  uint16 *address_per_group,
  rflm_ccs_rf_transaction_rffe_ext_rw_t *rffe_ext_data_ptr,
  uint32 action_time,
  uint8 data_ptr[][RFLM_TXAGC_GROUP_DATA_SIZE],
  rflm_txagc_group_token_type token_ptr[][RFLM_TXAGC_GROUP_TOKEN_SIZE]   
);
