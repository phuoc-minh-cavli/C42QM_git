...
$Header: //components/rel/rflm.mpss/1.10/services/diag/inc/rflm_diag_log.h#1 $
...
#define RFLM_DIAG_LOG_INVALID_CONTEXT_ID  (-1)
...
typedef struct
{
    ...
    uint32 *data;                                   // pointer to log payload
...
typedef struct
{
    ...
    void   *data_alloc_ptr;
    uint16 logBufSize;            // size of log data buffer in 32-bit words
...
extern int rflm_diag_allocate_context_custom_size(
                                      uint16 diag_packet_id, 
                                      uint16 rat, 
                                      uint16 user_tag_size,
                                      uint16 packet_size_bytes);
