...
/*==============================================================================
  ...
  Copyright (c) 2015 - 2023 Qualcomm Technologies, Inc. (QTI). All Rights Reserved
...
$Header: //components/rel/rflm.mpss/1.10/api/fw/rflm_api_lte.h#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/17/23   hd      Added rflm_lte_notch_control() API
07/13/20   gk      Added API(NB-IOT) to get lna offsets for all gains
09/18/18   as      Added support for 5MHz DL Hopping in CATM1
...
11/29/16   cv      Removed rotation programming 
11/11/16   qzh     Add CATM1 ul hopping related changes
07/12/16   qzh     Update zero out intf with FW
10/26/16   as      Update in Rx and Tx NPLER settings for CAT-M1
10/26/16   as      Clean up the code to remove duplicate APIs which are not being called by FW
...
#define RFLM_LTE_MAX_NUM_CARRIER                     1
...
#define RFLM_LTE_EXECUTE_TUNE_SCRIPTS_PRX_HANDLE       RFLM_LTE_EXECUTE_TUNE_SCRIPTS_TX0_HANDLE + 1
#define RFLM_LTE_EXECUTE_TUNE_SCRIPTS_MAX_HANDLE       RFLM_LTE_EXECUTE_TUNE_SCRIPTS_PRX_HANDLE + 1

#define RFLM_LTE_SUBCARRIER_SPACING_KHZ             15 /*LTE subcarrier spacing 15Khz*/
...
typedef enum
{
    ...
    RFLM_LTE_BW_20         = 5,
    RFLM_LTE_BW_40         = 6,
    RFLM_LTE_BW_20MHz_plus_20MHz = 7,  /*BW type for intra-band ULCA*/
    RFLM_LTE_NUM_OF_BW  
}rflm_lte_bw_type_s;
...
typedef enum
{
    RFLM_LTE_NB_CHAN_FORMAT_PUSCH_FORMAT1      = 0,
    RFLM_LTE_NB_CHAN_FORMAT_PUSCH_FORMAT2      = 1,
    RFLM_LTE_NB_CHAN_FORMAT_PRACH              = 2,
    RFLM_LTE_NB_NUM_OF_CHAN_FORMAT 
}rflm_lte_nb_chan_format_e;
...
typedef enum
{ 
  RFLM_LTE_NB_REGION_0 = 0,
  RFLM_LTE_NB_REGION_1,
  RFLM_LTE_NB_REGION_2,
  RFLM_LTE_NB_REGION_3,
  RFLM_LTE_NB_REGION_4,
  RFLM_LTE_NB_REGION_MAX
} rflm_lte_nb_region_type;
...
typedef struct
{
   uint32 symbol_start_txc_offset;//t2`
}rflm_catm_timestamp_params_s;
...
/*! @brief rflm_lte_update_tx_sf_input_s is the input from FW to schedule TXAGC 
  ...
  -	rflm_lte_pwr_db10: Desired Tx Power in dB10 unit.  
  ...
  - rflm_catm_timestamp_params: CATM1 specific timestamp parameters 
  - prach_repetition: Flag to indicate back to back PRACH repetition
*/
typedef struct
{
  ...
  rflm_catm_timestamp_params_s   rflm_catm_timestamp_params;
  boolean                        prach_repetition;
...
typedef struct
{
  int16     rx_gain_offset[RFLM_CMN_RXAGC_MAX_LNA_STATES]; // unit is dB/10
} rflm_lte_get_all_lna_settings_return_s;
...
typedef struct
{
  ...
  int32                     action_time;
  rflm_handle_rx_t          handle_id[RFLM_LTE_MAX_NUM_RX_PATHS];   
} rflm_lte_rx_toggle_pll_sigpath_input_s;
...
{
  uint8                     rx_path;
  rflm_lte_nb_region_type   nb_region;
  int32                     action_time;
  rflm_handle_rx_t          handle_id;
} rflm_lte_rx_nb_retune_input_s;
...
typedef struct
{
  ...
  int32                       action_time;
  boolean                     carrier_retune;
  uint8                       carrier_type;
  rflm_handle_rx_t            handle_id;
} rflm_lte_fs_retune_input_s;
...
rflm_err_t rflm_lte_get_all_gain_settings
( 
   rflm_handle_rx_t handle_id,
   rflm_lte_get_lna_settings_input_s  *lna_settings_info,
   rflm_lte_get_all_lna_settings_return_s *total_gain_offset
);
...
rflm_err_t rflm_lte_rx_toggle_pll_and_sigpath
(
  rflm_lte_rx_toggle_pll_sigpath_input_s *toggle_rx_pll_sigpath_info
);

rflm_err_t rflm_lte_rx_nb_retune(rflm_lte_rx_nb_retune_input_s *rx_nb_retune_info);
...
rflm_err_t rflm_lte_get_ul_dl_timing_offsets
(
   rflm_lte_ul_dl_timing_delta_s *update_ul_dl_timing_delta , 
   rflm_lte_cell_id_e             cell_index
);

rflm_err_t rflm_lte_fs_retune(rflm_lte_fs_retune_input_s *fs_retune_info);

rflm_err_t
rflm_lte_sync_trx_ustmr_to_modem_ustmr(rflm_lte_sync_trx_ustmr_s *sync_trx_ustmr_info);

#ifdef FEATURE_MODEM_CRS_CAPTURE
void rflm_lte_notch_control(rflm_handle_rx_t handle_id,boolean force_disable_notch);
#endif
