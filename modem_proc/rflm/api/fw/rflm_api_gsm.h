...
$Header: //components/rel/rflm.mpss/1.10/api/fw/rflm_api_gsm.h#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/23/20   mpa     To reduce RFLM overhead by 60qs
07/23/19   krk     Intf change for GW to modify PMON backoff in HMSC
06/13/19   krk     Notch_configuration for XO spur (+/-) channels to avoid sensitivity degradation
04/21/10   mpa     to change rflm_overhead to 55
26/02/19   mpa     JDET support on SDR105
01/30/18   mpa     AD_DAC changes
...
#define RFLM_GSM_TX_MAX_PROC_OVERHEAD_QS 210
#else
#define RFLM_GSM_TX_MAX_PROC_OVERHEAD_QS 110
#endif
...
#define RFLM_MAX_IP2_CAL_STEPS 0
#define MAX_IP2_CAL_STEPS_IN_CCS_QUEUE 0

#define MAX_IP2_STEP_TASKS_IN_CCS_QUEUE 0  // CCS_RF_TASKQ_NUM_ITEMS
#else
...
#define RFLM_GSM_NUM_TRANS_PER_JDET_READ 8
...
typedef enum 
{
 Notch_P_200,   /* Spur-channel;ZIF */
 Notch_P_335, /* Spur-channel;P_LIF */
 Notch_P_65, /* Spur-channel;N_LIF */
 Notch_N_200,   /* Spur+channel;ZIF */
 Notch_N_65, /* Spur+channel;P_LIF */
 Notch_N_335, /* Spur+channel;N_LIF */
 Notch_default,
 Notch_max 
}rflm_gsm_notch_index_type;
...
typedef struct
{
  ...
  boolean      immediate_trig_flag;
  uint32       rflm_gsm_intf_idx;
}rflm_gsm_fsk_init_ccs_input_type;
...
{
  rflm_ustmr_t execution_time;
}rflm_gsm_fsk_init_ccs_output_type;
...
typedef struct
{
  ...
  uint32       rf_sequence_number;
  uint8        pos_neg_flag;
}rflm_gsm_fsk_tx_ccs_input_type;
...
{
  rflm_ustmr_t execution_time;
}rflm_gsm_fsk_tx_ccs_output_type;
...
}rflm_gsm_sync_trx_ustmr_s;


typedef struct{
   /* TX chain index to activate */
   // uint32 tx_chain_index;  -- get it from TxLM handle
   /* Time to activate chain at */
   uint32 activation_time;

} rflm_gsm_tx_start_burst_input_type;

typedef struct{
   /* temporary place holder  -- chain index is from WTR SS*/
   boolean flag;
}rflm_gsm_tx_start_burst_output_type;


typedef struct{
   /* Time to deactivate tx chain.  Chain information from WTR SS */
   uint32 deactivation_time;
}rflm_gsm_tx_stop_burst_input_type;

typedef struct{
   /* temporary place holder if GFW needs status from FED */
   boolean flag;
}rflm_gsm_tx_stop_burst_output_type;
...
} rflm_gsm_rx_start_burst_input_type;

typedef struct{
   /* temporary place holder  -- chain index is from WTR SS*/
   boolean flag;
}rflm_gsm_rx_start_burst_output_type;


typedef struct{
   /* Time to deactivate tx chain.  Chain information from WTR SS */
   uint32 deactivation_time;
}rflm_gsm_rx_stop_burst_input_type;

typedef struct{
   /* temporary place holder if GFW needs status from FED */
   boolean flag;
}rflm_gsm_rx_stop_burst_output_type;
...
#if 1
extern rflm_err_t rflm_txagc_modem_enable_tx_dac
(
  uint32 handle_id,
...
extern rflm_err_t rflm_txagc_modem_disable_tx_dac
(
  uint32 handle_id,
...
extern rflm_err_t rflm_gsm_ustmr_release( uint32 device_id,rflm_gsm_subscription_id sub_id );
extern rflm_err_t rflm_gsm_ustmr_start( uint32 device_id,rflm_gsm_subscription_id sub_id );
...
/*============================================================================*/
/*! @fn rflm_gsm_rx_burst_start_config                                           */
/*! @brief A command for FW to call FED for CCS Tx event processing           */
/*! @details Tx burst event processing called for start, trans and stop with  */
/*!          slot id                                                          */

extern rflm_err_t rflm_gsm_rx_burst_start_config( rflm_handle_rx_t rxlm_handle,
												   rflm_gsm_rx_start_burst_input_type *vars_in,
												   rflm_gsm_rx_start_burst_output_type *vars_out);



/*============================================================================*/
/*! @fn rflm_gsm_rx_burst_stop_config                                           */
/*! @brief A command for FW to call FED for CCS Tx event processing           */
/*! @details Tx burst event processing called for start, trans and stop with  */
/*!          slot id                                                          */
extern rflm_err_t rflm_gsm_rx_burst_stop_config( rflm_handle_rx_t rxlm_handle,
												   rflm_gsm_rx_stop_burst_input_type *vars_in,
												   rflm_gsm_rx_stop_burst_output_type *vars_out);


/*============================================================================*/
/*! @fn rflm_gsm_tx_burst_start_config                                           */
/*! @brief A command for FW to call FED for CCS Tx event processing           */
/*! @details Tx burst event processing called for start, trans and stop with  */
/*!          slot id                                                          */

extern rflm_err_t rflm_gsm_tx_burst_start_config( rflm_handle_rx_t rxlm_handle,
												   rflm_gsm_tx_start_burst_input_type *vars_in,
												   rflm_gsm_tx_start_burst_output_type *vars_out);

/*============================================================================*/
/*! @fn rflm_gsm_tx_burst_stop_config                                           */
/*! @brief A command for FW to call FED for CCS Tx event processing           */
/*! @details Tx burst event processing called for start, trans and stop with  */
/*!          slot id                                                          */
extern rflm_err_t rflm_gsm_tx_burst_stop_config( rflm_handle_rx_t rxlm_handle,
												   rflm_gsm_tx_stop_burst_input_type *vars_in,
												   rflm_gsm_tx_stop_burst_output_type *vars_out);
...
extern rflm_err_t rflm_gsm_sync_trx_ustmr_to_modem_ustmr(rflm_gsm_sync_trx_ustmr_s *sync_trx_ustmr_info);
extern boolean rflm_get_therm_data( uint8 sub_id, uint32* therm_data );

uint8 get_debug_tx_afc_crash_count();


void set_debug_tx_afc_crash_count(uint8 val);

uint8 get_debug_tx_stop_crash_count();

void set_debug_tx_stop_crash_count(uint8 val);

extern int rflm_get_num_tasks(void* task_ptr);
...
extern rflm_err_t rflm_gsm_prog_fsk_init_ccs_script( rflm_handle_rx_t rflm_handle,
                                                 rflm_gsm_fsk_init_ccs_input_type *script_vars_in,
                                                 rflm_gsm_fsk_init_ccs_output_type *script_vars_out );

extern rflm_err_t rflm_gsm_prog_fsk_tx_band_ccs_script( rflm_handle_rx_t rflm_handle,
                                                    rflm_gsm_tx_band_ccs_input_type *script_vars_in,
                                                    rflm_gsm_tx_band_ccs_output_type *script_vars_out );
extern rflm_err_t rflm_gsm_prog_fsk_tx_start_ccs_script( rflm_handle_tx_t rflm_handle,
                                               rflm_gsm_fsk_tx_ccs_input_type *script_vars_in,
                                               rflm_gsm_fsk_tx_ccs_output_type *script_vars_out );
extern rflm_err_t rflm_gsm_prog_fsk_tx_afc_update_ccs_script( rflm_handle_tx_t rflm_handle,
                                               rflm_gsm_fsk_tx_ccs_input_type *script_vars_in,
                                               rflm_gsm_fsk_tx_ccs_output_type *script_vars_out );

extern rflm_err_t rflm_gsm_prog_fsk_tx_stop_ccs_script( rflm_handle_tx_t rflm_handle,
                                               rflm_gsm_fsk_tx_ccs_input_type *script_vars_in,
                                               rflm_gsm_fsk_tx_ccs_output_type *script_vars_out );
