...
$Header: //components/rel/rflm.mpss/1.10/api/ccs/rf_hal_bus_types.h#1 $
...
/*! @brief Invalid tag_id */
#define RF_HAL_TAG_ID_INVALID        0x0000

/*! @brief Token_id for single timer fixup */
#define RF_HAL_TIMER_TOKEN_ID        0xABCD

/*! @brief Max number of tokens per script */
#define RF_HAL_RFFE_GROUP_MAX_TOKEN_PER_SCRIPT      20
/*! @brief Read or write operations */
typedef enum
{
...
typedef enum
{
  ...
  RF_HAL_BUS_RESOURCE_RFFE_GROUP,    /*!< RFFE group write  */
...
/*! @brief RFFE transaction addr/data structure
 *
 * */

typedef struct
{
  uint8 *data;      /*!< pointer to the data location */
  uint16 addr;      /*!< start addr */
  uint16 num_bytes; /*!< num_bytes to copy */
} rf_hal_bus_add_group_type;

/*! @brief RFFE transaction tag structure
 *
 * */
...
} rf_hal_bus_token_group_type;


/*! @brief RFFE Group transaction data structure
 *
 * */
typedef struct
{

  rf_hal_bus_add_group_type rffe_addr;    /*! The RFFE address group*/
  rf_hal_bus_token_group_type *rffe_token;/*! pointer to the token group info */
  rf_hal_bus_rd_wr_type rd_wr;/*!< Read or write transaction. 1 = Read, 0 = Write */
  ...
  uint8 num_bytes;            /*!< Number of bytes to write or read; must be ONE for MASKED_WRITE type */
  int16 start_delta;
  ...
  uint16 shadowed_reg_idx;    /*!< Idx of shadowed register  */
  uint8 mask;                 /*!< mask for masked write     NOT supported for grouped API  */
  uint8 num_tokens;           /*!< Num of Tokens in this task */
} rf_hal_bus_rffe_group_type;
