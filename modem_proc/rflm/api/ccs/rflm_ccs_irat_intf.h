...
$Header: //components/rel/rflm.mpss/1.10/api/ccs/rflm_ccs_irat_intf.h#1 $
...
/* Writing token info.
   Used for writing token idx and sub event idx
*/
#define RFLM_CCS_TOKEN_INFO_SIZE 4

#define RFLM_CCS_RF_IRAT_SIZEOF_CCS_HEADER_BYTES   (((sizeof(rflm_ccs_rf_event_description_buffer_header_type)+3)>>2)<<2)
#define RFLM_CCS_RF_IRAT_SIZEOF_CCS_TASK_BYTES     ((((sizeof(ccs_rf_task_buf_item_t)+3)>>2)<<2) + RFLM_CCS_TOKEN_INFO_SIZE)
...
#define RFLM_CCS_IRAT_MAX_RFFE_GROUP_NUM_REGS 100
#define RFLM_CCS_IRAT_MAX_RFFE_GROUP_NUM_GROUPS 10
