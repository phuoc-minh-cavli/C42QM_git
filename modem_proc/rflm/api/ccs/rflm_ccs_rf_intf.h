...
$Header: //components/rel/rflm.mpss/1.10/api/ccs/rflm_ccs_rf_intf.h#1 $
...
typedef struct 
{
  ...
  uint32 seq_ccs_addr;       /*!< seq header CCS addr */
...
typedef struct rflm_ccs_rf_event_time_chk_config_s
{
  ...
  /*! @brief No ordering of callbacks based on their task's enqueuing ordering */ 
  boolean no_callback_order;
...
typedef struct rflm_ccs_enq_task_config_s
{
  rflm_ccs_rf_tq_handle_t *tq;           /*!< Task queue handle */
  ccs_rf_task_t *task;                   /*!< Task to enqueue */
  rflm_ccs_rf_task_callback_t callback;  /*!< Callback function for task completion */
  void *callback_arg;                    /*!< Argument to callback function */
  boolean no_cb_order;                   /*!< no callback order */
  rflm_tech_id_t tech_id;                /*!< tech id */
}rflm_ccs_enq_task_config_t;
...
extern int32 rflm_ccs_rf_enqueue_task
(
  rflm_ccs_enq_task_config_t * task_config_ptr
);

extern uint32 rflm_ccs_rf_tq_task_enqueue
(
   rflm_ccs_rf_tq_handle_t *tq,          /*!< Task queue handle */
   ccs_rf_task_t *task,                  /*!< Task to enqueue */
   void  *seq_ptr,                       /*!< seq to enqueue */
   rflm_ccs_rf_task_callback_t callback, /*!< Callback function for task completion */
   void *callback_arg,                   /*!< Argument to callback function */
   boolean no_cb_order,                  /*!< Flag to indicate call cb regardless of order>*/
   rflm_ccs_mmu_alloc_t * mmu_alloc,     /*!<MMU alloc info>*/
   uint32 * task_ccs_addr                /*!< task ccs addr>*/
 );


extern int32 rflm_ccs_rf_enqueue_issue_seq
  (
   rflm_ccs_enq_task_config_t * task_config_ptr,
   rflm_ccs_rf_seq_t *issue_seq_ptr
  );

extern boolean rflm_ccs_set_dynamic_rffe_clk(rffe_clk_info_type* channel_clk_info ); 
