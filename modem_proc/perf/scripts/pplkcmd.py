...
class Pplk(object):  # {{{
  ...
  def __init__(self, target, source, env, debug=True, keeplog=True):  # {{{
    ...
    self.q6z_flags = [
      i
      for i in env['USES_FLAGS']
      if i.lower().startswith('uses_q6zip_')
    ]
    ...
    pplklogname = self.short_buildpath
...
# http://docs.python.org/2/library/multiprocessing.html#using-a-pool-of-workers
  ...
  def qshrink(self, inpath, outpath):  # {{{
    ...
    if 'QSR_STRING' in [i.sh_nameStr for i in sh_table] or 'QSR_4_0_MSG' in [i.sh_nameStr for i in sh_table]:
      ...
      qs_build_path = os.path.abspath(os.path.join(self.buildpath,'build','myps','qshrink',))
      retcode, stdout, stderr = self.call([
        self.python,
        '-O',
