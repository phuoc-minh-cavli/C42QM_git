...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/perf.mpss/2.1.2.0/scripts/elfManipulator/elfManipulator.py#5 $
#  $DateTime: 2021/01/23 15:51:41 $
#  $Author: pwbldsvc $
#  $Change: 28419022 $
...
# --------  ------  ----    ----------------------------------------------------
# 06/13/18  che     1.15    CR 2260004 add external paging section manipulations: calculate hashes 
#                           for pagable sections, hashes for ondemand pages, remove external sections
...
def main():
    ...
    if options.ro_fatal:
        ...
        if sh != const.RC_ERROR:
            ...
            if sh.sh_size == 0:
                ...
                elf.moveupElfOffsetSectionByName("ro_fatal")
        else:
    ...
    print "Generating pageable module hashes..."
    extpl_paging_rx_sections = [ 
        (
    ...
    elf_name=baseELF.split("/")[-1]  ##linux
    elf_name=baseELF.split("\\")[-1] ##windows
    if "PAGEONLY" not in elf_name:
        try:
            print '__clade_comp_pd0_start__           : 0x%08x' %(elf.getSymbolByName('__clade_comp_pd0_start__').st_value)
            print '__clade_comp_pd0_end__             : 0x%08x' %(elf.getSymbolByName('__clade_comp_pd0_end__').st_value)
            print '__clade_region_high_pd0_start__    : 0x%08x' %(elf.getSymbolByName('__clade_region_high_pd0_start__').st_value)
            print '__clade_region_high_pd0_end__      : 0x%08x' %(elf.getSymbolByName('__clade_region_high_pd0_end__').st_value)
            print '__clade_region_high_dynamic_start__: 0x%08x' %(elf.getSymbolByName('__clade_region_high_dynamic_start__').st_value)
            print '__clade_region_high_dynamic_end__  : 0x%08x' %(elf.getSymbolByName('__clade_region_high_dynamic_end__').st_value)
        except:
            print "Clade1 Symbols not found"
    ##For Procesing the QDSP6SW MBN file
    clade_comp_section=elf.getSectionByName(".clade.comp")
    clade_comp_section_static_hi=elf.getSectionByName(".region_high_clade_static")
    clade_comp_section_static_lo=elf.getSectionByName(".region_low_clade_static")
    if clade_comp_section_static_hi != const.RC_ERROR or clade_comp_section_static_lo != const.RC_ERROR:
        print 'clade_comp_section_static_hi: addr: 0x%08x size: 0x%08x' %(clade_comp_section_static_hi.sh_addr, clade_comp_section_static_hi.sh_size)
        print 'clade_comp_section_static_lo: addr: 0x%08x size: 0x%08x' %(clade_comp_section_static_lo.sh_addr, clade_comp_section_static_lo.sh_size)
       
        clade_comp_section_hi_size=clade_comp_section_static_hi.sh_size
        clade_comp_section_lo_size=clade_comp_section_static_lo.sh_size
        #clade_comp_static_offfset=((clade_comp_section_hi_size/2)+clade_comp_section_lo_size)
        #print "clade_comp_static_offfset: 0x%08x" %(clade_comp_static_offfset)
        #if clade_comp_static_offfset&0x00FFFF>0:
        #    clade_comp_static_offfset=(clade_comp_static_offfset&0xFF0000)+0x10000
        #    elf.updateSymbolValueByName("clade_comp_static_offfset",clade_comp_static_offfset)
        #    print "clade_comp_static_offfset(updated): 0x%08x" %(clade_comp_static_offfset)
        #
        clade_region_high_dynamic_start = elf.getSymbolByName('__clade_region_high_dynamic_start__').st_value
        clade_region_high_pd0_start     = elf.getSymbolByName('__clade_region_high_pd0_start__').st_value
        clade_comp_static_offfset       = clade_region_high_dynamic_start - clade_region_high_pd0_start
        elf.updateSymbolValueByName("clade_comp_static_offfset",clade_comp_static_offfset)
        print "clade_comp_static_offfset (updated): 0x%08x" %(clade_comp_static_offfset)
        
    if clade_comp_section != const.RC_ERROR:
        clade_comp_start_address = clade_comp_section.sh_addr
        print "clade_comp_start_address: 0x%08x" %(clade_comp_start_address)
        print "clade_comp_size: 0x%08x" %(clade_comp_section.sh_size)
        elf.updateSymbolValueByName("clade_comp_size",clade_comp_section.sh_size)
        elf.updateSymbolValueByName("clade_comp_address",clade_comp_start_address)
    ###################################################################################################
    module_code_hashes=""
    for section in extpl_paging_rx_sections:
        sh_extpl = elf.getSectionByName(section[0])
        if section[0]==".clade.comp":
            extpl_m1_high = elf.getSectionByName(".high_catm1_dynamic")
            extpl_m1_low = elf.getSectionByName(".low_catm1_dynamic")
            if extpl_m1_high == const.RC_ERROR or extpl_m1_low == const.RC_ERROR:
                continue
        if sh_extpl != const.RC_ERROR:
            print (section[0],section[1],section[2])
            print ('start %x' %(elf.getSymbolByName(section[2]).st_value))
            print ('end %x' %(elf.getSymbolByName(section[1]).st_value))
            print "Generating module code hash for ", section[0]
            
            # hash for rx pages
            offset = elf.getSymbolByName(section[2]).st_value - sh_extpl.sh_addr
            size = elf.getSymbolByName(section[1]).st_value - elf.getSymbolByName(section[2]).st_value
            hash = hashlib.sha256(sh_extpl.contents[offset:offset+size])
            module_code_hashes = module_code_hashes + hash.digest()
            print "module %s rx size %d" % (section[0], size)
            print "    sha256 ", hash.hexdigest()

    module_data_hashes=""
    for section in extpl_paging_rw_sections:
        sh_extpl = elf.getSectionByName(section[0])
        if sh_extpl != const.RC_ERROR:
            print "Generating module data hash for ", section[0]
            ...
            hash = hashlib.sha256(sh_extpl.contents[offset:offset+size])
            module_data_hashes = module_data_hashes + hash.digest()
            print "module %s ro/rw size %d" % (section[0], size)
            print "    sha256 ", hash.hexdigest()
            if section[0] != '.clade.comp':
              print "removing section ", section[0]
              elf.removeSectionByName(section[0])
            
    hashtbl_sym = elf.getSymbolByName("extpl_module_rx_hashes")
    if hashtbl_sym != const.RC_ERROR:
        hasbtbl_sect = elf.sectionHeaderTable[hashtbl_sym.st_shndx]
        # For executable files, st_value holds a virtual address
        offset = hashtbl_sym.st_value - hasbtbl_sect.sh_addr
        hasbtbl_sect.contents = "".join([
            hasbtbl_sect.contents[:offset],
            module_code_hashes,
            hasbtbl_sect.contents[offset+len(module_code_hashes):] ])
    
    hashtbl_sym = elf.getSymbolByName("extpl_module_rw_hashes")
    if hashtbl_sym != const.RC_ERROR:
        hasbtbl_sect = elf.sectionHeaderTable[hashtbl_sym.st_shndx]
        # For executable files, st_value holds a virtual address
        offset = hashtbl_sym.st_value - hasbtbl_sect.sh_addr
        hasbtbl_sect.contents = "".join([
            hasbtbl_sect.contents[:offset],
            module_data_hashes,
            hasbtbl_sect.contents[offset+len(module_data_hashes):] ])
            
    sh_ondemand = elf.getSectionByName(".candidate_extpl_section_ondemand")
    if sh_ondemand != const.RC_ERROR:
        print "Generating OnDemand page hashes..."
        ondemand_pages = sh_ondemand.sh_size/4096
        ondemand_hashes = ""
        offset = 0
        print "Total OnDemand pages %d"%ondemand_pages
        for vpn in xrange(ondemand_pages):
            hash=hashlib.sha256(sh_ondemand.contents[offset:offset+4096])
            ondemand_hashes = ondemand_hashes + hash.digest()
            offset = offset + 4096
            print "page %d hash %s" % (vpn, hash.hexdigest())
 
        hashtbl_sym = elf.getSymbolByName("extpl_ondemand_hashes")
        hasbtbl_sect = elf.sectionHeaderTable[hashtbl_sym.st_shndx]
        offset = hashtbl_sym.st_value - hasbtbl_sect.sh_addr
        hasbtbl_sect.contents = "".join([
            hasbtbl_sect.contents[:offset],
            ondemand_hashes,
            hasbtbl_sect.contents[offset+len(ondemand_hashes):] ])

        print "removing section .candidate_extpl_section_ondemand" 
        elf.removeSectionByName(".candidate_extpl_section_ondemand")
    ...
    if options.overlay_mem_dump and overlay_mem_dumpSize >= 0:
        ...
        clade1CompressSh = elf.getSectionByName(clade1comp)
        ...
        if clade1CompressSh != const.RC_ERROR:
            clade1CompressSz = clade1CompressSh.sh_size

        if (overlay_mem_dumpSize) > (clade1CompressSz):
            print "FATAL ERROR: fw memdump +tcm dump exceeds "+clade1comp
            print "clade1 Compress size: %s bytes" % str(clade1CompressSz)
            print "FW memdump overlay size: %s bytes" % str(overlay_mem_dumpSize)
            print "Short by: %s bytes" % (clade1CompressSz -
