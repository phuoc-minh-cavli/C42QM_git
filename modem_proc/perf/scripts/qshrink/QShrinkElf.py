...
$Header: //components/rel/perf.mpss/2.1.2.0/scripts/qshrink/QShrinkElf.py#1 $
$Date: 2020/06/15 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
13/01/17   sh      QShrink 4.0 support for QSH Trace
...
def runQShrink(elfFile, outputFile,buildOutputPath,hashFile,qdbFile,
  ...
  print "Jsumit Qshrink Version:- ",ElfShrinkState.version
...
# Comment out the check as 7.3 link toolchain does not support clean up of unused functions from other sections
#  if (ElfShrinkState.totalQSR_STRINGlen+15360 < QSR_STRING_SIZE):
#    ElfShrinkState.outputLog.write(", ".join([
...
#    return
  ...
  if elfBytesOffset < 0:
    # Start editing the string and put out the error. Tricky handling here
    # Things are falling apart in memory tm part of QShrink. Unexpected should be expected.
    # Play with negative 
    # Increment in 500 bytes..confusing ?, welcome to QShrink world..
    #Update the ElfOffset of all the sections moved up, when QSR_STR section is shrunk
    sectionstart = QSR_STRING_OFFSET+QSR_STRING_SIZE+10000;
    for i in xrange(0, header['sectionHeaderNum']) :
      section_hdr = getSectionHeader(i);
      if ((section_hdr.data['sh_offset'] >  QSR_STRING_OFFSET) 
         and (section_hdr.data['sh_offset'] < sectionstart)):
        sectionstart = section_hdr.data['sh_offset'] 
    base = 4096*(QSR_STRING_SIZE/4096+1)-QSR_STRING_SIZE    
    if (((QSR_STRING_OFFSET+QSR_STRING_SIZE-elfBytesOffset) < sectionstart)
        and (elfBytesOffset <base)):
       elfBytesOffset = 0
    else:
      ElfShrinkState.outputLog.write(", ".join([
                        "ERROR: Unepected QShrink error , Contact Qshrink team","\n"]))
      ElfShrinkState.errCount += 1 
      return
  ...
  for i in range(0, int(header['programHeaderNum'])):
    ...
    if ph.data['p_vaddr'] == QSR_STRING_START:
      ...
      newSize = oldSize - QSR_STRING_SIZE + newQsrStringSectionSize
      if newQsrStringSectionSize == 0:
        newSize+=QSR_EMPTY_SIZE
  ...
  if ('QSR_STRING' in ElfShrinkState.sectionHeaderDict):
    ...
    for i in range(0, int(header['programHeaderNum'])):
      ...
      if ((ph.data['p_vaddr'] + size) <= QSR_STRING_START and
        ...
        if  align ==0:
          align = 4096
        QSR_vAddr = ph.data['p_vaddr'] + ph.data['p_memsz'] + align
        QSR_vAddr = QSR_vAddr - (QSR_vAddr % align)   
...
def qsr4process():
  ...
  processV4Structs()
  qsr4CreateHashFile()
  del ElfShrinkState.QSR4_MTRACESTRINGlist #free memory
  del ElfShrinkState.QSR4_QTRACESTRINGlist #free memory
...
# all symbol names
  ...
  # check for a QSR struct symbol
    ...
    elif QSR4MtraceStartRE.match(entry.name):
...
#=========================================================================
# Function : processV4MtraceStructs
#
# Arguments
#
# Description
#   Iterates through mtrace contents
#
# Return Values:
#  List of structs
#=========================================================================
...
#Read QSR_4_0_MSG section and create hash table and DIAG run time filtering tables
  ...
  ElfShrinkState.outputLog.write("".join(["\n","MTRACE Messages processed from address, ", hex(ElfShrinkState.QSR4MTRACEStart)," to, ",hex(ElfShrinkState.QSR4MTRACEEnd), "\n"])) 


#=========================================================================
# Function : processV4QtraceStructs
#
# Arguments
#
# Description
#   Iterates through Qtrace String contents
#
# Return Values:
#  List of structs
#=========================================================================
...
#Read QSR_4_0_MSG section 
  ...
  ElfShrinkState.outputLog.write("".join(["\n","QTRACE Str Messages processed from address, ", hex(ElfShrinkState.QSR4QTRACEStrStart)," to, ",hex(ElfShrinkState.QSR4QTRACEStrEnd), "\n"])) 
...
def qsr4RemoveSection(qsr_2_or_3):
  ...
  for i in range(0, header['programHeaderNum']):
    ph = ProgramHeader(i)
    #identify str section by offset
    if ((ph.data['p_offset'] == QSR_MSG_OFFSET) and
       (ph.data['p_vaddr'] == QSR_MSG_START)):
