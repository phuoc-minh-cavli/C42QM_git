...
class MemoryChecker(object):  # {{{
  ...
  external_swap_abs_syms = [
    '__extpl_swap_pool_start__',
    '__extpl_swap_pool_end',
  ]
  q6z_swap_pool_syms = q6z_swap_pool_int_syms + q6z_swap_pool_abs_syms
  dsm_symbol_re = r'^dsm_(ds|dup)_.+_array$'
  ...
  size_template = (
    ...
    # )
    ('Image', 'img_sz', 0),
    ('AMSS Heap', 'amss_mem_heap', 0),
    ('MPSS Heap','modem_mem_heap', 0),
    ...
    #('WLAN Reserved', 'wlan_reserved', 0),
    ('External Paging Swap pool', 'external_paging', 0),
    ('Kernel Stack', 'kernel_stack', 0),
    ('Extra', 'extra', 256 * (1 << 10)),  # was called 'QRegionReq'
    ('Padding', 'pad', 0),
  ...
  def dynamic_or_static(self):  # {{{
    ...
    if in_range(self.start_address,
      ...
      self.sym_sizes['amss_heap']['size'] = 0
    ...
    if ('mpss_heap' in self.sym_sizes and
      ...
      if in_range(self.start_address,
        ...
        self.sym_sizes['mpss_heap']['size'] = 0
    ...
    if in_range(
      self.start_address,
      self.budget,
      self.sym_sizes['modem_mem_heap']['start'],
      self.sym_sizes['modem_mem_heap']['size']
    ):
      self.sizes[self.sz_idx['img_sz']] -= \
        self.sym_sizes['modem_mem_heap']['size']

      self.sym_sizes['modem_mem_heap']['type'] = 'static'

    else:
      self.sym_sizes['modem_mem_heap']['type'] = 'dynamic'

    if in_range(
      self.start_address,
      self.budget,
      self.sym_sizes['amss_mem_heap']['start'],
      self.sym_sizes['amss_mem_heap']['size']
    ):
      self.sizes[self.sz_idx['img_sz']] -= \
        self.sym_sizes['amss_mem_heap']['size']

      self.sym_sizes['amss_mem_heap']['type'] = 'static'

    else:
      self.sym_sizes['amss_mem_heap']['type'] = 'dynamic'      
    
    
    if in_range(
      self.start_address,
      self.budget,
      self.sym_sizes['external_paging']['start'],
      self.sym_sizes['external_paging']['size']
    ):
      self.sizes[self.sz_idx['img_sz']] -= \
        self.sym_sizes['external_paging']['size']

      self.sym_sizes['external_paging']['type'] = 'static'

    else:
      self.sym_sizes['external_paging']['type'] = 'dynamic'      
  ...
  def get_image_and_pad_size(self):  # {{{
      ...
      if prev_start > 0 and prev_size > 0:
        ...
        if potential_pad >= 0:
          if (
            ...
            dp(''.join([
              'Padding of %8d bytes between segments 0x%08x and 0x%08x %s and %s' % (
                potential_pad,
                prev_start,
                start,
                self.elf.getSectionByAddress(prev_start).sh_nameStr,
                self.elf.getSectionByAddress(start).sh_nameStr
                )]))
            pad += potential_pad
            
            if potential_pad > 16*1024:
                self.padding_list.append(potential_pad)
    ...
    if alignment_pad > 16*1024:
        self.padding_list.append(alignment_pad)
    
    dp('Alignment padding of %8d (%d KB)added' % (alignment_pad,alignment_pad/1024))
  ...
  def get_sym_sizes(self):  # {{{
    ...
    sym_list = (
      ...
      + self.amss_heap_abs_syms
      + self.mpss_heap_abs_syms
      + self.external_swap_abs_syms
      + self.clade_region_high_syms
      + self.clade_region_low_syms
      + self.clade_comp_syms
      + self.clade_exception_low_large_syms
      + self.clade_exception_low_small_syms
      + self.clade_dict_syms
      + self.clade_exception_syms
      + self.clade_metadata_syms
    )
    print "======================================================================================"
    print "                             Linker Script Symbols Values                             "
    print "======================================================================================"
    for i in sym_list:
      try:
        syms[i] = self.symtab.get(i)
        print i,"				", str(hex(syms[i].oneline())).strip("L")
      except KeyError:
    ...
    clade1_compressed_input_hi=0
    clade1_compressed_input_low=0
    clade1_compressed_size=0
    exception_low_large_size=0
    exception_low_small_size=0
    clade1_dict_size=0
    clade1_exception_size=0
    clade1_metadata_size=0
    print "\n======================================================================================"
    print "\n\n"
    print "======================================================================================"
    print "                             CLade Memory Savings Report                              "
    print "======================================================================================"
    try:
        clade1_compressed_input_hi=syms["__clade_region_high_pd0_end__"].st_value-syms["__clade_region_high_pd0_start__"].st_value
        print ".clade_region_high %20s %.3f MB" %(" ",clade1_compressed_input_hi/1024.0/1024)
    except:
        print ".clade_region_high Sections not found in the build "
    try:
        clade1_compressed_input_low=syms["__clade_region_low_pd0_end__"].st_value-syms["__clade_region_low_pd0_start__"].st_value
        print ".clade_region_low %20s %.3f MB" %(" ",clade1_compressed_input_low/1024.0/1024)
    except:
        print ".clade_region_low section not found in the build "
    try:
        clade1_compressed_size=syms["__clade_comp_pd0_end__"].st_value-syms["__clade_comp_pd0_start__"].st_value
        print ".clade_comp %20s %.3f MB" %(" ",clade1_compressed_size/1024.0/1024)
    except:
        print ".clade_comp Sections not found in the build "
    try:
        exception_low_large_size=syms["__clade_exception_low_large_pd0_end__"].st_value-syms["__clade_exception_low_large_pd0_start__"].st_value
        print ".clade_exception_low %20s %.3f MB" %(" ",exception_low_large_size/1024.0/1024)
    except:
        print ".clade_exception_low_large Sections not found in the build "
    try:
        exception_low_small_size=syms["__clade_exception_low_small_pd0_end__"].st_value-syms["__clade_exception_low_small_pd0_start__"].st_value
        print ".clade_exception_low_small %20s %.3f MB" %(" ",exception_low_small_size/1024.0/1024)
    except:
        print ".clade_exception_low_small Sections not found in the build "
    try:
        clade1_dict_size=syms["__clade_dict_pd0_end__"].st_value-syms["__clade_dict_pd0_start__"].st_value
        print ".clade_dict %20s %.3f MB" %(" ",clade1_dict_size/1024.0/1024)
    except:
        print ".clade_dict Sections not found in the build "
    try:
        clade1_exception_size=syms["__clade_exception_high_pd0_end__"].st_value-syms["__clade_exception_high_pd0_start__"].st_value
        print ".clade_exception_high %20s %.3 MB" %(" ",clade1_exception_size/1024.0/1024)
    except:
        print ".clade_exception Sections not found in the build "
    try:
        clade1_metadata_size=syms["__clade_metadata_pd0_end__"].st_value-syms["__clade_metadata_pd0_start__"].st_value
        print ".clade_metadata %20s %.3f MB" %(" ",clade1_metadata_size/1024.0/1024)
    except:
        print ".clade_metadata Sections not found in the build "
    print "Clade1 Savings %20s %.3f MB" %(" ",(clade1_compressed_input_hi+clade1_compressed_input_low-clade1_compressed_size-exception_low_large_size-exception_low_small_size-clade1_dict_size-clade1_exception_size-clade1_metadata_size)/1024.0/1024)
    print "\n======================================================================================\n\n"
    ...
    try:
      sym_sizes['amss_mem_heap'] = {
        'start': syms[self.amss_heap_abs_syms[0]].st_value,
        'size': (
          syms[self.amss_heap_abs_syms[1]].st_value
          - syms[self.amss_heap_abs_syms[0]].st_value
        ),
      }
    ...
    try:
      sym_sizes['modem_mem_heap'] = {
        'start': syms[self.mpss_heap_abs_syms[0]].st_value,
        'size': (
          syms[self.mpss_heap_abs_syms[1]].st_value
          - syms[self.mpss_heap_abs_syms[0]].st_value
        ),
      }
    ...
    try:
      sym_sizes['external_paging'] = {
        'start': syms[self.external_swap_abs_syms[0]].st_value,
        'size': (
          syms[self.external_swap_abs_syms[1]].st_value
          - syms[self.external_swap_abs_syms[0]].st_value
        ),
      }
    ...
    #sym_sizes['wlan_reserved'] = {'start': 0,'size': 0, 'type': 'dynamic'}
    #sym_sizes['external_paging'] = {'start': 0,'size': 0, 'type': 'dynamic'}
  ...
  def distribute_sym_sizes(self):  # {{{
      ...
      for _, dest_key, _ in self.size_template:
        if dest_key in src_key:
          if 'size' in sym_sizes[src_key]:
            dp("dbg1: "+dest_key+" : "+src_key+" : %16d" %(sym_sizes[src_key]['size']))
            self.sizes[self.sz_idx[dest_key]] += sym_sizes[src_key]['size']

          del sym_sizes[src_key]
          break

    # Recoverable padding major list
    self.padding_list.sort(reverse=True)
    
    dp('padding_list  : '+str(self.padding_list))
    ...
    for padvalue in self.padding_list:
        if padvalue > 128*1024:
            padvalue -= 128*1024
            recoverable_128K += 1
        if padvalue > 64*1024:
            padvalue -= 64*1024
    ...
    dp('Recoverable 128K : '+str(recoverable_128K)+' required:2')
    dp('Recoverable  64K : '+str(recoverable_64K)+' required:2')
    dp('Recoverable  16K : '+str(recoverable_16K)+' required:8')

    extra_mem = 0
    
    # 128*2 + 64*2 + 16*8 = 512K reserve 
    if recoverable_128K < 2:
        extra_mem += (2-recoverable_128K)*128*1024
    
    if recoverable_64K < 2:
        extra_mem += (2-recoverable_64K)*64*1024

    if recoverable_16K < 8:
        extra_mem += (8-recoverable_16K)*16*1024

    dp("extra_mem : " +str(extra_mem))
    dp("dbg2extra initial value: "+str(self.sizes[self.sz_idx['extra']]))
    #self.sizes[self.sz_idx['extra']] = max(extra_mem,128*1024)
    dp("dbg2extra modified value: "+str(self.sizes[self.sz_idx['extra']]))
    
    # add up everything we missed into 'extra'
    for src_key, src_value in sym_sizes.iteritems():
      if 'size' in src_value:
        dp("dbg3extra:"+src_key+":%16d" %(src_value['size']))
        self.sizes[self.sz_idx['extra']] += src_value['size']

    # merge some calculated items into 'Extra'
    dp("dbg4extra:trace_buffer:"+str(self.sizes[self.sz_idx['trace_buffer']]))
    
    if self.sizes[self.sz_idx['trace_buffer']] > 16*1024:
        self.sizes[self.sz_idx['extra']] += 4 * self.sizes[self.sz_idx['trace_buffer']] # 4 H/W thread
    self.sizes[self.sz_idx['trace_buffer']] = 0
    
    dp("dbg5extra:kernel_stack:"+str(self.sizes[self.sz_idx['kernel_stack']]))
    
    if self.sizes[self.sz_idx['kernel_stack']] > 8*1024:
