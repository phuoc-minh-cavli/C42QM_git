...
#----------------------------------------------------------------------------
#
#  $Header: //commercial/MPSS.JO.1.2.r3.1/Main/modem_proc/tools/build/scons/scripts/analytic_data_builder.py#1 $
...
def generate(env):
    ...
    img_env.Replace(DEPENDENCY_TREE_DATA={})
    ...
    env.AddMethod(capture_dependency_item, "CaptureDependencyItem")
...
def capture_dependency_item(env, node, child):
    '''-------------------------------------------------------------------------
       capture_dependency_info

       Capture a single dependency item
    -------------------------------------------------------------------------'''
    env_subst = env.subst
    img_env = env.get('IMAGE_ENV')

    if img_env is not None:
        dependency_tree_data_dict = img_env.get('DEPENDENCY_TREE_DATA')
        tgt_root = img_env.get('TARGET_ROOT')

        if dependency_tree_data_dict is not None:
            key = \
                str(node).replace(os.sep, '/').replace(tgt_root, '').strip('/')
            value = \
                str(child).replace(os.sep, '/').replace(tgt_root, '').strip('/')
            if dependency_tree_data_dict.has_key(key) == False:
                dependency_tree_data_dict[key] = []
            if value not in dependency_tree_data_dict[key]:
                dependency_tree_data_dict[key].append(value)
...
def analytic_builder(target, source, env):
    ...
    json_dict["DependencyTree"].update(dependency_tree_data_dict)
