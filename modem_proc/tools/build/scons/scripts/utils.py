...
#------------------------------------------------------------------------------
#
#  $Header: //commercial/MPSS.JO.1.2.r3.1/Main/modem_proc/tools/build/scons/scripts/utils.py#2 $
#  $DateTime: 2020/07/29 05:06:42 $
#  $Author: svadaval $
#  $Change: 25171529 $
...
def generate(env):
    ...
    env.AddMethod(follow_dependency_info, "FollowDependencyInfo")
...
def add_library(env, targets, library, sources, pack=False, \
    ...
    if is_key_enable(env, targets):
        ...
        if 'USES_BINARY_DIST' in env:
            ...
            if is_build_internal(env, args.get('PACK_EXCEPTION')):
                library_node = env.Library(library, sources)
            elif cleanpack:
...
def follow_dependency_info(env, target):
    '''-------------------------------------------------------------------------
    capture_dependency_info

    recursive function to follow dependency info from top to bottom
    -------------------------------------------------------------------------'''
    if isinstance(target, list):
        for item in target:
            if isinstance(item, list):
                env.FollowDependencyInfo(item)
            elif isinstance(item, SCons.Node.NodeList):
                env.FollowDependencyInfo(item)
    elif isinstance(target, SCons.Node.NodeList):
        for node in target:
            env.FollowDependencyInfo(node)
    else:
        for child in target.children():
            env.CaptureDependencyItem(target, child)
            env.FollowDependencyInfo(child)
...
def bind_aliases_to_targets(env, target, alias_list=None):
    ...
    utility to bind image targets to image aliases
    '''
    dependency_tree = int(env.GetOption('dependency_tree'))
    ...
    if 'USES_ANALYTIC_DATA' in env and dependency_tree:
        env.FollowDependencyInfo(target)
