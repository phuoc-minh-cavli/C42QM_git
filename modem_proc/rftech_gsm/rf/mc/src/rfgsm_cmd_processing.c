...
/*===========================================================================
Copyright (c) 1999 - 2023 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/mc/src/rfgsm_cmd_processing.c#6 $
...
10/13/20   mpa     setting GSM and FSK mode
09/02/20   bpa     Fixing CW and KW
06/22/20   mpa     to fix switch case for fsk
06/26/19   mpa     To disable histogram profiling through macro
06/24/19   mpa     Enable HSITOGRAM profiling on script building times
04/01/19   krk     Moving WBEE thr and support flag update to enter mode req from wakeup req
12/20/18   mpa     Dummy checkin
12/14/18   mpa     PSM changes
12/12/18   hkm     WBEE changes for NBJDET
08/27/18   mpa     changes to support SPLL
06/11/18   sk      OFT Fixes
03/01/18   mpa     to send ip2_cal  as in false in entermode confirmation
02/28/18   mpa     removal of drx support
02/28/18   mpa     To disable ip2_cal
...
extern prof_hist_param_t rx_gsm_burst_build_scripts_hist;
...
static msgr_umid_type rfgsm_umid_req_list[] = 
{
  ...
  RFA_RF_GSM_PSM_ENTRY_IND,
  RFA_RF_GSM_FSK_ENTER_MODE_REQ,
  RFA_RF_GSM_FSK_TX_ENABLE_REQ,
  RFA_RF_GSM_FSK_TX_BAND_REQ,
  RFA_RF_GSM_FSK_TX_BURST_REQ,
  RFA_RF_GSM_FSK_TX_DISABLE_REQ,
  RFA_RF_GSM_FSK_TX_SLEEP_REQ,
  RFA_RF_GSM_FSK_EXIT_MODE_REQ,
...
static msgr_umid_type rfgsm_umid_req_sub2_list[] = 
{
  ...
  RFA_RF_GSM_PSM_ENTRY_SUB2_IND,
  RFA_RF_GSM_FSK_ENTER_MODE_SUB2_REQ,
  RFA_RF_GSM_FSK_TX_ENABLE_SUB2_REQ,
  RFA_RF_GSM_FSK_TX_BURST_SUB2_REQ,
  RFA_RF_GSM_FSK_TX_SLEEP_SUB2_REQ,
  RFA_RF_GSM_FSK_EXIT_MODE_SUB2_REQ,  
};
...
static msgr_umid_type rfgsm_umid_req_sub3_list[] = 
{
  ...
  RFA_RF_GSM_PSM_ENTRY_SUB3_IND,
  RFA_RF_GSM_FSK_ENTER_MODE_SUB3_REQ,
  RFA_RF_GSM_FSK_TX_ENABLE_SUB3_REQ,
  RFA_RF_GSM_FSK_TX_BURST_SUB3_REQ,
  RFA_RF_GSM_FSK_TX_SLEEP_SUB3_REQ,
  RFA_RF_GSM_FSK_EXIT_MODE_SUB3_REQ,  
};
                                 ...
                                 rfcom_device_enum_type rfm_dev )
{
  ...
  switch( dispatch_result )
  {
    case RFGSM_DISPATCH_INVALID_DEVICE:
      ERR_FATAL("GL1 passed RFGSM invalid device ID %d for UMID 0x%08 on sub %d", rfm_dev, umid, rf_task);
      break;

    case RFGSM_DISPATCH_NULL_INPUT_ARG:
      ERR_FATAL("GL1 passed RFGSM NULL input pointer for UMID 0x%08 on sub %d device ID %d", umid, rf_task, rfm_dev);
      break;
                                                        ...
                                                        uint32 umid_cnf )
{
      ...
      rfgsm_msg_ptr->enter_mode == RFGSM_PRX_WTR_DED_HOP_ENTER_MODE ) 
  {
    ...
    if ( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
    {
	  ...
	  rfgsm_core_data.rfgsm_core_handle_ptr->curr_stack=RFGSM_GSM_MODE;
      ...
      if( rfgsm_mc_enter_mode( &rfgsm_core_data,
                               ...
                               rfgsm_msg_ptr->timing_info, 
                               rfgsm_msg_ptr->nb_jdet_thr) == RFGSM_MC_SUCCESS )
      {
        /* RFM mode is now GSM */
  ...
  if(rfgsm_msg_ptr->cnf_required)
  {
   ...
   #else
    rfgsm_resp_msg.ip2_cal_supported        =     FALSE;
   #endif
                                                    ...
                                                    uint32 umid_cnf )
{
  ...
  if (rfgsm_msg_ptr->wakeup_mode == RFGSM_PRX_DRX_WAKEUP_MODE) 
  {
    ...
    rfgsm_core_data.prx_drx_combined_mode = TRUE;
  #else 
    rfgsm_core_data.prx_drx_combined_mode = FALSE; //FORCEFULLY SETTING TO FALSE
  #endif
  }
                                                      ...
                                                      uint32 umid_cnf )
{
  ...
  if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
  {
    ...
    {
      RFGC_T_MSG_5( MSG_LEGACY_HIGH, 
                    "RF GSM Cmd Proc Rx Burst prx_dev=%d, drx_dev=%d, num bursts=%d,GL1 lower ccs prio=%d, cnf=%d",
                    rfgsm_msg_ptr->device,
    ...
    {
	  RFGC_T_MSG_1( MSG_LEGACY_ERROR, 
                    "RF GSM Cmd Proc Rx Burst diversity has been enabled %d and disabling forcibly", 
    ...
    rfgsm_core_handle_ptr->hmsc_active = rfgsm_msg_ptr->hmsc_active;
    rfgsm_core_data.rfgsm_core_handle_ptr->nb_jdet_detected = rfgsm_msg_ptr->nb_jdet_req.nb_jdet_detected;
	rfgsm_core_data.rfgsm_core_handle_ptr->curr_stack=RFGSM_GSM_MODE;

    RFGC_T_MSG_8( MSG_LEGACY_HIGH, 
                    "RF GSM Cmd Proc Rx Burst prx_dev=%d num bursts=%d,arfcn= %d , band= %d, freq_err= %d burst_type=%d, nb_jdet_det %d hmsc_active %d", 
                    rfgsm_msg_ptr->device,
                    rfgsm_msg_ptr->rx_burst_params->num_rx_bursts,
                    rfgsm_msg_ptr->rx_burst_params->rx_chan_params[0].arfcn,
                    rfgsm_msg_ptr->rx_burst_params->rx_chan_params[0].band,
                    rfgsm_msg_ptr->rx_burst_params->rx_chan_params[0].freq_err,
                    rfgsm_msg_ptr->rx_burst_params->rx_chan_params[0].rx_burst_type,
                    rfgsm_msg_ptr->nb_jdet_req.nb_jdet_detected,
                    rfgsm_msg_ptr->hmsc_active );
    ...
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr,             //core handle
                     ...
                     ( (rfgsm_msg_ptr->rx_burst_params->num_rx_bursts & 0xF) | 
                       (rfgsm_msg_ptr->rx_diversity_device & 0xF) << 4 | 0),
                     geran_test_mode_api_get_FN(rfgsm_core_data.as_id), //SW frame num
                     start_ustmr,                       //start time USTMR
                     duration_us,                       //duration us
                     start_pcycle,                      //start time pcycles
  ...
  if (RFGSM_RX_MON_BURST == rfgsm_msg_ptr->rx_burst_params->rx_chan_params[0].rx_burst_type)
	...
	{
	  /*updating the histogram for rx */
	  prof_hist_update(&rx_gsm_burst_build_scripts_hist, duration_us);
  ...
  if (prof_hist_log_periodic(&gsm_rf_hist_packet_handle))
	{
		...
		prof_hist_clear(&rx_gsm_burst_build_scripts_hist);
      ...
      rfgsm_msg_ptr->exit_mode == RFGSM_PRX_DRX_EXIT_MODE ) 
  {
    ...
    if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
    {
      ...
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     ...
                     start_pcycle,            //start time pcycles
                     0 );                     //duration pcycles
      rfgsm_mc_set_psm_band(rfgsm_core_handle_ptr->band);
...
#ifdef PSM_DEBUG
  if(rfgsm_mc_store_offload_band_info()!=RFGSM_OFFLOAD_ERROR)
	{
  
	  RFGC_T_MSG_2( MSG_LEGACY_HIGH, 
					"RF GSM Cmd Proc PSM IND, band=%d, cnf=%d", 
					rfgsm_mc_get_psm_band(),
					rfgsm_msg_ptr->cnf_required );
	}
	else
	{
	   ...
	   RFGC_T_MSG_2( MSG_LEGACY_HIGH, 
					"RF GSM Cmd Proc PSM IND failed for band=%d, cnf=%d", 
					rfgsm_mc_get_psm_band(),
					rfgsm_msg_ptr->cnf_required );
	} 
#endif
...
rfgsm_dispatch_outcome_t rfgsm_dispatch_psm_entry_ind (rfa_rf_gsm_psm_entry_ind_s* rfgsm_msg_ptr, 
                                                         rf_task_num_type rf_task)
{
  ...
  {
    result=RFGSM_DISPATCH_UNSUCCESSFUL;
     RFGC_T_MSG_2( MSG_LEGACY_HIGH, 
                  "RF GSM Cmd Proc PSM IND failed for band=%d, cnf=%d", 
                  rfgsm_mc_get_psm_band(),
                  rfgsm_msg_ptr->cnf_required );
...
/*===========================================================================
  ...
  FUNCTION:  rfgsm_dispatch_fsk_enter_mode_req

===========================================================================*/
/*! @fn rfgsm_dispatch_fsk_enter_mode_req( rfa_rf_gsm_enter_mode_req_s* rfgsm_msg_ptr )
 *  @brief This function dispatches the initialisation of the RF Driver for 
 *  a specific radio transceiver for GSM.
 *  @param rfgsm_msg_ptr a pointer to the enter_mode structure variable
 *  payload.
 ...
 *  failure of the function.
 */
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_enter_mode_req( rfa_rf_gsm_enter_mode_req_s* rfgsm_msg_ptr, 
                                                        rf_task_num_type rf_task,
                                                        uint32 umid_cnf )
{
  /* ----------------------------------------------------------------------- */
...

  {
    ...
    else
    {
	  ...
	  {
         rfgsm_core_handle_ptr->curr_stack=RFGSM_FSK_MODE;
	  }
    }
    ...
    {
      RFGC_T_MSG_4( MSG_LEGACY_HIGH, 
                    "RF GSM Cmd Proc FSK Enter Mode, dev_id=%d, shared_mem_ptr=0x%x,rfgsm_msg_ptr->rxlm_buf_index.prx_idx=%d cnf=%d",
                    rfgsm_msg_ptr->prx_device,
                    rfgsm_msg_ptr->init_writes_buff_ptr,
                    rfgsm_msg_ptr->rxlm_buf_idx,
                    rfgsm_msg_ptr->cnf_required );
      ...
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                       RFA_RF_GSM_FSK_ENTER_MODE_REQUEST_START, //api
                       rf_task,               //rf task
                       start_time_qs,         //command proc start qs
                       0,                     //arg1
                       0,                     //arg2
      ...
      rfgsm_core_data.rfgsm_core_handle_ptr->curr_stack=RFGSM_FSK_MODE ;
      /* Enter GSM mode */
      if( rfgsm_mc_fsk_enter_mode( &rfgsm_core_data,
                               rfgsm_msg_ptr->rxlm_buf_idx, 
                               (void * const)rfgsm_msg_ptr->init_writes_buff_ptr,
                               rfgsm_msg_ptr->timing_info, 
                               rfgsm_msg_ptr->nb_jdet_thr) == RFGSM_MC_SUCCESS )
  ...
  if (cmd_result==RFA_RF_GSM_SUCCESS) 
  {
    ...
    if(dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
    {
      ...
      /* Log the RFM_DEVICE, RXLM BUFFER IDX and execution time in pcycles to the PRX device log */
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr,     //core handle
                       RFA_RF_GSM_FSK_ENTER_MODE_REQUEST_STOP, //api
                       rf_task,                   //rf task
                       start_time_qs,             //command proc start qs
                       rfgsm_msg_ptr->prx_device, //arg1
                       0,                         //arg2
  ...
  if(rfgsm_msg_ptr->cnf_required)
  {
    ...
    if (msgr_send_status != E_SUCCESS)
    {
      RFGC_T_MSG_2(MSG_LEGACY_ERROR, "RFGSM FSK Enter Mode MSGR Send UNSUCCESSFUL, error_no %d umid_cnf 0x%x",msgr_send_status,umid_cnf);
	  ASSERT(0);
    }
  }
...
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_tx_enable_req( rfa_rf_gsm_tx_enable_req_s* rfgsm_msg_ptr, 
                                                         rf_task_num_type rf_task,
                                                         uint32 umid_cnf)
{
  /* ----------------------------------------------------------------------- */
  ...
  {   
    ...
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     RFA_RF_GSM_FSK_TX_ENABLE_REQUEST_START, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
    ...
    if( rfgsm_mc_fsk_enable_tx( &rfgsm_core_data,
                            rfgsm_msg_ptr->txlm_buf_idx ) == RFGSM_MC_SUCCESS )
    {
      cmd_result = RFA_RF_GSM_SUCCESS;
    }
    ...
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle 
                     RFA_RF_GSM_FSK_TX_ENABLE_REQUEST_STOP, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
 ...
 *  failure of the function.
 */
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_tx_band_req( rfa_rf_gsm_set_tx_band_req_s* rfgsm_msg_ptr, 
                                                         rf_task_num_type rf_task,
                                                         uint32 umid_cnf)
{
  /* ----------------------------------------------------------------------- */
  ...
  if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
  {
    ...
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     RFA_RF_GSM_FSK_TX_BAND_REQUEST_START, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
    ...
    if(rfgsm_mc_fsk_tx_band(&rfgsm_core_data, 
                             rfgsm_msg_ptr->band, 
                             rfgsm_msg_ptr->sbi_set_tx_band_buff_ptr) == RFGSM_MC_SUCCESS )
    {
      cmd_result = RFA_RF_GSM_SUCCESS;
    ...
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     RFA_RF_GSM_FSK_TX_BAND_REQUEST_STOP, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
  ...
  {    
    RFGC_T_MSG_1( MSG_LEGACY_ERROR, "RF GSM Set FSK Tx Enable dispatcher failed variable verification for device %d", rfgsm_msg_ptr->device);
...
/*===========================================================================
  ...
  FUNCTION:  rfgsm_dispatch_fsk_tx_start_req
 ...
 *  failure of the function.
 */
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_tx_burst_req( rfa_rf_fsk_tx_burst_req_s* rfgsm_msg_ptr, 
                                                      rf_task_num_type rf_task,
                                                      uint32 umid_cnf )
{
  /* ----------------------------------------------------------------------- */
  ...
  rfgsm_dispatch_outcome_t      dispatch_result = RFGSM_DISPATCH_SUCCESSFUL;
  rfa_rf_gsm_cmd_outcome_t      cmd_result = RFA_RF_GSM_EXECUTION_ERROR;
  rfa_rf_fsk_tx_burst_cnf_s     rfgsm_resp_msg;
  errno_enum_type               msgr_send_status = E_FAILURE;
  rfgsm_core_handle_type        *rfgsm_core_handle_ptr = NULL;
  rfgsm_core_data_type          rfgsm_core_data;
  ...
  {
    RFGC_T_MSG_7( MSG_LEGACY_HIGH, 
                  "RF GSM Cmd Proc FSK Tx start, dev_id=%d, rf_freq_offset=%d, band=%d, channel freq=%d, freq err=%d, shared_mem_ptr=0x%x, cnf=%d",
                  rfgsm_msg_ptr->device, 
                  rfgsm_msg_ptr->tx_burst_params->tx_chan_params.rf_freq_offset,
                  rfgsm_msg_ptr->tx_burst_params->tx_chan_params.band, 
                  rfgsm_msg_ptr->tx_burst_params->tx_chan_params.rf_freq,
                  rfgsm_msg_ptr->tx_burst_params->tx_chan_params.freq_err,
                  rfgsm_msg_ptr->tx_burst_params->tx_burst_event_ptr,
                  rfgsm_msg_ptr->cnf_required );
    ...
    rfgsm_core_data.triple_buffer_id = rfgsm_msg_ptr->buffer_id;
    rfgsm_core_data.rfgsm_core_handle_ptr = rfgsm_core_handle_ptr;
	//rfgsm_core_data.rfgsm_core_handle_ptr->rf_fsk_tx_fc=rfgsm_msg_ptr->tx_burst_params->tx_chan_params.rf_freq;
  
  fc_residual_freq_in_hz = rfgsm_msg_ptr->tx_burst_params->tx_chan_params.rf_freq % 1000;
  
	rfgsm_core_data.rfgsm_core_handle_ptr->rf_fsk_offset_hz=(rfgsm_msg_ptr->tx_burst_params->tx_chan_params.rf_freq_offset)*1000;
  rfgsm_core_data.rfgsm_core_handle_ptr->rf_fsk_tx_fc_residual_freq_in_hz = fc_residual_freq_in_hz; /*the residual Freq in Hz will be added to the freq offset as the SDR API accepts the Fc in Khz */

	rfgsm_core_data.rfgsm_core_handle_ptr->curr_stack=RFGSM_FSK_MODE;
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     RFA_RF_GSM_FSK_TX_START_REQUEST_START, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
    ...
    if( rfgsm_mc_process_fsk_tx_burst( &rfgsm_core_data,
                                   rfgsm_msg_ptr->tx_burst_params ) == RFGSM_MC_SUCCESS )
    {
      cmd_result = RFA_RF_GSM_SUCCESS;
    }
	...
	duration_us=rfgsm_ustmr_count_diff_us_rtn(start_ustmr);							   
    RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle 
                     RFA_RF_GSM_FSK_TX_START_REQUEST_STOP, //api
                     rf_task,               //rf task
                     start_time_qs,         //command proc start qs
                     0,                     //arg1
                     0,                     //arg2
  ...
  {
    RFGC_T_MSG_1( MSG_LEGACY_ERROR, "RF GSM FSK Tx start dispatcher failed variable verification for device %d", rfgsm_msg_ptr->device);
  ...
  if(rfgsm_msg_ptr->cnf_required)
  {
    ...
    msgr_send_status = rfgsm_send_msgr_cnf((msgr_hdr_struct_type *)&rfgsm_resp_msg,
                                            umid_cnf,
                                            sizeof(rfa_rf_fsk_tx_burst_cnf_s));

    if (msgr_send_status != E_SUCCESS)
    {
      RFGC_T_MSG_1(MSG_LEGACY_ERROR, "RFGSM FSK Tx start MSGR Send UNSUCCESSFUL, error_no %d",msgr_send_status);
    }
  }

  return(dispatch_result);
}
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_tx_disable_req( rfa_rf_gsm_tx_disable_req_s* rfgsm_msg_ptr, 
                                                         rf_task_num_type rf_task,
                                                         uint32 umid_cnf)
{
  /* ----------------------------------------------------------------------- */
	...
	if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
	{
	  ...
	  RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
					   RFA_RF_GSM_FSK_TX_DISABLE_REQUEST_START, //api
					   rf_task, 			  //rf task
					   start_time_qs,		  //command proc start qs
					   0,					  //arg1
					   0,					  //arg2
	  ...
	  if( rfgsm_mc_fsk_disable_tx(&rfgsm_core_data,
							  rfgsm_msg_ptr->txlm_buf_idx) == RFGSM_MC_SUCCESS )
	  {
		cmd_result = RFA_RF_GSM_SUCCESS;
	  }
	  ...
	  RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
					   RFA_RF_GSM_FSK_TX_DISABLE_REQUEST_STOP, //api
					   rf_task, 			  //rf task
					   start_time_qs,		  //command proc start qs
					   0,					  //arg1
					   0,					  //arg2
	...
	{	 
	  RFGC_T_MSG_1( MSG_LEGACY_ERROR, "RF GSM FSK Tx disable dispatcher failed variable verification for device %d", rfgsm_msg_ptr->device);
...
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_tx_sleep_req( rfa_rf_gsm_sleep_req_s* rfgsm_msg_ptr, 
                                                   rf_task_num_type rf_task,
                                                   uint32 umid_cnf )
{
      ...
      || rfgsm_msg_ptr->sleep_mode == RFGSM_PRX_DRX_SLEEP_MODE ) 
  {
    ...
    if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
    {
      ...
      else
      {
        cmd_result =   RFA_RF_GSM_EXECUTION_ERROR;
        prx_device_cmd_result = RFA_RF_GSM_EXECUTION_ERROR;
        RFGC_T_MSG(MSG_LEGACY_ERROR, "RF GSM FSK Sleep cmd failed for prx device");
      }
	  if( rfgsm_mc_exit_mode(&rfgsm_core_data) == RFGSM_MC_SUCCESS )
      {
        cmd_result = RFA_RF_GSM_SUCCESS;
      ...
      {
        RFGC_T_MSG_1(MSG_LEGACY_ERROR, "RF GSM FSK Exit Mode cmd failed for dev : %d", rfgsm_core_data.rfm_dev );
        cmd_result = RFA_RF_GSM_EXECUTION_ERROR;
        prx_device_cmd_result = RFA_RF_GSM_EXECUTION_ERROR;
      }
    }
  ...
  if(rfgsm_msg_ptr->sleep_mode == RFGSM_DRX_SLEEP_MODE ||
     rfgsm_msg_ptr->sleep_mode == RFGSM_PRX_DRX_SLEEP_MODE )
  {
    if(dispatch_result == RFGSM_DISPATCH_SUCCESSFUL && rfgsm_core_handle_ptr_drx != NULL)
    {
      /* Log the RFM_DEVICE, RXLM BUFFER IDX and execution time in pcycles to the PRX device log */
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr_drx, //core handle
                       RFA_RF_GSM_RX_SLEEP_REQUEST_STOP, //api
...
rfgsm_dispatch_outcome_t rfgsm_dispatch_fsk_exit_mode_req( rfa_rf_gsm_exit_mode_req_s* rfgsm_msg_ptr, 
                                                       rf_task_num_type rf_task,
                                                       uint32 umid_cnf )
{	
...
/* ----------------------------------------------------------------------- */
  ...
  rfgsm_core_handle_type        *rfgsm_core_handle_ptr_drx = NULL;
  rfgsm_core_data_type          rfgsm_core_data;
  rfgsm_core_data_type          rfgsm_sleep_core_data;
  uint32 duration_us=0;
   ...
   if( !rfgsm_dispatch_check_device( rfgsm_msg_ptr->prx_device, rfgsm_msg_ptr->hdr.id ) )
	...
	/* PRX SLEEP Execution													   */
	 ...
	 if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
	 {
	   ...
	   /* Set the core data */
	   rfgsm_sleep_core_data.rf_task = rf_task;
	   rfgsm_sleep_core_data.rfm_dev = rfgsm_msg_ptr->prx_device;
	   rfgsm_sleep_core_data.sub_id = rfgsm_msg_ptr->sub_id;
	   rfgsm_sleep_core_data.as_id = gl1_hw_rf_map_rf_sub_id_to_as_id(rfgsm_msg_ptr->sub_id);
	   rfgsm_sleep_core_data.triple_buffer_id = RFLM_GSM_BUFFER_ID_NOT_REQUIRED;
	   rfgsm_sleep_core_data.rfgsm_core_handle_ptr = rfgsm_core_handle_ptr;
	   rfgsm_sleep_core_data.device_is_tx = rfgsm_core_get_device_tx_capability(rfgsm_msg_ptr->prx_device);
	   ...
	   if( rfgsm_mc_sleep(&rfgsm_sleep_core_data) == RFGSM_MC_SUCCESS )
	   {
		 /*rfgsm_mc function successful*/
		 cmd_result = RFA_RF_GSM_SUCCESS;
		 prx_device_cmd_result = RFA_RF_GSM_SUCCESS;
    ...
    if( dispatch_result == RFGSM_DISPATCH_SUCCESSFUL )
    {
      ...
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle
                     RFA_RF_GSM_FSK_EXIT_MODE_REQUEST_START, //api
                     rf_task,                 //rf task
                     start_time_qs,           //command proc start qs
                     0,                       //arg1
                     0,                       //arg2
...
#if 0
      RFGSM_DEBUG_EN_Q(rfgsm_core_handle_ptr, //core handle 
                       RFA_RF_GSM_FSK_EXIT_MODE_REQUEST_STOP, //api
                       rf_task,               //rf task
                       start_time_qs,         //command proc start qs
                       0,                     //arg1
                       0,                     //arg2
                       geran_test_mode_api_get_FN(rfgsm_core_data.as_id), //SW frame num
                       start_ustmr,           //start time USTMR
                       0,//rfgsm_ustmr_count_diff_us_rtn(start_ustmr), //duration us
                       start_pcycle,          //start time pcycles
                       0);//qurt_get_core_pcycles()-start_pcycle ); //duration pcycles
#endif
    }
    else /* dispatch_result != RFGSM_DISPATCH_SUCCESSFUL */
    {
...
rfgsm_dispatch_outcome_t rfgsm_req_dispatch(rfgsm_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    case RFA_RF_GSM_PROCESS_BURST_METRICS_REQ:
      ...
      break;
    case RFA_RF_GSM_PSM_ENTRY_IND:
	  req_cnf=rfgsm_dispatch_psm_entry_ind((rfa_rf_gsm_psm_entry_ind_s*)msg_ptr,RF_TASK_1);
	  break;
#ifdef RFGSM_FSK_ENABLE
    case RFA_RF_GSM_FSK_ENTER_MODE_REQ:
	 req_cnf=rfgsm_dispatch_fsk_enter_mode_req((rfa_rf_gsm_enter_mode_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_ENTER_MODE_CNF);
	 break;
    case RFA_RF_GSM_FSK_TX_ENABLE_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_enable_req((rfa_rf_gsm_tx_enable_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_TX_ENABLE_CNF);
	 break;
	case RFA_RF_GSM_FSK_TX_BAND_REQ:
	 req_cnf=rfgsm_dispatch_fsk_tx_band_req((rfa_rf_gsm_set_tx_band_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_TX_BAND_CNF);
     break;
    case RFA_RF_GSM_FSK_TX_BURST_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_burst_req((rfa_rf_fsk_tx_burst_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_TX_BURST_CNF);
	 break;
	 case RFA_RF_GSM_FSK_TX_DISABLE_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_disable_req((rfa_rf_gsm_tx_disable_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_TX_DISABLE_CNF);
	 break;
    case RFA_RF_GSM_FSK_TX_SLEEP_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_sleep_req((rfa_rf_gsm_sleep_req_s *)msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_TX_SLEEP_CNF);
	 break;
    case RFA_RF_GSM_FSK_EXIT_MODE_REQ:
  	req_cnf=rfgsm_dispatch_fsk_exit_mode_req((rfa_rf_gsm_exit_mode_req_s *) msg_ptr, RF_TASK_1, RFA_RF_GSM_FSK_EXIT_MODE_CNF);
	 break;
#endif
    default :
...
rfgsm_dispatch_outcome_t rfgsm_req_dispatch_2(rfgsm_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    case RFA_RF_GSM_PROCESS_BURST_METRICS_SUB2_REQ:
      ...
      break;
    case RFA_RF_GSM_PSM_ENTRY_SUB2_IND:
	  req_cnf=rfgsm_dispatch_psm_entry_ind((rfa_rf_gsm_psm_entry_ind_s*)msg_ptr,RF_TASK_2);
#ifdef RFGSM_FSK_ENABLE
	case RFA_RF_GSM_FSK_ENTER_MODE_SUB2_REQ:
		 req_cnf=rfgsm_dispatch_fsk_enter_mode_req((rfa_rf_gsm_enter_mode_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_ENTER_MODE_SUB2_CNF);
		 break;
	case RFA_RF_GSM_FSK_TX_BAND_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_band_req((rfa_rf_gsm_set_tx_band_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_TX_BAND_SUB2_CNF);
	    break;
	case RFA_RF_GSM_FSK_TX_ENABLE_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_enable_req((rfa_rf_gsm_tx_enable_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_TX_ENABLE_SUB2_CNF);
	 break;
	case RFA_RF_GSM_FSK_TX_BURST_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_burst_req((rfa_rf_fsk_tx_burst_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_TX_BURST_SUB2_CNF);
	    break;
	case RFA_RF_GSM_FSK_TX_DISABLE_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_disable_req((rfa_rf_gsm_tx_disable_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_TX_DISABLE_SUB2_CNF);
	 break;
	case RFA_RF_GSM_FSK_TX_SLEEP_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_sleep_req((rfa_rf_gsm_sleep_req_s *)msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_TX_SLEEP_SUB2_CNF);
        break;
	case RFA_RF_GSM_FSK_EXIT_MODE_SUB2_REQ:
		req_cnf=rfgsm_dispatch_fsk_exit_mode_req((rfa_rf_gsm_exit_mode_req_s *) msg_ptr, RF_TASK_2, RFA_RF_GSM_FSK_EXIT_MODE_SUB2_CNF);
		 break;
#endif
    default :
...
rfgsm_dispatch_outcome_t rfgsm_req_dispatch_3(rfgsm_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    case RFA_RF_GSM_PROCESS_BURST_METRICS_SUB3_REQ:
      ...
      break;
    case RFA_RF_GSM_PSM_ENTRY_SUB3_IND:
	  req_cnf=rfgsm_dispatch_psm_entry_ind((rfa_rf_gsm_psm_entry_ind_s*)msg_ptr,RF_TASK_3);
	  
#ifdef RFGSM_FSK_ENABLE
	case RFA_RF_GSM_FSK_ENTER_MODE_SUB3_REQ:
		 req_cnf=rfgsm_dispatch_fsk_enter_mode_req((rfa_rf_gsm_enter_mode_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_ENTER_MODE_SUB3_CNF);
		 break;
	case RFA_RF_GSM_FSK_TX_ENABLE_SUB3_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_enable_req((rfa_rf_gsm_tx_enable_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_TX_ENABLE_SUB3_CNF);
	    break;
	case RFA_RF_GSM_FSK_TX_BAND_SUB3_REQ:
	 req_cnf=rfgsm_dispatch_fsk_tx_band_req((rfa_rf_gsm_set_tx_band_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_TX_BAND_SUB3_CNF);
     break;
	case RFA_RF_GSM_FSK_TX_BURST_SUB3_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_burst_req((rfa_rf_fsk_tx_burst_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_TX_BURST_SUB3_CNF);
	    break;
	case RFA_RF_GSM_FSK_TX_SLEEP_SUB3_REQ:
		req_cnf=rfgsm_dispatch_fsk_tx_sleep_req((rfa_rf_gsm_sleep_req_s *)msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_TX_SLEEP_SUB3_CNF);
        break;
	case RFA_RF_GSM_FSK_TX_DISABLE_SUB3_REQ:
	 req_cnf=rfgsm_dispatch_fsk_tx_disable_req((rfa_rf_gsm_tx_disable_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_TX_DISABLE_SUB3_CNF);
     break;
	case RFA_RF_GSM_FSK_EXIT_MODE_SUB3_REQ:
		req_cnf=rfgsm_dispatch_fsk_exit_mode_req((rfa_rf_gsm_exit_mode_req_s *) msg_ptr, RF_TASK_3, RFA_RF_GSM_FSK_EXIT_MODE_SUB3_CNF);
		 break;
#endif
    default :
