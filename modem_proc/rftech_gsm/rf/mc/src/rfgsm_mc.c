...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/mc/src/rfgsm_mc.c#3 $
...
rfcom_gsm_band_type debug_band=RFCOM_NUM_GSM_BANDS;
...
static rfcom_gsm_band_type exit_band=RFCOM_NUM_GSM_BANDS;

extern uint32 nb_jdet_thr_g0;
extern uint32 nb_jdet_thr_g1;
                                         ...
                                         rfgsm_nb_jdet_threshold *nb_jdet_thr)
{
  ...
  if( mc_status != RFGSM_MC_SUCCESS )
  {
    ...
    /* Update the WBEE thr info to GL1*/
     if(nb_jdet_thr!=NULL)
  {
    //Update to L1 about the thresholds.
    for( uint8 i=RFCOM_BAND_GSM850; i< RFCOM_NUM_GSM_BANDS; i++)
    { 
      nb_jdet_thr->wbee_nb_jdet_threshold_g0[i] = nb_jdet_thr_g0;
      nb_jdet_thr->wbee_nb_jdet_threshold_g1[i] = nb_jdet_thr_g1;
    }  
    nb_jdet_thr->is_nb_jdet_supported = TRUE; //ALWAYS true for CT target.
    core_data->rfgsm_core_handle_ptr->is_nb_jdet_supported = TRUE;
  }
  else
  {
    RF_MSG_1(MSG_LEGACY_ERROR,"nb_jdet_thr is NULL", 0);
  }
                                        ...
                                        rfgsm_timing_info_type *timing_info )
{
  ...
  rfgsm_mc_status_type cm_enter_status = RFGSM_MC_SUCCESS;
#if 0
  /* Temporarily assign until core layer deprecation of core handle var */
  core_data->rfgsm_core_handle_ptr->buffer_id = core_data->triple_buffer_id;
...
rfgsm_mc_status_type rfgsm_mc_cm_exit( rfgsm_core_data_type *core_data )
{
  ...
  rfgsm_mc_status_type cm_exit_status = RFGSM_MC_SUCCESS;
#if 0
  /* Temporarily assign until core layer deprecation of core handle var */
  core_data->rfgsm_core_handle_ptr->buffer_id = core_data->triple_buffer_id;
...
rfgsm_mc_status_type rfgsm_mc_exit_mode(rfgsm_core_data_type *core_data)
{
  ...
  {
   if (!rfgsm_core_fsk_exit_mode(core_data, RFM_INVALID_MODE))
   {
    RF_MSG( MSG_LEGACY_ERROR, "rfgsm_mc_exit_mode() Failed for fsk");
    /* Release core lock */
    rf_common_leave_critical_section(&core_data->rfgsm_core_handle_ptr->rfgsm_core_enter_exit_crit_sect);
    return RFGSM_MC_CORE_API_FAILURE;
   }
                                            ...
                                            rfgsm_rx_burst_type rx_burst_type)
{
  ...
  switch ( synths )
  {
    ...
    case RFM_TUNE_RX:
        tune_to_chan_status &= rfgsm_core_tune_rx(core_data, channel,rx_burst_type );
        break;
                                           ...
                                           const void *shared_mem_ptr)
{
  ...
  rfgsm_core_set_tx_band(core_data, rfcom_band, (void*)shared_mem_ptr);
...
rfgsm_mc_status_type rfgsm_mc_update_freq_error( rfgsm_core_data_type *core_data, int32 freq_err, rfgsm_freq_err_type freq_err_type )
{
      ...
      (ftm_get_mode() == FTM_MODE && (rfgsm_core_get_gl1_freq_error_flag(core_data->rfgsm_core_handle_ptr) == TRUE)) )
  {
    ...
    {
      rfgsm_core_update_freq_error(core_data, freq_err);
    }
...
#ifdef RFGSM_FSK_ENABLE
rfgsm_mc_status_type rfgsm_mc_fsk_enable_tx( rfgsm_core_data_type *core_data,
                                         uint32 txlm_buf_idx )
{
  /* Temporarily assign until core layer deprecation of core handle var */
  core_data->rfgsm_core_handle_ptr->buffer_id = core_data->triple_buffer_id;
  ...
  /* Configure RFC for Tx, Setup up modem,  start temp comp, start vbatt comp */ 
  rfgsm_core_fsk_enable_tx(TRUE, txlm_buf_idx, core_data);
...
/*lint -esym(715, user_data_ptr, cb_handler) */ 
rfgsm_mc_status_type rfgsm_mc_fsk_enter_mode(rfgsm_core_data_type *core_data,
                                         uint32 txlm_buf_idx,
                                         const void *enter_mode_ptr,
                                         rfgsm_timing_info_type *timing_info,
                                         rfgsm_nb_jdet_threshold *nb_jdet_thr)
{
  ...
  /* Log the enter mode API call */
  RF_MSG( MSG_LEGACY_HIGH, "rfgsm_mc_fsk_enter_mode" );
  ...
  /* Execute core enter mode */
  if (!rfgsm_core_fsk_enter_mode(core_data, txlm_buf_idx, enter_mode_ptr))
  {
    RF_MSG( MSG_LEGACY_ERROR, "rfgsm_core_fsk_enter_mode() Failed" );
    return RFGSM_MC_CORE_API_FAILURE;
  }
...
/*
  ...
  @details
*/
rfgsm_mc_status_type rfgsm_mc_fsk_tx_band( rfgsm_core_data_type *core_data,
                                                      rfgsm_band_type tx_band,
                                                         const void *shared_mem_ptr)
{
  rfcom_gsm_band_type rfcom_band = RFCOM_NUM_GSM_BANDS;
  ...
  rfgsm_core_fsk_tx_band(core_data, rfcom_band, (void*)shared_mem_ptr);
...
rfgsm_mc_status_type rfgsm_mc_fsk_disable_tx( rfgsm_core_data_type *core_data,
                                          uint32 txlm_buf_idx )
{
  /* Temporarily assign until core layer deprecation of core handle var */
  core_data->rfgsm_core_handle_ptr->buffer_id = core_data->triple_buffer_id;
  ...
  /* Setup RFC to disable Tx, Stop temp comp, stop vbatt comp;  txlm_buffer_index NOT used */
  rfgsm_core_fsk_enable_tx(FALSE, 0, core_data);
...
*/
rfgsm_mc_status_type rfgsm_mc_process_fsk_tx_burst( rfgsm_core_data_type *core_data,
                                                rfa_rf_fsk_tx_burst_type *tx_burst_params )
{
  ...
  boolean rf_fc_tune_status=TRUE;
  ...
  for (index = 0; index < tx_burst_params->num_tx_slots; index++)
  {
    ...
    pwr_profiles[index].backoff[RFGSM_COEX_VBATT_BACKOFF] = tx_burst_params->tx_pwr_ctl_params[index].backoff[RFGSM_COEX_VBATT_BACKOFF];
     RF_MSG_2(MSG_LEGACY_ERROR,
             "index %d power_level %d", 
             index, pwr_profiles[index].power_level);
  ...
  if(mc_ret_status != RFGSM_MC_SUCCESS){ return mc_ret_status;}
     ...
     /*Convert the fc to Khz as GL1 passes the rf_freq in Hz*/
  
  tx_fc_in_khz = tx_burst_params->tx_chan_params.rf_freq/1000;

  /* Tune the UHF Tx synthesizers to the channel specified */
  rf_fc_tune_status = rfgsm_core_tune_fsk_tx(core_data,tx_fc_in_khz);
  if(rf_fc_tune_status==FALSE)
  {
  	  mc_ret_status=RFGSM_MC_CORE_API_FAILURE;
  }
  if(mc_ret_status != RFGSM_MC_SUCCESS){ return mc_ret_status;}
...
rfgsm_mc_status_type rfgsm_mc_sleep( rfgsm_core_data_type *core_data )
{
  ...
  {
    if (!rfgsm_core_fsk_sleep(core_data))
    {
      RF_MSG( MSG_LEGACY_ERROR, "rfgsm_core_fsk_sleep() Failed");
      return RFGSM_MC_CORE_API_FAILURE;
    }
  }
#endif
                                              ...
                                              void *tx_burst_event_ptr )
{
  ...
  {
   (void)rfgsm_core_setup_fsk_tx_burst(core_data, num_tx_slots, tx_burst_event_ptr );
  }
#endif
  RFGSM_DEBUG_EN_Q( core_data->rfgsm_core_handle_ptr, //core handle
 ...
 */
 void rfgsm_mc_jdet_config_rx_burst( rfgsm_core_data_type* core_data)
 {
  /* Time Profiling */
  unsigned long long start_pcycle = qurt_get_core_pcycles();
  uint32 start_ustmr = rfgsm_mc_read_ustmr_count();
...
/*!
  @brief
   ...
   @retval boolean - success/failure indication
*/
void rfgsm_mc_detune_rx_burst( rfgsm_core_data_type* core_data,
                                         ...
                                         rf_burst_type burst )
{
  ...
  if( !rfgsm_core_detune_rx_burst( core_data,
                                   enable_rx_diversity, //this will be false
                                                    burst))
  {
                                                ...
                                                boolean process_drx_burst)
{
  ...
  if(mc_ret_status == RFGSM_MC_SUCCESS)
  {
    mc_ret_status = rfgsm_mc_tune_to_chan( core_data, rx_chan_params[burst_num].arfcn, RFM_TUNE_RX, rx_chan_params[burst_num].rx_burst_type );
  }
                                                           ...
                                                           void **gfw_rf_burst_event_sm_ptrs )
{
  ...
  for( index = 0; index < num_rx_bursts; index++ )
  {
    ...
    mc_ret_status = rfgsm_mc_prepare_rx_burst(core_data,
                                              ...
                                              rx_chan_params,
                                              rxd_chan_params,
                                              process_drx_burst);//it is always FALSE
    ...
    rfgsm_mc_jdet_config_rx_burst( core_data);
    
	  /*Disable RX script building*/
	   rfgsm_mc_detune_rx_burst( core_data,
											   core_data->enable_rx_diversity,
...
{
  NULL, /* rfmeas_mc_irat_info_get_fp*/
  NULL, /* rfmeas_mc_enter_fp */
  NULL, /* rfmeas_mc_pre_config_fp */
  NULL, /* rfmeas_build_reset_scripts_fp */
  NULL, /* rfmeas_build_scripts_fp */
  NULL, /* rfmeas_mc_exit_fp */
};
...
boolean rfgsm_mc_retrieve_sawless_support(boolean support_matrix[RFM_MAX_WAN_DEVICES][RFGSM_MAX_UNIQUE_RFCOM_BANDS])
{
  ...
  {
    for ( j = 0 ; j < (RFCOM_NUM_GSM_BANDS) ; j++ )
    {
...
rfgsm_offload_status_type rfgsm_mc_store_offload_band_info()
{
  if(exit_band==RFCOM_NUM_GSM_BANDS)
  {
    RF_MSG(MSG_LEGACY_MED,"RFGSM store offload band info bypassed: invalid band passed");
    return RFGSM_OFFLOAD_INVALID_BAND_DATA;
  }
  if(rfgsm_core_store_offload_band_info(exit_band)==FALSE)
  {
    RF_MSG(MSG_LEGACY_ERROR,"RFGSM store offload band info FAILED");
    return RFGSM_OFFLOAD_ERROR;
  }
  return RFGSM_OFFLOAD_NO_ERROR;
}

void rfgsm_mc_nv_offload(rfcom_gsm_band_type band)
{

  debug_band=band;
  if(band>=RFCOM_NUM_GSM_BANDS)
  {
    RF_MSG_1(MSG_LEGACY_MED,"rfgsm_mc_nv_offload bypassed with band :%d",band);
	return;
  }
  rfgsm_core_nv_offload(band);
  RF_MSG_1(MSG_LEGACY_MED,"PSM rfgsm_mc_nv_offload was called with band :%d",band);
  return;
}

boolean rfgsm_mc_set_psm_band(rfcom_gsm_band_type band)
{
  exit_band=band;
  return TRUE;

}
rfcom_gsm_band_type rfgsm_mc_get_psm_band(void)
{
 return exit_band;
}
