...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/nv/src/rfgsm_nv_cmn.c#2 $
$DateTime: 2020/04/29 00:10:29 $ $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/24/20   mpa     clade compression changes
09/13/19   mpa     To read new ENV_GAIN V2 NVs 
09/13/19   mpa     To fix integer overflow in amam extrapolation 
...
01/28/19   mpa     Changes to support RGI52
...
/* RF LTE nv Memory allocation and relealse API's */

#ifndef FEATURE_CLADE_HEAP 
#define RFGSM_NV_MEM_ALLOC( size )                              \
           modem_mem_alloc(size , MODEM_MEM_CLIENT_RFA  ) 
#else
#define RFGSM_NV_MEM_ALLOC( size )                              \
           clade_heap_alloc( size ) 
#endif           

#ifndef FEATURE_CLADE_HEAP
#define RFGSM_NV_MEM_FREE(  alloc_ptr )                          \
           modem_mem_free( alloc_ptr, MODEM_MEM_CLIENT_RFA )
#else
#define RFGSM_NV_MEM_FREE( alloc_ptr )                          \
           clade_heap_free( alloc_ptr)
#endif
...
static uint8 offload_nv_band_mask=0;
static uint64 psm_band=0;
...
static const rfgsm_debug_nv_mask_info_type rfgsm_debug_nv_masks[RFGSM_DEBUG_NV_MAX_SUPPORTED] = 
{
  ...
  { RFGSM_SPARE_NV_OVERRIDE_IDX, NV_BIT6_MASK },        /* RFGSM_DEBUG_NV_VBATT_COMP_USB_ENABLE  */
...
/*RF_GSM_NV_PATH_0 */
{
  ...
  /*LNA enh switch points*/
   ...
   RFNV_GSM_C0_GSM850_RX_DC_CAL_DATA_I
},
/*RF_GSM_NV_PATH_1
 there is no GSM diversity NVs available - copy Chain 0 NV Ids*/
{
  ...
  /*ENV GAIN*/
   RFNV_GSM_C0_GSM850_ENV_GAIN_I_V2,
...
rfcom_gsm_band_type efs_offload_band=RFCOM_NUM_GSM_BANDS;
/*===========================================================================
...
FUNCTION RFGSM_NV_RX_WBDC_COMP
...
===========================================================================*/
gsm_rx_wbdc_comp_data_type rfgsm_nv_rx_wbdc_comp
(
  rfm_device_enum_type rfm_dev,
  rfnv_gsm_lna_state_type rfnv_gsm_lna_state
)
{
  rfgsm_core_handle_type *rfgsm_core_handle_ptr = NULL;
  rfgsm_nv_band_data_type *rfgsm_rx_nv_tbl = NULL;
  gsm_rx_cal_wbdc_data_nv_type *rfgsm_rx_wbdc_cal_tbl = NULL;  
  gsm_rx_wbdc_comp_data_type gsm_rx_wbdc_comp_data;

  gsm_rx_wbdc_comp_data.wbdc_offset_q =0;
  gsm_rx_wbdc_comp_data.wbdc_offset_i =0;

    rfgsm_core_handle_ptr = rfgsm_core_handle_get(rfm_dev);
  if (rfgsm_core_handle_ptr == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_handle_ptr is NULL!", 0);
    return gsm_rx_wbdc_comp_data;
  }
  ...
  /* Get the standard Rx Cal table in the firs instance */
  rfgsm_rx_wbdc_cal_tbl = &(rfgsm_rx_nv_tbl->rx_cal_wbdc_nv_data); 
    gsm_rx_wbdc_comp_data= rfgsm_rx_wbdc_cal_tbl->rx_wbdc_comp_data[rfnv_gsm_lna_state];
	return gsm_rx_wbdc_comp_data ;
...
void rfgsm_nv_init_amam_ampm_table(rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band)
{
  ...
  */
  ...
  if((amam_master_table_ptr->amam_tbl[0].data[1]*2) > amam_master_table_ptr->amam_tbl[0].data[2])
  {
  ...
  {
   amam_master_table_ptr->amam_tbl[0].data[0] = (uint16)( amam_master_table_ptr->amam_tbl[0].data[2]-(amam_master_table_ptr->amam_tbl[0].data[1]*2) ); 	
  }

  if((amam_master_table_ptr->amam_tbl[1].data[1]*2) > amam_master_table_ptr->amam_tbl[1].data[2])
  {
  ...
  {
    amam_master_table_ptr->amam_tbl[1].data[0] = (uint16)( amam_master_table_ptr->amam_tbl[1].data[2]-(amam_master_table_ptr->amam_tbl[1].data[1]*2) );  
  }
  if((amam_master_table_ptr->amam_tbl[2].data[1]*2) > amam_master_table_ptr->amam_tbl[2].data[2])
  {
  ...
  {
	  amam_master_table_ptr->amam_tbl[2].data[0] = (uint16)( amam_master_table_ptr->amam_tbl[2].data[2]-(amam_master_table_ptr->amam_tbl[2].data[1]*2) );  
  }
...
)
{
  ...
  {
    rfgsm_nv_common_config_storage.rx_static_nv_data_buffers[rfgsm_nv_common_config_storage.rx_buffer_count] = RFGSM_NV_MEM_ALLOC( sizeof(gsm_rx_static_nv_type) );
...
)
{
  if ( rfgsm_nv_common_config_storage.tx_buffer_count < RF_GSM_NV_MAX_STRUCTURES )
  {
    ...
    rfgsm_nv_common_config_storage.tx_static_nv_data_buffers[rfgsm_nv_common_config_storage.tx_buffer_count] = RFGSM_NV_MEM_ALLOC( sizeof(gsm_tx_static_nv_type));
...
)
{
  ...
  if ( rf_gsm_nv_config_storage.allocated < RF_GSM_NV_MAX_STRUCTURES )
  {
    ...
    rfgsm_nv_band_data_buffer = (rfgsm_nv_band_data_type*)RFGSM_NV_MEM_ALLOC( sizeof(rfgsm_nv_band_data_type));
...
)
{
  ...
  rf_gsm_nv_cmn_data = (rfgsm_nv_cmn_data_type *)RFGSM_NV_MEM_ALLOC( sizeof(rfgsm_nv_cmn_data_type));
...
)
{
  ...
  nv_data_ptr = (rfnv_item_type *)RFGSM_NV_MEM_ALLOC( sizeof(rfnv_item_type));
...
boolean
rfgsm_nv_load_wbdc_cal_items
(
  gsm_rx_cal_wbdc_data_nv_type* gsm_rx_cal_nv_struct,
  rfnv_item_id_enum_type rx_cal_data_item,
  rfgsm_nv_supported_type band,
  rex_tcb_type* caller_tcb_ptr,
  rex_sigs_type task_nv_wait_sig,
...
)
{
  ...
  if (nv_data_ptr == NULL)
  {
   ...
   MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_rx_cal_items:rx_cal_data_item %d ", rx_cal_data_item);	
    ...
    for( type_id_idx = 0; (type_id_idx < num_type_id) && (api_status == TRUE); type_id_idx++ )
    {
      ...
      /* EXTRACT ELEMENT corresponding to the this type ID */
       ...
       case RFNV_DATA_TYPE_ID_RGI_LIST:
         MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "Deprecated 32 step RGIs for SDR105"); 
          break;
       ...
       case RFNV_DATA_TYPE_ID_PMEAS_LIST:
          ...
          break;
	   case RFNV_DATA_RGI_LIST_TYPE_V2:
	   ...
	   case RFNV_DATA_PMEAS_LIST_TYPE_V2:
...
)
{
  ...
  rfnv_data_rgi_list_type_v2 rgi_node_data;
  ...
  if( api_status == TRUE )
  {
    ...
    *extracted_element_size = num_bytes_needed;
    ...
    if (rgi_node_data.mod == 0) /* GMSK case*/
    {
      ...
      for (rgi_idx = 0; rgi_idx < RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2; rgi_idx++)
      {
...
)
{
  ...
  boolean api_status = TRUE;
  /* The item to extract from NV */
  rfnv_data_pmeas_list_type_v2 pmeas_data;
  /* Contains the number of bytes for the element in NV */
  uint32 num_bytes_needed;
  /* For indexing */
  uint8 rgi_idx;
...
)
{
  ...
  /* Load Rx cal WBDC data */
  if ( retval == TRUE )
  {
    index_entry->last_nv_item_read = rfgsm_nv_items_map_ptr->rx_wbdc_cal_data + band;
    retval = rfgsm_nv_load_wbdc_cal_items( &rf_gsm_nv_data_buffer_ptr->rx_cal_wbdc_nv_data,
                                          rfgsm_nv_items_map_ptr->rx_wbdc_cal_data,
                                          band,
                                          caller_tcb_ptr,
                                          task_nv_wait_sig,
                                          task_wait_func_ptr );
...
)
{
  ...
  if(band < RFGSM_NV_MAX_SUPPORTED)
  {
	 ...
	 if((main_nv_band < RFGSM_NV_MAX_SUPPORTED ))
	 {
	 ...
	 {
	   MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_nv_load_band_specific_nv: main_nv_band :%d is invalid.", main_nv_band);
...
uint64 rfgsm_set_psm_band (rfcom_gsm_band_type band)
{
 
  switch(band)
   {
     
     case RFCOM_BAND_GSM850: psm_band=SYS_BAND_MASK_GSM_850; break;
     case RFCOM_BAND_GSM900: psm_band=SYS_BAND_MASK_GSM_EGSM_900; break;
     case RFCOM_BAND_GSM1800: psm_band=SYS_BAND_MASK_GSM_DCS_1800; break;
     case RFCOM_BAND_GSM1900: psm_band=SYS_BAND_MASK_GSM_PCS_1900; break;
     default:
             psm_band=0;
  	}
  return psm_band;	
}
uint64 rfgsm_get_psm_band()
{
return psm_band;
}

uint64 rfcom_gsm_band_convert_to_rf_sys_band(rfcom_gsm_band_type gsm_band)
{
  uint64 gsm_sys_band=0;
  
  switch(gsm_band)
   {
     case RFCOM_BAND_GSM850:   gsm_sys_band=SYS_BAND_MASK_GSM_850;break;
     case RFCOM_BAND_GSM900:   gsm_sys_band=SYS_BAND_MASK_GSM_EGSM_900;break;
     case RFCOM_BAND_GSM1800:  gsm_sys_band=SYS_BAND_MASK_GSM_DCS_1800;break;
     case RFCOM_BAND_GSM1900:  gsm_sys_band=SYS_BAND_MASK_GSM_PCS_1900;break;
	 default: break;
   }
  return gsm_sys_band;
}
...
uint64 rfgsm_nv_get_band_mask(rfcom_gsm_band_type gsm_band)
{

 //api to get grr band info
  uint64 gsm_band_mask=0;
 uint64 offload_band_mask=0;
 gsm_band_mask=rfcom_gsm_band_convert_to_rf_sys_band(gsm_band);
  uint64 perferred_band_mask = (SYS_BAND_MASK_GSM_EGSM_900 |
                                SYS_BAND_MASK_GSM_DCS_1800 |
                                SYS_BAND_MASK_GSM_850	    |
                                SYS_BAND_MASK_GSM_PCS_1900);
 offload_band_mask=perferred_band_mask^(gsm_band_mask);
 MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR, "PSM rfgsm_nv_get_band_mask: gsm_band_mask 0x%x perferred_band_mask 0x%x  offload_band_mask 0x%x ", gsm_band_mask,perferred_band_mask,offload_band_mask);	 
 return offload_band_mask;

}

uint64 rfgsm_nv_get_offload_band_mask()
{
 ...
 {
  offload_band_mask=rfgsm_nv_get_band_mask(rfm_gsm_psm_info.band);
 }
 return offload_band_mask;
  ...
  for (band = 0; band < RFCOM_NUM_GSM_BANDS; band++)
  {
    ...
    for (i = 0; i < 2; i++)  //  0 for RX and 1 for Tx in each band
    {
      ...
      band_mask = rfcommon_nv_get_rf_bc_config( (rf_path_enum_type)nv_cont_array.nv_container[i] );
       offload_band_mask=rfgsm_nv_get_offload_band_mask();
      rfnv_band = rfgsm_core_rfcom_band_to_nvband(band) ;
      MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"PSM rfgsm_nv_load_items, offload_band_mask 0x%x band_mask 0x%x  predef_band_mask[band] 0x%x ", offload_band_mask,band_mask,predef_band_mask[band]);

      if (band_mask & predef_band_mask[band] & (~offload_band_mask))
      {
        MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"PSM rfgsm_nv_load_items, band_mask 0x%x  started ", band_mask);
...
rfcommon_nv_status_type rfgsm_offload_nvs_handler(rf_apps_cmd_type* cmd_ptr,
                                                             rex_tcb_type *task_ptr,
                                                             rex_sigs_type task_wait_sig,
                                                             void (*task_wait_handler)(rex_sigs_type))
{
  ...
  rfgsm_nv_path_type rfgsm_nv_path;
  uint64 band_mask = 0;
  rfcom_gsm_band_type offload_band=(uint8)(cmd_ptr->payload[0]);
  uint64 offload_band_mask=0;
  rfgsm_nv_cmn_node_type *index_entry_cmn = NULL;  // for common NV table
  rfgsm_nv_band_node_type *index_entry = NULL;  // for band specific NV tables
  rfcom_gsm_band_type band = RFCOM_NUM_GSM_BANDS;
  ...
  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"PSM rfgsm_offload_nvs_handler CALLED with , offload_band_mask 0x%x  ", offload_nv_band_mask);
	  ...
	  for (band = 0; band < RFCOM_NUM_GSM_BANDS; band++)
	  {
		...
		nv_container_array[rfm_dev][band][i]= nv_cont_array.nv_container[i];
		 ...
		 offload_band_mask=rfgsm_nv_get_offload_band_mask();
	      rfnv_band = rfgsm_core_rfcom_band_to_nvband(band) ;
		MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"PSM rfgsm_offload_nvs_handler, offload_band_mask 0x%x band_mask 0x%x	predef_band_mask[band] 0x%x ", offload_band_mask,band_mask,predef_band_mask[band]);
		  
		if (band_mask & predef_band_mask[band] & (offload_band_mask))
		  {
		  MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"PSM rfgsm_offload_nvs_handler, band_mask 0x%x  started ", band_mask);
			index_entry = rfgsm_nv_alloc_band_nv_buf(rfm_dev, nv_cont_array.nv_container[i], rfnv_band);
...
void rfgsm_nv_deinit(rfm_device_enum_type rfm_dev)
{
  ...
  {
    RFGSM_NV_MEM_FREE( rfgsm_nv_data[nv_path].rfgsm_nv_cmn_node.rfgsm_nv_cmn_data_buffer_ptr);
  ...
  for (band = RFGSM_NV_850; band < RFGSM_NV_MAX_SUPPORTED; band++)
  {
    ...
    for (i = 0;i < 2;i++)
    {
      ...
      if (rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_data_buffer_ptr != NULL)
      {
        ...
        {
          RFGSM_NV_MEM_FREE( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_data_buffer_ptr->rx_static_nv_data_ptr);
        ...
        {
          RFGSM_NV_MEM_FREE( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_data_buffer_ptr->tx_static_nv_data_ptr);
        ...
        RFGSM_NV_MEM_FREE( rfgsm_nv_data[nv_path].rfgsm_nv_band_node[band].rfgsm_nv_band_data_buffer_ptr);
...
void rfgsm_nv_populate_coex_band_map_tbl( void )
{
  ...
  for (rfnv_band = 0; rfnv_band < RFGSM_NV_MAX_SUPPORTED; rfnv_band ++)
  {
    rfgsm_coex_nv_band_map_tbl[rfnv_band] = 
      RFGSM_NV_MEM_ALLOC( sizeof(rfgsm_coex_band_map_type));
...
void rfgsm_nv_sort_rx_cal_data(gsm_rx_cal_data_nv_type* rx_cal_nv_data, rfcom_gsm_band_type band, rfnv_item_size_type gsm_rx_cal_nv_read_size)
{
  ...
  temp_rx_cal_nv_data = (gsm_rx_cal_data_nv_type *)RFGSM_NV_MEM_ALLOC( sizeof(gsm_rx_cal_data_nv_type));
  ...
  for(arfcn_idx=0; arfcn_idx<temp_rx_cal_nv_data->rx_cal_chan_size; arfcn_idx++)
  {
   ...
   RFGSM_NV_MEM_FREE ( temp_rx_cal_nv_data);
}
