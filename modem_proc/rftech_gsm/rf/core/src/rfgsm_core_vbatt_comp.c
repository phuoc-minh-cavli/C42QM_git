...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core_vbatt_comp.c#1 $
$DateTime: 2020/04/16 12:20:47 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/23/19   krk     Hooking up with MCS api for usb status for accurate vbatt reading
05/27/19   krk     Fix for type error
...
#define USB_VBATT_COMPENSATION 300  /*in mv*/
...
void rfgsm_core_vbatt_read_handler(rf_apps_cmd_type* cmd_ptr) 
{
  ...
  curr_vbatt_read = (uint16)rfgsm_msm_get_adc_read(rfm_dev,flag);
   ...
   usb_connected = lmtsmgr_vbatt_get_usb_conn_state();

   if(usb_connected == TRUE)
  {
     curr_vbatt_read = curr_vbatt_read - USB_VBATT_COMPENSATION;
  }
  
  RFGC_MSG_2( MSG_LEGACY_HIGH, "rfgsm_core_vbatt_read_handler, USB connected %d vabtt reading %d ", usb_connected,curr_vbatt_read);
