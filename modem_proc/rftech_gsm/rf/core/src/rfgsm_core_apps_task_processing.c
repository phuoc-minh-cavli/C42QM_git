...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core_apps_task_processing.c#1 $
...
void rfgsm_core_apps_task_cmd_dispatch(rf_apps_cmd_type *cmd)
{
  ...
  switch ( cmd->msgr_hdr.id )
  {
    ...
    case RFGSM_CORE_APPS_TASK_VBATT_COMP_CMD:
      ...
      break;
    case RFGSM_CORE_APPS_TASK_OFFLOAD_NVS_CMD:
      rfgsm_offload_nvs_handler(cmd,rex_self(),RF_NV_CMD_SIG|RF_RPT_TIMER_SIG,rf_apps_task_handle_sigs);
