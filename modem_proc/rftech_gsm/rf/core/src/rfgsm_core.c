...
/*===========================================================================
Copyright (c) 2008 - 2018 by Qualcomm Technologies Incorporated. 
All Rights Reserved.

                        EDIT HISTORY FOR MODULE

  $Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core.c#2 $
  $DateTime: 2020/05/25 02:40:00 $ 
  $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
06/19/19   mpa     to support device status reads on SDR105
05/27/19   krk     NV ctrl for additional vbatt compensation with/without USB connected
05/24/19   mpa     Enable JDET by default
04/30/19   mpa     remove JDET_ENABLE macro
04/25/19   krk     NV control for JDET FR
04/05/19   mpa     Enable JDET on Twizy
04/05/19   mpa     print path_delay as part of tx_logging
04/01/19   krk     Moving WBEE thr and support flag update to enter mode req from wakeup req
03/12/19   krk     [CR2406347] WBEE based jammer detection fixes for GSM
03/08/19   mpa     Disbaling JDET on SDR105
...
08/31/18   hkm     Logging updates
08/27/18   mpa     Changes to support SPLL
05/18/18   hkm     Supporting GSM WTR105
02/22/18   mpa     To disable IP2_CAL
27/04/17   bp      Fixing KW Errors - JO302
...
static rflm_gsm_sync_trx_ustmr_s sync_gsm_trx_ustmr_info;
...
extern void rflm_dtr_rx_rcg_clk_gen(void);
...
boolean rfgsm_core_init(rfm_device_enum_type rfm_dev)
{
 ...
 // rfgsm_core_turn_on_vregs(rf_path, (uint8)RFLM_GSM_DEFAULT_SUBSCRIPTION, RF_PATH_RX_STATE);
  ...
  /*Enable TCXO buffer,
    deprecated the apis as it is not supported by the drivers*/
  ...
  /* update the device layer with HMSC information 
     deprecated as it is not used by the drivers*/
...
//  }
   ...
   */ 
   ...
   /*Disable TCXO buffer
     Deprecated the RFDEVICE_ENABLE_TCXO_BUFFER by drivers team*/
   //(void)rfdevice_gsm_cmd_dispatch( rfm_dev, rf_path, RFDEVICE_ENABLE_TCXO_BUFFER, &enable_tcxo_buffer);
...
void rfgsm_core_nv_offload(rfcom_gsm_band_type band)
{
  rf_apps_cmd_type *rfgsm_nv_read_cmd = NULL;
  if ( ( rfgsm_nv_read_cmd = rf_apps_get_buf() ) == NULL )
     {
       RF_MSG( RF_ERROR, "rfgsm_core_nv_offload: RF command queue is full!" );
     }
	else
	 {
	   RF_MSG_1( RF_ERROR, "PSM rfgsm_core_nv_offload: RF command with band %d ",band );
	   rfgsm_nv_read_cmd->msgr_hdr.id = RFGSM_CORE_APPS_TASK_OFFLOAD_NVS_CMD;
       rfgsm_nv_read_cmd->rex_hdr.cmd_hdr.tech_id = RF_APPS_GSM_ID;
       rfgsm_nv_read_cmd->payload[0] = (uint8)band;
       rf_apps_cmd_put( rfgsm_nv_read_cmd );
     }
}
...
unsigned char nb_jdet_switchpont = 8; /* The default value of switchpoint is -110 + 8 = -102 dBm */
uint32 nb_jdet_thr_g0 = 5308548;
uint32 nb_jdet_thr_g1 = 53085;
...
boolean rfgsm_core_init_once(void)
{
  ...
  int32 file_read_bytes=-1;
  const char *nb_jdet_switchpoint_filename = "/rf/nb_jdet_switchpoint.bin";
  const char *nb_jdet_thr_g0_file = "/rf/nb_jdet_thr_g0_file.bin";
  const char *nb_jdet_thr_g1_file = "/rf/nb_jdet_thr_g1_file.bin";
  ...
  file_read_bytes = rfcommon_efs_get(&nb_jdet_switchpoint_filename[0],
                                       (void*)&nb_jdet_switchpont,
                                       sizeof(nb_jdet_switchpont));
  
    if ( file_read_bytes <= 0 )
    {
      RF_MSG(MSG_LEGACY_HIGH,"nb_jdet_switchpont efs get:Could not open file");
      nb_jdet_switchpont = 8;
    }
    file_read_bytes = rfcommon_efs_get(&nb_jdet_thr_g0_file[0],
                                         (void*)&nb_jdet_thr_g0,
                                         sizeof(nb_jdet_thr_g0));
    if ( file_read_bytes <= 0 )
    {
      RF_MSG(MSG_LEGACY_HIGH,"nb_jdet_switchpont efs get:Could not open file");
      nb_jdet_thr_g0 = 5308548;
    }

    file_read_bytes = rfcommon_efs_get(&nb_jdet_thr_g1_file[0],
                                       (void*)&nb_jdet_thr_g1,
                                       sizeof(nb_jdet_thr_g1));
    if ( file_read_bytes <= 0 )
    {
      RF_MSG(MSG_LEGACY_HIGH,"nb_jdet_switchpont efs get:Could not open file");
      nb_jdet_thr_g1 = 53085;
    }
                                      ...
                                      rf_path_enum_type rf_path )
{
  ...
  if(enable)
  {
    ...
    if(prx_device)
    {
	   ...
	   //ADC RCG 
      rflm_dtr_rx_rcg_clk_gen();
	  
	  //ADC cal sequence
      rflm_dtr_rx_adc_calib_mode(); 
    ...
    rfcommon_bootup_init();

	//RFLM API to do USTMR FIXUP and enqueue to WTR.
    sync_gsm_trx_ustmr_info.sub_id=sub_id;
    rflm_gsm_sync_trx_ustmr_to_modem_ustmr(&sync_gsm_trx_ustmr_info);
    /*Enable Device Power Voting */
	...
	{
      if(!rf_buffer_clear((rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.fsk_enter_mode_script))
      {
        MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
            "RF GSM card fsk enter mode script buffer was unsuccessfully cleared", 0);
        return FALSE;
      } 
	}
#endif
  ...
  else 
  {
    ...
    if ((rfm_dev == RFM_DEVICE_0) || (rfm_dev == RFM_DEVICE_2))
    {
      /*RFDEICE_WAN_TX_RESET is not handled in the rfdevice_gsm_cmd_dispattch so drivers is not support this cmd*/
...
#endif
  for (i = RFM_DEVICE_0; i < RFM_MAX_WAN_DEVICES; i++)
  {

    rfgsm_mdsp_create_ccs_event_handles( (rfm_device_enum_type) i );
  } 
/* Clear the script buffer before use */
if(!rf_buffer_clear((rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.ustmr_sync_script))
{
  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
		"RF GSM card ustmr_sync_script buffer was unsuccessfully cleared", 0);
  return FALSE;
}

if(!rf_buffer_clear((rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.jdet_disable_trigger_script))
{
  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,
		"RF GSM card jdet_disable_trigger_script buffer was unsuccessfully cleared", 0);
  return FALSE;
}

if(!rfdevice_gsm_ustmr_sync(rfm_dev,(rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.ustmr_sync_script))
 {
	RF_MSG_2(MSG_LEGACY_ERROR,
			 "rfdevice_gsm_ustmr_sync is failed for rfm_dev  %d, and script 0x%x",
			 rfm_dev, rfgsm_core_handle_ptr->scripts.ustmr_sync_script);
	return FALSE;
 }
if(!rfdevice_gsm_disable_jdet(rfm_dev,RFDEVICE_CREATE_SCRIPT, (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.jdet_disable_trigger_script))
 {
	RF_MSG_2(MSG_LEGACY_ERROR,
			 "rfdevice_gsm_disable_jdet is failed for rfm_dev  %d, and script 0x%x",
			 rfm_dev, rfgsm_core_handle_ptr->scripts.jdet_disable_trigger_script);
	return FALSE;
 }


if(!rfgsm_core_dm_dynamic_jdet_disable_trigger_event_wrapper(core_data))
{
  RF_MSG(MSG_LEGACY_ERROR,
			 "rfgsm_core_dm_dynamic_jdet_disable_trigger_event_wrapper is failed for ");
  return FALSE;
}


if(!rfgsm_core_dm_dynamic_ustmr_sync_event_wrapper(core_data))
{
  RF_MSG(MSG_LEGACY_ERROR,
			 "rfgsm_core_dm_dynamic_ustmr_sync_event_wrapper is failed for ");
  return FALSE;
}
...
rfgsm_core_handle_ptr->tx_alternate_path = 0;
  ...
  temp_rx_num_trans = rf_buffer_get_num_trans((rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.rx_enter_mode_script, RF_BUFFER_RFFE_GROUP);
...
boolean rfgsm_core_fsk_enter_mode(rfgsm_core_data_type *core_data, uint32 rxlm_buf_idx, const void *shared_mem_ptr)
{
  /* Initialise variables */
  rf_path_enum_type rf_path;
  rfm_device_enum_type rfm_dev = core_data->rfm_dev;
  rfgsm_core_handle_type *rfgsm_core_handle_ptr = core_data->rfgsm_core_handle_ptr;
  boolean is_tx_device = TRUE;
  uint8 i=0;

  RF_MSG( MSG_LEGACY_HIGH,"rfgsm_core_fsk_enter_mode" );
  
  rf_path = rfcommon_core_device_to_path(rfm_dev);

  is_tx_device = rfgsm_core_is_device_prx(rfm_dev);

  /* Allocate the subscription RFLM DM memory */
  if( !rfgsm_core_dm_alloc_fsk_data_manager_mem() )
  {
...
rfgsm_core_handle_ptr->tx_alternate_path = 0;
...
#ifndef T_RUMI_EMULATION
  /* Enable RF Vregs, CCS cmd interface*/
  if( !rfgsm_core_enable_disable_hw(TRUE, core_data, is_tx_device, rf_path) )
  {
    RF_MSG(MSG_LEGACY_ERROR, "Enter mode rfgsm_core_enable_disable_hw failed!");
    return FALSE;
  }
  ...
  /* initialize the RFC data structures and power management for GSM mode based on alternate path */
  rfc_gsm_mode_config(rfm_dev, (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_enter_mode_script, rfgsm_core_handle_ptr->rx_alternate_path);


  /* initialize the modem for GSM. rx lm handle gets updated based on alternate path */
  //rfgsm_msm_init(rxlm_buf_idx, rfm_dev, FALSE, rfgsm_core_handle_ptr->rx_alternate_path);
  rfgsm_msm_enable_tx(rxlm_buf_idx, core_data->rfm_dev, FALSE, core_data->rfgsm_core_handle_ptr->tx_alternate_path);
  core_data->rfgsm_core_handle_ptr->txlm_buf_index = rxlm_buf_idx;
  ...
  if( core_data->device_is_tx )
  {
   ...
   (void)(rfdevice_gsm_tx_enter_mode(rfm_dev,
                                     RFDEVICE_CREATE_SCRIPT,
                                     (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.fsk_enter_mode_script
                                     ));
  }
  ...
  /* Perform any customer-requested initializations */
  rfgsmcust_init();
  RFGSM_CORE_SET_SUB_ACTIVE(rfgsm_core_handle_ptr->sub_id_mask, core_data->sub_id);
...
/*! @param core_data - data for dev, sub, task, core handle etc              */
/*! @retval boolean success status                                           */
boolean rfgsm_core_fsk_sleep( rfgsm_core_data_type *core_data )
{
  /* Initialise variables */
  rf_path_enum_type rf_path;
  ...
  /* Clear the enter mode event to prevent attempt to program stale event */
  if( !rfgsm_core_dm_clean_event( core_data, RFLM_GSM_FSK_ENTER_MODE_SCRIPT ) )
  {
...
/*! @param core_data - data for dev, sub, task, core handle etc              */
/*! @retval boolean success status                                           */
boolean rfgsm_core_fsk_exit_mode(rfgsm_core_data_type *core_data, rfcom_mode_enum_type to_mode)
{
  /* Initialise variables */
  rf_path_enum_type rf_path;
  boolean dev_is_prx = TRUE;
  ...
  if(!rfgsm_core_fsk_dm_free_data_manager_mem(sub_id))
  {
    RF_MSG_2(MSG_LEGACY_ERROR, 
  ...
  for (i = RFM_DEVICE_0; i < RFM_MAX_WAN_DEVICES; i++)
  {
    ...
    rfgsm_mdsp_free_ccs_event_handles( (rfm_device_enum_type) i );
  }
...
*/
boolean rfgsm_core_populate_fsk_burst_rflm_data_info( rfgsm_core_data_type *core_data,
                                                  rf_burst_type burst,
                                                  uint8 num_tx_slots,
                                                  void *burst_event_ptr )
{
  ...
  {
    if( !rfgsm_core_dm_populate_fsk_tx_burst_intf_data( core_data, burst_event_ptr, num_tx_slots ) )
    {
      RF_MSG(MSG_LEGACY_ERROR,"RF GSM FSK Tx burst RFLM data info update failed!");
    }
  }
  else
  {
    RF_MSG(MSG_LEGACY_ERROR,"RF GSM FSK invalid core burst type, RFLM data info not populated");
    return FALSE;
  }
                                                ...
                                                const void *enter_mode_ptr)
{
 ...
 if(core_data->rfgsm_core_handle_ptr->curr_stack==RFGSM_GSM_MODE)
  ...
  {
   if( !rfgsm_core_dm_dynamic_fsk_enter_event_wrapper( core_data ) )
       {
         RF_MSG(MSG_LEGACY_ERROR,"RF GSM FSK Enter mode RFLM data info event create failed!");
         ret_status &= FALSE;
       }
  }
 #endif
...
boolean rfgsm_core_get_therm_data(uint8 sub_id,uint32* therm_data)
{
  boolean ret_val=TRUE;
  //*therm_data=5082;//DEFAULT VALUE FOR THERM
  //RF_MSG(MSG_LEGACY_ERROR,"rfgsm_core_get_therm_data set to default 5082");
  ret_val=rflm_get_therm_data(sub_id,therm_data );
...
boolean rfgsm_core_prepare_therm_read(rfgsm_core_data_type *core_data, const void *shared_mem_ptr)
{
  ...
  default:
  case RF_TASK_1:
    core_data->rfgsm_core_handle_ptr->therm_read.fw_therm_read_umid = GERAN_FW_RFM_THERM_READ_CMD;
    core_data->rfgsm_core_handle_ptr->therm_read.fw_therm_rsp_umid = RFLM_GSM_THERM_READ_RSP;
    break;
#ifdef FEATURE_DUAL_SIM
  case RF_TASK_2:
    core_data->rfgsm_core_handle_ptr->therm_read.fw_therm_read_umid = GERAN_FW2_RFM_THERM_READ_CMD;
    core_data->rfgsm_core_handle_ptr->therm_read.fw_therm_rsp_umid = RFLM_GSM_THERM_READ_SUB2_RSP;
    break;
#endif
    /*! @todo add 3rd task here */
  }
   rfdevice_gsm_get_therm_read_script(core_data->rfm_dev,
  ...
  RF_MSG_2( MSG_LEGACY_ERROR, "RF GSM therm reads %d %d", therm_raw_value, therm_read_in_degC);
...
/*! @param shared_mem_ptr - GFW shared memory pointer                        */
/*! @retval boolean success status                                           */
boolean rfgsm_core_wakeup( rfgsm_core_data_type *core_data, 
                                   const void *shared_mem_ptr)
{
  ...
  rfcom_gsm_band_type i = RFCOM_BAND_GSM850;
  /* Get logical path and if device is PRx */
  rf_path = rfcommon_core_device_to_path(rfm_dev);
  dev_is_prx = rfgsm_core_is_device_prx(rfm_dev);
...
void rfgsm_core_turn_on_vregs(rf_path_enum_type path, uint8 sub_id, rfm_path_state rf_path_state)
{
  ...
  {
    RF_MSG_4( RF_FATAL, "RF GSM Vreg on count %d invalid for sub %d, path %d, rf_state %d", 
...
/*===========================================================================*/
/*! @fn      rfgsm_core_get_pll_tune_time_hmsc_pmon                          */
/*! @brief   Function for updating the final HMSC PMON offset to GL1         */
/*! @details This function is called only during HMSC. Expectation from RF   */
/*!          is to adjust the polarity based on WTR driver value and add     */
/*!          whatever RF offsets needed and pass it back to GL1              */
/*! @retval  HMSC PMON offset time in Qs                                     */
int32 rfgsm_core_get_pll_tune_time_hmsc_pmon(int16 pll_tune_qs)
{
  int32 hmsc_pmon_time;
  
  /* First do the offset adjustment within RF as done in DM */
  hmsc_pmon_time = pll_tune_qs + RFGSM_HMSC_PMON_TUNE_OFFSET_ADJUST_QS;

  /* Do the polarity adjustment within RF */
  hmsc_pmon_time = (int32)(-hmsc_pmon_time);

  /* Adjust the PMON offset with the HMSC PMON macro */
  hmsc_pmon_time = hmsc_pmon_time + RFGSM_HMSC_PMON_TX_REF_OFFSET_ADJUST_QS;

  return hmsc_pmon_time;
...
void rfgsm_core_get_timing_info(rfgsm_core_data_type *core_data, rfgsm_timing_info_type *timing_info, boolean hmsc_mode)
{
  ...
  //RX alpha value to L1
  timing_info->mon_rf_setup_time_qs = rfc_gsm_core_ptr->rx_timing->rx_alpha_qs;
  timing_info->rx_rf_setup_time_qs = rfc_gsm_core_ptr->rx_timing->rx_alpha_qs;

  timing_info->rx_idle_rf_setup_time_qs = rfc_gsm_core_ptr->rx_timing->rx_alpha_qs;
  //TX alpha value to L1
  timing_info->tx_rf_setup_time_qs = rfc_gsm_core_ptr->tx_timing->tx_alpha_qs;
...
/* Do the polarity adjustment in RF and pass to GL1 */
  timing_info->rx_pll_setup_qs = rfgsm_core_get_pll_tune_time_hmsc_pmon (core_data->rfgsm_core_handle_ptr->rx_timing_info.tune_fixup_rel_qs);
}
...
void rfgsm_core_tx_log(rfm_device_enum_type rfm_dev, rf_task_num_type rf_task)
{
  ...
  RFGC_L_MSG_5( MSG_LEGACY_HIGH,"tx_band = %d, tx_arfcn = %d num_slots = %d sar_state = %d path_delay = %d", 
        rfgsm_core_handle_ptr->tx_log.tx_band, rfgsm_core_handle_ptr->tx_log.tx_chan, rfgsm_core_handle_ptr->tx_log.num_slots, rfgsm_core_handle_ptr->tx_log.sar_state,rfgsm_core_handle_ptr->tx_log.delay_val);
  ...
  {
    RFGC_L_MSG_4( MSG_LEGACY_HIGH, "pwr_index=%d, pwr=%d, pwr_offset=%d, rgi=%d",
...
boolean rfgsm_core_data_init(rfm_device_enum_type rfm_dev)
{
  ...
  if ( rfgsm_core_handle_ptr->init_once == FALSE )
  {
	...
	rfgsm_core_handle_ptr->scripts.ustmr_sync_script = NULL;
	rfgsm_core_handle_ptr->scripts.jdet_disable_trigger_script = NULL;
    ...
    rfgsm_core_handle_ptr->scripts.fsk_enter_mode_script = NULL;
	for (i=0; i < TX_MAX_SCRIPT; i++)
    {
     rfgsm_core_handle_ptr->scripts.fsk_tx_enable_script[i] = NULL;
	 rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[i] = NULL;
    }
	
	for (i=0; i < TX_MAX_SCRIPT; i++)
    {
	 rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[i] = NULL;
	 rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[i] = NULL;
	}
	for(i=0;i<RFGSM_MAX_TX_SLOTS_PER_FRAME;i++)
	{
     rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_WTR_SCRIPT][i] = NULL;
	 rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_FE_SCRIPT][i] = NULL;
	}
	rfgsm_core_handle_ptr->scripts.fsk_tx_afc_neg_script = NULL;
	rfgsm_core_handle_ptr->scripts.fsk_tx_afc_pos_script = NULL;
	rfgsm_core_handle_ptr->scripts.fsk_exit_script   = NULL;
    ...
    rfgsm_core_handle_ptr->rf_has_five_gainranges = TRUE;
    ...
    rfgsm_core_handle_ptr->sawless_linearity_control.linearity_switching_thresholds.jdet_high_thresh = 0x12c0; //600 mv * 8 in Hex
    rfgsm_core_handle_ptr->sawless_linearity_control.linearity_switching_thresholds.jdet_med_thresh = 0xE10; //450 mV * 8 in Hex
    ...
    rfgsm_core_handle_ptr->usb_connected= rfgsm_nv_check_debug_nv(rfm_dev, RFGSM_DEBUG_NV_VBATT_COMP_USB_ENBALE);
  ...
  for (i=0; i<RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2; i++)
  {
  ...
  } // ^ i=0; i<RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2; i++
                                              ...
                                              uint8 lna_range )
{
  ...
  /*----------------------------------- set spectarl inversion data----------------------------------------------*/
   ...
   rfdevice_gsm_get_rx_rsb_data(core_data_ptr->rfm_dev,&rfdev_rsb[0]);
  ...
  /* Tell the library to tune */
    if(!rfdevice_gsm_tune_rx( rfm_dev, 
                        arfcn,
                        rfcom_band,
                        core_data_ptr->rfgsm_core_handle_ptr->lna_range,
                        (rf_buffer_intf*)core_data_ptr->rfgsm_core_handle_ptr->scripts.rx_start_script[RX_WTR_SCRIPT],
                        core_data_ptr->rfgsm_core_handle_ptr->enh_rx_on_flag,
                        core_data_ptr->rfgsm_core_handle_ptr->enh_rx_freq_offset,
 ...
 /* Tune RX*/
  ...
  /* Set TX band*/
  if(!rfdevice_gsm_set_tx_band( rfm_dev, 
  	                              rfcom_band,RFDEVICE_CREATE_SCRIPT,
  	                              (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.ip2_cal_setup_script))
  {
     ...
     rfdevice_gsm_clear_timed_writes( rfm_dev, NULL);
     return FALSE;
  }
   ...
   //rfdevice_gsm_cmd_dispatch( rfm_dev, RF_PATH_0, RFDEVICE_GSM_CLEAR_TIMED_WRITES, NULL);
   rfdevice_gsm_clear_timed_writes( rfm_dev, NULL);
//Current defnition is wrong. Will Comment till we get a correct defnition,
   /*Set TX arfcn*/
  if (!rfdevice_gsm_tune_tx(rfm_dev,
  ...
  /* Set TXAGC RGI*/
  rfgsm_core_handle_ptr->rgi_slot[0] =tx_rgi;
  mod_type[0]=RF_MOD_GMSK;
  
  if(! rfdevice_gsm_set_rgi(rfm_dev,rfgsm_core_handle_ptr->tx_band,rfgsm_core_handle_ptr->rgi_slot,
  	                         mod_type,(rf_buffer_intf **)rfgsm_core_handle_ptr->scripts.ip2_cal_setup_script,1, TRUE,
                             &trx_timer_fixup_array,
                             &timer_fixup_arr_size,
                             RFDEVICE_TRX_RX_WXE_BUF_0))
  {
...
//  rfdevice_gsm_cmd_dispatch( rfm_dev, RF_PATH_0, RFDEVICE_GSM_SET_HIGH_LIN_MODE, &high_lin_mode );
  //rfdevice_gsm_set_high_lin_mode( rfm_dev,&high_lin_mode );
...
boolean rfgsm_core_build_ip2_cal_script(rfgsm_core_data_type *core_data_ptr)
{
  ...
  //uint16 tx_rgi = RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2;
  //uint8 lna_range = 0;
...
boolean rfgsm_core_get_jdet_config_scripts(rfgsm_core_data_type *core_data_ptr)
{
  ...
  rflm_gsm_data_type *table_ptr;
   ...
   {
	 MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		   "RF GSM core DM dynamic event table_ptr NULL for sub %d", core_data_ptr->sub_id);
	 return FALSE;
   }
  ...
  {
     table_ptr->jdet_disable = FALSE;
  }


  if(rfgsm_core_handle_ptr->rx_burst_type ==RFGSM_RX_MON_BURST)
  	return TRUE;
  ...
  if ( !rfdevice_gsm_get_jdet_config_script(rfm_dev, 
  	                                        RFDEVICE_CREATE_SCRIPT,
                                            rfgsm_core_handle_ptr->band, 
                                            ((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.rx_start_script[RX_WTR_SCRIPT])))
  {
    RFGC_MSG_1( MSG_LEGACY_ERROR, "Failed to config JDET in Rx Start for device %d!", rfm_dev );
    return FALSE;
  }
...
{ 
  ...
  {
    num_trans = rf_buffer_get_num_trans(script_ptr, RF_BUFFER_RFFE_GROUP);
...
boolean rfgsm_core_store_offload_band_info(rfcom_gsm_band_type band)
{
  uint8 band_efs=(uint8)band;
  const char* offload_band_file_name="rf/offload_band_info.txt"; 
  int32 data_size=0;
  uint8 data_read=0;
  //read before write

  data_size=rfcommon_efs_get(offload_band_file_name, &data_read, 1 );
  if(data_size==-1)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_core_read_offload_band_info before write is failed for band: %d",band);
  }else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_core_read_offload_band_info before write is  band: %d",data_read);
  }	

  if(rfcommon_efs_put(offload_band_file_name, &band_efs, 1,FALSE)==FALSE)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_core_store_offload_band_info is failed for band: %d",band);
	return FALSE;			
  }
  ...
  if(data_size==-1)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_core_read_offload_band_info is after_write failed for band: %d",band);
  }else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfgsm_core_read_offload_band_info is after_write band: %d",data_read);
  }	
...
void rfgsm_core_populate_device_status_read_buffers(rfgsm_core_data_type *core_data, rf_burst_type burst_type)
{
  ...
  for(index = 0; index < core_data->rfgsm_core_handle_ptr->device_config_to_read.params[burst_type].num_regs; index++)
  {
    if(burst_type == RF_TX_BURST)
    {
      ...
      {
        rf_buffer_append_rffe_multi((rf_buffer_intf*)core_data->rfgsm_core_handle_ptr->scripts.tx_stop_script[TX_WTR_SCRIPT],
    ...
    if (burst_type == RF_RX_BURST)
    {
      ...
      {
        rf_buffer_append_rffe_multi((rf_buffer_intf*)core_data->rfgsm_core_handle_ptr->scripts.rx_stop_script[RX_WTR_SCRIPT],
...
void rfgsm_core_update_freq_error(rfgsm_core_data_type *core_data, int32  freq_err)
{
  ...
  rfdevice_gsm_set_freq_error( core_data->rfm_dev, &freq_err);
}
