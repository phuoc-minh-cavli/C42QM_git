...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core_data_manager.c#5 $
$DateTime: 2022/08/08 02:03:18 $
$Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/07/22   aad     [CR 3214895] To fix the crash during FSK tx start event
10/07/20   mpa     Ustmr fix during FSK
...
06/12/19   mpa     KW and CW error fixes
05/27/19   krk     Support for backward comptability in cal for Rx cal alone and Rx & WBDC cal
05/23/19   mpa     CW and KW warning fix
04/21/19   krk     FR46594: DC cancellation at ADC input to improve dynamic range of ADC
04/05/19   mpa     F3 reduction on Twizy
26/02/19   mpa     JDET support on SDR105
01/28/19   mpa     Changes to support THERM_READ
12/18/18   mpa     Fix CW error
11/12/18   mpa     KW error Fix
05/18/18   hkm     Extended scope of few function calls
22/02/18   mpa     To disable ip2_Cal
...
void* rfgsm_core_dm_get_data_table_event_addr( rflm_gsm_data_type *table_ptr,
                                               ...
                                               rflm_fsk_data_type *fsk_table_ptr);

 boolean rfgsm_core_dm_dynamic_event_wrapper( rfgsm_core_data_type* core_data,
                                                    rfgsm_core_dm_script_type script_type,
 ...
 *  @return rfgsm_rfc_event_table_type - pointer to the memory holding the information
 */
boolean rfgsm_core_dm_alloc_fsk_data_manager_mem(  )
{
  /* Initialise variables */
  rflm_gsm_dm_err_type rflm_gsm_dm_err = RFLM_GSM_DM_ERR_NONE;

  /* Allocate the data table pointer from DM */
  rflm_gsm_dm_err = rflm_fsk_dm_allocate_tech_buf( );
  if ( rflm_gsm_dm_err != RFLM_GSM_DM_ERR_NONE )
  {
    if ( rflm_gsm_dm_err == RFLM_GSM_DM_ALREADY_ALLOCATED )
    {
      ...
      MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, 
            "RF GSM core FSK DM alloc data table error code %d ",
            rflm_gsm_dm_err);
      return TRUE;
    }
    else
  ...
  MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, 
        "RF GSM core DM data table %d bytes allocated", 
        sizeof(rflm_fsk_data_type));
  return TRUE;
}


boolean rfgsm_core_fsk_dm_free_data_manager_mem(  )
{
  /* Initialise variables */
  rflm_gsm_dm_err_type rflm_gsm_dm_err = RFLM_GSM_DM_ERR_NONE;

  /* Free the DM GSM memory */
  rflm_gsm_dm_err = rflm_fsk_dm_deallocate_tech_buf(  );
  if( rflm_gsm_dm_err != RFLM_GSM_DM_ERR_NONE )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM free data table error code %d for ",
          rflm_gsm_dm_err);
    return FALSE;
  }

  MSG(MSG_SSID_RF, MSG_LEGACY_HIGH, "RF GSM core DM data table free for ");
  return TRUE;
}

boolean rfgsm_core_dm_populate_fsk_tx_band_intf_data( rfgsm_core_data_type *core_data, 
                                                  void *shared_mem_ptr )
{
  ...
  if( !rfgsm_mdsp_populate_fsk_tx_band_index( shared_mem_ptr, idx ) )
  {
...
*/
boolean rfgsm_core_dm_populate_fsk_tx_burst_intf_data( rfgsm_core_data_type *core_data, 
                                                   void *shared_mem_ptr, 
                                                   uint8 num_tx_slots )
{
  /* Initialise variables */
  ...
  if( !rfgsm_mdsp_populate_fsk_tx_start_index( shared_mem_ptr, idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx start FW intf data index update failed");
    return FALSE;
  }

  /* Populate the Tx start RFLM data info */
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              RFLM_GSM_FSK_TX_START_SCRIPT,
                                              0,
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx start FW intf data update failed");
    return FALSE;
  }
  ...
  /****************************** Tx AFC POS ***********************************/
  idx = rfgsm_core_dm_get_rflm_intf_data_idx();
  if( !rfgsm_mdsp_populate_fsk_tx_afc_pos_index( shared_mem_ptr, idx,0 ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx afc pos FW intf data index update failed");
    return FALSE;
  }

  /* Populate the Tx start RFLM data info */
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              RFLM_GSM_FSK_TX_AFC_POS_SCRIPT,
                                              0,
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx AFC POS FW intf data update failed");
    return FALSE;
  }
	/****************************** Tx AFC NEG ***********************************/										  
  idx = rfgsm_core_dm_get_rflm_intf_data_idx();
  if( !rfgsm_mdsp_populate_fsk_tx_afc_neg_index( shared_mem_ptr, idx,0 ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx AFC NEG FW intf data index update failed");
    return FALSE;
  }

  /* Populate the Tx start RFLM data info */
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT,
                                              0,
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx AFC NEG FW intf data update failed");
    return FALSE;
  }
  /****************************** Tx Stop ************************************/
  /* Get the current intf data index */
  ...
  if( !rfgsm_mdsp_populate_fsk_tx_stop_index( shared_mem_ptr, idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx stop FW intf data index update failed");
    return FALSE;
  }

  /* Populate the Tx start RFLM data info */
  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
                                              RFLM_GSM_FSK_TX_STOP_SCRIPT,
                                              num_tx_slots-1,
                                              idx ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Tx stop FW intf data update failed");
    return FALSE;
  }
 ...
 *  @return boolean        - function success status
 */
boolean rfgsm_core_dm_set_fsk_tx_path_delay( rflm_gsm_subscription_id sub_id,
                                         uint8 burst_buffer_id,
                                         int64 path_delay_val )
{
  ...
  table_ptr = rflm_fsk_dm_get_table_ptr( sub_id );
  if( table_ptr == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core FSK DM set Tx path delay table_ptr NULL for sub %d",
          sub_id);
    return FALSE;
  }
  ...
  /* Set the Tx data */
  table_ptr->rfgsm_fsk_tx_event.path_delay_val = path_delay_val;
                                                      ...
                                                      uint32 intf_idx )
{
  ...
  switch(rflm_script)
  {
    case RFLM_GSM_FSK_ENTER_MODE_SCRIPT:
    case RFLM_GSM_FSK_TX_ENABLE_SCRIPT:
    case RFLM_GSM_FSK_TX_BAND_SCRIPT:
    case RFLM_GSM_FSK_TX_START_SCRIPT:
    case RFLM_GSM_FSK_TX_FRONTEND_CONFIG_SCRIPT:
    case RFLM_GSM_FSK_TX_AFC_POS_SCRIPT:
    case RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT:
    case RFLM_GSM_FSK_TX_STOP_SCRIPT:
	rflm_intf_data_ptr->curr_stack= RFLM_FSK_MODE;
	break;
	default:
	rflm_intf_data_ptr->curr_stack= RFLM_GSM_MODE;
	break;
  }
  /* RFLM trace */
                                                     ...
                                                     void *shared_mem_ptr )
{
  ...
  /* Populate the FW shared mem index value */
 #ifdef RFGSM_FSK_ENABLE
  if(core_data->rfgsm_core_handle_ptr->curr_stack == RFGSM_GSM_MODE)
  	{
 #endif
  if( !rfgsm_mdsp_populate_enter_mode_index( shared_mem_ptr, idx ) )
  {
    ...
    return FALSE;
  	...
  	{
      if( !rfgsm_mdsp_populate_fsk_enter_mode_index( shared_mem_ptr, idx ) )
       {
         MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM FSK Enter mode FW intf data index update failed");
         return FALSE;
       }
                                              ...
                                              idx ) )
  {
    ...
    return FALSE;
  	...
  	{
	  if( !rfgsm_core_dm_populate_rflm_intf_data( core_data,
												 RFLM_GSM_FSK_ENTER_MODE_SCRIPT,
												 RFGSM_CORE_DM_BURST_NUM_NOT_REQUIRED,
												 idx ) )
		  {
			MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM core DM Enter mode FW intf data update failed");
                                               ...
                                               rflm_fsk_data_type *fsk_table_ptr)
{
  ...
  if ( table_ptr == NULL && fsk_table_ptr == NULL )
  {
    MSG_2 (MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM get event addr table ptr 0x%x fsk_table_ptr 0x%x",table_ptr,fsk_table_ptr);
    return NULL;
  }
  ...
  if ( table_ptr != NULL )
  {
  		...
  		switch ( script_type )
  		{
    		...
    		case RFLM_GSM_MON_JDET_READ_SCRIPT:
      			...
      			break;
     		case RFLM_GSM_USTMR_SYNC_SCRIPT:
	  			if(table_ptr!=NULL)
      			{
      				if ( task_q == RFLM_GSM_CCS_TASK_Q_A )
      				{
         				event_addr = (void*)&table_ptr->rfgsm_ustmr_sync_event.task_q_a;
      				}
      				else
      				{
        				MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Get event address for RFLM_GSM_USTMR_SYNC_SCRIPT:%d failed",burst_num);
      				}
	  			...
	  			else
	  			{
	      			if ( task_q == RFLM_GSM_CCS_TASK_Q_A )
			 		{
			   			event_addr = (void*)&fsk_table_ptr->rfgsm_ustmr_sync_event.task_q_a;
			 		}
			 		else
			 		{
			  			MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Get event address for FSK RFLM_GSM_USTMR_SYNC_SCRIPT:%d failed",burst_num);
			 		}

	  			}
      			break;      
     		case RFLM_GSM_JDET_DISABLE_TRIGGER_SCRIPT:
      			if ( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
         			event_addr = (void*)&table_ptr->rfgsm_jdet_disable_trigger_event.task_q_a;
      			}
      			else
      			{
        			MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Get event address for RFLM_GSM_JDET_DISABLE_TRIGGER_SCRIPT:%d failed",burst_num);
      			}
      			break;   
  ...
  if ( fsk_table_ptr!= NULL )
  {
	    switch ( script_type )
  		{  				
	  		case RFLM_GSM_FSK_ENTER_MODE_SCRIPT:
	  			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_init_event.task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_init_event.task_q_b;
      			}
     		...
     		case RFLM_GSM_FSK_TX_BAND_SCRIPT:
	  			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_band_event.task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_band_event.task_q_b;
      			}
	  			break;  
     		case RFLM_GSM_FSK_TX_START_SCRIPT:
	 			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_start.task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_start.task_q_b;
      			} 	
	  			break;  
     		case RFLM_GSM_FSK_TX_TRANS_SCRIPT:
	 			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_trans[0].task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_trans[0].task_q_b;
      			} 	
	  			break; 
	 		...
	 		case RFLM_GSM_FSK_TX_AFC_POS_SCRIPT:
	  			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
		   		{
			 		event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_afc_pos_event.task_q_a;
		   		}
		   		else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
		   		{
			 		event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_afc_pos_event.task_q_b;
		   		}	     
	  			break;
	  		case RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT:
		   		if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_afc_neg_event.task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_afc_neg_event.task_q_b;
      			} 	
		  		break;
        ...
        case RFLM_GSM_FSK_TX_FRONTEND_CONFIG_SCRIPT:
              ...
              if( task_q == RFLM_GSM_CCS_TASK_Q_A )
                {
                    event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_frontend_config.task_q_a;
                }
              else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
                {
                    event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_frontend_config.task_q_b;
                }
              break;
     		...
     		case RFLM_GSM_FSK_TX_STOP_SCRIPT:
	 			if( task_q == RFLM_GSM_CCS_TASK_Q_A )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_stop.task_q_a;
      			}
      			else if( task_q == RFLM_GSM_CCS_TASK_Q_B )
      			{
        			event_addr = (void*)&fsk_table_ptr->rfgsm_fsk_tx_event.tx_stop.task_q_b;
      			} 	
	  			break;  
    		default:
      			break;
...
/*===========================================================================*/
/*! @fn rfgsm_core_get_dm_script_ptr                                         */
/*! @brief This function gets DM memory addr and execute a generic mDSP event*/
/*!        creation API                                                      */
void* rfgsm_core_get_dm_script_ptr( uint8 sub_id,
                                    rfgsm_core_dm_script_type script_type,
                                    uint8 buffer_index,
                                    uint8 burst_num,
                                    rfgsm_core_dm_task_q task_q_id )
{
  ...
  rflm_fsk_data_type *fsk_table_ptr = NULL ;
  /* Get the data table pointer from DM */
  table_ptr = rflm_gsm_dm_get_table_ptr( sub_id );
  fsk_table_ptr = rflm_fsk_dm_get_table_ptr();
  if( table_ptr == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
  ...
  return(event_mem_ptr);
}


/*===========================================================================*/
/*! @fn rfgsm_core_dm_set_freq_error                                         */
/*! @brief This function sets the frequency error in DM to be applied against*/
/*!        arfcn->freq conversion to apply the correct DAC rate              */
void rfgsm_core_dm_set_freq_error( uint8 sub_id,
                                   int16 freq_error,
                                   rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  fsk_table_ptr = rflm_fsk_dm_get_table_ptr();
#ifdef RFGSM_FSK_ENABLE
  if(rfgsm_core_handle_ptr->curr_stack == RFGSM_GSM_MODE)
  {
#endif
  if( table_ptr == NULL )
  {
    ERR_FATAL( "rfgsm_core_dm_set_freq_error() table_ptr NULL for sub %d", sub_id, 0, 0);
    return;
  }

  /* Set the frequency error */
  table_ptr->rflm_gsm_vars.freq_error = freq_error;
#ifdef RFGSM_FSK_ENABLE
  }
  else
  {
    if( fsk_table_ptr == NULL )
    {
      ERR_FATAL( "rfgsm_core_dm_set_freq_error() fsk_table_ptr NULL for sub %d", sub_id, 0, 0);
      return;
    }
  /* Set the frequency error */
    fsk_table_ptr->rflm_gsm_vars.freq_error = freq_error;
                                   ...
                                   uint16 gain)
{
  ...
  rflm_gsm_rx_data* rx_data_ptr = NULL;
  rflm_gsm_data_type *table_ptr = NULL;
  gsm_rx_wbdc_comp_data_type gsm_rx_wbdc_comp_data;
  ...
  if (rfgsm_core_is_device_prx(core_data->rfm_dev)) 
  {
    ...
    rx_data_ptr->rf_gain_state = core_data->rfgsm_core_handle_ptr->lna_range;
	
	if((core_data->rfgsm_core_handle_ptr->lna_range < GAIN_RANGE_MAX) && ftm_gsm_cal_mode_enabled(core_data->rfm_dev ) == FALSE)
    {
       gsm_rx_wbdc_comp_data= rfgsm_nv_rx_wbdc_comp( core_data->rfm_dev, core_data->rfgsm_core_handle_ptr->lna_range);
   	    rx_data_ptr->wbdc_est_i =  gsm_rx_wbdc_comp_data.wbdc_offset_i;
	    rx_data_ptr->wbdc_est_q = gsm_rx_wbdc_comp_data.wbdc_offset_q;
	  
    }
	else
   	{
   	  rx_data_ptr->wbdc_est_i = 0;
	  rx_data_ptr->wbdc_est_q = 0;   	  
   	} 


    if( ftm_gsm_cal_mode_enabled(core_data->rfm_dev ) == TRUE)
      table_ptr->rflm_gsm_vars.rf_gain_state_in_cal = core_data->rfgsm_core_handle_ptr->lna_range;
    else
      table_ptr->rflm_gsm_vars.rf_gain_state_in_cal = GAIN_RANGE_MAX;
  }
  else
  {
    /* Set the Rx data */
  ...
  rx_data_ptr->notch_index = core_data->rfgsm_core_handle_ptr->notch_index;
                                           ...
                                           rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  {
    return (fsk_table_ptr->rfgsm_txfe_data.amam_ampm_tables[ping_pong_id].amam_table);
  }
#endif
}
                                            ...
                                            rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
   ...
   {
	 MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		"RF GSM core DM get Tx ampm fsk_table_ptr NULL for sub %d",
		sub_id);
	 return NULL;
   }
  ...
  {
    return (fsk_table_ptr->rfgsm_txfe_data.amam_ampm_tables[ping_pong_id].ampm_table);
  }
#endif
}
                                                 ...
                                                 rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    if( fsk_table_ptr == NULL )
	  {
		MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
			"RF GSM core DM get Tx PA up ramp fsk_table_ptr NULL for sub %d",
			sub_id);
		return NULL;
	  }
  ...
  {
   return fsk_table_ptr->rfgsm_txfe_data.pa_frame_ramps[ping_pong_id].pa_slot_ramps[slot].pa_ramp_up_table;
  }
#endif
}
                                                   ...
                                                   rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
   ...
   {
	 MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		 "RF GSM core DM get Tx PA down ramp fsk_table_ptr NULL for sub %d",
		 sub_id);
	 return NULL;
   }
  ...
  {
  return fsk_table_ptr->rfgsm_txfe_data.pa_frame_ramps[ping_pong_id].pa_slot_ramps[slot].pa_ramp_down_table;
  }
#endif
}
                                                        ...
                                                        rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {  
	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		"RF GSM core DM get Tx master PA up ramp fsk_table_ptr NULL for sub %d",
		sub_id);
	  return NULL;
    }
  }
  ...
  {
    return fsk_table_ptr->rfgsm_txfe_data.pa_master_ramps[tx_band].pa_ramp_up_table;
  }
#endif
}
                                                           ...
                                                           rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM get Tx master PA down ramp fsk_table_ptr NULL for sub %d",
          sub_id);
      return NULL;
    }
  ...
  {
    return fsk_table_ptr->rfgsm_txfe_data.pa_master_ramps[tx_band].pa_ramp_down_table;
  }
#endif
}
                                            ...
                                            rfgsm_core_handle_type* rfgsm_core_handle_ptr )
{
  ...
  {
#endif
    table_ptr->rfgsm_burst_event[burst_buffer_id].tx.amam_table = rfgsm_core_dm_get_amam_table_addr( sub_id, ping_pong_id,rfgsm_core_handle_ptr );
    table_ptr->rfgsm_burst_event[burst_buffer_id].tx.ampm_table = rfgsm_core_dm_get_ampm_table_addr( sub_id, ping_pong_id,rfgsm_core_handle_ptr );
  ...
  {
    fsk_table_ptr->rfgsm_fsk_tx_event.amam_table = rfgsm_core_dm_get_amam_table_addr( sub_id, ping_pong_id,rfgsm_core_handle_ptr );
    fsk_table_ptr->rfgsm_fsk_tx_event.ampm_table = rfgsm_core_dm_get_ampm_table_addr( sub_id, ping_pong_id,rfgsm_core_handle_ptr );
  
    if ( fsk_table_ptr->rfgsm_fsk_tx_event.amam_table == NULL )
    {
	  MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM set Tx amam/ampm tables amam table NULL");
	  return FALSE;
    }
  
    if ( fsk_table_ptr->rfgsm_fsk_tx_event.ampm_table == NULL )
    {
	  MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM set Tx amam/ampm tables ampm table NULL");
	  return FALSE;
    }
                                       ...
                                       rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  {
      table_ptr->rfgsm_burst_event[burst_buffer_id].tx.pa_ramp_up_profile[index] = rfgsm_core_dm_get_pa_ramp_up_table_addr( sub_id, ping_pong_id, index,rfgsm_core_handle_ptr );
      table_ptr->rfgsm_burst_event[burst_buffer_id].tx.pa_ramp_down_profile[index] = rfgsm_core_dm_get_pa_ramp_down_table_addr( sub_id, ping_pong_id, index,rfgsm_core_handle_ptr );
  }
#ifdef RFGSM_FSK_ENABLE
  }
  else
  {
    fsk_table_ptr->rfgsm_fsk_tx_event.pa_ramp_up_profile[index] = rfgsm_core_dm_get_pa_ramp_up_table_addr( sub_id, ping_pong_id, index,rfgsm_core_handle_ptr );
    fsk_table_ptr->rfgsm_fsk_tx_event.pa_ramp_down_profile[index] = rfgsm_core_dm_get_pa_ramp_down_table_addr( sub_id, ping_pong_id, index,rfgsm_core_handle_ptr );
  }
#endif  
  return TRUE;
}
                                       ...
                                       rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		 "RF GSM core DM set Tx ramp deltas fsk_table_ptr NULL for sub %d",
		 sub_id);
	 return FALSE;
    }
  ...
  {
     fsk_table_ptr->rfgsm_fsk_tx_band_event.pa_up_ramp_delta = pa_ramp_up_delta;
	 fsk_table_ptr->rfgsm_fsk_tx_band_event.pa_down_ramp_delta = pa_ramp_down_delta;
  }
#endif  
  return TRUE;
} 
                                    ...
                                    rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    fsk_table_ptr->rflm_gsm_vars.tx_alpha = tx_alpha;
    fsk_table_ptr->rflm_gsm_vars.tx_alpha_rflm_overhead = RFLM_GSM_TX_MAX_PROC_OVERHEAD_QS;
  }
#endif  
  return TRUE;
} 
...
/*---------------------------------------------------------------------------*/
/*! @fn rfgsm_core_dm_set_rx_timing_fixup
 *  @brief 
 *  @param sub_id - subscription
 *  @param alpha - Rx alpha value
 *  @return boolean - function success status
...
boolean rfgsm_core_dm_set_rx_timing_fixup( rflm_gsm_subscription_id sub_id,
                                           int16 tune_fixup_rel_qs ,
                                           uint16 disable_fixup_rel_qs)
{
  /* Initialise variables */
  rflm_gsm_data_type *table_ptr = NULL;
  ...
  /* Set the Rx data */
  table_ptr->rflm_gsm_vars.rx_tune_fixup_rel_qs = tune_fixup_rel_qs;
  table_ptr->rflm_gsm_vars.rx_disable_fixup_rel_qs = disable_fixup_rel_qs;
...
/*---------------------------------------------------------------------------*/
/*! @fn rfgsm_core_dm_set_tx_timing_fixup
 *  @brief 
 *  @param sub_id - subscription
 *  @param alpha - Rx alpha value
 *  @return boolean - function success status
...
boolean rfgsm_core_dm_set_tx_timing_fixup( rflm_gsm_subscription_id sub_id,
                                           int16 tune_fixup_rel_qs ,
                                           int16 disable_fixup_rel_qs,
                                           int16 agc_fixup_rel_qs,
                                           rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  /* Initialise variables */
  rflm_gsm_data_type *table_ptr = NULL;
 ...
 if(rfgsm_core_handle_ptr->curr_stack == RFGSM_GSM_MODE)
  {
#endif  
  table_ptr->rflm_gsm_vars.tx_tune_fixup_rel_qs =  tune_fixup_rel_qs;
  table_ptr->rflm_gsm_vars.tx_disable_fixup_rel_qs = disable_fixup_rel_qs;
  table_ptr->rflm_gsm_vars.tx_agc_fixup_rel_qs = agc_fixup_rel_qs;
#ifdef RFGSM_FSK_ENABLE
  }
 else
 {
   fsk_table_ptr->rflm_gsm_vars.tx_tune_fixup_rel_qs =  tune_fixup_rel_qs;
   fsk_table_ptr->rflm_gsm_vars.tx_disable_fixup_rel_qs = disable_fixup_rel_qs;
   fsk_table_ptr->rflm_gsm_vars.tx_agc_fixup_rel_qs = agc_fixup_rel_qs;
 }
#endif
  return TRUE;
}
                                    ...
                                    rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		"RF GSM core DM set Tx power fsk_table_ptr NULL for sub %d",
		sub_id);
	  return FALSE;
    }
  ...
  {
   fsk_table_ptr->rfgsm_fsk_tx_event.tx_pwr_level_dbm10[slot_num] = RFGSM_DM_DBM100_TO_DBM10(txPower);
  }
#endif
  /*For debug only*/
                                         ...
                                         rfdevice_trx_gsm_env_delay_type path_delay )
{
  ...
  /* Set the Tx data */
  memscpy(&table_ptr->rfgsm_burst_event[burst_buffer_id].tx.path_delay, 
          sizeof(table_ptr->rfgsm_burst_event[burst_buffer_id].tx.path_delay),
          &path_delay,
          sizeof(path_delay));
                                         ...
                                         rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
	   MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		 "RF GSM core DM set Tx path delay fsk_table_ptr NULL for sub %d",
		 sub_id);
	  return FALSE;
    }
  }
  ...
  {
	fsk_table_ptr->rfgsm_fsk_tx_event.path_delay_val = path_delay_val;
  }
#endif  
  return TRUE;
}
                                   ...
                                   rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
 ...
 /* Set the Tx data */
   fsk_table_ptr->rflm_gsm_vars.num_tx_kv_cal_bits = num_kv_bits;
 }
#endif
...
//  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM set Tx envelope gain env_gain %d", env_gain);
 ...
 {
  fsk_table_ptr->rfgsm_fsk_tx_event.env_gain[slot_num] = env_gain;
 }
#endif 
  return TRUE;
}
                                        ...
                                        rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
	   MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
		 "RF GSM core DM set Tx predist flag fsk_table_ptr NULL for sub %d",
		 sub_id);
	   return FALSE;
    }
  }
  ...
  {
   fsk_table_ptr->rfgsm_fsk_tx_event.predist_enabled[slot_num] = enabled;
  }
#endif
  return TRUE;
}
                                                         ...
                                                         rfgsm_core_handle_type* rfgsm_core_handle_ptr)
{
  ...
  else
  {
    ...
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
          "RF GSM core DM set Tx amam/ampm tbl flag fsk_table_ptr NULL for sub %d",
          sub_id);
      return FALSE;
    }
  }
  ...
  {
   fsk_table_ptr->rfgsm_fsk_tx_event.program_amam_ampm_tables = flag;
  }
#endif
  return TRUE;
}

void rf_gsm_dm_rf_event_token_update(uint8 script_loop,rf_buffer_intf* script, rflm_gsm_event_token_info *event_token_info,boolean is_rffe_group)
{
  rf_hal_bus_resource_script_type* script_ptr = NULL;
 ...
 if ((is_rffe_group == TRUE) && (NULL!= script) && (event_token_info->is_token_updated==FALSE))
 {
    /* Get script transactions */
    script_ptr = (rf_hal_bus_resource_script_type*)rf_buffer_get_script_ptr(script, RF_HAL_BUS_RESOURCE_RFFE_GROUP);

    if ((script_ptr != NULL) && (script_ptr->script_type == RF_HAL_BUS_RESOURCE_RFFE_GROUP))
    {
      for (group_loop = 0; group_loop < script_ptr->num_rffe_groups; group_loop++)
      {
        event_token_info->num_tokens += (script_ptr->script_ptr.rffe_groups)[group_loop].num_tokens; 
        for (token_loop = 0; token_loop < (script_ptr->script_ptr.rffe_groups)[group_loop].num_tokens; token_loop++)
        {
          if (token_index < RFLM_GSM_CCS_MAX_TOKEN_PER_EVENT)
          {
            event_token_info->token_type[token_index] =\
                  (rflm_gsm_fixup_token_type)((script_ptr->script_ptr.rffe_groups)[group_loop].rffe_token[token_loop].token_id);
            event_token_info->token_length_bytes[token_index] =\
                  (script_ptr->script_ptr.rffe_groups)[group_loop].rffe_token[token_loop].token_id_len;
            event_token_info->token_idx[token_index] = token_index;
//            MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "event_token_info->token_type[token_index]=%d", event_token_info->token_type[token_index]); 
            token_index++;
            event_token_info->is_token_updated = TRUE;
            
          }
          else
          {
            MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "Num tokens exceeded limit for tokens/EVENT, Max tokens=%d", RFLM_GSM_CCS_MAX_TOKEN_PER_EVENT); 
            break;
          }
        }
       }
...
/*! @brief This function gets DM memory addr and execute a generic mDSP event*/
/*!        creation API                                                      */
boolean rfgsm_core_dm_create_event( void** event_handle,
                                    ...
                                    rflm_fsk_data_type *fsk_table_ptr)
{
  ...
  if( !rfgsm_mdsp_create_buffer( event_handle,
                                 event_mem_ptr,
                                 script_type,
                                 task_q_id ) )
  {
                                                    ...
                                                    boolean is_rffe_group)
{
  ...
  if( !rfgsm_core_dm_create_event( event_handle_ptr , core_data, table_ptr, script_type, burst_num, task_q,fsk_table_ptr) )
  {
    ...
    switch(script_type)
    {
      ...
      case RFLM_GSM_RX_ACQ_START_SCRIPT:
      case RFLM_GSM_RX_PCH_START_SCRIPT:
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.rx_start_tokens),is_rffe_group);
      break;
      case RFLM_GSM_RX_STOP_SCRIPT:
      case RFLM_GSM_MON_STOP_SCRIPT:
      case RFLM_GSM_RX_ACQ_STOP_SCRIPT:
      case RFLM_GSM_RX_PCH_STOP_SCRIPT:
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.rx_stop_tokens),is_rffe_group);
      break;
      case RFLM_GSM_TX_START_SCRIPT:
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.tx_start_tokens),is_rffe_group);
      break;		
      case RFLM_GSM_TX_STOP_SCRIPT:	
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.tx_stop_tokens),is_rffe_group);
      break;
      case RFLM_GSM_TX_TRANS_SCRIPT:   
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.tx_trans_tokens),is_rffe_group);	
      break;
	  case RFLM_GSM_JDET_DISABLE_TRIGGER_SCRIPT:
        rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(table_ptr->rflm_gsm_vars.ccs_event_tokens.jdet_disable_tokens),is_rffe_group);
      break;
	  case RFLM_GSM_FSK_TX_START_SCRIPT:
	  	 rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(fsk_table_ptr->rflm_gsm_vars.ccs_event_tokens.fsk_tx_start_tokens),is_rffe_group);
      break;
	  case RFLM_GSM_FSK_TX_STOP_SCRIPT:
	  	 rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(fsk_table_ptr->rflm_gsm_vars.ccs_event_tokens.fsk_tx_stop_tokens),is_rffe_group);
      break;
	  case RFLM_GSM_FSK_TX_TRANS_SCRIPT:
	  	 rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(fsk_table_ptr->rflm_gsm_vars.ccs_event_tokens.fsk_tx_trans_tokens),is_rffe_group);
      break;
	  case RFLM_GSM_FSK_TX_AFC_POS_SCRIPT:
	  	 rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(fsk_table_ptr->rflm_gsm_vars.ccs_event_tokens.fsk_tx_afc_pos_tokens),is_rffe_group);
      break;
	  case RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT:
	  	 rf_gsm_dm_rf_event_token_update(index, ccs_scripts[index], &(fsk_table_ptr->rflm_gsm_vars.ccs_event_tokens.fsk_tx_afc_neg_tokens),is_rffe_group);
      break;
      default:
      break;
    }
  ...
  for (index = 0; index < num_scripts ; index++) 
  {
    if (NULL != ccs_scripts[index])
    {
      ...
      if( !rfgsm_core_dm_push_script( *event_handle_ptr , RF_HAL_BUS_RESOURCE_RFFE_GROUP, ccs_scripts[index], script_type, task_q, core_data->gl1_lower_rx_ccs_prio, core_data->rfgsm_core_handle_ptr->ssbi_protocol_supported) )
      {
        MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM dynamic event wrapper push failed");
        status &= FALSE;
      }
...
{
 boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* enter_mode_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.enter_mode_script,
                                                       NULL};
 if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  }
  else
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_enter_event_wrapper(), ccs_handles is NULL!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void*)&ccs_handles->entermode_event,
                                            0,
                                            is_rffe_group) )
  {
...
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_ustmr_sync_event_wrapper( rfgsm_core_data_type *core_data )
{
 boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* ustmr_sync_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.ustmr_sync_script,
                                                          NULL};
  if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_ustmr_sync_event_wrapper(), ccs_handles is NULL!");
	
  }

  /* Program the event to data table */
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_USTMR_SYNC_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)ustmr_sync_ccs_scripts_taskq_a,
                                            sizeof(ustmr_sync_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)ustmr_sync_ccs_scripts_taskq_a),
                                            (void*)&ccs_handles->ustmrsync_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM Enter mode RFLM dynamic event prog failed!");
...
boolean rfgsm_core_dm_dynamic_jdet_disable_trigger_event_wrapper( rfgsm_core_data_type *core_data )
{
  boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* jdet_disable_trigger_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.jdet_disable_trigger_script,
                                                          NULL};
  if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
    ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_jdet_disable_trigger_event_wrapper(), ccs_handles is NULL!");
  ...
  /* Program the event to data table */
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_JDET_DISABLE_TRIGGER_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)jdet_disable_trigger_scripts_taskq_a,
                                            sizeof(jdet_disable_trigger_scripts_taskq_a)/sizeof((rf_buffer_intf**)jdet_disable_trigger_scripts_taskq_a),
                                            (void*)&ccs_handles->jdet_disable_trigger_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM Enter mode RFLM dynamic event prog failed!");
...
boolean rfgsm_core_dm_dynamic_jdet_read_event_wrapper( rfgsm_core_data_type *core_data_ptr, uint8 burst_num, rflm_gsm_script_type jdet_script_type )
{
  ...
  if(core_data_ptr!=NULL && core_data_ptr->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data_ptr->rfm_dev];
  }
  else
  {
     MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_jdet_read_event_wrapper(), ccs_handles is NULL with core_data_ptr is 0x%x !",core_data_ptr);
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data_ptr,
                                            ...
                                            (void*)&ccs_handles->jdetread_event,
                                            burst_num,
                                            is_rffe_group) )
  {
...
boolean rfgsm_core_dm_dynamic_wakeup_event_wrapper( rfgsm_core_data_type *core_data )
{
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_wakeup_event_wrapper(), ccs_handles is NULL!");
...
boolean rfgsm_core_dm_dynamic_tx_band_event_wrapper( rfgsm_core_data_type *core_data )
{
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_tx_band_event_wrapper(), ccs_handles is NULL!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void*)&ccs_handles->tx_band_event,
                                            0,
                                            is_rffe_group) )
  {
...
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_fsk_enter_event_wrapper( rfgsm_core_data_type *core_data )
{
 boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* fsk_enter_mode_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_enter_mode_script,
                                                       NULL};
 if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_ENTER_MODE_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)fsk_enter_mode_ccs_scripts_taskq_a,
                                            sizeof(fsk_enter_mode_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)fsk_enter_mode_ccs_scripts_taskq_a),
                                            (void*)&ccs_handles->fsk_enter_mode_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM Enter mode RFLM dynamic event prog failed!");
...
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_fsk_tx_band_event_wrapper( rfgsm_core_data_type *core_data )
{
  boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* fsk_tx_band_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_band_script,
                                                    NULL};
  if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_fsk_tx_band_event_wrapper(), ccs_handles is NULL!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_BAND_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)fsk_tx_band_ccs_scripts_taskq_a,
                                            sizeof(fsk_tx_band_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)fsk_tx_band_ccs_scripts_taskq_a),
                                            (void*)&ccs_handles->fsk_tx_band_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx Enable RFLM dynamic event prog failed!");
    return FALSE;
  }
...
boolean rfgsm_core_dm_dynamic_fsk_tx_afc_pos_event_wrapper( rfgsm_core_data_type *core_data, uint8 num_tx_slots )
{
  boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* fsk_tx_afc_pos_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_afc_pos_script,
                                                    NULL};
  if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_fsk_tx_afc_pos_event_wrapper(), ccs_handles is NULL!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_AFC_POS_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)fsk_tx_afc_pos_ccs_scripts_taskq_a,
                                            sizeof(fsk_tx_afc_pos_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)fsk_tx_afc_pos_ccs_scripts_taskq_a),
                                            (void*)&ccs_handles->fsk_tx_afc_pos_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx AFC POS RFLM dynamic event prog failed!");
    return FALSE;
  }
...
boolean rfgsm_core_dm_dynamic_fsk_tx_afc_neg_event_wrapper( rfgsm_core_data_type *core_data, uint8 num_tx_slots )
{
  boolean is_rffe_group = TRUE;
  rfgsm_mdsp_ccs_handles_s* ccs_handles=NULL;
  rf_buffer_intf* fsk_tx_afc_neg_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_afc_neg_script,
                                                    NULL};
  if(core_data->rfm_dev < RFM_MAX_WAN_DEVICES)
  {
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
  ...
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_fsk_tx_afc_neg_event_wrapper(), ccs_handles is NULL!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)fsk_tx_afc_neg_ccs_scripts_taskq_a,
                                            sizeof(fsk_tx_afc_neg_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)fsk_tx_afc_neg_ccs_scripts_taskq_a),
                                            (void*)&ccs_handles->fsk_tx_afc_neg_event,
                                            0,
                                            is_rffe_group) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx AFC NEG RFLM dynamic event prog failed!");
    return FALSE;
  }
...
/*! @brief This function acts as a wrapper for the dynamic event method to be*/
/*!        backwards compatible with the old event building method           */
boolean rfgsm_core_dm_dynamic_fsk_tx_event_wrapper( rfgsm_core_data_type *core_data,
                                                uint8 num_tx_slots )
{
 boolean is_rffe_group = TRUE;
  ...
  rfgsm_mdsp_ccs_handles_s* ccs_handles = NULL;
   ...
   {
	  MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_tx_event_wrapper(), ccs_handles is NULL!");
  ...
  rf_buffer_intf* tx_frontend_config_ccs_scripts_taskq_b[] = { (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ATUNER_SCRIPT],
                                                               NULL};

  rf_buffer_intf* tx_start_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_WTR_SCRIPT],
                                                    (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_APT_SCRIPT],
                                                     NULL};

  rf_buffer_intf* tx_start_ccs_scripts_taskq_b[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ASM_SCRIPT],
                                                    (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_PA_SCRIPT],
                                                    (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_GRFC_SCRIPT],
                                                     NULL};

  rf_buffer_intf* tx_stop_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_WTR_SCRIPT],
                                                    NULL};
  rf_buffer_intf* tx_trans_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_WTR_SCRIPT][0],
                                                    NULL};
  rf_buffer_intf* tx_trans_ccs_scripts_taskq_b[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_FE_SCRIPT][0],
                                                    NULL};

  rf_buffer_intf* tx_stop_ccs_scripts_taskq_b[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_ASM_SCRIPT],
                                                   (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_PA_SCRIPT],
                                                   (rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_GRFC_SCRIPT],
                                                    NULL};
  rf_buffer_intf* tx_afc_pos_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_afc_pos_script,
                                                    NULL};
  rf_buffer_intf* tx_afc_neg_ccs_scripts_taskq_a[] = {(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_afc_neg_script,
                                                    NULL};
  ...
  /* Program the event to data table */
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_FRONTEND_CONFIG_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[FE_TASKQA],
                                            0,
                                            (!is_rffe_group) ))
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx Frontend Config RFLM dynamic event prog failed!");
    return FALSE;
  }
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[FE_TASKQB],
                                            0,
                                            (!is_rffe_group) ) )
  {
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[START_TASKQA],
                                            0,
                                            is_rffe_group) ) 
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx Start RFLM dynamic event prog failed!");
    return FALSE;
  }
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[START_TASKQB],
                                            0,
                                            (!is_rffe_group) ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx Start RFLM dynamic event prog failed!");
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[STOP_TASKQA],
                                            num_tx_slots-1,
                                            is_rffe_group
                                            ) )
  {
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_AFC_POS_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)tx_afc_pos_ccs_scripts_taskq_a,
                                            sizeof(tx_afc_pos_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)tx_afc_pos_ccs_scripts_taskq_a),
                                            (void *)&ccs_handles->fsk_tx_afc_pos_event,
                                            num_tx_slots-1,
                                            is_rffe_group
                                            ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM Tx AFC POS RFLM dynamic event prog failed!");
    return FALSE;
  }
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            RFLM_GSM_FSK_TX_AFC_NEG_SCRIPT,
                                            RFLM_GSM_CCS_TASK_Q_A,
                                            (rf_buffer_intf**)tx_afc_neg_ccs_scripts_taskq_a,
                                            sizeof(tx_afc_neg_ccs_scripts_taskq_a)/sizeof((rf_buffer_intf**)tx_afc_neg_ccs_scripts_taskq_a),
                                            (void *)&ccs_handles->fsk_tx_afc_pos_event,
                                            num_tx_slots-1,
                                            is_rffe_group
                                            ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM Tx AFC neg RFLM dynamic event prog failed!");
    return FALSE;
  }
  ...
  if( !rfgsm_core_dm_dynamic_event_wrapper( core_data,
                                            ...
                                            (void *)&ccs_handles->fsk_tx_event[STOP_TASKQB],
                                            num_tx_slots-1,
                                            (!is_rffe_group) ) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM FSK Tx Stop RFLM dynamic event prog failed!");
    return FALSE;
  }
  for( index = 0; index < 1; index++ )
   {
	 tx_transition_ccs_scripts_taskq_a[0]=(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_WTR_SCRIPT][0];
	 tx_transition_ccs_scripts_taskq_b[0]=(rf_buffer_intf *)core_data->rfgsm_core_handle_ptr->scripts.fsk_tx_trans_script[TX_TRANS_FE_SCRIPT][0];
    ...
    {
       MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM tx trans tq_b RFLM dynamic event prog passed!");
    }
    rflm_fsk_data_type *fsk_table_ptr = NULL;
    fsk_table_ptr=rflm_fsk_dm_get_table_ptr();
    if(fsk_table_ptr!=NULL)
  	{
  	  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Tx trans tq_b event wrapper num_task %d",
	  	    rflm_get_num_tasks(&fsk_table_ptr->rfgsm_fsk_tx_event.tx_trans[0].task_q_b));
  	}
    else
  	{
  	 MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"RF GSM trans tq_b RFLM dynamic event table_ptr is NULL");
  	}
   }
                                                ...
                                                uint8 rx_burst_num )
{
  ...
  {
     MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_rx_event_wrapper(), rfm_dev %d is invalid",core_data->rfm_dev);
	 
  }
  if(ccs_handles==NULL)
  {
     MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_rx_event_wrapper(), ccs_handles is NULL!");
  }
...
boolean rfgsm_core_dm_dynamic_therm_event_wrapper( rfgsm_core_data_type *core_data )
{
  ...
  ccs_handles = rfgsm_mdsp_ccs_handles[core_data->rfm_dev];
   ...
   {
	  MSG(MSG_SSID_RF, MSG_LEGACY_ERROR,"rfgsm_core_dm_dynamic_therm_event_wrapper(), ccs_handles is NULL!");
  ...
  event_handle=(void*)&ccs_handles->therm_read_event;
  /* Retrieve DM memory pointer for therm read event */
  if ((event_mem_ptr = rfgsm_core_dm_get_data_table_event_addr( table_ptr, 0, RFLM_GSM_THERM_READ_SCRIPT, 0, RFLM_GSM_CCS_TASK_Q_A,fsk_table_ptr )) == NULL)
  {
  ...
  /* Create the event */
  if( !rfgsm_core_dm_create_event(event_handle, core_data, table_ptr, RFLM_GSM_THERM_READ_SCRIPT, 0, RFLM_GSM_CCS_TASK_Q_A,fsk_table_ptr) )
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM therm event create failed");
    return FALSE;
  }

  if(ccs_handles == NULL||event_handle == NULL)
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "RF GSM core DM therm event handle is NULL");
    return FALSE;
  }

  /* Push any SSBIs from the device script to the event */
   if( !rfgsm_core_dm_push_script(*event_handle, RF_HAL_BUS_RESOURCE_RFFE_GROUP, script, RFLM_GSM_THERM_READ_SCRIPT, RFLM_GSM_CCS_TASK_Q_A, FALSE, FALSE) )
   {
...
boolean rfgsm_core_dm_hmsc_state( rflm_gsm_subscription_id sub_id,
                                   boolean hmsc_active)
{
  /* Initialise variables */
  rflm_gsm_data_type *table_ptr = NULL;
  ...
  /* Set the Tx data */
  table_ptr->rflm_gsm_vars.hmsc_active = hmsc_active;
