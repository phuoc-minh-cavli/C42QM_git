...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core_temp_comp.c#2 $
...
04/30/19   mpa     to enable temp comp disable by debug nv
04/05/19   mpa     disable therm_event  on Twizy
...
static const uint32 RFGSM_CORE_TEMP_COMP_START_INT = 100; /* milliseconds */

#define RFGSM_CORE_INITIAL_THERM_READ 0xFFFF
#define RFGSM_CORE_INVALID_THERM_READ 0xFFFF
...
boolean rfgsm_core_temp_comp_data_init(rfm_device_enum_type rfm_dev)
{
 ...
 /* Structure to hold temp comp information (default is set to mid-bins) */
  ...
  //rfgsm_debug_flags.disable_temp_comp =FALSE;

  
#endif
  if(rfgsm_debug_flags.disable_temp_comp)
  {
   MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"Therm read is disabled via Debug NV temp %d!", rfgsm_debug_flags.disable_temp_comp);
  }
  return TRUE;
}
...
#define RFGSM_CORE_TEMP_COMP_THERM_MAX 6323
#define RFGSM_CORE_TEMP_COMP_THERM_MIN 3602
...
void rfgsm_core_temp_comp_trigger(int32 rfm_dev)
{
  ...
  temp_comp_cmd->payload[0] = (uint8)rfm_dev;
  ...
  RF_MSG_3( RF_ERROR, "raw_therm %d "
					  "payload_1 %d "
					  "payload_2 %d", 
			          (rfgsm_core_handle_ptr->therm_read.raw_therm),(temp_comp_cmd->payload[1]),(temp_comp_cmd->payload[2]));
...
{ 
  ...
  rfgsm_core_temp_comp_update_req(rfm_dev, temp_comp_scaled_value, rfgsm_core_handle_ptr->temp_comp.temp_bin_16);
}
...
int32 rfgsm_core_temp_comp_get_bb_headroom(rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band)
{
  ...
  {
    RFGC_MSG_1( MSG_LEGACY_ERROR, "rfgsm_core_handle_ptr->rfgsm_core_bb_headroom_tbl[band] = %d", rfgsm_core_handle_ptr->rfgsm_core_bb_headroom_tbl[band] );
...
int16 rfgsm_core_temp_comp_proces_raw_therm_read( rfm_device_enum_type rfm_dev, uint16 current_therm_read ) 
{
  ...
  if((rfgsm_core_handle_ptr->therm_read.curr_therm_read > RFGSM_CORE_TEMP_COMP_THERM_MAX)||
  	(rfgsm_core_handle_ptr->therm_read.curr_therm_read < RFGSM_CORE_TEMP_COMP_THERM_MIN) )
  {
  ...
  if( enable_logging )
  {
    RFGC_MSG_1( MSG_LEGACY_HIGH, "Therm value = %d", (uint16)therm_value );
  }
...
/*!
   ...
   TRUE/FALSE
*/
static uint32 therm_preempt_count=0;
boolean rfgsm_core_process_therm_read_rsp(rfgsm_core_data_type *core_data, uint8 tag, boolean therm_read_ok)
{
  ...
  rf_hal_bus_sbi_type read_ssbi_buf[RFGSM_CORE_NUM_THERM_READ_SSBI_REGS];
  uint16 num_rffe_retrieved;
  uint16 num_ssbi_retrieved;
  rfdevice_id_enum_type rfdevice_id = RFDEVICE_INVALID;
  rfcom_gsm_band_type band = RFCOM_NUM_GSM_BANDS;
  uint32 therm_data=0XFFFFFFFF;
  if (therm_read_ok)
  {
    //get physical device information as WTR2605 therm read is based on ssbi
    band = rfc_gsm_get_supported_band(core_data->rfm_dev,RFC_CONFIG_TX);
    rfdevice_id = rfc_gsm_get_device_id(core_data->rfm_dev, RFC_CONFIG_TX, band, RFDEVICE_TRANSCEIVER, 0);
	   
    if(!rfgsm_mdsp_get_therm_data(core_data->rfgsm_core_handle_ptr, tag, &therm_data ))
    {
      RF_MSG_1( MSG_LEGACY_ERROR, "rfgsm_mdsp_ct_therm_data FAILED; event_handle_tag %d", tag );
	  return FALSE;
    }

         /*retrieve the raw therm read from device driver*/
         core_data->rfgsm_core_handle_ptr->therm_read.raw_therm = rfdevice_gsm_get_therm_read( core_data->rfm_dev,therm_data);
         RF_MSG_1( MSG_LEGACY_ERROR, "Therm read raw_therm; 0%x", core_data->rfgsm_core_handle_ptr->therm_read.raw_therm );
	/* trigger the temp comp calculations */
	rfgsm_core_temp_comp_trigger(core_data->rfm_dev);
  ...
  {
    therm_preempt_count++;
    RF_MSG_2( MSG_LEGACY_ERROR, "Therm read cancelled; event_handle_tag %d with count ", tag,therm_preempt_count );
...
void rfgsm_core_temp_comp_update_req(rfm_device_enum_type rfm_dev, int16 temp_comp_scaled_value, uint8 temp_bin_16)
{
 ...
 if (msgr_send_status != E_SUCCESS)
  {
    RF_MSG_2(RF_ERROR,"RF APPS cmd UPDATE_TEMP_COMP unsuccessful UMID 0x%x: msg_send failed: %d", RFA_RF_GSM_UPDATE_TEMP_COMP_REQ, msgr_send_status );
  }
  else
  {
    RF_MSG_2(RF_HIGH,"RF APPS cmd UPDATE_TEMP_COMP successful UMID 0x%x: msgr_send_success: %d", RFA_RF_GSM_UPDATE_TEMP_COMP_REQ, msgr_send_status );
  }
}
