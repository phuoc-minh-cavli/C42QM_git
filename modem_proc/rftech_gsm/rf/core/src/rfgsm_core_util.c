...
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/core/src/rfgsm_core_util.c#3 $
...
uint16 rfgsm_core_convert_tx_freq_to_arfcn(rfcom_gsm_band_type band, uint32 rf_freq_in_khz )
{
  uint16 arfcn=0;
  uint32 min_rf_freq=0;
  uint32 max_rf_freq=0;

  uint32 rf_freq;

  rf_freq = rf_freq_in_khz*1000;


  switch (band) 
  {
  case RFCOM_BAND_GSM850:
  	min_rf_freq = RFGSM_CORE_850_TX_MIN_FREQ;
    max_rf_freq = RFGSM_CORE_850_TX_MAX_FREQ;
    if(rf_freq>=max_rf_freq)
    {
       arfcn= 251;
	}
	else if(rf_freq <= min_rf_freq)
	{
      arfcn=128;
	}
	else
	{
      arfcn= ((rf_freq-RFGSM_CORE_850_TX_MIN_FREQ)/200000)+128;
	}
    
    break;

  case RFCOM_BAND_GSM900:
	min_rf_freq = RFGSM_CORE_900_TX_MIN_FREQ;
    max_rf_freq = RFGSM_CORE_900_TX_MAX_FREQ;
    

    //for 0 -124   freq is  890000000-914800000
    //for 975-1023 freq is  880200000-889800000
    if(rf_freq <=RFGSM_CORE_900_TX_MIN_FREQ)
    {
     arfcn=975;
	}
	else if(rf_freq<=889800000)
	{
     arfcn= ((rf_freq-890000000)/200000)+1024;
	}
	else if(rf_freq <=914800000)
	{
      arfcn= ((rf_freq-890000000)/200000);
	}
	else if(rf_freq >= RFGSM_CORE_900_TX_MAX_FREQ)
	{
      arfcn=124;
	}
  ...
  case RFCOM_BAND_GSM1800:
	min_rf_freq = RFGSM_CORE_1800_TX_MIN_FREQ;
    max_rf_freq = RFGSM_CORE_1800_TX_MAX_FREQ;
    if(rf_freq>=max_rf_freq)
    {
       arfcn= 885;
	}
	...
	{
      arfcn= ((rf_freq-RFGSM_CORE_1800_TX_MIN_FREQ)/200000)+512;
	}
	
    break;

  case RFCOM_BAND_GSM1900:
	min_rf_freq = RFGSM_CORE_1900_TX_MIN_FREQ;
    max_rf_freq = RFGSM_CORE_1900_TX_MAX_FREQ;
    if(rf_freq>=max_rf_freq)
    {
       arfcn= 810;
	}
	...
	{
      arfcn= ((rf_freq-RFGSM_CORE_1900_TX_MIN_FREQ)/200000)+512;
	}
    break;
  ...
  default:
   ...
   MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"Unsupported RF band: %d arfcn %d", band,arfcn);
  return arfcn;
