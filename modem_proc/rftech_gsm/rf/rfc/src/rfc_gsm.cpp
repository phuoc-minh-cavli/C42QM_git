...
$DateTime: 2023/05/02 02:07:24 $ $Author: mplcsds1 $
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/rfc/src/rfc_gsm.cpp#4 $

when       who     what, where, why
-------------------------------------------------------------------------------
04/03/23   kum     [CR3449917] FSK Tx script buffer cleared properly before reusing it
08/19/21   aad     Added tuner support for CT 2.0
04/17/19   tik     Separate PAM client voting for PC3 vs PC5
02/28/18   mpa     Disable IP2_caL
11/30/16   bp      CR_1095525: Featurize Qtuner code
...
#define RFC_GSM_RX_MDSP_OVERHEAD 230
...
*/
void rfc_gsm::fsk_tx_burst_config( rfm_device_enum_type rfm_dev,
                               rfcom_gsm_band_type rfgsm_band,
                               uint16 arfcn)
{
  uint8 i = 0;
  ...
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_GRFC_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_GRFC_SCRIPT],
                         RF_BUFFER_ALL);

  /*Clear the ASM device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ASM_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_ASM_SCRIPT],
                         RF_BUFFER_ALL);

  /*Clear the PA device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_PA_SCRIPT],
                         RF_BUFFER_ALL);
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_PA_SCRIPT],
                         RF_BUFFER_ALL);

#ifdef FEATURE_RF_HAS_QTUNER						 
  /*Clear the ATUNER device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ATUNER_SCRIPT],
                         RF_BUFFER_ALL);
#endif
  ...
  if (tx_supported == TRUE)
  {
    ...
    /* Configure for current band GRFC settings */
    rfc_common_tdd_update_grfc_script(rfc_gsm_tx_sig_list_ptrs[rfm_dev][rfgsm_band],
                                      (void *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_GRFC_SCRIPT],
                                      (void *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_GRFC_SCRIPT]);
...
//  /* configure the rf-front-end devices */
    ...
    for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
    {
      ...
      if (dev_list != NULL)
      {
        ...
        while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
        {
          status = fsk_tx_burst_device_config(rfm_dev, dev_list[index], rfgsm_band, arfcn, rfgsm_core_handle_ptr->txlm_buf_index);
...
/*!
  @brief
   ...
   device type, calls a specific API to configure the device for a specified tx band every tx burst.
*/
boolean rfc_gsm::fsk_tx_burst_device_config(rfm_device_enum_type rfm_dev,
                                        rfdevice_class *dev_obj,
                                        rfcom_gsm_band_type band,
                                        uint16 arfcn,
                                        uint32 txlm_buf_index)
...
//  uint32 freq=0;
  ...
  case RFDEVICE_ASM:
    ...
    rfdevice_asm_enable(&asm_cfg_s,
                        dev_obj,
                        (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ASM_SCRIPT],
                        RFDEVICE_CREATE_SCRIPT,
                        timing_info.start_delta);
    ...
    rfdevice_asm_disable(&asm_cfg_s,
                         dev_obj,
                         (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_ASM_SCRIPT],
                         RFDEVICE_CREATE_SCRIPT,
                         timing_info.stop_delta);
  ...
  case RFDEVICE_PA:
    ...
    rfdevice_set_pa_on_off( &pa_params_s, TRUE, (void *)dev_obj,
                            (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_PA_SCRIPT],
                            RFDEVICE_CREATE_SCRIPT,
                            timing_info.start_delta);

    rfdevice_set_pa_on_off( &pa_params_s, FALSE, (void *)dev_obj,
                            (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_stop_script[TX_PA_SCRIPT],
                            RFDEVICE_CREATE_SCRIPT,
                            timing_info.stop_delta);
  ...
  case RFDEVICE_TUNER_MANAGER:
    ...
    ret_val = ((rfcommon_atuner_intf*)dev_obj)->set_tx_mode(RFM_EGSM_MODE,
               ...
               arfcn,
               RFDEVICE_CREATE_SCRIPT,
               (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_ATUNER_SCRIPT],
               timing_info.start_delta,
               &script_token,
               &cl_info
               );
...
*/
void rfc_gsm::fsk_tx_gain_config( rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band,
                               ...
                               uint16 bias, uint8 apt_mode, boolean override_start_time)
{
  ...
  if (slot_num)
    ...
    apt_script_buf = (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_start_script[TX_APT_SCRIPT];
...
*/
int32 rfc_gsm::fsk_tx_disable( rfm_device_enum_type rfm_dev)
{
  ...
  /*Clear the PA device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_PA_SCRIPT],
                         RF_BUFFER_ALL);
  /*Clear the APT device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_APT_SCRIPT],
               RF_BUFFER_ALL);
#ifdef FEATURE_RF_HAS_QTUNER             
  /*Clear the FE device scripts before populating*/
  rf_buffer_clear_script((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_ATUNER_SCRIPT],
               RF_BUFFER_ALL);
#endif
  rfc_data = rfc_gsm_data::get_instance();
  if (rfc_data == NULL)
...
//  /* configure the rf-front-end devices */
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      ...
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        status = fsk_tx_disable_device_config(dev_list[index], (rfcom_gsm_band_type)cfg.band, arfcn, rfm_dev);
...
*/
boolean rfc_gsm::fsk_tx_disable_device_config(rfdevice_class *dev_obj, rfcom_gsm_band_type rfgsm_band, uint16 arfcn, rfm_device_enum_type rfm_dev)
{
  boolean status = FALSE;
  rfdevice_type_enum_type dev_type = RFDEVICE_TYPE_INVALID;
  rfdevice_pa_config_type pa_params_s;
  ...
  case RFDEVICE_PA:
    ...
    rfdevice_pa_sleep(&pa_params_s,
					dev_obj,
					(rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_PA_SCRIPT],
					RFDEVICE_CREATE_SCRIPT,
  ...
  case RFDEVICE_PAPM:
    ...
    ((rfdevice_papm *)dev_obj)->disable(rfm_dev, RFM_EGSM_MODE,
                                        (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_APT_SCRIPT],
                                        RFDEVICE_CREATE_SCRIPT);
    ((rfdevice_papm *)dev_obj)->trigger((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_APT_SCRIPT],
                                        rfm_dev,
                                        RFDEVICE_CREATE_SCRIPT,
                                        0);
#endif
  ...
  case RFDEVICE_TUNER_MANAGER:
    ...
    status = ((rfcommon_atuner_intf*)dev_obj)->tx_disable(RFM_EGSM_MODE,
                                                          ...
                                                          arfcn,
                                                          RFDEVICE_CREATE_SCRIPT,
                                                          (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_disable_script[TX_ATUNER_SCRIPT],
                                                          0,
                                                          &script_token );
#endif
#endif
...
*/
int32 rfc_gsm::fsk_tx_enable( rfm_device_enum_type rfm_dev)
{
  ...
  for (i = 0; i < RFDEVICE_TYPE_MAX_NUM; i++)
  {
    ...
    if (dev_list != NULL)
    {
      ...
      while (dev_list[index] != NULL && index < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
      {
        status = fsk_tx_enable_device_config(dev_list[index], rfm_dev);
...
boolean rfc_gsm::fsk_tx_enable_device_config(rfdevice_class *dev_obj, rfm_device_enum_type rfm_dev)
{
  boolean status = FALSE;
  rfdevice_type_enum_type dev_type = RFDEVICE_TYPE_INVALID;
  rfcom_band_type_u rfcom_band;
  ...
  case RFDEVICE_PA:
	...
	rfdevice_pa_wakeup(&pa_params_s,
                        dev_obj,
                        (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_enable_script[TX_PA_SCRIPT],
                        RFDEVICE_CREATE_SCRIPT,
                        RFCOM_INVALID_TIMING_OFFSET);
#endif
    status = TRUE;
  ...
  case RFDEVICE_PAPM:
    ...
    ((rfdevice_papm *)dev_obj)->enable(rfm_dev, 
                                       ...
                                       (rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_enable_script[TX_APT_SCRIPT],
                                       RFDEVICE_CREATE_SCRIPT);
    ((rfdevice_papm *)dev_obj)->trigger((rf_buffer_intf *)rfgsm_core_handle_ptr->scripts.fsk_tx_enable_script[TX_APT_SCRIPT],
                                       ...
                                       boolean bias_flag, uint16 bias, uint8 apt_mode, boolean override_start_time)
{
  ...
  case RFDEVICE_PA:
	...
	MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfdevice_set_pa_txagc: pa_range = %d; mod = %d;",pa_range, mod_type);
                            ...
                            rfm_path_state state, rfm_mode_enum_type mode)
{
  ...
  vreg_param.set_pwr_class(RFC_RF_POWER_CLASS_DEFAULT);
...
void rfc_gsm::init_rf_devices(rfc_cfg_params_type *cfg, rfc_device_info_type *device_info_ptr)
{
  ...
  rfdevice_trx_gsm_rx_intf_class *rx_dev_obj = NULL;
  rfdevice_trx_gsm_tx_intf_class *tx_dev_obj = NULL;
  ...
  while (i < device_info_ptr->num_devices)
  {
    ...
    case RFDEVICE_TRANSCEIVER:
      if (cmn_dev_obj != NULL)
      {
        ...
        {
          rx_dev_obj = (rfdevice_trx_gsm_rx_intf_class*)((rfdevice_trx_common_intf_class *)cmn_dev_obj)->get_tech_instance( RFDEVICE_TRX_STD_GSM, RFDEVICE_RX);

          if (rx_dev_obj != NULL)
          {
            gsm_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][dev_type][local_instance] = rx_dev_obj;
                    /* Call Rx set band data */
            rfdevice_gsm_rx_set_band_data
            (
              rx_dev_obj,
              (rfcom_gsm_band_type)cfg->band, 
        ...
        else if (cfg->rx_tx == RFC_CONFIG_TX)
        {
          ...
          tx_dev_obj = (rfdevice_trx_gsm_tx_intf_class*)((rfdevice_trx_common_intf_class *)cmn_dev_obj)->get_tech_instance( RFDEVICE_TRX_STD_GSM, RFDEVICE_TX);

          if (tx_dev_obj != NULL)
          {
            gsm_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][dev_type][local_instance] = tx_dev_obj;
