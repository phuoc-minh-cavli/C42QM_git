...
$DateTime: 2020/05/25 02:40:00 $ $Author: mplcsds1 $
$Header: //components/rel/rftech_gsm.mpss/2.10/rf/rfc/src/rfc_card_gsm.cpp#2 $
...
void rfc_gsm_fsk_tx_burst_config( rfm_device_enum_type rfm_dev,
                              rfcom_gsm_band_type band,
                              uint16 arfcn )
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  ...
  rfc_obj->fsk_tx_burst_config( rfm_dev,
                               band,
                               arfcn);
...
void rfc_gsm_fsk_tx_gain_config( rfm_device_enum_type rfm_dev, rfcom_gsm_band_type band,
                             rfgsm_modulation_type mod_type, uint8 slot_num, int32 pwr_in_dbm, 
                             uint8 pa_range, rf_buffer_intf *tx_trans_buffer, boolean bias_flag, 
                             uint16 bias , uint8 apt_mode, boolean override_start_time)
{
  ...
  rfc_obj->fsk_tx_gain_config(rfm_dev, band, mod_type, slot_num, 
                          pwr_in_dbm, pa_range, tx_trans_buffer, 
                          bias_flag, bias, apt_mode, override_start_time);
...
void rfc_gsm_fsk_tx_disable(rfm_device_enum_type rfm_dev)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  if ( rfc_obj == NULL )
  {
  ...
  rfc_obj->fsk_tx_disable(rfm_dev);

  return;
}
void rfc_gsm_fsk_tx_enable(rfm_device_enum_type rfm_dev)
{
  rfc_gsm *rfc_obj = rfc_gsm::get_instance();
  if ( rfc_obj == NULL )
  {
                                            ...
                                            uint32 instance)
{
  ...
  rfdevice_trx_gsm_rx_intf_class* rx_device = NULL;
  rfdevice_id_enum_type device_id = RFDEVICE_INVALID;

  rx_device = (rfdevice_trx_gsm_rx_intf_class*)rfc_gsm_get_phy_device_ptr(rfm_dev, rx_tx, band, dev_type, instance);
  if (rx_device == NULL)
  {
  ...
  device_id = rfdevice_id_get((void*)rx_device);
