...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/2.10/api/rfgsm_mdsp_cal.h#1 $ 
...
#define CAL_WTR_RFFE_BUF_MEM_SIZ (CAL_WTR_RFFE_BUF_ARR_SIZ * sizeof(rf_hal_bus_rffe_group_type))
/*===========================================================================
                 DEFINITIONS
=============================================================================*/
...
typedef struct
{
   ...
   uint16            num_wtr_rffe;
   rf_hal_bus_rffe_group_type       *wtr_rffe_scripts;
...
typedef struct
{
  ...
  */
  ...
  rf_hal_bus_rffe_group_type wtr_rffe_buf[CAL_WTR_RFFE_BUF_ARR_SIZ];
...
/*Trigger_burst_buffer+((max_da_steps+1)*num_pa_range*gmsk_8psk)*data_per_step+Tx_stop_buffer
  120                 +  ((52+1)*2*2)*52                                      +          20
*/
#define TRIGGER_BURST_DATA_PAYLOAD  120
#define MAX_DA_STEPS                 RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2
#define MAX_PA_RANGE_DA_CAL           2
#define MAX_MOD_TYPE_DA_CAL           2
#define DATA_PAYLOAD_PER_DA_STEP     52
#define TX_STOP_BUFFER_DATA_PAYLAOD  20
#define RFFE_GROUP_DATA_EXTRA_PAYLAOD 10 
#define RFFE_GROUP_DATA_PAYLAOD (TRIGGER_BURST_DATA_PAYLOAD\
	                             +((MAX_DA_STEPS+1)*MAX_PA_RANGE_DA_CAL*MAX_MOD_TYPE_DA_CAL*DATA_PAYLOAD_PER_DA_STEP)\
	                             +TX_STOP_BUFFER_DATA_PAYLAOD\
	                             +RFFE_GROUP_DATA_EXTRA_PAYLAOD)
	                             
typedef struct
{
uint8 data[RFFE_GROUP_DATA_PAYLAOD];
int current_data_index;
}wtr_rffe_group_data_s;

#define TRIGGER_BURST_TOKENS          2
#define TOKENS_PER_DA_STEP            2
#define TX_STOP_BUFFER_TOKENS         2
#define RFFE_GROUP_DATA_EXTRA_TOKENS 10 
#define RFFE_GROUP_TOKEN_PAYLOAD (TRIGGER_BURST_TOKENS\
	                             +((MAX_DA_STEPS+1)*MAX_PA_RANGE_DA_CAL*MAX_MOD_TYPE_DA_CAL*TOKENS_PER_DA_STEP)\
	                             +TX_STOP_BUFFER_TOKENS\
	                             +RFFE_GROUP_DATA_EXTRA_TOKENS)

/*TRIGGER_BURST_TOKENS+((max_da_steps+1)*num_pa_range*gmsk_8psk)*TOKENS_PER_DA_STEP+TX_STOP_BUFFER_TOKENS
  2                   +((52+1)*2*2)*2                                         +2
...
typedef struct
{
  ...
  rf_hal_bus_token_group_type tokens[RFFE_GROUP_TOKEN_PAYLOAD];
}wtr_rffe_token_data_s;
