...
Copyright (c) 2002 - 2021 by Qualcomm Technologies, Inc.  All Rights Reserved.
...
$Header: //components/rel/rftech_gsm.mpss/2.10/api/rfgsm_nv_cmn.h#2 $ 
$DateTime: 2021/09/14 06:54:23 $ 
$Author: mplcsds1 $
...
typedef enum
{
  ...
  RFGSM_DEBUG_NV_SAWLESS_DISABLE_HL_POWER_SCAN,
  RFGSM_DEBUG_NV_DISABLE_THERM_READ,
  RFGSM_DEBUG_NV_VBATT_COMP_USB_ENBALE,
...
typedef struct
{
  ...
  rfnv_item_id_enum_type rx_wbdc_cal_data;
...
}rfgsm_nv_ant_tuner_cl_ctrl_type;
...
typedef PACK (struct)
{
 rfgsm_nv_ant_tuner_type ant_tuner_rx;
 rfgsm_nv_ant_tuner_type ant_tuner_tx;
}gsm_ant_tuner_type;
...
typedef PACK (struct)
{
 uint16 rx_cal_chan;  /*Rx calibrated channel*/
 gsm_rx_wbdc_comp_data_type  rx_wbdc_comp_data[RFNV_GSM_NUM_LNA_STATE];	 /*WBDC comp data for all LNA states*/
} gsm_rx_cal_wbdc_data_nv_type;
...
{
  uint8  data_valid;
  int16 pmeas[RFNV_LINEAR_MAX_RGI_INDEX_PER_GAIN_STATE_V2];
...
typedef struct 
{
 ...
 /*!
  ...
  #else
    rfgsm_nv_ant_tuner_cl_ctrl_type ant_tuner_cl_data;  
  #endif	
...
typedef struct 
{
  ...
  */
  gsm_tx_cal_data_nv_type tx_cal_nv_data; 
    ...
    /*!
	@brief
	Holds the wbdc data of Rx cal
  */
  gsm_rx_cal_wbdc_data_nv_type rx_cal_wbdc_nv_data;
...
boolean get_enable_only_band_gsm(void);
rfgsm_band_type get_gsm_test_band(void);
