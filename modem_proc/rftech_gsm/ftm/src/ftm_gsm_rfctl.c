...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/2.10/ftm/src/ftm_gsm_rfctl.c#2 $
  $DateTime: 2021/04/02 04:25:14 $ 
  $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/02/21   aad     [CR2882743] KW Error fix for slot number index to array->ftm_slot_assign in API ftm_gsm_set_tx_slot_config
04/01/21   aad     [CR2882992] Removing unused API FTM_GSM_SET_PDM
03/15/21   aad     To correct ftm gain range to lna_range for CT 5 gain states
...
04/03/17   bp      Pass Access Burst TSC to GL1 in FTM mode
                            ...
                            ftm_lna_range_type range )
{
  ...
  */
  ...
  RFA_FTM_GSM_ISR_UNLOCK(&ftm_gsm_lock);
	MSG_1(MSG_SSID_FTM, MSG_LEGACY_ERROR,"GSM gain range: %d", gvars->ftm_gsm_state.ftm_lna_range);	
    ...
    else
    {
      if (gvars->tx_brst.data_source == (enum_type)FTM_TX_DATA_SOURCE_BUFFER)
      {
         ...
         * need to change if other task also need user defined random data.
         this is deprecated and ftm_kv_training data is not updated
         */
    ...
    for(slot_count = 0; slot_count < header_ptr->num_slot_payload; slot_count++)
    {
      ...
      if (payload_ptr->device_id == rfm_dev)
      {
        ...
        case TX_RGI_MEAS_OP:
          ...
          tx_slot_config.mod_type_is_gsm = payload_ptr->op_data.tx_rgi_meas_data.mod_type;
        ...
        case RX_DBM_MEAS_OP:
          ...
          gvars->ftm_gsm_state.ftm_lna_range = gvars->ftm_gain_range.range; //ftm lna are from 0-4 & gain range 0-4 so one to one mapping
...
void ftm_gsm_rx_burst_update_rssi(rfm_device_enum_type rfm_dev)
{
  ...
  if ( gvars->burst_metrics.ftm_rssi_counter % gvars->ftm_gsm_rssi_no_frames_avg )
  {
    ...
    if(gvars->wbdc_cal == TRUE)
	{
	  gvars->burst_metrics.wbdcEstI_PRx+=gvars->gsm_rx_burst_metrics.wbdcEstI_PRx;
	  gvars->burst_metrics.wbdcEstQ_PRx+=gvars->gsm_rx_burst_metrics.wbdcEstQ_PRx;
    }
  ...
  else
  {
    ...
    */
	...
	if(gvars->wbdc_cal == TRUE)
	{
	  ...
	  gvars->burst_metrics.wbdcEstI_PRx=gvars->burst_metrics.wbdcEstI_PRx /gvars->ftm_gsm_rssi_no_frames_avg;
	  gvars->burst_metrics.wbdcEstQ_PRx=gvars->burst_metrics.wbdcEstQ_PRx /gvars->ftm_gsm_rssi_no_frames_avg;
	}
    ...
    if(gvars->wbdc_cal == TRUE)
	{
	gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_i = gvars->burst_metrics.wbdcEstI_PRx;
	gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_q = gvars->burst_metrics.wbdcEstQ_PRx;
	}
    ...
    gvars->gsm_rx_burst_metrics.wbdcEstI_PRx =0;
    gvars->gsm_rx_burst_metrics.wbdcEstQ_PRx =0;
...
void ftm_gsm_do_rx_cmd_rx_burst_metrics( rfm_device_enum_type rfm_dev )
{
  ...
  */
  ...
  if(  gvars->ftm_gsm_rfctl_reset_rssi )
  {
     ...
     gvars->burst_metrics.ftm_rssi = 0;
	 gvars->burst_metrics.wbdcEstI_PRx=0;
	 gvars->burst_metrics.wbdcEstQ_PRx =0;
     gvars->burst_metrics.ftm_rssi_counter = 1;
  ...
  */
    ...
    if (gvars->multislot_rx_on )
    {
          ...
          (gvars->num_rx_slots < GERAN_API_MAX_GL1_DEFS_MAX_DL_MSGS) )
      {
        ...
        for (i = 0 ; i < gvars->num_rx_slots; i++)
        {
		  ...
		  if(gvars->wbdc_cal == TRUE)
		  {
		    temp_wbdc_i+=gvars->gsm_rx_multislot_metrics[i].wbdcEstI_PRx;
		    temp_wbdc_q+=gvars->gsm_rx_multislot_metrics[i].wbdcEstQ_PRx;
		  		 
		    gvars->gsm_rx_multislot_metrics[i].wbdcEstI_PRx =0;
		    gvars->gsm_rx_multislot_metrics[i].wbdcEstQ_PRx =0;	  
		  }
		...
		if(gvars->wbdc_cal == TRUE)
		{
		  gvars->gsm_rx_burst_metrics.wbdcEstI_PRx= temp_wbdc_i/gvars->num_rx_slots;
		  gvars->gsm_rx_burst_metrics.wbdcEstQ_PRx= temp_wbdc_q/gvars->num_rx_slots;
  ...
  if (!gvars->ftm_gsm_rssi_no_frames_avg)
  {
    ...
    gvars->burst_metrics.ftm_i_dc_offset_drx = 0;
    gvars->burst_metrics.ftm_q_dc_offset_drx = 0;
    gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_i=0;
    gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_q=0;
    return;
  }
    ...
    if(multi_slot_rx_flag)
    {
      ...
      if ( parm_ptr->gain_range_bitmask & (0x1<<i) )
      {
		...
		if(gvars->wbdc_cal == TRUE)
		{
		  if(i==0)
		  {
		    ftm_gsm_wbdc_conversion(gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_i,gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_q,
		                             &dc_val_i , &dc_val_q , TRUE);
		  }
		  else
		  {
		     ftm_gsm_wbdc_conversion(gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_i,gvars->ftm_gsm_state.ftm_wbdc_offset.ftm_wbdc_offset_q,
		                             &dc_val_i , &dc_val_q , FALSE);
		  }		
  
  
		parm_ptr->wbdcEstI_PRx[i]=dc_val_i;
		parm_ptr->wbdcEstQ_PRx[i]=dc_val_q;  

		MSG_2(MSG_SSID_RF,MSG_LEGACY_HIGH," multislot i dc offset is :%d for gain range %d!", parm_ptr->wbdcEstI_PRx[i],i );
        MSG_2(MSG_SSID_RF,MSG_LEGACY_HIGH,"multislot q dc offset is :%d for gain range %d!", parm_ptr->wbdcEstQ_PRx[i],i);
...
FUNCTION   ftm_gsm_do_wbdc_calibration_prx.
...
===========================================================================*/
void  ftm_gsm_do_rx_wbdc_calibration_prx( rfm_device_enum_type rfm_dev,
                                     ftm_gsm_rx_wbdc_cal_parms_type * parm_ptr )
{
  /* Use a temporary prx_drx parameters structure to call ftm_gsm_do_rx_calibration*/
  ftm_gsm_rxcal_parms_prx_drx_type params;
  ...
  /* Copy the results back to the original parameters ptr discarding the rxd results */
  memscpy( parm_ptr, sizeof(ftm_gsm_rx_wbdc_cal_parms_type), 
           &params, sizeof(ftm_gsm_rx_wbdc_cal_parms_type) );
}
...
FUNCTION   ftm_gsm_wbdc_conversion.

DESCRIPTION
   .Raw wbdc values will be passed
...
===========================================================================*/
  ...
  void ftm_gsm_wbdc_conversion(int32 wbdc_val_i,int32 wbdc_val_q,int32* dc_val_i ,int32* dc_val_q , boolean doubler_on)
  {
	  ...
	  /*Input voltage is the DC in the signal */
	...
	/*computing dc I value */
  
	 val_i = (wbdc_val_i * ref_factor*16)>>FTM_MSB_SHIFT;
		  *dc_val_i = val_i;
  
   /*computing dc Q value */
  
	 val_q = (wbdc_val_q * ref_factor*16)>>FTM_MSB_SHIFT; 
		  *dc_val_q = val_q;
  
	  MSG_1(MSG_SSID_FTM,MSG_LEGACY_HIGH,"ftm_gsm_wbdc_conversion: doubler on : %d ", doubler_on);  
	  MSG_2(MSG_SSID_FTM,MSG_LEGACY_HIGH,"ftm_gsm_wbdc_conversion: wbdc val i : %d ,wbdc val q  : %d ", wbdc_val_i,wbdc_val_q);	  
	  MSG_2(MSG_SSID_FTM,MSG_LEGACY_HIGH,"ftm_gsm_wbdc_conversion: wbdc val i in mv : %d  ,wbdc val q in mv : %d ", val_i,val_q);
                             ...
                             const ftm_gsm_tx_slot_config_type * tx_slot_config)
{
  ...
  gsm_linear_tx_gain_param_type tx_gain_param;
   ...
   if (tx_slot_config->slot_num >= FTM_MAX_GSM_SLOT_PER_FRAME)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Tx Slot is more than Maximum limit, Slot Number is %d ", tx_slot_config->slot_num);
    return;
  }
                                         ...
                                         int16* parameters )
{
  ...
  {
    (void)rfgsm_core_tune_rx( &rfgsm_core_data, gvars->ftm_gsm_state.ftm_channel,RFGSM_RX_DEFAULT_RX_BURST );
  }
  RFA_FTM_GSM_ISR_UNLOCK(&ftm_gsm_lock);

  rfgsm_core_set_radio_gain( rfm_dev, power_dbm16, &gvars->ftm_gain_range, enh_rx_params ); //gain range returned here is indexed 1-5
  ftm_gsm_set_lna_range( rfm_dev, gvars->ftm_gain_range.range ); //ftm lna are from 0-4 & gain range 0-4 so one to one mapping
                                     ...
                                     uint16 smps_bias )
{
  ...
  rfa_rf_gsm_ftm_set_linear_rgi_cmd.linear_rgi_params.mod_type_is_gsm   = mod_type;
                                         ...
                                         ftm_read_device_status* device_status)
{
  ...
  if (burst_index < RFGSM_DEVICE_STATUS_MAX_BURST_TYPE )
  {
    ...
    //HALF_RATE shouldbe 1 for SDR105, so hard coded to 1
    rfgsm_core_handle_ptr->device_config_to_read.params[burst_index].half_rate[reg_index] = 1;
