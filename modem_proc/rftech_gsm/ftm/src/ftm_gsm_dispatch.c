...
Copyright (c) 1994-2021 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
  ...
  $Header: //components/rel/rftech_gsm.mpss/2.10/ftm/src/ftm_gsm_dispatch.c#2 $ //source/qcom/qct/modem/rftech/gsm/main/1.0/ftm/src/ftm_gsm_dispatch.c#9
  $DateTime: 2021/04/02 04:25:14 $
  $Author: mplcsds1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/01/21   aad     [CR2882992] Removing unused case condition FTM_SET_PDM
                                   ...
                                   ftm_pkt_type  *cmd_ptr )
{
  ...
  case FTM_RX_GAIN_RANGE_CAL:
    ...
    break;
	    /* 46 */
  case FTM_RX_GAIN_RANGE_CAL_V2:
    vars_ptr->ftm_gsm_cal_flag = TRUE;
	vars_ptr->wbdc_cal = TRUE;	
    ftm_gsm_do_rx_wbdc_calibration_prx( rfm_dev,
                               &cmd_ptr->rf_params.ftm_rf_factory_data.gsm_struct.ftm_gsm_data_union.ftm_wbdccal_parms);
    vars_ptr->wbdc_cal = FALSE;
    vars_ptr->ftm_gsm_cal_flag = FALSE;
...
//for SDR105 it is compulsory to read reg twice backto back
