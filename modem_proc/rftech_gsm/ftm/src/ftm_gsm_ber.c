...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_gsm.mpss/2.10/ftm/src/ftm_gsm_ber.c#1 $ 

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/13/19   mpa     Changes to unload GERAN in endcall
07/04/19   mpa     To Load geran for Non-signalling
06/21/18   mpa     To remove WCDMA and CDMA1X dependencies
11/24/16   bp      Disable NAS functions for JO.3.0.2 variant
...
void ftm_gsm_ber_handle_rr_msgs( void *msg)
{
  ...
  **************************************************************************/
  ...
  case FTM_STOP_GSM_MODE_CNF:
	...
	if(!ftm_common_dmm_module_safe_unload(DM_ID_GERAN))
		{
		  MSG(MSG_SSID_FTM, MSG_LEGACY_HIGH, "FTM GSM ftm_common_dmm_module_safe_unload failed");
		}

    MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH, "FTM GSM active device is now invalid",0);

    geran_dmm_set_inactive();
                                       ...
                                       ftm_gsm_ber_pkt_type *cmd_ptr)
{
...
#ifdef FEATURE_RF_EXTPL_ENABLE
  dm_state_e_type gsm_dm_state=DM_STATE_INACTIVE;
  gsm_dm_state = dmm_get_module_state(DM_ID_GERAN);
 
  if(gsm_dm_state != DM_STATE_ACTIVE)
	{
	  if(ftm_common_dmm_module_safe_load(DM_ID_GERAN) != TRUE)
	  {
	    FTM_MSG_1(FTM_HIGH, "Failed to load GSM dmm module safely", 0);
	  }
	  else
	  {
         geran_dmm_set_loaded();
		
	     FTM_MSG_2(FTM_HIGH, "DMM module %d is opened. dmm state: %d geran_state loaded", DM_ID_GERAN, dmm_get_module_state(DM_ID_GERAN));
...
#endif
  {
    ...
    MSG_1(MSG_SSID_FTM, MSG_LEGACY_HIGH,"FTM GSM BER Dispatch: Received Command: %d", 
