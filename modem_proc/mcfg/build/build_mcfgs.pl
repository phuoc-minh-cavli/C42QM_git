#******************************************************************************
#* $Header: //components/rel/mcfg.mpss/10.1.1/build/build_mcfgs.pl#1 $
     ...
     else
     {
       ...
       if (!$SOURCES_ONLY) {
          if (!mbn_exists($cfg) or $FORCE_REBUILD) {
            ...
            my ($length_bytes, $num_items) = MCFGWrite::GenerateMCFGTargetFrmSourceXML($xml, $input_file_name, $cfg->{pathname}, $BUILD_ROOT, $CRM_BUILDID, $BUILD_FLAVOR);
            print "done\n";
          } else {
             print "The MBN for $cfg->{typename}:$cfg->{name} already exists.\n";
          }
...
sub parse_commandline_parameters {
   ...
   if ($print_usage) {
      ...
      print "  --flavor=<build_flavor>            The build flavor being compiled. Only used and passed by the build system for temp files. MBNs themselves are flavor independent.\n";
...
sub prepare_cfg {
   ...
   if (length($BUILD_FLAVOR)>0)
   {
     $cfg->{pathname} = "$MCFG_DIR/bsp/$cfg->{typename}_img/build/$cfg->{typename}/qdsp6/$BUILD_FLAVOR/configs/$cfg->{typename}/$CONFIG_VARIANT/$cfg->{variant_subpath}/$cfg->{typename}.mbn";
   }
   else
   {
...
sub build_default_source($) {
   ...
   my $PREGEN_FLAG = 1; # generate Default pre-generated.c file with highlight.
   ...
   MCFGWrite::GenerateMcfgSegmentFileXML($xml, $xmlFileLocation, $def_output_file_name, "$BUILD_ROOT/", $MCFGWrite::DEFAULT_C_OUTPUT_FILE_TYPE, 0, undef, $CRM_BUILDID, $PREGEN_FLAG);
