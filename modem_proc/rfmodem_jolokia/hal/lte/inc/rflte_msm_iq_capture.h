...
$Header: //components/rel/rfmodem_jolokia.mpss/1.10/hal/lte/inc/rflte_msm_iq_capture.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/05/19   sk      RF PAGE ONLY changes for NB/CATM
11/07/18   gk      NB-IoT Calibration support changes
05/10/17   qzh     Update NB1 IQ capture sample rate
...
/* Flag to track the FW response for the request .Before request , RFSW will set this flag to flase . After response from FW , the flag will be set to TRUE. */
   ...
   uint32* nb_buffer_start_addr;  /*Buffer start address passed to FW to hold 4800 uint32 IQ data for NB1. 8 byte aligned per FW requirement*/
...
/*! NBIOT sampling frequency = 480khz */
#define RFLTE_NB_IQ_SAMPLE_FREQ (4.8e5)

// Maximum buffer size passed to FW
#define RFLTE_NB_IQ_FW_BUFFER_SIZE 9000
...
void rflte_nb_msm_iq_capture_config(void);

  #ifdef FEATURE_NB1
    void rflte_nb_msm_iq_capture_process_samples(nb1fw_sys_rx_sample_capture_cnf_msg_s* rf_lte_msg_ptr);
  #endif
