...
/*==============================================================================
  ...
  $Header: //components/rel/rfmodem_jolokia.mpss/1.10/api/ccs/rf_hal_ccs.h#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
10/05/18   mns     device snapshot changes.
07/12/18   mns     RFFE GRP changes.
10/10/18   sbo     Support for dump of user requested registers during error handling
...
/*----------------------------------------------------------------------------*/
boolean 
event_builder_translate_payload_rffe_grp
( 
  rf_ccs_event_task_description_type* task_block_ptr,
  boolean is_issue_seq
);
...
/*----------------------------------------------------------------------------*/
boolean rf_hal_rffe_ext_read_bytes_offline
(
    uint8 channel, 
	uint8 slave_id, 
	boolean rd_delay, 
...
boolean rf_hal_rffe_read_byte_offline
(
    uint8 channel, 
	uint8 slave_id, 
	boolean rd_delay, 
