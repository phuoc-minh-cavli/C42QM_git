...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_nv/src/mcfg_fs.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    //MCFG_MSG_SPRINTF_2("mcfg_fs_write -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    status = mcfg_fs_build_path(MCFG_FS_TYPE_EFS, sub_id, path, f_name, MCFG_FS_MAX_PATH_LENGTH);
    f_name[MCFG_FS_MAX_PATH_LENGTH-1] = '\0';
    //MCFG_MSG_SPRINTF_2("mcfg_fs_read -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (status == MCFG_FS_STATUS_OK)
  {
    //MCFG_MSG_SPRINTF_2("mcfg_fs_delete unlink -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (status == MCFG_FS_STATUS_OK)
  {
    //MMCFG_MSG_SPRINTF_2("mcfg_fopen -  sub_id: 0x%0x, filepath %s", sub_id, f_name);
...
/*==========================================================================
  ...
  PARAMETERS
    ...
    fs_type   [in] file system type
      RETURN VALUE
    returns the new offset, per the call to efs_lseek(),
    -1 on error
