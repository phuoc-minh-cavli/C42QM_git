...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_nv/src/mcfg_nv.c#1 $
...
boolean cached_gcf_value = FALSE;
#endif
...
static const char mcfg_nv_filenames[MCFG_NV_NUM_NEW_ITEMS][MCFG_NV_FILENAME_MAX_LEN] =
{
  ...
  "mcfg_sw_config_digest_version",  /* MCFG_NV_ITEM_MCFG_SW_CONFIG_DIGEST_VERSION*/
  "mcfg_hw_config_digest_version",  /* MCFG_NV_ITEM_MCFG_HW_CONFIG_DIGEST_VERSION*/
  "mcfg_default_sw_digest_version",  /* MCFG_NV_ITEM_MCFG_DEFAULT_SW_CONFIG_DIGEST_VERSION*/
  "mcfg_default_hw_digest_version",  /* MCFG_NV_ITEM_MCFG_DEFAULT_HW_CONFIG_DIGEST_VERSION*/
  "mcfg_multi_mbn_digest_version",  /* MCFG_NV_ITEM_MCFG_MULTI_MBN_DIGEST_VERSION*/
  "mcfg_rfs_sw_digest_version",  /* MCFG_NV_ITEM_MCFG_RFS_SW_DIGEST_VERSION*/
  "mcfg_rfs_hw_digest_version",  /* MCFG_NV_ITEM_MCFG_RFS_HW_DIGEST_VERSION*/
  "mcfg_last_autoselect_plmn_impi"  /* MCFG_NV_ITEM_LAST_AUTOSELECT_PLMN_IMPI*/
...
static const mcfg_nv_indexed_item_info_s_type mcfg_nv_indexed_items[] =
{
  ...
  { MCFG_NV_ITEM_LAST_AUTOSELECT_PLMN,   1 },
};
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (item > MCFG_NVI_ITEM_FILES_START && item < MCFG_NVI_ITEM_FILES_END)
  {
    ...
    if (new_indexed != NULL)
    {
      if((item == MCFG_NV_ITEM_MCFG_SW_MUXD_VERSION_I) ||
         (item == MCFG_NV_ITEM_LAST_AUTOSELECT_PLMN))
      {
        *new_indexed = FALSE;
      }
      else
  ...
  //MCFG_MSG_SPRINTF_1("build path %s", path);
  return status;
} /* mcfg_nv_build_path() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    if (status == MCFG_NV_STATUS_OK)
    {
      //MCFG_MSG_SPRINTF_1("mcfg_nv_read path %s", path);
      ...
      if (efs_ret == -1)
      {
        //MCFG_MSG_ERROR_1("mcfg_nv_read efs_ret %s", efs_ret);
        switch (efs_errno)
        {
          case ENOENT:
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  MCFG_MSG_LOW_1("mcfg_nv_write status %d", status);  
  return status;
} /* mcfg_nv_write() */
...
mcfg_nv_status_e_type mcfg_nv_efs_update_symlink(char* org_path, uint32 item_size)  
{
  ...
  //MCFG_MSG_SPRINTF_1("mcfg_nv_efs_update_symlink org_path %s", org_path);
  ...
  if( (efs_stat(abs_path, &temp_buf) == -1) && (efs_errno == ENOENT) )
  {
    ...
    if( temp_data_ptr ) 
    {
      ...
      MCFG_MSG_SPRINTF_3("mcfg_nv_efs_update_symlink- Creating dummy abs_path: %s of size=%d efs_ret=%d", 
  ...
  else
  {
    ...
    {
      /* remove old link */
      MCFG_MSG_SPRINTF_1("mcfg_nv_efs_update_symlink- remove old link %s", org_path);
      if ( efs_unlink(org_path) == -1 )
      {
        MCFG_MSG_ERROR_1("mcfg_nv_efs_update_symlink - error in efs_unlink,EFS error = %d", efs_errno);
      }
      else
      {
        /* update link */
        MCFG_MSG_SPRINTF_2("mcfg_nv_efs_update_symlink- update link %s after removing old link to point to %s", org_path, abs_path);
        efs_ret = efs_symlink (abs_path, org_path);
      }
    } 
    else if ( S_ISREG(fmode) || S_ISITM(fmode) )
    {
      /* If it is a regular file or item file */
      /* Rename the file to the abspath, and convert the org_path to be link */
      MCFG_MSG_SPRINTF_1("mcfg_nv_efs_update_symlink- reg file or item file, rename %s", org_path);
      MCFG_MSG_SPRINTF_1("mcfg_nv_efs_update_symlink- reg file or item file, to %s", abs_path);
      if( efs_rename(org_path, abs_path) == -1 )
      {
        MCFG_MSG_ERROR_1("mcfg_nv_efs_update_symlink, efs_rename error = %d", efs_errno);
      }
      else
      {
        /* Create link a link to this path */
        MCFG_MSG_SPRINTF_2("mcfg_nv_efs_update_symlink- now create link %s to point to %s", org_path, abs_path);
        efs_ret = efs_symlink (abs_path, org_path);
      }
    }
    else
    {
      MCFG_MSG_HIGH_1("mcfg_nv_efs_update_symlink mode %x", fmode);
  }
  }
...
===========================================================================*/
boolean mcfg_nv_efslogging_enabled(void)
{
  struct fs_stat stat;
   ...
   uint8 subs_id) 
{
  ...
  if (status == MCFG_NV_STATUS_OK)
  {
    ...
    {
      /* File does not exist */
      //MCFG_MSG_SPRINTF_1("mcfg_nv_rm_symlink- file %s does not exist, skipping", symlink_path);
    }
    else
    {
      /* Find the type of file */
      ...
      {
        /* remove the link */
        MCFG_MSG_SPRINTF_1("mcfg_nv_rm_symlink- remove link %s", symlink_path);
      ...
      {
       // MCFG_MSG_SPRINTF_1("mcfg_nv_rm_symlink- file %s is not a link, skipping", symlink_path);
      }
...
";
  ...
  {
      // Create missing directories before the file creation
      if(efs_stat("/nv/item_files/conf", &stat_buf) == -1)
      {
         if(efs_stat("/nv/item_files", &stat_buf) == -1)
         {
            if(efs_stat("/nv", &stat_buf) == -1)
            {
               efs_mkdir("/nv", 0755);
            }
            efs_mkdir("/nv/item_files", 0755);
         }
         efs_mkdir("/nv/item_files/conf", 0755);
      }
...
/*==========================================================================
  ...
  FUNCTION mcfg_gcf_nv_read

  DESCRIPTION
    This function reads an GCF NV 947 (NV_GPRS_ANITE_GCF_I) 

  DEPENDENCIES

  PARAMETERS
    gcf_nv_value [out] Pointer to put the gcf value
    
  RETURN VALUE
    MCFG_NV_STATUS_OK on success
    MCFG_NV_STATUS_NOT_ACTIVE if item doesn't exist
    MCFG_NV_STATUS_EFS_ERR on NV read failure
    MCFG_NV_STATUS_NULL_PTR if gcf_nv_ptr is null

  SIDE EFFECTS
    none

===========================================================================*/
mcfg_nv_status_e_type mcfg_gcf_nv_read(
  boolean        *gcf_nv_ptr
)
{
  if(gcf_nv_ptr == NULL)
  {
    return MCFG_NV_NULL_PTR;
  }

  return mcfg_nv_read(NV_GPRS_ANITE_GCF_I, 0, 0, (void *)gcf_nv_ptr, sizeof(boolean));

} /* mcfg_gcf_nv_read */

/*==========================================================================

  FUNCTION mcfg_gcf_nv_get_status

  DESCRIPTION
    This function returns the status for GCF NV 947 read
  ...
  RETURN VALUE
    TRUE on successful read and value read was TRUE
    FALSE on read failure or null or value read was FALSE

  SIDE EFFECTS
    none

===========================================================================*/
boolean mcfg_gcf_nv_get_status(void)
{
  return cached_gcf_value;
} /* mcfg_gcf_nv_get_status */

/*==========================================================================

  FUNCTION mcfg_nv_update_cache

  DESCRIPTION
    This function re-reads and updates the status for cached gcf value
...
===========================================================================*/
void mcfg_nv_update_cache(void)
{
  mcfg_gcf_nv_read(&cached_gcf_value);
