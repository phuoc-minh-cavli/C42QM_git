...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_sel/src/mcfg_trl.c#1 $
...
#define MCFG_TRL_TLV_SIZE_MCFG_DIGEST     (32)
...
static boolean mcfg_trl_decode_mcfg_digest
(
  const mcfg_config_info_s_type *config_info,
  uint32                         offset,
  void                          *decode_buffer
...
static const mcfg_trl_tlv_info_s_type mcfg_trl_tlv_info[] =
{
  ...
  /* MCFG_TRL_TLV_ID_MCFG_DIGEST_VERSION */
  {
    MCFG_TRL_TLV_INFO_FLAG_SIZE_FIXED,
    MCFG_TRL_TLV_SIZE_MCFG_DIGEST,
    sizeof(mcfg_trl_mcfg_digest_s_type),
    mcfg_trl_decode_mcfg_digest
  },
...
/*===========================================================================
  ...
  FUNCTION mcfg_trl_decode_mcfg_digest

  DESCRIPTION
    Decodes the mcfg digest TLV data.
...
)
{
  const uint8 *packed_data;
  const uint32 *packed_data_32;
  mcfg_trl_mcfg_digest_s_type *mcfg_digest;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  MCFG_CHECK_NULL_PTR_RET_FALSE(config_info);
  MCFG_CHECK_NULL_PTR_RET_FALSE(decode_buffer);

  memscpy(decode_buffer, sizeof(mcfg_trl_mcfg_digest_s_type),
          (const uint8 *)(&config_info->description[offset] + sizeof(mcfg_trl_tlv_header_s_type)),
          sizeof(mcfg_trl_mcfg_digest_s_type));
