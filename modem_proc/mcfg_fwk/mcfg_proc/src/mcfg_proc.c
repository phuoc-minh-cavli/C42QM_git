...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_proc/src/mcfg_proc.c#1 $ $DateTime: 2020/02/19 23:07:19 $ $Author: pwbldsvc $ 
...
static boolean mcfg_update_required(uint8 cfg_type, uint8* ram_cust_data_ptr, uint32* ram_mcfg_version_ptr, uint8 subs_id, mcfg_config_id_s_type* current_id_ptr );
static boolean mcfg_extra_cleanup(uint8 cfg_type, uint8 subs_id );
static boolean mcfg_update_required_based_on_digest(uint8 cfg_type, mcfg_config_id_s_type* current_id, uint8 subs_id);
...
#endif /* FEATURE_MODEM_CFG_HISTORY_LOGGING */
static char saved_sw_mcfg_digest[MCFG_DIGEST_SIZE];
static char saved_hw_mcfg_digest[MCFG_DIGEST_SIZE];
static boolean mcfg_digest_available= FALSE;
...
/* -------------------------------------------------------------------- */
  ...
  memset(&mcfg_current_buf, 0, sizeof(mcfg_current_buf));
  ...
  do
  {
    ...
    if (mcfg_utils_space_available(current_mcfg_type, &mcfg_current_buf, TRUE) != MCFG_ERR_NONE)
    {
      MCFG_MSG_ERROR_2("No enough EFS space for cfg_type %d, subs_id %d",current_mcfg_type, mcfg_subs_num);
      modem_cfg_process_status = MODEM_CFG_PROC_LOAD_FAIL; 
      break;
    }
    ...
    /* To check if config has digest available or not */
    mcfg_digest_available = mcfg_util_digest_version_compatibility(ram_cust_data_ptr);
    MCFG_MSG_HIGH_1("mcfg_digest_available  = %d", mcfg_digest_available);
    ...
    if( mcfg_update_required(current_mcfg_type, ram_cust_data_ptr, &ram_mcfg_version, mcfg_subs_num, &mcfg_current_id))
    {
      ...
      if (refresh_type != MCFG_REFRESH_TYPE_SLOT)
      {
        if (mcfg_digest_available)
        {
           if(current_mcfg_type == MCFG_TYPE_SW)
           {
              mcfg_nv_write_status = mcfg_nv_write( MCFG_NV_ITEM_MCFG_SW_CONFIG_DIGEST_VERSION, 0, mcfg_subs_num,  
                                                 saved_sw_mcfg_digest, MCFG_DIGEST_SIZE);
           }
           else if (current_mcfg_type == MCFG_TYPE_HW)
           {
              mcfg_nv_write_status = mcfg_nv_write( MCFG_NV_ITEM_MCFG_HW_CONFIG_DIGEST_VERSION, 0,mcfg_subs_num, 
                                             saved_hw_mcfg_digest, MCFG_DIGEST_SIZE);
           }
        }
        if(current_mcfg_type == MCFG_TYPE_SW)
        {
  ...
  for(current_mcfg_type=MCFG_TYPE_MIN; current_mcfg_type <= MCFG_TYPE_MAX; current_mcfg_type++ )
  {
    ...
    for(mcfg_subs_num = MCFG_SUB_ID_FIRST; mcfg_subs_num < mcfg_max_subs; mcfg_subs_num++)
    {
      ...
      if( mcfg_update_required(current_mcfg_type, ram_cust_data_ptr, &ram_mcfg_version, mcfg_subs_num, &mcfg_current_id))
      {
        ...
        /* Update the correct MCFG Digest to NV to wrap up */
        if (mcfg_digest_available)
        {
           if(current_mcfg_type == MCFG_TYPE_SW)
           {
                            ...
                            mcfg_refresh_type_e_type refresh_type)
{
  ...
  if( ( mcfg_hdr->magic1 != MCFG_MAGIC ) || 
      ((mcfg_hdr->mcfg_format_ver_num != MCFG_FORMAT_NUM) && 
       (mcfg_hdr->mcfg_format_ver_num != MCFG_FORMAT_NUM_LEGACY)) ||
...
static boolean mcfg_update_required(uint8 cfg_type, uint8* ram_cust_data_ptr, uint32* ram_mcfg_version_ptr, uint8 subs_id, mcfg_config_id_s_type* current_id_ptr )
{
  ...
  if (mcfg_digest_available) 
  {
    return mcfg_update_required_based_on_digest(cfg_type, current_id_ptr, subs_id);
  }
...
/*===========================================================================
FUNCTION  mcfg_update_required_based_on_digest() 

DESCRIPTION
This function is used to compare current and previous digest version to determine 
if the update is required or not. 
...
static boolean mcfg_update_required_based_on_digest(uint8 cfg_type, mcfg_config_id_s_type* current_id, uint8 subs_id)
{
  MCFG_MSG_HIGH_1("inside mcfg_update_required_based_on_digest ..cfg_type is...(%d)", cfg_type);
  boolean   update_required=TRUE;
  char prev_mcfg_digest_version[MCFG_DIGEST_SIZE];
  mcfg_config_info_s_type info;
  mcfg_trl_mcfg_digest_s_type curr_mcfg_digest_version;
  memset((void *)prev_mcfg_digest_version, 0, MCFG_DIGEST_SIZE);
  mcfg_config_s_type config;

  if(current_id == NULL)
  {
    MCFG_MSG_ERROR("current_id is null ! ");
    return FALSE;
  }

  if (cfg_type == MCFG_TYPE_SW)
  {
     if (MCFG_NV_STATUS_OK == mcfg_nv_read(MCFG_NV_ITEM_MCFG_SW_CONFIG_DIGEST_VERSION,
                                          0, subs_id, prev_mcfg_digest_version, MCFG_DIGEST_SIZE))
     {
        MCFG_MSG_HIGH_1("prev_mcfg_digest_version read success  for cfg_type : (%d)!", cfg_type);  
     }
  }else if(cfg_type == MCFG_TYPE_HW)
  {
     if (MCFG_NV_STATUS_OK == mcfg_nv_read(MCFG_NV_ITEM_MCFG_HW_CONFIG_DIGEST_VERSION,
                                          0, subs_id, prev_mcfg_digest_version, MCFG_DIGEST_SIZE))
     {
        MCFG_MSG_HIGH_1("prev_mcfg_digest_version read success  for cfg_type : (%d)!", cfg_type);  
     }
  ...
  if (mcfg_utils_get_config_info(cfg_type, current_id, &info))
  {
    if (!mcfg_trl_decode_tlv(&info, MCFG_TRL_TLV_ID_MCFG_DIGEST_VERSION, 
             &curr_mcfg_digest_version, sizeof(mcfg_trl_mcfg_digest_s_type)))
    {
      MCFG_MSG_ERROR_1("Fail to decode mcfg digest version for (%d)!", cfg_type);
      update_required=FALSE;
    }
    else
    {
     if( memcmp(prev_mcfg_digest_version, curr_mcfg_digest_version.mcfg_digest, MCFG_DIGEST_SIZE) != 0 )
     {
       MCFG_MSG_MED_1("prev and curr mcfg digest version is not same for cfg_type : (%d)!", cfg_type);
       update_required = TRUE;
       if(cfg_type == MCFG_TYPE_SW)
       {
         memscpy (saved_sw_mcfg_digest, MCFG_DIGEST_SIZE, curr_mcfg_digest_version.mcfg_digest, MCFG_DIGEST_SIZE);
       }
       else if(cfg_type == MCFG_TYPE_HW)
       { 
         memscpy (saved_hw_mcfg_digest, MCFG_DIGEST_SIZE, curr_mcfg_digest_version.mcfg_digest, MCFG_DIGEST_SIZE);
       }  
     }
    else
    {
      MCFG_MSG_HIGH_1("prev and curr mcfg digest version is same for cfg_type : (%d)!", cfg_type);
      update_required = FALSE;
    }
   }
  }
  ...
  {
    MCFG_MSG_ERROR_1("Failed to get the config info for cfg_type : (%d)!", cfg_type);
    update_required = FALSE;
  }
  
  return(update_required);

}/*mcfg_update_required_based_on_digest()*/
                          ...
                          mcfg_refresh_type_e_type refresh_type)
{
  ...
  boolean mcfg_nvefs_update_status = TRUE;
...
boolean mcfg_create_efs_file(uint8* efsData, uint8 attributes, boolean restore_fact)
{
  ...
  {
    MCFG_MSG_SPRINTF_1("No need to modify EFS File: %s, skipping...", filePath);
    return TRUE;
  }
...
/* No use cases for Indexed EFS Item for now */
  ...
  if(attributes & U_ITEM_ATTRIB_MULTISIM)
  {
    ...
    if(subs_mask > MCFG_SUBS_MASK)
    {
       ...
       MCFG_MSG_SPRINTF_3("MultiSIM: mcfg_create_file - MCFG_NV_STATUS_BAD_ITEM, subs_id: 0x%0x, subs_mask=%d!!, filepath %s", 
    ...
    if( !mcfg_proc_subs_mask(subs_mask, subs_id) )
    {
       MCFG_MSG_SPRINTF_3("MultiSIM: mcfg_create_file - ignoring write for file = %s, subs_mask=%d, curr_subs_id=%d", 
  ...
  if(attributes & U_ITEM_ATTRIB_MUXD)
  {
    ...
    {
      MCFG_MSG_SPRINTF_1("Could not update symlink for EFS File: %s", filePath);
      return FALSE;
    }
  ...
  // MCFG_MSG_SPRINTF_3("mcfg_create_efs_file, EFS File: %s, length=%d, subs_id=%d", 
  ...
  if(efs_result == -1)
  {
    MCFG_MSG_SPRINTF_1("Could not write EFS File: %s", f_name);
    return FALSE;
  }
...
mcfg_nv_status_e_type mcfg_delete_efs_file(uint8* efsData, uint8 attrib )
{
  ...
  if(attrib & U_ITEM_ATTRIB_MULTISIM)
  {
    ...
    if(subs_mask > MCFG_SUBS_MASK)
    {
       ...
       MCFG_MSG_SPRINTF_3("MultiSIM: mcfg_delete_efs_file - MCFG_NV_STATUS_BAD_ITEM, subs_id: 0x%0x, subs_mask=%d!!, filepath %s", 
    ...
    if( !mcfg_proc_subs_mask(subs_mask, subs_id) )
    {
       MCFG_MSG_SPRINTF_3("MultiSIM: mcfg_delete_efs_file - ignoring delete for file = %s, subs_mask=%d, curr_subs_id=%d", 
  ...
  if(delete_flag)
  {
    //MCFG_MSG_SPRINTF_1("Deleting NV File: %s", file_path);
...
mcfg_nv_status_e_type mcfg_delete_data_profile(uint8* efsData, uint8 attrib )
{
  ...
  if(delete_flag)
  {
    //MMCFG_MSG_SPRINTF_1("Deleting NV File: %s", file_path);
...
)
{
  ...
  for (i = 1; i<num_mcfg_items; i++) 
  {
    ...
    mcfg_item_type_int = mcfg_parse_item_prefix(
       cfg_type, ram_cust_data_ptr, &mcfg_item_prefix, refresh_type, subs_id_input);
...
/* -------------------------------------------------------------------- */
  ...
  do
  {
    ...
    /* Check if mbn version is digest compatible or not */
    mcfg_digest_available = mcfg_util_digest_version_compatibility(ram_cust_data_ptr);
    MCFG_MSG_HIGH_1("mcfg_digest_available inside modem_cfg_cleanup  = %d", mcfg_digest_available);
    ...
    if( mcfg_update_required(type, ram_cust_data_ptr, &ram_mcfg_version, sub, &id))
    {
      MCFG_MSG_MED_2("cfg_cleanup: Update is required, type = %0x!, sub = %d", type, sub);
      //break;
    }
    ...
    if (!mcfg_cleanup_nvefs(type, num_mcfg_items, ram_cust_data_ptr, sub, refresh_type))
    {
    ...
    if (mcfg_digest_available) 
    {
       ...
       MCFG_MSG_HIGH_1("====Debug:In modem_cfg_cleanup , mcfg_digest_available = %d ====  ", mcfg_digest_available);
       mcfg_nv_del_status = mcfg_nv_delete((type == MCFG_TYPE_SW)? MCFG_NV_ITEM_MCFG_SW_CONFIG_DIGEST_VERSION : MCFG_NV_ITEM_MCFG_HW_CONFIG_DIGEST_VERSION
                                               , 0, sub);
       MCFG_MSG_HIGH_3("mcfg_nv_del_status is : %d, for type %d, subs_id = %d", mcfg_nv_del_status, type, sub);
       if (MCFG_NV_STATUS_OK != mcfg_nv_del_status)
       {
          MCFG_MSG_ERROR_2("MCFG NV delete failed, item=%d, err=%d.",
                               (type == MCFG_TYPE_SW)? MCFG_NV_ITEM_MCFG_SW_CONFIG_DIGEST_VERSION : MCFG_NV_ITEM_MCFG_HW_CONFIG_DIGEST_VERSION
                              , mcfg_nv_del_status);
       }
    }
