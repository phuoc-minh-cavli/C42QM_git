...
/*===========================================================================
  ...
  $Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_proc/src/mcfg_task.c#2 $ $DateTime: 2020/11/16 21:23:19 $ $Author: pwbldsvc $
...
#include "baseband_config_proc.h"
/*==============================================================================
...
static dog_report_type modem_cfg_dog_rpt_var = 0;  
...
#ifdef FEATURE_MCFG_MULTI_MBN_SUPPORT
      case MCFG_TASK_CMD_MULTIMBN:
        mcfg_multi_mbn_proc(FALSE);
        break;
#endif

#ifdef FEATURE_MCFG_REMOTE_STORAGE
      case MCFG_TASK_CMD_DISCOVER:
        if (mcfg_remote_discover(MCFG_RFS_PATH_DISCOVER))
        {
           /* reselect if auto discover is successfully updated */
          for (uim_cmd_payload.sloti = MCFG_SLOT_INDEX_FIRST; 
               uim_cmd_payload.sloti < MCFG_NUM_OF_UIM_SLOT;
               uim_cmd_payload.sloti++)
          {
            if (mcfg_uim_nv_autoselect_enabled(uim_cmd_payload.sloti))
            {
...
)
{
...
#ifdef FEATURE_MCFG_MULTI_MBN_SUPPORT
  process_status = mcfg_multi_mbn_proc(TRUE);
  ...
  MCFG_MSG_HIGH("Mulit mbn processing at boot up -DONE");
#endif /*FEATURE_MCFG_MULTI_MBN_SUPPORT*/ 
...
#if defined(FEATURE_MCFG_REMOTE_STORAGE) && !defined(FEATURE_MCFG_MULTI_MBN_SUPPORT)
...
static void mcfg_task_timer_cb(timer_cb_data_type data)
{
  mcfg_task_cmd_s_type *cmd;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  cmd = (mcfg_task_cmd_s_type *)data;
  if (cmd != NULL)
  {
    if (cmd->timer)
    {
      timer_undef(cmd->timer);
      mcfg_free(cmd->timer);
...
)
{
  ...
  timer_type       *mcfg_task_timer;  

  cmd = mcfg_task_cmd_alloc(cmd_id, 0);
  if (mcfg_tcb == NULL)
  {
  ...
  {
    MCFG_MSG_ERROR(" malloc failed for mcfg_task_send_cmd_w_delay()");
  }
  ...
  {
    MCFG_MSG_HIGH_1("cmd_id %d queued", cmd->cmd_id);
    q_put(&mcfg_task_cmd_q, &cmd->link);
    modem_cfg_os_set_sigs(mcfg_tcb, MODEM_CFG_CMD_Q_SIG);
  ...
  {
    MCFG_MSG_HIGH_2("task cmd_id %d queued with delay of %d ms", cmd->cmd_id, delay_in_ms);
    mcfg_task_timer = (timer_type *)mcfg_malloc(sizeof(timer_type));
    if (!mcfg_task_timer)
    {
      MCFG_MSG_ERROR(" malloc failed for mcfg_task_send_cmd_w_delay()");
    }
    ...
    {
      cmd->timer = mcfg_task_timer;

      /* define timer for timeout */
      timer_def(mcfg_task_timer,
                NULL,
                NULL,
                0,
                mcfg_task_timer_cb,
                (timer_cb_data_type)cmd);

      timer_set(mcfg_task_timer, delay_in_ms, 0, T_MSEC);
