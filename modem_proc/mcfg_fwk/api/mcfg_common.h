...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/api/mcfg_common.h#1 $ 
...
#define MCFG_CONFIG_LIST_SIZE_MAX         (128)
...
/* Enum for source of config files */
typedef enum mcfg_storage_mode_e{
  MCFG_STORAGE_FIRST = 0,  
  MCFG_STORAGE_LOCAL = 0,  //Traditionally loaded from PDC or embedded Multi MBN
  MCFG_STORAGE_REMOTE,     //Automatically loaded through remote discovery
  MCFG_NUM_OF_STORAGE_MODE
} mcfg_storage_mode_e_type;

#define MCFG_STORAGE_MASK_LOCAL  (1 << MCFG_STORAGE_LOCAL)
#define MCFG_STORAGE_MASK_REMOTE (1 << MCFG_STORAGE_REMOTE)
#define MCFG_STORAGE_MASK_ALL    (MCFG_STORAGE_MASK_LOCAL|MCFG_STORAGE_MASK_REMOTE)
...
/* Enum for types of config files stored */
typedef enum mcfg_comp_status_e{
  MCFG_COMP_STATUS_NONE = 0,
  MCFG_COMP_STATUS_COMPRESSED = 1,
  MCFG_COMP_STATUS_UNCOMP_AND_SWAPPED = 2,
  MCFG_COMP_STATUS_INVALID
} mcfg_comp_status_e_type;
...
typedef struct 
{
  ...
  /* Total size of the compressed config file */
  int comp_config_len;

  mcfg_comp_status_e_type comp_status;
  
  /* Pointer to the actual config buffer */
  void *config_addr;

  /* Pointer to the compressed config buffer */
  void *comp_config_addr;
