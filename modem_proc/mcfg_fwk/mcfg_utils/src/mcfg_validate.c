...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_utils/src/mcfg_validate.c#1 $ 
...
)
{
  ...
  {
     MCFG_MSG_SPRINTF_1("MCFG update fail for item %s", name);
  }
  else 
  {
    MCFG_MSG_SPRINTF_1("Successfully sent XML entry for item %s", name);
...
/* No use cases for Indexed EFS Item for now */
  ...
  if(attributes & U_ITEM_ATTRIB_MULTISIM)
  {
    ...
    {
      MCFG_MSG_SPRINTF_3("MCFG Validate: ignoring item for current subscription, subs_id: 0x%0x, subs_mask=%d, filepath %s", 
  ...
  if(attributes & U_ITEM_ATTRIB_MUXD)
  {
    ...
    if(mcfg_result != MCFG_NV_STATUS_OK)
    {
      MCFG_MSG_SPRINTF_1("Could not update symlink for EFS File: %s", f_name);
      return FALSE;
    }
  }
  ...
  if (mcfg_result == MCFG_NV_STATUS_OK)
  {
    ...
    if (mcfg_result == MCFG_NV_STATUS_OK)
    {
      ...
      {
        MCFG_MSG_SPRINTF_1("mcfg_validate_efs_file: No data to read for empty file %s", f_name);
        efs_close(fd);
      }
    }
...
boolean mcfg_cfg_diff_nvefs(uint8 cfg_type, uint32 num_mcfg_items, uint8* ram_cust_data_ptr, uint8 subs_id_input, boolean validate)
{
  ...
  nv_item_type          *nv_file_data_buf = NULL;
          ...
          if (validate)
          {
            ...
            if (mcfg_item_prefix.attrib & U_ITEM_ATTRIB_MULTISIM)
            {
              ...
              else 
              {
                ...
                MCFG_MSG_LOW_3("MultiSIM: mcfg_cfg_diff_nvefs - item %d, subs_mask=%d, curr_subs_id=%d", 
                                 (int)nv_mcfg_cust_item, subs_mask, subs_id);
                if( !mcfg_proc_subs_mask(subs_mask, subs_id) )
                {
                   MCFG_MSG_LOW_3("MultiSIM: mcfg_cfg_diff_nvefs - ignoring item %d, subs_mask=%d, curr_subs_id=%d", 
                                    (int)nv_mcfg_cust_item, subs_mask, subs_id);
