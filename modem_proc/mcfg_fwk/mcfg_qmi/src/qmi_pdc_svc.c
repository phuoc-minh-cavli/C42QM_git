...
$Header: //components/rel/mcfg_fwk.mpss/7.10.1.1/mcfg_qmi/src/qmi_pdc_svc.c#1 $ 
...
typedef struct 
{
  ...
  pdc_list_config_cache_type    list_config_cache;
} pdc_service_type;
...
static void pdc_send_list_configs_ind(pdc_client_type *, pdc_list_configs_req_msg_v01 *, pdc_list_config_cache_type *)UNUSED_ATTRIBUTE;
...
)
{
  ...
  if ( pdc_client )
  {
    ...
    if (pdc_client->qmi_client_handle == service->list_config_cache.qmi_client_handle &&
        !service->list_config_cache.config_list)
    {
      mcfg_free(service->list_config_cache.config_list);
      memset(&service->list_config_cache, 0, sizeof(pdc_list_config_cache_type));
    }
...
)
{
  ...
  if (!service->list_config_cache.config_list)
  {
    mcfg_free(service->list_config_cache.config_list);
  }
  memset(&service->list_config_cache, 0, sizeof(pdc_list_config_cache_type));
...
)
{
  ...
  {
    MCFG_MSG_HIGH_1( "pdc_handle_get_selected_config_req(): invalid subscription %lu", 
  ...
  {
    MCFG_MSG_HIGH_1( "pdc_handle_get_selected_config_req(): invalid slot %lu", 
                      request->slot_id );
    response->resp.error = QMI_ERR_NONE_V01;
  }
  else
...
)
{
  ...
  else if (!IS_VALID_PDC_CONFIG_ID_LEN(request->new_config_info.config_id_len))
  {
    ...
    response->resp.error = QMI_ERR_INVALID_ID_V01;
      ...
      (!IS_VALID_MCFG_SUBSCRIPTION_ID(request->subscription_id) ||
      (request->subscription_id > MCFG_SUB_ID_FIRST)))
  {
    ...
    response->resp.error = QMI_ERR_NOT_SUPPORTED_V01;
      ...
      (!IS_VALID_MCFG_SLOT_ID(request->slot_id) || (request->slot_id > MCFG_SLOT_INDEX_FIRST)))
  {
...
)
{
  ...
  cache    =  &service->list_config_cache;
  ...
  MCFG_MSG_LOW_1("pdc_handle_list_configs_req(): qmi_client_handle %x", 
  ...
  if (response->resp.error == QMI_ERR_NONE_V01)
  {
    if (cache->config_list)
    {
      ...
      if (client->qmi_client_handle != cache->qmi_client_handle || 
      ...
      {
        MCFG_MSG_LOW_1("pdc_handle_list_configs_req(): reset config_list for cached token %d", cache->ind_token);
        ...
        if (request->config_type_valid && request->multi_support_valid && request->multi_support)
        {
          ...
          MCFG_MSG_LOW_1("pdc_handle_list_configs_req(): ind_token %x", cache->ind_token);
        }
      }
    }
    else
    ...
    pdc_send_list_configs_ind(client, request, cache);
  }
...
)
{
  ...
  /* BEGIN: Check for subscription/slot to not to cross singleSIM capability */
  else if (request->subscription_id_valid == TRUE && 
      (!IS_VALID_MCFG_SUBSCRIPTION_ID(request->subscription_id) ||
      (request->subscription_id > MCFG_SUB_ID_FIRST)))
  {
    MCFG_MSG_ERROR_1( "pdc_handle_activate_config_req(): invalid subscription %lu", 
                      request->subscription_id );
    response->resp.error = QMI_ERR_NOT_SUPPORTED_V01;
  }
  else if (request->slot_id_valid == TRUE && 
      (!IS_VALID_MCFG_SLOT_ID(request->slot_id) || (request->slot_id > MCFG_SLOT_INDEX_FIRST)))
  {
    MCFG_MSG_ERROR_1( "pdc_handle_activate_config_req(): invalid slot %lu", 
                      request->slot_id );
    response->resp.error = QMI_ERR_NOT_SUPPORTED_V01;
  }
  /* END */
...
)
{
  ...
  /* Validate request */
  if (request->slot_id >= MCFG_NUM_OF_UIM_SLOT && request->slot_id != 0xFFFFFFFF && request->slot_id > MCFG_SLOT_INDEX_FIRST)
  {
...
{
  pdc_service_type *service;
  pdc_refresh_ind_msg_v01 *refresh_event_ind;
  ...
  service = NULL;
  refresh_event_ind = NULL;
  
  /* Check pass pointers */
  if ((slot_index == MCFG_REFRESH_INDEX_INVALID) || (sub_index == MCFG_REFRESH_INDEX_INVALID))
  {
    MCFG_MSG_ERROR( "pdc_generate_refresh_event_ind(): Invalid param");
    return;
  }
  
  /* Get memory to store the IND data */
  refresh_event_ind = mcfg_malloc(sizeof(pdc_refresh_ind_msg_v01)); 
  if (!refresh_event_ind)
  {
    MCFG_MSG_ERROR( "pdc_generate_refresh_event_ind(): malloc failed");
    return;
  }
  ...
  if (!service)
  {
    MCFG_MSG_ERROR( "pdc_generate_refresh_event_ind(): service pointer NULL");
    mcfg_free(refresh_event_ind);
    return;
  }
  
  /* Compose the IND payload */
  qmi_error = pdc_qmi_compose_refresh_event_ind(refresh_event_ind,
                                                &ind_size,
                                                event_reg_type,
                                                slot_index,
                                                sub_index);
  if ( qmi_error != QMI_CSI_NO_ERR )
  {
    MCFG_MSG_ERROR_1( "pdc_generate_refresh_event_ind(): generating IND returned error (%d)", qmi_error);
    mcfg_free(refresh_event_ind);
    return;
  }
  ...
  for ( i = 0; i < PDC_CLIENTS_MAX; i++ )
  {
    if ( service->pdc_clients[i].connected == TRUE )
    {
      if ( service->pdc_clients[i].event_reg.refresh_event == TRUE )
      {
        qmi_error = qmi_csi_send_ind(service->pdc_clients[i].qmi_client_handle,
        ...
        if (qmi_error != QMI_CSI_NO_ERR)
        {
          MCFG_MSG_ERROR_2( "Failure to send indication: %d, ind_size %lu", (int) qmi_error, ind_size );
        }
      } /* if ( service->pdc_clients[i].event_reg.selected_config == TRUE ) */
    } /* if ( service->pdc_clients[i].connected == TRUE ) */
  } /* for ( i = 0; i < PDC_CLIENTS_MAX; i++ ) */
  ...
  /* Free memory */
  mcfg_free(refresh_event_ind);
} /* pdc_generate_refresh_event_ind() */
...
)
{
  ...
  if (request->config_type_valid == TRUE && cache != NULL && 
      request->multi_support_valid == TRUE && request->multi_support == TRUE)
  {
    MCFG_MSG_MED_1("client supports multiple for type %d", request->config_type);
    multi_support = TRUE;
  }
  ...
  else
  {
    do
    {
      if (cache->config_list == NULL)
      {
        ...
        MCFG_MSG_MED_1("client multiple list_ret %d", list_ret);
        ...
        if (!cache->config_list)
        {
          p_list_configs_ind->error = QMI_ERR_INTERNAL_V01;
          p_list_configs_ind->config_list_valid = FALSE;
          break;
        }
      ...
      MCFG_MSG_MED_2("client multiple list_ret total %d num_of_sent %d", cache->total, cache->num_of_sent);
      ...
      for(i=0; i<PDC_CONFIG_LIST_SIZE_MAX_V01 && ((cache->num_of_sent + i) < cache->total); i++)
      {
        ...
        //MCFG_MSG_MED_2("index %d id len %d", i, p_list_configs_ind->config_list[i].config_id_len);
      }
  ...
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): config_list_len %d", 
                 p_list_configs_ind->config_list_len);
#ifdef MCFG_DEBUG
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): error %d", 
                 p_list_configs_ind->error);
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): config_list_valid %d", 
                 p_list_configs_ind->config_list_valid);
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): more_available_valid %d", 
                 p_list_configs_ind->more_available_valid);
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): more_available %d", 
                 p_list_configs_ind->more_available);
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): ind_token_valid %d", 
                 p_list_configs_ind->ind_token_valid);
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): ind_token %d", 
                 p_list_configs_ind->ind_token);
  for(i=0; i<PDC_CONFIG_LIST_SIZE_MAX_V01; i++)
  {
    MCFG_MSG_MED_2("id index %d id len %d", i, p_list_configs_ind->config_list[i].config_id_len);
  }
  MCFG_MSG_LOW_1("pdc_send_list_configs_ind(): qmi_client_handle %x", 
                 client->qmi_client_handle);
#endif
...
)
{
  ...
  if (request->config_id_valid == FALSE)
  {
    delete_config_ind.error = pdc_err_map_mcfg_to_qmi(
      mcfg_utils_delete_all(request->config_type, MCFG_STORAGE_MASK_ALL));
  }
  else
  {
  ...
  {
    delete_config_ind.error = pdc_err_map_mcfg_to_qmi(
        mcfg_utils_delete_config(request->config_type, &id, MCFG_STORAGE_MASK_ALL));
    }
  }
...
)
{
      ...
      request->activation_type == PDC_ACTIVATION_REFRESH_ONLY_V01)
  {
    ...
    {
      MCFG_MSG_LOW_1("QMI_PDC_ACTIVATE refresh sloti=%d", request->slot_id);
    ...
    else if (!request->slot_id_valid && !request->subscription_id_valid ) 
    {
      if(mcfg_refresh_is_ongoing(MCFG_REFRESH_INDEX_MASK_ALL))
      {
        ...
        activate_config_ind.error =  QMI_ERR_DEVICE_IN_USE_V01;
      }
      else
      {
        MCFG_MSG_LOW("QMI_PDC_ACTIVATE refresh together");
        memset(&global_pdc_svc.activate_cache[0], 0, (MCFG_NUM_OF_UIM_SLOT+1)*sizeof(pdc_activate_cache_type));

        p_cache = &global_pdc_svc.activate_cache[MCFG_NUM_OF_UIM_SLOT];
        p_cache->qmi_client_handle = client->qmi_client_handle;
        p_cache->ind_token_valid = request->ind_token_valid;
        p_cache->ind_token = request->ind_token;
        activate_config_ind.error = pdc_err_map_mcfg_to_qmi(mcfg_refresh_all_by_apps());
        if(activate_config_ind.error == QMI_ERR_NONE_V01)
        {
          /* indication will be sent out later */
          return;
...
)
{
  ...
  MCFG_MSG_MED_1("pdc_send_cached_activate_config_ind for sloti %d", slot_index);
  ...
  else
  {
    ...
    for (slot_index = 0; slot_index < MCFG_NUM_OF_UIM_SLOT; slot_index++)
    {
      ...
      {
        activate_config_ind.error = pdc_err_map_mcfg_to_qmi(p_cache->error);
      }
    }

    MCFG_MSG_MED("Refresh all completed");   
    ...
    if (qmi_error != QMI_CSI_NO_ERR)
    {
      MCFG_MSG_MED_1("failed to activate refresh all indication: %d", qmi_error);
    }
    else
    {
      MCFG_MSG_LOW("QMI_PDC_ACTIVATE_CONFIG_IND_V01 is sent for refresh all");
    }
