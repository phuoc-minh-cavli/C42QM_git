...
$Header: //components/rel/rfa.mpss/3.10/rf/mdsp/qdsp6_common/inc/rfcommon_mdsp_event_intf.h#1 $
...
typedef struct
{
  ...
  boolean use_ssbi_access;          /*!< Applicable only to SSBI transactions. If set, then SSBI_ACCESS task type is used.
                                     ...
                                     */
  uint16 subevent_id;                /* outer level token_id which determines which task does the transactions belong too */										 
