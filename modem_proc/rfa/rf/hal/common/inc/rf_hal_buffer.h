...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/hal/common/inc/rf_hal_buffer.h#1 $
...
/* default value of scaling factor for micro seconds */
#define RF_BUFFER_TIME_UNITY_SCALING_FACTOR 100

/* Macro to define number of bytes in a word */
#define RF_BUFFER_NUM_BYTES_IN_WORD   4
...
public:
  ...
  rf_script_buffer( uint16 ssbi_script_size,
                    ...
                    /* multiplication factor to convert microseconds to 
                       desired tech units*/
    uint32 scaling_factor_x100 = RF_BUFFER_TIME_UNITY_SCALING_FACTOR,
    uint16 rffe_group_num_groups =0,
    uint16 rffe_group_payload_size =0
  );
  ...
  (
    ...
    rf_hal_bus_token_group_type *rffe_tokens,
    uint8 num_tokens
  );

  void init_rffe_group();
...
private:
  ...
  boolean get_rffe_group_execution_time
  (
    rf_ccs_rffe_clk_enum  rffe_clk,
    uint16                num_bytes,
    int32&                exec_time_usec
  );

  /*! override from rftech to skip timer fixup during script building */
   boolean timer_fixup_entries_allowed; 
  ...
  /*! RFFE scripts in RF HAL format */
  rf_hal_bus_resource_script_type rffe_group;

  /*! Max number of rffe groups */
  uint16 max_num_rffe_groups;

  /*! Max payload size for rffe groups */
  uint16 max_rffe_group_payload_bytes;

  /*! payload bytes used */
  uint16 rffe_group_payload_bytes_used;

  /*! RFFE group payload block */
  uint8* rffe_group_payload_blk_ptr;

  /*! RFFE group payload block */
  uint8* rffe_group_curr_payload_ptr;

  /*! Max number of tokens */
  uint16 max_rffe_group_token_num;

  /*! Current number of tokens */
  uint16 rffe_group_current_token_num;

  /*! RFFE group token block */
  rf_hal_bus_token_group_type* rffe_group_token_blk_ptr;

  /*! RFFE group token block */
  rf_hal_bus_token_group_type* rffe_group_curr_token_ptr;

  /*! Current time (usec) to keep track of start_delta */
  int32 rffe_group_curr_time;  
...
/*----------------------------------------------------------------------------*/
extern rf_buffer_intf* 
rf_hal_buffer_create
(
  uint16 ssbi_script_size,
  uint16 rffe_script_size,
  uint16 grfc_script_size,
  uint16 rffe_group_num_groups,
  uint16 rffe_group_max_payload_size_in_uint32
);
