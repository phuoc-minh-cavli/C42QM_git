...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/task/common/src/rf_apps_task.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
04/09/20   gk      Sequence change(Move psm exit info init after rcinit handshake)
...
uint32 efs_data_size=0;
rfm_psm_gsm_info_s         rfm_gsm_psm_info={RFCOM_NUM_GSM_BANDS,FALSE};
rfcom_gsm_band_type efs_data=RFCOM_NUM_GSM_BANDS;
...
)
{
  ...
  /*Initialize hibernation & psm exit information*/
  rflte_psm_hibernation_info_init();
  rfgsm_psm_exit_info_init();
 ...
 /*!
   @brief
  To fill psm mode or hibernation info initializations for GSM
 ...
 */
 void rfgsm_psm_exit_info_init(void)
 {
   const char* offload_band_file_name="rf/offload_band_info.txt"; 
   uint8 data_temp=(uint8)RFCOM_NUM_GSM_BANDS;
   uint8* data= &data_temp;
   uint8 data_char=0;
   int32 data_max_size=1;
   int32 data_size=-1;
   rfcom_gsm_band_type gsm_band;
 
   data_size=rfcommon_efs_get(offload_band_file_name, data, 1 );
   efs_data_size=data_size;
	 MSG_3( MSG_SSID_RF, MSG_LEGACY_ERROR, "PSM rfcommon_efs_get: data is %d at address 0x%x with data_size %d ",*data,data,data_size  ); 
   if(data_size==-1)
	{
	  rfm_gsm_psm_info.psm_gsm_exit=FALSE;
	  rfm_gsm_psm_info.band=RFCOM_NUM_GSM_BANDS;
	  MSG( MSG_SSID_RF, MSG_LEGACY_ERROR, "data_size is -1	Failed "  ); 
 
	  return;
	}
   else
	{
	  data_char=(uint8)(*data);
	  if(data_char >='0')
	  {
	   data_char-='0';
	  }
	  gsm_band=(rfcom_gsm_band_type)(data_char);
	  efs_data=gsm_band;
	  if(gsm_band >RFCOM_NUM_GSM_BANDS)
	   {gsm_band=RFCOM_NUM_GSM_BANDS;}
	  rfm_gsm_psm_info.band=gsm_band;
	  rfm_gsm_psm_info.psm_gsm_exit=TRUE;
	MSG_4( MSG_SSID_RF, MSG_LEGACY_ERROR, "PSM data_char is %d gsm_band is %d rfm_gsm_psm_info.band %d rfm_gsm_psm_info.psm_exit %d  Failed ",data_char,gsm_band,rfm_gsm_psm_info.band,rfm_gsm_psm_info.psm_gsm_exit );  
	}
 ...
 void rf_apps_task_handle_sigs( rex_sigs_type mask )
{
 //TODO 
}
