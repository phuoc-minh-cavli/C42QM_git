...
$Header: //components/rel/rfa.mpss/3.10/rf/task/common/src/rf_cmd_dispatcher.c#2 $
...
)
{
  ...
  rf_module_dispatch_tbl[task_num] = 
            clade_heap_alloc( num_of_module * sizeof(rf_module_dispatch_tbl_type)
                         );
 #else
...
)
{
  ...
  /* Handle DO_THERM_READ CMD */
  else if ( cmd->msgr_hdr.id == RFLM_WCDMA_DO_THERM_READ_CMD )
  {
    rf_dispatch_command( rf_task_get_dispatch_handle(),cmd );
    rf_dispatch_done = TRUE;
  }
