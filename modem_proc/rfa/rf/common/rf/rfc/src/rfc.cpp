...
Copyright (c) 2011 - 2019 by QUALCOMM Technologies Inc.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/3.10/rf/common/rf/rfc/src/rfc.cpp#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/24/20   mp      Clade heap changes
07/30/19   am      Fix device creation issue for pc5 in LTE Page only
06/26/19   as      Revert "Made rfc_update_alt_port_mapping dummy"
06/24/19   am      Skip PA obj creation for lte page only
06/19/19   bm      Made rfc_update_alt_port_mapping dummy
04/19/19   am      Add RFFE V2 support
04/09/19   gk      KW fixes
03/05/19   sk      RF PAGE ONLY changes for NB/CATM
02/25/19   dh      Added support for PA therm device
02/08/19   sk      Skip RF_XO_CLK_2 is set XO drive strength for 9205
...
08/10/16   ds      Added FEATURE_CGPS to fix compilation erros for lteonlynogps.test flavor.
...
#define LTE_PAGE_ONLY_CHECK() (0)
#else 
#define LTE_PAGE_ONLY_CHECK() (1)
#endif
...
rfc_intf::rfc_intf(rf_hw_type rfhw, system_clock_enum sysclk)
{
  ...
  rfdevice_trx_common_intf_class *transceiver_cmn_device = NULL;
 ...
 /* boot_up_init sequence at a low clock rate as a bit stream on the GRFC line */
  ...
  {
    transceiver_cmn_device = (rfdevice_trx_common_intf_class*)cmn_rf_devices[RFDEVICE_TRANSCEIVER][0].device_obj;
  ...
  {
    transceiver_cmn_device = (rfdevice_trx_common_intf_class*)cmn_rf_devices[RFDEVICE_TRANSCEIVER][1].device_obj;
...
boolean rfc_intf::create_cmn_rf_devices(void)
{
  ...
  #ifdef FEATURE_CLADE_HEAP
  phys_devices_array = (rfc_physical_device_struct_type*)clade_heap_alloc(sizeof(rfc_physical_device_struct_type)*phys_devices_count);
  #else 
  ...
  while(phy_devices_cfg[index].rf_device_id != RFDEVICE_INVALID)
  {
   ...
   /*Create the physical device*/
    if ((phy_devices_cfg[index].rf_device_id == GEN_PA) && LTE_PAGE_ONLY_CHECK() == 1 ) 
    {
  ...
  while(logical_devices_cfg[index].rf_device_type != RFDEVICE_TYPE_INVALID)
  {
    ...
    else
    {
      ...
      if ( (logical_devices_cfg[index].rf_device_id == GEN_ASM) || (logical_devices_cfg[index].rf_device_id == GEN_PA) ||
      	     (logical_devices_cfg[index].rf_device_id == PA_THERM))
      {  
        if ( ((phy_devices_cfg[phy_dev_instance].rf_device_id == GEN_DEVICE) && (logical_devices_cfg[index].rf_device_id == GEN_PA) && LTE_PAGE_ONLY_CHECK() == 1 ) )
      {
...
void rfc_intf::create_gnss_rfc()
{
       ...
       (instance_id < RFC_MAX_DEVICE_INSTANCES) )
  {
    dev_param[RFM_DEVICE_GPS].cmn_device =
      (rfdevice_trx_common_intf_class*)cmn_rf_devices[dev_type][instance_id].device_obj;
  }
  else
  {
...
{
  rfdevice_trx_common_intf_class *common_device = NULL;
...

  {
    ...
    {
      common_device = ((rfdevice_trx_common_intf_class *)cmn_rf_devices[RFDEVICE_TRANSCEIVER][instance_num].device_obj);
...
boolean rfc_intf::create_asdiv_tuner_mgr( void )
{
  boolean status = TRUE;
    ...
    #ifdef FEATURE_CLADE_HEAP
    device_config = (rfm_devices_configuration_type *)clade_heap_alloc(sizeof(rfm_devices_configuration_type));
    #else 
...
)
{
  ...
  rfdevice_trx_common_intf_class* transceiver_device = NULL;
  ...
  else
  {
    ...
    if(device_obj != NULL)
    {
      ...
      {
        transceiver_device = (rfdevice_trx_common_intf_class*)device_obj;
      }
      else
      {
...
{
  rfdevice_trx_common_intf_class *temp_rfdevice_class = NULL;
  ...
  {
    temp_rfdevice_class = (rfdevice_trx_common_intf_class*)
...
boolean rfc_set_xo_drive_strength(void)
{
 ...
 /*Get the rfc instance*/
  ...
  for (clk_index = 0; clk_index < RF_XO_CLK_2; clk_index++)
  {
  ...
  switch (mode)
  {
    ...
    case RFCOM_GSM_MODE:
      ...
      vco_path = rfdevice_gsm_rx_get_vco_path(
                                               (rfdevice_trx_gsm_rx_intf_class*)trx_device,
                                               data_array,
                                               array_size
                                              );
    ...
    case RFCOM_LTE_MODE:
      ...
      vco_path = rfdevice_lte_rx_get_vco_path(
                                               (rfdevice_trx_lte_rx_intf_class*)trx_device,
                                               band.lte_band
                                              );
      break;
