...
$Header: //components/rel/rfa.mpss/3.10/rf/common/rf/mc/src/rfcommon_mc.c#2 $
...
04/12/17   gk      CW Fixes
03/28/17   cv      Added FTM reclaim support
12/09/15   ndb     Remove the un-used tuner code 
...
/* Used for mcfg_dynrec_load() argument */

#define    RFC_RECLAIM                1
#define    RFC_FTM_RECLAIM            2
#define    RFC_NV_INACTIVE            3
uint16 ftm_rfc_reclaim;
int32        ftm_nv_data;
...
/*----------------------------------------------------------------------------*/
void 
ftm_mode_check(rex_tcb_type *task_ptr, rex_sigs_type task_wait_sig, void (*task_wait_handler)( rex_sigs_type )  )
{

  nv_items_enum_type item_code = NV_FTM_MODE_I;
  nv_stat_enum_type  nv_read_status = NV_DONE_S;

  nv_read_status = rfcommon_nv_get_item( item_code, 
  	                                     (nv_item_type*)&ftm_nv_data, 
  	                                     task_ptr, 
  	                                     task_wait_sig, 
  	                                     task_wait_handler );
  ...
  {
      ftm_rfc_reclaim = RFC_NV_INACTIVE;
  }
  ...
  if( nv_read_status == NV_DONE_S  )
  {
    if(ftm_nv_data)
    {
      ftm_rfc_reclaim = RFC_FTM_RECLAIM;
    }
    else
    {
      ftm_rfc_reclaim = RFC_RECLAIM;
    }
  }
}  
                 ...
                 void (*task_wait_func_ptr)( rex_sigs_type ))
{
  ...
  int n;
  char *rfc_dynload_ftm[2];
  char dynrec_ftm[12] = {"rfc_ftm"};
  rfc_dynload_ftm[0] = dynrec_ftm;
  ...
  ftm_mode_check( caller_tcb_ptr, task_nv_wait_sig, task_wait_func_ptr );

  if( (ftm_rfc_reclaim == RFC_NV_INACTIVE)||(ftm_rfc_reclaim == RFC_FTM_RECLAIM) )
  {
#ifndef TEST_FRAMEWORK
    if ((n = mcfg_dynrec_load(&rfc_dynload_ftm[0], 1)) != 0)
    { 
      MSG_FATAL("ftm_rfc_reclaim: %d, failed to load rfc_ftm!!!", ftm_rfc_reclaim, 0,0);
    }
#endif
  }
...
)
{
  ...
  RF_MSG_5( RF_HIGH, "rfcommon_mc_xpt_txagc_override_v2: XPT Mode %d, "
...
#define HALF_RATE TRUE
// rd_delay as false.
#define RD_DELAY FALSE
#define REG_SIZE_MAX 255

boolean
rfcommon_mc_snapshot_rffe_devices
(
   void
)
{
  ...
  uint16 reg_list_device_count = snapshot_data->reg_list_phy_rffe_device_count;
  if ( snapshot_data->initialized == FALSE )
  {
    RF_MSG(RF_HIGH,"RFC uninit. Skip saving RFFE regs");
    return FALSE;
  }  
  for ( dev_idx = 0; dev_idx < reg_list_device_count && dev_idx < RFA_SNAPSHOT_MAX_CUSTOM_DEVICE_NUM; dev_idx ++ )
  {
    if ( snapshot_data->reg_list_rffe_dump[dev_idx] != NULL )
    {
      
      bus= snapshot_data->reg_list_rffe_dump[dev_idx]->bus;  
      slave_id= snapshot_data->reg_list_rffe_dump[dev_idx]->slave_id; 
	   
      if(snapshot_data->reg_list_rffe_dump[dev_idx]->data_dump_size != REG_SIZE_MAX)
	  {
        for (reg_idx = 0; reg_idx < snapshot_data->reg_list_rffe_dump[dev_idx]->data_dump_size; reg_idx++ )
	    {
		  addr = snapshot_data->reg_list_rffe_dump[dev_idx]->addr_list[reg_idx];
		  slave_id = snapshot_data->reg_list_rffe_dump[dev_idx]->slave_id;
		  bus = snapshot_data->reg_list_rffe_dump[dev_idx]->bus; 
          num_bytes=4;
          ...
          if (addr <= 0x1f)
          {
            if(rf_hal_rffe_read_byte_offline(bus,slave_id,RD_DELAY,HALF_RATE,addr,&data))
		    {
		      snapshot_data->reg_list_rffe_dump[dev_idx]->data_dump[reg_idx] = (uint32)data;
            }
			else
			{
				//RF_MSG(RF_HIGH, "rf_hal_rffe_read_byte_offline failed !!!");
			}
		  } // if addr <= 0x1f
		  else
		  {
            if(rf_hal_rffe_ext_read_bytes_offline(bus,slave_id,RD_DELAY,HALF_RATE,num_bytes,addr,&data))
            {
			  snapshot_data->reg_list_rffe_dump[dev_idx]->data_dump[reg_idx] = data;
		    }
			else
			{
			  //RF_MSG(RF_HIGH, "rf_hal_rffe_ext_read_byte_offline failed !!!");
			}
		  } // end add < 0x1f
		} // loop reg_idx	
	  } //   data_dump_size == REG_SIZE_MAX
	  else
	  {	  
	    //RF_MSG_1(RF_HIGH,"data_dump_size == REG_SIZE_MAX %d ", snapshot_data->reg_list_rffe_dump[dev_idx]->data_dump_size);
  	  }  
	}//reg_list_rffe_dump NULL  
    else
    {
      //RF_MSG(RF_HIGH,"RFC uninit. reg_dump_rffe NULL");
    }
  } // loop dev_idx
  snapshot_data->dump_successful = TRUE;
  return TRUE;
}

boolean rfcommon_mc_snapshot_cmn_reg
(
   void
)
{
  rf_time_type time_prof ;
  rf_time_tick_type time_tick;

  rfdevice_trx_common_intf_class* wtr_object = NULL;
  
  uint8 instance = 0;

  time_tick = rf_time_get_tick();
  
  for (instance = RFCMN_CORE_PHY_DEVICE_0; instance < RFCMN_CORE_MAX_PHY_DEVICES; instance++)
  {
    wtr_object = (rfdevice_trx_common_intf_class*) rfc_common_get_cmn_rf_device_object(RFDEVICE_TRANSCEIVER, instance);
	
    if ( wtr_object != NULL )
    {
      if( TRUE == rfdevice_cmn_reg_dump(wtr_object, RFDEVICE_CRITICAL_REG_DUMP, FALSE))
      {	  
        time_prof = rf_time_get_elapsed( time_tick, RF_MSEC );

        //RF_MSG_2(RF_HIGH, "WTR %d done %d", instance, time_prof);

        /* Only Support up to 1 WTR */
        break;
      }
      else
      {
        //RF_MSG(RF_HIGH,"rfcommon_mc_snapshot_cmn_reg: rfdevice_cmn_reg_dump returns false");  
        return FALSE;
      }
    }
    else
    {
      //RF_MSG(RF_HIGH,"rfcommon_mc_snapshot_cmn_reg: wtr obj NULL");		
      return FALSE;
    }
  }
  return TRUE;
...
/*!
  @brief
  The API reads back all WTR registers and save them in memory

  @details
  This API can be called by MPSS error handling routine at modem crash. It is
  essential that the API's implementation obeys modem stability team's restriction
  and requirements. Examples include: no mutex lock/unlock.
*/
static uint32 rf_mss_rfc_cfg_rcgr = 0;

void 
rfcommon_mc_snapshot_handler
(
   void
)
{
  rf_mss_rfc_cfg_rcgr =  HWIO_IN(MSS_RFC_CFG_RCGR);
  if(rf_mss_rfc_cfg_rcgr == 1)
  {
    RFLM_DIAG_TRACE_MED ( RFLM_DIAG_MSG_SSID_NONE, "Skipped saving RF registers: Modem off");	  
    return ; 
  }
  rf_time_type time_prof ;
  rf_time_tick_type time_tick = rf_time_get_tick();      
  time_tick = rf_time_get_tick();

  if ( rfcommon_mc_snapshot_rffe_devices() == TRUE )
  {
    time_prof = rf_time_get_elapsed( time_tick, RF_MSEC );
  }

  if ( FALSE == rfcommon_mc_snapshot_cmn_reg())
  {
    //RF_MSG(RF_HIGH, "mc_snapshot: cmn reg returns false");
  }
	  
  
  RFLM_DIAG_TRACE_MED ( RFLM_DIAG_MSG_SSID_NONE, "Finished saving RF registers");
}/* rfcommon_mc_snapshot_handler */
