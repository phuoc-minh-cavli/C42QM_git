...
10/23/20   gk      Fix for lte band bit mask update
12/04/19   mp      NBIOT Band86 changes
07/22/19   up      Added debug info for RF HAL vote done for PA therm reads.
07/08/19   bm      Changes to clear HAL vote when temp read fails in rfm_read_thermistor
06/17/19   dh      Fixed compilation warnings
04/08/19   up      Modified rfm_read_thermistor for PA therm read.
04/09/19   gk      KW & CW fixes
...
02/25/19   dh      Added support for PA therm read
11/01/18   sk      Linker error fix
10/25/18   sk      TRACK B Bringup changes
10/05/18   mns     Device snapshot changes.
...
04/30/18   gk      bootup optimizations added time profiling variables
...
09/26/16   qzh     Fix KW errors
06/03/16   qzh     Add CATM1 HD FDD related changes
03/15/16   ase     Protected RFGSM function calls under FEATURE_GSM
...
#include "rfcommon_core_maat_manager.h"
#endif

extern boolean rfcmn_core_maat_mgr_debug_flag;
...
#include <stringl.h>
#include "rflm_ccs_rf_rffe_log.h"


volatile rf_time_tick_fine rfm_init_time ;
volatile rf_time_tick_fine rflte_mc_init_time;
volatile rf_time_tick_fine rfc_common_init_time ;

rfm_cmd_buffer_type rfm_cmd_buffs;

extern boolean therm_mitigation_debug_flag;
/*----------------------------------------------------------------------------*/
...
rfdevice_iotfe_therm_debug_type iotfe_debug_info;
...
)
{
...
#endif 
  rfm_init_time = rf_time_get_tick().fine_timetick;
  ...
  if (rfm_initialized==RFM_INIT_NOT_RUN)
  {
    ...
    rfc_common_init_time = rf_time_get_tick().fine_timetick;
    common_init_success &= rfcommon_mc_init(RFCOM_TRANSCEIVER_0,
                                           caller_tcb_ptr, task_nv_wait_sig,
                                           task_wait_func_ptr);
    rfc_common_init_time = rf_time_get_tick().fine_timetick-rfc_common_init_time; 
    ...
    {
      rflte_mc_init_time = rf_time_get_tick().fine_timetick;
      ...
      technology_init_success = technology_init_success && (ret_val == TRUE);
      rflte_mc_init_time = rf_time_get_tick().fine_timetick-rflte_mc_init_time;
    }
#endif /*FEATURE_LTE*/       
  ...
  if( (common_nv_tbl_ptr != NULL)&&(common_nv_tbl_ptr->rf_lte_enable_txpl_debug_msg & RF_LTE_IRAT_DEBUG_MASK))
  {	
    rf_ccs_rffe_grfc_logging_t rffe_grfc_log_settings;
    uint32 grfc_log_mask_32 = 0;
    uint32 rffe_log_mask_32 = 7; // enable for all 3 channel: 7-> 111. 
	  
    RF_MSG_1( RF_HIGH, "RFLM_CCS_LOGGING: RFM_INIT: Start of RFFE logging: %d",  rffe_log_mask_32);	
    rffe_grfc_log_settings.new_rfc_log_mask =  (rffe_log_mask_32 << 1) | (grfc_log_mask_32 & 0x01); //final log_mask : bit 1 : grfc en/dis and rffe channels bitmask
    rflm_ccs_rf_rffe_logging_start(&rffe_grfc_log_settings);
  }
  ...
  if (err_crash_cb_register(rfcommon_mc_snapshot_handler) == FALSE)
  {
    RF_MSG( RF_ERROR, "rfm_init: Unable to register error fatal "
                      "handler callback function rfcommon_mc_snapshot_handler().");
  }
  rfm_init_time = rf_time_get_tick().fine_timetick-rfm_init_time;
                 ...
                 )
{
  ...
  case RFCOM_WCDMA_MODE:
    #ifdef FEATURE_WCDMA
    RFWCDMA_MC(sleep, ret_val)( device, user_data_ptr,(rfm_cb_handler_type) cb_handler,rxlm_buf_idx);
    #endif
    break;
                  ...
                  )
{
  ...
  case RFCOM_WCDMA_MODE:
    #ifdef FEATURE_WCDMA
    RFWCDMA_MC(wakeup, ret_val)(device,user_data_ptr,(rfm_cb_handler_type) cb_handler,rxlm_buf_idx );
    #endif
    break;
...
sys_lte_band_mask_e_type rfm_get_lte_band_mask ( void )
{
  uint64 band_mask = 0;
  uint8  loop_idx  = 0;
  sys_lte_band_mask_e_type rfm_lte_band_mask = {0}; 
  uint8 offset, b66_offset,b85_offset;
#ifndef TEST_FRAMEWORK
  memset(&rfm_lte_band_mask,0,sizeof(sys_lte_band_mask_e_type));
  if (256 == LTE_BAND_NUM)
  {
    offset = 191; 
    b66_offset = 14;
  ...
  if(rflte_get_is_psm_mode() == FALSE)
  {
    band_mask = rfcommon_mc_get_lte_band_mask();
  }
  else
  {
    if(rfm_init_complete == FALSE)
    {
      band_mask =  rflte_mc_get_band_mask_from_bc_config(rfcommon_nv_tbl.rf_lte_bc_config);
    }
    else
    {
  ...
  for (loop_idx = 50; loop_idx <= 61; loop_idx++)
  {
    if ((band_mask >> loop_idx) & 1)
    {
     ...
     if((loop_idx == 52)|(loop_idx == 53)|(loop_idx == 54)|(loop_idx == 55))//set 84th bit for band85 25 = 84-59
     {
       SYS_LTE_BAND_MASK_ADD_BAND(&rfm_lte_band_mask, loop_idx + b85_offset); 
     }
     else if (loop_idx == 51)
     {
      SYS_LTE_BAND_MASK_ADD_BAND(&rfm_lte_band_mask, loop_idx + b66_offset); 
     }
     else if((loop_idx == 56)|(loop_idx == 57)|(loop_idx == 58))
     {
 ...
 /*copy final band mask to intrenal structure for reference*/
 rfcommon_nv_tbl.rf_lte_policyman_band_mask[0]= rfm_lte_band_mask.bits_1_64;
 rfcommon_nv_tbl.rf_lte_policyman_band_mask[1]= rfm_lte_band_mask.bits_65_128;
 rfcommon_nv_tbl.rf_lte_policyman_band_mask[2]= rfm_lte_band_mask.bits_129_192;
 rfcommon_nv_tbl.rf_lte_policyman_band_mask[3]= rfm_lte_band_mask.bits_193_256;

  MSG_4(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfm_get_lte_band_mask rfm_init_complete:%d band_mask:0x%x bits_1_64:0x%x bits_65_128:0x%x",\
	  rfm_init_complete,band_mask,rfm_lte_band_mask.bits_1_64,rfm_lte_band_mask.bits_65_128);
...
uint64 rfm_get_lte_band_mask ( void )
{
  RFM_DEBUG_EN_Q(&rfm_get_lte_band_mask, 0, 0, 0, 0);
   ...
   {
     return rfcommon_nv_tbl.rf_lte_bc_config;
   } 
...
)
{
  ...
  case SYS_BAND_LTE_EUTRAN_BAND66:
    rf_card_band = RF_LTE_B66_BAND;
    break;
  
  case SYS_BAND_LTE_EUTRAN_BAND71:
    rf_card_band = RF_LTE_B71_BAND;
    break;

  case SYS_BAND_LTE_EUTRAN_BAND72:
    rf_card_band = RF_LTE_B72_BAND;
    break;

  case SYS_BAND_LTE_EUTRAN_BAND73:
    rf_card_band = RF_LTE_B73_BAND;
    break;

  case SYS_BAND_LTE_EUTRAN_BAND85:
    rf_card_band = RF_LTE_B85_BAND;
    break;

  case SYS_BAND_LTE_EUTRAN_BAND86:
    rf_card_band = RF_LTE_B86_BAND;
    break;
...
)
{
  ...
  boolean result = FALSE;
    ...
    /* Currently only implemented for LTE */  
      ...
      RF_MSG_1( RF_HIGH, "get_txpll_script rx_freq_error = %d hz", rx_freq_error_in_hz);
...
/*!
  @brief
  Configure Tx PLL script for correcting the tx error in NBIOT scenario
...
{
  rfm_get_txpll_script(mode, rx_freq_error_in_hz, txpll_script_buf_ptr);
} /* rfm_get_txpll_script */
...
/*!
  @brief
  Set the ul_carrier_freq_offset for ML1 to debug the callbox issue. This will be deleted after the callbox issue is resolved.
 
  @details
  Set the ul_carrier_freq_offset for ML1 to debug the callbox issue. This will be deleted after the callbox issue is resolved.

  @param ul_carrier_freq_offset 
  ul carrier freq offset (M_UL) value
  
*/
void rfm_nb_set_ul_carrier_freq_offset( int8 ul_carrier_freq_offset)
{
#ifndef FEATURE_RF_LTE_PAGE_ONLY
    rflte_state_set_ul_carrier_freq_offset(RFLTE_MC_PCELL_IDX, ul_carrier_freq_offset);
#endif
}
...
/*!
  @brief
  Get the max num of channel in one band search request in NBIOT scenario
 
  @details
  Get the max num of channel in one band search request in NBIOT scenario

  @return 
  the max num of channel in one band search request

  
*/
uint8 rfm_nb_get_fs_num_chan_capability(rfcom_lte_earfcn_type start_chan, boolean *end_adc_rate)
{
  return rflte_mc_rx_nb_get_fs_num_chan_capability(start_chan, end_adc_rate);
}
...
boolean rfm_get_thermistor_count(uint8* size_p)
{
  ...
  if(rfdevice_get_thermistor_count(size_p) == FALSE)
  {
    RF_MSG( RF_MED, "rfdevice_get_active_thermistor_size() failed. Returning FALSE");
    return FALSE;
  }

  if(therm_mitigation_debug_flag == TRUE)
  {
    RF_MSG_1( RF_HIGH, "rfdevice_get_active_thermistor_size(): thermistor_count:%d", *size_p);        
  }
...
boolean rfm_enumerate_thermistors(uint8* size_p, rfm_thermistor_type* therm_device_list_p)
{
  ...
  if(therm_mitigation_debug_flag == TRUE)
  {
    RF_MSG_1(RF_HIGH, "rfm_enumerate_thermistors(): therm_handle:%d", therm_device_list_p->handle);        
  }
...
/*!
  ...
  @param therm_device_list_p : in+out parameter to read the thermistor.

*/

boolean rfm_read_thermistor(rfm_thermistor_type* therm_device_info_p)
{
  ...
  /*Check if rfm_init is complete*/
  if (rfm_init_status == FALSE)
  {
   RF_MSG_1( RF_ERROR, "rfm_read_thermistor():  rfm_init_status %d is not successfull. Returning FALSE",rfm_init_status);
    return FALSE;
  }


  /*NULL pointer check*/
 if (therm_device_info_p == NULL)
  {
   RF_MSG( RF_ERROR, "rfm_read_thermistor():  null pointer detected. Returning FALSE");
    return FALSE;
  }

  status &= rf_hal_bus_enable(TRUE, &rf_rfm_hal_bus_client_handle, RF_HAL_CLIENT_COMMON);
  
  if (FALSE == status )
  {
    RF_MSG( RF_ERROR, "rfm_read_thermistor():  unable to enable HAL BUS. Returning FALSE");
	return FALSE;
  }
  else
  {
    iotfe_debug_info.client_handle = rf_rfm_hal_bus_client_handle;
    iotfe_debug_info.therm_mitigation_hal_vote_count++;
  }


  /*call the device interface api to query temperature of the requested thermistor*/
  if((rfdevice_read_thermistor(therm_device_info_p)) == FALSE)
  {
    RF_MSG( RF_HIGH, "rfm_read_thermistor(): API rfdevice_read_thermistor() failed. Returning FALSE");
    status = FALSE;
  }

  status &= rf_hal_bus_enable(FALSE, &rf_rfm_hal_bus_client_handle, RF_HAL_CLIENT_COMMON);

  if (FALSE == status )
  {
    RF_MSG( RF_ERROR, "rfm_read_thermistor():  unable to disable HAL BUS. Returning FALSE");
  }
  else
  {
    iotfe_debug_info.client_handle = rf_rfm_hal_bus_client_handle;
...
boolean rfm_query_thermistor(rfm_thermistor_handle_type therm_handle, rfm_thermistor_result_type* therm_result_p)
{
 ...
 // {
    RF_MSG( RF_ERROR, "rfdevice_query_thermistor() Not implemented ");
  //  return FALSE;
  //}
...
{
  uint8 dev_id =0;
  rfdevice_trx_common_intf_class *temp_cmn_dev_ptr = NULL;
...
void rfm_set_1rx_mode(boolean enable_1rx)
{
  RF_MSG_1(RF_HIGH, "rfm_set_1rx_mode: LTE 1Rx mode set to %d", enable_1rx);
}
#endif
...
/*!
  ...
  @details
  This function will be called by LTE L1 to update HD FDD flag 

  @param is_hd_fdd
  Boolean to set is_hd_fdd to true or false  
  
*/
void rfm_lte_set_hd_fdd_flag(boolean is_hd_fdd)
{
  rflte_state_set_hd_fdd_mode(is_hd_fdd);
  
  RF_MSG_1(RF_HIGH, "rfm_lte_set_hd_fdd_flag: HD FDD mode is set to %d", is_hd_fdd);
}
...
/*!
  ...
  @details
  This function will be called by LTE L1 to update LTE NB flag. 
  Will not be used currently, the flag will be set in nb_enter_mode req in RFSW internally. Will be deleted later after final decision. 

  @param is_lte_nb
  Boolean to set is_lte_nb to true or false  
  
*/
void rfm_lte_set_lte_nb_flag(boolean is_lte_nb)
{
  rflte_state_set_lte_nb_mode(is_lte_nb);
  
  RF_MSG_1(RF_HIGH, "rfm_lte_set_lte_nb_flag: LTE NB mode is set to %d", is_lte_nb);
}

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Update HD FDD flag in NBIOT mode 
...
*/
void rfm_lte_nb_set_hd_fdd_flag(boolean is_hd_fdd)
{
  rflte_state_set_hd_fdd_mode(is_hd_fdd);
  
  RF_MSG_1(RF_HIGH, "rfm_lte_nb_set_hd_fdd_flag: HD FDD mode is set to %d", is_hd_fdd);
}
...
/*
  @brief
  The API is used to update the MAAT parameters from upper layers,to be used for running
  the MAAT algorithm

  @details

  @param
  ptr to rfm_maat_params_type structure

  @return
  Returns TRUE or FALSE represents PASS/FAILURE respectively.
...
)
{
  ...
  if (maat_params == NULL)
  {
    RF_MSG( MSG_LEGACY_ERROR,"rfm_update_maat_info(): maat_params is NULL !!!" );
    status = FALSE;
  }
  else
  {
    /* Put the command in rf app  task */
    rf_apps_cmd_type *maat_update_cmd = NULL;

    /* Get the pointer to the free buffer of RF command queue. */
    if ( ( maat_update_cmd = rf_apps_get_buf() ) == NULL )
    {
      RF_MSG( RF_ERROR, "rfm_update_maat_info: RF command queue is full!" );
      status = FALSE;
      return status;
    }

    maat_update_cmd->msgr_hdr.id = RFCMN_CORE_MAAT_L1_UPDATE_CMD;
    maat_update_cmd->rex_hdr.cmd_hdr.tech_id = RF_APSS_MAAT_ID;

    memscpy(&maat_update_cmd->payload[0],sizeof(maat_update_cmd->payload),
            maat_params,sizeof(rfm_maat_params_type));    
    
    rf_apps_cmd_put( maat_update_cmd );

    if(rfcmn_core_maat_mgr_debug_flag)
    {
      RF_MSG_1( RF_HIGH,"MAAT_DEBUG: rfm_update_maat_info() is called. status:%d",status);
    }
  }
  return status;
#else
