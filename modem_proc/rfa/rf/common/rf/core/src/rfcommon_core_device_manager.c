  ...
  { 
    ...
    else
    {
      ...
      case RFCMN_CORE_DEV_ON:
                    ...
                    (rfcmn_core_mgr.gps_dev_mgr[cntr]->dev_id != RFCMN_CORE_MAX_PHY_DEVICES))
                {
                  ...
                  if (rfcmn_core_mgr.is_wwan_vote_on == FALSE)
                  {
                    rfcmn_core_mgr.gps_dev_mgr[cntr]->dev_status = 
                      (rfdevice_cmn_power_up(rfcmn_core_mgr.gps_dev_mgr[cntr]->dev,
                  ...
                  {
                    /* WWAN + GPS concurrency is not allowed */
		    RF_MSG(RF_ERROR,"WWAN is already ON. GPS cannot power up the device!!"); 
                    status = FALSE;
                  }
                }
                else
          ...
          else
          {
                 ...
                 dev_mgr_index++ )
            {
              ...
              if (rfcmn_core_mgr.is_wwan_vote_on == FALSE)
              {
                ...
                else /* GPS is already ON */
                {
                  /* WWAN + GPS concurrency is not supported */
                  RF_MSG(RF_ERROR,"GPS is already ON. WWAN cannot power up the device!!");  
                  status = FALSE;
                }
              }
              else
        ...
        case RFCMN_CORE_DEV_NON_CRITICAL_VOTE: /* not supported for SDR105 */
        default:
          RF_MSG_3(RF_ERROR,
...
boolean rfcommon_core_dev_mgr_init()
{
  boolean status = TRUE; 
  uint8 dev_id;
  rfdevice_trx_common_intf_class* temp_cmn_dev_param = NULL;
...

{
  ...
  */
  ...
  else /* (rfm_dev_val == rfcmn_core_mgr.rfm_dev_for_WWAN) */
  {
    if (rfcmn_core_mgr.is_wwan_vote_on == FALSE)
    {
          ...
          ++dev_index)
    {
      ...
      for (cntr = 0; cntr < rfcmn_core_mgr.phy_dev_count; ++cntr)
      {
        if (rfcmn_core_mgr.gps_dev_mgr[cntr] != NULL)
        {
          ...
          2) If no valid GPS vote present */
      ...
      if (transceiver_turn_off == TRUE)
      {
        ...
        /* Clear the PA therm template preload status flag */
        rfdevice_cmn_reset_iotfe_template_preload_flag(rfcmn_core_mgr.dev_mgr[dev_index].dev);
      }
    }
  }
