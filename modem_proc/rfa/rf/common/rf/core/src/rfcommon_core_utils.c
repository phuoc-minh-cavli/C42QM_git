...
$Header: //components/rel/rfa.mpss/3.10/rf/common/rf/core/src/rfcommon_core_utils.c#2 $
...
03/08/19   gk      B31 CatM1/NB1 enablement
...
05/10/16   vs      Fix the issue with W/T featurization
...
extern rfcom_lte_band_type rfcommon_core_convert_lte_band_num_to_rfcom_band( uint16 band_num);
...
int rfcommon_core_convert_sys_band_to_rfcom_band(sys_band_class_e_type band)
{
  ...
  switch(band)
  {
   ...
   case SYS_BAND_LTE_EUTRAN_BAND31:
      rf_card_band = RFCOM_BAND_LTE_B31;
      break; 
    ...
    case SYS_BAND_LTE_EUTRAN_BAND71:
      rf_card_band = RFCOM_BAND_LTE_B71;
      break;

    case SYS_BAND_LTE_EUTRAN_BAND72:
      rf_card_band = RFCOM_BAND_LTE_B72;
      break;

    case SYS_BAND_LTE_EUTRAN_BAND73:
      rf_card_band = RFCOM_BAND_LTE_B73;
      break;

    case SYS_BAND_LTE_EUTRAN_BAND85:
      rf_card_band = RFCOM_BAND_LTE_B85;
      break;

	case SYS_BAND_LTE_EUTRAN_BAND86:
      rf_card_band = RFCOM_BAND_LTE_B86;
      break;
...
sys_band_class_e_type rfcommon_core_convert_rfcom_band_to_sys_band(rfm_mode_enum_type tech, rfcom_band_type_u band)
{
  ...
  case RFM_LTE_FTM_MODE:
    ...
    case RFCOM_BAND_LTE_B71:
      sys_band = SYS_BAND_LTE_EUTRAN_BAND71;
      break;
    case RFCOM_BAND_LTE_B85:
      sys_band = SYS_BAND_LTE_EUTRAN_BAND85;
      break;
	case RFCOM_BAND_LTE_B86:
      sys_band = SYS_BAND_LTE_EUTRAN_BAND86;
      break;
...
*/
rfdevice_trx_common_intf_class* rfcommon_core_get_tx_device_ptr
(
...
{
  rfdevice_trx_common_intf_class* dev_ptr = NULL; 
...
/*!
  @brief
  This function is a helper function to convert band number to rfcom band.
  ...
  @param mode
  Input Param: Technology mode

  @param band_num
  Input param: band number to be converted

  @param *band
  Output param: rfcom band

  @return
  boolean: TRUE if API passed

*/
boolean rfcommon_core_convert_band_num_to_rfcom_band
(
...
)
{
  ...
  switch(mode)
  {
#ifdef FEATURE_LTE
    case RFM_LTE_MODE:
      band->lte_band = rfcommon_core_convert_lte_band_num_to_rfcom_band(band_num);
      if(band->lte_band >= RFCOM_NUM_LTE_BANDS)
      {
        status = FALSE;
        MSG_SPRINTF_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Unsupported band:%d",band_num);
      }
      break;
#endif
    ...
    default:
      status = FALSE;
      MSG_SPRINTF_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid mode:%d",mode);
      break;
  }

  return status;
}/*rfcommon_core_convert_band_num_to_rfcom_band*/
