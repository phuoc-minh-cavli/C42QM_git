...
$Header: //components/rel/rfa.mpss/3.10/rf/common/rf/core/src/rfcommon_core_txlin.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/18/2021 gk      F3 reduction( during Tx tune)
04/02/19   sk      Increase linearizers to support new bands
03/12/19   gk      Calibration fix for GRFC PA
03/05/19   sk      RF PAGE ONLY changes for NB/CATM
12/27/18   sk      LTE Heap to BSS migration
12/12/18   gk      CW fixes
07/16/18   sk      Pass LTE CAT type to PA drivers
06/11/18   sk      OFT Fixes
02/16/18   sk      [Phase2] Chiltepin LTE memory optimization
10/16/17   cv      Increasing the channel variable size 
10/16/17   gk      KW error fix
...
tx_linearizer_table_type tx_nv_linearizer_buffers[NV_TX_LIN_BUFFERS] = {0};

uint8 tx_nv_lin_buf_count = 0;
...
typedef struct 
{
  ...
  rf_buffer_intf* fbrx_lut_buffer[4];
...
#ifdef FEATURE_LTE
extern boolean rflte_state_get_lte_nb_mode(void);
extern void rflte_state_set_lte_nb_mode(boolean lte_nb_mode);

extern boolean rflte_state_get_hd_fdd_mode(void);

extern boolean rfdevice_lte_pa_is_internal_pa
        (
           rfm_device_enum_type device,
           rfcom_lte_band_type rf_band
        ); 

extern boolean rfdevice_lte_tx_get_rgi0_trigger_script
        (
    ...
    { 
      ...
      { 
        ...
        /*  modem_mem_free( tx_band_data->tx_pa_state_cal_data[ pa_idx ].
          ...
          memset( tx_band_data->tx_pa_state_cal_data[ pa_idx ].linearizer_index[ wave_idx ][ chan_idx ].node_data.lin_data,
                  0, sizeof(tx_linearizer_table_type) );
          
          if (tx_nv_lin_buf_count > 0)
          {
            tx_nv_lin_buf_count --;
          }
...
)
{
  ...
  **********************************************************************/
  ...
  if (lin_node_nv_index->upper_bound_chan!=0xFFFF)
  {
    ...
    lin_table_ptr = &tx_nv_linearizer_buffers[tx_nv_lin_buf_count%NV_TX_LIN_BUFFERS];
    tx_nv_lin_buf_count++;
...
)
{
  ...
  tx_linearizer_table_type *interim_tx_lut;
  ...
  else if ( lin_rev_used == TX_MULTI_LIN_REV_3 )
  {
    ...
    to the Pin compensation algorithm */
      status &= rfcommon_core_txlin_pin_comp( interim_tx_lut,
                                            output_tx_lut,
                                            first_sweep_lut,
                                            pin_comp_val,
                                            valid_lut_entries,
...
)
{
  ...
  for ( dev_count = 0 ; dev_count < RFCOM_MAX_TX_DEVICES_SUPPORTED ; dev_count++ )
  {
    ...
    txagc_buf[RFCMN_TXLIN_AGC_WTR_GW_TASK] = 
                      rf_hal_buffer_create( RFCMN_TXLIN_AGC_WTR_DATA_TASK_NUM_TRANS, 
                                        RFCMN_TXLIN_AGC_WTR_DATA_TASK_NUM_TRANS, 
                                            0,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS*RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_REGS );
    txagc_buf[RFCMN_TXLIN_AGC_WXE_BBF_TASK] = 
                      rf_hal_buffer_create( 0, 
                                        RFCMN_TXLIN_AGC_WXE_BBF_TASK_NUM_TRANS, 
                                            0,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS*RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_REGS );
    txagc_buf[RFCMN_TXLIN_AGC_WTR_EARLY_TRIG_TASK] = 
                      rf_hal_buffer_create( RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS, 
                                        RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS, 
                                            0,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS*RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_REGS );
    txagc_buf[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK] = 
                      rf_hal_buffer_create( RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS, 
                                            RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS, 
                                            0,
                                            RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
    ...
    tx_lin_info[dev_count].dummy_trig_buffer = rf_hal_buffer_create( 
                                          RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS,
                                          RFCMN_TXLIN_AGC_WTR_TRIG_TASK_NUM_TRANS, 
                                          0,
                                          RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
  ...
  memset( &tx_nv_linearizer_buffers[0], 0, sizeof(tx_linearizer_table_type)*NV_TX_LIN_BUFFERS );
  tx_nv_lin_buf_count = 0;
...
)
{
  ...
  uint8 wtr_group_count;
  const rf_hal_bus_resource_script_type *rffe_script_ptr = NULL;
  const rf_hal_bus_resource_script_type *ssbi_script_ptr = NULL;
  const rf_hal_bus_resource_script_type *dummy_rffe_script_ptr = NULL;
  rfdevice_trx_lte_tx_path_info_type path_info = {RFCOM_BAND_LTE_INVALID, 0,RFCOM_BW_LTE_INVALID, RFCOM_LTE};
  rfcom_lte_cat_type lte_cat = RFCOM_LTE;
  ...
  rf_buffer_intf* dummy_rgi_trig = NULL;

  rf_buffer_intf** txagc_buf = rfcommon_core_txlin_get_tx_agc_buffer ( device ) ;

  dummy_rgi_trig = rf_hal_buffer_create( RFCMN_TXLIN_AGC_WTR_DATA_TASK_NUM_TRANS, 
                                         RFCMN_TXLIN_AGC_WTR_DATA_TASK_NUM_TRANS, 
                                         0,
                                         RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS,
                                         RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_GROUPS*RFCMN_TXLIN_AGC_MAX_RFFE_GROUP_NUM_REGS );
  ...
  else
  {
    ...
    if ((rfm_mode == RFM_LTE_MODE) || (rfm_mode == RFM_LTE_FTM_MODE))
    {
      /*Determine LTE Category (LTE/CATM1/NB1) before getting the script */
      if (rflte_state_get_lte_nb_mode())
      {
        lte_cat = RFCOM_LTE_NB1;
      }
      else if(rflte_state_get_hd_fdd_mode())
      {
        lte_cat = RFCOM_LTE_CATM1;
      }
      
      path_info.freq_khz = freq_khz;
      path_info.rf_band = rfm_band.lte_band;
      path_info.rf_bw = bw_u.lte_bw;
      path_info.lte_cat = lte_cat;      
    }          
    ...
    for ( lin_index = 0; lin_index < num_queries; lin_index++ )
    {
      ...
      if ( full_bias_rgi != RFCOM_TXLIN_INVALID_RGI )
      {
        ...
        for (wxe_index = 0; wxe_index < RFCOMMON_MDSP_TXLIN_WXE_BUF_SIZE; wxe_index++)
        {
               ...
               lut_data.trigger_script != NULL )
        {
          ...
          /* Query LUT data */
            ret_val &= rfdevice_cmn_get_tx_lut_scripts( wtr_device,&lut_data,
                                  device, rfm_mode, 
                                  &path_info,
                                  RFDEVICE_CREATE_SCRIPT,
                                  (rfdevice_trx_txagc_template_type)wxe_index,
                                  pa_state,
                                  full_bias_rgi,
                                  TRUE);            
          ...
          iref_uamp_x10 = (uint16)lut_data.iref_uamp_x10;
            ...
            rffe_script_ptr = rf_buffer_get_script_ptr( lut_data.bbf_script,
                                                        RF_HAL_BUS_RESOURCE_RFFE_GROUP );

            if ( rffe_script_ptr != NULL && 
                 rffe_script_ptr->script_ptr.rffe_groups != NULL )
            {
              num_groups = rffe_script_ptr->num_rffe_groups;
              tx_front_end_addr->wxe_config_addr[wxe_index].num_non_trig_groups = num_groups;

           /*   RF_MSG_2( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): lin_idx : %d "
                                       "WXE BBF non trig num_groups %d", lin_index, num_groups ) ;*/

              for ( group_count = 0; group_count < num_groups; group_count++, wtr_group_count++)
          {
                for ( data_count = 0; 
                      data_count < rffe_script_ptr->
                                           script_ptr.rffe_groups[group_count].rffe_addr.num_bytes;
                      data_count++)
            {
                  tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].non_trig[wtr_group_count][data_count] =
                    rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.data[data_count];

              /*    RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                         "group buf_idx : %d, data_count : %d, data : %d", group_count, wtr_group_count,
                                        data_count, tx_lin_data->wtr_wxe_config.data[lin_index].non_trig[wtr_group_count][data_count] ) ; */                                   
          }

                tx_front_end_addr->wxe_config_addr[wxe_index].non_trig[group_count] = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.addr;

                tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_numb_bytes[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.num_bytes;

                tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_num_tokens[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].num_tokens;      

              /*  RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                         "addr : %d, num_bytes : %d, num_tokens : %d", group_count, tx_front_end_addr->wxe_config_addr.non_trig[group_count],
                                         tx_front_end_addr->wxe_config_addr.non_trig_numb_bytes[group_count],
                                         tx_front_end_addr->wxe_config_addr.non_trig_num_tokens[group_count]) ; */                 

                for ( token_count = 0; 
                      token_count < rffe_script_ptr->
                                           script_ptr.rffe_groups[group_count].num_tokens;
                      token_count++)
              {
                  tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id;

                  tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id_start_byte = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_start_byte;

                  tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id_len = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_len;  

               /*   RF_MSG_5( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                       "token count : %d, token id : %d, token start byte : %d, token length : %d",
                                       group_count,token_count,  tx_front_end_addr->wxe_config_addr.non_trig_token[group_count][token_count].token_id,
                                       tx_front_end_addr->wxe_config_addr.non_trig_token[group_count][token_count].token_id_start_byte,
                                       tx_front_end_addr->wxe_config_addr.non_trig_token[group_count][token_count].token_id_len) ; */                                   
                }                
            }
          ...
          {
              num_groups = rffe_script_ptr->num_rffe_groups;
              tx_front_end_addr->wtr_config_addr[wxe_index].num_non_trig_groups = num_groups;

            /*  RF_MSG_2( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): lin_idx : %d "
                                       "WTR Gain non Trig num_groups %d", lin_index, num_groups ) ;*/
              ...
              for ( group_count = 0; group_count < num_groups; group_count++, wtr_group_count++)
            {
              ...
              /*    RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                ...
                tx_front_end_addr->wtr_config_addr[wxe_index].non_trig[group_count] = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.addr;

                tx_front_end_addr->wtr_config_addr[wxe_index].non_trig_numb_bytes[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.num_bytes;

                tx_front_end_addr->wtr_config_addr[wxe_index].non_trig_num_tokens[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].num_tokens;  

              /*  RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                         "addr : %d, num_bytes : %d, num_tokens : %d", group_count, tx_front_end_addr->wtr_config_addr.non_trig[group_count],
                                         tx_front_end_addr->wtr_config_addr.non_trig_numb_bytes[group_count],
                                         tx_front_end_addr->wtr_config_addr.non_trig_num_tokens[group_count]) ;  */                                
                ...
                for ( token_count = 0; 
                      ...
                      token_count++)
              {
                  tx_front_end_addr->wtr_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id;

                  tx_front_end_addr->wtr_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id_start_byte = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_start_byte;

                  tx_front_end_addr->wtr_config_addr[wxe_index].non_trig_token[group_count][token_count].token_id_len = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_len; 

               /*   RF_MSG_5( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                       "token count : %d, token id : %d, token start byte : %d, token length : %d",
                                       group_count,token_count, tx_front_end_addr->wtr_config_addr.non_trig_token[group_count][token_count].token_id,
                                       tx_front_end_addr->wtr_config_addr.non_trig_token[group_count][token_count].token_id_start_byte,
                                       tx_front_end_addr->wtr_config_addr.non_trig_token[group_count][token_count].token_id_len) ; */                   
                }                
            }
          } /* if ( rffe_script_ptr->script_ptr.rffe != NULL ) */
          ...
          {
              num_groups = rffe_script_ptr->num_rffe_groups;
              tx_front_end_addr->wtr_config_addr[wxe_index].num_trig_groups = num_groups;

          /*    RF_MSG_2( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): lin_idx : %d "
                                       "WTR trig num_groups %d", lin_index, num_groups ) ; */
              ...
              for ( group_count = 0; group_count < num_groups; group_count++, wtr_group_count++)
            {
              for ( data_count = 0; 
                    data_count < rffe_script_ptr->
                      ...
                      data_count++)
              {
                  tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].trig[wtr_group_count][data_count] =
                    rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.data[data_count];

              /*    RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                         "group buf_idx : %d, data_count : %d, data : %d", group_count, wtr_group_count,
                                        data_count, tx_lin_data->wtr_wxe_config.data[lin_index].trig[wtr_group_count][data_count]) ; */                                
              }

                tx_front_end_addr->wtr_config_addr[wxe_index].trig[group_count] = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.addr;

                tx_front_end_addr->wtr_config_addr[wxe_index].trigger_num_bytes[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.num_bytes;
                
                tx_front_end_addr->wtr_config_addr[wxe_index].trig_num_tokens[group_count] =
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].num_tokens;

             /*   RF_MSG_4( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                         "addr : %d, num_bytes : %d, num_tokens : %d", group_count, tx_front_end_addr->wtr_config_addr.trig[group_count],
                                         tx_front_end_addr->wtr_config_addr.trigger_num_bytes[group_count],
                                         tx_front_end_addr->wtr_config_addr.trig_num_tokens[group_count]) ;  */                
                ...
                for ( token_count = 0; 
                      ...
                      token_count++)
                {
                  tx_front_end_addr->wtr_config_addr[wxe_index].trig_token[group_count][token_count].token_id = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id;

                  tx_front_end_addr->wtr_config_addr[wxe_index].trig_token[group_count][token_count].token_id_start_byte = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_start_byte;

                  tx_front_end_addr->wtr_config_addr[wxe_index].trig_token[group_count][token_count].token_id_len = 
                  rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_len;   

               /*   RF_MSG_5( RF_ERROR , "rfcommon_core_txlin_get_tx_device_lut_data(): Group : %d "
                                       "token count : %d, token id : %d, token start byte : %d, token length : %d",
                                       group_count,token_count, tx_front_end_addr->wtr_config_addr.trig_token[group_count][token_count].token_id,
                                       tx_front_end_addr->wtr_config_addr.trig_token[group_count][token_count].token_id_start_byte,
                                       tx_front_end_addr->wtr_config_addr.trig_token[group_count][token_count].token_id_len) ;  */                                  
                }                       
            }
          } /* if ( rffe_script_ptr->script_ptr.rffe != NULL ) */

            /*#ifdef TEST_FRAMEWORK
            bbf_debug[9] = (uint8)pa_state + '0';
            tx_gain_debug[9] = (uint8)pa_state + '0';
            trigger_debug[9] = (uint8)pa_state + '0';
            
            bbf_debug[16] = full_bias_rgi/10 + '0';
            tx_gain_debug[16] = full_bias_rgi/10 + '0';
            trigger_debug[16] = full_bias_rgi/10 + '0';
            
            bbf_debug[17] = full_bias_rgi%10 + '0';
            tx_gain_debug[17] = full_bias_rgi%10 + '0';
            trigger_debug[17] = full_bias_rgi%10 + '0';
    
            bbf_debug[28] = (uint8)wxe_index + '0';
            tx_gain_debug[28] = (uint8)wxe_index + '0';
            trigger_debug[28] = (uint8)wxe_index + '0';
    
            lte_print_rffe_script((void*)lut_data.bbf_script, "RGI_DATA", bbf_debug , TRUE );
            lte_print_rffe_script((void*)lut_data.tx_gain_script, "RGI_DATA", tx_gain_debug , TRUE );
            lte_print_rffe_script((void*)lut_data.trigger_script, "RGI_DATA", trigger_debug , TRUE );
            #endif  */ 

          /* Iref index in LUT */
            tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].iref_index = 
                                  rfcommon_msm_get_iref_lut_index( iref_uamp_x10 );

           /* Provide Phase comp value in LUT */
            tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].phase = lut_data.phase;
          ...
          */
          if ( ( row_info[lin_index].rsb_corr_coeffs.alpha != 
               ...
               ( row_info[lin_index].rsb_corr_coeffs.beta != 
                 RFCOM_TXLIN_INVALID_RSB_VAL ) )
          {
              tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].
    ...
    if ( tx_lin_data->full_bias_lin_size > 1 )
    {
            ...
            lin_index++ )
      {
        ...
        /* Non-trigger */
          for ( group_count = 0; 
                group_count < RFCOMMON_MDSP_TXLIN_NON_TRIG_GROUP_SIZE;
                group_count++ )
          {
        for ( data_count = 0; 
                  data_count < RFCOMMON_MDSP_TXLIN_GROUP_DATA_SIZE;
              data_count++ )
        {
          /* WTR Config */
              tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].non_trig[group_count][data_count] =
                tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].non_trig[group_count][data_count];
            }
        }

        /* Trigger */
          for ( group_count = 0; 
                group_count < RFCOMMON_MDSP_TXLIN_TRIG_GROUP_SIZE;
                group_count++ )
          {
        for ( data_count = 0; 
                  data_count < RFCOMMON_MDSP_TXLIN_GROUP_DATA_SIZE;
        ...
        {
          /* WTR Config */
              tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].trig[group_count][data_count] =
                tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].trig[group_count][data_count];
            }
        }

        /* Iref index in LUT */
          tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].iref_index = 
                        tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].iref_index;

	/*Phase value in LUT */
    	    tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].phase = 
  		    tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].phase;

        /* Pad the remainder RSB coeffients with last values 
        corresponding to valid RGI entry */
          tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].wtr_iq_adj_params.rsb_coeff_alpha = 
            tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].wtr_iq_adj_params.rsb_coeff_alpha ;
          tx_lin_data->wtr_wxe_config[wxe_index].data[lin_index].wtr_iq_adj_params.rsb_coeff_beta =  
            tx_lin_data->wtr_wxe_config[wxe_index].data[last_valid_index].wtr_iq_adj_params.rsb_coeff_beta ;
        }
      }
    }
   ...
   if ((rfm_mode == RFM_LTE_MODE) || (rfm_mode == RFM_LTE_FTM_MODE))
   {
      if (rfdevice_lte_pa_is_internal_pa(device,rfm_band.lte_band))
      {
        rf_buffer_clear( txagc_buf[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK] );
        
        /*For internal PA populate RGI0 buffer*/        
        ret_val &= rfdevice_lte_tx_get_rgi0_trigger_script(
                                        device,
                                        &path_info,
                                        txagc_buf[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK],
                                        RFDEVICE_CREATE_SCRIPT,
        ...
        ret_val &=  rfdevice_cmn_get_tx_dummy_script_trigger(wtr_device,dummy_rgi_trig);                                       
         
        /* ----------------------------- WTR RGI0 Config -------------------------- */
        rffe_script_ptr = rf_buffer_get_script_ptr( txagc_buf[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK],
                                                    RF_HAL_BUS_RESOURCE_RFFE_GROUP );

        dummy_rffe_script_ptr = rf_buffer_get_script_ptr( dummy_rgi_trig,
                                                    RF_HAL_BUS_RESOURCE_RFFE_GROUP );
        ...
        {
          num_groups = rffe_script_ptr->num_rffe_groups;
          tx_front_end_addr->wtr_rgi0_config_addr.num_groups = num_groups;
        
          for ( group_count = 0; group_count < num_groups; group_count++)
          {
            tx_front_end_addr->wtr_rgi0_config_addr.addr[group_count] = 
              rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.addr;

            tx_front_end_addr->wtr_rgi0_config_addr.num_bytes[group_count] =
              rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.num_bytes;

            tx_front_end_addr->wtr_rgi0_config_addr.num_tokens[group_count] =
              rffe_script_ptr->script_ptr.rffe_groups[group_count].num_tokens;   
            ...
            for ( data_count = 0; 
                  ...
                  data_count++)
            {
              tx_lin_data->wtr_rgi0_config.trig[group_count][data_count] =
                rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_addr.data[data_count];
            }                
            ...
            for ( token_count = 0; 
                  ...
                  token_count++)
            {
              tx_front_end_addr->wtr_rgi0_config_addr.token[group_count][token_count].token_id = 
              rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id;

              tx_front_end_addr->wtr_rgi0_config_addr.token[group_count][token_count].token_id_start_byte = 
              rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_start_byte;

              tx_front_end_addr->wtr_rgi0_config_addr.token[group_count][token_count].token_id_len = 
              rffe_script_ptr->script_ptr.rffe_groups[group_count].rffe_token[token_count].token_id_len;  
            }                
          }

          if (dummy_rffe_script_ptr!= NULL && 
              dummy_rffe_script_ptr->script_ptr.rffe_groups != NULL)
          {
            tx_front_end_addr->wtr_rgi0_config_addr.dummy =
              *((uint16*)dummy_rffe_script_ptr->script_ptr.rffe_groups[0].rffe_addr.data);
          }
        } /* if ( rffe_script_ptr->script_ptr.rffe != NULL ) */
      }
    }
  ...
  if (dummy_rgi_trig != NULL)
  {
    rf_buffer_destroy ( dummy_rgi_trig ); 
  }
  ...
  else
  {
    ...
    for ( lin_index = 0; lin_index < num_queries; lin_index++ )
    {
      ...
      if ( txagc_buf[RFCMN_TXLIN_AGC_PA_TASK] != NULL )
      {
        ...
        pa_params_s.trigger_type = RFDEVICE_PA_TRIGGER_NONE;
        pa_params_s.pa_gain_range = pa_range;
        pa_params_s.rfm_device = device;
        if ((rfm_mode == RFM_LTE_MODE) || (rfm_mode == RFM_LTE_FTM_MODE))
        {
          /*Determine LTE Category (LTE/CATM1/NB1) before getting the script */
          if (TRUE == rflte_state_get_lte_nb_mode())
          {
            lte_cat = RFCOM_LTE_NB1;
          }
          else if(TRUE == rflte_state_get_hd_fdd_mode())
          {
            lte_cat = RFCOM_LTE_CATM1;
          }
          pa_params_s.lte_cat = lte_cat;
        }  
	    ...
	    tx_lin_data->pa_config.data[lin_index].grfc[1] |= 
	     ...
	     /* Lin table GRFC[0] is used as bit mask for set task */
	    if ( grfc_script_ptr->script_ptr.grfc[trans_count].start_logic == 1)
	    {
	       tx_lin_data->pa_config.data[lin_index].grfc[0] |= 
		       (1 << grfc_script_ptr->script_ptr.grfc[trans_count].grfc_id);
	    }
        ...
        ret_val &= rfdevice_pa_gain_range_trigger( &pa_params_s, pa_device, 
                                        txagc_buf[RFCMN_TXLIN_AGC_PA_TASK],
                                        RFDEVICE_CREATE_SCRIPT, 
                                        RFCOM_INVALID_TIMING_OFFSET );    
             ...
             rffe_script_ptr->script_ptr.rffe != NULL )
        {
          ...
          for ( trans_count = 0; trans_count < num_trans; trans_count++ )
          {
            ...
            tx_front_end_addr->pa_config_addr[pa_state].
                                             trig_start_delta[trans_count] =
              rffe_script_ptr->script_ptr.rffe[trans_count].start_delta;
          }
        } /* if ( rffe_script_ptr->script_ptr.rffe != NULL ) */                                        
...
)
{
  ...
  trig_times[RFCMN_TXLIN_AGC_PAPM_LATE_TRIG_TASK] = 
    ...
    (rf_buffer_get_num_trans( tx_agc_buffer[RFCMN_TXLIN_AGC_PAPM_LATE_TRIG_TASK],
                              RF_BUFFER_RFFE_GROUP )*
                              rffe_trans_time_x10);
  
  /* WTR RGI 0 task */
  trig_times[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK] = 
    timing_params->wtr_rgi0_offset - 
    (rf_buffer_get_num_trans( tx_agc_buffer[RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK],
                              RF_BUFFER_RFFE )*
                              rffe_trans_time_x10);
...
)
{
  ...
  if ( dummy_trig_buf != NULL )
  {
    ...
    ret_val &= 
      rfdevice_cmn_get_tx_dummy_script_trigger( wtr_device, dummy_trig_buf ); 

    rffe_script_ptr = 
      rf_buffer_get_script_ptr( dummy_trig_buf, RF_HAL_BUS_RESOURCE_RFFE_GROUP );

    if ( rffe_script_ptr != NULL && 
         rffe_script_ptr->script_ptr.rffe_groups != NULL )
    {
      num_groups = rffe_script_ptr->num_rffe_groups;
      for ( trans_count = 0; trans_count < num_groups; trans_count++ )
      {
        if ( trans_count > 1 )
        {
        ...
        else
        {
          for (wxe_index = 0; wxe_index < RFCOMMON_MDSP_TXLIN_WXE_BUF_SIZE; wxe_index ++)
          {
            tx_front_end_addr->wtr_config_addr[wxe_index].dummy =
              rffe_script_ptr->script_ptr.rffe_groups[0].rffe_addr.addr;
            tx_front_end_addr->wxe_config_addr[wxe_index].dummy =
              rffe_script_ptr->script_ptr.rffe_groups[0].rffe_addr.addr;
            tx_front_end_addr->wtr_rgi0_config_addr.dummy =
              *((uint16*)rffe_script_ptr->script_ptr.rffe_groups[0].rffe_addr.data);            
          }
        }
      }
    }
    ...
    if ( txagc_buf[RFCMN_TXLIN_AGC_WXE_BBF_TASK] != NULL )
    {
           ...
           rffe_script_ptr->script_ptr.rffe_groups != NULL )
      {
        for (wxe_index = 0; wxe_index < RFCOMMON_MDSP_TXLIN_WXE_BUF_SIZE; wxe_index ++)
        {
          ...
          for ( trans_count = 0; trans_count < num_groups; trans_count++ )
          {
            tx_front_end_addr->wxe_config_addr[wxe_index].non_trig[trans_count] = 
              rffe_script_ptr->script_ptr.rffe_groups[trans_count].rffe_addr.addr;

            tx_front_end_addr->wxe_config_addr[wxe_index].non_trig_numb_bytes[trans_count] =
              rffe_script_ptr->script_ptr.rffe_groups[trans_count].rffe_addr.num_bytes;
          }
        }
      } /* if ( rffe_script_ptr->script_ptr.rffe != NULL ) */
    }
  ...
  if ( pa_trig_buf != NULL )
  {
    ...
    pa_params_s.bw = RFDEVICE_PA_BW_NOT_APPLICABLE;
    pa_params_s.trigger_type = RFDEVICE_PA_TRIGGER_TXAGC;
    pa_params_s.pa_gain_range = RFM_PA_R1_R0_0_0;
    pa_params_s.rfm_device = device;
    if ((rfm_mode == RFM_LTE_MODE) || (rfm_mode == RFM_LTE_FTM_MODE))
    {
      /*Determine LTE Category (LTE/CATM1/NB1) before getting the script */
...
)
{
  ...
  rfdevice_trx_lte_tx_path_info_type path_info = {RFCOM_BAND_LTE_INVALID, 0,RFCOM_BW_LTE_INVALID, RFCOM_LTE};
  rfcom_lte_cat_type lte_cat = RFCOM_LTE; 
  rfdevice_trx_lte_tx_path_info_type tx_path_info = {RFCOM_BAND_LTE_INVALID, 0,RFCOM_BW_LTE_INVALID, RFCOM_LTE};
  rfdevice_trx_timer_fixup_type trx_timer_fixup_array[RFDEVICE_TRX_MAX_TIMER_FIXUPS_PER_SCRIPT];
  uint8 timer_fixup_arr_size = 0;
  ...
  boolean force_new_task[RFCMN_TXLIN_AGC_MAX_TASK] =
  {
    ...
    FALSE,  /*RFCMN_TXLIN_AGC_WTR_RGI0_TRIG_TASK*/    
  ...
  else
  {
    ...
    if ( pa_device != NULL )
    {
     ...
     /*Dummy delta is passed for PA to ensure the sequence of TASK construction as 
       PA trigger scripts might have multiple start delta in scripts*/
      ret_val &= rfdevice_set_pa_txagc( &pa_params_s, &pa_txagc_s , pa_device, 
                                        txagc_buf[RFCMN_TXLIN_AGC_PA_TASK],
                                        RFDEVICE_CREATE_SCRIPT, 
                                        RFLM_TXAGC_PA_NON_TRIG_DUMMY_DELTA );

      pa_params_s.trigger_type = RFDEVICE_PA_TRIGGER_TXAGC;      
      ret_val &= rfdevice_pa_gain_range_trigger( &pa_params_s, pa_device, 
                                        txagc_buf[RFCMN_TXLIN_AGC_PA_TASK],
                                        RFDEVICE_CREATE_SCRIPT, 
                                        RFLM_TXAGC_PA_TRIG_DUMMY_DELTA );
      
      if (((rfm_mode == RFM_LTE_MODE) || (rfm_mode == RFM_LTE_FTM_MODE)) &&
           (TRUE == rfdevice_pa_is_internal_pa(pa_device)))
      {
        /*For internal PA populate RGI0 buffer*/
        tx_path_info.rf_band = rfm_band.lte_band;
        tx_path_info.freq_khz = freq_khz;
        tx_path_info.rf_bw = bw_u.lte_bw;
        tx_path_info.lte_cat = lte_cat;
    ...
    if ( wtr_device != NULL )
    {
      ...
      /* Query LUT data */
      ret_val &= rfdevice_cmn_get_tx_lut_scripts( wtr_device, &lut_data, 
                                                  device, rfm_mode, &path_info,
                                                  RFDEVICE_CREATE_SCRIPT,RFDEVICE_TRX_TXAGC_WXE_BUF_0,
                                                  RFM_PA_GAIN_STATE_0,0,TRUE );
     ...
     /* ret_val &= rfdevice_cmn_get_fbrx_enable_script( 
      ...
      if ( rffe_script_ptr != NULL && 
           rffe_script_ptr->script_ptr.rffe_groups != NULL &&
           rffe_script_ptr->num_rffe_groups > 0 )
      {
...
)
{
        ...
        lin_index++ )
  {
    ...
    if (( lin_index < RFCOMMON_MDSP_TXLIN_EPT_TBL_SIZE &&
         xpt_mode_type != RFCOMMON_MDSP_XPT_MODE_APT )&& (last_valid_index < RFCOMMON_MDSP_TXLIN_EPT_TBL_SIZE) )
    {
...
)
{
  ...
  if ( ret_val == TRUE )
  {
    RF_MSG_2( RF_MED, "rfcommon_core_txlin_update_linearizer: Configured for "
