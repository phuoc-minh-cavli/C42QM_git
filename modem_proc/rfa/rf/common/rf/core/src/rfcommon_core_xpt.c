...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/common/rf/core/src/rfcommon_core_xpt.c#1 $
...
)
{
  ...
  case RFM_LTE_FTM_MODE:
    #ifdef FEATURE_LTE
    bw_u.lte_bw = rflte_ftm_mc_sm_get_tx_bw();
    freq_khz = rflte_core_get_tx_freq_with_offset_from_uarfcn(rflte_ftm_mc_sm_get_tx_chan(),
                                                              rfm_band.lte_band);
...
)
{
  ...
  //api_status &= rflm_txagc_modem_xpt_load_dpd_tbls( &load_dpd_cfg, &index, &offset );
