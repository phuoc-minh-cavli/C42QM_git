...
/*===========================================================================
Copyright (c) 1999 - 2020  by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$DateTime: 2020/10/23 11:08:08 $ $Author: mplcsds1 $
$Header: //components/rel/rfa.mpss/3.10/rf/common/rf/nv/inc/rfcommon_nv_mm.h#2 $
...
#define RFC_NV_RF_CARD_CONFIG_SIZE_MAX  (  4 /*4 bytes rfc nv version*/  + 4 /*4 bytes for hardware_id*/  + 4 /*4 bytes for feature_set_id*/ + 1000 /*1000 bytes reserved*/ )


typedef PACK(struct){
   uint32 rfc_nv_version;
   uint32 rfc_hardware_id;
   uint32 rfc_feature_set_id;
   uint32 rfc_hw_version;
}rfc_nv_rf_card_config_type;



/*Common rfc NV structure to hold all rfc NVs*/
typedef struct{

 /*rf_card_config nv structure to hold the hwid , 
   hwid_version & feature id read from EFS*/
 rfc_nv_rf_card_config_type rfc_nv_rf_card_config;
...
typedef struct
{
  ...
  *   idx[3]: bands 193-256
  */
  uint64 rf_lte_policyman_band_mask[4];
...
/*!
  ...
  @details
  This function will be called at bootup to all read the rfc related NV items(e,g hardware_id , hardware_id) from EFS 
...
/*!
  @brief
  API to read a specific rfc nv data

  @details
  This function will be called at bootup to read read all the rfc related NV items(e,g hardware_id , hardware_id) from EFS 
