...
Program: FTM Sequencer
  ...
  $Id: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_sequencer.c#1 $
			...
			)
{
	...
	// Return value
	FTM_Sequencer_ErrorCode  _eErrorCode = FTM_Sequencer_Error_None;
        unsigned short _iFullMessageLength;
        unsigned short _iLOG_ITEM_Length;
	...
	if(!LogMessage)
	{
	  LogMessage = (FTM_SEQUENCER_FTM_SequencerLog *)
					ftm_malloc(sizeof(FTM_SEQUENCER_FTM_SequencerLog));
	  if(NULL == LogMessage)
	  {
	    _eErrorCode = FTM_Sequencer_Error_MemoryAllocation;
	    return _eErrorCode; 
	  }
	  memset( LogMessage, 0, sizeof(FTM_SEQUENCER_FTM_SequencerLog) );
	}
	// Length of entire message
	_iFullMessageLength = (unsigned short )(sizeof( FTM_SEQUENCER_DiagLogHeader )
	...
	_iLOG_ITEM_Length = (unsigned short) 
						(
	...
	/* Deallocate common static structure */  
	if(LogMessage != NULL)
	{
	  ftm_free((void *)LogMessage);
	  LogMessage = NULL;
	}
	return _eErrorCode;
