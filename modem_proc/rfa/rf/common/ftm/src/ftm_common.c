...
$Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_common.c#1 $
...
11/04/16   as      Added support to get sensitivity measurement for all LTE BW
...
const float limit_bw_kHz_LTE_1P4MHz = 540.0;
const float limit_bw_kHz_LTE_3MHz = 1350.0;
const float limit_bw_kHz_LTE_5MHz = 2250.0;
const float limit_bw_kHz_LTE_10MHz = 4500.0;
const float limit_bw_kHz_LTE_15MHz = 6750.0;
const float limit_bw_kHz_LTE_20MHz = 9000.0;
const float limit_bw_kHz_LTE_NB = 90.0;

const float limit_bw_kHz_TDSCDMA = 640.0;

int32 cos_value_tbl[14] = {
  -1073741824,	     //cos(-pi())
            0,       //cos(-pi()/2)   * 2^30
    759250125,       //cos(-pi()/4)   * 2^30
    992008094,       //cos(-pi()/8)   * 2^30
   1053110176,       //cos(-pi()/16)  * 2^30
   1068571464,       //cos(-pi()/32)  * 2^30
   1072448455,       //cos(-pi()/64)  * 2^30
   1073418433,       //cos(-pi()/128) * 2^30
   1073660973,       //cos(-pi()/256) * 2^30
   1073721611,       //cos(-pi()/512) * 2^30
   1073736770,       //cos(-pi()/1024) * 2^30
   1073740560,       //cos(-pi()/2048) * 2^30
   1073741508,       //cos(-pi()/4096) * 2^30
   1073741745        //cos(-pi()/8192) * 2^30  
};

int32 sin_value_tbl[14] = {
           0,	      //sin(-pi())
 -1073741824,	      //sin(-pi()/2)   * 2^30
  -759250125,	      //sin(-pi()/4)   * 2^30
  -410903207,	      //sin(-pi()/8)   * 2^30
  -209476638,	      //sin(-pi()/16)  * 2^30
  -105245103,	      //sin(-pi()/32)  * 2^30
   -52686014,	      //sin(-pi()/64)  * 2^30
   -26350944,	      //sin(-pi()/128) * 2^30
   -13176464,	      //sin(-pi()/256) * 2^30
    -6588356,	      //sin(-pi()/512) * 2^30
    -3294193,         //sin(-pi()/1024) * 2^30
    -1647098,         //sin(-pi()/2048) * 2^30
    -823549,          //sin(-pi()/4096) * 2^30
     -411774          //sin(-pi()/8192) * 2^30     
};
...
float ftm_common_get_cnr_bw_limits(ftm_rf_technology_type tech)
{
   ...
   switch(tech)
   {
      ...
      case FTM_RF_TECH_GSM:
         ...
         break;
      case FTM_RF_TECH_LTE:
         limit_bw_kHz = limit_bw_kHz_LTE_10MHz;
#ifdef FEATURE_LTE
         /*Get the LTE BW corresponding to Tx Tune BW from FTM State Machine*/
         if (TRUE == rflte_state_get_lte_nb_mode())
         {
           limit_bw_kHz = limit_bw_kHz_LTE_NB;
         }
         else
         {
           switch(rflte_ftm_mc_sm.carrier[0].rx_bw)
           {
             case RFCOM_BW_LTE_1P4MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_1P4MHz;
                break;
             case RFCOM_BW_LTE_3MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_3MHz;
                break;
             case RFCOM_BW_LTE_5MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_5MHz;
                break;
             case RFCOM_BW_LTE_10MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_10MHz;
                break;
             case RFCOM_BW_LTE_15MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_15MHz;
                break;
             case RFCOM_BW_LTE_20MHz:
                limit_bw_kHz = limit_bw_kHz_LTE_20MHz;
                break;
             default:
                limit_bw_kHz = limit_bw_kHz_LTE_10MHz;
                break;
