...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_calibration_v3_rx.c#1 $
    ...
    if ( device_ind < FTM_CAL_V3_RX_MAX_DEV )
    {
      ...
      while ( action_mask_1bit != 0 )
      {
        ...
        case RX_MEAS_ACTION_RxDCmV:
          ...
          {
            memscpy( res_write_ptr, sizeof(ftm_cal_rx_meas_lna_state_type),
                  &(cal_result->rx_meas_param[device_ind].rx_dc_offset.lna_state),
                    sizeof(ftm_cal_rx_meas_lna_state_type));
            UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                               sizeof(ftm_cal_rx_meas_lna_state_type) );
            free_size -= sizeof(ftm_cal_rx_meas_lna_state_type);
          ...
          /*------------------------------------------------------------------*/
          /* Populate catm_nb Mode */
          /*------------------------------------------------------------------*/
          if ( free_size >= sizeof(ftm_cal_rx_meas_catm_nb_mode) )
          {
            memscpy( res_write_ptr, sizeof(ftm_cal_rx_meas_power_mode_type),
                 &(cal_result->rx_meas_param[device_ind].rx_dc_offset.catm_nb_mode),
                    sizeof(ftm_cal_rx_meas_catm_nb_mode));
            UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                               sizeof(ftm_cal_rx_meas_catm_nb_mode) );
            free_size -= sizeof(ftm_cal_rx_meas_catm_nb_mode);
          } /* if ( free_size >= sizeof(ftm_cal_rx_meas_catm_nb_mode) ) */
          else
          {
            FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][RX_MEASURE.{0x%x}] "
                       "ResultPush : Rx DC mode - ReqSize %d FreeSize %d",
                       cal_data->seg_data.cal_instance,
                       FTM_CAL_OPCODE_RX_MEASURE,
                       sizeof(ftm_cal_rx_meas_catm_nb_mode), 
                       free_size );
          ...
          /*------------------------------------------------------------------*/
          /* Populate Rx DC I/Q offsets */
          /*------------------------------------------------------------------*/
          if ( free_size >= sizeof(ftm_cal_rx_meas_rx_dc_offset_type) )
          {
            memscpy( res_write_ptr, sizeof(ftm_cal_rx_meas_rx_dc_offset_type),
                 &(cal_result->rx_meas_param[device_ind].rx_dc_offset.rx_dc_offset_i),
                    sizeof(ftm_cal_rx_meas_rx_dc_offset_type));
            UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                               sizeof(ftm_cal_rx_meas_rx_dc_offset_type) );
            free_size -= sizeof(ftm_cal_rx_meas_rx_dc_offset_type);
          } /* if ( free_size >= sizeof(ftm_cal_rx_meas_rx_dc_offset_type) ) */
          else
          {
            FTM_MSG_4( FTM_ERROR, "[calv3][Segment %3d][RX_MEASURE.{0x%x}] "
                       "ResultPush : Rx DC I/Q offset - ReqSize %d FreeSize %d",
                       cal_data->seg_data.cal_instance,
                       FTM_CAL_OPCODE_RX_MEASURE,
                       sizeof(ftm_cal_rx_meas_rx_dc_offset_type), 
                       free_size );
          ...
          {
            memscpy( res_write_ptr, sizeof(ftm_cal_rx_meas_rx_dc_offset_type),
                 &(cal_result->rx_meas_param[device_ind].rx_dc_offset.rx_dc_offset_q),
                    sizeof(ftm_cal_rx_meas_rx_dc_offset_type));
            UPDATE_SIZE_N_PTR( written_size, res_write_ptr, 
                               sizeof(ftm_cal_rx_meas_rx_dc_offset_type) );
            free_size -= sizeof(ftm_cal_rx_meas_rx_dc_offset_type);
  ...
  while ( device_mask_1bit != 0 )
  {
    ...
    if ( device_ind < FTM_CAL_V3_RX_MAX_DEV )
    {
      ...
      while ( rx_meas_1bit != 0 )
      {
        ...
        // case RX_MEAS_RAW_RX_AGC:
        case RX_MEAS_ACTION_RxDCmV:
          memscpy( &(rm_unpack->rx_meas_param[device_ind].rx_dc_offset.lna_state),
                   sizeof(ftm_cal_rx_meas_lna_state_type),
                   payload_ptr, sizeof(ftm_cal_rx_meas_lna_state_type) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, 
                             sizeof(ftm_cal_rx_meas_lna_state_type) );
          memscpy( &(rm_unpack->rx_meas_param[device_ind].rx_dc_offset.catm_nb_mode),
                   sizeof(ftm_cal_rx_meas_catm_nb_mode),
                   payload_ptr, sizeof(ftm_cal_rx_meas_catm_nb_mode) );
          UPDATE_SIZE_N_PTR( payload_size, payload_ptr, 
                             sizeof(ftm_cal_rx_meas_catm_nb_mode) );
          break;
