...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_common_fbrx.c#1 $
...
ftm_rsp_pkt_type ftm_common_fbrx_enter_mode(ftm_common_pkt_type *cmd_ptr)
{
  ...
  for (i = 0; i < RFCOMMON_FBRX_NUM_DATA_BUF; i++)
  {
    if(!ftm_common_fbrx_data_bufs[i])
    {
      ftm_common_fbrx_data_bufs[i] = (ftm_common_fbrx_data_buf_type *)
						ftm_malloc(sizeof(ftm_common_fbrx_data_buf_type));
      if(NULL == ftm_common_fbrx_data_bufs[i])
      {
        FTM_MSG_3( FTM_ERROR, "ftm_common_fbrx_enter_mode: "
					"Unable to allocate memory for ftm_common_fbrx_data_bufs[%d]!",
						i, 0, 0);
        break;
      }
      memset( ftm_common_fbrx_data_bufs[i], 0,
					sizeof(ftm_common_fbrx_data_buf_type) );
    }
  }
...
ftm_rsp_pkt_type ftm_common_fbrx_exit(ftm_common_pkt_type *cmd_ptr)
{
  ...
  for (i = 0; i < RFCOMMON_FBRX_NUM_DATA_BUF; i++)
  {
    if(ftm_common_fbrx_data_bufs[i])
    {
      ftm_free((void *)ftm_common_fbrx_data_bufs[i]);
      ftm_common_fbrx_data_bufs[i] = NULL;
    }
  }
  return rsp;
}
...
ftm_rsp_pkt_type ftm_common_fbrx_store_results(ftm_common_pkt_type *cmd_ptr)
{
  ...
  if (param_ptr != NULL)
  {
    ...
    {
    #if 0 //compilation fix as fbrx api is removed
    ...
    else
    {
      ...
      {
        snprintf(&iq_file_name[0], RFCOMMON_FBRX_MAX_FILE_LENGTH, "fbrx_buf%05d_tx.bin", i);
        /* Open file for "write" */
        ...
        snprintf(&iq_file_name[0],RFCOMMON_FBRX_MAX_FILE_LENGTH,"fbrx_buf%05d_rx.bin", i);
        /* Open file for "write" */
        iq_fp = efs_efopen( &iq_file_name[0], "w" );
