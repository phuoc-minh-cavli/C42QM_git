...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_enh_internal_device_calibration_rsb.c#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/07/21   mzahra  add maximum check for num_gain_state
	...
	boolean api_status = TRUE;
	 ...
	 switch(ftm_rf_tech)
	 {
		...
		case FTM_RF_TECH_LTE:
		    ...
		    lte_carr_rxpath = RFLTE_MC_CARR_TYPE_PRX;
  ...
  rfdevice_trx_common_intf_class* device_ptr = NULL;
   ...
   switch (ftm_rf_tech)
   {
	...
	/*	ret_val = rfdevice_lte_tx_cmd_dispatch(device, 
      ...
      rfdevice_trx_cmn_read_therm(device_ptr, enh_therm_val, &therm_read_in_degC);  
...
)
{
	...
	switch(ftm_rf_tech)
	{
        #ifdef FEATURE_WCDMA
		case FTM_RF_TECH_WCDMA:
			*rxlm_buffer_idx = ftm_wcdma_rxlm_buffer[RFM_DEVICE_0].buf_idx;
...
#ifdef FEATURE_LTE 
		case FTM_RF_TECH_LTE:
			*rxlm_buffer_idx = rflte_ftm_mc_sm.carrier[0].rxlm_prx_buf_idx;
     ...
     #ifdef FEATURE_TDSCDMA
		case FTM_RF_TECH_TDSCDMA:
			*rxlm_buffer_idx = ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx;
	...
	FTM_MSG_1(FTM_MED, "rxlm prx buf:0x%x", *rxlm_buffer_idx);
	return api_status;
}

ftm_enh_internal_device_cal_error_code_type ftm_enh_internal_device_cal_get_stg_freq
(
  rfdevice_trx_common_intf_class * device_ptr,
...
)
{
  ...
  #ifdef FEATURE_LTE
  case FTM_RF_TECH_LTE:
    tx_freq = rflte_core_get_tx_freq_with_offset_from_uarfcn(
...
)
{
  ...
  // uint8 iqCaptureType = rsb_instr->iq_capture_type;   // not used for now, reserve it for future usuage
   ...
   uint8 rx_path = FTM_ENH_INTERNAL_DEVICE_CAL_RXRSB_RES_PRX1;	
   uint8 rx_path_num = 0;

   rflm_handle_rx_t rxlm_buffer_idx = 0;
   rflm_tech_id_t rflm_tech_id; 
  ...
  // uint8 rsb_save_efs_func_index = 0;
   ...
   //     ftm_rsb_efs_save = rfdevice_lte_rx_commit_rsb_data_to_efs;
	//	max_cal_elements = MAX_RSB_CAL_ELEMENT_LTE;
			  FTM_MSG(FTM_ERROR, "ftm_enh_internal_device_cal_rsb() failure, LTE tech not supported!");
...
//#if defined(FEATURE_BOLT_MODEM)
   ...
   {    
     if(carrier ==0)
     {
	   rfcommon_rx_msm_update_rsb_coeff(rxlm_buffer_idx,rsb_a_coeff,rsb_b_coeff, rflm_tech_id,msm_carr_bw,dbg_mode_en); 
     }
   }
...
//#endif
   ...
   if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
   {
	 if(carrier ==0)
	 {
	   ...
	   rx_path = FTM_ENH_INTERNAL_DEVICE_CAL_RXRSB_RES_PRX1;
	 }         
   }
...
//#endif	 
 	 for (rx_path_idx = 0; rx_path_idx < rx_path_num; rx_path_idx++)
 	 {
	    ...
	    if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
	    {
			ret_val = ftm_enh_internal_device_cal_rx_iq_capture(tech, carrier, rx_path, sample_size,iq_buf_ptr);
		    if(ret_val == FALSE)
		    {
		...
		if( dbg_mode_en >= 2 )
		{
			ftm_enh_internal_device_cal_save_iq( tech,band,rx_path, iq_buf_ptr, sample_size, 0);
			FTM_MSG_3(FTM_MED,
        ...
        { 
		  ...
		  if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
		  {
		    ret_val = rfdevice_lte_rx_calc_rsb_nv_params( (rfm_device_enum_type)rx_path, (rfcom_lte_band_type) band, /*RFCOM_BW_LTE_10MHz*/ bw,
	    ...
	    else if(tech == FTM_RF_TECH_WCDMA)
	    {
	      ...
	      if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
	      {
		    ret_val = rfdevice_wcdma_rx_get_rsb_cal_data_temp_comp((rfm_device_enum_type)rx_path, (rfcom_wcdma_band_type) band, &rsb_data);
		  }
		  else
		  {
	    ...
	    else if (tech == FTM_RF_TECH_TDSCDMA)
	    {
		  ...
		  if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
		  {
			ret_val = rfdevice_tdscdma_rx_get_rsb_nv_params((rfm_device_enum_type)rx_path, (rfcom_tdscdma_band_type) band, 
		  								    rsb_data.i_sq,rsb_data.q_sq, rsb_data.iq,therm_data,
	    ...
	    if(ret_val == TRUE)
	    {
	      rsb_result->rsb_res[rx_path][rsb_res_gain_index].nv_active = 1;
		  rsb_result->rsb_res[rx_path][rsb_res_gain_index].sin_theta_final = rsb_data.sin_theta_f;
		  rsb_result->rsb_res[rx_path][rsb_res_gain_index].cos_theta_final = rsb_data.cos_theta_f;
		  rsb_result->rsb_res[rx_path][rsb_res_gain_index].gain_inv_final = rsb_data.gain_inv_f; // hardcode to only one gain state for now
		  rsb_result->device_mask |= (1<<rx_path);
  		  rsb_result->rsb_res[rx_path][rsb_res_gain_index].rsb_before_cal = (int8) floor(compute_rsb(rsb_data)); 			
...
//#if defined(FEATURE_BOLT_MODEM)
			if(rsb_result->rsb_res[rx_path][rsb_res_gain_index].nv_active == 1)
			{
			  if(rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB)
			  { 		 
				rfcommon_rx_msm_update_rsb_coeff(rxlm_buffer_idx,-rsb_a_coeff,-rsb_b_coeff, rflm_tech_id,msm_carr_bw,dbg_mode_en); 			 
...
//#endif
		    ...
		    if( rsb_type == FTM_ENH_INTERNAL_DEVICE_CAL_RX_RSB )
		    {
		      ftm_enh_internal_device_cal_rx_iq_capture(tech, carrier, rx_path, sample_size,iq_buf_ptr);
		    ...
		    //	ftm_enh_device_cal_rsb_get_thermistor((rfm_device_enum_type)rx_path[rx_path_idx], band, &therm_data);	
			  ...
			  if(ret_val == TRUE)
			  {
 			    rsb_result->rsb_res[rx_path][rsb_res_gain_index].cos_theta_after_cal = rsb_data.cos_theta_f;
			    rsb_result->rsb_res[rx_path][rsb_res_gain_index].sin_theta_after_cal = rsb_data.sin_theta_f;	
			    rsb_result->rsb_res[rx_path][rsb_res_gain_index].gain_inv_after_cal = rsb_data.gain_inv_f;
			    rsb_result->rsb_res[rx_path][rsb_res_gain_index].rsb_after_cal = (int8) floor(compute_rsb(rsb_data));  
			  }
			  else
			  {
			    rsb_result->rsb_res[rx_path][0].rsb_after_cal = 0; 	
			  }
		  	}
			if( dbg_mode_en >= 2)  // re-check if iq is good after dynamic iqmc register update
   		    {
   			  ftm_enh_internal_device_cal_save_iq( tech, band,rx_path, iq_buf_ptr, sample_size, 1);								  
		    }			
	    }
	    else
	    {
		  rsb_result->rsb_res[rx_path][0].rsb_before_cal = 0; 
		  rsb_result->rsb_res[rx_path][0].rsb_after_cal = 0; 
		  
		 //add error mesg here
	    }
	    FTM_MSG_6(FTM_MED,"rsb cal: tech:%d, band:%d, carrier:%d, rx_path:%d, rsb_before_cal:%d, rsb_after_cal:%d",
	  	tech,band,carrier,rx_path,rsb_result->rsb_res[rx_path][0].rsb_before_cal, 
	  	rsb_result->rsb_res[rx_path][0].rsb_after_cal);	
 	 }	
   	}  	
   ...
   { 
	 {
      ...
      if(flag == 3) 
      {
	    rx_path= (device_mask & 0x03)>0? RFM_DEVICE_0: ( (device_mask & 0x30)>0 ? RFM_DEVICE_4: RFM_DEVICE_2 );
      }
      ret_val = ftm_rsb_efs_save((rfm_device_enum_type) rx_path,band,&rsb_ver_data);
	  if(ret_val ==FALSE)
	  {
		  if(rsb_ver_data.cal_data !=NULL)
		  {
