...
/*==============================================================================
  ...
  $Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_calibration_xo.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/18/19    kk     Adding support for XO DCC
12/05/18   sk      Compiler Warning Fixes
...
static ftm_lm_buf_type *ftm_xo_cal_rxlm_buffer[2];
  ...
  else
  {
    ...
    else
    {
      ...
      case 3:
        ...
        break;
      case 4:
        FTM_MSG(FTM_HIGH,"Dispatching XO Cal command: DC Cal + XO DCC Cal");
        ftm_xo_cal_dc_coarse_dcc_cal(req, rsp_pkt);
        break;
      default:
        ftm_rsp_data.cmd = FTM_RSP_BAD_CMD;
...
)
{
  ...
  /*In case of nb1 calibration set nb1 technology & pass */
  if(rflte_state_get_lte_nb_mode())
  {
    req->ftm_xo_cal_parameter.technology = TECHNOLOGY_NB1; 
    rflte_state_set_xo_cal_mode(TRUE);
  }
...
)
{
  ...
  //Update tech to indicate if XO duty cycle cal needs to be performed
  ftm_fac_cal_params.tech = (tcxomgr_cal_xo_cal_tech)(ftm_fac_cal_params.tech | 0x80);
  FTM_MSG_1(FTM_HIGH,"XO tech = 0x%x",ftm_fac_cal_params.tech);
...
)
{
  ...
  for (i = 0; i < 2; i++)
  {
    if(!ftm_xo_cal_rxlm_buffer[i])
    {
      ftm_xo_cal_rxlm_buffer[i] = (ftm_lm_buf_type *)
					ftm_malloc(sizeof(ftm_lm_buf_type));
      if(NULL == ftm_xo_cal_rxlm_buffer[i])
      {
        FTM_MSG_3( FTM_ERROR, "ftm_common_fbrx_enter_mode: "
				"Allocation failed for ftm_xo_cal_rxlm_buffer[%d]!", i, 0, 0);
        if (1 == i)
        {
          ftm_free((void *)ftm_xo_cal_rxlm_buffer[0]);
          ftm_xo_cal_rxlm_buffer[0] = NULL;
        }
        return ret_val;
      }
      memset( ftm_xo_cal_rxlm_buffer[i], 0, sizeof(ftm_lm_buf_type) );
    }
  }
...
uint8 i;
  ...
  /* Deallocate static structure */

  for (i = 0; i < 2; i++)
  {
    if(ftm_xo_cal_rxlm_buffer[i])
    {
      ftm_free((void *)ftm_xo_cal_rxlm_buffer[i]);
      ftm_xo_cal_rxlm_buffer[i] = NULL;
    }
  }
}/*ftm_xo_cal_exit_mode*/
...
)
{
  ...
  FTM_MSG_1(FTM_HIGH,"XO state returned: %d",p_rsp->state);
  p_rsp->nom_coef = p_cal_params->nom_coef;
  p_rsp->cal_coef= p_cal_params->cal_coef;
  FTM_MSG_1(FTM_HIGH,"Calcoeff: %d",p_rsp->cal_coef.c1);
  p_rsp->tcxomgr_cal_info_pkt = *p_cal_info;
  FTM_MSG_1(FTM_HIGH,"Final Trim :  %d",p_rsp->tcxomgr_cal_info_pkt.dc_xo_trim);
} /*ftm_xo_populate_response*/
