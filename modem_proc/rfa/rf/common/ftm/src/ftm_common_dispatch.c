...
Copyright (c) 2005 - 2018 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/rfa.mpss/3.10/rf/common/ftm/src/ftm_common_dispatch.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/21/21   gk      B8_B addition
09/13/19   mpa    To enable enh_temp__comp
...
)
{
  ...
  if (txdev_cmn != NULL)
  {
    RF_MSG(RF_ERROR, "ftm_common_get_txlut_info_for_red_cal - Calling deprecated api - "
                         "ftm_common_get_txlut_info_for_red_cal");
    ...
    */
    ...
    {
     //Not applicable for chiltepin + SDR105
...
ftm_rsp_pkt_type ftm_common_dispatch(ftm_common_pkt_type *cmd_ptr)
{
  ...
  uint32                         therm_val_32=0;
  uint16 ftm_debug_steps;
  uint16* ftm_debug_steps_p=NULL;
#ifdef FEATURE_RF_HAS_QTUNER
  ...
  if (cmd_ptr != NULL)
  {
   FTM_MSG_HIGH("In FTM_common dispatch  CALLED with cmd_id %d",cmd_ptr->ftm_hdr.cmd_id,0,0);
    ...
    case FTM_GET_THERM: 
      {
        ...
        */
        uint32 therm_val = 0;
		    FTM_MSG_HIGH("In FTM_GET_THERM CALLED ",0,0,0);
    rfgsm_core_handle_ptr=rfgsm_core_handle_get( RFM_DEVICE_0);
	rfdevice_gsm_get_therm_read_script(RFM_DEVICE_0,
                                      (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.therm_read_script,
                                      RFDEVICE_EXECUTE_IMMEDIATE,
                                      &therm_val,
                                      &therm_read_in_degC);
	FTM_MSG_HIGH("In FTM_GET_THERM THERM %d thermdegc %d",therm_val,therm_read_in_degC,0);
...
#ifdef CHECK_WITH_DEVICES

     rfdevice_gsm_cmd_dispatch(RFM_DEVICE_0,RF_PATH_0,RFDEVICE_GET_THERMISTER_VALUE, &enh_therm_val);
     Need to replace with thermistor driver function
#endif
    FTM_MSG_HIGH("In rfdevice_gsm_get_therm_read_script CALLED ",0,0,0);
    rfgsm_core_handle_ptr=rfgsm_core_handle_get( RFM_DEVICE_0);
	rfdevice_gsm_get_therm_read_script(RFM_DEVICE_0,
                                      (rf_buffer_intf*)rfgsm_core_handle_ptr->scripts.therm_read_script,
                                      RFDEVICE_EXECUTE_IMMEDIATE,
                                      &enh_therm_val,
                                      &therm_read_in_degC);
	FTM_MSG_HIGH("In rfdevice_gsm_get_therm_read_script THERM %d thermdegc %d",enh_therm_val,therm_read_in_degC,0);
#else
        therm_read_error = TRUE;
#endif
        break;
...
#if defined (FEATURE_CDMA1X) || defined (FEATURE_TDSCDMA)|| defined (FEATURE_WCDMA)|| defined (FEATURE_LTE)
...
//        rsp_pkt = ftm_common_calculate_xpt_pin_compression( cmd_ptr, ftm_rf_tech );
      ...
      switch (current_mode)
      {
        ...
        case RFM_TDSCDMA_MODE:
         ...
         case RFM_EGSM_MODE_3:
          ...
          {
            /* Commenting out call to below GSM API to remove compilation dependency on disabling QTUNER Flag*/
  ...
  case FTM_GET_QFE_HDET_REFLECTED_PWR:
    ...
    case FTM_DO_RFFE_MULTI_CMD:
      rsp_pkt = ftm_rffe_dispatch_multi((ftm_rffe_pkt_multi_byte_type*) cmd_ptr);
      rsp_pkt_created = TRUE;
      break;
...
)
{
  ...
  case FTM_COMMON_FBRX_ILPC_OVERRIDE:
    FTM_MSG_1( FTM_ERROR, "Common Radio Dispatch : Unsupported Command - %d",
