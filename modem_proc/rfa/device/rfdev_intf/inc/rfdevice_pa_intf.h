...
$Header: //components/rel/rfa.mpss/3.10/device/rfdev_intf/inc/rfdevice_pa_intf.h#1 $

when       who   what, where, why
--------   ---   ---------------------------------------------------------------
05/06/19   dh     Added an interface API for PA init() 
05/29/18   mms    Added an interface to check if device is IOTFE PA
07/13/18   dh     Added an lte_cat_tye param
...
extern "C" {
   ...
   (
    ...
    boolean rfdevice_pa_is_internal_pa
    (
      void *device_ptr
    );
  
    boolean rfdevice_pa_gain_range_trigger
    (
      rfdevice_pa_config_type *pa_config_p,
      void *device_ptr,
      void *buff_obj_ptr, 
