...
Copyright (c) 1999 - 2021,2023 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
04/17/19   ak	   Removed rflte_core_task_bootup_init api as part of Page only mode
04/05/19   mpa     Removal invalid GSM band ENUM 
...
07/24/18   gk      B66 catM1/NB1 enablement
07/12/18   gk      NB-IoT Calibration support changes
06/12/18   sk      1x/WCDMA/TDSCDMA removal changes
05/30/18   tik     updated LTE BW enum for NB-IoT Tx calibration - 200KHz
...
11/10/16   smi     Added enum indicating LTE category
10/13/16   ry      Added LTEE B66 enum for only compialtion dependency.
                   B66 Functionality not supported.
...
typedef enum 
{
  ...
  RFCOM_BAND_LTE_B73 = 62,
  RFCOM_BAND_LTE_B85 = 63,
  RFCOM_BAND_LTE_B86 = 64,
  RFCOM_BAND_LTE_B87 = 65,
  RFCOM_BAND_LTE_B88 = 66,
...
typedef enum 
{
  ...
  RFCOM_BW_LTE_200KHz = 12,
  RFCOM_NUM_LTE_BWS,
  RFCOM_BW_LTE_INVALID
} rfcom_lte_bw_type;

/* Enums for supported GSM bandwidths */
typedef enum 
{
  RFCOM_BW_GSM_1P4MHz,
  RFCOM_NUM_GSM_BWS,
  RFCOM_BW_GSM_INVALID
} rfcom_gsm_bw_type;
...
{
  RFGNSS_DEFAULT_MODE,
  RFGNSS_L1SSN1 = RFGNSS_DEFAULT_MODE,    /* Shared antenna, no eLNA, Sawless and Narrowband 1 */
  RFGNSS_L1SSW1,    /* Shared antenna, no eLNA, Sawless and Wideband 1 */
  RFGNSS_L1SSW2,    /* Shared antenna, no eLNA, Sawless and Wideband 2 */
  RFGNSS_L1E1N1,    /* Dedicated antenna, eLNA, Single SAW and Narrowband 1 */
  RFGNSS_L1E1W1,    /* Dedicated antenna, eLNA, Single SAW and Wideband 1 */
  RFGNSS_L1E1W2,    /* Dedicated antenna, eLNA, Single SAW and Wideband 2 */  
  RFGNSS_L1SSSN1,   /* Shared antenna, no eLNA, Sawless, Shared WWAN Path and Narrowband 1 */
  RFGNSS_L1SSSW1,   /* Shared antenna, no eLNA, Sawless, Shared WWAN Path and Wideband 1 */
  RFGNSS_L1SSSW2,   /* Shared antenna, no eLNA, Sawless, Shared WWAN Path and Wideband 2 */
  RFGNSS_M5ET,      /* Triband ELNA normal, Default*/
  RFGNSS_M5ET1,     /* Triband ELNA one Saw*/
  RFGNSS_M5HT,      /* Triband Non ELNA */
  RFGNSS_MTE,       /* Triband ELNA low IF*/
  RFGNSS_M5LT,      /* WGR targets Triband Non ELNA */
  RFGNSS_MODEMAX,
  RFGNSS_MODEINVALID = RFGNSS_MODEMAX
} rfgnss_mode_config_type;
...
typedef union
{
  ...
  rfcom_gnss_band_type gnss_band;
} rfcom_band_type_u;
...
typedef enum{
 IQ_CAPTURE_QSIZE16 = 0,
 IQ_CAPTURE_QSIZE20
}lte_iq_sample_capture_qsize_enum;
...
typedef struct
{
  ...
  lte_iq_sample_capture_qsize_enum iq_format;
  uint32 *buffer_start_addr;
}nb1_xo_cal_config_s;

void rflte_nb_xo_cal_msm_iq_capture_config(nb1_xo_cal_config_s *nb1_msm_iq_capture_req);

/* Structure containing psm & hibernation data */
typedef struct
{ 
 boolean hibernation_exit;
 boolean psm_mode;
 boolean psm_band_init_complete;
 boolean common_init_success;
 uint32 num_poi_nbr_bands;
 sys_sband_lte_e_type psm_prffered_band;
 sys_band_mask_type poi_nbr_band_bit_mask;
 sys_band_mask_type lte_band_bit_mask;
}rfm_psm_hibernation_info_s;


typedef struct
{
	rfcom_gsm_band_type band;
	boolean psm_gsm_exit;

}rfm_psm_gsm_info_s;

rfm_psm_hibernation_info_s *rflte_get_psm_hibernation_info();

sys_band_mask_type rflte_set_psm_pri_band_mask(sys_sband_lte_e_type lte_band);

/*XO Duty cycle data for XO DCC Calibration*/
typedef enum
{
  RFM_DCC_READBACK_LT_50_PERCENT, /* Indicates the duty cycle read is less than 50% */
  RFM_DCC_READBACK_EQ_50_PERCENT, /* Indicates the duty cycle read is equal to 50% */
  RFM_DCC_READBACK_GT_50_PERCENT,/* Indicates the duty cycle read is greater than 50% */
  RFM_DCC_READBACK_MAX,
  RFM_DCC_READBACK_INVALID = RFM_DCC_READBACK_MAX
}rfm_dcc_read_back_type;
...
typedef struct
{
  ...
  uint16 reg_dcc_out_val;
} rfm_dcc_info_type;


/*API to get psm & hibernation info*/
rfm_psm_hibernation_info_s *rflte_get_psm_hibernation_info();

/*API to initialize psm & hibernation info*/
void rflte_psm_hibernation_info_init();

/*API to get psm mode flag*/
boolean rflte_get_is_psm_mode();

/*API to get XO Duty cycle data for XO DCC Calibration and this will be exposed to TCXO MGR*/

boolean rflte_mc_get_xo_gen_dcc_val_v2(rfm_dcc_read_back_type* rfm_dcc_read_back, uint32 *reg_dcc_out_val);
