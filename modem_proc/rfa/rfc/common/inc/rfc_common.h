...
$Header: //components/rel/rfa.mpss/3.10/rfc/common/inc/rfc_common.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/19/19   am      Add RFFE V2 support
10/25/18   sk      TRACK B Bringup changes
10/05/18   mns     device shanpshot changes
...
#define RFC_MAX_TRX_DEVICE_INSTANCES_PER_BAND 1

#define RFC_ANT_NUM_INVALID 0xFF
...
typedef struct
{
  ...
  rfdevice_comm_proto_version proto_version;
...
/*API     performs sdr105 power on sequence */
void rfcommon_bootup_init();

/*API for extended registers writes */
void rfcommon_write_ext_registers();

/*API for extended register long writes */
void rfcommon_write_ext_long_registers();

/*APIs convert GPIOs to RFFE*/
void rfcommon_convert_gpio_to_rffe();

/*APIs convert GPIOs to GRFC*/
void rfcommon_convert_gpio_to_grfc();

void rffe_read_data();

/* API to get the signal number from msm signal type */
int32 rfc_common_get_grfc_num_of_msm_sig (rfc_msm_signal_type msm_signal);

void rfc_common_snapshot_init(void);
#ifdef __cplusplus
}
#endif
