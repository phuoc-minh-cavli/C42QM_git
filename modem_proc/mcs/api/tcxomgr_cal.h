...
$Header: //components/rel/mcs.mpss/5.1/api/tcxomgr_cal.h#5 $
...
/* note: when DCC Cal is enabled, this state will be limited to the
         bottom 8 bits in a shared structure with DCC state */
typedef enum
{
  TCXOMGR_CAL_STATE_NOT_CAL                  = 0x00,  /* intial uncalibrated state */
  TCXOMGR_CAL_STATE_COARSE                   = 0x01,  /* Coarse tuning completed   */
  TCXOMGR_CAL_STATE_SAMP                     = 0x02,  /* Samples Captured          */
  TCXOMGR_CAL_STATE_C1C2C3                   = 0x03,  /* C1C2C3 determined         */
  TCXOMGR_CAL_STATE_FINE                     = 0x04,  /* Fine tuning completed     */
  TCXOMGR_CAL_STATE_COMPLETED                = 0x05,  /* Fac Cal completed         */
  TCXOMGR_CAL_STATE_FAILED                   = 0x06,  /* Fac Cal failed            */
  TCXOMGR_CAL_STATE_MAX                      = 0x7FFF,
} tcxomgr_cal_state_enum_type;

/* tcxomgr factory dcc state*/
/* note: when DCC Cal is enabled, this state will be limited to the
         upper 8 bits in a shared structure with xo cal state */
typedef enum
{
  TCXOMGR_CAL_DCC_STATE_NOT_CAL              = 0x00,  /* intial uncalibrated or
                                                          unsupported state        */
  TCXOMGR_CAL_DCC_STATE_DCC_SUCCESS          = 0x01,  /* DCC Cal success           */
  TCXOMGR_CAL_DCC_STATE_DCC_SATURATED        = 0x02,  /* DCC Cal ended saturated   */
  TCXOMGR_CAL_DCC_STATE_DCC_FAILED           = 0x03,  /* DCC Cal failed            */
  TCXOMGR_CAL_DCC_STATE_INIT_FAILED          = 0x04,  /* DCC power up init failed  */
  TCXOMGR_CAL_DCC_STATE_DCC_SUCCESS_2ND      = 0x05,  /* DCC Cal succeded-2nd run  */
  TCXOMGR_CAL_DCC_STATE_DCC_SKIPPED          = 0x06,  /* DDC 2nd already run,
                                                          extra DCC Skipped        */
  TCXOMGR_CAL_DCC_STATE_IN_PROGRESS          = 0x10,  /* DCC in progress           */
  TCXOMGR_CAL_DCC_STATE_MAX                  = 0xFF,
} tcxomgr_dcc_state_enum_type;

#define DCC_SUCCESS_STATE_CK( state )                                         \
  ( ( (state) == TCXOMGR_CAL_DCC_STATE_DCC_SUCCESS     ) ||                   \
    ( (state) == TCXOMGR_CAL_DCC_STATE_DCC_SATURATED   ) ||                   \
    ( (state) == TCXOMGR_CAL_DCC_STATE_DCC_SUCCESS_2ND )    )
#define DCC_SUCCESS_FINAL_STATE_CK( state )                                   \
  ( (state) == TCXOMGR_CAL_DCC_STATE_DCC_SUCCESS_2ND )
...
typedef enum
{
  ...
  TCXOMGR_ERR_EST_NO_TONE                    = 19,    /* Fail because No tone is found in
                                                          valid ranges                      */
  TCXOMGR_ERR_EST_MULTI_TONES                = 20,    /* Fail because 2 or more tones are
                                                          found in valid ranges             */
  TCXOMGR_ERR_EST_JAMMER_OUTSIDE_RANGE       = 21,    /* Fail because extremely strong
                                                          jammer outside of valid freq range*/
  ...
  TCXOMGR_CAL_FAIL_PPS_CLIENT_WRONG_STATE    = 25,    /* PPS client stm in the wrong state  */
  TCOXMGR_CAL_FAIL_PPS_CLIENT_ENABLE_PULSE_IN= 26,    /* PPS client fail to enable pulse_in */
  TCOXMGR_CAL_FAIL_PPS_CLIENT_DISABLE_PULSE_IN = 27,  /* PPS client fail to disable pulse_in*/
  TCXOMGR_CAL_FAIL_PPS_SERVER_TIMEOUT        = 28,    /* PPS server request timeout         */
  TCXOMGR_CAL_FAIL_PPS_SERVER_NOT_CALIBRATED = 29,    /* PPS server is not calibrated       */
  TCXOMGR_CAL_FAIL_PPS_SERVER_XMIT_PULSE_OUT = 30,    /* Fail to transmit pulse_out         */
  TCXOMGR_CAL_FAIL_PPS_SERVER_WRONG_STATE    = 31,    /* PPS server stm in the wrong state  */
  TCXOMGR_CAL_FAIL_PPS_SERVER_STAT           = 32,    /* PPS server fail to provide cal stat*/
  TCXOMGR_CAL_FAIL_PPS_SERVER_SETUP          = 33,    /* PPS server fail to setup pulse_out */
  TCXOMGR_CAL_FAIL_PPS_SERVER_TEARDOWN       = 34,    /* PPS server fail to teardown pulse_out*/
  TCXOMGR_CAL_FAIL_PPS_SERVER_START          = 35,    /* PPS server fail to start pulse_out */
  TCXOMGR_CAL_FAIL_PPS_SERVER_STOP           = 36,    /* PPS server fail to stop pulse_out  */

  TCXOMGR_CAL_FAIL_DUTY_CYCLE                = 40,    /* XO DCC Generic failure             */
  TCXOMGR_CAL_FAIL_DUTY_CYCLE_RF_API         = 41,    /* XO DCC RF API failure              */
  TCXOMGR_CAL_FAIL_DUTY_CYCLE_PMIC_API       = 42,    /* XO DCC PMIC API failure            */
  TCXOMGR_CAL_FAIL_DUTY_CYCLE_MISSING_API    = 43,    /* XO DCC RF/PMIC API not defined     */

  TCXOMGR_CAL_FAIL_NUM_SAMP_OUT_OF_RANGE     = 44,    /* Number of samples in capture is
                                                         outside the range of 4K to 8K      */

  TCXOMGR_CAL_UNKNOWN_FAILURE                = 0xFE,
  TCXOMGR_CAL_FAIL_MAX                       = 0xFF
...
/* Factory calibration technology identifier */
/* note: lower order bits are reserved for RATS
         higher order bits are used denote other options */
typedef enum
{
  TECHNOLOGY_CDMA                            = 0x00,
  TECHNOLOGY_CDMA_DUTY_CYCLE                 = 0x80,
  TECHNOLOGY_WCDMA                           = 0x01,
  TECHNOLOGY_WCDMA_DUTY_CYCLE                = 0x81,
  TECHNOLOGY_GSM                             = 0x02,
  TECHNOLOGY_GSM_DUTY_CYCLE                  = 0x82,
  TECHNOLOGY_LTE                             = 0x03,
  TECHNOLOGY_LTE_DUTY_CYCLE                  = 0x83,
  TECHNOLOGY_TDSCDMA                         = 0x04,
  TECHNOLOGY_TDSCDMA_DUTY_CYCLE              = 0x84,
  TECHNOLOGY_CDMA_SEC                        = 0x05,
  TECHNOLOGY_CDMA_SEC_DUTY_CYCLE             = 0x85,
  TECHNOLOGY_GPS                             = 0x06,
  TECHNOLOGY_GPS_DUTY_CYCLE                  = 0x86,
  TECHNOLOGY_NB1                             = 0x07,
  TECHNOLOGY_NB1_DUTY_CYCLE                  = 0x87,
  TECHNOLOGY_PPS_INT                         = 0x08,
  TECHNOLOGY_MAX                             = 0x09,

  /* define a value for just Duty Cycle Cal, is does not need to be less
     than max, as there is no accessor functions needed for it */
  TECHNOLOGY_DUTY_CYCLE_ONLY                 = 0x8F,
} tcxomgr_cal_xo_cal_tech;
#define TCXOMGR_EXTRACT_XO_CAL_TECH( tech )   ( (tech) & 0xF )
#define TCXOMGR_EXTRACT_XO_DUTY_CYCLE( tech ) ( ( (tech) & 0x80 ) >> 3 )

/* Temp set used for RF Cal and EFS interfaces/structures
   note this type is used by RF Cal, it's size cannot change,
   reference is through tcxomgr_cal_info_type */
typedef struct
{
...
} tcxomgr_cal_temp_set_type;

/* Temp range used for RF Cal and EFS interfaces/structures
   note this type is used by RF Cal, it's size cannot change,
   reference is through tcxomgr_cal_info_type */
typedef struct
{
...
typedef struct{
  ...
  uint32                          rx_freq;             /* Carrier Frequency */
...
typedef enum
{
  ...
  TCXOMGR_XTAL_2016,
  TCXOMGR_XTAL_2520,
  TCXOMGR_XTAL_2016_38_4_MHZ,
  TCXOMGR_XTAL_1612_38_4_MHZ,
  TCXOMGR_XTAL_MAX
...
typedef struct
{
  ...
  uint16                          substage_time;       /* time to wait between samp
                                                           substages (ms)                 */
  uint16                          timeout_time;        /* maximum time to run each step
                                                           before declaring failure (ms)  */
  union
  {
...
typedef struct
{
  ...
  union
  {
    uint8                         fin_xo_trim;         /* post Fine DC xo_trim -
                                                           deprecated              */
    uint8                         duty_cycle_adj;      /* Duty Cycle Adjust
                                                           Final value             */
  };
...
/* Duty Cycle Calibration (DCC) parameters structure */
/* note this type is used by RF Cal, it's size cannot change */
typedef struct
{
  uint16                          update_nv;           /* update NV items flag
                                                           tcxomgr_cal_record_enum_type  */
  uint16                          dcc_state;           /* DCC state
                                                           tcxomgr_dcc_state_enum_type   */
} tcxomgr_cal_dcc_param_type;

/* Duty Cycle Calibration (DCC) informational structure */
/* note this type is used by RF Cal, it's size cannot change */
typedef struct
{
  uint16                          init_dcc_adj;        /* pre DCC adj val                */
  uint16                          fin_dcc_adj;         /* post DCC adj val               */
  uint32                          fine_dcc_reg;        /* fine DCC register value        */
  uint16                          dcc_state;           /* DCC state
                                                           tcxomgr_dcc_state_enum_type   */
  uint16                          fail_reason;         /* Failure reason
                                                           tcxomgr_cal_failure_enum_type */
} tcxomgr_cal_dcc_info_type;
...
typedef struct
{
  ...
  uint32                          update_flag;         /* bit mask to indicate what
...
FUNCTION TCXOMGR_CAL_DUTY_CYCLE

DESCRIPTION
  XO duty cycle calibration algorithm.
  This api is expected to be called from FTM
...
SIDE EFFECTS
  None
============================================================================*/
extern void tcxomgr_cal_duty_cycle
(
  tcxomgr_cal_dcc_param_type   *dcc_param,         /* factory cal parameters   */
  tcxomgr_cal_dcc_info_type    *info_pkt           /* cal info pkt             */
);
...
============================================================================*/
extern uint32 tcxomgr_dcc_get_adj( void );
