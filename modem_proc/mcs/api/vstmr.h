...
$Header: //components/rel/mcs.mpss/5.1/api/vstmr.h#9 $
...
/*! @brief Add alias for QTMR register */
#ifndef HWIO_MSS_QDSP6SS_QTMR_V1_CNTPCT_LO_0_IN
  #define MSS_QDSP6SS_QTMR_V1_CNTPCT_LO_0  QDSP6SS_QTMR_V1_CNTPCT_LO_0
#endif
...
typedef enum
{
  ...
  VSTMR_RTC_NB1_SUB_START,
  VSTMR_RTC_NB1              = VSTMR_RTC_NB1_SUB_START,    /*!< NB1 subscription */
  VSTMR_RTC_NB1_SUB_END      = VSTMR_RTC_NB1,

  VSTMR_NUM_RTC,
  VSTMR_RTC_INVALID          = VSTMR_NUM_RTC               /*!< Invalid RTC identifier */
} vstmr_rtc_id;

/*! @brief Enum to identify VFR */
typedef enum
{
  VSTMR_VFR_PRI = 0,     /*!< Primary VFR */
  VSTMR_VFR_SEC,         /*!< Secondary VFR */
  VSTMR_VFR_AUTO = 0xFF  /*!< Auto allocate VFR */
} vstmr_vfr_e;
...
/*! @brief Convert XO count to micro seconds */
#define VSTMR_XO_TO_US(xo_cnt)             (((xo_cnt) * 10)/192)

/*! @brief Convert micro seconds to nearest integer */
#define VSTMR_US_TO_XO(us)                 ((((us) * 192) + 5)/10)

/*! @brief Convert XO count to milliseconds, rounding to nearest ms. */
#define VSTMR_XO_TO_MS(xo_cnt)             (((xo_cnt) + 9600) / 19200)

/*! @brief Convert msec to XO counts. */
#define VSTMR_MS_TO_XO(ms)                 ((ms) * 19200)

#define VSTMR_EPOCH_FCW_ERR_OFS            0
#define VSTMR_EPOCH_FCW_ERR_WIDTH          16
#define VSTMR_EPOCH_FCW_ERR_MASK           (((1uLL << VSTMR_EPOCH_FCW_ERR_WIDTH) - 1) << VSTMR_EPOCH_FCW_ERR_OFS)

/*! @brief Scaling factor (shift value) for fcw_err stored in the epoch.
    We may discard some LSbs to get a wider error range. */
#define VSTMR_RTC_FCW_ERR_SCALE            1
#define VSTMR_RTC_FCW_ERR_SCALE_MASK       ((1 << VSTMR_RTC_FCW_ERR_SCALE) - 1)

#define VSTMR_EPOCH_FCW_ERR_MOD            (1 << (VSTMR_EPOCH_FCW_ERR_WIDTH - 1 + VSTMR_RTC_FCW_ERR_SCALE))

/*! @brief Compute MIN/MAX FCW's using the nominal.
*/
#define VSTMR_FCW_MIN(nominal)             (nominal - (VSTMR_EPOCH_FCW_ERR_MOD - 1) + VSTMR_RTC_FCW_ERR_SCALE_MASK)         
#define VSTMR_FCW_MAX(nominal)             (nominal + (VSTMR_EPOCH_FCW_ERR_MOD - 1) - VSTMR_RTC_FCW_ERR_SCALE_MASK)
...
extern void vstmr_epoch_advance(void);
...
/*!
    @brief
    Get USTMR count by reading QTimer counter's lower word (synchronized with
    MSS_STMR).
...
)
{
  ...
  #error code not present
#else
  return (HWIO_IN(MSS_QDSP6SS_QTMR_V1_CNTPCT_LO_0));
  #endif /* TEST_FRAMEWORK */
