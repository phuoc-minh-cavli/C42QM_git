...
$Header: //components/rel/mcs.mpss/5.1/api/msgr.h#3 $
...
/*! MSGR API version. Compare this to msgr_get_api() to ensure that MSGR API 
    is in sync.
*/
#define  MSGR_API_VERSION          (0x100)
...
extern uint32 msgr_get_api(void);
...
errno_enum_type msgr_client_create_ext
(
  msgr_client_t   *msgr_client_id,
  boolean          blockable
);
...
errno_enum_type msgr_client_delete_queue
(
  msgr_client_t   *msgr_client_id,
  msgr_id_t       *queue_id          
);

errno_enum_type msgr_client_add_queue_ext
(
  const char        *name,
  msgr_client_t     *msgr_client_id,
  uint8              pri,
...
errno_enum_type msgr_deregister_queue
(
  msgr_tech_module_type     self,              //!< TECH,MODULE Performing Operation
  msgr_client_t            *client_id,         //!< Message Client Handle
  msgr_id_t                 id,                //!< Message queue ID
  msgr_umid_type            umid               //!< UMID for which to register
);
...
errno_enum_type msgr_deregister_block_variant_queue
(
  msgr_tech_module_type   self,             //!< Deregistering TECH,MODULE
  msgr_client_t          *msgr_client_id,   //!< Message Client Handle
  msgr_id_t               id,               //!< Message queue ID
...
#define MSGR_GET_TIMESTAMP(hdr)    ((hdr)->timestamp)
...
/* Get receive delay for the message */
uint32 msgr_get_receive_delay
(
  msgr_hdr_struct_type    *msg_ptr  
);
