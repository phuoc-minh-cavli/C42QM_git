...
$Header: //components/rel/mcs.mpss/5.1/cxm/src/coex_algos.c#1 $
...
)
{
  ...
  if( msg_ptr->tech_id == CXM_TECH_LTE )
  {
    ...
    {
      CXM_MSG_3( HIGH, "Received LTE frame offset %d, previously sent %d & delta %d within threshold - no change reported",
                 filtered_tech_data.params.lte_params.frame_offset,
                 coex_wwan_state_info[CXM_TECH_LTE].state_data.params.lte_params.frame_offset,
...
)
{
  ...
  filter_index = 0;
  CXM_MSG_5( MED, "WWAN tech %d high priority event %d; start %d end %d; #freq %d",
             msg_ptr->tech_id, msg_ptr->op_id, msg_ptr->start,
             msg_ptr->end, msg_ptr->num_freqs );
