...
# --------------------------------------------------------------------------- #
# $Header: //components/rel/mcs.mpss/5.1/tools/build/msgr_builder.py#3 $
# --------------------------------------------------------------------------- #
...
MSGR_TECHS = [('MCS', 0), ('UMB', 1), ('MM', 2), ('WMX', 3), ('LTE', 4), 
              ...
              ('RFLM', 26), ('PROSE', 27), ('MCPM', 28), ('NR5G', 29),
              ('NB1', 30)]
...
def msgr_builder_impl(source, target):
    ...
    for each_file in source:
        ...
        for (key, value) in umid_dict.iteritems():
            try:
                # Check if an entry for the UMID is already present, then mark this one as duplicate
                if value['int_value'] in umid_data:
                    duplicate_umid_exists = True
                    print 'Duplicate UMID: {0}({1}), Src File1:{2}'.format(value['umid_alt_str'],hex(value['int_value']),value['src_file'])
                    print 'Duplicate UMID: {0}({1}), Src File2:{2}\n'.format(umid_data[value['int_value']]['umid_alt_str'],hex(value['int_value']),umid_data[value['int_value']]['src_file'])
                umid_data[value['int_value']] = value
            except KeyError:
                missing_umid_evals.append(key)
    
    # Exit if duplicate UMID is found in the system.
    # Commenting out until all duplicates are cleaned up.
    # if duplicate_umid_exists:
    #     sys.exit('msgr_builder.py: ERROR: Duplicate UMIDs found!!')
    ...
    for each_umid in umid_data.keys():
        ...
        if umid_value is not None:
            ...
            if re.search(NUM_EXPR_BLACK_LIST, 
                ...
                pattern2 = r'\b{0}\s*=\s*([^,\s]+)'.format(umid_data[each_umid]['raw'][4])
