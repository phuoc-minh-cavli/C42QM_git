...
$Header: //components/rel/mcs.mpss/5.1/tcxomgr/src/tcxomgr_data.c#8 $
...
#ifdef FEATURE_TCXOMGR_DUTY_CYCLE_CAL
/* weak definitions for RFM DCC API's */
boolean tcxomgr_data_weak_dcc_rf_set_api( uint32* temp )
{
  CAL_MSG( FATAL,
    "XO Duty Cycle Cal: Using Weak RF SET DCC API" );
  return FALSE;
}
boolean rfm_set_xo_gen_dcc_val( uint32* )                 __attribute__((weak, alias("tcxomgr_data_weak_dcc_rf_set_api")));

/* weak definition for PMIC DCC API's */
pm_err_flag_type tcxomgr_data_weak_dcc_pmapp_set_api( uint8 temp )
{
  CAL_MSG( FATAL,
    "XO Duty Cycle Cal: Using Weak PMIC SET DCC API" );
  return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
}
pm_err_flag_type pmapp_xo_core_set_xo_dcc_adj( uint8 )   __attribute__((weak, alias("tcxomgr_data_weak_dcc_pmapp_set_api")));
#endif /* FEATURE_TCXOMGR_DUTY_CYCLE_CAL */
...
static tcxomgr_efs_dpc_data     ft_table_efs[TCXOMGR_TABLE_SIZE];
...
/*--------------------------------------------------------------------------*/
  ...
  #ifndef FEATURE_TCXOMGR_PMIC_BRINGUP
  #ifdef FEATURE_TRACKB
  pm_err_ret = pm_dev_mega_xo_get_xo_trim(1,0,&pm_xo_trim_val);
  #else
  ...
  {
   #ifdef FEATURE_TRACKB
   (void) pm_dev_mega_xo_set_xo_trim( 1,0, tcxomgr.xo_trim.curr_val );
   #else
   (void) pm_dev_mega_xo_set_xo_trim( 0,0, tcxomgr.xo_trim.curr_val );
    ...
    DBG_0( ERROR, "tcxomgr_xo_trim_read called in PAGE ONLY" );
  #endif
...
/*--------------------------------------------------------------------------*/
  ...
  #ifdef FEATURE_TRACKB
  pm_err_ret = pm_dev_mega_xo_get_xo_trim(1,0, &(tcxomgr.xo_trim.curr_val));
  #else
  ...
  #else /* FEATURE_BUILD_PAGEONLY_IMAGE */
    DBG_0( ERROR, "tcxomgr_xo_trim_write called in PAGE ONLY" );
  #endif
...
============================================================================*/
typedef void (*tcxomgr_cal_duty_cycle_rfm_set_dcc_api)
(
  uint32*                 xo_gen_dcc_val
);

void tcxomgr_dcc_read( void )
{
  #ifdef FEATURE_TCXOMGR_DUTY_CYCLE_CAL
  tcxomgr_efs_dcc_data_type     dcc_adj;
  uint16                        size;
  int                           val;
  #ifndef FEATURE_TCXOMGR_PMIC_BRINGUP
  pm_err_flag_type              pm_ret          = PM_ERR_FLAG__SUCCESS;
  #endif
/*--------------------------------------------------------------------------*/

  /* Initialize values */
  tcxomgr.dcc.version     = 0;
  tcxomgr.dcc.xo_dcc_adj  = 0;
  tcxomgr.dcc.dcc_reg_val = 0;
  tcxomgr.dcc.dcc_state   = TCXOMGR_CAL_DCC_STATE_NOT_CAL;

  size = sizeof( tcxomgr_efs_dcc_data_type );

  /* Attempt to read from NV */
  val = efs_get( TCXOMGR_DCC_DATA_FILE,
                 (void *)(&dcc_adj),
                 size );
  ...
  {
    /* EFS returned error */
    DBG_1(ERROR, "DCC: Error reading EFS file %d", val);
  }
  else
  {
    tcxomgr.dcc.version     = dcc_adj.version;
    tcxomgr.dcc.xo_dcc_adj  = dcc_adj.xo_dcc_adj;
    tcxomgr.dcc.dcc_reg_val = dcc_adj.dcc_reg_val;
    tcxomgr.dcc.dcc_state   = (tcxomgr_dcc_state_enum_type)dcc_adj.dcc_state;

    DBG_4(HIGH, "dcc version = %d, dcc adj = %d, dcc reg value = %d, dcc state = %d ", tcxomgr.dcc.version, tcxomgr.dcc.xo_dcc_adj, tcxomgr.dcc.dcc_reg_val, tcxomgr.dcc.dcc_state );

    if ( ( tcxomgr.dcc.version <= TCXOMGR_DCC_VERSION ) &&
         ( DCC_SUCCESS_STATE_CK( tcxomgr.dcc.dcc_state ) ||
           ( tcxomgr.dcc.dcc_state == TCXOMGR_CAL_DCC_STATE_INIT_FAILED )
         )
       )
    {
      /* update PMIC DCC with initialized value */
      #if !(defined( FEATURE_TCXOMGR_PMIC_BRINGUP ) || \
            defined( TEST_FRAMEWORK ) )
      pm_ret = pmapp_xo_core_set_xo_dcc_adj( (uint8)tcxomgr.dcc.xo_dcc_adj );
      if ( pm_ret != PM_ERR_FLAG__SUCCESS )
      {
        /* something went wrong, exit here */
        DBG_1( ERROR,
          "DCC Init: PM Set API returned failure %d",
          pm_ret );
      ...
      if ( tcxomgr.dcc.version == TCXOMGR_DCC_VERSION )
      {
        /* propagate the value to RF */
        rfm_set_xo_gen_dcc_val( &tcxomgr.dcc.dcc_reg_val );
      }
      #endif
    }
    else
    {
      DBG_2( ERROR,
        "DCC Init: Wrong Version (%d) or un-calibrated (0x%x)",
        tcxomgr.dcc.version,
        tcxomgr.dcc.dcc_state );
    }
...
void tcxomgr_dcc_write( void )
{
  ...
  /* Get the size of the data to be write */
  size = sizeof(tcxomgr_efs_dcc_data_type);

  dcc_adj.version     = tcxomgr.dcc.version;
  dcc_adj.xo_dcc_adj  = tcxomgr.dcc.xo_dcc_adj;
  dcc_adj.dcc_state   = (uint16)tcxomgr.dcc.dcc_state;
  dcc_adj.dcc_reg_val = tcxomgr.dcc.dcc_reg_val;
  dcc_adj.reserved1   = 0;

  /* Write the DCC params to the EFS file */
  val = efs_put( TCXOMGR_DCC_DATA_FILE,
                 (void *)(&dcc_adj),
                 size,
                 O_CREAT|O_AUTODIR,
                 ALLPERMS );
  ...
  {
    /* EFS returned error */
    DBG_1(ERROR, "DCC: Error writing EFS file %d", val);
  }
...
FUNCTION TCXOMGR_DCC_GET_ADJ

DESCRIPTION
  This function returns the DCC adj register values to the callee

DEPENDENCIES
  None

RETURN VALUE
  UINT32 DCC recorded register value

SIDE EFFECTS
  None

============================================================================*/
uint32 tcxomgr_dcc_get_adj( void )
{
/*--------------------------------------------------------------------------*/

  return( tcxomgr.dcc.dcc_reg_val );
...
void xomgr_efs_debug_flags_read( void )
{
  ...
  tcxomgr_efs_debug_flags_struct_type      flags;
  uint16                                   size;
  int                                      val;
  ...
  size = sizeof(tcxomgr_efs_debug_flags_struct_type);

  /* Read debug flags */
  val = efs_get(TCXOMGR_DEBUG_FLAGS, (void *)(&flags), size);

  if ( flags.version == TCXOMGR_DEBUG_FLAG_VERSION )
  {
    tcxomgr.debug_flags = flags;
  }
  else
  {
    tcxomgr.debug_flags.version   = 0;
    tcxomgr.debug_flags.d_flags_1 = 0;
    tcxomgr.debug_flags.d_flags_2 = 0;
  }

  return;

} /* xomgr_efs_debug_flags_read */
...
)
{
  ...
  switch(table)
  {
    ...
    case TCXOMGR_TABLE_ROT_WWAN:
      filename = TCXOMGR_EFS_FT_ROT_TABLE_WWAN;
      break;
...
void tcxomgr_efs_vco_read ( void )
{
  ...
  for ( i = 0; i < VCO_TEMP_TABLE_SIZE; i++ )
  {
    ...
    tcxomgr.rot.temp[i]              = ROT_UNDEFINED;
    tcxomgr.rot.rotCount[i]          = 0;
  ...
  if ( !tcxomgr_diag_get_temp_table_override_enable() )
  {
    ...
    /* Attempt to read WWAN temperature-frequency table */
    if (tcxomgr_efs_ft_table_read(TCXOMGR_TABLE_ROT_WWAN, &vco, sizeof(vco)))
    {
      for ( i = 0; i < VCO_TEMP_TABLE_SIZE; i++ )
      {
        tcxomgr.rot.temp[ i ]        = vco.temp[ i ];
        tcxomgr.rot.rotCount[ i ]    = vco.rotCount[ i ];
      }
    }
...
void tcxomgr_efs_vco_write ( tcxomgr_ft_table_type table )
{
  ...
  if( (table != TCXOMGR_TABLE_WWAN) &&
      (table != TCXOMGR_TABLE_ROT_WWAN) &&
      (table != TCXOMGR_TABLE_GPS)  &&
  ...
  if ( !tcxomgr_diag_get_temp_table_override_enable() )
  {
    for ( i = 0; i < VCO_TEMP_TABLE_SIZE; i++ )
    {
      ...
      ft_table_efs[TCXOMGR_TABLE_ROT_WWAN].vco.temp[i]     =
        (int16)tcxomgr.rot.temp[i];
      ft_table_efs[TCXOMGR_TABLE_ROT_WWAN].vco.rotCount[i] =
        tcxomgr.rot.rotCount[i];
    ...
    if(table == TCXOMGR_TABLE_MAX)
    {
      ...
      tcxomgr_efs_ft_table_write(TCXOMGR_TABLE_ROT_WWAN,
          &ft_table_efs[TCXOMGR_TABLE_ROT_WWAN].vco, FALSE);
...
FUNCTION XOMGR_GET_ROT_TEMP_TABLE

DESCRIPTION
  This function retrieves the given temperature's index value.
  Index returned will be in the ROT_TEMP table domain [0, 63]
  The temperature should be passed in degree centigrade
DEPENDENCIES
  None

RETURN VALUE
  tcxomgr_info_type structure
...
tcxomgr_info_type xomgr_get_rot_temp_table
(
  /* Temperature in half DegC units */
  tcxomgr_temp_type               temp_halfDegC
)
{
  /* Interpolation point at cooler temperatures. */
  tcxomgr_rot_type                rot_low;

  /* Interpolation point at hotter temperatures. */
  tcxomgr_rot_type                rot_high;

  /* Interpolation point and mask, distance from current index. */
  int32                           index_count = 0;
  uint32                          interp_mask = 0;

  /* Used as an index to the table */
  int32                           temp_idx;

  /* Return value */
  tcxomgr_info_type               err;

/*--------------------------------------------------------------------------*/

  /* Initialize to defaults */
  tcxomgr_set_default_rgs_info( &err );
  err.freq_uncertainty         = 5;
  err.vco_type                 = TCXOMGR_FT_QUAL_DEFAULT;
  err.rot_value                = tcxomgr.vco.dflt;

  /* Don't retrive the table if the temperature is out of range or invalid */
  if ( !ROT_VALID_TEMP_HALFDEGC( temp_halfDegC ) )
  {
    DBG_1( ERROR, "Temp Table Req, Sat or Inv Temp (%d)",
           temp_halfDegC );
    return err;
  }

  /* convert thermistor reading to index,
     Valid index is from 0-ROT_TEMP_TABLE_SIZE (255) */
  temp_idx = ROT_TEMP_TO_INX( temp_halfDegC );

  /* check for an exact value in the temp table */
  if ( tcxomgr.rot.temp[ temp_idx ] != ROT_UNDEFINED )
  {
    err.vco_type  = TCXOMGR_FT_ROT_TABLE;
    err.rot_value = tcxomgr.rot.temp[ temp_idx ];
    DBG_3( HIGH, "Query ROT_TEMP[%3d]: type: %2d err: %4d",
           temp_idx,
           err.vco_type,
           err.rot_value );
  ...
  /* We have no table entry for the current temperature.  Look three
  ** entries in either direction (cooler and hotter) to find a valid entry
  ** in the table.  If we find one, use it.  Otherwise, use default. */
//DBG_6( FATAL, "-3:%6d  -2:%6d  -1:%6d    +1:%6d  +2:%6d  +3:%6d",
//       tcxomgr.rot.temp[ ( temp_idx - 3 ) ],
//       tcxomgr.rot.temp[ ( temp_idx - 2 ) ],
//       tcxomgr.rot.temp[ ( temp_idx - 1 ) ],
//       tcxomgr.rot.temp[ ( temp_idx + 1 ) ],
//       tcxomgr.rot.temp[ ( temp_idx + 2 ) ],
//       tcxomgr.rot.temp[ ( temp_idx + 3 ) ]
//       );
  for ( index_count = 1;
        index_count <= ROT_TEMP_TABLE_SEARCH_WIDTH;
        index_count++ )
  {
    /* Look lower in the temperature table for an entry. */
    rot_low  = ( ( ( temp_idx - index_count ) >= 0 )
                 ? tcxomgr.rot.temp[ ( temp_idx - index_count ) ]
                 : ROT_UNDEFINED );

    /* Look higher in the temperature table for an entry. */
    rot_high = ( ( ( temp_idx + index_count ) < ROT_TEMP_TABLE_SIZE )
                 ? tcxomgr.rot.temp[ ( temp_idx + index_count ) ]
                 : ROT_UNDEFINED );

    if ( rot_low  != ROT_UNDEFINED )
    {
      interp_mask = index_count << 4;
    }
    if ( rot_high != ROT_UNDEFINED )
    {
      interp_mask |= index_count;
    }

    if (( rot_low != ROT_UNDEFINED ) || ( rot_high != ROT_UNDEFINED ))
    {
      /* We found a temperature table entry close by.  Quit looking. */
      break;
    }
  ...
  /* if neither lower or higher ROT_TEMP values are defined use the default */
  if ( rot_low == ROT_UNDEFINED && rot_high == ROT_UNDEFINED )
  {
    err.vco_type  = TCXOMGR_FT_QUAL_DEFAULT;
    err.rot_value = tcxomgr.vco.dflt;
  ...
  {
    /* One or both of the lower & higher values are defined so interpolate */
    err.vco_type  = TCXOMGR_FT_ROT_TABLE_INTERP;

    if ( rot_low == ROT_UNDEFINED )
    {
      err.rot_value = rot_high;
    }
    else if ( rot_high == ROT_UNDEFINED )
    {
      err.rot_value = rot_low;
    }
    else
    {
      err.rot_value = ( rot_low + rot_high ) / 2;
    }
  }

  DBG_5( HIGH, "Query ROT_TEMP[%3d]: type: %2d err: %6d  (interp idx=%d, mask=%2x)",
         temp_idx,
         err.vco_type,
         err.rot_value,
         index_count,
...
/*--------------------------------------------------------------------------*/
  ...
  switch(vco_type)
  {
    ...
    case TCXOMGR_FT_ROT_TABLE_INTERP:
      int_vco_type = TCXOMGR_INT_FT_ROT_TABLE_INTERP;
      break;
    case TCXOMGR_FT_ROT_TABLE:
      int_vco_type = TCXOMGR_INT_FT_ROT_TABLE;
      break;
...
/*--------------------------------------------------------------------------*/
  ...
  {
    /* Get the internal VCO type enum */
    int_vco_type = tcxomgr_get_int_vco_type(vco_type, state);
    ...
    else if (int_vco_type == TCXOMGR_INT_FT_ROT_TABLE_INTERP)
    {
      /* check for normal operating temp range */
      if ( ROT_NORM_TEMP_CK( TCXOMGR_XO_TEMP_TO_DEGC( temp ) ) )
      {
        return ( 615 );  /* 0.6 ppm * 2^10 */
      }
      else /* normal operating temp range */
      {
        return ( 820 );  /* 0.8 ppm * 2^10 */
      }
    }
    else if (int_vco_type == TCXOMGR_INT_FT_ROT_TABLE)
    {
      /* check for normal operating temp range */
      if ( ROT_NORM_TEMP_CK( TCXOMGR_XO_TEMP_TO_DEGC( temp ) ) )
      {
        return ( 512 );  /* 0.5 ppm * 2^10 */
      }
      else /* normal operating temp range */
      {
        return ( 717 );  /* 0.7 ppm * 2^10 */
      }
    }
...
/*--------------------------------------------------------------------------*/
  ...
  /* Return the FT cal estimate if we have calibration performed  */
  {
    /* get FT table */
    if ( ( tcxomgr_get_crystal_type()!= TCXO_CRYSTAL ) &&
         ( XOMGR_DEBUG_GET_ROT_TEMP_TABLE_FLAG != 0 ) )
    {
      ft = xomgr_get_rot_temp_table( TCXOMGR_UINTS_TO_TEMP_HALF_DEGC( temp_struct.xo ) );
    }
    else
    {
...
/*--------------------------------------------------------------------------*/
  ...
  if(temp != TCXOMGR_INVALID_TEMP)
  {
    if(tcxomgr_ft_data_is_valid(TCXOMGR_TABLE_WWAN))
    {
      ...
      ft.ft_freq_uncertainty_ppb = tcxomgr_get_int_ppb_uncertainty(
          temp,
          (tcxomgr_vco_indicator_type) tcxomgr.fieldcal_data[TCXOMGR_TABLE_WWAN].cal_data.ft_qual_ind,
    ...
    else
    {
      ...
      ft.ft_freq_uncertainty_ppb = tcxomgr_get_int_ppb_uncertainty(
          temp,
          (tcxomgr_vco_indicator_type)tcxomgr.fieldcal_data[TCXOMGR_TABLE_GPS].cal_data.ft_qual_ind,
...
/*--------------------------------------------------------------------------*/
  ...
  if ( ft.vco_type != TCXOMGR_VCO_RGS )
  {
    if ( temp == TCXOMGR_INVALID_TEMP )
    {
      /* We do not know the current temperature, and RGS is not reliable, so
      ** return default values to the caller. */
      ft.vco_type   = TCXOMGR_FT_QUAL_DEFAULT;
      ft.rot_value  = 0;
      ft.vco_value  = 0;
    ...
    else
    {
      ...
      if ( !( ( ft.vco_type == TCXOMGR_FT_ROT_TABLE  ) ||
              ( ft.vco_type == TCXOMGR_FT_ROT_TABLE_INTERP )    ) )
      {
        /* read the field cal quality indicator to determine the vco_type */
        ft.vco_type  =
            (tcxomgr_vco_indicator_type)
  ...
  /* Verify returned rotator is within limits */
  if( ( ft.rot_value > MAX_ROT_OUT_VALUE ) ||
      ( ft.rot_value < MIN_ROT_OUT_VALUE )    )
  {
    ERR_FATAL( "Tcxomgr rotator out %d is out of bounds (temp %d, ft qual %d)",
               ft.rot_value,
               temp_struct.xo,
...
)
{
  ...
  {
    case XO_CRYSTAL:
      if ( XOMGR_DEBUG_GET_ROT_TEMP_TABLE_FLAG != 0 )
      {
        ft = xomgr_get_stored_ft_xo( &system_rgs, log_ptr );
      }
      else
      {
...
/*--------------------------------------------------------------------------*/
  ...
  if ( old_vco_temp_info.vco_type != TCXOMGR_VCO_DEFAULT )
  {
    ...
    new_vco_temp = VCO_TEMP_FILT(old_vco_temp, vco_acc) + old_vco_temp;
...
FUNCTION XOMGR_UPDATE_ROT_TEMP_TABLE

DESCRIPTION
 This function updates the XO temperature calibrated table
 The temperature should be in degree centigrade
...
)
{
  ...
  /* Previous values for current temperature */
  tcxomgr_vco_info_type                old_rot_temp_info;

  /* Previous ROT value for current temperature */
  tcxomgr_rotator_type                 old_rot_temp;

  /* New ROT value for current temperature */
  tcxomgr_rotator_type                 new_rot_temp;
...
/*--------------------------------------------------------------------------*/
  ...
  temp_halfDegC = TCXOMGR_UINTS_TO_TEMP_HALF_DEGC( temp );

  /* Don't update the table if the temperature is out of range or invalid */
  if ( !ROT_VALID_TEMP( temp ) )
  {
    DBG_1( MED, "Can't update Temperature Table. Out of range temperature: %d",
           temp );
    return;
  }

  /* Can't update ROT_TEMP if we're too close to last update */
  if ( time < ( tcxomgr.rot.last_update_secs + TCXOMGR_UPDATE_TEMP_TBL_SEC ) )
  {
    DBG_2( MED, "Can't update Temperature Table. too close to last wakeup: %d < ",
           time,
           ( tcxomgr.rot.last_update_secs + TCXOMGR_UPDATE_TEMP_TBL_SEC ) );
    return;
  }

  /* Get old ROT accumulator value */
  old_rot_temp_info = xomgr_get_rot_temp_table( temp_halfDegC );

  /* convert thermistor reading to index */
  idx = (uint8)ROT_TEMP_TO_INX(temp_halfDegC);

  old_rot_temp = old_rot_temp_info.rot_value;

  if ( tcxomgr.rot.temp[ idx ] != ROT_UNDEFINED )
  {
    /* Filter the rot and store it in the temp table */
    new_rot_temp = ROT_TEMP_FILT(old_rot_temp, rot_acc);

    DBG_4( HIGH, "ROT_TEMP[%d]: %4d+%4d --> %4d",
           idx,
           old_rot_temp,
           rot_acc,
           new_rot_temp );
  }
  else /* old rot temp type is rot default */
  {
    /* New entry into the FT table, store it as is */
    new_rot_temp = rot_acc;

    DBG_2( HIGH, "ROT_TEMP[%d]:      --> %4d (unfiltered)",
           idx,
           new_rot_temp );
  }

  /* Update RAM version of the temp table */
  tcxomgr.rot.temp[ idx ]     = new_rot_temp;
  tcxomgr.rot.rotCount[ idx ]++;

  /* Update time we last updated the ROT_TEMP table */
  tcxomgr.rot.last_update_secs = time;

} /* xomgr_update_rot_temp_table */
...
tcxomgr_cal_xtal_enum_type tcxomgr_efs_read_xtal_type ( void )
{
  ...
  if ( efs_get(TCXOMGR_XO_CRYSTAL_TYPE_FILE, (void *)efs.buffer, sizeof(xtal_type)) < 0 )
  {
...
)
{
  ...
  *((uint32*)(efs.buffer)) = xtal_type;
  ...
  /* Write to the file. Create it if it doesnt exist */
  val = efs_put(TCXOMGR_XO_CRYSTAL_TYPE_FILE, (void *)efs.buffer, sizeof(xtal_type),
                O_CREAT|O_AUTODIR, ALLPERMS);
  if (val < 0)
  {
    DBG_1(ERROR, "Error writing to EFS file %d", val);
