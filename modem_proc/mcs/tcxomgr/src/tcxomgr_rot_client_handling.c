...
$Header: //components/rel/mcs.mpss/5.1/tcxomgr/src/tcxomgr_rot_client_handling.c#3 $
...
--------   ---      ---------------------------------------------------------
05/08/17   sst      Add support for NB1
...
)
{
  ...
  case TCXOMGR_ROT_CLIENT_NB1:
    client_id = TCXOMGR_CLIENT_NB1;
    break;

  default:
    client_id = TCXOMGR_CLIENT_NONE;
    DBG_1(ERROR, "tcxomgr_xlate_rot_client_to_client_id: Invalid ROT Client %d",
          rgs_client);
    break;
  }

  return client_id;
} /* tcxomgr_xlate_rot_client_to_client_id */
...
)
{
  ...
  case TCXOMGR_CLIENT_NB1:
    rot_client = TCXOMGR_ROT_CLIENT_NB1;
    break;

  default:
    rot_client = TCXOMGR_ROT_CLIENT_MAX;
    DBG_1(ERROR, "tcxomgr_xlate_client_id_to_rot_client: Invalid Client Id %d",
          client_id);
    break;
  }

  return rot_client;
} /* tcxomgr_xlate_client_id_to_rot_client */
...
tcxomgr_vco_info_type tcxomgr_get_client_rgs (tcxomgr_client_info_struct_type client_info)
{
  ...
  {
    ft = xomgr_get_stored_ft_xo(&rgs_info->client_rgs[client_info.as_id][rgs_client], &rgs_log);
  }
  else
  {
...
tcxomgr_vco_info_type tcxomgr_get_subscription_rgs (tcxomgr_as_id_enum_type as_id)
{
  ...
  if ( TCXOMGR_IS_ROT_CLIENT_VALID(rgs_client) )
  {
    ...
    if ( XOMGR_DEBUG_GET_ROT_TEMP_TABLE_FLAG != 0 )
    {
      ft = xomgr_get_stored_ft_xo(&rgs_info->client_rgs[as_id][rgs_client], &rgs_log);
    }
    else
    {
  ...
  else
  {
    ...
    if ( XOMGR_DEBUG_GET_ROT_TEMP_TABLE_FLAG != 0 )
    {
      ft = xomgr_get_stored_ft_xo( NULL, &rgs_log );
    }
    else
    {
...
/*--------------------------------------------------------------------------*/
    ...
    */
       ...
       (field_cal->cal_data.ft_qual_ind == TCXOMGR_CAL_FT_QUAL_FT2))
    {
      ...
      ft_uncertainty = tcxomgr_get_int_ppb_uncertainty(
          ...
          (tcxomgr_vco_indicator_type)field_cal->cal_data.ft_qual_ind,
  ...
  {
    DBG_1( ERROR, "RGS - update temp tables (xtal type)", tcxomgr_get_crystal_type() );
    if( tcxomgr_get_crystal_type()!= TCXO_CRYSTAL )
    {
      rgs_data.id             = id;
      ...
      /* Update the temp vs XO freq table */
      xomgr_update_rot_temp_table( rot_val, time_in_sec, temp.xo );
    }
    else
    {
