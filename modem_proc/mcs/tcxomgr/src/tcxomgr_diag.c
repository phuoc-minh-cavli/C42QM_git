...
$Header: //components/rel/mcs.mpss/5.1/tcxomgr/src/tcxomgr_diag.c#2 $
...
tcxomgr_diag_type tcxomgr_diag;
...
FUNCTION TCXOMGR_DIAG_CAL_DBG_INIT

DESCRIPTION    This function is called when XO Calibration is started to
                request memory to store a circular buffer for debugging
                XO CAL issues (before device reset)
...
===========================================================================*/
static xo_cal_dbg_t xo_cal_dbg;
void tcxomgr_diag_cal_dbg_init (void)
{

  /*-----------------------------------------------------------------------*/

  if ( tcxomgr_diag.xo_cal_dbg != NULL )
  {
    /* memory is already initialized, no need to init again */
    return;
  }

  tcxomgr_diag.xo_cal_dbg = &xo_cal_dbg;

  if( tcxomgr_diag.xo_cal_dbg == NULL)
  {
    CAL_MSG_1(ERROR, "Unable to allocate %d bytes of memory for XO Cal Debug buffer",
        sizeof(xo_cal_dbg_t));
    return;
  }

  /* zero out data struct */
  memset( tcxomgr_diag.xo_cal_dbg, 0, sizeof( xo_cal_dbg_t ) );

  /* note: this memory is never cleared, on purpose */

} /* tcxomgr_diag_cal_dbg_init */

/*===========================================================================

FUNCTION TCXOMGR_DIAG_CAL_DBG_PRINT

DESCRIPTION    This function is prints out the circular buffer for debugging
                XO CAL issues (before device reset)
...
===========================================================================*/
static PACK(void *) tcxomgr_diag_cal_dbg_print
(
  PACK(void*) req_pkt,  /* request packet */
  uint16 pkt_len        /* request packet length */
)
{
  ...
  /* response pkt */
  DIAG_SUBSYS_TCXOMGR_XO_CAL_DBG_BUF_rsp_type *rsp_ptr;
  uint32  i,j;

  /*-----------------------------------------------------------------------*/

  /* Check if the request if valid */
  if(pkt_len != sizeof(DIAG_SUBSYS_TCXOMGR_XO_CAL_DBG_BUF_req_type))
  {
    MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,
           "Invalid diag request(%d)",
           pkt_len );
    return(NULL);
  }
  ...
  if ( tcxomgr_diag.xo_cal_dbg == NULL )
  {
    DBG_0( FATAL, "XO Cal DBG buffer == NULL" );

    return(NULL);
  }

  DBG_0( FATAL, "XO Cal DBG buffer ============" );
  for ( i = 1;
        i <= XO_CAL_DBG_SIZ;
        i++ )
  {
    j = (tcxomgr_diag.xo_cal_dbg->idx + i) % XO_CAL_DBG_SIZ;
    if ( tcxomgr_diag.xo_cal_dbg->buf[ j ].id != XCD_ID_BLANK )
    {
      DBG_5( FATAL, " [%4d]: 0x%2x [ 0x%10x, 0x%10x, 0x%10x ]",
             j,
             tcxomgr_diag.xo_cal_dbg->buf[ j ].id,
             tcxomgr_diag.xo_cal_dbg->buf[ j ].val_a,
             tcxomgr_diag.xo_cal_dbg->buf[ j ].val_b,
             tcxomgr_diag.xo_cal_dbg->buf[ j ].val_c );
    }
  }

  /* Allocate the memory for the response */
  rsp_ptr = (DIAG_SUBSYS_TCXOMGR_XO_CAL_DBG_BUF_rsp_type *)
            diagpkt_subsys_alloc((diagpkt_subsys_id_type)DIAG_SUBSYS_TCXOMGR,
                (diagpkt_subsys_cmd_code_type)XO_CAL_DBG_BUF,
              sizeof(DIAG_SUBSYS_TCXOMGR_XO_CAL_DBG_BUF_rsp_type));

  return( rsp_ptr );

} /* tcxomgr_diag_cal_dbg_print */
...
{
  /* request pkt */
  tcxomgr_diag_dc_coarse_cal_req_struct_type*          my_req_ptr;
  tcxomgr_diag_dc_coarse_cal_imm_rsp_struct_type*      imm_rsp_ptr = NULL;
  ...
  /* Check if the request if valid */
  if(pkt_len != sizeof(tcxomgr_diag_dc_coarse_cal_req_struct_type))
  {
    MSG_2(MSG_SSID_TCXOMGR, MSG_LEGACY_ERROR,
          "tcxomgr_diag_coarse_dc_cal_cmd: Invalid packet length: %d, Expected: %d",
          pkt_len, sizeof(tcxomgr_diag_dc_coarse_cal_req_struct_type));
    return NULL;
  }
  ...
  do
  {
    ...
    my_req_ptr = (tcxomgr_diag_dc_coarse_cal_req_struct_type *)req_pkt;
    ...
    imm_rsp_ptr = (tcxomgr_diag_dc_coarse_cal_imm_rsp_struct_type *)
                   diagpkt_subsys_alloc_v2 (
                     (diagpkt_subsys_id_type) DIAG_SUBSYS_TCXOMGR,
                     (diagpkt_subsys_cmd_code_type) COARSE_DC_CAL,
                     sizeof (tcxomgr_diag_dc_coarse_cal_imm_rsp_struct_type) );
...
)
{
  ...
  tcxomgr_diag_dc_coarse_cal_delayed_rsp_struct_type*  rsp_ptr = NULL;
  ...
  do
  {
    ...
    /* Allocate the memory for the response */
    rsp_ptr = (tcxomgr_diag_dc_coarse_cal_delayed_rsp_struct_type *)
              diagpkt_subsys_alloc_v2_delay (
                 (diagpkt_subsys_id_type) DIAG_SUBSYS_TCXOMGR,
                 (diagpkt_subsys_cmd_code_type) COARSE_DC_CAL,
                 tcxomgr_diag_cal_cmd_delay_rsp_id,
                 sizeof (tcxomgr_diag_dc_coarse_cal_delayed_rsp_struct_type) );
