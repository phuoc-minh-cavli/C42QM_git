...
$Header: //components/rel/mcs.mpss/5.1/tcxomgr/inc/tcxomgr_cal_i.h#4 $
...
#define CAL_IQ_SAMP_SIZE_4K       (4096)                         /* Size of the sram capture     */
#define CAL_IQ_SAMP_SIZE_8K       (CAL_IQ_SAMP_SIZE_4K * 2)      /* Size of the sram capture     */
#define CAL_IQ_SAMP_SIZE_16K      (CAL_IQ_SAMP_SIZE_4K * 4)      /* Size of the sram capture     */
...
#define CAL_FUNC_TRACE_DEBUG
#ifdef CAL_FUNC_TRACE_DEBUG
#define CAL_FUNC_TRACE( fmt )                                                 \
          MSG( MSG_SSID_TCXOMGR, MSG_LEGACY_LOW, fmt );
#else
#define CAL_FUNC_TRACE( fmt )
#endif
...
#define CAL_MSG_7( lvl, fmt, a1, a2, a3, a4, a5, a6, a7 )                     \
          MSG_7( MSG_SSID_TCXOMGR, lvl, fmt, a1, a2, a3, a4, a5, a6, a7 );
...
typedef enum
{
  TCXOMGR_TABLE_WWAN,
  TCXOMGR_TABLE_GPS,
  TCXOMGR_TABLE_MAX,    /* entries above this are used by Field cal */
  TCXOMGR_TABLE_ROT_WWAN,
  TCXOMGR_TABLE_SIZE
...
typedef struct
{
  uint32                      raw_sample_count;   /* Number of elements of
                                                     raw[] filled; varies
                                                     based on raw sample
                                                     format size              */
  uint16                      raw[ CAL_IQ_SAMP_SIZE_16K ];
                                                  /* raw sample data          */
  int16                       i[ CAL_IQ_SAMP_SIZE_8K ];
                                                  /* I data                   */
  int16                       q[ CAL_IQ_SAMP_SIZE_8K ];
                                                  /* Q data                   */
  int32                       p[ CAL_IQ_SAMP_SIZE_8K ];
  ...
  uint32                      sampling_freq_hz;   /* Sampling frequencey in Hz
                                                     used to perform sample capture.*/
...
/*---------------------------------------------------------------------------
  TCXO Manager's structure to represent DCC data in EFS
---------------------------------------------------------------------------*/
typedef PACK (struct)
{
  /* DCC data version */
  ...
  /* DCC register value at calibration */
  uint32                           dcc_reg_val;

  /* reserved fields */
  uint32                           reserved1;

} tcxomgr_efs_dcc_data_type;
...
FUNCTION tcxomgr_cal_nb1_init_capture
...
============================================================================*/
tcxomgr_cal_failure_enum_type  tcxomgr_cal_nb1_init_capture
(
  tcxomgr_cal_xo_cal_tech tech
);
...
FUNCTION tcxomgr_cal_nb1_capture_iq

DESCRIPTION
  This function captures the NB1 IQ samples.
...
============================================================================*/
tcxomgr_cal_failure_enum_type tcxomgr_cal_nb1_capture_iq
(
  tcxomgr_cal_xo_cal_tech tech,
  tcxomgr_cal_data_type *d
);
...
FUNCTION tcxomgr_cal_nb1_tear_capture
...
============================================================================*/
tcxomgr_cal_failure_enum_type tcxomgr_cal_nb1_tear_capture
(
  tcxomgr_cal_xo_cal_tech tech
);
...
FUNCTION TCXOMGR_CAL_COPY_TEMP

DESCRIPTION
  This function copies the temperatures between the two temp set types.
  This is needed to ensure efs and Cal interfaces do not change
...
============================================================================*/
void tcxomgr_cal_copy_temp
(
  tcxomgr_temp_set_type          temp,
  tcxomgr_cal_temp_set_type     *cal
);
...
FUNCTION tcxomgr_convert_sc8

DESCRIPTION
  This function copies samples in SC8 format modem firmware into the data
  structures used by XO calibration
...
============================================================================*/
void tcxomgr_convert_sc8(
    tcxomgr_cal_data_type       *d,             /* Out: formatted IQ samples */
    const uint16                *iqData,        /* Input: raw IQ samples */
...
DESCRIPTION
  This function copies samples in SC16 format modem firmware into the data
  structures used by XO calibration
...
FUNCTION tcxomgr_convert_pcfl20

DESCRIPTION
  This function converts samples in packed PCFL-20 format into 16-bit signed
  integers as expected by XO calibration

  On Bolt, LTE samples are in packed 20-bit PCFL format:
   - 4 bit exponent
   - 8 bit I
   - 8 bit Q
...
============================================================================*/
void tcxomgr_convert_pcfl20(
    tcxomgr_cal_data_type       *d,             /* Out: formatted IQ samples */
    const uint32                *pcfl_data_ptr, /* Input: raw IQ samples */
    uint32                      data_len,       /* Num of samples to format */
    uint32                      offset          /* Sample number to start at */
);
