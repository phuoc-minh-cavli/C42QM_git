...
$Header: //components/rel/mcs.mpss/5.1/tcxomgr/inc/tcxomgr_diag.h#3 $
...
typedef enum
{
  TCXOMGR_DIAG_MAX_EXTERNAL_ID,

  TCXOMGR_DIAG_START_INTERNAL_ID = 0xF0,
...
/*-------------------------------------------------------------------------
     XO CAL circular buffer for debug
-------------------------------------------------------------------------*/
/* Debug buffer message ID's */
typedef enum
{
  XCD_ID_BLANK,
  XCD_ID_END_XXXXXXXXXXXXXXXXX,

  XCD_ID_DC_C_START,
  XCD_ID_DC_C_XTAL,
  XCD_ID_SET_XO_TRIM,
  XCD_ID_SET_XO_TRIM_TS,
  XCD_ID_BUSY_WAIT_START,
  XCD_ID_BUSY_WAIT_END,
  XCD_ID_FREQ_ERR_EST_X,
  XCD_ID_FREQ_ERR_EST_P,
  XCD_ID_CALC_PHASE,
  XCD_ID_CALC_ERR,
  XCD_ID_FREQ_EST_RET,
  XCD_ID_GET_ERR_EST_PMIC_COMP,
  XCD_ID_CNS_XOT,
  XCD_ID_CNS_END,
  XCD_ID_CAL_DC_POST_SW,
  XCD_ID_CAL_DC_END,
  XCD_ID_DC_C_COMP,
  XCD_ID_DC_C_PASSED,
  XCD_ID_DC_C_END,
...
#define XO_CAL_DBG_SIZ 128

typedef struct
{
  uint32             idx;
  xo_cal_dbg_entry_t buf[XO_CAL_DBG_SIZ];
} xo_cal_dbg_t;

#define XO_CAL_DBG( ident, vala, valb, valc )                                                                           \
  {                                                                                                                     \
    if ( tcxomgr_diag.xo_cal_dbg != NULL )                                                                              \
    {                                                                                                                   \
      tcxomgr_diag.xo_cal_dbg->buf[ (tcxomgr_diag.xo_cal_dbg->idx % XO_CAL_DBG_SIZ) ].id = (ident);                     \
      tcxomgr_diag.xo_cal_dbg->buf[ (tcxomgr_diag.xo_cal_dbg->idx % XO_CAL_DBG_SIZ) ].val_a = (int32)(vala);            \
      tcxomgr_diag.xo_cal_dbg->buf[ (tcxomgr_diag.xo_cal_dbg->idx % XO_CAL_DBG_SIZ) ].val_b = (int32)(valb);            \
      tcxomgr_diag.xo_cal_dbg->buf[ (tcxomgr_diag.xo_cal_dbg->idx % XO_CAL_DBG_SIZ) ].val_c = (int32)(valc);            \
      tcxomgr_diag.xo_cal_dbg->idx++;                                                                                   \
      tcxomgr_diag.xo_cal_dbg->buf[ (tcxomgr_diag.xo_cal_dbg->idx % XO_CAL_DBG_SIZ) ].id = XCD_ID_END_XXXXXXXXXXXXXXXXX;\
    }                                                                                                                   \
  }
/*
  XO_CAL_DBG( XCD_ID_, 0, 0, 0 );
  v.v xo_cal_dbg_idx xo_cal_dbg
*/

/*-------------------------------------------------------------------------
     TCXOMGR Global struct
-------------------------------------------------------------------------*/
...
extern tcxomgr_diag_type tcxomgr_diag;
...
} tcxomgr_diag_dc_coarse_cal_req_struct_type;
...
} tcxomgr_diag_dc_coarse_cal_delayed_rsp_struct_type;

typedef PACK(struct)
{
  diagpkt_subsys_header_v2_type header;
} tcxomgr_diag_dc_coarse_cal_imm_rsp_struct_type;

/*--------------------------------------------------------------------------
                             XO_CAL_DBG_BUF print command

This interface provides a to print out the XO Cal Debug Circular Buffer.
--------------------------------------------------------------------------*/

DIAGPKT_SUBSYS_REQ_DEFINE(TCXOMGR, XO_CAL_DBG_BUF)
DIAGPKT_REQ_END

DIAGPKT_SUBSYS_RSP_DEFINE(TCXOMGR, XO_CAL_DBG_BUF)
DIAGPKT_RSP_END
...
===========================================================================*/
extern void tcxomgr_diag_cal_dbg_init (void);
