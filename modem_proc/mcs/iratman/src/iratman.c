...
$Header: //components/rel/mcs.mpss/5.1/iratman/src/iratman.c#1 $
...
)
{
  ...
  for( rat_idx = RAT_INT + 1; rat_idx < RAT_MAX; rat_idx++ )
  {
    ...
    if( ( node->hdr.rats_known & RAT_MASK( rat_idx ) ) &&
        ( node->data.lte.cc.meas_comp_algo[rat_idx] == IRATMAN_LTE_ALGO_RSRP )
      )
    {
      /* ----------------------- Validations ------------------------ */
...
*/
/*=========================================================================*/
STATIC void lte_process_meas_results
(
  iratman_rat_e_type          home_rat, //!< Home/Originating RAT
  iratman_rat_node_type*      node      //!< RAT node that was being measured
)
{
  ...
  meas_result = lte_meas_passed_thresh_criteria( home_rat, node );
  if( meas_result == IRATMAN_LTE_MEAS_RESULT_ABOVE_THRES_X )
  {
  ...
  if( curr_node != NULL )
  {
    ...
    else
    {
      ...
      if( rpt->lte_meas_cnf_params_ptr != NULL )
      {
        ...
        lte_process_meas_results( home_rat, curr_node );
...
)
{
  ...
  MSG_2( MSG_SSID_IRATMAN, MSG_LEGACY_HIGH,
         "Reporting meas results: home=%d, meas=%d",
         home_rat, meas_rat );
  ...
  switch( meas_rat )
  {
    case RAT_LTE:
    {
      ...
      lte_report_meas( home_rat, &node->lte, &report->lte );

      /* Check if any node passed reselection criteria */
      lte_check_reselection( home_rat, &node->lte, &ret_resel.lte );
...
)
{
  ...
  switch( meas_rat )
  {
    ...
    {
      /* Update node with measurement results */
      lte_report_meas( home_rat, &node->lte, NULL );
