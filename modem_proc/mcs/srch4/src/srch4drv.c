...
$Header: //components/rel/mcs.mpss/5.1/srch4/src/srch4drv.c#1 $
...
/*-----------------------------------------------------------------------*/
  ...
  "Search started with search type %d on queue %d with %d tasks, %d mode, Current Time: %ld", 
  queue->srch_type, 
  queue->queue_type, 
...
/*-----------------------------------------------------------------------*/
  ...
  if ( !cxm_tx_active )
  {
    ...
    if ( queue->repeat_search == TRUE )
    {
      ...
      MSG_1(MSG_SSID_DFLT, MSG_LEGACY_HIGH, "CXM is not active. \
  ...
  "Tasks Copied. Num Tasks Copied- %d, Num Total Tasks - %d, GTX Active %d",
  (fw_index), queue->num_tasks, cxm_tx_active);
}
...
#ifdef FEATURE_MODEM_COEXISTENCE_FW
    MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"lost dump initiated");
#endif
...
)
{
  ...
  else
  {
    ...
    {
      MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,
      "Schedule the entire search. Atomicity: %d, srch type: %d",
      atomic_search,queue->srch_type);
...
)
{
...
#endif
  {
    ...
    {
      MSG(MSG_SSID_DFLT, MSG_LEGACY_FATAL, "No results obtained and reprogramming all tasks");
      stored_results = num_results;
    }
#endif
...
)
{
  ...
  if ( queue != NULL )
  {
    ...
    if (!(IS_QUEUE_ACTIVE(queue)))
    {
      MSG(MSG_SSID_DFLT, MSG_LEGACY_FATAL, "Redundant Process Dump due to abort/dump-complete race condition");
      return;
    }
...
#endif	
    if ( num_results == search_rsp->numResults )
    {
      MSG_2(MSG_SSID_DFLT, MSG_LEGACY_FATAL,
      "No over-run in search scheduling. Num Results: %d, Search Mode: %d",
      num_results, queue->mode);
    }
...
#ifdef FEATURE_MODEM_COEXISTENCE_FW
    MSG_4(MSG_SSID_1X_SRCH, MSG_DBG_SRCH4,
    "Process dump initiated for tech %d, queue %d, num res %d, current time(USTMR) %ld",
    queue->srch_type, queue_id, num_results, srch4i_slottedscheduler_get_current_ustmr());
#else
    MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Process dump initiated for tech %d, queue %d",
    ...
    if ( dump != SRCH4_NO_DUMP)
    {
      ...
      {
        MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Srch4 successfully aborted for tech %d on %d",
        queue->srch_type, queue_id);
      ...
      if ( lost )
      {
        ...
        if ( queue->lost_cb_func != NULL )
        {
          /* Call the callback */
          (*queue->lost_cb_func)( queue->srch_cb_data );
          MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Lost callback initiated for tech %d, queue %d",
          tech, queue_id);
        }
      }
      else if ( aborted )
      {
        ...
        if ( queue->abort_cb_func != NULL )
        {
          ...
          MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Abort callback initiated for tech %d, queue %d",
          tech, queue_id);
        }
      }
      else /* normal status dump */
      {
		MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"normal status dump, MODEM_COEXISTENCE_FW");
      ...
      {
	MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"normal status dump");
        /* call the callback function */
        if ( queue->dump_cb_func != NULL )
        {
          (*queue->dump_cb_func)( queue->srch_cb_data );
          MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Dump callback initiated for tech %d, queue %d",
          tech, queue_id);
        }
      }
#endif
