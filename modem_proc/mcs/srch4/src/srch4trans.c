...
$Header: //components/rel/mcs.mpss/5.1/srch4/src/srch4trans.c#1 $
...
)
{
  ...
  if ( enable )
  {
    ...
    MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH, 
                  "MDSP SRCH4 APP -> enable (tech=%d, slp_mask=0x%4x)",
                   tech, srch4trans.mdsp.sleep_mask);
  }
  else
  {
    /* clear the bit */
    srch4trans.mdsp.sleep_mask &= ~( (uint32)tech );
    MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH, 
                  "MDSP SRCH4 APP -> disable(tech=%d, slp_mask=0x%4x)",
                   tech, srch4trans.mdsp.sleep_mask);
  }
...
)
{
         ...
         (uint32)SRCH4_1X_OFFTL_MASK ) )
  {
    ...
    {
       MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
...
int32 srch4_1x_offtl_register( void )
{
  ...
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH,"1x offtl_qpch reg srch4 (prev %d)",
...
void srch4_1x_offtl_deregister ( void )
{
  ...
  MSG_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "1x offtl_qpch dereg srch4 %d",
         srch4trans.queue.tech[ SRCH4_1X ].queue );
...
boolean srch4_1x_active( void )
{
  ...
  MSG_2(MSG_SSID_DFLT, MSG_LEGACY_HIGH,
        "queue=%d, tasks_left=%d",srch4trans.queue.tech[ SRCH4_1X ].queue,
        srch4_tasks_left( SRCH4_1X,
        srch4trans.queue.tech[ SRCH4_1X ].queue ));
...
)
{
  ...
  {
    MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "NULL pointer received in 1X start search");
    return drv_ret_val;
  }
...
)
{
  ...
  MSG_1(MSG_SSID_DFLT, MSG_LEGACY_HIGH, "1X Abort return value : %d", 
...
)
{
  ...
  if ( task_res == NULL )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_FATAL, "Trying to read invalid task" );
  }
...
)
{
  ...
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "1x NOT registered" );
    return( SRCH4_RET_NO_QUEUES );
  }
...
)
{
  ...
  if (srch_details == NULL)
  {
    MSG(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "Null pointer passed to HDR start srch");
    return drv_ret_val;
  }
...
)
{
  ...
  if ( queue == SRCH4_NO_QUEUE )
  {
    ...
    if ( queue == SRCH4_NO_QUEUE )
    {
      MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "No available queues (HDR)" );
      return( SRCH4_RET_NO_QUEUES );
    }
  }
  ...
  if ( task == NULL )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_FATAL, "Invalid queue (HDR)" );
    return( SRCH4_RET_INVALID_QUEUE );
  }
...
int32 srch4_aflt_continue_search( void )
{
  ...
  if ( queue == SRCH4_NO_QUEUE )
  {
    MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "AFLT NOT registered" );
    return( SRCH4_RET_NO_QUEUES );
  }
  ...
  while ( srch4trans.aflt.task_num > 0 )
  {
    ...
    if (tasks_left > 0)
    {
      ...
      if ( task == NULL )
      {
        MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Invalid queue (AFLT)" );
        return( SRCH4_RET_INVALID_QUEUE );
      }
...
#ifdef FEATURE_MODEM_COEXISTENCE_FW
  SRCH4_ENTER_CRIT_SECT();
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"Inactivity table invoked");
