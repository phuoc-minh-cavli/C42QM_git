...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/wwan_coex/src/wwan_coex_list.c#1 $
...
)
{
  ...
  if (i < WWAN_COEX_MAX_COMBS)
  {
    ...
    if (wwcoex_list_comb_table[i].comp_channel_func != NULL) 
    {
      ...
      compare_out->action = wwcoex_get_mitigation_action(compare_in->atech, compare_in->vtech);
      LMTSMGR_MSG_4(MED, "Found nonIM comp func %d: tech_band_mask 0x%x desense 0x%x aciton 0x%x", 
                         i, wwcoex_list_comb_table[i].tech_band_mask,
                         desense, compare_out->action );
    }
    else
    {
      LMTSMGR_MSG_3(MED, "Found nonIM combo index %d: tech_band 0x%x, desense 0x%x",
                       i, wwcoex_list_comb_table[i].tech_band_mask, 
