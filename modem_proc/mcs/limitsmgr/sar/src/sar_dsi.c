...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/sar/src/sar_dsi.c#2 $
...
)
{
  ...
  sar_get_usb_conn_state_resp_msg_v01 get_usb_state;
  /*-----------------------------------------------------------------------*/
  ...
  switch( msg_id )
  {
    ...
    case QMI_SAR_RF_SET_STATE_REQ_MSG_V01:
     ...
     case QMI_SAR_SET_USB_CONN_STATE_REQ_V01:
     {
       usb_is_connected = ((sar_set_usb_conn_state_req_msg_v01 *)req_c_struct)->usb_is_connected;

       LMTSMGR_MSG_1(HIGH, "Received QMI USB conn indication, state %d", usb_is_connected);

       response.result = QMI_RESULT_SUCCESS_V01;
       lmtsmgr_vbatt_set_usb_conn_state(usb_is_connected);

       /* send out response */
       req_cb_retval   = mqcsi_conn_mgr_send_resp_from_cb(
          connection_handle,
          req_handle,
          QMI_SAR_SET_USB_CONN_STATE_RESP_V01,
          &response,
     ...
     case QMI_SAR_GET_USB_CONN_STATE_REQ_V01:
     {
       /* Initialize structure holding response */
       memset( &get_usb_state, 0, sizeof( get_usb_state ) );

       /* Fill in the usb connection info and the response */
       get_usb_state.usb_is_connected = lmtsmgr_vbatt_get_usb_conn_state();
       get_usb_state.usb_is_connected_valid = TRUE;
       get_usb_state.resp.result = QMI_RESULT_SUCCESS_V01;

       /* send out response */
       req_cb_retval   = mqcsi_conn_mgr_send_resp_from_cb(
          connection_handle,
          req_handle,
          QMI_SAR_GET_USB_CONN_STATE_RESP_V01,
          &get_usb_state,
          sizeof( sar_get_usb_conn_state_resp_msg_v01 )
          );

       if( req_cb_retval == QMI_CSI_CB_NO_ERR )
       {
         LMTSMGR_MSG_1(MED, "Sent USB conn state Response to the Client %d", get_usb_state.usb_is_connected);
       }
       else
       {
         LMTSMGR_MSG_1(ERROR, "Could not send USB conn state response to the Client %d", req_cb_retval);
       }
     }
     break;
