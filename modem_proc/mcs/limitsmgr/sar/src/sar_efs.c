...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/sar/src/sar_efs.c#2 $
...
)
{
  ...
  /* Copy the EFS record onto heap */
  for (i = 1; i < MAX_DSI_LUT_ROWS; i++)
  {
    for (j = 0; j < MAX_DSI_LUT_COLUMNS; j++)
    {
      dsi_bias_lut->bias[i][j] = dsi_bias_rec->bias[i-1][j];
    }
  }
