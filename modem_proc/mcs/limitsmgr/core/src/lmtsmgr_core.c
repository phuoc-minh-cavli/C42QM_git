...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/core/src/lmtsmgr_core.c#5 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
11/21/16   jm      Featurize band avoidance processing
10/26/16   jm      KW fixes due to Single Sim Max Capability (continued)
10/04/16   jm      KW fixes due to Single Sim Max Capability
07/21/16   jm      Eliminate TRM calls in low memory feature
...
lmtsmgr_prio_criteria_type lmtsmgr_get_prio_criteria ( void )
{
  ...
  {
    /* we are in data + data High combination */
...
void lmtsmgr_update_client_prio_table ( void )
{
  ...
  if ((criteria != PRIO_CRITERIA_NONE) && (MAX_NUM_OF_SUBSCRIPTIONS > 1))
  {
                                                   ...
                                                   sp_client_enum_t client_id)
{
  ...
  {
    LMTSMGR_MSG_1(HIGH, "lmtsmgr_subs_prio_callback: id %d", client_id);
...
void lmtsmgr_sp_callback_process_input(void)
{
  ...
  LMTSMGR_MSG_2(HIGH, "Subs_Prio->SAR: num: %d, activeMask: %d",
  ...
  {
    LMTSMGR_MSG_3(HIGH, "Subs_Prio->SAR: num: %d, techMask[%d] %d",
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      LMTSMGR_MSG_4(ERROR, "Tech %d macro st %d, Tech %d macro st %d have equal prio",
                    tech1_prio->tech, tech1_macro_state,
                    tech2_prio->tech, tech2_macro_state);
...
)
{
  ...
  if (new_num_ul > 0)
  {
    ...
    lmtsmgr.tech_state[tech_id].pwr_info.new_limits.vbatt_plimit=DEFAULT_MAX_PLIMIT;
...
void lmtsmgr_process_coex_tech_state_change (void)
{
  ...
  LMTSMGR_MSG_1(MED, "Coex tech st ind: num_active_techs %d", num_active_techs);
  ...
  for (i=0; i<num_active_techs; i++)
  {
...
#endif /* Antenna Switch Div feature */
    LMTSMGR_MSG_6(MED, "Coex tech st ind: [%d] = Tech %d, as_id %d, state %d, tx %d, rx %d",
    ...
    LMTSMGR_MSG_6(MED, "Coex tech st ind: [%d] = distress_st %d, ul_bands[0]=%d, dl_bands[0]=%d, num_ul_bands=%d, num_dl_bands=%d",
                  i,
