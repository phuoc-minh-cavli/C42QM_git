...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/core/src/lmtsmgr_cmd.c#5 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
03/02/16   jm      Featurize GSM conversion call in Band Avoidance
...
)
{
  ...
  /* Process the vbatt based on tech and sub_tech */
  if (tech_id == CXM_TECH_LTE)
  {
    lmtsmgr_process_vbatt_freq_input(tech_id,
         lmtsmgr_target_tech->tech_data.params.lte_params.sub_tech);
  }
  else
  {
    lmtsmgr_process_vbatt_freq_input(tech_id, CXM_LTE_SUB_INVALID);
  }
...
)
{
  ...
  LMTSMGR_MSG_2(MED, "Received power update for tech %d num entries %d",
...
)
{
  ...
  memscpy(&lmtsmgr_timing_info->dtx_info, sizeof(lmtsmgr_timing_info->dtx_info),
          &lmtsmgr_target_tech->dtx_info, sizeof(lmtsmgr_target_tech->dtx_info));
...
)
{
  ...
  for (entry_pos=0; entry_pos<lmtsmgr_target_tech->serv_freq_data.num_entries; entry_pos++)
  {
    ...
    {
      LMTSMGR_MSG_2(ERROR, "Invalid direction %d! Pos: %d", msgr_serv_freq_entry->direction, entry_pos);
      return;
    }
  } /* End For */
