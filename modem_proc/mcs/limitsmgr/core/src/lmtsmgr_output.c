...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/core/src/lmtsmgr_output.c#5 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
06/07/18   rj      Added feature flag for W and TDS
...
void lmtsmgr_msgr_lte_pwrlim_trans ( void )
{
  ...
  int16 sar_plimit, coex_plimit, coex_slot_plimit, vbatt_plimit;
  ...
  vbatt_plimit = lmtsmgr.tech_state[CXM_TECH_LTE].pwr_info.curr_limits.vbatt_plimit;
  ...
  #ifdef RFA_RF_LTE_VBATT_BACKOFF_MASK
  /* Vbatt limiting is not available in all versions of the RF API.
   * Conditionally compile this interface code until the API is available. */
  transmit_limits.sg_txpl_params.plimit_type_mask |= RFA_RF_LTE_VBATT_BACKOFF_MASK;
  transmit_limits.sg_txpl_params.vbatt_plimit = vbatt_plimit;

  /* Message indicating we are sending */
  LMTSMGR_MSG_5(HIGH, "Sending limits %d (sar) %d (coex) %d (subframe) %d (vbatt) to tech %d",
                  ...
                  transmit_limits.sg_txpl_params.vbatt_plimit,
                  CXM_TECH_LTE );

  #else /* !RFA_RF_LTE_VBATT_BACKOFF_MASK */
  ...
  LMTSMGR_MSG_4(HIGH, "Sending limits %d (sar) %d (coex) %d (subframe) to tech %d",
                  ...
                  transmit_limits.sg_txpl_params.subframe_coex_plimit,
                  CXM_TECH_LTE );
  #endif /* RFA_RF_LTE_VBATT_BACKOFF_MASK */
...
)
{
  ...
  if (lmtsmgr.tech_state[tech].currList.num_ul_entries == 0)
  {
    ...
    pwr_info->new_limits.vbatt_plimit = DEFAULT_TXSTOP_LIMIT;
    ...
    pwr_info->curr_limits.vbatt_plimit = DEFAULT_TXSTOP_LIMIT;
  ...
  if ( (pwr_info->curr_limits.num_valid_sar == pwr_info->new_limits.num_valid_sar) &&
       ...
       (pwr_info->curr_limits.vbatt_plimit == pwr_info->new_limits.vbatt_plimit) )
  {
