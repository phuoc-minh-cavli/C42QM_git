...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/core/inc/lmtsmgr_diag.h#3 $
...
/* Diag command request to set the active tx for a tech, to test the vbatt
 * core algorithm in cases if tx can't be easily brought up */
DIAGPKT_SUBSYS_REQ_DEFINE(LIMITSMGR, VBATT_SET_ACTIVE_TX )
  uint8  tech_id;  /* Tech ID */
  uint8  sub_tech; /* Sub tech(CATM1 = 1, NB1 = 2) valid for LTE */
  uint16 ul_band;  /* Tech operating UL band */
  uint16 ul_freq;  /* Tech operating up-link frequency in MHz */
  uint16 ul_bw;    /* Tech operating up-link bandwidth in KHz */
  uint16 dl_band;  /* Tech operating DL band */
  uint16 dl_freq;  /* Tech operating down-link frequency in MHz */
  uint16 dl_bw;    /* Tech operating down-link bandwidth in KHz */
DIAGPKT_REQ_END

/* Diag command response to set active tx */
DIAGPKT_SUBSYS_RSP_DEFINE(LIMITSMGR, VBATT_SET_ACTIVE_TX )
