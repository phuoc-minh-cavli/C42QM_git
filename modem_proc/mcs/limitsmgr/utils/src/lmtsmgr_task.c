...
$Header: //components/rel/mcs.mpss/5.1/limitsmgr/utils/src/lmtsmgr_task.c#3 $
...
)
{
  ...
  while (1)
  {
    ...
    /* Process VBATT update */
    if ( curr_sigs_set & LMTSMGR_SAR_VBATT_SIG )
    {
      lmtsmgr_process_vbatt_event();
    }

    /* Process VBATT USB update */
    if (curr_sigs_set & LMTSMGR_SAR_VBATT_USB_SIG)
    {
      lmtsmgr_process_vbatt_usb_sig();
    }
