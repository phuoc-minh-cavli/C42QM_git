...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/api/memory/dsm_item.h#1 $
...
#else
  uint16               references; /* Reference count (ie duplicate         */
                                  /* count)                                */
  uint16 reserved_16; /*Reserved for future library use */
  uint8 reserved_8; /*Reserved for future library use */
#endif /* FEATURE_DSM_LOCKLESS */
...
static __inline uint32 dsm_item_get_time_stamp(
  const dsm_item_type* item
)
{
    return  (uint32)q_get_usr_data_s(&item->link);
}
