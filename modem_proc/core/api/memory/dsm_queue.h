...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/api/memory/dsm_queue.h#1 $
...
typedef struct dsm_watermark_type_s
{
  ...
  uint32  lo_watermark_q_cnt;	/* Low watermark for WM packet count (q_cnt) */
  uint32  hi_watermark;		/* Hi watermark for total bytes */
  uint32  hi_watermark_q_cnt;	/* Hi watermark for WM packet count (q_cnt) */
  ...
  wm_cb_type lowater_q_cnt_func_ptr;	/* Lo Watermark function pointer for DSM packets (q_cnt), Invoke
                                    function when lo_watermark_q_cnt is hit for number of packets     */								
  void * lowater_q_cnt_func_data;
  ...
  wm_cb_type hiwater_q_cnt_func_ptr;	/* Hi Watermark function pointer for DSM packets (q_cnt), Invoke
                                    function when hi_watermark_q_cnt is hit for number of packets     */								
  void * hiwater_q_cnt_func_data;
