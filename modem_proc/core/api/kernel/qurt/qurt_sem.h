...
/**@ingroup func_qurt_sem_down_timed  
  ...
  - If the count value is zero it is not decremented, and the thread is suspended on the
     semaphore. When the count value becomes nonzero (because another thread
     released the semaphore) it is decremented, and the suspended thread is awakened
     and gains access to the shared resource.The wait shall be terminated when the specified timeout expires
   If timeout expires  this wait shall be terminated and no access to the shared resource is granted.
