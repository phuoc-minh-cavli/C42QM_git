...
===============================================================================
 ...
 $Header: //components/rel/core.mpss/3.10/hwengines/bam/bam.c#2 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
10/31/18   AY      EOT is being ignored for MSS CE BAM's descriptor queued on Producer pipe.
...
#define BAM_MAX_NUM_ITER_EOT_CHECK 100
...
BAM_API_NON_PAGED void bam_pipe_eot_handler(pipe_dev *pipe,   bam_result_type *pollresult)
{
    ...
    volatile bam_iovec_type *iovec;
    uint32 num_results = 0, count= 0;
    ...
    while(pipe->acked_offset != endoffset)
    {
		...
		if (pipe->flags & BAM_FLAG_CHECK_EOT)
			{
			    do
				{
				...
				} while (count++ < BAM_MAX_NUM_ITER_EOT_CHECK);
                
			    if (count == BAM_MAX_NUM_ITER_EOT_CHECK)
			    {
				   ERR_FATAL("bam_pipe_eot_handler: EOT flag not set in desc for %d checks\n!", count, 0, 0);
			    } 
...
bam_handle bam_pipe_init(bam_handle bamhandle, uint32 pipe_num, bam_pipe_config_type *pipe_cfg, bam_callback_type *pipe_cb)
{
	...
	//Updating pipe flags for Producer pipe of MSS CE BAM 
	if ((pipe->pipe_cfg.dir == BAM_DIR_PRODUCER) && (bam->bam_tgt_cfg->options & BAM_TGT_CFG_CHECK_DESC_EOT))
	{
		pipe->flags = (pipe->flags | BAM_FLAG_CHECK_EOT);
	}
...
BAM_API_NON_PAGED bam_status_type bam_pipe_transfer(bam_handle pipehandle, uint32 buf_pa, uint16 buf_size, uint16 xfer_opts, void *user_data)
{
	...
	//Ignoring the EOT flag for descriptor queued on Producer pipe of MSS CE BAM
	if (pipe->flags & BAM_FLAG_CHECK_EOT)
	{
