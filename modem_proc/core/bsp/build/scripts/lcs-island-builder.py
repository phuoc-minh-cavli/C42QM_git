...
'''
    ...
    qurtisland.create_lcs(output_file_name, template_file_name, None, spec_dict, island_spec_file_name)
'''
...
def create_lcs(linker_script_name, template_script_name, full_file_names, island_dict, *spec_file_list, **kw):
    '''
    ...
    # Define a dictionary that will contain two unique keys per valid segment,
    # one for the '_sections' and one for '_specs'.  Originally, there was two
    # list object defined for the 'island' segment, now we have 2*N list objects,
    # where N is the number of valid segments for the image.
    link_dict = dict()
    
    # Instead of hard-coding the list of potential anchors, the list is generated
    # based on the valid segments passed into this script. 
    anchors = []
    
    # Add the required kernel anchors.
    anchors.append('QURT_ISLAND_UKERNEL')
    anchors.append('QURT_MAIN_UKERNEL')
    anchors.append('QURT_START')
    anchors.append('START_SECTION')
           
    # Define two list objects per valid segments in the dictionary.
    for segments in island_dict.keys():
      link_dict[segments + '_sections'] = []
      link_dict[segments + '_specs'] = []
      
      modified_segments = segments.upper()
      anchors.append(modified_segments + '_EXCLUDE')
      anchors.append(modified_segments + '_TEXT')
      anchors.append(modified_segments + '_RODATA')
      anchors.append(modified_segments + '_DATA')
      anchors.append(modified_segments + '_BSS')
      anchors.append(modified_segments + '_RO_SECTIONS')
      anchors.append(modified_segments + '_RW_SECTIONS')
      
    import pdb
    import os
    for fname in spec_file_list:
        f = open(fname, 'r')
        # Determine the segment that this spec file is within.  The name of the 
        # spec file must be in one of values assigned to a key (valid segments)
        # in the dictionary.  Otherwise, we don't know where to place it.
        current_segment = ''
        for segment in island_dict.keys():

         if os.path.basename(fname) in island_dict[segment]:
            current_segment = segment
        
        if current_segment is '':
         raise Exception('The ' + os.path.basename(fname) + ' specification file is not assigned to a valid segment.')
        ...
        for specline in f:
            ...
            if len(temp):
                if temp[0].startswith('#'):
                    ...
                    if temp[2] != '*':
                        # Add the entry to the '_sections' list object for the current segment.
                        link_dict[current_segment + '_sections'].append(temp)
                    else:
                        # Add the entry to the '_specs' list object for the current segment.
                        link_dict[current_segment + '_specs'].append(temp)
                else:
    ...
    for segment in island_dict.keys():
      link_dict[segment + '_names'] = []  # Declare it as empty list object in the dictionary.
      link_dict[segment + '_names'] = sorted(set([s[2] for s in link_dict[segment + '_sections']]))
    ...
    #
    regex_string = '(/\*\\s*(?:%s)\\s*\*/)' % '|'.join(anchors)
    ...
    while len(tokens) >= 2:
        ...
        if 'QURT_ISLAND_UKERNEL' in tokens[1]:
            if link_dict['island_sections'] or link_dict['island_specs']:
            ...
            for i in link_dict['island_sections']:
                if i[0] == '*' and i[1] != '*':
                    # Single object file
                    script_file += '      %s\n' % cpath(i[1])
                elif i[1] == '*' and i[0] != '*':
            ...
            # Determine what the current segment is in tokens[1] by checking 
            # the beginning of the string.
            valid_segment = ''
            for segments in island_dict.keys():
               if segments.upper() in tokens[1].split('_')[0]:
                  valid_segment = segments
            for nm in link_dict[valid_segment + '_names']:
                ...
                script_file += '   %s.IS.%s : %s{\n' % (nm,valid_segment,section_constraint)
                for i in link_dict[valid_segment + '_sections']:
                    if i[2] == nm:
                        if i[0] == '*' and i[1] != '*':
                            # Single object file
                            script_file += '      %s(%s)\n' % (cpath(i[1]), nm)
            ...
            # What segment did we find?
               valid_segment = ''
               for segments in island_dict.keys():
                  if segments.upper() + '_TEXT' in tokens[1]:
                      if 'ISLAND_TEXT' in tokens[1]:
					    script_file += ' *(.text.qurtos.island)\n'  
                      sections = ['.text .text.*']
                      valid_segment = segments
                      break;
                  elif segments.upper() + '_RODATA' in tokens[1]:
                      if 'ISLAND_RODATA' in tokens[1]:
                          script_file += ' *(.rodata.qurtos.island)\n'  
                      sections = ['.rodata .rodata.*']
                      valid_segment = segments
                      break;
                  elif segments.upper() + '_DATA' in tokens[1]:
                      if 'ISLAND_DATA' in tokens[1]:
                         script_file += ' *(.data.qurtos.island)\n'  
                      sections = ['.data .data.*', '.sdata .sdata.*']
                      valid_segment = segments
                      break;
                  elif segments.upper() + '_BSS' in tokens[1]:
                      if 'ISLAND_BSS' in tokens[1]:
                          script_file += ' *(.bss.qurtos.island)\n'  
                      sections = ['.bss .bss.* COMMON','.sbss .sbss.* .scommon .scommon.*']
                      valid_segment = segments
                      break;
               # Start writing the entries into the linker script.
               script_file += ' /*START_' + valid_segment.upper() + '*/\n'
               for sec in sections:
                   # Only iterate through the list object for the valid segment key.
                   for i in link_dict[valid_segment + '_specs']:
                       if i[0] == '*' and i[1] != '*':
                           # Single object file
                           script_file += '      %s(%s)\n' % (cpath(i[1]),sec)
                       elif i[1] == '*' and i[0] != '*':
                           ...
                           # Object within a library
                           script_file += '      %s:%s(%s)\n' % (cpath(i[0]), i[1], sec)
               # Finish writing the entries into the linker script.
               script_file += ' /*END_'+valid_segment.upper()+'*/ '
        tokens = tokens[2:]
