...
#===============================================================================
# Edit History
# $Header: //components/rel/core.mpss/3.10/bsp/build/scripts/tms_builder_rcinit.py#3 $
# $DateTime: 2019/04/16 02:55:23 $
# $Change: 18903933 $
# $Author: pwbldsvc $
#===============================================================================

# Module Version, Python Standard PEP8, 2001, https://www.python.org/dev/peps/pep-0008/
__version__ = "$Header: //components/rel/core.mpss/3.10/bsp/build/scripts/tms_builder_rcinit.py#3 $"
...
def tool_action(target, source, env):
   ...
   RCINIT_PREV_STACK_SIZE_DYNAMIC = 0                      # internal stats
   RCINIT_PREV_STACK_SIZE_STATIC = 0                       # internal stats
   RCINIT_PREV_STACK_SIZE_AUTO = 0                         # internal stats
   RCINIT_PREV_STACK_SIZE_TOTAL = 0                        # internal stats
   ...
   #Get Statistics before patching   
   ...
   for entry in rcinit_ents:
      ...
      if entry[RCINIT_INFO_API] == THREAD_API_FUNC or entry[RCINIT_INFO_API] == THREAD_API_POLICY:
         pass
      # API is DAL Workloop
      elif entry[RCINIT_INFO_API] == THREAD_API_DAL or entry[THREAD_API] == THREAD_API_POSIX or entry[THREAD_API] == THREAD_API_QURT or entry[THREAD_API] == THREAD_API_REX:
         # Static Stack
         RCINIT_PREV_STACK_SIZE_TOTAL = RCINIT_PREV_STACK_SIZE_TOTAL + env.TmsToInt(entry[RCINIT_INFO_STACK]) + RCINIT_STACK_OVERHEAD
         if entry[RCINIT_INFO_STACK_TYPE] == RCINIT_STACK_TYPE_STATIC and entry[RCINIT_INFO_ENTRY] != RCINIT_NULL:
            RCINIT_PREV_STACK_SIZE_STATIC = RCINIT_PREV_STACK_SIZE_STATIC + env.TmsToInt(entry[RCINIT_INFO_STACK]) + RCINIT_STACK_OVERHEAD
         elif entry[RCINIT_INFO_STACK_TYPE] == RCINIT_STACK_TYPE_AUTO and entry[RCINIT_INFO_ENTRY] != RCINIT_NULL:
            RCINIT_PREV_STACK_SIZE_AUTO = RCINIT_PREV_STACK_SIZE_AUTO + env.TmsToInt(entry[RCINIT_INFO_STACK]) + RCINIT_STACK_OVERHEAD
         else:
            RCINIT_PREV_STACK_SIZE_DYNAMIC = RCINIT_PREV_STACK_SIZE_DYNAMIC + env.TmsToInt(entry[RCINIT_INFO_STACK]) + RCINIT_STACK_OVERHEAD


   #Apply Patching
   for entry in env_rcinit[RCINIT_INFO_LIST_PATCH]:
      if entry.has_key(RCINIT_INFO_TYPE):
...
/*handle*/     &rcinit_rw_{0:s}
   ...
   if (RCINIT_STACK_SIZE_TOTAL != RCINIT_PREV_STACK_SIZE_TOTAL):
     PlaybookOutputLog.append('Build Stack Allocation ESTIMATES before patching {:s} Total, {:s} Dynamic, {:s} Static, {:s} Auto\n'.format(
        str(roundup(RCINIT_PREV_STACK_SIZE_TOTAL, RCINIT_STACK_ROUNDUP)),
        str(roundup(RCINIT_PREV_STACK_SIZE_DYNAMIC, RCINIT_STACK_ROUNDUP)),
        str(roundup(RCINIT_PREV_STACK_SIZE_STATIC, RCINIT_STACK_ROUNDUP)),
        str(roundup(RCINIT_PREV_STACK_SIZE_AUTO, RCINIT_STACK_ROUNDUP))))
