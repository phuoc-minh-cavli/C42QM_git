...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/fds/src/fds.c#2 $ $DateTime: 2019/06/19 10:03:46 $ $Author: pwbldsvc $
...
typedef struct
{
  ...
  fds_disable_scrub_resp_msg               scrub_disable_scrub_resp_msg;
  fds_enable_scrub_resp_msg                scrub_enable_scrub_resp_msg;
...
fds_scrub_disable_scrub (qmi_req_handle req_handle, unsigned int msg_id,
                         void *req_struct, unsigned int req_struct_len);

static qmi_csi_cb_error
fds_scrub_enable_scrub (qmi_req_handle req_handle, unsigned int msg_id,
                        void *req_struct, unsigned int req_struct_len);
                    ...
                    unsigned int req_struct_len, void *service_cookie)
{
  ...
  switch (msg_id)
  {
    ...
    case FDS_DISABLE_SCRUB_REQ:
      err = fds_scrub_disable_scrub (req_handle, msg_id, req_struct, req_struct_len);
    break;

    case FDS_ENABLE_SCRUB_REQ:
      err = fds_scrub_enable_scrub (req_handle, msg_id, req_struct,
	                                   req_struct_len);
    break;
...
{
  uint32 req_msg_len, resp_msg_len;
  fds_disable_scrub_resp_msg *resp_msg;
  qmi_csi_error service_err;
  int result;
  qmi_csi_cb_error cb_err = QMI_CSI_CB_INTERNAL_ERR;

  req_msg_len  = sizeof (fds_disable_scrub_req_msg);
  resp_msg_len = sizeof (fds_disable_scrub_resp_msg);

  resp_msg = &(fds_inst.fds_scrub_inst.scrub_disable_scrub_resp_msg);
  memset (resp_msg, 0, resp_msg_len);
  ...
  result = flash_scrub_disable_scrub ();
  if (result != 0)
  {
    FDS_MSG_ERR_1 ("%d, FDS: Scrub disabling scrub failed [result]", result);
    resp_msg->resp.error = fds_map_scrub_error_to_fds_error (result);
    goto END;
  }
                        ...
                        void *req_struct, unsigned int req_struct_len)
{
  ...
  int recompute_md5, result;
  uint32 req_msg_len, resp_msg_len;
  fds_enable_scrub_resp_msg *resp_msg;
  fds_enable_scrub_req_msg *req_msg;
  qmi_csi_error service_err;
  qmi_csi_cb_error cb_err = QMI_CSI_CB_INTERNAL_ERR;

  req_msg_len  = sizeof (fds_enable_scrub_req_msg);
  resp_msg_len = sizeof (fds_enable_scrub_resp_msg);

  resp_msg = &(fds_inst.fds_scrub_inst.scrub_enable_scrub_resp_msg);
  memset (resp_msg, 0, resp_msg_len);
  ...
  req_msg = (fds_enable_scrub_req_msg *) req_struct;
  recompute_md5 = req_msg->recompute_md5;
  result = flash_scrub_enable_scrub (recompute_md5);
  if (result != 0)
  {
    FDS_MSG_ERR_1 ("%d, FDS: Scrub enabling scrub failed [result]", result);
    resp_msg->resp.error = fds_map_scrub_error_to_fds_error (result);
    goto END;
  }
