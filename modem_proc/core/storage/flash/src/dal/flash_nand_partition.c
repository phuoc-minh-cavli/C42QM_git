 ...
 *
 * Copyright (c) 2008-2014, 2017-2018 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.10/storage/flash/src/dal/flash_nand_partition.c#2 $ $DateTime: 2018/10/31 09:47:12 $ $Author: pwbldsvc $
 *
 *
 ...
 * 10/27/17     svl     Fix mibib age initialization
  ...
  const unsigned char *name)
{
  ...
  {
    if (strncmp((const char *)"0:EFS2", (const char *)name, FLASH_PART_NAME_LENGTH) == 0)
    {
      if (strncmp(partentry->name, (const char *)"efs2", 
        FLASH_PART_NAME_LENGTH) == 0)
      {
        return TRUE;
      }
