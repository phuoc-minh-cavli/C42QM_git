 ...
 *
 * Copyright (c) 2008-2014, 2016, 2019-2020 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 02/09/20     sk		Fix KW issue
 * 06/18/21     sl      error fatal when dal fw returns no mem
 * 01/01/19     sl      Spi NAND support
 * 10/12/16     eo      nand driver multi-client support
 * 11/16/16     sb      Fix synchornization isssue in block_set_state API
...
int nand_entry_mem_alloc(uint32 hal_mem_size, uint32 dal_mem_size)
{
  ...
  if ((FLASH_DEVICE_DONE == result) && hal_mem_size)
  {
    ...
    else
    {
       ...
       *  MMU table for ARM11 */
      DALSYS_memset((void *)meminfo->hal_mem_info.VirtualAddr, 0x0, hal_mem_size);
    }
  }
  ...
  if ((FLASH_DEVICE_DONE == result) && dal_mem_size)
  {
    ...
    else
    {
      ...
      DALSYS_memset((void *)meminfo->drv_mem_info.VirtualAddr, 0x0, dal_mem_size);
  ...
  dalsys_mem_desc_list **phys_buff_list, uint32 operation)
{
  ...
  if (FLASH_HAL_DMA_ENABLE == nand_entry_init_data.dma_state)
  {
    ...
    if ((uint32) client_data->nand_phys_handle == DALSYS_MEM_ADDR_NOT_SPECIFIED)
    {
      ...
      void *tempList = DALFW_MemDescBufPtr(buff_list, 0);
      if (tempList != NULL) 
      {
        DALSYS_LogEvent(0,
         DALSYS_LOGEVENT_FATAL_ERROR,
         "DALFW failed to allocate %d Bytes",
         ((DALSysMemDescBuf *)tempList)->size);
      }
    }
  }
  else
  ...
  struct flash_block_vector *block_vector, uint32 iovec_count)
{
  ...
  flash_nand_clocks_request_complete();
