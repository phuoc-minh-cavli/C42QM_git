 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.10/storage/flash/src/iface/flash_bam_wrapper.h#3 $ $DateTime: 2018/10/31 09:47:12 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 10/12/18     sa      Added struct to change BAM pipe assignment.
 * 09/18/18     sb      Update QPIC BAM base address for Twizy
 * 02/20/17     svl     modified bam poll count to avoid timeout
...
/* QPIC 2.0 version, redefining here for bam wrapper module */
...
#define FLASH_BAM_MAX_POLL_COUNT 100000000  /* check and finetune this original value 100000 */

#ifdef QPIC_QPIC_XPU2_BASE
...
/* This struct stores information about bam pipes and masks. */
struct flash_bam_pipes
{
  uint8 nand_bam_cmd_pipe_no;  /* Stores the cmd pipe number */
  uint8 nand_bam_read_pipe_no; /* Stores the read pipe number */
  uint8 nand_bam_write_pipe_no; /* Stores the write pipe number */
  uint8 flash_bam_debug_pipe_count; /* Stores the total number of bam pipes */
  uint32 flash_bam_poll_cnt;       /* Stores bam poll count. */
  uint8 flash_bam_debug_peer_mask; /* Stores bam peer mask */
  uint8 flash_bam_debug_pipe_mask; /* Stores bam pipe mask */
};
