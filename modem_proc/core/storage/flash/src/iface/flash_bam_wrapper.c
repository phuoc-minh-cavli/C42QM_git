 ...
 *   order.
 *
 *  $Header: //components/rel/core.mpss/3.10/storage/flash/src/iface/flash_bam_wrapper.c#2 $ $DateTime: 2018/10/31 09:47:12 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 10/12/18     sa      Added struct to change BAM pipe assignment.
...
/* Struct used to initialize bam bibe numbers based on QPIC version */
struct flash_bam_pipes flash_bam_pipe;
...
int flash_bam_core_dump_mem_alloc(void)
{
  ...
  if (dump_buffer == NULL)
  {
    ...
    pipe_num_cnt = flash_bam_pipe.flash_bam_debug_pipe_count;
...
int flash_bam_core_dump(void)
{
   ...
   * pipe-5 cmd consumer. 
   */
  pipe_mask = flash_bam_pipe.flash_bam_debug_pipe_mask;
  peer_pmask = flash_bam_pipe.flash_bam_debug_peer_mask;
...
/* This function initialises the BAM pipes based on QPIC version */
void flash_bam_pipe_assignment(uint32 qpic_version)
{
  struct flash_bam_pipes *bam_pipe = &flash_bam_pipe;
  
  bam_pipe->flash_bam_debug_peer_mask = 0;
  bam_pipe->flash_bam_debug_pipe_count = 3;

  if(qpic_version >= FLASH_HAL_QPIC_CTRL_VERSION_2_0)
  {
    bam_pipe->flash_bam_debug_pipe_mask = 0x70;
	bam_pipe->nand_bam_cmd_pipe_no = 6;
	bam_pipe->nand_bam_read_pipe_no = 5;
	bam_pipe->nand_bam_write_pipe_no = 4;
  }
  else
  {
    bam_pipe->flash_bam_debug_pipe_mask = 0x38;
	bam_pipe->nand_bam_cmd_pipe_no = 5;
	bam_pipe->nand_bam_read_pipe_no = 4;
	bam_pipe->nand_bam_write_pipe_no = 3;
  }
}
...
int flash_bam_init(uint32 qpic_version)
{
  ...
  /*Configure the BAM pipe based on QPIC version*/
  flash_bam_pipe_assignment(qpic_version);
 ...
 /* NAND Command Pipe Initialization */ 
  ...
  if (result == FLASH_BAM_SUCCESS)
  {
    flash_bam_cmd_handle = 
      flash_bam_pipe_init(bam_pipe->nand_bam_cmd_pipe_no, &flash_bam_pipe_cfg);
 ...
 /* NAND Write Pipe Initialization */ 
  ...
  { 
    flash_bam_write_handle =
      flash_bam_pipe_init(bam_pipe->nand_bam_write_pipe_no, &flash_bam_pipe_cfg);
  ...
  { 
    flash_bam_read_handle =
      flash_bam_pipe_init(bam_pipe->nand_bam_read_pipe_no, &flash_bam_pipe_cfg);
...
int flash_bam_poll_pipe(bam_handle pipe_handle, void* user_data)
{
  ...
  bam_pipe->flash_bam_poll_cnt = FLASH_BAM_MAX_POLL_COUNT;
  
  while(bam_pipe->flash_bam_poll_cnt)
  {      
    ...
    bam_pipe->flash_bam_poll_cnt = bam_pipe->flash_bam_poll_cnt - 1;
  }
  
  if(!bam_pipe->flash_bam_poll_cnt)
  {      
    // Do error handling, etc.
    if (flash_bam_core_dump() != FLASH_BAM_SUCCESS)
    {
