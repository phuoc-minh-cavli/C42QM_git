/*===========================================================================
 ...
 Copyright (c) 2002-2013,2022 by Qualcomm Technologies Incorporated.
 All Rights Reserved.
 ...
 ===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/efs/src/fs_pm_super.c#2 $ $DateTime: 2022/04/19 02:28:12 $ $Author: pwbldsvc $

when          who     what, where, why
--------      ---     ------------------------------------------------------
2022-03-31    sk      Add API to fetch superblock info
...
int
fs_super_get_info (struct fs_block_info_t *fs_block_info)
{
  int is_nand = 0;
  if (fs_block_info == NULL)
  {
     /* Invalid fs_block_info pointer provided, return error */
	 return -1;
  }
  if (sb_ptr)
  {
    ...
    if (is_nand)
    {
	  fs_block_info->pages_per_block = sb_ptr->data->block_size;
	  fs_block_info->log_block_count = FS_NAND_LOG_REGION_COUNT;
	}
	else
	{
	  //In case of non-NAND device, return error
	  return -1;
	}
  }
  else
  {
    FS_ERR_FATAL ("Invalid superblock pointer, cannot fetch info", 0, 0, 0);
  }
  return 0;
}
