...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/efs/src/fs_util.c#2 $ $DateTime: 2019/12/24 01:37:59 $ $Author: pwbldsvc $
...
#define NORM_MONTH_TAB_MAX  (sizeof(norm_month_tab) / sizeof(uint16))
...
#define LEAP_MONTH_TAB_MAX  (sizeof(leap_month_tab) / sizeof(uint16))
...
fs_time_jul_to_secs (const fs_time_julian_type *julian)
{
  ...
  if ((julian->year & 0x3) == 0)
  {
     ...
     */
    FS_ASSERT (julian->month > 0);
    FS_ASSERT (julian->month <= LEAP_MONTH_TAB_MAX);
    if ((julian->month > 0) && (julian->month <= LEAP_MONTH_TAB_MAX))
    {
  ...
  else
  {
    ...
    FS_ASSERT (julian->month > 0);
    FS_ASSERT (julian->month <= NORM_MONTH_TAB_MAX);
    if ((julian->month > 0) && (julian->month <= NORM_MONTH_TAB_MAX))
    {
...
char*
fs_string_iter_get_next (struct fs_string_iter *str_iter)
{
  char *cur_str = NULL;
  char ch;

  FS_ASSERT (str_iter != NULL);
  FS_ASSERT (str_iter->str_buf != NULL);
  FS_ASSERT (str_iter->str_buf_len > 0);
  FS_ASSERT (str_iter->str_buf_len >= str_iter->cur_posn);

  if (str_iter->cur_posn >= str_iter->str_buf_len)
        return NULL;

  cur_str = &str_iter->str_buf[str_iter->cur_posn];

  for (; str_iter->cur_posn < str_iter->str_buf_len;
       ++str_iter->cur_posn)
  {
    ch = str_iter->str_buf[str_iter->cur_posn];

    if ((ch == str_iter->str_seperator1) ||
        (ch == str_iter->str_seperator2))
    {
      str_iter->str_buf[str_iter->cur_posn++] = str_iter->new_str_seperator;
