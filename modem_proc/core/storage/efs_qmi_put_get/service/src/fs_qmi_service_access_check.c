...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/efs_qmi_put_get/service/src/fs_qmi_service_access_check.c#1 $ $DateTime: 2018/06/26 03:18:17 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2017-12-18   as    Validate file paths as real paths, not just as strings
...
fs_qmi_service_access_check (const char *path)
{
  ...
  resolved_path = efs_normalize_path (path, abs_path, sizeof (abs_path), 1);
  if ((resolved_path == NULL) && (efs_errno != ENOENT))
  {
    return -1;
  }
  ...
  {
    resolved_path = efs_normalize_path_string (path, abs_path,
                      sizeof (abs_path));
    if (resolved_path == NULL)
    {
      return -1;
