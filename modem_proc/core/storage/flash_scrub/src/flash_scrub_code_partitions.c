 ...
 *
 * Copyright (C) 2013-2014, 2019 QUALCOMM Technologies, Inc.
 * Verbose Description
 *
 ***********************************************************************/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/flash_scrub/src/flash_scrub_code_partitions.c#4 $ $DateTime: 2019/11/21 03:06:43 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2019-08-20   as    Fix corner case of bad block marking in case of md5 mismatch
...
#define EFS_PARTITION_NAME "efs2"
...
void scrub_erase_scrub_partition()
{ 
  int32 block_no, result;
  int32 scrub_beg_blk = 0;
  int32 scrub_end_blk;
  flash_scrub_shim_handle scrub_handle;

  scrub_handle = flash_scrub_shim_open_partition (FLASH_SCRUB_PARTITION_NAME);
  if (scrub_handle == NULL)
  {
    FLASH_SCRUB_ERR_FATAL ("open-parti failed. parti = %s, %d, %d",
                           FLASH_SCRUB_PARTITION_NAME,0 ,0);
  }
  scrub_end_blk = flash_scrub_shim_get_block_count(scrub_handle) - 1;

  for( block_no=scrub_beg_blk; block_no<=scrub_end_blk; block_no++)
  {
    result = flash_scrub_shim_erase_block(scrub_handle, block_no);
    if (result != FLASH_DEVICE_DONE)
    {
      FLASH_SCRUB_MSG_ERR_2 ("[%d] Failed to erase block %d",
                           result, block_no);
      flash_scrub_shim_mark_block_bad (scrub_handle, block_no);
	}
  }
      ...
      if (result == sizeof (scrubc->md5_hash2))
      {
        ...
        else
        {
          FLASH_SCRUB_LOG_ERR_3 ("MD5 changed for parti=%c%c%c",
             parti_entry->name[0], parti_entry->name[1], parti_entry->name[2]);
        }
      }
...
scrub_code_create_config_file (struct scrub_code_info_type *scrubc)
{
  ...
  FLASH_SCRUB_LOG_MSG_3 ("scrubC config-file first = %c%c%c",
                       scrubc->config_file.first_code_parti_name[0],
                       scrubc->config_file.first_code_parti_name[1],
                       scrubc->config_file.first_code_parti_name[2]);

  FLASH_SCRUB_LOG_MSG_3 ("scrubC config-file last = %c%c%c",
                       scrubc->config_file.last_code_parti_name[0],
                       scrubc->config_file.last_code_parti_name[1],
                         ...
                         int incr)
{
  ...
  uint64 start_tick, stop_tick, diff_tick, time_in_us;
  uint32 time_in_ms, scrub_entry_length, code_entry_length;
  int move_code_block, move_scrub_block;
  flash_partentry_t new_code_entry, new_scrub_entry;

  start_tick = flash_scrub_timetick_get_timetick ();

  scrub_entry_length = scrub_entry->length;
  code_entry_length = code_entry->length;
  scrub_beg_blk = 0;
  ...
  if (code_handle == NULL)
  {
    FLASH_SCRUB_ERR_FATAL ("open-parti failed. parti = %c%c%c",
                           code_entry->name[0], code_entry->name[1],
                           code_entry->name[2]);
  }
  ...
  if (result != sizeof (scrubc->md5_hash2))
  {
    ...
    skip_mibib_update = 1;
    scrub_entry->length  = scrub_entry_length;
    code_entry->length = code_entry_length;
