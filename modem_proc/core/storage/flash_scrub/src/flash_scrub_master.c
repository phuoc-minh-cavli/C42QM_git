...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/storage/flash_scrub/src/flash_scrub_master.c#3 $ $DateTime: 2020/01/28 23:41:48 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2020-01-20   vm    Fix to disable automatic timer based Scrub.
2019-04-08   as    Allow disabling SCRUB feature at runtime.
2016-12-15   svl   added clocks vote-unvote at required places in scrub init.
...
/*---------------------------------------------------------------------------
 * Different call flows to the scrub_master_do_scrub() from non-timer triggers,
 * actual API that does scrubbing.
   flash_scrub_start_scrub_request(API)
                   |                   flash_scrub_start_scrub_request_from_qmi
                   |                        |
                   |                        |
         flash_scrub_start_scrub_request_helper
                          |
                          |
         flash_scrub_start_scrub_signal_received  flash_scrub_do_a_forced_scrub
                                         |               |
                                         |               |
                                  flash_scrub_start_scrub_helper
                                               |
                                               |
                                      scrub_master_do_scrub

 * Different paths for scrub timer getting set.

 scrub_master_do_scrub    flash_scrub_init
                                       flash_scrub_system_timer_signal_received
        |                       |                       |
   (sets scrub timer      (sets scrub timer      (sets scrub timer
    after every scrub      during init)          in 2,4,8,... secs)
    completes)                  |                       |
        |                       |                       |
        -------------------------------------------------
                                |
                                |
                  scrub_master_start_scrub_timer
                                 |
                         ..................
                       .... TIMER EXPIRES ...
                         .................
...
---------------------------------------------------------------------------*/
#define SCRUB_MASTER_INFO_FILE_VERSION_MIN         (1)
#define SCRUB_MASTER_INFO_FILE_VERSION             (2)
...
PACKED struct scrub_master_info_file_type
{
  ...
  uint8  is_scrub_disabled;
  uint8  reserved[7]; /* For future use so we don't end up breaking backward
                        compatibility in case we need to add more fields. Since
                        this is stored as an EFS Item file, we need to maintain
                        size of the structure. */
} PACKED_POST;

#define SCRUB_MASTER_DEBUG_FILE_VERSION_MIN         (1)
#define SCRUB_MASTER_DEBUG_FILE_VERSION             (2)
...
PACKED struct scrub_master_debug_file_type
{
  ...
  uint64 scrub_disable_time;
  uint64 scrub_disable_total_time;
  uint32 scrub_disable_count;
} PACKED_POST;
...
scrub_master_load_info_file (struct scrub_master_info_type *scrubm)
{
  ...
  if ((scrubm->info_file.version != SCRUB_MASTER_INFO_FILE_VERSION_MIN) &&
  ...
  {
    scrubm->info_file.version = SCRUB_MASTER_INFO_FILE_VERSION;
    scrubm->info_file.is_scrub_disabled = 0;
  }
...
scrub_master_load_debug_file (struct scrub_master_info_type *scrubm)
{
  ...
  if ((scrubm->debug_file.version != SCRUB_MASTER_DEBUG_FILE_VERSION_MIN) &&
  ...
  {
    scrubm->debug_file.version = SCRUB_MASTER_DEBUG_FILE_VERSION;
    scrubm->debug_file.scrub_disable_time = 0;
    scrubm->debug_file.scrub_disable_total_time = 0;
    scrubm->debug_file.scrub_disable_count = 0;
  }
...
static int
scrub_master_is_scrub_feature_enabled (struct scrub_master_info_type *scrubm)
{
  ...
  ASSERT (scrubm != NULL);
  scrubm->is_scrub_feature_enabled = 0;
  ...
  scrubm->is_scrub_feature_enabled = 1;

End:
  return scrubm->is_scrub_feature_enabled;
}
...
scrub_master_get_current_time (struct scrub_master_info_type *scrubm)
{
  ...
  {
    FLASH_SCRUB_MSG_HIGH_3 ("system time not valid\
                            curr_time = %d, scrubm->jan_1 = %d,\
                             scrubm->sys_time_last_scrub = %d",
                            curr_time, scrubm->jan_1_2013_in_ms, 
                            scrubm->info_file.system_time_of_last_scrub);
    curr_time = 0;
  }
...
scrub_master_do_scrub (struct scrub_master_info_type *scrubm)
{
  ...
  if (scrubm->info_file.is_scrub_disabled == 1)
  {
    FLASH_SCRUB_MSG_ERR_0 ("Scrubbing is disabled, so not doing scrub");
    return;
  }
...
scrub_master_start_scrub_timer (struct scrub_master_info_type *scrubm)
{
  ...
  {
    if (!(scrubm->config_file.forced_minimum_time_in_secs_between_2_scrubs == 0 &&
        scrubm->config_file.timer_value_in_secs_for_automatic_scrubs == 0 ))
    {
...
flash_scrub_init (void)
{
  struct scrub_master_info_type *scrubm;
  uint64 curr_time;
  int result, is_scrub_feature_enabled;
  ...
  /*
    Scrub Init makes flash calls without voting for required clocks
    This can cause unclocked access exception. So voting for required
    clocks before and after these flash operations are done.
    flash_scrub_shim_ops_start and flash_scrub_shim_ops_end are vote/
    unvote respectively.
  */
  ...
  is_scrub_feature_enabled = scrub_master_is_scrub_feature_enabled (scrubm);
  if (is_scrub_feature_enabled != 1)
  {
...
flash_scrub_start_scrub_helper (void)
{
  ...
  if (scrubm->is_scrub_feature_enabled != 1)
  {
    FLASH_SCRUB_MSG_ERR_0 ("Scrub feature is disabled, so not doing scrub");
    result = FLASH_SCRUB_FEATURE_DISABLED;
    goto End;
  }

  if (scrubm->info_file.is_scrub_disabled != 0)
  {
    FLASH_SCRUB_MSG_ERR_0 ("Scrub is disabled, so not doing scrub");
                                   ...
                                   int is_via_qmi)
{
  ...
  *last_scrub_time_in_secs =(scrubm->info_file.system_time_of_last_scrub)/1000;
      ...
      uint64 automatic_scrub_timer_value_in_secs)
{
  ...
  {
    result = 0;
    FLASH_SCRUB_LOG_MSG_3 ("Disabled scrub by changing\
                            auto-timer. old = %d, new = %d, min = %d",
       old_automatic_scrub_timer_value_in_secs,
       scrubm->config_file.timer_value_in_secs_for_automatic_scrubs,
       scrubm->config_file.forced_minimum_time_in_secs_between_2_scrubs);
    goto End;
...
{
  struct scrub_master_info_type *scrubm = NULL;
  int result = FLASH_SCRUB_ECONFIG;
  uint64 curr_time;
  ...
  scrub_erase_scrub_partition();

  scrubm->info_file.is_scrub_disabled = 1;
  scrub_master_save_info_file (scrubm);

  curr_time = time_get_ms_native ();
  scrubm->debug_file.scrub_disable_count++;
  scrubm->debug_file.scrub_disable_time = curr_time;
  scrub_master_save_debug_file (scrubm);
  FLASH_SCRUB_MSG_HIGH_0 ("Flash scrub is disabled successfully");
...
int flash_scrub_enable_scrub (int recompute_md5)
{
  struct scrub_master_info_type *scrubm = NULL;
  int result = FLASH_SCRUB_ECONFIG; 
  uint64 curr_time, scrub_disable_time;
  ...
  ASSERT (scrubm->is_scrub_in_progress == 0);
  ASSERT (scrubm->info_file.recompute_md5_after_fota == 0);

  scrubm->info_file.is_scrub_disabled = 0;
  scrub_master_save_info_file (scrubm);

  curr_time = time_get_ms_native ();
  scrub_disable_time = scrubm->debug_file.scrub_disable_time;
  scrubm->debug_file.scrub_disable_total_time += curr_time-scrub_disable_time;
  scrub_master_save_debug_file (scrubm);

  if (recompute_md5)
  {
    flash_scrub_notify_fota_update ();
  }

  scrub_master_start_scrub_timer(scrubm);
  FLASH_SCRUB_MSG_HIGH_0 ("Flash scrub is enabled successfully");
  result=0;
