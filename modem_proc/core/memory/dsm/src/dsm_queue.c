...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/memory/dsm/src/dsm_queue.c#1 $
...
PARAMETERS
   wm_ptr - Pointer to the watermark item header.  This has the pointers
            to the callbacks and the callback data associated with this 
            watermark.
   initial_count - This is the byte count of this water mark queue before the 
                   "operation".  This function is called after the "operation"
                   has changed the count for the watermark.  
   initial_q_count - Q_cnt (packet count) in the WM before enqueueing
...
dsmi_wm_check_levels_enqueue(dsm_watermark_type * wm_ptr, 
                     ...
                     uint32 initial_q_count)
{
  boolean check_cb_hilo; 
  check_cb_hilo = (wm_ptr->hiwater_func_ptr || wm_ptr->lowater_func_ptr || wm_ptr->hiwater_q_cnt_func_ptr);
  ...
  if(check_cb_hilo)
  {
	  ...
	  if((wm_ptr->hiwater_q_cnt_func_ptr) &&
	  	(wm_ptr->q_ptr->cnt > wm_ptr->hi_watermark_q_cnt) &&
         (initial_q_count <= wm_ptr->hi_watermark_q_cnt))
      {
        DSM_WM_CB(wm_ptr->hiwater_q_cnt_func_ptr,wm_ptr,wm_ptr->hiwater_q_cnt_func_data);
      }
  }
  if(initial_count == 0)
  {
...
PARAMETERS
   wm_ptr - Pointer to the watermark item header.  This has the pointers
            to the callbacks and the callback data associated with this 
            watermark.
   initial_count - This is the bytecount of this water mark queue before the 
                   "operation".  This function is called after the "operation"
                   has changed the count for the watermark.  
   initial_q_count - Q_cnt (packet count) in the WM before enqueueing
                     ...
                     uint32 initial_q_count)
{
  ...
  check_cb_hilo = (wm_ptr->hiwater_func_ptr || wm_ptr->lowater_func_ptr || wm_ptr->lowater_q_cnt_func_ptr);
  ...
  if(check_cb_hilo)
  {
	   ...
	   if((wm_ptr->lowater_q_cnt_func_ptr) &&
	   	   (initial_q_count >= wm_ptr->lo_watermark_q_cnt) &&
           wm_ptr->q_ptr->cnt < wm_ptr->lo_watermark_q_cnt)
       {
         DSM_WM_CB(wm_ptr->lowater_q_cnt_func_ptr,wm_ptr,
              wm_ptr->lowater_q_cnt_func_data);
       }
  }
  if ( wm_ptr->current_cnt == 0)
  {
...
{
  if((wm_ptr->lowater_func_ptr) &&
  	 (initial_count >= wm_ptr->lo_watermark) &&
     (wm_ptr->current_cnt < wm_ptr->lo_watermark))
  {
    DSM_WM_CB(wm_ptr->lowater_func_ptr,wm_ptr,
              wm_ptr->lowater_func_data);
...
{
  uint32 initial_count,initial_q_cnt;          /* initial count of bytes in WM           */
  uint32 pkt_len ;
    ...
    (wm_ptr->current_cnt < wm_ptr->dont_exceed_cnt)))
  {
	...
	initial_q_cnt = wm_ptr->q_ptr->cnt;
    ...
    -----------------------------------------------------------------------*/
    dsmi_wm_check_levels_enqueue_opt(wm_ptr, initial_count, initial_q_cnt);
...
{
  uint32 initial_count,initial_q_cnt;     /* initial number of bytes in watermark */
...
/*function q_get_s_ll_i updates q_ptr->cnt, so it is moved up, so that
  original value of q_ptr->cnt can be recored before it gets modified */    
  initial_count = wm_ptr->current_cnt;
  initial_q_cnt = wm_ptr->q_ptr->cnt;
  ...
  if((item_ptr = (dsm_item_type *)q_get_s_ll_i(wm_ptr->q_ptr)) != NULL)
  {
    ...
    dsmi_wm_check_levels_dequeue_opt(wm_ptr, initial_count,initial_q_cnt);
...
)
{
  ...
  switch(dsm_wm_cb_type)
  {
    case DSM_WM_LO:
     ...
     break;
    case DSM_WM_LO_Q_CNT:
     if (change_cb)
     {
       wm_ptr->lowater_q_cnt_func_ptr = cb;
     }
     if (change_cb_data)
     {
       wm_ptr->lowater_q_cnt_func_data = cb_data;
     }
     if (change_val)
     {
         if(wm_ptr->q_ptr->cnt >= wm_ptr->lo_watermark_q_cnt &&
            wm_ptr->q_ptr->cnt < val )
         {
           DSM_WM_CB(wm_ptr->lowater_q_cnt_func_ptr,wm_ptr,wm_ptr->lowater_q_cnt_func_data);
         }
         wm_ptr->lo_watermark_q_cnt = val;
     }
     break;
    ...
    case DSM_WM_HI:
      ...
      break;
    case DSM_WM_HI_Q_CNT:
      if (change_cb)
      {
        wm_ptr->hiwater_q_cnt_func_ptr = cb;
      }
      if (change_cb_data)
      {
        wm_ptr->hiwater_q_cnt_func_data = cb_data;
      }
      if (change_val)
      {
          if(wm_ptr->q_ptr->cnt <= wm_ptr->hi_watermark_q_cnt &&
             wm_ptr->q_ptr->cnt > val )
          {
            DSM_WM_CB(wm_ptr->hiwater_q_cnt_func_ptr,wm_ptr,wm_ptr->hiwater_q_cnt_func_data);
          }
          wm_ptr->hi_watermark_q_cnt = val;
      }
      break;
