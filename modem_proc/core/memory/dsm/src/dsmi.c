...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/memory/dsm/src/dsmi.c#1 $
...
#ifdef FEATURE_DSM_MEM_CHK 
  if(DSM_UNLIKELY((item_ptr->references == 0) || ((item_ptr->references >> 12) !=0)))
  { 
    ERR_FATAL("dsmi_ref_inc: Reference count is zero OR greater than 4095.",0,0,0); 
  } 
#endif /* FEATURE_DSM_MEM_CHK */ 
...
uint16 dsmi_ref_dec(dsm_item_type *item_ptr)
{
  ...
  if(DSM_UNLIKELY(item_ptr->references == 0))
  {
#ifdef FEATURE_DSM_MEM_CHK
