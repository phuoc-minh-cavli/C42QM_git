...
$Header: //components/rel/core.mpss/3.10/memory/qservices/src/queue_services.c#2 $ $DateTime: 2018/08/24 22:11:30 $ $Author: pwbldsvc $
...
#if ((defined FEATURE_QURT) || ((defined FEATURE_Q_WIN32_LOCKS) || (defined FEATURE_WINCE)) && !defined(FEATURE_WINCE_BOOTLOADER))
...
static void q_lock_s_init( q_type *q_ptr )
{
  ...
  #elif defined FEATURE_QURT
  qurt_pimutex_init(&(q_ptr->mutex));
  #else
...
static void q_lock_s_delete( q_type *q_ptr )
{
  ...
  #elif defined FEATURE_QURT
      qurt_pimutex_destroy(&(q_ptr->mutex));
  #else
...
void q_unlock_s( q_type *q_ptr )
{
  ...
  #elif defined FEATURE_QURT
   qurt_pimutex_unlock(&(q_ptr->mutex));
  #else 
