...
============================================================================*/
/*=======================================================================
$Header: //components/rel/core.mpss/3.10/power/thermal/src/common/therm_config.c#3 $
...
static void skip_line(char **ppos)
{
  ...
  if (!ptmp)
  {
    /* Must be last line and not newline terminated just advance to null char */
    *ppos = strchr(pos, '\0');
    return;
  }
                        ...
                        therm_monitor_field_type field_type)
{
  ...
  {
    /* Must be last line and not newline terminated just advance to prior to
       NULL char */
    ptmp = strchr(pos, '\0');
    if (ptmp)
    {
...
int therm_monitor_load_config(therm_monitor_type *config_data)
{
  ...
  do
  {
    ...
    {
      therm_log_printf(THERM_LOG_LEVEL_INFO, 0,
                       "Loading default config data");
      if(therm_monitor_load_default_config(config_data) == MONITOR_FAILED)
      {
      therm_log_printf(THERM_LOG_LEVEL_ERROR, 0,
                         "No default config data Found");
      break;
      }   
    }

    if (config_fd >= 0 && parse_config(config_data, config_fd) != MONITOR_SUCCESS)
    {
      therm_log_printf(THERM_LOG_LEVEL_ERROR, 0,
