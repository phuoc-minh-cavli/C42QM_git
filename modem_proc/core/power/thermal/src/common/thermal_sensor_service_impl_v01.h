#ifndef THERMAL_SENSOR_SERVICE_V01IMPL_H
#define THERMAL_SENSOR_SERVICE_V01IMPL_H
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*

              T H E R M A L _ S E N S O R _ S E R V I C E _ I M P L _ V 0 1  . h

GENERAL DESCRIPTION
  This is the file which defines the ts service Data structures.

  Copyright (c) 2012-2013 Qualcomm Technologies, Inc.
  All rights reserved.
  Confidential and Proprietary - Qualcomm Technologies, Inc.


  $Header: //components/rel/core.mpss/3.10/power/thermal/src/common/thermal_sensor_service_impl_v01.h#1 $
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/* This file was generated with Tool version 6.1
   It was generated on: Sat Mar  2 2013 (Spin 0)
   From IDL File: thermal_sensor_service_v01.idl */

#include "qmi_si.h"
#ifdef __cplusplus
extern "C" {
#endif
const qmi_implemented_messages *ts_get_service_impl_v01 (void);


#ifdef __cplusplus
}
#endif
#endif

