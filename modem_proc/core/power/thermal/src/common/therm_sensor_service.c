/*============================================================================
  ...
  Copyright (c) 2012-2013,2019 QUALCOMM Technologies, Incorporated.
  All Rights Reserved.
  Qualcomm Confidential and Proprietary
============================================================================*/
/*=======================================================================
$Header: //components/rel/core.mpss/3.10/power/thermal/src/common/therm_sensor_service.c#2 $
...
/**
  ...
  This function handles NPA event changes for sensor resources.  Sends
                                             ...
                                             unsigned int  data_size )
{
  ...
  therm_sensor_info        *sensor_info    = NULL;
  ...
  do
  {
    ...
    sensor_info = (therm_sensor_info *)therm_sensor_node.\
                                        resources[ind_info->sensor_idx].data;
                                           ...
                                           const char             *log_str)
{
  ...
  do
  {
    ...
    {
      therm_sensor_info *sensor_info = (therm_sensor_info *)therm_sensor_node.\
                                              resources[i].data;

      if ((sensor_info->export) &&
          (strcasecmp(sensor_info->npa_node_str, dev_id) == 0))
      {
        /* Found a match */
        *idx = i;
        break;
                                                 ...
                                                 void           *service_cookie)
{
  ...
  do
  {
    ...
    response->sensor_list_len = 0;

    therm_log_printf(THERM_LOG_LEVEL_INFO, 0,
                     "TS Get sensor list request");

    /* Copy cooling device info over to response. */
    /* Check for advertised flag of sensors allowed to be advertised. */
    for (i = 0; (i < therm_sensor_node.resource_count) &&
                (i < QMI_TS_SENSOR_LIST_MAX_V01); i++ )
    {
      therm_sensor_info *sensor_info = (therm_sensor_info *)therm_sensor_node.\
                                                            resources[i].data;
      if (sensor_info->export)
    {
      therm_log_printf(THERM_LOG_LEVEL_INFO, 2,
                         "TS Sensor %d %s", i, sensor_info->npa_node_str);
        strlcpy(response->sensor_list[response->sensor_list_len].sensor_id,
                sensor_info->npa_node_str,
                sizeof(response->sensor_list[response->sensor_list_len].sensor_id));
        response->sensor_list_len++;
      }
    }
                                                       ...
                                                       void           *service_cookie)
{
  ...
  if (send_immediate_resp)
  {
    ...
    if (QMI_CSI_NO_ERR == qmi_error)
    {
      therm_log_printf(THERM_LOG_LEVEL_INFO, 4,
                       "TS Indication SENSOR: %s, TYPE: %s, TEMP: %d, SEQNUM: %d",
                       ts_qmi_ind_msg_immediate.sensor_id.sensor_id,
