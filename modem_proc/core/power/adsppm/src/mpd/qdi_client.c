...
$Header: //components/rel/core.mpss/3.10/power/adsppm/src/mpd/qdi_client.c#1 $
*/
...
uint32 MMPM_Register_Ext(MmpmRegParamType *pRegParam)
{
    ...
    {
        ret = qurt_qdi_handle_invoke(Adsppm_Client_Ctx.handle, ADSPPM_REGISTER, pRegParam, &clientId);
        if(ret < 0)
        {
            clientId = 0;
        }
