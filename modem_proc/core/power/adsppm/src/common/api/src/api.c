...
$Header: //components/rel/core.mpss/3.10/power/adsppm/src/common/api/src/api.c#2 $
*/
...
uint32 isMipsRelatedBw(AdsppmBusPortIdType masterPort,AdsppmBusPortIdType slavePort)
{
	uint32 result = FALSE;
	int i = 0;
    ...
    ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_ERROR,
                        "MBusPortID %u SBusPortId %u", masterPort, slavePort);
...
static MMPM_STATUS apiAddBwRequest(uint32 clientId, MmpmRscIdType resourceId, MmpmGenBwReqType *pGenBwReq, MmpmApiType apiType, uint32 *pAsyncRscMap)
{
	...
	AdsppmBwSingleReqType adsppmDspBwReqData;
    uint32 bwNum, BwNumNonDsp;
	boolean otherBwReq = FALSE;
    ...
    else
    {
        ...
        for(bwNum = 0, BwNumNonDsp = 0; bwNum < pGenBwReq->numOfBw; bwNum++)
        {
            ...
            else
            {
						...
						mmpmToAdsppmBusPort[pBwInfo->busRoute.slavePort]))
				{
					...
					adsppmDspBwReqData.adsppmBwReqDataVal.busRoute.masterPort = AdsppmMapMMPMPortIdToADSPPMPortID(pBwInfo->busRoute.masterPort);
					adsppmDspBwReqData.adsppmBwReqDataVal.busRoute.slavePort = AdsppmMapMMPMPortIdToADSPPMPortID(pBwInfo->busRoute.slavePort);

					adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.usageType =
							(AdsppmBwUsageLpassType) pBwInfo->bwValue.busBwValue.usageType;
					adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.bwBytePerSec=
							pBwInfo->bwValue.busBwValue.bwBytePerSec;
					adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.usagePercentage =
							pBwInfo->bwValue.busBwValue.usagePercentage;

					adsppmDspBwReqData.adsppmBwReqDataVal.bwOperation = AdsppmBusBWOperation_BW;
					adsppmDspBwReqData.requestClass = resourceId == MMPM_RSC_ID_GENERIC_BW_EXT ? Adsppm_BwReqClass_Compensated : Adsppm_BwReqClass_Generic;


					ADSPPM_LOG_PRINTF_5(ADSPPM_LOG_LEVEL_INFO,
						"BW to DDR : MMPM   M=%s(%u), S=%s(%u), Operation=%u",
						MMPMBusPortName[pBwInfo->busRoute.masterPort],
						pBwInfo->busRoute.masterPort,
						MMPMBusPortName[pBwInfo->busRoute.slavePort],
						pBwInfo->busRoute.slavePort,
						adsppmDspBwReqData.adsppmBwReqDataVal.bwOperation);

					ADSPPM_LOG_PRINTF_3(ADSPPM_LOG_LEVEL_INFO,
							"BW to DDR: usage=%d, BW = %d at %d%% ",
							adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.usageType,
							(uint32 )adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.bwBytePerSec,
							adsppmDspBwReqData.adsppmBwReqDataVal.bwValue.busBwValue.usagePercentage);
					if(MMPM_STATUS_SUCCESS == sts)
					{
						sts = (MMPM_STATUS) RM_AddRequest(clientId, Adsppm_Rsc_Id_Mips,
							&adsppmDspBwReqData, 1, (apiType == MMPM_API_TYPE_SYNC));
					}
					if (MMPM_STATUS_SUCCESS == sts)
					{
        ...
        if((MMPM_STATUS_SUCCESS == sts) && (otherBwReq == TRUE))
        {
...
MMPM_STATUS MMPM_GetInfo(MmpmInfoDataType *pInfoData)
{
    ...
    case MMPM_INFO_ID_BW_EXT:
    {
        ...
        if (isMipsRelatedBw(busRoute.masterPort, busRoute.slavePort))
				{
            sts = (MMPM_STATUS) MIPS_GetCompensatedDDRBw(
                &pAdsppmCoreCtx->rscReqQ[Adsppm_Rsc_Id_Mips-1], &busRoute,
