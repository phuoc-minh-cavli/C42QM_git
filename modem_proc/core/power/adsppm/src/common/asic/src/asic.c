...
$Header: //components/rel/core.mpss/3.10/power/adsppm/src/common/asic/src/asic.c#2 $
*/
...
uint32 ACM_GetCppFromMpps(uint32 mppsTotal, uint32 numDominantThreads)
{
    uint32 cpp = 0;
    ...
    {
		ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO, "T: 1: %u 2: %u, 3: %u, 4 %u",
					(table->pRows[numDominantThreads - 1].oneThreadActive),
					(table->pRows[numDominantThreads - 1].twoThreadActive),
					(table->pRows[numDominantThreads - 1].threeThreadActive),
					(table->pRows[numDominantThreads - 1].fourThreadActive));

		ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO, "Cpp: 1: %u 2: %u, 3: %u, 4 %u",
					cppFactors->pData[0], cppFactors->pData[1], cppFactors->pData[2], cppFactors->pData[3]);

        cpp =
            ((table->pRows[numDominantThreads - 1].oneThreadActive)   * cppFactors->pData[0]) +
            ((table->pRows[numDominantThreads - 1].twoThreadActive)   * cppFactors->pData[1]) +
            ((table->pRows[numDominantThreads - 1].threeThreadActive) * cppFactors->pData[2]) +
            ((table->pRows[numDominantThreads - 1].fourThreadActive)  * cppFactors->pData[3]);
	cpp = cpp / 100;
    }

    ADSPPM_LOG_PRINTF_3(ADSPPM_LOG_LEVEL_INFO,
        "numDominantThreads: %u mppsTotal %u -> Cpp %u", numDominantThreads, mppsTotal, cpp);
