...
typedef struct
{
	...
	unsigned Ib_key; //IB_REQ_KEY
	unsigned Ib_length; //data length
	unsigned Ib_data; //actual data
	//Example 'KVP' for Ab vlue
	unsigned Ab_key; //AB_REQ_KEY
	unsigned Ab_length; //data length
	unsigned Ab_data; //actual data
	//Example 'KVP' for latency value
	unsigned lat_key; //LAT_REQ_KEY
	unsigned lat_length; //data length
	unsigned lat_data; //actual data
        ...
        unsigned int  data_size )
{
    ...
    if (NULL != gCAInfo.caRequestHandle.handle)
    {
		...
		gCAInfo.caRequestData.lat_key = LAT_REQ_KEY;
    	gCAInfo.caRequestData.lat_length = sizeof(gCAInfo.caRequestData.lat_data);
...
AdsppmStatusType caRequestCpuPerformanceLevel_NPA(AdsppmMipsAggregateDataType* pCpuData)
{
    ...
    if (NULL != pCpuData)
    {
        if (NULL != gCAInfo.caRequestHandle.handle)
        {
            ...
            gCAInfo.caRequestData.q6_data = pCpuData->mipsAggregate.mipsPerThread * MCA_Q6_CONVERT_FACTOR ; // Changing unit as per MCA: contains floor clk
            ...
            gCAInfo.caRequestData.lat_data = pCpuData->mipsToBWAggregateDataDdr.mipsToBW.bwValue.busBwAbIb.latencyNs;

            ADSPPM_LOG_PRINTF_7(ADSPPM_LOG_LEVEL_INFO, "NPA request for %s, MPPS: %d, Floor: %d, CPP: %d, Ib: %d, Ab: %d lat %d\n",AGGREGATOR_NODE_NAME,
                gCAInfo.caRequestData.mpps_data, gCAInfo.caRequestData.q6_data, gCAInfo.caRequestData.cpp_data,
                gCAInfo.caRequestData.Ib_data, gCAInfo.caRequestData.Ab_data, gCAInfo.caRequestData.lat_data);
...
AdsppmStatusType CA_RequestCpuPerformanceLevel(AdsppmMipsAggregateDataType* pCpuData)
{
    ...
    else
    {
		...
		sts = caRequestCpuPerformanceLevel_NPA(pCpuData);
    }
    ADSPPM_LOG_FUNC_PROFILEEND;
    ADSPPM_LOG_FUNC_EXIT(sts);
    return sts;
