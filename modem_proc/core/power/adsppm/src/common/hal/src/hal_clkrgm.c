...
Adsppm_Status ClkRgm_EnableHWClockCntl(AdsppmClkIdType clk, Hal_ClkRgmEnableDisableType state)
{
    ...
    if(( AdsppmClk_None < clk) && ( AdsppmClk_EnumMax > clk))
    {
        if(NULL != gClkRgmCtx.lock )
        {
            ...
            HalClk_ClockDescriptorType *pClk = &gClkRgmCtx.clockDesc[clkIdx];
            ...
            if(Hal_ClkRgmMemRetain == pwrState)
            {
                ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_INFO,
                 ...
                 dalResult = DalClock_ConfigClock(gClkRgmCtx.pHandle,
                   pClkHalDesc->clkHandle.clkId, CLOCK_CONFIG_CORE_MEM_ON);
            }
            else
            {
                ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_INFO,
                    ...
                    pClkHalDesc->pClkDescriptor->clkInfo.clkName);
                dalResult = DalClock_ConfigClock(gClkRgmCtx.pHandle,
                    pClkHalDesc->clkHandle.clkId , CLOCK_CONFIG_CORE_MEM_OFF);
            }
            if(DAL_SUCCESS != dalResult)
            {
...
static void clkrgm_SharedClkInit(void)
{
    AdsppmClkIdType i;
	uint32 clkIdx =0;
    for(i = AdsppmClk_None; i < AdsppmClk_EnumMax; i++)
    {
		if (ACM_IsClkIdValid(i) && ACM_GetClkIdx(i , &clkIdx) == Adsppm_Status_Failed)
            continue;
        HalClk_ClockDescriptorType *pClk = &gClkRgmCtx.clockDesc[clkIdx];
...
Adsppm_Status clkrgm_SharedClkControl(Hal_ClkRgmEnableDisableType stateRequested)
{
    ...
    for(i = AdsppmClk_None; i < AdsppmClk_EnumMax; i++)
    {
        ...
        if((NULL != pClk->pClkDescriptor) && (pClk->pClkDescriptor->clkType != AsicClk_TypeNone))
        {
            ...
            {
                result = ClkRgm_EnableClock(pClk->pClkDescriptor->clkId, stateRequested);
                ADSPPM_LOG_PRINTF_3(ADSPPM_LOG_LEVEL_INFO, "turn on/off shared clockid %d, state=%d clkId %d", i, stateRequested, pClk->pClkDescriptor->clkId);
            }
            if (Adsppm_Status_Success != result)
            {
	...
	for (i = 0; (Adsppm_Status_Success == result) && (i < numClkDesc); i++)
	{
        ...
        if(pClkHalDesc->pClkDescriptor->clkType != AsicClk_TypeNone)
        {
                ...
                (pClkHalDesc->pClkDescriptor->clkType != AsicClk_TypeCBC))
            {
				...
				case AsicClk_CntlHW:
					pClkHalDesc->currentState = Hal_ClkRgmDisable;
					result = ClkRgm_EnableHWClockCntl(pClkHalDesc->pClkDescriptor->clkId, Hal_ClkRgmEnable);
                break;
                default:
        ...
        if(NULL != gClkRgmCtx.lock)
        {
            ...
            if(NULL != pPwrHalDesc->pPwrDescriptor)
            {
                ...
                case AsicRsc_Power_Core:
					...
					if(NULL == pPwrHalDesc->pwrHandle.domainId)
						{
                        ADSPPM_LOG_PRINTF_0(ADSPPM_LOG_LEVEL_ERROR, "domainId is 0 In AsicRsc_Power_Core");
						break;
						}
                    if(Hal_ClkRgmEnable == state)
                    {
