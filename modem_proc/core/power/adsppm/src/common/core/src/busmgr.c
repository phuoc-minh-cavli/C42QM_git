...
$Header: //components/rel/core.mpss/3.10/power/adsppm/src/common/core/src/busmgr.c#2 $
*/
...
typedef struct
{
    ...
    uint32                      preCurrentDDRIndex; /* Used to get the DDR Values */
...
/* Global Flag is added to check if issue is hit
 * In BusMgr_ProcessRequest(), the pBwReqData->numOfBw > 1
 * but the bwBytePerSec = 0
 */
static boolean gBusMgrZeroBW = FALSE;
...
static Adsppm_Status BusMgr_GetCompensatedFudgedBwValueFromQ(
    coreUtils_Q_Type* pRequestQueue, uint64* pAb, uint64* pIb,
    AsicCompensatedBWFudgeFactor *pcompensatedBWFudgeFactorValues);
static Adsppm_Status busMgrGetExtRouteIndex(uint32 *index,
        AdsppmBusPortIdType masterPort, AdsppmBusPortIdType slavePort);
...
{
    Adsppm_Status sts = Adsppm_Status_Success;
    uint32 index = 0, numOfBW = 0, ddrIndex = 0;
    AdsppmBusBWRequestValueType* pCurrentBwVote = NULL;
    AdsppmBusBWRequestValueType* pPreviousBwVote = NULL;

    sts = busMgrGetExtRouteIndex(&ddrIndex, AdsppmBusPort_Adsp_Master, AdsppmBusPort_Ddr_Slave);

    for(index = 0; index < gBusMgrCtx.extRouteNum; index++)
    {
        /* Skip for DDR port, as only non-ddr are required */
		/* ddrIndex is valid only when sts = Adsppm_Status_Success */
        if(( ddrIndex == index) && sts == Adsppm_Status_Success)
			continue;
...
{
    Adsppm_Status sts = Adsppm_Status_Success;
    uint32 index = 0, bwNum = 0, ddrIndex;
    ...
    else
    {
        ...
        else
        {
            ...
            for(index = 0; index < gBusMgrCtx.extRouteNum; index++)
            {
				/* Dont clear data in case of DDR, that would be cleared separately */
				if ((sts == Adsppm_Status_Success) && (index == ddrIndex))
					continue;
            ...
            for(index = AdsppmClk_AhbI_Hclk; index < AdsppmClk_HwRsp_Core; index++)
            {
                ...
                if(RM_Valid == pRMHandle->validFlag)
                {
                    ...
                    else
                    {
                        ...
                        for(bwNum = 0; (bwNum < pBwReqData->numOfBw) && (Adsppm_Status_Success == sts); bwNum++)
                        {
							...
							/* TODO: this shouldnt be coming empty or zero */
							if(0 == pBwRequest->bwValue.busBwValue.bwBytePerSec)
							{
								ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO_EXT,
									"####BW request of Zero before from %s(%u) to %s(%u), exclude from aggregation",
...
/*index : output: index matched */
static Adsppm_Status busMgrGetExtRouteIndex(uint32 *index, AdsppmBusPortIdType masterPort, AdsppmBusPortIdType slavePort)
{
    Adsppm_Status sts = Adsppm_Status_Success;
    ...
    for(extRouteIndex = 0; extRouteIndex < gBusMgrCtx.extRouteNum; extRouteIndex++)
    {
        ...
        if((gBusMgrCtx.pExtRoutes[extRouteIndex].masterPort.adsppmMaster == masterPort) &&
           (gBusMgrCtx.pExtRoutes[extRouteIndex].slavePort.adsppmSlave == slavePort))
        {
    ...
    {
        ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_ERROR,
            "busMgrGetExtRouteIndex Invalid external route, M=%u, S=%u",
...
/* This function used only in case of 8909 */
void BusMgrPreAggregateClearDdrDataExtBW(void)
{
    Adsppm_Status sts = Adsppm_Status_Success;
    uint32 extRouteIndex = 0;
	...
	sts = busMgrGetExtRouteIndex(&extRouteIndex, AdsppmBusPort_Adsp_Master, AdsppmBusPort_Ddr_Slave);
	/* extRouteIndex is valid only when sts = Adsppm_Status_Success */
	if(Adsppm_Status_Success != sts)
	{
		ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_ERROR,
			"invalid index %u", sts);
	 ...
	 /* Clear the compensated & generic structures before using + the votes */
	gBusMgrCtx.extPreAggregateData.pGeneric[extRouteIndex].bwValue.busBwAbIb.Ab = 0;
	gBusMgrCtx.extPreAggregateData.pGeneric[extRouteIndex].bwValue.busBwAbIb.Ib = 0;
	gBusMgrCtx.extPreAggregateData.pGeneric[extRouteIndex].bwValue.busBwAbIb.latencyNs = 0;
	gBusMgrCtx.extPreAggregateData.pCompensated[extRouteIndex].bwValue.busBwAbIb.Ab = 0;
	gBusMgrCtx.extPreAggregateData.pCompensated[extRouteIndex].bwValue.busBwAbIb.Ib = 0;
	gBusMgrCtx.extPreAggregateData.pCompensated[extRouteIndex].bwValue.busBwAbIb.latencyNs = 0;
...
/* This function used only in case of 8909 */
Adsppm_Status BusMgrPreAggregateExtBW(
    AdsppmBusBWRequestValueType *pBwReqVal,
    AdsppmBwRequestClass requestClass)
{
    Adsppm_Status sts = Adsppm_Status_Success;
    uint32 extRouteIndex = 0;

	ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_INFO, "BusMgrPreAggregateExtBW class %u", requestClass);
    adsppm_lock(gBusMgrCtx.lock);

	sts = busMgrGetExtRouteIndex(&extRouteIndex, pBwReqVal->busRoute.masterPort, pBwReqVal->busRoute.slavePort);
	/* extRouteIndex is valid only when sts = Adsppm_Status_Success */
	if(Adsppm_Status_Success != sts)
	{
		ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_ERROR,
	...
	ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_ERROR, "usage %u usgperc %u, b/s %llu ",
						pBwReqVal->bwValue.busBwValue.usageType,
						pBwReqVal->bwValue.busBwValue.usagePercentage,
						(uint32)pBwReqVal->bwValue.busBwValue.bwBytePerSec,
						(uint32)(pBwReqVal->bwValue.busBwValue.bwBytePerSec >> 32));
	...
	if(requestClass == Adsppm_BwReqClass_Compensated)
	{
		...
		if((sts == Adsppm_Status_Success) &&
					(pBwReqVal->busRoute.masterPort == AdsppmBusPort_Adsp_Master) &&
					(pBwReqVal->busRoute.slavePort == AdsppmBusPort_Ddr_Slave))
		{
				...
				gBusMgrCtx.extPreAggregateData.maxAdspToDDrBw =
						...
						pBwReqVal->bwValue.busBwValue.bwBytePerSec);
		}
	}
	else // if(requestClass == Adsppm_BwReqClass_Generic)
	{
	...
	{
		ADSPPM_LOG_PRINTF_1(ADSPPM_LOG_LEVEL_ERROR,
				"External 8909 BW pre-aggregation failed: %u", sts);
	}
    adsppm_unlock(gBusMgrCtx.lock);
    return sts;
}
...
static void busMgrAggregateExternalBW(coreUtils_Q_Type *pBWReqQ)
{
    ...
    for(extRouteIndex = 0; extRouteIndex < gBusMgrCtx.extRouteNum; extRouteIndex++)
    {
        ...
        {
            /* Skipping to next index to avoid the values for DDR getting overwritten in pExtBwAggregateData */
            /* Anyhow the values here would all be zeros for DDR case for 8909 */
            ADSPPM_LOG_PRINTF_0(ADSPPM_LOG_LEVEL_ERROR, "SN: skipping INFO Only!!!");
            continue;
        }
         else
         {
...
/* This function used only in case of 8909 */
void BusMgrAggregateExtBW(AdsppmBusBWRequestValueType *pGetAggrData, coreUtils_Q_Type *pReqQ)
{
    Adsppm_Status sts = Adsppm_Status_Success;
    ...
    AdsppmBusBWRequestValueType *pPreAggregateGeneric;
    AdsppmBusBWRequestValueType *pPreAggregateCompensated;
    AdsppmBusBWRequestValueType *pCurrAggregate;
	...
	sts = busMgrGetExtRouteIndex(&extRouteIndex, pGetAggrData->busRoute.masterPort, pGetAggrData->busRoute.slavePort);

	/* extRouteIndex is valid only when sts = Adsppm_Status_Success */
	if(Adsppm_Status_Success == sts)
	{
        pAggregate = pGetAggrData;
        pPreAggregateGeneric = &(gBusMgrCtx.extPreAggregateData.pGeneric[extRouteIndex]);
        pPreAggregateCompensated = &(gBusMgrCtx.extPreAggregateData.pCompensated[extRouteIndex]);
            ...
            pAggregate->busRoute.slavePort == AdsppmBusPort_Ddr_Slave)
        {
            ...
            if(gBusMgrCtx.externalBwUpdatePending == FALSE)
                {
                    ...
                    if(compensatedBWFudgeFactorValues.floorFudgeFactor && compensatedBWFudgeFactorValues.ceilingFudgeFactor)
                    {
                        ...
                        BusMgr_GetCompensatedFudgedBwValueFromQ(pReqQ,
                                                        &pPreAggregateCompensated->bwValue.busBwAbIb.Ab,
                                                        &pPreAggregateCompensated->bwValue.busBwAbIb.Ib,
                                                        &compensatedBWFudgeFactorValues);
                    }
            ...
            ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO,
                    "BW ext Values after scaling factor: ib=%llu, ab=%llu",
       ...
       /* Place a copy of the aggregated value */
        pCurrAggregate = &(gBusMgrCtx.pExtBwAggregateData[gBusMgrCtx.preCurrentDDRIndex][extRouteIndex]);

        memscpy(pCurrAggregate, sizeof(AdsppmBusBWRequestValueType),
                pAggregate, sizeof(AdsppmBusBWRequestValueType));

        /* Update the index for next iteration */
        gBusMgrCtx.preCurrentDDRIndex ^= 1;

		ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO,
				"BW Values after generic factor: ib=%llu, ab=%llu",
...
Adsppm_Status BusMgr_GetAggregatedBwInfo(AdsppmInfoAggregatedBwType *pBusInfo)
{
    ...
    for(index = 0; index < gBusMgrCtx.extRouteNum; index++)
    {
		pBwVote = &(gBusMgrCtx.pExtBwAggregateData[gBusMgrCtx.preCurrentDDRIndex^1][index]);
	...
	/* Separating out the loops as different precurrent indices are used */
    for(index = 0; index < gBusMgrCtx.extRouteNum; index++)
    {
	...
	ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO, "Val extAhbDDrBwAb Ab=%llu, extAhbDdrBwIb=%llu",
        (uint32) pBusInfo->extAhbDdrBwAb,
        (uint32) (pBusInfo->extAhbDdrBwAb >> 32),
        (uint32) pBusInfo->extAhbDdrBwIb,
        (uint32) (pBusInfo->extAhbDdrBwIb >> 32));
    ...
    )
{
    ...
    // Go through each client's bw request; aggregate the BW requests only for
    // compensated request types - The Queue here is MIPS
    pRequest = (RMHandleType*) coreUtils_Q_Check(pRequestQueue);
    while(pRequest != NULL)
    {
        AdsppmBwSingleReqType* pBwReqData = (AdsppmBwSingleReqType*) pRequest->pRequestData_Ext;
        if((pRequest->validFlag_Ext == RM_Valid) &&
            (pBwReqData != NULL) &&
            (pBwReqData->requestClass == Adsppm_BwReqClass_Compensated))
        {
            AdsppmBusBWRequestValueType* pBwReq = &pBwReqData->adsppmBwReqDataVal;
    ...
    *pIb = finalIb;
    ...
    ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO,
        "After BW ConcurrencyQ Adjustment v2 - Ab %llu Ib %llu ",
        (uint32) *pAb, // two params for %llu
        (uint32) (*pAb >> 32),
        (uint32) *pIb, // two params for %llu
        (uint32) (*pIb >> 32));
...
/* This function used only in case of 8909 */
Adsppm_Status BusMgr_GetCompensatedDDRBw(
    coreUtils_Q_Type* pRequestQueue,
    AdsppmBusRouteType* pRoute,
    uint64* pTotalRequestedBw)
{
        ...
        (AdsppmBusPort_Ddr_Slave == pRoute->slavePort))
    {
        ...
        if(compensatedBWFudgeFactorValues.floorFudgeFactor && compensatedBWFudgeFactorValues.ceilingFudgeFactor)
                {
                    BusMgr_GetCompensatedFudgedBwValueFromQ (pRequestQueue,
                                                        &aggResult.Ab,
                                                        &aggResult.Ib,
                                                        &compensatedBWFudgeFactorValues);
                }
