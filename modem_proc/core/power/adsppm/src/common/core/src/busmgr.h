...
Adsppm_Status BusMgrPreAggregateExtBW(AdsppmBusBWRequestValueType *pBwReqVal, AdsppmBwRequestClass requestClass);

/**
 * @fn BusMgr_GetCompensatedDDRBw- Returns the total requested DDR BW from compensated
 *                              request types, for the specified bus route.
 *                              The output, in bytes/second, is written to the
 *                              pointer provided in pTotalRequestedBw parameter.
 */
Adsppm_Status BusMgr_GetCompensatedDDRBw(coreUtils_Q_Type* pRequestQueue, AdsppmBusRouteType* pRoute, uint64* pTotalRequestedBw);
