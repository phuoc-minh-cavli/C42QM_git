...
================================================================================
$Header: //components/rel/core.mpss/3.10/power/sleep2.0/src/os/qurt/sleep_os.c#4 $
$DateTime: 2019/06/24 23:57:28 $
==============================================================================*/
#include <stdlib.h>
#include "sleepi.h"
#include "rcinit.h"
...
kvp_t        *pSleepBimcKvp;
/* NPA query handle and client for BIMC to put DDR votes */
npa_query_handle g_SleepBimcQueryHandle;
npa_client_handle g_sleepBimcClientHandle;
/*==============================================================================
...
static void sleepOS_ClkBimcNodeAvailableCb( void *context,
                                          unsigned int event_type,
                                          void *data,
                                          unsigned int data_size );										  
 ...
 * sleep_create_in_stm_client_handle_cb
 *
 * @brief Obtains the query handle for clock bimc node when it
 *         becomes available.
 *        Also creates async client to put ddr bw vote. 
 */
static void sleepOS_ClkBimcNodeAvailableCb( void *context,
                                          unsigned int event_type,
                                          void *data,
...
{										  
  ...
  CORE_VERIFY_PTR(g_SleepBimcQueryHandle = 
                  npa_create_query_handle("/clk/bimc"));
				  
  CORE_VERIFY_PTR( g_sleepBimcClientHandle =
                     npa_create_sync_client("/clk/bimc", "clk_bimc_client",
                                            NPA_CLIENT_REQUIRED ) );
...
static void sleepOS_qurtInit( void )
{
  ...
  npa_resource_available_cb( "/clk/bimc",
                             sleepOS_ClkBimcNodeAvailableCb, NULL );							 
							 

  /* Set the initial solver configuration. */
  sleepSolver_setSolverFunction(&sleep_lookup_table_solver);

  /* KVP pointer for voting on clk bimc resource */
  pSleepBimcKvp  = kvp_create(0);
  CORE_VERIFY_PTR(pSleepBimcKvp);
...
void sleepOS_performLPMExit(void)
{
 ...
 /* Get all the profiling timestamp units after LPR exits since clocks may be required to
   * be turned back on as part of the exit to read them. */
