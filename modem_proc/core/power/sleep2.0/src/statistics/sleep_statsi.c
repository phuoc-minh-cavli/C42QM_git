...
================================================================================
$Header: //components/rel/core.mpss/3.10/power/sleep2.0/src/statistics/sleep_statsi.c#4 $
$DateTime: 2019/03/25 06:58:36 $
==============================================================================*/
#include <stdint.h>
#include <stdlib.h>
#include "sleep.h"
...
/*count for tracking large Warmboot latency*/
volatile uint32         g_WBCount=0;
...
static void sleepStats_logWakeupTimes()
{
  ...
  sleepLog_printf(SLEEP_LOG_LEVEL_PROFILING, 4 + (2 * 2),
...
void sleepStats_logWakeup(sleepStats_input *statInput)
{
  ...
  if(statInput->os_overhead.count > 0)
  {
    ...
    WBtime = TICKS_TO_US(statInput->os_overhead.sleep_resume - 
                                statInput->os_overhead.awake_time);
  
    if(WBtime > 3000)
    {
	  ++g_WBCount; 
      sleepLog_printf(SLEEP_LOG_LEVEL_PROFILING, 2, 
	                "(WB_3000 time: %duS)"
					"(WB_3000 count: %d)" , 
                    WBtime, g_WBCount); 
    }
