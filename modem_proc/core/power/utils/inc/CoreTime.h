...
$Header: //components/rel/core.mpss/3.10/power/utils/inc/CoreTime.h#2 $
==============================================================================*/
#ifndef CORE_TIME_H
#define CORE_TIME_H
...
CORE_STATIC_INLINE void CoreTimetick_busyWait(uint32 xoTicks)
{
  uint64 endTime;
  uint64 utimer;
  ...
  /* Calulcate the delayed timer value to compare to */
  endTime = utimer + xoTicks;

  /* Delay until the current time is past the delay value */
  do
  {
    __asm__ volatile ("%[utimer] = UTIMER" : [utimer] "=r"(utimer));
  }while(utimer < endTime);
