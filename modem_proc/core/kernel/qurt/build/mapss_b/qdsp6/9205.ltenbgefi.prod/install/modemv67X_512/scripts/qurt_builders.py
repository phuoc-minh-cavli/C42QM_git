...
def cust_config_emitter(target, source, env):
   ...
   env.Depends(target, "${Q6_RTOS_ROOT}/scripts/lib/ecc_xml.py")
...
#------------------------------------------------------------------------------
def modify_dbgr_command(filename, or_cmd, mod_cmd):
    f = open(filename, 'r')
    or_contents = f.read()
    f.close()
    
    mod_contents = or_contents.replace(or_cmd, mod_cmd)
    f = open(filename, 'w')
    f.write(mod_contents)
    f.close()

def debugger_add_cmd_file(env, cmdfile):
    env['DEBUGGER_CMD_FILE'] = cmdfile
    return
    
def test_sim_generate(env):
    env.AddMethod(debugger_add_cmd_file, 'DebuggerCmdFile')
...
def test_sim_builder(target, source, env):
    machine=''
    if env.subst("$Q6VERSION") == 'v56' or env.subst("$Q6VERSION") == 'v60' or env.subst("$Q6VERSION") == 'v61':
    ...
    if env.subst('$LIVEOUTPUT') and os.name == 'posix' and os.isatty(1):
        ...
        if env.subst('$DEBUGGER_TEST') == 'True':
            dbgr_cmd = "%s -S %s" % (env.subst("${DBGR}"), env['DEBUGGER_CMD_FILE'][0].get_abspath())
            dbgr_command = dbgr_cmd.split()
            modify_dbgr_command(env['DEBUGGER_CMD_FILE'][0].get_abspath(), "start_sim", str("start_sim ") + str(cmd))
            simproc = subprocess.Popen(dbgr_command, stdin=slave, stdout=slave, stderr=slave, close_fds=True, cwd=env.subst("${BUILD_DIR}"))
        else:
        ...
        try:
            while True:
                ...
                #
                    ...
                    if env.subst('$DEBUGGER_TEST') == 'True':
                        f = open((env.subst("${BUILD_DIR}") + '/dbgr_results.txt'), 'r')
                        result_lines = f.readlines()
                        f.close()
                        
                        test_passed = True
                        for each_line in result_lines:
                            if "PASS" not in each_line:
                                test_passed = False
                                break
                            if test_passed == True:
                                simproc.returncode = 0
...
def traptab_generator(source, target, env, for_signature):
   return ' '.join(['python ', str(source[0]),'-outtab', str(target[0]), '-inc', str(source[2]), '-itraps',str(source[3]), env['CC'],env.subst('$CCFLAGS'),' '.join("-I%s" % w for w in env['CPPPATH']),'-E -x c', str(source[1])])
...
def qurt_program_generate(env):
   def qurt_program_generator(source, target, env, for_signature):
      ...
      cmd = [control['LINK'],
             ...
             '--start-group',
             [str(source[i]) for i in control['OBJSRCS'] if os.path.basename(str(source[i])) != 'fini.o'],
             '--end-group',
             [str(source[i]) for i in control['OBJSRCS'] if os.path.basename(str(source[i])) == 'fini.o'],
             '-o', str(target[0])]
