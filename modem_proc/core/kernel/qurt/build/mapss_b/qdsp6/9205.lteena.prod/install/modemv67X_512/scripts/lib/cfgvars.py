...
#
#    KERNEL   -- indicates that associated symbol is owned by micro-kernel. It is placed in
#                .ukernel.main section
#
...
QURTK_BQ_shared
QURTK_bus_prio_cfg
QURTK_chicken_bits_chicken              KERNEL
QURTK_chicken_bits_chicken_2nd          KERNEL
QURTK_chicken_bits_chicken_3rd          KERNEL
QURTK_chicken_bits_duck                 KERNEL
QURTK_chicken_bits_rgdr                 KERNEL
QURTK_chicken_bits_rgdr_2nd             KERNEL
QURTK_chicken_bits_rgdr_3rd             KERNEL
QURTK_chicken_bits_turkey               KERNEL
...
qurtos_fastint_stack_island             ISLAND QURTOS
qurtos_fastint_stack_main               QURTOS
qurtos_main_bitmask
...
qurt_tcm_dump
qurt_tcm_dump_size
QURTK_tcm_save_size                     KERNEL
QURT_timer_intno                        ISLAND
    ...
    def add_section(self, secname, **kw):
        ...
        if L or prolog or epilog:
            ...
            for out_prefix, in_prefix in zip(out_prefixes, in_prefixes):
                self.add_override_section(secname, L, out_prefix=out_prefix, in_prefix=in_prefix, prolog=prolog, epilog=epilog)
    def add_override_section(self, secname, in_sections, out_prefix=None, in_prefix=None, prolog=None, epilog=None):
        if not out_prefix:
            out_prefix = ""
        if not in_prefix:
            in_prefix = ""
        ...
        if not epilog:
            epilog = []
        self.sec_list.append('  {0}{1} : {{'.format(out_prefix, secname))
        self.sec_list.extend(['    '+s for s in prolog])
        for A in in_sections:
            self.sec_list.append('    *({0}{1})'.format(in_prefix, A[0]))
