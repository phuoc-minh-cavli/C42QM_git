...
CONFIG_ZI_SPACE("Stack space for main thread",
                ...
                unsigned char [ROUNDUP({:MAINSTACKSIZE},8)],
                qurtos_app_stack,
                CACHEALIGNED);
...
CONFIG_ZI_SPACE("Space for tcm tag is not defined in the config xml",
                __attribute__((section(".bss.qurtos"))),
                unsigned char [ROUNDUP({:TCMSIZE},32)],
...
SET_CONFIG_VALUE(QURTK_timer_freq_init,{:TIMERFREQINIT});
#else
 ...
 */
#define NAMED_MMAP_INIT {{:NAMED_MEM_SECTIONS} 0}
...
/* Define stack memory for fast interrupt callback */
    ...
    unsigned long long qurtos_fastint_stack_island[{:QURT_CONFIG_MAX_HTHREADS}][DIVUP({:FASTINTSTACKSIZE},8)];
  ...
  #else
    unsigned long long qurtos_fastint_stack_main[{:QURT_CONFIG_MAX_HTHREADS}][DIVUP({:FASTINTSTACKSIZE},8)];
...
CONFIG_ZI_SPACE("",
                ...
                unsigned char [ROUNDUP({:KERNELHEAPSIZE},4)],
                qurtos_heap,
                CACHEALIGNED);
