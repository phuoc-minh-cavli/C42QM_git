...
kernel_db = [
    ...
    ["isr_default_priority", long_attr, "optional", "ISRDEFAULTPRIO",      60     ],
    ["max_callbacks_in_isr_frame", 
                           long_attr, "optional", "ISRMAXSHAREDINT",       30     ],
...
STM_tracemask_el = Element("trace_mask_shutdown",
...
Kernel_el = Element("kernel", object_cache_el(), ramfs_scan_el(), Fastint_stack_el, Mprotect.element(), Tcmdump_el, Autostack_el, VTLB_el, STM_tracemask_el, *[Element(e[0], value=(e[1],e[2])) for e in kernel_db]) 
...
def collect_kernel_element(cfg):
    ...
    try:
        cfg['ERRSTMTRACEMASK'] = k.find_child("trace_mask_shutdown").value
    except AttributeError:
        cfg['ERRSTMTRACEMASK'] = k.find_child("trace_mask").value
    ...
    if cfg['QURTKTCMDUMPTYPE'] == None:
        ...
        if tcm_dump_power_type != 'static' and tcm_dump_power_type != 'user' and tcm_dump_power_type != 'none' and tcm_dump_power_type != 'pool' and tcm_dump_power_type != 'boot' and tcm_dump_power_type != None:
            raise Exception('"power" attribute can only be "static", "user", "none", "pool", "boot" under tcm_dump tag in the cust_config xml file')
        ...
        if tcm_dump_power_type == None:
            if tcm_dump_error_type != 'static' and tcm_dump_error_type != 'user':
		...
		# "boot": static TCM will be saved to same location it was copied from during cold boot
        ...
        if tcm_dump_error_type == 'user' and tcm_dump_power_type == 'boot':
            cfg['QURTKTCMDUMPTYPE'] = '4'
    ...
    try: 
        ...
        #fixme: make sure autostack is not larger than vtlb entries:
        vtlb_entries = int(cfg['VTLB_ENTRIES'])
        if (auto_pool_size >> 12) > vtlb_entries:
            raise Exception('Number of autostack entries exceeds VTLB entries...')
