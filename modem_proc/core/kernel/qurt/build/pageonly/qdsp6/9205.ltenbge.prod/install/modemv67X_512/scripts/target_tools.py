...
v66default=os.path.join(default_root,"8.2.04")
v56wlandefault=os.path.join(default_root,"8.0.13")
v67default=os.path.join(earlyaccess_default_root,"8.3.alpha2")

toolsver_whitelist = ["3.1", "5.0", "5.1", "6.2", "6.4", "7.0", "7.1", "7.2", "7.3", "7.4", "8.0", "8.1", "8.2", "8.3"]
#------------------------------------------------------------------------------
...
def generate(env, target='staticv62'):
   #===============================================================================
   ...
   if target=='mpqurt':
      ...
      hexagon_hw_ver=65
   elif target=='ADSPv65MP_ISV':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/ADSPv65MP_ISV'
      hexagon_hw_ver=65
   elif target=='ADSPv66MP':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/ADSPv66MP'
      hexagon_hw_ver=66
   elif target=='ADSPv66L_1536':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/ADSPv66L_1536'
      hexagon_hw_ver=66
   elif target=='ADSPv66K_1024':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/ADSPv66K_1024'
      hexagon_hw_ver=66
      ...
      hexagon_hw_ver=66
   elif target=='wlanv67_rom':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/wlanv67_rom'
      hexagon_hw_ver=67
      ...
      hexagon_hw_ver=66
   elif target=='computev66_ISV':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/computev66_ISV'
      hexagon_hw_ver=66
   elif target=='computev66E_256':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/computev66E_256'
      hexagon_hw_ver=66
   elif target=='computev66A_512':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/computev66A_512'
      hexagon_hw_ver=66
   elif target=='computev67':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/computev67'
      hexagon_hw_ver=67
   elif target=='modemv66':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/modemv66'
      hexagon_hw_ver=66
   elif target=='modemv67':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/modemv67'
      hexagon_hw_ver=67
   elif target=='modemv67X_512':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/modemv67X_512'
      hexagon_hw_ver=67  	  
   elif target=='npuv66':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/npuv66'
      hexagon_hw_ver=66
   elif target=='sdkv65':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/sdkv65'
      hexagon_hw_ver=65
   elif target=='sdkv66':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/sdkv66'
      hexagon_hw_ver=66
   ...
   if hexagon_hw_ver == 66:
      ...
      if target == 'ADSPv66L_1536':
        env.Replace(T32_MCD_ARCH = "v66l_1536")
      if target == 'ADSPv66K_1024':
        env.Replace(T32_MCD_ARCH = "v66k_1024")
      if target == 'sensorsv66':
        env.Replace(T32_MCD_ARCH = "v66b_1536")
      if target == 'computev66':
        env.Replace(T32_MCD_ARCH = "v66g_1024")
      if target == 'computev66_ISV':
        env.Replace(T32_MCD_ARCH = "v66g_1024")
      if target == 'computev66E_256':
        env.Replace(T32_MCD_ARCH = "v66e_256")
      if target == 'computev66A_512':
        env.Replace(T32_MCD_ARCH = "v66a_512")
      if target == 'npuv66':
        env.Replace(T32_MCD_ARCH = "V66n_256")
      if target == 'sdkv66':
        env.Replace(T32_MCD_ARCH = "v66g_1024")
   if hexagon_hw_ver == 67:
      env.Replace(T32_SYS_CPU = "HexagonV66")
      if target == 'modemv67':
        env.Replace(T32_MCD_ARCH = "v67h_3072")
      if target == 'computev67':
        env.Replace(T32_MCD_ARCH = "v67g_1024")
      if target == 'wlanv67_rom':
        env.Replace(T32_MCD_ARCH = "v67c_0")
   ...
   if (hex_tools_root):
      ...
      elif hexagon_hw_ver == 67:
         hex_tools_root = v67default
      else:
   ...
   toolsver_match = re.search('\D*(\d+)\.(\d+).*', hex_tools_ver)
   ...
   if target=="wlanv67_rom":
     q6version = 'v'+str(hexagon_hw_ver)+'t'
   else:
     q6version = 'v'+str(hexagon_hw_ver)   
   hexagon_tools = "HEXAGON%s" % hexagon_hw_ver
   
   env.Replace(Q6_OSAM = 'v'+str(hexagon_hw_ver))
...
#variable used by testcases/Sconscript
   ...
   # HEXAGON debugger
   if(version_compare(hex_tools_ver, '7.0.0') > 0):
      env.Replace(DBGR = QDSP6QCBIN + "hexagon-lldb")
   else:
      env.Replace(DBGR = QDSP6QCBIN + "hexagon-gdb")
   ...
   # Disable SP update to support autostack v2 feature.
   env.Append(CFLAGS = "-fdisable-sp-updates")
   ...
   if target == 'wlanv67_rom': 
      env.Replace(BUILD_CCFLAGS = ["-D__QURTFILENAME__=\\\"$SOURCE.file\\\""] + ['-mpacket-split-sw',] + env['BUILD_CCFLAGS'])
   else:
       env.Replace(BUILD_CCFLAGS = ["-D__QURTFILENAME__=\\\"$SOURCE.file\\\""] + env['BUILD_CCFLAGS'])
   
   if target == 'ADSPv62MP' or target == 'computev65' or target == "sdkv65" or target == "ADSPv65MP_ISV":
      env.Replace(HVXCFLAGS = "-mhvx")
   elif target == 'computev66' or target == 'computev66E_256' or target == 'computev66A_512' or target =='computev67' or target == "sdkv66" or target == "computev66_ISV":
      env.Replace(HVXCFLAGS = "-mhvx -mhvx-length=128B")
   else:
      env.Replace(HVXCFLAGS = "")
   ...
   # Init FLAGS
   env.Replace(CCFLAGS   = "$HEXAGON_MACHINE $HEXAGONCC_WARN $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $HEXAGON_STKPRO $BUILD_CCFLAGS $HVXCFLAGS $DEBUGFLAGS $CC_DFLAGS $CFLAGS $LLVMCFLAGS $PICCFLAGS $ERRATACFLAGS $EXPOSE_API")
   env.Replace(CXXFLAGS  = "$HEXAGON_MACHINE $HEXAGONCXX_WARN $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $HEXAGON_STKPRO $BUILD_CCFLAGS $HVXCFLAGS $CC_DFLAGS $CFLAGS $LLVMCFLAGS $ERRATACFLAGS")
   env.Replace(ASMFLAGS  = "$HEXAGON_MACHINE $HEXAGONCC_WARN $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $HEXAGON_STKPRO $BUILD_CCFLAGS $HVXCFLAGS $CC_DFLAGS $ERRATACFLAGS $EXPOSE_API")
   env.Replace(ASFLAGS   = "$HEXAGON_MACHINE $HEXAGONCC_WARN $HEXAGON_UNDEF $HEXAGONCC_OPT $HEXAGON_DBG $HEXAGON_LANIND $HEXAGON_STKPRO $BUILD_CCFLAGS $HVXCFLAGS $CC_DFLAGS $PICCFLAGS $ERRATACFLAGS")
   ...
   env.Replace(QDSP6OBJS3  = [
      ...
      '${Q6_RTOS_LIBS}/libqurt_island.a',
      '${Q6_RTOS_LIBS}/libqurt_main.a',
