...
class Elf32ProgramHeader(QurtHeaderCodec):
    ...
    def oneline(p):
        return 'Vaddr %08X-%08X, Paddr %08X-%08X section Type %d' % (p.p_vaddr, p.p_vaddr + p.p_memsz-1, p.p_paddr, p.p_paddr + p.p_memsz-1,p.p_type)
...
class Elf32Object(object):
    ...
    def defined_all(self):
        #
        #  Return an iterator which returns:
        #    (symbol name, section)
        #  for every defined symbol in the object
        #
        for s in self.shdr:
            if s.sh_type == s.SHT_SYMTAB:
                strtab = self.shdr[s.sh_link].contents().read()
                ...
                for _ in range(s.sh_size / s.sh_entsize):
                    ...
                    if (sym.st_shndx == sym.SHN_COMMON):
                            sym.name = strtab[sym.st_name:].split('\0')[0]
                            yield sym.name, self.shdr[-1], sym.st_size
        ...
        for s in self.shdr:
            if s.sh_type == s.SHT_RELA:
                ...
                for r in relas:
                    ...
                    that_section_name = self.section_at(that_sym.st_shndx).name
                    yield this_section.name, [that_sym.name,that_section_name]
...
class Elf32TrialLink(object):
    ...
    def add_apis(self, api):
        self.apis.append(api)
    def add_ignorelist(self, ignorelist):
        self.ignorelist.append(ignorelist)
    def run(self):
        import fnmatch, os
        #
        ...
        global_symdefs = dict()
        global_symdefs_all = dict()
        global_relocations = dict()
        global_symdefs_sect = dict()
        for obj in self.objs:
			#
            ...
            for name, section, size in obj.defined_globals():
                if name in global_symdefs:
                    ...
                    print '%s defined at %s %s %s' % (name,
                                                      ...
                                                      section.obj.objname,
                                                      section.name)
                    print 'Multiple definitions of %s' % name
                global_symdefs[name] = (section, size)

			#
			#  Create a nested dictionary for all symbols (global and local) where 
			#  1st level Key-> Library, Value -> 2nd level dictionary
			#  2nd level Key-> Object,  Value -> 3rd level dictionary
			#  3rd level Key-> Section, Value -> list of (Symbol, size)
			#

            for name, section, size in obj.defined_all():
                templist = list()
                if section.obj.libname in global_symdefs_sect.keys():
                    if section.obj.objname in global_symdefs_sect[section.obj.libname].keys():
                        if section.name in global_symdefs_sect[section.obj.libname][section.obj.objname].keys():
                            templist = list(global_symdefs_sect[section.obj.libname][section.obj.objname][section.name])
                            templist.append((name,size))
                            global_symdefs_sect[section.obj.libname][section.obj.objname][section.name] = templist
                        else:
                            global_symdefs_sect[section.obj.libname][section.obj.objname][section.name] = [(name,size)]
                    else:
                        global_symdefs_sect[section.obj.libname][section.obj.objname] = {section.name:[(name,size)]}
                else:
                    global_symdefs_sect[section.obj.libname] = {section.obj.objname:{section.name:[(name,size)]}}
            ...
            for name, section, size in obj.defined_all():
                global_symdefs_all[name] = (section, size)

			#
			#  Create a nested dictionary for relocations where 
			#  1st level Key-> Library, Value -> 2nd level dictionary
			#  2nd level Key-> Object,  Value -> 3rd level dictionary
			#  3rd level Key-> Section, Value -> list of Relocations
			#

            for sect_name, [ref_sym_name,ref_sec_name] in obj.get_relocations():
                templist = set()
                if (obj.libname in global_relocations.keys()): 
                   if(obj.objname in global_relocations[obj.libname].keys()):
                       if(sect_name in global_relocations[obj.libname][obj.objname].keys()):
                           templist = set(global_relocations[obj.libname][obj.objname][sect_name])
                           templist.add((ref_sym_name,ref_sec_name))
                           global_relocations[obj.libname][obj.objname][sect_name] =  templist
                       else:
                           global_relocations[obj.libname][obj.objname][sect_name] = set([(ref_sym_name,ref_sec_name)])
                   else:
                       global_relocations[obj.libname][obj.objname] = {sect_name:set([(ref_sym_name,ref_sec_name)])}
                else:
                    global_relocations[obj.libname] = {obj.objname:{sect_name:set([(ref_sym_name,ref_sec_name)])}}
		...
		#
		#  Read all the base symbol list which would be used to search for dependencies 
		#  Get all the sections that these symbols belong to and add those sections to gray_sections directory.
        #
		#  gray_sections directory:   
		#  1st level Key-> Library, Value -> 2nd level dictionary
		#  2nd level Key-> Object,	Value -> List of sections marked gray
		#
        for api in self.apis:
            if api in global_symdefs_all.keys():
                # The symbol that has to be a global symbol.
                templist = set()
                total_original_size += global_symdefs_all[api][1]
                if global_symdefs_all[api][0].obj.libname in gray_sections.keys():
                    if global_symdefs_all[api][0].obj.objname in gray_sections[global_symdefs_all[api][0].obj.libname].keys():
                        templist = set(gray_sections[global_symdefs_all[api][0].obj.libname][global_symdefs_all[api][0].obj.objname])
                        templist.add(global_symdefs_all[api][0].name)
                        gray_sections[global_symdefs_all[api][0].obj.libname][global_symdefs_all[api][0].obj.objname] = list(templist)
                    else:
                        gray_sections[global_symdefs_all[api][0].obj.libname][global_symdefs_all[api][0].obj.objname] = [global_symdefs_all[api][0].name]
                else:
                    gray_sections[global_symdefs_all[api][0].obj.libname] = {global_symdefs_all[api][0].obj.objname:[global_symdefs_all[api][0].name]}
            else:
                # If symbol is not global, add to a list of undefined symbols. We will print those out
                undef_syms.append(api)

        for ignorelist in self.ignorelist:
            if ignorelist in global_symdefs_all.keys():
                ignore_sections.append(global_symdefs_all[ignorelist][0].name)
        #
        #  This is the main loop that finds out all the cross-references recursively.
        #  We will go through and process one gray section at a time. If a gray section has relocations
        #  to another symbol/section, add the corresponding section to the gray_sections directory.
        #  When a section is processed remove it from gray_sections directory and add it to black_sections directory
        #
        #  Both gray_sections and black_sections are nested dictionaries:
        #  1st level Key-> Library, Value-> 2nd level dictionary
        #  2nd level Key-> Object,  Value-> List of sections
        #
        while gray_sections:
            libraries = gray_sections.keys()
            for lib in libraries:
                objects = gray_sections[lib].keys()
                for obj in objects:
                    sections = gray_sections[lib][obj]
                    for sec in sections:
                        if sec in ignore_sections:
                            pass
                        elif lib not in black_sections.keys() or obj not in black_sections[lib].keys() or sec not in black_sections[lib][obj]:
                            # We process gray sections only if they are not already processed
                            if obj in global_relocations[lib].keys():
                                # This object has some relocations
                                temp_sec = []
                                if sec in global_relocations[lib][obj].keys():
                                    # The section has some relocations. Find out all the relocations from the lib->obj->section and mark those sections as gray.
                                    temp_sec = global_relocations[lib][obj][sec]
                                    temp_undef_sec = []
                                    temp_sec_1 = temp_sec.copy()
                                    for sect in temp_sec:
                                        if not sect[1]:
                                            # This is symbol based relocation. Get the section information from symbols. If this is a 
                                            # section based relocaiton, add the section to gray_sections
                                            if sect[0] in global_symdefs_all.keys():
                                                # The symbol is a global symbol. Get the library->obj->section information and update gray_sections
                                                templist = set()
                                                if global_symdefs_all[sect[0]][0].obj.libname in gray_sections.keys():
                                                    if global_symdefs_all[sect[0]][0].obj.objname in gray_sections[global_symdefs_all[sect[0]][0].obj.libname].keys():
                                                        templist = set(gray_sections[global_symdefs_all[sect[0]][0].obj.libname][global_symdefs_all[sect[0]][0].obj.objname])
                                                        templist.add(global_symdefs_all[sect[0]][0].name)
                                                        gray_sections[global_symdefs_all[sect[0]][0].obj.libname][global_symdefs_all[sect[0]][0].obj.objname] = list(templist)
                                                    else:
                                                        gray_sections[global_symdefs_all[sect[0]][0].obj.libname][global_symdefs_all[sect[0]][0].obj.objname] = [global_symdefs_all[sect[0]][0].name]
                                                else:
                                                    gray_sections[global_symdefs_all[sect[0]][0].obj.libname] = {global_symdefs_all[sect[0]][0].obj.objname:[global_symdefs_all[sect[0]][0].name]}
#                                                gray_sections[global_symdefs_all[sect[0]][0].obj.libname][global_symdefs_all[sect[0]][0].obj.objname] = list(set(gray_sections[global_symdefs_all[sect[0]][0].obj.libname][global_symdefs_all[sect[0]][0].obj.objname]))
                                                temp_sec_1.remove(sect)
                                            else:
                                                # Symbol not present in global symbol list. Add it to undefined symbols list.
                                                undef_syms.append(sect[0])
                                                temp_undef_sec.append(sect)
                                        else:
                                            # This is section based relocation. Check if the section was already added.
                                            if sect[1] in gray_sections[lib][obj]:
                                                # If section is already present, dont add it again
                                                temp_sec_1.remove(sect)
                                    temp_sec = temp_sec_1.copy()
                                    # Extend the gray list
                                    for sect in set(temp_sec)-set(temp_undef_sec):
                                        gray_sections[lib][obj].extend([sect[1]])
                                else:
                                    # This section doesn't have any relocations. Just add the section directly to the gray_list.
                                    if sec not in gray_sections[lib][obj]:
                                        gray_sections[lib][obj].extend([sec])

                            # Add the current section to black_sections directory
                            templist = set()
                            if lib in black_sections.keys():
                                if obj in black_sections[lib].keys():
                                    templist = set(black_sections[lib][obj])
                                    templist.add(sec)
                                    black_sections[lib][obj] = templist
                                else:
                                    black_sections[lib][obj] = [sec]
                            else:
                                black_sections[lib] = {obj:[sec]}

                        # Remove the current section from gray_sections directory
                        del_sec = sec
                        gray_sections[lib][obj] = list(set(gray_sections[lib][obj]))
                        gray_sections[lib][obj].remove(del_sec)

                    if len(gray_sections[lib][obj]) == 0:
                        # If the section list is empty, delete obj key
                        del gray_sections[lib][obj]
                if len(gray_sections[lib]) == 0:
                    # If object list is empty, delete lib key
                    del gray_sections[lib]

        # Travese through black_sections and create a list of blacklisted symbols. Basically since we have a 
        # list of sections corresponding to objects and libraries, go through global_symdefs_sect and get a
        # list of all symbols defined in those sections.
        for lib in black_sections:
            for obj in black_sections[lib]:
                sections = black_sections[lib][obj]
                for sec in sections:
                    if sec in global_symdefs_sect[lib][obj].keys():
                        for syms in global_symdefs_sect[lib][obj][sec]:
                            if (syms[0] not in self.apis) and (syms not in black_syms):
                                black_syms.append((syms[0],syms[1]))
        return total_original_size, black_syms, black_sections, undef_syms
        #
        #
        #
        print 'Trial Link!'
