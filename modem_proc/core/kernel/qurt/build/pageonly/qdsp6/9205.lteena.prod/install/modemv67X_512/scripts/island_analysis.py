...
def island_analysis_builder(target, source, env):
    ...
    for src in source:
        if str(src).endswith('.o'):
            ...
            elf = src
        elif str(src).endswith('.lcs'):
            analysis_cmd += "-T "+str(src)+" "
        else:
    ...
    analysis_cmd += "-m "+env.get('Q6VERSION')+" "

    lflags = env.get('LFLAGS')
    if lflags:
        lflags = lflags.split('-L',1)[1]
        lflags = lflags.split('-L')
        for flag in lflags:
            analysis_cmd += "-L "+flag+" "

    link_dirs = env.get('LINK_DIRS')
    if link_dirs:
        link_dirs = link_dirs.split('-L')
        for dirs in link_dirs:
            analysis_cmd += "-L "+dirs+" "
