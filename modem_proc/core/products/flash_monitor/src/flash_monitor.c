...
#define FLSH_MON_STACK_SIZE  (2048)
typedef unsigned char *rex_stack_pointer_type;
static rex_tcb_type        flsh_mon_service_tcb;
static char flsh_mon_stack[FLSH_MON_STACK_SIZE];
static rex_task_attributes_type flsh_mon_task_attributes;
...
static void flash_monitor_thread(uint32 handle)
{
	uint32 timer_delta=0,sigs,rc;
	...
	uint32 flash_monitor_poll_period;
	...
	while(1)
	{
		...
		if (sigs & FLASH_MONITOR_TIMER_SIGNAL)
		{
...
#ifdef FS_DEBUG
			FS_MSG_ERROR_1("Modem current_qtime %x",crnt_qtime);
			FS_MSG_ERROR_3("Modem Current_wrt_cnt %d prv_wrt_cnt %d wrt_cnt_per_min %d",currnt_wrt_cnt,prv_wrt_cnt,wrt_cnt_per_min);
			...
			if(wrt_cnt_per_min > wrt_cnt_per_min_max)
			{
				...
				{
					FS_MSG_ERROR_0("Modem EFS file write failed\n");
					efs_close(fd);
					return ;
				}
			...
			if(wrt_cnt_per_min > USER_CONFIG_THRESHOLD_VAL)
			{
				DalTimetick_GetTimetick64(fs_os_timetick_info.handle,&crnt_qtime);
#ifdef FS_DEBUG
				FS_MSG_ERROR_2("Threshold Reached %u wrt_cnt_per_min %d",crnt_qtime,wrt_cnt_per_min);
				FS_MSG_ERROR_1("Threshold from file /fsmon_threshold_cnt %d",threshold_wrt_cntr);
#endif
...
int init_flash_monitor_start()
{
	...
	char task_name[] = "FLASH_MONITOR_THRD";
	struct fs_stat buf;
	char str[MAX_STRNG_SIZE];
	
	rex_task_attr_init(& flsh_mon_task_attributes);
	rex_task_attr_set_stack_addr(&flsh_mon_task_attributes,
			(rex_stack_pointer_type)flsh_mon_stack);
	rex_task_attr_set_stack_size(&flsh_mon_task_attributes,
			FLSH_MON_STACK_SIZE);
	rex_task_attr_set_prio(&flsh_mon_task_attributes, 10);
	rex_task_attr_set_entry_func(& flsh_mon_task_attributes,
			flash_monitor_thread,
			NULL);
	rex_task_attr_set_initially_suspended(&flsh_mon_task_attributes,
			FALSE);
	rex_task_attr_set_task_name(&flsh_mon_task_attributes,
			task_name);
	rex_common_def_task(&flsh_mon_service_tcb, & flsh_mon_task_attributes);
	...
	if ( buf.st_size != 0)
	{
		...
		for (i = 0; str[i] != ','; ++i)
		{
			FS_MSG_ERROR_2("str[%d]=%c",i,str[i]);
			res = res*10 + str[i] - '0';
		}
...
void flash_monitor_shutdown_event_cb(void)
{
	...
	{
#ifdef FS_DEBUG		
		FS_MSG_ERROR_2("Threshold Reached %u wrt_cnt_per_min %d",psm_qtime,wrt_cnt_per_min);
