/*=============================================================================
  ...
  DEPENDENCIES: I2CDRV_AddDevice must be called first then other api's.
                ...
                Copyright c 2011-2016,2019 Qualcomm Technologies Incorporated.
                All Rights Reserved.
...
/*=========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/buses/i2c/src/drv/I2cDriver.c#2 $
  $DateTime: 2019/06/19 09:06:04 $$Author: pwbldsvc $
...
)
{
   ...
   case I2cDrvCmd_Vote_PnocOn:
      if(0 == pDesc->pnocRefCnt){
      res = I2cPlat_AddPnocVote(&pDesc->plat); 
	  if ( I2C_RES_SUCCESS == res ) { 
	     pDesc->pwState |= I2CDRV_PWSTATE_PNOC_ON;
	  }
	  else{
		    I2CDEVICE_LOG(ERROR,"I2cDrvCmd:Unable to on I2cPlat_VoteI2cPnoc.Result = 0x%x \n",res);
		 }
	  }
	  pDesc->pnocRefCnt ++;
	  break;
   ...
   case I2cDrvCmd_Vote_PnocOff :
      if(1 == pDesc->pnocRefCnt){
      res = I2cPlat_RemovePnocVote(&pDesc->plat);
      if ( I2C_RES_SUCCESS == res ) { 
	     pDesc->pwState &= ~I2CDRV_PWSTATE_PNOC_ON;
      }
	  else{
		    I2CDEVICE_LOG(ERROR,"I2cDrvCmd:failed to I2cPlat_VoteI2cPnocOff.Result = 0x%x \n",res);
		 }
	  }
	  if(pDesc->pnocRefCnt > 0 ){
	     pDesc->pnocRefCnt --;
	   }
	  break;
