...
/*
  ...
  ==================================================================== 
  $Header: //components/rel/core.mpss/3.10/systemdrivers/clock/hw/mdm9205/mss/src/ClockMSS.c#4 $
  $DateTime: 2019/01/25 04:24:13 $
  $Author: pwbldsvc $
 
  when       who     what, where, why
  --------   ---     -------------------------------------------------
  01/23/19   shm     Removing GPLL0 init since MSS does not have RW
                     permissions
...
)
{
  ...
  for (i = 0; i < pImageCtxt->pBSPConfig->nNumCPUPerfLevelConfigs; i++)
  {
    if (Clock_IsBSPSupported(&pCPUPerfConfig[i].HWVersion) == TRUE)
    {
      ...
      pImageCtxt->CPUCtxt.PerfConfig.nInitPerfLevel = pCPUPerfConfig[i].nInitPerfLevel;
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
  /* Configure the CPU to the init performance level.                      */
  /*-----------------------------------------------------------------------*/
