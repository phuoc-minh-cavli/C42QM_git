...
FUNCTION timetick_legacy_init

DESCRIPTION
  This is an RCINIT function that initializes the DAL timetick handle and
  sync object handle.
  This was added to avoid race conditions when legacy timetick APIs
  simultaneously invoke timetick_attach().
...
void timetick_legacy_init(void)
{
  timetick_attach();
}
