...
$Header: //components/rel/core.mpss/3.10/systemdrivers/tlmm/src/DALTLMM.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/07/19   shm     Ported TLMM platform detection logic from HB codebase
...
DALResult TLMM_InitPlatformIO(TLMMClientCtxt *pCtxt)
{
      ...
      (hPlatformInfo != NULL))
  {
    ...
    for (nIdx = 0; nIdx < nArrSize; nIdx++)
    {
      if (pDeviceInfo != NULL)
      {
            ...
            (PlatformInfo.fusion == pDeviceInfo[nIdx].Platform.fusion))
        {
          ...
          {
            if (DalPlatformInfo_GetKeyValue(
                  hPlatformInfo, pDeviceInfo[nIdx].nKVPS,
    ...
    if (FALSE == bHasPlatIO)
    {
      ...
      DALSYS_LogEvent(
        pCtxt->pDevCtxt->DevId, DALSYS_LOGEVENT_WARNING,
        "TLMM_InitPlatformIO: No associated platform found. \
         Using default configs of %s.", pPlatform->pszPlatform);
