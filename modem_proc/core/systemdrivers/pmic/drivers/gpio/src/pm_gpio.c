...
*
*  &copy; Copyright 2010-2012,2020 Qualcomm Technologies Incorporated, All Rights Reserved
*/
...
$Header: //components/rel/core.mpss/3.10/systemdrivers/pmic/drivers/gpio/src/pm_gpio.c#2 $
                                                  ...
                                                  pm_gpio_src_config_type source)
{
    ...
    errFlag = pm_comm_write_byte_mask(gpio_ptr->comm_ptr->slave_id, dig_in_ctl, 0x07, data, 0);
    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
...
pm_err_flag_type pm_dev_gpio_set_inversion_configuration(uint8 pmic_chip, pm_gpio_perph_index gpio, boolean  inversion)
{
    ...
    pm_register_address_type dig_out_source_ctl = gpio_ptr->gpio_register->base_address + (pm_register_address_type)(gpio_ptr->gpio_register->peripheral_offset*(uint8)gpio) +
              gpio_ptr->gpio_register->dig_out_source_ctl;

    errFlag = pm_comm_write_byte_mask(gpio_ptr->comm_ptr->slave_id, dig_out_source_ctl, 0x80, data, 0 ) ;
...
pm_err_flag_type pm_dev_gpio_set_source_configuration(uint8 pmic_chip,pm_gpio_perph_index gpio, pm_gpio_src_config_type  source)
{
    ...
    errFlag = pm_comm_write_byte_mask(gpio_ptr->comm_ptr->slave_id, dig_out_source_ctl, 0x0F, (pm_register_data_type)(source), 0 ) ;
...
pm_err_flag_type pm_dev_gpio_status_get(uint8 pmic_chip, pm_gpio_perph_index gpio , pm_gpio_status_type *gpio_status)
{
    ...
    pm_register_address_type status1 =  base_address + gpio_ptr->gpio_register->status;
	pm_register_address_type dig_out_source_ctl =  base_address + gpio_ptr->gpio_register->dig_out_source_ctl; 
    ...
    errFlag = pm_comm_read_byte(gpio_ptr->comm_ptr->slave_id,status1, &reg_data, 0 );
    if(PM_ERR_FLAG__SUCCESS == errFlag)
    {
        reg_data2 = reg_data & 0x80;
        gpio_status->gpio_status_enable = (pm_gpio_status_enable_type)(reg_data2>>7);
        reg_data = reg_data & 0x01;
        gpio_status->gpio_input_val = (pm_gpio_status_input_val_type)reg_data;
    }
    ...
    // Read inversion status and output source sel
    errFlag = pm_comm_read_byte(gpio_ptr->comm_ptr->slave_id, dig_out_source_ctl, &reg_data, 0 );
    if(PM_ERR_FLAG__SUCCESS == errFlag)
    {
    ...
    if(PM_ERR_FLAG__SUCCESS == errFlag)
    {
        gpio_status->gpio_dtest_buffer_on_off = ((reg_data & 0x80)==0x0) ? PM_GPIO_DTEST_DISABLE : PM_GPIO_DTEST_ENABLE;
        reg_data = reg_data & 0x07;
