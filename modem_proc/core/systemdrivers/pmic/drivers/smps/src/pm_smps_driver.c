...
$Header: //components/rel/core.mpss/3.10/systemdrivers/pmic/drivers/smps/src/pm_smps_driver.c#3 $
...
void pm_smps_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint8 pmic_index)
{
    ...
    if (smps_ptr != NULL)
    {
		...
		else if (((smps_ptr->pm_pwr_data.pwr_specific_info[smps_index].periph_type == PM_HW_MODULE_HF_BUCK) &&(peripheral_info->peripheral_subtype >= PM_HW_MODULE_HF_BUCK_VPWM_CTL)))
        {
