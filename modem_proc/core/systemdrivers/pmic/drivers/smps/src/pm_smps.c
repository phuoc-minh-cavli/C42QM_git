...
$Header: //components/rel/core.mpss/3.10/systemdrivers/pmic/drivers/smps/src/pm_smps.c#2 $ 
...
pm_err_flag_type pm_smps_switching_freq(uint8 pmic_chip, uint8 perph_index, pm_smps_switching_freq_type freq)
{
    ...
    else
    {
      ...
      /*PMIC5 clk_div is not protected*/
        if(pwr_data->pwr_specific_info[perph_index].is_periph_unified_reg == 0)
        {
            /* CLK_DIV needs protected access. Write to SEC_ACCESS register */
            reg =  smps_freq_periph_offset + pwr_data->pwr_reg_table->SEC_ACCESS;
...
pm_err_flag_type pm_smps_quiet_mode(uint8 pmic_chip, uint8 perph_index, pm_quiet_mode_type mode, uint16 voltage_mv)
{
    ...
    else
    {
        ...
        if(pwr_data->pwr_specific_info[perph_index].is_periph_unified_reg ==  TRUE)
        {
             reg = (pm_register_address_type)(pwr_data->pwr_specific_info[perph_index].periph_base_address + pwr_data->pwr_reg_table->QM_PS_EXIT);
        }
        else
        {
