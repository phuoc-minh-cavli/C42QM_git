...
$Header: //components/rel/core.mpss/3.10/boot/qfprom/hw/core_2_0/qfprom_target.c#2 $
...
static qfprom_fusemap_data fusemap_data =
{
  SECURITY_CONTROL_BASE,
  SECURITY_CONTROL_BASE_PHYS,
...
)
{
  ...
  raw_base_address_start = fusemap_data.sec_ctrl_base_phy + fusemap_data.raw_lowest_offset;
  
  raw_base_address_end = fusemap_data.sec_ctrl_base_phy + fusemap_data.raw_highest_offset;
    
  corr_base_address_start = raw_base_address_start + QFPROM_RAW_TO_CORRECTED_ADDRESS_OFFSET;
    
  corr_base_address_end = raw_base_address_end + QFPROM_RAW_TO_CORRECTED_ADDRESS_OFFSET ;
    
  if ( (( row_address >= corr_base_address_start ) && ( row_address <= corr_base_address_end )) ||
       (( row_address >= raw_base_address_start ) && ( row_address <= raw_base_address_end )) )
  {
    *active_row_address = fusemap_data.sec_ctrl_base + row_address - fusemap_data.sec_ctrl_base_phy;
