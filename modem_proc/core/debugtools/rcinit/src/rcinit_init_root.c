...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/3.10/debugtools/rcinit/src/rcinit_init_root.c#2 $
$DateTime: 2019/07/23 05:11:39 $
$Change: 19949364 $
$Author: pwbldsvc $
===============================================================================*/
...
// static DalDeviceHandle* RcinitTimetickHandle = NULL;

/*static void rcinit_get_timetick32(uint32_t* ticktype_p)
{
  if (NULL != RcinitTimetickHandle)
  {
    // Best effort, no failure action
    DalTimetick_Get(RcinitTimetickHandle, ticktype_p);
  }
}*/
...
static DALResult rcinit_worker(DALSYSEventHandle hUnused, void* tid)
{
   ...
   while (RCINIT_NULL != *rcinit_group_p)
   {
      ...
      if (IS_INITFN(rcinit_p))
      {
         ...
         // rcinit_get_timetick32(&rcinit_internal.initfn_time_log[initfn_time_log_idx].start_time);
#if !defined(RCINIT_EXCLUDE_KERNEL_QURT)
...
#endif
         // rcinit_get_timetick32(&rcinit_internal.initfn_time_log[initfn_time_log_idx].end_time);
         initfn_time_log_idx++; 
         if (initfn_time_log_idx >= RCINIT_INITFN_MAX)
           initfn_time_log_idx = 0;
...
static void rcinit_dal_loop_worker_create(void)
{
   ...
   /*if (DAL_SUCCESS != DalTimetick_Attach("SystemTimer", &RcinitTimetickHandle))
   {
