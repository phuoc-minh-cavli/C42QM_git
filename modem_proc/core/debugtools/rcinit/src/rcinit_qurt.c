...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/3.10/debugtools/rcinit/src/rcinit_qurt.c#3 $
...
static void TASK_ENTRY(void* argv)
{
   ...
   if (RCINIT_NULL != rcinit_p && RCINIT_NULL != rcinit_p->handle)
   {
      ...
      if (RCINIT_NULL != entry && RCINIT_ENTRY_NONE != entry)
      {
         ...
         if(RCINIT_TASK_MAX == rcinit_internal_hs_list_add(rcinit_internal.hs_init, rcinit_internal.group_curr, rcinit_p->name))
         {
           /* Couldnt add task to hs_init array, need to increase array size */
           ERR_FATAL("Couldnt add task to hs_init array, need to increase array size %d", RCINIT_TASK_MAX, 0, 0);
         }
