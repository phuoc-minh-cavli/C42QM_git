...
//     sys_m_initiate_restart_ssreq(&sys_m_diag_ind_cb);
      #else
        sys_m_reset_config_type cfg;
        sys_m_err_type err;
        
        SYS_M_RESET_CONFIG_INIT(cfg);
        
        SYS_M_CONFIG_SET_RESET_TYPE(cfg, SYS_M_RESTART);
        MSG(MSG_SSID_TMS, MSG_LEGACY_ERROR,"sys_m_restart_execute: calling sys_m_reset with RESTART input");
        err = sys_m_reset(&cfg);
        if (err != SYS_M_STATUS_SUCCESS)
          MSG_2(MSG_SSID_TMS, MSG_LEGACY_ERROR,"sys_m_restart_execute called into sys_m_reset with Restart input. status %d", tmp_diag_info->type, err);
      #endif
      break;
    }
      break;
...
//      sys_m_init_restart_ssreq_with_res_code(&sys_m_diag_ind_cb, SSREQ_QMI_RES_MODEM_CONF_CHANGE);
    ...
    case SYS_M_INITIATE_SHUTDOWN: 
    {
      ...
      SYS_M_CONFIG_SET_RESET_TYPE(cfg, SYS_M_SHUTDOWN);
      
      MSG(MSG_SSID_TMS, MSG_LEGACY_ERROR,"sys_m_restart_execute: calling sys_m_reset with SHUTDOWN input");
      err = sys_m_reset(&cfg);
      if (err != SYS_M_STATUS_SUCCESS)
        MSG_1(MSG_SSID_TMS, MSG_LEGACY_ERROR,"sys_m_restart_execute: sys_m_reset returned err %d", err);
      break;
    }
    default:
