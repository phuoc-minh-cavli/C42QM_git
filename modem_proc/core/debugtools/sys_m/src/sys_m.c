...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/3.10/debugtools/sys_m/src/sys_m.c#2 $
$DateTime: 2019/03/25 08:40:45 $
$Author: pwbldsvc $
===============================================================================*/
#include "err.h"
#include "rcevt_dal.h"
 ...
 *
 */
void sys_m_hibernation(void)
{
  int registrants_count = 0;
  int ack_count = 0;

  SYS_M_MSG_HIGH("sys_m_hibernation process begins");

  SYS_M_LOCK();  
  
  // RC-ECB Callbacks
  if(rcecb_signal_name(SYS_M_HIBERNATION)){}
  ...
  if(rcevt_signal_name(SYS_M_HIBERNATION)){}
  if(rcevt_wait_count_name(SYS_M_HIBERNATION_ACK, registrants_count + ack_count)){}

  SYS_M_UNLOCK();

  SYS_M_MSG_HIGH("sys_m_hibernation process ends");
}
...
void sys_m_shutdown_terminate_in_STM(boolean block)
{
  HAL_dogDisable();
  ...
  /* Block only if caller wants to block, else return */
  if (block == TRUE)
  {
    while(always_true)
    {
