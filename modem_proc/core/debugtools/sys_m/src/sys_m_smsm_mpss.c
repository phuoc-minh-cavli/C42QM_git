...
$Header: //components/rel/core.mpss/3.10/debugtools/sys_m/src/sys_m_smsm_mpss.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
3/14/2019  ab      Fix for concurrency issue between sys_m_smsm task and dog 
                   task related to sys_m_smsm_tx_context structure
...
#include "smp2p.h"
#include "DALSys.h"
/* Below npa will not be available in pageonly image */
...
int32 sys_m_smsm_apps_set_cached(SYS_M_SUBSYS2AP_SMSM_BITS bits)
{
   ...
   else if(sys_m_smp2p_return_val == SYS_M_SMP2P_INVALID_PARAM)
   {
     ...
     return SYS_M_SMP2P_INVALID_PARAM;
   }
   SYS_M_SMSM_UNLOCK();
