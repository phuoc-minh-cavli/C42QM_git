...
/*=============================================================================
Edit History
$Header: //components/rel/core.mpss/3.10/debugtools/rcevt/src/rcevt.c#2 $
$DateTime: 2019/05/28 07:17:32 $
$Change: 19337571 $
$Author: pwbldsvc $
===============================================================================*/
...
typedef struct
{
  rex_tcb_type *tcb;
  uint32        mask;
  DalTimetickTime64Type timetick;
}rcevt_log_type;

#define RCEVT_REX_LOG_MAX 16
rcevt_log_type rcevt_rex_log[RCEVT_REX_LOG_MAX];
uint32         rcevt_rex_log_idx = 0;

rex_tcb_type *debug_tcb = NULL;


/*===========================================================================

FUNCTION RCEVT_GET_TIMETICK
...
===========================================================================*/
void rcevt_get_timetick(DalTimetickTime64Type * tick)
{
      ...
      switch (type)
      {
         ...
         case RCEVT_SIGEX_TYPE_SIGREX:
         {
            ...
            //Debug part
            {
              ...
              tcb = rex_self();
              rex_get_task_name(tcb, task_name, sizeof(task_name), NULL);
            
              if (0 == strcmp(&task_name[0], "dmm"))
                debug_tcb = tcb;
            }
         }
            break;
         ...
         switch (nfy_p_type)
         {
            ...
            case RCEVT_SIGEX_TYPE_SIGREX:
            {
               ...
               if (nde_p->threshold >= nfy_p->threshold)
               {
                  ...
                  if (NULL != debug_tcb && nfy_p_sigex_p->signal == debug_tcb)
                  {
                    rcevt_get_timetick(&rcevt_rex_log[rcevt_rex_log_idx].timetick);
                    rcevt_rex_log[rcevt_rex_log_idx].tcb = nfy_p_sigex_p->signal;
                    rcevt_rex_log[rcevt_rex_log_idx].mask = nfy_p_sigex_p->mask;
                    rcevt_rex_log_idx = (rcevt_rex_log_idx + 1) % RCEVT_REX_LOG_MAX;
