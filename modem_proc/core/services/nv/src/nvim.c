...
Copyright (c) 2005-2016, 2018-2019 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/3.10/services/nv/src/nvim.c#5 $ $DateTime: 2019/08/14 00:45:33 $ $Author: pwbldsvc $
...
/* Pointer to the mutex object for protecting critical-section of nvim 
   code*/
static qurt_mutex_t nvim_qemu_mutex;
/*MACRO for locking/unlocking the qmutex*/
#define NVIM_LOCK()  \
  if (0==nvim_mutex_initialised)       \
    nvim_mutex_init();         \
  qurt_pimutex_lock(&nvim_qemu_mutex); \
  
#define NVIM_FREE()  \
  qurt_pimutex_unlock(&nvim_qemu_mutex); \
...
qurt_pimutex_init(&nvim_qemu_mutex);
#else /* FEATURE_QDSP6 */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
        ...
        {
          NV_MSG_HIGH_3("cmd_ptr=0x%x, cmd_ptr->done_q_ptr=0x%x, task_ptr=0x%x",cmd_ptr,cmd_ptr->done_q_ptr,task_ptr);
