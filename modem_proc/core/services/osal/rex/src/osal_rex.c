...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/services/osal/rex/src/osal_rex.c#2 $ $DateTime: 2019/01/24 02:45:47 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   --------------------------------------------------------- 
2018-06-25    kdey Fixed reordering of assembly instructions in diag_time_init
...
/*===========================================================================
  FUNCTION    OSAL_DOG_GET_REPORT_PERIOD

  DESCRIPTION
      Function to get the dog timeout value
       
  PARAMETERS
      osal_dog_report_type - Dog handle that the WD uses to identify the task
       
  DEPENDENCIES
      None

  RETURN VALUE
      Period in milliseconds during which task can report to the Dog

  SIDE EFFECTS
      None
  ===========================================================================*/
uint32 osal_dog_get_report_period(osal_dog_report_type handle)
{
	return(dog_get_report_period(handle));
}
...
/*===========================================================================
FUNCTION    OSAL_ATOMIC_BARRIER */

/*!
  @brief
    Acts as a memory barrier.
    All memory accesses before this should complete 
    before any memory accesses after this are performed.
...
*/
/*===========================================================================*/
void osal_atomic_barrier(void)
{
   return atomic_barrier();
