...
$Header: //components/rel/core.mpss/3.10/services/time/src/time_fn_map.h#2 $
...
typedef struct 
{
  ...
  #ifdef FEATURE_PMIC_RTC
  /* To send read command to PMIC   */
  pm_err_flag_type (*time_fn_map_rtc_get_cmd_fp)(uint8,  pm_rtc_time_type *);
  /* To send write command to PMIC   */
  pm_err_flag_type (*time_fn_map_rtc_set_cmd_fp)(uint8,  pm_rtc_time_type *);
  #endif /*FEATURE_PMIC_RTC*/
...
@param[in]                                                               
  time_ptr * - pointer to time for getting time          \n
