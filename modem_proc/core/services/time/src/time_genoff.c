...
static void timer_inaccuracy_write_to_efs(time_genoff_ptr);
...
)
{
  ...
  if ( ptime_genoff->bases_type == ATS_PSM )
  {
    ptime_genoff->generic_offset = offset;
    
    timer_inaccuracy_write_to_efs(ptime_genoff);
...
FUNCTION timer_inaccuracy_write_to_efs
...
=============================================================================*/
static void timer_inaccuracy_write_to_efs(time_genoff_ptr ptime_genoff)
{
  int64 delta_ms = 0;
  ...
  /* Write offset to efs */     
  if ((delta_ms > ptime_genoff->per_storage_spec.threshold) || 
      (delta_ms < ((-1)*ptime_genoff->per_storage_spec.threshold)))
  {
    if ( TIME_EFS_RW_STAT_OK != time_efs_rw( ptime_genoff->per_storage_spec.f_name,
                         &( ptime_genoff->generic_offset ), TIME_EFS_RW_CMD_WR ))
    {
      ATS_ERR_3("Error writing into EFS file bases_type=0x%x  generic_offset=0x%.8x%.8x",
                     ptime_genoff->bases_type,
                     (uint32)(ptime_genoff->generic_offset>>32),
                     (uint32)(ptime_genoff->generic_offset & 0xFFFFFFFF));
    }
...
static void timer_inaccuracy_set( time_genoff_args_ptr pargs )
{
  ...
  if ( !ptime_genoff->state.time_genoff_state_initialized )
  {
    ATS_ERR( "ATS_PSM not initialized" );
    return;
  }
