...
Copyright (c) 2003 - 2014, 2019-2020 by QUALCOMM Technologies Incorporated.  All Rights Reserved.
...
$Header: //components/rel/core.mpss/3.10/services/time/src/timer.c#4 $
...
#ifndef MAX_TIMER_BUFFER_NUMBER
#define MAX_TIMER_BUFFER_NUMBER   256
#endif
#ifndef MIN_TIMER_BUFFER_NUMBER
...
void timer_reset_check_pc_mask_if_set(unsigned int index, unsigned int decrease_count)
{
  ...
  if( timer_after_pc_mask[index] > 0)
  {
        ...
        (timer_after_pc_mask[3] <= 0) )
    {
      ...
      {
        MSG(MSG_SSID_ATS, MSG_LEGACY_LOW, "NPA Client is null for Timer to bring down the cpu");
      }
...
void timer_undefer_match_interrupt( void )
{
  ...
  {
    MSG(MSG_SSID_ATS, MSG_LEGACY_LOW, "NPA Client is null for Timer to bring up the cpu");
      }
    }
  }
  TIMER_PC_MASK_MUTEX_UNLOCK();
