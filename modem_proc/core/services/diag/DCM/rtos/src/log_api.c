...
Copyright (c) 2010 - 2016 , 2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/3.10/services/diag/DCM/rtos/src/log_api.c#5 $
...
02/24/16   as      Fixed log_shorten call for STM use case
02/17/16   tbg     Fixed log_free call for STM use case
...
===========================================================================*/
void log_set_timestamp_ext (void * log_hdr_ptr,unsigned int version, void* parameter)
{
  qword time;
  ...
  uint32 *id = NULL;
  diag_time_get(time);
  if((version==DIAGBUF_VERSION_1) && (diag_wcdma_wl1_frame_cb_ext != NULL))
  {
      id = (uint32 *)parameter;
...
//lint -save -e545 -e740 Suspicious use of & Unusual pointer cast
        ...
        log_set_timestamp_ext(hdr_ptr , version, id);
        hdr_ptr->len = (uint16)length;
        hdr_ptr->code = code;
...
{
  log_set_timestamp_ext(log_hdr_ptr,0,NULL);
} /* log_set_timestamp */
