...
Copyright (c) 2000-2013, 2015, 2017, 2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/3.10/services/diag/DCM/rtos/src/diagdiag.c#4 $
...
diagdiag_f3_listener (uint32 seq_num, const diag_msg_type * msg, void * param)
{
  ...
  if (msg)
  {
    ...
    else if (msg->cmd_code == DIAG_QSR4_EXT_MSG_TERSE_F)
    {
      msg_store_qsr4 = msg->msg_store.qsr_v4_msg;

      uint16 msg_ssid;
      uint32 msg_mask;

      qsr_hash_v4 = (uint32)msg_store_qsr4->qsr_hash;
      ...
      {
        MSG_2(MSG_SSID_LEGACY, MSG_LEGACY_HIGH,
             "F3 Listener unidentified QSR message cmd_code:%d hash_value:%d",
             msg->cmd_code, qsr_hash_v4);
      }
    }
