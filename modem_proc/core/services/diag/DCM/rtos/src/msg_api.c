...
Copyright (c) 2010-2019, 2022 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/3.10/services/diag/DCM/rtos/src/msg_api.c#7 $
...
11/30/17   kdey    Fixed LLVM errors
10/26/17   ph      Update the qshrink4 logic for unaligned addresses.
02/08/16   nk      Added support for WCDMA frame number registration for extended packets
02/23/16   ph      F3 trace dependency to diag services initialization is removed
12/13/15   ps      Fixed extended messages for F3 trace without diag message mask
10/29/15   nk      Changes to support for Qshrink 4.0 in MSG_ERR_LOG_EXT macro
10/15/15   gn      Fixed bug in sending F3s of SSID falling under unaligned mapping
08/10/15   ph      Resolve compiler warnings.
08/04/15   ph      Time Sync between processors within the chip-Diag changes.
07/24/15   ph      Added mutex protection for diag_f3_searchlist_ext().
07/20/15   ph      KW error fixes.
07/13/15   ph      F3 trace timestamps are saved properly now after qshrink4.
07/08/15   ph      Arguments are properly saved into trace buffer now.
...
03/15/15   xy      Added support for QShrink 4.0
...
extern diag_wcdma_wl1_frame_cb_ext_type diag_wcdma_wl1_frame_cb_ext;
...
extern char* const qsr_4_0_msg_aligned_start_address;
extern char* const qsr_4_0_msg_unaligned_start_address;
extern char* const qsr_4_0_msg_end_address;
...
{
  uint8 time_type = 0;
  time_type = msg_get_time_ext(ts,0,NULL);

  return time_type;
}

uint8
msg_get_time_ext(qword * ts,unsigned int version, void* parameter)
{
  uint8 time_type = 0;
  uint32 *id = NULL;
  /* Get timestamp for this message ASAP */
  if(diag_time_initialized && diag_time_get)
  {
  ...
  switch (version)
  {
    ...
    */
    id = (uint32*)parameter;
    if (diag_wcdma_wl1_frame_cb_ext != NULL)
    {
      frame_number = diag_wcdma_wl1_frame_cb_ext(*id) & MSG_FN_MASK;
    }
...
static byte msg_v4_stream_check(uint16 msg_v4_index)
{
  ...
  if ((!stream_id) || (diag_stm_enabled))
  {
    status = diag_f3_searchlist_ext(msg_v4_ss_table[msg_v4_index].ss_id, msg_v4_ss_table[msg_v4_index].ss_mask, NULL);
    if (status)
    {
      /* Use LISTENER stream bit for listeners */
      stream_id |= DIAG_STREAM_LISTENER;
    ...
    {
      status = diag_searchlist_search_all(msg_v4_ss_table[msg_v4_index].ss_id, &diag_f3_listeners, NULL, NULL);
      if (status)
      {
        /* Use LISTENER stream bit for listeners */
        stream_id |= DIAG_STREAM_LISTENER;
      ...
      if (msg)
      {
        if(ts_valid) /* client timestamp is valid, copy that into the header */
          ...
          else
          {
              ...
              msg->hdr.ts_type = msg_get_time_ext(&(msg->hdr.ts),version,&id);
              /*lint -restore */
          }
//if (diag_wcdma_wl1_frame_cb != NULL)
...
msg_send_ts_ext (const msg_const_type * const_blk, uint32 version, void *parameter, uint64 timestamp)
{
  ...
  if((version > DIAGBUF_VERSION_0) && (version <= DIAGBUF_MAX_VERSION_NUM))
  {
    if(parameter)
    {
      msg_save_trace((void *)const_blk, NULL, 0, (void *)msg, DIAG_F3_TRACE_STANDARD, (uint8)version, parameter, 0);
    }
  }
  else
  {
...
===========================================================================*/
static void diag_stm_msg_hdr_prep(msg_hdr_type *msg_hdr, unsigned int num_args , uint32 version, uint32 id)
{
  /* This is common to all F3s being sent over STM */
  msg_hdr->num_args = num_args;
  msg_hdr->ts_type = msg_get_time_ext(&(msg_hdr->ts), version, &id);
 // if (diag_wcdma_wl1_frame_cb != NULL)
...
void *diag_stm_qsr_msg_prep(const msg_qsr_const_type * const_blk, unsigned int num_args, uint32 isHash, uint32 *len, uint32 version, uint32 id)
{
  ...
  if(version == DIAGBUF_VERSION_1)
  {
    /* Compute extra space for multi-client header for packet if required */
    ext_hdr_padding = FPOS(msg_v3_multi_client_store_type, msg);
  }
  else
  {
    ext_hdr_padding = 0;
  ...
  if (msg)
  {
    ...
    *len = msg_size;
    ...
    diag_stm_msg_hdr_prep(msg_hdr, num_args, version , id);

    /* Copy descriptor fields */
    copy_ptr = (char*)msg + sizeof(msg_hdr_type) + ext_hdr_padding;
                                 ...
                                 byte stream_id, uint8 arg_bytes ,uint32 *len, uint32 version, uint32 id)
{
  ...
  if (version == DIAGBUF_VERSION_1)
  {
    ...
    if (msg_ptr)
    {
      ...
      *len = rsp_len;
      /* Set msg_header fields */
      diag_stm_msg_hdr_prep((msg_hdr_type *)msg, (arg_bytes<<4)|num_args,version,id);
  ...
  if(msg_enabled_mask != 0 && msg_service_init == TRUE)
  {
    ...
    /*  Allocates the buffer required, fills in the header  */
      ...
      else if MSG_STM(stream_id,stm_enabled)
      {
        ...
        {
            msg_hdr_type *msg_hdr = (msg_hdr_type *)msg;
             diag_stm_msg_hdr_prep(msg_hdr, int_cnt,0,0);
        }
  ...
  else if (version == DIAGBUF_VERSION_1)
  {
    ...
    if MSG_DIAGBUF(stream_id,stm_enabled)
    {
      ...
      if (msg_ext)
      {
        ...
        /* Update msg pointer to correct location */
        msg_hdr = (void*)&(msg_ext->msg.hdr);

        diag_stm_msg_hdr_prep(msg_hdr, int_cnt, version, *id);
      }
...
/*===========================================================================
FUNCTION qsr_dll_msg_send_prep

DESCRIPTION
   Internal function.
   Prepares the buffer that is sent to diag driver by the msg_send_dll* functions.
   The contents of const_blk are expanded in the client's context if the msg is
   not qshrunk.
 ...
 *const_blk          Static constant constructed using the different
                     xx_msg_const macros
  num_args           Number of arguments being passed in
  isHash			 True : Message is Qshrunk
					 False : Message is not Qshrunk

DEPENDENCIES
  msg_init() must have been called previously.  A free buffer must
  be available or the message will be ignored (never buffered).
===========================================================================*/
static byte* qsr_dll_msg_send_prep(const msg_qsr_const_type *const_blk,
                               unsigned int num_args, uint32 isHash)
{
  ...
  msg_ext_type *msg_temp = NULL;
  uint32 fmt_pos = 0;
  byte stream_id = 0;

  /* sr: We are checking mask from the msg mask array and not Qshrink table here.
     It is still unsure how are we getting the qshrink table.
     Check the runtime mask */
 // valid_ssid = msg_get_ssid_rt_mask(const_blk->desc.ss_id, &rt_mask);
  ...
  if(stream_id)
  {
    ...
    /* Get memory from diag buffer */

    msg = (void *) diagbuf_alloc (DIAG_EXT_MSG_F, alloc_len, 0, DIAGBUF_ALLOC_REQUEST_MSG, 0, 0);
    ...
    if (msg)
    {
      if (QSR_BITFLAG_IS_HASH & isHash)
      {
        /*Design not finalized for Qshrink*/
      } 
      else
      {
        msg_temp = (msg_ext_type *)msg;
        msg_get_time(&(msg_temp->hdr.ts));

        if (diag_wcdma_wl1_frame_cb != NULL)
          msg_temp->hdr.ts_type = MSG_TS_TYPE_GW;
        else
          msg_temp->hdr.ts_type = MSG_TS_TYPE_CDMA_FULL;

        msg_temp->hdr.cmd_code =  DIAG_EXT_MSG_F; 
        msg_temp->hdr.num_args = (uint8)num_args;
        msg_temp->hdr.drop_cnt = (unsigned char)((msg_drop_delta > 255) ? 255 : msg_drop_delta);
        msg_drop_delta = 0;   /* Reset delta drop count */

        /* expand it now.
         The order is: hdr (already done),desc,args,format string, filename. args are copied in the qsr_msg_send* functions */
        msg_temp->desc.line = const_blk->desc.line;
        msg_temp->desc.ss_id = const_blk->desc.ss_id;
        msg_temp->desc.ss_mask = const_blk->desc.ss_mask;

        /*Copy the description*/
        fmt_pos =  sizeof(msg_hdr_type);
        memcpy((void *)((char *)(msg_temp) + fmt_pos), (void *)&(const_blk->desc), sizeof(msg_desc_type));

        /*The arguments are copied in qsr_msg_send* functions*/
        /*Copy the fmt string*/
        fmt_pos = fmt_pos + sizeof(msg_desc_type) + num_args * sizeof(msg_temp->args[0]);
        memcpy((void *)((char *)(msg_temp) + fmt_pos), (void *)(const_blk->msg_hash + filename_len + 1),  MIN(MSG_MAX_STRLEN, (int)strlen((char *)const_blk->msg_hash + filename_len + 1)));

        /*Copy the null terminator*/
        fmt_pos = fmt_pos + MIN(MSG_MAX_STRLEN, (int)strlen((char *)const_blk->msg_hash + filename_len + 1));
        memcpy((void *)((char *)(msg_temp) + fmt_pos), &null_terminator, 1);
        fmt_pos = fmt_pos + 1;

        /*Copy the file name*/
        memcpy((void *)((char *)(msg_temp) + fmt_pos), (void *)(const_blk->msg_hash), filename_len);
  ...
  return (byte*)msg_temp;

} /* qsr_dll_msg_send_prep */
...
qsr_v3_int_msg_send ( const msg_v3_const_hash * const_blk, uint32 isHash, uint32 version, uint32 id)
{
  ...
  if((version > DIAGBUF_VERSION_0) && (version <= DIAGBUF_MAX_VERSION_NUM))
  {
      msg_save_trace((void *)const_blk, NULL, 0, (void *)msg_ext, DIAG_F3_TRACE_QSHRINK30, (uint8)version, (void *)&id, 0);
  }
  else
  {
     msg_save_trace((void *)const_blk, NULL, 0, (void *)msg, DIAG_F3_TRACE_QSHRINK30, 0, NULL, 0 );
  }
#endif /* FEATURE_SAVE_DEBUG_TRACE */
...
qsr_v3_int_msg_send_1(const msg_v3_const_hash *const_blk, uint32 isHash, uint32 version, uint32 id, uint32 xx_arg1)
{
  ...
  if((version > DIAGBUF_VERSION_0) && (version <= DIAGBUF_MAX_VERSION_NUM))
  {
    msg_save_trace((void *)const_blk, f3_args, 1, (void *)msg_ext, DIAG_F3_TRACE_QSHRINK30, (uint8)version, (void *)&id, 0);
  }
  else
  {
     msg_save_trace((void *)const_blk, f3_args, 1, (void *)msg, DIAG_F3_TRACE_QSHRINK30, 0, NULL, 0 );
  }
#endif /* FEATURE_SAVE_DEBUG_TRACE */
...
qsr_v3_int_msg_send_2(const msg_v3_const_hash *const_blk, uint32 isHash, uint32 version, uint32 id, uint32 xx_arg1, uint32 xx_arg2)
{
  ...
  if((version > DIAGBUF_VERSION_0) && (version <= DIAGBUF_MAX_VERSION_NUM))
  {
    msg_save_trace((void *)const_blk, f3_args, 2, (void *)msg_ext, DIAG_F3_TRACE_QSHRINK30, (uint8)version, (void *)&id, 0);
  }
  else
  {
     msg_save_trace((void *)const_blk, f3_args, 2, (void *)msg, DIAG_F3_TRACE_QSHRINK30, 0, NULL, 0 );
  }
...
qsr_v3_int_msg_send_3(const msg_v3_const_hash *const_blk, uint32 isHash, uint32 version, uint32 id, uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3)
{
  ...
  if((version > DIAGBUF_VERSION_0) && (version <= DIAGBUF_MAX_VERSION_NUM))
  {
    msg_save_trace((void *)const_blk, f3_args, 3, (void *)msg_ext, DIAG_F3_TRACE_QSHRINK30, (uint8)version, (void *)&id, 0);
  }
  else
  {
     msg_save_trace((void *)const_blk, f3_args, 3, (void *)msg, DIAG_F3_TRACE_QSHRINK30, 0, NULL, 0 );
  }
...
FUNCTION MSG_V2_SEND_DLL

DESCRIPTION
  This function is called by DLL clients. If the message is not qshrunk 
  then diag copies all of the contents in const_blk. Qshrink and F3s trace
  are not supported.
...
===========================================================================*/
void msg_v2_send_dll(const msg_v2_const_type *const_blk)
{
  byte *msg = NULL;
  const unsigned int num_args = 0;  /* # of message arguments */
  uint8 qshrink_type = 0;
  uint32 isHash = MSG_CHECK_IS_HASH(const_blk->msg);
  ...
  msg = qsr_dll_msg_send_prep((const msg_qsr_const_type *)const_blk, num_args, isHash);
...
===========================================================================*/
void msg_v2_send_1_dll(const msg_v2_const_type *const_blk, uint32 xx_arg1)
{
  byte *msg = NULL;
  msg_ext_type* msg_temp = NULL;
  const unsigned int num_args = 1;  /* # of message arguments */
...
===========================================================================*/
void msg_v2_send_2_dll(const msg_v2_const_type *const_blk, uint32 xx_arg1, uint32 xx_arg2)
{
  byte *msg = NULL;
  msg_ext_type* msg_temp = NULL;
  const unsigned int num_args = 2;  /* # of message arguments */
...
===========================================================================*/
void msg_v2_send_3_dll(const msg_v2_const_type *const_blk, uint32 xx_arg1, uint32 xx_arg2, uint32 xx_arg3)
{
  byte *msg = NULL;
  msg_ext_type* msg_temp = NULL;
  const unsigned int num_args = 3;  /* # of message arguments */
...
===========================================================================*/
void
msg_v2_send_var_dll (const msg_v2_const_type * const_blk,uint32 num_args, ...)
{
  msg_qsr_store_type *msg = NULL;
  uint32 *args = NULL;
  va_list arg_list;     /* ptr to the variable argument list */
  ...
  if(msg_enabled_mask != 0)
  {
      ...
      {
          /* Need to allocate from diagbuf and commit to buffer */
         msg = (msg_qsr_store_type *)qsr_dll_msg_send_prep ((const msg_qsr_const_type *)qsr_const_blk, num_args,
                                   MSG_CHECK_IS_HASH(const_blk->msg));
...
/*lint -restore */
  ...
  msg_save_trace((const void *)hash_const_blk, f3_args, MIN(num_args,MAX_VAR_ARGS), (void *)msg, DIAG_F3_TRACE_QSHRINK30, 0, NULL, 0);
#endif
...
msg_v3_send_var_ext(const msg_v3_const_type *const_blk, uint32 version, void *parameter, uint32 num_args, ...)
{
  ...
  switch (version)
  {
   ...
   //XXX TODO: don't cast like this --rh
    msg_save_trace((const void *)hash_const_blk, f3_args, args_count, (void *)msg_ext, DIAG_F3_TRACE_QSHRINK30, (uint8)version, parameter, 0);
#endif
...
void diag_f3_listener_notify (uint32 seq_num, void * msg, diag_searchlist_node_type * node)
{
  ...
  msg_v4_store_type *msg_store_qsr4 = NULL;
  ...
  if (node && msg)
  {
    ...
    else if(msg_notify->cmd_code == DIAG_QSR4_EXT_MSG_TERSE_F)
    {
      msg_store_qsr4 =  (msg_v4_store_type*)msg_notify->msg_store.qsr_v4_msg;
      if(msg_store_qsr4)
      {
        qsr_hash_v4 = msg_store_qsr4->qsr_hash;
        if(FALSE == msg_get_qsr4_table_entry(qsr_hash_v4, &ss_id, &ss_mask))
        {
          return;
        }
      }
