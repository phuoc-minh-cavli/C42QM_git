...
Copyright (c) 1990-2015, 2019 by Qualcomm Technologies, Incorporated.
All Rights Reserved.
...
$Header: //components/rel/core.mpss/3.10/services/diag/DCM/rtos/src/diagpkt.c#8 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/14/19   kdey    Porting of Qsockets
01/07/18   kdey    Featurizing macros under #ifndef to allow their definition outside diag
11/09/18   gn      DLL support for command registrations
...
unsigned int  cur_delayed_rsp_memory_used=0;
...
void* diagpkt_copy_master_table(diagpkt_master_table_type * tbl_ptr);
...
DESCRIPTION 
  ...
  master proc. Also frees the master table entry while deregistering, as
  the space for master tables are allocated from amss heap.
  
PARAMETERS
  tbl_ptr - the table that has to be deregistered
  
RETURN VALUE
  None
  
============================================================================= */
void diagpkt_fwd_cmd_dereg()
{
  int i = 0;
  int j = 0;
  int k = 0;
  ...
  for( i = 0; i < DIAGPKT_MSTR_TBL_SIZE; i++ )
  {
    if( diagpkt_table_fwd_status[i] ==  DIAG_CTRL_INFO_FWD_STATUS_DEREGISTERED )
    {
      ...
      len_written = diagcomm_ctrl_send_mult_pkt( &ctrl_pkt, DIAGCOMM_CTRL_MSG_HDR_SIZE + FPOS(diag_ctrl_msg_dereg_type, user_tbl), 
                                                (void *)(ctrl_pkt.data.ctrl_msg_dereg.user_tbl), userpkt_len, DIAGCOMM_PORT_SIO, DIAGCOMM_PORT_1, TRUE, &is_sent );					 
												 
      free (ctrl_pkt.data.ctrl_msg_dereg.user_tbl);

      if(diagpkt_master_table[i])
      {
        /* Free the master table as it is allocated from Heap */
        free((void*)diagpkt_master_table[i]);
      }
                          ...
                          diagcomm_enum_port_type ctrl_channel )
{
  ...
  while( (msg_pkt_ptr != NULL) && (data_pkt_len > 0) )
  {
    ...
    ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1, "Control packet received of type %d",msg_pkt_ptr->cmd_type);
    ...
    case DIAG_CTRL_MSG_DIAGMODE:
    {
      ...
      case DIAG_CTRL_MSG_FEATURE:
      {
        ...
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 1 , "diagpkt_process_ctrl_msg: Received APPS Feature mask = 0x%X", msg_pkt_ptr->data.ctrl_msg_feature.mask[0]);
        ...
        if (DIAG_IS_INT_FEATURE_BIT_SET(diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].feature_mask, F_DIAG_SOCKET_SUPPORT))
        {
  ...
  unsigned int length)
{
  ...
  if(item)
  {
      ...
      cur_delayed_rsp_memory_used += length;
      ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "1: cur_delayed_rsp_memory_used is %d Length is %d ", cur_delayed_rsp_memory_used, length );
    ...
    if (!item)
    {
      ...
      if (item)
      {
        ...
        else
        {
          ...
          {
            /* filtering out logging of response of ULog command send by qxdm to retrieve the ULog megssages */
            if(*(item->rsp.pkt + 1) != DIAG_SUBSYS_ULOG) {
              if((*(item->rsp.pkt + 0) == DIAG_BAD_CMD_F) || (*(item->rsp.pkt + 0) == DIAG_BAD_PARM_F) || (*(item->rsp.pkt + 0) == DIAG_BAD_LEN_F))
                ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 5, "diagpkt_rsp_send: Invalid command response - Sending response packet: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",\
                 item->rsp.pkt[0], item->rsp.pkt[1], item->rsp.pkt[2], item->rsp.pkt[3], item->rsp.pkt[4]);
             else
              ULogFront_RealTimePrintf(diag_cmd_ulog_handle, 4, "diagpkt_rsp_send: Sending response packet: 0x%02X 0x%02X 0x%02X 0x%02X",\
              item->rsp.pkt[0], item->rsp.pkt[1], item->rsp.pkt[2], item->rsp.pkt[3]);
          }
...
diagpkt_delay_rsp_send (void)
{
  ...
  while (!finished && status == DIAGBUF_READY_S)
  {
    if (item)
    {
      ...
      {
#ifndef DIAG_MP
        /*Check if there is data available in the DSM pool only we are on UART -- 
          Since we use DSM pools only in this configuration*/
        if( DSM_POOL_FREE_CNT(DSM_DIAG_SIO_TX_ITEM_POOL) == 0)
        {
          free_count = FALSE;
        }
      ...
      if (send.state == DIAG_SEND_STATE_COMPLETE)
      {
        ...
        osal_lock_mutex(&diag_delayed_response_mutex);
        cur_delayed_rsp_memory_used -=length;   
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "2: cur_delayed_rsp_memory_used is %d Length is %d ", cur_delayed_rsp_memory_used, length );
        osal_unlock_mutex(&diag_delayed_response_mutex);
...
/* ==========================================================================
FUNCTION DIAGPKT_COPY_MASTER_TABLE

DESCRIPTION 
  Copies the entire master table along with all the user table entries to a 
  newly allocated memory from AMSS Heap and returns the pointer to the newly
  allocated memory.

PARAMETERS
  tbl_ptr - Pointer to the master table entry

RETURN VALUE
  master_table_ptr - Pointer to the newly allocated memory from heap
  
============================================================================= */

void* diagpkt_copy_master_table(diagpkt_master_table_type * tbl_ptr)
{
  diagpkt_user_table_entry_type* user_table_ptr = NULL;
  diagpkt_master_table_type* master_table_ptr = NULL;
  
  uint16 user_table_size = 0;
  uint16 total_master_table_size = 0;

  if(!tbl_ptr || !(tbl_ptr->user_table))
    return NULL;

  if(!(tbl_ptr->user_table->func_ptr))
    ERR_FATAL("Table registered with function ptr as NULL", tbl_ptr->user_table, 0, 0);

  /* Size of user table to copy */
  user_table_size = (tbl_ptr->count) * sizeof(diagpkt_user_table_entry_type);
  
  //Total size of master table including the user table entries
  total_master_table_size = sizeof(diagpkt_master_table_type) + user_table_size;
  
  //Allocating memory for the entire size of master table calculate above
  master_table_ptr = (diagpkt_master_table_type*)malloc(total_master_table_size);
  
  //Sanity check for malloc failure on heap exhaustion
  if(!master_table_ptr)
    ERR_FATAL("Malloc failed in command registration for master table 0x%x of %d bytes", tbl_ptr, total_master_table_size, 0);

  /* Copy only the master table entries to the newly allocated memory. */
  memscpy(master_table_ptr, sizeof(diagpkt_master_table_type), tbl_ptr, sizeof(diagpkt_master_table_type));

  //Updating the local user table pointer to point to the user table entry in the allocated master table
  user_table_ptr = (diagpkt_user_table_entry_type*)((uint8*)master_table_ptr + sizeof(diagpkt_master_table_type));
    
  //Copying the user table entries
  memscpy(user_table_ptr,total_master_table_size - sizeof(diagpkt_master_table_type),tbl_ptr->user_table, user_table_size);
  
  //Making the user_table pointer point to the user table
  master_table_ptr->user_table = (diagpkt_user_table_entry_type*)user_table_ptr;

  return (void*)master_table_ptr;
}
...
============================================================================= */
void
diagpkt_tbl_reg (diagpkt_master_table_type * tbl_ptr, const diagpkt_user_table_entry_type * user_tbl_ptr)
{
  diagpkt_master_table_type* master_tbl_ptr = NULL;
  ...
  /* Update the user table ptr of master table entry (tbl_ptr) */
  tbl_ptr->user_table = user_tbl_ptr;
 ...
 /* Make sure this is initialized */
  ...
  /* Copy the user specified table to a newly allocated memory from AMSS Heap and
  update the user table pointer of master table entry */
  master_tbl_ptr = (diagpkt_master_table_type*)diagpkt_copy_master_table(tbl_ptr);
  
  if(!master_tbl_ptr)
    ERR_FATAL("NULL pointer is returned while allocating memory for master table entry at location 0x%x",tbl_ptr,0,0);
  ...
  while(diagpkt_master_tbl_index < DIAGPKT_MSTR_TBL_SIZE)
  {
    if( diagpkt_master_table[diagpkt_master_tbl_index] == NULL )
    {
      ...
      diagpkt_master_table[diagpkt_master_tbl_index] = master_tbl_ptr;
...
void diagpkt_tbl_dereg(diagpkt_master_table_type *tbl_ptr)
{
  ...
  for(i=0; i<DIAGPKT_MSTR_TBL_SIZE;i++)
  {
    ...
    {
	/*Mark the status as deregsitered if its not already marked as deregsitered -
...
#define DIAG_CM_POWER_OFF 6
  ...
  for (i = 0; !found && !skip_master_table && i < DIAGPKT_MSTR_TBL_SIZE; i++)
  {
	...
	/* Check if the command has been deregistered but the entry is not deleted in 
	master table as the ctrl packet has not been sent*/
    if( diagpkt_table_fwd_status[i] ==  DIAG_CTRL_INFO_FWD_STATUS_DEREGISTERED )
	{
	  break;
	}
