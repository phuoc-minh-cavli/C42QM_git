...
Copyright (c) 1992-2016, 2019, 2022 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/*==========================================================================

                           Edit History
  $Header: //components/rel/core.mpss/3.10/services/diag/DCM/common/src/msg.c#8 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/02/16   ph      Resolve KW errors in diag codebase.
12/23/15   ph      Extended messages are HDLC encoded properly now on standlone modem.
10/15/15   gn      Fixed bug in sending sprintf Extended messages
10/15/15   gn      Fixed bug in sending F3s of SSID falling under unaligned mapping
09/16/15   ph      Corrected binary search logic in msg_update_v3_ss_table().
09/16/15   ph      Corrected binary search logic in msg_update_v4_ss_table().
08/19/15   gn      Updated desc->pkt only if its desc->state is complete for Extended F3s.
06/19/15   sa      Re-enable QShrink4.0
06/11/15   sa      Fixed Compiler Warnings.
05/27/15   sa      Backed out Qshrink4.0
01/19/15   xy      Added support for QShrink 4.0
...
char* const qsr_4_0_msg_aligned_start_address = &__QSR_4_0_MSG_ALIGNED_START__;
char* const qsr_4_0_msg_unaligned_start_address = &__QSR_4_0_MSG_UNALIGNED_START__;
char* const qsr_4_0_msg_end_address = &__QSR_4_0_MSG_END__;
...
msg_init (void)
{
...
#if defined(DIAG_MP_MASTER)
  uint8 stream_index, port_num, temp = 0;
#endif
  ...
  for( port_num=0; port_num < NUM_SMD_PORTS; port_num++ )
  {
    for(temp=DIAG_MIN_PRESET_ID;temp<=DIAG_MAX_PRESET_ID;temp++)
    {
      for( index = 0; index < MSG_MASK_TBL_CNT; index++ )
      {
         diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SMD][port_num].msg_mask_preset_update[temp - 1][index] = FALSE;
      }
    }
  }
...
FUNCTION MSG_UPDATE_V3_SS_Table
...
SIDE EFFECTS
  None.
===========================================================================*/
static void msg_update_v3_ss_table (uint16 curr_ssid, uint32 ss_mask, uint8 stream_id, uint8 preset_id)
{
  int mid_index; /*need to be signed for binary search*/
  int start_index = 0;   /*need to be signed for binary search*/
  ...
  if (stream_id > DIAG_MAX_STREAM_ID || preset_id > diag_max_presets[stream_id-1])
  {
    return;
  }
...
typedef enum
{
  QSR4_MSG_EXPAND_STATE_HDR = 0,
  QSR4_MSG_EXPAND_STATE_HASHMSG_VALUE,
  QSR4_MSG_EXPAND_STATE_HASHDBASE_VALUE,
  QSR4_MSG_EXPAND_STATE_ARGS,
  QSR4_MSG_EXPAND_STATE_COMPLETE
}
qsr4_msg_expand_enum_type;
...
#if !defined(FEATURE_WINCE)
        msg_notify.cmd_code = DIAG_QSR4_EXT_MSG_TERSE_F;
        msg_notify.msg_store.qsr_v4_msg = msg_store;
#ifdef DIAG_LISTENERS_SUPPORT
...
void diag_ctrl_update_msg_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 5, "diag_ctrl_update_msg_mask: update msg mask - msg_mask_size = %d,\
                            ssid_first = %d, ssid_last = %d, stream_id = %d, status = %d", msg_mask_size, ssid_first, ssid_last, stream_id, status);
...
void diag_ctrl_update_msg_preset_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 5, "diag_ctrl_update_msg_preset_mask: update msg mask - msg_mask_size = %d, \
                           ssid_first = %d, ssid_last = %d, stream_id = %d, status = %d", msg_mask_size, ssid_first, ssid_last, stream_id, status);
...
void diag_ctrl_send_msg_ssid_range_ctrl_pkt(void)
{
     ...
     if(bt_msg_ssid_range_fwd_status == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT)
     {
          ...
          ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2 , "diag_ctrl_send_msg_ssid_range_ctrl_pkt: Sending BT Mask with - data_len = %d and num_ssid_ranges = %d",\
            ctrl_msg.data_len, ctrl_msg.data.ctrl_bt_msg_ssid_range.num_ssid_ranges);
...
void diag_ctrl_send_msg_bt_mask_ctrl_pkt(void)
{
     ...
     for (i = 0; i < MSG_MASK_TBL_CNT; i++)
     {
          ...
          if((tbl_ptr != NULL) && (bt_msg_mask_fwd_status[i] == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT))
          {
                ...
                ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3 , "diag_ctrl_send_msg_bt_mask_ctrl_pkt: Sending BT Mask with - data_len = %d from ssid_first = %d to ssid_last = %d",\
                  ctrl_msg.data_len, ctrl_msg.data.ctrl_bt_msg_mask.ssid_range.ssid_first, ctrl_msg.data.ctrl_bt_msg_mask.ssid_range.ssid_last);
