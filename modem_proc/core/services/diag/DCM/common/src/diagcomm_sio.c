...
Copyright (c) 2001-2015, 2019 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/3.10/services/diag/DCM/common/src/diagcomm_sio.c#5 $ $DateTime: 2019/09/30 01:26:23 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/14/19   kdey    Porting of Qsockets
12/09/15   gn      Fixed bug in sending Log pakets in Standalone configuration
...
/* The below callbacks are needed only in standalone scenario */
#ifndef DIAG_MP
...
#else
  static boolean bDiagAllowTxWmqFlow[NUM_SMD_PORTS] = {TRUE};
#endif /* DIAG_CONSUMER_API */

/* Compiling out the code rather than removing it in case we need to reference anytime in future*/
/* Done to handle the warnings related to functions not used */
#if 0
...
===========================================================================*/
/* Featurized this because there were no other references left which was getting compiled after featurizing DCI */
#ifdef DIAG_CONSUMER_API
static void
...
diagcomm_sio_open (diagcomm_enum_port_type port_num)
{
  ...
  #else
    ASSERT(DIAGCOMM_PORT_1 == port_num);
  #endif /* #ifdef DIAG_CONSUMER_API */
...
diagcomm_inbound (unsigned int *stream_len, diagcomm_enum_port_type port_num)
{
  ...
  *stream_len = diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][port_num].used;
   return (void *)&(diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][port_num].buf);
...
void diagcomm_sio_tx_dsm_level_reg( uint8 port_num, boolean reg_level )
{
  ...
  {
    dsm_unreg_mem_event_cb( DSM_DIAG_SIO_TX_ITEM_POOL, DSM_MEM_LEVEL_MANY, DSM_MEM_OP_NEW );
    dsm_unreg_mem_event_cb( DSM_DIAG_SIO_TX_ITEM_POOL, DSM_MEM_LEVEL_FEW, DSM_MEM_OP_FREE );
  }
#endif /* DIAG_MP */
...
void diagcomm_sio_open_init( diagcomm_io_conn_type * conn )
{
  ...
  #else
    ASSERT(DIAGCOMM_PORT_1 == conn->port_num); 
  #endif /* #ifdef DIAG_CONSUMER_API */
...
dsm_item_type * diagcomm_cpy_to_dsm(uint8 * buffer, uint16 len, boolean is_cmd_rsp_type)
{
   int bytes_to_cpy = 0;
   uint8 * ptr = NULL;
   uint16 dest_size = 0;
   ...
   dsm_item_type *item_ptr = diagcomm_outbound_alloc(FALSE, is_cmd_rsp_type);

   if (is_cmd_rsp_type) 
   {
      dest_size = DSMI_DIAG_SIO_RX_ITEM_SIZ;
   }
   else
   {
      dest_size = DSMI_DIAG_SIO_TX_ITEM_SIZ;
   }

   if (item_ptr) 
   {
      memscpy(item_ptr->data_ptr, dest_size, buffer, MIN(dest_size,len));
      item_ptr->used = MIN(dest_size,len);
   }
   else
   {
      return NULL;
   ...
   if( len > dest_size )
   {
     bytes_to_cpy = len - dest_size;
     ptr = (uint8 *)buffer;

     while (bytes_to_cpy > 0) 
     {
        ptr += dest_size;
        item_ptr = diagcomm_outbound_alloc(TRUE, is_cmd_rsp_type);

        if (item_ptr) 
        {
           memscpy(item_ptr->data_ptr, dest_size, ptr, MIN(dest_size,bytes_to_cpy));
           item_ptr->used = MIN(dest_size, bytes_to_cpy);
           bytes_to_cpy -= MIN(dest_size, bytes_to_cpy);
        }
        else
        {
           MSG(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "Ran out of SIO Item Pool DSM items");
        }
     }
   }

   return diagcomm_sio_outbound_item_ptr;
}
/*===========================================================================
...
DESCRIPTION
  This function requests a buffer from the diag communications layer and
  is called only in modem standalone configuration.
...
===========================================================================*/
dsm_item_type *
diagcomm_outbound_alloc (boolean is_chain, boolean is_cmd_rsp_type)
{
   dsm_item_type *item_ptr = NULL;

   if (is_cmd_rsp_type) 
   {
     item_ptr = dsm_new_buffer(DSM_DIAG_CMD_SIO_RX_ITEM_POOL);
   }
   else
   {
...
===========================================================================*/
void
diagcomm_send (void * ptr, unsigned int length, boolean is_cmd_rsp_type, diagcomm_enum_port_type port_num)
{
   ...
   diagcomm_io_conn_type *ctrl_conn = &diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1];
   ...
   /*This check is done only for sockets/smdl*/
   if (is_cmd_rsp_type) 
   {
      if ((diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][port_num].diag_prev_tx_pending == 1) &&
          (diagcomm_io_cmd_conn[DIAGCOMM_PORT_SIO][port_num].diag_resend_data != 1)) 
      {
         ASSERT(0);
      }
   } 
   ...
   {
      if ((diagcomm_io_conn[DIAGCOMM_PORT_SIO][port_num].diag_prev_tx_pending == 1) &&
          (diagcomm_io_conn[DIAGCOMM_PORT_SIO][port_num].diag_resend_data != 1)) 
      {
         ASSERT(0);
      }
   }
...
#ifndef DIAG_MP
      tx_params.dsm_ptr = (dsm_item_type *) diagcomm_cpy_to_dsm(ptr, length, is_cmd_rsp_type);
#else
   /* Just send immediately if there's no pkt to write */
   tx_params.params.buffer = ptr;
   tx_params.params.len = length;
   tx_params.params.flags = 0;
#endif

   if (is_cmd_rsp_type)
   {
      g_is_cmd_rsp_type = is_cmd_rsp_type;
   ...
   {
     if(diag_tx_mode[port_num].mode == DIAG_TX_MODE_STREAMING)
     {
         diagcomm_io_transmit( conn, &tx_params );
       }
       else
...
===========================================================================*/
                                                ...
                                                dsm_mem_op_enum_type mem_op)
  {
     ...
     {
        diagcomm_io_conn[DIAGCOMM_PORT_SIO][DIAGCOMM_PORT_1].allow_flow = FALSE;
