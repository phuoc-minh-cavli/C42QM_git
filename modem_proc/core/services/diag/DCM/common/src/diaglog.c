...
Copyright (c) 1990-2017, 2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/core.mpss/3.10/services/diag/DCM/common/src/diaglog.c#7 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/29/17   ph      Corrected implementation of diag_masks_enabled API.
02/03/17   rs      Fixed partial extended log packets issue when we avoid flusing buffer before sending extended logs.
...
07/21/15   sa      Updated the extended log header flag only if it is sent.
...
uint8 diag_log_config = DIAG_LOG_STREAM_NONE;
...
void log_init (void)
{
  ...
  for(temp=DIAG_MIN_PRESET_ID;temp<=DIAG_MAX_PRESET_ID;temp++)
  {
    diag_log_mask_mstr_preset[temp - 1].status = DIAG_CTRL_MASK_INVALID;
  }
  ...
  for( port_num = 0; port_num < NUM_SMD_PORTS; port_num++ )
  {
    for(temp=DIAG_MIN_PRESET_ID;temp<=DIAG_MAX_PRESET_ID;temp++)
    {
      ...
      {
         diagcomm_io_ctrl_conn[DIAGCOMM_PORT_SMD][port_num].log_mask_preset_update[temp-1][id] = FALSE;
      }
    }
  }
...
log_service_outbound (diag_send_desc_type *desc,byte stream_id)
{
  ...
  diagbuf_header_type *diagbuf_hdr2 = NULL;
  ...
  if (desc)
  {
    ...
    if (state == DIAG_SEND_STATE_START)
    {
      ...
      else
      {
        ...
        if((diagbuf_hdr1->version == DIAGBUF_VERSION_1 ) && (diagbuf_hdr1->chain == DIAGBUF_FIRST_PKT_CHAIN))
        {
          ...
          /* Move 16 bytes from Diagbuf header of 1st part of EXT log to get to Diagbuf Header of 2nd part of EXT header 
          4 bytes (Diagbuf header) + 8 bytes (Extended Log Header) + 2 bytes (DEAD pattern) + 2 Bytes (Pad length) = 16 bytes */
          diagbuf_hdr2 = (diagbuf_header_type*)((byte*)diagbuf_hdr1 + sizeof(diagbuf_header_type)+ diagbuf_hdr1->length + sizeof(uint16) + diagbuf_hdr1->pad_len);

          diag_pkt_len = diagbuf_hdr1->length + diagbuf_hdr2->length;
          #ifdef DIAG_CONSUMER_API
          ...
          if (send.state == DIAG_SEND_STATE_COMPLETE)
          {
		   ...
		   /* since send state is not DIAG_SEND_STATE_COMPLETE yet, so rolling back the cmd code to 0x10,
		      so that customer handler log_service_outbound() will be called when flow is enabled again 
			  instead of diagbuf_send_pkt() */
        ...
        if((diagbuf_hdr1->version == DIAGBUF_VERSION_2 ) && (diagbuf_hdr1->chain == DIAGBUF_FIRST_PKT_CHAIN))
        {
          ...
          send.terminate = FALSE;
          send.state = DIAG_SEND_STATE_START;
          /* Calculate length of log packet */
          /* Move from Diagbuf header of 1st part of EXT log to get to Diagbuf Header of 2nd part of EXT header 
          4 bytes (Diagbuf header) + <V2 EXT header length> + 2 bytes (DEAD pattern) + 2 Bytes (Pad length) */
          diagbuf_hdr2 = (diagbuf_header_type*)((byte*)diagbuf_hdr1 + sizeof(diagbuf_header_type) + diagbuf_hdr1->length + sizeof(uint16) + diagbuf_hdr1->pad_len);  
          diag_pkt_len = diagbuf_hdr1->length + diagbuf_hdr2->length;
    ...
    else
    {
      ...
      {
        /* Calculate length of log packet */
        diag_pkt_len = diagbuf_hdr1->length;
        (void) diagbuf_send_pkt (desc,stream_id);
      }
      else
      {
        /* Execution shouldn't have reached here */
        ERR_FATAL("diagbuf_hdr1 = NULL, desc->state %d", desc->state, 0, 0);
      }
    }
  }
...
FUNCTION LOG_MASKS_ENABLED_CHECK

DESCRIPTION
  This function checks the runtime masks for all log codes for a particular stream
  to determine if the log service is "off".  It will return TRUE if the
...
RETURN VALUE
  TRUE  - Masks are enabled.
  FALSE - Masks are disabled.

SIDE EFFECTS
  None.

===========================================================================*/
static boolean log_masks_enabled_check(byte stream_id)
{
  boolean masks_enabled = FALSE;
  uint32 id = 0;
  uint16 item = 0;
  uint8 stream_index = DIAG_ID_TO_INDEX(stream_id);

  if ( (stream_id > DIAG_MAX_STREAM_ID) || (stream_id < DIAG_MIN_STREAM_ID) )
  {
    MSG(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "log_masks_enabled_check Sanity check failed");
    return FALSE;
  }

  for (id = 0; !masks_enabled && id <= LOG_EQUIP_ID_MAX; id++)
  {
    for (item = 0; !masks_enabled && item <= log_last_item_tbl[id]; item++)
    {
      if (log_mask_enabled (log_mask[stream_index], id, item))
      {
        masks_enabled = TRUE;
      }
    }
...

  if(masks_enabled)
  {
    if (stream_id == DIAG_STREAM_1)
    {
      DIAG_SET_MASK(diag_log_config, DIAG_LOG_STREAM_1);
    }
#ifdef DIAG_STREAM_2_MASK_CHECK
    else if (stream_id == DIAG_STREAM_2)
    {
      DIAG_SET_MASK(diag_log_config, DIAG_LOG_STREAM_2);
    }
#endif

  }
  else
  {
    if (stream_id == DIAG_STREAM_1)
    {
      DIAG_CLR_MASK(diag_log_config, DIAG_LOG_STREAM_1);
    }
#ifdef DIAG_STREAM_2_MASK_CHECK
    else if (stream_id == DIAG_STREAM_2)
    {
      DIAG_CLR_MASK(diag_log_config, DIAG_LOG_STREAM_2);
    }
#endif
  }
...
static boolean log_sleep_vote (void)
{
  ...
  masks_enabled = log_masks_enabled_check(DIAG_STREAM_1);
...
void diag_ctrl_update_log_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 4 , "diag_ctrl_update_log_mask: update log mask - mask_len = %d,\
                                stream_id = %d, status = %d, equip_id = %d", mask_len, stream_id, status, equip_id);
...
void diag_ctrl_update_log_preset_mask( diag_ctrl_msg_type * ctrl_msg )
{
  ...
  ULogFront_RealTimePrintf(diag_debug_ulog_handle, 6 , "diag_ctrl_update_log_preset_mask: procid=%d stream=%d \
  preset=%d status=%d equip_id=%d len=%d", DIAG_MY_PROC_ID, stream_id, preset_id, status, equip_id, mask_len);
...
void diag_ctrl_send_log_range_ctrl_pkt(void)
{
  ...
  if(bt_log_range_fwd_status == DIAG_CTRL_INFO_FWD_STATUS_NOT_SENT)
  {
        ...
        ULogFront_RealTimePrintf(diag_debug_ulog_handle, 3 , "diag_ctrl_send_log_range_ctrl_pkt: Sending BT Mask with - data_len = %d and log_last_equipid = %d and num_equipid_ranges = %d",\
          ctrl_msg.data_len, ctrl_msg.data.ctrl_msg_bt_log_range.log_last_equipid, ctrl_msg.data.ctrl_msg_bt_log_range.num_equipid_ranges);
