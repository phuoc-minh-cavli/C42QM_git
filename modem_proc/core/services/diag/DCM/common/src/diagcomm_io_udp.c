...
/*===========================================================================
 ...
 $Header: //components/rel/core.mpss/3.10/services/diag/DCM/common/src/diagcomm_io_udp.c#4 $
...
06/10/15    ph     Handled the error scenarios in qpoll() properly to avoid port
                   loss issues when DCI is disabled.
...
#define UDP_POLL_TIMEOUT 100 //milliseconds
...
#ifdef FEATURE_SSM
extern boolean is_dci_allowed;
#endif /* FEATURE_SSM */


uint32 diag_pfds_index = 0;
static struct qpollfd diag_pfds[DIAG_MAX_NUM_SOCKETS];
diagcomm_io_conn_type *diag_sock_conn[DIAG_MAX_NUM_SOCKETS];

/*Timer to poll for the service address*/
...
int diagcomm_io_udp_get_srv_addr (ipcr_name_t name, struct qsockaddr_ipcr *srv_addr, int timeout)
{
  ...
  while(1)
  {
    ...
    if(rc > 0)
    {
      ...
      ULogFront_RealTimePrintf(diag_debug_ulog_handle, 2, "diagcomm_io_udp_get_srv_addr: Got service with node ID - %d, port ID - %d",\
            srv_addr->address.addr.port_addr.node_id, srv_addr->address.addr.port_addr.port_id);
...
int diagcomm_io_udp_open(diagcomm_io_conn_type * conn)
{
  ...
  if(! conn->isServer)
  {
      if(diagcomm_io_udp_get_srv_addr(conn->addr.address.addr.port_name, &srv_addr, wr_timeout))
     {
...
void diagcomm_io_udp_receive_data(void)
{
  ...
  while(TRUE)
  {
    rc = qpoll(&diag_pfds[0], diag_pfds_index, UDP_POLL_TIMEOUT);
      ...
      for(i=0;i<diag_pfds_index;i++)
      {
        if(diag_pfds[i].revents & QPOLLIN)
        {
           ...
           else if(0 == num_recv)
           {
             ...
             ULogFront_RealTimePrintf(diag_data_ulog_handle, 3, "diagcomm_io_udp_receive_data: 0 Byte packet is received -\
                                      Enabling flow control - channel_type = %d, io_type = %d, port_num = %d", conn->channel_type, conn->io_type, conn->port_num);
             diagcomm_io_enable_flow(conn);
           }
        }
        else
        {
            if( diag_pfds[i].revents )
            {
              if( diag_pfds[i].revents & QPOLLNVAL )
              {
                conn = diag_sock_conn[i];
                #ifdef DIAG_CONSUMER_API
                  if(conn->port_num == DIAGCOMM_PORT_2)
                  {
                    #ifdef FEATURE_SSM
                      is_dci_allowed = diag_dci_open_query();
                      if(is_dci_allowed)
                      {
                        MSG_3(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "qpoll returned %d error on port %d, handle %d ", rc, conn->port_num, conn->sock_handle);
                      }
                      else
                      {
                        /*Do Nothing */
              ...
              {
                 MSG_1(MSG_SSID_DIAG, MSG_LEGACY_ERROR,"qpoll returned %d error", rc);
              }
            }
        }
    }
