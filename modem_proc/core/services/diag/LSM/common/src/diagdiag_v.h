...
Copyright (c) 2001-2016, 2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.mpss/3.10/services/diag/LSM/common/src/diagdiag_v.h#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/03/16   gn      Added support for 12k log packet
04/20/16   ph      Diag log packet support for ULog
08/31/15   ph      Added support for buffering mode APIs.
08/12/15   ph      Reduced DIAG_QSHRINK task stack to 4KB.
...
#define DIAG_QSHRINK_4_SLEEP_SIG     0x200000
#define QSRHINK_BUFFER_SIZE 16000
#define STRESS_TEST_STACK_SIZE 512  /* Actual stack size gets to 512*8= 4096*/

#define STRESS_QSHRINK_4_STACK_SIZE (STRESS_TEST_STACK_SIZE)
...
#define DIAGDIAG_QSHRINK_4_RETRIEVE_DATABASE_MODEM               (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0016) /* 0x080F = 2070 */
#define DIAGDIAG_GET_TIME_API_MODEM            (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x0017) /* 0x817 = 2071 */
#define DIAGDIAG_LOCK_BUFFER_API_MODEM         (DIAGDIAG_SUBSYS_ID_MODEM_OFFSET + 0x001B) /* 0x81B = 2075 */
...
typedef PACK(struct) {
  ...
  uint32 file_len;
}diag_qshrink_4_file_info_type;

typedef PACK(struct) {
  uint8 status;
  uint8 num_files;
  diag_qshrink_4_file_info_type file_info[0];
} diag_qshrink_4_file_list_rsp_type;

typedef PACK(struct) {
  uint8 guid[QSHRINK_4_DATABASE_GUID_LENGTH];
} diag_qshrink_4_file_open_req_type;

typedef PACK(struct) {
  uint8 guid[QSHRINK_4_DATABASE_GUID_LENGTH];
  uint16 fd;  /*file descriptor of the database*/
  uint8 status;
} diag_qshrink_4_file_open_rsp_type;

typedef PACK(struct) {
  uint16 fd;              /*file descriptor of the database*/
  uint32 requested_bytes; /*requested number of bytes to read from the file*/
  uint32 offset;          /*offset in bytes from the start of the file*/
} diag_qshrink_4_file_read_req_type;


typedef PACK(struct) {
  uint16 fd;              /*file descriptor of the database*/
  uint32 offset;          /*offset in bytes from the start of the file*/
  uint32 num_read;        /*If successful, indicates the number of bytes read and returned in DATA_BLOCK. Successful value may */
			  /*range from 0 to REQUESTED_BYTES, and may contain less than REQUESTED_BYTES. A value of 0 indicates */
			  /*that the end of file has been reached and that OFFSET is equal to the size of the file.*/
			  /*If unsuccessful, set to -1.*/
  uint8 status;           /*Reports the status of the File Read command.0 : Success Other: Error Code*/
  byte data_block[0];     /*Data block read from the FD at the indicated OFFSET.*/
} diag_qshrink_4_file_read_rsp_type;

typedef PACK(struct) {
  uint16 fd;              /*file descriptor of the database*/
} diag_qshrink_4_file_close_req_type;

typedef PACK(struct) {
  uint16 fd;  /*file descriptor of the database*/
  uint8 status;/*Reports the status of the File Close command.0 : Success Other: Error Code*/
} diag_qshrink_4_file_close_rsp_type;

typedef PACK(union)
{
  diag_qshrink_4_file_open_req_type file_open_req;
  diag_qshrink_4_file_read_req_type file_read_req;
  diag_qshrink_4_file_close_req_type file_close_req;
  diag_qshrink_4_file_list_rsp_type file_list_rsp;
  diag_qshrink_4_file_open_rsp_type file_open_rsp;
  diag_qshrink_4_file_read_rsp_type file_read_rsp;
  diag_qshrink_4_file_close_rsp_type file_close_rsp;
}qshrink_4_op_data_type;
...
typedef enum {
  DIAG_QSHRINK_4_FILE_LIST = 0,
  DIAG_QSHRINK_4_FILE_OPEN = 1,
  DIAG_QSHRINK_4_FILE_READ = 2,
  DIAG_QSHRINK_4_FILE_CLOSE =3
} diag_qshrink_4_op_data_enum_type;

typedef enum {
  DIAG_QS4_SUCCESS = 0,
  DIAG_QS4_EPERM = 1,   /*Operation not permitted*/
  DIAG_QS4_ENOENT = 2,  /*No such file or directory*/
  DIAG_QS4_EINTR =3,    /*Interrupted by system call*/
  DIAG_QS4_EIO = 4,     /*I/O error*/
  DIAG_QS4_EBADF = 5,   /*Bad file number. Fd is not a valid file descriptor or is not open for reading*/
  DIAG_QS4_EAGAIN = 6,  /*Try again*/
  DIAG_QS4_ENOMEM = 7,  /*Out of memory*/
  DIAG_QS4_EACCESS = 8, /*Permission denied*/
  DIAG_QS4_EFAULT = 9,  /*Bad address. Outside of accessible address space.*/
  DIAG_QS4_EINVAL = 10, /*Invalid argument*/
  DIAG_QS4_EMFILE = 11, /*The process already has the maximum number of files open*/
  DIAG_QS4_ENAMETOOLONG = 12,  /*File name too long*/
  DIAG_QS4_ELOOP = 13,  /*Too many symbolic links encountered*/
  DIAG_QS4_EOVERFLOW = 14 /*Pathname refers to a file that is too large to be opened*/
} diag_qshrink_4_status_enum_type;

typedef PACK(struct) {
  diagpkt_subsys_hdr_type_v2 header; /* Sub System header for delayed response*/
  uint16 version;                     
  uint16 op_code;
  qshrink_4_op_data_type operation_data;
} diag_qshrink_4_database_file_retrieve_rsp_type;
...
typedef PACK(struct) {
  ...
  uint16 op_code;
  qshrink_4_op_data_type operation_data;
} diag_qshrink_4_database_file_retrieve_req_type;

/*this structure is to store the database information for core and user PD*/
typedef struct{
  uint8 guid[QSHRINK_4_DATABASE_GUID_LENGTH];
  char file_name[MAX_MSG_V4_DBASE_NAME_SIZE];
  int fd;
  uint32 file_len;  /*length of the database file*/
}diag_qshrink_4_database_file_type;
#define MAX_QSHRINK_DATABASE_FILE_NUMBER 2
#define QSHRINK_DATABASE_READ_DATABLOCK_SIZE 4000

#define DIAG_QSHRINK_CMD_VERSION_1 1
/*==========================================================================
...
RETURN 
  uint16 - index in the address table
===========================================================================*/

uint16 qsr_v4_get_address_table_index (const uint32 search_address);
