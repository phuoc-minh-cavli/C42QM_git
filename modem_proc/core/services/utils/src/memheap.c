...
$Header: //components/rel/core.mpss/3.10/services/utils/src/memheap.c#1 $ $DateTime: 2018/06/26 03:18:17 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
10/25/16   nk      Added mechanism to retry for non-zero magic number from prng api 
...
)
{
 ...
 // we should get a memheap critical section for magic number if it is not done yet;
  ...
  for(i=0;i<3;i++)
  {
	...
	if(magic_num[magic_num_index_local] != 0)
	{
	   break;
	}
    i++;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(chunks == 1)
  {
     chunks = 2;  /* To avoid 16 byte block creation */
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  MEMHEAP_ASSERT(theBlock->extra <= MEMHEAP_MAX_EXTRA);
   ...
   if(theBlock->start_flag != kStartBlock)
    {
      ...
      if((!((temp->freeBlock_frdOff)%kMinChunkSize)) 
	      &&(temp->pad  == magic_num[heap_ptr->magic_num_index])
		  &&(FOOTER_FRD_OFFSET_CHECK_SECTIONS(theBlock, heap_ptr, temp->freeBlock_frdOff))){
