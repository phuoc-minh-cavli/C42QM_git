...
/*===========================================================================
 ...
 $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_proto_utils.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
void icmp_read_tokens_nv( void )
{
                           ...
                           sizeof(icmp_reply_rate) ))
  {
    /*-----------------------------------------------------------------------
       ...
       -----------------------------------------------------------------------*/
    DS_ERROR_LOG_0( "Unable to read ICMP tokens NV, setting to default values");
...
{
  uint8 temp_domain_name_arr[DS_MAX_DOMAIN_NAME_SIZE] = {0};
  uint8 read_index = 0;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_ERROR_LOG_0( "NULL arguments" );
    return -1;
  }
  /* Ensure we have enough space in input domain name buffer to account for 
     length increment due to encoding. */
  if( *domain_name_len >= 
        ( DS_MAX_DOMAIN_NAME_SIZE - MAX_LENGTH_INCREMENT ))
  {
    DS_ERROR_LOG_1( "Insufficient space in domain name buffer %d", 
                     *domain_name_len );
    return -1;
  }
  ...
  {
    DS_ERROR_LOG_0("Domain name to encode is not NULL terminated.");
    return -1;
  }
  ...
  while( read_index < *domain_name_len )
  {
    ...
    else
    {
      ...
      {
        DS_ERROR_LOG_1( "Invalid label length %d", current_label_len );
        return -1;
      }
