...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_pkt_info_utils.c#3 $ $DateTime: 2021/01/11 06:33:33 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
01/11/21    ds     Code changes to skip processing of frag packets received
                   as part of A2 enquiry for MO-Ex packets
04/14/20    ds     Code changes for FRAG and ICMP support in DUN+AP call 
                   concurrency.
07/02/19    ds     Added NULL Check for default v6 address index in passed fwk 
                   instance before dereferncing it for IID for comparison.
05/02/19    ds     Added API to check whether destination address IID of the 
                   received packet is same as modem or not.
01/16/19    ds     Added support for RFC2460 Regulator Mandatory Requirement
10/23/18    ds     Fixed KW errors.
...
/*===========================================================================
FUNCTION      PS_PKT_INFO_V6_IID_MATCH

DESCRIPTION   This function checks if passed IID is same as default v6 IID
              of the PDN that network sent during PDN bring up.

DEPENDENCIES  None.

RETURN VALUE  TRUE if IID is same.
              otherwise FALSE

SIDE EFFECTS  None.
===========================================================================*/
boolean ps_pkt_info_v6_iid_match
(
...
)
{
  ...
  dsm_item_type *pkt_ptr      = *pkt_ref_ptr;
  uint64        dest_addr_iid = 0;

  if (fwk_ptr == NULL || pkt_ptr == NULL)
  {
    LOG_MSG_ERROR_0("Invalid Args.");
    DS_ASSERT( 0 );
  }

  /* Fetch the last 8 bytes(64 bits of IID) of the destination address. */
  (void) dsm_extract(pkt_ptr, IP6_BASE_HDR_DEST_ADDR_IID_OFFSET, 
                     &dest_addr_iid, 8);
  
  /*Now compare the fetched IID with modem IID. */
  if(fwk_ptr->v6_net_info.ipv6_addrs[DS_DEFAULT_V6_INDEX] != NULL && 
     fwk_ptr->v6_net_info.ipv6_addrs[DS_DEFAULT_V6_INDEX]->iid == dest_addr_iid)
  {
    return TRUE;
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    ...
    fragment.
  -------------------------------------------------------------------------*/
  LOG_MSG_DATA_PATH_INFO_0("ps_pkt_info_process_tx_fragment: Checking Fraghdl no NULL");
  if (NULL == fraghdl)
  {
    ...
    {
      LOG_MSG_DATA_PATH_INFO_0("ps_pkt_info_process_tx_fragment: Lookup failed allocating frag entry");
      /*---------------------------------------------------------------------
      ...
      allocated = TRUE;
      if( mi_ptr->rt_meta_info_ptr->pkt_info.is_fraghdl_allocated == 2 )
      {
        mi_ptr->rt_meta_info_ptr->pkt_info.is_fraghdl_allocated = 1;//Allocated
      }
      /*---------------------------------------------------------------------
        Must always have a valid meta info in the fraghdl to allow for
        sending icmp errors.
      ---------------------------------------------------------------------*/
      LOG_MSG_DATA_PATH_INFO_0("ps_pkt_info_process_tx_fragment: Initializing meta info.");
    ...
    switch( next_hdr )
    {
      ...
      case PS_IPPROTO_ICMP6:
            ...
            pi_ptr->ptcl_info.icmp.type == ICMP6_ECHO_RSP)
        {
          ...
          pi_ptr->ptcl_info.icmp.sequence_num = *((uint16*)&buf[6]);
        }
        break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == fraghdl)
  {
    if( NULL == (fraghdl = ip_fraghdl_lookup( pi_ptr )) )
    {
      LOG_MSG_INFO1_0("look up failed allocating new frag handle");      
      /*---------------------------------------------------------------------
        No fragment handling entry found, allocate a new one.
      ---------------------------------------------------------------------*/
      if( NULL == (fraghdl = ip_fraghdl_alloc( *pkt_ptr, offset, pi_ptr )) )
  ...
  pi_ptr->fraghdl = fraghdl;
  /* Place the frags into different queue such that after reassembly and
     pkt info generation, all the frags can be forwarded to right clients
     as per filters. */
  if(is_concurrency_on)
  { 
    /* Call API to put the packet in new queue. */
    LOG_MSG_INFO1_0("ps_pkt_info_process_rx_fragment(): inserting new frag");
    ip_fraghdl_queue_frag_packet(fraghdl, *pkt_ptr );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == ps_errno)
  {
    LOG_MSG_ERROR_0("ip_pkt_info_gen: NULL ps_errno");
    DS_ASSERT(0);
    return -1;
  }
  ...
  if ((NULL == mi_ref_ptr) || (NULL == pkt_ref_ptr))
  {
    *ps_errno = E_INVALID_ARG;
    LOG_MSG_ERROR_0("ip_pkt_info_gen: NULL arg");
    DS_ASSERT( 0 );
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR_0("ip_pkt_info_gen: unable to get IP version");
    *ps_errno = E_BAD_DATA;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR_1("ip_pkt_info_gen: Invalid IP version %d", ip_ver);
...
)
{
  ...
  switch((ps_ip_protocol_enum_type)protocol)
  {
      ...
      if(tcp_ack_prio_enabled)
      {
        switch(pkt_info_ptr->ip_vsn)
        {
          ...
          case IP_V6: 
            ...
            break;
          default:    
            LOG_MSG_ERROR_1("Invalid IP version %d", pkt_info_ptr->ip_vsn);
            return 0;
        }
    ...
    case PS_IPPROTO_ICMP6:
      ...
      (void)dsm_extract(pkt_ptr, dsm_offset, buf, 8*sizeof(uint8));
      ...
      if (pkt_info_ptr->ptcl_info.icmp.type == ICMP_ECHO_REPLY ||
          ...
          pkt_info_ptr->ptcl_info.icmp.type == ICMP6_ECHO_RSP)
      {
        /* Store the ID */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Check whether if this packet is recevied as part of A2 enquiry
       for MO exception packet, if yes then return wihtout processing 
       it further. */
    if (TRUE == mi_ptr->rt_meta_info_ptr->is_mo_ex)
    {
      LOG_MSG_DATA_PATH_INFO_0("ps_tx_pkt_info_generate(): Received FRAG "
                                "during MO-EX packet enquiry !! Return -1");
      return -1;
    }	  
#ifdef FEATURE_DATA_PS_IPV6
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (inst_ptr == NULL )|| (NULL == mi_ref_ptr) )
  {
    LOG_MSG_ERROR_1("filter_tx_pkt: Invalid fwk instance %p or meta_info_ref", inst_ptr);
    *ps_errno = E_INVALID_ARG;
    return -1;
  }

  if (FLTR_CLIENT_INPUT_MAX > client_id ||
      FLTR_CLIENT_MAX <= client_id)
  {
  ...
  LOG_MSG_DATA_PATH_INFO_3("ps_pkt_info_filter_tx_pkt: *mi_ref_ptr = 0x%x,"
                           ...
                           *mi_ref_ptr != NULL? PS_TX_META_IS_FILTER_MASK_SET(*mi_ref_ptr, client_id) : 0);
      ...
      !PS_TX_META_IS_FILTER_MASK_SET(*mi_ref_ptr, client_id)))
  {
    ...
    (
      mi_ptr,
      client_id,
      ds_ipfltr_execute (inst_ptr->ds_fwk_index,
                         client_id,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ( ds_fwk_inst_ptr == NULL ) || (NULL == mi_ref_ptr) )
  {
    LOG_MSG_ERROR_1("filter_tx_pkt: Invalid fwk instance %p or meta_info_ref", ds_fwk_inst_ptr);
    *ps_errno = E_INVALID_ARG;
    return -1;
  }
  ...
  {
    if( ps_rx_pkt_info_generate(pkt_ref_ptr, mi_ref_ptr, ps_errno, 
                                is_concurrency_on) == -1)
    {
      return -1;
    }
    ...
    (
      mi_ptr,
      client_id,
      ds_ipfltr_execute(  ds_fwk_inst_ptr->ds_fwk_index,
                           client_id,
                           DS_FWK_IPFLTR_SUBSET_ID_DEFAULT,
                           &PS_RX_META_GET_PKT_INFO (mi_ptr))
    );

    if (DS_IPFLTR_NOMATCH == 
    ...
    if( PS_RX_META_GET_FILTER_RESULT(mi_ptr, FLTR_CLIENT_SOCKETS) !=
        DS_IPFLTR_NOMATCH && 
        (NULL != fraghdl_ptr) )
    {
      ps_ip_fraghdl_set_fwk_instance(fraghdl_ptr,ds_fwk_inst_ptr);
    }    
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (sizeof(uint8) != dsm_extract(pkt_ptr, 0, &octet_val, sizeof(uint8)))
  {
    LOG_MSG_ERROR_0("ipfltr_process_pkt: unable to get IP version");
    *ps_errno = E_BAD_DATA;
    return -1;
  }
  ...
  if (IP_V4 != ip_ver && IP_V6 != ip_ver)
  {
    LOG_MSG_ERROR_1("ipfltr_process_pkt: Invalid IP version %d", ip_ver);
    *ps_errno = E_VERSION_MISMATCH;
    return -1;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR_0("ps_pkt_info_check_tcp_ack: unable to get IP version");
    *ps_errno = E_BAD_DATA;
    return TCP_ACK_NONE;
  }
