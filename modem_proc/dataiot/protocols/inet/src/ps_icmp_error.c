...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_icmp_error.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
)
{
  ...
  {
    //ICMPV4_INC_STATS(msgs_dropped_tx,1); /* IOE_LONLY */
  ...
  {
    LOG_MSG_ERROR_2("Cannot send ICMP error(type %d, code %d), invalid source"
  ...
  {
    LOG_MSG_ERROR_2("ICMP error message with type %d and code %d is rate limited,"
                    "cannot send", icmp->info.type, icmp->info.code);
    dsm_free_packet(data);
    return;
  }
  ...
  {
    LOG_MSG_ERROR_0("Error during icmpv4_hdr_create(), discarding packet");
    dsm_free_packet(data);
    return;
  }
  ...
  ip_cb.routing_cache = (ds_fwk_s_type*)pkt_info->if_ptr;
...
)
{
  ...
  if(!(icmp6->cmn_hdr.type == (uint8)ICMP6_PAR_PROBLEM &&
     ...
     (PS_IN6_IS_ADDR_MULTICAST(
      ...
      PS_IN6_IS_ADDR_MC_LINKLOCAL(
        pkt_info->ip_hdr.v6.hdr_body.base_hdr.dst_addr.ps_s6_addr)))
  {
    LOG_MSG_ERROR_2("Packet received on a multicast address, cannot send ICMPv6"
                    " error (type %d, code %d)",
  ...
  if(PS_IN6_IS_ADDR_UNSPECIFIED(
     ...
     PS_IN6_IS_ADDR_MULTICAST(
        pkt_info->ip_hdr.v6.hdr_body.base_hdr.src_addr.ps_s6_addr))
  {
    LOG_MSG_ERROR_2("Cannot send ICMPv6 error (type %d, code %d), invalid source"
                    " address of rx pkt",
                    icmp6->cmn_hdr.type, icmp6->cmn_hdr.code);
    dsm_free_packet(data);
    return;
  ...
  ds_fwk_get_ip_addr(pkt_info->if_ptr, IPV6_ADDR, &modem_ip_addr);

  /* Validate Modem Address */
  if(modem_ip_addr.type == IP_ADDR_INVALID)
  {
    //ICMPV6_INC_STATS(msgs_dropped_tx, 1); /* IOE_LONLY */
  ...
  switch(icmp6->cmn_hdr.type)
  {
    ...
    case ICMP6_PKT_TOO_BIG:
      ...
      {        
        icmp6->msg_type.mtu = DS_FWK_GET_MTU( pkt_info->if_ptr, IPV6_ADDR);
      }
      break;
  ...
  {
    LOG_MSG_ERROR_2("ICMPv6 error message with type %d and code %d is rate limited,"
  ...
  {
    LOG_MSG_ERROR_0("ps_icmp6_msg_create() conversion err, discard");
    dsm_free_packet(data);
    return;
  }
