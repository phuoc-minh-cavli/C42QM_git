...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/dsstcpi.h#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/09/20   ds      Code changes to add prototype for functions required for 
                   updating TCP State machine to LISTEN again for listening 
                   sokcets without clearing TCBs.
...
/*===========================================================================
FUNCTION DSSTCPI_RESET_LISTENER_TCP_SOCKET()

DESCRIPTION
  This function resets the listening TCP sokcet's TCB and moves back to
  listen state.

ARGUMENTS:
  tcb_ptr: This is a pointer to the TCP control block that is being reset.
  This argument must point to a valid TCP control block and cannot be NULL.
...
(
  struct tcb* tcb_ptr                            /* Ptr to TCB to be reset */
);
