...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_tcpin.c#6 $ $DateTime: 2022/07/22 09:16:19 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/07/21   ds      Code changes to reset the backoff counter and timer when 
                   ACK is received for Re-Tx'd TCP segments.
02/16/21   sr      Used common macros for socket option across sockets and 
                   protocols
12/23/20   rj      Add support for TCP/UDP stats.
04/08/20   ds      Code changes to add validation check for timer before 
                   cancelling it.
01/09/20   ds      Code changes to move to TCP_LISTEN state again if RST is 
                   received from peer during incoming connection establishment.
12/04/19   ds      Code changes to reinitialize the TCP congestion window once
                   all UL data is ACKed by N/w.
11/08/19   ds      Added break once matching listening TCB is found while 
                   processing incoming TCP connections. 
11/08/19   ds      Code changes to reset the keealive probes counter to 0
                   once ACK is recevied indicating active TCP session.                  
04/02/19   ds      Fixed Compiler warnings.
10/12/18   ds      Fixed the KW errors.
08/13/18   ds      Code changes as per new FWK design for IOT.
09/20/16   ss      Reset Backoff counter properly when TS Permitted option is
...
(
  struct tcb *tcb        /* ptr to TCP Task control block         */
)
{
...
(
  ...
  ip_version_enum_type     ip_ver             /* IP Version                */
)
{
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (header_ptr != NULL)
  {
    ...
    TCP_INC_STATS(rst_tx, 1) ; /* IOE_LONLY */
...
(
  ...
  struct tcp *seg,          /* ptr to TCP segment info            */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(!accept)
  {
    LOG_MSG_ERROR_1( "Seg: %lx not in window", seg->seq);
    TCP_INC_STATS(dup_bytes_rx, len); /* IOE_LONLY */
    TCP_INC_STATS(dup_segs_rx, 1); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,dup_bytes_rx, len); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,dup_segs_rx, 1); /* IOE_LONLY */
  ...
  if((dupcnt = (sint31)(tcb->rcv.nxt - seg->seq)) > 0)
  {
    ...
    if(dupcnt > 0)
    {
      TCP_INC_STATS(dup_bytes_rx, (uint32)dupcnt); /* IOE_LONLY */
      TCP_INC_STATS(dup_segs_rx, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,dup_bytes_rx, len); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,dup_segs_rx, 1); /* IOE_LONLY */
...
(
  struct tcb *tcb,                /* pointer to TCP control block */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   */
  if (tcb->family == DSS_AF_INET)
  {
    mtu = ps_ip_calc_path_mtu( IPV4_ADDR, 
                               tcb->ipcb.routing_cache ) - IPLEN - TCPLEN;
  }
  else if (tcb->family == DSS_AF_INET6)
  {
    mtu = ps_ip_calc_path_mtu( IPV6_ADDR, 
...
)
{
  ...
  DS_SYSTEM_HEAP_MEM_FREE(curr_block_ptr);
}
/*===========================================================================
...
)
{
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(temp_block_ptr,
                           sizeof(struct tcp_sequence_block_list),
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if(seg->ack == tcb->snd.una)
  {
    ...
    TCP_INC_STATS(dup_acks_rx, 1); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,dup_acks_rx, 1); /* IOE_LONLY */
  ...
  if (length > 0 )
  {
    TCP_INC_STATS(piggybacked_acks_rx, 1) ; /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,piggybacked_acks_rx, 1) ; /* IOE_LONLY */
  }
  ...
  if(tcb->flags.rtt_run && tcp_seq_ge(seg->ack,tcb->rttseq))
  {
    ...
    /* Handle resetting the backoff counter in case ACK is received for Re-Tx'd 
       TCP segments. */
    else
    {
      /* Reset the backoff level */
  ...
  LOG_MSG_INFO1_1( "tcp->sndcnt = %d ", tcb->sndcnt);
  tcb->snd.una  = seg->ack;
  ...
  {
    if(ps_timer_is_running(tcb->timer))
    {
  ...
  /* if TCP sndcnt becomes zero then re-initialize the TCP Congestion window so that 
     application can write data again. */
  if (tcb->sndcnt == 0)
  {
    tcb->cwind = MIN(3 * tcb->mss, PS_TCP_DEF_CWIND);
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( trim( tcb_ptr, seg, &tcp_seg_ptr, &length) == -1)
  {
    ...
    TCP_INC_STATS(dup_acks_tx, 1); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb_ptr,dup_acks_tx, 1); /* IOE_LONLY */
  ...
  {
    TCP_INC_STATS(out_of_order_segs_rx, 1); /* IOE_LONLY */
    TCP_INC_STATS(out_of_order_bytes_rx, length); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb_ptr,out_of_order_segs_rx, 1); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb_ptr,out_of_order_bytes_rx, length); /* IOE_LONLY */
    /*-----------------------------------------------------------------------
...
#define PS_TRANSPORT_CHECK_DSM_AVAIL(item)               \
  ...
  if(length > 0)
  {
    ...
    dsm_append( &tcb_ptr->rcvq, &tcp_seg_ptr);
    tcb_ptr->rcv.nxt += length;
    tcb_ptr->rcvcnt  += length; 
    tcb_ptr->rcv.wnd -= length; 
    DS_LEAVE_CRIT_SECTION(&global_ps_crit_section);    

    TCP_INC_STATS(bytes_rx, length); /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb_ptr,bytes_rx, length); /* IOE_LONLY */
    ...
    {
      /* The following params are also accessed in app context in dsstcp_read
       * So use critical section.*/
      DS_ENTER_CRIT_SECTION(&global_ps_crit_section);
      dsm_append( &tcb_ptr->rcvq, &reseq_data_ptr);
      tcb_ptr->rcv.nxt += reseq_hdr.length;
      tcb_ptr->rcvcnt  += reseq_hdr.length;//Please check if this is needed
      tcb_ptr->rcv.wnd -= reseq_hdr.length;// Please check if this is needed
      DS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (NULL != tcb_ptr)
    {
    DS_ASSERT(SOCKAVAIL != tcb_ptr->sockfd);
      ...
      {
      DS_ASSERT(TCP_CLOSED != tcb_ptr->state);
        return( tcb_ptr);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (NULL != listen_tcb_ptr)
  {
    ...
    {
      DS_ASSERT(SOCKAVAIL != listen_tcb_ptr->sockfd);
      ...
      ---------------------------------------------------------------------*/
      if (tcb_ptr == NULL &&
          (family == AF_UNSPEC || listen_tcb_ptr->family == family) &&
...
{
  return dsstcp_lookup_serv_af(conn, AF_UNSPEC);
}
...
{
  struct tcb  *tcb = NULL;      /* TCP Protocol control block     */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  TCP_INC_STATS(segs_rx,1); /* IOE_LONLY */
  if(ip_pkt_info_ptr->is_brdcst)
  {
    /* Any TCP packet arriving as a broadcast is
     * to be completely IGNORED!!
     */
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    dsm_free_packet(&tcp_seg_ptr);
    return;
  }
  ...
  if(cksum(&ip_pkt_info_ptr->pseudo_hdr,tcp_seg_ptr,length,offset) != 0)
  {
    ...
    TCP_INC_STATS(bad_chksum,1); /* IOE_LONLY */
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    LOG_MSG_ERROR_1( "TCP Checksum Err, Seq: %lx", seg.seq);
    dsm_free_packet(&tcp_seg_ptr);
    return;
  }
  ...
  /* Sanity check to match IP header length to actual payload length */
  {
    ...
    TCP_INC_STATS(bad_hdr_len,1); /* IOE_LONLY */
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    dsm_free_packet(&tcp_seg_ptr);
    return;
  }
  ...
  if(tcp_hdr_parse(tcp_seg_ptr,&hdrlen,&seg) == FALSE)
  {
    ...
    TCP_INC_STATS(too_short,1); /* IOE_LONLY */
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    LOG_MSG_ERROR_0( "Bad TCP Header, Ignoring Segment");
    dsm_free_packet(&tcp_seg_ptr);
    return;
  }
  /*-------------------------------------------------------------------------
  ...
  ip_cb.routing_cache = (ds_fwk_s_type*)ip_pkt_info_ptr->if_ptr;
...
#endif /* FEATURE_DATA_IS707_ASYNC */
  {
    ...
    else
    {
      ...
      if(current_connections <= (DSS_MAX_TCBS - TCP_LOW_THRESHOLD))
      {
        if( ip_pkt_info_ptr->ip_vsn == IP_V4 )
        {
          tcb = dsstcp_lookup_serv_af( &conn, DSS_AF_INET );
        }
        else if( ip_pkt_info_ptr->ip_vsn == IP_V6 )
        {
          tcb = dsstcp_lookup_serv_af( &conn, DSS_AF_INET6 );
        }
        else
        {
  ...
  if( tcb == NULL)
  {
    TCP_INC_STATS(dropped_segs_noconn_rx,1); /* IOE_LONLY */
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    dsm_free_packet(&tcp_seg_ptr);
    LOG_MSG_ERROR_0("Received segment for invalid connection");
    tcp_send_reset(&seg,&ip_pkt_info_ptr->pseudo_hdr,&ip_cb, ip_pkt_info_ptr->ip_vsn);
    return;
  }
  TCP_INC_INSTANCE_STATS(tcb,segs_rx,1); /* IOE_LONLY */
  ...
  case TCP_CLOSED:
    ...
    -----------------------------------------------------------------------*/
    if( !DS_FWK_INST_IS_VALID(tcb->ipcb.routing_cache, (ip_addr_enum_type)ip_pkt_info_ptr->ip_vsn) )
    {
      tcp_send_reset(&seg,&ip_pkt_info_ptr->pseudo_hdr,&ip_cb, ip_pkt_info_ptr->ip_vsn);
    }
    else
    {
      tcp_send_reset(&seg,&ip_pkt_info_ptr->pseudo_hdr,&tcb->ipcb, ip_pkt_info_ptr->ip_vsn);
    }
    return;
  ...
  case TCP_LISTEN:
    if(seg.rst)
    {
      TCP_INC_STATS(rst_rx,1); /* IOE_LONLY */
      TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    ...
    if(seg.syn)
    {
      ...
      (
        &(tcb->ipcb),
        (ds_fwk_s_type*)ip_pkt_info_ptr->if_ptr
      );

      /* (Security check is bypassed) */
      /* page 66 */
      TCP_INFO_CHANGE_UPCALL(tcb);
      IP_INFO_CHANGE_UPCALL(&(tcb->ipcb), tcb->sockfd, tcb->scb_ptr, tcb->info_change_upcall);
  ...
  if( tcp_seq_lt(seg.seq + length , tcb->rcv.nxt)  ||
      ...
      tcb->rcv.wnd != 0) )
  {
    LOG_MSG_ERROR_1("Seqno out of receive window. Dropping packet: seqno %d",
                    seg.seq );
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    dsm_free_packet(&tcp_seg_ptr);
    if(!seg.rst)
    {
      ...
      TCP_INC_STATS(dup_acks_tx, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,dup_acks_tx, 1); /* IOE_LONLY */
      tcb->flags.force = 1;
      tcp_output(tcb);
    }
    return;
  ...
  if(seg.rst)
  {
    ...
    {
      /*---------------------------------------------------------------------
        Go back to listen state only if this was not a cloned or active
        server TCB
      ---------------------------------------------------------------------*/
      dsstcpi_reset_listener_tcp_socket(tcb);
    }
  ...
  if(!seg.ack_flag)
  {
    ...
    TCP_INC_STATS(dropped_segs_rx,1); /* IOE_LONLY */
    LOG_MSG_ERROR_1( "Missing ACK, Discard Seg: %lx", seg.seq);
    dsm_free_packet(&tcp_seg_ptr);
    return;
  }
  ...
  switch(tcb->state)
  {
    ...
    case TCP_LAST_ACK:
      ...
      if ( tcb->persist_timer != PS_TIMER_INVALID_HANDLE )
      {
        ...
        {
          if(ps_timer_is_running(tcb->persist_timer))
          {
  ...
  switch(tcb->state)
  {
    ...
    case TCP_FINWAIT1:
    case TCP_FINWAIT2:
      notify_app = tcp_enqueue_data(tcb, tcp_seg_ptr, &seg, length);
      break;
    case TCP_TIME_WAIT:
      /*---------------------------------------------------------------------
  ...
  if ( TRUE == notify_app && tcb->r_upcall != NULL)
  {
