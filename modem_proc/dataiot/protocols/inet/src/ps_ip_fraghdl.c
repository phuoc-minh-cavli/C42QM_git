...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_ip_fraghdl.c#2 $
  $DateTime: 2020/04/15 22:43:35 $
  $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/14/20   ds      Made the function ip6_fraghdli_put() and ip4_fraghdli_put()
                   global by removing static keyword so that it can be
                   called from other locations.
04/09/19   ds      Code changes to return NULL in case packet passed is not
                   actually a fragment.
...
typedef struct ip_fraghdl_s_type
{
...
#endif /* FEATURE_DATA_PS_IPV6 */
  void *fwk_ptr;               /* fwk ptr, using which ICMP error messages
  ...
  q_type frag_queue_ex;           /* Fragment queue to store fragments to 
                                     later send to framework */
} ip_fraghdl_s_type;
...
)
{
  ...
  if( NULL != fraghdl_ptr->device_ptr )
  {
    ...
    {
      LOG_MSG_ERROR_0("Frag Q is empty!!");
      return;
    }

    if ( IP_FRAGHDL_BRIDGE_TYPE_IFACE_BRIDGE == fraghdl_ptr->device_type )
    {
      fwk_ptr = (ds_fwk_s_type *)fraghdl_ptr->device_ptr;
      LOG_MSG_INFO1_1("FWK Bridging - Sending all the queued fragments onto "
               "fwk Instance 0x%x", fwk_ptr);
      ...
      do
      {
        PS_TX_META_INFO_DUP(frag_meta_info_ptr,
                            &tmp_meta_info_ptr);
        (void)fraghdl_ptr->frag_tx_cmd_f_ptr(fwk_ptr,
                                             &item_ptr,
                                             tmp_meta_info_ptr);
        item_ptr = ip_fraghdl_get_fragment(fraghdl_ptr);
  ...
  else
  {
       ...
       )
    {
      ...
      fwk_ptr = (ds_fwk_s_type *)meta_pkt_info_ptr->if_ptr;

#ifdef FEATURE_DATA_PATH_INFO
      if (NULL != fwk_ptr)
      {
        LOG_MSG_INFO1_1("Meta_info's fwk inst 0x%x",fwk_ptr );
      }
#endif
    }
    ...
    else
    {
      ...
      -------------------------------------------------------------------*/
      LOG_MSG_ERROR_0("Out-Of-Order fragment, drop it!");
      dsm_free_packet(&item_ptr);
    }
  }
         ...
         next_hdr !=PS_IPPROTO_AH)
  {
    switch( next_hdr )
    {
      ...
      case PS_IPPROTO_DEST_OPT_HDR:
        if (0 == dsm_extract(item_ptr, pkt_offset,
                             extract_buf, sizeof(uint16)))
        {
          LOG_MSG_ERROR_0("ip6_frag: unable to get Dest Opt hdr len");
          return;
        }
      ...
      case PS_NO_NEXT_HDR:
      default:
        LOG_MSG_ERROR_1("%d Header Not Supported",next_hdr);
        return;
    }
  }
...
)
{
  ...
  for(i = 0; i < NUM_FRAGHDL ; i++ )
  {
    ...
    {
      ret = (void *)fraghdl_ptr;
      LOG_MSG_INFO1_0("found matching frag handle");
      break;
    }
  }
...
)
{
  ...
  if(next_frag == NULL)
  {
    LOG_MSG_INFO1_0("inserting new frag");
  ...
  if( NULL != fraghdl_ptr )
  {
    ...
    switch( pkt_info->ip_vsn )
    {
      ...
      case IP_V6:
        ...
        fraghdl_ptr->fwk_ptr = pkt_info->if_ptr;
        break;
#endif /* FEATURE_DATA_PS_IPV6 */
...
)
{
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_1("frag handle q_cnt %d",q_cnt(&fraghdl_ptr->frag_queue));
...
/*===========================================================================
FUNCTION IP_FRAGHDL_GET_FRAGMENT_EX()

DESCRIPTION
  Dequeues and returns the fragment in the queue.

PARAMETERS
  handle - Fragment handle to get all the fragments.
...
SIDE EFFECTS
  ...
  fragments cont_start_bytes may be invalid after a get()
===========================================================================*/
dsm_item_type *ip_fraghdl_get_fragment_ex
(
  void *handle
)
{
  ...
  /*-------------------------------------------------------------------------
    Get and return the fragment from the queue.
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_1("frag handle q_cnt %d",q_cnt(&fraghdl_ptr->frag_queue_ex));
  pkt_ptr = q_get( &fraghdl_ptr->frag_queue_ex );

  return pkt_ptr;
} /* ip_fraghdl_get_fragment_ex() */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if( NULL == fraghdl_ptr )
  {
    LOG_MSG_ERROR_0( "NULL handle passed to ip_fraghdl_free()" );
    return;
  }

  if( FALSE == fraghdl_ptr->in_use )
  {
    LOG_MSG_ERROR_0( "handle passed to ip_fraghdl_free() is not in use");
    return;
  }
  ...
  /*-------------------------------------------------------------------------
    Remove any fragments currently in the queue
  -------------------------------------------------------------------------*/
  while( NULL != (item_ptr = q_get(&fraghdl_ptr->frag_queue_ex)) )
  {
    dsm_free_packet( &item_ptr );
  }
...
)
{
  ...
  for(i = 0; i < NUM_FRAGHDL ; i++ )
  {
    ...
    (void) q_init(&fraghdl_ptr->frag_queue_ex);
  }
...
SIDE EFFECTS
===========================================================================*/
void ps_ip_fraghdl_set_fwk_instance
(
  void          *handle,
...
)
{
  ...
  fraghdl_ptr->fwk_ptr = ds_fwk_inst_ptr;

} /* ps_ip_fraghdl_set_fwk_instance */
...
SIDE EFFECTS
===========================================================================*/
ds_fwk_s_type * ps_ip_fraghdl_get_fwk_instance
(
  void     *handle
)
{
  ...
  return fraghdl_ptr->fwk_ptr;

} /* ps_ip_fraghdl_get_fwk_instance */

/*===========================================================================
FUNCTION IP_FRAGHDL_QUEUE_FRAG_PACKET()

DESCRIPTION
  queues the paassed fragment into the given fragment handle.

PARAMETERS
  handle -- Fragment handle to operate on.
  pkt_ptr -- fragment packet to be added in frag queue.
...
SIDE EFFECTS
===========================================================================*/
void ip_fraghdl_queue_frag_packet
(
  void           *handle,
  dsm_item_type  *pkt_ptr
)
{
  ...
  /*-------------------------------------------------------------------------
    Verify arguments
  -------------------------------------------------------------------------*/
  if( NULL == fraghdl_ptr || NULL == pkt_ptr )
  {
    LOG_MSG_ERROR_2( "NULL arg: handle 0x%x, pkt_ptr 0x%x, pkt_info 0x%x",
         (uint32)handle, (uint32)pkt_ptr );
    DS_ASSERT( 0 );
    return;
  }
  ...
  fragment_length = dsm_length_packet(pkt_ptr);
  (void) dsm_dup_packet( &fragment_ptr, pkt_ptr, 0, fragment_length );
  LOG_MSG_INFO1_0("put fragment back in queue if PDN shared");
  q_put( &fraghdl_ptr->frag_queue_ex, &fragment_ptr->link );
