...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_tcphdr.c#2 $ $DateTime: 2021/06/07 02:45:44 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/30/21   vls      TCP window scale option max allowed value of 14 
                   instead of current limit of 13
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_ASSERT(tcph->optlen <= TCP_MAXOPT);
  ...
  for( cp=tcph->options, i=tcph->optlen; i>0;)
  {
    ...
    switch( kind)
    {
      case TCP_MSS_KIND:
        ...
        {
          LOG_MSG_ERROR_1(" Malformed TCP MSS option: kindlen %d", kindlen);
          return FALSE;
        }
        break;
      ...
      case TCP_SACK_PERMITTED_KIND:
        ...
        {
          LOG_MSG_ERROR_1(" Malformed TCP SACK PERMITTED option: kindlen %d",
              kindlen);
          return FALSE;
        }
        break;
      ...
      case TCP_SACK_KIND:
        ...
        if (((kindlen - 2) & 0x7) != 0)
        {
          LOG_MSG_ERROR_1(" Malformed TCP SACK options: kindlen %d", kindlen);
          return FALSE;
        }
    ...
    case TCP_TIME_STAMP_KIND:
        ...
        {
          LOG_MSG_ERROR_1(" Malformed TCP TIME STAMP option: kindlen %d",
                   kindlen);
          return FALSE;
        }
        break;
      ...
      case TCP_WINDOW_SCALE_KIND:
        ...
        {
          LOG_MSG_ERROR_2(" Malformed TCP WINDOW SCALE option: kindlen %d  scale %d",
                           kindlen,*cp);
          return FALSE;
        }
        break;
      ...
      default:
        LOG_MSG_ERROR_1 ("Unrecognized TCP option %d!  ignoring", kind);
        break;
    }
    /*-----------------------------------------------------------------------
