...
Copyright (c) 1995-2023 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_tcpout.c#6 $ $DateTime: 2023/05/03 05:25:43 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/19/23    my     Fixed TCP retranmit timer invalid issue.
05/18/21    rk     Fixed tcp segmentation issue.
...
extern uint32 min_rto; /* min_rto timer for TCP */
...
//  ps_iface_type      *ps_iface_ptr;
  ...
  struct ds_flow_s          *ds_flow_ptr = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(;;)
  {
    ...
    {
      /*******************************************************************************/
      ds_fwk_inst_ptr = (ds_fwk_s_type*)PS_TX_META_GET_ROUTING_CACHE(tcb->ipcb.meta_info_ptr);
      ds_flow_ptr = (struct ds_flow_s*)PS_TX_META_GET_FILTER_RESULT(tcb->ipcb.meta_info_ptr,
                                                   FLTR_CLIENT_QOS_OUTPUT);
      /********************************************************************************/
      if (ds_flow_ptr != NULL)
      {
        if (!ds_fwk_is_flow_enabled(ds_flow_ptr))
        {
    ...
    /*-----------------------------------------------------------------------
      Reset the number of bytes already pushed to the dsm buffer 
      while preparing next tcp segment.
    -----------------------------------------------------------------------*/
    copied = 0;
  ...
  /*-----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
    ...
    seg.ack = tcb->rcv.nxt;
    seg.wnd = tcb->rcv.wnd  >> tcb->rcv.scale; //Sid
    seg.up = 0;

    if( seg.ack != tcb->last_ack_sent && dsize != 0)
    {
      TCP_INC_STATS(piggybacked_acks_tx, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,piggybacked_acks_tx, 1); /* IOE_LONLY */
    }
    ...
    if(dsize != 0)
    {
      ...
      while ( copied < dsize )
      {
        ...
        if (item_ptr == NULL)
        {
          LOG_MSG_ERROR_0("Running out of buffers");
        ...
        if ( extracted != to_copy )
        {
          ...
          else
          {
            ...
            ---------------------------------------------------------------*/
            LOG_MSG_ERROR_0("tcp sndq is corrupted");
            dsm_free_packet( &pkt_head_ptr);
    ...
    {
      TCP_INC_STATS(segs_rexmit, 1); /* IOE_LONLY */
      TCP_INC_STATS(bytes_rexmit, MIN(tcb->snd.nxt - tcb->snd.ptr, ssize)); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,segs_rexmit, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb, 
                             bytes_rexmit,
                             MIN(tcb->snd.nxt - tcb->snd.ptr, ssize)); /* IOE_LONLY */
    }
    tcb->snd.ptr += ssize;
    ...
    if (ssize != 0 && tcb->persist_backoff == 0)
    {
      ...
      if (ps_timer_is_running(tcb->timer) == FALSE)
      {
        ...
        else if ((tcb->total_backoff_time !=0 && tcb->backoff != 0) && tcb->tcp_rexmit_timer_val == 0)
        {
          tcp_timeout_cb(tcb);
        }
        else 
        {
          LOG_MSG_ERROR_0("TCP Re-Tx timer value computed is 0, don't sent anything !!");
          return;
        }
      }
    ...
    {
      TCP_INC_STATS(window_probe_tx, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb,window_probe_tx, 1);
    }
    tcb->numb_bytes_in_transit += ssize;

    TCP_INC_STATS(bytes_tx, dsize) ; /* IOE_LONLY */
    TCP_INC_STATS(segs_tx, 1) ; /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,bytes_tx, dsize) ; /* IOE_LONLY */
    TCP_INC_INSTANCE_STATS(tcb,segs_tx, 1) ; /* IOE_LONLY */
    /*-----------------------------------------------------------------------
