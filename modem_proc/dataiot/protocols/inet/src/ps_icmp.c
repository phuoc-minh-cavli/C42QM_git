...
/*===========================================================================
 ...
 $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_icmp.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
)
{
  ...
  switch(_icmp_hdr.type)
  {
    ...
    default:
      LOG_MSG_ERROR_0("Unknown ICMP error");
      return DS_EHOSTUNREACH;
  }
}/* icmp_get_errno() */
/*===========================================================================
...
)
{
  ...
  {
    LOG_MSG_ERROR_0("Null parameter to icmp_get_conn_struct()");
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR_0("Error in IP header parse");
    return FALSE;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR_0("NULL parameter passed to icmp_process_error()");
    if(bpp && *bpp)
    {
      dsm_free_packet(bpp);
    }
  ...
  switch(icmp_ptr->info.type)
  {
    ...
    case ICMP_PARAM_PROB :
      ...
      {
        LOG_MSG_ERROR_0("Error in dsm_peek_byte()");
        dsm_free_packet(bpp);
        return;
      }
      break;
  ...
  switch(protocol)
  {
    ...
    default:
      LOG_MSG_ERROR_0("Unknown transport protocol");
      dsm_free_packet(bpp);
      break;
  }
...
)
{
  switch(type)
  {
    ...
    default :
      LOG_MSG_ERROR_1("Invalid ICMP message type %d", type);
      return 0;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  /**/
  //ICMPV4_INC_STATS(msgs_tx, 1); /* IOE_LONLY */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 /*------------------------------------------------------------------------
  ...
  {
    LOG_MSG_ERROR_0( "Truncated packet received" );
  ...
  {
    LOG_MSG_ERROR_0("Invalid ICMP Checksum, Discard");
    dsm_free_packet( &item_ptr);
    return;
  }
  ...
  {
    LOG_MSG_ERROR_0("icmp_hdr_parse conversion err, discard");
    dsm_free_packet( &item_ptr);
    return;
  }
  ...
  default:
    ...
    {
      LOG_MSG_ERROR_1( "Unsupported ICMP message %d, Discard", type);
      dsm_free_packet( &item_ptr);
    }
    return;
  } /* end switch */
  ...
  {
    LOG_MSG_ERROR_0("ICMP reply is rate limited, cannot send the reply.");
    dsm_free_packet(&item_ptr);
    return;
  }
  ...
  {
    LOG_MSG_ERROR_0("icmp_hdr_create() conversion err, discard");
    dsm_free_packet( &item_ptr);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_0("ICMP message is rate limited, cannot send the reply.");
    dsm_free_packet(&item_ptr);
    return;
  }
