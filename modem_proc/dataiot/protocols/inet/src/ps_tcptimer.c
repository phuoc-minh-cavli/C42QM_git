...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_tcptimer.c#3 $ $DateTime: 2020/12/23 03:28:10 $ $Author: pwbldsvc $
...
04/08/20   ds      Code changes to add validation check for timer before 
                   cancelling it.
12/04/19   ds      Code changes to adjust the TCP CWIND as per its current
                   value.
10/10/19   ds      Code changes to reset TCP congestion window to 2*MSS
                   in case of Re-Tx.
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /*-------------------------------------------------------------------------
    ...
    If DS_ASSERTs are turned on, this condition should be caught.
    If DS_ASSERTs are turned off, we exit from this function so that invalid
  ...
  /*-------------------------------------------------------------------------
    Cancel timer.
  -------------------------------------------------------------------------*/
  if(ps_timer_is_running(tcb_ptr->timer))
  {
  ...
  switch(tcb_ptr->state)
  {
    ...
    default:
      ...
      TCP_INC_STATS(rexmit_timeouts, 1); /* IOE_LONLY */
      TCP_INC_INSTANCE_STATS(tcb_ptr,rexmit_timeouts, 1); /* IOE_LONLY */
      tcb_ptr->timeouts++;
      tcb_ptr->backoff++;
           ...
           tcb_ptr->backoff >= TCP_MAX_BACKOFFS)
      {
        ...
        TCP_INC_STATS(conn_timeout_aborts, 1); /* IOE_LONLY */
      ...
      else
      {
        ...
        tcb_ptr->cwind = MIN(tcb_ptr->cwind, 2 * tcb_ptr->mss);
