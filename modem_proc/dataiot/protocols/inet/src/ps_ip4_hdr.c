...
Copyright (c) 2004-2022 Qualcomm Technologies Incorporated. 
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_ip4_hdr.c#3 $ $DateTime: 2022/05/24 00:01:24 $ $Author: pwbldsvc $
...
)
{
  uint16 hdr_len;                        /*                  Header length */
  uint16 flags_offset;                   /* Fragmentation flags and offset */
  dsm_item_type *hdr_ptr = NULL;
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  if( data_payload == NULL || *data_payload == NULL  || NULL == hdr)
  {
    LOG_MSG_ERROR_0("ps_ip4_hdr_create_ex(): PS_BAD_ARG_OR_NULL_PTR");
    return FALSE;
  }
  ...
  if((hdr_len > (IPLEN + IP_MAXOPT)) || ((hdr_len & 0x3) != 0))
  {
    LOG_MSG_INFO1_1("Malformed options as IP header length including options is %d"
        ,hdr_len);
    return FALSE;
  }
  ...
  /*-------------------------------------------------------------------------
    Push the created IP header on the dsm chain.
  -------------------------------------------------------------------------*/
  hdr_ptr = mutils_dsm_anysize_new_buffer(MUTILS_DSM_CLIENT_PS,
  ...
  -----------------------------------------------------------------------*/
  ASSERT((hdr_ptr != NULL) &&
         (hdr_ptr->used + hdr_len <= hdr_ptr->size));
        
  memscpy((hdr_ptr->data_ptr + hdr_ptr->used),
          (hdr_ptr->size - hdr_ptr->used),
          hdr, hdr_len);
     
  /* increase the used fields */
  hdr_ptr->used += hdr_len;
     
  /* set the cache flush flag to ture */
  dsm_item_set_cache_operation(hdr_ptr,DSM_PKT_CACHE_FLUSH_ON);

  /* Append the  header to the payload to complete the packet 
     consisting of (Hdr + payload). */
  dsm_append(&hdr_ptr, data_payload);   
  
  /*-----------------------------------------------------------------
    Udpate the DSM pointer to point to new DSM item having 
    IPv4 header.
  -----------------------------------------------------------------*/
  *data_payload = hdr_ptr;
