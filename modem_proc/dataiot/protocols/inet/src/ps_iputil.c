...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/src/ps_iputil.c#2 $ $DateTime: 2020/12/23 02:05:09 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/22/20   ds      Added utility functions to print DSM hexdump.
...
uint16 eac
(
  uint32 sum     /* Carries in high order 16 bits */
)
{
  uint16 csum;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
/*===========================================================================
FUNCTION      PS_IPUTIL_HEXDUMP

DESCRIPTION   Dumps a buffer in hex. 
  
DEPENDENCIES  None
  
PARAMETERS    buf    - Pointer to the buffer.
...
)
{
  ...
  if ( printbuf == NULL )
  {
    DS_MSG0( MSG_LEGACY_HIGH, "NULL BUFFER" );
    return;
  }
  
  for ( idx=1; idx<=length; idx++ )
  {
    snprintf(temp, sizeof(printbuf[idx])+3, "%02x ", printbuf[idx-1]);
    temp += 3;
    
    if ( !(idx%16) )
    {
      DS_MSG_SPRINTF_1( MSG_LEGACY_HIGH, "%s", str );
...
/*===========================================================================
FUNCTION      PS_IPUTIL_DSM_HEXDUMP

DESCRIPTION   Dumps the data content of a DSM in hex. 
  
DEPENDENCIES  None
  
PARAMETERS    dsm_item - Double pointer to the DSM item.

RETURN VALUE  None
  
SIDE EFFECTS  None
===========================================================================*/
void ps_iputil_dsm_hexdump
(
...
)
{
  uint32         total_len = 0;
  uint32         cnt = 0;
  dsm_item_type *tmpdsm = *dsm_item;
    
  if ( NULL == tmpdsm )
  {
    DS_MSG0( MSG_LEGACY_HIGH, "NULL DSM_ITEM" );
    return;
  }

  total_len = dsm_length_packet(tmpdsm);
  DS_MSG1( MSG_LEGACY_HIGH, "DSM MSG Length: %d", total_len );

  if (NULL == tmpdsm->pkt_ptr)
  {
    /* Means only 1 dsm item...life is easy */
    ps_iputil_hexdump(tmpdsm->data_ptr, tmpdsm->used);
  }
  else
  {
    /* More than 1 dsm item */
    while (cnt<total_len)
    {
      ps_iputil_hexdump(tmpdsm->data_ptr, tmpdsm->used);
      cnt += tmpdsm->used;
      tmpdsm = tmpdsm->pkt_ptr;
    }
  }
  return;
} /* ps_iputil_dsm_hexdump() */
