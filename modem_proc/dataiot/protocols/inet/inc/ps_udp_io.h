...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/inc/ps_udp_io.h#2 $ $DateTime: 2020/12/23 03:28:10 $ $Author: pwbldsvc $
...
struct udp_cb
{
  ...
  dsm_watermark_type  rcvq_wm;  /* udp receive queue watermark			  */
  q_type rcvq; 				 /* UDP receive queue for the socket		*/
  uint32 rcv_pktcnt;			 /* Number of received packets				*/
  uint32 rcvbuf;				 /* Maximum number of bytes allowed in rcvq */
  boolean cant_recv_more;		 /* Cannot receive data for this connection */
  dsm_watermark_type  errq_wm;  /* udp error queue watermark				*/
  q_type errq; 				 /* UDP error queue for the socket			*/
  uint32 err_pktcnt;			 /* Number of error pkts enqueued in errq	*/
  
  void   * errq_ptr;
  void (*info_change_upcall) ( void * scb_ptr,
                               struct ps_event_info_s* pcb_info 
                             );
};
