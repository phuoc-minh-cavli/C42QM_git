...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/inc/ps_tcp_hdr.h#2 $ $DateTime: 2021/01/07 04:13:24 $ $Author: pwbldsvc $
...
01/09/20    ds     Added parameters to take back up of window size and window 
                   scale.
...
struct tcb_rcv_seq_s
{
  ...
  uint32       wnd_bkup;         /* Backup for original receive window     */
  uint16       up;               /* Receive urgent pointer                 */
  uint8        scale;            /* Our window scale                       */
  uint8        scale_bkup;       /* Backup for original window scale       */
};
...
struct tcb_flags_s
{
  ...
  unsigned int cant_recv_more:1; /* Cannot receive any data for this conn  */
  ...
  unsigned int is_server:1;      /* TCP socket is client or server type    */
};
