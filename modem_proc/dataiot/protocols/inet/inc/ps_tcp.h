...
Copyright (c) 1995-2018 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/inet/inc/ps_tcp.h#2 $ $DateTime: 2023/01/31 04:42:27 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
11/08/19    ds     Added new parameters in total_keepalive_probes in TCB 
                   structure to keep track of number of keep alive probes.
...
/* API for updating TCP timer when we NTN(network) */
boolean set_ntn_timer(void);
boolean reset_ntn_timer(void);
...
struct tcb
{
  ...
  /*-------------------------------------------------------------------------
    Call when some info relevant to the upper layer changes
  -------------------------------------------------------------------------*/
  void (*info_change_upcall) ( void * scb_ptr,
                               struct ps_event_info_s * ps_event_info
                             );
  ...
  uint32 rcvcnt;                 /* Count of items on rcvq                 */ //DSS_PROTO_INT
