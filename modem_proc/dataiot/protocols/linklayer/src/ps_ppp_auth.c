...
Copyright (c) 1995-2019 QUALCOMM Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_ppp_auth.c#1 $
...
03/14/19    ds     Code changes to remove usage of MMGSDI API from linklayer.
...
===========================================================================*/
#define PS_PPP_AUTH_ASSERT(expression)  \
           ps_ppp_auth_assert_wrapper(__LINE__, expression)
...
#ifdef FEATURE_RUIM_FUNC_INTERFACE
#ifdef FEATURE_MMGSDI_SESSION_LIB
static ds_crit_sect_type  ppp_reg_crit_section;
#endif /* FEATURE_MMGSDI_SESSION_LIB */
#endif /* FEATURE_RUIM_FUNC_INTERFACE */
...
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
 ...
 FUNCTION ps_ppp_auth_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_AUTH_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_auth_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_AUTH_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_auth_assert_wrapper */
...
)
{
  ...
  if ( NULL == event ) 
  {
    /*"PPP Client Reg Error: Invalid input Parameter"*/
...
)
{
  if (NULL == event)
  {
    /*"ppp_mmgsdi_session_evt_cb: MMGSDI event callback "
    ...
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      LOG_MSG_INFO2_1("ppp_mmgsdi_session_evt_cb: Processing " 
              "MMGSDI_SUBSCRIPTION_READY_EVT for client_id %d", 
                      event->client_id);
      DS_ENTER_CRIT_SECTION (&ppp_reg_crit_section);
      ...
      DS_LEAVE_CRIT_SECTION (&ppp_reg_crit_section);
      break;
...
)
{
  ...
  if( NULL == cnf_ptr )
  {
    /*"ppp_mmgsdi_session_open_cb: MMGSDI session open "
...
)
{
  ...
  {
    /*("ppp_client_reg_cmd_hdlr: Invalid cmd")*/
    PS_PPP_AUTH_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( NV_DONE_S == ds_get_nv_item( NV_ESN_I, &ps_cave_chap_esn) )
  {
...
#ifdef FEATURE_MMGSDI_SESSION_LIB
  DS_INIT_CRIT_SECTION (&ppp_reg_crit_section);
...
)
{
  ...
  {
    /*( "Invalid arguments passed: device, config")*/
    PS_PPP_AUTH_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  case PPP_DO_EAP:
    ...
    if(ppp_cb_ptr->auth.flags & PPP_AP_REMOTE)
    {
      /*("EAP in Authenticator mode is not supported")*/
      PS_PPP_AUTH_ASSERT(0);
      return;        
    }
    else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*("cannot process CHAP for device!")*/
    PS_PPP_AUTH_ASSERT(0);
    return;
  }
  ...
  switch(auth_hdr.code)
  {
    case PPP_CHAP_CHALLENGE:
    {
      ...
      if( -1 == challenge_len )
      {
        /* "Malformed CHAP packet received" */
        DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
        dsm_free_packet(item_ref_ptr);
        return;
      }
      ...
      if(ppp_cb_ptr->auth.chap_challenge_ptr == NULL)
      {
        ...
        -------------------------------------------------------------------*/
        /* dev <device> chap_proc() alloc failed */
        DATA_PROTO_ERROR_LOG_1(PS_MEM_ALLOC_ERR,device);
        break;
      }

      ppp_cb_ptr->auth.chap_challenge_ptr->used = (uint16)challenge_len;
      PS_PPP_AUTH_ASSERT(ppp_cb_ptr->auth.chap_challenge_ptr->used <=
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"dev < ppp_cb_ptr->fsm[LCP].device> auth-ack mem alloc failed" */
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           auth_success,ppp_cb_ptr->fsm[LCP].device);
    return;
  }
  ...
  *insert_ptr++ = ppp_cb_ptr->auth.active_id; /* this was stored on rx */
  ...
  if (insert_ptr == NULL)
  {
    /*"authi_send_success():NULL ptr returned by put16"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    return;
  }
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"dev <ppp_cb_ptr->fsm[LCP].device> auth-req mem alloc failed"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           auth_req,ppp_cb_ptr->fsm[LCP].device);
    goto give_up;
  }
  ...
  *insert_ptr++ = ++ppp_cb_ptr->auth.active_id;
  ...
  if (insert_ptr == NULL)
  {
    /*"papi_send_auth_req(): NULL ptr returned by put16"*/
    DATA_PROTO_ERROR_LOG_0(PS_BAD_ARG_OR_NULL_PTR);
    goto give_up;
  }
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == cnf_ptr )
  {
    /*"chapi_uim_3gpd_calc_chap_resp_rpt: UIM 3GPD CHAP"
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {	
    ...
    *insert_ptr++ = PPP_CHAP_MD5_SIZE;
    ...
    {
      /*("chapi_calc_chap_md5_ruim_response: Incorrect Paramter values
         supplied to mmgsdi_session_compute_ip_auth algo")*/
      PS_PPP_AUTH_ASSERT(0);
      return NULL;
    }
    ...
    (void)ds_wait((rex_sigs_type)1 << PS_UIM_CMD_SIGNAL); /* Wait for completion*/
      ...
      { 
        ...
        else
        {
          ...
          nv_item.ds_sip_nai_info.index = 
                                 ppp_cb_ptr->auth.info.nai_entry_index;
          nv_status = ds_get_nv_item( NV_DS_SIP_NAI_INFO_I, 
                                      &nv_item );  
         /*-------------------------------------------------------------
          ...
          *response_len =  PPP_AUTH_CODE_SIZE
          ...
          {
            /*"dev <ppp_cb_ptr->fsm[LCP].device> CHAP resp mem alloc failed"*/
            DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                               response_ptr,ppp_cb_ptr->fsm[LCP].device);
            return NULL;
          }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    /*dev <ppp_cb_ptr->fsm[LCP].device> empty username  */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr->fsm[LCP].device); 
    dsm_free_packet(&(ppp_cb_ptr->auth.chap_challenge_ptr));
    return;
  }
  ...
  {
    /* "dev ppp_cb_ptr->fsm[LCP].device empty secret" */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr->fsm[LCP].device);
    dsm_free_packet(&(ppp_cb_ptr->auth.chap_challenge_ptr));
    return;
  }
  ...
  -------------------------------------------------------------------------*/
  if(ppp_cb_ptr->auth.info.is_pwd_set_by_client == FALSE)
  {
    /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"dev <ppp_cb_ptr->fsm[LCP].device> CHAP challenge mem alloc failed"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,
                           chal_ptr,ppp_cb_ptr->fsm[LCP].device);
    goto give_up;
  }
