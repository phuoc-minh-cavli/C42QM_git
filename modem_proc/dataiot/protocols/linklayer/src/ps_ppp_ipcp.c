...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_ppp_ipcp.c#3 $
  $Author: pwbldsvc $ $DateTime: 2021/01/07 02:55:54 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
06/19/20    sg     Code changes to fix IP address swap issue for subsequent 
                   DUN call.
...
===========================================================================*/
#define PS_PPP_IPCP_ASSERT(expression)  \
             ps_ppp_ipcp_assert_wrapper(__LINE__, expression)
...
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--*/
 ...
 FUNCTION ps_ppp_ipcp_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_IPCP_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_ipcp_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_IPCP_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_ipcp_assert_wrapper */
...
)
{
  ...
  fsm_ptr->fwk_input     = (ppp_fwk_input_f_type) ds_fwk_input;
  fsm_ptr->fwk_config    = NULL;
...
)
{
  ...
  if(ppp_cb_ptr == NULL)
  {
    /*("ipcp_start bad arg: NULL ppp_cb passed")*/
    PS_PPP_IPCP_ASSERT(0);
    return;
  }
...
//  ps_phys_link_type * phys_link_ptr;
  ...
  if(fsm_ptr->local.work & IPCP_N_1ADDRESS)
  {
    ...
    -----------------------------------------------------------------------*/
    fsm_ptr->fwk_inst_ptr->v4_net_info.ipv4_addr = lworkp->address.ps_s_addr; 
  }

  if((fwk_ptr != NULL) && (fsm_ptr->remote.work & IPCP_N_1ADDRESS))
  {
    fwk_ptr->v4_net_info.gateway.ps_s_addr = rworkp->address.ps_s_addr;
  }
#if 0
  /*-------------------------------------------------------------------------
...
#endif
  LOG_MSG_INFO1_2("Device(%d):fwk(0x%x) configured for IPv4",
                  device, fwk_ptr);
} /* ipcp_setup_device() */
...
)
{
  ...
  {
    /*"Invalid args in ipcp_check"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr,ohp);
      ...
      !((side_p->will | side_p->want) & (1 << n_type)))
  {
    ...
    {
      /*"Invalid option in ipcp_check"*/
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,used);
      return -1;
    }
      ...
      (ohp->type != IPCP_COMPRESS && ohp->length > option_length[n_type]))
  {
    ...
    /*"Invalid option length in ipcp_check"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,ohp->length,ohp->type);
    return CONFIG_NAK;
  }
  ...
  case IPCP_1ADDRESS:
    ...
    if(!request )
    {
      ...
      else if(swork_ptr->address.ps_s_addr == 0L)
      {
        ...
        {
          fwk_ptr = ppp_cb_ptr->fsm[IPCP].fwk_inst_ptr;
          swork_ptr->address.ps_s_addr = fwk_ptr->v4_net_info.ipv4_addr;
        }
      }
      break;
    } /* if(!request) */
  ...
  case IPCP_PRIMARY_DNS:
    ...
    -----------------------------------------------------------------------*/
    if ((fsm_ptr->device == PPP_RM_DEV || fsm_ptr->device == PPP_RM_2_DEV) &&
    swork_ptr->primary_dns.ps_s_addr != swant_ptr->primary_dns.ps_s_addr &&
  ...
  case IPCP_COMPRESS:
    ...
    case PPP_VJCH_PROTOCOL:
      ...
      if ( ohp->length > IPCP_COMPRESS_VJ_SIZ )
      {
        ...
        /*"Invalid option length in ipcp_check"*/
        DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ohp->length);
        return CONFIG_NAK;
      }
  ...
  if(used > 0 )
  {
    ...
    if( used != dsm_pullup(item_ref_ptr, NULL, used ) )
    {
      /*"Unable to toss extra bytes in option"*/
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,used);
    }
  }
...
)
{
  ...
  {
    /*"Invalid fsm_ptr in ipcp_opening"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
...
)
{
  ...
  {
    /*"Invalid fsm_ptr in ipcp_stopping"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
