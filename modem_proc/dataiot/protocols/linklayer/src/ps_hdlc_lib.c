...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_hdlc_lib.c#2 $
  $Author: pwbldsvc $ $DateTime: 2022/05/30 03:18:32 $
...
---------------------------------------------------------------------------*/
static ds_crit_sect_type  hdlci_hw_deframer_crit_sect;
static ds_crit_sect_type  hdlci_hw_framer_crit_sect;
...
)
{
  ...
  {
    LOG_MSG_ERROR_0("Allocated DSM item has NULL data_ptr");
    ASSERT(0);
    return -1;
  }
  /*-------------------------------------------------------------------------
...
)
{
  ...
  if(config == NULL)
  {
    LOG_MSG_ERROR_0("NULL Config passed to hdlc_init_config");
    ASSERT(0);
    return;
  }
...
)
{
  ...
  if(config == NULL)
  {
    LOG_MSG_ERROR_0("NULL Config passed to HDLC Alloc");
    ASSERT(0);
    return NULL;
  }
  ...
  if(instance == NULL)
  {
    LOG_MSG_ERROR_0("Unable to allocate memory for HDLC instance");
    ASSERT(0);
    return NULL;
  }
  ...
  {
    DS_INIT_CRIT_SECTION(&hdlci_hw_framer_crit_sect);
    DS_INIT_CRIT_SECTION(&hdlci_hw_deframer_crit_sect);
...
)
{
  ...
  if(hdlc_instance_is_valid(*instance))
  {
    ...
    {
      DS_DESTROY_CRIT_SECTION(&hdlci_hw_framer_crit_sect);
      DS_DESTROY_CRIT_SECTION(&hdlci_hw_deframer_crit_sect);
...
)
{
  ...
  LOG_MSG_INFO1_0("hdlc_reset(): Called");
  ULOG_RT_PRINTF_0(ds_fwk_get_ulog_handle(), "hdlc_reset(): Called");
  ...
  /*-------------------------------------------------------------------------
    ...
    do the framing and unframing function pointers (frame_f and unframe_f).
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1_1("hdlc_reset(): freeing the unframed pkt ptr 0x%x", instance->unframed_pkt_ptr );
  ULOG_RT_PRINTF_1(ds_fwk_get_ulog_handle(),"hdlc_reset(): freeing the unframed pkt ptr 0x%x",
                  instance->unframed_pkt_ptr );
  ...
  LOG_MSG_INFO1_0("hdlc_reset(): freeing the unframed pkt q" );
  ULOG_RT_PRINTF_0(ds_fwk_get_ulog_handle(),"hdlc_reset(): freeing the unframed pkt q");
  while( ( item_ptr =
