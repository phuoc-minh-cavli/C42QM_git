...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_ppp_fsm.c#2 $
  $Author: pwbldsvc $ $DateTime: 2020/08/06 23:37:13 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
08/06/20    sg     Code changes to correct the F3 message to log the
                   correct ID.
...
---------------------------------------------------------------------------*/
static ds_crit_sect_type  ppp_cb_lock[PPP_MAX_DEV];

#define PS_PPP_FSM_ASSERT(expression)  \
           ps_ppp_fsm_assert_wrapper(__LINE__, expression)
...
/*===========================================================================
 ...
 FUNCTION ps_ppp_fsm_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_FSM_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_fsm_assert_wrapper
 (
...
)
{
  ...
  LOG_MSG_INFO2_3( "fsm_send: dev=%d, code=%d, id=%d",
                 fsm_ptr->device, code, fsm_ptr->id );

#ifdef FEATURE_DATA_SEND_PPP_CTL_ON_DOS
  ppp_cb_ptr = ppp_cb_array + (uint8)fsm_ptr->device;
  if( NULL == ppp_cb_ptr )
  {
    /*"Invalid ppp_cb_ptr in fsm_send"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr);
    dsm_free_packet(&bp);
    return -1;
  }
#endif
...
)
{
  ...
  if( bpp  == NULL || *bpp == NULL )
  {
     ...
     DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,bpp);
     return -1;
  }
...
)
{
  ...
  if(reply_bp == NULL )
  {
    /* no dsm item for reply! */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,reply_bp);
    dsm_free_packet(&bp);
    return -1;
  }
  ...
  while (signed_length > 0  &&  ntohopt(&option, &bp) != -1)
  {
    ...
    {
      /*"fsm_request(): option length greater than packet length"*/
    ...
    if ( (remaining = option.length - OPTION_HDR_LEN) > 0 )
    {
      ...
      if ( option_bp == NULL )
      {
        /* fsm_request(): dsm_dup_packet failed */
        DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,option_bp);
        dsm_free_packet(&bp);
        dsm_free_packet(&reply_bp);
        return -1;
      }
    ...
    if ( (option_result = (*fsm_ptr->pdc->check)( fsm_ptr, &option,
        &bp, TRUE )) < 0 )
    {
      /* fsm_request(): option check failed */
      DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,option_result);
      dsm_free_packet(&bp);
      dsm_free_packet(&reply_bp);
  ...
  case ECHO_REPLY:
    if( PPP_LCP_PROTOCOL == fsm_ptr->pdc->protocol )
    {
      ...
      if( ppp_cb_ptr->lsd.config.result_cback != NULL )
      {
        ...
        PS_TIMER_FREE_HANDLE( ppp_cb_ptr->lsd.timer );
...
)
{
  ...
  if ( fsm_ptr == NULL                 ||
       ...
       fsm_ptr->device >= PPP_MAX_DEV )
  {
    /* Invalid args in fsm_passive_wakeup */
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
...
)
{
  ...
  {
    /*("fsm_down: One or more values are NULL")*/
    PS_PPP_FSM_ASSERT(0);
    return;
  }
...
)
{
  ...
  if ( fsm_ptr->pdc == NULL )
  {
    /*("NULL fsm_ptr->pdc for fsm for device")*/
    PS_PPP_FSM_ASSERT(0);
    return;
  }
