...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_ppp_lcp.c#1 $
...
===========================================================================*/
#define PS_PPP_LCP_ASSERT(expression)  \
              ps_ppp_lcp_assert_wrapper(__LINE__, expression)
...
/*===========================================================================
 ...
 FUNCTION ps_ppp_lcp_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_LCP_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_lcp_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_LCP_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_lcp_assert_wrapper */
...
)
{
  ...
  {
    /*("lcp_option(): One or more parameters null")*/
    PS_PPP_LCP_ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR_0("lcp_option(): One or more parameters null");
  ...
  if((-1 == n_type) || (sizeof(option_length) <= n_type))
  {
    ...
    dsm_free_packet(item_head_ptr);
    /* ("lcp_option : Unrecognized option"
                     " can't include!")*/
    PS_PPP_LCP_ASSERT(0);
    return;
  }
...
)
{
      ...
      (ohp->type != LCP_AP && ohp->length > option_length[n_type]))
  {
    ...
    /*"Invalid option length in lcp_check"*/
    DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,ohp->length,ohp->type,used);
    return CONFIG_NAK;
  }
  ...
  case LCP_AP: /* Authentication protocol */
    ...
    case PPP_PAP_PROTOCOL:
      ...
      if ( ohp->length > LCP_AP_PAP_OPTION_SIZE )
      {
        ...
        /*"Invalid option length in lcp_check"*/
        DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,ohp->length,used);
        return CONFIG_NAK;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"Invalid fsm_ptr in lcp_opening"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
...
)
{
  ...
  if(fsm_ptr == NULL || fsm_ptr->device >= PPP_MAX_DEV )
  {
    /*"Invalid fsm_ptr in lcp_closing"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,fsm_ptr);
    return;
  }
