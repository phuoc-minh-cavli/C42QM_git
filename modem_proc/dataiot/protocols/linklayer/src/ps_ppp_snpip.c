...
Copyright (c) 2003-2020 Qualcomm Technologies Incorporated.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_ppp_snpip.c#2 $ $DateTime: 2020/04/15 22:43:35 $ $Author: pwbldsvc $


when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/14/20    ds     Code changes for to support DUN+AP call concurrency.
04/08/19    ds     Memory optimization/cleanup related changes.
04/02/19    ds     Fixed Compiler warnings.
30/08/16    kc     Added changes for FR21288
...
= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =*/
#define PS_PPP_SNPIP_ASSERT(expression)  \
                ps_ppp_snpip_assert_wrapper(__LINE__, expression)
...
typedef struct
{
  ds_fwk_s_type      * if_ptr;            /* rx iface for this PPP dev     */
  void               * ev_buf_handle;        /* buffer for fltr_update ev cb  */
  ds_pdn_event_enum_type ds_pdn_ipfltr_event_reg_array[1]; /*  ev reg array  */
...
===========================================================================*/
 ...
 FUNCTION ps_ppp_snpip_assert_wrapper
 
 DESCRIPTION
   Wrapper function for PS_PPP_SNPIP_ASSERT
 ...
 ===========================================================================*/
 static void ps_ppp_snpip_assert_wrapper
 (
 ...
 )
 {
   if ( !expression )
   {
     ERR_FATAL("PS_PPP_SNPIP_ERR_FATAL at line:%d ",  
                                    line_num,0,0);  
   }
 }/* ps_ppp_snpip_assert_wrapper */ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_transition_state: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
#if 0
Deepak: revisit again
          if(NULL != snpipi_cb_arr[dev].ev_buf_handle)
          {
             if(-1 == ps_iface_unsubscribe_event_list(snpipi_cb_arr[dev].ev_buf_handle,
                                                      snpipi_cb_arr[dev].iface_ipfltr_event_reg_array,
                                                      1))
             {
               LOG_MSG_ERROR_0("Failed to unsubscribe event");
               ASSERT(0);
               return;
             }
          }
          ...
          /* free event handle of PPP dev */
          if(NULL != snpipi_cb_arr[dev].ev_buf_handle)
          {
            //ps_iface_free_event_handle(snpipi_cb_arr[dev].ev_buf_handle); 
            snpipi_cb_arr[dev].ev_buf_handle = NULL;
          }
          break;
    ...
    case SNOOP_IP_NONE_STATE:
      ...
      switch(snpipi_cb_arr[dev].state)
      {
        ...
        case SNOOP_IP_NULL_STATE:
          ...
          if(NULL == snpipi_cb_arr[dev].ev_buf_handle)
          {
            /*("ev_buf_handle is NULL");*/
           // PS_PPP_SNPIP_ASSERT(0);
            //return;
          }
#if 0
          snpipi_cb_arr[dev].ds_pdn_ipfltr_event_reg_array[0] = PS_IFACE_IPFLTR_UPDATED_EV; 

          if (-1 == ps_iface_subscribe_event_list(
                     snpipi_cb_arr[dev].ev_buf_handle,
                     snpipi_cb_arr[dev].iface_ipfltr_event_reg_array,
                     1))
          {
            /*("Event reg failed, cann't snoop IP");*/
            PS_PPP_SNPIP_ASSERT(0);
            return;
          }
#endif
      ...
      switch(snpipi_cb_arr[dev].state)
      {
        case SNOOP_IP_ALL_STATE:
...
#if 0
          snpipi_cb_arr[dev].iface_ipfltr_event_reg_array[0] = IFACE_IPFLTR_UPDATED_EV; 
          ...
          if (-1 == ps_iface_subscribe_event_list(
                     ...
                     1))
          {
            /*("Event reg failed, can't snoop IP");*/
            PS_PPP_SNPIP_ASSERT(0);
            return;
          }
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ppp_dev_up_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
  ...
  switch(snpipi_cb_arr[dev].state)
  {
    case SNOOP_IP_NULL_STATE:
      ...
      ---------------------------------------------------------------------*/
      snpipi_cb_arr[dev].if_ptr = ppp_cb_array[dev].fsm[IPCP].fwk_inst_ptr;
      if(DS_FWK_NUM_FILTERS(snpipi_cb_arr[dev].if_ptr,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ppp_dev_down_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ip_fltr_enabled_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ip_fltr_disabled_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ip_comp_on_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("snoop_ipi_process_ip_comp_off_ev: Invalid device");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
...
)
{
  ...
  ds_ipfltr_client_id_enum_type  client_id = FLTR_CLIENT_SOCKETS;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*( "Invalid arguments: device, protocol,"
                     " meta_info_ref_ptr NULL"); */
    PS_PPP_SNPIP_ASSERT(0);
    return SNOOP_CB_FORWARD_PKT;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(dev >= PPP_MAX_DEV)
  {
    /*("Invalid device, post event failed");*/
    PS_PPP_SNPIP_ASSERT(0);
    return;
  }
