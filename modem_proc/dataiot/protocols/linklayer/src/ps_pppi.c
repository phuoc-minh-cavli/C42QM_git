...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/protocols/linklayer/src/ps_pppi.c#6 $
...
05/22/19    ds     Code changes to add fwk instance validation before
                   sending packet.
...
===========================================================================*/
#define PS_PPPI_ASSERT(expression)  \
          ps_pppi_assert_wrapper(__LINE__, expression)
...
FUNCTION ps_pppi_assert_wrapper

DESCRIPTION
  Wrapper function for PS_PPPI_ASSERT
...
===========================================================================*/
static void ps_pppi_assert_wrapper
(
...
)
{
  if ( !expression )
  {
    ERR_FATAL("PS_PPPI_ERR_FATAL at line:%d ",  
                                   line_num,0,0);  
  }
}/* ps_pppi_assert_wrapper */
...
{                                                                           \
  ...
  {                                                                         \
    PS_PPPI_ASSERT( (param_ptr->framed_pkt_tail_ptr) != NULL );                     \
...
)
{
  ...
  for(loop = (uint8)PPP_MIN_DEV; loop < (uint8)PPP_MAX_DEV; loop += 1)
  {
    ...
    ppp_cb_array[loop].setup_timer = ps_timer_alloc( pppi_i_setup_timout_cb,
                                                     (void*)( loop ) );
    PS_PPPI_ASSERT(ppp_cb_array[loop].setup_timer != PS_TIMER_INVALID_HANDLE);
    ...
    {
      /*("Unable to allocate HDLC instance, dev")*/
      PS_PPPI_ASSERT(0);
      return;
    ...
    -----------------------------------------------------------------------*/
    DS_INIT_CRIT_SECTION( pppi_get_crit_sect_lock( (ppp_dev_enum_type)loop) );
...
)
{
  ...
  PS_PPPI_ASSERT( ppp_cb_ptr->hdlc_handle != NULL );
...
)
{
  ...
  -------------------------------------------------------------------------*/
  ULOG_RT_PRINTF_1(ds_fwk_get_ulog_handle(),"pppi_start(): "
                   "Now freeing the configbuf_ptr 0x%x", 
                   configbuf_ptr);
  PS_MEM_FREE( configbuf_ptr );
  ULOG_RT_PRINTF_1(ds_fwk_get_ulog_handle(),"pppi_start(): "
                   "After freeing, configbuf_ptr = 0x%x", 
                   configbuf_ptr);
...
)
{
  ...
  else
  {
    ...
    ppp_mode_info.mode   = (uint16)PPP_INTERNAL_MODE;
    pppi_set_mode( ppp_mode_info );
    ppp_cb_ptr->fsm[stop_info.protocol].fwk_inst_ptr = NULL;
...
)
{
...
#endif /* FEATURE_DATA_PS_IPV6 */
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  LOG_MSG_INFO1_0("pppi_abort(): Called");
  ULOG_RT_PRINTF_0(ds_fwk_get_ulog_handle(),"pppi_abort(): Called");
  ...
  ppp_cb_ptr->fsm[IPCP].fwk_inst_ptr= NULL;

#ifdef FEATURE_DATA_PS_IPV6
  ppp_cb_ptr->fsm[IPV6CP].fwk_inst_ptr = NULL;
#endif /* FEATURE_DATA_PS_IPV6 */
  ...
  PS_PPPI_ASSERT( ppp_cb_ptr->hdlc_handle != NULL );
  LOG_MSG_INFO1_0("pppi_abort(): Calling hdlc_reset() to free the hdlc handle");
  ULOG_RT_PRINTF_0(ds_fwk_get_ulog_handle(),
                   "pppi_abort(): Calling hdlc_reset() to free the hdlc handle");
...
)
{
  ...
  {
    /*("PPP %d could not set mode, no bridge dev")*/
    PS_PPPI_ASSERT(0);
  }

  PS_PPPI_ASSERT( PPP_IS_CURR_TASK_OWNER() );
  switch( mode_info.mode )
  {
    case PPP_RELAY_MODE:
      switch( old_mode )
      {
        ...
        default:
          /*("Invalid PPP mode transition, dev, mode")*/
          PS_PPPI_ASSERT(0);
          return;
      } /* switch( old_mode ) */
    ...
    case PPP_PSEUDO_NET_MODE:
      ...
      {
        /*("PPP dev, unable to set HDLC mode to software")*/
        PS_PPPI_ASSERT(0);
        return;
      }
      ...
      switch( old_mode )
      {
        ...
        case PPP_FULL_NET_MODE:
          ...
          PS_PPPI_ASSERT( ioctl_data.unframe_state == FINDING_7E  ||
    ...
    default:
      /*"Invalid PPP mode requested, dev(mode_info.device), old_mode "
       "new_mode" */
      DATA_PROTO_ERROR_LOG_3(PS_BAD_ARG_OR_NULL_PTR,
                      mode_info.device, old_mode, mode_info.mode);
      break;
  } /* switch( mode_info.mode ) */
...
)
{
...
#endif /* FEATURE_DATA_PS_PPP_IPHC */
#ifdef FEATURE_DATA_PS_PPP_SLHC
  ...
  {
    /*("Invalid dsm passed to pppi_outgoing")*/
    PS_PPPI_ASSERT(0);
    return -1;
  }
...
#endif /* FEATURE_DATA_PS_PPP_SLHC */
#ifdef FEATURE_DATA_PS_PPP_ROHC
       /*--------------------------------------------------------------------
  ...
  *cp++ = (protocol & 0x00ff);
                           ...
                           DSM_DS_SMALL_ITEM_POOL) < len)
  {
     ...
     dsm_free_packet(incoming_pkt_ptr);
     PS_TX_META_INFO_FREE(&meta_info_ptr);
     /*(" Running out of memory: dumping PPP frames")*/
     PS_PPPI_ASSERT(0);
     return -1;
  }
  ...
  if (is_tcp_ack)
  {
	...
	LOG_MSG_INFO1_1( "dev %d ppi outgoing >>>>", device);
...
)
{
  ...
  ds_fwk_s_type *fwk_inst_ptr = NULL;
#ifdef FEATURE_DATA_PS_PPP_SLHC
  ...
  {
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,item_ref_ptr);
    return;
  }

  if( device >= PPP_MAX_DEV )
  {
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
    dsm_free_packet(item_ref_ptr);
    PS_RX_META_INFO_FREE( &meta_info_ptr );
    return;
  }
  ...
  case PPP_EAP_PROTOCOL:
    ...
    case PPP_IP_PROTOCOL:
      ...
      else
      {
        ...
        /* Allocate Metainfo_ex buffer to pass RX metainfo_ptr */
          ...
          {
            //pppi_receive: Metainfo_ex pool out of memory 
          ...
          //if Meta info ptr is NULL then allocate one
          if (meta_info_ex_ptr->rx_meta_info_ptr == NULL)
          {
            PS_RX_META_INFO_GET(meta_info_ex_ptr->rx_meta_info_ptr);
            if (NULL == meta_info_ex_ptr->rx_meta_info_ptr)
            {
              dsm_free_packet(item_ref_ptr);
              LOG_MSG_ERROR_0("pppi_receive: Failed to get rx meta info buf");
...
#ifdef FEATURE_DATA_PS_464XLAT
      /* Get the Fwk pointer  */
      fwk_inst_ptr = ppp_cb_ptr->fsm[IPCP].fwk_inst_ptr;
        LOG_MSG_INFO1_1("pppi_receive(): fwk_inst_ptr = 0x%x,",fwk_inst_ptr);
        if(FALSE == ds_fwk_inst_is_valid_ex(fwk_inst_ptr,IPV4_ADDR))
        {
          LOG_MSG_INFO1_0("pppi_receive(): Invalid Fwk Instance.");
          dsm_free_packet(item_ref_ptr);
          PS_RX_META_INFO_FREE(&meta_info_ptr);
          PS_META_INFO_FREE_EX(&meta_info_ex_ptr);
          return;
        ...
        if(fwk_inst_ptr->clat_pdn_cntxt != NULL )
        {
          clat_pdn_context = (ds_fwk_clat_pdn_cntxt_info_type*)fwk_inst_ptr->clat_pdn_cntxt;
          LOG_MSG_INFO1_1("pppi_receive(): clat_pdn_context = 0x%x,",clat_pdn_context);
          LOG_MSG_INFO1_1("pppi_receive(): clat_pdn_context->client_pdn_handle = 0x%x,",clat_pdn_context->client_pdn_handle);
          instance = (void *)clat_pdn_context->client_pdn_handle;
          LOG_MSG_INFO1_1("pppi_receive(): instance = 0x%x,",instance);
          ...
          (void)dsm_extract(*item_ref_ptr, dsm_offset, buf, 2*sizeof(uint16));
          src_port = *((uint16*)&buf[0]);
          dst_port = *((uint16*)&buf[2]);
          LOG_MSG_INFO1_2("pppi_receive(): src port = 0x%x, dst port = 0x%x,",src_port, dst_port);
          if(dst_port == PS_DNS_PROXY_SERVER_PORT)
          {
             clat_sm_ptr  = (ps_clat_sm_type*)instance;
             LOG_MSG_INFO1_1("pppi_receive(): clat_sm_ptr = 0x%x,",clat_sm_ptr);
          ...
          if(-1 == ps_clat_ip_to_ip6(item_ref_ptr, fwk_inst_ptr, instance ))
          {
      ...
      /* Get the Fwk pointer  */
      fwk_inst_ptr = ppp_cb_ptr->fsm[IPCP].fwk_inst_ptr;
      if(ds_fwk_inst_is_valid_ex(fwk_inst_ptr,IPV4_ADDR) || 
         (fwk_inst_ptr->is_clat_capable && 
         ds_fwk_inst_is_valid_ex(fwk_inst_ptr,IPV6_ADDR)))
      {
        (void)ppp_cb_ptr->fsm[IPCP].fwk_input
              (
                fwk_inst_ptr,
                item_ref_ptr,
                meta_info_ex_ptr
      ...
      else /*fwk_inst is invalid, free the pkt*/
      {
        dsm_free_packet(item_ref_ptr);
        PS_RX_META_INFO_FREE(&meta_info_ptr);
        PS_RX_META_INFO_FREE(&(meta_info_ex_ptr->rx_meta_info_ptr));
        PS_META_INFO_FREE_EX(&meta_info_ex_ptr);
        DS_MSG0(MSG_LEGACY_HIGH,"Invalid v4 FWK instance..Freeing packet");
        return;
      }
    }
    break;
    ...
    case PPP_IPV6_PROTOCOL:
      ...
      /* Get the Fwk pointer  */
      fwk_inst_ptr = ppp_cb_ptr->fsm[IPV6CP].fwk_inst_ptr;
      if(ds_fwk_inst_is_valid_ex(fwk_inst_ptr,IPV6_ADDR))
      {
        (void)ppp_cb_ptr->fsm[IPV6CP].fwk_input
              (
                fwk_inst_ptr,
                item_ref_ptr,
                meta_info_ex_ptr
      ...
      else
      {
        ...
        PS_RX_META_INFO_FREE(&meta_info_ptr);
        PS_META_INFO_FREE_EX(&meta_info_ex_ptr);
        DS_MSG0(MSG_LEGACY_HIGH,"Invalid v6 FWK instance..Freeing packet");
        return;
      }
    }
    break;
    ...
    case PPP_ROHC_LCID_PROTOCOL:
      ...
      else
      {
        ...
        uint32 ncp_ver;
        ip_addr_enum_type       ip_ver = IP_ADDR_INVALID;
        ...
        ncp_ver = IPCP;
#endif
        ip_ver = (ncp_ver == IPCP) ? IPV4_ADDR : IPV6_ADDR;
        /* Get the Fwk pointer  */
        fwk_inst_ptr = ppp_cb_ptr->fsm[ncp_ver].fwk_inst_ptr;
        if(ds_fwk_inst_is_valid_ex(fwk_inst_ptr, ip_ver)
        {
          (void)ds_fwk_input( fwk_inst_ptr,
                              item_ref_ptr, NULL);
        }
        else
        {
          LOG_MSG_ERROR_0("pppi_recieve(): Invalid Fwk Instance");
          dsm_free_packet(item_ref_ptr);
...
)
{
  ...
  if( device >= PPP_MAX_DEV )
  {
    /*"Invalid dev <device> in pppi_discard"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
...
)
{
  ...
  if( device >= PPP_MAX_DEV )
  {
    /*Invalid args in pppi_send_prot_rej"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
    dsm_free_packet(&item_ptr);
    return FALSE;
  }
...
)
{
  ...
  if(NULL == fsm_ptr)
  {
    /*("pppi_call_event_callback: fsm_ptr is NULL")*/
    PS_PPPI_ASSERT(0);
    return;
  }
  ...
  if (PPP_MAX_DEV <= fsm_ptr->device 
      ...
      || PPP_MAX_EV <= event)
  {
    /*( "Invalid args passed. device, protocol, event")*/
    PS_PPPI_ASSERT(0);
    return;
  }
  ...
  DS_ENTER_CRIT_SECTION( pppi_get_crit_sect_lock(fsm_ptr->device) );
  ...
  while( NULL != event_ptr )
  {
    switch(protocol)
    {
      ...
      default:
        ...
        DS_LEAVE_CRIT_SECTION( pppi_get_crit_sect_lock(fsm_ptr->device) );
        /*("Invalid protocol")*/
        PS_PPPI_ASSERT(0);
        return;
...
)
{
  ...
  if( PPP_MAX_DEV <= device ||
      PPP_PROTOCOL_MAX <= protocol )
  {
    /*( "Invalid args passed. device, protocol")*/
    PS_PPPI_ASSERT(0);
    return;
  }
...
)
{
  ...
  {
    /*"Invalid DSM"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,framed);
    return;
  }

  if( device >= PPP_MAX_DEV )
  {
    /*"Invalid device in internal mode hdlr"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
  ...
  ppp_cb_array = fsm_get_ppp_cb_array();
  ppp_cb_ptr = &(ppp_cb_array[device]);
  PS_PPPI_ASSERT( ppp_cb_ptr->mode == PPP_INTERNAL_MODE ||
...
)
{
  ...
  if( device >= PPP_MAX_DEV )
  {
    /*"Invalid dev in pseudonet fwd data"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
    return;
  }
  ...
  if(ppp_cb_ptr->bridge_dev >= PPP_MAX_DEV)
  {
    /*"Invalid bridge dev in pseudonet fwd data"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,ppp_cb_ptr->bridge_dev);
    return;
  }
...
)
{
  ...
  PS_PPPI_ASSERT( ( in_pkt_ptr != NULL ) && ( (*in_pkt_ptr) != NULL ) );
  ...
  PS_PPPI_ASSERT( read_ptr <= end_ptr );
  while( *in_pkt_ptr != NULL )
  {
    ...
    PS_PPPI_ASSERT( *read_ptr == HDLC_FLAG );
    found_end_of_packet = TRUE;
    break;
  } /* while( *in_pkt_ptr != NULL ) */
...
)
{
  ...
  PS_PPPI_ASSERT( ppp_cb_ptr->pending_mode == PPP_INTERNAL_MODE ||
...
)
{
  ...
  {
    /*"Invalid DSM"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,in_pkt_ptr);
    return;
  }

  if( device >= PPP_MAX_DEV )
  {
    /*"Invalid dev in pseudonet mode hdlr"*/
    DATA_PROTO_ERROR_LOG_1(PS_BAD_ARG_OR_NULL_PTR,device);
  ...
  while( *in_pkt_ptr != NULL )
  {
    ...
    {
      case FINDING_7E:
        LOG_MSG_INFO3_1("Dev %d Pseudonet in FINDING_7E state", device);
        for( ; *read_ptr != HDLC_FLAG && read_ptr < end_ptr ; read_ptr++ )
        {
          /* NOOP */
        }
      ...
      case FINDING_SOP:
        LOG_MSG_INFO3_1("Dev %d Pseudonet in FINDING_SOP state", device);
        /*--------------------------------------------------------------------
        ...
        -------------------------------------------------------------------*/
        PS_PPPI_ASSERT( param_ptr->snoop_pkt_ptr == NULL );
        /* fall through */

      case SNOOPING:
        LOG_MSG_INFO3_1("Dev %d Pseudonet in SNOOPING state", device);
      ...
      case FINDING_EOP:
        LOG_MSG_INFO3_1("Dev %d Pseudonet in FINDING_EOP state", device);
        ...
        PS_PPPI_ASSERT( param_ptr->state == FINDING_SOP ||
                param_ptr->state == FINDING_EOP );
        continue;

      case UNFRAMING:
        LOG_MSG_INFO3_1("Dev %d Pseudonet in UNFRAMING state", device);
        /*-----------------------------------------------------------------
        ...
        PS_PPPI_ASSERT( param_ptr->state == FINDING_7E  ||
                param_ptr->state == FINDING_SOP ||
                param_ptr->state == UNFRAMING );
        continue;

      default:
        PS_PPPI_ASSERT(0);  /* Sanity check for param_ptr->state */
        break;
...
)
{
  ...
  if(NULL == fsm_ptr)
  {
    /*("pppi_fsm_call_event_callback: fsm_ptr is NULL")*/
    PS_PPPI_ASSERT(0);
    return;
  }
  ...
  {
    /*( "Invalid args passed. device, event")*/
    PS_PPPI_ASSERT(0);
    return;
  }
  /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*"PPP device/dsm item is NULL"*/
    DATA_PROTO_ERROR_LOG_2(PS_BAD_ARG_OR_NULL_PTR,device_ptr,dsm_item_ptr);
    return -1;
  }
