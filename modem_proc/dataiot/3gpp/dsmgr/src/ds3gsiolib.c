...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/3gpp/dsmgr/src/ds3gsiolib.c#7 $ $DateTime: 2022/10/06 21:19:58 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
02/15/21   ks      Reverting the fix for flushing queued URCs to allow bootup URCs.
11/20/17   skc     Fixed issue while flushing queued URCs.
06/13/17   skc     Initializing sio open parameter.
05/15/17   skc     Fixed issue with deasserting DSR at DTR Low.
05/01/17   skc     Fixed escape sequence detection handling for
                   multi-port configuration.
08/19/16   skc     Fixed issue related to cd assert for DUN call.
...
static ULogHandle ds3gsiolib_ulog_handle = NULL;
...
static  void ds3g_siolib_rts_changed_cb
( 
  void* cb_data 
);
...
ds3g_siolib_timer_s_type ds3g_siolib_timer_table[]=
{
  {DS3G_SIOLIB_TIMER_WINK_DCD_1, ds3g_siolib_timer_cb, DS3G_SIOLIB_DCD_WINK_INTERVAL, 0},
  {DS3G_SIOLIB_TIMER_WINK_DCD_2, ds3g_siolib_timer_cb, DS3G_SIOLIB_DCD_WINK_INTERVAL, 0}
};
...
/*===========================================================================
FUNCTION DS3GSIOLIB_GET_ULOG_HANDLE
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle ds3gsiolib_get_ulog_handle()
{
  return ds3gsiolib_ulog_handle;
}

/*===========================================================================
FUNCTION DS3GSIOLIB_ULOG_REGISTERr
...
SIDE EFFECTS
  None
===========================================================================*/
void ds3gsiolib_ulog_register(void)
{
  ULogFront_RealTimeInit(&ds3gsiolib_ulog_handle, 
                               "DS3GSIOLIB_REG_ULOG",	  
                               8192,
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  //LOG_MSG_INFO1_1("QMI DMSI PSM ULOG REGISTER Start",result);
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds3g_siolib_timer_enum_type));
  ...
  DS_SIO_MSG1_HIGH("Timer Id %d expired", (ds3g_siolib_timer_enum_type*)cb_data);
  cmd_ptr->cmd_payload_ptr = cb_data;
  cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_TIMER_EXPIRED;
  ds_put_cmd( cmd_ptr );
...
/*===========================================================================
FUNCTION      DS3G_SIOLIB_PROCESS_TIMER_EXPIRED_CMD

DESCRIPTION   This function processes the DS_CMD_SIOLIB_TIMER_EXPIRED. It determines 
...
SIDE EFFECTS  None
===========================================================================*/
void  ds3g_siolib_process_timer_expired_cmd
(
...
)
{
  ...
  ds3g_siolib_timer_enum_type   timer_id   = DS3G_SIOLIB_TIMER_MIN;
  uint32                        local_error =  0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_TIMER_EXPIRED)
    {
      local_error = 2;
      break;
    }
    ...
    -------------------------------------------------------------------------*/
    timer_id = *(ds3g_siolib_timer_enum_type*)(cmd_ptr->cmd_payload_ptr);

    if ((timer_id < DS3G_SIOLIB_TIMER_MIN)||(timer_id >=DS3G_SIOLIB_TIMER_MAX))
    {
      local_error = 3;
    }
    ...
    switch(timer_id)
    {
      case DS3G_SIOLIB_TIMER_WINK_DCD_1:
        port_index = DS3G_SIOLIB_PORTID_0;
        break;
      case DS3G_SIOLIB_TIMER_WINK_DCD_2:
    ...
    ret_val = ds3g_siolib_ex_set_cd_state( DS3G_SIOLIB_CD_WINKING_END, port_id );
  ...
  if(local_error != 0)
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_ERROR, 
		 "ds3g_siolib_process_timer_expired_cmd() returning with local_error %d", local_error);
  }
  return;
} /* ds3g_siolib_process_timer_expired_cmd() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if((port_id > DS3G_SIOLIB_PORT_NONE) && (port_id < DS3G_SIOLIB_PORT_MAX))
  {
    ...
    (void) rex_set_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_DATA_PORT_1ST_SIO_RX_SIG));
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERR_FATAL("input port is DS3G_SIOLIB_PORT_NONE");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(post_cmd == TRUE)
  {
    ...
    cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_DSM_MEM_CTRL;
    dsm_mem_ctrl_ptr 
    ...
    DS_SIO_MSG1_HIGH( "post DS task cmd:%d", DS_CMD_SIOLIB_DSM_MEM_CTRL );
    ds_put_cmd( cmd_ptr ); 
  }
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_DSM_MEM_CTRL)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_DTR_CHANGED;
  dtr_changed_cmd_ptr 
...
FUNCTION DS3G_SIOLIB_RTS_CHANGED_CB

DESCRIPTION
  Callback function registered with SIO, used to inform DS of any changes in
  the state of the RTS signal on any port

  This function simply posts a DS_RTS_EVENT_CMD to DS.  /@SANJAYKC: are you going to do
...
===========================================================================*/
LOCAL void ds3g_siolib_rts_changed_cb( void* cb_data )
{
  ds_command_type                *cmd_ptr = NULL;
  ds3g_siolib_rts_chg_cmd_type   *rts_changed_cmd_ptr = NULL;
  ds3g_siolib_port_e_type         port = (ds3g_siolib_port_e_type)cb_data;
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if(port > DS3G_SIOLIB_PORT_MAX || port <= DS3G_SIOLIB_PORT_NONE)
  {
    DS_SIO_MSG1_ERROR("RTS CB on invalid port %d", port);
    return;
  }

  cmd_ptr = ds_allocate_cmd_buf(sizeof(ds3g_siolib_rts_chg_cmd_type));
  if( (cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL) )
  {
    ASSERT(0);
    return;
  ...
  dsati_sio_set_rts_changed_cb(port, TRUE);

  cmd_ptr->hdr.cmd_id = DS_CMD_ATCOP_SIO_RTS_CHANGED_CMD;
  rts_changed_cmd_ptr 
    = (ds3g_siolib_rts_chg_cmd_type*)cmd_ptr->cmd_payload_ptr;

  rts_changed_cmd_ptr->port_id = (int)port;
  ds_put_cmd( cmd_ptr );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_CD_CHANGED;
...
DESCRIPTION
  ...
  the escape code (+++) is detected (in Rawdata Mode or Packet Mode).

  This function simply sets the DS_SIO_ESCAPE_SIG and enable the flag
...
===========================================================================*/
#define DS3G_SIOLIB_ESCAPE_DETECTED_CB(port_index) \
void  ds3g_siolib_escape_detected_cb_ ## port_index \
( \
  void \
) \
{ \
  ds3g_siolib_port_e_type port_id = ds3g_siolib_port_signal_status[port_index].port_id; \
  ds3g_siolib_state_info_type *state = NULL; \
  if((port_id > DS3G_SIOLIB_PORT_NONE) && (port_id < DS3G_SIOLIB_PORT_MAX)) \
  { \
    state = &ds3g_siolib_state[port_id]; \
    state->serial_info.esc_seq_detected = TRUE; \
    (void)rex_set_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_SIO_ESCAPE_SIG) ); \
  } \
  else \
  { \
...
/* Callback function for DS3G_SIOLIB_PORTID_0 */
DS3G_SIOLIB_ESCAPE_DETECTED_CB(0)

/* Callback function for DS3G_SIOLIB_PORTID_1 */
DS3G_SIOLIB_ESCAPE_DETECTED_CB(1)
...
{
  (void) rex_set_sigs(ds_get_ds_task_ptr(), GET_SIG(DS_DTR_EVENT_SIG));
        ...
        dsm_pullup(&item_ptr, &port_id, sizeof(ds3g_siolib_port_e_type)))
    {
      if (DS3G_SIOLIB_OK == ds3g_siolib_ex_is_dtr_asserted(&dtr_status, port_id))
      {
        ...
        if (TRUE == dtr_status)
        {
             ...
             dsat_vend_psm_image_indication(DSAT_PSM_IMAGE_BOOT_FULL);
...
void ds3g_siolib_init ( void )
{
  ...
  if(ds3gsiolib_get_ulog_handle() == NULL)
  {
    ds3gsiolib_ulog_register();
  }

  if(ds3gsiolib_get_ulog_handle() != NULL)
  {
     ULOG_RT_PRINTF_0(ds3gsiolib_get_ulog_handle(),
         "ds3g_siolib_init()");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 ------------------------------------------------------------------------*/
  ...
  if(ds3gsiolib_get_ulog_handle() != NULL)
  {
     ULOG_RT_PRINTF_1(ds3gsiolib_get_ulog_handle(),
        "Registered call back with RDM with rdm_ret %d", rdm_ret);
  }
...
FUNCTION DS3G_SIOLIB_INIT_TIMERS
   
DESCRIPTION
  This function performs allocation of timers used in DS3GSIOLIB.
...
===========================================================================*/
void ds3g_siolib_init_timers()
{
  ds3g_siolib_timer_enum_type timer_idx;
  ds3g_siolib_timer_s_type    *t_ptr = NULL;
  /*-------------------------------------------------------------------------
   Initialize and Register Wink DCD expiration callbacks.
   -------------------------------------------------------------------------*/
  for(timer_idx = DS3G_SIOLIB_TIMER_MIN; timer_idx < DS3G_SIOLIB_TIMER_MAX; timer_idx++ )
  {
    t_ptr = &ds3g_siolib_timer_table[timer_idx];
    t_ptr->timer_handle = ps_timer_alloc(ds3g_siolib_timer_cb, (void*)(&t_ptr->timer_id));
  }
}
...
#ifdef FEATURE_DATA_ATCOP
  open_param.rx_bitrate = (sio_bitrate_type)(dsat_num_item_type)dsatutil_get_val(DSAT_EXT_IPR_IDX,0,NUM_TYPE);
  open_param.tx_bitrate = (sio_bitrate_type)(dsat_num_item_type)dsatutil_get_val(DSAT_EXT_IPR_IDX,0,NUM_TYPE);
#endif /*FEATURE_DATA_ATCOP*/
  ...
  else
  {
    ...
    switch(port_id) 
    {
      ...
      case SIO_PORT_USB_CSVT: 
        DS_SIO_MSG3_MED("ds3g_siolib_open port_id : %d open_port %d", port_id, open_port, 0);
        ...
        /* Register for RTS event with DS3G_SIO port */
        ioctl_param.enable_rts_ready_event_ext.cb_func = ds3g_siolib_rts_changed_cb;
        ioctl_param.enable_rts_ready_event_ext.cb_data = (void*)open_port;

        sio_ioctl( state->serial_info.stream_id,
                   SIO_IOCTL_ENABLE_RTS_EVENT_EXT,
                   &ioctl_param );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&open_param, 0, sizeof(sio_open_type));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      (state->serial_info.close_in_progress == FALSE) )
  {
    ...
    DS_SIO_MSG1_HIGH("CD Asserted on port %d", port_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      (state->serial_info.close_in_progress == FALSE) )
  {
    ...
    DS_SIO_MSG1_HIGH("CD Deassert on port %d",port_id);
    ret_val = DS3G_SIOLIB_OK;
  }
  else
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     ds3g_siolib_active_port == DS3G_SIOLIB_PORT_NONE)
  {
    ...
    DS_SIO_MSG1_MED("calling dsat_process_sio_command on port %d", port_id);
...
===========================================================================*/
static void ds3g_siolib_handle_sio_escape_sig 
(
  ds3g_siolib_port_e_type port_id
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_COMPLETE_ONLINE_CMD_SWITCH;
...
===========================================================================*/
#define DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB_FUNC(port_index) \
        ds3g_siolib_signal_complete_online_data_switch_cb_ ## port_index


#define DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB(port_index) \
static void ds3g_siolib_signal_complete_online_data_switch_cb_ ## port_index \
(void) \
{ \
  ds_command_type *cmd_ptr; \
  ds3g_siolib_state_info_type *state; \
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ \
  ...
  if ( (cmd_ptr = ds_allocate_cmd_buf(DS_CMD_PAYLOAD_SIZE_ZERO)) != NULL ) \
  { \
    /* We are here as flush has been already done, so setting no wait for flush */ \
    \
    ...
    \
    cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_COMPLETE_ONLINE_DATA_SWITCH; \
...
DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB(0)

DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB(1)
...
)
{
  ...
  ds3g_siolib_state_info_type *state;
  ds3g_siolib_status_e_type ret_val;
  ds_call_type_e_type call_type = ds3g_siolib_ex_get_current_call_type(port_id);
  ds3g_siolib_portid_e_type port_index;

  port_index = ds3g_siolib_get_index_from_allocated_port(port_id);
  
  if(port_index == DS3G_SIOLIB_PORTID_NONE ||
     port_index == DS3G_SIOLIB_PORTID_MAX)
  {
    DATA_ERR_FATAL(" InValid Port Index");
    return ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (state->at_state == DS3G_SIOLIB_ONLINE_CMD_AT_STATE)
  {
    ...
    if (flush_tx)
    {
      ...
      if(port_index == 1)
      {
        ret_val = ds3g_siolib_ex_flush_tx(DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB_FUNC(1), port_id);
      }
      else
      {
        ret_val = ds3g_siolib_ex_flush_tx(DS3G_SIOLIB_SIGNAL_COMPLETE_ONLINE_DATA_SWITCH_CB_FUNC(0), port_id);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_DTR_CHANGED)
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "Invalid cmd: %d", cmd_ptr->hdr.cmd_id);
    return;
  }
  ...
  if(!ds3g_siolib_port_is_in_active_call(port_id))
  {
    ...
    else
    {
      ...
      else
      {
        ...
        /* DSR should be deasserted only when &S is 1. */
#ifdef FEATURE_DATA_ATCOP
        if ( (dsat_num_item_type)dsatutil_get_val(
                                     DSAT_BASIC_N_S_IDX,0,NUM_TYPE) == DS3G_DSR_CONNECT_ON )
        {
          (void)ds3g_siolib_dsr_deassert(port_id);
          DS_SIO_MSG0_MED("AT&S= 1; DSR DeAsserted at DTR Low");
        }
#endif /* FEATURE_DATA_ATCOP */
      }
#ifdef FEATURE_IRDA
  ...
  if (ret_val == DS3G_SIOLIB_OK)
  {
    if (dtr_status == FALSE) 
    {
#ifdef FEATURE_DATA_ATCOP    
      if ((dsat_num_item_type)dsatutil_get_val(
                                       DSAT_BASIC_N_D_IDX,0,NUM_TYPE) == 0)
      {
        ...
        else
        {
          ...
          return;
                                       ...
                                       DSAT_BASIC_N_D_IDX,0,NUM_TYPE) == 1) && 
               (state->at_state == DS3G_SIOLIB_ONLINE_DATA_AT_STATE))
      {
        ...
        if((hdlr_tbl_ptr->at_escape_sequence_ex_handler != NULL &&
          ...
          else
          {
            ...
            return;
                                       ...
                                       DSAT_BASIC_N_D_IDX,0,NUM_TYPE) == 2)
      {
      ...
      {
        DS_SIO_MSG1_ERROR("Invalid AT&D value %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_CD_CHANGED)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (set_sigs & GET_SIG(DS_DATA_PORT_1ST_SIO_RX_SIG)) != 0 )
  {
    ...
    ret_val = rex_clr_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_DATA_PORT_1ST_SIO_RX_SIG) );
    if(!(ret_val & GET_SIG(DS_DATA_PORT_1ST_SIO_RX_SIG)))
    {
  ...
  if( (set_sigs & GET_SIG(DS_DTR_EVENT_SIG)) != 0 )
  {
    DS_SIO_MSG0_MED( "Rx'ed DS_DTR_EVENT_SIG");
    ret_val = rex_clr_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_DTR_EVENT_SIG) );
    if(!(ret_val & GET_SIG(DS_DTR_EVENT_SIG)))
    {
      DS_SIO_MSG0_MED("DS_DTR_EVENT_SIG was not active");
    }
    ds3g_siolib_handle_dtr_event_sig();
  }
  if( (set_sigs & GET_SIG(DS_SIO_RX_PKT_SIG)) != 0 )
  {
    DS_SIO_MSG0_MED( "Rx'ed SIO_RX_PKT_SIG");
    ret_val = rex_clr_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_SIO_RX_PKT_SIG) );
    if(!(ret_val & GET_SIG(DS_SIO_RX_PKT_SIG)))
    {
      DS_SIO_MSG0_MED("DS_SIO_RX_PKT_SIG was not active");
    }
  }

  if( (set_sigs & GET_SIG(DS_SIO_ESCAPE_SIG)) != 0 )
  {
    ds3g_siolib_port_e_type port_id;
    DS_SIO_MSG0_MED( "Rx'ed SIO_ESCAPE_SIG" );
    ret_val = rex_clr_sigs( ds_get_ds_task_ptr(), GET_SIG(DS_SIO_ESCAPE_SIG) );
    if(!(ret_val & GET_SIG(DS_SIO_ESCAPE_SIG)))
    {
      DS_SIO_MSG0_MED("DS_SIO_ESCAPE_SIG was not active");
    }    
    for ( port_id = DS3G_SIOLIB_UART1_PORT; port_id < DS3G_SIOLIB_PORT_MAX; ++port_id )
    {
      if ( ds3g_siolib_state[port_id].serial_info.esc_seq_detected == TRUE )
      {
        ds3g_siolib_state[port_id].serial_info.esc_seq_detected = FALSE;
        ds3g_siolib_handle_sio_escape_sig(port_id);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    ULOG_RT_PRINTF_1(ds3gsiolib_get_ulog_handle(),
        "ds3g sio process cmd id [%d]", cmd_ptr->hdr.cmd_id);
  }
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
    ...
    case DS_CMD_SIOLIB_ENTER_ONLINE_DATA_TX_FLUSH:
      ds3g_siolib_process_enter_online_data_mode( TRUE,
	       (ds3g_siolib_port_e_type)cmd_ptr->cmd_payload_ptr);
      break;
    ...
    case DS_CMD_SIOLIB_TIMER_EXPIRED:
      ds3g_siolib_process_timer_expired_cmd( cmd_ptr );
      break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_CLOSE_COMPLETE;
  ds_put_cmd( cmd_ptr );
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  if( ((dsat_num_item_type)dsatutil_get_val(
                                     DSAT_BASIC_N_C_IDX,0,NUM_TYPE) == 0) || 
       ((dsat_num_item_type)dsatutil_get_val(
                                     DSAT_BASIC_N_C_IDX,0,NUM_TYPE) == 2))
  {
    dcd_state = ON;
  }
  /*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
                                     ...
                                     DSAT_BASIC_N_C_IDX,0,NUM_TYPE) == 1 )
  {
    dcd_state = OFF;
  }
#endif /* FEATURE_DATA_ATCOP */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 ------------------------------------------------------------------------*/     
            ...
            port_count < DS3G_SIOLIB_PORT_MAX; port_count++) 
      {
            ...
            ( dtr_status == TRUE) )
        {
          ...
          else
          {
            ...
            /* Send URC only to unblocked port; if blocked discard the URC for that port */
            if (TRUE == dsatcmdp_is_sio_info_inited(port_count))
            {
              ...
              DS_SIO_MSG3_HIGH("port %d block_indications_flag %d sio_info_ptr 0x%x",
                               port_count, block_indications_flag,
                               sio_info_ptr);
            }
            ...
            {
              dsatcmdp_add_urc_to_queue(port_count, tmp_ptr);
            }
            else
            {
              /* In case of multi-port configuration, one of the port may be Active.
                 Sending URC may effect the processing of AT Command on another port.
                 So we are restricting to send URC in case if there is any active and
                 queuing the same */
              ...
              else if (sio_info_ptr)
              {
                /* Queue URC only when atleast one AT command has been run on this port
                 * otherwise discard the URC
                 */
                dsatcmdp_add_urc_to_queue(port_count, tmp_ptr);
              }
              else
  ...
  default:
   ...
   /* When a URC is generated and AT command is already in process, check for active port.
      Call the service ports only when the active port is none */
  if( (DS3G_SIOLIB_UNSOLICTED_FAX_RESULT_CODE != tx_type) && 
      ((rearm_autodetect && (ds3g_siolib_active_port == DS3G_SIOLIB_PORT_NONE)) || 
...
/*===========================================================================
FUNCTION DS3G_SIOLIB_GET_TX_TYPE

DESCRIPTION
  This function gets the tx_type for unsolicited data.
  the tx_type is used in ds3g_siolib_setup_tx() while doing tx to SIO.
  We use tx_type to process unsolicited data such as RSSI, SMS...etc.
  Function call also specifies the port to which the unsolicited data
  has to be sent.
...
ds3g_siolib_tx_e_type ds3g_siolib_get_tx_type()
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  return ds3g_siolib_tx_type;
} /* ds3g_siolib_get_tx_type() */
