...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/3gpp/dsmgr/src/ds3gsiolib_ex.c#2 $ $DateTime: 2022/05/13 03:58:57 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERR_FATAL("NULL passed for alloc_info_ptr");
    return return_port;
  }  
  ...
  for(i=0; i<DS3G_SIOLIB_PORTID_MAX; i++)
  {
    ...
    if( DS3G_SIOLIB_PORT_NONE == ds3g_siolib_port_allocated[i] )
    {
      ...
      if( DS3G_SIOLIB_AUTODETECT_SERIAL_STATE == state->serial_info.serial_state )
      {
        ...
        ds3g_siolib_port_signal_status[i].port_id = port_id;
        ds3g_siolib_port_signal_status[i].call_type = call_type;
      }
      else
      {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERR_FATAL("Port Id is DS3G_SIOLIB_PORT_NONE");
    return result;
  }
  ...
  if(NULL == state)
  {
    DS_ERR_FATAL(" InValid Port State");
    return result;
  } 
  /*-------------------------------------------------------------------------
  ...
  {
    DS_ERR_FATAL("Callback func. ptr = NULL in SIOLIB");
    return FALSE;
  }
  return result;
                                     ...
                                     DSAT_BASIC_N_C_IDX,0,NUM_TYPE) == 2 )    
{
         switch (port_index)
         {
            case DS3G_SIOLIB_PORTID_0:
              t_ptr = &ds3g_siolib_timer_table[DS3G_SIOLIB_TIMER_WINK_DCD_1];
            ...
            case DS3G_SIOLIB_PORTID_1:
              t_ptr = &ds3g_siolib_timer_table[DS3G_SIOLIB_TIMER_WINK_DCD_2];
              ps_timer_start(t_ptr->timer_handle, t_ptr->duration);
              ds3g_siolib_port_signal_status[port_index].dcd_winking = ON;
              break;
            default:
  ...
  else
  {
    ...
    if( ds3g_siolib_port_signal_status[port_index].CD_asserted_already )
    {
      if( (ret_val = ds3g_siolib_cd_deassert(port_id))== DS3G_SIOLIB_OK )
      {
        ...
        ds3g_siolib_port_signal_status[port_index].port_id = DS3G_SIOLIB_PORT_NONE;
        ds3g_siolib_port_signal_status[port_index].call_type = DS_CALL_TYPE_UNSPECIFIED;
      }
    }
    else
    {
      ...
      (state->serial_info.close_in_progress == FALSE) )
  {
    ...
    if ( stream_mode == SIO_DS_AUTODETECT_MODE ) 
    {
      ...
      if ( ( rx_wm_ptr == NULL ) && ( tx_wm_ptr == NULL ) )
      {
       ...
       {
         DS_ERR_FATAL("WM pointers not null in autodetect mode ");
         return DS3G_SIOLIB_NO_PORT_OPEN;
       }
    }
    else
    ...
    switch( stream_mode )
    {
      ...
      case SIO_DS_RAWDATA_MODE:
        ...
        if(port_index == DS3G_SIOLIB_PORTID_1)
        {
          ioctl_param.record_escape_func_ptr = DS3G_SIOLIB_ESCAPE_DETECTED_CB_FUNC(1);
        }
        else
        {
          ioctl_param.record_escape_func_ptr = DS3G_SIOLIB_ESCAPE_DETECTED_CB_FUNC(0);
        }
...
} /* ds3g_siolib_ex_is_rts_asserted() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(port_id <= DS3G_SIOLIB_PORT_NONE ||
	 port_id >= DS3G_SIOLIB_PORT_MAX)
  {
    DS_ERR_FATAL("port id ==DS3G_SIOLIB_PORT_NONE");
    return ret_val;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( state->serial_info.stream_id != SIO_NO_STREAM_ID )
  {
    ...
    -------------------------------------------------------------------------*/
    if (state->serial_info.flow_ctl_mask == DS_FLOW_ENABLE)
    {
...
{
  ds3g_siolib_portid_e_type port_index;
  ds_call_type_e_type call_type = DS_CALL_TYPE_UNSPECIFIED;
  
  for(port_index = DS3G_SIOLIB_PORTID_0; port_index < DS3G_SIOLIB_PORTID_MAX; port_index++)
  {
    if(ds3g_siolib_port_signal_status[port_index].port_id == port_id)
    {
      call_type = ds3g_siolib_port_signal_status[port_index].call_type;
      break;
    }
  }

  DS_AT_MSG3_HIGH("Call type %d for port_id %d port_index %d", call_type, port_id, port_index);
  return call_type;
}
