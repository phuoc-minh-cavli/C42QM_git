...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/3gpp/dsmgr/src/ds3grdmif.c#3 $ $DateTime: 2022/10/06 21:19:58 $ $Author: pwbldsvc $
...
static ULogHandle ds3grdmif_ulog_handle = NULL;

/*---------------------------------------------------------------------------
  Structure for DS_CMD_SIOLIB_RDM_OPEN
---------------------------------------------------------------------------*/
typedef struct
{
...
/*---------------------------------------------------------------------------
  Structure for DS_CMD_SIOLIB_RDM_CLOSE
---------------------------------------------------------------------------*/
typedef struct
{
...
===========================================================================*/
/*===========================================================================
FUNCTION DS3GRDMIF_GET_ULOG_HANDLE
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle ds3grdmif_get_ulog_handle()
{
  return ds3grdmif_ulog_handle;
}

/*===========================================================================
FUNCTION DS3GRDMIF_ULOG_REGISTERr
...
SIDE EFFECTS
  None
===========================================================================*/
void ds3grdmif_ulog_register(void)
{
  ULogFront_RealTimeInit(&ds3grdmif_ulog_handle, 
                               "DS3GRDMIF_ULOG",	  
                               8192,
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ds3grdmif_get_ulog_handle() == NULL)
  {
    ds3grdmif_ulog_register();
  }


  if(ds3grdmif_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_1(ds3grdmif_get_ulog_handle(),
     "ds3gi_rdm_open_cb(): Callback received from RDM for sio port id: %d", port_id);
  }
  ...
  if( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL) )
  {
    ...
    -----------------------------------------------------------------------*/
    cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_RDM_OPEN;
    rdm_open_cmd_ptr = (ds3g_rdm_open_cmd_type*)cmd_ptr->cmd_payload_ptr;
    rdm_open_cmd_ptr->port_id = port_id;

    if(ds3grdmif_get_ulog_handle() != NULL)
    {
      ULOG_RT_PRINTF_0(ds3grdmif_get_ulog_handle(),
           "ds3gi_rdm_open_cb(): Posting DS_CMD_SIOLIB_RDM_OPEN to DS Task");
    }
    ds_put_cmd( cmd_ptr );
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (cmd_ptr != NULL) && (cmd_ptr->cmd_payload_ptr != NULL) )
  {
    /*-----------------------------------------------------------------------
      Fill in the parameters, and send the command to the DS task.
    -----------------------------------------------------------------------*/
    cmd_ptr->hdr.cmd_id = DS_CMD_SIOLIB_RDM_CLOSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_RDM_OPEN)
  {
  ...
  DATA_3GMGR_MSG1(MSG_LEGACY_HIGH, "Rx'ed RDM_OPEN_CMD, port id: %d", port_id);
   ...
   {
     ULOG_RT_PRINTF_1(ds3grdmif_get_ulog_handle(),
             "Rx'ed RDM_OPEN_CMD, port id: %d", port_id);
   }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(cmd_ptr->hdr.cmd_id != DS_CMD_SIOLIB_RDM_CLOSE)
  {
    DATA_3GMGR_MSG1(MSG_LEGACY_LOW, "Invalid cmd: %d", cmd_ptr->hdr.cmd_id);
    return;
  }
