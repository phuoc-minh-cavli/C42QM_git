...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/3gpp/dsmgr/inc/ds3gsiolib.h#2 $ $DateTime: 2021/02/01 05:46:44 $ $Author: pwbldsvc $
...
#define DS_FLOW_UMTS_RMSM_MASK    0x00000002 
#define DS_FLOW_PKT_WM_MASK       0x00000004  /*  Packet mode watermark */
#define DS_FLOW_DSM_SMALL_Q_MASK  0x00000008  /*  Out of small items mask. */
#define DS_FLOW_DSM_LARGE_Q_MASK  0x00000010  /*  Out of large items mask. */
#define DS_FLOW_TECH_SUSP_MASK    0x00000020  /*  Tech suspension status.  */
#define DS_FLOW_ALL_MASK          0xFFFFFFFF  /*  Flow enable/disable override. */

/*-------------------------------------------------------------------------
   Timers used in DS3GSIOLIB
-------------------------------------------------------------------------*/
typedef enum
{
  DS3G_SIOLIB_TIMER_MIN,
  DS3G_SIOLIB_TIMER_WINK_DCD_1 = DS3G_SIOLIB_TIMER_MIN,		 /* timer used to wink DCD */
  DS3G_SIOLIB_TIMER_WINK_DCD_2,		 /* timer used to wink DCD */
  DS3G_SIOLIB_TIMER_MAX
} ds3g_siolib_timer_enum_type;
...
typedef enum
{
  ...
  DS3G_SIOLIB_FSK_DATA_START,                      /* Start of FSK Data entry */
  DS3G_SIOLIB_FSK_DATA_END,                          /* End of FSK Data entry */
#endif
...
typedef struct
{
  ...
  boolean                      esc_seq_detected; /* Escape Sequence Detected */
...
/*---------------------------------------------------------------------------
  Structure for DS_CMD_SIOLIB_DTR_CHANGED
---------------------------------------------------------------------------*/
typedef struct
{
...
/*---------------------------------------------------------------------------
  Structure for DS_CMD_ATCOP_SIOLIB_RTS_CHANGED_CMD
---------------------------------------------------------------------------*/
typedef struct
{
  int port_id;            /* Port on which the RTS has changed */
} ds3g_siolib_rts_chg_cmd_type;

/*---------------------------------------------------------------------------
  Structure for DS_CMD_SIOLIB_CD_CHANGED
---------------------------------------------------------------------------*/
#ifdef FEATURE_DSAT_MDM_FUSION
typedef struct
{
...
/*---------------------------------------------------------------------------
  Structure for DS_CMD_SIOLIB_DSM_MEM_CTRL
---------------------------------------------------------------------------*/
typedef struct
{
...
typedef void (*ds3g_siolib_timer_cb_f_type)(void *cb_data);

typedef struct
{
  ds3g_siolib_timer_enum_type timer_id;
  ds3g_siolib_timer_cb_f_type cb_func;
  dword                       duration;
  ps_timer_handle_type        timer_handle;
}ds3g_siolib_timer_s_type;
...
/*---------------------------------------------------------------------------
  DS3GSIOLIB Timer Table
---------------------------------------------------------------------------*/
extern ds3g_siolib_timer_s_type ds3g_siolib_timer_table[];
...
void ds3g_siolib_init_timers ( void );
...
(
  ds_command_type  *cmd_ptr                         /* Command to be processed */
);
...
extern ds3g_siolib_tx_e_type ds3g_siolib_get_tx_type();
...
FUNCTION DS3G_SIOLIB_ESCAPE_DETECTED_CB_<PORT_INDEX>
...
DESCRIPTION
  ...
  This function simply sets the DS_SIO_ESCAPE_SIG and flag for
...
#define DS3G_SIOLIB_ESCAPE_DETECTED_CB_FUNC(port_index) ds3g_siolib_escape_detected_cb_ ## port_index

void DS3G_SIOLIB_ESCAPE_DETECTED_CB_FUNC(0) ( void );

void DS3G_SIOLIB_ESCAPE_DETECTED_CB_FUNC(1) ( void );
