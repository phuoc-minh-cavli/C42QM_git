...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/api/ps_rx_meta_info.h#1 $
...
typedef struct
{
  ...
  ds_ipfltr_result_type       fi_result[FLTR_CLIENT_INPUT_MAX];
...
#define PS_RX_META_RESET_FILTER_RESULT(rx_mi_ptr, client_id)                   \
  (rx_mi_ptr)->fi_result[(client_id)] = DS_IPFLTR_NOMATCH;            \
  (rx_mi_ptr)->fi_mask &= ~((uint32) 1 << (client_id));
