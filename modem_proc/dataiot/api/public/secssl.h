...
/*=============================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/api/public/secssl.h#1 $ 
  $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/24/19   ds      Code changes to use event mask from sock callback event
                   info instead of dss_getnextevent.
01/16/19   ds      Added support for SSL_VERIFY_NONE.
12/13/18   ds      SSL code rearchitecture for low tier chipsets.
05/10/17   ks      TLSv1.2 support
...
typedef enum
{
  ...
  SECSSL_SES_VERIFY_MODE = 30
    /**<SSL session's verify mode>*/
} secssl_ses_parameter_enum_type;

/** Parameter for the SSL Verify Mode.
*/
typedef enum
{
  SECSSL_VERIFY_PEER = 0,              /**< SSL_VERIFY_PEER */
  /*Currently supported in client mode. The server certificate is verified.
   If the verification process fails, the handshake is immediately terminated 
   with an alert message containing the reason for the verification failure. 
   If no server certificate is sent, because an anonymous cipher is used, 
   SSL_VERIFY_PEER is ignored.*/

  SECSSL_VERIFY_NONE,                  /**< SSL_VERIFY_NONE */
  /*Currently supported in client mode. If not using an anonymous cipher, 
  ...
  SECSSL_VERIFY_FAIL_IF_NO_PEER_CERT,  /**< SSL_VERIFY_FAIL_IF_NO_PEER_CERT */
  /*Currently NOT supported. In client mode, ignored*/

  SECSSL_VERIFY_CLIENT_ONCE,           /**< SSL_VERIFY_CLIENT_ONCE */
  /*Currently NOT supported. In client mode, ignored*/
} secssl_verify_mode_enum_type;
...
typedef union
{
  ...
  secssl_verify_mode_enum_type            verify_mode_pdata;
...
typedef void (*secssl_cb_fptr_type)( sint15, void* );
...
(
  ...
  secssl_cb_fptr_type    ssl_cb_fptr
);
