...
#define IPSECSA_MAX_NUM_SA  3 
  ...
  typedef enum
  {
    ...
    IPSECSA_SEQ_NUM_IN          = 21, /*! ESP Sequence number of IN SA */
    IPSECSA_SEQ_NUM_OUT         = 22, /*! ESP Sequence number of OUT SA */
...
/** @defgroup ipsec_datatypes_SAO2 Security Association Object SetOpt and GetOpt Data Types
  ...
  IPSECSA_SEQ_NUM_IN        -> uint32
  IPSECSA_SEQ_NUM_OUT       -> uint32
  </pre>
*/
...
struct IPSecSAParams_s_type
{
    ...
    uint32                       SASeqNoIn;
    uint32                       SASeqNoOut;
    IPSecSAProtoEType            eSAProto;
    IPSecEncapsulateEType        eEncapsMode;
    IPSecAlgoEType               HashAlgoType;
    IPSecAlgoEType               encAlgoType;
    IPSecKeyType                 CryptoKeyIn;
    IPSecKeyType                 CryptoKeyOut;
    IPSecKeyType                 HashKeyIn; 
    IPSecKeyType                 HashKeyOut; 
    IPSecIVType                  IVKeys;
    IPSecLifeType                lifetimeSec;
};

typedef struct IPSecSAParams_s_type IPSecSAParams_s;

struct IPSecSPParams_s_type
{
  IPSecSP_DirectionEType        SP_Direction;
  IPSecAddrType                 SourceAddrType;
  IPSecAddrType                 DestAddrType;
  IPSecProtocolType             eProto;
  uint16                        SrcPort;
  uint16                        DestPort;
  IPSecEncapsulateEType         eEncapsMode;
  IPSecSP_ProtectionType        *sp_protection;
};

typedef struct IPSecSPParams_s_type IPSecSPParams_s;
