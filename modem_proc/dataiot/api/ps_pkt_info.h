...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/api/ps_pkt_info.h#2 $ $DateTime: 2020/04/15 22:43:35 $ $Author: pwbldsvc $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/14/20    ds     Code changes for DL ICMP support during concurrency between 
                   A7 and DUN.
...
struct ip_pkt_info_s
{
  ...
  union
  {
    struct
    {
      ...
      uint16      sequence_num;      /** < ICMP sequence number for echo/reply */
  ...
  boolean     is_pkt_info_valid;     /**< Used to avoid multiple packet info 
                                        generations */
  boolean     direction;              /**< Direction showing packet coming from  
                                       uplink or downlink path */
  uint16      pkt_group;             /*Packet Group used for RM clients*/
  void*       if_ptr;                /**< Iface on which IP pkt is rcv'ed    */
  void*       fraghdl;               /**< Fragment handler                   */
  uint8       is_fraghdl_allocated;  /*0:Not allocated 1:Allocated 2:Invalid*/
};
