...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/api/ps_rt_meta_info.h#2 $
  $Author: pwbldsvc $ $DateTime: 2021/01/11 06:33:33 $
...
#define IP_FLTR_CLIENT_OUTPUT_MAX ((uint32)FLTR_CLIENT_MAX - (uint32)FLTR_CLIENT_INPUT_MAX)
    /**< @relates ps_rt_meta_info_type
         Indicates whether a particular filter client has already been executed. */
  ds_ipfltr_result_type       fi_result[IP_FLTR_CLIENT_OUTPUT_MAX];
  ...
  boolean                           is_rmnet_packet;    
  /**< Flag to check if packet was sent by rmnet client*/
  boolean                           is_mo_ex;
  /**< Flag to check if packet needs to be frag queue or not*/
...
#define GET_INDEX_FROM_CLIENT_ID(client_id)                         \
  ((uint8)(client_id) - (uint8)FLTR_CLIENT_INPUT_MAX)
...
#define PS_RT_META_SET_FILTER_RESULT(rt_mi_ptr, client_id, in_fi_result) \
  (rt_mi_ptr)->fi_result[(GET_INDEX_FROM_CLIENT_ID(client_id))] =    \
    (ds_ipfltr_result_type)(in_fi_result);                         \
  (rt_mi_ptr)->fi_mask |= ((uint32) 1 << (GET_INDEX_FROM_CLIENT_ID(client_id)));
