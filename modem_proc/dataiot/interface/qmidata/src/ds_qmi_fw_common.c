...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmidata/src/ds_qmi_fw_common.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (x_p == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"No transaction container! Discarding service %d message",
  ...
  while (sdu_in)
  {
    ...
    if (temp == -1)
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Short QMI svc header! Discarding transaction");
      dsm_free_packet( &sdu_in );
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR,"Short QMI svc payload! Discarding service %d message",
                      msg_hdr->common_hdr.service);
      dsm_free_packet( &sdu_in );
      ds_qmi_fw_free_transaction( &x_p );
      return;
  ...
  DS_MSG3(MSG_LEGACY_MED,"Dispatch the transaction(%x) with (%d cmds) ctl= %x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unable to send Response for cmd %d", cmd_buf_p->cmd_type);
    dsm_free_packet(&msg_ptr);
    ds_qmi_fw_free_cmd_buf(&cmd_buf_p);
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Unable to push pending response to send queue");
    dsm_free_packet(&msg_ptr);
    ds_qmi_fw_free_cmd_buf(&cmd_buf_p);
    return FALSE;
  }
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    ...
    {
      DS_MSG0(MSG_LEGACY_MED,"qmi_response pending");
      return TRUE;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending Indication to QMI Framework: clid:%d, tx_id:%d, ind_id:%d",
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Unable to send QMI indication to QMI Framework" );
    dsm_free_packet(&ind);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Too many commands in transaction");
    return NULL;
  }
  ...
  {
      DS_MSG0(MSG_LEGACY_ERROR,"Invalid state pointer !");
      return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *cmd_buf_handle = NULL; // free the callers reference immediately
  ...
  DS_MSG1(MSG_LEGACY_LOW,"Freeing QMI command buffer %x", freeing_cmd_buf_p);
  ...
  if (freeing_cmd_buf_p)
  {
    ...
    {
      DS_MSG0(MSG_LEGACY_MED,"Freeing cmd_buf for global transaction");
    }
  }
  else
  {
    DS_MSG0(MSG_LEGACY_MED,"Attempt to free NULL command buffer!");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
      DS_MSG0(MSG_LEGACY_ERROR,"Invalid client state pointer!");
      return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (*x_p == NULL)
  {
    DS_MSG0(MSG_LEGACY_MED,"Attempt to free a NULL transaction pointer!");
    return;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Transaction in process, "
...
)
{
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(async_cmd_buf, sizeof(ds_qmi_fw_async_cmd_buf_type),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG3(MSG_LEGACY_ERROR,"Unrecognized type (=%d) for service (=%d)! cmd_entries(=%d) Return error",
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Response pending for cmd_buf_p %p", cmd_buf_p);
