...
$Header: //components/rel/dataiot.mpss/2.0/interface/qmidata/src/ds_qmi_nv_util.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_LOW,"returning status=%d", status);
...
{
  return qmi_nv_read_ex(item,ep_id,mux_id,data_ptr,data_size);
} /* qmi_nv_read() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_MSG3(MSG_LEGACY_MED,"build path failed for item=%d ep_id=%d and mux_id=%d",item,ep_id,mux_id);
    }
  }

  if (status != QMI_NV_STATUS_OK)
  {
  DS_MSG1(MSG_LEGACY_MED,"returning status=%d", status);
  }
...
{
  return qmi_nv_write_ex(item,ep_id,mux_id,data_ptr,data_size);
} /* qmi_nv_write() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    {
      DS_MSG3(MSG_LEGACY_MED,"build path failed for item=%d ep_id=%d mux_id=%d",item,ep_id,mux_id);
    }
  }
...
#if 0
//This api was used in dms for ERI file read not used in chiltepin and in rmnet for 
QMI_NV_FILE_ROAM_IND_LIST not used in the chiltepin
/*==========================================================================
...
{
  return qmi_nv_file_read_ex(file,data_ptr,data_size,file_size);
} /* qmi_nv_file_read() */
...
{
   return qmi_nv_file_write_ex(file,data_ptr,data_size);
} /* qmi_nv_file_write() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    //fg_subs_id = ps_utils_convert_subs_ps_to_mcfg(ps_subs_id);
    status = qmi_nvi_file_build_path(file, path, sizeof(path));
    ...
    if (status == QMI_NV_STATUS_OK)
    {
      status = qmi_nvi_file_put_contents(path, (char *) data_ptr, data_size );
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(path, QMI_NV_MAX_EFS_PATH_SIZE, char*);
  if (path == NULL)
  {
  ...
  if (fd < 0)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"EFS qmi conf file open failed: %s", qmi_conf_path);
    DS_SYSTEM_HEAP_MEM_FREE(path);
    return QMI_NV_STATUS_EFS_ERR;
  }
  ...
  for (i = 0; i < ARR_SIZE(item); i++)
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR,"Unknown NV item %d", item[i]);
      continue;
    }
    ...
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Insuffient buffer");
      break;
    }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"EFS qmi conf file write failed: %s", qmi_conf_path);
