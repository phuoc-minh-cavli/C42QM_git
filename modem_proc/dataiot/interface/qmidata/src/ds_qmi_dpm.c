...
)
{
  ...
  DS_MSG1(MSG_LEGACY_MED,"QMI DPM Service init State %d", qmi_dpmi_inited);
  if (!qmi_dpmi_inited)
  {
    /*-------------------------------------------------------------------------
  ...
  DS_MSG1(MSG_LEGACY_MED,"qmi_csi_register success %d", rc);
#ifdef QMI_DPM_DEBUG
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( NULL != q_linear_search( &dpm_svc_ptr->dpmi_client_q,
                                ...
                                client_handle ) )
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_svc_connect_cb: DPM client 0x%p present",
                    client_handle);
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_svc_connect_cb: Maximum number of clients already allocated %d",
                    DPMI_MAX_CLIENTS);
    rc = QMI_CSI_CB_CONN_REFUSED;
    goto bail;
  }

  DS_SYSTEM_HEAP_MEM_ALLOC(clnt_info_ptr, sizeof(qmi_dpmi_client_info_type),
                           qmi_dpmi_client_info_type*);
  if(!clnt_info_ptr)
  {
    rc = QMI_CSI_CB_CONN_REFUSED;
  ...
  *connection_handle = clnt_info_ptr;
  ...
  DS_MSG1(MSG_LEGACY_MED,"dpmi_svc_connect_cb: Alloc client 0x%p", clnt_info_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_svc_connect_cb: DPM client 0x%p not found",
                    connection_handle);
    return;
  ...
  if ( TRUE != q_delete_ext( &dpm_svc_ptr->dpmi_client_q,
                             &clnt_info_ptr->link ) )
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_svc_connect_cb: DPM client 0x%p could not be deleted from queue",
                    clnt_info_ptr);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == clnt_info_ptr)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_handle_req_cb: Invalid clnt 0x%p",
                    connection_handle);
  ...
  if(msg_id < (sizeof(req_handle_table) / sizeof(*req_handle_table)))
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR,"dpmi_handle_req_cb: NULL message ID handler: %d",
                      msg_id);
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"dpmi_handle_req_cb: Invalid message ID: %d", msg_id);
    rc = QMI_CSI_CB_UNSUPPORTED_ERR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_ASSERT((sig == QMI_QCSI_SERVICE_DPM_SIGNAL) && svc_ptr);
...
)
{
  ...
  DS_MSG1(MSG_LEGACY_MED,"Client 0x%p requesting Open port", clnt_info);
  ...
  if (req_msg->software_data_port_list_valid == TRUE)
  {
    for (iter = 0; iter < req_msg->software_data_port_list_len; iter++)
    {
      if (FALSE == qmi_dpmi_convert_ep_id_to_int(req_msg->software_data_port_list[iter].ep_id.ep_type,
                                                 ...
                                                 &ep_id))
      {
        DS_MSG2(MSG_LEGACY_ERROR,"Unknown value of End point id received with ep-type %d,iface_id %d ",
                        req_msg->software_data_port_list[iter].ep_id.ep_type,
                        req_msg->software_data_port_list[iter].ep_id.iface_id);
        continue;
      }
      err_code = qmux_add_data_port(glink_sio_get_sio_id_from_name(req_msg->software_data_port_list[iter].port_name), 
                                    ep_id, 0, 0);
      if (err_code == FALSE)
      {
        DS_MSG1(MSG_LEGACY_ERROR,"Could not add data port with ep_id %d", ep_id);
      }
    }
  }
  ...
  if (req_msg->control_port_list_valid == TRUE)
  {
    for (iter = 0; iter < req_msg->control_port_list_len; iter++)
    {
      ...
      err_code = qmux_add_control_port(glink_sio_get_sio_id_from_name(req_msg->control_port_list[iter].port_name),
                                       ep_id);
      if (err_code == FALSE)
      {
        DS_MSG1(MSG_LEGACY_ERROR,"Could not add control port with ep_id %d", ep_id);
      }
    }
  }
...
)
{
  ...
  DS_MSG1(MSG_LEGACY_MED,"Client 0x%p requesting close port", clnt_info);
  ...
  if (req_msg->control_port_list_valid == TRUE)
  {
    for (iter = 0; iter < req_msg->control_port_list_len; iter++)
    {
      err_code = qmux_remove_control_port(glink_sio_get_sio_id_from_name(req_msg->control_port_list[iter].port_name));
    }
  }
  if (req_msg->data_port_list_valid == TRUE)
  {
    for (iter = 0; iter < req_msg->data_port_list_len; iter++)
    {
      ...
      err_code = qmux_remove_data_port(ep_id);
       if (err_code == FALSE)
       {
         DS_MSG1(MSG_LEGACY_ERROR,"Could not remove data port with ep_id %d", ep_id);
       }
    }
  }
...
)
{
  ...
  DS_MSG1(MSG_LEGACY_MED,"Client 0x%p querying supported capabilities", clnt_info);
