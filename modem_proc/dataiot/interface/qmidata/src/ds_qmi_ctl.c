...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmidata/src/ds_qmi_ctl.c#1 $
...
12/17/10    kk     Added log messages before DS_ASSERTs.
...
static qmi_ctli_cmd_hdlr_type  qmi_ctl_cmd_callbacks[QMI_CTL_CMD_MAX] =
{
  ...
  QMI_CTL_HDLR(QMI_CTLI_CMD_VAL_SET_DATA_FORMAT, NULL),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Recd invalid inst_id '%d' in qmi_ctl_init",
                    (int32)qmi_inst);
    DS_ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERR_FATAL("Recd NULL ptr in qmi_ctl_get_instance_id");
    return FALSE;
  }

  if (QMI_INSTANCE_MAX <= qmi_instance)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Recd invalid inst_id '%d' in qmi_ctl_get_instance_id",
                    (int32)qmi_instance);
    DS_ASSERT(0);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid QCTL cmd:%d ", QMI_CTLI_CMD_VAL_GET_CLIENT_ID);
    return;
  }
  ...
  DS_MSG2(MSG_LEGACY_MED,"Sending QMI_CTL Client %d for service %d",
...
send_result:
  ...
  if(FALSE == qmi_ctli_send( &qmi_ctl_state[client_reg_result->inst_id],
                 ...
                 response_ptr ))
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Unable to send a response!");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT( QMUX_SERVICE_MAX > svc_id );

  DS_MSG0(MSG_LEGACY_MED,"qmi_ctl_notify_service_ready");
  ...
  if(!is_legacy_svc)
  {
    ...
    for(index= QMI_INSTANCE_MIN; index < QMI_INSTANCE_MAX; index++)
    {
      ...
      if (!report_svc_avail)
      {
        DS_MSG1(MSG_LEGACY_MED,"No Indication sent for service [%d]", svc_id);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Client registered for service availability "
                     "notification!");
    ...
    if (!report_svc_avail)
    {
      DS_MSG1(MSG_LEGACY_MED,"Service [%d] not found in svc_avail_list.", svc_id);
      return FALSE;
    }
    else
    {
      ...
      DS_MSG1(MSG_LEGACY_MED,"Indicate QMI Service [%d] Ready on instance: [%d]",
                       svc_id );
      ...
      if( FALSE == qmi_ctli_send( ctl_sp,
                                  ...
                                  ind ) )
      {
        DS_MSG0(MSG_LEGACY_ERROR,"Unable to generate Event Report IND");
        return FALSE;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    DS_ERR_FATAL("Can't perform close on NULL svc ptr");
    return;
  }
  ...
  DS_MSG0(MSG_LEGACY_MED,"QMI link closed, reset QMI link ID");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    ...
    DS_ERR_FATAL("Recd NULL svc ptr, dropping message");
    goto send_result;
  }
  else if (NULL == sdu_in)
  {
    DS_ERR_FATAL("Recd NULL SDU ptr, dropping message");
    goto send_result;
  }

  DS_MSG2(MSG_LEGACY_MED,"Got QMI_CTL command type %d (%d bytes)",
                   qmi_ctl_cmd, sdu_len);
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Unrecognized msg! (%d) ignoring", qmi_ctl_cmd);

    if( FALSE == qmi_svc_put_result_tlv
             (&response_ptr, QMI_RESULT_FAILURE, QMI_ERR_INVALID_QMI_CMD) )
    {
      DS_MSG1(MSG_LEGACY_MED,"Unable to send response for qmi_ctl cmd %d",
                       qmi_ctl_cmd);
      dsm_free_packet(&response_ptr);
      return FALSE;
    }
  ...
  switch (ctl_flags & QMI_CTL_FLAG_MASK_MSGTYPE)
  {
    ...
    default:
      DS_MSG1(MSG_LEGACY_MED,"Invalid msg type! (%d) ignoring",
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"No action for msg (%d), ignoring", qmi_ctl_cmd);
    response_ptr = NULL;
  }
...
send_result:
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"(CTL msg generated no response)");
    return FALSE;
  }
  else if (response_ptr == QMI_SVC_RESPONSE_PENDING)
  {
    DS_MSG0(MSG_LEGACY_MED,"(CTL response is pending)");
    return TRUE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if (FALSE == qmi_ctli_is_valid_client_id(sp,clid))
  {
    ...
    DS_ERR_FATAL("Recd pkt from invalid client, dropping message");
    return;
  }
  else if (NULL == sdu_in)
  {
    DS_ERR_FATAL("Recd NULL SDU ptr, bailing out");
    return;
  }
  ...
  {
    DS_MSG2(MSG_LEGACY_MED, "Got short QMI_CTL message! (expected=0x%X, got=0x%X bytes",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    ...
    DS_ERR_FATAL("Unable to send pkt on NULL svc ptr");
    return FALSE;
  }
  ...
  DS_MSG2(MSG_LEGACY_MED,"Send QMI_CTL type %d (%d bytes)",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      default:
      {
        DS_MSG1(MSG_LEGACY_MED,"Unrecognized TLV type (%d)", type);
        break;
      }
    }
    ...
    {
       DS_MSG2(MSG_LEGACY_MED,"Invalid TLV len (%d) for type (%d)", len, type);
  ...
  DS_MSG1(MSG_LEGACY_MED,"Got QMI_CTL set instance command (instance = %d)",
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Missing required parameters TLV!");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(v_out_reqd, sizeof(v_out_reqd_type),
                           v_out_reqd_type*);
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(v_out_addendum_opt, sizeof(v_out_addendum_opt_type),
                           v_out_addendum_opt_type*);
  ...
  -------------------------------------------------------------------------*/
send_result:
  DS_SYSTEM_HEAP_MEM_FREE(v_out_reqd);
  DS_SYSTEM_HEAP_MEM_FREE(v_out_addendum_opt);
  result = (errval == QMI_ERR_NONE ? QMI_RESULT_SUCCESS
                                   : QMI_RESULT_FAILURE);
...
)
{
  ...
  qmi_ctli_cmd_buf_type  * cmd_payload = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      default:
        DS_MSG1(MSG_LEGACY_MED,"Unexpected TLV type (%d)", type);
        break;
    }
  ...
  if (!got_v_in_reqd)
  {
    DS_MSG0(MSG_LEGACY_MED,"Mandatory TLV not present!");
    errval = QMI_ERR_MISSING_ARG;
    goto send_result;
  }
  ...
  -------------------------------------------------------------------------*/
  DS_MSG1(MSG_LEGACY_MED,"Got QMI_CTL get CLID command (svc = %d)",
  ...
  if(svc_cfg == NULL)
  {
    DS_MSG0(MSG_LEGACY_MED,"QMI_CTL get CLID - Service based on new framework");
    ...
    if(retval != QMI_ERR_NONE)
    {
     ...
     DS_MSG2(MSG_LEGACY_MED,"QMI_CTL get CLID pending response svc: %d, tx_id: %d",
  ...
  /*-----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
   cmd_ptr = ds_allocate_cmd_buf(sizeof(qmi_ctli_cmd_buf_type));
   ...
   cmd_ptr->hdr.cmd_id = DS_QMI_CTL_SVC_CMD;
   cmd_payload = (qmi_ctli_cmd_buf_type *)cmd_ptr->cmd_payload_ptr;

  cmd_payload->cmd_id = QMI_CMD_LEGACY_SVC_ALLOC_CLID;
  cmd_payload->data.client_reg.qmux_svc_handle = ctl_sp->qmux_svc_handle;
  cmd_payload->data.client_reg.qmux_type = v_in_required.qmux_type;
  cmd_payload->data.client_reg.tx_id = tx_id;  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == sp)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Recd NULL svc ptr, dropping message");
    if ((NULL != sdu_in) && (NULL != *sdu_in))
    {
      dsm_free_packet(sdu_in);
    }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Recd pkt from invalid client, dropping message");
    if ((NULL != sdu_in) && (NULL != *sdu_in))
    {
      dsm_free_packet(sdu_in);
    }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Recd NULL SDU ptr, bailing out");
    return NULL;
  }
  ...
  -------------------------------------------------------------------------*/
   DS_MSG2(MSG_LEGACY_MED,"Got QMI_CTL release CLID command (svc = %d, clid = %d)",
  ...
  if(svc_cfg == NULL)
  {
    ...
    if(retval != QMI_ERR_NONE)
    {
     ...
     DS_MSG2(MSG_LEGACY_MED,"QMI_CTL release CLID pending response svc: %d, tx_id: %d",
                     common_msg_hdr.service, common_msg_hdr.transaction_id);
    return QMI_SVC_RESPONSE_PENDING;
  }
  ...
  /*-----------------------------------------------------------------------
   ...
   cmd_payload->cmd_id = QMI_CMD_LEGACY_SVC_DEALLOC_CLID;
   cmd_payload->data.client_reg.qmux_svc_handle = ctl_sp->qmux_svc_handle;
   cmd_payload->data.client_reg.qmux_type = v_io_required.qmux_type;
   cmd_payload->data.client_reg.clid = v_io_required.clid;  
   cmd_payload->data.client_reg.tx_id = tx_id;  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid link protocol(s) specified: 0x%x!", link_prot);
  ...
  data_format.link_prot = (rmnet_sm_link_prot_e_type)link_prot;
   ...
   /*-------------------------------------------------------------------------
    ...
    - allocate a client ID from the specified QMI service
  -------------------------------------------------------------------------
  DS_MSG3(MSG_LEGACY_MED,"Got QMI_CTL set data format:Qos[%d] Link Prot[%d] UL TLP [%d]",
                   data_format.qos,
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Got QMI_CTL set data format on reverse IP port, just return ");
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Data format already set by WDA");
    errval = QMI_ERR_INVALID_OPERATION;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_MED,"Received QMI_CTL_SYNC_REQ, sending QMI_CTL_SYNC_RESP");
...
)
{
  ...
  if (NULL == sp)
  {
    DS_ERR_FATAL("Can't perform sync on NULL svc ptr");
    return;
  }

  DS_MSG0(MSG_LEGACY_MED,"Sending QMI_CTL_SYNC indication");
  ...
  if( FALSE == qmi_ctli_send( sp,
                              ...
                              ind ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Can't send QMI_CTL_SYNC indication!");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_LOW,"qmi_ctl_send_sync: %d", qmi_inst);

  if( FALSE == qmi_ctl_global.inited )
  {
    DS_ERR_FATAL("Port init attempted before QMI init completed");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR, "Could not get cmd buffer from QMI task for cmd id %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_reg_pwrsave_mode");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_pwrsave_mode_ind");

  ctl_sp = (qmi_ctli_state_type *) sp;
  if( NULL == ctl_sp ) 
  {
    DS_ERR_FATAL("Null ctl_sp in ctl_evt_rpt_ind");
    return;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Client not registered for power save indication!");
    return;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Failed to query power save state!");
    return;
  }
  v_out1.prev_state_handle = prev_state_handle;
    
  DS_MSG3(MSG_LEGACY_MED,"Indicate power save state change: inst=%d curr=0x%x, prev=0x%x",
  ...
  if( FALSE == qmi_ctli_send( sp,
			      ...
			      ind ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Unable to generate Event Report IND");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_cfg_pwrsave_mode");
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      case QMI_TYPE_PWRSAV_INDSET:
        ...
        {
          DS_MSG3(MSG_LEGACY_MED,"Invalid TLV len (%d>%d) for type (0x%X)",
      ...
      default:
        DS_MSG1(MSG_LEGACY_MED,"Unexpected TLV type (0x%X)", type);
        break;
    }
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid power save state: %d",
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid service identifier: %d", v_in_required.svc_id);
    errval = QMI_ERR_INVALID_ARG;
    goto send_result;
  }
  ...
  -------------------------------------------------------------------------*/
  DS_MSG3(MSG_LEGACY_MED,"Got QMI_CTL configure power save mode "
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid num_inds: (%d) ", num_inds);
    errval = QMI_ERR_INVALID_ARG;
    goto send_result;
  }
  ...
  if( !spec_stored )
  {
    DS_MSG0(MSG_LEGACY_MED,"Error storing power save spec!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_set_pwrsave_mode");
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid power save state: %d", new_state_handle);
    errval = QMI_ERR_INVALID_ARG;
    goto send_result;
  }
  ...
  -------------------------------------------------------------------------*/
  DS_MSG1(MSG_LEGACY_MED, "Got QMI_CTL set power save mode (state = 0x%x)",
                   new_state_handle);
  ...
  if( !qmi_svc_set_pwrsave_state( ctl_sp->qmux_svc_handle,
				  ...
				  &old_state_handle ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Error setting power save mode!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_get_pwrsave_mode");
  
  /*-------------------------------------------------------------------------
    Perform action/gather required information for response
    - get the power save mode for the QMI instance
  -------------------------------------------------------------------------*/
  DS_MSG0(MSG_LEGACY_MED,"Got QMI_CTL get power save mode");

  if( !qmi_svc_get_pwrsave_state( ctl_sp->qmux_svc_handle,
                                  &v_out_required.state_handle ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Error getting power save mode!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }

  DS_MSG1(MSG_LEGACY_MED,"Query returned state: 0x%x", v_out_required.state_handle);
#ifdef FEATURE_DATA_QMI_RESP_FILTERING
  if( !qmi_svc_get_response_reporting_in_current_pwrsave_mode
                                  ( ctl_sp->qmux_svc_handle,
                                  &v_out_opt.response_reporting ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Error getting power save mode response reporting!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_MED,"qmi_ctli_configure_response_filtering_in_pwrsave_mode");
  ...
  /*-------------------------------------------------------------------------
    ...
    - discard unexpected TLVs
  -------------------------------------------------------------------------*/
  DS_SYSTEM_HEAP_MEM_ALLOC (v_in_required, sizeof(response_reporting_type),
                            response_reporting_type*);
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
       case QMI_TYPE_REQUIRED_PARAMETERS:
         ...
         {
           DS_MSG2(MSG_LEGACY_MED,"T (%d) Invalid L (%d bytes) V - discarding",
                           type, len);
           errval = QMI_ERR_ARG_TOO_LONG;
           goto send_result;
         }
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid number of power save state: %d",
  ...
  for (index = 0 ; index < v_in_required->n_pwrsave_state;index++)
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR,"Invalid power save state: %d",
                       v_in_required->reporting_tuple[index].pwrsave_state_handle);
      errval = QMI_ERR_INVALID_ARG;
      goto send_result;
    }
  ...
  for (index = 0 ; index < v_in_required->n_pwrsave_state;index++)
  {
    ...
    if( !spec_stored )
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Error storing power save spec!");
      errval = QMI_ERR_INTERNAL;
      goto send_result;
    }
  }

  DS_SYSTEM_HEAP_MEM_ALLOC(v_out_reporting_opt, sizeof(response_reporting_type),
                           response_reporting_type*);
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Error: Invalid QMI INSTANCE!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
send_result:
  ...
  if (v_out_reporting_opt != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(v_out_reporting_opt);
  }

  if (v_in_required != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(v_in_required);
  }
...
"qmi_ctli_get_response_filtering_setting_in_pwrsave_mode");
  ...
  if( !response_reporting_spec )
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Error: Invalid QMI INSTANCE Found!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QMI_INSTANCE_MAX <= qmi_inst)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Recd invalid inst_id '%d' in qmi_ctl_pwrsave_mode_change_notify",
                    (int32)qmi_inst);
    DS_ASSERT(0);
    return FALSE;
  }
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctl_pwrsave_mode_change_notify");
 
  /* Generate indication for state transition on return to normal power mode. */
  if( QMI_SVC_PWRSAVE_STATE_NORMAL == new_state_handle )
  {
     DS_MSG2(MSG_LEGACY_MED,"Generate power save indication: new=0x%X old=0x%X",
...
)
{
  ...
  qmi_error_e_type   errval;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_MSG0(MSG_LEGACY_MED,"Enter qmi_ctli_sync_oprt_mode_online");
  ...
  {
    DS_ERR_FATAL("Mem alloc from system heap failed.");
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Target in P-LPM mode, bailing out");
    goto bail_out;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Target in FTM mode, bailing out");
    goto bail_out;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Unable to issue a Online command ");
    errval = QMI_ERR_INTERNAL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(v_in_svc_list, sizeof(svc_list_type),
                           svc_list_type*);
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_set_svc_avail_list");
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid service set length: %d",
  ...
  for(index=0; index<v_in_svc_list->num_svcs; index++)
  {
    ...
    {
       DS_MSG2(MSG_LEGACY_MED,"Invalid service identifier on index[%d]: %d",
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC (v_out_svc_list, sizeof(svc_list_type),
                            svc_list_type*);
  ...
  if( !response_reporting_spec )
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Error: Invalid QMI Service Available List!");
    errval = QMI_ERR_INTERNAL;
    goto send_result;
  }
...
send_result:
  ...
  if (v_in_svc_list != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(v_in_svc_list);
  }

  if (v_out_svc_list != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(v_out_svc_list);
  }
...
(
 ...
 /*-------------------------------------------------------------------------------*/
   ...
   DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_get_svc_avail_list");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* check that service_handle or svc_list is not NULL
     in case DS_ASSERT is removed */
  if (sp == NULL || in_svc_list == NULL)
  {
    DS_ERR_FATAL("QMI_SVC list NULL!");
    return FALSE;
  }
  if (num_svcs == 0)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Incorrect Service Available list req!");
    return FALSE;
  }

  ctl_sp = (qmi_ctli_state_type *) sp;

  DS_MSG0(MSG_LEGACY_LOW,"ctli_store_service_available_list");
  ...
  while(current != NULL)
  {
    ...
    DS_SYSTEM_HEAP_MEM_FREE(current);
    current = temp;
  }
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC (ctl_sp->svc_avail_list, sizeof(qmi_svc_avail_list_type),
                            qmi_svc_avail_list_type*);
  ...
  {
    DS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(temp, sizeof(qmi_svc_avail_list_type),
                                    qmi_svc_avail_list_type*);

    if ( temp == NULL)
    {
      DS_SYSTEM_HEAP_MEM_FREE(ctl_sp->svc_avail_list);
      return FALSE;
    }
...
/*-------------------------------------------------------------------------*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"ctli_get_available_service_list");
  ...
  {
    DS_ERR_FATAL("QMI_SVC state pointer,svc list, num_svcs NULL!");
    return FALSE;
  }
  ...
  if(ctl_sp->svc_avail_list == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI Service Available List not initialized!");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT( QMUX_SERVICE_MAX > service_id );
  
  /* check that service_id is valid in case DS_ASSERT is removed */
  if (service_id >= QMUX_SERVICE_MAX )
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI service ID not valid (%d)!  Discarding", service_id);
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI SVC Avail List is not initialized!");
    return FALSE;
  }
  ...
  while (svc_avail_list != NULL)
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"Notifying Control Point about service [%d] availability",
                      service_id);
      return TRUE;
    }
    svc_avail_list = svc_avail_list->next;
  ...
  DS_MSG1(MSG_LEGACY_ERROR,"Ignoring Notification to Control Point about "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_LOW,"qmi_ctli_cfg_pwrsave_mode_ext");
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      case QMI_TYPE_PWRSAV_INDSET_EXT:
        ...
        DS_SYSTEM_HEAP_MEM_ALLOC(v_in_list, len,  v_in_opt_type*);
  ...
  if (got_v_in_opt && v_in_list != NULL)
  {
    ...
    {    
      DS_MSG1(MSG_LEGACY_MED,"Invalid num_inds: (%d) ", v_in_list->num_inds);
      errval = QMI_ERR_INVALID_ARG;
      goto send_result;
    }
  }
  ...
  {
    DS_SYSTEM_HEAP_MEM_ALLOC(v_in_list, sizeof(v_in_opt_type), v_in_opt_type*);
  ...
  DS_MSG3(MSG_LEGACY_MED,"Got QMI_CTL configure power save mode ext "
                   "(state=0x%X service=0x%X inds=%d)",
                   v_in_required.state_handle,
                   v_in_required.svc_id,
                   v_in_list->num_inds);
...
send_result:
  ...
  if (v_in_list != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(v_in_list);
  }
