...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/utils/src/ps_handle_mgr.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"ps_handle_mgr_init_client(): "
  ...
  {
    DS_MSG3(MSG_LEGACY_ERROR,"ps_handle_mgr_init_client(): "
  ...
  {
    DS_ERROR_FATAL("ps_handle_mgr_init_client(): Max Base handle + # of items > pow(2, 31) - 1");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (client_id >= PS_HANDLE_MGR_CLIENT_MAX)
  {
    DS_MSG1(MSG_LEGACY_HIGH,"ps_handle_mgr_get_handle(): "
                    "Invalid ID %d", client_id);
    ASSERT(0);
  ...
  if (index < 0)
  {
    DS_MSG2(MSG_LEGACY_ERROR,"ps_handle_mgr_get_handle(): "
  ...
  if (!( client_info_ptr->is_inited))
  {
    DS_MSG1(MSG_LEGACY_ERROR,"ps_handle_mgr_get_handle(): "
        ...
        client_info_ptr->max_handle_used_so_far)
  {
    ...
    {
      DS_MSG3(MSG_LEGACY_HIGH,"ps_handle_mgr_get_handle(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (client_id >= PS_HANDLE_MGR_CLIENT_MAX)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"ps_handle_mgr_get_index(): "
                    "Invalid ID %d", client_id);
    DS_ASSERT(0);
