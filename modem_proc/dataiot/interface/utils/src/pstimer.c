...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/utils/src/pstimer.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
08/16/09    pp     Use timer critical sections for mutual exclusion instead
                   ...
                   race condition between timer_expiry and timer_cancel.
11/28/08    pp     Use of DS_ENTER_CRIT_SECTION/UNLOCK macros for better task(s)
...
---------------------------------------------------------------------------*/
static ds_crit_sect_type ps_timer_crit_section;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"ps_timeri_handle_to_timer(): Invalid timer 0x%x",
             timer_handle);
    return NULL;
  }
  
  DS_MSG1(MSG_LEGACY_ERROR,"ps_timeri_handle_to_timer(): index %d", 
          (timer_handle & PS_TIMER_HANDLE_MASK));
  ...
  if(ps_timer == NULL)
  {
     DS_MSG1(MSG_LEGACY_ERROR,"ps_timeri_handle_to_timer(): timer is NULL AT index %d", 
             (timer_handle & PS_TIMER_HANDLE_MASK));	 
     return NULL;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"ps_timeri_handle_to_timer(): Timer 0x%x is freed",
                    timer_handle);
    return NULL;
  }
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"ps_timeri_handle_to_timer(): Invalid timer 0x%x, "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(cmd == PS_TIMER_CALLBACK_CMD);

  timer_handle = (ps_timer_handle_type) user_data_ptr;

  DS_ENTER_CRIT_SECTION(&ps_timer_crit_section);
  ...
  do
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timeri_client_cmd_cb(): Couldn't find timer 0x%x",
                      timer_handle);
      break;
    }
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timeri_client_cmd_cb(): Not calling callback for "
    ...
    if (ps_timer->extended_time_left > 0)
    {
      ...
      {
        DS_MSG2(MSG_LEGACY_MED,"ps_timeri_handler_cb(): "
                        "Timer %d, remaining time : %lld",
                        (ps_timer-ps_timer_list[0]),
  ...
  DS_LEAVE_CRIT_SECTION(&ps_timer_crit_section);

  if (cb_fcn != NULL )
  {
    DS_MSG1(MSG_LEGACY_MED,"ps_timeri_client_cmd_cb(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timeri_handler_cb(): Couldn't find timer 0x%x",
                      timer_handle);
      break;
    }
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timeri_client_cmd_cb(): Ignoring callback for "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS_INIT_CRIT_SECTION(&ps_timer_crit_section);
  ...
  DS_MSG1(MSG_LEGACY_MED,"ps_timer_init(): Max timer value supported is %lld ms",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_DESTROY_CRIT_SECTION(&ps_timer_crit_section);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (loop = 0; loop < PS_TIMER_MAX; loop++ )
  {
   ...
   if(ps_timer_list[index] == NULL)
   {    
      DS_SYSTEM_HEAP_MEM_ALLOC(ps_timer_list[index] ,sizeof(ps_timer_entry_type),
                               ps_timer_entry_type *);
      if (ps_timer_list[index] == NULL)
      {  
         DS_LEAVE_CRIT_SECTION(&ps_timer_crit_section);
      ...
      DS_ASSERT( timer_handle != PS_TIMER_INVALID_HANDLE);
  ...
  DS_MSG2(MSG_LEGACY_MED,"ps_timer_alloc(): Allocated timer 0x%x CB Register 0x%x Timer 0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG2(MSG_LEGACY_MED,"ps_timer_start(): Timer 0x%x, timeout %lld ms",
                  timer_handle, timer_len);

  if (timer_len <= 0)
  {
    DS_MSG2(MSG_LEGACY_ERROR,"ps_timer_start(): Invalid timeout %lld, Timer 0x%x",
                    timer_len, timer_handle);
    return PS_TIMER_FAILURE;
  }
  if(timer_handle == PS_TIMER_INVALID_HANDLE)
  {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_start(): Invalid handle , Timer 0x%x",
              timer_handle);      
      return PS_TIMER_FAILURE;   
  }
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_start(): Couldn't find timer 0x%x",
                      timer_handle);
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timer_start(): Timer 0x%x, removing old callback that "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"ps_timer_cancel(): Timer 0x%x", timer_handle);

  if(timer_handle == PS_TIMER_INVALID_HANDLE)
  {
     DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_start(): Invalid timer_handle , Timer 0x%x",
             timer_handle);      
     return PS_TIMER_FAILURE;
  }
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_cancel(): Couldn't find timer 0x%x",
                      timer_handle);
      ret_val = PS_TIMER_FAILURE;
      break;
    }
    ...
    if (ps_timer->cback_pending == TRUE)
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timer_cancel(): Timer 0x%x, removing old callback "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"ps_timer_free(): Timer 0x%x", timer_handle);

  if(timer_handle == PS_TIMER_INVALID_HANDLE)
  {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_free(): Invalid timer_handle , Timer 0x%x",
           timer_handle);      
      return PS_TIMER_FAILURE; 
  }
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_free(): Couldn't find timer 0x%x",
                      timer_handle);
      ret_val = PS_TIMER_FAILURE;
      break;
    }
    ...
    if (ps_timer->cback_pending == TRUE)
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timer_free(): Timer 0x%x, removing old callback "
                      "that hasn't been processed yet", timer_handle);
      ps_timer->cback_pending = FALSE;
    }
    ...
    -----------------------------------------------------------------------*/
    ps_timer_list[(timer_handle & PS_TIMER_HANDLE_MASK)] = NULL;
    memset(ps_timer, 0, sizeof(ps_timer_entry_type));	

    DS_SYSTEM_HEAP_MEM_FREE(ps_timer);
    ret_val = PS_TIMER_SUCCESS;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"ps_timer_is_running(): Timer 0x%x", timer_handle);
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"ps_timer_is_running(): Couldn't find timer 0x%x",
                      timer_handle);
    ...
    DS_MSG2(MSG_LEGACY_MED,"ps_timer_is_running(): Timer 0x%x is_running %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"ps_timer_remaining(): Timer 0x%x", timer_handle);
  ...
  do
  {
    ...
    if ((ps_timer = ps_timeri_handle_to_timer(timer_handle)) == NULL)
    {
      DS_MSG1(MSG_LEGACY_MED,"ps_timer_remaining(): Couldn't find timer 0x%x",
                      timer_handle);
      time_left = -1;
      break;
    }
