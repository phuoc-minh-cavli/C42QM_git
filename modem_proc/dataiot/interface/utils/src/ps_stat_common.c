...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/utils/src/ps_stat_common.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
)
{
  ...
  if (access_ptr == NULL)
  {
    DS_ERROR_FATAL("ps_stat_get_common(): ps_stat_get:variable access_ptr should not be NULL");
    return E_NOT_ALLOWED ;
  }

  if ( stat_ptr == NULL)
  {
    DS_ERROR_FATAL("ps_stat_get_common(): ps_stat_get:variable stat_ptr should not be NULL");
    return E_NOT_ALLOWED ;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_stat_get_common(): "
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid stat value passed to ps_stat_get_common ");
  ...
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_stat_get_common(): "
                    " Insufficient memory allocated for ptr return_value %d access_ptr[stat].len %d",ret_len, access_ptr[stat].len);
    return E_NOT_ALLOWED ;
  }
