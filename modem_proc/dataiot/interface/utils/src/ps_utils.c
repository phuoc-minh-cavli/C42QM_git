...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/utils/src/ps_utils.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_get_nv_item(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ps_nv_cmd_buf.status != NV_DONE_S )
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_HIGH,"ps_put_nv_item(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ps_nv_cmd_ext_buf.context = SYS_MODEM_AS_ID_1;//ps_utils_convert_subs_ps_to_sys(ps_subs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ps_nv_cmd_buf.status != NV_DONE_S )
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_put_nv_item(): "
                    "NV Write Failed Item %d Code %d",
                    ps_nv_cmd_buf.item, ps_nv_cmd_buf.status );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG_SPRINTF_2(MSG_LEGACY_MED,"memdump(): "
...
)
{
  ...
  return ps_create_efs_config_file_ext(conf_file_path);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_create_efs_config_file(): "
  ...
  -------------------------------------------------------------------------*/
  DS_MSG0(MSG_LEGACY_MED,"ps_create_efs_config_file(): "
                  "EFS: Creating conf file if necessary");
  result = ps_path_is_directory_ext("/nv");
  if( 0 != result )
  {
    ...
    if ( -1 == result )
    {
      DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_create_efs_config_file(): "
                      "Create EFS Dir Failed: error %d", mcfg_fs_errno( file_type ));
      return mcfg_fs_errno( file_type );
    } 
  }

  result = ps_path_is_directory_ext("/nv/item_files");
  if( 0 != result )
  {
    /* Directory doesn't exist yet */
    DS_MSG0(MSG_LEGACY_MED,"ps_create_efs_config_file(): "
  ...
  result = ps_path_is_directory_ext("/nv/item_files/conf");
  if( 0 != result )
  {
    /* Directory doesn't exist yet */
    DS_MSG0(MSG_LEGACY_MED,"ps_create_efs_config_file(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
  ...
  {
    DS_MSG0(MSG_LEGACY_HIGH,"ps_create_efs_item_file(): "
  ...
  if ( 0 > config_fd )
  {
    ...
    {
      DS_MSG0(MSG_LEGACY_ERROR,"ps_create_efs_item_file(): "
                      "EFS: Config file not present");
      DS_MSG0(MSG_LEGACY_MED,"ps_create_efs_item_file(): "
                      "EFS: Creating config file");
      result = ps_create_efs_config_file_ext(conf_file_path);
      if ( 0 != result )
      {
        DS_MSG_SPRINTF_1(MSG_LEGACY_HIGH,"ps_create_efs_item_file(): "
      ...
      if ( 0 > config_fd )
      {
        DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_create_efs_item_file(): "
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(file_loc, item_file_path_size + 1, char*);
  if (NULL == file_loc)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_create_efs_item_file(): "
...
bail:  
  if (file_loc)
  {
    DS_SYSTEM_HEAP_MEM_FREE(file_loc);
  }
...
{
  //ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
  return ps_read_efs_nv_ext(item_file_path,nv_info_ptr,nv_info_size);
} /* ps_read_efs_nv() */
...
)
{
  ...
  mcfg_fs_sub_id_e_type mcfg_subs_id = MCFG_FS_SUBID_0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_MSG0(MSG_LEGACY_MED,"ps_read_efs_nv_ext(): "
                  "Reading from EFS");

  if ( NULL == item_file_path || NULL == nv_info_ptr )
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_read_efs_nv(): "
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_read_efs_nv(): "
...
{
  //ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
  return ps_write_efs_nv_ext(item_file_path,nv_info_ptr,nv_info_size);
} /* ps_write_efs_nv() */
...
)
{
  ...
  mcfg_fs_sub_id_e_type mcfg_subs_id = MCFG_FS_SUBID_0;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_MSG0(MSG_LEGACY_MED,"ps_write_efs_nv(): "
                  "Writing to EFS");

  if ( NULL == item_file_path || NULL == nv_info_ptr )
  {

    DS_MSG0(MSG_LEGACY_ERROR,"ps_write_efs_nv(): "
                   "item_file_path is NULL or nv_info_ptr is NULL");
    return -1;
 ...
 //mcfg_subs_id = ps_utils_convert_subs_ps_to_mcfg(ps_subs_id);
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_write_efs_nv(): "
...
{
  //ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
  return ps_path_is_directory_ext(dirname);
...
)
{
  ...
  if( MCFG_FS_STATUS_OK != rsp )
  {
    ...
    if( ENOENT != rsp )
    {
      DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_path_is_directory(): "
...
{
 //ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
  return ps_efs_file_init_ext(file_path,sm);
} /* ps_efs_file_init() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"ps_efs_file_init(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(NULL == sm)
  {
    DS_MSG0(MSG_LEGACY_MED,"ps_efs_file_close_ext(): "
                    "Input parameters are NULL!");
    return;
  }
...
{
  //ps_sys_subscription_enum_type ps_subs_id = PS_SYS_DEFAULT_SUBS;
  return ps_efs_tokenizer_ext(sm,begin,end);
}/* ps_efs_tokenizer() */
...
)
{
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_tokenizer(): "
  ...
  *end   = 0; 
  ...
  {      
    ...
    {
      DS_MSG0(MSG_LEGACY_MED,"ps_efs_tokenizer(): "
  ...
  -----------------------------------------------------------------------*/
  {
    ...
    if( bytes_read > 0 ) 
    {
      ...
      if(*begin != 0)
      {
        retval= ps_efs_tokenizer_ext( sm, &dummy, end ); /* Call the function 
                                              again because you could be in the
                                              middle of reading a token */
      }
      else
      {
        retval = ps_efs_tokenizer_ext( sm, begin, end);
      }
...
)
{
  ...
  *config_fd = mcfg_fopen (config_path, 
  ...
  if ( 0 > *config_fd )
  {
    if ( ENOENT == mcfg_fs_errno( file_type ) ) /* Conf file does not exist */
    {
      DS_MSG0(MSG_LEGACY_MED,"ps_cfg_get_file_ext(): EFS: Config file not present");
      ret_val = ps_create_efs_config_file_ext(config_path);
      if ( 0 != ret_val )
      {
        DS_MSG_SPRINTF_1(MSG_LEGACY_MED,"ps_cfg_get_file_ext(): EFS: Error creating config file "
                        "result %d", ret_val);
        return ret_val;
      }
      *flag_new = TRUE;
      ...
      *config_fd = mcfg_fopen (config_path,
      ...
      if ( 0 > *config_fd )
      {
        DS_MSG_SPRINTF_1(MSG_LEGACY_MED,"ps_cfg_get_file_ext(): EFS: Error opening config file, "
    ...
    {
      DS_MSG0(MSG_LEGACY_HIGH,"ps_cfg_get_file_ext(): Error opening config file, ");
      return -1;
    }
  }
...
)
{
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_get_conf_string() : Failed to allocate memory"
                     	 );
    return -1;
  }
  ...
  for (index = 0; index < ps_efs_conf_items; index ++)
  {
    ...
    {
      DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR, "Position %d exceeds max length", file_path_buf_len);
      return -1;
    }							
  }
...
)
{
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_write_items(): Failed to get ps_cfg_item_info");
    return -1; 
  }
  
  if (NULL == data_conf_path)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_write_items(): Failed to get file path");
    return -1; 
  }
  ...
  do {
    ...
    if ( 0 != ret_val )
    {
      DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_efs_write_items(): Failed to get file %d", ret_val);
      ret_val = -1;
      break;
    }

    /* Allocate memory for file_read_buf */
    DS_SYSTEM_HEAP_MEM_ALLOC(file_path_buf, ps_efs_conf_items*max_path_size,
                             char*);

    if( file_path_buf == NULL )
    {
      DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_write_items(): Failed to allocate"
    ...
    if( 0 == conf_info_size )
    {
      DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_write_items(): Failed to read cfg data");
      ret_val = -1;
      break;
    }

    /* Allocate memory for file_read_buf */
    DS_SYSTEM_HEAP_MEM_ALLOC(file_read_buf, conf_info_size, char*);
    ...
    if (FALSE == flag_new_file)
    {
      ...
      if( efs_ret < 0 )
      {
        DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_write_items(): Error reading file:"  );
      }  
    }
        ...
        || TRUE == flag_new_file ) 
    {
      ...
      if( (fs_size_t)conf_info_size != mcfg_fwrite(conf_fd,
                                                file_path_buf,
                                                   ...
                                                   file_type))
      {
        DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_efs_write_items(): mcfg_fwrite error, bytes: %d", conf_info_size);
          ret_val = -1;
      }
    }    
  } while (0);
  ...
  DS_SYSTEM_HEAP_MEM_FREE( file_read_buf );
  DS_SYSTEM_HEAP_MEM_FREE( file_path_buf );
  
  /* EFS Close */
  efs_ret = mcfg_fclose (conf_fd, file_type);
  if ( 0 != efs_ret )
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_efs_write_items(): Error closing config file "
                    "result %d", ret_val);
    ret_val = efs_ret;
  }
...
)
{
  ...
  DS_MSG0(MSG_LEGACY_MED,"ps_efs_init(): Initializing EFS config item files");
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_efs_init(): Item files write failed.");
  }
...
/*SYS_MODEM_AS_ID_3*/
  ...
  DS_MSG_SPRINTF_2(MSG_LEGACY_MED,"ps_utils_convert_subs_id(): "
...
/*MMGSDI_SLOT_3*/
  ...
  DS_MSG_SPRINTF_2(MSG_LEGACY_MED,"ps_utils_convert_slot_id(): "
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_MSG_SPRINTF_2(MSG_LEGACY_MED,"ps_utils_convert_refresh_type_mcfg_to_ps(): "
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_utils_mcfg_refresh_ind_cb(): NULL refresh pointer");
    return FALSE;
  }
  ...
  /*-----------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------*/
  event_info = ds_system_heap_mem_alloc( sizeof(mcfg_refresh_info_s_type),__FILENAME__ , __LINE__ );

  if(NULL == event_info)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_utils_mcfg_refresh_ind_cb(): Allocation failed");
    return FALSE;
  }
...
)
{
  ...
  do
  {
    ...
    ps_refresh_info.refresh_subs = DS_SYS_PRIMARY_SUBS;
    ...
    if (TRUE == ps_utils_reg_clients.is_initialized)
    {
                   ...
                   client_count < ps_utils_reg_clients.total_reg_count ;client_count++)
      {
        ...
        if (NULL != ps_utils_reg_clients.clients_reg_data[client_count].func_ptr)
        {
          ...
          DS_MSG_SPRINTF_1(MSG_LEGACY_MED,"ps_utils_mcfg_refresh_ind_handler() :"
    ...
    -------------------------------------------------------------------------*/
    DS_SYSTEM_HEAP_MEM_FREE(data_ptr);
...
void ps_utils_refresh_init ( void )
{
 ...
 /*-------------------------------------------------------------------------
  ...
  DS_MSG_SPRINTF_1(MSG_LEGACY_MED,"ps_utils_refresh_init(): Init Complete is %d",
...
)
{
  ...
  DS_MSG_SPRINTF_3(MSG_LEGACY_MED,"ps_utils_refresh_reg(): "
