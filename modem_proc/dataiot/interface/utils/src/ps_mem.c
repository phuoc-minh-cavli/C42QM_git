...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/utils/src/ps_mem.c#1 $
  $DateTime: 2020/03/17 08:03:16 $Author: $
...
---------------------------------------------------------------------------*/
static ds_crit_sect_type  ps_mem_crit_section;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_memi_pool_init(): "
  ...
  if (buf_memory == NULL)
  {
    ...
    DS_MSG0(MSG_LEGACY_ERROR,"ps_memi_pool_init(): "
                    "NULL ptr");
    return -1;
  }
  ...
  {
    MEM_INC_INSTANCE_STATS(pool_id, alloc_errors, 1);
    DS_MSG_SPRINTF_3(MSG_LEGACY_ERROR,"ps_memi_pool_init(): "
  ...
  DS_ENTER_CRIT_SECTION(&ps_mem_crit_section);
  ...
  *last = NULL;   /* Last buffer in the pool */
  ...
  DS_LEAVE_CRIT_SECTION(&ps_mem_crit_section);
  return 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_INIT_CRIT_SECTION(&ps_mem_crit_section);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_DESTROY_CRIT_SECTION(&ps_mem_crit_section);
...
)
{
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_get_buf():poolid: %d"
  ...
  {
    DS_SYSTEM_HEAP_MEM_ALLOC(buf_hdr, 
  ...
  if ( buffer != NULL )
  {
         ...
         ps_mem_pool_info[poolid].num_used )
    {
      ...
      DS_MSG_SPRINTF_2(MSG_LEGACY_HIGH,"ps_mem_get_buf(): "
    ...
    DS_BRANCH_LEAVE_CRIT_SECTION(&ps_mem_crit_section);
  ...
  else
  {
    ...
    DS_BRANCH_LEAVE_CRIT_SECTION(&ps_mem_crit_section);
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_get_buf(): "
                    "Out of memory in pool %d", poolid);
    if( ps_mem_pool_info[poolid].is_static == FALSE )
    {
      DS_SYSTEM_HEAP_MEM_FREE(buf_hdr);
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (buf_ptr == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_is_valid(): "
                    "NULL ptr");
    return FALSE;
  }
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_is_valid(): "
  ...
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_mem_is_valid(): "
  ...
  else
  {
    ...
    if ((int *) ps_mem_pool_info[poolid].first_element[index] !=
          (int *) buf_hdr)
    {
      DS_MSG_SPRINTF_3(MSG_LEGACY_ERROR,"ps_mem_is_valid(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (buf == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_free(): "
                    "Null buffer");
    return;
  }
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_free(): "
  ...
  *buffer = (int) ps_mem_pool_info[poolid].free_list;
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_MED,"ps_mem_free(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_mem_reg_mem_avail_cb(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(poolid >= PS_MEM_MAX_POOLS)
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_mem_less_than_few_free_items(): "
                    "Pool ID %d meets or exceeds MAX POOLS %d",
                    (uint32)poolid, (uint32)PS_MEM_MAX_POOLS);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(poolid >= PS_MEM_MAX_POOLS)
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_index_to_buf(): Invalid poolid %d", poolid);
    ASSERT(0);
    return NULL;
  }
  ...
  if (index < 0 || index >= ps_mem_pool_info[poolid].total_num)
  {
    DS_MSG_SPRINTF_2(MSG_LEGACY_HIGH,"ps_mem_index_to_buf(): "
  ...
  if( ps_mem_pool_info[poolid].is_static == TRUE )
  {
    ...
    {
      DS_MSG_SPRINTF_2(MSG_LEGACY_ERROR,"ps_mem_index_to_buf(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == buf)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_buf_to_index(): "
                    "NULL buf");
    return -1;
  }
  ...
  {
    DS_MSG_SPRINTF_3(MSG_LEGACY_ERROR,"ps_mem_buf_to_index(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( buf == NULL )
  {
    DS_MSG0(MSG_LEGACY_HIGH,"ps_mem_dup(): "
                    "NULL ptr");
    return NULL;
  }
  ...
  buf_hdr =  ((ps_mem_buf_hdr_type *)buf) - 1;
  poolid = buf_hdr->poolid;
  DS_MSG3(MSG_LEGACY_HIGH,"ps_mem_dup(): "
                    "poolid %d buf 0x%p buf_hdr 0x%p",poolid,buf,buf_hdr);
  if (!ps_mem_is_valid(buf, (ps_mem_pool_enum_type)poolid))
  {
    DS_BRANCH_LEAVE_CRIT_SECTION(&ps_mem_crit_section);
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_dup(): "
                    "Invalid buf 0x%p", buf);
    DS_ASSERT(0);
    return NULL;
  }
...
)
{
  ...
  {
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_ext_mem_get_buf(): "
...
)
{
  ...
  {
    DS_ERROR_FATAL("ps_mem_get_stat_instance_all(): Invalid stat value passed to ps_mem_get_stat_i_all");
  ...
  {
    DS_ERROR_FATAL("ps_mem_get_stat_instance_all:Null value of pool_info_ptr passed");
    return E_NOT_ALLOWED ;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_get_stat_instance_all(): "
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx = (PS_MEM_DUMMY_POOL_TYPE + 1); idx < PS_MEM_MAX_POOLS; ++idx)
    {
      ...
      if (log_pkt_avail_size < desc_size)
      {
        ...
        if (NULL == log_pkt_ptr)
        {
          DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_get_desc_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    ...
    for (idx = (PS_MEM_DUMMY_POOL_TYPE + 1); idx < PS_MEM_MAX_POOLS; ++idx)
    {
      ...
      if (NULL == log_pkt_ptr)
      {
        DS_MSG0(MSG_LEGACY_ERROR,"ps_mem_get_stat_log_pkt(): "
  ...
  else
  {
    ...
    {
      DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_get_stat_log_pkt(): "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (poolid != PS_MEM_DUMMY_POOL_TYPE)
  {
    ...
    DS_MSG_SPRINTF_1(MSG_LEGACY_ERROR,"ps_mem_pool_free(): Invalid poolid %d", poolid);
    DS_ASSERT(0);
    return;
  }
