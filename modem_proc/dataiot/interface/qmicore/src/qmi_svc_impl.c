...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/qmi_svc_impl.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( errval == QMI_ERR_NONE_V01 )
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_ERROR, "Not enough memory to allocate for "
  ...
  if (errval == QMI_ERR_NONE_V01)
  {
    if ( QMI_IDL_LIB_NO_ERR != qmi_idl_message_encode(svc_obj,
                                                 ...
                                                 &rsp_enc_len) )
    {
      DS_MSG2(MSG_LEGACY_ERROR, "Message Encode Failed for QMI_GET_SUPPORTED_MSGS, "
                       "MsgID:0x%x, Msg:0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( errval == QMI_ERR_NONE_V01 )
  {
    ...
    if ( 0 != qmi_result )
    {
      DS_MSG1(MSG_LEGACY_ERROR, "Message Decode Failed for QMI_GET_SUPPORTED_FIELDS,"
  ...
  if (errval == QMI_ERR_NONE_V01)
    ...
    if ( QMI_IDL_LIB_NO_ERR != qmi_idl_message_encode(svc_obj,
                                                   ...
                                                   &rsp_enc_len) )
    {
      DS_MSG2(MSG_LEGACY_ERROR, "Message Encode Failed for QMI_GET_SUPPORTED_FIELDS,"
                       "MsgID:0x%x, Msg:0x%x",
