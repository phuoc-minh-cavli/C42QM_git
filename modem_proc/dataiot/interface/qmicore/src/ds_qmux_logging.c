...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/ds_qmux_logging.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(item_ptr == NULL) 
  {
    ...
    -----------------------------------------------------------------------*/
    DS_MSG0(MSG_LEGACY_ERROR,"NULL item ptr, frame skipped.");
    return;
  }

  if (qmi_instance >= QMI_INSTANCE_MAX) 
  {
    DS_MSG1(MSG_LEGACY_ERROR,"invalid instance! %d", qmi_instance);
    DS_ASSERT(0);
  }

  if(direction == QMUX_TX) 
  {
    DS_MSG0(MSG_LEGACY_MED,"Logging QMUX_TX");
  }
  else if(direction == QMUX_RX)
  {
    DS_MSG0(MSG_LEGACY_LOW,"Logging QMUX_RX");
  }
  else
  {
    DS_MSG1(MSG_LEGACY_ERROR,"invalid direction! %d", direction);
    DS_ASSERT(0);
  }
  ...
  if(log_ptr == NULL) 
  {
    DS_MSG0(MSG_LEGACY_LOW,"Can't log QMI packet - no diag bufs!");
    return;
  }
  ...
  if(dsm_seek_extract(&item_ptr, 
                      ...
                      &len) == FALSE)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"seek extract failed!");
    log_free(log_ptr);
    return;
  }
