...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/ds_qmi_svc.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  // free here 
  DS_MSG0(MSG_LEGACY_ERROR,"No Memory - Unable to put param TLV");
  return FALSE;
...
)
{
  ...
  DS_MSG2(MSG_LEGACY_ERROR,"No Memory - Unable to put param TLV file id %d,line %d",file_id,line);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    /*-----------------------------------------------------------------------
      ...
      caller
    -----------------------------------------------------------------------*/
    DS_MSG1(MSG_LEGACY_ERROR,"Unable to send Response or ind for cmd %d",cmd_val);
    dsm_free_packet(pkt);
    return FALSE;
  }
} /* qmi_svc_prepend_msg_hdr() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Too many commands in transaction");
    return NULL;
  }
  ...
  if (x_p->cl_sp)  // cl_sp != NULL means this transaction is client specific
  {
	...
	if (NULL == cmd_buf_p)
	{
      DS_MSG0(MSG_LEGACY_ERROR,"No command buffers available!");
      return NULL;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *cmd_buf_handle = NULL; // free the callers reference immediately
  ...
  DS_MSG1(MSG_LEGACY_HIGH,"Freeing QMI command buffer %x", freeing_cmd_buf_p);

  if (freeing_cmd_buf_p)
  {
    DS_ASSERT(freeing_cmd_buf_p->x_p != NULL);
    /* check that freeing_cmd_buf_p->x_p is not NULL in case ASSERT is removed */
    if (freeing_cmd_buf_p->x_p == NULL)
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Freeing cmd_buff pointer NULL!");
      return;
    }
    cl_sp = freeing_cmd_buf_p->x_p->cl_sp;

    /* remove reference to this command buffer from transaction */
    DS_ASSERT(freeing_cmd_buf_p->cmd_no < MAX_MSGS_IN_BUNDLE );
    /* check that cmd_no is in valid range in case ASSERT is removed */
    if (freeing_cmd_buf_p->cmd_no >= MAX_MSGS_IN_BUNDLE)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"Freeing cmd_buff cmd no invalid (%d)!",
                       freeing_cmd_buf_p->cmd_no);
      return;
    }

    DS_MSG3(MSG_LEGACY_LOW,"About to clear reference to cmd_buf_p (%x) from x_p(%x:%d)",
                     freeing_cmd_buf_p,
                     freeing_cmd_buf_p->x_p,
                     freeing_cmd_buf_p->cmd_no );
    DS_ASSERT(freeing_cmd_buf_p != NULL);
    ...
    if (freeing_cmd_buf_p->x_p == NULL)
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Freeing cmd_buff transaction pointer NULL!");
      return;
    }
    ...
    DS_MSG3(MSG_LEGACY_LOW,"Check if need to clear pending response (%x) from x_p(%x:%d)",
                     freeing_cmd_buf_p,
                     freeing_cmd_buf_p->x_p,
                     freeing_cmd_buf_p->cmd_no );
    ...
    {
      DS_MSG0(MSG_LEGACY_HIGH,"Freeing cmd_buf for global transaction");
    }
    
  }
  else
  {
    DS_MSG0(MSG_LEGACY_HIGH,"Attempt to free NULL command buffer!");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else /* if no client sp, must be a broadcast indication */
  {
    ...
    if(svc_sp->bcast_transaction_buf_free == FALSE)
    {
      ...
      ---------------------------------------------------------------------*/
      DS_ERROR_FATAL("Broadcast indication in use!");
      return NULL;
    }

    DS_MSG0(MSG_LEGACY_HIGH,"allocating the global transaction");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (*x_p == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Attempt to free a NULL transaction pointer!");
    return;
  }
  svc_sp = (qmi_common_svc_state_type *) (*x_p)->svc_sp; 
  ...
  if (svc_sp == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Ignoring free transaction, "
  ...
  for( i = 0; i < MAX_MSGS_IN_BUNDLE; i++ )
  {
    ...
    if ((*x_p)->cmd_list[i])
    {
      ...
      else
      {
        ...
        -------------------------------------------------------------------*/
        free_x = FALSE;
        DS_MSG0(MSG_LEGACY_ERROR,"Freeing transaction while command in progress - deferred.");
  ...
  if (free_x)
  {
    ...
    if (cl_sp)
    {
      DS_ASSERT(*x_p != &svc_sp->bcast_transaction);
    ...
    else
    {
      ...
      ---------------------------------------------------------------------*/
      DS_MSG0(MSG_LEGACY_ERROR,"free the global transaction");
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Transaction in process, "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"No available service clids!" );
  ...
  DS_MSG2(MSG_LEGACY_HIGH,"Alloc clid %d for service %x", cl_sp->clid, svc_sp);
  cl_sp->seq_no = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(svc_sp != NULL);
  DS_ASSERT(cl_sp != NULL);
  svc_handle = (qmux_svc_info_type *)svc_sp->qmux_svc_handle;

  DS_ASSERT(svc_handle->cfg.cbs.reset_client != NULL);
  DS_MSG1(MSG_LEGACY_ERROR,"Dealloc service clid %d", cl_sp->clid);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_ERROR,"QMI link closed, do service cleanup");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(sp != NULL);
  DS_ASSERT(qmux_svc_handle != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(svc_sp != NULL);
  svc_handle = (qmux_svc_info_type *)svc_sp->qmux_svc_handle;
  DS_ASSERT(svc_handle != NULL);

  DS_ASSERT(cmd_buf_p != NULL);
  DS_ASSERT(sdu_in != NULL);

  DS_MSG2(MSG_LEGACY_LOW, "Got QMI Service cmd type %d (%d bytes)",
  ...
  {
    DS_MSG2(MSG_LEGACY_MED, "Unrecognized type (=%d) for service (=%d)! Return error",
  ...
  switch (cmd_buf_p->x_p->ctl & QMI_FLAG_MASK_MSGTYPE)
  {
    ...
    case QMI_FLAG_MSGTYPE_IND:
      ...
      break;
    default:
      DS_MSG1(MSG_LEGACY_HIGH,"Invalid command type! (%d)",
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"Unhandled (%d cmd) msg type (%d)",
                     cmd_buf_p->x_p->ctl&QMI_FLAG_MASK_MSGTYPE,
                     cmd_buf_p->cmd_type);
    response_ptr = NULL;
  }
...
send_result:
  ...
  {
    DS_MSG0(MSG_LEGACY_MED, "(Service generated no response)");
    qmi_svc_free_cmd_buf(&cmd_buf_p);
    return FALSE;
  }
  else if (response_ptr == QMI_SVC_RESPONSE_PENDING)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Response pending for cmd_buf_p %p", cmd_buf_p);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    ...
    {
      DS_MSG3(MSG_LEGACY_MED,"Transaction %x Command #%d (%x) processing failed",
                       x_p, i, cmd_buf_p);
      free_t = TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == cl_sp)
  {
    ...
    DS_MSG2(MSG_LEGACY_HIGH,"Got QMI servce %x message for unassigned client %d! discarding",
                     svc_sp, clid);
    dsm_free_packet (&sdu_in);
    return;
  }
  ...
  if (msg_x_id == 0)
  {
    ...
    DS_MSG2(MSG_LEGACY_ERROR, "Dropping pkt with x_id '0' (svc_sp = 0x%x, cl_id = %d)",
                     svc_sp, clid );
    dsm_free_packet(&sdu_in);
    return;
  }
  ...
  {
    DS_MSG3(MSG_LEGACY_LOW, "Recvd request bundle (svc_sp = 0x%x, x_id = %x, ctl = %x)",
  ...
  if (x_p == NULL)
  {
    DS_MSG1(MSG_LEGACY_MED, "No transaction container!  Discarding service %x message",
                     svc_sp);
    dsm_free_packet(&sdu_in);
    return; 
  }
  ...
  while (sdu_in)
  {
    ...
    if (temp == -1)
    {
      DS_MSG0(MSG_LEGACY_MED,"Short QMI svc header!  Discarding transaction");
      dsm_free_packet( &sdu_in );
      qmi_svc_free_transaction( &x_p );
      return;
    }
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"Short QMI svc payload!  Discarding service %x message",
                       svc_sp);
      dsm_free_packet( &sdu_in );
      qmi_svc_free_transaction( &x_p );
      return;
    ...
    if (cmd_buf_p == NULL)
    {
      DS_MSG1(MSG_LEGACY_MED,"No QMI command buffer!  Discarding service %x message",
                       svc_sp);
      dsm_free_packet(&sdu_in);
      qmi_svc_free_transaction( &x_p );
      return;
  ...
  DS_MSG3(MSG_LEGACY_LOW,"Dispatch the transaction(%x) with (%d cmds) ctl= %x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (*cmd_buf_p) != x_p->cmd_list[(*cmd_buf_p)->cmd_no] )
  {
    ...
    -----------------------------------------------------------------------*/
    DS_ERROR_FATAL("Command is linked to transaction it doesn't belong to! discarding");
    dsm_free_packet( msg );
    qmi_svc_free_cmd_buf( cmd_buf_p );
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT( QMI_INSTANCE_MAX > qmi_instance );
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI instance out of range (%d)!  Discarding",
                    qmi_instance);
    return FALSE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI service ID not valid (%d)!  Discarding", svc_id);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      msg_type != QMI_FLAG_MSGTYPE_RESP)
  {
    ...
    DS_MSG0(MSG_LEGACY_ERROR,"Can't send msg_type %d in response to command!");
  ...
  else
  {
                                ...
                                cmd_buf_p->cmd_type ) )
    {
      ...
      DS_MSG3(MSG_LEGACY_MED,"Filtered QMI message: type=%d svc=%d msg=%d",
  ...
  {
    DS_ERROR_FATAL("Unable to push pending response to send queue");
    DS_ASSERT(msg_ptr == NULL); // msg_ptr is freed in queue_response
    return FALSE;
  }
  ...
  for( i = 0; i < x_p->n_cmds; i++ )
  {
    DS_ASSERT( x_p->resp_list[i] != QMI_SVC_RESPONSE_PENDING );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Unable to send Response for cmd %d",
                     cmd_buf_p->cmd_type);
    /*-----------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Client %d invalid/already released, dropping indication",
                     clid);
    dsm_free_packet(&ind);
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"No transaction buffer to send indication!");
  ...
  if (cl_sp != NULL)
  {
    ...
    DS_MSG2(MSG_LEGACY_MED,"Client id %d, seq no: %d", cl_sp->clid, x_p->x_id);
  }

  cmd_buf_p = qmi_svc_new_cmd_buf(x_p);
  if (cmd_buf_p == NULL)
  {
    DS_MSG0(MSG_LEGACY_MED,"No command buffer to send indication!");
    dsm_free_packet(&ind);
    qmi_svc_free_transaction(&x_p);
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Unable to send the indication!");
    /*-----------------------------------------------------------------------
      Note - no need to free msg_ptr, taken care of in 
             qmi_svc_prepend_msg_hdr()
    -----------------------------------------------------------------------*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Sent the indication (for cmd=%d)!");
  }
...
/*-------------------------------------------------------------------------*/
  ...
  DS_ASSERT(common_msg_hdr != NULL);
  DS_ASSERT((qmi_cmd == QMI_CMD_GET_CLIENT_ID_RESP) ||
         (qmi_cmd == QMI_CMD_RELEASE_CLIENT_ID_RESP));  
  
  cmd_ptr = qmi_get_cmd_data_buf();
  if (cmd_ptr == NULL)
  {
    DS_MSG2(MSG_LEGACY_ERROR,"No cmd buffer! service %d client ID:%d response message ",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG2(MSG_LEGACY_MED,"Sending alloc clid %d result from QMI Legacy service  %d",
...
/*-------------------------------------------------------------------------*/
  ...
  DS_MSG2(MSG_LEGACY_MED,"Sending dealloc clid %d result from QMI Legacy service %d",
                   common_msg_hdr->client_id, common_msg_hdr->service);
...
)
{
  ...
  if(NULL == svc_cfg )
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid service handle");
  ...
  DS_MSG1(MSG_LEGACY_MED,"Assigned client ID %d", common_msg_hdr.client_id);
...
{ 
  ...
  if (svc_cfg->cbs.dealloc_clid)
  {
    ...
    {
	  DS_MSG0(MSG_LEGACY_ERROR,"qmi_svc_dealloc_clid() failed after call to service dealloc_clid callback!");
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *out_spec_ptr = NULL;
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid qmi inst %d", qmi_inst);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *out_spec_ptr = NULL;
  ...
  for (index = 0; index < QMI_SVC_PWRSAVE_SPECS_MAX; index++)
  {
    ...
    if (spec_ptr == NULL)
    {
      DS_SYSTEM_HEAP_MEM_ALLOC(spec_ptr, sizeof(qmi_svc_power_spec_s_type),
  ...
  DS_MSG1(MSG_LEGACY_MED,"No spec slot available inst %d", qmi_inst);
  return FALSE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(ind_list != NULL);

  /* check that service_handle is not NULL in case ASSERT is removed */
  if (service_handle == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI_SVC service_handle pointer NULL!");
    return FALSE;
  }
  ...
  if (ind_list == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI_SVC ind_list pointer NULL!");
    return FALSE;
  ...
  qmi_instance = qmi_instance_by_qmux_handle (svc_s);
  DS_ASSERT (qmi_instance < QMI_INSTANCE_MAX);
  ...
  if( svc_id >= QMUX_SERVICE_MAX )
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Service specified not valid: %d", svc_id);
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Can't set NORMAL state spec");
    return FALSE;
  }
  ...
  DS_MSG1(MSG_LEGACY_MED,"Storing power save spec: handle=0x%X", state_handle);
  spec_ptr->state_handle = state_handle;

  /* Free old ind spec */
  if (spec_ptr->svc_inds[j] != NULL)
  {
    DS_SYSTEM_HEAP_MEM_FREE(spec_ptr->svc_inds[j]);
  }

  /* Allocate new ind spec. One extra element for num_inds */
  DS_SYSTEM_HEAP_MEM_ALLOC(spec_ptr->svc_inds[j], (num_inds + 1) * sizeof(uint16),
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(old_state_handle != NULL);
  
  if (qmi_instance >= QMI_INSTANCE_MAX)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Instance specified not valid: %d", qmi_instance);
    return FALSE;
  }
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"Setting power save state: new=0x%X old=0x%X",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(service_handle != NULL);
  DS_ASSERT(old_state_handle_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(service_handle != NULL);
  DS_ASSERT(state_handle_ptr != NULL);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR, "Invalid instance specified: %d", instance );
    DS_ASSERT (0);
    return;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR, "Specified instance initialized, ignoring power event: %d",
                     	 instance );
    return;
  }
  ...
  if( !qmi_set_pwrsave_state_by_instance( instance,
					  ...
					  &old_state_handle ) )
  {
    DS_MSG1(MSG_LEGACY_MED,"Failed to set new power state: %d", instance);
  }
  else
  {
    ...
    if( old_state_handle != state_handle )
    {
      if( !qmi_ctl_pwrsave_mode_change_notify( instance,
					       ...
					       old_state_handle ) )
      {
        DS_MSG1(MSG_LEGACY_MED,"Failed to notify on power state change: %d",
                        instance);
      }
    }
    else
    {
      DS_MSG1(MSG_LEGACY_ERROR,"No power state change, skipping notification: %d",
                      instance);
    }
  }
...
)
{
  ...
  if (NULL == spc_code)
  {
    DS_MSG0(MSG_LEGACY_ERROR, "Unable to function on NULL SPC code" );
    return retval;
  }

  DS_MSG2(MSG_LEGACY_MED, "Initiating check with spc_code_ptr=0x%x and otksl_flag=%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (service_handle == NULL)
  {
    DS_ERROR_FATAL("QMI_SVC service_handle pointer is NULL!");
    return FALSE;
  }
  ...
  if (state_handle == QMI_SVC_PWRSAVE_STATE_NORMAL)
  {
    DS_MSG0(MSG_LEGACY_MED,"Can't set NORMAL state spec");
    return FALSE;
  }
  ...
  /* Store power save spec */
  DS_MSG2(MSG_LEGACY_MED,"Storing power save spec: handle=0x%X, "
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_ERROR_FATAL("QMI_SVC service_handle pointer NULL!");
    return FALSE;
  }
...
)
{
  ...
  {
    DS_MSG4(MSG_LEGACY_MED,"Invalid TLV len (%d) for type (%d) file id (%d) line (%d)",length,type,file_id,line);
    return FALSE;
  }
  return TRUE;
}
 ...
 )
 {
  ...
  /*--------------------------------------------------------------------------------*/
   if( msg_ptr != NULL && *msg_ptr != NULL ) // We reutrn QMI_ERR_NONE_V01 if msg_ptr is NULL. 
   {
     ...
     DS_MSG1(MSG_LEGACY_MED,"decode:dsm_pullup size %d",size);

     if ( size > 0 )
     {
       DS_SYSTEM_HEAP_MEM_ALLOC(buf,size,uint8*);
       if ( buf != NULL )
       {
         ...
         else
         {
           ...
           DS_MSG1(MSG_LEGACY_ERROR,"decode: dsm_pullup failed size %d", size);
         }
         DS_SYSTEM_HEAP_MEM_FREE(buf);
       }
       else
       {
         DS_MSG1(MSG_LEGACY_ERROR,"decode: no memory for buffer %d",size);
	 ...
	 {
	   DS_MSG2(MSG_LEGACY_ERROR,"decode: failed retval (%d) errval (%d)", retval, errval);
     }
   }
    return errval;
...
)
{
  ...
  {
    DS_SYSTEM_HEAP_MEM_ALLOC(rsp_buf,rsp_buf_len,uint8*);
    if ( rsp_buf != NULL && p_src != NULL)
    {
      ...
      if ( err_val == QMI_IDL_LIB_NO_ERR )
      {
        ...
        {
          DS_MSG1(MSG_LEGACY_ERROR,"Pushdown Failed for encode %d", size_buf);
  ...
  DS_MSG3(MSG_LEGACY_MED,"encode eval (%d) err_val (%d) retval (%d)",eval,err_val,retval);
  if ( rsp_buf != NULL ) 
  {
    DS_SYSTEM_HEAP_MEM_FREE(rsp_buf);
  }
  return retval;
