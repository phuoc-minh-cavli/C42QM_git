...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/ds_qmi_svc_ext.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *tx_rate = 0;
  ...
  switch( supp_channel_info.supp_channel )
  {
    ...
    {
      //qmi_if_get_wcdma_default_channel_rate(tx_rate, rx_rate ); Not for Chiltepin
      break;
    }
     case QMI_CHANNEL_SUPP_TDSCDMA:
    {
      //qmi_if_get_tdscdma_default_channel_rate(tx_rate, rx_rate ); Not for chiltepin
      break;
    }
#ifdef FEATURE_LTE
    ...
    default:
    {
      *rx_rate = 0;
      *tx_rate = 0;
      DS_MSG0(MSG_LEGACY_ERROR, "Invalid channel support type!");
      break;
    }
  }
...
#ifdef FEATURE_DATA_QMI_WDS_AUTOCONNECT
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  DS_SYSTEM_HEAP_MEM_ALLOC(dcc_nv_item_ptr, sizeof(nv_item_type), nv_item_type*);
                                      ...
                                      sizeof(nv_enable_check)))
  {
    ...
    if (NV_DONE_S == ds_get_nv_item(NV_DS_MIP_ACTIVE_PROF_I, dcc_nv_item_ptr))
    {
  ...
  -------------------------------------------------------------------------*/
  DS_SYSTEM_HEAP_MEM_FREE(dcc_nv_item_ptr);

  return allow_auto;
}
#endif /* FEATURE_DATA_QMI_WDS_AUTOCONNECT */
