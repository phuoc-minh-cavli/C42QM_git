...
/*===========================================================================
   ...
   $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/qmi_sap_xport_qmux.c#1 $ 
...
{
  DS_MSG2(MSG_LEGACY_MED,"get_next_cl_sp [%p] [%p]: not supported", sp, cl_sp);
  return NULL;
...
{
  DS_MSG1(MSG_LEGACY_ERROR,"reset client [%p] - not supported", cl_sp_in);
  return;
 ...
 )
{
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(msg, len, void*);
  if(msg)
  {
    ...
    {
      /* ERROR */
      DS_MSG2(MSG_LEGACY_ERROR,"Error encountered receiving message from clid %d in QSAP! (%d)",
                      clid, rc);
      /* Send an error response */
    }
    DS_SYSTEM_HEAP_MEM_FREE(msg);
...
)
{
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid xport handle [%p] ", xh);
    return;
  }
  ...
  do
  {
         ...
         qmi_qsapi_is_valid_clid_cb(sp, clid) )
    {
      ...
      {
        DS_MSG2(MSG_LEGACY_ERROR,"Failed to dealloc service [%d] clid [%d]",
                        svc_info->service_id, clid);
      }
      else
      {
        DS_MSG2(MSG_LEGACY_MED,"Dealloc service [%d] clid [%d]",
                        svc_info->service_id, clid);
      }
    } 
  } while (clid++ != 0xFF);
...
)
{
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(xh, sizeof(qmi_sap_xport_handle),
                           qmi_sap_xport_handle*);
 ...
 )
{
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service ID %d", service_id);
    xh->common.qmux_svc_handle = NULL;
  }

  if(!xh->common.qmux_svc_handle)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QSAP Call to qmux_reg_service failed!");
 ...
 )
{
  ...
  if ( (xh == NULL) || (xh->common.qmux_svc_handle == NULL) ||
       (len < 5) )
  {
    DS_MSG2(MSG_LEGACY_MED,"Invalid QSAP handle 0x%p or len %d", handle, len);
    DS_ASSERT(0);
    return QMI_SAP_INTERNAL_ERR;
  }
 ...
 )
{
  if (handle)
  {
    DS_SYSTEM_HEAP_MEM_FREE(handle);
  }
}/* qsap_xport_close */
