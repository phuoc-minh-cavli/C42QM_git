...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/ds_qmux.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
#define qmi_qmux_free_cmd_buf(buf_ptr) DS_SYSTEM_HEAP_MEM_FREE(buf_ptr)
...
{
  uint8              i;
  qmi_cmd_init_legacy_services_type * cmd_payload = NULL;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Failed to initialize QMI EFS NV conf file");
  }
  ...
  /*-----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
   cmd_ptr = ds_allocate_cmd_buf(sizeof(qmi_cmd_init_legacy_services_type));
   if ((cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL))
   {
     DS_ERR_FATAL("Out of memory");
     return FALSE;
    }
    cmd_ptr->hdr.cmd_id = DS_QMI_INIT_LEGACY_SERVICES_CMD;
    cmd_payload = (qmi_cmd_init_legacy_services_type *)cmd_ptr->cmd_payload_ptr;
	
  cmd_payload->num_qmi_instance = QMI_INSTANCE_MAX;
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Couldn't initialize QMI_PWRMGMT_SHIM module!");
  }
#endif /* FEATURE_DATA_QMI_APPS_POWER_COLLAPSE */
...
{
  DS_MSG0(MSG_LEGACY_MED,"qmux_reg_vs_service_list is deprecated");
  return TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Service ID %d out of range!", service);
    return NULL;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI instance out of range (%d)!", qmi_instance);
    return NULL;
  }
  if( qmux_powerup_inited != TRUE )
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX Powerup initialization has not happened!");
    return NULL;
  }
  ...
  if (cfg == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI service cfg is NULL!");
    return NULL;
  }

  if( !qmux_state[qmi_instance].initialized )
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI link %d not initialized!", qmi_instance);
    return NULL;
  }
  ...
  if ( svc_s == NULL)
  {
    DS_SYSTEM_HEAP_MEM_ALLOC(svc_s, sizeof(qmux_svc_info_type),
                             qmux_svc_info_type*);
    if (svc_s == NULL)
    {
      return NULL;
  ...
  {
    DS_MSG2(MSG_LEGACY_ERROR,"Service %d already registered with QMUX#%d!",
  ...
  if ( ((cfg->cbs.recv == NULL) && 
       ...
       ((cfg->cbs.recv != NULL) && 
        ((cfg->cmd_hdlr_array != NULL) || (cfg->cmd_num_entries != 0))) )*/
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid QMI service config");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (service_handle == NULL)
  {
    ...
    DS_MSG0(MSG_LEGACY_ERROR,"Deregistering NULL service handle!");
	return;
  }
  ...
  if (qmi_instance >= QMI_INSTANCE_MAX)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Deregistering invalid service handle (0x%x)!",
                     service_handle);
    return;
  }
  ...
  DS_MSG2(MSG_LEGACY_MED,"Deregistering service %d from QMUX#%d",
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service [%d]", service_id);
    ASSERT(0);
    return;
  }
  ...
  if (cmd_ptr == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"No cmd buffer! service %d response message ",
                     service_id);
    ASSERT(0);
    return;
  }
  ...
  DS_MSG1(MSG_LEGACY_ERROR,"Posting cmd [%d] to QMI Modem task.",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while( fcs_num_bytes--)
  {
    ...
    {
      DS_MSG0(MSG_LEGACY_MED,"Error getting SDU octet");
      return FALSE;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_ERROR_FATAL("Error calculating FCS value");
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Out of memory on SDU frame FCS");
    return FALSE;
  }
  ...
  if( sizeof(flag) != dsm_pushdown_tail( qmux_sdu_pptr, 
                                         ...
                                         DSM_DS_SMALL_ITEM_POOL ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Out of memory on SDU frame append stop flag");
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG0(MSG_LEGACY_MED,"qmux_unframe_sdu");
  ...
  if( *pending_sdu_pptr )
  {
    DS_MSG0(MSG_LEGACY_MED,"Prepending previous SDU to current");
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Error extracting QMUX PDU start flag");
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"QMUX PDU start flag mismatch: 0x%X", flag);
    dsm_free_packet( &sdu_ptr );
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Error extracting QMUX PDU length");
    dsm_free_packet( &sdu_ptr );
    return FALSE;
  }
  ...
  if( (sdu_len-QMUX_FRAME_PDU_LEN_OVERHEAD) != pdu_len )
  {
    ...
    {
      DS_MSG2(MSG_LEGACY_MED,"SDU length greater than expected size: %d > %d",
    ...
    DS_MSG0(MSG_LEGACY_MED,"Caching current SDU for next segmet");
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Error creating QMUX PDU");
  ...
  if( !dsm_peek_byte (sdu_ptr,
		      ...
		      &expected_fcs ) )
  {
    DS_MSG0(MSG_LEGACY_MED,"Error extracting QMUX PDU FCS");
    dsm_free_packet( &pdu_ptr );
    dsm_free_packet( &sdu_ptr );
    return FALSE;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED,"Error calculating FCS on PDU");
    dsm_free_packet( &pdu_ptr );
    dsm_free_packet( &sdu_ptr );
    return FALSE;
  }
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"FCS mismatch on SDU: 0x%X != 0x%X",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"0-length QMUX SDU (%d type, clid %d)!  Discarding",
                     service, clid);
    dsm_free_packet (&qmux_sdu);
    return;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_MED, "SIO control stream not open!  freeing tx pkt");
    dsm_free_packet (&qmux_sdu);
    return;
  }
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR, "Ports not active!  freeing tx pkt");
    dsm_free_packet (&qmux_sdu);
    return;
  }
  ...
  if( IS_FRAMING_REQUIRED( iftype ) )
  {
    ...
    {
      DS_MSG2(MSG_LEGACY_MED,"Failed on PDU framing(%d type, clid %d) Discarding",
                       service, clid);
      dsm_free_packet (&qmux_sdu);
      return;
    }
...
)
{
  ...
  if (service_handle == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX send with NULL service handle!");
    dsm_free_packet (&qmux_sdu);
    return;
  }
  ...
  if (qmux_sdu == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX SDU NULL!");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI instance out of range (%d)!  Discarding", instance);
    return;
  }

  DS_MSG1(MSG_LEGACY_MED,"QMI instance %d link down - reset all services", instance);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"QMI Legacy Service cleanup on instance %d", instance);
  qmux_s = &qmux_state[instance];
...
{
  qmi_qmux_cmd_buf_type * cmd_payload = NULL;
  ds_command_type         *cmd_ptr;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_MED,"QMI clean instance %d resources", instance);
  ...
  /*-----------------------------------------------------------------------
   Get a DS command buffer
   -----------------------------------------------------------------------*/
   cmd_ptr = ds_allocate_cmd_buf(sizeof(qmi_qmux_cmd_buf_type));
   if ((cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL))
   {
     DS_MSG0(MSG_LEGACY_ERROR,"Out of memory");
     return ;
    }
    cmd_ptr->hdr.cmd_id = DS_QMI_QMUX_CMD;
    cmd_payload = (qmi_qmux_cmd_buf_type *)cmd_ptr->cmd_payload_ptr;
	
	cmd_payload->cmd_id = QMI_CMD_LEGACY_SVC_QMI_LINK_CLOSED;
	cmd_payload->data.clean_instance = instance;
	ds_put_cmd(cmd_ptr);
 /*
  cmd_ptr = (qmi_qmux_cmd_buf_type *)qmi_qmux_get_cmd_buf(QMI_CMD_LEGACY_SVC_QMI_LINK_CLOSED);
  if(cmd_ptr == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Out of QMUX cmd buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qmux_handle == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX handle NULL!");
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       qmi_instance++, qmux_s++)
  {
    ...
    if( IS_FRAMING_REQUIRED( qmux_s->io.port_info.frame_mode ) )
    {
      ...
      {
        DS_MSG1(MSG_LEGACY_ERROR,"Error on unframing SDU: %d", qmi_instance);
        continue;
      }
      ...
      if (qmux_pdu == NULL)
      {
        DS_MSG1(MSG_LEGACY_MED,"Partial PDU cached for next cycle: %d",
                         qmi_instance);
        continue;
      }
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid QMUX Service handle! (0x%x)", service_handle);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI instance %d out of range", qmi_inst);
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMUX instance %d not initialized", qmi_inst);
    return SIO_PORT_NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMI inst [%d] port not initialized", qmi_inst);
    return FALSE;
  }
  ...
  DS_MSG3(MSG_LEGACY_MED,"QMI inst [%d] stream ID [%d] DTR status [%d]",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX instance already initialized!");
    return TRUE;
  }
  ...
  DS_MSG1(MSG_LEGACY_MED, "Set control tx_wm to be infinite size: %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qmux_s == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMUX state NULL!");
    return FALSE;
  }
  ...
  if (qmux_pdu == NULL)
  {
   ...
   DS_MSG0(MSG_LEGACY_ERROR,"QMUX PDU NULL!");
	return FALSE;
  }
  
  DS_MSG2(MSG_LEGACY_MED, "Received QMUX PDU (%d bytes) on QMI link %d",
  ...
  {
    DS_MSG2(MSG_LEGACY_MED,"Invalid control frame len (%d) expected (%d)!",
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"QMI service message not handled");
    return FALSE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid QMUX type (%d)!", qmux_hdr.svc_type);
    return FALSE;
  }
  ...
  if((qmux_hdr.svc_type == QMI_SIM_INTERNAL) || (qmux_hdr.svc_type == QMI_STX))
  {
    ...
    {
      DS_MSG1(MSG_LEGACY_MED,"Request is dropped. Blocking service %d to SGLTE",
                       qmux_hdr.svc_type);
      return FALSE;
    }
  }
  ...
  {
    DS_MSG3(MSG_LEGACY_MED,"Dispatching svc_type %d msg (%d bytes) for client (%d)",
                     qmux_hdr.svc_type,
                     dsm_length_packet(*qmux_pdu),
                     qmux_hdr.clid );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( (qmux_frame_mode_type)iftype != qmux_s->io.port_info.frame_mode )
  {
    ...
    DS_MSG2(MSG_LEGACY_ERROR, "IFType configuration mismatch (%d!=%d), dropping packet",
  ...
  else
  {
	...
	break;
      ...
      default:
      {
	DS_MSG1(MSG_LEGACY_ERROR,"Invalid control frame type (%d)!", iftype);
	break;
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( QMUX_DEVSTREAM_CONTROL == qmux_s->io.port_info.qmi_stream )
  {
    ...
    {
      DS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(sioctl_config.open_param, sizeof(sio_open_param_type),
                                      sio_open_param_type*);
    ...
    {
      DS_SYSTEM_HEAP_MEM_FREE(sioctl_config.open_param);
    }
  }
  else
  {
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR, "SIO open failed on stream: %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qmux_s->io.sio_handle == SIO_NO_STREAM_ID)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"QMUX sio_handle stream ID not set (%d)!",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Could not get cmd buffer from QMI modem task for cmd id %d",
                    cmd );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG2(MSG_LEGACY_MED,"QMUX process rmnet open_close result:%d EP:0x%x",
                  result, ep_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (dtr_status == qmux_s->dtr_status)
  {
    ...
    DS_MSG2(MSG_LEGACY_MED,"Ignore DTR [%d] on QMI inst [%d]", dtr_status, inst);
    return;
  }
  qmux_s->dtr_status = dtr_status;
  QMI_LEAVE_CRIT_SECTION(&global_qmi_crit_section);

  DS_MSG2(MSG_LEGACY_ERROR,"DTR [%d] on QMI inst [%d]", dtr_status, inst);
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Can't allocate QMI cmd buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG2(MSG_LEGACY_ERROR,"Invalid qmux state [%p], inst [%d]", qmux_s, qmi_inst);
...
/*-------------------------------------------------------------------------*/
  ...
  switch(cmd_buf_ptr->cmd_id)
  {
    ...
    default:
      DS_MSG1(MSG_LEGACY_MED,"Ignoring QMI QMUX cmd %d", cmd_buf_ptr->cmd_id);
      break;
...
/*-------------------------------------------------------------------------*/
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(request, sizeof(qmi_port_request_type),
...
/*-------------------------------------------------------------------------*/
  while (qmi_port_cfg_queue != NULL && !qmi_port_cfg_in_progress)
  {
    ...
    DS_SYSTEM_HEAP_MEM_FREE(elem_ptr);
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid SIO port");
    return FALSE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Control port 0x%x open queued", sio_port);
    return TRUE;
  }

  DS_MSG2(MSG_LEGACY_MED,"Adding QMI control port 0x%x, default EP 0x%x",
  ...
  for (i = 0; i < QMI_INSTANCE_MAX; i++)
  {
        ...
        qmux_state[i].io.port_info.sio_port == sio_port)
    {
      ...
      DS_MSG1(MSG_LEGACY_ERROR,"QMI control port 0x%x exists", sio_port);
      QMI_LEAVE_CRIT_SECTION(&qmi_port_cfg_crit_section);
      return FALSE;
    }
  }
  ...
  if (!result)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Failed to add control port 0x%x", sio_port);
  }
  return result;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Control port 0x%x close queued", sio_port);
    return TRUE;
  }

  DS_MSG1(MSG_LEGACY_MED,"Removing QMI control port 0x%x", sio_port);
  ...
  if (!result)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Failed to remove QMI control port 0x%x", sio_port);
  }
  return result;
...
/*-------------------------------------------------------------------------*/
  ...
  if (ep_id == 0)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid ep_id");
    return FALSE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Data EP 0x%x open queued", ep_id);
    return TRUE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Failed to add data EP 0x%x", ep_id);
  }
  return result;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Data EP 0x%x close queued", ep_id);
    return TRUE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Failed to remove data EP 0x%x", ep_id);
  }
  return result;
