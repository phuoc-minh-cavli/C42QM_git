...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/qmi_cmd_process.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
{
  qmi_cmd_recv_legacy_svc_req_type *  cmd_payload;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
   cmd_ptr = ds_allocate_cmd_buf(sizeof(qmi_cmd_recv_legacy_svc_req_type));
   if ((cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL))
   {
     DS_ERROR_FATAL("Out of memory");
     return FALSE;
    }
    cmd_ptr->hdr.cmd_id = DS_QMI_RECV_LEGACY_SERVICES_REQ_CMD;
    cmd_payload = (qmi_cmd_recv_legacy_svc_req_type *)cmd_ptr->cmd_payload_ptr;
	
	cmd_payload->clid = qmux_hdr_clid;
	cmd_payload->svc_handle = svci;
	cmd_payload->sdu_in = sdu_in;
...
/*
  ...
  if(NULL == cmd_ptr)
  {
    DS_ERROR_FATAL("QMI can't get modem memory heap allocation");
    dsm_free_packet (&sdu_in);
    return FALSE;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  DS_ASSERT(cmd == QMI_CMD);
  DS_ASSERT(cmd_ptr->cmd.qmi.id < QMI_CMD_MAX);
  ...
  switch (cmd_ptr->cmd.qmi.id)
  {
    ...
    case QMI_CMD_DTR_STATUS:
      ...
      break;
    default:
      DS_MSG1(MSG_LEGACY_MED,"Ignoring QMI cmd %d", cmd_ptr->cmd.qmi.id);
      break;
  }
} /* qmi_process_cmd() */
