...
/*===========================================================================
   ...
   $Header: //components/rel/dataiot.mpss/2.0/interface/qmicore/src/qmi_framework.c#1 $ 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (msg_buf == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Alloc alloc_msg_buf: Can't get memory allocation!");
    DS_ASSERT((int)msg_buf);
  }
  return msg_buf;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(msg_buf == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"NULL pointer passed to qmi_frameworki_dealloc_msg_buf");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_ASSERT((int)msg_hdr);
  DS_ASSERT((int)msg_ptr);
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"No msg_buffer!  Discarding service %x message",
                     msg_hdr->common_hdr.service);
    dsm_free_packet(&msg_ptr);
    return FALSE;
  }

  DS_MSG5(MSG_LEGACY_MED,"Sending svc %d msg to QMUX: "
...
#endif
  {
    ...
    if (cmd_ptr == NULL)
    {
      DS_MSG1(MSG_LEGACY_ERROR,"No command buffer!  Discarding service %x message",
                       msg_hdr->common_hdr.service);
      dsm_free_packet(&msg_ptr);
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Sending direct service %d message to QMUX layer",
                     msg_hdr->common_hdr.service );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( qmi_svc_filter_message( (qmi_instance_e_type) msg_buf->msg_hdr.common_hdr.qmi_instance,
                               ...
                               int_cmd_id) )
  {
    DS_MSG3(MSG_LEGACY_MED,"Filtered QMI message: type=%d svc=%d ind msg id=%d",
  ...
  if (msg_ptr)
  {
	...
	{
	  DS_MSG3(MSG_LEGACY_MED,"Send QMI service: %d tx_id: %d via QMUX - SIO to client: %d",
                       msg_buf->msg_hdr.common_hdr.service,
                       msg_buf->msg_hdr.common_hdr.transaction_id,
...
/*-----------------------------------------------------------------*/
  ...
  DS_ASSERT((int)msg_hdr);
  DS_ASSERT(msg_hdr->common_hdr.qmi_instance < QMI_INSTANCE_MAX   );
  DS_ASSERT(msg_hdr->common_hdr.service      < QMUX_SERVICE_MAX);   
  ...
  if (msg_hdr->common_hdr.client_id == QMI_SVC_CLID_BROADCAST)
  {
    ...
    {
      DS_MSG2(MSG_LEGACY_ERROR,"Got QMI service %x message for broadcast client msg type %d! discarding",
  ...
  if (svc_info == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service [%d]", msg_hdr->common_hdr.service);
    return FALSE;
  }
  ...
  {
    DS_MSG3(MSG_LEGACY_ERROR,"Got QMI service %x message for unassigned client %d instance %d! discarding",
                     msg_hdr->common_hdr.service,
                     msg_hdr->common_hdr.client_id,
...
/*-------------------------------------------------------------------------*/
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid service or common_msg_hdr!");
    return;
  }
  ...
  if (svc_info == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service [%d]", common_msg_hdr->service);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_ASSERT((int)sdu_in);
  DS_ASSERT(qmi_instance < QMI_INSTANCE_MAX   );
  DS_ASSERT(service      < QMUX_SERVICE_MAX);
  ...
  {
    DS_MSG2(MSG_LEGACY_ERROR, "Got QMI service %x message for unassigned client %d! discarding",
                     service, client_id);
  ...
  if (svc_info == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service [%d]", service);
    dsm_free_packet (&sdu_in);
    return FALSE;
  }
  ...
  {
    DS_MSG2(MSG_LEGACY_ERROR,"Got QMI service %x message for invaid instance: %d",
  ...
  DS_MSG2(MSG_LEGACY_MED,"QMI service %x receives QMI service request(transaction id = %x, ctl = %x)",
                   msg_x_id, msg_ctl);
  ...
  if(svc_info->cfg.cbs.cmd_hdlr)
  {
    ...
    DS_MSG3(MSG_LEGACY_MED,"Calling QMI Service:%d cmd hdlr. clid:%d, tx_id:%d",
                     msg_hdr.common_hdr.service,
                     msg_hdr.common_hdr.client_id,
                     msg_hdr.common_hdr.transaction_id);
  ...
  {
    DS_MSG3(MSG_LEGACY_ERROR,"Invalid QMI service:%d cmd hdlr. Request is dropped. clid:%d, tx_id:%d",
                     msg_hdr.common_hdr.service,
                     msg_hdr.common_hdr.client_id,
                     msg_hdr.common_hdr.transaction_id);
    dsm_free_packet (&sdu_in);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if((qmi_cmd_id_e_type)qmi_cmd == QMI_CMD_FRAMEWORK_SEND_IND)
  {
                    ...
                    (qmi_instance_e_type)msg_hdr->common_hdr.qmi_instance))
    {
      ...
      {
        DS_MSG1(MSG_LEGACY_MED,"Client %d has been released, drop indication",
                        msg_hdr->common_hdr.client_id);
        goto func_end;
      }
    ...
    if (temp == -1)
    {
      DS_MSG0(MSG_LEGACY_ERROR ,"Couldn't extract the ind_cmd_id %d from msg_ptr");
      goto func_end;
    }
    ...
    {
      DS_MSG0(MSG_LEGACY_ERROR ,"Couldn't append the ind_cmd_id to msg_ptr");
      goto func_end;
    }
  }
  else
  {
    DS_MSG1(MSG_LEGACY_ERROR ,"Invalid QMI svc cmd to process cmd %d", qmi_cmd);
    goto func_end;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR ,"Msg header validation failed - unable to send Response for cmd %d",
                     msg_hdr->msg_ctl_flag);
...
)
{
  ...
  if(client_reg_result->service_id >= QMUX_SERVICE_MAX)
  {
    DS_MSG1(MSG_LEGACY_ERROR ,"Invalid service id %d",
            client_reg_result->service_id);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  DS_ASSERT((int)common_hdr);
  
  if (common_hdr->service >= QMUX_SERVICE_MAX)
  {
    DS_MSG1(MSG_LEGACY_ERROR ,"Invalid service %d", common_hdr->service);
  ...
  if ( (svc_info == NULL) || (!svc_info->registered) )
  {
    ...
    DS_MSG1(MSG_LEGACY_MED,"Service %d not registered with QMI Framework!",
                     common_hdr->service);
    return QMI_ERR_INVALID_SERVICE_TYPE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR ,"Maximum clients reached for service",
                     svc_info->service_id);
  ...
  if(svc_info->cfg.cbs.alloc_clid)
  {
    ...
    -------------------------------------------------------------------------*/
      ...
      DS_MSG3(MSG_LEGACY_MED,"Alloc new Framework Clid: %d Service: %d and Instance: %x",
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR, "Clid allocation failed due to invalid svc cb");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (svc_info == NULL)
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknown service [%d]", common_hdr->service);
    return QMI_ERR_INVALID_SERVICE_TYPE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Invalid clid:%d to deallocate ", common_hdr->client_id );
  ...
  if ( svc_info->cfg.cbs.dealloc_clid )
  {
    ...
    -------------------------------------------------------------------------*/
    svc_info->seq_no[common_hdr->client_id-1] = 0;
    DS_MSG3(MSG_LEGACY_MED,"Dealloc the Framework Clid: %d Service: %d and Instance: %x",
                     common_hdr->client_id,
                     common_hdr->service,
                     common_hdr->qmi_instance);
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR, "Clid deallocation failed due to invalid svc cb");
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT(instance < QMI_INSTANCE_MAX);
  DS_MSG1(MSG_LEGACY_MED,"QMI Framework clean instance %d resources", instance);
    ...
    if (svc_info->registered == TRUE)
    {
      for (j = 0 ; j < (QMI_FRAMEWORK_SVC_MAX_CLIENTS - 1); j++)
      {
        if(svc_info->client_instance_map[j] == instance) 
        {
          ...
          DS_MSG3(MSG_LEGACY_MED,"QMI link closed, do service %d cleanup- instance:%d - clid:%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (svc_info == NULL)
  {
    ...
    DS_SYSTEM_HEAP_MEM_ALLOC(svc_info, sizeof(qmi_framework_svc_info_type),
  ...
  if(ret_val == -1)
  {
    DS_MSG1(MSG_LEGACY_MED,"qmi_framework_reg_service failed for service = %d",service);	
    DS_MSG1(MSG_LEGACY_MED,"qmi_framework_reg_service failed for err %d",err);
    return err;
  }
  else
  {
    DS_MSG1(MSG_LEGACY_MED,"service %d successfully registered",service);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Service %d not registered with QMI Framework!", service);
    return;
  }
  ...
  for (i = 0 ; i < (QMI_FRAMEWORK_SVC_MAX_CLIENTS - 1); i++)
  {
    if(svc_info->client_instance_map[i] != QMI_INSTANCE_MAX) 
    {
      ...
      DS_MSG3(MSG_LEGACY_MED,"QMI Framework dereg service, do service %d cleanup- instance:%d - clid:%d",
                       svc_info->service_id,
  ...
  DS_MSG1(MSG_LEGACY_MED,"Deregistering service %d from QMI Framework",
                   svc_info->service_id);
  /*-------------------------------------------------------------------------
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_MED,"Invalid Transaction ID: %d, Discarding the Service response",
                     msg_hdr->common_hdr.transaction_id);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR," Invalid service %d or cl id %d ",
                    msg_hdr->common_hdr.service);
    return FALSE;
  }
  /*-------------------------------------------------------------------------
  ...
  if ( qmi_framework_svc_state[msg_hdr->common_hdr.service] == NULL || 
       ...
       msg_hdr->common_hdr.client_id >= QMI_FRAMEWORK_SVC_MAX_CLIENTS ) 
  {
    DS_MSG2(MSG_LEGACY_ERROR," Invalid service %d or cl id %d ",
                    msg_hdr->common_hdr.service, 
                    msg_hdr->common_hdr.client_id);
  }
  else
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending service %d Indication to QMI Framework: clid: %d, tx_id: %d", 
                  msg_hdr->common_hdr.service, 
                  msg_hdr->common_hdr.client_id, 
                  msg_hdr->common_hdr.transaction_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (msg_hdr == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Alloc msg_header: No Can't get memory allocation!");
    DS_ASSERT((int)msg_hdr);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(msg_hdr == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"NULL pointer passed to qmi_framework_dealloc_msg_header");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending service %d alloc clid %d result %d to framework",
...
/*-------------------------------------------------------------------------*/
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending service %d dealloc clid %d result %d to framework",
                   common_msg_hdr->service, common_msg_hdr->client_id, result);
...
)
{
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending alloc clid %d result to Framework. service %d", 
                  common_msg_hdr->client_id, 
                  common_msg_hdr->service, 
                  errval);
...
)
{
 /*-------------------------------------------------------------------------*/
  ...
  DS_MSG3(MSG_LEGACY_MED,"Sending dealloc clid %d result to Framework. service %d", 
                  common_msg_hdr->client_id, 
                  common_msg_hdr->service, 
                  errval);
