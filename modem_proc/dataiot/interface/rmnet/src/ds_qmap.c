...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/rmnet/src/ds_qmap.c#1 $
...
#define ds_qmap_free_cmd_buf(buf_ptr) DS_SYSTEM_HEAP_MEM_FREE(buf_ptr)
...
)
{
  ...
  //qmi_task_set_qmap_cmd_handler(ds_qmap_process_cmd); Done in ds_qmi_if.c
  ...
  if (qmap_cmd_resp_time > 0)
  {
    ...
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Failed to registered QMAP cmd callback");
    }
  }
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DS_MSG3(MSG_LEGACY_ERROR,"Invalid parameters %p %p %d",
  ...
  if (pkt == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Unable to get a new DSM item");
    return FALSE;
  }
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Failed to build DSM pkt for QMAP cmd %d", cmd);
    return FALSE;
  }
  ...
  if (!result)
  {
    DS_MSG2(MSG_LEGACY_ERROR,"Failed to send QMAP cmd [%d] to SIO port 0x%x",
  ...
  if (DS_QMAP_IS_FC_CMD(cmd))
  {
    ...
    DS_MSG6(MSG_LEGACY_ERROR,"Sent QMAP cmd [%d] IP [%d] seq [%d] "
  ...
  {
    DS_MSG2(MSG_LEGACY_ERROR,"Sent QMAP cmd [%d] to stream 0x%x",
  ...
  if (!cmd_info_ptr)
  {
    ...
    DS_SYSTEM_HEAP_MEM_ALLOC(qmap_cmd_info, sizeof(ds_qmap_cmd_info_type),
    ...
    DS_SYSTEM_HEAP_MEM_ALLOC(qmap_cmd_info->cmd_ptr, cmd_size, void*);
    ...
    {
      DS_SYSTEM_HEAP_MEM_FREE(qmap_cmd_info);
      return result;
    }
  ...
  while (q_count--)
  {
    ...
    if (item_ptr == NULL)
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Invalid queue entry ! Skipping");
    }
    ...
    else
    {
      ...
      DS_SYSTEM_HEAP_MEM_FREE(item_ptr->cmd_ptr);
      DS_SYSTEM_HEAP_MEM_FREE(item_ptr);
    }
  }
...
{
  ds_qmap_cmd_buf_type         * cmd_payload = NULL;
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
   cmd_ptr = ds_allocate_cmd_buf(sizeof(ds_qmap_cmd_buf_type));
   if ((cmd_ptr == NULL) || (cmd_ptr->cmd_payload_ptr == NULL))
   {
     DS_MSG0(MSG_LEGACY_ERROR,"Out of memory");
     return ;
    }
    cmd_ptr->hdr.cmd_id = DS_QMI_QMAP_CMD;
    cmd_payload = (ds_qmap_cmd_buf_type *)cmd_ptr->cmd_payload_ptr;

	cmd_payload->cmd_id = QMI_CMD_QMAP_CMD_RESP_CB;
    cmd_payload->qmap.qmap_resp.stream_id  = stream_id;
    cmd_payload->qmap.qmap_resp.cmd_name   = cmd_name;
    cmd_payload->qmap.qmap_resp.cmd_type   = cmd_type;
    cmd_payload->qmap.qmap_resp.tx_id      = tx_id;
    cmd_payload->qmap.qmap_resp.dsm_ptr    = dsm_ptr;
  ...
  while (q_count--)
  {
    ...
    if (item_ptr != NULL)
    {
	 ...
	 timer_undef( &(item_ptr->qmap_cmd_timer) );
        DS_SYSTEM_HEAP_MEM_FREE(item_ptr->cmd_ptr);
        DS_SYSTEM_HEAP_MEM_FREE(item_ptr);
        DS_MSG2(MSG_LEGACY_ERROR,"QMAP FC cmd [%d] IP [%d] removed", cmd, ip);
      }
    }
  }
...
/*-------------------------------------------------------------------------*/
  ...
  if (cmd_ptr == NULL)
  {
    DS_ERROR_FATAL("NULL QMAP cmd_ptr!");
    return;
  }
  ...
  switch (cmd_buf_ptr->cmd_id)
  {
    ...
    default:
      DS_MSG1(MSG_LEGACY_ERROR,"Ignoring QMAP cmd %d", cmd_buf_ptr->cmd_id);
      break;
  }
 return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-----------------------------------------------------------------------
   ...
   cmd_payload->cmd_id = QMI_CMD_QMAP_CMD_TIMER_EXP_CB;
   cmd_payload->qmap.timer_info.search_id  = search_id;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (pkt_size == 0)
  {
    DS_MSG0(MSG_LEGACY_ERROR, "Invalid DSM ptr " );
  ...
  if (DS_QMAP_IS_FC_CMD(cmd_name))
  {
      ...
      {
        DS_MSG1(MSG_LEGACY_ERROR, "Invalid QMAP FC payload length %d", pkt_size );
        goto send_result;
      }
  }
  else
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Ingoring CB for non FC cmds %d ", cmd_name );
    goto send_result;
  }
  ...
  DS_MSG6(MSG_LEGACY_ERROR,"Recvd QMAP resp cmd [%d] IP [%d] seq [%d] "
                  "qos [0x%x] strm [0x%x] tx_id [0x%x]",
  ...
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Invalid/Old resp recvd: Ignoring!");
  }
  else
  {
    ...
    DS_SYSTEM_HEAP_MEM_FREE(cmd_info_ptr->cmd_ptr);
    DS_SYSTEM_HEAP_MEM_FREE(cmd_info_ptr);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_MSG1(MSG_LEGACY_ERROR,"Timer exp for QMAP resp id %d", search_id);
  ...
  if (cmd_info_ptr == NULL)
  {
    DS_MSG0(MSG_LEGACY_ERROR,"Old timer: Ignoring!" );
  }
  else
  {
    // Stop corresponding timer 
    ...
    else
    {
      ...
      DS_MSG0(MSG_LEGACY_ERROR, "Max retries reached. No more retx !" );
      // Stop the timer
      timer_undef( &(cmd_info_ptr->qmap_cmd_timer) );
      DS_SYSTEM_HEAP_MEM_FREE(cmd_info_ptr->cmd_ptr);
      DS_SYSTEM_HEAP_MEM_FREE(cmd_info_ptr);
    ...
    if (!result)
    {
      DS_MSG0(MSG_LEGACY_ERROR,"Unable to resend QMAP pkt ");
    }
  }
