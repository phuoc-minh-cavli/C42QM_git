...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2004 - 2022 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatme.c#7 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/20/22   ks      Fixed an issue related to +CFUN=0 after n1 timer expires.
03/28/21   ks      Removed incorrect SET_PENDING flag used for CEDRXS URC.
03/17.22   asa     Fixed issue related to cedrx URC
01/12.20   sp      Fixed issue related to +CFUN=1 blocked during RPM
09/29/20   sp      Fixed issue related to CFUN to check RPM allowed.
10/0/20     mks    To check CM api if LPM is allowed.
...
08/19/16   skc     Fixed issue related to duplicate call of csp, spn
                   and imsi read handler.
08/19/16   skc     Adding support for ESC SEQUENCE for PS Data call.
05/24/16   skc     Fixed CPBS issue for LD.
...
-------------------------------------------------------------------------*/
/* ATD Dial Table */
extern dsat_dial_type dsati_dial_table[DSAT_MODE_MAX];
/*--------------------------------------------------------------------------
...
extern uint16 requested_edrx_val[];
extern sys_radio_access_tech_e_type edrx_rat_table[];
...
)
{
  ...
  if(dd_val_ptr->mem[0] != '\0' && dd_val_ptr->index > 0)
  {
    ...
    if( !strncmp((char *) dd_val_ptr->mem, 
                ...
                (dsat_num_item_type)dsatutil_get_val(enum_val,0,NUM_TYPE)], CPBS_VAL_LEN + 1) )
...
)
{
  ...
  dsatme_pbm_state_info *pbm_val = NULL;

  pbm_val = dsat_get_base_addr(DSAT_PBM_VALS, FALSE);

  if (number != NULL)
  {
    if (NULL == pbm_val->ld_rec_ptr)
    {
      pbm_val->ld_rec_ptr = dsat_alloc_memory(sizeof(ld_rec_type), FALSE);
    }

    pbm_val->ld_rec_ptr->state = LD_DIALED;

    /* prepare the ld_rec for updating last dial ph book */
    pbm_val->ld_rec_ptr->rec.index = -1;
	/* need to verify with CPB_MAX_NUM_LENGTH */
    (void) strlcpy(pbm_val->ld_rec_ptr->rec.number, number, PBM_NUM_SIZE);

    if(type != -1)
    {
      pbm_val->ld_rec_ptr->rec.type= (uint8) type;
    }
    else
    {
      ...
      if(number[0] == '+')
      {
        pbm_val->ld_rec_ptr->rec.type = TA_INTERNATIONAL;
      }
      else
      {
        pbm_val->ld_rec_ptr->rec.type = TA_OTHER;
      }
    }
    ...
    (void) dsatutil_memscpy((void*)pbm_val->ld_rec_ptr->rec.text,
            PBM_TEXT_SIZE_BYTES,(void*)text,PBM_TEXT_SIZE_CHARS);  
    }
    else
    {
      pbm_val->ld_rec_ptr->rec.text[0] = '\0';
    }
    ...
    pbm_val->ld_rec_ptr->rec.time = time_jul_to_secs(&julian_time);
    pbm_val->ld_rec_ptr->rec.duration = 0;

    pbm_val->ld_rec_ptr->rec.is_answered = FALSE;
  }
  else
  {
...
)
{
  ...
  if(pin2_locked)
  {
    ...
    pin_data.data_ptr = (uint8 *)pbm_val->me_cpbs_params.password;

#ifdef FEATURE_DSAT_CUST
    pin_data.data_len = MMGSDI_PIN_MAX_LEN;
#else
    pin_data.data_len = (mmgsdi_len_type)strlen((char*) pbm_val->me_cpbs_params.password);
    ...
    else if (mmgsdi_status == MMGSDI_INCORRECT_PARAMS)
    {
      ...
      if (IS_VALID_APPS_ID(cmd_info_ptr->apps_id))
      {
        me_val->dsat_mask_pin2_state = FALSE;
      }
  ...
  else
  {
    ...
    pb_id_local.device_type = pbm_val->me_cpbs_params.pbm_device;
  ...
  /* clear the password, we are not allowed to maintain a copy
     of the pin2 code */
  dsatutil_reset_password(&pbm_val->me_cpbs_params.password[0],
                            sizeof(pbm_val->me_cpbs_params.password));
...
)
{
  ...
  if( (tok_ptr->op == NA) || 
      ...
      (tok_ptr->op == (NA | QU)) )
  {
    memset( (void*)pbm_val->me_cpbs_params.display_name, 0, 
            sizeof(pbm_val->me_cpbs_params.display_name) );
    (void)strlcpy( pbm_val->me_cpbs_params.display_name, 
                       (char *)parse_table->name,
                       sizeof(pbm_val->me_cpbs_params.display_name));

    if( cpbs_parse_arguments(tok_ptr, &pbm_val->me_cpbs_params) != DSAT_OK)
    {
      result = DSAT_ERROR;
    }
    else /* the arguments are correct */
    {
      ...
      else
      {
        ...
        cb_expected = CPB_PBM_CB_FILE_INFO;
        /* error cases are also handled in the call-back */
        dsat_pb_id.device_type = pbm_val->me_cpbs_params.pbm_device;
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    if( (result = cpbr_parse_arguments(tok_ptr, &pbm_val->cpbr_index.current, 
                                       &pbm_val->cpbr_index.last, res_buff_ptr)
		                              ) != DSAT_OK )
    {
      return result;
    }

    memset( (void*)pbm_val->cpbr_index.display_name, 0, 
            sizeof(pbm_val->cpbr_index.display_name) );
    
    (void)strlcpy( pbm_val->cpbr_index.display_name, 
                       (char *) parse_table->name,
                       sizeof(pbm_val->cpbr_index.display_name));
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    pbm_result =  pbm_session_enum_rec_init_ext( 
                           ...
                           &pbm_val->dsat_pbm_handler);

    /* Search initialization successful */
    if (pbm_result == PBM_SUCCESS )
    {
       memset( (void*)pbm_val->cpbf_index.display_name, 0, 
            sizeof(pbm_val->cpbf_index.display_name) );
    
      (void)strlcpy( pbm_val->cpbf_index.display_name, 
                       (char *) parse_table->name,
                       sizeof(pbm_val->cpbf_index.display_name));
      /* start the search */
      result = cpb_pbm_find_handler();
      pbm_session_enum_rec_init_ext_free_handle(pbm_val->dsat_pbm_handler);
    }
...
{ DSAT_CME_SUPS_DFL_INVALID_NUMBER,  "invalid deflected to number"},
  ...
  /* cmee enables the ME error result code */
  if( (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_CMEE_IDX,0,NUM_TYPE) == 1 || 
    (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_CMEE_IDX,0,NUM_TYPE) == 2 )
  {
    int index = 0;
      ...
      switch ( mmgsdi_result )
      {
        ...
        case MMGSDI_INCORRECT_CODE:
          ...
          if ( DSAT_CME_NO_ERROR == 
               (error_code =
                me_choose_cme_error((dsat_cpin_required_e_type)me_val->dsat_pin_blocked)) )
          {
        ...
        case MMGSDI_CODE_BLOCKED:
          ...
          if ( DSAT_CME_NO_ERROR ==
               (error_code =
                me_choose_cme_error((dsat_cpin_required_e_type)me_val->dsat_pin_required)))
          {
  ...
  {      
    ...
    res_buff_ptr->used =
      (word)snprintf((char*)res_buff_ptr->data_ptr,res_buff_ptr->size,
                            ...
                            dsat_qcsim_stat[me_val->dsat_qcsimstat_curr]);
  }
  else
  {
    /* wrong operation */
...
dsat_result_enum_type dsatme_init_sim_filename( void )
{
  ...
  cpls_val = (dsat_num_item_type)dsatutil_get_val(
                    DSATETSI_EXT_ACT_CPLS_ETSI_IDX,0,NUM_TYPE);
...
)
{
  ...
  for ( ;pbm_val->cpbr_index.current <= pbm_val->cpbr_index.last; ++pbm_val->cpbr_index.current )
  {
    dsat_pbm_rec_id_type   rec_id;
    ...
    record_ptr = (dsat_pbm_record_s_type *)dsat_alloc_memory(sizeof(dsat_pbm_record_s_type),FALSE);
    dsat_pb_id.device_type = me_cpbs_entry.device;
    rec_id = pbm_session_location_to_record_id(dsat_pb_id,pbm_val->cpbr_index.current);
    ...
    {
      if ( pbm_val->cpbr_index.current == pbm_val->cpbr_index.last )
      {
    ...
    if( result == DSAT_ASYNC_CMD )
    {
      ...
      if ( DSM_POOL_FREE_CNT(DSM_DS_LARGE_ITEM_POOL) < DSAT_FREE_ITEM_TRIGGER )
      {
        ...
        ps_timer_start( t_ptr->timer_handle, t_ptr->duration );
        dsatutil_free_memory((void *)record_ptr);
...
)
{
  ...
  if (record_ptr != NULL)
  {
    ...
    raw_rsp_ptr->used = (word) snprintf((char *)raw_rsp_ptr->data_ptr,
                                     raw_rsp_ptr->size,
                                     "%s: %d,\"%s\",%d,\"%s\",%d",
                                     pbm_val->cpbr_index.display_name,
                                     pbm_val->cpbr_index.current,
                                     record_ptr->number,
                                     record_ptr->type,
                                     name,
  ...
  else 
  {
    ...
    /* This block only executed if read of a single entry results in
       ...
       simply ignored */
    if(pbm_val->cpbr_index.last == pbm_val->cpbr_index.current &&
...
)
{
  ...
  while ( (pbm_result = pbm_session_enum_next_rec_id_ext(&rec_id, pbm_val->dsat_pbm_handler))== PBM_SUCCESS )
  {
  ...
  if (record_ptr != NULL && record_ptr->number[0] != '\0')
  {
    ...
    raw_rsp_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);
      ...
      raw_rsp_ptr->used=(word)snprintf((char *)raw_rsp_ptr->data_ptr,
                                       raw_rsp_ptr->size,
                                       "%s: %d,\"%s\",%d,\"%s\"",
                                       pbm_val->cpbf_index.display_name,
                                       loc_id,
                                       record_ptr->number,
                                       record_ptr->type,
                                       name);
...

{
  ...
  if(status == PBM_SUCCESS)
  {
    ...
    if(get_pb_storage(info_ptr->pbm_device, storage))
    {
      ...
      if (find_index)
      {
        DSATUTIL_SET_VAL(enum_val,0,0,index,NUM_TYPE)
        ...
        if (pbm_val->me_cpbs_params.is_read)
        {
...
LOCAL void cpb_pbm_cb_file_info_handler_output_info(int used)
{
  ...
  if (used < 0 || me_cpbs_entry.total < 1) 
  {
    ...
    DS_ASSERT(me_cpbs_entry.total == -1);

    raw_rsp_ptr->used = (word) snprintf(buff_ptr,
                                              raw_rsp_ptr->size,
                                              "%s: \"%s\"",
                                              pbm_val->me_cpbs_params.display_name,
...
)
{
  ...
  record_ptr = (dsat_pbm_record_s_type *)dsat_alloc_memory(sizeof(dsat_pbm_record_s_type),FALSE);
  converted_len = (uint16)((pbmutils_wstrlen((uint16*)srch_str) + 1) * sizeof(uint16));
  ...
  if (pbm_result == PBM_SUCCESS )
  {
    while ( (pbm_result = pbm_session_enum_next_rec_id_ext(&rec_id, dsat_handler))== PBM_SUCCESS )
    {
      ...
      if (pbm_result == PBM_SUCCESS )
      {
        ...
        if(!memcmp((void *)srch_str, (void *)record_ptr->text,converted_len))
        {
...
)
{
      ...
      device == PBM_OCI )
  {
    if ( ( (app_type == MMGSDI_APP_SIM) && (device == PBM_LND) ) ||
         ...
         ( (app_type == MMGSDI_APP_USIM) && (device == PBM_OCI) ) )
    {
...
)
{
  ...
  else /* set command */
  {
    ...
    if(tok_ptr->args_found == 0)
    {
      ...
      if(pbm_val->dsatme_nv_only == TRUE)
      {
...
)
{
  ...
  else /* user input a number */
  {
    ...
    if ( !dsatutil_strip_quotes_out( tok_ptr->arg[1], 
                                     tok_ptr->arg[1], tok_ptr->arg_length[1]))
    {
      return DSAT_ERROR;
    }
  ...
  else /* user give a adn number */
  {
    ...
    if ( !dsatutil_strip_quotes_out( tok_ptr->arg[5], 
                                     tok_ptr->arg[5], tok_ptr->arg_length[5]))
    {
      return DSAT_ERROR;
    }
...
)
{
  ...
  if(dd_val_ptr->str == NULL)
  {
    dd_val_ptr->str = dsat_alloc_memory(dial_ptr->len + 1, FALSE);
    /* This memory will be freed at the end of command execution or 
       if missed it will be freed at beginning of new AT command execution */
    if(dd_val_ptr->str == NULL)
    {
      return FALSE;
    }
    memset(dd_val_ptr->str, 0, (dial_ptr->len + 1));
  }
...
)
{
  ...
  srch_str = dsat_alloc_memory(MAX_LINE_SIZE + 1, FALSE);
  ...
  temp_val = (dsat_num_item_type)dsatutil_get_val(enum_val,0,NUM_TYPE);
  dd_val_ptr->srch_pb = (int8) temp_val;
  dd_val_ptr->record.number[0] = '\0';
  memset(srch_str, '\0', MAX_LINE_SIZE + 1);
  ...
  while(dsat_cpbs_storage_valstr[dd_val_ptr->srch_pb][0] != '\0')
  {
    ...
    if (!get_pbm_device(dsat_cpbs_storage_valstr[dd_val_ptr->srch_pb], &pbm_device))
    {
      ...
      /* can't map mem to a valid pbm device */
      dsatutil_free_memory(srch_str);
      return DSAT_ERROR;
    }

    /* Perform search in the PBM */
    result = dd_pbm_find_handler((byte *)srch_str, dd_val_ptr, pbm_device);
    /* prepare the last dial number for saving to the last dial ph book */
    if( (result == DSAT_NO_RSP) || (result == DSAT_ASYNC_CMD) )
    {
      /* the number was dialed, init the ld_rec */
...
)
{
  ...
  else
  {
    ...
    case CLCK_MMGSDI_PIN1_STATUS:
      result = dsatetsicmif_report_clck (NULL, cmd_info_ptr);
      break;
  ...
  if (CHECK_PENDING(DSATETSI_EXT_ACT_CPOL_ETSI_IDX,0,DSAT_PENDING_CPOL_WRITE))
  {
    ...
    if (CHECK_PENDING(DSATETSI_EXT_ACT_CPOL_ETSI_IDX,0,DSAT_PENDING_CPOL_WRITE) && 
        (me_val->dsat_cpol_write_params.index != CPOL_INVALID_INDEX &&
          (me_val->dsat_cpol_write_params.index > num_rec || 
            me_val->dsat_cpol_write_params.index < 1 )))
    {
      SET_PENDING(DSATETSI_EXT_ACT_CPOL_ETSI_IDX ,0, DSAT_PENDING_CPOL_NONE)
...
)
{
  ...
  if (CHECK_PENDING(DSATETSI_EXT_ACT_CUAD_ETSI_IDX,0,DSAT_PENDING_CUAD_READ))
  {
    ...
    {
      me_val->dsat_cuad_recs.num_of_rec = 
        sim_info->file_attr.file_info.linear_fixed_file.num_of_rec;
      me_val->dsat_cuad_recs.rec_len = 
        sim_info->file_attr.file_info.linear_fixed_file.rec_len;
      me_val->dsat_cuad_recs.curr_rec = 0;
    }
    else
    {
      me_val->dsat_cuad_recs.num_of_rec = 0;
      me_val->dsat_cuad_recs.curr_rec = 0;
      me_val->dsat_cuad_recs.rec_len = 0;
      SET_PENDING(DSATETSI_EXT_ACT_CUAD_ETSI_IDX ,0, DSAT_PENDING_CUAD_NONE)
      result = dsat_send_cme_error(DSAT_CME_SIM_FAILURE);
      return result;
    }
    DS_AT_MSG2_HIGH("In _cuad_get_file_attr_handler - num_of_recs %d - rec_len %d",
                  me_val->dsat_cuad_recs.num_of_rec,me_val->dsat_cuad_recs.rec_len);

    if(me_val->dsat_cuad_recs.num_of_rec > me_val->dsat_cuad_recs.curr_rec )
    {
      me_val->dsat_cuad_recs.curr_rec++;
...
)
{
  ...
  DS_ASSERT( (res_buff_ptr != NULL) && (cmd_ptr != NULL) );
...
#define FIRST_RECORD 1
  ...
  DS_ASSERT(me_val->dsat_cuad_recs.curr_rec == sim_info->accessed_rec_num);
  ...
  DS_AT_MSG2_HIGH("In _cuad_read_handler - curr_rec %d - status %d ",
                  me_val->dsat_cuad_recs.curr_rec,sim_info->cmd_status);


  if(sim_info->cmd_status == MMGSDI_SUCCESS)
  {
    res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_LARGE_ITEM_POOL, FALSE);
    if(me_val->dsat_cuad_recs.curr_rec == FIRST_RECORD)
    {
      res_buff_ptr->used = (word)snprintf( (char*)res_buff_ptr->data_ptr, 
                                              res_buff_ptr->size, 
                                              "+CUAD: " );
...
#define ACT_EUTRAN          0x40
  ...
  mmgsdi_session_get_operator_name_info_type         operator_name;  
  mmgsdi_plmn_info_static_type                      *plmn_info =NULL;
  ...
  memset(&operator_name,0,sizeof(mmgsdi_session_get_operator_name_info_type));
  if (me_val->dsat_plmn_wact_supported )
  {
    num_rec = 
  ...
  */
             ...
             user_pref_networks->length < SYS_USER_PPLMN_LIST_MAX_LENGTH;)
  {
    ...
    if ( me_val->dsat_plmn_wact_supported )  /* If Access Technology is supported */
    {
      /*
  ...
  DS_SYSTEM_HEAP_MEM_ALLOC(plmn_info, (sizeof(mmgsdi_plmn_info_static_type)* plmn_id_list.num_of_plmn_ids),
  ...
  operator_name.plmn_info_ptr = plmn_info;
    ...
    {
      result =  dsatme_mmgsdi_get_operator_name_cb(DSAT_MMGSDI_GET_PREF_NETS_NAME         ,
                                                               plmn_id_list.num_of_plmn_ids,
                                                               &operator_name);
      DS_AT_MSG0_MED ( "Query EONS for Long and short names");
        DS_SYSTEM_HEAP_MEM_FREE(plmn_info);
...
)
{
  ...
  DS_ASSERT( (NULL != resp_ptr) && (NULL != user_data_ptr) );
...
===========================================================================*/
dsat_result_enum_type dsatme_mmgsdi_get_operator_name_cb
(
  uint32                                             client_ref,
  int32                                              plmn_ids,
  mmgsdi_session_get_operator_name_info_type         *session_get_operator_name_cnf
)
{
  uint16 i;
  mmgsdi_plmn_info_static_type * src_plmn_info_ptr = NULL;
  ds_at_plmn_info * dst_plmn_info_ptr       = NULL;
  ds_at_mmgsdi_oper_name_type oper_name ={0};
  dsat_result_enum_type  result = DSAT_FAILURE;

  oper_name.client_ref = client_ref;

  oper_name.num_of_plmn_ids = plmn_ids;
  DS_AT_MSG1_ERROR("mmgsdi_session_get_operator_name API call failed %d",plmn_ids);

  oper_name.spn.eons_encoding = session_get_operator_name_cnf->spn.eons_encoding;
    
  oper_name.spn.spn_data_len = 0;

    if((session_get_operator_name_cnf->spn.eons_encoding != MMGSDI_EONS_ENC_INVALID) &&
       (session_get_operator_name_cnf->spn.spn_data_len != 0))
    {
          oper_name.spn.spn_data_len =   session_get_operator_name_cnf->spn.spn_data_len;

      (void) dsatutil_memscpy((void*)oper_name.spn.spn_data,
                               oper_name.spn.spn_data_len,
                               session_get_operator_name_cnf->spn.spn_data,
                               session_get_operator_name_cnf->spn.spn_data_len  );
      }

    oper_name.plmn_info_ptr =
      ( ds_at_plmn_info **)dsat_alloc_memory(sizeof(ds_at_plmn_info *)
                                                     *(oper_name.num_of_plmn_ids), FALSE);
      DS_AT_MSG1_ERROR("mmgsdi_session_get_operator_name API call failed %p",oper_name.plmn_info_ptr);

    src_plmn_info_ptr = session_get_operator_name_cnf->plmn_info_ptr;
    ...
    for( i =0 ; i < oper_name.num_of_plmn_ids ; i++)
    {
      ...
      DS_AT_MSG3_HIGH("Allocated %x memory of %d size for %d index",dst_plmn_info_ptr,sizeof(ds_at_plmn_info),i);
      if((dst_plmn_info_ptr != NULL) && (src_plmn_info_ptr != NULL))
      {
        ...
        dst_plmn_info_ptr->long_name_encoding = src_plmn_info_ptr->plmn_long_name.plmn_encoding;
        dst_plmn_info_ptr->short_name_encoding = src_plmn_info_ptr->plmn_short_name.plmn_encoding;

        dst_plmn_info_ptr->long_name_len = src_plmn_info_ptr->plmn_long_name.plmn_data_len;
        dst_plmn_info_ptr->short_name_len = src_plmn_info_ptr->plmn_short_name.plmn_data_len;
        ...
        if(src_plmn_info_ptr->plmn_long_name.plmn_data_len != 0)
        {
        (void) dsatutil_memscpy((void*)dst_plmn_info_ptr->long_name,
                 2*DSAT_MAX_FULLNAME_STR_LEN,
                (void*)src_plmn_info_ptr->plmn_long_name.plmn_data,
                 dst_plmn_info_ptr->long_name_len);
          
        }
        if(src_plmn_info_ptr->plmn_short_name.plmn_data_len != 0)
        {
          (void) dsatutil_memscpy((void*)dst_plmn_info_ptr->short_name,
                   2*DSAT_MAX_SHORTNAME_STR_LEN,
                  (void*)src_plmn_info_ptr->plmn_short_name.plmn_data,
    ...
    if(oper_name.num_of_plmn_ids)
       result = dsat_mmgsdi_get_operator_name_handler(&oper_name);
...
(
  ds_at_mmgsdi_oper_name_type  *oper_name
)
{
  uint32                        j;
  dsat_result_enum_type         result = DSAT_ASYNC_EVENT;
  dsatcmif_servs_state_info *ph_val = NULL;
       ...
       (oper_name->client_ref == DSAT_MMGSDI_GET_OPER_NAME_LIST))
    {
      ...
      result = dsatetsicall_report_cops_result(oper_name);
    }
    ...
    { 
      ...
      {
       DS_AT_MSG1_ERROR("mmgsdi_session_get_operator_name API call failed %p",oper_name->plmn_info_ptr[j]);
...
)
{
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    DSATUTIL_SET_VAL(DSAT_EXT_CFUN_IDX,1,0,DSAT_ME_DONT_RESET,MIX_NUM_TYPE)
        ...
        dsatparm_exec_param_cmd (mode, parse_table, tok_ptr, res_buff_ptr))
    {
      ...
      if ((dsat_num_item_type) dsatutil_get_val(
             DSAT_EXT_CFUN_IDX,1,MIX_NUM_TYPE)== (dsat_num_item_type)DSAT_ME_DO_RESET &&
           (dsat_num_item_type)dsatutil_get_val(
             DSAT_EXT_CFUN_IDX,0,MIX_NUM_TYPE) != (dsat_num_item_type)DSAT_ME_FUNC_FULL )
      {

        DSATUTIL_SET_VAL(DSAT_EXT_CFUN_IDX,0,0,old_opmode,MIX_NUM_TYPE)
    ...
    /* For mode ONLINE no need to check for reset allowed */
    if((dsat_num_item_type)dsatutil_get_val(
          DSAT_EXT_CFUN_IDX,0,MIX_NUM_TYPE) != (dsat_num_item_type)DSAT_ME_FUNC_FULL)
    {
      if(TRUE != cm_rpm_check_reset_allowed() )
      { 
        DS_AT_MSG0_HIGH("+CFUN reset not allowed");
        DSATUTIL_SET_VAL(DSAT_EXT_CFUN_IDX,0,0,old_opmode,MIX_NUM_TYPE)
        return DSAT_ERROR;
      }
    }
...
)
{
  ...
  if (CHECK_PENDING(DSAT_EXT_CFUN_IDX,0,DSAT_PENDING_CFUN_WRITE))
  {
   ...
   /* If not resetting and current and requested opmode 
    ...
    if ( (dsat_num_item_type)dsatutil_get_val(
          DSAT_EXT_CFUN_IDX,1,MIX_NUM_TYPE) == (dsat_num_item_type)DSAT_ME_DONT_RESET &&
    ...
    else
    {
     ...
     /* Reset support for CFUN = 6 */
      if ( (dsat_num_item_type)DSAT_ME_DO_RESET == 
           (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_CFUN_IDX,1,MIX_NUM_TYPE) ||
            ...
            DSAT_EXT_CFUN_IDX,0,MIX_NUM_TYPE))) )
      {
         ...
         if(FALSE == ((DSAT_ME_FUNC_OFFLINE == old_opmode )&&((dsat_num_item_type)DSAT_ME_FUNC_RESET == (dsat_num_item_type)dsatutil_get_val(
            DSAT_EXT_CFUN_IDX,0,MIX_NUM_TYPE))))
         {
           DSATUTIL_SET_VAL(DSAT_EXT_CFUN_IDX,0,0,DSAT_ME_FUNC_OFFLINE,MIX_NUM_TYPE)
         }
         if(TRUE == cm_rpm_check_reset_allowed())
         {
      ...
      if ( (dsat_num_item_type)dsatutil_get_val(
            DSAT_EXT_CFUN_IDX,0,MIX_NUM_TYPE) == (dsat_num_item_type)DSAT_ME_FUNC_MIN)
      {
...
dsat_result_enum_type dsatme_process_cfun_card_pdown_cmd(void)
{
  ...
  me_val = dsat_get_base_addr(DSAT_MMGSDI_VALS, FALSE);
  me_val->dsat_is_card_power_down = TRUE;

  ret_val = mmgsdi_card_pdown( me_val->dsat_mmgsdi_client_id,
                                   MMGSDI_SLOT_1,
...
)
{
  ...
  if (TRUE == me_val->dsat_is_card_power_down)
  {
    me_val->dsat_is_card_power_down = FALSE;
    me_val->dsat_card_error_event_count = 0;
    me_val->dsat_num_of_card_operations = 0;

      ret_val = mmgsdi_card_pup( me_val->dsat_mmgsdi_client_id,
                                 MMGSDI_SLOT_1,
                                 dsatetsime_mmgsdi_card_power_ctl_cb,
      ...
      if (MMGSDI_RESTRICTED_IN_ONCHIP_MODE != ret_val)
      {
        me_val->dsat_num_of_card_operations++;
        result = DSAT_ASYNC_CMD;
...
FUNCTION  DSATME_PROCESS_CEDRX_CMD
...
===========================================================================*/
dsat_result_enum_type dsatme_process_cedrx_cmd
(
...
)
{
  ...
  /* Mapping CM RAT type to DSAT CEDRXS RAT type */
  while(rat_ind < DSAT_EDRX_RAT_MAX)
  {
    if( ph_info->edrx_info.rat_type_edrx == edrx_rat_table[rat_ind] )
    {
      break;
    }
    rat_ind++;
  ...
  if(ph_info->edrx_info.rat_type_edrx == SYS_RAT_LTE_M1_RADIO_ACCESS)
  {
  ...
  switch ( cmd_type )
  {
    case DSAT_PENDING_CEDRXS_WRITE:
      {
    ...
    case DSAT_PENDING_CEDRXRDP_READ:   
      {
        dsm_item_type *res_buff_ptr = NULL;
        res_buff_ptr = dsat_dsm_new_buffer(DSM_DS_SMALL_ITEM_POOL, FALSE);       
        DS_AT_MSG2_ERROR("Get File attributes request on an invalid file  %d %d",ph_val->dsat_edrx_info.edrx_enabled,0);        
        DS_AT_MSG2_ERROR("DSAT_PENDING_CEDRXRDP_READ  %d ,%d ",ph_val->dsat_edrx_info.rat_type_edrx, ph_info->edrx_info.rat_type_edrx);
        SET_PENDING( DSATETSI_EXT_ACT_CEDRXRDP_ETSI_IDX, 0, DSAT_PENDING_CEDRXRDP_NONE )      
        
        if( ph_val->dsat_edrx_info.rat_type_edrx == ph_info->edrx_info.rat_type_edrx && 
            ph_val->dsat_edrx_info.edrx_enabled == TRUE )
        {
          ...
          else
          {
            ...
            if(edrx_rat.enabled)
            {
              dsat_bin_conversion(edrx_rat.cycle_length, req_edrx, sizeof(req_edrx), DSAT_MAX_EDRX_VAL_LENGTH);
            }
            else
            {
              req_edrx[0]='\0';
          ...
          dsat_bin_conversion(ph_val->dsat_edrx_info.edrx_cycle_length, edrx_cyc, sizeof(edrx_cyc), DSAT_MAX_EDRX_VAL_LENGTH);
    ...
    case DSAT_PENDING_CEDRXS_URC:
      {
        ...
        if( ph_info->edrx_info.edrx_enabled == TRUE )
        {
          ...
          dsat_bin_conversion(ph_info->edrx_info.edrx_ptw, edrx_ptw, sizeof(edrx_ptw), DSAT_MAX_EDRX_VAL_LENGTH);
          res_buff_ptr->used = (word) snprintf ( (char *) res_buff_ptr->data_ptr,
                                               res_buff_ptr->size,
                                               "CEDRXP: %X,\"%s\",\"%s\",\"%s\"",
                                               rat_ind,
