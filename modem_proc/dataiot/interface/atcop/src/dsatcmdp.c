...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatcmdp.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/20/17   skc     Fixed issue while flushing queued URCs.
05/01/17   skc     Featurizing PBM Code.
04/28/17   skc     Restricting URC to report if there is any active port
                   in case of multi-port scenrio.
...
---------------------------------------------------------------------------*/
nv_cmd_type ds_nv_baud_cmd_buf ={{0}};
...
void dsatcmdp_init( void )
{
  ...
  /*-------------------------------------------------------------------------
    Initialize dynamic allowed list of AT cmds based on EFS
  -------------------------------------------------------------------------*/
  dsat_init_allowed_list_efs();
...
void dsatcmdp_deinit( void )
{
  ...
  /*------------------------------------------------------------------------
    Deinitialize dynamic allowed list of AT cmds based on EFS
  -------------------------------------------------------------------------*/
  dsat_deinit_allowed_list_efs();
...
)
{
  ...
  if (NULL == dsatcmdp_sio_info_ptr[port_id])
  {
    ...
    dsatcmdp_sio_info_ptr[port_id]->cmd_line_buffer = dsat_alloc_memory(MAX_LINE_SIZE, FALSE);
    memset(dsatcmdp_sio_info_ptr[port_id]->cmd_line_buffer, 0, MAX_LINE_SIZE);
    /* Memory will be there till the scope of ATCoP Engine */

    dsatcmdp_sio_info_ptr[port_id]->prev_line_buffer = dsat_alloc_memory(MAX_LINE_SIZE, FALSE);
    memset(dsatcmdp_sio_info_ptr[port_id]->prev_line_buffer, 0, MAX_LINE_SIZE); 
    /* Memory will be there till the scope of ATCoP Engine */
...
#endif /* FEATURE_DSAT_MDM_FUSION */
  {
    ...
    */
...
#endif /* FEATURE_DSAT_EXT_CLIENT_SUPPORT */
    {
      ...
      DS_AT_MSG2_HIGH("ATCoP Operating mode = %d, Present mode = %d",
        curr_operating_mode, dsatcmdp_get_current_mode());
...
/*Displaying Command name */
      ...
      DS_AT_MSG2_HIGH("CPBS:: memory state:- %d, PBM pref = %d, ",
                      ...
                      (dsat_num_item_type)dsatutil_get_val(DSAT_VENDOR_QCPBMPREF_IDX,0,NUM_TYPE));
      ...
      if (NULL != me_val)
      {
        ...
        DS_AT_MSG2_HIGH("Pin State :: PIN REQURIED : %d PIN BLOCKED : %d",
                      me_val->dsat_pin_required,me_val->dsat_pin_blocked);
        ...
        if((me_val->dsat_mask_pin2_state == FALSE)&&
      ...
      if (ETSI_CMD_MODE == curr_operating_mode)
      {
        if (!(table_ptr->attrib & RESTRICTED))
        {
          ...
          if (TRUE == dsatcmdp_restricted_commands[gw_apps_id])
          {
      ...
      {           
        ...
        if ( table_ptr->proc_func != NULL )
        {
          ...
          result_code = table_ptr_ex->proc_func( dsatcmdp_at_state,
                                              ...
                                              raw_rsp_ptr );
                                              */
		  result_code = table_ptr->proc_func( dsatcmdp_at_state,
                                              table_ptr,
                                              &(token_ptr->token),
                                              raw_rsp_ptr );
        }
  ...
  if (NULL != formatted_rsp_ptr)
  {
    ...
    if ( result_code != DSAT_NO_RSP )
    {
      ...
      if ((ETSI_CMD_MODE == curr_operating_mode))
      {
        ...
        if(((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CGEREP_IDX,0,
...
#endif /* FEATURE_DSAT_ETSI_DATA */
      dsatcmdp_flush_urc_on_all_unblocked_port();
    }
...
)
{
  ...
  if (buf_ptr != NULL && total_tokens < TOKEN_BUF_CNT)
  {
    ...
    for ( i = 0; i < token_ptr->args_found; i++ )
    {
      ...
      buf_ptr->token.arg_length[i] = token_ptr->arg_length[i];
    }
    ...
    {
      DS_ASSERT( token_fill_ptr < &token_buffer[TOKEN_BUF_SIZE] );
...
)
{
  ...
  DS_ASSERT( token_empty_ptr >= token_buffer &&
...
DESCRIPTION
  This function handle URC which will be either send to port or queue it.
...
void dsatcmdp_urc_send_or_queue
(
  dsm_item_type           *dsm_item_ptr
)
{
  ...
  {
    DS_AT_MSG0_ERROR("dsm_item_ptr NULL");
    ASSERT(0);
  }

  buffer_size = (uint16) dsm_length_packet(dsm_item_ptr);
  ...
  for (port_id = DS3G_SIOLIB_DATA_PORT; port_id < DS3G_SIOLIB_PORT_MAX; port_id++)
  {
    ...
    if (SIO_NO_STREAM_ID != state->serial_info.stream_id && state->at_state != DS3G_SIOLIB_ONLINE_DATA_AT_STATE &&
        state->serial_info.serial_state == DS3G_SIOLIB_AUTODETECT_SERIAL_STATE)
    {
      (void)ds3g_siolib_ex_is_dtr_asserted(&dtr_status, port_id);

      DS_AT_MSG2_HIGH("port_id %d dtr_status %d", port_id, dtr_status);
      ...
      if (TRUE == dtr_status)
      {
        ...
        if (buffer_size != dsm_dup_packet(&dup_buffer, dsm_item_ptr, 0, buffer_size))
        {
...
void dsatcmdp_flush_urc_on_all_unblocked_port(void)
{
  ...
  for (port_id = DS3G_SIOLIB_DATA_PORT; port_id < DS3G_SIOLIB_PORT_MAX; port_id++)
  {
    ...
    if (SIO_NO_STREAM_ID != state->serial_info.stream_id)
    {
      ...
      if (TRUE == dtr_status)
      {
        /* If the port info initialized i.e. first AT command is received on the port
           and port is not blocking, we will flush URC.
           If blocked, we will flush after processing of AT Command on that port */
        if (TRUE == dsatcmdp_is_sio_info_inited(port_id))
        {
          sio_info_ptr = dsatcmdp_get_sio_info_ptr(port_id);
          if(!sio_info_ptr->block_indications_flag)
          {
...
)
{
                    ...
                    q_get( &sio_info_ptr->urc_msg_q )) != NULL )
  {
    ...
    DS_AT_MSG3_HIGH("URC: flushed - Q count: %d, DSM item ptr: %p for port_id %d",
                    q_cnt(&sio_info_ptr->urc_msg_q), msg_ptr->dsm_item_ptr, port_id);
...
boolean dsatcmdp_abort_command( void )
{
       ...
       ( abort_cmd_table_entry_ptr_ex != NULL ) )
  {
    ...
    DS_ASSERT( abort_cmd_table_entry_ptr_ex->abort_func != NULL );
  ...
  else if (curr_ext_at_abort_fwd_ptr != NULL) 
  {
    /* Verify abort handler pointer is valid.  Pointer should never be
       NULL when this function is called to abort a command. */
    DS_ASSERT( curr_ext_at_abort_fwd_ptr->ext_at_abort_cb != NULL );
    ...
    if(dsatcmdp_abortable_state != NOT_ABORTABLE)
    {
      ...
      */
      ...
      at_handle = (uint32)dsatcmdp_processing_fwd_cmd.port;
...
)
{
  ...
  if ( (dsat_num_item_type)dsatutil_get_val(
                               DSAT_EXT_FCLASS_IDX,0,NUM_TYPE) == 2 )
  {
    temp_val = (dsat_num_item_type)dsatutil_get_val(DSAT707_FAX_FLO_IDX,0,NUM_TYPE);
  ...
  {
    temp_val = (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_IFC_IDX,0,NUM_TYPE);
    tx_flow_ctl = convert_flow_control( (flow_ctl_type) temp_val,TRUE );
    temp_val = (dsat_num_item_type)dsatutil_get_val(DSAT_EXT_IFC_IDX,1,NUM_TYPE);
    rx_flow_ctl = convert_flow_control( (flow_ctl_type)temp_val ,TRUE );
  }
...
#ifdef FEATURE_DSAT_ETSI_DATA
  gprs_dd_val = dsat_get_base_addr(DSAT_MD_VALS, TRUE);
  if (NULL != gprs_dd_val)
  {
    if(((dsat_num_item_type)dsatutil_get_val( DSATETSI_EXT_CGEREP_IDX,0,
            NUM_TYPE) == DSAT_CGEREP_BUFFER_FWD_TE )||(gprs_dd_val->flush_gprs_buffer == TRUE))
...
dsat_result_enum_type dsatcmdp_process_cmd_forwarding ( void )
{
  ...
  {
    DS_AT_MSG0_HIGH("at forward list is empty: do not check queue");
    return DSAT_OK;
  }
  ...
  {	
    ...
    {
       DS_AT_MSG_SPRINTF_2_HIGH("Found match for remote client processing cmd=%s Port = %d",
                 token_ptr->token.name,(int)active_port);
      me_val = dsat_get_base_addr(DSAT_MMGSDI_VALS, TRUE);
      if ((NULL != me_val) && (me_val->dsat_mask_pin2_state == FALSE))
      {
         me_val->dsat_mask_pin2_state = TRUE;
      }
      ...
      dsatcmdp_processing_fwd_cmd.client_id= fwd_at_cmd_p->client_id;
 ...
 #ifdef FEATURE_DSAT_ATCOP_VEND
      if ( 0 ==  dsatutil_strcmp_ig_sp_case( (const byte *)token_ptr->token.name,(const byte *)"+IPR"))
      {
        if((token_ptr->token.arg[0] != NULL) &&(token_ptr->token.op == (NA|EQ|AR)))
        {
          if( ATOI_OK == dsatutil_atoi(&value ,token_ptr->token.arg[0] ,10))
          {
            //get the index from the table
            baud = dsatvend_get_ipr_baud_rate(token_ptr->token.arg[0]);		  
            cm_psm_set_uart_baude_rate((int)baud);
          }
        }
      }
#endif
                                       ...
                                       fwd_at_cmd_p->user_info_ptr))
      {
        ...
        /* Check if command is abortable and set flag if it is. Also,
           ...
           set flag to insure command abort processing will be ended
           if send response function is called (default behavior). */
        if ( fwd_at_cmd_p->is_abortable )
        {
         DS_ASSERT( fwd_at_cmd_p->ext_at_abort_cb != NULL );

          dsatcmdp_abortable_state = ABORTABLE;
          /* Command is abortable and save pointer
             to external abort CB and set SIO preprocessor mode to
             abort command mode. */
          curr_ext_at_abort_fwd_ptr = fwd_at_cmd_p;
          dsatprep_set_preprocessor_mode(DSATI_ABORT_CMD_MODE, active_port);
        }
        dsatcmdp_end_abort_in_send_rsp = TRUE;         
        /* Process command if processing function pointer not NULL. */
...
)
{
  ...
  boolean                      block_indications_flag = FALSE;
  boolean                      is_port_sio_info_inited = FALSE;

  if ((dsat_num_item_type) dsatutil_get_val(DSAT_BASIC_Q_IDX,0,NUM_TYPE) == 0)
  {
          ...
          else
          {
            ...
            else
            {
              ...
              else
              {
                ...
                else if (is_port_sio_info_inited)
                {
                  /* Queue URC only when atleast one AT command has been run on this port
                   * otherwise discard the URC
                   */
...
{
  process_send_urc(buffer, rsp_type);
}
...
)
{
  ...
  switch(dsat_rmsm_cmd)
  {
    case DSAT_RMSM_CGACT_CMD:
      /* Command posting will be done within dsatetsipkt_cgact_done_handler for CGACT*/
      dsatetsipkt_cgact_done_handler(dsat_rmsm_info, dsat_rmsm_info.user_info_ptr);
      break;
    case DSAT_RMSM_QCDGEN_CMD:
      if(CHECK_PENDING(DSAT_VENDOR_QCDGEN_IDX,0,DSAT_PENDING_TRUE))
      {
        SET_PENDING(DSAT_VENDOR_QCDGEN_IDX ,0, DSAT_PENDING_FALSE)
        post_cmd = TRUE;
      }
      break;
    case DSAT_RMSM_CSODCP_CMD:
      if(CHECK_PENDING(DSATETSI_EXT_CSODCP_IDX,0,DSAT_PENDING_TRUE))
      {
        SET_PENDING(DSATETSI_EXT_CSODCP_IDX ,0, DSAT_PENDING_FALSE)
        post_cmd = TRUE;
      }
      break;
    default:
...
{
  ds_at_crtdcp_cmd_type *crtdcp_info_ptr = NULL;
  ds_command_type       *cmd_buf  = NULL;
  ...
  crtdcp_info_ptr = cmd_buf->cmd_payload_ptr;

  cmd_buf->hdr.cmd_id = DS_CMD_ATCOP_CRTDCP_CMD;

  crtdcp_info_ptr->profile_number = profile_number;
  crtdcp_info_ptr->data_frame = *data_frame;
  ds_put_cmd(cmd_buf);
}
...
)
{
  ...
  DS_ASSERT((format_cmd_ptr != NULL) && (cmd_name != NULL) && (token_ptr != NULL));
  ...
  temp1 = (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S3_IDX,0,NUM_TYPE);
  format_cmd_ptr->data_ptr[format_cmd_ptr->used++] = temp1;
  temp1 = (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S4_IDX,0,NUM_TYPE);
  format_cmd_ptr->data_ptr[format_cmd_ptr->used++] = temp1;
