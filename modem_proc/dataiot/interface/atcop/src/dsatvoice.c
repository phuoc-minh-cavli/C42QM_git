...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatvoice.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/21/16   skc     Fixed $QCCHV issue
...
extern voice_state_type voice_state[CM_CALL_ID_MAX];
  ...
  for ( call_id = 0; call_id < CM_CALL_ID_MAX; call_id++ )
  {
    ...
    if ( voice_state[call_id].state == DSAT_VOICE_STATE_WAITING )
    {
...
) )
      {
        ...
        voice_state[call_id].cmd_active = VOICE_CMD_ATA;
...
)
{
  ...
  else
  {
    ...
    if ( VOICE_CMD_ATD == voice_state[call_id].cmd_active || 
         VOICE_CMD_ABORT == voice_state[call_id].cmd_active )
    {
...
)
{
  ...
  for ( call_id = 0; call_id < CM_CALL_ID_MAX; call_id++ )
  {
    ...
    if (( voice_state[call_id].state == DSAT_VOICE_STATE_WAITING )||
       ( voice_state[call_id].state == DSAT_VOICE_STATE_ACTIVE ))
    {
...
{
  return dsatvoice_is_voice_call( DSAT_CALLID_ALL );
}
...
)
{
  ...
  if( DSAT_CALLID_ALL == call_id )
  {
    ...
    {
      /* If DSAT_VOICE_STATE_WAITING or DSAT_VOICE_STATE_ACTIVE call ID is found (there should be only one).*/
      if (( voice_state[index].state == DSAT_VOICE_STATE_WAITING )||
         ( voice_state[index].state == DSAT_VOICE_STATE_ACTIVE ))
      {
        return TRUE;
      } 
    }
...
{
  voice_send_result_code(DSAT_CRC_CODE_VOICE);
}
