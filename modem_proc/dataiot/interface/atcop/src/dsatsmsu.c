...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatsmsu.c#1 $
...
)
{
  ...
  mem_store = dsatsmsi_memory_lookup((int)dsatsmsi_get_mem_val(CPMS_MEM1,cmd_mode), 
                                     CPMS_MEM1,cmd_mode);
...
)
{
  ...
  if(cmd_mode == ETSI_CMD_MODE)
  {
    mem_val = (dsat_num_item_type)dsatutil_get_val(
                     DSATETSI_EXT_ACT_CPMS_ETSI_IDX,mem_index,NUM_TYPE);
  }
  else 
#endif /* FEATURE_ETSI_SMS */
  {
...
)
{
  ...
  mem_val = dsatsmsi_get_mem_val(CPMS_MEM1,cmd_mode);
