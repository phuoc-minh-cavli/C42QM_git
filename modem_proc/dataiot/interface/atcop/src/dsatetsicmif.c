...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsicmif.c#4 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
05/15/23   ks      Enhancing +COPS, +CEREG commands to support NTN RAT.
04/20/18   ya	   Macro to Function conversion
10/16/17   rk      Fixed compilation issue in ATCoP for LTE B66.
08/19/16   skc     Fixed issue with USSD Notification and CMGS
...
)
{
  ...
  DS_ASSERT(dial_ptr->attrib & DIAL_ATTRIB_SEMICOLON);
  ...
  if ( call_val->etsicall_num_active_calls > 0 )
  {
    ...
    call_val->etsicall_was_call_active = TRUE;
  }
  ...
  if(dial_ptr->attrib & DIAL_ATTRIB_UP_G)
  {
    ...
    set_call_param_by_ccug(&call_orig_params.forward_cug_info, 
      (const dsat_num_item_type *)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,0,STR_TYPE));
  }
  ...
  else
  {
    ...
    if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,0,NUM_TYPE) == 0)
    {
      call_orig_params.forward_cug_info.present = FALSE;
    }
    else
...
/*---------------------------------------------------------*/
  ...
  user_pref.pref_duration = 0;
  user_pref.srv_domain_pref = CM_SRV_DOMAIN_PREF_PS_ATTACH;
  
  cm_result =cm_user_pref_update_req(&user_pref,
...
)
{
  ...
  if (first_call)
  {
    ...
    sups_val->dsat_ss_cm_data.done_class = 0;
    sups_val->dsat_ss_cm_data.pending_msg = FALSE;
    sups_val->dsat_ss_cm_data.pending_rsp = FALSE;
  ...
  if (sups_val->dsat_ss_cm_data.user_data.class == sups_val->dsat_ss_cm_data.done_class) 
  {
    dsm_item_type * res_buff_ptr;
      
    /* Clear password if FACILITY operation */
    if (DSAT_SSOP_FACILITY == sups_val->dsat_ss_cm_data.user_data.operation)
    {
      dsatutil_reset_password (
        sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.passwd,
        sizeof(sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.passwd));
    }

    /* Send a line terminator at end of response series (if any) */
    if (TRUE == sups_val->dsat_ss_cm_data.pending_rsp)
    {
  ...
  /* Current class is all classes specified in argument to AT command and 
     not yet processed */
  sups_val->dsat_ss_cm_data.curr_class = 
    sups_val->dsat_ss_cm_data.user_data.class ^ sups_val->dsat_ss_cm_data.done_class;
  
  /* Check for out of control loop (should not happen) */
  if (((uint16)DSAT_SS_CLASS_ALL < sups_val->dsat_ss_cm_data.curr_class) ||
      ((uint16)DSAT_SS_CLASS_MIN == sups_val->dsat_ss_cm_data.curr_class))
  {
  ...
  while ( DSAT_SS_CLASS_MAX != dsat_bsg_conversion_table[index].class )
  {
    ...
    if ( ( sups_val->dsat_ss_cm_data.curr_class & 
         ...
         (uint16)dsat_bsg_conversion_table[index].class )
    {
      ...
      sups_val->dsat_ss_cm_data.curr_class = (uint16)dsat_bsg_conversion_table[index].class;
      break;
    }
    index++;
  ...
  DS_ASSERT( bsg_type != MAX_CODE);

  /* Invoke the correct Call Manager API */  
  DS_AT_MSG1_MED("Sending message to CM for class: %d",
           dsat_bsg_conversion_table[index].class);

  switch (sups_val->dsat_ss_cm_data.user_data.operation)
  {
    /* Facility class action */
    case DSAT_SSOP_FACILITY:
      /* Call appropriate API based on Mode */
      switch (sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.mode)
      {
        case DSAT_CLCK_MODE_UNLOCK:
          sups_val->dsat_ss_cm_data.cm_parms.deact.ss_code = sups_val->dsat_ss_cm_data.sc_code;
          sups_val->dsat_ss_cm_data.cm_parms.deact.basic_service.present = 
            (NO_CODE == bsg_type)? FALSE: TRUE;
          sups_val->dsat_ss_cm_data.cm_parms.deact.basic_service.bs_code = bsg_code;
          sups_val->dsat_ss_cm_data.cm_parms.deact.basic_service.code_type = bsg_type;
          /*Building MMI string */
          dsat_ss_params.mode = MCS_SUPS_MODE_DISABLE;
          dsat_ss_params.code = sups_val->dsat_ss_cm_data.sc_code;
          dsat_ss_params.service_class = sups_val->dsat_ss_cm_data.user_data.class;
...
#ifdef FEATURE_ATCOP_TEMP_CM_TODO
          cm_result = cm_sups_cmd_deactivate_exten(etsicmif_sups_exten_cmd_cb_func, 
                                             &sups_val->dsat_ss_cm_data,
                                             dsatcm_client_id,
                                             &sups_val->dsat_ss_cm_data.cm_parms.deact,
                                             &dsat_dialed_digits,
                                             CM_ORIG_CLIENT_CATEG_USER);
#else
          cm_result = FALSE;
        ...
        case DSAT_CLCK_MODE_LOCK:
          ...
          sups_val->dsat_ss_cm_data.cm_parms.act.ss_code = sups_val->dsat_ss_cm_data.sc_code;
          sups_val->dsat_ss_cm_data.cm_parms.act.basic_service.present = 
            (NO_CODE == bsg_type)? FALSE: TRUE;
          sups_val->dsat_ss_cm_data.cm_parms.act.basic_service.bs_code = bsg_code;
          sups_val->dsat_ss_cm_data.cm_parms.act.basic_service.code_type = bsg_type;
          /*Building MMI string */
          ...
          cm_result = cm_sups_cmd_activate_exten(etsicmif_sups_exten_cmd_cb_func, 
                                           &sups_val->dsat_ss_cm_data,
                                           dsatcm_client_id,
                                           &sups_val->dsat_ss_cm_data.cm_parms.act,
                                           &dsat_dialed_digits,
                                           CM_ORIG_CLIENT_CATEG_USER);
#else
          cm_result = FALSE;
        ...
        case DSAT_CLCK_MODE_QUERY:
          sups_val->dsat_ss_cm_data.cm_parms.interr.ss_code = sups_val->dsat_ss_cm_data.sc_code;
          sups_val->dsat_ss_cm_data.cm_parms.interr.basic_service.present = FALSE;
          sups_val->dsat_ss_cm_data.cm_parms.interr.basic_service.bs_code = bsg_code;
          sups_val->dsat_ss_cm_data.cm_parms.interr.basic_service.code_type = bsg_type;
          /*Building MMI string */
          ...
          cm_result = cm_sups_cmd_interrogate_exten(etsicmif_sups_exten_cmd_cb_func, 
                                              &sups_val->dsat_ss_cm_data,
                                              dsatcm_client_id,
                                              &sups_val->dsat_ss_cm_data.cm_parms.interr,
                                              &dsat_dialed_digits,
                                              CM_ORIG_CLIENT_CATEG_USER);
#else
          cm_result = FALSE;
        ...
        default:
          DS_AT_MSG1_ERROR("Unrecognized mode value: %d",
                    sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.mode);
          return DSAT_ERROR;
      }
      if (cm_result)
      {
        DS_AT_MSG0_HIGH("Setting +CLCK pending  True");
        sups_val->dsat_ss_cm_data.pending_clck_pwd_ind = TRUE;
      }
      break;

    default:
      DS_AT_MSG1_ERROR("Unrecognized operation value: %d",
               sups_val->dsat_ss_cm_data.user_data.operation);
      return DSAT_ERROR;
  }
...
dsat_result_enum_type dsatetsicmif_sups_change_password (void)
{
  ...
  reg_pwd_cmd_parms.ss_code.ss_code = sups_val->dsat_ss_cm_data.sc_code;
    /*Building MMI string */
  ...
  (void) dsatutil_memscpy((void*)reg_pwd_cmd_parms.ss_old_pwd.ss_password,MAX_PWD_CHAR,
          (void*)sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.passwd,
          MAX_PWD_CHAR );
  reg_pwd_cmd_parms.ss_old_pwd.present = TRUE;
  dsat_ss_params.cmd.passwd.old_passwd = (char *)&sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.passwd[0];

  /* New Password */
  (void) dsatutil_memscpy((void*)reg_pwd_cmd_parms.ss_new_pwd.ss_password,MAX_PWD_CHAR,
          (void*)sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.new_passwd,
          MAX_PWD_CHAR );
  reg_pwd_cmd_parms.ss_new_pwd.present = TRUE;
  dsat_ss_params.cmd.passwd.new_passwd =(char *) &sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.new_passwd[0];
  ...
  dsat_ss_params.cmd.passwd.new_passwd_again = (char *)&sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.new_passwd[0];
...
dsat_result_enum_type dsatetsicmif_sups_process_ussd (void)
{
  ...
  switch (sups_val->dsat_ss_cm_data.ussd_state)
  {
    ...
    case DSAT_CUSD_STATE_SS_REQUEST:
      result = etsicmif_sups_send_ussd_request_response( FALSE, 0);
      break;
      
    default:
      DS_AT_MSG1_ERROR("Unsupported USSD state: %d",
                sups_val->dsat_ss_cm_data.ussd_state);
      result = DSAT_ERROR;
  }
...
)
{
  ...
  switch (sups_cmd->cmd)
  {
    ...
    case CM_SUPS_CMD_USS_RES:
      switch (sups_cmd->cmd_err)
      {
        ...
        case CM_SUPS_CMD_ERR_OFFLINE_S:
          if(sups_val->dsat_interr_ss_service == clip || 
             ...
             sups_val->dsat_interr_ss_service == clir )
          {
            result = dsatetsicall_report_ss_lineid_status(
                     &sups_val->dsat_interr_ss_service, NULL, FALSE);
          }
          else
          {
      ...
      if ((sups_val->dsat_ss_cm_data.user_data.operation == DSAT_SSOP_FACILITY)&&
                 (sups_cmd->cmd_err != CM_SUPS_CMD_ERR_NOERR))
      {
        sups_val->dsat_ss_cm_data.pending_clck_pwd_ind = FALSE;
      }
      break;
    ...
    case CM_SUPS_CMD_ABORT:
      ...
      if (sups_val->dsat_ss_cm_data.user_data.operation == DSAT_SSOP_FACILITY)
      {
        ...
        DS_AT_MSG1_HIGH("Clearing Panding variable status %d",
                    sups_val->dsat_ss_cm_data.pending_clck_pwd_ind);
      }
...
)
{
  ...
  /* control loop over +CLCK class parameter values */
  ...
  switch (sups_event->event)
  {
    ...
    case CM_SUPS_EVENT_REG_PASSWORD_CONF:
      ...
      else
      {
        ...
        if(sups_val->dsat_ss_abort_status == TRUE)
        {  
          /* We don't have to do anything */
           sups_val->dsat_ss_abort_status = FALSE;
        }
      }
      if (sups_val->dsat_ss_cm_data.user_data.operation == DSAT_SSOP_FACILITY)
      {
      ...
      if (einfo_ptr->ss_success)
      {
        ...
        switch(einfo_ptr->conf_type)
        {
          ...
          case BSG_LIST_INFO:
            ...
            {
              case DSAT_SSOP_FACILITY:
                result =  dsatetsicmif_report_clck (sups_event, NULL);
                break;
          ...
          case SS_STATUS_INFO:
            if(sups_val->dsat_interr_ss_service == colp || 
               sups_val->dsat_interr_ss_service == clip ||
               sups_val->dsat_interr_ss_service == allLineIdentificationSS ||
               sups_val->dsat_interr_ss_service == clir )
            {
              result = dsatetsicall_report_ss_lineid_status(
                         &sups_val->dsat_interr_ss_service, &einfo_ptr->ss_status, TRUE);
            }
            else if(sups_val->dsat_ss_abort_status == TRUE)
            {
          ...
          case CC_CAUSE_INFO:
            ...
            if(sups_val->dsat_ss_abort_status == FALSE)
            {
              result = dsat_send_cme_error(DSAT_CME_SUPS_NETWORK_REJECTED);
            }
            else
          ...
          case CLI_RESTRICTION_INFO:
            /* Introduced for +CLIR */
            if(sups_val->dsat_interr_ss_service == clir)
            {
              result = dsatetsicall_report_clir_status(sups_event);
            }
            else if(sups_val->dsat_ss_abort_status == TRUE)
            {
              /* We don't have to do anything */
    ...
    case CM_SUPS_EVENT_GET_PASSWORD_IND:
      ...
      result = etsicmif_sups_send_password(einfo_ptr); 
      sups_val->dsat_ss_cm_data.pending_clck_pwd_ind = FALSE;
      break;
    ...
    case CM_SUPS_EVENT_PROCESS_USS_CONF:
      if(!einfo_ptr->ss_success)
      {
        ...
        if (1 == (dsat_num_item_type)dsatutil_get_val(
                               DSATETSI_EXT_ACT_CUSD_ETSI_IDX,0,MIX_NUM_TYPE))
        {
          result = dsatetsicmif_gen_cmee_error (DSAT_CME_SUPS_UNKNOWN_NETMSG);
        }
      }

      /* Cleanup USSD transaction */
      dsatetsicmif_set_cusd_state(&(sups_val->dsat_ss_cm_data), DSAT_CUSD_STATE_NULL, 0);
      break;
    default:
      break;
  }
  ...
  if ((DSAT_ASYNC_EVENT == result) &&
      (TRUE == class_loop) &&
      (FALSE == sups_val->dsat_ss_cm_data.abort_cmd))
  {
    /* Generate next CM message (possible) */
    sups_val->dsat_ss_cm_data.done_class |= sups_val->dsat_ss_cm_data.curr_class;
    result = dsatetsicmif_sups_class_action (FALSE);
  }
...
)
{
  ...
  sups_val->dsat_ss_cm_data.abort_cmd = TRUE;
...
)
{
  ...
  if(DSAT_OK == etsipkt_get_profile_list(&profile_list,&profile_count))
  {
    DS_SYSTEM_HEAP_MEM_FREE(profile_list);    
  }
  dsat_pdp_state.valid_profile_count = profile_count;
...
)
{
  ...
  ciot_lte_pref.lte_nb1_ntn_band_pref = SYS_LTE_BAND_MASK_CONST_NO_CHG;
...
LOCAL boolean etsicmif_sups_lookup_netsc ()
{
  ...
  while (DSAT_SSOP_MAX != sc_conversion_table[index].operation )
  {
    ...
    {
      if ((DSAT_SSOP_FACILITY == sups_val->dsat_ss_cm_data.user_data.operation) &&
           (sc_conversion_table[index].code == (uint8)sups_val->dsat_ss_cm_data.user_data.opdata.fac_lock.facility))
      {
        sups_val->dsat_ss_cm_data.sc_code = sc_conversion_table[index].net_sc;
        result = TRUE;
        break;
      }
    }
  ...
  DS_ASSERT( sups_val->dsat_ss_cm_data.sc_code > 0 );
...
)
{
  ...
  { 
    result= dsatetsicmif_gen_cmee_error(error_code); 
  } 
  else 
  { 
...
)
{
  ...
  if (sups_val->dsat_ss_cm_data.pending_clck_pwd_ind != TRUE)
  {
...
LOCAL dsat_result_enum_type etsicmif_sups_send_ussd_cmd ( void )
{
  ...
  ssus_ptr = &sups_val->dsat_ss_cm_data.user_data.opdata.ussd;
...
)
{
  ...
  /* Setup USS Data structure */
  etsicmif_get_cusd_info(&(sups_val->dsat_ss_cm_data), &uss_res_parms.invoke_id, NULL);
  ...
  sups_val->dsat_ss_cm_data.pending_msg = TRUE;
...
LOCAL dsat_result_enum_type etsicmif_sups_send_release( void )
{
  ...
  etsicmif_get_cusd_info(&(sups_val->dsat_ss_cm_data), &release_cmd_parms.ss_ref, &release_cmd_parms.ss_oper);
...
DESCRIPTION
  This function sends error response back to network. 

DEPENDENCIES
  None
...
(
  boolean                    error,
  byte                       code,
  cusd_state_e_type          sups_val
)
{
  if( DSAT_CUSD_STATE_SS_NOTIFY == sups_val) 
  { 
    (void)etsicmif_sups_send_ussd_notify_response( error, code ); 
  } 
  else 
  { 
    (void)etsicmif_sups_send_ussd_request_response( error, code ); 
  }
...
)
{
  ...
  #define SEND_RESPONSE( error, code ) \
  			send_response(error, code, sups_val->dsat_ss_cm_data.ussd_state )
...
)
{
  ...
  if( ((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CTZR_ETSI_IDX,
                    0,NUM_TYPE) == TRUE )&& event_info->last_reported_tz_changed == TRUE )
  {
    dsm_item_type *res_buff_ptr;
...
)
{
  ...
  if (NULL != sups_event)
  {
    ...
    sups_val->dsat_ss_cm_data.pending_rsp = TRUE;
