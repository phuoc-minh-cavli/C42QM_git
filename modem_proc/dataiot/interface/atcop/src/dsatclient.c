...
DESCRIPTION
  ...
  Copyright (c) 2005 - 2018 by Qualcomm Technologies Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatclient.c#1 $
...
FUNCTION DSATCLIENT_SEND_PROCESS_RESPONSE

DESCRIPTION
  This function will send the response message.
...
void dsatclient_send_process_resp
(
  dsat_ext_client_id_type     client_id,
  void*                       client_data,
  boolean                     passfail,
  dsat_fwd_resp_enum_type     rsp_typ,
...
#define DSATCLIENT_SEND_PROCESS_RESP(passfail , rsp_typ)\
		  dsatclient_send_process_resp(client_id,client_data,passfail,rsp_typ,cb_func)
  ...
  */
    ...
    ( current_client == client_id ))
  {
    ...
    local_cmd_buff = (char *) dsat_alloc_memory(cmd_len + 1,FALSE);
    ...
    local_cmd_buff[cmd_len++]= (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S3_IDX,0,NUM_TYPE);
