...
$Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatutil.c#3 $ $DateTime: 2020/10/21 00:45:19 $ $Author: pwbldsvc $


when       who     what, where, why
--------   ---     ------------------------------------------------------------------
05/22/20   ks      Fixed AT command not throwing error when string is not null-terminated 
                   after end quote found.
04/10/20   sp      Fixed sending small letters issue in UCS2 format.
...
)
{
       ...
       index++)
  {
    ...
    {
      if(quoted_str[index + 2] != '\0')
      {
        DS_AT_MSG0_MED("String is not terminated after end quote found");
        return FALSE;
      }
...
)
{
  ...
  {
    stripped_arg = dsat_alloc_memory(arg_length + 1, FALSE);
    ...
    switch (action_quote)
    {
      case ACTION_QUOTE_BOTH_OK:
        ...
        if(arg[0] == '"')
        {
          (void) dsatutil_strip_quotes_out(arg, stripped_arg, arg_length + 1);
      ...
      case ACTION_QUOTE_YES_QUOTE:
        ...
        if(!dsatutil_strip_quotes_out(arg, stripped_arg, arg_length + 1))
        {
          dsatutil_free_memory(stripped_arg);
          return PARAM_MISS;
        }
        use_arg = stripped_arg;
        break;
...
)
{
   ...
   //@SANJAYKC: TODO
   cm_pref_mode = ds_pdn_hdlr_get_current_network_mode();
        ...
        if (ATOI_OK == dsatutil_atoi(&cid, options[SC98_OPTION], 10))
        {
          if ( cid > MAX_BYTE_VALUE ||
               DS_PROFILE_RESULT_SUCCESS != 
                 ds_profile_setting_from_profile_id( (uint16)cid,
                                                        &profile))
          {
  ...
  else
  {
    ...
    if(
       ...
       ( (!(dial_ptr->attrib & DIAL_ATTRIB_DN_G)) && 
         ((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,0,NUM_TYPE) == 1))
      )
    {
      mod_info_ptr->modifier_present = TRUE;
      ...
      if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,1,NUM_TYPE) != 10)
      {
	if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,1,NUM_TYPE) > 9)
        {
          DS_ASSERT(0);
          return DSAT_ERROR;
        }
        ...
        mod_info_ptr->modifier_info.cug.cug_index_val = (byte)
          (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,1,NUM_TYPE);
      }
      else
      {
      ...
      switch((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,2,NUM_TYPE))
      {
        case 0:
        ...
        default:
          DS_AT_MSG1_HIGH("incorrect dsat_ccug_val[2]: %d", (dsat_num_item_type)
            dsatutil_get_val(DSATETSI_EXT_ACT_CCUG_ETSI_IDX,2,NUM_TYPE));
          mod_info_ptr->modifier_info.cug.suppress_pref_cug = FALSE;
          mod_info_ptr->modifier_info.cug.suppress_outgoing_access = FALSE;
          result = DSAT_ERROR;
          break;
...
)
{
  ...
  if( nv_status == NV_DONE_S)
  {
    ...
    memset ((void*)&dsat_pkt_dial_string_table, 0,sizeof(dsati_pkt_dial_string_table_s_type));
...
)
{
  ...
  DS_ASSERT(NULL == state_ptr->cmd_mem_ptr);

  state_ptr->cmd_mem_ptr = data_ptr;
  state_ptr->cmd_idx     = cmd_idx;
...
void dsatutil_free_cmd_mem_ptr(void)
{
  ...
  {
    if(state_ptr->cmd_idx == DSAT_BASIC_ACT_D_IDX)
    {
#ifdef FEATURE_ETSI_PBM
      etsime_dd_type *ptr = (etsime_dd_type*)state_ptr->cmd_mem_ptr;
      if(ptr->str)
      {
        dsatutil_free_memory(ptr->str);
      }
    ...
    state_ptr->cmd_idx = DSAT_BASIC_ACT_MAX_IDX;
  }
...
)
{
  ...
  DS_ASSERT(( in_str != NULL ) && ( out_str != NULL ));
   ...
   switch ( in_chset )
    {
      ...
      case ALPHA_UCS2:
        ...
        if ( strchr(hex_chars, UPCASE(*in_str_dup)) == NULL ||
             strchr(hex_chars, UPCASE(*(in_str_dup + 1))) == NULL ||
             strchr(hex_chars, UPCASE(*(in_str_dup + 2))) == NULL ||
             strchr(hex_chars, UPCASE(*(in_str_dup + 3))) == NULL ) 
        {
          return NULL;
        }
...
)
{
  ...
  DS_ASSERT( NULL != gsm_out && NULL != *gsm_out );
...
)
{
  ...
  DS_ASSERT( NULL != gsm_in && NULL != *gsm_in && NULL != pbm_out );
...
#define MASK_BITS_LESS_THAN_8 0x07
  ...
  DS_ASSERT( ( NULL != addr_mask_num ) && ( NULL != addr_mask_str ) );
  ...
  if ( (STRING_TO_INT == mode && DELIMITERS_PER_IPV6 == num_delimiters ) ||
       (INT_TO_STRING == mode && DS_PROFILE_IP_V6 == *ip_vsn) )
  {
    ...
    /* for subnet mask to string conversion create a dummy address representing
      ...
      cgpiaf_subnet_format  = (dsat_num_item_type)dsatutil_get_val(
                                     DSATETSI_EXT_CGPIAF_IDX,1,NUM_TYPE);
...
{
  const char *hex_string_ptr = hex_string;
  uint32 idx;
  uint8  int_val;

  if(hex_string_ptr == NULL || 
     (hex_string_ptr + 1) == NULL || /* Hex string should have alteast two characters*/
      buf_out == NULL ||             /* Memory should be already allocated */
      num_octets == 0)
  {
    return FALSE;
  }

  for(idx = 0; ((idx < num_octets) && (hex_string_ptr != NULL)); idx++ )
  {
    int_val = 0;
    /* Taking two characters and converting to decimal value */
    if(!dsatutil_hex_to_uint8(hex_string_ptr, &int_val))
    {
      return FALSE;
    }
    /* Storing in buffer */
    *(buf_out + idx) = int_val;
    /* Look for next 2 characters for hex */
    hex_string_ptr += 2;
  }
