...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 1995 - 2019 by Qualcomm Technologies Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatpar.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
/* Buffer to store token line data in. We will leave it in global space for debugging purpose.
   Memory will be allocated in dsatcmdp_process_cmd_line and will be there until the scope of ATCoP Engine */
LOCAL byte *token_line = NULL;
...
)
{
  ...
  if(token_line == NULL)
  {
    token_line = (byte*)dsat_alloc_memory(MAX_LINE_SIZE, FALSE);
    if(token_line == NULL)
    {
  ...
  while ( *curr_char_ptr != '\0' && result_code == DSAT_OK )
  {
    ...
    switch ( UPCASE( *curr_char_ptr ) )
    {
      ...
      case 'D':
        ...
        DS_ASSERT( result_code != DSAT_UNREC );
        if ( result_code != DSAT_ERROR )
        {
          result_code = dsatcmdp_queue_token( &token );          
        }
    ...
    while ( (c = *r_ptr) != '\0' && c != ';' && t != 0 && *htp_ptr )
    {
      if ( c != ' ' )
      {
        if ( UPCASE( c ) == *htp_ptr )
        {
          ...
          DS_ASSERT( w_ptr < &tok_ptr->working_at_line[MAX_LINE_SIZE] );
