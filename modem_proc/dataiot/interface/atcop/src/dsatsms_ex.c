...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatsms_ex.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
sms_cmd_sm_type *dsatsmsi_cmd_sm =NULL;
...
LOCAL wms_memory_store_e_type dsatsms_full_mem_store = {0};
#endif /*  FEATURE_ETSI_SMS */
      ...
      if(handler_table[index].event == sms_info->event)
      {
        ...
        if ( result != DSAT_ASYNC_CMD && result != DSAT_ASYNC_EVENT )
        {
          if (dsat_sms_wm.current_cnt != 0)
          {
            ...
            (void)rex_set_sigs( rcinit_lookup_rextask("ds"), GET_SIG(DS_AT_SMS_SIG) );
#else
            (void)rex_set_sigs( &ds_tcb, GET_SIG(DS_AT_SMS_SIG) );
          ...
          {
#ifdef FEATURE_MODEM_RCINIT_PHASE2
            (void)rex_set_sigs( rcinit_lookup_rextask("ds"), GET_SIG(DS_AT_MT_MSG_SIG) );
#else
            (void)rex_set_sigs( &ds_tcb, GET_SIG(DS_AT_MT_MSG_SIG) );
#endif /* FEATURE_MODEM_RCINIT_PHASE2 */
...
void dsatsms_init_sms_ex ( void )
{
  ...
  mt_msg_queue_init();
#endif /* FEATURE_ETSI_SMS  */
  if(dsatsmsi_cmd_sm == NULL)
    DS_SYSTEM_HEAP_MEM_ALLOC_ASSERT(dsatsmsi_cmd_sm, sizeof(sms_cmd_sm_type), sms_cmd_sm_type *);
  return;
}/* dsatsms_init_sms_ex */
...
#ifdef FEATURE_ETSI_SMS
    DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CPMS_ETSI_IDX,2,0,
...
)
{
  ...
  if (TRUE == dsatme_is_thin_ui())
  {
    ...
    (void) dsatsmsi_route_config ( MT_VAL_1, DS_VAL_0);
  }
  ...
  DSATUTIL_SET_VAL(DSATETSI_EXT_CGSMS_IDX,0,0,
...
/* +CIND URC is appliable only for GW memories */
  ...
  /* Check for clearing SMSFULL +CIND indicator */
    ...
    if( (DSAT_CIND_BINARY_SET == 
        dsatetsime_get_indicator_state( DSAT_CIND_INDEX_SMSFULL)) &&
        (dsatsms_full_mem_store == event_info->u.mem_status.mem_store) )
    {
      ...
      if (0 < event_info->u.mem_status.free_slots)
      {
        ...
        dsatsms_full_mem_store = WMS_MEMORY_STORE_NONE;
      }
    }
...
)
{
  ...
  {
    DS_MSG1(MSG_LEGACY_ERROR,"Unknon Cmd mode",cmd_mode);
    return;
  }
