...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatrsp.c#2 $ $DateTime: 2023/07/14 05:32:54 $ $Author: pwbldsvc $
...
)
{
...
#endif /* FEATURE_DSAT_ETSI_MODE */
  ds3g_siolib_tx_e_type current_tx_type = DS3G_SIOLIB_UNSOLICTED_NONE;
  /*-------------------------------------------------------------------------
  ...
  *in_ptr_ptr = NULL;
  ...
  DS_ASSERT( out_ptr_ptr != NULL && *out_ptr_ptr != NULL );  
  ...
  DS_ASSERT( dest_ptr < dest_end_ptr );
  ...
  for ( write_data = TRUE; write_data; )
  {
    ...
    if ( dest_ptr >= (dest_end_ptr - 6) )
    {
      ...
      DS_AT_MSG1_HIGH("dsat_fmt_response(): ds3g_siolib_tx_type %d ",
                      ds3g_siolib_get_tx_type() );
      /* URC should be stored in URC queue available for each port. */
      if (ds3g_siolib_get_tx_type())
      {
        current_tx_type = ds3g_siolib_get_tx_type();
        dsatcmdp_urc_send_or_queue( *out_ptr_ptr );
        ds3g_siolib_set_tx_type(current_tx_type, DS3G_SIOLIB_DATA_PORT);
      }
      else
      {
    ...
    if ( header )
    {
      ...
      if ( (dsat_num_item_type)dsatutil_get_val(
                                  DSAT_BASIC_V_IDX,0,NUM_TYPE) == 1 )
      {
         temp_s3_val = (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S3_IDX,0,NUM_TYPE);
         temp_s4_val = (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S4_IDX,0,NUM_TYPE);
  ...
  if ( trailer == TRUE )
  {
    ...
    *dest_ptr++ = (byte) temp_s3_val;
    if ( (dsat_num_item_type)dsatutil_get_val(
                   DSAT_BASIC_V_IDX,0,NUM_TYPE)== 1 || !result_code )
    {
      temp_s4_val = (dsat_num_item_type)dsatutil_get_val(DSAT_SREG_S4_IDX,0,NUM_TYPE);
      *dest_ptr++ = (byte) temp_s4_val;
    }
...
)
{
  ...
  */
  if ( (dsat_num_item_type)dsatutil_get_val(
                 DSAT_BASIC_N_E_IDX,0,NUM_TYPE) == AMP_E_RADIO_RATE )
  {
...
(void) dsatutil_memscpy((void*)dsat_report_rate_val.rate.ps_call.apn,
        DS_PDN_APM_MAX_APN_STRING_LEN,(void*)connect_rate.rate.ps_call.apn,
