...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsicmif_ex.c#4 $
...
4/24/17    rk      Fixed issue with UE Automatic PLMN selection mode.
1/19/17    skc     Added enhancement on CHLD and CLCC for VoLTE MPTY Call.
12/07/16   skc     Fixed compilation issue in LTENB flavor.
08/19/16   skc     Fixed USSD issue when both MO and MT USSD present.
08/09/16   skc     Reduced MAX number fo RAT PRI LIST entries for IOE build to 2.
...
} dsatetsicmif_act_pref_xref[] =
{
  ...
  { DSAT_COPS_ACT_EUTRAN_NB_S1,    CM_MODE_PREF_LTE_ONLY },
#ifdef FEATURE_NBIOT_NTN
  { DSAT_COPS_ACT_SATELLITE_EUTRAN_NB_S1,    CM_MODE_PREF_LTE_ONLY },
...
)
{
  ...
  DS_AT_MSG2_HIGH("Received command %d Error status is %d",
                  sups_cmd->cmd, sups_cmd->cmd_err);
  ...
  {
    result = dsatdl_vtable.dsatetsicmif_cm_sups_cmd_handler_fp(sups_cmd, result);
  }
  ...
  if (DSAT_ASYNC_CMD != result)
  {
    ...
    sups_val->dsat_ss_cm_data.curr_class = (uint16)DSAT_SS_CLASS_MAX;
...
)
{
  ...
  switch (sups_event->event)
  {
    ...
    case CM_SUPS_EVENT_REG_PASSWORD_CONF:
      ...
      if (einfo_ptr->ss_success)
      {
        ...
        switch(einfo_ptr->conf_type)
        {
          ...
          case SS_ERROR_INFO:
            (void)etsicmif_report_sups_error (sups_event);
      ...
      else
      {
        ...
        result = DSAT_CMD_ERR_RSP;
        if (DSAT_ERROR ==
            etsicmif_report_sups_error (sups_event))
        {
          result = DSAT_ERROR;
        }
      }
    ...
    case CM_SUPS_EVENT_USS_RES: 
      ...
      dsatetsicmif_set_cusd_state(&(sups_val->dsat_ss_cm_data), DSAT_CUSD_STATE_CLEAR_MT_USSD, 0);
      /* Return to command mode */
      result = DSAT_OK;
      break;
          ...
          case SS_ERROR_INFO:
            ...
            else
            {
                               ...
                               DSATETSI_EXT_ACT_CUSD_ETSI_IDX,0,MIX_NUM_TYPE))
              {
                ...
                result = etsicmif_report_sups_error(sups_event);
              }
            }
            break;
    ...
    case CM_SUPS_EVENT_USS_NOTIFY_IND:
      ...
      dsatetsicmif_set_cusd_state(&(sups_val->dsat_ss_cm_data), DSAT_CUSD_STATE_SS_NOTIFY,
                                                          einfo_ptr->invoke_id);
    ...
    case CM_SUPS_EVENT_USS_IND:
      ...
      dsatetsicmif_set_cusd_state(&(sups_val->dsat_ss_cm_data), DSAT_CUSD_STATE_SS_REQUEST,
                                                    einfo_ptr->invoke_id);
      /* Report USSD result code */
      result = etsicmif_report_ussd_result_ex(sups_event,
                                              DSAT_CUSD_RESULT_MORE);
  ...
  {
    result = dsatdl_vtable.dsatetsicmif_cm_sups_event_handler_fp( sups_event, result);
  }
...
#define SEND_SUPS_ERROR( error_code ) \
        result = etsicmifi_send_sups_error(error_code);
        ...
        case MN_NETWORK_NOT_RESPONDING:
          ...
          switch(event_ptr->event)
          {
            ...
            case CM_SUPS_EVENT_RELEASE_USS_IND:
              ...
              result = etsicmif_report_ussd_result_ex (event_ptr, DSAT_CUSD_RESULT_TIMEOUT);
              break;
...
)
{
  ...
  {
    result = dsatdl_vtable.etsicmif_send_sups_error_fp(error_code);
  }
...
)
{
  ...
  etsicmif_get_cusd_info(&(sups_val->dsat_ss_cm_data), &uss_notify_parms.invoke_id, NULL);
...
)
{
  ...
  if( TRUE == send_ack )
  {
    (void)etsicmif_sups_send_ussd_notify_response (FALSE, 0);
  }
...
#define DSAT_TIMEZONE_NEG  -96
  ...
  if (NULL == mm_info_ptr)
  {
    DS_AT_MSG0_ERROR("NULL mm_info_ptr ");
    return result;
  }

  dsatetsicmif_process_cm_mm_info_net_reg(mm_info_ptr);
  ...
  if( mm_info_ptr->time_zone_avail == TRUE )
  {
    ...
    {
      if(ph_val->last_reported_tz != mm_info_ptr->time_zone)
      {
        ph_val->last_reported_tz = mm_info_ptr->time_zone;
        event_info->last_reported_tz_changed = TRUE;
      }
      if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CTZU_ETSI_IDX,0,NUM_TYPE) == 1 )
      {
  ...
  else if ( mm_info_ptr->univ_time_and_time_zone_avail == TRUE)
  {
    ...
    if(( DSAT_TIMEZONE_NEG <= dsat_time_zone )&& (dsat_time_zone <=DSAT_TIMEZONE_HIGH))
    {
      if(ph_val->last_reported_tz != mm_info_ptr->univ_time_and_time_zone.time_zone )
      {
        ph_val->last_reported_tz = mm_info_ptr->univ_time_and_time_zone.time_zone;
        event_info->last_reported_tz_changed = TRUE;
      }
      if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CTZU_ETSI_IDX,0,NUM_TYPE) == 1 )
      {
...
#define COPS_NULL_PLMN 0xFF
  ...
  {
    ph_val->dsat_net_reg_state.net_id_info.present = TRUE;
    ph_val->dsat_net_reg_state.net_id_info.plmn = mm_info_ptr->plmn;
  }
...
)
{
  ...
  if (NULL == event_ptr)
  {
    DS_AT_MSG0_ERROR("NULL event_ptrstack_id");
    return DSAT_ASYNC_EVENT;
  }

  siglvl = dsatetsime_convert_rssi(
             event_ptr->event_info.rssi,
  ...
  {
    result = dsatetsime_change_indicator_state( DSAT_CIND_INDEX_SIGNAL, 0);
  }
  else
  {
    result = dsatetsime_change_indicator_state( DSAT_CIND_INDEX_SIGNAL, siglvl);
...
)
{
  ...
  DS_ASSERT( NULL != net_act_ptr );
  DS_ASSERT( NULL != cm_rat_ptr );
...
)
{
  ...
  DS_AT_MSG3_MED("ATCOP changing network reg preference: mode=%d act=%d current_mode %d",
            net_mode, net_act, current_mode);
  ...
  if (TRUE !=
      dsatetsicmif_lookup_cm_rat_pref( FALSE, &current_act,
                                       &ph_val->dsat_net_reg_state.cmph_pref.
                                         network_rat_mode_pref ))
  {
    current_act = DSAT_COPS_ACT_MAX; /* never match */
  }
  /* Get current_act for CIOT */
  DSATETSICALL_GET_CIOT_LTE_COPS_PREF(current_act, DSAT_COPS_ACT_MAX)
  ...
  ph_val->dsatetsicall_network_list.requested_pref.mode = net_mode;
  if ( NULL != plmn_ptr )
  {
    ph_val->dsatetsicall_network_list.requested_pref.plmn = *plmn_ptr;
  }
  else
  {
    memset( ph_val->dsatetsicall_network_list.requested_pref.plmn.identity, 0, 
            sizeof(ph_val->dsatetsicall_network_list.requested_pref.plmn.identity) );
  }
  ph_val->dsatetsicall_network_list.requested_pref.act  = net_act;
#ifdef FEATURE_DSAT_HIGH_TIER
  ...
  case DSAT_COPS_MODE_AUTO:
    ...
    user_pref.network_sel_mode_pref = CM_NETWORK_SEL_MODE_PREF_AUTOMATIC;
        ...
        user_pref.mode_pref = act_mode_pref;
  ...
  case  DSAT_COPS_MODE_MANAUTO:
    ...
    */    
    ...
    if( cops_no_mode_change == TRUE )
    {
      if( ph_val->net_pref.no_of_args == 3)
      {
        act_mode_pref = CM_MODE_PREF_NO_CHANGE;
        ph_val->net_pref.no_of_args = 0;
      }
    }
    ...
    if(act_mode_pref == CM_MODE_PREF_LTE_ONLY)
    {
      if(net_act == DSAT_COPS_ACT_EUTRAN)
      {
        user_pref.ciot_lte_pref.lte_op_mode_pref = CM_LTE_IOT_OP_MODE_PREF_M1;
      }
      else if(net_act == DSAT_COPS_ACT_EUTRAN_NB_S1)
      {
        user_pref.ciot_lte_pref.lte_op_mode_pref = CM_LTE_IOT_OP_MODE_PREF_NB1;
      }
#ifdef FEATURE_NBIOT_NTN
      else if(net_act == DSAT_COPS_ACT_SATELLITE_EUTRAN_NB_S1)
      {
        user_pref.ciot_lte_pref.lte_op_mode_pref = CM_LTE_IOT_OP_MODE_PREF_NB1_NTN;
      }
#endif /* FEATURE_NBIOT_NTN */
      else
      {
        user_pref.ciot_lte_pref.lte_op_mode_pref = CM_LTE_IOT_OP_MODE_PREF_WB; 
      } 
...
#ifdef FEATURE_NBIOT_NTN
      user_pref.ciot_lte_pref.lte_nb1_ntn_band_pref = SYS_LTE_BAND_MASK_CONST_NO_CHG;
#endif /* FEATURE_NBIOT_NTN */
    }
...
#ifdef FEATURE_DSAT_LTE
         ||((net_act == DSAT_COPS_ACT_EUTRAN || net_act == DSAT_COPS_ACT_EUTRAN_NB_S1 
#ifdef FEATURE_NBIOT_NTN
         || net_act == DSAT_COPS_ACT_SATELLITE_EUTRAN_NB_S1
#endif /* FEATURE_NBIOT_NTN */
            ) && 
...
)
{
      ...
      (CM_CALL_TYPE_VOICE == event_ptr->call_type) )
  {
    ...
    call_val->dsatetsicmif_call_end_status.domain = DSAT_CES_DOMAIN_CS;
    call_val->dsatetsicmif_call_end_status.error_info.cs_domain.end_status =
      event_ptr->end_status;
    call_val->dsatetsicmif_call_end_status.error_info.cs_domain.cc_cause =
      event_ptr->dsat_mode_info.info.gw_cs_call.cc_cause;
    call_val->dsatetsicmif_call_end_status.error_info.cs_domain.cc_reject =
      event_ptr->dsat_mode_info.info.gw_cs_call.cc_reject;
  }
  else if( CM_CALL_TYPE_PS_DATA == event_ptr->call_type )
  {
    /* Cache the status codes */
    call_val->dsatetsicmif_call_end_status.domain = DSAT_CES_DOMAIN_PS;
    call_val->dsatetsicmif_call_end_status.error_info.ps_domain.sys_mode = 
      event_ptr->sys_mode;
    if(event_ptr->sys_mode == SYS_SYS_MODE_LTE)
    {
      call_val->dsatetsicmif_call_end_status.error_info.ps_domain.esm_cause = 
         event_ptr->dsat_mode_info.info.lte_call.esm_cause;
      call_val->dsatetsicmif_call_end_status.error_info.ps_domain.esm_local_cause= 
         event_ptr->dsat_mode_info.info.lte_call.esm_local_cause;
    }
    else
    {
     call_val->dsatetsicmif_call_end_status.error_info.ps_domain.pdp_cause_type =
      event_ptr->dsat_mode_info.info.gw_ps_call.pdp_cause_type;
     call_val->dsatetsicmif_call_end_status.error_info.ps_domain.cause =
