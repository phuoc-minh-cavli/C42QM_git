...
EXTERNALIZED FUNCTIONS INTERNAL TO DSAT UNIT    
  ...
  process_fsk_data_entry_mode
    This function performs the FSK data entry data preprocessing
    necessary to accept FSK data for (QCFSK command).
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatprep.c#2 $ $DateTime: 2021/02/01 05:46:44 $ $Author: pwbldsvc $

when       who     what, where, why
-------    ---     ---------------------------------------------------------
01/25/21   ks      Added support for FSK related AT commands.
04/17/17   ss      Buffer used to store SMS in SMS entry mode initialized to 0
...
#if defined(FEATURE_ETSI_SMS) || defined(FEATURE_DSAT_MMGSDI_AUTH) || defined(FEATURE_DSAT_ATCOP_VEND)
static void send_string
(
  const char *
);
#endif /* defined(FEATURE_ETSI_SMS) || defined(FEATURE_DSAT_MMGSDI_AUTH) || defined(FEATURE_DSAT_ATCOP_VEND) */
...
static boolean process_fsk_data_entry_mode
(
  dsat_mode_enum_type,
  dsm_watermark_type *
);
...
#ifdef FEATURE_DSAT_ATCOP_VEND
#ifdef FEATURE_FSK_TX
            || ( preprocessor_mode == DSATI_FSK_DATA_ENTRY_MODE )
#endif /* FEATURE_FSK_TX */     
#endif
            ))
      {
...
#ifdef FEATURE_DSAT_ATCOP_VEND
#ifdef FEATURE_FSK_TX
  case DSATI_FSK_DATA_ENTRY_MODE:
    /* Enter FSK Data entry mode. */
    preprocessor_mode = mode;    

    /* Send FSK Data entry prompt to TE. */
    send_string( prompt );
    ...
    /* Assert the DCD line for SMS message entry. */
    ds3g_siolib_set_client_type ( DS3G_SIOLIB_CLIENT_ATCOP );
    (void) ds3g_siolib_set_cd_state( DS3G_SIOLIB_FSK_DATA_START );
    break;

  case DSATI_FSK_DATA_END_MODE:
    /* We have either received all the bytestosend characters for FSK data or */
    /* escape character. Reset the AT command mode to AT state but do */
    /* not enable the autodetect yet. Eventual result code generation */
    /* will enable the autodetect mode                                */
    preprocessor_mode = DSATI_AT_CMD_MODE;
...
#ifdef FEATURE_FSK_TX
    case DSATI_FSK_DATA_ENTRY_MODE:
      abort_call = process_fsk_data_entry_mode( at_state, wm_item_ptr );
      break;
#endif /* FEATURE_FSK_TX */
#endif
      ...
      case DSAT_CMD_PREP_STATE_FOUND_AT:            
                                       ...
                                       DSAT_SREG_S3_IDX,0,NUM_TYPE) )   
        {
                                       ...
                                       DSAT_SREG_S5_IDX,0,NUM_TYPE) )  
          {
                                   ...
                                   + MAX_LINE_SIZE - 2) )
            {
              ...
              *sio_info_ptr->cmd_line_buffer = '\0';   
            }
            /*------------------------------------------------------------
        ...
        else                       
        {
          ...
          *sio_info_ptr->build_cmd_ptr = '\0';
          ...
          ---------------------------------------------------------------*/
          sio_info_ptr->build_cmd_ptr = sio_info_ptr->cmd_line_buffer;
          sio_info_ptr->prev_cmd_ptr = sio_info_ptr->prev_line_buffer;
...
)
{
  ...
  if ( msg_item_ptr == NULL )
  {
    ...
    memset(msg_item_ptr->data_ptr,0,msg_item_ptr->size);
            ...
            msg_item_ptr )
    {
      ...
      case NORMAL_STATE:
        ...
        if ( ((unicode_mode && (ccw == '\b')) || (!unicode_mode && (cc == '\b')) )
             ...
             0,NUM_TYPE) == 1 && op_mode == ETSI_CMD_MODE) ||
...
#ifdef FEATURE_ETSI_SMS 
               && ( ((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,
               0,NUM_TYPE) == 1 && op_mode == ETSI_CMD_MODE) || op_mode == CDMA_CMD_MODE )
#endif /* FEATURE_ETSI_SMS  */
          )
        {
          /*--------------------------------------------------------------
...
FUNCTION PROCESS_FSK_DATA_ENTRY_MODE

DESCRIPTION

  This function performs the FSK Data entry data preprocessing
  necessary to accept a FSK Data message.  It performs the following
  functions on FSK data entered:
  
   - Echo characters entered based on E command setting
   - Perform line editing functions based on command line editing
   - Detect <ESC> character and cancel message sending or writing
     command when received
   - Complete processing of FSK Data command when <bytestosend> data is
     received
   - Ignore non-hex characters and don't echo them.
   - Set the SIO data preprocessor mode to DSATI_AT_CMD_MODE once <bytestosend>
     data is received or <ESC> is received and return DCD to its previous state.
...
#define IS_HEX(c) \
  ...
  dsm_item_type *echo_item_ptr = NULL;        /* echo item                 */
  byte *item_data_ptr;                        /* item from queue data ptr  */  
  static dsm_item_type *msg_item_ptr = NULL;  /* ptr to FSK data item   */
  static byte *msg_ptr;                       /* pointer to FSK data     */
  static byte *start_line_ptr;                /* start of line pointer     */
  byte cc;                                    /* current character         */
  byte cce;                                   /* current echo character    */
  enum                                        /* FSK Data entry states  */
  {
    NORMAL_STATE,
    ERROR_STATE
  } fsk_data_entry_state;
  ...
  /*----------------------------------------------------------------------
    Get a new buffer to hold the FSK Data if one is not already
    allocated.
  ----------------------------------------------------------------------*/
  if ( msg_item_ptr == NULL )
  {
  ...
  /*---------------------------------------------------------------------
    Dequeue each item from watermark, extract FSK Data, and pass
    to FSK Data handler.
  ---------------------------------------------------------------------*/

  fsk_data_entry_state = NORMAL_STATE;
  while ( atprep_dsm_item_ptr != NULL ||
          (atprep_dsm_item_ptr = dsm_dequeue( wm_item_ptr )) != NULL )
  {
            ...
            msg_item_ptr )
    {
      ...
      /*------------------------------------------------------------------
        Process data received for FSK Data.
      ------------------------------------------------------------------*/
      switch ( fsk_data_entry_state )
      {
      case NORMAL_STATE:
        /*----------------------------------------------------------------
          Normal processing for FSK Data.
        ----------------------------------------------------------------*/
        if ( cc == '\b' )
        {
          /*--------------------------------------------------------------
        ...
        {
          /*--------------------------------------------------------------
            ESC character - Cancel FSK Data send or write command
          --------------------------------------------------------------*/
            
          /* Notify FSK handler command has been cancelled. */
          dsatvend_fsk_data_abort_cmd( TRUE );
          
          /* Kill echo and send EOM string. */
          store_char = FALSE;
          send_string( eom );

          /* Return DCD to its state before FSK Data entry and set
             SIO data preprocessor back to AT command mode. */      
          ds3g_siolib_set_client_type ( DS3G_SIOLIB_CLIENT_ATCOP );

          (void) ds3g_siolib_set_cd_state( DS3G_SIOLIB_FSK_DATA_END );

          dsatprepi_set_preprocessor_mode( DSATI_FSK_DATA_END_MODE, port );

          /* Free the DSM item holding FSK Data. */
          (void) dsm_free_buffer( msg_item_ptr );
          msg_item_ptr = NULL;
        }
        else
        {
          /*--------------------------------------------------------------
            Write HEX character to message buffer
          --------------------------------------------------------------*/  
          if ( IS_HEX(cc))
          {
            if ( msg_item_ptr->used < msg_item_ptr->size)
            {
              ...
              *msg_ptr++ = cc;
              msg_item_ptr->used++;
              if(msg_item_ptr->used == bytestosend * 2)
              {
                /*--------------------------------------------------------------
                 End of message body
                --------------------------------------------------------------*/  
                ...
                /* NULL Terminate the string */
                msg_item_ptr->data_ptr[msg_item_ptr->used]='\0';
                /* Send FSK Data to FSK command handler. */
                dsatvend_send_fsk_data( msg_item_ptr->data_ptr, msg_item_ptr->used );
            ...
            {
              /* Buffer overflow, FSK Data is too large. */
              fsk_data_entry_state = ERROR_STATE;
              
              /* Notify FSK handler command error has occurred and command
                 should be aborted. */
              ...
              dsatprepi_set_preprocessor_mode( DSATI_AT_CMD_MODE, port );
              DS_AT_MSG0_ERROR("FSK Data is too large!");
            }
          }
          else
          {
      ...
      case ERROR_STATE:
        /*----------------------------------------------------------------
          Error processing of data for FSK Data message. In this state just
          ignore the remaining data left in the watermark to empty the
          watermark.
        ----------------------------------------------------------------*/
