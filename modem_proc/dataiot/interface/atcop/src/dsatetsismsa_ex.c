...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsismsa_ex.c#3 $ $DateTime: 2020/11/27 22:36:28 $ $Author: pwbldsvc $

when       who     what, where, why

--------   ---     ----------------------------------------------------------
11/09/20   ks      Added support for preserving URC indices across PSM cycles.
09/07/20   ks      Fixed an issue with +CMT URC not being given because ATCOP
                   was not freeing up RAM memory.
04/28/17   rg      Fixed to delete SM from GW RAM for no message class SM only
...
)
{
  ...
  cnmi_bfr_val = (dsat_num_item_type)dsatutil_get_val(
                    DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_BFR_VAL,NUM_TYPE);
    ...
    else
    {
      ...
      if ( msg_sent == TRUE )
      {
        ...
        if ( (dsat_num_item_type)dsatutil_get_val(
                 DSATETSI_EXT_CSMS_IDX,0,NUM_TYPE) == 1)
        {
...
)
{
  ...
  cnmi_bm_val = (dsat_num_item_type)dsatutil_get_val(
                    DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_BM_VAL,NUM_TYPE);
  cnmi_mode_val = (dsat_num_item_type)dsatutil_get_val(
                    DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_MODE_VAL,NUM_TYPE);
...
)
{
      ...
      ( dsatsmsi_pres_cmd.cmd_name == SMS_CNMA ) )
  {
    ...
    {
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CNMI_ETSI_IDX,index,0,0,NUM_TYPE)
    ...
    (void) dsatsmsi_route_config ( MT_VAL_0, DS_VAL_0);
...
)
{
  ...
  if ( cnmi_ds_val == 2 )
  {
    ...
    result =
      handle_mt_notification ( MT_STATUS_REPORT, return_val, mem_store);
    return result;
  }
