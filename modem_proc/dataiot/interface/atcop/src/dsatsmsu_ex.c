...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatsmsu_ex.c#1 $
...
)
{
  ...
  {
    dsatdl_vtable.dsatsmsu_send_async_indication_to_te_fp(msg_type, msg_index, mem_store, cmd_mode);
  }
...
)
{
  ...
  route.pp_routes[0].mem_store = 
    dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
                DSATETSI_EXT_ACT_CPMS_ETSI_IDX,2,NUM_TYPE), CPMS_MEM3,ETSI_CMD_MODE);
  ...
  if ( mt_val == 2 )
  {
    if ((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CSMS_IDX,0,NUM_TYPE) == 0)
    {
      route.pp_routes[0].route = WMS_ROUTE_TRANSFER_AND_ACK;
