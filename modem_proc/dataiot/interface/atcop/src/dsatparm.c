...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatparm.c#2 $ $DateTime: 2020/08/05 02:45:39 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/  
  ...
  { 
    ...
    if (ds3g_siolib_is_baud_valid ((sio_bitrate_type)temp_val) == FALSE)
    {
      ...
      result = DSAT_ERROR;
      DSATUTIL_SET_VAL(DSAT_EXT_IPR_IDX,0,0,old_ipr_val,NUM_TYPE)
    }
    else
    {
      /*-------------------------------------------------------------------------
...
)
{
  ...
  for (index = 0; index < tok_ptr->args_found; index++)
  {
    ...
    byte token_arg_length = tok_ptr->arg_length[index];
        ...
        (CONFIG | STRING) )
    {
      ...
      if (0 != *use_arg)
      {
        stripped_arg = dsat_alloc_memory(token_arg_length + 1, FALSE);
        if ( !stripped_arg )
        {
          return FALSE;
        }
        ...
        {
          if(!dsatutil_strip_quotes_out(token_arg_ptr, stripped_arg, token_arg_length))
          {
...
)
{
  ...
  for (i = 0; i < sizeof_table; ++i)
  {
    ...
    if ( (void*)dsatutil_get_val_ext(table[i].cmd_id,0,STR_TYPE) != NULL && (table + i)->name[0] != '$')
    {
