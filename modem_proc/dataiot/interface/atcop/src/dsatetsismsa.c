...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   ...
   Copyright (c) 2004 - 2020 by Qualcomm Technologies, Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsismsa.c#2 $ $DateTime: 2020/09/08 01:01:07 $ $Author: pwbldsvc $

when       who     what, where, why

--------   ---     ----------------------------------------------------------
08/24/20   ks      Fixed an issue with +CMT URC not being given because ATCOP 
                   was not freeing up RAM memory.
09/07/17   rg      Fixed to delete SM from GW RAM for no message class SM only
                   when CSMS is not set to 1 and <mt> in CNMI is not set
                   to 2
...
)
{
  ...
  if (NULL != res_buff)
  {
    ...
    if ( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE) == 0 )
    {
      /* We are in PDU mode */
...
)
{
       ...
       dsatsmsi_pres_cmd.cmd_name == SMS_CNMA )
  {
    ...
    modify_tag ( &event_info->u.msg_status );
  }
  else if( dsatsmsi_pres_cmd.cmd_name == SMS_CMSS )
  {
  ...
  else
  {
    if ( dsatsmsi_pres_cmd.cmd_name == SMS_CNMA )
    {
      ...
      else if (temp_ts_data->u.gw_pp.u.deliver.dcs.msg_class ==
                                               WMS_MESSAGE_CLASS_NONE &&
               temp_ts_data->u.gw_pp.u.deliver.dcs.msg_waiting ==
                                               WMS_GW_MSG_WAITING_NONE)
      {
        /* Free message if message class is none and message waiting is none */

        /* If +CNMI MT is set to 2 and +CSMS is set to 1 then the message needs to
         * ack through +CNMA and in all other cases this message does not need ack
         * so free the element from the queue */

        if (((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CNMI_ETSI_IDX,
                                      CNMI_MT_VAL,NUM_TYPE) != 2) ||
            ((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CSMS_IDX,
                                      0,NUM_TYPE) != 1))
        {
           boolean ack_required = FALSE;
           dsatetsismsi_free_element( ack_required);
  ...
  else 
  {
    ...
    if ( dsatsmsi_cmd_sm->num_of_indices != 0 )
    {
      mem_store = dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
       DSATETSI_EXT_ACT_CPMS_ETSI_IDX,0,NUM_TYPE), CPMS_MEM1,ETSI_CMD_MODE);
...
)
{
  ...
  else
  {
    ...
    {   
      ...
      DSATUTIL_SET_VAL(DSATETSI_EXT_CSMP_IDX,2,0,read_templ_ptr->pid,MIX_NUM_TYPE)
    }
    ...
    {   
     ...
     (void) wms_ts_encode_dcs( &read_templ_ptr->dcs, &dcs );
      DSATUTIL_SET_VAL(DSATETSI_EXT_CSMP_IDX,3,0,dcs,MIX_NUM_TYPE)
    } 
  ...
  if ( event_info->u.submit_report.report_status == WMS_RPT_OK )
  {
         ...
         ( event_info->u.submit_report.tpdu_type != WMS_TPDU_NONE ) )
    {
      ...
      *data_ptr++ = ',';
      ...
      if ( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE) == 1 )
      {
...
)
{
  ...
  if ( dsatsmsi_pres_cmd.cmd_name == SMS_CMGL)
  {
    ...
    {
      /* We are not done with reading the list */
      if ( TRUE == dsatsmsi_read_msg_list() )
      {
        result = DSAT_ASYNC_CMD;
      }
      else
