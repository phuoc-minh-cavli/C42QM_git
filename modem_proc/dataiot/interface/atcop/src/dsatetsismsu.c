...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   ...
   Copyright (c) 2002 - 2012,2014-2015,2017-2018 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsismsu.c#1 $
...
DESCRIPTION
  This function sets the file name based on the apps_enabled.
...
)
{
  ...
  #define UPDATE_USED(res_buff_ptr,buff_len)\
  			update_used(res_buff_ptr, &buff_len, w_ptr)
  ...
  switch ( tpdu_msg_ptr->u.gw_pp.tpdu_type )
  {
    case WMS_TPDU_DELIVER:
    {
      ...
      *w_ptr++ = ',';
      ...
      if((dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CSDH_IDX,0,NUM_TYPE))
      {
      ...
      if(is_alpha_present)
      {
        *w_ptr++ = '"';
        ...
        (void) dsatutil_memscpy((void*)w_ptr,
                buff_len,(void*)&me_val->dsat_alpha_text[0],
                MIN(DSAT_MAX_DISPLAY_ALPHA_LEN,strlen(me_val->dsat_alpha_text)));
        is_alpha_present = FALSE;
        w_ptr += strlen(me_val->dsat_alpha_text);
        *w_ptr++ = '"';
      }
#endif /* FEATURE_ETSI_PBM */	  
       ...
       dsatsmsi_pres_cmd.cmd_name == SMS_CNMA )
  {
    ...
    if (msg_ptr->u.gw_message.raw_ts_data.tpdu_type != WMS_TPDU_STATUS_REPORT)
    {
      ...
      */
      if(is_alpha_present)
      {
        uint8 len = strlen(me_val->dsat_alpha_text);
        for( i=0; i<len; i++)
        {
          if( me_val->dsat_alpha_text[i] < 16 )
          {
            *w_ptr++ = '0';
          }
          w_ptr = dsatutil_itoa((uint32) me_val->dsat_alpha_text[i], w_ptr, 16 );
        }
        is_alpha_present = FALSE;
      }
#endif /* FEATURE_ETSI_PBM */	  
...
)
{
  ...
  if (msg_ptr->tpdu_type == WMS_TPDU_SUBMIT)
  {
    ...
    temp_vp = (byte *)dsatutil_get_val(
                            DSATETSI_EXT_CSMP_IDX,1,MIX_STR_TYPE);
...
)
{
  ...
  temp_fo_val = (byte *)dsatutil_get_val(DSATETSI_EXT_CSMP_IDX,0,MIX_STR_TYPE);
  ...
  if ( (fo & 0x03) == 1 )
  {
    ...
    temp_vp_val = (byte *)dsatutil_get_val(DSATETSI_EXT_CSMP_IDX,1,MIX_STR_TYPE);
    /* if <vpf> of NONE */
    if ( (fo & 0x18) == 0 )
    {
...
)
{
  ...
  str_item1 = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,0,MIX_STR_TYPE);
  str_item2 = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,1,MIX_STR_TYPE);
  if ( *str_item1 != '\0' )
  {
    if ( *str_item2 != '\0' )
    {
...
#define UCS2 2
  ...
  DS_ASSERT( (res_ptr != NULL) && (type_of_addr != NULL));
  ...
  *res_ptr++ = '"';
  ...
  if ( (*type_of_addr == 145)  && ( ((dsat_num_item_type)dsatutil_get_val(
                        DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE) ) || ( is_csca ) ) )
  {
    *addr_ira_ptr++ = '+';
    is_plus_presnt = TRUE;
  }
  ...
  {
    DS_AT_MSG2_MED("Found name = %s for number =%s in recevied SMS",me_val->dsat_alpha_text     
                                                            ,num_buf);
  ...
  else if( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE))
  {
