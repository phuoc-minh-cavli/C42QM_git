...
INITIALIZATION AND SEQUENCING REQUIREMENTS
   ...
   Copyright (c) 2001 - 2014,2018 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsime_ex.c#1 $ $DateTime: 2020/03/17 08:03:16 $ $Author: pwbldsvc $
...
)
{
  ...
  if (MAX_CIND_INDICATORS > indicator)
  {
    ...
    current_cind_val = (dsat_num_item_type)dsatutil_get_val(
                   DSATETSI_EXT_ACT_CIND_ETSI_IDX,indicator,NUM_TYPE);
    if ( current_cind_val != value )
    {
      ...
      /* Set indicator state */
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CIND_ETSI_IDX,indicator,0,value,NUM_TYPE)

      if (DSAT_DL_VALIDATE_SYMBOL_ADDR(dsatdl_vtable.dsatetsime_report_ciev_result_fp))
      {
        dsatdl_vtable.dsatetsime_report_ciev_result_fp(indicator, value);
      }
    }
  }
  else
...
)
{
  ...
  ind_state = (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CIND_ETSI_IDX,indicator,NUM_TYPE);
...
)
{
  ...
  switch (mmgsdi_event)
  {
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      ...
      if (me_val->dsat_qcsimstat_curr != DSAT_QCSIMSTAT_SIM_INIT_COMPLETED)
      {
        me_val->dsat_qcsimstat_curr = DSAT_QCSIMSTAT_SIM_INIT_COMPLETED;
...
)
{
  ...
  case MMGSDI_PIN1_EVT:
    switch(pin_status)
    {
      case MMGSDI_PIN_ENABLED_NOT_VERIFIED:
        me_val->dsat_pin_error = DSAT_CME_NO_ERROR;
      ...
      case MMGSDI_PIN_BLOCKED:
        me_val->dsat_pin_error = DSAT_CME_NO_ERROR;
        me_val->dsat_pin_required |= (uint32)DSAT_CPIN_SIM_PIN_REQUIRED;
        me_val->dsat_pin_blocked |= (uint32)DSAT_CPIN_SIM_PIN_REQUIRED;
        dsatcmdp_set_restricted_mode( TRUE, apps_id );
  ...
  case MMGSDI_SUBSCRIPTION_READY_EVT:   /* Compatibility with UI */
    ...
    me_val->dsat_sim_init_comp = TRUE;
    me_val->dsat_pin_error = DSAT_CME_NO_ERROR;
  ...
  case MMGSDI_PIN2_EVT:
    switch(pin_status)
    {
      case MMGSDI_PIN_ENABLED_NOT_VERIFIED:
        me_val->dsat_pin_error = DSAT_CME_NO_ERROR;
        me_val->dsat_pin_required |= (uint32)DSAT_CPIN_SIM_PIN2_REQUIRED;
      ...
      case MMGSDI_PIN_BLOCKED:
        me_val->dsat_pin_error = DSAT_CME_NO_ERROR;
        me_val->dsat_pin_required |= (uint32)DSAT_CPIN_SIM_PIN2_REQUIRED;
        me_val->dsat_pin_blocked |= (uint32)DSAT_CPIN_SIM_PIN2_REQUIRED;
...
(
  ...
  mmgsdi_file_enum_type  file_gsm,
  mmgsdi_file_enum_type  file_umts,
...
#define SET_FILENAME(app_type, file_gsm, file_umts)\
			set_filename(app_type, file_gsm, file_umts, filename)

  DS_ASSERT(filename != NULL ); 
...
)
{
  ...
  restrict_pin = ( me_val->dsat_pin_required & 
...
void dsatetsime_init_auto_nitz_setting_from_nv(void)
{
                                         ...
                                         &nv_item) == NV_DONE_S)
  {
    ...
    {
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CTZU_ETSI_IDX,0,0,0,NUM_TYPE)
    }
    else
    {
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CTZU_ETSI_IDX,0,0,1,NUM_TYPE)
    }
  }
} /* dsatetsime_init_auto_nitz_setting_from_nv */
