...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatetsismsc.c#2 $ $DateTime: 2020/11/27 22:36:28 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/09/20   ks      Added support for preserving URC indices across PSM cycles.
08/19/16   skc     Fixed CPMS display issue.
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
    ...
    DSATUTIL_SET_VAL(DSATETSI_EXT_CGSMS_IDX,0,0,srv_num,NUM_TYPE)
  ...
  else if ( tok_ptr->op == (NA|QU) )
  {
    cgsms_val =(dsat_num_item_type)dsatutil_get_val(
                  DSATETSI_EXT_CGSMS_IDX,0,NUM_TYPE);
  ...
  if ( tok_ptr->op == (NA|EQ|AR) || (tok_ptr->op == (NA|QU)) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      mem3_type = (dsat_num_item_type)dsatutil_get_val(
          DSATETSI_EXT_ACT_CPMS_ETSI_IDX,CPMS_MEM3,NUM_TYPE);
      result = dsatparm_exec_param_cmd ( 
                                        mode,
                                        parse_table,
                                        tok_ptr,
    ...
    if ( result == DSAT_OK ) /* Set command passed */
    {
      ...
      mem_store = dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
          DSATETSI_EXT_ACT_CPMS_ETSI_IDX,dsatsmsi_mem_report.mem,NUM_TYPE), 
      ...
      if( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_CPMS_ETSI_IDX,
                      dsatsmsi_mem_report.mem,NUM_TYPE) == 3 )
	  {
...
#define INATL_IND 0x10
  ...
  if (tok_ptr->op == (NA|EQ|AR))
  {
    ...
    else
    {
      ...
      str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,0,MIX_STR_TYPE);
      (void) strlcpy( (char *)dsat_csca_prev_val[0].string_item,
                        (const char *)str_item,
                        CSCA_SCA_STR_MAX_LEN+1);
      str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,1,MIX_STR_TYPE);
      ...
      if ( *s == '+' )
      {
        (void)snprintf((char*)dsat_csca_sca_val,
      ...
      (void)snprintf ( (char*)dsat_csca_tosca_val,
      ...
      temp_val = (dsat_num_item_type)dsatutil_get_val(
                                            DSATETSI_EXT_CSMP_IDX,2,MIX_NUM_TYPE);
        
      template_ptr->u.gw_template.pid = (wms_pid_e_type)temp_val;

      temp_val = (dsat_num_item_type)dsatutil_get_val(
                                            DSATETSI_EXT_CSMP_IDX,3,MIX_NUM_TYPE);
  ...
  if ( (tok_ptr->op == (NA|EQ|AR)) || (tok_ptr->op == (NA|QU)))
  {
    if (tok_ptr->op == (NA|EQ|AR))
    {
      ...
      temp_str_val = (byte *)dsatutil_get_val(
                            DSATETSI_EXT_CSMP_IDX,0,MIX_STR_TYPE);
...
)
{
  ...
  else
  {
    mode_val = (dsat_num_item_type)dsatutil_get_val(
                      DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_MODE_VAL,NUM_TYPE);
  }
  ...
  else
  {
    if ( ( tok_ptr->op == (NA|EQ|AR) ) || ( tok_ptr->op == (NA) ) )
    {
      ...
      {
        dsat_psm_urc_update_bit_mask(DSAT_PSM_URC_CNMI_MODE+index, 
                                  DSATETSI_EXT_ACT_CNMI_ETSI_IDX, 
                                  index);
      }
      dsat_num_item_type mt_val = (dsat_num_item_type)dsatutil_get_val(
          DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_MT_VAL,NUM_TYPE);
      dsat_num_item_type ds_val = (dsat_num_item_type)dsatutil_get_val(
          DSATETSI_EXT_ACT_CNMI_ETSI_IDX,CNMI_DS_VAL,NUM_TYPE);
      ...
      result = dsatsmsi_route_config ( mt_val, ds_val ); 
...
)
{
  ...
  mem_store = dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
                   DSATETSI_EXT_ACT_CPMS_ETSI_IDX,0,NUM_TYPE),
  ...
  if ( ( tok_ptr->op == (NA|EQ|AR) ) || ( tok_ptr->op == (NA) ) )
  {
    ...
    if ( tok_ptr->op == (NA|EQ|AR) )
    {
      ...
      if( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE) )
      {
    ...
    if( tok_ptr->args_found == 1 )
    {
      ...
      sms_msg_ptr->msg_hdr.mem_store = 
        dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
                   DSATETSI_EXT_ACT_CPMS_ETSI_IDX,1,NUM_TYPE), 
...
)
{
  ...
  if ( tok_ptr->op == (NA|EQ|AR) )
  {
                                    ...
                                    0,NUM_TYPE) != 1 )
    {
      ...
      dsatetsismsi_free_element ( ack_required );
      res_buff_ptr->used = 
       ...
       tok_ptr->op == (NA) )
  {
    ...
    else
    {
      ...
      DSATUTIL_SET_VAL(DSATETSI_EXT_ACT_CMMS_ETSI_IDX,
                        0,0,dsat_cmms_dflm.default_v,NUM_TYPE)
    }
    ...
    if ( result == DSAT_OK )  
    {
      ...
      temp_cmms = (dsat_num_item_type)dsatutil_get_val(
                   DSATETSI_EXT_ACT_CMMS_ETSI_IDX,0,NUM_TYPE);
...
-------------------------------------------------------------*/
  ...
  str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,
  ...
  str_item = (byte *)dsatutil_get_val(
                            DSATETSI_EXT_CSMP_IDX,CSMP_FO,MIX_STR_TYPE);
  ...
  (void) dsatutil_memscpy((void*)ds_nv_item.ds_at_csas_csmp_settings.fo,
    ...
    str_item = (byte *)dsatutil_get_val(
                            DSATETSI_EXT_CSMP_IDX,CSMP_VP,MIX_STR_TYPE);
  ...
  num_item = (dsat_num_item_type)dsatutil_get_val(
                                            DSATETSI_EXT_CSMP_IDX,CSMP_PID,MIX_NUM_TYPE);
  
  ds_nv_item.ds_at_csas_csmp_settings.pid = (uint8)num_item;

  num_item = (dsat_num_item_type)dsatutil_get_val(
                                            DSATETSI_EXT_CSMP_IDX,CSMP_DCS,MIX_NUM_TYPE);
...
#ifdef FEATURE_ETSI_SMS_CB
#ifdef FEATURE_ATCOP_TEMP_SMS_ALT
...
#endif /* FEATURE_ATCOP_TEMP_SMS_ALT */
#endif /* FEATURE_ETSI_SMS_CB */
...
-------------------------------------------------------------*/
  ...
  -------------------------------------------------------------*/
  memset( (void*)&ds_nv_item, 0, sizeof(ds_nv_item));
  if ( dsatutil_get_nv_item( NV_DS_AT_CSAS_CSCA_SETTINGS_I, &ds_nv_item ) != NV_DONE_S )
  {
    return DSAT_ERROR;
  }
  str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,CSCA_SCA,MIX_STR_TYPE);
  ...
  str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCA_IDX,CSCA_TOSCA,MIX_STR_TYPE);
  ...
  -------------------------------------------------------------*/
  memset ( (void*)&ds_nv_item, 0, sizeof(ds_nv_item));
  if ( dsatutil_get_nv_item( NV_DS_AT_CSAS_CSMP_SETTINGS_I, &ds_nv_item) != NV_DONE_S )
  {
    return DSAT_ERROR;
  }
  str_item = (byte *)dsatutil_get_val(
  ...
  DSATUTIL_SET_VAL(DSATETSI_EXT_CSMP_IDX,CSMP_PID,0,ds_nv_item.ds_at_csas_csmp_settings.pid,MIX_NUM_TYPE)
  DSATUTIL_SET_VAL(DSATETSI_EXT_CSMP_IDX,CSMP_DCS,0,ds_nv_item.ds_at_csas_csmp_settings.dcs,MIX_NUM_TYPE)
...
)
{
  ...
  if ( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_CMGF_IDX,0,NUM_TYPE) ) /*we are in text mode, so send text mesage*/
  {
...
)
{
  ...
  else /* SMS_CMGW */
  {
    sms_msg_ptr->msg_hdr.mem_store = 
      dsatsmsi_memory_lookup((dsat_num_item_type)dsatutil_get_val(
        DSATETSI_EXT_ACT_CPMS_ETSI_IDX,1,NUM_TYPE), CPMS_MEM2,ETSI_CMD_MODE);
  }
...
)
{
  ...
  else if ( dsatsmsi_pres_cmd.cmd_name == SMS_CMGC )
  {
    if ( (len < 2) || (len%2) )
    {
      ...
      DS_AT_MSG2_ERROR("dsatetsisms_process_template_read %d read mask, %d write mask",len,(len%2));
       err_info = DSAT_CMS_INVALID_TXT_PARAM;
       goto send_result;
    }
...
)
{
  ...
  switch( mode )
  {
    ...
    case DSAT_CSCB_MODE_DEL_ALL:
    {
      ...
      str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCB_IDX,2,MIX_STR_TYPE);
      str_item[0] = '\0';
      break;
    }
...
)
{
  ...
  str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCB_IDX,CSCB_MIDS,MIX_STR_TYPE);
  memset ( (void *)str_item, 0x0, CSCB_MIDS_STR_MAX_LEN);

  str_item = (uint8 *)dsatutil_get_val(DSATETSI_EXT_CSCB_IDX,CSCB_DCSS,MIX_STR_TYPE);
  memset ( (void *) str_item, 0x0, CSCB_DCSS_STR_MAX_LEN);
  
  if ( dsatutil_get_nv_item( NV_DS_AT_CSAS_CSCB_SETTINGS_I, &ds_nv_item ) != NV_DONE_S )
  {
    return DSAT_ERROR;
  }
  DSATUTIL_SET_VAL(DSATETSI_EXT_CSCB_IDX,CSCB_MODE,0,
