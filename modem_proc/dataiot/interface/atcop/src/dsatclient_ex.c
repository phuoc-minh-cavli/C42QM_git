...
DESCRIPTION
  ...
  Copyright (c) 2005 - 2017,2020 by Qualcomm Technologies Incorporated.
  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatclient_ex.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/04/17   ss      Added support for remote AT commands based on EFS.
03/28/17   ss      Added +IPR to AT FWD allowed command list.
10/05/16   skc     Added +CMUX to AT FWD allowed command list.
...
===========================================================================*/
  ...
  */
  LOCAL byte allowed_list[][MAX_CMD_SIZE]={"+CLVL","+CKPD","+CMUT","+CTSA","+IPR",
                                           "+CBKLT","+CFUN","+CDIS","+CRSL","+CMAR",
                                           "+CSO","+CSS","+CBC","$QCPWRDN","+CMUX",""};
  ...
  */
  uint32 dsat_fwd_at_clients[DSAT_MAX_CLIENT]={0};
...
(
  uint32                              client_id,      /*  registered client_id  */
...
(
  uint32           client_id /* Registered client id */
)
{
...
)
{
  ...
  for ( i = 0; i < num_valid_cmds; ++i)
  {
    ...
    forward_at_cmd->at_cmd_list[i].is_support_token_ex =
                                    at_cmd_list[i].is_support_token_ex;
  }
  forward_at_cmd->client_id = client_id;
  ...
  while(fwd_at_cmd_p != NULL)
  {
    ...
    /* Do not over write any of the dsatcmdp_processing_fwd_cmd
      ...
      /* DS3G_SIOLIB_PORT_MAX ==> This is used to to uniquely identify if this is for QMI DMS
          */
      is_support_token_ex = fwd_at_cmd_p->is_support_token_ex;
...
LOCAL boolean client_validate_fwd_at_cmds 
(
  uint32                                    client_id,  /* Registered client */
...
)
{
  ...
  {
    DS_AT_MSG1_ERROR("Num_valid_cmds(%d) is greater than allowed length per request",
                     num_valid_cmds);
    return FALSE;
  }
    ...
    if(valid_cmd == FALSE)
    {
      ...
      while( token_cat < NUM_AT_CMD_CATEGORIES)
      {
        ...
        if(!no_match)
        {
          ...
          {
            DS_AT_MSG0_ERROR("AT spec command is not allowed to register for forwarding");
            return FALSE;
          }
          break;
        }
...
)
{
  ...
  for( i=0; i<num_valid_cmds; i++ )
  {
    ...
    dsat_fwd_at_cmd_p->is_support_token_ex = at_cmd_list[i].is_support_token_ex;
...
LOCAL boolean client_validate_fwd_client_id 
(
  uint32                              client_id  /* client_id for validation */
)
{
  int i =0;

  /* Pre condition check */
  if(client_id == DSAT_INVALID_CLIENT_ID)
  {
    DS_AT_MSG0_ERROR("Invalid client id");
    return FALSE;
  }
  ...
  DS_AT_MSG1_ERROR("Client id (%d) not found", client_id);
  return FALSE;
...
(
  ds_command_enum_type            cmd_id,         /* Command ID */
...
void dsat_init_allowed_list_efs ()
{
     ...
     (status.st_size != 0) )
  {
    ...
    if (fd < 0)
    {
      DS_AT_MSG1_ERROR("efs_open failed: error code - %d", efs_errno);
