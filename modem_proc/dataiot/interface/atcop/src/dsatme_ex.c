...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2004 - 2018 by Qualcomm Technologies Incorporated.
   All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/atcop/src/dsatme_ex.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/09/20   ks      Added support for preserving URC indices across PSM cycles.
04/20/18   ya	   Macro to Function conversion
11/13/17   skc     Fixed issue where subscription status flag was not updated properly.
...
FUNCTION  SEND_PASS_MMGSDI_REFRESH_COMPLETE

DESCRIPTION
  This function is used to send MMGSDI refresh complete pass status.
  
DEPENDENCIES
  None

RETURN VALUE
  TRUE: if pass refresh completed and FALSE otherwise
...
boolean send_pass_mmgsdi_refresh_complete
(
  cb_cmd_e_type       refresh_mask,
  dsat_apps_id_e_type apps_id
)
{
  ...
  if( (DSAT_MMGSDI_REFRESH_MASK_CB & refresh_mask ) && ( me_val->dsat_refresh_fcn_ind.num_files == 1 )) 
  {
    me_mmgsdi_refresh_complete(TRUE,apps_id);
	memset((void*)&me_val->dsat_refresh_fcn_ind,0x0,sizeof(dsat_fcn_ind_type));
  }
  else if ( me_val->dsat_refresh_fcn_ind.num_files > 1) 
  {
	me_val->dsat_refresh_fcn_ind.num_files--;
...
DESCRIPTION
  This function is used to send MMGSDI refresh complete fail stat
  
DEPENDENCIES
  None

RETURN VALUE
  TRUE: if fail refresh completed and FALSE otherwise
...
boolean send_fail_mmgsdi_refresh_complete
(
  cb_cmd_e_type       refresh_mask,
  dsat_apps_id_e_type apps_id
)
{
  ...
  if ( (DSAT_MMGSDI_REFRESH_MASK_CB & refresh_mask ) && 
					 ( me_val->dsat_refresh_fcn_ind.num_files != 0 )) 
  {
...
)
{
  ...
  pbm_val = dsat_get_base_addr(DSAT_PBM_VALS, TRUE);
  if (NULL == pbm_val)
  {
    return;
  }
...
dsat_num_item_type  enum_val;
  ...
  if((dsat_num_item_type)dsatutil_get_val(enum_val,0,NUM_TYPE) == 0 && 
...
)
{
  ...
  pbm_val = dsat_get_base_addr(DSAT_PBM_VALS, TRUE);
  if ((NULL == pbm_val) || (NULL == pbm_val->ld_rec_ptr))
  {
    return ;
  }

  if (pbm_val->ld_rec_ptr->state == LD_DIALED)
  {
    pbm_val->ld_rec_ptr->state = LD_CONNECTED;

    pbm_val->ld_rec_ptr->rec.is_answered = TRUE;
    ...
    pbm_val->ld_rec_ptr->conn_time = time_jul_to_secs(&julian_time);
  }
...
)
{
  ...
  if(pbm_val->ld_rec_ptr->state != LD_NONE)
  {
    if(pbm_val->ld_rec_ptr->state == LD_CONNECTED)
    {
      /* get the duration of the call */
      time_julian_type     julian_time;
      dword               cur_time;
      
      DS_ASSERT(pbm_val->ld_rec_ptr->rec.is_answered);
      ...
      pbm_val->ld_rec_ptr->rec.duration = cur_time - pbm_val->ld_rec_ptr->conn_time;
    }

    if(pbm_val->ld_rec_ptr->rec.text[0] != '\0' )
    {
      (void) dsatutil_memscpy((void*)name,
            PBM_TEXT_SIZE_CHARS,(void*)pbm_val->ld_rec_ptr->rec.text,PBM_TEXT_SIZE_BYTES);
      data_sizes[num_fields++] = (pbmutils_wstrlen(name)+ 1) * sizeof(uint16);	  
    }

    data_sizes[num_fields++] =  strlen(pbm_val->ld_rec_ptr->rec.number)+ 1;
    ...
    /* reset the state */
    pbm_val->ld_rec_ptr->state = LD_NONE;

    dsatutil_free_memory(pbm_val->ld_rec_ptr);
    pbm_val->ld_rec_ptr = NULL;
  }
} /* dsatme_ld_save */
...
)
{
  ...
  if(status == PBM_SUCCESS)
  {
    ...
    DS_ASSERT(me_cpbs_entry.device == info_ptr->pbm_device);
...
===========================================================================*/
dsat_result_enum_type dsat_mmgsdi_client_init
(
...
)
{
  ...
  DS_AT_MSG4_ERROR("Session_type = %d, client %d Session_id = %d, App type = %d",
  ...
  me_val->dsat_mmgsdi_client_id = client_id;

  switch (session_info_ptr->session_type)
  {
    case MMGSDI_GW_PROV_PRI_SESSION:
      dsat_qcsimapp_info[DSAT_APPS_ID_PRIMARY].session_id =
        session_info_ptr->session_id;
      break;

    case MMGSDI_GW_PROV_SEC_SESSION:
      dsat_qcsimapp_info[DSAT_APPS_ID_SECONDARY].session_id =
        session_info_ptr->session_id;
      break;

    case MMGSDI_GW_PROV_TER_SESSION:
      dsat_qcsimapp_info[DSAT_APPS_ID_TERTIARY].session_id =
        session_info_ptr->session_id;
      break;
...
void dsatme_initialize_to_default(dsat_mem_type mem_type,void *ptr)
{
  ...
  switch(mem_type)
  {
    ...
    {
      dsatme_mmgsdi_state_info  *ss = (dsatme_mmgsdi_state_info  *)ptr;
      ss->dsat_mmgsdi_client_id = (mmgsdi_client_id_type)MMGSDI_INVALID_SESSION_ID;

      ss->dsatme_plmn_mode_sel_bit    = CSP_PLMN_MODE_BIT_UNSET;
      ss->dsat_pin_required           = DSAT_CPIN_NONE_REQUIRED;
      ss->dsat_mask_pin2_state        = TRUE;
      ss->dsat_sim_init_comp          = FALSE;
      ss->dsat_pin_error = DSAT_CME_SIM_NOT_INSERTED;
      /* Initialize the plmn selctor */
      ss->dsat_sim_filename_plmn_selector = MMGSDI_NO_FILE_ENUM;
#ifdef FEATURE_DSAT_ATCOP_VEND
      ss->dsat_qcsimstat_curr         = DSAT_QCSIMSTAT_UNKNOWN;
      ss->is_qcsimstat_updated        = FALSE;
#endif
      ss->dsat_pin_error = DSAT_CME_SIM_NOT_INSERTED;
    }
    break;
    ...
    case DSAT_PBM_VALS:
    {
      dsatme_pbm_state_info  *ss = (dsatme_pbm_state_info  *)ptr;
    ...
    {
      dsat_common_state_info *state_info = (dsat_common_state_info  *)ptr;
      state_info->dsat_qcsimapp_val_updated   = DSAT_QCSIMAPP_VAL_INVALID;
      state_info->dsat_qcsimapp_val_user_flag = FALSE;
    }
    break;
    case DSAT_MD_VALS:
    {
    
      dsatetsipkt_gprs_msg_info  *gprs_msg_info = (dsatetsipkt_gprs_msg_info  *)ptr;
      gprs_msg_info->flush_gprs_buffer = FALSE;
#ifdef FEATURE_DSAT_ETSI_DATA
      /* Initialize the GPRS events message queue for +CGEREP command */
      (void) q_init(&gprs_msg_info->etsipkt_gprs_msg_q );
#endif /* FEATURE_DSAT_ETSI_DATA */
    }
    break;
    case DSAT_PSM_URC_VALS:
    {
      dsat_psm_atcop_urc_state_info *psm_urc_info = (dsat_psm_atcop_urc_state_info *)ptr;
      memset ((void *)psm_urc_info, 0x0, sizeof(dsat_psm_atcop_urc_state_info));
    }
    break;
    default:
     break;
...
mmgsdi_return_enum_type mmgsdi_status = MMGSDI_ERROR;
  ...
  DS_AT_MSG3_HIGH("dsatme_mmgsdi_event_handler for event: %d, Session id: %d Apps ID: %d",
                               mmgsdi_event,
  ...
  switch (mmgsdi_event)
  {
    case MMGSDI_CARD_INSERTED_EVT:
      ...
      me_val->dsat_pin_error      = DSAT_CME_SIM_BUSY;
      break;
    ...
    case MMGSDI_CARD_ERROR_EVT:
      ...
      {
        me_val->dsat_pin_error = DSAT_CME_SIM_NOT_INSERTED;
      }
      else
      {
        me_val->dsat_pin_error = DSAT_CME_SIM_FAILURE;
      }
      break;
    ...
    case MMGSDI_SESSION_CHANGED_EVT:
      ...
      else
      {
        ...
        /* Clearing subscription status; it will be true only 
           during subscription ready event */
        dsat_qcsimapp_info[apps_id].active   = FALSE;

        me_val = dsat_get_base_addr(DSAT_MMGSDI_VALS, TRUE); 
        if (NULL != me_val)
        {
          memset((void *)&me_val->dsat_spn_info,0x00,sizeof(dsat_mmgsdi_spn_info_s_type));
          me_val->dsatme_plmn_mode_sel_bit = CSP_PLMN_MODE_BIT_UNSET;
          /* Update the variable as this is equivalent to SIM restart */
          me_val->dsat_sim_init_comp = FALSE;
          /* Update the CPIN status to be SIM FAILURE */
          /*Clear PUK status*/
          me_val->dsat_pin_blocked  = 0;
          /*Set PIN to PIN NONE*/
          me_val->dsat_pin_required = DSAT_CPIN_NONE_REQUIRED;
          me_val->dsat_pin_error = DSAT_CME_SIM_BUSY;
        } 
      }
        ...
        if (IS_VALID_APPS_ID(apps_id))
        {
          ...
          switch (perso_event)
          {
            case MMGSDI_PERSO_NW:      /* NET Personalization PIN required */
              ...
              {
                me_val->dsat_pin_required |= (uint32)DSAT_CPIN_NET_PIN_REQUIRED;
              }
              else if(mmgsdi_event_info->perso_info.status == 
                   MMGSDI_PERSO_STATUS_UNLOCKED)
              {
                me_val->dsat_pin_required &= ~(uint32)DSAT_CPIN_NET_PIN_REQUIRED;
                me_val->dsat_pin_blocked &= ~(uint32)DSAT_CPIN_NET_PIN_REQUIRED;
            ...
            case MMGSDI_PERSO_NS:      /* NETSUB Personalization PIN required */
              ...
              {
                me_val->dsat_pin_required |= (uint32)DSAT_CPIN_NETSUB_PIN_REQUIRED;
              }
              else if(mmgsdi_event_info->perso_info.status == 
                   MMGSDI_PERSO_STATUS_UNLOCKED)
              {
                me_val->dsat_pin_required &= ~(uint32)DSAT_CPIN_NETSUB_PIN_REQUIRED;
                me_val->dsat_pin_blocked &= ~(uint32)DSAT_CPIN_NETSUB_PIN_REQUIRED;
                /* Check for disabling restricted command mode */
                respond = TRUE;
              }
              else if(mmgsdi_event_info->perso_info.status == 
            ...
            case MMGSDI_PERSO_SP:      /* SP Personalization PIN required */
              if(mmgsdi_event_info->perso_info.status == 
                                                  MMGSDI_PERSO_STATUS_LOCKED)
              {
                me_val->dsat_pin_required |= (uint32)DSAT_CPIN_SP_PIN_REQUIRED;
              }
              else if(mmgsdi_event_info->perso_info.status == 
                   MMGSDI_PERSO_STATUS_UNLOCKED)
              {
                me_val->dsat_pin_required &= ~(uint32)DSAT_CPIN_SP_PIN_REQUIRED;
                me_val->dsat_pin_blocked &= ~(uint32)DSAT_CPIN_SP_PIN_REQUIRED;
                /* Check for disabling restricted command mode */
                respond = TRUE;
              }
              else if(mmgsdi_event_info->perso_info.status == 
            ...
            case MMGSDI_PERSO_CP:      /* CORP Personalization PIN required */
              if(mmgsdi_event_info->perso_info.status == 
                                                 MMGSDI_PERSO_STATUS_LOCKED)
              {
                me_val->dsat_pin_required |= (uint32)DSAT_CPIN_CORP_PIN_REQUIRED;
              }
              else if(mmgsdi_event_info->perso_info.status == 
                   MMGSDI_PERSO_STATUS_UNLOCKED)
              {
                me_val->dsat_pin_required &= ~(uint32)DSAT_CPIN_CORP_PIN_REQUIRED;
                me_val->dsat_pin_blocked &= ~(uint32)DSAT_CPIN_CORP_PIN_REQUIRED;
                /* Check for disabling restricted command mode */
                respond = TRUE;
              }
              else if(mmgsdi_event_info->perso_info.status == 
            ...
            case MMGSDI_PERSO_SIM:      /* SIM Personalization PIN required */
              if(mmgsdi_event_info->perso_info.status ==
                                                    MMGSDI_PERSO_STATUS_LOCKED)
              {
                me_val->dsat_pin_required |= (uint32)DSAT_CPIN_PH_FSIM_PIN_REQUIRED;
              }
              else if(mmgsdi_event_info->perso_info.status == 
                   MMGSDI_PERSO_STATUS_UNLOCKED)
              {
                me_val->dsat_pin_required &= ~(uint32)DSAT_CPIN_PH_FSIM_PIN_REQUIRED;
                me_val->dsat_pin_blocked &= ~(uint32)DSAT_CPIN_PH_FSIM_PIN_REQUIRED;
                /* Check for disabling restricted command mode */
                respond = TRUE;
              }
              else if(mmgsdi_event_info->perso_info.status == 
              ...
              case MMGSDI_MAX_PERSO_FEATURE_ENUM:
              {
                if (mmgsdi_event_info->perso_info.status == MMGSDI_PERSO_STATUS_DONE)
                {
                  ...
                  me_val->dsat_pin_required &= ~pin_bitmask;
                  me_val->dsat_pin_blocked  &= ~pin_bitmask;
                  respond = TRUE;
                }
              }
              break;
...
#define CPHS_VAS_OFFSET    0xC0 /* Value Added Service (VAS)group code*/
  ...
  prev_csp_val = me_val->dsatme_plmn_mode_sel_bit;
      ...
      ( index < cmd_rd_ptr->data_len) )
  {
    ...
    if(cmd_rd_ptr->data[index+1] & PLMN_MODE_BIT)
    {
      ...
      me_val->dsatme_plmn_mode_sel_bit = CSP_PLMN_MODE_BIT_ON;
    }
    else
    {
      /* Manual PLMN Selection is not selected */
      me_val->dsatme_plmn_mode_sel_bit = CSP_PLMN_MODE_BIT_OFF;
    }
  }
  else
  { 
  ...
  DS_AT_MSG3_HIGH("CSP prev val = %d current val = %d COPS mode = %d",prev_csp_val,
                          me_val->dsatme_plmn_mode_sel_bit, (dsat_num_item_type)
                          dsatutil_get_val(DSATETSI_EXT_ACT_COPS_ETSI_IDX,0,MIX_NUM_TYPE));
  /*
     When PLMN mode bit changes from 1 to 0 and the current mode is manual,
     it must be changed to Automatic mode 
  */
  
  if( ( me_val->dsatme_plmn_mode_sel_bit == CSP_PLMN_MODE_BIT_OFF ) &&
      ( prev_csp_val == CSP_PLMN_MODE_BIT_ON ) &&
      ( (dsat_num_item_type)dsatutil_get_val(DSATETSI_EXT_ACT_COPS_ETSI_IDX,
          0,MIX_NUM_TYPE) == (dsat_num_item_type)DSAT_COPS_MODE_MANUAL))
  {
    ...
    ph_val->dsatetsicall_network_list.requested_pref.mode = DSAT_COPS_MODE_AUTO;
...
#define SPN_ENCODING_UCS2_TYPE_82  0x82
  ...
  switch (spn_in[0]) 
  {
    ...
    default:
      ...
      memscpy((char*)spn_out, DSAT_MAX_SPN_NAME_LEN,spn_in, DSAT_MAX_SPN_NAME_LEN);
...
#define OPERATOR_NAME_INDEX 1
  ...
  if( MMGSDI_SUCCESS != cmd_rd_ptr->cmd_status )
  {
   ...
   if (SEND_FAIL_MMGSDI_REFRESH_COMPLETE(refresh_mask,apps_id)) return DSAT_ASYNC_EVENT;
    return DSAT_ASYNC_EVENT;
  }
  memset((void *)&me_val->dsat_spn_info,0x00,sizeof(dsat_mmgsdi_spn_info_s_type));
  me_val->dsat_spn_info.operator_name_len = 0;
  /* Byte 0 contains the display condition */
  me_val->dsat_spn_info.display_cond = cmd_rd_ptr->data[DISPLAY_COND_INDEX];

  /* From Byte 1 to length -1, the operator name is present */
  if ( cmd_rd_ptr->data[OPERATOR_NAME_INDEX] != 0xFF )
  {

    me_val->dsat_spn_info.operator_name_len = 
      dsatme_extract_operator_name((char*)&cmd_rd_ptr->data[OPERATOR_NAME_INDEX], 
                                   (MAX_MMGSDI_BUFFER_LEN - 1),
                                   (char*)me_val->dsat_spn_info.operator_name,
                                   DSAT_MAX_SPN_NAME_LEN, 
                                   &me_val->dsat_spn_info.encoding_type);
    me_val->dsat_spn_info.spn_info_valid = TRUE;
  }
  else
  {
    DS_AT_MSG1_ERROR("SPN Display Info is invalid %x",
                    cmd_rd_ptr->data[OPERATOR_NAME_INDEX]);
    me_val->dsat_spn_info.spn_info_valid = FALSE;
  }
  if (SEND_PASS_MMGSDI_REFRESH_COMPLETE(refresh_mask,apps_id)) return DSAT_ASYNC_EVENT;
  return DSAT_ASYNC_EVENT;
...
)
{
  ...
  memset((void *)&me_val->dsat_imsi_info,0x00,sizeof(dsat_mmgsdi_imsi_info_s_type));
   /* Copy the entire data */

  (void) dsatutil_memscpy((void*)&me_val->dsat_imsi_info.data[0],
          MMGSDI_IMSI_LEN,(void*)&cmd_rd_ptr->data[0],
           MIN(cmd_rd_ptr->data_len, MAX_MMGSDI_BUFFER_LEN) ); 
  me_val->dsat_imsi_info.len =(uint16) cmd_rd_ptr->data_len;
...
)
{
  ...
  DEBUG ( DS_ASSERT(cb_expected == (cb_cmd_e_type)cnf_ptr->response_header.client_data) ;);
...
)
{
  ...
  switch ( client_ref )
  {
    ...
    default:
       ...
       DS_ASSERT(0); /* for testing purpose */
  }
...
)
{
  ...
  (void) dsatutil_memscpy((void*)&me_val->dsat_refresh_fcn_ind,
          sizeof(dsat_fcn_ind_type),
          (void*)&mmgsdi_event_info->u.fcn_ind,
  	      sizeof(dsat_fcn_ind_type));

  if( (0 == me_val->dsat_refresh_fcn_ind.num_files)||
      (me_val->dsat_refresh_fcn_ind.num_files > DSAT_MAX_NUM_REFRESH_FILES))
  {
  ...
  else
  {
    DS_AT_MSG1_HIGH("MMGSDI refresh complete has %d number of files",
                                               me_val->dsat_refresh_fcn_ind.num_files);
    while((index < me_val->dsat_refresh_fcn_ind.num_files)&& (FALSE != pass_fail))
    {
      switch(me_val->dsat_refresh_fcn_ind.file_list[index])
      {
        case MMGSDI_GSM_CSP:
