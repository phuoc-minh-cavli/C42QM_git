...
/*===========================================================================
  ...
  $PVCSPath: $
  $Header: //components/rel/dataiot.mpss/2.0/interface/dss/src/dss_ping_comm_mgr.c#2 $ $DateTime: 2020/05/17 01:16:07 $ $Author: pwbldsvc $
...
/* DSS Ping APIs not supported in IOE builds */
...
typedef struct
{
  ...
  void                                    *app_user_data;
  dss_net_lib_info_type_s                  net_policy;
...
extern ds_crit_sect_type dss_ping_crit_section;
/*===========================================================================
...
)
{
  ...
  int icmp_type;
  int icmp_code;
  uint32 sock_opt_len = sizeof(icmp_type);

  dssi_ping_comm_mgr_ping_sess_type *ping_sess_ptr = NULL;
  
  if (ping_ptr == NULL)
  {
    LOG_MSG_ERROR_0("dss_ping_socket: ping_sess_ptr is NULL");
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }

  LOG_MSG_INFO1_3("dss_ping_socket(): "
                  "icmp_ver %d, type %d, code %d", icmp_ver,type, code);

  ping_sess_ptr = (dssi_ping_comm_mgr_ping_sess_type*)ping_ptr;
  ...
  sockfd = dss_socket(ping_sess_ptr->net_mgr_handle,
                      family,
                      DSS_SOCK_DGRAM,
                      protocol,
                      dss_errno);
  ...
  if (sockfd == DSS_ERROR)
  {
    LOG_MSG_ERROR_1("dss_ping_socket(): "
                    "Socket creation failed for Nethdl %d",
                    ping_sess_ptr->net_mgr_handle);
    return DSS_ERROR;
  }

  icmp_type = type;
  if (dss_setsockopt(sockfd,
                     DSS_IPPROTO_ICMP,
                     DSS_ICMP_TYPE,
                     (void*)&icmp_type,
                     &sock_opt_len,
  ...
  {
    LOG_MSG_ERROR_1("dss_ping_socket(): "
                    "Set sock opt DSS_ICMP_TYPE failed for sock %d",
                    sockfd);
    return DSS_ERROR;
  }
  ...
  {
    LOG_MSG_ERROR_1("dss_ping_socket(): "
                    "Set sock opt DSS_ICMP_CODE failed for sock %d",
                    sockfd);
    return DSS_ERROR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Get from Modem heap */
  DS_SYSTEM_HEAP_MEM_ALLOC( buf, total_size, void* );
  return buf;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ping_sess_ptr->sockfd != 0 )
  {
    ...
    ret = dss_dereg_ip_filter(ping_sess_ptr->sockfd,
                        ping_sess_ptr->filter_handle,
                        &dss_errno);
    if (ret == DSS_ERROR)
    {
     LOG_MSG_ERROR_2("dssi_ping_comm_mgr_close_conn(): "
                      "DSS filter dereg failed errno %d, sockfd %d",
  ...
  if(ping_sess_ptr->net_mgr_handle != DSS_ERROR)
  {
    ...
    ret = dss_bringdown_call(ping_sess_ptr->net_mgr_handle, errno_ptr);
    if( DSS_ERROR == ret )
    {
...
)
{
  ...
  int16                              sockfd;
  uint32                             event_mask;
  void                               *sock_cb_user_data = NULL;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if (sock_callback_info == NULL)
  {
    LOG_MSG_ERROR_0("dssi_ping_comm_mgr_sock_event_cb_fn(): "
                    "sock_callback_info is NULL");
    return;
  }

  dss_nethandle = sock_callback_info->dss_nethandle;
  sockfd = sock_callback_info->sockfd;
  event_mask = sock_callback_info->event_mask;
  sock_cb_user_data = sock_callback_info->sock_cb_user_data;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(ping_sess_ptr->icmp_ver == DSSICMP_ICMP_V4)
  {
    ping_sess_ptr->sockfd = dss_ping_socket( ping_sess_ptr->icmp_ver,
                                            (uint8)ICMP_V4_ECHO_REQ_MSG_TYPE,
                                            (uint8)ICMP_ECHO_MSG_CODE,
                                            ping_sess_ptr,
                                            errno_ptr);
  }
  else
  ...
  if( ping_sess_ptr->sockfd == DSS_ERROR )
  {
    ...
    if(dss_bringdown_call(ping_sess_ptr->net_mgr_handle,
                          errno_ptr) == DSS_ERROR)
    {
      LOG_MSG_ERROR_1("dssi_ping_comm_mgr_net_handle_enetisconn(): "
                      "PDN bringdown error %d", *errno_ptr);
    }
  ...
  if ( TRUE == ret )
  {
    ...
    if(ping_sess_ptr->icmp_ver == DSSICMP_ICMP_V4)
    {
      ...
      filter.next_prot_hdr.icmp.type       = ICMP_V4_ECHO_RSP_MSG_TYPE;
      filter.next_prot_hdr.icmp.code       = ICMP_ECHO_MSG_CODE;
      filter.next_prot_hdr.icmp.id         = icmp_msg_id;
    }
    else
    {
      filter.ip_vsn = IPV6_ADDR;
      ...
      filter.next_prot_hdr.icmp.field_mask = IPFLTR_MASK_ICMP_ALL;
      filter.next_prot_hdr.icmp.type       = ICMP_V6_ECHO_RSP_MSG_TYPE;
      filter.next_prot_hdr.icmp.code       = ICMP_ECHO_MSG_CODE;
      filter.next_prot_hdr.icmp.id         = icmp_msg_id;
    }
 
    ret_val = dss_reg_ip_filter(ping_sess_ptr->sockfd,
                                1,
                                &filter,
                                &ping_sess_ptr->filter_handle,
                                &dss_errno);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (net_callback_info == NULL)
  {
    LOG_MSG_ERROR_0("dssi_ping_comm_mgr_net_event_cb_fn(): "
                    "net_callback_info is NULL");
    return;
  }

  net_mgr_handle = net_callback_info->dss_nethandle;
  net_cb_user_data = net_callback_info->net_cb_user_data;
  ...
  {
    DS_ERROR_LOG_0("dssi_ping_comm_mgr_net_event_cb_fn: Invalid NET event");
    return;
  }
  ...
  if ( acb_ptr == NULL)
  {
    DS_ERROR_LOG_0("dssi_ping_comm_mgr_net_event_cb_fn: ACB pointer is NULL");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *errno_ptr = DSS_SUCCESS;
  ...
  if(ping_sess_ptr->app_net_policy_specified == FALSE)
  {
    ...
    if(ping_sess_ptr->icmp_ver == DSSICMP_ICMP_V4)
    {
      ping_sess_ptr->net_policy.family = DSS_AF_INET;
    }
    else if(ping_sess_ptr->icmp_ver == DSSICMP_ICMP_V6)
    {
  ...
  ping_sess_ptr->net_policy.net_cb = dssi_ping_comm_mgr_net_event_cb_fn;
  ping_sess_ptr->net_policy.net_cb_user_data = (void*)ping_sess_msg_id;
  ping_sess_ptr->net_policy.sock_cb = dssi_ping_comm_mgr_sock_event_cb_fn;
  ping_sess_ptr->net_policy.sock_cb_user_data = (void*)ping_sess_msg_id;

  ping_sess_ptr->net_mgr_handle =dss_bringup_call(&ping_sess_ptr->net_policy,
                                                  errno_ptr);
  ...
  if( ping_sess_ptr->net_mgr_handle != DSS_ERROR && *errno_ptr == DSS_SUCCESS)
  {
    ...
    net_callback_info->dss_nethandle =  ping_sess_ptr->net_mgr_handle;
    net_callback_info->dss_errno = DS_ENETISCONN;
    net_callback_info->net_cb_user_data = (void*)ping_sess_msg_id;

    dssi_ping_comm_mgr_net_event_cb_fn(net_callback_info);
    DS_SYSTEM_HEAP_MEM_FREE(net_callback_info);

    return DSS_SUCCESS;
  }
  else if (ping_sess_ptr->net_mgr_handle != DSS_ERROR && *errno_ptr == DS_EWOULDBLOCK)
  {
    LOG_MSG_INFO1_0("dssi_ping_comm_mgr_bring_up_net_iface(): "
                    "PDN bring up in progress");
    return DSS_SUCCESS;
  }
  else if( ping_sess_ptr->net_mgr_handle == DSS_ERROR)
  {
    LOG_MSG_ERROR_1("dssi_ping_comm_mgr_bring_up_net_iface(): "
                    "PDN bringup error %d", *errno_ptr);
    return DSS_ERROR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ENTER_CRIT_SECTION(&dss_ping_crit_section);
     ...
     == NULL)
  {
    ...
    DS_LEAVE_CRIT_SECTION(&dss_ping_crit_section);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ping_sess_ptr == NULL )
  {
    ...
    DS_BRANCH_LEAVE_CRIT_SECTION(&dss_ping_crit_section);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *errno_ptr = DSS_SUCCESS;
  ...
  {
    /* copy the elements of net lib structure to ping session strucuture */
    memscpy(&ping_sess_ptr->net_policy, sizeof(dss_net_lib_info_type_s),
                 net_policy_ptr, sizeof(dss_net_lib_info_type_s));
				  
    /* Allocate memory to store APN string, this is to avoid corruption which
       might happen when the original address is free'd */

    ping_sess_ptr->apn_len = strlen((const char *)net_policy_ptr->name);

    if (ping_sess_ptr->apn_len != 0)
    {
      //Allocate one byte more than the APN length to account for NULL termination
    DS_SYSTEM_HEAP_MEM_ALLOC(ping_sess_ptr->net_policy.name,
                             ((ping_sess_ptr->apn_len)+1), byte *);
    if (NULL == ping_sess_ptr->net_policy.name)
    {
      LOG_MSG_ERROR_0("dss_ping_comm_mgr_create_ping_session(): "
                      "Failed to allocate memory for net policy name buffer");
      return DSS_PING_INVALID_HANDLE;
    }
	
    /* copy APN string to newly allocated address */
      strlcpy((char *)ping_sess_ptr->net_policy.name, 
              (const char *)net_policy_ptr->name, (ping_sess_ptr->apn_len)+1);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( ping_sess_ptr->retransmit_timer_handle != PS_TIMER_INVALID_HANDLE )
  {
    PS_TIMER_FREE_HANDLE(ping_sess_ptr->retransmit_timer_handle);
  }

  if( ping_sess_ptr->response_timeout_timer_handle != PS_TIMER_INVALID_HANDLE )
  {
    PS_TIMER_FREE_HANDLE(ping_sess_ptr->response_timeout_timer_handle);
  }
  ...
  /* Free the memory allocated to store APN string */
  if (ping_sess_ptr->apn_len != 0)
  {
    DS_SYSTEM_HEAP_MEM_FREE(ping_sess_ptr->net_policy.name);
    ping_sess_ptr->apn_len = 0;
  }
