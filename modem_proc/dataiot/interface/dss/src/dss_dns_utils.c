...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ret = snprintf(dst, size, "%u.%u.%u.%u", src[0], src[1], src[2], src[3]);

  if (ret >= (int32)size)
  {
    MSG_MED("dnsi_inet_ntop4(): Require %d octets for conversion have %d "
            "octets.  Returning DS_EMSGTRUNC", ret, size, 0 );
    *dss_errno = DS_EMSGTRUNC;
    retval = DSS_ERROR;
  }
  else if (ret == -1)
  {
    MSG_MED("dnsi_inet_ntop4(): Failed to convert address.  Returning "
            "DS_ENORECOVERY", 0, 0, 0 );
  ...
  for (i = 0; i < (int)(sizeof(struct ps_in6_addr) / sizeof(int16)); i++)
  {
         ...
         (best.len == 5 && words[5] == 0xffff)))
    {
      if (dnsi_inet_ntop4(src+12, tp, sizeof tmp - (uint32)(tp - tmp),
                                                    dss_errno) == DSS_ERROR)
      {
        QSR_MSG_MED( 3282440877ULL, "dnsi_inet_ntop6(): Failed to convert v4-mapped-v6 address.",
                0, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
        return DSS_ERROR;
      }
      tp += strlen(tp);
      break;
    ...
    {
      MSG_MED("dnsi_inet_ntop6(): Insufficient memory, need %d octets, have "
              "%d octets.  Returning DS_EMSGTRUNC",
              sizeof(uint32), endp - tp, 0 );
      *dss_errno = DS_EMSGTRUNC;
      return DSS_ERROR;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *tp = 0;
  ...
  while ((ch = *src++) != '\0')
  {
    ...
    {
      new = (uint32)(*tp * 10 + (pch - digits));

      if (saw_digit && *tp == 0)
      {
        MSG_MED("dnsi_inet_pton4(): For octet %d value started with 0.  "
      ...
      if (new > 255)
      {
        MSG_MED("dnsi_inet_pton4(): Value %d greater then 0xff in octet %d.  "
                "Return DS_NAMEERR", new, octets, 0 );
        *dss_errno = DS_NAMEERR;
        return DSS_ERROR;
      }
      *tp = (unsigned char)new;
      if (!saw_digit)
      {
        if (++octets > 4)
        {
          MSG_MED("dnsi_inet_pton4(): Number of octets %d greater then 4.  "
                  "Return DS_NAMEERR", octets, 0, 0 );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
    ...
    else
    {
      if (ch == '.' && saw_digit)
      {
        if (octets == 4)
        {
          MSG_MED("dnsi_inet_pton4(): '.' found after 4 octets.  Return "
                  "DS_NAMEERR", 0, 0, 0 );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
      ...
      {
        MSG_MED("dnsi_inet_pton4(): %c found in octet %d no digits seen.  "
  ...
  if (octets < 4)
  {
    QSR_MSG_MED( 1762994741ULL, "dnsi_inet_pton4(): Only %d octets found. Return DS_NAMEERR",
            octets, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
    *dss_errno = DS_NAMEERR;
    return DSS_ERROR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (*src == ':')
  {
    if (*++src != ':')
    {
      MSG_MED("dnsi_inet_pton4(): Address begins with single ':'.  Return "
              "DS_NAMEERR", 0, 0, 0 );
      *dss_errno = DS_NAMEERR;
      return DSS_ERROR;
    }
  ...
  while ((ch = *src++) != '\0')
  {
    ...
    if (pch != NULL)
    {
      ...
      if (++seen_xdigits > 4)
      {
        QSR_MSG_MED( 3078057067ULL, "dnsi_inet_pton4(): %d digits seen.  Return DS_NAMEERR",
                seen_xdigits, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
        *dss_errno = DS_NAMEERR;
        return DSS_ERROR;
      }
      continue;
    ...
    if (ch == ':')
    {
      ...
      if (!seen_xdigits)
      {
        if (colonp)
        {
          QSR_MSG_MED( 156346190ULL, "dnsi_inet_pton4(): Duplicate :: found.  Return DS_NAMEERR",
                  0, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
      ...
      else
      {
        if (*src == '\0')
        {
          MSG_MED("dnsi_inet_pton4(): ':' found at end of address.  "
      ...
      {
        MSG_MED("dnsi_inet_pton4(): Insufficient memory.  Need %d octets, "
    ...
    MSG_MED("dnsi_inet_pton4():  Found ch %c, need %d octets, have %d "
  ...
  if (colonp != NULL)
  {
    ...
    if (tp == endp)
    {
      QSR_MSG_MED( 1584793162ULL, "dnsi_inet_pton4(): Insufficient memory.  Return DS_EMSGTRUNC",
              0, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
      *dss_errno = DS_EMSGTRUNC;
      return DSS_ERROR;
    }
  ...
  if (tp != endp)
  {
    MSG_MED("dnsi_inet_pton4(): Out of %d octets, processed %d octets.  "
...
DEPENDENCIES
  ...
  To convert an IPv4 address it is suggested to be INET_ADDRSTRLEN in size
  while to convert an IPv6 address it is suggested to be INET6_ADDRSTRLEN.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (dss_errno == NULL)
  {
    QSR_MSG_MED( 3484520138ULL,  "dss_inet_ntop(): Called with dss_errno NULL", 0, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
    return DSS_ERROR;
  }

  if (src == NULL || dst == NULL)
  {
    QSR_MSG_MED( 1976184846ULL,  "dss_inet_ntop(): Called with src %p, dst %p", src, dst, 0 );//auto-gen, to change remove 'QSR_' and first param
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  if (dst_size == 0)
  {
    QSR_MSG_MED( 3674801011ULL,  "dss_inet_ntop(): dst_size 0", dst_size, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (dss_errno == NULL)
  {
    QSR_MSG_MED( 4070451367ULL,  "dss_inet_pton(): Called with dss_errno NULL", 0, 0, 0 );//auto-gen, to change remove 'QSR_' and first param
    return DSS_ERROR;
  }

  if (src == NULL || dst == NULL)
  {
    QSR_MSG_MED( 2377518026ULL,  "dss_inet_pton(): Called with src %p, dst %p", src, dst, 0 );//auto-gen, to change remove 'QSR_' and first param
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }

  if ((af == (int32)DSS_AF_INET && dst_size < sizeof(struct ps_in_addr)) ||
      (af == (int32)DSS_AF_INET6 && dst_size < sizeof(struct ps_in6_addr)))
  {
    QSR_MSG_MED( 3744626851ULL,  "dss_inet_pton(): dst_size %d too small for af %d",
             dst_size, af, 0 );//auto-gen, to change remove 'QSR_' and first param
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  for(index = 0; index < sizeof(struct ps_in_addr); index++)
  {
   ...
   /*------------------------------------------------------------------------
    ...
    while( '.' != *cp && 0 != *cp )
    {
      ...
      ---------------------------------------------------------------------*/
      if( ((int64) value * (int64) base) + number > (int64) 0xffffffff )
      {
        return DSS_ERROR;
      }

      value = (value * (uint32)base) + (uint32)number;
    } /* while */
