...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/dns/src/ps_dnsi_io_mgr.c#4 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/21/19   sr      Created module.
...
===========================================================================*/
void ps_dnsi_io_mgri_sock_close
(
  ps_dnsi_io_mgr_cb_type  * io_mgr_cb_ptr
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  -------------------------------------------------------------------------*/
  dsm_free_packet( &(io_mgr_cb_ptr->dns_query_ptr) );
  if (io_mgr_cb_ptr->cback_f_ptr != NULL)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    if (NULL != io_mgr_cb_ptr->fwk_inst)
    {
      retval = dss_dereg_ip_filter( io_mgr_cb_ptr->sockfd,
                                    io_mgr_cb_ptr->filter_handle,
                                    &ps_errno );
      if(DSS_ERROR == retval)
      {
      ...
      {
        LOG_MSG_INFO1_2("Deregistered filters installed on sock %d, fwk 0x%x",
                        io_mgr_cb_ptr->sockfd, io_mgr_cb_ptr->fwk_inst);
...
SIDE EFFECTS
  None
===========================================================================*/
void ps_dnsi_io_mgri_sock_event_cback
(
  dss_sock_callback_info_s *sock_callback_info
)
{
  int16     nethandle = DSS_ERROR;
  int16     sockfd;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_0("sock_callback_info is Null");
    return;
  }

  nethandle = sock_callback_info->dss_nethandle;
  sockfd = sock_callback_info->sockfd;
  sock_event_mask = sock_callback_info->event_mask;
  user_data_ptr = sock_callback_info->sock_cb_user_data;
  
  if( NULL == user_data_ptr )
  {
    LOG_MSG_ERROR_0("Null user data");
    return;
  }
  ...
  if( res_cb_ptr == NULL )
  {
    LOG_MSG_ERROR_0("res_cb_ptr is NULL");
    return;
  }

  if (res_cb_ptr->io_mgr_handle == PS_HANDLE_MGR_INVALID_HANDLE)
  {
    LOG_MSG_ERROR_0("Invalid IO mgr handle in res_cb_ptr");
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  io_mgr_cb_ptr->fwk_inst             = res_cb_ptr->fwk_inst;
  io_mgr_cb_ptr->is_registered_to_filters = FALSE;

  memscpy (&io_mgr_cb_ptr->net_policy, sizeof (dss_net_lib_info_type_s),
...
//TODO: Like in DHCP6 server, create a ACB and then associate socket with it
  /*-------------------------------------------------------------------------
    Create socket using dss_socket_create().
  -------------------------------------------------------------------------*/
  io_mgr_cb_ptr->sockfd = dss_socket(res_cb_ptr->net_mgr_handle,
                                     family,
                                     DSS_SOCK_DGRAM,
                                     PS_IPPROTO_UDP,
                                     ps_errno);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( io_mgr_cb_ptr->is_registered_to_filters )
  {
    //Change Dereg Ip filter logic to filter handle
    if (NULL != io_mgr_cb_ptr->fwk_inst)
    {
      retval = dss_dereg_ip_filter( io_mgr_cb_ptr->sockfd,
                                    io_mgr_cb_ptr->filter_handle,
  ...
  -------------------------------------------------------------------------*/
  //TODO: is_routable same as flow is enabled? Should we check for flow??
