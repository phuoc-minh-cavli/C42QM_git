...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/dns/src/ps_dnsi_msg_mgr.c#4 $
...
#include "dss.h"  //needed for ps_sockaddr used by DSM functions.
...
extern boolean dns_reserved_bits_config;
...
)
{
  ...
  char  * temp_dst_buf = NULL;
 uint32   domain_name_len = 0; /* Used to detect pointer loops       */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for(;;)
  {
    ...
    else
    {
      ...
      if( DSS_DNS_MAX_DOMAIN_NAME_LEN <= domain_name_len )
      {
        LOG_MSG_ERROR_1("Domain name len is exceeds max length %d", domain_name_len);
        retval = DSS_ERROR;
        break;
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* check NV 73927 is set */
    if(dns_reserved_bits_config && (dns_hdr_ptr->flags.flag_bits.reserved_bits  != 0)) 
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *ps_errno = DSS_SUCCESS;
  ...
  if( NULL == dns_response_ptr || NULL == rr_q )
  {
  ...
  while( NULL != (rr_node = q_get(&ps_dnsi_msg_mgr_rr_q)) )
  {
    ...
    {
      if( *cname_data_ptr == NULL )
      {
        *cname_data_ptr = ps_dnsi_mem_alloc( DSS_DNS_MAX_DOMAIN_NAME_LEN );
