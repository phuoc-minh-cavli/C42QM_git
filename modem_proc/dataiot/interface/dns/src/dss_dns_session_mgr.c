...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/dns/src/dss_dns_session_mgr.c#2 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (param_name)
  {
    case DSS_DNS_CONFIG_PARAMS_IFACE_ID:
      DS_ENTER_CRIT_SECTION(&global_ps_crit_section);
      if (1/*sizeof(dss_iface_id_type) != param_len*/)
      {
    ...
    case DSS_DNS_CONFIG_PARAMS_NET_POLICY:
      DS_ENTER_CRIT_SECTION(&global_ps_crit_section);
      if (sizeof(dss_net_lib_info_type_s) != param_len)
      {
        LOG_MSG_ERROR_1 ("Invalid param length for %d", param_name);
        *dss_errno = DS_EFAULT;
      }
