...
EXTERNALIZED FUNCTIONS
  ...
  ps_dns_cache_add_rr_q()
    Adds a Queue of RRs to the cache.

  ps_dns_cache_find_rr_q()
...
/*===========================================================================
  ...
  $Header: //components/rel/dataiot.mpss/2.0/interface/dns/src/ps_dns_cache.c#3 $
  $DateTime: 2021/01/27 01:30:19 $ $Author: pwbldsvc $
...
===========================================================================*/
/*===========================================================================
FUNCTION  PS_DNS_CACHEI_MEM_ALLOC
...
===========================================================================*/
static void* ps_dns_cachei_mem_alloc
(
  uint32 size
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_1("ps_dns_cachei_mem_alloc(): "
                    "Invalid size %d in ps_dnsi_memalloc()", size );
    return NULL;
  }

  total_size = ((sizeof(int32) - 1) + size) & (~(sizeof(int32) - 1));
  DS_SYSTEM_HEAP_MEM_ALLOC(buf_ptr, total_size, void*);

  /* Get from Modem heap */
  return buf_ptr;
} /* ps_dns_cachei_mem_alloc() */

/*===========================================================================
FUNCTION  PS_DNS_CACHEI_DUP_RR_Q()
...
===========================================================================*/
static int16 ps_dns_cachei_dup_rr_q
(
  q_type      * dup_rr_q,
  q_type      * rr_q,
  int16       * ps_errno
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  DS_ASSERT( NULL != dup_rr_q && NULL != ps_errno );
  ...
  while( rr_node != NULL )
  {
    ...
    {
      dup_rr_node->rdata = ps_dns_cachei_mem_alloc( (uint32)rr_node->rdata_len );
      if( NULL == dup_rr_node->rdata )
      {
        PS_MEM_FREE(dup_rr_node);
        LOG_MSG_ERROR_0("ps_dns_cachei_dup_rr_q(): "
  ...
  if( ret_val == DSS_ERROR )
  {
    while( ( dup_rr_node = q_get( dup_rr_q ) ) != NULL )
    {
      ...
      {
        DS_SYSTEM_HEAP_MEM_FREE(dup_rr_node->rdata);
      }
      PS_MEM_FREE(dup_rr_node);
    }
...
} /* ps_dns_cachei_dup_rr_q() */

/*===========================================================================
FUNCTION  PS_DNS_CACHEI_FREE_ENTRY()
...
SIDE EFFECTS
  None.
===========================================================================*/
static void ps_dns_cachei_free_entry
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO2_1("ps_dns_cachei_free_entry(): "
  ...
  LOG_MSG_INFO2_2("ps_dns_cachei_free_entry(): "
                  "Freeing entry 0x%p from cache for iface %d",
                  cache_entry_data,
                  cache_entry->fwk_inst );

  if (cache_entry->fwk_inst == NULL)
  {
     LOG_MSG_ERROR_0("ps_dns_cachei_free_entry(): "
                     "Invalid fwk, is passed");
     DS_ASSERT(0);
     return;
  }

  fwk_inst = cache_entry->fwk_inst;
  ...
  {
    PS_TIMER_FREE_HANDLE( cache_entry->pstimer );
  }
  ...
  while( NULL != (rr_node = q_get( &cache_entry->rr_q )) )
  {
     if ( 0 < rr_node->rdata_len )
     {
       DS_SYSTEM_HEAP_MEM_FREE(rr_node->rdata);
     }
     PS_MEM_FREE(rr_node);
  }
#ifdef FEATURE_Q_NO_SELF_QPTR
  q_delete( &(fwk_inst->dns_cache_q),
            &( cache_entry->link ) );
#else
...
} /* ps_dns_cachei_free_entry() */

/*===========================================================================
FUNCTION  PS_DNS_CACHEI_PTON4()
...
===========================================================================*/
static int ps_dns_cachei_pton4
(
  const char    *src,
  unsigned char *dst,
  int16         *dss_errno
  ...
  *tp = 0;
  ...
  while ((ch = *src++) != '\0')
  {
    if ((pch = strchr(digits, ch)) != NULL)
    {
      ...
      if (saw_digit && *tp == 0)
      {
        LOG_MSG_INFO2_1("ps_dns_cachei_pton4(): For octet %d value started with 0.  "
      ...
      if (new_digit > 255)
      {
        LOG_MSG_INFO2_2("ps_dns_cachei_pton4(): Value %d greater then 0xff in octet %d.  "
      ...
      *tp = (unsigned char)new_digit;
      if (!saw_digit)
      {
        if (++octets > 4)
        {
          LOG_MSG_INFO2_1("ps_dns_cachei_pton4(): Number of octets %d greater then 4.  "
                          "Return DS_NAMEERR", octets );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
    ...
    else
    {
      if (ch == '.' && saw_digit)
      {
        if (octets == 4)
        {
          LOG_MSG_INFO2_0("ps_dns_cachei_pton4(): '.' found after 4 octets.  Return "
                          "DS_NAMEERR" );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
      ...
      {
        LOG_MSG_INFO2_2("ps_dns_cachei_pton4(): %c found in octet %d no digits seen.  "
  ...
  if (octets < 4)
  {
    LOG_MSG_INFO2_1("ps_dns_cachei_pton4(): Only %d octets found. Return DS_NAMEERR",
                    octets );
    *dss_errno = DS_NAMEERR;
    return DSS_ERROR;
  }
...
===========================================================================*/
static int ps_dns_cachei_pton6
(
  const char    *src,
  unsigned char *dst,
  int16         *dss_errno
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (*src == ':')
  {
    if (*++src != ':')
    {
      LOG_MSG_INFO2_0("ps_dns_cachei_pton6(): Address begins with single ':'.  Return "
                      "DS_NAMEERR" );
      *dss_errno = DS_NAMEERR;
      return DSS_ERROR;
    }
  ...
  while ((ch = *src++) != '\0')
  {
    ...
    if (pch != NULL)
    {
      ...
      if (++seen_xdigits > 4)
      {
        LOG_MSG_INFO2_1("ps_dns_cachei_pton6(): %d digits seen.  Return DS_NAMEERR",
                        seen_xdigits );
        *dss_errno = DS_NAMEERR;
        return DSS_ERROR;
      }
    ...
    if (ch == ':')
    {
      ...
      if (!seen_xdigits)
      {
        if (colonp)
        {
          LOG_MSG_INFO2_0("ps_dns_cachei_pton6(): Duplicate :: found.  Return DS_NAMEERR" );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
        colonp = tp;
      ...
      else
      {
        if (*src == '\0')
        {
          LOG_MSG_INFO2_0("ps_dns_cachei_pton6(): ':' found at end of address.  "
                          "Return DS_NAMEERR" );
          *dss_errno = DS_NAMEERR;
          return DSS_ERROR;
        }
      ...
      {
        LOG_MSG_INFO2_2("ps_dns_cachei_pton6(): Insufficient memory.  Need %d octets, "
    ...
    if (ch == '.' && ((tp + sizeof(struct ps_in_addr)) <= endp) &&
        ps_dns_cachei_pton4(curtok, tp, dss_errno) == DSS_SUCCESS)
    {
    ...
    LOG_MSG_INFO2_3("ps_dns_cachei_pton6(): "
  ...
  if (colonp != NULL)
  {
    ...
    if (tp == endp)
    {
      LOG_MSG_INFO2_0("ps_dns_cachei_pton6(): Insufficient memory.  Return DS_EMSGTRUNC" );
      *dss_errno = DS_EMSGTRUNC;
      return DSS_ERROR;
    }
  ...
  if (tp != endp)
  {
    LOG_MSG_INFO2_2("ps_dns_cachei_pton6(): Out of %d octets, processed %d octets.  "
...
SIDE EFFECTS
  ...
  Any errors are returned in dss_errno argument.
===========================================================================*/
static int32 ps_dns_cachei_pton
(
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (dss_errno == NULL)
  {
    LOG_MSG_INFO2_0("ps_dns_cachei_pton(): Called with dss_errno NULL" );
    return DSS_ERROR;
  }

  if (src == NULL || dst == NULL)
  {
    LOG_MSG_INFO2_2("ps_dns_cachei_pton(): Called with src %p, dst %p",
                    src, dst );
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  {
    LOG_MSG_INFO2_2("ps_dns_cachei_pton(): dst_size %d too small for af %d",
                    dst_size, af );
    *dss_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  {
  case DSS_AF_INET:
    retval = ps_dns_cachei_pton4(src, dst, dss_errno);
    break;

  case DSS_AF_INET6:
    retval = ps_dns_cachei_pton6(src, dst, dss_errno);
    break;
...
/*===========================================================================
FUNCTION PS_DNS_CACHEI_ADD_TRAILING_DOT_TO_HOSTNAME
...
SIDE EFFECTS
  None.
===========================================================================*/
void ps_dns_cachei_add_trailing_dot_to_hostname
(
  char *hostname
)
{
  ...
  retval = ps_dns_cachei_pton( hostname,
                                     DSS_AF_INET,
                                     &temp_in_addr,
  ...
  if( DSS_ERROR == retval )
  {
    ...
    if (0 == query_len)
    {
      DATA_ERR_FATAL( "ps_dns_cachei_add_trailing_dot_to_hostname(): length of hostname is zero");
      return;
    }
...
} /* ps_dns_cachei_add_trailing_dot_to_hostname() */

/*===========================================================================
STATIC FUNCTION PS_DNS_CACHEI_FLUSH_CACHE_CMD_HANDLER
...
SIDE EFFECTS
  None.
===========================================================================*/
static void ps_dns_cachei_flush_cache_cmd_handler
(
...
)
{
  ...
  LOG_MSG_INFO2_3 ("ps_dns_cachei_flush_cache_cmd_handler()",
                 "Processing iface event command %d fwk inst resolver 0x%x",
                 ps_cmd, fwk_inst_ptr);

  if (PS_DNS_FLUSH_CACHE_CMD != ps_cmd )
  {
    LOG_MSG_ERROR_2 ("ps_dns_cachei_flush_cache_cmd_handler()",
                   "Unexpected iface command %d", ps_cmd);
    return;
  }

  if (NULL == fwk_inst_ptr)
  {
    LOG_MSG_ERROR_1 ("ps_dns_cachei_flush_cache_cmd_handler()",
                   "NULL user data");
    return;
  }

  fwk_inst = (ds_fwk_s_type*)fwk_inst_ptr;
  LOG_MSG_INFO1_1("ps_dns_cachei_flush_cache_cmd_handler(): "
                  "Flushing the cache for fwk inst %p",
                  fwk_inst);

  DS_ENTER_CRIT_SECTION(&global_ps_crit_section);


  while( NULL != (cache_entry = q_get( &(fwk_inst->dns_cache_q) ) ) )
  {
    ps_dns_cachei_free_entry( cache_entry );
  }
  DS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
  return;
} /* ps_dns_cachei_flush_cache_cmd_handler() */

/*===========================================================================
STATIC FUNCTION PS_DNS_CACHEI_FLUSH_CACHE_ENTRY_CMD_HANDLER
...
SIDE EFFECTS
  None.
===========================================================================*/
static void ps_dns_cachei_flush_cache_entry_cmd_handler
(
  ps_cmd_enum_type                      ps_cmd,
...
)
{
  ...
  {
    LOG_MSG_ERROR_2 ("ps_dns_cachei_flush_cache_entry_cmd_handler()",
                   "Unexpected iface command %d", ps_cmd);
    return;
  }
  ...
  {
    LOG_MSG_ERROR_1 ("ps_dns_cachei_flush_cache_entry_cmd_handler()",
                   "NULL user data");
    return;
  }
  ...
  fwk_inst = (ds_fwk_s_type*) cmd_entry_ptr->fwk_inst;
  
  LOG_MSG_INFO2_3 ("ps_dns_cachei_flush_cache_entry_cmd_handler()",
                 "Processing iface event command %d iface resolver 0x%x",
                 ps_cmd, cmd_entry_ptr);
                  
  if (fwk_inst == NULL)
  {
    LOG_MSG_ERROR_0("ps_dns_cachei_flush_cache_entry_cmd_handler(): "
                    "Invalid fwk instance, is passed");
    goto bail;
  }
  ...
  LOG_MSG_INFO1_1("ps_dns_cachei_flush_cache_entry_cmd_handler(): "
                  "Flushing cache entry for fwk inst %p",
                  fwk_inst);
  ...
  cache_entry = q_check( &(fwk_inst->dns_cache_q) );
  while( NULL != cache_entry )
  {
    next_cache_entry = q_next( &(fwk_inst->dns_cache_q),
                               &(cache_entry->link) );
    if(
    ...
    {/*The entry match so free it*/
      LOG_MSG_INFO2_0("ps_dns_cachei_flush_cache_entry_cmd_handler(): "
                      "Found a match");
      ps_dns_cachei_free_entry( cache_entry );
...
bail:
  ...
  DS_SYSTEM_HEAP_MEM_FREE(cmd_entry_ptr);
} /* ps_dns_cachei_flush_cache_entry_cmd_handler() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR_0("ps_dns_cache_init(): "
...
/*===========================================================================
FUNCTION  PS_DNS_CACHE_ADD_RR_Q()
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2_3("ps_dns_cache_add_rr_q(): "
                  "Adding qtype %d, qclass %d, qdata 0x%p to cache",
                  query_type, query_class, query_data_ptr);

  LOG_MSG_INFO2_3("ps_dns_cache_add_rr_q(): "
                  "Adding qtype %d, qclass %d, to cache for fwk %p",
                  query_type,
                  query_class,
                  fwk_inst);


  if( NULL == ps_errno || fwk_inst == NULL)
  {
    DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): NULL errno");
    return DSS_ERROR;
  }
  ...
  if( PS_DNSI_QUERY_TYPE_MAX  <= query_type   ||
      ...
      NULL == query_data_ptr )
  {
    DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): Invalid arguments");
    *ps_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  if( DSS_ERROR == ps_dns_cache_getconst( PS_DNS_CACHE_CONST_MAX_ENTRIES,
                                          ...
                                          sizeof(int32) ) )
  {
    DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): Can't get const for max cache entries");
    *ps_errno = DS_EINVAL;
    return DSS_ERROR;
  }
  /*-------------------------------------------------------------------------
    ...
    if( cache_entry->query_type  == (uint16)query_type      &&
        ...
        ((0 == strncasecmp( cache_entry->query_data,
         ...
         (0 == strncasecmp( cache_entry->cname,
                             ...
                             PS_DNSI_MAX_DOMAIN_NAME_LEN ))) )
    {
      LOG_MSG_INFO2_0("ps_dns_cache_add_rr_q(): "
                      "Found a cache match");
      in_cache = TRUE;
      break;
    } /* if */

    cache_entry = q_next( &fwk_inst->dns_cache_q,
                          &(cache_entry->link) );
  }
  ...
  if (TRUE == in_cache)
  {
    ...
    {
      LOG_MSG_ERROR_1("ps_dns_cache_add_rr_q(): "
...
#ifdef FEATURE_Q_NO_SELF_QPTR
    q_delete( &fwk_inst->dns_cache_q, &( cache_entry->link ) );
#else
    q_delete( &( cache_entry->link ) );
#endif

    q_put( &fwk_inst->dns_cache_q, &( cache_entry->link ) );
  ...
  if( NULL != rr_q ) {
    ...
    if( DSS_ERROR == ps_dns_cache_getconst( PS_DNS_CACHE_CONST_MAX_TTL,
                                            &cache_max_ttl,
                                            sizeof(uint32) ) ) {
      DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): Can't get const for max cache TTL");
      ps_dns_cachei_free_entry( cache_entry );
      result = DSS_ERROR;
      goto bail;
    }
    ...
    if( DSS_ERROR == ps_dns_cache_getconst( PS_DNS_CACHE_CONST_MIN_TTL,
                                            &cache_min_ttl,
                                            sizeof(uint32) ) ) {
      DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): Can't get const for min cache TTL");
      ps_dns_cachei_free_entry( cache_entry );
      result = DSS_ERROR;
      goto bail;
    }
  ...
  if( cache_max_entries <= q_cnt( &fwk_inst->dns_cache_q ) )
  {
     LOG_MSG_INFO1_1("ps_dns_cache_add_rr_q(): "
                     "Cache on iface %x full, oldest entry removed",
                     fwk_inst);

     // Get the first cache entry from the queue and remove it.
     ps_dns_cachei_free_entry( q_check( &fwk_inst->dns_cache_q ) );
  }
  ...
  /*-------------------------------------------------------------------------
    ...
    ps_dns_cachei_free_entry the specific cache can be
    determined (there is a cache per iface_id).
    -------------------------------------------------------------------------*/
  cache_entry->fwk_inst = fwk_inst;
  /*-------------------------------------------------------------------------
    As ps_dns_cachei_free_entry() dequeues a node from ps_dnsi_cache_q,
    cache_entry is enqueued before hand so that ps_dns_cachei_free_entry()
  ...
  -------------------------------------------------------------------------*/
  cache_entry->pstimer = ps_timer_alloc( ps_dns_cachei_free_entry,
                                         (void *) cache_entry );

  if( (uint32)PS_TIMER_INVALID_HANDLE == cache_entry->pstimer )
  {
    LOG_MSG_ERROR_0("ps_dns_cache_add_rr_q(): "
  ...
  if( NULL == rr_q)
  {
    if( DSS_ERROR == ps_dns_cache_getconst( PS_DNS_CACHE_CONST_NEGATIVE_TTL,
                                            ...
                                            sizeof(int32) ) )
    {
      DATA_ERR_FATAL("ps_dns_cache_add_rr_q(): Can't get const for negative cache TTL");
      ps_dns_cachei_free_entry( cache_entry );
      result = DSS_ERROR;
      goto bail;
    }
  ...
  else
  {
    ...
    if( DSS_ERROR ==
          ps_dns_cachei_dup_rr_q( &(cache_entry->rr_q), rr_q, ps_errno ) )
    {
      LOG_MSG_ERROR_0("ps_dns_cache_add_rr_q(): "
                      "Couldn't dup rr_q");
      ps_dns_cachei_free_entry( cache_entry );
...
/*===========================================================================
FUNCTION  PS_DNS_CACHE_FIND_RR_Q()
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2_3("ps_dns_cache_find_rr_q(): "
                  "Finding qtype %d, qclass %d, qdata 0x%p in cache",
                  query_type, query_class, query_data_ptr);
  LOG_MSG_INFO2_3("ps_dns_cache_find_rr_q(): "
                  "Finding qtype %d, qclass %d, in cache %p",
                  query_type,
                  query_class,
                  fwk_inst);
  ...
  if( NULL == ps_errno || fwk_inst == NULL)
  {
    LOG_MSG_ERROR_0("ps_dns_cache_find_rr_q(): NULL errno");
    return DSS_ERROR;
  }

  if( NULL == rr_q )
  {
    LOG_MSG_ERROR_0("ps_dns_cache_find_rr_q(): NULL rr_q");
    *ps_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  if( PS_DNSI_QUERY_TYPE_MAX  <= query_type   ||
      ...
      NULL == query_data_ptr )
  {
    LOG_MSG_ERROR_0("ps_dns_cache_find_rr_q(): Invalid arguments");
    *ps_errno = DS_EFAULT;
    return DSS_ERROR;
  }
  ...
  while( NULL != cache_entry )
  {
    if( cache_entry->query_type  == (uint16)query_type              &&
        ...
        ((0 == strncasecmp( cache_entry->query_data,
         ...
         (0 == strncasecmp( cache_entry->cname,
                             ...
                             PS_DNSI_MAX_DOMAIN_NAME_LEN ))) )
    {
      LOG_MSG_INFO2_0("ps_dns_cache_find_rr_q(): "
                      "Found a cache match");
      if( DSS_SUCCESS !=
            ps_dns_cachei_dup_rr_q( rr_q, &cache_entry->rr_q, ps_errno ) )
      {
        LOG_MSG_ERROR_1("ps_dns_cache_find_rr_q(): "
    ...
    cache_entry = q_next( &fwk_inst->dns_cache_q, &(cache_entry->link) );
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1_1("ps_dns_cache_flush(): "
                  "Flushing the cache for fwk %p",
                  fwk_inst);

  if( NULL == ps_errno || fwk_inst == NULL)
  {
     DATA_ERR_FATAL("ps_dns_cache_flush(): NULL errno");
     return DSS_ERROR;
  }

  ps_dns_cachei_flush_cache_cmd_handler(PS_DNS_FLUSH_CACHE_CMD,
                                        fwk_inst );
...
SIDE EFFECTS
  None.
===========================================================================*/
int16 ps_dns_cache_flush_entry
(
  ds_fwk_s_type                  *fwk_inst,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( NULL == ps_errno  || fwk_inst == NULL)
  {
    DATA_ERR_FATAL("ps_dns_cache_flush_entry(): NULL errno");
    return DSS_ERROR;
  }

  if( NULL == hostname_ptr )
  {
    DATA_ERR_FATAL("ps_dns_cache_flush_entry(): NULL hostname_ptr");
    *ps_errno = DS_EINVAL;
    return DSS_ERROR;
  }

  DS_SYSTEM_HEAP_MEM_ALLOC(cmd_entry_ptr, sizeof(ps_dns_flush_cache_entry_cmd_type),
                           ps_dns_flush_cache_entry_cmd_type*);
  ...
  if (0 == strlen(cmd_entry_ptr->hostname))
  {
    LOG_MSG_ERROR_0("ps_dns_cache_flush_entry(): "
  ...
  ps_dns_cachei_add_trailing_dot_to_hostname(cmd_entry_ptr->hostname);
  cmd_entry_ptr->fwk_inst= fwk_inst;
