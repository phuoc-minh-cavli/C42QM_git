...
$Header: //components/rel/rftech_lte.mpss/2.10/rf/rfc/src/rfc_lte_data.cpp#1 $
...
boolean rfc_lte_data::iotfe_timing_cfg_data_get( rfc_cfg_params_type *cfg, rfc_timing_info_type **ptr)
{
  if (NULL != ptr)
  {
    *ptr = NULL;
  ...
  MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "LTE RFC RFC IOTFE timing info data NOT available", 0);
  return FALSE;
}
...
boolean rfc_lte_data_get_iotfe_timing_info(rfc_cfg_params_type *cfg, rfc_timing_info_type **rfc_timing_info_pptr)
{
   rfc_lte_data *lte_data_ptr = rfc_lte_data::get_instance();
   ...
   if( lte_data_ptr->iotfe_timing_cfg_data_get(cfg,rfc_timing_info_pptr) == FALSE)
   {
     return FALSE;
   }
