...
$Header: //components/rel/rftech_lte.mpss/2.10/rf/rfc/src/rfc_card_lte.cpp#1 $
...
void rfc_lte_manage_vregs(rfm_device_enum_type device, rfcom_lte_band_type band, rfm_path_state state)
{
  ...
  rflte_pm_info.pwr_cls  = RFC_RF_POWER_CLASS_DEFAULT;
  boolean lte_nb_mode = rflte_mc_state.lte_nb_mode;

#ifndef FEATURE_RF_LTE_PAGE_ONLY
  if( lte_nb_mode == TRUE )
  {
    rflte_pm_info.pwr_cls = (rfc_rf_pwr_cls)rflte_nb_mc_get_ue_power_class_per_band(band);
	MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH," NB mode power class %d",rflte_pm_info.pwr_cls);  
  }
  else
  {
	rflte_pm_info.pwr_cls = (rfc_rf_pwr_cls)rflte_mc_get_ue_power_class_per_band(band);
	MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH," CATM1 mode power class %d",rflte_pm_info.pwr_cls);  
  }
#endif
...
rfdevice_trx_common_intf_class * rfc_lte_get_common_rxtx_device
(
  rfm_device_enum_type device, 
  rfcom_lte_band_type band,
  uint32 alt_path
