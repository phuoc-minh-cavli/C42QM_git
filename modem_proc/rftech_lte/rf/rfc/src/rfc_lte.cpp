...
$Header: //components/rel/rftech_lte.mpss/2.10/rf/rfc/src/rfc_lte.cpp#1 $
...
void rfc_lte_intf::manage_vregs(rfc_power_management_info* pm_info)
{
  ...
  vreg_param.set_pwr_class(pm_info->pwr_cls);
...
void rfc_lte_intf::init_rf_devices(rfc_cfg_params_type *cfg, rfc_device_info_type *device_info_ptr)
{
  ...
  rfdevice_trx_lte_rx_intf_class *rx_dev_obj = NULL;
  rfdevice_trx_lte_tx_intf_class *tx_dev_obj = NULL;
  rfdevice_trx_phy_path_enum_type phy_path= RFDEVICE_TRX_MAX_PATHS;
  uint8 ant_num = 0;
  rfdevice_trx_tech_type trx_tech = RFDEVICE_TRX_STD_INVALID;
...
#if 0
  if (rflte_util_is_band_tdd(band_u.lte_band))
  {
    trx_tech = RFDEVICE_TRX_STD_LTETDD;
  }
  else
  {
    trx_tech = RFDEVICE_TRX_STD_LTEFDD;
  }
#endif
  trx_tech = RFDEVICE_TRX_STD_LTE;
  ...
  while (i < device_info_ptr->num_devices)
  {
    ...
    switch(dev_type)
    {
        ...
        if (RFC_CONFIG_RX == cfg->rx_tx) /* rx device data */
        {
          if (trx_dev_instance < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
          {
            ...
            rx_dev_obj = 
              ...
              ((rfdevice_trx_common_intf_class*)cmn_dev_obj)\
                    -> get_tech_instance( trx_tech,
                                          RFDEVICE_RX);
      ...
      case RFDEVICE_COUPLER:
          ...
          case RFDEVICE_THERM_MITIGATION:
             if (cfg->rx_tx == RFC_CONFIG_TX)
             {
                if (therm_dev_instance < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
                {
                   lte_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][dev_type][therm_dev_instance] =\
                      cmn_dev_obj;
                   therm_dev_instance++;
                }
                else
                {
                   MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "PA THERM device instance overflow therm_dev_instance:"
                         " %d", therm_dev_instance);
                }
             }
