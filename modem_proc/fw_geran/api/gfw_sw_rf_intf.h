...
$Header: //source/qcom/qct/modem/fw/components/fw_geran.mpss/rel/15.2/api/gfw_sw_rf_intf.h#1 $
...
// --------------------------------------------------------------------------
// FSK Tx Start, Transition and Stop buffer definitions
// --------------------------------------------------------------------------

typedef struct GfwStartFskTxBufType
{
  uint32 rflmStart;
  boolean ccsStartIndexUsed;
  uint32 rfSeqNr;
...
typedef struct GfwTransFskTxBufType
{
  uint32 rflmTrans[MDSP_MAX_NUM_TRANS_BUF]; //finalize # buffers needed, for 16bpf time gap might be small, so RF might need 2 trans
} GfwTransFskTxBuf;


typedef struct GfwStopFskTxBufType
{
  uint32 rflmStop;
  boolean ccsStopIndexUsed;
} GfwStopFskTxBuf;
// --------------------------------------------------------------------------
// FSK Tx Event definition
// --------------------------------------------------------------------------

typedef struct tag_GfwFskTxEvent
{
  GfwStartFskTxBuf start;
  GfwTransFskTxBuf trans[2]; //finalize # buffers needed
  GfwStopFskTxBuf  stop;
} GfwFskTxEvent;
...
typedef struct tag_GfwFskRfBurstEvent
{
   GfwFskTxEvent fskTxEvent;
} GfwFskRfBurstEvent;

// --------------------------------------------------------------------------
// FSK Enter Mode Event definition
// --------------------------------------------------------------------------
typedef struct tag_GfwFskEnterModeEvent
{
   uint32 rflmFskEnterMode;
} GfwFskEnterModeEvent;

typedef union tag_GfwFskRfInitEvent
{
    GfwFskEnterModeEvent fskTxEnterMode;
    GfwWakeUpCcsEvent wakeUp;  
} GfwFskRfInitEvent;
...
// GFW RF events table containing all possible RF events
// --------------------------------------------------------------------------
typedef struct tag_GfwFskRfEventTable
{
    GfwFskRfBurstEvent burstEvent;
	GfwFskRfInitEvent fskRfInitEvent;
    GfwRfSetTxBandEvent setTxBandEvent;
} GfwFskRfEventTable;
