...
$Header: //components/rel/rftech_gnss.mpss/2.10/gen8/rf/mc/src/rfgnss_mc.c#1 $
...
rf_hal_bus_client_handle* rfgnss_hal_bus_client_handle;
...
int32 rfgnss_mc_configure ( rfcom_device_enum_type device, rfgnss_mode_cfg_enum_type rfgnss_mode )
{
  ...
  // vote for RFCMD app
  rf_hal_bus_enable(TRUE, &rfgnss_hal_bus_client_handle, RF_HAL_RFCMD_APP_CLIENT(GPS));
...
int32 rfgnss_mc_exit ( rfcom_device_enum_type device )
{
  ...
  rf_hal_bus_enable(FALSE, &rfgnss_hal_bus_client_handle, RF_HAL_RFCMD_APP_CLIENT(GPS));
...
{
  return rf_hal_bus_enable(enable, &rfgnss_hal_bus_client_handle, RF_HAL_RFCMD_APP_CLIENT(GPS));
}
/*----------------------------------------------------------------------------*/
/*!
   @brief
