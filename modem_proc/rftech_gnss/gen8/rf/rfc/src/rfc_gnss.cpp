...
$Header: //components/rel/rftech_gnss.mpss/2.10/gen8/rf/rfc/src/rfc_gnss.cpp#1 $
...
rfc_gnss::rfc_gnss()
{
  ...
  uint8 dev_type = 0;
  // initialize the gnss_rf_devices
  for(dev_type = 0; dev_type < RFDEVICE_TYPE_MAX_NUM; dev_type++)
  {
  	...
  	{
      gnss_rf_devices[dev_type][dev_instance] = NULL;
  	}
  }
...
void rfc_gnss::init_rf_devices()
{
  ...
  uint8 trx_device_instance = 0;
  uint8 asm_device_instance = 0;
  uint8 asm_port_temp_state = RFC_PORT_STATE_DEFAULT;
  ...
  gnss_band.gnss_band = RFCOM_BAND_GNSS_L1;
  i = 0;
  local_instance = 0;
  while (i < device_info->num_devices && local_instance < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
  {
    ...
    if(cmn_dev_obj == NULL)
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfc_gnss::init_rf_devices: Null device! Type is %d", dev_type);
      i++;
      continue;
    }
    ...
    switch(dev_type)
      ...
      case RFDEVICE_TRANSCEIVER:
        gnss_rf_devices[dev_type][trx_device_instance] = cmn_dev_obj;
		trx_device_instance++;
        break;
		
	  case RFDEVICE_ASM:
	    gnss_rf_devices[dev_type][asm_device_instance] = cmn_dev_obj;
		asm_device_instance++;
		
		/* Update the Rx ASM Port state with temp_port_state and Call the ASM API below */
  	    ((rfdevice_asm *)cmn_dev_obj)->set_rx_switch_state(RFCOM_GNSS_MODE, gnss_band, asm_port_temp_state);
                                             ...
                                             uint8 instance)
{
  ...
  if (dev_type >= RFDEVICE_TYPE_MAX_NUM || instance >= RFC_MAX_DEVICE_INSTANCES_PER_BAND)
  {
  ...
  dev_obj = gnss_rf_devices[dev_type][instance];
  return dev_obj;
}
