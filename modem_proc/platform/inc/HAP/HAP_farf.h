...
#define _FARF_1(x, ...) \
    do { \
        if(0 == (HAP_debug_v2)) { \
            ...
            void (*HAP_debug_v2_ptr)(int level, const char* file, int line, const char* format, ...); \
            __asm__ __volatile__("%0 = %1" : "=r"(HAP_debug_v2_ptr) : "r"(HAP_debug_v2)); \
            HAP_debug_v2_ptr(FARF_##x##_LEVEL, __FILENAME__, __LINE__, ##__VA_ARGS__); \
        } \
      } while (0)
