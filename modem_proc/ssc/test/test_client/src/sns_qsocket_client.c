...
#define SNS_CLIENT_THREAD_STACK_SIZE 2048
...
typedef struct sns_client_request
{
  struct sns_client_request *next;
...
typedef struct sns_qsocket_state
{
  ...
  /* Protects all state */
  struct sns_test_osa_mutex *mutex;

  /* Ensures qpoll() is not called when no clients are present */
  struct sns_test_osa_sem *sem;
  ...
  size_t new_fds_len;
  /* Whether new_fds has been updated and needs to be applied */
  bool new_fds_valid;
} sns_qsocket_state;
...
static uint8_t thread_stack[SNS_CLIENT_THREAD_STACK_SIZE];
 ...
 */
static sns_client_request*
get_request(sns_client *client, uint32_t request_id)
{
  sns_client_request *last = NULL, *request = NULL;
  bool found = false;

  sns_test_osa_mutex_lock(qsocket_state->mutex);
  ...
  sns_test_osa_mutex_unlock(qsocket_state->mutex);
  return request;
}
    ...
    void *resp_cb_data)
{
  ...
  request = sns_test_osa_malloc(sizeof(*request));
...
handle_resp(sns_client *client, void *msg, uint32_t msg_len)
{
  ...
  {
    sns_client_request *request = get_request(client, resp.request_id);

    if(NULL != request)
    {
      if(NULL != request->resp_cb)
        request->resp_cb(client, resp.error, request->resp_cb_data);
      sns_test_osa_free(request);
    }
  }
}
...
main_thread(void *arg)
{
  ...
  sns_test_osa_mutex_lock(state->mutex);
  ...
  for(;;)
  {
    ...
    {
      sns_test_osa_mutex_unlock(state->mutex);
      sns_test_osa_sem_wait(state->sem);
      sns_test_osa_mutex_lock(state->mutex);
    }

    if(qsocket_state->new_fds_valid)
    {
      sns_test_osa_free(state->fds);
      state->fds = state->new_fds;
      state->fds_len = state->new_fds_len;

      state->new_fds = NULL;
      state->new_fds_len = 0;
      qsocket_state->new_fds_valid = false;
    }

    // If the only fd was just removed
    if(0 == state->fds_len)
      continue;
...
lookup_server_addr(uint32_t timeout, struct qsockaddr_ipcr *srv_addr)
{
  ...
  else
  {
    ...
    while(0 == rv)
    {
      ...
      if(rc > 0)
      {
        SNS_LOG(INFO, "Got service with addr <%" PRIu32 ",%" PRIu32 ">",
...
one_time_init(void)
{
  if(NULL == qsocket_state)
  {
    ...
    qsocket_state = sns_test_osa_malloc(sizeof(*qsocket_state));
    ...
    err = sns_test_osa_mutex_create(&qsocket_state->mutex);
    SNS_ASSERT(0 == err);

    err = sns_test_osa_sem_create(&qsocket_state->sem);
    SNS_ASSERT(0 == err);

    err = sns_test_osa_thread_create(&qsocket_state->thread, "QSOCKET_CLIENT",
        &main_thread, qsocket_state, &thread_stack[0], sizeof(thread_stack));
    SNS_ASSERT(0 == err);
  }
}
    ...
    sns_client_error error_cb, void *error_cb_data)
{
  ...
  {
    sns_test_osa_mutex_lock(qsocket_state->mutex);
    client = sns_test_osa_malloc(sizeof(*client));
    SNS_ASSERT(NULL != client);
    ...
    else
    {
      ...
      new_fds = sns_test_osa_malloc(sizeof(struct qpollfd) * (start_len + 1));
      ...
      qsocket_state->new_fds_valid = true;

      sns_test_osa_free(qsocket_state->new_fds);
      qsocket_state->new_fds = new_fds;

      if(0 == start_len)
        sns_test_osa_sem_post(qsocket_state->sem);
      else
...
sns_client_deinit(sns_client *client)
{
  ...
  while(NULL != request)
  {
    sns_client_request *temp = request;
    request = request->next;
    sns_test_osa_free(temp);
  }
  client->requests = NULL;
    ...
    sns_client_resp resp_cb, void *resp_cb_data)
{
  ...
  {
    void *encoded_msg = sns_test_osa_malloc(encoded_len);
    pb_ostream_t stream;
