...
/* File used to synchronize SSC and HLOS parsing of above file */
static char registry_config_ctrl_file[SNS_REGISTRY_DIR_LEN_MAX] = "/persist/sensors/registry/sns_reg_ctrl";
...
extern char temp_file_name[SNS_REGISTRY_DIR_LEN_MAX];
...
set_item_data(sns_registry_item *item, char const *value)
{
  ...
  if(SNS_REGISTRY_TYPE_INTEGER == item->type)
  {
    ...
    if(0 != errno)
      SNS_SPRINTF(ERROR, sns_fw_printf, "stroll error %i (%s)",
...
update_item(sns_registry_item *item, sns_registry_item const *new)
{
  ...
  {
    /* If the previous item was a string free the memory */
...
check_dir(char const *dir)
{
  ...
  {
    /* checking errno == 0 is a workaround to fix the directory is not
       created for windows platform if errno is defined in errno.h and
       ENOENT is 2 (standard) but, errno is not set properly (to ENOENT)
       in fasRPC platform_fs */
    if(ENOENT == errno || 0 == errno) // If the directory does not exist
    {
    ...
    sns_registry_hw const hw_info[], uint32_t hw_info_len)
{
  ...
  uint32_t len;
  char *key, *value;
  char *buf = sns_malloc(SNS_HEAP_MAIN, SNS_REGISTRY_DIR_LEN_MAX);
  SNS_ASSERT(NULL != buf);
  ...
  else
  {
    do
    {
      ...
      else
      {
        ...
        /*properties must start with persist.vendor*/
        if((strlen(key) > strlen("persist.vendor.")) &&
            (0 == strncmp(key, "persist.vendor.", strlen("persist.vendor."))))
          {
            /*read the property*/
            sns_registry_get_property(key, buf, SNS_REGISTRY_DIR_LEN_MAX);
            hw_value = buf;
            found = true;
          }
 ...
 */
static sns_rc
parse_file(char *json, uint32_t json_len, sns_list_iter *groups, bool *is_config)
{
  uint32_t cidx = 0;
  uint32_t len;
  char *key, *value;
    ...
    do
    {
      ...
      if(0 == strncmp(key, "config", strlen(key)))
      {
        if(0 != dynamic_hw_info_len)
          len = parse_config(&json[cidx], json_len - cidx, dynamic_hw_info,
              dynamic_hw_info_len);
        else
    ...
    size_t file_size, bool is_config)
{
  ...
  else
  {
    ...
    else
    {
      ...
      else if(SNS_RC_SUCCESS != rc || 0 == sns_list_iter_len(&groups_iter))
      {
        SNS_SPRINTF(ERROR, sns_fw_printf,
      ...
      else if(is_config)
      {
            ...
            NULL != sns_list_iter_curr(&groups_iter);)
        {
          sns_registry_group *temp_group = (sns_registry_group*)
            sns_list_item_get_data(sns_list_iter_remove(&groups_iter));

          rv |= sns_registry_group_merge(temp_group);
          sns_registry_group_free(temp_group);
        }
      }
      else if(1 == sns_list_iter_len(&groups_iter))
      {
        sns_registry_group *temp_group = (sns_registry_group*)
          sns_list_item_get_data(sns_list_iter_remove(&groups_iter));
...
parse_device_hw_info(char *config_contents)
{
  ...
  for(token_idx = 0; MAX_CONFIG_ENTRIES > token_idx; token_idx++)
  {
    ...
    SNS_SPRINTF(LOW, sns_fw_printf, "%s", config_line);
  ...
  if(MAX_CONFIG_ENTRIES == token_idx)
    SNS_SPRINTF(ERROR, sns_fw_printf, "dynamic_hw_info exceeded length");
  ...
  for(int i = 0; i < sns_registry_hw_info_len; i++)
  {
    ...
    if(config[i].value != NULL) {
      SNS_SPRINTF(LOW, sns_fw_printf, "HW Config: '%s' - '%s'",
        ...
        SNS_PRINTF(ERROR, sns_fw_printf, "registry: fail to read file");
    }
  }
}
...
{
  update_persist_mnt_path(temp_file_name);
  update_persist_mnt_path(registry_config_file);
  update_persist_mnt_path(registry_config_ctrl_file);
}
...
sns_registry_group_save(sns_registry_group *group)
{
  ...
  do
  {
    ...
    else
    {
      ...
      if(save_registry_file(temp_file_name, json, cidx))
      {
        errno = 0;
        if(0 != rename(temp_file_name, full_path))
          SNS_PRINTF(ERROR, sns_fw_printf, "rename error %i", errno);
        else
          rv = SNS_RC_SUCCESS;
...
sns_registry_group_free(sns_registry_group *group)
{
  ...
  {
    sns_registry_item *item = (sns_registry_item*)
      sns_list_item_get_data(sns_list_iter_remove(&iter));
...
sns_registry_item*
sns_registry_item_find(sns_registry_group const *group, char const *item_name)
{
  sns_list_iter iter;
