...
sns_reg_init_task(void *arg)
{
  ...
  /* Bumps up clock untill registry init complete;
   * avoid the boottime regression */
  sns_scpm_mgr_client *scpm_client = sns_pwr_scpm_mgr_create_client();
  sns_pwr_scpm_mgr_mcps_bump_up(scpm_client);

  sns_pwr_sleep_mgr_set_latency((void *)this,
      SNS_REGISTRY_APPS_AWAKE_LATENCY_USEC);
 ...
 */
static sns_rc
handle_write_req(sns_sensor_instance *instance, char const *name,
    pb_buffer_arg const *payload)
{
  ...
  else
  {
    ...
    {
      rv = sns_registry_group_merge(group);
    }
    sns_registry_group_free(group);
  }

  write_event.status = SNS_RC_SUCCESS == rv ? SNS_REGISTRY_WRITE_STATUS_SUCCESS
    : SNS_REGISTRY_WRITE_STATUS_ERROR_OTHER;

  pb_send_event(instance, sns_registry_write_event_fields, &write_event,
      sns_get_system_time(), SNS_REGISTRY_MSGID_SNS_REGISTRY_WRITE_EVENT,
      NULL);
  return rv;
}
    ...
    if(pb_decode(&stream, sns_std_request_fields, &request))
    {
      ...
      else if(SNS_REGISTRY_MSGID_SNS_REGISTRY_WRITE_REQ == new_req->message_id)
      {
        ...
        if(NULL == name)
          ...
          handle_write_req(inst, name, &payload);
      }
    }
    else
