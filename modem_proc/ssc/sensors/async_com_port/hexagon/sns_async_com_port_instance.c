...
{
  sns_list_iter iter;
  sns_list_iter_init(&iter, &state->pending_transactions, true);
  return sns_list_iter_len(&iter);
}
                                       ...
                                       sns_sensor_instance* const this)
{
  ...
  sns_list_iter_init(&iter, &state->pending_transactions, false);
  sns_list_item_init(&tx->list_entry, tx);
  sns_list_iter_insert(&iter, &tx->list_entry, true);
}
                                                     ...
                                                     sns_async_com_port_instance_state *state)
{
  sns_list_iter iter;
  uint8_t i;
  sns_list_item *curr_pending_tx = NULL;
  for(curr_pending_tx = sns_list_iter_init(&iter, &state->pending_transactions, true);
    NULL != curr_pending_tx;
    curr_pending_tx = sns_list_iter_advance(&iter))
  {
    if( tx == sns_list_item_get_data(curr_pending_tx))
    {
 ...
 sns_async_com_port_start_next_transaction(sns_sensor_instance *const this)
{
  ...
  sns_list_iter_init(&iter, &state->pending_transactions, true);
  sns_async_com_port_queued_transaction *tx =
    (sns_async_com_port_queued_transaction *)sns_list_iter_get_curr_data(&iter);
...
sns_async_com_port_inst_init(sns_sensor_instance* const this, sns_sensor_state const *sensor_state)
{
  ...
  sns_list_init(&state->pending_transactions);
...
sns_async_com_port_inst_deinit(sns_sensor_instance* const this)
{
  ...
  {
    state->current_tx = sns_list_iter_get_curr_data(&iter);
    sns_async_com_port_send_error(this, SNS_RC_FAILED);
  ...
  if(SNS_ASYNC_COM_PORT_MSGID_SNS_ASYNC_COM_PORT_CONFIG == client_request->message_id)
  {
    ...
    if(pb_decode_request(client_request, sns_async_com_port_config_fields, NULL, &async_port_config))
    {
      ...
      if( SNS_RC_SUCCESS == ret_val)
      {
        ...
        SNS_INST_PRINTF(LOW, this, "opening port for async inst:%d type:%d addr:%d",
                        port_config.bus_instance, port_config.bus_type, port_config.slave_control);
