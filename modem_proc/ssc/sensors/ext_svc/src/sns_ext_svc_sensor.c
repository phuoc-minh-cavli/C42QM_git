...
event_task(void *arg)
{
  ...
  for(;;)
  {
    ...
    for(sns_list_iter_init(&iter, &sensor_state->events, false);
...
init(sns_sensor* const this)
{
  ...
  sns_list_init(&sensor_state->services);
  sns_list_init(&sensor_state->events);
    ...
    void const *c_struct, uint32_t msg_id)
{
  ...
  else
  {
    ...
    else
    {
      ...
      sns_list_item_init(&event->list_entry, event);
      sns_osa_lock_acquire(sensor_state->events_lock);
      sns_list_iter_init(&iter, &sensor_state->events, false);
      sns_list_iter_insert(&iter, &event->list_entry, true);
      sns_osa_lock_release(sensor_state->events_lock);
