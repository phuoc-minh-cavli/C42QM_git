...
sns_ext_svc_get(sns_list *services, uint32_t const svc_id)
{
  sns_list_iter iter;

  for(sns_list_iter_init(&iter, services, true);
      NULL != sns_list_iter_curr(&iter);
      sns_list_iter_advance(&iter))
  {
    sns_ext_service *curr = (sns_ext_service*)sns_list_iter_get_curr_data(&iter);
...
void
sns_ext_svc_add(sns_list *services, uint32_t svc_id, qmi_idl_service_object_type svc_obj)
{
  sns_list_iter iter;
  sns_ext_service *svc;
  ...
  svc->svc_id = svc_id;
  svc->svc_obj = svc_obj;
  sns_list_item_init(&svc->list_entry, svc);
  sns_list_iter_init(&iter, services, false);
  sns_list_iter_insert(&iter, &svc->list_entry, true);
}
