    ...
    struct sns_request const *request)
{
  ...
  {
    sns_fw_sensor_instance *instance = (sns_fw_sensor_instance*)this;
    sns_isafe_list_iter iter, req_iter;
    bool done = false;

    for(sns_isafe_list_iter_init(&iter, &instance->client_req_lists, true);
        NULL != sns_isafe_list_iter_curr(&iter) && !done;
        sns_isafe_list_iter_advance(&iter))
    {
      sns_client_req_list *req_list = (sns_client_req_list*)
        sns_isafe_list_iter_get_curr_data(&iter);
    ...
    struct sns_request const *request)
{
  ...
  sns_isafe_list_iter_insert(&iter, &fw_request->stream->list_entry_source, true);
...
SNS_SECTION(".text.sns") bool
sns_instance_streams_in_island(sns_sensor_instance const *instance)
{
  sns_isafe_list_iter iter;
  bool rv = true;
  ...
  for(sns_fw_data_stream *data_stream = sns_isafe_list_iter_get_curr_data(&iter);
      NULL != data_stream;
