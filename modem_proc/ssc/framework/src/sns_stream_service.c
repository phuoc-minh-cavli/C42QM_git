/*=============================================================================
  ...
  $Id: //components/rel/ssc.slpi/5.1/framework/src/sns_stream_service.c#2 $
 ...
 * created.
 *
 * PEND: This function is inherently not thread safe, and should be removed.
 * Below the dst_instance/dst_sensor is accessed, without acquiring its lock.
 * Specifically, a race condition may occur with sns_diag_move_sensor.
 *
...
handle_stream_destroy(sns_sensor *sensor, sns_fw_request *request)
{
  ...
  if(NULL != existing_req)
  {
    ...
    bad_instance = sns_sensor_util_find_instance(sensor,
      (sns_request*)existing_req, sensor->sensor_api->get_sensor_uid(sensor));
...
handle_req(sns_sensor *sensor, sns_fw_request *pend_req)
{
  ...
  {
    // PEND: Confirm that pend_req->stream is no longer on any instance
    if(NULL == instance)
  ...
  {
    bool valid_req =
      pend_req->stream->client_request == pend_req ||
    ...
    // PEND: Confirm that pend_req->stream is on 'instance'
  }
    ...
    struct sns_request *request)
{
  ...
  island_req = SNS_ISLAND_STATE_NOT_IN_ISLAND != stream->island_operation &&
    ...
    sns_island_is_island_ptr((intptr_t)stream->data_source->sensor.sensor_api->set_client_request) &&
    (NULL == stream->data_source->sensor.instance_api || NULL == stream->data_source->sensor.instance_api->set_client_config ||
    sns_island_is_island_ptr((intptr_t)stream->data_source->sensor.instance_api->set_client_config));
