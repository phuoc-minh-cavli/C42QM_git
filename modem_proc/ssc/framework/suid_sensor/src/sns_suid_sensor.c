...
decode_attr_value(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  ...
  /* sns_free for this allocation will be called in default_sensor_match_attr_cb() */
...
default_sensor_process_attribute_cb(void *buf, uint32_t buf_len, void *arg)
{
  ...
  if(!rc)
  {
    SNS_PRINTF(ERROR, sns_fw_printf, "default_sensor_process_attribute_cb, pb_decode() failed");
  }
  else
  {
...
default_sensor_match_attr_cb(struct sns_attribute_info *attr_info, void *arg)
{
  ...
  if(!item->match_found && sns_attr_info_get_available(attr_info))
  {
    ...
    if(0 == strcmp(datatype, item->datatype))
    {
      ...
      {
        void *attr_val = NULL;
        sns_def_sensor_attr *attr = (sns_def_sensor_attr*)
          sns_isafe_list_iter_get_curr_data(&iter);
        rc = sns_attr_info_process_attribute(attr_info, attr->attr_id,
        ...
        else
        {
          if(sns_memcmp(attr_val, attr->attr_val, attr->attr_val_size))
          {
            SNS_SPRINTF(MED, sns_fw_printf,
                "default_sensor_match_attr_cb: attribute id %d mismatch found %s",
...
default_sensor_item_decode_cb(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  ...
  else if(0 == strncmp((char*)item_name.buf, "val", item_name.buf_len))
  {
    ...
    sensor_attr->attr_val = sns_malloc(SNS_HEAP_MAIN, sensor_attr->attr_val_size);
    SNS_ASSERT(sensor_attr->attr_val_size == 0 || sensor_attr->attr_val != NULL);
    sns_memscpy(sensor_attr->attr_val, sensor_attr->attr_val_size,
                data_ptr, sensor_attr->attr_val_size );
  }
...
default_sensor_attr_group_decode_cb(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  ...
  if(!rv)
  {
    /* If this is the only item in the list do not return */
...
default_sensor_group_decode_cb(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
  ...
  else
  {
    ...
    SNS_SPRINTF(MED, sns_fw_printf,
        "default_sensor_group_decode_cb, dt=%s", sensor_info->datatype);
...
/**
 * Given a Sensor type, determine whether it is present in the default sensors list.
 *
 ...
 */
static bool
is_sensor_in_default_list(sns_encode_suid_arg *suid_arg)
{
  sns_suid_sensor_state *state =
    (sns_suid_sensor_state*)suid_sensor->sensor.state->state;
  sns_isafe_list_iter iter;
 ...
 * first matching SUID only if there is no default specified for the type through registry.
 *
    ...
    void *const *arg)
{
  ...
  /* If Sensor is present in default_sensor.json from registry do not use the first */
