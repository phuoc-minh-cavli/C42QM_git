 ...
 *
 * Note : declared and updated in sns_client_manager.c
 */
extern uint32_t sns_diag_max_sensor_log_size;

/* Combined payload array on heap to avoid stack overflow
 *
 * Note : declared and updated in sns_client_manager.c
 */
extern void *combined_payload;
...
typedef struct sns_cm_inst_state
{
  ...
  /* SNS_DIAG SUID; used to filter sending attribute requests */
  sns_sensor_uid diag_suid;
  /* To update APSS and act as body guard for wakeup sensors */
  smp2p_context_type *smp2p_instance_ctx;
  /* QMI client handle as received in the connect_cb */
  qmi_client_handle qmi_handle;
  /* Unique key to differentiate two smp2p request */
  uint32_t smp2p_instance_data;
