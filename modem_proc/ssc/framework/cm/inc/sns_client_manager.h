...
#define SNS_CM_MAX_BATCH_HEAP_USAGE_LOW   500*1024
...
typedef enum sns_cm_flush_type
{
  ...
  /* Used to identify a flush requested by batch period expiry. */
...
typedef struct sns_cm_request
{
 ...
 /* Set to true if received flush event complete for SNS_CM_BATCH_FLUSH.
  * Set to false when CM gets a batch of data.
  * Not used for anything related to SNS_CM_CLIENT_FLUSH. */
 bool received_batch_flush_event;
} sns_cm_request;
