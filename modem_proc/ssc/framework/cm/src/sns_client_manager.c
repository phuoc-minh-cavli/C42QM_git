...
/* Allocate combined payload array on heap to avoid stack overflow.
 * Used to send client API diag packets */
void* combined_payload = NULL;

/* This variable stores the maximum diag log packet size and is updated during
 * client manager instance initialization.
 * It is used to create an array of markers to split larger qmi indication payloads
 * into smaller payloads which can be submitted to diag service
 */
uint32_t sns_diag_max_sensor_log_size = 0;
...
void
sns_cm_set_latency_vote(sns_sensor *this, sns_std_client_processor proc_type)
{
  if(sns_cm_is_island_blocked(this, proc_type))
  {
    ...
    sns_pwr_sleep_mgr_set_latency((void *)this, SNS_CM_APPS_AWAKE_LATENCY_USEC);
  }
  else
  {
 ...
 */
void
cm_handle_low_batch_memory(sns_fw_sensor *cm_sensor, bool drop_events)
{
 ...
 */
static void
cm_handle_timer_signal(sns_fw_sensor *sensor)
{
 ...
 */
static void sns_cm_task(void *arg)
{
 ...
 *
 * PEND: Update simulation QMI implementation to use these signals.
 */
static void
...
bool
sns_cm_is_island_blocked(sns_sensor *this, sns_std_client_processor proc_type)
{
...
/*=============================================================================
  ...
  ===========================================================================*/
static void
cm_switch_non_wkup_streaming_req_to_batching_req(sns_data_stream *data_stream,
           ...
           NULL != cm_request->data_stream)
        {
          ...
          if(!(state->registry_config.max_batch_disabled))
          {
            cm_switch_non_wkup_streaming_req_to_batching_req(
                cm_request->data_stream,cm_request,enable_max_batch);
          }
        }
      }
...
SNS_SECTION(".text.island") cm_proc_info*
sns_cm_get_proc_info(sns_sensor *this, sns_std_client_processor proc_type)
{
 ...
 */
static void __attribute__ ((noinline))
cm_handle_remote_proc_event(sns_sensor *this)
{
  sns_cm_state *state = (sns_cm_state*)this->state->state;
  ...
  while(NULL != event)
  {
    ...
    if(SNS_REGISTRY_MSGID_SNS_REGISTRY_READ_EVENT == event->message_id)
    {
      ...
      else
      {
        ...
        for(int i = 0; i < ARR_SIZE(cm_registry_groups); i++)
        {
 ...
 */
static void
cm_send_registry_request(sns_sensor *const this, char *reg_group_name)
{
  uint8_t buffer[100];
  int32_t encoded_len;
  sns_cm_state *state = (sns_cm_state*)this->state->state;
 ...
 */
static void
cm_send_remote_proc_request(sns_cm_state *state, sns_sensor *const this)
{
...
static sns_rc
cm_sensor_notify_event(sns_sensor *const this)
{
  sns_cm_state *state = (sns_cm_state*)this->state->state;
  bool complete = sns_suid_lookup_complete(&state->suid_lookup_data);

  sns_suid_lookup_handle(this, &state->suid_lookup_data);
  if(complete != sns_suid_lookup_complete(&state->suid_lookup_data))
  {
...
*/
static void
cm_publish_attributes(sns_sensor *const this)
{
  //sns_cm_state *state = (sns_cm_state *)this->state->state;
  {
 ...
 */
static sns_rc
cm_sensor_init(sns_sensor *const this)
{
  sns_rc rc = SNS_RC_SUCCESS;
  sns_osa_lock_attr attr;
  sns_cm_state *state = (sns_cm_state*)this->state->state;
  char *smp2p_remote_name = "sleepstate_see";
  sns_service_manager *service_mgr = this->cb->get_service_manager(this);
  ...
  sns_suid_lookup_add(this, &state->suid_lookup_data, "diag_sensor");
  ...
  cm_publish_attributes(this);
  {
    ...
    if(SMP2P_STATUS_SUCCESS != smp2p_create(&state->smp2p_ctx,
                                  SMEM_APPS,
                                  smp2p_remote_name))
    {
      SNS_PRINTF(HIGH, this, "smp2p_create failed");
      state->smp2p_ctx = NULL;
    }
  }
  rc = sns_cm_service_register(this);

  sns_diag_max_sensor_log_size =
      diag_service->api->get_max_log_size(diag_service)
      - sns_diag_get_preencoded_size(SNS_DIAG_ENC_SIZE_CLIENT_API_HDR);
  if(NULL == combined_payload)
  {
    combined_payload = sns_malloc(SNS_HEAP_MAIN,
        sns_diag_max_sensor_log_size);
  }
...
sns_rc
cm_sensor_deinit(sns_sensor *const this)
{
  UNUSED_VAR(this);
  sns_rc rc = SNS_RC_SUCCESS;
  ...
  if(NULL != combined_payload)
  {
    sns_free(combined_payload);
    combined_payload = NULL;
  }
  return rc;
}
...
cm_sensor_set_client_request(sns_sensor *const this,
    sns_request const *curr_req, sns_request const *new_req, bool remove)
{
  UNUSED_VAR(this);
  UNUSED_VAR(curr_req);
  UNUSED_VAR(new_req);
...
SNS_SECTION(".text.island") sns_sensor_uid const*
cm_get_sensor_uid(sns_sensor const *const this)
{
  UNUSED_VAR(this);
...
bool
cm_parse_reg_config(sns_registry_data_item *reg_item, pb_buffer_arg *item_name,
...
cm_proc_info*
sns_cm_create_proc_info(sns_sensor *this,
...
void
sns_cm_reval_and_register_timers(sns_sensor *const this,
    sns_std_client_processor proc_type, sns_cm_request* cm_req)
{
  sns_cm_state *cm_state = (sns_cm_state*)this->state->state;
  sns_list_iter iter;
  bool proc_fnd = false;
      ...
      sns_list_iter_advance(&iter))
  {
    ...
    if(proc_fnd && 0 != proc_info->time.reg_time)
    {
...
sns_rc
sns_register_cm(sns_register_cb const *register_api)
{
