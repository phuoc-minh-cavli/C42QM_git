...
/* Min duration between two smp2p_write() API call. */
#define SNS_CM_MIN_SMP2P_WRITE_INTERVAL_NS (200*1000*1000)

/* Size of the client API encoded SUID event size. This needs to be subtracted
 * from the returned max payload size which is used as threshold for adding
 * event markers. The encoded size is 20 bytes */
#define SNS_CLIENT_API_EVENT_SUID_ENCODED_SIZE 30

/* Assuming number of events batched cannot go beyond this number.
 * This assumption is practically valid. */
#define CM_TRIM_ALL_EVENTS UINT32_MAX
...
/* This variable stores smp2p_write() request timestamp.
 * It's used to keep tack of interval between two smp2p_write() calls.
 */
static sns_time smp2p_write_ts=0;
...
/**
 * This function calls smp2p_write() API for wake up request. It ensures
 * Apps is awake for 200ms so that client doesn't miss reading the event.
 * This function should be called after qmi_csi_send_ind()
 */
static void cm_smp2p_write(sns_sensor_instance *this, sns_cm_request *cm_request)
{
  int32 status = SMP2P_STATUS_SUCCESS;
  sns_cm_inst_state *client = (sns_cm_inst_state*)this->state->state;
  sns_time current_time = sns_get_system_time();
  sns_time min_smp2p_write_interval = sns_convert_ns_to_ticks(SNS_CM_MIN_SMP2P_WRITE_INTERVAL_NS);

  if(min_smp2p_write_interval > (current_time - smp2p_write_ts))
  {
#if ENABLE_DEBUG_PRINT
     SNS_INST_PRINTF(HIGH, this, "Skipping smp2p_write() API call");
#endif
     return;
  }
  if((SNS_CLIENT_DELIVERY_WAKEUP == cm_request->req_msg.susp_config.delivery_type) &&
     (SNS_STD_CLIENT_PROCESSOR_APSS == cm_request->req_msg.susp_config.client_proc_type))
  {
    if(NULL != client->smp2p_instance_ctx)
    {
      client->smp2p_instance_data = ~client->smp2p_instance_data;
      status = smp2p_write(client->smp2p_instance_ctx, client->smp2p_instance_data);

      if(SMP2P_STATUS_SUCCESS == status)
      {
        smp2p_write_ts = current_time;
      }
      else
      {
        SNS_INST_PRINTF(ERROR, this, "failed smp2p_write() status %d", status);
      }
    }
    else
    {
      SNS_INST_PRINTF(HIGH, this, "smp2p_instance_ctx is NULL. Can't call smp2p_write()");
    }
  }
}

/**
 * This function encodes and sends a QMI indication for single/ multiple events.
 * The events in the indication are logged only if the indication is sent
 * successfully.
 *
 ...
 * @param[i] diag_event_markers Array of markers based on diag log size
 * @param[i] diag_event_markers_array_size Number of elements in diag_event_markers array
 * @param[i] client_event_suid_bytes Size of encoded suid in indication payload
 */
static qmi_csi_error cm_send_qmi_ind(
    ...
    const uint32_t* diag_event_markers,
    uint32_t diag_event_markers_array_size,
    uint32_t client_event_suid_bytes)
{
  sns_cm_inst_state *client = (sns_cm_inst_state*)this->state->state;
  ...
  {
    //logging the events only if the qmi_send was successful
    if(NULL == diag_event_markers)
    {
      //streaming use case
    ...
    else
    {
      //batching use case
      int index = 1;
      void* payload_start = ind->payload;

      // first event packet has the encoded suid at the start
      // of the payload
      sns_fw_log_client_api_ind(this, &sensor->sensor,
                                  cm_get_sensor_uid(NULL),
                                  ind->client_id, payload_start,
                                  diag_event_markers[0]);
      payload_start = (void*)((uint8_t*)payload_start + diag_event_markers[0]);

      while(index < diag_event_markers_array_size && diag_event_markers[index] !=0)
      {

        // from second packet onwards, prepend the encoded suid
        uint32_t combined_payload_len = client_event_suid_bytes +
            diag_event_markers[index];
        sns_memzero(combined_payload, sns_diag_max_sensor_log_size);

        //copy the encoded suid
        sns_memscpy(combined_payload, client_event_suid_bytes,
            ind->payload, combined_payload_len);
        //copy the actual payload
        sns_memscpy((uint8_t*)combined_payload + client_event_suid_bytes,
            diag_event_markers[index],
            payload_start, combined_payload_len - client_event_suid_bytes);

        sns_fw_log_client_api_ind(this, &sensor->sensor,
                                  cm_get_sensor_uid(NULL),
                                  ind->client_id, combined_payload,
                                  combined_payload_len);

#if ENABLE_DEBUG_PRINT
        SNS_INST_PRINTF(MED, this, "Diag marker : %u, index : %u, suid header bytes : %u, "
            "combined_payload len : %u", diag_event_markers[index], index,
            client_event_suid_bytes, combined_payload_len);
#endif

        payload_start = (void*)((uint8_t*)payload_start + diag_event_markers[index]);
        index++;
      }
    }
  }
  ...
  else
  {
    if(cm_ind_pb_encode_event(&stream, event, this))
    {
       rv = cm_send_qmi_ind(this, sensor,
          client->use_jumbo_report, stream.bytes_written, NULL, 0, 0);
      //We can hit this error if the client disconnected while
      //we were sending data.So printing a high log here.
  ...
  sns_cm_request *cm_request)
{
  ...
  {
    cm_bin = (sns_cm_batch_bin*) sns_isafe_list_item_get_data(curr_batch_buff);

    //Updating timestamp based on data events only.
  ...
  SNS_SPRINTF(MED, sns_fw_printf,
      "cm_req %0x Items after %d first_ts %llu last_ts %llu",
      cm_request,sns_isafe_list_iter_len(&iter), cm_request->first_batch_ts,
      cm_request->last_batch_ts);
#endif
}
 ...
 * @param[i] trim_count: Trim these many event provided trim_all is false
 */
SNS_SECTION(".text.island")
...
)
{
  ...
  {
    cm_bin = (sns_cm_batch_bin*) sns_isafe_list_iter_get_curr_data(&batch_bin_iter);
  ...
  //Update the time stamps as per remaining data
 ...
 * It also adds markers for sending diag packets such that the size
 * of each diag packet is less than the permissible max log size
 *
 * @param[i] this              The client manager instance.
 * @param[i] sensor            The sensor which generated the event.
    ...
    uint32_t *last_sent_msg_id)
{
  ...
  sns_memscpy(&uid, sizeof(uid), &suid, sizeof(suid));
  qmi_csi_error err = QMI_CSI_DEFERRED;

  uint32_t diag_marker_threshold = sns_diag_max_sensor_log_size -
      SNS_CLIENT_API_EVENT_SUID_ENCODED_SIZE;
  uint32_t diag_event_markers_array_size = (int)((SNS_CM_MAX_IND_LEN/diag_marker_threshold) + 1);
  uint32_t diag_event_markers [diag_event_markers_array_size];
  uint32_t diag_event_markers_index = 0;
  uint32_t curr_diag_log_packet_size = 0;
  uint32_t client_event_suid_bytes = 0;

  sns_memzero(diag_event_markers, sizeof(diag_event_markers));
  ...
  while(NULL != sns_isafe_list_iter_curr(&iter))
  {
    ...
    uint32_t curr_event_size = 0;
    uint32_t stream_bytes_before_event = 0;

    if((last_sent_msg == batch_bin->event.message_id) &&
       (last_sent_ts == batch_bin->event.timestamp))
    {
      SNS_PRINTF(HIGH, sns_fw_printf, "Duplicate event id %d ts %"PRIu64" req %0x",
    ...
    if(message_full)
    {
      ...
      // when the stream is reset, update the current log packet size
      // and the encoded suid bytes. Also, clear the array and reset the
      // array index
      curr_diag_log_packet_size = stream.bytes_written;
      client_event_suid_bytes = stream.bytes_written;
      diag_event_markers_index = 0;
      sns_memzero(diag_event_markers, sizeof(diag_event_markers));
    }

    stream_bytes_before_event = stream.bytes_written;
    message_full = !cm_ind_pb_encode_event(&stream, &batch_bin->event, this);
    //encoded size of an event added to the stream
    curr_event_size = stream.bytes_written - stream_bytes_before_event;
    curr_diag_log_packet_size += curr_event_size;

    // If packet size overflows after adding an event,
    // add a marker to the array at the size before adding
    // the most recent event
    if(curr_diag_log_packet_size > diag_marker_threshold)
    {
      diag_event_markers[diag_event_markers_index++] = curr_diag_log_packet_size -
          curr_event_size;
      curr_diag_log_packet_size = curr_event_size;
    }
    ...
    {
      //add the last marker when message is full
      diag_event_markers[diag_event_markers_index++] = curr_diag_log_packet_size;

      *last_sent_msg_id = batch_bin->event.message_id;

      err = cm_send_qmi_ind(this, sensor, client->use_jumbo_report, stream.bytes_written,
          diag_event_markers, diag_event_markers_array_size, client_event_suid_bytes);
      if(QMI_CSI_CONN_BUSY == err)
      {
        //Do not drop events if QMI busy, drop for all other QMI errors
        SNS_INST_PRINTF(HIGH, this, "Received QMI_CSI_CONN_BUSY for client %i",
            client->client_id);
        break;
      }

      cm_trim_batch_buffer_list(cm_request, send_count );
      send_count = 0;
  ...
  if ((QMI_CSI_NO_ERR == err) || (QMI_CSI_CONN_BUSY == err))
  {
    cm_smp2p_write(this, cm_request);
  }
}
    ...
    bool drop_events)
{
  ...
  if(drop_events)
  {
     ...
     cm_trim_batch_buffer_list(cm_req, SNS_CM_BATCH_BIN_DROP_COUNT);
  }
}
 ...
 */
SNS_SECTION(".text.island")
static uint32_t cm_remove_oldest_events(
    sns_cm_request *cm_request)
{
 ...
 */
SNS_SECTION(".text.island")
static uint32_t sns_cm_drop_oldest_batch_data(sns_fw_sensor *cm_sensor)
{
          ...
          sns_isafe_list_iter_advance(&req_iter))
      {
            ...
            cm_request->req_msg.request.batching.flush_only)
        {
          if(cm_request->last_batch_ts - cm_request->first_batch_ts >=
             cm_request->compare_period)
          {
            dropped += cm_remove_oldest_events(cm_request);
          }
        }
      }
    }
   ...
   sns_sensor_instance *this)
{
  ...
  if(NULL != cm_request)
  {
    ...
    {
      if(cm_request->received_batch_flush_event)
      {
        SNS_INST_PRINTF(LOW, this, "Filter redundant flush event.");
        return rc;
      }
      else
      {
        /* Set to true if received flush event complete for batch data.*/
        cm_request->received_batch_flush_event = true;
      }
    }

    /* Set to false if received non flush event data.*/
    if(SNS_STD_MSGID_SNS_STD_FLUSH_EVENT != event->message_id)
    {
      cm_request->received_batch_flush_event = false;
    }
    ...
    {
      uint32_t dropped_cnt = sns_cm_drop_oldest_batch_data(cm_sensor);
      if(0 < dropped_cnt &&
         SNS_ISLAND_STATE_IN_ISLAND == sns_island_get_island_state() &&
         !(cm_state->registry_config.max_batch_disabled))
      {
        // Dropping old data didn't cause an island exit.
        // Attempt to alloc island memory.
        cm_batch_bin = (sns_cm_batch_bin*)sns_malloc(
          SNS_HEAP_ISLAND,struct_len);
      }
      else if(!(cm_state->registry_config.max_batch_disabled))
      {
        SNS_ISLAND_EXIT();
...
static void cm_send_attribute_request(
    ...
    sns_sensor_instance *instance)
{
  sns_cm_inst_state *state = (sns_cm_inst_state*)instance->state->state;
  size_t encoded_len;
  uint8_t buffer[50];
  sns_std_request std_req = sns_std_request_init_default;
  sns_std_attr_req attr_req = sns_std_attr_req_init_default;
  sns_sensor_uid suid = sns_get_suid_lookup();

  attr_req.has_register_updates = false;

  if(sns_sensor_uid_compare((sns_sensor_uid*)&cm_request->req_msg.suid, &suid) ||
     sns_sensor_uid_compare((sns_sensor_uid*)&cm_request->req_msg.suid, &state->diag_suid))
  {
    SNS_INST_PRINTF(HIGH, instance,
        "Do not send attribute request for suid/diag sensors");
  }
  else if(cm_request->req_msg.msg_id == SNS_STD_MSGID_SNS_STD_ATTR_REQ)
  {
    SNS_INST_PRINTF(HIGH, instance,
        "Do not send attribute request if CM request was for an attribute");
  }
  else
  {
    encoded_len = pb_encode_request(buffer, sizeof(buffer),
        &attr_req ,sns_std_attr_req_fields , &std_req);
    if(encoded_len > 0)
    {
      ...
      SNS_INST_PRINTF(HIGH, instance,
          "Sending attribute request for sensor %x", cm_request->req_msg.suid);
    }
  }
}
    ...
    const int msg_id_cnt)
{
  ...
  if(req_msg->request.has_batching)
  {
    ...
    // If has_flush_period false and batch period == 0, then, flush_period = UINT32_MAX
    // If has_flush_period false and batch period > 0, then, flush_period = batch_period
    if((0 == cm_request->req_msg.request.batching.has_flush_period) &&
       (0 == cm_request->req_msg.request.batching.batch_period))
    {
      cm_request->req_msg.request.batching.has_flush_period = true;
      cm_request->req_msg.request.batching.flush_period = UINT32_MAX;  
    }
    if(0 == cm_request->req_msg.request.batching.flush_period)
    {
      cm_request->req_msg.request.batching.has_flush_period = true;
  ...
  {
    cm_request->req_msg.request.batching.batch_period = 0;
    // If has_batching = false, flush_period -> MAX
    cm_request->req_msg.request.batching.flush_period = UINT32_MAX;
    cm_request->req_msg.request.batching.has_flush_period = true;
   ...
   sns_cm_request      *cm_request)
{
  ...
  for( ;; )
  {
    ...
    if( ( send_flag ) && ( !batch_flag ) )
    {
      ...
      if( ( SNS_STD_MSGID_SNS_STD_FLUSH_EVENT == event->message_id ) &&
          ( SNS_CM_CLIENT_FLUSH == cm_request->flush_type ) )
      {
        cm_request->active_flush_req_count--;
        while( cm_request->active_flush_req_count > 0) 
        {
          cm_send_ind(this, event, src_sensor, cm_request);
          cm_request->active_flush_req_count--;
        }
    ...
    if( ( true == send_flag ) &&
        ...
        ( SNS_CM_NO_FLUSH != cm_request->flush_type) )
    {
      sns_cm_send_all_data_for_proc(cm_sensor, proc_type, false);
    }
    ...
    sns_sensor *cm_sensor)
{
  ...
  if(NULL != req)
  {
    ...
    if(pb_decode(&stream, sns_std_request_fields, &decoded_req))
    {
      ...
      //Disable flush only as well with the registry flag
      //This flag is disabled for regression island mode testing
      //so that the SDC delivers data to algorithms and the algorithms
      //are exercised in island mode
      if(decoded_req.has_batching &&
         (decoded_req.batching.has_flush_only) &&
         (decoded_req.batching.flush_only) &&
         (cm_state->registry_config.max_batch_disabled) &&
         !cm_request->req_msg.request.is_passive &&
         (SNS_CLIENT_DELIVERY_NO_WAKEUP ==
             cm_request->req_msg.susp_config.delivery_type))
      {
        decoded_req.batching.has_flush_only = false;
        decoded_req.batching.flush_only = false;
      }
   ...
   sns_client_req_msg_v01 *qmi_req)
{
      ...
      sns_isafe_list_iter_advance(&req_list_iter))
  {
                              ...
                              (sns_sensor_uid*)&req_msg->suid))
    {
      ...
      cm_trim_batch_buffer_list(cm_req_msg, CM_TRIM_ALL_EVENTS);
   ...
   sns_sensor_state const *sensor_state)
{
  ...
  sns_cm_state *state = (sns_cm_state*)sensor_state->state;
  ...
  sns_suid_lookup_get(&state->suid_lookup_data,
      "diag_sensor", &cm_inst_state->diag_suid);
