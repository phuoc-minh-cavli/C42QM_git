   ...
   void              **connection_handle)
{
  ...
  else
  {
    ...
    if(NULL != instance)
    {
      ...
      if (cm_state->smp2p_ctx)
      {
        client->smp2p_instance_ctx = cm_state->smp2p_ctx;
        client->smp2p_instance_data = (uint32_t)client_id_next;
      } else {
        client->smp2p_instance_ctx = NULL;
      }
   ...
   void *service_cookie)
{
  ...
  else
  {
    ...
    sns_fw_log_inst_map(s_instance);
...
#if defined(SSC_TARGET_MDM9205) 
  qmi_sap_error qmi_sap_err = QMI_SAP_NO_ERR;
  qmi_sap_client_handle sap_client_handle = NULL;
#endif
...
#if defined(SSC_TARGET_MDM9205) 
  qmi_sap_err = qmi_sap_register(SNS_CLIENT_SVC_get_service_object_v01(), NULL, &sap_client_handle);  
  SNS_PRINTF(HIGH, sensor, "qmi_sap_register - %i", qmi_sap_err);
  SNS_ASSERT(QMI_SAP_NO_ERR == qmi_sap_err);
#endif
