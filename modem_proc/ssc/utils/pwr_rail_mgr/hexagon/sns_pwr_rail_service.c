 ...
 *
 * $Id: //components/rel/ssc.slpi/5.1/utils/pwr_rail_mgr/hexagon/sns_pwr_rail_service.c#4 $
 * $DateTime: 2019/10/15 00:07:17 $
 * $Change: 20935119 $
 *
 **/
...
SNS_SECTION(".text.sns") static void sns_check_pwr_rail_client_count(void)
{
  ...
  if((pwr_rail_client_count == 0) && (pwr_rail_service.sleep_set == false))
  {
    ...
    if(NULL != pwr_rail_service.pwr_client)
    {
       ...
       sns_pwr_scpm_mgr_set_mcps_vote(pwr_rail_service.pwr_client, SNS_Q6_MCPS_DEFAULT_LEVEL);
       sns_pwr_sleep_mgr_unblock_tcm_pwr_collapse();
	   sns_pwr_scpm_mgr_bw_vote(SSC_BW_ZERO);
	}
  ...
  else if((pwr_rail_client_count > 0) && (pwr_rail_service.sleep_set == true))
  {
    ...
    if(SNS_Q6_MCPS_DEFAULT == sns_pwr_scpm_mgr_get_current_mcps_vote())
    {
	  if(NULL != pwr_rail_service.pwr_client)
	  {
	    sns_pwr_scpm_mgr_set_mcps_vote(pwr_rail_service.pwr_client, SNS_Q6_MCPS_LOW_SVS_LEVEL);
		sns_pwr_sleep_mgr_block_tcm_pwr_collapse();
	  }
    }
    sns_pwr_scpm_mgr_bw_vote(SNS_SSC_BW_DEF_VOTE);
...
SNS_SECTION(".text.sns") static bool sns_rail_name_is_valid(sns_rail_name const* rail)
{
  ...
  */
  if(   0 != strncmp(PMIC_NPA_GROUP_ID_SENSOR_VDD, rail->name, sizeof(PMIC_NPA_GROUP_ID_SENSOR_VDD))
     ...
     && 0 != strncmp(SNS_PWR_RAIL_VDD, rail->name, sizeof(SNS_PWR_RAIL_VDD))
