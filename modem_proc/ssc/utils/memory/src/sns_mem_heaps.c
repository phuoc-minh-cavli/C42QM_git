...
/* Only trigger low mem callback when free memory difference is bigger than
 * this threshold */
...
typedef struct
{
  /* Present on sns_mem_heap_info::lowmem_cb_list or lowmem_cb_list.
   * Default on former; placed on latter when low memory reached. */
...
void    *sns_pram_addr SNS_SECTION(".bss.sns_island_heap");
uint32_t sns_pram_size SNS_SECTION(".bss.sns_island_heap");
...
static sns_list lowmem_cb_list;
...
sns_mem_heap_check_lowmem(sns_mem_heap_id heap_id, bool force)
{
  ...
  // PEND: Accessing allocated_bytes could be optimized by using atomics
  heap_info[heap_id].heap_head->lock_fnc_ptr(heap_info[heap_id].heap_head);
  free_bytes = heap_info[heap_id].heap_head->total_bytes -
               heap_info[heap_id].heap_head->allocated_bytes;
  heap_info[heap_id].heap_head->free_fnc_ptr(heap_info[heap_id].heap_head);

  sns_osa_lock_acquire(&heap_info_lock);
  for(sns_list_iter_init(&iter, &heap_info[heap_id].lowmem_cb_list, true);
      ...
      sns_list_iter_advance(&iter))
  {
    ...
    {
      sns_list_iter low_mem_iter;

      SNS_ISLAND_EXIT();
      sns_list_iter_init(&low_mem_iter, &lowmem_cb_list, true);
      sns_list_iter_remove(&iter);
      sns_list_iter_insert(&low_mem_iter, &cb->list_entry, true);
...
{
  UNUSED_VAR(arg);
  sns_list_iter client_iter, low_mem_iter;
  ...
  while(1)
  {
    ...
    sns_list_iter_init(&low_mem_iter, &lowmem_cb_list, true);
    while(NULL != sns_list_iter_curr(&low_mem_iter))
    {
      sns_mem_heap_low_mem_client *cb = (sns_mem_heap_low_mem_client*)
          sns_list_item_get_data(sns_list_iter_remove(&low_mem_iter));
      sns_osa_lock_release(&heap_info_lock);
      size_t before, after;
      sns_time start, end;
      ...
      SNS_PRINTF(HIGH, sns_fw_printf,
          "Low memory callaback 0x" SNS_DIAG_PTR " dropped %d bytes on heap %d in %d ms",
          cb->callback, (before-after), cb->heap_id, ((end-start)*100)/1920000);
      sns_osa_lock_acquire(&heap_info_lock);
      sns_list_iter_init(&client_iter, &heap_info[cb->heap_id].lowmem_cb_list, true);
      sns_list_iter_insert(&client_iter, &cb->list_entry, true);
    }
...
sns_heap_init(uint8_t *pram_heap_head)
{
  ...
  for(int i = 0; i < SNS_HEAP_MAX; i++)
  {
    ...
    sns_list_init(&heap_info[i].lowmem_cb_list);
  }
...
sns_malloc(sns_mem_heap_id heap_id, size_t size)
{
  ...
  sns_mem_heap_check_lowmem(heap_id, force_lowmem);
  return ptr;
}
  ...
  sns_mem_heap_lowmem_cb callback, intptr_t callback_arg)
{
  ...
  if(sns_island_is_island_ptr((intptr_t)heap_info[heap_id].heap_memory))
  ...
  sns_osa_lock_acquire(&heap_info_lock);
  sns_list_iter_init(&iter, &heap_info[heap_id].lowmem_cb_list, true);
  sns_list_item_init(&callback_data->list_entry, callback_data);
  sns_list_iter_insert(&iter, &callback_data->list_entry, true);
  sns_osa_lock_release(&heap_info_lock);
}
...
sns_memmgr_log_heap_usage(uint64_t cookie)
{
  for(int i = 0; i < ARR_SIZE(heap_info); i++)
  {
    ...
    if(NULL != log)
    {
      ...
      heap_info[i].heap_head->lock_fnc_ptr(heap_info[i].heap_head);
      log->total_memory = heap_info[i].heap_head->total_bytes;
      log->used_memory = heap_info[i].heap_head->used_bytes;
      log->has_cookie = true;
      log->cookie = cookie;
      heap_info[i].heap_head->free_fnc_ptr(heap_info[i].heap_head);
