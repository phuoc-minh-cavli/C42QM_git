...
DESCRIPTION
 ...
 Copyright (c) 1997-2019 Qualcomm Technologies, Inc.
 All Rights Reserved.
...
$Id: //components/rel/ssc.slpi/5.1/utils/memory/src/sns_memheap_lite.c#1 $
...
#ifdef SNS_VALGRIND
  VALGRIND_CREATE_MEMPOOL(heap_ptr, 0, 0);
#endif
  return;
} /* END mem_init_heap */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
#ifdef SNS_VALGRIND
    VALGRIND_MEMPOOL_ALLOC(heap_ptr, freeBlock, actualSize);
    ANNOTATE_NEW_MEMORY(freeBlock, actualSize);
    VALGRIND_MAKE_MEM_DEFINED(freeBlock, blockHeaderSize);
#endif
...
)
{
  ...
  sns_mem_block_header_type *blockPtr;
...
#ifdef SNS_VALGRIND
  VALGRIND_HG_CLEAN_MEMORY(blockPtr, theBlock->forw_offset);
#else
  UNUSED_VAR(blockPtr);
#endif
...
#ifdef SNS_VALGRIND
  VALGRIND_MAKE_MEM_UNDEFINED(blockPtr, sizeBlockHeader);
  VALGRIND_MEMPOOL_FREE(heap_ptr, blockPtr);
#endif
  END_CRITICAL_SECTION(heap_ptr);
