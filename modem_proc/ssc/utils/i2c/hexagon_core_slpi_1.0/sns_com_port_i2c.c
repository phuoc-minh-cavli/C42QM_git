...
/**--------------------------------------------------------------------------*/
typedef struct sns_i2c_bus_resource_s
{
  sns_bus_type bus_type;
  uint8_t      bus_instance;
  uint8_t      ref_count;
} sns_i2c_bus_resource_s;
...
static sns_i2c_bus_resource_s sns_i2c_bus_resources[ MAX_INSTANCES_SUPPORTED ];
SNS_SECTION(".data.sns")
static  sns_osa_lock *sns_i2c_bus_resource_lock = NULL;
SNS_SECTION(".data.sns")
static unsigned int sns_i2c_initialized = 0;
...
SNS_SECTION(".text.sns") sns_rc sns_pwr_update_i2c_private(sns_sync_com_port_handle *port_handle, bool power_bus_on)
{
  sns_com_port_priv_handle *priv_handle = (sns_com_port_priv_handle *)port_handle;
  sns_bus_info *bus_info = &priv_handle->bus_info;
...
sns_rc
sns_com_port_check_and_turn_on_bus(sns_sync_com_port_handle *port_handle)
{
  int i;
  ...
  sns_com_port_priv_handle *priv_handle = (sns_com_port_priv_handle *)port_handle;
  sns_com_port_config      *com_config = &priv_handle->com_config;
  sns_bus_type              bus_type = com_config->bus_type;
  uint8_t                   bus_instance = com_config->bus_instance;

  qurt_atomic_barrier();
  if(qurt_atomic_compare_and_set( &sns_i2c_initialized, 0, 1 ))
  {
    ...
    SNS_ASSERT(SNS_RC_SUCCESS == rc);
    rc = sns_osa_lock_create(&lock_attr, &sns_i2c_bus_resource_lock);
    SNS_ASSERT(SNS_RC_SUCCESS == rc);
    sns_i2c_initialized = 2;
  }
  while((volatile unsigned int)sns_i2c_initialized != 2 )
  { 
    qurt_atomic_barrier(); 
  }
  if( bus_type == SNS_BUS_SPI ||
      bus_type == SNS_BUS_UART )
  {
    return SNS_RC_FAILED;
  }
  sns_osa_lock_acquire(sns_i2c_bus_resource_lock);

  for( i = 0; i < MAX_INSTANCES_SUPPORTED; i++ )
  {
    if( bus_type == sns_i2c_bus_resources[i].bus_type &&
        bus_instance == sns_i2c_bus_resources[i].bus_instance )
    {
      break;
    }
  }
  ...
  if( i == MAX_INSTANCES_SUPPORTED )
  {
    for( i = 0; i < MAX_INSTANCES_SUPPORTED; i++ )
    {
      if( sns_i2c_bus_resources[i].bus_instance == 0 )
      {
        sns_i2c_bus_resources[i].bus_type = bus_type;
        sns_i2c_bus_resources[i].bus_instance = bus_instance;
        break;
      }
    }
  }
  ...
  if( i < MAX_INSTANCES_SUPPORTED )
  {
    sns_i2c_bus_resources[i].ref_count++;
    if( sns_i2c_bus_resources[i].ref_count == 1 )
    {
      SNS_ISLAND_EXIT();
      rc = sns_pwr_update_i2c_private(port_handle, true);
    }
  }
  else
  {
  ...
  sns_osa_lock_release(sns_i2c_bus_resource_lock);
  return rc;
}
...
SNS_SECTION(".text.sns")
sns_rc
sns_com_port_check_and_turn_off_bus(  sns_sync_com_port_handle *port_handle)
{
  int i;
  sns_rc rc = SNS_RC_SUCCESS;
  ...
  if( i < MAX_INSTANCES_SUPPORTED )
  {
    ...
    {
      SNS_ISLAND_EXIT();
      sns_i2c_bus_resources[i].ref_count = 0;
      sns_i2c_bus_resources[i].bus_instance = 0;
      sns_i2c_bus_resources[i].bus_type = 0;
      rc = sns_pwr_update_i2c_private(port_handle, false);
    }
    else if( sns_i2c_bus_resources[i].ref_count > 0 )
    {
      sns_i2c_bus_resources[i].ref_count--;
    }
  }
...
SNS_SECTION(".text.sns") sns_rc sns_pwr_update_i2c_internal(sns_sync_com_port_handle *port_handle, bool power_bus_on)
{
  ...
  if(power_bus_on)
  {
    ret = sns_com_port_check_and_turn_on_bus(port_handle);
  }
  else
  {
    ret = sns_com_port_check_and_turn_off_bus(port_handle);
  }
