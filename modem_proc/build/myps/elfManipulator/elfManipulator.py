...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/build.mpss/2.2.8.1.c5.0.1/myps/elfManipulator/elfManipulator.py#1 $
...
def main():
    ...
    extpl_paging_sections = [ 
        (
         ".candidate_extpl_section_nb1", 
         "__swapped_segments_extpl_nb1_rodata_end__", "__swapped_segments_extpl_nb1_text_start__",
         "__swapped_segments_extpl_nb1_rwdata_end__", "__swapped_segments_extpl_nb1_rwdata_start__"
        ),

        (
          ".candidate_extpl_section_m1", 
          "__swapped_segments_extpl_m1_rodata_end__", "__swapped_segments_extpl_m1_text_start__",
          "__swapped_segments_extpl_m1_rwdata_end__", "__swapped_segments_extpl_m1_rwdata_start__"),

        (
          ".candidate_extpl_section_geran", 
          "__swapped_segments_extpl_geran_rodata_end__", "__swapped_segments_extpl_geran_text_start__",
          "__swapped_segments_extpl_geran_rwdata_end__", "__swapped_segments_extpl_geran_rwdata_start__",
        ),

        (
          ".candidate_extpl_section_gps", 
          "__swapped_segments_extpl_gps_rodata_end__", "__swapped_segments_extpl_gps_text_start__",
          "__swapped_segments_extpl_gps_rwdata_end__", "__swapped_segments_extpl_gps_rwdata_start__"
        ),
        (
          ".candidate_extpl_section_foo", 
          "__swapped_segments_extpl_foo_rodata_end__", "__swapped_segments_extpl_foo_text_start__",
          "__swapped_segments_extpl_foo_rwdata_end__", "__swapped_segments_extpl_foo_rwdata_start__"
        ),
    ]
    ...
    module_hashes=""
    for section in extpl_paging_sections:
        sh_extpl = elf.getSectionByName(section[0])
        if sh_extpl != const.RC_ERROR:
            print "module %s total size %d" % (section[0], sh_extpl.sh_size)
            # hash for rx/ro pages
            size = elf.getSymbolByName(section[1]).st_value - elf.getSymbolByName(section[2]).st_value
            hash = hashlib.sha256(sh_extpl.contents[0:size])
            module_hashes = module_hashes + hash.digest()
            print "module %s rx/ro size %d" % (section[0], size)
            print "    sha256 ", hash.hexdigest()
            
            # hash for rw pages
            offset = size
            size = elf.getSymbolByName(section[3]).st_value - elf.getSymbolByName(section[4]).st_value
            hash = hashlib.sha256(sh_extpl.contents[offset:offset+size])
            module_hashes = module_hashes + hash.digest()
            print "module %s rw size %d" % (section[0], size)
            print "    sha256 ", hash.hexdigest()
            print "removing section ", section[0]
            elf.removeSectionByName(section[0])

    hashtbl_sym = elf.getSymbolByName("extpl_module_hashes")
    if hashtbl_sym != const.RC_ERROR:
        ...
        hasbtbl_sect.contents = "".join([
            ...
            module_hashes,
            hasbtbl_sect.contents[offset+len(module_hashes):] ])
