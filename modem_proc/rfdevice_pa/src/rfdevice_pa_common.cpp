...
$Header: //components/rel/rfdevice_pa.mpss/1.30/src/rfdevice_pa_common.cpp#1 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
07/11/19   dh      IOTFE therm mitigation fixes
07/11/19   dh      Added PA sleep call during boot-up sequence 
04/12/19   up      KW error fix in prepare_buffer
04/09/19   up      KW error correction.
02/22/19   dh      Added support for therm sequence of IOTFE therm
01/18/19   dh      Removed re-programming of INIT scripts during Wakeup()
09/25/18   vp      Added support for RFFE device get register address list
09/24/18   dh      Include ASM trigger write as  part of Trigger script during PA GAIN script 
                    building for IOTFE
09/17/18   vp      Added logic to store the object of GRFC PA
09/05/18   dh      Added support of relative delta between trigger writes
07/13/18   dh      Added support for fetching port information of IOTFE PA
06/14/18   vp      Featurization of WCDMA, CDMA and TDSCDMA code
05/23/18   vp      Added support for 16-bit product ID
05/17/17   dh      Added init() script in pa constructor
04/26/17   dh      KW error fix
...
rfdevice_pa_data * rfdevice_pa_data_create(uint16 mfg_id, uint16 prd_id, uint8 rev_id);
...
rfdevice_logical_component* create_gen_pa_object(rfdevice_physical_device *rfdevice_physical_ptr,rfc_logical_device_info_type *logical_device_cfg)
{
  ...
  if(phy_device_cfg->rf_device_comm_protocol == RFDEVICE_COMM_PROTO_RFFE)
  {
  ...
  else if(phy_device_cfg->rf_device_comm_protocol == RFDEVICE_COMM_PROTO_GRFC)
  {
    temp_obj = new rfdevice_grfc_pa(rfdevice_physical_ptr ,phy_device_cfg,logical_device_cfg); /* create the PA device */

    if (temp_obj != NULL )
    {
      init_status = ((rfdevice_grfc_pa *)temp_obj)->init_status;
...
rfdevice_pa_common::rfdevice_pa_common(rfdevice_physical_device *phy_dev_obj_ptr,rfc_phy_device_info_type *phy_device_info, rfc_logical_device_info_type *logical_device_info)
{
  ...
  if((phy_device_info != NULL) && (logical_device_info != NULL) && (logical_device_info->rf_device_type == RFDEVICE_PA))
  {
    ...
    this->is_internal_pa    = FALSE;
    this->pa_trigger_delta = 0;
    this->asm_trigger_delta = 0;
    this->is_asm_trigger_req = FALSE;

    this->pa_data_ptr = check_efs_pa(this->mfg_id, (uint8)this->prd_id, (uint8)this->rev_id);
    if (this->pa_data_ptr == NULL)
    {
      this->pa_data_ptr = rfdevice_pa_data_create(this->mfg_id, this->prd_id, (uint8)this->rev_id);
    }
    if (this->pa_data_ptr == NULL)
    {
    ...
    if (this->pa_data_ptr != NULL)
    {
      this->pa_data_ptr->timing_info_get(&timing_info);
    }
    /*Update is_internal_pa_device flag based on mfg and prd ids*/
    this->update_internal_pa_flag();
    ...
    /* Do  PA INIT after PA object created */
    if(this->pa_data_ptr != NULL)
    {
    init();

      pa_params.trigger_type = RFDEVICE_PA_TRIGGER_NONE;
      sleep(&pa_params, NULL, RFDEVICE_EXECUTE_IMMEDIATE, 0);
    }
    
  }
  else
  {  
    this->rf_device_id      =  RFDEVICE_INVALID;
    this->rf_device_type    =  RFDEVICE_TYPE_INVALID;
    this->instance_num      =  0x0;
    this->rffe_bus          =  0x0; 
    this->default_usid      =  0x0; 
    this->assigned_usid     =  0x0; 
    this->gsid              =  0x0; 
    this->mfg_id            =  0x0; 
    this->prd_id            =  0x0; 
    this->rev_id            =  0x0; 
    ...
    rfdevice_pa_common::parent_physical_device_ptr = NULL;
    this->pa_data_ptr = NULL;
   
  }

  /*Update physical device register address list*/
  update_phy_dev_reg_list();
}
...
//Validity check on requested PA settings
  ...
  if(this->is_internal_pa)
  {
    this->pa_data_ptr->get_port(pa_params_p->lte_cat, pa_params_p->band.lte_band, &port_num);
  }
  else
  {
  ...
  if ((pa_params_p->trigger_type != RFDEVICE_PA_TRIGGER_NONE) && (num_trans != 0))
  {
    ...
    status &= pa_data_ptr->settings_data_get(&pa_params_cfg, &pa_reg_ag);
    status &= prepare_buffer(&pa_reg_ag, script_buffer, execution_type, script_timing, &num_trans, TRUE);
...
//Validity check on requested PA settings
  ...
  /*Prepare the script buffer with the obtained ag settings*/
  status &= prepare_buffer(&pa_reg_ag, script_buffer, execution_type, script_timing, &num_trans, FALSE);
  ...
  if (pa_params_p->trigger_type != RFDEVICE_PA_TRIGGER_NONE)
  {
    ...
    if((execution_type == RFDEVICE_EXECUTE_IMMEDIATE) && (is_internal_pa == TRUE))
    {
      // during DA->PA, ASM trigger should be written ahead of PA trigger by asm_settling_delay
      if (pa_params_p->pa_gain_range == 0)
      {
        this->pa_trigger_delta = this->timing_info.asm_settling_delay;
        this->asm_trigger_delta = 0;
      }
      // during PA->DA, ASM trigger should be offset by asm_trigger_offset
      else if (pa_params_p->pa_gain_range == 1)
      {
        this->pa_trigger_delta = 0;
        this->asm_trigger_delta = this->timing_info.asm_trigger_offset;
      }       
    
    this->is_asm_trigger_req = TRUE;
    }
    ...
    int16 script_timing)
{
  ...
  if (pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_ALL || pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_ON_OFF || pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_TXAGC)
  {
    ...
    status &= prepare_buffer(&pa_settings, script_buffer, execution_type, script_timing, &num_trans, TRUE);
...
/*----------------------------------------------------------------------------*/
/*!
  @Prepare the scripts for PA trigger are required to set PA GAIN range

  @param pa_params_p
  PA config parameter that contains: mode, band ,pa_gain_range, trigger_type
  ...
  returns TRUE if successful else FALSE.
*/
boolean rfdevice_pa_common::pa_gain_range_trigger(rfdevice_pa_cfg* pa_params_p, rf_buffer_intf *script_buffer, rf_device_execution_type execution_type,
    int16 script_timing)
{
  boolean status = TRUE;
  rfdevice_pa_cfg_params_type pa_cfg = {RFDEVICE_PA_DATA_REQ_INVALID,0,0,0};
  ...
  if (pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_ALL || pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_ON_OFF || pa_params_p->trigger_type == RFDEVICE_PA_TRIGGER_TXAGC)
  {
    RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_PA_C, MSG_LEGACY_HIGH,"pa_gain_range_trigger(usid=%d, exec_type=%d, timing=%d)",
                       this->assigned_usid, execution_type, script_timing);
    ...
    {
      this->pa_trigger_delta = 0;
      this->asm_trigger_delta = -(this->timing_info.asm_settling_delay);
    }
    // during PA->DA, ASM trigger should be offset by asm_trigger_offset
    else if (pa_params_p->pa_gain_range == 1)
    {
...
#if 0
    // during DA->PA, ASM trigger should be written ahead of PA trigger by asm_settling_delay
    if (pa_params_p->pa_gain_range == 1)
    {
      this->pa_asm_trigger_delta = -(this->timing_info.asm_settling_delay);
    }
    // during PA->DA, ASM trigger should be offset by asm_trigger_offset
    else if (pa_params_p->pa_gain_range == 0)
    {
      this->pa_asm_trigger_delta = this->timing_info.asm_trigger_offset;
    }
#endif
    ...
    if(!status)
    {
      RF_MSG(RF_ERROR, "pa_gain_range_trigger(): API failed");
      return status;
    }
  }
  return status;
...
void rfdevice_pa_common::init()
{
  ...
  { 
    ...
    pa_params_cfg.req = RFDEVICE_PA_TRIGGER_DATA;  /* PA Trigger reg settings */
    status &= pa_data_ptr->settings_data_get(&pa_params_cfg, &pa_reg_ag);
    status &= prepare_buffer(&pa_reg_ag, NULL, RFDEVICE_EXECUTE_IMMEDIATE, 0, &num_trans, TRUE);
  }
...
)
{
  ...
  {
    print_device_info();
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid band_port_map_info. mode = %d band = %d", mode, band.lte_band);
    return FALSE;
  }
  ...
  {
    print_device_info();
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"Un-supported Tech = %d and band = %d", mode, band.lte_band);
  }
    ...
    boolean is_trigger_data)
{
  ...
  else if (pa_settings->num_regs == 0)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"valid pointer but no register writes, num_regs = %d", 
          pa_settings->num_regs);
    return status;
  }

  // Final script should not exceed the script size
  if((*num_trans + pa_settings->num_regs) >= RFDEVICE_PA_MAX_REG_SIZE)
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"script size exceeds the buffer size, num_trans = %d, num_regs = %d", 
          *num_trans, pa_settings->num_regs);
    return FALSE;
  }

  script_idx = (*num_trans);

  for (uint8 i=0; (i < pa_settings->num_regs) && (script_idx < RFDEVICE_PA_MAX_REG_SIZE); i++)
  {
  ...
  if((*num_trans+1) >= RFDEVICE_PA_MAX_REG_SIZE)
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR,"num_trans %d is out of bounds; Script Size: %d",
          *num_trans, RFDEVICE_PA_MAX_REG_SIZE);
    return FALSE;
  }

  //Maintain relative delta between two trigger writes
  if(this->is_internal_pa == TRUE)
  {
    if(is_trigger_data == TRUE)
    {
      if(this->is_asm_trigger_req == TRUE)
      {
        this->rffe_script[*num_trans].start_delta = timing + this->pa_trigger_delta;
        this->rffe_script[*num_trans+1].start_delta = timing + this->asm_trigger_delta;
        this->pa_trigger_delta = 0;
        this->asm_trigger_delta = 0;
        this->is_asm_trigger_req = FALSE;
      }
...
/*!
  @brief
  interface function for check if PA device is internal

  @details
  Checks if PA device used is internal or external

  @return
  boolean indicating internal or external device
*/
boolean rfdevice_pa_common::is_internal_pa_device()
{
   return this->is_internal_pa;
}

void rfdevice_pa_common::update_internal_pa_flag()
{
  if(this->mfg_id == 0x217 && this->prd_id == 0xC7)
  {
    this->is_internal_pa = TRUE;
  }
}

/*----------------------------------------------------------------------------*/
/*!
  @brief get rffe registers adrress list for PA

  @details
  This function is used to update rffe register list for PA 
*/

void rfdevice_pa_common::update_phy_dev_reg_list(void)
{
  ...
  if (NULL == parent_physical_device_ptr)
  {
    RF_MSG(RF_ERROR, "rfdevice_pa_common::update_phy_dev_reg_list(): parent_physical_device_ptr is NULL");
    return;
  }

  /* call the parent physical device api to initialize the register address list,
  this api will only be triggered the first time it is called */
  
  parent_physical_device_ptr->initialize_reg_addr_list();

  /* Check for NULL pointers */
  if (NULL == parent_physical_device_ptr->rffe_reg_addr_list ||
      NULL == parent_physical_device_ptr->reg_addr_occur_table)
  {
    RF_MSG(RF_ERROR, "rfdevice_pa_common::update_phy_dev_reg_list(): rffe_reg_addr_list or reg_addr_occur_table is NULL");
    
    parent_physical_device_ptr->reg_addr_dump_status = RFDEVICE_REG_ADDR_ERROR_FATAL;
    return;
  }

 
  /* Stop further updating if the status is already ERROR_FATAL */
  if (parent_physical_device_ptr->reg_addr_dump_status == RFDEVICE_REG_ADDR_ERROR_FATAL)
  {
    return;
  }

  /*  Add common register address  */
  for (uint8 address = 0x1d; address <= 0x21; address++)
  {
    if(parent_physical_device_ptr->reg_addr_occur_table[address] == FALSE)
    {
      parent_physical_device_ptr->reg_addr_occur_table[address] = TRUE;
      parent_physical_device_ptr->rffe_reg_addr_list[parent_physical_device_ptr->reg_addr_list_index++] = address;
    }
  }

  boolean setting_data_get_status;
  rfdevice_pa_cfg_params_type pa_cfg;
  rfdevice_pa_reg_settings_type pa_settings = { 0 };

  /*Initialize parameters to get settings*/
  pa_cfg.port = 0;

  /* ======[ Validity Check ]=================================================== */
  if (NULL == this->pa_data_ptr)
  {
    RF_MSG(RF_ERROR, "rfdevice_pa_common::update_phy_dev_reg_list(): NULL data pointer. Returning FALSE.");
    
    parent_physical_device_ptr->reg_addr_dump_status = RFDEVICE_REG_ADDR_ERROR_FATAL;
    return;
  }

  /*Loop through each script type to get register addresses */ 
  for (uint16 script_req = 0; script_req < RFDEVICE_PA_DATA_REQ_INVALID; script_req++)
  {
    pa_cfg.req = (rfdevice_pa_data_req_enum_type)script_req;
    setting_data_get_status = pa_data_ptr->settings_data_get(&pa_cfg, &pa_settings);
    
    /* Not all logical devices have all of the request types, if the status is False, just perform skip */
    if (setting_data_get_status)
    {
      for (uint16 j = 0; j < pa_settings.num_regs; j++)
      {
        //Skip invalid register address 0xFF
        if(pa_settings.addr[j] == 0xFF)
        {
          continue;
        }
	if (parent_physical_device_ptr->reg_addr_list_index >= RFDEVICE_PHY_DEV_REG_LIST_SIZE)
        {
          RF_MSG_1(RF_ERROR, "rfdevice_pa_common::update_phy_dev_reg_list(): "
          "pa %d too many rffe register addresses. Need larger buffer ",
          this->rf_device_id);
        ...
        if (pa_settings.addr[j] < RFDEVICE_PHY_DEV_REG_FLAG_LIST_SIZE)
        {
          if (parent_physical_device_ptr->reg_addr_occur_table[pa_settings.addr[j]] == FALSE)
          {
            /* To remove duplicates for address from 0x00 to 0xFF, set the visited address to TRUE */
            parent_physical_device_ptr->reg_addr_occur_table[pa_settings.addr[j]] = TRUE;
            
            /* Store the current register address to the physical device class */
            parent_physical_device_ptr->rffe_reg_addr_list[parent_physical_device_ptr->reg_addr_list_index++] = pa_settings.addr[j];
  ...
  parent_physical_device_ptr->reg_addr_dump_status = RFDEVICE_REG_ADDR_SUCCESS;
...
/*!
  @brief
  Function to execute qtherm sequence    

  @details
  Function to execute pa therm sequence for IOTFE
...
*/
boolean rfdevice_pa_common::read_pa_therm(rfdevice_read_imm_cfg* therm_read_cfg_p)
{
  boolean status = TRUE;
  ...
  this->pa_data_ptr->execute_therm_sequence(this->rffe_bus, this->assigned_usid);
...
boolean rfdevice_pa_common::execute_pa_on_for_therm()
{
  boolean status = TRUE;
  ...
  this->pa_data_ptr->execute_pa_on_therm_sequence(this->rffe_bus, this->assigned_usid);
...
boolean rfdevice_pa_therm_rffe_out_single
(
  uint8 bus_id,
  uint8 slave_id,
  uint16 reg,
...
{
  rf_hal_bus_resource_script_settings_type settings = { 1, FALSE, FALSE, FALSE, FALSE };
  
  rf_hal_bus_rffe_type script;
  script.channel = bus_id;
  script.slave_id = slave_id;
  script.addr = reg;
  script.num_bytes = 1;
  script.half_rate = settings.half_rate;
  script.rd_delay = settings.rd_delay;
  script.start_delta = 0;
  script.data[0] = value;
  script.rd_wr = RF_HAL_BUS_WRITE; 

  return (rfhal_rffe_write_cmn(&script,bus_id,FALSE,1,&settings) == RF_HAL_BUS_SUCCESS);
