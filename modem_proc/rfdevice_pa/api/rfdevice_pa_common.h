...
$Header: //components/rel/rfdevice_pa.mpss/1.30/api/rfdevice_pa_common.h#1 $
$Author: pwbldsvc $
...
/* Constructor overloaded for physical device support */
  rfdevice_pa_common(rfdevice_physical_device *phy_dev_obj_ptr,rfc_phy_device_info_type *phy_device_info, rfc_logical_device_info_type *logical_device_info); 
        ...
        boolean pa_gain_range_trigger(rfdevice_pa_cfg* pa_params_p, rf_buffer_intf *script_buffer, rf_device_execution_type execution_type,
            int16 script_timing);
        ...
        boolean is_internal_pa_device();

        boolean read_pa_therm(rfdevice_read_imm_cfg* therm_read_cfg_p);

        boolean execute_pa_on_for_therm();
    ...
    protected:
        ...
        rfdevice_pa_timing_info_type timing_info;
        int16 pa_trigger_delta; /* relatvie timing delay required between triggers writes of PA & ASM*/
        int16 asm_trigger_delta; /* relatvie timing delay required between triggers writes of PA & ASM*/
        boolean is_asm_trigger_req;
    ...
    private:
        ...
        boolean prepare_buffer(rfdevice_pa_reg_settings_type *settings, rf_buffer_intf *script_buffer,
                               rf_device_execution_type execution_type, int16 timing, uint8 *num_trans, boolean is_asm_trigger_required = FALSE);
        ...
        void update_internal_pa_flag();

        boolean is_internal_pa; /*flag to indicate whether is it IOTFE PA*/
  
    };

  boolean rfdevice_pa_therm_rffe_out_single(uint8 bus_id, uint8 slave_id, uint16 reg, uint8 value);
