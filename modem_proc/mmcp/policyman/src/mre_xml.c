...
/*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/policyman/src/mre_xml.c#1 $
...
#define UT_VALIDATE(pred, str, a, b, c) 
...
{
  boolean haveTag = mre_optional_tag(ppDoc, ppTag);
  ...
  )
{
  ...
  *ppElem = (mre_xml_element_t *) mre_mem_alloc(sizeof(mre_xml_element_t));
  ...
  MALFORMED_IF    (!mre_require_tag(ppDoc, &(*ppElem)->pTag));

  status = mre_optional_attributes(ppDoc, *ppElem);
  DONE_IF(status == MRE_STATUS_ERR_MALFORMED_XML);
  
  MALFORMED_IF    (!mre_consume_past(ppDoc, "?>"));
...
{
  mre_status_t status = mre_optional_processing(ppDoc, ppElem);

  return (status == MRE_STATUS_SUCCESS)?
                        MRE_STATUS_SUCCESS
  ...
  )
{
  ...
  NOT_PRESENT_IF  (!mre_optional_tag(ppDoc, &pAttrib->pName));
  ...
  MALFORMED_IF    (!mre_require_string(ppDoc, &pAttrib->pValue));
  ...
  )
{
  ...
  while (TRUE)
  {
    status = mre_optional_attribute(ppDoc, &attrib);
    ...
    {
      pElem->pAttributes = mre_set_new(
                              sizeof(mre_xml_attribute),
                              5,
                              MRE_SET_NO_MAX_SIZE,
                              NULL,
                              mre_attribute_free
  ...
  )
{
  ...
  while (!END_OF_DOC && !mre_lookahead(ppDoc, "</"))
  {
     ...
     */
    status = mre_optional_element(ppDoc, &pNewElem);
    ...
    {
      pElem->pChildren = mre_set_new(
                            sizeof(mre_xml_element_t *),
                            10,
                            MRE_SET_NO_MAX_SIZE,
                            NULL,
                            mre_child_free
  ...
  while (mre_consume_comment(ppDoc));
}
  ...
  )
{
  ...
  if (NEXT_CHAR != '<')
  {
    mre_collect_text(ppDoc, &pElem->pText, '<');
    mre_trim_trailing_whitespace(pElem->pText);
  }
  else
  {
    mre_optional_elements(ppDoc, pElem);
  }
  ...
  )
{
  ...
  *ppElem = (mre_xml_element_t *) mre_mem_alloc(sizeof(mre_xml_element_t));
  ...
  status = mre_require_element_content(ppDoc, *ppElem);
...
{
  mre_status_t status = mre_optional_element(ppDoc, ppElem);
  ...
  )
{
  ...
  *ppRoot = pRoot;
  ...
  MRE_XML_MSG("###################################################", 0, 0, 0);
  MRE_UT_MSG("Beginning XML parsing", 0, 0, 0);

  /*  Optionally consume a UTF-8 BOM.
   */
  if (mre_consume(ppDoc, bom))
  {
    MRE_UT_MSG("Skipped BOM at start of XML", 0, 0, 0);
  }

  /*  Get the XML information and validate it.
   */
  status = mre_require_processing(ppDoc, &pRoot->pHeader);
  DONE_IF(status != MRE_STATUS_SUCCESS);
  {
    char const  *pStr;
    ...
    UT_VALIDATE((pStr != NULL && strcmp(pStr, "xml") == 0),
                "File must start with an \"?xml\" element", 0, 0, 0
               );
    ...
    UT_VALIDATE((pStr != NULL && (strcmp(pStr, "UTF-8") == 0 || strcmp(pStr, "ISO-8859-1") == 0)),
                "encoding must be UTF-8 or ISO-8859-1", 0, 0, 0
               );
   ...
   */
  status = mre_require_element(ppDoc, &pRoot->pDocument);
...
{
  return (pElement == NULL || pElement->pChildren == NULL)?
                0
