...
/*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/policyman/src/mre_efs.c#1 $
...
)
{
  ...
  */
  ...
  {
    MRE_MSG_ERROR_3( "Error %d from mcfg_fs_stat(sub %d), efs_err %d",
                     mcfg_status,
                     mcfg_subs,
  ...
  */
  ...
  if (MCFG_FS_STATUS_OK != mcfg_status)
  {
    MRE_MSG_ERROR_4( "Error %d from mcfg_fs_read(%d bytes, sub %d), efs_err %d",
                     mcfg_status,
                     *pSize,
                     mcfg_subs,
                     mcfg_fs_errno(MCFG_FS_TYPE_EFS) );
...
)
{
  ...
  if (MCFG_FS_STATUS_OK != mcfg_status)
  {
    MRE_MSG_ERROR_4( "Error %d from mcfg_fs_write(%d bytes, sub %d), efs_err %d",
                     mcfg_status,
                     dataSize,
                     mcfg_subs,
                     mcfg_fs_errno(MCFG_FS_TYPE_EFS) );
