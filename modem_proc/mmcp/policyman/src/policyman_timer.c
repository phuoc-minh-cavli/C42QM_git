...
/*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/policyman/src/policyman_timer.c#1 $
...
{
  return (policyman_timer_t *) mre_named_object_find_with_subs(
                                  pName,
  ...
  )
{
  if (pTimer != NULL)
  {
    ...
    POLICYMAN_MSG_HIGH_2("timer id %d has new interval %d", pTimer->id, pTimer->interval);
  }
}
...
/*-------- policyman_timer_get_policy_id --------*/
uint32
policyman_timer_get_policy_id(
  policyman_timer_t *pTimer
  )
{
  return (pTimer == NULL) ? 0 : policyman_timer_get_policy(pTimer)->id;
}
  ...
  )
{
  ...
  if (!timer_is_active(&pTimer->timer))
  {
    ...
    POLICYMAN_MSG_HIGH_3("Starting timer %d for subs %d with duration %d secs",
                            pTimer->id,
                            pTimer->asubs_id,
                            pTimer->interval );
  }
  ...
  )
{
  ...
  if (timer_is_active(&pTimer->timer))
  {
    ...
    POLICYMAN_MSG_HIGH_2("Stopping timer %d for subs %d", pTimer->id, pTimer->asubs_id);
  }
...
)
{
  ...
  POLICYMAN_MSG_HIGH_2("Updating backoff timer: new index = %d, new interval = %d", pTimer->tableIndex, pTimer->interval);
...
)
{
  ...
  POLICYMAN_MSG_HIGH_2("Process expired timer %d for subs %d", pTimer->id, pTimer->asubs_id);
  ...
  {
    policyman_state_handle_update(policyman_timer_get_subs(pTimer));
  }
  ...
  )
{
  ...
  pCmd = (policyman_timer_cmd_t *) policyman_cmd_new(
                                      sizeof(policyman_timer_cmd_t),
                                      policyman_execute_process_timer_evt,
                                      NULL,
...
)
{
  ...
  if (success)
  {
    ...
    switch(timer_type)
    {
      ...
      default:
        POLICYMAN_MSG_ERROR("timer type %d not handled", timer_type, 0, 0);
        pTimerGroup = NULL;
        break;
    }
...
)
{
  ...
  {
    pCond->pTimer = (policyman_timer_t *) mre_named_object_find(
                            pCond->pTimerName,
                            POLICYMAN_NAMED_TIMER,
                            pCondition->pPolicy
                            );
  ...
  if (pCond->pTimer != NULL)
  {
    ...
    POLICYMAN_MSG_HIGH_3("condition <timer_expired> for subs %d timer %d returns %d", pCond->pTimer->asubs_id, pCond->pTimer->id, result);
  }
  else
  {
    POLICYMAN_MSG_HIGH_1("condition <timer_expired> for no timer returns %d", result);
  }
  return result;
}
  ...
  )
{
  policyman_timer_expired_condition_t *pCondition =
                                    (policyman_timer_expired_condition_t *) pObj;
  ...
  )
{
  ...
  policyman_timer_expired_condition_t *pCondition = NULL;
  char const                          *pStr;

  /*  Allocate the condition
   */
  pCondition = (policyman_timer_expired_condition_t *)
                  policyman_mem_alloc(sizeof(policyman_timer_expired_condition_t));
  ref_cnt_obj_init(pCondition, policyman_condition_time_expired_dtor);
  pCondition->isTrue = policyman_condition_timer_expired_evaluate;
...
Done:
  ...
  *ppCondition = (mre_condition_t *) pCondition;
  ...
  POLICYMAN_MSG_MED("policyman_timer_expired_condition_t returned status %d", status, 0, 0);
  ...
  )
{
  ...
  if (pPolicy == NULL)
  {
    POLICYMAN_MSG_ERROR("Internal error - executing new timer action with no policy", 0, 0, 0);
    return FALSE;
  }
  ...
  policyman_named_object_init(
            (policyman_named_object_t *) pTimer,
  ...
  if (!succeeded)
  {
    POLICYMAN_MSG_HIGH("Couldn't initialize timer %d", pTimer->id, 0, 0);
    POLICYMAN_UT_MSG("Couldn't initialize timer %s", pTimer->pName, 0, 0);
  }
  ...
  )
{
  ...
  if (pStr == NULL)
  {
    POLICYMAN_UT_MSG("<define_timer>: needs a 'units' attribute", 0, 0, 0);
    goto Done;
  }
  ...
  {
    POLICYMAN_UT_MSG("<define_timer>: invalid units for timer: %s", pStr, 0, 0);
    POLICYMAN_UT_MSG("Valid values are 'min' and 'sec'", 0, 0, 0);
  }
...
/*-------- get_timer_class --------*/
static policyman_timer_class_t
get_timer_class(
  policyman_xml_element_t const *pElem
  )
{
  ...
  else
  {
    ...
    POLICYMAN_UT_MSG("<define_timer>: invalid class for timer: %s", pStr, 0, 0);
    POLICYMAN_UT_MSG("Valid values are 'fixed' and 'backoff'", 0, 0, 0);
  }
  ...
  )
{
  ...
  {
    POLICYMAN_UT_MSG("<define_timer>: fixed interval timer must not have text", 0, 0, 0);
    goto Done;
  }
  ...
  if (pStr == NULL)
  {
    POLICYMAN_UT_MSG("<define_timer>: fixed interval timer must have an 'interval' attribute", 0, 0, 0);
    goto Done;
  }
  ...
  if (interval == 0)
  {
    POLICYMAN_UT_MSG("<define_timer>: invalid interval for timer: %s", pStr, 0, 0);
  }
  ...
  )
{
  ...
  if (pStr != NULL)
  {
    POLICYMAN_UT_MSG("<define_timer>: 'interval' attribute not allowed for backoff timer", 0, 0, 0);
    goto Done;
  }
  ...
  if (pStr == NULL)
  {
    POLICYMAN_UT_MSG("<define_timer>: backoff timer requires a table of intervals", 0, 0, 0);
    goto Done;
  }
  ...
  while (policyman_get_token(&pStr, token, sizeof(token)))
  {
    ...
    if (interval == 0)
    {
      POLICYMAN_UT_MSG("<define_timer>: invalid interval for backoff interval: %s", token, 0, 0);
  ...
  )
{
  ...
  */
  ...
  if (pName == NULL)
  {
    POLICYMAN_UT_MSG("<define_timer>: needs a 'name' attribute", 0, 0, 0);
    goto Done;
  }
   ...
   */
  pAction = (named_timer_action_t *)
                policyman_mem_alloc(sizeof(named_timer_action_t));
  ref_cnt_obj_init(pAction, policyman_named_timer_action_dtor);
  pAction->execute = policyman_timer_new_action_execute;
...
Done:
  POLICYMAN_MSG_HIGH_1("<define_timer> returned status %d", status);
  return status;
}
  ...
  )
{
  ...
  if (pName == NULL)
  {
    POLICYMAN_UT_MSG("<%s> needs a 'name' attribute", pTag, 0, 0);
    goto Done;
  }

  pAction = (timer_action_t *) policyman_mem_alloc(sizeof(timer_action_t));
  ref_cnt_obj_init(pAction, policyman_timer_action_dtor);
  pAction->execute = pfnExecute;
  ...
  )
{
  ...
  if (pTimer == NULL && pTimerAction->pTimerName != NULL)
  {
    ...
    policyman_mem_free((void *) pTimerAction->pTimerName);
    pTimerAction->pTimerName = NULL;
    if (pTimer != NULL)
    {
  ...
  )
{
  ...
  if (pTimer != NULL)
  {
    POLICYMAN_MSG_HIGH_1("executing <expired_timer_handled> on timer %d", policyman_timer_get_id(pTimer));
    policyman_timer_set_handled(pTimer);
    succeeded = TRUE;
  }
  else
  {
    POLICYMAN_MSG_HIGH_0("No timer found for <expired_timer_handled>");
  }
  ...
  )
{
  ...
  if (pTimer != NULL)
  {
    POLICYMAN_MSG_HIGH_1("executing <timer_start> on timer %d", policyman_timer_get_id(pTimer));
    policyman_timer_start(pTimer);
    succeeded = TRUE;
  }
  else
  {
    POLICYMAN_MSG_HIGH_0("No timer found for <timer_start>");
  }
...
policyman_timer_stop_execute(
  ...
  if (pTimer != NULL)
  {
    POLICYMAN_MSG_HIGH_1("executing <timer_stop> on timer %d", policyman_timer_get_id(pTimer));
    policyman_timer_stop(pTimer);
    succeeded = TRUE;
  }
  else
  {
    POLICYMAN_MSG_HIGH_0("No timer found for <timer_stop>");
  }
  ...
  )
{
  ...
  /* No policy associated with netscan timer, runs for all policies
   */
  ...
  if (!succeeded)
  {
    POLICYMAN_MSG_HIGH_1("Couldn't initialize timer %d", pTimer->id);
