...
/*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/policyman/src/policyman_msgr.c#1 $
...
*/
static policyman_msgr_reg_entry_t
*policyman_msgr_set_find(
  msgr_umid_type  umid
  )
{
  ...
  )
{
  ...
  {
    POLICYMAN_MSG_ERROR("item not added to set, index %d found %d", i, found, 0);
  }
...
)
{
  ...
  if (err == E_SUCCESS) 
  { 
    POLICYMAN_MSG_HIGH("umid %d registered with MSGR", umid, 0, 0);
    policyman_msgr_set_add(umid, pfnCallback, pContext);
  }
  else
  {
    POLICYMAN_MSG_ERROR("Unable to register for UMID 0x%x with MSGR err %d", umid, err, 0);
  }
}
  ...
  )
{
  ...
  POLICYMAN_MSG_HIGH("msgr_client_create returned %d", msgr_err, 0, 0 );
  if (msgr_err != E_SUCCESS)
  {
    POLICYMAN_MSG_ERROR("Unable to create MSGR client", 0, 0, 0);
    return;
  }
 
  msgr_err = msgr_client_set_rex_q(
  ...
  POLICYMAN_MSG_HIGH("msgr_client_set_rex_q returned %d", msgr_err, 0, 0 );
  ...
  )
{
  ...
  POLICYMAN_MSG_HIGH("Processing MSGR queue", 0, 0, 0 );
  ...
  while ((pEntry = q_get(&policyman_msgr_q)) != NULL)
  {
    ...
    {
      POLICYMAN_MSG_HIGH("msg id %d found in set, calling callback function", pMsg->id, 0, 0);
