...
/*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/policyman/src/mre_regex.c#1 $
...
*/
typedef boolean (*regex_match_fn_t)(regex_elem_data_t *pData, char c);
  ...
  )
{
  ...
  regex_elem_t      elem;
  mre_regex_t *pRegEx = mre_set_new(
                                sizeof(regex_elem_t),
                                12,
                                MRE_SET_NO_MAX_SIZE,
                                regex_elem_compare,
...
{
  boolean isMatch = mre_set_iterate(pRegEx, regex_do_match, (char *) &pStr, NULL);
