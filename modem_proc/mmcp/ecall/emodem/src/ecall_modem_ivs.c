...
$Header: //components/rel/mmcp.mpss/6.1.10/ecall/emodem/src/ecall_modem_ivs.c#1 $
...
/* Stores the MSD state in modem*/
typedef enum 
{
  ECALL_MODEM_HAS_MSD_DATA = 0,      /* Modem has MSD and is ready to transfer after RX start frames*/
  ECALL_MODEM_HAS_NO_MSD_DATA,       /* Modem does not have MSD, after RX start frames App responsibility to give MSD*/
  ECALL_MODEM_WAITING_FOR_MSD_DATA,  /* Modem requested MSD from APP, waiting for response */
  ECALL_MODEM_RCVD_MSD_DATA          /* Modem received MSD from APP, in case of additional data request */
}ecall_modem_msd_state;
...
LOCAL struct
{   
  ecall_modem_msd_state  ecall_msd_state;
...
)
{
  ...
  else if(ecall_modem_ivs.ecall_msd_state == ECALL_MODEM_WAITING_FOR_MSD_DATA)
  {
    /* We move to WAITING_FOR_MSD after receiving 3 start messages, once modem has the MSD
       the MSD from APP state changes to HAS_MSD after the next start message IVS can start sending MSD */
    ECALL_MSG_0( ECALL_HIGH, "IVS: received START !> ignoring message waiting for MSD from APP" );
  }
  else if(ecall_modem_ivs.ecall_msd_state == ECALL_MODEM_RCVD_MSD_DATA )
  {
    ECALL_MSG_0( ECALL_HIGH, "IVS: received START  ECALL_MODEM_RCVD_MSD_DATA to ECALL_MODEM_HAS_MSD_DATA" );
    ecall_modem_ivs.ecall_msd_state = ECALL_MODEM_HAS_MSD_DATA;
  }
  else 
  {
  ...
  { 
    if( ecall_modem_ivs.ecall_msd_state == ECALL_MODEM_HAS_MSD_DATA )
    {
    ...
    {
      ECALL_MSG_1( ECALL_HIGH, "IVS: received START (metric %d), waiting for MSD from APP..",
                                metric );

      if(ecall_modem_ivs.ecall_msd_state != ECALL_MODEM_WAITING_FOR_MSD_DATA)
      {
        // Sends NO_MSD to APP once and waits until it get the MSD data
        ecall_modem_ivs.ecall_msd_state = ECALL_MODEM_WAITING_FOR_MSD_DATA;
    ...
    if(ecall_modem_ivs.tx_mode == PULL_MODE)
    {
      /* For mulitple MSD transmission if the modem already has the MSD,
         it has to register TX and transmit MSD */
      #if ( (defined FEATURE_ECALL_HAS_VOICE_A) || (defined FEATURE_ECALL_HAS_VOICE_B) )
      ...
      if(ecall_voc_tx_register() == FALSE)
      {
        ECALL_MSG_0( ECALL_ERROR, "post ECALL_IVS_ERR notfication " );
        ecall_ivs_status.tx_status = ECALL_IVS_ERR;
        ecall_post_client_notify();               
        return;
      }
...
)
{
   ...
   ecall_modem_ivs.ecall_msd_state      = ECALL_MODEM_HAS_NO_MSD_DATA;
...
)
{
  ...
  if(ecall_modem_ivs.ecall_msd_state == ECALL_MODEM_WAITING_FOR_MSD_DATA)
  {
    ECALL_MSG_0( ECALL_HIGH, "ecall_start ECALL_MODEM_WAITING_FOR_MSD_DATA to ECALL_MODEM_RCVD_MSD_DATA" );
    ecall_modem_ivs.ecall_msd_state = ECALL_MODEM_RCVD_MSD_DATA;
  }
  else
  {
    ECALL_MSG_0( ECALL_HIGH, "ecall_start ECALL_MODEM_HAS_MSD_DATA " );
    ecall_modem_ivs.ecall_msd_state = ECALL_MODEM_HAS_MSD_DATA;
  }
}
...
/*==========================================================================
  ...
  FUNCTION ECALL_VOICE_API

  DESCRIPTION
    Provides a single API entry point into the ECALL module.

  DEPENDENCIES
    FEATURE_ECALL_HAS_SINGLE_VOICE_SESSION
  ...
  RETURN VALUE
    ...
    @return                ECALL_VOICE_EVERSION
                           eCall Single Voice Session feature not defined
