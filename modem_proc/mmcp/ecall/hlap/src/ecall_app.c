...
$Header: //components/rel/mmcp.mpss/6.1.10/ecall/hlap/src/ecall_app.c#1 $
...
)
{
  ...
  if (cmd_ptr != NULL)
  {
    ECALL_MSG_2( ECALL_HIGH, "SESSION_QMI_START: type_of_call %d, activation_type %d",
                              type_of_call,
                              activation_type );
...
)
{
  ...
  if (cmd_ptr != NULL)
  {
    ECALL_MSG_3( ECALL_HIGH, "SESSION_QMI_START_PER_SUBS: type_of_call %d, activation_type %d, as_id %d",
                              type_of_call,
                              activation_type,
                              as_id );
    cmd_ptr->data.start_session_config.qmi_start = TRUE;
    ...
    cmd_ptr->data.start_session_config.data_block_ptr = data_block_ptr;
    cmd_ptr->data.start_session_config.as_id = as_id;
    /* Post a command to the eCall App task to process this session_start */
    cmd_ptr->cmd_id = ECALL_APP_CMD_SESSION_START;
    ecall_app_task_put_cmd(cmd_ptr);
    return TRUE; /* Command queued up successfully*/
