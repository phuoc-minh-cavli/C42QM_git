...
$Header: //components/rel/mmcp.mpss/6.1.10/api/mm_rr.h#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  ,RR_MTC_PSM_EDRX_PARAMS_UPDATE_IND  = 0x15
  ,RR_GMM_READY_TIMER_STATUS_IND   = 0x16
  ,RR_DEVICE_PRIORITY_IND          = 0x17
  ,RR_WWAN_SET_PRIORITY_IND        = 0x18
  ...
  ,RR_PS_IMPLICIT_REJECT_IND       = 0x9f
...
typedef struct
{
 ...
 typedef struct rr_mtc_nw_config_param_update_ind_s
{
  IMH_T                    message_header;
  boolean                  psm_enabled;
  ...
  sys_modem_as_id_e_type   as_id;
} rr_mtc_nw_config_param_update_ind_T;

 typedef struct rr_gmm_ready_timer_status_ind_s
 {
   IMH_T                    message_header;
   boolean                  timer_active ;
   sys_modem_as_id_e_type   as_id;
 } rr_gmm_ready_timer_status_ind_T;

 
 typedef enum
 {
   MM_NONE_PRIORITY = -1, /*FOR INTERNAL USE ONLY!*/
   MM_LOW_PRIORITY, 
   MM_HIGH_PRIORITY,
 } mm_priority_indicator_t;

 
 typedef struct rr_device_priority_ind_s
 {
   IMH_T                      message_header;
   mm_priority_indicator_t    device_priority;
   boolean                    eab_enabled; /*!< Boolean to indicate whether EAB is enabled or not*/
   boolean                    eab_cat_b;   /*!< Boolean to indicate whether UE belongs to EAB category B */
   boolean                    eab_cat_c;   /*!< Boolean to indicate whether UE belongs to EAB category C */
   
   sys_modem_as_id_e_type     as_id;
 } rr_device_priority_ind_T;


 typedef struct rr_ps_implicit_reject_ind_s
 
 {
   IMH_T                     message_header;
   dword                     wait_time; 
   sys_modem_as_id_e_type    as_id;
 } rr_ps_implicit_reject_ind_T;


/*************************************************************
* RR_WWAN_SET_PRIORITY_IND   primitive
* FROM : MM
* TO   : RR
* DESCRIPTION :
* Used by MM to inform high priority indication for Stop mode procedures like OTA PS Detach, 
    Deactivation, RRC connection abort to prioritize WWAN tech over GPS.
*************************************************************/
typedef struct rr_wwan_set_prioirity_ind_s 
{
   IMH_T                        message_header;
   boolean                      high_priority;   /*TRUE if WWAN tech is prioritized over GPS for time critical procedure otherwise FALSE*/
} rr_wwan_set_prioirity_ind_T;
...
typedef union
{
  ...
  rr_mtc_nw_config_param_update_ind_T rr_mtc_nw_config_param_update_ind;
