...
$Header: //components/rel/mmcp.mpss/6.1.10/api/esm_cm_msg_if.h#1 $
...
typedef struct 
{
  ...
  uint8                        seq_num;
} esm_data_transport_req_T;

/* This structure is used to send bearer context restore request to NAS via CM
   upon wake up from PSM */
typedef struct 
{
  msgr_hdr_s                  msg_hdr;
  eps_bearer_id_T             eps_bearer_id;
...
} esm_restore_bearer_ctxt_req_T;


/* This structure is used to send a response to NAS via CM after restoring
   all bearer contexts in DS upon wake up from PSM */
typedef struct 
{
  msgr_hdr_s  msg_hdr;
  boolean     status;
} esm_restore_ctxt_psm_rsp_T;
