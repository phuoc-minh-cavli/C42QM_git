...
$Header: //components/rel/mmcp.mpss/6.1.10/api/emm_irat_if_msg.h#1 $
...
typedef enum
{
  ...
  LTE_NAS_IRAT_ATTACH_REJECT,
  LTE_NAS_IRAT_T3410_EXPIRY
}emm_irat_failure_cause_type ;

typedef enum
{
  EMM_INVALID_STATE_PERSISTENT = 0,
  EMM_INVALID_STATE_REG_IN_PROGRESS,
  EMM_INVALID_STATE_TEMP,
  EMM_INVALID_STATE_CPSR_IN_PROGRESS
}emm_invalid_state_e;
...
typedef union
{
  ...
  lte_nas_emm_cause_type         emm_cause;
    ...
    /**< Detailed invalid cause*/
  emm_invalid_state_e            state;
...
typedef struct
{
  boolean esm_data_transport;
  boolean status;
}esm_data_transport_status_type;
