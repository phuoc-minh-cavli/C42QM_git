...
/**===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/api/sys_v.h#1 $
...
typedef enum
{
  ...
  SYS_SERVICE_SEARCH_CELL_SEARCH,   /* Cell param search*/
  SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH,
...
typedef struct
{
  ...
  boolean                er_without_pdn_supported;
...
typedef enum {
  ...
  ** purpose only */
  ...
  /* Nas will use this during LTE RLF to prioritize RPLM/EPLMN 
  ** after suitable search end as well, to avoid call drop 
  */
  SYS_ADDTNL_INFO_IMS_CALL_CONNECTED,

  /* For internal use and range check
  */

  SYS_ADDTNL_INFO_START_MO_EXCEPTION,
  /*Indicate to NAS that this srv_req is for MO exception data*/

   #ifdef FEATURE_FMD_SPEED_INFO
  SYS_ADDTNL_INFO_PSEUDO_ONLINE_MODE,
  /* Indicate to NAS this srv_req is to enter into pseudo online mode*/
  #endif
  SYS_ADDTNL_INFO_MAX
...
/* 1xSRLTE mode continue with activation
 */
  SYS_ACTIVATION_ALLOWED_CONTINUE = 0,

/* G only or active rat is None, rat pri list has
** G only, wait for 1 sec and continue activation
 */
