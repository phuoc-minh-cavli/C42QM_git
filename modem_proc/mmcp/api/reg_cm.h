...
$Header: //components/rel/mmcp.mpss/6.1.10/api/reg_cm.h#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  ,CM_DDS_SWITCH_IND
  ,CM_PSM_READY_REQ
  ,CM_PSM_SET_CONFIG_PARAMS_REQ
  ,CM_PSM_GET_TIMER_INFO_REQ
  ,CM_SET_EDRX_CFG_REQ
  ,CM_GET_EDRX_PARAMS_REQ
  ,CM_ENTER_PSM_MODE_REQ    
  ,CM_MCC_CHANGE_IND
  ,CM_PSM_SAVE_MMGSDI_EF_REQ
} cm_to_reg_cmd_type;
...
typedef enum
{
  ...
  REG_CM_NETWORK_LIST_FAIL_IN_CONNECTED_MODE,  
...
typedef enum 
{
  ...
  REG_CM_NETWORK_LIST_REQ_GPS_ADVANCE_SEARCH,/* GPS advance search request  */
...
typedef struct cm_network_list_req_s
{
  ...
  ** in bplmn search by lower layers*/  
  ...
  uint8                                               gsp_wwan_scan_mask;
  /*  search parameter send by GPS for scan purpose  */
...
/*                                                                            */
/******************************************************************************/
typedef struct cm_mcc_change_ind_s
{  
  IMH_T                          message_header;  
  uint32                          mcc;
} cm_mcc_change_ind_s_type;
...
typedef struct
{
  IMH_T  message_header;
  dword  t3402_value;
} cm_nas_t3402_changed_ind_s_type;
...
typedef struct cm_reg_reject_ind_s
{
  ...
  sys_radio_access_tech_e_type                        rat;
  sys_lac_type                                        tac;
  sys_sys_mode_e_type                                 ciot_lte_mode;	 /* LTE mode of operation */  
  sys_csg_id_type                                     csg_id;
  sys_modem_as_id_e_type                              as_id;
  sys_modem_stack_id_e_type                          stack_id;
...
/**************************************************************/
/* CM_PSM_AUTO_READY_IND                                      */
/* Primitive to inform CM that NAS is ready to enter PSM due  */
/* to active timer expiry                                     */ 
/**************************************************************/
typedef struct cm_psm_auto_ready_ind_s
{
  IMH_T                   message_header;
  sys_modem_as_id_e_type  as_id;
  uint32                  periodic_update_timer_value;
}cm_psm_auto_ready_ind_s_type;


/*****************************************************************/
/* CM_PSM_TIMER_CHANGE_IND                                       */
/* Primitive to inform CM that NW didn't accept the active timer */
/* and/or periodic update timer values negotiated by the UE in   */
/* attach/tau request message and provided alternate values in   */
/* attach/tau accept message to the UE                           */ 
/*****************************************************************/
typedef struct cm_psm_timer_change_ind_s
{
  IMH_T                   message_header;
  sys_modem_as_id_e_type  as_id;
  sys_radio_access_tech_e_type  active_rat;
...
}cm_psm_timer_change_ind_s_type;


/************************************************************/
/* CM_PSM_SET_CONFIG_PARAMS_REQ                             */
/* Primitive to set/change PSM configuration params such as */
/* psm_enabled/active timer/periodic update timer           */
/************************************************************/
typedef struct cm_psm_set_config_params_req_s
{
  IMH_T                   message_header;
  sys_modem_as_id_e_type  as_id;
  boolean                 psm_enabled;
  ...
  uint32                  gprs_ready_timer_value;
  uint16                  change_mask;
}cm_psm_set_config_params_req_s_type;


/***************************************************************************/
/* CM_PSM_SET_CONFIG_PARAMS_RSP                                            */
/* Primitive to acknowledge that NAS received CM_PSM_SET_CONFIG_PARAMS_REQ */
/***************************************************************************/
typedef struct cm_psm_set_config_params_rsp_s
{
  IMH_T                   message_header;
  sys_modem_as_id_e_type  as_id;
}cm_psm_set_config_params_rsp_s_type;


/********************************************************************/
/* CM_PSM_GET_TIMER_INFO_REQ                                        */
/* Primitive to get active timer and periodic update timer from NAS */
/********************************************************************/
typedef struct cm_psm_get_timer_info_req_s
{
  IMH_T                    message_header;
  sys_modem_as_id_e_type   as_id;
  boolean                  user_config;   /* TRUE - User Configured values ; FALSE - Network Provided values */
}cm_psm_get_timer_info_req_s_type;


/********************************************************************/
/* CM_PSM_GET_TIMER_INFO_RSP                                        */
/* Primitive to send active timer and periodic update timer to CM   */
/********************************************************************/
typedef struct cm_psm_get_timer_info_rsp_s
{
  IMH_T                       message_header;
  sys_modem_as_id_e_type      as_id;
  uint32                      active_timer_value;
  ...
  uint32                      periodic_rau_timer_value;
  uint32                      gprs_ready_timer_value;
}cm_psm_get_timer_info_rsp_s_type;
...
/****************************************************/
typedef struct cm_enter_psm_mode_req_s
{
  IMH_T                  message_header;
  sys_modem_as_id_e_type as_id;
}cm_enter_psm_mode_req_s_type;
...
/* Primitive to trigger context saving to enter PSM */ 
/****************************************************/
typedef struct cm_psm_save_mmgsdi_ef_req_s
{
  IMH_T                  message_header;
  sys_modem_as_id_e_type as_id;
}cm_psm_save_mmgsdi_ef_req_s_type;

/***********************************************************************/
/* CM_PSM_SAVE_MMGSDI_EF_RSP                                           */
/* Primitive to inform CM with the status of CM_PSM_SAVE_MMGSDI_EF_REQ */ 
/***********************************************************************/
typedef struct cm_psm_save_mmgsdi_ef_rsp_s
{
  IMH_T                      message_header;
  boolean                    status;
  sys_modem_as_id_e_type     as_id;
}cm_psm_save_mmgsdi_ef_rsp_s_type;


/*******************************************************************/
/* CM_ENTER_PSM_MODE_RSP                                           */
/* Primitive to inform CM with the status of CM_ENTER_PSM_MODE_REQ */ 
/*******************************************************************/
typedef struct cm_enter_psm_mode_rsp_s
{
  IMH_T                      message_header;
  boolean                    status;
...
/*************************************************************/
typedef struct cm_set_edrx_cfg_req_s
{
  IMH_T                        message_header;
  sys_modem_as_id_e_type       as_id;
  ...
  uint16                       change_mask;
}cm_set_edrx_cfg_req_s_type;


/******************************************************************/
/* CM_SET_EDRX_CFG_RSP                                            */
/* Primitive to acknowledge that NAS received CM_SET_EDRX_CFG_REQ */
/******************************************************************/
typedef struct cm_set_edrx_cfg_rsp_s
{
  IMH_T                   message_header;
  sys_modem_as_id_e_type  as_id;
}cm_set_edrx_cfg_rsp_s_type;
...
/********************************************************************/
typedef struct cm_get_edrx_params_req_s
{
  IMH_T                        message_header;
  sys_modem_as_id_e_type       as_id;
  sys_radio_access_tech_e_type rat_type;
}cm_get_edrx_params_req_s_type;


/********************************************************************/
/* CM_GET_EDRX_PARAMS_RSP                                           */
/* Primitive to send eDRX paging time window & cycle length to CM   */
/********************************************************************/
typedef struct cm_get_edrx_params_rsp_s
{
  IMH_T                        message_header;
  sys_modem_as_id_e_type       as_id;
  sys_radio_access_tech_e_type rat_type;
  ...
  uint8                        edrx_ptw;
  uint8                        edrx_cycle_length;
}cm_get_edrx_params_rsp_s_type;


/********************************************************************/
/* CM_EDRX_PARAMS_CHANGE_IND                                        */
/* Primitive to inform CM if eDRX is currently supported at NAS or  */
/* AS while the device supports the usage of eDRX                   */
/********************************************************************/
typedef struct cm_edrx_params_change_ind_s
{
  IMH_T                        message_header;
  sys_modem_as_id_e_type       as_id;
  sys_radio_access_tech_e_type rat_type;
  ...
  uint8                        edrx_ptw;
  uint8                        edrx_cycle_length;
}cm_edrx_params_change_ind_s_type;
