...
$Header: //components/rel/mmcp.mpss/6.1.10/api/sm_cm.h#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
typedef enum 
{
  ...
  SMREG_RESTORE_CTXT_PSM_REQ,
  SMREG_RESTORE_PDP_CTXT_RSP,
...
/* ---------------------------------------------------------------------
** Primitive : SMREG-RESTORE-CTXT-PSM-REQ
** Description: Request for DS (via CM) to trigger PSM context restoration.
** -------------------------------------------------------------------*/
typedef struct smreg_restore_psm_ctxt_req
{
   sys_modem_as_id_e_type    as_id;
} smreg_restore_psm_ctxt_req_T;


/* ---------------------------------------------------------------------
** Primitive  : SMREG-RESTORE-CTXT-PSM-RSP
** Description: DS (via CM) responds status of PSM context restore request after restoring all PDP contexts.
** -------------------------------------------------------------------*/

typedef struct smreg_restore_psm_ctxt_rsp
{
  boolean                                 status;
  sys_modem_as_id_e_type    as_id;
} smreg_restore_psm_ctxt_rsp_T;

/* ---------------------------------------------------------------------
** Primitive  : SMREG-RESTORE-PDP-CTXT-REQ
** Description: DS responds status of PSM context restore request after restoring all PDP contexts.
** -------------------------------------------------------------------*/

typedef struct smreg_restore_pdp_ctxt_req {
...
/**< Minimum Quality of Service required for the PDP context*/
  ...
  sys_modem_as_id_e_type               as_id;
} smreg_restore_pdp_ctxt_req_T;



/* ---------------------------------------------------------------------
** Primitive  : SMREG-RESTORE-PDP-CTXT_RSP
** Description: NAS responds status of PDP context restore request after restoring 
**              PDP contexts for requested NSAPI.
** -------------------------------------------------------------------*/

typedef struct smreg_restore_pdp_ctxt_rsp
{
   nsapi_T                    nsapi;
   byte                       connection_id;
   byte                       esm_conn_id;
  sys_modem_as_id_e_type      as_id;
} smreg_restore_pdp_ctxt_rsp_T;
