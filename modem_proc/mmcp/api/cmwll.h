...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/api/cmwll.h#1 $
...
typedef struct
{
  ...
  union
  {
    ...
    cm_psm_get_timer_info_rsp_s_type       psm_get_timer_info_rsp;
   /* To provide active timer and periodic update timer values to CM */

    cm_psm_set_config_params_rsp_s_type    psm_set_config_params_rsp;
    /* Currently not used, defined for future purpose */

    cm_psm_timer_change_ind_s_type         psm_timer_change_ind;
    /* To indicate CM about change in active timer or periodic update timer value */

    cm_psm_auto_ready_ind_s_type           cm_psm_auto_ready_ind;
    /* To indicate CM that NAS is ready to enter PSM */

    cm_get_edrx_params_rsp_s_type          get_edrx_cnf;
      /* Cnf for get EDRX request */

    cm_set_edrx_cfg_rsp_s_type             set_edrx_cnf;
      /* Cnf for set EDRX request */

    cm_edrx_params_change_ind_s_type       edrx_change_info;
      /* Indication for the change in the value of  EDRX*/

    cm_enter_psm_mode_rsp_s_type           cm_enter_psm_mode_rsp;
    /* To indicate CM that NAS entered PSM*/

    smreg_restore_psm_ctxt_req_T           restore_psm_ctxt_req;
    /* Request for DS (via CM) to trigger PSM context restoration */

    smreg_restore_pdp_ctxt_rsp_T          restore_pdp_ctxt_rsp;
    /*NAS responds status of PDP context restore request after restoring 
      PDP contexts for requested NSAPI*/

    cm_call_ctrl_req_s_type               call_ctrl_req;
    /* Call control req */
	
    cm_psm_save_mmgsdi_ef_rsp_s_type      mmgsdi_ef_rsp;
    /* NAS response MMGSDI EF RSP*/
