...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/api/public/cm_gw.h#1 $
...
*/
#define LL_COMPATIBILITY_DATA_SIZE          16  
...
/** Maximum length of the epco contents.
*/
...
typedef struct 
{
  boolean valid;/* Whether indicator is present. */
  byte    dl_data_exp;
}release_assistance_ind_type;

#define MAX_CP_SETUP_LEN  251

typedef struct hc_config
{
  boolean     valid;
  /**< Whether a header compression is present. */
  byte        len;
  ...
  /** Additional header compression context setup parameters**/
  byte        hc_ctx_setup_container[MAX_CP_SETUP_LEN];
  /** Additional header compression context setup container**/
}hc_config_T;

typedef struct cp_only_ind
{
  boolean     valid;
  /**< Whether indication is valid. */
  boolean      cp_only_indication;
  /**< cp only indication IE. */
}cp_only_ind_T;
...
typedef enum
{
  ...
  LTE_NAS_LRRC_CONN_EST_FAILURE_ACCESS_BARRED_EAB,
                /**< &nbsp; */
  EMM_DETACHED_PSM                                  
  /* Indicates call ended due to PSM*/
...
typedef enum{
  ...
  NAS_ESM_PDN_TYPE_IPv4v6_ONLY_ALLOWED =57,                  /**< &nbsp; */
  NAS_ESM_PDN_TYPE_NON_IP_ONLY_ALLOWED =58,                  /**< &nbsp; */
...
typedef struct 
{
   boolean                   valid;       /* Is backoff timer present or not */
...
typedef struct reattempt_ind_s
{
  boolean valid;                    /* Whether Re-attempt indicator is present. */
  byte    reattempt_ind_content;    /* Re attempt indicator value consisting of RATC_VAL and EPLMN_VAL */
} reattempt_ind_T;  
...
typedef struct splmn_rate_ctrl
{
  boolean     valid;
  /**< Whether Serving PLMN rate control IE is present. */
  word        splmn_rate_ctrl_val;
