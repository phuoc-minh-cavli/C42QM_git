...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/api/public/cmipapp.h#1 $
...
typedef enum
{
  ...
  */
  ...
  CMIPAPP_END_CAUSE_OFFLINE = 618,
      /**< Call end cause due to UE going to LPM/APM */

  CMIPAPP_END_CAUSE_DRB_REJ = 619,
      /**< Call end cause due DRB REJ */

  CMIPAPP_END_CAUSE_UNWANTED_CALL = 623,
   /*FR49014: User ended the MT call, suspecting it to be   
     Unwanted_robocall
   */
...
typedef enum
{
  ...
  CMIPAPP_CLIENT_END_CAUSE_PROTOCOL_Q850_ERROR = CM_CALL_END_PROTOCOL_Q850_ERROR,
  /* End reason type Q.850 */

  CMIPAPP_CLIENT_END_CAUSE_CALL_ONGOING_CW_DISABLED = CM_CALL_END_INCOM_REJ_CAUSE_CALL_ONGOING_CW_DISABLED,

  CMIPAPP_CLIENT_END_INCOM_REJ_CAUSE_CALL_ON_OTHER_SUB = CM_CALL_END_INCOM_REJ_CAUSE_CALL_ON_OTHER_SUB,

  CMIPAPP_CLIENT_END_INCOM_REJ_CAUSE_1X_COLLISION = CM_CALL_END_INCOM_REJ_CAUSE_1X_COLLISION,

  
  CMIPAPP_CLIENT_END_CAUSE_SRV_NOT_REGISTERED = CM_CALL_END_CAUSE_SRV_NOT_REGISTERED,
  /* IMS not registered for service or new initial registration is pending (waiting on the ongoing call to terminate )*/ 

  CMIPAPP_CLIENT_END_CAUSE_CALL_TYPE_NOT_ALLOWED = CM_CALL_END_CAUSE_CALL_TYPE_NOT_ALLOWED,
  /* Call type not allowed on the current RAT */
  
  CMIPAPP_CLIENT_END_CAUSE_EMRG_CALL_ONGOING  = CM_CALL_END_CAUSE_EMRG_CALL_ONGOING,
  /* Emergency call ongoing */
  
  CMIPAPP_CLIENT_END_CAUSE_CALL_SETUP_ONGOING = CM_CALL_END_CAUSE_CALL_SETUP_ONGOING,
  /* Establishing Another Call  */
  
  CMIPAPP_CLIENT_END_CAUSE_MAX_CALL_LIMIT_REACHED = CM_CALL_END_CAUSE_MAX_CALL_LIMIT_REACHED,
  /* Max allowed calls already in progress  */
  
  CMIPAPP_CLIENT_END_CAUSE_UNSUPPORTED_SIP_HDRS = CM_CALL_END_CAUSE_UNSUPPORTED_SIP_HDRS,
  /* SIP hdrs recvd are not accepted (missing tag in supported hdr or Unsupported tag in require header, etc) */
  
  CMIPAPP_CLIENT_END_CAUSE_UNSUPPORTED_SDP = CM_CALL_END_CAUSE_UNSUPPORTED_SDP,
  /* SDP recvd is malformed or not accepted ( IP type mismatch, codec not supported, etc)  */
  
  CMIPAPP_CLIENT_END_CAUSE_CALL_TRANSFER_ONGOING = CM_CALL_END_CAUSE_CALL_TRANSFER_ONGOING,
  /* Call transfer in progress */
  
  CMIPAPP_CLIENT_END_CAUSE_PRACK_TIMEOUT = CM_CALL_END_CAUSE_PRACK_TIMEOUT,
  /* Call failed while waiting for PRACK for 183  */
  
  CMIPAPP_CLIENT_END_CAUSE_QOS_FAILURE = CM_CALL_END_CAUSE_QOS_FAILURE,
  /* Call failed due to lack of dedicated bearer  */
  
  
  CMIPAPP_CLIENT_END_CAUSE_ONGOING_HANDOVER = CM_CALL_END_CAUSE_ONGOING_HANDOVER,
  /* Call end due to ongoing handover */
  
  CMIPAPP_CLIENT_END_CAUSE_VT_WITH_TTY_NOT_ALLOWED = CM_CALL_END_CAUSE_VT_WITH_TTY_NOT_ALLOWED,
  /* TTY and VT not supported together */
  
  CMIPAPP_CLIENT_END_CAUSE_CALL_UPGRADE_ONGOING = CM_CALL_END_CAUSE_CALL_UPGRADE_ONGOING,
  /* Upgrade request is in progress */
  
  CMIPAPP_CLIENT_END_CAUSE_CONFERENCE_WITH_TTY_NOT_ALLOWED = CM_CALL_END_CAUSE_CONFERENCE_WITH_TTY_NOT_ALLOWED,
  /* Call from conferenec server recvd when TTY is ON */
  
  CMIPAPP_CLIENT_END_CAUSE_CALL_CONFERENCE_ONGOING = CM_CALL_END_CAUSE_CALL_CONFERENCE_ONGOING,
  /* Conference call ongoing */
  
  CMIPAPP_CLIENT_END_CAUSE_VT_WITH_AVPF_NOT_ALLOWED = CM_CALL_END_CAUSE_VT_WITH_AVPF_NOT_ALLOWED,
  /* VT call with AVPF */
  
  CMIPAPP_CLIENT_END_CAUSE_ENCRYPTION_CALL_ONGOING = CM_CALL_END_CAUSE_ENCRYPTION_CALL_ONGOING,
  /* encryption call couldn't coexist with other calls */

  CMIPAPP_CLIENT_END_CAUSE_INCOM_REJ_UI_NOT_READY = CM_CALL_END_INCOM_REJ_CAUSE_UI_NOT_READY,

  CMIPAPP_CLIENT_END_CAUSE_CS_CALL_ONGOING = CM_CALL_END_CAUSE_CS_CALL_ONGOING
...
typedef enum
{
  ...
  CMIPAPP_INCOM_REJ_CAUSE_UNWANTED_CALL,
    /* Indicated received call is unwanted_robocall
    */
...
typedef struct cmipapp_end_params_s
{
  ...
  /* indication from RIL that received call is Robocall */
...
typedef struct cmipapp_ind_mo_progr_info_s
{
  ...
  cm_ip_history_info_s_type    *history_info;
    /* IP history info from mo_progr_info
    */
...
/* Indicates the incoming call validation info provided 
** by the network
*/
typedef enum
{
  
  CMIPAPP_TN_VALIDATION_NONE = 0,
	/* No telephone Num validation performed */
	
  CMIPAPP_TN_VALIDATION_PASS,
	/* telephone Num validation passed */
	
  CMIPAPP_TN_VALIDATION_FAIL,
	/* telephone Num validation failed */
	
  CMIPAPP_TN_VALIDATION_MAX
	/* FOR INTERNAL USE OF CM ONLY! */

}cmipapp_verstat_value_e_type;
...
typedef struct cmipapp_ind_mt_ring_cnf_s
{
  ...
  cmipapp_verstat_value_e_type call_verstat;
...
typedef struct cmipapp_ind_call_end_s
{
  ...
  cm_ip_call_end_uri_s_type     *alternate_uri;
    /* Alternate URI sent by IMS when end_cause is CMIPAPP_END_CAUSE_MULTIPLE_CHOICES, 
    ** CMIPAPP_END_CAUSE_MOVED_PERMANENTLY, CMIPAPP_END_CAUSE_MOVED_TEMPORARILY
    ** or CMIPAPP_END_CAUSE_USE_PROXY. Inform the user of the new alternate URI to which call 
    ** can be attempted.
    */
...
typedef struct cmipapp_ind_hold_s
{
  ...
  cm_ipapp_cause_e_type        cause;
   ...
   cm_ip_call_end_reason_s_type  *end_reason;
	 /* Call End reason String , if available from IMS NW
	 */

   uint16                         sip_error_code;
	 /* Call End Sip Error code , if available from IMS NW
	 */
...
{  
  ...
  cm_ip_call_end_reason_s_type  *end_reason;
	/* Sups failure reason string, if available from IMS NW*/

  uint16                         sip_error_code;
	/* Sups failure Sip Code, if available from IMS NW*/
...
DESCRIPTION
Return ecmp value of a particular app_id. If app_id is out of range return CMIPAPP_ECMP_3GPP
