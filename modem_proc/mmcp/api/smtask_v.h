...
$Header: //components/rel/mmcp.mpss/6.1.10/api/smtask_v.h#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
typedef enum
{
  ...
  /*--------------------------------------------------------*/
  SMREG_PDP_ACTIVATE_REQ = 0x01, /* CM commands start here */
  ...
  SMREG_RESTORE_CTXT_PSM_RSP,
  SMREG_RESTORE_PDP_CTXT_REQ,
  ...
  /*--------------------------------------------------------*/
  GMMSM_ESTABLISH_CNF = 0x1E,  /* GMM commands start here */
  GMMSM_ESTABLISH_IND,
  GMMSM_ESTABLISH_REJ,
  ...
  /*This indication is sent from GMM to SM after IRAT to G, so that SM can locally deactivate the PDP which don't have valid GERAN Params
    Currently,this is being done after RAU complete indication. This indication is added to address No-RAU cases.  */
  GMMSM_IRAT_PS_SERVICE_IND,
  GMMSM_RESTORE_CTXT_PSM_REQ,
  GMMSM_SAVE_CTXT_PSM_REQ, 

  /*--------------------------------------------------------*/
  /* Command Ids sent by SNDCP entity                       */
  /*--------------------------------------------------------*/
  SNSM_ACTIVATE_RESP ,  /* SNDCP commands start here */
  ...
  /*--------------------------------------------------------*/
  MSM_SM_ACTIVATION_REQ ,  /* MSM commands start here */
...
{   
  ...
  {  
    ...
    gmmsm_irat_ps_service_ind_T       gmmsm_irat_ps_service_ind;
    gmmsm_save_ctxt_psm_req_T         gmmsm_save_ctxt_psm_req;
    gmmsm_restore_ctxt_psm_req_T      gmmsm_restore_ctxt_psm_req; 
    ...
    smreg_restore_pdp_ctxt_req_T   restore_pdp_ctxt_req;
    smreg_restore_psm_ctxt_rsp_T   restore_psm_ctxt_rsp;
...
/* Function used by other entities to put a command in MM command Queue and set ESM_CMD_Q_SIG */
extern void esm_put_cmd(void* cmd_ptr);


/* Function used by other entities to get SM command type */
extern sm_cmd_type *sm_get_cmd_buf(void);

/* Function used SM to allocate a message before sending to DS */
extern ds_emm_cmd_type *ds_get_cmd_buf(void);
...
extern void ds_pdn_mgr_nas_to_ds_msg_hdlr(void * ptr);
