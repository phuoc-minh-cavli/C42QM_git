...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdsr.c#1 $
...
)
{
  ...
  */
  ...
  /* Initialize the status bit mask to NONE. used for irat.
  */
...
)
{
  ...
  switch( sr_ptr->sys.mode)
  {
    ...
    default:
      SD_MSG_HIGH_4( "SDSR: %d,  Sys mode=%d,           band=0x%08x%08x,chan=%d",
                        sr_ref,sr_ptr->sys.mode,
                        QWORD_HIGH(sr_ptr->sys.band), QWORD_LOW(sr_ptr->sys.band));
  ...
  switch (sr_ptr->sys.mode)
  {
    ...
    #ifdef FEATURE_MMODE_3GPP
      SD_MSG_HIGH_4( "plmn=%d, %d, %d, sel_mode=%d",
...
)
{
  ...
  switch( comp_cat )
  {
    ...
    case SDSR_COMP_CELL_HOME:
      ...
      */
      cdma_home_blksys = sdprl_cdma_bc0_sys_to_blksys(
...
#if defined(FEATURE_MMODE_3GPP) || ( defined(SD_DEBUG) &&!defined(_lint) ) 
...
)
{
  ...
  */
  ...
  {
    if(avail_ptr->sdsr_rec_type != SDSR_GWL_LST &&
  ...
  */
  ...
  {
    if(tbl_ptr->sdsr_rec_type != SDSR_GWL_LST &&
...
)
{
  ...
  sr_ref = sdsr_tbl_store_2( sr_ptr );
  ...
  )
{
  ...
  #else
  #define INITIAL_LIST_ARRAY_SIZE 3
  #endif
...
void                  sdsr_lists_init( sd_ss_e_type ss )
{
  ...
  #elif FEATURE_MMODE_HYBR2_ENABLED
  ...
  {      
    ...
    case SD_SS_HYBR_1:
      ...
      sdsr_lists_clr_with_range(SDSR_1ST_SYS, SDSR_MAX);
      break;
...
)
{
  ...
  */
  ...
  if( sr.sid_expected.net_select_mode == SD_NET_SELECT_MODE_HPLMN)
   ...
   #endif
   if(sr.sid_expected.net_select_mode == SD_NET_SELECT_MODE_MANUAL_SYS)
   {
