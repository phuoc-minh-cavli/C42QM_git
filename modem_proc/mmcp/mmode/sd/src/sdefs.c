...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdefs.c#1 $
...
{  
   ...
   {SDEFS_RAT_ACQ_ORDER_SUB2,  SDEFS_RAT_ACQ_ORDER_SUB2_F_NAME} ,
   #endif
   ...
   {SDEFS_MRU_EXT_NAM_1,           SDEFS_MMODE_MRU_EXT_NAM_1_F_NAME} ,
   {SDEFS_MRU_EXT_NAM_2,           SDEFS_MMODE_MRU_EXT_NAM_2_F_NAME} ,    
...
)
{
  ...
  /*if file to be read have more size then the 
  ** size intended to be copied in buffer for efs item  
  */
  if (st.st_size > ((uint32)*size_ptr))
    return FALSE;
...
)
{
  ...
  if(status != MCFG_FS_STATUS_OK)
  {
    SD_MSG_HIGH_3("mcfg fs read : file:%s,error status %d errno %d", file,status, mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    *size_ptr = 0;
    return FALSE;
  }
...
)
{
  ...
  if( efs_id == SDEFS_RAT_ACQ_ORDER_SUB2 ||
      efs_id == SDEFS_RAT_ACQ_ORDER )
  {
