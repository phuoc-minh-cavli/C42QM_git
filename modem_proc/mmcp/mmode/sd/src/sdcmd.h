...
typedef struct {
  ...
  ** multiple tasks being processed simultaneously */
  ...
  boolean is_gw_scan_attempted;
  /* Indicates if a ACQ GWL triggered from last ACQ FAIL */
  #endif
} sd_ss_s_type;
...
typedef struct
{
  ...
  sys_chan_type                 chan;
  sys_srv_capability_e_type     plmn_srv_cap; /* applicable to gw not LTE */
  ...
  sys_extend_srv_info_e_type    extend_srv_info;
  #ifdef FEATURE_MMODE_3GPP2
  sys_reg_domain_e_type         reg_domain; /* used for camp mode */
  #endif
  #ifndef FEATURE_MMODE_DISABLE_CSG
  const sys_csg_info_s_type*    csg_info_ptr;
  sys_plmn_id_s_type*           registered_plmn; /* csg */
  #endif
...
void                      sd_si_update_if_srv_is_off(
        ...
        sd_mode_e_type                  ciot_lte_mode,
         /* ciot lte mod eof operation */

         sys_neg_ciot_cap                neg_ciot_cap,
         /* Negotiated ciot capabiltiies with NW */
...
void    sd_si_update(
      ...
      /*Camped lte ciot mode of operation */
      sd_mode_e_type                  ciot_lte_mode,

      /* Accepted ciot network capabilities */
      sys_neg_ciot_cap                 neg_ciot_cap,
...
sd_ss_act_e_type
  sdcmd_user_ss_pref8( sd_ss_ind_user_ss_pref_cmd *cmd);
