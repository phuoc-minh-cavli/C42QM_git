...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdmmss.c#1 $
...
#if defined FEATURE_MMSS_3_1 || !defined FEATURE_MMODE_DISABLE_MANUAL_BST
/*===========================================================================
...
)
{
  ...
  */
  ...
  SD_MSG_HIGH_3("sys type %d loc %d pri %d",rec_ptr->sys_type,
                                        rec_ptr->pri_class,
                                          rec_ptr->sys_pri);
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  ...
  for (i = 0; i < (rec_ptr->loc_param_val.num_sys_loc_tag); i++)
  {
    ...
    */
    ...
    SD_MSG_HIGH_2("tag num %d sys_type %d",i,
    ...
    */
    switch (rec_ptr->loc_param_val.loc_tag[i].sys_type)
    {
      ...
      case SDMMSS_SYS_TYPE_LTE:
        ...
        SD_MSG_HIGH_4("plmn %x %x %x, plmn range %d",
...
FUNCTION sdmmss_encode_tot_tbl()
...
===========================================================================*/
prl_tot_validation_write_enum_type sdmmss_encode_tot_tbl(
...
)
{
  ...
  */
  size = sizeof(tot->version)*SDMMSS_DECODE_SIZE_8_BITS;
  ...
  */
   ...
   tot->num_rat == 0)
  {
    ...
    return PRL_TOT_INVALID_NUM_OF_RATS;
  }

  /* Encode num rats
  */
  size = sizeof(tot->num_rat)*SDMMSS_DECODE_SIZE_8_BITS;
  ...
  /* Encode acq sys mode
  */
  size = SDMMSS_DECODE_SIZE_8_BITS;
  for(i = 0; i < tot->num_rat; i++)
  {
    ...
    */
    ...
    {
      return PRL_TOT_INVALID_ACQ_SYS_MODE;
    }
  ...
  return PRL_TOT_VALID;

}//sdmmss_encode_tot_tbl

/*===========================================================================

FUNCTION sdss_validate_and_write_tot()

DESCRIPTION
  Validate and encode the Technology order table.
  If it's valid, write into EFS.
...
===========================================================================*/
EXTERN prl_tot_validation_write_enum_type sdmmss_validate_and_write_tot( 
  const sd_rat_acq_order_s_type     * tot
    /* pointer to TOT info to be encoded.
    */
)
{

  static byte                    *efs_tot      = NULL;
  prl_tot_validation_write_enum_type is_valid   = PRL_TOT_VALID_WRITE_NONE;
  ...
  */
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Allocate memory for binary TOT if it is not already done
  */
  if(!efs_tot)
  {
    efs_tot =(byte*)modem_mem_alloc( SDMMSS_BST_TBL_MAX_SIZE,
                                         MODEM_MEM_CLIENT_MMODE);
  ...
  memset(efs_tot,0,PRL_TOT_TBL_MAX_SIZE);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Valicate and encode TOT table.
  */
  is_valid = sdmmss_encode_tot_tbl(tot,efs_tot);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* If validation failed, return the error.
  */
  if (is_valid != PRL_TOT_VALID)
  {
    SD_MSG_HIGH_1("TOT validation error = %d", is_valid);
  }
  else
  {
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    /* Write the encoded buffer to EFS.
    */
    is_written = sdefs_write(SDEFS_RAT_ACQ_ORDER,efs_tot,PRL_TOT_TBL_MAX_SIZE);

    SD_MSG_MED_1("TOT is_written = %d", is_written);

    if (is_written)
    {
      is_valid = PRL_TOT_VALID_WRITTEN_SUCCESS;
    }
    else
    {
      is_valid = PRL_TOT_VALID_WRITTEN_ERR;
    }
  }
  ...
  */
  if(efs_tot != NULL)
  {
    modem_mem_free((void *) efs_tot, MODEM_MEM_CLIENT_MMODE);
    efs_tot = NULL;
  }  
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  SD_MSG_HIGH_6("EqPRL: sys tbl entry %d rec_type %d acq_idx %d, geo %d pref %d pri %d",
             enc_sys_tbl->num_entries,
             sys_rec->rec_type,
             sys_rec->acq_index,
             sys_rec->geo,
...
)
{
  ...
  */
  ...
  for(i = 0; i<mlpl_lst->length; i++)
  {
    ...
    SD_MSG_HIGH_3("%x %x %x",mlpl_lst->info[i].plmn.identity[0],
