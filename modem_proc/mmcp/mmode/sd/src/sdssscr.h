...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdssscr.h#1 $
...
SD_BEGIN_ENUM(sdssscr_con_e) {
  ...
  */
  ...
  #endif
  SD_CONSTR_ELEMENT(CONT_OOS), /* used to start timer corresponding to the current OOS stage. Used only for 3GPP at the moment*/

                              /* 129 */
  SD_CONSTR_ELEMENT(LOOP_TIME_OOS),  /*LOOP_TIME_OOS - Repeat executing the script
                                      ** between this construct and the matching LOOP till OOS timer expires  */
  SD_CONSTR_ELEMENT(CONT_LMTD_BSR),  /* used to start a timer corresponding to limited to full BSR stage */
  SD_CONSTR_ELEMENT(INC_COUNT),      /* used to increment count related to full band scans in OOS or BSR timeline */
  SD_CONSTR_ELEMENT(RESET_TIMELINE), /* used to reset BPLMN stage or OOS stage. Used only for 3GPP at the moment*/
...
typedef enum {
  ...
  SDSSSCR_HDR_ACQ_ROTATE_SRV_LOST, /* SRV_LOST   */
  SDSSSCR_CDMA_ACQ_MICRO,     /* One CDMA Micro Acq  */
  SDSSSCR_SRV_LOST_HDR_CDMA_HO,   /* Service lost while in HDR to CDMA Handoff */
  #endif
  ...
  SDSSSCR_ACQ_DB_SCAN_DURING_SLEEP_TIME,            /* Script for ACQ DB Scan for 30s */
  ...
  #endif
  SDSSSCR_PWR_SAVE_SCAN_GW,         /*Script to issue scan in power save timeline */
...
#define SDSSSCR_CONT_TYPE_NONE                         ((word) (0))
...
typedef enum {
  ...
  /* EFS items associated with 3GPP OOS timeline start
  */
  SDSSSCR_EFS_ITEM_3GPP_SLOW_SCAN_ENABLED = 11,
  /* EFS id to enable/disable slow OOS/limited to full scans
     Default value = 0 ( disabled) 
     Can be either 1 or 0
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_T_NORM_MAX = 12,
  /* EFS id to determine max time between scans in normal mode
     ( not stationary and not low battery). 
   Default value = 60. Should be an integer in the range 30 - 180.
   */

  SDSSSCR_EFS_ITEM_3GPP_OOS_REPS_PER_STAGE = 13,
  /**< EFS id to set the number of scans per stage of the OOS timeline 
    Default value = 10.
    It can only be an integer in the range 1 - 15.
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_T_SLOW_MAX = 14,
  /**< OOS timer will be capped at this value .
    Default value = 300s.
    It can be an integer in the range 30 - 600s
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_TIMER_VALUE1 = 15,
  /**< Timer value 1 used for Normal OOS scan timeline 
    The timer values must be integers in the range 1 to 600 to be considered valid
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA = 16,
  /**< Timer Delta value for incrementing OOS stage values
  */

  /* EFS items associated with 3GPP OOS timeline end
  */

  /* EFS items associated with 3GPP  limited to Full BSR start 
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX = 17,
  /* EFS id to determine max time between limited to full scans in normal mode
     ( not stationary and not low battery). 
   Default value = 60. Should be an integer in the range 30 - 180.
   */

  SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE = 18,
  /**< EFS id to set the number of scans per stage of the BSR timeline 
    Default value = 1.
    It can only be an integer in the range 1 - 15.
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_MAX_TIME = 19,
  /**< BSR timer will be capped at this value .
    Default value = 300s.
    It can be an integer in the range 1 - 600s
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_INITIAL_VAL = 20,
  /**< Timer value TO used for inital BSR 
    The timer values must be integers in the range 1 to 600 to be considered valid
  */

  SDSSSCR_EFS_ITEM_3GPP_BSR_TIME_DELTA = 21 ,
  /**< Timer value Tdelta used for BSR
     Timer[stagex] = T0 + Tdelta * x      
  */

  /* EFS items associated with 3GPP  limited to Full BSR start 
  */

  SDSSSCR_EFS_ITEM_3GPP_OOS_SCAN_COUNTER = 22,
  /**< EFS id to determine number of rounds of scan UE has to do 
    after detecting OOS before entering power save. Range 1-10
  */
