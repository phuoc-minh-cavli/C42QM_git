...
$Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/prldiag.c#1 $
  ...
  do
  {
    ...
    */
    ...
    else
    {
      ...
      nv_pr_list.size = (word)bit_count;
...
)
{
  ...
  */
    ...
    SD_MSG_LOW_0 ( "bad parm (byte cnt 0, or seqnum*block size too big)");
    return ( diagpkt_err_rsp ( DIAG_BAD_PARM_F, req_ptr, pkt_len ) );
  }
  ...
  #else
  {
      ...
      SD_MSG_HIGH_0("Failed to read PR List as PRL is disabled!");
