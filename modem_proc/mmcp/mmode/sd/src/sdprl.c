...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdprl.c#1 $
...
DESCRIPTION
  Given a band_pref in MRU format, maps it to sd band_pref.  THe function does not apply to LTE.
...
DESCRIPTION
  Given a band_pref in sd band_pref format, maps it to MRU format. The function does not 
  apply to LTE.
...
FUNCTION sdprl_cdma_bc0_sys_to_blksys

DESCRIPTION
  Given an CDMA band-class 0 system, this function returns the cellular
  system (in blksys representation) that is associated with this CDMA system.
...
RETURN VALUE
  ...
  input CDMA band-class 0 system.
...
===========================================================================*/
EXTERN  sd_blksys_e_type           sdprl_cdma_bc0_sys_to_blksys(
...
)
{
  ...
  */
  SD_MSG_LOW_1( "Bad cdma_bc0_sys=%d",cell_sys);
  return SD_BLKSYS_MAX;

} /* sdprl_cdma_bc0_sys_to_blksys */
...
)
{
  ...
  */
  ...
  for( i=0; (sr_ptr=sdsr_list_get_ptr(SDSR_PRL_LST, i)) != NULL; i++ )
  {
    if( sdss_sr_is_mode_pref(sr_ptr, mode_pref) &&
        sdss_sr_is_band_pref(sr_ptr, lte_band_pref, band_pref, tds_band_pref, SD_SS_MAIN) )
    {
      return TRUE;
    }
  }
  ...
  */
  for( i=0; (sr_ptr=sdsr_list_get_ptr(SDSR_GWL_LST, i)) != NULL; i++ )
  {
    ...
    #else
    if( sdss_sr_is_mode_pref(sr_ptr, mode_pref) &&
        sdss_sr_is_band_pref(sr_ptr, lte_band_pref) )
    #endif
    {
      return TRUE;
    }
...
)
{
  ...
  sd_mmode_efs_mru_table_s_type   mru_buf_ext;
  int32                           size_mru_ext;  
  sdefs_id_e_type                 efs_id_ext;  
  #endif
  #else
  ...
  ** return FALSE.
  */
  /* If build supports 256 LTE bands , read from new extended MRU NV
  ** else from old MRU NV 
  */
  #if defined(LTE_BAND_NUM) && (LTE_BAND_NUM == 256)
  if(nam == SD_NAM_1)
  {
    efs_id_ext = SDEFS_MRU_EXT_NAM_1;
  }
  else
  {
    efs_id_ext = SDEFS_MRU_EXT_NAM_2;
  }  
  
  size_mru_ext = sizeof(mru_buf_ext);
  memset(&mru_buf_ext, 0, size_mru_ext);    
  #endif
  ...
  memset(&mru_parameters, 0, size_mru);


  /* Initialize the RAM to default values if the read operation failed */
  /* Try to read from new extended MRU NV . 
  ** If it fails, attempt reading from old MRU NV - this is to be backward compatible and reduce 
  ** effect when MRU is copied in old path only
  ** If both reads fail, initialize MRU table to default .
  */
  #if defined(LTE_BAND_NUM) && (LTE_BAND_NUM == 256)
  if( !sdefs_read(efs_id_ext, (byte *)&mru_buf_ext, &size_mru_ext ))
  #endif 
  {

    SD_MSG_HIGH_0("MRU_READ: MRU EXT NV failed");
    if( !sdefs_read(efs_id, (byte *)&mru_parameters, &size_mru ))
    {
      SD_MSG_HIGH_0("MRU_READ: MRU NV failed");
  ...
  {
    SD_MSG_HIGH_1("MRU_READ: MRU EXT NV  read success version %d", mru_buf_ext.version);
    memscpy(&mru_parameters,  sizeof(mru_parameters), &mru_buf_ext.mru_tbl, sizeof(mru_buf_ext.mru_tbl));
  }
  #endif
...
)
{
  ...
  for( i=0; i < NV_MRU_TABLE_SIZE && isSame == TRUE; i++ )
  {
    ...
    switch(buffer1[i].mode)
    {
      ...
      default:
        SD_MSG_LOW_1("MRU: record %-2d unhandled mode",i);
        break;
...
)
{
  ...
  */
  ...
  for( i=0; (i < stored_mru_size && i < NV_MRU_TABLE_SIZE); i++ )
  {
    ...
    */
    ...
    switch(mru_buf_curr[i].mode)
    {
      ...
      #endif
      default:
        SD_MSG_LOW_2("MRU Entry[%02d] : Mode %d ",i,mru_buf_curr[i].mode);
      break;
    }
  }
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* If build supports 256 LTE bands , also write to new extended MRU */
  #if defined(LTE_BAND_NUM) && (LTE_BAND_NUM == 256)
  if(nam == SD_NAM_1)
  {
    efs_id = SDEFS_MRU_EXT_NAM_1;
  }
  else
  {
    efs_id = SDEFS_MRU_EXT_NAM_2;
  }
  /* #------# */
  memscpy(&mru_buf_ext.mru_tbl, sizeof(mru_buf_ext.mru_tbl), &mru_parameters, size_mru);
  size_mru_ext += size_mru;

  /* Write default version*/
  mru_buf_ext.version = MRU_BUF_EXT_DEFAULT_VERSION;
  size_mru_ext += sizeof(uint8);

  sdefs_write(efs_id, (byte *)&mru_buf_ext, size_mru_ext );
  #endif  
...
static sdsr_ref_type sdprl_copy_gwl_mru_to_list(
  ...
  sdprl_util_updt_sid_exp_from_3gpp_rec( gwl_sys->plmn_id,
   ...
   /*Bounds Checking for Num Rats
   */
   if( !(INRANGE( gwl_sys->num_rats, 1, SYS_MAX_NUM_3GPP_RATS)) )
   {
     /*Num rats value corrupted.Return to prevent writing of garbage information
     */
...
)
{
  ...
  */
  ...
  /* If we got here, the mode, band and/or CDMA channel do not
     ...
     mcc_list_ptr)
    {
      ...
      if( mcc > SD_MCC_NONE  && mcc < SD_MCC_MAX )
      {
        ...
        */
        ...
        SD_MSG_HIGH_2("EXTRACT_MCC: count=%d, mcc=%d", 
          ...
          sdprl_parser_sys_is_in_geo(prl_parser, sr1_ptr) )
      {
        ...
        */
        ...
        if( sdprl_parser_sys_is_more_pref(prl_parser,
                                          ...
                                          sr2_ptr->sys.mode) )
        {
          SD_MSG_HIGH_2("sdprl_parser_sys_is_more_pref:TRUE:,geo_idx=%d,lvl:%d",
                        prl_parser->geo_idx, match_lvl);
          if (prl_sr != NULL)
          {
            modem_mem_free((void *)(prl_sr), MODEM_MEM_CLIENT_MMODE);
          }
        ...
        SD_MSG_HIGH_2("sdprl_parser_sys_is_more_pref:FALSE:,geo_idx=%d,lvl:%d",
                      prl_parser->geo_idx, match_lvl);
        if (prl_sr != NULL)
        {
          modem_mem_free((void *)(prl_sr), MODEM_MEM_CLIENT_MMODE);
...
static void sdprl_extract_mcc_mnc_from_card_imsi(void)
{
...
static boolean sdprl_set_roam_ind_cmp_imsi_oh_mcc_mnc(word srv_sys_mcc, byte srv_sys_mnc)
{
	...
	word decode_mcc=0,  decode_card_mcc=0, decode_card_mnc =0, decode_mnc=0;
	word encoded_mcc =0 ; byte encoded_mnc =0;
      ...
      */
	   ...
	   */
	   ...
	   SD_MSG_HIGH_4(" decode_mcc - %d , decode_card_mcc - %d ,decode_mnc - %d , decode_card_mnc - %d ",decode_mcc,decode_card_mcc,decode_mnc,decode_card_mnc);
	   
       if ( (decode_mcc !=  decode_card_mcc)||(decode_mnc !=  decode_card_mnc))
	   {
	   	      return TRUE;
	   }
  ...
  */
                      ...
                      SD_SS_MODE_PREF_CDMA_HDR ) )
  {
        ...
        hdr_mcc_list_ptr && hdr_mcc_list_ptr->count > 0)
    {
      ...
      SD_MSG_HIGH_1("MMSOA: with mcc_list_cnt=%d", hdr_mcc_list_ptr->count);
      ...
      sdss_report_mcc_to_cm( ss, hdr_mcc_list_ptr->mcc_list[0], plmn, sd_sr_ptr->sys.mode );
      #endif
    }
  }
...
)
{
  ...
  */
  if (service_status != SYS_SRV_STATUS_LIMITED_REGIONAL)
  {
    ...
    #ifdef FEATURE_MMODE_SC_SVLTE
    sdss_report_mcc_to_cm(ss, mcc, plmn, sd_sr_ptr->sys.mode);
    #endif
  }
...
static  boolean                   sdprl_ral_read(sys_modem_as_id_e_type asubs_id)
{
  ...
  sdefs_id_e_type                 efs_id;
  int32                           size_ral;
  int32                           size_ral_exp;/*Expected RAL Size*/
  ...
  size_ral_exp = sizeof(sd_mmode_ral_table_entry_u_type) * SD_RAL_TABLE_SIZE;
  size_ral = sizeof(ral_parameters);

  
  /*Initialization*/
  memset(&ral_parameters,0, sizeof(sd_mmode_ral_table_s_type));
  memset(&ral_buf,0, size_ral_exp);

  /* Initialize the RAM to default values if the read operation failed
  */
  if( !sdefs_fs_read_per_subs(efs_id, (byte *)&ral_parameters,
                               &size_ral, asubs_id)
      || (size_ral_exp > size_ral)
  )
  {
    SD_MSG_HIGH_2( "Invalid RAL: exp_size:%d, file_size:%d",
                    size_ral_exp ,
                    size_ral);
      /* Initialize the entries SD_MODE_INACT for all.
      */
    ...
    if( ral_buf[i].mode != (byte) SD_MODE_INACT )
    {
      ...
      case SD_MODE_GWL:
          ...
          */
          ...
          /*Bounds Checking for Num Rats
          */
          if( !(INRANGE( ral_buf[i].gwl.num_rats, 1, SYS_MAX_NUM_3GPP_RATS)) )
          {
            /*Num rats value corrupted.Break to prevent writing of garbage data
            */
            break;
          }
...
EXTERN  boolean                   sdprl_init( void )
{
  ...
  */
  ...
  (void) sdnv_read(NV_SRDA_ENABLED_I, item_ptr);
...
)
{
  ...
  */
  ...
  sd_ss_band_pref_e_type    supp_gsm_band_pref = sdss_get_supp_gsm_band_pref(SD_SS_BAND_PREF_ANY);
  //sd_ss_band_pref_e_type    supp_wcdma_band_pref = sdss_get_supp_wcdma_band_pref(SD_SS_BAND_PREF_ANY);
  sd_ss_band_pref_e_type    common_band_gsm = SD_SS_BAND_PREF_NONE;
  sd_ss_band_pref_e_type    common_band_wcdma = SD_SS_BAND_PREF_NONE;
  ...
  */
  ...
  common_band_gsm = SD_GET_COMMON_BAND(supp_gsm_band_pref, SD_GW_BAND_GROUP);
  //common_band_wcdma = SD_GET_COMMON_BAND(supp_wcdma_band_pref, SD_GW_BAND_GROUP);
  
  gw_band_pref = common_band_gsm | common_band_wcdma;
      ...
      */
      switch( sys_rec->rec_type )
      {
        ...
        case PRL_SYS_REC_MCC_MNC:
          ...
          SD_MSG_HIGH_2 ("MCC = %d, MNC = %d ", sys_rec->rec.mcc_mnc.mcc,
...
)
{
  ...
  */
  ...
  switch( acq_rec_ptr->acq_type )
  {
   ...
   SD_MSG_HIGH_2( "ACQ_MAP_TBL : Acq ix: %d, Number of records :%d",\
                 acq_ix,num_1x_sys );
 ...
 )
{
   ...
   if( acq_map_ptr == NULL)
   {
    SD_MSG_MED_1(" sdprl_acq_map_q_get_item return NULL for acq_index =%d",acq_ix);
    return;
   }
...
)
{
  ...
  */
  ...
  #endif
  {
    SD_MSG_HIGH_0("reading TOT list on subscription available");
    sdss_extract_tot(SD_SS_MAIN); 
  }
...
===========================================================================*/
void  sdprl_nam_sel_lte_only(void)
{
  int i = 0; 
  sdsr_list_clr( SDSR_GWL_LST );
  sdsr_list_clr( SDSR_MANUAL_LST);
