...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdssscr.c#2 $
...
===========================================================================*/
#define ACQ( LIST )         {SDSSSCR_CON_ACQ}, \
                                        {SDSR_##LIST},
...
DEFINITION
   Repeat executing the script between this construct and the matching LOOP
  construct for a time determined by the current OOS timer
 .
CONTROL
  Script execution continues immediately with the script construct that
  follows this construct.
...
===========================================================================*/
#define LOOP_TIME_OOS           {SDSSSCR_CON_LOOP_TIME_OOS}, 
...
EXAMPLE
  ...
  for 60 seconds.  When 60 seconds expire the SS-Client shall call on the
  SD_SS_IND_MISC_TIMER SS-Indication function.
Note: used by limited to full selection 3gpp script.
===========================================================================*/
...
EXAMPLE
  ...
  is the value stored in the configurable items table at index
  SDSS_NV_ITEM_TIMER_HDR_LONG.
Note: used by 3gpp2 scripts only.
===========================================================================*/
...
DEFINITION
  Start Out of service (OOS) timer in 3GPP Mode
...
EXAMPLE
  CONT_OOS - Start OOS timer

===========================================================================*/
#define CONT_OOS                     {SDSSSCR_CON_CONT_OOS},
...
DEFINITION
  Start limited to full BSR timer
...
EXAMPLE
  CONT_LMTD_BSR - Start bsr timer

===========================================================================*/
#define CONT_LMTD_BSR                     {SDSSSCR_CON_CONT_LMTD_BSR},
...
DEFINITION
 Increment counter related to a specific operation
 
CONTROL
  

EXAMPLE
  INC_COUNT( FULL_BAND_OOS )- increments value corresponding to the number of 
  full band OOS scans.
  INC_COUNT( FULL_BAND_BSR )- increments value corresponding to the number of 
  full band BSR scans.

===========================================================================*/
#define INC_COUNT( VAL )                     {SDSSSCR_CON_INC_COUNT},\
                                             {SDSS_INC_COUNT_##VAL},
...
DEFINITION
  Reset OOS Timeline or BSR timline ( depending on whether there is limited srv
  or not )
...
EXAMPLE
  RESET_TIMELINE - RESET OOS/BSR timeline

===========================================================================*/
#define RESET_TIMELINE                     {SDSSSCR_CON_RESET_TIMELINE},
...
===========================================================================*/
#define REJECT_SYS                        {SDSSSCR_CON_REJECT},
...
DEFINITION
  Evaluates to TRUE iff VAL equals the current reselection mode 
  VAL can take on one of the following values:
...
EXAMPLE
  ...
  is between SDSS_RESEL_MODE_FULL_SRV
...
static const  sdssscr_elem_u_type   ssscr_user_pwr_save_exit_gw_pwrup_wakeup_acq[] = {
  ...
  SRV_STATUS( NO_SRV )            /* Turn off service status */
  ACT_SCR( SRV_LOST_GW )    /* Do service lost scr for powe rup */
  #ifdef FEATURE_EOOS_UMTS
...
static const  sdssscr_elem_u_type   ssscr_user_pwr_save_exit_gw_wakeup_acq[] = {
  ...
  CPY( SRCH_LST, GWL_LST )         /* Append GWL systems to SRCH list */
  #ifdef FEATURE_MMODE_3GPP2
  IF( SRCH_LST_IS_EMPTY, EQUAL_TO, TRUE ) /* If SRCH_LST is empty, app prl list */
    APP( SRCH_LST,PRL_LST )
  ENDIF
...
*/
  ...
  static const sdssscr_elem_u_type  ssscr_user_ss_pref_csg_change_opr[] = {
    ...
    ENDIF
    //NEW_ACQ
    ACQ( SRCH_LST )/* Acquire full services on the first system */

    ACQ( SRV_SYS )     /* Try reacquiring serving system */
...
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_gw_emerg_call_orig_full_srv_req_man[] = {
  ...
  ACQ( SRCH_LST )   /* try acquiring FULL SRV on MANUAL system */
...
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_opr_gw_emerg_call_orig_ltd[] = {
  ...
  IF(FULL_SRV_REQ_EMERG,EQUAL_TO,TRUE)
   NEST_IF(PREV_ORIG_MODE_VAL, EQUAL_TO, SD_SS_ORIG_MODE_VAL_MANUAL)
    ...
    ACQ( SRCH_LST )   /* try acquiring FULL SRV on MANUAL system */
   NEST_ELSE
    ACQ( GWL_LST )   /* Trigger FULL SRV acquisition on GWL list */
   NEST_ENDIF
  ENDIF

  ACQ( GWL_LTD_LST )          /* Try acq any system */
  RESEL_STT( SS_PREF )            /* Set reselection state to SS_PREF */
  ACT_SCR( SRV_LOST_NORM_SLNT )   /* Do service lost normal silent */
  END
};
...
/* Script for when the user changes the SS-Preference and:
** 1. Current system is GW auto/manual mode.
** 2. Current system is GW in limited service.
** 3. Starting a normal call.
*/
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_opr_gw_call_orig_ltd[] = {

  /* Condition */
  IS_EVENT( USER_SS_PREF )        /* User changes the SS-Preference */
  IS_MODE_PREF( SRV_SYS, TDS_GSM_WCDMA  )    /* Srv sys is a TGW system */
  IS_ORIG_MODE( MAN_OR_NORM_ORIG )          /* Normal call */
  IS_SRV_STATUS( LIMITED_ANY )        /* Currently, have limited service */
...
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_opr_gw_orig_start_cs[] = {
  ...
  /* Script */
  RESEL_STT( SS_PREF )            /* Set reselection state to SS_PREF */
  ACQ(IDL_SYS)       /* Kick stack to acquire both service */
  PROC_EVT( GWL_OPR_SYS_LOST )     /* Do system lost processing */
  END
};
...
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_opr_gw_orig_start_csps[] = {
  ...
  /* Script */
  RESEL_STT( SS_PREF )            /* Set reselection state to SS_PREF */
  ACQ(IDL_SYS, ALLOWED)       /* Kick stack to acquire both service */
  PROC_EVT( GWL_OPR_SYS_LOST )     /* Do system lost processing */
  END
};
...
static const sdssscr_elem_u_type  ssscr_user_ss_pref_acq_resel[] = {
  ...
  #endif
  ACQ( IDL_SYS )         /* Try reacquiring the IDL_SYS */
  ACT_SCR( SRV_LOST_NORM )        /* Do normal service lost processing */
  END
};
...
** 1. SS-pref changes
** 2. The reason is call orgination end
** 3. State is in verify
** 4. If in limited regional service, continue
** 5. If main stack is on BSR and LTE connected mode not set and having no service: Continue with BSR
** 6. Otherwise, do service lost script
*/
static const  sdssscr_elem_u_type   ssscr_user_ss_pref_orig_end_vrfy[] = {
  ...
  ** from NAS. */
  IF(SRV_STATUS, EQUAL_TO, SDSSSCR_SRV_STATUS_VAL_LIMITED_REG)
  CONT(0)
  ENDIF
...
static const  sdssscr_elem_u_type   ssscr_user_nam_sel_gw[] = {
  ...
  /* Condition */
  IS_EVENT( USER_NAM_SEL )        /* User changes the NAM selection */
  //IS( CAP_3GPP_PREF_3GPP_ONLY )    
  /* GWL only capable targets no 1x.
...
static const  sdssscr_elem_u_type   ssscr_user_cdma_redir_lte_invalid[] = {
  ...
  REJECT_SYS                          /* Reject the redirection */
  PREV_SCR                        /* Continue with previous script */
...
static const  sdssscr_elem_u_type ssscr_int_srv_lost_lte_conn_mode[] = {
  ...
  IF( SRCH_LST_IS_VALID, EQUAL_TO, TRUE )
...
static const  sdssscr_elem_u_type ssscr_int_srv_lost_rlf_during_emerg[] = {
  ...
  IF( SRCH_LST_IS_EMPTY, NOT_EQUAL_TO, TRUE ) 
    ACQ( SRCH_LST )/* Acquire full service on the SRCH_LST */
  ENDIF
...
static const  sdssscr_elem_u_type   ssscr_int_srv_lost_emerg_orig_gw[] = {
  ...
  ACT_SCR( SRV_LOST_GW ) /* Do service lost scr for call orig in power up */
  ACT_SCR( PWR_SAVE_GW )          /* Nothing to acq - activate pwr-save scr*/
...
static const sdssscr_elem_u_type  ssscr_int_srv_lost_emerg_cb[] = {
  ...
  //AVOID_CANCEL( SRV_SYS )       /*   3. Cancel acq avoid of srv system */
  //ACQ_MODE( FULL )              /*   4. Do full acquisitions */
  ACQ ( SRV_SYS )       /*   5. Try reacquiring the serving system */
  ACT_SCR( SRV_LOST_EMERG_MODE )  /* Failed acq srv_sys,try all sys forever */
  END
};
#endif
...
static const  sdssscr_elem_u_type   ssscr_int_resel_lmtd_none[] = {
  ...
  RESEL_MODE( FULL_SRV )        /* Set reselection mode to full srv
                                  ** reselection stage 1.
                                  */
  RESET_TIMELINE                /* Set initial BPLMN stage */
  PROC_EVT( INT_RESEL )           /* Do reselection processing */
  END
};
...
static const  sdssscr_elem_u_type   ssscr_int_resel_lmtd_reacq[] = {
  ...
  SRV_REQ_TYPE( NORMAL )
  ACQ( IDL_SYS )             /*  Try acquiring limited GW service    */
...
/* Script for sleep interval during the full service reselection process 
*/
static const  sdssscr_elem_u_type   ssscr_int_resel_lmtd_wait[] = {

  /* Condition */
  IS_EVENT( INT_RESEL )           /* Calling for reselection processing */
  IS_RESEL_STT( WAIT )            /* Reselection in wait state */
  IS_RESEL_MODE( FULL_SRV)        /* Full srv reselection  */

  /* Script */
  CONT_LMTD_BSR                  /* Set limited to full BSR timer according to the curr stage */
  ...
  INC_COUNT(FULL_BAND_BSR)          /* Increment the count for full band scans */
...
static const  sdssscr_elem_u_type   ssscr_called_resel_full_srv_manual[] = {
  ...
  //DEL_SYS( SRCH_LST, NON_COMPLY )    
  /* Delete the non-complying sytems from
                                     ** the list
                                     */
  //NEW_ACQ
...
/* Script */
  ...
  RESET_GW_ACQ_SCAN_STATUS        /* Reset the GW acq scan status */
  #endif
  GET_NET ( MANUAL_LST,GWL_LAST_USER_SYS )
...
static const sdssscr_elem_u_type  ssscr_called_pwr_save_gw_pwrup[] = {
  ...
  RESET_TIMELINE                   /* Resets OOS timeline to start at initial step */
  ...
  LOOP_FOREVER                   
    ...
    ACT_SCR(ACQ_DB_SCAN_DURING_SLEEP_TIME)          /* Do ACQ DB Scan */    
    NEST_ELSE
    CONT_OOS
    NEST_ENDIF
    ENDIF
    #else
    IF(SCAN_SCOPE_RULE,CONTAINS,SYS_SCAN_SCOPE_RULE_ACQ_DB_OOS)
    ACT_SCR(ACQ_DB_SCAN_DURING_SLEEP_TIME)
    ELSE
    CONT_OOS
    ENDIF
    #endif
    ...
    INC_COUNT(FULL_BAND_OOS)
    //ACQ_MODE( MINI )              /* Switch to the mode other than FULL2 */

    #ifdef FEATURE_LIMITED_SCAN_OPT
    ACT_SCR(GWL_NO_SRV_LMTD_SYS_SCAN)
    #else
    ACQ( GWL_LAST_LTD_SYS )/* Try acquiring limited GWL service */
    #endif
...
static const sdssscr_elem_u_type ssscr_called_gwl_no_srv_plmn_found_lmtd_sys_scan[] = {
  ...
  IF(IS_RAL_STORED_IN_EFS, EQUAL_TO, TRUE) /* if RAL feature is enabled */
...
static const sdssscr_elem_u_type  ssscr_called_acq_db_scan_during_sleep_time[] = {

  #ifdef FEATURE_MMODE_3GPP
  /* OOS Enhancement with ACQ DB Scan */
  CONT3(T_ACQ_SCAN)
  
  LOOP_TIME_OOS             /* ACQ DB Scan during Deep Sleep */
  //NEW_ACQ
  SCAN_SCOPE_TYPE( ACQ_DB )
  SRV_REQ_TYPE(NORMAL)
  ACQ( PWR_SAVE_LST )  /* Acquire the last active system */
...
static const sdssscr_elem_u_type  ssscr_called_pwr_save_gw[] = {
  ...
  LOOP_FOREVER                   /* Loop forever */
    ...
    CLR( GWL_LAST_LTD_SYS )        /* Clear GWL LAST LTD system             */
    ACQ_MODE( EOOS_FULL2 )
    ACQ( PWR_SAVE_LST, ANY, SDSSSCR_FULL2_SYS, 1 )
                                  /* Acquire full services */
    PROC_EVT( INT_GW_ACQ_PLMN_RETURN )
...
/*---------------------------------------------------------------------------
                            SDSSSCR_PWR_SAVE_SCAN_GW
---------------------------------------------------------------------------*/
/* POWER SAVE SCAN SCRIPT to give acquisition during power save on GSM/UMTS target.
** Attempt for full service followed by the limited on the last acquired band
** before returning to the previous script.
*/

static const sdssscr_elem_u_type  ssscr_called_pwr_save_scan_gw[] = {

  //NEW_ACQ                       /* Start a new acquisition */
  LOOP_NUM_CFG( SCAN_COUNT )
    CLR( GWL_LAST_LTD_SYS )        /* Clear GWL LAST LTD system */
    ACQ( PWR_SAVE_LST )
                                  /* Acquire the last active system */
    INC_COUNT(FULL_BAND_OOS)
...
static const sdssscr_elem_u_type  ssscr_called_srv_lost_gw_emerg_call_orig_pwrup[] = {
  ...
  LOOP_FOREVER
    ...
    CLR( GWL_LAST_LTD_SYS )        /* Clear GWL LAST LTD system */
    //ACQ_MODE( FULL2 )
    ACQ( SRCH_LST )/* Acquire full services  */
    FULL_SRV_LOST
    ACQ_FAIL                      /*   Indicate that acquisition failed */
  LOOP
...
static const sdssscr_elem_u_type  ssscr_called_srv_lost_gw_emerg_call_orig[] = {
  ...
  LOOP_FOREVER /* Loop until timer expire.*/
    ...
    FULL_SRV_LOST
    //NEW_ACQ                       /* Start a new acquisition */
    ACQ( SRCH_LST )/* Acquire the last active system */
    FULL_SRV_LOST
    //NEW_ACQ                       /* Start a new acquisition */
    ACQ( SRCH_LST )/* Acquire the last active system */
    FULL_SRV_LOST
...
static const sdssscr_elem_u_type  ssscr_called_srv_lost_gw_orig[] = {
  ...
  //NEW_ACQ                    /* Start a new acquisition */
  ACQ( SRCH_LST )        /* Acquire full service on the first system */
  SRV_STATUS( NO_SRV )       /* Turn off service status */
...
static const sdssscr_elem_u_type  ssscr_called_srv_lost_gw_pwrup[] = {
  ...
  LOOP_LIST( SRCH_LST )         /* Loop for count of SRCH LIST time. */
    ...
    //ACQ_MODE( FULL2 )
    #ifdef FEATURE_MMODE_DYNAMIC_SV_OPERATION_SWITCH
    NEST_IF(SV_DYNAMIC_SWITCH_ENABLED, EQUAL_TO, TRUE)
      ACQ_CNT( SRCH_LST, REG_BEST_PREF, SDSSSCR_FULL2_SYS, 1)
    NEST_ELSE
    ACQ_CNT( SRCH_LST, ANY, SDSSSCR_FULL2_SYS, 1 )
    NEST_ENDIF
...
static const sdssscr_elem_u_type ssscr_called_non_sv_acq_db_scan_mru[] = {
  ...
  #else
  #ifndef FEATURE_JCDMA
  //IF(ACQ_STATUS, EQUAL_TO, SYS_ACQ_STATUS_SUCCESS)
  SRV_STATUS( NO_SRV )            /* Set service status to NO_SRV  */
  //ENDIF
  #endif
  IF (SCAN_SCOPE_RULE,CONTAINS,SYS_SCAN_SCOPE_RULE_ACQ_DB_OOS)
    //NEW_ACQ
    SCAN_SCOPE_TYPE( ACQ_DB )          /* Acq DB Scan */
    ACQ( TEMP_SYS )         /* Try acquiring recent MRU sys */
    SCAN_SCOPE_TYPE( FULL_BAND )    /* Reset the scan scope type to FULL_BAND */
  ENDIF
  #endif
...
static const sdssscr_elem_u_type ssscr_called_acq_db_scan_pwr_up_3gpp[] = {
  ...
  IF(ORIG_MODE,EQUAL_TO,SD_SS_ORIG_MODE_MANUAL_IDLE)
    ...
    CPY( SRCH_LST, MANUAL_LST )       /* Copy user list to srch list */
    //DEL_SYS( SRCH_LST, NON_COMPLY ) /* Delete the sytems from the list */
    ACQ( SRCH_LST )         /* Try acquiring recent MRU sys */      
  ELSE
    CPY( SRCH_LST,GWL_LST )
    #ifdef FEATURE_MMODE_3GPP2
...
FUNCTION sdssscr_validate_efs_oos_items

DESCRIPTION
  validates oos values read from EFS and updates if invalid .
  If any timer used for OOS is invalid, all OOS timers are defaulted

CONSTRAINTS
All timers should be in the defined range.
t_norm_max < t_slow_max
t_0 + x * t_delta < t_norm_max => t_0 < t_norm_max and t_delta < t_norm_max 
...
===========================================================================*/
static void                    sdssscr_validate_efs_oos_items(
   ...
   )
{
  ...
  t_norm_max = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_T_NORM_MAX,asubs_id); 
  if (!INRANGE(t_norm_max,SD_3GPP_T_NORM_MAX_RANGE_LOW,SD_3GPP_T_NORM_MAX_RANGE_HIGH))
  {
    goto oos_invalid;
  }
  /* T_slow max should be greater than T_norm_max . Define the max as 3 times the default value */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_T_SLOW_MAX,asubs_id); 
  if (!INRANGE(val,t_norm_max,SD_3GPP_OOS_T_SLOW_MAX_RANGE_HIGH) )
  {
    goto oos_invalid;
  }  

  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_REPS_PER_STAGE,asubs_id); 
  if (!INRANGE(val,SD_3GPP_REPS_PER_STAGE_MIN,SD_3GPP_REPS_PER_STAGE_MAX) )
  {
    goto oos_invalid;
  }
  /* T_0 can take values between 1s and T_NORM_MAX_HIGH . If T_0 is out of this range or
   ** if T_0 is greater than the value read in t_norm_max , go to default 
  */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_TIMER_VALUE1,asubs_id); 
  if (!INRANGE(val,SD_TIMER_MIN,SD_3GPP_T_NORM_MAX_RANGE_HIGH ) ||
      val > t_norm_max) 
  {
    goto oos_invalid;
  }

  /* T_delta can take values between 0s and MAX_TIME_DELT . If T_delta is out of this range or
   ** if T_delta is greater than the value read in t_norm_max , go to default 
  */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA,asubs_id); 
  if (!INRANGE(val,0,SD_3GPP_MAX_TIME_DELTA)  ||
      val > t_norm_max) 
  {
    goto oos_invalid;
  }

  /* SCAN_COUNTER can take values between SCAN_COUNTER_MIN and SCAN_COUNTER_MAX . If SCAN_COUNTER is out of this range 
   ** go to default 
  */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_OOS_SCAN_COUNTER,asubs_id); 
  if (!INRANGE(val,SD_3GPP_SCAN_COUNTER_MIN,SD_3GPP_SCAN_COUNTER_MAX))
  {
    goto oos_invalid;
  }

  /* If we got here, all values read from EFS were valid
  */
  return;

  /* If we got here, either slow scan is disabled or one of the OOS efs items was not configured correctly
  */
  oos_invalid:
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_T_NORM_MAX] = SD_3GPP_OOS_T_NORM_MAX_DEFAULT;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_REPS_PER_STAGE] = SD_3GPP_OOS_REPS_PER_STAGE;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_T_SLOW_MAX] = SD_3GPP_OOS_T_SLOW_MAX;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_TIMER_VALUE1] = SD_3GPP_OOS_TIMER_VALUE1 ;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA] = SD_3GPP_OOS_TIME_DELTA;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_SCAN_COUNTER] = SD_3GPP_OOS_SCAN_COUNTER;
  SD_MSG_HIGH_0( "Using default values for OOS");

} //sdssscr_validate_efs_oos_items
/*===========================================================================

FUNCTION sdssscr_validate_efs_bsr_items

DESCRIPTION
  validates bsr alues read from EFS and updates if invalid .
  If any timer used for BSR is invalid, all BSR timers are defaulted
...
===========================================================================*/
static void                    sdssscr_validate_efs_bsr_items(
   ...
   )
{
  ...
  t_norm_max = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX,asubs_id); 
  if (!INRANGE(t_norm_max,SD_3GPP_T_NORM_MAX_RANGE_LOW,SD_3GPP_T_NORM_MAX_RANGE_HIGH))
  {
    goto bsr_invalid;
  }
  ...
  /* T_slow max should be greater than T_norm_max . Define the max as 3 times the default value */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_BSR_MAX_TIME,asubs_id); 
  if (!INRANGE(val,t_norm_max,SD_3GPP_BSR_MAX_TIME * 3) )
  {
    goto bsr_invalid;
  }  

  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE,asubs_id); 
  if (!INRANGE(val,SD_3GPP_REPS_PER_STAGE_MIN,SD_3GPP_REPS_PER_STAGE_MAX) )
  {
    goto bsr_invalid;
  }
   ...
   ** if T_0 is greater than the value read in t_norm_max , go to default 
  */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_BSR_INITIAL_VAL,asubs_id); 
  if (!INRANGE(val,SD_TIMER_MIN,SD_3GPP_T_NORM_MAX_RANGE_HIGH ) ||
      val > t_norm_max) 
  {
    goto bsr_invalid;
   ...
   ** if T_delta is greater than the value read in t_norm_max , go to default 
  */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_BSR_TIME_DELTA,asubs_id); 
  if (!INRANGE(val,0,SD_3GPP_MAX_TIME_DELTA)  ||
      val > t_norm_max) 
  {
    goto bsr_invalid;
  ...
  */
  ...
  /* If we got here, either slow scan is disabled or one of the BSR efs items was not configured correctly
  */
  bsr_invalid:
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX] = SD_3GPP_BSR_T_NORM_MAX_DEFAULT;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE] = SD_3GPP_BSR_REPS_PER_STAGE;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_MAX_TIME] = SD_3GPP_BSR_MAX_TIME;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_INITIAL_VAL] = SD_3GPP_BSR_INITIAL_VAL ;
  sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_TIME_DELTA] = SD_3GPP_BSR_TIME_DELTA;
  SD_MSG_HIGH_0( "Using default values for BSR");

} //sdssscr_validate_efs_bsr_items
/*===========================================================================

FUNCTION sdssscr_slow_scan_efs_items_validate

DESCRIPTION
  validates bsr and oos values read from EFS and updates if invalid .
  If any timer used for OOS is invalid, all OOS timers are defaulted
  If any timer used for BSR is invalid, all BSR timers are defaulted
...
===========================================================================*/
static void                    sdssscr_slow_scan_efs_items_validate(
   ...
   )
{
  ...
  /* Validate acq db scan items */
  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_T_ACQ_SCAN,asubs_id); 
  if (!INRANGE(val,SD_TIMER_MIN,SD_T_ACQ_SCAN * 20) )
  {
    sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_T_ACQ_SCAN] = SD_T_ACQ_SCAN;
  }

  val = sdssscr_efs_item_get(SDSSSCR_EFS_ITEM_3GPP_SLOW_SCAN_ENABLED,asubs_id); 
  if (val == 0)
  {
    sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX] = SD_3GPP_BSR_T_NORM_MAX_DEFAULT;
    sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE] = SD_3GPP_BSR_REPS_PER_STAGE;
    ...
    sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA] = SD_3GPP_OOS_TIME_DELTA;
    sd_efs_items[asubs_id].items[SDSSSCR_EFS_ITEM_3GPP_OOS_SCAN_COUNTER] = SD_3GPP_OOS_SCAN_COUNTER;
    SD_MSG_HIGH_0( "Using default values for OOS/BSR");

    return;
  }
  
  sdssscr_validate_efs_bsr_items(asubs_id);
  sdssscr_validate_efs_oos_items(asubs_id);

} //sdssscr_slow_scan_efs_items_validate
/* <EJECT> */
/*===========================================================================
  ...
  */
  const sdssscr_sd_efs_items_s_type sd_efs_items_default = {
    ...
    */
    ...
    /* Default values for all the items stored in the EFS array. */
    {
      ...
      0,                                      /* SDSSSCR_EFS_ITEM_3GPP_SLOW_SCAN_ENABLED */
      
       SD_3GPP_OOS_T_NORM_MAX_DEFAULT ,        /* SDSSSCR_EFS_ITEM_3GPP_OOS_T_NORM_MAX */            
      
       /* 13 */
       SD_3GPP_OOS_REPS_PER_STAGE,             /* SDSSSCR_EFS_ITEM_3GPP_OOS_REPS_PER_STAGE */
      
       SD_3GPP_OOS_T_SLOW_MAX,                   /*  SDSSSCR_EFS_ITEM_3GPP_OOS_T_SLOW_MAX */
      
       SD_3GPP_OOS_TIMER_VALUE1,               /* SDSSSCR_EFS_ITEM_3GPP_OOS_TIMER_VALUE1 */
      
       SD_3GPP_OOS_TIME_DELTA,                 /* SDSSSCR_EFS_ITEM_3GPP_OOS_TIME_DELTA */
      
       /* 17*/
       SD_3GPP_BSR_T_NORM_MAX_DEFAULT ,     /* SDSSSCR_EFS_ITEM_3GPP_BSR_T_NORM_MAX */            
      
       SD_3GPP_BSR_REPS_PER_STAGE,             /* SDSSSCR_EFS_ITEM_3GPP_BSR_REPS_PER_STAGE */
      
       SD_3GPP_BSR_MAX_TIME,                   /*  SDSSSCR_EFS_ITEM_3GPP_BSR_MAX_TIME */
      
       SD_3GPP_BSR_INITIAL_VAL ,               /* SDSSSCR_EFS_ITEM_3GPP_BSR_INITIAL_VAL */
      
       /* 21 */
       SD_3GPP_BSR_TIME_DELTA,                 /* SDSSSCR_EFS_ITEM_3GPP_BSR_TIME_DELTA */

       SD_3GPP_OOS_SCAN_COUNTER,
  ...
  for(j=0; j<MAX_SIMS; j++)
  {
    ...
    */
    ...
    /* validate values read from EFS */
    if(is_validated == FALSE)
    {
      sdssscr_slow_scan_efs_items_validate(j);
    }
    /*#ifndef FEATURE_DISABLE_SD_INIT_NV_WRITE*/
    if( is_write_back )
    {
  ...
  */
  ...
  case SDSSSCR_CON_LOOP_TIME_TEMP_GSRDM: 
    ...
    *prm3_ptr = (sdssscr_prm_type) sd_cfg_items.items[ *prm3_ptr ];
    ...
    #endif
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    case SDSSSCR_CON_LOOP_TIME_OOS:  /* LOOP_TIME_OOS()                 */
    ...
    */
    ...
    case SDSSSCR_CON_CONT_OOS:
    case SDSSSCR_CON_CONT_LMTD_BSR:
    case SDSSSCR_CON_RESET_TIMELINE:
       idx_delta = 1;
      break;
    ...
    #endif
    case SDSSSCR_CON_INC_COUNT:          /* INC_COUNT ( VAL )*/
      idx_delta = 2;
      break;
  ...
  do
  {
    ...
    */
    switch( scr_con )/*lint -e788 */
    {
      ...
      */
      ...
      case SDSSSCR_CON_LOOP_TIME_OOS:    /* LOOP_TIME_OOS*/
        ...
        if( parser_ptr->loop_type == SDSSSCR_CON_MAX )
        {
          parser_ptr->loop_type = SDSSSCR_CON_LOOP_TIME_OOS;
          parser_ptr->loop_idx  = scr_idx;
          parser_ptr->loop_line = scr_line;
          parser_ptr->loop_val  = uptime +
                                    sdss_get_curr_oos_timer(parser_ptr->ss);
      
          SD_MSG_HIGH_3( "@@@@ LOOP_TIME_OOS, uptime=%d, loop_uptime=%d , ss=%d",
                        uptime, parser_ptr->loop_val,parser_ptr->ss);
        }
      ...
      case SDSSSCR_CON_LOOP:          /* LOOP()                         */
        ...
        */
        ...
        switch( parser_ptr->loop_type )
        {
          ...
          case SDSSSCR_CON_LOOP_TIME_TEMP_GSRDM:
          #endif
          case SDSSSCR_CON_LOOP_TIME_OOS:
    ...
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
           ...
           scr_con == SDSSSCR_CON_LOOP_TIME_OOS||
           is_skip_loop );
