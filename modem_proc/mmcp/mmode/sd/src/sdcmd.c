...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/sd/src/sdcmd.c#1 $
...
)
{
  ...
  */
  ...
  {
    sd_ss_ptr(ss)->gpp2.return_cause = return_cause;
  }
  #else
...
)
{
  ...
  */
    ...
    )
  {
    ...
    #else
    SD_MSG_HIGH_1( "lte connected mode %d", sdss_is_lte_connected_mode(ss) );
    #endif
...
)
{
  ...
  */
  ...
  SD_MSG_HIGH_1("SD_SS:FRFL: Updated Acq params list=%d", list);
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
)
{
  ...
  */
  ...
  SD_MSG_HIGH_3( "ss = %d system rec band: 0x%08x %08x ",
  ...
  */
  ...
  SD_MSG_HIGH_1("ue_cap.num_items = %d", ue_cap.num_items);
  ...
  if(sdss_is_mmss_operation(ss) 
    ...
    && !(sdss_is_ral_stored_in_efs(ss) && sdss_is_orig_mode(ss,SD_SS_ORIG_MODE_VAL_EMERG))
    #endif
   )
  {
    ...
    for(i = 0;i< sr_ptr->sid_expected.prm.gwl.num_rats;i++)
    {
      ...
      */ 
      if( mode == SD_MODE_LTE && 
          ...
          (ss == SD_SS_HYBR_2 && sdss_is_sfmode_1x_sxlte(sdss_map_ss_to_as_id(ss))))
        )
      {
    ...
    for(i = 0; i < ue_cap.num_items; i++)
    {
      ...
      switch(ue_cap.priority_list_info[i].acq_sys_mode)
      {
        ...
        case SYS_SYS_MODE_LTE:
          ...
          #if defined(FEATURE_MMODE_GW) && defined(FEATURE_MMODE_9X05_EMERG_CALL)
          if (!((ue_cap.priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE) 
                #if defined(FEATURE_CIOT)
                || (ue_cap.priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_M1)
                #endif
               )
              && sdss_is_emer_orig_in_volte_only(ss) &&
              (sdss_get_sim_state(ss) == SYS_SIM_STATE_AVAILABLE))
          ...
          if( (ue_cap.priority_list_info[i].acq_sys_mode == SD_MODE_LTE )
              #if defined(FEATURE_CIOT)
              || (ue_cap.priority_list_info[i].acq_sys_mode == SD_MODE_LTE_M1)
              || (ue_cap.priority_list_info[i].acq_sys_mode == SD_MODE_LTE_NB1)
               #endif
            )
          {
            rat_pri_ptr->priority_list_info[j].band_cap.lte_band_cap =
                                ue_cap.priority_list_info[i].band_cap.lte_band_cap;
            SD_MSG_HIGH_1("rat_info->acq_sys_mode: LTE ,band_pref = %d", band_pref);
          ...
          else
          {
            rat_pri_ptr->priority_list_info[j].band_cap.chgwt_band_cap =
                                ue_cap.priority_list_info[i].band_cap.chgwt_band_cap;
            SD_MSG_HIGH_4( "rat_info->acq_sys_mode: %d, rat_pri_ptr->priority_list_info[j].band_cap: 0x%08x %08x,band_pref = %d",
                           rat_info->acq_sys_mode,
          ...
          */
          if ((rat_info->acq_sys_mode == SD_MODE_LTE) 
              #if defined(FEATURE_CIOT)
              || (rat_info->acq_sys_mode == SD_MODE_LTE_M1)
              || (rat_info->acq_sys_mode == SD_MODE_LTE_NB1)
              #endif
             )
          {
             lte_band_pref = sdsr_map_sys_rec_lte_band_to_lte_band_pref(sr_ptr->sys.lte_band);
          ...
          #endif
          #if defined FEATURE_MMODE_3GPP2 || defined SD_GSM_SUPPORTED || defined SD_WCDMA_SUPPORTED
          else
          {
             band_pref = sr_ptr->sys.band;
          }
          ...
          */
          ...
          /* if in CIOT full_RAT, LTE RATs needs to be scanned even for PS_INVALID case
          ...
          if((rat_info->acq_sys_mode == SD_MODE_LTE 
             ...
             #endif
             #if defined(FEATURE_CIOT)
             && !sdss_is_in_full_rat_mode_ciot(ss)
             #endif
             &&(ss_mode_pref != SD_SS_MODE_PREF_LTE
              #ifdef FEATURE_MMODE_3GPP2
               ||(ss == SD_SS_HYBR_2 && sdss_is_sfmode_1x_sxlte(sdss_map_ss_to_as_id(ss)))
          ...
          {
            SD_MSG_HIGH_3("Skip RAT mode = %d as sim state:%d is invalid or domain sel:%d",
                     rat_info->acq_sys_mode,sdss_get_sim_state(ss),sdss_get_domain_pref(ss));
            continue;
          }
          ...
          /* if in CIOT full_RAT, LTE RATs needs to be scanned even for PS_INVALID case
                   * to determine location */
          if((rat_info->acq_sys_mode == SD_MODE_GSM)
          ...
          /* If the SIM state is PS invalid or CS_PS_Invalid no need to scan LTE_NB1*/
          /* if in CIOT full_RAT, LTE_NB1 RATs needs to be scanned even for PS_INVALID or CS_PS_Invalid case
                   * to determine location */
          if((rat_info->acq_sys_mode == SD_MODE_LTE_NB1)
              && ((sdss_get_sim_state(ss)== SYS_SIM_STATE_PS_INVALID)
          ...
          #if defined(FEATURE_CIOT)
          else if((sr_ptr->sid_expected.net_select_mode == SD_NET_SELECT_MODE_LIMITED) &&
                   (rat_info->acq_sys_mode == SYS_SYS_MODE_LTE_NB1)
                 )
          {
            if(rat_info->acq_sys_mode == SYS_SYS_MODE_GSM)
            {
			...
			else if(rat_info->acq_sys_mode == SYS_SYS_MODE_LTE_NB1)
			{
              rat_info->bst_band_cap.lte_band_cap = SYS_LTE_BAND_MASK_CONST_NONE;
			}
            rat_info->bst_rat_acq_required = FALSE;
            SD_MSG_HIGH_0("Skip limited routine acqusiition for NB1");
          }
          #endif
          #if defined(FEATURE_MMODE_9X05_EMERG_CALL)
          /* If UE is trying to perform initial attach on LTE during an emergency call
          ...
          */ 
          ...
          {
            SD_MSG_HIGH_0("LTE_911:skip GWT or NB1 full scan");
            rat_info->bst_rat_acq_required = FALSE;
            #if defined(FEATURE_CIOT)
            if(rat_info->acq_sys_mode == SD_MODE_LTE_NB1)
            {
          ...
          #endif
          #if defined(FEATURE_CIOT) && !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
          /* if in CIOT full_RAT, LTE RATs needs to be scanned if for PS_INVALID case
                   * to determine location */
          else if(((sdss_is_orig_mode(ss,SD_SS_ORIG_MODE_VAL_EMERG) &&
                    !sdss_is_in_full_rat_mode_ciot(ss)) ||
                   sdss_is_orig_mode(ss,SD_SS_ORIG_MODE_VAL_VOLTE)) &&
                  (rat_info->acq_sys_mode != SD_MODE_LTE_M1))
          {
            SD_MSG_HIGH_0("skip G & NB1 during VOLTE call");
            rat_info->bst_rat_acq_required = FALSE;
            #if defined(FEATURE_CIOT)
            if(rat_info->acq_sys_mode == SD_MODE_LTE_NB1)
            {
          ...
          #endif
          else if(sdss_is_orig_mode(ss,SD_SS_ORIG_MODE_VAL_MAN_OR_NORM_ORIG)
            &&(rat_info->acq_sys_mode == SD_MODE_LTE_NB1
              #ifdef FEATURE_MMODE_DISABLE_NON_LTE_VOICE
              || rat_info->acq_sys_mode == SD_MODE_GSM
              #endif
              ))
          {
            SD_MSG_HIGH_1("skip %d during voice call",rat_info->acq_sys_mode);
...
)
{
  ...
  sd_ss_ciot_lte_pref_s_type     get_net_ciot_lte_pref;
  sd_ss_mode_pref_e_type          acq_sys_mode_pref;
   /* clear rat pri ptr before updating from acq record
  */
  memset(rat_pri_ptr, 0, sizeof(sys_rat_pri_list_info_s_type));
  memset(&get_net_ciot_lte_pref, 0, sizeof(sd_ss_ciot_lte_pref_s_type));

  #if defined(FEATURE_CIOT)
  sdss_get_net_ciot_lte_pref(ss, &get_net_ciot_lte_pref);
  #endif

  for(i = 0;i< rat_acq_order.num_rat;i++)
  {
    acq_sys_mode_pref = sdss_map_mode_to_mode_pref(rat_acq_order.acq_sys_mode[i]);
    if ( SD_MODE_CONTAIN( temp_pref, acq_sys_mode_pref ))
    {
      #if defined(FEATURE_SD_LTE)
      if ((rat_acq_order.acq_sys_mode[i] == SD_MODE_LTE) 
           #if defined(FEATURE_CIOT)
           && (get_net_ciot_lte_pref.lte_op_mode_pref & BM(SD_MODE_LTE))
           #endif
         )
      {
        ...
        #else
        lte_band_pref = SYS_LTE_BAND_MASK_COMMON(&(get_net_ptr->lte_band_pref),&(get_net_ciot_lte_pref.lte_band_pref));
        #endif
        //temp_pref = SD_MODE_DELETE(temp_pref, SD_SS_MODE_PREF_LTE);
      }
      #if defined(FEATURE_MMODE_M1)
      else if ((rat_acq_order.acq_sys_mode[i] == SD_MODE_LTE_M1) &&
               (get_net_ciot_lte_pref.lte_op_mode_pref & BM(SD_MODE_LTE_M1)))
      {
        get_sys_mode = SD_MODE_LTE_M1;
        lte_band_pref = SYS_LTE_BAND_MASK_COMMON(&(get_net_ptr->lte_band_pref),&(get_net_ciot_lte_pref.lte_m1_band_pref));	
        SD_MSG_HIGH_0("get net: adding M1");
      }
      #endif
      #if defined(FEATURE_MMODE_NB1)
      else if ((rat_acq_order.acq_sys_mode[i] == SD_MODE_LTE_NB1) &&
               (get_net_ciot_lte_pref.lte_op_mode_pref & BM(SD_MODE_LTE_NB1)))
      {
        get_sys_mode = SD_MODE_LTE_NB1;
        lte_band_pref = SYS_LTE_BAND_MASK_COMMON(&(get_net_ptr->lte_band_pref),&(get_net_ciot_lte_pref.lte_nb1_band_pref));
        SD_MSG_HIGH_0("get net: adding NB1");
      }
      #endif
      else
      #endif
      ...
      #ifdef SD_WCDMA_SUPPORTED
      if (rat_acq_order.acq_sys_mode[i] == SD_MODE_WCDMA)
      {
    ...
    if( get_sys_mode != SD_MODE_LTE  
        #ifdef FEATURE_CIOT 
        && get_sys_mode != SYS_SYS_MODE_LTE_M1
         && get_sys_mode != SYS_SYS_MODE_LTE_NB1
        #endif
     )
    {
    ...
    #endif
    if(SYS_LTE_BAND_MASK_CHK_IF_EMPTY(&lte_band_pref))
    {
      continue;
    }
  ...
  for(i = 0; i < rat_pri_ptr->num_items; i++ )
  {
    ...
    #ifdef FEATURE_CIOT
    && rat_pri_ptr->priority_list_info[i].acq_sys_mode != SYS_SYS_MODE_LTE_M1
    && rat_pri_ptr->priority_list_info[i].acq_sys_mode != SYS_SYS_MODE_LTE_NB1
    #endif
    )
    {
    ...
    {
      SD_MSG_HIGH_2("GET_NET:sys_mode = %d, acq_reqd = %d",rat_pri_ptr->priority_list_info[i].acq_sys_mode,
                                                           rat_pri_ptr->priority_list_info[i].bst_rat_acq_required);
...
#endif /* #ifdef FEATURE_MMODE_3GPP */
  ...
  */
  {
    ...
    SD_MSG_HIGH_2("Mapping net_sel_type %d -> %d",
...
)
{
  ...
  */
  ...
  static sd_ss_s_type*  sd_ss_arr[1] = {&sd_ss};  
  SYS_ARG_NOT_USED(ss);
  return sd_ss_arr[0];
...
boolean                    sd_is_opti_ss_pref_change(
       ...
       sdss_pref_updated_mask_type   pref_change,
...
)
{
  ...
  */
      ...
      sdss_get_is_RLF_during_emerg(ss) == TRUE )
  {
    ...
    */
    if(!((SD_IS_BIT_CONTAIN (ss_mode_pref ,SD_SS_MODE_PREF_LTE))
          #if defined(FEATURE_CIOT)
          || (ciot_lte_pref.lte_op_mode_pref & BM(SD_MODE_LTE_M1))
          #endif
        )
      )
    {
  ...
  */
  ...
  #if defined (FEATURE_MMODE_HYBR2_ENABLED) && defined FEATURE_MMODE_3GPP2
       ...
       if (sdsr_list_cnt(SDSR_MORE_PREF_LST_) > 0)
       {
        ...
        start_sys = SDSR_SRV_SYS;
         ...
         (sd_get_ss_state(SD_SS_MAIN) == SDSS_STATE_OPR 
          || (sdss_get_scan_scope_rule(SD_SS_MAIN) & SYS_SCAN_SCOPE_RULE_ACQ_DB_NO_TRM
...
{  
  ...
  */
  ...
  #if defined(FEATURE_MMODE_3GPP) && defined(FEATURE_MMODE_9X05_EMERG_CALL)
  if (sdss_is_emer_orig_in_volte_only(ss) && 
      (si_info_ptr->srv_status != SYS_SRV_STATUS_SRV) &&
      (sdss_get_sim_state(ss) == SYS_SIM_STATE_AVAILABLE))
  {
...
)
{
  ...
  */
  ...
  man_sr_ptr = sdsr_list_get_ptr( lst, 0 );
  ...
  */
      ...
      */
      ...
      switch( sr_ptr->sys.mode )
      {
        ...
        case SD_MODE_GWL:
          ...
          ss_act_ptr->prm.acq_gwl.acq_order_pref  = SD_SS_ACQ_ORDER_PREF_AUTO;
          #endif
          ...
          */
          ...
          #ifdef FEATURE_MMODE_9X05_EMERG_CALL
          ...
          */
          ...
          #else
          sd_get_band_mode_lte( &ss_act_ptr->prm.acq_gwl.gwl_mode_pref,
                                  &ss_act_ptr->prm.acq_gwl.lte_band_pref,
                                  ss_mode_pref,
                                  ss_band_pref_lte);
          #endif
          ...
          */
              ...
              ss_orig_mode == SD_SS_ORIG_MODE_EMERG_CB )
          {
            ...
            if (sdss_get_is_RLF_during_emerg(ss) == TRUE ) 
            {
          ...
          if( ss_orig_mode == SD_SS_ORIG_MODE_EMERG_ORIG)
          {
             ss_act_ptr->prm.acq_gwl.net_sel_type =
                                    SYS_NETWORK_SELECTION_MODE_LIMITED;
             if( sdss_is_in_full_rat_mode_ciot(ss) )
             {
               ss_act_ptr->prm.acq_gwl.srv_req_type = SD_SS_SRV_REQ_TYPE_FULL_RAT;
             }
          }
          #endif
          ...
          */
          ...
          /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
          #if defined FEATURE_AVOID_DUP_BAND_SCAN || defined FEATURE_LIMITED_SCAN_OPT         
          ...
          */
          {
            ...
            SD_MSG_MED_0("update rat_pri_list with filtered bands");
          ...
          */
          ...
          if( sdss_get_camp_mode() == SD_SS_PREF_CAMP_MODE_OFF  &&
              *sdss_misc_move_to_unrestricted() == TRUE
            )
          {
            if((ss_act_ptr->prm.acq_gwl.srv_req_type == SD_SS_SRV_REQ_TYPE_NORMAL        ||
               ss_act_ptr->prm.acq_gwl.srv_req_type == SD_SS_SRV_REQ_TYPE_SYSTEM_CHANGE ||
            ...
            *sdss_misc_move_to_unrestricted() = FALSE;
          }
  ...
  //        SD_MSG_HIGH_1("srv_req_type-%d",ss_act_ptr->prm.acq_gwl.srv_req_type);
          ...
          */
          ...
          #if defined(FEATURE_CIOT)
          /* Mark GW acquisition triggered */
          ss_ptr->is_gw_scan_attempted = TRUE;
          #endif
    ...
    #ifdef FEATURE_MMODE_3GPP
    {
      ...
      if  (( ss_gwl_app_type ==  MMGSDI_APP_SIM ) 
       ...
       {
         /*If inserted SIM is 2G disable LTE bands */
         SD_MSG_HIGH_0("2G SIM inserted disabled LTE ");
      ...
      sdss_get_network_pref_read( ss,
                                  ...
                                  NULL);
      #endif
      /* If the SIM state is PS invalid no need to search LTE*/
      if(SD_MODE_CONTAIN( ss_mode_pref, SD_SS_MODE_PREF_LTE) &&
         ((sdss_get_sim_state(ss)== SYS_SIM_STATE_PS_INVALID) ||
          (sdss_get_sim_state(ss)== SYS_SIM_STATE_CS_PS_INVALID))
        )
      {
        SD_MSG_HIGH_0("Skip LTE as sim state is invalid");
        ss_act_ptr->prm.get_net_gw.lte_band_pref = SYS_LTE_BAND_MASK_CONST_NONE;
      }
      ...
      ** preferences of get network request.*/
      ...
      #else
      sd_get_band_mode_lte(&ss_act_ptr->prm.get_net_gw.gw_mode_pref,
                             &ss_act_ptr->prm.get_net_gw.lte_band_pref,
                             ss_mode_pref,
                             ss_band_pref_lte);
      #endif
      ...
      ** preferences of get network request.*/
       ...
       #if !defined FEATURE_MMODE_DISABLE_MANUAL_BST
       {
         ...
         if(get_net_gw.gw_mode_pref != SD_SS_MODE_PREF_NONE)
         {
           ...
           sdss_get_network_pref_read( ss,
                                   ...
                                   &ss_band_pref_lte,
                                   NULL);
           sd_get_band_mode_lte(&get_net_gw.gw_mode_pref,
                                   &get_net_gw.lte_band_pref,
                                   ss_mode_pref,
                                   ss_band_pref_lte);
...
)
{
  ...
  *list     = ss_ptr->acq_list;
  ...
  SD_MSG_HIGH_1("SD_SS:FRFL: Read Acq params list=%d,", *list);
} 
#endif
    ...
    if( in_rat_pri_ptr->priority_list_info[i].bst_rat_acq_required == TRUE &&
        (
         ((in_rat_pri_ptr->priority_list_info[i].acq_sys_mode != SYS_SYS_MODE_LTE 
          #ifdef FEATURE_CIOT
          && in_rat_pri_ptr->priority_list_info[i].acq_sys_mode != SYS_SYS_MODE_LTE_M1
          && in_rat_pri_ptr->priority_list_info[i].acq_sys_mode != SYS_SYS_MODE_LTE_NB1
          #endif
         ) &&
...
)
{
  ...
  if( rtrn_act_ptr != NULL )
  {
    ...
    SD_CRIT_SECT_ENTER();
    sd_ss_ptr(ss)->act = rtrn_act_ptr->act;
...
#if defined (FEATURE_MMODE_HYBR2_ENABLED) || defined(FEATURE_MMODE_HYBR3_ENABLED)
/*===========================================================================
    ...
    case SD_SS_ACT_ACQ_GWL:
      ...
      */
      ...
      #else
      {
        ...
        #else
        if(!SYS_LTE_BAND_MASK_CHK_IF_EQUAL(&act_a_ptr->prm.acq_gwl.lte_band_pref,
                                           &act_b_ptr->prm.acq_gwl.lte_band_pref)
        #endif
        {
          return FALSE;
...
#if defined FEATURE_MMODE_3GPP2 && defined FEATURE_MMODE_3GPP
/*===========================================================================
...
===========================================================================*/
  void                            sd_si_update(
        ...
        /* camped lte mode of operation */
        sd_mode_e_type                  ciot_lte_mode,

        /* Accepted cio capabilities from NW */
        sys_neg_ciot_cap                 neg_ciot_cap,

        /* epco supported */
        boolean                          epco_supported,

        /* SMS status over control plane */
...
/*lint -esym(715, csg_info_ptr) */ /* param not referenced */
{
  ...
  */
  ...
  else
  {
    ...
    if(ss == SD_SS_MAIN && mode == SD_MODE_CDMA && !is_silent)
    {
  ...
  ** realign flag to TRUE */
  if(sdss_is_c2k_alt_algr_operation() && sdss_is_1x_sxlte())
  {
    if(mode == SD_MODE_CDMA
      &&  sys_srv_status_is_srv(srv_status)
      && (!sd_is_same_sid(si_info_ptr->mode, &prev_sid, sr_ptr) ||
  ...
  */
  ...
  {
    SD_MSG_HIGH_1("Invalid reg_domain %d changing to NOT_APPLICABLE",
                                                              reg_domain);
    reg_domain = SYS_REG_DOMAIN_NOT_APPLICABLE;
  }
  #endif
  ...
  if( si_info_ptr->srv_status           == srv_status &&
      ...
      ( mode_info_ptr == NULL ||
       ...
       ( 
         ...
         si_info_ptr->mode_info.gwl.rac_or_mme_code == mode_info_ptr->gwl.rac_or_mme_code
         #ifdef FEATURE_MMODE_GW
      ...
      */
      ...
      #endif 
      #if defined(FEATURE_CIOT)
      && (si_info_ptr->ciot_lte_mode == ciot_lte_mode)
      && (0 == memcmp(&si_info_ptr->neg_ciot_cap, &neg_ciot_cap, sizeof(sys_neg_ciot_cap)))
      && (si_info_ptr->cp_sms_service_status == cp_sms_service_status)
      #endif
      && (si_info_ptr->epco_supported == epco_supported)
    )
  {
    return;
  }
  ...
  */
  ...
  #if defined(FEATURE_CIOT)
  si_info_ptr->ciot_lte_mode     = ciot_lte_mode;
  si_info_ptr->neg_ciot_cap          = neg_ciot_cap;
  si_info_ptr->cp_sms_service_status = cp_sms_service_status;
  #endif
  ...
  */
  ...
  
  {
    ...
    sys_lte_band_mask_e_type    ss_lte_band_pref = SYS_LTE_BAND_MASK_CONST_ANY;
  ...
  */
  ...
  #if defined (FEATURE_LTE) 
  SD_MSG_HIGH_2( "ims_voice_avail=%d, emerg %d",
               si_info_ptr->lte_ims_voice_avail,si_info_ptr->lte_ims_emerg_avail);
  #endif

  SD_MSG_HIGH_6( "sd_si_update | mode=%d,band=%d,roam=%d, req_id=%u, sd_si_ptr->req_id=%u, extend srv info=%d", 
                 sr_ptr->sys.mode, band, sr_ptr->roam, req_id, sd_si_ptr(ss)->si_info.sys_sel_pref_req_id,
  ...
  #if defined(FEATURE_CIOT)
  SD_MSG_HIGH_6( "ciot_lte_mode =%d, neg_ciot_cap capabilities cp_ciot_supported=%d, up_ciot_supported=%d"
                 "hc_cp_ciot_supported =%d, s1_u_data_supported =%d, esr_without_pdn_supported =%d",
                si_info_ptr->ciot_lte_mode, si_info_ptr->neg_ciot_cap.cp_ciot_supported,
                si_info_ptr->neg_ciot_cap.up_ciot_supported,
                si_info_ptr->neg_ciot_cap.hc_cp_ciot_supported,
                si_info_ptr->neg_ciot_cap.s1_u_data_supported,
                si_info_ptr->neg_ciot_cap.esr_without_pdn_supported);
  

  SD_MSG_HIGH_1("cp_sms_service_status =%d",si_info_ptr->cp_sms_service_status);
  #endif
  SD_MSG_HIGH_1("epco_supported = %d",si_info_ptr->epco_supported);
...
void                      sd_si_update_if_srv_is_off(
       ...
       sd_mode_e_type               ciot_lte_mode,
         /* ciot let mode of operation 
         */

       sys_neg_ciot_cap              neg_ciot_cap,
         /*  Negotiated ciot capabilities with NW
         */
...
)
{
  ...
  sys_lte_band_mask_e_type band_capability_lte = SYS_LTE_BAND_MASK_CONST_NONE;
  #ifndef FEATURE_MMODE_LTE_ONLY
...
)
{
  ...
  */
  SD_CRIT_SECT_ENTER();
  act = sd_ss_ptr(SD_SS_MAIN)->act;
  if( rtrn_act_ptr != NULL && act != SD_SS_ACT_CONTINUE )
  {
...
)
{
  ...
  */
      ...
      ss_act.act != SD_SS_ACT_CONTINUE )
  {
    ...
    */
    ss_ptr->gpp2.gps_is_opti_ss_pref = TRUE;
  ...
  ** preference change.
  */
  else if( ss_ptr->gpp2.gps_is_opti_ss_pref &&
           ...
           orig_mode == SD_SS_ORIG_MODE_NONE )
  {
    ...
    */
    ss_ptr->gpp2.gps_is_opti_ss_pref = FALSE;
...
#endif /*  #if defined(FEATURE_HDR) || defined(FEATURE_HDR_HYBRID)*/
...
void sd_display_eoos_params (const sd_ss_act_acq_gwl_s_type*  act_ptr)
{
    ...
    SD_MSG_HIGH_5( "EOOS for GW: mode_pref: 0x%x use timer: 0x%x new acq: 0x%x EOOS for GW scan times: wcdma_scan: %ld [ms] gsm_scan: %ld [ms]",
...
)
{
  ...
  */
  #ifdef FEATURE_SD_LTE
  sdss_get_nas_addl_info(ss, &ext_srv_info);
  #ifdef FEATURE_MMODE_9X05_NOTREQ
  if (lte_cs_capability != SYS_LTE_CS_CAPABILITY_NONE)
  {
    si_info_ptr->lte_cs_capability = lte_cs_capability;
...
)
{
  ...
  */
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  #if defined FEATURE_SD_LTE && !defined FEATURE_MMODE_DISABLE_LTE_VOICE
...
)
{
  ...
  #if defined(FEATURE_CIOT)
  /* if GWL acquisition is triggered from last full_srv_lost indication 
     * mark is_scan_attempted to TRUE */
  if( si_ptr->full_srv_lost_cb_func != NULL &&
      ss_ptr->is_gw_scan_attempted == TRUE )
  {
    SD_MSG_HIGH_1( "full_srv_lost ss=%d",ss);
    si_ptr->full_srv_lost_cb_func( ss, TRUE);
    ss_ptr->is_gw_scan_attempted = FALSE;
  }
  else
  #endif
  if( si_ptr->full_srv_lost_cb_func != NULL )
  {
    si_ptr->full_srv_lost_cb_func( ss, FALSE  );
  }
...
)
{
  ...
  SD_MSG_HIGH_3("SD->CM: MMSOA: MCC =%d to CM, ss=%d, mode=0x%x", mcc, ss, mode);
...
#endif /* #ifdef FEATURE_MMODE_3GPP2*/
  ...
  else
  {
    ...
    else
    {
      ...
      */
      ...
      #else
      sdss_get_user_mode_pref(ss, &ss_true_mode_pref,
                               NULL, NULL, NULL);
      #endif
...
)
{
  ...
  sdss_pref_updated_mask_type is_pref_change;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  SD_MSG_HIGH_5( "main:pref_reason=%d, orig_mode=%d, mode_pref=0x%x band_pref:(MSB)(LSB) cgw=0x%08x 0x%08x",
  ...
  SD_MSG_HIGH_4( "domain_pref=0x%x,pref_update_reason: %d,user_mode_pref %d, req Id %d",
                cmd->domain_pref,
                cmd->pref_update_reason, cmd->user_mode_pref, cmd->req_id);
  SD_MSG_HIGH_3( "camp_mode: %d, CSG_RAT=%d,user_mode_pref: %d", cmd->camp_mode, cmd->csg_rat,cmd->user_mode_pref);

  #ifdef FEATURE_CIOT
  SD_MSG_HIGH_1( "lte_op_mode %d LTE M1 band pref", cmd->ciot_lte_pref.lte_op_mode_pref);
  sd_print_lte_band_mask(cmd->ciot_lte_pref.lte_m1_band_pref);
  sd_print_lte_band_mask(cmd->ciot_lte_pref.lte_nb1_band_pref);
  sd_print_lte_band_mask(cmd->ciot_lte_pref.lte_band_pref);
  ...
  cmd->orig_mode = sdss_compute_orig_mode(cmd->orig_mode, \
    sdss_get_supp_mode_pref_mmode(cmd->mode_pref,cmd->band_pref, cmd->band_pref_lte, SD_SS_BAND_PREF_NONE), \
    NULL);
  #endif
  ...
  */
    ...
    )
  {
    ...
    sd_ss_ptr(SD_SS_HYBR_HDR)->gpp2.is_start_instance = TRUE;
  }
  #endif
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  is_pref_change = sdss_pref_update( cmd->pref_reason,
   ...
   SD_MSG_HIGH_2("sd_ss_ind_user_ss_pref7 | setting csg data csg_rat = %d is_pref_change %d", 
                       cmd->csg_rat,is_pref_change);

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /* Check if prl preference changed.
  */
  #if !defined FEATURE_MMODE_DISABLE_PRL && defined FEATURE_MMODE_3GPP2
  ...
  */
  ...
  #endif
  if ( cmd->pref_reason == SD_SS_PREF_REAS_USER_RESEL &&
  ...
  #if defined FEATURE_AVOID_DUP_BAND_SCAN && !defined FEATURE_MMODE_DISABLE_LTE_VOICE
  ...
  */
  ...
  if( sd_ss_act_is_equal(curr_act_ptr, prev_act_ptr, SD_SS_MAIN))
  {
...
)
{
  ...
  #ifndef FEATURE_MMODE_LTE_ONLY
  sys_lte_band_mask_e_type band_ptr_lte = SYS_LTE_BAND_MASK_CONST_ANY;
  #endif
...
)
{
  ...
  */
  ...
  SD_ASSERT( INRANGE(cmd->pos, 0, sdsr_list_cnt(sdss_sr_list_map2(SD_SS_HYBR_2,SDSR_MEAS_LST))) );
  SD_ASSERT( cmd->rtrn_mode_ptr != NULL );
  SD_ASSERT( cmd->rtrn_band_ptr != NULL );
  SD_ASSERT( cmd->rtrn_chan_ptr != NULL );
...
)
{
  ...
  */
  ...
  SD_ASSERT( INRANGE(cmd->pos, 0, sdsr_list_cnt(sdss_sr_list_map2(SD_SS_HYBR_3,SDSR_MEAS_LST))) );
  SD_ASSERT( cmd->rtrn_mode_ptr != NULL );
  SD_ASSERT( cmd->rtrn_band_ptr != NULL );
  SD_ASSERT( cmd->rtrn_chan_ptr != NULL );
    ...
    sdcmd_hybr_2_user_pwr_up( sd_ss_ind_user_pwr_up_cmd *cmd)
{
  ...
  */
  ...
  #if defined FEATURE_MMODE_DUAL_SIM && defined FEATURE_MMODE_3GPP2
  if(sdss_is_featuremode(SYS_OVERALL_FEATURE_MODE_MULTISIM) &&
      ...
      sdcmd_hybr_3_user_pwr_up( sd_ss_ind_user_pwr_up_cmd *cmd)
{
  ...
  */
  ...
  #if defined(FEATURE_MMODE_HYBR3_ENABLED) && defined(FEATURE_MMODE_3GPP2)
...
sd_ss_act_e_type    sdcmd_hybr_2_user_ss_pref6( sd_ss_ind_user_ss_pref_cmd  *cmd)
{
  ...
  ** If not exit it and acquire srv on CS RATs
  */
  #if defined FEATURE_LTE && !defined FEATURE_MMODE_DISABLE_LTE_VOICE
  if ((cmd->pref_reason == SD_SS_PREF_REAS_ORIG_START_CS) &&
      (cmd->orig_mode == SD_SS_ORIG_MODE_EMERG_ORIG ) &&
  ...
  */
  ...
  #if defined FEATURE_MMODE_HYBR2_ENABLED && !defined FEATURE_MMODE_DISABLE_LTE_VOICE
  if( cmd->orig_mode == SD_SS_ORIG_MODE_EMERG_ORIG && 
...
)
{
  ...
  */
  ...
  if (is_get_net_allowed)
  {
    ...
    sdss_get_network_pref_update( ss, cmd->mode_pref, cmd->band_pref, cmd->band_pref_lte, 
                                  cmd->list_type, cmd->ciot_lte_pref);
  ...
  */
  ...
  sd_ss_ptr(ss)->true_curr_act.prm.get_net_gw.cell_search_num_cells = cmd->cell_search_num_cells;
  if( cmd->list_type == SD_NETWORK_LIST_GPS_ADV_SEARCH )
  {
    sd_ss_ptr(ss)->true_curr_act.prm.get_net_gw.gps_wwan_scan_params      = cmd->gps_wwan_scan_params;
  }
  if(NULL != cmd->rtrn_act_ptr)
  {
    cmd->rtrn_act_ptr->prm.get_net_gw.network_list_type         = cmd->list_type;
    cmd->rtrn_act_ptr->prm.get_net_gw.cell_search_num_cells     = cmd->cell_search_num_cells;
    if( cmd->list_type == SD_NETWORK_LIST_GPS_ADV_SEARCH )
    {
      cmd->rtrn_act_ptr->prm.get_net_gw.gps_wwan_scan_params     = cmd->gps_wwan_scan_params;
    }
  }
...
)
{
  ...
  */
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  memset(&neg_ciot_cap, 0, sizeof(neg_ciot_cap));
...
)
{
  ...
  SD_ASSERT( sdss_is_supp_sys(SD_MODE_CDMA, sd_map_band_to_band_pref(cmd->band), cmd->chan) );
...
)
{
  ...
  */
  ...
  ss_ptr->gpp2.cdma_act_mask = ( sd_ss_cdma_act_mask_e_type )SD_BIT_ADD(
                                ss_ptr->gpp2.cdma_act_mask, cmd->cdma_act_mask );

  SD_MSG_HIGH_3( "Cdma_activity set to TRUE. Start 1x activity 0x%x mask 0x%x, Mask from 1x = 0x%x",
                 cmd->cdma_act_mask, ss_ptr->gpp2.cdma_act_mask, cmd->cdma_act_mask);
...
)
{
  ...
  */
  ...
  ss_ptr->gpp2.cdma_act_mask = ( sd_ss_cdma_act_mask_e_type )SD_BIT_DELETE(
                                        ss_ptr->gpp2.cdma_act_mask, cmd->cdma_act_mask );
  ...
  if( ss_ptr->gpp2.cdma_act_mask == SD_SS_CDMA_ACT_MASK_NONE )
  {
  ...
  */
  ...
  SD_MSG_HIGH_3( "Stop activity 0x%x, mask 0x%x, ss_state %d",
                 cmd->cdma_act_mask, ss_ptr->gpp2.cdma_act_mask, ss_state );
...
)
{
  ...
  **    preference.
  */
  if( ss_ptr->gpp2.is_start_instance &&
      sdss_get_opr_mode(SD_SS_MAIN) == SDSS_OPR_MODE_ONLINE )
  {
    ...
    ss_ptr->gpp2.is_start_instance = FALSE;
  }
...
)
{
  ...
  */
  ...
  SD_ASSERT_ENUM_IS_INRANGE( cmd->band, SD_BAND_MAX );
  SD_ASSERT( sdss_is_supp_sys(SD_MODE_HDR, sd_map_band_to_band_pref(cmd->band), cmd->chan) );
...
)
{
  ...
  */
  ...
  ss_ptr->gpp2.hdr_act_mask = (sd_ss_hdr_act_mask_e_type)SD_BIT_ADD(
                                ss_ptr->gpp2.hdr_act_mask, cmd->hdr_act_mask );

  SD_MSG_HIGH_2("Start activity 0x%x mask 0x%x",
              cmd->hdr_act_mask, ss_ptr->gpp2.hdr_act_mask);
...
)
{
  ...
  */
  ss_ptr->gpp2.hdr_act_mask = (sd_ss_hdr_act_mask_e_type)SD_BIT_DELETE(
                                        ss_ptr->gpp2.hdr_act_mask, cmd->hdr_act_mask );
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  #ifdef FEATURE_HDR_HYBRID
  if( ss_ptr->gpp2.hdr_act_mask == SD_SS_HDR_ACT_MASK_NONE )
  {
    /* Set hdr activity flag to true */
  ...
  */
  ...
  SD_MSG_HIGH_3("Stop activity 0x%x, mask 0x%x, ss_state %d",
              cmd->hdr_act_mask, ss_ptr->gpp2.hdr_act_mask, ss_state);
...
)
{
  ...
  */
  if(  sd_misc_is_hybr_opr()   &&
      ( SD_IS_BIT_CONTAIN( ss_ptr->gpp2.hdr_act_mask, SD_SS_HDR_ACT_MASK_BCMCS )
                            ||
        SD_IS_BIT_CONTAIN( ss_ptr->gpp2.hdr_act_mask, SD_SS_HDR_ACT_MASK_TRAFFIC ))
                            &&
      cmd->resel_is_allowed )
  {
    SD_ERR_2( "No Resel in HDR act!!! resel %d, act 0x%x",
                  cmd->resel_is_allowed, ss_ptr->gpp2.hdr_act_mask);
...
)
{
  ...
  */
  ...
  #elif defined FEATURE_LIMITED_SCAN_OPT
  ...
  */
  if( gwl_scan_status == SD_SS_GWL_SCAN_STATUS_COMPLETE_PLMN_FOUND )
  {
    ...
    sd_si_update( ss,
                  ...
                  #endif
                  FALSE,
                  si_info_ptr->ciot_lte_mode,
                  si_info_ptr->neg_ciot_cap,
                  si_info_ptr->epco_supported,
                  si_info_ptr->cp_sms_service_status
                );
  }
  #endif
  if(cmd->req_id > 0)
...
#if defined FEATURE_WRLF_SYSTEM_SEL && defined FEATURE_WCDMA && !defined FEATURE_MMODE_DISABLE_CSG
...
)
{
  ...
  */
  #if defined (FEATURE_3GPP_CSFB) && defined(FEATURE_MMODE_9X05_EMERG_CALL)
  if ((sdss_get_orig_mode(ss) == SD_SS_ORIG_MODE_PPCSFB_EMERG_ORIG) && 
      sdss_get_is_full_srv_req_during_emerg(ss))
  {
  ...
  act = sd_ss_event_proc( SD_SS_MAIN, SDSS_EVT_GWL_OPR_SYS_LOST, cmd->rtrn_act_ptr );
...
)
{
  ...
  SD_MSG_HIGH_3("LTE srv RLF ss=%d sim_state=%d data_suspend=%d",
              ss, cmd->sim_state, cmd->data_suspend);
  ...
  ** acq failure if necessary */
  ...
  if( ss_ptr->ss_state != SDSS_STATE_OPR)
  {
    ...
    SD_MSG_HIGH_2("LTE Srv Lost in state -%d, pref_reason=%d", ss_ptr->ss_state,sdss_get_pref_reason(cmd->ss));
  ...
  */
  #ifdef FEATURE_MMODE_GW
  si_info_ptr->mode_info.gwl.lac             = SDSS_INVALID_TAC_INFO;
  #endif
  si_info_ptr->mode_info.gwl.rac_or_mme_code = SDSS_INVALID_RAC_OR_MME_CODE_INFO;
...
)
{
  sd_ss_act_e_type         act = SD_SS_ACT_CONTINUE;
  sd_ss_e_type             ss = cmd->ss;
  sd_band_e_type           band = SD_BAND_MAX;
  ...
  */
  ...
  SD_MSG_HIGH_2("ims_voice=%d, ims_emerg=%d", cmd->lte_ims_voice_avail, cmd->lte_ims_emerg_avail);
  #endif

  #if defined(FEATURE_CIOT)
  SD_MSG_HIGH_6("ciot_lte_mode %d cp_ciot_supported %d up_ciot_supported %d esr_without_pdn %d hc_cp_ciot %d s1_u_supp %d",
                 cmd->ciot_lte_mode, cmd->neg_ciot_cap.cp_ciot_supported, cmd->neg_ciot_cap.up_ciot_supported,
                 cmd->neg_ciot_cap.esr_without_pdn_supported, cmd->neg_ciot_cap.hc_cp_ciot_supported,
                 cmd->neg_ciot_cap.s1_u_data_supported);

  SD_MSG_HIGH_1("SMS over CP status %d",cmd->cp_sms_service_status);
  #endif
  ...
  */
  ...
  /* Update LTE CS CAPABILITY in SDSS */
   sdss_update_nas_addl_info(ss, cmd->extend_srv_info);
  ...
  */
  ...
  #if defined(FEATURE_CIOT)
  /* In CIOT flavors, map limited service on NB1 to srv lost */
  /* if in CIOT full_RAT, mapping to FAIL is not required as it may 
     * trigger service_req which anyway will happen with full RAT exit pref */
  if ((cmd->service_status == SYS_SRV_STATUS_LIMITED) &&
      ((cmd->mode == SYS_SYS_MODE_LTE) &&
        (cmd->ciot_lte_mode == SYS_SYS_MODE_LTE_NB1)
      ) &&
       !sdss_is_in_full_rat_mode_ciot(ss)
     )
  {
    ...
    SD_MSG_HIGH_1("Ltd servcie in NB1 mapped to sys_lost, ss_state=%d",
                           sd_ss_ptr(ss)->ss_state);
    srv_lost_cmd.ss                   = ss;
    srv_lost_cmd.rtrn_act_ptr         = cmd->rtrn_act_ptr;
    srv_lost_cmd.sim_state            = cmd->sim_state;
  ...
  */
  #ifdef FEATURE_MMODE_DUAL_SIM
  if(sdss_is_featuremode(SYS_OVERALL_FEATURE_MODE_MULTISIM) &&
  	 (ss == SD_SS_HYBR_3 || !sdss_is_1x_sxlte() ))
  {
    colloc_lst = 
      #ifdef FEATURE_MMODE_HYBR3_ENABLED
...
)
{
  ...
  act = sd_ss_event_proc( SD_SS_MAIN, SDSS_EVT_GWL_OPR_ACC_DONE, cmd->rtrn_act_ptr );
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
)
{
  ...
  sd_band_e_type      band = SYS_BAND_CLASS_NONE;
  sd_ss_e_type        ss = cmd->ss;
  sd_ss_band_pref_e_type band_srched = SD_SS_BAND_PREF_NONE;
  ...
  */
  ...
  sd_ss_act_s_type    *ss_act_ptr = NULL;
  #if (defined ( FEATURE_WRLF_SYSTEM_SEL) && defined(FEATURE_WCDMA)) || defined(FEATURE_MMODE_DUAL_SIM)
  ...
  SD_MSG_HIGH_6("SRV_3GPP: GWL acq success,mode=%d,ss=%d, status=%d, srv_domain=%d sim_state=%d acq_sys_ref %d",
              cmd->mode,
              ss,
              cmd->service_status,
              cmd->srv_domain, cmd->sim_state, acq_sys_ref
              );
  SD_MSG_HIGH_4("srv info %d %d %d, ext_srv_info %d",
             cmd->is_plmn_forbidden,
             cmd->roaming_ind,
             cmd->data_suspend, cmd->extend_srv_info);
  ...
  #if defined(FEATURE_LTE)
  SD_MSG_HIGH_2("ims_avail=%d,emerg_avail=%d",cmd->lte_ims_voice_avail, cmd->lte_ims_emerg_avail);
  #endif
...
/* We should reset the request ID from last  sd action. 
  ...
  */
  if (sdss_get_is_full_srv_req_during_emerg(ss))
  {
    SD_MSG_HIGH_0("sdcmd_gw_acq_plmn_success_ss_2 - reset full srv req falg due to full srv");
    sdss_check_reset_emerg_rat_pri_list_info(ss);
    sdss_update_is_full_srv_req_during_emerg(ss,FALSE);
  }
  #endif
  ...
  */
  ...
  #if defined ( FEATURE_WRLF_SYSTEM_SEL) && defined(SD_WCDMA_SUPPORTED)
  ...
  if( sd_get_ss_state(ss) == SDSS_STATE_OPR )
  {
    ...
    #endif
    #ifdef FEATURE_CIOT
    infocmd.ciot_lte_mode = cmd->ciot_lte_mode;
    infocmd.cp_sms_service_status = cmd->cp_sms_service_status;
    infocmd.neg_ciot_cap.cp_ciot_supported = cmd->neg_ciot_cap.cp_ciot_supported;
    infocmd.neg_ciot_cap.esr_without_pdn_supported = cmd->neg_ciot_cap.esr_without_pdn_supported;
    infocmd.neg_ciot_cap.hc_cp_ciot_supported = cmd->neg_ciot_cap.hc_cp_ciot_supported;
    infocmd.neg_ciot_cap.s1_u_data_supported = cmd->neg_ciot_cap.s1_u_data_supported;
    infocmd.neg_ciot_cap.up_ciot_supported = cmd->neg_ciot_cap.up_ciot_supported;
    #endif
  ...
  */
  {
    ...
    ** map acq success to acq fail.
    */
    /* In CIOT full RAT mode LTE acq is issued even in PS_INVALID cases */
    ...
    { 
      SD_MSG_HIGH_0("Sim state change CS/PS INVALID to AVAILABLE,map to ACQ_FAIL");	   	
    ...
    */
    ...
    if((cmd->mode == SD_MODE_LTE) &&
       (cmd->service_status == SYS_SRV_STATUS_LIMITED) &&
       (cmd->ciot_lte_mode == SD_MODE_LTE_NB1) &&
        !sdss_is_in_full_rat_mode_ciot(ss)
      )
    {
      is_map_to_acq_fail = TRUE;
      SD_MSG_HIGH_0("MAp to acq fail due to lack of full srv in ciot mode");
    }
    #endif
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
/* In global mode if NAS reports service on a RPLMN for a csg service request treat that as acq failure
  ...
  ** So SD core has to treat the limited service as NO service itself.*/
  ...
  #if ((defined (FEATURE_WRLF_SYSTEM_SEL) && defined (SD_WCDMA_SUPPORTED)) || defined FEATURE_MMODE_DUAL_SIM)
...
)
{
  ...
  */
  ...
  #if defined(FEATURE_CIOT)
  SD_MSG_HIGH_1("ciot_lte_mode %d", cmd->ciot_lte_mode);
  #endif
  SD_ASSERT( cmd->mode == SD_MODE_GSM || cmd->mode == SD_MODE_WCDMA ||
     cmd->mode == SD_MODE_LTE || cmd->mode == SD_MODE_TDS);
  ...
  */
  ...
  if(cmd->mode != SD_MODE_LTE)
  {
    cmd->ciot_lte_mode = SD_MODE_NONE;
  }
  #endif
...
)
{
  ...
  */
  ...
  if((sdss_is_lte_connected_mode (ss)))
  {
    ...
    sdss_reset_lte_connected_mode(ss);
    SD_MSG_HIGH_1("UE ending LTE connected mode, scanscope %d", sdss_get_scan_scope_rule(ss));
    ...
    */
    ...
    if (!sdss_is_cap_3gpp_only(SD_SS_MAIN) && !( sdss_is_featuremode(SYS_OVERALL_FEATURE_MODE_MULTISIM) &&
                                                 sdss_is_sfmode_svlte( (uint8)SD_SS_MAIN) &&
...
)
{
  ...
  */
  ...
  if(curr_act_ptr->prm.get_net_gw.network_list_type == SD_NETWORK_LIST_GPS_ADV_SEARCH )
  {
     SD_MSG_HIGH_2("GET NET RPT plmn list status %d list_type %d",
                       cmd->plmn_list_status,
                       curr_act_ptr->prm.get_net_gw.network_list_type);
  }
  else
#endif
  if(curr_act_ptr->prm.get_net_gw.network_list_type == SD_NETWORK_LIST_CELL_SEARCH)
  {
    SD_MSG_HIGH_4("GET NET RPT plmn list status %d list_type %d, CELL LEN lte %d, geran %d ",
                       cmd->plmn_list_status,
                       curr_act_ptr->prm.get_net_gw.network_list_type,
                       cmd->plmn_list_ptr->cell_info_list.lte_cell_list.num_cell,
                       cmd->plmn_list_ptr->cell_info_list.geran_cell_list.num_cell);
  }
  else
  {
    SD_MSG_HIGH_3("GET NET RPT plmn list status %d list_type %d plmn list length %d",
                       cmd->plmn_list_status,
                       curr_act_ptr->prm.get_net_gw.network_list_type,
  ...
  if( curr_act_ptr->prm.get_net_gw.network_list_type == SD_NETWORK_LIST_CELL_SEARCH ||
      curr_act_ptr->prm.get_net_gw.network_list_type == SD_NETWORK_LIST_GPS_ADV_SEARCH ||
      ...
      cmd->plmn_list_status == SYS_PLMN_LIST_REJ_IN_RLF)
  {
    ...
    { 
        ...
        )
      {
        ...
        sdss_set_mode_band_pref(ss, user_mode_pref, user_band_pref, lte_band_pref, SD_SS_BAND_PREF_NONE);
...
)
{
  ...
  */
  ...
  #endif
  #if defined(FEATURE_SD_LTE) && defined(SD_DEBUG) && !defined(FEATURE_MMODE_LTE_ONLY)
  #error code not present
#endif
  #ifdef FEATURE_MMODE_HYBR3_ENABLED
  ...
  */
  ...
  SD_MSG_HIGH_2("sdcmd_nam_sel3()-msim:%d,issxlte:%d",
                sdss_is_featuremode(SYS_OVERALL_FEATURE_MODE_MULTISIM),
                sdss_is_1x_sxlte());
  #endif
  ...
  */
     ...
     sdss_get_sim_state(SD_SS_MAIN) == SYS_SIM_STATE_NOT_AVAILABLE )
  {
    ...
    */
    if(!sdss_is_gprs_anite_gcf(SD_SS_MAIN) && !sdss_is_scan_scope_rule_avail(SD_SS_MAIN))
    {
      /*NV not configured*/
      if( (sdss_get_operator_name ( SYS_MODEM_AS_ID_1 ) != SD_OPERATOR_CMCC)&& 
          sdss_is_usim_compliant_for_cmcc(SYS_MODEM_AS_ID_1))
      {
        scan_scope_rule_val = sdss_get_scan_scope_rule(SD_SS_MAIN);
        ...
        sdss_set_scan_scope_rule(scan_scope_rule_val, SD_SS_MAIN);
      }
    }
  }
  ...
  */
     ...
     sdss_get_sim_state(SD_SS_HYBR_2) == SYS_SIM_STATE_NOT_AVAILABLE )
  {
    ...
    /* Read scan_scope_rule from EFS
    */
    if(!sdss_is_gprs_anite_gcf(SD_SS_HYBR_2) && !sdss_is_scan_scope_rule_avail(SD_SS_HYBR_2))
    {
      /*NV not configured*/
      ...
      {
        scan_scope_rule_val = sdss_get_scan_scope_rule(SD_SS_HYBR_2);
        scan_scope_rule_val = scan_scope_rule_val |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_RLF |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_PWR_UP |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_OOS;
        sdss_set_scan_scope_rule(scan_scope_rule_val, SD_SS_HYBR_2);
      }
    }
  }
  #endif
     ...
     sdss_get_sim_state(SD_SS_HYBR_3) == SYS_SIM_STATE_NOT_AVAILABLE )
  {
    ...
    /* Read scan_scope_rule from EFS
    */
    if(!sdss_is_gprs_anite_gcf(SD_SS_HYBR_3) && !sdss_is_scan_scope_rule_avail(SD_SS_HYBR_3))
    {
      /*NV not configured*/
      ...
      {
        scan_scope_rule_val = sdss_get_scan_scope_rule(SD_SS_HYBR_3);
        scan_scope_rule_val = scan_scope_rule_val |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_RLF |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_PWR_UP |
                              SYS_SCAN_SCOPE_RULE_ACQ_DB_OOS;
        sdss_set_scan_scope_rule(scan_scope_rule_val, SD_SS_HYBR_3);
      }
    }
  }
  #endif
  ...
  */
  #ifdef FEATURE_MMODE_DISABLE_PRL
  sdprl_nam_sel_lte_only();
  #else
  ...
  */
  ...
  #endif
  {
    ...
    SD_MSG_HIGH_6("MAIN_SD_NAM_SEL: mode_pref=0x%x, orig_mode=%d band_pref:(MSB)(LSB) cgw=0x%08x 0x%08x domain_pref=%d, user_mode_pref=0x%x",
                  cmd->mode_pref, cmd->orig_mode,
                  QWORD_HIGH(cmd->band_pref),
                  QWORD_LOW(cmd->band_pref), cmd->domain_pref, cmd->mode_pref);
   ...
   #ifdef FEATURE_HDR
   sd_ss_ptr(SD_SS_MAIN)->gpp2.is_start_instance = TRUE;
  ...
  */
     ...
     )
  {
      ...
      /* Fetch the user system record.
      */
      user_sys_ptr   = sdsr_list_get_ptr( SDSR_MANUAL_LST, 0 );
      if(user_sys_ptr != NULL)
      {
  ...
  */
  ...
  /* Reset action */
  sd_ss_ptr(SD_SS_MAIN)->act = SD_SS_ACT_MAX;
...
===========================================================================*/
boolean                     sdcmd_hybr_per_ss_nam_sel_1(
      sd_nam_sel_cmd *cmd
)
{
  ...
  */
    ...
    #endif
  {
   ...
   #ifdef FEATURE_HDR
   sd_ss_ptr(ss)->gpp2.is_start_instance = TRUE;
   #endif
  }
...
)
{
  ...
  */
  ...
  act = sd_ss_event_proc(SD_SS_MAIN, SDSS_EVT_USER_TERM_GET_NET_GW, cmd->rtrn_act_ptr );
...
/*===========================================================================
FUNCTION sd_get_band_mode_lte

DESCRIPTION
  get supported lte band pref.

  MODE
DEPENDENCIES

RETURN VALUE
  lte band pref and mode pref.

SIDE EFFECTS
  None.

===========================================================================*/
void                      sd_get_band_mode_lte(
...
{
  sys_lte_band_mask_e_type lte_band_pref_none = SYS_LTE_BAND_MASK_CONST_NONE;
  *mode_ptr = SD_GET_COMMON_MODE(*mode_ptr, sys_mode_pref );
  *band_ptr_lte = SYS_LTE_BAND_MASK_COMMON (band_ptr_lte, &sys_band_pref_lte);

  *band_ptr_lte = sdss_get_supp_mode_band_pref_lte(*mode_ptr, *band_ptr_lte);
  if (SYS_LTE_BAND_MASK_CHK_IF_EQUAL(band_ptr_lte, &lte_band_pref_none))
  {
    *mode_ptr = SD_SS_MODE_PREF_NONE;
  }  
} /* sd_get_band_mode_mmode */

#if defined(FEATURE_CIOT)
/*===========================================================================
FUNCTION sdss_is_in_full_rat_mode_ciot

DESCRIPTION
  Returns TRUE if orig_mode EMERG
  and not a call orig and if GW subsc available
...
===========================================================================*/
boolean sdss_is_in_full_rat_mode_ciot(
    sd_ss_e_type              ss
)
{
  ...
  if( sdss_get_orig_mode(ss)  == SD_SS_ORIG_MODE_EMERG_ORIG &&
      !sdss_is_pref_reason( ss, SDSSSCR_PREF_REAS_VAL_CALL_ORIG) &&
      sdss_get_subsc_type(ss) == SDSS_SUBSC_GW )
  {
    return TRUE;
  }
