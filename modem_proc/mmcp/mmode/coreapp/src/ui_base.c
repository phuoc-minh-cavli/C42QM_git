...
$Header: //components/rel/mmcp.mpss/6.1.10/mmode/coreapp/src/ui_base.c#1 $
...
void ui_init( void )
{
  ...
  ** and start our first tick timer */
  ...
  for(loop=0;loop<=CM_CALL_EVENT_EXIT_TC;loop++)
  {
    bit_mask_256_set_bit(&event_mask,loop);
  }
    ...
    case CM_CALL_EVENT_SETUP_IND:
      {
        ...
        if( bThinUI_setup_resp_req )
        {
          ...
          cm_util_set_default_voice_bearer_cap( &setup_params.bearer_capability_1 );
