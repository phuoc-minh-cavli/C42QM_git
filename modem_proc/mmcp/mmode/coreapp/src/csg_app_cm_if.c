...
void csg_app_cm_trigger_search(boolean is_app_triggered, boolean send_config_update, boolean is_immediate_search)
{
          ...
          ) )
  {
    ...
    {
      TUI_MSG_HIGH_6("Trigger interval %d in minutes, mode = %d, list type %d, network_type = %d, band_pref %ld, select_config = %d",
      ...
      TUI_MSG_HIGH_6("lte_band_pref %ld, mode_pref = %d, nw_list_type = %d, Roaming status = %d, srv_status = %d, timer = %d",
                     ui_base_csg_config_global.csg_search_select_config.lte_band_pref,
                     mode_pref, network_list_type,
                     csg_app_ss_evt_global.ss_info[asubs_id].stack_info[0].roam_status,
                     csg_app_ss_evt_global.ss_info[asubs_id].stack_info[0].srv_status,
                     ui_base_csg_config_global.timer.interval);
...
void csg_app_cm_if_ss_evt_cb (  cm_ss_event_e_type ss_event,  const cm_mm_msim_ss_info_s_type *ss_info_ptr)
{
  ...
  if(ss_info_ptr->number_of_stacks <=2 && ss_info_ptr->asubs_id < SYS_MODEM_AS_ID_3 )
  {
    ...
    TUI_MSG_HIGH_6("TUI ss_event %d, sys_mode=%d, srv_status=%d, change_fields=%ld, change_fields2=%ld, csg_id=%d",
                   ss_event, ss_info_ptr->stack_info[0].sys_mode,
                   ss_info_ptr->stack_info[0].srv_status,
                   ss_info_ptr->stack_info[0].changed_fields,
                   ss_info_ptr->stack_info[0].changed_fields2, csg_id);
