...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cm.c#1 $
...
#ifdef FEATURE_REF_TRACKER
static cm_sns_cmd_s_type* cm_cmd_alloc_sns_init(void)
{
   cm_sns_cmd_s_type      *cm_sns_cmd_ptr = (cm_sns_cmd_s_type*)cm_cmd_alloc(sizeof(cm_sns_cmd_s_type));
    cm_sns_cmd_ptr->client_id                 = CM_CLIENT_ID_ANONYMOUS;
    cm_sns_cmd_ptr->client_ptr                = CM_CLIENT_PTR_NONE;
    cm_sns_cmd_ptr->cmd                       = CM_SNS_CMD_NONE;
    cm_sns_cmd_ptr->cmd_type                  = CM_CMD_TYPE_SNS;

	return cm_sns_cmd_ptr;
...
/*===========================================================================
FUNCTION cm_cmd_alloc_mbms_init

DESCRIPTION
  Allocate a MBMS command buffer and initialize its MBMS command fields
...
** warning
*/
static cm_mbms_cmd_s_type *cm_cmd_alloc_mbms_init( void )
{
  cm_mbms_cmd_s_type            *mbms_cmd_ptr       = (cm_mbms_cmd_s_type*)cm_cmd_alloc(sizeof(cm_mbms_cmd_s_type));

  cm_mbms_cmd_info_s_type  *cmd_info_ptr;
  ...
  */
    /*
    ** 1. Initialize MBMS command fields
    */
    mbms_cmd_ptr->client_id                 = CM_CLIENT_ID_ANONYMOUS;
    mbms_cmd_ptr->client_ptr                = CM_CLIENT_PTR_NONE;
    mbms_cmd_ptr->cmd                       = CM_MBMS_CMD_NONE;
  mbms_cmd_ptr->cmd_type                  = CM_CMD_TYPE_MBMS;

    /*
    ** 2. Initialize MBMS command information fields.
    */
    cmd_info_ptr               = CMD_INFO_PTR( mbms_cmd_ptr );
    cmd_info_ptr->cmd_type     =  SYS_GW_MBMS_REQ_NONE;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  return mbms_cmd_ptr;
} /* cm_cmd_alloc_mbms_init() */
...
static cm_call_cmd_s_type *cm_cmd_alloc_call_init( void )
{
  ...
  */
    ...
    */
    ...
    #ifdef FEATURE_CM_LTE
    cmd_info_ptr->lte_info.pdn_data.mo_exception_data = FALSE;
    #endif
    ...
    */
    ...
    cmd_info_gw_ps_ptr->low_priority_signaling = FALSE;
...
void cm_cmd_ph_init( cm_ph_cmd_s_type * ph_cmd_ptr)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    ...
    #ifdef FEATURE_CIOT
    cmd_info_ptr->ciot_lte_pref.lte_op_mode_pref  = CM_LTE_IOT_OP_MODE_PREF_NONE;
    SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&cmd_info_ptr->ciot_lte_pref.lte_m1_band_pref);
    SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&cmd_info_ptr->ciot_lte_pref.lte_nb1_band_pref);
    SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&cmd_info_ptr->ciot_lte_pref.lte_band_pref);
    cmd_info_ptr->user_mcc                        = 0xFFF;
    #endif
    cmd_info_ptr->cell_search_num_cells = 0;
...
/*lint -esym(715, asubs_id) */
{
  ...
  #endif
  #ifdef FEATURE_MMODE_ENABLE_PS
  cm_call_cmd_info_gw_ps_s_type  *ps_cmd_mode_info_ptr;
  #endif
  #endif /* FEATURE_MMODE_GW */
    ...
    */
    ...
    if( gw_ps_orig_params_ptr != NULL )
    {
      ...
      */
      ...
      ps_cmd_mode_info_ptr->low_priority_signaling  = gw_ps_orig_params_ptr->low_priority_signaling;
  ...
  */
  ...
  SYS_ARG_NOT_USED( srv_type );
  SYS_ARG_NOT_USED( calling_number );
  SYS_ARG_NOT_USED( called_number );
  SYS_ARG_NOT_USED( alpha_buf );
  SYS_ARG_NOT_USED( cdma_orig_params_ptr );
  SYS_ARG_NOT_USED( gw_cs_orig_params_ptr );
  SYS_ARG_NOT_USED( gw_ps_orig_params_ptr );
  SYS_ARG_NOT_USED( ip_orig_params_ptr );
  SYS_ARG_NOT_USED( switch_out_call_id );
  ...
  SYS_ARG_NOT_USED( return_call_id_ptr);
  SYS_ARG_NOT_USED( switch_out_call_id );
  
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_ORIG,
...
)
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
    ...
    case CM_CALL_MODE_INFO_GW_PS:
      ...
      ps_cmd_mode_info_ptr->low_priority_signaling   =
               ans_params_ptr->ans_params.gw_ps_ans.low_priority_signaling;
  ...
  cm_cmd_queue( (cm_cmd_type*)call_cmd_ptr );
    ...
    SYS_ARG_NOT_USED( data_block_ptr );
    SYS_ARG_NOT_USED( ans_params_ptr );
...
/*lint -esym(715,client_id) */
{
  ...
  SYS_ARG_NOT_USED( call_id );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( setup_params_ptr );
...
)
{
  ...
  */
    ...
    /* Copy extended lte protocol configuration options */
    cmd_info_ptr->lte_info.ext_protocol_conf_opt.valid =
        act_bearer_rsp_ptr->ext_protocol_conf_opt.valid;

    cmltecall_copy_ext_protocol_config_options( &(cmd_info_ptr->lte_info.ext_protocol_conf_opt),
      &(act_bearer_rsp_ptr->ext_protocol_conf_opt) );


    CM_MSG_MED_3("cm_mm_call_cmd_act_bearer_rsp(), epco_valid = %d len =%d dsm_ptr =%p",
                 act_bearer_rsp_ptr->ext_protocol_conf_opt.valid,
                 act_bearer_rsp_ptr->ext_protocol_conf_opt.len,
                 act_bearer_rsp_ptr->ext_protocol_conf_opt.dsm_ptr);
...
)
{
  ...
  if ( restore_req_ptr == NULL )
  {
    if( cmd_cb_func != NULL )
    {
      CM_ERR_0("ERROR: NULL act_bearer_rsp_ptr");
      cmd_cb_func( data_block_ptr, CM_CALL_CMD_RESTORE_BEARER_CTXT_REQ,
                   CM_CALL_CMD_ERR_NULL_PARAM_P);
    }
  ...
  */
    ...
    call_cmd_ptr->data_block_ptr  = data_block_ptr;
    call_cmd_ptr->client_id       = client_id;
    call_cmd_ptr->cmd             = CM_CALL_CMD_RESTORE_BEARER_CTXT_REQ;
    ...
    if (sys_mode != SYS_SYS_MODE_LTE)
    {
      ...
      call_cmd_ptr->cmd_subs            = restore_req_ptr->gw_restore_bearer_req_params.asubs_id;

      cmd_mode_info_ptr->nsapi          = restore_req_ptr->gw_restore_bearer_req_params.nsapi;
      cmd_mode_info_ptr->pdp_group_id   = restore_req_ptr->gw_restore_bearer_req_params.sm_group_id;
      cmd_mode_info_ptr->pdp_addr       = restore_req_ptr->gw_restore_bearer_req_params.pdp_addr;
      cmd_mode_info_ptr->apn_name       = restore_req_ptr->gw_restore_bearer_req_params.apn_name;
      cmd_mode_info_ptr->qos            = restore_req_ptr->gw_restore_bearer_req_params.neg_qos;
      cmd_mode_info_ptr->minimum_qos    = restore_req_ptr->gw_restore_bearer_req_params.minimum_qos;
      cmd_mode_info_ptr->h_comp         = restore_req_ptr->gw_restore_bearer_req_params.h_comp;
      cmd_mode_info_ptr->d_comp         = restore_req_ptr->gw_restore_bearer_req_params.d_comp;
      cmd_mode_info_ptr->bcm            = restore_req_ptr->gw_restore_bearer_req_params.bcm;
      cmd_mode_info_ptr->tft            = restore_req_ptr->gw_restore_bearer_req_params.tft;
      cmd_mode_info_ptr->primary_pdp    = restore_req_ptr->gw_restore_bearer_req_params.is_primary_pdp;
    }
    else
    {
      #ifdef FEATURE_CM_LTE
      ...
      call_cmd_ptr->cmd_subs                = restore_req_ptr->lte_restore_bearer_req_params.asubs_id;
	  
      cmd_info_ptr->lte_info.as_id          = restore_req_ptr->lte_restore_bearer_req_params.asubs_id;
      cmd_info_ptr->lte_info.eps_bearer_id  = restore_req_ptr->lte_restore_bearer_req_params.eps_bearer_id;
      cmd_info_ptr->lte_info.sdf_qos        = restore_req_ptr->lte_restore_bearer_req_params.sdf_qos;
      cmd_info_ptr->lte_info.sdf_id         = restore_req_ptr->lte_restore_bearer_req_params.sdf_id;
      cmd_info_ptr->lte_info.gsm_umts_connection_id  = restore_req_ptr->lte_restore_bearer_req_params.ti;
      cmd_info_ptr->lte_info.pdp_group_id   = restore_req_ptr->lte_restore_bearer_req_params.sm_group_id;
      cmd_info_ptr->lte_info.pdn_addr       = restore_req_ptr->lte_restore_bearer_req_params.pdn_address;
      cmd_info_ptr->lte_info.apn_name       = restore_req_ptr->lte_restore_bearer_req_params.apn_name;
      cmd_info_ptr->lte_info.neg_qos        = restore_req_ptr->lte_restore_bearer_req_params.neg_qos;
      cmd_info_ptr->lte_info.neg_llc_sapi   = restore_req_ptr->lte_restore_bearer_req_params.neg_llc_sapi;
      cmd_info_ptr->lte_info.apn_ambr       = restore_req_ptr->lte_restore_bearer_req_params.apn_ambr;
      cmd_info_ptr->lte_info.lbi            = restore_req_ptr->lte_restore_bearer_req_params.lbi;
      cmd_info_ptr->lte_info.h_comp         = restore_req_ptr->lte_restore_bearer_req_params.h_comp;
      cmd_info_ptr->lte_info.d_comp         = restore_req_ptr->lte_restore_bearer_req_params.d_comp;
      cmd_info_ptr->lte_info.bcm            = restore_req_ptr->lte_restore_bearer_req_params.bcm;
  
      if(restore_req_ptr->lte_restore_bearer_req_params.default_bearer_ctxt)
      {
        cmd_info_ptr->lte_info.bearer_type = CM_LTE_BEARER_TYPE_DEFAULT;
      }
      else
      {
        cmd_info_ptr->lte_info.bearer_type = CM_LTE_BEARER_TYPE_DEDICATED;
      }
      #endif
    }
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  SYS_ARG_NOT_USED( restore_req_ptr );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( client_id );
...
} /* cm_mm_call_restore_bearer_ctxt_req() */
    ...
    )
{
  ...
  */
  ...
  call_cmd_ptr->data_block_ptr  = data_block_ptr;
  call_cmd_ptr->client_id       = client_id;
  call_cmd_ptr->cmd             = CM_CALL_CMD_RESTORE_CTXT_RSP;
  call_cmd_ptr->cmd_subs        = asubs_id;
    ...
    /* Copy command information fields. */
    cmd_info_ptr                           = CMD_INFO_PTR( call_cmd_ptr );
    cmd_info_ptr->psm_ctxt_restore_status  = status;
    ...
    else
    {
      ...
      /* System mode is LTE  */ 
      cmd_info_ptr->info_type           = CM_CALL_MODE_INFO_LTE;
      cmd_info_ptr->lte_info.as_id      = asubs_id;
      #endif
    }
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_RESTORE_CTXT_RSP,
      CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
...
)
{
  ...
  */
    ...
    cmd_info_ptr->lte_info.esm_dummy_flag = pdn_connectivity_ptr->esm_dummy_flag;
...
)
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
    ...
    /* Copy extended lte protocol configuration options */
    cmd_info_ptr->lte_info.ext_protocol_conf_opt.valid =
        pdn_disconnect_ptr->ext_protocol_conf_opt.valid;

    cmltecall_copy_ext_protocol_config_options( &(cmd_info_ptr->lte_info.ext_protocol_conf_opt),
      &(pdn_disconnect_ptr->ext_protocol_conf_opt) );
    ...
    CM_MSG_MED_3("cm_mm_call_cmd_pdn_disconnect_req(), epco_valid = %d len =%d dsm_ptr =%p",
                 pdn_disconnect_ptr->ext_protocol_conf_opt.valid,
                 pdn_disconnect_ptr->ext_protocol_conf_opt.len,
                 pdn_disconnect_ptr->ext_protocol_conf_opt.dsm_ptr);
...
)
{
  ...
  */
    ...
    /* Copy extended lte protocol configuration options */
    cmd_info_ptr->lte_info.ext_protocol_conf_opt.valid =
        res_alloc_ptr->ext_protocol_conf_opt.valid;

    cmltecall_copy_ext_protocol_config_options( &(cmd_info_ptr->lte_info.ext_protocol_conf_opt),
      &(res_alloc_ptr->ext_protocol_conf_opt) );
    ...
    CM_MSG_MED_3("cm_mm_call_cmd_res_alloc_req(), epco_valid = %d len =%d dsm_ptr =%p",
                 res_alloc_ptr->ext_protocol_conf_opt.valid,
                 res_alloc_ptr->ext_protocol_conf_opt.len,
                 res_alloc_ptr->ext_protocol_conf_opt.dsm_ptr);
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
)
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
   ...
   memscpy(&(cmd_info_ptr->lte_info.hc_config), sizeof(hc_config_T), 
            &(res_mod_ptr->hc_config), sizeof(hc_config_T));
    ...
    /* Copy extended lte protocol configuration options */
    cmd_info_ptr->lte_info.ext_protocol_conf_opt.valid =
        res_mod_ptr->ext_protocol_conf_opt.valid;

    cmltecall_copy_ext_protocol_config_options( &(cmd_info_ptr->lte_info.ext_protocol_conf_opt),
      &(res_mod_ptr->ext_protocol_conf_opt) );


    CM_MSG_MED_3("cm_mm_call_cmd_act_bearer_rsp(), epco_valid = %d len =%d dsm_ptr =%p",
                 res_mod_ptr->ext_protocol_conf_opt.valid,
                 res_mod_ptr->ext_protocol_conf_opt.len,
                 res_mod_ptr->ext_protocol_conf_opt.dsm_ptr);
...
)
{
  ...
  */
    ...
    /* Copy extended lte protocol configuration options */
    cmd_info_ptr->lte_info.ext_protocol_conf_opt.valid =
        mod_bearer_rsp_ptr->ext_protocol_conf_opt.valid;

    cmltecall_copy_ext_protocol_config_options( &(cmd_info_ptr->lte_info.ext_protocol_conf_opt),
      &(mod_bearer_rsp_ptr->ext_protocol_conf_opt) );


    CM_MSG_MED_3("cm_mm_call_cmd_mod_bearer_rsp(), epco_valid = %d len =%d dsm_ptr =%p",
                 mod_bearer_rsp_ptr->ext_protocol_conf_opt.valid,
                 mod_bearer_rsp_ptr->ext_protocol_conf_opt.len,
                 mod_bearer_rsp_ptr->ext_protocol_conf_opt.dsm_ptr);
...
/*lint -esym(715,reestab_params_ptr) */
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
    ...
    */
    ...
    else
    {
      ...
      cmd_info_ptr->lte_info.mo_exception_data = 
	      reestab_params_ptr->rab_reestab_params.cm_lte_rab_reestab_params.mo_exception_data;
      CM_MSG_HIGH_2( "sys_lte_reestab_calltype = %d, mo_exception_data = %d",
	  	reestab_params_ptr->sys_lte_reestab_calltype,
	  	reestab_params_ptr->rab_reestab_params.cm_lte_rab_reestab_params.mo_exception_data);
      #endif
  ...
  return FALSE;
  #endif /* else of (FEATURE_MMODE_3GPP) */
}
#ifdef FEATURE_NON_DEMAND_PAGED_FUNCTION
#error code not present
#endif
...
/*lint -esym(715,call_id) */
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
    ...
    */
    ...
    cmd_mode_info_ptr->low_priority_signaling = modify_params_ptr->low_priority_signaling;
...
===========================================================================*/
uint8  cm_get_lte_rpm_parameter_max_event_counter()
{  
  uint8             max_event_counter = 0;
  cmrpm_lte_s_type *lte_rpm_ptr       = cmrpm_lte_ptr();
   
  if( lte_rpm_ptr != NULL && lte_rpm_ptr->is_rpm_enabled )
  {
    max_event_counter = lte_rpm_ptr->max_event_counter;
  }   
  return max_event_counter;
}
 ...
 ===========================================================================*/
uint16   cm_get_lte_rpm_parameter_max_event_timer()
{  
  uint16            max_event_timer = 0; 
  cmrpm_lte_s_type *lte_rpm_ptr     = cmrpm_lte_ptr();
  
  if( lte_rpm_ptr != NULL && lte_rpm_ptr->is_rpm_enabled )
  {
    max_event_timer = lte_rpm_ptr->max_event_timer;
  }  
  return max_event_timer;
}
...
uint8  cm_get_rpm_parameter_N1()
 {  
    uint8 N1=0;
    
    cmrpm_s_type                  *rpm_ptr        = cmrpm_ptr();
     
    if( rpm_ptr != NULL && rpm_ptr->is_rpm_enabled )
    {
      
       N1 = rpm_ptr->app_reset.max_num_reset;
 ...
 uint16   cm_get_rpm_parameter_T1()
 {
     ...
     if( rpm_ptr != NULL && rpm_ptr->is_rpm_enabled )
    {
       ...
       T1 = rpm_ptr ->mmreject_reset.average_reset_time;
...
)
{
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_END,
      CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
...
/*lint -esym(818, end_params_ptr) */
{
  ...
  call_cmd_ptr = cm_cmd_alloc_call_init();
    ...
    */
    ...
    return TRUE;
  #else
  if((cmd_call_id_cb_func != NULL ) && (end_params_ptr != NULL))
  {
    int end_call_loop = 0;
    
    for (end_call_loop=0; end_call_loop < num_call_ids; ++end_call_loop) 
    {
      cmd_call_id_cb_func( data_block_ptr,
                           CM_CALL_CMD_END,
                           CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE,
                           end_params_ptr[end_call_loop].call_id );
    }
  }
  return FALSE;
  #endif
...
)
{
  ...
  */
  ...
  SYS_ARG_NOT_USED( client_id );
  SYS_ARG_NOT_USED( sups_params_ptr );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( sups_type );
  SYS_ARG_NOT_USED( asubs_id );
  
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_SUPS,
  	CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
...
/*lint -esym(715, call_type) */
{
  ...
  */
  ...
  SYS_ARG_NOT_USED( call_type );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( call_state );
  SYS_ARG_NOT_USED( ho_msg_ptr );
  SYS_ARG_NOT_USED( return_call_id_ptr );

  
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_CALL_CMD_UMTS_CDMA_HANDOVER_REQ,
  	CM_CALL_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
...
)
{
  ...
  {
    ULOG_RT_PRINTF_2(cm_get_ulog_handle(), "=CM= MPH CMD - cm_ph_cmd_oprt_mode(), mode = %d , client = %d",oprt_mode,client_id);
  }
...
)
{
  ...
  cm_cmd_queue( (cm_cmd_type*)ph_cmd_ptr );
   ...
   if( cmd_cb_func != NULL )
   {
     cmd_cb_func( data_block_ptr, CM_PH_CMD_AVOID_SYS,
       CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
   }
   return FALSE;
   #endif
...
)
{
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_PACKET_STATE,
      CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
...
{
  #if !defined(FEATURE_MMODE_LTE_ONLY)
  cm_ph_cmd_s_type          *ph_cmd_ptr;
  cm_ph_cmd_info_s_type     *cmd_info_ptr;
  boolean                    status;
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_PERSO_INFO_AVAILABLE,
      CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
...
)
{
 ...
 CM_MSG_HIGH_4( "cm_ph_cmd_subscription_available3, cdma=%d, gwl=%d gw2=%d, gw3=%d",
  ...
  if(cm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_4(cm_get_ulog_handle(), "=CM= cm_ph_cmd_subscription_available3, cdma=%d, gwl=%d gw2=%d, gw3=%d",cdma_status,gwl_status, gw2_status,gw3_status);
  }
...
)
{
  ...
  if(cm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_5(cm_get_ulog_handle(), "=CM= CM_PH_CMD_SUBSCRIPTION_NOT_AVAILABLE, cdma=%d, gwl=%d, gw2=%d, gw3=%d cause=%d",
        cdma_status,gwl_status,gw2_status,gw3_status,cause);
  }
...
/*lint -esym(715,client_id) */
{
  ...
  cm_ph_cmd_get_networks_per_subs_ciot(cmd_cb_func,
                                       data_block_ptr,
                                       client_id,
                                       list_type,
                                       network_type,
...
SIDE EFFECTS
  ...
  NETWORKS_CONF event.
===========================================================================*/
boolean cm_ph_cmd_get_networks_per_subs_ciot(
...
/*lint -esym(715,client_id) */
{
  ...
  return cm_ph_cmd_get_networks_per_subs_ciot_num_cells( cmd_cb_func,
                                                         data_block_ptr,
                                                         client_id,
                                                         list_type,
                                                         network_type,
...
/*lint -esym(715,client_id) */
{
  ...
  ph_cmd_ptr = cm_cmd_alloc_ph_init();
    ...
    */
    ...
    {
      cmd_info_ptr->cell_search_num_cells = cell_search_num_cells;
    }

    #if defined(FEATURE_CIOT)
    cmd_info_ptr->ciot_lte_pref_get_net.lte_op_mode_pref   = ciot_lte_pref.lte_op_mode_pref;
    cmd_info_ptr->ciot_lte_pref_get_net.lte_m1_band_pref   = ciot_lte_pref.lte_m1_band_pref;
    cmd_info_ptr->ciot_lte_pref_get_net.lte_nb1_band_pref  = ciot_lte_pref.lte_nb1_band_pref;
    cmd_info_ptr->ciot_lte_pref_get_net.lte_band_pref      = sys_map_band_mask_to_lte_band_mask(lte_band_pref_lmt);
    
    CM_MSG_HIGH_2("sys_sel_pref(),ciot pref's lte_op_mode %d cell_search_num_cells %d", 
        ciot_lte_pref.lte_op_mode_pref, cell_search_num_cells);
    cm_print_lte_band_mask(ciot_lte_pref.lte_m1_band_pref);
    cm_print_lte_band_mask(ciot_lte_pref.lte_nb1_band_pref);
    #endif
...
/*lint -esym(715, rtre_config) */
{
  #if ( defined(FEATURE_UIM_RUIM) && defined(FEATURE_UIM_RUN_TIME_ENABLE) ) && defined(FEATURE_MMODE_3GPP2)
  cm_ph_cmd_s_type        *ph_cmd_ptr;
  cm_ph_cmd_info_s_type   *cmd_info_ptr;
  ...
  ph_cmd_ptr = cm_cmd_alloc_ph_init();
    ...
    */
         ...
         BETWEEN(default_data_subs, SYS_MODEM_AS_ID_NONE,SYS_MODEM_AS_ID_MAX)))
    {
      ...
      CM_MSG_HIGH_6("Reject dual stdby, stdby pref %d active subs %d pri subs %d dflt voice %d dflt data %d dds type %d",
    ...
    */
    ...
    CM_MSG_HIGH_5("Current DS prefs: standby pref %d active subs %d priority subs %d dflt voice %d dflt data %d",
    ...
    CM_MSG_HIGH_6("New DS prefs: stdby pref %d active subs %d pri subs %d dflt voice %d dflt data %d dds_cause %d",
    ...
    if( cmd_cb_func != NULL )
    {
      cmd_cb_func( data_block_ptr, CM_PH_CMD_DUAL_STANDBY_PREF,
        CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
    }
    return FALSE;
   #endif
...
)
{
  ...
  */
  ...
  return TRUE;
   ...
   if( cmd_cb_func != NULL )
   {
     cmd_cb_func( data_block_ptr, CM_PH_CMD_ACTIVATE_SUBS,
       CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
   }
   return FALSE;
#endif
...
)
{
 ...
 /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  CM_MSG_HIGH_2("Current preferences: active subs %d, New preferences: deactivated subs %d",
                cmph_ptr()->active_subs, subs_mask);
...
#error code not present
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_UPDATE_CDMA_CAPABILITY,
      CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
...
DESCRIPTION
  Command CM to inform GNSS ( Glonass GPS ) Session Status ON/OFF Status
...
{
  cm_ss_cmd_s_type        *ss_cmd_ptr   = NULL;
  cm_ss_cmd_info_s_type   *cmd_info_ptr = NULL;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/


  CM_MSG_HIGH_2("CM_SS_CMD_SEND_GNSS_SESSION_STATUS, gnss_session_active=%d,client=%d", gnss_session_active,
                                                          client_id);
  ...
  */
    ...
    /* Get a reference to command information.
    */
    cmd_info_ptr = CMD_INFO_PTR( ss_cmd_ptr );
    ...
    */
    ...
    ss_cmd_ptr->data_block_ptr  = data_block_ptr;
    ss_cmd_ptr->client_id       = client_id;
    ss_cmd_ptr->cmd             = CM_SS_CMD_SEND_GNSS_SESSION_STATUS;

    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

    /* Copy command information fields.
    */
    cmd_info_ptr->gnss_session_active  = gnss_session_active;
...
DESCRIPTION
  Command CM to get LTE TX Activity reported last by LTE ML1.
...
SIDE_EFFECT
  If command is successful, CM clients will be notified of the
  corresponding CM event - CM_SS_EVENT_LTE_RRC_TX_INFO.
...
)
{
  ...
  CM_MSG_HIGH_1( "CM_SS_CMD_GET_LTE_RRC_TX_INFO, client=%d ", client_id );
  ...
  */
    ...
    */
    ...
    ss_cmd_ptr->data_block_ptr  = data_block_ptr;
    ss_cmd_ptr->client_id       = client_id;
    ss_cmd_ptr->cmd             = CM_SS_CMD_GET_LTE_RRC_TX_INFO;
...
)
{
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_SMS_CMD_MO_MSG, CM_SMS_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
}
...
/*lint -esym(715, bc_enable) */
{
    ...
    #else /* ( defined(FEATURE_MMODE_3GPP2) ) */
    if( cmd_cb_func != NULL )
    {
      cmd_cb_func( data_block_ptr, CM_SMS_CMD_MO_MSG,
        CM_SMS_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
...
)
{
  ...
  #if (defined FEATURE_MMODE_GW || (defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE))) 
  cm_sups_cmd_s_type         *sups_cmd_ptr;
...
/*lint -esym(715,subs_id)*/
{
  ...
  */
  ...
  CM_MSG_HIGH_3("cm_stats_set_modem_info_mask_per_subs(): asubs_id %d, module %d, bit_mask %d",
  ...
  */
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_STATS_CMD_SET_MODEM_INFO,
...
/*lint -esym(715, client_id)*/
{
  ...
  */
  ...
  memscpy(cmd_info_ptr->flow_req.flow_reg_setup_array,
          sizeof(cmd_info_ptr->flow_req.flow_reg_setup_array),
          flow_reg_setup_array_ptr,
...
/*lint -esym(715, client_id)*/
{
  ...
  */
  ...
  /* Check for valid number of flow ids */
  if( !INRANGE(num_of_flows, 1, cmutil_get_sys_bcmcs_max_flows() ))
  {
  ...
  */
  ...
  /*
  ** If any of above validations failed invoke the callback function and
  ** return failure.
  */
  ...
  */
  ...
  bcmcs_cmd_ptr->data_block_ptr  = data_block_ptr;
  bcmcs_cmd_ptr->client_id       = client_id;
  bcmcs_cmd_ptr->cmd             = CM_BCMCS_CMD_REG_REQUEST_F;
  ...
  cmd_info_ptr->reg_req.num_of_flows = num_of_flows;

  /*lint -save -e668 Possibly passing a null pointer to function
  **                 Fine as num_of_flows is 0 so we won't copy anything
  */
  memscpy(cmd_info_ptr->reg_req.flow_addr_array ,
          cmd_info_ptr->reg_req.num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type),
          flow_ids_array_ptr,
          num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type));
   /*lint -restore */
...
/*lint -esym(715, client_id)*/
{
  ...
  */
  ...
  bcmcs_cmd_ptr->data_block_ptr  = data_block_ptr;
  bcmcs_cmd_ptr->client_id       = client_id;
  bcmcs_cmd_ptr->cmd             = CM_BCMCS_CMD_REG_HANDOFF_F;
...
/*lint -esym(715, enable_strict_caching)*/
{
  ...
  CM_ASSERT_ENUM_IS_INRANGE(enable_strict_caching, SYS_BCMCS_BOM_CACHING_MAX);
  ...
  bcmcs_cmd_ptr->data_block_ptr  = data_block_ptr;
  bcmcs_cmd_ptr->client_id       = client_id;
  bcmcs_cmd_ptr->cmd             = CM_BCMCS_CMD_BOM_CACHING_SETUP_F;
  ...
  cmd_info_ptr->bom_cache_setup.enable_strict_caching =
                                                  enable_strict_caching;

  cmd_info_ptr->bom_cache_setup.stale_BOM_factor = stale_BOM_factor;
...
/*lint -esym(818, data_block_ptr)*/
{
  #if defined (FEATURE_MBMS)
  cm_bcmcs_cmd_s_type             *bcmcs_cmd_ptr;
  cm_bcmcs_cmd_info_u_type        *cmd_info_ptr;
  ...
  */
  bcmcs_cmd_ptr = cm_cmd_alloc_bcmcs_init();
    ...
    bcmcs_cmd_ptr->data_block_ptr  = data_block_ptr;
    bcmcs_cmd_ptr->client_id       = client_id;
    bcmcs_cmd_ptr->cmd             = CM_BCMCS_CMD_FLOW_DEACT_REQUEST_F;
...
{
  #if ((defined CM_GW_SUPPORTED && defined FEATURE_MULTIMEDIA_FALLBACK) || (defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE))) \
      && !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
  cm_call_cmd_s_type                *call_cmd_ptr;
  cm_call_cmd_info_s_type           *cmd_info_ptr;
  cm_call_cmd_info_gw_cs_s_type     *cmd_mode_info_ptr;
...
/*lint -esym(715,drx_coefficient)*/
{
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr          = data_block_ptr;
  ph_cmd_ptr->client_id               = client_id;
  ph_cmd_ptr->cmd                     = CM_PH_CMD_SET_LOCATION_INFO;
  ...
  */
  cmd_info_ptr                         = CMD_INFO_PTR( ph_cmd_ptr );
  cmd_info_ptr->cmd_err                = CM_PH_CMD_ERR_NOERR;
  cmd_info_ptr->cmd_subs               = loc_info->asubs_id;
  cmd_info_ptr->user_mcc               = loc_info->mcc;

  CM_MSG_MED_2("as_id=%d, user_mcc=%d",
             cmd_info_ptr->cmd_subs, cmd_info_ptr->user_mcc);
  ...
  */
  ...
  CM_MSG_HIGH_0("cm_ph_cmd_set_location_info_per_sub() is not supported");
  ...
  #endif
}/* cm_ph_cmd_set_user_mcc_per_sub */
...
*/
{
  ...
  CM_ASSERT(cmd_ptr != NULL);
        ...
        && (defined(FEATURE_MMODE_SS_SUPS)|| defined(FEATURE_MMODE_USS_SUPS))
  ...
  else
  {
    ...
    */
    ...
    #endif
    #if !defined(FEATURE_MMODE_LTE_ONLY)
    if (new_call_type == CM_CALL_TYPE_PS_DATA)
    {
    ...
    #if defined( FEATURE_GSTK ) && (defined FEATURE_MMODE_3GPP) && !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
 ...
 )
{
  ...
  if(!(sd_misc_is_mode_pref(cm_util_map_sys_sys_mode_to_sd_ss_mode_pref(sys_mode),sd_mode_pref)))
  {
    return FALSE;
  }

  return(sd_misc_is_supp_pref(cm_util_map_sys_sys_mode_to_sd_ss_mode_pref(sys_mode),
...

{
  ...
  if (mbms_generic_cmd == NULL)
  {
    CM_ERR_0( "mbms_generic_cmd pointer sent in cm_mbms_cmd_generic() in NULL");
    if (cmd_cb_func != NULL)
    {
      CM_ERR_0( "mbms_generic_cmd ptr is NULL");
      cmd_cb_func(data_block_ptr,
                  CM_MBMS_CMD_GENERIC,
  ...
  if (!BETWEEN(mbms_generic_cmd->cmd_type, SYS_GW_MBMS_REQ_NONE,SYS_GW_MBMS_REQ_MAX))
  {
    if (cmd_cb_func != NULL)
    {
      CM_ERR_1( "mbms_generic_cmd %d is out of range",
              mbms_generic_cmd->cmd_type);
  ...
  mbms_cmd_ptr = cm_cmd_alloc_mbms_init();

  /*-----------------------------------------------------------------------*/

  /* Copy command header fields. */
  mbms_cmd_ptr->cmd_cb_func     = cmd_cb_func;
  mbms_cmd_ptr->data_block_ptr  = data_block_ptr;
  mbms_cmd_ptr->client_id       = client_id;
  mbms_cmd_ptr->cmd             = CM_MBMS_CMD_GENERIC;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Fill in the command buffer and queue it */

  /* Copy command information fields. */
  cmd_info_ptr               = CMD_INFO_PTR( mbms_cmd_ptr );
  cmd_info_ptr->cmd_type     = mbms_generic_cmd->cmd_type ;
  *(&cmd_info_ptr->cmd) = *(&mbms_generic_cmd->cmd);
  /*lint -restore */
  ...
  /* Queue the command to CM.
  */
  cm_cmd_queue( (cm_cmd_type*)mbms_cmd_ptr );
...
)
{
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_AC_CMD_CLEAR_1XPPP,
...
)
{
  ...
  #ifdef FEATURE_CIOT
  user_pref->ciot_lte_pref.lte_op_mode_pref       = CM_LTE_IOT_OP_MODE_PREF_NO_CHANGE;
  user_pref->ciot_lte_pref.lte_m1_band_pref  = SYS_LTE_BAND_MASK_CONST_NO_CHG;
  user_pref->ciot_lte_pref.lte_nb1_band_pref = SYS_LTE_BAND_MASK_CONST_NO_CHG;
  #endif
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_6("sys_sel_pref(),domain_pref %d,net_sel_mode %d ,prl_pref %d,req_id %u, CSG ID %d, CSG RAT %d",
  ...
  CM_MSG_HIGH_2("sys_sel_pref(),ue_usage_setting %d voice_domain_pref %d",
                      cmd_info_ptr->ue_usage_setting, cmd_info_ptr->voice_domain_pref);
  ...
  #if defined(FEATURE_CIOT)
  cmd_info_ptr->ciot_lte_pref.lte_op_mode_pref   = user_pref->ciot_lte_pref.lte_op_mode_pref;
  cmd_info_ptr->ciot_lte_pref.lte_m1_band_pref   = user_pref->ciot_lte_pref.lte_m1_band_pref;
  cmd_info_ptr->ciot_lte_pref.lte_nb1_band_pref  = user_pref->ciot_lte_pref.lte_nb1_band_pref;
  cmd_info_ptr->ciot_lte_pref.lte_band_pref      = user_pref->lte_band_pref;

  CM_MSG_HIGH_1("sys_sel_pref(),ciot pref's lte_op_mode %d", user_pref->ciot_lte_pref.lte_op_mode_pref);
  cm_print_lte_band_mask(user_pref->ciot_lte_pref.lte_m1_band_pref);
  cm_print_lte_band_mask(user_pref->ciot_lte_pref.lte_nb1_band_pref);
  #endif
...
)
{
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, ac_cmd,
      CM_AC_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
  ...
  )
{
  ...
  */
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_SET_TRM_PRIORITY,
      CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
  #endif
...
)
{
  ...
  */
  ...
  /* Print debug messages for params.
  */
  CM_MSG_HIGH_2("cm_ph_cmd_set_data_priority_per_subs(): priority_type %d, sub %d, enqueuing SET_DATA_PRIRORITY_CMD",
  ...
  */
  ...
  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_DATA_PRIORITY,
      CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
  return FALSE;
...
{
  #if defined(FEATURE_CM_LTE) && defined(FEATURE_CIOT)
...
#if 0
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( lte_data_trasport_req_ptr == NULL )
  {
    if( cmd_cb_func != NULL )
    {
      CM_ERR_0("ERROR: NULL pdn_connectivity_ptr");
      cmd_cb_func( data_block_ptr, CM_CALL_CMD_LTE_DATA_TRANSPORT_REQ,
                   CM_CALL_CMD_ERR_NULL_PARAM_P);
    }
  ...
  */
    ...
    cmd_info_ptr                  = CMD_INFO_PTR( call_cmd_ptr );
	 ...
	 CM_MSG_HIGH_1( "cm_lte_data_transport_req() - client = %d", client_id);
#if 0 
    cmd_info_ptr->lte_info.dsm_item      = lte_data_trasport_req_ptr->dsm_item;
    cmd_info_ptr->lte_info.eps_bearer_id = lte_data_trasport_req_ptr->eps_bearer_id;
    cmd_info_ptr->lte_info.release_assistance_ind 
                                         = lte_data_trasport_req_ptr->release_assistance_ind;
    cmd_info_ptr->lte_info.as_id         = lte_data_trasport_req_ptr->asubs_id;
    cmd_info_ptr->lte_info.mo_exception_data = lte_data_trasport_req_ptr->mo_exception_data;
    cmd_info_ptr->lte_info.empty_user_data_container = lte_data_trasport_req_ptr->empty_user_data_container;
    cmd_info_ptr->lte_info.seq_num = lte_data_trasport_req_ptr->seq_num;

    CM_MSG_MED_3("cm_lte_data_transport_req(), eps_bearer_id = %d release_assistance_ind %d %d",\
                lte_data_trasport_req_ptr->eps_bearer_id, 
                lte_data_trasport_req_ptr->release_assistance_ind.valid,
                lte_data_trasport_req_ptr->release_assistance_ind.dl_data_exp);

    CM_MSG_MED_5("cm_lte_data_transport_req(), dsm_item = %p asub_id %d  mo_exception_data = %d empty_user_data_container = %d sequence_num = %d",\
                lte_data_trasport_req_ptr->dsm_item, 
                lte_data_trasport_req_ptr->asubs_id,
                lte_data_trasport_req_ptr->mo_exception_data,
                lte_data_trasport_req_ptr->empty_user_data_container,
                lte_data_trasport_req_ptr->seq_num);
#endif
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  #else
  SYS_ARG_NOT_USED( lte_data_trasport_req_ptr );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( client_id );
...
=================================================*/
boolean cm_ph_cmd_timer_info_ready(
...
)
{
  ...
  */
  ...
  ph_cmd_ptr->data_block_ptr  = data_block_ptr;
  ph_cmd_ptr->client_id       = client_id;
  ph_cmd_ptr->cmd             = CM_PH_CMD_TIMER_INFO;
...
void cm_client_get_ph_info(
cm_ph_info_s_type* ph_info
)
{
  (void) cmph_info_get( ph_info, CM_SS_MAIN );
}
/*===========================================================================
...
{
  byte allocated_calls = 0;
  cm_call_id_type   call_id;
  for(call_id = 0; call_id < CMCALL_MAX_CALL_OBJ; call_id++)
  {
    if(cmcall_ptr(call_id) != NULL)
    {
      allocated_calls|=(uint8)BM(call_id);
    }
  }
...
FUNCTION: cm_call_info_get_call_id

===========================================================================*/
/**
  Clients call this function to know call info of specific call

@return
  TRUE: if call_id found
...
)
{
  ...
  return cmcall_info_get(call_ptr, call_info_ptr);
}
...
FUNCTION: cm_sns_set_sensor_config

===========================================================================*/
/**
  Clients call this function to set the  sesnor configuration
...
boolean cm_sns_cmd_set_config(cm_sns_cmd_cb_f_type cmd_cb_func,
                                      cm_client_id_type  client_id,
                                      const cm_sns_config_data_s_type* sns_arr_ptr,
                                       uint8 num_of_sns)
{

  cm_sns_cmd_s_type       *sns_cmd_ptr = NULL;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;

  CM_ASSERT( INRANGE(client_id,CM_CLIENT_TYPE_NONE,CM_CLIENT_TYPE_MAX));

  /* Check for valid number of sns ids */
  if( !INRANGE(num_of_sns, CM_SNS_TYPE_ALS, CM_SNS_TYPE_ENV_SENSOR_MAX))
  {
  ...
  if (sns_arr_ptr == NULL)
  {
    cmd_err = CM_SNS_CMD_ERR_ARRAY_PTR_NULL;
  }
  ...
  ** return failure.
  */
  if (cmd_err != CM_SNS_CMD_ERR_NOERR)
  {
    if( cmd_cb_func != NULL )
    {
      cmd_cb_func( CM_SNS_CMD_SET_CONFIG,
                   cmd_err);
    }
   ...
   /* Now that all function parameters are correct,
  ** get command buffer to queue CM_SNS_CMD_CONFIG
  */
  sns_cmd_ptr = cm_cmd_alloc_sns_init();


  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_CMD_SET_CONFIG;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_config_req.len  = num_of_sns;

  memscpy(cmd_info_ptr->cmd_s.sns_config_req.sns_arr,
          num_of_sns * sizeof(cm_sns_config_data_s_type),
          sns_arr_ptr,
          num_of_sns * sizeof(cm_sns_config_data_s_type));

  cm_cmd_queue( (cm_cmd_type*)sns_cmd_ptr );
...
boolean cm_sns_cmd_get_config_req(cm_sns_cmd_cb_f_type cmd_cb_func,
                                      cm_client_id_type  client_id,
                                     cm_sns_id_e_type   sensor_id)
{

  cm_sns_cmd_s_type 		   *sns_cmd_ptr;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;
  CM_ASSERT( BETWEEN(client_id,CM_CLIENT_TYPE_NONE,CM_CLIENT_TYPE_MAX));
  ...
  */
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_CMD_GET_CONFIG;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;
  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_registry_config.sensor_id = sensor_id;
  cm_cmd_queue( (cm_cmd_type*)sns_cmd_ptr );
...
boolean cm_sns_cmd_set_config_non_env(cm_sns_cmd_cb_f_type cmd_cb_func,
                                      cm_client_id_type  client_id,
                                      cm_sns_config_s_type* sns_config_req_ptr,
                                       uint8 num_of_sns)
...

{
  ...
  if( !INRANGE(num_of_sns, CM_SNS_TYPE_ALS, CM_SNS_TYPE_MAX))
  {
    cmd_err = CM_SNS_CMD_ERR_CONFIG_INVALID;
  }
  ...
  */
  ...
  memscpy(&cmd_info_ptr->cmd_s.sns_config_req,
          sizeof(cm_sns_config_s_type),
          sns_config_req_ptr,
          sizeof(cm_sns_config_s_type));
...
boolean cm_sns_phy_sns_test_req(cm_sns_cmd_cb_f_type cmd_cb_func, cm_client_id_type   client_id, cm_sns_id_e_type sensor_id, cm_sns_phy_test_e_type phy_sns_test)
{
  cm_sns_cmd_s_type       *sns_cmd_ptr = NULL;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_PHY_TEST;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_phy_test.sensor_id = sensor_id;
  cmd_info_ptr->cmd_s.sns_phy_test.phy_sns_test = phy_sns_test;
...
boolean cm_sns_phy_sns_test_get_data_req(cm_sns_cmd_cb_f_type cmd_cb_func, cm_client_id_type  client_id,cm_sns_id_e_type sensor_id, boolean is_enabled, float sample_rate, uint32 report_rate)
{
  cm_sns_cmd_s_type       *sns_cmd_ptr = NULL;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_PHY_TEST_GET_DATA;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_phy_test_get_data.sensor_id = sensor_id;
  cmd_info_ptr->cmd_s.sns_phy_test_get_data.is_enabled = is_enabled;
  cmd_info_ptr->cmd_s.sns_phy_test_get_data.sample_rate = sample_rate;
  cmd_info_ptr->cmd_s.sns_phy_test_get_data.report_rate = report_rate;
...
boolean cm_sns_set_sensor_logging_level(cm_sns_cmd_cb_f_type cmd_cb_func, cm_client_id_type   client_id, sns_tracker_log_level logging_level)
{
  cm_sns_cmd_s_type             *sns_cmd_ptr   = NULL;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_SET_LOGGING_LEVEL;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_logging_level = logging_level;
...
boolean cm_sns_serial_number_req(cm_sns_cmd_cb_f_type cmd_cb_func, cm_client_id_type  client_id,cm_sns_id_e_type sensor_id)
{
  cm_sns_cmd_s_type       *sns_cmd_ptr = NULL;
  cm_sns_cmd_info_s_type        *cmd_info_ptr  = NULL;
  cm_sns_cmd_err_e_type          cmd_err	   = CM_SNS_CMD_ERR_NOERR;
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_GET_SERIAL_NUM;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;
...
#endif
#if 0
void cm_sns_update_req(cm_sns_cmd_cb_f_type cmd_cb_func,
	                                     cm_client_id_type  client_id,
	                                     cm_sns_id_e_type sensor_id,
	                                     boolean enable);
{
  ...
  /* Check for valid number of sns ids */
  if( !INRANGE(sensor_id, 1, (CM_SNS_TYPE_ENV_SENSOR_MAX-1)))
  {
    cmd_err = CM_SNS_NUM_SNS_NOT_IN_RANGE;
  }
  ...
  */
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_UPDATE_CMD;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_update_req.enable = enable;
  cmd_info_ptr->cmd_s.sns_update_req.sensor_id = sensor_id;
...
boolean cm_sns_cmd_data_req(cm_sns_cmd_cb_f_type cmd_cb_func,
                                      cm_client_id_type  client_id,
									...
									)
{
  ...
  */
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_CMD_DATA_REQ;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_data_req.sensor_id_mask = sensor_id_mask;
  #ifdef FEATURE_MMODE_TX_21
  cmd_info_ptr->cmd_s.sns_data_req.is_logging_mode = is_logging_mode;
  #endif
  cm_cmd_queue( (cm_cmd_type*)sns_cmd_ptr );
                                     ...
                                     uint32   sensor_id_mask )
{
  ...
  */
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_CMD_OD_SNS_REQ;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;

  cmd_info_ptr                   = CMD_INFO_PTR( sns_cmd_ptr );
  cmd_info_ptr->cmd_s.sns_od_sns_req.sensor_id_mask = sensor_id_mask;
...
cm_sns_event_info_s_type cm_sns_get_event_info(void)
{
  cm_sns_event_info_s_type event_info = {0};
  cmsns_get_event_info(&event_info);
  return event_info;
}
...
boolean cm_sns_get_if_psm_entry_in_progress(void)
{
  return cmsnsmgr_is_psm_entry_in_progress();
...
===========================================================================*/
boolean cm_sns_cmd_reset_config(cm_sns_cmd_cb_f_type cmd_cb_func,cm_client_id_type  client_id)
{
  ...
  */
  ...
  sns_cmd_ptr->client_id         = client_id;
  sns_cmd_ptr->cmd               = CM_SNS_CMD_RESET_CONFIG;
  sns_cmd_ptr->cmd_cb_func       = cmd_cb_func;
...
boolean cm_sns_set_motion_sensor_config(boolean is_motion_enable)
{
  cm_sns_motion_sensor_config_req_s_type *sns_config_req = 
               (cm_sns_motion_sensor_config_req_s_type*)cm_mem_malloc(sizeof(cm_sns_motion_sensor_config_req_s_type));
  
  if(sns_config_req != NULL)
  {
     memset(sns_config_req, 0, sizeof(cm_sns_motion_sensor_config_req_s_type));
     sns_config_req->motion_enable = is_motion_enable;
     CM_MSG_HIGH_1(" Client seting motion config = %d ",is_motion_enable);
     cm_msgr_send(MM_CM_SNS_MGR_SNS_MOTION_CONFIG_REQ,MSGR_MM_CM_SNS_MGR,
           (msgr_hdr_s*)sns_config_req,sizeof(cm_sns_motion_sensor_config_req_s_type));
     cm_mem_free(sns_config_req);
     return TRUE;
  }
  return FALSE;
}
...
===========================================================================*/
/**
  Clients call this function to set the baudrate
  if baudrate is invalid set it to 112500
...
void cm_psm_set_uart_baude_rate(uint16 uart_baud_rate)
{
  ...
  cm_psm_uart_baud_rate_ind_s_type *uart_baud_rate_ind = NULL;
  CM_MSG_HIGH_1(" Baud rate received = %d ",uart_baud_rate);
  
  uart_baud_rate_ind = (cm_psm_uart_baud_rate_ind_s_type*)cm_mem_malloc(sizeof(cm_psm_uart_baud_rate_ind_s_type));
  
  if(uart_baud_rate_ind != NULL)
  {
     memset(uart_baud_rate_ind, 0, sizeof(cm_psm_uart_baud_rate_ind_s_type));
     uart_baud_rate_ind->uart_baud_rate = uart_baud_rate;
    
     cm_msgr_send(MM_CM_PSM_UART_BAUD_RATE_IND,MSGR_MM_CM_PSM,
           (msgr_hdr_s*)uart_baud_rate_ind,sizeof(cm_psm_uart_baud_rate_ind_s_type));
     cm_mem_free(uart_baud_rate_ind);
     return ;
  }
...
FUNCTION cm_ph_cmd_get_networks_advance_search
...
SIDE EFFECTS
  ...
  NETWORKS_CONF event.
===========================================================================*/
boolean cm_ph_cmd_get_networks_advance_search(
        ...
        cm_ciot_lte_op_mode_e_type     lte_op_mode, /*  M1 or NB1*/
        
        sys_wwan_adavnce_scan_pram_s_type  *advn_srch_param,
           /*   All the advance search parameter to control the scan 
           Result  */
        uint8   gps_wwan_scan_act_mask
           /*   mask to know if if GPS procedure is highest than WWAN procedure
          and also the blind scan */
)
{
  ...
  if (advn_srch_param == NULL)
  {
	if( cmd_cb_func != NULL )
    {
      cmd_cb_func( data_block_ptr,CM_PH_CMD_GET_NETWORKS,CM_PH_CMD_ERR_INPUT_NULL_POINTER);
    }
  ...
  cmd_info_ptr->band_pref_lmt     = CM_BAND_PREF_ANY;
  cmd_info_ptr->lte_band_pref_lmt = sys_map_band_mask_to_lte_band_mask(CM_BAND_PREF_LTE_ANY);
#ifdef   FEATURE_GPS_WWAN_PRIORITY_SEARCH

  memscpy(&(cmd_info_ptr->gps_wwan_scan_info) ,sizeof(sys_wwan_adavnce_scan_pram_s_type), 
                            advn_srch_param,sizeof(sys_wwan_adavnce_scan_pram_s_type));
  cmd_info_ptr->gps_wwan_scan_act_mask = gps_wwan_scan_act_mask;
#endif
#if defined(FEATURE_CIOT)
  cmd_info_ptr->ciot_lte_pref_get_net.lte_op_mode_pref   = lte_op_mode;
  cmd_info_ptr->ciot_lte_pref_get_net.lte_m1_band_pref   = SYS_LTE_M1_BAND_MASK_CONST_ANY;
  cmd_info_ptr->ciot_lte_pref_get_net.lte_nb1_band_pref  = SYS_LTE_NB1_BAND_MASK_CONST_ANY;
  cmd_info_ptr->ciot_lte_pref_get_net.lte_band_pref   =    SYS_LTE_BAND_MASK_CONST_ANY;

  CM_MSG_HIGH_4("cm_ph_cmd_get_networks_advance_search(), gps_wwan_scan_act_mask = %d ciot pref's lte_op_mode %d network type %d list type = %d", 
                                       gps_wwan_scan_act_mask,lte_op_mode, network_type,list_type);
  cm_print_lte_band_mask( cmd_info_ptr->ciot_lte_pref_get_net.lte_m1_band_pref);
  cm_print_lte_band_mask( cmd_info_ptr->ciot_lte_pref_get_net.lte_nb1_band_pref);
#endif
  ...
  */
  CM_MSG_HIGH_5("CM_PH_CMD_GET_NETWORKS: network_type %d, list_type %d, subs_id %d. GW  BAND PREF 0x%08x %08x ",
                  network_type, list_type, SYS_MODEM_AS_ID_1,QWORD_HIGH(CM_BAND_PREF_ANY), QWORD_LOW(CM_BAND_PREF_ANY));
...
#else
  SYS_ARG_NOT_USED( list_type );
  SYS_ARG_NOT_USED( data_block_ptr );
  SYS_ARG_NOT_USED( client_id );
  SYS_ARG_NOT_USED( network_type );
  SYS_ARG_NOT_USED( lte_op_mode );
  SYS_ARG_NOT_USED( gps_wwan_scan_act_mask );

  if( cmd_cb_func != NULL )
  {
    cmd_cb_func( data_block_ptr, CM_PH_CMD_GET_NETWORKS,
    CM_PH_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE);
  }
...
{
   cm_cmd_user_pref_update_type user_pref = {0};
   cm_rat_acq_order_pref_s_type      rat_acq_order_pref = {0};
   
   user_pref.req_id = 0;
   user_pref.client_id = client_id;
   user_pref.asubs_id = SYS_MODEM_AS_ID_1;
   user_pref.mode_pref = mode_pref;
   user_pref.pref_term = term_pref;
   user_pref.pref_duration = pref_duration;
   user_pref.acq_order_pref = acq_order_pref;
   ...
   user_pref.band_pref = band_pref;
   user_pref.prl_pref = prl_pref;
   user_pref.roam_pref = roam_pref;
   user_pref.hybr_pref = hybr_pref;
   user_pref.srv_domain_pref = srv_domain_pref;
   user_pref.network_sel_mode_pref = network_sel_mode_pref;
   user_pref.plmn_ptr = plmn_ptr;

   user_pref.lte_band_pref = sys_lte_band_pref_lte_const_no_chg();
   user_pref.tds_band_pref = CM_BAND_PREF_NO_CHANGE;
   ...
   return cm_user_pref_update_req(&user_pref,cmd_cb_func,data_block_ptr);
...
FUNCTION cm_ph_get_ciot_device_band_capability

DESCRIPTION
   Sync API to get the RF band capability.

DEPENDENCIES
  cm_init_before_task_start() must have already been called.

RETURN VALUE
  TRUE  = CM has valid band on any rat
  FALSE = CM has NO valids bands on any rat

SIDE EFFECTS
  If command is successful, CM clients will receive the bands
===========================================================================*/
					 
boolean cm_ph_get_ciot_device_band_capability(
                     uint64* band_bref_gsm,
                     sys_lte_band_mask_e_type*  band_pref_NB1,
                     sys_lte_band_mask_e_type*  band_pref_M1)
{
  return cmph_get_ciot_rf_band_capability(band_bref_gsm,band_pref_NB1,band_pref_M1);
}


#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH_TEST

boolean cm_gps_wwan_read_and_fill_scan_param_from_efs(sys_loc_wwan_scan_assistance_data_s_type *wwan_scan_params)
{
   mmode_test_gps_wwan_scan_param_s_type  efs_data = {0};
   int32  size_data = sizeof(mmode_test_gps_wwan_scan_param_s_type);
   
   memset((void *)&efs_data,0,size_data);
   
   if(!cmefs_read(CMEFS_MMODE_GPS_WWAN_SCAN, (byte*)&efs_data, &size_data))
   {
     CM_ERR_0("GPS WWAN scan params EFS read failed"); 
     return FALSE;
   }

   if(!((efs_data.rat == SYS_RAT_GSM_RADIO_ACCESS) || (efs_data.rat == SYS_RAT_LTE_M1_RADIO_ACCESS)||
       (efs_data.rat == SYS_RAT_LTE_NB1_RADIO_ACCESS) ))
   {
       
     CM_ERR_1("GPS WWAN scan params rat not supported = %d ",efs_data.rat); 
     return FALSE;
   }
   wwan_scan_params->rat = efs_data.rat;
   if(wwan_scan_params->rat == SYS_RAT_GSM_RADIO_ACCESS)
   	{
       wwan_scan_params->wwan_rat_scan_param.gsm_params = efs_data.wwan_rat_scan_param.gsm_params;
   	}
   #ifdef FEATURE_MMODE_M1
   else if(wwan_scan_params->rat == SYS_RAT_LTE_M1_RADIO_ACCESS )
   	{
       wwan_scan_params->wwan_rat_scan_param.m1_scan_params= efs_data.wwan_rat_scan_param.m1_scan_params;
   	}
   #endif
   else if (wwan_scan_params->rat == SYS_RAT_LTE_NB1_RADIO_ACCESS )
   	{
       wwan_scan_params->wwan_rat_scan_param.nb1_scan_params= efs_data.wwan_rat_scan_param.nb1_scan_params;
   	}
  return TRUE;
}
...
FUNCTION cmtest_diag_cmd_ph_cmd_cb_func

DESCRIPTION
    its print the ph command and ph_cmd_err
...
void cmtest_diag_cmd_ph_cmd_cb_func(
    void                    *data_block_ptr,
    cm_ph_cmd_e_type        ph_cmd,
    cm_ph_cmd_err_e_type    ph_cmd_err
)
{
  CM_MSG_LOW_0( "=================PH CMD ERR================== CL");
  CM_MSG_LOW_2( "Cmd. client cmd=%d, err=%d",ph_cmd,ph_cmd_err);
  CM_PH_CMD_PRT( ph_cmd );
  CM_PH_CMD_ERR_PRT( ph_cmd_err );
  CM_MSG_LOW_0( "============================================== CL");
}

/*==========================================================================

FUNCTION cm_diag_trigger_advance_gps_scan_request

DESCRIPTION
  Triggered gps scan request by diag command
  Arguement:- is_high_prio_search 
             TRUE --> GPS search is high priority
             FALSE --> GPS search is lowe priority
...
void cm_diag_trigger_advance_gps_scan_request(uint16 scan_mask  )
{

   boolean  efs_read_val = FALSE;
   uint8 num_val = 0;
   sys_wwan_adavnce_scan_pram_s_type *wwan_scan_params_ad = NULL;
   sys_loc_wwan_scan_assistance_data_s_type *wwan_scan_params = (sys_loc_wwan_scan_assistance_data_s_type *)cm_mem_malloc(sizeof(sys_loc_wwan_scan_assistance_data_s_type));
   cm_mode_pref_e_type mode_pref_local = CM_MODE_PREF_NONE;
   cm_ciot_lte_op_mode_e_type   lte_op_mode_local = CM_LTE_IOT_OP_MODE_PREF_NONE;


   CM_MSG_HIGH_1("cm_diag_trigger_advance_gps_scan_request with scan mask = %d ",scan_mask);


   efs_read_val = cm_gps_wwan_read_and_fill_scan_param_from_efs(wwan_scan_params);

   if(efs_read_val == FALSE)
   {
      cm_mem_free(wwan_scan_params);
	  return;
   }


	wwan_scan_params_ad = (sys_wwan_adavnce_scan_pram_s_type *)cm_mem_malloc(sizeof(sys_wwan_adavnce_scan_pram_s_type));

	wwan_scan_params_ad->assist_param.rat = wwan_scan_params->rat;

	CM_MSG_HIGH_1("The rat after EFS read is %d ",wwan_scan_params->rat);
	if(wwan_scan_params->rat == SYS_RAT_LTE_M1_RADIO_ACCESS)
	{
	  CM_MSG_HIGH_1("M1 The no of element is %d ",wwan_scan_params->wwan_rat_scan_param.m1_scan_params.num_m1_ad_element);
	        wwan_scan_params_ad->assist_param.wwan_rat_scan_param.m1_scan_params = wwan_scan_params->wwan_rat_scan_param.m1_scan_params;
	  num_val = 	wwan_scan_params->wwan_rat_scan_param.m1_scan_params.num_m1_ad_element;
	     
	}
	if(wwan_scan_params->rat == SYS_RAT_LTE_NB1_RADIO_ACCESS)
	{
	  CM_MSG_HIGH_1("NB1 The no of element is %d ",wwan_scan_params->wwan_rat_scan_param.nb1_scan_params.num_nb1_ad_element);
	    wwan_scan_params_ad->assist_param.wwan_rat_scan_param.nb1_scan_params = wwan_scan_params->wwan_rat_scan_param.nb1_scan_params;
	  num_val = 	wwan_scan_params->wwan_rat_scan_param.nb1_scan_params.num_nb1_ad_element;
	}
	if(wwan_scan_params->rat == SYS_RAT_GSM_RADIO_ACCESS)
	{
	  CM_MSG_HIGH_1("GSM The no of element is %d ",wwan_scan_params->wwan_rat_scan_param.gsm_params.num_gsm_ad_element);
	  wwan_scan_params_ad->assist_param.wwan_rat_scan_param.gsm_params = wwan_scan_params->wwan_rat_scan_param.gsm_params;
	  num_val = 	wwan_scan_params->wwan_rat_scan_param.gsm_params.num_gsm_ad_element;
	}

	wwan_scan_params_ad->control_param.max_cells_per_channel = 5;
	cm_mem_free(wwan_scan_params);

	
	CM_MSG_HIGH_1("The rat after copy to ad structure read is %d ",wwan_scan_params_ad->assist_param.rat);

	if (num_val > SYS_MAX_EARFCN_SCAN_REQ)
  	{
      if(cm_get_ulog_handle() != NULL)
      {
       ULOG_RT_PRINTF_2(cm_get_ulog_handle(), "=CM= Net list request diag caller - num ad element = :%d - rat = :%d ",
                                                   num_val,wwan_scan_params_ad->assist_param.rat );
      }
      CM_ERR_FATAL_0(" netlist crash");
    }
  

   if(wwan_scan_params_ad->assist_param.rat  == SYS_RAT_GSM_RADIO_ACCESS )
   {
       mode_pref_local = CM_MODE_PREF_GSM_ONLY;
   }

   #ifdef FEATURE_MMODE_M1
   if ( wwan_scan_params_ad->assist_param.rat ==  SYS_RAT_LTE_M1_RADIO_ACCESS )   
   {
     mode_pref_local = CM_MODE_PREF_LTE_ONLY;
     lte_op_mode_local = CM_LTE_IOT_OP_MODE_PREF_M1;
   }
   #endif
   if (wwan_scan_params_ad->assist_param.rat  == SYS_RAT_LTE_NB1_RADIO_ACCESS)
   {
     mode_pref_local = CM_MODE_PREF_LTE_ONLY;
     lte_op_mode_local = CM_LTE_IOT_OP_MODE_PREF_NB1;
   }
    /* User does get networks.
  */
   CM_MSG_HIGH_3("cm_diag_trigger_advance_gps_scan_request mode pref= %d  lte_op_mode = %d rat = %d ",mode_pref_local,
                                          lte_op_mode_local,wwan_scan_params_ad->assist_param.rat );
   
   if(wwan_scan_params_ad->assist_param.rat == SYS_RAT_LTE_M1_RADIO_ACCESS)
   {
	 CM_MSG_HIGH_1(" M1 sending The no of element is %d ",wwan_scan_params_ad->assist_param.wwan_rat_scan_param.m1_scan_params.num_m1_ad_element);
   }
   if(wwan_scan_params_ad->assist_param.rat == SYS_RAT_LTE_NB1_RADIO_ACCESS)
   {
	 CM_MSG_HIGH_1("NB1 sending The no of element is %d ",wwan_scan_params_ad->assist_param.wwan_rat_scan_param.nb1_scan_params.num_nb1_ad_element);
   }
   if(wwan_scan_params_ad->assist_param.rat == SYS_RAT_GSM_RADIO_ACCESS)
   {
     CM_MSG_HIGH_1(" GSM sending The no of element is %d ",wwan_scan_params_ad->assist_param.wwan_rat_scan_param.gsm_params.num_gsm_ad_element);
   }

   
   cm_ph_cmd_get_networks_advance_search (
    cmtest_diag_cmd_ph_cmd_cb_func,
    NULL,
    CM_CLIENT_TYPE_GPS,
  ...
  cm_mem_free(wwan_scan_params_ad);
