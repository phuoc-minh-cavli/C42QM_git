...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmmsc_auto.c#1 $
...
)
{
  ...
  */
  is_ph_obj_cdma_mode =
  cmcall_misc_is_mode_pref( ph_orig_ptr->orig->orig_mode_pref ,CM_MODE_PREF_CDMA_HDR_ONLY );
  ...
  */
    ...
    )
  {
    ...
    */
    mode_pref = cmcall_remove_mode_pref_components( ph_ptr->main_stack_info.pref_info.mode_pref,
                                                    1,
...
)
{
  ...
  pref_info.ciot_lte_pref = ph_ptr->main_stack_info.pref_info.ciot_lte_pref;
...
)
{
  ...
  */
  ...
  #if defined(FEATURE_MMODE_SC_SVLTE) || defined(FEATURE_MMODE_3GPP2)
...
)
{
  ...
  if(cmph_is_subs_feature_mode_srlte(state_machine->asubs_id))
  {
    ...
    CM_MSG_HIGH_1("srlte: ue_mode_forced %d",
...
)
{
  ...
  CM_MSG_HIGH_6("IS_HYBR2_ALLOWED: sfmode1=%d, ue_mode=%d, is_trm_svlte=%d, asubs_id=%d, is_msim_normal_mode=%d, oprt_mode=%d",
                 cmph_get_subs_feature_mode(asubs_id),
                 ue_mode,
                 cmph_ptr()->dbg_info.is_trm_svlte,
                 asubs_id,is_msim_normal_mode,cmph_ptr()->oprt_mode);
...
)
{
  ...
  */
  ...
  #if defined(FEATURE_MMODE_3GPP2) && defined(FEATURE_HDR)
...
)
{
  ...
  else
  {
    ...
    mode = mode & SD_SS_MODE_PREF_LTE;
    #elif defined(FEATURE_MMODE_GPRS_LTE)
    mode = mode & SD_SS_MODE_PREF_GSM_LTE;
    #endif
...
{
  if(state_machine->stack_common_info.timer.acq_hybr_1_uptime != uptime)
  {
...
{
  #if defined( FEATURE_MMODE_3GPP2 ) || defined(FEATURE_MMODE_SC_SVLTE)
  cm_orig_q_s_type *hybr1_top = cmtask_orig_para_get_top(CM_SS_HYBR_1);
  ...
  switch(ss)
  {
    case CM_SS_HYBR_1:
    {
      ...
      if(*cmph_hdr_kick_pending())
      {
        ...
        #ifdef FEATURE_HDR_HYBRID
        if( cmcall_is_hybr_hdr_orig())
        {
          pref_package.pref_reas = SD_SS_PREF_REAS_ORIG_START_PS;
        }
        #endif
...
)
{
  ...
  switch(stack_idx)
  {
   ...
   #if defined(FEATURE_MMODE_SC_SVLTE) || defined(FEATURE_MMODE_CDMA) || defined(FEATURE_MMODE_SC_SGLTE)
    case 2:
...
)
{
  ...
  //CM_MSG_HIGH_4("RPT->MSC: SRV_IND sub %d stack = %d, srv_status = %d, mode = %d",
  //                 msc->asubs_id, stack_idx, srv_status, mode);
