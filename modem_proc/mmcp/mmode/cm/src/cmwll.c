...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmwll.c#1 $
...
FUNCTION cm_msm_get_buf_else_err_fatal

DESCRIPTION
  Allocate a MBMS mgr Task command buffer. Error fatal if allocation is NOT
  successful.
...
===========================================================================*/
msm_ext_cmd_type *cm_msm_get_buf_else_err_fatal(void)
/*lint -esym(714,cm_msm_get_buf_else_err_fatal) */
/*lint -esym(765,cm_msm_get_buf_else_err_fatal)
...
*/
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Try allocating a MBMS mgr Task command buffer
  */
  msm_cmd_ptr = msm_get_cmd_buf ();
...
FUNCTION cm_msm_send_cmd

DESCRIPTION
  Send a command to the MBMS mgr task.
...
===========================================================================*/
void cm_msm_send_cmd(

  msm_ext_cmd_type   *msm_cmd_ptr
  /* Pointer to MBMS mgr command */
)
...
{  
  ...
  CM_ASSERT( msm_cmd_ptr != NULL );

  CM_MSG_HIGH_0 ( "========= FORWARD CMD TO MBMS MGR ========");
  CM_MSG_HIGH_1( "Send cmd %d to MBMS", msm_cmd_ptr->cmd_hdr.cmd_id);  

  msm_put_cmd( msm_cmd_ptr );
  //#else
