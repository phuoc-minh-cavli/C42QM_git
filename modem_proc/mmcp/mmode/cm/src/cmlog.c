...
$Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmlog.c#1 $
...
/*lint -esym(715, ph_event) */
{
  ...
  if (diag_client.oprt_mode != ph_info_ptr->oprt_mode)
  {
    ...
    */
    if(ph_info_ptr->oprt_mode == SYS_OPRT_MODE_OFFLINE_CDMA || ph_info_ptr->oprt_mode == SYS_OPRT_MODE_OFFLINE)
    {
...
)
{
    ...
    case CM_PH_EVENT_NAM_SEL:
      ...
      *pref_ptr = (uint8)ph_info_ptr->asubs_id;
      ...
      #endif /* FEATURE_MMODE_DUAL_SIM */
      {
        ...
        #if defined(FEATURE_MMODE_GW) || defined(FEATURE_MMODE_3GPP2)
  ...
  */
  ...
  #else
  log_ptr->test_control_type     = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  */
  ...
  #else
  log_ptr->cdma_lock_mode        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->answer_voice          = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->roam_pref             = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->nam_sel               = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->curr_nam              = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->packet_state          = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hybr_pref             = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->ddtm_pref             = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->cur_ddtm_status       = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->ddtm_act_mask         = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->orig_ddtm_act_mask    = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->ddtm_num_srv_opt      = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  for ( i = 0; i < log_ptr->ddtm_num_srv_opt; i++ )
  {
   log_ptr->ddtm_srv_opt_list[i] = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  }
  log_ptr->is_cdma_subscription_available =
                                 CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->rtre_config                    = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->rtre_control                   = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  ...
  #else
  log_ptr->acq_order_pref        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  
  #if defined(FEATURE_TDSCDMA)
  log_ptr->tds_band_pref         = ph_info_ptr->tds_band_pref; /*lint !e713 */
  #else
  log_ptr->tds_band_pref         = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  #else
  log_ptr->cphs_allowed         = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->als_allowed          = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->line                 = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->line_switching       = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
...
void cmlog_set_cm_offline_signal_recvd(void)
{
  ...
  */
  CM_ASSERT(ph_info_ptr->oprt_mode == SYS_OPRT_MODE_OFFLINE_CDMA || ph_info_ptr->oprt_mode == SYS_OPRT_MODE_OFFLINE);
...
)
{
  ...
  switch ( ss_event )
  {
    case CM_SS_EVENT_SRV_CHANGED:
      ...
      *srv_ptr = SYS_MODEM_AS_ID_1 ;
       ...
       
      {
    ...
    #if defined(FEATURE_CIOT)
        if ( diag_client.ciot_lte_mode != ss_info_ptr->ciot_lte_mode)
        {
          value = (int)(diag_client.ciot_lte_mode = ss_info_ptr->ciot_lte_mode);
          event_report_payload( EVENT_CM_SYSTEM_MODE, sizeof(value), &value );
        }
    #endif
        {
          
          if((ss_info_ptr->changed_fields & CM_SS_EVT_PS_DATA_SUSPEND_MASK) != 0)
          {
            uint8 result[3];
            result[0] = (uint8)SYS_MODEM_AS_ID_1;
        ...
        #ifdef FEATURE_MMODE_GW
        if(((ss_info_ptr->changed_fields & CM_SS_EVT_SRV_IND_MASK) !=0) &&
  ...
  */
  ...
  log_ptr->asubs_id               = SYS_MODEM_AS_ID_1 ;
  for(index_val = 0 ; index_val< 1 ; index_val++)
  {
  log_ptr->stack_info[index_val].changed_fields         = ss_info_ptr->changed_fields;
  log_ptr->stack_info[index_val].signal_strength_changed_fields        = ss_info_ptr->signal_strength_changed_fields;
  log_ptr->stack_info[index_val].is_operational         = TRUE ;
  log_ptr->stack_info[index_val].true_srv_status           = ss_info_ptr->true_srv_status;

  log_ptr->stack_info[index_val].srv_status             = ss_info_ptr->srv_status;

  log_ptr->stack_info[index_val].srv_domain             = ss_info_ptr->srv_domain;
  log_ptr->stack_info[index_val].srv_capability         = ss_info_ptr->srv_capability;
  log_ptr->stack_info[index_val].sys_mode               = ss_info_ptr->sys_mode;
  log_ptr->stack_info[index_val].roam_status            = ss_info_ptr->roam_status;

  log_ptr->stack_info[index_val].sys_id.id_type         = ss_info_ptr->sys_id.id_type;
  ...
  switch( ss_info_ptr->sys_id.id_type )
  {
    ...
    case SYS_SYS_ID_TYPE_IS95:
      ...
      log_ptr->stack_info[index_val].sys_id.id.is95.sid = ss_info_ptr->sys_id.id.is95.sid;
      log_ptr->stack_info[index_val].sys_id.id.is95.nid = ss_info_ptr->sys_id.id.is95.nid;
      break;
    ...
    case SYS_SYS_ID_TYPE_IS856:
      ...
      for( i=0; i<16; i++ )
      {
        log_ptr->stack_info[index_val].sys_id.id.is856[i] = ss_info_ptr->sys_id.id.is856[i];
      }
      break;
    #endif
    ...
    case SYS_SYS_ID_TYPE_UMTS:
      ...
      log_ptr->stack_info[index_val].sys_id.id.plmn_lac.lac = ss_info_ptr->sys_id.id.plmn_lac.lac;
      CM_MSG_HIGH_3( "plmn_identity: id %d %d %d",ss_info_ptr->sys_id.id.plmn.identity[0],
                                                  ss_info_ptr->sys_id.id.plmn.identity[1],
      ...
      for( i=0; i<3; i++ )
      {
        ...
        log_ptr->stack_info[index_val].sys_id.id.plmn.identity[i] =
                            ss_info_ptr->sys_id.id.plmn.identity[i];
  ...
  #ifdef FEATURE_MMODE_3GPP2
  log_ptr->stack_info[index_val].is_sys_forbidden       = ss_info_ptr->is_sys_forbidden;
  if(ss_info_ptr->sys_mode == SYS_SYS_MODE_HDR)
  {
    #if defined(FEATURE_HDR)
    log_ptr->stack_info[index_val].rat_mode_info.hdr_mode_info.is_colloc              = ss_info_ptr->rat_mode_info.hdr_mode_info.is_colloc;
    #endif
  }
  else
  {
    #if defined(FEATURE_CDMA)
    log_ptr->stack_info[index_val].rat_mode_info.cdma_mode_info.is_colloc              = ss_info_ptr->rat_mode_info.cdma_mode_info.is_colloc;
    #endif
  }
  #endif
  
  log_ptr->stack_info[index_val].rssi                   = ss_info_ptr->rssi;
  #ifdef FEATURE_WCDMA
  log_ptr->stack_info[index_val].ecio                   = ss_info_ptr->ecio;
  log_ptr->stack_info[index_val].io                     = ss_info_ptr->io;
  #endif
  log_ptr->stack_info[index_val].sinr                   = ss_info_ptr->sinr;
  #if defined(FEATURE_CDMA) || defined(FEATURE_HDR)
  log_ptr->stack_info[index_val].psist                  = ss_info_ptr->psist;  
  log_ptr->stack_info[index_val].prev_idle_digital_mode = ss_info_ptr->prev_idle_digital_mode;
  log_ptr->stack_info[index_val].cur_idle_digital_mode  = ss_info_ptr->cur_idle_digital_mode;
  #endif

  #if defined(FEATURE_HDR)
  log_ptr->stack_info[index_val].rat_mode_info.hdr_mode_info.hdr_hybrid             = ss_info_ptr->rat_mode_info.hdr_mode_info.hdr_hybrid;
  log_ptr->stack_info[index_val].rat_mode_info.hdr_mode_info.hdr_active_prot        = ss_info_ptr->rat_mode_info.hdr_mode_info.hdr_active_prot;
  #endif
  ...
  switch ( ss_info_ptr->sys_mode )
  {
    ...
    case SYS_SYS_MODE_TDS:
      ...
      /* Copy GW info here
      */
      log_ptr->stack_info[index_val].mode_info.gw_info.bit_err_rate   = ss_info_ptr->bit_err_rate;
      log_ptr->stack_info[index_val].mode_info.gw_info.sim_state      = ss_info_ptr->sim_state;
      log_ptr->stack_info[index_val].mode_info.gw_info.plmn_forbidden = ss_info_ptr->plmn_forbidden;

      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.plmn_avail                =
                    ss_info_ptr->mm_information.plmn_avail;
      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.full_name_avail           =
               ss_info_ptr->mm_information.full_name_avail;
      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.short_name_avail          =
              ss_info_ptr->mm_information.short_name_avail;
      ...
      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.lsa_identity_avail        =
            ss_info_ptr->mm_information.lsa_identity_avail;
      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.daylight_saving_adj_avail =
        ss_info_ptr->mm_information.daylight_saving_adj_avail;

      log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.voice_support_on_lte = ss_info_ptr->lte_ims_voice_avail;
      ...
      if( log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.plmn_avail )
      {
        for( i=0; i<3; i++ )
        {
          log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.plmn.identity[i] =
              ss_info_ptr->mm_information.plmn.identity[i];
        }
      ...
      if( log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.full_name_avail )
      {
        ...
        for( i=0; i<log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.full_name.length; i++)
        {
          log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.full_name.name[i] =
             ss_info_ptr->mm_information.full_name.name[i];
        }
      }
      else
      {
      ...
      if( log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.short_name_avail )
      {
        ...
        for( i=0; i<log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.short_name.length; i++)
        {
          log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.short_name.name[i] =
            ss_info_ptr->mm_information.short_name.name[i];
        }
      }
      else
      {
      ...
      if( log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.univ_time_and_time_zone_avail )
      {
        ...
        log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.univ_time_and_time_zone.time_zone =
          ss_info_ptr->mm_information.univ_time_and_time_zone.time_zone;
      ...
      if( log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.lsa_identity_avail )
      {
        log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.lsa_identity.type =
             ss_info_ptr->mm_information.lsa_identity.type;

        for(i=0; i<3; i++ )
        {
          log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.lsa_identity.identity[i] =
            ss_info_ptr->mm_information.lsa_identity.identity[i];
        }
      }
      else
      {
      ...
      log_ptr->stack_info[index_val].mode_info.gw_info.mm_information.daylight_saving_adj =
           ss_info_ptr->mm_information.daylight_saving_adj;
      break;
  ...
  if(ss_info_ptr->sys_mode == SYS_SYS_MODE_GSM)
  {
    ...
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.lac = ss_info_ptr->lac;
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.rac_or_mme_code = ss_info_ptr->rac_or_mme_code;
    #if !defined(FEATURE_MMODE_DISABLE_CSG)
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.csg_id = ss_info_ptr->csg_info.csg_id ;
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.hybrid_cell = ss_info_ptr->csg_info.hybrid_cell ;
    #else
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.csg_id = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.hybrid_cell = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
    #endif
    #endif
  }
  else if(ss_info_ptr->sys_mode == SYS_SYS_MODE_LTE)
  {
    #if defined(FEATURE_LTE)
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.rsrp = ss_info_ptr->rsrp;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.rsrq = ss_info_ptr->rsrq;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.lac  = ss_info_ptr->lac;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.tac  = ss_info_ptr->tac;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.rac_or_mme_code = ss_info_ptr->rac_or_mme_code;
    #if defined(FEATURE_MMODE_EMBMS)
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.embms_info.embms_coverage_status = ss_info_ptr->embms_info.embms_coverage_status;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.embms_info.trace_id = ss_info_ptr->embms_info.trace_id;
    #endif
    #if !defined(FEATURE_MMODE_DISABLE_CSG)
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.csg_info.csg_id  = ss_info_ptr->csg_info.csg_id ;
    log_ptr->stack_info[index_val].rat_mode_info.lte_mode_info.csg_info.hybrid_cell = ss_info_ptr->csg_info.hybrid_cell;
    #else
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.csg_id = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
    log_ptr->stack_info[index_val].rat_mode_info.gsm_mode_info.csg_info.hybrid_cell = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
    #endif
  ...
  #endif
  #ifdef FEATURE_MMODE_GW
  log_ptr->stack_info[index_val].cipher_domain = ss_info_ptr->cipher_domain;
  #endif
  ...
  log_ptr->stack_info[index_val].rssi2                           = ss_info_ptr->rssi2;
  #ifdef FEATURE_WCDMA
  log_ptr->stack_info[index_val].sir                             = ss_info_ptr->sir;
  log_ptr->stack_info[index_val].pathloss                        = ss_info_ptr->pathloss;
  log_ptr->stack_info[index_val].rscp                            = ss_info_ptr->rscp;
  log_ptr->stack_info[index_val].ecio_for_display                = ss_info_ptr->ecio_for_display;
  log_ptr->stack_info[index_val].ecio            = ss_info_ptr->ecio;
  #endif
  log_ptr->stack_info[index_val].cell_info.cell_id = ss_info_ptr->cell_info.cell_id;
  ...
  #ifdef FEATURE_MMODE_GW
  log_ptr->stack_info[index_val].cell_srv_ind.hs_ind     = ss_info_ptr->cell_srv_ind.hs_ind;
  log_ptr->stack_info[index_val].cell_srv_ind.dtm_supp   = ss_info_ptr->cell_srv_ind.dtm_supp;
  log_ptr->stack_info[index_val].cell_srv_ind.egprs_supp = ss_info_ptr->cell_srv_ind.egprs_supp;
  ...
  log_ptr->stack_info[index_val].bcmcs_srv_status = ss_info_ptr->bcmcs_srv_status;
  #endif
  ...
  log_ptr->stack_info[index_val].active_band               = ss_info_ptr->active_band;
  #ifdef FEATURE_MMODE_3GPP2
  log_ptr->stack_info[index_val].active_channel            = ss_info_ptr->active_channel;
  #endif
  #if defined(FEATURE_HDR)
  log_ptr->stack_info[index_val].rat_mode_info.hdr_mode_info.hdr_ln_status = ss_info_ptr->rat_mode_info.hdr_mode_info.hdr_ln_status;
  #endif
  ...
  #endif
  log_ptr->changed_fields_subs = ss_info_ptr->changed_fields ;
...
)
{
  ...
  switch ( ss_event )
  {
    ...
    #if defined(FEATURE_CIOT)
        if ( diag_client.ciot_lte_mode != ss_info_ptr->stack_info[cmss_map_ss_to_stack_info(CM_SS_MAIN)].ciot_lte_mode)
        {
          value = (int)(diag_client.ciot_lte_mode = ss_info_ptr->stack_info[cmss_map_ss_to_stack_info(CM_SS_MAIN)].ciot_lte_mode);
          event_report_payload( EVENT_CM_SYSTEM_MODE, sizeof(value), &value );
        }
    #endif
  ...
  */
  ...
  for(index_val = 0 ; index_val< CM_NO_STACKS ; index_val++)
  {
  log_ptr->stack_info[index_val].changed_fields         = ss_info_ptr->changed_fields;
...
)
{
  ...
  */
  ...
  #else
  log_ptr->is_colloc              = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->psist                  = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  #else
  log_ptr->hdr_hybrid             = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_srv_status         = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_roam_status        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_rssi               = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_ecio               = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_io                 = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_sinr               = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_active_prot        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  #else
  log_ptr->prev_idle_digital_mode = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->cur_idle_digital_mode  = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  switch ( ss_info_ptr->sys_mode )
  {
    ...
    case SYS_SYS_MODE_TDS:
      ...
      /* Copy GW info here
      */
      log_ptr->mode_info.gw_info.bit_err_rate   = ss_info_ptr->bit_err_rate;
      log_ptr->mode_info.gw_info.sim_state      = ss_info_ptr->sim_state;
      log_ptr->mode_info.gw_info.plmn_forbidden = ss_info_ptr->plmn_forbidden;
  ...
  #else
  log_ptr->cipher_domain = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  #else
  log_ptr->main_thr_status.ss_thr_status = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->main_thr_status.thr_uptime    = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->main_thr_status.idx           = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->main_thr_status.call_status   = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  #else
  log_ptr->hdr_thr_status.ss_thr_status  = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_thr_status.thr_uptime     = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_thr_status.idx            = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_thr_status.call_status    = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  
  log_ptr->hdr_sys_id.id_type = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hdr_active_prot           = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hybrid_active_band        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->hybrid_active_channel     = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;

  log_ptr->hdr_ln_status = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;    
  #endif
  ...
  #else
  log_ptr->pathloss                        = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->sir                             = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->ecio_for_display                = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->gw_ecio            = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  log_ptr->cell_srv_ind.hs_call_status
                                 = ss_info_ptr->cell_srv_ind.hs_call_status;
  #else
  log_ptr->cell_srv_ind.hs_ind     = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->cell_srv_ind.dtm_supp   = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->cell_srv_ind.egprs_supp = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  log_ptr->cell_srv_ind.hs_call_status
                                 = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  ...
  log_ptr->plus_dial_setting = ss_info_ptr->plus_dial_setting;
  #else
  log_ptr->plus_dial_setting = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
  
  #if defined(FEATURE_CDMA) || defined(FEATURE_HDR)
  log_ptr->def_roam_ind      = ss_info_ptr->def_roam_ind;
  #else
  log_ptr->def_roam_ind      = CM_LOG_PACKET_CONTENTS_NOT_DEFINED;
  #endif
...
)
{
  ...
  #endif
  {
    ...
    #ifdef FEATURE_MMODE_3GPP2 
    srv_info[93] = (uint8)srv_info_ptr->plmn_reg_type;  /* only for camp mode */
    #endif
            ...
            )
{
  ...
  byte        num_chars = 0;
  #ifdef FEATURE_MMODE_3GPP
  byte        uss_ascii_text[200]; /* Holds USS data converted to ascii */
  byte        *log_ptr = NULL;
  int         log_size;
  #endif
    ...
    case CM_SUPS_EVENT_USS_IND:
      {
                                                        ...
                                                        DEFAULT_ALPHABET)
        {
          ...
          */
          ...
          num_chars = cm_util_gsm_alphabet_to_ascii_internal( uss_ascii_text,sizeof(uss_ascii_text),
...
void cmlog_init (void)
{
  ...
  event_reg.bits_1_64 = CM_PH_EVENT_ALL;
  ...
  event_reg.bits_1_64 = CM_SS_EVENT_ALL;
  ...
  bit_mask_256_clear_all(&event_reg);
  
  for(loop=0;loop<CM_CALL_EVENT_MAX;loop++)
  {
    bit_mask_256_set_bit(&event_reg,loop);
  }
