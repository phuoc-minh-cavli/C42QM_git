...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmbcmcs.c#1 $
...
FUNCTION cmbcmcs_mbms_info_get

DESCRIPTION

  Copy MBMS flow info into a specified BCMCS event info buffer.
...
===========================================================================*/
/*lint --e{528} suppress cmbcmcs_mbms_info_get not referenced
** warning
*/
static boolean cmbcmcs_mbms_info_get(
...
/*lint -esym(715, bcmcs_info_ptr) not referenced */
{
  ...
  */
  ...
  cmbcmcs_s_type  *bcmcs_ptr = cmbcmcs_ptr();
    /* Point at BCMCS object
    */
  sys_bcmcs_flow_addr_s_type *flow_address_ptr = NULL;
  sys_bcmcs_flow_addr_s_type *flow_addr_ptr    = NULL;
  ...
  */
  ...
  {
    /* Copy MBMS specific info
    */
    bcmcs_info_ptr->num_of_flows  = (uint8)bcmcs_ptr->mbms_flow.num_flows;

    if (cmss_ptr()->info.sys_mode != SYS_SYS_MODE_NO_SRV)
    {
      bcmcs_info_ptr->protocol = cmss_ptr()->info.sys_mode;
    }

    for (flows=0; flows < bcmcs_info_ptr->num_of_flows; ++flows)
    {
      flow_address_ptr = &(bcmcs_info_ptr->flow_status_array[flows].flow_address);
      flow_addr_ptr    = &(bcmcs_ptr->mbms_flow.flow_addr[flows]);

      *flow_address_ptr = *flow_addr_ptr;
    }
   }
...
/*lint -esym(529, bcmcs_ptr) not referenced */
{
  ...
  */
  ...
  #endif
  #if (defined (FEATURE_MBMS))
  (void) cmbcmcs_mbms_info_get( bcmcs_info_ptr);
  #endif
...
)
{
  ...
  uint8                 num_of_flows   =  0;
  uint8                 max_bcmcs_flows = cmutil_get_sys_bcmcs_max_flows();
  hdrcp_msg_req_u       *hdrcp_req;


  CM_ASSERT( bcmcs_cmd_ptr != NULL );

  num_of_flows = MIN(bcmcs_cmd_ptr->info.reg_req.num_of_flows,
                     max_bcmcs_flows);

  hdrcp_req = (hdrcp_msg_req_u*) cm_mem_malloc(sizeof(hdrcp_msg_req_u));

  CM_MSG_HIGH_1("send HDR_CP_BCMCS cmd=%d", bcmcs_cmd_ptr->cmd);

  switch( bcmcs_cmd_ptr->cmd )
  {
    case CM_BCMCS_CMD_REG_REQUEST_F:
      hdrcp_req->bcmcs_send_reg.num_of_flows = num_of_flows;
      memscpy(hdrcp_req->bcmcs_send_reg.flow_addrs,
              num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type),
              bcmcs_cmd_ptr->info.reg_req.flow_addr_array,
              num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type));

      (void) cm_msgr_send( HDR_CP_BCMCS_SEND_REGISTRATION_REQ, MSGR_MM_CM, &(hdrcp_req->hdr), sizeof(hdrcp_msg_req_u) );
      break;

    #ifdef FEATURE_HDR_BCMCS_DYNAMIC_MODEL
    case CM_BCMCS_CMD_REG_HANDOFF_F:
      hdrcp_req->bcmcs_reg_handoff.num_of_flows = num_of_flows;
      memscpy(hdrcp_req->bcmcs_reg_handoff.flow_addrs,
              num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type),
              bcmcs_cmd_ptr->info.reg_req.flow_addr_array,
              num_of_flows * sizeof(sys_bcmcs_flow_addr_s_type));

      (void) cm_msgr_send( HDR_CP_BCMCS_ENABLE_REG_HANDOFF_REQ, MSGR_MM_CM, &(hdrcp_req->hdr), sizeof(hdrcp_msg_req_u) );
      break;

    case CM_BCMCS_CMD_BOM_CACHING_SETUP_F:
      hdrcp_req->bcmcs_bom_caching.BOM_caching_setup =
          bcmcs_cmd_ptr->info.bom_cache_setup.enable_strict_caching;
      hdrcp_req->bcmcs_bom_caching.stale_BOM_factor =
          bcmcs_cmd_ptr->info.bom_cache_setup.stale_BOM_factor;

      (void) cm_msgr_send( HDR_CP_BCMCS_BOM_CACHING_SETUP_REQ, MSGR_MM_CM, &(hdrcp_req->hdr), sizeof(hdrcp_msg_req_u) );
      break;
    #endif
  ...
  }//switch( BCMCS_CMD_PTR(cmd_ptr)->cmd )
...
#if defined (FEATURE_MBMS)
/*===========================================================================
MACRO         CMMBMS_CONV_SDP_TMGI_TO_NAS_TMGI

DESCRIPTION   Given the TMGI from SDP, converts it to TMGI in NAS format.

PARAMETERS    tmgi_sdp -- TMGI from SDP. TMGI is character array of digits.
              tmgi_nas -- TMGI in NAS format. TMGI has service_id and plmn_id
                          fields.

DEPENDENCIES  Do a memset on NAS TMGI structure before calling the function.
...
static boolean cmmbms_conv_sdp_tmgi_to_nas_tmgi(

    sys_mbms_tmgi_s_type *tmgi_sdp,

    sys_mbms_tmgi_uint_old_s_type *tmgi_uint
)
...
{
  uint64 tmgi;
         /* Variable to hold 15 digits TMGI*/

  uint8  local_tmgi[SYS_MAX_MBMS_TMGI_LENGTH];
         /* SYS_MAX_MBMS_TMGI_LENGTH is 16*/

  boolean ret_val = FALSE;

  if(tmgi_sdp->present == FALSE ||
     tmgi_sdp->length >= sizeof(tmgi_sdp->value)||
     tmgi_sdp->length >= sizeof(local_tmgi)
    )
  {
    CM_MSG_HIGH_0("MBMS: Invalid TMGI passed from SDP\n");
  }
  else
  {

   memscpy( local_tmgi, sizeof( local_tmgi ), tmgi_sdp->value, tmgi_sdp->length );

   /* Need to feed the array to atoll so ending it with NULL*/
   local_tmgi[tmgi_sdp->length ] = NULL ;

   /* Convert local_tmgi string to integer */
   tmgi  = strtoul((char *)local_tmgi, NULL, 0);

   CM_MSG_HIGH_1("MBMS: TMGI received from SDP: %llu",tmgi);

   if(tmgi > 0xFFFFFF)
   {
     tmgi_uint->plmn_id_present = TRUE;
     tmgi_uint->plmn_id = (uint32)(tmgi & 0xFFFFFF);
     tmgi_uint->service_id = (uint32)(tmgi >> 24);
   }
   else
   {
     tmgi_uint->plmn_id_present = FALSE;
     tmgi_uint->service_id = (uint32)tmgi;
   }
   ret_val = TRUE;
  }
...

{
  ...
  */
  ...
  switch (bcmcs_ptr->bcmcs_cmd.cmd)
  {
    case CM_BCMCS_CMD_FLOW_REQUEST_F:
      /* Copy cmd name */
      msm_cmd_ptr->cmd_hdr.cmd_id = MBMS_SERVICE_ACTIVATED_REQ;

      /* Copy num flows */
      msm_cmd_ptr->cmd.cm_mbms_service_act_req.num_mbms_flows =
      bcmcs_ptr->bcmcs_cmd.info.flow_req.num_of_flows;


      /* Loop through individ flows and copy them */
      for (flows=0;
           flows<msm_cmd_ptr->cmd.cm_mbms_service_act_req.num_mbms_flows;
           ++flows
          )
      {
        ret_val = cmmbms_conv_sdp_tmgi_to_nas_tmgi (
                    &bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi,
                    &bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi_conv);

        CM_MSG_HIGH_4 ("ret_val = %d, old tmgi length = %d, val = %d %d", ret_val,
                     bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi.length,
                     bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi.value[0],
                     bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi.value[1]);


        CM_MSG_HIGH_3 ("NEW tmgi service id = %d, plmn id present = %d plmn id = %d",
                      bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi_conv.service_id,
                      bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi_conv.plmn_id_present,
                      bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows].mbms_srv_info.tmgi_conv.plmn_id);

        msm_cmd_ptr->cmd.cm_mbms_service_act_req.mbms_flow_info[flows] =
        bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows];
      }

      /* Send the cmd to MBMS MGR */
    ...
    case CM_BCMCS_CMD_FLOW_DEACT_REQUEST_F:
      ...
      /* Copy cmd name */
      msm_cmd_ptr->cmd_hdr.cmd_id = MBMS_SERVICE_DEACTIVATED_REQ;

      /* Copy num flows */
      msm_cmd_ptr->cmd.cm_mbms_service_deact_req.num_mbms_flows =
      bcmcs_ptr->bcmcs_cmd.info.flow_req.num_of_flows;

      /* Loop through individ flows and copy them */
      for (flows=0;
           flows<msm_cmd_ptr->cmd.cm_mbms_service_deact_req.num_mbms_flows;
           ++flows
          )
      {
        ret_val = cmmbms_conv_sdp_tmgi_to_nas_tmgi (
        ...
        msm_cmd_ptr->cmd.cm_mbms_service_deact_req.mbms_flow_info[flows] =
        bcmcs_ptr->bcmcs_cmd.info.flow_req.flow_addr_array[flows];
      }
    ...
    default:
      CM_ERR_1 ("Unhandled BCMCS request %d",
               bcmcs_ptr->bcmcs_cmd.cmd);
      break;
...
FUNCTION cmbcmcs_refresh_mbms_flow_addr

DESCRIPTION

  Allocate and initialize memory for MBMS flow address.
...
===========================================================================*/
void cmbcmcs_refresh_mbms_flow_addr( int num_of_flows )
{
  ...
  CM_MSG_HIGH_1("Allocating %d flow_addr",num_of_flows);

  if(bcmcs_ptr->mbms_flow.flow_addr != NULL)
  {
    CM_ERR_0("Resetting flow addresses");
    cm_mem_free(bcmcs_ptr->mbms_flow.flow_addr);
    bcmcs_ptr->mbms_flow.flow_addr = NULL;
  }

  bcmcs_ptr->mbms_flow.flow_addr = (sys_bcmcs_flow_addr_s_type)
                      cm_mem_malloc(num_of_flows*sizeof(sys_bcmcs_flow_addr_s_type));
  ...
  switch( cm_rpt_ptr->hdr.cmd )
  {
    ...
    */
    case CM_RPT_MBMS_ACT_CNF:
      ...
      /* Copy MBMS report
      */
      bcmcs_ptr->mbms_flow.num_flows =
                           (int)cm_rpt_ptr->cmd.mbms_srv_act_cnf.num_mbms_flows;

	  CM_MSG_HIGH_1 ("MBMS CM_RPT_MBMS_ACT_CNF, flows %d",
                   bcmcs_ptr->mbms_flow.num_flows);

      if (bcmcs_ptr->mbms_flow.num_flows > SYS_MAX_MBMS_SERVICES )
      {

        bcmcs_ptr->mbms_flow.num_flows = SYS_MAX_MBMS_SERVICES;
      }

      /* Copy all the flows
      */
      cmbcmcs_refresh_mbms_flow_addr(bcmcs_ptr->mbms_flow.num_flows);

      for (flow=0; flow<bcmcs_ptr->mbms_flow.num_flows; ++flow)
      {

        bcmcs_ptr->mbms_flow.flow_addr[flow] =
                   cm_rpt_ptr->cmd.mbms_srv_act_cnf.mbms_flow_info[flow];

        CM_MSG_HIGH_1 ("MBMS CM_RPT_MBMS_ACT_CNF, port %d",
                   bcmcs_ptr->mbms_flow.flow_addr[flow].port_number);
      }
    ...
    case CM_RPT_MBMS_DEACT_CNF:
      ...
      */
      bcmcs_ptr->mbms_flow.num_flows =
                      (int)cm_rpt_ptr->cmd.mbms_srv_deact_cnf.num_mbms_flows;

      CM_MSG_HIGH_1 ("MBMS CM_RPT_MBMS_DEACT_CNF, flows %d",
                   bcmcs_ptr->mbms_flow.num_flows);
      ...
      */
      ...
      for (flow=0; flow<bcmcs_ptr->mbms_flow.num_flows; ++flow)
      {
        ...
        bcmcs_ptr->mbms_flow.flow_addr[flow] =
                   cm_rpt_ptr->cmd.mbms_srv_deact_cnf.mbms_flow_info[flow];

        CM_MSG_HIGH_1 ("MBMS CM_RPT_MBMS_DEACT_CNF, port %d",
                   bcmcs_ptr->mbms_flow.flow_addr[flow].port_number);
      }

      /* Move BCMCS into CM_BCMCS_STATE_REQUESTING state
      */
      cmbcmcs_event (CM_BCMCS_EVENT_FLOW_DEACT_REQUEST_SUCCESS);
