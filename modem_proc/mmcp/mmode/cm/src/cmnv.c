...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmnv.c#1 $
...
)
{
   static const struct {
     ...
     {"/nv/item_files/modem/mmode/rpm_info" , CMNV_EFS_ID_RPM_INFO},
     {"/nv/item_files/data/3gpp/rpm_suppported_sim" , CMNV_EFS_ID_RPM_MCC_MNC_LIST},
     {"/nv/item_files/modem/mmode/tds_cmrpm_info", CMNV_EFS_ID_TDS_RPM_INFO},
     {"/nv/item_files/modem/mmode/mode_disabled_in_TSTS", CMNV_EFS_ID_MODE_DISABLED_IN_TSTS},
     {"/nv/item_files/modem/mmode/rpm_sys_time", CMNV_EFS_ID_RPM_SYS_TIME},
     {"/nv/item_files/modem/mmode/mtcsfb_call_redial", CMNV_EFS_ID_MTCSFB_CALL_REDIAL_OVER_1X},
  };
...
)
{
  ...
  */
  if( cmnv_cmd_sync.status != NV_DONE_S )
  {
    CM_MSG_HIGH_2( "NV read failed, item=%d, stat=%d",
             cmnv_cmd_sync.item, cmnv_cmd_sync.status);
    ...
    switch( cmnv_cmd_sync.item )
    {
     ...
     /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      case NV_SERVICE_DOMAIN_PREF_I:
        ...
        data_ptr->service_domain_pref.srv_domain = CM_SRV_DOMAIN_PREF_CS_PS;
        break;
     ...
     case NV_GSM_AMR_CALL_CONFIG_I:
        data_ptr->gsm_amr_call_config = TRUE;
        break;
     ...
     /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
      ...
      case NV_UMTS_CALL_VT_CODEC_LIST_I:
        data_ptr->umts_call_vt_codec_list = TRUE;
        break;
