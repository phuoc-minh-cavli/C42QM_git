...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmtask.c#1 $
...
static ULogHandle cm_ulog_handle = NULL;
...
extern void qmi_mmode_init_before_task_start(void);
...
/*===========================================================================
FUNCTION CALL_MANAGER_TASK_HANDLE_SIGS
...
===========================================================================*/
void call_manager_task_handle_sigs(void);
...
)
{
  ...
  /* 
  ** If FEATURE CLADE HEAP is defined dynamically allocate through clade heap alloc
  */
  tmp = (cm_cmd_type*)cm_mem_malloc_ext(size);
...
void cm_cmd_dealloc( void  *ptr )
{
  ...
  */
  ...
  if(tmp != NULL)
  {
    ...
    if(tmp->cmd_type != CM_CMD_TYPE_CLIENT)
    {
  ...
  /*
  ** If FEATURE CLADE HEAP is defined dynamically deallocate  through clade heap free
  */
  cm_mem_free_ext(ptr);
...
void *cm_mem_malloc_debug(
  ...
  const char *file,
    /* File name containing this allocation request */
...
FUNCTION cm_mem_alloc_ext

DESCRIPTION
  Allocate a CM command buffer from cm_mem_malloc_debug when FEATURE CLADE HEAP is not defined
  Allocate a CM command buffer from clade_heap_alloc when FEATURE CLADE HEAP is  defined
...
void* cm_mem_malloc_debug_ext(const char *file,uint32 line,size_t size,char *objname)
{
#ifndef FEATURE_CLADE_HEAP
  return cm_mem_malloc_debug(file,line,size,objname);
#else
  SYS_ARG_NOT_USED(objname);
  SYS_ARG_NOT_USED(file);
  SYS_ARG_NOT_USED(line);
  return clade_heap_calloc(1, size);//only size args , no client argument 
...
FUNCTION cm_mem_dealloc_ext

DESCRIPTION
  Deallocate a CM command buffer from cm_mem_malloc_debug when FEATURE CLADE HEAP is not defined
  Deallocate a CM command buffer from clade_heap_alloc when FEATURE CLADE HEAP is  defined
...
void cm_mem_free_debug_ext(void* ptr,char *objname,const char *funcName, int lineNumber )
{
#ifndef FEATURE_CLADE_HEAP
  cm_mem_free_debug(ptr,objname,funcName,lineNumber);
#else
  SYS_ARG_NOT_USED(objname);
  SYS_ARG_NOT_USED(funcName);
  SYS_ARG_NOT_USED(lineNumber);
  clade_heap_free(ptr); //only pointer to be freed is passed , no client info
#endif
}
...
void cm_cmd_queue( cm_cmd_type *cmd_ptr )
{
  ...
  switch( CMD_TYPE(cmd_ptr) )
  {
    ...
    #endif /* (FEATURE_MMODE_GW) || (FEATURE_IP_CALL) */
    #if defined(FEATURE_MMODE_GW) && defined(FEATURE_WMS_CM_NAS_INTERFACES)
    #error code not present
#endif /* FEATURE_MMODE_GW || FEATURE_WMS_CM_NAS_INTERFACES */
...
)
{
  ...
  #ifdef FEATURE_CIOT
  orig_ptr->ciot_lte_pref.lte_op_mode_pref = CM_LTE_IOT_OP_MODE_PREF_NONE;
  SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&orig_ptr->ciot_lte_pref.lte_band_pref);
  SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&orig_ptr->ciot_lte_pref.lte_m1_band_pref);
  SYS_LTE_BAND_MASK_CLR_ALL_BANDS(&orig_ptr->ciot_lte_pref.lte_nb1_band_pref);
  #endif
...
)
{
  ...
  */
  ...
  CM_MSG_LOW_2( "cm_ph_orig_q_put(), q_add=%p, count=%d",q_ptr, q_cnt(q_ptr));
...
)
{
  ...
  if( q_orig_ptr != NULL && q_orig_ptr->orig !=NULL && q_orig_ptr->orig->act_id == *compare_ptr )
  {
    return( TRUE );
  }
...
)
{
  ...
  switch(ss)
  {
    ...
    default:
      ...
      **     CM_ERR exits when CM_DEBUG is on
      */
      CM_MSG_LOW_1("cmtask_orig_para_delete() on ss %d", ss);
      /*lint -restore */
    top_ptr_changed = FALSE;
    break;
  }
  ...
  switch(ss)
  {
   ...
   case CM_SS_HYBR_3 :
    ...
    default:
      ...
      **     CM_ERR exits when CM_DEBUG is on
      */
      CM_MSG_LOW_1( "cmtask_orig_para_insert() on ss %d", ss);
      /*lint -restore */
      return FALSE;
  }
 ...
 */
 if ( mode == SD_MODE_GSM && SD_SS_MAIN == ss)
 {
...
void cm_gsm_codec_info_cb(sys_codec_ho_info codec_ho_info)
{
  ...
  CM_MSG_HIGH_0("cm_gsm_codec_info_cb");
  if ( (cmd_ptr = cm_get_cmd_buf()) != NULL )
  {
...
void cm_wcdma_codec_info_cb(sys_codec_ho_info codec_ho_info)
{
  ...
  CM_MSG_HIGH_0("cm_wcdma_codec_info_cb");
  if ( (cmd_ptr = cm_get_cmd_buf()) != NULL )
  {
    cmd_ptr->cmd.codec_info = codec_ho_info;
...
void cm_td_codec_info_cb(sys_codec_ho_info codec_ho_info)
{
  ...
  CM_MSG_HIGH_0("cm_tdscdma_codec_info_cb");
  if ( (cmd_ptr = cm_get_cmd_buf()) != NULL )
  {
    cmd_ptr->cmd.codec_info = codec_ho_info;
...
void cm_full_srv_lost_cb(
  ...
  /* Indicates if GW scan attempted from last acq_fail */
)
{
  ...
  cm_rpt_ptr->full_srv_lost.hdr.cmd  = CM_FULL_SRV_LOST_F;
  cm_rpt_ptr->full_srv_lost.ss       = ss;
  #if defined(FEATURE_CIOT)
  cm_rpt_ptr->full_srv_lost.is_scan_attempted = is_scan_attempted;
  #endif
...
static void cmtask_msgr_init(void)
{
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
               MSGR_ID_REX,MM_CM_RESTORE_CTXT_PSM_REQ);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
               MSGR_ID_REX,MM_CM_RESTORE_BEARER_CTXT_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                 MSGR_ID_REX,MM_CM_ESM_DATA_TRANSPORT_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                 MSGR_ID_REX,MM_CM_ESM_DATA_TRANSPORT_CNF);
  CM_ASSERT(msgr_error == E_SUCCESS);
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                 MSGR_ID_REX,DS_3GPP_MO_EXP_ACTIVITY_STATUS_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_SET_CFG_PARAMS_REQ);
  CM_ASSERT(msgr_error == E_SUCCESS);
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_READY_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_READY_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_ENTER_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_CONTEXT_SAVE_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_AUTO_READY_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                 MSGR_ID_REX,LTE_RRC_TX_STATUS_EXT_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_DEEP_SLEEP_TIMER_EXPIRY_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_PSM_DEEP_SLEEP_EXIT_REQ);
  CM_ASSERT(msgr_error == E_SUCCESS);

#ifdef FEATURE_REF_TRACKER
   msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_SNS_MGR_SNS_MOD_SAMPLE_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);  
  
   msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_SNS_MGR_SNS_QMI_CMD_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);
  ...
  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_SNS_MGR_SNS_MOTION_RSP);
  CM_ASSERT(msgr_error == E_SUCCESS);

  msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_SNS_MGR_SNS_MOTION_CONFIG_REQ);
  CM_ASSERT(msgr_error == E_SUCCESS);

   msgr_error = msgr_register(MSGR_MM_CM,&cm_msgr_client,
                             MSGR_ID_REX,MM_CM_SNS_MGR_REGISTRY_IND);
  CM_ASSERT(msgr_error == E_SUCCESS);
#endif
...
/*===========================================================================
FUNCTION cm_get_ulog_handle

DESCRIPTION
  Returns ULOG Handle for CM task
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle cm_get_ulog_handle()
{
  return cm_ulog_handle;
}

/*===========================================================================
FUNCTION cm_ulog_register

DESCRIPTION
  This function call registers CM Task with ULOG
...
SIDE EFFECTS
  None
===========================================================================*/
void cm_ulog_register(void)
{
  ULogResult cm_buffer_status;
  cm_buffer_status = ULogFront_RealTimeInit(&cm_ulog_handle,
                               "CM_ULOG",  
                               512, 
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  CM_MSG_HIGH_1("ULOG REGISTER cm_buffer_status: %d", cm_buffer_status);
}
...
DESCRIPTION
  Initialize MMODE queues and other objects that needs to be initialized
...
void cm_init_before_task_start( void )
{
  ...
  /******************  SD Module INIT BEFORE TASK START ***********************/
  sd_init_before_app_task_start();
  
  /******************  MMOC Module INIT BEFORE TASK START ***********************/
  mmoc_init_before_task_start();
  
  /******************  CM Module INIT BEFORE TASK START ***********************/
  call_manager_init_before_task_start();

  /******************  QMI Module INIT BEFORE TASK START ***********************/
...
#error code not present
  ...
  */
  ...
  {
    ULOG_RT_PRINTF_0(cm_get_ulog_handle(), "=CM= CM task init done");
  }
...
===========================================================================*/
#ifdef FEATURE_MODEM_CONFIG_REFRESH
static void cm_nv_refresh( sys_modem_as_id_e_type   asubs_id )
{
  ...
  */
  ...
  #ifdef FEATURE_MMODE_3GPP && (defined(FEATURE_MMODE_SS_SUPS) || defined (FEATURE_MMODE_USS_SUPS))
  /* Initialize the CM SUPS object.
  */
...
)
{
  ...
  switch(gen_rpt_ptr->hdr.cmd)
  {
    ...
    case CM_MCFG_REFRESH_RPT:
      cm_nv_refresh(gen_rpt_ptr->generic_rpt.refresh_info.as_id);
      CM_MSG_HIGH_1("MCFG:refresh complete %d",gen_rpt_ptr->generic_rpt.refresh_info.reference_id);
      mcfg_refresh_done(gen_rpt_ptr->generic_rpt.refresh_info.reference_id);
      break;
    #endif
    default:
      break;
  ...
  */
  if ( rpt_buf != NULL )
  {
      ...
      cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.ip_gen_param != NULL)
    {
      ...
      /* For the History Info reference object in mt_invite, CM needs to wait till sending
       * CM_CALL_EVENT_INCOM (CM_IP_CALL_IND_MT_RING_CNF) to QMI-Voice to release the object.
       * CM will release it after sending CM_CALL_EVENT_INCOM.
       */
    }
    ...
    else if(cmipapp_rpt_ptr->hdr.cmd == CM_IP_CALL_IND_CONNECTED &&
      cmipapp_rpt_ptr->ip_ind.ind.connected.call_attrib.history_info != NULL)
    {
      REF_CNT_OBJ_RELEASE_IF(cmipapp_rpt_ptr->ip_ind.ind.connected.call_attrib.history_info);
    }
    else if( (cmipapp_rpt_ptr->hdr.cmd == CM_IP_CALL_IND_MO_PROGR_INFO) &&
             (  (cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.progr == CMIPAPP_PROG_SESSION) ||
                (cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.progr == CMIPAPP_PROG_MO_RINGBACK) ) &&      
             (cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.history_info != NULL) )
    /* History Info is only applicable for CMIPAPP_PROG_SESSION (PROGRESS_EARLY_MEDIA) or
     * CMIPAPP_PROG_MO_RINGBACK (PROGRESS_INFO_IND).
     */
    {
      REF_CNT_OBJ_RELEASE_IF(cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.history_info);
    }
...
static void cm_wrpt_handle( cm_hdr_type       *rpt_ptr )
{
  ...
  */
  ...
  #if !defined(FEATURE_MMODE_DISABLE_LTE_VOICE) && !defined(FEATURE_MMODE_LTE_ONLY)
  ...
  cmbcmcs_gw_rpt_proc(rpt_ptr);
  cmmbms_rpt_proc(rpt_ptr);
  #endif
...
void cm_msgr_handle( void )
{
  ...
  while ((rpt_ptr = q_get(&cm_msgr_q)) != NULL)
  {
    ...
    else if(CM_MSGR_MO_EXCEPTION_IND( rpt_ptr->cmd.mo_ex_ind.hdr.id) )
    {
        CM_MSG_HIGH_0("MSGR RXD: MO_EXCEPTION_IND");
        cmph_mo_exception_ind_proc(rpt_ptr->cmd.mo_ex_ind.mo_excp_status_ind);
    }
    ...
    else if (CM_MSGR_MSG_CPHY_RSSI(rpt_ptr->cmd.lte_sys.lte_rssi_ind.msgr_hdr.id))
    {
      ...
      else
      {
        CM_MSG_LOW_0("LTE_CPHY_RSSI_IND with invalid values. Skip.");
             ...
             (CM_MSGR_MSG_ESM_CIOT_RPT(rpt_ptr->cmd.esm.msg_hdr.id))
            )
    {
    ...
    else if (CM_MSGR_MSG_CPHY_TX_STATUS(rpt_ptr->cmd.lte_sys.lte_rrc_tx_ext_ind.msg_hdr.id))
    {
      CM_MSG_HIGH_0("MSGR RXD: LTE_RRC_TX_STATUS_EXT_IND");
      cmss_msgr_ml1_tx_stats_proc((void *)(&(rpt_ptr->cmd.lte_sys)));
    }
    ...
    else if(rpt_ptr->cmd.client_cmd.msg_hdr.id == MM_CM_AUDIO_RELEASE_IND)
    {
      ...
      cmipcall_process_audio_release();
    }
    #endif
    else if (rpt_ptr->cmd.subsc_chgnd_rsp.msg_hdr.id == MM_CM_SUBSC_CHGND_RSP)
    {
      CM_MSG_HIGH_1("MSGR RXD: MM_CM_SUBSC_CHGND_RSP on sub %d",rpt_ptr->cmd.subsc_chgnd_rsp.as_id);
	...
	{
      CM_MSG_HIGH_2("MSGR RXD: Received VOLTE_STATUS_NOTIFICATION_CMD as_id %d, volte_mode_enable %d",
	  	rpt_ptr->cmd.volte_status_cmd.as_id, rpt_ptr->cmd.volte_status_cmd.volte_mode_enable);
...
#ifdef FEATURE_REF_TRACKER
    else if(CM_MSGR_MSG_SNS_MGR(rpt_ptr->cmd.sns_sample_data.msg_hdr.id) )
    {
      cmsns_msgr_cmd_proc(&(rpt_ptr->cmd.sns_sample_data));
    }
#endif
...
#if defined(FEATURE_IP_CALL) && (defined(FEATURE_MMODE_SS_SUPS) || defined (FEATURE_MMODE_USS_PS_SUPS))
...
void cm_rpt_handle( void )
{
  ...
  */
  while( (rpt_ptr = (cm_hdr_type *) cm_rpt_q_get()) != NULL )
  {
    ...
    {
      CM_MSG_HIGH_1("RPT RXD: SD cmd=%d", rpt_ptr->cmd);
    ...
    {
      CM_MSG_HIGH_1("RPT RXD: 1X cmd=%d", rpt_ptr->cmd);
    ...
    {
      CM_MSG_HIGH_1("RPT RXD: GW cmd=%d", rpt_ptr->cmd);
    ...
    {
      CM_MSG_HIGH_1("RPT RXD: GENERIC cmd=%d", rpt_ptr->cmd);
  ...
  while( (cmd_ptr = cm_client_cmd_q_peek()) != NULL )
  {
    ...
    */
    switch ( CMD_TYPE(cmd_ptr) )
    {
      ...
      case CM_CMD_TYPE_MBMS:
      #if defined (FEATURE_MBMS)
        cmmbms_client_cmd_proc( MBMS_CMD_PTR(cmd_ptr) );
      #endif
      break;
...
#ifdef FEATURE_REF_TRACKER
     case CM_CMD_TYPE_SNS:
        cmsns_client_cmd_proc(SNS_CMD_PTR(cmd_ptr));
	    break;
#endif
      /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
        ...
        */
        {
          ...
          #endif
          #if defined(FEATURE_MMODE_HYBR3_ENABLED)
        ...
        if(PH_CMD_PTR(hold_cmd_ptr)->info.hold_cmd_reason == CM_HOLD_CMD_REAS_ADV_SEARCH_VOICE_CALL &&
           cmcall_find_active_voice_call() != CM_CALL_ID_INVALID )
        {
          CM_MSG_HIGH_1("Voice call did not end, so don't process GPS ADV Search cmd %d",PH_CMD_PTR(hold_cmd_ptr)->cmd);
          hold_cmd_ptr = next_cmd_ptr;
          continue;
        }
...
===========================================================================*/
static void cmtask_handle_sigs(rex_sigs_type sigs)
{
...
)
{
  ...
  /*GPIO Toggling for power measurment breakdown at RC Init Complete*/
  MCPM_GPIO_TOGGLE(RCM_TRIGGER_1)

  /** Handle combined signals for all tasks */
  call_manager_task_handle_sigs();
}
...
void call_manager_task_handle_sigs(void)
{
  ...
  static rex_sigs_type  cmtask_sig_mask;
  static rex_sigs_type  mmoc_sig_mask;
  ...
  {
    sigs = rex_wait(cmtask_sig_mask | mmoc_sig_mask | qmi_mmodei_enabled_sig_mask);

   (void) rex_clr_sigs( cm_tcb_ptr, sigs );
   
    mmoc_handle_sigs(sigs);
    
    cmtask_handle_sigs(sigs);
