...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmdbg.c#1 $
...
static const char* cm_sns_cmd_e_type_string_map[CM_SNS_CMD_MAX] = {
...
static const char* cm_call_event_e_type_string_map[ CM_CALL_EVENT_MAX ] = {
        ...
        "CM_CALL_EVENT_LTE_DATA_TRASPORT_CNF",
        "CM_CALL_EVENT_LTE_DATA_TRASPORT_IND"
};
...
static const char* cm_name_generic_type_string_map [(CM_RPT_GENERIC_END - CM_RPT_GENERIC_BEGIN) - 1] = /*lint !e641 !e656 */
{
  ...
  */
  ...
  "CM_DATA_PSM_READY_RSP",
  "CM_DATA_PSM_READY_IND",
  "CM_DATA_PSM_ENTER_RSP",
  "CM_PSM_DEEP_SLEEP_TIMER_EXPIRY_IND"
};
...
/*lint -restore */
{
  ...
  "CM_RESTORE_CTXT_PSM_REQ",
  "CM_RESTORE_PDP_CTXT_RSP",
   ...
   void                  *buff)
{
  cm_ph_cmd_s_type*   ph_cmd_ptr = ((cm_ph_cmd_s_type*)buff);
      ...
      #else
      pref_chg_buff->csg_id                 = SYS_CSG_ID_INVALID;
      #endif
        ...
        switch((cm_name_type)event_type)
        {
          case CM_NETWORK_LIST_CNF:
          {
            ...
            net_list_cnf_buff->status     = ((cm_network_list_cnf_s_type*)buff)->status;
              net_list_cnf_buff->length     = ((cm_network_list_cnf_s_type*)buff)->available_plmn_list.plmn_list.length;
          ...
          {
            CMDBG_MEM_ALLOC(ss_buff, cm_rpt_full_srv_lost_entry_s_type)
  
            ss_buff->ss                = ((cm_full_srv_lost_type*)buff)->ss;
            #if defined(FEATURE_CIOT)
            ss_buff->is_scan_attempted  = ((cm_full_srv_lost_type*)buff)->is_scan_attempted;
            #endif
          ...
          case CM_MMGSDI_CARD_RPT:
          {
            ...
            switch(((mmgsdi_event_data_type*)buff)->evt)
            {
              ...
              {
                card_rpt_buff->activated     = ((mmgsdi_event_data_type*)buff)->data.session_changed.activated;
                cmdbg_add_new_entry(buff_type, event_type , (void*)card_rpt_buff);
                break;
              }
              case MMGSDI_SUBSCRIPTION_READY_EVT:
              {
                card_rpt_buff->app_type  = ((mmgsdi_event_data_type*)buff)->data.subscription_ready.app_info.app_data.app_type;
                cmdbg_add_new_entry(buff_type, event_type , (void*)card_rpt_buff);
                break;
              }
              ...
              {
                card_rpt_buff->stage = ((mmgsdi_event_data_type*)buff)->data.refresh.stage;
                card_rpt_buff->mode = ((mmgsdi_event_data_type*)buff)->data.refresh.mode;    
              }
...
void cmdbg_stringmap_verify()
{
  ...
  #if (defined(CM_GW_SUPPORTED) || ( defined(FEATURE_CM_LTE) && defined (FEATURE_IP_CALL))) \
    && (defined(FEATURE_MMODE_USS_SUPS) || defined(FEATURE_MMODE_SS_SUPS))
  ...
  for( i=0; i<(CM_RPT_1X_END - CM_RPT_1X_BEGIN - 4); i++ )
  {
...
)
{
...
#ifdef FEATURE_MMODE_F3_REDUCTION  	
  ...
  case CM_CMD_TYPE_SNS:
    CMDBG_CMD_MSG( "sns cmd", SNS_CMD_PTR(cmd_ptr)->cmd,
                     CM_SNS_CMD_MAX,
                     cm_sns_cmd_e_type_string_map,
                     SNS_CMD_PTR(cmd_ptr)->client_id);
    break;
#endif
...
)
{
  ...
  */
  ...
  case CM_CMD_TYPE_PH:
    ...
    #else
    CM_MSG_HIGH_1("<<CM phevt=%d",ph_event);
    #endif
  ...
  case CM_CMD_TYPE_SS:
    ...
    #else
    CM_MSG_HIGH_1("<<CM ssevt=%d", ss_event);
    #endif
...
//  case CM_SAVE_DATA_CNF:
  ...
  case CM_SET_EDRX_CFG_RSP:
  case CM_GET_EDRX_PARAMS_RSP:
  case CM_EDRX_PARAMS_CHANGE_IND:
  case CM_RESTORE_CTXT_PSM_REQ:
  case CM_RESTORE_PDP_CTXT_RSP:
    break;
...
)
{
  ...
  */
  ...
  else
  {
    ...
    {
      CM_MSG_HIGH_2( "CM>> LL cmd=%d, call id=%d", mc_cmd_ptr->hdr.cmd, call_id);
    }
    else
    {
...
)
{
  ...
  cmdbg_print_message("- fav mode pref = %d(%s)",
                      ...
                      CMDBG_STR_MAP_LOOKUP(favored_mode_pref,
                                           ...
                                           cm_mode_pref_e_type_string_map));
  #else
  CM_MSG_HIGH_4(" call_type = %d srv_type = %d mode pref = %d fav mode pref = %d", 
  call_type, srv_type, mode_pref, favored_mode_pref );
