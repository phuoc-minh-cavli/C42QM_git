  ...
  switch( call_type )
  {
    ...
    */
    ...
    case CM_CALL_TYPE_STD_OTASP:
      if(cmcall_misc_is_mode_pref(mode_pref, CM_MODE_PREF_CDMA_HDR_ONLY))
      {
        return TRUE;
      }
...

{
  ...
  */
  if( cmph_ptr()->is_svlte_supported &&
     ( cmph_ptr()->ss_susp_info[CM_SS_HYBR_2].is_suspend == FALSE ||
  ...
  */
  if( cmph_ptr()->is_svdo_supported &&
     ( cmph_ptr()->ss_susp_info[CM_SS_HYBR_1].is_suspend == FALSE ||
...
)
{
  ...
  ** if service type is CDMA_HDR or CDMA-SPECIFIC.
  */
  if( call_ptr->srv_type == CM_SRV_TYPE_CDMA_HDR ||
...
)
{
  ...
  else
  {
    for(i=0; i<cahce_rpt_ptr->num_of_calls; i++)
	{
      ...
      cmxcall_fill_srvcc_orig_param(call_ptr, &mc_ptr->origination); 
...
)
{
  ...
  */
  ...
  CM_MSG_HIGH_1("START cmxcall_mc_rpt_proc, cmd=%d",
  ...
  break;
         ...
         )
      {
        ...
        */
        if ( cmcall_is_1x_data_srv_opt(cm_mc_rpt_ptr->incoming_call.so) &&
            ( ph_ptr->is_svdo_supported || ph_ptr->is_svlte_supported )
           )
        {
          ...
          */
          ...
          if ( ( ph_ptr->is_svlte_supported &&
                 ph_ptr->ss_susp_info[CM_SS_HYBR_2].is_suspend == FALSE
               ) ||
    ...
    case CM_INCOMING_PAGE_F:
     ...
     /* If there is VoLTE call existing, 1X call should be rejected
       */
      else if (ph_ptr->is_svlte_supported == TRUE &&
          cmcall_is_there_volte_call_obj(CM_CALL_TYPE_VOICE_MASK |
                                       CM_CALL_TYPE_EMERG_MASK |
                                       CM_CALL_TYPE_VT_MASK) != CM_CALL_ID_INVALID)
      {
      ...
      else if( ph_ptr->is_svdo_supported == FALSE &&
               cmss_is_hybr2_operational() == FALSE)
      {
        /* If  SVDO & SVLTE is not supported, Immediately respond to 1xcp to
        ** proceed with call
        */
        CM_MSG_HIGH_2("SO:Page proceed %d cmss_is_hybr2_operational %d",ph_ptr->is_svdo_supported,
                   cmss_is_hybr2_operational());
        cmxcall_mc_incoming_page_proceed(TRUE);
      }
      ...
      else
      {
        ...
        */
          ...
          && cmph_ptr()->is_tau_needed == FALSE)
        {
          ...
          */
          cmcsfbcall_send_orig_req_for_mt_call (CM_MT_UNALLOCATED_CALL_ID);
