...
void cmrpm_tds_init( void )
{
  ...
  CM_MSG_HIGH_4("RPM params loaded: is_rpm %d, mmreject_reset: short_timer %d, long_timer %d, NV reset counter %d",
  ...
  */
  ...
  {    
    ...
    if((rpm_ptr->mmreject_reset.timer != 0 )&&
      (curr_sys_time >= rpm_ptr->mmreject_reset.timer))
    {
      CM_MSG_HIGH_0("TDS-RPM: Call ended, reset timr expired, reset ue, send LPM, check LPM in cmrpm_tds_timer_proc later");
