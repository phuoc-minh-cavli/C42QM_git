...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmipcall.c#1 $
  ...
  */
  switch( cmipapp_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_IP_CALL_IND_CALL_END:
      ...
      CM_MSG_HIGH_4("IP RXD: CALL_END, end_cause=%d, client_end_cause=%d, call_id=%d, is_unwanted_call %d",
		...
		*/
		...
		if(cmipapp_rpt_ptr->ip_ind.ind.call_end.id == CM_CALL_ID_INVALID)
	    {
	       /*
	            ...
	            */
	       CM_MSG_HIGH_0("Incoming call end reported by IMS to CM when no call_id exist");
      ...
      */
      ...
      /* 1) If emergency call originated over WLAN fails and IMS returns a FALLBACK_TO_CS end reason,
             ...
             */
      #if defined (FEATURE_IP_CALL) && defined (FEATURE_CM_LTE)
      if((cmph_sub_info->wifi_e911_support) &&
        (call_ptr->call_type == CM_CALL_TYPE_EMERGENCY) &&
        (call_ptr->is_vowlan == TRUE) &&
        (ims_end_cause == CMIPAPP_END_CAUSE_FALLBACK_TO_CS))
      {
        ...
        case CM_CALL_STATE_ORIG:
          ...
          */
          ...
          else
          {
            ...
            if(call_ptr->end_status == CM_CALL_END_REL_NORMAL)
            {
               #ifndef FEATURE_MMODE_DISABLE_LTE_VOICE
               cmcall_end(call_ptr);
               #endif
...
SIDE EFFECTS
  None
===========================================================================*/
void cmipcall_process_audio_release(void)
{
  #if defined(FEATURE_IP_CALL) && defined(FEATURE_CM_LTE) && !defined (FEATURE_MMODE_LTE_ONLY) && !defined(FEATURE_MMODE_GPRS_LTE)
  /* audio session released confirmation received from IMS 
          can send the srvcc call cache context to NAS now */
      ...
      for ( i = 0; i < call_cmd_ptr->info.num_end_call_ids; i++ )
      {
        ...
        */
        if (call_ptr->call_type == CM_CALL_TYPE_EMERGENCY && 
            ...
            ph_ptr->voip_cancel_retry_period != 0      ) 
        {
          ph_ptr->voip_cancel_retry_uptime = time_get_uptime_secs() + 
                                    ph_ptr->voip_cancel_retry_period;
  ...
  switch( call_cmd_ptr->cmd )
  {
    ...
    case CM_CALL_CMD_ANSWER:
      ...
      */
      ...
      if ((call_cmd_ptr->info.end_status == CM_CALL_END_INCOM_REJ) || (call_cmd_ptr->info.end_status == CM_CALL_END_IP_UNWANTED_CALL))
      {
...
)
{
  ...
  */
  ...
  {
     // As CM is not sending the call end event to QMI, we will need to send the MM_CM_MT_CALL_END_AUTO_REJECT_IND to QMI.
     cmcall_send_mt_call_end_auto_reject_ind(call_ptr->asubs_id,\
                                             call_ptr->call_type,\
