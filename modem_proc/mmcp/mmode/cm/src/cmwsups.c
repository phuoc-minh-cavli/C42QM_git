...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmwsups.c#1 $
...
/* This is the common code so putting under FEATURE_MMODE_USS_SUPS 
     below is commented not removed for tracking purpose.*/
     /* need to define flag separaterly for CS_PS flag  */
...
static cm_sups_info_s_type* cmwsups_alloc_sups_info_ptr(
...
#if defined(FEATURE_MMODE_USS_CS_SUPS)  
  sups_ptr->sups_svc_domain_pref[0] = SYS_SUPP_SRV_DOMAIN_PREF_CS_ONLY;
  return;
#else
...
{ 
  cm_ss_e_type  ss = cmph_map_subs_to_ss(as_id);
  ...
  *sups_info_pptr = cmsups_get_sups_info_ptr(ss);
  if (*sups_info_pptr == NULL)
  {
    *sups_info_pptr = cmwsups_alloc_sups_info_ptr (ss);
  }
...
#endif
#ifdef FEATURE_DUAL_SIM
  if( cmph_map_subs_to_ss(cmd_info_ptr->cmd_subs_id) == CM_SS_HYBR_2 )
  {
    if (sups_ptr->hybr_gw_sups_info == NULL)
    {
...
)
{
  ...
  #if defined(FEATURE_MMODE_USS_CS_SUPS) || defined(FEATURE_MMODE_SS_SUPS)
  ...
  */
  if((!cmsups_is_wlan_sups_cmd(sups_cmd_ptr->cmd))&&(ph_ptr->oprt_mode != SYS_OPRT_MODE_ONLINE))
  {
   ...
   {
     CM_MSG_HIGH_0("high priority search progress reject SUPS");	  
     return CM_SUPS_CMD_ERR_OTHER; 
   }
#endif
  ...
  {
    #if defined(FEATURE_IP_CALL) && (defined (FEATURE_MMODE_SS_SUPS) || defined(FEATURE_MMODE_USS_SUPS))
    ...
    #endif
    #if (defined FEATURE_MMODE_SS_SUPS || FEATURE_MMODE_USS_CS_SUPS ) && !defined FEATURE_MMODE_DISABLE_LTE_VOICE
    ...
    #endif      
    #if defined (FEATURE_MMODE_SS_SUPS)
     /* If CS route is calculated , reject all Sups request */
    if(cmsups_ptr()->sups_cmd_route == CMSUPS_CMD_ALLOW_CS || cmsups_ptr()->sups_cmd_route == CMSUPS_CMD_REJECT)
    {
      CM_MSG_HIGH_0("ROUTE_FAIL for SUPS CMD. SUPS CMD on CS not allowed");
      return CM_SUPS_CMD_ERR_ROUTE_FAILURE;
    }
    #endif
    #else
    cmsups_ptr()->sups_cmd_route = CM_SUPS_CMD_ERR_ROUTE_FAILURE;
    #endif
  }
  else
  {
    /*  CS specific code for USSD    */
    #if defined (FEATURE_MMODE_USS_CS_SUPS) || defined FEATURE_MMODE_SS_SUPS
    cmsups_ptr()->sups_cmd_route = CMSUPS_CMD_ALLOW_CS;
    #else
    CM_MSG_HIGH_0("ROUTE_FAIL for SUPS CMD. NO LTE Capability");
    return CM_SUPS_CMD_ERR_ROUTE_FAILURE;
    #endif
  }
  CM_MSG_HIGH_2("sups_cmd=%d, route=%d",sups_cmd_ptr->cmd, cmsups_ptr()->sups_cmd_route);

  #if (defined(FEATURE_MMODE_LTE_ONLY) || defined(FEATURE_MMODE_GPRS_LTE))&& !defined(FEATURE_MMODE_USS_CS_SUPS)
  if( cmsups_ptr()->sups_cmd_route == CMSUPS_CMD_ALLOW_CS )
  {
    cmd_err = CM_SUPS_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE;
    CM_MSG_HIGH_0("CM_SUPS_CMD_ERR_FUNCTION_UNAVAILABLE_DUE_TO_UNDEFINED_FEATURE");
  }
  #endif

  {
    ...
    {
      CM_MSG_HIGH_2("srv_status=%d, cmregprx->state=%d",srv_status, cmregprx_stack_info_ptr->state);
...
===========================================================================*/
static void cmsups_create_interrogate_cnf_internal(cm_sups_cmd_info_s_type *sups_cmd_info_ptr)
{
...
===========================================================================*/
static void cmsups_create_activate_cnf_internal(cm_sups_cmd_info_s_type *sups_cmd_info_ptr)
{
  cm_rpt_type *cm_rpt_ptr           = NULL;
  sys_modem_as_id_e_type asubs_id   = sups_cmd_info_ptr->cmd_subs_id;
...
===========================================================================*/
static void cmsups_create_deactivate_cnf_internal(cm_sups_cmd_info_s_type *sups_cmd_info_ptr)
{
  cm_rpt_type *cm_rpt_ptr           = NULL;
  sys_modem_as_id_e_type asubs_id   = sups_cmd_info_ptr->cmd_subs_id;
  volte_sub_info_s_type* volte_sub_info_ptr = cmph_get_volte_sub_info_ptr(asubs_id);
...
)
{
  ...
  case CM_SUPS_CMD_PROCESS_USS:
      ...
      )
    {
      ...
      sups_cmd_info_ptr->uss_data.size = num_bytes;
    ...
    */
    ...
    {
         if(sups_cmd_info_ptr->uss_data.size > MAX_USS_BYTES)
      {
        sups_cmd_info_ptr->uss_data.size = MAX_USS_BYTES;
    ...
    CM_MSG_HIGH_4( "uss data sent to clients and NAS. present:%d coding scheme: %d size: %d uss data: %s",
                  ...
                  sups_cmd_info_ptr->uss_data.uss_data_coding_scheme,
		  		  sups_cmd_info_ptr->uss_data.size, sups_cmd_info_ptr->uss_data.ussData );
...
)
{
 ...
 // #if defined(FEATURE_MMODE_USS_CS_SUPS)
  #if defined (FEATURE_MMODE_SS_SUPS)|| defined(FEATURE_MMODE_USS_CS_SUPS)
  mn_cnm_cmd_type         *mn_cmd_ptr;
    /* MN command pointer */
  #endif
  uint8                    num_bytes = 0;
...
#if !defined (FEATURE_MMODE_DISABLE_LTE_VOICE) || defined FEATURE_MMODE_SS_SUPS
  ...
  */
  ...
  if (cmph_is_dsds() || cmph_is_tsts())
  {
    ...
    {
      CM_MSG_HIGH_1("Voice/VT call active on other subs", other_as_id);
    }
    else
    {
...
#if defined(FEATURE_MMODE_USS_CS_SUPS)  || FEATURE_MMODE_SS_SUPS
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
					/*-------------------------*/
					/*	  ReleaseSS request    */
					/*-------------------------*/
...
#endif
#ifdef FEATURE_MMODE_USS_CS_SUPS
    case CM_SUPS_CMD_PROCESS_USS:
...
/** below common code for both CS USSD/SUPS  */

    default:
      CM_ERR_1( "Unknown SUPS command %d received from client",
               sups_cmd_ptr->cmd );
      return;
...
FUNCTION cmwsups_alloc_sups_info_ptr
...
)
{
  ...
  {
    sups_ptr->hybr_gw_sups_info = cmsups_info_alloc();
    return (sups_ptr->hybr_gw_sups_info);
  }
  else
#endif
  #ifdef FEATURE_MMODE_HYBR3_ENABLED
  ...
  {
    sups_ptr->hybr_gw3_sups_info = cmsups_info_alloc();
    return (sups_ptr->hybr_gw3_sups_info);
  }
  else
  #endif /* FEATURE_MMODE_HYBR3_ENABLED */
  {
    sups_ptr->sups_info = cmsups_info_alloc();
    return (sups_ptr->sups_info);
  }        
        ...
        while(sups_list != NULL)
        {
          if (sups_list->cmd == CM_SUPS_CMD_PROCESS_USS)
          {
            ...
            { 
              ...
              { 
                CM_MSG_HIGH_1("sups_ptr->hybr_gw_sups_info_list:",sups_ptr->hybr_gw_sups_info_list);
                sups_ptr->hybr_gw_sups_info_list = NULL; 
              } 
            ...
            { 
              ...
              { 
                CM_MSG_HIGH_1("sups_ptr->sups_info_list:",sups_ptr->sups_info_list);
                sups_ptr->sups_info_list = NULL; 
              } 
              cmwsups_mem_free_sups_list_info(sups_list, &(sups_ptr->sups_info_list_head));
              CM_MSG_HIGH_1("sups_ptr->sups_info_list_head:",sups_ptr->sups_info_list_head);
...
/* need to  featurized withe CS_PS flag separtely*/
#if defined(FEATURE_MMODE_USS_SUPS ) || defined FEATURE_MMODE_SS_SUPS
  ...
  */
  ...
  {
//#if defined (FEATURE_MMODE_USS_SUPS)//!defined(FEATURE_MMODE_DISABLE_LTE_SUPS)
...
/*lint -esym(529, ss)*/
{
  ...
  */
  ...
  if(ss == CM_SS_HYBR_2)
  {
    if(sups_ptr->hybr_gw_sups_info == NULL)
    {
      sups_info_ptr = cmwsups_alloc_sups_info_ptr (ss);
...
DESCRIPTION
  Forward a UT client sups commands to MN. This Function called when IMS
  sends fallback to CS.
...
)
{
 ...
 switch( cmd )
 {
    ...
    case CM_SUPS_CMD_REGISTER:
      ...
      /* ss reference */
	  mn_cmd_ptr->cmd.cm_register_ss_req.cm_ss_ref = sups_info->ss_ref;

      /* ss code */
      mn_cmd_ptr->cmd.cm_register_ss_req.ss_code = sups_info->ss_code;

      /* Copy command information fields. */

      mn_cmd_ptr->cmd.cm_register_ss_req.basic_service = sups_info->basic_service;

      mn_cmd_ptr->cmd.cm_register_ss_req.forwarded_to_number =
                  sups_info->forwarded_to_number;

      mn_cmd_ptr->cmd.cm_register_ss_req.forwarded_to_subaddress =
                               sups_info->forwarded_to_subaddress;

      mn_cmd_ptr->cmd.cm_register_ss_req.cm_no_reply_condition_timer =
                  sups_info->nr_timer;

      mn_cmd_ptr->cmd.cm_register_ss_req.as_id  = sups_info->asubs_id;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    case CM_SUPS_CMD_ERASE:
      ...
      mn_cmd_ptr->cmd.cm_erase_ss_req.cm_ss_ref = sups_info->ss_ref;

      mn_cmd_ptr->cmd.cm_erase_ss_req.ss_code = sups_info->ss_code;

      mn_cmd_ptr->cmd.cm_erase_ss_req.basic_service =
                            sups_info->basic_service;

      mn_cmd_ptr->cmd.cm_erase_ss_req.as_id  = sups_info->asubs_id;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    case CM_SUPS_CMD_ACTIVATE:
      ...
      /* ss reference */
      mn_cmd_ptr->cmd.cm_activate_ss_req.cm_ss_ref = sups_info->ss_ref;

      /* ss code */
      mn_cmd_ptr->cmd.cm_activate_ss_req.ss_code = sups_info->ss_code;

      /* Copy command information fields. */
      mn_cmd_ptr->cmd.cm_activate_ss_req.basic_service =
                  sups_info->basic_service;

      mn_cmd_ptr->cmd.cm_activate_ss_req.as_id  = sups_info->asubs_id;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    case CM_SUPS_CMD_DEACTIVATE:
      ...
      mn_cmd_ptr->cmd.cm_deactivate_ss_req.as_id  = sups_info->asubs_id;
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    case CM_SUPS_CMD_INTERROGATE:
      ...
      mn_cmd_ptr->cmd.cm_interrogate_ss_req.as_id  = sups_info->asubs_id;

      break;

	  default:
      CM_MSG_HIGH_1( "Unknown UT SUPS command %d received from client for fallback",cmd );
      return;
