...
$Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmdiag.c#1 $
...
*/
{
  ...
  CM_MSG_MED_0 ("!!!!!diag_orig_req is empty!!!!!");
  #endif              /* FEATURE_HWTC */
...
/*lint -esym(715,pkt_len)*/
{
  ...
  CM_MSG_HIGH_3("cmdiag_cm_state_info standby_pref %d, active_subs %d gw_sub_avail %d",\
    ph_ptr->standby_pref, ph_ptr->active_subs, ph_ptr->is_gwl_subscription_available);
  #else
  CM_MSG_HIGH_1("cmdiag_cm_state_info gw_sub_avail %d", ph_ptr->is_gwl_subscription_available);
  #endif
  ...
  if (rsp_ptr != NULL)
  {
    ...
    #endif
    {
      ...
      rsp_ptr->ciot_lte_mode         = diag_client_ptr->ciot_lte_mode;
     #endif
    }
  }
  /*lint -restore */
...
PACKED void *cmdiag_cm_set_system_selection (PACKED void* req_pkt, uint16 pkt_len)
{
    ...
    if (!req_ptr || pkt_len < sizeof(DIAG_SUBSYS_CM_CM_SYSTEM_SELECTION_PREFERENCE_req_type) )
    {
        ...
        user_pref_ptr->plmn_ptr = &plmn_struct;
        ...
        */
        ...
        {
          user_pref_ptr->network_sel_mode_pref = (cm_network_sel_mode_pref_e_type) req_ptr->network_sel_mode_pref;
        }

        if( req_ptr->mode_pref < CM_MODE_PREF_MAX )
        {
            user_pref_ptr->mode_pref = (cm_mode_pref_e_type) req_ptr->mode_pref;
        }

        if( req_ptr->acq_order_pref < CM_GW_ACQ_ORDER_PREF_MAX )
        {
            user_pref_ptr->acq_order_pref = (cm_gw_acq_order_pref_e_type) req_ptr->acq_order_pref;
        }
        ...
        */
        ...
        {
            user_pref_ptr->band_pref = (cm_band_pref_e_type) req_ptr->band_pref;
        }
        /*lint -restore */

        if( req_ptr->roam_pref < CM_ROAM_PREF_MAX )
        {
            user_pref_ptr->roam_pref = (cm_roam_pref_e_type) req_ptr->roam_pref;
        }

        if( req_ptr->hybr_pref < CM_HYBR_PREF_MAX )
        {
            user_pref_ptr->hybr_pref = (cm_hybr_pref_e_type) req_ptr->hybr_pref;
        }

        if( req_ptr->srv_domain_pref < CM_SRV_DOMAIN_PREF_MAX )
        {
            user_pref_ptr->srv_domain_pref = (cm_srv_domain_pref_e_type) req_ptr->srv_domain_pref;
        }
        
        /* Issue SYS_SEL_PREF & Fill the response packet */
        rsp_ptr->status = cm_user_pref_update_req
                          (user_pref_ptr,
                            NULL,
                            NULL);

        cm_cmd_dealloc((void*)user_pref_ptr);
    }
    /*lint -restore */
...
PACKED void *cmdiag_cm_call_end (PACKED void* req_pkt, uint16 pkt_len)
{
  ...
  {
    /* Get params from Request Packet */
    num_call_ids = MIN( req_ptr->call_id_count, CMCALL_MAX_CALL_OBJ );
...
#if (defined(FEATURE_CM_MMGPS) || defined(FEATURE_CM_LCS))&& defined(FEATURE_MMODE_SS_SUPS)
/*===========================================================================
...
/*=========================================================
Function: cmdiag_cm_call_cmd_modify_req
Description:

          This function process the packet from QXDM to initiate a modify request,
          when on a voice or cs data call, to change from voice to cs data call and vice versa.

=========================================================*/
/*lint -esym(765, cmdiag_cm_call_cmd_modify_req)
** Can't be static, to be called by DIAG via COMMAND DISPATCH TABLE
*/
PACKED void *cmdiag_cm_call_cmd_modify_req  (PACKED void* req_pkt, uint16 pkt_len)
{
  DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_REQ_req_type *req_ptr =
    (DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_REQ_req_type *)req_pkt;
  DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_REQ_rsp_type *rsp_ptr;

  cmcall_s_type *call_id_object = NULL;
  cm_call_type_e_type new_call_type;

  const int rsp_len = sizeof(DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_REQ_rsp_type);
  cmlog_client_s_type* diag_client_ptr = cmlog_get_diag_client();

  /* Allocate the memory for this */
  rsp_ptr = (DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_REQ_rsp_type *)
  diagpkt_subsys_alloc (DIAG_SUBSYS_CM, CM_CALL_CMD_MODIFY_REQ, rsp_len);
  ...
  if (!rsp_ptr)
  {
    CM_MSG_HIGH_0("modify request memory alloc failed for response packet");
    return NULL;
  }
   /* Fill the memory returned, with 0s */
  rsp_ptr->status = 0;
  ...
  if (!req_ptr)
  {
    CM_ERR_0(" call cmd modify req sent by user is NULL ");
    rsp_ptr->status = FALSE;
    return rsp_ptr;
  }
  else
  {
    call_id_object = cmcall_ptr (req_ptr->call_id);
    if (call_id_object->call_type == CM_CALL_TYPE_VOICE)
    {
      new_call_type = CM_CALL_TYPE_CS_DATA;
    }
    else if (call_id_object->call_type == CM_CALL_TYPE_CS_DATA)
    {
      new_call_type = CM_CALL_TYPE_VOICE;
    }
    else
    {
      CM_MSG_HIGH_0("modification failed: call was neither voice nor cs_data");
      return NULL;
    }
    rsp_ptr->status = cm_mm_call_cmd_modify_req (
                                           NULL,
                                           NULL,
                                           diag_client_ptr->client,
                                           req_ptr->call_id,
  ...
  /* Return status.*/
  return rsp_ptr ;
} /*cmdiag_cm_call_cmd_modify_req */

/*=========================================================
Function: cmdiag_call_cmd_modify_rsp
Description:

          This function process the packet from QXDM to response to an
          incoming modify (voice to cs data call and vice versa) request.

=========================================================*/
/*lint -esym(765, cmdiag_call_cmd_modify_rsp)
** Can't be static, to be called by DIAG via COMMAND DISPATCH TABLE
*/
PACKED void *cmdiag_call_cmd_modify_rsp  (PACKED void* req_pkt, uint16 pkt_len)
{
  DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_RSP_req_type *req_ptr =
    (DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_RSP_req_type *)req_pkt;
  DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_RSP_rsp_type *rsp_ptr;

  const int rsp_len = sizeof(DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_RSP_rsp_type);
  cmlog_client_s_type* diag_client_ptr = cmlog_get_diag_client();

  /* Allocate the memory for this */
  rsp_ptr = (DIAG_SUBSYS_CM_CM_CALL_CMD_MODIFY_RSP_rsp_type *)
  diagpkt_subsys_alloc (DIAG_SUBSYS_CM, CM_CALL_CMD_MODIFY_RSP, rsp_len);
  ...
  if (!rsp_ptr)
  {
    ...
    CM_MSG_HIGH_0("modify res memory alloc failed for response packet");
    return NULL;
  }
  ...
  if (!req_ptr)
  {
    CM_ERR_0(" call cmd modify rsp sent by user is NULL ");
    rsp_ptr->status = FALSE;
    return rsp_ptr;
  }
  else
  {
    rsp_ptr->status = cm_mm_call_cmd_modify_res (
                                     NULL,
                                     NULL,
                                     diag_client_ptr->client,
                                     req_ptr->call_id,
                                     req_ptr->gw_cs_modify_res_para,
                                     req_ptr->cc_cause_ptr_para
                                     );
...
)
{
  ...
  */
  ...
  {
    user_pref_ptr->lte_band_pref = sys_map_band_mask_to_lte_band_mask((cm_band_pref_e_type) req_ptr->lte_band_pref);
  }
  /*lint -restore */
...
static const diagpkt_user_table_entry_type cm_subsys_diag_tbl[] =
{
  ...
  #if ((defined CM_GW_SUPPORTED) && defined (FEATURE_MULTIMEDIA_FALLBACK))
  {CM_CALL_CMD_MODIFY_REQ,                  CM_CALL_CMD_MODIFY_REQ,
                                            cmdiag_cm_call_cmd_modify_req},
  {CM_CALL_CMD_MODIFY_RSP,                  CM_CALL_CMD_MODIFY_RSP,
                                            cmdiag_call_cmd_modify_rsp},
  #endif
