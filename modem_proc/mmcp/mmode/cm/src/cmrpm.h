...
/* This holds RPM Feature enablement information stored in CM_EFS_RPM_FEATURE_ENABLE
 * Need to declare as PACKED since this structure is also stored in
 * CM-NV
 */
typedef PACKED struct PACKED_POST{
  boolean                          rpm_feature_enabled;
  /* Indicate whether RPM feature is enabled or not */

}cmrpm_feature_enabled_s_type;
...
typedef struct cmrpm_s_type {
  ...
  uint32     pwr_on_sys_time;
  /* The system time when RPM is initialized at power on */
  ...
  boolean                   is_rpm_feature_enabled_nv_set;
  /* Indicate whether RPM feature enabled NV is set or not */

  boolean                          is_rpm_enabled;
  /* Indicate whether RPM functionality should be enabled or not */
  
  boolean                  is_card_inserted_processed;
  /* Indicate if RPM init has processed the card inserted event
  ** Set to FALSE when CMMMGSDI gets card inserted evt
  ** Set to TRUE during RPM init at subscription available
  */

  boolean is_rpm_enabled_in_sim;
  /* Indicates whether RPM functionality is configured from SIM or EFS. TRUE indicates configured from sim */
...
===========================================================================*/
extern void cmrpm_subs_avail_init(void);
