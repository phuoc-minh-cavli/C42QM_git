...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmautoreg.c#1 $
...
SIDE EFFECTS
  None
===========================================================================*/
static void cm_auto_reg_cts_msg_timer_cb(
                      ...
                      const cm_mm_ss_info_s_type *ii_info_ptr)
{
      ...
      ( ii_info_ptr->srv_status == SYS_SRV_STATUS_SRV ) )
  {
    ...
    {
      CM_MSG_HIGH_2("cm_auto_reg_cmds:%d, cm_auto_reg_imsi_mcc:%d",cm_auto_reg_cmds, cmmmgsdi_get_imsi_mcc());
...
/* First 4 bytes ignored */
  ...
  if(chars_to_write)
  {
    ...
    (void) snprintf(model_info,tmp_len+1,"<a1><b1>%s-%s</b1><b2>",(char *)cm_auto_reg_efs_data->manufacturer_code,short_model);
...
CM_ASSERT(client_ts_data != NULL);
  ...
  switch(event)
  {
    ...
    CM_MSG_HIGH_3("MT SMS message recv mode:%d TsID:%d;acq_req:%d",info_ptr->mt_message_info.message.msg_hdr.message_mode,
    ...
    else
    {
      ...
      if( info_ptr->mt_message_info.message.u.cdma_message.is_tl_ack_requested == TRUE ) 
      {
        cm_auto_reg_msg_send_ack(info_ptr->mt_message_info.transaction_id,WMS_ERROR_TEMP, WMS_TL_INVALID_TELESERVICE_ID_S);
        }
      }
    ...
    /* Deregister  WMS after receiving ack/nack for "ack of confirm sms" */
    cm_auto_reg_dereg();
...
void cm_auto_reg_send_sms(unsigned long data)
{
  ...
  wms_status  = wms_msg_send(cm_auto_reg_wms_cid, cm_auto_reg_send_reg_msg_cb, 0, WMS_SEND_MODE_CLIENT_MESSAGE, cm_auto_reg_wms_msg);
...
boolean cm_auto_reg_info_complete(void)
{
    ...
    {  
      ...
      /* Deactivate WMS after receiving ack/nack for "ack of confirm sms" 
      cm_auto_reg_dereg();*/
      return TRUE;
    }
  return FALSE;
...
{
  mmgsdi_slot_id_enum_type slot = cm_mmgsdi_ssn_tbl_get_slot_for_type(MMGSDI_1X_PROV_PRI_SESSION);
  /* If the UIM_CDMA_ESN_ME field did not change at startup... */
  if (!mmgsdi_get_esn_me_change_flag(slot))
  {
