...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmsds.c#1 $
...
)
{
  ...
  if((cmsds_get_config_ptr(sub_id)->is_ims_enabled) &&
    cmph_get_sub_config_ptr(sub_id)->active_ims_stack == SYS_IMS_STACK_STATUS_ENABLED_THPY &&
    sds_ptr->ims_reg_status_voice[SYS_SYS_MODE_LTE] != SYS_IMS_REG_STATE_DEREGISTERED)
  {
    return TRUE;
  }
  else if (((sds_ptr->ims_reg_status_voice[SYS_SYS_MODE_LTE] == SYS_IMS_REG_STATE_NEEDS_REG) ||
...
static void cmsds_send_tau_complete(uint8 inst_id, nas_tau_type tau_type)
{
  ...
  CM_MSG_HIGH_0("Sending tau complete");
...
)
{
  ...
  sys_sys_mode_mask_e_type mode_cap;

  if(!cmpmprx_get_mode_capability(ph_ptr->main_stack_info.asubs_id,&mode_cap))
  {
    mode_cap = (sys_sys_mode_mask_e_type)SD_SS_MODE_PREF_ANY;
  }
  ...
  if((sds_ptr->sib8_available == TRUE) &&
     ...
     (sd_misc_is_mode_pref(cmph_map_cm_mode_pref_to_sd_mode_pref(CMPH_SS_MODE_PREF(as_id,ph_ptr)),
                             SD_SS_MODE_PREF_CDMA)) &&
     cmph_is_lte_capable_on_sub(as_id) &&
     (mode_cap & SD_SS_MODE_PREF_CDMA)
    )
  {
    return TRUE;
  }
...
)
{
 ...
 .*/
  ...
  if(is_voice_supported)
  {
    ...
    */
    if (sds_ptr->voice_support_status == FALSE)
    {
  ...
  else
  {
    ...
    ** clients if not already done
    */
    if ( sds_ptr->voice_support_status  == TRUE)
    {
      ...
      if (ss == CM_SS_MAIN)
      {
        ss_ptr->info.changed_fields |= CM_SS_EVT_VOICE_SUPPORT_ON_LTE_MASK;
      }
      #if defined FEATURE_MMODE_DUAL_SIM || defined FEATURE_MMODE_SC_SVLTE
      else if (ss == CM_SS_HYBR_2)
      {
...
)
{
   ...
   )
  {
    ...
    else if(sys_plmn_id_is_valid(sds_ptr->va_type.associated_plmn_id) &&
      sys_plmn_match(old_plmn, sds_ptr->va_type.associated_plmn_id))
    {
...
)
{
  ...
  CM_MSG_MED_5("Domsel: current vocie domain preferance =%d ims_avail=%d sms_only=%d,ue_usage_setting=%d,lte_disable_duration=%d",
                    cmsds_config->voice_domain_pref, si_info_ptr->lte_ims_voice_avail,
               cmsds_config->is_sms_only,cmsds_config->ue_usage_setting,
               cmsds_config->lte_disable_duration);

  CM_MSG_MED_5("Domsel: curr sms domain %d, sms_mandatory %d, sms_s102_supp %d, extend_srv_info %d, ims_enabled %d",\
                    cmsds_config->sms_domain_pref[si_info_ptr->mode],
                    cmsds_config->is_sms_mandatory,cmsds_config->is_sms_over_s102_supported,
  ...
  {
    CM_MSG_HIGH_0("Skipping domain selection either due to activation or lack of stable srv");
    return;
  }
       ...
       (sds_ptr->is_plmn_blocking_in_progress == FALSE))
  {
    ...
    #if !defined(FEATURE_MMODE_LTE_ONLY) && !defined(FEATURE_MMODE_GPRS_LTE) && !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
    if(!cmsds_is_volte_possible(si_info_ptr->lte_ims_voice_avail, as_id))
    {
      sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_VOICE] = CM_IMS_REG_STATUS_FAIL_PERMANENT;
  ...
  {
    cmsds_send_tau_complete(SYS_AS_ID_TO_INST_ID(as_id), sds_ptr->tau_type);
...
)
{
  ...
  {
    #if !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
    if ((sds_ptr->lte_ims_voice_avail != ims_voice_avail) &&
             (cmsds_config->voice_domain_pref != SYS_VOICE_DOMAIN_PREF_CS_VOICE_ONLY))
    {
...
)
{
  ...
  ** SMS over IMS is not allowed by device config itself */
  if ((cmsds_config->is_sms_mandatory == TRUE) &&
      (cmsds_config->sms_domain_pref[SYS_SYS_MODE_LTE] == SYS_SMS_DOMAIN_PREF_PS_SMS_NOT_ALLOWED ||
      (cmsds_config->sms_domain_pref[SYS_SYS_MODE_LTE] == SYS_SMS_DOMAIN_PREF_PS_SMS_PREF &&
        sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_SMS] == CM_IMS_REG_STATUS_FAIL_PERMANENT)) &&
       sys_mode == SYS_SYS_MODE_LTE )
  {
    boolean is_csfb_call = FALSE;
    #if defined (FEATURE_3GPP_CSFB) || defined (FEATURE_LTE_TO_1X)
    is_csfb_call = cmcall_are_there_csfb_call_preferences(ss);
    #endif

    /* Perform PLMN blocking as SMS is not available
       ** PLMN blocking will be performed only when there is no CSFB call
       */
    if((is_csfb_call == FALSE) &&
...
)
{
  ...
  */
          ...
          (SYS_SRV_DOMAIN_PS_ONLY == si_info_ptr->srv_domain))
  {
    ...
    else if((si_info_ptr->cp_sms_service_status == SYS_CP_SMS_STATUS_SERVICE_TEMP_FAILURE) &&
            (SD_MODE_LTE_NB1 == si_info_ptr->ciot_lte_mode)
           )
    {
      /* Indicate SMS over CP support for SMS service*/
      cmsds_update_and_send_sms_dom_sel_ind (ss, SYS_CM_DOMAIN_SEL_DOMAIN_NO_SMS, sys_mode);
    }
    else if((si_info_ptr->cp_sms_service_status == SYS_CP_SMS_STATUS_AVAILABLE) &&
            (SD_MODE_LTE_NB1 == si_info_ptr->ciot_lte_mode)
           )
    {
      /* Indicate SMS over CP support for SMS service*/
  ...
  {
    #if defined(FEATURE_CIOT)
    if((si_info_ptr->cp_sms_service_status == SYS_CP_SMS_STATUS_AVAILABLE) &&
       (SD_MODE_LTE_NB1 == si_info_ptr->ciot_lte_mode)
      )
    {
      /* Indicate SMS over CP support for SMS service*/
    ...
    #if (defined(FEATURE_CDMA_800) || defined(FEATURE_CDMA_1900)) && \
     defined( FEATURE_LTE_TO_1X )
    else if ((cmsds_config->is_sms_over_s102_supported == TRUE)                      &&
    ...
    #endif
    else if ((SYS_CP_SMS_STATUS_SERVICE_TEMP_FAILURE == si_info_ptr->cp_sms_service_status) &&
             (SD_MODE_LTE_NB1 == si_info_ptr->ciot_lte_mode)
             )
    {
      cmsds_update_and_send_sms_dom_sel_ind (ss, SYS_CM_DOMAIN_SEL_DOMAIN_NO_SMS, sys_mode);
    ...
    else
    {
     ...
     #if (defined(FEATURE_CDMA_800) || defined(FEATURE_CDMA_1900)) && \
     defined( FEATURE_LTE_TO_1X )
    if ((cmsds_config->is_sms_over_s102_supported == TRUE)                      &&
    ((cmsds_get_csfb_type_allowed(asubs_id) == CM_CALL_CSFB_TYPE_1XCSFB) ||
    (cmsds_is_1xcsfb_activation_allowed(asubs_id) == TRUE))
    )
    {
...
)
{
     ...
     sys_mode != SYS_SYS_MODE_CDMA)
  {
    if (ims_reg_status->is_registered == TRUE)
    {
      ...
      sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_SMS] = CM_IMS_REG_STATUS_NONE;
    ...
    else
    {
      ...
      {
        sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_SMS] = CM_IMS_REG_STATUS_FAIL_TEMPORARY;
      }
      else
      {
        sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_SMS] = CM_IMS_REG_STATUS_FAIL_PERMANENT;
...
)
{
  ...
  if(reg_status_ptr->is_registered == FALSE &&
...
)
{
  ...
  #if !defined(FEATURE_MMODE_LTE_ONLY) && !defined(FEATURE_MMODE_GPRS_LTE)
  if(!(cmsds_skip_ims_reg_status_wait_timer_clear(&(cm_cmd_ptr->cm_domain_sel_update_ims_reg_status_Ind.data))) )
  {
  ...
  */
  ...
  {
    CM_MSG_HIGH_3("Ignore IMS REG status due to lack of FULL SRV or sys_mode mismatch. CM's sys_mode %d srv_status %d srv_domain %d",\
    si_info_ptr->mode, si_info_ptr->srv_status,si_info_ptr->srv_domain );
...
)
{
  ...
  #else
  CM_MSG_HIGH_1("Invalid message not supported %d", cm_cmd_ptr->msg_hdr.id);
  #endif
}
...
)
{
  ...
  #if !defined(FEATURE_MCM_LTE_ONLY_BUILD)
...
)
{
  ...
  */
  ...
  #if defined(FEATURE_CIOT)
  /*Do not block PLMN when cp sms status is not available and sim state is cp invalid */
  if((sd_ss_gw_get_sim_state(ss) == SYS_SIM_STATE_CP_SMS_INVALID) &&
     (block_reas == CM_PLMN_BLOCK_REAS_NO_SMS) &&
     (si_info.cp_sms_service_status == SYS_CP_SMS_STATUS_NOT_AVAILABLE)
     )
  {
    CM_MSG_HIGH_2("DOM_SEL: Skip PLMN blocking .SIM CP SMS Invalid, cp_sms_service_status =%d, block_reas=%d",\
                   si_info.cp_sms_service_status, block_reas);
    return;
  }
  #endif
  ...
  ** and reject plmn blocking only if resultant mode as per SD's preferences
  ** is LTE_only
  */
  sd_misc_get_mode_band_per_ss(ss, &ss_mode_pref, &ss_band_pref, &ss_lte_band_pref, &ss_tds_band_pref);

  CM_MSG_HIGH_2("sd_mode_pref:%d, plmn blocking in lte_only mode %d", ss_mode_pref, cmsds_get_config_ptr(asubs_id)->is_plmn_block_req_in_lte_only_mode);
  CM_MSG_HIGH_4("band_pref:(MSB)(LSB) cgw=0x%08x 0x%08x, TDS = 0x%08x 0x%08x",
               QWORD_HIGH(ss_band_pref),
               QWORD_LOW(ss_band_pref),
               QWORD_HIGH(ss_tds_band_pref),
               QWORD_LOW(ss_tds_band_pref));
  cm_print_lte_band_mask(ss_lte_band_pref);
  is_lte_only = (cmcall_is_lte_only_mode(cmph_map_sd_mode_pref_to_cm_mode_pref(ss_mode_pref), 
                                         cmph_map_sd_band_pref_to_cm_band_pref(ss_band_pref),
                                         ss_lte_band_pref,
                                         cmph_map_sd_tds_band_pref_to_cm_tds_band_pref(ss_tds_band_pref)));
  ...
  else
  {
    #if defined(FEATURE_MMODE_GPRS_LTE)
    {
      ...
      if(block_reas == CM_PLMN_BLOCK_REAS_NO_VOICE)
      {
       /* Do not block PLMN in L+G flavor if block_reas is no voice as only LTE can be used for voice services.
        */
        CM_MSG_HIGH_2("DOM_SEL: L+G flavor Skip PLMN blocking as G doesn't support voice , block_reas %d, srv_domain %d",
        block_reas,
      ...
      if((block_reas == CM_PLMN_BLOCK_REAS_NO_SMS) &&
               (CMPH_SS_SRV_DOMAIN_PREF(asubs_id, ph_ptr) != SYS_SRV_DOMAIN_CS_PS) &&
               (cmsds_get_config_ptr(asubs_id)->is_plmn_block_req_in_lte_only_mode == FALSE)
               )
      {
        /* L+G flavor without CS registration is basically lte only so skip plmn blocking only 
           if is_plmn_block_req_in_lte_only_mode is not set
        */
        CM_MSG_HIGH_2("DOM_SEL: L+G flavor Skip PLMN blocking, block_reas %d, srv_domain %d",
        block_reas,
        CMPH_SS_SRV_DOMAIN_PREF(asubs_id, ph_ptr));
        return;
      }
  ...
  if (((sds_ptr->lte_cs_capability == SYS_LTE_CS_CAPABILITY_BARRED)
      ...
      && (sds_ptr->csfb_status == CM_1XCSFB_NONE)
      #endif
     ) || (sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_VOICE] == CM_IMS_REG_STATUS_FAIL_PERMANENT_TILL_PWR_CYCLE)
     #if defined(FEATURE_CIOT)
       || (block_reas == CM_PLMN_BLOCK_REAS_NO_SMS && si_info.cp_sms_service_status == SYS_CP_SMS_STATUS_SERVICE_NOT_AVAILABLE_IN_PLMN)
     #endif
     )
  {
...
)
{
  ...
  if (cmsds_config->ue_usage_setting == SYS_UE_USAGE_SETTING_VOICE_CENTRIC)
  {
      ...
      cmsds_update_and_send_voice_dom_sel_ind (ss, SYS_CM_DOMAIN_SEL_DOMAIN_NONE,SYS_SYS_MODE_LTE);
      if (sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_VOICE] == CM_IMS_REG_STATUS_FAIL_TEMPORARY &&
        cmsds_config->voice_domain_pref != SYS_VOICE_DOMAIN_PREF_CS_VOICE_ONLY)
      {
...
)
{
  ...
  if (cmsds_config->voice_domain_pref == SYS_VOICE_DOMAIN_PREF_IMS_PS_VOICE_ONLY)
  {
    if (reg_status_ptr->is_registered == TRUE)
    {
      ...
      sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_VOICE] = CM_IMS_REG_STATUS_NONE;
    ...
    else
    {
      ...
      {
        sds_ptr->ims_registration_status[CM_IMS_CALL_TYPE_VOICE] = CM_IMS_REG_STATUS_FAIL_TEMPORARY;
...
)
{
  ...
  #endif
  {
    ...
    if (csfb_type == CM_CALL_CSFB_TYPE_PPCSFB)
    {
      ...
      cmsds_update_and_send_voice_dom_sel_ind(ss, SYS_CM_DOMAIN_SEL_DOMAIN_3GPP,SYS_SYS_MODE_LTE);
     CM_MSG_HIGH_0( "Domsel:CS PREFERRED: PP  possible");
    }
    #endif
...
)
{
  ...
  if (sds_ptr->is_dual_rx_1xcsfb == TRUE                                &&
      ...
      ph_ptr->is_cdma_subscription_available == TRUE                   &&
     (sd_misc_is_mode_pref(cmph_map_cm_mode_pref_to_sd_mode_pref(CMPH_SS_MODE_PREF(asubs_id,ph_ptr)),
                       SD_SS_MODE_PREF_CDMA))
     )
  {
...
)
{
  ...
  CM_MSG_HIGH_2 ("pm_mode_mask %d,mode_pref =%d ",\
  ...
  switch( sd_rpt_ptr->hdr.cmd )
  {
    ...
    case CM_SRV_IND_INFO_F:
    {
      ...
      ** or when UE camp on 1x while UE is not in SRLTE mode */
      #if (defined( FEATURE_DOMAIN_SELECTION )&& !defined(FEATURE_MMODE_LTE_ONLY) && !defined(FEATURE_MMODE_GPRS_LTE))
               ...
               sd_rpt_ptr->srv_ind_info.si_info.srv_domain == SYS_SRV_DOMAIN_CAMPED ))
           {
             if(cmph_is_lte_capable_on_sub(asub_id))
             {
               if(!(cmss_is_hybr2_operational() && cmph_is_subs_feature_mode_1x_sxlte(asub_id )))
               {
                 ...
                 sds_ptr->ims_reg_status_sms[SYS_SYS_MODE_LTE] = SYS_IMS_REG_STATE_UNKNOWN;
                 #if !defined(FEATURE_MMODE_DISABLE_LTE_VOICE)
               ...
               {
                 CM_MSG_LOW_0("HDR stack is operational, do not reset IMS related variables");
               }
               #endif
               else
               {
...
lte_3gpp_release_ver_e         lte_rel_version;
  ...
  sds_ptr->is_plmn_blocking_in_progress = FALSE;
  sds_ptr->is_plmn_blocking_delayed = FALSE;
  for(i = 0; i < ARR_SIZE(sds_ptr->config) ; i++)
  {
  ...
  else
  {
      ...
      /* The value is changed as this PL is only for MTC devices
         set default value to SYS_UE_USAGE_SETTING_DATA_CENTRIC */
      sds_ptr->config[i].ue_usage_setting = SYS_UE_USAGE_SETTING_DATA_CENTRIC;
    }
  }
  ...
  for(i = 0; i < ARR_SIZE(sds_ptr->config) ; i++)
  {
    ...
    else
    {
      ...
      sds_ptr->config[i].voice_domain_pref = SYS_VOICE_DOMAIN_PREF_IMS_PS_VOICE_ONLY;
      sds_ptr->config[i].is_voice_domain_pref_locked = FALSE;
      CM_MSG_HIGH_2 ("Domsel: Sub %d Set voice_domain_pref = %d",
                     i, sds_ptr->config[i].voice_domain_pref);
     #endif
  }
...
)
{
  ...
  *dom_sel_ptr   = SYS_CM_DOMAIN_SEL_DOMAIN_NONE;
  ...
  /* If emergency origination is happening, and current IMS system does
  ** not support emergency, check if CSFB is possible
  */
...
===========================================================================*/
void cmsds_process_tau_complete_ind(emm_tau_complete_ind_s_type *tau_complete_ind)
{
  cmsds_s_type   *sds_ptr = cmsds_ptr();
  ...
  if(sys_plmn_match(tau_complete_ind->plmn, si_info_ptr->sid.plmn.plmn_id) &&
    (tau_complete_ind->tac == si_info_ptr->mode_info.gwl.tac))
  {
    cmsds_send_tau_complete(tau_complete_ind->msg_hdr.inst_id, tau_complete_ind->tau_type);
    sds_ptr->is_tau_needed = FALSE;
    sds_ptr->tau_type = NAS_3GPP_INVALID_TYPE;
  }
  else
  ...
  CM_MSG_HIGH_3(" si_info_ptr.mode_info.gwl.tac %d tau_complete_ind tac %d is_tau_needed %d",\
                si_info_ptr->mode_info.gwl.tac, tau_complete_ind->tac, sds_ptr->is_tau_needed);
  return;
}
