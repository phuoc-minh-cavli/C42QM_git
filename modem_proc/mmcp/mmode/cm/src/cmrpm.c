       ...
       (curr_oprt_mode == SYS_OPRT_MODE_FTM)))
  {
              ...
              (rpm_ptr->sys_time_efs == cm_util_get_curr_systime())))
    {
      ...
      */
      ...
      CM_MSG_HIGH_2("Write time %d to EFS, pwr_on_sys_time %d", sys_time_pwr_down, rpm_ptr->pwr_on_sys_time);
...
static boolean cmrpm_write_efs_info(const cmrpm_s_type   *rpm_ptr )
{
  ...
  else
  {
    ...
    */
    ...
    CM_MSG_HIGH_2("rpm_write_efs_info - timer:%d, counter: %d",
      rpm_efs_info.app_reset_timer, rpm_efs_info.app_reset_counter);
...
{ 
  ...
  if (mmgsdi_rpm_has_iccid_changed(MMGSDI_SLOT_1, &is_sim_changed) == MMGSDI_SUCCESS)
  {
    CM_MSG_HIGH_2("check_sim_changed: is_sim_changed: %d, is_card_inserted_procd %d",
                  is_sim_changed, is_card_inserted_processed);
  ...
  {
    CM_ERR_1("check_sim_changed: Failed to query SIM changed or not, is_card_inserted_procd %d",
             is_card_inserted_processed);
  }
  return FALSE;
  #else
  return FALSE;
...
)
{
  ...
  if((rpm_ptr != NULL) && rpm_ptr->is_rpm_enabled_in_sim == FALSE)
  {
    return FALSE;
  }
  /* Allocate memory for RPM OMC */
...
static boolean cmrpm_is_rpm_sim(void)
{
  ...
  if(cmmmgsdi_read_imsi_hplmn(&hplmn) == FALSE)
  {    
...
static void cmrpm_check_timer_expired( void )
{
  ...
  CM_MSG_MED_4("check_timer: rpm_ptr->sys_time_efs:%d, coin_cell_support:%d, curr_systime:%d, pwr_on_sys_time:%d",
               rpm_ptr->sys_time_efs, rpm_ptr->is_coin_cell_support, curr_system_time, rpm_ptr->pwr_on_sys_time);
  ...
  */
  ...
  {
    CM_MSG_HIGH_3("App Reset Timer expired - curr_systime:%d, timer:%d, counter: %d",
      curr_system_time, rpm_ptr->app_reset.timer, rpm_ptr->app_reset.counter);
  ...
  */
  ...
  if(cmrpm_is_rpm_enabled() &&
       ...
       (cmd_info_ptr->oprt_mode == SYS_OPRT_MODE_FTM)))
    {
      ...
      CM_MSG_HIGH_3("cmrpm_process_app_reset: app_reset.timer %d, valid_oprt_change %d, client_id %d",
                rpm_ptr->app_reset.timer, valid_oprt_change, ph_cmd_ptr->client_id);
...

{
  ...
  CM_MSG_MED_4("app_reset_reject: rpm_ptr->sys_time_efs:%d, coin_cell_support:%d, curr_systime:%d, pwr_on_sys_time:%d",
               rpm_ptr->sys_time_efs, rpm_ptr->is_coin_cell_support, curr_sys_time, rpm_ptr->pwr_on_sys_time);
  ...
  */	      
  if(rpm_ptr->is_coin_cell_support == FALSE )
  {
    curr_sys_time = (rpm_ptr->sys_time_efs) + (curr_sys_time - rpm_ptr->pwr_on_sys_time);
  }
...
{
  return ((cmrpm_ptr()->is_rpm_enabled) && (cmrpm_ptr()->is_rpm_feature_enabled_nv_set));
}
...
boolean cmrpm_init( void )
{
  ...
  rpm_ptr->is_rpm_enabled_in_sim = FALSE;
    ...
    if(cmrpm_is_rpm_sim())
    {
          ...
          &(rpm_ptr->mmreject_reset.average_reset_time)) != FALSE)         
      {
        ...
        */
        ...
        rpm_ptr->is_rpm_enabled_in_sim = TRUE;
  ...
  if(rpm_ptr->is_rpm_enabled)
  {
    ...
    */
    ...
    CM_MSG_HIGH_6("RPM Parameter mmreject_reset: T1:%d, timer:%d, C-R-1:%d, LR-2:%d, LR-2 timer:%d, pwr_on_sys_time:%d",
      rpm_ptr->mmreject_reset.average_reset_time,
      rpm_ptr->mmreject_reset.timer,
      rpm_ptr->mmreject_reset.counter,
      rpm_ptr->mmreject_reset.leak_rate_hour,
      rpm_ptr->mmreject_reset.leak_rate_timer,
      rpm_ptr->pwr_on_sys_time);
...
)
{
  ...
  CM_MSG_MED_4("rpm_timer_proc: rpm_ptr->sys_time_efs:%d, coin_cell_support:%d, curr_systime:%d, pwr_on_sys_time:%d",
               rpm_ptr->sys_time_efs, rpm_ptr->is_coin_cell_support, curr_sys_time, rpm_ptr->pwr_on_sys_time);
...

{
  ...
  CM_MSG_MED_5("cmrpm_rpt_proc: cmd=%d, rpm_ptr->sys_time_efs:%d, coin_cell_support:%d, curr_systime:%d, pwr_on_sys_time:%d",
               cm_rpt_ptr->hdr.cmd, rpm_ptr->sys_time_efs,
               rpm_ptr->is_coin_cell_support, curr_system_time, rpm_ptr->pwr_on_sys_time);
...
/*===========================================================================
FUNCTION cmrpm_subs_avail_init

DESCRIPTION
  Initializes the RPM object. Not supported in MSIM config.
...
===========================================================================*/
void cmrpm_subs_avail_init(void)
{
  ...
  if(TRUE == cmrpm_ptr()->is_rpm_feature_enabled_nv_set)
  {
    is_rpm = cmrpm_init();
  }
...
)
{
  ...
  /* Do not process gsdi refresh for RPM if RPM feature enabled NV is not set */
  if(FALSE == cmrpm_ptr()->is_rpm_feature_enabled_nv_set)
  {
	  return;
  }
...
FUNCTION cmrpm_set_is_card_inserted_processed

DESCRIPTION
  Updates value of CM RPM is_card_inserted_processed
...
===========================================================================*/
void cmrpm_set_is_card_inserted_processed(
  boolean is_card_inserted_processed
)
{
  cmrpm_s_type  *rpm_ptr = cmrpm_ptr();

  rpm_ptr->is_card_inserted_processed = is_card_inserted_processed;
}
