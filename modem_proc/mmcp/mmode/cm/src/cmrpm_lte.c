...
)
{
  ...
  CM_MSG_HIGH_6("write_efs_sys_time: oprt_mode_chg prev %d curr %d is_rpm %d sys_time_efs %d coin_cell_support %d systime %d",
  ...
  */
       ...
       (curr_oprt_mode == SYS_OPRT_MODE_RESET)))
  {
              ...
              (rpm_ptr->sys_time_efs == cm_util_get_curr_systime())))
    {
       ...
       */
      sys_time_pwr_down = rpm_ptr->sys_time_efs + (cm_util_get_curr_systime() - rpm_ptr->pwr_on_sys_time);
      CM_MSG_HIGH_2("write_efs_sys_time: write system time: %d to EFS. pwr_on_sys_time = %d",
                    sys_time_pwr_down, rpm_ptr->pwr_on_sys_time);
...
static boolean cmrpm_lte_write_efs_rpm_info(const cmrpm_lte_s_type   *rpm_ptr )
{
  ...
  else
  {
    ...
    CM_MSG_HIGH_6("write_efs_rpm_info: is_rpm_enabled %d, max_event_counter %d, max_event_timer %d, last_emm_reject_cause %d, event_timer %d, emm_gen_counter %d",
...
static boolean cmrpm_lte_check_sim_changed( void )
{
  ...
  boolean                       is_card_inserted_processed = rpm_ptr->is_card_inserted_processed;

  mmgsdi_slot_id_enum_type slot = MMGSDI_SLOT_1;
#ifdef FEATURE_MMGSDI_SESSION_LIB
  slot = cm_mmgsdi_ssn_tbl_get_slot_for_type(MMGSDI_GW_PROV_PRI_SESSION);
  slot = ((slot == MMGSDI_MAX_SLOT_ID_ENUM)?MMGSDI_SLOT_1:slot);
#endif

  cmrpm_set_is_card_inserted_processed(TRUE);

  if(mmgsdi_rpm_has_iccid_changed(slot, &is_sim_changed) == MMGSDI_SUCCESS)
  {
    CM_MSG_HIGH_3("check_sim_changed: slot %d, is_sim_changed: %d, is_card_inserted_procd %d",
                  slot, is_sim_changed, is_card_inserted_processed);

    if(is_sim_changed && !is_card_inserted_processed)
    {
      return TRUE;
    }
  }
  ...
  {
    CM_ERR_2("check_sim_changed: Failed to query SIM changed or not, slot %d, is_card_inserted_procd %d",
             slot, is_card_inserted_processed);
  }
  return FALSE;
}
...
static boolean cmrpm_lte_check_timer_expired( void )
{
  ...
  CM_MSG_MED_4("check_timer: sys_time_efs: %d coin_cell_support: %d pwr_on_sys_time %d curr_systime: %d",
               rpm_ptr->sys_time_efs, rpm_ptr->is_coin_cell_support, rpm_ptr->pwr_on_sys_time, curr_system_time);
  ...
  */
  ...
  {
    curr_system_time = rpm_ptr->sys_time_efs + (curr_system_time - rpm_ptr->pwr_on_sys_time);
  }
...
boolean cmrpm_lte_init( void )
{
  ...
  {
    CM_MSG_HIGH_2("cmrpm_lte_init: is_rpm_enabled %d, is_gcf_enabled %d",
                  read_rpm_efs_info.is_rpm_enabled, *(cmph_get_gprs_anite_gcf_ptr()));
        ...
        (TRUE  == read_rpm_efs_info.is_rpm_enabled) )
    {
      ...
      rpm_ptr->pwr_on_sys_time = cm_util_get_curr_systime();
      ...
      CM_MSG_HIGH_6("is_rpm %d, max_event_counter %d, max_event_timer %d, event_timer %d, last_emm_reject_cause %d, emm_gen_counter %d",
      ...
      CM_MSG_HIGH_1("pwr_on_sys_time %d", rpm_ptr->pwr_on_sys_time);
...
===========================================================================*/
cm_rpm_ret_e_type cmrpm_lte_set_rpm_parameters_req( cm_mm_lte_rpm_parameters_s_type set_cm_rpm_parameters_req)
{

  cmrpm_lte_s_type *lte_rpm_ptr = cmrpm_lte_ptr();
  cmrpm_lte_s_type  old_rpm_info;

  cm_rpm_ret_e_type  cm_rpm_ret_value_new = CM_RPM_FAILED_RPM_DISABLED;

  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  if( lte_rpm_ptr == NULL || !lte_rpm_ptr->is_rpm_enabled)
  {
    return cm_rpm_ret_value_new ;
  }
  ...
  {
    cm_rpm_ret_value_new = CM_RPM_SUCCESS_RPM_ENABLED;
...
===========================================================================*/
 cm_rpm_ret_e_type cmrpm_lte_send_get_rpm_parameters_rsp(cm_mm_lte_rpm_parameters_s_type *lte_rpm_params )
{
  ...
  {
    cm_rpm_ret_value_new= CM_RPM_SUCCESS_RPM_ENABLED;
    lte_rpm_params->max_event_counter = cmrpm_lte_ptr()->max_event_counter;
    lte_rpm_params->max_event_timer   = cmrpm_lte_ptr()->max_event_timer;
  }  

  CM_MSG_HIGH_3("get_rpm: send GET RPM_PARAMETERS_RSP with rsp = %d, max_event_counter = %d, max_event_timer =%d", \
                cm_rpm_ret_value_new,
                lte_rpm_params->max_event_counter ,
                lte_rpm_params->max_event_timer);
...
DESCRIPTION
  ...
  RPM is not supported in MSIM config.
...
void cmrpm_lte_set_rpm_config_req( boolean set_rpm_config_req)
{
  ...
  if(rpm_ptr->is_rpm_enabled != set_rpm_config_req)
  {
    ...
    */
    ...
    else if(cmrpm_lte_write_efs_rpm_info(rpm_ptr) == FALSE)
    {
...
)
{
  ...
  CM_MSG_HIGH_3("process_app_reset: client_id %d, from mode %d -> mode %d",
                ph_cmd_ptr->client_id, cmph_ptr()->oprt_mode, cmd_info_ptr->oprt_mode);
  ...
  CM_MSG_HIGH_1("process_app_reset: valid_oprt_change %d",
                valid_oprt_change);
  ...
  if( TRUE  == valid_oprt_change )
  {
    ...
    CM_MSG_HIGH_2("process_app_reset: efs_read_status %d, efs_time_read_status %d",
                  efs_read_status, efs_time_read_status);

    if( (TRUE == efs_read_status) &&
        (TRUE == efs_time_read_status) )
    {
      CM_MSG_HIGH_2("process_app_reset: is_rpm_enabled %d, is_gcf_enabled %d",
                    read_rpm_efs_info.is_rpm_enabled, *(cmph_get_gprs_anite_gcf_ptr()));
      ...
      (FALSE == cmrpm_lte_check_timer_expired()) )
  {
    ...
    for (i=0; i < CM_MAX_NUM_OF_TAC; i++)
    {
      if(IS_VALID_TAC(rpm_ptr->emm_15_reject[i].lte_tai.tac))
      {
        ...
        CM_MSG_HIGH_5("tai_list_ind: valid cmrpm tac[%d] = %d, plmn %d %d %d",
                      i, cmrpm_tai.tac,
...
)
{
  ...
  ** If UE is registered in both CS and PS on LTE:
    ...
    */
    case CM_SRV_IND_INFO_F:
      if( (sd_rpt_ptr->srv_ind_info.si_info.srv_status == SYS_SRV_STATUS_SRV) &&
          (sd_rpt_ptr->srv_ind_info.si_info.mode == SYS_SYS_MODE_LTE) &&
...
)
{
  ...
  */
  ...
  if( rpm_ptr == NULL || cm_rpt_ptr == NULL)
  {
    return;
  }
  ...
  switch( cm_rpt_ptr->hdr.cmd )
  {
    case CM_REG_REJECT_IND:
      if(rpm_ptr->is_rpm_enabled)
      {
        ...
        CM_MSG_HIGH_6("reg_rej_ind: sys_time_efs: %d, coin_cell: %d, curr_system_time: %d, reject_domain: %d, reject_cause: %d, tac: %d",
              ...
              if( is_tac_already_present == FALSE )
              {
                ...
                for ( i = 0; i < CM_MAX_NUM_OF_TAC; i++)
                {
                  if(!IS_VALID_TAC(rpm_ptr->emm_15_reject[i].lte_tai.tac))
                  {
                    ...
                    CM_MSG_HIGH_6("reg_rej_ind: Added tac[%d] = %d, Added plmn %d %d %d, Counter = %d",
                                  i, rpm_ptr->emm_15_reject[i].lte_tai.tac,
...
void cmrpm_lte_send_update_ftai_list(sys_rpm_ftai_list_action_e_type action)
{
  ...
  mm_ptr->cmd.cm_mm_update_ftai_list.as_id            = SYS_MODEM_AS_ID_1;
