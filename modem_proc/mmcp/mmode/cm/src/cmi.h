...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmi.h#1 $
...
#define CM_IS_MODE_3GPP(mode) \
   ((mode == SYS_SYS_MODE_GSM) ||(mode == SYS_SYS_MODE_WCDMA) ||(mode == SYS_SYS_MODE_LTE)||(mode == SYS_SYS_MODE_TDS)||(mode == SYS_SYS_MODE_LTE_M1)||(mode == SYS_SYS_MODE_LTE_NB1))
...
typedef enum cm_hold_cmd_reas_e {
  ...
  CM_HOLD_CMD_REAS_ADV_SEARCH_VOICE_CALL,
   /*  Hold advance search command till the voice call ends    */
...
typedef struct cm_call_cmd_info_s
{
  ...
  */
  ...
  boolean                             psm_ctxt_restore_status;
  /* DS indicates if ctxt restoration was successful*/
...
typedef struct cm_ph_cmd_info_s {
  ...
  cm_ciot_lte_pref_s_type       ciot_lte_pref_get_net;
  /* Indicates preferences for LTE to be scanned in manual search*/
...
#if 0
typedef struct cm_sns_update_info_s
{
  /* Allow enable disable of the sensor at
    run time*/
  cm_sns_id_e_type sensor_id;
  boolean enable;

}cm_sns_update_info_s_type; 
#endif

typedef struct cm_sns_data_req_info_s
{
  /* no of query app interesetd in sensor  */
  int  sensor_id_mask;
#ifdef FEATURE_MMODE_TX_21  
  boolean is_logging_mode;
#endif
}cm_sns_data_req_info_s_type;

typedef struct cm_sns_ondemand_sns_req_s
{
   
   int  sensor_id_mask;
}cm_sns_ondemand_sns_req_s_type;

typedef struct cm_sns_phy_test_req_s
{
  cm_sns_id_e_type sensor_id;
  cm_sns_phy_test_e_type phy_sns_test;
}cm_sns_phy_test_req_s_type;

typedef struct cm_sns_phy_test_get_data_req_s
{
  cm_sns_id_e_type sensor_id;
  boolean  is_enabled;
  float sample_rate;
  uint32 report_rate;
}cm_sns_phy_test_get_data_req_s_type;

typedef struct cm_cns_serial_num_s
{
   cm_sns_id_e_type sensor_id;
   boolean serial_number_req;
}cm_cns_serial_num_s_type;

typedef struct cm_sns_get_registry_config_req_s
{
  cm_sns_id_e_type sensor_id; 
}cm_sns_get_registry_config_req_s_type;
#if 0
...
#endif
typedef struct cm_sns_cmd_info_s
{
  union 
  	{
      cm_sns_data_req_info_s_type sns_data_req;
	  cm_sns_config_s_type        sns_config_req;
	  cm_sns_ondemand_sns_req_s_type sns_od_sns_req;
	  cm_sns_phy_test_req_s_type sns_phy_test;
	  cm_sns_phy_test_get_data_req_s_type sns_phy_test_get_data;
	  cm_cns_serial_num_s_type sns_serial_num_req;
	  //cm_sns_qmi_sns_cmd_rsp_s_type  sns_qmi_rsp;
      cm_sns_get_registry_config_req_s_type sns_registry_config;
      #ifdef FEATURE_MMODE_TX_21
      sns_tracker_log_level sns_logging_level;
      #endif
  	}cmd_s;
}cm_sns_cmd_info_s_type;


typedef struct cm_sns_cmd_s{
   CM_CMD_COMMON_FIELDS;
   cm_sns_cmd_cb_f_type *cmd_cb_func;
   cm_sns_cmd_e_type    cmd;
   cm_sns_cmd_info_s_type info;
...
typedef union cm_lte_sys_u {
  ...
  lte_rrc_tx_stat_ext_ind_s            lte_rrc_tx_ext_ind;
...
typedef struct cm_mo_exception_activity_ind_s
{
   /* Message router header */
  msgr_hdr_s          hdr;
  boolean          mo_excp_status_ind;
  /* true mean mo excp started /false meand it is ended  */

}cm_mo_exception_activity_ind_s_type;
...
typedef union cm_msgr_cmd_u {
  ...
  emm_tau_complete_ind_s_type emm_tau_complete_ind;
  ...
  /* MO_Exception indication */
  cm_mo_exception_activity_ind_s_type mo_ex_ind;
...
#ifdef FEATURE_REF_TRACKER 
  cm_sns_mod_sensor_cmd_u_type  sns_sample_data;
  cm_sns_motion_sensor_config_req_s_type sns_motion_config;
#endif
...
#ifdef FEATURE_REF_TRACKER 
#define SNS_CMD_PTR(cmd_ptr)       ((cm_sns_cmd_s_type*)cmd_ptr)
#endif
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#define CM_MSGR_MSG_ESM( msg_id ) \
    (( msg_id >= MM_CM_ACT_DEFAULT_BEARER_CONTEXT_REQUEST_IND &&  msg_id <= MM_CM_ESM_NOTIFICATION_IND) || \
      (msg_id == MM_CM_RESTORE_CTXT_PSM_REQ || msg_id == MM_CM_RESTORE_BEARER_CTXT_RSP))
...
/* Macro to check MSG ID for LTE ML1/CPHY TX mode status for LTE
*/
#define CM_MSGR_MSG_CPHY_TX_STATUS( msg_id ) ( msg_id == LTE_RRC_TX_STATUS_EXT_IND )
...
#define CM_MSGR_MSG_ESM_CIOT_RPT( msg_id ) \
    ((msg_id == MM_CM_ESM_DATA_TRANSPORT_IND) || \
     (msg_id == MM_CM_ESM_DATA_TRANSPORT_CNF))
...
/* Macro to check if MO exception is started/end
*/
#define CM_MSGR_MO_EXCEPTION_IND( msg_id ) \
                            ( msg_id == DS_3GPP_MO_EXP_ACTIVITY_STATUS_IND )
...
#define CM_MSGR_MSG_PSM( msg_id )\
                           ( MSGR_TECH_MODULE_VAL(msg_id) == MSGR_MM_CM_PSM)
#ifdef FEATURE_REF_TRACKER 				
		   
#define CM_MSGR_MSG_SNS_MGR(msg_id)\
	                       ( MSGR_TECH_MODULE_VAL(msg_id) == MSGR_MM_CM_SNS_MGR)
#endif
