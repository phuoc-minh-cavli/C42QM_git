...
$Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmipapp.c#1 $
...
)
{
  ...
  switch (cmcall_end_cause)
  {
    ...
    case CM_CALL_END_LPM:             
      cmipcall_end_cause = CMIPAPP_END_CAUSE_OFFLINE;
      break;

    case CM_CALL_END_DRB_REJ:             
      cmipcall_end_cause = CMIPAPP_END_CAUSE_DRB_REJ;
      break;

    case CM_CALL_END_IP_UNWANTED_CALL:
      cmipcall_end_cause = CMIPAPP_END_CAUSE_UNWANTED_CALL;
      break;
                          ...
                          )
{
  ...
  */
  ...
  /*reset ECMP to 3GPP*/
  cmipapp_arr[app_id].ecmp  = CMIPAPP_ECMP_3GPP;
...
#if !defined FEATURE_MMODE_DISABLE_LTE_VOICE || defined FEATURE_MMODE_USS_PS_SUPS || defined FEATURE_MMODE_SS_SUPS
/*===========================================================================
                                       ...
                                       /*lint -esym(715,ecmp) */
{
  ...
  */
  if(app_id == CMIPAPP_ID_INVALID ||
     app_id >= ARR_SIZE(cmipapp_arr))
  {
    CM_MSG_HIGH_1("app_id %d is not valid", app_id);
    return CMIPAPP_STATUS_CALL_TYPE_REG_FAILED;
  }
  ...
  /* Sanity check on call_type and system mode
  */
  CM_ASSERT (BETWEEN (ecmp, CMIPAPP_ECMP_NONE,
    CMIPAPP_ECMP_MAX)); /*lint !e641 */

  /* Access the table to set call_type and system mode
  */
  rex_enter_crit_sect(cmipapp_crit_sect_ptr());
  /* Serialize data access cmipapp_arr[] */
  cmipapp_arr[app_id].ecmp  =  ecmp;
  rex_leave_crit_sect(cmipapp_crit_sect_ptr());

  CM_MSG_HIGH_2 (" App %d registers for ECMP %d",
    app_id, ecmp);
...
{ 
  ...
  /* Check for invalid app id
  */
  if (app_id == CMIPAPP_ID_INVALID || app_id >= ARR_SIZE(cmipapp_arr))
  {
...
{ 
#if defined(FEATURE_IP_CALL) 
  cmipapp_ind_s_type      *ip_ind=NULL;
  /* cm ipapp report */
  ...
  */
  CM_ASSERT_ENUM_IS_INRANGE (app_run_status, CMIPAPP_RUN_STATUS_MAX); /*lint !e641 */
   ...
   /* Intialization for ip_ind */
  ip_ind=(cmipapp_ind_s_type *)cm_mem_malloc(sizeof(cmipapp_ind_s_type));
  

  ip_ind->hdr.asubs_id = SYS_MODEM_AS_ID_1;
...
RETURN VALUE
TRUE            - if app_id is allocated already and copies cb

SIDE EFFECTS

Modifies global cmipapp_arr[].
===========================================================================*/
boolean cmipapp_get_cb(
...
{
  if( app_id >= ARR_SIZE(cmipapp_arr)||cmipapp_arr[app_id].app_id == CMIPAPP_ID_INVALID)
  {
    return FALSE;
  }

  *app_cb_ptr = cmipapp_arr[app_id].app_cb;
 ...
 )
{
  ...
  case CM_IP_CALL_IND_SUPS_CONF:
    ...
    if(cmipapp_rpt_ptr->ip_ind.ind.sups_conf.end_reason != NULL &&
	  cmipapp_rpt_ptr->ip_ind.ind.sups_conf.sups_type != CM_CALL_SUPS_TYPE_RESUME_PER_CALL &&
	  cmipapp_rpt_ptr->ip_ind.ind.sups_conf.sups_type != CM_CALL_SUPS_TYPE_HOLD_CALL &&
	  cmipapp_rpt_ptr->ip_ind.ind.sups_conf.sups_type != CM_CALL_SUPS_TYPE_HOLD_PER_CALL
    )
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.sups_conf.end_reason);
      CM_MSG_MED_0("cmippap_add_ref :Added ref for sups conf");
    }
  ...
  case CM_IP_CALL_IND_MT_INVITE:
    ...
    {
        CM_MSG_HIGH_0("cmippap_add_ref :Added ref for history_info");
        ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.mt_invite.call_attrib_info.history_info);
    }
   ...
   case CM_IP_SUPS_INTERROGATE_SS_CONF:
	if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.call_barring_num_info_list != NULL)
	{
	   ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.interrogate_ss_res.call_barring_num_info_list);
        ...
        if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.sups_ss_error_text != NULL)
        {
          ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.sups_ss_error_text);
        }
	break;
   ...
   case CM_IP_CALL_IND_CALL_END:
	if(cmipapp_rpt_ptr->ip_ind.ind.call_end.end_reason != NULL)
    {
      ...
      CM_MSG_MED_0("cmippap_add_ref :Added ref for end reason");
        ...
        {
           ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.call_end.alternate_uri);
           CM_MSG_MED_0("cmippap_add_ref :Added ref for alternate URI");
	}
	break;
  ...
  case CM_IP_CALL_IND_CONNECTED:
    ...
    {
      CM_MSG_HIGH_0("cmippap_add_ref :Added ref for history_info");
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.connected.call_attrib.history_info);
    }
    break;
  ...
  case CM_IP_CALL_IND_MO_PROGR_INFO:
        ...
        (cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.progr == CMIPAPP_PROG_MO_RINGBACK) )
    {
      ...
      {
        CM_MSG_HIGH_0("cmippap_add_ref :Added ref for history_info");
        ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.mo_progr_info.history_info);
      }
    }
    break;

  case CM_IP_CALL_IND_CALL_HOLD:
    if(cmipapp_rpt_ptr->ip_ind.ind.hold.end_reason != NULL)
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.hold.end_reason);
      CM_MSG_MED_0("cmippap_add_ref :Added ref for hold reason");
    }
    break;

  case CM_IP_CALL_IND_CALL_RETRIEVE:
    if(cmipapp_rpt_ptr->ip_ind.ind.retrieve.end_reason != NULL)
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.retrieve.end_reason);
      CM_MSG_MED_0("cmippap_add_ref :Added ref for retrieve reason");

    }
    break;
	
  case CM_IP_SUPS_UNSTRUCTURED_SS_IND:
	if(cmipapp_rpt_ptr->ip_ind.ind.unstructuresd_ss_ind.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.unstructuresd_ss_ind.sups_ss_error_text);
    }
    break;

  case CM_IP_SUPS_RELEASE_UNSTRUCTURED_SS_IND:
	if(cmipapp_rpt_ptr->ip_ind.ind.rel_unstructured_ss_ind.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.rel_unstructured_ss_ind.sups_ss_error_text);
    }
    break;

  case CM_IP_SUPS_PROCESS_UNSTRUCTURED_SS_CONF:
	if(cmipapp_rpt_ptr->ip_ind.ind.process_unstructured_ss_conf.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_add_ref(cmipapp_rpt_ptr->ip_ind.ind.process_unstructured_ss_conf.sups_ss_error_text);
    }
    break;
...
FUNCTION cmipapp_release_reference_alternate_uri
...
===========================================================================*/
void cmipapp_release_reference_alternate_uri
(
  cmcall_s_type           *call_ptr
)
{
  ...
  if(call_ptr!= NULL)
  {
    if( call_ptr->cmcall_mode_info.info.ip_info.alternate_uri!= NULL)
    {
      CM_MSG_HIGH_0("Released ref for alternate_uri");
      REF_CNT_OBJ_RELEASE_IF(call_ptr->cmcall_mode_info.info.ip_info.alternate_uri);
    }
  }
}
 ...
 )
{
  ...
  case CM_IP_SUPS_INTERROGATE_SS_CONF:
	...
	if(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.interrogate_ss_conf.sups_ss_error_text);
    }
    break;
  ...
  case CM_IP_SUPS_UNSTRUCTURED_SS_IND:
	if(cmipapp_rpt_ptr->ip_ind.ind.unstructuresd_ss_ind.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.unstructuresd_ss_ind.sups_ss_error_text);
    }
    break;
  ...
  case CM_IP_SUPS_RELEASE_UNSTRUCTURED_SS_IND:
	 if(cmipapp_rpt_ptr->ip_ind.ind.rel_unstructured_ss_ind.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.rel_unstructured_ss_ind.sups_ss_error_text);
    }
    break;
  ...
  case CM_IP_SUPS_PROCESS_UNSTRUCTURED_SS_CONF:
	if(cmipapp_rpt_ptr->ip_ind.ind.process_unstructured_ss_conf.sups_ss_error_text != NULL)
    {
      ref_cnt_obj_release(cmipapp_rpt_ptr->ip_ind.ind.process_unstructured_ss_conf.sups_ss_error_text);
    }
   break;	
  ...
  */
  ...
  #endif
  {
    ...
    */
    ...
    #endif
    if (ss_info_ptr->sys_mode == SYS_SYS_MODE_LTE)
    {
      srv_status.lte_srv_status = ss_info_ptr->srv_status;
    }
    else
    { 
      CM_MSG_MED_2("System mode  = %d Srv status = %d",
        ss_info_ptr->sys_mode,ss_info_ptr->srv_status);
    } 

  }

  cmd_common.asubs_id = SYS_MODEM_AS_ID_1;
  ...
  for (list_loop = 0; list_loop < CMIPAPP_POOL_SIZE; ++list_loop)
  {
    ...
    if(do_callback)
    {
      ...
      */
      ...
      CM_MSG_HIGH_2("CLNUP: send oprt_mode is_cb_success %d, app_id %d", \
...
/*===========================================================================
FUNCTION cmipapp_get_ecmp_val

DESCRIPTION
Return ecmp value of a particular app_id. If app_id is out of range return CMIPAPP_ECMP_NONE

RETURN VALUE
TRUE - cmipapp_ecmp_e_type
FALSE - NULL
===========================================================================*/
cmipapp_ecmp_e_type cmipapp_get_ecmp_val(cmipapp_id_type     app_id)
{
  if ( !(app_id < ARR_SIZE(cmipapp_arr)))     
  {
    CM_MSG_MED_1 ("Incorrect app id %d", app_id);
    return CMIPAPP_ECMP_3GPP;
  }

  return cmipapp_arr[app_id].ecmp;
}
                                ...
                                )
{
  ...
  ** to call_ptr */
  ...
  CM_MSG_HIGH_4 ("CM->IMS: CMIPAPP: Sending ANSWER, call_id %d, cause %d, is_accept %d, is_unwanted_call %d",
                call_id, answer_cause, accept, call_attrib.is_unwanted_call);
                                  ...
                                  )
{
  ...
  if(end_params.end_cause == CMIPAPP_END_CAUSE_UNWANTED_CALL)
  {
    end_params.is_unwanted_call = TRUE;
  }
  else
  {
    end_params.is_unwanted_call = FALSE;
  }
  ...
  if(sub_fn_ext_ptr)
  {
    CM_MSG_HIGH_5("CM->IMS: CMIPAPP: Sending IP END EXT, call_id  %d, app_id %d, as_id %d end_cause %d, unwanted_call %d",
                  call_id, app_id,cmd_common.asubs_id, end_params.end_cause, end_params.is_unwanted_call);

    return sub_fn_ext_ptr(call_id, cmipapp_arr[app_id].user_data_block, end_params, cmd_common);
  }
  else if(sub_fn_ptr)
  {
    CM_MSG_HIGH_4("CM->IMS: CMIPAPP: Sending IP END, call_id  %d, app_id %d, as_id %d, unwanted_call %d",
                  call_id, app_id,cmd_common.asubs_id, end_params.is_unwanted_call);
    return sub_fn_ptr(call_id, cmipapp_arr[app_id].user_data_block, cmd_common);
  }
  else
  {
    CM_MSG_HIGH_3("CM->IMS: CMIPAPP: Sending IP END, call_id  %d, app_id %d, unwanted_call %d",
                  call_id, app_id, end_params.is_unwanted_call);
...
{   
  ...
  cmipapp_ind_s_type          *ipapp_ind=NULL;
    /* Report being sent to CM
    */
  ...
  */ 
  ipapp_ind=(cmipapp_ind_s_type *)cm_mem_malloc(sizeof(cmipapp_ind_s_type));
...
)
{
  ...
  {    
    ...
    sys_modem_as_id_e_type asubs_id = cmph_get_sub_with_lte_cap();

    subsc_chgnd_ind.subs_cmd = subs_cmd;    
    subsc_chgnd_ind.as_id = asubs_id;
    subsc_chgnd_ind.msg_hdr.inst_id= SYS_AS_ID_TO_INST_ID(asubs_id);
    ...
    CM_MSG_HIGH_3("CM->MSGR: MM_CM_SUBSC_CHGND_IND subs_cmd=%d, asubs_id=%d, subs_id_bitmask %d",
                     subs_cmd, asubs_id, subs_id_bitmask);    
  }
