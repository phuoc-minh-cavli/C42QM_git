...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmutil.c#1 $
...
#if defined(FEATURE_MMODE_USS_SUPS) || FEATURE_MMODE_SS_SUPSs
...
uint8 cmutil_get_sys_bcmcs_max_flows()
{
  ...
  return (uint8)SYS_BCMCS_MAX_FLOWS;
...
{
  if(gsm7_char >= (sizeof(gsm7_to_utf16)/sizeof(gsm7_to_utf16[0])))
    return gsm7_to_utf16[0];
...
)
{
  ...
  (void) cm_util_gsm7_to_iso8859(
            ...
            ascii_string,
            MAX_DISPLAY_TEXT_LEN, /* need *some* length */
            &num_converted
            );
...
)
{
  ...
  memset(&default_bearer_capability, 0, sizeof(cm_bearer_capability_s_type));
...
)
{
   ...
   */
...
#else
	     /* octet 3b */
     bc->gsm_speech_ver_pref_2                    = GSM_INVALID_SPEECH_VER;  /* GSM Enhanced Full Rate  */

     /* octet 3c */
     bc->gsm_speech_ver_pref_3                    = GSM_INVALID_SPEECH_VER;  /* GSM FR */
#endif
   ...
   else
   {
     ...
     bc->gsm_speech_ver_pref_2                    = GSM_INVALID_SPEECH_VER; /* GSM Half Rate Version 0 or 1 */
     /* octet 3c */
#endif
     #ifdef FEATURE_GSM_HALF_RATE
#ifdef FEATURE_ENABLE_GSM_VOCODER
     bc->gsm_speech_ver_pref_3                    = GSM_HR_SPEECH_VER_1; /* GSM Half Rate Version 0 or 1 */
#else
	 bc->gsm_speech_ver_pref_3                    = GSM_INVALID_SPEECH_VER; /* GSM Half Rate Version 0 or 1 */
#endif
     #else
     bc->gsm_speech_ver_pref_3                    = GSM_INVALID_SPEECH_VER;
...
)
{
  ...
  if((sups_str_len > CM_MAX_NUMBER_CHARS) || \
...
uint8 cmutil_num_of_subs_avail(void)
{
  ...
  #else
  CM_MSG_HIGH_2("oprt_mode_online: num_of_subs_avail %d is_gwl_sub_avail %d",
    num_of_subs_avail,
    ph_ptr->is_gwl_subscription_available);
...
boolean cmutil_all_card_avail(void)
{
  ...
  mmgsdi_available_apps_type     *card_info=NULL;
  
  card_info=(mmgsdi_available_apps_type *)cm_mem_malloc(sizeof(mmgsdi_available_apps_type));
...
FUNCTION cm_util_map_sys_sys_mode_to_sd_ss_mode_pref

DESCRIPTION
  This function maps the SYS_SYS_MODE_* to SD_SS_MODE_PREF_*.
...
===========================================================================*/
sd_ss_mode_pref_e_type cm_util_map_sys_sys_mode_to_sd_ss_mode_pref( sys_sys_mode_e_type sys_mode )
{
  if( sys_mode == SYS_SYS_MODE_GW )
  {
    return SD_SS_MODE_PREF_GW;
  }
  if( sys_mode == SYS_SYS_MODE_GWL )
  {
    return SD_SS_MODE_PREF_GWL;
  }
  return (sd_ss_mode_pref_e_type)BM(sys_mode);
}
