...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmph.h#1 $
...
/* Default ACQ FAIL count to enter full RAT in case not configured through NV */
#define DEFAULT_FULL_RAT_ACQ_FAIL_CNT 10
#endif

/* LTE CIOT NV 73912 version corresponding to LTE 256 bands structure */
#define LTE_CIOT_NV_BAND_256_VER 2
...
typedef enum cmph_pref_nochg_e{
  ...
  CMPH_PREF_NOCHG_LTE_IOT_OP_MODE,
  CMPH_PREF_NOCHG_LTE_M1_BAND_PREF,
  CMPH_PREF_NOCHG_LTE_NB1_BAND_PREF,
...
/* Enumeration PSM context restore state */
typedef enum cmph_psm_context_restore_status_e {

  CMPH_PSM_CONTEXT_RESTORE_STATUS_NONE,
    /* CM has not recvd PSM context resotre status */

  CMPH_PSM_CONTEXT_RESTORE_STATUS_SUCCESS,
    /* CM has recvd PSM context restore status from DS with success */

  CMPH_PSM_CONTEXT_RESTORE_STATUS_FAILED,
    /* CM has recvd PSM context restore status from DS with failure */

  CMPH_PSM_CONTEXT_RESTORE_STATUS_MAX

}cmph_psm_context_restore_status_e_type;
...
typedef struct cmph_stack_info_s {
  ...
  */
  ...
  sys_loc_wwan_scan_result_s_type      *gps_scan_result;
...
typedef struct cmph_dbg_info {
  boolean          is_trm_svlte;
  /* Store the trm svlte capability read at CM initiatlization */

  boolean          is_trm_svdo;
  /* Store the trm svdo capability read at CM initiatlization */
...
typedef struct cmph_config_s{
  ...
  /**< Scan Scope rule: FULL_BAND/ACQ_DB for BPLMN/OOS scans */
  sys_scan_scope_rule_e_type         scan_scope_rule;
...
typedef struct cmph_s  {
  ...
  #if defined(FEATURE_MMODE_SUPPORT_SUPS)
  ...
  */
  ...
  uint8                            full_rat_acq_fail_cnt;
      /**< CIOT Acq fail count to enter Full RAT */
      
  uint8                            current_acq_fail_cnt;
      /**< CIOT Current Acq Fail count */
      
  #endif

  boolean                          bootup_nv_read_complete;
  cmph_psm_context_restore_status_e_type psm_context_restore_status;
  uint8       gps_wwan_scan_act_mask;
  boolean     is_gps_search_high_priority;
  boolean     is_gps_search_mplmn_abort_event_sent;
  dword                          gps_wwan_scan_up_time;
  boolean    is_wwan_suspend_req_send;
  #ifdef FEATURE_FMD_SPEED_INFO
  boolean is_oprt_modes_pseudo_online ;
  #endif
...
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
...
#else
#define CMPH_SS_FEATURE_MODE(ss, ph_ptr)\
                   (ph_ptr->main_stack_info.feature_mode) /*lint -restore */
#endif
...
#else
#define CMPH_SS_DISABLE_CALL_TYPE_MASK( asubs_id, ph_ptr ) \
                                            (ph_ptr->main_stack_info.disable_call_type_mask)
...
#define CMPH_SET_PRST_HYBR_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
            ...
            ph_ptr->prst_main_stack_info[ CM_NAM_1 ].hybr_pref = val; }\
          else {\
...
#define CMPH_SET_PRST_HYBR_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].hybr_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].hybr_pref = val; }
#else
#define CMPH_SET_PRST_HYBR_PREF( asubs_id, ph_ptr, val ) \
             (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].hybr_pref = val)
...
#define CMPH_PRST_HYBR_PREF( ph_ptr ) \
                     ( ph_ptr->prst_main_stack_info[ CM_NAM_1 ].hybr_pref )
...
#define CMPH_SS_LTE_IOT_OP_MODE( asubs_id, ph_ptr ) \
   ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?ph_ptr->hybr_3_stack_info.pref_info.ciot_lte_pref.lte_op_mode_pref:\
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
           ph_ptr->main_stack_info.pref_info.ciot_lte_pref.lte_op_mode_pref: \
                        ph_ptr->hybr_2_stack_info.pref_info.ciot_lte_pref.lte_op_mode_pref\
                     )) /*lint -restore */
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
/* Macro to return a pointer to the mode preference of requested ss */
#define CMPH_PRST_SS_LTE_IOT_OP_MODE( asubs_id, ph_ptr ) \
  ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?ph_ptr->prst_hybr_3_stack_info[ph_ptr->curr_nam].ciot_lte_pref.lte_op_mode_pref :\
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
           ph_ptr->prst_main_stack_info[ph_ptr->curr_nam].ciot_lte_pref.lte_op_mode_pref: \
                        ph_ptr->prst_hybr_2_stack_info[ph_ptr->curr_nam].ciot_lte_pref.lte_op_mode_pref\
                     )) /*lint -restore */
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the mode preference of requested asubs_id */
...
#else
#define CMPH_PRST_SS_MODE_PREF( asubs_id, ph_ptr ) \
                  (ph_ptr->prst_main_stack_info[CM_NAM_1].mode_pref)
#endif
...
#define CMPH_SET_PRST_MODE_PREF( asubs_id, ph_ptr, val ) \
            if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ...
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].mode_pref = val; }\
                    else {\
...
#define CMPH_SET_PRST_MODE_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].mode_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].mode_pref = val; }
#else
#define CMPH_SET_PRST_MODE_PREF( asubs_id, ph_ptr, val ) \
              (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].mode_pref = val)
...
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
                  ph_ptr->prst_main_stack_info[ CM_NAM_1 ].user_net_sel_mode:\
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
/* Macro to return a pointer to the LTE band preference of requested ss */
#define CMPH_SS_CIOT_LTE_PREF( asubs_id, ph_ptr ) \
	                     ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?ph_ptr->hybr_3_stack_info.pref_info.ciot_lte_pref:\
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
...
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the LTE band preference of requested asubs_id */
#define CMPH_SS_CIOT_LTE_PREF( asubs_id, ph_ptr ) \
/*lint -save -e506 */( (asubs_id != SYS_MODEM_AS_ID_2)? \
                        ph_ptr->main_stack_info.pref_info.ciot_lte_pref: \
                        ph_ptr->hybr_2_stack_info.pref_info.ciot_lte_pref \
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
/* Macro to return a pointer to the LTE band preference of requested ss */
#define CMPH_PRST_CIOT_LTE_PREF( asubs_id, ph_ptr ) \
	                     ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref:\
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref: \
                        ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref \
                     )) /*lint -restore */
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the LTE band preference of requested asubs_id */
...
/* Macro to return a pointer to the LTE wideband band preference of requested ss */
#define CMPH_SS_LTE_WB_BAND_PREF( asubs_id, ph_ptr ) cmph_get_ss_lte_wb_band_pref( asubs_id, ph_ptr )


/* Macro to return a pointer to the LTE M1 band preference of requested ss */
#define CMPH_SS_LTE_M1_BAND_PREF( asubs_id, ph_ptr ) cmph_get_ss_lte_m1_band_pref( asubs_id, ph_ptr )

/* Macro to return a pointer to the LTE NB1 band preference of requested ss */
#define CMPH_SS_LTE_NB1_BAND_PREF( asubs_id, ph_ptr ) cmph_get_ss_lte_nb1_band_pref( asubs_id, ph_ptr )
...
#define CMPH_SET_PRST_BAND_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ...
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].band_pref = val; }\
                    else {\
...
#define CMPH_SET_PRST_BAND_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].band_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].band_pref = val; }
#else
#define CMPH_SET_PRST_BAND_PREF( asubs_id, ph_ptr, val ) \
            (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].band_pref  = val)
...
#define CMPH_SET_PRST_LTE_BAND_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ...
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].lte_band_pref = val; }\
                    else {\
...
#define CMPH_SET_PRST_LTE_BAND_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].lte_band_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].lte_band_pref = val; }
#else
#define CMPH_SET_PRST_LTE_BAND_PREF( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].lte_band_pref  = val)
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_SET_PRST_LTE_M1_BAND_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref = val; }\
                    else if (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2) {\
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_m1_band_pref = val; }\
                    else {\
                        ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref = val; }
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the persistent band preference
** of the current NAM.
...
#define CMPH_SET_PRST_LTE_M1_BAND_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_m1_band_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref = val; }
#else
#define CMPH_SET_PRST_LTE_M1_BAND_PREF( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_m1_band_pref  = val)
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_SET_PRST_LTE_NB1_BAND_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref = val; }\
                    else if (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2) {\
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_nb1_band_pref = val; }\
                    else {\
                        ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref = val; }
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the persistent band preference
** of the current NAM.
...
#define CMPH_SET_PRST_LTE_NB1_BAND_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_nb1_band_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref = val; }
#else
#define CMPH_SET_PRST_LTE_NB1_BAND_PREF( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_nb1_band_pref  = val)
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_SET_PRST_LTE_IOT_OP_MODE( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_op_mode_pref = val; }\
                    else if (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2) {\
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_op_mode_pref = val; }\
                    else {\
                        ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_op_mode_pref = val; }
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
/* Macro to return a pointer to the persistent band preference
** of the current NAM.
...
#define CMPH_SET_PRST_LTE_IOT_OP_MODE( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_op_mode_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_op_mode_pref = val; }
#else
#define CMPH_SET_PRST_LTE_IOT_OP_MODE( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_op_mode_pref  = val)
...
#define CMPH_SET_PRST_TDS_BAND_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
                        ...
                        ph_ptr->prst_main_stack_info[ CM_NAM_1 ].tds_band_pref = val; }\
                    else {\
...
#define CMPH_SET_PRST_TDS_BAND_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].tds_band_pref  = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].tds_band_pref = val; }
#else
#define CMPH_SET_PRST_TDS_BAND_PREF( asubs_id, ph_ptr, val ) \
          (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].tds_band_pref  = val)
...
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 ?\
             ph_ptr->prst_main_stack_info[ CM_NAM_1 ].band_pref:\
...
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 ?\
             ph_ptr->prst_main_stack_info[ CM_NAM_1 ].lte_band_pref:\
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_PRST_SS_LTE_M1_BAND_PREF( asubs_id, ph_ptr ) \
	      ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?\
	            ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref:\
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 ?\
             ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_m1_band_pref:\
             ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref\
        )) /*lint -restore */
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
#define CMPH_PRST_SS_LTE_M1_BAND_PREF( asubs_id, ph_ptr ) \
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_PRST_SS_LTE_NB1_BAND_PREF( asubs_id, ph_ptr ) \
	      ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?\
	            ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref:\
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 ?\
             ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_nb1_band_pref:\
             ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref\
        )) /*lint -restore */
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
#define CMPH_PRST_SS_LTE_NB1_BAND_PREF( asubs_id, ph_ptr ) \
...
#ifdef FEATURE_MMODE_HYBR3_ENABLED
#define CMPH_PRST_SS_CIOT_NV_VERSION( asubs_id, ph_ptr ) \
	      ( (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3)?\
	            ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref_version :\
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 ?\
             ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref_version:\
             ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref_version\
        )) /*lint -restore */
#elif defined(FEATURE_MMODE_DUAL_SIM) || defined (FEATURE_MMODE_SC_SVLTE )\
        || defined(FEATURE_MMODE_SC_SGLTE)
#define CMPH_PRST_SS_CIOT_NV_VERSION( asubs_id, ph_ptr ) \
...
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
           ph_ptr->prst_main_stack_info[ CM_NAM_1 ].tds_band_pref: \
...
#define CMPH_PRST_LTE_M1_BAND_PREF( ph_ptr ) \
                     ( ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_m1_band_pref )

#define CMPH_PRST_LTE_NB1_BAND_PREF( ph_ptr ) \
                     ( ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_nb1_band_pref )

#define CMPH_PRST_LTE_IOT_OP_MODE( ph_ptr ) \
                     ( ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ciot_lte_pref.lte_op_mode_pref )
...
/*lint -save -e506 */((cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2) \
                   ?ph_ptr->prst_main_stack_info[ CM_NAM_1 ].ue_usage_setting\
...
/*lint -save -e506 */((cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2) \
                   ?ph_ptr->prst_main_stack_info[ CM_NAM_1 ].voice_domain_pref\
...
#define CMPH_SET_PRST_PRL_PREF( asubs_id, ph_ptr, val ) \
      if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
       ...
       ph_ptr->prst_main_stack_info[ CM_NAM_1 ].prl_pref = val; }\
      else {\
...
#define CMPH_SET_PRST_PRL_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].prl_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].prl_pref = val; }
...
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
              ph_ptr->prst_main_stack_info[ CM_NAM_1 ].prl_pref:\
...
#define CMPH_SET_PRST_ROAM_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
            ...
            ph_ptr->prst_main_stack_info[ CM_NAM_1 ].roam_pref = val; }\
          else {\
...
#define CMPH_SET_PRST_ROAM_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].roam_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].roam_pref = val; }
#else
#define CMPH_SET_PRST_ROAM_PREF( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].roam_pref = val)
...
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
                  ph_ptr->prst_main_stack_info[ CM_NAM_1 ].roam_pref:\
...
#define CMPH_SET_PRST_ACQ_ORDER_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
            ...
            ph_ptr->prst_main_stack_info[ CM_NAM_1 ].acq_order_pref = val; }\
          else {\
...
#define CMPH_SET_PRST_ACQ_ORDER_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].acq_order_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].acq_order_pref = val; }
#else
#define CMPH_SET_PRST_ACQ_ORDER_PREF( asubs_id, ph_ptr, val ) \
          (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].acq_order_pref = val)
...
/*lint -save -e506 */( (cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2)? \
              ph_ptr->prst_main_stack_info[ CM_NAM_1 ].acq_order_pref:\
...
#define CMPH_SET_PRST_NETWORK_SEL_MODE_PREF( asubs_id, ph_ptr, val ) \
        if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
          ...
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].network_sel_mode_pref  = val; } \
        else {\
...
#define CMPH_SET_PRST_NETWORK_SEL_MODE_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].network_sel_mode_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].network_sel_mode_pref = val; }
#else
#define CMPH_SET_PRST_NETWORK_SEL_MODE_PREF( asubs_id, ph_ptr, val ) \
        (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].network_sel_mode_pref = val)
...
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 \
             ?ph_ptr->prst_main_stack_info[ CM_NAM_1 ].network_sel_mode_pref\
...
#define CMPH_SET_PRST_SRV_DOMAIN_PREF( asubs_id, ph_ptr, val ) \
          if (cmph_map_subs_to_ss(asubs_id) == CM_SS_HYBR_3) {\
            ...
            ph_ptr->prst_main_stack_info[ CM_NAM_1 ].srv_domain_pref = val; } \
          else {\
...
#define CMPH_SET_PRST_SRV_DOMAIN_PREF( asubs_id, ph_ptr, val ) \
        if (asubs_id != SYS_MODEM_AS_ID_2) {\
          ph_ptr->prst_main_stack_info[ CM_NAM_1 ].srv_domain_pref = val; } else {\
          ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].srv_domain_pref = val; }
#else
#define CMPH_SET_PRST_SRV_DOMAIN_PREF( asubs_id, ph_ptr, val ) \
         (ph_ptr->prst_main_stack_info[ CM_NAM_1 ].srv_domain_pref = val)
...
/*lint -save -e506 */(cmph_map_subs_to_ss(asubs_id) != CM_SS_HYBR_2 \
            ?ph_ptr->prst_main_stack_info[ CM_NAM_1 ].srv_domain_pref\
...
#define CMPH_PRST_HYBR_2_LTE_M1_BAND_PREF( ph_ptr ) \
                 (ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref)

#define CMPH_PRST_HYBR_2_LTE_NB1_BAND_PREF( ph_ptr ) \
                 (ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref)

#define CMPH_PRST_HYBR_2_LTE_IOT_OP_MODE( ph_ptr ) \
                 (ph_ptr->prst_hybr_2_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_op_mode_pref )
...
#define CMPH_PRST_HYBR_3_LTE_M1_BAND_PREF( ph_ptr ) \
                 (ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_m1_band_pref)

#define CMPH_PRST_HYBR_3_LTE_NB1_BAND_PREF( ph_ptr ) \
                 (ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_nb1_band_pref)

#define CMPH_PRST_HYBR_3_LTE_IOT_OP_MODE( ph_ptr ) \
                 (ph_ptr->prst_hybr_3_stack_info[ ph_ptr->curr_nam ].ciot_lte_pref.lte_op_mode_pref )
...
FUNCTION cmph_map_cm_lte_op_mode_to_sd_lte_op_mode

DESCRIPTION
  Map CM lte op mode-preference type to SD lte op mode-preference type.

DEPENDENCIES
  none

RETURN VALUE
  The SD lte op mode-preference that corresponds to the input CM lte op mode-preference.

SIDE EFFECTS
  none

===========================================================================*/
sd_ss_ciot_lte_op_mode_e_type   cmph_map_cm_lte_op_mode_to_sd_lte_op_mode(

    cm_ciot_lte_op_mode_e_type lte_op_mode
        /* Mode pref to map */
);

/*===========================================================================

FUNCTION cmph_map_sd_lte_op_mode_to_cm_lte_op_mode

DESCRIPTION
  Map SD lte op mode-preference type to CMlte op mode-preference type.

DEPENDENCIES
  none

RETURN VALUE
  The CM lte op mode-preference that corresponds to the input SD lte op mode-preference.

SIDE EFFECTS
  none

===========================================================================*/
 cm_ciot_lte_op_mode_e_type  cmph_map_sd_lte_op_mode_to_cm_lte_op_mode(

    sd_ss_ciot_lte_op_mode_e_type  lte_op_mode
        /* Mode pref to map */
);
...
FUNCTION cmph_map_sd_mode_to_sys_sys_mode

DESCRIPTION
  Map SD mode type to CM SYS mode type.
...
===========================================================================*/
extern sys_sys_mode_e_type cmph_map_sd_mode_to_sys_sys_mode(sd_mode_e_type  mode);
...
FUNCTION cmph_mo_execption_event_proc

DESCRIPTION
  Processes MO exception indications.
...
extern void cmph_mo_exception_ind_proc(boolean mo_exception_ind);
...
FUNCTION cmph_map_cm_ss_to_rat_acq_order_subs

DESCRIPTION
  Maps CM SS to the active subscription id, also checks that the sub id is within the range of
  rat_acq_order array.
...
===========================================================================*/
sys_modem_as_id_e_type cmph_map_cm_ss_to_rat_acq_order_subs(
  cm_ss_e_type       ss
);

/*===========================================================================

FUNCTION cmph_map_as_id_to_rat_acq_order_subs

DESCRIPTION
  Returns as_id which is in the range of rat_acq_order array
...
===========================================================================*/
sys_modem_as_id_e_type cmph_map_as_id_to_rat_acq_order_subs(
  sys_modem_as_id_e_type as_id
);
...
#endif /* #if !defined(FEATURE_MMODE_DISABLE_CSG) */
...
/*===========================================================================
FUNCTION cmph_process_context_save_rsp

DESCRIPTION
Process mmgsdi context save response
...
===========================================================================*/
void cmph_process_context_save_rsp( void );

#if defined(FEATURE_CIOT)
/*===========================================================================
FUNCTION cmph_update_current_acq_fail_cnt

DESCRIPTION
 Updates cmph_update_current_acq_fail_cnt
...
===========================================================================*/
void cmph_update_current_acq_fail_cnt( boolean is_reset );

/*===========================================================================
FUNCTION cmph_check_and_enter_full_rat_mode

DESCRIPTION
 Enforce full RAT if current_acq_fail_cnt reaches full_rat_acq_fail_cnt
...
===========================================================================*/
boolean cmph_check_and_enter_full_rat_mode( void);

/*===========================================================================
FUNCTION cmph_increment_acq_fail_cnt

DESCRIPTION
 Increase acq_fail cnt
 Write acq_fail cnt to NV
 Enforce full RAT if current_acq_fail_cnt reaches full_rat_acq_fail_cnt
...
===========================================================================*/
void cmph_increment_acq_fail_cnt(
    boolean         is_scan_attempted
);

/*===========================================================================
FUNCTION cmph_check_and_exit_full_rat_mode

DESCRIPTION
 if device is in Full RAT, enforce Full RAT exit
...
===========================================================================*/
boolean cmph_check_and_exit_full_rat_mode( boolean force_pref );

/*===========================================================================
FUNCTION cmph_cmd_set_location_info

DESCRIPTION
 Send user mcc to NAS and force pref if device is in no_srv
...
===========================================================================*/
void cmph_cmd_set_location_info(
  cm_ph_cmd_s_type            *ph_cmd_ptr
);
...
FUNCTION cmph_get_ss_lte_wb_band_pref

DESCRIPTION
return the LTE wideband band pref on the sub.
...
===========================================================================*/
sys_lte_band_mask_e_type cmph_get_ss_lte_wb_band_pref(
...
FUNCTION cmph_get_ss_lte_m1_band_pref

DESCRIPTION
return the LTE M1 band mask on the sub.
...
===========================================================================*/
sys_lte_band_mask_e_type cmph_get_ss_lte_m1_band_pref(
...
FUNCTION cmph_get_ss_lte_nb1_band_pref

DESCRIPTION
return the LTE NB1 band mask on the sub.
...
===========================================================================*/
sys_lte_band_mask_e_type cmph_get_ss_lte_nb1_band_pref(
...
FUNCTION cmsns_send_motion_status_from_nv

DESCRIPTION
  To send the  motion status to APP from NV setting
...
===========================================================================*/
void cmsns_send_motion_status_from_nv(void);


void cmsns_enable_motion_in_full_image_for_testing(void);

void cmsns_mgr_run_state_machine(void);

/*===========================================================================

FUNCTION cmph_is_gps_wwan_high_priority_search_cmd

DESCRIPTION
  check if it is high priority search 
...
boolean cmph_is_gps_wwan_high_priority_search_cmd( cm_ph_cmd_s_type           *ph_cmd_ptr);

/*===========================================================================

FUNCTION cmph_process_buff_gps_cmd_after_all_call_end

DESCRIPTION
   it will process the buffered GPS command after call end
...
void cmph_process_buff_gps_cmd_after_all_call_end(void);

/*===========================================================================

FUNCTION cmph_get_ciot_rf_band_capability

DESCRIPTION
    get the RF bands from SD
...
boolean cmph_get_ciot_rf_band_capability(
