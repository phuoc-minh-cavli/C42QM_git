...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/cm/src/cmefs.c#1 $
...
static const cmefs_tbl_s_type cmefs_tbl[] =
{
  ...
  { CMEFS_MMODE_CIOT_LTE_PREF,          CMEFS_NV_MMODE_DIR "ciot_lte_preferences"},
  ...
  { CMEFS_SCAN_SCOPE_RULE,              CMEFS_NV_MMODE_DIR "scan_scope_rule"},
  ...
  { CMEFS_FULL_RAT_ACQ_FAIL_CNT,         CMEFS_NV_MMODE_DIR "full_rat_acq_fail_cnt"},
  { CMEFS_CURRENT_ACQ_FAIL_CNT,          CMEFS_NV_MMODE_DIR "current_acq_fail_cnt"},
  { CMEFS_RPM_FEATURE_ENABLED,           CMEFS_NV_MMODE_DIR "rpm_feature_enabled"},
  { CMEFS_PSM_RTC_CONFIG,                CMEFS_NV_MMODE_DIR "psm_rtc_config"},
  { CMEFS_PSM_CONTEXT,                   CMEFS_NV_MMODE_DIR "psm_context"},
  { CMEFS_LTE_SOFT_FAILURE_RETRY_COUNT,      CMEFS_NV_MMODE_DIR "lte_softfailure_count"},
  { CMEFS_PSM_CONFIG_EXT2,                   CMEFS_NV_MMODE_DIR "psm_config_ext_2"},
  { CMEFS_PSM_CONTEXT_EXT,                   CMEFS_NV_MMODE_DIR "psm_context_ext"},
  { CMEFS_SOI_CONTEXT,                   CMEFS_NV_MMODE_DIR "soi_context"},
  { CMEFS_LTE_RPM_INFO_2,               CMEFS_NV_MMODE_DIR "lte_rpm_info_2"},
  { CMEFS_MMODE_GPS_WWAN_SCAN,               CMEFS_NV_MMODE_DIR "wwan_scan_param"}
...
===========================================================================*/
mcfg_fs_sub_id_e_type cmefs_map_as_id_to_mcfg_id(
...
/nv/item_files/modem/mmode/ciot_lte_preferences\n\
/nv/item_files/modem/mmode/rpm_feature_enabled\n\
/nv/item_files/modem/mmode/sd/enhanced_f3_dbg\n\
/nv/item_files/modem/mmode/psm_rtc_config\n\
/nv/item_files/modem/mmode/lte_softfailure_count\n\
/nv/item_files/modem/mmode/full_rat_acq_fail_cnt\n\
/nv/item_files/modem/mmode/psm_config_ext_2\n\
/nv/item_files/modem/mmode/wwan_scan_param\n\
";
  ...
  */
  ...
  /* Read contents of current conf file, check if file already exists */
  conf_data = (char *)cm_mem_malloc(strlen(mmode_config_items));
  if((mcfg_fs_read(CMEFS_MMODE_CONF_F_NAME,
                conf_data, strlen(mmode_config_items), MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE)) == MCFG_FS_STATUS_OK)
    {
    /*If file is already present, check if contents are the same as what we want to write*/
    if(memcmp(conf_data, mmode_config_items, strlen(mmode_config_items)) == 0)
    {
      CM_MSG_HIGH_0("CONF FILE: mmode.conf contents are the same, skip writing to EFS");
      write_conf = FALSE;
    }
  }
  if(write_conf)
  {
    CM_MSG_HIGH_0("CONF FILE: write mmode.conf to EFS");
    if(mcfg_fs_write(CMEFS_MMODE_CONF_F_NAME,mmode_config_items,
                     strlen(mmode_config_items),MCFG_FS_O_CREAT|MCFG_FS_O_AUTODIR|MCFG_FS_O_TRUNC,
                     MCFG_FS_ALLPERMS, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE) != MCFG_FS_STATUS_OK)
    {
      CM_MSG_HIGH_1("CONF FILE: error writing to EFS %d", mcfg_fs_errno(MCFG_FS_TYPE_EFS));
    }
  }
  cm_mem_free(conf_data);
  return;
}
...
boolean                  cmefs_get2(
...
)
   {
	 ...
	 */
	 ...
	 if(status != MCFG_FS_STATUS_OK)
	 {
	   CM_MSG_HIGH_3("mcfg_fs_read error, sub_id %d, status %d, errno %d", sub_id, status,
						mcfg_fs_errno(MCFG_FS_TYPE_EFS));
	   return FALSE;
	 }
