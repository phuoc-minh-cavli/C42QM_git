...
void qm_tib_update_time_info( cm_mm_msim_ss_info_s_type *p_info )
{
  ...
  if(p_info)
  {
    ...
    #endif
    /* One of them would be TRUE, Either the PRI Sub has 1x or HDR*/
  }
}
...
boolean qm_tib_is_time_info_changed( cm_mm_msim_ss_info_s_type *p_info )
{
  ...
  if( p_info != NULL)
  {
    ...
    #endif
    #if defined(FEATURE_HDR)
    if( qm_util_is_in_service( QM_SUBS_PRI, QM_RAT_1X, p_info, FALSE, &stack ) && p_info->stack_info[stack].mode_info.cdma_info.time_info.sys_mode == SYS_SYS_MODE_HDR )
    {
...
boolean qm_tib_is_enabled( void )
{
  ...
  if( !tib_global.is_efs_read ) // we haven't read the timer interval yet, read EFS file and start timer
  {
    ...
    status = efs_get(TIB_EFS_FILE, (byte *)&tib_global.timer.interval, sizeof(uint32) );
