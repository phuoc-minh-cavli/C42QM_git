...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/qmi/src/qmi_voice_call_list.c#1 $ $DateTime:$ $Author:$
...
*/
/*=========================================================================*/
static uint32 qmi_voice_call_list_count_num_of_voice_calls_by_state
( 
  uint16 state_mask,
  uint8  as_id
)
...
{ 
  ...
  pub_ptr->is_sip_error_code_available = FALSE;
  memset(&pub_ptr->end_reason_text,0,sizeof(pub_ptr->end_reason_text));
  pub_ptr->is_ip_hist_info_available = FALSE;
  memset(&pub_ptr->ip_hist_info, 0, sizeof(pub_ptr->ip_hist_info));
  pub_ptr->is_alternate_uri_available = FALSE;
  memset(&pub_ptr->alternate_uri,0,sizeof(pub_ptr->alternate_uri));
  ...
  QM_MSG_HIGH_7( " num_len=%d, num=%s, num_pres =%d, name=%s, name_pres =%d,is_as_id_valid=%d, as_id=%d,",
                   pub_ptr->num.len, pub_ptr->num.buf, pub_ptr->num.pi, pub_ptr->cnap_info.name, pub_ptr->cnap_info.name_presentation,
  ...
  #else
  QM_MSG_HIGH_4( " a_attrib_valid=%d, a_attrib=%d, attrib_status_valid=%d, attrib_status=%d",
                   pub_ptr->is_audio_attrib_valid, pub_ptr->audio_attrib,
                   pub_ptr->is_attrib_status_valid, pub_ptr->attrib_status);
  #endif
...
)
{
  ...
  (void) QMI_VOICE_CM_IF_STR_PRINTF( label, sizeof( label ), "Call Type=%d call id %d: %s -> %s",
                   info_ptr->call_type , info_ptr->call_id, qmi_voice_call_list_lookup_state_name( info_ptr->prev_state ), 
                   qmi_voice_call_list_lookup_state_name( info_ptr->state ));
...
)
{
  ...
  for (i=0; i<call_info_list.num_of_calls; i++)
  {
       ...
       QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type))
    {
      ...
      #else
      QM_MSG_MED_5("Call Info: call_id: %d, peer: audio:%d, a_cause:%d, local: audio:%d, a_cause:%d",
                   info->call_info[i].call_id,
                   info->call_info[i].peer_cap.audio_attrib,
                   info->call_info[i].peer_cap.audio_restrict_cause,
                   info->call_info[i].local_cap.audio_attrib,
    ...
    if (call_data->is_sip_error_code_available== TRUE)
    {
      info->call_info[i].is_sip_error_code_available = TRUE;
      info->call_info[i].sip_error_code= call_data->sip_error_code;
      QM_MSG_HIGH_1("Call Info:Sip error code Len %d", info->call_info[i].sip_error_code);
    }
    ...
    if(call_data->is_called_party_ringing_status_avail)
    {
      ...
      info->call_info[i].is_called_party_ringing             = call_data->is_called_party_ringing;
    ...
    /* Only send ip_hist_info when call state is ALERTING/INCOMING/ACTIVE, and if it is an IP call */
    if (  ( (call_data->state == QMI_VOICE_CALL_LIST_STATE_ALERTING)||
             (call_data->state == QMI_VOICE_CALL_LIST_STATE_INCOMING) ||
             (call_data->state == QMI_VOICE_CALL_LIST_STATE_ACTIVE))   && 
          (call_data->is_ip_hist_info_available == TRUE) &&
          QMI_VOICE_CM_IF_VOIP_CALL(info->call_info[i].call_type) )
    {
      info->call_info[i].is_ip_hist_info_available = TRUE;
      info->call_info[i].ip_hist_info = call_data->ip_hist_info;
      QM_MSG_HIGH_1("Call Info:ip_hist_info Len %d", info->call_info[i].ip_hist_info.ip_hist_info_length);

      /* Reset the is_ip_hist_info_available flag after filling the current info buffer */
      call_data->is_ip_hist_info_available = FALSE;
    }

    if(call_data->is_unwanted_call_feature_available)
    {
      info->call_info[i].is_unwanted_call_feature_available = TRUE;
      info->call_info[i].is_unwanted_call.is_nw_support_unwanted_call = call_data->is_unwanted_call.is_nw_support_unwanted_call;
      info->call_info[i].is_unwanted_call.mt_call_verstat = call_data->is_unwanted_call.mt_call_verstat;
      QM_MSG_HIGH_2("Call info : nw_support_unwanted_call %d, mt_call_verstat %d",info->call_info[i].is_unwanted_call.is_nw_support_unwanted_call, info->call_info[i].is_unwanted_call.mt_call_verstat);
    }

    if(call_data->is_alternate_uri_available)
    {
      info->call_info[i].is_alternate_uri_available = TRUE;
      info->call_info[i].alternate_uri = call_data->alternate_uri;
      QM_MSG_HIGH_1("Call Info: Alternate URI Len %d", info->call_info[i].alternate_uri.alternate_uri_length);
    }
...
) 
{
  ...
  while ( buf_ptr != NULL )
  {
    if ( buf_ptr->pub.call_id == call_id )
    {
      ...
      memscpy(info_ptr, sizeof(qmi_voice_call_list_public_type), &buf_ptr->pub, sizeof(buf_ptr->pub));
      status = E_SUCCESS;
      break;
    }
    else
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    buf_ptr->pub.conn_index = (uint8)conn_index;
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_sip_error_code

===========================================================================*/
/*!
    @brief
    Update the sip error code in the list with given Call ID
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    QM_MSG_MED_2("qmi_voice_call_list_update_sip_error_code: call_id %d, sip_code %d",
                  call_id,sip_code
                   );
    buf_ptr->pub.sip_error_code= sip_code;
   
    buf_ptr->pub.is_sip_error_code_available= TRUE;
  }
  else
  {
    /* Call not in the list */
...
} /* qmi_voice_call_list_update_sip_error_code() */
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_alternate_uri

===========================================================================*/
/*!
    @brief
    Change the alternate URI of the entry in the list with given Call ID
...
)
{
  ...
  if (alternate_uri_ptr == NULL)
  {
    QM_MSG_ERROR("alternate_uri_ptr is NULL");
    return E_FAILURE;
  }
  ...
  if ( buf_ptr != NULL )
  {
    QM_MSG_MED_3("update_alternate_uri: call_id %d, current character count %d, new character count %d",
                 call_id,
                 buf_ptr->pub.alternate_uri.alternate_uri_length,
                 alternate_uri_ptr->uri_len );
    buf_ptr->pub.alternate_uri.alternate_uri_length = alternate_uri_ptr->uri_len;
    memscpy( buf_ptr->pub.alternate_uri.alternate_uri_buf, sizeof(buf_ptr->pub.alternate_uri.alternate_uri_buf),
             alternate_uri_ptr->uri, alternate_uri_ptr->uri_len );
    buf_ptr->pub.is_alternate_uri_available = TRUE;
  }
  else
  {
    /* Call not in the list */
...
} /* qmi_voice_call_list_update_alternate_uri() */
...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    qmi_voice_cm_util_get_ss_info(&util_sys_info);
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_mt_call_verstat_info

===========================================================================*/
/*!
    @brief
    updating the incoming call verstat info, informing if the call is a unwanted call or not. 
    Also updated is info about the nw, if the nw can handle the unwanted_call scenario.
...
*/
/*=========================================================================*/
IxErrnoType qmi_voice_call_list_update_mt_call_verstat_info
(
	cm_call_id_type call_id,
	cm_verstat_s_type unwanted_call_verstat
)
{
  IxErrnoType status = E_SUCCESS;
  qmi_voice_call_list_buf_type *buf_ptr = NULL;
  ...
  buf_ptr = qmi_voice_call_list_find( call_id );
    ...
    /* Log CList changes */
    QM_MSG_HIGH_3( "MT_unwanted_call info: call id %d, nw_support_unwanted_call -> %d, mt_call_verstat -> %d",
                   call_id, unwanted_call_verstat.is_nw_support_unwanted_call, unwanted_call_verstat.call_verstat);
  if ( buf_ptr != NULL )
  {
    if((unwanted_call_verstat.is_nw_support_unwanted_call) || (unwanted_call_verstat.call_verstat != CM_TN_VALIDATION_NONE))
    {
      buf_ptr->pub.is_unwanted_call_feature_available = TRUE;
      buf_ptr->pub.is_unwanted_call.is_nw_support_unwanted_call = unwanted_call_verstat.is_nw_support_unwanted_call;
      buf_ptr->pub.is_unwanted_call.mt_call_verstat = unwanted_call_verstat.call_verstat;
    }
  }
  else
  {
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_peer_call_capabilities

===========================================================================*/
/*!
    @brief
    Update the peer capabilities for the call
...
/*===========================================================================
  ...
  FUNCTION:  qmi_voice_call_list_update_ip_hist_info

===========================================================================*/
/*!
    @brief
    Update the IP history info for the call
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    if( (ip_hist_info_ptr!= NULL) &&
        (ip_hist_info_ptr->hist_info_text != NULL) &&
        (ip_hist_info_ptr->hist_info_len  != 0) )
    {
      buf_ptr->pub.is_ip_hist_info_available= TRUE;
      buf_ptr->pub.ip_hist_info.ip_hist_info_length = ip_hist_info_ptr->hist_info_len;
      memscpy(buf_ptr->pub.ip_hist_info.ip_hist_info_buf, sizeof(buf_ptr->pub.ip_hist_info.ip_hist_info_buf),
              ip_hist_info_ptr->hist_info_text, 2*ip_hist_info_ptr->hist_info_len);
    }
  }
  else
  {
  ...
  return status;
} /* qmi_voice_call_list_update_ip_hist_info() */
...
)
{
  ...
  if ( buf_ptr != NULL )
  {
    ...
    if(call_info_ptr->mode_info.info.ip_call.call_attrib_info.caller_name->caller_name_len > 0)
    {
      ...
      buf_ptr->pub.ip_caller_name.name_len = (uint8)call_info_ptr->mode_info.info.ip_call.call_attrib_info.caller_name->caller_name_len;
    }
    else
    {
...
)
{
  ...
  QM_MSG_MED_1("qmi_voice_call_list_get_callid_info : Not Found call list entry :  call id %d", call_id);
  return NULL;
