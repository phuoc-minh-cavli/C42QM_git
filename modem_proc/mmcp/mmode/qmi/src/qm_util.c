...
)
{
  ...
  fd =  efs_open(item_file_path, O_RDONLY);

  if ( fd < 0 )
  {
    QM_MSG_ERROR_1("open efs_err=%d", fd);
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Read data from file.*/
  retval = efs_read(fd, efs_info_ptr, (fs_size_t)efs_info_size);
...
)
{
  ...
  if ( out_buf != NULL )
  {
    ...
    while ( j < data_len )
    {
      ...
      {
        //3 to take care of 1 space + 1 Byte hex(2 Bytes in string)
        sn_size = snprintf( (char*)out_ptr, 3, " %X", data_ptr[j] );
        if ( sn_size > 0 )
        {
          out_ptr += sn_size;
          i += sn_size;
