...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/qmi/src/qm_mstats.c#1 $
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM) || defined(FEATURE_CDMA) || defined(FEATURE_HDR)
struct qm_mstats_s
{
  #if defined (FEATURE_CDMA)
    ...
    if ( subs_id > QM_SUBS_MIN && subs_id < QM_SUBS_MAX )
    {
      switch ( p_mstats_info->type )
      {
        ...
        case SYS_MODEM_MODULE_GSM:
          ...
          global.mstats[subs_id].gsm_mstats->g_ta = (uint16)p_mstats_info->prm.gsm_info.g_ta;
...
{
#if !defined (CFG_LTE_NO_VOICE) && ! defined (CFG_LTE_VOICE)
  uint32 i;
#endif
...
// TLV 0x22
      ...
      for( i=0; i < rsp_msg->wcdma_lte.umts_lte_nbr_cell_len; i++ )
      {
        ...
        QM_MSG_HIGH_6("earfcn = %d, cell id = %d, srxlev = %d,rsrp = %d, rsrq = %d, TDD Cell = %d", 
                p_wcdma->wcdma_monitored_set_info.lte_cell[i].earfcn, p_wcdma->wcdma_monitored_set_info.lte_cell[i].cell_id, p_wcdma->wcdma_monitored_set_info.lte_cell[i].s_rxlev,
                p_wcdma->wcdma_monitored_set_info.lte_cell[i].rsrp, p_wcdma->wcdma_monitored_set_info.lte_cell[i].rsrq, p_wcdma->wcdma_monitored_set_info.lte_cell[i].is_tdd_freq);
      }
      ...
      if( p_wcdma->wcdma_rrc_state == SYS_WCDMA_RRC_STATE_CELL_DCH )
      {
        ...
        QM_MSG_HIGH_5("WCDMA Active Set Ref RL: uarfcn %d, psc %d, cell_id %d, rac %d, lac %d", p_wcdma->wcdma_active_set_reference_rl_info.uarfcn, p_wcdma->wcdma_active_set_reference_rl_info.psc, p_wcdma->wcdma_active_set_reference_rl_info.cell_id,
                                      p_wcdma->wcdma_active_set_reference_rl_info.rac_id, p_wcdma->wcdma_active_set_reference_rl_info.lac_id);
  ...
  #endif
  #else
  QM_MSG_ERROR("Applicable only for non LTE RAT's");
  #endif
}
