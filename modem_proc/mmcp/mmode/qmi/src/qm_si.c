...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qm_si_process_cm_ss_event( cm_mm_ss_info_s_type *p_ss_info, cm_ss_event_e_type evt )
{
  ...
  #else
  enum qm_subs_e asubs_id = QM_SUBS_PRI;
  #endif
  ...
  switch ( evt )
  {
    case CM_SS_EVENT_SRV_CHANGED:
      ...
      #endif
      {
        if ( p_ss_info->srv_status != SYS_SRV_STATUS_PWR_SAVE &&
             p_ss_info->srv_status != SYS_SRV_STATUS_NO_SRV )
        {
          current_active |= qm_util_map_cm_sys_mode_to_qm_bitmask( p_ss_info->sys_mode );
        }
      }
...
{
  return (boolean) ( global.rat_in_svc[subscription] & qm_util_map_rat_enum_to_bitmask( rat ) );
}
...
{
  return global.rat_in_svc[subscription];
}
