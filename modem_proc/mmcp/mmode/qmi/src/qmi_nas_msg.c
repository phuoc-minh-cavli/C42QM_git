...
/*=========================================================================== 
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/qmi/src/qmi_nas_msg.c#1 $ $Change: 46174567 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
{ 
  ...
  if ( !( p_msg->signal_strength_valid || p_msg->report_rf_band_info_valid || p_msg->report_reg_reject_valid || 
          p_msg->rssi_indicator_valid || p_msg->ecio_indicator_valid ||  p_msg->sinr_indicator_valid 
          #ifndef FEATURE_MMODE_QMI_LTE_ONLY
          || p_msg->report_error_rate_valid || p_msg->io_indicator_valid 
          #endif
          || p_msg->rsrq_indicator_valid || p_msg->ecio_threshold_indicator_valid || p_msg->sinr_threshold_indicator_valid 
          || p_msg->lte_snr_delta_indicator_valid || p_msg->lte_rsrp_delta_indicator_valid ) ) 
  { 
...
)
{
  ...
  if (!(p_msg->reg_sys_sel_pref_valid || p_msg->reg_network_time_valid    || p_msg->sys_info_valid  ||  
        p_msg->sig_info_valid    || p_msg->err_rate_valid || p_msg->reg_current_plmn_name_valid   || 
        p_msg->reg_rf_band_info_valid    ||p_msg->network_reject_valid        || p_msg->reg_operator_name_data_valid || 
        p_msg->reg_csp_plmn_mode_bit_valid || p_msg->reg_lte_sib16_network_time_valid || p_msg->reg_lte_cphy_ca_valid   ||
        p_msg->reg_ssac_info_valid         || p_msg->reg_emm_t3402_change_valid   || p_msg->reg_acb_info_change_valid ||
        p_msg->reg_t3346_timer_status_change_valid || p_msg->reg_edrx_change_info_ind_valid || p_msg->lte_rrc_tx_info_ind_valid || 
        p_msg->reg_jamming_status_ind_valid
        #ifdef FEATURE_ENABLE_QMI_DMS
        || p_msg->reg_psm_config_params_change_ind_valid || p_msg->reg_psm_status_ind_valid || p_msg->reg_operating_mode_ind_valid 
        #endif
        || p_msg->reg_sfn_threshold_ind_valid || p_msg->reg_wwan_sleep_info_ind_valid
        )
      )
  #elif defined(FEATURE_MMODE_QMI_GPRS_LTE)
  if (!(p_msg->reg_sys_sel_pref_valid      || p_msg->req_serving_system_valid  ||
        p_msg->subscription_info_valid      || p_msg->reg_network_time_valid    ||
        p_msg->sys_info_valid              || p_msg->sig_info_valid               || p_msg->err_rate_valid            ||
        p_msg->reg_managed_roaming_valid ||
        ...
        p_msg->reg_ssac_info_valid         || p_msg->reg_emm_t3402_change_valid   || p_msg->reg_acb_info_change_valid ||
        p_msg->reg_t3346_timer_status_change_valid || p_msg->reg_edrx_change_info_ind_valid || p_msg->lte_rrc_tx_info_ind_valid || 
        p_msg->reg_jamming_status_ind_valid || p_msg->reg_wwan_sleep_info_ind_valid
        #ifdef FEATURE_ENABLE_QMI_DMS
        || p_msg->reg_psm_config_params_change_ind_valid || p_msg->reg_psm_status_ind_valid || p_msg->reg_operating_mode_ind_valid
        #endif 
        || p_msg->reg_sfn_threshold_ind_valid
        )
      )
  #else
  if (!(p_msg->reg_sys_sel_pref_valid      || p_msg->reg_ddtm_events_valid        || p_msg->req_serving_system_valid  ||
        ...
        p_msg->reg_current_plmn_name_valid || p_msg->reg_embms_status_valid       || p_msg->reg_rf_band_info_valid    ||
        p_msg->network_reject_valid        || p_msg->reg_operator_name_data_valid || p_msg->reg_csp_plmn_mode_bit_valid ||
        p_msg->reg_ims_pref_status_valid    || p_msg->reg_e911_state_ready_status_valid ||
        p_msg->reg_lte_sib16_network_time_valid ||
        p_msg->reg_lte_cphy_ca_valid       || p_msg->reg_subscription_change_valid ||
        p_msg->reg_ssac_info_valid         || p_msg->reg_emm_t3402_change_valid   || p_msg->reg_acb_info_change_valid ||
        p_msg->reg_t3346_timer_status_change_valid  || p_msg->reg_call_mode_change_valid || p_msg->reg_edrx_change_info_ind_valid ||
        p_msg->lte_rrc_tx_info_ind_valid || p_msg->reg_manual_scan_fail_valid || p_msg->reg_ssac_change_info_valid || 
        p_msg->reg_data_subs_priority_change_valid || p_msg->reg_jamming_status_ind_valid
        #ifdef FEATURE_ENABLE_QMI_DMS
        || p_msg->reg_psm_config_params_change_ind_valid ||p_msg->reg_psm_status_ind_valid || p_msg->reg_operating_mode_ind_valid
        #endif
        || p_msg->reg_sfn_threshold_ind_valid 
...
qmi_error_type_v01 qmi_nas_0021_req_check( nas_perform_network_scan_req_msg_v01 *p_msg)
{
  ...
  if ( p_msg->network_type_valid )
  {
    ...
    {
      QM_MSG_HIGH_1("perform_network_scan invalid network_type %d", p_msg->network_type);
  ...
  if ( p_msg->scan_type_valid )
  {
    if ( p_msg->scan_type != NAS_SCAN_TYPE_PLMN_V01 
      ...
      #endif
      #endif
      && p_msg->scan_type != NAS_SCAN_TYPE_CELL_SEARCH_V01
      )
    {
      QM_MSG_HIGH_1("perform_network_scan invalid scan_type %d", p_msg->network_type);
      return QMI_ERR_INVALID_ARG_V01;
    }
  }

  if( p_msg->lte_m1_band_pref_valid && p_msg->lte_m1_band_pref_ext_valid)//only one may be used
  {
    return QMI_ERR_INVALID_ARG_V01;
  }

  if( p_msg->lte_nb1_band_pref_valid && p_msg->lte_nb1_band_pref_ext_valid)//only one may be used
  {
    return QMI_ERR_INVALID_ARG_V01;
  }
...
qmi_error_e_type qmi_nas_0031_rsp_write (struct nas_0031_rsp_s *p_msg, dsm_item_type **response)
{
  ...
  if (p_msg->t13_valid == TRUE)
  {

    memscpy((void*) &nas_0031_rsp_p.t13.ciot_lte_op_mode, sizeof(nas_0031_rsp_p.t13.ciot_lte_op_mode),
            (void*) &p_msg->t13.ciot_lte_op_mode, sizeof(nas_0031_rsp_p.t13.ciot_lte_op_mode));

    if (FALSE == qmi_svc_put_param_tlv(response, 
                                       NAS_0031_RSP_T13,
                                       sizeof(nas_0031_rsp_p.t13),
                                       &nas_0031_rsp_p.t13))
    {
      qmi_err_write(NAS_0031_RSP_T13, &nas_0031_rsp_p.t13);
      dsm_free_packet(response);
      return QMI_ERR_NO_MEMORY;
    }
  }
...
)
{
  ...
  if ( p_msg->emergency_mode_valid )
  {
    ...
    {
      QM_MSG_HIGH("set_sys_sel_pref emergency mode check fail");
      return QMI_ERR_INVALID_ARG_V01;
    }
  }
  ...
  if ( p_msg->change_duration_valid )
  {
    if ( 
         #ifndef FEATURE_MMODE_DISABLE_EMERGENCY
         ! p_msg->emergency_mode_valid    && 
         #endif
         ! p_msg->mode_pref_valid       && 
         #ifndef FEATURE_MMODE_QMI_LTE_ONLY
         ! p_msg->band_pref_valid           &&
         #endif 
         #if defined(FEATURE_MMODE_CDMA_800) || defined(FEATURE_MMODE_CDMA_1900) 
         ! p_msg->prl_pref_valid          && ! p_msg->roam_pref_valid       &&
         #endif
          ! p_msg->lte_band_pref_valid    && ! p_msg->lte_band_pref_ext_valid &&
         ! p_msg->net_sel_pref_valid      && ! p_msg->srv_domain_pref_valid && ! p_msg->gw_acq_order_pref_valid   &&
         #if defined(FEATURE_TDSCDMA)
         ! p_msg->tdscdma_band_pref_valid && 
         #endif
         ! p_msg->acq_order_valid       && ! p_msg->srv_reg_restriction_valid && 
         ! p_msg->usage_setting_valid     && ! p_msg->voice_domain_pref_valid )
    {
      return QMI_ERR_MISSING_ARG_V01;
    }
  ...
  if( p_msg->rat_valid )
  {
    ...
    if(p_msg->rat == NAS_RADIO_IF_LTE_V01)
    {
      if((p_msg->man_ciot_lte_mode_valid == FALSE) ||
         !((p_msg->man_ciot_lte_mode == NAS_CIOT_SYS_MODE_LTE_M1_V01) ||
           (p_msg->man_ciot_lte_mode == NAS_CIOT_SYS_MODE_LTE_NB1_V01)
          )
        )
      {
        QM_MSG_ERROR_2("Reject manual plmn selection due to invalid man_ciot_lte_op_mode %d %d",
                        p_msg->man_ciot_lte_mode_valid, p_msg->man_ciot_lte_mode);
        return QMI_ERR_INVALID_ARG_V01;
      }
    }
  }
...
{ 
  // If ALL optional TLVs are absent return error 
  if ( ! p_msg->rssi_threshold_list_valid     && ! p_msg->ecio_threshold_list_valid  &&
       ! p_msg->lte_snr_threshold_list_valid    && ! p_msg->lte_rsrq_threshold_list_valid &&
       ! p_msg->lte_rsrp_threshold_list_valid && ! p_msg->lte_sig_rpt_config_valid
       #ifdef FEATURE_WCDMA
       && ! p_msg->rscp_threshold_list_valid     
       #endif
       #ifndef FEATURE_MMODE_QMI_LTE_ONLY
       #if defined(FEATURE_HDR)
       && ! p_msg->hdr_sinr_threshold_list_valid 
       #endif
       && ! p_msg->io_threshold_list_valid  
       #endif
       )
  { 
    return QMI_ERR_MISSING_ARG_V01;
  ...
  if ( ( p_msg->rssi_threshold_list_valid     && p_msg->rssi_threshold_list_len     >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       ( p_msg->ecio_threshold_list_valid     && p_msg->ecio_threshold_list_len     >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       #ifdef FEATURE_HDR
       ( p_msg->hdr_sinr_threshold_list_valid && p_msg->hdr_sinr_threshold_list_len >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       #endif
       ( p_msg->lte_snr_threshold_list_valid  && p_msg->lte_snr_threshold_list_len  >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       ( p_msg->io_threshold_list_valid       && p_msg->io_threshold_list_len       >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       ( p_msg->lte_rsrq_threshold_list_valid && p_msg->lte_rsrq_threshold_list_len >  MAX_NUM_SIG_INFO_THRESHOLDS) ||
       ( p_msg->lte_rsrp_threshold_list_valid && p_msg->lte_rsrp_threshold_list_len >  MAX_NUM_SIG_INFO_THRESHOLDS) 
       #ifdef FEATURE_WCDMA
       || ( p_msg->rscp_threshold_list_valid     && p_msg->rscp_threshold_list_len     >  MAX_NUM_SIG_INFO_THRESHOLDS)       
       #endif 
       #ifdef FEATURE_TDSCDMA
       || ( p_msg->tds_sinr_threshold_list_valid && p_msg->tds_sinr_threshold_list_len >  MAX_NUM_SIG_INFO_THRESHOLDS)
       #endif
    )
  {
    return QMI_ERR_INVALID_ARG_V01;
...
{ 
  ...
  else
  {
    if ( 
         ...
         p_msg->t01.radio_if != QMI_NAS_RADIO_IF_CDMA20001X &&
...
{ 
  ...
  if ( 
       #ifndef FEATURE_MMODE_QMI_LTE_ONLY
       #ifdef FEATURE_MMODE_3GPP2
       p_msg->radio_if != NAS_RADIO_IF_CDMA_1X_V01 &&
...
qmi_error_e_type qmi_nas_0068_ind_write (struct nas_0068_ind_s *p_msg, dsm_item_type **response)
{
  ...
  if (p_msg->t12_valid == TRUE)
  {
    memscpy((void*) &nas_0068_ind_p.t12.ciot_lte_op_mode, sizeof(nas_0068_ind_p.t12.ciot_lte_op_mode),
            (void*) &p_msg->t12.ciot_lte_op_mode, sizeof(nas_0068_ind_p.t12.ciot_lte_op_mode));

    if ( !qmi_svc_put_param_tlv( response, NAS_0068_IND_T12, sizeof(nas_0068_ind_p.t12), &nas_0068_ind_p.t12 ) )
    {
      /*Could not write radio_if tlv 0x01 to message: out of memory*/
      qmi_err_write(NAS_0068_IND_T12, &nas_0068_ind_p.t12); 
      dsm_free_packet(response);
      return QMI_ERR_NO_MEMORY;
    }
  }
...
{ 
  ...
  if ( 
       #ifdef FEATURE_MMODE_3GPP2
       ! p_msg->cdma_rssi_threshold_list_valid    && ! p_msg->cdma_rssi_delta_valid           && ! p_msg->cdma_ecio_threshold_list_valid  &&
       ...
       #ifdef FEATURE_WCDMA
       ! p_msg->wcdma_rssi_threshold_list_valid &&
 ...
 #else
   if ( ! p_msg->lte_rssi_threshold_list_valid     && ! p_msg->lte_rssi_delta_valid            && ! p_msg->lte_snr_threshold_list_valid    &&
       ! p_msg->lte_snr_delta_valid               && ! p_msg->lte_rsrq_threshold_list_valid   && ! p_msg->lte_rsrq_delta_valid            &&
       ! p_msg->lte_rsrp_threshold_list_valid     && ! p_msg->lte_rsrp_delta_valid            && ! p_msg->lte_sig_rpt_config_valid   )
  ...
  if ( 
       #ifdef FEATURE_MMODE_3GPP2
       ( p_msg->cdma_rssi_threshold_list_valid    && p_msg->cdma_rssi_delta_valid    ) ||
...
#else
  if ( ( p_msg->lte_rssi_threshold_list_valid     && p_msg->lte_rssi_delta_valid     ) ||
       ( p_msg->lte_snr_threshold_list_valid      && p_msg->lte_snr_delta_valid      ) ||
       ( p_msg->lte_rsrq_threshold_list_valid     && p_msg->lte_rsrq_delta_valid     ) ||
       ( p_msg->lte_rsrp_threshold_list_valid     && p_msg->lte_rsrp_delta_valid     ) 
    )
  ...
  if ( 
       #ifdef FEATURE_MMODE_3GPP2
       ( p_msg->cdma_rssi_threshold_list_valid    && p_msg->cdma_rssi_threshold_list_len    == 0 ) ||
...
#else
  if ( ( p_msg->lte_rssi_threshold_list_valid     && p_msg->lte_rssi_threshold_list_len     == 0 ) ||
       ( p_msg->lte_snr_threshold_list_valid      && p_msg->lte_snr_threshold_list_len      == 0 ) ||
       ( p_msg->lte_rsrq_threshold_list_valid     && p_msg->lte_rsrq_threshold_list_len     == 0 ) ||
       ( p_msg->lte_rsrp_threshold_list_valid     && p_msg->lte_rsrp_threshold_list_len     == 0 )
    )
  ...
  // Ensure if threshold sent values are not too large
  if ( 
       #ifdef FEATURE_CDMA
       ( p_msg->cdma_rssi_threshold_list_valid    && p_msg->cdma_rssi_threshold_list_len    > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->cdma_ecio_threshold_list_valid    && p_msg->cdma_ecio_threshold_list_len    > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       #endif
       #ifdef FEATURE_HDR
       ( p_msg->hdr_rssi_threshold_list_valid     && p_msg->hdr_rssi_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->hdr_ecio_threshold_list_valid     && p_msg->hdr_ecio_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->hdr_sinr_threshold_list_valid     && p_msg->hdr_sinr_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->hdr_io_threshold_list_valid       && p_msg->hdr_io_threshold_list_len       > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       #endif
       #ifdef FEATURE_GSM
       ( p_msg->gsm_rssi_threshold_list_valid     && p_msg->gsm_rssi_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       #endif
       #ifdef FEATURE_WCDMA
       ( p_msg->wcdma_rssi_threshold_list_valid   && p_msg->wcdma_rssi_threshold_list_len   > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->wcdma_ecio_threshold_list_valid   && p_msg->wcdma_ecio_threshold_list_len   > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       #endif
       ( p_msg->lte_rssi_threshold_list_valid     && p_msg->lte_rssi_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->lte_snr_threshold_list_valid      && p_msg->lte_snr_threshold_list_len      > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->lte_rsrq_threshold_list_valid     && p_msg->lte_rsrq_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->lte_rsrp_threshold_list_valid     && p_msg->lte_rsrp_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) 
       #ifdef FEATURE_TDSCDMA
       || ( p_msg->tdscdma_rscp_threshold_list_valid && p_msg->tdscdma_rscp_threshold_list_len > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->tds_rssi_threshold_list_valid     && p_msg->tds_rssi_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->tds_ecio_threshold_list_valid     && p_msg->tds_ecio_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS ) ||
       ( p_msg->tds_sinr_threshold_list_valid     && p_msg->tds_sinr_threshold_list_len     > MAX_NUM_SIG_INFO2_THRESHOLDS )
       #endif
     )
  {
    return QMI_ERR_INVALID_ARG_V01;
  ...
  if ( 
       #ifdef FEATURE_MMODE_3GPP2
       ( p_msg->cdma_rssi_delta_valid    && p_msg->cdma_rssi_delta    < NAS_006C_MIN_DELTA ) ||
...
#else
  if (( p_msg->lte_rssi_delta_valid     && p_msg->lte_rssi_delta     < NAS_006C_MIN_DELTA ) ||
       ( p_msg->lte_snr_delta_valid      && p_msg->lte_snr_delta      < NAS_006C_MIN_DELTA ) ||
       ( p_msg->lte_rsrq_delta_valid     && p_msg->lte_rsrq_delta     < NAS_006C_MIN_DELTA ) ||
       ( p_msg->lte_rsrp_delta_valid     && p_msg->lte_rsrp_delta     < NAS_006C_MIN_DELTA ) 
    )
...
qmi_error_type_v01 qmi_nas_0072_req_check(nas_update_ims_status_req_msg_v01 *p_msg)
{
  ...
  if (
      #ifdef FEATURE_MMODE_3GPP2
      p_msg->sys_mode != NAS_RADIO_IF_CDMA_1XEVDO_V01 && 
  ...
  #else
  if (p_msg->sys_mode != NAS_RADIO_IF_LTE_V01)
  #endif
  {
