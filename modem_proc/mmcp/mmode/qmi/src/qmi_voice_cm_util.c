...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/mmode/qmi/src/qmi_voice_cm_util.c#1 $ $DateTime:$ $Author:$
...
#if !defined (FEATURE_NO_QMI_VOICE)
...
DESCRIPTION
 To copy the utf16 sups string/sip code from CM.
...
(
 boolean* failure_cause_description_valid,
 uint32* failure_cause_description_len, 
 uint16* failure_cause_description, 
 boolean* sip_code_valid,
 uint16* sip_error_code, 
 cm_ip_sups_error_s_type* cm_struct,
 uint16 cm_sip_error_code

)
{
  QM_MSG_MED_1("Sups qmi_voice_cm_util_copy_sups_failure_string sip_code=%d", cm_sip_error_code);
   
  if(cm_sip_error_code>0)
  {
    *sip_error_code =cm_sip_error_code;
    *sip_code_valid=TRUE;
  }
  ...
  if(cm_struct == NULL)
  {
   ...
   {
    *failure_cause_description_valid= TRUE;
    *failure_cause_description_len = (uint16) cm_struct->err_text_length;
    memscpy( failure_cause_description, 2*(cm_struct->err_text_length), cm_struct->err_text, 2*(cm_struct->err_text_length));
...
typedef struct
{
  qmi_voice_cm_if_sys_mode_e_type     sys_mode ;
...
)
{
      ...
      ( qmi_voice_call_list_is_call_present( call_info_ptr->call_id ) ) )
  {
      ...
      if(dcs == CM_USSD_DCS_7_BIT)
      {
        ...
        num_bytes_req = (uint8)((call_info_ptr->enc_alpha.num_characters * 7.0)/8.0); //Number of bytes required to pack num_characters
...
)
{
  ...
  {
    ss_info->sys_mode = qmi_voice_cm_util_map_qmi_to_cm_sys_mode(util_info.ss_info.sys_mode);
    ss_info->srv_status = util_info.ss_info.srv_status;

    ret = TRUE;
  }

  QMI_MMODE_LEAVE_CRIT_SECTION(&util_info.ss_cs);
  QM_MSG_MED_2("Get SS Info:  sys_mode %d, srv_status %d", ss_info->sys_mode, ss_info->srv_status);
  return ret;
...
)
{
  ...
  util_info.ss_info.sys_mode   = qmi_voice_cm_util_map_cm_to_qmi_sys_mode(ss_info_ptr->sys_mode);
  util_info.ss_info.srv_status = ss_info_ptr->srv_status;
...
/*===========================================================================
  FUNCTION qmi_voice_cm_util_map_cm_ip_end_cause_to_qmi_end_reason()

  DESCRIPTION
  Map the IP end cause to QMI end cause.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
qmi_voice_cm_if_end_reason_e_type qmi_voice_cm_util_map_cm_ip_end_cause_to_qmi_end_reason
(
  cm_call_end_e_type  ip_end_cause
)
{
  qmi_voice_cm_if_end_reason_e_type  end_reason = QMI_VOICE_CM_IF_END_BAD_REQ_WAIT_INVITE;
  uint8 i=0;
  uint8 total_size_of_end_reason_table=0;


  qmi_voice_cm_end_cause_to_qmi_end_reason_table_s_type qmi_voice_cm_if_end_reason_conv_table[] =
  {
    {CM_CALL_END_INCOM_REJ_CAUSE_CALL_ONGOING_CW_DISABLED,         QMI_VOICE_CM_IF_END_INCOMING_REJ_CAUSE_CALL_ONGOING_CW_DISABLED},
    {CM_CALL_END_INCOM_REJ_CAUSE_CALL_ON_OTHER_SUB,         QMI_VOICE_CM_IF_END_INCOMING_REJ_CAUSE_CALL_ON_OTHER_SUB},
    {CM_CALL_END_INCOM_REJ_CAUSE_1X_COLLISION,         QMI_VOICE_CM_IF_END_INCOMING_REJ_CAUSE_1X_COLLISION},
    {CM_CALL_END_CAUSE_SRV_NOT_REGISTERED,         QMI_VOICE_CM_IF_END_CAUSE_SRV_NOT_REGISTERED},
    {CM_CALL_END_CAUSE_CALL_TYPE_NOT_ALLOWED,         QMI_VOICE_CM_IF_END_CAUSE_CALL_TYPE_NOT_ALLOWED},
    {CM_CALL_END_CAUSE_EMRG_CALL_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_EMRG_CALL_ONGOING},
    {CM_CALL_END_CAUSE_CALL_SETUP_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_CALL_SETUP_ONGOING},
    {CM_CALL_END_CAUSE_MAX_CALL_LIMIT_REACHED,         QMI_VOICE_CM_IF_END_CAUSE_MAX_CALL_LIMIT_REACHED},
    {CM_CALL_END_CAUSE_UNSUPPORTED_SIP_HDRS,         QMI_VOICE_CM_IF_END_CAUSE_UNSUPPORTED_SIP_HDRS},
    {CM_CALL_END_CAUSE_UNSUPPORTED_SDP,         QMI_VOICE_CM_IF_END_UNSUPPORTED_SDP},
    {CM_CALL_END_CAUSE_CALL_TRANSFER_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_CALL_TRANSFER_ONGOING},
    {CM_CALL_END_CAUSE_PRACK_TIMEOUT,         QMI_VOICE_CM_IF_END_CAUSE_PRACK_TIMEOUT},
    {CM_CALL_END_CAUSE_QOS_FAILURE,         QMI_VOICE_CM_IF_END_CAUSE_QOS_FAILURE},
    {CM_CALL_END_CAUSE_ONGOING_HANDOVER,         QMI_VOICE_CM_IF_END_CAUSE_ONGOING_HANDOVER},
    {CM_CALL_END_CAUSE_VT_WITH_TTY_NOT_ALLOWED,         QMI_VOICE_CM_IF_END_CAUSE_VT_WITH_TTY_NOT_ALLOWED},
    {CM_CALL_END_CAUSE_CALL_UPGRADE_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_CALL_UPGRADE_ONGOING},
    {CM_CALL_END_CAUSE_CONFERENCE_WITH_TTY_NOT_ALLOWED,         QMI_VOICE_CM_IF_END_CAUSE_CONFERENCE_WITH_TTY_NOT_ALLOWED},
    {CM_CALL_END_CAUSE_CALL_CONFERENCE_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_CALL_CONFERENCE_ONGOING},
    {CM_CALL_END_CAUSE_VT_WITH_AVPF_NOT_ALLOWED,         QMI_VOICE_CM_IF_END_CAUSE_VT_WITH_AVPF_NOT_ALLOWED},
    {CM_CALL_END_CAUSE_ENCRYPTION_CALL_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_ENCRYPTION_CALL_ONGOING},
    {CM_CALL_END_INCOM_REJ_CAUSE_UI_NOT_READY,         QMI_VOICE_CM_IF_END_CAUSE_INCOM_REJ_CAUSE_UI_NOT_READY},
    {CM_CALL_END_CAUSE_CS_CALL_ONGOING,         QMI_VOICE_CM_IF_END_CAUSE_CS_CALL_ONGOING}

  };


  total_size_of_end_reason_table = sizeof(qmi_voice_cm_if_end_reason_conv_table)/sizeof(qmi_voice_cm_end_cause_to_qmi_end_reason_table_s_type);

  while(i < total_size_of_end_reason_table)
  {
    if(ip_end_cause == qmi_voice_cm_if_end_reason_conv_table[i].cause)
    {
  ...
  return end_reason;
}/* qmi_voice_cm_util_map_cm_ip_end_cause_to_qmi_end_reason() */
...
)
{
  ...
  switch(qmi_rej_cause)
  {
    ...
    case QMI_VOICE_CM_IF_REJ_CAUSE_UNWANTED_CALL:
      cm_rej_cause = CM_CALL_END_IP_UNWANTED_CALL;
      break;
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
===========================================================================*/
boolean qmi_voice_cm_util_is_sys_mode(uint16 mode_mask)
{
  uint16 sys_mode=0;

  sys_mode = (uint16) util_info.ss_info.sys_mode;
...
boolean qmi_voice_cm_util_copy_barr_num_list_info_from_cm_to_qmi(ie_call_barring_num_info_T *cm_data, qmi_voice_cm_if_icb_sc_s *qmi_data, const uint8 copy_len)
{
  ...
  for(i=0; i<copy_len && i<QMI_VOICE_CM_IF_MAX_BARR_LIST_LEN; i++)
  {
    ...
    {
      qmi_data[i].barr_num_status = TRUE;
    }//else inactive=0
