...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/reg/src/reg_cm.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
FUNCTION NAME
  ...
  cm_network_list_cnf_is_valid

==============================================================================*/

boolean cm_network_list_cnf_is_valid
(
  cm_network_list_cnf_s_type* msg_p
)
{
  boolean valid = FALSE;
  ...
  if (msg_p)
  {
    valid = (boolean)(sys_plmn_id_is_valid            ( msg_p->rplmn ) &&
                      sys_detailed_plmn_list_is_valid ( &(msg_p->available_plmn_list.plmn_list) )); //this is not used anywhere
  }
...
RETURN VALUE
  ...
  boolean  Flag indicating whether the cm_mcc_change_ind_is_valid
           contains valid data (TRUE) or invalid data (FALSE). 

==============================================================================*/
boolean cm_mcc_change_ind_is_valid
(
