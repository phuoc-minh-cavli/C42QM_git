...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/reg/src/reg_sim.c#1 $
...
typedef enum reg_sim_mode_pref_e
{
  ...
  REG_SIM_MODE_PREF_LTE_M1_RADIO_ACCESS_PREFERRED,
  REG_SIM_MODE_PREF_LTE_NB1_RADIO_ACCESS_PREFERRED,
...
typedef struct reg_sim_nasconfig_s
{
 ...
 uint16  dcn_id;
  ...
  boolean eutran_not_allowed;
#endif
} reg_sim_nasconfig_s_type;

#define NAS_MMGSDI_INVALID_CLIENT_ID     0x00000000FFFFFFFF

#define NAS_MMGSDI_INVALID_SESSION_ID     0x00000000FFFFFFFF
...
extern uint8  mm_emm_max_fplmn_count_dos_mitigation_sim[MAX_NAS_STACKS];
...
extern uint8  mm_emm_max_fplmn_count_dos_mitigation;
#endif
...
*/
reg_sim_fplmn_counter_list_s_type reg_sim_gprs_fplmn_list_sim[MAX_NAS_STACKS];
#define reg_sim_gprs_fplmn_list reg_sim_gprs_fplmn_list_sim[reg_sub_id]

/*
**  DOS Mitigation Forbidden PLMN List.
*/
reg_sim_dos_mitigation_plmn_list_s_type reg_sim_dos_mitigation_fplmn_list_sim[MAX_NAS_STACKS];
#define reg_sim_dos_mitigation_fplmn_list reg_sim_dos_mitigation_fplmn_list_sim[reg_sub_id]
...
**  GPRS Forbidden PLMN List.
*/
reg_sim_fplmn_counter_list_s_type reg_sim_gprs_fplmn_list;

/*
**  DOS Mitigation Forbidden PLMN List.
*/
reg_sim_dos_mitigation_plmn_list_s_type reg_sim_dos_mitigation_fplmn_list;
...
static boolean reg_nv_extended_fplmn_icc = FALSE;
  ...
  static uint8  reg_sim_num_file_read[MAX_NAS_STACKS] = {0,0,0};
#elif defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE 
  static uint8  reg_sim_num_file_read[MAX_NAS_STACKS] = {0,0};
#else
  static uint8  reg_sim_num_file_read = 0;
#endif
  reg_as_id_e_type reg_sim_as_id = REG_AS_ID_1;
...
#ifdef FEATURE_DUAL_SIM
boolean reg_sim_revert_sim_state_on_psm_restore_failure_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
#define reg_sim_revert_sim_state_on_psm_restore_failure  reg_sim_revert_sim_state_on_psm_restore_failure_sim[reg_as_id]
#else
boolean reg_sim_revert_sim_state_on_psm_restore_failure = FALSE;
#endif
...
)
{
  ...
  if (cnf_ptr != NULL)
  {
#if defined (FEATURE_DUAL_SIM ) || defined(FEATURE_SGLTE)
#ifdef FEATURE_DUAL_SIM
    reg_sim_as_id = reg_sim_get_as_id(cnf_ptr->response_header.session_id);
    sub_id = reg_sub_id_stack[reg_sim_as_id];
#else
    reg_sim_as_id = reg_ps_stack_id;
#endif 
     local_as_id = reg_sim_as_id;
#ifdef FEATURE_SGLTE
...
#endif 
    if(reg_sim_num_fcn_files_sim[reg_sim_as_id])
    {
      reg_sim_num_file_read[reg_sim_as_id]++;
      if (status != MMGSDI_SUCCESS)
      {
        MSG_ERROR_DS_1( local_as_id+1, "=REG= ASYNC File read failed", cnf_ptr->response_header.client_data);
        read_status[reg_sim_as_id] = FALSE;
      }
#else
    if(reg_sim_num_fcn_files)
    {
      reg_sim_num_file_read++;
      if (status != MMGSDI_SUCCESS)
      {
        MSG_ERROR_DS_1( local_as_id+1, "=REG= ASYNC File read failed", cnf_ptr->response_header.client_data);
        read_status = FALSE;
      ...
      {
        MSG_HIGH_DS_1( local_as_id+1, "=REG= ASYNC file %d read success", cnf_ptr->response_header.client_data);
        /*
      ...
      if (reg_sim_num_fcn_files_sim[reg_sim_as_id] == reg_sim_num_file_read[reg_sim_as_id])
      {
        reg_sim_per_subs_refresh_complete
        (
          read_status[reg_sim_as_id],
          (sys_modem_as_id_e_type)reg_sim_as_id
        );
        reg_sim_num_fcn_files_sim[reg_sim_as_id] = 0;
        reg_sim_num_file_read[reg_sim_as_id] = 0;
        read_status[reg_sim_as_id] = TRUE;
      }
#else
      if (reg_sim_num_fcn_files == reg_sim_num_file_read)
      {
    ...
    {
      MSG_ERROR_DS_0( local_as_id+1, "=REG= Refresh list NULL");
    }
  }
  else
  {
    MSG_ERROR_DS_0( reg_sim_as_id+1, "=REG= Invalid ASYNC OP COMPLETE CB");
  }
}
...
/*==============================================================================
FUNCTION NAME
  reg_sim_set_fast_hplmn_tag

DESCRIPTION
  Function sets fast first hplmn search tag read from NAS configuration file

RETURN VALUE
  None
==============================================================================*/
void reg_sim_set_fast_hplmn_tag
(
  boolean fast_hplmn_tag
)
{
  reg_sim_nasconfig.fast_hplmn_tag = fast_hplmn_tag;
}
/*==============================================================================
FUNCTION NAME
  reg_sim_get_fast_hplmn_tag

DESCRIPTION
  Function checks if  fast first hplmn search tag from NAS configuration file is TRUE or not

RETURN VALUE
  fast hplmn tag value
==============================================================================*/

boolean reg_sim_get_fast_hplmn_tag
(
  void
)
{
    return reg_sim_nasconfig.fast_hplmn_tag;
}
...
/*==============================================================================
FUNCTION NAME
  reg_sim_set_attach_with_imsi_cfg

DESCRIPTION
  Function sets Attach with IMSI config read from NAS configuration file

RETURN VALUE
  None
==============================================================================*/
void reg_sim_set_attach_with_imsi_cfg
(
  boolean attach_with_imsi
)
{
  reg_sim_nasconfig.attach_with_imsi = attach_with_imsi;
}

/*==============================================================================
FUNCTION NAME
  reg_sim_per_subs_chk_attach_with_imsi_cfg

DESCRIPTION
  Function returns Attach with IMSI config status read from NAS configuration file

RETURN VALUE
  TRUE or FALSE
==============================================================================*/
boolean reg_sim_per_subs_chk_attach_with_imsi_cfg
(
  sys_modem_as_id_e_type sub_id
)
{
#ifdef FEATURE_DUAL_SIM
  return reg_sim_nasconfig_sim[sub_id].attach_with_imsi;
#else
  (void)sub_id;
  return reg_sim_nasconfig.attach_with_imsi;
#endif
}
...
/*==============================================================================
FUNCTION NAME
  reg_sim_set_eutran_not_allowed

DESCRIPTION
  Function sets eutran_not_allowed value read from NAS configuration file

RETURN VALUE
  None
==============================================================================*/
void reg_sim_set_eutran_not_allowed 
(
  boolean eutran_not_allowed
)
{
  reg_sim_nasconfig.eutran_not_allowed = eutran_not_allowed;
}

/*==============================================================================
FUNCTION NAME
  reg_sim_get_eutran_not_allowed

DESCRIPTION
  Function checks if eutran_not_allowed tag from NAS configuration file is TRUE or not

RETURN VALUE
  eutran_not_allowed tag value
==============================================================================*/

boolean reg_sim_get_eutran_not_allowed
(
  void
)
{
    MSG_HIGH_DS_1(REG_SUB, "=REG= reg_sim_get_eutran_not_allowed() returns %d", reg_sim_nasconfig.eutran_not_allowed);
    return reg_sim_nasconfig.eutran_not_allowed;
}
...
DESCRIPTION
  Function sets MO exception flag  read from NAS configuration file

RETURN VALUE
  None
==============================================================================*/
void reg_sim_set_mo_excp_cfg
(
  boolean mo_excp
)
{
  reg_sim_nasconfig.mo_excp_cfg = mo_excp;
}
...
RETURN VALUE
  None
==============================================================================*/
void reg_sim_set_dcn_cfg
(
  boolean dcn_cfg, uint16 dcn_id
)
{
  reg_sim_nasconfig.dcn_cfg = dcn_cfg;
  reg_sim_nasconfig.dcn_id  = dcn_id;
}


boolean  reg_sim_check_dcn_cfg
( 
  void
)
{
  return reg_sim_nasconfig.dcn_cfg ;
}


uint16  reg_sim_get_dcn_id
( 
  void
)
{
  return reg_sim_nasconfig.dcn_id ;
}

/*==============================================================================
FUNCTION NAME
  reg_sim_per_subs_chk_mo_excep_cfg

DESCRIPTION
  Function returns Mo exceptio config status read from NAS configuration file

RETURN VALUE
  TRUE or FALSE
==============================================================================*/
boolean reg_sim_per_subs_chk_mo_excep_cfg
(
  sys_modem_as_id_e_type sub_id
)
{
#ifdef FEATURE_DUAL_SIM
  return reg_sim_nasconfig_sim[sub_id].mo_excp_cfg;
#else
  (void)sub_id;
  return reg_sim_nasconfig.mo_excp_cfg;
#endif
}
...
)
{
  ...
  switch(sim_rat_search_order)
  {
     ...
     case REG_SIM_MODE_PREF_LTE_RADIO_ACCESS_PREFERRED:
        ...
        break;
     case REG_SIM_MODE_PREF_LTE_NB1_RADIO_ACCESS_PREFERRED:
        pref_rat = SYS_RAT_LTE_NB1_RADIO_ACCESS;
        break;
     case REG_SIM_MODE_PREF_LTE_M1_RADIO_ACCESS_PREFERRED:
        pref_rat = SYS_RAT_LTE_M1_RADIO_ACCESS;
        break;
#endif /* FEATURE_LTE */  
     default:
       return;
...
)
{
  ...
  if (reg_sim_gprs_fplmn_list_sim[as_id].length > 0)
  {
    ...
    for (i = 0; (i < reg_sim_gprs_fplmn_list_sim[as_id].length) && (reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i].is_forbidden == TRUE); i++)
    {
      sys_plmn_get_mcc_mnc
      (
        reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i].plmn,
  ...
  if (reg_sim_gprs_fplmn_list.length > 0)
  {
    ...
    for (i = 0; (i < reg_sim_gprs_fplmn_list.length) && (reg_sim_gprs_fplmn_list.plmn_info[i].is_forbidden == TRUE); i++)
    {
      sys_plmn_get_mcc_mnc
      (
        reg_sim_gprs_fplmn_list.plmn_info[i].plmn,
        &plmn_id_is_undefined,
        &mnc_includes_pcs_digit,
        &mcc,
        &mnc
...
FUNCTION NAME
  ...
  reg_sim_find_plmn_in_fplmn_counter_list

==============================================================================*/

boolean reg_sim_find_plmn_in_fplmn_counter_list
(
        sys_plmn_id_s_type                 plmn,
  const reg_sim_fplmn_counter_list_s_type* plmn_list_p,
        uint32*                            position_p
)
{
  boolean found = FALSE;
  ...
  {
    if ( sys_plmn_match(plmn_list_p->plmn_info[i].plmn, plmn) )
    {
      found = TRUE;
      *position_p = i;
      break;
  ...
  MSG_HIGH_DS_4(REG_SUB, "=REG= reg_sim_find_plmn_in_fplmn_counter_list() returns %d for plmn %x %x %x", 
                found, plmn.identity[0], plmn.identity[1], plmn.identity[2]);
  return found;
}
...
FUNCTION NAME
  ...
  reg_sim_find_plmn_in_dos_mitigation_counter_list

==============================================================================*/

boolean reg_sim_find_plmn_in_dos_mitigation_counter_list
(
        sys_plmn_id_s_type                       plmn,
  const reg_sim_dos_mitigation_plmn_list_s_type* plmn_list_p,
        uint32*                                  position_p
)
{
  boolean found = FALSE;
  ...
  MSG_HIGH_DS_4(REG_SUB, "=REG= reg_sim_find_plmn_in_dos_mitigation_counter_list() returns %d for plmn %x %x %x", 
                found, plmn.identity[0], plmn.identity[1], plmn.identity[2]);
  return found;
}
...
#else
  reg_sim_set_fast_hplmn_tag(TRUE); 
#endif
#endif
...
void reg_sim_reset_nasconfig
(
  void
)
{
  reg_sim_nasconfig.nas_sig_low_priority = FALSE;
  reg_sim_nasconfig.attach_with_imsi = FALSE;
  reg_sim_nasconfig.min_periodic_srch_timer = 0;
  reg_sim_nasconfig.eab_cfg = FALSE;
  reg_sim_nasconfig.t3245_behavior = FALSE;
#if defined (FEATURE_NAS_REL12 ) || defined (FEATURE_LTE_REL12)
  reg_sim_nasconfig.fast_hplmn_tag = FALSE;
#endif
#ifdef FEATURE_CIOT
   reg_sim_nasconfig.mo_excp_cfg = FALSE;
   reg_sim_nasconfig.dcn_cfg = FALSE;
   reg_sim_nasconfig.dcn_id = FALSE;
#endif
#ifdef  FEATURE_LTE_REL12
   reg_sim_nasconfig.eutran_not_allowed = FALSE;
#endif
}
    ...
    case SYS_SIM_STATE_AVAILABLE:
      switch (sim_state)
      {
        ...
        case SYS_SIM_STATE_CS_PS_INVALID:
          ...
          break;
        case SYS_SIM_STATE_CP_SMS_INVALID:
          reg_sim_state = SYS_SIM_STATE_CP_SMS_INVALID;
          break;
...
)
{
  ...
  memset((void *)&imsi[0],0x00,sizeof(imsi));
...
)
{
  ...
  memset((void *)&imsi[0],0x00,sizeof(imsi));
  memset((void *)&admin_data[0],0x00,sizeof(admin_data));
...
#endif 
   if ( reg_sim_card_mode_sim[as_id] == MMGSDI_APP_USIM )
  {
...
)
{
  ...
  */
  ...
  if (POLICYMAN_SUCCEEDED(status_pm))
  {
        ...
        for(j = num_of_hplmn_rat_found; j < rat_order->num_items; j++)
        {
          ...
          flag_umts_found = TRUE;
            ...
            case SYS_RAT_LTE_M1_RADIO_ACCESS:            
                if(hplmn_w_act_list->plmn_w_act[i].act[0] & 0x40)
                {
                  rat_found = SYS_RAT_LTE_M1_RADIO_ACCESS;
                }
                break;
            case SYS_RAT_LTE_NB1_RADIO_ACCESS:            
                if(hplmn_w_act_list->plmn_w_act[i].act[0] & 0x40)
                {
                  rat_found = SYS_RAT_LTE_NB1_RADIO_ACCESS;
                }
                break;
...
/*
  3GPP 23.122
  T is either in the range 2 hours to 240 hours, using 2 hour steps from 2 hours to 80 hours and 4 hour steps from 84 hours
  to 240 hours, or it indicates that no periodic attempts shall be made.
  If no value for T is stored in the SIM, a default value of 72 hours is used.  
*/
    hplmn_search_period = 36;
#else
...
)
{
  ...
  if (rplmn_rat_search_order == REG_SIM_MODE_PREF_UNDEFINED)
  {
     ...
     if (reg_state_is_active_rat_lte(rat_search_order.rat_list[0]))
     {
  ...
  if (rplmn_rat_search_order == REG_SIM_MODE_PREF_LTE_RADIO_ACCESS_PREFERRED)
  {
    ...
    {
       MSG_HIGH_DS_0(REG_SUB, "=REG= EPS RPLMN is undefined, picking up CS/PS RPLMN");
...
)
{
  ...
  memset((void *)&loci[0],0x00,sizeof(loci));
  sys_plmn_undefine_plmn_id (&rplmn);
...
)
{
  ...
  memset((void *)&psloci[0],0x00,sizeof(psloci));
  sys_plmn_undefine_plmn_id (&rplmn);
...
)
{
  ...
  if(
#ifdef FEATURE_LTE
      ((reg_state_is_active_rat_lte(current_service_state->active_rat))&&
         sys_plmn_match(reg_sim_read_eps_rplmn(),current_service_state->plmn)) ||
#endif
       (
#ifdef FEATURE_LTE        
	 (reg_state_is_active_rat_lte(current_service_state->active_rat) == FALSE) &&
#endif
...
#else
    if(sys_plmn_match(reg_sim_read_eps_rplmn(),current_service_state->plmn))
#endif
   {
     return TRUE;
   }
...
)
{
  ...
  memset((void *)&epsloci[0],0x00,sizeof(epsloci));
  sys_plmn_undefine_plmn_id (&rplmn);
...
#endif 
   if ( reg_sim_rplmn_rat_search_order == REG_SIM_MODE_PREF_LTE_RADIO_ACCESS_PREFERRED)
  {
...
#endif /*ifndef FEATURE_RAT_PRIORITY_LIST*/
{
...
#ifndef FEATURE_DISABLE_RPLMNACT
  memset( (void *)&rplmn_w_act, 0, sizeof(reg_sim_plmn_w_act_s_type));
  /*
  ...
  */
       ...
       if ( sys_plmn_match( rplmn, oem_list_efs->plmn[i]))       
         {
           ...
           MSG_HIGH_DS_0(REG_SUB, "=REG= Match between RPLMN and NV PLMN");
             rplmn_rat_search_order = REG_SIM_MODE_PREF_UNDEFINED;
             break;
         }
     }
   }
  ...
  /* First check if the shared LTE AcT bit has been set at all*/
  if(rplmn_rat_search_order == REG_SIM_MODE_PREF_LTE_RADIO_ACCESS_PREFERRED)
  {  
      int position_NB1 = -1;
      int position_M1 = -1;
      /* Check if other LTE rats are present */
      reg_sim_is_rat_present_3GPP_capability_list(req_rat_list, 
                SYS_RAT_LTE_NB1_RADIO_ACCESS, &position_NB1);

      reg_sim_is_rat_present_3GPP_capability_list(req_rat_list, 
                SYS_RAT_LTE_M1_RADIO_ACCESS, &position_M1);

      
      if ((position_NB1 != -1) || (position_M1 != -1))
      {
          /* At least one of the non-legacy LTE rats is present*/
          reg_nv_read_rplmnact(&rplmnact);
          
          if ((position_NB1 != -1)&&( rplmnact.act[1] & LTE_NB1_ACT_BIT))
          {      
               /* NB1 is present and the NB1 bit is set */
               rplmn_rat_search_order = REG_SIM_MODE_PREF_LTE_NB1_RADIO_ACCESS_PREFERRED;
          }
          else if ((position_M1 != -1)&&( rplmnact.act[1] & LTE_M1_ACT_BIT))
          {      
              /* M1 is present and the M1 bit is set */
              rplmn_rat_search_order = REG_SIM_MODE_PREF_LTE_M1_RADIO_ACCESS_PREFERRED;
          }
      }
  }
#endif
...
case REG_SIM_MODE_PREF_TDSCDMA_RADIO_ACCESS_PREFERRED:
    ...
    case REG_SIM_MODE_PREF_LTE_RADIO_ACCESS_PREFERRED:
      ...
      break;
    case REG_SIM_MODE_PREF_LTE_M1_RADIO_ACCESS_PREFERRED:
      MSG_HIGH_DS_0(REG_SUB,"=REG= LAST RPLMN RAT LTE M1");
      break;
    case REG_SIM_MODE_PREF_LTE_NB1_RADIO_ACCESS_PREFERRED:
      MSG_HIGH_DS_0(REG_SUB,"=REG= LAST RPLMN RAT LTE NB1");
      break;
#endif
...
)
{
  ...
  */
  ...
  if (reg_state_is_active_rat_lte(rat))
  {
    rplmnact.act[0] = 0x40;
#ifdef FEATURE_CIOT
   /* Need to set bits to identify the flavor of LTE RAT */
    if (rat == SYS_RAT_LTE_M1_RADIO_ACCESS)
    {
        rplmnact.act[1] = LTE_M1_ACT_BIT;
    }
    else if (rat == SYS_RAT_LTE_NB1_RADIO_ACCESS)
    {
        rplmnact.act[1] = LTE_NB1_ACT_BIT;
    }
    else 
    {
         rplmnact.act[1] = 0x00;
  ...
  */
  if ((rat == SYS_RAT_GSM_RADIO_ACCESS) || reg_state_is_active_rat_lte(rat))
  {
  ...
  MSG_HIGH_DS_3(REG_SUB, "=REG= Writing RPLMN ACT %d %d cache for RAT - %d", rplmnact.act[0],rplmnact.act[1],rat);
...
)
{
  ...
  memset(&fplmn_list, 0xFF, sizeof(reg_sim_plmn_list_s_type));
  /*
  ...
  */
  ...
  MSG_HIGH_DS_1(REG_SUB,"=REG= FPLMN list length = %d", fplmn_list.length);
  ...
  if (TRUE == include_temp_fplmn)
  {
     /* Append the temporary FPLMN list starting from index = "pos" which 
     ** If end of list reached, overwrite the top entries in the list 
     */
#ifdef FEATURE_DUAL_SIM
     for (i = 0; i < reg_sim_dos_mitigation_fplmn_list_sim[as_id].length; i++)
#else
     for (i = 0; i < reg_sim_dos_mitigation_fplmn_list.length; i++)
#endif
     {
        if (pos == REG_SIM_PLMN_LIST_LENGTH)
        {
           pos = 0;
        }
#ifdef FEATURE_DUAL_SIM
        if (reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[i].is_forbidden)
        {
           fplmn_list_p->plmn[pos] = reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[i].plmn;
           pos++;
           fplmn_list_p->length++;
        }
#else
        if (reg_sim_dos_mitigation_fplmn_list.plmn_info[i].is_forbidden)
        {
           fplmn_list_p->plmn[pos] = reg_sim_dos_mitigation_fplmn_list.plmn_info[i].plmn;
           pos++;
           fplmn_list_p->length++;
        }
#endif
     }
     status = (pos > 0) ? TRUE: FALSE;
  }

   /*
  ** Output the Forbidden PLMN List.
  */
  MSG_HIGH_DS_1(REG_LOCAL_SUB, "=REG= Forbidden PLMN list (length = %d)", fplmn_list_p->length);

  if (fplmn_list_p->length > 0)
  {
    MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG=  # MCC-MNC");

    for (i = 0; i < fplmn_list_p->length; i++)
    {
      sys_plmn_get_mcc_mnc
      (
        fplmn_list_p->plmn[i],
      ...
      if(!sys_plmn_id_is_undefined(fplmn_list_p->plmn[i]))
      {
        if(mnc_includes_pcs_digit)
        {
          MSG_HIGH_DS_3(REG_LOCAL_SUB,"=REG= %2lu %03lu-%03lu", i, mcc, mnc);
...
)
{
  ...
  */
#ifdef FEATURE_DUAL_SIM
  status = reg_sim_read_fplmn_list(&fplmn_list, TRUE, as_id);
#else
  status = reg_sim_read_fplmn_list(&fplmn_list, TRUE);
#endif
...
#ifdef FEATURE_DUAL_SIM
            (TRUE == reg_sim_remove_plmn_from_dos_mitigation_fplmn_list(rplmn, as_id))
#else
            (TRUE == reg_sim_remove_plmn_from_dos_mitigation_fplmn_list(rplmn))
#endif

       )
      {
         MSG_HIGH_DS_0(REG_LOCAL_SUB, "=REG= Remove PLMN from DoS FPLMN list");
         list_modified = TRUE;
      }
      else
      {
...
)
{
  ...
  MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= reg_sim_add_plmn_to_fplmn_list(): PLMN %x %x %x, dos_mitigation_req = %d", 
                plmn.identity[0], plmn.identity[1], plmn.identity[2], dos_mitigation_req);
...
MSG_ERROR_DS_0(REG_LOCAL_SUB, "=REG= Add HPLMN to FPLMN list");
     ...
     (reg_is_t3245_timer_active() == FALSE))
  {
     ...
     */
   #ifdef FEATURE_DUAL_SIM
     status = reg_sim_read_fplmn_list(&fplmn_list, FALSE, as_id);
   #else
     status = reg_sim_read_fplmn_list(&fplmn_list, FALSE);
   #endif
  ...
  {
#ifdef FEATURE_DUAL_SIM
     list_modified = reg_sim_add_plmn_to_dos_mitigation_fplmn_list(plmn, as_id);
#else
     list_modified = reg_sim_add_plmn_to_dos_mitigation_fplmn_list(plmn);
#endif
  }
  return list_modified;
}
...
)
{
  ...
  ** Read the Forbidden PLMN List from the SIM.
  */
  status = reg_sim_read_fplmn_list(&fplmn_list, TRUE, sub_id);
...
)
{
  ...
  */
  if ( !reg_sim_plmn_backoff_forbidden(plmn,as_id) )
  {
    ...
    */
    if ( reg_sim_backoff_fplmn_list.length < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH )
    {
      reg_sim_backoff_fplmn_list.length++;
    }
    /*
    ...
    */
    ...
    {
      reg_sim_backoff_fplmn_list.length = REG_SIM_BACKOFF_FPLMN_LIST_LENGTH;
...
)
{
  ...
  reg_sim_backoff_fplmn_list.length = MIN(reg_sim_backoff_fplmn_list.length,
                                                    REG_SIM_BACKOFF_FPLMN_LIST_LENGTH);
...
DESCRIPTION
  This function restores the backoff FPLMN list upon wake up from PSM sleep.

RETURN VALUE
  None
==============================================================================*/
void reg_sim_restore_psm_backoff_fplmn_list(void)
{
  byte i = 0, j = 0;
  uint64 time_elapsed = 0;
  rex_timer_cnt_type time_remaining = 0, min_timer_count = 0;

  reg_check_for_null_ptr((void*)reg_nv_psm_cache_ptr);
        
  time_elapsed = time_get_ms_native() - reg_nv_psm_cache_ptr->timers_timestamp;

  /* Go through the entire fplmn list and re-adjust timer count if the timer has not
     expired, or remove the fplmn from the list if the timer has expired in psm sleep */
  for(i = 0; (i < reg_nv_psm_cache_ptr->backoff_fplmn_list.length) &&
                                             (i < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH); i++)
  {
    if(reg_nv_psm_cache_ptr->backoff_fplmn_list.plmn_timer_list[i].timer_count != 
                                                    REG_TIMERS_MAX_TIMER_VALUE)
    {
      time_remaining = (reg_nv_psm_cache_ptr->backoff_fplmn_list.plmn_timer_list[i].timer_count +
                        reg_nv_psm_cache_ptr->backoff_fplmn_rem_duration);

      if((uint64)time_remaining > time_elapsed)
      {
        /* Timer has not expired in psm sleep, re-adjust the timer count */
        reg_nv_psm_cache_ptr->backoff_fplmn_list.plmn_timer_list[i].timer_count = 
                                       time_remaining - (rex_timer_cnt_type)time_elapsed;
      }
      else /* Timer has expired in psm sleep, remove fplmn from list */
      {
        reg_nv_psm_cache_ptr->backoff_fplmn_list.length--;

        for (j = i; j < reg_nv_psm_cache_ptr->backoff_fplmn_list.length; j++)
        {
          reg_nv_psm_cache_ptr->backoff_fplmn_list.plmn_timer_list[j] = 
                       reg_nv_psm_cache_ptr->backoff_fplmn_list.plmn_timer_list[j + 1];
        }
      }
    }
  }
  ...
  {
    reg_sim_backoff_fplmn_list = reg_nv_psm_cache_ptr->backoff_fplmn_list;

    for(i = 0; (i < reg_sim_backoff_fplmn_list.length) &&
                                               (i < REG_SIM_BACKOFF_FPLMN_LIST_LENGTH); i++)
    {
      if(((min_timer_count == 0) ||
          (min_timer_count > reg_sim_backoff_fplmn_list.plmn_timer_list[i].timer_count)) &&
...
)
{
  ...
  */
  if ( reg_sim_find_plmn_in_fplmn_counter_list(rplmn, &reg_sim_gprs_fplmn_list_sim[as_id], &position) )
  {
    /*
    ...
    */
    ...
    MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Removed RPLMN %x %x %x from GPRS forbidden list. New list length = %d",
                  rplmn.identity[0], rplmn.identity[1],rplmn.identity[2], reg_sim_gprs_fplmn_list_sim[as_id].length);

    for (i = position; i < reg_sim_gprs_fplmn_list_sim[as_id].length; i++)
    {
      reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i].plmn = reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i+1].plmn;
      reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i].counter = reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i+1].counter;
      reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i].is_forbidden = reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[i + 1].is_forbidden;
    }
    list_updated = TRUE;
  ...
  ** Search for the RPLMN in the GPRS Forbidden PLMN List.
  */
  if ( reg_sim_find_plmn_in_fplmn_counter_list(rplmn, &reg_sim_gprs_fplmn_list, &position) )
  {
    /*
    ** If the RPLMN is found in the GPRS Forbidden PLMN List then
    ** remove the RPLMN from the list.
    */
    ...
    MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Removed RPLMN %x %x %x from GPRS forbidden list. New list length = %d",
                  rplmn.identity[0], rplmn.identity[1],rplmn.identity[2], reg_sim_gprs_fplmn_list.length);  

    for (i = position; i < reg_sim_gprs_fplmn_list.length; i++)
    {
      reg_sim_gprs_fplmn_list.plmn_info[i].plmn = reg_sim_gprs_fplmn_list.plmn_info[i+1].plmn;
      reg_sim_gprs_fplmn_list.plmn_info[i].counter = reg_sim_gprs_fplmn_list.plmn_info[i+1].counter;
      reg_sim_gprs_fplmn_list.plmn_info[i].is_forbidden = reg_sim_gprs_fplmn_list.plmn_info[i+1].is_forbidden; 
    }
    list_updated = TRUE;
    reg_sim_print_gprs_forbidden_plmn_list();
  }
...
)
{
  ...
  */
  ...
  if(!dos_mitigation_req &&
     reg_sim_check_if_t3245_behavior_enabled(as_id) &&
     (reg_is_t3245_timer_active(as_id) == FALSE))
  {
    reg_timers_start_t3245_timer(REG_T3245_DEFUALT_VALUE,as_id);
  }
  ...
  ** GPRS Forbidden PLMN List.
  */
  if ( !reg_sim_find_plmn_in_fplmn_counter_list(plmn, &reg_sim_gprs_fplmn_list_sim[as_id], &position) )
  {
    /*
    ...
    ** Copy the PLMN into the empty position.
    */
    reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[reg_sim_gprs_fplmn_list_sim[as_id].length - 1].plmn = plmn;
    reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[reg_sim_gprs_fplmn_list_sim[as_id].length - 1].is_forbidden = TRUE;
    if (dos_mitigation_req)
    {
      reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[reg_sim_gprs_fplmn_list_sim[as_id].length - 1].counter++;
    }
    else
    {
       /*Set counter to 0xFF to distinguish PLMNs added due to DoS mitigation*/
       reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[reg_sim_gprs_fplmn_list_sim[as_id].length - 1].counter = 0xFF;
    }

    list_updated = TRUE;
    MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Added new entry with PLMN %x %x %x to GPRS forbidden list. New list length = %d",
                  plmn.identity[0], plmn.identity[1],plmn.identity[2], reg_sim_gprs_fplmn_list_sim[as_id].length);

  }
  else
  {
    if (dos_mitigation_req && 
        (reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[position].counter < mm_emm_max_attempt_counter_dos_mitigation_sim[as_id]))
    {
       reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[position].counter++;
       reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[position].is_forbidden = TRUE;
       list_updated = TRUE;
       MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Updated counter for PLMN %x %x %x already in GPRS forbidden list. New list length = %d",
                  plmn.identity[0], plmn.identity[1],plmn.identity[2], reg_sim_gprs_fplmn_list_sim[as_id].length);
    }
    else
    {
       reg_sim_gprs_fplmn_list.plmn_info[position].is_forbidden = TRUE;
       MSG_ERROR_DS_0(REG_LOCAL_SUB, "=REG= PLMN already in GPRS FPLMN list");
    }
  }
  ...
  ** GPRS Forbidden PLMN List.
  */
  if ( !reg_sim_find_plmn_in_fplmn_counter_list(plmn, &reg_sim_gprs_fplmn_list, &position) )
  {
    /*
    ** The PLMN is not already contained in the GPRS Forbidden PLMN
    ** List so simply add it to the bottom of the list.
    ...
    ** Copy the PLMN into the empty position.
    */
    reg_sim_gprs_fplmn_list.plmn_info[reg_sim_gprs_fplmn_list.length - 1].plmn = plmn;
    reg_sim_gprs_fplmn_list.plmn_info[reg_sim_gprs_fplmn_list.length - 1].is_forbidden = TRUE;
    if (dos_mitigation_req)
    {
      reg_sim_gprs_fplmn_list.plmn_info[reg_sim_gprs_fplmn_list.length - 1].counter++;
    }
    else
    {
       /*Set counter to 0xFF to distinguish PLMNs added due to DoS mitigation*/
       reg_sim_gprs_fplmn_list.plmn_info[reg_sim_gprs_fplmn_list.length - 1].counter = 0xFF;
    }

    MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Added PLMN %x %x %x to GPRS forbidden list. New list length = %d",
                  plmn.identity[0], plmn.identity[1],plmn.identity[2], reg_sim_gprs_fplmn_list.length);
    list_updated = TRUE;
  }
  else
  {
    if (dos_mitigation_req && 
        (reg_sim_gprs_fplmn_list.plmn_info[position].counter < mm_emm_max_attempt_counter_dos_mitigation))
    {
       reg_sim_gprs_fplmn_list.plmn_info[position].counter++;
       reg_sim_gprs_fplmn_list.plmn_info[position].is_forbidden = TRUE;
       list_updated = TRUE;
       MSG_HIGH_DS_4(REG_LOCAL_SUB, "=REG= Updated counter for PLMN %x %x %x already present in GPRS forbidden list. New list length = %d",
                  plmn.identity[0], plmn.identity[1],plmn.identity[2], reg_sim_gprs_fplmn_list.length);
    }
    else
    {
...
FUNCTION NAME
  ...
  reg_sim_remove_plmn_from_dos_mitigation_fplmn_list

==============================================================================*/
boolean reg_sim_remove_plmn_from_dos_mitigation_fplmn_list
(
  sys_plmn_id_s_type plmn
#ifdef FEATURE_DUAL_SIM
  ,sys_modem_as_id_e_type as_id
...
)
{
  ...
  */
  if ( sys_plmn_id_is_undefined(plmn) )
  {
    MSG_ERROR_DS_0(REG_SUB, "=REG= Remove undefined PLMN from DoS FPLMN list");
    return result;
  }
  ...
  */
#ifdef FEATURE_DUAL_SIM
  if (reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, &reg_sim_dos_mitigation_fplmn_list_sim[as_id], &position) )
  {
     /*
     ** REG removes holes(invalid FPLMN) from the FPLMN list
     */
     for (; (position < reg_sim_dos_mitigation_fplmn_list_sim[as_id].length-1) && (position < REG_SIM_DOS_MITIGATION_PLMN_LIST_LENGTH-1); position++)
     {
       reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position].plmn = reg_sim_dos_mitigation_fplmn_list_sim[as_id].plmn_info[position+1].plmn;
     }

     result = TRUE;
  }
#else
  if (reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, &reg_sim_dos_mitigation_fplmn_list, &position) )
  {
     /*
     ** REG removes holes(invalid FPLMN) from the FPLMN list
     */
     for (; (position < reg_sim_dos_mitigation_fplmn_list.length-1) && (position < REG_SIM_DOS_MITIGATION_PLMN_LIST_LENGTH-1); position++)
     {
       reg_sim_dos_mitigation_fplmn_list.plmn_info[position].plmn = reg_sim_dos_mitigation_fplmn_list.plmn_info[position+1].plmn;
     }

     result = TRUE;
  }
#endif
  MSG_HIGH_DS_4(REG_SUB, "=REG=  reg_sim_remove_plmn_from_dos_mitigation_fplmn_list():PLMN %x %x %x, result %d",
                  plmn.identity[0], plmn.identity[1],plmn.identity[2], result);
  return result;
}

/*===========================================================================

FUNCTION  reg_sim_add_plmn_to_dos_mitigation_fplmn_list

DESCRIPTION
If PLMN is not present in DoS Mitigation FPLMN list then this function add PLMN 
to the list and updates the corresponding counter
...
===========================================================================*/
boolean reg_sim_add_plmn_to_dos_mitigation_fplmn_list
(
  sys_plmn_id_s_type plmn
#ifdef FEATURE_DUAL_SIM
  ,sys_modem_as_id_e_type as_id
...
)
{
  ...
  uint32 position = 0;
  int32  i;
  reg_sim_dos_mitigation_plmn_list_s_type* dos_fplmn_list;

  /*
  ** Verify that the PLMN is defined before adding it to the Forbidden PLMN List.
  */
  if ( sys_plmn_id_is_undefined(plmn) )
  {
    MSG_ERROR_DS_0(REG_SUB, "=REG= Add undefined PLMN to FPLMN list");
    return list_updated;
  }
#ifdef FEATURE_DUAL_SIM
  dos_fplmn_list = &reg_sim_dos_mitigation_fplmn_list_sim[as_id];
 #else
  dos_fplmn_list = &reg_sim_dos_mitigation_fplmn_list;
#endif

  /*
  ** Check to see if the PLMN is already contained in the Forbidden PLMN List.
  */
  if ( !reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, dos_fplmn_list, &position) )
  {
    /*
    ** The PLMN is not already contained in the Forbidden PLMN
    ** List so simply add it to the bottom of the list.
    */
    if (dos_fplmn_list->length < mm_emm_max_fplmn_count_dos_mitigation )
    {
      dos_fplmn_list->length++;
    }
    /*
    ** The Forbidden PLMN List is full so the first (oldest) PLMN
    ** is removed from the top of the list and the new PLMN is added
    ...
    */
    ...
    {
      dos_fplmn_list->length = mm_emm_max_fplmn_count_dos_mitigation;

      for (i = 0; i < (int32)(dos_fplmn_list->length - 1); i++)
      {
        dos_fplmn_list->plmn_info[i].plmn = dos_fplmn_list->plmn_info[i + 1].plmn;
        dos_fplmn_list->plmn_info[i].counter = dos_fplmn_list->plmn_info[i + 1].counter;
        dos_fplmn_list->plmn_info[i].is_forbidden= dos_fplmn_list->plmn_info[i + 1].is_forbidden;
      }
    }
    ...
    */
    ...
    MSG_HIGH_DS_4(REG_SUB, "=REG= Adding PLMN %x %x %x to temp DoS FPLMN, length = %d ",
                  plmn.identity[0], plmn.identity[1], plmn.identity[2], dos_fplmn_list->length);
    dos_fplmn_list->plmn_info[dos_fplmn_list->length - 1].plmn = plmn;

    dos_fplmn_list->plmn_info[dos_fplmn_list->length - 1].counter++;

    dos_fplmn_list->plmn_info[dos_fplmn_list->length - 1].is_forbidden = TRUE;
    list_updated = TRUE;
  }
  else
  {
    if (dos_fplmn_list->plmn_info[position].counter < mm_emm_max_attempt_counter_dos_mitigation)
    {
      dos_fplmn_list->plmn_info[position].counter++;
      MSG_HIGH_DS_4(REG_SUB, "=REG= Incrementing counter for PLMN %x %x %x to temp DoS FPLMN, length = %d ",
                    plmn.identity[0], plmn.identity[1], plmn.identity[2], dos_fplmn_list->length);
       if (dos_fplmn_list->plmn_info[position].is_forbidden == FALSE)
      {
         dos_fplmn_list->plmn_info[position].is_forbidden = TRUE;
         list_updated = TRUE;
      }

      if (dos_fplmn_list->plmn_info[position].counter == mm_emm_max_attempt_counter_dos_mitigation)
      {
         MSG_HIGH_DS_0(REG_SUB, "=REG= Adding temp FPLMN to SIM FPLMN ");
//         (void)reg_sim_remove_plmn_from_dos_mitigation_fplmn_list(plmn);
#ifdef FEATURE_DUAL_SIM
                           reg_sim_remove_plmn_from_dos_mitigation_fplmn_list(plmn, as_id);
#else
                           reg_sim_remove_plmn_from_dos_mitigation_fplmn_list(plmn);
#endif
#ifdef FEATURE_DUAL_SIM
         return reg_sim_add_plmn_to_fplmn_list(plmn, FALSE, (sys_modem_as_id_e_type)reg_sub_id);
#else
         return  reg_sim_add_plmn_to_fplmn_list(plmn, FALSE); 
#endif /*FEATURE_DUAL_SIM */
       }
       else
       {
         MSG_ERROR_DS_0(REG_SUB, "=REG= Max attempts exceeded for PLMN");
       }
    }
  }
...
)
{
  ...
  */
  ...
  if(reg_nv_read_data_roaming() == SYS_DATA_ROAMING_INTERNATIONAL_OFF)
  {
    sys_plmn_get_mcc_mnc
    (
      plmn,
    ...
    if(data_roaming_disabled)
    {
      if(mnc_includes_pcs_digit)
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= International roaming disabled, PS not allowed on PLMN %03lu-%03lu", mcc,mnc);
      }
      else
      {
        MSG_HIGH_DS_2(REG_SUB, "=REG= International roaming disabled, PS not allowed on PLMN %03lu-%02lu", mcc,mnc);
      }
      return TRUE;
    }
  }
  ...
  if (reg_sim_find_plmn_in_fplmn_counter_list( plmn, &reg_sim_gprs_fplmn_list_sim[as_id], &position ))
  {
     return reg_sim_gprs_fplmn_list_sim[as_id].plmn_info[position].is_forbidden;
  }
#else
  if (reg_sim_find_plmn_in_fplmn_counter_list( plmn, &reg_sim_gprs_fplmn_list, &position ))
  {
     return reg_sim_gprs_fplmn_list.plmn_info[position].is_forbidden;
  }
#endif
  return FALSE;
}
...
)
{
  ...
  for (i = 0; ((i < plmn_list_p->length) && (i < REG_SIM_PLMN_LIST_LENGTH)); i++)
  {
...
void reg_sim_imp_variable_prints()
{
  ...
  (
   ...
   else
   {
     ...
     */
#ifdef FEATURE_DUAL_SIM
      status = reg_sim_read_fplmn_list(&fplmn_list, FALSE, (sys_modem_as_id_e_type)reg_sub_id);
#else
      status = reg_sim_read_fplmn_list(&fplmn_list, FALSE);
#endif
...
)
{
  ...
  memset((void *)&file_tst[0],0x00,sizeof(file_tst));
...
#else
#ifdef FEATURE_SGLTE
      memscpy(&reg_sim_mmgsdi_info_sim[REG_AS_ID_1].session_id, sizeof(mmgsdi_session_id_type), &(cnf_ptr->session_open_cnf.session_id),
             sizeof(mmgsdi_session_id_type));
...
#else
      memscpy(&reg_sim_mmgsdi_info.session_id, sizeof(mmgsdi_session_id_type), &(cnf_ptr->session_open_cnf.session_id),
              sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(REG_SUB, "=REG= MMGSDI session open cnf Status %d Session ID: 0x%x %x", 
                    status, 
...
#if defined(FEATURE_SGLTE) || defined(FEATURE_DUAL_SIM)
                reg_sim_num_fcn_files_sim[reg_sim_as_id] == reg_sim_num_file_read[reg_sim_as_id]
#else
                reg_sim_num_fcn_files == reg_sim_num_file_read	
#endif
				)
            {
              reg_sim_refresh_complete
...
FUNCTION NAME
  ...
  reg_sim_mmgsdi_steering_of_roaming_refresh_ind

==============================================================================*/
boolean reg_sim_mmgsdi_steering_of_roaming_refresh_ind
(
    const mmgsdi_data_type *       refresh_plmn_list
)
 {
      reg_cmd_type *reg_cmd_p;
      uint32  i;
      ...
      if ((reg_cmd_p = reg_get_cmd_buf()) != NULL)
      {
       ...
       // memscpy(&reg_cmd_p->cmd.sim_refresh_ind.refresh, sizeof(reg_cmd_p->cmd.sim_refresh_ind.refresh),
        ...
        reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_len = MIN( refresh_plmn_list->data_len  , (REG_SIM_PLMN_LIST_LENGTH * REG_SIM_PLMN_W_ACT_LENGTH));
#ifdef FEATURE_MODEM_HEAP
...
#endif
        reg_cmd_p->cmd.sim_refresh_ind.refresh.stage = MMGSDI_REFRESH_STAGE_START;
        reg_cmd_p->cmd.sim_refresh_ind.refresh.mode = MMGSDI_REFRESH_STEERING_OF_ROAMING;
        ...
        memscpy(reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_ptr,(unsigned int)(reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_len) ,
                refresh_plmn_list->data_ptr,(unsigned int)(refresh_plmn_list->data_len));
        ...
        for (i = 0; i <( uint32)(reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_len); i=i+REG_SIM_PLMN_W_ACT_LENGTH)
        {
           MSG_HIGH_DS_3(REG_SUB, "=REG= OPLMN list  %X %X %X",
                             reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_ptr[i],
                             reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_ptr[i+1],
                             reg_cmd_p->cmd.sim_refresh_ind.refresh.plmn_list.data_ptr[i+2]);
        }
      }
      /*
        ** Put on REG command queue
     ...
     */
                                             ...
                                             &file_length) == MMGSDI_SUCCESS)
     {
       ...
       */
       ...
       {
          MSG_ERROR_DS_0(REG_SUB, "=REG= Read EF EHPLMN list failed or OEM EHPLMN undefined or USIM compliant, use NV");    
     ...
     {
          MSG_ERROR_DS_0(REG_SUB, "=REG= Read EF EHPLMN attributes list failed or OEM EHPLMN undefined or USIM compliant, use EFS");
          /*Read the EHPLMN list from EFS_NV file*/
          status = reg_nv_read_efs_ehplmn_file(&ehplmn_list);
     }
  } /* reg_sim_card_mode == MMGSDI_APP_USIM */
...
)
{
  ...
  sub_id = REG_LOCAL_SUB;
#endif
  MSG_HIGH_DS_0(sub_id, "=REG= Low Priority PLMN list cleared!!");
}
...
)
{
  ...
  client_id = NAS_MMGSDI_INVALID_CLIENT_ID;
  /* Register the MMGSDI event callback function */

  client_id = mmgsdi_client_id_and_evt_reg_sync( reg_sim_mmgsdi_evt_cb, NULL, 0);

  if (client_id != NAS_MMGSDI_INVALID_CLIENT_ID)
  {
  ...
  {
    MSG_ERROR_DS_1(REG_SUB, "=REG=  MMGSDI client ID  cmd fail : STATUS %d",(uint8) mmgsdi_status);
  }

  MSG_HIGH_DS_1(REG_SUB, "=REG= Reg called MMGSDI to register for client ID and evt, MMGSDI Registration STATUS: %d",mmgsdi_status);
}
...
)
{
  ...
  else
  {
...
#endif
     mmgsdi_status = mmgsdi_session_open_ext_sync(reg_sim_mmgsdi_info.client_id,
             session_type_mask, reg_sim_session_generic_event_cb,
             &mmgsdi_session_info_ptr);

    if ( mmgsdi_status == MMGSDI_SUCCESS )
    {
      MSG_HIGH_DS_2(REG_LOCAL_SUB, "=REG= mmgsdi_session_open_ext_sync() success client_id %d, session_type_mask %d", reg_sim_mmgsdi_info.client_id, session_type_mask);

#ifdef FEATURE_DUAL_SIM
      rex_enter_crit_sect(&reg_sim_crit_sect);
      if(reg_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].session_type 
           == (mmgsdi_session_type_enum_type) mmgsdi_session_info_ptr.session_type)
      { 
        as_id = SYS_MODEM_AS_ID_1;
      }
      else if(reg_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_2].session_type 
      ...
      {
        MSG_FATAL_DS(REG_LOCAL_SUB, "=REG= Invalid session type: %d", mmgsdi_session_info_ptr.session_type, 0, 0);
      }

      memscpy(&reg_sim_mmgsdi_info_sim[as_id].session_id, sizeof(mmgsdi_session_id_type) , &(mmgsdi_session_info_ptr.session_id),
                                        sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(REG_LOCAL_SUB, "=REG= MMGSDI session open cnf Status %d Session ID: 0x%x %x", 
                    mmgsdi_status, 
                    (dword)(reg_sim_mmgsdi_info_sim[as_id].session_id >> 32), 
...
#else
#ifdef FEATURE_SGLTE
      memscpy(&reg_sim_mmgsdi_info_sim[REG_AS_ID_1].session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
             sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(REG_SUB, "=REG= MMGSDI session open cnf Status %d Session ID: 0x%x %x", 
                    mmgsdi_status, 
                    (dword)(reg_sim_mmgsdi_info_sim[REG_AS_ID_1].session_id >> 32), 
                    (dword)(reg_sim_mmgsdi_info_sim[REG_AS_ID_1].session_id & 0xFFFFFFFF));
#else
      memscpy(&reg_sim_mmgsdi_info.session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
              sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(REG_SUB, "=REG= MMGSDI session open cnf Status %d Session ID: 0x%x %x", 
                    mmgsdi_status, 
                    (dword)(reg_sim_mmgsdi_info.session_id >> 32), 
...
#endif
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_SGLTE)
        reg_cmd_p->cmd.sim_subs_ready_ind.slot_id = mmgsdi_session_info_ptr.slot_id;
#endif
        reg_put_cmd(reg_cmd_p);
      }
      else
    ...
    {
      MSG_ERROR_DS_2(REG_SUB, "ERROR:  mmgsdi_session_open_ext_sync() status %d, session_type_mask %d", (uint8) mmgsdi_status, session_type_mask);
    }
...
)
{
   ...
   ** Verify that the PLMN is defined before removing it from the
   ...
   memset(&reg_sim_dos_mitigation_fplmn_list, 0, sizeof(reg_sim_dos_mitigation_plmn_list_s_type));
   reg_sim_dos_mitigation_fplmn_list.length = 0;
...
boolean reg_sim_is_plmn_dos_forbidden
(
...
)
{
   ...
   for(i = 0; i < dos_fplmn_list->length ; i++)
   {
     if ( (sys_plmn_match(plmn_id,dos_fplmn_list->plmn_info[i].plmn)) &&
          (dos_fplmn_list->plmn_info[i].is_forbidden == TRUE) &&
          (dos_fplmn_list->plmn_info[i].counter <  mm_emm_max_attempt_counter_dos_mitigation) )
     {
       result = TRUE;
     }
   }

   MSG_HIGH_DS_4(REG_SUB, "=REG= PLMN %x %x %x present in temp DoS FPLMN list = %d",
                  plmn_id.identity[0], plmn_id.identity[1], plmn_id.identity[2], result);
   return result;
}

boolean reg_sim_is_plmn_dos_gprs_forbidden
(
  sys_plmn_id_s_type plmn_id
#ifdef FEATURE_DUAL_SIM
  ,sys_modem_as_id_e_type as_id
...
)
{
...
#ifdef FEATURE_DUAL_SIM
    dos_fplmn_list = (reg_sim_dos_mitigation_plmn_list_s_type*)&reg_sim_gprs_fplmn_list_sim[as_id];
#else
    dos_fplmn_list = (reg_sim_dos_mitigation_plmn_list_s_type*)&reg_sim_gprs_fplmn_list;
#endif
   ...
   MSG_HIGH_DS_4(REG_SUB, "=REG= PLMN %x %x %x present in temp GPRS DoS FPLMN list = %d",
                  plmn_id.identity[0], plmn_id.identity[1], plmn_id.identity[2], result);
   return result;
}

void reg_sim_cleanup_dos_fplmn_list_on_t3247_exp
(
#ifdef FEATURE_DUAL_SIM
  sys_modem_as_id_e_type as_id
#else
...
)
{
   uint32 i = 0;
   boolean status = FALSE;
   reg_sim_fplmn_counter_list_s_type* fplmn_list;
   reg_sim_dos_mitigation_plmn_list_s_type* dos_fplmn_list;

#ifdef FEATURE_DUAL_SIM
   fplmn_list = &reg_sim_gprs_fplmn_list_sim[as_id];
   dos_fplmn_list = &reg_sim_dos_mitigation_fplmn_list_sim[as_id];
#else
   fplmn_list = &reg_sim_gprs_fplmn_list;
   dos_fplmn_list = &reg_sim_dos_mitigation_fplmn_list;
#endif

   MSG_HIGH_DS_0(REG_SUB, "=REG= reg_sim_cleanup_dos_fplmn_list_on_t3247_exp()");

   /*Mark GPRS PLMN as not forbidden if counter < max */
   for(i = 0; i < fplmn_list->length ; i++)
   {
     if ( (sys_plmn_id_is_undefined(fplmn_list->plmn_info[i].plmn) == FALSE) &&
          (fplmn_list->plmn_info[i].is_forbidden == TRUE) &&
          (fplmn_list->plmn_info[i].counter <  mm_emm_max_attempt_counter_dos_mitigation) )
     {
       fplmn_list->plmn_info[i].is_forbidden = FALSE;
       status = TRUE;
     }
   }
   ...
   status = FALSE;
   /*Mark FPLMN as not forbidden if counter < max */
   for(i = 0; i < dos_fplmn_list->length ; i++)
   {
     if ( (sys_plmn_id_is_undefined(dos_fplmn_list->plmn_info[i].plmn) == FALSE) &&
          (dos_fplmn_list->plmn_info[i].is_forbidden == TRUE) &&
          (dos_fplmn_list->plmn_info[i].counter <  mm_emm_max_attempt_counter_dos_mitigation) )
     {
       dos_fplmn_list->plmn_info[i].is_forbidden = FALSE;
       status = TRUE;
     }
   }
...
void reg_sim_reset_dos_fplmn_list
(
#ifdef FEATURE_DUAL_SIM
  sys_modem_as_id_e_type as_id
#else
...
)
{
   ...
   MSG_HIGH_DS_0(REG_SUB, "=REG= reg_sim_reset_dos_fplmn_list()");
   ...
   /*Clear temporary DoS FPLMN list */
#ifdef FEATURE_DUAL_SIM
   memset(&reg_sim_dos_mitigation_fplmn_list_sim[as_id], 0, sizeof(reg_sim_dos_mitigation_plmn_list_s_type));
   reg_sim_dos_mitigation_fplmn_list_sim[as_id].length = 0;
#else
   memset(&reg_sim_dos_mitigation_fplmn_list, 0, sizeof(reg_sim_dos_mitigation_plmn_list_s_type));
   reg_sim_dos_mitigation_fplmn_list.length = 0;
#endif

   /*Clear GPRS FPLMN list entries which were added due to DoS mitigation */   
   for (i = 0; i < fplmn_list->length ; i++)
   {
     if ( (sys_plmn_id_is_undefined(fplmn_list->plmn_info[i].plmn) == FALSE) &&
          (fplmn_list->plmn_info[i].counter < mm_emm_max_attempt_counter_dos_mitigation) )
     {
       fplmn_list->plmn_info[i].is_forbidden = FALSE;
       fplmn_list->plmn_info[i].counter =  0;
     }
   }
...
void reg_sim_reset_plmn_counter_in_fplmn_list
(
  sys_plmn_id_s_type plmn
#ifdef FEATURE_DUAL_SIM
  ,sys_modem_as_id_e_type as_id
...
)
{
   ...
   if (sys_plmn_id_is_undefined(plmn))
   {
     MSG_HIGH_DS_0(REG_SUB, "=REG= Incorrect PLMN received");
     return;
   }
   ...
   if (reg_sim_find_plmn_in_fplmn_counter_list(plmn, fplmn_list, &position) )
   {
     if ((fplmn_list->plmn_info[position].counter != 0xFF) &&
         (fplmn_list->plmn_info[position].counter > 0))
     {
        fplmn_list->plmn_info[position].is_forbidden = FALSE;
        fplmn_list->plmn_info[position].counter =  0;
        status = TRUE;
     }
   }

   MSG_HIGH_DS_4(REG_SUB, "=REG= reg_sim_reset_plmn_counter_in_fplmn_list(): GPRS FPLMN %x %x %x, status %d",
                 plmn.identity[0], plmn.identity[1], plmn.identity[2], status);
   ...
   if (reg_sim_find_plmn_in_dos_mitigation_counter_list(plmn, dos_fplmn_list, &position) )
   {
      if (dos_fplmn_list->plmn_info[position].counter > 0)
      {
         dos_fplmn_list->plmn_info[position].counter = 0;
         dos_fplmn_list->plmn_info[position].is_forbidden = FALSE;
         status = TRUE;
      }
   }

   MSG_HIGH_DS_4(REG_SUB, "=REG= reg_sim_reset_plmn_counter_in_fplmn_list(): Temp DoS FPLMN %x %x %x, status %d",
                 plmn.identity[0], plmn.identity[1], plmn.identity[2], status);
...
FUNCTION NAME
  ...
  reg_sim_check_mcc_same_country per sub

==============================================================================*/
boolean reg_sim_check_mcc_same_country_per_sub
(
  sys_plmn_id_s_type plmn,
  sys_modem_as_id_e_type as_id
)
