...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/reg/src/reg_mode_automatic_csg_selection.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
)
{
  ...
  {
    if(reg_state_is_active_rat_lte(initial_rat) || initial_rat == SYS_RAT_UMTS_RADIO_ACCESS)
    {
    ...
    else
    {
      ...
      {
        if((reg_state_is_active_rat_lte(rplmn_rat_search_order.rat_list[index]))|| 
...
boolean finished_automatic_csg
(
...
#ifdef FEATURE_CIOT
  reg_mode_unexpected_start_higher_priority_plmn_selection_ciot,
  reg_mode_unexpected_start_higher_priority_irat_plmn_selection_ciot,
#endif
  finished,
  number
};
