...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/reg/src/reg_task.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
static ULogHandle reg_ulog_handle = NULL;
/*==============================================================================
...
"/nv/item_files/modem/nas/ciot_params\n"\
"/nv/item_files/modem/nas/ciot_mccs_bands_params_m1\n"\
"/nv/item_files/modem/nas/ciot_mccs_bands_params_nb1\n"\
"/nv/item_files/modem/nas/ciot_mcc_rats_params\n"\
"/nv/item_files/modem/nas/interlace_scan_timer\n"\
"/nv/item_files/modem/nas/nb1_tier3_timers\n\
";
...
)
{
  ...
  reg_check_for_null_ptr((void*)cmd);
    ...
    case  CM_PSM_SET_CONFIG_PARAMS_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_psm_set_config_params_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_psm_set_config_params_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
    ...
    case  CM_PSM_GET_TIMER_INFO_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_psm_get_timer_info_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_psm_get_timer_info_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
    ...
    case  CM_ENTER_PSM_MODE_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_enter_psm_mode_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_enter_psm_mode_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
    ...
    case  CM_SET_EDRX_CFG_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_set_edrx_cfg_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_set_edrx_cfg_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
    ...
    case  CM_GET_EDRX_PARAMS_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_get_edrx_params_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_get_edrx_params_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
    case CM_MCC_CHANGE_IND:
        cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_mcc_change_ind_s_type) - sizeof(IMH_T)) % 256;
        cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_mcc_change_ind_s_type) - sizeof(IMH_T)) / 256;
        break;
    ...
    case CM_PSM_SAVE_MMGSDI_EF_REQ:
       cmd->cmd.hdr.message_len_lsb =
          (sizeof(cm_psm_save_mmgsdi_ef_req_s_type) - sizeof(IMH_T)) % 256;
      cmd->cmd.hdr.message_len_msb =
          (sizeof(cm_psm_save_mmgsdi_ef_req_s_type) - sizeof(IMH_T)) / 256;  
     break;
...
boolean reg_mcfg_refresh_cb(mcfg_refresh_info_s_type *p_info)
{
   ...
   reg_check_for_null_ptr((void *)p_info);
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle reg_get_ulog_handle()
{
  return reg_ulog_handle;
}

/*===========================================================================
FUNCTION reg_ulog_register

DESCRIPTION
  This function call registers REG Task with ULOG
...
SIDE EFFECTS
  None
===========================================================================*/
void reg_ulog_register(void)
{
  ULogResult result = ULogFront_RealTimeInit(&reg_ulog_handle, 
                               "NAS_REG_ULOG",	  
                               512,
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  MSG_HIGH_DS_1(REG_SUB,"=REG= ULOG REGISTER RESULT: %d", result);
}
