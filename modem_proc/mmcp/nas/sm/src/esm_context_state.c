...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_context_state.c#1 $
...
(
  struct esm_eps_bearer_context* ctxt_ptr, const emm_act_default_bearer_context_accept_T* msg_ptr   /*!< REQUIRED short parameter description */
)
{
  MSG_SPRINTF_3(MSG_SSID_LTE,
                MSG_LEGACY_HIGH, 
  ...
  if((msg_ptr->ext_protocol_conf_opt.valid == TRUE) &&
     (msg_ptr->ext_protocol_conf_opt.dsm_ptr != NULL))
  {
    ctxt_ptr->epco.dsm_ptr = msg_ptr->ext_protocol_conf_opt.dsm_ptr;
    dsm_free_packet((dsm_item_type**)&(ctxt_ptr->epco.dsm_ptr));
...
(
  struct esm_eps_bearer_context* ctxt_ptr, const emm_act_dedicated_bearer_context_accept_T* msg_ptr, struct esm_bcm* mgr_ptr   /*!< REQUIRED short parameter description */
)
{
  MSG_SPRINTF_3(MSG_SSID_LTE, 
                MSG_LEGACY_HIGH, 
  ...
  for(index = 0; index < msg_ptr->num_removed_bearers; index++)
  {
    ...
    else if(ctxt_ptr->eps_bearer_id == msg_ptr->removed_eps_bearers[index].eps_id)
    {
      /*
        ...
        CR 2134897 - UE workaround is implemented for abnormal network behavior which can lead 
        to huge delay for ping due to hung EPS bearer connection setup.

        If NW removes radio bearer in RRC reconfiguration message that is associated to an EPS 
        bearer which belongs to a PDN being disconnected by UE, UE will proceed with PDN 
        Disconnect procedure without deactivate the bearer locally. 
      
      */
      if (TRUE == esm_check_pdn_disconnect_sent_by_bid(ctxt_ptr->eps_bearer_id))
      {
        MSG_HIGH_DS_1(SM_SUB,"ESM: Ignoring this BID- %d for handling of late EPS bearer context deactivation"
                             "request for UE requested PDN disconnection",
...
#ifdef FEATURE_CIOT
              ||
              ctxt_ptr->cp_only == TRUE
#endif
           )
    {
      MSG_HIGH_DS_1(SM_SUB,"ESM: Active BC matching active EPS ind - BID %d or it is CP only type",
                    ctxt_ptr->eps_bearer_id);
      need_deactivation = FALSE;
      break;
    }
...
(
  struct esm_eps_bearer_context* ctxt_ptr,  /*!< REQUIRED short parameter description */
  const emm_modify_bearer_context_rej_T* msg_ptr   /*!< REQUIRED short parameter description */
)
{
  MSG_SPRINTF_3(MSG_SSID_LTE, 
                MSG_LEGACY_HIGH, 
...
(
  struct esm_eps_bearer_context* ctxt_ptr,  /*!< REQUIRED short parameter description */
  const emm_modify_bearer_context_accept_T* msg_ptr,   /*!< REQUIRED short parameter description */
  struct esm_bcm* mgr_ptr
)
{
  MSG_SPRINTF_3(MSG_SSID_LTE, 
