...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smutils.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#include "ULogFront.h"
extern volatile sm_esm_debug_buffer_s_type sm_esm_debug_buffer[SM_DEBUG_MESSAGE_BUFFER_SIZE];
...
extern void sm_send_sndcp_enter_psm_ind(void);
#endif
...
#ifdef FEATURE_MBMS
    case MBMS_RECEPTION_CLASS:
      MSG_HIGH_DS_0(SM_SUB,"MBMS: RRC EST cause = RRC_EST_MBMS_RECEPTION");
      rrc_cause = RRC_EST_MBMS_RECEPTION;
      break;      

    case MBMS_PTP_RB_REQUEST_CLASS:
      MSG_HIGH_DS_0(SM_SUB,"MBMS: RRC EST cause = RRC_EST_MBMS_PTP_RB_REQUEST");
      rrc_cause = RRC_EST_MBMS_PTP_RB_REQUEST;
      break;
#endif
...
byte sm_get_most_demanding_traffic_class(boolean include_active_pending)
{
  ...
  for (index = 0; index < MAX_PDP_NUM ; index++)
  {
    if (pdp_list[index] != NULL &&
        ...
        (include_active_pending && (pdp_list[index]->pdp_state == PDP_ACTIVE_PENDING)) ||
...
#else //FEATURE_NAS_REL12_SPEC_CHANGE
...
void sm_perform_local_pdp_deactivation(pdp_context_info *pdp)
{
  ...
  MSG_HIGH_DS_1(SM_SUB,"Locally deactivating PDP NSAPI %d",pdp->nsapi.nsapi);
    ...
    {
      MSG_HIGH_DS_1(SM_SUB, "Local Deactivation changing state to PDP_INACTIVE TI=%d",pdp->trans_id);
    ...
    #ifdef FEATURE_MBMS
    if (pdp->pdp_state == MBMS_ACTIVE_PENDING)
      {
        //Send the SMREG_MBMS_ACTIVATE_REJ to the CM.
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
        sm_send_cm_confirm(pdp, SMREG_MBMS_ACTIVATE_REJ, SM_NETWORK_CAUSE);
...
//#endif
  ...
  for (i=0; i<MAX_PDP_NUM; i++)
  {
    ...
    {
      MSG_HIGH_DS_1(SM_SUB,"MBMS: Deactivating MBMS %d locally", i);
      sm_perform_local_pdp_deactivation(pdp_list[i]);
    }
  } // MBMS_PENDING later part of the MBMS code will not be executed as all the MBMS contexts are set to PDP_INACTIVE here
#endif
  ...
  for (i=0; i<MAX_PDP_NUM; i++)
  {
        ...
        
    {
      ...
      #ifdef FEATURE_MBMS
      if (!pdp_list[i]->is_mbms)
        {
          MSG_HIGH_DS_1(SM_SUB, "MBMS: Local Deactivation changing state to PDP_INACTIVE TI=%d",i);
      #endif
...
===========================================================================*/
void sm_deact_associated_mbms_ctxt(const pdp_context_info *cur_pdp)
{
  word index;
  if (cur_pdp->is_mbms == TRUE)
  {
    MSG_HIGH_DS_0(SM_SUB,"MBMS: MBMS ctxt no associated PDP");
    return;
  }
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if (pdp_list[index] != NULL && 
        (pdp_list[index]->trans_id != INVALID_ID) && 
        (index != cur_pdp->trans_id) &&
        (pdp_list[index]->linked_ti == cur_pdp->trans_id) &&
        (pdp_list[index]->is_mbms == TRUE))
    {
      MSG_HIGH_DS_1(SM_SUB,"MBMS: Deactivating MBMS %d locally", index);
      sm_perform_local_pdp_deactivation(pdp_list[index]);
    }
  }
}
...
===========================================================================*/
void sm_all_mbms_local_deactivation ()
{
  word i;
  MSG_HIGH_DS_0(SM_SUB,"MBMS:Locally Deactivating all MBMS ");
    ...
    case MS_GMM_SM:
      switch (cmd_ptr->header.cmd_id)
      {
       ...
       case GMMSM_SAVE_CTXT_PSM_REQ:
          as_id = cmd_ptr->cmd.gmmsm_save_ctxt_psm_req.as_id;
          break;

       case GMMSM_RESTORE_CTXT_PSM_REQ:
          as_id = cmd_ptr->cmd.gmmsm_restore_ctxt_psm_req.as_id;
          break;
    ...
    case MS_CM_SM:
      switch (cmd_ptr->header.cmd_id)
      {
        ...
        case MSM_SM_ACTIVATION_REQ:
          as_id = cmd_ptr->cmd.mbms_act_req.as_id; 
          break;

        case MSM_SM_ACTIVATE_REJ_RSP:
          as_id = cmd_ptr->cmd.mbms_act_rej_resp.as_id; 
          break;

        case MSM_SM_ABORT_REQ:
          as_id = cmd_ptr->cmd.mbms_abort_req.as_id; 
          break;
#endif
        ...
        case SMREG_RESTORE_CTXT_PSM_RSP:
          as_id = cmd_ptr->cmd.restore_psm_ctxt_rsp.as_id; 
          break;

        case SMREG_RESTORE_PDP_CTXT_REQ:
          as_id = cmd_ptr->cmd.restore_pdp_ctxt_req.as_id; 
          break;
...
DESCRIPTION
  This function reads SM related PSM context information from EFS
...
SIDE EFFECTS
  None
===========================================================================*/
void sm_psm_read_cache(sm_psm_cache_s_type *psm_cache_ptr)
{
  sm_psm_cache_ptr = (sm_psm_cache_s_type *) modem_mem_calloc(1, sizeof(sm_psm_cache_s_type), MODEM_MEM_CLIENT_NAS);
  sm_check_for_null_ptr((void*)sm_psm_cache_ptr);
  memset((void*)sm_psm_cache_ptr, 0, sizeof(sm_psm_cache_s_type));

  memscpy(sm_psm_cache_ptr, sizeof(sm_psm_cache_s_type),
                       psm_cache_ptr, sizeof(sm_psm_cache_s_type));
...
DESCRIPTION
  This function saves the SM related PSM context in cache in EFS
...
SIDE EFFECTS
  None
===========================================================================*/
void sm_psm_write_cache(sm_psm_cache_s_type *psm_cache_ptr)
{
  ...
  {
    memscpy(psm_cache_ptr, sizeof(sm_psm_cache_s_type),
                        sm_psm_cache_ptr, sizeof(sm_psm_cache_s_type));

#ifdef FEATURE_MODEM_HEAP
    modem_mem_free( sm_psm_cache_ptr , MODEM_MEM_CLIENT_NAS);
    sm_psm_cache_ptr = NULL;
#else
    gs_free( sm_psm_cache_ptr);
    sm_psm_cache_ptr = NULL;
#endif
...
DESCRIPTION
 This function saves PSM related PDP context NSAPI , Radio Priority , Connection ID  in EFS upon entering PSM mode.
...
SIDE EFFECTS
 None
/*============================================================================*/
boolean sm_save_psm_context(void)
{
  boolean result = TRUE;
  byte index = 0;
  byte total_active_pdp = 0;

#ifndef FEATURE_MODEM_HEAP
  sm_psm_cache_ptr = (sm_psm_cache_s_type *) gs_alloc(sizeof(sm_psm_cache_s_type));
#else
  sm_psm_cache_ptr = (sm_psm_cache_s_type *) modem_mem_calloc(1, sizeof(sm_psm_cache_s_type), MODEM_MEM_CLIENT_NAS);
#endif
  sm_check_for_null_ptr((void*)sm_psm_cache_ptr);
  ...
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if (pdp_list[index] != NULL && pdp_list[index]->pdp_state == PDP_ACTIVE)
    {
      sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi = pdp_list[index]->nsapi.nsapi;
      sm_psm_cache_ptr->gsm.psm_pdp[index].radio_pri = pdp_list[index]->radio_pri;
      sm_psm_cache_ptr->gsm.psm_pdp[index].packet_flow_id = pdp_list[index]->flow_id;
      sm_psm_cache_ptr->gsm.psm_pdp[index].connection_id  = pdp_list[index]->trans_id;
      total_active_pdp++;
       MSG_HIGH_DS_1(SM_SUB,"=SM= total_active_pdp  = %d", total_active_pdp);
    }
  }
  
  if (total_active_pdp == 0)
  {
    sm_psm_cache_ptr->gsm.psm_pdp[0].nsapi = 0;
  }

  /* Indicates SNDCP to store the last XID context negotiated with network in EFS upon PSM enter mode command. */
  if (result == TRUE)
  {
    sm_send_sndcp_enter_psm_ind();
  }
...
DESCRIPTION
 This function restores all PDP context NSAPI and related parameters stored in EFS
 upon wake up from PSM that existed in the previous GPRS session before
 entering PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
boolean sm_restore_psm_context(void)
{
  boolean result = TRUE;
  byte index = 0;
  boolean valid_pdp_found = FALSE;
  ...
  if(sm_psm_cache_ptr != NULL)
  {
    ...
    for(index = 0; index < MAX_PDP_NUM; index++)
    {
      if((sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi >= MIN_NSAPI_NUM) && 
         (sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi <= MAX_NSAPI_NUM))
      {
        valid_pdp_found = TRUE;
        MSG_HIGH_DS_1(SM_SUB,"=SM= Valid PDP Found  = %d", valid_pdp_found);
      }
      else if (sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi != 0)
      {
        MSG_HIGH_DS_1(SM_SUB,"=SM= Valid PDP Not Found  = %d", valid_pdp_found);
        valid_pdp_found = FALSE;
        break;
      }
    }
  }
  
  result = (result && valid_pdp_found);

  if(sm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_1(sm_get_ulog_handle(), "=SM= Valid PDP Found = %d", valid_pdp_found);
  }
  MSG_HIGH_DS_1(SM_SUB,"=SM= Valid PDP Found  = %d", valid_pdp_found);
...
DESCRIPTION
 This function is used to send a request to DS via CM to restore all PDP
 upon wake up from PSM that existed in the previous GPRS session before
 entering PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
void sm_send_cm_restore_ctxt_psm_req (void)
{
  ...
  if (ds_rpt != NULL)
  {
    ds_rpt->cmd.mm_restore_psm_ctxt_req.message_header.message_set = MS_MM_DS;
    ds_rpt->cmd.mm_restore_psm_ctxt_req.message_header.message_id = (int) MM_RESTORE_CTXT_PSM_REQ;
    ds_rpt->cmd.mm_restore_psm_ctxt_req.sys_mode =  SYS_SYS_MODE_GSM;
    PUT_IMH_LEN( sizeof(mm_restore_psm_ctxt_req_T) - sizeof(IMH_T),
                  &ds_rpt->cmd.mm_restore_psm_ctxt_req.message_header );
#ifdef FEATURE_DUAL_SIM
    ds_rpt->cmd.restore_psm_ctxt_req.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
    ...
    sm_add_message_to_debug_buffer(MS_MM_DS,
    ...
    if (sm_get_ulog_handle() != NULL)
    {
      ULOG_RT_PRINTF_0(sm_get_ulog_handle(), "=SM= Sent CM_RESTORE_CTXT_PSM_REQ");
    }
  
    MSG_HIGH_DS_0(SM_SUB, "=SM= Sent CM_RESTORE_CTXT_PSM_REQ to CM");
    
    //Send to DS
    ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.mm_restore_psm_ctxt_req);
...
DESCRIPTION
  This function processes SMREG_RESTORE_CTXT_PSM_RSP from CM after PDP context is restored 
  at DS upon exit from PSM mode.
...
void sm_cm_process_restore_ctxt_psm_rsp (mm_restore_psm_ctxt_rsp_T *restore_psm_ctxt_rsp)
{
...
#ifdef FEATURE_DUAL_SIM
#ifndef FEATURE_DUAL_DATA
     if(SM_IS_VALID_AS_ID(restore_psm_ctxt_rsp->as_id) == FALSE)
     {
        MSG_HIGH_DS_2( SM_SUB,"SM AS ID Conflict sm_as_id : %d, message as_id: %d", sm_as_id, restore_psm_ctxt_rsp->as_id);
        return ;
     }
#endif
#endif
     ...
     if(sm_get_ulog_handle() != NULL)
     {
        ULOG_RT_PRINTF_1(sm_get_ulog_handle(), "=SM= received SMREG_RESTORE_CTXT_PSM_RSP status = %d", restore_psm_ctxt_rsp->status);
     }

     MSG_HIGH_DS_1(SM_SUB, "=SM= received SMREG_RESTORE_CTXT_PSM_RSP status = %d", restore_psm_ctxt_rsp->status);

#ifdef FEATURE_GSM_GPRS
     if (restore_psm_ctxt_rsp->status)
     {
       for (index = 0; index < MAX_PDP_NUM; index++)
       {
             ...
             (cur_pdp->pdp_state == PDP_ACTIVE))
         {
             /* ------------------------------------------------------------------
                    ...
                    ** -----------------------------------------------------------------*/
           if (cur_pdp->llc_sapi.valid && cur_pdp->radio_pri.valid )
           {
            
             if (sm_get_ulog_handle() != NULL)
             {
               ULOG_RT_PRINTF_2(sm_get_ulog_handle(), "=SM= sent GSNSM_MODIFY_IND NSAPI = %d PDP = %d ", cur_pdp->nsapi.nsapi, cur_pdp->pdp_state);
             }
             cur_pdp->llc_state = LLC_PSM_MODIFY_CNF_PENDING;
     ...
     sm_send_gmm_restore_ctxt_psm_cnf(restore_psm_ctxt_rsp->status);
...
DEPENDENCIES
  None
===========================================================================*/
void sm_mmgsdi_in_traffic_channel_mode
(
  boolean               in_traffic
)
{

  uint16 mmgsdi_call_mask = 0;
  ...
  {
    MSG_ERROR_DS(MM_SUB,"Unable to toggle UIM power control sync",0,0,0) ;
    return;
  }

  MSG_HIGH_DS_1(SM_SUB,"Informing traffic state to MMGSDI, in_traffic: %d", in_traffic);
