...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_bpm.c#1 $
...
(
  const emm_pdn_connectivity_req_s_type* data_ptr,  /*!< REQUIRED short parameter description */
...
{  
  ...
  for(; ((index != mgr_ptr->cur_pti_index)&&(index < 255)); index++)
  {
    if(index == 0)
    {
...
)
{
  ...
  for(index = 0; index < ESM_MAX_BP_PROCS ; index++)
  {
    ...
    if (mgr_ptr->esm_bp[index].inuse == FALSE)
    {
      ...
      nas_assert_check((boolean)(mgr_ptr->esm_bp[index].pti != 0));
...
)
{
  ...
  esm_free_epco_payload(&proc_ptr->pending_msg_ptr->esm_out_msg);
...
/*!
    @brief
    During UICC validation, MMCM_CALL_CTRL_RSP is received from 
...
)
{
  ...
  nas_assert_check((boolean)(req_ptr->esm_hdr.lbi >= MIN_NSAPI_NUM));
...
#else
    MSG_HIGH_DS_1(SM_SUB, "ESM: Invalid BEARER RESRC ALLOC REQ - BID %d",
                  req_ptr->esm_hdr.lbi);
#endif
    return FALSE;
  }
...
/*==============================================================================
  ...
  FUNCTION:  esm_handle_dl_retx_msg

==============================================================================*/
/*!
    @brief
    This function checks if ESM needs to ignore coming in ESM OTA message in case
    a DL message from NW is a retrasmission of a request of UE has already 
    accepted

    @detail
*/
/*============================================================================*/
boolean  esm_handle_dl_retx_msg
(
  lte_nas_esm_incoming_msg_type    *esm_incoming_msg_ptr, 
  esm_bpm_T                        *mgr_ptr,
  byte                             pti,
...
{
   esm_bearer_context_info_shared_T  bc_info_shared;
   lte_nas_message_id msg_id = esm_incoming_msg_ptr->nas_hdr.msg_id;

   MSG_ERROR_DS_1(SM_SUB, "ESM: Unknown or unassigned PTI %d received by ESM ", pti);
        
   mgr_ptr->find_bearer_context_ptr(bearer_id, &bc_info_shared);

   if ( (pti == ESM_INVALID_PTI) &&
        (bc_info_shared.state != ESM_BC_ACTIVE) )
   {
      return FALSE;
   }
   else
   {
      if(esm_retx_ul_msg(msg_id, pti, bc_info_shared) == TRUE)
      {
        esm_bpm_build_and_send_autonomous_accept(msg_id, bearer_id);
      }
      else
      {
        esm_bpm_build_and_send_autonomous_reject(esm_incoming_msg_ptr,NAS_ESM_PTI_MISMATCH);
      }
...
/*==============================================================================
  ...
  FUNCTION:  esm_bpm_chk_esm_data_transport_req_allowance

==============================================================================*/
/*!
    @brief
    This function checks if data tranport is posssible

    @detail
*/
/*============================================================================*/
boolean   esm_bpm_chk_esm_data_transport_req_allowance
(
  emm_esm_data_transport_req_type*        msg_ptr,
  esm_bpm_T                        *mgr_ptr
)
{
  int index = 0;
  ...
  /*24.301 6.1.2
      During procedures related to EPS bearer contexts, the MME and the UE shall not initiate 
      the transport of user data via the control plane procedure until the ongoing procedure is completed.
      Except for the remote UE report procedure and ESM information request procedure, during transaction 
      related procedures, the MME and the UE shall not initiate the transport of user data via the control 
      plane procedure until the ongoing procedure is completed.
  */
  for(index=0; index < ESM_MAX_BP_PROCS; index++)
  {
    if(mgr_ptr->esm_bp[index].inuse == TRUE)
    {
      ...
      esm_send_cm_mo_esm_data_transport_status_ind(&emm_failure_cause, FALSE, msg_ptr->seq_num);
      free_dsm_esm_data_transport_req(msg_ptr);
      return FALSE;
    }
  }

  return TRUE;
}/* esm_bpm_chk_esm_data_transport_req_allowance */
#endif
/*==============================================================================
...
)
{
  ...
  sm_check_for_null_ptr((void *)cmd_ptr);
  sm_check_for_null_ptr((void *)mgr_ptr);
  ...
  switch (cmd_ptr->cmd.hdr.message_id)
  {
    case MMCM_CALL_CTRL_RSP:
      MSG_HIGH_DS_0(SM_SUB, "ESM: MSG_ID == MMCM_CALL_CTRL_RSP");
      proc_ptr = esm_bpm_find_uicc_proc_state_machine(mgr_ptr,
                                                      cmd_ptr->cmd.cm_mm_call_ctrl_rsp.ps_params.lte_ps_params.pti);
      ...
      MSG_HIGH_DS_1(SM_SUB, "ESM: PACKED_CNF status = %d",
                    cmd_ptr->cmd.cm_mm_call_ctrl_rsp.call_control_result);
      ...
      {            
        ...
        proc_ptr->esm_proc_state_ptr->esm_proc_process_pdn_connect_req(proc_ptr,&(cmd_ptr->cmd.emm_pdn_connectivity_req));     
        MSG_HIGH_DS_0(SM_SUB, "ESM: releasing the resources");
        esm_bpm_release_proc_resources(mgr_ptr, proc_ptr);           
        return result;
      }
      switch(cmd_ptr->cmd.cm_mm_call_ctrl_rsp.call_control_result)
      {
        ...
        case ESM_MSG_MODIFIED:
          ...
          msg_len = (byte)cmd_ptr->cmd.cm_mm_call_ctrl_rsp.ps_params.lte_ps_params.length;
          ...
          memscpy((void*)&msg_pkt[0],sizeof(msg_pkt),
                  (void*)&cmd_ptr->cmd.cm_mm_call_ctrl_rsp.ps_params.lte_ps_params.pkd_msg,
                  msg_len);
          /*Do reset and then give this structure to decode function, otherwise there are chances of junk values being present*/
          memset(&(cmd_ptr->cmd.emm_pdn_connectivity_req),0,sizeof(emm_pdn_connectivity_req_s_type));
          /*Decode the encoded message coming from CM*/
          decode_pdn_conn_req(&msg_pkt[0],/**/
                               msg_len,
                              &(cmd_ptr->cmd.emm_pdn_connectivity_req) /*Message structure*/);       
          
#ifdef FEATURE_LTE_REL10
          /* Send pdn connectivity reject ind to CM when APN is blocked */
          if(cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.apn_name.valid == TRUE)
          {
            if(ds_3gpp_is_apn_blocked(cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.apn_name) == TRUE)
            {
          ...
          {
            cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.pco_ciphering_flag
              = proc_ptr->pco_ciphering_flag;
          }

          cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.protocol_config_options =
             proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.protocol_conf_opt;

          cmd_ptr->cmd.emm_pdn_connectivity_req.esm_hdr.sdf_id = proc_ptr->sdf_id;
          cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.seq_num = 
          ...
          {
            cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.low_priority_signaling = 
              proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.low_priority_indicator;

          }
#endif
          cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.hc_config =
             proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.hc_config;

          cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.ext_protocol_conf_opt =
             proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.ext_protocol_conf_opt;
          cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.mo_exception_data = 
		  	 proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.mo_exception_data;
        ...
        default:
          MSG_FATAL_DS(SM_SUB, "ESM: Invalid PDN CONN PACKED CNF %d", 
                    cmd_ptr->cmd.cm_mm_call_ctrl_rsp.call_control_result,0,0);
          break;
      }
      break;
    case EMM_ESM_DATA_IND:
      event.esm_in_msg = (dword)EMM_ESM_DATA_IND;
      event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,
      ...
      if(result != TRUE)
      {
        ...
        esm_free_dl_msg_epco_dsm(&cmd_ptr->cmd.esm_data_ind.esm_incoming_msg);
      ...
      if (esm_hdr->pti == ESM_INVALID_PTI)
      {
        ...
        else if ((ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_REQUEST == cmd_ptr->cmd.esm_data_ind.esm_incoming_msg.nas_hdr.msg_id) &&
                 (esm_handle_dl_retx_msg(&cmd_ptr->cmd.esm_data_ind.esm_incoming_msg,
                                               mgr_ptr,
                                               esm_hdr->pti, 
                                               esm_hdr->bearer_id) == TRUE))
        {
          /* Return false so that BCM ignores it */
          return FALSE;
        }
      ...
      if(index == ESM_MAX_BP_PROCS)
      {
        ...
        (void)esm_handle_dl_retx_msg(&cmd_ptr->cmd.esm_data_ind.esm_incoming_msg,
                                            mgr_ptr,
                                            esm_hdr->pti, esm_hdr->bearer_id);
        esm_free_dl_msg_epco_dsm(&cmd_ptr->cmd.esm_data_ind.esm_incoming_msg);
        result = FALSE;
      }
      break;
    case EMM_ESM_FAILURE_IND:
      event.esm_in_msg = (dword)EMM_ESM_FAILURE_IND;
      event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,sizeof(event), (void*)&event);

      switch(esm_get_module_from_trans_id(cmd_ptr->cmd.esm_failure_ind.trans_id))
      {
#ifdef FEATURE_CIOT
        case LTE_ESM_EDT:
          esm_send_cm_mo_esm_data_transport_status_ind(&cmd_ptr->cmd.esm_failure_ind.emm_failure_cause,
                                                       FALSE, 
                                                       (uint8)(cmd_ptr->cmd.esm_failure_ind.trans_id & 0xFF));
          break;
#endif
        case LTE_ESM_BPM:
          for (index = 0; index < ESM_MAX_BP_PROCS; index++)
    ...
    case EMM_ESM_SIG_CONNECTION_RELEASED_IND:
      event.esm_in_msg = (dword)EMM_ESM_SIG_CONNECTION_RELEASED_IND;
      event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,sizeof(event), (void*)&event);
      /* Ignore this in BPM for now and let any timers run. */ 
      break;
    case EMM_ESM_ACTIVE_EPS_IND:
      event.esm_in_msg = (dword)EMM_ESM_ACTIVE_EPS_IND;
      event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,sizeof(event), (void*)&event);
      break;
#ifdef FEATURE_CIOT
  case EMM_ESM_DATA_TRANSPORT_REQ:
      event.esm_in_msg = (byte)EMM_ESM_DATA_TRANSPORT_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
      if( esm_bpm_chk_esm_data_transport_req_allowance(&(cmd_ptr->cmd.esm_data_transport_req), mgr_ptr) == TRUE)
      {
        esm_process_data_transport_req(&(cmd_ptr->cmd.esm_data_transport_req));
        return result;
      }
      break;
#endif
    ...
    case NAS_ESM_BEARER_RESOURCE_ALLOC_REQ:
      event.esm_in_msg = (dword)NAS_ESM_BEARER_RESOURCE_ALLOC_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
    ...
    case EMM_PDN_CONNECTIVITY_REQ:
      ...
      if(esm_chk_pdn_conn_allowance(&cmd_ptr->cmd.emm_pdn_connectivity_req, 
                                     mgr_ptr) == FALSE)
      {
        esm_build_and_send_pdn_connectivity_reject_ind(
          NULL, cmd_ptr->cmd.emm_pdn_connectivity_req.esm_hdr.sdf_id,
          NULL, ESM_LOCAL_CUASE_NONE, NULL);
        return TRUE;      
      }
#endif
#endif
      event.esm_in_msg = (dword)EMM_PDN_CONNECTIVITY_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
      proc_ptr = esm_get_free_proc_instance_and_pti(mgr_ptr);
      sm_check_for_null_ptr((void *)proc_ptr);

      MSG_HIGH_DS_4(SM_SUB, "ESM: Rcved PDN CONN REQ - Seq num %d, Req type %d  exp data %d esm_dummy_flag %d",
                    cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.seq_num,
                    cmd_ptr->cmd.emm_pdn_connectivity_req.request_type,
                    cmd_ptr->cmd.emm_pdn_connectivity_req.pdn_data.mo_exception_data,
                    cmd_ptr->cmd.emm_pdn_connectivity_req.esm_dummy_flag);
      //esm_get_epco_dsm_attachment(cmd_ptr);
#ifdef FEATURE_LTE_UICC_CALL_CONTROL
     if(cmd_ptr->cmd.emm_pdn_connectivity_req.esm_dummy_flag == FALSE)
     {
      if(cmd_ptr->cmd.emm_pdn_connectivity_req.request_type == ESM_EMERGENCY)
      {
#ifdef FEATURE_NAS_E911_ENABLE
        /* Skip call cotrol for emergency PDN CONN REQ */
        proc_ptr->esm_proc_state_ptr->esm_proc_process_pdn_connect_req
                           (proc_ptr,&(cmd_ptr->cmd.emm_pdn_connectivity_req));
#else
      /* E911 not enable on this build,upper layer should not send EMC call, but for some reason they have sent out
          ESM should reject it.*/
       MSG_ERROR_DS_0(SM_SUB, "ESM: EMC call not supported on this build ");
       esm_build_and_send_pdn_connectivity_reject_ind(
          NULL, cmd_ptr->cmd.emm_pdn_connectivity_req.esm_hdr.sdf_id,
          NULL, ESM_LOCAL_CUASE_NONE, NULL);
        return TRUE; 
    ...
    case EMM_PDN_DISCONNECT_REQ:
      event.esm_in_msg = (dword)EMM_PDN_DISCONNECT_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
      ...
      bearer_id = cmd_ptr->cmd.emm_disconnect_req.eps_bearer_id;
      //esm_get_epco_dsm_attachment(cmd_ptr);
      ...
      {
        if( (cmd_ptr->cmd.emm_disconnect_req.esm_local_cause.valid == FALSE) || 
            (cmd_ptr->cmd.emm_disconnect_req.esm_local_cause.local_cause != 
                                              ...
                                              ESM_LOCAL_DEACT))
        {
          ...
          if(mgr_ptr->check_detach_trigger(bearer_id))
          {
            ...
            if ((cmd_ptr->cmd.emm_disconnect_req.ext_protocol_conf_opt.valid)&&
                (cmd_ptr->cmd.emm_disconnect_req.ext_protocol_conf_opt.dsm_ptr != NULL))
            {
              dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_disconnect_req.ext_protocol_conf_opt.dsm_ptr));
            }
          ...
          else
          {
            proc_ptr = esm_get_free_proc_instance_and_pti(mgr_ptr);
            sm_check_for_null_ptr((void *)proc_ptr);
            proc_ptr->esm_proc_state_ptr->esm_proc_process_pdn_disconnec_req(proc_ptr,&(cmd_ptr->cmd.emm_disconnect_req) );
          }
        }
        else
        {
          proc_ptr = esm_get_free_proc_instance_and_pti(mgr_ptr);
          sm_check_for_null_ptr((void *)proc_ptr);
          local_cause = cmd_ptr->cmd.emm_disconnect_req.esm_local_cause;
          MSG_HIGH_DS_0(SM_SUB, "ESM: DS Bearer contexts are transfered due to IRAT/Local deact, locally deactivating the contexts");
    ...
    case NAS_ESM_BEARER_RESOURCE_ALLOC_ABORT_REQ:
      event.esm_in_msg = (dword)NAS_ESM_BEARER_RESOURCE_ALLOC_ABORT_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
      ...
      if (proc_ptr != NULL)
      {
        ...
        if (emm_chk_nb1_rat() == TRUE)
        {
		  ...
		  local_cause.local_cause = ESM_LOCAL_CUASE_NONE;
          proc_ptr->esm_proc_state_ptr->esm_proc_clean_up(
                                         proc_ptr, local_cause);
		  esm_bpm_release_proc_resources(mgr_ptr, proc_ptr);
		}
		else
#endif
    ...
    case EMM_PDN_CONNECTIVITY_ABORT_REQ:
      event.esm_in_msg = (dword)EMM_PDN_CONNECTIVITY_ABORT_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG,sizeof(event), (void*)&event);
      ...
      proc_ptr = esm_bpm_find_proc_state_machine(mgr_ptr,cmd_ptr->cmd.emm_pdn_connectivity_abort_req.esm_hdr.sdf_id,
      ...
      {
        MSG_ERROR_DS_1(SM_SUB, "ESM: UNKNOWN Bearer Resource Request is asked to be aborted with SDF_ID=%d",
                       cmd_ptr->cmd.emm_pdn_connectivity_abort_req.esm_hdr.sdf_id);
        esm_build_and_send_pdn_connectivity_abort_rsp(cmd_ptr->cmd.emm_pdn_connectivity_abort_req.esm_hdr.sdf_id,
                                                      FALSE);
      }

      break;
    case EMM_ESM_GET_PDN_CONNECTIVITY_REQ_IND:
      event.esm_in_msg = (dword)EMM_ESM_GET_PDN_CONNECTIVITY_REQ_IND;
      event_report_payload(EVENT_LTE_ESM_INCOMING_MSG,sizeof(event), (void*)&event);
    ...
    case EMM_BEARER_RESRC_MOD_REQ:
      event.esm_in_msg = (dword)EMM_BEARER_RESRC_MOD_REQ;
      event_report_payload(EVENT_LTE_CM_INCOMING_MSG, sizeof(event), 
                           (void*)&event);
     // esm_get_epco_dsm_attachment(cmd_ptr);
      if(esm_validate_bearer_resrc_mod_req_bearer_id(
                 cmd_ptr->cmd.emm_bearer_resource_modification_req.esm_hdr.lbi,
                 mgr_ptr) == FALSE)
      {
        if (cmd_ptr->cmd.emm_bearer_resource_modification_req.ext_protocol_conf_opt.valid)
        {
          dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_bearer_resource_modification_req.ext_protocol_conf_opt.dsm_ptr));
          cmd_ptr->cmd.emm_bearer_resource_modification_req.ext_protocol_conf_opt.dsm_ptr = NULL;
        }
        esm_build_and_send_cm_bearer_resrc_mod_reject(
              cmd_ptr->cmd.emm_bearer_resource_modification_req.esm_hdr.sdf_id,
              NULL,
              LTE_NAS_ESM_UNKNOWN_EPS_BEARER_CONTEXT,
              cmd_ptr->cmd.emm_bearer_resource_modification_req.esm_hdr.lbi);
        return result;
      ...
      proc_ptr->esm_proc_state_ptr->esm_proc_process_bearer_resrc_mod_req(proc_ptr,
                             &(cmd_ptr->cmd.emm_bearer_resource_modification_req));
      esm_proc_state_info_log.instance_id = proc_ptr->instance_id;  
      esm_proc_state_info_log.proc_state = (byte)proc_ptr->esm_proc_state_ptr->state();
      esm_proc_state_info_log.pti = (byte)proc_ptr->pti;
      esm_proc_state_info_log.sdf_id = (byte)proc_ptr->sdf_id;
    ...
    case EMM_ESM_DATA_CNF:
      esm_send_cm_mo_esm_data_transport_status_ind(NULL, TRUE,
                                                   (uint8)(cmd_ptr->cmd.esm_failure_ind.trans_id & 0xFF));
      return FALSE;/* Return false so that BCM ignores it */
    case EMM_ESM_ATTACH_STATUS_IND:
      MSG_HIGH_DS_1(SM_SUB, "ESM: attach without PDN value from EMM received = %d", cmd_ptr->cmd.esm_attach_ind.attach_without_pdn);
      if(cmd_ptr->cmd.esm_attach_ind.attach_without_pdn == TRUE)
      {
         esm_set_attach_without_pdn(ESM_BC_ON_ATTACH_ATTACH_WITHOUT_PDN);
      }
    
     return FALSE;
#endif

    case EMM_ESM_UNBLOCK_ALL_APNS_IND:
...
)
{
  ...
  nas_assert_check((boolean)(instance_id < ESM_MAX_BP_PROCS));
  ...
  event.esm_timer_event = (byte)(cmd_ptr->cmd.esm_timer_expired_ind.timer_id);
