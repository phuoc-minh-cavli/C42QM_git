...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smtask.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
static ULogHandle sm_ulog_handle = NULL;

/* ----------------- ----------------------
  ** Variables needed for LTE and GW NAS 
**----------------------------------------*/
...
volatile sm_esm_debug_buffer_s_type sm_esm_debug_buffer[SM_DEBUG_MESSAGE_BUFFER_SIZE];
dword sm_esm_debug_buffer_index = 0;
/*-------------------------------*/

#ifdef FEATURE_DUAL_DATA
sm_psm_cache_s_type *sm_psm_cache_ptr_sim[MAX_AS_IDS];
#define sm_psm_cache_ptr sm_psm_cache_ptr_sim[sm_as_id]
#else
sm_psm_cache_s_type *sm_psm_cache_ptr;
#endif
...
boolean sm_pdp_sig_in_progress_last_sent_sim[MAX_AS_IDS];
...
extern boolean rrcudt_search_nas_msg_from_udt_q
(
  byte pd_ti,
  byte nas_msg_type,
...
boolean sm_pdp_sig_in_progress_last_sent;
...
void sm_proc_timer_expired(word con_id, sm_timer_T sm_timer_type)
{
  ...
  mm_pdp_deactivate_req_T *pdp_deactivate = NULL;
  ...
  ** ------------------------------------------------------------*/
  if (sm_timer_type == TIMER_ESTABLISH_PENDING)
  {
    ...
    if (max_est_timer_count < MAX_EST_TIMER_CNT)
    {
      ...
      {
        #ifdef FEATURE_MBMS
        if (pdp_list[con_id]->is_mbms && (pdp_list[con_id]->pdp_state == PDP_INACTIVE || 
           pdp_list[con_id]->pdp_state == MBMS_ACTIVE_PENDING))
        {
          // Send the MBMS activation request again 
          MSG_MED_DS_1(SM_SUB, "MBMS: GMMSM ESTABLISH timer expired, cnt %d",max_est_timer_count);
          if (pdp_list[con_id]->cm_pending_msg == (int)MSM_SM_ACTIVATE_REJ_RSP)
          {
            sm_continue_mbms_activate_rej_rsp(pdp_list[con_id]);
          }
          else
          {
            sm_continue_mbms_activation(pdp_list[con_id]);
          }
        }
        else
        #endif
    ...
    else
    {
      ...
      for (i = 0; i < MAX_PDP_NUM; i++)
      {
        if (pdp_list[i] != NULL && pdp_list[i]->pdp_con_state == SIG_CONNECTION_PENDING)
        {
          ...
          #ifdef FEATURE_MBMS
          if (pdp_list[i]->is_mbms && (pdp_list[i]->pdp_state == PDP_INACTIVE || 
              pdp_list[i]->pdp_state == MBMS_ACTIVE_PENDING))
          {
            MSG_HIGH_DS_2(SM_SUB, "MBMS: MAX COUNT expired TI=%d, State=%d ",i,pdp_list[i]->pdp_state); 
            if (pdp_list[i]->cm_pending_msg == (int)MSM_SM_ACTIVATE_REJ_RSP)
            {
            ...
            else
            {
              ...
              pdp_list[i]->current_cause = (cc_cause_T)PDP_ACTIVATE_MAX_TIMEOUT; //MBMS_PENDING
          ...
          else if(pdp_list[i]->pdp_state == PDP_MODIFY_PENDING)
          {
            ...
            if (is_umts_mode())
            {
              ...
              else if (pdp_list[con_id]->cm_pending_msg == (int)MS_MODIFY_PDP_ACCEPT ||
                         pdp_list[con_id]->cm_pending_msg == (int)MODIFY_PDP_REJECT)
              {
                MSG_MED_DS_1(SM_SUB,"Can not send NW Modify Rej/Cnf ConId=%d", pdp_list[con_id]->trans_id);
              }
              else
              {
  ...
  ** ------------------------------------------------------------*/
  else if (sm_timer_type == TIMER_ACTIVE_PENDING)
  {
    ...
    if (sm_max_timer_count[con_id] < MAX_TIMER_CNT)
    {
      ...
      {
        #ifdef FEATURE_MBMS
        if (pdp_list[con_id]->is_mbms)
        {
          // Send the MBMS activation request again 
          MSG_HIGH_DS_1(SM_SUB, "MBMS: ACTIVATE timer expired, cnt %d",sm_max_timer_count[con_id]);
          sm_continue_mbms_activation(pdp_list[con_id]);
        }
        else
        {
    ...
    else
    {
      if (pdp_list[con_id] != NULL)
      {
        #ifdef FEATURE_MBMS
        if (pdp_list[con_id]->is_mbms)
          MSG_HIGH_DS_0(SM_SUB, "MBMS: ACTIVATE timer MAX COUNT expired");    
        else
        #endif
  ...
  else if (sm_timer_type == TIMER_INACTIVE_PENDING)
  {
    ...
    if (sm_max_timer_count[con_id] < MAX_TIMER_CNT)
    {
      ...
      if (pdp_list[con_id] != NULL)
      {
        ...
        if (!rrcudt_search_nas_msg_from_udt_q((byte)((con_id<<4)|SM_PD),(byte)DEACTIVATE_PDP_REQUEST,RRC_PS_DOMAIN_CN_ID))  // CRITICAL_MBMS_PENDING
  #endif
        sm_continue_pdp_deactivate(pdp_list[con_id]);
  #ifdef FEATURE_NAS_UDT_OOS
        else
          sm_start_timer (pdp_list[con_id]->trans_id, TIMER_INACTIVE_PENDING);   // restart the timer
  #endif
        }
...
#ifdef FEATURE_MBMS
        if(pdp_list[con_id]->is_mbms == FALSE)
        {
          sm_deact_associated_mbms_ctxt(pdp_list[con_id]);
        }
#endif
        sm_continue_pdp_deactivate(pdp_list[con_id]);
        pdp_list[con_id]->current_cause = (cc_cause_T)RAB_SETUP_FAILURE;
...
#ifdef FEATURE_MBMS
  if( connection_id != INVALID_ID && pdp_list[connection_id] != NULL && 
       pdp_list[connection_id]->is_mbms )
  {
    for (i=MIN_MBMS_NSAPI_NUM; i<MAX_NSAPI_NUM; i++)
    {
...
#ifdef FEATURE_MBMS
  if( connection_id != INVALID_ID && pdp_list_sim[as_id][connection_id] != NULL && 
       pdp_list_sim[as_id][connection_id]->is_mbms )
  {
    for (i=MIN_MBMS_NSAPI_NUM; i<MAX_NSAPI_NUM; i++)
    {
...
/*===========================================================================
FUNCTION SM_RESERVE_PSM_RESTORED_NSAPI
...
SIDE EFFECTS
  None
===========================================================================*/
void sm_reserve_psm_restored_nsapi
(
  byte    connection_id,
  byte    nsapi,
  sys_modem_as_id_e_type as_id
)
{
  ...
  nsapi_list_sim[as_id][nsapi] = connection_id;
  
#else
  nsapi_list[nsapi] = connection_id;
#endif
  rex_leave_crit_sect(&sm_crit_sect);
}
...
void sm_init_pdp_context (pdp_context_info *pdp)
{
  ...
  pdp->mbms_bear_cap.valid = FALSE;
  pdp->tmgi.valid = FALSE;
  #endif
#ifdef FEATURE_DUAL_SIM
...
gs_status_T sm_send_routine( void *msg_p )
{
    ...
    if((cmd_ptr = sm_get_cmd_buf()) != NULL)
    {
       ...
       // sm_put_cmd((sm_cmd_type*) cmd_ptr); // MDOUBT
       return GS_SUCCESS;
    }
...
void sm_init( void )
{
 word i;
  ...
  rex_def_timer( &gmmsm_establish_timer, rex_self(), (rex_sigs_type)TIMER_ESTABLISH_PENDING );
  ...
  for(i=0;i<SM_DEBUG_MESSAGE_BUFFER_SIZE;i++)
  {
  sm_esm_debug_buffer[i].debug_buffer_type.sm_info_debug_buffer.message_set=0;
  sm_esm_debug_buffer[i].debug_buffer_type.sm_info_debug_buffer.message_id=0;
  #ifdef FEATURE_LTE
  sm_esm_debug_buffer[i].debug_buffer_type.esm_info_debug_buffer.msg_umid=0;
  #endif
  sm_esm_debug_buffer[i].sm_esm_message_direction = TO_SM;
  sm_esm_debug_buffer[i].as_id =0;
  }
...
/*===========================================================================
FUNCTION sm_get_ulog_handle

DESCRIPTION
  Returns ULOG Handle for SM task
...
SIDE EFFECTS
  None
===========================================================================*/
ULogHandle sm_get_ulog_handle()
{
  return sm_ulog_handle;
}

/*===========================================================================
FUNCTION sm_ulog_register

DESCRIPTION
  This function call registers SM Task with ULOG
...
SIDE EFFECTS
  None
===========================================================================*/
void sm_ulog_register(void)
{
  ULogResult result = ULogFront_RealTimeInit(&sm_ulog_handle,
                               "NAS_SM_ULOG",  
                               512, 
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  MSG_HIGH_DS_1(SM_SUB,"=SM= ULOG REGISTER RESULT: %d", result);
}
...
/*===========================================================================
	...
	DESCRIPTION
	  ...
	  This function returns a DS command buffer to the calling function.
	  This function dynamically allocates memory for the necessary buffer.
	  Note that this buffer is freed by calling ds_free_cmd_buf(). 
	...
	RETURN VALUE
	  ...
	  A buffer of type ds_emm_cmd_type. NULL is returned if memory is unavailable.
...
ds_emm_cmd_type *ds_get_cmd_buf( void )
{
  ds_emm_cmd_type *ptr;

  /* Allocating memory for DS Command buffer from the TMC heap */
  if ((ptr = (ds_emm_cmd_type *)modem_mem_calloc(1, sizeof(ds_emm_cmd_type), MODEM_MEM_CLIENT_NAS)) == NULL)
  {
    MSG_ERROR_DS_0(SM_SUB,"Could not allocate memory for DS cmd q!");
  }
  return(ptr);
}
