...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_context_active.c#1 $
...
)
{
  ...
  if((ctxt_ptr->default_bearer_ctxt == TRUE)&&(ctxt_ptr->check_detach_trigger(bearer_id)))
  {
    MSG_HIGH_DS_0(SM_SUB, "ESM: default bearer is deactivated of last PDN, Need to detach.");
  ...
  esm_build_and_send_deactivate_bearer_context_accept(bearer_id,NULL,&ctxt_ptr->epco);
