...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_proc_active.c#1 $
  ...
  switch(data_ptr->nas_hdr.msg_id)
  {
...
#ifdef FEATURE_CIOT
          if(proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.esm_dummy_flag == TRUE)
          { 
           esm_build_and_send_esm_dummy_accept(proc_ptr);
	       result = FALSE;
	       send_reject_ind = FALSE;
	       return result;
          }
#endif
          if(proc_ptr->chk_on_attach_def_bc_setup_ptr() == TRUE)
    ...
    case PDN_DISCONNECT_REJECT:
                                                          ...
                                                          UNASSIGNED_BEARER_ID)
      {
        ...
        {
          esm_stop_timer(ESM_TIMER_T3492, proc_ptr);
          /*free as we don't pass to DS*/
          if (data_ptr->pdn_disconnect_rej.ext_protocol_conf_opt.dsm_ptr != NULL) 
          {
            dsm_free_packet((dsm_item_type **)&(data_ptr->pdn_disconnect_rej.ext_protocol_conf_opt.dsm_ptr));
            data_ptr->pdn_disconnect_rej.ext_protocol_conf_opt.dsm_ptr = NULL;
          }
          ...
          if((emm_get_3gpp_rel_ver_per_sub((sys_modem_as_id_e_type)sm_as_id)
                                                         >= LTE_3GPP_REL13) &&
             (ota_cause.valid == TRUE) &&
             (data_ptr->pdn_disconnect_rej.esm_cause.esm_cause 
                                 == NAS_ESM_LAST_PDN_DISCONNECTED_NOT_ALLOWED)
#ifdef FEATURE_CIOT
    ...
    case BEARER_RESOURCE_MODIFICATION_REJECT:
      ...
      break;
#endif /*#ifdef FEATURE_NAS_SM_UE_INIT_QOS*/
    case ESM_INFORMATION_REQUEST:
      ...
      { /* Do not reject PDN from here, EMM willc lear it after getting STATUS message */ 
        esm_send_esm_status(&data_ptr->pdn_connectivity_rej.lte_nas_esm_hdr,
                             NAS_ESM_UNKNOWN_EPS_BEARER_CONTEXT);
      }
      break;
