...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smsndcp.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#endif
        /* XID parameter restore is not performed during 1st time PDP activation */
        cmd_ptr->mes.snsm_activ_ind.psm_restore = FALSE;
...
#endif
        /* XID parameters restore also required at SNDCP upon PSM exit to avoid unncessary XID negotiation with network.*/
        if (cur_pdp->llc_state == LLC_PSM_MODIFY_CNF_PENDING)
        {
          cmd_ptr->mes.snsm_modify_ind.psm_restore = TRUE;
        }
        else
        {
          cmd_ptr->mes.snsm_modify_ind.psm_restore = FALSE;
        }
...
void process_snsm_activate_resp(byte nsapi)
{
  ...
  if (((trans_id != INVALID_ID) && (trans_id >= MIN_MO_TI && trans_id < MAX_PDP_NUM)) && pdp_list[trans_id] != NULL && 
...
void process_snsm_modify_resp(byte nsapi)
{
  ...
  if (trans_id == INVALID_ID || trans_id >= MAX_PDP_NUM || pdp_list[trans_id] == NULL )
  {
  ...
  else if (pdp_list[trans_id]->llc_state == LLC_PSM_MODIFY_CNF_PENDING)
  {
    MSG_HIGH_DS_0(SM_SUB,"Received LLC PSM Modify confirm");
    if (pdp_list[trans_id]->pdp_state == PDP_ACTIVE)
    {
      /* reset the LLC state of this PDP context to Active */
      pdp_list[trans_id]->llc_state = LLC_ACTIVE;
    }
...
#ifdef FEATURE_MBMS
              if(pdp_list[cnt]->is_mbms == FALSE)
              {
                sm_deact_associated_mbms_ctxt(pdp_list[cnt]);
              }
#endif        
...
/*===========================================================================
FUNCTION SM_SEND_SNDCP_ENTER_PSM_IND

DESCRIPTION
 SM sends GSNSM_ENTER_PSM_IND command to SNDCP to indicate XID context needs to be stored in EFS upon PSM enter mode command.
...
===========================================================================*/
void sm_send_sndcp_enter_psm_ind(void)
{
  gsmsn_mes_type *cmd_ptr;
  ...
  if (cmd_ptr != NULL)
  {
    cmd_ptr->mes.snsm_enter_psm_ind.prim  = GSNSM_ENTER_PSM_IND;

#ifdef FEATURE_DUAL_SIM
    cmd_ptr->mes.snsm_enter_psm_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

    MSG_HIGH_DS_0(SM_SUB,"Sent GSNSM_ENTER_PSM_IND SM->SNDCP ");
