...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smcm.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
LOCAL void sm_pdp_activate (byte connection_id, 
                              mm_pdp_activate_req_T *pdp_act);

/* -----------------------------------------------------------------------
** FUNCTION     : SM_PDP_SEND_CM_CALL_CONTROL()
** DESCRIPTION  : This function sends the Call Control Request to CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_pdp_send_cm_call_control ( pdp_context_info *pdp);

/* -----------------------------------------------------------------------
** FUNCTION     : SM_CM_PROCESS_CALL_CONTROL_RSP()
** DESCRIPTION  : This function sends the Call Control Request to CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_cm_process_call_control_rsp ( cm_mm_call_ctrl_rsp_s_type *call_ctrl_rsp );


#ifdef FEATURE_MBMS

/* -----------------------------------------------------------------------
** FUNCTION     : SM_MBMS_ACTIVATE()
** DESCRIPTION  : This function processes MBMS activation request by CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_mbms_activate (byte connection_id,
                              const smreg_mbms_activate_req_T *pdp_act);
/* -----------------------------------------------------------------------
** FUNCTION     : SM_MBMS_ABORT()
** DESCRIPTION  : This function processes MBMS abort request by CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_mbms_abort (const smreg_mbms_abort_req_T* mbms_abort);
...
LOCAL void sm_pdp_deactivate (byte con_id, 
                               mm_pdp_deactivate_req_T *pdp_deactivate);

/* -----------------------------------------------------------------------
** FUNCTION     : SM_PDP_MODIFY()
** DESCRIPTION  : This function processes PDP modify request by CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_pdp_modify (byte con_id, mm_pdp_modify_req_s_type *pdp_modify);
...
LOCAL void sm_sec_pdp_activate(byte con_id, 
                                mm_pdp_activate_sec_req_T* sec_pdp_req);
...
LOCAL void sm_pdp_nw_modify_rej (byte connection_id, 
                              mm_pdp_nw_modify_rej_T *pdp_nw_mod_rej);

/* -----------------------------------------------------------------------
** FUNCTION  sm_cm_process_restore_pdp_ctxt_req()
** DESCRIPTION  : This function processes PDP restore request by CM.
** ----------------------------------------------------------------------- */
LOCAL void sm_cm_process_restore_pdp_ctxt_req(mm_restore_pdp_ctxt_req_s_type *restore_pdp);
...
boolean sm_unreserve_connection_id_int(byte connection_id)
{
   ...
   ** --------------------------------------------------------------------*/
   ...
   {
     #ifndef FEATURE_GPRS_CALLCONTROL_SM
...
#ifdef FEATURE_GPRS_CALLCONTROL_SM
...
- If the PDP type is IP, dynamic addressing is applicable.*/
    ...
    /* Set NAS signaling low priority indicator for the PDP context */
    cur_pdp->low_priority_signaling = pdp_act->low_priority_signaling;
    ...
    */
    ...
    else if(ds_3gpp_is_apn_blocked(cur_pdp->apn_name))
    {
    ...
    else
    {
      ...
      cur_pdp->as_id = pdp_act->as_id ;
       ...
       sm_pdp_send_cm_call_control(cur_pdp);

       /* Inform GSDI that data call is connected for polling */
       sm_mmgsdi_in_traffic_channel_mode(TRUE);
...
DESCRIPTION
  This function sends the Call Control Request to CM. 
...
===========================================================================*/
void sm_pdp_send_cm_call_control
(
...
)
{
	...
	//Copy the contents to CM 
	
	cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.length  = mesg_bit_len;
	memscpy((void*)&cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.pdp_act_mesg[0],
		  sizeof(cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.pdp_act_mesg),
		  (void*)l3_buf,
		  cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.length);
	
	cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.connection_id  = pdp->trans_id;
	cm_rpt->cmd.call_ctrl_req.ps_params.gw_ps_params.apn_name = pdp->apn_name;
	
	cm_rpt->cmd.call_ctrl_req.ps_rat = SYS_RAT_GSM_RADIO_ACCESS;
	cm_rpt->hdr.cmd = CM_CALL_CTRL_REQ;
	
	MSG_HIGH_DS_0(SM_SUB, "=SM= SM sent CM_CALL_CTRL_REQ");
	cm_put_cmd(cm_rpt);
...
/*===========================================================================
FUNCTION SM_CM_PROCESS__CALL_CONTROL_RSP

DESCRIPTION
  This function handles the Call Control Response from CM. 
...
void sm_cm_process_call_control_rsp
(
  cm_mm_call_ctrl_rsp_s_type *call_ctrl_rsp
)
{
  /* current pdp context in the transaction */
   ...
   con_id = call_ctrl_rsp->ps_params.gw_ps_params.connection_id;

   MSG_HIGH_DS_2(SM_SUB, "=SM= SM sent CM_CALL_CTRL_RSP received with  con_id : %d, Status : %d", con_id, call_ctrl_rsp->call_control_result);
   
   if (!(con_id >= MIN_MO_TI && con_id < MAX_PDP_NUM))
   {
	  return;
   }
   cur_pdp = pdp_list[con_id];
  ...
  switch(call_ctrl_rsp->call_control_result)
  {
     case ESM_MSG_ACCEPTED:
     case ESM_MSG_MODIFIED:
      
      status=  unpack_pdp_activate_mesg ( (uint16)call_ctrl_rsp->ps_params.gw_ps_params.length, 
	  	                                                     &call_ctrl_rsp->ps_params.gw_ps_params.pdp_act_mesg[0], cur_pdp);

	//Check APN if its in Blocking list
	if(ds_3gpp_is_apn_blocked(cur_pdp->apn_name))
	{
	  MSG_HIGH_DS_0(SM_SUB,"PDP act Rej, APN blocked");
	  cur_pdp->current_cause = (cc_cause_T)SM_APN_BLOCKED;
	  //Reject the PDP context as duplicate context is detected
...
)
{
  ...
  #ifdef FEATURE_MBMS
  if (cur_pdp->is_mbms == TRUE)
  {
    MSG_HIGH_DS_1( SM_SUB,"MBMS: sm_pdp_deactivate not intended for MBMS, ignoring this message %d", con_id);
    return;
  }
  #endif
...
#ifdef FEATURE_MBMS
  else if (cur_pdp->is_mbms == TRUE)
  {
    MSG_HIGH_DS_1(SM_SUB,"MBMS:TI %d is MBMS Ctxt can not be deact MO", cur_pdp->trans_id);
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_PDP_DEACTIVATE_REJ, SM_INTERNAL_CAUSE); // MBMS_PENDING
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    return;
  }
  {
    sm_deact_associated_mbms_ctxt(cur_pdp);
  }
#endif
  ...
  if (cur_pdp->pdp_state == PDP_ACTIVE || cur_pdp->pdp_state == PDP_MODIFY_PENDING)
  {
	...
	#ifdef FEATURE_MBMS
			  sm_deact_associated_mbms_ctxt(pdp_list[index]);
	#endif
			  if (is_umts_mode())
			  {
				// Inform the RABM that the PDP context is being deactivated
...
===========================================================================*/
void sm_pdp_modify (byte con_id, mm_pdp_modify_req_s_type *pdp_modify)
{
  pdp_context_info *cur_pdp; /* current pdp context in the transaction */
  ...
  if (cur_pdp->is_mbms == TRUE)
  {
    MSG_HIGH_DS_1( SM_SUB,"MBMS: Modify Request cannot be for a MBMS Context TI=%d", con_id);
    cur_pdp->current_cause = (cc_cause_T)PDP_MODIFY_COLLISION; // MBMS_PENDING, wht should be the cause
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_PDP_MODIFY_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    return;
  ...
  else
  {
   ...
   /* if (pdp_modify->llc_sapi.valid)
    ...
    /* Set NAS signaling low priority indicator for the PDP context */
    cur_pdp->low_priority_signaling = pdp_modify->low_priority_signaling;
...
if ( ue_revision == UE_REV_PRE_REL99 )
{
  ...
  ** --------------------------------------------------------------------*/
  ...
  else if(ds_3gpp_is_apn_blocked(pdp_list[pri_pdp_index]->apn_name))
  {
  ...
  else
  {
    ...
    ** ----------------------------------------------------------------------*/
    ...
    /* Set NAS signaling low priority indicator for the PDP context */
    cur_pdp->low_priority_signaling = sec_pdp_req->low_priority_signaling;
...
void sm_continue_pdp_activation(pdp_context_info* pdp)
{
  ...
  byte *l3_buf; /* Buffer to store the packed L3 msg*/
#ifdef FEATURE_WCDMA
  rrc_establish_cause_e_type cause_type;
#endif
  byte traffic_class = pdp->qos.traffic_class;

#if defined(FEATURE_NAS_REL12) && defined(FEATURE_WCDMA)
  /*---------------------------------------------------------------------------------------------------------------------
    From 24.008 Annex L, Table L.1.2
     If the ACTIVATE PDP CONTEXT REQUEST has the Request Type not set to "emergency", 
     the RRC establishment cause shall be set to either ‘Originating Conversational Call’ or 
	 ‘Originating Streaming Call’ or ‘Originating Interactive Call’ or ‘Originating Background Call ‘
	 depending on the Traffic Class in QoS of the "most demanding" Traffic Class, considering 
	 all active PDP contexts together with the PDP context to be activated. (see Note 1) 

     If Traffic Class in QoS is not ‘Conversational Class’ or ‘Streaming Class’ or ‘Interactive Class’ 
	 or ‘Background Class’ but is ‘Subscribed Traffic Class’, then ‘Originating Subscribed traffic Call’ shall be used.
	 
     NOTE 1:	For classification of "most demanding" Traffic Class the following ranking order applies: 
	 "Conversational" followed by "Streaming" followed by "Interactive" followed by "Background", where 
	 "Conversational" is the most demanding Traffic class in terms of being delay sensitive. 
  -------------------------------------------------------------------------------------------------------------------------*/
  if (mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12)
  {
   if ( pdp->qos.traffic_class != SUBSCRIBED_CLASS)
   {
	  //Get the most demanding traffic class including the PDP  context to be activated
      traffic_class = sm_get_most_demanding_traffic_class(TRUE);
      if (pdp->qos.traffic_class  < traffic_class)
      {
        traffic_class = pdp->qos.traffic_class;
      }
    }
  }
#endif
#ifdef FEATURE_WCDMA
  cause_type = convert_to_rrc_est_cause(traffic_class);
#endif
  ...
  {    
    ...
    ** -----------------------------------------------------------------*/
    ...
    if ( !timer_est_pen_flag )
    {
      ...
      if (pdp->is_mbms == TRUE )
      {
        cause_type = RRC_EST_MBMS_RECEPTION;
      }
      #endif
      
      send_gmm_establish_req(
#ifdef FEATURE_WCDMA
      cause_type
#endif
);    
...
)
{
  ...
  #ifdef FEATURE_MBMS
  if (pdp_list[con_id]->is_mbms == TRUE)
  {
    MSG_HIGH_DS_1( SM_SUB,"MBMS: sm_pdp_activate_rej_rsp not intended for MBMS, ignoring this message %d", con_id);
    return;
  }
  #endif
...
)
{
  ...
  switch (cm_cmd)
  {
    ...
    case SMREG_MBMS_ACTIVATE_REJ:
            msm_cmd = msm_get_cmd_buf();
            if (msm_cmd == NULL)
            {
              MSG_HIGH_DS_1(SM_SUB,"MBMS: Unable to get allocate MSM Buffer TransId: %d", con_id);
              return ;
            }
            msm_cmd->cmd.sm_mbms_act_rej.connection_id = con_id;

            msm_cmd->cmd.sm_mbms_act_rej.cause_type= SM_INTERNAL_CAUSE;
            msm_cmd->cmd.sm_mbms_act_rej.cause.int_cause = cause;
            MSG_HIGH_DS_2(SM_SUB,"MBMS: Sending SMREG_MBMS_ACTIVATE_REJ to MSM Cause:%d, type:%d",
            msm_cmd->cmd.sm_mbms_act_rej.cause.int_cause, msm_cmd->cmd.sm_mbms_act_rej.cause_type);

            msm_cmd->cmd_hdr.cmd_id = MSM_SM_ACTIVATION_REJ;
            msm_put_cmd(msm_cmd);
...
)
{
  ...
  ds_emm_cmd_type *ds_rpt;

  switch (cm_cmd)
  {
    case SMREG_PDP_ACTIVATE_REJ:
          /* ---------------------------------------------------------------
          *    Sending PDP_ACTIVATE_REJ to DS
          * ----------------------------------------------------------------*/
          
          // Set the connection Id
          ds_rpt = ds_get_cmd_buf();
          if(ds_rpt!=NULL)
          {
            ds_rpt->cmd.pdp_activate_rej.connection_id = con_id;
    
            // Pack the Cause based on if its internal or external
            ds_rpt->cmd.pdp_activate_rej.cause_type = SM_INTERNAL_CAUSE;             
            ds_rpt->cmd.pdp_activate_rej.cause.int_cause = cause;     
            MSG_HIGH_DS_2(SM_SUB,"Send MM_PDP_ACTIVATE_REJ to DS Cause:%d, type:%d",
                         ds_rpt->cmd.pdp_activate_rej.cause.int_cause, ds_rpt->cmd.pdp_activate_rej.cause_type);
            // Set the CM report header and the command
            //ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_REJ;
			
			// Set the CM report header and the command
		    ds_rpt->cmd.pdp_activate_rej.message_header.message_set = MS_MM_DS;
		    ds_rpt->cmd.pdp_activate_rej.message_header.message_id = (int) MM_PDP_ACTIVATE_REJ;
		    PUT_IMH_LEN( sizeof(mm_pdp_activate_rej_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_activate_rej.message_header );
#ifdef FEATURE_DUAL_SIM
            ds_rpt->cmd.pdp_activate_rej.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
            mm_add_message_to_debug_buffer(MS_MM_DS,
                                          (dword)ds_rpt->cmd.hdr.message_id,
                                           FROM_MM,
                                           (sys_modem_as_id_e_type)sm_as_id);
       
	        if (sm_check_if_active_pdp_exists() == FALSE)
            {
            ...
            //Send to DS
            ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_activate_rej);

            //Clean the DS message buffer
            modem_mem_free(ds_rpt, MODEM_MEM_CLIENT_NAS);
			
          }
          else
          {
            MSG_ERROR_DS_0(SM_SUB,"CM Error: Not able to Allocate DS Buffer");
          }
    ...
    case SMREG_PDP_DEACTIVATE_REJ:
          ...
          if(ds_rpt!=NULL)
          {
            ds_rpt->cmd.pdp_deact_rej.connection_id = con_id;
    
            // Pack the NSAPI 
            ds_rpt->cmd.pdp_deact_rej.cause_type = SM_INTERNAL_CAUSE;             
            ds_rpt->cmd.pdp_deact_rej.cause.int_cause = cause; 
          MSG_HIGH_DS_2(SM_SUB,"Send MM_PDP_DEACTIVATE_REJ to DS Cause:%d, type:%d",
                             ds_rpt->cmd.pdp_deact_rej.cause.int_cause, ds_rpt->cmd.pdp_deact_rej.cause_type);
            // Set the CM report header and the command
            //ds_rpt->hdr.cmd = CM_PDP_DEACTIVATE_REJ;
            // Set the CM report header and the command
            ds_rpt->cmd.pdp_deact_rej.message_header.message_set = MS_MM_DS;
            ds_rpt->cmd.pdp_deact_rej.message_header.message_id = (int) MM_PDP_DEACTIVATE_REJ;
            PUT_IMH_LEN( sizeof(mm_pdp_deactivate_rej_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_deact_rej.message_header );

#ifdef FEATURE_DUAL_SIM
            ds_rpt->cmd.pdp_deact_rej.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
            mm_add_message_to_debug_buffer(MS_MM_DS,
                                           (dword)ds_rpt->cmd.hdr.message_id,
                                            FROM_MM,
            ...
            //Send to DS
            ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_deact_rej);
     ...
     case SMREG_PDP_MODIFY_REJ:
	  ...
	  if(ds_rpt!=NULL)
	  {
		ds_rpt->cmd.pdp_modify_rej.connection_id = con_id;

		ds_rpt->cmd.pdp_modify_rej.cause_type = SM_INTERNAL_CAUSE;			   
		ds_rpt->cmd.pdp_modify_rej.cause.int_cause = cause; 	
	      MSG_HIGH_DS_2(SM_SUB,"Send PDP_MODIFY_REJ to CDSM Cause:%d, type:%d",
						 ds_rpt->cmd.pdp_modify_rej.cause.int_cause, ds_rpt->cmd.pdp_modify_rej.cause_type);
		// Set the CM report header and the command
		//ds_rpt->hdr.cmd = CM_PDP_MODIFY_REJ;
	       ds_rpt->cmd.pdp_modify_rej.message_header.message_set = MS_MM_DS;
		 ds_rpt->cmd.pdp_modify_rej.message_header.message_id = (int) MM_PDP_DEACTIVATE_REJ;
		 PUT_IMH_LEN( sizeof(mm_pdp_modify_rej_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_modify_rej.message_header );
			
#ifdef FEATURE_DUAL_SIM
		ds_rpt->cmd.pdp_modify_rej.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ...
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_modify_rej);
    ...
    default :
        MSG_HIGH_DS_1(SM_SUB,"Unknown command (SM -> DS) received %x", cm_cmd);
        break;
  }
...
/*===========================================================================
FUNCTION SM_SEND_DS_CONFIRM 

DESCRIPTION
  This function sends a confirmation to the DS task after processing the REQ. 
  This includes both the CNF primitives and REJ primitives.
...

{
...
#endif    
  {
    ...
    if(ds_rpt == NULL)
    {
      MSG_HIGH_DS_0(SM_SUB,"Unable to get allocate DS rpt Buffer");
      return ;
    }
	MSG_HIGH_DS_3( SM_SUB," Sending CMD :0x%X to DS TI: %d, NSAPI: %d",
    ...
    switch (cm_cmd)
    {
      case SMREG_PDP_ACTIVATE_CNF:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_activate_cnf.connection_id = cur_pdp->trans_id;
        //Set the Group id
        ds_rpt->cmd.pdp_activate_cnf.group_id = cur_pdp->group_id;
        //Set the net_qos_diff_flag
        ds_rpt->cmd.pdp_activate_cnf.is_net_alloc_qos_diff_from_req = cur_pdp->is_net_alloc_qos_diff_from_req;
  

        // Pack the NSAPI
        ds_rpt->cmd.pdp_activate_cnf.nsapi = cur_pdp->nsapi;
        // Pack PDP address     
        ds_rpt->cmd.pdp_activate_cnf.pdp_addr = cur_pdp->pdp_addr;
        // Pack QOS
        ds_rpt->cmd.pdp_activate_cnf.qos = cur_pdp->qos;
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_activate_cnf.config_options = cur_pdp->config_options;
  
#ifdef FEATURE_DUAL_SIM  
        ds_rpt->cmd.pdp_activate_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ...
        ** -----------------------------------------------------------------*/
        ...
        // Set the CM report header and the command
		ds_rpt->cmd.pdp_activate_cnf.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_activate_cnf.message_header.message_id = (int) MM_PDP_ACTIVATE_CNF;
		PUT_IMH_LEN( sizeof(mm_pdp_activate_cnf_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_activate_cnf.message_header );
        //ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_CNF;
        ...
        ds_rpt->cmd.pdp_activate_cnf.esm_conn_id = cur_pdp->esm_conn_id;
#endif
#endif /*FEATURE_LTE*/
#ifdef FEATURE_DUAL_IP_BEARER
        ds_rpt->cmd.pdp_activate_cnf.cause_type = SM_NETWORK_CAUSE;
        ds_rpt->cmd.pdp_activate_cnf.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
#endif

        //Send to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_activate_cnf);
		
        break;
        
#ifdef FEATURE_NAS_SM_MT_PDP
      ...
      case SMREG_PDP_ACTIVATE_IND:
        ...
        * ----------------------------------------------------------------*/
        ...
        //Set the Group id
        ds_rpt->cmd.pdp_act_ind.group_id = cur_pdp->group_id;
        // Pack PDP address
        ds_rpt->cmd.pdp_act_ind.pdp_addr = cur_pdp->pdp_addr;
        // Pack the APN 
        ds_rpt->cmd.pdp_act_ind.apn_name = cur_pdp->apn_name;
  
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_act_ind.config_options = cur_pdp->config_options;
#endif
#ifdef FEATURE_GPRS_CALLCONTROL_SM
        ds_rpt->cmd.pdp_act_ind.llc_sapi.valid = TRUE;
        ds_rpt->cmd.pdp_act_ind.llc_sapi.sapi = LLC_SAPI_3;
        ds_rpt->cmd.pdp_act_ind.msg_type = (byte)ACTIVATE_PDP_REQUEST;
        ds_rpt->cmd.pdp_act_ind.nsapi.nsapi = cur_pdp->nsapi.nsapi;
        ds_rpt->cmd.pdp_act_ind.sm_pd = SM_PD;
#endif
        // Set the connection Id
        ds_rpt->cmd.pdp_act_ind.connection_id = cur_pdp->trans_id;
  
#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_act_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ...
        ** -----------------------------------------------------------------*/
        ...
        // Set the CM report header and the command
        ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_IND;
        break;
 #endif
      ...
      case SMREG_PDP_DEACTIVATE_CNF:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_deact_cnf.connection_id = cur_pdp->trans_id;
  
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_deact_cnf.nsapi = cur_pdp->nsapi;
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_deact_cnf.config_options = cur_pdp->config_options;
#endif
  
#ifdef FEATURE_DUAL_SIM   
        ds_rpt->cmd.pdp_deact_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_DEACTIVATE_CNF;
		ds_rpt->cmd.pdp_deact_cnf.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_deact_cnf.message_header.message_id = (int) MM_PDP_DEACTIVATE_CNF;
		PUT_IMH_LEN( sizeof(mm_pdp_deactivate_cnf_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_deact_cnf.message_header );
	...
	//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_deact_cnf);
        break;
      ...
      case SMREG_PDP_DEACTIVATE_IND:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_deact_ind.connection_id = cur_pdp->trans_id;
        
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_deact_ind.nsapi = cur_pdp->nsapi;
        /* From 24.008 section 6.3 Coordination between SM and GMM for supporting ISR
        ** The MS shall change its TIN to "P-TMSI" to deactivate ISR:
        **    b)upon deactivation of the last PDP context in the MS
        */
        ...
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_deact_ind.cause_type = cause_type;             
        if (cause_type == SM_INTERNAL_CAUSE)
        {
            ds_rpt->cmd.pdp_deact_ind.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;     
        }
        else
        {
            ds_rpt->cmd.pdp_deact_ind.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
        }

		
#ifndef FEATURE_LTE_REL12
       //Set the Default Values to backoff timer
        ds_rpt->cmd.pdp_deact_ind.backoff_timer.active = TRUE;
        ds_rpt->cmd.pdp_deact_ind.backoff_timer.timer_count = 0;
#endif

#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_deact_ind.config_options = cur_pdp->config_options;
#endif

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_deact_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
#ifdef FEATURE_NAS_REL11
        ds_rpt->cmd.pdp_deact_ind.backoff_timer = cur_pdp->sm_gprs_type3_timer;
#endif
        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_DEACTIVATE_IND;
	    ds_rpt->cmd.pdp_deact_ind.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_deact_ind.message_header.message_id = (int) MM_PDP_DEACTIVATE_IND;
		PUT_IMH_LEN( sizeof(mm_pdp_deactivate_ind_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_deact_ind.message_header );
		...
		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_deact_ind);
        break;
      ...
      case SMREG_PDP_DEACTIVATE_PROMOTE_IND:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_deact_prom_ind.connection_id = cur_pdp->trans_id;
        
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_deact_prom_ind.nsapi = cur_pdp->nsapi;
        //Pack the promoted secondary to pri con id
        ds_rpt->cmd.pdp_deact_prom_ind.promote_con_id = cur_pdp->linked_ti;
    
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_deact_prom_ind.cause_type = cause_type;             
        if (cause_type == SM_INTERNAL_CAUSE)
        {
            ds_rpt->cmd.pdp_deact_prom_ind.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;     
        }
        else
        {
            ds_rpt->cmd.pdp_deact_prom_ind.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
        }
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_deact_prom_ind.config_options = cur_pdp->config_options;
#endif

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_deact_prom_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

        // Set the CM report header and the command
        ds_rpt->hdr.cmd = CM_PDP_DEACTIVATE_PROMOTE_IND;
        cur_pdp->pdp_state = PDP_INACTIVE;
#ifdef FEATURE_LTE
#ifdef FEATURE_NAS_SM_IRAT
      ...
      case SMREG_PDP_DEACTIVATE_PROMOTE_CNF:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_deact_prom_cnf.connection_id = cur_pdp->trans_id;
        
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_deact_prom_cnf.nsapi = cur_pdp->nsapi;
        //Pack the promoted secondary to pri con id
        ds_rpt->cmd.pdp_deact_prom_cnf.promote_con_id = cur_pdp->linked_ti;
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_deact_prom_cnf.config_options = cur_pdp->config_options;
#endif

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_deact_prom_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

        // Set the CM report header and the command
        ds_rpt->hdr.cmd = CM_PDP_DEACTIVATE_PROMOTE_CNF;
        cur_pdp->pdp_state = PDP_INACTIVE;
#ifdef FEATURE_LTE
#ifdef FEATURE_NAS_SM_IRAT
      ...
      case SMREG_PDP_MODIFY_CNF:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_modify_cnf.connection_id = cur_pdp->trans_id;
  
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_modify_cnf.nsapi = cur_pdp->nsapi;
        // Pack the qos
        ds_rpt->cmd.pdp_modify_cnf.qos = cur_pdp->qos; 
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_modify_cnf.config_options = cur_pdp->config_options;
#endif
        /* -------------------------------------------------------------------
        ** Set the cm_pending_msg to UNKNOWN_CMD, as the CM cmd has been
        ** processed and confirmation/rejection is being sent to the CM task
...
#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_modify_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_MODIFY_CNF;
		ds_rpt->cmd.pdp_modify_cnf.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_modify_cnf.message_header.message_id = (int) MM_PDP_MODIFY_CNF;
		PUT_IMH_LEN( sizeof(mm_pdp_modify_cnf_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_modify_cnf.message_header );

	//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_modify_cnf);
      ...
      case SMREG_PDP_MODIFY_IND:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_modify_ind.connection_id = cur_pdp->trans_id;
        //Reset the MT orig flag
        cur_pdp->mt_orig_type = FALSE;
  
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_modify_ind.nsapi = cur_pdp->nsapi;
        // Pack the qos
        ds_rpt->cmd.pdp_modify_ind.qos = cur_pdp->qos;
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_modify_ind.config_options = cur_pdp->config_options;
#endif

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_modify_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ...
        ** -----------------------------------------------------------------*/
        ...
        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_MODIFY_IND;
		ds_rpt->cmd.pdp_modify_ind.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_modify_ind.message_header.message_id = (int) MM_PDP_MODIFY_IND;
		PUT_IMH_LEN( sizeof(mm_pdp_modify_ind_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_modify_ind.message_header );
				
        ds_rpt->cmd.pdp_modify_ind.tft = cur_pdp->tft;
        MSG_HIGH_DS_2(SM_SUB, "BCM Mode for TI %d is %d", cur_pdp->trans_id, cur_pdp->bcm);
#ifdef FEATURE_LTE
		...
		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_modify_ind);
      ...
      case SMREG_PDP_ACTIVATE_SEC_CNF:
        ...
        * ----------------------------------------------------------------*/
        ...
        cur_pdp->mt_orig_type = FALSE;    // Clear the MT indication flag
        //set the connection id
        ds_rpt->cmd.pdp_act_sec_conf.connection_id = cur_pdp->trans_id;
        //Set the net_qos_diff_flag
        ds_rpt->cmd.pdp_act_sec_conf.is_net_alloc_qos_diff_from_req = cur_pdp->is_net_alloc_qos_diff_from_req;
        //Pack the QoS
        ds_rpt->cmd.pdp_act_sec_conf.qos = cur_pdp->qos;
        //Pack the NSAPI
        ds_rpt->cmd.pdp_act_sec_conf.nsapi = cur_pdp->nsapi;
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_act_sec_conf.config_options = cur_pdp->config_options;
#endif

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_act_sec_conf.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ...
        ** -----------------------------------------------------------------*/
        ...
        //ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_SEC_CNF;
		ds_rpt->cmd.pdp_act_sec_conf.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_act_sec_conf.message_header.message_id = (int) MM_PDP_ACTIVATE_SEC_CNF;
		PUT_IMH_LEN( sizeof(mm_pdp_activate_sec_cnf_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_act_sec_conf.message_header );
        ...
        cur_pdp->esm_conn_id = esm_update_esm_context_from_sm(cur_pdp);
        ds_rpt->cmd.pdp_act_sec_conf.esm_conn_id = cur_pdp->esm_conn_id;
#endif
#endif /*FEATURE_LTE*/

		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_act_sec_conf);
      ...
      case SMREG_PDP_ACTIVATE_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_activate_rej.nsapi = cur_pdp->nsapi;
  
        // Set the connection Id
        ds_rpt->cmd.pdp_activate_rej.connection_id = cur_pdp->trans_id;
  
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_activate_rej.cause_type = cause_type;             
        if (cause_type == SM_INTERNAL_CAUSE)
        {
          ds_rpt->cmd.pdp_activate_rej.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;     
        }
        else
        {
          ds_rpt->cmd.pdp_activate_rej.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
        }
        ds_rpt->cmd.pdp_activate_rej.nas_max_ota_negotiated_len = nas_negotiated_ota_len;
  
        MSG_HIGH_DS(SM_SUB,"Send PDP_ACTIVATE_REJ to CM TI: %d, Cause:%d, type:%d",
                       cur_pdp->trans_id, ds_rpt->cmd.pdp_activate_rej.cause.int_cause, ds_rpt->cmd.pdp_activate_rej.cause_type);
  
        // Pack Protocol Configurations
        ds_rpt->cmd.pdp_activate_rej.config_options = cur_pdp->config_options;
        ds_rpt->cmd.pdp_activate_rej.backoff_timer = cur_pdp->sm_gprs_type3_timer;
#ifdef FEATURE_LTE_REL12
        ds_rpt->cmd.pdp_activate_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif		
        ds_rpt->cmd.pdp_activate_rej.pdp_reject_param.apn_name = cur_pdp->apn_name;
        ds_rpt->cmd.pdp_activate_rej.pdp_reject_param.pdp_type = cur_pdp->pdp_addr.pdp_type_num;        
      ...
      case SMREG_PDP_ACTIVATE_SEC_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Pack the NSAPI
        ds_rpt->cmd.pdp_act_sec_rej.nsapi = cur_pdp->nsapi;
  
        // Set the connection Id
        ds_rpt->cmd.pdp_act_sec_rej.connection_id = cur_pdp->trans_id;
  
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_act_sec_rej.cause_type = cause_type;
        if (cause_type == SM_INTERNAL_CAUSE)
        {
            ds_rpt->cmd.pdp_act_sec_rej.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;
        }
        else
        {
            ds_rpt->cmd.pdp_act_sec_rej.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
        }
        ds_rpt->cmd.pdp_act_sec_rej.nas_max_ota_negotiated_len = nas_negotiated_ota_len;
        MSG_HIGH_DS(SM_SUB," Send PDP_ACTIVATE_SEC_REJ to CM for TI: %d, Cause:%d, type:%d",
                       cur_pdp->trans_id, ds_rpt->cmd.pdp_act_sec_rej.cause.int_cause, ds_rpt->cmd.pdp_act_sec_rej.cause_type);
  
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_act_sec_rej.config_options = cur_pdp->config_options;
#endif
        ds_rpt->cmd.pdp_act_sec_rej.backoff_timer = cur_pdp->sm_gprs_type3_timer;
#ifdef FEATURE_LTE_REL12
		ds_rpt->cmd.pdp_act_sec_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif

        ds_rpt->cmd.pdp_act_sec_rej.pdp_reject_param.apn_name = cur_pdp->apn_name;
        ds_rpt->cmd.pdp_act_sec_rej.pdp_reject_param.pdp_type = cur_pdp->pdp_addr.pdp_type_num; 
        if (cur_pdp->nsapi.valid == TRUE)
        (void)sm_release_nsapi_sm (cur_pdp->trans_id);
...
#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_act_sec_rej.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        if (sm_check_if_active_pdp_exists() == FALSE)
        {
          /* Inform GSDI that data call is disconnected to stop polling */
        ...
        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_SEC_REJ;
        ds_rpt->cmd.pdp_act_sec_rej.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_act_sec_rej.message_header.message_id = (int) MM_PDP_ACTIVATE_SEC_REJ;
		PUT_IMH_LEN( sizeof(mm_pdp_activate_sec_rej_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_act_sec_rej.message_header );
		
		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_act_sec_rej);
      ...
      case    SMREG_PDP_MODIFY_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_modify_rej.connection_id = cur_pdp->trans_id;
        ...
        ** -----------------------------------------------------------------*/
        ...
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_modify_rej.nsapi = cur_pdp->nsapi;
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_modify_rej.cause_type = cause_type;             
        if (cause_type == SM_INTERNAL_CAUSE)
        {
            ds_rpt->cmd.pdp_modify_rej.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;     
        }
        else
        {
            ds_rpt->cmd.pdp_modify_rej.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
        }
        ds_rpt->cmd.pdp_modify_rej.nas_max_ota_negotiated_len = nas_negotiated_ota_len;
#ifdef FEATURE_REL5
        // Pack Protocol Configuration Optoins
        ds_rpt->cmd.pdp_modify_rej.config_options = cur_pdp->config_options;
#endif
        MSG_HIGH_DS(SM_SUB,"Send PDP_MODIFY_REJ to CM for TI: %d, Cause:%d, type:%d",
                     cur_pdp->trans_id,ds_rpt->cmd.pdp_modify_rej.cause.int_cause,
                     ds_rpt->cmd.pdp_modify_rej.cause_type);

#ifdef FEATURE_DUAL_SIM
        ds_rpt->cmd.pdp_modify_rej.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
        ds_rpt->cmd.pdp_modify_rej.backoff_timer = cur_pdp->sm_gprs_type3_timer;
#ifdef FEATURE_LTE_REL12
		ds_rpt->cmd.pdp_modify_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif
        ...
        //ds_rpt->hdr.cmd = CM_PDP_MODIFY_REJ;
        ds_rpt->cmd.pdp_modify_rej.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_modify_rej.message_header.message_id = (int) MM_PDP_MODIFY_REJ;
		PUT_IMH_LEN( sizeof(mm_pdp_modify_rej_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_modify_rej.message_header );
      ...
      case    SMREG_PDP_DEACTIVATE_REJ:
        ...
        * ----------------------------------------------------------------*/
        ...
        // Set the connection Id
        ds_rpt->cmd.pdp_deact_rej.connection_id = cur_pdp->trans_id;
  
        // Pack the NSAPI 
        ds_rpt->cmd.pdp_deact_rej.nsapi = cur_pdp->nsapi;
        // Pack the Cause based on if its internal or external
        ds_rpt->cmd.pdp_deact_rej.cause_type = cause_type;             
        if (cause_type == SM_INTERNAL_CAUSE)
        {
            ds_rpt->cmd.pdp_deact_rej.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause; 
            MSG_HIGH_DS_3(SM_SUB,"Send PDP_DEACTIVATE_REJ to CM for TI: %d, Cause:%d, type:%d",
                         cur_pdp->trans_id, ds_rpt->cmd.pdp_deact_rej.cause.int_cause, 
                         ds_rpt->cmd.pdp_deact_rej.cause_type);
        }
        else
        {
            ds_rpt->cmd.pdp_deact_rej.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
            MSG_HIGH_DS_3(SM_SUB,"Send PDP_DEACTIVATE_REJ to CM for TI: %d, Cause:%d, type:%d",
                         cur_pdp->trans_id, 
                         ds_rpt->cmd.pdp_deact_rej.cause.ext_cause, 
                         ds_rpt->cmd.pdp_deact_rej.cause_type);
        }
        cur_pdp->pdp_state = PDP_INACTIVE;
#ifdef FEATURE_LTE
        ...
        case SMREG_PDP_ACTIVATE_SEC_IND:
          ...
          * ----------------------------------------------------------------*/
          ...
          ds_rpt->cmd.pdp_act_sec_ind.connection_id = cur_pdp->trans_id;
          ds_rpt->cmd.pdp_act_sec_ind.group_id = pdp_list[cur_pdp->linked_ti]->group_id;
          
          // Pack Protocol Configuration Optoins
          ds_rpt->cmd.pdp_act_sec_ind.config_options = cur_pdp->config_options;

          //Pack the QoS
          ds_rpt->cmd.pdp_act_sec_ind.qos = cur_pdp->qos;

          //Pack the TFT
          ds_rpt->cmd.pdp_act_sec_ind.tft = cur_pdp->tft;

#ifdef FEATURE_GPRS_CALLCONTROL_SM
          ds_rpt->cmd.pdp_act_sec_ind.nsapi = cur_pdp->nsapi;
          ds_rpt->cmd.pdp_act_sec_ind.llc_sapi.valid = TRUE;
          ds_rpt->cmd.pdp_act_sec_ind.llc_sapi.sapi = LLC_SAPI_3;
          
          ds_rpt->cmd.pdp_act_sec_ind.msg_type = (byte)ACTIVATE_SEC_PDP_REQUEST;          
          ds_rpt->cmd.pdp_act_sec_ind.sm_pd = SM_PD;      
#endif /*FEATURE_GPRS_CALLCONTROL_SM*/           
        ...
        ** -----------------------------------------------------------------*/
        ...
        // Set the CM report header and the command
        //ds_rpt->hdr.cmd = CM_PDP_ACTIVATE_SEC_IND;
        ds_rpt->cmd.pdp_act_sec_ind.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_act_sec_ind.message_header.message_id = (int) MM_PDP_ACTIVATE_SEC_IND;
		PUT_IMH_LEN( sizeof(mm_pdp_activate_sec_ind_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_act_sec_ind.message_header );
				
#ifdef FEATURE_DUAL_SIM 
        ds_rpt->cmd.pdp_act_sec_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_act_sec_ind);
        ...
        case SMREG_PDP_NW_MODIFY_REQ:
          ...
          * ----------------------------------------------------------------*/
          ...
          ds_rpt->cmd.pdp_nw_modify_req.connection_id = cur_pdp->trans_id;
          ds_rpt->cmd.pdp_nw_modify_req.config_options = cur_pdp->config_options;           
          ds_rpt->cmd.pdp_nw_modify_req.tft = cur_pdp->tft;
          ds_rpt->cmd.pdp_nw_modify_req.nsapi = cur_pdp->nsapi;         
          ds_rpt->cmd.pdp_nw_modify_req.qos = cur_pdp->req_qos;
          ...
          ** -----------------------------------------------------------------*/
          ...
          // Set the CM report header and the command
          //ds_rpt->hdr.cmd = CM_PDP_NW_MODIFY_REQ;
		ds_rpt->cmd.pdp_nw_modify_req.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.pdp_nw_modify_req.message_header.message_id = (int) MM_PDP_NW_MODIFY_REQ;
		PUT_IMH_LEN( sizeof(mm_pdp_nw_modify_req_T) - sizeof(IMH_T),
                &ds_rpt->cmd.pdp_nw_modify_req.message_header );
#ifdef FEATURE_DUAL_SIM
          ds_rpt->cmd.pdp_nw_modify_req.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

		//Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.pdp_nw_modify_req);
		
        break;

        case SMREG_RESTORE_PDP_CTXT_RSP:
           /* ---------------------------------------------------------------
           *    Sending SMREG_RESTORE_PDP_CTXT_RSP  to CM after restoring
          ...
          // Pack the NSAPI
          ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.nsapi = cur_pdp->nsapi;
          ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.connection_id = cur_pdp->trans_id;

#ifdef FEATURE_DUAL_SIM  
          ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
          ...
          // Set the CM report header and the command
          //ds_rpt->hdr.cmd = CM_RESTORE_PDP_CTXT_RSP;
		ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.message_header.message_id = (int) MM_RESTORE_PDP_CTXT_RSP;
		PUT_IMH_LEN( sizeof(mm_restore_pdp_ctxt_rsp_s_type) - sizeof(IMH_T),
                &ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.message_header );
          ...
          MSG_HIGH_DS_1(SM_SUB,"PDP Context restored, updating ESM context for %d", cur_pdp->trans_id);
          cur_pdp->esm_conn_id = esm_update_esm_context_from_sm(cur_pdp);
          ds_rpt->cmd.mm_restore_pdp_ctxt_rsp.esm_conn_id = cur_pdp->esm_conn_id;

#endif
#endif /*FEATURE_LTE*/

         MSG_HIGH_DS_1( SM_SUB,"=SM= sent SMREG_RESTORE_PDP_CTXT_RSP NSAPI = %d", cur_pdp->nsapi.nsapi);

         if(sm_get_ulog_handle() != NULL)
         {
           ULOG_RT_PRINTF_1(sm_get_ulog_handle(), "=SM= sent SMREG_RESTORE_PDP_CTXT_RSP NSAPI = %d",cur_pdp->nsapi.nsapi);
         }
		 
        //Send cmd to DS
        ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.mm_restore_pdp_ctxt_rsp);
...
)
{
...
#ifdef FEATURE_LTE_REL12
        cm_rpt->cmd.pdp_act_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif		
...
#ifdef FEATURE_LTE_REL12
		cm_rpt->cmd.pdp_act_sec_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif
...
#ifdef FEATURE_LTE_REL12
		cm_rpt->cmd.pdp_modify_rej.reattempt_indicator = cur_pdp->reattempt_ind;
#endif
      ...
      case    SMREG_PDP_DEACTIVATE_REJ:
        ...
        ** -----------------------------------------------------------------*/
        ...
        case SMREG_RESTORE_PDP_CTXT_RSP:
          ...
          // Pack the NSAPI
          cm_rpt->cmd.restore_pdp_ctxt_rsp.nsapi = cur_pdp->nsapi;
          cm_rpt->cmd.restore_pdp_ctxt_rsp.connection_id = cur_pdp->trans_id;

#ifdef FEATURE_DUAL_SIM  
          cm_rpt->cmd.restore_pdp_ctxt_rsp.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
          ...
          // Set the CM report header and the command
          cm_rpt->hdr.cmd = CM_RESTORE_PDP_CTXT_RSP;
          ...
          MSG_HIGH_DS_1(SM_SUB,"PDP Context restored, updating ESM context for %d", cur_pdp->trans_id);
          cur_pdp->esm_conn_id = esm_update_esm_context_from_sm(cur_pdp);
          cm_rpt->cmd.restore_pdp_ctxt_rsp.esm_conn_id = cur_pdp->esm_conn_id;
  ...
  else
  {
    ...
    if (msm_cmd == NULL)
    {
        MSG_HIGH_DS_0(SM_SUB,"MBMS: Unable to get allocate MSM Buffer");
    }
    else 
    {
      switch (cm_cmd)
      {
        case SMREG_PDP_DEACTIVATE_CNF:
          ...
          *    Sending SMREG_PDP_DEACTIVATE_CNF to CM
          * ----------------------------------------------------------------*/
            MSG_HIGH_DS_1( SM_SUB,"MBMS: Sending SMREG_PDP_DEACTIVATE_CNF to MSM %d",cur_pdp->trans_id);
          msm_cmd->cmd.sm_mbms_deact_conf.connection_id = cur_pdp->trans_id;
          msm_cmd->cmd.sm_mbms_deact_conf.multicast_addr = cur_pdp->mip;
          msm_cmd->cmd.sm_mbms_deact_conf.mbms_config_options =  cur_pdp->mbms_prot_cnfg;

          cur_pdp->cm_pending_msg = UNKNOWN_CMD;
        

#ifdef FEATURE_DUAL_SIM
          msm_cmd->cmd.sm_mbms_deact_conf.as_id= (sys_modem_as_id_e_type)sm_as_id;
#endif

          // Set the MSM report header and the command
          msm_cmd->cmd_hdr.cmd_id = MSM_SM_DEACTIVATION_CNF;
        ...
        case SMREG_PDP_DEACTIVATE_IND:
          ...
          *    Sending SMREG_PDP_DEACTIVATE_IND to CM
          * ----------------------------------------------------------------*/
            MSG_HIGH_DS_1( SM_SUB,"MBMS: Sending SMREG_PDP_DEACTIVATE_IND to MSM %d",cur_pdp->trans_id);  
            
          msm_cmd->cmd.sm_mbms_deact_ind.connection_id = cur_pdp->trans_id;
          msm_cmd->cmd.sm_mbms_deact_ind.multicast_addr = cur_pdp->mip;
          msm_cmd->cmd.sm_mbms_deact_ind.mbms_config_options = cur_pdp->mbms_prot_cnfg;

          cur_pdp->cm_pending_msg = UNKNOWN_CMD;

#ifdef FEATURE_DUAL_SIM
          msm_cmd->cmd.sm_mbms_deact_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

          msm_cmd->cmd_hdr.cmd_id = MSM_SM_DEACTIVATION_IND;
        ...
        case SMREG_MBMS_ACTIVATE_CNF:
          /* ---------------------------------------------------------------
          *    Sending SMREG_MBMS_ACTIVATE_CNF to CM
          * ----------------------------------------------------------------*/

            MSG_HIGH_DS_1(SM_SUB,"MBMS: Sending SMREG_MBMS_ACTIVATE_CNF to MSM %d",cur_pdp->trans_id);
          msm_cmd->cmd.sm_mbms_act_conf.connection_id = cur_pdp->trans_id;

          // Pack the NSAPI
          msm_cmd->cmd.sm_mbms_act_conf.nsapi = cur_pdp->nsapi;

          // Pack Multicase address
          msm_cmd->cmd.sm_mbms_act_conf.multicast_addr = cur_pdp->mip;

          // Pack MBMS protocol config options
          msm_cmd->cmd.sm_mbms_act_conf.mbms_config_options = cur_pdp->mbms_prot_cnfg;

          // Pack TMGI
          msm_cmd->cmd.sm_mbms_act_conf.tmgi = cur_pdp->tmgi;
...
#ifdef FEATURE_DUAL_SIM
          msm_cmd->cmd.sm_mbms_act_conf.as_id= (sys_modem_as_id_e_type)sm_as_id;
#endif

          msm_cmd->cmd_hdr.cmd_id = MSM_SM_ACTIVATION_CNF;

        break;

        case SMREG_MBMS_ACTIVATE_IND:
          /* ---------------------------------------------------------------
           *    Sending SMREG_MBMS_ACTIVATE_IND to CM
           * ----------------------------------------------------------------*/
            MSG_HIGH_DS_1(SM_SUB,"MBMS: Sending SMREG_MBMS_ACTIVATE_IND to MSM %d",cur_pdp->trans_id);
          msm_cmd->cmd.sm_mbms_act_ind.connection_id = cur_pdp->trans_id;

          // Pack Multicast address
          msm_cmd->cmd.sm_mbms_act_ind.multicast_addr = cur_pdp->mip;

          // Pack the APN
          msm_cmd->cmd.sm_mbms_act_ind.apn_name = cur_pdp->apn_name;

          // Pack MBMS Protocol Configuration Optoins
          msm_cmd->cmd.sm_mbms_act_ind.mbms_config_options = cur_pdp->mbms_prot_cnfg;

          msm_cmd->cmd.sm_mbms_act_ind.nsapi = cur_pdp->nsapi;
          msm_cmd->cmd.sm_mbms_act_ind.linked_ti = cur_pdp->linked_ti;
...
#ifdef FEATURE_DUAL_SIM
          msm_cmd->cmd.sm_mbms_act_ind.as_id= (sys_modem_as_id_e_type)sm_as_id;
#endif

          // Set the CM report header and the command
          msm_cmd->cmd_hdr.cmd_id = MSM_SM_ACTIVATION_IND;

        break;

        case SMREG_MBMS_ACTIVATE_REJ:
          /* ---------------------------------------------------------------
           *    Sending SMREG_MBMS_ACTIVATE_REJ to CM
           * ----------------------------------------------------------------*/
          msm_cmd->cmd.sm_mbms_act_rej.connection_id = cur_pdp->trans_id;

          // Pack the NSAPI
          msm_cmd->cmd.sm_mbms_act_rej.nsapi = cur_pdp->nsapi;

          // Pack Multicase address
          msm_cmd->cmd.sm_mbms_act_rej.multicast_addr = cur_pdp->mip;

          // Pack the Cause based on if its internal or external
          msm_cmd->cmd.sm_mbms_act_rej.cause_type= cause_type;

          if (cause_type == SM_INTERNAL_CAUSE)
          {
            msm_cmd->cmd.sm_mbms_act_rej.cause.int_cause = (sm_cm_status_T)cur_pdp->current_cause;
          }
          else
          {
            msm_cmd->cmd.sm_mbms_act_rej.cause.ext_cause = (sm_status_T)cur_pdp->current_cause;
          }

          // Pack Protocol Configurations
          msm_cmd->cmd.sm_mbms_act_rej.mbms_config_options = cur_pdp->mbms_prot_cnfg;
...
#ifdef FEATURE_DUAL_SIM
          msm_cmd->cmd.sm_mbms_act_rej.as_id= (sys_modem_as_id_e_type)sm_as_id;
#endif
...
void sm_send_cm_ps_sig_rel_confirm(ps_sig_rel_status_T status_t)
{
  ...
  * ----------------------------------------------------------------*/    
  ...
  if(cm_rpt!=NULL)
  {
    ...
    mm_add_message_to_debug_buffer(MS_CM_SM,
                          (dword)cm_rpt->hdr.cmd,
                           FROM_MM,
...
DESCRIPTION
This function processes the MBMS context activate request.
...
before attempting to send the L3 message to the network.
===========================================================================*/
void sm_mbms_activate
(
 byte con_id,
 const smreg_mbms_activate_req_T *mbms_act
 )
{
  pdp_context_info *cur_pdp; // current pdp context in the transaction
  byte linked_ti;
...
#ifdef FEATURE_DUAL_SIM 
#ifndef FEATURE_DUAL_DATA
 if(SM_IS_VALID_AS_ID(mbms_act->as_id) == FALSE)
 {
    cur_pdp->current_cause = (cc_cause_T)SM_NO_RADIO_AVAILABLE;
    sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
    return ;
...
// MBMS_PENDING very CRITICAL
  for (i = 0 ; i < MAX_PDP_NUM ; i++)
  {
      if (pdp_list[i] != NULL && 
          pdp_list[i]->trans_id != INVALID_ID && 
          pdp_list[i]->mip.addr.v4 == mbms_act->multicast_addr.addr.v4) 
      {
        con_id = (byte) i;
        break;
      }
  ...
  if (i == MAX_PDP_NUM)
  {
    MSG_HIGH_DS_1( SM_SUB,"MBMS: conid for mip=%d not found",
      mbms_act->multicast_addr.addr.v4);
    return;
  }

  MSG_HIGH_DS_2( SM_SUB,"MBMS: MS MBMS activation conid=%d, mip=%d",
    con_id,mbms_act->multicast_addr.addr.v4);
  ...
  if(cur_pdp == NULL)
  {
    MSG_ERROR_DS_1( SM_SUB,"PDP is NULL, Connection id %d is unreserved", con_id);
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm_ti_cause (con_id, INVALID_CONNECTIN_ID, SMREG_MBMS_ACTIVATE_REJ);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    return ;
  }

  linked_ti = mbms_act->linked_ti;
  /* Set the transaction Id same as the Connection Id */
  cur_pdp->trans_id = con_id;
  ...
  /* ----------------------------------------------------------------------
   ** Reject if the MBMS context is already active for the given con_id.
   ** Also the internal reject cause is set properly
   ** --------------------------------------------------------------------*/
  if (   (! (con_id >= MIN_MT_TI && con_id <= MAX_MT_TI))     ||
    ( !cur_pdp->mt_orig_type )  ||
    cur_pdp->pdp_state != PDP_INACTIVE) // later change the original code in pdp activate req
  {
    MSG_ERROR_DS_2(SM_SUB,"MBMS: Reject MBMS Activate Req, TI=%d, mbms state=%d", con_id,cur_pdp->pdp_state);
    cur_pdp->current_cause = (cc_cause_T)INVALID_CONNECTIN_ID;
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
  }
  else if( ( linked_ti == INVALID_ID || pdp_list[linked_ti] == NULL) || 
    ( pdp_list[linked_ti] != NULL&& !(pdp_list[linked_ti]->pdp_state == PDP_ACTIVE || pdp_list[linked_ti]->pdp_state == PDP_MODIFY_PENDING)) )
  {
    MSG_ERROR_DS_2(SM_SUB,"MBMS: Reject MBMS Activate Req, Linked TI=%d, PDP state=%d",linked_ti, pdp_list[linked_ti]->pdp_state);

    cur_pdp->current_cause = (cc_cause_T)SM_MESG_TYPE_NOT_COMPATIBLE; // MBMS_PENDING
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
  }
  ...
  {
    /* -----------------------------------------------------------------------
     ** Set the MBMS parameters for the MBMS context based on the Connection Id.
     ** ---------------------------------------------------------------------*/
    // later check this nsapi assignment
    if (cur_pdp->nsapi.valid)
      nsapi = cur_pdp->nsapi.nsapi;
    else
      nsapi = sm_per_subs_reserve_nsapi (con_id,(sys_modem_as_id_e_type)sm_as_id);
    ...
    MSG_HIGH_DS_1(SM_SUB,"MBMS: CM MBMS Activate Request for NSAPI = %d", cur_pdp->nsapi.nsapi);

    //set the pdp as primary PDP context
    cur_pdp->is_primary_pdp = FALSE;

    // Set the RABM response state for this MBMS context to RABM_INITIAL.
    cur_pdp->rab_state = RABM_INITIAL;

    /* set the current pending CM message */
    cur_pdp->cm_pending_msg = (int)MSM_SM_ACTIVATION_REQ;

    cur_pdp->group_id = pdp_list[linked_ti]->group_id; // later how the group ID is set
    ...
    /* Set the APN Name */
    cur_pdp->apn_name = mbms_act->apn_name;

    /* Set the MBMS multicast address */ //MBMS_PENDING very CRITICAL
    //memcpy(&cur_pdp->pdp_addr, &mbms_act->multicast_addr, sizeof(sys_ip_address_s_type));

    /*Set the MBMS multicast addr alloc type*/
    cur_pdp->pdp_addr_alloc_type = (cur_pdp->pdp_addr.pdp_addr_len == 2)
      ? PDP_ADDR_STATIC : PDP_ADDR_DYNAMIC;

    /* Set the MBMS Protocol configuration */
    cur_pdp->mbms_prot_cnfg = mbms_act->mbms_config_options;

    /* Set the MBMS Bearer Capability */
    cur_pdp->mbms_bear_cap = mbms_act->mbms_bearer_cap;

    if ( cur_pdp->nsapi.valid == FALSE )
    {
      //If NSAPIs exceeds max then reject the PDP context
      MSG_ERROR_DS_1(SM_SUB,"MBMS: NSAPI not available, Reject the MBMS ACTIVATE req TI=%d",con_id);
      cur_pdp->current_cause = (cc_cause_T)INVALID_NSAPI;
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    ...
    {
#ifdef FEATURE_DUAL_SIM
      cur_pdp->as_id = mbms_act->as_id;
#endif
      sm_continue_mbms_activation(cur_pdp);
    }
  }
}
...
DESCRIPTION
This function processes MBMS context activation after receiving the
a successful GPRS attach indication from GMM.
...
===========================================================================*/
void sm_continue_mbms_activation(pdp_context_info* pdp)
{
  word mesg_bit_len;              /* Length of the packed L3 mesg in bits */
  xlate_status_T status;          /* status of the packed L3 message */
  ...
  if (!sig_connection_available)
  {
    MSG_HIGH_DS_0( SM_SUB,"MBMS: Send GMMSM_ESTABLISH_REQ, REQ CONN ");
    // Change the PDP SIG connect state to SIG_CONNECTION_PENDING
    pdp->pdp_con_state = SIG_CONNECTION_PENDING;
    /* ------------------------------------------------------------------
     ** Set T3380 timer and put connection Id into the timers_pending.
    ...
    if ( !timer_est_pen_flag )
    {
      ...
      send_gmm_establish_req(
#ifdef FEATURE_WCDMA
      convert_to_rrc_est_cause(pdp->qos.traffic_class)
#endif
);
    }
    else
  ...
  else
  {
    ...
    /* Pack the Layer 3 Message and send it to GMM */
    status = pack_layer3_mesg(&mesg_bit_len, l3_buf, pdp, ACTIVATE_MBMS_REQUEST);

    if (status == L3_PARSE_SUCCESS)
    {
      sm_start_timer(pdp->trans_id, TIMER_ACTIVE_PENDING);
      /* Change the PDP context state */
      pdp->pdp_state = MBMS_ACTIVE_PENDING;
      MSG_HIGH_DS_0(SM_SUB,"MBMS: state change to MBMS_ACTIVE_PENDING");
    ...
    {
      MSG_ERROR_DS_0( SM_SUB,"MBMS: Unable to Encode ACTIVATE_MBMS_REQUEST (OTA) mesg, INVALID field");
      pdp->current_cause = (cc_cause_T)INVALID_FIELD;
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      sm_send_cm_confirm(pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    }
    if(l3_buf != NULL)
    {
...
DESCRIPTION
This function sends a reject message for a Mobile Terminated MBMS Activation
request.
...
// MBMS_PENDING very CRITICAL
  for (i = 0 ; i < MAX_PDP_NUM ; i++)
  {
      if (pdp_list[i] != NULL && pdp_list[i]->mip.addr.v4 == rej_rsp->multicast_addr.addr.v4)
      {
        con_id = (byte) i;
        break;
      }
  ...
  {
    MSG_HIGH_DS_1( SM_SUB,"MBMS: conid for mip=%d not found",
      rej_rsp->multicast_addr.addr.v4);
    return;
  }

  MSG_HIGH_DS_2( SM_SUB,"MBMS: MS Reject to Network MBMS activation conid=%d, mip=%d",
    con_id,rej_rsp->multicast_addr.addr.v4);

  /*Verify the con id falls in the MT PDP range*/
  if (con_id < MAX_MO_TI )
  {
    MSG_ERROR_DS_1( SM_SUB,"ConId out of range: %d, Not a valid MT call", con_id);
    return;
  }
  ...
  //Set the CM pending message to MSM_SM_ACTIVATE_REJ_RSP
  cur_pdp->cm_pending_msg = (int)MSM_SM_ACTIVATE_REJ_RSP;

  sm_continue_mbms_activate_rej_rsp (cur_pdp);
...
DESCRIPTION
This function continues the request MBMS activation reject response procedure.
It packs the Layer 3 message in to the OTA format and sends the
SM-PDU to the GMM to be sent to the Network.
...
SIDE EFFECTS
===========================================================================*/
void sm_continue_mbms_activate_rej_rsp(pdp_context_info *cur_pdp)
{
  xlate_status_T status;
  word mesg_bit_len;
  byte *l3_buf ;
  ...
  else
  {
    ...
    /* Pack the Layer 3 Message and send it to GMM */
    status = pack_layer3_mesg(&mesg_bit_len, l3_buf, cur_pdp, REQUEST_MBMS_ACTIVATION_REJ);

    if (status == L3_PARSE_SUCCESS)
    {
      MSG_HIGH_DS_0( SM_SUB,"MBMS: Send REQUEST_MBMS_ACTIVATION_REJ  OTA");
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      sm_send_cm_confirm(cur_pdp, SMREG_PDP_DEACTIVATE_CNF,SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
      /*
      if (cur_pdp->nsapi.valid == TRUE)
      {
         (void)sm_release_nsapi (cur_pdp->trans_id);
      }
      sm_init_pdp_context (cur_pdp);
      */
      // Send the packed SM-PDU to the GMM to be sent to the Network
      send_gmm_smpdu(mesg_bit_len, l3_buf, FALSE,FALSE);
    }
    ...
    {
      MSG_ERROR_DS_0( SM_SUB,"MBMS: Unable to Encode OTA mesg, INVALID field");
      cur_pdp->current_cause = (cc_cause_T)INVALID_FIELD;
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
...
** DESCRIPTION  : This function processes MBMS abort request by CM.
     ...
     RETURN VALUE
      ...
      SIDE EFFECTS
** ----------------------------------------------------------------------- */
void sm_mbms_abort (const smreg_mbms_abort_req_T* mbms_abort)
{
  int i, k;
  msm_ext_cmd_type *msm_cmd; //MBMS_PENDING
#ifdef FEATURE_DUAL_SIM
  boolean process_mbms_abort = TRUE;
#endif


  MSG_HIGH_DS_1( SM_SUB,"MBMS: CM Request to abort mbms one ctxt(0)/(>1) %d", mbms_abort->length);

#ifdef FEATURE_DUAL_SIM
#ifdef FEATURE_DUAL_DATA
    if(SM_IS_VALID_AS_ID(mbms_abort->as_id) == FALSE)
    {
      MSG_ERROR_DS_2( SM_SUB,"SM AS ID Mismatch sm_as_id : %d, message as_id: %d", sm_as_id , mbms_abort->as_id);
      process_mbms_abort = FALSE;
    }
#endif
#endif

  #ifdef FEATURE_DUAL_SIM
  if (process_mbms_abort == TRUE)
  {
#endif
    if (mbms_abort->length == 0)
    {
      for (i = 0 ; i < MAX_PDP_NUM; i++)
      {
        if (pdp_list[i] != NULL && 
            pdp_list[i]->trans_id != INVALID_ID  && 
            pdp_list[i]->is_mbms == TRUE)
        {
          sm_perform_local_pdp_deactivation(pdp_list[i]);
        }
      }
    }
    else
    {
      for (i = 0 ; i < mbms_abort->length ; i++)
      {
        for (k = 0 ; k < MAX_PDP_NUM; k++)
        {
          if ( pdp_list[k] != NULL && 
             (pdp_list[k]->is_mbms == TRUE ) && 
             (pdp_list[k]->mip.addr.v4 == mbms_abort->multicast_addr[i].addr.v4) )
          {
            sm_perform_local_pdp_deactivation(pdp_list[k]);
          }
        }
      }
    }
  ...
  if (msm_cmd != NULL) 
  {
    msm_cmd->cmd_hdr.cmd_id = MSM_SM_ABORT_CNF;
    memscpy(&msm_cmd->cmd.sm_mbms_abort_cnf, sizeof(smreg_mbms_abort_cnf_T), mbms_abort,
             sizeof(smreg_mbms_abort_req_T));
  }
...
//#endif
  ...
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
   ...
   #ifdef FEATURE_MBMS
               sm_deact_associated_mbms_ctxt(pdp_list[sec_index]);   
   #endif       
  ...
  ** ---------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (status == L3_PARSE_SUCCESS)
    {
      ...
      if (is_nw_initiated_qos_support == TRUE)
	  {
        // Do not need to update REQ Qos if NW initiated QoS is not supported,
        // we must have updated it already while processing request from NW
...
DESCRIPTION
 This function processes request from DS to restore a primary/secondary
 PDP upon wake up from PSM that existed in the previous GPRS session before
 entering PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
void sm_cm_process_restore_pdp_ctxt_req
(
  mm_restore_pdp_ctxt_req_s_type *restore_pdp
)
{
...
#ifdef FEATURE_DUAL_SIM
#ifndef FEATURE_DUAL_DATA
  if ((SM_IS_VALID_AS_ID(restore_pdp->as_id)==FALSE))
  {
    MSG_ERROR_DS_2( SM_SUB,"SM AS ID Mismatch sm_as_id : %d, message as_id: %d", sm_as_id , restore_pdp->as_id);
    cur_pdp->current_cause = (cc_cause_T)SM_NO_RADIO_AVAILABLE;
    /* Might send failure response in future revisions */
    //sm_send_cm_confirm_ti_cause (con_id, INVALID_CONNECTIN_ID, SMREG_RESTORE_PDP_CTXT_RSP);
    return ;
  }
#endif
#endif
  ...
  if (sm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(sm_get_ulog_handle(), "=SM= received SMREG_RESTORE_PDP_CTXT_REQ from CM");
  }
  
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if(restore_pdp->nsapi.nsapi == (sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi))
    {
      MSG_HIGH_DS_2(SM_SUB, "PSM NSAPi %d Conn ID %d", sm_psm_cache_ptr->gsm.psm_pdp[index].nsapi, sm_psm_cache_ptr->gsm.psm_pdp[index].connection_id);
      con_id = sm_psm_cache_ptr->gsm.psm_pdp[index].connection_id;
      break;
    }
  }

  MSG_HIGH_DS_1(SM_SUB, "restore_pdp->nsapi.nsapi = %d ",restore_pdp->nsapi.nsapi );
  /* ----------------------------------------------------------------------
  ** Reject if the PDP context is already active for the given con_id.
  ** Also the internal reject cause is set properly 
  ** --------------------------------------------------------------------*/
  if ((con_id >= MAX_PDP_NUM) || (pdp_list[con_id] == NULL && sm_get_connection_id_int (con_id) == FALSE ))
  {
    MSG_ERROR_DS_1( SM_SUB,"PDP is NULL/ INVALID CON_ID, Connection id %d is not reserved", con_id);
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    /* Might send failure response in future revisions */
    //sm_send_cm_confirm_ti_cause (con_id, INVALID_CONNECTIN_ID, SMREG_RESTORE_PDP_CTXT_RSP);
#endif
       ...
       cur_pdp->pdp_state != PDP_INACTIVE)
  {
    ...
    cur_pdp->current_cause = (cc_cause_T)INVALID_CONNECTIN_ID;
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_RESTORE_PDP_CTXT_RSP, SM_INTERNAL_CAUSE);
#endif
    return ;
  }

#ifdef FEATURE_SECONDARY_PDP_PROMOTION_REMOVAL
  pri_pdp_index = sm_get_linked_ti(restore_pdp->sm_group_id);
#else
  pri_pdp_index = sm_get_primary_index(restore_pdp->sm_group_id);
#endif

  MSG_HIGH_DS_4(SM_SUB, "Restore PDP NSAPI = %d Primary PDP Index = %d, Radio Priority = %d Packet Flow ID = %d", 
                                                                   restore_pdp->nsapi.nsapi,
                                                                   pri_pdp_index, 
                                                                   sm_psm_cache_ptr->gsm.psm_pdp[index].radio_pri.radio_priority,
                                                                   sm_psm_cache_ptr->gsm.psm_pdp[index].packet_flow_id.packet_flow_id );


   /* For a secondary PDP context restore request, there must be a corresponding
          linked primary PDP context already present  */

  if ((restore_pdp->is_primary_pdp == FALSE) &&
      (pri_pdp_index == INVALID_ID 
#ifndef FEATURE_SECONDARY_PDP_PROMOTION_REMOVAL
       || pdp_list[pri_pdp_index] == NULL || 
       pdp_list[pri_pdp_index]->pdp_state == PDP_INACTIVE
#endif
  ...
  {
     MSG_ERROR_DS_1( SM_SUB,"Invalid Secondary PDP Restore Request by CM for ConId %d", con_id);
  }
  else 
  {
       /* set the NSAPI for this PDP */
       if ((con_id < MIN_MT_TI) &&
           (restore_pdp->nsapi.valid == TRUE))
       {
         /* set the NSAPI for this PDP */
         cur_pdp->nsapi = restore_pdp->nsapi;
         sm_reserve_psm_restored_nsapi(con_id, cur_pdp->nsapi.nsapi, (sys_modem_as_id_e_type)sm_as_id);

         /* set the RADIO Priority and Packet Flow Id for this PDP */
         cur_pdp->radio_pri = sm_psm_cache_ptr->gsm.psm_pdp[index].radio_pri;
         cur_pdp->flow_id   = sm_psm_cache_ptr->gsm.psm_pdp[index].packet_flow_id;

         /*set the pdp as primary PDP context */
         cur_pdp->is_primary_pdp = restore_pdp->is_primary_pdp;

         /* Set the RABM response state for this PDP context to RABM_INITIAL. */
         cur_pdp->rab_state      = RABM_INITIAL;
          cur_pdp->pdp_state     = PDP_ACTIVE;
         /*Generate group id*/
         cur_pdp->group_id       = restore_pdp->sm_group_id;

         /* set the LLC SAPI for this PDP */
         cur_pdp->llc_sapi = restore_pdp->llc_sapi;
         ...
         cur_pdp->qos         = restore_pdp->neg_qos; 
         cur_pdp->minimum_qos = restore_pdp->minimum_qos;
         cur_pdp->llc_sapi    = restore_pdp->llc_sapi;

         /* Set the APN Name */
         cur_pdp->apn_name    = restore_pdp->apn_name;

         /* Set the PDP address */
         cur_pdp->pdp_addr    = restore_pdp->pdp_addr;
...
-                 If the PDP type is IP, dynamic addressing is applicable.*/
         ...
         cur_pdp->pdp_addr_alloc_type = (restore_pdp->pdp_addr.pdp_addr_len )
                                   ? PDP_ADDR_STATIC : PDP_ADDR_DYNAMIC;

         /* Set the Compression Requirements */
         cur_pdp->h_comp      = restore_pdp->h_comp;
         cur_pdp->d_comp      = restore_pdp->d_comp;

         cur_pdp->bcm         = restore_pdp->bcm;
         cur_pdp->tft         = restore_pdp->tft;
         sm_send_cm_confirm(cur_pdp, SMREG_RESTORE_PDP_CTXT_RSP, SM_INTERNAL_CAUSE);
     }
  }
} /* end sm_cm_process_restore_pdp_ctxt_req() */
...
void sm_unblock_all_apn(sys_modem_as_id_e_type as_id)
{
  ...
  MSG_HIGH_DS_0( SM_SUB," Sending MM_UNBLOCK_ALL_APN_IND to DS ");

  if (ds_rpt!=NULL)
  {
    
#ifdef FEATURE_DUAL_SIM
      ds_rpt->cmd.mm_unblock_all_apn.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif

      ds_rpt->cmd.mm_unblock_all_apn.message_header.message_set = MS_MM_DS;
      ds_rpt->cmd.mm_unblock_all_apn.message_header.message_id = (int) MM_UNBLOCK_ALL_APN_IND;
      PUT_IMH_LEN( sizeof(mm_unblock_all_apn_ind_T) - sizeof(IMH_T),
                &ds_rpt->cmd.mm_unblock_all_apn.message_header );
    ...
    //Send cmd to DS
    ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.mm_unblock_all_apn);
  ...
  {
    MSG_ERROR_DS_0(SM_SUB,"DS Error: Not able to Allocate DS Buffer");
  }
...
void sm_check_for_null_ptr( void* msg_p)
{
   if(msg_p == NULL)
   {
     MSG_FATAL_DS(SM_SUB,"=SM= Error: NULL PTR check failed", 0, 0, 0);
   }
}

/*===========================================================================

FUNCTION SM_PROCESS_DS_CMD

DESCRIPTION

  This function processes the command received from the DS task. 
...
===========================================================================*/
void sm_process_ds_cmd(mm_cmd_type* cmd_ptr)
{
  // Process the Command sent by CM task 
  MSG_HIGH_DS_1( SM_SUB,"DS Command received for CMD_ID:0x%X", cmd_ptr->cmd.hdr.message_id );
  ...
  switch (cmd_ptr->cmd.hdr.message_id)
  {
    ...
    case MM_PDP_ACTIVATE_REQ :
      ...
      sm_pdp_activate(cmd_ptr->cmd.pdp_activate_req.connection_id, 
    ...
    case MM_PDP_DEACTIVATE_REQ:
      ...
      sm_pdp_deactivate (cmd_ptr->cmd.pdp_deactivate_req.connection_id, 
    ...
    case MM_PDP_MODIFY_REQ:
      ...
      sm_pdp_modify (cmd_ptr->cmd.pdp_modify_req.connection_id,
                     &cmd_ptr->cmd.pdp_modify_req);
      break;

    case MM_PDP_SEC_ACTIVATE_REQ:
      
      sm_sec_pdp_activate (cmd_ptr->cmd.pdp_sec_activate_req.connection_id, 
    ...
    case MM_PDP_ACTIVATE_REJ_RSP:
      ...
      sm_pdp_activate_rej_rsp (cmd_ptr->cmd.pdp_activate_rej_resp.connection_id, 
                           &cmd_ptr->cmd.pdp_activate_rej_resp);
      break;

#endif

#if 0 //Not needed for 9x05
...
#endif
      sm_pdp_nw_modify_cnf(cmd_ptr->cmd.pdp_nw_modify_cnf.connection_id);
      break;
...
#endif
      sm_pdp_nw_modify_rej(cmd_ptr->cmd.pdp_nw_modify_rej.connection_id, 
                                           &cmd_ptr->cmd.pdp_nw_modify_rej);
      break;

    case MM_RESTORE_PSM_CTXT_RSP:
      sm_cm_process_restore_ctxt_psm_rsp(&(cmd_ptr->cmd.mm_restore_psm_ctxt_rsp));
      break;

    case MM_RESTORE_PDP_CTXT_REQ:
      sm_cm_process_restore_pdp_ctxt_req(&(cmd_ptr->cmd.mm_restore_pdp_ctxt_req));
      break;

    /*case SMREG_BLOCKED_APN_LIST:

      sm_process_blocked_apn_list(&(cmd_ptr->cmd.blocked_apn_list_ind));
    break;*/    //RAVI_9x05_CHECK

    case MMCM_CALL_CTRL_RSP:
        sm_cm_process_call_control_rsp(&(cmd_ptr->cmd.cm_mm_call_ctrl_rsp));
        break;

#endif 

    default:
      MSG_ERROR_DS_1(SM_SUB," UNKNOWN cmd from CM : %d", cmd_ptr->cmd.hdr.message_id);
      break;
  };
}
...
void sm_process_cm_cmd(sm_cmd_type* cmd_ptr)
{
  ...
  switch (cmd_ptr->header.cmd_id)
  {
    ...
    case MSM_SM_ACTIVATION_REQ:
       #ifdef FEATURE_DUAL_DATA
        if(SM_IS_VALID_AS_ID(cmd_ptr->cmd.mbms_act_req.as_id) == FALSE)
        { 
          MSG_HIGH_DS_2( SM_SUB,"SM AS ID Conflict sm_as_id : %d, message as_id: %d", sm_as_id , cmd_ptr->cmd.mbms_act_req.as_id);
          return;
        }
      #endif
      sm_mbms_activate(cmd_ptr->header.connection_id, &cmd_ptr->cmd.mbms_act_req);
    break;

    case MSM_SM_ACTIVATE_REJ_RSP:

      #ifdef FEATURE_DUAL_DATA
      if(SM_IS_VALID_AS_ID(cmd_ptr->cmd.mbms_act_rej_resp.as_id) == FALSE) 
      { 
          return;
       }
      #endif
      sm_mbms_activate_rej_rsp (cmd_ptr->header.connection_id,
                           &cmd_ptr->cmd.mbms_act_rej_resp);
      break;

      case MSM_SM_ABORT_REQ:
      #ifdef FEATURE_DUAL_DATA
       if(SM_IS_VALID_AS_ID(cmd_ptr->cmd.mbms_abort_req.as_id) == FALSE)
       { 
          return;
       }
      #endif
      sm_mbms_abort (&cmd_ptr->cmd.mbms_abort_req);
      break;
    ...
    case SMREG_RESTORE_CTXT_PSM_RSP:
      sm_cm_process_restore_ctxt_psm_rsp(&(cmd_ptr->cmd.restore_psm_ctxt_rsp));
      break;

    case SMREG_RESTORE_PDP_CTXT_REQ:
      sm_cm_process_restore_pdp_ctxt_req(&(cmd_ptr->cmd.restore_pdp_ctxt_req));
      break;
#endif 
    case SMREG_BLOCKED_APN_LIST:
