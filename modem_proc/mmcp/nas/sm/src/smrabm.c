...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smrabm.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
  ...
  switch (cmd_ptr->header.cmd_id)
  {
    ...
    case  RABMSM_ACTIVATE_RSP:    
      ...
      ** -----------------------------------------------------------------*/
      ...
      if ((con_id == INVALID_ID) || !(con_id >= MIN_MO_TI && con_id < MAX_PDP_NUM))
      {
      ...
      ** ------------------------------------------------------------------*/
      ...
      else if (cur_pdp->pdp_state == MBMS_ACTIVE)
      {
        /* Clear RABM Response sanity timer */
        sm_stop_timer(cur_pdp->trans_id); 
#ifdef FEATURE_WCDMA
