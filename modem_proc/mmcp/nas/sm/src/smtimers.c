...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smtimers.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void sm_start_timer( word timer_id, sm_timer_T sm_timer_type )
{
  ...
  {
   #ifdef FEATURE_DUAL_DATA
       status = gs_start_timer( GS_TIMER_QUEUE_SM, (byte) ( (timer_id&0x0f)| ((0x01 & sm_as_id)<<4)), sm_timer_expiry, num_millisecs, (byte) sm_timer_type );
   #else
       status = gs_start_timer( GS_TIMER_QUEUE_SM, (byte) (timer_id&0x0f), sm_timer_expiry, num_millisecs, (byte) sm_timer_type );
   #endif
...
void sm_stop_timer( word timer_id )
{
   ...
   else if (sm_timer_status[timer_id] != (word)SM_TIMER_STOPPED)
   {
      MSG_HIGH_DS_1(SM_SUB, "SM stopping %u", timer_id);
       ...
       gs_status = gs_cancel_timer( GS_TIMER_QUEUE_SM, (timer_id_T) ( (timer_id&0x0f)| ((0x01 & sm_as_id)<<4)) );
   ...
   #else
      gs_status = gs_cancel_timer( GS_TIMER_QUEUE_SM, (timer_id_T) timer_id );
...
void sm_process_timer_expiry_cmd ( sm_cmd_type *cmd_ptr )
{
  ...
  { 
    ...
    gs_clear_timer(GS_TIMER_QUEUE_SM, cmd_ptr->cmd.timer_expiry.timer_id);
