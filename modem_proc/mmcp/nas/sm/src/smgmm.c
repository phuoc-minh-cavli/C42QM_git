...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smgmm.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
extern boolean sm_pdp_sig_in_progress_last_sent_sim[MAX_AS_IDS];
#define sm_pdp_sig_in_progress_last_sent sm_pdp_sig_in_progress_last_sent_sim[sm_as_id]

#else
extern uint32 sm_tlli;
extern boolean sm_pdp_sig_in_progress_last_sent;
...
/* -----------------------------------------------------------------------
** FUNCTION     : SM_PROC_REQUEST_MBMS_ACTIVATION()

** DESCRIPTION  : processes MT, REQUEST_MBMS_ACTIVATION SM-SM message.
** ----------------------------------------------------------------------- */
LOCAL void sm_proc_request_mbms_activation (pdp_context_info *cur_pdp);


/* -----------------------------------------------------------------------
** FUNCTION     : SM_PROC_ACTIVATE_MBMS_ACCEPT()

** DESCRIPTION  : This function processes ACTIVATE_MBMS_ACCEPT SM-SM message.
** ----------------------------------------------------------------------- */
LOCAL void sm_proc_activate_mbms_accept(pdp_context_info *cur_pdp);

/* -----------------------------------------------------------------------
** FUNCTION     : SM_PROC_ACTIVATE_MBMS_REJECT()

** DESCRIPTION  : This function processes ACTIVATE_MBMS_REJECT SM-SM message.
** ----------------------------------------------------------------------- */
LOCAL void sm_proc_activate_mbms_reject(pdp_context_info *cur_pdp);
    ...
    for (list_index = 0; list_index < GMM_RECEIVE_NPDU_NUMBER_LIST_MAX_NSAPIS; list_index++ )
    {
      if ( valid_list_p[list_index] )
      {
        ...
        npdu_num_list_p[list_index] =
          (uint8)gsndcp_snsm_seq_ind( sm_tlli, (uint8)(list_index + MIN_NSAPI_NUM), npdu_num_list_p[list_index]
#ifdef FEATURE_DUAL_SIM
                                      ,as_id
#else
...
void sm_process_gmm_cmd(sm_cmd_type* cmd_ptr)
{
  ...
  switch (cmd_ptr->header.cmd_id)
  {
   ...
   case GMMSM_SAVE_CTXT_PSM_REQ:
      sm_process_save_ctxt_psm_req(cmd_ptr);
      break;

   case GMMSM_RESTORE_CTXT_PSM_REQ:
      sm_process_restore_ctxt_psm_req(cmd_ptr);
      break;
...
void send_gmm_ok_to_tuneaway_ind(void)
{
  ...
  else
  {
    ...
    if (gmm_cmd != NULL)
    {
      send_gmm_ps_call_status_ind((boolean)!(current_ok_for_tuneaway_status));
...
void send_gmm_smpdu(word mesg_len, byte* l3_buf, boolean pdp_inactive_flag, boolean ps_vote_flag)
{
  ...
  if (gmm_cmd != NULL)
  {
    ...
    //mm_put_cmd (gmm_cmd);
    gmm_handle_connection_management( gmm_cmd );

#ifndef FEATURE_MODEM_HEAP
   gs_free(gmm_cmd);
#else
   modem_mem_free(gmm_cmd, MODEM_MEM_CLIENT_NAS);
#endif
  }
  else
  {
...
===========================================================================*/
void send_gmm_ps_call_status_ind (boolean pdp_sig_in_progress)
{
  if (sm_pdp_sig_in_progress_last_sent != pdp_sig_in_progress)
  {
    mm_cmd_type * gmm_cmd;
    gmm_cmd = mm_get_cmd_buf();
    if (gmm_cmd != NULL)
    {
      sm_pdp_sig_in_progress_last_sent = pdp_sig_in_progress;
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.header.message_set     = MS_GMM_SM;
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.header.message_id      = GMMSM_PS_CALL_STATUS_IND;
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.header.message_len_lsb = 0;
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.header.message_len_msb = 0;  
#ifdef FEATURE_DUAL_SIM
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.as_id = (sys_modem_as_id_e_type)sm_as_id;
#endif
      gmm_cmd->cmd.gmmsm_ps_call_status_ind.pdp_sig_in_progress = pdp_sig_in_progress;

      MSG_HIGH_DS_1( SM_SUB,"Sending PS CALL STATUS IND to GMM pdp_sig_in_progress %d", pdp_sig_in_progress);
      //mm_put_cmd (gmm_cmd);
      gmm_handle_connection_management( gmm_cmd );
#ifndef FEATURE_MODEM_HEAP
   gs_free(gmm_cmd);
  ...
  ** -------------------------------------------------------------------*/
  for (i=0; i<MAX_PDP_NUM; i++)
  {
    if (pdp_list[i] != NULL)
    {
      ...
      #ifdef FEATURE_MBMS
        else if (pending_msg == (int)MSM_SM_ACTIVATION_REQ )
        {
        MSG_MED_DS_1(SM_SUB,"MBMS: nsapi# %d being activated", pdp_list[i]->nsapi.nsapi);
          sm_continue_mbms_activation(pdp_list[i]);
        }
        else if (pending_msg == (int)MSM_SM_ACTIVATE_REJ_RSP)
        {
        MSG_MED_DS_1(SM_SUB,"MBMS: Network initiated MBMS request for %d being rejected", pdp_list[i]->trans_id);
          sm_continue_mbms_activate_rej_rsp (pdp_list[i]);
        }
        #endif
...
{                            
  ...
  sm_pdp_sig_in_progress_last_sent = FALSE;
...
void sm_process_sm_pdu(gmmsm_unitdata_ind_T* sm_pdu)
{
  ...
  mm_pdp_activate_rej_resp_T *rej_resp = NULL;
#ifdef FEATURE_MBMS
  smreg_mbms_activate_rej_resp_T *mbms_rej_resp = NULL;
#endif
...
//   word stored_trans_id;
  ...
  if(sm_pdu->message_len < 2 )
  {
    MSG_ERROR_DS_0(SM_SUB,"Invalid msg length to get PD or Extended TI received in the message");
    return;	
  }
  ...
  if(sm_pdu->message_len < (data_index + 1) )
  {
	MSG_ERROR_DS_0(SM_SUB,"Invalid msg length to get msg type in the message");  
	return;
  }
...
#ifdef FEATURE_MBMS
#ifdef FEATURE_MODEM_HEAP
  mbms_rej_resp = modem_mem_calloc(1,sizeof(smreg_mbms_activate_rej_resp_T),MODEM_MEM_CLIENT_NAS);
#else
  mbms_rej_resp = (smreg_mbms_activate_rej_resp_T *)gs_alloc(sizeof(smreg_mbms_activate_rej_resp_T));
#endif
  if (mbms_rej_resp == NULL)
  {
    MSG_ERROR_DS_0(SM_SUB,"Memory Allocation failed");
    return;
  }
  ...
  #ifdef FEATURE_MBMS
  (void) memset(mbms_rej_resp,0,sizeof(smreg_mbms_activate_rej_resp_T));
  #endif
  ...
  if (status == L3_PARSE_SUCCESS)
  {
        ...
        (SM_IS_VALID_AS_ID(sm_pdu->as_id) == FALSE) )
    {
      ...
      {
#ifdef FEATURE_MODEM_HEAP
         modem_mem_free(mbms_rej_resp,MODEM_MEM_CLIENT_NAS);
#else
         gs_free(mbms_rej_resp);
#endif    
         mbms_rej_resp = NULL;
      }
#endif
    ...
    if (recd_msg == REQUEST_PDP_ACTIVATION
        #ifdef FEATURE_MBMS
        || recd_msg == REQUEST_MBMS_ACTIVATION
        #endif
...
//#endif /*FEATURE_DISABLE_MT_PDP*/
          ...
          case REQUEST_MBMS_ACTIVATION:
            ...
            if (sm_validate_dupliacte_collision_pdp(req_pdp_cxt))
            {
              ...
              else
              {
                ...
                cur_pdp->mip = req_pdp_cxt->mip;
  
                /* Copy the MBMS Protocol Config Options*/
                cur_pdp->mbms_prot_cnfg = req_pdp_cxt->mbms_prot_cnfg;
  
                /*Copy the Allocation type*/
                cur_pdp->pdp_addr_alloc_type = req_pdp_cxt->pdp_addr_alloc_type;
  
                /*Copy the Linked TI*/
                cur_pdp->linked_ti= req_pdp_cxt->linked_ti;
  
                /*Copy the Group ID*/
                // cur_pdp->group_id= pdp_list[cur_pdp->linked_ti].group_id; // MBMS_PENDING
  
                sm_proc_request_mbms_activation(cur_pdp);
              }
            }
            break;

          case ACTIVATE_MBMS_ACCEPT:
            MSG_HIGH_DS_0( SM_SUB,"MBMS: Received ACTIVATE_MBMS_ACCEPT");
            sm_proc_activate_mbms_accept(cur_pdp);
            break;

          case ACTIVATE_MBMS_REJECT:
            
            sm_proc_activate_mbms_reject(cur_pdp);
            break;
      ...
      else
      {
          ...
          else
          {
             ...
             ** -------------------------------------------------------------------*/
            ...
            #ifdef FEATURE_MBMS
             else if (recd_msg == REQUEST_MBMS_ACTIVATION)
             {
              MSG_HIGH_DS_1(SM_SUB,"MBMS: Invalid mandatory info in REQUEST_MBMS_ACTIVATION for ti %d",
                          req_pdp_cxt->trans_id);
               mbms_rej_resp->cause = SM_INVALID_MANDATORY_INFO;
               //memcpy(&(mbms_rej_resp.nsapi), &(pdp_list[req_pdp_cxt.linked_ti].nsapi), sizeof (nsapi_T));
               //set the orig type to MT
               pdp_list[req_pdp_cxt->trans_id]->mt_orig_type = TRUE;
               pdp_list[req_pdp_cxt->trans_id]->is_mbms= TRUE;
               pdp_list[req_pdp_cxt->trans_id]->mip = req_pdp_cxt->mip;
               mbms_rej_resp->multicast_addr = req_pdp_cxt->mip;
               mbms_rej_resp->mbms_config_options = req_pdp_cxt->mbms_prot_cnfg;
               sm_mbms_activate_rej_rsp(req_pdp_cxt->trans_id, mbms_rej_resp);
             }
             #endif
             ...
             else
             {
               ...
               /* As per Rel-12 Spec CR: 2558; REQUEST SECONDARY PDP CONTEXT ACTIVATION REJECT message with 
                  cause # 96 "Invalid mandatory information" shall be returned */
               else if ((recd_msg == REQUEST_SEC_PDP_ACTIVATION) && (mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12) )
               {
                  MSG_HIGH_DS_1(SM_SUB, "Invalid mandatory info in REQUEST_SEC_PDP_ACTIVATION for TI: %d",cur_pdp->trans_id);
                  cur_pdp->config_options.valid = FALSE;
        ...
        ** -----------------------------------------------------------------*/
...
#endif /*FEATURE_GSM_GPRS*/        
        {
          ...
          )
          {
            ...
            {
              #ifdef FEATURE_MBMS
              sm_deact_associated_mbms_ctxt(pdp_list[i]);
              #endif
...
void sm_proc_request_pdp_activation (pdp_context_info *cur_pdp)
{
  ...
  ** ----------------------------------------------------------------------*/
      ...
      )
  {
    ...
    #ifdef FEATURE_MBMS
      if (cur_pdp->is_mbms == TRUE) // this field will be untouched during the unpacking of PDP Req msg,
      {
        MSG_HIGH_DS_1(SM_SUB, "MBMS: Network req PDP activation for Active MBMS TI=%d",cur_pdp->trans_id);
      }
    #endif
  ...
  #ifdef FEATURE_MBMS
  else if (cur_pdp->pdp_state == MBMS_ACTIVE_PENDING)
  {
    MSG_HIGH_DS_1(SM_SUB,"MBMS: MBMS activation is already pending on con id %d",cur_pdp->trans_id);
    saved_nsapi = cur_pdp->nsapi.nsapi;
    saved_ti = cur_pdp->trans_id;

    cur_pdp->current_cause = (cc_cause_T)PDP_MBMS_REQUEST_COLLISION; // just for now MBMS_PENDING
    //Send RABMSM_DEACTIVATE_IMPLICIT_IND to RABM
    sm_notify_rabm((int)RABMSM_DEACTIVATE_IMPLICIT_IND, cur_pdp->nsapi.nsapi); //MBMS_PENDING
...
void sm_proc_request_pdp_deactivation(pdp_context_info *cur_pdp)
{
       ...
       (cur_pdp->pdp_state != PDP_MODIFY_PENDING)) 
  {
    ...
    #ifdef FEATURE_MBMS
        MSG_ERROR_DS_1( SM_SUB,"Received REQ_PDP_DEACTIVATE for MBMS(1)/PDP(0)=%d",cur_pdp->is_mbms);
    #endif
  ...
  else if (cur_pdp->is_mbms == TRUE)
  {
    ...
    sm_stop_timer(cur_pdp->trans_id);
    /* Pack the Layer 3 Message and send it to GMM */
    status = pack_layer3_mesg(&mesg_bit_len, l3_buf, cur_pdp, DEACTIVATE_PDP_ACCEPT);
    if (status == L3_PARSE_SUCCESS)
    {
      // Send the packed SM-PDU to the GMM to be sent to the Network
      send_gmm_smpdu(mesg_bit_len, l3_buf, FALSE,FALSE);
...
void sm_proc_request_sec_pdp_activation (pdp_context_info *cur_pdp)
{
  ...
  #ifdef FEATURE_MODEM_HEAP
  l3_buf = (byte *)modem_mem_calloc(1,max_l3_message,MODEM_MEM_CLIENT_NAS);
...
#else
  l3_buf = (byte *)gs_alloc(max_l3_message);
  ...
  memset((void *)l3_buf,0,max_l3_message);
  ...
  {       
    ...
    if (cur_pdp->is_mbms == TRUE) // this field will be untouched during the unpacking of PDP Req msg,
    {
      MSG_HIGH_DS_1(SM_SUB, "MBMS: Network req Secondary PDP activation for Active MBMS TI=%d",cur_pdp->trans_id);
    }
#endif /*FEATURE_MBMS*/    
    ...
    {
      cur_pdp->current_cause = (cc_cause_T)PDP_MBMS_REQUEST_COLLISION;         
      // Send the SMREG_PDP_ACTIVATE_REJ to the CM.
      sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
    }
  #endif /*FEATURE_MBMS*/    
      ...
      case SM_MESG_TYPE_NOT_COMPATIBLE:
       ...
       if (cur_pdp->pdp_state == PDP_ACTIVE_PENDING)
        ...
        else if (cur_pdp->pdp_state == MBMS_ACTIVE_PENDING)
        {
          ...
          // Set the cause to appropriate reject cause value.
          pdp_list[con_id]->current_cause = cur_pdp->current_cause; // MBMS_PENDING

          // Close PS Signaling Session
          MSG_HIGH_DS_0(SM_SUB, "Calling GMM Close Session Req");
          send_gmm_close_session_req();
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
          sm_send_cm_confirm(pdp_list[con_id], SMREG_MBMS_ACTIVATE_REJ,
                         SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
        }
#endif
...
)
{
  ...
  if (pdp->pdp_state == PDP_ACTIVE)
  {
    ...
    ** ---------------------------------------------------------------------*/
#ifdef FEATURE_MBMS
    if(pdp->is_mbms == FALSE)
    {
      sm_deact_associated_mbms_ctxt(pdp);
    }
#endif
...
#ifdef FEATURE_MBMS
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
     if (cur_pdp->is_mbms == TRUE) // in case of MBMS we are using DEACTIVATE IMPLICIT MBMS_PENDING IMP code to return
     {
       sm_send_cm_confirm(cur_pdp, SMREG_PDP_DEACTIVATE_CNF, SM_NETWORK_CAUSE);
     }
     else
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
#endif
...
#ifdef FEATURE_MBMS
    else if ( (pdp_list[con_id]->pdp_state == MBMS_ACTIVE) ||
                 (pdp_list[pdp->trans_id] != NULL && 
                   pdp_list[pdp->trans_id]->is_mbms && pdp_list[con_id]->pdp_state == PDP_ACTIVE_PENDING) )
    {
        sm_perform_local_pdp_deactivation(pdp_list[con_id]);
        return TRUE;
    }
    else if (pdp_list[con_id]->pdp_state == MBMS_ACTIVE_PENDING)
    {
      MSG_HIGH_DS_1(SM_SUB,"MBMS: Dup MT received while MO req gone, drop MT %d",pdp->trans_id);
...
#ifdef FEATURE_MBMS
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      else if (pdp_list[i]->cm_pending_msg == (int)MSM_SM_ACTIVATION_REQ)
      {
        sm_send_cm_confirm(pdp_list[i], SMREG_MBMS_ACTIVATE_REJ, SM_INTERNAL_CAUSE);
      }
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
#endif
...
/*===========================================================================
FUNCTION SM_CHECK_PS_CALL_IN_PROGRESS

DESCRIPTION
  This function checks if PS call activity like activation/modification/deact is in progrss
...
===========================================================================*/
boolean sm_check_ps_call_in_progress()
{
  boolean ps_call_in_progress = FALSE;
  word index;
  if(timer_est_pen_flag == TRUE)
  {
    ps_call_in_progress = TRUE;
  }
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if (pdp_list[index] != NULL 
...
void sm_modify_rab(boolean mt_page)
{
  ...
  for (index = 0; index < MAX_PDP_NUM; index++)
  {
    if (pdp_list[index] != NULL)
    {
      ...
      {
        if(ds_3gpp_is_apn_blocked(pdp_list[index]->apn_name))
        {
...
DESCRIPTION
  This function processes REQUEST_MBMS_ACTIVATION received from the network.
...
===========================================================================*/
void sm_proc_request_mbms_activation (pdp_context_info *cur_pdp)
{
  byte nsapi, reject_request=FALSE;
  word mesg_bit_len;
  uint16 max_l3_message = gmm_max_L3_msg_length(MM_PS_DOMAIN);
  ...
  xlate_status_T status;
  byte saved_nsapi, saved_ti;
  pdp_state_T linked_ti_state=PDP_INACTIVE;


  // if MBMS is not supported, then then higher layers to send the reject msg MBMS_PENDING

      MSG_HIGH_DS_1(SM_SUB, "MBMS: REQUEST MBMS ACTIVATION, TI=%d",cur_pdp->trans_id);

  // if linked TI doesnt exists or is not in ACTIVE state then reject message
  if ( (cur_pdp->linked_ti >= MAX_PDP_NUM) ||
         pdp_list[cur_pdp->linked_ti] == NULL || 
        !(pdp_list[cur_pdp->linked_ti]->pdp_state == PDP_ACTIVE ||
          pdp_list[cur_pdp->linked_ti]->pdp_state == PDP_MODIFY_PENDING) )
  {
        l3_buf = (uint8*) modem_mem_calloc(1, max_l3_message, MODEM_MEM_CLIENT_NAS);
        sm_check_for_null_ptr((void*)l3_buf);
        MSG_HIGH_DS_2(SM_SUB, "MBMS: Send REQUEST_MBMS_ACTIVATION_REJ to network, Linked_TI=%d, Linked_TI state=%d",
          cur_pdp->linked_ti, linked_ti_state);

        /* Pack the Layer 3 Message and send it to GMM */
        cur_pdp->current_cause = (cc_cause_T) SM_INSUFFICIENT_RESOURCES; //MBMS_PENDING
      ...
      {
        MSG_ERROR_DS_0( SM_SUB, "MBMS: Error in Encoding the Request MBMS activation reject msg");
      }
  ...
  /* -----------------------------------------------------------------------
  ** Check if the PDP/MBMS context is already active.  If so the SM will locally
  ** deactivate the PDP/MBMS context and then proceed with the Network requested
  ** PDP/MBMS context activation.
  ** ----------------------------------------------------------------------*/
  // how to ensure that all the linked TI are deactivated locally later
  if (cur_pdp->pdp_state == MBMS_ACTIVE ||
    cur_pdp->pdp_state == PDP_ACTIVE ||
    cur_pdp->pdp_state == PDP_MODIFY_PENDING)
  {
    pdp_address_T stored_pdp_addr;
    apn_T         stored_apn_name;
    mbms_prot_config_options_T stored_protocol_config={0};
    byte saved_linked_ti;
    boolean mbms_prot_cnfg_valid = FALSE;

    MSG_HIGH_DS_2(SM_SUB, "MBMS: Network Requested MBMS activation for MBMS/PDP=%d, state=%d",
      cur_pdp->is_mbms,cur_pdp->pdp_state);

    saved_nsapi = cur_pdp->nsapi.nsapi;
    saved_ti = cur_pdp->trans_id;
    saved_linked_ti= cur_pdp->linked_ti;

    // Set the state to Inactive and send the deactivation Indication to CM.
    cur_pdp->pdp_state = PDP_INACTIVE;

    //Reset the con state
    cur_pdp->pdp_con_state = SIG_CONNECTION_NONE;

    //Send RABMSM_DEACTIVATE_IMPLICIT_IND to RABM later remove this code
    sm_notify_rabm((int)RABMSM_DEACTIVATE_IMPLICIT_IND, cur_pdp->nsapi.nsapi);

    /* Store the MBMS address */
    stored_pdp_addr = cur_pdp->pdp_addr;

    /* Store the APN Name */
    stored_apn_name = cur_pdp->apn_name;

    /* Store the Protocol Config Options*/
    if (cur_pdp->mbms_prot_cnfg.valid)
    {
      mbms_prot_cnfg_valid = TRUE;
      stored_protocol_config = cur_pdp->mbms_prot_cnfg;
    }
...
#ifndef FEATURE_SECONDARY_PDP_PROMOTION_REMOVAL
    // Send the SMREG_PDP_DEACTIVATE_IND to the CM.
    // later: will CM assigns the Group id. coz this msg makes to default
    // Who will initiate the Deactivation if the teardown indicator flag is ON. IMP
    //      in that case then we have to make a check that linked TI is not in the group of the TearDown. IMP
    ...
    {
      //Now trigger MBMS Activation for the NSAPI
      cur_pdp->nsapi.valid = TRUE;
      cur_pdp->nsapi.nsapi = saved_nsapi;
      cur_pdp->trans_id = saved_ti;
      nsapi_list[saved_nsapi] = saved_ti;
     ...
     /* Retore the MBMS address */
      cur_pdp->pdp_addr = stored_pdp_addr;
  
      /* Restore the APN Name */
      cur_pdp->apn_name = stored_apn_name;
  
      /* Restore the MBMS Protocol Config */
      if (mbms_prot_cnfg_valid == TRUE)
      {
        cur_pdp->mbms_prot_cnfg = stored_protocol_config;
      }
  
      // Set the mt_orig_type to true for Mobile terminated request
      cur_pdp->mt_orig_type = TRUE;
      cur_pdp->is_mbms = TRUE;
  
      // Send the SMREG_MBMS_ACTIVATE_IND to the CM to continue.
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
      sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_IND, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
    }
  ...
  else if (cur_pdp->pdp_state == PDP_ACTIVE_PENDING)
  {
    ...
    MSG_HIGH_DS_1(SM_SUB,"MBMS: PDP activation is already PDP_ACTIVE_PENDING on con id %d",cur_pdp->trans_id);
    ...
    cur_pdp->current_cause = (cc_cause_T)PDP_MBMS_REQUEST_COLLISION; // later
    //Send RABMSM_DEACTIVATE_IMPLICIT_IND to RABM later remove this code
    sm_notify_rabm((int)RABMSM_DEACTIVATE_IMPLICIT_IND, cur_pdp->nsapi.nsapi);
  ...
  else if (cur_pdp->pdp_state == MBMS_ACTIVE_PENDING)
  {
    ...
    MSG_HIGH_DS_1(SM_SUB,"MBMS: MBMS activation is already MBMS_ACTIVE_PENDING on con id %d",cur_pdp->trans_id);
    ...
    cur_pdp->current_cause = (cc_cause_T)MBMS_DUPLICATE; // later
    //Send RABMSM_DEACTIVATE_IMPLICIT_IND to RABM later remove this code
    sm_notify_rabm((int)RABMSM_DEACTIVATE_IMPLICIT_IND, cur_pdp->nsapi.nsapi);
  ...
  else if (cur_pdp->pdp_state == PDP_INACTIVE_PENDING)
  {
    ...
    MSG_HIGH_DS_2(SM_SUB,"MBMS: MBMS/PDP=%d PDP_INACTIVE_PENDING on con id %d",cur_pdp->is_mbms, cur_pdp->trans_id);
    ...
    cur_pdp->current_cause = (cc_cause_T)PDP_MBMS_REQUEST_COLLISION; // MBMS_PENDING
    //Send RABMSM_DEACTIVATE_IMPLICIT_IND to RABM later remove this code
    sm_notify_rabm((int)RABMSM_DEACTIVATE_IMPLICIT_IND, cur_pdp->nsapi.nsapi); // MBMS_PENDING
  ...
  {
    cur_pdp->is_mbms = TRUE;
    // If already CM request is gone and waiting for CM response, then drop the duplicate request
    if (cur_pdp->mt_orig_type)
    {
      //Do nothing just drop it
      return;
    ...
    if (reject_request)
    {
      l3_buf = (uint8*) modem_mem_calloc(1, max_l3_message, MODEM_MEM_CLIENT_NAS);
      sm_check_for_null_ptr((void*)l3_buf);
        // later code this packing function.
      /* Pack the Layer 3 Message and send it to GMM */
      status = pack_layer3_mesg(&mesg_bit_len, l3_buf, cur_pdp, REQUEST_MBMS_ACTIVATION_REJ);
    ...
    // Send the SMREG_MBMS_ACTIVATE_IND to the CM.
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_IND, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
  }
...
===========================================================================*/
void sm_proc_activate_mbms_accept(pdp_context_info *cur_pdp)
{
  if (cur_pdp->pdp_state == MBMS_ACTIVE_PENDING)
  {
    // Change the state of the MBMS context
    MSG_HIGH_DS_1(SM_SUB, "MBMS: Changing conid=%d state to MBMS_ACTIVE",cur_pdp->trans_id);
    cur_pdp->pdp_state = MBMS_ACTIVE;
    ...
    {
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
        // Send the SMREG_MBMS_ACTIVATE_CNF to the CM.
        sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_CNF, SM_INTERNAL_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
      }
  }
  else
  {
    // Received PDP accept message in wrong state, Ignore
    MSG_ERROR_DS_2( SM_SUB,"MBMS: Received ACTIVATE_MBMS_ACCEPT in wrong for conid=%d, state %d",
      cur_pdp->trans_id, cur_pdp->pdp_state);
...
DESCRIPTION
  This function processes the Network sent Reject message for MBMS context activate request.
...
===========================================================================*/
void sm_proc_activate_mbms_reject(pdp_context_info *cur_pdp)
{

  MSG_HIGH_DS_1(SM_SUB,"MBMS: Rcvd MBMS Reject, Cause = %d",cur_pdp->current_cause);

  if (cur_pdp->pdp_state == MBMS_ACTIVE_PENDING)
  {
    // Change the state of the MBMS context back to INACTIVE.
    MSG_MED_DS_1( SM_SUB, "MBMS: Changing MBMS %d state to PDP_INACTIVE",cur_pdp->trans_id);
    cur_pdp->pdp_state = PDP_INACTIVE;
    //Reset the PDP con state
    cur_pdp->pdp_con_state = SIG_CONNECTION_NONE;
    ...
    // Send the SMREG_PDP_ACTIVATE_REJ to the CM.
#if defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)
    sm_send_cm_confirm(cur_pdp, SMREG_MBMS_ACTIVATE_REJ, SM_NETWORK_CAUSE);
#endif /*defined (FEATURE_GSM_GPRS) || defined (FEATURE_WCDMA)*/
  }
  else
  {
    // Received MBMS accept message in wrong state, Ignore
    MSG_ERROR_DS_1( SM_SUB,"MBMS: Received ACTIVATE_MBMS_REJECT in wrong state %d",cur_pdp->pdp_state);
...
#ifdef FEATURE_MBMS
    // process MBMS context Status
    if (gmm_pdp_mbms_context_status->mbms_status_length == 0)
    {
      // FEATURE_DUAL_SIM may need to check in future if all the mbms contexts need to be locally deactivated irrespective of DUAL SIM
      sm_all_mbms_local_deactivation();
    }
    else
    {
      nsapi_id = 128;
      for (i = start_index_mbms ; i < gmm_pdp_mbms_context_status->mbms_status_length+start_index_mbms ; i++)
      {
          working_byte = gmm_pdp_mbms_context_status->value[i];
          for (j = 0 ; j < 8 ; j++)
          {
            if (( nsapi_id != MAX_NSAPI_NUM) && (nsapi_list[nsapi_id] != INVALID_ID) && 
                pdp_list[nsapi_list[nsapi_id]] != NULL)
            {
                if ((working_byte & mask[j]) == 0)
                {
      ...
      // process remaining nsaps
      for ( ; nsapi_id < MAX_NSAPI_NUM ; nsapi_id++)
      {
        if (nsapi_list[nsapi_id] != INVALID_ID && pdp_list[nsapi_list[nsapi_id]] != NULL )
        {
          sm_perform_local_pdp_deactivation(pdp_list[nsapi_list[nsapi_id]]);
        }
      }
...
void sm_process_status_ind (byte trans_id)
{
  ...
  if (trans_id >= MIN_MO_TI && trans_id < MAX_PDP_NUM && pdp_list[trans_id] != NULL)
  {
...
)
{
 ...
 /*==============================================================================
 FUNCTION  sm_process_save_ctxt_psm_req
 
 DESCRIPTION
  This function processes request from GMM to save current active PDP context in SM cache while UE enters PSM mode.
 ...
 SIDE EFFECTS
  None
 /*============================================================================*/
 void sm_process_save_ctxt_psm_req (sm_cmd_type *cmd_ptr)
 {

   boolean result = FALSE;

   MSG_HIGH_DS_0(SM_SUB, "SM received GMMSM_SAVE_CTXT_PSM_REQ");

#ifdef FEATURE_DUAL_SIM
#ifndef FEATURE_DUAL_DATA
  ctxt_as_id = cmd_ptr->cmd.gmmsm_save_ctxt_psm_req.as_id;

  if (SM_IS_VALID_AS_ID(cmd_ptr->cmd.gmmsm_save_ctxt_psm_req.as_id) == FALSE)
  {
    MSG_HIGH_DS_2( SM_SUB,"SM AS ID Conflict sm_as_id : %d, message as_id: %d", sm_as_id , cmd_ptr->cmd.gmmsm_save_ctxt_psm_req.as_id);

    sm_send_gmm_save_ctxt_psm_cnf(FALSE);
    ctxt_as_id = SYS_MODEM_AS_ID_NONE;
    return ;
  }
#endif /*#ifdef FEATURE_DUAL_DATA*/
   ...
   result = sm_save_psm_context();
   
   sm_send_gmm_save_ctxt_psm_cnf(result);
 }

 
 /*==============================================================================
 FUNCTION  sm_process_restore_ctxt_psm_req
 
 DESCRIPTION
  This function processes request from GMM to restore last stored PDP context upon exiting PSM mode.
 ...
 SIDE EFFECTS
  None
 /*============================================================================*/
 void sm_process_restore_ctxt_psm_req (sm_cmd_type *cmd_ptr)

 {
  boolean result = FALSE;

  MSG_HIGH_DS_0(SM_SUB, "=SM= SM received GMMSM_RESTORE_CTXT_PSM_REQ");

#ifdef FEATURE_DUAL_SIM
#ifndef FEATURE_DUAL_DATA
  ctxt_as_id = cmd_ptr->cmd.gmmsm_restore_ctxt_psm_req.as_id;
  if (SM_IS_VALID_AS_ID(cmd_ptr->cmd.gmmsm_restore_ctxt_psm_req.as_id) == FALSE)
  {
    MSG_HIGH_DS_2( SM_SUB,"SM AS ID Conflict sm_as_id : %d, message as_id: %d", sm_as_id , cmd_ptr->cmd.gmmsm_restore_ctxt_psm_req.as_id);
    sm_send_gmm_restore_ctxt_psm_cnf(FALSE);
    ctxt_as_id = SYS_MODEM_AS_ID_NONE;
    return ;
  }
#endif /*#ifdef FEATURE_DUAL_DATA*/
  ...
  {
    ULOG_RT_PRINTF_0(sm_get_ulog_handle(), "=SM= SM received GMMSM_RESTORE_CTXT_PSM_REQ");
  }

  result = sm_restore_psm_context();
  sm_send_cm_restore_ctxt_psm_req();
  
}


/*==============================================================================
FUNCTION  sm_send_gmm_restore_ctxt_psm_cnf

DESCRIPTION
 This function is used to send a response to GMM for GMMSM_RESTORE_CTXT_PSM_REQ after restoring SM related PSM context.
...
SIDE EFFECTS
 None
/*============================================================================*/
void sm_send_gmm_restore_ctxt_psm_cnf
(
  boolean status
)
{
  ...
  if (gmm_cmd != NULL)
  {
    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.header.message_set     = MS_GMM_SM;
    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.header.message_id      = GMMSM_RESTORE_CTXT_PSM_CNF;
    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.header.message_len_lsb = 0;
    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.header.message_len_msb = 0;   

#ifdef FEATURE_DUAL_SIM
    #ifdef FEATURE_DUAL_DATA
    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
    if (SM_IS_VALID_AS_ID(ctxt_as_id) == FALSE)
      gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.as_id= ctxt_as_id;  
    else
      gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.as_id= (sys_modem_as_id_e_type)sm_as_id;  
    #endif /*#ifdef FEATURE_DUAL_DATA*/
#endif

    gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.status = status;

    if(sm_get_ulog_handle() != NULL)
    {
      ULOG_RT_PRINTF_1(sm_get_ulog_handle(), "=SM= Sent GMMSM_RESTORE_CTXT_PSM_CNF status = %d", status);
    }
    MSG_HIGH_DS_1(SM_SUB, "Sending GMMSM_RESTORE_CTXT_PSM_CNF, status = %d ", status);
    //mm_put_cmd (gmm_cmd);
    gmm_process_restore_ctxt_psm_cnf(gmm_cmd->cmd.gmmsm_restore_ctxt_psm_cnf.status);
#ifndef FEATURE_MODEM_HEAP
   gs_free(gmm_cmd);
#else
   modem_mem_free(gmm_cmd, MODEM_MEM_CLIENT_NAS);
...
SIDE EFFECTS
 None
/*============================================================================*/
void sm_send_gmm_save_ctxt_psm_cnf
(
  boolean status
)
{
  ...
  if (gmm_cmd != NULL)
  {
    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.header.message_set     = MS_GMM_SM;
    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.header.message_id      = GMMSM_SAVE_CTXT_PSM_CNF;
    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.header.message_len_lsb = 0;
    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.header.message_len_msb = 0;

#ifdef FEATURE_DUAL_SIM
    #ifdef FEATURE_DUAL_DATA
    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.as_id = (sys_modem_as_id_e_type)sm_as_id;
    #else
    if (SM_IS_VALID_AS_ID(ctxt_as_id) == FALSE)
      gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.as_id= ctxt_as_id;  
    else
      gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.as_id= (sys_modem_as_id_e_type)sm_as_id;
    #endif /*#ifdef FEATURE_DUAL_DATA*/
#endif

    gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.status = status;

    MSG_HIGH_DS_1(SM_SUB, "Sending GMMSM_SAVE_CTXT_PSM_CNF, status = %d ", status);
   // mm_put_cmd (gmm_cmd);
    gmm_process_save_ctxt_psm_cnf(gmm_cmd->cmd.gmmsm_save_ctxt_psm_cnf.status);
#ifndef FEATURE_MODEM_HEAP
   gs_free(gmm_cmd);
#else
   modem_mem_free(gmm_cmd, MODEM_MEM_CLIENT_NAS);
