...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_bcm.c#1 $
...
#ifdef FEATURE_CIOT
  ctxt_ptr->cp_only = FALSE;
  ctxt_ptr->hc_config.valid = FALSE;
  ctxt_ptr->splmn_rate_ctrl.valid = FALSE;
#endif
  ctxt_ptr->epco.valid = FALSE;
  ctxt_ptr->epco.len = 0;
  ctxt_ptr->epco.dsm_ptr = NULL;
...
)
{
  ...
  {               
    case ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT_REQUEST:
      ...
      if (ctxt_ptr == NULL && bearer_id >= MIN_NSAPI_NUM && bearer_id < MAX_NSAPI_NUM)
	  {
    ...
    case ESM_DATA_TRANSPORT:
      esm_send_data_transport_ind(data_ptr);
      break;
...
)
{
  ...
  switch(msg_ptr->emm_cause.local_cause)
  {
    ...
    default:
      ...
      {
        nas_assert_check((boolean)(instance_id < ESM_MAX_EPS_BEARERS));
...
/*==============================================================================
FUNCTION  esm_bcm_process_save_ctxt_psm_req

DESCRIPTION
 This function processes request from EMM to save context in ESM to enter PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
static void esm_bcm_process_save_ctxt_psm_req
(
  esm_bcm_T *mgr_ptr
)
{
  ...
  MSG_HIGH_DS_0(SM_SUB, "ESM: ESM received EMM_ESM_SAVE_CTXT_PSM_REQ");

  result = esm_save_psm_context(mgr_ptr);
  
  esm_send_emm_save_ctxt_psm_rsp(result);
}

/*==============================================================================
FUNCTION  esm_bcm_process_restore_ctxt_psm_req

DESCRIPTION
 This function processes request from EMM to restore context upon exiting PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
static void esm_bcm_process_restore_ctxt_psm_req(void)
{
  boolean result = FALSE;

  if(sm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(sm_get_ulog_handle(), "=ESM= ESM received EMM_ESM_RESTORE_CTXT_PSM_REQ");
  }
  MSG_HIGH_DS_0(SM_SUB, "ESM: ESM received EMM_ESM_RESTORE_CTXT_PSM_REQ");

  result = esm_restore_psm_context();
  
  if(result == TRUE)
  {
    esm_send_cm_restore_ctxt_psm_req();
  }
  else
  {
    esm_send_emm_restore_ctxt_psm_rsp(FALSE);
  }
}
...
)
{
  ...
  switch (cmd_ptr->cmd.hdr.message_id)
  {
...
#ifdef FEATURE_CIOT
      esm_set_attach_without_pdn(ESM_BC_ON_ATTACH_DEF_BEARER_NONE);
#endif
      break;
    ...
    case EMM_ESM_GET_PDN_CONNECTIVITY_REQ_IND:
      ...
      local_cause.valid = TRUE;
      esm_bcm_clean_up(mgr_ptr,local_cause); 
      esm_build_and_send_get_pdn_connectivity_req_ind(cmd_ptr->cmd.esm_get_pdn_connectivity_req.seq_num,
                                                      cmd_ptr->cmd.esm_get_pdn_connectivity_req.responding_to_page,
                                                      cmd_ptr->cmd.esm_get_pdn_connectivity_req.attach_without_pdn_supported);
      break;
#if defined(FEATURE_NAS_GW) && defined(FEATURE_NAS_SM_IRAT)
  ...
  case NAS_ESM_ISR_STATUS_CHANGE_IND:
    ...
    case EMM_ESM_SAVE_CTXT_PSM_REQ:
      esm_bcm_process_save_ctxt_psm_req(mgr_ptr);
      break;

    case EMM_ESM_RESTORE_CTXT_PSM_REQ:
      esm_bcm_process_restore_ctxt_psm_req();
      break;

#ifdef FEATURE_CIOT
    case EMM_ESM_CIOT_RAT_CHANGE_IND:
      MSG_HIGH_DS_1(SM_SUB, "ESM: EMM sent CIOT RAT change indication status(%d)",
                    cmd_ptr->cmd.esm_ciot_rat_change_ind.target_rat);
      esm_bcm_deactivate_non_transferable_ctxts_irat_m1_nb1(mgr_ptr, &(cmd_ptr->cmd.esm_ciot_rat_change_ind) );
      break;
#endif
...
//    break;
    default:
      MSG_ERROR_DS_1(SM_SUB, "ESM: ESM received UNKNOWN message from EMM with id %d", cmd_ptr->cmd.hdr.message_id);
      break;
  }
} /* handle_emm_message() */
...
/*==============================================================================
FUNCTION  esm_bcm_process_restore_bearer_ctxt_req

DESCRIPTION
 This function processes request from DS to restore a default/dedicated
 bearer upon wake up from PSM that existed in the previous LTE session before
 entering PSM
...
SIDE EFFECTS
 None
/*============================================================================*/
static void esm_bcm_process_restore_bearer_ctxt_req
(
  mm_cmd_type *cmd_ptr,
  esm_bcm_T    *mgr_ptr
)
{
  emm_restore_bearer_ctxt_req_s_type *msg_ptr = &cmd_ptr->cmd.emm_restore_bearer_ctxt_req;
  esm_eps_bearer_context_T *ctxt_ptr = NULL;
  byte index;

  /* Handle this request only when no bearer exists with the bearer id 
     indicated in the request */
  if(esm_bcm_find_bearer_context(mgr_ptr, msg_ptr->eps_bearer_id) != NULL)
  {
    MSG_FATAL_DS(SM_SUB, "ESM: Bearer ID %d already exists. Ignoring", msg_ptr->eps_bearer_id,0,0);
    return;
  }

  ctxt_ptr = esm_bcm_allocate_context(mgr_ptr, msg_ptr->eps_bearer_id);
  sm_check_for_null_ptr((void*)ctxt_ptr);

  /* For a dedicated bearer context restore request, there must be a corresponding
     linked default bearer context already present */
  if((msg_ptr->default_bearer_ctxt == FALSE) &&
     (esm_bcm_find_bearer_context(mgr_ptr, msg_ptr->lbi) == NULL))
  {
    MSG_FATAL_DS(SM_SUB, "ESM: LBI %d for Dedicated BID %d doesn't exist",
                    msg_ptr->lbi,msg_ptr->eps_bearer_id,0);
  }

  /* Restore bearer context */
  ctxt_ptr->eps_bearer_id = msg_ptr->eps_bearer_id;
  ctxt_ptr->default_bearer_ctxt = msg_ptr->default_bearer_ctxt;
  ctxt_ptr->sdf_id = msg_ptr->sdf_id;
  ctxt_ptr->sdf_qos = msg_ptr->sdf_qos;
#if defined(FEATURE_NAS_GW) && defined(FEATURE_NAS_SM_IRAT)
  ctxt_ptr->neg_qos = msg_ptr->neg_qos;
  ctxt_ptr->neg_llc_sapi = msg_ptr->neg_llc_sapi;
  ctxt_ptr->ti = msg_ptr->ti;
  ...
  for(index = 0; index < ESM_MAX_EPS_BEARERS; index++)
  {
    if(ctxt_ptr->eps_bearer_id == sm_psm_cache_ptr->lte.psm_bc[index].eps_bearer_id)
    {
      ctxt_ptr->radio_pri = sm_psm_cache_ptr->lte.psm_bc[index].radio_pri;
      ctxt_ptr->packet_flow_id = sm_psm_cache_ptr->lte.psm_bc[index].packet_flow_id;
      break;
    }
  }
#endif

  if(msg_ptr->default_bearer_ctxt == TRUE)
  {
    mgr_ptr->first_def_bearer_ctxt_received = TRUE;
    ctxt_ptr->sm_group_id = msg_ptr->sm_group_id;
    ctxt_ptr->pdn_address = msg_ptr->pdn_address;
    ctxt_ptr->apn_name = msg_ptr->apn_name;
    ctxt_ptr->apn_ambr = msg_ptr->apn_ambr;
    ctxt_ptr->bcm = msg_ptr->bcm;

    if(mgr_ptr->on_attach_def_bearer_pending == ESM_BC_ON_ATTACH_DEF_BEARER_NONE)
    {
  ...
  /* Send bearer context restore response to DS via CM */
  esm_send_cm_restore_bearer_ctxt_rsp(ctxt_ptr->eps_bearer_id, ctxt_ptr->connection_id);

  /* Inform GSDI that data call is connected for polling */
  sm_mmgsdi_in_traffic_channel_mode(TRUE);

} /* end esm_bcm_process_restore_bearer_ctxt_req() */
...
)
{
  ...
  if(con_id == ESM_INVALID_CON_ID)
  {
    if(cmd_ptr->cmd.hdr.message_id == EMM_RESTORE_BEARER_CTXT_REQ)
    {
      if(sm_get_ulog_handle() != NULL)
      {
        ULOG_RT_PRINTF_1(sm_get_ulog_handle(), "=ESM= received EMM_RESTORE_BEARER_CTXT_REQ, Bearer ID =%d",
                         cmd_ptr->cmd.emm_restore_bearer_ctxt_req.eps_bearer_id);
      }
      MSG_HIGH_DS_1(SM_SUB, "ESM: received EMM_RESTORE_BEARER_CTXT_REQ, Bearer ID =%d",
                    cmd_ptr->cmd.emm_restore_bearer_ctxt_req.eps_bearer_id);
      esm_bcm_process_restore_bearer_ctxt_req(cmd_ptr, mgr_ptr);
      return;
    }
    else if(cmd_ptr->cmd.hdr.message_id  == MM_RESTORE_PSM_CTXT_RSP)  
    {
      if(sm_get_ulog_handle() != NULL)
      {
        ULOG_RT_PRINTF_0(sm_get_ulog_handle(), "=ESM= received MM_RESTORE_PSM_CTXT_RSP");
      }
      MSG_HIGH_DS_0(SM_SUB, "ESM: received MM_RESTORE_PSM_CTXT_RSP");
      esm_send_emm_restore_ctxt_psm_rsp(cmd_ptr->cmd.mm_restore_psm_ctxt_rsp.status);
      return;
    }
  }
  ...
  switch (cmd_ptr->cmd.hdr.message_id)
  {
    ...
    case EMM_ESM_DATA_TRANSPORT_REQ:
      break;
    case EMM_ESM_ATTACH_STATUS_IND:  
      break;
#endif
    case EMM_RAB_REESTAB_REQ:
      MSG_HIGH_DS_2(SM_SUB, "ESM: received EMM_RAB_REESTAB_REQ, Bearer ID =%d exp data = %d",
                    cmd_ptr->cmd.emm_rab_reestab_req.eps_bearer_id,
                    cmd_ptr->cmd.emm_rab_reestab_req.mo_exception_data);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_rab_reestab_req.eps_bearer_id >= MIN_NSAPI_NUM));
	  if (ctxt_ptr != NULL)
	  {
        ctxt_ptr->last_msg_sent = ESM_LAST_UNDEFINED;
        esm_build_and_send_service_request(ctxt_ptr,cmd_ptr->cmd.emm_rab_reestab_req.call_type,
			                             cmd_ptr->cmd.emm_rab_reestab_req.mo_exception_data
			                                 ); 
	  }	
	  else
    ...
    case EMM_ACT_DEFAULT_BEARER_CONTEXT_ACCEPT:
      MSG_HIGH_DS_1(SM_SUB, "ESM: received Default Bearer CTXT ACCEPT, Bearer ID =%d \n",
                    cmd_ptr->cmd.emm_act_default_bearer_context_accept.eps_bearer_id);
      //esm_get_epco_dsm_attachment(cmd_ptr);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_act_default_bearer_context_accept.eps_bearer_id >= MIN_NSAPI_NUM));
      if (ctxt_ptr != NULL)
      {
        state_ptr->state_name();
        state_ptr->ctxt_process_act_default_bearer_ctxt_accept(ctxt_ptr,&(cmd_ptr->cmd.emm_act_default_bearer_context_accept));
        if(mgr_ptr->on_attach_def_bearer_pending == ESM_BC_ON_ATTACH_DEF_BEARER_PENDING)
        {
          mgr_ptr->on_attach_def_bearer_pending = ESM_BC_ON_ATTACH_DEF_BEARER_SUCCESS;
      ...
      {
        if (cmd_ptr->cmd.emm_act_default_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr != NULL)
        {
          dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_act_default_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr));
        }
        MSG_ERROR_DS_1(SM_SUB,"ESM: No context for con id %d exists at NAS",con_id);
      }
      break;
    case EMM_ACT_DEFAULT_BEARER_CONTEXT_REJ:
      MSG_LOW_DS_2(SM_SUB, "ESM: Enter BCM: handle_CM_message, received Default Bearer CTXT REJECT, Bearer Id=%d, Cause=%d\n",
                   cmd_ptr->cmd.emm_act_default_bearer_context_rej.eps_bearer_id,
                   cmd_ptr->cmd.emm_act_default_bearer_context_rej.esm_cause.esm_cause);
      //esm_get_epco_dsm_attachment(cmd_ptr);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_act_default_bearer_context_rej.eps_bearer_id >= MIN_NSAPI_NUM));
      if (ctxt_ptr != NULL)
      {
        state_ptr->ctxt_process_act_default_bearer_ctxt_reject(ctxt_ptr,&(cmd_ptr->cmd.emm_act_default_bearer_context_rej));
      ...
      {
        if (cmd_ptr->cmd.emm_act_default_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr != NULL)
        {
          dsm_free_packet((dsm_item_type**)&(cmd_ptr->cmd.emm_act_default_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr));
          cmd_ptr->cmd.emm_act_default_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr = NULL;
        }
        MSG_ERROR_DS_1(SM_SUB,"ESM: No context for con id %d exists at NAS",con_id);
      }
    ...
    case EMM_ACT_DEDICATED_BEARER_CONTEXT_ACCEPT:
      MSG_HIGH_DS_1(SM_SUB, "ESM: received Dedicated Bearer CTXT ACCEPT, Bearer ID =%d \n",
                    cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept.eps_bearer_id);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept.eps_bearer_id >= MIN_NSAPI_NUM));
      //esm_get_epco_dsm_attachment(cmd_ptr);
      if (ctxt_ptr != NULL)
      {
        ...
        {
          state_ptr->ctxt_process_act_dedicated_bearer_ctxt_accept(ctxt_ptr,&(cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept),mgr_ptr);
        ...
        else /* If the state is not SUCCESS, then it must have FAILED, so we need to deactivate this context*/
        {
          ...
          esm_build_and_send_deactivate_ind(ctxt_ptr,local_cause,cause);
          if (cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr != NULL) 
          {
            dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr));
            cmd_ptr->cmd.emm_act_dedicated_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr = NULL;
          }
          esm_bcm_reset_ctxt(ctxt_ptr);
          ctxt_ptr = NULL;
        }
    ...
    case EMM_ACT_DEDICATED_BEARER_CONTEXT_REJ:
      MSG_HIGH_DS_2(SM_SUB, "ESM: received Dedicated Bearer CTXT REJECT, Bearer ID =%d , Caus=%d\n",
                    cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.eps_bearer_id,
                    cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.esm_cause.esm_cause);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.eps_bearer_id >= MIN_NSAPI_NUM));
      //esm_get_epco_dsm_attachment(cmd_ptr);
      if (ctxt_ptr != NULL)
      {
        state_ptr->ctxt_process_act_dedicated_bearer_ctxt_reject(ctxt_ptr,&(cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej));
        esm_modem_mem_free(mgr_ptr->esm_bc[con_id]);
        mgr_ptr->esm_bc[con_id] = NULL;
        ctxt_ptr = NULL;
      }
      ...
      {
        if (cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr != NULL) 
        {
          dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr));
          cmd_ptr->cmd.emm_act_dedicated_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr = NULL;
        }
        MSG_ERROR_DS_1(SM_SUB,"ESM: No context for con id %d exists at NAS",con_id);
      }
      break;  
    ...
    case EMM_MODIFY_BEARER_CONTEXT_REJ:
      MSG_HIGH_DS_1(SM_SUB, "ESM: Received MODIFY BEARER CTXT REJ, Bearer ID =%d",
                    cmd_ptr->cmd.emm_modify_bearer_context_rej.eps_bearer_id);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_modify_bearer_context_rej.eps_bearer_id 
                 >= MIN_NSAPI_NUM));
      //esm_get_epco_dsm_attachment(cmd_ptr);
      if (ctxt_ptr != NULL && esm_ignore_mod_due_to_collision(cmd_ptr) == FALSE)
      {
        state_ptr->ctxt_process_modify_bearer_ctxt_rej(ctxt_ptr,
                                  &cmd_ptr->cmd.emm_modify_bearer_context_rej);
      }
      else
      {
        if (cmd_ptr->cmd.emm_modify_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr != NULL) 
        {
          dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_modify_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr));
          cmd_ptr->cmd.emm_modify_bearer_context_rej.ext_protocol_conf_opt.dsm_ptr = NULL;
        }
        MSG_ERROR_DS_1(SM_SUB,"ESM: No context for con id %d exists at NAS",con_id);
      }
      break;
    case EMM_MODIFY_BEARER_CONTEXT_ACCEPT:
      MSG_HIGH_DS_1(SM_SUB, "ESM: Received MODIFY BEARER CTXT ACCEPT, Bearer ID =%d",
                    cmd_ptr->cmd.emm_modify_bearer_context_accept.eps_bearer_id);
      nas_assert_check((boolean)(cmd_ptr->cmd.emm_modify_bearer_context_accept.eps_bearer_id 
                 >= MIN_NSAPI_NUM));
      //esm_get_epco_dsm_attachment(cmd_ptr);
      if (ctxt_ptr != NULL)
      {
        state_ptr->ctxt_process_modify_bearer_ctxt_accept(ctxt_ptr,
                               &cmd_ptr->cmd.emm_modify_bearer_context_accept,mgr_ptr);
#if defined(FEATURE_NAS_GW) && defined(FEATURE_NAS_SM_IRAT)
      ...
      {
        if (cmd_ptr->cmd.emm_modify_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr != NULL) 
        {
          dsm_free_packet((dsm_item_type **)&(cmd_ptr->cmd.emm_modify_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr));
          cmd_ptr->cmd.emm_modify_bearer_context_accept.ext_protocol_conf_opt.dsm_ptr = NULL;
        }
        MSG_ERROR_DS_1(SM_SUB,"ESM: No context for con id %d exists at NAS",con_id);
      }
      break;
...
/*==============================================================================
  ...
  FUNCTION:  esm_bcm_deactivate_non_transferable_ctxts_irat_m1_nb1
 
  This function is called by ESM to deactivate any bearer contexts that
  are non transferbale upon IRAT between NB1 and CATM1

==============================================================================*/
void esm_bcm_deactivate_non_transferable_ctxts_irat_m1_nb1
(
  struct esm_bcm* mgr_ptr,
  esm_ciot_rat_change_ind_T* msg_ptr 
)
{
  byte index = 0;
  esm_cause_T cause;
  ...
  sys_radio_access_tech_e_type   target_rat = msg_ptr->target_rat;
  uint8 total_num_bearers = 0;
#if defined(FEATURE_NAS_GW) && defined(FEATURE_NAS_SM_IRAT)
  lte_nas_isr_status_type isr;
  isr.valid = FALSE;
#endif
  ...
  for (index=0; index < ESM_MAX_EPS_BEARERS; index++)
  {
    ...
    if (ctxt_ptr != NULL)
    {
      ...
      if( state_ptr->state() != ESM_BC_INACTIVE )
      {
         /* First deactivate the associated dedicated bearers and then the default bearer for below cases:
         ** (i)  Non-IP PDNs are not supported on CATM1
         ** (ii) IMS PDNs are not supported on NB1
         */
        if(((target_rat == SYS_RAT_LTE_M1_RADIO_ACCESS) &&
             (ds3gpp_is_bearer_for_nip_pdn(ctxt_ptr->eps_bearer_id))) ||
           ((target_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
             ds3gpp_is_bearer_for_ims_pdn(ctxt_ptr->eps_bearer_id)))
        {
          if(ctxt_ptr->default_bearer_ctxt == TRUE)
          {
            esm_bcm_deactivate_associated_ded_ctxts(mgr_ptr,local_cause,cause,ctxt_ptr->eps_bearer_id);
        ...
        else if ((target_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
                 (ctxt_ptr->default_bearer_ctxt == TRUE))
        {
          /* Deactivate dedicated bearers on IRAT to NB1 as QoS is not supported */
          esm_bcm_deactivate_associated_ded_ctxts(mgr_ptr,local_cause,cause,ctxt_ptr->eps_bearer_id);
          reset_flag = FALSE;
        }
        else
  ...
  /* Deactivate additional bearers on NB1 as MAX 2 PDNs and 2 bearers are allowed */
  for (index=0; index < ESM_MAX_EPS_BEARERS; index++)
  {
    esm_eps_bearer_context_T*  ctxt_ptr =  mgr_ptr->esm_bc[index];
    if ((ctxt_ptr != NULL && ctxt_ptr->connection_id != ESM_INVALID_CON_ID) &&
        (target_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS))
    { 
      if (total_num_bearers < MAX_ALLOWED_EPS_BEARERS_NB1)
      {
        total_num_bearers++;
        MSG_SPRINTF_2(MSG_SSID_LTE, 
                      MSG_LEGACY_HIGH, 
                      "ESM: BC id = %d , STATUS=%s\n",
                      index,
                      ctxt_ptr->eps_bearer_state_ptr->get_state_name());
      }
      else
      {
        MSG_HIGH_DS_1(SM_SUB, "ESM: BC id = %d Deactivate additional bearers on NB1 \n", index);
        esm_build_and_send_deactivate_ind(ctxt_ptr,local_cause,cause);
        esm_bcm_reset_ctxt(ctxt_ptr);
        ctxt_ptr = NULL;
      }
  ...
  esm_build_and_send_eps_bearer_status_update_ind(mgr_ptr
#if defined(FEATURE_NAS_GW) && defined(FEATURE_NAS_SM_IRAT)
    , isr
#endif
  	);
...
void esm_bcm_deactivate_non_transferable_ctxts_to_GW(esm_bcm_T* mgr_ptr)
{
  ...
  for(index=0; index < ESM_MAX_EPS_BEARERS; index++)
  {
	...
	if (ctxt_ptr != NULL)
	{
      ...
      if( state_ptr->state() != ESM_BC_INACTIVE )
      {
#if defined(FEATURE_NAS_SM_IRAT)
        /* First deactivate the associated dedicated bearers and then the default bearer*/
