...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/smix.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#define MIN_TFT_IE_LEN    1
#define MAX_TFT_IE_LEN    255
...
#ifdef FEATURE_MBMS
/* -----------------------------------------------------------------------
** FUNCTION     : PACK_MBMS_ACTIVATE_MESG()

** DESCRIPTION  : Packs the ACTIVATE_MBMS_REQUEST SM-SM peer message.
** ----------------------------------------------------------------------- */
xlate_status_T pack_mbms_activate_mesg( word* length, byte* ext_buf,
                                             const pdp_context_info* pdp );


/* -----------------------------------------------------------------------
** FUNCTION     : PACK_REQ_MBMS_ACTIVATE_REJ()

** DESCRIPTION  : Encodes the REQ_MBMS_ACTIVATE_REJ message sent to Network.
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T pack_req_mbms_activate_rej( word* length, byte* ext_buf,
                                                const pdp_context_info* pdp );


/* -----------------------------------------------------------------------
** FUNCTION     : UNPACK_REQUEST_MBMS_ACTIVATION()

** DESCRIPTION  : decodes the REQUEST_MBMS_ACTIVATION msg sent by Network
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T unpack_request_mbms_activation( uint16 len, byte* ext_buf,
                                                    pdp_context_info *pdp);


/* -----------------------------------------------------------------------
** FUNCTION     : UNPACK_ACTIVATE_MBMS_ACCEPT()

** DESCRIPTION  : Decodes the received ACTIVATE_MBMS_ACCEPT msg from network.
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T unpack_activate_mbms_accept(  uint16 len, byte* ext_buf,
                                                  pdp_context_info *pdp);

/* -----------------------------------------------------------------------
** FUNCTION     : UNPACK_ACTIVATE_MBMS_REJECT()

** DESCRIPTION  : decodes the ACTIVATE_MBMS_REJECT msg sent by Network.
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T unpack_activate_mbms_reject(  uint16 len, byte* ext_buf,
                                                  pdp_context_info *pdp);
...
/* -----------------------------------------------------------------------
** FUNCTION     : UNPACK_REATTEMPT_IND()

** DESCRIPTION  : Helper function that unpacks the Re-Attempt Indicator IE
** ----------------------------------------------------------------------- */
LOCAL xlate_status_T unpack_reattempt_ind(byte* ext_buf, uint16 *index, 
                                          pdp_context_info* pdp);
...
)
{
  ...
  switch (msg_type)
  {
    ...
    case ACTIVATE_MBMS_REQUEST:      
      status = pack_mbms_activate_mesg(length, ext_buf, pdp);
    break;

    case REQUEST_MBMS_ACTIVATION_REJ:      
      status = pack_req_mbms_activate_rej(length, ext_buf, pdp);
    break;
...
)
{
  ...
  ** --------------------------------------------------------------*/
  ...
  {
    ext_buf[msg_length++] = (IEI_DEVICE_PROPERTIES | pdp->low_priority_signaling);
  }
...
{  
  ...
  if (pdp->is_mbms == TRUE)
  {
      // Check if MBMS Protocol Configuration options field is included MBMS_PENDING
      // as of now this is just a dummy one with length 1 and data contents
      if (pdp->mbms_prot_cnfg.valid)
      {
        // pack the IE value of Optional Protocol Configurations
        ext_buf[mesg_length++] = IEI_MBMS_PROT_CONFIG_OPT;
        ext_buf[mesg_length++] = 0x1;
        // Pack the IEI length : This is temporarily set to '0' by upper layers.
        ext_buf[mesg_length++] = pdp->mbms_prot_cnfg.data;
      }
  }
  else
  {
...
)
{
  ...
  if ((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL10) &&
      (reg_sim_per_subs_chk_low_pri_cfg((sys_modem_as_id_e_type)mm_sub_id) == TRUE))
  {
    ext_buf[mesg_length++] = (IEI_DEVICE_PROPERTIES | pdp->low_priority_signaling);
  }
...
DESCRIPTION
  This function packs the MBMS context status for 128 NSAPIs(128-254) in 16*8 bits
...
byte sm_pack_mbms_context_status(byte *ext_buf)
{
  word nsapi_index;
  boolean status;
  word i;
  word last_active_mbms_context_nsapi = 0;

  //Reset the buffer
  (void)memset(ext_buf, 0, MAX_MBMS_STATUS_CONTEXT_LENGTH); // MBMS_PENDING
  //Now Pack the bits
  for (nsapi_index = MIN_MBMS_NSAPI_NUM; nsapi_index < MAX_NSAPI_NUM; nsapi_index++)
  {
    if ((nsapi_list[nsapi_index] != INVALID_ID) && 
        pdp_list[nsapi_list[nsapi_index]] != NULL &&
        (pdp_list[nsapi_list[nsapi_index]]->is_mbms == TRUE) )
    {
      status = (pdp_list[nsapi_list[nsapi_index]]->pdp_state == MBMS_ACTIVE)? TRUE: FALSE; // MBMS_PENDING check other states

      //Now pack this status
      i = (nsapi_index-MIN_MBMS_NSAPI_NUM)/8 ;
      b_packb(status, ext_buf+i, (word)((BYTE_LEN_INBITS - 1) - (128*(i+1) -nsapi_index)), 1);
      if (status == TRUE)
      {
        last_active_mbms_context_nsapi = nsapi_index;
      }
    }
  }

  if (last_active_mbms_context_nsapi == 0)
    return (byte) last_active_mbms_context_nsapi;

  return (byte) ((last_active_mbms_context_nsapi-MIN_MBMS_NSAPI_NUM)/8 +1);
}
  ...
  *recd_msg = (sm_layer_msg_T)ext_buf[get_message_data_index (ext_buf)-1];
  ...
  switch (*recd_msg)
  {
    ...
    case SM_STATUS:
      ...
      /*Assign default SM cause*/
      pdp->status_message_cause = 0x22;
      if( length > 2)
      {
        /*Get the SM cause*/
    ...
    case REQUEST_MBMS_ACTIVATION:
        status = unpack_request_mbms_activation(length, ext_buf, pdp);
    break;

    case ACTIVATE_MBMS_ACCEPT:
        status = unpack_activate_mbms_accept(length, ext_buf, pdp);
    break;

    case ACTIVATE_MBMS_REJECT:
      status = unpack_activate_mbms_reject(length, ext_buf, pdp);
    break;
    #endif
...
)
{
  ...
  ** --------------------------------------------------------------------*/
  ...
  if (msg_length < SM_ACTIVATE_PDP_CONTEXT_ACCEPT_LEN_REL97)
  {
  ...
  if ((msg_index + iei_length) >= msg_length)
  {
    return L3_INVALID_MANDATORY_INFO; 
  }
  ...
  ** ------------------------------------------------------------------- */
  ...
  while (msg_index < msg_length)
  {
    ...
    //Check if Length IE is present or not; If not present no need to enter Switch case
    ...
    ** --------------------------------------------------------------- */
    ...
    {  
      case IEI_PDP_ADDRESS:
        ...
        if (((optional_ie_index == 0)&& (iei_length > 0)) && 
      ...
      case IEI_PROTOCOL_CFG_OPTIONS:
        ...
        if (((optional_ie_index <= 1)&& (iei_length > 0)) && 
              ((msg_index + iei_length) < msg_length))
        {
      ...
      case IEI_PACKET_FLOW_IDENT:
        ...
        if (((optional_ie_index <= 2)&& (iei_length > 0)) && 
              ((msg_index + iei_length) <= msg_length))
        {
      ...
      case IEI_SM_CAUSE:
        ...
        if (((optional_ie_index <= 3)&& (iei_length > 0)) && 
              ((msg_index + iei_length) <= msg_length))
        {
...
)
{
  ...
  {  
    ...
    if( (msg_index < msg_length) && (ext_buf[msg_index] == (IEI_TI_INDICATOR | 0x01)))
    {
  ...
  ** ------------------------------------------------------------------- */
   ...
   while (msg_index < msg_length)
   {
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      break;
#ifdef FEATURE_MBMS
      case IEI_MBMS_PROT_CONFIG_OPT:

        iei_length = ext_buf[msg_index++];
        if (((optional_ie_index <= 1)&& (iei_length > 0)) && 
              ((msg_index + iei_length) <= msg_length))
        {
          pdp->mbms_prot_cnfg.valid = TRUE;
          pdp->mbms_prot_cnfg.data = ext_buf[msg_index++];
          optional_ie_index = 1;
        }
        else
        {
...
)
{
  ...
  ** ------------------------------------------------------------------- */
  ...
  pdp->sm_gprs_type3_timer.valid = FALSE;
  pdp->reattempt_ind.valid = FALSE;
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      if (((optional_ie_index <= 2)&& (iei_length > 0)) && 
            ...
            ((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12)
              #ifdef FEATURE_LTE
              ||(emm_3gpp_spec_ver >=LTE_3GPP_REL12)
              #endif
             )            
          )
      {
         status = unpack_reattempt_ind(ext_buf, &msg_index, pdp);   
    ...
    ** --------------------------------------------------------------- */
    ...
    {  
      ...
      case IEI_PROTOCOL_CFG_OPTIONS:
        ...
        if (((optional_ie_index <= 4 )&& (iei_length > 0)) &&
              ((msg_index + iei_length) < msg_length))
        {
          status = unpack_proto_config(ext_buf, &msg_index, pdp);   
...
)
{
  ...
  // Check if the received message has the minimum length required.
  if (msg_length < SM_MODIFY_PDP_CONTEXT_REQUEST_LEN_REL97)
  {
    MSG_ERROR_DS_0(SM_SUB,"Invalid mandatory field error");
    return L3_INVALID_MANDATORY_INFO; 
  }
...

{
  ...
  iei_length = (uint16)(ext_buf[msg_index++]);

  //Check the range for IEI length; shall not be less than ONE and more than 255.
  if (iei_length < ((MIN_TFT_IE_LEN)) || iei_length > ((MAX_TFT_IE_LEN)))
  {
     //MSG_HIGH_DS_1(SM_SUB, "unpack_tft : TFT  IE Length : %d, is not in the range !! ", iei_length);
     return L3_PARSE_INVALID;
  }
  iei_length = iei_length - 1;
  ...
  MSG_HIGH_DS_3(SM_SUB, "unpack_tft : IE Length : %d, num_filters : %d, Operation_Code : %d", iei_length+1, num_filters, operation_code);
  ...
  if (operation_code == 5)
  {
    ...
    {
       //For OP CODE 5; we need to have one mandatory byte for each filter; 
       if( iei_length  > i )
       {
  ...
  if (operation_code == 1 || operation_code == 3 || operation_code == 4)
  {
    ...
    for(i=0; i < num_filters; i++)
    {
		//For OP CODE 1 or 3 or 4; we need to have three mandatory bytes; if not parse failure
        if( (iei_length - ( msg_index - start_index )) < ((num_filters-i)*3) )
        {
           //MSG_HIGH_DS_0(SM_SUB, "unpack_tft : More number of filters than the actual IE length ");
           //Skip the remaining parsing
           //msg_index = (uint16)(msg_index + (iei_length - ( msg_index - start_index )) );
            return L3_PARSE_INVALID;
        }
        ...
        if ((pdp->tft.filter_list[i].content_length > MAX_FILTER_CONTENT_LENGTH) || ( ((pdp->tft.filter_list[i].content_length + msg_index ) - start_index) > iei_length ) )
        {
            //MSG_HIGH_DS_3(SM_SUB, "unpack_tft : Filter Content length : %d is more than the actual IE length, Start index : %d, Msg_index : %d ", pdp->tft.filter_list[i].content_length, start_index, msg_index );
            //Skip the remaining parsing
            //msg_index = (uint16)(msg_index + (iei_length - ( msg_index - start_index )) );
            return L3_PARSE_INVALID;
         }
...
byte unpack_qos (byte* ext_buf, pdp_context_info *pdp, uint16 *msg_length)
{
  ...
  //Set the current index : Which is the index of Length IE
  msg_index = (uint16)(*msg_length);
  ...
  // Check if Current BYTE and next two bytes can be decoded
  if ( ((cur_pos/8 + 2 ) - msg_index) <= ie_length) 
  {
  ...
  // Check if Current BYTE and next SEVEN bytes can be decoded
  if ( ( ie_length > 3 ) && ( ((cur_pos/8 + 7 ) - msg_index) <= ie_length) )
  {  
...
#ifdef FEATURE_REL5   
    if ((ie_length > 0xB )&& ( ((cur_pos/8) - msg_index) <= ie_length) )
    {
      //unpack OCTET 13
      ...
      if ((ie_length > 12) && ( ((cur_pos/8) - msg_index) <= ie_length) )
      {
        //unpack OCTET 14
      ...
      if ((ie_length > 13) && ( ((cur_pos/8) - msg_index) <= ie_length) )
      {
        //unpack OCTET 14
     ...
     if(mm_nv_wcdma_hsupa_category >= MM_RRC_WCMDA_DC_HSUPA_CATEGORY)
     {
      if ((ie_length > 14) && ( ((cur_pos/8) - msg_index) <= ie_length) )
      {
        //unpack OCTET 15
      ...
      if ((ie_length > 15) && ( ((cur_pos/8) - msg_index) <= ie_length) )
      {
        //unpack OCTET 16
...

{
  ...
  pdp->sm_gprs_type3_timer.valid  = TRUE ;  
...
#ifdef FEATURE_LTE_REL12
  if (pdp->sm_gprs_type3_timer.active == TRUE)
#endif
  {
...
DESCRIPTION
  This function (decodes) unpacks the re-attempt indicator IE.
...
===========================================================================*/
xlate_status_T unpack_reattempt_ind
(
  byte* ext_buf,
  uint16 *index,
  pdp_context_info* pdp
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* ------------------------------------------------
  ** The Re-attempt indicator information element  is as shown 
     (spec 3GPP TS 24.301, section 9.9.4.13A) 
   
               8   7   6   5   4   3    2   1
              -------------------------------
    Octet1   |   Re-Attempt Indicator IEI    |
              -------------------------------
    Octet2   | Length of Reattempt Ind IE    |
              -------------------------------
    Octet3   | 0   0   0   0   0   0   0   1 |
              -------------------------------

  ** ------------------------------------------------ */

  /* Read reattempt indicator content octet */
  pdp->reattempt_ind.reattempt_ind_content = ext_buf[msg_index++];
  
  pdp->reattempt_ind.valid = TRUE;
...
)
{
  ...
  if (sm_msg == REQUEST_PDP_ACTIVATION_REJ ||
       ...
       #ifdef FEATURE_MBMS
       || sm_msg == REQUEST_MBMS_ACTIVATION_REJ
       #endif
       )
  {
    ti_flag = TRUE;
...
DESCRIPTION
  This function (decodes) unpacks REQUEST_MBMS_ACTIVATION sent by Network
...
===========================================================================*/
xlate_status_T unpack_request_mbms_activation
(
  uint16 msg_length,
  byte* ext_buf,
  pdp_context_info *pdp
)
{
  ...
  MSG_MED_DS_0( SM_SUB,"MBMS: Unpack REQUEST_MBMS_ACTIVATION message");

  // Check if the received message has the minimum length required.
  if (msg_length < SM_REQUEST_MBMS_CONTEXT_ACTIVATION_LEN)
  {
    MSG_ERROR_DS_0(SM_SUB,"MBMS: Invalid mandatory field error");
    return L3_INVALID_MANDATORY_INFO;
  }
  ...
  ** --------------------------------------------------------------------*/
  ...
  //unpack the linked NSAPI
  linked_nsapi = ext_buf[msg_index++];
  pdp->linked_ti = (byte)GET_TRANSID(linked_nsapi);
  ...
  // Unpack the PDP address IE
  status = unpack_pdp_addr (ext_buf, &msg_index, pdp); // check used pdp_addr instead of mbms_addr

  pdp->pdp_addr_alloc_type = (pdp->pdp_addr.pdp_addr_len)
                              ?PDP_ADDR_STATIC:PDP_ADDR_DYNAMIC;

  // MBMS_PENDING very CRITICAL
  if (pdp->pdp_addr.pdp_type_num == IPV4_TYPE)
  {
    pdp->mip.type = SYS_IPV4_ADDR;
    pdp->mip.addr.v4 = (pdp->pdp_addr.address[0] << 24) +
                                  (pdp->pdp_addr.address[1] << 16) +
                                  (pdp->pdp_addr.address[2] << 8) +
                                  (pdp->pdp_addr.address[3]);
  }
  MSG_HIGH_DS_2(SM_SUB,"MBMS: MIP type = %d, addr = %u ", pdp->mip.type ,pdp->mip.addr.v4 );
  if (status != L3_PARSE_SUCCESS)
  {
    return status;
  }
  ...
  if (msg_index == msg_length)
  {
    MSG_HIGH_DS_0(SM_SUB,"MBMS: APN missing in the Request Message");
    return L3_INVALID_MANDATORY_INFO;
  }
  ...
  else if ((msg_index + iei_length) <= msg_length)
  {
    pdp->apn_name.valid = TRUE;
    pdp->apn_name.apn_addr_len = iei_length;
    if (pdp->apn_name.apn_addr_len > MAX_APN_ADR_LEN)
    ...
    memscpy(pdp->apn_name.address, sizeof(pdp->apn_name.addres),
            &ext_buf[msg_index], pdp->apn_name.apn_addr_len );
    msg_index = (uint16)(msg_index + iei_length);
  }
  else
  ...
  ** Process non imperative IEs in the message, if included.
  ** --------------------------------------------------------------------*/
  pdp->mbms_prot_cnfg.valid = FALSE;
  optional_ie_index = 0;
  ...
  while (msg_index < msg_length)
  {
    ...
    {
      /* ------------------------------------------------------------------
      ** Optional IEs listed in order specified in TS 24.008 Table 9.5.25
      ** --------------------------------------------------------------- */

        // unpack MBMS Protocol Configuration Options
        // as of now the length is set to 0x1, and data is of length one byte,
        // change incase there is any inconsistency.
      case IEI_MBMS_PROT_CONFIG_OPT:
...
DESCRIPTION
  This function (decodes) unpacks the ACTIVATE_MBMS_ACCEPT message received
  from the Network.
...
===========================================================================*/
xlate_status_T unpack_activate_mbms_accept
(
  uint16 msg_length,
  byte* ext_buf,
  pdp_context_info *pdp
)
{
  ...
  MSG_HIGH_DS_1( SM_SUB,"MBMS: Unpack ACTIVATE_MBMS_ACCEPT message, msg_length=%d",msg_length);

  // Check if the received message has the minimum length required.
  if (msg_length < SM_ACTIVATE_MBMS_CONTEXT_ACCEPT_LEN) // later this is changed to 8 from 7
  {
    MSG_HIGH_DS_0(SM_SUB,"MBMS: Invalid mandatory field error");
    return L3_INVALID_MANDATORY_INFO;
  }
  ...
  ** --------------------------------------------------------------------*/
  ...
  //skip IEI TMGI Identity
  //msg_index++;
      iei_length = ext_buf[msg_index++];
      pdp->tmgi.mbms_serv_id[0] = ext_buf[msg_index++];
      pdp->tmgi.mbms_serv_id[1] = ext_buf[msg_index++];
      pdp->tmgi.mbms_serv_id[2] = ext_buf[msg_index++];

      MSG_HIGH_DS_1( SM_SUB,"MBMS: iei_length=%d",iei_length);
//As per spec 24.008 in section 10.5.6.13
//If octet 6 is included, then octets 7 and 8 shall also be included.
      if (iei_length > 3)
      {
        if (msg_index + 3 > msg_length)
        {
            MSG_HIGH_DS_0(SM_SUB,"MBMS: Invalid mandatory field error: Missing plmn_id");
            return L3_INVALID_MANDATORY_INFO;
        }
        pdp->tmgi.valid = TRUE;
        pdp->tmgi.plmn_id.identity[0] = ext_buf[msg_index++];
        pdp->tmgi.plmn_id.identity[1] = ext_buf[msg_index++];
        pdp->tmgi.plmn_id.identity[2] = ext_buf[msg_index++];
      }

  // unpack the LLC-SAPI field. this field is not used for UMTS. later IMP relate this with NSAPI received in Req msg
  if (msg_index < msg_length)
  {
    pdp->llc_sapi.valid = TRUE;
    pdp->llc_sapi.sapi = ext_buf[msg_index++];
  ...
  {
    MSG_HIGH_DS_0(SM_SUB,"MBMS: Invalid llc_sapi mandatory field error");
    return L3_INVALID_MANDATORY_INFO;
  }
  ...
  while (msg_index < msg_length)
  {
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      ** --------------------------------------------------------------- */
        ...
        // unpack MBMS Protocol Configuration Options
        // as of now the length is set to 0x1, and data is of length one byte, change incase there is any inconsistency.
      case IEI_MBMS_PROT_CONFIG_OPT:
        ...
        {
          pdp->mbms_prot_cnfg.valid = TRUE;
          pdp->mbms_prot_cnfg.data = ext_buf[msg_index++]; // check how to capture other bytes in the IE
          optional_ie_index = 1;
        }
        else
        {
...
DESCRIPTION
  This function (encodes) packs MBMS_ACTIVATE_MESG sent to the Network.
...
===========================================================================*/
xlate_status_T pack_mbms_activate_mesg
(
  word* length, byte* ext_buf,
  const pdp_context_info* pdp
)
{
  ...
  ** return a L3_PARSE_INVALID to the caller.
  ** --------------------------------------------------------------*/
  if ((!pdp->nsapi.valid) || (!pdp->llc_sapi.valid) || (!pdp->apn_name.valid) ||
      (!pdp->pdp_addr.valid) || (!pdp->mbms_bear_cap.valid))
  {
    MSG_ERROR_DS_0( SM_SUB,"MBMS: Mandatory Field is not Set in MBMS ACTIVATE MSG");
    return L3_PARSE_INVALID;
  }
  ...
  ** --------------------------------------------------------------*/
  ...
  //Pack the PD, TI and the Message type
  pack_pd_ti_msgtype (ext_buf, pdp->trans_id, ACTIVATE_MBMS_REQUEST);
  msg_length += get_message_data_index (ext_buf);  // Move the message length
  ...
  // Pack the Requested Bearer Capabilities
  ext_buf[msg_length++] = IEI_MBMS_BEARER_CAP;
  ext_buf[msg_length] = 0x1;

#ifdef FEATURE_REL5
  if ((is_umts_mode()) && 
      (ue_revision == UE_REV_REL99) && 
      (mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL5) )
  {
    ext_buf[msg_length] = 0x2;
  }else
#endif

    msg_length++;

  ext_buf[msg_length++] = pdp->qos.max_bit_rare_downlink;

  #ifdef FEATURE_REL5
  if ((is_umts_mode()) && 
      (ue_revision == UE_REV_REL99) && (mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL5) )
    {
    ext_buf[msg_length++] = pdp->qos.ext_max_bit_rate_downlink;
    }
  #endif
  ...
  ext_buf[msg_length++] = pdp->pdp_addr.pdp_type_org;
  ext_buf[msg_length++] = pdp->pdp_addr.pdp_type_num;
  memscpy(&ext_buf[msg_length], (gmm_max_L3_msg_length(MM_PS_DOMAIN) - msg_length) , pdp->pdp_addr.address,
            pdp->pdp_addr.pdp_addr_len);

  msg_length = (uint16)(msg_length + pdp->pdp_addr.pdp_addr_len);


  //ext_buf[msg_length++] = IEI_ACC_POINT_NAME;
  ext_buf[msg_length++] = (byte)pdp->apn_name.apn_addr_len;
  memscpy(&ext_buf[msg_length], (gmm_max_L3_msg_length(MM_PS_DOMAIN) - msg_length), pdp->apn_name.address,
    pdp->apn_name.apn_addr_len );
  msg_length = (uint16)(msg_length + pdp->apn_name.apn_addr_len);
  ...
  ** --------------------------------------------------------------*/
  ...
  {
    // pack the IE value of Optional Protocol Configurations
    ext_buf[msg_length++] = IEI_MBMS_PROT_CONFIG_OPT;
    ext_buf[msg_length++] = 0x1;
    // Pack the IEI length : This is temporarily set to '0' by upper layers.
    ext_buf[msg_length++] = pdp->mbms_prot_cnfg.data;
...
DESCRIPTION
  This function (encodes) the REQ_MBMS_ACTIVATE_REJ message sent to Network.
...
===========================================================================*/
xlate_status_T pack_req_mbms_activate_rej
(
  word* length, byte* ext_buf,
  const pdp_context_info* pdp
)

{
  ...
  //Pack the PD, TI and the Message type
  pack_pd_ti_msgtype (ext_buf, pdp->trans_id, REQUEST_MBMS_ACTIVATION_REJ);
  msg_length += get_message_data_index (ext_buf);  // Move the message length

  //Pack the SM cause for rejection
  ext_buf[msg_length++] = (byte)pdp->current_cause;

    // Check if MBMS Protocol Configuration options field is included
  // as of now this is just a dummy one with length 1 and data contents
  if (pdp->mbms_prot_cnfg.valid)
  {
    // pack the IE value of Optional Protocol Configurations
...
DESCRIPTION
  This function (decodes) unpacks ACTIVATE_MBMS_REJECT message sent by Network
...
===========================================================================*/
xlate_status_T unpack_activate_mbms_reject
(
  uint16 msg_length,
  byte* ext_buf,
  pdp_context_info *pdp
)
{
  ...
  // Check if the received message has the minimum length required.
  if (msg_length < SM_ACTIVATE_MBMS_CONTEXT_REJECT_LEN)
  {
    MSG_ERROR_DS_0(SM_SUB,"MBMS: Invalid mandatory field error");
    return L3_INVALID_MANDATORY_INFO;
  }

  MSG_MED_DS_0( SM_SUB,"MBMS: Unpack REQUEST_MBMS_REJECT message");
  ...
  while (msg_index < msg_length)
  {
    ...
    switch (ext_buf[msg_index++])
    {
      ...
      case IEI_MBMS_PROT_CONFIG_OPT:
        ...
        {
          pdp->mbms_prot_cnfg.valid = TRUE;
          pdp->mbms_prot_cnfg.data = ext_buf[msg_index++]; // check how to capture other bytes in the IE later
          optional_ie_index = 1;
        }
        else
        {
...
xlate_status_T unpack_linked_ti(byte* ext_buf, pdp_context_info *pdp, uint16* cur_pos)
{
  ...
  ie_len = ext_buf[msg_index];
  msg_index++; //incrementing length feild
  
  // Check the ie_len for minimum before unpacking TI/TI flag
  if(ie_len < 1)
  {
    return L3_PARSE_INVALID_PD_TI;
  }
  ...
  if (trans_id == 0x07)
  {
    // Check the ie_len before accessing another byte
    if(ie_len < 2) 
    {
      return L3_PARSE_INVALID_PD_TI;
    }
