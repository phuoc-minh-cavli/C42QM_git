...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/sm/src/esm_proc_state.c#1 $
...
(
  struct esm_bearer_resource_proc* proc_ptr, const emm_pdn_connectivity_req_s_type* msg_ptr   /*!< REQUIRED short parameter description */
)
{
  ...
  if((msg_ptr->pdn_data.ext_protocol_conf_opt.valid == TRUE)&&
     (msg_ptr->pdn_data.ext_protocol_conf_opt.dsm_ptr != NULL))
  {
    proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.ext_protocol_conf_opt.dsm_ptr = \
                                                    msg_ptr->pdn_data.ext_protocol_conf_opt.dsm_ptr;
    dsm_free_packet((dsm_item_type**)&(proc_ptr->pending_msg_ptr->esm_out_msg.pdn_connectivity_req.ext_protocol_conf_opt.dsm_ptr));
...
(
  struct esm_bearer_resource_proc* proc_ptr, 
  const emm_bearer_resrc_alloc_req_s_type* msg_ptr   /*!< REQUIRED short parameter description */
)
{
...
(
  struct esm_bearer_resource_proc* proc_ptr,
  emm_pdn_disconnect_req_s_type* dat_ptr
)
{
...
)
{
  ...
  */
  ...
  if(proc_ptr->chk_on_attach_def_bc_setup_ptr() == TRUE)
  {
    ...
    nas_assert_check((boolean)(status == TRUE));
  }
