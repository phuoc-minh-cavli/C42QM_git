...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/rabm/src/rabmtimers.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void rabm_start_timer( word timer_id )
{
  ...
  {
     #ifdef FEATURE_DUAL_DATA
       status = gs_start_timer( GS_TIMER_QUEUE_SM, (byte) ( (timer_id&0x0f)| ((0x01 & sm_as_id)<<4)), 
                             rabm_timer_expiry, (dword)TIME_RAB_EST_PENDING, (byte)0 );
     #else
           status = gs_start_timer( GS_TIMER_QUEUE_SM, (byte) (timer_id&0x0f), 
                             rabm_timer_expiry, (dword)TIME_RAB_EST_PENDING, (byte)0 );
     #endif
