...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/rabm/src/rabmtask.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
boolean rabm_get_as_id( rabm_cmd_type* cmd_ptr )
{
  ...
  mm_add_message_to_debug_buffer(cmd_ptr->header.message_set,
...
#ifdef FEATURE_MBMS
  if (cmd_ptr->header.cmd_id == RABMAS_RAB_ESTABLISH_IND ||
      cmd_ptr->header.cmd_id == RABMAS_RAB_RELEASE_IND ||
      cmd_ptr->header.cmd_id == RABMAS_STATUS_IND)
  {
    rab_id =  cmd_ptr->cmd.rrc_establish_ind.rabid;
    if (rab_id >= MIN_MBMS_NSAPI_NUM)
    {
      MSG_HIGH_DS_1(RABM_SUB,"RABM Received RRC CMD: MBMS: Received RRC cmd  for RAB id %d", rab_id);
      return;
    }
  }
#endif
...
boolean rabm_rabid_validate(rab_query_T query_type, byte rab_id )
{
  ...
  #ifdef FEATURE_MBMS
  if ((mbms_rab_id == MAX_NSAPI_NUM) ||
    (mbms_rab_id >=  MIN_MBMS_NSAPI_NUM && 
    GET_TRANSID_PER_SUB(mbms_rab_id, as_id) != INVALID_ID))
  {
      MSG_HIGH_DS_1(RABM_LOCAL_SUB,"MBMS: RAB Validation for MBMS RAB:%d", mbms_rab_id);
      status = TRUE;
  } else
  #endif
...
boolean rabm_rabid_validate(rab_query_T query_type, byte rab_id )
{
  ...
  if ((mbms_rab_id == MAX_NSAPI_NUM) ||
    (mbms_rab_id >=  MIN_MBMS_NSAPI_NUM && 
    GET_TRANSID(mbms_rab_id) != INVALID_ID))
  {
      MSG_HIGH_DS_1(RABM_SUB,"MBMS: RAB Validation for MBMS RAB:%d", mbms_rab_id);
      status = TRUE;
  } else
  #endif
...
void rabm_send_gmmrabm_reestablish_req(byte rab_id)
{
 ...
 /*--------------------------------------------------------------
  ...
  If the request is not to re-establish RABs for emergency bearer services, the RRC establishment cause 
  shall be set to either 'Originating Conversational Call' or 'Originating Streaming Call' or 
  'Originating Interactive Call' or 'Originating Background Call ' - depending on the Traffic Class in QoS 
  of the "most demanding" Traffic Class, considering all active PDP contexts. (see Note 1)
  ...
  {
    //Get the most demanding traffic class WITH OUT including the PDP contexts to be activated
    traffic_class = sm_get_most_demanding_traffic_class(FALSE);
  }
  else
  {
    //Get the most demanding traffic class WITH including the PDP contexts to be activated
...
void rabm_send_combined_gmmrabm_reestablish_req()
{
 ...
 /*--------------------------------------------------------------
  ...
  {
    //Get the most demanding traffic class WITH OUT including the PDP contexts to be activated
    rrc_est_cause = convert_to_rrc_est_cause(sm_get_most_demanding_traffic_class(FALSE));  
  }
  else
  {
    //Legacy Behaviour : Get the most demanding traffic class WITH including the PDP contexts to be activated
    rrc_est_cause = convert_to_rrc_est_cause(sm_get_most_demanding_traffic_class(TRUE));
  } 
