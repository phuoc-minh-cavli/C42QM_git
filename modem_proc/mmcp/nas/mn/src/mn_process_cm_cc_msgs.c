...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_process_cm_cc_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
         ...
         else
         {
           if(connection_id < MAXNO_CC_TRANSACTIONS)
           {
                        ...
                        cc_transaction_information_p[active_connection_id]->cc_state != CC_STATE_U10) )
              {
                  ...
                  if(mn_call_information_p[active_connection_id] != NULL)
                  {
...
VOID_FUNC   MN_process_cm_mo_end_x_call_req(cm_n_x_send_req_T *message)
{
  ...
  {
    MSG_ERROR_DS_2(MN_SUB,"=MNCNM= Received Invalid as_id=%d, mn_as_id=%d", message->as_id, mn_as_id); 
    cm_cc_cause.present = FALSE;
    cm_cc_reject.present = TRUE;
    cm_cc_reject.rej_type = CNM_MN_REJECT_CAUSE;
    cm_cc_reject.rej_value = CNM_INVALID_USER_DATA;
   /* Inform the CM */
    MN_put_cm_mt_end_call_ind(message->connection_id, 
                             cm_cc_cause, 
#if defined(FEATURE_DUAL_SIM )
                             message->as_id, 
#endif  
