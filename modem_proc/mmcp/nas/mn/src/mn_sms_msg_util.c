...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_sms_msg_util.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
      ...
      case RP_DATA_N:
         {
            ...
            */
            ...
            if (valid_message == TRUE)
            {
               ...
               SM_RL_DATA_IND->sms_verstat_status = sms_verstat_status;
...
SM_RL_REPORT_REQ_T *SM_RL_REPORT_REQ)
{
   ...
   {   
       ...
       if (tpdu_length > 0)
       {
         ...
         for (i = 0; ((i < tpdu_length) && (i < TPDU_SIZE)); ++i)
         {
            rpdu[i+4] = tpdu_ptr[i];
         }
...
#else
        MN_handle_MNSMS_ERROR_IND(&p_mnsms_error_ind->message_header);
#endif        
    ...
    switch ( ims_msg.rpdu[0]&0x07 )
    {
      case RP_DATA_N :
      {
        ...
        if (connection_id == MAXNO_SMS_TRANSACTIONS)
        {
          ...
          memset((void *)&mnsms_est_ind,0,sizeof(MNSMS_EST_IND_T));
          ...
          mnsms_est_ind.sms_verstat_status = ims_msg.ims_verstat_status;
      ...
      case RP_ERROR_N :
      {
        ...
        if(connection_id != MAXNO_SMS_MO_TRANSACTIONS)
        {
          ...
          memset((void *)&mnsms_data_ind,0,sizeof(MNSMS_DATA_IND_T));
           ...
           * send message to MN
           */      
           MN_handle_MNSMS_DATA_IND(&mnsms_data_ind.message_header);
