...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_get_cnm_ss_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
  ...
  if (*ptr == IEI_FACILITY)
  {
    ...
    if (2/*TL*/ + ie_len/*V*/ + sizeof(connection_id_T) + sizeof(sys_modem_as_id_e_type)< msg_len)
    {
      if (*(ptr+2+ie_len) == IEI_CAUSE) /* next ie is cause */
      {
        ...
        memsmove (ptr, (last_octet_ptr - ptr + 1), ptr+2+ie_len, *(ptr+2+ie_len+1)+2); /* move cause ie to front */
