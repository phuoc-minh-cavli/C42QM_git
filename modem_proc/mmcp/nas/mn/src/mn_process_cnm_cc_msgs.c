...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_process_cnm_cc_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
     ...
     {  
        ...
        if(result == TRUE)
        {
           ...
           else
           {
                 ...
                 {
                    if((mn_call_information_p[message_ptr->connection_id] != NULL) && (mn_call_information_p[message_ptr->connection_id]->active_bearer_capability == 0))
                    {
                    ...
                    {
                       if((mn_call_information_p[message_ptr->connection_id] != NULL) &&
                          (((mn_call_information_p[message_ptr->connection_id]->bearer_capability_2.information_transfer_capability != ITC_SPEECH) &&
...
VOID_DATA MN_handle_MNCC_REEST_IND(void)
{
   ...
   if(call_active == TRUE)
   {
      ...
      {
        MSG_HIGH_DS_1(MN_SUB,"=MNCNM= Cannot process MNCC_REEST_IND as mn_call_information_p[%d] is NULL",
                   connection_id);
      }
      else 
      {
...
VOID_FUNC  MN_configure_TCH(MNCC_SYNC_IND_T * message_ptr)
{
    ...
    else
    {
         ...
         (message_ptr->reason == GSM_CHANNEL_MODE_MODIFIED))
      {
          ...
          if(message_ptr->reason == WCDMA_RAB_RECONFIGURED)
             mn_call_information_p[message_ptr->connection_id]->radio_access_bearer_id = message_ptr->radio_access_bearer_id;      
#endif
