...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_put_cm_cc_msgs.c#1 $   
...

{
   ...
   if(notification_indicator == BEARER_CHANGED)
   {
     if(mn_call_information_p[connection_id] != NULL)
     {
       message.rab_id.present = TRUE;
       message.rab_id.radio_access_bearer_id= mn_call_information_p[connection_id]->radio_access_bearer_id;
     }
     else
     {
...

/*
 *   Function name : MN_put_cm_mt_modify_ind
 *   ---------------
 *
 *   Description:
 *   ------------
 *  This function generates the cm_mt_modify_ind message to the 
 *  mmi. Allows modification of call status(ie speech or data).
 *  
 *
 *  Parameters:
 *  -----------
...
VOID_FUNC MN_put_cm_mt_modify_ind( connection_id_T     connection_id,
...

{
   ...
   if(connection_id >= MAXNO_CC_TRANSACTIONS)
   {
      MSG_ERROR_DS_0(MN_SUB,"=MNCNM= ERROR: The connection id is invalid");
     
   }
   else
   {
      /* Set up the message header */
      memset(&message, 0x00, sizeof(cm_mt_modify_ind_T)); 
      message.message_header.message_set = MS_MN_CM;
      message.message_header.message_id = (word)CM_MT_MODIFY_IND;
      message_size = sizeof(cm_mt_modify_ind_T) - sizeof(IMH_T);
      ...
      message.bearer_capability = modify_data_ptr->cm_bearer_capability;

      /* Set up the low layer compatibility */
      if (modify_data_ptr->repeat_indication_llc == 0)
      {
         message.low_layer_compatibility.present  = FALSE;
      }
      else
      {
         message.low_layer_compatibility = 
             modify_data_ptr->cm_low_layer_compatibility;

         message.low_layer_compatibility.present  = TRUE;
      }

      /* Set up the high level compatibility data */
      if (modify_data_ptr->repeat_indication_hlc == 0)
      {
         message.high_layer_compatibility.present  = FALSE;
      }
      else
      {
         message.high_layer_compatibility = 
             modify_data_ptr->cm_high_layer_compatibility;

         message.high_layer_compatibility.present  = TRUE;
      }

      /* Set up the reverse call setup direction */
      message.reverse_call_setup_direction = 
          modify_data_ptr->reverse_call_direction;

      /* Immediate modify indication */
      message.immediate_modification = 
          modify_data_ptr->immediate_modify_ind;

      /* Network-initiated Service Upgrade indication */
      message.nw_initiated_ind = 
          modify_data_ptr->nw_initiated_ind;
      ...
      if ( status != GS_SUCCESS)
      {
         MSG_ERROR_DS_0(MN_SUB,"=MNCNM= ERROR: Send cm_mt_modify_ind message failure");   
      }
