...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_get_cnm_cc_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
MNCC_CALL_PROC_data_T *proc_data_ptr)
{
   ...
   if(message_position < message_length)
   {
      ...
      if(message_ptr->data[data_index] == IEI_BEARER_CAPABILITY)
      {
         ...
         if(message_ptr->data[data_index] == IEI_NETWORK_CC_CAPABILITY)
         {
             proc_data_ptr->network_cc_capability.present = TRUE;
             proc_data_ptr->network_cc_capability.length = 1;
             proc_data_ptr->network_cc_capability.capability = message_ptr->data[data_index+2];
             data_index += 3;
         }  
         else
         {
             proc_data_ptr->network_cc_capability.present = FALSE;
             proc_data_ptr->network_cc_capability.length = NULL;
             proc_data_ptr->network_cc_capability.capability = NULL;
         }
        ...
        case IEI_REDIR_PARTY_SUBADDR:
           {
              ...
              if(ie_length > CM_CALLING_PARTY_SUBADDRESS_LENGTH)
              {
                call_data_ptr->redirect_party_subaddress.length = CM_CALLING_PARTY_SUBADDRESS_LENGTH;
              }
              else
              {
              ...
              {
                 if (index < CM_CALLING_PARTY_SUBADDRESS_LENGTH)
                 {
