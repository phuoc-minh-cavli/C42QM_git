...
<$Header: //components/rel/mmcp.mpss/6.1.10/nas/mn/src/mn_timers.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $>
...
VOID_FUNC MN_start_cc_timer(connection_id_T connection_id, byte message_IEI)
{
   ...
   gs_timer_queue_id_T queue_id = GS_TIMER_QUEUE_MN_CNM;
                     ...
                     byte            sms_timer_id)
{
   ...
   dword       num_milli_seconds,TR1M_value;
   byte        timer_offset;
   timer_id_T  timer_id;
   ...
   {
      TR1M_value = SMS_TR1M_TIMEOUT_VALUE+emm_get_timer_extention_value();
      /* when SMS is over IMS, NAS should start Timerf value if it is greater than TR1M . Other wise, NAS should start TR1M timer */
      num_milli_seconds = ((mo_sms_transport_type == NAS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS) ? (MAX(sip_config_timer_f,TR1M_value )) : TR1M_value );
...
VOID_FUNC MN_handle_timeouts(IMH_T *message)
{
   ...
   if(message_ptr->IMH.message_id == MID_TIMER_EXPIRY)
   {
      ...
      if((message_ptr->timer_id >= MN_CC_TIMER) && (message_ptr->timer_id < MN_SS_TIMER))
      {
         ...
         switch(cc_command)
         {
            case CCE_CALL_HOLD_REQ:
            {
             ...
             if ((connection_id < MAXNO_CC_TRANSACTIONS) && (cc_transaction_information_p[connection_id] != NULL))
             {
...
#ifndef FEATURE_MMCP_DISABLE_CS_USSD
      if( (message_ptr->timer_id >= MN_SS_TIMER) &&
             (message_ptr->timer_id < (MN_SS_TIMER + MAXNO_INVOKE_IDS)) )
      {
         ...
         switch(ss_code)
         {
#ifndef FEATURE_NAS_DISABLE_SUPL_SERVICES
#ifndef FEATURE_NAS_DISABLE_VOICE
		  case holdMPTY:
            {
      ...
      if (message_ptr->timer_id == (MN_SMS_TIMER + TRAM_TIMER_OFFSET))  /* TRAM */
      {
