...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cb/src/cb_task.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#ifdef FEATURE_MBMS
   if (requested_mask & MSM_CMD_Q_SIG)
   {
     (void) rex_clr_sigs( cb_tcb_ptr, MSM_CMD_Q_SIG );

     if (q_cnt( &msm_cmd_q ) )
     {
        queue_mask |= MSM_CMD_Q_SIG;
     }
   }
...
#ifdef FEATURE_MBMS
      if (rex_signals_mask & MSM_CMD_Q_SIG)
      {
         (void) rex_clr_sigs( cb_tcb_ptr, MSM_CMD_Q_SIG );

         if (q_cnt( &msm_cmd_q ) == 0)   /* No elements in queue ? */
         {
            rex_signals_mask &= ~MSM_CMD_Q_SIG;  /* Turn off bit */
         }
