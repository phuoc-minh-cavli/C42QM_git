...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cb/src/cb_schedule.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
  ...
  else
  {
     ...
     else
     {
       ...
       for(dindex=0;dindex<length_of_serial_number_list;dindex++)
       {
         ...
         sfn_found = cb_search_sfn_scheduling_table(scheduling_table, sfn, scheduling_period_length, serial_number);
         MSG_HIGH_DS_4(cb_as_id,"serial number = %x appended to SFN = %x, CTCH_bs_index = %x, sfn_found = %d",serial_number,sfn,CTCH_bs_index,sfn_found);
       }
     }
  }
}
...
)
{
  ...
  MSG_HIGH_DS_1(cb_as_id,"BMC: Sending CB_SCHEDULING_REQ(CTCH_DRX_MODE) request with length = %d :",scheduling_command_length) ;
...
)
{
  ...
  MSG_HIGH_DS_3(cb_as_id,"BMC: Processing Scheduling information,arrival_sfn = %ul, Offset CTCH BS index = %d and Sch Period length = %d",
                          arrival_sfn,offset_ctch_bs_index, cb_bmc_scheduling_period_length);
...
)
{
  ...
  MSG_HIGH_DS_4(cb_as_id,"CB DRX: scheduling_table = %x, scheduling_period_length is %d, Processing Scheduling table (%lu to %lu)",
                                  scheduling_table,scheduling_period_length,next_first_sfn_sch_prd, next_last_sfn_sch_prd);
...
)
{
  ...
  */
  ...
  MSG_HIGH_DS_2(cb_as_id,"CB DRX: Begin slot = %d, End Slot = %d",begin_scheduling_slot,end_scheduling_slot);
  ...
  MSG_HIGH_DS_3(cb_as_id,"CB DRX: Processing Scheduling information, Slots %d-%d New#%d",
  ...
  MSG_HIGH_DS_6(cb_as_id,"CB DRX: msgbitmap[0-5]- 0X%02X%02X%02X0X%02X%02X%02X",
                 *(scheduling_page_buff+CB_NEW_MSG_BITMAP_OFFSET),
