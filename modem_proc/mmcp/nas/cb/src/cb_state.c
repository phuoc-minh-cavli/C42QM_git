...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cb/src/cb_state.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
  ...
  switch (cur_state->state)
  {
    ...
    case CB_ACTIVE_STATE :
      if ((!cb_supported) || (!cb_service))
      {
        ...
        MSG_HIGH_DS_1(cb_as_id,"CB: Moved into IDLE, COLLECTION INITIATED substate, bmc_rx_suspended = %d",bmc_rx_suspended) ;
#ifdef FEATURE_UMTS_BMC
  ...
  switch (cur_state->state)
  {
    ...
    case CB_ACTIVE_STATE :
      if (!cb_supported)
      {
        ...
        MSG_HIGH_DS_0(cb_as_id,"CB: Moved into IDLE, COLLECTION INITIATED substate ") ;
#ifdef FEATURE_UMTS_BMC
        MSG_HIGH_DS_1(cb_as_id," bmc_rx_suspended = %d",bmc_rx_suspended);
        if (current_rat == CB_UMTS_MODE && bmc_rx_suspended)
        {
          bmc_rx_suspended = FALSE;
        }
...
)
{
  ...
  MSG_HIGH_DS_6(cb_as_id,"BMC: PLMN %X %X %X, LAC %X %X, CELL ID %X",
                               incoming_plmn[0],incoming_plmn[1],incoming_plmn[2],
                               incoming_lac[0],incoming_lac[1],
                               *(dword *)incoming_cell_id) ;
...
)
{
  ...
  switch (cb_incoming_primitive->command.message_header.message_id)
  {
    case RR_CELL_CHANGE_IND :
      ...
      MSG_HIGH_DS_1(cb_as_id,"CB: Received RR_CELL_CHANGE_IND, cb_gsm_feature_disabled=%d", cb_gsm_feature_disabled) ;
...
)
{
  ...
  gs_clear_timer(GS_TIMER_QUEUE_CB, timer_id);
...
)
{
  ...
  MSG_HIGH_DS_1(cb_as_id,"CB: Received DL_RLC_BLOCK_IND in %d state",cur_state->state) ;
  if((cur_state->state == CB_IDLE_STATE)
#ifdef FEATURE_WCDMA_ETWS
     && (cb_etws_state.state == CB_IDLE_STATE)
#endif
     ...
     )
  {
      ...
      for (pdu_index = 0; pdu_index < pdu_cnt_local; pdu_index++)
      {
        ...
        MSG_HIGH_DS_2(cb_as_id,"DSM item rlc_pdu_ptr = %x, item_ptr = %x",rlc_pdu_ptr,item_ptr);
      }
  }
...
)
{
  ...
  new_state = *cur_state ;
  MSG_HIGH_DS_1(cb_as_id,"BMC: Received WL1 message = %d",cb_incoming_primitive->command.message_header.message_id);
