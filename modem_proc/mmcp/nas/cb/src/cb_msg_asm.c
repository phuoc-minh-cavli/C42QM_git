...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cb/src/cb_msg_asm.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
)
{
  ...
  *number_of_pages = paging_block[CB_PAGE_PARAMETER_POS] & 
  ...
  /* Extract Data Coding Scheme */
  *data_coding_scheme = paging_block[CB_DATA_CODING_SCHEME] ;

  MSG_HIGH_DS_7(cb_as_id,"CB: Received GS %02X, Message Code %02X, Update Number %02X, Message ID %02X, %d of %d pages, data coding scheme %X,",
                      *gs,*message_code,*message_id,*update_number,*page_number,*number_of_pages,*data_coding_scheme) ;
...
)
{
  ...
  boolean message_is_requested = FALSE;
  ...
  */
  ...
  message_is_requested = cb_message_is_requested(message_id);
  MSG_HIGH_DS_4(cb_as_id,"CB: Received first paging block with message id = %d message code = %d number_pagers = %d, present_in_search list = %d",
                          message_id, message_code, number_of_pages,message_is_requested);
  if (!message_is_requested)
  {
    return NULL ;
  }
  
  
#if 0 // disable this as CB is disable 
    ...
    case CB_NORMAL_PAGE :
      {
        ...
        else
        {
          ...
          {
            dword x = gs_enquire_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(CB_TIMER_DUP_DET));
          ...
          {
            message_ptr->message_header.current_message_descriptor->timer_offset = cb_dup_timer_value - gs_enquire_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(CB_TIMER_DUP_DET));
            if(cb_next_min_offset == CB_DUP_TIMER_VALUE_INVALID)
            {
              cb_next_min_offset = message_ptr->message_header.current_message_descriptor->timer_offset;
            }
...
)
{
  switch (CB_CALCULATE_PAGING_SLOT(missing_fn))
  {
    ...
    case CB_FIFTH_BLOCK:
      ...
      MSG_HIGH_DS_0(cb_as_id,"CB: First paging block is missing, Next state is CB_RECEIVING_FIRST_PB_STATE") ;
                                                                  ...
                                                                  page_index++)
    {
      ...
      if (new_incoming_page != NULL)
      {
       ...
       (*current_message_ptr)->message_header.
        ...
        MSG_HIGH_DS_4(cb_as_id,"BMC: Processing %d from %d offset into %d, Total length %d",
                                        received_sfn,pdu_offset,page_data_offset,element_data_length) ;
  ...
  //once all page process delete the message descriptor
    ...
    else
    {
       /* Reset Dup_detection_timer if new update number in existing message receieved and timer value is set with this message's timer_offset   */ 
          ...
          {
              (*current_message_ptr)->message_header.current_message_descriptor->timer_offset = cb_dup_timer_value - gs_enquire_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(CB_TIMER_DUP_DET));
              if(cb_next_min_offset == CB_DUP_TIMER_VALUE_INVALID)
              {
                  cb_next_min_offset = (*current_message_ptr)->message_header.current_message_descriptor->timer_offset;
              }
  ...
  *msg_type = rlc_pdu_ptr[CB_BMC_MESSAGE_TYPE_OFFSET] ;
  ...
  if (*msg_type == CBS_SCHEDULE_TYPE)
  {
    ...
    *scheduling_period_length = 
    ...
    MSG_HIGH_DS_1(cb_as_id,"CB: Received Scheduling message with Scheduling Period length = %d",
                                               *scheduling_period_length) ;
  }
  else if (*msg_type == CBS_MESSAGE_TYPE)
  {
    ...
    *number_of_pages = rlc_pdu_ptr[CB_BMC_NUMBER_OF_PAGES_OFFSET] ;
    ...
    MSG_HIGH_DS_6(cb_as_id,"CB: Received Message ID %02X, GS %02X, Message Code %02X, Update Number %02X, data coding scheme %X, Number of pages %d",
                               *message_id,*gs,*message_code,*update_number,*data_coding_scheme,*number_of_pages) ;
...
)
{
  ...
  *serial_number = ((rrc_etws_prim_notification_ind->sr_num[0] << 8) | 
  ...
  MSG_HIGH_DS_2(cb_as_id,"CB ETWS: Received CB Message ID %02X, Serial number ID %02X",*message_id,*serial_number) ;
}
...
)
{
  if(is_update_number_not_changed (existing_message_code->update_number,
                                  new_update_number))
  {
    MSG_HIGH_DS_2(cb_as_id,"CB ETWS: GS indicator: old = %d, new = %d",existing_message_code->gs_indicator,new_gs);
...
static boolean is_etws_message_id_requested( word message_id)
{
  ...
  **Search list
  */
  MSG_HIGH_DS_1(cb_as_id,"CB ETWS: Received Eartquake ETWS ID = %d",message_id);
  switch (message_id) 
  {
...
)
{
    ...
    *message_code = (word)message_code_high | 
    ...
    /* Extract Update Number */
    *update_number = (byte)(serial_number & CB_BMC_UPDATE_NUMBER_MASK) ;
    MSG_HIGH_DS_3(cb_as_id,"CB ETWS: Received CB GS %02X, Message Code %02X, Update Number %02X",
                             *gs,*message_code,*update_number) ;
}
...
)
{
  ...
  */
  message_id_requested = is_etws_message_id_requested(message_id);
  MSG_HIGH_DS_2(cb_as_id,"CB ETWS: Received message id %d , present in search list = %d" ,message_id,message_id_requested);
  ...
  */
  ...
  {
    current_etws_message_code->timer_offset = cb_dup_timer_value - gs_enquire_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(CB_TIMER_DUP_DET));
    if(cb_next_min_offset == CB_DUP_TIMER_VALUE_INVALID)
    {
      cb_next_min_offset = current_etws_message_code->timer_offset;
    }
...
#ifndef FEATURE_BMC_RLC_MULTIPLE_PDU_SUPPORT
        MSG_HIGH_DS_2(cb_as_id,"BMC: Received PDU by RLC (SFN %d), first_pdu_indicator = %d",received_sfn,first_pdu_indicator) ;
#endif
