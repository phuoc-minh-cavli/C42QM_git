...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cb/src/cb_utils.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void cb_process_mode_change_indication(byte val)
{
  ...
  MSG_HIGH_DS_1(cb_as_id,"CB :Receive REG_MODE_CHANGE_STATE_IND with Subs Active = %d",subs_active);
  if(subs_active==0)
  {
    return;
  }
...
)
{
    ...
    while (next_message_code_ptr != NULL)
    {
      if ((next_message_code_ptr->message_code == message_code)
            &&  sys_plmn_match (*(sys_plmn_id_s_type *)cb_current_plmn, 
                          *(sys_plmn_id_s_type *)next_message_code_ptr->plmn))
      {
        break ;
      }
  ...
  current_message_ptr->
            message_header.current_message_descriptor = next_message_code_ptr ;
...
void cb_start_etws_timer( timer_id_T timer_id, dword num_millisecs )
{
   ...
   status = gs_start_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(timer_id), cb_timer_expiry, num_millisecs, (byte) 0 );

   cb_timer_status [timer_id-1]= TIMER_ACTIVE;

   MSG_HIGH_DS_4(cb_as_id, "Start %d, timeout %u:%u, Status = %d",
             timer_id, (word) (num_millisecs /1000L), (word)(num_millisecs % 1000L), status);
}
...
)
{
  ...
  MSG_HIGH_DS_1(cb_as_id,"CB: Sending CB_SKIP_REQ request with length == %d",skip_list_length) ;
...
)
{
  ...
  #error code not present
#else
  status = gs_send_message(GS_QUEUE_UASMS, (void*)&data_ind, TRUE);
#endif
  MSG_HIGH_DS_1(cb_as_id,"CB: CB data indication to WMS, status = %d",status) ;
...
#error code not present
  ...
  */
  ...
  status = gs_send_message(GS_QUEUE_UASMS, (void*)&etws_prim_notification_ind, TRUE);
  MSG_HIGH_DS_2(cb_as_id,"CB ETWS: CB ETWS primary notification to WMS status = %d, Security:%d",status,warning_security_info_present) ;
...
#error code not present
  ...
  */
  ...
  #error code not present
#else
  status = gs_send_message(GS_QUEUE_UASMS, (void*)etws_sec_notification_ind, TRUE);
#endif
  MSG_HIGH_DS_1(cb_as_id,"CB ETWS: CB ETWS Secondary notification to WMS, status = %d",status) ;
...
)
{
  ...
  #error code not present
#else
  status = gs_send_message(GS_QUEUE_UASMS, (void*)&cell_change_ind, TRUE);
#endif
  MSG_HIGH_DS_1(cb_as_id,"CB: Sent CB Cell Change indication to WMS, status = %d",status) ;
...
)
{
  ...
  if (process_search_list)
  {
    ...
    MSG_HIGH_DS_1(cb_as_id,"CB: Search list length = %d",search_list_len);
...
void cb_start_gsm_pseudo_drx_mode(boolean current_cycle_expired)
{
  ...
  MSG_HIGH_DS_6(cb_as_id,"CB PSEUDO DRX: Sending CB_SCHEDULING_REQ(DRX) Curr:%ld(length %d slots) Next:%ld, Scheduling FN list length is %d (O:%d/P:%d):",
  ...
  {
    MSG_HIGH_DS_6(cb_as_id,"CB PSEUDO DRX: Slot# %lu %lu %lu  FN# %lu %lu %lu",
...
)
{
  ...
  #error code not present
#else
  status = gs_send_message(GS_QUEUE_UASMS, (void*)&duplication_ind, TRUE);
#endif
  MSG_HIGH_DS_1(cb_as_id,"CB: Duplication indication to WMS, Sent status = %d",status) ;
...
)
{
  ...
  status = gs_send_message(GS_QUEUE_UASMS, (void*)&cb_error_indication, TRUE);

  MSG_HIGH_DS_2(cb_as_id,"CB: Error(%d) indication to WMS, Sent status = %d",error_value,status) ;
...
)
{
  ...
  MSG_HIGH_DS_3(cb_as_id,"LC id changed from %d to %d register back with WM,old water mark flag = %d",
            bmc_rlc_state.rlc_lc_id, rlc_lc_id,bmc_rlc_state.rlc_wm_reg_flag);
...
void cb_ctch_pdu_dropped(dword pdu_sfn)
{
  ...
  MSG_HIGH_DS_2(cb_as_id,"CB: Sending event CB_PDU_DROPPED for SFN:%u if pdu_found = %d is set",pdu_sfn,pdu_found) ;
}
...
pkt_size = NAS_STD_OFFSETOF(LOG_UMTS_NAS_CB_LOG_PACKET_C_type, cb_ota_log_packet) + msg_size;
...
void cb_dup_stop_timer( timer_id_T timer_id )
{
   ...
   if (cb_timer_status[timer_id-1] != TIMER_STOPPED)
   {
      ...
      gs_status = gs_cancel_timer( GS_TIMER_QUEUE_CB, CB_TIMER_TAG_AS_ID(timer_id) );
