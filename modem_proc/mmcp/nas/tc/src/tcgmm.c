...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/tc/src/tcgmm.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#ifdef FEATURE_MBMS
// Length the TC reply message to SS
#define TC_REPLY_LENGTH_FOR_RLC_SDU_COUNTER_REQUEST 6
#endif
// Dummy DCCH enable flag
boolean dummy_dcch_enabled;

#ifdef FEATURE_MBMS
int mtch_rab_index = -1;
#endif
/* ========================================================================
...
** DESCRIPTION  : This function packs the L3 TC message.
** ----------------------------------------------------------------------- */
LOCAL void tc_close_ue_test_loop(byte* recd_msg, byte tc_length);
...
#ifdef FEATURE_MBMS
/* -----------------------------------------------------------------------
** FUNCTION     : TC_RLC_SDU_COUNTER_REQUEST()

** DESCRIPTION  : This function sends the RLC_SDU_COUNTER value to the NW
** ----------------------------------------------------------------------- */
LOCAL void tc_rlc_sdu_counter_request(void);
...
void send_gmm_tcpdu(byte mesg_type)
{
...
#ifdef FEATURE_MBMS
  if (mesg_type == (byte) RLC_SDU_COUNTER_RESPONSE)
  {
    l3_buf[2] = (byte)((sdu_counter & 0xFF000000) >> 24);
    l3_buf[3] = (byte)((sdu_counter & 0x00FF0000) >> 16);
    l3_buf[4] = (byte)((sdu_counter & 0x0000FF00) >> 8);
    l3_buf[5] = (byte)(sdu_counter & 0x000000FF);
  }
#endif
  ...
  if (gmm_cmd != NULL)
  {
    ...
    if (mesg_type == (byte) RLC_SDU_COUNTER_RESPONSE)
    {
      MSG_HIGH_DS_1(TC_SUB,"MBMS: RLC_SDU_COUNTER_RESPONSE value: %d ", sdu_counter);
      gmm_cmd->cmd.gmmtc_unitdata_req.header.message_len_lsb = TC_REPLY_LENGTH_FOR_RLC_SDU_COUNTER_REQUEST;
      tc_reply_length = TC_REPLY_LENGTH_FOR_RLC_SDU_COUNTER_REQUEST;
    }
#endif
...
void tc_process_tc_pdu(byte* tc_data, byte tc_length)
{
  ...
  switch(recd_msg)
  {
    case CLOSE_UE_TEST_LOOP:
      tc_close_ue_test_loop(ext_buf,tc_length);
    break;
...
#ifdef FEATURE_MBMS
    case RLC_SDU_COUNTER_REQUEST:
      MSG_HIGH_DS_0(TC_SUB,"MBMS: Received RLC_SDU_COUNTER_REQUEST message");
      tc_rlc_sdu_counter_request();
    break;
#endif
...
===========================================================================*/
void tc_close_ue_test_loop(byte* recd_msg, byte tc_length)
{
  ...
  else
  {
    ...
    byte lc_index;
#ifdef FEATURE_MBMS
    byte mbms_service_id;
    uint8 mbms_mtch_id; //MBMS_LATER on max size of id
#endif
...
#ifdef FEATURE_MBMS
    else if((recd_msg[2] & 0x03) == 2)
    {
      // Received Loopback Mode 3 setup message
	  tc_current_state = TC_LOOPBACK_MODE3;
    }
#endif
    ...
    if (tc_current_state == TC_LOOPBACK_MODE1)
    {
      if (tc_length < 4)
      {
        ERR( "Mandatory IE missing in TC msg for Mode1, Discarding the Invalid TC PDU", 0, 0, 0);
        return;
      }
      if( recd_msg[3]  >  (tc_length - 4) )
      {
        ERR( "Invalid Lengh in TC msg for Mode1, Discarding the Invalid TC PDU", 0, 0, 0);
        return;
      }
...
#ifdef FEATURE_MBMS
    else if ((tc_current_state == TC_LOOPBACK_MODE3) && (tc_rat_rab_established_on == SYS_RAT_UMTS_RADIO_ACCESS))
    {
      sdu_counter = 0;
      mbms_service_id = recd_msg[3];
      //mbms_service_id = 2; // MBMS_PENDING_HACK
      //get LCid for this service_id from RRC
#ifdef FEATURE_SEGMENT_LOADING
      mbms_mtch_id = IWCDMA_rrc_get_mtch_id_for_service_id (ptr_W, (uint32)mbms_service_id);
#else
      mbms_mtch_id = rrc_get_mtch_id_for_service_id ((uint32)mbms_service_id);
#endif
      MSG_HIGH_DS_2(TC_SUB,"MBMS: service_id: %d, mtch_id: %d", mbms_service_id, mbms_mtch_id);
      for (rab_index=0; rab_index<MAX_RAB_TO_SETUP; rab_index++)
      {
        if (lb_rab_list[rab_index] == NULL)
        {
#ifdef FEATURE_MODEM_HEAP      
               lb_rab_list[rab_index] = (rab_context_info*) modem_mem_calloc( 1, sizeof(rab_context_info), MODEM_MEM_CLIENT_NAS);
#else
               lb_rab_list[rab_index] = &lb_rab_list_static [rab_index];
#endif
               if (lb_rab_list[rab_index] == NULL)
               {
                   MSG_HIGH_DS_1(TC_SUB, "Mem Alloc failed for rabId: %d", rab_index);
                   return ;
               }
               tc_nas_rab_list_init ((byte)rab_index);
               lb_rab_list[rab_index]->rab_established = TRUE;
               mtch_rab_index = rab_index;
               lb_rab_list[rab_index]->lc_info_ptr.num_ul_lc_ids_for_rab = 0;
               lb_rab_list[rab_index]->lc_info_ptr.num_dl_lc_ids_for_rab = 1;
               lb_rab_list[rab_index]->lc_info_ptr.dl_lc_id[ 0] = mbms_mtch_id ;
               MSG_HIGH_DS_1(TC_SUB,"MBMS: Changing Dont Exceed Cnt for RAB = %d, LC index = %d", rab_index);
               // set dont exceed cnt for this lc id to max and func ptr to valid
               lb_rab_list[rab_index]->lb_rlc_dl_wm[0].dont_exceed_cnt        = MAX_LB_SDU_SIZE * 20;
               /*lint -e{64} Reviewed with the process group and recommended for suppression */
               lb_rab_list[rab_index]->lb_rlc_dl_wm[0].each_enqueue_func_ptr  = lb_rx_data_from_rlc;
               lb_register_dl_lc_rlc_wm((byte)rab_index, mbms_mtch_id) ;
               break;
        }
      }
    }
...
#ifdef FEATURE_MBMS
    if (mtch_rab_index != -1 && lb_rab_list[mtch_rab_index] != NULL)
    {
       lb_deregister_rab_wm_rlc ((byte) mtch_rab_index);
       lb_rab_cleanup ((byte) mtch_rab_index);
    }
#endif
...
DESCRIPTION
  This function processes RLC_SDU_COUNTER_REQUEST TC-PDU sent by GMM. 
...
===========================================================================*/
void tc_rlc_sdu_counter_request()
{
  if (tc_current_state == TC_LOOPBACK_MODE3) 
  {
    send_gmm_tcpdu(RLC_SDU_COUNTER_RESPONSE);    /*lint !e641 */
  }
  else
  {
    MSG_HIGH_DS_1(TC_SUB,"MBMS: TC_RLC_SDU_COUNTER_REQUEST message Rxd in wrong tc_current_state: %d ",tc_current_state);
  }
}
