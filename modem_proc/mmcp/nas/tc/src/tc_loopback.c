...
/*===========================================================================
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/nas/tc/src/tc_loopback.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
FUNCTION LB_MODE3_RX_HANDLER

DESCRIPTION
  This function is called by the TC task for processing the received RLC
  data in test loop mode 3.
...
===========================================================================*/
void lb_mode3_rx_handler (uint32 wm_key)
{
  dsm_item_type *dsm_item_ptr = NULL; // Pointer to a DSM item from the WM
  uint32 rab_id  = lb_get_rab_id_from_key (wm_key);
  uint32 lc_id   = lb_get_chan_id_from_key (wm_key);

  MSG_HIGH_DS_3(TC_SUB, "MBMS: TC Mode3 Handler for wm_id = %d, for RAB = %d, DL LC index=%d",wm_key,rab_id,lc_id);
  // Retrieve the data from DSM WM 
  for (dsm_item_ptr = dsm_dequeue( &lb_rab_list[rab_id]->lb_rlc_dl_wm[lc_id]);
       dsm_item_ptr;
       dsm_item_ptr = dsm_dequeue(&(lb_rab_list[rab_id]->lb_rlc_dl_wm[lc_id])) )
  {
    sdu_counter++;
    MSG_HIGH_DS_1(TC_SUB,"MBMS: SDU Counter value = %d, ",sdu_counter);
    dsm_free_packet( &dsm_item_ptr );
  }
  post_self_cmd_flag[rab_id][lc_id] = FALSE;
} /* lb_mode3_rx_handler() */
...
#ifdef FEATURE_MBMS
   else if (tc_current_state == TC_LOOPBACK_MODE3)
   {
      lb_mode3_rx_handler(wm_key);
   }   
#endif
   else 
   {
