...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_serv_req_queue.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_send_est_request_from_service_queue(void)
{
...
#endif
           if(cnm_request_id.CNM_service_type == SHORT_MESSAGE_SERVICE_ESTABLISHMENT)
           {
   ...
   if(is_valid_serv_req && msg_present)
   {
     ...
     CNM_send_message ((IMH_T *) &mmcnm_est_req,
                    GS_QUEUE_MM);
#else
		  send_message_to_mm(&mmcnm_est_req.header,sizeof(mmcnm_est_req_T));
#endif
   }
   else
   {
...
#else
	  send_message_to_mm((IMH_T*)&mmcnm_est_req.header, sizeof(mmcnm_est_req_T));
#endif
