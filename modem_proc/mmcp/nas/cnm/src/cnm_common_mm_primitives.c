...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_common_mm_primitives.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_send_mmcnm_rel_req(IMH_T *message)
{
 ...
 #else
		 send_message_to_mm(&mmcnm_rel_req.header,sizeof(mmcnm_rel_req_T));
#endif
...
#endif
     if(GET_PD(mmcnm_data_req->data[0]) == PD_SMS)
     {
...
VOID_FUNC CNM_fill_mmcnm_est_req(mmcnm_est_req_T* mmcnm_est_req)
{
  if(MNCNM_IS_SINGLE_STACK)
  {
     if(mmcnm_est_req->CNM_service_type == SHORT_MESSAGE_SERVICE_ESTABLISHMENT)
     {
       mmcnm_est_req->as_id = sm_rl_transaction_information[mm_connection_pending.connection_id].as_id;
     }
#ifndef FEATURE_NAS_DISABLE_SUPL_SERVICES
     else if(mmcnm_est_req->CNM_service_type == SUPPLEMENTARY_SERVICE_ACTIVATION)
     {
...
#endif
       MSG_ERROR_DS_0(MN_SUB, "=MNCNM = Unknown service type");
     }
  }
#ifdef FEATURE_DUAL_ACTIVE
  else
