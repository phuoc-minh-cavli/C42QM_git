...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_send_msg.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_send_message( IMH_T * p_message, gs_queue_id_T queue)
{
    ...
    if(p_message->message_id == MMCNM_DATA_REQ)  
    {
	  ...
	  send_message_to_mm(local_p_message,sizeof(mmcnm_data_req_T));
#endif
    }
    ...
    else if(p_message->message_id == MMCNM_EST_REQ)  
    {
	  ...
	  send_message_to_mm(&local_mmcnm_est_req->header,sizeof(mmcnm_est_req_T));
#endif
