...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_timers.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
                             ...
                             )
{
   ...
   gs_timer_queue_id_T timer_queue = GS_TIMER_QUEUE_MN_CNM;
    ...
    */
...
#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_ACTIVE)
     {
       if((!MNCNM_IS_SINGLE_STACK) && (mn_dsda_as_id == (mn_dsda_as_id_e_type)SYS_MODEM_AS_ID_2)) /* set the timer only if the device mode is DSDA */
       {
         ...
         timer_queue = GS_TIMER_QUEUE_MN_CNM_SUB2;
       }
     }
#endif
...
VOID_FUNC CNM_stop_timer( timer_id_T timer_id, byte ti_pd )
{
    ...
    */
#ifdef FEATURE_CNM_UNIT_TEST
   MSG_MED_DS_1(MN_SUB,"=MNCNM= Stop %s\n", DEBUG_get_timer_name(timer_id));
#else
   MSG_MED_DS_1(MN_SUB,"=MNCNM= Stop timer %d\n",timer_id);
#endif
              ...
              if ( action != 0)
              {
                  ...
                  switch (timer_expiry_message->timer_id)
                  {
...
#else
 			              MN_handle_MNCC_REJ_IND(&p_mncc_rej_ind->message_header);
                      ...
                      */
                         ...
                         break;
#if  !defined(FEATURE_NAS_DISABLE_VOICE) && defined(ENABLE_CC_SUPL_SERVICES)                    
                     case TIMER_T324:
