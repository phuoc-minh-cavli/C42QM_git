...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_cc_dtmf.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_clear_dtmf_queue(connection_id_T connection_id)
{
  ...
  if(connection_id < MAXNO_CC_TRANSACTIONS)
  {
    ...
    {
      MSG_HIGH_DS_1(MN_SUB,"=MNCNM= Cannot Clear DTMF queue as cc_transaction_information_p[%d] id NULL",connection_id);
    }
    if (current_cc_dtmf_state == WAIT_FOR_START_DTMF_ACK)
    {
       ...
       CNM_send_message( (IMH_T *)&mncc_start_dtmf_cnf,
                       GS_QUEUE_MN_CM );
#else
       MN_process_MNCC_START_DTMF_CNF(&mncc_start_dtmf_cnf.message_header);
#endif
    ...
    else if(current_cc_dtmf_state == WAIT_FOR_STOP_DTMF_ACK)
    {
        ...
        CNM_send_message ((IMH_T *)&mncc_stop_dtmf_cnf,
                    GS_QUEUE_MN_CM );
#else
        MN_process_MNCC_STOP_DTMF_CNF(&mncc_stop_dtmf_cnf.message_header);
#endif
    }
