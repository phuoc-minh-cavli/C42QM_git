...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_cc_process_specific_msgs1.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC   CNM_process_alerting_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   */
   ...
   MN_handle_MNCC_ALERT_IND(&mncc_alerting_ind.message_header);
...
VOID_FUNC   CNM_process_call_proceeding_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   if ( syntax_ok == TRUE )
   {
      ...
      CNM_send_message( (IMH_T *)&mncc_call_proc_ind,
                        GS_QUEUE_MN_CM );
#else
      MN_handle_MNCC_CALL_PROC_IND(&mncc_call_proc_ind.message_header);
#endif
   }
   else
   {
...
VOID_FUNC   CNM_process_connect_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   if(syntax_ok)
   {
      ...
      CNM_send_message( (IMH_T *)&mncc_setup_cnf,
                       GS_QUEUE_MN_CM );
#else
    MN_process_MNCC_SETUP_CNF(&mncc_setup_cnf.message_header);
#endif
   }
   else
   {
...
VOID_FUNC   CNM_process_connect_ack_message( connection_id_T connection_id ) 
{
   ...
   memset(&mncc_setup_compl_ind, 0, sizeof(MNCC_SETUP_COMPL_IND_T));
   ...
   CNM_send_message((IMH_T *)&mncc_setup_compl_ind, 
                  GS_QUEUE_MN_CM );
#else
   MN_handle_mt_call_answer((IMH_T *)&mncc_setup_compl_ind.message_header);
#endif
}
   ...
   */
   ...
   if ( syntax_ok == TRUE && progress_ind_valid == TRUE )
   {
      ...
      CNM_send_message((IMH_T *)&mncc_progress_ind,
                                    GS_QUEUE_MN_CM );
#else
	 MN_handle_MNCC_PROGRESS_IND(&mncc_progress_ind.message_header);
#endif
...
VOID_FUNC   CNM_process_setup_message( connection_id_T connection_id, boolean syntax_ok, byte cause)
{
   ...
   if ( syntax_ok )
   {
      ...
      CNM_send_message( (IMH_T *)&mncc_setup_ind,
                        GS_QUEUE_MN_CM );
#else
	  MN_handle_mt_call_setup(&mncc_setup_ind.message_header);
#endif
   }
   else
   {
...
#else
			MN_handle_MNCC_REL_IND(&mncc_rel_ind.message_header);
#endif
            }
          }
          break; 
       ...
       */
       ...
       if( syntax_ok )
       {
          ...
          CNM_send_message( (IMH_T *)p_mncc_disc_ind,
                           GS_QUEUE_MN_CM );
	#else
	    MN_handle_MNCC_DISC_IND(&p_mncc_disc_ind->message_header); //9x05 Optimization
	#endif
       }
       else
       {
           ...
           */
    
           {
               ...
               CNM_send_message( (IMH_T *)p_mncc_rel_ind,
                                GS_QUEUE_MN_CM );
#else
			   MN_handle_MNCC_REL_IND(&p_mncc_rel_ind->message_header);
#endif
   ...
   if ( syntax_ok )
   {
      ...
      if ( last_state == CC_STATE_U1 )
      {
         ...
         CNM_send_message( (IMH_T *)&mncc_rej_ind,
                       GS_QUEUE_MN_CM );
#else
         MN_handle_MNCC_REJ_IND(&mncc_rej_ind.message_header);
