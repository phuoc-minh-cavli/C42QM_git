...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_sms_process_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
VOID_FUNC CNM_SMS_process_SMS ( IMH_T * SMS_input_primitive )
{
...
#if defined(FEATURE_DUAL_SIM )
	unsigned int	   length;
	GET_IMH_LEN (length, &(((mmcnm_data_ind_T *) ((void *)SMS_input_primitive))->msg_head.IMH))
#endif
   ...
   case MS_CC_MM_PS:
       {
           ...
           case MMCNM_DATA_IND:
               ...
               p_mmcnm_data_ind = (mmcnm_data_ind_T *) ((void *)SMS_input_primitive);
			   ...
			   if(MNCNM_IS_SINGLE_STACK)
			   {
				  local_as_id = (sys_modem_as_id_e_type)(p_mmcnm_data_ind->data[length -3]);
				
				  if(((p_mmcnm_data_ind->msg_head.type == CP_DATA) ||(p_mmcnm_data_ind->msg_head.type == CP_ERROR) || (p_mmcnm_data_ind->msg_head.type == CP_ACK))&& 
					  (sms_transaction_information[connection_id].sms_state == SMS_IDLE) )
				  {
					  mn_as_id = local_as_id;
					  mn_message_as_id = (byte)(mn_as_id + 1);
   ...
   */
       ...
       (SMS_input_primitive->message_id == MMCNM_EST_REJ))
   {
       ...
       /* The bleow case is added to handle the RF unavailable cause in LTE only device(DSDS disabled) */
#if !defined(FEATURE_DUAL_SIM ) && (defined(FEATURE_CATM_LTE_EXT_GNSS_COEX) || defined(FEATURE_WWAN_GNSS_DYNAMIC))
       if(((mmcnm_est_rej_T *)SMS_input_primitive)->cause_value == AS_REJ_ABORT_RADIO_UNAVAILABLE)
       {
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
         mmcnm_est_rej = (mmcnm_est_rej_T *) ((void *)SMS_input_primitive);
         CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, AS_REJ_ABORT_RADIO_UNAVAILABLE, mmcnm_est_rej->cause_value, mmcnm_est_rej->domain);
...
#endif
       {
		 ...
		 else if(((mmcnm_est_rej_T *)SMS_input_primitive)->cause_value  == EMM_REJ_HARD_FAIL)
         {
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
           mmcnm_est_rej = (mmcnm_est_rej_T *) ((void *)SMS_input_primitive);
           CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, EMM_REJ_HARD_FAIL, mmcnm_est_rej->cause_value, mmcnm_est_rej->domain);
#else
           CNM_send_MNSMS_ERROR_IND( connection_id, (byte)0, EMM_REJ_HARD_FAIL);
#endif
         }
                                         ...
                                         connection_id_T connection_id )
{
   ...
   */
   memset(&mnsms_est_ind, 0, sizeof(MNSMS_EST_IND_T));
      ...
      (CNM_check_validity_and_process_short_message(connection_id) == TRUE))
   {
      ...
      /* Set verstat status to default value 0*/
      mnsms_est_ind.sms_verstat_status = 0;
      ...
      offset = sizeof(((cp_data_msg_T *)((void *)message))->msg_head.ti_pd)+
                 sizeof(((cp_data_msg_T *)((void *)message))->msg_head.type)+ 1;   /* 1 for length field*/

#if defined(FEATURE_DUAL_SIM) 
      offset = offset + 1 ;    /* as_id */
#endif
      GET_IMH_LEN(length, &(((cp_data_msg_T *)((void *)message))-> msg_head.IMH));
      length = length-offset;  /* remaining length */

      if(rpdu_len > length)
      {
        rpdu_len = MIN(length, sizeof(mnsms_est_ind.rpdu.data));
      }
...
#else
     MN_handle_MNSMS_EST_IND(&mnsms_est_ind.message_header);
#endif
 ...
 *
 ******************************************************************************/
boolean CNM_active_SMS(void)
{
  byte  ti; /* transaction id */
  boolean active;

  for (ti=0, active=FALSE; ti<7; ti++) /* just for MO SMS transactions */
  {
...
boolean  is_mo_cp_data(byte *nas_msg_p)
{
    mmcnm_data_req_T *msg;
    msg=(mmcnm_data_req_T *)((void *)nas_msg_p);
     if(((msg->data[0] & 0xf0) < MAXNO_SMS_MO_TRANSACTIONS) && (msg->data[1]==CP_DATA))
         return TRUE;
     else
         return FALSE;
}
