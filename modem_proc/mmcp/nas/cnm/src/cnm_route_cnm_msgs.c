...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_route_cnm_msgs.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#else
				MN_handle_MNCC_SYNC_IND(&CNM_mmcc_sync_ind.header);
#endif
         ...
         switch (CNM_input_primitive->message_id)
         {
...
#endif
#ifndef FEATURE_MMCP_DISABLE_CS_USSD
                  if ( GET_PD(p_mmcnm_data_ind->msg_head.ti_pd) == PD_SS)
                  {
                     CNM_handle_supplementary_services(CNM_input_primitive);
                  }
                  else 
#endif
                  if ( GET_PD(p_mmcnm_data_ind->msg_head.ti_pd) == PD_SMS)
                  {
                     CNM_handle_short_message_services(CNM_input_primitive);
                  }
                  else
            ...
            case MMCC_SYNC_IND:
                    ...
                    )
               {
                   ...
                   if(CNM_active_SMS() == TRUE)
                   {
                       if(cp_data_l2_ack==FALSE)
                       {
                           for (ti=0; ti<7; ti++) /* calculating ti, only for MO SMS */
                           {
                           ...
                           {                   
                              CNM_stop_timer( TIMER_TC1M_MO,(unsigned char)FORM_TI_PD( ti, PD_SMS) );
                              CNM_start_timer( TIMER_TC1M_MO, ti, PD_SMS
#if defined(FEATURE_DUAL_SIM )
...
#if defined(FEATURE_CCBS) && !defined (FEATURE_NAS_DISABLE_VOICE)
