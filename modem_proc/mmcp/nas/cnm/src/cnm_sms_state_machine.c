...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/cnm/src/cnm_sms_state_machine.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
      ...
      case SMS_IDLE:
         switch (trigger)
         {
			...
			case S_CP_DATA_event:
			   MSG_HIGH_DS_1(MN_SUB,"=MNCNM= S_CP_ACK_FALSE/ERROR/DATA_event conn id = %d",connection_id);
      ...
      case SMS_MM_CONN_EST:
         switch (trigger)
         {
            ...
            case S_CP_DATA_event:
               ...
               if ( connection_id < 8 )
               {
                  byte rl_state=sm_rl_transaction_information[connection_id].rl_state;                  
                  CNM_check_for_pending_message();
