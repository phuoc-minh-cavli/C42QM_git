...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/inc/lte_nas_emm_message_description.h#1 $
$Author: pwbldsvc $
...
/* TS 24.008, section 10.5.3.5a Network Name
   The Network Name is a type 4 information element with a minimum length of 3 octets.
   No upper length limit is specified except for that given by the maximum number of
   octets in a L3 message (see 3GPP TS 44.006 [19])

   As per TS 44.006 maximum number of octets in a L3 message is 251. Since Network Name IE
   has 2 octets for IEI and Length indicator part, hence max value of network name length is 251 - 2 = 249 octets */
#define MAX_NW_NAME_LEN 249
...
typedef enum
{
  MO_REQ = 0,
  MT_REQ = 1
}lte_nas_emm_eps_data_service_value_type;

/*Ref 24.301 9.9.3.47 Data service type*/
typedef struct
{
  boolean                                  active_flag;
  lte_nas_emm_eps_data_service_value_type  data_service_val;
}lte_nas_emm_eps_data_service_type;
#endif
...
#ifdef FEATURE_CIOT
  INTEGRITY_PROTECT_PARTIALLY_CIPHER       = 5,
#endif
  SERV_REQ                                 = 12,
...
typedef struct
{
  lte_nas_dcn_type  dcn_list[LTE_NAS_MAX_DCN_IDS];
}lte_nas_dcn_lst_type;
...
{
  boolean sms_only;
  //preferred CIoT network behavior
  boolean pnb_cp_ciot;
  boolean pnb_up_ciot;
  boolean signalling_active_flag;
}lte_nas_additional_update_type;
#endif
