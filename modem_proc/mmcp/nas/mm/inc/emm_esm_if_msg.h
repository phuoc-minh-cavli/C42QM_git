...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/inc/emm_esm_if_msg.h#1 $
...
#endif
  ,MSGR_DEFINE_UMID(NAS, EMM, RSP, RESTORE_CTXT_PSM, 0x08, emm_restore_ctxt_psm_rsp_T)
  ,MSGR_DEFINE_UMID(NAS, EMM, RSP, SAVE_CTXT_PSM, 0x09, emm_save_ctxt_psm_rsp_T)
} ;
...
/* This structure is used to send a response to EMM after restoring
   all bearer contexts in ESM upon wake up from PSM */
typedef struct
{
  msgr_hdr_s  msg_hdr;
  boolean     status;
} emm_restore_ctxt_psm_rsp_T;


/* This structure is used to send a response to EMM after saving
   contexts in ESM to enter PSM */
typedef struct
{
  msgr_hdr_s  msg_hdr;
  boolean     status;
} emm_save_ctxt_psm_rsp_T;
...
#endif
  emm_restore_ctxt_psm_rsp_T    emm_restore_ctxt_psm_rsp;
  emm_save_ctxt_psm_rsp_T       emm_save_ctxt_psm_rsp; 
} emm_msgr_cmd_type ;
