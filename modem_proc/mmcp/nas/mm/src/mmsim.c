...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmsim.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#ifdef FEATURE_DUAL_SIM
boolean mm_sim_revert_sim_state_on_psm_restore_failure_sim[MAX_NAS_STACKS] = {FALSE, FALSE};
#define mm_sim_revert_sim_state_on_psm_restore_failure mm_sim_revert_sim_state_on_psm_restore_failure_sim[mm_as_id]
#else
boolean mm_sim_revert_sim_state_on_psm_restore_failure = FALSE;
#endif
...
)
{
   ...
   if(status == TRUE)
   {
      ...
      MSG_HIGH_DS_4(MM_SUB,"=MM= TMSI: %X %X %X %X", location_information.tmsi[0],
                                 location_information.tmsi[1],
                                 location_information.tmsi[2],
                                 location_information.tmsi[3]);
      ...
      MSG_HIGH_DS_6(MM_SUB, "=MM= PLMN: %X %X %X\n =MM= LAI: %X %X\n =MM= UPDATE_STATUS: %X", 
                                  location_information.lai[0],
                                  location_information.lai[1],
                                  location_information.lai[2],
                                  location_information.lai[3],
...
boolean mm_load_sim_data( void )
{
  ...
  */
  if (!mm_imsi_is_native)
  {
    ...
    MSG_HIGH_DS_0(MM_SUB,"=MM= Resetting is_accepted_on_lte flag due to sim change and writing it back to efs ");
...
)
{
  ...
  else
  {
     ...
     mmgsdi_status = mmgsdi_session_open_ext_sync(mm_sim_mmgsdi_info_sim[as_id].client_id,
                                                  session_type_mask, mm_sim_session_generic_event_cb,
    ...
    if ( mmgsdi_status == MMGSDI_SUCCESS )
    {
      ...
      memscpy(&mm_sim_mmgsdi_info_sim[as_id].session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
                                        sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(MM_LOCAL_SUB, "=MM= MMGSDI session open cnf Status %d Session ID: 0x%x %x", status,
      (dword)(mm_sim_mmgsdi_info_sim[as_id].session_id >> 32), (dword)(mm_sim_mmgsdi_info_sim[as_id].session_id & 0xFFFFFFFF));
      rex_leave_crit_sect(&mm_sim_crit_sect);
#else
#ifdef FEATURE_SGLTE
      memscpy(&mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
                                        sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(MM_SUB, "=MM= MMGSDI session open cnf Status %d Session ID: 0x%x %x", status,
      (dword)(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].session_id >> 32), (dword)(mm_sim_mmgsdi_info_sim[SYS_MODEM_AS_ID_1].session_id & 0xFFFFFFFF));
#else
      memscpy(&mm_sim_mmgsdi_info.session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
                                        sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(MM_SUB, "=MM= MMGSDI session open cnf Status %d Session ID: 0x%x %x", status,
      (dword)(mm_sim_mmgsdi_info.session_id >> 32), (dword)(mm_sim_mmgsdi_info.session_id & 0xFFFFFFFF));
#endif
   ...
   mmgsdi_status = mmgsdi_session_open_ext_sync(mm_sim_mmgsdi_info.client_id,
                                              session_type_mask, mm_sim_session_generic_event_cb,
                                              &mmgsdi_session_info_ptr);
    ...
    { 
      ...
      memscpy(&mm_sim_mmgsdi_info.session_id, sizeof(mmgsdi_session_id_type), &(mmgsdi_session_info_ptr.session_id),
                                        sizeof(mmgsdi_session_id_type));
      MSG_HIGH_DS_3(MM_SUB, "=MM= MMGSDI session open  Status %d Session ID: 0x%x %x", mmgsdi_status,
      (dword)(mm_sim_mmgsdi_info.session_id >> 32), (dword)(mm_sim_mmgsdi_info.session_id & 0xFFFFFFFF));
