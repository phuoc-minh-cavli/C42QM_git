...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmmsecurity.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
**-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------**/
    ...
    /* Spec CR 2688: Spec 24.008 4.7.1.5.1:
       * In A/Gb mode, update old or stored PTMSI values only when it receives a different one from the network */
      if (!is_gsm_mode() || 
          (memcmp(gmm_stored_ptmsi.digit, ptmsi_reallocation_command_msg.allocated_ptmsi, PTMSI_SIZE) != 0))
#endif
