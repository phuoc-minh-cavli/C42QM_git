...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmplmnsel.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#ifdef FEATURE_CIOT
      mm_remove_rat_from_rat_pri_list(&rr_plmn_select_req.rat_pri_list_info,SYS_SYS_MODE_LTE_M1);
      mm_remove_rat_from_rat_pri_list(&rr_plmn_select_req.rat_pri_list_info,SYS_SYS_MODE_LTE_NB1);
#endif
...

      {
         ...
         ** ---------------------------------------------------------------- */
                       ...
                       LAI_SIZE ) == 0 ) )
         {
            ...
            /* If UE camps on an already updated PLMN which is not a backoff PLMN, then no need to continue running backoff timer T3246 */
#ifdef FEATURE_NAS_REL10
            if ((mm_timer_status[TIMER_T3246] == TIMER_ACTIVE) &&
                (mm_is_plmn_equivalent_t3246_plmn() == FALSE))
            {
