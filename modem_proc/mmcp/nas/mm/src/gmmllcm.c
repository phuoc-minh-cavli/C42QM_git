...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmmllcm.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
extern void glif_process_paging_response(void);
...
extern boolean gmm_check_cell_forbidden (void);
...
void glif_llc_send_resume_request( void )
{
  ...
  /* Indicates G-LLC to restore the last XID context negotiated with network from EFS upon REL12 PSM exit. */
  if ((mm_psm_r12_enabled == TRUE) &&
      (mm_psm_restore_pending == TRUE))
  {
    glif_resume_req_ptr->mes.llgmm_resume_req.psm_restore = TRUE;
  }
  else
  {
    glif_resume_req_ptr->mes.llgmm_resume_req.psm_restore = FALSE;
  }
...
)
{
  ...
  if (gmm_psm_timer_ctxt.t3324_value != NAS_INVALID_PSM_TIMER_VALUE)
  {
    ...
    if (gmm_psm_timer_ctxt.t3324_value != 0)
    {
      mm_start_timer( TIMER_T3324, gmm_psm_timer_ctxt.t3324_value );
    }
    else
    {
...
)
{
  ...
  */
  ...
  /* Inform current status of ready timer (active or inactive) to GRR. */
  mm_send_rr_ready_timer_status_ind(FALSE);
...
)
{
  ...
  */
  ...
  /* Inform current status of ready timer (active or inactive) to GRR. */
  mm_send_rr_ready_timer_status_ind(TRUE);
  ...
  if (memcmp((void *)glif_tlli, (void *)new_tlli, PTMSI_SIZE))
  {
    ...
    */
    if (((glif_tlli[0] & 0xC0) >> 6) == 3) /* TS 23.003 Table 1 */
    {
      ...
      sm_put_cmd( sm_cmd );
#endif
      sm_tlli = GLIF_XLATE_TLLI(glif_tlli);
...
)
{
  ...
  else
  {
      ...
      /* If the MS is configured for "AttachWithIMSI" and entering a new PLMN which is neither 
    ** RPLMN nor equivalent PLMN, MS should use a randomly selected random TLLI for the 
    ** transmission of the ATTACH REQUEST message. */
...
)
{
  ...
  if (trigger_cause == GMMLLC_PAGE_RESPONSE)
  {
...
)
{
  ...
  if (gmm_psm_timer_ctxt.psm_active == TRUE)
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= Ignoring Paging during PSM active");
    return;
  }
      ...
      */
...
#endif
      {
        ...
        **(24.008 section 4.7.5.1) --------------------------------------------------*/
        ...
        gmm_responded_ps_page = TRUE;
#endif
        gmm_initiate_routing_area_update() ;
      }
#ifdef FEATURE_NAS_REL10
      else if((gmm_low_priority_indicator == TRUE) &&
              (mm_timer_status[TIMER_T3346] == TIMER_ACTIVE) &&
              (gmm_state == GMM_REGISTERED) &&
              (gmm_substate == GMM_NORMAL_SERVICE))
      {
        /* 
        ** In case T3346 is running due to implicit reject for low priority indicator 
        ** then stop T3346 on receiving and enable PS at GRR before sending page response 
        */                 
        if (gmm_congestion_due_to_implicit_reject == TRUE)
        {
          mm_stop_timer(TIMER_T3346);
          gmm_is_congestion_due_to_rau = FALSE;
          gmm_congestion_due_to_implicit_reject = FALSE;
          gmm_responded_ps_page = TRUE;
          glif_process_paging_response();
        }
      }
#endif
    }
...
)
{
  ...
  is_cell_forbidden = gmm_check_cell_forbidden();
...
)
{
  ...
  if (primitive->cmd.llgmm_status_ind.protocol_disc == PD_GPRS_SM)       
  {
    if (primitive->cmd.llgmm_status_ind.cause == GLLCMM_L2_TX_FAILURE)
    {
      ...
      sm_process_status_ind (sm_cmd->header.connection_id);
  ...
  { 
    if (primitive->cmd.llgmm_status_ind.cause == GLLCMM_L2_TX_FAILURE)
    {
      ...
      { 
        ...
        {
          MSG_HIGH_DS_0(MM_SUB, "=MM= T3346 running no need to retransmit attach");
        }
        else
        { 
          gmm_initiate_gprs_attach(); 
      ...
      { 
        ...
        {
          MSG_HIGH_DS_0(MM_SUB, "=MM= T3346 running no need to retransmit RAU");
        }
        else
        {
          gmm_initiate_routing_area_update(); 
...
/* 24.301 5.5.3.3.2 d) */        
         ...
         (gmm_state == GMM_REGISTERED_INITIATED)&&(gmm_attach_type == GMM_COMBINED_GPRS_IMSI_ATTACH))
#endif
         )&&
      ...
      else if(gmm_last_signalling_message_type == GMM_ATTACH_REQUEST)
      {
        gmm_attach_tx_is_acked = TRUE;
      }
      else
      {
        MSG_HIGH_DS_1(MM_SUB, "=MM= GLIF: Ignore STATUS IND for message type = %d ",gmm_last_signalling_message_type );
    ...
    default:
      {
        ...
        if (glif_state != next_glif_state)
        {
          ...
          if (primitive->cmd.hdr.message_id == GLLCMM_LLGMM_RDY_TMR_IND)
          {
            if ((gmm_psm_timer_ctxt.t3324_value != NAS_INVALID_PSM_TIMER_VALUE) &&
...
FUNCTION      gmm_reset_new_tilli

DESCRIPTION
  Reet the value of new tilli
...
void gmm_reset_new_tilli()
{
	memset((void *)glif_tlli,0xFF,sizeof(glif_tlli_type)) ;	
}
...
FUNCTION      GLIF_PROCESS_PAGING_RESPONSE

DESCRIPTION
  This function processes Page response message which has to be transmitted after
  GRR camping for Packet services will be completed in implicit reject in GSM.
...
===========================================================================*/
void glif_process_paging_response
(
 void
)
{
   /* LLC will pend  paging response until PS acess is enabled at GRR*/
   glif_send_llc_trigger_request(GMMLLC_PAGE_RESPONSE);

#ifdef FEATURE_NAS_FREE_FLOAT   
   /* Enable PS access after sending  paging response tso that page response PDU will be processed 1st at LLC */
   if((glif_grr_ser_req_state == GLIF_NO_SERVICE) &&
      (GLIF_XLATE_TLLI(glif_tlli) != 0xFFFFFFFF))
   {
     glif_grr_send_service_req();        
...
} /* end glif_process_paging_response() */
...
DESCRIPTION
  This function sends TLLI assignment request in inactive state immediately after UE exits from PSM mode for PDP context restore.
...
void gmmllc_tlli_assign_on_psm_exit()
{
  glif_tlli_assign(glif_get_tlli(GLIF_FOREIGN_TLLI_TYPE),FALSE) ;
}

boolean glif_get_ready_timer_deactivated()
{
  return glif_ready_timer_deactivated;
}

void glif_set_ready_timer_deactivated(boolean gprs_ready_timer_deactivated)
{
  glif_ready_timer_deactivated = gprs_ready_timer_deactivated;
}

boolean gmm_check_cell_forbidden (void)
{
  boolean is_cell_forbidden = FALSE;
