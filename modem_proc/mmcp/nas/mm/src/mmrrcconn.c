...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmrrcconn.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void send_cmd_to_tdscdma_rrc( rrc_cmd_type *cmd )
{
   ...
   switch (cmd->cmd_hdr.cmd_id)
   {
      ...
      case RRC_DEACT_REQ:
       ...
       case RRC_LTE_RESELECTION_ALLOWED_STATUS_IND:
         MSG_HIGH_DS_1(MM_SUB,"=MM= MM sent TDSRRC_LTE_RESELECTION_ALLOWED_STATUS_IND to RRC can_use_pseudo = %d",cmd->cmd.rrc_psuedo_lte_list_update_ind.can_use_pseudo);          
...
#if defined (FEATURE_NAS_REL11) || defined (FEATURE_NAS_DISABLE_VOICE)	   
      ...
      if((pending_gmm_msg[4] & (0x0F)) == PD_GMM && pending_gmm_msg[5] == GMM_SERVICE_REQUEST)
      {
        ...
        /* for the first time, when PDP activate req is coming the value would be all zero in context status, so don't check for the first time */          
          if(((gmm_service_request_msg_T*)pending_gmm_msg)->data[2] || ((gmm_service_request_msg_T*)pending_gmm_msg)->data[3])
          {
            ...
            MSG_HIGH_DS_4( MM_SUB, "=MM= Old PDP context status %d, %d, New PDP context status %d, %d\n", 
        ...
        else if(gmm_service_request_type == GMM_DATA)
        {
          ...
          MSG_HIGH_DS_4( MM_SUB, "=MM= Old Uplink data status: %d, %d, New Uplink data status: %d, %d\n", ((gmm_service_request_msg_T*)pending_gmm_msg)->data[index], 
                                        ((gmm_service_request_msg_T*)pending_gmm_msg)->data[index+1],
