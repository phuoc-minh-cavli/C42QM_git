...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmmreg.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
/*4.7.5.1.5 (j)*/
       ...
       )
    {
      ...
      /* If UE is configured for "AttachwithIMSI" and current camped PLMN is Non-RPLMN 
      **  or Non-EPLMN then attach with IMSI is performed instead of RAU procedure */
      if (mm_check_attach_with_imsi(MM_PS_DOMAIN) == TRUE)
      {
        MSG_HIGH_DS_0( MM_SUB, "=MM= AttachWithIMSI is TRUE- Start Attach procedure");
      ...
      {
		MSG_HIGH_DS_1( MM_SUB, "=MM= gmm_trigger_attach_on_ps_call = %d", gmm_trigger_attach_on_ps_call);
        if ((gmm_plmn_changed( gmm_previous_registration_serving_rai.plmn_id, gmm_current_lai.PLMN_id ) ||
            (gmm_rai_changed( gmm_previous_registration_serving_rai.location_area_code,
                              gmm_previous_registration_serving_rai.routing_area_code,
                              mm_serving_plmn.lac, mm_serving_plmn.rac )) ||
            ((gmm_follow_on_request && gmm_trigger_attach_on_ps_call == FALSE) ||
             (mm_follow_on_request && (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1)))
#if (defined (FEATURE_GSM) || defined (FEATURE_WCDMA)) && defined(FEATURE_NAS_GW)
             ||
        ...
        else
        {
            ...
            if ((gmm_rau_attempt_counter >= 5) ||(gprs_attach_attempt_counter >= 5))
          {
            ...
            mm_serving_plmn.cause = MMR_CAUSE_FAILURE_MAX_ATTEMPTED;
    ...
    else  /* LU requested but LAI forbidden */
    {
      ...
      if (((gmm_check_gprs_forbidden_rai(mm_serving_plmn.info.plmn,mm_serving_plmn.lac,mm_serving_plmn.rac)) ||
          ...
          (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS))
      {
        mm_serving_plmn.cause = MMR_CAUSE_FAILURE_PLMN_NOT_ALLOWED_FOR_GPRS_SERVICES;
...
/*Reset PS handover RAU initiation flag*/
    ...
    */
...
#ifdef FEATURE_LTE
#ifdef FEATURE_CIOT
	  emm_set_is_t3346_for_mo_exception(FALSE);
#endif
#endif
    }
#endif
    ...
    if (t3310_timeout_counter == 0)
    {
      ...
      if (gprs_attach_attempt_counter == 0)
      {
        gmm_acked_tx_retry_timeout_counter = 0;
      }
    }
...
#ifdef FEATURE_NAS_REL10
    /*Evaluate low priority indicator value for Device Properties IE */
    gmm_evaluate_low_priority_indicator_value();
#endif

      /*Evaluate extended DRX value to be used for negotiation for 1st two retransmission*/
    if (is_gsm_mode() && 
        (mm_edrx_cfg.gsm.enabled == TRUE) &&
        (gmm_acked_tx_retry_timeout_counter < 2))
    {
      mm_send_rr_mtc_param_update_ind(TRUE);
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled = TRUE;
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.ptw = mm_edrx_cfg.gsm.ptw;
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.cycle_length = mm_edrx_cfg.gsm.cycle_length;

      MSG_HIGH_DS_3(MM_SUB, "=MM= EDRX edrx_supported = %d edrx_ptw = %d edrx_cycle_length = %d", 
	                                 mm_edrx_cfg.gsm.enabled, mm_edrx_cfg.gsm.ptw, mm_edrx_cfg.gsm.cycle_length);
    }
    else
    {
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled = FALSE;
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.ptw = NAS_INVALID_EDRX_VALUE;
      gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.cycle_length = NAS_INVALID_EDRX_VALUE;
    }
    
    if (is_gsm_mode() && (mm_psm_r12_enabled == TRUE))
    {
      gmm_psm_timer_ctxt.t3324_value_to_nw     = mm_psm_active_timer;
      gmm_psm_timer_ctxt.t3314_value_to_nw     = mm_psm_gprs_ready_timer;
      gmm_psm_timer_ctxt.t3312_ext_value_to_nw = mm_psm_periodic_rau_update_timer;

      MSG_HIGH_DS_4(MM_SUB, "=MM= PSM Rel12 PSM supported = %d t3324_to_nw  = %d t3312_to_nw = %d t3314_value_to_nw = %d", 
	                                 mm_psm_r12_enabled, gmm_psm_timer_ctxt.t3324_value_to_nw,
                                     gmm_psm_timer_ctxt.t3312_ext_value_to_nw,
                                     gmm_psm_timer_ctxt.t3314_value_to_nw);
    }
    else
    {
      gmm_psm_timer_ctxt.t3324_value_to_nw     = NAS_INVALID_PSM_TIMER_VALUE;
      gmm_psm_timer_ctxt.t3314_value_to_nw     = NAS_INVALID_PSM_TIMER_VALUE;
      gmm_psm_timer_ctxt.t3312_ext_value_to_nw = NAS_INVALID_PSM_TIMER_VALUE;
    }
    ...
    ** --------------------------------------------- */
	...
	if(gmm_trigger_attach_on_ps_call == TRUE)
    {
  ...
  ** ------------------------------------ */
  ...
  else /* gprs_attach_attempt_counter >= 5 */
  {
    ...
    **----------------------------------------------------------*/
    ...
    /* ---------------------------- ---------------------------- ----------------------------
     * Spec CR 2390: 24.008: 4.7.2.7:
     * The default value of timer T3302 is used - If a new PLMN which is not in the list of equivalent PLMNs 
     * has been entered, the attach procedure fails, and the attach attempt counter is equal to 5. 
     * ---------------------------- ---------------------------- ----------------------------*/
    if((gmm_t3302_timer_value != mm_get_timeout(TIMER_T3302)) && 
       (gmm_plmn_changed(gmm_previous_plmn_with_t3302_value, mm_serving_plmn.info.plmn))&& 
    ...
    {
      gmm_t3302_timer_value = mm_get_timeout(TIMER_T3302);
      MSG_HIGH_DS_1(MM_SUB,"=MM= New PLMN which is not in the list of equivalent PLMNs has been entered, gmm_t3302_timer_value = %d", gmm_t3302_timer_value);
    }
#endif
    ...
    **----------------------------------------------------------*/
    MSG_HIGH_DS_0(MM_SUB,"=MM= GPRS attach failed for max times. Deleting EPLMNs.");
    mm_serving_plmn.update_equiv_plmn_list = TRUE;
    memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
#ifndef FEATURE_NAS_CS_DISABLED
    if ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
    ...
    if (mm_reg_waiting_for_reg_cnf)
    {
      if (mm_ready_to_send_reg_cnf)
      {
        mm_serving_plmn.cause         = mmr_cause;
        //mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
        mm_send_mmr_reg_cnf(NULL,mm_serving_plmn.info.service_status);
...
void gmm_handle_nmo1_reject_cause( byte gmm_cause, boolean attach_reject, boolean list_of_receive_npdu_numbers_present)
{
                    switch (gmm_cause)
                    {
                        case IMSI_UNKNOWN_IN_HLR:
                          if (attach_reject == TRUE)
                          {
                            gmm_set_substate( GMM_NORMAL_SERVICE );
...
#ifdef FEATURE_NAS_REL12
                            /* Spec 24.008 4.7.5 - CR 2486: Reset GPRS RAU attempt counter */
                            gmm_rau_attempt_counter = 0;
...
The behaviour of the MS in those cases is specified in subclause 4.7.5.2.5.*/
                        default:
                          ...
                          */
                          ...
                          if(mm_managed_roaming_enabled && (gmm_cause == NETWORK_FAILURE))
                          {
                                ...
                                if((mm_possible_imsi_attach) && (mm_system_information.ATT))
                                {
                                  ...
                                  if (is_gsm_mode()&&
                                      ((gmm_is_ptmsi_present_in_prev_gmm_msg) || (gmm_is_tmsi_present_in_prev_gmm_msg)||
                                       (list_of_receive_npdu_numbers_present)))
                                  {
...
void gmm_handle_ota_reject_cause( byte gmm_cause, boolean attach_reject)
{
        mmr_service_ind_s_type     mmr_service_ind;
		byte gmm_cause_local = gmm_cause;

		if(attach_reject == TRUE &&
		  (gmm_cause == MS_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK || 
		   gmm_cause == IMPLICITLY_DETACHED))
		{
			gmm_cause = NETWORK_FAILURE;
		}
        switch (gmm_cause)
        {
                  case ILLEGAL_MS:
                  ...
                  case GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED:
                    ...
                    ** ------------------------------------------------------------ */
                    ...
                    if (attach_reject == FALSE)
                    {
                      gmm_process_gprs_release_ind(mm_convert_gmm_to_sm_cause(gmm_cause));
                    }
                    ...
                    if ((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_PS) &&
                        ...
                        (((attach_reject ==FALSE) &&
                         ...
                         ((attach_reject==TRUE) && (gmm_attach_type == GMM_COMBINED_GPRS_IMSI_ATTACH))))
                    {
                    ...
                    else
                    {
					  ...
					  {
                        mmr_service_ind.cause = mm_convert_to_reg_cause(gmm_cause );
					  }
                    ...
                    if (mm_reg_waiting_for_reg_cnf)
                    {
                      ...
                      if ((mm_state != MM_IDLE)||
                          (((mm_timer_status[TIMER_T3211] == TIMER_ACTIVE) ||
                            ...
                            (mm_lu_attempt_counter <4)))
                      {
                        mm_serving_plmn.cause = MMR_CAUSE_FAILURE_GPRS_FORBIDDEN_REMAIN_ON_PLMN;
                      }
                      if (mm_serving_plmn.network_op_mode == NETWORK_OP_MODE_1)
                      {
                        mm_serving_plmn.cause = MMR_CAUSE_FAILURE_REMAIN_ON_PLMN;
                  ...
                  case IMPLICITLY_DETACHED:
                    ...
                    ** ------------------------------------------------------------- */
                    if (gmm_cause == MS_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK)
                    {
                      gmm_update_status = GMM_GU2_NOT_UPDATED;
                      gmm_delete_ps_location_information();
                      gmm_delete_gprs_cksn();
...
#endif
					  {
						  if ((gmm_manual_mode_user_selection_procedure == FALSE) &&
                  ...
                  case NO_SUITABLE_CELLS_IN_LA:
                    ...
                    ** GMM-DEREGISTERED.
                    ** ------------------------------------------------------------- */
                    if (gmm_cause == PLMN_NOT_ALLOWED)
                    {
                      gmm_update_status = GMM_GU3_PLMN_NOT_ALLOWED;

                      mm_add_plmn_to_fplmn_list( mm_serving_plmn.info.plmn, FALSE);
...
#ifdef FEATURE_NAS_REL12
                    /* Spec 24.008 4.7.5: Reset GPRS RAU attempt counter */
                    gmm_rau_attempt_counter = 0;
                    MSG_HIGH_DS_1( MM_SUB, "=MM= GPRS RAU Attempt counter = %d", gmm_rau_attempt_counter);
#endif
                    gprs_attach_attempt_counter = 0;
                    ...
                    if(mm_managed_roaming_enabled && 
                       ...
                       (gmm_cause == PLMN_NOT_ALLOWED ||
                        gmm_cause == LA_NOT_ALLOWED ||
                        gmm_cause == NATIONAL_ROAMING_NOT_ALLOWED) && 
                    ...
                    if ((gmm_cause == PLMN_NOT_ALLOWED) ||
                        (gmm_cause == LA_NOT_ALLOWED) ||
						(gmm_cause == NATIONAL_ROAMING_NOT_ALLOWED && attach_reject == TRUE))
                    {
                      gmm_delete_ps_location_information();
                      gmm_delete_gprs_cksn();
  ...
  */
                    ...
                    ** ------------------------------------------------------------- */
                    if (gmm_cause == LA_NOT_ALLOWED)
                    {
                      ...
                      ** --------------------------------------------------------- */
                      ...
                      mm_serving_plmn.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
#else
                      mm_add_forbidden_regional_roaming( &gmm_current_lai );
                      mm_serving_plmn.cause = MMR_CAUSE_FAILURE_REMAIN_ON_PLMN;
#endif
                    }
                    else if ((gmm_cause == NATIONAL_ROAMING_NOT_ALLOWED) ||
                             (gmm_cause == NO_SUITABLE_CELLS_IN_LA))
                    {
                      mm_add_forbidden_national_roaming( &gmm_current_lai );
                      mm_serving_plmn.cause =
                        MMR_CAUSE_FAILURE_INITIATE_PLMN_SELECTION;
                    }
                    if(gmm_cause == NO_SUITABLE_CELLS_IN_LA)
                    {
                      mm_serving_plmn.cause = MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED;
                      if(((attach_reject ==FALSE) &&
                           ((gmm_ra_update_type == GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH) ||
...
#endif
                      {
                         mm_serving_plmn.cause = mm_convert_to_reg_cause(
                                                              gmm_cause );
                      }
                  ...
                  case GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN:
                    ...
                    ** ------------------------------------------------------- */
                    ...
                    {
                      reg_sim_add_plmn_to_gprs_fplmn_list( mm_serving_plmn.info.plmn, FALSE
#ifdef FEATURE_DUAL_SIM
			  ...
			  * wait for Registration to conclude.
                  ...
                  default:
                    ...
                    ** attempt counter.
                    ** ---------------------------------------------------------- */
                      if(gmm_cause == MM_SEMANTICALLY_INCORRECT_MESSAGE ||
                         gmm_cause == MM_INVALID_MANDATORY_INFORMATION ||
                         gmm_cause == MM_MESSAGE_TYPE_NON_EXISTENT_OR_NOT_IMPLEMENTED ||
                         gmm_cause == MM_INFORMATION_ELEMENT_NON_EXISTENT_OR_NOT_IMPLEMENTED ||
                         gmm_cause == MM_PROTOCOL_ERROR_UNSPECIFIED)
                      {
                    ...
                    if ((gmm_cause == IMSI_UNKNOWN_IN_HLR) ||
                        (gmm_cause == IMSI_UNKNOWN_IN_VLR) ||
                        (gmm_cause == NETWORK_FAILURE) ||
                        (gmm_cause == CONGESTTION) ||
                        (gmm_cause == MSC_TEMPORARILY_NOT_REACHABLE))
                    {
                      mm_send_mmr_reg_reject_ind(SYS_SRV_DOMAIN_PS_ONLY,
                                    gmm_cause,
                                    mm_serving_plmn.info.plmn,
...
#endif
  byte                    i;
  uint64                  t3312_timer_value = 0;
  byte                    previous_plmn_identity[PLMN_SIZE];
  boolean                 psm_update  = FALSE;
  boolean                 edrx_update = FALSE;
 #if defined(FEATURE_LTE)
  ...
  switch (message->cmd.hdr.message_set)
  {
    ...
    case MS_MM_TDSRRC:
      switch (message->cmd.hdr.message_id)
      {
        ...
        mm_emm_reset_dos_counters_per_plmn(mm_serving_plmn.info.plmn,
#ifdef FEATURE_LTE 
                              LTE_NAS_CAUSE_NONE,
                              ATTACH_ACCEPT,
#endif
                              (boolean)((attach_accept_msg->attach_result == GMM_COMBINED_GPRS_IMSI_ATTACHED)?FALSE:TRUE),
#ifdef FEATURE_NAS_GW
                              MM_PS_DOMAIN,
                              GMM_ATTACH_ACCEPT
#endif
                ...
                **-------------------------------------------------------------*/  
                    ...
                    (gmm_attach_type == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED))
                {
                  ...
                  { 
                    ...
                    {
                      gmm_handle_nmo1_reject_cause(attach_accept_msg->gmm_cause,TRUE, FALSE);
#if 0
...
/*24.008: The behaviour of the MS in those cases is specified in subclause 4.7.3.2.5.*/
...
#ifdef FEATURE_MBMS    
                /***********************************************************************
                ** section 10.5.5.23 Network feature support 
                ** The purpose of the network feature support information element is to 
                ** indicate whether certain features are supported by the network. 
                ** If this IE is not included then the respective features are not 
                ** supported.            
                *************************************************************************/ 
                
                gmm_send_msm_mbms_supp_ind(attach_accept_msg->nw_ftr_support);
#endif
                ...
                /* Process Active Timer T3324 from network */
                psm_update = gmm_process_receive_psm_timers(attach_accept_msg->t3324_value_present,
                                                            attach_accept_msg->t3324_value);
                /* Process eDRX parameters from network */
                edrx_update = mm_process_edrx_params(attach_accept_msg->edrx_params_present,
                                                     attach_accept_msg->edrx_params);
                if (psm_update || edrx_update)
                {
                  /* Send eDRX/PSM update to GRR */
                  mm_send_rr_mtc_param_update_ind(FALSE);
                }
                ...
                {
                  if (gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status == NAS_EDRX_CFG_REQ_PROCESSING)
                  {
                    gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status = NAS_EDRX_CFG_REQ_NONE;
                  }

                  if (gmm_psm_timer_ctxt.psm_set_cfg_req_status == NAS_PSM_CFG_REQ_PROCESSING)
                  {
                    gmm_psm_timer_ctxt.psm_set_cfg_req_status = NAS_PSM_CFG_REQ_NONE;
                  }
                }
...
#else
                  else if ((cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CMCC) || (cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CU) || (mm_is_qrd_device) || 
#ifdef DISABLE_9x05_OPTIMIZATIONS
                           (mm_optimised_ps_signalling_connection_release)
#endif
)
#endif
                  { 
                ...
                {
                  MSG_HIGH_DS_2(MM_SUB, "=MM= PSM(%d)/EDRX(%d) Set cfg params req pending, Start RAU procedure ",
                                                               gmm_psm_timer_ctxt.psm_set_cfg_req_status,
                                                               gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status);
                  gmm_set_state(GMM_REGISTERED);
                  gmm_update_ra_type();
                  gmm_initiate_routing_area_update();
                }
            ...
            case GMM_ATTACH_REJECT:
              ...
              {
                if(gmm_check_if_cause12_to_cause15_allowed() == TRUE && 
...
#ifdef FEATURE_NAS_REL10
              if(attach_reject_msg.t3346_value_present)
              {
                gmm_t3346_backoff_timer_value = gmm_convert_timer_value_ie_to_usecs(
                                                                          attach_reject_msg.t3346_value );
              }
#endif
               gmm_handle_ota_reject_cause( attach_reject_msg.gmm_cause, TRUE);
    ...
    case MS_TIMER:
      switch (message->cmd.timer_expiry.timer_id)
      {
        case TIMER_T3310:
          ...
          if (gmm_attach_tx_is_acked)
          {
            gmm_acked_tx_retry_timeout_counter++;
          }

          /*Invalidate extended DRX value after attach procedure fails for 1st two attempt due to no response*/
          if (is_gsm_mode() && 
             (mm_edrx_cfg.gsm.enabled == TRUE) &&
             (gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled) &&
             (gmm_acked_tx_retry_timeout_counter >= 2))
          {
            /* Invalidate eDRX params */
            mm_invalidate_edrx_params();
...
-if the mobile station enters PMM-IDLE mode (Iu mode only).*/
			...
			gmm_attach_fail_counter++;
            gmm_increment_gprs_attach_attempt_counter();
			if(gmm_attach_fail_counter == 5 )
			{
				gmm_trigger_attach_on_ps_call = TRUE;
			}
...
/*Reset PS handover RAU initiation flag*/
    ...
    ** ------------------------------------------------------------------------- */
    ...
    if(gmm_detach_after_rau_pending == TRUE && (mm_timer_status[TIMER_T3321] == TIMER_ACTIVE))
    {
    ...
    ** ------------------------------------------------------------------ */
    ...
    if (is_gsm_mode() && 
        ...
        (gmm_acked_tx_retry_timeout_counter < 2))
    {
      /* Send lowest eDRX value to GERAN for Non-updated RAU*/
      if ((gmm_plmn_changed( gmm_stored_rai.plmn_id, mm_serving_plmn.info.plmn ))||
          (gmm_rai_changed( gmm_stored_rai.location_area_code,
                            gmm_stored_rai.routing_area_code,
                            mm_serving_plmn.lac, mm_serving_plmn.rac )))
    ...
    if(gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status == NAS_EDRX_CFG_REQ_PENDING)
    {
      gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status = NAS_EDRX_CFG_REQ_PROCESSING;
    }
    ...
    if (gmm_psm_timer_ctxt.psm_set_cfg_req_status == NAS_PSM_CFG_REQ_PENDING)
    {
      gmm_psm_timer_ctxt.psm_set_cfg_req_status = NAS_PSM_CFG_REQ_PROCESSING;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ------------------------------------ */
  ...
  /* After eDRX and PSM params processing, MM sends eDRX/PSM update to GRR */
  mm_send_rr_mtc_param_update_ind(FALSE);
  ...
  else /* routing_area_updating_attempt_counter >= 5 */
  {
    ...
    ** ----------------------------------------------------------------------------- */
        ...
        (gmm_ra_update_type == GMM_COMBINED_RA_LA_UPDATING_WITH_IMSI_ATTACH))
    {
      ...
      /* Delete equivalent PLMN list */
#ifdef FEATURE_NAS_REL12
      /* Spec 24.008 - CR 2335: Delete EPLMN list only when both LU and RAU failed for max times */
...
#ifdef FEATURE_NAS_REL12
    /* Spec 24.008 4.7.5.1.5 and 4.7.5.2.5 - CR 2335: Delete EPLMN list only when both LU and RAU failed for max times */
    if((mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY)
#ifndef FEATURE_NAS_CS_DISABLED 
       ||
...
#endif
    {
      MSG_HIGH_DS_0(MM_SUB,"=MM= RAU failed for max times. Deleting EPLMNs.");
      mm_serving_plmn.update_equiv_plmn_list = TRUE;
      memset(&mm_serving_plmn.equiv_plmn_list, 0x00, sizeof(equiv_plmn_list_s_type));
    }
  }
...
void gmm_handle_routing_area_update( mm_cmd_type *message )
{
  ...
  byte message_id             = 0;
  uint64                      t3312_timer_value = gmm_t3312_timer_value; 
              ...
              ** --------------------------------------------------- */
               ...
               **---------------------------------------------*/
...
#ifdef FEATURE_NAS_REL12
                      /*24.008   C1-142471  CR 2591  When PTAU timer is deactivated*/
                 && !((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12)&&
                          (emm_ctrl_data_ptr->t3412_value == 0))
#endif
                      && emm_ctrl_data_ptr->TIN == NAS_MM_TIN_GUTI)
#endif
                   ) &&
                ...
                if ((ra_update_accept_msg->update_result == GMM_RA_UPDATED_AND_ISR_ACTIVATED ||
                    ...
                    /*  if the MS had initiated the routing area updating procedure due to a change in the UE's usage setting or the voice domain preference for E-UTRAN,
                                           the MS shall set the TIN to "P-TMSI" and shall stop the periodic tracking area update timer T3412, if running;
                     */
#ifdef FEATURE_NAS_REL12
	                      /*C1-143343 CR  2583*/
                      if(mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12)
...
#endif
                              (boolean)((ra_update_accept_msg->update_result == GMM_COMBINED_RA_LA_UPDATED 
#if defined(FEATURE_LTE)
                      || ra_update_accept_msg->update_result == GMM_COMBINED_RA_LA_UPDATED_AND_ISR_ACTIVATED
#endif 
                ...
                ** --------------------- --------------------------*/
                   ...
                   )
                {
                             ...
                             )
                  {
                    ...
                    {
                      gmm_handle_nmo1_reject_cause(ra_update_accept_msg->gmm_cause,FALSE, ra_update_accept_msg->list_of_receive_npdu_numbers_present);
#if 0
...
/*24.008: The behaviour of the MS in those cases is specified in subclause 4.7.5.2.5.*/
                ...
                ** supported.            
                *************************************************************************/ 
               gmm_send_msm_mbms_supp_ind(ra_update_accept_msg->nw_ftr_support);
#endif
#ifndef FEATURE_NAS_DISABLE_VOICE
#if defined(FEATURE_LTE) && defined(FEATURE_LTE_REL9)
        ...
        ** accept. So encoding the PDP STATUS IE in this case.  
                *************************************************************************/         
...
#else
                if ((cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CMCC) || (cm_get_operator_info(SYS_MODEM_AS_ID_1) == OPERATOR_CU) || (mm_is_qrd_device) 
#ifdef DISABLE_9x05_OPTIMIZATIONS
                 || (mm_optimised_ps_signalling_connection_release)
#endif
)
#endif
                {
                  ...
                  is_data_available =  ds3gpp_is_ps_data_available();
#endif
                }
#ifdef FEATURE_WCDMA
#ifdef FEATURE_REL6_PS_FOP
                ...
                cell_notification_ind = ra_update_accept_msg->cell_notification_present ;
                 ...
                 /* Process Active Timer T3324 from network */
                psm_update = gmm_process_receive_psm_timers(ra_update_accept_msg->t3324_value_present,
                                                            ra_update_accept_msg->t3324_value);

                /* Process eDRX parameters from network */
                edrx_update = mm_process_edrx_params(ra_update_accept_msg->edrx_params_present,
                                                     ra_update_accept_msg->edrx_params);
                if (psm_update || edrx_update)
                {
                  /* Send eDRX/PSM update to GRR */
                  mm_send_rr_mtc_param_update_ind(FALSE);
...
#endif
                  if((protocol_discriminator == PD_GMM) && (message_id == GMM_DETACH_REQUEST))
                  {
              ...
              ** ----------------------------------------------------------- */
              ...
              // Sending NW REJECT EVENT TO GSTK
...
#ifdef FEATURE_NAS_REL12
                    /* Spec 24.008 CR 2415: EPLMNs not to be deleted for reject causes #9 and #10 also*/
                    (ra_update_reject_msg.gmm_cause != MS_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK) &&
                    (ra_update_reject_msg.gmm_cause != IMPLICITLY_DETACHED) &&
#endif
               ...
               gmm_reject_cause = ra_update_reject_msg.gmm_cause;
...
#ifdef FEATURE_NAS_REL10
                if(ra_update_reject_msg.t3346_value_present)
                {
                  gmm_t3346_backoff_timer_value = gmm_convert_timer_value_ie_to_usecs(
                ...
                gmm_handle_ota_reject_cause( ra_update_reject_msg.gmm_cause, FALSE);
      ...
      switch (message->cmd.timer_expiry.timer_id)
      {
        case TIMER_T3330:
          ...
          ** ------------------------------------------------------------------- */
          ...
          if (gmm_rau_tx_is_acked)
          {
            gmm_acked_tx_retry_timeout_counter++;
          }

          /*Invalidate extended DRX value after RAU procedure fails for 1st two attempt due to no response*/
          if (is_gsm_mode() && 
              (mm_edrx_cfg.gsm.enabled == TRUE) &&
              (gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled) &&
              (gmm_acked_tx_retry_timeout_counter >= 2))
  ...
  if ((gmm_substate == GMM_LIMITED_SERVICE)||
      ...
      ((gmm_state == GMM_REGISTERED_INITIATED)&&
       ...
       ((message->cmd.hdr.message_id == (int)MMR_STOP_MODE_REQ) &&
        (message->cmd.mmr_stop_mode_req.stop_mode_reason == SYS_STOP_MODE_REASON_PSM) &&
        ((cm_is_psm_context_saved((sys_modem_as_id_e_type)mm_as_id) == TRUE) || 
         (gmm_psm_local_detach_enabled)))
#endif
#ifdef FEATURE_LTE
      ||((message->cmd.hdr.message_id == (int)MMR_STOP_MODE_REQ)&&
...
{
    if(gmm_check_if_cause12_to_cause15_allowed() == TRUE && 
        detach_request_msg_p->gmm_cause == LA_NOT_ALLOWED)
    {
    ...
    else if (gmm_state == GMM_DEREGISTERED_INITIATED)
    {
      ...
      ** ------------------------------------------------------------------ */
...
#endif     
      {
        ...
        {
          gmm_process_gprs_release_ind(mm_convert_gmm_to_sm_cause(detach_request_msg.gmm_cause));
        }
        else
        {
          gmm_process_gprs_release_ind(NO_GPRS_CONTEXT);
      ...
      if (detach_request_msg.detach_type == GMM_REATTACH_REQUIRED) 
      {
        ...
        /* Spec 24.008, Spec CR 2546,
         * The MS can stop the T3346 when receiving DETACH REQUEST message with 
         * detach type indicating "re-attach required".
         *
         * Table 11.3a/3GPP TS 24.008: GPRS Mobility management timers - MS side
         * Stop Condition for T3346 - Paging received or DETACH REQUEST with the 
         * detach type "re-attach required" received
         */
...
#ifdef FEATURE_NAS_REL12
       /*in Iu mode and A/Gb mode, after intersystem change from S1 mode, 
         and the GMM receives an indication of "RRC connection failure" from lower layers 
        due to lower layer failure while in S1 mode.
         In this case, if the TIN indicates "RAT-related TMSI", 
         the MS shall set the TIN to "GUTI" before initiating the routing area updating procedure;
       */
	    if(((mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL12) &&
	         emm_ctrl_data_ptr->TIN == NAS_MM_TIN_RAT_RELATED_TMSI ))
	    {
...
/*24.008: 4.7.5.1: The normal routing area updating procedure is initiated: in A/Gb mode, after Inter RAT handover from S1 mode or Iu mode.*/
                                                       ...
                                                       )
        {
          ...
          ** ----------------------------------------------------------------- */
           ...
           ** nor equivalent PLMN then Attach with IMSI shall be performed instead of RAU procedure */
          ...
          if ((gmm_plmn_changed( gmm_previous_serving_rai.plmn_id,
              ...
              (mm_check_attach_with_imsi(MM_PS_DOMAIN) == TRUE) &&
              ((gmm_state == GMM_SERVICE_REQUEST_INITIATED) ||
               (gmm_state == GMM_DEREGISTERED_INITIATED) ||
               (gmm_state == GMM_REGISTERED)))
          {
...
#ifdef FEATURE_NAS_REL12
          /* Spec 4.7.3, 4.7.5 CR 2386 and CR 2486: *
           * GPRS attach attempt counter OR RAU attempt counter shall be reset when a new PLMN is selected */
          if(gmm_plmn_changed(gmm_previous_registration_serving_rai.plmn_id, mm_serving_plmn.info.plmn))
          {	  
            if ((gmm_state == GMM_DEREGISTERED) ||
                (gmm_state == GMM_DEREGISTERED_INITIATED) ||
                (gmm_state == GMM_REGISTERED_INITIATED))
          ...
          switch (gmm_state)
          {
           ...
           /*Reset 3330 attempt counter as RAU is being triggered on new RAI*/
            ...
            case GMM_DEREGISTERED:
              ...
              else if ((mm_check_attach_with_imsi(MM_PS_DOMAIN) == TRUE) &&
                       (gmm_substate == GMM_NORMAL_SERVICE))
              {
                /*  If UE is configured for "AttachwithIMSI" and current camped PLMN is neither RPLMN 
                ** nor equivalent PLMN then attach with IMSI is performed on entering new PLMN */
                gmm_initiate_gprs_attach();
              }
            ...
            case GMM_ROUTING_AREA_UPDATING_INITIATED:
              ...
              {             
                ...
                ** nor equivalent PLMN then attach with IMSI shall be performed instead of RAU procedure */
                ...
                if (mm_check_attach_with_imsi(MM_PS_DOMAIN) == TRUE)
                {
                  MSG_HIGH_DS_0( MM_SUB, "=MM= AttachWithIMSI is TRUE- Perform local detach and Start Attach procedure");
                  gmm_handle_local_gprs_detach();
                  gmm_initiate_gprs_attach();
                }
                else
...
#endif /* #if defined(FEATURE_GSM_GPRS) && defined (FEATURE_NAS_GW) */
        if((mm_serving_plmn.last_plmn_service_capability ==SYS_SRV_CAPABILITY_CS_SERVICE_ONLY ) && //CHKDBG
           (mm_serving_plmn.info.plmn_service_capability == SYS_SRV_CAPABILITY_CS_PS_SERVICE))
        {
...
boolean gmm_check_rau_reject_for_integrity_protection(mm_cmd_type *message)
{
  ...
  if(msg_index < MIN(msg_length,gmm_max_octet_length))
  {
    if(!gmm_integrity_protection_activated &&
       (mm_serving_csg_id != SYS_CSG_ID_INVALID) &&
       (message->cmd.mm_data_ind.L3_data[2] == CSG_NOT_AUTHORIZED))
...
#ifdef FEATURE_CIOT          
          /* Run a local detach if RATs have changed and UE was attached before*/
          if (message->cmd.rr_plmn_select_cnf.service_state != MM_AS_NO_SERVICE)
          {
             if ((gmm_state != GMM_DEREGISTERED) && 
             ...
             {
                 MSG_HIGH_DS_0(MM_SUB, "=MM= Local Detach on RAT Change");
                 emm_ctrl_data_ptr->attached_lte_rat = SYS_RAT_NONE;
                 gmm_handle_local_gprs_detach();
             }
          }
...
#endif
              {
                /* For REL 12: 24.008 4.7.5.1.5 case #k, CR 2411 : RAU should be aborted and detach should go */
                gmm_initiate_gprs_detach( message ); 
              }
#endif /*#if (defined (FEATURE_GSM) || defined (FEATURE_WCDMA)) && defined(FEATURE_NAS_GW)*/
        ...
        case TIMER_T3311:
                    ...
                    ((gmm_substate == GMM_SUSPENDED) || (is_gsm_mode() && gmm_is_suspended)))
          {
            gmm_gprs_attach_pending = TRUE;
                  ...
                  (!gmm_is_congestion_due_to_rau && !gmm_periodic_rau_pending && !gmm_ra_update_pending ) &&
#ifdef FEATURE_LTE
                  (emm_ctrl_data_ptr->TIN != NAS_MM_TIN_GUTI) &&
#endif 
                  /*  ---------------------------------------------------------- 
