...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmdeact.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#endif
#if defined(FEATURE_NAS_GW) && defined (FEATURE_LTE)
               else if(mm_last_active_rat_stopped == SYS_RAT_LTE_RADIO_ACCESS)
               {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       )
   {
     ...
     {
       /*Reset T3346 started due to implcit reject from GRR on deactivation*/
      ...
      for (i=0; i < MAX_MM_TIMER+1; i++)
      {
         if ((mm_timer_status[i] != TIMER_STOPPED) &&
             ...
             ((i != EMM_FORBIDDEN_TAI_CLEAR_TIMER) ||
              (mmcoord_pending_reg_message.cmd.mmr_stop_mode_req.stop_mode_reason !=
                       SYS_STOP_MODE_REASON_IRAT))&&
              !( i == TIMER_T3247 && is_t3247_need_not_be_stopped))
         {
            mm_stop_timer( i );
      ...
      */
            ...
            SYS_STOP_MODE_REASON_OFFLINE)
      {
         rr_deact_req.deact_reason = RR_MM_OFF_LINE;
               ...
               SYS_STOP_MODE_REASON_PSM)
      {
         rr_deact_req.deact_reason = RR_MM_PSM;
      }
      else
      {
