...
extern uint8   mm_emm_is_t3247_enabled_sim[MAX_NAS_STACKS];
...
#else
extern uint8  mm_emm_is_t3247_enabled;
...
extern void mm_send_mmr_sim_state_revalidate_ind( const sys_sim_state_e_type sim_state);
...
void mm_set_nas_bit_mask(uint16 bit_mask)
{
  ...
  mm_client_activated_group = bit_mask &  0x007F; 
...
boolean mm_get_nas_stats_info(sys_nas_information_s_type* nas_information)
{
  ...
  else
  {
...
#ifdef FEATURE_LTE
    if(mm_client_activated_group & SYS_NAS_EMM_INFO_MASK)
    {
       /*send this always whenever client queries about this field*/
       nas_information->changed_bitmask |= SYS_NAS_EMM_INFO_MASK;
    }
#endif
#ifdef FEATURE_NAS_GW
    ...
    {
      nas_information->periodic_timers.T3212_value= (dword)mm_stored_per_ms_t3212_value;
    }
    else
#endif
    {
      ...
      nas_information->periodic_timers.T3312_value= (dword)gmm_t3312_timer_value;
...
#ifdef FEATURE_LTE
    nas_information->emm_state = EMM_GET_STATE();
    ...
    {
      nas_information->emm_connection_state = 1;
    }
    else
    {
      nas_information->emm_connection_state = 0;
    }

    MSG_HIGH_DS_2(MM_SUB,"=MM= NAS INFO state = %d , connection status= %d  ",
                                        nas_information->emm_state,nas_information->emm_connection_state);
#endif
...
FUNCTION mm_convert_acq_mode_to_sys_acq

DESCRIPTION
  This function converts the acquisition mode to radio access techonology
...
{  
  if(rat == SYS_SYS_MODE_GSM)
    return SYS_RAT_GSM_RADIO_ACCESS;
     
  if(rat == SYS_SYS_MODE_WCDMA)
    return SYS_RAT_UMTS_RADIO_ACCESS;
     
  if(rat == SYS_SYS_MODE_LTE)
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_psm_read_cache(mm_psm_cache_s_type *psm_cache_ptr)
{
  mm_psm_cache_ptr = (mm_psm_cache_s_type *) modem_mem_calloc(1, sizeof(mm_psm_cache_s_type), MODEM_MEM_CLIENT_NAS);
  mm_check_for_null_ptr((void*)mm_psm_cache_ptr);
  memset((void*)mm_psm_cache_ptr, 0, sizeof(mm_psm_cache_s_type));

  memscpy(mm_psm_cache_ptr, sizeof(mm_psm_cache_s_type),
                       psm_cache_ptr, sizeof(mm_psm_cache_s_type));

  mm_psm_restore_pending = TRUE;
  if (mm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(mm_get_ulog_handle(), "=MM= MM PSM Context retrieved from CM");
  }
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_read_psm_config(void)
{
  mcfg_fs_status_e_type status = MCFG_FS_STATUS_EFS_ERR, status1 = MCFG_FS_STATUS_EFS_ERR;
  cm_psm_config_s_type      psm_config;
  nas_psm_timer_info_s_type nas_psm_timer_info;
  nas_ext_psm_timer_info_s_type nas_ext_psm_timer_info;
  boolean psm_btw_edrx_enabled = FALSE;
  nas_psm_context_cache_s *psm_context = NULL;
  uint32 context_size = 0;

  mm_psm_r12_enabled = FALSE;
  mm_psm_restore_pending = FALSE;
  mm_psm_ready_req_rejected = FALSE;
  mm_psm_mode_req_received = FALSE;
  mm_psm_nv_list_bitmask = 0;
  mm_psm_cache_ptr = NULL;
     
  memset((void*)&psm_config, 0, sizeof(cm_psm_config_s_type));
  memset((void*)&nas_psm_timer_info, 0, sizeof(nas_psm_timer_info_s_type));
  memset((void*)&nas_ext_psm_timer_info, 0, sizeof(nas_ext_psm_timer_info_s_type));
  ...
  {
    mm_psm_r12_enabled = FALSE; /* Default value disabled */
	// Enable by default as MMODE has enabled by default
  ...
  {
    mm_psm_r12_enabled = (((psm_config.psm_version 
    ...
    {
      dog_hb_report(DOG_MM_RPT);
      status1 = mcfg_fs_read("/nv/item_files/modem/nas/nas_psm_timer_info", 
                              &nas_psm_timer_info,sizeof(nas_psm_timer_info_s_type),
                               MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_as_id);

      if((status1 != MCFG_FS_STATUS_OK) ||
         (mm_validate_psm_timer_info(nas_psm_timer_info.active_timer, nas_psm_timer_info.periodic_update_timer) == FALSE))
      {
        nas_psm_timer_info.active_timer = 20; /* Default 20 seconds */
        nas_psm_timer_info.periodic_update_timer = 24 * 60 * 60; /* Default 24 hours */
        nas_ext_psm_timer_info.gprs_ready_timer = 20; /* Default 20 seconds */
        nas_ext_psm_timer_info.periodic_rau_update_timer = 24 * 60 * 60; /* Default 24 hours */
      }
      else
      {

        status1 = mcfg_fs_read("/nv/item_files/modem/nas/nas_ext_psm_timer_info", 
                                &nas_ext_psm_timer_info,sizeof(nas_ext_psm_timer_info_s_type),
                                MCFG_FS_TYPE_EFS,(mcfg_fs_sub_id_e_type)mm_as_id);

        if((status1 != MCFG_FS_STATUS_OK) ||
           (mm_validate_psm_timer_info(nas_ext_psm_timer_info.gprs_ready_timer, nas_ext_psm_timer_info.periodic_rau_update_timer) == FALSE))
        {
          nas_ext_psm_timer_info.gprs_ready_timer = 20; /* Default 20 seconds */
          nas_ext_psm_timer_info.periodic_rau_update_timer = 24 * 60 * 60; /* Default 24 hours */
        }
      ...
      mm_psm_active_timer = (nas_psm_timer_info.active_timer) * 1000;
      mm_psm_periodic_tau_update_timer = (uint64)(nas_psm_timer_info.periodic_update_timer) * 1000;

      /* Update Extended timer value in milliseconds */
      mm_psm_gprs_ready_timer = (nas_ext_psm_timer_info.gprs_ready_timer) * 1000;
      mm_psm_periodic_rau_update_timer = (uint64)(nas_ext_psm_timer_info.periodic_rau_update_timer) * 1000;
    }
    ...
    {
      mm_psm_nv_list_bitmask = 0xFFFFFFFF;
    }
  }
  else
  {
  ...
  if(mm_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_9(mm_get_ulog_handle(),
    "=MM= EFS psm_config read: status = %d, overall_psm_enabled = %d, psm_r12_enabled = %d, psm_version = 0x%x, \
    EFS nas_psm_timer_info read: status = %d. active_timer = %u sec , gprs ready timer = %u sec , periodic tau update timer = %u sec, periodic rau update timer = %u sec ",
    status, psm_config.is_enabled, mm_psm_r12_enabled, psm_config.psm_version,
    status1, mm_psm_active_timer/1000, mm_psm_gprs_ready_timer/1000, mm_psm_periodic_tau_update_timer/1000, mm_psm_periodic_rau_update_timer/1000);

    ULOG_RT_PRINTF_1(mm_get_ulog_handle(), "=MM= psm_btw_edrx_enabled %d", psm_btw_edrx_enabled);
  }
   
  MSG_HIGH_DS_7(MM_SUB, "=MM= EFS psm_config read: status = %d, overall_psm_enabled = %d, psm_r12_enabled = %d, psm_version = 0x%x, \
  EFS nas_psm_timer_info read: status = %d. active_timer = %u sec , periodic update timer = %u sec ",
  status, psm_config.is_enabled, mm_psm_r12_enabled, psm_config.psm_version, 
  status1, mm_psm_active_timer/1000, mm_psm_periodic_tau_update_timer/1000);

  MSG_HIGH_DS_3(MM_SUB, "=MM=  psm_btw_edrx_enabled %d, gprs ready timer = %u sec, periodic rau update timer = %u sec",
                 psm_btw_edrx_enabled, mm_psm_gprs_ready_timer/1000, mm_psm_periodic_rau_update_timer/1000);
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_psm_update_cache_bitmask
(
  uint32 bitmask,
  mm_nv_list_psm_e_type NV_EFS
)
{
  REG_CLEAR_BM_32BIT(mm_psm_nv_list_bitmask, NV_EFS);
}
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_psm_write_cache(mm_psm_cache_s_type *psm_cache_ptr)
{
  ...
  {
    memscpy(psm_cache_ptr, sizeof(mm_psm_cache_s_type),
                         mm_psm_cache_ptr, sizeof(mm_psm_cache_s_type));

    modem_mem_free(mm_psm_cache_ptr, MODEM_MEM_CLIENT_NAS);
    mm_psm_cache_ptr = NULL;
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_psm_update_restore_status(void)
{
  mm_nv_list_psm_e_type NV_EFS = 0;
...
#endif
  {
    if(mm_psm_cache_ptr != NULL)
    {
      ...
      MSG_HIGH_DS_2(MM_SUB, "=MM= Stored Mask %d  Current Mask = %d", mm_psm_cache_ptr->psm_nv_list_bitmask, 
                                                                      mm_psm_nv_list_bitmask);
      if(mm_get_ulog_handle() != NULL)
      {
         ULOG_RT_PRINTF_2(mm_get_ulog_handle(),"=MM= Stored Mask %d  Current Mask = %d", mm_psm_cache_ptr->psm_nv_list_bitmask, 
                                                                                         mm_psm_nv_list_bitmask);
      }

      for(NV_EFS = 0; NV_EFS < MM_PSM_NV_EFS_MAX; NV_EFS++)
      {
        if((REG_CHECK_BM_32BIT(mm_psm_cache_ptr->psm_nv_list_bitmask, NV_EFS) == TRUE) &&
            (REG_CHECK_BM_32BIT(mm_psm_nv_list_bitmask, NV_EFS) == FALSE))
        {
          mm_psm_restore_pending = FALSE;
          mm_psm_free_cache();
          if(mm_get_ulog_handle() != NULL)
...
sys_psm_status_e_type mm_check_ready_for_psm(void)
{
  ...
  if ( is_umts_mode() || is_gsm_mode())
  {
    ...
    else if (((is_gmm_registering == TRUE) ||
               (mm_state != MM_IDLE) ||
               (rrc_connection_status != MMRRC_IDLE) ||
               (mm_sms_is_active == TRUE) ||
               (is_ps_call_active == TRUE)
          ||
          (is_gsm_mode() && (gmmllc_get_glif_state() == GMM_READY_STATE))
          ||
    ...
    else if ((gmm_psm_timer_ctxt.t3324_value != NAS_INVALID_PSM_TIMER_VALUE) &&
             (gmm_psm_timer_ctxt.allow_psm_in_oos_ltd_srv == FALSE) &&
...
SIDE EFFECTS
  None
===========================================================================*/
boolean mm_validate_psm_timer_info
(
  uint32 active_timer,
...
)
{
  ...
  else
  {
    for(i=1; i<32; i++)
    {
      if((active_timer == 2*i)     /* multiples of 2 seconds upto 62 seconds */
         ||
         (active_timer == 60*i)    /* multiples of 1 minute upto 31 minutes */
         ||
         (active_timer == 6*60*i)) /* multiples of 6 minutes upto 186 minutes */
      {
        valid = TRUE;
        break;
      }
  ...
  /* Validate periodic update timer if active timer is valid */

  if((valid == TRUE) && (periodic_update_timer != 0))
  {
    valid = FALSE;
    for(i=1; i<32; i++)
    {
      if((periodic_update_timer == 2*i)         /* multiples of 2 seconds upto 62 seconds */
         ||
         (periodic_update_timer == 30*i)        /* multiples of 30 seconds upto 930 seconds */
         ||
         (periodic_update_timer == 60*i)        /* multiples of 1 minute upto 31 minutes */
         ||
         (periodic_update_timer == 10*60*i)     /* multiples of 10 minutes upto 310 minutes */
         ||
         (periodic_update_timer == 60*60*i)     /* multiples of 1 hour upto 31 hours */
         ||
         (periodic_update_timer == 10*60*60*i)  /* multiples of 10 hours upto 310 hours */
         ||
         (periodic_update_timer == 320*60*60*i)) /* multiples of 320 hours upto 9920 hours */
      {
        valid = TRUE;
        break;
      }
  ...
  if(valid == FALSE)
  {
    MSG_ERROR_DS_2(MM_SUB, "=MM= Invalid PSM timer info: active_timer %d, periodic update timer %d",
                           active_timer, periodic_update_timer);
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean mm_update_psm_config_params
(
  mmr_psm_set_config_params_req_s_type *msg_ptr
)
{
  boolean nw_update_needed = FALSE;
  boolean psm_r12_enabled_old = mm_psm_r12_enabled;
  boolean timer_changed = FALSE;
  mcfg_fs_status_e_type efs_status = MCFG_FS_STATUS_EFS_ERR;

  /* Check which bits in the bitmask changed, update the psm_rel12 flag and
     PSM timer NV accordingly */
  if(msg_ptr->change_mask != 0)
  {
    if(((msg_ptr->change_mask & CM_PSM_CONFIG_INFO_MASK_PSM_ENABLED) != 0) &&
       (msg_ptr->psm_enabled != mm_psm_r12_enabled))
    {
      mm_psm_r12_enabled = msg_ptr->psm_enabled;
    }
    
    /* Timer values in msg_ptr are in seconds. Convert to milliseconds before using */
    if(((msg_ptr->change_mask & CM_PSM_CONFIG_INFO_MASK_ACTIVE_TIMER) != 0) &&
       ((msg_ptr->active_timer_value * 1000) != mm_psm_active_timer))
    {
      mm_psm_active_timer = (msg_ptr->active_timer_value * 1000);
      timer_changed = TRUE;
    }

    if(((msg_ptr->change_mask & CM_PSM_CONFIG_INFO_MASK_GPRS_READY_TIMER) != 0) &&
       ((msg_ptr->gprs_ready_timer_value * 1000) != mm_psm_gprs_ready_timer))
    {
      mm_psm_gprs_ready_timer = (msg_ptr->gprs_ready_timer_value * 1000);
      timer_changed = FALSE;
    }

    if(((msg_ptr->change_mask & CM_PSM_CONFIG_INFO_MASK_PERIODIC_UPDATE_TIMER) != 0) &&
       ((msg_ptr->periodic_tau_timer_value * 1000) != mm_psm_periodic_tau_update_timer))
    {
      mm_psm_periodic_tau_update_timer = ((uint64)msg_ptr->periodic_tau_timer_value * 1000);
      timer_changed = TRUE;
    }

    if(((msg_ptr->change_mask & CM_PSM_CONFIG_INFO_MASK_PERIODIC_RAU_TIMER) != 0) &&
       ((msg_ptr->periodic_rau_timer_value * 1000) != mm_psm_periodic_rau_update_timer))
    {
      mm_psm_periodic_rau_update_timer = ((uint64)msg_ptr->periodic_rau_timer_value * 1000);
      timer_changed = TRUE;
    }

    /* Update NV/EFS if timer value changed */
    if(timer_changed == TRUE)
    {
      nas_psm_timer_info_s_type nas_psm_timer_info;
      nas_ext_psm_timer_info_s_type nas_ext_psm_timer_info;
      ...
      nas_psm_timer_info.active_timer = mm_psm_active_timer/1000;
      nas_psm_timer_info.periodic_update_timer = (uint32)(mm_psm_periodic_tau_update_timer/1000);

      nas_ext_psm_timer_info.gprs_ready_timer = mm_psm_gprs_ready_timer/1000;
      nas_ext_psm_timer_info.periodic_rau_update_timer = (uint32)(mm_psm_periodic_rau_update_timer/1000);

      efs_status = mcfg_fs_write("/nv/item_files/modem/nas/nas_psm_timer_info", 
                                 &nas_psm_timer_info,
                                 sizeof(nas_psm_timer_info_s_type),
                                 MCFG_FS_O_RDWR|MCFG_FS_O_CREAT|MCFG_FS_O_TRUNC|MCFG_FS_O_AUTODIR,
                                 MCFG_FS_ALLPERMS, 
                                 MCFG_FS_TYPE_EFS,
                                 (mcfg_fs_sub_id_e_type)mm_sub_id);

      efs_status = mcfg_fs_write("/nv/item_files/modem/nas/nas_ext_psm_timer_info", 
                                 &nas_ext_psm_timer_info,
                                 sizeof(nas_ext_psm_timer_info_s_type),
                                 MCFG_FS_O_RDWR|MCFG_FS_O_CREAT|MCFG_FS_O_TRUNC|MCFG_FS_O_AUTODIR,
                                 MCFG_FS_ALLPERMS, 
                                 MCFG_FS_TYPE_EFS,
                                 (mcfg_fs_sub_id_e_type)mm_sub_id);
    ...
    /* Update network if PSM mode changed, or PSM is enabled and active and/or 
       periodic update timer configuration changed */
    if((mm_psm_r12_enabled != psm_r12_enabled_old)
       ||
       ((mm_psm_r12_enabled == TRUE) &&
        (timer_changed == TRUE)))
    {
      nw_update_needed = TRUE;
    }

    MSG_HIGH_DS_8(MM_SUB, "=MM= PSM config params update: psm_r12_cur = %d, psm_r12_old = %d, timer_changed = %d, nw_update = %d, active_timer = %u sec, periodic_tau_update_timer = %u sec, efs_status = %d, efs_errno = %d",
                           mm_psm_r12_enabled, psm_r12_enabled_old, timer_changed, nw_update_needed,
                           mm_psm_active_timer/1000, mm_psm_periodic_tau_update_timer/1000, efs_status, efs_errno);
    MSG_HIGH_DS_2(MM_SUB, "=MM= PSM config params update: gprs ready timer = %u sec, periodic_rau_update_timer = %u sec",
                           mm_psm_gprs_ready_timer/1000, mm_psm_periodic_rau_update_timer/1000);
  }

  return nw_update_needed;
}
...
(
  ...
  boolean dos_mitigation_req
)
{

#ifdef FEATURE_DUAL_SIM
  if(reg_sim_add_plmn_to_fplmn_list( mm_serving_plmn.info.plmn, dos_mitigation_req, (sys_modem_as_id_e_type)mm_sub_id))
#else
  if (reg_sim_add_plmn_to_fplmn_list(plmn, dos_mitigation_req)) 
...
{
#ifdef FEATURE_DUAL_SIM
  if (reg_sim_add_plmn_to_gprs_fplmn_list( mm_serving_plmn.info.plmn, dos_mitigation_req, (sys_modem_as_id_e_type)mm_sub_id))
#else
  if (reg_sim_add_plmn_to_gprs_fplmn_list(plmn, dos_mitigation_req))
#endif /*FEATURE_DUAL_SIM */
  {
  #ifdef FEATURE_LTE_TO_1X
  ...
  ms_op_mode_local = mm_serving_plmn_sim[stack_id_nas].ms_op_mode;
    ...
    if(is_umts_mode_per_nas_stacks(stack_id_nas) || is_gsm_mode_per_nas_stacks(stack_id_nas))
    {
      ...
      if(((ms_op_mode_local == SYS_SRV_DOMAIN_PS_ONLY) ||
        ...
        ((is_gsm_mode_per_nas_stacks(stack_id_nas) && ds3gpp_is_ps_data_available() && (gmm_is_ps_active_sim[stack_id_nas] == TRUE))
#if defined(FEATURE_LTE)
...
#endif
         && (gmm_substate == GMM_NORMAL_SERVICE)
         && ( ( is_gsm_mode() && (gmm_is_ps_active == TRUE) && ds3gpp_is_ps_data_available())
#if defined(FEATURE_LTE)
...
void mm_log_mm_reg_variables()
{
...
#endif  
  {
    ...
    MSG_HIGH_DS_6( i, "=MM= mm_need_to_process_sim_avail_req/waiting_for_open=%2x, session_type = %d, mm_sim_card_mode=%d, Active Rat=%d(0=gsm, 1=wcdma, 2=lte), mm_waiting_for_deact_cnf =%d, mm_waiting_for_ph_status_change_cnf =%d", 
  ...
  if(MM_SUB_IS_SGLTE)
  {
    MSG_HIGH_DS_5(MM_SUB,"=MM= NAS SGLTE current stack status = %d  sglte mode = %d,mm_cs_stack_id= %d, reg_sglte_ue_mode = %d  reg_cs_stack_id = %d ",
                                        mm_sglte_current_stack_status,mm_sglte_mode,mm_cs_stack_id,
                                        reg_sglte_ue_mode,reg_cs_stack_id);
  }
#endif
#ifdef FEATURE_LTE
  MSG_HIGH_DS_5(MM_SUB,"=EMM= EMM State = %d, EMM Substate = %d,  EMM Connection State = %d, LRRC Active = %d, Last LRRC deact conf status = %d", 
  ...
  MSG_HIGH_DS_4(MM_SUB,"=EMM= LRRC deact req reason =%d, LRRC deact req pending =%d, LRRC aborting conn = %d, Registered serv domain = %d", 
...
void mm_log_mm_reg_variables()
{
...
#else
  MSG_HIGH_DS_2(MM_SUB,"=MM= mm_sim_card_mode=%d, Active Rat=%d(0=gsm, 1=wcdma, 2=lte)",
   mm_sim_card_mode, mm_serving_plmn.info.active_rat);
#endif
#ifdef FEATURE_LTE
  ...
  MSG_HIGH_DS_2(MM_SUB,"=EMM= LRRC aborting conn = %d, Registered serv domain = %d", 
                            emm_ctrl_data_ptr->aborting_conn, emm_ctrl_data_ptr->srv_domain_registered);
#ifdef FEATURE_NAS_GW
...
#endif
#ifdef FEATURE_LTE
  if(is_lte_mode() && (emm_ctrl_data_ptr->aborting_conn))
  {
    MSG_HIGH_DS_1(MM_SUB,"=MM= RAT %d (0=GSM, 1=WCDMA, 2=LTE, 3=TDS) was not able to respond to NAS for ABORT_REQ",mm_serving_plmn.info.active_rat);
  }
  ...
  else
  {
    ...
    else
    {
...
#ifdef FEATURE_LTE
       if(mm_client_activated_group_sim[as_id] & SYS_NAS_EMM_INFO_MASK)
       {
         /*send this always whenever client queries about this field*/
         nas_information->changed_bitmask |= SYS_NAS_EMM_INFO_MASK;
       }
       ...
       if (mm_nas_changed_group_sim[as_id] & SYS_NAS_PERIODIC_TIMER_MASK)
       {
         ...
         nas_information->periodic_timers.T3312_value= (dword)gmm_t3312_timer_value_sim[as_id];
...
#ifdef FEATURE_LTE
       nas_information->emm_state = emm_ctrl_data_ptr->emm_state;
       if (!((connection_state == EMM_IDLE_STATE) || ((connection_state == EMM_RELEASING_RRC_CONNECTION_STATE) 
                                                       && (emm_ctrl_data_ptr->aborting_conn == FALSE))))
       {
         nas_information->emm_connection_state = 1;
...
void mm_per_nas_stacks_set_nas_bit_mask(uint16 bit_mask, sys_modem_as_id_e_type stack_id_nas)
{
  ...
  {
    rex_enter_crit_sect(&mm_ftd_crit_sect);
    mm_client_activated_group_sim[stack_id_nas] = bit_mask & 0x007F; 
    rex_leave_crit_sect(&mm_ftd_crit_sect);
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean mm_write_backoff_remaining_info_to_efs(void)
{
  int32 efs_status;
  ...
  if(((gmm_t3346_backoff_timer_value != 0)
	  ...
	  || (mm_t3246_backoff_timer_value != 0)
#endif
	  ) || (mm_is_t3246_or_t3346_value_non_zero))
  {
#ifdef FEATURE_DUAL_SIM
  if(!MM_SUB_IS_SGLTE &&
     (mm_sub_id == MM_AS_ID_2))
  {
	...
	backoff_timer.is_t3346_started_for_mo_excp = FALSE;
#ifdef FEATURE_LTE
#ifdef FEATURE_CIOT
   backoff_timer.is_t3346_started_for_mo_excp = emm_ctrl_data_ptr->is_t3346_started_for_mo_excp;
#endif
#endif
    ...
    {
      MSG_HIGH_DS_3(MM_SUB, "=MM= Updated t3346 timer val from NV - %lu , t3246 timer val from NV - %lu efs_status - %d", backoff_timer.t3346_value, backoff_timer.t3246_value,efs_status);
      MSG_HIGH_DS_3(MM_SUB, "=MM= Updated t3346 backoff PLMN with val %X,%X,%X", backoff_timer.t3346_plmn.identity[0],backoff_timer.t3346_plmn.identity[1],backoff_timer.t3346_plmn.identity[2]);
      MSG_HIGH_DS_3(MM_SUB, "=MM= Updated t3246 backoff PLMN with val %X,%X,%X", backoff_timer.t3246_plmn.identity[0], backoff_timer.t3246_plmn.identity[1],backoff_timer.t3246_plmn.identity[2]);
      if(mm_psm_cache_ptr != NULL)
      {
        REG_SET_BM_32BIT(mm_psm_cache_ptr->psm_nv_list_bitmask, MM_PSM_EFS_MM_BACKOFF_REMAINING_INFO);
      }
    }
  }
#endif
...
)
{
  ...
  MSG_HIGH_DS_2(MM_LOCAL_SUB, "=EMM= umid: %d message_direction: %d", msg_umid,mm_emm_message_direction);
  mm_msg_info = 0;
...
void nas_assert_check(boolean condition)
{
  if(condition == FALSE)
  {
    MSG_FATAL_DS(MM_SUB,"=MM= Error: Assert condition check fail", 0, 0, 0);
  }
}
...
FUNCTION mm_checkif_rat_pri_list_has_only_rat

DESCRIPTION
  This function verifies if the passed rat is the only rat present in the
  rat priority list.
...
{
  boolean is_rat_found = FALSE;
  sys_sys_mode_e_type  acq_sys_mode;
  uint8 i=0;
  sys_rat_pri_list_info_s_type  *rat_pri_list_info = reg_mode_curr_rat_pri_list_get();
     ...
     for (j = 0; j < rat_pri_list_info_2->num_items; j++)
     {
              ...
              rat_pri_list_info_2->priority_list_info[j].acq_sys_mode)
        {
          if(rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE ||
             rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_M1 || 
             rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_NB1
            )
          {
...
boolean mm_rat_pri_list_match_ciot
(
  const sys_rat_pri_list_info_s_type    *rat_pri_list_info_1, 
  const sys_rat_pri_list_info_s_type    *rat_pri_list_info_2
)
{
  uint32 i,gsm_index1= 0,gsm_index2 =0;
  boolean is_lte_found_in_list1, is_grat_found_in_list1;
  boolean is_lte_found_in_list2, is_grat_found_in_list2;
 
  is_lte_found_in_list1 = FALSE;
  is_grat_found_in_list1 = FALSE;


  for (i = 0; i < rat_pri_list_info_1->num_items; i++)
  {
    if(rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_GSM)
    {
      is_grat_found_in_list1 = TRUE;
      gsm_index1 = i;
    }
    else if(rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_M1)
    {
      is_lte_found_in_list1 = TRUE;
    }
    else if(rat_pri_list_info_1->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_NB1)
    {
      is_lte_found_in_list1 = TRUE;
    }
  }
  
  is_lte_found_in_list2 = FALSE;
  is_grat_found_in_list2 = FALSE;

  for (i = 0; i < rat_pri_list_info_2->num_items; i++)
  {
    if(rat_pri_list_info_2->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_GSM)
    {
      is_grat_found_in_list2 = TRUE;
      gsm_index2 = i;
    }
    else if(rat_pri_list_info_2->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_M1)
    {
      is_lte_found_in_list2 = TRUE;
    }
    else if(rat_pri_list_info_2->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_NB1)
    {
      is_lte_found_in_list2 = TRUE;
    }
  }

  if ((is_lte_found_in_list1 != is_lte_found_in_list2) ||
      (is_grat_found_in_list1 != is_grat_found_in_list2))
    return FALSE;

  if ((is_grat_found_in_list2 == TRUE) &&
      (rat_pri_list_info_1->priority_list_info[gsm_index1].band_cap.chgwt_band_cap !=
              rat_pri_list_info_2->priority_list_info[gsm_index2].band_cap.chgwt_band_cap))
    return FALSE;
    ...
    case MS_MM_REG :
      {
        switch (primitive->message_id)
        {
          ...
          case MMR_SIM_STATE_REVALIDATE_IND:
            ((mmr_sim_state_revalidate_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;
          ...
          case MMR_PSM_AUTO_READY_IND:
            ((mmr_psm_auto_ready_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;

          case MMR_PSM_TIMER_CHANGE_IND:
            ((mmr_psm_timer_change_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;

          case MMR_PSM_SET_CONFIG_PARAMS_RSP:
            ((mmr_psm_set_config_params_rsp_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;

          case MMR_PSM_GET_TIMER_INFO_RSP:
            ((mmr_psm_get_timer_info_rsp_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;

          case MMR_PSM_RESTORE_FAILED_IND:
            ((mmr_psm_restore_failed_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;

          case MMR_EDRX_PARAMS_CHANGE_IND:
            ((mmr_edrx_params_change_ind_s_type*)primitive)->as_id = (sys_modem_as_id_e_type)mm_as_id;
            break;
        ...
        switch (primitive->message_id)
        {
         ...
         case RR_SERVICE_STATUS_IND:
          ...
          case RR_MTC_PSM_EDRX_PARAMS_UPDATE_IND:
            ((rr_mtc_nw_config_param_update_ind_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
            break;
          
          case RR_GMM_READY_TIMER_STATUS_IND:
            ((rr_gmm_ready_timer_status_ind_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
            break;

          case RR_DEVICE_PRIORITY_IND:
            ((rr_device_priority_ind_T*)primitive)->as_id = (sys_modem_as_id_e_type)mm_sub_id;
            break;
   ...
   if ( mm_held_cnm_service_req.present != FALSE )    /* contains a message */
   {
          ...
          )
      {
         ...
         gs_read_coded_classmark_2( (classmark_2_ie_T *)write_ptr, mm_gsm_supported,
                                  (boolean)(mm_wcdma_supported||mm_tdscdma_supported), is_umts_mode(), mm_system_information.mscr );
#endif
                                ...
                                byte   *msg_ptr)
{
   ...
   if ( mm_dual_standby_mode  || mm_sglte_mode)
   {
     ...
     pkt_size = NAS_STD_OFFSETOF(LOG_UMTS_DS_NAS_OTA_MESSAGE_LOG_PACKET_C_type, nas_message) + msg_size;
   ...
   {
     /* Calculate log packet to be sent */
     pkt_size = NAS_STD_OFFSETOF(LOG_UMTS_NAS_OTA_MESSAGE_LOG_PACKET_C_type, nas_message) + msg_size;
...
void mmsend_mm_info_log_packet(void)
{
   ...
   MSG_HIGH_DS_8(MM_SUB, "=MM= NW Op mode %d, service_type %d, rac %d\n =MM= Selected PLMN %X %X %X \n =MM= Selected LAC %X %X", 
             mm_serving_plmn.network_op_mode,mm_serving_plmn.info.plmn_service_capability, mm_serving_plmn.rac,
             mm_serving_plmn.info.plmn.identity[0],mm_serving_plmn.info.plmn.identity[1], mm_serving_plmn.info.plmn.identity[2],
             mm_serving_plmn.lac.lac[0], mm_serving_plmn.lac.lac[1]);
...
#if !defined(FEATURE_WCDMA) && !defined(FEATURE_CIOT)
   ...
   switch(event)
    {
...
#if defined (FEATURE_DUAL_SIM) || defined (FEATURE_WWAN_GNSS_DYNAMIC)
      ...
      case MM_STD_LCS_IN_PROGRESS:
               ...
               *cause_value=OTHER_CAUSE;       
               break;  
#endif
      case MM_STD_T3230_TIMEOUT:
	  *cause_type=MM_REJECT_CAUSE; 
          *cause_value=MM_REJ_TIMER_T3230_EXP;       
          break;
...
)
{
  ...
  MSG_HIGH_DS_4(MM_SUB, "=MM= PPAC_PRESENT: %d CS_REG_ALLOWED: %d PS_REG_ALLOWED: %d PAGING_ALLOWED: %d",
  ...
  else if (mm_state == MM_IDLE )
  {
    ...
    else if (mm_idle_substate != MM_PLMN_SEARCH)
    {
      if (mm_serving_plmn.mm_service_state == SERVICE_NOT_AVAILABLE)
      {
        ...
        mm_serving_plmn.cause = MMR_CAUSE_NOT_SPECIFIED;
      }
      else
      {
        mm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
    ...
    for (j = 0; j < rat_pri_list_info_message->num_items; j++)
    {
          ...
          rat_pri_list_info->priority_list_info[i].acq_sys_mode) 
      {
        if(rat_pri_list_info_message->priority_list_info[j].acq_sys_mode == SYS_SYS_MODE_LTE ||
           rat_pri_list_info_message->priority_list_info[j].acq_sys_mode == SYS_SYS_MODE_LTE_M1 ||
           rat_pri_list_info_message->priority_list_info[j].acq_sys_mode == SYS_SYS_MODE_LTE_NB1
          )
        {
...
FUNCTION mm_checkif_lte_rat_added_to_rat_pri_list

DESCRIPTION
  This function cheks if any lte rat is present in list1 and not present in list2
...
{
    return (mm_checkif_rat_added_to_rat_pri_list(rat_pri_list_info_new,
                                                rat_pri_list_info_old,
                                                SYS_RAT_LTE_RADIO_ACCESS)
#ifdef FEATURE_CIOT
    || mm_checkif_rat_added_to_rat_pri_list(rat_pri_list_info_new,
                                                rat_pri_list_info_old,
...
FUNCTION mm_is_rat_gwt

DESCRIPTION
  This function verifies if the passed rat is of GWT type
...
{
    return ((rat == SYS_RAT_GSM_RADIO_ACCESS)
...
void mm_add_lai_to_foreground_search_lai_timer_list(byte *foreground_search_lai)
{
  ...
  if (!mm_is_lai_in_foreground_search_lai_timer_list(foreground_search_lai) )
  {
    ...
    */
    ...
    if(mm_is_per_ms_t3212_present)
    {
      mm_start_foreground_search_timer((dword)mm_stored_per_ms_t3212_value);
    }
    else
#endif
    {
...
void mm_print_lu_reject_cause_for_lai_in_foreground_search_lai_list(byte * serving_lai)
{
  ...
  if(found == TRUE)
  {
    for (j = 0; j < MAX_LU_ATTEMPT_SIZE; j++)
    {
     ...
     MSG_HIGH_DS_3(MM_SUB, "=MM= Serving LAI [%X %X] is in FOREGROUND SEARCH LAI TIMER LIST\nForeground search timer value remaining for serving LAI : %d",mm_foreground_search_lai_timer_list.lai_timer_list[i].lai[3],
                  mm_foreground_search_lai_timer_list.lai_timer_list[i].lai[4],timeout_value);
   }
}
...
void mm_inform_updated_foreground_search_list(void)
{
 ...
 if (mm_is_rat_lte(mm_serving_plmn.info.active_rat) == FALSE)
 {
   for (i = 0; (i < mm_foreground_search_lai_timer_list.length) && (i < MAX_FOREGROUND_SEARCH_LAI_TIMER_LIST_LENGTH); i++)
   {
...
void mm_convert_rat_to_tech_id(sys_radio_access_tech_e_type  rat_arg, sys_sys_mode_e_type *tech_id_arg)
{
    ...
    else if(rat_arg == SYS_RAT_LTE_NB1_RADIO_ACCESS)
    {
      *tech_id_arg = SYS_SYS_MODE_LTE_NB1;
    }
    else if(rat_arg == SYS_RAT_LTE_M1_RADIO_ACCESS)
    {
      *tech_id_arg = SYS_SYS_MODE_LTE_M1;
    }
...
#ifdef FEATURE_LTE
            && 
           (!mm_checkif_lte_rat_added_to_rat_pri_list(&message->cmd.mmr_reg_req.rat_pri_list_info,
                                                    &mm_rat_pri_list_info))
#endif  
            && 
             (mm_already_camped( message))))
...
#ifdef FEATURE_1XSRLTE
  if(emm_ue_is_in_srlte_state(emm_ctrl_data_ptr))
  {
...
)
{
  ...
  for (i = 0; i < rat_pri_list_info->num_items; i++)
  {
    if(rat_pri_list_info->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE ||
      rat_pri_list_info->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_M1 ||
      rat_pri_list_info->priority_list_info[i].acq_sys_mode == SYS_SYS_MODE_LTE_NB1)
    {
...
)
{
  ...
  if((ptw > 0x0F) || (cycle_length > 0x0F))
  {
    MSG_ERROR_DS_3(MM_SUB, "=MM= Invalid eDRX params for RAT %d: PTW %d, Cycle length %d",
                           rat, ptw, cycle_length);
    valid = FALSE;
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean mm_update_edrx_cfg_params
(
  mmr_set_edrx_cfg_req_s_type *msg_ptr
)
{
  boolean nw_update_needed = FALSE;
  boolean params_changed = FALSE;
  boolean edrx_enabled_old = FALSE;
  boolean edrx_enabled_new = FALSE;
  mcfg_fs_status_e_type efs_status = MCFG_FS_STATUS_EFS_ERR;

  /* Check which bits in the bitmask changed, update the parameters in NV
     accordingly */
  if((msg_ptr->change_mask != 0)
     &&
     ((msg_ptr->rat_type == SYS_RAT_LTE_RADIO_ACCESS) ||
#ifdef FEATURE_CIOT
#ifndef FEATURE_CATM1_DISABLE
      (msg_ptr->rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS) ||
#endif
      (msg_ptr->rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS) ||
#endif
      (msg_ptr->rat_type == SYS_RAT_GSM_RADIO_ACCESS) ||
      (msg_ptr->rat_type == SYS_RAT_UMTS_RADIO_ACCESS) ||
      (msg_ptr->rat_type == SYS_RAT_TDS_RADIO_ACCESS)))
  {
    if(msg_ptr->rat_type == SYS_RAT_LTE_RADIO_ACCESS)
    {
      edrx_enabled_old = mm_edrx_cfg.lte.enabled;
    }
#ifdef FEATURE_CIOT
    else if(msg_ptr->rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS)
    {
      edrx_enabled_old = mm_edrx_cfg.M1.enabled;
    }
    else if(msg_ptr->rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS)
    {
      edrx_enabled_old = mm_edrx_cfg.NB1.enabled;
    }
#endif
    else if(msg_ptr->rat_type == SYS_RAT_GSM_RADIO_ACCESS)
    {
      edrx_enabled_old = mm_edrx_cfg.gsm.enabled;
    }
    else if(msg_ptr->rat_type == SYS_RAT_UMTS_RADIO_ACCESS)
    {
      edrx_enabled_old = mm_edrx_cfg.umts.enabled;
    }

    if((msg_ptr->change_mask & SYS_EDRX_CONFIG_INFO_MASK_EDRX_ENABLED) != 0)
    {
      if((msg_ptr->rat_type == SYS_RAT_LTE_RADIO_ACCESS) &&
         (msg_ptr->edrx_enabled != mm_edrx_cfg.lte.enabled))
      {
        mm_edrx_cfg.lte.enabled = msg_ptr->edrx_enabled;
        edrx_enabled_new = mm_edrx_cfg.lte.enabled;
      }
#ifdef FEATURE_CIOT
      else if((msg_ptr->rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS) &&
         (msg_ptr->edrx_enabled != mm_edrx_cfg.M1.enabled))
      {
        mm_edrx_cfg.M1.enabled = msg_ptr->edrx_enabled;
        edrx_enabled_new = mm_edrx_cfg.M1.enabled;
      }
      else if((msg_ptr->rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
         (msg_ptr->edrx_enabled != mm_edrx_cfg.NB1.enabled))
      {
        mm_edrx_cfg.NB1.enabled = msg_ptr->edrx_enabled;
        edrx_enabled_new = mm_edrx_cfg.NB1.enabled;
      }
#endif
      else if((msg_ptr->rat_type == SYS_RAT_GSM_RADIO_ACCESS) &&
              (msg_ptr->edrx_enabled != mm_edrx_cfg.gsm.enabled))
      {
        mm_edrx_cfg.gsm.enabled = msg_ptr->edrx_enabled;
        edrx_enabled_new = mm_edrx_cfg.gsm.enabled;
      }
      else if ((msg_ptr->rat_type == SYS_RAT_UMTS_RADIO_ACCESS) &&
               (msg_ptr->edrx_enabled != mm_edrx_cfg.umts.enabled))
      {
        mm_edrx_cfg.umts.enabled = msg_ptr->edrx_enabled;
        edrx_enabled_new = mm_edrx_cfg.umts.enabled;
      }
    }

    /* Ignore eDRX params in other than these cases as eDRX mode in those cases is disabled */ 
    if((edrx_enabled_new == TRUE) /* eDRX mode changed from disable to enable */
       ||
       ((edrx_enabled_old == TRUE) && /* eDRX mode not changed (bitmask not set) and is currently enabled */
        (((msg_ptr->change_mask & SYS_EDRX_CONFIG_INFO_MASK_EDRX_ENABLED) == 0)
         || /* eDRX mode stays enabled (bitmask set but mode didn't change from enabled) */
         (edrx_enabled_old == msg_ptr->edrx_enabled))))
    {
      if((msg_ptr->change_mask & SYS_EDRX_CONFIG_INFO_MASK_PTW) != 0)
      {
        if ((msg_ptr->rat_type == SYS_RAT_LTE_RADIO_ACCESS) &&
            (msg_ptr->edrx_ptw != mm_edrx_cfg.lte.ptw))
        {
          mm_edrx_cfg.lte.ptw = msg_ptr->edrx_ptw;
          params_changed = TRUE;
        }
#ifdef FEATURE_CIOT
        else if ((msg_ptr->rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS) &&
            (msg_ptr->edrx_ptw != mm_edrx_cfg.M1.ptw))
        {
          mm_edrx_cfg.M1.ptw = msg_ptr->edrx_ptw;
          params_changed = TRUE;
        }
        else if ((msg_ptr->rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
            (msg_ptr->edrx_ptw != mm_edrx_cfg.NB1.ptw))
        {
          mm_edrx_cfg.NB1.ptw = msg_ptr->edrx_ptw;
          params_changed = TRUE;
        }
#endif
        /* PTW Field is not applicable in GSM */
        else if ((msg_ptr->rat_type == SYS_RAT_UMTS_RADIO_ACCESS) &&
                  (msg_ptr->edrx_ptw != mm_edrx_cfg.umts.ptw))
        {
          mm_edrx_cfg.umts.ptw = msg_ptr->edrx_ptw;
          params_changed = TRUE;
        }
      }

      if((msg_ptr->change_mask & SYS_EDRX_CONFIG_INFO_MASK_EDRX_CYCLE_LENGTH) != 0)
      {
        if((msg_ptr->rat_type == SYS_RAT_LTE_RADIO_ACCESS) &&
           (msg_ptr->edrx_cycle_length != mm_edrx_cfg.lte.cycle_length))
        {
          mm_edrx_cfg.lte.cycle_length = msg_ptr->edrx_cycle_length;
          params_changed = TRUE;
        }
#ifdef FEATURE_CIOT
        else if((msg_ptr->rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS) &&
           (msg_ptr->edrx_cycle_length != mm_edrx_cfg.M1.cycle_length))
        {
          mm_edrx_cfg.M1.cycle_length = msg_ptr->edrx_cycle_length;
          params_changed = TRUE;
        }
        else if((msg_ptr->rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
           (msg_ptr->edrx_cycle_length != mm_edrx_cfg.NB1.cycle_length))
        {
          mm_edrx_cfg.NB1.cycle_length = msg_ptr->edrx_cycle_length;
          params_changed = TRUE;
        }
#endif
        else if((msg_ptr->rat_type == SYS_RAT_GSM_RADIO_ACCESS) &&
                (msg_ptr->edrx_cycle_length != mm_edrx_cfg.gsm.cycle_length))
        {
          mm_edrx_cfg.gsm.cycle_length = msg_ptr->edrx_cycle_length;
          params_changed = TRUE;
        }
        else if((msg_ptr->rat_type == SYS_RAT_UMTS_RADIO_ACCESS) &&
                (msg_ptr->edrx_cycle_length != mm_edrx_cfg.umts.cycle_length))
        {
          mm_edrx_cfg.umts.cycle_length = msg_ptr->edrx_cycle_length;
          params_changed = TRUE;
        }
      }
    }
    ...
    if((edrx_enabled_new == TRUE) /* eDRX mode changed from disable to enable */
       ||
       ((edrx_enabled_old == TRUE) && /* eDRX mode changed from enable to disable */
        (edrx_enabled_old != msg_ptr->edrx_enabled) &&
        ((msg_ptr->change_mask & SYS_EDRX_CONFIG_INFO_MASK_EDRX_ENABLED) != 0))
       || /* eDRX params changed */
       (params_changed == TRUE))
    {
      ...
      edrx_cfg.lte.enabled = mm_edrx_cfg.lte.enabled;
      edrx_cfg.lte.ptw = mm_edrx_cfg.lte.ptw;
      edrx_cfg.lte.cycle_length = mm_edrx_cfg.lte.cycle_length;
      edrx_cfg.gsm.enabled = mm_edrx_cfg.gsm.enabled;
      edrx_cfg.gsm.ptw = mm_edrx_cfg.gsm.ptw;
      edrx_cfg.gsm.cycle_length = mm_edrx_cfg.gsm.cycle_length;
      edrx_cfg.umts.enabled = mm_edrx_cfg.umts.enabled;
      edrx_cfg.umts.ptw = mm_edrx_cfg.umts.ptw;
      edrx_cfg.umts.cycle_length = mm_edrx_cfg.umts.cycle_length;

#ifdef FEATURE_CIOT
      edrx_cfg.version = mm_edrx_cfg.version;
      edrx_cfg.M1.enabled = mm_edrx_cfg.M1.enabled;
      edrx_cfg.M1.ptw = mm_edrx_cfg.M1.ptw;
      edrx_cfg.M1.cycle_length = mm_edrx_cfg.M1.cycle_length;
      edrx_cfg.NB1.enabled = mm_edrx_cfg.NB1.enabled;
      edrx_cfg.NB1.ptw = mm_edrx_cfg.NB1.ptw;
      edrx_cfg.NB1.cycle_length = mm_edrx_cfg.NB1.cycle_length;
#endif

      efs_status = mcfg_fs_write("/nv/item_files/modem/nas/nas_edrx_config", 
                                 &edrx_cfg,
                                 sizeof(nas_edrx_config_s_type),
                                 MCFG_FS_O_RDWR|MCFG_FS_O_CREAT|MCFG_FS_O_TRUNC|MCFG_FS_O_AUTODIR,
                                 MCFG_FS_ALLPERMS, 
                                 MCFG_FS_TYPE_EFS,
                                 (mcfg_fs_sub_id_e_type)mm_sub_id);
    ...
    MSG_HIGH_DS_6(MM_SUB, "=MM= EDRX config params update: edrx_enabled_old = %d, edrx_enabled_new = %d, params_changed = %d, nw_update = %d, efs_status = %d, efs_errno = %d",
                           edrx_enabled_old, edrx_enabled_new, params_changed,
                           nw_update_needed, efs_status, efs_errno);
  }
  return nw_update_needed;
}
...
SIDE EFFECTS
  None
===========================================================================*/
void mm_invalidate_edrx_params (void)
{
  if (gmm_edrx_params_gsm_ctxt.edrx_params.enabled == TRUE)
  {
  ...
  {
    mm_send_mmr_edrx_params_change_ind(SYS_RAT_GSM_RADIO_ACCESS,
                                       gmm_edrx_params_gsm_ctxt.edrx_params.enabled,
                                       gmm_edrx_params_gsm_ctxt.edrx_params.ptw,
                                       gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length);
  }
}
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None
 ===========================================================================*/
 boolean mm_process_edrx_params
 (
 ...
 )
 {
   ...
   {
     /* With fake eDRX NV enabled, eDRX negotiation will always be successful in NAS irrespective of
            testbox return value and NAS will inform the eDRX value configured in below NV to lower layer 
            after every successful registration.*/
     edrx_params = gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.cycle_length;
     edrx_params_exists = TRUE;
   }
   
   MSG_HIGH_DS_3(MM_SUB, "=MM= EDRX params exists = %d New edrx_params = %d Old edrx_cycle = %d",edrx_params_exists,edrx_params,gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length);
   
   if (gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled == TRUE)
   {
     ...
     else
     {
       ...
       /* to_nw values different from from_nw for eDRX cycle length */
       if ((gmm_edrx_params_gsm_ctxt.edrx_params.enabled == FALSE) ||
           (gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length != NAS_INVALID_EDRX_VALUE &&
           (gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length != (edrx_params & 0x0F))) ||
           (gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.cycle_length != (edrx_params & 0x0F)))
       {
         edrx_params_changed = TRUE;
       }
 
       gmm_edrx_params_gsm_ctxt.edrx_params.enabled = TRUE;
       gmm_edrx_params_gsm_ctxt.edrx_params.ptw = 0; /* PTW not used in GSM mode */
       gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length = (byte)(edrx_params & 0x0F);
     }
   }
   else
   {
     if (gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length != NAS_INVALID_EDRX_VALUE)
     {
       /* The purpose of Attach/RAU request was to disable (currently enabled) eDRX */
       edrx_params_changed = TRUE;
     }
     gmm_edrx_params_gsm_ctxt.edrx_params.enabled = FALSE;
     gmm_edrx_params_gsm_ctxt.edrx_params.ptw = NAS_INVALID_EDRX_VALUE;
   ...
   return edrx_params_changed;
 } /* end mm_process_edrx_params() */
   ...
   SIDE EFFECTS
      None
===========================================================================*/
   boolean mm_check_attach_with_imsi( mm_cn_domain_type_T cn_domain )
   {
     boolean attach_with_imsi = FALSE;
       ...
       )
     {
      ...
      camped_plmn = mm_serving_plmn.info.plmn;
      ...
      if(cn_domain == MM_CS_DOMAIN &&
         has_valid_tmsi())
      {
        rplmn.identity[0] = mm_location_information.lai[0];
        rplmn.identity[1] = mm_location_information.lai[1];
        rplmn.identity[2] = mm_location_information.lai[2];
      }
      else
#endif
      if((cn_domain == MM_PS_DOMAIN) && 
               (gsm_spt || umts_spt || tds_spt) &&
#ifdef FEATURE_LTE
...
#ifdef FEATURE_LTE
       else if((cn_domain == MM_PS_DOMAIN) &&
       ...
       {
         attach_with_imsi = TRUE;
       }
     }
   
     MSG_HIGH_DS_2(MM_SUB,"=EMM= AttachWithIMSI = %d, config = %d", attach_with_imsi, config);
   
     return attach_with_imsi;
   } /* end mm_check_attach_with_imsi() */
#endif

/*===========================================================================

FUNCTION mm_get_nv_nas_rel_compliance_per_subs

DESCRIPTION 
  This function returns the value of global "mm_nv_nas_rel_compliance" 
...
===========================================================================*/
ue_nas_rel_compliance_type mm_get_nv_nas_rel_compliance_per_subs
(
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
  sys_modem_as_id_e_type sub_id
#else
...
)
{
  ...
  if(IS_NOT_VALID_SUB_ID(sub_id))
  {
    MSG_ERROR_DS(MM_SUB, "=MM= mm_get_nv_nas_rel_compliance_per_subs : Invalid AS ID: %d", sub_id+1,0,0);
    return NV_NAS_COMPLIANCE_REL10;
  }
  else
  {
  	return mm_nv_nas_rel_compliance_sim[sub_id];
  }
#else
  return mm_nv_nas_rel_compliance;
#endif
}
/*===========================================================================
Function: mm_populate_mm_msg_info

Description:
This function populates the value in mm_msg_info variable 
for mm messages for debugging purposes.

============================================================================*/

void mm_populate_mm_msg_info(mm_cmd_type* message)
{
        ...
        */
        ...
        case RR_SERVICE_IND_FROM_OTA_MSG:
           ...
           mm_msg_info : bit 0 :service_state
                         bits 2-1 : failure_cause
              */
          mm_msg_info = (byte)message->cmd.rr_service_ind_from_ota_msg.service_state;
          mm_msg_info = (byte)(mm_msg_info|(((message->cmd.rr_service_ind_from_ota_msg.failure_cause)&0x03)<<1));
        ...
        case RR_ABORT_IND:
          ...
          break;
        case RR_PAGING_IND:
          mm_msg_info = message->cmd.rr_paging_ind.paging_type;
          break;
        default:
          break;
      }
      ...
      switch (message->cmd.hdr.message_id)
      {
        ...
        case RRC_SERVICE_IND_FROM_OTA_MSG:
          ...
          mm_msg_info : bit 0 :service_status
                        ...
                        bits 4-2: no_service_cause
          */
          mm_msg_info = message->cmd.rrc_service_ind_from_ota_msg.service_status;
          mm_msg_info = (byte)(mm_msg_info|(((message->cmd.rrc_service_ind_from_ota_msg.acq_status)&0x01)<<1));
          mm_msg_info = (byte)(mm_msg_info|(((message->cmd.rrc_service_ind_from_ota_msg.no_service_cause)&0x07)<<2));
          break;
        case RRC_ABORT_IND:
        ...
        case RRC_OPEN_SESSION_CNF:
          /*
          mm_msg_info : bits 1-0:status
                        bit  2  :cn_domain_id
          */
          mm_msg_info = (byte)message->cmd.rrc_open_session_cnf.status;
          mm_msg_info = (byte)(mm_msg_info | (((message->cmd.rrc_open_session_cnf.cn_domain_id)&0x01)<<2));
          break;
        case RRC_CLOSE_SESSION_IND:
          /*
          mm_msg_info : bit 0:cause
                        bit 1:cn_domain_id
          */
          mm_msg_info = (byte)message->cmd.rrc_close_session_ind.cause;
          mm_msg_info = (byte)(mm_msg_info | (((message->cmd.rrc_close_session_ind.cn_domain_id)&0x01)<<1));
          break;      
        case RRC_ACTIVATION_IND:
          mm_msg_info = message->cmd.rrc_activation_ind.status;
          break;
        case RRC_PAGE_IND:
          mm_msg_info = message->cmd.rrc_page_ind.cn_domain_id;
          break;
        default:
           break;
      }
    ...
    case MS_MM_REG:
      switch (message->cmd.hdr.message_id)
      {
        case MMR_REG_REQ:
          ...
          /*
                  mm_msg_info : bit 4-5 :requested rat.
                  GSM: 00
                  UMTS: 01
                  LTE: 10
                  TDS: 11
          */       
          if(message->cmd.mmr_reg_req.rat == SYS_RAT_GSM_RADIO_ACCESS)
          {
            mm_msg_info = (byte)(mm_msg_info | 0x00);
          }
          else if(message->cmd.mmr_reg_req.rat == SYS_RAT_UMTS_RADIO_ACCESS)
          {
            mm_msg_info = (byte)(mm_msg_info | 0x10);
          }
          else if(message->cmd.mmr_reg_req.rat == SYS_RAT_LTE_RADIO_ACCESS)
          {
            mm_msg_info = (byte)(mm_msg_info | 0x20);
          }
          else if(message->cmd.mmr_reg_req.rat == SYS_RAT_TDS_RADIO_ACCESS)
          {
            mm_msg_info = (byte)(mm_msg_info | 0x30);
          }
          break;
    ...
    case MS_LLC_GMM :
      switch (message->cmd.hdr.message_id)
      {
        case GLLCMM_LL_UNITDATA_IND:
         if (message->cmd.ll_unitdata_ind.l3_pdu_len > 1) 
         {
...
#ifdef FEATURE_DUAL_SIM
boolean mm_per_subs_is_sim_need_to_be_ps_invalid_after_power_off(sys_modem_as_id_e_type sub_id)
{
  if(IS_NOT_VALID_SUB_ID(sub_id))
  {
     MSG_ERROR_DS(MM_SUB, "=MM= Invalid AS ID:%d", sub_id+1,0,0);
  ...
  return mm_is_sim_to_be_ps_invalid_after_power_off_sim[sub_id];
}
#endif
...
===========================================================================*/
boolean mm_get_t3247_behavior_enabled
(
   void
)
{
#ifdef FEATURE_DUAL_SIM
  if (reg_sim_check_if_t3245_behavior_enabled((sys_modem_as_id_e_type)mm_sub_id))
#else
  ...
  {
    return mm_emm_is_t3247_enabled;
  }
}

#ifdef FEATURE_DUAL_SIM
boolean mm_per_subs_get_t3247_behavior_enabled
  (
    sys_modem_as_id_e_type sub_id
  )
  {
	if (reg_sim_check_if_t3245_behavior_enabled((sys_modem_as_id_e_type)sub_id))
	{
	  return FALSE;
	}
	else
	{
	  return mm_emm_is_t3247_enabled_sim[sub_id];
	}
  }
#endif
...
===========================================================================*/
void mm_set_t3247_behavior_enabled
(
...
{
#ifdef FEATURE_DUAL_SIM
  mm_emm_is_t3247_enabled_sim[sub_id]  = enable_t3247;
#else
  mm_emm_is_t3247_enabled = enable_t3247;
#endif
}



boolean mm_is_sim_need_to_be_ps_invalid_after_power_off()
{
  MSG_HIGH_DS_1(MM_SUB,"SIM need to be PS invalid after power off - %d",mm_is_sim_to_be_ps_invalid_after_power_off);
  return mm_is_sim_to_be_ps_invalid_after_power_off;
}
#ifdef FEATURE_CIOT
/*===========================================================================
...
boolean mm_is_psm_enabled (void)
{
  ...
  {   
    emm_ctrl_data_ptr = emm_db_get_ctrl_data();    
    return (emm_ctrl_data_ptr->t3324_value != NAS_INVALID_PSM_TIMER_VALUE);
  }
  return FALSE;
}
#endif
...
boolean nas_mm_cmd_set_edrx_req(
...
)
{
   ...
   if(set_edrx_info_ptr == NULL)
   {
     MSG_ERROR_DS_0(MM_SUB,"=MM= Unexpected MMR_SET_EDRX_CFG_REQ cmd ");
     return FALSE;
   }
   ...
   {
      /* Construct message */
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.message_header.message_set = MS_MM_REG;
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.message_header.message_id  = MMR_SET_EDRX_CFG_REQ;

     PUT_IMH_LEN (sizeof(cmd_ptr->cmd.mmr_set_edrx_cfg_req) - sizeof(IMH_T),
                       &(cmd_ptr->cmd.mmr_set_edrx_cfg_req.message_header));

     cmd_ptr->cmd.mmr_set_edrx_cfg_req.rat_type               = set_edrx_info_ptr->rat_type;
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.edrx_enabled           = set_edrx_info_ptr->edrx_enabled;
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.edrx_ptw               = set_edrx_info_ptr->edrx_ptw;
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.edrx_cycle_length      = set_edrx_info_ptr->edrx_cycle_length;
     cmd_ptr->cmd.mmr_set_edrx_cfg_req.change_mask            = set_edrx_info_ptr->change_mask;
  
     MSG_HIGH_DS_5(MM_SUB,"MMR_SET_EDRX_CFG_REQ edrx_endabled =%d, rat=%d, ptw=%d, cycle_len = %d, change_mask = %d",
              set_edrx_info_ptr->edrx_enabled,set_edrx_info_ptr->rat_type ,(int)set_edrx_info_ptr->edrx_ptw,
             (int)set_edrx_info_ptr->edrx_cycle_length,(int)set_edrx_info_ptr->change_mask);
...
boolean nas_mm_cmd_set_drx_req(
...
)
{
   ...
   {
     MSG_ERROR_DS_0(MM_SUB,"=MM= Unexpected MMR_SET_DRX_REQ command ");
     return FALSE;
   }
   ...
   {
      /* Construct message */
     cmd_ptr->cmd.mmr_set_drx_req.message_header.message_set = MS_MM_REG;
     cmd_ptr->cmd.mmr_set_drx_req.message_header.message_id  = MMR_SET_DRX_REQ;

     PUT_IMH_LEN (sizeof(cmd_ptr->cmd.mmr_set_drx_req) - sizeof(IMH_T),
                       &(cmd_ptr->cmd.mmr_set_drx_req.message_header));

     cmd_ptr->cmd.mmr_set_drx_req.drx_coefficient          = set_drx_info_ptr->drx_coefficient;

     MSG_HIGH_DS_1(MM_SUB,"MMR_SET_DRX_REQ drx_coefficient =%d ", set_drx_info_ptr->drx_coefficient);
...
boolean nas_mm_cmd_get_drx_req (void)
{
   mm_cmd_type   *cmd_ptr;
   ...
   {
      /* Construct message */
     cmd_ptr->cmd.mmr_get_drx_req.message_header.message_set = MS_MM_REG;
     cmd_ptr->cmd.mmr_get_drx_req.message_header.message_id  = MMR_GET_DRX_REQ;

     PUT_IMH_LEN (sizeof(cmd_ptr->cmd.mmr_get_drx_req) - sizeof(IMH_T),
                       &(cmd_ptr->cmd.mmr_get_drx_req.message_header));
...
boolean nas_mm_cmd_get_edrx_req(
...
)
{
   ...
   if(get_edrx_info_ptr == NULL)
   {
     MSG_ERROR_DS_0(MM_SUB,"=MM= Unexpected MMR_GET_EDRX_PARAMS_REQ command ");
     return FALSE;
   }
   ...
   {
      /* Construct message */
     cmd_ptr->cmd.mmr_get_edrx_params_req.message_header.message_set = MS_MM_REG;
     cmd_ptr->cmd.mmr_get_edrx_params_req.message_header.message_id  = MMR_GET_EDRX_PARAMS_REQ;

     PUT_IMH_LEN (sizeof(cmd_ptr->cmd.mmr_get_edrx_params_req) - sizeof(IMH_T),
                       &(cmd_ptr->cmd.mmr_get_edrx_params_req.message_header));

     cmd_ptr->cmd.mmr_get_edrx_params_req.rat_type          = get_edrx_info_ptr->rat_type;

     MSG_HIGH_DS_1(MM_SUB,"MMR_GET_EDRX_PARAMS_REQ rat_type =%d ", get_edrx_info_ptr->rat_type);
...
boolean nas_reg_cmd_set_hplmn_search_timer (
...
)
 {
   ...
   if (set_hplmn_search_info_ptr == NULL)
   {
     MSG_ERROR_DS_0(MM_SUB,"=MM= Unexpected REG_SET_HPLMN_SEARCH_TIMER_REQ command ");
     return FALSE;
   }
   ...
   {
      /* Construct message */
     cmd_ptr->cmd.set_hplmn_search_timer_req.message_header.message_set = MS_REG_REG;
     cmd_ptr->cmd.set_hplmn_search_timer_req.message_header.message_id  = REG_SET_HPLMN_SEARCH_TIMER_REQ;

     PUT_IMH_LEN (sizeof(cmd_ptr->cmd.set_hplmn_search_timer_req) - sizeof(IMH_T),
                       &(cmd_ptr->cmd.set_hplmn_search_timer_req.message_header));

     cmd_ptr->cmd.set_hplmn_search_timer_req.timer_value          = set_hplmn_search_info_ptr->timer_value;

     MSG_HIGH_DS_1(MM_SUB,"MMR_SET_DRX_REQ drx_coefficient =%d ", set_hplmn_search_info_ptr->timer_value);
...
uint32 nas_cmd_get_hplmn_search_timer(void)
{
...
uint32 nas_cmd_get_ciot_irat_search_timer(void)
{

  ciot_params_s_type ciot_param = reg_nv_get_ciot_params();
  uint32 timer_value = ciot_param.irat_timer;
  
  MSG_HIGH_DS_1(REG_SUB, "=REG= CIOT IRAT timer value = %d", timer_value);
  return timer_value;
}
#endif
/*==============================================================================
...
==============================================================================*/
boolean mm_partial_search_allowed_get( void )
{
  return mm_partial_search_allowed;
}
/*==============================================================================
FUNCTION NAME:

mm_partial_search_allowed_set
==============================================================================*/
void mm_partial_search_allowed_set
(
  boolean          is_allowed
)
{
  mm_partial_search_allowed = is_allowed;
}
/*==============================================================================
FUNCTION NAME:
...
==============================================================================*/
boolean mm_is_gsm_active_for_thermal_read()
{
 MSG_HIGH_DS_1(MM_SUB,"mm_is_gsm_active_rat_for_thermal_read is - %d",mm_is_gsm_active_rat_for_thermal_read);
 return mm_is_gsm_active_rat_for_thermal_read;
}
/*===========================================================================
FUNCTION mm_update_scanning_rat

DESCRIPTION

Function to check scanning rat
...
===========================================================================*/
void mm_update_scanning_rat(sys_scan_params_s_type* scan_params)
{
   ...
   {
     if(emm_ctrl_data_ptr->loaded_lte_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS)
     {
       scan_params->scanning_rat = SYS_SYS_MODE_LTE_NB1;
     }
     else if(emm_ctrl_data_ptr->loaded_lte_rat == SYS_RAT_LTE_M1_RADIO_ACCESS)
     {
       scan_params->scanning_rat = SYS_SYS_MODE_LTE_M1;
     }
     else
     {
       scan_params->scanning_rat = SYS_SYS_MODE_LTE;
     }
   }
#endif				   
#ifdef FEATURE_NAS_GW		   
   if(mm_waiting_for_service_cnf)
   {
     scan_params->scanning_rat = SYS_SYS_MODE_GSM;
   }
#endif
...
/*===========================================================================
FUNCTION mm_update_camped_rat

DESCRIPTION

Function to check current camped rat
...
===========================================================================*/
void mm_update_camped_rat(sys_scan_params_s_type* scan_params)
{
#ifdef FEATURE_LTE
   emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
   emm_ctrl_data_ptr = emm_db_get_ctrl_data();
   ...
   {
     scan_params->current_camped_rat = SYS_SYS_MODE_LTE_NB1;
   }
   else if(emm_ctrl_data_ptr->plmn_service_state->active_rat  == SYS_RAT_LTE_M1_RADIO_ACCESS)
   {
     scan_params->current_camped_rat = SYS_SYS_MODE_LTE_M1;
   }
   else if(emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_RADIO_ACCESS)
   {
     scan_params->current_camped_rat = SYS_SYS_MODE_LTE;
   }
#endif				  
#ifdef FEATURE_NAS_GW			  
   if(mm_serving_plmn.info.active_rat == SYS_RAT_GSM_RADIO_ACCESS)
   {
     scan_params->current_camped_rat = SYS_SYS_MODE_GSM;
   }
#endif
...
/*===========================================================================
FUNCTION mm_udpate_roaming_state
...
===========================================================================*/
void mm_udpate_roaming_state(sys_scan_params_s_type *scan_params)
{
   ...
   if ( sys_plmn_match(hplmn, mm_serving_plmn.info.plmn) )
 #else
   if ( reg_sim_is_ehplmn(mm_serving_plmn.info.plmn) ) 
 #endif
   {
     scan_params->roam_state = SYS_ROAM_STATUS_OFF;
   }
   else if ((reg_nv_is_roaming_on_in_eplmn == FALSE)&&(sys_eplmn_list_equivalent_plmn(mm_serving_plmn.info.plmn)&&
   ...
   {
     scan_params->roam_state = SYS_ROAM_STATUS_ON;
   }
...
===========================================================================*/
void nas_command_get_scan_params(sys_scan_params_s_type *scan_params)
{
   if(scan_params == NULL)
   {
     MSG_FATAL_DS(MM_SUB, "=MM= Invalid Scan Params",0,0,0);
   }
    scan_params->roam_state=SYS_ROAM_STATUS_NONE;
	scan_params->current_camped_rat=SYS_SYS_MODE_NONE;
	scan_params->scanning_rat = SYS_SYS_MODE_NONE;
       ...
       (emm_ctrl_data_ptr->reg_req_pended_indication  == TRUE )
		...
		|| (mm_waiting_for_service_cnf)
     	 ...
     	 mm_waiting_for_service_cnf
	   	...
	   	{
         mm_update_scanning_rat(scan_params);
        }
	}
    else
	{
     mm_udpate_roaming_state(scan_params);
     if( 
#ifdef FEATURE_LTE
		(emm_ctrl_data_ptr->reg_req_pended_indication  == TRUE )
#ifdef FEATURE_NAS_GW				
        ...
        {
		 mm_update_camped_rat(scan_params);		  
		}
	}
	MSG_HIGH_DS_3(MM_SUB,"roam_state %d, current_camped_rat %d ,scanning rat %d",scan_params->roam_state,scan_params->current_camped_rat,scan_params->scanning_rat);
}
...
======================================================================*/
boolean mm_eplmn_list_equivalent_plmn(sys_plmn_id_s_type plmn)
{
#if defined FEATURE_DUAL_SIM
	  return sys_eplmn_per_nas_stacks_list_equivalent_plmn(plmn,mm_sub_id,mm_as_id);
#else
	  return sys_eplmn_list_equivalent_plmn(plmn);
#endif
}

boolean mm_check_gcf_flag_enabled()
{
