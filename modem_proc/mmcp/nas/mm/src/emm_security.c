...
extern boolean  read_security_context_only;
...
void send_sec_mode_cmd_log_pkt(lte_nas_emm_current_sec_context_type *emm_current_sec_context_log, emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  mm_check_for_null_ptr((void *)context_ptr);
...
log_native_sec_ctxt.eps_nas_sec_context_len = ((log_native_sec_ctxt.eps_nas_sec_context_len>>8)|(log_native_sec_ctxt.eps_nas_sec_context_len<<8));
  ...
  sim_buffer += offset;
   ...
   if( usim_security_context_ptr->nas_ul_cnt[0] == 0 )
   {
    ...
    {
      usim_security_context_ptr->nas_ul_cnt[0] = usim_security_context_ptr->nas_ul_cnt[0] - 1;
    }
 ...
 /* NAS integrity and ciphering algorithms */
  ...
  if( len != 1)
  {
    MSG_ERROR_DS_1(MM_SUB, "=EMM= length of NAS Int and Enc algorithms = %d - invalid contents in the SIM",
                    len);
    return FALSE;
  }
...
void emm_invalidate_gmm_parameters
(
  boolean invalidate_sim
)
{
...
)
{
  ...
  memscpy((void*)&nas_sec_param_to_eutra[0],
   ...
   mm_check_for_null_ptr((void *) context_info);
  ...
  if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
  {
    MSG_HIGH_DS_7(MM_SUB, "=EMM= Received LTE_RRC_NAS_LTE_KEY_IND:\
                  nonce_mme[0] = 0x%02x, nonce_mme[1] = 0x%02x, nonce_mme[2] = 0x%02x, nonce_mme[3] = 0x%02x,\
                  KSI = %d, INT ALG = %d, ENC ALG = %d",
...
)
{
  ...
  mm_check_for_null_ptr((void *)mapped_context_ptr);
    ...
    (current_context_ptr->context_type == CONTEXT_TYPE_FULL_NATIVE_EPS))
 {
  ...
  nas_assert_check((boolean)( emm_compute_nas_int_key(emm_ctrl_data_ptr, mapped_context_ptr) == TRUE ));
  MSG_HIGH_DS_0(MM_SUB, "=EMM= Compute NAS_CIPHERING KEY");
  nas_assert_check((boolean)( emm_compute_nas_enc_key(emm_ctrl_data_ptr, mapped_context_ptr) == TRUE ));
...
)
{
  ...
  boolean plain_msg = (emm_ctrl_data_ptr->nas_incoming_msg_ptr->emm_incoming_msg.auth_rej.security_hdr == PLAIN_NAS_MSG)? 
                       TRUE: FALSE;

  boolean dos_mitigation_reqd = emm_check_if_sec_attack_mitigation_possible(LTE_NAS_CAUSE_NONE, 
                                                           plain_msg,
                                                           EMM_AUTHENTICATION_REJECT,
                                                           &dos_specific_ftai);
  ...
  */
  ...
  /* The following must be done when an authentication reject is received
     ...
     6. Invalidate USIM (until power down/ USIM removed) - USIM is marked as illegal if
        a. T3247 feature is not supported (OR)
        b. If T3247 feature is supported and the counter for USIM reaches its max value
  ...
  */
  ...
  if (FALSE == dos_mitigation_reqd)
  {
     ...
     {
       emm_invalidate_gmm_parameters(TRUE);
     }
#endif
     if (mm_emm_max_attempt_counter_dos_mitigation > 0)
     {
        mm_emm_usim_gprs_counter =mm_emm_max_attempt_counter_dos_mitigation;
        MSG_HIGH_DS_1(MM_SUB, "=EMM= USIM GPRS counter = %d", mm_emm_usim_gprs_counter);
  ...
  else
  {
     ...
     MSG_HIGH_DS_2(MM_SUB, "=EMM= USIM GPRS counter = %d, non-GPRS counter = %d", mm_emm_usim_gprs_counter, mm_emm_usim_non_gprs_counter);
     ...
     if(mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, 
        ...
        mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, 
                                     SYS_RAT_TDS_RADIO_ACCESS) )
     {
       emm_invalidate_gmm_parameters(FALSE);
     }
#endif
     emm_invalidate_mobility_info(EMM_STATUS_ROAMING_NOT_ALLOWED,
     ...
     MSG_HIGH_DS_0(MM_SUB, "=EMM= Add TAI to FTAI list for roaming - Auth Rej");
  ...
  {
    reg_service_report(MMR_CAUSE_FAILURE_LAI_NOT_ALLOWED, FALSE, emm_ctrl_data_ptr);
  }
...
void emm_load_security_context(emm_ctrl_data_type *emm_ctrl_data_ptr)
{
  ...
  MSG_HIGH_DS_1(MM_SUB, "=EMM= =EMM= NV_PREF_MODE_I = %d",emm_ctrl_data_ptr->emm_nv_pref_mode_read_ptr->pref_mode.mode);
   ...
   /* Read the USIM EPS NSC file */
     ...
     */
     if((emm_compute_nas_int_key(emm_ctrl_data_ptr, context_ptr) == FALSE )||
        (emm_compute_nas_enc_key(emm_ctrl_data_ptr,context_ptr) == FALSE))
     {
       MSG_FATAL_DS(MM_SUB,"=EMM= INT OR CIP key compute fail",0,0,0);
     }
  }
   else
   {
...
boolean emm_write_efs_security_context(emm_security_context_info* security_context_ptr)
{
  ...
  */
  ...
  mm_check_for_null_ptr((void *)security_context_ptr);
...
boolean emm_update_usim_eps_security_context( emm_security_context_info  *context_info_ptr)
{
 ...
 /* KSI  -tag ,length, value =  1 byte*/  
  ...
  nas_assert_check((boolean)(( MAX_SIM_BUFFER - pos ) > (word)sizeof( context_info_ptr->nas_kasme)));
  ...
  nas_assert_check( (boolean)(pos < (MAX_SIM_BUFFER - 1)));
  ...
  nas_assert_check((boolean)( ( MAX_SIM_BUFFER - pos ) > (word)sizeof( context_info_ptr->nas_ul_cnt) ));
  ...
  */
   ...
   {
	 context_info_ptr->nas_ul_cnt[0] = context_info_ptr->nas_ul_cnt[0] + 1;  
   }
  ...
  if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
  {
    MSG_HIGH_DS_8(MM_SUB, "=EMM= UL COUNT to be written to USIM UL_NAS_COUNT [0]= %d UL_NAS_COUNT [1]= %d \
                                 ...
                                 DL_NAS_COUNT [2]= %d DL_NAS_COUNT [3]= %d",
                                  swapped_nas_count[0],swapped_nas_count[1],
  ...
  nas_assert_check ((boolean)( pos  < ( MAX_SIM_BUFFER - ( 1 + offset ))));
                                   ...
                                   emm_ctrl_data_type                *emm_ctrl_data_ptr)
{
  ...
  {
    memscpy((void*)&prev_dl_cnt_smc[0],
             sizeof(prev_dl_cnt_smc),
             (void*)&current_ctxt->nas_dl_cnt[0],
  ...
  */
             ...
             ( emm_security_mode_cmd->nas_key_set_identifier_sgsn.keyset_id != NO_KEY_AVAIALABLE) )
  {
    ...
    {
      if (overwritten_current_context == TRUE)
      {
  ...
  if (overwritten_current_context == TRUE)
  {
    emm_ctrl_data_ptr->curr_smc_status = LTE_SMC_STATUS_PROCESSED_CHANGE_ALGOS_OR_UL_COUNT;
  }
  else
  {
    emm_ctrl_data_ptr->curr_smc_status = LTE_SMC_STATUS_PROCESSED;
  }
  ...
  if((emm_compute_nas_int_key(emm_ctrl_data_ptr, context_info) == FALSE ) ||
     (emm_compute_nas_enc_key(emm_ctrl_data_ptr,context_info) == FALSE ))
  ...
  if( !emm_perform_smc_integriy_check( emm_ctrl_data_ptr, context_info ) )
  {
    ...
    mm_check_for_null_ptr((void *)tmp_ctxt);
    ...
    else
    {
      ...
      else
      {
        ...
        if(context_info!=NULL)
        {
          ...
          memscpy((void*)&context_info->nas_dl_cnt[0],
                  sizeof(context_info->nas_dl_cnt),
                  (void*)&prev_dl_cnt_smc[0],
                  sizeof(prev_dl_cnt_smc));
          emm_ctrl_data_ptr->last_incoming_sqn = context_info->nas_dl_cnt[0];
          MSG_HIGH_DS_0(MM_SUB, "=EMM= now computing NAS_INTEGRITY KEY");
          if( emm_compute_nas_int_key(emm_ctrl_data_ptr, context_info) == FALSE )
          {
...
#ifdef FEATURE_LTE_REL14
  if((emm_security_mode_cmd->hash_mme_exists == TRUE) &&
     ((EMM_GET_STATE() == EMM_REGISTERED_INITIATED) || 
      (EMM_GET_STATE() == EMM_TRACKING_AREA_UPDATING_INITIATED)))
  {
    replay_nas_msg = emm_replay_nas_message(emm_security_mode_cmd->hash_mme, emm_ctrl_data_ptr);
  }
#endif


  emm_send_security_mode_complete(imeisv_req,
#ifdef FEATURE_LTE_REL14
                                  replay_nas_msg,
#endif
                                  emm_ctrl_data_ptr);
...
#ifdef FEATURE_LTE_REL14
    smc_complete->replayed_nas_msg_exists = replay_nas_msg;
#endif
...
,  emm_security_context_info *context_info
)
 {
                                 ...
                                 &curr_security_info, FALSE,TRUE ) )
   {
    ...
    {
        ULOG_RT_PRINTF_0(mm_get_ulog_handle(), "=EMM= Integrity check fail");
	}
   }
  ...
  if( context_ptr != NULL )  
  {
...
#endif
    {
      ...
      */
      ...
      if(read_security_context_only == FALSE)
      {
        ...
        if((security_hdr_type == INTEGRITY_PROTECT_SECURITY_MODE_CMD)
           && (emm_ctrl_data_ptr->curr_smc_status ==  LTE_SMC_STATUS_NONE))
        {
          MSG_HIGH_DS_0(MM_SUB, "=EMM= OTA message is SMC, process later");
          return nas_security;
        }
      }
        /* increment the NAS SQN, if the NAS count was not reset due to an SMC.
      ...
      if((emm_ctrl_data_ptr->last_incoming_sqn >= sqn )
          && ((!smc)
          || (emm_ctrl_data_ptr->curr_smc_status 
                         == LTE_SMC_STATUS_PROCESSED_CHANGE_ALGOS_OR_UL_COUNT)))
      {
...
#endif
  {
    if(emm_guti_realloc_cmd->dcn_id_valid)
    {
      emm_process_dcn_list(emm_guti_realloc_cmd->dcn_id,emm_ctrl_data_ptr);
    }
  }
#endif
  /* Update EMM USIM */
    ...
    if ( temp_context_ptr != NULL )
  {
    ...
    { /* FULL native exists , proceed to writing to USIM */
     ...
     {   
       ...
       {
         nas_assert_check((boolean)(emm_write_efs_security_context(&curr_security_context) == 1 ));
       }
                          ...
                          emm_ctrl_data_type   *emm_ctrl_data_ptr)
{
  ...
  info->ciot_lte_mode    = SYS_SYS_MODE_NONE;
       ...
       info->ciot_lte_mode    = mm_convert_sys_rat_to_acq_mode(emm_ctrl_data_ptr->plmn_service_state->active_rat);
#else
...
)
{
 ...
 mm_check_for_null_ptr((void *)ctxt_ptr);
                                      ...
                                      auth_gprs_cipherkey_type    *Ck_srvcc)
{
  ...
  */
  ...
  if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
  {
    ...
    MSG_HIGH_DS_0(MM_SUB, "=EMM= SRVCC CK - including index [0-15]");
    for(i = 0; i+7 < MAX_AUTH_DATA_LENGTH; i = i+8) 
    {
      MSG_HIGH_DS_8(MM_SUB,"=EMM= 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
...
void emm_get_eps_params( mm_gw_identity_param_type *mm_identity)
{
  ...
  nas_assert_check( emm_compute_nas_token( emm_ctrl_data_ptr,
  ...
  nas_assert_check( emm_compute_ck_ik_from_kasme( emm_ctrl_data_ptr, context_ptr, &mm_identity->Ik, &mm_identity->Ck, FALSE ));
                ...
                auth_autn_type autn_value)
{
    ...
    memset((void *)&rand_value_cache.value_data[0], 0, sizeof(rand_value_cache.value_data));
    memscpy((void *)&rand_value_cache.value_data[0], sizeof(rand_value_cache.value_data),
        (void *)&rand_value.value_data[0], rand_value.value_length);

    autn_cache.value_length = autn_value.value_length;
    memset((void *)&autn_cache.value_data[0], 0, sizeof(autn_cache.value_data));
    memscpy((void *)&autn_cache.value_data[0], sizeof(autn_cache.value_data),  
        (void *)&autn_value.value_data[0], autn_value.value_length);
}
#ifdef FEATURE_LTE_REL14
/*===========================================================================
FUNCTION EMM_COMPUTE_HASH

DESCRIPTION
  This function generates a 64bit HASH of the entire plain Attach/TAU request
  message as described in Annex I TS 33.401
...
)
{
  ...
  **/
  ...
  byte index = 0;
  byte input_key[32] = {0};
  byte output_str[32] = {0};

  if((emm_ctrl_data_ptr->replayed_msg.msg_ptr != NULL) &&
     (emm_ctrl_data_ptr->replayed_msg.msg_len > 0))
  {

    nas_errno = lte_security_generate_key(input_key, /* Input key*/
                                          (uint8 *)emm_ctrl_data_ptr->replayed_msg.msg_ptr, /* Input string ptr */
                                          &emm_ctrl_data_ptr->replayed_msg.msg_len,         /* Input string length*/
                                          (uint8 *)output_str); /* Output string */

    if(nas_errno != E_SUCCESS)
    {
      MSG_ERROR_DS_0(MM_SUB, "=EMM= Error generating HASH-UE");
      return;
    }

    /* compute HASH-UE out of the 64 least significant bits of the 256 bits KDF output */
    memscpy((void*)hash_ue, hash_ue_len,
            (void*)&output_str[24], 8);
  }
}

/*===========================================================================
FUNCTION EMM_REPLAY_NAS_MESSAGE

DESCRIPTION
  This function compares 64bit HASH (HASH-UE) of the entire plain Attach/TAU request
  message with the HASH-MME provided by the network

DEPENDENCIES
  None

RETURN VALUE
  TRUE if HASH-MME doesn't match with HASH-UE, else FALSE

SIDE EFFECTS
  None
===========================================================================*/
boolean emm_replay_nas_message
(
  uint64              hash_mme,
  emm_ctrl_data_type  *emm_ctrl_data_ptr
)
{
  ...
  emm_compute_hash(&hash_ue[0], sizeof(hash_ue), emm_ctrl_data_ptr);

  result = (memcmp((void*)&hash_mme,
                   (void*)&hash_ue[0],
                   sizeof(hash_ue)) != 0) ? TRUE : FALSE;
  
  MSG_HIGH_DS_8(MM_SUB, "=EMM= hash_mme = 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x",
                (hash_mme >>  56) & 0xff, (hash_mme >>  48) & 0xff, (hash_mme >>  40) & 0xff, (hash_mme >>  32) & 0xff,
                (hash_mme >>  24) & 0xff, (hash_mme >>  16) & 0xff, (hash_mme >>  8) & 0xff, hash_mme & 0xff);

  MSG_HIGH_DS_8(MM_SUB, "=EMM= hash_ue = 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", 
                hash_ue[0], hash_ue[1], hash_ue[2], hash_ue[3],
                hash_ue[4], hash_ue[5], hash_ue[6], hash_ue[7]);

  MSG_HIGH_DS_1(MM_SUB, "=EMM= emm_replay_nas_message() returns %d", result);
  return result;
}

/*===========================================================================
FUNCTION EMM_RESET_REPLAY_MESSAGE

DESCRIPTION
  This function frees the memory allocated for Replay message
...
)
{
  if(emm_ctrl_data_ptr->replayed_msg.msg_ptr != NULL)
  {
    emm_modem_mem_free((void*)emm_ctrl_data_ptr->replayed_msg.msg_ptr, emm_ctrl_data_ptr);
