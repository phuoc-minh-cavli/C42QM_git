...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmstate.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
state_event_T table[] =
{
...
#if defined (FEATURE_SGLTE) || defined (FEATURE_WWAN_GNSS_DYNAMIC)
   ...
   /* Spec 24.008, Spec CR 2616
    */
   {  5,  25, MM_STD_RR_CONNECTION_REL_NOT_ALLOWED,   ACTION_29 },
#endif
   ...
   { 17,  9,  MM_STD_T3230_TIMEOUT,                   ACTION_18 },
#if defined ( FEATURE_MMGPS ) || defined ( FEATURE_CGPS_UMTS_CP )
 ...
 */
   ...
   if (entry_found)
   {
           ...
           (mm_csfb_state == MM_CSFB_STATE_MT_CALL_ORIG))
        {
          if((current_state == MM_IDLE) && 
             ((next_state == MM_WAIT_FOR_RR_CONNECTION_MM) ||
              (next_state == MM_WAIT_FOR_NETWORK_COMMAND)))
          {
            mm_send_rr_csfb_blind_redir_to_lte_req();
          }
     ...
     /* MM releases control of resel decision when MM state becomes IDLE */
        ...
        else if (mm_need_to_send_no_srv_on_call_end_due_to_dos == TRUE)
        {
          if(mm_plmn_search_in_progress )
		   ...
		   MSG_HIGH_DS_0(MM_SUB,"Snding no service after Call end");
