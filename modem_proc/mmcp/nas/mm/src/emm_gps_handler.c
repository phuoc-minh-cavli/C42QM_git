...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_gps_handler.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $ 
...
)
{
  ...
  uint8 num_attach, i;
  dsm_item_type            *dsm_arr_ptr[EMM_GENERIC_NAS_TRANSPORT_MAX_DSM_ARRAY_SIZE] = {NULL, NULL};
  ...
  for(i=0; i< num_attach && i<EMM_GENERIC_NAS_TRANSPORT_MAX_DSM_ARRAY_SIZE ;i++)
  {
    ...
    mm_check_for_null_ptr((void *)&dsm_arr_ptr[i]);
  }
...
{  
  ...
  if(((uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length <= DSM_DS_SMALL_ITEM_SIZ) && 
     ((uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length <= NAS_REM_SMALL_ITEM_POOL_SIZE))
  {
    MSG_HIGH_DS_1(MM_SUB, "=EMM= dsm push len %d small item pool", (uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length);
    /* Pushdown DSM */
    dsm_packet_length = dsm_pushdown(&dsm_ptr, 
                                     dl_gen_nas_msg_ptr->generic_msg_container.container_value_ptr,
                                     (uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length, 
                                     DSM_DS_SMALL_ITEM_POOL);

    if((uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length != dsm_packet_length)
    {
      ...
      len = dsm_pushdown(&dsm_ptr,dl_gen_nas_msg_ptr->generic_msg_container.container_value_ptr,(uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length, DSM_DS_LARGE_ITEM_POOL);
      if(len != (uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length) 
      {
  ...
  else
  {
    ...
    if((uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length != dsm_packet_length)
    {
      ...
      /* If Small item pool is running out always try Large item pool */
      len = dsm_pushdown(&dsm_ptr,dl_gen_nas_msg_ptr->generic_msg_container.container_value_ptr,(uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length, DSM_DS_SMALL_ITEM_POOL);
      if(len != (uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length) 
      {
        MSG_FATAL_DS(MM_SUB,"=EMM= Failed to push len %d small item pool",(uint16)dl_gen_nas_msg_ptr->generic_msg_container.container_length,0,0);
      }
    }
  }
  ...
  if(dl_gen_nas_msg_ptr->additional_info_exists)
  {
    /*push down into appropriate pool*/                          
    /*if we do not have enough small items use large item pool*/
    if(((uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length <= DSM_DS_SMALL_ITEM_SIZ) && 
     ((uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length <= NAS_REM_SMALL_ITEM_POOL_SIZE))
    {
      MSG_HIGH_DS_1(MM_SUB, "=EMM= dsm push len %d small item pool", (uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length);
      /* Pushdown DSM */
      dsm_packet_length = dsm_pushdown(&dsm_opt_iei_ptr, 
                                     dl_gen_nas_msg_ptr->additional_info.additional_info_value_ptr,
                                     (uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length, 
                                     DSM_DS_SMALL_ITEM_POOL);

      if((uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length != dsm_packet_length)
      {
        /*free any partial memory allocated */
        dsm_free_packet(&dsm_opt_iei_ptr);

        MSG_HIGH_DS_1(MM_SUB, "=EMM= dsm push len %d large item pool", dsm_packet_length);
        /* If Small item pool is running out always try Large item pool */
        len = dsm_pushdown(&dsm_opt_iei_ptr,dl_gen_nas_msg_ptr->additional_info.additional_info_value_ptr,(uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length, DSM_DS_LARGE_ITEM_POOL);
        if(len != (uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length) 
        {
        MSG_FATAL_DS(MM_SUB,"=EMM= Failed to push len %d large item pool",dsm_packet_length,0,0);
        }
      }
    ...
    else
    {
      ...
      if((uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length != dsm_packet_length)
      {
        ...
        /* If Small item pool is running out always try Large item pool */
        len = dsm_pushdown(&dsm_opt_iei_ptr,dl_gen_nas_msg_ptr->additional_info.additional_info_value_ptr,(uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length, DSM_DS_SMALL_ITEM_POOL);
        if(len != (uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length) 
        {
          MSG_FATAL_DS(MM_SUB,"=EMM= Failed to push len %d small item pool",(uint16)dl_gen_nas_msg_ptr->additional_info.additional_info_length,0,0);
        }
      }
    }
    ...
    mm_check_for_null_ptr((void *)att_iei_ptr);
    msgr_set_dsm_attach(att_iei_ptr, dsm_opt_iei_ptr);
    mm_check_for_null_ptr((void *)dsm_opt_iei_ptr); 
  }

  emm_msgr_send_error((msgr_hdr_s *)&dl_generic_cmd, 
               sizeof(lte_nas_emm_dl_generic_transport_s_type));

  MSG_HIGH_DS_0(MM_SUB, "=EMM= NAS_EMM_DL_GENERIC_NAS_TRANSPORT_IND ");
