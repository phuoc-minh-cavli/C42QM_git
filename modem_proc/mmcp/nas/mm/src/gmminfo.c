...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmminfo.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void gmm_handle_information (byte *L3_data ,IMH_T message_header, boolean cs_domain )
{
  ...
  {
    MSG_ERROR_DS_1(MM_SUB, "=MM= Discard msg (%d) Integ check fail", L3_data);
  }
  else
  {
    byte message_id   = L3_data[1];
    GET_IMH_LEN(message_len,&message_header);

    if ( (cs_domain == FALSE && 
           (message_header.message_set != MS_MM_RRC && message_header.message_set != MS_MM_TDSRRC)) ||
          (cs_domain == TRUE && 
           (message_header.message_set != MS_MM_RR && message_header.message_set != MS_MM_TDSRRC)) ||
         (message_id != GMM_INFORMATION  &&  message_id != MM_INFORMATION) ||
         message_len < GMM_INFORMATION_MSG_MIN_LENGTH )
    {
      if ((message_header.message_set != MS_MM_RRC) &&
          (message_header.message_set != MS_MM_TDSRRC))
      {
        MSG_ERROR_DS_1( MM_SUB, "=MM= Unexpected Message Set (%d)", message_header.message_set);
      }
      else if ( message_id != GMM_INFORMATION  &&  message_id != MM_INFORMATION )
      {
        ...
        while ( (index < message_len) && (valid_msg))
        {
          switch (L3_data[index] )
          {
              ...
              ** -------------------------------------------------------------------- */
              ...
              {
                 ie_length = MIN(L3_data[index+1] , MM_MAX_NETWORK_NAME_LEN);
              }
              else
              {
              ...
              {
                info->full_name.spare_bits = (L3_data[index+2] & 0x07);

                info->full_name.add_country_initials = ((L3_data[index+2] & 0x08) == 0x08);
              }
              else
              {
                MSG_ERROR_DS_0(MM_SUB,"Index out of bounds and buffer overflow");
              ...
              if ( valid_msg )
              {
                ...
                {
                   memscpy(info->full_name.name, SYS_MAX_FULL_NAME_CHARS, &(L3_data[index+3]), (ie_length-1));
                }
                else
                {
                  MSG_ERROR_DS_0(MM_SUB,"Index out of bounds and buffer overflow");
            ...
            case IEI_NETWORK_SHORT_NAME :
              ...
              ** ---------------------------------------------------------------------- */
              ...
              if((index < message_len-2))
              {
                info->short_name.spare_bits = (L3_data[index+2] & 0x07);

                info->short_name.add_country_initials = ((L3_data[index+2] & 0x08) == 0x08);
              }
              else
              {
                MSG_ERROR_DS_0(MM_SUB,"Index out of bounds and buffer overflow");
              ...
              if ( valid_msg )
              {
                ...
                {
                  memscpy(info->short_name.name, SYS_MAX_SHORT_NAME_CHARS, &(L3_data[index+3]), (ie_length-1));
                }
                else
                {
                  MSG_ERROR_DS_0(MM_SUB,"Index out of bounds and buffer overflow");
              ...
              ** ----------------------------------------------- */
              if((index < message_len-7))
              {
                info->univ_time_and_time_zone.year      =
                  mm_convert_semi_octets(L3_data[index+1] );
                info->univ_time_and_time_zone.month     =
                  mm_convert_semi_octets(L3_data[index+2] );
                info->univ_time_and_time_zone.day       =
                  mm_convert_semi_octets(L3_data[index+3] );
                info->univ_time_and_time_zone.hour      =
                  mm_convert_semi_octets(L3_data[index+4] );
                info->univ_time_and_time_zone.minute    =
                  mm_convert_semi_octets(L3_data[index+5] );
                info->univ_time_and_time_zone.second    =
                  mm_convert_semi_octets(L3_data[index+6] );
                info->univ_time_and_time_zone.time_zone =
                  mm_convert_time_zone(L3_data[index+7] );
              ...
              if((index < message_len-4))
              {
                ...
                ***********************************************************/
                if (ie_length != 0)
                {
                  ...
                  info->lsa_identity.identity[0] = L3_data[index+4];
                  info->lsa_identity.identity[1] = L3_data[index+3];
                  info->lsa_identity.identity[2] = L3_data[index+2];
...
/*if CIOT rats, then send rat as LTE and ciot_lte as the active rat*/
#ifdef FEATURE_CIOT
