...
extern boolean is_last_stop_reason_diff;
...
#ifdef FEATURE_MBMS
extern void gmm_process_rrc_mbms_conn_req_ind(mm_cmd_type *message);
#endif
...
FUNCTION mm_check_if_lte_rat_changed

DESCRIPTION
  This function is used to check if there has been a change between the LTE
  rat passed in and the currently active LTE rat.
  
DEPENDENCIES
  
RETURN VALUE
  Returns TRUE if there is a difference between the passed in LTE rat
  and the cached value in the database, and if the cached value
  is not in the state which indicates that the LTE has been deactivated.
...
static boolean mm_check_if_lte_rat_changed(sys_radio_access_tech_e_type rat)
{    
   return (emm_ctrl_data_ptr->plmn_service_state->active_rat != rat);
}

/*************************************************************
FUNCTION mmcoord_stop_lte

DESCRIPTION
  This function processes stops deactivates the LTE when
  another rat needs to be acquired.
...
SIDE EFFECTS
  None
 *************************************************************/
static void mmcoord_stop_lte (void)
{
...
FUNCTION mmcoord_route_oos_rlf_mm_emm_messages_lte

DESCRIPTION
  This function overrides the function mmcoord_route_mm_emm_messages()
  It is called only during rat change
...
===========================================================================*/
boolean mmcoord_route_oos_rlf_mm_emm_messages_lte 
(
  void        *command_ptr,
#ifdef FEATURE_LTE  
  ...
  switch(mm_cmd->cmd.hdr.message_set)
  {
        ...
        *************************************************/
...
#ifdef FEATURE_LTE 
          if(mm_is_rat_lte(mm_cmd->cmd.mmr_reg_req.rat))
          {
        ...
        case MMR_PLMN_SEARCH_REQ:
          ...
          {                                    
            ...
            if ((emm_ctrl_data_ptr_arg->rrc_deact_req.deact_cnf_pending == TRUE)||
    ...
    case MS_TIMER:
      ...
      if((mm_cmd->cmd.timer_expiry.timer_id >= MIN_MM_TIMER && 
         ...
         ((mm_cmd->cmd.timer_expiry.timer_id != TIMER_T3325)))
      {
        mmcoord_route_mm_message(mm_cmd,mm_emm_dispatcher_fn_ptr );
      }
      else
    ...
    case MS_MM_MM:
       switch(mm_cmd->cmd.hdr.message_id)
       {
        case MM_WWAN_TECH_SUSPEND_IND:
           mmcoord_process_wwan_tech_suspend_ind(mm_cmd);
           break;
...
static void process_rat_change_req_lte(mm_cmd_type *cmd_ptr,mm_emm_dispatcher_fp **mm_emm_dispatcher_fn_ptr)
{
  emm_failure_type emm_failure_cause;
  ...
  switch(cmd_ptr->cmd.hdr.message_set)
  {
    case MS_MM_REG:
      ...
      {
      case MMR_PLMN_SEARCH_REQ:                    
             if(mm_is_rat_lte(cmd_ptr->cmd.mmr_plmn_search_req.rat)
             && (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat))) 
             {
                 /* Store the data for future execution */                   
                 emm_ctrl_data_ptr->reg_list_pended_indication = TRUE ;
				 emm_ctrl_data_ptr->reg_req_pended_indication = FALSE ; 
                 emm_cache_reg_cmd(cmd_ptr, emm_ctrl_data_ptr);
                 /* Deactivate the LTE AS */
                 mmcoord_stop_lte ();             
             }
			 /*Route all incoming messages during OOS/RLF, to mm_emm_oos_rlf_msg_handler*/
             *mm_emm_dispatcher_fn_ptr = mm_emm_oos_rlf_msg_handler_lte; 
             break;
        case MMR_REG_REQ: // RAT CHANGE DUE TO OOS/RLF   
          if(mm_is_rat_lte(cmd_ptr->cmd.mmr_reg_req.rat)
             && (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_reg_req.rat))) 		  
          {
              /* Case: LTE was requested and another LTE rat is already active*/
              /* Store MMR_REG_REQ parameters */
...
)
{
  ...
  switch(emm_msgr_cmd_ptr->emm_msgr_cmd.msg_hdr.id)
  {
    ...
    case NAS_EMM_DATA_REQ:
      if((emm_msgr_cmd_ptr->emm_msgr_cmd.emm_data_req.esm_out_msg.nas_hdr.msg_id == ESM_DATA_TRANSPORT)&&
         (emm_msgr_cmd_ptr->emm_msgr_cmd.emm_data_req.esm_out_msg.esm_data_transport.dsm_ptr != NULL))
      {
        dsm_free_packet(&emm_msgr_cmd_ptr->emm_msgr_cmd.emm_data_req.esm_out_msg.esm_data_transport.dsm_ptr);
      }
      emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
      emm_build_and_send_failure_ind(LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE, 
                                     LTE_NAS_NO_FAILURE, 
...
#if defined(FEATURE_CIOT) && !defined(FEATURE_NAS_GW)
boolean emm_process_oos_rlf_rrc_primitive_lte
(
    ...
    else if(emm_ctrl_data_ptr->pending_reg_cmd_ptr != NULL)
    {
      ...
      switch(emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.hdr.message_id)
      {
        ...
        emm_ctrl_data_ptr->plmn_service_state->active_rat = 
                   emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.mmr_plmn_search_req.rat;
#endif
...
boolean mmcoord_stop_gw(mm_cmd_type *message)
{
   ...
   if ((gmm_low_priority_indicator == TRUE) &&
       (gmm_congestion_due_to_implicit_reject == TRUE) &&
       (mm_timer_status[TIMER_T3346] == TIMER_ACTIVE))
   {
     /*Reset T3346 started due to implcit reject from GRR on interRAT to LTE*/
  ...
  */ 
  ...
  if (message->cmd.mmr_reg_req.req_service_domain == SYS_SRV_DOMAIN_NO_SRV)
  {
    ...
    else
    {
      ...
      mm_send_mmr_reg_cnf(NULL,SYS_SRV_STATUS_NO_SRV);
      mm_reg_waiting_for_reg_cnf = FALSE;
...
{
  mm_serving_plmn.cause = MMR_CAUSE_FAILURE_REJECT_REMAIN_ON_PLMN;
  //mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
...
/*change made so that reg don't send reg req on gsm after split,while ps on gsm not completed
  in merged condition*/
    ...
    ** ------------------------------------------------------- */
    //mm_serving_plmn.available_plmn_list.length  = 0; //CHKDBG
    ...
    if(mm_wcdma_rlf_state == WCDMA_RLF_STARTED)
    {
      ...
      ** ---------------------------------------------------------*/
          ...
          (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_PS_ONLY)))
      {
        ...
        {
#if defined(FEATURE_WCDMA) && defined(FEATURE_WRLF_SYSTEM_SEL) && !defined(FEATURE_NAS_CS_DISABLED)
      ...
      else
      {
            ...
            if(mm_is_multimode_sub())
       {
          ...
          else
          {
            ...
            */
#ifndef FEATURE_RAT_PRIORITY_LIST
            MSG_HIGH_DS_4(MM_SUB,"=MM= Dual Mode: %d - %d,Band Pref: 0x%X - 0x%X", mm_dual_mode, message->cmd.mmr_reg_req.dual_mode,
                                    mm_ue_band_class_pref, message->cmd.mmr_reg_req.band_pref);
...
#else
#ifdef FEATURE_CIOT
      (mm_rat_pri_list_match_ciot(&message->cmd.mmr_reg_req.rat_pri_list_info,
...
/*sglte logic for the pending case*/
    ...
    ** -----------------------------------------------------------------*/
    ...
    else
    {
      ...
      if (is_gsm_mode() &&
          gmm_wwan_tech_active == FALSE)
      {
        mm_send_rr_wwan_set_prioirity_ind(TRUE);
        gmm_wwan_resumption_req = TRUE;
      }
...
#endif
#if defined FEATURE_GPS_WWAN_PRIORITY_SEARCH
          else if(stop_mode_reason == SYS_STOP_MODE_REASON_SUSPEND_STACK)
          {
          	mmcoord_handle_deep_sleep_advance_scan();
          }
#endif
...
/*Reset flags at powerdown.*/         
          ...
          if ((stop_mode_reason == SYS_STOP_MODE_REASON_PSM) &&
               (cm_is_psm_context_saved((sys_modem_as_id_e_type)mm_as_id) == TRUE))
          {
...
#if defined(FEATURE_WCDMA) && defined(FEATURE_NAS_GW)
                   || ((mm_serving_plmn.info.active_rat == SYS_RAT_UMTS_RADIO_ACCESS) && (rrcrcr_is_proc_active_sub(local_as_id) == TRUE))
#endif
#ifdef FEATURE_TDSCDMA   
                   || ((mm_serving_plmn.info.active_rat == SYS_RAT_TDS_RADIO_ACCESS) && (tdsrrcrcr_is_proc_active_sub(local_as_id) == TRUE))
#endif
              )
            {
...
void mmcoord_process_rrc_sync_ind( mm_cmd_type *message )
{
  ...
  ciphering_enabled = message->cmd.rrc_sync_ind.ciphering_enabled;
#endif
...
#ifdef FEATURE_UMTS_VOICE_CIPHERING_IND
      case RRC_RAB_ESTABLISHED :
      case RRC_LOOPBACK_ESTABLISHED : 
      case RRC_HANDOVER :
      case RRC_CIPHERING_CHANGE :
        if(message->cmd.rrc_sync_ind.cn_domain_id == RRC_CS_DOMAIN_CN_ID)
        {
           mm_send_cm_cipher_ind(SYS_SRV_DOMAIN_CS_ONLY,ciphering_enabled);
        }
  ...
  switch (message->cmd.rrc_service_cnf.service_status)
  {
    case MM_AS_NO_SERVICE:
      ...
      if (mm_reg_waiting_for_reg_cnf)
      {
       ...
       // mmcoord_mmr_reg_cnf.available_plmn_list.length = num_plmns; //CHKDBG
        sys_detailed_plmn_list_s_type             available_plmn_list;
        available_plmn_list.length = num_plmns;
        ...
        for (entry = 0; entry < num_plmns; entry++ )
        {
          ...
          reg_plmn_list = &available_plmn_list.info[entry];
        ...
        mm_serving_plmn.cause = MMR_CAUSE_NOT_SPECIFIED;
        mm_serving_plmn.ue_oos = message->cmd.rrc_service_cnf.ue_oos;
        mm_send_mmr_reg_cnf(&available_plmn_list,mm_serving_plmn.info.service_status);
        mm_reg_waiting_for_reg_cnf = FALSE;
        mm_ready_to_send_reg_cnf   = FALSE;
      }
      break;
            ...
            **----------------------------------------------------------*/
                     ...
                     mm_serving_plmn.info.active_rat,(sys_modem_as_id_e_type)mm_as_id,mm_set_cm_stack_id(mm_as_id)) == FALSE))
            {
              ...
              mm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
                                               ...
                                               SYS_SRV_STATUS_LIMITED,
                                               TRUE );  
              MSG_HIGH_DS_0(MM_SUB, "=MM= SGLTE UE MODE CHANGE PENDED sending Limited to REG, REG will send Limited Regional to CM Or Don't Register on CMCC network when CT disabled, sending Limited") ;          
              ...
              if (mm_reg_waiting_for_reg_cnf)
              {
                ...
                //mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
                mm_serving_plmn.cause = MMR_CAUSE_FAILURE_REMAIN_ON_PLMN;
               // mmcoord_mmr_reg_cnf.service_state.service_status = SYS_SRV_STATUS_LIMITED; //CHKDBG
                if(cm_per_subs_is_reg_allowed(mm_serving_plmn.info.plmn,
                     mm_serving_plmn.info.active_rat,(sys_modem_as_id_e_type)mm_as_id,mm_set_cm_stack_id(mm_as_id)) == FALSE)
                {
                  mm_serving_plmn.cause = MMR_CAUSE_NOT_SPECIFIED;
                }
                mm_send_mmr_reg_cnf(NULL,SYS_SRV_STATUS_LIMITED);
                mm_reg_waiting_for_reg_cnf = FALSE;
                mm_ready_to_send_reg_cnf   = FALSE;
             }
             else 
...
void mmcoord_handle_wrlf_rr_plmn_select_cnf(mm_cmd_type *message )
{
  ...
  mm_waiting_for_service_cnf = FALSE;
  mm_serv_change_pending     = FALSE;
  mm_serving_plmn.ue_oos = message->cmd.rr_plmn_select_cnf.ue_oos; //CHKDBG
  ...
  switch (message->cmd.rr_plmn_select_cnf.service_state)
  {
    case MM_AS_NO_SERVICE:
      ...
      if (mm_reg_waiting_for_reg_cnf)
      {
        ...
        for (entry = 0; entry < num_plmns; entry++)
        {
          available_plmn_list.info[entry] =
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (message->cmd.hdr.message_id == (int) RR_PLMN_SELECT_CNF)
  {
    ...
    {
      MSG_HIGH_DS_4( MM_SUB, "=MM= Unexpected RR_PLMN_SELECT_CNF: mm_waiting_for_service_cnf = %d, mm_waiting_for_deact_cnf = %d, mm_nreg_req_received = %d, mm_waiting_for_stop_mode_cnf = %d",
                            mm_waiting_for_service_cnf,mm_waiting_for_deact_cnf,
                            mm_nreg_req_received, mm_waiting_for_stop_mode_cnf);
    }
#ifdef FEATURE_LTE
    ...
    else
    {
      mm_waiting_for_service_cnf = FALSE;
      mm_serv_change_pending     = FALSE;
      mm_serving_plmn.ue_oos = message->cmd.rr_plmn_select_cnf.ue_oos;
      MSG_HIGH_DS_1(MM_SUB ,"=MM= ue_oos flag is %d", message->cmd.rr_plmn_select_cnf.ue_oos);
#ifdef FEATURE_EOOS
     #error code not present
#endif
...
#ifdef FEATURE_CIOT
 if(emm_ctrl_data_ptr->conn_suspended_rat != mm_serving_plmn.info.active_rat )
  {
   ...
   emm_ctrl_data_ptr->conn_suspended_rat = SYS_RAT_NONE;
          ...
          ** ----------------------------------------------------- */
          ...
          mm_serving_plmn.t3122_running =  message->cmd.rr_plmn_select_cnf.t3122_running;
          gmm_cell_peo_supported = message->cmd.rr_plmn_select_cnf.cell_supports_peo;

          MSG_HIGH_DS_2(MM_SUB ,"=MM= t3122_running is %d GSM PEO = %d ", message->cmd.rr_plmn_select_cnf.t3122_running, message->cmd.rr_plmn_select_cnf.cell_supports_peo);
          ...
          ** ------------------------------------------------------------------ */
          ...
          gmm_handle_message( message );
#ifdef FEATURE_LTE
		 if ((gmm_state == GMM_REGISTERED) && mm_is_rat_lte(emm_ctrl_data_ptr->loaded_lte_rat ) &&
				(emm_ctrl_data_ptr->attached_lte_rat == SYS_RAT_GSM_RADIO_ACCESS))
				{
					gmm_reset_new_tilli();
...
#ifdef FEATURE_CIOT		  
	 if(emm_ctrl_data_ptr->emm_start_mo_excp_signalling)
	{
	  	emm_process_pending_mo_exception_data(TRUE) ;
	  emm_ctrl_data_ptr->emm_start_mo_excp_signalling = FALSE;
	}
          ...
          ** ------------------------------------------------ */
            ...
            **----------------------------------------------------------*/
                     ...
                     mm_serving_plmn.info.active_rat,(sys_modem_as_id_e_type)mm_as_id,mm_set_cm_stack_id(mm_as_id)) == FALSE))
            {
              ...
              mm_update_mm_serving_plmn_info( mm_serving_plmn.info.sim_state,
                                               ...
                                               SYS_SRV_STATUS_LIMITED,
                                               TRUE );       
              MSG_HIGH_DS_0(MM_SUB, "=MM= SGLTE UE MODE CHANGE PENDED sending Limited Regional Or Don't Register on CMCC network when CT disabled, sending Limited") ;      
              if (mm_reg_waiting_for_reg_cnf)
              {
                /* Set the REG CNF plmn service state to the Serving PLMN info */
                //mmcoord_mmr_reg_cnf.service_state = mm_serving_plmn.info;
                
                sys_srv_status_e_type       service_status = SYS_SRV_STATUS_LIMITED_REGIONAL;
                if(cm_per_subs_is_reg_allowed(mm_serving_plmn.info.plmn,
                     mm_serving_plmn.info.active_rat,(sys_modem_as_id_e_type)mm_as_id,mm_set_cm_stack_id(mm_as_id)) == FALSE)
                {
                ...
                mm_send_mmr_reg_cnf(NULL,service_status);
                mm_reg_waiting_for_reg_cnf = FALSE;
                mm_ready_to_send_reg_cnf   = FALSE;
             }
             else 
             {
               mmr_service_ind_s_type  mmr_service_ind_temp;
               mmr_service_ind_temp.service_state = mm_serving_plmn.info;
               mmr_service_ind_temp.service_state.service_status = SYS_SRV_STATUS_LIMITED_REGIONAL;                 
               if(cm_per_subs_is_reg_allowed(mm_serving_plmn.info.plmn,
                     mm_serving_plmn.info.active_rat,(sys_modem_as_id_e_type)mm_as_id,mm_set_cm_stack_id(mm_as_id)) == FALSE)
               {
                 mmr_service_ind_temp.service_state.service_status = SYS_SRV_STATUS_LIMITED;
          ...
          if ((mm_mmr_cell_service_ind_pending) && 
               (mm_serving_plmn.only_for_reg == FALSE) && //CHKDBG
               (sys_plmn_match( mm_serving_plmn.last_plmn, mm_serving_plmn.info.plmn)) && //CHKDBG
               ...
               &&(mm_serving_plmn.last_active_rat == mm_serving_plmn.info.active_rat))
          {
...
#ifdef FEATURE_SGLTE
          if ((mm_process_if_both_stack_has_camped_non_sglte_plmn(mm_ps_stack_id)==TRUE)||
      ...
      switch (message->cmd.rr_service_ind.service_state)
      {
        ...
        case MM_AS_SERVICE_AVAILABLE:
          ...
          ** -------------------------------------------- */
          ...
          gmm_cell_peo_supported = message->cmd.rr_service_ind.cell_supports_peo;

          MSG_HIGH_DS_2(MM_SUB ,"=MM= t3122_running is %d PEO = %d", message->cmd.rr_service_ind.t3122_running, message->cmd.rr_service_ind.cell_supports_peo);
...
#ifdef FEATURE_SGLTE
          if ((mm_process_if_both_stack_has_camped_non_sglte_plmn(mm_cs_stack_id)== TRUE)||
               (mm_is_sglteplmn_in_sglte_roam(mm_serving_plmn.info.plmn ) == TRUE)||
               (mm_is_non_sglte_plmn_in_sglte_home(mm_serving_plmn.info.plmn)==TRUE))
          
          {
            MSG_HIGH_DS_0(MM_SUB, "=MM= UE Mode change pending, Send Limited to REG , REG will send limited regional to CM");
            mm_substate_control( MM_STD_LIMITED_SERVICE );
...
/*24.008: 4.7.5.1.5 (j)	Timer T3346 is running
	The MS shall not start the routing area updating procedure unless the MS is accessing the network with access class 11 â€“ 15*/
/*4.4.4.9 (j)Timer T3246 is running
The MM connection establishment shall not be initiated unless the MS is accessing the network with access class 11 â€“ 15. */
...
The MM connection establishment shall not be initiated unless the MS is accessing the network with access class 11 â€“ 15. */
      ...
      if (message->cmd.rr_service_ind.service_state ==  MM_AS_SERVICE_AVAILABLE) 
      {
        ...
        {
          mm_reg_cnf_only_for_reg_local = mm_serving_plmn.only_for_reg;       
          ...
          *************************************************************/
          ...
          if((mm_serving_plmn.only_for_reg == mm_reg_cnf_only_for_reg_local == TRUE))
          {
            mm_serving_plmn.only_for_reg = TRUE; //CHKDBG
          }
          else
          {
...
*/          
          ...
          ** -------------------------------------------- */
          ...
          gmm_cell_peo_supported = message->cmd.rr_service_ind_from_ota_msg.cell_supports_peo;

          MSG_HIGH_DS_2(MM_SUB ,"=MM= t3122_running is %d PEO = %d", message->cmd.rr_service_ind_from_ota_msg.t3122_running, message->cmd.rr_service_ind_from_ota_msg.cell_supports_peo);
      ...
      ** ------------------------------------------------------------- */
      ...
      if (message->cmd.rr_service_ind_from_ota_msg.service_state ==  MM_AS_SERVICE_AVAILABLE) 
      {
        if (mm_needs_to_send_mmr_service_ind || (mmcoord_cell_id_changed && !mm_reg_ind_sent))
        {
          ...
          *************************************************************/
          ...
          mm_to_only_reg = mm_to_only_reg_local;
          mm_serving_plmn.only_for_reg = mm_reg_cnf_only_for_reg_local;
        }
      }
      mmcoord_cell_id_changed = FALSE;
#ifdef FEATURE_NAS_GW
      ...
      if ((mm_mmr_cell_service_ind_pending) && 
             (mm_serving_plmn.only_for_reg == FALSE) &&
             (sys_plmn_match( mm_serving_plmn.last_plmn, mm_serving_plmn.info.plmn)) &&
             (mm_serving_plmn.last_rac == mm_serving_plmn.rac) &&
             (mm_serving_plmn.last_lac == b_unpackw(mm_serving_plmn.lac.lac,0,16)) &&
             (mm_serving_plmn.last_active_rat == mm_serving_plmn.info.active_rat))
      {
  ...
  if (message->cmd.rrc_page_ind.message_header.message_id == (int) RRC_PAGE_IND)
  {
    ...
    ** -------------------------------------------- */
    ...
    else /* cn_domain_id == RRC_PS_DOMAIN_CN_ID */
    {
      ...
      {
        MSG_HIGH_DS_0(MM_SUB,"=MM= ignoring page on thsi sub as Cm is starting a voice call on other sub");
        return;
      }
#endif    
      gmm_handle_message( message );
...
#endif
      if(mm_wrlf_pending_rrc_message.cmd.hdr.message_id == (int) RRC_SERVICE_CNF)
      {
...
))
  {
    switch (message->cmd.hdr.message_id)
    {
      ...
      case RRC_MBMS_CONN_REQ_IND:
        MSG_HIGH_DS_2(MM_SUB, "=MM= MBMS: RRC ESTB: %d, SERVICE REQ: %d", message->cmd.rrc_mbms_conn_ind.rrc_establish_cause,\
          message->cmd.rrc_mbms_conn_ind.service_type);
        if ( !(message->cmd.rrc_mbms_conn_ind.rrc_establish_cause == MBMS_RECEPTION ||
          message->cmd.rrc_mbms_conn_ind.rrc_establish_cause == MBMS_PTP_RB_REQUEST) ||
          !(message->cmd.rrc_mbms_conn_ind.service_type == (rrc_mbms_conn_service_type_e_type) GMM_MBMS_MULTICAST_SERVICE_RECEPTION ||
          message->cmd.rrc_mbms_conn_ind.service_type == (rrc_mbms_conn_service_type_e_type) GMM_MBMS_BROADCAST_SERVICE_RECEPTION) )
        {
            valid_command = FALSE;
        }
        break;
  ...
  if (message->cmd.hdr.message_set == MS_MM_RR)
  {
    switch (message->cmd.hdr.message_id)
    {
      ...
      case RR_PLMN_LIST_CNF:
        if ((message->cmd.rr_plmn_list_cnf.service_search != SYS_SERVICE_SEARCH_CELL_SEARCH &&
#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH			
			message->cmd.rr_plmn_list_cnf.service_search != SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH &&
#endif
			message->cmd.rr_plmn_list_cnf.available_plmn_list.plmn_list.length >
             SYS_PLMN_LIST_MAX_LENGTH) || 
            (message->cmd.rr_plmn_list_cnf.service_search == SYS_SERVICE_SEARCH_CELL_SEARCH &&
			message->cmd.rr_plmn_list_cnf.available_plmn_list.cell_info_list.geran_cell_list.num_cell>
             SYS_PCI_SCAN_MAX_NUM_PCI))
        {
          valid_command = FALSE;
        }
        break;
      ...
      case RR_PS_IMPLICIT_REJECT_IND:
        break;
...
void mmcoord_process_mmr_ph_status_change_req( mm_cmd_type *message )
{
  ...
  if (message->cmd.hdr.message_id == (int) MMR_PH_STATUS_CHANGE_REQ)
  {
    ...
    ** -------------------------------------------------------------------- */
     ...
     {
       (void)mm_write_backoff_remaining_info_to_efs();
     }
    ...
    case MS_MM_RR :
      ...
      switch (message->cmd.hdr.message_id)
      {
       ...
       case RR_GMM_GPRS_RESUMPTION_IND:
        ...
        case RR_PS_IMPLICIT_REJECT_IND:
          mm_sub_id = (mm_as_id_e_type)message->cmd.rr_ps_implicit_reject_ind.as_id;
          break;
...
#ifdef FEATURE_MBMS
      case RRC_MBMS_CONN_REQ_IND:
        mm_sub_id = (mm_as_id_e_type)message->cmd.rrc_mbms_conn_ind.as_id;
        break;
#endif
      case RRC_FAST_DORMANCY_CNF:
  ...
  case MS_MM_REG:
    ...
    case MMR_DATA_PRIORITY_IND :
      ...
      case MMR_PROFILE_TYPE_IND:
       ...
       case MMR_PSM_SET_CONFIG_PARAMS_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_psm_set_cfg_params_req.as_id;
         break;

       case MMR_PSM_GET_TIMER_INFO_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_psm_get_timer_info_req.as_id;
         break;

       case MMR_ENTER_PSM_MODE_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_enter_psm_mode_req.as_id;
         break;

       case MMR_SET_EDRX_CFG_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_set_edrx_cfg_req.as_id;
         break;

       case MMR_GET_EDRX_PARAMS_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_get_edrx_params_req.as_id;
         break;
      case MMR_SEND_PLMN_HOME_ROAM:
        mm_as_id = (mm_as_id_e_type)message->cmd.mmr_send_plmn_home_roam_as_id;
         break;

       case MMR_PSM_SAVE_MMGSDI_EF_REQ:
         mm_as_id = (mm_as_id_e_type)message->cmd.mmr_psm_save_mmgsdi_ef_req.as_id;
       break;
  ...
  case MS_GMM_SM:
    ...
    switch (message->cmd.hdr.message_id)
    {
      ...
      case GMMSM_PS_CALL_STATUS_IND:
        ...
        break;
      case GMMSM_SAVE_CTXT_PSM_CNF:
        mm_sub_id = (mm_as_id_e_type)message->cmd.gmmsm_save_ctxt_psm_cnf.as_id;
        break;
      case GMMSM_RESTORE_CTXT_PSM_CNF:
        mm_sub_id = (mm_as_id_e_type)message->cmd.gmmsm_restore_ctxt_psm_cnf.as_id;
        break;
...
void mm_update_data_priority(mm_data_pri_client_enum_T client_id, sys_data_priority_e_type priority)
{
 ...
 /* Evaluate current data priorty */
  ...
  if(priority == SYS_DATA_PRIORITY_HIGH)
  {
    set_priority = 1;
  }

  mm_data_pri_value &= ~( 1 << (uint8)client_id);

  mm_data_pri_value |= set_priority<<(uint8)client_id;


  /* Evaluate the new data priortiy*/
  if(mm_data_pri_value != 0)
  {
  ...
  for(i=0;i<MAX_MM_TIMER+1;i++)
  {
    if (mm_timer_status[i] == TIMER_ACTIVE && 
        (i != TIMER_T3212 && i != TIMER_T3312 &&
         i != TIMER_T3346 && i != TIMER_T3246 && i != TIMER_T3324) &&
        (i != TIMER_MM_FORBIDDEN_LAI_CLEAR))
    {
      if (gs_modify_timer_ext(
                               get_mm_timer_queue(),
                               i,
                               TIMER_MODIFY_PAUSE) != GS_SUCCESS)
      {
...
#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH	
void mmcoord_handle_deep_sleep_advance_scan(void)
{
  /* if FEATURE_LTE is defined stopping of MM and EMM timers is
   *  done in "mmcoord_route_mm_emm_messages()" handler 
   */
  ...
  if (mm_serving_plmn.info.active_rat == SYS_RAT_GSM_RADIO_ACCESS)
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= Send RR_STOP_MODE_REQ for Deep sleep advance Scan");
    mm_deep_sleep_pending = TRUE;
    mm_send_rr_stop_mode_req(RR_SUSPEND_STACK);
  }
#ifdef FEATURE_WCDMA
  else
  {
...
void mmcoord_process_mmr_multimode_subs_chgd_req( mm_cmd_type *message )
{
  ...
  if (message->cmd.hdr.message_id == (int) MMR_MULTIMODE_SUBS_CHGD_REQ)
  {
    ...
    if(prev_multimode_as_id != new_multimode_as_id)
    {
      ...
      mm_per_subs_copy_forbidden_lai_list((byte)new_multimode_as_id,(byte)prev_multimode_as_id);
...
{
  nas_assert_check((boolean)(mm_emm_dispatcher_fn_ptr != NULL));
...
)
{
  ...
  switch(emm_msgr_cmd->emm_msgr_cmd.msg_hdr.id)
  {
    ...
    case NAS_EMM_DATA_REQ:
#ifdef FEATURE_CIOT
      if((emm_msgr_cmd->emm_msgr_cmd.emm_data_req.esm_out_msg.nas_hdr.msg_id == ESM_DATA_TRANSPORT) &&
         (emm_msgr_cmd->emm_msgr_cmd.emm_data_req.esm_out_msg.esm_data_transport.dsm_ptr != NULL))
      {
        dsm_free_packet(&emm_msgr_cmd->emm_msgr_cmd.emm_data_req.esm_out_msg.esm_data_transport.dsm_ptr);
      }
#endif
      emm_failure_cause.cause_type = LTE_NAS_IRAT_INVALID_STATE;
      emm_build_and_send_failure_ind(LTE_NAS_SERVICE_REQ_FAILED_NO_THROTTLE, 
...
#ifdef FEATURE_LTE_TO_1X
    case NAS_EMM_1XCSFB_ESR_CALL_REQ:
      MSG_ERROR_DS(MM_SUB,"NAS_EMM_1XCSFB_ESR_CALL_REQ is received during OOS/RLF, this message shall be currently discarded",0,0,0);
      break;
    case NAS_EMM_1XCSFB_ESR_CALL_ABORT_REQ:
  ...
  *************************************************/
  ...
  case LTE_RRC_DEACTIVATE_CNF:
    ...
    if ((mmcoord_svc_cnf_pending) &&
        (((mmcoord_pending_reg_list_req_message.cmd.hdr.message_set == MS_MM_RR) &&
           (mmcoord_pending_reg_list_req_message.cmd.hdr.message_id  == (int) RR_PLMN_SELECT_CNF)) 
#if defined(FEATURE_WRLF_SYSTEM_SEL) && defined(FEATURE_NAS_GW)
       ||
         ((mmcoord_pending_reg_list_req_message.cmd.hdr.message_set == MS_MM_RRC) &&
          (mmcoord_pending_reg_list_req_message.cmd.hdr.message_id  == (int) RRC_SERVICE_CNF))
#endif
        ))
    {
      /*Route all incoming messages to normal_msg_handler, now new RAT is ACTIVE*/
    ...
    else if(emm_ctrl_data_ptr->reg_req_pended_indication == TRUE)
    {
...
#endif
      if (is_last_stop_reason_diff == FALSE)
      {
        MSG_HIGH_DS_0(MM_SUB,"=EMM= Reset reg_req_pended_indication to FALSE");
	  ...
	  {
		is_last_stop_reason_diff = FALSE;  
	  }	  
    }
    else if(emm_ctrl_data_ptr->pending_reg_cmd_ptr != NULL)
    {
...
void process_rat_change_req(mm_cmd_type *cmd_ptr,mm_emm_dispatcher_fp **mm_emm_dispatcher_fn_ptr)
{
  ...
  switch(cmd_ptr->cmd.hdr.message_set)
  {
    case MS_MM_REG:
      ...
      case MMR_PLMN_SEARCH_REQ:
             if(mm_is_rat_lte(cmd_ptr->cmd.mmr_plmn_search_req.rat)
             && (emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_GSM_RADIO_ACCESS)
             ) 
             {
                 /* GW will be stopped in this branch, EMM will store the command parameters */
                 /* Case: LTE is requested, and LTE is not active */
                 mm_reg_waiting_for_reg_cnf = FALSE;
                 mm_serv_change_pending = FALSE;
#ifndef FEATURE_NAS_CS_DISABLED
                     ...
                     )
                 {
                     if(mm_in_conn_management_state() == TRUE) 
                     {
						if((cmd_ptr->cmd.hdr.message_set == MS_MM_REG) && (cmd_ptr->cmd.hdr.message_id == MMR_PLMN_SEARCH_REQ))
                  		{
                  		  mm_send_empty_mmr_plmn_search_cnf(&(cmd_ptr->cmd.mmr_plmn_search_req),REG_MM_PLMN_SEARCH_ABORTED);
                  		}
						else
						{
							if((mmcoord_pending_reg_message.cmd.mmr_reg_req.network_selection_mode== SYS_NETWORK_SELECTION_MODE_AUTOMATIC &&
             ...
             else if(mm_is_rat_lte(cmd_ptr->cmd.mmr_plmn_search_req.rat)
             && (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat))) 
             {
                 /* Store the data for future execution */                   
                 emm_ctrl_data_ptr->reg_list_pended_indication = TRUE ;
             ...
             else if((cmd_ptr->cmd.mmr_plmn_search_req.rat == SYS_RAT_GSM_RADIO_ACCESS)
             && (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat))) 
             {
                 /* Case : LTE was active, and the request comes for GSM */ 
                 /* Store the data for future execution */                   
                 mm_plmn_list_req_pending = TRUE;
				 emm_ctrl_data_ptr->reg_req_pended_indication = FALSE;
                 mmcoord_pending_reg_list_req_message.cmd.hdr = cmd_ptr->cmd.hdr;
                 mmcoord_pending_reg_list_req_message.cmd.mmr_plmn_search_req = cmd_ptr->cmd.mmr_plmn_search_req;
                 mmcoord_reg_req_pending = FALSE;
                 /* Deactivate the LTE AS */
                 mmcoord_stop_lte ();
             }
	    *mm_emm_dispatcher_fn_ptr = mm_emm_oos_rlf_msg_handler;
        ...
        case MMR_REG_REQ:
          if(mm_is_rat_lte(cmd_ptr->cmd.mmr_reg_req.rat)
             && mm_is_rat_gwt(emm_ctrl_data_ptr->plmn_service_state->active_rat)
             ) 
          {
            /* GW will be stopped in this branch, EMM will store the MMR_REG_REQ parameters */
            /* Case: LTE is requested, and LTE is not active */
            if (
#ifndef FEATURE_NAS_IRAT_DISABLED
...
#else
            /*Save the REG_REQ in the pending list*/
         ...
         else if(mm_is_rat_lte(cmd_ptr->cmd.mmr_reg_req.rat)
             && (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_reg_req.rat))) 		  
          {
              /* Case: LTE was requested and another LTE rat is already active*/
              /* Store MMR_REG_REQ parameters */
              ...
              emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr = &mmcoord_pending_reg_message;     
              memscpy(emm_ctrl_data_ptr->reg_req_pending_mm_cmd_ptr, sizeof(mm_cmd_type),cmd_ptr,sizeof(mm_cmd_type));
			  if(emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS && emm_ctrl_data_ptr->plmn_service_state->sim_state == SYS_SIM_STATE_CP_SMS_INVALID)
			  {
          ...
          else if(mm_is_rat_gwt(cmd_ptr->cmd.mmr_reg_req.rat))
          {
            /*Save the REG_REQ in the pending list*/
            memscpy(&mmcoord_pending_reg_message, sizeof(mm_cmd_type),cmd_ptr,sizeof(mm_cmd_type));
#if defined(FEATURE_LTE)
  ...
  switch(mm_cmd->cmd.hdr.message_set)
  {
        ...
        *************************************************/
             ...
             (mm_cmd->cmd.mmr_reg_req.rat == SYS_RAT_TDS_RADIO_ACCESS))
          {
            ...
            if (is_umts_mode() || is_gsm_mode())
            {
              ...
              if (mmcoord_svc_cnf_pending == TRUE)
              {
                // Reset this flag as we will not be processing any pending reg_req in G/W now.
                mmcoord_svc_cnf_pending = FALSE;
				memscpy(&mmcoord_pending_reg_list_req_message, sizeof(mm_cmd_type),mm_cmd,sizeof(mm_cmd_type));
...
#ifdef FEATURE_LTE 
          else if(mm_is_rat_lte(mm_cmd->cmd.mmr_reg_req.rat))
          {
        ...
        case MMR_PLMN_SEARCH_REQ:
          ...
          if(is_lte_mode())
          {
            ...
            if((reg_req_pended) && 
               (mm_checkif_rat_pri_list_has_lte_rat(
               &(emm_ctrl_data_ptr_arg->reg_req_pending_ptr->rat_pri_list_info))==FALSE )&&
            ...
            else if(emm_ctrl_data_ptr_arg->rrc_deact_req.deact_cnf_pending == TRUE)
            {
             ...
             if((mm_cmd->cmd.mmr_plmn_search_req.service_search == SYS_SERVICE_SEARCH_NONE ||
                  mm_cmd->cmd.mmr_plmn_search_req.service_search == SYS_SERVICE_SEARCH_CELL_SEARCH)
                  && mm_cmd->cmd.mmr_plmn_search_req.network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL 
                  && mm_cmd->cmd.mmr_plmn_search_req.rat == SYS_RAT_GSM_RADIO_ACCESS)
              {
                MSG_HIGH_DS_0(MM_SUB, "=MM= Pending MMR_PLMN_SEARCH_REQ - MM Waiting for STOP MODE CNF from AS");
                mmcoord_reg_req_pending = FALSE;
                mm_plmn_list_req_pending = TRUE;
...
#endif
          { 
            /* Active RAT is GSM now */
            if((emm_ctrl_data_ptr_arg->pending_reg_cmd_ptr != NULL) &&
               (emm_ctrl_data_ptr_arg->pending_reg_cmd_ptr->cmd.hdr.message_id
                   == MMR_PLMN_SEARCH_REQ))
          {
              /* Cache the abort request */
              emm_cache_reg_cmd(mm_cmd, emm_ctrl_data_ptr_arg);
            }
            mmcoord_route_mm_message(mm_cmd,mm_emm_dispatcher_fn_ptr);
          }
        ...
        case MMR_STOP_MODE_REQ:
          ...
          */
          ...
          if(is_lte_mode())
          {
            ...
            mm_reg_waiting_for_reg_cnf = FALSE;
            MSG_HIGH_DS_0(MM_SUB,"=MM= MM received  MMR_STOP_MODE_REQ, sending it to EMM");
			if ((mm_plmn_search_in_progress == FALSE) && (mm_plmn_list_req_pending == TRUE)) 
			{
			  mm_plmn_list_req_pending = FALSE;
			}
            if((emm_ctrl_data_ptr_arg->rrc_deact_req.deact_cnf_pending == FALSE)||
...
#else
              process_rat_change_req(&mmcoord_pending_reg_message,mm_emm_dispatcher_fn_ptr);
#endif
            }
            else if(mm_ps_detach_state == MM_PS_DETACH_STATE_WAITING_FOR_REG_REQ)
            {
...
#ifdef FEATURE_CIOT            
            else if((emm_ctrl_data_ptr->reg_list_pended_indication  == TRUE) &&    
                ((emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.hdr.message_set == MS_MM_REG) &&
                (emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.hdr.message_id  == (int) MMR_PLMN_SEARCH_REQ)))
            {
                ...
                emm_ctrl_data_ptr->reg_list_pended_indication = FALSE;
                /* Need to set the active rat which was reset after G was stopped */
                emm_ctrl_data_ptr->plmn_service_state->active_rat = 
                    emm_ctrl_data_ptr->pending_reg_cmd_ptr->cmd.mmr_plmn_search_req.rat;
                emm_process_reg_command(emm_ctrl_data_ptr->pending_reg_cmd_ptr,
                    emm_ctrl_data_ptr);

                MSG_HIGH_DS_0(MM_SUB, "=MM= Freeing Pended Cache");
                /* Free the memory */
                emm_modem_mem_free(emm_ctrl_data_ptr->pending_reg_cmd_ptr,
                              emm_ctrl_data_ptr);
                emm_ctrl_data_ptr->pending_reg_cmd_ptr = NULL;
    ...
    case MS_TIMER:
      ...
      if(((mm_cmd->cmd.timer_expiry.timer_id >= MIN_MM_TIMER && 
         ...
         &&
         (((mm_cmd->cmd.timer_expiry.timer_id != TIMER_T3346) &&
           (mm_cmd->cmd.timer_expiry.timer_id != TIMER_T3324) &&
           (mm_cmd->cmd.timer_expiry.timer_id != TIMER_T3325)) ||
...
DESCRIPTION
This is a function which handles MM_WWAN_TECH_STATUS_IND WWAN tech status posted by MM itself when WWACH tech status
change happens at access stratum and post the indication to DS.
...
void mmcoord_process_wwan_tech_suspend_ind
( 
  mm_cmd_type *cmd_ptr
)
{
  ...
  gmm_wwan_tech_active = !(cmd_ptr->cmd.mm_wwan_tech_suspend_ind.tech_suspend);
  
  if(is_gsm_mode()|| 
	((emm_chk_nb1_rat() || emm_chk_m1_rat()) && 
	((gmm_wwan_tech_active == TRUE)||(connection_state == EMM_CONNECTED_STATE))))
	{
  
	    MSG_HIGH_DS_3( SM_SUB," Sending MM_WWAN_TECH_STATUS_IND to DS Suspend = %d active_rat = %d gmm_wwan_tech_active = %d", cmd_ptr->cmd.mm_wwan_tech_suspend_ind.tech_suspend,
																									mm_serving_plmn.info.active_rat,gmm_wwan_tech_active);
		ds_rpt = ds_get_cmd_buf();

		if (ds_rpt!=NULL)
		{
		ds_rpt->cmd.mm_wwan_tech_status_ind.message_header.message_set = MS_MM_DS;
		ds_rpt->cmd.mm_wwan_tech_status_ind.message_header.message_id = (int) MM_WWAN_TECH_STATUS_IND;
		PUT_IMH_LEN(sizeof(mm_wwan_tech_status_ind_s_type) - sizeof(IMH_T),
                    &ds_rpt->cmd.mm_wwan_tech_status_ind.message_header );

		ds_rpt->cmd.mm_wwan_tech_status_ind.is_suspend = cmd_ptr->cmd.mm_wwan_tech_suspend_ind.tech_suspend;
		...
		//Send cmd to DS
		ds_pdn_mgr_nas_to_ds_msg_hdlr((void *)&ds_rpt->cmd.mm_wwan_tech_status_ind);
	...
	{
		MSG_HIGH_DS_2( SM_SUB," Not Sending MM_WWAN_TECH_STATUS_IND to DS since RRC connection state is  = %d , gmm_wwan_tech_active = %d", connection_state, gmm_wwan_tech_active);
	}
#ifdef FEATURE_NAS_GW
#ifdef FEATURE_NAS_FREE_FLOAT
   if ((is_gsm_mode()) &&
   ...
   /* Enable PS at GRR on WWAN Tech resumption*/
   else if ((is_gsm_mode()) &&
            (mm_serving_plmn.gmm_service_state == SERVICE_AVAILABLE) &&
...
/*===========================================================================
FUNCTION: MMCOORD_CHECK_PSM_RESTORE_STARTED

DESCRIPTION
  This function checks if PSM context restore is started

DEPENDENCIES
  None

RETURN VALUE
  TRUE if restore is started, FALSE otherwise

SIDE EFFECTS
  None
===========================================================================*/
boolean mmcoord_check_psm_restore_started
(
#ifdef FEATURE_LTE
emm_ctrl_data_type *emm_ctrl_data_ptr
#endif
)
{
  ...
  MSG_HIGH_DS_1(MM_SUB, "=MM= Start PSM restore context = %d", mm_psm_restore_pending);
  ...
  {
    if(reg_nv_per_subs_psm_restore_pending((sys_modem_as_id_e_type)mm_as_id,
                                           mm_psm_cache_ptr->plmn,
                                           mm_psm_cache_ptr->rat,
                                           mm_psm_cache_ptr->sim_state) == FALSE)
    {
      mm_psm_restore_pending = FALSE;
      mm_psm_free_cache();
      if(mm_get_ulog_handle() != NULL)
      {
        ULOG_RT_PRINTF_0(mm_get_ulog_handle(), "=MM= Error: reg_psm_restore status not pending");
      }
    } /* PSM currently supported for LTE RAT only */
    else if((mm_imsi_is_native == FALSE) 
             ||
            ((mm_psm_cache_ptr->rat != SYS_RAT_GSM_RADIO_ACCESS)
#ifdef FEATURE_LTE
             &&
             (mm_is_rat_lte(mm_psm_cache_ptr->rat) == FALSE)
#endif
            ))
    {
      ...
      if(mm_get_ulog_handle() != NULL)
      {
        ULOG_RT_PRINTF_2(mm_get_ulog_handle(), "=MM= Error: PSM restore failed: mm_imsi_is_native %d,  rat %d",
                         mm_imsi_is_native, mm_psm_cache_ptr->rat);
      }
      mm_psm_free_cache();
      sm_psm_free_cache();
    }
    ...
    {
      if(mm_psm_cache_ptr->sim_state == SYS_SIM_STATE_CS_INVALID)
      {
        mm_serving_plmn.info.sim_state = SYS_SIM_STATE_CS_INVALID;
        mm_sim_revert_sim_state_on_psm_restore_failure = TRUE;
      }
      result = TRUE;
    }
    ...
    if(result == TRUE)
    {
      ...
      uint64 periodic_tau_tolerance = 4*60*1000; /* 4 minutes (in milliseconds) */
      boolean stop_restore = FALSE;

      time_elapsed = current_timestamp - mm_psm_cache_ptr->timers_timestamp;
      
      /* Do not attempt to restore context if System time is lost due to abrupt loss of power */
      if(current_timestamp <= mm_psm_cache_ptr->timers_timestamp)
      {
        stop_restore = TRUE;
      }
#ifdef FEATURE_LTE
               ...
               mm_is_multimode_sub() &&
#endif
              (mm_is_rat_lte(mm_psm_cache_ptr->rat)))
      {
         /* Do not attempt to restore context if elapsed time is more than the saved remaining
             ** time for TAU plus 4 minutes tolerance to account for mobile reachable timer in the NW 
              */
        if((mm_psm_cache_ptr->lte.t3412_timer != 0) &&
           (time_elapsed >= (mm_psm_cache_ptr->lte.t3412_timer + periodic_tau_tolerance)))
        {
          stop_restore = TRUE;
        }
        else if(mm_psm_cache_ptr->lte.ftai_clear_timer != 0)
        {
          /* Check if FTAI clear timer expired in psm sleep. If not expired, then
                ** restore the FTAI lists and restart the timer for remaining duration. 
                */
          if((uint64)mm_psm_cache_ptr->lte.ftai_clear_timer > time_elapsed)
          {
            mm_start_timer(EMM_FORBIDDEN_TAI_CLEAR_TIMER, 
                              (uint64)mm_psm_cache_ptr->lte.ftai_clear_timer - time_elapsed);
            *(emm_ctrl_data_ptr->rrc_forbidden_list_ptr) = mm_psm_cache_ptr->lte.rrc_forbidden_list;
            *(emm_ctrl_data_ptr->forbidden_for_service_list_ptr) = mm_psm_cache_ptr->lte.forbidden_for_service_list;

            (void)pthread_mutex_lock(emm_ctrl_data_ptr->manual_ftai_list_mutex_ptr);
            *(emm_ctrl_data_ptr->manual_ftai_list_ptr) = mm_psm_cache_ptr->lte.manual_ftai_list;
            (void)pthread_mutex_unlock(emm_ctrl_data_ptr->manual_ftai_list_mutex_ptr);
          }
          else if((dword)mm_psm_cache_ptr->t3247_value> time_elapsed)
          {
            mm_start_timer(TIMER_T3247, 
                           (dword)mm_psm_cache_ptr->t3247_value - time_elapsed);
            *(emm_ctrl_data_ptr->rrc_forbidden_list_ptr) = mm_psm_cache_ptr->lte.rrc_forbidden_list;
            *(emm_ctrl_data_ptr->forbidden_for_service_list_ptr) = mm_psm_cache_ptr->lte.forbidden_for_service_list;
...
#endif
#ifdef FEATURE_NAS_GW
      if (mm_psm_cache_ptr->rat == SYS_RAT_GSM_RADIO_ACCESS)
      {
         /* Do not attempt to restore context if elapsed time is more than the saved remaining
             ** time for TAU plus 4 minutes tolerance to account for mobile reachable timer in the NW */
        if(time_elapsed >= (mm_psm_cache_ptr->gsm.t3312_timer + periodic_tau_tolerance))
        {
          stop_restore = TRUE;
        }
        else if(mm_psm_cache_ptr->gsm.ftai_clear_timer != 0)
        {
           /* Check if FTAI clear timer expired in psm sleep. If not expired, then
                ** restore the FTAI lists and restart the timer for remaining duration. */
          if((uint64)mm_psm_cache_ptr->gsm.ftai_clear_timer > time_elapsed)
          {
            mm_start_timer(TIMER_MM_FORBIDDEN_LAI_CLEAR, 
                              (uint64)mm_psm_cache_ptr->gsm.ftai_clear_timer - time_elapsed);
            mm_per_subs_restore_forbidden_lai_list(mm_psm_cache_ptr->gsm.forbidden_lai_list);
            mm_per_subs_restore_lai_reject_list(&mm_psm_cache_ptr->gsm.manual_forbidden_lai_reject_list);
          }
        }
      }

#endif
      if(stop_restore == TRUE)
      {
        ...
        MSG_HIGH_DS_4(MM_SUB, "=MM= Restore failed: cur_sys_time %u sec, saved_sys_time %u sec, rem_tau_time %u sec, elapsed_time %u sec",
                      current_timestamp/1000, mm_psm_cache_ptr->timers_timestamp/1000,
                      mm_psm_cache_ptr->lte.t3412_timer/1000, time_elapsed);

        if(mm_get_ulog_handle() != NULL)
        {
          ULOG_RT_PRINTF_4(mm_get_ulog_handle(), "=MM= Restore failed: cur_sys_time %u sec, saved_sys_time %u sec, rem_tau_time %u sec, elapsed_time %u sec",
                           current_timestamp/1000, mm_psm_cache_ptr->timers_timestamp/1000,
                           mm_psm_cache_ptr->lte.t3412_timer/1000, time_elapsed);
        }
  ...
  if(result == TRUE)
  {
    /* Begin PSM restore. PSM currently supported for LTE RAT only */
    switch(mm_psm_cache_ptr->rat)
    {
#ifdef FEATURE_LTE
      case SYS_RAT_LTE_RADIO_ACCESS:
      case SYS_RAT_LTE_M1_RADIO_ACCESS:
      case SYS_RAT_LTE_NB1_RADIO_ACCESS:
        result = emm_restore_ctxt_on_psm_exit(emm_ctrl_data_ptr);
        break;
#endif
#ifdef FEATURE_NAS_GW
     case SYS_RAT_GSM_RADIO_ACCESS:
        result = gmm_restore_ctxt_on_psm_exit();
        break;
#endif
      default:
        mm_send_mmr_psm_restore_failed_ind();
  ...
  return result;
} /* end mmcoord_check_psm_restore_started() */
...
void mmcoord_process_subsription_ready(void)
{
  ...
  else 
  {
    ...
    if( status )
    {
      ...
      /*Read 3GPP rel version*/
      emm_read_sim_available_nv(emm_ctrl_data_ptr);
#endif
    } 
  }
#if defined(FEATURE_DUAL_SIM) && !defined(FEATURE_LTE)
  ...
  if((mm_get_ulog_handle() != NULL) && (status == FALSE) && (mm_psm_restore_pending == TRUE))
  {
    ULOG_RT_PRINTF_0(mm_get_ulog_handle(), "=MM= Error: PSM restore failed");
  }

  /* If PSM restore is started then SIM AVAILABLE confirmation will be sent by the handler
     which processes the PSM restore response from CM for the particular RAT */
  if((status == FALSE) ||
     (mmcoord_check_psm_restore_started(
#ifdef FEATURE_LTE
                                         emm_ctrl_data_ptr
#endif
                                       ) == FALSE))
       ...
       switch(cmd_ptr->cmd.hdr.message_id)
       {
...
#endif
		{
            ...
            if(!is_lte_mode())
            {
                ...
                {
                    /* LTE is requested */
                    /* UE not yet camped or searched. Check if RATs need to change */

                    if ((emm_ctrl_data_ptr->loaded_lte_rat != cmd_ptr->cmd.mmr_plmn_search_req.rat) &&
                        (emm_ctrl_data_ptr->loaded_lte_rat != SYS_RAT_NONE))
                {
                    ...
                    {
                        emm_ctrl_data_ptr->loaded_lte_rat = cmd_ptr->cmd.mmr_plmn_search_req.rat;
                    ...
                    mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat))
                {
                    /* Request for LTE and either camped or searched*/                        
                    process_rat_change_req(cmd_ptr,mm_emm_dispatcher_fn_ptr);
                }
				else if ((cmd_ptr->cmd.mmr_plmn_search_req.rat == SYS_RAT_GSM_RADIO_ACCESS)&& 
					(emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_NONE )&&
                     mm_is_rat_lte(emm_ctrl_data_ptr->loaded_lte_rat))
...
#else 
/* case of LTE only build */
#ifdef FEATURE_CIOT
             if(is_lte_mode()&&(mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat)==TRUE))
             {             
                   process_rat_change_req_lte(cmd_ptr,mm_emm_dispatcher_fn_ptr);               
             }
            else if (mm_is_rat_lte(cmd_ptr->cmd.mmr_plmn_search_req.rat) &&
                   (emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_NONE))
                {
                    /* Search for LTE is requested */
                    /* UE not yet camped or searched. Check if RATs need to change */
...
#ifdef FEATURE_CIOT
#ifdef FEATURE_NAS_GW
                /* Determine if the rat needs to be stopped */
                if ((cmd_ptr->cmd.mmr_plmn_search_req.rat == SYS_RAT_GSM_RADIO_ACCESS)||                                  
                    (mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_plmn_search_req.rat)&&
        ...
        case MMR_REG_REQ:
          ...
          */
          if(!(mm_checkif_rat_pri_list_has_lte_rat(&cmd_ptr->cmd.mmr_reg_req.rat_pri_list_info)) 
			  ||
...
#ifdef FEATURE_CIOT
               /* Check if LTE flavor has changed */
               || mm_check_if_lte_rat_changed(cmd_ptr->cmd.mmr_reg_req.rat)
#endif
              ) 
            {
          ...
          {
            if(mm_is_rat_lte(cmd_ptr->cmd.mmr_reg_req.rat)) //if requested RAT is LTE
            {
              /**********************************************
        ...
        case MMR_STOP_MODE_REQ:
            ...
            if(((cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == 
              ...
              || (cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason ==
               ...
               && (mm_is_rat_lte(mm_serving_plmn.info.active_rat) == FALSE)
               && (mm_deep_sleep_alt_scan == TRUE)
#ifdef FEATURE_NAS_GW
               && (mm_waiting_for_stop_mode_cnf==FALSE)
#endif
            ...
            || (cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == 
               ...
               || (cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason == 
                                              SYS_STOP_MODE_REASON_DUAL_SWITCH))
            {
#ifndef FEATURE_GPS_WWAN_PRIORITY_SEARCH     
          
                 MSG_HIGH_DS_0(MM_SUB,"=MM= ALT SCAN/SUSPEND_STACK. Pause all running timers (except periodic timers)"); 
                 mmcoord_pause_active_mm_timers();		  
#else
              if(cmd_ptr->cmd.mmr_stop_mode_req.stop_mode_reason != SYS_STOP_MODE_REASON_SUSPEND_STACK)
              	{
				MSG_HIGH_DS_0(MM_SUB,"=MM= ALT SCAN/SUSPEND_STACK. Pause all running timers (except periodic timers)"); 
				mmcoord_pause_active_mm_timers();
              	}
        ...
        case MMR_ENTER_PSM_MODE_REQ:
	     ...
	     if(is_umts_mode() || is_gsm_mode()
          ...
          { 
			...
			if(emm_ctrl_data_ptr->fmd_force_local_detach == TRUE) 
			{
				mm_send_mmr_enter_psm_mode_rsp(TRUE);
			}
            else 
#endif
			{
     ...
     case MS_TIMER:
       ...
       if(((cmd_ptr->cmd.timer_expiry.timer_id >= MIN_MM_TIMER && 
          ...
          (((cmd_ptr->cmd.timer_expiry.timer_id != TIMER_T3346) &&
           ...
           ((is_umts_mode() || is_gsm_mode())))) ||
           ((is_umts_mode() || is_gsm_mode()) &&(cmd_ptr->cmd.timer_expiry.timer_id == TIMER_T3247)))
       {
         mmcoord_route_mm_message( cmd_ptr,mm_emm_dispatcher_fn_ptr );        
       }
#endif
    ...
    case MS_CM_MM:
     ...
     case MS_MM_MM:
       switch(cmd_ptr->cmd.hdr.message_id)
       {
        ...
        case MM_WWAN_TECH_SUSPEND_IND:
           mmcoord_process_wwan_tech_suspend_ind(cmd_ptr);
           break;
    ...
    case MS_WMS_MM:
       ...
       {
         case MM_SMS_STATUS_CMD:
           emm_process_wms_sms_info(cmd_ptr,emm_ctrl_data_ptr_arg);
           break;
         default:
           mmcoord_route_mm_message( cmd_ptr,mm_emm_dispatcher_fn_ptr );
           break;
...
/*===========================================================================
FUNCTION  mmcoord_process_psm_set_config_params_req

DESCRIPTION: This function processes MMR_PSM_SET_CONFIG_PARAMS_REQ message
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_psm_set_config_params_req
(
  mm_cmd_type *mm_cmd
)
{
  ...
  msg_ptr = &mm_cmd->cmd.mmr_psm_set_cfg_params_req;

  mm_send_mmr_psm_set_config_params_rsp();

  MSG_HIGH_DS_5(MM_SUB, "=MM= Received MMR_PSM_SET_CONFIG_PARAMS_REQ: psm_enabled = %d, active_timer_value = %d, periodic_update_timer_value = %d, change_mask = %d, PSM NV value = %d ",
                           msg_ptr->psm_enabled, msg_ptr->active_timer_value, msg_ptr->periodic_tau_timer_value, msg_ptr->change_mask,mm_psm_r12_enabled);
  update_result = mm_update_psm_config_params(msg_ptr);

 #if defined(FEATURE_GSM_GPRS) && defined(FEATURE_NAS_GW)						   
  if(is_gsm_mode())
  {
    MSG_HIGH_DS_6(MM_SUB, "=MM= update_result = %d, gmm_state = %d,\
    gmm_substate = %d, reg_req_pended = %d, psm_set_cfg_req_status = %d,\
    reg_complete_ack = %d", update_result, gmm_state, gmm_substate, mmcoord_reg_req_pending,
    gmm_psm_timer_ctxt.psm_set_cfg_req_status, gmm_last_signalling_message_type);

    /* Trigger RAU if possible */
    if((update_result == TRUE) &&
       (gmm_psm_timer_ctxt.psm_set_cfg_req_status == NAS_PSM_CFG_REQ_NONE) &&
       (gmm_state == GMM_REGISTERED) &&
       ((gmm_substate == GMM_NORMAL_SERVICE) ||
        (gmm_substate == GMM_ATTEMPTING_TO_UPDATE) ||
        (gmm_substate == GMM_ATTEMPTING_TO_UPDATE_MM)) &&
       (mmcoord_reg_req_pending == FALSE) &&
       (sm_check_pdp_procedure_pending() == FALSE)
       )
    {
      gmm_psm_timer_ctxt.psm_set_cfg_req_status = NAS_PSM_CFG_REQ_PENDING;
    ...
    } /* Else remember to trigger RAU later */
    else if((update_result == TRUE) &&
            (gmm_state != GMM_DEREGISTERED) &&
            (gmm_state != GMM_DEREGISTERED_INITIATED))
    {
      MSG_HIGH_DS_0(MM_SUB, "=EMM= Pended psm_set_cfg_req_status");
      gmm_psm_timer_ctxt.psm_set_cfg_req_status = NAS_PSM_CFG_REQ_PENDING;
    }
  }
#endif
...
/*===========================================================================
FUNCTION  mmcoord_process_psm_get_timer_info_req

DESCRIPTION: This function processes MMR_PSM_GET_TIMER_INFO_REQ message
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_psm_get_timer_info_req(mm_cmd_type *message)
{

  boolean user_config = message->cmd.mmr_psm_get_timer_info_req.user_config;
...
/*===========================================================================
FUNCTION  mmcoord_process_enter_psm_mode_req

DESCRIPTION: This function processes MMR_ENTER_PSM_MODE_REQ message
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_enter_psm_mode_req(void)
{

  /* Save GMM/SM context only if GMM  is in Registered state and GMM substate is Normal service */
#ifdef FEATURE_NAS_GW
  if ((gmm_state == GMM_REGISTERED) &&
      (gmm_substate == GMM_NORMAL_SERVICE))
  {
    gmm_send_sm_save_ctxt_psm_req();
  }
  else
#endif
  {
...
/*===========================================================================
FUNCTION  mmcoord_process_psm_save_mmgsdi_ef_req
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_psm_save_mmgsdi_ef_req(void)
{
...
/*===========================================================================
FUNCTION  mmcoord_process_set_edrx_cfg_req

DESCRIPTION: This function processes MMR_SET_EDRX_CFG_REQ message
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_set_edrx_cfg_req
(
  mm_cmd_type *mm_cmd
)
{
  ...
  boolean update_result = FALSE;
  msg_ptr = &mm_cmd->cmd.mmr_set_edrx_cfg_req;
  ...
  EFS:gsm_edrx_enabled = %d, EFS:gsm_ptw = %d, EFS: gsm_cycle_length = %d",
  msg_ptr->change_mask, msg_ptr->rat_type, msg_ptr->edrx_enabled, 
  msg_ptr->edrx_ptw, msg_ptr->edrx_cycle_length, mm_edrx_cfg.gsm.enabled,
  mm_edrx_cfg.gsm.ptw, mm_edrx_cfg.gsm.cycle_length);

  /* Update the edrx config params if changed */
  update_result = mm_update_edrx_cfg_params(msg_ptr);
 
 #if defined(FEATURE_GSM_GPRS) && defined(FEATURE_NAS_GW)
  if(msg_ptr->rat_type == SYS_RAT_GSM_RADIO_ACCESS)
  {
    MSG_HIGH_DS_6(MM_SUB, "=MM= update_result = %d, gmm_state = %d,\
    gmm_substate = %d, reg_req_pended = %d, edrx_set_cfg_req_status = %d,\
    reg_complete_ack = %d", update_result, gmm_state, gmm_substate, mmcoord_reg_req_pending,
    gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status, gmm_last_signalling_message_type);

    /* Trigger RAU if possible */
    if((update_result == TRUE) &&
       (gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status == NAS_EDRX_CFG_REQ_NONE) &&
       (gmm_state == GMM_REGISTERED) &&
       ((gmm_substate == GMM_NORMAL_SERVICE) ||
        (gmm_substate == GMM_ATTEMPTING_TO_UPDATE) ||
        (gmm_substate == GMM_ATTEMPTING_TO_UPDATE_MM)) &&
    ...
    {
      gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status = NAS_EDRX_CFG_REQ_PENDING;
            ...
            (gmm_state != GMM_DEREGISTERED_INITIATED))
    {
      ...
      gmm_edrx_params_gsm_ctxt.edrx_cfg_req_status = NAS_EDRX_CFG_REQ_PENDING;
           ...
           (mm_is_rat_lte(msg_ptr->rat_type)) &&
           (EMM_GET_STATE() != EMM_DEREGISTERED) &&
           (EMM_GET_STATE() != EMM_DEREGISTERED_INITIATED))
  {
    MSG_HIGH_DS_0(MM_SUB, "=MM= Pended LTE eDRX_set_cfg_req");
    mm_set_emm_edrx_cfg_req_status(NAS_EDRX_CFG_REQ_PENDING);
  }
#endif
...
/*===========================================================================
FUNCTION  mmcoord_process_get_edrx_params_req

DESCRIPTION: This function processes MMR_GET_EDRX_PARAMS_REQ message
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_get_edrx_params_req
(
  mm_cmd_type *mm_cmd
)
{
  if(mm_cmd->cmd.mmr_get_edrx_params_req.rat_type == SYS_RAT_LTE_RADIO_ACCESS)
  {
    /* Send the configuration parameters from NV cache */
...
#ifdef FEATURE_CIOT
  else if(mm_cmd->cmd.mmr_get_edrx_params_req.rat_type == SYS_RAT_LTE_M1_RADIO_ACCESS)
  {
    /* Send the configuration parameters from NV cache */
  ...
  else if(mm_cmd->cmd.mmr_get_edrx_params_req.rat_type == SYS_RAT_LTE_NB1_RADIO_ACCESS)
  {
    /* Send the configuration parameters from NV cache */
...
#ifdef FEATURE_NAS_GW
  if(mm_cmd->cmd.mmr_get_edrx_params_req.rat_type == SYS_RAT_GSM_RADIO_ACCESS)
  {

    if (gmm_edrx_params_gsm_ctxt.edrx_params.enabled == TRUE)
    {
       /* Send the configuration parameters received from network in Attach/RAU Accept */
...
#endif
  {
    /* To be implemented when eDRX for UMTS RAT is supported */
  }
}

/*===========================================================================
FUNCTION  mmcoord_process_send_plmn_home_roam
...
SIDE EFFECTS
  None
===========================================================================*/
void mmcoord_process_send_plmn_home_roam
(
  mm_cmd_type *mm_cmd
)
{
    ...
    emm_ctrl_data_ptr->home_mcc = mm_cmd->cmd.mmr_send_plmn_home_roam_ind.home;
    emm_send_rrc_system_update(emm_ctrl_data_ptr);
}
...
void mmcoord_route_mm_message( mm_cmd_type *message,   mm_emm_dispatcher_fp **mm_emm_dispatcher_fn_ptr)
{
 ...
 #ifdef FEATURE_MBMS
          case RRC_MBMS_CONN_REQ_IND:
            gmm_process_rrc_mbms_conn_req_ind (message);
            break;
 #endif
...
#endif
          case MMR_PSM_SET_CONFIG_PARAMS_REQ:
            mmcoord_process_psm_set_config_params_req(message);
            break;

          case MMR_PSM_GET_TIMER_INFO_REQ:
            mmcoord_process_psm_get_timer_info_req(message);
            break;

          case MMR_ENTER_PSM_MODE_REQ:
            mmcoord_process_enter_psm_mode_req();
            break;

          case MMR_SET_EDRX_CFG_REQ:
            mmcoord_process_set_edrx_cfg_req(message);
            break;

          case MMR_GET_EDRX_PARAMS_REQ:
            mmcoord_process_get_edrx_params_req(message);
            break;
            
          case MMR_SEND_PLMN_HOME_ROAM:
            mmcoord_process_send_plmn_home_roam(message);
            break;

          case MMR_PSM_SAVE_MMGSDI_EF_REQ:
            mmcoord_process_psm_save_mmgsdi_ef_req();
            break;

          default:
            MSG_ERROR_DS( MM_SUB, "=MM= Unexpected REG cmd %d", message->cmd.hdr.message_id, 0,0 );
        }
      }
      break;
