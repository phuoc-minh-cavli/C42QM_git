...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmm_main.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
boolean mm_is_t3246_or_t3346_value_non_zero_sim[MAX_NAS_STACKS];
...
boolean mm_is_t3246_or_t3346_value_non_zero;
#endif
#endif
#ifdef FEATURE_LTE
...
#if defined FEATURE_DUAL_SIM || defined FEATURE_SGLTE
gmm_gprs_psm_timer_ctx_s_type gmm_psm_timer_ctxt_sim[MAX_NAS_STACKS];
#define gmm_psm_timer_ctxt gmm_psm_timer_ctxt_sim[mm_as_id]

#else
gmm_gprs_psm_timer_ctx_s_type gmm_psm_timer_ctxt;
#endif
...
uint64 gmm_t3312_timer_value_sim[MAX_NAS_STACKS];
...
*/
boolean gmm_attach_tx_is_acked_sim[MAX_NAS_STACKS];
...
gprs_edrx_params_s_type gmm_edrx_params_gsm_ctxt_sim[MAX_NAS_STACKS];
#define gmm_edrx_params_gsm_ctxt gmm_edrx_params_gsm_ctxt_sim[mm_as_id]

byte    gmm_acked_tx_retry_timeout_counter_sim[MAX_NAS_STACKS];
...
byte gmm_acked_tx_retry_timeout_counter;


uint64 gmm_t3312_timer_value;
...
boolean gmm_congestion_due_to_implicit_reject;
boolean gmm_responded_ps_page;
boolean gmm_low_priority_indicator;
sys_plmn_id_s_type  gmm_highest_preferred_oplmn;
#endif

boolean  gmm_cell_peo_supported;
...
** Status indication from GERAN that ATTACH Request message has been txed successfully has been received
*/
boolean gmm_attach_tx_is_acked;
...
gprs_edrx_params_s_type gmm_edrx_params_gsm_ctxt;
...
uint64 gmm_t3323_timer_value_sim[MAX_AS_IDS];
...
{
  boolean is_lte_present_in_rat_list = mm_checkif_rat_pri_list_has_lte_rat(
...
#if defined(FEATURE_LTE) && defined(FEATURE_NAS_SM_IRAT)
          if( emm_ctrl_data_ptr->TIN == NAS_MM_TIN_RAT_RELATED_TMSI)
          {
...
#endif
#if defined (DISABLE_9x05_OPTIMIZATIONS) || !defined(FEATURE_NAS_IMS_VOICE_DISABLED) || !defined(FEATURE_NAS_DISABLE_VOICE)
#ifdef FEATURE_LTE
            else if ((
#ifdef DISABLE_9x05_OPTIMIZATIONS
                 (gmm_set_drx_req_pending == TRUE) || //M_Doubt
#endif
...
The normal routing area updating procedure is initiated: in A/Gb mode, after Inter RAT handover from S1 mode or Iu mode.*/
	...
	case GMM_WAITING_PSM_RESTORE_RSP:
      break;
  ...
  if (is_gsm_mode())
  {
    if (gmm_state == GMM_REGISTERED)
    {
      ...
      if ((old_substate != GMM_NORMAL_SERVICE) && 
          (new_substate == GMM_NORMAL_SERVICE)) 
      { 
         gmm_psm_timer_ctxt.allow_psm_in_oos_ltd_srv = FALSE;

         /* If R12 PSM enabled then enter PSM if PSM conditions are satisfied.
              ** This code handles the case when PSM could not be entered previously
              **  as the UE was not in GMM-REGISTERED.NORMAL-SERVICE state */
        if(((mm_psm_ready_req_rejected == TRUE) ||
...
void gmm_initialize( void )
{
  ...
  ** ------------------------------------------------------- */
  ...
  gmm_low_priority_indicator           = FALSE;

  gmm_highest_preferred_oplmn.identity[0]= 0xFF;
  gmm_highest_preferred_oplmn.identity[1]= 0xFF;
  gmm_highest_preferred_oplmn.identity[2]= 0xFF;
#endif
#ifdef FEATURE_NAS_REL11
#ifdef FEATURE_NAS_GW
  gmm_is_lw_to_g_interrat_in_progress = FALSE;
...
/* TSTS Variables Initialization  - Start */
 ...
 /* PS Follow on Proceed flag */
  ...
  gmm_psm_timer_ctxt.allow_psm_in_oos_ltd_srv = FALSE;
  gmm_psm_timer_ctxt.t3324_expired = FALSE;
  gmm_psm_timer_ctxt.psm_active    = FALSE;

  gmm_cell_peo_supported = FALSE;
  gmm_acked_tx_retry_timeout_counter = 0;
  gmm_wwan_tech_active = TRUE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** -------------------- */
  ...
  //sm_put_cmd( sm_cmd );
      ...
      sm_process_sm_pdu(&sm_cmd->cmd.gmm_unitdata_ind);
#ifndef FEATURE_MODEM_HEAP
	  mem_free( &(tmc_heap), sm_cmd->cmd.gmm_unitdata_ind.data );
#else
	  modem_mem_free(sm_cmd->cmd.gmm_unitdata_ind.data, MODEM_MEM_CLIENT_NAS);
#endif
#ifndef FEATURE_MODEM_HEAP
...
)
{
  ...
  if(length < 2 || length > MAX_OCTETS_L3_MESSAGE)
  {
    MSG_ERROR_DS_1( MM_SUB, "=MM= Unexpected message length %u", length);
    return;
  }
  ...
  /* Append AS ID */
  {
    ...
    ** Copy the OTA message
    ** -------------------- */
    (void) memscpy((void *)incoming_sms.data, sizeof(incoming_sms.data),
        (void *)(&message->cmd.mm_data_ind.L3_data[2]),
                            length ) ;
    incoming_sms.data[length] = (byte)mm_sub_id;
  }
#else
  PUT_IMH_LEN(length, &incoming_sms.msg_head.IMH);
...
#else
	CNM_route_CNM_messages((IMH_T*)&incoming_sms);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (message->cmd.hdr.message_set)
  {
    ...
    case MS_MM_RR:
      switch (message->cmd.hdr.message_id)
      {
        ...
        case RR_PS_IMPLICIT_REJECT_IND:
          gmm_handle_extended_wait_time(message);      
          break;
    ...
    **--------------------------------------------------------------------------------------------------*/
...
#endif                
                  {
                    gmm_handle_information (message->cmd.mm_data_ind.L3_data,message->cmd.mm_data_ind.message_header, FALSE);
                  }
                  break;
          ...
          {
            if (message->cmd.mm_data_ind.message_header.message_len_lsb >= 2)
            {
