...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/msg_lib_encode_esm.c#1 $
$Author: pwbldsvc $
...
#ifdef FEATURE_CIOT
#define RELEASE_ASSISTANCE_IND_IEI 0xF
#endif
...
===========================================================================*/
void encode_esm_info_resp(lte_nas_esm_info_resp   *esm_info_resp, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  ...
  /*-------------------------------------------------------------
          Ext Protocol configuration options (optional)
  -------------------------------------------------------------*/
  esm_len = *pos;
  encode_ext_prot_conf_opt(&esm_info_resp->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len); 
}
/*===========================================================================
...
===========================================================================*/
void encode_pdn_disconnect_req(lte_nas_esm_pdn_disconnect_req   *pdn_disconnect_req, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  byte num_bits_to_write = 0;
  word esm_len;
  ...
  *pos += num_bits_to_write; 
  ...
  -------------------------------------------------------------*/
  esm_len = *pos;
  encode_ext_prot_conf_opt(&pdn_disconnect_req->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);
}
/*===========================================================================
...
===========================================================================*/
void encode_modifi_bearer_ctxt_accept(lte_nas_esm_modify_eps_bearer_ctxt_accept *modifi_bearer_ctxt_accept, byte *ota_buf, word *pos, size_t ota_buf_len)
{
  word esm_len;
  ...
  encode_ext_prot_conf_opt(&modifi_bearer_ctxt_accept->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);
}
/*===========================================================================
...
===========================================================================*/
void encode_modifi_bearer_ctxt_rej(lte_nas_esm_modify_eps_bearer_ctxt_rej *modify_bearer_ctxt_rej, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  byte num_bits_to_write = 0;
  word esm_len;
  ...
  encode_ext_prot_conf_opt(&modify_bearer_ctxt_rej->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);
}
/*===========================================================================
...
===========================================================================*/
void encode_bearer_modifi_req(lte_nas_esm_bearer_resource_modifi_req *bearer_modifi_req, byte *ota_buf, word *pos, size_t ota_buf_len)
{
  byte num_bits_to_write = 0;
  word esm_len;
  ...
  MSG_LOW_DS_0(MM_SUB, "Encoding bearer resource modification request\n");
...
#ifdef FEATURE_CIOT
    /*------------------------------------------------
           HC CONFIGURATION OPTIONS
  --------------------------------------------------*/
  if(bearer_modifi_req->hc_config.valid == TRUE)
  {
    encode_hc_config_ie(&bearer_modifi_req->hc_config,ota_buf,pos,&esm_len);
  }
#endif
  encode_ext_prot_conf_opt(&bearer_modifi_req->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);
}
/*===========================================================================
...
===========================================================================*/
void encode_bearer_alloc_req(lte_nas_esm_bearer_resource_allocation_req *bearer_alloc_req, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  byte num_bits_to_write = 0;
  word esm_len;
  emm_ctrl_data_type *emm_ctrl_data_ptr = NULL;
...
#endif
  encode_ext_prot_conf_opt(&bearer_alloc_req->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);
}
/*===========================================================================
...
===========================================================================*/
void encode_deact_bearer_context_accept(lte_nas_esm_deact_eps_bearer_context_accept *deact_bearer_context_acc, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  word esm_len;
  ...
  encode_ext_prot_conf_opt(&deact_bearer_context_acc->ext_protocol_conf_opt, ota_buf, pos, &esm_len, ota_buf_len); 
  MSG_LOW_DS_0(MM_SUB, "Deactivate bearer context accept encoding complete\n");
}
/*===========================================================================
...
===========================================================================*/
void encode_eps_bearer_context_accept(lte_nas_esm_act_dedicated_bearer_context_accept *dedicated_bearer_context_acc, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  word esm_len;
  /*------ WRITE THE LENGTH OF THE OFFSET ----*/
  esm_len = *pos;
  encode_prot_conf_opt(&dedicated_bearer_context_acc->protocol_conf_opt,ota_buf,pos,&esm_len);  
  encode_ext_prot_conf_opt(&dedicated_bearer_context_acc->ext_protocol_conf_opt, ota_buf, pos,&esm_len, ota_buf_len);  
  MSG_HIGH_DS_0(MM_SUB, "Activate dedicated bearer context accept encoding complete\n");
}
/*===========================================================================
...
===========================================================================*/
void encode_act_def_bearer_context_accept(lte_nas_esm_act_default_bearer_context_accept *act_def_bearer_context_acc, byte *ota_buf, word *pos, word *esm_container_len,size_t ota_buf_len)
{  
  /*------ WRITE THE LENGTH OF THE OFFSET ----*/
  *esm_container_len  += 3;  
  encode_prot_conf_opt(&act_def_bearer_context_acc->protocol_conf_opt,ota_buf,pos,esm_container_len);
  encode_ext_prot_conf_opt(&act_def_bearer_context_acc->ext_protocol_conf_opt, ota_buf, pos, esm_container_len, ota_buf_len); 
  MSG_HIGH_DS_0(MM_SUB, "Activate default bearer context accept encoding complete\n");
}
...
)
{
  ...
  *esm_container_len += 1;
  ...
  /* Skip encoding of PCO and subsequent IEs if PDN Connectivity Request encoding is done for UICC call control */
  if(encoding_for_uicc == FALSE)
  {
    /*------------------------------------------------
    ...
    /*------------------------------------------------
        HC CONFIGURATION OPTIONS
    --------------------------------------------------*/
    if(pdn_conn_req->hc_config.valid == TRUE)
    {
      encode_hc_config_ie(&pdn_conn_req->hc_config,ota_buf,pos,esm_container_len);
    }
#endif 
    /*------------------------------------------------
        EPCO CONFIGURATION OPTIONS
    --------------------------------------------------*/
    if (pdn_conn_req->ext_protocol_conf_opt.valid == TRUE) 
    {
      encode_ext_prot_conf_opt(&pdn_conn_req->ext_protocol_conf_opt, ota_buf, pos, esm_container_len, ota_buf_len); 
    }
  }
}
...
FUNCTION  ENCODE_ESM_DATA_TRANSPORT

DESCRIPTION
  This function encodes the ESM_DATA_TRANSPORT message
...
===========================================================================*/
void encode_esm_data_transport
(
...
)
{
  ...
  b_packw(esm_data_transport->user_data_container.container_length,ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  ...
  if(esm_data_transport->empty_user_data_container == FALSE )
  {
    memscpy((void*)(ota_buf + (*pos/8)), //Skip the 'already encoded bytes'
          ota_buf_len - (*pos/8),         
          (void*)esm_data_transport->user_data_container.container_value_ptr,
          esm_data_transport->user_data_container.container_length);
  }
  *pos += num_bits_to_write * esm_data_transport->user_data_container.container_length;
  

   /*--------------------------------
    Release assistance indication
  --------------------------------*/
  if(esm_data_transport->release_ind.valid)
  {
     /*----------------------
             IEI
    -----------------------*/
    num_bits_to_write = 4;  
    b_packb(RELEASE_ASSISTANCE_IND_IEI,ota_buf,*pos,num_bits_to_write); 
    *pos += num_bits_to_write+2;//adjust pos and skip two spare bits

    num_bits_to_write = 2;
    b_packb(esm_data_transport->release_ind.dl_data_exp,ota_buf,*pos,num_bits_to_write);//make sure dl_data_exp value should be range in 0-3
    *pos += num_bits_to_write;
  }
...
===========================================================================*/
void encode_dedicated_eps_bearer_context_rej(lte_nas_esm_act_dedicated_bearer_context_rej *dedicated_bearer_cont_rej, byte *ota_buf, word *pos,size_t ota_buf_len)
{
  byte num_bits_to_write = 0;  
  ...
  *pos += num_bits_to_write; 
  ...
  encode_ext_prot_conf_opt(&dedicated_bearer_cont_rej->ext_protocol_conf_opt, ota_buf, pos, pos, ota_buf_len);
}
...
)
{
  ...
  *pos += 8; 
  ...
  if(act_default_bc_rej_ptr->protocol_conf_opt.valid == TRUE)  
  {
    encode_ext_prot_conf_opt(&act_default_bc_rej_ptr->ext_protocol_conf_opt,
                         ota_buf,
                         pos,
                         pos,
                         ota_buf_len); 
...
lte_nas_esm_outgoing_msg_type *get_esm_container(lte_nas_outgoing_msg_type *out_buf)
{
  ...
  else if(out_buf->nas_hdr.pd == EPS_MOBILITY_MANAGEMENT_MESSAGES)
  {
    ...
    case CP_SERVICE_REQUEST:
      out_msg = (lte_nas_esm_outgoing_msg_type *)&out_buf->emm_outgoing_msg.cp_service_req.esm_msg_container;
      break;
#endif
...
)
{
  ...
  mm_check_for_null_ptr((void *)esm_outgoing_msg);
  mm_check_for_null_ptr((void *)out_buf);
  ...
  switch(esm_outgoing_msg->nas_hdr.msg_id)
  {
    ...
    case PDN_CONNECTIVITY_REQUEST:
      ...
      if(esm_outgoing_msg->pdn_connectivity_req.esm_dummy_flag == TRUE)
      {
        esm_outgoing_msg->pdn_connectivity_req.lte_nas_hdr.msg_id = ESM_DUMMY_MESSAGE;
		MSG_HIGH_DS_0(MM_SUB, "Encoding ESM dummy message\n");
        encode_nas_esm_header(ota_buf,
                              pos,
      ...
      {
        num_bits_to_write = 16;
        b_packw((byte)(((*pos - esm_len_octet)- esm_container_len_offset)/8),ota_buf,esm_container_len_offset,num_bits_to_write);
      }
      break;
    ...
    case BEARER_RESOURCE_ALLOCATION_REQUEST:
      ...
      encode_bearer_alloc_req(&esm_outgoing_msg->bearer_alloc_req,ota_buf, pos, ota_buf_len);
      break;
    case BEARER_RESOURCE_MODIFICATION_REQUEST:
      ...
      encode_bearer_modifi_req(&esm_outgoing_msg->bearer_resource_modifi_req,ota_buf, pos,ota_buf_len);
      break;  
    case MODIFY_EPS_BEARER_CONTEXT_ACCEPT:
      ...
      encode_modifi_bearer_ctxt_accept(&esm_outgoing_msg->modify_eps_bearer_ctxt_accept,ota_buf, pos,ota_buf_len);
      break;
    case MODIFY_EPS_BEARER_CONTEXT_REJECT:
      ...
      encode_modifi_bearer_ctxt_rej(&esm_outgoing_msg->modify_eps_bearer_ctxt_rej,ota_buf, pos, ota_buf_len);
      break;
    case DEACTIVATE_EPS_BEARER_CONTEXT_ACCEPT:   
      ...
      encode_deact_bearer_context_accept(&esm_outgoing_msg->deactivate_eps_bearer_context_accept,ota_buf, pos,ota_buf_len );
      break;   
    case PDN_DISCONNECT_REQUEST:   
      ...
      encode_pdn_disconnect_req(&esm_outgoing_msg->pdn_disconnect_req,ota_buf, pos,ota_buf_len );
      break;
    case ESM_INFORMATION_RESPONSE:   
      ...
      encode_esm_info_resp(&esm_outgoing_msg->esm_info_resp,ota_buf, pos,ota_buf_len );
      break;
    case ESM_STATUS:
      encode_nas_esm_header(ota_buf,
                            pos,
    ...
    case ESM_DATA_TRANSPORT:
      MSG_HIGH_DS_0(MM_SUB, "LTE_NAS_MSG_LIB: Encoding ESM_DATA_TRANSPORT");
      /*--------------------------------------------------------
                  LENGTH OF ESM CONTAINER
      ---------------------------------------------------------*/
      esm_container_len_offset = *pos;
      ...
      encode_nas_esm_header(ota_buf,
                            pos,
                            (byte)esm_outgoing_msg->esm_data_transport.lte_nas_hdr.pd,
                            esm_outgoing_msg->esm_data_transport.lte_nas_esm_hdr.bearer_id,
                            esm_outgoing_msg->esm_data_transport.lte_nas_esm_hdr.pti,
                            (byte)esm_outgoing_msg->esm_data_transport.lte_nas_hdr.msg_id);
      encode_esm_data_transport(&esm_outgoing_msg->esm_data_transport,ota_buf,ota_buf_len,pos);
      /* ----- Length of the ESM Container ----- */
      if(out_buf->nas_hdr.pd == EPS_MOBILITY_MANAGEMENT_MESSAGES)
      {
        num_bits_to_write = 16;
        b_packw((word)(((*pos - esm_len_octet)- esm_container_len_offset)/8),ota_buf,esm_container_len_offset,num_bits_to_write);
      }
      break;
      ...
      encode_nas_esm_header(ota_buf,
                            pos,
                            (byte)esm_outgoing_msg->esm_dummy_message.lte_nas_hdr.pd,
                            esm_outgoing_msg->esm_dummy_message.lte_nas_esm_hdr.bearer_id,
                            esm_outgoing_msg->esm_dummy_message.lte_nas_esm_hdr.pti,
                            (byte)esm_outgoing_msg->esm_dummy_message.lte_nas_hdr.msg_id);
      if(out_buf->nas_hdr.pd == EPS_MOBILITY_MANAGEMENT_MESSAGES)
      {
        /* ----- Length of the ESM Container ----- */
        num_bits_to_write = 16;
