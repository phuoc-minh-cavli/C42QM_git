...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_gps_lib.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $ 
...
)
{
  ...
  {
    MSG_FATAL_DS(MM_SUB,"=EMM= Illigal length returned generic nas msg",0,0,0) ;
  }
...
)
{
             ...
             i < EMM_GENERIC_NAS_TRANSPORT_MAX_DSM_ARRAY_SIZE; i++)
  {
    ...
    nas_assert_check((boolean)(dsm_arr_ptr[i] == NULL));
...
)
{
  ...
  mm_check_for_null_ptr((void *)pended_message_ptr) ; 
...
)
{
  ...
  MSG_HIGH_DS_0(MM_SUB, "=EMM= UL_GENERIC_NAS_TRANSPORT succeded");
   ...
   emm_msgr_send((msgr_hdr_s *)&ul_generic_nas_cnf, 
                 sizeof(lte_nas_emm_ul_generic_transport_cnf_type));
}
#endif
