...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_connection_handler.c#1 $
...
boolean emm_skip_ac_barring_chk(lte_rrc_est_cause_e  rrc_est_cause,emm_ctrl_data_type  *emm_ctrl_data_ptr)
{
     ...
     if(emm_ctrl_data_ptr->sib2_ac_barring_info.ac_Barring_r12_present == TRUE)
  {
       ...
       (rrc_est_cause == LTE_RRC_EST_CAUSE_MO_SIGNALING)))
    {
   ...
   else if((EMM_GET_STATE() == EMM_SERVICE_REQUEST_INITIATED) && 
      ...
      (emm_ctrl_data_ptr->service_req_cause == LTE_RRC_EST_CAUSE_MO_DATA))
   {
      skip_ac_barring_chk = TRUE;
   }
#endif
    MSG_HIGH_DS_1(MM_SUB, "EMM: Skip access class barring check %d",skip_ac_barring_chk);
   return skip_ac_barring_chk;
...
#ifdef FEATURE_CIOT
    case LTE_RRC_EST_CAUSE_MO_EXCEPTION_DATA:
     call_is_barred = (0 != (emm_ctrl_data_ptr->access_barring_flag & (byte)EMM_MO_EXCEPTION_BARRED)) ? TRUE : FALSE ;
     timer_ptr = &emm_ctrl_data_ptr->emm_access_barring_mo_excep_data_timer;
	 break;
#endif
...
)
{
  ...
  nas_assert_check((boolean)(EMM_GET_CONNECTION_STATE() == EMM_IDLE_STATE));
  actual_calltype = emm_ctrl_data_ptr->call_type;
  MSG_HIGH_DS_2(MM_SUB, "=EMM= Start RRC connection establishment with cause = %d actual_calltype =%d ",lte_rrc_est_cause,actual_calltype);
  

  if((emm_skip_ac_barring_chk(lte_rrc_est_cause,emm_ctrl_data_ptr)== FALSE)&&
  ...
  {
    emm_ctrl_data_ptr->send_on_resume = send_on_resume;
  }
#endif
...
)
{
  ...
  actual_calltype = emm_ctrl_data_ptr->call_type;
  MSG_HIGH_DS_1(MM_SUB, "=EMM= actual_calltype =%d ",actual_calltype);
  ...
  {
    rrc_est_cause = LTE_RRC_EST_CAUSE_MO_EXCEPTION_DATA;
  }
#endif

  if((emm_skip_ac_barring_chk(rrc_est_cause,emm_ctrl_data_ptr)== FALSE)&&
     (emm_connection_establishment_barred(rrc_est_cause, 
                                         &rem_value,
                                         emm_ctrl_data_ptr) == TRUE))
  {
  ...
  else if(EMM_GET_STATE() == EMM_REGISTERED_INITIATED)
  {
	...
	if (emm_up_opt_active(emm_ctrl_data_ptr) == TRUE && emm_ctrl_data_ptr->is_conn_suspended == TRUE)
	{
      emm_ctrl_data_ptr->send_on_resume = startup_message->send_on_resume;
	}
#endif
...
FUNCTION   emm_post_timer_expiry

DESCRIPTION
  This function posts barring timer expiry to emm queue
  depends on timer id
...
#else
void emm_post_timer_expiry(timer_id_T timer_id)
#endif
{
  mm_cmd_type    *cmd_ptr;
...
/*For EMM sub id and as id are same, so passing the sub id with timer expiry*/
  ...
  cmd_ptr->cmd.timer_expiry.timer_id = timer_id;
...
#endif
#ifdef FEATURE_CIOT
    case EMM_MO_EXCEPTION_BARRED:
	  MSG_HIGH_DS_0(MM_SUB, "=EMM= Barring timer for MO exception Barred has expired");
	  status = EMM_TIMER_BARRING_STATUS_STOP; 
...
#ifdef FEATURE_CIOT
      || (barring_mask ==  (dword)EMM_MO_EXCEPTION_BARRED)
#endif
    )
  {
    if(emm_ctrl_data_ptr->ongoing_detach_procedure == TRUE)
...
#if defined  FEATURE_DUAL_SIM 
      emm_post_timer_expiry(sub_id,EMM_ACCESS_BARRING_TIMER);
#else
    ...
    if (((1<<parsing_index) & rrc_call_barring_info_field) != 0)
    {
      switch (parsing_index)
      {
        ...
        case LTE_RRC_EST_CAUSE_MO_DATA:
		  ...
		  status = EMM_TIMER_BARRING_STATUS_START;
		  category = EMM_TIMER_BARRING_MO_DATA;
		  emm_send_emm_timer_barring_status_ind(status, category);
...
#ifdef FEATURE_CIOT
         case LTE_RRC_EST_CAUSE_MO_EXCEPTION_DATA:
     	  MSG_HIGH_DS_1(MM_SUB, "=EMM= Start barring timer for MO exception cause (%u msec)", barring_timer_length);
		  status = EMM_TIMER_BARRING_STATUS_START;
		  category = EMM_TIMER_BARRING_MO_EXP_DATA;
		  emm_send_emm_timer_barring_status_ind(status, category);
			 
		  emm_handle_barring_timer(&emm_ctrl_data_ptr->access_barring_flag,
				   (dword)EMM_MO_EXCEPTION_BARRED,barring_timer_length,&emm_ctrl_data_ptr->emm_access_barring_mo_excep_data_timer) ;
...
#ifdef FEATURE_CIOT
boolean emm_handle_extended_wait_time_for_cp
(
  emm_ctrl_data_type *emm_ctrl_data_ptr
)
{
 ...
 if ((emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS)&&
		emm_ctrl_data_ptr->extended_wait_time_for_cp != 0)
 {
   if(emm_bit_mask_check(emm_ctrl_data_ptr->nas_ciot_capability_config, EMM_CP_BACKOFF) == TRUE && 
   	  emm_state != EMM_REGISTERED_INITIATED)
   {
     emm_handle_timer_t3448(TRUE, (emm_ctrl_data_ptr->extended_wait_time_for_cp * 1000));
	 result = TRUE;
   }
   else
   {
	 mm_stop_timer(TIMER_T3346);
	 mm_start_timer(TIMER_T3346, (emm_ctrl_data_ptr->extended_wait_time_for_cp * 1000)); 
  ...
  {
   MSG_HIGH_DS_2(MM_SUB, "=EMM= ignore ext wait time for CP %d emm_state %d ",emm_ctrl_data_ptr->extended_wait_time_for_cp,EMM_GET_STATE());
  }
  return result;	
}
...
)
{
  ...
  /* Start timer T3346 if lower layers provided a non-zero value for extendedWaitTime,
     and (the conection establishment cause was delay tolerant access or the UE is operating in NB-S1 mode) , else ignore the
...
)
{
  ...
  emm_failure_type               emm_failure_cause;
  boolean responding_to_page = emm_ctrl_data_ptr->responding_to_page;
  ...
  switch (rrc_conn_est_cnf->est_status)
  {
    case LTE_RRC_CONN_EST_SUCCESS:
      if(emm_ctrl_data_ptr->aborting_conn == FALSE)
      {
	   ...
	   else if(emm_ctrl_data_ptr->emm_ignore_tau_req == TRUE)
	  {
...
#ifdef FEATURE_CIOT
     if(emm_ctrl_data_ptr->emm_start_mo_excp_signalling == FALSE )
#endif
     {
      ...
      *emm_ctrl_data_ptr->t3324_expired_ptr = FALSE;
      ...
      MSG_HIGH_DS_2(MM_SUB, "=EMM= RRC connection RESUME = %d SUSPENDED = %d",rrc_conn_est_cnf->resumption_success,rrc_conn_est_cnf->suspended);
      if (emm_up_opt_active(emm_ctrl_data_ptr) == TRUE && emm_ctrl_data_ptr->is_conn_suspended == TRUE)
      {
        emm_ctrl_data_ptr->is_conn_suspended = FALSE;
		emm_ctrl_data_ptr->conn_suspended_rat = SYS_RAT_NONE;
		emm_ctrl_data_ptr->drb_pending_post_resume = TRUE;
		
		if (emm_ctrl_data_ptr->send_on_resume == FALSE && rrc_conn_est_cnf->resumption_success == TRUE)
		{
		  /* 24.301:   5.3.1.3 Suspend and resume of the NAS signalling connection 
		  NOTE: If a NAS message is discarded and not sent to the network, the uplink NAS COUNT value corresponding to that message is 
		  reused for the next uplink NAS message to be sent.  */
          ...
          if( context_ptr != NULL  )
          {
            ...
            if(mmgsdi_is_test_mode_logging_enabled_for_subscription((sys_modem_as_id_e_type)mm_sub_id))
            {
              MSG_HIGH_DS_1(MM_SUB, "=EMM= Since CONN resume succedeed, roll back the UL NAS count by 1 ,over_flow_cnt = %d",over_flow_cnt);
            }
		  ...
		  // NAS will not get an L2 ACK for conenction setup message, need to remove from cache after resumption success
		  emm_del_con_setup_msg(emm_ctrl_data_ptr);
		}
    ...
    case LTE_RRC_CONN_EST_FAILURE_ACCESS_BARRED_EAB:
...
#ifdef FEATURE_CIOT
   	  MSG_HIGH_DS_1(MM_SUB,"=EMM= RRC conn still suspended %d ",rrc_conn_est_cnf->suspended);
      /* UE can still be in suspended state after resume fails */		   
	  if (emm_up_opt_active(emm_ctrl_data_ptr) == TRUE && emm_ctrl_data_ptr->is_conn_suspended == TRUE)
	  {
		emm_ctrl_data_ptr->is_conn_suspended = rrc_conn_est_cnf->suspended;
		
		if(emm_ctrl_data_ptr->is_conn_suspended == TRUE)
		{
	    emm_ctrl_data_ptr->conn_suspended_rat = emm_ctrl_data_ptr->plmn_service_state->active_rat;
		}
	  }
#endif					
	  ...
	  emm_failure_cause.deprior_started = rrc_conn_est_cnf->deprior_is_active;
...
#ifdef FEATURE_CIOT
        MSG_HIGH_DS_5(MM_SUB, "=EMM= SR failed mt_page_rach_retry_needed %d rach_retry_counter %d responding_to_page %d emm_ctrl_data_ptr->is_conn_suspended %d ds3gpp_is_ue_ciot_only() %d",emm_ctrl_data_ptr->mt_page_rach_retry_needed,emm_ctrl_data_ptr->rach_retry_counter,responding_to_page,emm_ctrl_data_ptr->is_conn_suspended,ds3gpp_is_ue_ciot_only());
        if( emm_ctrl_data_ptr->mt_page_rach_retry_needed == TRUE &&
	    rrc_conn_est_cnf->est_status == LTE_RRC_CONN_EST_FAILURE_TIMER_EXPIRED &&
	    emm_ctrl_data_ptr->rach_retry_counter == 0 &&
            responding_to_page == TRUE &&
            emm_cp_opt_active( emm_ctrl_data_ptr )== TRUE)
        {
          emm_ctrl_data_ptr->rach_retry_counter++;
          emm_ctrl_data_ptr->responding_to_page = TRUE;
          if ( emm_ctrl_data_ptr->is_conn_suspended == FALSE || ( ds3gpp_is_ue_ciot_only() == TRUE))
          {
              emm_delete_pended_message_with_msg_id(CP_SERVICE_REQUEST,emm_ctrl_data_ptr);
...
)
{
...
#endif
  boolean tau_recover_needed = FALSE;
  ...
  emm_ctrl_data_ptr->extended_wait_time = rrc_conn_rel_ind->extended_wait_time;
    ...
    {
      emm_ctrl_data_ptr->extended_wait_time_for_cp = rrc_conn_rel_ind->extended_wait_time_for_cp;
    }
    MSG_HIGH_DS_3(MM_SUB, "=EMM= extended_wait_time_for_cp in rel ind = %d, current extended_wait_time_for_cp %d tau_req_due_to_rach_fail_rlf %d",
                            rrc_conn_rel_ind->extended_wait_time_for_cp,emm_ctrl_data_ptr->extended_wait_time_for_cp,rrc_conn_rel_ind->tau_req_due_to_rach_fail_rlf);
#endif 
  MSG_HIGH_DS_3(MM_SUB, "=EMM= connection_state = %d, ext_wait_time = %d suspend = %d",
                connection_state, emm_ctrl_data_ptr->extended_wait_time, rrc_conn_rel_ind->suspended);
  ...
  emm_ctrl_data_ptr->sync_drb_status = FALSE;
   ...
   if( ds3gpp_is_ue_ciot_only() == TRUE)
   {
    tau_recover_needed = dl_data_expected;
   }
   else
   {
     tau_recover_needed = !is_user_data_pending;
   }
   MSG_HIGH_DS_2(MM_SUB,"=EMM= is_user_data_pending %d dl_data_expected %d",is_user_data_pending,dl_data_expected);
#else
...
#ifdef FEATURE_CIOT
  if (emm_up_opt_active(emm_ctrl_data_ptr) == TRUE)
  {
    emm_ctrl_data_ptr->is_conn_suspended = rrc_conn_rel_ind->suspended;
	...
	if(emm_ctrl_data_ptr->drb_pending_post_resume == TRUE)
	{
  ...
  /* 24.301 5.5.3.2.2 i - RRC connection failure and no user data pending */
		  ...
		  (rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_RLF_SEC_NOT_ACTIVE))
  {
    ...
    if(((emm_state == EMM_REGISTERED) || 
        (emm_state == EMM_SERVICE_REQUEST_INITIATED)) && 
        (tau_recover_needed == TRUE || 
         (rrc_conn_rel_ind->rel_reason == LTE_RRC_CONN_REL_RLF && 
          rrc_conn_rel_ind->tau_req_due_to_rach_fail_rlf == TRUE))&&
        (emm_ctrl_data_ptr->emm_update_status == EMM_STATUS_UPDATED))
    {
	  ...
	  {
        emm_ctrl_data_ptr->rau_pending_ll_failure = TRUE; 
	  }
#endif
#endif
    }
  ...
  if((emm_state == EMM_REGISTERED) && (tau_cause != TAU_CAUSE_INVALID))
  {
    ...
    /* At this point, there might be a pended REG request. If a deact is in progress
    ** do not execute
    */
  if ((emm_reg_container != (emm_reg_container_type)NULL)&&(emm_ctrl_data_ptr->rrc_deact_req.deact_cnf_pending == FALSE))
  {
    emm_reg_container(emm_ctrl_data_ptr) ;
  }
      ...
      (emm_state == EMM_DEREGISTERED_INITIATED)))
  {
    ...
    MSG_HIGH_DS_2(MM_SUB,"=EMM= T3412 started, %u sec: %u ms - Conn Rel", 
                  (dword)(emm_ctrl_data_ptr->t3412_value / 1000L),
                   (word)(emm_ctrl_data_ptr->t3412_value % 1000L));
  }
  else
  {
    MSG_HIGH_DS_3(MM_SUB,"=EMM= T3412 not started, %u sec: %u ms, EMM state %d - Conn Rel", 
                  (dword)(emm_ctrl_data_ptr->t3412_value / 1000L),
                  (word)(emm_ctrl_data_ptr->t3412_value % 1000L),
                  EMM_GET_STATE());
  }
