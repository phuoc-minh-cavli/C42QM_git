...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/msg_lib_encode_common_emm_ie.c#1 $
$Author: pwbldsvc $
...
void encode_ue_nw_capability_ie(lte_nas_emm_ue_capability_type *ue_capability,word *pos,byte *ota_buf)
{
  ...
  if(emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS ||
  	 emm_bit_mask_check(emm_ctrl_data_ptr->nas_ciot_capability_config, EMM_CE_RESTRICTION) == TRUE)
  {
    len = 7;
  }
#ifndef FEATURE_CATM1_DISABLE
  if(emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_M1_RADIO_ACCESS &&
  	 emm_bit_mask_check(emm_ctrl_data_ptr->nas_ciot_capability_config, EMM_UP_CIOT) == TRUE)
  {
	len = 6;
  }
#endif
  ...
  for(octet = 1; octet <= len; octet ++)
  {
    switch(octet)
    {
      ...
      *pos += num_bits_to_write;
      ...
      {   
        num_bits_to_write =1;
#if defined (FEATURE_LTE_REL10) && !defined(FEATURE_NAS_DISABLE_VOICE)
         /*ACC-CSFB*/
    ...
    case 6:
      ...
      {
      num_bits_to_write = 1;
      b_packb((byte)ue_capability->epco_ciot,ota_buf,*pos,num_bits_to_write);
     *pos += num_bits_to_write;
      b_packb(ue_capability->hc_cp_ciot,ota_buf,*pos,num_bits_to_write);
     *pos += num_bits_to_write;

      b_packb(ue_capability->er_without_pdn,ota_buf,*pos,num_bits_to_write);
      *pos += num_bits_to_write;

      b_packb(ue_capability->s1_u,ota_buf,*pos,num_bits_to_write);
      *pos += num_bits_to_write;

      b_packb(ue_capability->up_ciot,ota_buf,*pos,num_bits_to_write);
      *pos += num_bits_to_write;

      b_packb(ue_capability->cp_ciot,ota_buf,*pos,num_bits_to_write);
      *pos += num_bits_to_write;
      ...
      case 7:
        ...
        if (emm_ctrl_data_ptr->plmn_service_state->active_rat == SYS_RAT_LTE_NB1_RADIO_ACCESS )
        {
          ...
          *pos += num_bits_to_write;
          ...
          num_bits_to_write = 1;
          b_packb(ue_capability->cp_backoff,ota_buf,*pos,num_bits_to_write);//CP backoff
          *pos += num_bits_to_write;

          num_bits_to_write = 1;
          b_packb(ue_capability->ce_restriction,ota_buf,*pos,num_bits_to_write);//CE restriction
          *pos += num_bits_to_write;


          b_packb((byte)0,ota_buf,*pos,num_bits_to_write);//skip other byte
		  *pos += num_bits_to_write;
          if((nb1_rrc_cap_multi_drb_is_supported() == TRUE) && 
             (emm_bit_mask_check(emm_ctrl_data_ptr->nas_ciot_capability_config, EMM_S1_U) == TRUE))
          {
            b_packb((byte)1,ota_buf,*pos,num_bits_to_write);//support multiple DRB
          }
          else
          {
            b_packb((byte)0,ota_buf,*pos,num_bits_to_write);//skip multiple DRB
          }
          *pos += num_bits_to_write;
        }
	else
