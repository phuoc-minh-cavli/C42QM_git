...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmmmsg.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
extern byte gmm_encode_common_ie_message
(
  byte                              *data,
  byte                              ra_update_type,
  byte                              msg_index,
  byte                              max_index_size,
  boolean                           is_lte_supported
);
...
#ifdef FEATURE_MBMS
/*===========================================================================
FUNCTION GET_MBMS_CONTEXT_STATUS

DESCRIPTION
  This function creates MBMS Context Status indicating which NSAPIs are in use
  by SM.
...
RETURN VALUE
  MBMS Context Status indicating which NSAPIs are currently in use.

SIDE EFFECTS
  None

===========================================================================*/
static gmm_ie_mbms_context_status_T get_mbms_context_status(void)
{
  gmm_ie_mbms_context_status_T mbms_cxt_status;

  mbms_cxt_status.length = sm_pack_mbms_context_status( mbms_cxt_status.value );

  return mbms_cxt_status;
}

#endif
/*===========================================================================
FUNCTION is_lte_fdd_supported

DESCRIPTION
  This function determines if LTE FDD is supported

DEPENDENCIES
  None

RETURN VALUE
  TRUE is LTE FDD is supported

SIDE EFFECTS
  None

===========================================================================*/
static boolean is_lte_fdd_supported (void)
{
    return ((mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, SYS_RAT_LTE_RADIO_ACCESS) &&
   ...
   || (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, SYS_RAT_LTE_M1_RADIO_ACCESS) &&
             lte_rrc_lte_fdd_is_supported(mm_get_lte_band_capacity(&mm_rat_pri_list_info)) 
    #ifdef FEATURE_SGLTE
             && !MM_IS_IN_SGLTE_STATE_4
    #endif
    ...
    || (mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, SYS_RAT_LTE_NB1_RADIO_ACCESS) &&
             lte_rrc_lte_fdd_is_supported(mm_get_lte_band_capacity(&mm_rat_pri_list_info)) 
    #ifdef FEATURE_SGLTE
             && !MM_IS_IN_SGLTE_STATE_4
    #endif
...
/*===========================================================================
FUNCTION is_lte_tdd_supported

DESCRIPTION
  This function determines if LTE TDD is supported
...
static boolean is_lte_tdd_supported (void)
{
    return ((mm_checkif_rat_pri_list_has_rat(&mm_rat_pri_list_info, SYS_RAT_LTE_RADIO_ACCESS) &&
...
#else
      /* the rf_power_capability in the hw_classmark should be used */
      /* irrespective of whether wcdma is being used or not         */

      /* If Multiband is not enabled, the Access tech is set to PGSM,
      ** For PGSM, set the power class to 4, will fix this once we address
      ** the multiband support
      */
      hw_classmark->current_rf_power_capability = 0x4;

      rf_power_capability = hw_classmark->current_rf_power_capability & 0x07;
...
#if defined(FEATURE_NAS_REL12) && defined(FEATURE_REL_12_MSRAC_CHANGE)

    /*  Spec 24.008, Spec CRs 2343, 2547, 2630, 2709, 2775
     *
     */
...
NAS_EXIT_CRIT_SECT(mm_rat_prioritylist_crit_sec);
   ...
   /* If both "E-UTRA FDD support" and "E-UTRA TDD support" bits are set to '0', 
    ...
    csn_encode_bits(encoder, 2, mm_nv_tighter_capability);
#if defined(FEATURE_NAS_REL12) && defined(FEATURE_REL_12_MSRAC_CHANGE)
    /*  Spec 24.008, Spec CRs 2343, 2547, 2630
     *
     */
...
#else /* #if defined(FEATURE_GSM) && defined(FEATURE_NAS_GW) */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  byte   max_data_size =   sizeof(gmm_ie_LV_ms_network_caps_T)
                ...
                + 3  /*T3324 value*/
                + 3  /*T3312 extended value*/
                + 3;  /*Extended DRX parameters*/
#else                
                + 1  /*Network support feature */
                + 3  /*T3324 value*/
                + 3  /*T3312 extended value*/
...
#if defined(FEATURE_LTE)
  is_lte_supported = (mm_checkif_rat_pri_list_has_lte_rat(&mm_rat_pri_list_info)
#ifdef FEATURE_SGLTE
                        && !MM_IS_IN_SGLTE_STATE_4
#endif
                     );
 ...
 */
  ...
  ** nor equivalent PLMN then include IMSI in the Mobile identity IE
  ** ------------------------------------------------------------------------- */

  if (ptmsi_present &&
      (mm_check_attach_with_imsi(MM_PS_DOMAIN) == FALSE))
  {
    /* -----------------------------------------------------------------------------
  ...
  ** ----------------------------------------- */
  ...
  msg_index =  gmm_encode_common_ie_message (&attach_request.data[0], attach_type, msg_index, max_data_size, is_lte_supported);
...
byte gmm_encode_common_ie_message
(
  byte                              *data,
  byte                              ra_update_type,
  byte                              msg_index,
...
/* ------------------------------------
  ...
  // if (get_revision_level_indicator()) 
    ...
    ** ---------------------------------------------------------------------- */
          ...
          emm_ctrl_data_ptr->emm_guti_valid == TRUE ))
    {
      ...
      {
        data[msg_index] = GMM_IEI_PTMSI_SIGNATURE;
      }
      msg_index = msg_index + 1;
      for (i=0; i < PTMSI_SIGNATURE_SIZE; i++)
      {
        ...
        {
          data[msg_index+i] = mm_mapped_gw_identity.ptmsi_signature.value[i];
        }
      } /* end for (i=0; i < PTMSI_SIGNATURE_SIZE; i++) */      
...
#endif
    {
      ...
      if (ptmsi_signature_present)
      {
        ...
        for (i=0; i < PTMSI_SIGNATURE_SIZE; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = gmm_stored_ptmsi_signature.value[i];
          }
        } /* end for (i=0; i < PTMSI_SIGNATURE_SIZE; i++) */
  ...
  ** ------------------------------------------------------ */
  if (is_gsm_mode() && (mm_psm_r12_enabled == TRUE))
  {
    if(msg_index < max_index_size)
    {
      data[msg_index] = GMM_IEI_READY_TIMER_VALUE; /* IEI on first byte */
    }
    msg_index = msg_index + 1;
  
    if(msg_index < max_index_size) 
    {
      data[msg_index] = gmm_encode_gprs_timer_unit_and_value(mm_psm_gprs_ready_timer, 
                                                 GPRS_TIMER);
    }
    msg_index = msg_index + 1;
  }
  else
  {
    ...
    {
      set_requested_ready_timer( &data[msg_index], &msg_index );
    }
  }
...
#endif
    {
            /*  If eDRX has been negotiated and MS supports PEO then the mobile station shall not request 
          **   values for the SPLIT_PG_CYCLE or the NON_DRX_TIMER parameter.
            */
      if (!(is_gsm_mode() && 
           (gmm_cell_peo_supported) &&
           ...
           (gmm_edrx_params_gsm_ctxt.edrx_params.cycle_length != NAS_INVALID_EDRX_VALUE )))
      {
        ...
        {
          data[msg_index]   = GMM_IEI_DRX_PARAMETER;
          data[msg_index+1] = drx_parameter.value[0];
          data[msg_index+2] = drx_parameter.value[1];
        }
  ...
  ** --------------------- */
  if (((gmm_state != GMM_REGISTERED_INITIATED) &&
      ...
      ((gmm_state == GMM_REGISTERED_INITIATED) &&
       ((ra_update_type & 0x07) == GMM_COMBINED_GPRS_IMSI_ATTACH)))
  {
    tmsi_present = FALSE;
    ...
    if (!tmsi_present)
    {
      if(msg_index < max_index_size)
      {
        data[msg_index] = (GMM_IEI_TMSI_STATUS << 4) | tmsi_present;
      }
      msg_index = msg_index + 1;
    }
  }
...
#endif
  {
        ...
        (gmm_state != GMM_REGISTERED_INITIATED))
    {
      ...
      ** ---------------- */
      if(msg_index+1 < max_index_size)
      {
        data[msg_index]   = GMM_IEI_PTMSI;
        data[msg_index+1] = TMSI_SIZE + 1;
      }
      ...
      ** ------------------------------------------------ */
            ...
            emm_ctrl_data_ptr->emm_guti_valid == TRUE ))
      {
        for (i=0; i < PTMSI_SIZE; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = mm_mapped_gw_identity.ptmsi.digit[i];
          }
        }
      }     
      else
#endif
      {
        for (i=0; i < PTMSI_SIZE; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = gmm_stored_ptmsi.digit[i];
          }
        }
      }
      msg_index = msg_index + PTMSI_SIZE;
    ...
    if (gmm_state != GMM_REGISTERED_INITIATED)
    {
      ...
      ** ------------------------------- */
      ...
      if (msg_index+1 < max_index_size)
      {
        data[msg_index]   = GMM_IEI_MS_NETWORK_CAPABILITY;
        data[msg_index+1] = ms_network_capability.length;
      }
      msg_index = msg_index + 2;
      ...
      for (i=0; i < ms_network_capability.length; i++)
      {
        if((msg_index+i) < max_index_size)
        {
          data[msg_index+i] = ms_network_capability.value[i];
        }
      }
      msg_index = msg_index + ms_network_capability.length;
      ...
      if (msg_index+1 < max_index_size)
      {
        data[msg_index]   = GMM_IEI_PDP_CONTEXT_STATUS;
        data[msg_index+1] = pdp_context_status.length;
      }
      msg_index = msg_index + 2;
      ...
      for (i=0; i < pdp_context_status.length; i++)
      {
        if((msg_index+i) < max_index_size)
        {
          data[msg_index+i] = pdp_context_status.value[i];
        }
      }
      msg_index = msg_index + pdp_context_status.length;
#ifdef FEATURE_MBMS    
      //Pack the MBMS Status IE        
      mbms_context_status = get_mbms_context_status();
    
      if (mbms_context_status.length > 0)
      {
        if(msg_index+1 < max_index_size)
        {
          data[msg_index] = GMM_IEI_MBMS_CONTEXT_STATUS;
          data[msg_index+1] = mbms_context_status.length;
        }
        msg_index = msg_index + 2;
        for (i=0; i < mbms_context_status.length; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = mbms_context_status.value[i];
          }
        }
        msg_index = msg_index + mbms_context_status.length;
      }
#endif
    }
#if defined(FEATURE_LTE)
    ...
    ** ---------------------  */
         ...
         ((ra_update_type & 0x07) != GMM_PERIODIC_UPDATING)) ) )
    {
      ...
      if(msg_index + 1 < max_index_size)
      {
        data[msg_index] = GMM_IEI_UE_NW_CAPABILITY;
      }
      msg_index += 1;
      if(msg_index + MAX_SIZE_OF_UE_NE_CAPABILITY_IE < max_index_size)
      {
        encode_ue_nw_cap_ie(&ue_capability,&msg_index,data);
      }
    }
         ...
         emm_ctrl_data_ptr->emm_guti_valid == TRUE))
    {
      ...
      if(mm_has_valid_ptmsi() && has_valid_rai())
      {
        ...
        ** ---------------- */
        if(msg_index+1 < max_index_size)
        {
          data[msg_index]   = GMM_ADDITIONAL_MOBILE_IDENTITY;
          data[msg_index+1] = TMSI_SIZE + 1;
        }
        ...
        ** ------------------------------------------------ */
        ...
        if(msg_index < max_index_size)
        {
          data[msg_index] = GMM_ADDITIONAL_OLD_RAI;
        }
        msg_index = msg_index + 1;
        ...
        if(msg_index < max_index_size)
        {
          data[msg_index] = PLMN_SIZE + LAC_SIZE + sizeof(routing_area_code_T);
        }
        msg_index = msg_index + 1;
        ...
        for (i=0; i < PLMN_SIZE; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = gmm_stored_rai.plmn_id.identity[i];
          }  
        }
        msg_index = msg_index + PLMN_SIZE; 
        ...
        for (i=0; i < LAC_SIZE; i++)
        {
          if((msg_index+i) < max_index_size)
          {
            data[msg_index+i] = gmm_stored_rai.location_area_code.lac[i];
          }  
        }
        msg_index = msg_index + LAC_SIZE; 
        ...
        if(msg_index < max_index_size)
        {
          data[msg_index] = gmm_stored_rai.routing_area_code;
        }        
        msg_index = msg_index + 1; 
      }
    }
    ...
    if((emm_ctrl_data_ptr->emm_3gpp_spec_ver >= LTE_3GPP_REL9 ) )
    {
          ...
          (mmcoord_pending_reg_message.cmd.mmr_reg_req.lte_cs_domain_param.is_voice_domain_ie_valid == TRUE))
      {
        if(msg_index+2 < max_index_size)
        {
          data[msg_index]   = GMM_VOICE_DOMAIN_PREF_IEI;
          data[msg_index+1] = 1; //length of the VOICE DOMAIN PREF IE
          data[msg_index+2] = 
    ...
    *-----------------------------*/
        ...
        (gmm_state != GMM_REGISTERED_INITIATED))
    {
      if(msg_index < max_index_size)
      {
        data[msg_index] = (byte) GMM_IEI_PTMSI_TYPE | ptmsi_type_iei_value;
      }        
      msg_index = msg_index + 1; 
    }
#endif
    ...
    ** ------------------------------------------------ */ 
        ...
        (reg_sim_per_subs_chk_low_pri_cfg((sys_modem_as_id_e_type)mm_sub_id) == TRUE))
    {
      if(msg_index < max_index_size)
      {
        data[msg_index] = (byte)(GMM_IEI_DEVICE_PROPERTIES) | gmm_low_priority_indicator;
      }
      msg_index = msg_index + 1;
    }
    ...
    ** ------------------------------------------------ */
    if(mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL10)
    {
      if(msg_index < max_index_size)
      {
        data[msg_index] = (byte)(GMM_IEI_UL_NW_FTR_SUPPORT) | 0x01;
      }
      msg_index = msg_index + 1;
    }
#endif
    ...
    **-------------------------------------------*/
    if((mm_nv_exclude_old_lai_type_ie != TRUE) &&
       (((gmm_state != GMM_REGISTERED_INITIATED) &&
        ...
        ((gmm_state == GMM_REGISTERED_INITIATED) &&
         (((ra_update_type & 0x07) == GMM_COMBINED_GPRS_IMSI_ATTACH) || 
           ((ra_update_type & 0x07) == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED)))) &&
         mm_has_valid_lai())
    {
      if(msg_index < max_index_size)
      {
         data[msg_index] = GMM_IEI_OLD_LAI;
      }
      msg_index = msg_index + 1;    
      
      if(msg_index < max_index_size)
      {
         data[msg_index] = LAI_SIZE;
      }
      msg_index = msg_index + 1;
      ...
      for (i=0; i < LAI_SIZE; i++)
      {
        if((msg_index+i) < max_index_size)
        {
           data[msg_index+i] = mm_location_information.lai[i];
        }  
      }
 ...
 /***Spec 24.008 4.7.5.2.1:
     ...
     **====================================================================================**/
      ...
      if(mm_nv_nas_rel_compliance >= NV_NAS_COMPLIANCE_REL11)
      {
              ...
              (gmm_attach_type == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED)))))
        {
          if(msg_index < max_index_size)
          {
            data[msg_index] = GMM_IEI_TMSI_BASED_NRI_CONTAINER;// IEI on first byte
          }
          msg_index = msg_index + 1;
          if(msg_index < max_index_size) 
          {
            data[msg_index] = GMM_IEI_TMSI_BASED_NRI_CONTAINER_CONTENT_LENGTH;//NRI_CONTENT_LENGTH
          }
          msg_index = msg_index + 1;
          if(msg_index < max_index_size)
          {
            data[msg_index] = mm_location_information.tmsi[1];
          // bit 23 to 16 of tmsi
          }
          msg_index = msg_index + 1;
          if(msg_index < max_index_size)
          {
            data[msg_index] = (mm_location_information.tmsi[2] & 0xC0);
  ...
  {
    /* ------------------------------------------------------
    ** T3324 active timer value
    ** ------------------------------------------------------ */

    if(msg_index < max_index_size)
    {
      data[msg_index] = GMM_IEI_T3324_VALUE; /* IEI on first byte */
    }
    msg_index = msg_index + 1;

    if(msg_index < max_index_size) 
    {
      data[msg_index] = 1; /*T3324_CONTENT_LENGTH */
    }
    msg_index = msg_index + 1;
    if(msg_index < max_index_size)
    {
      data[msg_index] = gmm_encode_gprs_timer_unit_and_value(mm_psm_active_timer, 
                                                                               GPRS_TIMER_2);
    }
    msg_index = msg_index + 1;
    ...
    ** ------------------------------------------------------ */
    ...
    if(msg_index < max_index_size)
    {
      data[msg_index] = GMM_IEI_EX_T3312_VALUE; /* IEI on first byte */
    }
    msg_index = msg_index + 1;
    if(msg_index < max_index_size) 
    {
      data[msg_index] = 1; /* T3312_EXT_CONTENT_LENGTH */
    }
    msg_index = msg_index + 1;
    if(msg_index < max_index_size)
    {
      data[msg_index] = gmm_encode_gprs_timer_unit_and_value(mm_psm_periodic_rau_update_timer,GPRS_TIMER_3);
    }
    msg_index = msg_index + 1;
  }
  /* ------------------------------------------------------
   ...
   ** ------------------------------------------------------ */
    if (gmm_edrx_params_gsm_ctxt.edrx_params_to_nw.enabled)
    {
      if(msg_index < max_index_size)
      {
        data[msg_index] = GMM_IEI_EXT_DRX_PARAMS_VALUE;
      }
      msg_index = msg_index + 1;
      if(msg_index < max_index_size) 
      {
        data[msg_index] = GMM_EXT_DRX_PARAMS_CONTENT_LENGTH;
      }
      msg_index = msg_index + 1;

      if(msg_index < max_index_size)
      {
        data[msg_index] = (byte)mm_edrx_cfg.gsm.cycle_length; 
      }
      msg_index = msg_index + 1;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** query PS again */
  ...
  if (!data_is_available)
  {
    ...
    data_is_available = ds3gpp_is_ps_data_available();
#endif
  }
  ...
  ** ----------------------------------------- */
  ...
  msg_index =  gmm_encode_common_ie_message (&ra_update_request.data[0], ra_update_type, msg_index, max_index_size, is_lte_supported);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ** ---------------------- */
  if ((gmm_service_request_type != GMM_SIGNALLING) &&
      ...
      #ifdef FEATURE_MBMS
      &&
      (gmm_service_request_type != (byte)GMM_MBMS_MULTICAST_SERVICE_RECEPTION) &&
      (gmm_service_request_type != (byte)GMM_MBMS_BROADCAST_SERVICE_RECEPTION) 
      #endif
      )
  {
  ...
  ** ------------------------------------------------ */
  ...
  if (mbms_context_status.length > 0)
  {
    if(msg_index+1 < max_data_size)
    {
      service_request_msg.data[msg_index++] = GMM_IEI_MBMS_CONTEXT_STATUS;    
      service_request_msg.data[msg_index++] = mbms_context_status.length; 
    }
    else
    {
      MSG_ERROR_DS_0(MM_SUB,"=MM= Index out of bound and buffer overflow");
    ...
    for (i=0; i < mbms_context_status.length; i++)
    {
      if(msg_index < max_data_size)
      {
        service_request_msg.data[msg_index++] = mbms_context_status.value[i];
      }          
      else
      {
        MSG_ERROR_DS_0(MM_SUB,"=MM= Index out of bound and buffer overflow");
  ...
  ** ------------------------------------------------ */ 
      ...
      (reg_sim_per_subs_chk_low_pri_cfg((sys_modem_as_id_e_type)mm_sub_id) == TRUE))
  {
    if(msg_index < max_data_size)
    {
      service_request_msg.data[msg_index] = (byte)(GMM_IEI_DEVICE_PROPERTIES) | gmm_low_priority_indicator;
    }
    msg_index = msg_index + 1;
  }
#endif
...
#endif
  attach_accept_msg->t3324_value_present     = FALSE;
  attach_accept_msg->edrx_params_present     = FALSE;
      ...
      ** --------------------------------------------------------------- */
      ...
      case IEI_EMERGENCY_NUM_LIST:
        ...
        if ((optional_ie_index <= 9) && (emergency_list_length > 0)&&
            ((msg_index + emergency_list_length + 2) <= msg_length))
        {
      ...
      case GMM_IEI_T3323_VALUE:
        ...
        case GMM_IEI_T3324_VALUE:
            ...
            ((msg_index + sizeof( gmm_ie_TLV_timer_value_T )) <= msg_length))
         {
           ...
           attach_accept_msg->t3324_value_present = TRUE;
           optional_ie_index = 16;
         }
         msg_index = msg_index + sizeof( gmm_ie_TLV_timer_value_T );
        break;

      case GMM_IEI_EXT_DRX_PARAMS_VALUE:
        if ((optional_ie_index <= 16) &&
            ((msg_index + sizeof( gmm_ie_TLV_edrx_params_T )) <= msg_length))
        {
          if (msg_index+2 < gmm_max_octet_len)
          {
          ...
          attach_accept_msg->edrx_params_present = TRUE;
          optional_ie_index = 17;
        }
        msg_index = msg_index + sizeof( gmm_ie_TLV_edrx_params_T );
        break;
        ...
        else
        {
          ...
          ** ------------------------------------------------------------ */
          ...
          else
          {
            ...
            ** A 0 valued bit 8 indicates that the following octet is a length octet
            ** --------------------------------------------------------------------- */
            if( msg_index+1 < MIN(msg_length,gmm_max_octet_length))
            {
              msg_index =
                msg_index + 2 + message->cmd.mm_data_ind.L3_data[msg_index+1];
            }
      ...
      ** --------------------------------------------------------------- */
      case GMM_IEI_PTMSI_SIGNATURE:
            ...
            ((msg_index + sizeof( gmm_ie_TV_ptmsi_signature_T )) <= msg_length))
        {
          for (i=0; i < PTMSI_SIGNATURE_SIZE; i++)
          {
            if(msg_index+1+i < MIN(msg_length,max_L3_length ))
            {
        ...
        else
        {
          ...
          ** ------------------------------------------------------------ */
          ...
          else
          {
            ...
            ** A 0 valued bit 8 indicates that the following octet is a length octet
            ** --------------------------------------------------------------------- */
            if( msg_index+1 <MIN( msg_length,max_L3_length ))
            {
              msg_index =
                msg_index + 2 + message->cmd.mm_data_ind.L3_data[msg_index+1];
            }
...
#endif
  ra_update_accept_msg->t3324_value_present = FALSE;
  ra_update_accept_msg->edrx_params_present = FALSE;
      ...
      ** --------------------------------------------------------------- */
      ...
      case IEI_EMERGENCY_NUM_LIST:
        ...
        if ((optional_ie_index <= 11) && (emergency_list_length > 0)&&
            ((msg_index + 2 + emergency_list_length) <= msg_length))
        {
      ...
      case GMM_IEI_T3324_VALUE:    
            ...
            ((msg_index + sizeof( gmm_ie_TLV_timer_value_T )) <= msg_length))
         {
           ...
           ra_update_accept_msg->t3324_value_present = TRUE;
           optional_ie_index = 19;
         }
         msg_index = msg_index + sizeof( gmm_ie_TLV_timer_value_T );
        break;

      case GMM_IEI_EXT_DRX_PARAMS_VALUE:
        if ((optional_ie_index <= 19) &&
             ((msg_index + sizeof( gmm_ie_TLV_edrx_params_T )) <= msg_length))
        {
          if (msg_index+2 < max_L3_length)
          {
          ...
          ra_update_accept_msg->edrx_params_present = TRUE;
          optional_ie_index = 20;
        }
        msg_index = msg_index + sizeof( gmm_ie_TLV_edrx_params_T );
        break;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (valid_msg)
  {
    ...
    if(msg_index < MIN(msg_length,gmm_max_octet_length))
    {
      ...
      {
        if( msg_index+2 < MIN(msg_length,gmm_max_octet_length))
        {
...
)
{
  ...
  /* Spec 24.008, Spec CR 2589, Section 4.7.7.2,
   * In Iu mode, an MS not capable of A/Gb mode shall ignore the Ciphering Algorithm IE 
   * in the AUTHENTICATION_AND_CIPHERING REQUEST message.
   */
  
  if (mm_gsm_supported && !is_req_algo_supported)
#else
...
boolean gmm_is_periodic_rau_accept_msg_valid( mm_cmd_type  *message)
{
   ...
   if(valid_msg)
   {
     if(msg_index < MIN(msg_length,max_L3_length))
     {
   ...
   if(valid_msg)
   {
     if((msg_index < MIN(msg_length,max_L3_length)) && (message->cmd.mm_data_ind.L3_data[msg_index] == GMM_IEI_PTMSI))
     {
       if(msg_index+2 < MIN(msg_length,max_L3_length)) 
       {
