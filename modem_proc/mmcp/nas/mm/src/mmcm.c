...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmcm.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   else if (is_OTA_message( msg_p ))
   {
      ...
      {
#ifdef FEATURE_NAS_DISABLE_VOICE
        //rejecting any incoming OTA for Voice call 
        if((byte)GET_PD( msg_p->cmd.rr_data_ind.layer3_message[0]) == PD_CC || 
           (byte)GET_PD( msg_p->cmd.rr_data_ind.layer3_message[0]) == PD_SS)
        {
          return ;
        }
#endif
         ...
         /* Append AS ID */
         {
            ...
            if(length < MAX_OCTETS_L3_MESSAGE)
            {
...
#else
	CNM_route_CNM_messages((IMH_T*)&msg_p->cmd);
#endif
