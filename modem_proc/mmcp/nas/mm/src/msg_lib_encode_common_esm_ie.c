...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/msg_lib_encode_common_esm_ie.c#1 $
$Author: pwbldsvc $
...
#define EXT_PROTOCOL_CONF_OPTIONS_IEI 0x7B
...
void encode_esm_tft(tft_type_T *tft, byte *ota_buf, word *pos, word *esm_container_len)
{
  ...
  if(tft->valid == TRUE)
  {
    ...
    {
      MSG_ERROR_DS_1(MM_SUB, "=NAS_MSG_LIB= received TFT length = %d, truncating this to 254",
    ...
    *esm_container_len += num_bits_to_write;
    ...
    {
      MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= received invalid Operation Code for encoding");
    }                        
    ...
    case 4: /*Replace packet filters in existing TFT*/    
                    ...
                    loop < MAX_PACKET_FILTERS; loop++)
      {
        ...
        { 
          MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= received TFT length does not match with the parameters in TFT, skipping the rest of TFT encoding");
          return;
        }
    ...
    case 5: /*Delete Packet filter from existing TFT*/
      ...
      if(tft->num_filters != 0)
      {
        MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= received DELETE_PKT_FILTER_FROM_EXISTING_TFT w/o pkt filter");
      }
      for(loop = 0; loop < tft->num_filters &&
                    loop < MAX_PACKET_FILTERS; loop++)
      {
      ...
      break;
    case 6: /*NO_TFT*/    
      MSG_HIGH_DS_0(MM_SUB, "=NAS_MSG_LIB= No TFT Packet Filter");
      break;
    default:
      MSG_FATAL_DS(MM_SUB,"=NAS_MSG_LIB= received invalid TFT Operation code",0,0,0);
      break;
    }
    ...
    if(tft->e_bit)
    {
      ...
      while(tft_length > (3*sizeof(byte))) /*If length is greater than 3 octets*/
      {
        ...
        {
          MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= TFT length does not match with the contents length, skipping TF encoding");
          return;
        }        
        ...
        *esm_container_len += num_bits_to_write;
        ...
        if(tft_length > tft->param_list[loop].content_length)
        {
          for(index=0;index < tft->param_list[loop].content_length; index++)
          {
            ...
            { 
              MSG_ERROR_DS_0(MM_SUB, "=NAS_MSG_LIB= there was an error while encoding TFT PARAMETER LIST, skipping the rest of TFT encoding");
              return;
            }
          }
        }
...
void encode_prot_conf_opt(protocol_cfg_options_T *prot_conf_opt, byte *ota_buf, word *pos, word *esm_container_len)
{
  ...
  if(prot_conf_opt->valid == TRUE)
  {
    ...
    {
      MSG_ERROR_DS_2(MM_SUB,"=NAS_MSG_LIB= Number of protocol ids = %d. Max allowed = %d. Skipping PCO encoding.",
    ...
    for(loop=0;loop<(prot_conf_opt->num_protocol_ids);loop++)
    {
      ...
      {
        MSG_ERROR_DS_3(MM_SUB,"=NAS_MSG_LIB= Number of protocol contents bytes in protocol id #%d = %d. Max allowed = %d. Skipping PCO encoding.",
                       (loop + 1),
...
FUNCTION  ENCODE_EXT_PROT_CONF_OPT

DESCRIPTION
  This function encodes the EXT_PROTOCOL_CONFIGURATION_OPTIONS information Element
...
===========================================================================*/
void encode_ext_prot_conf_opt(ext_protocol_cfg_options_T *ext_prot_conf_opt, byte *ota_buf, word *pos, word *esm_container_len,size_t ota_buf_len)
{
  byte num_bits_to_write = 0;

  if((ext_prot_conf_opt->valid == TRUE) &&
     (ext_prot_conf_opt->content_ptr != NULL))
  {
    num_bits_to_write = 8;

    b_packb((byte)EXT_PROTOCOL_CONF_OPTIONS_IEI,ota_buf,*pos,num_bits_to_write);
    *pos += num_bits_to_write; 
    *esm_container_len += 1;

    /* ---- Length of EXT_PROTOCOL_CONFIGURATION_OPTIONS ---- */
    num_bits_to_write = 16;
    b_packw(ext_prot_conf_opt->len,ota_buf,*pos,num_bits_to_write);
    *pos += num_bits_to_write; 
    *esm_container_len += 2;
    ...
    memscpy((void*)(ota_buf + (*pos/8)), //Skip the 'already encoded bytes'
            ota_buf_len - (*pos/8),
            (void *)ext_prot_conf_opt->content_ptr, 
            ext_prot_conf_opt->len); 
    *pos += num_bits_to_write * ext_prot_conf_opt->len;
    *esm_container_len += ext_prot_conf_opt->len;
  }
}
#ifdef FEATURE_CIOT
/*------------------------------------------------
...
{
  byte num_bits_to_write =0, index =0;
  word hc_config_index = 0, hc_config_len =0;
  word len_offset = 0;

  num_bits_to_write = 8;

  b_packb((byte)HC_CONFIG_IEI,ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  *esm_container_len += 1;

  /* ---- Length of HC CONFIG IE ---- */
  b_packb(hc_config->len,ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  *esm_container_len += 1;

  /* ---- RoHC profile  ---- */
  b_packb(hc_config->rohc_profile,ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  *esm_container_len += 1;
  hc_config->len--;

  /* ---- MAX_CID ---- */
  b_packb(hc_config->max_cid[0],ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  *esm_container_len += 1;
  hc_config->len--;

  b_packb(hc_config->max_cid[1],ota_buf,*pos,num_bits_to_write);
  *pos += num_bits_to_write; 
  *esm_container_len += 1;
  hc_config->len--;
  ...
  if((hc_config->len) > 0)
  {
    ...
    num_bits_to_write = 8;
    b_packb(hc_config->hc_additional_parameters[0],ota_buf,*pos,num_bits_to_write);
    *pos += num_bits_to_write; 
    *esm_container_len += 1;
    hc_config->len--;
  }
  ...
  num_bits_to_write = 8;
  for(index=0; index < (hc_config->len) && index < (MAX_CP_SETUP_LEN);index++)
  { 
    /* ---- protocol ID[0] ---- */
    b_packb(hc_config->hc_ctx_setup_container[index],ota_buf,*pos,num_bits_to_write);  
    *pos += num_bits_to_write; 
    *esm_container_len += 1;
    hc_config->len--;
  }
