...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmconn.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
))
   {
      switch (msg_p->cmd.hdr.message_id)
      {
            ...
            ** ------------------------------------------------------------- */
                   ...
                   (gmm_attach_type == GMM_GPRS_ATTACH_WHILE_IMSI_ATTACHED)) ) ))
            {
              MSG_HIGH_DS_2( MM_SUB, "=MM= MM ignored RR_PAGING_IND, combined NMO1 Reg in progress, gmm state is %d, gmm_ra_update_type - %d", gmm_state,gmm_ra_update_type);
                     ...
                     !mm_plmn_match(mm_serving_plmn.info.plmn, mmcoord_pending_reg_message.cmd.mmr_reg_req.plmn )) 
            {
               MSG_HIGH_DS_0(MM_SUB,"=MM= ignoring page as MPLMN search on different PLMN");
            }
         ...
         case RR_DATA_IND:
            ...
            if ( (get_OTA_msg_type(msg_p) == CM_SERVICE_ACCEPT ) ||
...
/* 24.301 5.5.3.3.2 */
              ...
              {
                  if ((mm_location_information.location_update_status == NOT_UPDATED)&& 
...
(
                 ...
                 default:
                    ...
                    if (mm_state == MM_LOCATION_UPDATE_REJECTED && 
                        mm_lu_reject_info.reject_cause == NATIONAL_ROAMING_NOT_ALLOWED)
                    {
                         MSG_HIGH_DS_2(MM_SUB ,"=MM= Hard reject normal call due to MM state %d and LU reject %d", mm_state,mm_lu_reject_info.reject_cause);
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
...
#else
       MSG_ERROR_DS_0(MM_SUB, "=MM= CS domain call/SMS unexpected");
#endif
      }
   }
...

   {
      ...
      if (msg_p->cmd.hdr.message_id == (int)RR_SYNC_IND)
      {
         ...
         MSG_HIGH_DS_2( MM_SUB, "=MM= MM rec'd RR_SYNC_IND reason %d, Ciphering enabled %d",
             msg_p->cmd.rr_sync_ind.sync_reason, msg_p->cmd.rr_sync_ind.ciphering_enabled);
#else
         MSG_HIGH_DS_2( MM_SUB, "=MM= MM rec'd RR_SYNC_IND reason %d, Ciphering enabled %d",
...
#ifdef FEATURE_UMTS_VOICE_CIPHERING_IND
         mm_send_cm_cipher_ind(SYS_SRV_DOMAIN_CS_ONLY,msg_p->cmd.rr_sync_ind.ciphering_enabled);
#endif
...
/* Spec 24.008, Spec CR 2421, Section 4.5.1.1
* 
* In A/Gb mode, the MS shall consider the MM connection establishment to be completed when
*       i)  receiving an indication from the RR sublayer that the ciphering mode setting procedure 
*           is completed in MM sublayer state WAIT FOR OUTGOING MM CONNECTION, or 
*       ii) receiving a CM SERVICE ACCEPT message.
* 
* In Iu mode, the MS shall consider the MM connection establishment to be completed when
*       i)  receiving an indication from the RR sublayer that the security mode control procedure 
*           is completed in MM sublayer state WAIT FOR OUTGOING MM CONNECTION, or 
*       ii) receiving a CM SERVICE ACCEPT message. 
*/
            if(mm_state == MM_WAIT_FOR_ADDITIONAL_OUTGOING_MM_CONNECTION && 
                     mm_get_nv_nas_rel_compliance_per_subs(
#ifdef FEATURE_DUAL_SIM
                                      (sys_modem_as_id_e_type)mm_as_id
#endif
                                     ) >= NV_NAS_COMPLIANCE_REL12)
              {     
                MSG_HIGH_DS_1(MM_SUB,"SYNC indication ignored in MM State = %d for R12",mm_state);
              }
              else
              {
         ...
         else
         {
            ...
            if((msg_p->cmd.rr_sync_ind.sync_reason == MM_RR_RES_ASS || msg_p->cmd.rr_sync_ind.sync_reason == MM_RR_CHANNEL_MODE_MODIFIED) &&
               (mm_state == MM_WAIT_FOR_NETWORK_COMMAND))
            {
              mm_stop_timer(TIMER_T3240);
            }
...
#else
	  CNM_route_CNM_messages((IMH_T *)&mmcc_sync_ind);
#endif
            }
         }
      }/* RR_SYNC_IND */
