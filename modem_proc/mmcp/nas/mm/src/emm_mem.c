...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_mem.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void emm_mem_free(void *ptr,emm_ctrl_data_type *emm_ctrl_data_ptr)
{
#ifndef FEATURE_CLADE_HEAP
  emm_modem_mem_free(ptr, emm_ctrl_data_ptr);
#else
  emm_mem_free_from_clad(ptr);
#endif
}

 void* emm_mem_alloc(size_t x,emm_ctrl_data_type *emm_ctrl_data_ptr)
{
#ifndef FEATURE_CLADE_HEAP
  return emm_modem_mem_alloc(x,emm_ctrl_data_ptr);
#else
  return emm_mem_alloc_from_clad(x); 
#endif
}
