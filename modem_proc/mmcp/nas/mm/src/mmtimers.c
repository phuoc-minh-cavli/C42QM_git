...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmtimers.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#endif
#ifdef FEATURE_DUAL_SIM
extern uint8  mm_emm_t3247_test_val_sim[MAX_NAS_STACKS];
...
===========================================================================*/
dword mm_timer_get_random_t3247_value( )
{
 ...
 dword mm_get_timeout( timer_id_T timer_id )
 {
   ...
   switch ( timer_id )
   {
...
#ifdef FEATURE_LTE
      case TIMER_T3417 :
        return_value = 5000 + emm_get_timer_extention_value();
        break ;
      case TIMER_T3421 :
        return_value = 15000 + emm_get_timer_extention_value();
        break ;
      case TIMER_T3430 :
        return_value = 15000 + emm_get_timer_extention_value();
        break;
      case TIMER_T3418:
        return_value = 20000 + emm_get_timer_extention_value();
        break;
      case TIMER_T3416:
        return_value = 30000 + emm_get_timer_extention_value();
        break;
      case TIMER_T3411 :
        return_value  = 10000;
        break;
      ...
      case TIMER_EMM_PS_DETACH:
        /*5 is the max retry times for OTA detach and 150000(ms) is T3421 default value, 3000(ms) is buffer timer value after T3421 expiry
        for legacy RAT, TIMER_EMM_PS_DETACH duration is 78s and for NB1 it will be 1278s*/
        return_value = 5 * (emm_get_timer_extention_value() + 15000) + 3000;
        break;
#endif /*FEATURE_LTE*/
   ...
   return return_value * 6; /* Increased timeout values for debugging */
#else
   return return_value;
#endif
}
...
void mm_start_timer( timer_id_T timer_id, uint64 num_millisecs )
{
   ...
   MSG_HIGH_DS_3( MM_SUB, "=MM= Start %d, timeout %u:%u",
             timer_id, (dword) (num_millisecs /1000L), (word)(num_millisecs % 1000L) );
   ...
   {
     status = gs_start_timer_ext( GS_TIMER_QUEUE_MM,
                                  timer_id,
                                  mm_timer_expiry,
                                  num_millisecs,
                                  (byte) 0,
   ...
   {
     status = gs_start_timer_ext( GS_TIMER_QUEUE_MM_SUB2,
                                  timer_id,
   ...
   {
     status = gs_start_timer_ext( GS_TIMER_QUEUE_MM_SUB3,
                                  timer_id,
...
void mm_stop_timer( timer_id_T timer_id )
{
   ...
   if (mm_timer_status[timer_id] != TIMER_STOPPED)
   {
      ...
      gs_status = gs_cancel_timer_ext( GS_TIMER_QUEUE_MM,
                                       timer_id);
...
gs_timer_queue_id_T get_mm_timer_queue( void )
{
  ...
  {
    return GS_TIMER_QUEUE_MM;
  }
  else if(mm_as_id == (mm_as_id_e_type)SYS_MODEM_AS_ID_2)
  {
    return GS_TIMER_QUEUE_MM_SUB2;
  }
#if defined(FEATURE_TRIPLE_SIM) || defined(FEATURE_SGLTE_DUAL_SIM)
  else if(mm_as_id == (mm_as_id_e_type)SYS_MODEM_AS_ID_3)
  {
    return GS_TIMER_QUEUE_MM_SUB3;
    }
#endif
  else
  {
