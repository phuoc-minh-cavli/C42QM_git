...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmforbid.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
void mm_send_rr_forbidden_lai_update(void)
{
   ...
   for(count=0; count<mm_foreground_search_lai_timer_list.length; count++)
   {
     if((mm_foreground_search_lai_timer_list.lai_timer_list[count].is_ps_registered == FALSE) ||
        (mm_is_rat_lte(mm_serving_plmn.info.active_rat)))
     {
...
)
{
  ...
  for(i=0;i<lai_reject_list_sim[as_id].length;i++)
  {
    if(memcmp(&lai_reject_list_sim[as_id].lai[i],
              ...
              LAI_SIZE ) == 0 )    /* if match found */
    {
      MSG_MED_DS_5(MM_LOCAL_SUB, "=MM= LAI found in Reject List. PLMN [%X %X %X], LAC [%X %X]",
               mm_lai.PLMN_id.identity[0], mm_lai.PLMN_id.identity[1],
               mm_lai.PLMN_id.identity[2], mm_lai.location_area_code[0],
               mm_lai.location_area_code[1]);
      return TRUE;
    }
  }
...
void mm_per_subs_store_forbidden_lai_list
(
 forbidden_list_T    *list_ptr
)
{
   memscpy((forbidden_list_T *)list_ptr, 2*sizeof (forbidden_list_T), 
                              (forbidden_list_T*)&forbidden_list, 2*sizeof (forbidden_list_T));
}
...
void mm_per_subs_store_lai_reject_list
(
 lai_reject_list_T    *list_ptr
)
{
   memscpy((lai_reject_list_T *)list_ptr,sizeof (lai_reject_list_T), 
                              (lai_reject_list_T*)&lai_reject_list,sizeof (lai_reject_list_T));
}
...
void mm_per_subs_restore_forbidden_lai_list
(
 forbidden_list_T    *list_ptr
)
{
   memscpy((forbidden_list_T *)&forbidden_list,2*sizeof (forbidden_list_T), 
                              (forbidden_list_T*)list_ptr,2*sizeof (forbidden_list_T));
}
...
void mm_per_subs_restore_lai_reject_list
(
 lai_reject_list_T    *list_ptr
)
{
   memscpy((lai_reject_list_T *)&lai_reject_list,sizeof (lai_reject_list_T), 
                              (lai_reject_list_T*)list_ptr,sizeof (lai_reject_list_T));
}
