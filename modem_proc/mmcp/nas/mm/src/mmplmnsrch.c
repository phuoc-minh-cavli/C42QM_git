...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmplmnsrch.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
   ...
   switch ( msg_p->cmd.hdr.message_set )
   {
    ...
    || defined (FEATURE_RAT_PRIORITY_LIST)
               ...
               */
...
#endif
#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH
               if(msg_p->cmd.rr_plmn_list_cnf.service_search != SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH)
#endif
			   {
                 mmr_plmn_search_cnf->found_plmn_list = msg_p->cmd.rr_plmn_list_cnf.available_plmn_list;
			   }
               ...
               {
                 mmr_plmn_search_cnf->plmn_search_result = REG_MM_PLMN_SEARCH_ABORTED;
				 memset(&(mmr_plmn_search_cnf->found_plmn_list), 0, sizeof(mmr_plmn_search_cnf->found_plmn_list));
               } 
               else
#endif
               ...
               if(msg_p->cmd.rr_plmn_list_cnf.service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH)
               {
		          ...
		          ref_cnt_obj_add_ref(msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result);
			 
                  if(msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result != NULL)
                  {
				    MSG_HIGH_DS_1(MM_SUB, "=MM= Number of elements = %d", msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result->wwan_scan_result_per_rat.num_scan_result_per_rat);	
                    mmr_plmn_search_cnf->found_plmn_list.gps_wwan_scan_result = msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result;
					memscpy( mmr_plmn_search_cnf->found_plmn_list.gps_wwan_scan_result,sizeof(sys_loc_wwan_scan_result_s_type),
		              msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result,sizeof(sys_loc_wwan_scan_result_s_type));
			        
			        mmr_plmn_search_cnf->plmn_search_result = REG_MM_PLMN_SEARCH_SUCCESS;
                  }
			      else
			      {
				    MSG_FATAL_DS(MM_SUB,"=EMM= RRC_GPS_WWAN_SCAN_RESULT is NULL",msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result, 0,0);
			      }
                }     
#endif
               ...
               if((mmr_plmn_search_cnf->plmn_search_result != REG_MM_PLMN_SEARCH_PARTIAL) ||
                  ((mm_emm_is_qrd_device == TRUE || mm_partial_search_allowed_get() == TRUE) &&
                   ((mmr_plmn_search_cnf->service_search != SYS_SERVICE_SEARCH_CELL_SEARCH && 
                     mmr_plmn_search_cnf->found_plmn_list.plmn_list.length != 0 )|| 
                    (mmr_plmn_search_cnf->found_plmn_list.cell_info_list.geran_cell_list.num_cell != 0 && 
                       mmr_plmn_search_cnf->service_search == SYS_SERVICE_SEARCH_CELL_SEARCH))&&
                   (mmr_plmn_search_cnf->plmn_search_result == REG_MM_PLMN_SEARCH_PARTIAL)))
               {
                 mm_send_mmr_plmn_search_cnf( mmr_plmn_search_cnf );
...
#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH
               if(msg_p->cmd.rr_plmn_list_cnf.service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH &&
			     msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result != NULL)
			   {
                 ref_cnt_obj_release(msg_p->cmd.rr_plmn_list_cnf.available_plmn_list.gps_wwan_scan_result);
			   }
#endif
               /*
    ...
    || defined (FEATURE_RAT_PRIORITY_LIST)
               ...
               */
               ...
               {
                    mmr_plmn_search_cnf->available_plmn_list.info[entry].list_category =
                       SYS_DETAILED_PLMN_LIST_CATEGORY_OTHER;

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].plmn =
                       mmumts_convert_rrc_plmn_id_to_nas_plmn_id(
                          msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].plmn);

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].plmn_forbidden = FALSE;

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].plmn_service_capability =
                       msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].service_capability;

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].rat =
                       msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].rat;

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].signal_quality =
                       msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].signal_quality;

                    mmr_plmn_search_cnf->available_plmn_list.info[entry].signal_strength =
                       msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].signal_strength;
                    mmr_plmn_search_cnf->available_plmn_list.info[entry].earfcn =                              
                   ...
                   {
                     mmr_plmn_search_cnf->available_plmn_list.info[entry].csg_info.csg_id = SYS_CSG_ID_INVALID;
                   }
                   else
                   {
                     mmr_plmn_search_cnf->available_plmn_list.info[entry].csg_info = 
                       msg_p->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[entry].csg_info;
                   }
#endif  
               }

               mmr_plmn_search_cnf->available_plmn_list.length = num_plmn;
               ...
               {
                 mm_process_csg_search_list(mm_csg_search_type,
                                     &mmr_plmn_search_cnf->available_plmn_list);
               }
#endif 
               if((mmr_plmn_search_cnf->plmn_search_result != REG_MM_PLMN_SEARCH_PARTIAL) ||
...
void mm_request_list_from_rr(mmr_plmn_search_req_s_type *plmn_srch_ptr)
{
   ...
   */
...
#endif
   if( plmn_srch_ptr->rat == SYS_RAT_GSM_RADIO_ACCESS)
   {
      ...
      rr_plmn_list_req->cell_search_num_cells = plmn_srch_ptr->cell_search_num_cells;
      mm_plmn_search_in_progress = TRUE;
      mm_serving_plmn.info.active_rat = SYS_RAT_GSM_RADIO_ACCESS;
      mm_is_gsm_active_rat_for_thermal_read = TRUE;
      mm_plmn_list_req_pending = FALSE;
#ifdef FEATURE_GPS_WWAN_PRIORITY_SEARCH	
	  if(plmn_srch_ptr->service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH)
	  {
	    rr_plmn_list_req->gps_wwan_search_info = plmn_srch_ptr->gps_wwan_search_info;
	  }
#endif	  
#ifdef FEATURE_LTE
#ifdef FEATURE_CIOT
   ...
   else if(mm_serving_plmn.info.active_rat == SYS_RAT_NONE)
   {
     ...
     MSG_HIGH_DS_0(MM_SUB,"=MM= MM received MMR_PLMN_SEARCH_REQ when active rat is NONE, sending search confirmation with result aborted");
     mmr_plmn_search_cnf->network_selection_mode = plmn_srch_ptr->network_selection_mode;
     mmr_plmn_search_cnf->plmn_search_result = REG_MM_PLMN_SEARCH_ABORTED;
     memset(&(mmr_plmn_search_cnf->found_plmn_list), 0, sizeof(mmr_plmn_search_cnf->found_plmn_list));
     mmr_plmn_search_cnf->transaction_id = plmn_srch_ptr->transaction_id;
     mmr_plmn_search_cnf->service_search = plmn_srch_ptr->service_search; 
     mm_send_mmr_plmn_search_cnf( mmr_plmn_search_cnf );
#ifndef FEATURE_MODEM_HEAP
     gs_free(mmr_plmn_search_cnf);
#else
