...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_csfb_handler.c#1 $
...
)
{
  ...
  emm_msgr_send((msgr_hdr_s *)ul_msg_req_ptr, 
               sizeof(lte_rrc_irat_tunnel_ul_msg_req_s));
...
)
{
  ...
  emm_msgr_send((msgr_hdr_s *)dl_msg_ind_ptr, 
               sizeof(emm_irat_dl_msg_ind_type));
...
void emm_send_irat_imsi_attach_ind(void)
{
  ...
  emm_msgr_send_error((msgr_hdr_s *)&imsi_attach_ind, 
               sizeof(emm_irat_imsi_attach_ind_type));
...
)
{
  ...
  emm_msgr_send_error((msgr_hdr_s *)&ctxt_lost_ind, 
               sizeof(emm_irat_ctxt_lost_ind_type));
...
)
{
  ...
  emm_msgr_send((msgr_hdr_s *)&irat_ul_msg_cnf,sizeof(emm_irat_ul_msg_cnf_type));
...
)
{
             ...
             i < LTE_RRC_CDMA_TUNNEL_MAX_DSM_ARRAY_SIZE; i++)
  {
    ...
    nas_assert_check((boolean)(dsm_ptr[i] == NULL));
...
)
{
  ...
  for(i = 0; (i < num_dsm_ptr) && (i < LTE_RRC_CDMA_TUNNEL_MAX_DSM_ARRAY_SIZE); i++)
  {
    ...
    mm_check_for_null_ptr((void *)&dsm_ptr[i]);
  }
  ...
  emm_msgr_send((msgr_hdr_s *)&ul_msg_req, 
               sizeof(lte_irat_cdma_tunnel_ul_msg_req_s));
...
{  
  ...
  nas_assert_check((boolean)(num_attach != 0));
...
)
{
  ...
  emm_msgr_send((msgr_hdr_s *)&dl_msg_ind, 
               sizeof(emm_irat_3gpp2_dl_msg_ind_type));
...
{
  emm_csfb_cmd_type  *csfb_cmd_ptr = (emm_csfb_cmd_type *)cmd_ptr; 
