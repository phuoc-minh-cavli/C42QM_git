...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/mmsecurity.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
   ...
   if( is_OTA_message(msg_p) )
   {
      ...
      switch (get_OTA_msg_type( msg_p ))
      {
         case IDENTITY_REQUEST:
            if(message_len < MM_IDENTITY_REQ_MSG_MIN_LENGTH)
	    {
	      MSG_ERROR_DS( MM_SUB, "=MM= Invalid MM IDENTITY message", 0, 0, 0);
         ...
         case AUTHENTICATION_REQUEST:
            if(message_len < MM_AUTH_REQ_MSG_MIN_LENGTH) 
	    {
	      MSG_ERROR_DS( MM_SUB, "=MM= Invalid Authentication Request message", 0, 0, 0);
         ...
         case AUTHENTICATION_REJECT:
            if(message_len < MM_AUTH_REJECT_MSG_MIN_LENGTH) 
	    {
	      MSG_ERROR_DS( MM_SUB, "=MM= Invalid Authentication Reject message", 0, 0, 0);
     ...
     if (msg_p->cmd.hdr.message_set == MS_TIMER)
     {
       switch (msg_p->cmd.timer_expiry.timer_id)
       {
         ...
         case TIMER_T3216:
           ...
           /* Spec CR 2686: Spec 24.008 4.3.2.6:
            * If an MS has an MM connection for an emergency call established or is establishing an MM connection for an
            * emergency call when timer T3214 or T3216 expires, the MS shall not deem that the network has failed the 
...
void mm_perform_authentication(void)
{
  ...
  auth_autn_type autn ;
  byte  ciphering_key_sequence_number = 0;
