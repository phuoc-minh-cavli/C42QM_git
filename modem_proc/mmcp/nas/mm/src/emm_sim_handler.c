...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/emm_sim_handler.c#1 $
...
lte_rrc_imsi_s lte_convert_imsi_to_digits(imsi_data_T imsi)
{
   ...
   nas_assert_check((boolean)(imsi.length <= 8));

   /* start from the 2nd byte of IMSI value */
   for(index = 1; index < imsi.length; index++)
   {
     if((rrc_index + 2) >= LTE_RRC_MAX_IMSI_DIGITS)
     {
       MSG_FATAL_DS(MM_SUB,"=EMM= Num of IMSI digits greater than %d, IMSI len %d, rrc_idx %d",
                               LTE_RRC_MAX_IMSI_DIGITS, imsi.length, rrc_index);
     }
...
)
{
  ...
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Saved GUTI is invalid");
  }
  encode_guti_ie(&emm_ctrl_data_ptr->emm_guti,&pos,&epsloci[EMM_USIM_GUTI_OFFSET]);

  if(emm_ctrl_data_ptr->last_registered_tai_valid == FALSE)
  {
    MSG_HIGH_DS_0(MM_SUB, "=EMM= Saved last registered TAI is invalid");
  }
