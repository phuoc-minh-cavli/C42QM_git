...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/mm/src/gmmconn.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
                  ...
                  switch (service_reject_msg.gmm_cause)
                  {
                    ...
                    case GPRS_SERVICES_AND_NON_GPRS_SERVICES_NOT_ALLOWED:
                      ...
                      ** ---------------------------------------------------------- */
                      ...
                      gmm_process_gprs_release_ind(mm_convert_gmm_to_sm_cause(service_reject_msg.gmm_cause));
...
boolean gmm_check_service_reject_for_integrity_protection(mm_cmd_type *message)
{
  ...
  if(msg_index < MIN(msg_length,gmm_max_octet_length))
  {
   if(!gmm_integrity_protection_activated &&
       ...
       (message->cmd.mm_data_ind.L3_data[msg_index] == CSG_NOT_AUTHORIZED))
   {
              ...
              else if (gmm_rau_is_required == TRUE)
              {
                ...
                gmm_initiate_routing_area_update();
                      ...
                      (!gmm_rai_changed(gmm_stored_rai.location_area_code, gmm_stored_rai.routing_area_code, 
                        mm_serving_plmn.lac, mm_serving_plmn.rac)) &&
                      (!gmm_plmn_changed(gmm_stored_rai.plmn_id, mm_serving_plmn.info.plmn))) && 
                      (mm_timer_status[TIMER_T3311] == TIMER_ACTIVE) && (emm_ctrl_data_ptr->TIN == NAS_MM_TIN_GUTI)) 
              {
		        MSG_HIGH_DS_0( MM_SUB, "=MM= Sending MMCNM_ESTABLISH_REJ : Due to Timer 3311 running");
#ifdef FEATURE_SMS_ENHANCED_DOMAIN_SELECTION
                mm_send_mmcnm_est_rej(MM_REJECT_CAUSE, MM_REJ_NO_SERVICE, MMCC_SMS_PS);
#else
                mm_send_mmcnm_est_rej(MM_REJECT_CAUSE, MM_REJ_NO_SERVICE);
              ...
              ** ----------------------------------------------------------*/
              ...
              else if(mm_is_mplmn_search_in_progress() == TRUE)
              {
                  ...
                  gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
                      ...
                      (mm_timer_status[TIMER_T3311] == TIMER_ACTIVE) && (emm_ctrl_data_ptr->TIN == NAS_MM_TIN_GUTI))
              {
                  MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ : Due to Timer 3311 running");
                  gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
              }
#endif
              else
	...
	The MS shall not start the service request procedure unless the MS is accessing the network with access class 11 � 15*/
                    ...
                    )
                {
                  ...
                  **************************************************************************/
                  ...
                  {
                     MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ due to RAU failed which is started because of Directed Signalling connection Reestablishment/ T3311 is running");
                     gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(PROTOCOL_ERROR_UNSPECIFIED));
                  }
                  else
                  {
          ...
          else
          {
            MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMSM_ESTABLISH_REJ");
            gmm_send_gmmsm_establish_rej(mm_convert_gmm_to_sm_cause(ABORT_SERVICE_NOT_AVAILABLE));
            /* Indicate forced attach command to CM upon client call origination */
            if ((gmm_state == GMM_NULL) && 
                 (mm_serving_plmn.ms_op_mode == SYS_SRV_DOMAIN_CS_ONLY))
             {
                 ...
                 user_pref.client_id  = CM_CLIENT_ID_ANONYMOUS;
                 MSG_HIGH_DS_1(MM_SUB, "=MM= Forced attach command pref = %d",user_pref.srv_domain_pref);
                 if(FALSE == cm_user_pref_update_req(&user_pref,NULL,NULL))
                 {
                   MSG_HIGH_DS_0(MM_SUB, "=MM= Forced attach command failure");
                 }
             }
          }
          break;
        ...
        case GMMSM_PS_CALL_STATUS_IND:
          if (message->cmd.gmmsm_ps_call_status_ind.pdp_sig_in_progress == FALSE)
          {
            if ((is_gsm_mode() &&
        ...
        case GMMSM_SAVE_CTXT_PSM_CNF:
          {
            MSG_MED_DS_0(MM_SUB, "=MM= Received GMMSM_SAVE_CTXT_PSM_CNF, process it");
            gmm_process_save_ctxt_psm_cnf(message->cmd.gmmsm_save_ctxt_psm_cnf.status);
          }
          break;

        case GMMSM_RESTORE_CTXT_PSM_CNF:
          {
            MSG_MED_DS_0(MM_SUB, "=MM= Received GMMSM_RESTORE_CTXT_PSM_CNF, process it");
            gmm_process_restore_ctxt_psm_cnf(message->cmd.gmmsm_restore_ctxt_psm_cnf.status);
          }
          break;
#endif
              ...
              if (is_umts_mode())
              {
                ...
                **************************************************************************/
                    ...
                    )
                {
                  ...
                  MSG_HIGH_DS_0( MM_SUB, "=MM= Sending GMMRABM_REESTABLISH_REJ:RAU started because of Directed Signalling Reestablishment is failed/T3311 is still running" );
...
FUNCTION GMM_PROCESS_RRC_MBMS_CONN_REQ_IND

DESCRIPTION
  This function processes RRC_MBMS_CONN_REQ_IND from RRC
...
===========================================================================*/
void gmm_process_rrc_mbms_conn_req_ind(mm_cmd_type *message)
{
       rrc_establish_cause_e_type rrc_establish_cause=RRC_EST_TERM_UNKNOWN;

       MSG_HIGH_DS_1( MM_SUB, "=MM= MBMS: Received RRC_MBMS_CONN_REQ_IND : MBMS RRC Cause: %d", message->cmd.rrc_mbms_conn_ind.rrc_establish_cause);

       if (message->cmd.rrc_mbms_conn_ind.rrc_establish_cause == MBMS_RECEPTION) 
          rrc_establish_cause = RRC_EST_MBMS_RECEPTION;
       else if (message->cmd.rrc_mbms_conn_ind.rrc_establish_cause == MBMS_PTP_RB_REQUEST )
          rrc_establish_cause = RRC_EST_MBMS_PTP_RB_REQUEST;
       ...
       if ((gmm_state == GMM_REGISTERED) &&
            ...
            if (is_umts_mode())
            {
              ...
              ** ---------------------------------------------------------------------*/
              if ((gmm_substate == GMM_ATTEMPTING_TO_UPDATE) ||
                  (gmm_substate == GMM_LIMITED_SERVICE))
              {
                MSG_HIGH_DS_1( MM_SUB, "=MM= MBMS: Cannot initiate Service Request due to wrong GMM substate %d", gmm_substate);
              }
              else if (mm_serving_plmn.info.service_status == SYS_SRV_STATUS_NO_SRV)
              {
                MSG_HIGH_DS_0( MM_SUB, "=MM= MBMS: Cannot initiate Service Request due to No Service ");
              }                            
              else if (pmm_mode == PMM_CONNECTED)
              {
                /* Check AC blocking restrictions */
                ...
                ****************************************************************/
                ...
                {
                  MSG_HIGH_DS_1( MM_SUB, "=MM= MBMS: Cannot initiate Service Request due to AC blocking %d",
                             ps_cell_access );
                }
                else
                {
                  MSG_HIGH_DS_0( MM_SUB, "=MM= MBMS: Initiate Service Request (GMM already in PMM_CONNECTED mode)" );
                  gmm_service_request_type      = (byte) message->cmd.rrc_mbms_conn_ind.service_type;
                  gmm_initiate_service_request(
                    rrc_establish_cause,
                    mm_get_current_security_context( RRC_PS_DOMAIN_CN_ID ),
                    NULL
              ...
              else
              {
                    ...
                    )
                {
                  MSG_HIGH_DS_1( MM_SUB, "=MM= MBMS: Cannot initiate Service Request due to AC blocking %d",
                    ...
                    || (gmm_rau_is_required == TRUE))
                {
                  MSG_HIGH_DS_0( MM_SUB, "=MM= MBMS: Initiate Service Request ");
                  gmm_service_request_type      = (byte) message->cmd.rrc_mbms_conn_ind.service_type;
                  gmm_initiate_service_request(
                    rrc_establish_cause,
                    mm_get_current_security_context( RRC_PS_DOMAIN_CN_ID ),
                ...
                {
                  MSG_HIGH_DS_1( MM_SUB, "=MM= MBMS: Cannot initiate Service Request due to GMM update status != GMM_GU1_UPDATED, it is %d",gmm_update_status );
                }
              }
#endif
            }
          ...
          {
            MSG_HIGH_DS_0( MM_SUB, "=MM= MBMS: Cannot initiate Service Request, GMM not in \
              GMM_REGISTERED and SERVICE_AVAILABLE");
          }

} /* end mmcoord_process_rrc_mbms_conn_ind */
