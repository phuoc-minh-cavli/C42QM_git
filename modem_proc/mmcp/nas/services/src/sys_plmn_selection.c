...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/services/src/sys_plmn_selection.c#1 $ $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#ifdef FEATURE_CIOT
    /* Only selected RATs are allowed */
    if ((acq_sys_mode != SYS_SYS_MODE_GSM) &&
        (acq_sys_mode != SYS_SYS_MODE_LTE_M1) &&
        (acq_sys_mode != SYS_SYS_MODE_LTE_NB1) &&
