...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/services/src/cfa_msm.c#1 $  $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#define CFA_VERSION  0xA1
   ...
   else
   {
     ...
     packet_size = (int) ( NAS_STD_OFFSETOF(LOG_UMTS_CALL_FLOW_ANALYSIS_C_type, Message) + msg_size );
     STATS(rex_enter_crit_sect(&cfa_stats_crit_sect);)
...
)
{
  ...
  STATS( int x; )
  /* Set minimum rsp_len */
  unsigned int rsp_len = NAS_STD_OFFSETOF(DIAG_SUBSYS_UMTS_UMTS_CFA_CONFIG_F_rsp_type, op_data);
...
static void cfa_send_event_if_neccessary( byte queue, byte* msg_ptr )
{
    ...
    case MS_RR_L2:        
        ...
        case DL_DATA_REQ:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_data_req_T, layer3_message[0]);;
            isTx  = TRUE;
            break;
        }
        case DL_RECONNECT_REQ:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_reconnect_req_T, layer3_message[0]);;
            isTx  = TRUE;
            break;
        }
        case DL_RESUME_REQ:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_resume_req_T, layer3_message[0]);;
            isTx  = TRUE;
            break;
        }
        case DL_UNIT_DATA_REQ:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_unit_data_req_T, layer3_message[0]);;
            isTx  = TRUE;
            break;
        }
        case DL_ESTABLISH_REQ:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_establish_req_T, layer3_message[0]);;
            isTx  = TRUE;
            break;
        }
        ...
        case DL_DATA_IND:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_data_ind_T, layer3_message[0]);;
            isTx  = FALSE;
            break;
        }
        case DL_UNIT_DATA_IND:
        {
            l3msg = msg_ptr+NAS_STD_OFFSETOF(dl_unit_data_ind_T, layer3_message[0]);;
            isTx  = FALSE;
            break;
        }
        default:
