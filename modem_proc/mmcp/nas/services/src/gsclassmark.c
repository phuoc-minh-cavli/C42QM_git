...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/services/src/gsclassmark.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
byte gs_get_classmark_1(boolean gsm_supported, boolean wcdma_in_use, boolean MSCR99)
{
...
#else
  ue_force_rel_mode_type           rr_nv_force_mscr_r99 = NV_FORCE_UE_REL99;
#endif
  /**********************************************************************************/
  ...
  nas_assert_check((boolean)((ES_IND | 0x1) == 0x1));
  ...
  nas_assert_check((boolean)((classmark_revision_level | 0x3) == 0x3));
  ...
  nas_assert_check((boolean)((rf_power_capability | 0x7) == 0x7));
...
)
{
...
#endif
#if !defined(FEATURE_NAS_DISABLE_VOICE) || defined(FEATURE_GSM) || defined(FEATURE_WCDMA)
  classmark_3_ie_T classmark3;
#endif
#if defined(FEATURE_GSM_MULTIBAND) && defined(FEATURE_GSM)
  byte no_of_bands;
...
#endif
  mm_check_for_null_ptr((void *)classmark_2_ptr);
  ...
  {
    /*****************************************************************************************/
    /* Multiband not in use... if GSM_900 band being used, hw_classmark contains information */
    /* of whether EGSM is supported or not                                                   */
    /*****************************************************************************************/
    #ifdef GSM_900
      freq_capability =        hw_classmark.freq_capability;
    #else
      /* some other band is in use */
      freq_capability = 0;
    #endif
  }
#endif
...
#else
  ss_screening_indicator_local = 0;
#endif
...
#else
    /* don't include field for single band MS */
    gs_bit_stream_insert_field(&bit_stream,1,0);
#endif /* FEATURE_GSM_MULTIBAND */
  ...
  /* single band support field encoding */
  {
    ...
    if (wcdma_supported)
    {
      ...
      if (gsm_supported)
      {
        ...
        /* include the field Single Band Support */
        gs_bit_stream_insert_field(&bit_stream,1,1);

        /* by definition a single band is supported */
        #ifdef GSM_900
        {
          /* hw_classmark indicates if it is PGSM or EGSM */
          if (hw_classmark.freq_capability == 0)
          {
            /* PGSM encoded as 0001 */
            gs_bit_stream_insert_field(&bit_stream,4,0x1);
          }
          else
          {
            /* EGSM encoded as 0000 */
            gs_bit_stream_insert_field(&bit_stream,4,0x0);
          }
        }
        #else /* GSM_900 not defined */
        {
          #ifdef DCS_1800
...
NAS_EXIT_CRIT_SECT(mm_rat_prioritylist_crit_sec);
  ...
  /*  Spec 24.008, Spec CRs 2341, 2343, 2472, 2469, 2547, 2709, 2775
   *
   */
...
)
{
  ...
  */
  ...
  gs_bit_stream_insert_field(&bit_stream,1,0);
    ...
    /* SAIC/DARP Support */
#if defined(FEATURE_GSM_EGPRS_SAIC) && defined (FEATURE_NAS_GW)
    gs_bit_stream_insert_field(&bit_stream,2,gmm_nv_saic_feature_support);
    if (gmm_nv_saic_feature_support)
    {
      CM3_supported = TRUE;
...
#elif defined(FEATURE_GSM_GPRS) && defined (FEATURE_NAS_GW)
