...
$Header: //components/rel/mmcp.mpss/6.1.10/nas/services/src/gsvoiceif.c#1 $    $DateTime: 2023/04/25 14:39:18 $ $Author: pwbldsvc $
...
#if !defined (FEATURE_NAS_DISABLE_VOICE) 
#include "rex.h"
#include "sys_type.h"
...
void mvssup_amr_set_awb_mode(mvs_amr_mode_type amr_ul_mode);
...
void mvssup_amr_set_amr_mode(mvs_amr_mode_type amr_ul_mode)
{
  return;
}
void mvssup_gsm_set_dtx_mode( mvs_dtx_mode_type dtx_mode )
{
   return;
}

/*void mvs_enable(mvs_client_type client, mvs_mode_type mode,mvs_packet_ul_fn_type ul,mvs_packet_dl_fn_type dl,int x)
{

return ;
}*/

void mvs_standby (mvs_client_type client, mvs_mode_type mode)
{
...
void mvs_acquire(mvs_client_type client,ghdi_mvs_cb_func_fp p)
{
...
atomic_word_t  ghdi_mvs_cb_func_fp;		
...
uint32  ghdi_get_sampling_rate(void)
{
   MSG_HIGH_0("Wrong API called - ghdi_get_sampling_rate() "); 
   return GHDI_EAMR_SR_8000;   
}

void    ghdi_reset_sampling_rate(void)
{
   MSG_HIGH_0("Wrong API called - ghdi_reset_sampling_rate() "); 
}
...
{
   MSG_HIGH_0("Wrong API called - ghdi_control_subs_tch_gating() "); 
   return GHDI_SUCCESS;
}
...
{
   MSG_HIGH_0("Wrong API called - ghdi_control_tch_gating_subs() "); 
   return GHDI_SUCCESS;
}

void ghdi_gsm_set_dtx_mode(boolean dtx_mode)
{
  MSG_HIGH_0("Wrong API called - ghdi_gsm_set_dtx_mode() "); 
} 
