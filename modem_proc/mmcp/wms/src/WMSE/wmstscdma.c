/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmstscdma.c#1 $
...
#define MSG_DUP_PARM MSG_HIGH_1("Duplicate parm: %d", parm_id)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((bit_pos + 5) > (uint32)(parm_len * 8))
  {
      MSG_HIGH_2( "decoding failed: minimum length check failed for decode address  %d>%d",
                  bit_pos + 5,
                  parm_len * 8 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* For Type and odd flag */
  if ((bit_pos + 4) > (uint32)(parm_len * 8))
  {
      MSG_HIGH_2( "decoding failed: minimum length check failed for decode subaddress  %d>%d",
                  bit_pos + 4,
                  parm_len * 8 );
      return WMS_INVALID_PARM_SIZE_S;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  ...
  while( st == WMS_OK_S  )
  {
    ...
    if( (pos + parm_len > bd_len) || (parm_len == 0 && parm_id != WMS_BD_ALERT))
    {
      /* parm data passes the max length of bearer data
      */
      MSG_HIGH_2( "decoding: parameter (id=%d) extends beyond bd size %d",
      ...
      case WMS_BD_CALLBACK:
        ...
        */
        ...
        for( i=0; i < cl_bd_ptr->callback.number_of_digits; i++ )
        {
          ...
          if( bit_pos + digit_size > (bd_len*8) )
          {
               return WMS_INVALID_PARM_SIZE_S;
          }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    tl_msg_ptr->cl_bd.num_messages  = 0;
    if(OTA_msg_ptr->data_len < 1 )
    {
      return WMS_INVALID_PARM_SIZE_S;
    }
...
)
{
  ...
  */
  ...
  {
    /* Maximum number of records in CMAS message :WMS_MAX_CMAE_RECORDS */
    if(num_records >= WMS_MAX_CMAE_RECORDS)
    {
       MSG_HIGH_1("num_records exceeds max limit %d", num_records);  
       break;  // out of while
    }
