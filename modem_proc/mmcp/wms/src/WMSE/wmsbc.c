/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmsbc.c#1 $
...
)
{
  ...
  MSG_HIGH_2( "BC_MM: client_id=%d, event=%d",
              client_id,
              event );
...
)
{
  ...
  MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Notify: %d",
                 event);
  ...
  */
  for( i = appInfo_ptr->bc.gw.gw_cb_table_size - 1; i >= 0 ; i-- )
  {
    ...
    else if (FALSE == appInfo_ptr->bc.gw.gw_cb_table[i].active_service)
    {
      ...
      *index = (uint32)i;
      *service_ptr = &appInfo_ptr->bc.gw.gw_cb_table[i];
      MSG_HIGH_1( "BC: found free entry %d", i);
    }
  }
...
)
{
  ...
  switch( cmd_ptr->hdr.cmd_id )
  {
   ...
   case WMS_CMD_BC_MM_MSG_DELETE_ALL_INDICATION:
    ...
    default:
      MSG_ERROR_1("Invalid BC MM cmd: %d", cmd_ptr->hdr.cmd_id);
      break;
  }
...
)
{
  ...
  */
  ...
  if (result == TRUE)
  {
    MSG_HIGH_0("GW CB hdr MATCHED");
  }
  else
  {
    MSG_HIGH_0("GW CB hdr NO match");
  }
