/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmsts.c#1 $
...
)
{
  ...
  */
  ...
  {
    /* the digits: */
    for( i=0; ( i< MIN(addr->number_of_digits, WMS_ADDRESS_MAX-1) ) ; i++ )
    {
...
)
{
  ...
  */
  ...
  else
  {
    ...
    // Check Current position + Length of Address IE is exceeding the actual length given in raw_ts_data_ptr
    if((pos+(2+((1+data[pos])/2))) >= raw_ts_data_ptr->len)
    {
      MSG_HIGH_3("WMS_TS_DEBUG : pos = %d, IE Length = %d, PDU Len = %d",pos, data[pos], raw_ts_data_ptr->len);           
       return WMS_INVALID_PARM_SIZE_S;
    }
    /* TP-OA
    */
    ...
    */
    ...
    //Per 3GPP 23.040 : 9.2.2.1	SMS DELIVER type : Time stamp is 7 Octets, Check MAX against Current + 6 Octets.
    if( pos + 6  >= raw_ts_data_ptr->len)
    {
       return WMS_INVALID_PARM_SIZE_S;
    }
    /* TP-SCTS
  ...
  //  deliver->user_data_len = data[pos];
    // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
     if( deliver->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
       {
                  MSG_HIGH_3("WMS_TS_DEBUG : pos = %d, IE Length = %d, PDU Len = %d",pos, data[pos], raw_ts_data_ptr->len);
          return WMS_INVALID_PARM_SIZE_S;
       }
     ...
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( pos + data[pos] >= raw_ts_data_ptr->len)
       {
                  MSG_HIGH_3("WMS_TS_DEBUG : pos = %d, IE Length = %d, PDU Len = %d",pos, data[pos], raw_ts_data_ptr->len);
          return WMS_INVALID_PARM_SIZE_S;
       }
  ...
  */
  ...
  if( deliver_report_ack->mask & WMS_TPDU_MASK_USER_DATA )
  {
    if( pos >= raw_ts_data_ptr->len)
    {
     ...
     if( deliver_report_ack->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
       {
...
)
{
  ...
  */
  ...
  if( deliver_report_error->mask & WMS_TPDU_MASK_USER_DATA )
  {
     ...
     if( deliver_report_error->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
       {
...
)
{
  ...
  */
  ...
  //Per 3GPP 23.040 : 9.2.2.3  SMS STATUS REPORT type : Time stamp is 7 Octets, Check MAX against Current + 6 Octets.
  if( pos + 6  >= raw_ts_data_ptr->len)
  {
      return WMS_INVALID_PARM_SIZE_S;
  }
         ...
         && (status_report->mask != 0))
     {
       ...
       */
       ...
       if( status_report->mask & WMS_TPDU_MASK_USER_DATA )
       {
         ...
         if( status_report->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
         {
           // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
           if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
           {
...
)
{
  ...
  */
  ...
  //Check if TP-MR and TP-PID and TP-CT and TP-MN can be decoded.
  if( pos >= raw_ts_data_ptr->len)
  {
      return WMS_INVALID_PARM_SIZE_S;
  }
  ...
  */
  ...
  // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
  if( pos + command->command_data_len >= raw_ts_data_ptr->len)
  {
     return WMS_INVALID_PARM_SIZE_S;
  }
  pos ++;
...
)
{
  ...
  */
  ...
  if( (submit->validity.format == WMS_GW_VALIDITY_ABSOLUTE) || (submit->validity.format == WMS_GW_VALIDITY_ENHANCED) )
  {
    //Per 3GPP 23.040 : 9.2.3.12.2/3  TP-VP ABSOLUTE/ENHANCED format is 7 Octets, Check MAX against Current + 6 Octets.
    if( pos + 6  >= raw_ts_data_ptr->len)
    {
      return WMS_INVALID_PARM_SIZE_S;
    }
  ...
  */
  ...
  if( submit->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
  {
    // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
    if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
    {
...
)
{
  ...
  */
  ...
  /* TP-PI  : Mandatory parmameter
  */
  submit_report_ack->mask = data[pos];
  pos ++;

  //Per 3GPP 23.040 : 9.2.2.2a (ii) Time stamp is 7 Octets, Check MAX against Current + 6 Octets.
  if( pos + 6  >= raw_ts_data_ptr->len)
  {
  ...
  */
  ...
  if( submit_report_ack->mask & WMS_TPDU_MASK_USER_DATA )
  {
     ...
     if( submit_report_ack->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
       {
...
)
{
  ...
  */
  ...
  if( submit_report_error->mask & WMS_TPDU_MASK_USER_DATA )
  {
     ...
     if( submit_report_error->dcs.alphabet == WMS_GW_ALPHABET_7_BIT_DEFAULT )
     {
       // Check Current position + Length of User Data IE is exceeding the actual length given in raw_ts_data_ptr
       if( (pos*8) + (data[pos] * 7) >= (raw_ts_data_ptr->len)*8 )
       {
...

{
  ...
  if( format == WMS_ETWS_NOTIFICATION_FORMAT_UMTS )
  {
    ...
    {
      if( pos + CB_PAGE_LEN  >= WMS_ETWS_MAX_MSG_LEN)
      {
         return WMS_INVALID_PARM_SIZE_S;
      }
