/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmscfg.c#1 $
...
)
{
  ...
  MSG_HIGH_2( "CFG: client_id=%d, event=%d",
              client_id,
              event );
...
)
{
...

  {
    ...
    case WMS_CFG_EVENT_CB_ERROR:
      ...
      break;
    default:
      MSG_HIGH_1("Unhandled cfg event %d", event);
      break;
    }

    //evt_str = wms_dbg_get_cfg_event_string((uint16)event);

    MSG_SPRINTF_2( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Notify: %d (as_id %d)",
                   event,
                   as_id );
  }
...
)
{
  ...
  do
  {
    ...
    if (NULL == appInfo_ptr)
    {
      MSG_ERROR_0("wms_cfg_send_ready_event_ex: NULL appInfo_ptr");
      break;
    }
...
)
{
  ...
  {
    MSG_ERROR_0("Invalid as_id in cmd_ptr");
...
)
{
  ...
  {
    MSG_ERROR_1("Invalid AS ID %d", appInfo_ptr->asid);
    return;
  }
  ...
  if (NULL != cfg_event_ptr)
  {
    ...
    default:
      MSG_ERROR_1("Invalid mem_store %d", mem_store);
      break;
    }
...
)
{
  ...
  MSG_LOW_0("wms_cfg_do_template_list()");
  ...
  if (NULL != cfg_event_ptr)
  {
    ...
    default:
      MSG_ERROR_1("Invalid Memory Store %d for do_template_list", mem_store);
      break;
    }
...
)
{
  ...
  MSG_LOW_1("wms_cfg_do_sts_report_list() mem store %d", mem_store);
  ...
  if (NULL != cfg_event_ptr)
  {
    ...
    default:
      MSG_ERROR_1("Invalid Memory Store %d for wms_cfg_do_sts_report_list", mem_store);
      break;
    }
...
)
{
  ...
  do
  {
    ...
    {
      MSG_ERROR_1("Invalid client ID %d", cmd_ptr->hdr.client_id);
...
#else
    if ( (SYS_SRV_STATUS_NO_SRV  == appInfo_ptr->stack_info.srv_status)  ||
           (SYS_SRV_STATUS_PWR_SAVE == appInfo_ptr->stack_info.srv_status) ||
           (SYS_SYS_MODE_NO_SRV == appInfo_ptr->stack_info.sys_mode) )
      {
        appInfo_ptr->srv_available = FALSE;
      }
      else
...
)
{
  ...
  {
    MSG_HIGH_2( "wms_cfg_send_sim_file_read_complete_evt_proc() tx_id: %d, send_evt_cb: (0x%x)", tx_id, send_evt_cb );
  }
...
#else
  //  cfg_s_ptr->wms_cm_subs_info.sub->session_type_1x = MMGSDI_MAX_SESSION_TYPE_ENUM;
 // cfg_s_ptr->wms_cm_subs_info.sub->is_available_1x = FALSE;
  cfg_s_ptr->wms_cm_subs_info.sub.session_type_gw = MMGSDI_MAX_SESSION_TYPE_ENUM;
  cfg_s_ptr->wms_cm_subs_info.sub.is_available_gw = FALSE;
 // cfg_s_ptr->wms_cm_subs_info.sub->is_operational  = FALSE;
#endif
...
)
{
  ...
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_cfg_init()");
  }
  ...
  wms_nv_efs_init();
  cfg_s_ptr->store_to_sim_if_nv_full = TRUE;
...
)
{
  ...
  if( tag == WMS_TAG_MO_TEMPLATE )
  {
    ...
    switch( mem_store )
    {
      ...
      default:
        MSG_ERROR_1("Invalid mem store %d", mem_store);
        break;
    }
  }
#ifdef FEATURE_GWSMS
...
)
{
...
#else
    route = cfg_s_ptr->routes.pp_routes[deliver->dcs.msg_class];
#endif
   MSG_HIGH_2("Route store route = %d  mem store = %d ",route.route , route.mem_store);
    if (WMS_MEMORY_STORE_SIM  == route.mem_store ||
        WMS_MEMORY_STORE_USIM == route.mem_store)
    {
	    MSG_HIGH_2("Route store APP type = %d  return is = %d ",appInfo_ptr->mmgsdi_appInfo.app_type,  wms_sim_mmgsdi_get_supproted_mem_store(appInfo_ptr->mmgsdi_appInfo.app_type));
        MSG_HIGH_1("Not updating store type final type is %d",route.mem_store);
  ...
  MSG_HIGH_2("Route store route = %d  mem store = %d  after Dcs check ",route.route , route.mem_store);
#ifdef FEATURE_CPHS
  if ( wms_cfg_check_cphs_msg(&deliver->address) )
  {
    ...
    routes_set = TRUE;
    MSG_HIGH_2("Route store route = %d  mem store = %d  In cphs check   ",route.route , route.mem_store);
  }
#endif
  ...
  for (i = 0; i < deliver->user_data.num_headers; i++)
  {
         ...
         (deliver->user_data.headers[i].u.special_sm.msg_waiting_kind <= WMS_GW_MSG_WAITING_VIDEOMAIL) )
    {
      if (WMS_GW_MSG_WAITING_DISCARD == deliver->user_data.headers[i].u.special_sm.msg_waiting)
      {
        ...
        route.mem_store = WMS_MEMORY_STORE_NONE;
        routes_set      = TRUE;
		MSG_HIGH_2("Route store route = %d  mem store = %d  In deliver header check  ",route.route , route.mem_store);
      }
  ...
  if (!routes_set)
  {
	  MSG_HIGH_2("Route store route = %d  mem store = %d  In route not set  ",route.route , route.mem_store);
         ...
         msg_waiting )
    {
	  ...
	  MSG_HIGH_2("Route store route = %d  mem store = %d  In route not set pid check   ",route.route , route.mem_store);
    }
    else if (WMS_GW_MSG_WAITING_DISCARD == deliver->dcs.msg_waiting)
    {
      ...
      route.route     = WMS_ROUTE_TRANSFER_AND_ACK;
      route.mem_store = WMS_MEMORY_STORE_NONE;
	  MSG_HIGH_2("Route store route = %d  mem store = %d  In route not set dcs_waiting check  ",route.route , route.mem_store);
    }
    else
    {
#ifdef FEATURE_WMS_GLOBAL_RETAIN
...
#else
  if (cfg_s_ptr->routes.transfer_status_report == TRUE)
  {
    route->route     = WMS_ROUTE_TRANSFER_AND_ACK;
    route->mem_store = WMS_MEMORY_STORE_NONE;
  }
...
)
{
  ...
  for ( stack_idx = 0; stack_idx < MIN(cmd_ss_ptr->number_of_stacks, CM_NO_STACKS); stack_idx++ )
  {
    ...
    if ( cmd_ss_stack_ptr->is_stack_operational )
    {
      ...
      if ( SYS_SYS_ID_TYPE_IS95 == cmd_ss_stack_ptr->sys_id.id_type )
      {
        ...
        if ( NULL != appInfo_ptr )
        {
          //For 1xCSFB, disable LTE  bearer when UE moves back to 1X.
          if ( (SYS_SYS_ID_TYPE_UMTS == appInfo_ptr->stack_info[stack_idx].sys_id.id_type) &&
...
void wms_populate_app_info_stack(wms_prov_app_info_s_type       *appInfo_ptr,wms_cmd_ss_change_info_s_type  *cmd_ss_ptr)
{
 if((appInfo_ptr != NULL) && (cmd_ss_ptr != NULL) )
 	{
      appInfo_ptr->stack_info.roam_status = cmd_ss_ptr->roam_status;
      appInfo_ptr->stack_info.srv_domain = cmd_ss_ptr->srv_domain;
      appInfo_ptr->stack_info.srv_status = cmd_ss_ptr->srv_status;
      appInfo_ptr->stack_info.sys_id = cmd_ss_ptr->sys_id;
      appInfo_ptr->stack_info.sys_srv_has_changed = cmd_ss_ptr->sys_srv_has_changed;
      appInfo_ptr->stack_info.sys_mode = cmd_ss_ptr->sys_mode;	
 	}
}
...
)
{
  ...
  // A: Yes. LTE can only be acquired using GW_xyz session. 1X session cannot acquire LTE.
    ...
    appInfo_ptr            = NULL;
      ...
      MSG_HIGH_5( "wms_cfg_ss_change_info: as_id = %d, srv_status = %d, sys_mode = %d, id_type = %d, srv_domain = %d",
                   cmd_ptr->hdr.as_id,        
                   cmd_ss_ptr->srv_status,
                   cmd_ss_ptr->sys_mode,
                   cmd_ss_ptr->sys_id.id_type,
                   cmd_ss_ptr->srv_domain );
      ...
      if (SYS_MODEM_AS_ID_1 == cmd_ptr->hdr.as_id)
      {
         if ( NULL != cfg_s_ptr )
         {
            if (SYS_SRV_STATUS_SRV == cmd_ss_ptr->srv_status)
            {
               cfg_s_ptr->sys_mode = cmd_ss_ptr->sys_mode;
            }
            else
            {
               cfg_s_ptr->sys_mode = SYS_SYS_MODE_NO_SRV;
      ...
      //IS95 (CDMA)
      if ( SYS_SYS_ID_TYPE_IS95 == cmd_ss_ptr->sys_id.id_type )
      {
        cfg_s_ptr->hs_based_plus_dial_setting         = cmd_ss_ptr->hs_based_plus_dial_setting;
        //cfg_s_ptr->home_mobile_country_code           = cmd_ss_ptr->home_mobile_country_code;
        cfg_s_ptr->current_mobile_country_code        = cmd_ss_ptr->current_mobile_country_code;
        cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_1X] = (SYS_SRV_STATUS_SRV == cmd_ss_ptr->srv_status);
        ...
        {
          //For 1xCSFB, disable LTE  bearer when UE moves back to 1X.
          if ( (SYS_SYS_ID_TYPE_UMTS == appInfo_ptr->stack_info.sys_id.id_type) /*&&
               (appInfo_ptr->stack_info[stack_idx].asubs_id == cmd_ss_ptr->stack_info[stack_idx].asubs_id)*/ )
          {
            cfg_s_ptr->active_bearers[WMS_BEARER_LTE] = FALSE;
            appInfo_ptr->srv_available = FALSE;
          ...
          //appInfo_ptr->stack_info[stack_idx] = cmd_ss_ptr->stack_info[stack_idx];
          wms_populate_app_info_stack(appInfo_ptr,cmd_ss_ptr);
        }
        ...
        if ( NULL != appInfo_ptr )
        {
          ...
          appInfo_ptr->in_roaming = (SYS_ROAM_STATUS_ON == cmd_ss_ptr->roam_status);
          MSG_HIGH_1("in_roaming global set as %d", appInfo_ptr->in_roaming);
          ...
          if (NULL != appInfo_ptr)
          {
            MSG_HIGH_0("Clear up GW appInfo_ptr->stack_info on SYS_MODEM_AS_ID_1");
            memset(&appInfo_ptr->stack_info, 0, sizeof(wms_ss_stack_info_s_type));
          }
        }
        else
        {
      ...
      else if(SYS_SYS_ID_TYPE_IS856 == cmd_ss_ptr->sys_id.id_type )
      {
#ifdef FEATURE_CDSMS
        appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode( cmd_ptr->hdr.as_id, WMS_MESSAGE_MODE_GW );
        if ( NULL != appInfo_ptr )
...
#ifdef FEATURE_GWSMS
      // UMTS (TDS/WCDMA/GSM/LTE)
      else if ( SYS_SYS_ID_TYPE_UMTS == cmd_ss_ptr->sys_id.id_type )
      {
        cfg_s_ptr->active_bearers[WMS_BEARER_LTE] = ( (SYS_SRV_STATUS_SRV == cmd_ss_ptr->srv_status) && 
                                                      (SYS_SYS_MODE_LTE == cmd_ss_ptr->sys_mode) );

        lte_is_available                            = wms_cfg_lte_service_available( cmd_ss_ptr->srv_status,
                                                                                     cmd_ss_ptr->sys_mode );
        appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(SYS_MODEM_AS_ID_1, WMS_MESSAGE_MODE_CDMA);
        ...
        {
          //For 1xCSFB, disable 1x bearer when UE moves back to LTE.
          if ( (SYS_SYS_ID_TYPE_IS95 == appInfo_ptr->stack_info.sys_id.id_type) /*&&
               (appInfo_ptr->stack_info[stack_idx].asubs_id == cmd_ss_ptr->stack_info[stack_idx].asubs_id)*/ )
          {
            appInfo_ptr->srv_available = FALSE;
            cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_1X] = FALSE;
        ...
        if ( (NULL == appInfo_ptr) && 
             ( (SYS_SRV_STATUS_LIMITED == cmd_ss_ptr->srv_status) ||
                (SYS_SRV_STATUS_LIMITED_REGIONAL == cmd_ss_ptr->srv_status) ) )
        {
          MSG_ERROR_0("No valid as_id mapping yet - using default as_id"); 
           appInfo_ptr = wms_get_gw_appinfo_by_asid( cmd_ptr->hdr.as_id );
        ...
        /*
          ...
          // Allow ETWS and CMAS when in limited service without SIM initialization
          if ( (SYS_SRV_STATUS_LIMITED == cmd_ss_ptr->srv_status) ||
               (SYS_SRV_STATUS_LIMITED_REGIONAL == cmd_ss_ptr->srv_status) )
          {
            if (!appInfo_ptr->sim_init_complete)
            {
          ...
          if (SYS_MODEM_AS_ID_1 == cmd_ptr->hdr.as_id)
          {
            ...
            if (NULL != appInfo_ptr)
            {
              MSG_HIGH_0("Clear up CDMA appInfo_ptr->stack_info on SYS_MODEM_AS_ID_1");
              memset(&appInfo_ptr->stack_info, 0, sizeof(wms_ss_stack_info_s_type));
            }
          }
        }
  ...
  */
       ...
       
  {
    ...
    if ( NULL != appInfo_ptr && SYS_MODEM_AS_ID_1 == appInfo_ptr->asid ) 
    {
      ...
      if ( (SYS_SYS_MODE_LTE == appInfo_ptr->stack_info.sys_mode) )
      {
