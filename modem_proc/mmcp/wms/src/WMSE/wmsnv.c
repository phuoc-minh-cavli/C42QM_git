 /*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmsnv.c#1 $
 ...
 */
char *wms_config_items = NULL;

#ifdef FEATURE_WMS_EFS_ENABLE
char *wms_config_items = "/nv/item_files/modem/sms/enable_fdn_control\n\
...
extern boolean asid_is_valid(sys_modem_as_id_e_type asid);
...
uint16 wms_nv_convert_to_context(sys_modem_as_id_e_type as_id)
{
  ...
  MSG_HIGH_1("Returning default context for asid %d: 0", as_id);
  return 0;
}
  ...
  */
  if( ret_status == NV_NOTACTIVE_S || ret_status == NV_FAIL_S )
  {
    ...
    switch( item )
    {
      ...
      default:
        MSG_HIGH_1( "Nonactive NV item 0x%x", item);
...
)
{
  ...
  {
    MSG_ERROR_0("Could not authenticate storage.  Read failed");
    return FALSE;
  }
  ...
  else /* File opened successfully for reading ... */
  {
    ...
    if ( NULL == packed_data )
    {
      MSG_ERROR_0("wms_mem_malloc returned NULL");
...
)
{
  ...
  if ( !wms_nv_cd_msg_store_authenticate() )
  {
    MSG_ERROR_0("Could not authenticate storage.  Write failed");
    return FALSE;
  }
...
)
{
  ...
  {
    MSG_ERROR_0("wns_nv_replace_cd_sms failed");
    return FALSE;
  }
...
)
{
  ...
  if ( !wms_nv_cd_msg_store_authenticate() )
  {
    MSG_ERROR_0("Could not authenticate storage.  Delete failed");
    return FALSE;
  }
...
)
{
  ...
  {
    MSG_ERROR_0("Write NV_SMS_CD_PARMS_I failed");
    res = FALSE;
  }
...
)
{
  ...
  {
    MSG_ERROR_0("wns_nv_replace_gw_sms failed");
    return FALSE;
  }
...
)
{
  ...
  else /* File opened successfully for deleting msg ... */
  {
    ...
    if ( NULL == clear_data )
    {
      MSG_ERROR_0("wms_mem_alloc returned NULL");
      (void) mcfg_fclose( fd, MCFG_FS_TYPE_EFS );
      return FALSE;
    }
...
#else
    if( wms_nv_read_wait_per_subs(SYS_MODEM_AS_ID_1, NV_SMS_CFG_ROUTING_I, & nvi ) != NV_DONE_S )
#endif
  {
...
)
{
  ...
  if( (nv_st = wms_nv_read_wait_per_subs( as_id, NV_SMS_GW_PARMS_I, & nvi )) == NV_DONE_S )
  {
    if (wms_ts_decode_smsp((uint8 *)nvi.sms_gw_parms.data,
                           ...
                           template_ptr) == FALSE)
    {
      MSG_ERROR_1("Encode NV_SMS_GW_PARMS_I failed for as id = %d", as_id);
      res = FALSE;
    }
    /*
...
)
{
  ...
  {
    MSG_ERROR_1("Encode NV_SMS_GW_PARMS_I failed as id = %d", as_id);
    res = FALSE;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_nv_init()");
  }
...
#ifdef FEATURE_MODEM_CONFIG_REFRESH
  {
    ...
    MSG_HIGH_0("Register for NV refresh: mcfg_refresh_register()");
  ...
  /* Init for NV params */
#ifdef FEATURE_WMS_RC_INIT_DISABLE
  ...
  if (NULL != cdma_template_ptr)
  {
    ...
    wms_mem_free (cdma_template_ptr );
	cdma_template_ptr = NULL;
  }
#endif
...
static void wms_nv_determine_max_gw_slots( void )
{
  ...
  MSG_HIGH_0( "wms_nv_determine_max_gw_slots()");
...
static uint8 wms_nv_gw_msg_store_authenticate( void )
{
  ...
  MSG_HIGH_0( "wms_nv_gw_msg_store_authenticate()");
...
static uint8 wms_nv_gw_msg_store_is_version_valid( void )
{
  ...
  MSG_HIGH_0( "wms_nv_gw_msg_store_is_version_valid()");
...
static uint8 wms_nv_gw_msg_store_init ( void )
{
  ...
  MSG_HIGH_0( "wms_nv_gw_msg_store_init()");
...
static uint8 wms_nv_gw_msg_store_create_header_file ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_gw_msg_store_create_header_file()");
...
static uint8 wms_nv_gw_msg_store_create_data_file ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_gw_msg_store_create_data_file()");
...
static uint8 wms_nv_gw_msg_store_update_header_file ( wms_message_index_type index, uint8 tag )
{
    ...
    MSG_HIGH_0( "wms_nv_gw_msg_store_update_header_file()");
...
static uint8 wms_nv_gw_msg_store_read_tags ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_gw_msg_store_read_tags()");
    ...
    else
    {
        ...
        if ( NULL == tags )
        {
           MSG_ERROR_0("wms_mem_alloc failed");
           (void) mcfg_fclose( fd, MCFG_FS_TYPE_EFS );
           return FALSE;
        }
...
static uint8 wms_nv_cd_msg_store_authenticate( void )
{
  ...
  MSG_HIGH_0( "wms_nv_cd_msg_store_authenticate()");
...
static uint8 wms_nv_cd_msg_store_is_version_valid( void )
{
  ...
  MSG_HIGH_0( "wms_nv_cd_msg_store_is_version_valid()");
...
static uint8 wms_nv_cd_msg_store_init ( void )
{
  ...
  MSG_HIGH_0( "wms_nv_cd_msg_store_init()");
...
static uint8 wms_nv_cd_msg_store_create_header_file ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_cd_msg_store_create_header_file()");
...
static uint8 wms_nv_cd_msg_store_create_data_file ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_cd_msg_store_create_data_file()");
...
static uint8 wms_nv_cd_msg_store_update_header_file ( wms_message_index_type index, uint8 tag )
{
    ...
    MSG_HIGH_0( "wms_nv_cd_msg_store_update_header_file()");
...
static uint8 wms_nv_cd_msg_store_read_tags ( void )
{
    ...
    MSG_HIGH_0( "wms_nv_cd_msg_store_read_tags()");
...
)
{
  ...
  if ( NULL == file_path )
  {
    MSG_HIGH_0("EFS file map failed");
    return;
  }
  ...
  MSG_HIGH_1( "Maximum no. of messages allowed in ECBM: %d", lcfg_s_ptr->mt_msg_limit_in_ecbm_mode );
...
)
{
  ...
  {
    // Failed to read, item is 'inactive'
    MSG_HIGH_1( "mcfg_fs_read(wms_nv_efs_read_sms_allowed_in_ecbm) status %d", res );
  ...
  MSG_HIGH_1( "SMS is allowed in ECBM mode: %d", lcfg_s_ptr->sms_allowed_in_ecbm );
...
)
{
  ...
  {
	MSG_HIGH_1("invalid as_id:%d",as_id);
    return FALSE;          
  }
  ...
  if ( !wms_nv_refresh_item.has_been_read[as_id] )
  {
    ...
    {
      /* Open/Read file */
      MSG_HIGH_1( "Open/Read file mcfg_fs_read(refresh_vote_ok) = %s", file_path );
...
static void wms_nv_efs_read_enable_fdn_control(void)
{
  ...
  MSG_HIGH_1("FDN setting = %d", cfg_s_ptr->enable_fdn_control);
}
...
void wms_nv_efs_read_store_to_sim_if_nv_full ( void )
{
  ...
  MSG_HIGH_1("WMS_NV_EFS_STORE_TO_SIM_IF_NV_FULL setting = %d", cfg_s_ptr->store_to_sim_if_nv_full);
...
static void wms_nv_efs_read_CS_domain_fallback(void)
{
  ...
  if ( MCFG_FS_STATUS_OK != res )
  {
    ...
    if ( MCFG_FS_STATUS_OK != res )
    {
      MSG_ERROR_1( "mcfg_fs_write status = %d", res);
    }
  }

  MSG_HIGH_1("CS domain fallback on LTE = %d", cfg_s_ptr->cs_domain_fallback_on_lte);
}
#endif /* FEATURE_GWSMS */
...
static void wms_nv_efs_read_1x_sms_and_voice(void)
{
  ...
  MSG_HIGH_1("1x SMS and Voice Allowed = %d", cfg_s_ptr->simul_sms_voice_over_1x_allowed);
}
#endif
...
)
{
  ...
  MSG_HIGH_1("Telecom EFsmsp fallback allowed = %d", fallback_value);
...
static void wms_nv_efs_read_sms_retry_limit(void)
{
...
#else
  const char *file_path = "/nv/item_files/modem/sms/sms_retry_limit";
#endif
  /* Open/Read file */
  MSG_HIGH_1( "Open/Read file=%s", file_path);
  ...
  MSG_HIGH_1("SMS retry limit = %d", cfg_s_ptr->sms_retry_limit);
}
...
void wms_nv_efs_read_disable_lte_cb_dup_detection ( void )
{
  ...
  MSG_HIGH_1("WMS_NV_EFS_DISABLE_LTE_CB_DUP_DETECTION setting = %d", cfg_s_ptr->lte_cb_dup_detection_is_disabled);
...
void wms_nv_efs_read_disable_pres_bc_alert ( void )
{
  ...
  MSG_HIGH_1("WMS_NV_EFS_DISABLE_PRES_BC_ALERT setting = %d", cfg_s_ptr->presidential_bc_alert_is_disabled);
...
void wms_nv_efs_read_stk_sms_enable_retry ( void )
{
  ...
  /* Open/Read file */
  MSG_HIGH ( "Open/Read file=%s",file_path, 0, 0 );
  ...
  MSG_HIGH_1("WMS_NV_EFS_STK_SMS_RETRY_ENABLE setting = %d",cfg_s_ptr->stk_sms_retry_enable);
...
void wms_nv_efs_init(void)
{
  ...
  if ( MCFG_FS_STATUS_OK != mcfg_fs_stat( WMS_EFS_CONF_F_NAME, &file_stat, MCFG_FS_TYPE_EFS, MCFG_FS_SUBID_NONE ) )
  {
    ...
    {
      MSG_HIGH_1( "Failed to open wms.conf error %d", conf_fd);
    }
      ...
      case POLICYMAN_ITEM_DEVICE_CONFIGURATION:
        {
          ...
          {
            MSG_HIGH_0("No change in device mode");
          }
      ...
      case POLICYMAN_ITEM_UE_MODE:
        {
          ...
          {
            MSG_HIGH_0("No change in UE mode");
          }
       ...
       default:
         MSG_HIGH_1("Ignoring change in policy item %d", pitem_id);
         break;
    }
  } /* for */
...
)
{
  ...
  policyman_item_id_t const  id           = POLICYMAN_ITEM_DEVICE_CONFIGURATION;
#ifndef FEATURE_WMS_GLOBAL_RETAIN
  ...
  if (POLICYMAN_SUCCEEDED(status))
  {
    ...
    for( as_id = SYS_MODEM_AS_ID_1; ((size_t)as_id < num_of_sims); as_id++ )
    {
      ...
      if(POLICYMAN_SUCCEEDED(policyman_device_config_get_subs_feature( pItem, 
                                                                       ...
                                                                       &temp_sub_feature_mode )))
      {
        lcfg_s_ptr->sub_feature_mode = temp_sub_feature_mode;
      }
      else
      {
        lcfg_s_ptr->sub_feature_mode = SYS_SUBS_FEATURE_MODE_NORMAL;
      }
#endif
  }
...
)
{
  ...
  MSG_HIGH_0( "In wms_nv_refresh_cb" );
  ...
  {
    MSG_ERROR_0("Out of cmd buf for wms_nv_refresh_cb");
    return FALSE;
  }
