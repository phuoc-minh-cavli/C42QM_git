/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wms.c#1 $
...
#define WMS_OFFSETOF(type,field) __builtin_offsetof(type, field)
...
static ULogHandle wms_ulog_handle = NULL;
...
#endif
extern void qmi_wmsi_init(void);
extern boolean qmi_wms_sig_hdlr( qmi_wms_sig_enum_type, void * );
extern boolean qmi_wms_qcsi_sig_hdlr( qmi_wms_sig_enum_type);
...
ULogHandle wms_get_ulog_handle()
{
  return wms_ulog_handle;
}
...
static void wms_msgr_init(void)
{
  ...
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_msgr_init()");
  }
...
#if !defined(FEATURE_WMS_LTE_ONLY) && defined (FEATURE_DOMAIN_SELECTION)
  ...
  /* Register with MSG ROUTER for DS  loop back notification */
  msgr_error = msgr_register( MSGR_WMS_DS_3GPP, &wms_msgr_client, wms_msgr_rex_q_id, DS_3GPP_TLB_MODE_H_ACT_DEACT_IND );
  if (msgr_error != E_SUCCESS)
  {
      MSG_ERROR_1("msgr_register(DS_3GPP_TLB_MODE_H_ACT_DEACT_IND) failed with msgr error = %d", msgr_error);
  }

  /* Register with MSG ROUTER for UE entering PSM deep sleep mode */
  msgr_error = msgr_register(MSGR_WMS_CM, &wms_msgr_client, wms_msgr_rex_q_id, MM_CM_PSM_DEEP_SLEEP_ENTER_IND);
  if (msgr_error != E_SUCCESS)
  {
      MSG_ERROR_1("msgr_register(MM_CM_PSM_DEEP_SLEEP_ENTER_IND) failed with msgr error = %d", msgr_error);
  }

  /* Register with MSG ROUTER for UE exiting PSM deep sleep mode */
  msgr_error = msgr_register(MSGR_WMS_CM, &wms_msgr_client, wms_msgr_rex_q_id,  MM_CM_PSM_DEEP_SLEEP_EXIT_IND);
  if (msgr_error != E_SUCCESS)
  {
      MSG_ERROR_1("msgr_register(MM_CM_PSM_DEEP_SLEEP_EXIT_IND) failed with msgr error = %d", msgr_error);
  }
...
)
{
  ...
  while ((rpt_ptr = q_get(&wms_msgr_q)) != NULL)
  {
   ...
   // currently getting compilation error once DS support will enable it.
     case DS_3GPP_TLB_MODE_H_ACT_DEACT_IND:
	   {
		 wms_cfg_s_type   *cfg_s_ptr = wms_cfg_s_ptr();		
		 cfg_s_ptr->loop_back_notify = rpt_ptr->cmd.ds_loop_back_notify.act_status ;
		  MSG_HIGH_1( "loop_back_notification from DS with status:%d",cfg_s_ptr->loop_back_notify);
      ...
      case MM_CM_PSM_DEEP_SLEEP_ENTER_IND:
         {
           wms_cfg_s_type   *cfg_s_ptr = wms_cfg_s_ptr();
           cfg_s_ptr->in_psm_deep_sleep  = TRUE;
           MSG_HIGH_1("Received Deep Sleep Enter IND (in_psm_deep_sleep = %d)", cfg_s_ptr->in_psm_deep_sleep);
         }
        break;

      case MM_CM_PSM_DEEP_SLEEP_EXIT_IND:
         {
           wms_cfg_s_type   *cfg_s_ptr = wms_cfg_s_ptr();
           cfg_s_ptr->in_psm_deep_sleep  = FALSE;
           MSG_HIGH_1("Received Deep Sleep Exit IND (in_psm_deep_sleep = %d)", cfg_s_ptr->in_psm_deep_sleep);
         }
        break;

      default:
        MSG_HIGH_1("Receive unexpected msg id %d", rpt_ptr->cmd.msg_hdr.id);
        break;
    }
...
void wms_ulog_register(void)
{
  ULogResult wms_buffer_status;
  wms_buffer_status = ULogFront_RealTimeInit(&wms_ulog_handle,
                               "WMS_ULOG",  
                               512, 
                               ULOG_MEMORY_LOCAL,
                               ULOG_LOCK_OS);
  MSG_HIGH_1("ULOG REGISTER wms_buffer_status: %d", wms_buffer_status);
}
...
)
{
  ...
  MSG_HIGH_0("RCINIT_DBG: Calling rcinit_handshake_startup()");
  ...
  MSG_HIGH_0("RCINIT_DBG: Calling wms_init_before_task_start() from wms_init");
...
)
{
...
#ifdef FEATURE_WMS_DEBUG_ENABLE
#ifndef FEATURE_WMS_LTE_ONLY
  char               *cmd_str;
  ...
  if (NULL == cmd_ptr->hdr.appInfo_ptr)
  {
    switch (cmd_ptr->hdr.cmd_id)
    {
      ...
      default:
      {
        ...
        MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "Command found without appInfo_ptr: %d",
                       cmd_ptr->hdr.cmd_id);
      }
    }
      ...
      default:
        {
          ...
          ret_cmd_err = WMS_CMD_ERR_BUSY;
          MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "Command not allowed during refresh: %d",
                         cmd_ptr->hdr.cmd_id);
        }
        break;
    }
       ...
       (!cmd_ptr->hdr.appInfo_ptr->sim_init_complete) )
  {
    ...
    {
      MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "1X init is not complete: %d",
                     cmd_ptr->hdr.cmd_id);
    }
       ...
       (!cmd_ptr->hdr.appInfo_ptr->sim_init_complete) )
  {
    ...
    default:
        MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Unhandled command, reporting busy: %d",
                       cmd_ptr->hdr.cmd_id);
        ret_cmd_err = WMS_CMD_ERR_BUSY;
        break;
    }
    ...
    if (WMS_CMD_ERR_NONE != ret_cmd_err)
    {
      MSG_SPRINTF_2( MSG_SSID_DFLT, MSG_LEGACY_ERROR, "GW init is not complete: %d, asid %d",
                     cmd_ptr->hdr.cmd_id,
...
)
{
...
#endif
#endif
  MSG_SPRINTF_4( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Putting %d (client %d, as_id %d, appInfo_ptr 0x%08x)",
                 cmd_ptr->hdr.cmd_id,
  ...
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_4(wms_get_ulog_handle(), "Putting %d (client %d, as_id %d, appInfo_ptr 0x%08x)",
                 cmd_ptr->hdr.cmd_id,
                 cmd_ptr->hdr.client_id,
                 cmd_ptr->hdr.as_id,
                 cmd_ptr->hdr.appInfo_ptr);
...
)
{
  ...
  MSG_SPRINTF_4( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Processing %d (client %d, as_id %d, appInfo_ptr 0x%08x)",
                 cmd_ptr->hdr.cmd_id,
                 cmd_ptr->hdr.client_id,
                 cmd_ptr->hdr.as_id,
                 cmd_ptr->hdr.appInfo_ptr );
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_4(wms_get_ulog_handle(), "Processing %d (client %d, as_id %d, appInfo_ptr 0x%08x)",
                 cmd_ptr->hdr.cmd_id,
                 cmd_ptr->hdr.client_id,
                 cmd_ptr->hdr.as_id,
                 cmd_ptr->hdr.appInfo_ptr);
...
)
{
  ...
  if (0 != (sigs & TASK_STOP_SIG))
  {
	...
	(void) msgr_deregister( MSGR_WMS_DS_3GPP,
                            &wms_msgr_client,
                            DS_3GPP_TLB_MODE_H_ACT_DEACT_IND );

     (void) msgr_deregister( MSGR_WMS_CM,
                             &wms_msgr_client,
  ...
  if(sigs & QMI_WMS_SERVICE_WMS_SIGNAL) 
  {        
    MSG_HIGH_0("QMI_WMS_SERVICE_WMS_SIGNAL");
    (void) rex_clr_sigs( wms_myself, QMI_WMS_SERVICE_WMS_SIGNAL );
    
    if ( qmi_wms_sig_hdlr((qmi_wms_sig_enum_type) QMI_WMS_SERVICE_WMS_SIGNAL, NULL) == FALSE)
     {
         MSG_HIGH_1("qmi_wms_task(): QMI_WMS_SERVICE_SIGNAL %d", QMI_WMS_SERVICE_WMS_SIGNAL);
          wms_sigs = rex_get_sigs(wms_myself);
  ...
  if(sigs & QMI_WMS_SERVICE_WMS_QCSI_SIGNAL)
  {
    MSG_HIGH_0("QMI_WMS_SERVICE_WMS_QCSI_SIGNAL");

    (void) rex_clr_sigs( wms_myself, QMI_WMS_SERVICE_WMS_QCSI_SIGNAL );

    if ( qmi_wms_qcsi_sig_hdlr((qmi_wms_sig_enum_type) QMI_WMS_SERVICE_WMS_QCSI_SIGNAL) == FALSE)
     {
         MSG_HIGH_1("qmi_wms_task(): QMI_WMS_SERVICE_QCSI_SIGNAL %d", QMI_WMS_SERVICE_WMS_QCSI_SIGNAL);
         wms_sigs = rex_get_sigs(wms_myself);
...
#ifndef FEATURE_WMS_TASK_MERGE_DISABLE
  (void) rex_set_sigs( wms_myself, QMI_WMS_SERVICE_WMS_QCSI_SIGNAL );
  (void) rex_set_sigs( wms_myself, QMI_WMS_SERVICE_WMS_SIGNAL );
#endif				    
#ifdef FEATURE_MODEM_RCINIT
...
)
{
  ...
  if (cmd_err != WMS_CMD_ERR_NONE)
  {
    ...
    MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Error: %d",
                   cmd_err);
  }
...
)
{
  ...
  do
  {
	...
	MSG_HIGH_5( "In wms_msg_ms_send_common(client %d, as_id %d, send_mode %d message_mode %d transport_type %d)",
              client_id,
              as_id,
              send_mode,
              message_ptr->msg_hdr.message_mode,
              transport_type);
...
wms_status_e_type wms_msg_ms_build_and_send_cmd(
...
)
{
    ...
    if(client_id == WMS_CLIENT_TYPE_QMI)  
 	{
      wms_process_cmd(cmd_ptr);
 	}
    else
    {
      	wms_put_cmd( cmd_ptr );
...
)
{
  ...
  do
  {
	...
	MSG_HIGH_3( "In wms_msg_ms_ack_ext(client %d, as_id %d message_mode %d)",
              client_id,
              as_id,
              ack_info_ptr->message_mode);
...
#endif
  st = wms_msg_ms_build_and_send_cmd(client_id,as_id,cmd_cb,user_data,mem_store,index,cmd_id,0);
  return st;
} /* wms_msg_ms_read */
...
)
{
  ...
  do
  {
	...
	MSG_HIGH_3( "In wms_msg_ms_write(client %d, as_id %d , mem_store %d)",
              client_id,
              as_id,
              message_ptr->msg_hdr.mem_store);
...
)
{
  ...
  do
  {
	...
	MSG_HIGH_3( "In wms_msg_ms_write_template(client %d, as_id %d, mem_store %d)",
              client_id,
              as_id,
              message_ptr->msg_hdr.mem_store);
...
#endif 
  st = wms_msg_ms_build_and_send_cmd(client_id,as_id,cmd_cb,user_data,mem_store,0,cmd_id,tag);
...
#endif
   st = wms_msg_ms_build_and_send_cmd(client_id,as_id,cmd_cb,user_data,mem_store,index,cmd_id,tag);
  return st;
...
#endif
    st = wms_msg_ms_build_and_send_cmd(client_id,as_id,cmd_cb,user_data,mem_store,0,cmd_id,0);
  return st;
...
)
{
  ...
  do
  {
	...
	MSG_HIGH_3( "In wms_msg_ms_write_sts_report(client %d, as_id %d, mem_store %d)",
              client_id,
              as_id,
              message_ptr->msg_hdr.mem_store);
...
#endif 
      st =  wms_debug_send_cmd(client_id,cmd_cb,user_data,SYS_MODEM_AS_ID_1,WMS_MESSAGE_MODE_GW,WMS_CMD_CFG_GET_LINK_CONTROL);
#else /* FEATURE_GWSMS */
  wms_notify_client_cmd_status(cmd_cb, cmd_id, user_data, WMS_CMD_ERR_UNSUPPORTED);
  st = WMS_UNSUPPORTED_S;
#endif /* FEATURE_GWSMS */
...
)
{
  ...
  MSG_HIGH_1("wms_cfg_ms_set_message_waiting,as_id = %d in WMS context", as_id);
  if (NULL == mwi_info)
  {
    /* MWI Info is NULL */
    wms_notify_client_cmd_status(cmd_cb, cmd_id, user_data, WMS_CMD_ERR_NULL_PTR);
...
wms_status_e_type wms_debug_send_cmd(  wms_client_id_type              client_id,
  wms_cmd_cb_type                 cmd_cb,
  const void                     *user_data,
    sys_modem_as_id_e_type     as_id ,
    wms_message_mode_e_type    messag_mode,
     wms_cmd_id_e_type   cmd_id 
  ...
  )
  {
   ...
   do
   {
     ...
     appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode(as_id, messag_mode);
...
#endif
   st =  wms_debug_send_cmd(client_id,cmd_cb,user_data,SYS_MODEM_AS_ID_1,WMS_MESSAGE_MODE_CDMA,WMS_CMD_DBG_RESET_TL_SEQ_NUM);
#else /* FEATURE_CDSMS */
  wms_notify_client_cmd_status(cmd_cb, cmd_id, user_data, WMS_CMD_ERR_UNSUPPORTED );
  st = WMS_UNSUPPORTED_S;
#endif /* FEATURE_CDSMS */
...
#endif
    st =  wms_debug_send_cmd(client_id,cmd_cb,user_data,SYS_MODEM_AS_ID_1,WMS_MESSAGE_MODE_GW,WMS_CMD_DBG_SET_MSG_REF_NUM);
#else /* FEATURE_GWSMS */
  wms_notify_client_cmd_status(cmd_cb, cmd_id, user_data, WMS_CMD_ERR_UNSUPPORTED );
  st = WMS_UNSUPPORTED_S;
#endif /* FEATURE_GWSMS */
...
#endif
     st =  wms_debug_send_cmd(client_id,cmd_cb,user_data,SYS_MODEM_AS_ID_1,WMS_MESSAGE_MODE_GW,WMS_CMD_DBG_CLEAR_SMMA);
#else /* FEATURE_GWSMS */
  wms_notify_client_cmd_status(cmd_cb, cmd_id, user_data, WMS_CMD_ERR_UNSUPPORTED );
  st = WMS_UNSUPPORTED_S;
#endif /* FEATURE_GWSMS */
