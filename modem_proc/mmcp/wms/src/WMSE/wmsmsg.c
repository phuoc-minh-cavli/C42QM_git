/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmsmsg.c#1 $
...
wms_msg_transport_verstat_status_e_type wms_get_ims_verstat_status(uint8 status)
{
   ...
   {
     case 0:
      wms_status = WMS_VERSTAT_STATUS_NO_VALIDATION;
      break;

     case 1:
      wms_status = WMS_VERSTAT_STATUS_VALIDATION_PASS;
      break;

     case 2:
      wms_status = WMS_VERSTAT_STATUS_VALIDATION_FAIL;
      break;

     default:
      wms_status = WMS_VERSTAT_STATUS_VALIDATION_MAX;
      break;
   }
...
)
{
   ...
   if ( (SYS_MODEM_AS_ID_NONE >= as_id) || (MAX_AS_IDS <= as_id) )
   {
     MSG_ERROR("Invalid AS ID %d", as_id, 0, 0);
...
)
{
   ...
   for (i = lower_bound; i < upper_bound; i++)
   {
      if (NULL == cfg_s_ptr->ram_gw_sms[i])
      {
         ...
         if(NULL == cfg_s_ptr->ram_gw_sms[i])
         {
            MSG_ERROR_0("wms_mem_malloc failed");
...
#ifdef FEATURE_CDSMS
#ifdef FEATURE_WMS_PP_DOWNLOAD_ENABLE
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  MSG_HIGH_2( "MSG: client_id=%d, event=%d",
              client_id,
              event );

  if( WMS_PM_NOTIFY_ALL == client_ptr->msg_notify_type )
  {
    MSG_HIGH_0("PM: The client requested to send all events");
  ...
  {
    MSG_HIGH_0("PM: The client's customized filter allows this event");
    ok_to_send_event = TRUE;
  }
  ...
  {
     MSG_HIGH_0("Modem / Apps Client without PM");
     ok_to_send_event = TRUE;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  MSG_SPRINTF_1(MSG_SSID_DFLT, MSG_LEGACY_HIGH, "Notify: %d", event);
...
#if defined (FEATURE_FUZZ_TEST) && !defined (FEATURE_WMS_LTE_ONLY)
...
static wms_msg_cdma_message_record_type* wms_msg_cdma_get_free_mt_rec(void)
{
  ...
  for( i=0; i<WMS_MAX_MT_MESSAGES; i++ )
  {
    if( NULL == msg_s_ptr->cdma_mt_rec[i] )
    {
       ...
       if(NULL == msg_s_ptr->cdma_mt_rec[i])
       {
          MSG_HIGH_0("wms_mem_malloc failed");
          break;
       }
...
//#endif
   ...
   if (WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS != cmd_ptr->cmd.msg_send.transport_type &&
        ( (NULL != cfg_s_ptr) && cfg_s_ptr->lpm) )
    {   
      return WMS_CMD_ERR_MSG_LPM;
    }

#ifndef FEATURE_WMS_LTE_ONLY
   /* Special handling: Reject non-IMS MO SMS if device is in HVoLTE mode and VoLTE call is active on-going on same sub, since
       1X bearer won't be available. */
   if ( ( WMS_MSG_TRANSPORT_TYPE_SMS_OVER_IMS != cmd_ptr->cmd.msg_send.transport_type ) && 
          wms_cfg_check_hvolte_oper_mode()                                              &&
        ( ( NULL != cfg_s_ptr ) && ( !cfg_s_ptr->active_bearers[WMS_BEARER_CDMA_1X] ) ) &&
...
)
{
  ...
  if ( NULL != appInfo_ptr) 
  {
...
#ifdef FEATUTRE_NEW_CM_INTERFACE_DISABLE
    ...
    for ( stack_idx = 0; stack_idx < appInfo_ptr->number_of_stacks; stack_idx++ ) 
    {
      ...
      if  ( ((WMS_GW_DOMAIN_PREF_CS_ONLY == appInfo_ptr->curr_gw_domain_pref)  ||
               (WMS_GW_DOMAIN_PREF_CS_PREFERRED == appInfo_ptr->curr_gw_domain_pref)) &&
              ((SYS_SRV_DOMAIN_CS_ONLY == appInfo_ptr->stack_info.srv_domain) || 
               (SYS_SRV_DOMAIN_CS_PS == appInfo_ptr->stack_info.srv_domain)) )
        {
          return &appInfo_ptr->stack_info;
        }

        if  ( ((WMS_GW_DOMAIN_PREF_PS_ONLY == appInfo_ptr->curr_gw_domain_pref)  || 
               (WMS_GW_DOMAIN_PREF_PS_PREFERRED == appInfo_ptr->curr_gw_domain_pref)) &&
              ((SYS_SRV_DOMAIN_PS_ONLY == appInfo_ptr->stack_info.srv_domain) || 
               (SYS_SRV_DOMAIN_CS_PS == appInfo_ptr->stack_info.srv_domain)) )
        {
          return  &appInfo_ptr->stack_info;
        }
    ...
    else /* SEND FROM MEMORY */
    {
      switch (msg_s_ptr->gw_mo_rec[rec_index]->msg_hdr.mem_store)
      {
        ...
        case WMS_MEMORY_STORE_NV_GW:
          ...
          {
            sim_sms.status = sim_data[0];
            sim_sms.len = (uint8)(WMS_SIM_SMS_MAX-1);
          ...
          else
          {
            ...
            else
            {
              ...
              if ( GSTK_ACCESS_TECH_LTE == mo_sms_control_buf.rat )
              {
                ...
                ** ---------------------------------------- */
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[0] = 0xFF;

                /* ----------------------------------------
                ** PLMN octet 2 = MNC digit 3 | MCC digit 3
                ** ---------------------------------------- */
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[1] = 0xFF;

                /* ----------------------------------------
                ** PLMN octet 3 = MNC digit 2 | MNC digit 1
                ** ---------------------------------------- */
                mo_sms_control_buf.loc_info.gwl_info.mcc_and_mnc[2] = 0xFF;
                ...
                mo_sms_control_buf.loc_info.gwl_info.loc_area_code[0] = 0xFF; 
                mo_sms_control_buf.loc_info.gwl_info.loc_area_code[1] = 0xFF;
              }
              // Use RRC cell change event
              else 
#if !defined(FEATURE_WMS_LTE_ONLY) || defined (FEATURE_GSM)             
#if defined(FEATURE_WMS_LTE_ONLY)
...
#endif /* FEATURE_SMS_PWS_OVER_LTE */
			  if ( GSTK_ACCESS_TECH_GSM == mo_sms_control_buf.rat ) 
              {
    ...
    if(send_mode == WMS_SEND_MODE_MEMORY_STORE)
    {
      ...
      switch(msg_s_ptr->gw_mo_rec[rec_index]->msg_hdr.mem_store)
      {
        case WMS_MEMORY_STORE_RAM_GW:
          ...
          if( (NULL != cfg_s_ptr) && (NULL != cfg_s_ptr->ram_gw_sms[i]) )
          {
  ...
  */
  if(cmd_err == WMS_CMD_ERR_NONE)
  {
...
#endif /* FEATURE_GWSMS_MO_CONTROL */
    {
      ...
      if (WMS_CMD_ERR_NONE == cmd_err)
      {
        //Reject SMS if UE is in PSM deep sleep mode.
        if (cfg_s_ptr->in_psm_deep_sleep)
        {
           cm_psm_deep_sleep_exit_req_s_type cm_psm_exit;

           cmd_err = WMS_CMD_ERR_MSG_PSM;

           msgr_init_hdr((msgr_hdr_struct_type *)&cm_psm_exit,
                     MSGR_WMS_CM,
                     MM_CM_PSM_DEEP_SLEEP_EXIT_REQ );
  
           msgr_set_hdr_variant((msgr_hdr_struct_type*)&cm_psm_exit,(uint8)(mo_rec_ptr->as_id));

           if (E_SUCCESS != msgr_send((msgr_hdr_struct_type*)&cm_psm_exit,
                                  sizeof(cm_psm_deep_sleep_exit_req_s_type)))
           {
...
void wms_check_and_update_mo_record(  
      wms_cmd_type   *cmd_ptr,  wms_prov_app_info_s_type   *appInfo_ptr,
      wms_cmd_err_e_type cmd_err,int rec_index,
      sys_modem_as_id_e_type as_id   )
{

   if(cmd_ptr== NULL || appInfo_ptr == NULL)
   	{
...
)
{
  ...
  if (NULL == appInfo_ptr)
  {
     ...
     MSG_ERROR_1("Invalid AS ID %d", as_id);
     wms_client_cmd_status( cmd_ptr, cmd_err );
     return cmd_err;
  }
  ...
  {
#ifdef FEATURE_WMS_CDSMS_EANBLE
#ifdef FEATURE_CDSMS
...
#endif
      wms_check_and_update_mo_record(cmd_ptr,appInfo_ptr,cmd_err,rec_index,as_id);
      break;
...
)
{
  ...
  {
    cmd_err =  WMS_CMD_ERR_AS_ID;
    MSG_ERROR_1("Invalid AS ID %d", cmd_ptr->hdr.as_id);
    wms_client_cmd_status( cmd_ptr, cmd_err );
    return cmd_err;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
                   ...
                   == WMS_MESSAGE_MODE_GW )
  {
    ...
    if (cfg_s_ptr->in_psm_deep_sleep)
    {
      ...
      msgr_set_hdr_variant((msgr_hdr_struct_type*)&cm_psm_exit,(uint8)(cmd_ptr->hdr.as_id));
...
)
{
  ...
  switch (mem_store)
  {
    ...
    case WMS_MEMORY_STORE_RUIM:
      ...
      {
        cmd_err =  WMS_CMD_ERR_MSG_INDEX;
        MSG_ERROR_1("Invalid message index %d", cmd_ptr->cmd.msg_read.index);
      }
      else if(appInfo_ptr->sms_rec_tags[i] == WMS_TAG_NONE)
      {
        cmd_err =  WMS_CMD_ERR_MSG_EMPTY_MESSAGE;
...
#ifdef FEATURE_WMS_VOICE_MAIL_DISABLE
...
)
{
  ...
  switch( mem_store )
  {
    ...
    case WMS_MEMORY_STORE_RUIM:
      ...
      {
        cmd_err =  WMS_CMD_ERR_MSG_INDEX;
        MSG_ERROR_1("Invalid message index %d", cmd_ptr->cmd.msg_delete.index);
      }
      else if(appInfo_ptr->sms_rec_tags[i] == WMS_TAG_NONE)
      {
        cmd_err =  WMS_CMD_ERR_MSG_EMPTY_MESSAGE;
...
#ifdef FEATURE_WMS_GLOBAL_RETAIN
    ...
    case WMS_MEMORY_STORE_NV_GW:
      ...
      */
      ...
      {
        uint32 sms_rec_len = (uint32)(WMS_SIM_SMS_MAX-1);
        if (appInfo_ptr->sms_rec_len > 0)
        {
          sms_rec_len = (uint32)MIN(WMS_SIM_SMS_MAX-1, appInfo_ptr->sms_rec_len-1);
        }
...
#ifndef	FEATURE_WMS_UNUSED_QMI_API_ENABLE


#ifndef FEATURE_WMS_RC_INIT_DISABLE
#ifdef FEATURE_CDSMS
 extern wms_cdma_template_s_type            *cdma_template_ptr ;
#endif
 extern wms_gw_template_s_type              *gw_template_ptr;
void wms_read_cdma_template_from_nv_once()
{

#ifdef FEATURE_CDSMS
  uint8		*nv_cdma_data = NULL;
  static boolean read_once = FALSE;
...
void wms_read_gw_template_from_nv_once ()
{
  ...
  if(read_once == FALSE)
  {
     ...
     for (i = 0; i < j; i++) 
     {
        ...
        if (NULL != gw_template_ptr)
        {
		  ...
		  wms_mem_free( gw_template_ptr );
		  gw_template_ptr = NULL;
        }
     }
  }
...
void wms_read_retry_from_nv_once()
{
  static boolean read_once = FALSE;
...
/*===========================================================================
FUNCTION: wms_read_all_nv_related_once()                           (INTERNAL)

DESCRIPTION:
  This function reads below NVs only once before processing of the first command in WMS queue.
   -> CDMA template
   -> GW template
   -> Retry period
   -> Retry Interval
...
===========================================================================*/
void wms_read_all_nv_related_once()
{
  ...
  {
    read_once = TRUE;
	wms_read_cdma_template_from_nv_once();
	wms_read_gw_template_from_nv_once();
	wms_read_retry_from_nv_once();
  }
...
)
{
      ...
      if (cmd_err == WMS_CMD_ERR_NONE )
      {
        ...
        if (WMS_CMD_ERR_NONE == cmd_err )
        {
          ...
          if (template_ptr->alpha_id.len > 0 && NULL != template_ptr->alpha_id.data)
          {
            ...
            appInfo_ptr->gw_template.alpha_id.len  = template_ptr->alpha_id.len;
          }
          else
          {
          ...
          if ((NULL != appInfo_ptr->gw_template.alpha_id.data) && (NULL != template_ptr->alpha_id.data))
          {
  ...
  if (cmd_err == WMS_CMD_ERR_NONE )
  {
    ...
    {
      (void)wms_msg_event_notify( WMS_MSG_EVENT_SMSC_ADDR_CHANGE, &msg_event_info );
    }
...
)
{
  ...
  msg_event_info_ptr->status_info.client_id                       = (wms_client_type_e_type) WMS_CLIENT_TYPE_QMI;
...
)
{
  ...
  switch (mem_store)
  {
    ...
    case WMS_MEMORY_STORE_USIM:
      ...
      {
        cmd_err =  WMS_CMD_ERR_MSG_INDEX;
        MSG_ERROR_1("Invalid message index %d", idx);
        break;
      }
...
/* --------------------------------------------------------- */
    ...
    MSG_HIGH_1("got ack st=0x%lx", cmd_ptr->cmd.mc_mo_status.status);
    MSG_HIGH_1("seq=%d", rec_ptr->seq_num);
...
/* -------------------------------------------------------- */
    ...
    MSG_HIGH_1("got MO status = %d", cmd_ptr->cmd.mc_mo_status.status);
    MSG_HIGH_1("seq=%d", rec_ptr->seq_num);
    MSG_HIGH_1("tid=0x%lx", rec_ptr->user_data);
...
#if defined (FEATURE_CDSMS_BROADCAST) && !defined (FEATURE_WMS_LTE_ONLY)
#ifdef	FEATURE_WMS_CB_ENABLE
...
)
{
  ...
  {
	  ULOG_RT_PRINTF_2(wms_get_ulog_handle(), "wms_msg_transport_init transport_type: %d, transport_cap: %d", 
              transport_type, transport_cap );
  }
...
)
{
  ...
  if (NULL != msg_event_ptr)
  {
    if (NULL != transport_ptr)
    {
      ...
      MSG_HIGH_3( "Current transport cap = %d; transport capability notified to clients = %d allocated ptr = 0x%x",
                  transport_ptr->cap,
      ...
      if (NULL != client_ptr)
      {
        ...
        {
          MSG_HIGH_0("calling client cb function");
...
)
{
  ...
  else
  {
    ...
    if(wms_get_ulog_handle() != NULL)
    {
      ULOG_RT_PRINTF_1(wms_get_ulog_handle(), " TRANSPORT %d deregisters with WMS.", transport_id);
    }
  }
  ...
  switch (rpt_ind_ptr->ind_type)
  {
   ...
   case WMS_MSG_TRANSPORT_MT_SMS_IND:
     ...
     if (WMS_FORMAT_GW_PP == rpt_ind_ptr->mt_sms.format)
     {
        ...
        mn_msg.ims_verstat_status = rpt_ind_ptr->ims_verstat_status;
        mn_msg.rpdu_length = (byte) MIN(rpt_ind_ptr->mt_sms.data_len, sizeof(mn_msg.rpdu));

        MSG_HIGH_1("Verstat status indicated by IMS = %d", mn_msg.ims_verstat_status);
...
)
{
  ...
  {
    MSG_HIGH_1("Invalid asid = %d", as_id);
    return WMS_GENERAL_ERROR_S;          
  }
  ...
  do
  {
    ...
    if (WMS_OK_S == st)
    {
      ...
      (void)memscpy (msg_event_info.transport_mwi_info.msg_account_addr,
                     msg_event_info.transport_mwi_info.address_len,
...
#ifndef	FEATURE_WMS_UNUSED_QMI_API_ENABLE    // enable the below 2 command for MT message let see if it is the reason for failure disable later 
...
)
{
  ...
  */
  if (wms_msg_get_free_mt_record( &i ) == FALSE)
  {
    ...
    MSG_ERROR_0("WMS MT FAILED, ");
    MSG_ERROR_0("TP CAUSE : WMS_TP_CAUSE_ERROR_IN_MS, ");
    MSG_ERROR_0("RP_CAUSE: WMS_RP_CAUSE_TEMPORARY_FAILURE, ");
  }
  else
  {
    gstk_sms_tpdu_type  gstk_sms_tpdu;
      ...
      */
      ...
      else
      {
         ...
         if( gstk_status != GSTK_SUCCESS )
         {
           ...
           MSG_ERROR_0("TP CAUSE : WMS_TP_CAUSE_SIM_DATA_DOWNLOAD_ERROR, ");
           MSG_ERROR_0("RP_CAUSE: WMS_RP_CAUSE_PROTOCOL_ERROR_UNSPECIFIED, ");
         }
  ...
  switch( mem_store )
  {
    case WMS_MEMORY_STORE_RAM_GW:
      ...
      for ( msg_index = 0; msg_index < max_msg_slots; msg_index++ )
      {
        ...
        if (WMS_CMD_ERR_NONE != cmd_err)
        {
          MSG_ERROR_1("Invalid message index %d", msg_index);
...
)
{
  ...
  */
  ...
  msg_event_info.mt_message_info.transport_type               = transport_type;
  msg_event_info.mt_message_info.verstat_status               = wms_get_ims_verstat_status(ims_verstat_status);
  ...
  /* if the client ID is DS and loop back activation is received form DS, then always send the MT message to DS alone */

  if(parsed_client == WMS_CLIENT_TYPE_DS_3GPP && cfg_s_ptr->loop_back_notify)
  {
    MSG_HIGH_1("MT SMS: Message Consumed by DS with  Client id: %d", parsed_client);
  ...
  switch( routing.route )
  {
    ...
    case WMS_ROUTE_TRANSFER_AND_ACK:
     ...
     /* For DS client need to send the ack report to DS..then only DS will trigger the MO SMS if there are multiple packets*/
     if(parsed_client == WMS_CLIENT_TYPE_DS_3GPP && cfg_s_ptr->loop_back_notify)
     {
	 if( wms_msg_get_free_mt_record( & i ) == TRUE )
         {
	   msg_s_ptr->gw_mt_rec[i]->as_id          = as_id;
           msg_s_ptr->gw_mt_rec[i]->smi            = uConnectionId;
	   msg_s_ptr->gw_mt_rec[i]->client_id      = (wms_client_id_type)parsed_client;
           msg_s_ptr->gw_mt_rec[i]->tid            = transaction_id;
	   msg_s_ptr->gw_mt_rec[i]->ack_client_id  = (wms_client_id_type)parsed_client;
	 }
...
#if !defined(FEATURE_WMS_LTE_ONLY) || defined(FEATURE_GWSMS)
    if( WMS_DOMAIN_PS == domain )
    {
    ...
    )
  {
    ...
    else
    {
      ...
      */
      ...
      {
        cmd_ptr->hdr.appInfo_ptr = wms_get_lte_appinfo();
      }
      else
      {
        cmd_ptr->hdr.appInfo_ptr = wms_get_appinfo_1x();
...
)
{
  ...
  if ( !asid_is_valid(appInfo_ptr->asid) )
  {
    MSG_ERROR_1("invalid asid %d", appInfo_ptr->asid);
    return;
  }

#ifdef FEATURE_SMS_PWS_OVER_LTE
  // Do duplicate detection based on the NV - DISABLE_LTE_CB_DUP_DETECTION - value
  if (!cfg_s_ptr->lte_cb_dup_detection_is_disabled && !message_from_lte)
  {
#ifndef FEATURE_WMS_LTE_ONLY
    wms_gw_cb_page_header_s_type *page_header_ptr = NULL;
    ...
    if ((page_header_ptr = wms_mem_malloc(sizeof(wms_gw_cb_page_header_s_type))) != NULL)
    {
      if (wms_ts_decode_gw_cb_page_header(cb_page, page_header_ptr) != 0)
      {
        if (WMS_GW_CB_GEO_SCOPE_PLMN == page_header_ptr->geo_scope &&
            TRUE == wms_msg_gw_cb_msg_is_cmas(page_header_ptr->cb_srv_id))
        {
          time_t                    search_time;
          lte_rrc_cmas_ind_s        lte_cmas_rec;
          wms_cell_info_s_type      curr_cell_info = wms_msg_gw_cb_get_cell_info_by_appInfo( appInfo_ptr );
          
          lte_cmas_rec.msg_id = (lte_rrc_msg_id_t) page_header_ptr->cb_srv_id;
          lte_cmas_rec.sr_num = page_header_ptr->raw_serial_number;
...
)
{
  ...
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_msg_offline"); 
  }
...
#else
  num_details.current_mobile_country_code = cfg_s_ptr->current_mobile_country_code;
  num_details.home_mobile_country_code = cfg_s_ptr->current_mobile_country_code;
