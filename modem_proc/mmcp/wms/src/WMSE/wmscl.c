/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmscl.c#1 $
...
#if defined (FEATURE_CDSMS) && !defined (FEATURE_WMS_LTE_ONLY)
...
#endif
		if ( (WMS_BEARER_LTE == bearer) && client_ptr->active_bearers[bearer] &&
...
)
{
  ...
  if ( (uint8)WMS_CLIENT_TYPE_MAX <= client_id )
  {
    ...
    MSG_ERROR_1("Client id is out of range. client_id = %d", client_id);
  }
  else
  {
    client_ptr = wms_client_ptr_fetch(client_id);
    ...
    if (NULL == client_ptr)
    {
      MSG_ERROR_0("NULL client_ptr. Client must have been initialized!");
      st = WMS_CLIENT_ERR_CLIENT_ID;
    }
    else
    {
...
)
{
  ...
  else if ( WMS_CLIENT_CH_PREF_MAX <= ch_pref )
  {
    ...
    MSG_ERROR_1("Channel pref is out of range. ch_pref = %d", ch_pref);
  }
  else
  {
    client_ptr = wms_client_ptr_fetch(client_id);
...
)
{
  ...
  if ( (uint8)WMS_CLIENT_TYPE_MAX <= client_id )
  {
    ...
    st = WMS_CLIENT_ERR_CLIENT_ID;
    MSG_ERROR_1("client_id out of range = %d", client_id);
  }
  else if ( NULL == ch_pref )
  {
     MSG_ERROR_0("ch_pref null pointer.");
...
)
{
  ...
  else if (WMS_DC_SO_MAX <= dc_so)
  {
    ...
    MSG_ERROR_1("Service option is out of range: so = %d", dc_so);
  }
  else
  {
    client_ptr = wms_client_ptr_fetch(client_id);
...
)
{
  ...
  {
     MSG_ERROR_0("dc_so null pointer.");
...
)
{
  ...
  {
     MSG_ERROR_0("follow_on_dc null pointer.");
