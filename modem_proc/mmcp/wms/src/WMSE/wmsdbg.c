/*====*====*====*====*====*====*====*====*====*====*==*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmsdbg.c#1 $
...
)
{
  ...
  MSG_HIGH_2( "DBG: client_id=%d, event=%d",
              client_id,
              event );
...
)
{
  ...
  {
    MSG_ERROR_0("Retry Interval exceeds Retry Period");
...
)
{
  ...
  {
    MSG_ERROR_0("Retry period exceeds Max value");
    cmd_err = WMS_CMD_ERR_DBG_BAD_PARAM;
  }
             ...
             rpt_ind_ptr->mt_sms.data_len > (5 + addr_len))
         {
           ...
           if (tpdu_len > 0) {
              ...
              wms_msg_sm_data_ind_proc(as_id,
                                       ...
                                       rpt_ind_ptr->ims_verstat_status
                                       );
              cmd_err = WMS_CMD_ERR_NONE;
           }
         }
...
static wms_dbg_cmd_err_info_s_type wms_dbg_cmd_err_info[] =
{
  ...
  {WMS_CMD_ERR_MSG_PSM,                  "WMS_CMD_ERR_MSG_PSM"},
