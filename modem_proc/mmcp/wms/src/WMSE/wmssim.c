/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmssim.c#1 $
...
)
{
  ...
  if (NULL == appInfo_ptr)
  {
    MSG_ERROR_0("wms_sim_write_cbmi: appInfo_ptr is NULL");
    return FALSE;
  }
...
)
{
  ...
  if (NULL == appInfo_ptr)
  {
    MSG_ERROR_0("wms_sim_write_cbmir: appInfo_ptr is NULL");
    return FALSE;
  }
...
)
{
  ...
  if (NULL == appInfo_ptr)
  {
    MSG_ERROR_0("wms_sim_init_lang_files: appInfo_ptr is NULL");
    return WMS_CMD_ERR_NULL_PTR;
  }
...
#endif /* FEATURE_GWSMS */
#endif
#ifndef FEATURE_WMS_NEW_UIM_INTERFACE_DISABLE
extern void wms_sim_client_id_init_sync( wms_cfg_s_type          *lcfg_s_ptr);
#endif
...
)
{
  ...
  #ifdef FEATURE_WMS_NEW_UIM_INTERFACE_DISABLE
  ...
  MSG_HIGH_0("wms_sim_mmgsdi_init");
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_sim_mmgsdi_init()");
  }
...
#else
  client_id = mmgsdi_client_id_and_evt_reg_sync( wms_sim_mmgsdi_event_cb,
                                                NULL,0);

 if(MMGSDI_CLIENT_ID_ZERO == client_id)
 {
   MSG_ERROR_1("wrong mmgdi client id  %d", client_id);
   return ;
 }
 lcfg_s_ptr->wms_sim_mmgsdi_init_state = WMS_SIM_MMGSDI_INIT_COMPLETE;
 lcfg_s_ptr->wms_sim_mmgsdi_client = client_id;   
 wms_sim_client_id_init_sync(lcfg_s_ptr);
#endif
...
#define MAX_NO_SESSIONS 2

void wms_sim_client_id_init_sync( wms_cfg_s_type          *lcfg_s_ptr)

{
  byte num_of_sessions = MAX_NO_SESSIONS;
  mmgsdi_session_open_info_type   * mmgsdi_session_info_ptr =  (mmgsdi_session_open_info_type *)wms_mem_malloc( num_of_sessions*sizeof( mmgsdi_session_open_info_type ) );
  int idx = 0;
  wms_prov_mmgsdi_app_info_s_type *wms_mmgsdi_appInfo_ptr = NULL;
  mmgsdi_return_enum_type  mmgsdi_res;

  uint32                   session_mask = MMGSDI_GW_PROV_PRI_SESSION_MASK |                                     
                                            MMGSDI_1X_PROV_PRI_SESSION_MASK ;

  MSG_HIGH_0("wms_sim_client_id_init_sync");
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), "wms_sim_client_id_init_sync()");
  }

      // Open the subscription sessions (all of them, always)
    mmgsdi_res = mmgsdi_session_open_ext_sync( lcfg_s_ptr->wms_sim_mmgsdi_client,
                                          session_mask,
   ...
   if (MMGSDI_SUCCESS != mmgsdi_res)
    ...
    {        
      ...
      for (idx = 0; idx < num_of_sessions; idx++)
      {
       
      
        wms_mmgsdi_appInfo_ptr = wms_get_mmgsdi_appInfo_by_session_type(mmgsdi_session_info_ptr[idx].session_type);
      
        if (NULL != wms_mmgsdi_appInfo_ptr)
        {
          wms_mmgsdi_appInfo_ptr->session_id              = mmgsdi_session_info_ptr[idx].session_id;
          wms_mmgsdi_appInfo_ptr->slot_id                 = mmgsdi_session_info_ptr[idx].slot_id;
          wms_mmgsdi_appInfo_ptr->session_type            = mmgsdi_session_info_ptr[idx].session_type;
          wms_mmgsdi_appInfo_ptr->app_type                = mmgsdi_session_info_ptr[idx].app_info.app_type;
        ...
        {
          MSG_HIGH_0("wms_mmgsdi_appInfo_ptr is NULL");
        }
      
      }
   }
   WMS_MEM_FREEIF(mmgsdi_session_info_ptr);   
...
)
{
...
#else
       appInfo_ptr->asid    = SYS_MODEM_AS_ID_1;
#endif
    }

     MSG_HIGH_3( "Allocated appInfo_ptr for session_id: 0x%x, session_type: %d - 0x%x,event:%d", 
  ...
  if(wms_get_ulog_handle() != NULL)
  {
	  ULOG_RT_PRINTF_3(wms_get_ulog_handle(), " Allocated appInfo_ptr for session_id: 0x%x, session_type: %d - 0x%x,event:%d", 
						wms_mmsgdi_appInfo->session_id,
                 wms_mmsgdi_appInfo->session_type,
                 appInfo_ptr);
  }
...
)
{
  ...
  if(wms_get_ulog_handle() != NULL)
  {
	  ULOG_RT_PRINTF_1(wms_get_ulog_handle(), " wms_sim_mmgsdi_event_cb(), event = %d", 
							 event->evt);
  }
  if ((MMGSDI_FILE_UPDATE_EVT  == event->evt) &&
       (event->data.file_update.file_data.data_len > 0))
   {
     data_ptr = (uint8*)wms_mem_malloc((uint32)event->data.file_update.file_data.data_len);
     if (NULL == data_ptr)
     {
  ...
  if (proc_in_wms_task)
  {
    ...
    if (NULL != cmd_ptr)
    {
	  ...
	  // Copy the buffer for update events
      if (NULL != data_ptr)
      {
        cmd_ptr->cmd.mmgsdi_event_cb.event.data.file_update.file_data.data_ptr = data_ptr;

        (void)memscpy( data_ptr,
                       (uint32)event->data.file_update.file_data.data_len,
                       event->data.file_update.file_data.data_ptr,
                       (uint32)event->data.file_update.file_data.data_len );
      }
...
)
{
 ...
 if (NULL != lcfg_s_ptr->wms_app1_info &&
  ...
  else if (NULL != lcfg_s_ptr->wms_app0_info &&
...
)
{
  ...
  for ( indx = 0; indx < appInfo_ptr->number_of_stacks; indx++ ) 
  {
   ...
   MSG_HIGH_2("appInfo_ptr->stack_info  srv_status = %d, sys_mode = %d",
                appInfo_ptr->stack_info.srv_status,
                appInfo_ptr->stack_info.sys_mode );

    if ( (SYS_SRV_STATUS_SRV == appInfo_ptr->stack_info.srv_status) && 
         (sys_mode == appInfo_ptr->stack_info.sys_mode) )
    {
      MSG_HIGH_1( "Found sys_mode = %d srv available", sys_mode );
      return TRUE;
    }
...
)
{
  ...
  if (NULL == info_ptr)
  {
    ...
    MSG_ERROR_2( "Could not map asid %d and message_mode %d to an appinfo",
                 ...
                 message_mode );
 #else
    MSG_ERROR_1( "Could not  message_mode %d to an appinfo",                 
                 message_mode );
 #endif
  }
...
#endif
    default:
      MSG_ERROR_1("Unmapped as id: %d", asid);
      break;
  }

  if (NULL == appInfo_ptr)
  {
    MSG_ERROR_0("Could not get appInfo");
  }
...
)
{
    ...
    MSG_HIGH_2( "wms_sim_mmgsdi_delete_data: file_enum 0x%x record_number %d",
                file_enum,
                record_number );
...
STATIC void wms_sim_mmgsdi_get_app_capabilities_cb_proc(
  const mmgsdi_session_app_capabilities_cnf_type  *cnf_ptr,
  mmgsdi_session_id_type         session_id
)  
{
  wms_prov_app_info_s_type  *appInfo_ptr = NULL;
  ...
  appInfo_ptr = wms_get_appinfo_by_session_id(session_id);

  if (NULL == appInfo_ptr)
  {
    MSG_ERROR_0("NUll app info ptr ");
    return;
  }
#ifdef FEATURE_WMS_PBM_ENABLE
  appInfo_ptr->fdn_enabled = cnf_ptr->fdn_enabled;
#endif
  ...
  (void)mmgsdi_session_is_service_available( session_id,
                                             srv_type,
                                             wms_sim_mmgsdi_response_cb,
...
)
{
  ...
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_0(wms_get_ulog_handle(), " In wms_sim_mmgsdi_read_sms_services()");
  }
  ...
  if (NULL != appInfo_ptr)
  {
    ...
    case MMGSDI_APP_USIM:
      {
		mmgsdi_session_app_capabilities_cnf_type *get_app_capabilities = NULL;       
        get_app_capabilities = (mmgsdi_session_app_capabilities_cnf_type *) wms_mem_malloc ( sizeof(mmgsdi_session_app_capabilities_cnf_type) );
		
		if(get_app_capabilities != NULL)
		{
          if(MMGSDI_SUCCESS != mmgsdi_session_get_app_capabilities_sync(
                                  appInfo_ptr->mmgsdi_appInfo.session_id,
                                   get_app_capabilities))
          {
             MSG_HIGH_0("mmgsdi_session_get_app_capabilities failed");
             WMS_MEM_FREEIF(get_app_capabilities);
             return ;
          }
          wms_sim_mmgsdi_get_app_capabilities_cb_proc(get_app_capabilities,appInfo_ptr->mmgsdi_appInfo.session_id);
          WMS_MEM_FREEIF(get_app_capabilities);
		}
      }
      break;
...
void wms_sim_mmgsdi_register_for_file_update(
  wms_prov_mmgsdi_app_info_s_type *app_mmgsdi_Info_ptr
)
{
   ...
   if (NULL != appInfo_ptr)
   {
           ...
           wms_nv_rtre_control_is_nv() )
      {
        ...
        status = mmgsdi_session_deregister_for_refresh( appInfo_ptr->mmgsdi_appInfo.session_id,
                                                        ...
                                                        (mmgsdi_client_data_type)appInfo_ptr );
#else 
        status = mmgsdi_session_deregister_for_refresh_sync( appInfo_ptr->mmgsdi_appInfo.session_id,
                                                             FALSE );
      ...
      else
      {
        ...
        status = mmgsdi_session_register_for_refresh_sync( appInfo_ptr->mmgsdi_appInfo.session_id,
                                                      refresh_files,
                                                      TRUE,
                                                      FALSE);
...
)
{
  ...
  if (NULL != appInfo_ptr)
  {
    MSG_HIGH_1("Setting Init state of all SIM files in WMS cache to %d", inited);
  ...
  {
    MSG_ERROR_0("wms_sim_set_sim_files_init_states : appInfo_ptr is NULL");
  }
  ...
  if (NULL != appInfo_ptr)
  {
    switch(file_enum_cache_index)
    {
      ...
      case SMSR_FILE_INDEX:
        ...
        break;	
#ifdef FEATURE_WMS_PP_DOWNLOAD_ENABE
      case MWIS_FILE_INDEX:
        file_enum = appInfo_ptr->mwis_file_enum;
        break;	
#endif
      default :
        MSG_ERROR_1("Invalid file enum index :- %d", file_enum_cache_index);
        break;
    }
  }
  else
  {
    MSG_ERROR_0("wms_sim_map_cache_index_to_file_enum : appInfo_ptr is NULL");
  }
...
)
{
  ...
  next_file_enum = wms_sim_map_cache_index_to_file_enum(next_file_enum_index, appInfo_ptr);
   ...
   MSG_HIGH_2("wms_sim_mmgsdi_read_next_sim_file: aFile index = %d enum val is  %d", start_index,next_file_enum );
#ifdef FEATURE_WMS_CB_ENABLE
...
#else
  if(SMSS_FILE_INDEX == next_file_enum_index)
  {

	(void)wms_sim_mmgsdi_read_transparent( appInfo_ptr,
                                           next_file_enum,
                                           2 );
	 MSG_HIGH_0("wms_sim_mmgsdi_read_next_sim_file: read transparrent ");
  }
else
#endif
  {
    /* read next file */
	MSG_HIGH_0("wms_sim_mmgsdi_read_next_sim_file: read wms_sim_mmgsdi_get_file_attr ");
...
)
{
  ...
  {  
      ...
      MSG_HIGH_3( " MO Control Enabled: %d, PP Download Enabled: %d, CB Download Enabled: %d", 
...
)
{
  ...
  default:
    MSG_ERROR_1("Unhandled refresh stage: %d", event->data.refresh.stage);
    break;
...
)
{
  ...
  if(wms_get_ulog_handle() != NULL)
  {
	  ULOG_RT_PRINTF_1(wms_get_ulog_handle(),"In wms_sim_cache_clean_up, appInfo_ptr = 0x%x", appInfo_ptr);
  ...
  if (NULL != appInfo_ptr)
  {
    ...
    appInfo_ptr->voice_mail_index                       = 0;
#endif
#ifdef FEATURE_GWSMS
  ...
  {
    MSG_ERROR_0("wms_sim_cache_clean_up : NULL appInfo_ptr");
  }
...
)
{
  ...
  MSG_HIGH_1("In wms_sim_mmgsdi_event_cb_proc(): process event = %d", event->evt);
  if(wms_get_ulog_handle() != NULL)
  {
    ULOG_RT_PRINTF_1(wms_get_ulog_handle(),"In wms_sim_mmgsdi_event_cb_proc(): process event = %d", event->evt);
  } 
  ...
  if (event->evt >= MMGSDI_SUBSCRIPTION_READY_EVT)
  {
   ...
   if (event->session_id == lcfg_s_ptr->wms_mmgsdi_app1_info.session_id)
    {
      if (NULL == lcfg_s_ptr->wms_app1_info)
      {
   ...
   else if (event->session_id == lcfg_s_ptr->wms_mmgsdi_app0_info.session_id)
    {
      if (NULL == lcfg_s_ptr->wms_app0_info)
      {
  ...
  if ((NULL == appInfo_ptr) && (event->evt != MMGSDI_SIM_BUSY_EVT))
  {
    switch (event->evt)
    {
    case MMGSDI_CARD_INSERTED_EVT:
  ...
  case MMGSDI_FDN_EVT:
    ...
    MSG_HIGH_0( "Received MMGSDI_FDN_EVT with FDN enabled " );
#ifdef 	FEATURE_WMS_PBM_ENABLE
...
)
{
  ...
  default:
    {
      MSG_ERROR_1("Unexpected file_enum %d", file_enum);
    }
    break;
  }
  ...
  /* case MMGSDI_SESSION_GET_APP_CAPABILITIES_CNF:
   ...
   case MMGSDI_GET_FILE_ATTR_CNF:
      ...
      case MMGSDI_USIM_MWIS:
         ...
         break;
      default:
         MSG_ERROR_1("Unexpected file_enum %d", cnf_ptr->get_file_attr_cnf.access.file.file_enum);
         break;
      }
      break;
...
#endif
      default:
         MSG_ERROR_1("Unexpected file_enum %d", cnf_ptr->read_cnf.access.file.file_enum);
         break;
      }
...
#else
    for(file_index = SMSS_FILE_INDEX; file_index < MAX_FILE_INDEX; file_index++)
#endif
    {
...
)
{
       ...
       (NULL != data_ptr) )
  {
    ...
    else
    {
	  ...
	  MSG_HIGH_2(" message_referance: %d, smss_mem_full_flag : %d ",appInfo_ptr->smss_last_message_number,appInfo_ptr->smss_mem_full_flag);
...
)
{
  ...
  if (next_record_num <= appInfo_ptr->smsp_rec_count)
  {
    ...
    if (MMGSDI_SUCCESS != ret)
    {
      MSG_ERROR_0("mmgsdi_session_read_record failed");
    }
  }
  else
  {
...
)
{
  ...
  /* Free the memory allocated for update evt */
  WMS_MEM_FREEIF(event->data.file_update.file_data.data_ptr);

} /* wms_sim_mmgsdi_file_update_event_proc() */


rex_tcb_type *wms_get_wms_tcb_ptr()
{
  return wms_myself;
}
