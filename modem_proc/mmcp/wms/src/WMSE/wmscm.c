/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  ...
  $Header: //components/rel/mmcp.mpss/6.1.10/wms/src/WMSE/wmscm.c#1 $
...
)
{
 ...
 #endif
  {
    ...
    {
      appInfo_ptr = wms_get_appinfo_by_asid_and_message_mode( 
#ifdef FEATUTRE_NEW_CM_INTERFACE_DISABLE
    ...
    if ( NULL != appInfo_ptr )
    {
      ...
      MSG_HIGH_3( "ECBM enabled: %d  message_mode %d, sms_in_ecbm: %d",
    ...
    {
      MSG_ERROR_1( "appInfo_ptr is NULL:  message_mode %d",message_mode );
...
#else
   cfg_s_ptr->routes.as_id = SYS_MODEM_AS_ID_1;
   if (wms_nv_read_routing(&cfg_s_ptr->routes) == FALSE )
  {
    // Message routes init: these are the default settings
#ifdef FEATURE_GWSMS
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_0].route        = WMS_ROUTE_TRANSFER_AND_ACK;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_0].mem_store    = WMS_MEMORY_STORE_NONE;

    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_1].route        = WMS_ROUTE_STORE_AND_NOTIFY;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_1].mem_store    = WMS_MEMORY_STORE_NV_GW;

    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_2].route        = WMS_ROUTE_STORE_AND_NOTIFY;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_2].mem_store    = WMS_MEMORY_STORE_SIM;

    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_3].route        = WMS_ROUTE_STORE_AND_NOTIFY;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_3].mem_store    = WMS_MEMORY_STORE_NV_GW;

    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_NONE].route     = WMS_ROUTE_STORE_AND_NOTIFY;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_NONE].mem_store = WMS_MEMORY_STORE_NV_GW;

    cfg_s_ptr->routes.transfer_status_report = FALSE;
#endif /* FEATURE_GWSMS */

#ifdef FEATURE_CDSMS
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_CDMA].route     = WMS_ROUTE_STORE_AND_NOTIFY;
    cfg_s_ptr->routes.pp_routes[WMS_MESSAGE_CLASS_CDMA].mem_store = WMS_MEMORY_STORE_NV_CDMA;
#endif /* FEATURE_CDSMS */
...
#ifndef FEATURE_NEW_CM_INTERFACE_DISABLE
void wms_process_subs_event_from_ph_event( cm_ph_info_s_type  *ph_info_ptr )
{


  wms_cfg_s_type                 *cfg_s_ptr   = wms_cfg_s_ptr();
  wms_prov_app_info_s_type       *appInfo_ptr;
     wms_prov_app_info_s_type       *appInfo_ptr2;
  ...
  appInfo_ptr2 = wms_get_appinfo_by_session_type(MMGSDI_GW_PROV_PRI_SESSION);

  if(appInfo_ptr2 == NULL )

  {
     MSG_HIGH_0("CM_PH_EVENT_INFO app info ptr NULL return");
	 return;
  }


  MSG_HIGH_1( "PH_INFO_RECEIVED: session  ( avail:%d)",
                                        ph_info_ptr->is_gw_subscription_available);  
  
  cur_is_available =  cfg_s_ptr->wms_ph_info.is_gw_subscription_available;


  MSG_HIGH_1( "PH_INFO_STORED: session (GW avail:%d) ",                   
                                      cfg_s_ptr->wms_ph_info.is_gw_subscription_available
                                     );
  
  if(cur_is_available != ph_info_ptr->is_gw_subscription_available)
  {
    // Initialize GW domain preference from NV
     wms_cm_init_domain_pref(appInfo_ptr2);

	 appInfo_ptr2->is_available = ph_info_ptr->is_gw_subscription_available;

              // If the previous state was ready, send not ready
     if (cur_is_available)
     {
                // GW mapped, available -> GW mapped, not available
        wms_cfg_report_not_ready_event(appInfo_ptr2);
     }
     else
     {
                // GW mapped, not available -> GW mapped, available
        wms_sim_do_subs_ready(appInfo_ptr2);
     }
...
#ifdef FEATURE_NEW_CM_INTERFACE_DISABLE
...
#else
    MSG_HIGH_1("ph_info_ptr->is_gw_subscription_available = %d",       ph_info_ptr->is_gw_subscription_available);
    MSG_HIGH_1("ph_info_ptr->ph_state = %d",                           ph_info_ptr->ph_state);
    MSG_HIGH_1("ph_info_ptr->oprt_mode = %d",                          ph_info_ptr->oprt_mode);
    MSG_HIGH_1("ph_info_ptr->mode_capability = %d",                    ph_info_ptr->mode_capability);

    if((CM_PH_EVENT_SUBSCRIPTION_AVAILABLE == ph_event)|| (CM_PH_EVENT_SUBSCRIPTION_NOT_AVAILABLE == ph_event))
    {
      wms_process_subs_event_from_ph_event(ph_info_ptr);
	  cfg_s_ptr->wms_ph_info.is_gw_subscription_available = ph_info_ptr->is_gw_subscription_available;
	}
	cfg_s_ptr->wms_ph_info.ph_state = ph_info_ptr->ph_state;
	cfg_s_ptr->wms_ph_info.oprt_mode =  ph_info_ptr->oprt_mode;
	cfg_s_ptr->wms_ph_info.mode_capability = ph_info_ptr->mode_capability;
...
void wms_cm_ss_event_cb(
  cm_ss_event_e_type                ss_event,
  const cm_mm_ss_info_s_type  *cm_ss_info_ptr
)
{
  wms_cmd_id_e_type				cmd_id = WMS_CMD_DUMMY;
  wms_cmd_ss_change_info_s_type   tmp_ss_info;
  ...
  switch( ss_event )
  {
    ...
    case CM_SS_EVENT_INFO:
      ...
      MSG_HIGH_6( "srv_status %d, sys_mode %d, id_type %d, srv_domain %d changed_fields = %d roam_status = %d",
              cm_ss_info_ptr->srv_status,
              cm_ss_info_ptr->sys_mode,
              cm_ss_info_ptr->sys_id.id_type,
              cm_ss_info_ptr->srv_domain,
               cm_ss_info_ptr->changed_fields,
               cm_ss_info_ptr->roam_status);
      
      
      tmp_ss_info.sys_srv_has_changed   = FALSE;
      
      
      tmp_ss_info.sys_mode    = cm_ss_info_ptr->sys_mode;
      tmp_ss_info.srv_domain  = cm_ss_info_ptr->srv_domain;
      tmp_ss_info.srv_status  = cm_ss_info_ptr->srv_status;
      tmp_ss_info.sys_id      = cm_ss_info_ptr->sys_id;
      tmp_ss_info.roam_status = cm_ss_info_ptr->roam_status;
      
      tmp_ss_info.umts_plmn_changed = ( (SYS_SYS_ID_TYPE_UMTS == cm_ss_info_ptr->sys_id.id_type) &&
                                       (cm_ss_info_ptr->changed_fields & CM_SS_EVT_SYS_ID_MASK) );
      
      if ( (cm_ss_info_ptr->changed_fields & CM_SS_EVT_SRV_DOMAIN_MASK)    ||
        (cm_ss_info_ptr->changed_fields & CM_SS_EVT_SYS_MODE_MASK)       ||
        (cm_ss_info_ptr->changed_fields & CM_SS_EVT_SRV_STATUS_MASK)     ||
        (cm_ss_info_ptr->changed_fields & CM_SS_EVT_ROAM_STATUS_MASK)    ||
        (cm_ss_info_ptr->changed_fields & CM_SS_EVT_SYS_ID_MASK) )
      {
        tmp_ss_info.sys_srv_has_changed = TRUE;
        cmd_id = WMS_CMD_SS_CHANGE_INFO;
      }
...
#else
event_mask.bits_1_64 = CM_PH_EVENT_ALL;
  status = cm_client_ph_reg( cm_s_ptr->cm_client,
                             wms_cm_ph_event_cb,
                             CM_CLIENT_EVENT_REG,
                              event_mask,
...
#else
event_mask.bits_1_64 = CM_SS_EVENT_ALL;

 status = cm_mm_client_ss_reg( cm_s_ptr->cm_client,
                                     wms_cm_ss_event_cb,
                                     CM_CLIENT_EVENT_REG,
                                      event_mask ,
                                       NULL);
