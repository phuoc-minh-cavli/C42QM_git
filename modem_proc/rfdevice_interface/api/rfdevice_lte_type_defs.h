...
$DateTime: 2020/04/16 12:19:53 $ $Author: pwbldsvc $
$Header: //components/rel/rfdevice_interface.mpss/1.40/api/rfdevice_lte_type_defs.h#1 $ 
   
when       who     what, where, why 
--------   ---     -------------------------------------------------------------
07/11/19   up      Updated the structure rfdevice_lte_ffh_data_type
06/26/19   up      Updated LTE RX and TX event timing structures
04/01/19   up      Added Invalid frequency enum.
01/15/19   am      Added NB tone_spacing type enum
11/22/18   up      Updated struct rfdevice_lte_rx_rsb_offset_type with lte_cat
10/25/18   up      Mainlined the SDR105 interface changes
07/26/18   up      Added the enum: rfdevice_trx_pll_option_info_type
07/02/18   up      Added new structure rfdevice_lte_ffh_data_type
06/27/18   up      RFDEVICE_LTE_MAX_LNA_STATES is updated to 9 for SDR105
04/19/18   up      RFDEVICE_LTE_MAX_LNA_STATES is modified to 8 for SDR105
02/06/18   up      Mofified for SDR105, FR 46405: SDR105 TRX (IOTRF) 
                   device driver addition/Modification
11/08/16   ry      LTE Cat M1 timing updates
...
#define RFDEVICE_LTE_MAX_LNA_STATES 9

//Below delays are the combined sum of all the relative delays in HDET Enable template.
#define RFDEVICE_LTE_CATM_HDET_CCS_DELAY 250
#define RFDEVICE_LTE_NB_HDET_CCS_DELAY_15KHZ 520
#define RFDEVICE_LTE_NB_HDET_CCS_DELAY_3P75KHZ 1320

#define RFDEVICE_TRX_INVALID_FREQ 0xFFFFFFFF
...
typedef struct
{
  ...
  uint32 dflo_config;
  /*! fbadc_reset */
  uint32 fbadc_ctrl_reset;
  /*! fbadc_enable */
  uint32 fbadc_ctrl_enable;
}
rfdevice_lte_ffh_data_type;
...
typedef enum
{
  RFDEVICE_LTE_NB_3p75khz,
  RFDEVICE_LTE_NB_15khz,
  RFDEVICE_LTE_NB_TONE_SPACE_MAX_NUM,
  RFDEVICE_LTE_NB_TONE_SPACE_INVALID,
} rfdevice_lte_nb_tone_spacing_type;
...
typedef enum
{
 ...
 RFDEVICE_LTE_TIMING_FDD_RX_RX_EN_HIGH,
 RFDEVICE_LTE_TIMING_FDD_RX_RX_EN_LOW, 
...
typedef struct
{
  ...
  rfcom_lte_band_type rf_band;
  // Structure which specifies a unique physical path from antenna to WTR VCO
  rfm_device_enum_type rfm_device;
}rfdevice_trx_lte_rx_path_toggle_info_type;



/*----------------------------------------------------------------------------*/
/*! @brief Structure containing pll and sig path toggle which need to passed to device API */
typedef enum
{
  /*! @brief enum to toggle on pll/sigpath but skip the rxsm */
  RFDEVICE_TRX_RX_TOGGLE_ON,
  /*! @brief enum to toggle off pll/sigpath but skip the rxsm and Rx PC don't care */
  RFDEVICE_TRX_RX_TOGGLE_OFF,
  /*! @brief enum to use different WxE mem to avoid collision during back to back Rx Off */
 // RFDEVICE_TRX_RX_TOGGLE_OFF_DWPTS,
  /*! @brief enum to fully turn off the pll, sigpath and rxsm and Rx PC don't care*/
  RFDEVICE_TRX_RX_FULL_DISABLE,
  /*! @brief enum to fully turn on the pll, sigpath and rxsm and Rx PC don't care */
  RFDEVICE_TRX_RX_FULL_ENABLE,
  /*! @brief enum to toggle off pll/sigpath but keep the rx phase continuity ON */
 // RFDEVICE_TRX_RX_TOGGLE_OFF_PC_ON,
  /*! @brief enum to use different WxE mem to avoid collision during back to back Rx Off with PC On */
 // RFDEVICE_TRX_RX_TOGGLE_OFF_PC_ON_DWPTS
  
}rfdevice_trx_lte_rx_toggle_type;
...
typedef enum
{
  /*! @brief enum to turn on sigpath */
  RFDEVICE_TRX_RX_SIGPATH_OFF,
  /*! @brief enum to toggle off pll/sigpath but skip the rxsm and Rx PC don't care */
  RFDEVICE_TRX_RX_SIGPATH_ON
}rfdevice_trx_lte_rx_sigpath_toggle_type;
#endif


/*----------------------------------------------------------------------------*/
/*! @brief Structure containing timing info for LTE Rx events */
typedef struct
{
  /*! @brief PLL timing */
  int16 pll_on;
  ...
  /*! @brief Signal path timings */
  int16 sigpath_on;
  int16 sigpath_on_spll;
  int16 sigpath_off;

  /*! @brief AGC timing */
  int16 agc;
}rfdevice_trx_lte_rx_event_timing_info_type;
/*----------------------------------------------------------------------------*/
/*! @brief Structure containing timing info for LTE Tx events */
typedef struct
{
  /*! @brief PLL timings */
  int16 pll_on;
  int16 pll_lpm_to_hpm;
  int16 pll_hpm_to_lpm;
  int16 pll_off;
  ...
  /*! @brief Tx OFF->ON AGC timings */
  int16 tx_on_ul_sf_agc;
  int16 tx_on_prach_or_srs_only_agc;
  int16 off_to_on_tx_rgi_trig;
  int16 off_to_on_prach_srs_only_offset;
  
  /*! @brief Continuous Tx AGC timings */
  int16 cont_ul_sf_agc;
  int16 cont_tx_rgi_trig;
  int16 cont_post_ue_srs_agc;
  int16 cont_tx_rgi_early_trig;
  int16 cont_tx_rgi_late_trig;
  int16 cont_tx_pre_ue_srs_offset;
  int16 cont_tx_post_ue_srs_offset;
  int16 cont_tx_pre_ue_srs_offset_tm8;
  int16 cont_tx_post_ue_srs_offset_tm8;
  int16 cont_tx_pre_ue_srs_offset_early;
  int16 cont_tx_pre_ue_srs_offset_late;
  int16 cont_tx_post_ue_srs_offset_early;
  int16 cont_tx_post_ue_srs_offset_late;
  
  /*! @brief RGI0 timing */
  int16 tx_off_ul_sf_rgi0 ;
  int16 tx_off_cell_srs_ul_sf_rgi0;
  int16 tx_off_agc_rgi0;

    /*! @brief FFH timing */
  int16 ffh_off_to_on_prach;
  int16 ffh_off_to_on_srs;
  int16 ffh_off_to_on_puxch;
  int16 ffh_puxch_to_puxch;
  int16 ffh_srs_to_puxch;
  int16 ffh_puxch_to_srs;
  int16 ffh_srs_to_srs;
  int16 ffh_on_to_off;

}rfdevice_trx_lte_tx_event_timing_info_type;
...
/*! 
  @brief 
  Structure containing various parameters that indicate WTR capabilities
*/
typedef struct
{
  /*! Flag indicating whether phase continuity is supported or not */
  boolean is_rx_phase_cont_supported;
  /*! Maximum RX bandwidth supported in kHz */
  uint16 max_rx_bw_khz;
}rfdevice_trx_lte_rx_capabilities;
...
/*! 
  @brief 
  Structure containing pll_option and its index in the PLL LUT for a band */
typedef struct 
{
  /*! pll option for a band */
  uint16 pll_option;

  /*! lut index corresponding to the pll option */
  uint16 lut_index;
} rfdevice_trx_pll_option_info_type;


/*! @brief Enum indicating the FFH WXE buffer */
typedef enum
{
   RFDEVICE_TRX_FFH_WXE_BUF_0,        /* FFH WXE template for FFH 0 buffer */
   RFDEVICE_TRX_FFH_WXE_BUF_1,        /* FFH WXE template for FFH 1 buffer */
   RFDEVICE_TRX_FFH_WXE_BUF_NUM,      
   RFDEVICE_TRX_FFH_WXE_BUF_INVALID,
} rfdevice_trx_lte_ffh_template_type; 
