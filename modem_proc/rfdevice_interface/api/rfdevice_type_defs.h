...
$Header: //components/rel/rfdevice_interface.mpss/1.40/api/rfdevice_type_defs.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/25/18   up      Mainlined SDR105 interface changes
06/27/18   up      Modified  RFDEVICE_MAX_PA_STATES to 9 for SDR105 
06/13/18   vp      Featurization of WCDMA code 
...
#define RFDEVICE_MAX_RX_GAIN_STATES 9
#define RFDEVICE_BITS_PER_RX_GAIN_STATE 4
#define RFDEVICE_RX_GAIN_STATE_INVALID 0xF


#define RFDEVICE_RX_SET_GAIN_STATE_MAP(G0, G1, G2, G3, G4, G5, G6, G7) \
                    ( G0 << (0 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G1 << (1 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G2 << (2 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G3 << (3 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G4 << (4 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G5 << (5 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G6 << (6 * RFDEVICE_BITS_PER_RX_GAIN_STATE) |              \
                      G7 << (7 * RFDEVICE_BITS_PER_RX_GAIN_STATE) )

#define RFDEVICE_RX_GET_GAIN_STATE_MAP(gainmap, systemgain) \
          ( ((gainmap) >> ((systemgain) * RFDEVICE_BITS_PER_RX_GAIN_STATE)) & 0xF )
...
/*!
  Enum to indicate the voltage supply to Transceiver
*/
typedef enum
{
  RFDEVICE_TRX_VDD_VOLTAGE_LOW,
  RFDEVICE_TRX_VDD_VOLTAGE_HIGH,
  RFDEVICE_TRX_VDD_VOLTAGE_INVALID
}rfdevice_trx_vdd_voltage_enum_type;
...
} rfdevice_trx_gsm_env_delay_type;
