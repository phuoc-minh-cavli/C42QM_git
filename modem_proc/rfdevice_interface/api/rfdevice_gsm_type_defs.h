...
$Header: //components/rel/rfdevice_interface.mpss/1.40/api/rfdevice_gsm_type_defs.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/24/18   bm      Added rfdevice_trx_gsm_env_delay_type structure
02/09/18   smi     Added below new structure
                      rfdevice_gsm_tx_timing_info_type 
                      rfdevice_gsm_rx_timing_info_type
...
typedef struct
{
  uint16 pll_lock_time_qs;
  int16 tune_fixup_rel_qs;    //Relative to slot start
  int16 agc_fixup_rel_qs;     //Relative to slot start
  int16 disable_fixup_rel_qs; //Relative to slot end
  uint16 reg_prog_time;
}rfdevice_gsm_tx_timing_info_type;

/*----------------------------------------------------------------------------*/
/*! @brief Structure containing timing info for GSM Rx events */
typedef struct
{
  uint16 pll_lock_time_qs;
  int16 tune_fixup_rel_qs;  //Relative to slot start
...
/*! @brief Structure containing info for RSB correction */
typedef struct
{
  uint32 rsb_group;
  boolean is_inv_reqd;
}rfdevice_trx_gsm_rx_rsb_info_type;
#if 0
/*
