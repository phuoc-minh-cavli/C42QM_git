...
#define T APR_RPD 
#define U APR_SPD 
int32_t is_fusion=0; //if it is fusion, this value is updated to 1.
...
// qsocket_unit_test_start();
  ...
  if ( DalPlatformInfo_IsFusion() ) //Exercise only for fusion
  {
	 #ifdef QSOCKET_PORTS
     rc = apr_qsocket_init();
	 #endif
...
APR_INTERNAL int32_t apr_ipc_write_fn(uint32_t port_index,void *buf, uint32_t length)
{
    ...
    /*Identify qsocket is defined based on the domain id */
    if ( ( is_fusion ) && ( ( dst_domain_id == APRV2_IDS_DOMAIN_ID_ADSP_V) ||  ( dst_domain_id == APRV2_IDS_DOMAIN_ID_MODEM_V) ) )
    {
	 #ifndef SIM_DEFINED
	 #ifdef QSOCKET_PORTS
     return apr_qsocket_tx(port_index, buf, length);
	 #endif
     #endif
...
APR_INTERNAL int32_t aprv2_ipc_send (aprv2_packet_t* packet)
{
  ...
  if ( packet == NULL || packet->dst_addr == NULL)
  {
    return APR_EBADPARAM;
  }
  ...
  if ( dst_domain_id >= sizeof(aprv2_ipc_glink_send_dispatch_table)/sizeof(aprv2_ipc_glink_send_dispatch_table[0]) )
  {
	  return APR_EBADPARAM;
  }
...
APR_INTERNAL int32_t aprv2_ipc_free (aprv2_packet_t *packet)
{
  ...
  if (packet == NULL || packet->src_addr == NULL)
  {
    return APR_EBADPARAM;
  }
   ...
   if ( src_domain_id >= sizeof(aprv2_ipc_glink_send_dispatch_table)/sizeof(aprv2_ipc_glink_send_dispatch_table[0]) )
   {
	  return APR_EBADPARAM;
   }
