/*
   Copyright (C) 2013-2014, 2018 QUALCOMM Technologies Incorporated.
   All rights reserved.
   Qualcomm Confidential and Proprietary

   $Header: //components/rel/avs.mpss/8.0.c12/vsd/voicecfg/src/voicecfg.c#1 $
   $Author: pwbldsvc $
*/
...
)
{
  ...
  for ( ;; )
  {
    ...
    {
      MSG( MSG_SSID_DFLT, MSG_LEGACY_MED,  "voicecfg_get_item(): No items " \
    ...
    {
      MSG_1( MSG_SSID_DFLT, MSG_LEGACY_ERROR,  "voicecfg_get_item(): Couldn't " \
             "find item_id(0x%08X)",
             item_id );
      rc = APR_ENOTEXIST;
      break;
