/*
   ...
   $Header: //components/rel/avs.mpss/8.0.c12/vsd/vadapter/src/CT/gva_module.c#1 $
   $Author: pwbldsvc $
*/
...
/* SELF APIs. */
#include "../../inc/CT/protected/gva_if.h"
#include "../../inc/CT/protected/gva_iresource_if.h"
#include "../../inc/CT/private/gva_i.h"
...
{
  gva_modem_subs_object_t* subs_obj = session_obj->active_subs_obj;
  ...
  for( ;; )
  {
        ...
        ( evt_params->rx_mode == VS_VOC_BANDWIDTH_WB ) )
    {
      ...
      MSG_3( MSG_SSID_DFLT, MSG_LEGACY_MED,
             ...
             subs_obj->asid, evt_params->rx_mode, sample_rate_cmd.sample_rate );
    }
    break;
  }
...
)
{
  ...
  for( ;; )
  {
    ...
    subs_obj = ( gva_modem_subs_object_t* ) packet->token;
    if( subs_obj == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( APR_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
         ...
         ( session_obj->va_gva_event_cb != NULL ) )
    {
      ...
      /* Not using gva_publish_vocoder_state wrapper as session->active_subs_obj
       * is NULL during select owner event. */
      if( ( NULL != subs_obj ) &&
          ( VOICELOG_IEVENT_VOCODER_STATE_REQUESTED != session_obj->vocoder_state ) )
      {
        /* Publish vocoder state for voice call on a subscription. */
...
)
{
  ...
  if ( APR_EOK == rc )
  {
    ...
    if ( GVA_VSID_UNDEFINED_V != subs_obj->pending_vsid )
    {
      ...
      subs_obj->pending_vsid = GVA_VSID_UNDEFINED_V;
      gva_prepare_and_dispatch_cmd_packet( NULL,
        GVA_ICOMMON_CMD_SET_ASID_VSID_MAPPING, (void*)&gva_map_cmd,  sizeof( gva_map_cmd ) );
    }
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = APRV2_PKT_GET_PAYLOAD( gva_iresource_cmd_grant_t, packet );
    if( cmd_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( APR_EUNEXPECTED );
      break;
    ...
    if ( ctrl->state == GVA_GATING_CMD_STATE_EXECUTE )
    {
      ...
      {
        MSG( MSG_SSID_DFLT, MSG_LEGACY_HIGH,
             "gva_process_resource_grant_cmd() - IVOICE_EVENT_START_REQUEST not "
             "received from GSM" );
        break;
      }
...
)
{
  ...
  aprv2_packet_t* packet = ctrl->packet;
  gva_session_object_t* session_obj = NULL;
  gva_iresource_cmd_revoke_t* cmd_params = NULL;
  gva_simple_job_object_t* simple_obj = NULL;
  ...
  for ( ;; )
  {
    ...
    cmd_params = APRV2_PKT_GET_PAYLOAD( gva_iresource_cmd_revoke_t, packet );
    if( cmd_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( APR_EUNEXPECTED );
      break;
...
)
{
  ...
  for ( ;; )
  {
    ...
    cmd_params = APRV2_PKT_GET_PAYLOAD( gva_icommon_cmd_set_asid_vsid_mapping_t, packet );
    if( cmd_params == NULL )
    {
      GVA_REPORT_FATAL_ON_ERROR ( APR_EUNEXPECTED );
      break;
                               ...
                               ( ( apr_list_node_t** ) &work_item ) ) == APR_EOK )
  {
    ...
    switch( packet->opcode )
    {
     ...
     case GVA_ICOMMON_CMD_SET_ASID_VSID_MAPPING:
       rc = gva_process_set_asid_vsid_mapping_cmd( packet  );
       break;
...
)
{
  ...
  case GVA_IRESOURCE_CMD_REVOKE:
  case GVA_ICOMMON_CMD_SET_ASID_VSID_MAPPING:
    rc = gva_prepare_and_dispatch_cmd_packet ( NULL, cmd_id, params, size );
    break;
