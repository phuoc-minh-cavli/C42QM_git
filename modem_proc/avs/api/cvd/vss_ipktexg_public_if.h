...
/*===========================================================================
  ...
  $Header: //components/rel/avs.mpss/8.0.c12/api/cvd/vss_ipktexg_public_if.h#1 $
  $Author: pwbldsvc $
...
/** 
  ...
  CVD in APQ-ADSP is responsible for exchanging the ADSP mailbox memory configuration
  with VS in MDM-Modem before the voice packet exchange starts during a voice call.
  Modem CVD client(VS) needs this to map the physical memory (carved out
  from APQ DDR for mailbox packet exchange )with MVM.
  It should be contiguous memory worth 80KB and dedicated for Vocoder packet exchange
  between ADSP and Modem.
 
  This shall be set after bootup as soon as this information is available to HLOS.
  HLOS voice driver will also have to keep track of ADSP and MDM PDR/SSR events and
  trigger the setting of IPA config after every ADSP/MDM PDR/SSR UP notification.
 
  Access control over the mailbox buffer shall be configured correctly for APQ-APPS,
  APQ-ADSP, and PCIe/IPA to access.
 
  @apr_msgpayload {vss_ipktexg_cmd_set_mailbox_memory_config_t}
  @table{weak__vss__ipktexg__cmd__set__mailbox__memory__config__t}
...
/* Type definition for vss_ipktexg_cmd_set_mailbox_memory_config_t. */
typedef struct vss_ipktexg_cmd_set_mailbox_memory_config_t vss_ipktexg_cmd_set_mailbox_memory_config_t;

#include "apr_pack_begin.h"

/** @weakgroup vss_ipktexg_cmd_set_mailbox_memory_config_t
@{ */
...
/** 
  Requests the mailbox memory configuration information for ADSP 
  mailbox memory. VS shall trigger this request as part of modem 
  callsetup sequence. VS shall use this configuration to map 
  this physical memory (carved out from APQ DDR for mailbox 
  packet exchange )with MVM. VS shall trigger this request only 
  incase of fusion platform where Modem Q6 sits on MDM and ADSP 
  is part of APQ. To reduce Modem callsetup latency, VS shall 
  request this only for the first voice call after 
  bootup or MDM SSR/PDR and store this for subsequent use. 
   
  @apr_msgpayload
  None.

  @return
  #VSS_IPKTEXG_RSP_MAILBOX_MEMORY_CONFIG -- Success.
  @par
  APRV2_IBASIC_RSP_RESULT -- Failure (refer to
  @xrefcond{Q2,80-N1463-2,80-NA601-5}). 
*/ 

#define VSS_IPKTEXG_CMD_REQ_MAILBOX_MEMORY_CONFIG ( 0x0001333C ) 

/** 
  Response to VSS_IPKTEXG_CMD_REQ_MAILBOX_MEMORY_CONFIG cmd.
   
  @apr_msgpayload{vss_ipktexg_rsp_mailbox_memory_config_t}
  @table{weak__vss__ipktexg__rsp__mailbox__memory__config__t}
  ...
  @dependencies
  CVD should have already received this info from HLOS. 
  If not it shall return an error response. */

#define VSS_IPKTEXG_RSP_MAILBOX_MEMORY_CONFIG ( 0x0001333D ) 

/* Type definition for vss_ipktexg_rsp_mailbox_memory_config_t. */
typedef struct vss_ipktexg_rsp_mailbox_memory_config_t vss_ipktexg_rsp_mailbox_memory_config_t;

#include "apr_pack_begin.h"
/** @weakgroup vss_ipktexg_rsp_mailbox_memory_config_t
@{ */
/* Structure for specifying mailbox ipa config.
*/
struct vss_ipktexg_rsp_mailbox_memory_config_t
{
  uint64_t mailbox_mem_address_adsp;
  /**< Physical address(understandable to ADSP) of Mailbox Memory carved out from APQ DDR 
       specifically for mailbox packet exchange between ADSP and MDM. */
