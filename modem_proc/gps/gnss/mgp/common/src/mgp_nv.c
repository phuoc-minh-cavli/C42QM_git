...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/mgp/common/src/mgp_nv.c#1 $
...
void mgp_nv_init (void)
{
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_INIT: already initialized", 0 ,0 ,0);
  }
} /* mgp_nv_init */
...
uint32 mgp_nv_write_reg (uint32 thread_id, mgp_nv_cmd_cb_f_type nv_clnt_cb_fn)
{
  ...
  if (mgp_nv_initialized == FALSE)
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_HIGH,"MGP_NV_INIT: Initializing MGP NV module", 0, 0, 0);
    mgp_nv_init();
  }
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_INIT: FAILED - Unknown parameter", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_INIT: FAILED - NULL parameter", 0, 0, 0);
    return (uint32)MGP_NV_CLIENT_HANDLE_ERROR;
  }
  ...
  MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_INIT: FAILED - max # of clients reached", 0, 0, 0);
  return (uint32)MGP_NV_CLIENT_HANDLE_ERROR;
} /* mgp_nv_write_reg */
...
static void mgp_nv_handle_wr_cmd (void)
{
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_HANDLE_WR_CMD: MGP NV not initialized", 0 ,0 ,0);
  }
  return;
...
boolean mgp_nv_write (nv_items_enum_type nv_item, const void* item_ptr, uint32 handle)
{
  ...
  if (mgp_nv_initialized)
  {
    ...
    if (nv_ptr == NULL)
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - no buffer", 0, 0, 0);
    ...
    if (item_ptr == NULL)
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - NULL parameter", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - bad handle", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: MGP NV not initialized", 0 ,0 ,0);
    return FALSE;
  }
} /* mgp_nv_write */
...
boolean mgp_nv_write_done (void)
{
  if(mgp_nv_initialized == FALSE)
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: Queue Not Initialized", 0, 0, 0);
    return FALSE;  
  }
  ...
  if (nv_ptr == NULL)
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: No write requested", 0, 0, 0);
    os_MemFree((void**)&nv_ptr);
    return FALSE;
  }
  else
  {
    ...
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - Non-matching item", 0, 0, 0);
      os_MemFree((void**)&nv_ptr);
      return FALSE;
    }
    else
    {
      ...
      {
        /* NV write failed. */
        MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - Error Status %d", mgp_nv_cmd.status, 0, 0);
        os_MemFree((void**)&nv_ptr);
        return FALSE;
      }
      ...
      {
        MSG_1(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_WRITE: FAILED - bad parameter %d", nv_ptr->handle);
        os_MemFree((void**)&nv_ptr);
        return FALSE;
      }
      else
...
static boolean mgp_nv_read_default (nv_items_enum_type nv_item, const void *item_ptr)
{
  ...
  MSG_3(MSG_SSID_MGP, MSG_LEGACY_MED,"MGP_NV_READ: Default NV item %d returned", nv_item, 0, 0);
  ...
  switch (nv_item)
  {
    ...
    case NV_CGPS_ME_RESERVED2_I:
      ...
      /* LTE TTR will be disabled by default. */
      /* This is a public file, so #define cannot be used */
      ((nv_item_type*)item_ptr)->cgps_me_reserved2 |= (1 << 8); //ME_RESERVED2_DISABLE_LTE_TIME_TRANSFER;
      break;
    ...
    case NV_AAGPS_IPC_DM_THREAD_MASK_I:
      ...
      for (i = 0; i < NUM_OF_AAGPS_IPC_DM_THREAD_MASK_I; i++)
      {
        ...
        else
        {
          ...
          {
            /* Enable IPC logging for TLE task */
            ((nv_item_type *)item_ptr)->aagps_ipc_dm_thread_mask[i] = 0x0FFFFFFF;
          }
          else
          {
...
boolean mgp_nv_read (nv_items_enum_type nv_item, void* item_ptr, os_ThreadIdEnumType thread_id)
{
  ...
  if (item_ptr == NULL)
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_READ: FAILED - NULL parameter", 0, 0, 0);
    return FALSE;
  }
  ...
  if (cmd_buf.status != NV_DONE_S)
  {
    ...
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_MED,"MGP_NV_READ: Reading default value for item %d", nv_item, 0, 0);
      return TRUE;
    }
    else
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_ERROR,"MGP_NV_READ: FAILED - Error Status %d", cmd_buf.status, 0, 0);
      return FALSE;
    }
  }
...
void mgp_nv_ValidateIpcMasks(uint32 * p_NvIpcMasks)
{
  ...
  for(u_i=0; u_i<NUM_OF_AAGPS_IPC_DM_THREAD_MASK_I; ++u_i)
  {
    if(p_NvIpcMasks[u_i])
    {
      MSG_3(MSG_SSID_MGP, MSG_LEGACY_MED,"cd_NvInit: Requested NV_AAGPS_IPC_DM_THREAD_MASK_I is used",
              0, 0, 0);
      return;
    }
  }
  ...
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_MED,"cd_NvInit: NV_AAGPS_IPC_DM_THREAD_MASK_I set to default values",
            0, 0, 0);
  }
  else
  {
    MSG_3(MSG_SSID_MGP, MSG_LEGACY_MED,"cd_NvInit: NV_AAGPS_IPC_DM_THREAD_MASK_I unable to set to default values...using 0",
            0, 0, 0);
  }
}
