...
/*============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/loc_mw/aon/src/aon_batchtransac.cpp#1 $ 
...
----------  ---  ----------------------------------------------------------- 
09/27/16    ss    Allow CPI rate for Time only batching to be configurable via NV
...
boolean AonBatchTransac::getEqGmClientConfig(AonGmClientConfig &gmConfig)
{
  ...
  AON_MSG_HIGH("AonBatchTransac : getEqGmClientConfig gmConfig bo min = %d, bo max = %d, session timeout = %d", 
    gmConfig.mGmBackOffMin, gmConfig.mGmBackOffMax, gmConfig.mSessionTimeout);
  
  AON_MSG_HIGH("AonBatchTransac : getEqGmClientConfig gmConfig cpi req rate = %d", gmConfig.mCpiReqRate, 0, 0);

  AON_MSG_HIGH(" Default motion sensing dist GPS: %d, CPI %d", 
...
{
  AON_MSG_HIGH("AonBatchTransac breachRspHandler invoked, mode = %d", mMode, 0,0);
...
boolean AonBatchTransac :: addGfRspHandler(uint32 txId, uint32 gfId, boolean result)
{
  if(TRUE == result && mMode == AON_BATCHING_MODE_TIME_AND_DISTANCE)
  {
    ...
    {
      AON_MSG_HIGH("AonBatchTransac :: addGfRspHandler starting timer with interval = %d secs", mMinInterval,0,0);
