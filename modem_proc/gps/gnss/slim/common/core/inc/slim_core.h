...
============================================================================*/
/* $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/slim/common/core/inc/slim_core.h#1 $ */
...
/*
  For details, check SLIM CORE version tracking QWiki page at:
  http://qwiki.qualcomm.com/qct-gpsone/SLIM/SLIM_CORE#Version
  
  When making any modification to any file in slim\common\core\ folder,
  please make appropriate changes at following places:
    1. Update SLIM CORE version tracking table at the Qwiki link above, and
    2. Update the SLIM CORE version id in slim_core.h header (this) file

  Updated: 
    01/25/2017   -- initial version
*/
#define SLIM_CORE_VERSION_ID  (1)

/*----------------------------------------------------------------------------
 * Preprocessor Definitions and Constants
 * -------------------------------------------------------------------------*/

/*!  Timer interval for initial service discovery */
#define SLIM_INIT_SERVICE_DISCOVERY_INTERVAL_MS (6000) /* 7s */

/*!  Timer interval for initial few SLIM state logging */
#define SLIM_STATE_LOGGING_INIT_INTERVAL_MS    (5000)
#define SLIM_STATE_LOGGING_INIT_INTERVAL_COUNT   (20)

/*!  Timer interval for steady-state SLIM state logging */
#define SLIM_STATE_LOGGING_INTERVAL_MS       (300000)
...
/**
@brief This function returns SLIM-CORE version.

@return SLIM CORE version
*/
uint32_t slim_GetCoreVersion
(
  void
);
...
/**
@brief SLIM core start providers.

This function starts SLIM providers, specifically, starts the service discovery
for SSC and SAML providers. 

This method shall be called on pz_SvcDiscoveryTimer timer expiry well after
other subsystem startup is confirmed, so it is safe to initiate requests to 
external (non-SLIM) tasks.

@sa slim_CoreInit
@sa slim_CoreStart
@ingroup slim_Core

@retval TRUE if service discovery for all providers was success,
        FALSE otherwise
*/
boolean slim_CoreStartProviders
(
  void
);
...
/**
@brief Performs execution of log timer event handling

@param[in] q_EventParam Event parameter.

@retval next log timeout duration in msec
*/
uint32 slim_CoreProcessLogTimerEvent
(
  uint32 q_EventParam
);

/**
@brief Performs execution of service discovery timer event handling
...
@retval TRUE if service discovery was success,
        FALSE otherwise
*/
boolean slim_CoreProcessSvcDiscoveryTimerEvent
(
  uint32 q_EventParam
);
