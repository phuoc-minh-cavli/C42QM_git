...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/src/tm_l1_iface.c#1 $
  $DateTime: 2020/07/21 21:23:44 $
  $Author: pwbldsvc $

  when        who     what, where, why
  --------    ---     ------------------------------------------------------
  04/10/15    kpn     Add AS ID support to lte_ml1_register_gps_event_cb()
  02/27/17    mj      Sanity check for as_id
...
static boolean l1_wcdma_cell_meas_update( l1_cgps_event_data_s_type *p_l1_event_data )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Unsupported event type: %u", 
              p_l1_event_data->e_event_type, 0, 0 );
    return FALSE;
  }
  if ( l1_cell_meas_update_ptr == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "l1_cell_meas_update_ptr is NULL", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending L1 msg to TM thread",0,0,0);
    (void)os_IpcDelete(ipc_msg_ptr);
    ipc_msg_ptr = NULL;
  } /* End status */
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"sending L1 msg to TM thread", 0,0,0);
  } /* end else status */
...
static boolean l1_lte_cell_meas_update( lte_ml1_gps_event_data_s *p_l1_event_data  )
{
  ...
  if ( os_IpcSend(ipc_msg_ptr, (uint32)THREAD_ID_SM_TM) == FALSE )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending LTE L1 msg to TM thread",0,0,0);
    (void)os_IpcDelete(ipc_msg_ptr);
    ipc_msg_ptr = NULL;
	return FALSE;
  } /* End status */
...
void tm_lte_l1_iface_proc_event( void *p_msg )
{
  ...
  if( p_msg == NULL )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "TM_L1_IFACE: Invalid NULL pointer in proc_event", 0, 0, 0 );
    return;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TM_L1_IFACE: tm_lte_l1_iface_proc_event called", 0, 0, 0 );
...
void tm_l1_iface_init( void )
{
...
#ifdef FEATURE_CGPS_LTE_CELLDB
  uint8 u_num_subs;
  sys_modem_as_id_e_type u_sub_index;
#endif
  ...
  /* Register an event handler with WCDMA L1 */
    ...
    #endif
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"WCMDA L1 event_cb reg. failed", 0, 0, 0);
    }
 ...
 */
  ...
  #ifdef FEATURE_CGPS_LTE_CELLDB
  u_num_subs = tm_cm_get_num_of_subs();

  for (u_sub_index = SYS_MODEM_AS_ID_1; u_sub_index < u_num_subs; ++ u_sub_index)
  {
#ifdef FEATURE_GNSS_NO_LL
    lte_ml1_register_gps_event_cb(l1_lte_cell_meas_update);
#else
    lte_ml1_register_gps_event_cb(u_sub_index, l1_lte_cell_meas_update);
#endif /* FEATURE_GNSS_NO_LL */
  }
  #endif
}
...
void tm_l1_iface_send_wcdma_cell_meas_req(sys_modem_as_id_e_type as_id)
{
  ...
  {
    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_l1_iface_send_wcdma_cell_meas_req: Invalid as_id %u", as_id);
    return;
  }  
...
void tm_l1_iface_send_wcdma_cell_meas_req(void)
{
  ...
  if (cmd_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"tm_l1_iface_send_wcdma_cell_meas_req: cmd_ptr is NULL",0,0,0);
  return;
  }

  cmd_ptr->cmd_hdr.cmd_id = CPHY_LSM_CELL_MEAS_REQ;

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Sending L1 cmd CPHY_LSM_CELL_MEAS_REQ(0x%x)", cmd_ptr->cmd_hdr.cmd_id, 0, 0 );
