...
/*============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/src/tm_time_util.c#1 $
 ...
 * @param[out] q_TotalDays Total days computed
 *
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"null ptr passed fixrpt=%x utc1=%x utc2=%x", 
              pz_Fix, pz_PdsmTimeOut, pz_UtcTimeOut);
    return FALSE;
  }

  v_RetVal = tm_util_get_fix_time(pz_Fix, &z_Time, &q_TotalDays);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Invalid argument: 0x%x, %d", pz_AgnssTime, b_DeltaT, 0 );
    return FALSE;
  }
...
)
{
  ...
  /* Range check */
  if ( (pz_TimeIn->q_Day > 31) || (pz_TimeIn->q_Month > 12) || (pz_TimeIn->q_Year < 1970) ||
       (pz_TimeIn->q_Hour > 23) || (pz_TimeIn->q_Minute > 59) || (pz_TimeIn->q_Second > 59) ||
       (pz_TimeIn->q_Millisecond > 999) )
  {
    MSG_7( MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Caltime out of range %u, %u, %u time %u, %u, %u, %u",
           pz_TimeIn->q_Day, pz_TimeIn->q_Month, pz_TimeIn->q_Year, pz_TimeIn->q_Hour, pz_TimeIn->q_Minute,
           pz_TimeIn->q_Second, pz_TimeIn->q_Millisecond );
    return ( FALSE );
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_util_construct_utc_time failed! GPSWeek=%u GPSMs=%u UTCMs=%u",
...
)
{
  ...
  /* Sanity */
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Glo Time Info: %u, %u, %u",
      ...
      C_GLO_FOUR_YEAR_UNKNOWN == u_GloNumFourYear)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Glo Time Info Invalid: %u", u_GloNumFourYear, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"UTC Msec From Glo Invalid", 0, 0, 0);
    return FALSE;
  }
...
{
  const sm_NavPositionStructType *pz_NavPos = NULL;

  if (NULL == pz_Fix || NULL == pz_TimeOut || NULL == q_TotalDays)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"null ptr passed in=%x out=%x", pz_Fix, pz_TimeOut, 0);
    return FALSE;
  }
...
boolean tm_util_get_utc_total_secs(const sm_FixInfoStructType *pz_GnssFix, int64 *pr_total_utc_secs)
{
  ...
  q_GpsTimeSec = (pz_GnssFix->z_NavPos.q_GpsTimeMs/1000L);
  
  /* Get complete GPS time and then convert it into complete UTC time for leap second time change comparison*/
  *pr_total_utc_secs = ((int64)pz_GnssFix->z_NavPos.w_GpsWeek * DAYS_IN_ONE_WEEK * DAY_SECS) + (int64)q_GpsTimeSec
                   - (int64)tm_core_info.config_info.z_LeapSecondInfo.u_DeltaLeapSec + (GPS_TO_UTC_DELTA_MSECS/1000L);

  if(*pr_total_utc_secs < (int64)0)
  {
    return FALSE;
  }
  return TRUE;
