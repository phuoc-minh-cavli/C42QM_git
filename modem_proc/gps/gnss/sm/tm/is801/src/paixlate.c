...
$Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/is801/src/paixlate.c#1 $
...
do{                                                                        \
  ...
  {                                                                        \
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Malformed packet. Insufficent bytes %d to decode",          \
                    b_len, 0, 0);                                          \
    return FALSE;                                                          \
  }                                                                        \
}while(0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"SESS_START & SESS_SOURCE both set to 1",0,0,0);
    status = FALSE;
  }
  else if (int_ptr->sess_start && int_ptr->sess_end)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"SESS_START & SESS_END both set to 1",0,0,0);
    status = TRUE;  /* this case could be erroneous, but not necessarily */ /*lint !e506 */
  }
  else if (int_ptr->pd_msg_type != PD_MSG_TYPE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv RECV'D PD_MSG_TYPE value",0,0,0);
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Insufficient bytes to decode", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"REQHDR RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv REQ_CODE value in REQ HDR",0,0,0);
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"PR RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"PPM RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"LR_PDE_REQ RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos%8)));
    status = FALSE;
  }
...
)
{
  ...
  if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"TOM RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos%8)));
    status = FALSE;
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv cancel_type value in CACK req",0,0,0);
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"CACK RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"RESPHDR1 RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"RESPHDR2 RES=%d mp=%d, nb=%d", reserved, msg_pos, (8-(msg_pos % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"decoded length mismatch in LR resp",0,0,0);
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"LR RES=%d mp=%d, nb=%d", reserved, offset, (8-(offset % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  if (TRUE == int_ptr->fix.code_ph_par_incl) /*lint !e506 */
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv REF_PN value in AA resp",0,0,0);
    }
  }
  ...
  for (i=0; i < int_ptr->fix.num_sv && i< MAX_NUM_SATS; i++) 
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv SV_PRN num in AA resp",0,0,0);
      return FALSE;
    }
    ...
    if (TRUE == int_ptr->fix.dopp_incl) /*lint !e506 */
    {
      ...
      if (TRUE == int_ptr->fix.add_dopp_incl) /*lint !e506 */
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv DOPPLER_WIN value in AA resp",0,0,0);
          return FALSE;
        }
        else
        {
    ...
    if (TRUE == int_ptr->fix.code_ph_par_incl) /*lint !e506 */
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"Inv sv_code_ph in AA resp",0,0,0);
        return FALSE;
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"Inv sv_code_ph_int in AA resp",0,0,0);
        return FALSE;
      }
      else
      {
...

    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"Inv sv_code_ph_win in AA resp",0,0,0);
        return FALSE;
  ...
  if ((offset + BYTE_PAD)/8 != resp_par_length) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"decoded length mismatch in AA resp",0,0,0);
  
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"AA RES=%d mp=%d, nb=%d", reserved, offset, (8-(offset % 8)));
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv REF_BIT value in SA resp",0,0,0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"num_dr_p=%d",int_ptr->fix.num_dr_p,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"part num > total num parts SA resp",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Insufficient buffer %d to decode all SA bits %d",MAX_NAV_BITS_IN_BYTES, int_ptr->fix.dr_size, 0);
  }
  ...
  for (i = 0; i < num_dr_p; i++) 
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Insufficient number of bytes %d. need %d Malformed PDDM", 
    ...
    for (j = 0; j < int_ptr->var[i].num_sv_dr; j++) 
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv SV_PRN value in SA resp",0,0,0);
        return  FALSE;
      }
    }
  }
  ...
  if (((offset) + BYTE_PAD)/8 != resp_par_length) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"decoded length mismatch in SA resp",0,0,0);
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"mp=%d, rpl=%d", offset, resp_par_length, 0);
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"SA RES=%d mp=%d, nb=%d", reserved, offset, (8-(offset % 8)));
    status = FALSE;
  } 
  else
  {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"Almanac response part num > total parts",0,0,0); 
    return FALSE;
  }
  ...
  if ((offset + BYTE_PAD)/8 != resp_par_length) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"decoded length mismatch in Almanac resp",0,0,0);
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"offset = %d, rpl=%d", offset, resp_par_length, 0);
  
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"Almanac RES=%d offset=%d, nb=%d", reserved, offset, (8-(offset % 8)));
...
)
{
  ...
  if (int_ptr->fix.u_Part_Num > int_ptr->fix.u_Total_Parts)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"total parts > part num GPS Ephemeris response",0,0,0);
    return FALSE;
  }
  ...
  if ((offset + BYTE_PAD)/8 != resp_par_length) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"decoded length mismatch in Ephemeris resp",0,0,0);
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"offset=%d,rpl=%d", offset,resp_par_length, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv resp_par_len in REJ resp",0,0,0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv REQ_CODE in REJ resp",0,0,0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"inv REJ_REASON in REJ resp",0,0,0);
    status = FALSE;
  }
  else if (reserved != PAI_RESERVED_VAL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_FATAL,"REJ RES=%d offset=%d, nb=%d", reserved, offset, (8-(offset % 8)));
    status = FALSE;
  }
  else
  {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Input pointer is NULL", 0, 0, 0);
    return FALSE;
  }
  ...
  switch(req_msg_ptr->request_type)
  {
    case IS801_FWD_LINK_REQ_MASK_MSI:
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "MS Info Request.", 0, 0, 0);
      break;

    case IS801_FWD_LINK_REQ_MASK_AMWF:
      /* AMWF request contains no additional fields, so nothing to decode */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "AMWF Request.", 0, 0, 0);
      break;

    case IS801_FWD_LINK_REQ_MASK_PRM:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "PR Request.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_REQ_MASK_PPM:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "PPM Request.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_REQ_MASK_CAN:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "CACK Request.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_REQ_MASK_LR:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "LR PDE Request.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_REQ_MASK_TOM:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "TOM Request.", 0, 0, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Malformed PDDM. Insufficient bytes", 0, 0, 0);
    return FALSE;
  }
  ...
  switch(resp_par_ptr->resp_type)
  {
    case IS801_FWD_LINK_RESP_MASK_REJ:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Rej Response.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_RESP_MASK_AA:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "AA Response.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_RESP_MASK_SA:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "SA Response.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_RESP_MASK_ALM:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "ALM Response.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_RESP_MASK_EPH:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED, "EPH Response.", 0, 0, 0);
    ...
    case IS801_FWD_LINK_RESP_MASK_LR:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "LR Response.", 0, 0, 0);
    ...
    /* IS801-B handshake compliant */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "Unsupported response %d, ignore it", 0, 0, 0);
      retVal = TRUE;
    break;
