...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/1x_up/vx/src/tm_vx_ni_client.c#1 $  
  ...
  */
  ...
  {     
    ...
    else if(tm_vx_ni_session_info.pos_tech == TM_VX_MT_SMS_POS_TECH_IND_CELL_SECTOR)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Cell/sector report", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Pos tech for tracking session", 0, 0, 0);
      return FALSE;
    }
  }
...
boolean tm_vx_ni_client_sess_param_check(void)
{
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Allow cell/sector NI Session", 0, 0, 0);
      return TRUE;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"NI Session is not allowed by TM-Core", 0, 0, 0);
      return FALSE;
    }
  }
}
...
boolean tm_vx_ni_client_start_session(void)
{
  ...
  if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V1_ONLY)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Starting V1 NI session", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Starting V2 NI session", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Vx config", 0, 0, 0);
    ret_val = FALSE;
  }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Reset NI Client", 0, 0, 0);
...
void tm_vx_ni_client_tracking_timer_handler(void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"NI tracking timer expires, num of fixes left %d", tm_vx_ni_session_info.pd_option_param.fix_rate.num_fixes - tm_vx_ni_session_info.current_fix_count, 0, 0);
  
  if(tm_vx_ni_client_continue_tracking() == FALSE)
  {
    if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed to continue V2 tracking session, send cancel MO SMS to MPC", 0, 0, 0);
