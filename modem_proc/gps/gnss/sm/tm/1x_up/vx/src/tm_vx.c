...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/1x_up/vx/src/tm_vx.c#1 $  
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid msg, can not proceed with logging", 0, 0, 0);
    return;
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid input parameters", 0, 0, 0);  
    return;
  }
  ...
  else if(recv_data_type == TM_VX_COMM_RECV_STATE_PAYLOAD)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"log buffer size too small", 0, 0, 0);  
    }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid app_index", 0, 0, 0);
    return FALSE;
  }  
}
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Link Type", 0, 0, 0);
    return FALSE;
  }
}
...
boolean tm_vx_app_trustness_setting(pdsm_client_type_e_type app_type)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"NI is non-Trusted..",0,0,0);
    return FALSE;
  }
  ...
  if(app_type == PDSM_CLIENT_TYPE_BROWSER)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"WAP is Trusted..",0,0,0);
      return TRUE; /*lint !e506 */
    }
  }
  else if(app_type == PDSM_CLIENT_TYPE_JAVA)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"JAVA is Trusted..",0,0,0);
      return TRUE; /*lint !e506 */
    }
  }
  else if(app_type == PDSM_CLIENT_TYPE_BREW)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"BREW is Trusted..",0,0,0);
      return TRUE; /*lint !e506 */
    }
  }
  else /* MS Resident */
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MS resident is Trusted..",0,0,0);
      return TRUE; /*lint !e506 */
    }
  }
...
void tm_vx_ruim_read_complete(const prtl_event_u_type  *event_payload)
{
   ...
   MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RUIM ASYNC READ MPC IP: %x PORT :%x",tm_vx_gen_info.vx_config.vx_mpc_adrs, tm_vx_gen_info.vx_config.vx_mpc_port_id, 0);
}
...
boolean tm_vx_ip_disconnect_from_mpc(boolean force_abort)
{
  ...
  if(ret_val == PDCOMM_RESULT_OK)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM Vx MPC Disconnected", 0, 0, 0);
      (void)tm_vx_update_comm_state(TM_VX_COMM_TYPE_TCP, TM_VX_COMM_STATE_DISCONNECTED);
      return TRUE; /*lint !e506 */
    }
    else if(ret_val == PDCOMM_RESULT_WAIT)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM Vx MPC Disconnecting", 0, 0, 0);
      (void)tm_vx_update_comm_state(TM_VX_COMM_TYPE_TCP, TM_VX_COMM_STATE_DISCONNECTING);
      return TRUE; /*lint !e506 */
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"pd_comms_app_disconnect returned error: %d",ret_val,0,0);
      return FALSE;
    }
}
...
) 
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Re-initialize TM VX", 0, 0, 0);  
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Abort Vx IP connection", 0, 0, 0);
...
)
{
  ...
  if(start_sess_req_param == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null start_sess_req_param", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Client type out of range", 0, 0, 0);
    return TM_VX_GENERAL_CHECK_PARAMETER_ERROR;  
  }
  ...
  if(tm_vx_gen_info.app_info[start_sess_req_param->client_type].trusted == FALSE)
  {
    ...
    if((active_app_idx > (int)PDSM_CLIENT_TYPE_NONE) && (active_app_idx < (int)PDSM_CLIENT_TYPE_MAX))
    {
      ...
      */
      if(active_app_idx == PDSM_CLIENT_TYPE_NI)
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MO over NI, need user consent", 0, 0, 0);
      ...
      else
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MO over MO is not allowed", 0, 0, 0);
        ...
        else
        {
          ...
          */
          if(active_app_idx == PDSM_CLIENT_TYPE_BREW)
          {
            ...
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"MO over MO is not allowed", 0, 0, 0);
              ret_val = TM_VX_GENERAL_CHECK_PARAMETER_ERROR;
            }
          }
          else
    ...
    {
      /* No active Vx session */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"No active vx session", 0, 0, 0);
      ret_val = TM_VX_GENERAL_CHECK_OK;
    }      
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Bypass check for trusted cleint type %d", start_sess_req_param->client_type, 0, 0);
    ret_val = TM_VX_GENERAL_CHECK_OK;
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Recv'd NULL pointer", 0, 0, 0);  
    ret_val = FALSE;
  }
  else
  {
...
boolean tm_vx_sms_connect(void)
{
  ...
  if(tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_SMS].handle_valid == FALSE)
  {
    ...
    if(ret_val != PDCOMM_RESULT_OK)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"pd_comms_app_get_handle returned error: %d",ret_val,0,0);
      return FALSE;
    }
    else
    {
    ...
    &&(tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_SMS].comm_state != TM_VX_COMM_STATE_CONNECTED))
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"pdcomm_client_connect returned error: %d", ret_val, 0, 0);
      return FALSE;
    }
    else
    {
...
void tm_vx_get_pde_info(pdsm_pd_server_info_s_type *server_info)
{
  ...
  if(server_info==NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"NULL server", 0, 0 ,0);
    return;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"No active client", 0, 0 ,0);
  }  
}
...
boolean tm_vx_handle_mpc_connected(void)
{
  ...
  if((active_app_idx > (int)PDSM_CLIENT_TYPE_NONE) && (active_app_idx < (int)PDSM_CLIENT_TYPE_MAX))
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Connected to MPC when app is %d state", tm_vx_gen_info.app_info[active_app_idx].app_state, 0, 0);
    ...
    switch(tm_vx_gen_info.app_info[active_app_idx].app_state)
    {
      case TM_VX_APP_STATE_WAIT_TO_SEND_SPPREQ:
      {
        ...
        {      
          ...
          if(msg_length > 0)
          {
            ...
            if(ret_val != PDCOMM_RESULT_WAIT)
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error (%d) when trying to send data",ret_val,0,0);
              result = FALSE;
            }
            else
            {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error when createing SPPReq", 0, 0, 0);
            result = FALSE;
          }
      ...
      case TM_VX_APP_STATE_WAIT_TO_SEND_POS_RPT:
      {
        if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
        {
          ...
          {          
            ...
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error when createing V2 Pos report", 0, 0, 0);
              result = FALSE;
            }          
            (void)pdsm_freebuf((char *)msg_ptr);
          }
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Connected to MPC in an invalid state", 0, 0, 0);
          result = FALSE;          
        }
        break;
      }
      ...
      case TM_VX_APP_STATE_WAIT_TO_SEND_CANCEL:
      {
        if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
        {
          ...
          {          
            ...
            {
              MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error when createing V2 mo cancel", 0, 0, 0);
              result = FALSE;
            }          
            (void)pdsm_freebuf((char *)msg_ptr);
          }
      ...
      default:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid state ( %d ) for MPC connection", tm_vx_gen_info.app_info[active_app_idx].app_state, 0, 0);
        result = FALSE;
      break;
    }  
  }
...
boolean tm_vx_handle_tcp_incomming_data(const void *msg_data_ptr)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Received Null msg_data_ptr",0,0,0);
    return FALSE;
  }
  ...
  if((active_app_idx > (int)PDSM_CLIENT_TYPE_NONE) && (active_app_idx < (int)PDSM_CLIENT_TYPE_MAX))
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TCP Msg length = %d", read_hdr_ptr->w_data_len, 0, 0);
      ...
      */    
      if(tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].recv_state == TM_VX_COMM_RECV_STATE_HEADER)
      {
        ...
        if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V1_ONLY)
        {
          ...
          if((*((uint8 *)read_hdr_ptr->p_data) == (uint8)TM_VX_VERSION_V1_ONLY) && (read_hdr_ptr->w_data_len >= TM_V1_TCP_HDR_SIZE))
          {
            //tm_vx_gen_info.sess_info.v1_sppresp.version = TM_VX_VERSION_V1_ONLY;          
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length = *((uint8 *)read_hdr_ptr->p_data + 1); /*lint !e415 */
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length <<= 8;
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length |= *((uint8 *)read_hdr_ptr->p_data + 2); /*lint !e415 !e416 !e831 */
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length -= 3;
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"V1 Msg Payload length = %d", tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length, 0, 0);
            ret_val = TRUE; /*lint !e506 */
          }
          else
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Recv'd known message", 0, 0, 0);
            ret_val = FALSE;
          }
        }
        else if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
        {
          ...
          if(   (*((uint8 *)read_hdr_ptr->p_data) == (uint8)TM_VX_VERSION_V2_ONLY) && (read_hdr_ptr->w_data_len >= TM_V2_TCP_HDR_SIZE)
             && ((*((uint8 *)read_hdr_ptr->p_data + 1) == TM_V2_MESSAGE_TYPE_SPPRESP) || (*((uint8 *)read_hdr_ptr->p_data + 1) == TM_V2_MESSAGE_TYPE_POS_RPT_RSP))
            ) /*lint !e415 */
          {
            if(*((uint8 *)read_hdr_ptr->p_data + 1) == TM_V2_MESSAGE_TYPE_SPPRESP) /*lint !e415 */
            {
            ...
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length = *((uint8 *)read_hdr_ptr->p_data + 2); /*lint !e415 !e416 !e831 */
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length <<= 8;
            tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length |= *((uint8 *)read_hdr_ptr->p_data + 3); /*lint !e415 !e416 !e831 */
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"V2 Msg Payload length = %d", tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].incoming_data_length, 0, 0);
            ret_val = TRUE; /*lint !e506 */
          }
          else
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Recv'd unknown message", 0, 0, 0);
            ret_val = FALSE;
          }
        }       
        ...
        if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V1_ONLY)
        {
          switch(tm_vx_gen_info.app_info[active_app_idx].app_state)
          {
            case TM_VX_APP_STATE_WAIT_TO_RECV_SPPRESP:
            {
              ...
              {
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error decoding V1 SPPResp", 0, 0, 0);
              }
        ...
        else if (tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
        {
          switch(tm_vx_gen_info.app_info[active_app_idx].app_state)
          {
            case TM_VX_APP_STATE_WAIT_TO_RECV_SPPRESP:
            {
              ...
              {
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error decoding V2 SPPResp", 0, 0, 0);
              }
            ...
            {              
              ...
              {
                MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error decoding V2 Pos Report Response", 0, 0, 0);
              }
    ...
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Received data of zero length => Server tears down the connection", 0, 0, 0);
      ret_val = FALSE;
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"No active app", 0, 0, 0);
    ret_val = FALSE;  
  }  
...
boolean tm_vx_handle_tcp_read_notification(void)
{
  ...
  if(tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].recv_state == TM_VX_COMM_RECV_STATE_HEADER)
  {
    if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V1_ONLY)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Reading V1 header",0,0,0);    

      read_params.w_data_len = TM_V1_TCP_HDR_SIZE;
    }
    else if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Reading V2 header",0,0,0);
      read_params.w_data_len = TM_V2_TCP_HDR_SIZE;
    }  
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Reading Vx payload", 0, 0, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error (%d) when trying to send data", result, 0, 0);
    return FALSE;
  }
  else
  {
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Vx TCP/IP comm recv'd state change notification, new stae = %d", notify_ptr->z_params.e_new_state, 0, 0);
    ...
    else if(notify_ptr->z_params.e_new_state == PDCOMM_STATE_DISCONNECTED)
    {
      ...
      {    
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MS-MPC connection issue during SPPREQ/SPPRESP send/recv. Force re-init.", 0,0,0);
          (void)os_TimerStop(tm_vx_gen_info.sess_info.mpc_connection_timer);        
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Unknown state %d",notify_ptr->z_params.e_new_state,0,0);
      result = FALSE;
    }
  }  
  else if(notify_ptr->e_type == READ_DATA)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Vx TCP/IP comm recv'd read data notification", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read event in Disconnecting state after receiving SPPResp, ignore it", 0, 0, 0);
      result = TRUE;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Read data in error state %d", tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_TCP].comm_state, 0, 0);
      result = FALSE;
    }
  }
  else if(notify_ptr->e_type == ERROR_RPT)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Vx TCP/IP comm recv'd error_rpt notification", 0, 0, 0);
   (void)tm_vx_update_comm_state(TM_VX_COMM_TYPE_TCP, TM_VX_COMM_STATE_DISCONNECTED);
    ...
    if((tm_vx_gen_info.sess_info.active_app_index > (int)PDSM_CLIENT_TYPE_NONE) && (tm_vx_gen_info.sess_info.active_app_index < (int)PDSM_CLIENT_TYPE_MAX))
    {
      if (tm_vx_gen_info.app_info[tm_vx_gen_info.sess_info.active_app_index].app_state == TM_VX_APP_STATE_WAIT_TO_SEND_SPPREQ) 
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MS-MPC connection issue during SPPREQ. Force stop timer.", 0,0,0);
        (void)os_TimerStop(tm_vx_gen_info.sess_info.mpc_connection_timer);      
  	    tm_vx_ms_mpc_timer_handler();  /* Force the timer expiration handler routine to clean up Vx state machine */
      }
      else if  (tm_vx_gen_info.app_info[tm_vx_gen_info.sess_info.active_app_index].app_state == TM_VX_APP_STATE_WAIT_TO_RECV_SPPRESP)
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MS-MPC connection issue during SPPRESP. Force stop timer.", 0,0,0);
        (void)os_TimerStop(tm_vx_gen_info.sess_info.t7_timer);   
...
void tm_vx_release_current_session(void)
{
  ...
  if((active_app_idx > (int)PDSM_CLIENT_TYPE_NONE) && (active_app_idx < (int)PDSM_CLIENT_TYPE_MAX))
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Release current active session", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"No active session", 0, 0, 0);
  }
}
...
{  
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Trying to resume MO, but pending idx is invalid", 0, 0, 0);
  }
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_vx_handle_client_init", 0, 0, 0);
  ...
  {    
    ...
    {    
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid client type %d", client_type, 0, 0);
        ret_val = FALSE;
      }
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null app_info_ptr", 0, 0, 0); 
      ret_val = FALSE;
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null clint_init_info_ptr", 0, 0, 0);
    ret_val = FALSE;
  }
...
void tm_vx_handle_ms_abort_session_req(const tm_sess_stop_reason_e_type abort_reason)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Ignore pd session done", 0, 0, 0);
  }
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_vx_handle_client_release", 0, 0, 0);
  ...
  {    
    if(tm_vx_gen_info.sess_info.active_app_index == clint_release_info_ptr->client_type)
    {
      if(clint_release_info_ptr->client_type == PDSM_CLIENT_TYPE_BREW)
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Release Vx session for BREW client", 0, 0, 0);
          tm_vx_handle_ms_abort_session_req(TM_STOP_REASON_GENERAL_ERROR);
          ret_val = TRUE;
        }
        else
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Can not release Vx session because of Brew class id does not match", 0, 0, 0);
          ret_val = FALSE;
        }
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Release Vx session for MI client", 0, 0, 0);
        tm_vx_handle_ms_abort_session_req(TM_STOP_REASON_GENERAL_ERROR);
        ret_val = TRUE;
      }
    }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Client type does not match", 0, 0, 0);
      ret_val = FALSE;    
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null clint_release_info_ptr", 0, 0, 0);
    ret_val = FALSE;
  }
...
)
{
    ...
    &&(tm_vx_gen_info.sess_info.pending_mo_app_index != PDSM_CLIENT_TYPE_NONE))
  {
    ...
    */
    ...
    if(app_resp == PDSM_LCS_NOTIFY_VERIFY_DENY)
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"User choose MO over NI", 0, 0, 0);
    ...
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"User continue with NI", 0, 0, 0);
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Ingore user resposne", 0, 0, 0);
    }
  }
}
...
)
{
  ...
  if((read_hdr_ptr->w_data_len > 0) && (read_hdr_ptr->w_data_len <= TM_VX_MAX_MSG_SIZE))
  {
    ...
    if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V1_ONLY)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Handle V1 MT SMS", 0, 0, 0);
        ...
        if(ret_val)
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed V1 MT SMS check", 0, 0, 0);
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error decoding V1 MT SMS", 0, 0, 0);
        }
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Got MT SMS with wrong version", 0, 0, 0);
        (void) tm_v1_send_mo_sms(TM_VX_MSG_V1_CONSENT_IND_PROTOCOL_ERROR,
                          &(tm_vx_gen_info.sess_info.incoming_v1_mt_sms));
      }
    }
    else if(tm_vx_gen_info.vx_config.vx_version == TM_VX_VERSION_V2_ONLY)
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Handle V2 MT SMS", 0, 0, 0);
        ...
        if(ret_val)
        {
          ...
          {
            MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed V2 MT SMS check", 0, 0, 0);
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error decoding V2 MT SMS", 0, 0, 0);
        }
  ...
  else
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Received SMS Msg with invalid length: %d", read_hdr_ptr->w_data_len, 0, 0);
...
)
{
  ...
  if(notify_ptr->e_type == STATE_CHANGE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Vx SMS comm recv'd state change notification, new stae = %d", notify_ptr->z_params.e_new_state, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Vx SMS comm recv'd read data notification", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Read data in error state %d", tm_vx_gen_info.comm_info[TM_VX_COMM_TYPE_SMS].comm_state, 0, 0);
      result = FALSE;
    }
  }
  return result;
...
)
{
  ...
  if(msg_data_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Null msg_data_ptr", 0, 0, 0);
    return FALSE;
  }
  ...
  switch(msg_type)
  {
    ...
    default:
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Received unknown message", 0, 0, 0);
      ret_val = FALSE;
...
void tm_vx_ms_mpc_timer_handler(void)
{      
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MS-MPC timer expired", 0, 0, 0);
...
void tm_vx_auth_duration_timer_handler(void)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Authention timer expired", 0, 0, 0);
...
void tm_vx_t7_timer_handler(void)
{      
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"T7 timer expired", 0, 0, 0);
  ...
  if(timer_param != NULL)
  {
    if((timer_param->q_TimerId & TM_1X_UP_TIMER_ID_DISPATCH_MASK) == TM_1X_UP_VX_TIMER_ID_BASE)
    {
      switch(timer_param->q_TimerId)
      {
        ...
        default:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Unknown timer expired", 0, 0, 0);
        break;
      }
    }
    else
...
===========================================================================*/
boolean tm_vx_fallback_to_standalone(boolean v_wake_tm_core)
{
   ...
   */
   req_param.start_param.wake_tm_core         = v_wake_tm_core;  
   ...
   {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MSA Request, should not fall back to Standalone", 0,0,0);
     return FALSE;
   }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"invalid client type", 0, 0, 0);
    return PRTL_START_SESS_REQ_NOT_ALLOWED;  
  }
  ...
  else if(auth_check == TM_VX_APP_AUTH_REQUIRED)
  {
    ...
    if (start_sess_req_param->pd_option.operation_mode!=PDSM_SESSION_OPERATION_MSBASED)
    {
      ...
      if(retVal == TRUE) /*lint !e506 !e731 */
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Waiting for authentication", 0, 0, 0);
        return PRTL_START_SESS_REQ_WAIT;  
      }
      else
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Failure when connecting to MPC; will try to fallback to SA", 0, 0, 0);
        retVal = tm_vx_fallback_to_standalone(TRUE); 
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Successfully called IS801_drv_req after MPC error",0,0,0);
          return PRTL_START_SESS_REQ_ALLOWED;
        }
      }
    }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Unknown error", 0, 0, 0);
    tm_vx_reinit(FALSE, TM_STOP_REASON_NONE);    
    return PRTL_START_SESS_REQ_NOT_ALLOWED;  
  }
...
boolean tm_vx_sess_data_dload_needed(tm_aiding_data_wishlist_s_type* aiding_data_status_ptr)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Pos_Unc=%dm,Eph_validity=%d,Alm_validity=%d",
  ...
  if (operation_mode == PDSM_SESSION_OPERATION_MSBASED)
  {
         ...
         (status_ptr->z_PosUnc.f_PosUnc >= TM_LR_THRESHOLD_NO_POS))
    {
      ...
      */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MS-Based data needed",0,0,0);
      ret_val = TRUE;
    }
  }
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"TM_VX: App state = %d, session_info_type %d", tm_vx_gen_info.app_info[tm_vx_gen_info.sess_info.active_app_index].app_state, sess_info_type, 0);
	  ...
	  (tm_vx_gen_info.app_info[tm_vx_gen_info.sess_info.active_app_index].app_state==TM_VX_APP_STATE_WAIT_TO_SEND_SPPREQ))
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Throttle this connection to MPC, will try to fallback to SA",0,0,0);
      retVal = tm_vx_fallback_to_standalone(TRUE); 
      if(retVal == FALSE) /* if we could not fallback, then abort session */ 
      {  
        tm_vx_reinit(FALSE, TM_STOP_REASON_NONE);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Successfully called IS801_drv_req after MPC throttle",0,0,0);
        return TRUE;
      }
    }

    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"No MPC throttling necessary...",0,0,0);
	  retVal = tm_vx_ip_connect_to_mpc();
  ...
  {
    /* no need to 'wake up tm core' as we are not starting a new session now.
     * we already have a fix! */
    (void) tm_vx_fallback_to_standalone(FALSE);
          ...
          (tm_vx_gen_info.app_info[tm_vx_gen_info.sess_info.active_app_index].pd_req_info.client_id !=  PDSM_CLIENT_TYPE_NI) ) 
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_VX:Unhandled session info while MSB operation in V2 is still pending MPC connection",0,0,0);
    return TRUE;
  }
  else
  {
...
)
{
  ...
  {  
    ...
    switch(*tr_id_ptr & TM_1X_UP_TRANSACTION_ID_DISPATCH_MASK)
    {
      ...
      default:
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid transaction id received", 0, 0, 0);
        result = FALSE;
      break;
    }
  }
  ...
  if(result == FALSE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Handling T7 timer expiry for V2 MO; calling is801 driver to proceed with SA session", 0, 0, 0);

    /* first try to get the SA fix on its way */
    result = tm_vx_fallback_to_standalone(TRUE); 
    ...
    {
       MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Successfully called IS801_drv_req after T7 timer expiry",0,0,0);
    }
  }
  ...
  {  
    if(timer_param != NULL)
    {
      ...
      { 
        ...
        default:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Recv'd unknown timer expiry event", 0, 0, 0);
        break;
      }
    }
    else
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid vxVersion for SetVxConfig: %d", vx_config_info_ptr->vxVersion, 0, 0);
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"null event_payload pointer", 0, 0, 0);
    return FALSE;
  }
    ...
    if(event_type == PRTL_EVENT_TYPE_PHONE)
    {
      ...
      switch(event_payload->phone_event_payload.event)
      {
        ...
        case TM_PHONE_EVENT_IN_UMTS_COVERAGE:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Abort Vx session in UMTS system", 0, 0, 0);
          tm_vx_reinit(TRUE, TM_STOP_REASON_GENERAL_ERROR);
        break;
    ...
    else if(event_type == PRTL_EVENT_TYPE_PA)
    {
      ...
      if(event_payload->pa_event_payload.pa_event == PDSM_PA_1X_MPC_NETWORK_ADRS)
      {
        if(event_payload->pa_event_payload.pa_info_ptr != NULL)
        {
          ...
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Updated MPC address %x and port %d", tm_vx_gen_info.vx_config.vx_mpc_adrs, tm_vx_gen_info.vx_config.vx_mpc_port_id, 0);
        }
      }
    }    
  }
...
)
{
  ...
  if(answer_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"null answer_ptr pointer", 0, 0, 0);
    return FALSE;
  }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_vx_prtl_init", 0, 0, 0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Ignore pd ifno", 0, 0, 0);
  }  
#endif
