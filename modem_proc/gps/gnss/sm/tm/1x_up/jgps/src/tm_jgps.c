...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/1x_up/jgps/src/tm_jgps.c#1 $  
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"mode = %d",mode,0,0);
...
)
{
  ...
  do{
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid input params, fail start req", 0,0,0);
      break;
    }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid Session request mode (%d)", sess_info_ptr->req_mode, 0,0);
...
)
{
  ...
  do{
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid input params, fail stop req", 0,0,0);
      break;
    }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"End current session (curr mode: %d)", curr_mode, 0,0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Sess Req mode(%d) does not match current session mode (%d)",
                smod_req_mode,
...
)
{
  ...
  do{
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid input params, fail info available", 0,0,0);
      break;
    }
    ...
    case PRTL_EVENT_TYPE_PA:
      ...
      if(event_payload->pa_event_payload.pa_event == PDSM_PA_APP_INFO)
      {
        if(event_payload->pa_event_payload.pa_info_ptr != NULL)
        {
          ...
          {
             MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Updated app info", 0,0,0);
          }
        }
        else
        {
      ...
      break;
    default:
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Ignore event type=%d event=%d",event_type,
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"tm_jgps_prtl_init", 0, 0, 0);
