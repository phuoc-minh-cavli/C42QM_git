...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/1x_up/src/tm_1x_up.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    if (status == FALSE)
    {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "Error translating.", 0, 0, 0);
        return 0;
    }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"MSID RUIM READ START",0,0,0);

  memset((void *)&tm_1x_up_nv_read_buffer, 0x0, sizeof(tm_1x_up_nv_read_buffer));   
  (void)tm_core_nv_async_read(NV_CURR_NAM_I,(void *)&tm_1x_up_nv_read_buffer);
} /* End tm_1x_up_read_msid_fields( ) */

/*===========================================================================

FUNCTION   tm_1x_up_read_curr_nam_complete

DESCRIPTION
  This function is used to read NAM parameters from NV for initializing MSID
  The functoin is called after PRTL_EVENT_TYPE_RUIM_NV_READ event is sent by
  tm_core post Asynchronus read completion for NV_CURR_NAM_I.

RETURN VALUE
  None

============================================================================*/
void tm_1x_up_read_curr_nam_complete( const prtl_event_u_type  *event_payload )
{
  byte   tm_1x_up_nam = 0;

  if (event_payload == NULL) 
  {
     MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "MSID RUIM READ Failed, event_payload NULL");
     return;
  }
  ...
  MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_MED, "CURRENT NAM : %d", tm_1x_up_nam);
...
FUNCTION   tm_1x_up_read_post_cur_nam_op

DESCRIPTION
  This function is used to read remaining MSID fields parameters from NV for
  initializing MSID local structure
  The functoin is called after PRTL_EVENT_TYPE_RUIM_NV_READ event is sent by
  tm_core post Asynchronus read completion for NV_CURR_NAM_I and an IPC is
  queued to initiate remaining fields to be read.
 
  IMP: Do not call tm_core_nv_async_read from any sm_nv_async_read/write CB's

RETURN VALUE
  None

============================================================================*/
void tm_1x_up_read_post_cur_nam_op(void)
{
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_MED, "Read MSID fields post CURR NAM");
  /* Read NV 32 NV_MIN1_1*/
  (void)tm_core_nv_async_read(NV_MIN1_I,(void *)&tm_1x_up_nv_read_buffer);
  /* Read NV 33 NV_MIN2_1*/
  (void)tm_core_nv_async_read(NV_MIN2_I,(void *)&tm_1x_up_nv_read_buffer);
  /* Read NV 176 NV_IMSI_MCC_I*/
  (void)tm_core_nv_async_read(NV_IMSI_MCC_I,(void *)&tm_1x_up_nv_read_buffer);
  /* Read Nv 177 NV_IMSI_11_12_I*/
...
void tm_1x_up_read_msid_fields_complete( const prtl_event_u_type  *event_payload )
{
  ...
  if (event_payload == NULL) 
  {
     MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"MSID RUIM READ Failed, event_payload NULL",0,0,0);
     return;
  }

  tm_1x_up_nam = tm_1x_up_nv_read_buffer.min1.nam;
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"CURRENT NAM : %d",tm_1x_up_nam,0,0);

  tm_1x_up_nv_read_buffer.min1.nam = tm_1x_up_nam;
  tm_1x_up_imsi_s1 = event_payload->ruim_read_nv_payload.tm_1x_up_imsi_s1;
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"IMSI_S1 : %d",tm_1x_up_imsi_s1,0,0);

  tm_1x_up_nv_read_buffer.min2.nam = tm_1x_up_nam;
  tm_1x_up_imsi_s2 = event_payload->ruim_read_nv_payload.tm_1x_up_imsi_s2;
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"IMSI_S2 : %d",tm_1x_up_imsi_s2,0,0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"MCC : %d",tm_1x_up_msid_fields.msid.type2.imsi.class_type.zero.type.three.mcc,0,0);
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"IMSI_11_12 : %d",tm_1x_up_msid_fields.msid.type2.imsi.class_type.zero.type.three.imsi_11_12,0,0);
...
void tm_1x_up_msb_throttling_init(void)
{
  ...
  */
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"max[0]=%d,max[1]=%d",tm_1x_up_config.msb_throttling_info.back_off_max_duration[0],tm_1x_up_config.msb_throttling_info.back_off_max_duration[1],0);
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"min[0]=%d,min[1]=%d",tm_1x_up_config.msb_throttling_info.back_off_min_duration[0],tm_1x_up_config.msb_throttling_info.back_off_min_duration[1],0);
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"reset[0]=%d,reset[1]=%d",tm_1x_up_config.msb_throttling_info.back_off_reset_duration[0],tm_1x_up_config.msb_throttling_info.back_off_reset_duration[1],0);
...
void tm_1x_up_ruim_read_complete(const prtl_event_u_type  *event_payload)
{
    if (event_payload == NULL) 
    {
       MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"RUIM 1X UP Read Failed, event_payload NULL",0,0,0);
       return;
    }
    /* INIT the PDE IP Addr */
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"RUIM ASYNC READ 1X UP PDE IP: %x PORT :%x",tm_1x_up_config.pde_adrs, tm_1x_up_config.port_id, 0);
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM_1X_UP Init", 0, 0, 0);
