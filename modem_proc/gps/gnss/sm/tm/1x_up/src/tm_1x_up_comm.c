...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/tm/1x_up/src/tm_1x_up_comm.c#1 $
...
uint32 tm_1x_up_comm_get_next_transaction_id(void)
{
        ...
        ||(pdcomm_info.tr_id_info.curr_id >= (pdcomm_info.tr_id_info.base + pdcomm_info.tr_id_info.range)))
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Transaction id restart at base 0x%x",pdcomm_info.tr_id_info.curr_id,0,0);
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Current transaction id 0x%x",pdcomm_info.tr_id_info.curr_id,0,0);
  }
...
boolean tm_1x_up_comm_get_pde_url_addr( void )
{
  ...
  */
  if(tm_1x_up_session_info.api_param.pd_api_info.pd_option.lsinfo.server_option == PDSM_SERVER_OPTION_USE_DEFAULT)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Default server option", 0, 0, 0);
    i=0;
    ...
    { 
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PDE URL addr length exceeds maximum", 0, 0, 0);
      return FALSE;
    }
    else
    {
              ...
              ( i < ( TM_PDE_MAX_URL_SIZE - 1 ) ) )
      {
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Got malicious port number", 0, 0, 0);
          return FALSE;
        }
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Wrong PDE port", 0, 0, 0);
        return FALSE;
      }
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"PDE port is decoded as %d", connect_param.z_link_params.z_tcp_params.w_ip_port, 0, 0);
    }
  }
  else /* Local */
  {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Local server option", 0, 0, 0);
      i=0;
...
boolean tm_1x_up_comm_connect( void )
{
  ...
  if(TM_CORE_CONFIG_DD(gnss_1x_up_supl_enable) != 0)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Supl failed to open connection to PDE",0,0,0);
      return FALSE;
    }
    return TRUE;
#endif
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Can't get handle!",0,0,0);
    return FALSE;
  }
  ...
  if(tm_1x_up_session_info.api_param.pd_api_info.pd_option.lsinfo.server_option == PDSM_SERVER_OPTION_USE_DEFAULT)
  {
    ...
    else if(tm_1x_up_config.pde_addr_url[0] != '\0')
    {
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Can not get PDE URL addr",0,0,0);
        return FALSE;
      }
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Neither IP nor URL addr is available",0,0,0);
      return FALSE;
    }
  }
  else if(tm_1x_up_session_info.api_param.pd_api_info.pd_option.lsinfo.server_option == PDSM_SERVER_OPTION_USE_LOCAL)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Incorrect server addr_type",0,0,0);
      return FALSE;
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Incorrect server option",0,0,0);
    return FALSE;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"pdcomm_client_connect returned: %d",ret_val,0,0);
  return TRUE;
}
...
boolean tm_1x_up_comm_get_handle( void )
{
  ...
  if(pdcomm_info.handle_valid == FALSE)
  {
    ...
    else
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"pd_comms_app_get_handle returned handle: %d",pdcomm_info.handle.q_handle_id,0,0);
      return TRUE; 
    }
  }
  return TRUE; 
...
)
{
  ...
  if( data == NULL || size == 0 )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Invalid error: Data is null or size (%d)", size, 0,0);
    return FALSE;
  }
  ...
  { 
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PDCOMM TD: pdcomm_client_write returned error: %d",comm_ret_val,0,0);
      ret_val = FALSE;
    }
  }
  return ret_val;
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_1x_up_comm_send:  Error in length passed in.", 0, 0, 0);
    return FALSE;
  }

  if (start == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_1x_up_comm_send:  Null pointer passed in.", 0, 0, 0);
    return FALSE;
  }
     ...
     )
  {
    ...
    if (0 == cust_hdr_size)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_1x_up_comm_send:  Failure making custom header .", 0, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_1x_up_comm_send: length:%d cust_hdr_size: %d, exceeds TM_MAX_BUFFSIZE.",length, cust_hdr_size, 0);
      return FALSE;
    }
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "tm_1x_up_comm_send:  custom header too long %d .", cust_hdr_size, 0, 0);
      /* failure making custom header */
      return FALSE;
    }
	...
	{
	  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR," Overall packet size cannot exceed TM_MAX_BUFFSIZE", 0, 0, 0);
	  return FALSE;
	}
	
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_1x_up_comm_send:  Sending %d bytes with custom header", pdcomm_info.buffer.num_bytes_to_write, 0, 0);
  ...
  else
  {
    ...
    uint16 offset = 0;
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH, "tm_1x_up_comm_send:  Sending %d bytes", pdcomm_info.buffer.num_bytes_to_write, 0, 0);
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR," Overall packet size should be less than length = %d", length, 0, 0);
	  return FALSE;
    }
  }
...
pd_comms_return_type tm_1x_up_comm_disconnect(boolean abrt )
{
  ...
  if(ret_val == PDCOMM_RESULT_OK)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM 1X UP disconnected", 0, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"TM 1X UP disconnecting", 0, 0, 0);
...
boolean tm_1x_up_comm_read(uint16 length, boolean wait_all )
{
  ...
  {   
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error reading the data",0,0,0);
      return FALSE;
    }
...
boolean tm_1x_up_comm_connect_with_pde(void)
{
  if ( tm_1x_up_comm_get_state() == PDCOMM_STATE_CONNECTED)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Already connected!",0,0,0); 
  ...
  else if ( tm_1x_up_comm_get_state() == PDCOMM_STATE_CONNECTING)            
  {
    ...
    */
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Waiting to get connected",0,0,0); 
    return TRUE;
  }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"pdcomm_state=%d",tm_1x_up_comm_get_state(),0,0);

  if(pdcomm_info.wait_for_sess_end_ack == TRUE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Delay close connection as waiting for sess_end ACK",0,0,0);
    return;
  }
  ...
  switch( tm_1x_up_comm_get_state() )
  {
    ...
    case PDCOMM_STATE_CONNECTING:
      ...
      */
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Failed starting disconnect timer",0,0,0);
      }
...
boolean tm_1x_up_handle_comm_read_data(void *msg_data_ptr)
{
    ...
    if(read_hdr_ptr->w_data_len == 0)
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Received data of zero length from TM-Core",0,0,0);
      return FALSE;
    }
    ...
    if ( tm_1x_up_comm_get_state()!= PDCOMM_STATE_CONNECTED )
    {
      ...
      ** connected.
      */
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Received data from TM-Core in error state %d",
    ...
    */
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read data of length %d",read_hdr_ptr->p_data[0],0,0);
      pdcomm_info.num_bytes_to_read = read_hdr_ptr->p_data[0]; 
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Forward data of length %d to IS801 module for decoding",
...
boolean tm_1x_up_handle_comm_ioctl_ack(void *msg_data_ptr)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"IOCTL Ack for req %d",ioctl_data_ptr->q_ioctl_request,0,0);
#ifdef FEATURE_GPSONE_UPDATE_BSID  
  if(ioctl_data_ptr->q_ioctl_request == PDCOMM_IOCTL_CMD_FORCE_DORMANCY)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"force dorm opn successful",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"unforce dorm opn successful",0,0,0);
  }
#endif  
  return TRUE;
}
...
boolean tm_1x_up_handle_comm_write_ack(void *msg_data_ptr)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"PDCOMM TD: write ack received for %d bytes",write_hdr_ptr->q_bytes_written,0,0);
  ...
  if(tm_1x_up_is801_drv_send_and_log_rlink_pddm())
  {
    if(pdcomm_info.wait_for_sess_end_ack == TRUE)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Received sess_end ACK",0,0,0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"Waiting for LR in MSB fix",0,0,0);
...
boolean tm_1x_up_handle_comm_notification(void *msg_data_ptr)
{
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"PDCOMM status update %d", notify_hdr_ptr->e_type, 0, 0);  
    if(notify_hdr_ptr->e_type == READ_DATA)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read notification from TM-Core",0,0,0);
      if ( tm_1x_up_comm_get_state() == PDCOMM_STATE_CONNECTED )
      {
        if(pdcomm_info.recv_state == TM_1X_UP_COMM_RECV_HEADER)
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read IS801 header",0,0,0);
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Read IS801 payload of length %d",pdcomm_info.num_bytes_to_read,0,0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Read data in error state %d", tm_1x_up_comm_get_state(),0,0);
      }
    }
    else if(notify_hdr_ptr->e_type == STATE_CHANGE)
    {
      params = &notify_hdr_ptr->z_params; 
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"State change notification New state %d Curr state %d",params->e_new_state,tm_1x_up_comm_get_state(),0);

      if(params->e_new_state == PDCOMM_STATE_CONNECTED)
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH," Connected With PDE",0,0,0);
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Curr state=%d Prev DisConnect TrId= %x PDCOMM TrId 0x%x",tm_1x_up_comm_get_state(),pdcomm_info.disconnect_tr_id,notify_hdr_ptr->q_transaction_id);
        pdcomm_info.curr_state = PDCOMM_STATE_DISCONNECTED;
        ...
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Previous disconnect completed",0,0,0);
          (void)os_TimerStop(tm_1x_up_session_info.tm_1x_up_disconnect_timer);
          tm_1x_up_comm_reset();
        }
      }
      else if(params->e_new_state == PDCOMM_STATE_CONNECTING)
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"PPP opened",0,0,0);
      ...
      else if(params->e_new_state == PDCOMM_STATE_DISCONNECTING)
      {
        ...
        if ( notify_hdr_ptr->q_transaction_id == pdcomm_info.disconnect_tr_id )
        {
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Previous disconnect in progress",0,0,0);
          (void)os_TimerStop(tm_1x_up_session_info.tm_1x_up_disconnect_timer);
          tm_1x_up_comm_reset();
        }
      }
      ...
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Unknown state %d",params->e_new_state,0,0);
      }
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error report",0,0,0);
      params = &notify_hdr_ptr->z_params; 
...
)
{
    ...
    if(TM_CORE_CONFIG_DD(gnss_1x_up_supl_enable) !=0 )
    {
      ...
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Not expecting to handle comm message", 0, 0, 0);
      return FALSE;
    }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Reset PDCOMM related info",0,0,0);
  pdcomm_info.wait_for_sess_end_ack = FALSE;
