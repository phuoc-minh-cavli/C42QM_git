...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Handle Sess timeout",0,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Timer event not handled",0,0,0);
  }
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Handle get PPM timeout",0,0,0);
  if ( !tm_1x_cp_get_ppm( ) )
  {
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"Handle PPM timeout",0,0,0);
  if (tm_1x_cp_session_active() != TRUE) 
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"PPM timeout not expected",0,0,0);
    return;
  }
  ...
  if(tm_1x_cp_session_info.ppm_timeout == TM_MGP_GET_PPM_TIMEOUT_MIN)
  {
    ...
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Send MSInfo resp",0,0,0);
