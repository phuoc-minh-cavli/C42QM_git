...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/common/src/gnss_msgr_task.c#1 $
  $DateTime: 2020/07/21 21:23:44 $
  $Author: pwbldsvc $
  
when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/30/18   sum     DogHb sequence update
04/10/17   kpn     L+L: Register NAS messages for number of SUBs configured
04/04/17   kpn     Register NAS messages only for MSGR_NUM_VARIANT number of variants
09/27/16   tf      Signal Task Ready after IPC handling is started
07/19/16   kpn     Migration from Inst Id to Variant
...
#include "dsm_pool.h" / * DSM_DS_POOL_SIZE is defined in dsm_init.h . Added that header above and lets if compilation passes*/
#else
#include "dsm_init.h"
#endif
...
static boolean gnss_msgr_setup_client( void )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to get client ID", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to create GNSS MSGR client error=%d", (uint32)estatus, 0, 0);
    return FALSE;
  }
  ...
  if ( estatus != E_SUCCESS )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to add rex_q for GNSS MSGR error=%d", (uint32)estatus, 0, 0);

    msgr_client_delete( gnss_msgr_client_id_ptr );
    if ( estatus != E_SUCCESS )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to delete GNSS MSGR client error=%d", (uint32)estatus, 0, 0);
    }
...
static void gnss_msgr_module_init(void)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GNSS MSGR: client successfully created",0,0,0);
  }
  else
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: client creation failed!",0,0,0);
  }
}
...
static void gnss_msgr_start_up(void)
{
  ...
  /* All initialization complete. Signal Task Ready and start processing messages */
  os_TaskReadyAck();
  os_TaskStopRegister();
...
static void gnss_msgr_handle_task_offline( void )
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: TASK OFFLINE received",0,0,0);
...
{
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: TASK STOP received",0,0,0);
...
static boolean gnss_msgr_send_nas_failure_event(uint32 error_code)
{
  ...
  if (ipc_msg_ptr != NULL)
  {
    ...
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to send IPC mesage",0,0,0);     
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to create IPC mesage",0,0,0); 
    retVal = FALSE;
  } /* End ipc_msg_ptr */
...
static void gnss_msgr_handle_msgr_msg(void)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_MED,"GNSS MSGR: Got MSGR message(cnt=%d), sending NAS_DL_EVENT...", msg_cnt,0,0);
...
void gnss_msgr_proc(void)
{
  ...
  for(;;)
  {
    ...
    if( NULL == ipc_msg_ptr )
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NULL pointer rcvd from os_IpcReceive()", 0, 0, 0);
      continue;
    }
    ...
    if (u_gnss_msgr_task_online)
    {
      ...
      {    
        ...
        default:
          MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Unknown IPC MSG ID",0,0,0);
          break;
      }/* End switch MsgId */
    }
    (void)os_IpcDelete(ipc_msg_ptr); 
...
)
{
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: Starting GNSS MSGR task (ThreadID=%d)...", THREAD_ID_GNSS_MSGR,0,0);
...
boolean gnss_msgr_register_nas_msgs( void )
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Can't registering for NAS MSGR messages since GNSS client not created!",0,0,0);
    return FALSE;
  }

  if (gnss_msgr_is_nas_msg_registered == TRUE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NAS MSGR messages already registered!",0,0,0);
    return FALSE;
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: Registering for NAS MSGR messages",0,0,0);

  /* Registering for all allowed Subs irrespective of the current active profile [single-SIM/Multi-SIM profile]
  ...
  for (u_index = 0; u_index < u_num_subs; u_index ++)
  {
    ...
    estatus = msgr_register_variant(MSGR_GPS_TM,
                                    gnss_msgr_client_id_ptr,
                                    MSGR_ID_REX,
                                    NAS_EMM_DL_GENERIC_NAS_TRANSPORT_IND,
                                    (uint8) e_as_id); // downlink NAS msg

    if ( estatus != E_SUCCESS )
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to register NAS DL message error=%d, variant=%d", (uint32)estatus, e_as_id);
      return FALSE;
    }
    ...
    estatus = msgr_register_variant( MSGR_GPS_TM,
                                     ...
                                     (uint8) e_as_id); // success confirmation for uplink NAS msg

    if ( estatus != E_SUCCESS )
    {
      // de-register the first one
      (void)msgr_deregister_variant(MSGR_GPS_TM,
                                    gnss_msgr_client_id_ptr,
                                    NAS_EMM_DL_GENERIC_NAS_TRANSPORT_IND,
                                    (uint8) e_as_id);

      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to register NAS UL CNF message error=%d, variant=%d", (uint32)estatus, e_as_id);
      return FALSE;
    }
    ...
    estatus = msgr_register_variant(MSGR_GPS_TM,
                                    ...
                                    (uint8) e_as_id); // failure for NAS transport
    ...
    if ( estatus != E_SUCCESS )
    {
      ...
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to register NAS failure IND message error=%d, variant=%d", (uint32)estatus, e_as_id);
      return FALSE;
    }
  }
  ...
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: LTE not enabled, can't register for NAS MSGR messages");
...
boolean gnss_msgr_deregister_nas_msgs(void)
{
  ...
  if (gnss_msgr_is_client_created != TRUE)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Can't deregistering for NAS MSGR messages since GNSS client not created!");
    return FALSE;
  }

  if (gnss_msgr_is_nas_msg_registered != TRUE)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NAS MSGR messages not registered!");
    return FALSE;
  }

  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: Deregistering for NAS MSGR messages");

  /* De-Register for all allowed subs. */
  for (u_index = 0; u_index < u_num_subs; u_index ++)
  {
    /* Variant ID here is nothing but AS ID. Explicilty assigning AS ID constants for variant ID while registering NAS messages
       so as to be inline with the implementation while sending and receiving NAS messages. This is to avoid any porting issues later. */
    ...
    estatus = msgr_deregister_variant( MSGR_GPS_TM,                            
                               gnss_msgr_client_id_ptr,
                               NAS_EMM_DL_GENERIC_NAS_TRANSPORT_IND,
                               (uint8) e_as_id); // downlink NAS msg
    ...
    if (estatus != E_SUCCESS)
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to deregister NAS DL message error=%d, variant=%d", (uint32)estatus, e_as_id);
      bRet = FALSE;
    }
    ...
    estatus = msgr_deregister_variant(MSGR_GPS_TM,
                                      gnss_msgr_client_id_ptr,
                                      NAS_EMM_UL_GENERIC_NAS_TRANSPORT_CNF,
                                      (uint8) e_as_id); // confirmation for uplink NAS msg
    if (estatus != E_SUCCESS)
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to deregister NAS UL CNF message error=%d, variant=%d", (uint32)estatus, e_as_id);
      bRet = FALSE;
    }
    ...
    estatus = msgr_deregister_variant(MSGR_GPS_TM,
                                      gnss_msgr_client_id_ptr,
                                      NAS_EMM_GENERIC_NAS_TRANSPORT_FAILURE_IND,
                                      (uint8) e_as_id); // NAS failure indication
    if (estatus != E_SUCCESS)
    {
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to deregister NAS failure IND message error=%d, variant=%d", (uint32)estatus, e_as_id);
      bRet = FALSE;
    }
  }
...
)
{
  ...
  errno_enum_type  estatus;
  lte_nas_emm_ul_generic_transport_s_type ul_nas_msg = {0};
  ...
  if (gnss_msgr_is_client_created != TRUE)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Can't send NAS MSGR message since GNSS client not created!",0,0,0);
    return FALSE;
  }

  if (pkt_ptr == NULL || pkt_size == 0 || pkt_size > 65535) // 16 bit len, up to 65535
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: invalid NAS MSGR message(size=%d), can't send!",pkt_size,0,0);
    return FALSE;
  }

  if (NULL == &(ul_nas_msg.msg_hdr))
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NULL ptr");
    return FALSE;
  }
  ...
  msgr_set_hdr_variant((msgr_hdr_s *)(&(ul_nas_msg.msg_hdr)), INST_ID_TO_SYS_AS_ID(inst_id));
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NULL MSGR attachment ptr! LPP=0x%x NASID=0x%x",lpp_att_ptr,nasid_att_ptr,0);
    return FALSE;
  }

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: NAS_EMM_UL_GENERIC_NAS_TRANSPORT_REQ payload (DSM size=%d)",pkt_size,0,0);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Only %d bytes pushed to DSM 0x%x (%d payload)", num_bytes, lpp_dsm_ptr, pkt_size);
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Only %d bytes pushed to DSM 0x%x (%d payload)", num_bytes, nasid_dsm_ptr, nasid_bytes);
    dsm_free_packet(&lpp_dsm_ptr); // can take NULL pointer
  ...
  */
  ...
  if (estatus != E_SUCCESS )
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: Failed to send NAS msg with NASID=%d size=%d transid=0x%x",nas_id,pkt_size,nas_transid);
    dsm_free_packet(&lpp_dsm_ptr); // can take NULL pointer
    dsm_free_packet(&nasid_dsm_ptr); // can take NULL pointer
    return FALSE;
  }
  ...
  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: sent NAS msg with NASID=%d size=%d trans_id=0x%x",nas_id,pkt_size,nas_transid);
  return TRUE;

#else /* FEATURE_LTE */

  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: LTE not enabled, can't send NAS MSGR messages",0,0,0);
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: invalid parameter (buf_size=%d)!",buf_size,0,0);
    return FALSE;
  }
  ...
  *inst_id = 0;
  ...
  if (msg_ptr == NULL)
  {
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: No NAS message available!!");
    return FALSE;
  }

  /* Variant extracted is the AS ID. */
  *inst_id = (uint8) SYS_AS_ID_TO_INST_ID(MSGR_GET_VARIANT(&msg_ptr->msg.hdr));
  ...
  {  
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: received DL_GENERIC_NAS_TRANSPORT msg");

    do
    {
      num_attach = msgr_get_num_attach(&msg_ptr->msg.hdr);
      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"GNSS MSGR: No of attach items for pull up = %d", num_attach);

      if (num_attach != 2 ||
          (lpp_att_ptr = msgr_get_attach(&msg_ptr->msg.hdr, 0)) == NULL ||
          (nasid_att_ptr = msgr_get_attach(&msg_ptr->msg.hdr, 1)) == NULL)
      {
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: wrong attachments! num=%d LPP=0x%x NASID=0x%x", 
      ...
      {  
        MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NULL DSM ptr! LPP=0x%x NASID=0x%x!",lpp_dsm_ptr,nasid_dsm_ptr);
        bRet = FALSE;
        break;
      }
      ...
      {
        MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: buf size %d too small (LPP msg size %d)", buf_size, dsm_packet_length);
        bRet = FALSE;
        break;
      }

      MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: Length of LPP Message = %d",dsm_packet_length);
      ...
      if (dsm_pull_bytes != dsm_packet_length || lpp_dsm_ptr != NULL)
      {
        ...
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: LPP DSM 0x%x only %d bytes pulled (length=%d)",
      ...
      {
        MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: received wrong size %d for NAS ID for LPP", nasid_bytes);
        bRet = FALSE; 
        break;
      }
      ...
      {
        // last byte is pulled, DSM will be returned to free queue
        MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: NASID DSM 0x%x only %d bytes pulled (length=%d)",
      ...
      MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_LOW,"GNSS MSGR: received NAS ID %d (size %d)", *nas_id, nasid_bytes);
    }while(0);
  ...
  { 
    MSG_1(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: received NAS_EMM_UL_GENERIC_NAS_TRANSPORT_CNF msg TrasID=0x%x",
             msg_ptr->msg.nas_ul_cnf.trans_id);
  }
  else if (msg_ptr->msg.hdr.id == NAS_EMM_GENERIC_NAS_TRANSPORT_FAILURE_IND)
  {
    MSG_2(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: received NAS_EMM_GENERIC_NAS_TRANSPORT_FAILURE_IND msg TrasID=0x%x Failure=%d",
             msg_ptr->msg.nas_failure_ind.trans_id, (uint32)msg_ptr->msg.nas_failure_ind.cause_type);
  ...
  {  
    MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"GNSS MSGR: received unknown message from NAS module, discarding it");
    bRet = FALSE;
  }
  ...
  MSG(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"GNSS MSGR: LTE not enabled, can't receive NAS MSGR messages");
...
)
{
  return FALSE;
#if 0
  smart_lpp_cp_receive_nas_msg(pkt_buf, buf_size, pkt_size, nas_id, inst_id);
#endif
}
