...
/*===========================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/sm/common/src/lsmp_api.c#1 $ $DateTime: 2020/07/21 21:23:44 $ $Author: pwbldsvc $
...
boolean lsmp_get_param(lsmp_get_msg_type msg)
{
  ...
  switch (msg.param)
  {
    case LSMP_PREMIUM_SERVICES_TYPE:
		...
		{
		  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in param_value_size in request",0,0,0); 
		  return status;
		}
        break;
	...
	case LSMP_DEVICE_CONFIG_TYPE:
		...
		{
		  MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in param_value_size for lsmp_device_config request",0,0,0); 
		  return status;
		}
        break;	

	default:
		MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Unknown param type in request",0,0,0);
		return status;
  }
  ...
  if (ipc_msg_ptr == NULL)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"IPC message allocation failed for %d",msg.param,0,0);
  }
  else
  {
    ...
    if (status == FALSE)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending LSMP GET PARAM msg_id=%d to TM thread",ipc_msg_ptr->q_MsgId,0,0);
      status = os_IpcDelete(ipc_msg_ptr);
      ipc_msg_ptr = NULL;
    } 
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Sending LSMP GET PARAM msg_id=%d to TM thread", ipc_msg_ptr->q_MsgId,0,0);
	  status = TRUE;
    } 
  }
#endif
...
boolean lsmp_send_premium_service_ind_status(boolean ind_status)
{
  ...
  if (NULL == ipc_msg_ptr)
  {
    MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"IPC message allocation failed for lsmp_send_premium_service_ind_status", 0, 0, 0);
  }
  else
  {
    *(ipc_msg_ptr->p_Data) = ind_status;
    ...
    if (FALSE == status)
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_ERROR,"Error in sending LSMP SEND PREMIUM SERVICE IND STATUS msg_id=%d to TM thread",ipc_msg_ptr->q_MsgId, 0, 0);
      status = os_IpcDelete(ipc_msg_ptr);
      ipc_msg_ptr = NULL;
    }
    else
    {
      MSG_3(MSG_SSID_GPSSM, MSG_LEGACY_HIGH,"Sending LSMP SEND PREMIUM SERVICE IND STATUS msg_id=%d to TM thread", ipc_msg_ptr->q_MsgId, 0, 0);
	  status = TRUE;
    }
  }
#endif
