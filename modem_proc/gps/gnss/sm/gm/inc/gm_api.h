...
#define GEOFENCE_POS_SOURCE_NV_GTP_2_0_MASK 0x4
...
typedef enum
{
  ...
  GEOFENCE_CLIENT_GTP_1_0,   /*GTP 1.0  client*/
  GEOFENCE_CLIENT_GTP_2_0,   /*GTP 2.0  client*/
...
typedef enum
{
  ...
  GEOFENCE_CLIENT_POS_FIX_GTP_1_0_CAPABLE = 0x100, /*GTP 1.0 Fix capable*/
  GEOFENCE_CLIENT_POS_FIX_GTP_2_0_CAPABLE = 0x200, /*GTP 2.0 Fix capable*/
...
typedef enum
{
  ...
  GEOFENCE_UNSOL_EVENT_MGP_UNLOADED /* MGP is unloaded so GNSS tracking is not possible */
...
typedef enum
{
  ...
  GEOFENCE_POS_FIX_SRC_GTP_1_0 = 0x40, /*GTP 1.0 pos fix*/
  GEOFENCE_POS_FIX_SRC_GTP_2_0 = 0x80, /*GTP 2.0 pos fix*/
...
typedef enum
{
  ...
  GEOFENCE_POS_FIX_FAIL_MGP_UNAVAIL, /*MGP is not available / cannot be loaded*/
...
#define SM_GM_ENGINE_CONFIG_ACCEPT_CELL_CPI_FIXES_MASK                (0x100000)
/* Enable GTP2.0 position source in GM*/
#define SM_GM_ENGINE_CONFIG_ENABLE_GTP_2_0_MASK                       (0x200000)
...
typedef struct
{
  ...
  pdsm_lite_pd_session_operation_e_type pos_operation_mode; /*Position operation mode*/
...
typedef struct
{
  ...
  sm_FixInfoStructType pos_fix_rpt;/*Position fix assosiated with the breach*/
...
typedef struct
{
  ...
  sm_FixInfoStructType z_pos_fix_rpt;/*Position fix assosiated with the breach*/
...
typedef struct
{
  ...
  pdsm_lite_pd_session_operation_e_type q_gm_pos_operation_mode;
  ...
  boolean v_accept_cell_cpi_fixes;

  /* Enable GTP2.0 */
  boolean q_enable_gtp_2_0; 
...
typedef struct
{
  uint32 reserved;
  sm_gm_pos_fix_src pos_fix_src;
  sm_gm_pos_fix_fail_reason_e e_fail_reason;
...
typedef struct
{
  ...
  tm_sess_stop_reason_e_type e_abort_reason;
...
/*MGP load status used by TM to provide notification to GM*/
typedef struct
{
  boolean loadAvailable; // Flag to indicate if the MGP load is available to be loaded if requested 
}sm_gm_mgp_load_avail_type;
...
Parameters:
  pdsm_lite_gps_lock_e_type: GPS lock state

Return value:
  void
=============================================================================*/
void gm_update_gps_lock_state(pdsm_lite_gps_lock_e_type  gps_lock);
...
Return value:
  void
=============================================================================*/
void gm_position_fix_fail_update(sm_gm_position_fix_fail_type *p_fix_fail);
...
Return value:
  void
=============================================================================*/
void gm_update_pd_event(pdsm_lite_pd_event_type  e_pd_event, tm_sess_stop_reason_e_type e_abort_reason);
...
/*===========================================================================
  FUNCTION gm_update_gnss_engine_state_change

  DESCRIPTION
	This function handles the GNSS Engine State Change Update
  ...
  ===========================================================================*/
void gm_update_gnss_engine_state_change(const sm_RcvrStateChangeStructType *pz_rcvrState);


/*===========================================================================
FUNCTION gm_api_get_backoff_info

DESCRIPTION
  This function returns the following backoff information
* Remaining geofence backoff time in ms
* Distance to the closest geofence in meters

DEPENDENCIES

RETURN VALUE
  TRUE if geofences are active in system and info is populated.
  FALSE if no geofences or input validation failed.

SIDE EFFECTS

===========================================================================*/
boolean gm_api_get_backoff_info(uint32 *pq_backoffTimeMs, uint32 *pq_distToClosestGf);


/*===========================================================================
FUNCTION gm_mgp_update_load_availability

DESCRIPTION
  This function is used to update the availability of MGP load from TM to GM
...
===========================================================================*/
void gm_mgp_update_load_availability(sm_gm_mgp_load_avail_type *p_status);
