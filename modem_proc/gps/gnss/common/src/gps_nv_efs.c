...
/*==============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/common/src/gps_nv_efs.c#1 $
...
)
{
  ...
  if (newline)
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS_REG: could not add new line to file name file_size %d",
              file_name_size, 0, 0);
      file_name_size = 0;
    }
  }
...
)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: Registry Initialization", 0, 0, 0);

  /* Validate Parameters */
  if (cgps_nv_efs_reg_items == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_REG: Null table pointer.", 0, 0, 0);
    return FALSE;
  } /* if null table*/

  if (psz_pathname_prefix == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_REG: Null pathname", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_REG: invalid range: %d, %d", table_start, table_end, 0);
    return FALSE;
  }
  ...
  switch (subsys)
  {
    ...
    case CGPS_NV_EFS_LOC:
      ...
      break;
    default:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: No config file available for this type %d", subsys, 0, 0);
      return FALSE;
  }

  if( conf_file_name == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Config file name is still NULL", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Invalid File system specified %d", fs, 0, 0);
    return FALSE;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Invalid Subscription ID specified %d", subid, 0, 0);
    return FALSE;
  }
  ...
  if (mkdir_result != 0)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv returned %d", efs_errno, 0, 0);
  }
  mkdir_result = efs_mkdir(conf_dir_name2, CGPS_NV_EFS_PERMS);
  if (mkdir_result != 0)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv/item_files returned %d", efs_errno, 0, 0);
  }
  mkdir_result = efs_mkdir(conf_dir_name3, CGPS_NV_EFS_PERMS);
  if (mkdir_result != 0)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv/item_files/conf returned %d", efs_errno, 0, 0);
  }
       ...
       temp_stat.st_size == 0 )
  {
    ...
    if (file_handle == -1)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d creating config file", efs_errno, 0, 0);
      return FALSE;
    }
    ...
    while ((table_idx >= 0) && (error_detected == FALSE))
    {
      ...
      while ((bytes2write > 0) && (error_detected == FALSE))
      {
        ...
        {
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS:Incorrect File Name length. Limit is :%d, Current size:%d",
        ...
        if (bytes_written < 0)
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d writing to config file", efs_errno, 0, 0);
          error_detected = TRUE;
        }
        else
        {
    ...
    {
      /* Error while closing */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d closing config file", efs_errno, 0, 0);
      error_detected = TRUE;
    } /* mcfg_fclose check */
#else
  /* create conf directory if it doesn't exist */
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv returned %d", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
    return FALSE;
  }
  ...
  if (mkdir_result != 0 &&
      mcfg_fs_errno((mcfg_fs_e_type)fs) != EEXIST)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv/item_files returned %d", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
    return FALSE;
  }
  ...
  if (mkdir_result != 0 &&
      mcfg_fs_errno((mcfg_fs_e_type)fs) != EEXIST)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS: mcfg_mkdir /nv/item_files/conf returned %d", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
    return FALSE;
  }
       ...
       temp_stat.st_size == 0 )
  {
    ...
    if (file_handle == -1)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d creating config file", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
      return FALSE;
    }
    ...
    while ((table_idx >= 0) && (error_detected == FALSE))
    {
      ...
      while ((bytes2write > 0) && (error_detected == FALSE))
      {
        ...
        if (bytes_written < 0)
        {
          MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d writing to config file", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
          error_detected = TRUE;
        }
        else
        {
    ...
    {
      /* Error while closing */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS: Error %d closing config file", mcfg_fs_errno((mcfg_fs_e_type)fs), 0, 0);
      error_detected = TRUE;
    } /* mcfg_fclose check */
#endif /* FEATURE_CGPS_LEGACY_NV_API */
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_REG: invalid type: %d", type, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: Invalid NV Item specified %d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  if ((entry_ptr = gps_nv_efs_find_subsys_entry(item)) == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: subsystem not initialized for item:%d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  if( data_ptr == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: data pointer is null", 0, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if( fs <= CGPS_NV_FS_TYPE_MIN ||
      fs >= CGPS_NV_FS_TYPE_MAX)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: Invalid File system specified %d", fs, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if( subid <= CGPS_NV_SUBID_MIN ||
      subid > CGPS_NV_SUBID_MAX)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: Invalid Subscription ID specified %d", subid, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if (status == -1)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: EFS read error %d, %d", status, efs_errno, 0);
...
)
{
  ...
  if( item < CGPS_NV_EFS_MIN_NUM_ITEMS ||
      item >= CGPS_NV_EFS_MAX_NUM_ITEMS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Invalid NV Item specified %d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  if ((entry_ptr = gps_nv_efs_find_subsys_entry(item)) == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: subsystem not initialized for item:%d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }

  if( data_ptr == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: data pointer is null", 0, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if( fs <= CGPS_NV_FS_TYPE_MIN ||
      fs >= CGPS_NV_FS_TYPE_MAX)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Invalid File system specified %d", fs, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if( subid <= CGPS_NV_SUBID_MIN ||
      subid > CGPS_NV_SUBID_MAX)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Invalid Subscription ID specified %d", subid, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if (data_size != item_reg_ptr->size)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Destination buffer size incorrect", 0, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if (status != ENOERR)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE1: EFS write error %d, %d", status, efs_errno, 0);
  ...
  if (status != MCFG_FS_STATUS_OK)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE2: EFS write error %d, %d", status, mcfg_fs_errno((mcfg_fs_e_type)fs), 0);
    return CGPS_NV_EFS_REG_RW_STAT_WR_ERR;
  }
  #endif /* FEATURE_CGPS_LEGACY_NV_API */
                         ...
                         const cgps_nv_efs_reg_item_struct* cgps_nv_efs_reg_items)
{
  ...
  if( type <= CGPS_NV_EFS_SUBSYS_MIN ||
      type >= CGPS_NV_EFS_SUBSYS_MAX )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_INIT: Invalid type: %d", type, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_INIT: Invalid table indexes start:%d, end:%d",
  ...
  if( cgps_nv_efs_reg_items == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: NULL Registry pointer", 0, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
                                ...
                                )
{
  ...
  if( subid <= CGPS_NV_SUBID_MIN ||
      subid >  CGPS_NV_SUBID_MAX )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: Invalid Sub ID %d", subid, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
                                                           ...
                                                           )
{
  ...
  if( cgps_nv_efs_reg_items == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: NULL Registry pointer for item:%d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
                                                                  ...
                                                                  )
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_READ: Invalid item index, start:%d, end:%d, item:%d",
...
cgps_nv_efs_reg_rw_stat_enum_type cgps_nv_efs_reg_item_write(cgps_nv_efs_reg_item_type const item,
                                                             ...
                                                             const cgps_nv_efs_reg_item_struct  *cgps_nv_efs_reg_items)
{
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS_REG: Registry Item Write Started for item:%d", item, 0, 0);
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Invalid item index, start:%d, end:%d, item:%d",
              (int) table_start, (int) table_end, (int) item);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
  ...
  if( cgps_nv_efs_reg_items == NULL )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: NULL Registry pointer for item:%d", item, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
...
{
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_NV_EFS_WRITE: Registry Item Write Started for item:%d", item, 0, 0);
  ...
  if( subid <= CGPS_NV_SUBID_MIN ||
      subid > CGPS_NV_SUBID_MAX )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_NV_EFS_WRITE: Invalid Sub ID %d", subid, 0, 0);
    return CGPS_NV_EFS_REG_RW_STAT_INVALID;
  }
...
boolean gps_nv_efs_init (cgps_nv_efs_subsys_type subsys,
                         ...
                         const char* psz_pathname_prefix)
{
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPS_NV_EFS: Registry Init Started for subsystem:%d", subsys, 0, 0);
...
{
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPS_NV_EFS: Registry Item Read Started for item:%d", item, 0, 0);
  return (cgps_nv_efs_reg_item_write_internal(item, data_size, data_ptr));
}
