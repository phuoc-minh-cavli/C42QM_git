...
/*==============================================================================
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/common/src/gps_nv_loader.c#1 $
...
static const int32 gps_basic_nv_items_type_int[][NV_MAX_SIZE_TYPE_INT] =
{
  ...
  /*!!!!!IF THERE IS A CHANGE TO 1937 THEN MUST CHANGE IN LocMw (SESSION_END_THRESHOLD) ALSO TO SAME VALUE!!!!!*/
  ...
  { 4914,          10 }, /* NV_CGPS_ELEVATION_MASK_I                      */
#else
...
#ifdef FEATURE_GNSS_GEN9_AUTOMOTIVE_ENABLED
  { 5594,          1 }, /* NV_CGPS_SBAS_ENABLED_I                        */
  { 5595,       0x0F }, /* NV_CGPS_SBAS_USER_PREFERENCE_I                */
  { 5596,          0 }, /* NV_CGPS_DPO_CONTROL_I                         */
#else
  ...
  { 6486, GPS_UTC_OFFSET }, /* NV_CGPS_UTC_GPS_TIME_OFFSET_I                 */
...
#ifdef FEATURE_GNSS_GEN9_AUTOMOTIVE_ENABLED
  { 6767,          1 }, /* NV_GNSS_TIMED_OUTPUT_MODE_SELECTION_I         */
#else
  ...
  { 6780,          0 }, /* NV_GNSS_MAX_NUM_GPS_SV_MEASUREMENTS_IN_FIX_I  */
  { 6781,          5 }, /* NV_GNSS_SPCFC_GPS_SV_MEASURES_NOT_IN_FIX_I    */
  { 6782,         30 }, /* NV_GNSS_MAX_NUM_GLO_SV_MEASUREMENTS_IN_FIX_I  */
  { 6783,        600 }, /* NV_GNSS_SPCFC_GLO_SV_MEASURES_NOT_IN_FIX_I    */
...
static const int32 gps_basic_nv_items_type_ipc_dm[][NV_MAX_SIZE_TYPE_IPC_DM] =
{
  { 4544, 0x00000000,
          ...
          0xFFFFFFFF, /* IPCs to LOC_LTE_OTDOA_CONTROL task */
          0xFFFFFFFF, /* IPCs to LOC_LTE_OTDOA_MEASPROC task */
...
)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gps_NvWriteDoneCb :NV Write Failed on item %d Reason %d", e_NvItem, e_Status, 0);
  }
...
)
{
  ...
  if (p_NvItemData != NULL)
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"WRITING  GPS NV Item: %d [%d/%d]", p_NvItemData->item, z_gps_NvDataDb.q_NvWrites, z_gps_NvDataDb.q_NvAdded);
  }
  else
  {
    /* No GPS NV Items exist in the Queue, which means all NV items written successfully */
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"COMPLETED WRITING [%d/%d] GPS NV ITEMS TO DEFAULT VALUES at time=%d", z_gps_NvDataDb.q_NvWritesDone, z_gps_NvDataDb.q_NvWrites, timetick_get());
  }
} /* gps_NvWriteItem */
...
)
{
  ...
  if (b_gps_NvInitialized)
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"QUEUEING GPS NV ITEM: %d", e_NvItem, 0, 0);
...
void gps_NvWriteDone (void)
{
  ...
  if (NULL != nv_ptr)
  {
    ...
    if (z_gps_NvCmd.item == nv_ptr->item)
    {
      ...
      {
        /* NV write failed. */
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gps_NvWriteDone: FAILED - Error Status %d", z_gps_NvCmd.status, 0, 0);
      }

      /* Out-of-bound array index sanity check */
      else if (nv_ptr->handle >= GPS_NV_CLIENT_HANDLE_MAX)
      {
        MSG_1(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gps_NvWriteDone: FAILED - bad parameter %d", 
          nv_ptr->handle);
      }
      else
      {
...
void gps_NvInitDefaultWrites (void)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"STARTED QUEUEING GPS NV ITEMS at time=%d", timetick_get(), 0, 0);
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"COMPLETED QUEUEING %d GPS NV ITEMS at time=%d", z_gps_NvDataDb.q_NvAdded, timetick_get(), 0);

  /* Starting Writing NV's one by one from Queue */
  if (b_gps_NvInitialized)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"STARTED WRITING %d GPS NV ITEMS TO DEFAULT VALUES at time=%d", z_gps_NvDataDb.q_NvAdded, timetick_get(), 0);
    gps_nv_item_s_type* p_NvItemData = NULL;
    p_NvItemData = (gps_nv_item_s_type*)q_check(&z_gps_NvWriteCmdQueue);
    gps_NvWriteItem(p_NvItemData);
  }
...
)
{
  ...
  if ((NV_DONE_S != n_enum_state) && (NV_NOMEM_S != n_enum_state))
  {
        ...
        (GPS_NV_ITEMS_TYPE_IPC_DM == e_NvDataType) )
    {
      ...
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"FAILED WRITE FOR GPS NV ITEM: %d, Continue...", e_NvItem, 0, 0);
      }
    }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"FAILED for GPS NV item: %d, ERROR CODE :%d", e_NvItem, n_enum_state, 0);
  }
