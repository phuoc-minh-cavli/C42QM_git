...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/common/inc/location_common.h#1 $
...
/* Pedestrain speed model for uncertainty propagation in meters/second */
#define LOC_PROP_PDR_SPEED_MPS      ((FLT)(1.5))
...
/* Car speed model for uncertainty propagation in meters/second */
#define LOC_PROP_CAR_SPEED_MPS             ((FLT)(25.0))

/* Car acceleration model for uncertainty propagation in meters/second^2 */
#define LOC_PROP_CAR_ACCEL_MPSS            ((FLT)(1.0))

/* Minimum time difference in seconds that results in usage of the airplane 
  uncertainty propagation model */
#define LOC_TIME_DIFF_AIR_SPEED_INIT       ((FLT)(600.0))

/* Air acceleration model for uncertainty propagation in meters/second^2 */
#define LOC_PROP_AIR_ACCEL_MPSS            ((FLT)(2.0))
...
/*=============================================================================
  ...
  PARAMETERS    f_PropagatedErrEllipseMeters[]: Propagated Punc as a 2D error 
                ...
                f_CarSpeedUncMps - Car speed uncertainty value in m/sec
...
void loc_PropagatePuncVelUnc( FLT f_PropagatedErrEllipseMeters[],
                              ...
                              FLT f_CarSpeedUncMps);

/*=============================================================================

  FUNCTION      loc_PropagatePedPunc()

  DESCRIPTION   Propagate the initial position uncertainties of the navigation 
                solution to a specific time using a Pedestrian Punc propagation 
                model. 
...
=============================================================================*/
void loc_PropagatePedPunc(FLT f_PropagatedErrEllipseMeters[],
                          FLT *pf_PropagatedPunc3dMeters,
                          FLT f_PropagatedPuncLatLonMeters[],
                          FLT f_TimeDiffSec,
                          const loc_InitUncDataStructType *pz_InitUnc );
 ...
 ******************************************************************************
*/
double loc_CalcDistBetLatLongHaversine(
 ...
 *
 * DESCRIPTION:
 *  Utility function to get default Car speed uncertainty value in m/sec.
 *  Replaces use of #define for all function calls.
 * 
 * PARAMETERS: 
 *  None
 *
 * RETURN VALUE:
 *  Default Car speed uncertainty value in m/sec
 *
 ******************************************************************************
*/
float loc_getDefaultCarSpeedUncMps();
