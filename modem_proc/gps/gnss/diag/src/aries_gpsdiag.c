...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss/diag/src/aries_gpsdiag.c#1 $
...
--------   ---     ---------------------------------------------------------- 
07/21/17   sum     Use rex_self() in call to rex_kill_task()
10/30/17   nath    Updated data types of alongAxisUnc and perpAxisUnc to float
03/30/17   sum     Updated x_VelHorizU from velHor to velHorU.  
...
#include "event.h"
#if 0 /*TODO: [TBD][APP] will put it back if gpsdiag_positioneventcb*/
#include "tm_common.h"
#endif 
...
static const char *c_sm_query_file_description = "$Id: //components/rel/gnss8.mpss/9.5.1.1/gnss/diag/src/aries_gpsdiag.c#1 $";
...
static boolean gpsdiag_TaskIsRunning = FALSE;
...
#if 0 //#Todo:9x05[DEL][APP] pdsm_pd_data_type has been removed
...
static PACKED void* gpsdiag_GeraTestCmd(gpsdiag_GeraTestCmdReqType* p_Req, uint16 w_PktLength);
...
typedef struct
{
  ...
  sm_FixInfoStructType     z_pos_rpt;
  pdsm_lite_pd_event_type  pd_event_type;
...
{
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG task initializing...", 0, 0, 0);

   /* Enable the IPC module for GPSDIAG task */
   if(!os_IpcStart((uint32)THREAD_ID_GPSDIAG))
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG: IPC Start Failed", 0, 0, 0);
   }
...
void gpsdiag_TaskCleanup(void)
{
   ...
   for(q_ClientType = 0; q_ClientType < (uint32)PDSM_CLIENT_TYPE_MAX; q_ClientType++)
   {
      ...
      if(NULL != gpsdiag_ClientTable[q_ClientType].p_AppTrackingTimer)
      {
         ...
         {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Invalid timer in cleanup", 0, 0, 0);
         }
   ...
   /* Pass rex_self() instead of gpsdiag_tcb to avoid any issues with kill() failure
      due to possible call to create() with the same context while kill() is ongoing */
   rex_kill_task(rex_self()); /* commit suicide */
} /* gpsdiag_TaskCleanup */
...
boolean gpsdiag_ProcessMsg (const os_IpcMsgType* p_Msg)
{
   ...
   else
   {
      ...
      case C_OS_MSG_ID_TIMER_EXPIRY:
         ...
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"C_OS_MSG_ID_TIMER_EXPIRY IpcMsg received (Id %d, Param #%d).", q_TimerId, q_Param, 0);
      ...
      case C_GPSDIAG_TEST_PDAPI:
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"C_GPSDIAG_TEST_PDAPI IpcMsg received.", 0, 0, 0);
         /* Call dispatch function here */
         break;

      case C_GPSDIAG_TEST_APP_TRACKING:
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"C_GPSDIAG_TEST_APP_TRACKING IpcMsg received.", 0, 0, 0);
        /* Call dispatch function here */
      ...
      case C_GPSDIAG_TEST_LCS:
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"C_GPSDIAG_TEST_LCS IpcMsg received.", 0, 0, 0);
         /* Call dispatch function here */
         break;
      case C_GPSDIAG_END_TASK:
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"C_GPSDIAG_END_TASK IpcMsg received ", 0, 0 ,0);
         u_retVal = FALSE;
         break;
...

{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IpcMsg %d NOT sent! GPSDIAG task not running!", q_MsgId, 0, 0);
  ...
  if (p_Msg)
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG IpcMsg %d sent.", p_Msg->q_MsgId, 0, 0);
      return GPSDIAG_SUCCESS;
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IpcMsg %d NOT sent!!", p_Msg->q_MsgId, 0, 0);
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IpcMsg is NULL!", 0, 0, 0);
  }
  return GPSDIAG_ERROR;
}
...
void gpsdiag_task (dword ignored)
{
   ...
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG task running... %d", gpsdiag_task_count, 0, 0);
   ...
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IPC Msg %d Not Sent.", C_GPSDIAG_TEST_PDAPI, 0, 0);
   }
#endif /* INCLUDE_PDAPI_TEST */
   ...
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IPC Msg %d Not Sent.", C_GPSDIAG_TEST_APP_TRACKING, 0, 0);
   }
   ...
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IPC Msg %d Not Sent.", C_GPSDIAG_TEST_LCS, 0, 0);
   }
#endif /* INCLUDE_LCS_CMDS */
   ...
   while(u_processResult)
   {
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IPC Msg %d Not Deleted.", p_Msg->q_MsgId, 0, 0);
      }
   }
...
void gpsdiag_init (void)
{
   ...
   if(gpsdiag_registered == 0)
   {
    ...
    {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG Task Mutex Initialization Success.", 0, 0, 0);
    }
    else
    {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG Task Mutex Initialization Failed.", 0, 0, 0);
    }
    ...
    {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM TD Task Mutex Initialization Success.", 0, 0, 0);
    }
    else
    {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM TD Task Mutex Initialization Failed.", 0, 0, 0);
    }
#endif /* FEATURE_CGPS_PDCOMM_TEST_DRIVER */
...
{
/*TODO: [TBD][APP] We do not know if this function is needed when we are already reporting 
position report using sm_log_diag_position sm_log_report_position*/
#if 0
/*TODO: [MODIFY][APP] #9x05: Right now the code has been modified to fill the structure using 
  cached position report. But it has to be modified to reverse translate from QMI to PDSM 
  structure.
*/
   pdsm_client_type_e_type e_ClientType;
   sm_FixInfoStructType *lr_data = &z_cached_pos_info.z_pos_rpt;
   int8 b_LeapSeconds = 0;
   /* Retrieve Client Type (stored in p_DataBlock) */
   e_ClientType = *((pdsm_client_type_e_type*)p_DataBlock);
   DBL heading_angle;
   DBL vel_east = lr_data->z_NavPos.f_VelEnu[0];
   DBL vel_north = lr_data->z_NavPos.f_VelEnu[1];
   
   DBL f_tmp = sqrt(vel_east * vel_east + vel_north * vel_north);
   
   FLT ext_loc_uncrtnty_a  = lr_data->z_NavPos.f_ErrorEllipse[1];
   FLT ext_loc_uncrtnty_p  = lr_data->z_NavPos.f_ErrorEllipse[2];
   me_Clock z_Clock;
   /* Client Type Sanity Check */
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PositionEventCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }

   if(p_PdInfo == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PositionEventCb: No Valid Information", 0, 0, 0);
      return;
   }
  ...
  qmiLocEventPositionReportIndMsgT_v02 pz_PositionReportIndMsg = p_PdInfo->pd_info.pz_PositionReportIndMsg;
  gpsdiag_pPdData.lat  = (int32)tm_util_nearest_long(lr_data->z_NavPos.d_PosLla[0] * RAD2DEG *
                                                      LAT_LONG_SCALE);
  gpsdiag_pPdData.lon = (int32)tm_util_nearest_long(lr_data->z_NavPos.d_PosLla[0] * RAD2DEG *
                                                      LAT_LONG_SCALE);  gpsdiag_pPdData.time_stamp ; //dword
  gpsdiag_pPdData.loc_uncertainty_ang (uint8)(lr_data->z_NavPos.f_ErrorEllipse[0] / 5.625F); //byte
  gpsdiag_pPdData.loc_uncertainty_a = (uint8)tm_util_select_pos_unc(lr_data->z_NavPos.f_ErrorEllipse[1]); //byte
  gpsdiag_pPdData.loc_uncertainty_p = (uint8)tm_util_select_pos_unc(lr_data->z_NavPos.f_ErrorEllipse[2]); //byte
  gpsdiag_pPdData.opt_field_mask = PDSM_PD_UNCERTAINTY_VALID | PDSM_PD_MSEC_VALID | PDSM_PD_EXT_STATUS_VALID | PDSM_PD_ALTITUDE_VALID;
    
    if(lr_data->z_NavPos.z_PosFlags.b_IsVelValid)
    {
       gpsdiag_pPdData.opt_field_mask |= PDSM_PD_VELOCITY_VALID;
    }
    if (0 == tm_core_info.config_info.z_LeapSecondInfo.u_DeltaLeapSecUnc)
    {
       gpsdiag_pPdData.opt_field_mask |= PDSM_PD_UTCOFFSET_VALID;
    }
    if( lr_data->z_NavPos.z_PosFlags.b_IsGpsUsed == FALSE ||lr_data->z_NavPos.z_PosFlags.b_IsGloUsed == TRUE)
    {
       gpsdiag_pPdData.opt_field_mask &= ~PDSM_PD_MSEC_VALID;
    } ; //pdsm_pd_opt_field
  gpsdiag_pPdData.altitude = (sint15)tm_util_nearest_long(lr_data->z_NavPos.d_PosLla[2])- 500;;
  tm_util_calculate_heading(lr_data, &heading_angle);
  
  gpsdiag_pPdData.heading = (uint16)tm_util_nearest_long(heading_angle * HEADING_SCALE); //word
  if (f_tmp < TM_CORE_MIN_VELOCTY)
    {
       gpsdiag_pPdData.velocity_hor = 0;
    }
    else
    {
       gpsdiag_pPdData.velocity_hor = (uint16)tm_util_nearest_long(4.0 * f_tmp);
    }
  if(lr_data->z_NavPos.z_PosFlags.b_Is3D)
  {
    gpsdiag_pPdData.fix_type = TRUE;
    gpsdiag_pPdData.velocity_ver = (int1)tm_util_nearest_long(2.0 * lr_data->z_NavPos.f_VelEnu[2]);
  }

  gpsdiag_pPdData.loc_uncertainty_v = (uint8)tm_util_select_pos_unc(lr_data->z_NavPos.f_PuncVertMeters); //byte
  gpsdiag_pPdData.position_mode = 2; //pdsm_pd_position_mode_e_type
   
  gpsdiag_pPdData.positioning_source = 0;//pdsm_pd_positioning_source_type
  gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_OLDNEW;
  
  if(lr_data->z_NavPos.b_IsRefLoc)
  { 
    gpsdiag_pPdData.positioning_source |=  POSITION_SOURCE_DEFAULT;
  }  
  else 
  {
    ...
    if (lr_data->z_NavPos.z_PosFlags.b_IsGpsUsed)
    {
      gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_GPS;
      if (lr_data->z_NavPos.z_PosFlags.b_IsSft == TRUE)
      {
        gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_SFT;
      }
    }
  
    if (lr_data->z_NavPos.z_PosFlags.b_IsGloUsed)
    {
      gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_GLO;
      if (lr_data->z_NavPos.z_PosFlags.b_IsSft == TRUE)
      {
        gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_SFT;
      }
   ...
   if (lr_data->z_NavPos.z_PosFlags.b_IsKf == TRUE)
   {
     gpsdiag_pPdData.positioning_source |= POSITION_SOURCE_KF_WLS;
   }

   gpsdiag_pPdData.loc_uncertainty_conf = LOC_UNC_CONFIDENCE; //byte
   gpsdiag_pPdData.gpsUtcOffset = tm_core_info.config_info.z_LeapSecondInfo.u_DeltaLeapSec;
   
/* Fill in pdsm_pd_ext_pos_data_s_type values */
  gpsdiag_pPdData.ext_pos_data.lat = (int32)lr_data->z_NavPos.d_PosLla[0]* C_2_32_OVER_PI;
  gpsdiag_pPdData.ext_pos_data.lon = (int32)lr_data->z_NavPos.d_PosLla[1]* C_2_32_OVER_PI;
  gpsdiag_pPdData.ext_pos_data.f_alongAxisUnc = (uint16)ext_loc_uncrtnty_a;
  gpsdiag_pPdData.ext_pos_data.f_perpAxisUnc = (uint16)ext_loc_uncrtnty_p;
  float velHorU = sqrt(SQR(lr_data->z_NavPos.f_VelEnu[0]) + SQR(lr_data->z_NavPos.f_VelEnu[1]));
  gpsdiag_pPdData.ext_pos_data.velHorU =  (velHorU < (float)(TM_CORE_MIN_VELOCTY))? 0 : (uint16)velHorU* 100 ;
  gpsdiag_pPdData.ext_pos_data.velHor = (velHorU < (float)(TM_CORE_MIN_VELOCTY))? 0 : (uint16)velHorU* 100 ;
  gpsdiag_pPdData.ext_pos_data.velVert = (int32) lr_data->z_NavPos.f_VelEnu[2] * 100;;
  gpsdiag_pPdData.ext_pos_data.timestamp_tow_ms = (uint32)z_Clock.z_Time.q_GpsMsec; //dword
  gpsdiag_pPdData.ext_pos_data.timestamp_gps_week = (uint16)z_Clock.z_Time.w_GpsWeek;
  gpsdiag_pPdData.ext_pos_data.pos_reported_to_network = (uint8)FALSE; //boolean
  gpsdiag_pPdData.ext_pos_data.velUncHoriz;
  gpsdiag_pPdData.ext_pos_data.velUncVert;
   if (ext_vel_horiz_unc * 100 > (float)CGPS_MAX_UINT16)
   {
     gpsdiag_pPdData.ext_pos_data.velUncHoriz = (uint32)CGPS_MAX_UINT16;
   }
   else
   {
     gpsdiag_pPdData.ext_pos_data.velUncHoriz = (uint32)(ext_vel_horiz_unc * 100);
   }
   
   //p_LogPkt->w_VelUncHoriz = (uint32)p_ExtPosData.velUncHoriz;
   if (ext_vel_vert_unc * 100 > (float)CGPS_MAX_UINT16)
   {
     gpsdiag_pPdData.ext_pos_data.velUncVert = (uint32) CGPS_MAX_UINT16;
   }
   else
   {
     gpsdiag_pPdData.ext_pos_data.velUncVert = (uint32)(ext_vel_vert_unc * 100);
   }
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PositionEventCb: PDSM_PD_EVENT_POSITION", 0, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"[Location Info] Latitude:  %d", gpsdiag_pPdData.lat, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"                Longitude: %d", gpsdiag_pPdData.lon, 0, 0);
   }

   if(u_PdEvent & PDSM_PD_EVENT_VELOCITY)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PositionEventCb: PDSM_PD_EVENT_VELOCITY", 0, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"[Velocity Info] Horizonal: %d", gpsdiag_pPdData.velocity_hor, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"                Vertical:  %d", gpsdiag_pPdData.velocity_ver, 0, 0);
   }

   if(u_PdEvent & PDSM_PD_EVENT_HEIGHT)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PositionEventCb: PDSM_PD_EVENT_HEIGHT", 0, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"[Altitude Info] Altitude:  %d", gpsdiag_pPdData.altitude, 0, 0);
   }
                          ...
                          const pdsm_pd_info_s_type* p_PdInfo)
{
  ...
  os_AppTrackType z_AppTrack;
   ...
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_DoneEventCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }

   if(u_PdEvent & PDSM_PD_EVENT_DONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_DoneEventCb: PDSM_PD_EVENT_DONE", 0, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"GPSDIAG Application Tracking Complete", 0, 0, 0);
      }
     ...
     */
     if ((gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.u_AppTrackingTimerExpired) &&
        ...
        (gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.e_FixType == GPSDIAG_FIX_TYPE_APP_TRACKING))
     {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_DoneEventCb: Starting Application Tracking Again...", 0, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_DoneEventCb: PDSM_PD_EVENT_GPS_DONE", 0, 0, 0);
   }
} /* gpsdiag_DoneEventCb */
                         ...
                         const pdsm_pd_info_s_type* p_PdInfo)
{
   ...
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndEventCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }

   if(u_PdEvent & (pdsm_pd_event_type)PDSM_PD_EVENT_END)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_EndEventCb: PDSM_PD_EVENT_END", 0, 0, 0);

      /* Report an unsuccessful PDSM session back to QXDM */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG Position Fix Unsuccessful (%d/%d)",
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndEventCb: Unknown event %d", u_PdEvent, 0, 0);

      /* Report an unsuccessful PDSM session back to QXDM */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG Location Fix Unsuccessful", 0, 0, 0);
   }
                           ...
                           const pdsm_pd_info_s_type* p_PdInfo)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartEventCb: PDSM_PD_EVENT_BEGIN", 0, 0, 0);
   }
    else if(u_PdEvent & PDSM_PD_EVENT_GPS_BEGIN)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartEventCb: PDSM_PD_EVENT_GPS_BEGIN", 0, 0, 0);
   }
                             ...
                             const pdsm_pd_info_s_type* p_PdInfo)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_FailureEventCb: PDSM_PD_EVENT_UPDATE_FAILURE", 0, 0, 0);
   }
                                 ...
                                 const pdsm_pd_info_s_type* p_PdInfo)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_PdEventsDispatchCb: No Valid Information. %p, %p", p_PdInfo, p_DataBlock, 0);
      return;
   }
   ...
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PdEventsDispatchCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }
   ...
   if((p_PdInfo->client_id != gpsdiag_ClientTable[e_ClientType].l_ClientId))
   {
      ...
      {
//      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Duplicate event for Client ID %d", e_ClientType, 0, 0);
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PdEventsDispatchCb: Unknown Client ID %d", e_ClientType, 0, 0);
      }
      return;
   }
   ...
   else if(u_PdEvent & (PDSM_PD_EVENT_DONE |PDSM_PD_EVENT_GPS_DONE))
   {
      gpsdiag_DoneEventCb(p_DataBlock, u_PdEvent, p_PdInfo);
   }
   else if(u_PdEvent & (pdsm_pd_event_type)(PDSM_PD_EVENT_END ))
   {
      gpsdiag_EndEventCb(p_DataBlock, u_PdEvent, p_PdInfo);
   }
   else if(u_PdEvent & (PDSM_PD_EVENT_BEGIN | PDSM_PD_EVENT_GPS_BEGIN))
   {
      gpsdiag_StartEventCb(p_DataBlock, u_PdEvent, p_PdInfo);
   }
   else if(u_PdEvent & (PDSM_PD_EVENT_UPDATE_FAILURE ))
   {
                                 ...
                                 const pdsm_pa_info_s_type* p_PaInfo)
{
   ...
   if(p_PaInfo == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PaEventsDispatchCb: No Valid Information", 0, 0, 0);
      return;
   }
   ...
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PaEventsDispatchCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }
...
//      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Duplicate event for Client ID %d", e_ClientType, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_PaEventsDispatchCb: Unknown Client ID %d", e_ClientType, 0, 0);
      }
      return;
   }

   if(u_PaEvent & PDSM_PA_EVENT_GPS_LOCK)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PaEventsDispatchCb: PDSM_PA_EVENT_GPS_LOCK received", 0, 0, 0);
   }
  
   else if(u_PaEvent & PDSM_PA_EVENT_DELETE_PARAMS)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PaEventsDispatchCb: PDSM_PA_EVENT_DELETE_PARAMS received", 0, 0, 0);
   }
     else if(u_PaEvent & PDSM_PA_EVENT_DPO_CONTROL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_PaEventsDispatchCb: PDSM_PA_EVENT_DPO_CONTROL received", 0, 0, 0);
   }
} /* gpsdiag_PaEventsDispatchCb */

   #if INCLUDE_LCS_CMDS
#if 0 /*#9x05: Ideally The above flag should have been disable as LCS is not supported */
/*===========================================================================
                                  ...
                                  const pdsm_lcs_info_s_type* p_LcsInfo)
{
   ...
   if(p_LcsInfo == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsEventsDispatchCb: No Valid Information", 0, 0, 0);
      return;
   }
   ...
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsEventsDispatchCb: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }
...
//      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Duplicate event for Client ID %d", e_ClientType, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsEventsDispatchCb: Unknown Client ID %d", e_ClientType, 0, 0);
      }
      return;
   }
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_EVENT_VX_NOTIFY_VERIFY_REQ received", 0, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"notify_priv_type = %d", p_LcsInfo->lcs_info.vx_req.notification_priv_type, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"pos_mode = %d", p_LcsInfo->lcs_info.vx_req.pos_mode, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[0] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[0], 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[1] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[1], 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[2] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[2], 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[3] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[3], 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[4] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[4], 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Requestor_id[5] = %c", p_LcsInfo->lcs_info.vx_req.requester_id.requester_id[5], 0, 0);
   }
   else if(u_LcsEvent & PDSM_LCS_EVENT_SUPL_NOTIFY_VERIFY_REQ)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_EVENT_SUPL_NOTIFY_VERIFY_REQ received", 0, 0, 0);

      /* Save SUPL data from server (passed in LCS Info struct) */
      if(p_LcsInfo->lcs_cb_cmd_type == PDSM_LCS_CMD_TYPE_SUPL_REQ)
      {
#if 0    //TODO: [DEL][APP]#9x05: gpsdiag_zSuplData deleted
         gpsdiag_zSuplData = p_LcsInfo->lcs_info.supl_req;
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: Received SUPL data", 0, 0, 0);
#endif 
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsEventsDispatchCb: Unexpected SUPL data", 0, 0, 0);
      }
   }
   else if(u_LcsEvent & PDSM_LCS_EVENT_UMTS_CP_NOTIFY_VERIFY_REQ)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_EVENT_UMTS_CP_NOTIFY_VERIFY_REQ received", 0, 0, 0);
      ...
      {
#if 0 //TODO:[DEL][APP] Remove gpsdiag_zUmtsCpData     
         gpsdiag_zUmtsCpData = p_LcsInfo->lcs_info.umts_cp_req;
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: Received UMTS CP data", 0, 0, 0);
#endif
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsEventsDispatchCb: Unexpected UMTS CP data", 0, 0, 0);
      }
   }
   else if(u_LcsEvent & PDSM_LCS_VX_EVENT_USER_NOTIFY_SERVICE_INTERACTION)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_VX_EVENT_USER_NOTIFY_SERVICE_INTERACTION received", 0, 0, 0);
   }
   else if(u_LcsEvent & PDSM_LCS_EVENT_CLIENT_STATUS)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_EVENT_CLIENT_STATUS received", 0, 0, 0);
   }
   else if(u_LcsEvent & PDSM_LCS_EVENT_CLIENT_VX_CONFIG)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsEventsDispatchCb: PDSM_LCS_EVENT_CLIENT_VX_CONFIG received", 0, 0, 0);
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"vx version = %d, trustness setting = %d", p_LcsInfo->lcs_info.vx_config.vxVersion,  p_LcsInfo->lcs_info.vx_config.trustedApp, 0);
   }
} /* gpsdiag_LcsEventsDispatchCb */
#endif
   #endif /* INCLUDE_LCS_CMDS */
   
/*#9x05:gpsdiag_ExtStatEventsDispatchCb removed as pdsm_ext_status_info_s_type is not supported */
                              ...
                              pdsm_pd_cmd_err_e_type u_PdCmdErr)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_EndSessionCmdCb: Err=%d", u_PdCmdErr, 0, 0);
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_EndSessionCmdCb: Success.", 0, 0, 0);
      ...
      if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
         e_ClientType == PDSM_CLIENT_TYPE_NONE)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndSessionCmdCb: Client Type Error %d", e_ClientType, 0, 0);
         return;
      }
                             ...
                             pdsm_pd_cmd_err_e_type u_PdCmdErr)
{
   ...
   if(u_PdCmdErr != PDSM_PD_CMD_ERR_NOERR)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetPosFixCmdCb: Err=%d", u_PdCmdErr, 0, 0);
      ...
      if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
         e_ClientType == PDSM_CLIENT_TYPE_NONE)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetPosFixCmdCb: Client Type Error %d", e_ClientType, 0, 0);
         return;
      }
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetPosFixCmdCb: Success.", 0, 0, 0);
   }
                                   ...
                                   pdsm_pd_cmd_err_e_type u_PdCmdErr)
{
   ...
   if(u_PdCmdErr != PDSM_PD_CMD_ERR_NOERR)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectExternPosCmdCb: Err=%d", u_PdCmdErr, 0, 0);
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_InjectExternPosCmdCb: Success.", 0, 0, 0);
   }
                            ...
                            pdsm_pd_cmd_err_e_type u_PdCmdErr)
{
   ...
   if(u_PdCmdErr != PDSM_PD_CMD_ERR_NOERR)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetPosDlCmdCb: Err=%d", u_PdCmdErr, 0, 0);
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetPosDlCmdCb: Success.", 0, 0, 0);
   }
                            ...
                            pdsm_pa_cmd_err_e_type u_PaCmdErr)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_SetParamCmdCb: Err=%d", u_PaCmdErr, 0, 0);
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_SetParamCmdCb: Success.", 0, 0, 0);
   }
                            ...
                            pdsm_pa_cmd_err_e_type u_PaCmdErr)
{
   ...
   if(u_PaCmdErr != PDSM_PA_CMD_ERR_NOERR)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetParamCmdCb: Err=%d", u_PaCmdErr, 0, 0);
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetParamCmdCb: Success.", 0, 0, 0);
   }
} /* gpsdiag_GetParamCmdCb */

   #if INCLUDE_LCS_CMDS
#if 0 //#9x0r: LCS_CMDS not supported
/*===========================================================================
                       ...
                       pdsm_lcs_cmd_err_e_type p_LcsCmdError)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsCmdCb: LCS Vx Cmd Not Recognized", 0, 0, 0);
      return;
   }
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsCmdCb: Err=%d", p_LcsCmdError, 0, 0);
   }
   else
   {
      if(u_LcsCmd == PDSM_LCS_CMD_NOTIFY_VERIFY_RESP)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsCmdCb: PDSM_LCS_CMD_NOTIFY_VERIFY_RESP", 0, 0, 0);
      }
      else if(u_LcsCmd == PDSM_LCS_CMD_CLIENT_APP_INFO_INIT)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsCmdCb: PDSM_LCS_CMD_CLIENT_APP_INFO_INIT", 0, 0, 0);
      }
      else if(u_LcsCmd == PDSM_LCS_CMD_CLIENT_RELEASE)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsCmdCb: PDSM_LCS_CMD_CLIENT_RELEASE", 0, 0, 0);
      }
      else if(u_LcsCmd == PDSM_LCS_CMD_GET_CLIENT_STATUS)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsCmdCb: PDSM_LCS_CMD_GET_CLIENT_STATUS", 0, 0, 0);
      }
      else if(u_LcsCmd == PDSM_LCS_CMD_QUERY_VX_CONFIG)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsCmdCb: PDSM_LCS_CMD_QUERY_VX_CONFIG", 0, 0, 0);
      }
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"p_LcsCmdError: Success.", 0, 0, 0);
   }
} /* gpsdiag_LcsCmdCb */
#endif 
   #endif /* INCLUDE_LCS_CMDS */
...
PACKED void* gpsdiag_ApiCmdsDispatch (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(p_Req->z_DiagCmdHdr.u_CgpsCmdCode == (uint8)CGPS_INIT_CMD)
   {
     ...
     if(p_Req->l_ClientId == PDSM_CLIENT_TYPE_TEST2_LEGACY_VAL)
     {
       e_ClientType = PDSM_CLIENT_TYPE_TEST2 ;
     }
     else
     {
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_ApiCmdsDispatch: ClientType Error %d Clientid: %d Cmdcode:%u",
                 e_ClientType,
                 p_Req->l_ClientId,
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_ApiCmdsDispatch: Client Type %d", e_ClientType, 0, 0);
   }

   switch(p_Req->z_DiagCmdHdr.u_CgpsCmdCode)
   {
   case CGPS_INIT_CMD:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ApiCmd: CGPS_INIT_CMD", 0, 0, 0);

      gpsdiag_ClientTable[e_ClientType].l_ClientId   = pdsm_lite_client_init((pdsm_client_type_e_type)e_ClientType);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_INIT_CMD: Cannot initialize PDAPI!!", 0, 0, 0);
         e_Status = GPSDIAG_ERROR;
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CGPS_INIT_CMD: ClientType=%d, Client ID %d", e_ClientType, gpsdiag_ClientTable[e_ClientType].l_ClientId, 0);
         e_Status = GPSDIAG_SUCCESS;
      }
      break;

   case CGPS_ACTIVATE_CMD:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ApiCmd: CGPS_ACTIVATE_CMD", 0, 0, 0);

      /* Client ID Sanity Check */
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_ACTIVATE_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }
      e_Result = pdsm_lite_client_act(p_Req->l_ClientId);
      e_Status = (e_Result == PDSM_CLIENT_OK) ? GPSDIAG_SUCCESS : GPSDIAG_ERROR;
      break;

   case CGPS_DEACTIVATE_CMD:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ApiCmd: CGPS_DEACTIVATE_CMD", 0, 0, 0);

      /* Client ID Sanity Check */
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_DEACTIVATE_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }
      e_Result = pdsm_lite_client_deact(p_Req->l_ClientId);
      e_Status = (e_Result == PDSM_CLIENT_OK) ? GPSDIAG_SUCCESS : GPSDIAG_ERROR;
      break;

   case CGPS_RELEASE_CMD:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"ApiCmd: CGPS_RELEASE_CMD", 0, 0, 0);

      /* Client ID Sanity Check */
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_RELEASE_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }
      e_Result = pdsm_lite_client_release(p_Req->l_ClientId);
      e_Status = (e_Result == PDSM_CLIENT_OK) ? GPSDIAG_SUCCESS : GPSDIAG_ERROR;
      break;
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_ApiCmdsDispatch: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
...
PACKED void* gpsdiag_RegCmdsDispatch (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_RegCmdsDispatch: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_RegCmdsDispatch: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   case CGPS_REGISTER_PD_CMD:
      ...
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_REGISTER_PD_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }

      if(p_Req->u_RegisterType == (uint8)GPSDIAG_REG_TYPE_REGISTER)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"RegCmd: CGPS_REGISTER_PD_CMD: Register: %d, %u", e_ClientType, gpsdiag_ClientTable[e_ClientType].l_ClientId, 0);
         e_Result = pdsm_lite_client_pd_reg(p_Req->l_ClientId,
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"RegCmd: CGPS_REGISTER_PD_CMD: Deregister", 0, 0, 0);
         e_Result = pdsm_lite_client_pd_reg(p_Req->l_ClientId,
                                       NULL,
   ...
   case CGPS_REGISTER_PA_CMD:
      ...
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_REGISTER_PA_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }

      if(p_Req->u_RegisterType == (uint8)GPSDIAG_REG_TYPE_REGISTER)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_PA_CMD: Register", 0, 0, 0);
         e_Result = pdsm_lite_client_pa_reg(p_Req->l_ClientId,
                                       (void*)&(gpsdiag_ClientTable[e_ClientType].e_ClientType),
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_PA_CMD: Deregister", 0, 0, 0);
         e_Result = pdsm_lite_client_pa_reg(p_Req->l_ClientId,
                                       NULL,
   ...
   case CGPS_REGISTER_LCS_CMD:
#if 0 /*TODO[DEL][APP]#9x05: LCS not supported*/
      /* Client ID Sanity Check */
      if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_REGISTER_LCS_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
      }

      if(p_Req->u_RegisterType == (uint8)GPSDIAG_REG_TYPE_REGISTER)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_LCS_CMD: Register", 0, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_LCS_CMD: Deregister", 0, 0, 0);
         e_Result = pdsm_client_lcs_reg(p_Req->l_ClientId,
                                        NULL,
   ...
   case CGPS_REGISTER_EXT_STAT_CMD:
      {
#if 0 /*#9x05 EXT_STAT not supported*/
         /* Client ID Sanity Check */
         if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
         {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CGPS_REGISTER_EXT_STAT_CMD: UNKNOWN CLIENT ID!!", 0, 0, 0);
         }

         if(p_Req->u_RegisterType == (uint8)GPSDIAG_REG_TYPE_REGISTER)
         {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_EXT_STAT_CMD: Register", 0, 0, 0);;
         ...
         {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"RegCmd: CGPS_REGISTER_EXT_STAT_CMD: Deregister", 0, 0, 0);
            e_Result = pdsm_client_ext_status_reg(p_Req->l_ClientId,
                                                  NULL,
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_RegCmdsDispatch: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_StartCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_StartCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client ID Sanity Checks */
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId == GPSDIAG_INIT_VALUE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Must initialize PDAPI client first", 0, 0, 0);
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_StartCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
   ...
   if((gpsdiag_eSessionType)p_Req->u_SessionType == GPSDIAG_SESS_TYPE_DATA_DLOAD)
   {
      ...
      if(e_Result)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG Start: Data D/L", 0, 0, 0);
      }
      else
      {
         /* If the function couldn't be called, there was no buffer for the request. */
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG Start: Data D/L Unsuccessful %d!!", e_Result, 0, 0);
      }
   }
   ...
   else
   {
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartCmd: Stopping Application Tracking", 0, 0, 0);
      ...
      else if(gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.e_FixType == GPSDIAG_FIX_TYPE_APP_TRACKING)
      {
          ...
          */
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartCmd: Starting Application Tracking", 0, 0, 0);
      ...
      else /* GPSDIAG_FIX_TYPE_ONESHOT */
      {
         ...
         if(e_Result)
         {
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartCmd: Single Shot Position Fix", 0, 0, 0);
         }
         else
         {
            /* If the function couldn't be called, there was no buffer for the request. */
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG Start: Position Fix Unsuccessful %d!!", e_Result, 0, 0);
         }
      }
   }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_StartCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_EndCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_EndCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
  ...
  if ((gpsdiag_eEndType)p_Req->q_EndType == GPSDIAG_END_DLOAD)
  {
   ...
   else
   {
      ...
      if(gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.e_FixType == GPSDIAG_FIX_TYPE_APP_TRACKING)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_EndCmd: Attempting to stop App Tracking...", 0, 0, 0);
         gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.q_NumFixesLeft = 0;
      }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_EndCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_SetParamCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_SetParamCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_SetParamCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_SetParamCmd: UNKNOWN CLIENT ID [%d]!!", p_Req->l_ClientId, 0, 0);
   ...
   case GPSDIAG_PA_DELETE_PARAMS_EX:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_DELETE_PARAMS1", 0, 0, 0);
   ...
   case GPSDIAG_PA_DELETE_PARAMS:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_DELETE_PARAMS", 0, 0, 0);
      ...
      z_ParamSetVal.pa_set = PDSM_LITE_PA_DELETE_PARAMS_EX1; //pdsm_pa_e_type
      z_ParamSetVal.info.delete_params = z_PaDelParams; //pdsm_delete_parms_type
   ...
   case GPSDIAG_PA_NMEA_CONFIG_INFO:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_NMEA_CONFIG_INFO", 0, 0, 0);
      /* Verify that the parameter length is correct */
   ...
   case GPSDIAG_PA_DPO_CONTROL:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_DPO_CONTROL %d", p_Req->z_ParamVal.u_DpoControl, 0, 0);
   ...
   case GPSDIAG_PA_MOLR_AD_CALLFLOW_SELECT:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_MOLR_AD_CALLFLOW_SELECT", 0, 0, 0);
      /* Verify that the parameter length is correct */
      if(p_Req->w_Length != sizeof(uint8))
      {
         /* Return a diag error packet indicating bad parameters were passed */
   ...
   case GPSDIAG_PA_SET_EXTERNAL_SPI:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_SET_EXTERNAL_SPI", 0, 0, 0);
   ...
   case GPSDIAG_PA_XTRAT_CLIENT_TOKEN:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_XTRAT_CLIENT_TOKEN", 0, 0, 0);
   ...
   case GPSDIAG_PA_XTRAT_USER_SESSION_CONTROL:
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_XTRAT_USER_SESSION_CONTROL setparam:%d  diag:%d",
   ...
   case GPSDIAG_PA_DATA_ENABLE:
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_DATA_ENABLE setparam Mask:%d  diag:%d",
              z_ParamSetVal.info.data_enable , p_Req->z_ParamVal.u_DataEnable, 0);
#endif
      break;
#if 0
   case GPSDIAG_PA_LBS_APN_PROFILE_LIST:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_LBS_APN_PROFILE_LIST", 0, 0, 0);
  ...
  case GPSDIAG_PA_XTRA_APN_PROFILE_LIST:
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_XTRA_APN_PROFILE_LIST", 0, 0, 0);
     if(p_Req->w_Length != sizeof(gpsdiag_apn_profiles_type) * C_GPSDIAG_MAX_APN_PROFILES)
     {
        /* Return a diag error packet indicating bad parameters were passed */
        return(diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, w_PktLength));
   ...
   case GPSDIAG_PA_SUPL_VERSION:
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_SUPL_VERSION setparam Mask:%d  diag:%d",
   ...
   case GPSDIAG_PA_SUPL_SECURITY:
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_SUPL_SECURITY setparam Mask:%d  diag:%d",
   ...
   case GPSDIAG_PA_LPP_ENABLE:
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Param: GPSDIAG_PA_LPP_CONFIG setparam Mask:%d  diag:%d",
   ...
   case GPSDIAG_PA_ON_DEMAND_READY:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"ON Demand Deprecated",0,0,0);    
    /* Return a diag error packet indicating bad parameters were passed */
    return(diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, w_PktLength));
    /*Dont break. Let it fall to default*/
    break;
    
   default:
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Param: UNKNOWN PARAMETER %d!!", p_Req->u_Param, 0, 0);

      /* Return a diag error packet indicating bad parameters were passed */
      return(diagpkt_err_rsp(DIAG_BAD_PARM_F, p_PktReq, w_PktLength));
   }

 if (z_ParamSetVal.pa_set != PDSM_PA_NONE)
 {
 ...
 else 
 {
  e_Status = GPSDIAG_ERROR;
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_SetParamCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_GetParamCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetParamCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetParamCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetParamCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
  ...
  e_Result = pdsm_get_parameters(gpsdiag_GetParamCmdCb,
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetParamCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_GetLastKnownPosCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if (NULL == p_PktReq)
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetLastKnownCmd: Null pointer", 0, 0, 0);
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetLastKnownPosCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetLastKnownPosCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetLastKnownPosCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetLastKnownPosCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
      ...
      p_Rsp->w_ExtLocUncA     = (uint16) gpsdiag_pPdData.ext_pos_data.f_alongAxisUnc;
      p_Rsp->w_ExtLocUncP     = (uint16) gpsdiag_pPdData.ext_pos_data.f_perpAxisUnc;
      p_Rsp->x_VelHorizU      = (uint16)  gpsdiag_pPdData.ext_pos_data.velHorU;
...
//  diagpkt_commit((PACKED void*)p_Rsp);
 ...
 return NULL;
   ...
   #if INCLUDE_LCS_CMDS
#if 0 /*#9x05: LCS_CMDS not supported*/
/*===========================================================================
...
PACKED void* gpsdiag_NtfyVrfySndUsrActnCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
...
/*     {                                                                                                                        */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: SUPL data verified!!", 0, 0, 0);
...
/*     else                                                                           */
/*     {                                                                              */
/*       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: SUPL data unverified!!", 0, 0, 0); */
/*     }                                                                              */
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: SUPL SLP Address (IPV4)", 0, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: SUPL SLP Address (IPV6)", 0, 0, 0);
      ...
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: SUPL SLP Address (URL)", 0, 0, 0);
...
/*     {                                                                                    */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_NtfyVrfySndUsrActnCmd: UMTS CP data verified", 0, 0, 0);
#if 0 //TODO:[DEL][APP] Remove gpsdiag_zUmtsCpData
...
/*     else                                                                            */
/*     {                                                                               */
/*       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: UMTS CP data unverified", 0, 0, 0); */
/*     }                                                                               */
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: Unknown CmdType!!", 0, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_NtfyVrfySndUsrActnCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_LcsVxInitClientCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxInitClientCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsVxInitClientCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client Type Sanity Check */
   if(gpsdiag_ClientTable[e_ClientType].e_ClientType != (pdsm_client_type_e_type)e_ClientType)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxInitClientCmd: UNKNOWN CLIENT TYPE!!", 0, 0, 0);
   }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxInitClientCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_LcsVxInitClientAppInfoCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   else
   {
      /* Map Client Id with its corresponding Client Type */
      e_ClientType = p_Req->q_ClientType;
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsVxInitClientAppInfoCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client Type Sanity Check */
   if (gpsdiag_ClientTable[e_ClientType].e_ClientType != (pdsm_client_type_e_type)e_ClientType)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxInitClientAppInfoCmd: UNKNOWN CLIENT TYPE!!", 0, 0, 0);
   }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxInitClientAppInfoCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_LcsVxReleaseClientCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxReleaseClientCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsVxReleaseClientCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client Type Sanity Check */
   if(gpsdiag_ClientTable[e_ClientType].e_ClientType != (pdsm_client_type_e_type)e_ClientType)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxReleaseClientCmd: UNKNOWN CLIENT TYPE!!", 0, 0, 0);
   }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxReleaseClientCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_LcsVxQueryConfigCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxQueryConfigCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsVxQueryConfigCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client Type Sanity Check */
   if(gpsdiag_ClientTable[e_ClientType].e_ClientType != (pdsm_client_type_e_type)e_ClientType)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxQueryConfigCmd: UNKNOWN CLIENT TYPE!!", 0, 0, 0);
   }
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxQueryConfigCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_LcsVxGetClientTypeCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxGetClientTypeCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxGetClientTypeCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
//  gpsdiag_eStatusType               e_Status = GPSDIAG_SUCCESS;
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxGetStatusCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_LcsVxGetStatusCmd: Client Type %d", e_ClientType, 0, 0);
   }

   /* Client Type Sanity Check */
   if(gpsdiag_ClientTable[e_ClientType].e_ClientType != (pdsm_client_type_e_type)e_ClientType)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxGetStatusCmd: UNKNOWN CLIENT TYPE!!", 0, 0, 0);
   }
...
//  e_Status = (e_ResultBool) ? GPSDIAG_SUCCESS : GPSDIAG_ERROR;
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_LcsVxGetStatusCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void*  gpsdiag_Trigger_Xtrat_Session (PACKED void* p_PktReq, uint16 w_PktLength)
{
  ...
  if (p_Req->session_control == 0)
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_Trigger_Xtrat_Session: An IPC to stop xtrat session was sent", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_XtraT_Cmd: Failed to send an IPC to stop xtrat session", 0, 0, 0);
    }
  }
  else if(p_Req->session_control == 1)
  {
    ...
    if (session_trigger_success)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_Trigger_Xtrat_Session: An IPC to start xtrat session was sent", 0, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_XtraT_Cmd: Failed to send an IPC to start xtrat session", 0, 0, 0);
    }
  }
  else
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_XtraT_Cmd: Bad parameters were passed to XTRAT session controller -choose between either 0-stop session /1-trigger a new session", 0, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_Trigger_Xtrat_Session: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
...
PACKED void* gpsdiag_GetPdapiVersCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_GetPdapiVersCmd: Accessing PDAPI version number...", 0, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GetPdapiVersCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_InjectTimeCmd(PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectTimeCmd: Client Type Error %d", e_ClientType, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_InjectTimeCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectTimeCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectTimeCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_ExternCoarsePosInjCmd(PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   gpsdiag_eStatusType                e_Status = GPSDIAG_SUCCESS;
   uint32                             e_ClientType;
   pdsm_pd_external_pos_info_ex_type     z_ext_pos_info;
   ...
   if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_ExternCoarsePosInjCmd: Client Type Error!! %lu, %lu", e_ClientType, p_Req->l_ClientId, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_ExternCoarsePosInjCmd: Client Type %d", e_ClientType, 0, 0);
   }
   ...
   if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_ExternCoarsePosInjCmd: UNKNOWN CLIENT ID!!", 0, 0, 0);

      /* Return a diag error packet indicating packet not allowed in this mode */
      return(diagpkt_err_rsp(DIAG_BAD_MODE_F, p_PktReq, w_PktLength));
   }

   z_ext_pos_info.pos_info.opt_field_mask =
   (pdsm_pd_extern_coarse_pos_opt_field)p_Req->opt_field_mask;

   z_ext_pos_info.pos_info.time_info_type =
   (pdsm_pd_extern_coarse_pos_time_stamp_e_type)p_Req->time_info_type;

   z_ext_pos_info.pos_info.timestamp_sec = p_Req->timestamp_sec;

   z_ext_pos_info.pos_info.source =
   (pdsm_pd_extern_coarse_pos_source_e_type)p_Req->source;

   z_ext_pos_info.pos_info.latitude = p_Req->latitude;
   z_ext_pos_info.pos_info.longitude = p_Req->longitude;
   z_ext_pos_info.pos_info.loc_unc_hor = p_Req->loc_unc_hor;
   z_ext_pos_info.pos_info.alt_def = (pdsm_pd_extern_coarse_pos_alt_def_e_type)p_Req->alt_def;
   z_ext_pos_info.pos_info.altitude = p_Req->altitude;
   z_ext_pos_info.pos_info.loc_unc_vert = p_Req->loc_unc_vert;
   z_ext_pos_info.pos_info.confidence_hor = p_Req->confidence_hor;
   z_ext_pos_info.pos_info.confidence_vert = p_Req->confidence_vert;
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_ExternCoarsePosInjCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_QueryGpsStateCmd(PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_QueryGpsStateCmd: Client Type Error %d", q_ClientType, 0, 0);
      e_CmdErrorType = DIAG_BAD_PARM_F;
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_QueryGpsStateCmd: Client Type %d", q_ClientType, 0, 0);
   ...
   (void)pdsm_pd_query_gps_state( NULL, NULL, l_ClientId);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_QueryGpsStateCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
                                ...
                                pdsm_xtra_cmd_err_e_type e_XtraCmdErr)
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb: user_data ptr 0x%x, cmdtype %d, cmderr %d",
    p_DataBlock, e_XtraCmd, e_XtraCmdErr);

  if ( e_XtraCmd >= PDSM_XTRA_CMD_MAX )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_process_XtraCmdCb: Invalid command %d",
               e_XtraCmd,0,0);
    return;
  }
  if ( e_XtraCmdErr > PDSM_XTRA_CMD_ERR_OTHER )
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_process_XtraCmdCb: Invalid command error %d",
               e_XtraCmdErr,0,0);
    return;
  }
  ...
  case PDSM_XTRA_CMD_SET_DATA:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd SET DATA (%d)", e_XtraCmd,0,0);
    ...
    else
    {
       ...
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_process_XtraCmdCb: Xtra Data Pkt Unsuccesful err:%d",
                  e_XtraCmdErr,0,0);
    }
  ...
  case PDSM_XTRA_CMD_TIME_INFO_INJ:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd TIME INJECT (%d)",
             e_XtraCmdErr,0,0);
    break;

  case PDSM_XTRA_CMD_INITIATE_DOWNLOAD:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd INIT DOWNLOAD Err (%d)",
             e_XtraCmdErr,0,0);
    break;

  case PDSM_XTRA_CMD_SET_AUTO_DOWNLOAD_PARAMS:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd SET DLD PARAMS Err (%d)",
             e_XtraCmdErr,0,0);
    break;

  case PDSM_XTRA_CMD_QUERY_DATA_VALIDITY:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd QUERY DATA  Err (%d)",
             e_XtraCmd, e_XtraCmdErr,0);
    break;
  default:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_process_XtraCmdCb:XTRA Cmd ignored (%d)", e_XtraCmd,0,0);
    break;
  }
...
PACKED void* gpsdiag_InjectXtraDataCmd(PACKED void* p_PktReq, uint16 w_PktLength)
{
  ...
  if(e_ClientType >= (uint32)PDSM_CLIENT_TYPE_MAX ||
     e_ClientType == (uint32)PDSM_CLIENT_TYPE_NONE)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectXtraDataCmd: Client Type Error type:%d id:%d",
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_InjectXtraDataCmd: Client Type %d id:%d", e_ClientType,
             p_Req->l_ClientId, 0);
  }
  ...
  if(gpsdiag_ClientTable[e_ClientType].l_ClientId != p_Req->l_ClientId)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectXtraDataCmd: UNKNOWN CLIENT ID!! 1:%d 2:%d",
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectXtraDataCmd, Payload exceeds limit,Xtralen:%d part:%d Totlen:%d",
  ...
  else
  {
    ...
    if (FALSE == u_Status)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectXtraDataCmd, Injection rejected,  datelen: %d  segment:%d  totalparts:%d",
                 p_Req->q_Xtra_Data_Len, p_Req->q_Data_Part_Number, p_Req->q_Data_Total_Parts);
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_InjectXtraDataCmd, Injection sucessful,  datelen: %d  segment:%d  totalparts:%d",
                p_Req->q_Xtra_Data_Len, p_Req->q_Data_Part_Number, p_Req->q_Data_Total_Parts);
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_InjectXtraDataCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
...
PACKED void* gpsdiag_Req1xEvdoRfInfoCmd (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   if(p_Rsp == NULL)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_Req1xEvdoRfInfoCmd: Rsp packet is NULL!!", 0, 0, 0);
   }
   else
   {
      p_Rsp->q_Status      = (uint32)GPSDIAG_SUCCESS;
...
PACKED void* gpsdiag_PdapiCmdHandler (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CmdCode NOT DIAG_SUBSYS_CMD_F.", 0, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"SubsysId NOT DIAG_SUBSYS_GPS.", 0, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_PdapiCmdHandler - CgpsCmdCode:%d len:%d",
      ...
      else
      {
         ...
         case CGPS_XTRA_INJECT_DATA_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_XTRA_INJECT_DATA_CMD pktlen:%d",
                     w_PktLength, 0, 0);
            return gpsdiag_InjectXtraDataCmd(p_PktReq, w_PktLength);

         case CGPS_XTRA_CLIENT_REGISTER_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_XTRA_CLIENT_REGISTER_CMD pktlen:%d",
                     w_PktLength, 0, 0);
            return gpsdiag_XtraRegCmd(p_PktReq, w_PktLength);

         case CGPS_START_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_START_CMD", 0, 0, 0);
            return gpsdiag_StartCmd(p_PktReq, w_PktLength);

         case CGPS_END_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_END_CMD", 0, 0, 0);
            return gpsdiag_EndCmd(p_PktReq, w_PktLength);

         case CGPS_SET_PARAM_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_SET_PARAM_CMD", 0, 0, 0);
            return gpsdiag_SetParamCmd(p_PktReq, w_PktLength);

         case CGPS_GET_PARAM_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_GET_PARAM_CMD", 0, 0, 0);
            return gpsdiag_GetParamCmd(p_PktReq, w_PktLength);

         case CGPS_GET_LAST_KNOWN_POS_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_GET_LAST_KNOWN_POS_CMD", 0, 0, 0);
            return gpsdiag_GetLastKnownPosCmd(p_PktReq, w_PktLength);

         case CGPS_GET_PDAPI_VERS_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_GET_PDAPI_VERS_CMD", 0, 0, 0);
            return gpsdiag_GetPdapiVersCmd(p_PktReq, w_PktLength);


         case CGPS_INJECT_TIME_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_INJECT_TIME_CMD", 0, 0, 0);
            return gpsdiag_InjectTimeCmd(p_PktReq, w_PktLength);

         case CGPS_EXTERN_COARSE_POS_INJ_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_EXTERN_COARSE_POS_INJ_CMD", 0, 0, 0);
            return gpsdiag_ExternCoarsePosInjCmd(p_PktReq, w_PktLength);
#if 0 //#9x05 LCS Not supported
         case CGPS_LCS_VX_INIT_CLIENT_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_INIT_CLIENT_CMD", 0, 0, 0);
            return gpsdiag_LcsVxInitClientCmd(p_PktReq, w_PktLength);

         case CGPS_LCS_VX_INIT_CLIENT_APP_INFO_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_INIT_CLIENT_APP_INFO_CMD", 0, 0, 0);
            return gpsdiag_LcsVxInitClientAppInfoCmd(p_PktReq, w_PktLength);

         case CGPS_LCS_VX_RELEASE_CLIENT_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_RELEASE_CLIENT_CMD", 0, 0, 0);
            return gpsdiag_LcsVxReleaseClientCmd(p_PktReq, w_PktLength);

         case CGPS_LCS_VX_QUERY_CONFIG_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_QUERY_CONFIG_CMD", 0, 0, 0);
            return gpsdiag_LcsVxQueryConfigCmd(p_PktReq, w_PktLength);

         case CGPS_LCS_VX_GET_CLIENT_TYPE_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_GET_CLIENT_TYPE_CMD", 0, 0, 0);
            return gpsdiag_LcsVxGetClientTypeCmd(p_PktReq, w_PktLength);

         case CGPS_LCS_VX_GET_STATUS_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_LCS_VX_GET_STATUS_CMD", 0, 0, 0);
            return gpsdiag_LcsVxGetStatusCmd(p_PktReq, w_PktLength);
#endif
         case CGPS_QUERY_GPS_STATE_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_QUERY_GPS_STATE_CMD", 0, 0, 0);
            return gpsdiag_QueryGpsStateCmd(p_PktReq, w_PktLength);

         case CGPS_TRIGGER_XTRAT_SESSION:
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_TRIGGER_XTRAT_SESSION", 0, 0, 0);
           return gpsdiag_Trigger_Xtrat_Session(p_PktReq, w_PktLength);

         case CGPS_TEST_FORCE_MEM_ALLOC_ERROR:
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_TEST_FORCE_MEM_ALLOC_ERROR", 0, 0, 0);
           return gpsdiag_TestForceMemAllocError(p_PktReq, w_PktLength);

         case CGPS_QUERY_VERSION:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_QUERY_VERSION", 0, 0, 0);
            return gpsdiag_QueryVersionCmd(p_PktReq, w_PktLength);

         case CGPS_OEM_CONTROL:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_OEM_CONTROL", 0, 0, 0);
            return gpsdiag_OemControl(p_PktReq, w_PktLength);

         case CGPS_OEM_DRSYNC_SET_CFG:
           MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_OEM_DRSYNC_SET_CFG", 0, 0, 0);
           return gpsdiag_OemDrSyncSetCfg(p_PktReq, w_PktLength);

         case CGPS_INTERNAL_REQ_1XEVDO_RF_INFO_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_INTERNAL_REQ_1XEVDO_RF_INFO_CMD", 0, 0, 0);
            return gpsdiag_Req1xEvdoRfInfoCmd(p_PktReq, w_PktLength);

         case CGPS_GEOFENCE_IN_MEM_LOG_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_GEOFENCE_IN_MEM_LOG_CMD", 0, 0, 0);
            return gpsdiag_GeofenceInMemLog(p_PktReq, w_PktLength);

         case CGPS_GERA_TEST_CMD:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"CgpsCmdCode: CGPS_GERA_TEST_CMD", 0, 0, 0);
            return gpsdiag_GeraTestCmd(p_PktReq, w_PktLength);

         default:
            MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"CgpsCmdCode: UNKNOWN GPS CMD CODE!!", 0, 0, 0);
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"SubsysCmdCode NOT CGPS_DIAG_PDAPI_CMD.", 0, 0, 0);
                          ...
                          const pdsm_xtra_info_s_type *p_XtraInfo )
{
  ...
  MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraEventCb: Event %d, usr ptr 0x%x", u_XtraEvent, p_UserData, 0);

  if (p_XtraInfo == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR," gpsdiag_XtraEventCb: No Valid Information in Callback", 0, 0, 0);
    return;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_XtraEventCb:Invalid event:%d id:$d",
  ...
  case PDSM_XTRA_EVENT_STATUS:
    ...
    if (p_XtraInfo->xtra_union.xtra_download_info.flags == XTRA_DOWNLOAD_INFO_FLAGS_STATUS)
    {
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_XtraEventCb:autoDld Event Ignored: STATUS (%d)",
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_XtraEventCb: DATA VALIDITY INFO Event",
               0,0,0);
    }
    break;

  case PDSM_XTRA_EVENT_DOWNLOAD_REQ:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraEventCb: DLD REQ ignored", 0,0,0);
    break;

  case PDSM_XTRA_EVENT_TIME_REQ:
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraEventCb: TIME REQ Ignored", 0,0,0);
    break;
...
PACKED void* gpsdiag_XtraRegCmd(PACKED void* p_PktReq, uint16 w_PktLength)
{
  ...
  gpsdiag_ClientTable[PDSM_CLIENT_TYPE_XTRA].l_ClientId
    = pdsm_lite_client_init(PDSM_CLIENT_TYPE_XTRA);
  ...
  {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"unable to initialize xtra client; exiting id:%d \n",
  ...
  else
  {
    ...
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraRegCmd: XTRA Registration succeeded  %d\n",
                gpsdiag_ClientTable[PDSM_CLIENT_TYPE_XTRA].l_ClientId, 0, 0);
    }
    else
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraRegCmd: XTRA Registration Failed  %d retvalReg:%d \n",
                gpsdiag_ClientTable[PDSM_CLIENT_TYPE_XTRA].l_ClientId, retVal, 0);
    }

    retVal2 = pdsm_lite_client_act(gpsdiag_ClientTable[PDSM_CLIENT_TYPE_XTRA].l_ClientId) ;

    if (retVal2 != PDSM_CLIENT_OK)
    {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_XtraRegCmd: XTRA Client Activation Failed  id:%d retValAct:%d \n",
  ...
  if(p_Rsp == NULL)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_RegCmd: Rsp packet is NULL!!", 0, 0, 0);
  }
  else
  {
    p_Rsp->z_DiagCmdHdr.u_CgpsCmdCode = p_Req->z_DiagCmdHdr.u_CgpsCmdCode;
                          ...
                          rex_task_func_type p_task_entry_func)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot initialize task attributes.  Error = %d", error, 0, 0);
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot set task Stack Addr.  Error = %d", error, 0, 0);
    return error;
  }
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot set task Stack Size.  Error = %d", error, 0, 0);
    return error;
  }
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot set task Priority.  Error = %d", error, 0, 0);
    return error;
  }
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot set task Entry Function.  Error = %d", error, 0, 0);
    return error;
  }
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot set task name.  Error = %d", error, 0, 0);
    return error;
  }
  ...
  if(error != REX_SUCCESS)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"Cannot define task with Rex.  Error = %d", error, 0, 0);
    return error;
  }
...
PACKED void* gpsdiag_StartTask (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {
      /* Create GPSDIAG task */
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Creating GPSDIAG task", 0, 0, 0);
                                             ...
                                             (rex_task_func_type) gpsdiag_task))
      {
         ...
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG task started.", 0, 0, 0);
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG task NOT started.", 0, 0, 0);
      }
   }
   os_MutexUnlock(p_GpsDiagTaskMutex); /* TASKFREE(); */
...
//  diagpkt_commit(p_EndRsp);
   ...
   {
     MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Ending GPSDIAG task", 0, 0, 0);
     if(gpsdiag_SendMsg(0, THREAD_ID_GPSDIAG, C_GPSDIAG_END_TASK, NULL))
     {
       MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"GPSDIAG IPC Msg %d Not Sent.", C_GPSDIAG_END_TASK, 0, 0);
     }

     MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"GPSDIAG task ended", 0, 0, 0);
   }
   os_MutexUnlock(p_GpsDiagTaskMutex); /* TASKFREE(); */
...
PACKED void* pdcommtd_StartTask (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {      
      ...
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Creating PDCOMM_TD task", 0, 0, 0);
                                             ...
                                             (rex_task_func_type) pdcomm_test_driver_task))
      {
         ...
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM_TD task started.", 0, 0, 0);
      }
      else
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"PDCOMM_TD task NOT started.", 0, 0, 0);
      }
   }
   else
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"A instance of PDCOMM_TD task already running",0,0,0);
   }
...
PACKED void* pdcommtd_EndTask (PACKED void* p_PktReq, uint16 w_PktLength)
{
   ...
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"Ending PDCOMM TD task", 0, 0, 0);
      rex_kill_task(&pdcommtd_tcb);
      pdcommtd_TaskIsRunning = FALSE;
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"PDCOMM TD task ended", 0, 0, 0);
   }
   os_MutexUnlock(p_PdcommTdTaskMutex);
...
static void gpsdiag_InitAppTracking (void)
{
   ...
   for(q_ClientType = 0; q_ClientType < (uint32)PDSM_CLIENT_TYPE_MAX; q_ClientType++)
   {
      ...
      if(gpsdiag_ClientTable[q_ClientType].p_AppTrackingTimer == NULL)
      {
         MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_InitAppTracking: Init Failed.", 0, 0, 0);
      }
   }

   MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_InitAppTracking: Init Succeeded", 0, 0, 0);
...
static void gpsdiag_StartAppTracking (pdsm_client_type_e_type e_ClientType)
{
  ...
  if (e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_StartAppTracking: Client Type Error %d", e_ClientType, 0, 0);
    return;
  }
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartAppTracking: App Tracking Stopped!!", 0, 0, 0);
    return;
  }
  ...
  if (!pdsm_get_position(gpsdiag_GetPosFixCmdCb,
                         ...
                         gpsdiag_ClientTable[e_ClientType].l_ClientId))
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_StartAppTracking: Unable to call pdsm_get_position",0 ,0, 0);
  }
  else
  {
    ...
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartAppTracking: New Position Fix Request (%d/%d)",
            (gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.q_NumFixesTried),
            (gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.q_NumFixesLeft + gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.q_NumFixesTried),
            0);
    ...
    else
    {
      ...
      if (!os_TimerStart(gpsdiag_ClientTable[e_ClientType].p_AppTrackingTimer,
                         ...
                         gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.q_NumFixesTried))
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartAppTracking: Timer Start Failed.", 0, 0, 0);
      }
      else
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_StartAppTracking: Timer Start Succeeded.", 0, 0, 0);
      }
    }
  }
...
static void gpsdiag_RestartAppTracking (pdsm_client_type_e_type e_ClientType)
{
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_RestartAppTracking: Application Tracking Timer Expired", 0, 0, 0);

   /* Client Type Sanity Check */
   if(e_ClientType >= PDSM_CLIENT_TYPE_MAX ||
      e_ClientType == PDSM_CLIENT_TYPE_NONE)
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_RestartAppTracking: Client Type Error %d", e_ClientType, 0, 0);
      return;
   }
   ...
   if((gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.u_PdsmGetPosDone) &&
      ...
      (gpsdiag_ClientTable[e_ClientType].z_FixFlowCtrl.e_FixType == GPSDIAG_FIX_TYPE_APP_TRACKING))
   {
      MSG_3(MSG_SSID_GPS, MSG_LEGACY_MED,"gpsdiag_RestartAppTracking: Starting Application Tracking Again...", 0, 0, 0);
                                                         ...
                                                         const gpsdiag_PaDelParamsType *pSrc)
{
   ...
   if(pDest && pSrc)
   {
      pDest->pdsm_lite_delete_parms_flags = 0; /* reset all flags to prevent spurious bit setting */
      ...
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH;
      }

      if(GPSDIAG_PA_DELETE_ALM & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM;
      }

      if(GPSDIAG_PA_DELETE_POS & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_POS;
      }

      if(GPSDIAG_PA_DELETE_TIME & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_TIME;
      }

      if(GPSDIAG_PA_DELETE_IONO & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_IONO;
      }

      if(GPSDIAG_PA_DELETE_UTC & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_UTC;
      }

      if(GPSDIAG_PA_DELETE_HEALTH & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_HEALTH;
      }

      if(GPSDIAG_PA_DELETE_SVDIR & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR;
      }

      if(GPSDIAG_PA_DELETE_SVSTEER & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVSTEER;
      }

      if(GPSDIAG_PA_DELETE_SADATA & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SADATA;
      }

      if(GPSDIAG_PA_DELETE_RTI & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_RTI;
      }

      if(GPSDIAG_PA_TIME_UNC & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_TIME_UNC;
      }

      if(GPSDIAG_PA_POS_UNC & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_POS_UNC;
      }

      if(GPSDIAG_PA_TIME_OFFSET & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_TIME_OFFSET;
      }

      if(GPSDIAG_PA_POS_OFFSET & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_POS_OFFSET;
      }

      if(GPSDIAG_PA_DELETE_ALM_CORR & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_CORR;
      }

      if(GPSDIAG_PA_FREQ_BIAS_EST & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_FREQ_BIAS_EST;
      }

      if(GPSDIAG_PA_DELETE_SV_NO_EXIST & pSrc->q_DelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SV_NO_EXIST;
      }

      //translate newer gnss flags
      if(GNSSDIAG_PA_DELETE_EPH_GLO        & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH_GLO;
      }

      if(GNSSDIAG_PA_DELETE_EPH_SBAS       & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH_SBAS;
      }

      if(GNSSDIAG_PA_DELETE_ALM_GLO        & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_GLO;
      }

      if(GNSSDIAG_PA_DELETE_ALM_SBAS       & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_SBAS;
      }

      if(GNSSDIAG_PA_DELETE_SVDIR_GLO      & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR_GLO;
      }

      if(GNSSDIAG_PA_DELETE_SVDIR_SBAS     & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR_SBAS;
      }

      if(GNSSDIAG_PA_DELETE_SVSTEER_GLO    & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVSTEER_GLO;
      }

      if(GNSSDIAG_PA_DELETE_ALM_CORR_GLO   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_CORR_GLO;
      }

      if(GNSSDIAG_PA_DELETE_CLOCK_INFO   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_CLOCK_INFO;
      }

      // BDS support
      if(GNSSDIAG_PA_DELETE_EPH_BDS       & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH_BDS;
      }

      if(GNSSDIAG_PA_DELETE_ALM_BDS        & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_BDS;
      }

      if(GNSSDIAG_PA_DELETE_SVDIR_BDS      & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR_BDS;
      }

      if(GNSSDIAG_PA_DELETE_SVSTEER_BDS    & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVSTEER_BDS;
      }

      if(GNSSDIAG_PA_DELETE_ALM_CORR_BDS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_CORR_BDS;
      }

      // GAL support
      if(GNSSDIAG_PA_DELETE_EPH_GAL       & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH_GAL;
      }

      if(GNSSDIAG_PA_DELETE_ALM_GAL        & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_GAL;
      }

      if(GNSSDIAG_PA_DELETE_SVDIR_GAL      & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR_GAL;
      }

      if(GNSSDIAG_PA_DELETE_SVSTEER_GAL    & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVSTEER_GAL;
      }

      if(GNSSDIAG_PA_DELETE_ALM_CORR_GAL   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_CORR_GAL;
      }

      if(GNSSDIAG_PA_DELETE_GNSS_SV_BLACKLIST_GPS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_GNSS_SV_BLACKLIST_GPS;
      }

      if(GNSSDIAG_PA_DELETE_GNSS_SV_BLACKLIST_GLO   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_GNSS_SV_BLACKLIST_GLO;
      }

      if(GNSSDIAG_PA_DELETE_GNSS_SV_BLACKLIST_BDS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_GNSS_SV_BLACKLIST_BDS;
      }

      if(GNSSDIAG_PA_DELETE_GNSS_SV_BLACKLIST_GAL   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_GNSS_SV_BLACKLIST_GAL;
      }

      if(GNSSDIAG_PA_DELETE_GNSS_SV_BLACKLIST_QZSS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_GNSS_SV_BLACKLIST_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_EPH_QZSS       & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_EPH_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_ALM_QZSS        & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_SVDIR_QZSS      & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVDIR_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_SVSTEER_QZSS    & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_SVSTEER_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_ALM_CORR_QZSS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_ALM_CORR_QZSS;
      }

      if(GNSSDIAG_PA_DELETE_IONO_QZSS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_IONO_QZSS;
      }

	  if(GNSSDIAG_PA_DELETE_IONO_BDS   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_IONO_BDS;
      } 

	  if(GNSSDIAG_PA_DELETE_IONO_GAL   & pSrc->q_GnssDelParamsFlags)
      {
         pDest->pdsm_lite_delete_parms_flags |= PDSM_PA_DELETE_IONO_GAL;
      } 
...
{
   MSG_3(MSG_SSID_GPS, MSG_LEGACY_HIGH,"gpsdiag_ExternalSPInjectCb received cmd_type = %d, error =%d", pd_cmd, pd_cmd_err, 0);
   return;
}
...
static PACKED void* gpsdiag_OemDre(gpsdiag_OemControlReqType* p_Req, uint16 w_PktLength)
{
   ...
   if (FEATURE_OEMDRE_NONE != oemdre_Resp)
   {
      ...
      {
        MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_OemDre - response packet not allocated!", 0, 0, 0);
      }
   }
   else
   {
...
static PACKED void* gpsdiag_GeofenceInMemLog(gpsdiag_GeofenceInMemLogCmdReqType* p_Req, uint16 w_PktLength)
{
  ...
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_OemGeofenceLog - response packet not allocated!", 0, 0, 0);
  }
   return p_RspPkt;
...
{
   PACKED void* p_RspPkt = NULL;
   gpsdiag_GeraTestCmdRspType* p_Resp;
   boolean b_Resp = FALSE;

   if (NULL == p_Req)
   {
      MSG(MSG_SSID_GPSSM, MSG_LEGACY_ERROR, "gpsdiag_GeraTestCmd - p_Req is NULL");
      return  p_RspPkt; 
   }
  ...
  p_RspPkt = diagpkt_subsys_alloc( (diagpkt_subsys_id_type)DIAG_SUBSYS_GPS,
                                    CGPS_DIAG_PDAPI_CMD,
                                    sizeof(gpsdiag_GeraTestCmdRspType) );
  if (NULL != p_RspPkt)
  {
    p_Resp = (gpsdiag_GeraTestCmdRspType*) p_RspPkt;
    p_Resp->z_DiagCmdHdr = p_Req->z_DiagCmdHdr;
    p_Resp->l_TestClientId = p_Req->l_TestClientId;
    p_Resp->q_Status  = (uint32) b_Resp;
  }
  else
  {
    MSG_3(MSG_SSID_GPS, MSG_LEGACY_ERROR,"gpsdiag_GeraTestCmd - response packet not allocated!", 0, 0, 0);
  }
   return p_RspPkt;

} /* end gpsdiag_GeraTestCmd() */
