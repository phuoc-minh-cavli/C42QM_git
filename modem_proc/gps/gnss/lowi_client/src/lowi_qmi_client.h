...
public:
   ...
   *             lost
   */
  virtual void notifyServiceStatus (bool found) = 0;
...
private:
  ...
  qmi_txn_handle                    mTxn;
  qmi_idl_service_object_type       mLowiServiceObject;
...
//  virtual void run ();
  /**
   * Called when the Service Up notification is received to get 
   * the Client Handle 
   */ 
  bool initQMIClient ();
...
public:
  ...
  */
  ...
  /**
   * This callback function is called by the QCCI infrastructure 
   * when the service status changes. 
   * 
   * @param[in]   user_handle     Handle used by the infrastructure to
   *                              identify different clients.
   * @param[in]   svc_obj         Service Object type
   * @param[in]   svc_event       Service Event type
   * @param[in]   err_cb_data     User-data
   */
  static void serviceStatusCallback (qmi_client_type /* user_handle */,
                                     qmi_idl_service_object_type /*svc_obj */,
                                     qmi_client_notify_event_type svc_event,
                                     void *err_cb_data);
  ...
  /**
   * Sends Ranging request to WLAN QMI Service
   * Response contains the ranging response
   * @param QmiLowiRangingRequest_v01* LOWIRequest that needs to 
   *                                   be sent over QMI
   * @param[out] QMI error code       Only valid if the return value is true
   * @return bool true if there was an attempt to send the request over QMI
   */
   ...
   * or time out.
   * NOTE: This function or ayncInit should be called before
   * sending any request, or expect request failure
  ...
  /**
   * Sends the Async initialization request 
   * QMI client implementation detects the service async and 
   * notifies the LOWIQMIClientListenerInterface
   * 
   * NOTE: This function or init() should be called before sending
   * any request, or expect request failure
   * 
   * @return bool true if success, false otherwise
   */
