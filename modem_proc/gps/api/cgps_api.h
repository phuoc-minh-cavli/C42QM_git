...
Version Control
  ...
  $Header: //components/rel/gnss8.mpss/9.5.1.1/api/cgps_api.h#1 $
...
#include "cai.h"
#else
	/* Currently Copied from cai.h for the 9x05 as a workaround */
...
typedef PACKED struct
{
   ...
   * Note: This variable is overloaded with time diff (in msec) between 1x reading TTr latch data
   * from GTS DB and time at which the latched TTr data was actually captured (sent by CC) */
...
typedef struct
{
  ...
  /* LPM RTC value latched at the same time as the first latched CDMA/GNSS RTC
     ...
     Note: this variable is overloaded with latched USTMR value from CC-GTS interface */
  ...
  /* Universal STMR value latched at the same time as the first latched
     ...
     Note: this variable is overloaded with USTMR value used for obtaining CDMA RTC */
...
typedef struct
{
  ...
  uint64             t_RefOscUpdateTimestamp;
