...
$Header: //components/rel/gnss8.mpss/9.5.1.1/api/gnss_api.h#1 $

when         who     what, where, why
----------   ---     ------------------------------------------------------
07/31/2019   tn      update gnss_LnaConfigEnumType
...
/* Create TECH MODULE MACRO FOR GPS RCVR */
#define MSGR_MODULE_GPS_RCVR 0x03
#define MSGR_GPS_RCVR   MSGR_TECH_MODULE(MSGR_TECH_GPS, MSGR_MODULE_GPS_RCVR)

enum
{
  MSGR_DEFINE_UMID(GPS, RCVR, IND, GNSS_CRIT_DEMOD, 0x01, NULL)
}; 

/* GNSS critical demod indication structure */
typedef struct {
  msgr_hdr_struct_type  hdr;                   /*!< MSGR header */
  uint8 u_GnssInCritDemod;  /* GNSS in critical demod */
  uint16 w_Reserved1; /* Bit0: 0 - Mission mode, 1 - Rfdev mode*/
  uint32 w_Reserved2;
} gnss_CriticalDemodInd; 

/* GNSS Receiver State */
enum
{
  MSGR_DEFINE_UMID(GPS, RCVR, IND, GNSS_RCVR_STATUS, 0x02, NULL)
}; 

/* GNSS receiver On/Off status structure */
typedef struct {
  msgr_hdr_struct_type  hdr;                   /*!< MSGR header */
  uint8 u_RcvrStatus;  /* 0- OFF; 1-ON */
  uint16 w_Reserved1;
  uint32 w_Reserved2;
} gnss_MsgrRcvrStatus; 
...
#define MSGR_GPS_MC   MSGR_TECH_MODULE(MSGR_TECH_GPS, 0x02)

typedef struct{
   boolean   u_EnableSession; // Flag to decide whether to do gps session or not
} gnss_mc_session_sts_info_s;
...
typedef struct
{
  ...
  gnss_mc_session_sts_info_s      z_SessionStatusInfo;  /* GPS Session ON or OFF */
} gnss_mc_session_sts_ind_s; 

enum
{
  MSGR_DEFINE_UMID(GPS, MC, IND, SESSION_STS,
                   0x01, gnss_mc_session_sts_ind_s)
};
...
typedef enum
{
  ...
  GNSS_RF_TWO_SAW_HAS_GLNA,   // 5
  GNSS_RF_SINGLE_SAW_HAS_GLNA, // 6
  GNSS_RF_SHARED_ANT_MBPATH_NO_ELNA,        // 7
  GNSS_RF_SHARED_ANT_NO_ELNA,               //8
  GNSS_RF_SHARED_ANT_NO_ELNA_SINGLE_SAW,    //9
  GNSS_RF_SHARED_ANT_ELNA,                  //10
  GNSS_RF_DEDICATED_ANT_NO_ELNA,            //11
  GNSS_RF_DEDICATED_ANT_NO_ELNA_SINGLE_SAW,  //12
  GNSS_RF_DEDICATED_ANT_ELNA                //13
