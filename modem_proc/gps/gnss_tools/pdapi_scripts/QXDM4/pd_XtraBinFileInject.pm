################################################################################
# $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss_tools/pdapi_scripts/QXDM4/pd_XtraBinFileInject.pm#1 $
...
sub GpsXtraBinFileSend
{
  ...
  my $savefile = "c:\\predictedorbit\\xtra.bin";
  #print ("\n\n\n~~~~~~~~~~~~~~~~~~~~~~injecting bin file from PC ~~~~~~~~~~~~~~~~~~~~~\n\n\n");                
  my $timeslept = 0;    
  my $XtraInjectCmdRsp = 0;
  
  open (MYBINFILE, "c:\\predictedorbit\\xtra.bin") or die "\n", $!, "Does Xtra file exist\?\n\n";
  ...
  {           
    ...
    while(1)
    {
      ...
      #if we have waited for  more than 50 seconds, then let us abort Saying that
      # Xtra injection is not supported on this target.
      if ( $timeslept == 50000000 ) 
      {
        printf  main::LOGFILE  ("\nABORTING Test due to TIMEOUT of Injected Xtra Data Cmd, This maybe either because : !!\n\n");	                
        printf  main::LOGFILE  ("\n 1\). This build doesn't support XTRA diag Cmds!! OR\n\n");	                        
        printf  main::LOGFILE  ("\n 2\). The GPS engine is in  a bad state and not responding to  GPS diag Cmds!!\n\n");	                
