######################################################################
# $Header: //components/rel/gnss8.mpss/9.5.1.1/gnss_tools/pdapi_scripts/QXDM4/pd_SendGeraTestCmd.pm#1 $
# $DateTime: 2020/07/21 21:23:44 $
#
# Define class in Perl to send GERA test commands
#
######################################################################

package pd_SendGeraTestCmd;
...
my %fields = (
  ...
  gera_test_command  => undef, # (4 bytes)
  gera_client_id     => undef, # (4 bytes)
  ex_enable_client_mask => undef, # (4 bytes)
);
...
sub SetCaptureCmd
{
  ...
  my @arg_bytes = uint32toBytes($self->{"gera_test_command"});
  my $i = 10;
  foreach (@arg_bytes)
  {
    $req_pkt->Put($i, $_);
  ...
  my @arg_bytes = uint32toBytes($self->{"gera_client_id"});
  my $i = 14;
  foreach (@arg_bytes)
  {
    $req_pkt->Put($i, $_);
  ...
  my @arg_bytes = uint32toBytes($self->{"ex_enable_client_mask"});
  my $i = 18;
  foreach (@arg_bytes)
  {
    $req_pkt->Put($i, $_);
