# 
#$Header: //components/rel/gnss8.mpss/9.5.1.1/gnss_tools/standalone_rfv/QXDM4/bus_bw_test/cgps_BusBwTest.pl#1 $ $DateTime: 2020/07/21 21:23:44 $ $Author: pwbldsvc $
#*****************************************************************************
...
if ( $ARGV[2] eq 'maxsys' )
{
    my $indirect = int($ARGV[3]);    
    if (not defined $indirect) {
        printf ("Missing indirect argument for maxsys.\n");
        exit();
    }

    my $direct = int($ARGV[4]);
    if (not defined $direct) {
        printf ("Missing direct argument for maxsys.\n");
        exit();
    }
    # maxsys takes # of indirect and direct channels
    $req->{"integ_length"} = (($indirect & 255) << 8) | ($direct & 0xff);
}
...
Syntax:   perl cgps_BusBwTest.pl <COM PORT> <START/STOP> <TEST TYPE>
          ...
          TEST TYPE:        (maxdma/maxsys) 
          maxsys            #indirect_chs   #direct_chs
