...
/* $Header: //components/rel/geran.mpss/5.2.0/api/geran_grr_msg.h#2 $ */
...
enum
{
  ...
  MSGR_DEFINE_UMID(GERAN, GRR, REQ, CELL_SELECT_INFO,         0x16, geran_grr_cell_select_info_req_t),

  /*! @brief The last message in the enum */
   GERAN_GRR_REQ_LAST = GERAN_GRR_CELL_SELECT_INFO_REQ
};
...
/*! @brief Message for Cell info Request from QMI ,to sharecell reselect parameters on response
*/
typedef struct
{
  /*! @brief Message router header. */
...
} geran_grr_cell_select_info_req_t;
...
typedef union
{
  ...
  geran_grr_cell_select_info_req_t    cell_select_info_req;
} geran_grr_msg_req_u;
...
enum
{
  ...
  MSGR_DEFINE_UMID(GERAN, GRR, RSP, CELL_SELECT_INFO,         0x14, geran_grr_cell_select_info_rsp_t),

  /*! @brief The last message in the enum */
  GERAN_GRR_RSP_LAST = GERAN_GRR_CELL_SELECT_INFO_RSP
};
...
/*! @brief Message for cell select info Response
*/
typedef struct
{
  /*! @brief Message router header. */
  ...
  /*! @brief  whether  ue camped or not. */
  boolean              is_ue_camped;
  
  /*! @brief Gives RXLEV_ACCESS_MIN parameter recieved in SI3/SI4 for the serving cell */
  byte                 rxlev_access_min;
  
} geran_grr_cell_select_info_rsp_t;
...
typedef union
{
  ...
  geran_grr_cell_select_info_rsp_t         cell_select_info_rsp;
} geran_grr_msg_rsp_u;
/*
...
} geran_grr_update_forbidden_bands_ind_t;
...
/*!@brief                        
  ...
  Functionality for this signal is under FEATURE_GERAN_X2G_ACQ_DB_SCAN.
*/
typedef struct
{
  /*! @brief Indicates arfcn of the GSM cell*/
