...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gllc/src/gllcfrme.c#4 $
***
...
void gllc_assemble_frame_hdr( void )
{
  ...
  LLC_FRAME_SET_SAPI( buf, (uint32)gllc_assy_info.lle );
  ...
  case GLLC_I_S_FORMAT:
    ...
    if ( frame_p -> i_s_frame.cmd_res_S1S2 == GLLC_SACK )
    {
      ...
      (
        &buf[5],
        ( sizeof(buf) - 5 ),
        gllc_assy_info.sack_bitmap_R_p,
  ...
  gllc_ass_frame_header_fcs( buf, hdr_length );

  (void)gprs_pdu_append_head(
    &gllc_assy_info.grr_ul_mes -> pdu_ptr, buf, hdr_length );

  gllc_assy_info.fr_hdr_length = hdr_length;
...
void gllc_assemble_frame_info_and_fcs( void )
{
  ...
  case GLLC_I_S_FORMAT:
         ...
         (gllc_llme.gea != GMMLLC_GEA_UNDEFINED)    )
    {
      ...
      (uint32)
      (
        gllc_lle[ gllc_assy_info.lle ].ack.input_offset_IOV_I
        +
    ...
    else
    {
      ...
      l3_pdu_temp = gprs_pdu_duplicate(
        frame_p -> i_s_frame.l3_pdu, 0, gllc_assy_info.fr_info_length );

      gllc_ass_frame_info_fcs( &l3_pdu_temp, gllc_assy_info.fr_info_length );
    }
    ...
    (
      &gllc_assy_info.grr_ul_mes -> pdu_ptr,
      &l3_pdu_temp
    );
    break;
  ...
  case GLLC_S_FORMAT:
    ...
    gllc_assy_info.fr_info_length = 0;
    gllc_ass_frame_info_fcs( &l3_pdu_temp, gllc_assy_info.fr_info_length );
  ...
  case GLLC_UI_FORMAT:
    ...
    {
      fcs_length = gllc_n202_lookup[ gllc_llme.llc_version ];

      if ( fcs_length > gllc_assy_info.fr_info_length )
        fcs_length = gllc_assy_info.fr_info_length;
    }
    else
      fcs_length = gllc_assy_info.fr_info_length;
    ...
    if ( frame_p -> ui_frame.encrypted_E )
    {
      ...
      (
        (
          gllc_llme.input_offset_IOV_UI
          ^
          (
            (
              gllc_sapi_addr_lookup[gllc_assy_info.lle] *   /* SAPI */
              0x08000000UL                                  /* 2^27 */
            )
            +
            0x80000000UL                                    /* 2^31 */
      ...
      gprs_pdu_augment_tail(
        &gllc_assy_info.grr_ul_mes -> pdu_ptr, &l3_pdu_temp );
    ...
    else
    {
      ...
      gllc_ass_frame_info_fcs( &frame_p -> ui_frame.l3_pdu, fcs_length );
...
/*lint -restore*/
        ...
        if ( xid_info_p -> contents & GLLC_LAY3_XID    )
        {
          ...
          l3_pdu_temp = gprs_pdu_construct( buf, (uint16)(bufp - buf) );
        ...
        else
        {
          ...
          l3_pdu_temp = gprs_pdu_construct( buf, (uint16)(bufp - buf) );
          gllc_assy_info.fr_info_length = (uint16)(bufp - buf);
        }
      }
    }
    ...
    else if ( frame_p -> u_frame.cmd_res_M4_M1 == GLLC_FRMR )
    {
      ...
      l3_pdu_temp = gprs_pdu_construct( buf, 10 );
      gllc_assy_info.fr_info_length = 10;
    }
    ...
    gllc_ass_frame_info_fcs( &l3_pdu_temp, gllc_assy_info.fr_info_length );
    gprs_pdu_augment_tail( &gllc_assy_info.grr_ul_mes -> pdu_ptr, &l3_pdu_temp );
    break;
  ...
  default:
    ...
    MSG_GERAN_ERROR_1_G( "GLC GRR Ass. Info: Invalid fr fmt: %d", frame_p -> format);
    break;
...
void gllc_disassemble_frame_hdr( void )
{
  ...
  ll_pdu_ptr = &gllc_disassy_info.grr_dl_mes -> ll_pdu;
  ...
  if ( !gprs_pdu_strip_head( ll_pdu_ptr, &buf[0], 2 ) )
    gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_SHORT_ERR;

  /* Prepare a frame reject ctrl field block in case of a FRMR condition. */

  gllc_disassy_info.rej_fr_ctrl_field[0] = buf[1];  /* Exclude addr field. */

  /* Disassemble frame header. */

  gllc_disassy_info.lle = LLC_FRAME_GET_SAPI(buf);
  ...
  gllc_disassy_info.frame.format = LLC_FRAME_GET_FORMAT( buf );
  ...
  case GLLC_I_S_FORMAT:           /* Information plus supervisory format.  */
    ...
    gllc_disassy_info.rej_fr_ctrl_field[1] = buf[2];/* Exclude addr field. */
    gllc_disassy_info.rej_fr_ctrl_field[2] = buf[3];

    if ( LLC_FRAME_GET_CR( buf ) == 1 )
      gllc_disassy_info.frame.i_s_frame.cmd_or_res_CR = GLLC_COMMAND;
    else
      gllc_disassy_info.frame.i_s_frame.cmd_or_res_CR = GLLC_RESPONSE;

    gllc_disassy_info.rejected_frame_CR =
      gllc_disassy_info.frame.i_s_frame.cmd_or_res_CR; /* In case of FRMR. */

    gllc_disassy_info.frame.i_s_frame.ack_req_A =
      LLC_I_S_FRAME_GET_A( buf );
    gllc_disassy_info.frame.i_s_frame.send_seq_num_N_S =
      LLC_I_S_FRAME_GET_N_S( buf );
    gllc_disassy_info.frame.i_s_frame.recv_seq_num_N_R =
      LLC_I_S_FRAME_GET_N_R( buf );
    gllc_disassy_info.frame.i_s_frame.cmd_res_S1S2 =
      LLC_I_S_FRAME_GET_S1S2( buf );

    /* Extract any SACK bitmap that may be present and calculate partial
       FCS over the header. */

    if ( gllc_disassy_info.frame.i_s_frame.cmd_res_S1S2 == GLLC_SACK )
    {
      ...
      gllc_disassy_info.rej_fr_ctrl_field[3] = buf[4];
      gllc_disassy_info.frame.i_s_frame.sack_bitmap_len_K =
      ...
      (
        ...
        (
          ll_pdu_ptr, &buf[5],
          (uint16)(gllc_disassy_info.frame.i_s_frame.sack_bitmap_len_K + 1)
        )
      )
      {
        gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_SHORT_ERR;
      ...
      gllc_disassy_info.rej_fr_ctrl_field[4] = buf[5];
      gllc_disassy_info.rej_fr_ctrl_field[5] = buf[6];

      /* Copy the SACK bitmap to it's intended destination. */

      (void)memscpy
      (
        gllc_disassy_info.frame.i_s_frame.sack_bitmap_R,
        sizeof(gllc_disassy_info.frame.i_s_frame.sack_bitmap_R),
        &buf[5],
        gllc_disassy_info.frame.i_s_frame.sack_bitmap_len_K + 1
      );

      gllc_dis_frame_header_fcs
      (
        buf,
        (uint16)(5 + gllc_disassy_info.frame.i_s_frame.sack_bitmap_len_K + 1)
      );
    }
    else
      gllc_dis_frame_header_fcs( buf, 4 );
    break;
  ...
  case GLLC_S_FORMAT:            /* Supervisory format.                    */
    ...
    if ( !gprs_pdu_strip_head( ll_pdu_ptr, &buf[2], 1 ) )
    {
      ...
      gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FRMR_ERR;
      gllc_disassy_info.fr_dis_err_map |= GLLC_FRMR_W1_INVALID_I_FIELD;
      gllc_disassy_info.fr_dis_err_map |= GLLC_FRMR_W3_INVALID_CTRL_FIELD;
    }

    gllc_disassy_info.rej_fr_ctrl_field[1] = buf[2];/* Exclude addr field. */

    if ( LLC_FRAME_GET_CR( buf ) == 1 )
      gllc_disassy_info.frame.s_frame.cmd_or_res_CR = GLLC_COMMAND;
    else
      gllc_disassy_info.frame.s_frame.cmd_or_res_CR = GLLC_RESPONSE;

    gllc_disassy_info.rejected_frame_CR =
      gllc_disassy_info.frame.s_frame.cmd_or_res_CR; /* In case of FRMR. */

    gllc_disassy_info.frame.s_frame.ack_req_A =
      LLC_S_FRAME_GET_A( buf );
    gllc_disassy_info.frame.s_frame.recv_seq_num_N_R =
      LLC_S_FRAME_GET_N_R( buf );
    gllc_disassy_info.frame.s_frame.cmd_res_S1S2 =
      LLC_S_FRAME_GET_S1S2( buf );

    /* Extract any SACK bitmap that may be present and calculate partial
       FCS over the header. */

    if ( gllc_disassy_info.frame.s_frame.cmd_res_S1S2 == GLLC_SACK )
    {
      /* A selective acknowledgement bitmap is attached to the header so
      ...
      if ( temp < (1 + 3) )
      {
        ...
        gllc_disassy_info.frame.s_frame.sack_bitmap_len_K = 0;
      }
      else if ( temp > (GLLC_SACK_BMP_LEN + 3) )
      {
        /* Supervisory frame has an incorrect length - too long. */

        gllc_disassy_info.frame.s_frame.sack_bitmap_len_K =
          GLLC_SACK_BMP_LEN - 1;
      ...
      else
      {
        ...
        gllc_disassy_info.frame.s_frame.sack_bitmap_len_K = (uint8)(temp - 3 - 1);
      ...
      (void)memscpy
      (
        gllc_disassy_info.frame.s_frame.sack_bitmap_R,
        sizeof(gllc_disassy_info.frame.s_frame.sack_bitmap_R),
        &buf[3],
        (gllc_disassy_info.frame.s_frame.sack_bitmap_len_K + 1)
      );

      gllc_dis_frame_header_fcs
      (
        buf,
        (uint16)(3 + gllc_disassy_info.frame.s_frame.sack_bitmap_len_K + 1)
      );
    }
    else
      gllc_dis_frame_header_fcs( buf, 3 );
    break;
  ...
  case GLLC_UI_FORMAT:           /* Unconfirmed information format.        */
    ...
    if ( LLC_FRAME_GET_CR( buf ) == 1 )
      gllc_disassy_info.frame.ui_frame.cmd_or_res_CR = GLLC_COMMAND;
    else
      gllc_disassy_info.frame.ui_frame.cmd_or_res_CR = GLLC_RESPONSE;

    gllc_disassy_info.rejected_frame_CR =
      gllc_disassy_info.frame.ui_frame.cmd_or_res_CR; /* In case of FRMR. */

    gllc_disassy_info.frame.ui_frame.send_seq_num_N_U  =
      LLC_UI_FRAME_GET_N_U( buf );
    gllc_disassy_info.frame.ui_frame.encrypted_E =
      LLC_UI_FRAME_GET_E( buf );
    gllc_disassy_info.frame.ui_frame.protected_mode_PM =
  ...
  case GLLC_U_FORMAT:            /* Unnumbered format.                     */
    ...
    if ( LLC_FRAME_GET_CR( buf ) == 1 )
      gllc_disassy_info.frame.u_frame.cmd_or_res_CR = GLLC_COMMAND;
    else
      gllc_disassy_info.frame.u_frame.cmd_or_res_CR = GLLC_RESPONSE;

    gllc_disassy_info.rejected_frame_CR =
      gllc_disassy_info.frame.u_frame.cmd_or_res_CR; /* In case of FRMR. */

    gllc_disassy_info.frame.u_frame.poll_final_PF =
      LLC_U_FRAME_GET_PF( buf );
    gllc_disassy_info.frame.u_frame.cmd_res_M4_M1 =
      LLC_U_FRAME_GET_M4_M1( buf );

    /* Verify the validity of the U frame control field and flag a frame
       reject condition if invalid. */

    if ( !gllc_accept_u_frame_ctrl_field(&gllc_disassy_info.frame.u_frame) )
    {
      gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FRMR_ERR;
      gllc_disassy_info.fr_dis_err_map |= GLLC_FRMR_W3_INVALID_CTRL_FIELD;
    }

    gllc_dis_frame_header_fcs( buf, 2 );
    break;
...
void gllc_disassemble_frame_info_and_fcs( void )
{
  ...
  if ( temp >= 3 )
  {
    gllc_disassy_info.fr_info_length = temp - 3;
    ...
    temp = gllc_disassy_info.fr_info_length;

    if ( temp > sizeof(gllc_disassy_info.fr_info_debug_sample) )
      temp = sizeof(gllc_disassy_info.fr_info_debug_sample);

    ll_pdu_dup_ptr = gprs_pdu_duplicate( *ll_pdu_ptr, 0, temp );
    (void)gprs_pdu_strip_head(
      &ll_pdu_dup_ptr, gllc_disassy_info.fr_info_debug_sample, temp );
  }
  else
  {
    /* Insufficient FCS octets. No point continuing with this frame. */

    gllc_disassy_info.fr_info_length  = 0;
    gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_SHORT_ERR;
    return;                                                  /*** RETURN ***/
  }
  ...
  if ( GLLC_LLE_IS_3_5_9_11( gllc_disassy_info.lle )) 
  {
    if ( !(gllc_lle[gllc_disassy_info.lle].gen.pdp_active  ) &&
          (gllc_disassy_info.frame.format == GLLC_UI_FORMAT)    )
    {
      gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_ADDR_ERR;
      return;                                                /*** RETURN ***/
    }
  ...
  case GLLC_I_S_FORMAT:           /* Information plus supervisory format.  */
    ...
    if ( (gllc_llme.gea != GMMLLC_GEA_NONE     ) &&
         (gllc_llme.gea != GMMLLC_GEA_UNDEFINED)    )
    {
      decipher_oc = gllc_lle[ gllc_disassy_info.lle ].ack.decipher_oc;
      ...
      if ( gllc_disassy_info.frame.i_s_frame.send_seq_num_N_S <
           gllc_lle[ gllc_disassy_info.lle ].ack.recv_state_var_V_R )
      {
      ...
      (uint32)
      (
        gllc_lle[ gllc_disassy_info.lle ].ack.input_offset_IOV_I
        +
        gllc_disassy_info.frame.i_s_frame.send_seq_num_N_S       /* LFN */
        +
        decipher_oc                                              /* OC  */
      );
      ...
      (
        ...
        (
          ll_pdu_ptr, gllc_disassy_info.fr_info_length,
          gllc_disassy_info.fr_info_length, gea_input_seed
        )
      )
      {
    ...
    (
      !gllc_dis_frame_info_fcs(ll_pdu_ptr, gllc_disassy_info.fr_info_length )
    )
    {
      gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FCS_ERR;
    }
    ...
    if ( !gllc_disassy_info.fr_dis_err_map )
    {
      ...
      uint16 max_info_octets_N201_I =
        gllc_lle[ gllc_disassy_info.lle ].ack.max_info_octets_N201_I;
#endif
      ...
      if ( gllc_lle[ gllc_disassy_info.lle ].unnum.xid_pending &
           GLLC_N201_I_XID                                       )
      {
        if
        ( 
           gllc_lle[ gllc_disassy_info.lle ].unnum.xid_req.n201_i_xid >
      ...
      if ( gllc_disassy_info.fr_info_length > max_info_octets_N201_I )
      {
        gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FRMR_ERR;
        gllc_disassy_info.fr_dis_err_map |= GLLC_FRMR_W2_I_FIELD_LENGTH_ERR;
      }
      else if ( gllc_disassy_info.fr_info_length == 0 )
        gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_SHORT_ERR;
    }
    ...
    if ( !gllc_disassy_info.fr_dis_err_map )
    {
      if ( *ll_pdu_ptr != NULL )
      {
        gllc_disassy_info.frame.i_s_frame.l3_pdu  = *ll_pdu_ptr;
        *ll_pdu_ptr                               = NULL;
      }
    }
    break;
  ...
  case GLLC_UI_FORMAT:           /* Unconfirmed information format.        */
    ...
    fcs_length = gllc_disassy_info.fr_info_length;

    if ( (gllc_disassy_info.frame.format == GLLC_UI_FORMAT  ) &&
        !(gllc_disassy_info.frame.ui_frame.protected_mode_PM)    )
    {
      if ( fcs_length > gllc_n202_lookup[gllc_llme.llc_version] )
        fcs_length = gllc_n202_lookup[gllc_llme.llc_version];
    }
         ...
         (gllc_disassy_info.frame.ui_frame.encrypted_E)    )
    {
      ...
      (
        ( gllc_disassy_info.frame.ui_frame.send_seq_num_N_U !=
          gllc_lle[ gllc_disassy_info.lle ].unack.recv_state_var_V_UR )
        ||
        ( gllc_lle[ gllc_disassy_info.lle ].unack.recv_state_var_V_UR == 0 )
      )
      {
      ...
      decipher_oc = gllc_lle[ gllc_disassy_info.lle ].unack.decipher_oc;
      ...
      do
      {
        ...
        (
          (
            ...
            (
              (
                gllc_sapi_addr_lookup[gllc_disassy_info.lle] *    /* SAPI */
                0x08000000UL                                      /* 2^27 */
              )
              +
              0x80000000UL                                        /* 2^31 */
          ...
          +
          gllc_disassy_info.frame.ui_frame.send_seq_num_N_U       /* LFN */
          +
          decipher_oc                                             /* OC  */
        );
          ...
          if ( decipher_reattempt_countdown != 0 )
          {
            ...
            *ll_pdu_ptr = gprs_pdu_copy( ll_pdu_copy_ptr );
            ...
            (
              ...
              gllc_sapi_addr_lookup[ gllc_disassy_info.lle ],
              gllc_disassy_info.frame.ui_frame.send_seq_num_N_U,
              decipher_oc
            ); 
          }
        ...
        else
        {
          ...
          if ( decipher_oc != gllc_lle[ gllc_disassy_info.lle ].unack.decipher_oc )
          {
            MSG_GERAN_HIGH_3_G
            (
              "GLC LL %d: Info - Decipher OC Update %d to %d",
              gllc_sapi_addr_lookup[ gllc_disassy_info.lle ],
              gllc_lle[ gllc_disassy_info.lle ].unack.decipher_oc,
              decipher_oc
            ); 

            gllc_lle[ gllc_disassy_info.lle ].unack.decipher_oc = decipher_oc;
            gllc_debug_print_lle_cipher_params( gllc_disassy_info.lle );
          }
          break;
        }
      }
    ...
    else if ( !gllc_dis_frame_info_fcs( ll_pdu_ptr, fcs_length ) )
    {
      gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FCS_ERR;
    }
    ...
    if ( !gllc_disassy_info.fr_dis_err_map )
    {
      ...
      uint16 max_info_octets_N201_U =
        gllc_lle[ gllc_disassy_info.lle ].unack.max_info_octets_N201_U;
#endif
      ...
      if ( gllc_lle[ gllc_disassy_info.lle ].unnum.xid_pending & GLLC_N201_U_XID )
      {
        if
        ( 
           gllc_lle[ gllc_disassy_info.lle ].unnum.xid_req.n201_u_xid >
           max_info_octets_N201_U
        )
        {
          max_info_octets_N201_U = gllc_lle[ gllc_disassy_info.lle ].unnum.xid_req.n201_u_xid;
        }
      }
      
      if ( gllc_disassy_info.fr_info_length > max_info_octets_N201_U )
      {
        gllc_disassy_info.fr_dis_err_map |= GLLC_DIS_FRMR_ERR;
        gllc_disassy_info.fr_dis_err_map |= GLLC_FRMR_W2_I_FIELD_LENGTH_ERR;
      }
    ...
    if ( !gllc_disassy_info.fr_dis_err_map )
    {
      if ( *ll_pdu_ptr != NULL )
      {
        gllc_disassy_info.frame.ui_frame.l3_pdu = *ll_pdu_ptr;
        *ll_pdu_ptr                             = NULL;
      }
    }
    break;
  ...
  case GLLC_U_FORMAT:            /* Unnumbered format.                     */
    ...
    xid_info_p = &gllc_disassy_info.frame.u_frame.info_field.xid_info;

    gllc_disassy_info.frame.u_frame.xid = GLLC_XID_NONE;
    ...
    (
      (gllc_disassy_info.frame.u_frame.cmd_res_M4_M1 == GLLC_SABM) ||
      (gllc_disassy_info.frame.u_frame.cmd_res_M4_M1 == GLLC_UA  ) ||
      (gllc_disassy_info.frame.u_frame.cmd_res_M4_M1 == GLLC_XID )
    )
    {
      /* These commands/responses may carry an XID parameter negotiation
         information field. */

      if ( gllc_disassy_info.frame.u_frame.cmd_or_res_CR == GLLC_COMMAND )
        gllc_disassy_info.frame.u_frame.xid = GLLC_XID_REQ;
      else
        gllc_disassy_info.frame.u_frame.xid = GLLC_XID_NEG;
          ...
          if ( xid_type != GLLC_LAY3_XID_TYPE )
          {
            ...
            case GLLC_VERSION_XID_TYPE:
              ...
              if ( !(xid_info_p -> contents & GLLC_VERSION_XID) )
              {
                ...
                else if ( gllc_disassy_info.frame.u_frame.xid == GLLC_XID_NEG )
            ...
            default:
              ...
              if ( gllc_disassy_info.frame.u_frame.xid == GLLC_XID_NEG )
    ...
    else if ( gllc_disassy_info.frame.u_frame.cmd_res_M4_M1 == GLLC_FRMR )
    {
      ...
      else
      {
        ...
        (void)memscpy
        (
          gllc_disassy_info.frame.u_frame.info_field.frmr_info.rej_fr_ctrl_field,
          sizeof(gllc_disassy_info.frame.u_frame.info_field.frmr_info.rej_fr_ctrl_field),
          buf,
          6
        );
        ...
        gllc_disassy_info.frame.u_frame.info_field.frmr_info.send_state_var_V_S =
          LLC_U_FRAME_GET_FRMR_V_S( buf );
        gllc_disassy_info.frame.u_frame.info_field.frmr_info.recv_state_var_V_R =
          LLC_U_FRAME_GET_FRMR_V_R( buf );
        gllc_disassy_info.frame.u_frame.info_field.frmr_info.rejected_frame_CR =
          LLC_U_FRAME_GET_FRMR_CR( buf );
        gllc_disassy_info.frame.u_frame.info_field.frmr_info.reject_status_W4_W1 =
