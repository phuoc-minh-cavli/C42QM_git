...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gllc/src/gllclient.c#5 $
***
...
LOCAL void gllcmm_ll_sig_put_mes_buf( gprs_ll_dl_sig_mes_t *mes_ptr )
{
  ...
  default:
    MSG_GERAN_LOW_1("GLLGMM CLIENT: Filtered Msg Id %d",
      mes_ptr -> mes.prim);
...
#error code not present
    ...
    MSG_GERAN_LOW_1("GLLGMM CLIENT: Filtered Msg Id %d", mes_ptr -> mes.prim);
  ...
  )
  {
    if( cmd_ptr->cmd.ll_unitdata_ind.l3_pdu[1]  == GLLC_EGPRS_BLOCK_LOOPBACK_CMD )
    {
     /* EGPRS Switched Radio Block Loopback Mode command */
      ...
      MSG_GERAN_HIGH_2("GLLGMM CLIENT: EGPRS_BLOCK_LOOPBACK_CMD  M=%d ts_offset=%d Intercept", mode, ts_offset);
    ...
    else if( cmd_ptr->cmd.ll_unitdata_ind.l3_pdu[1]  == GLLC_GPRS_TEST_MODE_CMD )
    {
      ...
      MSG_GERAN_HIGH_2("GLLGMM CLIENT: GPRS_TEST_MODE_CMD M=%d ts_offset=%d Intercept", mode, ts_offset);
...
LOCAL void gllcsms_ll_sig_put_mes_buf( gprs_ll_dl_sig_mes_t *mes_ptr )
{
  ...
  default:
    MSG_GERAN_LOW_1("GLLSMS CLIENT: Filtered Msg Id %d",
      mes_ptr -> mes.prim);
...
#error code not present
  ...
  if ( mes_ptr -> mes.prim == LL_UNITDATA_IND )
  {
    ...
    MSG_GERAN_LOW_1("GLSMS CLIENT: Filtered Msg Id %d", mes_ptr -> mes.prim);
...
void gmmllc_ll_put_mes_dyn(gmmllc_ll_mes_type* mes_ptr)
{
  ...
  case GMMLLC_LL_UNITDATA_REQ:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GMMLLC_LL_UNITDATA_REQ,
    ...
    if ( dat_mes_ptr != NULL )
    {
      ...
      dat_mes_ptr -> mes.unitdata_req_mes.l3_pdu =
        gprs_pdu_construct( mes_ptr -> mes.ll_unitdata_req.l3_pdu,
                            mes_ptr -> mes.ll_unitdata_req.l3_pdu_len );

      GPRS_MEM_FREE( mes_ptr -> mes.ll_unitdata_req.l3_pdu );

      gllc_ll_ul_dat_put_mes_buf_dyn( dat_mes_ptr);
    }
    else
  ...
  case GMMLLC_LL_XID_REQ:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GMMLLC_LL_XID_REQ,
      sizeof( gmmllc_ll_xid_req_type ),
      (uint8 *)&mes_ptr -> mes.ll_xid_req
    );

    sig_mes_ptr = gllc_ll_ul_sig_get_mes_buf_dyn();
    ...
    if ( sig_mes_ptr != NULL )
    {
      ...
      sig_mes_ptr -> mes.xid_req_mes.l3_xid_req =
        gprs_pdu_construct( mes_ptr -> mes.ll_xid_req.l3_xid_req,
                            mes_ptr -> mes.ll_xid_req.l3_xid_req_len );

      GPRS_MEM_FREE(  mes_ptr -> mes.ll_xid_req.l3_xid_req );

      gllc_ll_ul_sig_put_mes_buf_dyn( sig_mes_ptr);
    }
    else
      ERR_GERAN_FATAL_0_G( "GLLGMM CLIENT: Heap Exhaustion.");
    break;
  ...
  case GMMLLC_LL_XID_RES:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GMMLLC_LL_XID_RES,
    ...
    if ( sig_mes_ptr != NULL )
    {
      ...
      sig_mes_ptr -> mes.xid_res_mes.l3_xid_neg =
        gprs_pdu_construct( mes_ptr -> mes.ll_xid_res.l3_xid_neg,
  ...
  default:
    ...
    MSG_GERAN_LOW_1("GLLGMM CLIENT: Invalid Msg Id %d", mes_ptr -> mes.prim);
...
void gsmsllc_ll_put_mes_dyn(gsmsllc_ll_mes_type* mes_ptr)
{
  gprs_ll_ul_sig_mes_t *sig_mes_ptr;
  gprs_ll_ul_dat_mes_t *dat_mes_ptr;
  ...
  case GSMSLLC_LL_UNITDATA_REQ:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GSMSLLC_LL_UNITDATA_REQ,
  ...
  case GSMSLLC_LL_XID_REQ:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GSMSLLC_LL_XID_REQ,
  ...
  case GSMSLLC_LL_XID_RES:
    ...
    cfa_log_packet_ex
    (
      gllc_gs_queue, MS_LLC_GMM, (uint8)GSMSLLC_LL_XID_RES,
  ...
  default:
    ...
    MSG_GERAN_LOW_1("GLLSMS CLIENT: Invalid Msg Id %d", mes_ptr -> mes.prim);
...
void gllclient_reg_gmm_gsms_proxy(void)
{
...
boolean gllclient_gea_classmark_enquiry_dyn
(
...
)
{
  ...
  return ( gllc_is_gea_supported(  gea ) );
} /* end of gllclient_gea_classmark_enquiry_dyn() */
...
boolean gllclient_gea_classmark_enquiry_per_subs_dyn
(
...
{
  return gllc_is_gea_supported( gea );
} /* end of gllclient_gea_classmark_enquiry_per_subs_dyn() */
