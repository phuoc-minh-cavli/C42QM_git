...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gllc/src/gllcxid.c#5 $   
***
...
LOCAL gllc_llme_gan_state_t gllc_lle_last_gan_state[GLLC_LL_NUM_SAPIS] = {GLLC_LLME_GAN_INACTIVE};
...
void gllc_llme_reset_l2_xid_parameters(void)
{
  gllc_llme.llc_version = GLLC_DFLT_VERSION;
...
void gllc_llme_reset_IOV_xid_parameters(void)
{
  uint32 lle;
  
  MSG_GERAN_LOW_2_G( "GLC ME: Info IOV-UI/IOV-I Default Reset", 0 ,0 );
  
  gllc_llme.input_offset_IOV_UI = GLLC_DFLT_IOV_UI;
  gllc_diag_rpt_llme_state();

  for ( lle = 0; lle <= (uint32)GLLC_LL_LAST_SAPI; lle++ )
  {
    gllc_lle[lle].ack.input_offset_IOV_I = gllc_dflt_IOV_I[lle];
...
void gllc_lle_reset_l2_xid_parameters( gllc_ll_sapi_t lle )
{
  (void)memset( &gllc_lle[lle].unnum.xid_req, 0, sizeof(gllc_xid_info_t) );
  ...
  gllc_lle[lle].gen.retrx_interval_T200 = gllc_dflt_T200_ms[lle];
  gllc_lle[lle].ack.retrx_interval_T201 = gllc_dflt_T200_ms[lle];

  if ( gllc_dflt_T200_ms[lle] != gllc_prfd_T200_ms[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_T200_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].gen.max_retrx_cnt_N200 = gllc_dflt_N200[lle];

  if ( gllc_dflt_N200[lle] != gllc_prfd_N200[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_N200_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].unack.max_info_octets_N201_U = gllc_dflt_N201_U_octets[lle];

  if ( gllc_dflt_N201_U_octets[lle] != 
         gllc_prfd_N201_U_octets[gllc_llme.gan_state][lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_N201_U_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].ack.max_info_octets_N201_I = gllc_dflt_N201_I_octets[lle];

  if ( gllc_dflt_N201_I_octets[lle] != 
         gllc_prfd_N201_I_octets[gllc_llme.gan_state][lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_N201_I_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].ack.max_reseq_buf_octets_mD =
    (uint16)(gllc_dflt_mD_octet16[lle] * 16);

  if ( gllc_dflt_mD_octet16[lle] != gllc_prfd_mD_octet16[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_mD_XID;  
  ...
  if ( gllc_dflt_mU_octet16[lle] != gllc_prfd_mU_octet16[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_mU_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].ack.max_reseq_buf_frames_kD = gllc_dflt_kD_frames[lle];

  if ( gllc_dflt_kD_frames[lle] != gllc_prfd_kD_frames[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_kD_XID;  

  /* --------------------------------------------------------------------- */

  gllc_lle[lle].ack.max_retrx_buf_frames_kU = gllc_dflt_kU_frames[lle];

  if ( gllc_dflt_kU_frames[lle] != gllc_prfd_kU_frames[lle] )
    gllc_lle[lle].unnum.xid_pending |= GLLC_kU_XID;  

  /* --------------------------------------------------------------------- */

  gllc_diag_rpt_lle_xid( lle );
...
)
{
  ...
  else if ( u_frame -> xid == GLLC_XID_NEG )
  {
    ...
    if ( xid_neg -> contents & GLLC_VERSION_XID )
    {
      ...
      if ( (gllc_lle[lle].state    == GLLC_LLE_ABM         ) &&
           (xid_neg -> version_xid != gllc_llme.llc_version)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_VERSION_XID) &&
           (xid_neg -> version_xid >
            gllc_lle[lle].unnum.xid_req.version_xid                )    )
      {
        xid_neg_valid = FALSE;
      }
    ...
    if ( xid_neg -> contents & GLLC_T200_XID )
    {
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_T200_XID      ) &&
           (xid_neg -> t200_xid < gllc_lle[lle].unnum.xid_req.t200_xid)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if (!(gllc_lle[lle].unnum.xid_req.contents & GLLC_T200_XID) &&
           (xid_neg -> t200_xid <
            gllc_lle[lle].gen.retrx_interval_T200 / 100         )    )
      {
        xid_neg_valid = FALSE;
      }
    }
    ...
    if ( xid_neg -> contents & GLLC_N200_XID )
    {
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_N200_XID      ) &&
           (xid_neg -> n200_xid < gllc_lle[lle].unnum.xid_req.n200_xid)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if (!(gllc_lle[lle].unnum.xid_req.contents & GLLC_N200_XID      ) &&
           (xid_neg -> n200_xid < gllc_lle[lle].gen.max_retrx_cnt_N200)    )
      {
        xid_neg_valid = FALSE;
      }
    }
    ...
    if ( xid_neg -> contents & GLLC_N201_U_XID )
    {
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_U_XID) &&
           (xid_neg -> n201_u_xid >
            gllc_lle[lle].unnum.xid_req.n201_u_xid                )    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if (!(gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_U_XID) &&
           (xid_neg -> n201_u_xid >
            gllc_lle[lle].unack.max_info_octets_N201_U            )    )
      {
        xid_neg_valid = FALSE;
      }
    }
    ...
    if ( xid_neg -> contents & GLLC_N201_I_XID )
    {
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_I_XID) && 
           (xid_neg -> n201_i_xid >
            gllc_lle[lle].unnum.xid_req.n201_i_xid                )    )
      {
        xid_neg_valid = FALSE;
      }
    ...
    if ( xid_neg -> contents & GLLC_mD_XID )
    {
      ...
      if ( (gllc_lle[lle].state == GLLC_LLE_ABM           ) &&
           (xid_neg -> md_xid <
            gllc_lle[lle].ack.max_reseq_buf_octets_mD / 16)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_mD_XID    ) &&
           (xid_neg -> md_xid > gllc_lle[lle].unnum.xid_req.md_xid)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      {      
        ...
        if (!(xid_neg -> contents & GLLC_N201_I_XID        ) &&
             (xid_neg -> md_xid <
              gllc_lle[lle].ack.max_info_octets_N201_I / 16)    )
        {
          xid_neg_valid = FALSE;
        }
      }
    ...
    if ( xid_neg -> contents & GLLC_mU_XID )
    {
      ...
      if ( (gllc_lle[lle].state == GLLC_LLE_ABM           ) &&
           (xid_neg -> mu_xid <
            gllc_lle[lle].ack.max_retrx_buf_octets_mU / 16)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_mU_XID    ) &&
           (xid_neg -> mu_xid > gllc_lle[lle].unnum.xid_req.mu_xid)    )
      {
        xid_neg_valid = FALSE;
      }
    ...
    if ( xid_neg -> contents & GLLC_kD_XID )
    {
      ...
      if ( (gllc_lle[lle].state == GLLC_LLE_ABM      ) &&
           (xid_neg -> kd_xid <
            gllc_lle[lle].ack.max_reseq_buf_frames_kD)    )
      {
        xid_neg_valid = FALSE;
      }
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_kD_XID    ) &&
           (xid_neg -> kd_xid > gllc_lle[lle].unnum.xid_req.kd_xid)    )
      {
        xid_neg_valid = FALSE;
      }
    ...
    if ( xid_neg -> contents & GLLC_kU_XID )
    {
      ...
      if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_kU_XID    ) &&
           (xid_neg -> ku_xid > gllc_lle[lle].unnum.xid_req.ku_xid)    )
      {
        xid_neg_valid = FALSE;
      }
...
)
{
  ...
  {
    gllc_lle[lle].unnum.xid_req = u_frame -> info_field.xid_info;

    /* Strip out any layer 3 related information. */

    gllc_lle[lle].unnum.xid_req.contents      &= ~GLLC_LAY3_XID;
    gllc_lle[lle].unnum.xid_req.l3_xid.length  = 0;
    gllc_lle[lle].unnum.xid_req.l3_xid.params  = NULL;
  }
  else
    (void)memset( &gllc_lle[lle].unnum.xid_req, 0, sizeof(gllc_xid_info_t) );
...
)
{
  ...
  {
    gllc_lle[lle].unnum.xid_neg = u_frame -> info_field.xid_info;

    /* Strip out any layer 3 related information. */

    gllc_lle[lle].unnum.xid_neg.contents      &= ~GLLC_LAY3_XID;
    gllc_lle[lle].unnum.xid_neg.l3_xid.length  = 0;
    gllc_lle[lle].unnum.xid_neg.l3_xid.params  = NULL;
    ...
    gllc_lle[lle].unnum.xid_pending |= gllc_lle[lle].unnum.xid_neg.contents;
  }
  else
    (void)memset( &gllc_lle[lle].unnum.xid_neg, 0, sizeof(gllc_xid_info_t) );
...
boolean gllc_lle_generate_l2_xid_req( gllc_ll_sapi_t lle )
{
  /* --------------------------------------------------------------------- */
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_VERSION_XID )
  {
    /* Can't negotiate LLC Version in the ABM state. */

    if ( gllc_lle[lle].state != GLLC_LLE_ABM )
    {
      gllc_lle[lle].unnum.xid_req.contents |= GLLC_VERSION_XID;

      /* If current value is equal to the default value, then propose
         preferred value, else propose the last successfully negotiated
         value. */
     
      if ( gllc_llme.llc_version == GLLC_DFLT_VERSION )
        gllc_lle[lle].unnum.xid_req.version_xid = GLLC_PRFD_VERSION;
      else
        gllc_lle[lle].unnum.xid_req.version_xid = gllc_llme.llc_version;
    }
  }

  /* --------------------------------------------------------------------- */

  if ( gllc_lle[lle].unnum.xid_pending & GLLC_T200_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_T200_XID;
    ...
    if ( gllc_lle[lle].gen.retrx_interval_T200 == gllc_dflt_T200_ms[lle] )
      gllc_lle[lle].unnum.xid_req.t200_xid =
        (uint16)(gllc_prfd_T200_ms[lle] / 100);
    else
      gllc_lle[lle].unnum.xid_req.t200_xid =
        (uint16)(gllc_lle[lle].gen.retrx_interval_T200 / 100);
  }

  /* --------------------------------------------------------------------- */

  if ( gllc_lle[lle].unnum.xid_pending & GLLC_N200_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_N200_XID;

    /* If current value is equal to the default value, then propose preferred
       value, else propose the last successfully negotiated value. */

    if ( gllc_lle[lle].gen.max_retrx_cnt_N200 == gllc_dflt_N200[lle] )
      gllc_lle[lle].unnum.xid_req.n200_xid = gllc_prfd_N200[lle];
    else
      gllc_lle[lle].unnum.xid_req.n200_xid =
       (uint8)gllc_lle[lle].gen.max_retrx_cnt_N200;
  }

  /* --------------------------------------------------------------------- */

  if ( gllc_lle[lle].unnum.xid_pending & GLLC_N201_U_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_N201_U_XID;
    ...
    if ( gllc_lle[lle].unack.max_info_octets_N201_U ==
         gllc_dflt_N201_U_octets[lle]                  )
    {
      gllc_lle[lle].unnum.xid_req.n201_u_xid = 
        gllc_prfd_N201_U_octets[gllc_llme.gan_state][lle];
    }
    else if ( gllc_llme.gan_state != gllc_lle_last_gan_state[lle] )
    {
      gllc_lle[lle].unnum.xid_req.n201_u_xid = 
        gllc_prfd_N201_U_octets[gllc_llme.gan_state][lle];
    }
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_N201_I_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_N201_I_XID;
    ...
    if ( gllc_lle[lle].ack.max_info_octets_N201_I ==
    ...
    if ( gllc_lle[lle].state == GLLC_LLE_ABM )
    {
      ...
      if ( gllc_lle[lle].unnum.xid_req.n201_i_xid <
           gllc_lle[lle].ack.max_info_octets_N201_I )
      {
        gllc_lle[lle].unnum.xid_req.n201_i_xid =
          gllc_lle[lle].ack.max_info_octets_N201_I;
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_mD_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_mD_XID;

    /* If current value is equal to the default value, then propose preferred
       value, else propose the last successfully negotiated value. */
    
    if ( (gllc_lle[lle].ack.max_reseq_buf_octets_mD / 16) ==
         gllc_dflt_mD_octet16[lle]                           )
    {
      gllc_lle[lle].unnum.xid_req.md_xid = gllc_prfd_mD_octet16[lle];
      ...
      if ( gllc_lle[lle].state == GLLC_LLE_ABM )
      {
        ...
        if ( gllc_lle[lle].unnum.xid_req.md_xid <
             gllc_lle[lle].ack.max_reseq_buf_octets_mD / 16 )
        {
          gllc_lle[lle].unnum.xid_req.md_xid =
            gllc_lle[lle].ack.max_reseq_buf_octets_mD / 16;
        }
      }
    }
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_mU_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_mU_XID;

    /* If current value is equal to the default value, then propose preferred
       value, else propose the last successfully negotiated value. */
    
    if ( (gllc_lle[lle].ack.max_retrx_buf_octets_mU / 16) ==
         gllc_dflt_mU_octet16[lle]                           )
    {
      gllc_lle[lle].unnum.xid_req.mu_xid = gllc_prfd_mU_octet16[lle];
      ...
      if ( gllc_lle[lle].state == GLLC_LLE_ABM )
      {
        ...
        if ( gllc_lle[lle].unnum.xid_req.mu_xid <
             gllc_lle[lle].ack.max_retrx_buf_octets_mU / 16 )
        {
          gllc_lle[lle].unnum.xid_req.mu_xid =
            gllc_lle[lle].ack.max_retrx_buf_octets_mU / 16;
        }
      }
    }
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_kD_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_kD_XID;

    /* If current value is equal to the default value, then propose preferred
       value, else propose the last successfully negotiated value. */
    
    if ( gllc_lle[lle].ack.max_reseq_buf_frames_kD ==
         gllc_dflt_kD_frames[lle]                     )
    {      
      gllc_lle[lle].unnum.xid_req.kd_xid = gllc_prfd_kD_frames[lle];
      ...
      if ( gllc_lle[lle].state == GLLC_LLE_ABM )
      {
        ...
        if ( gllc_lle[lle].unnum.xid_req.kd_xid <
             gllc_lle[lle].ack.max_reseq_buf_frames_kD )
        {
          gllc_lle[lle].unnum.xid_req.kd_xid =
            gllc_lle[lle].ack.max_reseq_buf_frames_kD;
        }
      }
    }
  ...
  if ( gllc_lle[lle].unnum.xid_pending & GLLC_kU_XID )
  {
    gllc_lle[lle].unnum.xid_req.contents |= GLLC_kU_XID;

    /* If current value is equal to the default value, then propose preferred
       value, else propose the last successfully negotiated value. */
    
    if ( gllc_lle[lle].ack.max_retrx_buf_frames_kU ==
         gllc_dflt_kU_frames[lle]                     )
    {
      gllc_lle[lle].unnum.xid_req.ku_xid = gllc_prfd_kU_frames[lle];
      ...
      if ( gllc_lle[lle].state == GLLC_LLE_ABM )
      {
        ...
        if ( gllc_lle[lle].unnum.xid_req.ku_xid <
             gllc_lle[lle].ack.max_retrx_buf_frames_kU )
        {
          gllc_lle[lle].unnum.xid_req.ku_xid =
            gllc_lle[lle].ack.max_retrx_buf_frames_kU;
        }
      }
    }
  ...
  gllc_lle_last_gan_state[lle] = gllc_llme.gan_state;

  return ( gllc_lle[lle].unnum.xid_req.contents ? TRUE:FALSE );
...
void gllc_lle_generate_l2_xid_neg( gllc_ll_sapi_t lle )
{
  /* --------------------------------------------------------------------- */
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_VERSION_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_VERSION_XID;

    /* Is proposed Version Number in valid range? */

#if ( GLLC_MIN_VERSION == 0 ) /* Avoid compiler warning when 0. */
    if ( gllc_lle[lle].unnum.xid_req.version_xid <= GLLC_MAX_VERSION )
#else
    if ( (gllc_lle[lle].unnum.xid_req.version_xid >= GLLC_MIN_VERSION) &&
         (gllc_lle[lle].unnum.xid_req.version_xid <= GLLC_MAX_VERSION)    )
#endif
    {
      /* Version negotiation sense is down. */

#if ( GLLC_PRFD_VERSION == 0 ) /* Avoid compiler warning when 0. */
      gllc_lle[lle].unnum.xid_neg.version_xid = GLLC_PRFD_VERSION;
#else
      if ( gllc_lle[lle].unnum.xid_req.version_xid >= GLLC_PRFD_VERSION )
        gllc_lle[lle].unnum.xid_neg.version_xid = GLLC_PRFD_VERSION;
      else   
        gllc_lle[lle].unnum.xid_neg.version_xid =
          gllc_lle[lle].unnum.xid_req.version_xid;
#endif
    }
    else
    {
      ...
      gllc_lle[lle].unnum.xid_neg.version_xid = gllc_llme.llc_version;
    }
  }

  /* --------------------------------------------------------------------- */

  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_UI_XID )
  {
    ...
    gllc_lle[lle].unnum.xid_neg.contents  |= GLLC_IOV_UI_XID;
    gllc_lle[lle].unnum.xid_neg.iov_ui_xid =
      gllc_lle[lle].unnum.xid_req.iov_ui_xid;
  }
  
  /* --------------------------------------------------------------------- */

  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_I_XID )
  {
    /* NOTE: Unnumbered transmission handling will not permit this parameter
             to be transmitted from the MS in the uplink direction, but it
             needs to be included here to allow the parameter to be set. */
    
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_IOV_I_XID;
    gllc_lle[lle].unnum.xid_neg.iov_i_xid =
      gllc_lle[lle].unnum.xid_req.iov_i_xid;
  }
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_T200_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_T200_XID;

    /* Is proposed T200 in valid range? */

    if ( (gllc_lle[lle].unnum.xid_req.t200_xid >= (GLLC_MIN_T200_MS / 100)) &&
         (gllc_lle[lle].unnum.xid_req.t200_xid <= (GLLC_MAX_T200_MS / 100))    )
    {
      /* T200 negotiation sense is up. Negotiate up to the MS acceptable
         minimum if proposed value is below this. */

      if ( gllc_lle[lle].unnum.xid_req.t200_xid <=
           GLLC_MIN_ACCEPTABLE_T200_MS / 100       )
      {
        gllc_lle[lle].unnum.xid_neg.t200_xid =
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N200_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_N200_XID;

    /* Is proposed N200 in valid range? */

    if ( (gllc_lle[lle].unnum.xid_req.n200_xid >= GLLC_MIN_N200) &&
         (gllc_lle[lle].unnum.xid_req.n200_xid <= GLLC_MAX_N200)    )
    {
      /* N200 negotiation sense is up. Accept proposed value as there are
         no MS imposed limits. */

      gllc_lle[lle].unnum.xid_neg.n200_xid = 
        gllc_lle[lle].unnum.xid_req.n200_xid;
    }
    else
    {
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_U_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_N201_U_XID;

    /* Is proposed N201-U in valid range? */

    if ( (gllc_lle[lle].unnum.xid_req.n201_u_xid >= gllc_min_N201_U_octets[lle]) &&
         (gllc_lle[lle].unnum.xid_req.n201_u_xid <= gllc_max_N201_U_octets[lle])    )
    {
      /* N201-U negotiation sense is down. Accept proposed value as there are
         no MS maximum requirements. */
  
      gllc_lle[lle].unnum.xid_neg.n201_u_xid =
        gllc_lle[lle].unnum.xid_req.n201_u_xid;
    }
    else
    {
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_I_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_N201_I_XID;

    /* Is proposed N201-I in valid range? */

    if ( (gllc_lle[lle].unnum.xid_req.n201_i_xid >= GLLC_MIN_N201_I) &&
         (gllc_lle[lle].unnum.xid_req.n201_i_xid <= GLLC_MAX_N201_I)    )
    {
      /* N201-I negotiation sense is down. Accept proposed value as there are
         no MS imposed limits. */
  
      gllc_lle[lle].unnum.xid_neg.n201_i_xid =
        gllc_lle[lle].unnum.xid_req.n201_i_xid;
    }
    else
    {
    ...
    if ( gllc_lle[lle].state == GLLC_LLE_ABM )
    {
      if ( gllc_lle[lle].unnum.xid_neg.n201_i_xid <
           gllc_lle[lle].ack.max_info_octets_N201_I )
      {
        gllc_lle[lle].unnum.xid_neg.n201_i_xid =
          gllc_lle[lle].ack.max_info_octets_N201_I;
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_kD_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_kD_XID;

    /* Is kD in valid range? Code with Lint in mind. */

#if ( GLLC_MAX_KD_FRAMES < 255 )
    if ( (gllc_lle[lle].unnum.xid_req.kd_xid >= GLLC_MIN_KD_FRAMES) &&
         (gllc_lle[lle].unnum.xid_req.kd_xid <= GLLC_MAX_KD_FRAMES)    )
#else
    if ( gllc_lle[lle].unnum.xid_req.kd_xid >= GLLC_MIN_KD_FRAMES )
#endif
    {
      ...
      {
        gllc_lle[lle].unnum.xid_neg.kd_xid = GLLC_MAX_ACCEPTABLE_KD_FRAMES;
      }
      else   
        gllc_lle[lle].unnum.xid_neg.kd_xid =
          gllc_lle[lle].unnum.xid_req.kd_xid;
    }
    else
    {
    ...
    if ( gllc_lle[lle].state == GLLC_LLE_ABM )
    {
      if ( gllc_lle[lle].unnum.xid_neg.kd_xid <
           gllc_lle[lle].ack.max_reseq_buf_frames_kD )
      {
        gllc_lle[lle].unnum.xid_neg.kd_xid =
          gllc_lle[lle].ack.max_reseq_buf_frames_kD;
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_mD_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_mD_XID;
    ...
    if
    (
      (gllc_lle[lle].unnum.xid_req.md_xid > GLLC_MAX_MD_OCTET16)
      ||
      (
        (gllc_lle[lle].unnum.xid_req.md_xid  < GLLC_MIN_MD_OCTET16) &&
        (gllc_lle[lle].unnum.xid_req.md_xid != 0                  )
      )
    )
    {
      gllc_lle[lle].unnum.xid_neg.md_xid =
        gllc_lle[lle].ack.max_reseq_buf_octets_mD / 16;
    }
    ...
    else if ( gllc_lle[lle].unnum.xid_req.md_xid == 0 )
      gllc_lle[lle].unnum.xid_neg.md_xid = 0;
    ...
    else if ( gllc_prfd_mD_octet16[lle] == 0 )
    {
      ...
      if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_N201_I_XID )
        n201_i = gllc_lle[lle].unnum.xid_neg.n201_i_xid;
      else
        n201_i = gllc_lle[lle].ack.max_info_octets_N201_I;

      if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_kD_XID )
        kD = gllc_lle[lle].unnum.xid_neg.kd_xid;
      else
        kD = gllc_lle[lle].ack.max_reseq_buf_frames_kD;

      /* Is the product of N201-I and kD greater than the requested
         (proposed) mD parameter? */ 
      
      if ( (n201_i * kD) > (gllc_lle[lle].unnum.xid_req.md_xid * 16) )
      {
        /* It is so start by reducing N201-I. */

        n201_i = (uint16)(gllc_lle[lle].unnum.xid_req.md_xid * 16) / kD;
        ...
        if ( n201_i >= GLLC_MIN_N201_I ) 
        {
          ...
          gllc_lle[lle].unnum.xid_neg.contents  |= GLLC_N201_I_XID;
          gllc_lle[lle].unnum.xid_neg.n201_i_xid = n201_i;
        }
        else
        {
          ...
          kD     = (uint8)(gllc_lle[lle].unnum.xid_req.md_xid * 16) / n201_i;
        
          gllc_lle[lle].unnum.xid_neg.contents |=
            ( GLLC_N201_I_XID | GLLC_kD_XID );

          gllc_lle[lle].unnum.xid_neg.n201_i_xid = n201_i;
          gllc_lle[lle].unnum.xid_neg.kd_xid     = kD;
        }
      }
    }
    ...
    else
    {
      ...
      {
        gllc_lle[lle].unnum.xid_neg.md_xid = GLLC_MAX_ACCEPTABLE_MD_OCTET16;
      }
      else   
        gllc_lle[lle].unnum.xid_neg.md_xid =
          gllc_lle[lle].unnum.xid_req.md_xid;
      ...
      if ( (gllc_lle[lle].unnum.xid_neg.md_xid * 16) < n201_i )
        gllc_lle[lle].unnum.xid_neg.md_xid = (n201_i / 16) + 1;
    }
  }
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_kU_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_kU_XID;

    /* Is kU in valid range? Code with Lint in mind. */

#if ( GLLC_MAX_KU_FRAMES < 255 )
    if ( (gllc_lle[lle].unnum.xid_req.ku_xid >= GLLC_MIN_KU_FRAMES) &&
         (gllc_lle[lle].unnum.xid_req.ku_xid <= GLLC_MAX_KU_FRAMES)    )
#else
    if ( gllc_lle[lle].unnum.xid_req.ku_xid >= GLLC_MIN_KU_FRAMES )
#endif
    {
      ...
      {
        gllc_lle[lle].unnum.xid_neg.ku_xid = GLLC_MAX_ACCEPTABLE_KU_FRAMES;
      }
      else   
        gllc_lle[lle].unnum.xid_neg.ku_xid =
          gllc_lle[lle].unnum.xid_req.ku_xid;
    }
    else
    {
    ...
    if ( gllc_lle[lle].state == GLLC_LLE_ABM )
    {
      if ( gllc_lle[lle].unnum.xid_neg.ku_xid <
           gllc_lle[lle].ack.max_retrx_buf_frames_kU )
      {
        gllc_lle[lle].unnum.xid_neg.ku_xid =
          gllc_lle[lle].ack.max_retrx_buf_frames_kU;
  ...
  if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_mU_XID )
  {
    gllc_lle[lle].unnum.xid_neg.contents |= GLLC_mU_XID;
    ...
    if
    (
      (gllc_lle[lle].unnum.xid_req.mu_xid > GLLC_MAX_MU_OCTET16)
      ||
      (
        (gllc_lle[lle].unnum.xid_req.mu_xid  < GLLC_MIN_MU_OCTET16) &&
        (gllc_lle[lle].unnum.xid_req.mu_xid != 0                  )
      )
    )
    {
      gllc_lle[lle].unnum.xid_neg.mu_xid =
        gllc_lle[lle].ack.max_retrx_buf_octets_mU / 16;
    }
    ...
    else if ( gllc_lle[lle].unnum.xid_req.mu_xid == 0 )
      gllc_lle[lle].unnum.xid_neg.mu_xid = 0;
    ...
    else if ( gllc_prfd_mU_octet16[lle] == 0 )
    {
      ...
      if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_kU_XID )
        kU = gllc_lle[lle].unnum.xid_neg.ku_xid;
      else
        kU = gllc_lle[lle].ack.max_retrx_buf_frames_kU;

      /* Is the product of N201-I and kU greater than the requested
         (proposed) mU parameter? */ 
      
      if ( (n201_i * kU) > (gllc_lle[lle].unnum.xid_req.mu_xid * 16) )
      {
        /* It is so start by reducing N201-I. */

        n201_i = (uint16)(gllc_lle[lle].unnum.xid_req.mu_xid * 16) / kU;
        ...
        else
        {
          ...
          n201_i = GLLC_MIN_N201_I;
          kU     = (uint8)(gllc_lle[lle].unnum.xid_req.mu_xid * 16) / n201_i;
        
          gllc_lle[lle].unnum.xid_neg.contents |=
            ( GLLC_N201_I_XID | GLLC_kU_XID );

          gllc_lle[lle].unnum.xid_neg.n201_i_xid = n201_i;
          gllc_lle[lle].unnum.xid_neg.ku_xid     = kU;
        }
      }
    }
    ...
    else
    {
      ...
      {
        gllc_lle[lle].unnum.xid_neg.mu_xid = GLLC_MAX_ACCEPTABLE_MU_OCTET16;
      }
      else   
        gllc_lle[lle].unnum.xid_neg.mu_xid =
          gllc_lle[lle].unnum.xid_req.mu_xid;
      ...
      if ( (gllc_lle[lle].unnum.xid_neg.mu_xid * 16) < n201_i )
        gllc_lle[lle].unnum.xid_neg.mu_xid = (n201_i / 16) + 1;
    }
  }
...
)
{
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_VERSION_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_VERSION_XID)    )
  {
    gllc_llme.llc_version = gllc_lle[lle].unnum.xid_neg.version_xid;
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_VERSION_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_VERSION_XID )
  {
    gllc_llme.llc_version = gllc_lle[lle].unnum.xid_req.version_xid;
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_VERSION_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_VERSION_XID )
  {
    if ( gllc_llme.llc_version == gllc_lle[lle].unnum.xid_neg.version_xid )
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_VERSION_XID;
  }
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_UI_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_IOV_UI_XID)    )
  {
    gllc_llme.input_offset_IOV_UI = gllc_lle[lle].unnum.xid_neg.iov_ui_xid;
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_IOV_UI_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_UI_XID )
  {
    gllc_llme.input_offset_IOV_UI = gllc_lle[lle].unnum.xid_req.iov_ui_xid;
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_IOV_UI_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_IOV_UI_XID )
  {
    gllc_llme.input_offset_IOV_UI = gllc_lle[lle].unnum.xid_neg.iov_ui_xid;
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_IOV_UI_XID;
  }
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_I_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_IOV_I_XID)    )
  {
    gllc_lle[lle].ack.input_offset_IOV_I =
      gllc_lle[lle].unnum.xid_neg.iov_i_xid;
    
    gllc_lle[lle].unnum.xid_pending &= ~GLLC_IOV_I_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_IOV_I_XID )
  {
    gllc_lle[lle].ack.input_offset_IOV_I =
      gllc_lle[lle].unnum.xid_req.iov_i_xid;
  ...
  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_IOV_I_XID) &&
            (u_frame != NULL                                      )    )
  {
    if ( u_frame -> cmd_res_M4_M1 == GLLC_UA )
    {
    ...
    else if ( gllc_lle[lle].ack.input_offset_IOV_I ==
              gllc_lle[lle].unnum.xid_neg.iov_i_xid   )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_IOV_I_XID;
    }
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_T200_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_T200_XID)    )
  {
    gllc_lle[lle].gen.retrx_interval_T200 =
      gllc_lle[lle].unnum.xid_neg.t200_xid * 100;

    gllc_lle[lle].ack.retrx_interval_T201 =
      gllc_lle[lle].unnum.xid_neg.t200_xid * 100;

    gllc_lle[lle].unnum.xid_pending &= ~GLLC_T200_XID;
  }
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_T200_XID )
  {
    gllc_lle[lle].gen.retrx_interval_T200 =
      gllc_lle[lle].unnum.xid_req.t200_xid * 100;
  ...
  else if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_T200_XID )
  {
    if ( gllc_lle[lle].gen.retrx_interval_T200 ==
         gllc_lle[lle].unnum.xid_neg.t200_xid * 100 )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_T200_XID;
    }
  ...
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N200_XID )
  {
    gllc_lle[lle].gen.max_retrx_cnt_N200 =
      gllc_lle[lle].unnum.xid_req.n200_xid;
  ...
  else if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_N200_XID ) 
  {    
    if ( gllc_lle[lle].gen.max_retrx_cnt_N200 ==
         gllc_lle[lle].unnum.xid_neg.n200_xid    )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_N200_XID;
    }
  ...
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_U_XID )
  {
    gllc_lle[lle].unack.max_info_octets_N201_U =
      gllc_lle[lle].unnum.xid_req.n201_u_xid;
  ...
  else if ( gllc_lle[lle].unnum.xid_neg.contents & GLLC_N201_U_XID )
  {
    if ( gllc_lle[lle].unack.max_info_octets_N201_U ==
         gllc_lle[lle].unnum.xid_neg.n201_u_xid        )
    {
  ...
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_N201_I_XID )
  {
    gllc_lle[lle].ack.max_info_octets_N201_I =
      gllc_lle[lle].unnum.xid_req.n201_i_xid;
  ...
  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_N201_I_XID) &&
            (u_frame != NULL                                       )    )
  {
    if ( (gllc_lle[lle].state      == GLLC_LLE_LOC_ESTABLISH) &&
    ...
    else if ( gllc_lle[lle].ack.max_info_octets_N201_I ==
              gllc_lle[lle].unnum.xid_neg.n201_i_xid      )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_N201_I_XID;
    }
  ...
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_kD_XID )
  {
    gllc_lle[lle].ack.max_reseq_buf_frames_kD =
      gllc_lle[lle].unnum.xid_req.kd_xid;
  ...
  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_kD_XID) &&
            (u_frame != NULL                                   )    )
  {
    if ( (gllc_lle[lle].state      == GLLC_LLE_LOC_ESTABLISH) &&
         (u_frame -> cmd_res_M4_M1 == GLLC_UA               )    )
    ...
    else if ( gllc_lle[lle].ack.max_reseq_buf_frames_kD ==
              gllc_lle[lle].unnum.xid_neg.kd_xid           )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_kD_XID;
    }
  ...
  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_kU_XID )
  {
    gllc_lle[lle].ack.max_retrx_buf_frames_kU =
      gllc_lle[lle].unnum.xid_req.ku_xid;
  ...
  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_kU_XID) &&
            (u_frame != NULL                                   )    )
  {
    if ( (gllc_lle[lle].state      == GLLC_LLE_LOC_ESTABLISH) &&
         (u_frame -> cmd_res_M4_M1 == GLLC_UA               )    )
    ...
    else if ( gllc_lle[lle].ack.max_retrx_buf_frames_kU ==
              gllc_lle[lle].unnum.xid_neg.ku_xid           )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_kU_XID;
    }
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_mD_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_mD_XID)    )
  {
    if ( gllc_lle[lle].unnum.xid_neg.md_xid == 0 )
    {
    ...
    gllc_lle[lle].ack.max_reseq_buf_octets_mD =
      (uint16)(gllc_lle[lle].unnum.xid_neg.md_xid * 16);

    gllc_lle[lle].unnum.xid_pending &= ~GLLC_mD_XID;
  }

  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_mD_XID )
  {
    gllc_lle[lle].ack.max_reseq_buf_octets_mD =
      (uint16)(gllc_lle[lle].unnum.xid_req.md_xid * 16);

    gllc_lle[lle].unnum.xid_pending &= ~GLLC_mD_XID;
  }

  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_mD_XID) &&
            (u_frame != NULL                                   )    )
  {
    if ( (gllc_lle[lle].state      == GLLC_LLE_LOC_ESTABLISH) &&
         (u_frame -> cmd_res_M4_M1 == GLLC_UA               )    )
    {
      if ( (uint16)(gllc_lle[lle].unnum.xid_neg.md_xid * 16) <
           gllc_lle[lle].ack.max_reseq_buf_octets_mD           )
      {
    ...
    else if ( gllc_lle[lle].ack.max_reseq_buf_octets_mD ==
              (uint16)(gllc_lle[lle].unnum.xid_neg.md_xid * 16) )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_mD_XID;
    }
  ...
  if ( (gllc_lle[lle].unnum.xid_req.contents & GLLC_mU_XID) &&
       (gllc_lle[lle].unnum.xid_neg.contents & GLLC_mU_XID)    )
  {
    if ( gllc_lle[lle].unnum.xid_neg.mu_xid == 0 )
    {
      /* The LLE which originated the negotiated response does not wish
    ...
    gllc_lle[lle].ack.max_retrx_buf_octets_mU =
      (uint16)(gllc_lle[lle].unnum.xid_neg.mu_xid * 16);

    gllc_lle[lle].unnum.xid_pending &= ~GLLC_mU_XID;
  }

  else if ( gllc_lle[lle].unnum.xid_req.contents & GLLC_mU_XID )
  {
    gllc_lle[lle].ack.max_retrx_buf_octets_mU =
      (uint16)(gllc_lle[lle].unnum.xid_req.mu_xid * 16);

    gllc_lle[lle].unnum.xid_pending &= ~GLLC_mU_XID;
  }

  else if ( (gllc_lle[lle].unnum.xid_neg.contents & GLLC_mU_XID) &&
            (u_frame != NULL                                   )    )
  {
    if ( (gllc_lle[lle].state      == GLLC_LLE_LOC_ESTABLISH) &&
         (u_frame -> cmd_res_M4_M1 == GLLC_UA               )    )
    {
      if ( (uint16)(gllc_lle[lle].unnum.xid_neg.mu_xid * 16) <
           gllc_lle[lle].ack.max_retrx_buf_octets_mU           )
      {
    ...
    else if ( gllc_lle[lle].ack.max_retrx_buf_octets_mU ==
              (uint16)(gllc_lle[lle].unnum.xid_neg.mu_xid * 16) )
    {
      gllc_lle[lle].unnum.xid_pending &= ~GLLC_mU_XID;
    }
...
{
  MSG_GERAN_HIGH_9_G( "GLC LL %d: Info XID contents: 0x%x, XID Errors: 0x%x, XID Vers: %d, IOV-UI: 0x%x, IOV-I:  0x%x, N200:%d, N201-U:%d, N201-I:%d",
    gllc_sapi_addr_lookup[lle],
    xid_info_p -> contents,
    xid_info_p -> errors,
    xid_info_p -> version_xid,
    xid_info_p -> iov_ui_xid,
    xid_info_p -> iov_i_xid,
    xid_info_p -> n200_xid,
    xid_info_p -> n201_u_xid,
    xid_info_p -> n201_i_xid);

  MSG_GERAN_HIGH_5_G( "XID T200:%d, mD:%d, mU:%d, kD:%d, kU:%d",
    xid_info_p -> t200_xid,
    xid_info_p -> md_xid,
    xid_info_p -> mu_xid,
    xid_info_p -> kd_xid,
