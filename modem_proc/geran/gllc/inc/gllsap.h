...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gllc/inc/gllsap.h#3 $
***
...
/* LL SAPIs used by the SNDCP/LLC entity. */
typedef enum
{
...
#define GSN_SAPI_MAX        (GSN_SAPI_LAST+1)
...
/* ------------------------------------------------------------------------
   LL_ENTER_PSM_IND Service Primitive structure def for use in the uplink.
   ------------------------------------------------------------------------ */

typedef struct
{
  boolean          sapi_active;  /* tells if SAPI is active or not */
  dsm_item_type    *l3_xid_info; /* Layer3 XID info */
} psm_sapi_info_t;

typedef struct
{
  ll_ul_prim_t          prim;                   /* LL_ENTER_PSM_IND */

  psm_sapi_info_t       psm_info[GSN_SAPI_MAX]; /* info that need to be saved for each SAPI */
} ll_enter_psm_ind_t;
