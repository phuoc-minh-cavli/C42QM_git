...
$Header: //components/rel/geran.mpss/5.2.0/gl2/src/l2_send.c#3 $
$DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
...
static dl_data_mux_ind_T demux_callbacks = {{0},NULL,NULL};
 ...
 *
 */
static gs_queue_id_T l2_get_l1_queue_id(void)
{
...
static void l2_send_ph_data_cnf(l2_store_T *l2_data_ptr, ph_data_cnf_type_t cnf_type, uint32 frame_num)
{
  ...
  gs_status = gs_send_message(l2_get_l1_queue_id(), &ph_data_cnf, TRUE);
...
{
  if (GS_SUCCESS != gs_send_message(GS_QUEUE_RR, msg_ptr, FALSE))
  {
                              ...
                              boolean     short_header)
{
  ...
  {
    MSG_GERAN_LOW_1("L2 segment length %d exceeds max", (int)l2_data_ptr->l2_segment_length);
  }
...
void l2_send_ph_random_access_req(const dl_random_access_req_T *req_ptr)
{
  ...
  gs_status = gs_send_message(l2_get_l1_queue_id(), &ph_random_access_req, TRUE);
...
void l2_send_ph_random_access_ind(const dl_random_access_req_T *req_ptr, const boolean repeat_ind)
{
  ...
  gs_status = gs_send_message(l2_get_l1_queue_id(), &ph_random_access_ind, TRUE);
...
void l2_send_ph_data_req(l2_store_T *l2_data_ptr, SAPI_T sapi, channel_info_T *channel_info, byte frame_length)
{
  ...
  gs_status = gs_send_message(l2_get_l1_queue_id(), &ph_data_req, TRUE);
...
void l2_send_ph_stop_sacch_measurement_report(l2_store_T *l2_data_ptr)
{
  ...
  gs_status = gs_send_message(l2_get_l1_queue_id(), &ph_stop_sacch_measurement_report, TRUE);
...
static boolean l2_cbch_demultiplexer(l2_store_T *l2_data_ptr, dl_cb_block_ind_T *msg_ptr)
{
  ...
  msg_ptr->as_id = SYS_MODEM_AS_ID_1;
  MSG_GERAN_HIGH_2_G("Sending CB_BLOCK_IND with FN=%d (TC=%d)", dest_ptr->Frame_no, cbch_tc);
  
  if (GS_SUCCESS == gs_send_message(GS_QUEUE_CB, msg_ptr, FALSE))
  {
