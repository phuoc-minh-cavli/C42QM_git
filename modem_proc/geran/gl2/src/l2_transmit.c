...
$Header: //components/rel/geran.mpss/5.2.0/gl2/src/l2_transmit.c#3 $ $DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
 ...
 *
 */
static void l2_stale_cache_failure(dl_random_access_req_T *cache_ptr)
{
  if (cache_ptr != NULL)
  {
...
void l2_check_transmit_on_receive(l2_store_T *l2_data_ptr)
{
  ...
  if ((l2_data_ptr->current_l2_channel_type == DCCH) && (l2_data_ptr->last_dcch_type == SDCCH))
  {
        ...
        (l2_data_ptr->sapi0.T200_channel_type == DCCH))
    {
      ...
      MSG_GERAN_LOW_0("Pseudo T200(0) timeout");
        ...
        (l2_data_ptr->sapi3.T200_channel_type == DCCH))
    {
      ...
      MSG_GERAN_LOW_0("Pseudo T200(3) timeout");
