...
$Header: //components/rel/geran.mpss/5.2.0/gl2/src/l2_msg.c#4 $ $DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
...
******************************************************/
#define L2_PAD_FRAME(buf_ptr,oct_cnt) l2_generate_filler_dyn((buf_ptr),(oct_cnt))
  ...
  *valid_frame = FALSE;
      ...
      if (((byte) (control_field & 0x02) >> 1) == 0)
      {
        ...
        else
        {
          ...
          else if (((byte) (control_field & 0x0C) >> 2) == 0)
          {
            ...
            /***************************/
            MSG_GERAN_HIGH_4_G("S-frm%d:RR  N(R):%1d C(R):%1d P(F):%1d", SAPI,l2_data_ptr->received_NR,l2_data_ptr->received_CR,l2_data_ptr->received_PF);
  ...
  else if (LPD == 1)
  {
    ...
    if (cbch_frame_no != 0)
    {
      ...
      MSG_GERAN_LOW_0_G("Cell Broadcast is not supported");
#endif /* !FEATURE_GERAN_CBCH_SUPPORT */
    }
    else
    {
      MSG_GERAN_LOW_0_G("Frame number for CB Block is 0, ignored");
      filler_frame = TRUE;
    }
  }
  else
  ...
  {
    MSG_GERAN_HIGH_6_G("Could not decode ch %1d,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X",
      l2_data_ptr->current_l2_channel_type,l2_frame[0],l2_frame[1],l2_frame[2],l2_frame[3],l2_frame[4]);
  }
...
/*
 *   Function name:  l2_generate_filler_dyn
 *   ----------------------------
 *
 ...
 *   --------
 */
void l2_generate_filler_dyn(byte *spare_octets, uint16 num_octets)
{
      ...
      (l3_message_length  <= N201_MAX))
  {
    ...
    if (channel_info->N201 > l3_message_length)
    {
      L2_PAD_FRAME(sabm_frame + L2_HEADER_LENGTH + l3_message_length,
                   channel_info->N201 - l3_message_length);
    }
  }
  else
  {
    L2_PAD_FRAME(sabm_frame + L2_HEADER_LENGTH, channel_info->N201);
  }
...
void l2_code_ua(byte PF, sapi_info_T* sapi_info, channel_info_T* channel_info)
{
  ...
  L2_PAD_FRAME(ua_frame + L2_HEADER_LENGTH, channel_info->N201);
...
void l2_code_dm(byte PF, sapi_info_T* sapi_info, channel_info_T* channel_info)
{
  ...
  L2_PAD_FRAME(dm_frame + L2_HEADER_LENGTH, channel_info->N201);
...
void l2_code_disc(sapi_info_T* sapi_info, channel_info_T* channel_info, boolean retransmission)
{
  ...
  L2_PAD_FRAME(disc_frame + L2_HEADER_LENGTH, channel_info->N201);
...
void l2_code_ui(sapi_info_T *sapi_info, channel_info_T *channel_info, boolean short_header, byte *l3_message_to_send, byte l3_message_length)
{
  ...
  {
    L2_PAD_FRAME(ui_frame + L2_HEADER_LENGTH, channel_info->N201);
  }
...
void l2_code_i(byte PF, boolean previous_vs, sapi_info_T* sapi_info, channel_info_T* channel_info, boolean retransmission)
{
  ...
  MSG_GERAN_LOW_5("Code i frame: SAPI%d VS:%d VR:%d prev:%d PF:%d",sapi_info->sapi,sapi_info->VS,sapi_info->VR,previous_vs,PF);
  ...
  if (information_length <= N201_MAX)
  {
    ...
    if (channel_info->N201 > information_length)
    {
      L2_PAD_FRAME(i_frame + L2_HEADER_LENGTH + information_length,
                   channel_info->N201 - information_length);
    }
...
void l2_code_rr(byte CR, byte PF, sapi_info_T* sapi_info, channel_info_T* channel_info)
{
  ...
  L2_PAD_FRAME(rr_frame + L2_HEADER_LENGTH, channel_info->N201);
...
void l2_code_rej(byte CR, byte PF, sapi_info_T* sapi_info, channel_info_T* channel_info)
{
  ...
  L2_PAD_FRAME(rej_frame + L2_HEADER_LENGTH, channel_info->N201);
