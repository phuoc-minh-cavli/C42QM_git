...
$Header: //components/rel/geran.mpss/5.2.0/gl2/src/l2_main.c#5 $ $DateTime: 2019/07/05 02:39:06 $ $Author: pwbldsvc $
...
geran_gl2_intf_s l2_vtbl =
{
  l2_generate_filler_dyn /* l2_generate_filler */
};
#endif /* FEATURE_GERAN_CS_SUPPORT */
...
===========================================================================*/
static void l2_initialise(boolean from_power_on)
{
  ...
  if (l2_data_ptr != NULL)
  {
    ...
    if (sapi0_ptr != NULL)
    {
      ...
      sapi0_ptr->T200   = SAPI0_T_200;
      sapi0_ptr->T_ack  = SAPI0_T_ACK;
    ...
    if (sapi3_ptr != NULL)
    {
      ...
      sapi3_ptr->T200   = SAPI3_T_200;
      sapi3_ptr->T_ack  = SAPI3_T_ACK;
...
static void l2_handle_message( l2_queue_msg_type* message )
{
  ...
  {
    MSG_GERAN_LOW_0("Empty L2 message!");
  }
}
...
static void l2_initialise_task_data(void)
{
  ...
  (void) gs_init(GS_QUEUE_LAYER_2,
                 geran_tcb_read(GERAN_TASK_ID_GL2),
                 L2_CMD_Q_SIG,
                 &l2_cmd_q,
                 NULL,
...
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
void geran_gl2_init(void)
{
  gsm_l2_fn_init();
  return;
}

void geran_gl2_deinit(void)
{
  geran_q_destroy(&l2_cmd_q);

  return;
}

void geran_gl2_suspend(void)
{
  return;
}

void geran_gl2_resume(void)
{
  return;
}
...
static l2_store_T * l2_process_mdl_release_req(l2_msg_type *msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    if (l2_data_ptr != NULL)
    {
      ...
      l2_initialise(FALSE);
      MSG_GERAN_LOW_0("MDL_RELEASE_REQ resetting L2");
  ...
  if (msg_ptr != NULL)
  {
    ...
    if (l2_data_ptr != NULL)
    {
      ...
      if (L2_RACH_CHANNEL_TYPE(req_ptr->l2_channel_type))
      {
        ...
        l2_send_ph_random_access_ind(cache_ptr, FALSE);
  ...
  switch(new_message_ptr->message_header.message_id)
  {
    ...
    case DL_ESTABLISH_REQ:
      ...
      if (l2_data_ptr->establish_mode == CONTENTION_RESOLUTION)
      {
        ...
        {
          MSG_GERAN_LOW_0("Ignore DL_ESTABLISH_REQ, len too large");
          break;
        }
      }
      ...
      else
      {
        ...
        /***********************************************/
        MSG_GERAN_LOW_1("SAPI%d undefined",l2_data_ptr->current_SAPI);
      }
      break;
    ...
    case DL_DATA_REQ:
      ...
      {
        MSG_GERAN_LOW_0("Ignore DL_DATA_REQ, len too large");
        break;
      }
    ...
    case DL_SUSPEND_REQ:
      ...
      else
      {
        ...
        /***********************************************/
        MSG_GERAN_LOW_0("Suspension not allowed for SAPI");
      }
      break;
    ...
    case DL_RESUME_REQ:
          ...
          (new_message_ptr->dl_resume_req.l3_message_length > N201_MAX))
      {
        ...
        /***********************************************/
        MSG_GERAN_LOW_0("Invalid length or SAPI in DL-RESUME-REQ");
        break;
      }
    ...
    case DL_RECONNECT_REQ:
      ...
      {
        MSG_GERAN_LOW_0("Ignore DL_RECONNECT_REQ message, length too large");
        break;
      }
...
static l2_store_T *l2_handle_ph_data_ind(l2_msg_type *new_message_ptr)
{
      ...
      (l2_data_ptr->current_l2_channel_type == BCCH))
  {
    ...
    if (message_length > sizeof(ph_data_ind_T) - sizeof(IMH_T) - N201_MAX)
    {
      ...
      /* If the pseudo length is less than 6, its probably (!) ignorable, unless
       * we're on our own paging block, in which case we need the null message
       * for DSC evaluation. Also, let any Sys Info messages through, as spec 
       * does not mandate Sys Info messages to have correct pseudo length.
       */
#ifdef FEATURE_GERAN_EXTENDED_DRX
      #define NULL_MSG_VALID_RO(l2_frame) ((l2_frame[6] != 0x2B) || (l2_frame[7] != 0x2B))
#endif /* FEATURE_GERAN_EXTENDED_DRX */

      if ((ph_data_ind_ptr->layer2_frame[0] >= (6 << 2))      || /* Pseudo Length greater than 6         */
#ifdef FEATURE_GERAN_EXTENDED_DRX
          ((l2_data_ptr->current_l2_channel_type == CCCH) && NULL_MSG_VALID_RO(ph_data_ind_ptr->layer2_frame)) || /* Null frame with valid rest octets */
#endif /* FEATURE_GERAN_EXTENDED_DRX */
          ((ph_data_ind_ptr->layer2_frame[2] & 0xF8) == 0x18) || /* SI 1, 2, 3, 4, 5, 6, 7, 8            */
          ((ph_data_ind_ptr->layer2_frame[2] & 0xF8) == 0x00) || /* SI 9, 13, 14, 2bis, 2ter, 2quater    */
          ((ph_data_ind_ptr->layer2_frame[2] & 0xFC) == 0x3C) || /* SI 16, 17                            */
          ((ph_data_ind_ptr->layer2_frame[2] & 0xF8) == 0x40) || /* SI 15, 18, 19, 20, 13alt, 2n, 21, 22 */
          ((ph_data_ind_ptr->layer2_frame[2] & 0xFF) == 0x4F) || /* SI 23                                */
          (own_paging_block))
      {
      ...
      else
      {
        ...
        if (l2_data_ptr->current_l2_channel_type == BCCH)
        {
          ...
          MSG_GERAN_HIGH_9("MSG-012[%02X,%02X,%02X] MSG-345[%02X,%02X,%02X] MSG-678[%02X,%02X,%02X]",
  ...
  else
  {
    ...
    if (message_length > sizeof(ph_data_ind_T) - sizeof(IMH_T) - N201_MAX)
    {
      ...
      if ((this_SAPI == 0) && (frame_valid == TRUE))
      {
        ...
        if (frame_type == UA)
        {
          MSG_GERAN_LOW_0("Received a UA frame");
        ...
        {
          MSG_GERAN_LOW_3("Rcvd DM frame %X %X %X",
                  (int) ph_data_ind_ptr->layer2_frame[0],
                  (int) ph_data_ind_ptr->layer2_frame[1],
                  (int) ph_data_ind_ptr->layer2_frame[2]);
        ...
        {
          MSG_GERAN_LOW_0("Receive a short header frame");
          own_paging_block = ph_data_ind_ptr->own_paging_block;
          channel_number= ph_data_ind_ptr->channel_number;
...
#else  /* FEATURE_GERAN_CBCH_SUPPORT */
      MSG_GERAN_LOW_0("Cell Broadcast is not supported");
...
static void l2_lib_read_message(l2_msg_type *new_message_ptr)
{
  ...
  {
    l2_data_ptr = NULL;
    MSG_GERAN_LOW_2("Unknown message(%d,%d)",new_message_ptr->message_header.message_set,new_message_ptr->message_header.message_id);
  }
