...
$Header: //components/rel/geran.mpss/5.2.0/gl2/src/l2_sapi0.c#2 $ $DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
...
static void l2_store_resume_message(l2_store_T *l2_data_ptr)
{
  if (l2_data_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_LOW_1("Resume msg length %d invalid exceeds DCCH max",(int)(l2_data_ptr->l3_message_length));
...
void l2_SAPI0_control(l2_store_T *l2_data_ptr, l2_event_T SAPI0_event)
{
  ...
  {
    MSG_GERAN_MED_2("L2 SAPI0 event: %d and state: %d",SAPI0_event, l2_data_ptr->SAPI0_state);
      ...
      case SAPI0_NULL:
        {
          ...
          {
            MSG_GERAN_LOW_0("Got a release req, when no link is established");
            l2_release_link(l2_data_ptr,SAPI0,l2_data_ptr->current_l2_channel_type);
            l2_data_ptr->SAPI0_state = SAPI0_NULL;
          }
          else
          {
            MSG_GERAN_LOW_1("Ignored event %d",SAPI0_event);
          }
          break;
        }
      ...
      case SAPI0_IDLE:
        {
          if (SAPI0_event == EV_RELEASE_REQUEST)
          {
            ...
            MSG_GERAN_LOW_0("Got a release req, when in idle?");
            l2_release_link(l2_data_ptr,SAPI0,l2_data_ptr->current_l2_channel_type);
          }
          else if (SAPI0_event == EV_ESTABLISH_REQUEST)
          {
      ...
      case SAPI0_ESTABLISHMENT_PENDING:
        {
          if (SAPI0_event == EV_RELEASE_REQUEST)
          {
            MSG_GERAN_LOW_0("Got a release req, when link not established");
            l2_data_ptr->release_mode = LOCAL_END_RELEASE;
            l2_release_link(l2_data_ptr,SAPI0,l2_data_ptr->current_l2_channel_type);
            l2_data_ptr->SAPI0_state = SAPI0_IDLE;
          }
      ...
      case SAPI0_RELEASE_PENDING:
        {
          if (SAPI0_event == EV_RELEASE_REQUEST)
          {
            MSG_GERAN_LOW_0("Got a release req, while already releasing");
            l2_data_ptr->release_mode = LOCAL_END_RELEASE;
            l2_release_link(l2_data_ptr,SAPI0,l2_data_ptr->current_l2_channel_type);
            l2_data_ptr->SAPI0_state = SAPI0_IDLE;
          }
      ...
      case SAPI0_LINK_ESTABLISHED:
        {
          ...
          else if (SAPI0_event == EV_CONNECT_RECEIVED)
          {
            ...
            MSG_GERAN_LOW_1("SAPI0 PH_CONNECT_IND from Layer 1: channel %d",
      ...
      case LINK_SUSPENDED:
        {
          if (SAPI0_event == EV_RELEASE_REQUEST)
          {
            MSG_GERAN_LOW_0("Got a release req, in suspended");
            l2_data_ptr->release_mode = LOCAL_END_RELEASE;
          ...
          else if (SAPI0_event == EV_DATA_REQUEST)
          {
            ...
            /* we resume                                                     */
            /*****************************************************************/
            MSG_GERAN_LOW_0("Data request when suspended");

            if (  l2_data_ptr->sapi0.DCCH_channel.outstanding_u_frame == FALSE )
            {
              MSG_GERAN_LOW_0("Outstanding frame=0 when suspended");
            }
    ...
    if (l2_data_ptr->old_SAPI0_state != l2_data_ptr->SAPI0_state)
    {
      ...
      MSG_GERAN_HIGH_1("L2 SAPI0 State = %d", l2_data_ptr->SAPI0_state);
