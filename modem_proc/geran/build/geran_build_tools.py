...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/geran.mpss/5.2.0/build/geran_build_tools.py#6 $
#  $DateTime: 2019/04/15 01:13:13 $
#
...
def build_variation(env, variation):
  ...
  if _PL_check(env, ['MPSS.NI.', \
    ...
    result = _PL_check(env, ['MPSS.CT.', 'MPSS.TH.', 'MPSS.AT.'])
...
def set_up_component_build_config(
  ...
  if UNIT_NAME != 'API':
    ...
    if build_variation(env, 'GERAN_INTERNAL_APIS'):
      ...
      L1_UNITS =       ['GDRIVERS', 'GDRIVERS_STATIC', 'GL1', 'GL1_STATIC', 'GMDSP', 'GMDSP_STATIC', 'GPLT']
      PROTOCOL_UNITS = ['GCOMMON', 'GDIAG', 'GL2', 'GLLC', 'GMAC', 'GRLC', 'GRR', 'GRR_STATIC', 'GCOMMONSTATSRC', 'GCOMMONSTATINC', 'GMAC_STATIC', 'GSNDCP']
  ...
  if UNIT_TEST:
    ...
    if DELIVER_SOURCE:
      ...
      if len(SOURCES):  # if there is any
        if UNIT_NAME == 'GRR' and ('USES_FEATURE_DYNAMIC_LOADING_GERAN' in env) and ('USES_MODEM_DYNAMIC_LOADING' in env):
          SHARED_LIBRARY='rr'
          env.Append(CFLAGS='-DDYNAMIC_LINKING_ENABLED')
          env.AddSharedLibrary(['MODEM_MODEM'], '${BUILDPATH}/'+SHARED_LIBRARY, [SOURCES])
        elif DMM and ('USES_FEATURE_DYNAMIC_LOADING_GERAN' in env) and ('USES_FEATURE_EXTERNAL_PAGING' in env):
          env.AddLibrary( IMAGES, join('${BUILDPATH}', '%s_%s_dmm' % (SU_NAME.lower(), UNIT_NAME.lower())), [SOURCES] )
        else:
          env.AddLibrary( IMAGES, join('${BUILDPATH}', '%s_%s' % (SU_NAME, UNIT_NAME)), [SOURCES] )
    else:
      if DMM and ('USES_FEATURE_DYNAMIC_LOADING_GERAN' in env) and ('USES_FEATURE_EXTERNAL_PAGING' in env):
        env.AddBinaryLibrary( IMAGES, join('${BUILDPATH}', '%s_%s_dmm' % (SU_NAME, UNIT_NAME)), [SOURCES] ,
    ...
    if PRIVATE_SOURCES != []:
      ...
      if DMM and ('USES_FEATURE_DYNAMIC_LOADING_GERAN' in env_private) and ('USES_FEATURE_EXTERNAL_PAGING' in env_private):
        env_private.AddBinaryLibrary( IMAGES, join('${BUILDPATH}', 'lib_%s_%s_dmm' %
                            (SU_NAME.lower(), UNIT_NAME.lower())), [PRIVATE_SOURCES],
    ...
    if TDS_RRC_SOURCES != []:
      ...
      #                                          or COMPILE_L1_OPT_AC_PROTECTED_LIBS
      if DMM and ('USES_FEATURE_DYNAMIC_LOADING_GERAN' in env) and ('USES_FEATURE_EXTERNAL_PAGING' in env):
        env.AddBinaryLibrary( IMAGES, join('${BUILDPATH}', 'tds_rrc_%s_%s_dmm' % (SU_NAME.lower(), UNIT_NAME.lower())), [TDS_RRC_SOURCES],
