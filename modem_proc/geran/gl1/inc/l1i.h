...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/l1i.h#14 $
$DateTime: 2020/08/24 00:47:58 $ $Author: pwbldsvc $

when       who      what, where, why
--------   -------- ---------------------------------------------
19/08/20   sal      CR2758915:GSM: Send QMI-NAS Ind whenever jammer status is changed.
28/04/20   rks      CR2673341 GL1 Changes for GPS search FR implementation 
...
12/07/19   ktg      CR2484848 FR 54634: Indicate Jammer status to DS
...
11/07/18   km       CR2276497 Deprecate usage of MCPM_Get_PwrOpt_Level()
...
04/07/17   hd       CR2070706 change the type of ext paging multiframe variable to 16 bits
23/02/17   hd       CR2010727 Properly calculate paging group for higher eDRx
06/02/17   akm      CR1113531 Ncell BCCH colliding with Page decode both getting failed
11/01/17   nm       CR1087215 FR 36703: Rel.13 GERAN Power Efficient Operation (PEO)
30/12/16   rv       CR1087205 FR36017: Extended Access Barring 
30/12/16   km       CR1102445 GSM diag functionality to simulate IRAT, mobility and REL-13 features
...
typedef enum
{
   ...
   ,L1_CALL_IN_TASK_TURN_TX_OFF        = 0x3F
...
typedef struct {
   ...
   uint16           decstat[RR_MAX_PSCAN_FREQ_SCAN + 1];
#endif
...
typedef struct {
   ...
   imsi_mod_10000000_T imsi_mod_10000000;
#endif
   boolean          updated;
...
#define SERV_BCCH_MASK_21             (1uL<<4)
...
#define SERV_EXT_BCCH_MASK_21         (1uL << 4)
#define SERV_EXT_BCCH_MASK_ALL        0x000000ff

#ifdef FEATURE_GERAN_EAB
#define SI21_RPOS_INVALID  0xFFFF
#define SI9_SCHED_INVALID  0xFF
#endif
...
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
#define GSM_CLEANUP_TIME_MS  20
#define GSM_STOP_DURATION_MS 10
#define ARB_PSCAN_MIN_MS 100
#define ARB_ACCESS_DURATION_MS  5000
...
typedef enum
{
    IDLE_ACT_TYPE_SERV_CCCH,
    IDLE_ACT_TYPE_SERV_BCCH,
    IDLE_ACT_TYPE_OTHERS
} l1_idle_serv_act_type_T;
...
typedef struct {
   ...
   uint32                    si_21_M[2];
   uint16                    si_21_P[2];
   normal_extended_t         si21_bcch_type[2];
   boolean                   si_21_pos_set;
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
   boolean                   si_9_sched_now;   /* valid only for testing */
   boolean                   si_21_read;
#endif
   uint8                     si9_sched_where; /* Valid for SI_9 only */
   uint8                     si_sched_info_count; /* Valid only for SI 21 */
   si_sched_info_T           si_sched_info; /* Valid only for SI 21 */
#endif
   uint32                    BCCH_mask;
...
#ifdef FEATURE_GERAN_EXTENDED_DRX
   layer1_edrx_peo_params_T   edrx_peo_params;
   l1_idle_serv_act_type_T    idle_serving_act_type;
#endif
...
#ifdef FEATURE_GERAN_SIMULATE_ARB_UNLOAD
#define MIN_WAIT_ARB_UNLOAD      5000 /* 5 seconds*/
#define LOAD_WAIT_ARB_UNLOAD     1000  /* 1 seconds */
#endif
...
#define DTM_PS_RECONFIG_PENDING() ((l1_tsk_buffer.l1_state == L1_DTM_MODE) && \
 ((get_dtm_isr_state() == L1_DTM_PS_RECONFIG) || \
  (get_dtm_isr_state() == L1_DTM_CS_PS_RECONFIG)))\

#define GPRS_PS_RECONFIG_PENDING (L1_TRANSFER_COMMAND == l1_tskisr_blk->main_command)

#define PS_RELEASE_CNF_REQUIRED_BY_ISR() \
  ((l1_transfer_data[()].confirm_pending == SEND_IN_ISR) && \
   (l1_transfer_data[()].release_pending != L2_NO_RELEASE))\
...
#ifdef FEATURE_FSK_SUPPORT
   boolean                    fw_fsk_tx_rsp_received;
#endif
} ISRTIM_CMD_BLK;
...
/* Globals for SCE */
extern boolean l1_get_serv_pwr_meas_permitted(void);
extern void l1_set_serv_pwr_meas_permitted(boolean serv_pwr_meas_permitted );


#ifdef FEATURE_GSM_DTM
/* DTM event variable, responsible for l1_dtm_mode state machine transitions */
extern void l1_set_dtm_command_event(l1_dtm_command_event_t dtm_command_event);
extern l1_dtm_command_event_t l1_get_dtm_command_event(void);
extern l1_dtm_mode_state_t get_dtm_isr_state(void);
#endif
...
===========================================================================*/
void l1_init_idle_data(void);
#ifdef FEATURE_GERAN_CS_SUPPORT
void l1_init_dedicated_data(void);
#endif /* #ifdef FEATURE_GERAN_CS_SUPPORT */
ARFCN_T gl1_get_serving_cell_arfcn(void);
dBx16_T gl1_get_serving_cell_rx_power(void);
uint8   gl1_get_serving_cell_BSIC(void);
void gpl1_gprs_setup_idle_non_drx(void);

gs_status_T gl1_set_sacch_tx_pwr_control( boolean tx_pwr_control );
boolean     gl1_sacch_tx_pwr_control_is_active(void);

boolean gl1_cell_valid_state_check( const byte gl1_state );

void  gl1_mutex_lock   (void);
void  gl1_mutex_unlock (void);
...
***************************************************/
void        L1_send_MPH_SURROUND_MEAS_IND(void);
void        L1_send_MPH_NC_MEASUREMENT_IND(void);
void        L1_send_MPH_SURROUND_UPDATE_FAILURE_IND( ARFCN_T ARFCN, boolean state_swap   );

void        L1_send_PH_CONNECT_IND(l2_channel_type_T chnl_type  );
void        L1_send_PH_READY_TO_SEND_IND(l2_channel_type_T chnl_type  );
...
void        L1_send_MPH_SERVING_IDLE_MEAS_IND( byte   );
void        L1_send_MPH_SERVING_AUX_MEAS_IND( boolean own_paging_block   );
void        L1_send_MPH_READ_SYS_INFO_FAILURE_IND(boolean state_swap, boolean transfer  );

#ifdef FEATURE_GTOW_BACKGROUND_PLMN_SEARCH
void        L1_send_MPH_INTERRAT_LIST_SRCH_TIME_IND(uint32 timer_value  );
void        L1_send_MPH_SUSPEND_GSM_MODE_CNF(uint32 timer_value, boolean suspended  );
void        L1_send_MPH_RESUME_GSM_MODE_CNF(void);
#endif

void        L1_send_MPH_SERVING_MEAS_IND( byte );

#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
extern void L1_send_MPH_SIMULATE_R13_PARAM_FROM_DIAG_IND(uint8 simu_pro, uint8 simu_param  );
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

void        L1_send_L1_ISR_SINGLE_BLK_FAILURE_CNF(void);
void        L1_send_gprs_gmac_message(l1_mac_sig_t *common_msg_ptr, uint8 *pacch_data,uint32 FN  );
...
dBx16_T     L1_pscan_get_threshold(void);
void        L1_pscan_set_threshold(dBx16_T new_threshold  );
...
#ifdef FEATURE_GSM_DTM
void l1_ded_store_ftn_info(gl1_msg_frame_tick_type ftn_type  );
gl1_msg_frame_tick_type l1_ded_get_ftn_info(void);
#endif

#ifdef FEATURE_GERAN_CS_SUPPORT 
void l1_ded_store_bad_sacch_block_cnt(int value  );
int l1_ded_get_bad_sacch_block_cnt(void);
...
boolean l1_tch_is_rfacch_decode_frame(uint16 fn_mod42432, uint8* rfacch_buffer_index  );
void clear_rfacch_decode_frame(uint8 rfacch_buffer_index  );
void l1_tch_set_rfacch_fw_buffer_index(uint8 index  );
uint8 l1_tch_get_rfacch_fw_buffer_index(void);

extern void l1_rsacch_init_params (boolean ded_active  );
...
void  L1_send_LTE_CPHY_IRAT_MEAS_G2L_INIT_REQ( boolean  CM_mode   );


void  L1_send_LTE_CPHY_IRAT_MEAS_G2L_DEINIT_REQ(void);

void  L1_send_LTE_CPHY_IRAT_MEAS_G2L_ABORT_REQ(void);

void  l1_sc_lte_process_search_results( const lte_cphy_irat_meas_search_cnf_s*  msg_ptr   );

void  l1_sc_lte_process_meas_results( const lte_cphy_irat_meas_meas_cnf_s*  msg_ptr   );

boolean L1_send_LTE_CPHY_IRAT_MEAS_G2L_TIMED_SRCH_MEAS_REQ( uint16 freq_index, boolean ml1_gap_init, lte_earfcn_t nextFreq_t  boolean gap_start_asap);

void    l1_sc_lte_process_timed_search_results(lte_cphy_irat_meas_timed_srch_meas_cnf_s*  msg_ptr    );
...
void  L1_send_TDS_IRAT_BUILD_RF_SCRIPTS_CMD (void);

void  L1_send_TDS_IRAT_STARTUP_REQ (gsm_l1_tds_gap_info_type *gap_info_ptr_t  );

void  L1_send_TDS_IRAT_ACQ_REQ (const tdsirat_acq_list_type * acq_list_ptr_t  );

void  L1_send_TDS_IRAT_MEAS_REQ (const tdsirat_freq_list_type * meas_list_ptr_t  );

void  L1_send_TDS_IRAT_STOP_CMD (void);

void  L1_send_TDS_RSSI_MEAS_RSP ( gl1_hw_x2g_rssi_meas_ctl_t *ptr );

void L1_send_TDS_G2T_ABORT_REQ (void);
...
void  l1_sc_tds_process_acq_results( const tdsirat_freq_list_type*  acq_rsp_ptr_t   );

void  l1_sc_idle_tds_acq_results( const tdsirat_freq_list_type*  acq_rsp_ptr_t   );

void  l1_sc_ded_tds_acq_results( const tdsirat_freq_list_type*  acq_rsp_ptr_t   );

void  l1_sc_tds_process_meas_results( const tdsirat_freq_list_type*  meas_rsp_ptr_t   );

void  l1_sc_idle_tds_meas_results( const tdsirat_freq_list_type*  meas_rsp_ptr_t   );

void  l1_sc_ded_tds_meas_results( const tdsirat_freq_list_type*  meas_rsp_ptr_t   );

void l1_sc_tds_process_abort(void);
void l1_sc_g2t_shutdown (void);
...
#endif
#ifdef FEATURE_FSK_SUPPORT
   rfa_rf_gsm_enter_mode_cnf_s            rfa_rf_fsk_enter_mode_cnf;
#endif
...
uint32  debug_mode_freq;
uint8   debug_mode_delta;     /*Delta of frequency in kHz*/
uint8   baud_rate; /*0-3466bps(16bits/frame) 1-1733bps(8/frame) 2-866bps(4/frame)*/
uint8   gap_time_frames; /*stored in number of frames*/
uint8   pcl_fix[3];
uint8   data_index;
uint8   data[2][200];
uint8   data_length[2];
boolean new_data_received;
uint8   current_freq_index;
uint8   next_freq_i;
...
uint8 delta;
uint8 num_freq;
uint32 freq_i[FSK_MAX_FREQ_ARRAY_LENGTH_PER_INDEX];
} gl1_fsk_hop_table_type;

typedef struct
{
boolean valid;
gl1_fsk_hop_table_type hop_table[FSK_MAX_FREQ_INDEX];
}gl1_fsk_hop_table_db_t;
#endif
...
===========================================================================*/
extern void l1_idle_saic_disable_timer_start(void);
...
===========================================================================*/
extern void l1_idle_saic_disable_timer_deactivate(void);

/*===========================================================================
FUNCTION l1_idle_saic_disable_timer_create

DESCRIPTION
  Undefines the idle saic disable timer
===========================================================================*/
void l1_idle_saic_disable_timer_deinit(void);
...
===========================================================================*/
extern void l1_idle_saic_disable_timer_create(void);

#ifdef FEATURE_WTOG_BACKGROUND_PLMN_SEARCH
extern uint32  l1_get_drx_interval(void);
extern void    l1_decrement_drx_interval(void);
extern void    l1_subtract_drx_interval( uint32 sub_t   );
extern void    l1_set_drx_interval( uint32 val );
extern boolean l1_get_bplmn_suspend_in_progress(void);
extern void    l1_set_bplmn_suspend_in_progress(boolean l1_bplmn_suspend_in_progress);
#endif /* FEATURE_WTOG_BACKGROUND_PLMN_SEARCH */
...
#define GERAN_UNLOAD_TIME          40

extern uint32 g_gps_min_wait;
extern uint32 g_gps_load_wait ;
extern uint32 g_gps_overhead_FN;
extern uint32 g_gps_load_frames;
extern uint32 g_total_sleep_frames_in_unload;

#ifdef FEATURE_FSK_SUPPORT
extern gl1_fsk_db_t gl1_fsk_db;
extern gl1_fsk_hop_table_db_t gl1_fsk_hop_table_db;
#endif

extern void gl1_drx_reset_tick(void);
extern void gl1_hw_wmgr_drx_require_tick(uint32 reprog_fn);
	
uint32 gl1_drx_get_next_wakeup_FN(void);
extern uint32 gl1_idle_get_gap_to_next_activity(uint32 frame_offset);
extern void gl1_hw_reconfigure_slpc_dmm (void);
#endif
void gl1_clkdata_speed(boolean dataSpeedReq, uint16 speedState, uint16 prevSpeedState );

void gl1_dtm_speedup_notify(void);
void gl1_clkdata_state_init(void);

extern void    l1_set_asynch_int_possible(void);
extern boolean l1_get_asynch_int_possible(void);
extern byte    l1_get_current_rr_l1_msg_id(void);

extern boolean l1_get_epd_pref(void);
extern void  l1_set_epd_pref(rr_l1_epd_preference_t epdpref   );

extern boolean l1_is_cell_acq_complete(void);

#ifdef FEATURE_GSM_ADAPTIVE_TMLN
boolean gl1_check_if_current_activity_is_xcch(void);
...
boolean l1_get_qta_active(void);
#endif

void L1_send_X2G_CM_GSM_INIT_RSP(    rfcom_mode_enum_type  serving_rat );
...
void    l1_sc_send_irat_g2l_init(boolean g2l_neighbours    );
void gl1_hw_abort_idle_act_prog(void);
boolean gl1_check_if_current_activity_is_xcch(void);
void gl1_update_scheduled_activity(void);

void l1_call_in_task_turn_tx_off(void );

void gl1_ds_jammer_notify(void);
#ifdef FEATURE_GERAN_CT20_MODEM
void gl1_qmi_nas_send_jamming_ind(uint32 jammer_status);
#endif /*FEATURE_GERAN_CT20_MODEM*/
#endif /* L1I_H */
