...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/gpl1_gprs_isr.h#3 $
...
void     prach_pbcch_pccch_data( gl1_defs_rx_pkt_data_type  *msg_data,
                                 boolean                     valid );
prach_params_T*        gl1_ms_switch_prach_params_idle_if_data(void);
/****************** Packet Transfer mode Related functions *********************/

boolean  gpl1_gprs_control_transfer( l1_mode_command_T        mode);

void gpl1_gprs_notify_start(transfer_data_T *transfer_data_ptr );

void gpl1_gprs_handle_tbf_complete(transfer_data_T *transfer_data_ptr  );

boolean gpl1_gprs_handle_reconfig( transfer_data_T*   transfer_data_ptr,
                                       l1_mode_command_T  mode,
                                       uint8 l1_state  );
const gpl1_ps_store_T *  gpl1_read_xfer_isr_store(void);


void gpl1_gprs_update_usf_det( boolean  usf_detected  );

void gpl1_gprs_update_first_rx( boolean first_rx);

void gpl1_gprs_update_enable_tx( boolean enable_tx );
...
void gpl1_gprs_update_num_rx_tn( uint8  num_rx_tn );

void gpl1_gprs_update_abort_callbacks( boolean  abort_callbacks );

void gpl1_gprs_init_metrics_cb_store(void);

void gpl1_gprs_init_data_cb_store(uint8 index, gl1_defs_rx_pkt_data_type *buff_ptr );

void gpl1_gprs_init_dl_fifo(void);
...
RETURN VALUE
===========================================================================*/
uint8 gpl1_get_ul_slammed_tn(uint8 tn_index);
#endif
...
#define GPL1_RESET_DEFER_BUFF()  gpl1_gprs_update_deferred_buff(NULL,FALSE,RESET_BUFFER)
#define GPL1_SET_ABORT_FLAG() gpl1_gprs_update_deferred_buff(NULL,TRUE,UPDATE_ABORT_FLAG)
#define GPL1_CLEAR_ABORT_FLAG() gpl1_gprs_update_deferred_buff(NULL,FALSE,UPDATE_ABORT_FLAG)

#define GPL1_DISABLE_TX()  gpl1_gprs_update_enable_tx(FALSE)
#define GPL1_ENABLE_TX()   gpl1_gprs_update_enable_tx(TRUE)

#define GPL1_RESET_USF_DETECT()  gpl1_gprs_update_usf_det(FALSE)
#define GPL1_SET_USF_DETECT()  gpl1_gprs_update_usf_det(TRUE)
#define GPL1_RESET_ABORT_CB()  gpl1_gprs_update_abort_callbacks(FALSE)
#define GPL1_SET_ABORT_CB()  gpl1_gprs_update_abort_callbacks(TRUE)
...
boolean gpl1_gprs_awaiting_release(void);

boolean gpl1_gprs_leaving_transfer(void);

#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )

boolean gpl1_gprs_reconfig_progress( uint32 frames_required );
 ...
 * block statistics.
 */
void  gpl1_reset_uplink_tbf_blk_count(void);
void  gpl1_increment_uplink_tbf_data_blk_count(void);
void  gpl1_increment_uplink_tbf_dummy_blk_count(void);

       /**************** DTM mode Related functions ****************/
#ifdef FEATURE_GSM_DTM

l1_dtm_mode_T gpl1_dtm_control(  void );
l1_dtm_mode_T gpl1_dtm_control_cs_ps(dtm_data_T   *dtm_data, boolean apply_freq_redef  );
boolean gpl1_dtm_complete_abort_procedure( l1_dtm_mode_T abort_status   );
void gpl1_dtm_invalidate_mac_l1_release_from_message_store(void);

#ifdef  FEATURE_GSM_EDTM

extern void  gpl1_decrement_fn_on_next_tbase_change(void);
 ...
 * boundaries.
 */
void  gpl1_pacch_fifo_delete_msg( uint8         csn_msg_type);

/* This function queues the mac l1 delete ul ctrl msg */
extern void  gpl1_queue_mac_l1_delete_ul_ctrl_msg( uint8  message_type_t  );

/* This function removes the mac l1 delete ul ctrl msg */
extern void  gpl1_remove_mac_l1_delete_ul_ctrl_msg(void);

extern const q_mac_l1_delete_ul_ctrl_msg_t*  gpl1_read_mac_l1_delete_ul_ctrl_msg(void);

#endif

#endif

extern void  gl1_egprs_open_srb_loopback(void);
...
void     gpl1_gprs_transfer_send_ftn( uint8             *pacch_pdtch_ul_blk_start_cnt,
                                      ...
                                      l1_mode_command_T  mode  );
void     gpl1_gprs_config_agc(void);

uint8*   gpl1_gprs_get_ul_data( gl1_defs_coding_type *coding_scheme );

void gpl1_gprs_get_arfcns(int16 start, uint16 increment, uint16 num_arfcns, ARFCN_T arfcns[] );


void     gpl1_gprs_reset_pacch_fifo(void);
...
boolean  gpl1_gprs_check_tbf_suspension( uint32  start_frame,
                                         uint8   no_of_frames,
                                         boolean check_ptcch_t  );

void     gpl1_gprs_init_tbf_suspension(void);

void     gpl1_gprs_set_serving_psi1_tbf_suspension( uint32 psi1_fn );

uint32   gpl1_gprs_get_serving_psi1_fn( void );

boolean  gpl1_get_tbf_resumption_fn(uint32 *resumption_frame_nbr);
...
void  gpl1_gprs_send_l1_mac_single_block_sent(void);

void  gpl1_gprs_send_rlc_ph_ready_to_send_ind( void );

void  gpl1_gprs_send_l1_mac_ph_connect_ind( uint16 tbf_type);
#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )
void gpl1_gprs_send_l1_mac_ph_connect_ind_trm_failed(uint16 tbf_type   );
...
void  gpl1_gprs_send_l1_mac_usf_detected(void);

void  gpl1_gprs_send_l1_mac_tbf_rel_confirm( release_T tbf_type);

#ifdef FEATURE_GSM_GPRS_READ_CCCH_IN_XFER
boolean gpl1_conflict_check_for_ccch (void);
...
void gpl1_send_mac_ph_cs_connect_ind( uint8 cs_timeslot );
...
void l1_sc_balist_gprs_set_sync_in_task( void *);
void gpl1_gprs_change_timeslot ( byte new_ts   );

/*  Neighbour Cell Measurements */
boolean  gpl1_gprs_measurements( l1_mode_command_T);
...
uint8  gpl1_gprs_ts_conv       ( uint8  un_slammed  ); // kf time slot slamming
uint8  gpl1_gprs_ts_conv_unslam( uint8  slammed); // kf time slot un slamming

void  gpl1_transfer_service_pending_sysinfo_reqs(void);
...
RETURN VALUE
===========================================================================*/
void gpl1_reset_stop_sc_tick_trans(void);
...
RETURN VALUE
===========================================================================*/
void gpl1_stop_sc_tick_trans(void);
void gpl1_start_sc_tick_trans(void);
boolean gpl1_get_stop_sc_tick_trans(void);
...
===========================================================================*/
boolean gpl1_get_egprs_ir_mem_status(void);
