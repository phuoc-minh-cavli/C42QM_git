...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/gpl1_gprs_task.h#2 $
...
#define SET_SINGLE_BLOCK_RECV()  (l1_transfer_data.config_bitmap |= 0x1)
#define SET_DYNAMIC_TBF_RECV()   (l1_transfer_data.config_bitmap |= 0x4)
#define SET_DOWNLINK_TBF_RECV()  (l1_transfer_data.config_bitmap |= 0x8)
#define SET_FREQ_CONFIG_RECV()   (l1_transfer_data.config_bitmap |= 0x10)
#define SET_DUMMY_CTRL_RECV()    (l1_transfer_data.config_bitmap |= 0x20)
#define SET_UPLINK_DATA_VALID()  (l1_transfer_data.config_bitmap |= 0x40)
#define RESET_BITMAP()           (l1_transfer_data.config_bitmap = 0)

/*  Check to see if the above bits have been set. */
#define SINGLE_BLOCK_RECV()          ((l1_transfer_data.config_bitmap & 0x1) ? TRUE : FALSE)
#define FREQ_CONFIG_RECV()           ((l1_transfer_data.config_bitmap & 0x10) ? TRUE : FALSE)
#define UPLINK_DATA_VALID()          ((l1_transfer_data.config_bitmap & 0x40) ? TRUE : FALSE)
#define DUMMY_CTRL_RECV()            ((l1_transfer_data.config_bitmap & 0x20) ? TRUE : FALSE)

/*  Clear specific bits. Only valid if less than 8 bits used */
#define CLEAR_SINGLE_BLOCK_RECV()    (l1_transfer_data.config_bitmap &= 0xfe) 
#define CLEAR_FREQ_CONFIG_RECV()     (l1_transfer_data.config_bitmap &= 0xef) 
#define CLEAR_UPLINK_DATA_VALID()    (l1_transfer_data.config_bitmap &= 0xbf) 

/* used to check if allocations are complete
 * so that L1_TRANSFER state can be entered
 */
#define SINGLE_BLOCK_ALLOC_COMP()    ((l1_transfer_data.config_bitmap & 0x51) == 0x51 ? TRUE : FALSE)
#define SINGLE_DL_BLOCK_ALLOC_COMP() ((l1_transfer_data.config_bitmap & 0x11) == 0x11 ? TRUE : FALSE)
#define DYNAMIC_TBF_ALLOC_COMP()     ((l1_transfer_data.config_bitmap & 0x34) == 0x34 ? TRUE : FALSE)
#define DOWNLINK_TBF_ALLOC_COMP()    ((l1_transfer_data.config_bitmap & 0x18) == 0x18 ? TRUE : FALSE)
 ...
 */
#define MULTI_BLOCK_ALLOC_COMP()     ((l1_transfer_data.config_bitmap & 0x71) == 0x71 ? TRUE : FALSE)
...
/* PROTOTYPES */
boolean gpl1_gprs_handle_tbf_release(release_tbf_t   release_tbf );
boolean gpl1_gprs_check_frequency_params(frequency_list_T *freq_list, boolean hopping_flag );
void L1_send_l1_mac_freq_ass_conf(byte error,tbf_freq_cnf_t tbf_type );
...
void    gpl1_gprs_report_period_done_trans(void);
void    gpl1_gprs_serving_cell_meas_if(void);
void    gpl1_gprs_handle_reselection_bcch_mode(mph_gprs_reselection_req_t *msg_ptr);

void gpl1_gprs_send_mac_51_sb_failure(void *dummy_param );
gprs_l1_idle_data_t* gl1_ms_switch_gprs_idle_data(void);
...
void gpl1_gprs_send_l1_mac_psho_cnf(psho_status_t psho_status );
#endif
...
void gpl1_dtm_set_current_rr_mac_l1_msg_id( IMH_T message  );
void gpl1_dtm_data_init(void);
void gl1_complete_channel_assignment (void);
void gl1_complete_channel_handover ( boolean  apply_afc  );
void gl1_complete_channel_reconnect(void);
#endif

/* Functions used to reset critical data variables and flags */
void gpl1_dynamic_data_init(void);
void gpl1_single_block_data_init(void);
