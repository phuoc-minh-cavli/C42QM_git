...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/l1_task.h#9 $
...
10/04/17   nm       CR2028516 Detect bsic change and send indication to GRR
...
/* IRAT status flags */
boolean l1_sc_get_wcdma_update_list_irat_active(void);
void    l1_sc_set_wcdma_irat_active(boolean value);
void    l1_sc_set_wcdma_mcpm_irat_active(boolean value);
boolean l1_sc_check_irat_active(void);

/* Only need 1 flag for L or T */
void    l1_sc_set_skip_g2x_pkt_txfr(boolean value);
void    l1_sc_ts_changed(void);

#ifdef  FEATURE_GSM_TO_LTE
boolean l1_sc_get_lte_irat_active(void);
#endif
   ...
   * are active
   */
boolean l1_sc_get_tds_irat_active(void);
void    l1_sc_set_tds_irat_active(boolean value);
void    l1_sc_set_tds_issue_cleanup(boolean value);
boolean l1_sc_get_tds_issue_cleanup(void);
void    l1_sc_set_tds_state(byte value);
byte    l1_sc_get_tds_state(void);
extern boolean l1_rcvd_non_drx_ind;
#endif

#ifdef FEATURE_WTOG_BACKGROUND_PLMN_SEARCH
extern void l1_set_wtog_bplmn_task_state(background_HPLMN_activity_type state);
extern void l1_set_wtog_bplmn_task_substate(l1_wtog_bplmn_sub_state_T sub_state);
extern background_HPLMN_activity_type l1_get_wtog_bplmn_task_state(void);
extern l1_wtog_bplmn_sub_state_T l1_get_wtog_bplmn_task_substate(void);
#endif
...
extern boolean l1_get_background_wcdma_plmn_scan_active(void);
extern void l1_set_background_wcdma_plmn_scan_active(boolean background_wcdma_plmn_scan_active);
#endif

void l1_set_rach_ready_to_send( boolean ready_to_send );
...
===========================================================================*/
void l1_rach_timer_create(void);
void l1_rach_timer_deinit(void);
...
void     L1_handle_idle_mode(IMH_T *);
...
boolean  L1_select_next_ARFCN(word   *ARFCN_ptr );
void     L1_send_ISRTIM_FIRST_FCB_REQ(int ARFCN_index );
void     L1_send_ISRTIM_SCH_REQ(int ARFCN_ptr );
void     L1_send_ISRTIM_ACQ_REQ(ARFCN_T ARFCN );
void     L1_send_ISRTIM_ACQ_LIST_REQ(uint16 ARFCN_index );
void     L1_send_ISRTIM_DECODE_BCCH_LIST_REQ(  void );

void     L1_send_ISRTIM_SCAN_REQ( byte command );
...
void     L1_send_MPH_RESET_IND(void);
void     L1_send_MPH_RESET_CNF(void);
void     L1_send_MPH_POWER_SCAN_CNF( boolean l1_suspending );

void     L1_send_MPH_SELECT_SPECIFIC_BCCH_CNF(boolean found, byte *sch_block);
void     L1_send_MPH_RESELECTION_CNF(boolean found);

void     L1_send_MPH_DECODE_BCCH_LIST_CNF( boolean l1_suspending );
void     L1_send_MPH_DECODE_BCCH_LIST_FAILURE_IND (ARFCN_T arfcn, rr_l1_decode_bcch_failure_T failure_cause);
void     L1_send_MPH_FREQUENCY_REDEFINITION_IND( boolean channels_in_same_band, boolean starting_time_elapsed );
void     L1_send_MPH_COEX_MSC_CHANGE_IND(uint8 ul_slot_restriction, uint8 dl_slot_restriction);

void     L1_send_MPH_BSIC_CHANGE_IND(void);

void     L1_send_MPH_FCCH_SCH_DECODE_IND(ARFCN_T arfcn, 
                                                      byte bsic, 
                                                      byte rxlev, 
                                                      uint16 fcch_snr
...
void     L1_send_ISR_DEACT_CNF ( boolean deep_sleep );


void     L1_send_MPH_CHANNEL_MODE_MODIFY_CNF(byte error);
void     L1_send_MPH_HANDOVER_IND(byte error);
void     L1_send_MPH_HANDOVER_CNF(dword ob_timing_diff, byte prev_chnl_tmng_adv);
void     L1_send_MPH_STOP_HANDOVER_ACCESS_CNF(dword ob_timing_diff, byte  prev_chnl_tmng_adv);

void     L1_send_MPH_IMMED_ASSIGN_CNF(byte error);
...
===========================================================================*/
void     l1_sc_reset_G2T_state(void);
#endif

void     l1_sc_wcdma_set_mode_from_init(boolean gsm_only);

void     L1_send_MPH_UE_MODE_CHANGE_CNF (void);

void     l1_sc_mode_cnf_pending(boolean SetPending , uint16 ratInactiveBit);

boolean  l1_sc_g2x_shutdown_cnf_pending(void);
...
#endif
boolean  L1_non_connected_state(void);

extern void L1_handle_MPH_DSC_THRESHOLD_IND( IMH_T *msg_header );
extern void L1_enable_dual_receive(void);

void     L1_handle_inactive_mode(IMH_T *);
void     L1_handle_MPH_START_GSM_MODE_REQ ( IMH_T  *msg_header );
void     L1_send_MPH_START_GSM_MODE_CNF (void);
void     L1isr_send_MPH_STOP_GSM_MODE_CNF (void);
void     gsm_l1_wait (rex_sigs_type mask);
#ifdef FEATURE_FSK_SUPPORT
void     L1_handle_task_fsk_start( void );
#endif

#ifdef FEATURE_GSM_DTM
void gpl1_gprs_handle_dtm_mode(IMH_T *);
void gl1_save_current_cell_data(boolean store_afc_info);
#endif

void     L1_send_MPH_START_IDLE_MODE_CNF(void);
void     L1_send_MPH_ABORT_RA_CNF( gl1_mode_t l1_mode);


void l1_isr_watchdog_kick (void);
void l1_isr_watchdog_activate (void);
void l1_isr_watchdog_deactivate (void);
void l1_isr_watchdog_deinit(void);

#ifdef FEATURE_GTOW_BACKGROUND_PLMN_SEARCH
void L1_resume_req(void);
void L1_setup_SUSPEND(void);
void L1_handle_suspend_mode(IMH_T  *msg_header);
void L1_send_ISR_SUSPEND_CNF ( uint32 timer_value );
#endif

#ifdef FEATURE_WTOG_BACKGROUND_PLMN_SEARCH
void L1_send_MPH_POWER_SCAN_FOR_BPLMN_CNF( boolean send_decode_bcch_list_req );
void L1_send_MPH_START_GSM_MODE_FOR_BPLMN_CNF (void);
void L1_send_MPH_RESUME_GSM_MODE_FOR_BPLMN_CNF (void);
void L1_send_MPH_SUSPEND_GSM_MODE_IND (void);
...
void L1_send_MPH_NV_REFRESH_CNF(void);
#endif /* FEATURE_MODEM_CONFIG_REFRESH */

#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )
void L1_send_MPH_L1_DS_ABORT_IND(  rr_ds_abort_type_T abort_cause, uint32 timeout);
#endif /* FEATURE_DUAL_SIM | FEATURE_TRIPLE_SIM */

void l1_set_grm_status( gl1_trm_state_t trm_status );

#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
void L1_send_MPH_SURROUND_IRAT_MEAS_IND( rr_l1_irat_rat_e rat, const rr_l1_irat_measurements_u*  measurements_ptr);
void L1_send_not_found_MPH_SURROUND_DED_ID_MEAS_IND( rr_l1_irat_rat_e rat, uint32 uearfcn );
#endif

void l1_do_panic_reset_after_delay(void);
boolean l1_check_for_panic_reset(void);
uint32 l1_get_panic_reset_received_frame_no(void);
void l1_set_panic_reset_received_frame_no(  uint32 frame_no);
void l1_reset_panic_reset_received_frame_no(void);

/* shared functions between l1_sc_wcdma.c and sc_trans_mease.c*/
boolean l1_sc_wcdma_tick_idle_id_in_tbf( uint16_t );

/* This function is used to allow clients to reset the
 * current_rr_l1_msg_id variable.
 */
extern void  l1_reset_rr_current_msg_id(void);

extern void l1_sc_g2x_pending_shutdown(void);

extern void l1_sc_mcpm_w_meas_stop(void);
#ifdef FEATURE_CGPS_TIME_TAGGING
extern void init_l1_utils_data(void);
#endif /* FEATURE_CGPS_TIME_TAGGING */
extern void init_gpl1_gprs_ptcch_data(void);
extern void init_local_idle_data(void);
extern void init_l1_bcch_list_data(void);
extern void init_l1_acq_data(void);
extern void init_gpl1_gprs_serving_cell_data(void);
extern void init_gpl1_gprs_single_data(void);
extern void init_l1_isr_data(void);
extern void init_l1_multi_sim(void);
extern void init_sc_wcdma_data(void);

extern void L1_sdcch_init_data_struct(void);
extern void L1_tch_init_data_struct(void);
extern void l1_tch_amr_init_data(void);
extern rex_tcb_type* l1_task_tcb_read(void);

#ifdef FEATURE_FSK_SUPPORT
extern void gl1_hw_tcxo_inact(boolean deep_sleep);
extern void gl1_hw_fsk_exit_mode_cmd(void);
extern void gl1_hw_rf_task_fsk_exit_mode (
...
extern void mdsp_fsk_write_tx_data(uint8* msg, uint8 data_length);
#endif

void    l1_acch_nv_flag_initialise      (void);
...
void start_tx_div_antenna_alg ( boolean packet_trans );
...
*/
void   gl1_handle_message_router_msgs(void);
#endif
...
void gpl1_gprs_handle_rlc_ph_data_req_sig(void);
void gsm_l1_report_watchdog(void);
void l1_isr_watchdog_check (void);
