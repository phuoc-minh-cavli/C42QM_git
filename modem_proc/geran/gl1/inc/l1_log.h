...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/l1_log.h#6 $ $DateTime: 2019/03/07 08:05:30 $ $Author: pwbldsvc $

when       who      what, where, why
--------   ---      ----------------------------------------------------------
07/03/19   kh       CR2381037 Realigning data structures for Padding reduction
18/04/15   cjl      CR891663 Recover unused l1_RR_log_msg buffer space
...
#define L1_MSG_LOG_LEN 30
#define L1_MSG_RR_L1_LOG_LEN  15 
typedef struct { 
   uint32 FN;
     uint16 msg_set;
     uint16 msg_ID;
...
#define L1_FM_LOG_LEN 30

typedef struct {
   uint32 FN;
     uint32 fm_task:8;
     uint32 fm_state:8;
     uint32 fm_frames_reqd:8;
     uint32 event:8;
} fm_log_element_T;
#endif
...
#endif
#ifdef L1_MSG_TRAFFIC_LOGGING
     l1_msg_traffic_log_T l1_RR_log_msg[L1_MSG_RR_L1_LOG_LEN];
...
#ifdef FEATURE_GL1_VAR_LOGGING
#define L1_LOG_VAR(status,fname, var_id)      l1_var_log(status,__LINE__,fname,var_id);
#else
#define L1_LOG_VAR(status,fname, var_id)      ((void)(0));
#endif
...
void l1_log_cxm_table_init(void);
...
void    l1_log_power_scan             (void);
void    l1_log_agc_params             ( void );
void    l1_log_afc_params             ( void );
void l1_log_sacch_burst_metrics (gl1_msg_metrics_rpt *rpt_ptr[] );
...
void    l1_log_message_metrics        ( gl1_msg_chan_type      channel,
                                        gl1_msg_decode_rpt    *rpt_ptr  );

void  l1_log_message_metrics_compact  ( gl1_msg_chan_type   channel,
                                        gl1_msg_decode_rpt *rpt_ptr  );

void l1_log_rfacch_metrics(gl1_msg_decode_rpt *rpt_ptr  );
...
boolean l1_log_paging_data            (void);
void    l1_log_calibration_values     ( void );
boolean l1_log_ncell_engine_mode      (void);
void    l1_log_ncell_ba_list          (void);
void    l1_log_ncell_acq ( byte mode, uint8 rank, ARFCN_T arfcn, dBx16_T rx_power  );
boolean l1_log_serving_cell_power     (void);
void    l1_log_tx_lev_and_ta ( byte tx_lev, byte ta  );

/* EVENTS */
void    l1_log_state                  (void);

#ifdef FEATURE_GSM_AMR
void l1_log_ratscch_in_dtx (void);
void l1_log_facch_in_dtx (void);
void l1_log_facch_and_ratscch_collision (void);
void l1_log_facch_and_sid_update_collision (void);
void l1_log_ratscch_and_sid_update_collision (void);
void l1_log_amr_state_change (l1_log_amr_state_change_T *data_t );
void l1_log_amr_cmi_phase_change(l1_log_amr_cmi_phase_change_T *data_t );
void l1_log_amr_req_act_timer_expiry(l1_log_amr_req_act_timer_expiry_T *data_t );
void l1_log_amr_ack_act_timer_expiry(l1_log_amr_ack_act_timer_expiry_T *data_t  );
void l1_log_amr_ratscch_req(l1_log_amr_ratscch_req_T *req_t );
void l1_log_amr_ratscch_rsp( uint8 rsp_t  );
void l1_log_amr_cmc_turnaround_time ( uint16 elapsed_frames_t  );
#endif /* FEATURE_GSM_AMR */

void l1_log_serving_aux_meas (void);
void l1_log_ncell_aux_meas (void);
void l1_log_ncell_bcch_burst_metrics(gl1_msg_metrics_rpt *rpt[] );
void l1_log_ncell_bcch_msg_metrics (ccch_decode_rpt *rpt );
void l1_log_ncell_sch_burst_metrics(gl1_msg_metrics_rpt *rpt );
void l1_log_ncell_sch_msg_metrics (sch_decode_rpt *rpt );

#ifdef L1_MSG_TRAFFIC_LOGGING
void l1_log_msg_traffic(uint8 msg_set, uint8 msg_ID, uint32 FN, boolean incoming_message );
#endif

#ifdef FEATURE_GL1_VAR_LOGGING
void l1_var_log( uint8 var,uint32 linenum,l1_fname_T fname,uint16 var_id);
void l1_rcv_log( uint8 recive_in_progress,uint32 linenum, l1_fname_T fname);
void l1_monscan_log( uint8 monscan_in_progress,uint32 linenum,l1_fname_T fname );
#if defined ( FEATURE_WCDMA ) || defined ( FEATURE_GSM_TDS )|| defined ( FEATURE_GSM_TO_LTE )
void l1_wsrch_id_log( uint8 wcdma_srch_id_progress,uint32 linenum,l1_fname_T fname );
void l1_wsrch_log( uint8 wcdma_srch_in_progress ,uint32 linenum,l1_fname_T fname );
#endif
void l1_idle_monitors_log( uint8 idle_moniters ,uint32 linenum,l1_fname_T fname );
#endif

void l1_log_packet_init(gsmdiag_pkt_info_t *info_ptr, log_code_type pkt_id, unsigned int pkt_size );

uint8 l1_log_get_TxPa_state(void);   /* CMAPI feature for Tx pa gain state */

void l1_log_convert_txlev_to_pwr_dBm(eng_mode_sys_band_T band, uint8 txlev, uint8 *power_out);

void l1_log_signal_info(geran_eng_mode_signal_info_t *signal_info_ptr);

#ifdef FEATURE_GSM_LOG_COEX
  void l1_log_coex_params(volatile GfwCxmLogPacketBuff* cxm_log_buf_ptr_t );
#endif
