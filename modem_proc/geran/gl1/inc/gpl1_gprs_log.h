...
void gpl1_log_gprs_burst_metrics_a ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_b ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_c ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_d ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_e ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
#if 0
void gpl1_log_gprs_burst_metrics_a_ver2 ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_b_ver2 ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_c_ver2 ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );
void gpl1_log_gprs_burst_metrics_d_ver2 ( gl1_msg_dl_chn_type channel,uint32 fn, const gl1_defs_rx_pkt_metrics_type *rpt_ptr_t  );

void gpl1_log_gprs_message_metrics_b ( gl1_msg_dl_chn_type   channel, uint32 fn,  const gl1_defs_rx_pkt_data_type *rpt_ptr_t  );
void gpl1_log_gprs_message_metrics_c ( gl1_msg_dl_chn_type   channel, uint32 fn,  const gl1_defs_rx_pkt_data_type *rpt_ptr_t  );
void gpl1_log_gprs_message_metrics_d ( gl1_msg_dl_chn_type   channel, uint32 fn,  const gl1_defs_rx_pkt_data_type *rpt_ptr_t  );
#endif
void gpl1_log_gprs_message_metrics_a_ver2 ( gl1_msg_dl_chn_type   channel, uint32 fn,  const gl1_defs_rx_pkt_data_type *rpt_ptr,
                                            const usf_dl_tfi_info_t         *usf_dl_tfi_info_t  );
...
void  gpl1_log_edge_message_metrics  ( gl1_msg_dl_chn_type channel, uint32 fn, byte tn,
                                       const GfwEnhancedEdgeLogPacket *edge_data_t );
void gpl1_log_srb_hard_decisions  ( uint32 FN, uint16* HD_ptr, uint16 mcs_t  );

#ifdef FEATURE_GSM_LOG_IQ_SAMPLES
void gpl1_log_mdsp_debug_buffer( uint16 *MDSP_buffer_p, uint32 FN,
                                 uint16 TSC, uint16 channel_t  );
#endif /* FEATURE_GSM_LOG_IQ_SAMPLES */

void gpl1_log_gprs_air_interface_summary(boolean transfer_ind_t );
void gpl1_log_agc (const sys_algo_agc_T *agc_ptr, const gprs_agc_info_t *gprs_agc_info_t  );

void gpl1_log_timing_advance ( const ta_description_T*  active_ta_params, uint8  ta_value_t  );

void gpl1_log_gprs_power_control(gl1_msg_ul_chn_type channel, gl1_defs_tn_type tn, dBx16_T tx_pwr, gl1_defs_coding_type cs_t );
void gpl1_log_gprs_transfer_summary(void);
void convert_txlev_to_pwr_dBm(sys_band_T band, uint8 txlev, uint8 *power_out);

/* Uplink tbf data/dummy block count */
void  gpl1_log_uplink_tbf_data_block_count( const ul_data_blk_count_t*  ul_data_blk_count_ptr_t  );
