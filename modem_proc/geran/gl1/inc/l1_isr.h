...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/l1_isr.h#4 $
...
23/08/18   akm      CR2298585 CBCH featurization in GERAN L1
...
extern boolean                  dtm_assign_cs_present;

extern l1_serving_cell_meas_T   l1_serving_cell_meas;

extern byte l1_get_current_timeslot(void);
extern void l1_set_current_timeslot(byte current_timeslot );
...
extern boolean l1_get_cch_ciphering_flag(void);
extern void l1_set_cch_ciphering_flag(boolean cch_ciphering_flag );
extern  boolean cch_ciphering_flag;
extern  byte  no_of_messages;

/* Globals for L1_access_isr */

extern void l1_set_rach_read_all_AGCH(boolean rach_read_all_AGCH);
...
extern uint32 l1_get_ind_timer_value(void);
extern uint32 l1_get_ind_timer_FN(void);
#endif
#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )
extern void l1_rach_timer_start(void);
extern void l1_rach_timer_deactivate(void);
...
boolean l1_check_RR_idle_mode_meas_enabled(void);
void l1_set_L1_ready_for_sc_start(l1_sc_mode_T mode);
void l1_clear_L1_ready_for_sc_start(void);
void l1_set_L1_ready_for_go(boolean L1_ready_for_sc_go);
void l1_clear_drx_ind_data(void);
#endif /* FEATURE_IDLE_DRX_SCALING */
extern uint8 gl1_get_num_idle_subs( void );
void    l1_tdma_1_ISR( sys_modem_as_id_e_type as_id);
boolean L1_get_bcch( uint32 *frames_until_active_ptr );
uint32 L1_gap_to_next_bcch( uint32 bcch_mask, uint32 ext_bcch_mask );
boolean L1_get_bcch_list( void );
void    L1_get_sch (void);
boolean L1_idle_mode_ISR( boolean leaving_idle_mode );
...
boolean L1_control_full_rate ( dedicated_data_T       *ded_dtr_ptr,
                               ...
                               boolean                 apply_freq_redef);
void    L1_process_EQU_data( byte channel_type, byte  meas_type);
boolean L1_access_ISR(l1_mode_command_T  mode);

void         L1_control_random_access(idle_data_T *data);
extern void  l1_access_isr_set_mac_sb_failure(void);

boolean L1_send_random_burst(void);
boolean L1_determine_time(word Mod42432, word starting_time);
boolean L1_control_dedicated(void);
boolean L1_determine_start_time ( void );
void    L1_change_channel(  dedicated_data_T       *ded_dtr_ptr,
                            channel_information_T  *chnl1_info_ptr, boolean dtm_assignment);
void    L1_change_timeslot( byte new_ts );

#ifdef FEATURE_GSM_GPRS_READ_CCCH_IN_XFER
byte gpl1_read_page_mode(byte *message);
#endif /* FEATURE_GSM_GPRS_READ_CCCH_IN_XFER */

extern void gpl1_stop_sc_tick_trans(void);
extern void gpl1_start_sc_tick_trans(void);

#ifdef FEATURE_GSM_DTM
void    L1_dtm_change_timeslot( byte new_ts, channel_information_T  *chnl1_info_ptr );
#endif /* End of FEATURE_GSM_DTM */
boolean L1_control_SDCCH_4( dedicated_data_T      *ded_data,
                            channel_information_T *chnl_1_data
                          );
boolean L1_control_SDCCH_8( dedicated_data_T      *ded_data,
                            channel_information_T *chnl_1_data
                           );
void    L1_send_dedicated_report(void);
void    L1_power_scan(void);
void    l1_pscan_mcpm_change( boolean pscan_start );
...
void L1_free_CCH_message(l2_channel_type_T channel_type, dedicated_data_T  *ded_data);
void L1_get_CCH_filler_message( l2_channel_type_T channel_type,
                                byte              *op_buffer_ptr,
                                uint16            *num_octets_copied_ptr );
boolean L1_CCH_message_pending( l2_channel_type_T channel_type );

void    L1_set_synthesiser ( uint16 channel_no, uint8 timeslot_type );
void    L1_initialise_isr(void);
...
void    L1_send_PH_RANDOM_ACCESS_CNF(uint16 ra_content, word sent_FNmod42432);

void    L1_send_ISR_SCH_CNF (boolean  success, byte *sch_dta_ptr);
void    L1_send_ISR_DECODE_BCCH_LIST_SCH_CNF(boolean  success,
                                             uint16   qbit_lag,
                                             uint32   fn_lag,
                                             uint8    bsic );

void    L1_send_ISR_FIRST_FCB_CNF (boolean  success);
void    L1_send_ISR_SCAN_CNF (void);
void    L1_set_AFC (void) ;
boolean L1_control_half_rate( dedicated_data_T       *ded_dtr_ptr,
                              channel_information_T  *chnl1_info_ptr,
                              boolean                 apply_freq_redef);
void    L1_terminate_dedicated( void);
void    L1_transfer_new_data (void);
void    L1_send_MPH_BLOCK_QUALITY_IND(byte   chnl_type,
                                        gl1_block_quality_t  good_frame);
void    L1_correct_timing ( void );
void    L1isr_send_PH_READY_TO_SEND_IND (byte    channel_type);
void    L1isr_send_MPH_IMMED_ASSIGN_CNF (byte    error);
void    L1isr_send_MPH_CHANNEL_RELEASE_CNF (byte    error);
void    L1isr_send_MPH_SET_CIPHERING_CNF (byte    error);
void    L1isr_send_PH_CONNECT_IND(channel_type_T chnl_type);
void    L1isr_send_MPH_CHANNEL_ASSIGN_CONF(byte    error_code);
#ifdef FEATURE_GSM_DTM
void L1isr_send_MPH_DTM_CHANNEL_RELEASE_IND (byte error, boolean ps_release, boolean cs_release);
void L1isr_send_MPH_DTM_CHANNEL_ASSIGN_CNF (void);
...
void    L1_retrieve_txlev ( uint8 block_start, uint8 burst_spacing, int16 block_tx_power_index[]);
void    L1_map_txlev_to_power_index( int16 tx_lev[], int16 power_index[], uint8 num_values );
byte    L1_change_ta( byte demanded_ta, byte ta_in_use );
void    L1_set_reporting_period(uint16 reporting_period_end_minus_1, uint16 reporting_period);
int16   L1_get_txlev_for_FN(uint32 fn);

void    L1isr_send_L1_REP_PERIOD_IND(void);
void    L1_handle_nt_downlink ( dedicated_data_T       *ded_dtr_ptr );
void    L1_handle_nt_uplink ( dedicated_data_T       *ded_dtr_ptr );
dword   L1_change_FN ( dword FN_lag );
void    L1isr_send_MPH_RECONNECT_CHANNEL_CNF(void);
boolean L1_more_abs_to_send(dedicated_data_T *ded_d_ptr, byte  DCCH_type);
void    L1_send_ISR_HANDOVER_CNF (void);
boolean L1_send_access_burst(void);
#ifdef FEATURE_GERAN_CS_SUPPORT
uint8   L1isr_get_jump_to_sdcch(void);
#endif /* #ifdef FEATURE_GERAN_CS_SUPPORT */
void L1_restart_frames_in_random_access_mode(void);
boolean L1_background_HPLMN_search_in_progress(void);
...
void L1_send_CB_MISSING_BLOCK_IND( uint32 fn );

#ifdef FEATURE_GSM_CB_DRX_SUPPORTED
extern void L1_send_CB_SCHED_EXPIRY_IND( cb_channel_ind_type chan );

extern void L1_send_CB_SCHEDULING_CNF( uint32 *fn_list );
void L1_send_CB_SKIP_CNF( uint32 *fn_list );

#endif /*FEATURE_GSM_CB_DRX_SUPPORTED */

extern boolean L1_get_bcch_list_fcch_sch_bcch(void);
extern void L1_bcch_list_acq_bcch_resume (  void );
extern void L1_bcch_list_acq_bcch_suspend ( void* dummy );
...
extern boolean l1_get_gsm_stop_req_in_queue(void);
extern void l1_set_gsm_stop_req_in_queue(boolean gsm_stop_req_in_queue);
#endif /* FEATURE_WTOG_BACKGROUND_PLMN_SEARCH */

extern void gl1_wait_for_dpc(void);

void l1_clear_cbch_scheduling(void);
...
SIDE EFFECTS
  None
===========================================================================*/
eng_mode_chan_type_t l1_get_tch_chan_mode(void);
...
===========================================================================*/
#ifdef FEATURE_GERAN_CS_SUPPORT
void l1_indicate_g2w_ho( boolean g2w_flag );
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1_check_for_g2w_ho(void);

/*===========================================================================

FUNCTION  l1_is_idle_rx_in_progress

DESCRIPTION
  Return if any idle Rx activity is in progress.

DEPENDENCIES
  None

RETURN VALUE
  TRUE   if any idle Rx activity is in progress.
  FALSE  otherwise
...
boolean l1_is_idle_rx_in_progress(void);

#if !defined (FEATURE_GL1_GPLT) && defined (FEATURE_CGPS_TIME_TAGGING)
/*===========================================================================
...
===========================================================================*/
extern void l1_isr_run_gps_tick_handler(void);
#endif /* !FEATURE_GL1_GPLT &&  FEATURE_CGPS_TIME_TAGGING*/
