...
$Header: //components/rel/geran.mpss/5.2.0/gl1/inc/gl1_arbitrator_tables.h#3 $ $DateTime: 2018/10/10 02:31:27 $ $Author: pwbldsvc $
...
extern void (*garb_resolve_conflict_lut[GARB_CHAN_MAX][GARB_CHAN_MAX])(uint8 ts, boolean is_rx, uint8 conflict_index, cxm_activity_type* conflict_priority_tier_t);
...
/* VBATT : TXLEV to dBm */
extern const uint16 txlev_to_dBm[SYS_BAND_CELL_850+1][MAX_TXLEV+1];

/* VBATT : P1 (dBm) + P2 (dBm) = MAX(P1, P2) + LUTx[ABS(P1 - P2)] */
extern const double LUTx[LUTX_MAX_IDX];

/* VBATT : Proportion SUB1/SUB2 backoffs */
extern const double LUTy[LUTY_MAX_IDX+1][2];
...
extern const uint8 p1p2_dbm_to_backoff_index[MAX_DBM-25+1][MAX_DBM-25+1];
