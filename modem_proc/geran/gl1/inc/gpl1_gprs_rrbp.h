...
uint32 gpl1_gprs_get_ul_rrbp_msg_fn(void);
...
rrbp_msg_t  gpl1_gprs_single_tx_rrbp(
  uint8 rrbp_to_tx,
  frequency_information_T *freq_info_ptr_t 
  );
...
uint8 gpl1_gprs_get_matching_rrbp_msg_type(
  rrbp_msg_t rrbp_msg_type_t 
  );

uint8 gpl1_gprs_get_mathching_rrbp_index(void );
  
void gpl1_gprs_calc_num_ab_dl_rrbp(void  );

uint8 gpl1_gprs_get_num_rrbp_msg(void  );

uint8 gpl1_gprs_get_dl_num_rrbp_msg(void  );

uint8 gpl1_gprs_get_dl_ab_num_rrbp_msg(void  );

uint8 gpl1_gprs_get_ul_num_rrbp_msg(void  );

uint8 gpl1_gprs_get_num_rrbp_msg_txd(void  );

void  gpl1_gprs_reset_num_rrbp_msg_txd(void);
...
uint32 gpl1_gprs_check_for_rrbp_msg_FN(
     rrbp_msg_t *rrbp_msg_type, pcr_format_t* rrbp_burst_type, uint8 *timeslot 
     );
...
void gpl1_gprs_check_for_rrbp_duplicates(void  );

void gpl1_gprs_delete_rrbp_entry(
  uint8 rrbp_idx_t 
  );

void gpl1_gprs_delete_dl_rrbp_entries(void  );

void gpl1_gprs_reset_rrbp_enteries(void  );

void gpl1_gprs_reset_rrbp_msgs_txd(void  );

void  gpl1_gprs_clean_rrbp_enteries(
  uint32    current_fn, 
  uint8     num_rx_pdch,
  uint8*    tra_trb_cancel_ts_alloc_ptr,
  uint8*    l1_cancel_ts_alloc_ptr_t 
  );
 ...
 */
void  gpl1_rrbp_delete_msg( uint8  csn_msg_type_t  );

#endif
#endif /* FEATURE_GSM_DTM */

#ifdef FEATURE_G2X_TUNEAWAY
boolean gpl1_expired_rrbp_msg( uint32 current_fn _t );
boolean gpl1_expired_rrbp_msg_in_gta(void);
void gpl1_set_expired_rrbp_msg_in_gta( boolean expired_t  );
