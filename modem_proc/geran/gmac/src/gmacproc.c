...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacproc.c#3 $   
***
...
***
*****************************************************************************/
gmac_global_data_t gmac_global_data;
...
void gmac_comproc_handle_ppcta(void)
{
  ...
  MSG_GERAN_HIGH_2_G("MAC Rcvd PACKET_PWR_CTRL_TMNG_ADV, pwr_ctrl %d, TA %d",
    ul_ppcta_msg->power_control.present,ul_ppcta_msg->global_packet_ta_present);

  /* Always send Power Control Parameters to L1*/
  if(ppcta_data->power_control_flag == valid) 
  {
    ul_ppcta_msg->power_control.present = TRUE;
    gmac_power_control_params_ie(&(ppcta_data->pwr_ctrl_paramtrs), &(ul_ppcta_msg->power_control));
  }
  else
  {
  ...
  if(ppcta_data->global_timing_adv_flag == valid) 
  {
    ...
    gmac_global_packet_ta_ie(&(ppcta_data->global_pkt_timing), &(ul_ppcta_msg->global_packet_ta));
  }
  else
  {
  ...
  if(ppcta_data->global_pwr_ctrl_flag == valid) 
  {
    ...
    gmac_global_power_ctrl_params_ie(&(ppcta_data->global_pwr_ctrl), &(grr_pwr_ctrl_msg->update.global_power_control_params));
  ...
  memset(&local_mac_mac_sig, 0, sizeof(mac_int_sig_t));
    ...
    if ((tbf_est_set == FALSE) || ((tbf_est_set == TRUE) && (est_new_ul_tbf == FALSE)))
    {
      ...
      *********************************************************************/
      ...
      if( ((global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[0] & 0x08) >> 3) == 1)
      {
        ...
        if(( ((global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[0] & 0xc0) >> 6) == 2) && (global_data_ptr->channel_msg != SEGMENTED_MSG))
        {
          ...
          {     
            ...
            MSG_GERAN_LOW_2_G("MAC info error in PWR option rbsn_bit %d fs_bit %d",rbsn_bit,fs_bit);
            /*use default "control_ack_value" and send PCA, best to always send a PCA when we are releasing */
          }
        }
      }
      ...
      rr_read_public_store(RRPS_CELL_OPTIONS_CONTROL_ACK_TYPE,&control_ack_type);
      ...
      **********************************************************************/
      ...
      MSG_GERAN_MED_0_G("MAC Sent MAC_L1_PACCH_RRBP & MAC_L1_RELEASE");

      gmac_util_block_rlc(NO_DEL, GMAC_PROC, __LINE__);
      ...
      else
      {
        ...
        mac_log_packet_tbf_rel_event(UL_TBF_NORMAL);
      ...
      *********************************************************************/ 
      ...
      if(control_ack_type == ACCESS_BURST )
      {
        ...
        *******************************************************************/
        ...
        gprs_mac_mac_acc_func(&local_mac_mac_sig);
        
        MSG_GERAN_MED_0_G("MAC Sent MAC_TI_RE_ESTABLISH & MAC_L1_PACCH_RRBP");
        ml_pacch_rrbp_sig_ptr->absolute_fn =  gmac_rrbp_absolute_fn(global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.frame_no,rrbp_value);
        ml_pacch_rrbp_sig_ptr->ts_of_dl_msg = global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.timeslot;
        ml_pacch_rrbp_sig_ptr->rrbp_msg = PCA_UL_FAI_MSG;
#ifdef FEATURE_GSM_EDTM
      ...
      else
      {
        ...
        ****************************************************************/
        ...
        global_data_ptr->prr_re_est_rrbp_fn =  gmac_rrbp_absolute_fn(global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.frame_no,rrbp_value);
