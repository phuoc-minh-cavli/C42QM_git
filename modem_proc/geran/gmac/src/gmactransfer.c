...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmactransfer.c#3 $
***
...
#error code not present
#else
static gmac_transfer_data_t gmac_transfer_data;
#endif /*PERLUTF*/
...
===
===  DESCRIPTION
===  Returns a pointer to transfer data space 
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
gmac_transfer_data_t* gmac_get_transfer_data_ptr(void)
{
  return &gmac_transfer_data;
}
...
#ifdef FEATURE_GPRS_PS_HANDOVER
static void gmac_tran_send_gl1_psho_cancel_pending_config(void);
...
static void gmac_tran_ps_handover_data_ind(void);
static void gmac_tran_store_tbf_rel_cnf(void);
...
static void gmac_tran_psho_restart_timers(gprs_mac_transfer_substates_t gmac_transfer_substate);
#endif /*FEATURE_GPRS_PS_HANDOVER*/

static void gmac_ul_transfer_init(void);

static void gmac_transfer_tbf_rel_confirm(tbf_released_t tbf_rel);
...
static void gmac_transfer_edtm_t3196_expiry_handler(void);

static void gmac_transfer_edtm_ph_cs_connect_handler(void);
...
static void gmac_tran_edtm_cs_rel_handle_abnormal_rel(void);
...
#endif /*FEATURE_GSM_EDTM*/
#ifdef FEATURE_GPRS_PS_HANDOVER
static void gmac_transfer_initiate_local_release_of_old_cell_dl_tbfs(void);
#endif /*FEATURE_GPRS_PS_HANDOVER*/
static void gmac_tran_handle_unexpected_rlc_tbf_req(void);
...
===
===========================================================================*/
void gmac_init_transfer_data(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
...
===
===========================================================================*/
void gmac_init_transfer_data_non_zero(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
...
static void gmac_transfer_tbf_rel_confirm(tbf_released_t tbf_rel)
{
  ...
  gmac_tran_log_state_change(TRANSFER_INACTIVE, __LINE__);
  transfer_data_ptr->mac_transfer_substate = TRANSFER_INACTIVE;
  ...
  MSG_GERAN_LOW_0_G("mac info wait_for_ul_rel_confirm = FALSE");
  ...
  MSG_GERAN_MED_2_G("MAC info abnormal_ul_and_dl_rel_flg = %d, wait_for_ul_rel_confirm = %d",
    transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg,
    global_data_ptr->c_flags.wait_for_ul_rel_confirm);
  ...
  {
    gmac_util_send_pmr_cnf_to_grr(MAC_GRR_FAILURE);
  }
  ...
  {
    gmac_stop_t3196();
    gmac_stop_edtm_pkt_cs_req_retry_timer();
    gmac_send_mac_grr_edtm_pkt_cs_est_rej(EDTM_CS_REJ_TBF_RELEASE_IN_PROGRESS);
  ...
  MSG_GERAN_LOW_0_G(" pccn_pending = NO_PENDING_PCCN");
  ...
  gmac_util_release_container(SERVING_CELL_CONTAINER);
  ...
  switch (transfer_data_ptr->gmac_transfer_tbf_rel_cause)
  {
    case GMAC_TRANSFER_TBF_REL_NORMAL:
      ...
      if (transfer_data_ptr->t_flags.dtm_active == TRUE)
      {
          ...
          gprs_mac_mac_acc_func(&global_data_ptr->mac_mac_sig);
      }
      else
#endif /*FEATURE_GSM_EDTM*/
      {
      ...
      {
        MSG_GERAN_LOW_0_G("GMAC T3172 running on exit from Transfer, leave RLC blocked");

        /* Check that RLC is still blocked */
        (void)gmac_util_check_rlc_blocked_flag(BLOCKED,GMAC_TRAN, __LINE__);
      }
      else
      {
        if ( gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == BLOCKED )
        {
          gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_TRAN, __LINE__);
        }
      }

      gmac_util_send_grr_mac_status(MAC_IDLE);
      break;
    ...
    case GMAC_TRANSFER_TBF_REL_GRR_NO_SERVICE:
      ...
      global_data_ptr->mac_mode = NULL_MODE;
      MSG_GERAN_MED_1_G("MAC set mode %d, Sent MAC_TN_NO_SERVICE",global_data_ptr->mac_mode);
      (void)mac_log_state();

      gmac_util_send_grr_mac_status(MAC_IDLE);

      global_data_ptr->c_flags.no_service_penidng = TRUE;
    ...
    default:
      ...
      {
        MSG_GERAN_ERROR_1_G("GMAC BAD tbf rel cause: 0x%x, initiated Recovery Restart",transfer_data_ptr->gmac_transfer_tbf_rel_cause);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("GMAC BAD tbf rel cause: 0x%x", transfer_data_ptr->gmac_transfer_tbf_rel_cause);
      }
    break;
  }
  ...
  */
  ...
  gmac_eng_mode_update_tbf_info(global_data_ptr->gmac_current_mac_mode, global_data_ptr->current_tbf_mode, TRUE);
...
===
===========================================================================*/
void gmac_init_transfer_flags(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
...
===
===========================================================================*/
void transfer_mode_handler(uint8 task_id, uint8 signal_id)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  gmac_timers_t* mac_timers_ptr = gmac_get_timers_ptr();
  ...
  {
    MSG_GERAN_MED_5_G("MAC info block dl msg(during ul,dl rel) =%d, %d state = %d, dl_ass_tlli %x, ul_est_tlli %x",
  ...
  if (task_id == MS_MAC_L1)
  {
    ...
    {
      MSG_GERAN_MED_3_G("MAC Rcvd TRANSFER mode sig %d, task %d, substate %d", signal_id, task_id, transfer_data_ptr->mac_transfer_substate);
    }
  }
  else
  {
      ...
      if(task_id == MS_MAC_MAC)
      {
        ...
        switch (signal_id)
        {
          ...
          case MAC_NT_UL_DTM_DYNAMIC:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_NT_UL_DTM_DYNAMIC");
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
            ...
            gmac_tran_log_state_change(TRANSFER_UL_DYNAMIC, __LINE__);
            transfer_data_ptr->mac_transfer_substate = TRANSFER_UL_DYNAMIC;
            (void)mac_log_state();
            gmac_reset_rac_params();
            gmac_start_t3180();

            gmac_eng_mode_update_tbf_info(global_data_ptr->gmac_current_mac_mode, global_data_ptr->current_tbf_mode, FALSE);

            break;

          case MAC_NT_DL_DTM_DYNAMIC:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_NT_DL_DTM_DYNAMIC");
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC T3172 running on entry to Transfer, leave RLC blocked");
              (void)gmac_util_check_rlc_blocked_flag(BLOCKED,GMAC_TRAN, __LINE__);
            }

            gmac_tran_log_state_change(TRANSFER_DL, __LINE__);
            transfer_data_ptr->mac_transfer_substate = TRANSFER_DL;
            (void)mac_log_state();

            gmac_eng_mode_update_tbf_info((uint8)GERAN_ENG_MODE_INVALID_VALUE_OF_PARAMETER, global_data_ptr->current_tbf_mode,FALSE);

            break;

          case MAC_NT_UL_DL_DTM_DYNAMIC:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_NT_UL_DL_DTM_DYNAMIC");
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
            ...
            transfer_data_ptr->gmac_transfer_ul_request_count++;
            transfer_data_ptr->t_flags.dtm_active = TRUE;
            gmac_tran_log_state_change(TRANSFER_CONCURRENT_DYNAMIC_TRANSFER, __LINE__);
          ...
          case MAC_IT_UL_DYNAMIC:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_IT_UL_DYNAMIC");
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
            ...
            if (global_data_ptr->msg_ptr->mac_int_sig.msg.mac_it_ul_dynamic.waiting_to_connect_realloc_flg  == TRUE)
            {
              ...
              MSG_GERAN_LOW_0_G("GMAC infoWaiting for Con ind for re-alloc PUA rcvd in 1p cont res ");
              gmac_tran_log_state_change(TRANSFER_UL_DYNAMIC_REALLOC, __LINE__);
          ...
          case MAC_NT_DL_DYNAMIC:
            ...
            {
              MSG_GERAN_LOW_0_G("MAC Rcvd MAC_NT_DL_DYNAMIC");
            }

            if(signal_id == (uint8)MAC_IT_DL_DYNAMIC)
            {
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_IT_DL_DYNAMIC");
            }
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
          ...
          case MAC_IT_UL_DYN_EGPRS_PUA_RCVD_IN_CONT_RES:
            ...
            ************************************************************************/
            ...
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_IT_UL_DYN_EGPRS_PUA_RCVD_IN_CONT_RES");
            gmac_ul_transfer_init();
            gmac_dl_transfer_init();
            (void)gmac_util_check_rlc_blocked_flag(UNBLOCKED,GMAC_TRAN, __LINE__);
            ...
            if (gmac_tdynamic_handle_egprs_cont_res_pua(global_data_ptr->msg_ptr->mac_int_sig.msg.mac_it_ul_epua_rcvd.egprs_c_r_pua_ptr))
            {
              gmac_stop_t3168();
              gmac_tran_log_state_change(TRANSFER_UL_DYNAMIC_REALLOC, __LINE__);
              transfer_data_ptr->mac_transfer_substate = TRANSFER_UL_DYNAMIC_REALLOC;
      ...
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            ...
            {
              gmac_transfer_dl_data_ind();
            }
            break;

          case L1_MAC_QUAL_MEASURMENTS:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_QUAL_MEASURMENTS");
            gmac_transfer_dl_pkt_acknack(&(global_data_ptr->msg_ptr->l1_mac_sig.msg.qual_measurements));
            break;

          case L1_MAC_TBF_REL_CONFIRM:
#ifdef FEATURE_GPRS_PS_HANDOVER
            gmac_tran_dl_process_rel_cnf(global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
#else /*FEATURE_GPRS_PS_HANDOVER*/

            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_TBF_REL_CONFIRM");
            ...
            {
              gmac_transfer_tbf_rel_confirm(DL_TBF_RELEASE_CONF);
            }
            else
            {
          ...
          case L1_MAC_RRBP_FAILED_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_RRBP_FAILED_IND");
            if(global_data_ptr->msg_ptr->l1_mac_sig.msg.rrbp_failed_ind.rrbp_contained_chan_des == TRUE)
            {
              ...
              /* No need to unblock RLC if it is blocked due to some reson (e.g. due to Abnormal DL Release). */
              if (gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == UNBLOCKED)
              {
                gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
                gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_TRAN,__LINE__);
              }
            }
            break;
          ...
          case MAC_PH_CS_CONNECT_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CS_CONNECT_IND");
            gmac_transfer_edtm_ph_cs_connect_handler();
          break;
          ...
          case L1_MAC_FREQ_ASS_CNF:
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC L1_MAC_FREQ_ASS_CNF rxd for EDTM in TRANSFER_DL");
            }
            else
            {
              MSG_GERAN_LOW_0_G("GMAC L1_MAC_FREQ_ASS_CNF not expected in TRANSFER_DL");
            }
          break;
#endif /*FEATURE_GSM_EDTM*/
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          case RM_DL_PKT_ACKNACK_REQ:
            ...
            gmac_transfer_dl_pdan_handler(&(global_data_ptr->msg_ptr->rlc_mac_sig.msg.dl_pkt_acknack_req),FALSE);
            break;
          ...
          case RM_DL_RELEASE_IND:
            ...
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.dl_release_ind.cause == REL_DL_NORMAL)
            {
              ...
              if (transfer_data_ptr->t_flags.gmac_transfer_ul_req)
              {
                ...
                gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,DONT_REL_RLC_DL,NO_DEL,FALSE);

                mac_log_dl_tbf_release(MAC_LOG_DL_RELEASE_IND);
              }
              else
              {
                gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
                gmac_transfer_dl_release_clean();
                gmac_transfer_dl_normal_release_to_idle();
              }
            }
            else
            {
          ...
          case RM_DL_PKT_CTRL_ACK_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_DL_PKT_CTRL_ACK_REQ");
          ...
          case RM_UL_EST_TBF_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_UL_EST_TBF_REQ");
            if (gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == BLOCKED )
            {
            ...
            else if (global_data_ptr->c_flags.tbf_release_pending_flg)
            {
              ...
              MSG_GERAN_LOW_0_G("GMAC tbf_release_pending_flg set, RLC not blocked in T_DL, Ignore RLC req");
            }
            else
            {
      ...
      else if (task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          case MAC_TIMER_T3172:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3172");
            ...
            if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
            {
              if (gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == BLOCKED )
              {
                gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_TRAN_UL, __LINE__);
              }
              else
              {
                /* RLC should have been blocked when T3172 was started, find cause of error*/
                MSG_GERAN_LOW_0_G("GMAC T3172 expired, RLC not blocked, No action");
              }
            }
            else
            {
              MSG_GERAN_LOW_0_G("GMAC releasing L1, so ignoring MAC_TIMER_T3172 signal");
            }

            break;

          case MAC_TIMER_T3168:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3168");
            ...
            {
              gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
              mac_log_dl_tbf_release(MAC_LOG_DL_EXPIRED_T3168);
            }
            break;
          ...
          case MAC_TIMER_T3200_8:
               ...
               t3200_expiry_handler((mac_mac_sig_id_t)signal_id );
          break;
          ...
          case MAC_TIMER_T3196:
            gmac_transfer_edtm_t3196_expiry_handler();
          break;
#endif /*FEATURE_GSM_EDTM*/
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          case GRR_MAC_SEND_MEAS_RPT:
            ...
            {
              if(gmac_util_encode_ul_control_msg_handler(PMR,TRUE) == PASS)
              {
                gmac_util_send_ul_control_msg_handler(PMR, DL_ONLY_PRESENT, GRR_CNF_NEEDED);
              }
          ...
          case GRR_MAC_SEND_PCCF:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PCCF");
            if(gmac_util_encode_ul_control_msg_handler(PCCF,TRUE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PCCF, DL_ONLY_PRESENT, GRR_CNF_NEEDED);
            }
            break;
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            ...
            mac_log_dl_tbf_release(MAC_LOG_DL_GPRS_SUSPENSION);
            break;

          case GRR_MAC_SEND_PCCN_REQ :
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PCCN");
            if(gmac_util_encode_ul_control_msg_handler(PCCN,TRUE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PCCN, DL_ONLY_PRESENT, GRR_CNF_NEEDED);
            }
            break;

          case GRR_MAC_SEND_PSS_REQ :
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PSS");
            if(gmac_util_encode_ul_control_msg_handler(PSS,TRUE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PSS, DL_ONLY_PRESENT, GRR_CNF_NEEDED);
            }
            break;
          ...
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
            ...
            if( gmac_tran_dtm_chan_ass_handler(
                  ...
                  &transfer_data_ptr->gmac_transfer_ul_rlc_info_store,
                  DL_TBF_PRESENT ) == NO_DTM_ASS_ERROR )
            {
#ifdef FEATURE_GSM_EDTM
              if ( global_data_ptr->gmac_edtm_enabled )
              {
                ...
                {
                    transfer_data_ptr->mac_tran_substate_str = gmac_convert_transfer_substate_to_string(transfer_data_ptr->mac_transfer_substate);

                    MSG_SPRINTF_1( MSG_SSID_DFLT, MSG_LEGACY_MED, "GMAC State tran = %s", transfer_data_ptr->mac_tran_substate_str);
                }
              }
              else
#endif /*FEATURE_GSM_EDTM*/
              {
                gmac_tran_log_state_change(TRANSFER_DL_REASSIGN, __LINE__);
          ...
          case GRR_MAC_PSHO_REQ:
            ...
            if(gmac_util_ps_ho_req_handler(transfer_data_ptr->mac_transfer_substate) == TRUE)
            {
              ...
              if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_type == PSHO_GTOG)
              {
                ...
                {
                  gmac_util_send_grlcdl_psho_start_ind();
                }
                grlc_dl_start_t3190();

                gmac_util_send_grlcul_psho_start_ind();
            ...
            {
              gmac_tran_psho_trigger_pccf_sending(transfer_data_ptr->mac_transfer_substate);
              gmac_util_reset_psho_structure();
            }
          break;
#endif /*FEATURE_GPRS_PS_HANDOVER*/
    ...
    case TRANSFER_UL_DYNAMIC:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          ...
          case L1_MAC_SINGLE_BLOCK_SENT:
            ...
            {
              transfer_data_ptr->gmac_transfer_ul_request_count++;
              rr_read_public_store(RRPS_CELL_OPTIONS_T3168,&transfer_data_ptr->t3168_time);
              gmac_start_t3168(transfer_data_ptr->t3168_time);
              global_data_ptr->c_flags.gmac_prr_on_pacch_state = GMAC_PRR_ON_PACCH_AWAIT_REQ;
            }
            break;
          ...
          case L1_MAC_QUAL_MEASURMENTS:
            ...
            if (global_data_ptr->c_flags.gmac_prr_on_pacch_state == GMAC_PRR_ON_PACCH_AWAIT_QUAL)
            {
              ...
              gmac_send_prr(NO_RE_EST,FALSE);
            }
            break;

          case L1_MAC_1ST_DATA_BLOCK_SENT:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_1ST_DATA_BLOCK_SENT");
            gmac_stop_t3164();
            gmac_start_t3180();
            break;
          ...
          case L1_MAC_TBF_REL_CONFIRM:
#ifdef FEATURE_GPRS_PS_HANDOVER
            gmac_tran_ul_dynamic_process_rel_cnf(global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
#else /*FEATURE_GPRS_PS_HANDOVER*/
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_TBF_REL_CONFIRM");
            ...
            {
              gmac_transfer_tbf_rel_confirm(UL_TBF_RELEASE_CONF);
              global_data_ptr->gmac_local_tfi_store.ul_tfi_valid = FALSE;
            }
#endif /*FEATURE_GPRS_PS_HANDOVER*/
            break;
          ...
          case L1_MAC_FREQ_ASS_CNF:
            if ( global_data_ptr->gmac_edtm_enabled &&
                 transfer_data_ptr->t_flags.edtm_realloc_in_progress )
            {
              MSG_GERAN_LOW_0_G("GMAC L1_MAC_FREQ_ASS_CNF rxd for EDTM in TRANSFER_UL_DYNAMIC");
            }
            else
            {
              MSG_GERAN_LOW_0_G("GMAC L1_MAC_FREQ_ASS_CNF not expected in TRANSFER_UL_DYNAMIC");
            }
          break;
#endif /*FEATURE_GSM_EDTM*/

          default:
            MSG_GERAN_LOW_3_G( "Unexpected L1 signal in state %d task id %d sig id 0x%x",
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          case RM_UL_REALLOC_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_UL_REALLOC_REQ");
            transfer_data_ptr->gmac_transfer_ul_request_count = 0;
            ...
            gmac_transfer_qual_meas_req_to_L1();
            break;

          case RM_UL_RELEASE_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_UL_RELEASE_IND");
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause != REL_UL_NORMAL)
            {
              ...
              mac_log_ul_tbf_release(MAC_LOG_UL_RELEASE_IND);
            }
            else
            {
              ...
              gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
              gmac_tran_send_mac_l1_release(UL_TBF_NORMAL);
          ...
          case RM_UL_EST_TBF_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_UL_EST_TBF_REQ");
            gmac_tran_handle_unexpected_rlc_tbf_req();
            break;

#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
          case RM_UL_EXT_TBF_START_IND:
            /* Indicate GL1 for start of EXT_UL_TBF period */
            if (transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              gmac_util_send_mac_l1_ext_tbf_start_ind(transfer_data_ptr->gmac_tran_last_puan_fn);
            }
            break;
#endif /* FEATURE_GERAN_POWER_EFFICIENT_OPERATION */

          default:
            MSG_GERAN_LOW_3_G( "Unexpected RLC signal in state %d task id %d sig id 0x%x",
                                 transfer_data_ptr->mac_transfer_substate,task_id, signal_id);
      ...
      else if (task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          case MAC_TIMER_T3180:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3180");
            gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,
                                            NO_RLC_DL_TBF_PRESENT_TO_RELEASE,
                                            NO_DEL,
                                            FALSE);

            mac_log_ul_tbf_release(MAC_LOG_UL_EXPIRED_T3180);
            break;

          case MAC_TIMER_T3164_TRANSFER:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3164_TRANSFER");
            gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,
                                            NO_RLC_DL_TBF_PRESENT_TO_RELEASE,
                                            NO_DEL,
                                            FALSE);
            mac_log_ul_tbf_release(MAC_LOG_UL_EXPIRED_T3164);
            break;
          ...
          case MAC_TIMER_T3168:
            ...
            else
            {
              gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,
                                              ...
                                              NO_DEL,
                                              FALSE);
              mac_log_ul_tbf_release(MAC_LOG_UL_EXPIRED_T3168);
            }
            break;

          case MAC_TIMER_T3164:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3164");
...
#ifdef FEATURE_GSM_EDTM
          case MAC_TIMER_PKT_CS_REQ_RETRY:
            MSG_GERAN_LOW_0_G("GMAC EDTM PKT CS REQ RETRY Timer expired");
            gmac_transfer_edtm_pkt_cs_req_handler(
              UL_ONLY,
              global_data_ptr->gmac_local_edtm_pcr_params.edtm_pcr_cause,
              EDTM_PKT_CS_REQUEST_RETRY);
          ...
          case MAC_TIMER_T3196:
            MSG_GERAN_LOW_0_G("GMAC EDTM T3196 Timer expired");
            gmac_transfer_edtm_t3196_expiry_handler();
          break;
#endif /*FEATURE_GSM_EDTM*/

          default:
            MSG_GERAN_LOW_3_G( "Unexpected MAC signal in state %d task id %d sig id 0x%x",
                                 transfer_data_ptr->mac_transfer_substate,task_id, signal_id);
            /* unknown internal signal from MAC */
            break;
        }
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          case GRR_MAC_SEND_MEAS_RPT:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_MEAS_RPT");
            if(gmac_util_encode_ul_control_msg_handler(PMR,FALSE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PMR, UL_PRESENT, GRR_CNF_NEEDED);
            }
          ...
          case GRR_MAC_SEND_PCCF:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PCCF");
            if(gmac_util_encode_ul_control_msg_handler(PCCF,FALSE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PCCF, UL_PRESENT, GRR_CNF_NEEDED);
            }
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            ...
            transfer_data_ptr->gmac_transfer_tbf_rel_cause = GMAC_TRANSFER_TBF_REL_GRR_NO_SERVICE;
            mac_log_ul_tbf_release(MAC_LOG_UL_GPRS_SUSPENSION);
            break;

          case GRR_MAC_SEND_PCCN_REQ :
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PCCN");
            if(gmac_util_encode_ul_control_msg_handler(PCCN,FALSE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PCCN, UL_PRESENT, GRR_CNF_NEEDED);
            }
            break;

          case GRR_MAC_SEND_PSS_REQ :
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_SEND_PSS");
            if(gmac_util_encode_ul_control_msg_handler(PSS,FALSE) == PASS)
            {
              gmac_util_send_ul_control_msg_handler(PSS, UL_PRESENT, GRR_CNF_NEEDED);
            }
            break;
          ...
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
            ...
            if( gmac_tran_dtm_chan_ass_handler(
                  &global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_dtm_channel_assign_req,
                  &transfer_data_ptr->gmac_transfer_ul_rlc_info_store,
                  UL_TBF_PRESENT ) == NO_DTM_ASS_ERROR )
            {
#ifdef FEATURE_GSM_EDTM
              if ( global_data_ptr->gmac_edtm_enabled )
              {
  ...
  case TRANSFER_DL_REASSIGN:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_dl_release == FALSE)
            {
              gmac_transfer_dl_reassign_data_ind();
            }
          ...
          case MAC_PH_CONNECT_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CONNECT_IND");
            ...
            if (global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
            {
              ...
              if (global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == DOWNLINK_TBF_ESTABLISHED)
              {
                ...
                {
                  MSG_GERAN_LOW_0_G("GMAC T3172 still running leave RLC blocked");
                }
              }
              else
              {
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("GMAC AB Rel, Expected DL (0x01) in PH_CONNECT_IND, received: 0x%x, initiated Recovery Restart",
                    global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("GMAC AB Rel, Expected DL (0x01) in PH_CONNECT_IND, received: 0x%x",
                            global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
            }
          ...
          case L1_MAC_FREQ_ASS_CNF:
            MSG_GERAN_LOW_0_G("GMAC INFO L1 Freq Cnf ignored for freq cleanup");

            if(global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_freq_ass_cnf.band_supported == FALSE)
            {
              MSG_GERAN_LOW_0_G("GMAC Freq Cnf Cleanup, L1 indicating band NOT supported");
            }
          break;
          ...
          case RM_DL_RELEASE_IND:
            ...
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.dl_release_ind.cause == REL_DL_NORMAL)
            {
              ...
              if (transfer_data_ptr->t_flags.gmac_transfer_ul_req)
              {
                ...
                gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,DONT_REL_RLC_DL,NO_DEL,FALSE);
                mac_log_dl_tbf_release(MAC_LOG_DL_NORMAL_RELEASE);
              }
              else
              {
                gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
        ...
        case RM_UL_EST_TBF_REQ:
            ...
            if (gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == BLOCKED )
            {
              /* RLC has attempted access when we have blocked it.  This might
                 ...
                 Because a race condition is a possibility we have to assume that this has happened
                 in this case */
              MSG_GERAN_LOW_0_G("GMAC RLC attempted access whilst blocked, ignore msg, assume race condition");
            }
            else
            {
              ...
              else
              {
                ...
                MSG_GERAN_LOW_0_G("GMAC Can't handle tbf req in T_DL_REASS, block RLC until CONNECT_IND");
                gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
              }
            }
            break;
      ...
      else if (task_id == MS_MAC_MAC)
      {
        ...
        {
          case MAC_TIMER_T3172:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TIMER_T3172 in T_DL_REASS, No action until CONNECT_IND");
            break;
      ...
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_realloc_data_ind();
            }
            break;
            ...
            if(transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              if (global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_TBF_ESTABLISHED)
              {
                ...
                if (transfer_data_ptr->mr_ul_alloc_cnf_ptr != NULL)
                {
                  ...
                  grlc_put_cmd_mac(transfer_data_ptr->mr_ul_alloc_cnf_ptr);
                }
                else
                {
              ...
              else
              {
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("GMAC AB Rel, Expected UL (0x00) in PH_CONNECT_IND, received: 0x%x, initiated Recovery Restart",
                    global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("GMAC AB Rel, Expected UL (0x00) in PH_CONNECT_IND, received: 0x%x", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
            }
            else
        ...
        switch (signal_id)
        {
          ...
          case RM_UL_RELEASE_IND:
            ...
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause != REL_UL_NORMAL)
            {
              if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause == REL_UL_ABNORMAL_CELL_RESEL)
              {
                gmac_abnormal_release_procedure(RR_CELL_RESEL_REQ,REL_RLC_DL,NO_DEL,FALSE);
              }
              else
              {
                gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
    ...
    ************************************************/
    case TRANSFER_DL_WITH_DYNAMIC_ALLOC:
      if(task_id == MS_MAC_L1)
      {
        ...
        {
          case MAC_PH_DATA_IND:
            gmac_transfer_dl_dynamic_alloc_data_ind();
            break;
          ...
          case MAC_PH_CONNECT_IND:
            ...
            if(transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              MSG_GERAN_MED_1_G("MAC Rcvd MAC_PH_CONNECT_IND for tbf %d",
                global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
              ...
              if (global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_TBF_ESTABLISHED)
              {
                ...
                if ( global_data_ptr->current_test_mode != TEST_MODE_OFF )
                {
                  ...
                  if ( transfer_data_ptr->mr_dl_test_mode_ind_ptr != NULL )
                  {
                    ...
                    grlc_put_cmd_mac(transfer_data_ptr->mr_dl_test_mode_ind_ptr);
              ...
              {
                MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CONNECT_IND for DL");
              ...
              else
              {
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("GMAC AB Rel, Expected UL (0x00) or DL (0x01) in PH_CONNECT_IND, received: 0x%x, initiated Recovery Restart",
                                      global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("GMAC AB Rel, Expected UL (0x00) or DL (0x01) in PH_CONNECT_IND, received: 0x%x", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
            }
            else
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            ...
            if (global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released == UL_DL_TBF_RELEASE_CONF)
            {
              ...
              gmac_transfer_tbf_rel_confirm(UL_DL_TBF_RELEASE_CONF);
            }
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          case RM_DL_RELEASE_IND:
            ...
            {
              gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,DONT_REL_RLC_DL,NO_DEL,FALSE);
              mac_log_ul_tbf_release(MAC_LOG_UL_RLC_DL_RELEASE_IND);
              mac_log_dl_tbf_release(MAC_LOG_DL_RELEASE_IND);
            }
            break;
          ...
          case RM_UL_RELEASE_IND:
            ...
            if(transfer_data_ptr->t_flags.waiting_for_ph_con_for_dl == TRUE)
            {
              if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause != REL_UL_NORMAL)
              {
                ...
                mac_log_ul_tbf_release(MAC_LOG_UL_RELEASE_IND);
                mac_log_dl_tbf_release(MAC_LOG_DL_RLC_UL_RELEASE_IND);
              }
              else
              {
                MSG_GERAN_LOW_0_G("RM_UL_RELEASE_IND with normal cause not expected in this state");
              }
            }
            else
            {
               MSG_GERAN_LOW_0_G("RM_UL_RELEASE_IND rxd when not waiting for DL con");
            }
            break;
        ...
        switch (signal_id)
        {
          ...
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
            ...
            if( gmac_tran_dtm_chan_ass_handler(
                  &global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_dtm_channel_assign_req,
                  &transfer_data_ptr->gmac_transfer_ul_rlc_info_store,
                  UL_DL_TBF_PRESENT) == NO_DTM_ASS_ERROR )
            {
#ifdef FEATURE_GSM_EDTM
              if ( global_data_ptr->gmac_edtm_enabled )
              {
...
#endif /*FEATURE_GSM_EDTM*/
              {
                gmac_tran_log_state_change(TRANSFER_DL_REASSIGN_DYNAMIC_ALLOC, __LINE__);
      ...
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
             gmac_tdynamic_dl_assign_data_ind();
            }
            break;
          ...
          case MAC_PH_CONNECT_IND:
            ...
            if(transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              else
              {
                ...
                {
                  ERR_GERAN_FATAL_1_G("GMAC AB Rel, Expected DL (0x01) in PH_CONNECT_IND, received: 0x%x", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          case RM_UL_RELEASE_IND:
            ...
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause != REL_UL_NORMAL)
            {
              MSG_GERAN_LOW_0_G("MAC Rcvd UL abnormal release cause");
              if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause == REL_UL_ABNORMAL_CELL_RESEL)
              {
                gmac_abnormal_release_procedure(RR_CELL_RESEL_REQ,
                                                NO_RLC_DL_TBF_PRESENT_TO_RELEASE,
            ...
            {
              MSG_GERAN_LOW_0_G("MAC UL TBF normal release, awaiting for PH_CONNECT_IND to establish DL");
              gmac_transfer_ul_dynamic_release_clean();
              gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
              gmac_tran_send_mac_l1_release(UL_TBF_NORMAL);
              transfer_data_ptr->t_flags.ul_tbf_normal_release_ind = TRUE;
      ...
      else if (task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          case MAC_TIMER_T3180:
            ...
            mac_log_ul_tbf_release(MAC_LOG_UL_EXPIRED_T3180);
            mac_log_dl_tbf_release(MAC_LOG_DL_EXPIRED_T3180);
            break;
...
#endif /*FEATURE_GSM_EDTM*/
              {
                gmac_tran_log_state_change(TRANSFER_UL_DYNAMIC_REALLOC_DL_ASSIGN, __LINE__);
    ...
    ****************************************************/
    case TRANSFER_DL_REASSIGN_DYNAMIC_ALLOC:
      if(task_id == MS_MAC_L1)
      {
        ...
        {
          case MAC_PH_DATA_IND:
            gmac_transfer_dl_reassign_dynamic_alloc_data_ind();
            break;
          ...
          case MAC_PH_CONNECT_IND:
            ...
            if(transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              else
              {
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("GMAC AB Rel, Expected UL_DL (0x02) in PH_CONNECT_IND, received: 0x%x, initiated Recovery Restart",
                                      global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("GMAC AB Rel, Expected UL_DL (0x02) in PH_CONNECT_IND, received: 0x%x", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
    ...
    case TRANSFER_UL_DYNAMIC_REALLOC_DL_ASSIGN:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_realloc_dl_assign_data_ind();
            }
            break;
          ...
          case MAC_PH_CONNECT_IND:
            ...
            if(transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              else
              {
                ...
                {
                  ERR_GERAN_FATAL_1_G("MAC AB Rel, Expected UL_DL (0x02) in PH_CONNECT_IND, received: 0x%x", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
          ...
          case L1_MAC_TBF_REL_CONFIRM:
                    ...
                    && (transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE))
            {
              ...
              gmac_tran_log_state_change(TRANSFER_UL_DYNAMIC_DL_ASSIGN, __LINE__);
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          default:
            MSG_GERAN_LOW_3_G( "Unexpected GRR signal in state %d task id %d sig id 0x%x",
                                 transfer_data_ptr->mac_transfer_substate,task_id, signal_id);
            /* unknown signal from GRR */
            break;
        }
    ...
    case TRANSFER_CONCURRENT_DYNAMIC_TRANSFER:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_concurrent_data_ind();
            }
            break;
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          case RM_UL_RELEASE_IND:
            ...
            {
              MSG_GERAN_LOW_0_G("MAC Rcvd UL normal release cause");
              gmac_transfer_ul_dynamic_release_clean();
              gmac_util_block_rlc(NO_DEL, GMAC_TRAN, __LINE__);
              gmac_tran_send_mac_l1_release(UL_TBF_NORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_NORMAL_RELEASE);
          ...
          case RM_DL_PKT_ACKNACK_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd RM_DL_PKT_ACKNACK_REQ");
            gmac_transfer_dl_pdan_handler(&(global_data_ptr->msg_ptr->rlc_mac_sig.msg.dl_pkt_acknack_req),TRUE);
            break;
          ...
          case RM_DL_RELEASE_IND:
            ...
            {
              gmac_transfer_dl_release_clean();
              gmac_transfer_dl_normal_release_with_ul();
            }
            else
            {
              gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,DONT_REL_RLC_DL,NO_DEL,FALSE);
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_NO_PS_ACCESS_IND in T_CON_DYN");
    ...
    case TRANSFER_CONCURRENT_DYNAMIC_DL_REASSIGN:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_concurrent_dl_reassign_data_ind();
            }
            break;
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_NO_PS_ACCESS_IND in T_CON_DYN_DL_REASS");
            gmac_tran_check_dl_tbf_status();
    ...
    case TRANSFER_CONCURRENT_DYNAMIC_REALLOC:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_concurrent_realloc_data_ind();
            }
            break;
      ...
      else if(task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          default:
            MSG_GERAN_LOW_3_G( "Unexpected signal RLC in state %d task id %d sig id 0x%x",
                                 transfer_data_ptr->mac_transfer_substate,task_id, signal_id);
            /* unknown signal from RLC */
            break;
        }
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_NO_PS_ACCESS_IND in T_CON_DYN_REALLOC");
            gmac_tran_check_dl_tbf_status();
          ...
          case GRR_MAC_EDTM_PKT_CS_EST_REQ:
            ...
            gmac_transfer_edtm_pkt_cs_req_handler(
              UL_AND_DL,
              global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_edtm_pkt_cs_est_req.rr_est_cause,
              EDTM_FIRST_PKT_CS_REQUEST);          break;
#endif /*FEATURE_GSM_EDTM*/
    ...
    case TRANSFER_CONCURRENT_DL_REASSIGN_DYNAMIC_REALLOC:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case MAC_PH_DATA_IND:
            if(transfer_data_ptr->t_flags.block_dl_msg_during_abnormal_ul_release == FALSE)
            {
              gmac_tdynamic_concurrent_realloc_dl_reassign_data_ind();
            }
            break;
          ...
          case MAC_PH_CONNECT_IND:
            ...
            if (transfer_data_ptr->t_flags.abnormal_ul_and_dl_rel_flg == FALSE)
            {
              ...
              MSG_GERAN_MED_3_G("MAC Rcvd MAC_PH_CONNECT_IND for tbf %d, waiting_dl %d, waiting_ul %d",
                global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind,
              ...
              if (global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_DOWNLINK_TBF_ESTABLISHED)
              {
                ...
                gmac_eng_mode_update_tbf_info(global_data_ptr->gmac_current_mac_mode,
                                              global_data_ptr->current_tbf_mode, FALSE);
              ...
              else
              {
                ...
                {
                  MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CONNECT_IND for UL");
            ...
            {
              MSG_GERAN_LOW_0_G("MAC info ABNORMAL UL & DL rel in progress,Ignore PH_CONNECT_IND");
            }
            break;
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_NO_PS_ACCESS_IND in T_CON_DYN_REALL_DL_REASS");
    ...
    *****************************************************************/
    case TRANSFER_PS_HANDOVER:
      switch (global_data_ptr->gmac_ps_ho.psho_control_params.psho_substate)
      {
        ...
        case PSHO_WAITING_FOR_L1_CNF:
              ...
              case L1_MAC_PSHO_CNF:
                ...
                else
                {
                  if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_tbf_type == PSHO_UL_DL_TBF)
                  {
                    ...
                    gmac_tran_send_grlcdl_psho_complete_ind(PSHO_FAILURE);
                  }

                  /*GMAC sends psho complete ind to GRLC UL task with PSHO FAILURE*/
                  gmac_tran_send_grlcul_psho_complete_ind(PSHO_FAILURE);
                  ...
                  if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_type == PSHO_GTOG)
                  {
                    ...
                    gmac_tran_psho_process_saved_msgs(transfer_data_ptr->mac_transfer_substate);
                    ...
                    gmac_tran_psho_restart_timers(transfer_data_ptr->mac_transfer_substate);
                  }

                  /*GMAC sending PSHO CNF to GRR with PSHO FAILURE*/
                  gmac_util_send_grr_psho_cnf(PSHO_FAILURE);
          ...
          else if(task_id == MS_MAC_RLC)
          {
            switch (signal_id)
            {
              case RM_UL_RELEASE_IND:
              case RM_DL_RELEASE_IND:
                gmac_tran_store_rlc_release_ind(signal_id);
                break;
            ...
            switch (signal_id)
            {
              ...
              case MAC_TIMER_T3218:
                ...
                if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_type == PSHO_GTOG)
                {
                  ...
                  gmac_tran_send_gl1_psho_status(PSHO_FAILURE);
            ...
            switch (signal_id)
            {
              case L1_MAC_USF_DETECTED:
                        ...
                        ||(global_data_ptr->gmac_ps_ho.psho_control_params.psho_sync_type == PSHO_SYNC))
                {
                  ...
                  gmac_util_send_grr_psho_cnf(PSHO_SUCCESS);

                  gmac_stop_t3218();/*Stop T3218*/
                  gmac_stop_t3172();/*T3172 might be running so stop T3172*/
              ...
              case MAC_PH_DATA_IND:
                ...
                gmac_tran_ps_handover_data_ind();
                break;
              ...
              case GRR_MAC_PSHO_COMPLETE_IND:
                if(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_psho_complete_ind.psho_status == PSHO_SUCCESS)
                {
                  ...
                  if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_type == PSHO_GTOG)
                  {
                    ...
                    gmac_tran_send_grlcul_psho_complete_ind(PSHO_SUCCESS);

                    if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_tbf_type == PSHO_UL_DL_TBF)
                    {
                      /*update dl tbf global variables*/
                      /*GMAC sends complete ind with PSHO SUCCESS to GRLC DL task*/
                      gmac_tran_send_grlcdl_psho_complete_ind(PSHO_SUCCESS);
                    ...
                    else if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_tbf_type == PSHO_UL_TBF)
                    {
                      ...
                      gmac_transfer_initiate_local_release_of_old_cell_dl_tbfs();
                      gmac_transfer_dl_release_clean();
                    }
                    ...
                    gmac_tran_send_gl1_psho_status(PSHO_SUCCESS);
        ...
        default:
          if(geran_get_nv_recovery_restart_enabled())
          {
            MSG_GERAN_ERROR_1_G( "Unknown psho substate %d, initiated Recovery Restart",
              global_data_ptr->gmac_ps_ho.psho_control_params.psho_substate);
            geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
          }
          else
          {
            ERR_GERAN_FATAL_1_G("Unknown psho substate %d", global_data_ptr->gmac_ps_ho.psho_control_params.psho_substate);
          }
          /* unknown signal from RLC */
          break;
      }
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G( "Unknown state %d, initiated Recovery Restart",transfer_data_ptr->mac_transfer_substate);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("Unknown state %d", transfer_data_ptr->mac_transfer_substate);
      }
                                                                   ...
                                                                   )
{
  ...
  if(transfer_substate ==  TRANSFER_DL_REASSIGN)
  {
    ...
    if( rex_get_timer(&mac_timers_ptr->timer_t3172) == 0 )
    {
      ...
      if (gmac_util_is_rlc_blocked(GMAC_TRAN, __LINE__) == BLOCKED)
      {
        gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_UTIL, __LINE__);
      }
    }
    return(TRANSFER_DL);
  }
     ...
     ||(transfer_substate ==  TRANSFER_DL_REASSIGN_DYNAMIC_ALLOC))
  {
    ...
    {
      gmac_transfer_dl_release_clean();
      gmac_util_block_rlc(NO_DEL,GMAC_UTIL, __LINE__);
      gmac_tran_send_mac_l1_release(DL_TBF_NORMAL);
      global_data_ptr->c_flags.tbf_release_pending_flg = TRUE;
      transfer_data_ptr->t_flags.dl_tbf_normal_release_ind = FALSE;
    }
    return(TRANSFER_DL);
...
static void gmac_tran_send_gl1_psho_cancel_pending_config(void)
{
  ...
  gprs_l1_mac_acc_func(&mac_l1_sig);
}
/*===========================================================================
===
...
static void gmac_tran_send_grlcul_psho_complete_ind(psho_status_t psho_status)
{
  ...
  if (mac_rlc_sig_ptr != NULL)
  {
    ...
    grlc_put_cmd_mac(mac_rlc_sig_ptr);
  }
  else
  {
    ERR_GERAN_FATAL_0_G("MAC error MALLOC failed");
...
static void gmac_tran_psho_trigger_pccf_sending(gprs_mac_transfer_substates_t transfer_substate)
{
  ...
  rr_read_public_store(RRPS_TLLI,&local_rrps_tlli);
  ...
  if(gcsn1_encoder(PACKET_CELL_CHANGE_FAILURE, &(global_data_ptr->gmac_local_pccf_params.csn_encoded_pccf[1]), &input_data) == PASS)
  {
    ...
    {
      gmac_util_send_ul_control_msg_handler(PCCF, DL_ONLY_PRESENT, GRR_CNF_NOT_NEEDED);
    }
    else
    {
      gmac_util_send_ul_control_msg_handler(PCCF, UL_PRESENT, GRR_CNF_NOT_NEEDED);
    }
  }
}
/*===========================================================================
...
===
===========================================================================*/
void gmac_tran_psho_handler(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  ...
  if(gmac_tran_check_psho_addressing() == TRUE)
  {
    ...
    gmac_tran_send_gl1_psho_cancel_pending_config();
    MSG_GERAN_MED_0_G("GMAC ignores the pending tbfs ");
    transfer_data_ptr->mac_transfer_substate = gmac_tran_ignore_pending_tbfs(transfer_data_ptr->mac_transfer_substate);
    (void)mac_log_state();
  }
}
/*===========================================================================
...
#ifdef FEATURE_GSM_DTM
  gmac_tran_rel_state_handle_rel_cnf( tbf_released );
#endif /*FEATURE_GSM_DTM*/
...
static void gmac_tran_psho_process_saved_msgs(gprs_mac_transfer_substates_t transfer_substate)
{
  ...
  if(global_data_ptr->gmac_ps_ho.psho_control_params.tbf_rel_cnf_ptr != NULL)
  {
    ...
    if(transfer_substate == TRANSFER_DL)
    {
      ...
      gmac_tran_dl_process_rel_cnf(tbf_released);
    }
    else if(transfer_substate == TRANSFER_UL_DYNAMIC)
    {
      ...
      gmac_tran_ul_dynamic_process_rel_cnf(tbf_released);
    }
    else if(transfer_substate == TRANSFER_CONCURRENT_DYNAMIC_TRANSFER)
    {
      gmac_tran_concurrent_process_rel_cnf(tbf_released);
    }
  }
  if(global_data_ptr->gmac_ps_ho.psho_control_params.ul_release_ind_ptr != NULL)
  {
...
===
===========================================================================*/
boolean gmac_tran_check_psho_addressing(void)
{
  boolean addr_match = FALSE;
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    mac_header_tfi_check = gmac_mac_control_header_tfi_checking();
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
      if(psho_data_ptr->global_tfi_present == 0)
      {
        addr_match = gmac_util_global_tfi(psho_data_ptr->global_tfi);
      }
    }
...
)
{
  ...
  switch(transfer_data_ptr->mac_transfer_substate)
  {
    case TRANSFER_INACTIVE:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_0_G("MAC error not from T_INACTIVE, initiated Recovery Restart");
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
    ...
    case TRANSFER_DL_REASSIGN:
      ...
      gmac_transfer_ul_tbf_abnormal_abort(rlc_del_ul_pdu);
    ...
    case TRANSFER_UL_DYNAMIC_REALLOC:
      gmac_transfer_ul_dynamic_release_clean();
      gmac_util_block_rlc(rlc_del_ul_pdu, GMAC_TRAN, __LINE__);
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_0_G("MAC error invalid T substate, initiated Recovery Restart");
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
  ...
  */
  ...
  {
    gmac_send_mac_grr_abnormal_release(mac_release_cause);
                                       ...
                                       tbf_released_t     tbf_rel)
{
  ...
  if ( chan_rel_ptr->dtm_ps_release == TRUE )
  {
         ...
         ( chan_rel_ptr->dtm_ps_rel_cause == NORMAL_REL ) )
    {
      ...
      gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,TRUE);
      transfer_data_ptr->gmac_transfer_tbf_rel_cause = GMAC_TRANSFER_TBF_REL_GRR_NO_SERVICE;
    }
    else
    {
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_0_G("GMAC DTM Invalid dtm_ps_rel_cause, initiated Recovery Restart");
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
    ...
    gmac_transfer_tbf_rel_confirm(tbf_rel);
  }
  else /* Enhanced CS release, PS remaining active*/
  {
    ...
    if(geran_get_nv_recovery_restart_enabled())
    {
      MSG_GERAN_ERROR_0_G("GMAC Error in DTM Chan Rel, EDTM NOT currently supported, initiated Recovery Restart");
      geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
    }
    else
    {
                           ...
                           dtm_tbf_type_t                   current_tbf_assigned )
{
  ...
  if ( ass_status == NO_DTM_ASS_ERROR )
  {
    ...
    {
      gmac_send_mac_l1_freq_config( &mac_l1_freq_config );
    }

    gmac_send_mac_l1_dtm_channel_assign_req( &mac_l1_dtm_chan_ass_req );
  ...
  {
    gmac_send_mac_grr_dtm_channel_assign_rej( ass_status );
  }
...
static void gmac_tran_rel_state_handle_rel_cnf( tbf_released_t tbf_released )
{
  ...
  switch ( transfer_data_ptr->t_flags.gmac_tran_rel_state )
  {
    case NO_REL:
      /* no change in state */
      MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION in NO_REL state");
    break;
    ...
    case UL_NORM:
      switch ( tbf_released )
      {
        case DL_TBF_RELEASE_CONF:
          /* no change in state */
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, DL_TBF_RELEASE_CONF rcvd in UL_NORM state");
        break;
        ...
        case UL_DL_TBF_RELEASE_CONF:
          transfer_data_ptr->t_flags.gmac_tran_rel_state = NO_REL;
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_DL_TBF_RELEASE_CONF rcvd in UL_NORM state");
        break;

        default:
          MSG_GERAN_LOW_1_G("GMAC Rel State, BAD tbf_released value:0x%x",tbf_released);
        break;
      }
    break;
    ...
    case DL_NORM:
      switch ( tbf_released )
      {
        ...
        case UL_TBF_RELEASE_CONF:
          /* no change in state */
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_TBF_RELEASE_CONF rcvd in DL_NORM state");
        break;

        case UL_DL_TBF_RELEASE_CONF:
          transfer_data_ptr->t_flags.gmac_tran_rel_state = NO_REL;
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_DL_TBF_RELEASE_CONF rcvd in DL_NORM state");
        break;
    ...
    case UL_AB:
      switch ( tbf_released )
      {
        case DL_TBF_RELEASE_CONF:
          /* no change in state */
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, DL_TBF_RELEASE_CONF rcvd in UL_AB state");
        break;
        ...
        case UL_DL_TBF_RELEASE_CONF:
          transfer_data_ptr->t_flags.gmac_tran_rel_state = NO_REL;
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_DL_TBF_RELEASE_CONF rcvd in UL_AB state");
        break;
    ...
    case DL_AB:
      switch ( tbf_released )
      {
        ...
        case UL_TBF_RELEASE_CONF:
          /* no change in state */
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_TBF_RELEASE_CONF rcvd in DL_AB state");
        break;

        case UL_DL_TBF_RELEASE_CONF:
          transfer_data_ptr->t_flags.gmac_tran_rel_state = NO_REL;
          MSG_GERAN_LOW_0_G("GMAC Rel State, BAD TRANSITION, UL_DL_TBF_RELEASE_CONF rcvd in DL_AB state");
        break;
    ...
    default:
      MSG_GERAN_LOW_1_G("GMAC Rel State, BAD gmac_tran_rel_state value:0x%x",transfer_data_ptr->t_flags.gmac_tran_rel_state);
    break;
  }

  MSG_GERAN_LOW_1_G("GMAC Rel State, rel state changed to:0x%x",transfer_data_ptr->t_flags.gmac_tran_rel_state);
}
...
static void gmac_tran_rel_state_handle_connect_ind( uint8 tbf_type_connected )
{
  ...
  switch ( transfer_data_ptr->t_flags.gmac_tran_rel_state )
  {
    ...
    case UL_NORM:
      switch ( tbf_type_connected )
      {
        ...
        default:
          MSG_GERAN_LOW_1_G("GMAC Rel State, BAD tbf_type_connected value:0x%x",tbf_type_connected);
        break;
      }
    break;
  ...
  MSG_GERAN_LOW_1_G("GMAC Rel State, CONNECT_IND, rel state changed to:0x%x",transfer_data_ptr->t_flags.gmac_tran_rel_state);
}
...
void gmac_tran_rel_state_handle_rel_req( release_tbf_t tbf_rel_type )
{
    ...
    case NO_REL:
      switch ( tbf_rel_type )
      {
        ...
        case UL_AND_DL_TBF_NORMAL:
          MSG_GERAN_LOW_0_G("GMAC Rel State, MAC_L1_RELEASE, UL_AND_DL_TBF_NORMAL should not be used");
        break;
        ...
        default:
          MSG_GERAN_LOW_1_G("GMAC Rel State, BAD tbf_rel_type value:0x%x",tbf_rel_type);
        break;
      }
    break;
  ...
  MSG_GERAN_LOW_1_G("GMAC Rel State, MAC_L1_RELEASE, rel state changed to:0x%x",transfer_data_ptr->t_flags.gmac_tran_rel_state);
...
void gmac_tran_pcco_handler(gprs_mac_transfer_substates_t mac_tran_substate)
{
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
        ...
        {
          case tlli_valid:
            addr_match = gmac_match_old_or_new_tlli(pcco_data_ptr->tlli);
            break;

          case global_tfi_valid:
            /* if stored TFI = received TFI then match else ignore message */
            addr_match = gmac_util_global_tfi(pcco_data_ptr->global_tfi);
            break;
                                              ...
                                              gmac_freq_cnf_l1_cnf_msg_t  l1_cnf_msg)
{
  ...
  switch (l1_cnf_msg)
  {
    case GMAC_SEND_TS_RECONF_TO_L1:
      gmac_transfer_send_stored_ts_reconfig_to_l1();
    break;

    case GMAC_SEND_DL_CONF_TO_L1:
      gmac_transfer_send_stored_dl_config_to_l1();
    break;
    ...
    case GMAC_SEND_DYN_CONF_TO_L1:
      ...
      gprs_l1_mac_acc_func(&transfer_data_ptr->gmac_transfer_store_l1_dyn_realloc);
    break;

    default:
      MSG_GERAN_LOW_1_G("GMAC INFO BAD l1_cnf_msg: 0x%x",l1_cnf_msg);
    break;
  }
  ...
  switch (rlc_ass_msg)
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("GMAC INFO BAD rlc_ass_msg: 0x%x",rlc_ass_msg);
    break;
  }
}
...
===
===========================================================================*/
void gmac_transfer_simulate_freq_cnf(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  ...
  switch ( transfer_data_ptr->mac_transfer_substate )
  {
    case TRANSFER_DL_REASSIGN:
    case TRANSFER_CONCURRENT_DYNAMIC_DL_REASSIGN:
      gmac_transfer_send_freq_cnf_msgs(GMAC_SEND_DL_ASS_TO_RLC, GMAC_SEND_DL_CONF_TO_L1);
    break;

    case TRANSFER_UL_DYNAMIC_DL_ASSIGN:
      gmac_transfer_send_freq_cnf_msgs(GMAC_NO_RLC_ASS_MSG, GMAC_SEND_DL_CONF_TO_L1);
    break;

    case TRANSFER_DL_WITH_DYNAMIC_ALLOC:
      if (global_data_ptr->csn_dec_hdr_ptr->message_type == PACKET_UPLINK_ASSIGNMENT)
      {
        gmac_transfer_send_freq_cnf_msgs(GMAC_NO_RLC_ASS_MSG, GMAC_SEND_DYN_CONF_TO_L1);
      }
      ...
      else
      {
        if(geran_get_nv_recovery_restart_enabled())
        {
          MSG_GERAN_ERROR_2_G("GMAC unknown msg_type: 0x%x in transfer substate: 0x%x, initiated Recovery Restart",
                   global_data_ptr->csn_dec_hdr_ptr->message_type,transfer_data_ptr->mac_transfer_substate);
          geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
        }
        else
        {
          ERR_GERAN_FATAL_2_G("GMAC unknown msg_type: 0x%x in transfer substate: 0x%x", global_data_ptr->csn_dec_hdr_ptr->message_type,transfer_data_ptr->mac_transfer_substate);
        }
      }
    break;
    ...
    case TRANSFER_DL_REASSIGN_DYNAMIC_ALLOC:
      gmac_transfer_send_freq_cnf_msgs(GMAC_SEND_DL_ASS_TO_RLC, GMAC_SEND_TS_RECONF_TO_L1);
    break;
...
case TRANSFER_CONCURRENT_DL_REASSIGN_DYNAMIC_REALLOC:
      ...
      else
      {
        ...
        {
          ERR_FATAL("GMAC unknown msg_type: 0x%x in transfer substate: 0x%x", global_data_ptr->csn_dec_hdr_ptr->message_type,transfer_data_ptr->mac_transfer_substate, 0);
        }
      }
    ...
    case TRANSFER_UL_DYNAMIC_REALLOC_DL_ASSIGN:
      gmac_transfer_send_freq_cnf_msgs(GMAC_NO_RLC_ASS_MSG, GMAC_SEND_TS_RECONF_TO_L1);
    break;
    ...
    case TRANSFER_CONCURRENT_DYNAMIC_TRANSFER:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("GMAC simulate freq cnf called in wrong state: 0x%x, initiated Recovery Restart",
                  transfer_data_ptr->mac_transfer_substate);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("GMAC simulate freq cnf called in wrong state: 0x%x", transfer_data_ptr->mac_transfer_substate);
      }
    break;
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("GMAC unknown mac transfer substate: 0x%x, initiated Recovery Restart",transfer_data_ptr->mac_transfer_substate);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("GMAC unknown mac transfer substate: 0x%x", transfer_data_ptr->mac_transfer_substate);
      }
    break;
  }
}
...
static void gmac_transfer_edtm_t3196_expiry_handler(void)
{
  ...
  gmac_send_mac_l1_delete_ul_ctrl_msg(PACKET_CS_REQUEST);

  gmac_send_mac_grr_edtm_pkt_cs_est_rej(EDTM_CS_REJ_T3196_EXPIRY);
}
                                                  ...
                                                  mac_edtm_pkt_cs_req_retry_t cs_req_retry)
{
  ...
  else /*UL or UL_DL */
  {
    ...
    else if ( grlc_ul_is_cv_active() == TRUE )
    {
      send_edtm_pcr = FALSE;
  ...
  if (send_edtm_pcr)
  {
    if (cs_req_retry == EDTM_FIRST_PKT_CS_REQUEST)
    {
      ...
      gmac_start_t3196();
      gmac_start_edtm_pkt_cs_req_retry_timer();
    ...
    if(active_tbf == DL_ONLY)
    {
      pkt_cs_req_encoding = gmac_util_encode_ul_control_msg_handler(PCR,TRUE);
    }
    else
    {
      pkt_cs_req_encoding = gmac_util_encode_ul_control_msg_handler(PCR,FALSE);
    }
         ...
         active_tbf == UL_AND_DL )
    {
      ...
      {
        gmac_util_send_ul_control_msg_handler(PCR, UL_PRESENT, GRR_CNF_NEEDED);
      }
      else
      {
        if(geran_get_nv_recovery_restart_enabled())
        {
          MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart for EDTM PACKET CS REQ during Transfer");
          geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
        }
        else
        {
    ...
    else
    {
      ...
      gmac_util_send_ul_control_msg_handler(PCR, DL_ONLY_PRESENT, GRR_CNF_NEEDED);
    }
  }
  else
  {
    gmac_send_mac_grr_edtm_pkt_cs_est_rej(edtm_pcr_rej_cause);
  }
}
...
===
===========================================================================*/
void gmac_transfer_edtm_pkt_cs_cmd_handler(void)
{
  boolean addr_match = FALSE;
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
      addr_match = gmac_util_global_tfi(pkt_cs_cmd_ptr->global_tfi);
    }
    else if (mac_header_tfi_check == MAC_HEADER_TFI_MATCH_FOUND)
    {
      addr_match = TRUE;
...
static void gmac_transfer_edtm_clear_ul_tbf( mac_edtm_tbf_clear_action_t tbf_action )
{
  ...
  switch ( tbf_action )
  {
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("GMAC bad tbf_action: 0x%x, initiated Recovery Restart",tbf_action);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("GMAC bad tbf_action: 0x%x", tbf_action);
      }
    break;
  }
}
...
static void gmac_transfer_edtm_restore_tbf_data( active_tbf_t restore_tbf_type )
{
  ...
  switch ( restore_tbf_type )
  {
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("GMAC bad TBF type: %d, initiated Recovery Restart",restore_tbf_type);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("GMAC bad TBF type: %d", restore_tbf_type);
      }
    break;
  }
}
  ...
  switch(*transfer_state)
  {
    case TRANSFER_DL:
      switch ( dtm_tbf_type )
      {
        ...
        case UL_TBF_PRESENT:
          gmac_transfer_edtm_clear_dl_tbf(EDTM_CLEAR_ACTIVE_TBF);
        ...
        default:
           if(geran_get_nv_recovery_restart_enabled())
          {
            MSG_GERAN_ERROR_1_G("GMAC EDTM bad tbf type: %d, initiated Recovery Restart",dtm_tbf_type);
            geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
          }
          else
          {
            ERR_GERAN_FATAL_1_G("GMAC EDTM bad tbf type: %d", dtm_tbf_type);
          }
        break;
      }
      break;
    ...
    case TRANSFER_UL_DYNAMIC:
      switch ( dtm_tbf_type )
      {
          ...
          case DL_TBF_PRESENT:
            gmac_transfer_edtm_clear_ul_tbf(EDTM_CLEAR_ACTIVE_TBF);
            grlc_dl_start_t3190();
            transfer_data_ptr->t_flags.waiting_for_ph_con_for_dl = TRUE;
            *transfer_state = TRANSFER_DL_REASSIGN;
          break;
    ...
    case TRANSFER_DL_REASSIGN:
      ...
      {
          case NO_TBF_PRESENT:
            gmac_transfer_edtm_restore_tbf_data(DL_ONLY);
            transfer_data_ptr->t_flags.waiting_for_ph_con_for_dl = FALSE;
    ...
    case TRANSFER_UL_DYNAMIC_REALLOC:
      ...
      {
        case NO_TBF_PRESENT:
          gmac_transfer_edtm_restore_tbf_data(UL_ONLY);
    ...
    case TRANSFER_UL_DYNAMIC_DL_ASSIGN:
      ...
      {
          case NO_TBF_PRESENT:
            gmac_transfer_edtm_clear_dl_tbf(EDTM_CLEAR_PENDING_TBF);
            *transfer_state = TRANSFER_UL_DYNAMIC;
          break;
    ...
    case TRANSFER_CONCURRENT_DL_REASSIGN_DYNAMIC_REALLOC:
      ...
      {
        case NO_TBF_PRESENT:
          gmac_transfer_edtm_restore_tbf_data(UL_AND_DL);
          *transfer_state = TRANSFER_CONCURRENT_DYNAMIC_TRANSFER;
        break;
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("EDTM handler called in wrong state: 0x%x, initiated Recovery Restart",*transfer_state);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("EDTM handler called in wrong state: 0x%x", *transfer_state);
      }
    break;
  }
}
                                       ...
                                       active_tbf_t                  active_tbfs )
{
  ...
  mac_tran_str = gmac_convert_transfer_substate_to_string(transfer_state);
  ...
  if (addr_match)
  {
    ...
    {
      gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_CS_RELEASE);
    ...
    switch (pcri_tbf_type)
    {
      case NO_TBF_PRESENT:
        switch(transfer_state)
        {
          ...
          case TRANSFER_CONCURRENT_DYNAMIC_REALLOC:
          case TRANSFER_CONCURRENT_DL_REASSIGN_DYNAMIC_REALLOC:
            result = gmac_tran_edtm_pkt_cs_rel_msg_handler(pcri_ptr,active_tbfs,ctrl_ack_set);

            if (result == PCRI_PASS)
            {
              gmac_tran_edtm_cs_rel_handle_glob_pkt_ta(pcri_ptr->global_pkt_timing);
              gmac_send_mac_grr_edtm_cs_release_ind(EDTM_CS_REL_ENHANCED);
            }
            else if (result == PCRI_ABNORMAL_REL)
            {
              gmac_tran_edtm_cs_rel_handle_abnormal_rel();
            }
          break;
        ...
        switch(transfer_state)
        {
          ...
          case TRANSFER_DL_REASSIGN:
            MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC, PCRI Expected DL TBF, received UL TBF in %s, AB Release",
                            mac_tran_str);
            gmac_tran_edtm_cs_rel_handle_abnormal_rel();
          break;
          ...
          case TRANSFER_UL_DYNAMIC_REALLOC:
            ...
            if (result == PCRI_PASS)
            {
              if (pcri_ptr->freq_params_present)
              {
                ...
                (void)gmac_util_frequency_param_ie(FALSE,TRUE);
              }
              gmac_send_mac_grr_edtm_cs_release_ind(EDTM_CS_REL_ENHANCED);
              gmac_tran_send_mac_l1_edtm_cs_rel_tbf_alloc_msg(&transfer_data_ptr->mac_l1_edtm_cs_rel_alloc_msg);
              transfer_state = TRANSFER_UL_DYNAMIC_REALLOC;
            }
            else if (result == PCRI_ABNORMAL_REL)
            {
          ...
          case TRANSFER_CONCURRENT_DL_REASSIGN_DYNAMIC_REALLOC:
            MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC, PCRI Expected UL+DL TBF, received UL TBF in %s, AB Release",
                            mac_tran_str);
            gmac_tran_edtm_cs_rel_handle_abnormal_rel();
          break;
      ...
      case DL_TBF_PRESENT:
        switch(transfer_state)
        {
          ...
          case TRANSFER_UL_DYNAMIC_REALLOC:
            MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC, PCRI Expected UL TBF, received DL TBF in %s, AB Release",
                            mac_tran_str);
            gmac_tran_edtm_cs_rel_handle_abnormal_rel();
          break;
          ...
          case TRANSFER_DL_WITH_DYNAMIC_ALLOC:
            ...
            else
            {
              MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC, PCRI Expected UL+DL TBF, received DL TBF in %s, AB Release",
                            mac_tran_str);
              gmac_tran_edtm_cs_rel_handle_abnormal_rel();
            }
          break;
      ...
      case UL_DL_TBF_PRESENT:
        switch(transfer_state)
        {
          ...
          case TRANSFER_DL_REASSIGN:
            MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC, PCRI Expected DL TBF, received UL+DL TBF in %s, AB Release",
                            mac_tran_str);
            gmac_tran_edtm_cs_rel_handle_abnormal_rel();
          break;
          ...
          case TRANSFER_DL_REASSIGN_DYNAMIC_ALLOC:
            ...
            if (ctrl_ack_set == TRUE)
            {
              MSG_SPRINTF_1( MSG_SSID_DFLT,
                           MSG_LEGACY_MED,
                           "GMAC,PCRI Ctrl Ack removes DL, Cannot realloc UL in state: %s, AB Release",
                            mac_tran_str);
              gmac_tran_edtm_cs_rel_handle_abnormal_rel();
            }
            else
...
static void gmac_tran_edtm_cs_rel_handle_abnormal_rel(void)
{
  ...
  gmac_send_mac_grr_edtm_cs_release_ind(EDTM_CS_REL_ABNORMAL);
}
                                                                      ...
                                                                      boolean                  ctrl_ack_set)
{
  ...
  msg_validation_status = gmac_tran_edtm_validate_pkt_cs_rel_msg(pcri_ptr,active_tbfs,ctrl_ack_set);
  ...
  if ( msg_validation_status == PCRI_PASS )
  {
    ...
    if ( (pcri_ptr->num_mult_ul_assignments > 0) ||
         ( ( pcri_ptr->num_mult_dl_assignments > 0) &&
             ctrl_ack_set == FALSE) )
    {
      gmac_tran_edtm_populate_cs_rel_config_msgs(pcri_ptr,ctrl_ack_set);

      gmac_tran_edtm_handle_coding_scheme_change(pcri_ptr);
    }
...
static void gmac_tran_edtm_handle_coding_scheme_change(packet_cs_release_ind_t *pcri_ptr)
{
  ...
  if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
  {
    ...
    (void)gmac_get_egprs_mcs_from_pkt_cs_rel(pcri_ptr,&pcri_mcs);
  ...
  else if(global_data_ptr->current_tbf_mode == TBF_MODE_GPRS)
  {
    ...
    (void)gmac_get_cs_from_pkt_cs_rel(pcri_ptr,&pcri_cs);
                                                       ...
                                                       boolean                  ctrl_ack_set)
{
       ...
       ( transfer_data_ptr->mac_l1_edtm_cs_rel_alloc_msg.tbf_type == UL_TBF_PRESENT ) )
  {
    ...
    (void)gmac_get_ul_ts_alloc_from_pkt_cs_rel(pcri_ptr, &ul_ts_alloc);
    ...
    if (pcri_ptr->egprs_mode_tbfs == 1)
    {
      /**** mcs ****/
      /* return already checked in validate funciton*/
      (void)gmac_get_egprs_mcs_from_pkt_cs_rel(pcri_ptr,&transfer_data_ptr->gmac_transfer_ul_rlc_info_store.mcs);
      ...
      /**** ws ****/
      /* return already checked in validate funciton*/
      (void)gmac_get_ul_ws_from_pkt_cs_rel(pcri_ptr, &ul_egprs_ws);

      decoded_ul_ws = gmac_egprs_decode_ul_window_size(ul_egprs_ws, ul_ts_alloc);
    ...
    {
      /* return already checked in validate funciton*/
      (void)gmac_get_cs_from_pkt_cs_rel(pcri_ptr,&coding_scheme);
    ...
    /**** num_timeslots ****/
    transfer_data_ptr->gmac_transfer_ul_rlc_info_store.num_timeslots = gmac_utils_calculate_nts(ul_ts_alloc);
       ...
       ( transfer_data_ptr->mac_l1_edtm_cs_rel_alloc_msg.tbf_type == DL_TBF_PRESENT ) )
  {
    ...
    /**** current_egprs_ul_ws ****/
    /* return already checked in validate funciton*/
    (void)gmac_get_dl_ws_from_pkt_cs_rel(pcri_ptr, &dl_egprs_ws);
                                                                       ...
                                                                       boolean                  ctrl_ack_set)
{
  ...
  if ( gmac_edtm_pkt_cs_rel_tbfs_releasing(active_tbfs) == TRUE )
  {
  ...
  if (pcri_ptr->num_mult_dl_assignments == 1)
  {
    if (ctrl_ack_set == TRUE)
    {
      ...
      if (grlc_dl_t3192_status() != TMR_ACTIVE)
      {
  ...
  if (pcri_ptr->num_mult_ul_assignments == 1)
  {
    ...
    if ( gmac_get_ul_ts_alloc_from_pkt_cs_rel(pcri_ptr,&ul_ts_alloc) == FALSE )
    {
    ...
    if (gmac_get_usf_from_pkt_cs_rel(pcri_ptr,
                                    &transfer_data_ptr->mac_l1_edtm_cs_rel_alloc_msg) == FALSE )
    {
  ...
  if ( gmac_util_multislot_class_check(ul_ts_alloc,
                                       ...
                                       local_eda,
                                       0,
                                       FALSE, PSHO_UPDATE_PARAMS_NOT_REQUIRED ) == FALSE )
#else
  if ( gmac_util_multislot_class_check(ul_ts_alloc,
                                       dl_ts_alloc,
                                       local_eda,
  ...
  {
    if ( gmac_get_dl_ws_from_pkt_cs_rel(pcri_ptr, &dl_egprs_ws) == FALSE )
    {
      MSG_GERAN_HIGH_0_G("GMAC Pkt CS Rel, DL EGPRS window size not present, AB release ");
      return PCRI_ABNORMAL_REL;
    }

    if ( gmac_egprs_decode_dl_window_size(dl_egprs_ws,dl_ts_alloc) == WS_ALLOCATION_INVALID )
    {
       ...
       (pcri_ptr->egprs_mode_tbfs == 1) )
  {
    ...
    if ( gmac_get_ul_ws_from_pkt_cs_rel(pcri_ptr, &ul_egprs_ws) == FALSE )
    {
      MSG_GERAN_HIGH_0_G("GMAC Pkt CS Rel, UL EGPRS window size not present, AB release ");
      return PCRI_ABNORMAL_REL;
    }

    if ( gmac_egprs_decode_ul_window_size(ul_egprs_ws, ul_ts_alloc) == WS_ALLOCATION_INVALID )
    {
    ...
    if (gmac_get_egprs_mcs_from_pkt_cs_rel(pcri_ptr,&coding_scheme) == FALSE)
    {
  ...
  {
    if (gmac_get_cs_from_pkt_cs_rel(pcri_ptr,&coding_scheme) == FALSE)
    {
  ...
  if (pcri_ptr->freq_params_present)
  {
    if (gmac_util_frequency_param_ie(FALSE,FALSE) != NO_ERRORS )
    {
                                                  ...
                                                  uint8                   *egprs_mcs)
{
  ...
  if (status == TRUE)
  {
    if ( gmac_util_egprs_mcs_valid(*egprs_mcs) == FALSE )
    {
                                            ...
                                            mac_l1_edtm_cs_rel_tbf_alloc_t *edtm_cs_realloc_ptr)
{
  ...
  {
    num_ts_alloc = gmac_utils_calculate_nts(ul_ts_alloc);
                                                    ...
                                                    uint8                   *ts_alloc)
{
  ...
  *ts_alloc = 0;
  ...
  if ( gmac_utils_calculate_nts(pcri_ptr->multiple_ul_assignment.timeslot_description.ms_timeslot_allocation)
...
static boolean gmac_edtm_pkt_cs_rel_tbfs_releasing(active_tbf_t active_tbfs)
{
  ...
  {
    MSG_GERAN_LOW_1_G("GMAC bad active TBFs value: 0x%x",active_tbfs);
  }

  if (tbfs_releasing)
    MSG_GERAN_HIGH_2_G("GMAC EDTM DL releasing, AB CS rel, rel_state:0x%x, active_tbfs:0x%x",
      transfer_data_ptr->t_flags.gmac_tran_rel_state,
      active_tbfs);
...
char * gmac_convert_transfer_substate_to_string( gprs_mac_transfer_substates_t transfer_state )
{
  ...
  switch ( transfer_state )
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("GMAC Transfer substate invalid: 0x%x",transfer_state);
    break;
  }
...
===
===========================================================================*/
active_tbf_t gmac_tran_get_tbfs_active(void)
{
  ...
  switch (transfer_data_ptr->mac_transfer_substate)
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("GMAC gmac_tran_tbfs_active called in bad state:0x%x",transfer_data_ptr->mac_transfer_substate);
    break;
  }
...
===
===========================================================================*/
gprs_mac_transfer_substates_t gmac_tran_get_transfer_state(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
...
===
===========================================================================*/
transfer_flags_t gmac_tran_get_t_flags(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
                                   ...
                                   active_tbf_t active_tbf)
{
...
#ifdef FEATURE_GSM_GPRS_MSC33
  if ( gmac_utils_calculate_ts_alloc_ws(global_data_ptr->gmac_local_ul_ts_alloc) == GMAC_SHIFTED_USF_NUM_OF_UL_TS)
  {
    /* An UL window size of 5 means Shifted USF is being used */

    second_lowest_ts = gmac_utils_get_second_lowest_ts_allocated(global_data_ptr->gmac_local_ul_ts_alloc);
    ...
    {
      lowest_ts = gmac_utils_get_lowest_ts_allocated(global_data_ptr->gmac_local_ul_ts_alloc);
  ...
  else if(active_tbf == DL_ONLY)
  {
     if((global_data_ptr->gmac_local_dl_ts_alloc & timeslot_available) == 0)
     {
       ...
       gmac_transfer_ul_tbf_abnormal_abort(NO_DEL);
...
)
{
  ...
  {
    gmac_tran_rel_state_handle_rel_req( tbf_release_desc );
  }
#endif /*FEATURE_GSM_DTM*/
...
/*event logging changes*/
  mac_log_packet_tbf_rel_event( tbf_release_desc );
...
/*tlx pemr logging changes for idle cases */
  ...
  rrbp_sig_ptr->absolute_fn = gmac_rrbp_absolute_fn(fn, rrbp);
  ...
  gprs_l1_mac_acc_func(&ml_rrbp_sig);
...
===
===========================================================================*/
gprs_mac_transfer_substates_t gmac_transfer_get_transfer_state(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
...
void gmac_tran_log_state_change(gprs_mac_transfer_substates_t new_state, uint32 line)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();

  MSG_GERAN_MED_3_G("GMAC State TRANSFER %d -> %d, line %d", transfer_data_ptr->mac_transfer_substate, new_state, line);
}
