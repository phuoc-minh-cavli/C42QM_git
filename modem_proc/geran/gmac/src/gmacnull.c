...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacnull.c#6 $
***
...
***
*****************************************************************************/
static gmac_null_data_t gmac_null_data;
...
***
*****************************************************************************/
static void gmac_null_tbf_rel_confirm(void);
static void gmac_util_send_grr_mac_rach_cnf(void);

static void gmac_null_send_release_to_l1( gmac_null_tbf_rel_t null_rel_reason, release_tbf_t rel_type);

static void gmac_null_send_ul_tbf_req(uint8 access_type, uint8 radio_priority, boolean ps_page_resp);

static boolean gmac_null_handle_ul_sb_est_pmr(void);
static boolean gmac_null_handle_ul_sb_est_pccf(void);

static void gmac_null_send_sb_config(void);
...
static boolean gmac_null_ul_tbf_req(void);
...
static void gmac_null_check_for_polled_dl_ass(void);
...
static boolean gmac_null_handle_ul_sb_est_prr(void);

static gmac_null_data_t* gmac_get_null_data_ptr(void);
...
static void gmac_null_log_state_change(gprs_mac_null_substates_t new_state, uint32 line)
{
  gmac_null_data_t* null_data_ptr = gmac_get_null_data_ptr();

  MSG_GERAN_MED_3_G("GMAC State NULL %d -> %d, line %d", null_data_ptr->mac_null_substate, new_state, line);
}
...
===
===========================================================================*/
void gmac_init_null_data_non_zero(void)
{
  gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr();
...
===
===========================================================================*/
void gmac_set_mac_state_for_reset(void)
{
  gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr();
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();

  gmac_null_log_state_change(NULL_RESET, __LINE__);
...
static void gmac_null_tbf_rel_confirm(void)
{
  ...
  switch (null_data_ptr->gmac_null_tbf_rel_cause)
  {
    case GMAC_NULL_TBF_REL_NORMAL:
      ...
      gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_NULL,__LINE__);
      ...
      gmac_util_complete_pcco_process(PCCO_FAILURE_NO_RESPONSE,MAC_INITIATED,GMAC_NULL,__LINE__);
      gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF,MAC_INITIATED,GMAC_NULL,__LINE__);

      gmac_null_log_state_change(NULL_GPRS_51_ACTIVE, __LINE__);
    ...
    case GMAC_NULL_TBF_REL_UL_EST_INT_BY_DL_SB_EST:
      ...
      * DL SB has either been rcvd or the DL SB connection is terminated*/
      ...
      MSG_GERAN_LOW_0_G("MAC info action IA for DL SB now L1 has release UL connection");
      if(gmac_null_dl_sb_est_handler(&null_data_ptr->temp_gm_dl_sb_est,
                                      &null_data_ptr->gmac_null_mac_l1_dl_sb_config,
                                      &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
      {
       /* wait for starting time to occur before configuring RLC dl tbf */
       gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
       gmac_send_mac_l1_dl_sb_config(&null_data_ptr->gmac_null_mac_l1_dl_sb_config);
       ...
       gmac_null_log_state_change(NULL_DOWNLINK_SB_ESTAB, __LINE__);
      ...
      else
      {
        ...
        *est would not have been aborted and hence we would not have got here be here  */
        ...
        {
          MSG_GERAN_ERROR_0_G("GMAC info stored DL SB config has been corrupted, initiated Recovery Restart");
      ...
      gmac_util_send_grr_mac_status(MAC_IDLE_BLOCKED);
      break;
    ...
    case GMAC_NULL_TBF_REL_UL_EST_INT_BY_DL_EST:
      ...
      * DL has either been rcvd or the DL SB connection is terminated*/
      ...
      MSG_GERAN_LOW_0_G("MAC info action IA for DL now L1 has release UL connection");
                                      ...
                                      &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
      {
       /* wait for starting time to occur before configuring RLC dl tbf */
       gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
       gmac_send_mac_l1_dl_config(&null_data_ptr->gmac_null_mac_l1_dl_config);
       gmac_null_check_for_polled_dl_ass();
       ...
       gmac_null_log_state_change(NULL_DL_TBF_EST, __LINE__);
    ...
    case GMAC_NULL_GRR_INITIATED_NO_PS_ACCESS:
      ...
      /*complete any PCCO activity and PCCF activities*/
      gmac_util_complete_pcco_process(PCCO_FAILURE_NO_RESPONSE,GRR_INITIATED,GMAC_NULL,__LINE__);
      gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF,GRR_INITIATED,GMAC_NULL,__LINE__);

      gmac_null_log_state_change(NULL_INACTIVE, __LINE__);
      null_data_ptr->mac_null_substate = NULL_INACTIVE;
      (void)mac_log_state();
    ...
    default:
      ...
      MSG_GERAN_LOW_2_G("GMAC info No L1 release requested but L1_MAC_TBF_REL_CONFIRM rcvd. rel_cause %d, state %d",
  ...
  MSG_GERAN_MED_2_G("GMAC NULL TBF_REL_CNF, releasing_l1 %d, tbf_rel_cause %d",
...
static void gmac_null_send_release_to_l1( gmac_null_tbf_rel_t null_rel_reason, release_tbf_t rel_type)
{
  ...
  mac_log_packet_tbf_rel_event(rel_type);

  MSG_GERAN_MED_3_G("MAC Sent MAC_L1_RELEASE, current tbf_rel_cause %d, requested_rel %d, releasing_l1 %d ",
  ...
  /* Send MAC_L1_RELEASE */
  MSG_GERAN_LOW_0_G("MAC Sent MAC_L1_RELEASE");
...
static boolean gmac_null_handle_ul_sb_est_prr(void)
{
  ...
  if ( gmac_null_handle_ul_sb( fc_msg_ptr, mac_l1_sb_ptr ) == FALSE )
  {
    return FALSE;
  }
  ...
  {
    MSG_GERAN_MED_3_G("MAC egprs 2p IA with MB alloc, blocks_alloc %d, EGPRS_alloc:%d EGPRS_PCR:%d",
      (global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_num_radio_blocks + 1),
      global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_alloc,
      egprs_pkt_chan_req);
    ...
    if(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_alloc == TRUE)
    {
      ...
      {
        MSG_GERAN_LOW_1_G("MAC Error in MB alloc for,IA ignored. Reserved blocks allocated %d",
                           global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_num_radio_blocks + 1);
        return FALSE;
      }
      ...
      {
        MSG_GERAN_LOW_1_G("MAC ERR acc_tech_req out of range, count:%d",
    ...
    else if ( (egprs_pkt_chan_req == FALSE) && (global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_alloc == FALSE))
    {
      ...
      else
      {
        ...
        MSG_GERAN_LOW_0_G("MAC Error egprs alloc indicated but SB alloc assigned in IA,ignored IA ???");
          return FALSE;
      }
    }
    else
    {
      MSG_GERAN_LOW_2_G("MAC info ERROR in IA for SB/MB assigment egprs_pkt_chan_req %d egprs_alloc %d",
  ...
  {
   /* GPRS 2p ass*/
   MSG_GERAN_LOW_0_G("MAC gprs 2p access IA with SB alloc");
   global_data_ptr->egprs_2p_sb_mb_alloc = GPRS_SB_ALLOC;
  }
...
static boolean gmac_null_handle_ul_sb_est_pmr(void)
{
  ...
  if(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.egprs_ul_sb_est.egprs_alloc == TRUE)
  {
     ...
     MSG_GERAN_LOW_0_G("GMAC Received 2-phase alloc for EGPRS, Invalid NW response to Chan Req (Single Block)");
     return FALSE;
  }
                                       ...
                                       mac_sb_tbf_information_T *mac_l1_sb_ptr)
{
  ...
  if ( gmac_gprs_check_frequency_params(
              &(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.frequency_list),
                global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_sb_est.hopping_flag ) == FALSE )
  {
    MSG_GERAN_LOW_0_G("GMAC INFO Freq Band Check Passed");
  }
  else
  {
...
static void gmac_null_send_sb_config(void)
{
  ...
  gprs_l1_mac_acc_func(&null_data_ptr->gmac_null_ul_sb_store);
}
...
static boolean gmac_null_ul_dyn_tbf(mr_ul_alloc_cnf_t *store_rlc_ul_alloc_data_ptr)
{
  ...
  if(grr_msg_ptr->tbf_starting_time_flag == valid)
  {
    ...
    {
      gmac_rrbp_send_pca(grr_msg_ptr->tbf_starting_time, grr_msg_ptr->tn, FALSE, 0, PCA_51_MSG);
    }
  }
  else
  {
  ...
  if ( grr_msg_ptr->egprs_ul_tbf.egprs_alloc )
  {
    ...
    if ( grr_msg_ptr->egprs_ul_tbf.bep_period2_present )
    {
      if ( grr_msg_ptr->egprs_ul_tbf.bep_period2 > 15 )
      {
        ...
        MSG_GERAN_LOW_1_G("MAC ERR EGPRS bep_period2 out of range, BP2:0x%x",grr_msg_ptr->egprs_ul_tbf.bep_period2);
      }
      else
      {
  ...
  if ( grr_msg_ptr->egprs_ul_tbf.egprs_alloc )
  {
    ...
    if ( gmac_util_egprs_mcs_valid(grr_msg_ptr->egprs_ul_tbf.egprs_channel_coding_command) == FALSE )
    {
      egprs_tbf_valid = FALSE;
      MSG_GERAN_LOW_1_G("MAC ERR EGPRS chan code invalid ignore assignment, mcs:0x%x",
    ...
    if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_tx,store_rlc_ul_alloc_data_ptr->ws) == FAIL_RESULT)
    {
      egprs_tbf_valid = FALSE;
...
static boolean gmac_null_ul_tbf_req(void)
{
  ...
  gmac_global_data_t            *global_data_ptr = gmac_get_global_data_ptr();
    ...
    if (global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_tbf_est.alloc_flag == dynamic)
    {
      ...
      if ( !gmac_null_ul_dyn_tbf(&null_data_ptr->store_rlc_ul_alloc_data) )
      {
        return FALSE;
      }
    ...
    else
    {
      ...
      MSG_GERAN_LOW_0_G("MAC error UL TBF type not supported");
    ...
    if ( global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_tbf_est.egprs_ul_tbf.egprs_alloc )
    {
      ...
      {
        MSG_GERAN_MED_1_G("MAC ERR acc_tech_req out of range, count:%d",
    ...
    if ( gmac_gprs_check_frequency_params(
              &(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_tbf_est.frequency_list),
                global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_ul_tbf_est.hopping_flag ) == FALSE )
    {
      MSG_GERAN_LOW_0_G("GMAC INFO Freq Band Check Passed");
    }
    else
...
===
===========================================================================*/
void gmac_init_null_flags(void)
{
 gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr();
...
static void gmac_util_send_mr_ul_alloc_cnf(mr_ul_alloc_cnf_t *store_rlc_ul_alloc_data_ptr)
{
  ...
  if (mr_alloc_cnf_ptr != NULL)
  {
    ...
    grlc_put_cmd_mac(mr_alloc_cnf_ptr);
  }
  else
  {
...
static boolean gmac_check_egprs_dl_ass(gm_dl_tbf_est_t *dl_assign_ptr)
{
  ...
  if ( TRUE == dl_assign_ptr->egprs_alloc )
  {
    ...
    if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_rx,temp_ws) == FAIL_RESULT)
    {
      egprs_tbf_valid = FALSE;
    ...
    {
      egprs_tbf_valid = FALSE;
      MSG_GERAN_LOW_1_G("MAC ERR EGPRS Link Qual Meas Mode out of range :%d",dl_assign_ptr->egprs_lq_meas_mode);
    }
    ...
    if ( dl_assign_ptr->egprs_bep_period2_present == TRUE )
    {
      ...
      {
         egprs_tbf_valid = FALSE;
         MSG_GERAN_LOW_1_G("MAC ERR EGPRS BEP Period2 out of range :%d",dl_assign_ptr->egprs_bep_period2);
      }
    }
  }
...
static void  gmac_null_rej_dtm_ass (dtm_ass_status_t dtm_ass_rej)
{
    ...
    gmac_util_block_rlc(NO_DEL,GMAC_NULL,__LINE__);

    MSG_GERAN_MED_1_G("MAC GRR_MAC_DTM_CHANNEL_ASSIGN_REQ contains invalid ass reason%d",dtm_ass_rej);
    gmac_send_mac_grr_dtm_channel_assign_rej(dtm_ass_rej);
    gmac_util_send_grr_mac_status(MAC_IDLE);
}
...
)
{
  ...
  if ( gmac_gprs_check_frequency_params(
              &(dl_sb_assign_ptr->frequency_list),
                dl_sb_assign_ptr->hopping_flag) == FALSE )
  {
...
)
{
  ...
  if(gmac_check_egprs_dl_ass(dl_assign_ptr) == TRUE)
  {
    ...
    if ( gmac_gprs_check_frequency_params(
                &(dl_assign_ptr->frequency_list),
                  dl_assign_ptr->hopping_flag) == FALSE )
    {
      MSG_GERAN_LOW_0_G("GMAC INFO Freq Band Check Passed");
  ...
  if (tbf_ass_ok == FALSE)
    MSG_GERAN_MED_0_G("MAC info error in DL ass, Ignore");
  return(tbf_ass_ok);
}
...
static void gmac_null_check_for_polled_dl_ass(void)
{
  ...
  if (null_data_ptr->gmac_store_51access_polling.polling_flag == TRUE)
  {
    ...
    gmac_rrbp_send_pca(null_data_ptr->gmac_store_51access_polling.starting_time, null_data_ptr->gmac_store_51access_polling.tn, FALSE, 0, PCA_51_MSG);
    null_data_ptr->gmac_store_51access_polling.polling_flag = FALSE;
  }
...
===
===========================================================================*/
void null_mode_handler(uint8 task_id, uint8 signal_id)
{
  gmac_null_data_t   *null_data_ptr = gmac_get_null_data_ptr();
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_timers_t* mac_timers_ptr = gmac_get_timers_ptr();

  /* Reset the flag here as this msg is not processed in some of the sub-states. */
  if ((task_id == MS_MAC_MAC) && ((signal_id == MAC_TN_NO_SERVICE) || (signal_id == MAC_IN_NO_SERVICE)))
  {
    global_data_ptr->c_flags.no_service_penidng = FALSE;
  }
  ...
  if (task_id == MS_MAC_L1)
  {
    ...
    {
      MSG_GERAN_MED_3_G("MAC Rcvd NULL mode sig %d, task %d, substate %d", signal_id, task_id, null_data_ptr->mac_null_substate);
    }
  }
  else
  {
        ...
        switch (signal_id)
        {
          ...
          case GRR_MAC_PS_ACCESS_REQ:
            ...
            else
            {
              ...
              MSG_GERAN_LOW_0_G("GMAC T3172 Running. Wait for T3172 to expire before resuming PS access");

              gmac_util_block_rlc(NO_DEL,GMAC_IDLE,__LINE__);

               /*  52 page mode handling is under review              */
              /*reflects better what was in the original code Current*/
              gmac_util_initialise_page_mode();

              /* Initialise multislot class data - this is based on the NV multislot class */
              /* value in the Public Store                                                 */
              gmac_multislot_class_init();
              ...
              /*send cnf to RR*/
              gmac_send_mac_grr_ps_access_cnf();
          ...
          case GRR_MAC_RACH_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_RACH_REQ");
            gmac_util_send_grr_mac_rach_cnf();
            break;
      ...
      else if (task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          case RM_PS_ACCESS_CNF:
            ...
            if(gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__) == BLOCKED)
            {
          ...
          case RM_UL_EST_TBF_REQ:
            ...
            MSG_GERAN_LOW_0_G("GMAC Info Rcvd RM_UL_EST_TBF_REQ race condition sig Ignored");
            (void) gmac_util_is_rlc_blocked(GMAC_NULL,__LINE__);
            break;
      ...
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_SEND_MEAS_RPT:
            MSG_GERAN_MED_1_G("MAC Rcvd GRR_MAC_SEND_MEAS_RPT, T3172 running:%d",
            ...
            if( rex_get_timer(&mac_timers_ptr->timer_t3172) == 0 )
            {
              if (gmac_util_encode_ul_control_msg_handler(PMR,FALSE) == PASS)
              {
                ...
                gmac_util_send_grr_mac_status(MAC_IDLE_BLOCKED);
                gmac_null_send_ul_tbf_req(GMAC_SINGLE_BLOCK_ACCESS, RR_PRIORITY_CLASS_1,FALSE);
                gmac_null_log_state_change(NULL_UPLINK_SB_ESTAB, __LINE__);
            ...
            {
              MSG_GERAN_LOW_0_G( "GMAC T3172 running. Can't send PMR");
          ...
          case GRR_MAC_SEND_PCCF:
            ...
            if (gmac_util_encode_ul_control_msg_handler(PCCF,FALSE) == PASS)
            {
                ...
                gmac_util_start_pccf_process(SENDING_VIA_SB,GMAC_NULL,__LINE__,(uint8)NULL_GPRS_51_ACTIVE);
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC info Failed to CSN encode PCCF");
          ...
          case GRR_MAC_DL_SB_EST:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_DL_SB_EST");
            ...
            {
             /*assignment error*/
              MSG_GERAN_LOW_0_G("MAC info error in DL SB ass, Ignore IA");
            }

            break;

          case GRR_MAC_DL_TBF_EST:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_DL_TBF_EST");
          ...
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
            ...
            {
              MSG_GERAN_LOW_0_G("MAC rcvd GRR_MAC_DTM_CHANNEL_ASSIGN_REQ :- DL_TBF_PRESENT ");
              ...
              if (null_data_ptr->gmac_null_dtm_ass_status == NO_DTM_ASS_ERROR)
              {
                ...
                gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
                gmac_send_mac_l1_dtm_channel_assign_req(&null_data_ptr->gmac_null_mac_l1_dtm_chan_ass_req);
              ...
              {
                gmac_null_rej_dtm_ass(null_data_ptr->gmac_null_dtm_ass_status);
                gmac_null_log_state_change(NULL_INACTIVE, __LINE__);
                null_data_ptr->mac_null_substate = NULL_INACTIVE;
              }
            }
            ...
            {
              gmac_null_rej_dtm_ass(DTM_ASS_REJ_PROTOCOL_ERR);
              gmac_null_log_state_change(NULL_INACTIVE, __LINE__);
              null_data_ptr->mac_null_substate = NULL_INACTIVE;
            }
            break;
          ...
          case GRR_MAC_DTM_UL_DCCH_IND:
            MSG_GERAN_LOW_0_G("MAC unexpected recpeiton of GRR_MAC_DTM_UL_DCCH_IND");
            break;
#endif /* FEATURE_GSM_DTM */

#ifdef FEATURE_GSM_EDTM
          case GRR_MAC_EDTM_PKT_CS_EST_REQ:
            MSG_GERAN_LOW_0_G("GMAC Rejected GRR_MAC_EDTM_PKT_CS_EST_REQ, Not in Contention Resolution");
            gmac_send_mac_grr_edtm_pkt_cs_est_rej(EDTM_CS_REJ_CANNOT_SERVICE_REQUEST);
          break;
#endif /*FEATURE_GSM_EDTM*/
      ...
      else if (task_id == MS_MAC_MAC)
      {
        switch (signal_id)
        {
          ...
          case MAC_TN_NO_SERVICE:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_TN_NO_SERVICE");
          ...
          case MAC_IN_NO_SERVICE:
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_IN_NO_SERVICE");
          ...
          case MAC_TIMER_T3172:
            MSG_GERAN_LOW_1_G("MAC Rcvd MAC_TIMER_T3172, NO_SERVICE_PEND %d",
              global_data_ptr->c_flags.no_service_penidng);

            if (global_data_ptr->c_flags.no_service_penidng == FALSE)
            {
              /* If t3172 is runninig RLC will have been blocked. Unblock RLC*/
              gmac_util_unblock_rlc(NO_CELL_CHANGE_PURGE_GMM_SIG,GMAC_NULL,__LINE__);
            }
            break;
      ...
      else if (task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          case RM_UL_EST_TBF_REQ:
            ...
            {
              MSG_GERAN_LOW_1_G("GMAC TEST Mode: %d, ordered when in NULL", 
            ...
            global_data_ptr->access_requested = determine_access_requested();
            ...
            MSG_GERAN_HIGH_2_G("MAC Rcvd RM_UL_EST_TBF_REQ, UL est tlli %x, gmac_store_ul_est_tlli %x",
            ...
            gmac_null_dtm_send_ul_tbf_req(null_data_ptr->rlc_access_type,
                                          &(global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_est_tbf_req));
#else
            ...
            gmac_null_log_state_change(NULL_UPLINK_TBF_ESTAB, __LINE__);
      ...
      else if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            MSG_GERAN_LOW_1_G("GMAC info rcvd L1_MAC_TBF_REL_CONFIRM should not get this signal in this state gmac_null_tbf_rel_cause %d",
        ...
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            ...
            if(null_data_ptr->gmac_null_releasing_l1 == TRUE )
            {
              ...
              gmac_null_send_release_to_l1(GMAC_NULL_GRR_INITIATED_NO_PS_ACCESS,UL_TBF_ABNORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_CELL_RESELECTION);
            }
            else
            {
          ...
          case GRR_MAC_UL_TBF_EST:
            if ( gmac_null_ul_tbf_req() == TRUE )
            {
              ...
              if((global_data_ptr->access_tech_req_present == TRUE) && (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS))
              {
               ...
               gmac_idle_egprs_rac_procedure_2(FALSE,FALSE,FALSE,FALSE);
              }

              /* send MAC_L1_DUMMY_CTRL_BLOCK to L1 */
              gmac_ul_dummy_blk_to_l1();

              /*if pccf has to be sent*/
              if(gmac_util_get_pccf_state() == SENDING_VIA_UL_PACCH)
              {
                MSG_GERAN_LOW_0_G("MAC info PCCF SENT TO L1");
              ...
              MSG_GERAN_MED_0_G("MAC Rcvd GRR_MAC_UL_TBF_EST, Sent MAC_L1_DYNAMIC_TBF_CONFIG");
              ...
              gmac_send_mac_l1_dyn_config(&null_data_ptr->gmac_null_store_l1_dyn_realloc.mac_l1_dynamic_tbf_config);

              gmac_null_log_state_change(NULL_UPLINK_TBF_ACCESS, __LINE__);
            ...
            else
            {
              ...
              gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_NORMAL,UL_TBF_ABNORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_ASSIGNMENT_DATA_ERROR);
            }
            break;

          case GRR_MAC_UL_SB_EST:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_UL_SB_EST");

#ifdef FEATURE_GSM_DTM
            global_data_ptr->gmac_dtm_cs_ts_alloc = GMAC_NO_TS_ALLOCATED_IN_BITMAP;
#endif /*FEATURE_GSM_DTM*/

            if ( gmac_null_handle_ul_sb_est_prr() == TRUE )
            {
              gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
              ...
              gmac_restart_ul_await_ass_guard(TRUE,global_data_ptr->gmac_51_sb_params.sb_tbf.starting_time);

              gmac_send_mac_l1_sb_config(&global_data_ptr->gmac_51_sb_params);
            ...
            {
              MSG_GERAN_LOW_0_G("MAC info error in GRR_MAC_UL_SB_EST ass, abort estab");
          ...
          case GRR_MAC_DL_TBF_EST:
                                            ...
                                            &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
            {
              ...
              **********************************************************************/
              ...
              MSG_GERAN_LOW_0_G("MAC info UL int by DL TBF");
             ...
             * establishes  the interrupted UL                                             */
             MSG_GERAN_LOW_0_G("MAC info error in GRR_MAC_DL_TBF_EST ass, abort estab");
             gmac_util_block_rlc(NO_DEL,GMAC_NULL,__LINE__);
             null_data_ptr->gmac_null_tbf_rel_cause = GMAC_NULL_TBF_REL_NORMAL;
             gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_NORMAL,UL_TBF_ABNORMAL);
             mac_log_ul_tbf_release(MAC_LOG_UL_ASSIGNMENT_DATA_ERROR);
          ...
          case GRR_MAC_SEND_PCCF:
            ...
            if (gmac_util_encode_ul_control_msg_handler(PCCF,FALSE) == PASS)
            {
              gmac_util_start_pccf_process(SENDING_VIA_UL_PACCH,GMAC_NULL,__LINE__,(uint8)NULL_UPLINK_TBF_ESTAB);
            }
            else
            {
              MSG_GERAN_LOW_0_G("GMAC info Failed to CSN encode PCCF");
          ...
          case GRR_MAC_DL_SB_EST:
            ...
            else
            {
              ...
              * establishes  the interrupted UL                                             */
              ...
              MSG_GERAN_LOW_0_G("MAC info error in GRR_MAC_DL_SB_EST ass, abort estab");
              gmac_util_block_rlc(NO_DEL,GMAC_NULL,__LINE__);
              null_data_ptr->gmac_null_tbf_rel_cause = GMAC_NULL_TBF_REL_NORMAL;
              gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_NORMAL,UL_TBF_ABNORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_ASSIGNMENT_DATA_ERROR);
          ...
          case GRR_MAC_DTM_CHANNEL_ASSIGN_REQ:
                 ...
                 ( global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_dtm_channel_assign_req.dtm_tbf_type == UL_DL_TBF_PRESENT ) )
            {
              ...
              if ( null_data_ptr->gmac_null_dtm_ass_status == NO_DTM_ASS_ERROR)
              {
                ...
                {
                  gmac_ul_dummy_blk_to_l1();
                  gmac_util_send_mr_ul_alloc_cnf(&null_data_ptr->gmac_null_mr_ul_alloc_cnf_sig);
                }
                else
                {
                  MSG_GERAN_LOW_0_G("MAC INFO UL request interrupted by DL est");
                  /*establishing DL block RLC until DL has been connected*/
                  gmac_util_block_rlc(NO_DEL,GMAC_NULL,__LINE__);
                }

                gmac_send_mac_l1_dtm_channel_assign_req(&null_data_ptr->gmac_null_mac_l1_dtm_chan_ass_req);
                gmac_null_log_state_change(NULL_DTM_CONNECT_PENDING, __LINE__);
                null_data_ptr->mac_null_substate = NULL_DTM_CONNECT_PENDING;
              }
              else
              {
            ...
            {
              MSG_GERAN_LOW_1_G("MAC Rcvd GRR_MAC_DTM_CHANNEL_ASSIGN_REQ dtm tbf type %d",
                                global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_dtm_channel_assign_req.dtm_tbf_type);
          ...
          case GRR_MAC_DTM_UL_DCCH_IND:
            ...
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_DTM_UL_DCCH_IND");
            ...
            gmac_util_send_mr_ul_alloc_cnf(&null_data_ptr->store_rlc_ul_alloc_data);

            gmac_null_log_state_change(NULL_DTM_GTTP_UL_CON, __LINE__);
        ...
        switch (signal_id)
        {
          case L1_MAC_TBF_REL_CONFIRM:
            ...
            else
            {
              ...
              if(geran_get_nv_recovery_restart_enabled())
              {
                MSG_GERAN_ERROR_1_G("MAC error TBF_REL_CNF %d, initiated Recovery Restart",global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
                geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
              }
              else
              {
                ERR_GERAN_FATAL_1_G("MAC error TBF REL CNF %d", global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
              }
        ...
        switch (signal_id)
        {
          ...
          case GRR_MAC_DL_TBF_EST:
                                            ...
                                            &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
            {
               ...
               gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_UL_EST_INT_BY_DL_EST,UL_TBF_ABNORMAL);
               mac_log_ul_tbf_release(MAC_LOG_UL_PDA_ABORT);
          ...
          case GRR_MAC_DL_SB_EST:
                                            ...
                                            &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
            {
              ...
              MSG_GERAN_LOW_0_G("GMAC info abort UL est, rel L1 and wait in state for rel confirm");
              /*Block RLC*/
              gmac_util_block_rlc(NO_DEL,GMAC_NULL,__LINE__);

              /*release L1 and wait in state for l1 release confirmation */
              gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_UL_EST_INT_BY_DL_SB_EST,UL_TBF_ABNORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_PDA_ABORT);
            ...
            {
             /*assignment error*/
             MSG_GERAN_LOW_0_G("MAC info error DL SB in ass, Ignore IA continue with UL est");
            }
            break;

          case GRR_MAC_RACH_REQ:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_RACH_REQ");
            gmac_util_send_grr_mac_rach_rej();
            break;

          case GRR_MAC_UL_TBF_EST:
            MSG_GERAN_LOW_0_G("GMAC INFO Not expecting second GRR_MAC_UL_TBF_EST in NULL_UPLINK_TBF_ACCESS");
      ...
      else if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          ...
          case L1_MAC_FREQ_ASS_CNF:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_FREQ_ASS_CNF, ignored for Freq Cnf Cleanup");
          ...
          case MAC_PH_CONNECT_IND:
            MSG_GERAN_MED_2_G("MAC Rcvd MAC_PH_CONNECT_IND, tbf_type:%d, releasing_l1: %d",
              global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind,
              null_data_ptr->gmac_null_releasing_l1);
            ...
            if(null_data_ptr->gmac_null_releasing_l1 == FALSE )
            {
              if (global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_TBF_ESTABLISHED)
              {
                ...
                gmac_util_send_mr_ul_alloc_cnf(&null_data_ptr->store_rlc_ul_alloc_data);
                MSG_GERAN_LOW_0_G("MAC Sent MAC_NI_UL_1P_ASS");
                ...
                MSG_GERAN_MED_0_G("MAC Sent MAC_NI_UL_1P_ASS, MAC mode IDLE");
                (void)mac_log_state();
              }
              else
              {
                MSG_GERAN_LOW_1_G("MAC error ABNORMAL REL connect fail tbf_type_ind= %d",
                                   global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC info Releasing L1, MAC_PH_CONNECT_IND ignored");
            }
            break;
    ...
    case NULL_UPLINK_SB_ESTAB:
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            ...
            MSG_GERAN_MED_3_G("GMAC Rcvd GRR_MAC_NO_PS_ACCESS_IND pdu deletion type %d, ul_sb_to_l1 %d, releasing_l1 %d",
              global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_no_ps_access_ind.delete_pdu,
              null_data_ptr->n_flags.gmac_null_ul_sb_to_l1,
              null_data_ptr->gmac_null_releasing_l1);
            ...
            *******************************************************************************/
            ...
            switch ( null_data_ptr->gmac_null_ul_sb_type )
            {
              case NULL_UL_SB_TYPE_PCCF:
                if(gmac_null_handle_ul_sb_est_pccf() == TRUE)
                {
                  /******* simulate Freq Conf being received ******************************/
                  gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
                  gmac_util_send_ul_control_msg_handler(PCCF, UL_PRESENT, GRR_CNF_NOT_NEEDED);
                  gmac_null_send_sb_config();
                  null_data_ptr->n_flags.gmac_null_ul_sb_to_l1 = TRUE;
                }
                else
                {
              ...
              case NULL_UL_SB_TYPE_PMR:
                if(gmac_null_handle_ul_sb_est_pmr() == TRUE)
                {
                  /******* simulate Freq Conf being received ******************************/
                  gmac_send_mac_l1_freq_config(&null_data_ptr->gmac_null_mac_l1_freq_config);
                  gmac_util_send_ul_control_msg_handler(PMR, UL_PRESENT, GRR_CNF_NOT_NEEDED);
                  gmac_null_send_sb_config();
                  null_data_ptr->n_flags.gmac_null_ul_sb_to_l1 = TRUE;
                }
                else
              ...
              default:
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("GMAC INFO unxpected gmac_null_ul_sb_type:0x%x, initiated Recovery Restart",null_data_ptr->gmac_null_ul_sb_type);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("GMAC INFO unxpected gmac_null_ul_sb_type:0x%x", null_data_ptr->gmac_null_ul_sb_type);
                }
              break;
            }
            break;
                                            ...
                                            &null_data_ptr->gmac_null_mac_l1_freq_config)  == TRUE)
            {
              ...
              else
              {
                ...
                if (null_data_ptr->gmac_null_ul_sb_type == NULL_UL_SB_TYPE_PRR)
                {
                  ...
                  if(geran_get_nv_recovery_restart_enabled())
                  {
                    MSG_GERAN_ERROR_0_G("GMAC unexpected UL SB type NULL_UL_SB_TYPE_PRR, initiated Recovery Restart");
                    geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                  }
                  else
                  {
          ...
          case GRR_MAC_DL_TBF_EST:
            ...
            {
             /*assignment error*/
              MSG_GERAN_LOW_0_G("MAC info error in GRR_MAC_DL_TBF_EST, Abort Est");
          ...
          case GRR_MAC_SEND_PCCF:
            ...
            if (gmac_util_encode_ul_control_msg_handler(PCCF,FALSE) == PASS)
            {
              ...
              if (null_data_ptr->gmac_null_ul_sb_type == NULL_UL_SB_TYPE_PMR)
              {
                ...
                gmac_send_mac_l1_delete_ul_ctrl_msg(PACKET_MEASUREMENT_REPORT);
#endif /*FEATURE_GSM_EDTM*/
          ...
          case GRR_MAC_SEND_MEAS_RPT:
            ...
            MSG_GERAN_LOW_0_G("GMAC RCVD MAC Rcvd GRR_MAC_SEND_MEAS_RPT while est UL: fail to send PMR");
            gmac_util_send_pmr_cnf_to_grr(MAC_GRR_FAILURE);
            break;
          ...
          case GRR_MAC_UL_TBF_EST:
            ...
            *******************************************************************************/
            ...
            MSG_GERAN_LOW_0_G("GMAC Rcvd unexpected GRR_MAC_UL_TBF_EST");
            ...
            switch (null_data_ptr->gmac_null_ul_sb_type)
            {
              ...
              default:
                MSG_GERAN_LOW_0_G("GMAC ignored GRR_MAC_UL_TBF_EST");
              break;
            }
            break;
      ...
      else if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          ...
          case L1_MAC_FREQ_ASS_CNF:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_FREQ_ASS_CNF rxd ignored for Freq Cleanup");
          ...
          case L1_MAC_QUAL_MEASURMENTS:
            if(null_data_ptr->gmac_null_releasing_l1 == FALSE )
            {
              ...
              if(global_data_ptr->egprs_2p_sb_mb_alloc == GPRS_SB_ALLOC)
              {
                gmac_send_prr(global_data_ptr->idle_mode_ul_tbf_release_action,FALSE);
              }
              else
              {
                gmac_idle_egprs_rac_procedure_2(TRUE,global_data_ptr->c_flags.egprs_rac_retran_req,FALSE,FALSE);
              }
            }
            else
            {
              /*Releasing L1 no action wait for L1_MAC_TBF_REL_CONFIRM*/
              MSG_GERAN_LOW_0_G("GMAC Info L1_MAC_QUAL_MEASURMENTS, No action wait for L1_MAC_TBF_REL_CONFIRM");
            }
          break;
          ...
          case MAC_PH_CONNECT_IND:
            if(null_data_ptr->gmac_null_releasing_l1 == FALSE )
            {
              if ( null_data_ptr->gmac_null_ul_sb_type == NULL_UL_SB_TYPE_PRR )
              {
                ...
                gmac_null_log_state_change(NULL_GPRS_51_ACTIVE, __LINE__);
                MSG_GERAN_MED_0_G("MAC Rcvd MAC_PH_CONNECT_IND, MAC mode IDLE");
                null_data_ptr->mac_null_substate = NULL_GPRS_51_ACTIVE;
                global_data_ptr->mac_mode = IDLE_MODE;
                (void)mac_log_state();
              }
              else
              {
                ...
                MSG_GERAN_LOW_0_G("GMAC INFO MAC_PH_CONNECT_IND,No action wait for L1_MAC_SINGLE_BLOCK_SENT");
              }
            }
            else
            {
          ...
          case L1_MAC_SINGLE_BLOCK_SENT:
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_SINGLE_BLOCK_SENT");
            ...
            if(null_data_ptr->gmac_null_releasing_l1 == FALSE)
            {
              ...
              if (null_data_ptr->gmac_null_ul_sb_type == NULL_UL_SB_TYPE_PCCF)
              {
                gmac_util_complete_pccf_process(SUCCESSFULLY_SENT_PCCF,MAC_INITIATED,GMAC_NULL,__LINE__);
              }
              else
              {
                gmac_util_send_pmr_cnf_to_grr(MAC_GRR_SUCCESS);
                memset(&global_data_ptr->gmac_local_pmr_params,0,sizeof(gmac_pmr_params_t));
              }
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC info Releasing L1, L1_MAC_SINGLE_BLOCK_SENT ignored");
            }
            break;

          case L1_MAC_51_SB_FAILURE:
            MSG_GERAN_MED_2_G("MAC Rcvd L1_MAC_51_SB_FAILURE, cause:%d, sb_type:%d",
              null_data_ptr->gmac_null_tbf_rel_cause,
              null_data_ptr->gmac_null_ul_sb_type);
            ...
            if (null_data_ptr->gmac_null_tbf_rel_cause == GMAC_NULL_NO_RELEASE_IN_PROGRESS)
            {
              ...
              if( null_data_ptr->gmac_null_ul_sb_type == NULL_UL_SB_TYPE_PRR)
              {
                MSG_GERAN_LOW_0_G("GMAC info L1_MAC_51_SB_FAILURE during 2p access");
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            ...
            else
            {
              if(geran_get_nv_recovery_restart_enabled())
              {
                MSG_GERAN_ERROR_1_G("MAC error TBF_REL_CNF %d, initiated Recovery Restart",
    ...
    case NULL_DOWNLINK_SB_ESTAB:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          ...
          case L1_MAC_FREQ_ASS_CNF:
            MSG_GERAN_LOW_0_G("MAC rvcd L1_MAC_FREQ_ASS_CNF, no action");
          ...
          case MAC_PH_CONNECT_IND:
            MSG_GERAN_MED_1_G("MAC Rcvd MAC_PH_CONNECT_IND, releasing_l1", null_data_ptr->gmac_null_releasing_l1);
            if (null_data_ptr->gmac_null_releasing_l1 == FALSE)
            {
            ...
            {
              MSG_GERAN_LOW_0_G("GMAC Ignore PH_CONNECT_IND, releasing in NULL_DOWNLINK_SB_ESTAB state");
            }
            break;

          case L1_MAC_51_SB_FAILURE:
            MSG_GERAN_MED_1_G("MAC Rcvd L1_MAC_51_SB_FAILURE, cause: %d", null_data_ptr->gmac_null_tbf_rel_cause);
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            MSG_GERAN_MED_1_G("MAC Rcvd L1_MAC_TBF_REL_CONFIRM, tbf_type:%d",
              global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
            ...
            {
              MSG_GERAN_LOW_1_G(" **** MAC info L1 rel cnf for UL, absorb %d *****",
                                 global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
            }
            break;
      ...
      else if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_NO_PS_ACCESS_IND:
            ...
            gmac_null_send_release_to_l1(GMAC_NULL_GRR_INITIATED_NO_PS_ACCESS,DL_TBF_ABNORMAL);
            mac_log_ul_tbf_release(MAC_LOG_UL_CELL_RESELECTION);
            break;
          ...
          case GRR_MAC_SEND_PCCF:
            MSG_GERAN_LOW_0_G("GMAC Unexpected GRR_MAC_SEND_PCCF while performing DL SB est. Sig Ignored");
        ...
        switch (signal_id)
        {
          case MAC_PH_CONNECT_IND:
            ...
            if (null_data_ptr->gmac_null_releasing_l1 == FALSE)
            {
              MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CONNECT_IND");
              {
                ...
                global_data_ptr->mac_mac_sig.task_id = MS_MAC_MAC;
                gprs_mac_mac_acc_func(&global_data_ptr->mac_mac_sig);
                gmac_util_send_grr_mac_status(MAC_TRANSFER);

                global_data_ptr->mac_mode = TRANSFER_MODE;
                MSG_GERAN_HIGH_1_G("MAC Sent MAC_NT_DL_DYNAMIC, MAC set MAC_MODE : %d",
                  global_data_ptr->mac_mode);
              }
            }
            else
            {
              MSG_GERAN_LOW_0_G("GMAC Ignore MAC_PH_CONNECT_IND rxd whilst releasing in NULL_DL_TBF_EST");
            }
            break;
          ...
          case L1_MAC_TBF_REL_CONFIRM:
            ...
            else
            {
              if(geran_get_nv_recovery_restart_enabled())
              {
                MSG_GERAN_ERROR_1_G("MAC error TBF REL CNF %d, initiated Recovery Restart",global_data_ptr->msg_ptr->l1_mac_sig.msg.tbf_released.tbf_released);
                geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
              }
              else
              {
    ...
    case NULL_DTM_CONNECT_PENDING:
      if(task_id == MS_MAC_L1)
      {
        switch (signal_id)
        {
          case L1_MAC_FREQ_ASS_CNF:
            /*NO action*/
            MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_FREQ_ASS_CNF");
            break;
          ...
          case MAC_PH_CONNECT_IND:
            if (null_data_ptr->gmac_null_releasing_l1 == FALSE)
            {
              ...
              if(global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_TBF_ESTABLISHED)
              {
                ...
                global_data_ptr->mac_mode = TRANSFER_MODE;
                MSG_GERAN_HIGH_1_G("MAC Sent MAC_NT_UL_DTM_DYNAMIC, MAC set MAC_MODE : %d",
                  global_data_ptr->mac_mode);
              ...
              else if(global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == DOWNLINK_TBF_ESTABLISHED)
              {
                ...
                global_data_ptr->mac_mode = TRANSFER_MODE;
                MSG_GERAN_HIGH_1_G("MAC Sent MAC_NT_DL_DTM_DYNAMIC, MAC set MAC_MODE : %d",
                  global_data_ptr->mac_mode);
              ...
              else if(global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind == UPLINK_DOWNLINK_TBF_ESTABLISHED)
              {
                ...
                global_data_ptr->mac_mode = TRANSFER_MODE;
                MSG_GERAN_HIGH_1_G("MAC Sent MAC_NT_UL_DL_DTM_DYNAMIC, MAC set MAC_MODE : %d",
                  global_data_ptr->mac_mode);
              ...
              else
              {
                if(geran_get_nv_recovery_restart_enabled())
                {
                  MSG_GERAN_ERROR_1_G("MAC info Unexpected connection type %d, initiated Recovery Restart",
                    global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                  geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
                }
                else
                {
                  ERR_GERAN_FATAL_1_G("MAC info Unexpected connection type %d ", global_data_ptr->msg_ptr->l1_mac_sig.msg.connect_ind.tbf_type_ind);
                }
              }
            }
            else
          ...
          case MAC_PH_CS_CONNECT_IND:
            ...
            MSG_GERAN_LOW_0_G("MAC Rcvd MAC_PH_CS_CONNECT_IND, no action for DTM estab from NULL");
            break;
#endif /*FEATURE_GSM_EDTM*/
      ...
      else if (task_id == MS_MAC_RLC)
      {
        switch (signal_id)
        {
          ...
          case RM_UL_RELEASE_IND:
            /* Case where GMAC gets GRR_MAC_DTM_CHANNEL_ASSIGN_REQ from GRR and
             * sends MR_UL_ALLOC_CNF to GRLC and MAC_L1_DTM_CHANNEL_ASSIGN_REQ to GL1.
             *
             * However, before we could get MAC_PH_CONNECT_IND if we get 
             * RM_UL_RELEASE_IND then we need to release the channel. */
            MSG_GERAN_MED_1_G("MAC Rcvd RM_UL_RELEASE_IND from GRLC (cause=%d)", global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause);
            if (global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause == REL_UL_ABNORMAL_WITH_ACCESS_RETRY)
            {
              gmac_util_block_rlc(NO_DEL, GMAC_NULL, __LINE__);
              null_data_ptr->gmac_null_tbf_rel_cause = GMAC_NULL_TBF_REL_NORMAL;
              gmac_null_send_release_to_l1(GMAC_NULL_TBF_REL_NORMAL, UL_AND_DL_TBF_ABNORMAL);
              mac_log_ul_tbf_release(MAC_LOG_UL_RELEASE_IND);
            }
            else
            {
              MSG_GERAN_LOW_0_G("MAC Rcvd RM_UL_RELEASE_IND with wrong release cause");
            }
            break;
    ...
    case NULL_DTM_GTTP_UL_CON:
      if(task_id == MS_MAC_RR)
      {
        switch (signal_id)
        {
          ...
          case GRR_MAC_DTM_UL_DCCH_CNF:
            MSG_GERAN_LOW_0_G("MAC rcvd GRR_MAC_DTM_UL_DCCH_CNF");
            gmac_util_send_ul_release_to_rlc(REL_UL_NORMAL);

            if(gmac_util_is_rlc_blocked(GMAC_NULL,__LINE__) == BLOCKED)
            {
          ...
          case GRR_MAC_DTM_CHANNEL_RELEASE_IND:
            MSG_GERAN_LOW_0_G("MAC Rcvd GRR_MAC_DTM_CHANNEL_RELEASE_IND");
                                                  ...
                                                  uint8                access_phase )
{
  ...
  if ( egprs_capable_cell == TRUE )
  {
    ...
    if ( egprs_pkt_chan_req == TRUE )
    {
      if ( data_req_cause == GRR_CAUSE_LAY3_DATA )
      {
        if ( rlc_mode == GRLC_MODE_UNACK )
        {
          ...
          MSG_GERAN_LOW_0_G("GMAC RACH type: EGPRS cell, 11 bit RACH, 2phase");
        }
        else    /* RLC ACK Mode */
        {
          if ( num_of_blocks <= 8 )
          {
              ...
              MSG_GERAN_LOW_0_G("GMAC RACH type: EGPRS cell, 11 bit RACH, 1phase");
          }
          else
          {
            ...
            switch ( access_phase )
            {
              ...
              default:
                MSG_GERAN_LOW_1_G("GMAC UNKNOWN access phase: 0x%x, overriden with EGPRS 11bit 1phase",access_phase);
                access_type = GMAC_EGPRS_ONE_PHASE_ACCESS;
              break;
            }
          }
      ...
      else /*GMM Sig, LLC Sig, Page Response or Cell Update */
      {
...
#endif /* FEATURE_GERAN_IMPLICIT_PS_REJECT */
        {
          ...
          MSG_GERAN_LOW_0_G("GMAC RACH type: EGPRS cell, 11 bit RACH, signalling");
        }
      }
    }
    ...
    {    /* uses two-phase / single block */
      ...
      MSG_GERAN_LOW_0_G("GMAC RACH type: EGPRS cell, 8 bit RACH, 2phase");
           ...
           existing GPRS code */
  {
    ...
    if( rlc_mode == GRLC_MODE_UNACK)
    {
      ...
      MSG_GERAN_LOW_0_G("GMAC RACH type: GPRS cell, 8bit RACH, single block");
    }
    else
    {
  ...
  MSG_GERAN_HIGH_4_G("GMAC RACH type: GPRS %d, 11_bit_RACH %d, rlc_mode:%d, access type: 0x%x",
    egprs_capable_cell, egprs_pkt_chan_req, rlc_mode, access_type);
...
/*===========================================================================
===
===  FUNCTION    gprs_odm_info_is_assigned_tbf_egprs_dyn()
===
===  DESCRIPTION
...
===
===========================================================================*/
boolean gprs_odm_info_is_assigned_tbf_egprs_dyn(void)
{
...
===
===========================================================================*/
gprs_mac_null_substates_t gmac_null_get_null_state(void)
{
  gmac_null_data_t *null_data_ptr = gmac_get_null_data_ptr();
