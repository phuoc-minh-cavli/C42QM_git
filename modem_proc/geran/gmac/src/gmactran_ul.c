...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmactran_ul.c#6 $
***
...
***
*****************************************************************************/
static void gmac_util_send_stop_rlc_tx_to_l1(void);
static void gmac_transfer_ul_tbf_release(boolean dl_tbf_present);
...
===
===========================================================================*/
void gmac_init_tran_ul_data(void)
{
  /*No data to reset*/
}
...
#define BEP_PERIOD2_FIELD_NOT_SPECIFIED 0xFF
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
      ...
      switch (pda_ptr->tlli_gtfi_pkt_ref_valid)
      {
        case tlli_valid:
          ...
          */
          MSG_GERAN_LOW_1_G("PACKET_DOWNLINK_ASS addressed by tlli %x",pda_ptr->tlli);
          /*TLLI in DL CNF to RLC*/
          ...
          if(global_data_ptr->gmac_local_tfi_store.dl_tfi_valid)
          {
            if(pda_ptr->tlli != global_data_ptr->gmac_store_dl_ass_tlli)
            {
              ...
              MSG_GERAN_LOW_3_G("MAC  gmac_store_dl_ass_tlli %x,L_tlli %x o_tlli %x",
            ...
            {
               MSG_GERAN_LOW_1_G("MAC  gmac_store_dl_ass_tlli %x,",global_data_ptr->gmac_store_dl_ass_tlli);
               addr_match = TRUE;
            }
          }
          else if(global_data_ptr->gmac_local_tfi_store.ul_tfi_valid)
          {
            ...
            {
              MSG_GERAN_LOW_1_G("MAC  gmac_store_ul_est_tlli %x,",global_data_ptr->gmac_store_ul_est_tlli);
        ...
        case global_tfi_valid:
          ...
          addr_match = gmac_util_global_tfi(pda_ptr->global_tfi);
  ...
  if (addr_match)
  {
    MSG_GERAN_HIGH_5_G("MAC Rcvd PACKET_DOWNLINK_ASSIGNMENT dl_ass_tlli %x,L_tlli %x o_tlli %x, ts_alloc %d, mode %d",
      global_data_ptr->gmac_store_dl_ass_tlli,
      local_rrps_tlli,
      old_local_rrps_tlli,
    ...
    if (global_data_ptr->csn_dec_hdr_ptr->error_code == MESSAGE_ESCAPE_ERROR)
    {
      ...
      gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_DOWNLINK_ASSIGNMENT);
    ...
    {
      MSG_GERAN_LOW_0_G("GMAC, Freq Params rxd in PDA whilst in DTM, AB NORM REL");
      gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
      mac_log_ul_tbf_release(MAC_LOG_UL_FREQ_ERROR_ACCESS_RETRY);

      if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE)
      {
        mac_log_dl_tbf_release(MAC_LOG_DL_FREQ_ERROR_ACCESS_RETRY);
      }
    ...
    {
      MSG_GERAN_LOW_0_G("GMAC, PDA has a timeslot alloc of 0, ignore msg");
      return (pda_accept_flag);
    }
    ...
    if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
    {
       ...
       {
          MSG_GERAN_LOW_0_G("MAC info:- in EGPRS mode. PDA ignored no rel 99 features");
          return (pda_accept_flag);
       }
       else
       {
         ...
         {
           MSG_GERAN_LOW_0_G("MAC Info:- in EGPRS. Rel99 features contain EGPRS data");

           transfer_data_ptr->gmac_transfer_store_dl_tbf_msg.tbf_mode = TBF_MODE_EGPRS;

           /*Check validity of DL ws*/
           assigned_dl_ws = gmac_egprs_decode_dl_window_size(pda_ptr->egprs_win_sz, pda_ptr->timeslot_alloc);
         ...
         else
         {
           ...
           MSG_GERAN_LOW_0_G("MAC info:- in EGPRS. Rel99 features DON'T contain EGPRS data,ignore PDA");
           return (pda_accept_flag);
         }
       }
    }
    else /*gprs*/
    {
       ...
       {
        MSG_GERAN_LOW_0_G("MAC info:- in GPRS. Rel99 features contain EGPRS data, igonre PDA");
         return (pda_accept_flag);
       }
    }
  ...
  /* The value of MAC_MODE may be changed between DYNAMIC and EXTENDED DYNAMIC
    ...
    if(global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE)
    {
      if(global_data_ptr->gmac_store_dl_rlc_mode != pda_ptr->rlc_mode)
      {
        ...
        if ( (grlc_dl_t3192_status() == TMR_ACTIVE) && (pda_ptr->ctrl_ack == 1) )
        {
        ...
        else
        {
          ...
          MSG_GERAN_MED_2_G("MAC info:- Current rlc mode is %d and Recieved rlc mode %d, REL_DL_TBF Locally",
    ...
    */
...
#endif
    {
      if (pda_ptr->ctrl_ack == 0)
      {
        ...
        MSG_GERAN_LOW_0_G("MAC info PDA Ctrl Ack 0");
        gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_DOWNLINK_ASSIGNMENT);
        return (pda_accept_flag);
      }
    }
    else
    {
      ...
      {  
        ...
        rr_read_public_store( RRPS_MS_ANITE_GCF_FLAG, &anite_gcf_flag);
             ...
             transfer_data_ptr->mac_transfer_substate == TRANSFER_UL_DYNAMIC_DL_ASSIGN) )
        {
          ...
          MSG_GERAN_LOW_0_G("PDA Ctrl ack = 1 and T3192 not running when DL TBF not active, ignore ctrl ack");
        }
        else
        {              
          /* Error in CONTROL_ACK - ignore PDA and inform network */
          MSG_GERAN_LOW_0_G("MAC info PDA Ctrl Ack 1");
          gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_DOWNLINK_ASSIGNMENT);
          return (pda_accept_flag);
        }
      }
         ...
         (current_mac_mode == MAC_L1_MAC_MODE_EXTENDED_DYNAMIC) )
    {
      ...
      if ( gmac_util_multislot_class_check(gmac_ul_ts_alloc,
                                           ...
                                           transfer_data_ptr->t_flags.dtm_active, PSHO_UPDATE_PARAMS_NOT_REQUIRED) == TRUE )
#else
      if ( gmac_util_multislot_class_check(gmac_ul_ts_alloc,
                                           pda_ptr->timeslot_alloc,
                                           current_mac_mode,
    ...
    else
    {
       ...
       {
          /* ignore PDA */
          MSG_GERAN_LOW_0_G("No DL currently active, invalid PDA Ass config");
          return (pda_accept_flag);
       }

    }
/*event logging changes*/
    mac_log_gprs_pda_event( pda_ptr->timeslot_alloc, global_data_ptr->gmac_re_assign_dl_tfi );

    /* If TA params not changed, do not send */
    if (gmac_update_local_ta_params(&(pda_ptr->pkt_timing_adv)))
    {
      (void)gmac_packet_ta_ie(&(pda_ptr->pkt_timing_adv), &(transfer_data_ptr->gmac_transfer_store_dl_tbf_msg.ta_params));
    }
    else
    {
    ...
    {
      if (gmac_util_tbf_starting_ie(&(pda_ptr->tbf_start_frm_num),&(transfer_data_ptr->gmac_transfer_store_dl_tbf_msg.tbf_start_time),TRUE))
      {
    ...
    {
      freq_param_result = gmac_util_frequency_param_ie(TRUE,TRUE);
      ...
      {
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
        mac_log_ul_tbf_release((mac_log_ul_tbf_release_t)freq_param_result);
        if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
           mac_log_dl_tbf_release((mac_log_dl_tbf_release_t)freq_param_result);}
        return (pda_accept_flag);
      }
      else if(freq_param_result == FREQ_ERROR_PARTIAL_SYS_ACQ)
      {
        gmac_abnormal_release_procedure(RR_PART_SYS_INFO_REQ,REL_RLC_DL,NO_DEL,FALSE);
        mac_log_ul_tbf_release((mac_log_ul_tbf_release_t)freq_param_result);
        if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
           mac_log_dl_tbf_release((mac_log_dl_tbf_release_t)freq_param_result);}
        return (pda_accept_flag);
      ...
      else
      {
        /* unknown error type */
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
        mac_log_ul_tbf_release(MAC_LOG_UL_UNKNOWN_ERROR);
        if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
           mac_log_dl_tbf_release(MAC_LOG_DL_UNKNOWN_ERROR);}
        return (pda_accept_flag);
      }
    }
    else
...
===
===========================================================================*/
boolean gmac_tdynamic_handle_ptr(void)
{
  ...
  {
    /* if stored TFI = received TFI then match else ignore message */
    if (gmac_util_global_tfi(ptr_ptr->global_tfi))
    {
      MSG_GERAN_MED_6_G("MAC Rcvd PACKET_TIMESLOT_RECONFIGURE, tfi_dir:%d, ul_tfi %d, dl_tfi %d, dl_ass_tlli %x,L_tlli %x ,o_tlli %x",
        ptr_ptr->global_tfi.direction,
        ptr_ptr->global_tfi.uplink_tfi,
        ptr_ptr->global_tfi.dwnlink_tfi,
        global_data_ptr->gmac_store_dl_ass_tlli,
        local_rrps_tlli,
        old_local_rrps_tlli);
      ...
      if (global_data_ptr->csn_dec_hdr_ptr->error_code == MESSAGE_ESCAPE_ERROR)
      {
        MSG_GERAN_LOW_0_G("MAC info PTR ME");
        gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_TIMESLOT_RECONFIGURE);
      ...
      {
        MSG_GERAN_LOW_0_G("GMAC, Freq Params rxd in PTR whilst in DTM, AB NORM REL");
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
      ...
      MSG_GERAN_MED_5_G("MAC info PTR assigned tbf mode %d, PTR type %d, alloc_flag %d, usf_bit %d, DL_alloc %d",
        global_data_ptr->current_tbf_mode,
        ptr_ptr->egprs_ptr,
        ptr_ptr->alloc_flag,
        ptr_ptr->dynamic_alloc.usf_bitmap,
      ...
      if (ptr_ptr->alloc_flag == dynamic)
      {
        ...
        {
          MSG_GERAN_LOW_2_G("GMAC, PTR has a timeslot alloc of 0, UL TS alloc:0x%x, DL TS alloc:0x%x, ignore msg",
        ...
        if ((gmac_pkt_tmslt_reconfig_dynamic_alloc_ie(&transfer_data_ptr->gmac_transfer_store_ts_reconfig)) == FALSE)
        {
          gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
          if (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid == TRUE){
             mac_log_ul_tbf_release(MAC_LOG_UL_ASSIGNMENT_DATA_ERROR);}
          if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
             mac_log_dl_tbf_release(MAC_LOG_DL_ASSIGNMENT_DATA_ERROR);}
          return (ptr_accept_flag);
        }
      }
      else
      {
        MSG_GERAN_LOW_0_G("MAC error PTR fixed alloc");
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
        if (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid == TRUE){
           mac_log_ul_tbf_release(MAC_LOG_UL_INVALID_MAC_MODE);}
        if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
           mac_log_dl_tbf_release(MAC_LOG_DL_INVALID_MAC_MODE);}
        return (ptr_accept_flag);
      }
      ...
      if(global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE)
      {
        if(global_data_ptr->gmac_store_dl_rlc_mode != ptr_ptr->dwnlk_rlc_mode)
        {
          ...
          /*IF t3192 is active and crtl_ack BIT in pda is set then a change of RLC mode is allowed*/
          if ( (grlc_dl_t3192_status() == TMR_ACTIVE) && (ptr_ptr->control_ack == 1) )
          {
          ...
          else
          {
            MSG_GERAN_MED_2_G("MAC info:- Current rlc mode is %d and Recieved rlc mode is %d ",
                              global_data_ptr->gmac_store_dl_rlc_mode,ptr_ptr->dwnlk_rlc_mode);
            MSG_GERAN_LOW_0_G("MAC info:- GMAC is releasing the downlink and uplink tbf locally because difference of rlc mode in PDA");
      ...
      */
...
#endif
      {
        if (ptr_ptr->control_ack == 0)
        {
          ...
          MSG_GERAN_LOW_0_G("MAC info PTA Ctrl Ack 0");
          gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_TIMESLOT_RECONFIGURE);
          return (ptr_accept_flag);
        }
      }
      else
      {
        if (ptr_ptr->control_ack == 1)/* CR 105837 */
        {
          ...
          {
            global_data_ptr->current_rcvd_dl_ctrl_ack = 0;
            MSG_GERAN_LOW_0_G("PTR Ctrl ack = 1 and T3192 not running when DL TBF not active, ignore ctrl ack");
          }
          else
          {
      ...
      else
      {
              ...
              ( transfer_data_ptr->mac_transfer_substate == TRANSFER_UL_DYNAMIC_DL_ASSIGN ) )
        {
          ...
          if (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid == TRUE){
             mac_log_ul_tbf_release(MAC_LOG_UL_ASSIGNMENT_ADDRESSING_ERROR);}
          if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
             mac_log_dl_tbf_release(MAC_LOG_DL_ASSIGNMENT_ADDRESSING_ERROR);}
          return (ptr_accept_flag);
        }
        else
        {
      ...
      else
      {
        ...
        if (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid == TRUE){
             mac_log_ul_tbf_release(MAC_LOG_UL_MULTISLOT_ERROR);}
        if (global_data_ptr->gmac_local_tfi_store.dl_tfi_valid == TRUE){
             mac_log_dl_tbf_release(MAC_LOG_DL_MULTISLOT_ERROR);}
        return (ptr_accept_flag);
      }

/*event logging changes*/
      mac_log_ptr_event(ts_alloc_bitmap, ptr_ptr->dwnlk_ts_alloc, global_data_ptr->gmac_re_assign_ul_tfi, global_data_ptr->gmac_re_assign_dl_tfi);
      ...
      if (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
      {
        ...
        {
          /*Check validity of DL ws*/
          assigned_dl_ws = gmac_egprs_decode_dl_window_size(ptr_ptr->dl_egprs_window_size, ptr_ptr->dwnlk_ts_alloc);
        ...
        {
          assigned_ul_ws = gmac_egprs_decode_ul_window_size(ptr_ptr->ul_egprs_window_size,ptr_ptr->dynamic_alloc.usf_bitmap);
        ...
        if ( gmac_util_egprs_mcs_valid(ptr_ptr->egprs_chan_coding_cmd) == FALSE )
        {
      ...
      {
        freq_param_result = gmac_util_frequency_param_ie(FALSE,TRUE);
        if (freq_param_result == NO_ERRORS)
        {
...
/*fix for cs change handling*/
         ...
         MSG_GERAN_LOW_3_G("t_cs_reconfig_flg %d ,l_cs_setting %d, ptr_chan_coding_cmd %d",
      ...
      {
          MSG_GERAN_LOW_1_G("Invalid TBF mode = %d",global_data_ptr->current_tbf_mode);
          gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
...
===
===========================================================================*/
boolean gmac_tdynamic_handle_pua(void)
{
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
      ...
      switch (pua_ptr->tlli_gtfi_pkt_ref_valid)
      {
        ...
        case tlli_valid:
          ...
          **       is not valid but at present it is treated as valid.
          */
          addr_match = gmac_match_old_or_new_tlli(pua_ptr->tlli);
          break;

        case global_tfi_valid:
          /* if stored TFI = received TFI then match else ignore message */
          addr_match = gmac_util_global_tfi(pua_ptr->global_tfi);
          break;
    ...
    if (addr_match)
    {
      ...
      if (global_data_ptr->csn_dec_hdr_ptr->error_code == MESSAGE_ESCAPE_ERROR)
      {
        ...
        gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_UPLINK_ASSIGNMENT);
      ...
      {
        MSG_GERAN_LOW_0_G("GMAC, Freq Params rxd in PUA whilst in DTM, AB NORM REL");
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
      ...
      MSG_GERAN_MED_3_G("MAC info PUA assigned tbf mode %d, PUA type %d, usf_bit %d",
        global_data_ptr->current_tbf_mode,
        pua_ptr->egprs_pua,
        pua_ptr->dynamic_alloc.usf_bitmap);
      ...
      if (pua_ptr->allocation_flag == dynamic_alloc_e)
      {
       ...
       /*Check for PUA with no timeslots allocated*/
        ...
        {
          MSG_GERAN_LOW_0_G("GMAC, PUA has a timeslot alloc of 0, ignore msg");
          return (pua_accept_flag);
        }
...
/*event logging changes*/
        mac_log_gprs_pua_event(ts_alloc_bitmap, pua_ptr->chan_coding_cmd );
        ...
        if (gmac_util_multislot_class_check(
              ...
              global_data_ptr->gmac_dtm_cs_ts_alloc,
              transfer_data_ptr->t_flags.dtm_active, PSHO_UPDATE_PARAMS_NOT_REQUIRED) == FALSE)
#else
        if (gmac_util_multislot_class_check(
             ts_alloc_bitmap,
             global_data_ptr->gmac_local_dl_ts_alloc,
       ...
       /* starting time */
        ...
        {
          if (gmac_util_tbf_starting_ie(&(pua_ptr->dynamic_alloc.tbf_start_frm_num), &(ul_tbf_msg_ptr->starting_time),TRUE) == FALSE)
          {
            /* Error with TBF Starting time - abort UL */
            gmac_transfer_ul_tbf_abort_alloc();
            return (pua_accept_flag);
          }
        ...
        gmac_global_dynamic_alloc_ie(ul_tbf_msg_ptr, pua_ptr);

        /* PACKET TA IE */
        /* If TA params not changed, do not send */
        if (gmac_update_local_ta_params(&(pua_ptr->pkt_timing_adv)))
        {
          (void)gmac_packet_ta_ie(&(pua_ptr->pkt_timing_adv), &(ul_tbf_msg_ptr->timing_advance_params));
        }
        else
        {
      ...
      if (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
      {
        ...
        if ( gmac_util_egprs_mcs_valid(pua_ptr->egprs_chan_coding_cmd) == FALSE )
        {
          MSG_GERAN_LOW_0_G("MAC info ERROR PUA contains invalid EGPRS MCS, ab rel ");
          gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
        ...
        MSG_GERAN_MED_1_G("MAC info EGPRS PUA, Acc Tech Req %d", pua_ptr->access_technologies_req_flag);
      ...
      else
      {
        ...
        gprs_l1_mac_acc_func(&mac_l1_sig_uldcf);
      }
      ...
      {
/*fix for cs change handling*/
        MSG_GERAN_LOW_3_G("t_cs_reconfig_flg %d ,l_cs_setting %d, pua_chan_coding_cmd %d",
                          transfer_data_ptr->t_flags.gmac_transfer_cs_reconfig_flag,
      ...
      transfer_data_ptr->gmac_transfer_ul_rlc_info_store.num_timeslots = 
        gmac_utils_calculate_nts(pua_ptr->dynamic_alloc.usf_bitmap);
...
===
===========================================================================*/
void gmac_tdynamic_handle_puan(boolean dl_present)
{
  ...
  if (gmac_util_global_tfi(puan_tfi) == TRUE)
  {
    MSG_GERAN_MED_3_G("MAC info PUAN, UL TFI %d, TBF mode %d PUAN Type %d",
    ...
    fai_set =  gmac_egprs_fai_set(puan_ptr);

#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
    /* Store Frame number of PUAN for later use */
    transfer_data_ptr->gmac_tran_last_puan_fn = global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.frame_no;
#endif /* FEATURE_GERAN_POWER_EFFICIENT_OPERATION */

    MSG_GERAN_LOW_1_G("MAC Rcvd PACKET_UPLINK_ACK_NACK UL TFI %d",global_data_ptr->gmac_local_tfi_store.ul_tfi);

    if (global_data_ptr->csn_dec_hdr_ptr->error_code == MESSAGE_ESCAPE_ERROR)
    {
      MSG_GERAN_LOW_0_G("MAC info PUAN ME");
      gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_UPLINK_ACK_NACK);
      return;
    }

    if (global_data_ptr->csn_dec_hdr_ptr->error_code == NON_DISTRIBUTION_ERROR)
    {
      MSG_GERAN_LOW_0_G("MAC info PUAN NON_DISTRIBUTION_ERROR");
      gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_UPLINK_ACK_NACK);
      return;
    }
          ...
          (!(global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[0] & 0x08))))
      {
        ...
        else
        {
          ...
          if (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
          {
             ...
             /*log puan*/
             mac_log_ul_acknack(global_data_ptr->csn_msg_bit_length, global_data_ptr->dl_msg);
          ...
          {
            gmac_read_acknack_desc_ie(puan_ptr->ack_nack_desc,&(ul_pkt_acknack_msg->acknack_desc));
          ...
          if ((puan_ptr->pkt_timing_adv_flag == valid) &&
              (gmac_util_handle_power_ctrl_TA_IE(puan_ptr->pkt_timing_adv,&(pwr_ctrl_msg->global_packet_ta))))
          {
          ...
          if (puan_ptr->pwr_ctrl_flag == valid)
          {
            ...
            gmac_power_control_params_ie(&(puan_ptr->pwr_ctrl_paramtrs), &(pwr_ctrl_msg->power_control));
          }
          else
          {
...
/*fix for cs change handling*/
          ...
          if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
          {
            MSG_GERAN_LOW_3_G("t_cs_reconfig_flg %d ,l_cs_setting %d, puan_chan_coding_cmd %d",
                              transfer_data_ptr->t_flags.gmac_transfer_cs_reconfig_flag,
            ...
            if (global_data_ptr->gmac_local_cs_setting != puan_ptr->egprs_chan_coding_cmd)
            {
              ...
              gprs_l1_mac_acc_func(&mac_l1_sig_ta);
          ...
          gmac_util_send_stop_rlc_tx_to_l1();

          /* send MR_UL_PKT_ACKNACK_IND message to RLC */
          MSG_GERAN_LOW_1_G("MAC Sent MR_UL_PKT_ACKNACK_IND, FAI: %d", fai_set);
          ...
          if(fai_set)
          {
...

            {
              if (dl_present)
              {
                (void)gmac_normal_ul_tbf_release_proc(global_data_ptr->tbf_est_allowed_flag, dl_present, FALSE);
              }
              else
              {
                (void)gmac_normal_ul_tbf_release_proc(global_data_ptr->tbf_est_allowed_flag, dl_present, global_data_ptr->gmac_transfer_ul_cv_zero_ind);
              }
              mac_log_ul_tbf_release(MAC_LOG_UL_FAI_RELEASE);
            }
            else
            {
              ...
              (pwr_ctrl_msg->global_packet_ta_present))
          {
            ...
            gprs_l1_mac_acc_func(&mac_l1_sig_pcp);
          }
        }
      }
    }
...
===
===========================================================================*/
void gmac_transfer_ul_dynamic_release_clean(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  gmac_stop_t3180();
...
===
===========================================================================*/
void gmac_transfer_ul_tbf_abort_alloc(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  /* 44.060 8.1.2.5.1
...
void gmac_transfer_ul_tbf_abnormal_abort(delete_pdu_cause_t delete_pdu_cause)
{
  ...
  gmac_stop_t3168();
  gmac_util_block_rlc(delete_pdu_cause, GMAC_TRAN_UL, __LINE__);
  transfer_data_ptr->t_flags.gmac_transfer_ul_req = FALSE;
  transfer_data_ptr->t_flags.gmac_local_ul_tbf_est_pending = FALSE;
  transfer_data_ptr->gmac_transfer_ul_request_count = 0;
  global_data_ptr->gmac_local_tfi_store.ul_tfi_valid = FALSE;
...
===
===========================================================================*/
void gmac_transfer_handle_ppcta(boolean ul_present)
{
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    switch (ppcta_data->tlli_gtfi_pkt_ref_valid)
    {
      case global_tfi_valid:
        ...
        if (global_data_ptr->csn_dec_hdr_ptr->error_code == MESSAGE_ESCAPE_ERROR)
        {
          MSG_GERAN_LOW_0_G("MAC info PPCTA ME");
          gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_MESS_ESCAPE_CAUSE, PACKET_PWR_CTRL_TMNG_ADV);
          return;
        }

        /* if stored TFI = received TFI then match else ignore message */
        if (gmac_util_global_tfi(ppcta_data->global_tfi))
        {
          if (ul_present)
          {
            rrbp_control_block_handler_2(PCA_UL_MSG);
...
===
===========================================================================*/
void gmac_transfer_handle_par(void)
{
  ...
  if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
  {
    while ((addr_match == FALSE) && (rej_struct_count > 0))
    {
      ...
      else if (rej_ptr->tlli_gtfi_pkt_ref_valid == global_tfi_valid)
      {
        ...
        addr_match = gmac_util_global_tfi(rej_ptr->global_tfi);
      }
    ...
    if( rex_get_timer(&mac_timers_ptr->timer_t3168) != 0 )
    {
            ...
            (transfer_data_ptr->mac_transfer_substate == TRANSFER_CONCURRENT_DYNAMIC_DL_REASSIGN) )
       {
          /* Note this code was added to fix a problem with TC 42.3.3.3 in
           ...
           {
                gmac_start_t3172(rej_ptr->wait_indication, rej_ptr->wait_indication_size);
           }
...
===
===========================================================================*/
void gmac_transfer_qual_meas_req_to_L1(void)
{
  mac_l1_sig_t          mac_l1_sig;
...
===
===========================================================================*/
void gmac_tdynamic_usf_detected(void)
{
   gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
   gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
   gmac_timers_t* mac_timers_ptr = gmac_get_timers_ptr();
...
===
===========================================================================*/
void gmac_tdynamic_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    case PACKET_UPLINK_ACK_NACK:
      gmac_tdynamic_handle_puan(FALSE);
      break;
    ...
    case PACKET_TBF_RELEASE:
      gmac_transfer_ul_tbf_release(FALSE);
      break;
...
===
===========================================================================*/
void gmac_tdynamic_realloc_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    ...
    case PACKET_UPLINK_ASSIGNMENT:
      ...
      {
        if ( gmac_tdynamic_handle_pua() == TRUE )
        {
          if (((packet_uplink_assignment_t *)global_data_ptr->csn_dec_data_ptr)->freq_param_flag == valid)
          {
            gmac_transfer_simulate_freq_cnf();
...
===
===========================================================================*/
void gmac_tdynamic_dl_assign_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    ...
    case PACKET_TBF_RELEASE:
      gmac_transfer_ul_tbf_release(TRUE);
      break;
...
===
===========================================================================*/
void gmac_tdynamic_realloc_dl_assign_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
...
static void gmac_transfer_ul_tbf_release(boolean dl_tbf_present)
{
  ...
  {
    MSG_GERAN_MED_3_G("MAC Rcvd PACKET_TBF_RELEASE rel cause %d, ul_rel %d, dl_rel %d",
    ...
    else if ( packet_rel_ptr->dwnlink_release )
    {
      if ( dl_tbf_present )
      {
        MSG_GERAN_LOW_0_G("GMAC info PKT TBF rel DL, perform DL AB procedure");
        gmac_transfer_dl_release_clean();
        gmac_tran_send_mac_l1_release(DL_TBF_ABNORMAL);
        mac_log_dl_tbf_release(MAC_LOG_DL_TBF_RELEASE);
      }
...
boolean gmac_tdynamic_handle_egprs_cont_res_pua(packet_uplink_assignment_t *pua_ptr)
{
  ...
  MSG_GERAN_MED_3_G("MAC Rcvd PACKET_UPLINK_ASSIGNMENT UL TFI %d, TBF mode %d, PUA type %d",
    global_data_ptr->gmac_local_tfi_store.ul_tfi,
    global_data_ptr->current_tbf_mode,
    pua_ptr->egprs_pua);
  ...
  ****************************************************************************/
  if (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
  {
    ...
    else
    {
      ...
      {
        MSG_GERAN_LOW_2_G(" MAC info ERROR in ass ul ws < curr ws . curr ws = %d , ass ws = %d",
                          global_data_ptr->current_egprs_ul_ws,assigned_ul_ws);
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,NO_DEL,FALSE);
  ...
  if (pua_ptr->freq_param_flag == valid)
  {
    freq_param_result = gmac_egprs_cont_res_pua_freq_param_ie_util(FALSE,TRUE,&(pua_ptr->freq_params));
