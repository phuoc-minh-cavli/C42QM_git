...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmactran_dl.c#6 $
***
...
***
*****************************************************************************/
gmac_transfer_dl_data_t gmac_transfer_dl_data;
...
static boolean gmac_transfer_dl_handle_pua(void);
...
static void gmac_transfer_dl_store_pdan_info(void);
static void gmac_transfer_dl_qual_meas(void);
...
===
===  DESCRIPTION
===  Returns a pointer to transfer dl data space 
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
gmac_transfer_dl_data_t* gmac_get_transfer_dl_data_ptr(void)
{
  return &gmac_transfer_dl_data;
}
...
===
===========================================================================*/
void gmac_init_tran_dl_data(void)
{
  gmac_transfer_dl_data_t *transfer_dl_data_ptr = gmac_get_transfer_dl_data_ptr();
...
===
===========================================================================*/
void gmac_init_tran_dl_data_non_zero(void)
{
  gmac_transfer_dl_data_t *transfer_dl_data_ptr = gmac_get_transfer_dl_data_ptr();
...
static void gmac_transfer_dl_tbf_release(boolean ul_tbf_ind)
{
  ...
  {
    MSG_GERAN_MED_4_G("MAC Rcvd PACKET_TBF_RELEASE rel cause: %d, dl rel: %d, ul rel: %d, ul_tbf_ind %d",
    ...
    if (ul_tbf_ind)
    {
      ...
      if (packet_rel_ptr->uplink_release)
      {
        ...
        MSG_GERAN_LOW_0_G("GMAC rx Pkt PDCH Rel for UL in DL state, UL not connected yet");
        global_data_ptr->c_flags.tbf_release_pending_flg = TRUE;
        gmac_util_block_rlc(NO_DEL, GMAC_TRAN_DL, __LINE__);
 ...
 #define EGPRS_TBF 1
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    if (addr_match)
    {
      ...
      MSG_GERAN_MED_3_G("MAC info TBF_Mode %d, PUA type %d, USF_bit %d",
        global_data_ptr->current_tbf_mode, pua_ptr->egprs_pua, pua_ptr->dynamic_alloc.usf_bitmap);
      ...
      if (pua_ptr->allocation_flag == dynamic_alloc_e)
      {
        ...
        {
          MSG_GERAN_LOW_0_G("GMAC, PUA allocated 0 timeslots, ignore msg");
          return (pua_accept_flag);
        }
...
/*event logging changes*/
        ...
        {
          if (gmac_util_tbf_starting_ie(&(pua_ptr->dynamic_alloc.tbf_start_frm_num), &(uld_msg_ptr->starting_time),TRUE) == FALSE)
          {
        ...
        gmac_global_dynamic_alloc_ie(uld_msg_ptr, pua_ptr);
        ...
        {
          (void)gmac_packet_ta_ie(&(pua_ptr->pkt_timing_adv), &(uld_msg_ptr->timing_advance_params));
        }
        else
        {
      ...
      if (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
      {
        ...
        MSG_GERAN_MED_1_G("MAC info EGPRS PUA ass, ACC Tech %d", pua_ptr->access_technologies_req_flag);

        /*************  Check if RAC info has been requested ****************/
        if (pua_ptr->access_technologies_req_flag == valid)
        {
           MSG_GERAN_LOW_0_G("MAC info RAC acquisition not applicable to TRANSFER mode, request ignored");
        }
      ...
      else
      {
        ...
        gprs_l1_mac_acc_func(&mac_l1_sig_ul);
...
/*fix for cs change handling*/
          MSG_GERAN_LOW_3_G("cs_reconfig_flg %d ,l_cs_setting %d, pua_chan_coding_cmd %d",
...
static boolean gmac_transfer_handle_pda(uint8 current_mac_mode)
{
      ...
      switch (pda_ptr->tlli_gtfi_pkt_ref_valid)
      {
        case tlli_valid:
          ...
          */
          ...
          MSG_GERAN_LOW_2_G("PACKET_DOWNLINK_ASS addressed by tlli %x, gmac_store_dl_ass_tlli %x",     
  ...
  if (addr_match)
  {
    MSG_GERAN_HIGH_5_G("MAC Rcvd PACKET_DOWNLINK_ASS, dl_ass_tlli %x,L_tlli %x o_tlli %x, ts_alloc %d, mode %d",
      global_data_ptr->gmac_store_dl_ass_tlli,local_rrps_tlli,old_local_rrps_tlli, pda_ptr->timeslot_alloc, global_data_ptr->current_tbf_mode);
    ...
    {
      MSG_GERAN_LOW_0_G("GMAC, PDA timeslot allocation is 0, ignore msg");
      return (pda_accept_flag);
    }
    ...
    */
...
#endif
    {
      if (pda_ptr->ctrl_ack == 0)
      {
        ...
        MSG_GERAN_LOW_0_G("MAC info PDA Ctrl Ack fail 0");
    ...
    else
    {
      ...
      {
        /* Error in CONTROL_ACK - ignore PDA and inform network */
        MSG_GERAN_LOW_0_G("MAC info PDA Ctrl Ack fail 1");
        gmac_transfer_send_tbf_status(MOBILE_TBF_STATUS_NON_DIST_PART_ERR, PACKET_DOWNLINK_ASSIGNMENT);
        return (pda_accept_flag);
      }
    }
...
/*event logging changes*/
    ...
    if (pda_ptr->tbf_start_flag)
    {
      ...
      {
        gmac_abnormal_release_procedure(RR_RETURN_TO_IDLE,REL_RLC_DL,DEL_ONE,FALSE);
        mac_log_dl_tbf_release(MAC_LOG_DL_TBF_STARTING_TIME_ERROR_WITHOUT_RETRY);
        return (pda_accept_flag);
      }
    }
    else
...
===
===========================================================================*/
void gmac_dl_transfer_init(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  gmac_transfer_dl_data_t *transfer_dl_data_ptr = gmac_get_transfer_dl_data_ptr();
...
===
===========================================================================*/
void gmac_transfer_dl_release_clean(void)
{
...
===
===========================================================================*/
void gmac_transfer_dl_normal_release_to_idle(void)
{
  mac_l1_sig_t        mac_l1_sig;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();

#ifdef FEATURE_GSM_DTM
  gmac_tran_rel_state_handle_rel_req( DL_TBF_NORMAL );
#endif /*FEATURE_GSM_DTM*/

  global_data_ptr->c_flags.tbf_release_pending_flg = TRUE;
/*event logging changes*/
  mac_log_packet_tbf_rel_event( DL_TBF_NORMAL );
...
===
===========================================================================*/
void gmac_transfer_dl_normal_release_with_ul(void)
{
  mac_l1_sig_t   mac_l1_sig;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
void gmac_transfer_send_tbf_status(uint8 cause, uint8 mess_type)
{
  ...
  else
  {
    ...
    {
      MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart TBF STATUS");
...
===
===========================================================================*/
void gmac_transfer_send_stored_dl_config_to_l1(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  ...
  gprs_l1_mac_acc_func((mac_l1_sig_t *)(&transfer_data_ptr->gmac_transfer_store_dl_tbf_msg));
}
...
===
===========================================================================*/
void gmac_transfer_send_stored_ts_reconfig_to_l1(void)
{
  gmac_transfer_data_t *transfer_data_ptr = gmac_get_transfer_data_ptr();
  ...
  gprs_l1_mac_acc_func((mac_l1_sig_t *)&transfer_data_ptr->gmac_transfer_store_ts_reconfig);
}
...
===
===========================================================================*/
void gmac_transfer_dl_pkt_acknack(l1_mac_qual_measurements_t *rx_qual_meas_ptr)
{
  mac_l1_sig_t                mac_l1_sig;
  gmac_global_data_t          *global_data_ptr = gmac_get_global_data_ptr();
...
while(scan_pdan_q)
{
  ...
  if (pdan_info_ptr != NULL)
  {
    ...
    ml_pacch_rrbp_sig_ptr->absolute_fn =  gmac_rrbp_absolute_fn(pdan_data_ptr->frame_number, pdan_data_ptr->rrbp);


    if(rx_qual_meas_ptr->rrbp_fn == ml_pacch_rrbp_sig_ptr->absolute_fn)
    {
      rex_timer_cnt_type t3172_status = 0;

      global_data_ptr->csn_encoder_ul_msg.type.pkt_dwnlk_ack_nack.downlink_tfi = pdan_info_ptr->pdan_elem.tfi;

      gmac_compile_acknack_desc_ie(&(global_data_ptr->csn_encoder_ul_msg.type.pkt_dwnlk_ack_nack.ack_nack_desc),pdan_info_ptr->pdan_elem.acknack_desc);

      gmac_transfer_pdan_chan_qual_rep_struct(&(global_data_ptr->csn_encoder_ul_msg.type.pkt_dwnlk_ack_nack),rx_qual_meas_ptr);

      t3172_status = rex_get_timer(&mac_timers_ptr->timer_t3172);
      MSG_GERAN_HIGH_3_G("MAC info DL ACKNAK (DL TFI %d), ul_est_pending %d, T3172 running %d",
        pdan_info_ptr->pdan_elem.tfi,
      ...
      if ((transfer_data_ptr->t_flags.gmac_local_ul_tbf_est_pending) && (t3172_status == 0))
      {
        uint8 t3168_timeout;
        ...
        rr_read_public_store(RRPS_CELL_OPTIONS_T3168,&t3168_timeout);
        gmac_start_t3168(t3168_timeout);

        grlc_ul_restart_await_ass_guard( 30000);

        MSG_GERAN_LOW_1_G("MAC info DL ACKNAK w UL(DL TFI %d)",pdan_info_ptr->pdan_elem.tfi);
      }
      else
      {
        ...
        MSG_GERAN_LOW_1_G("MAC info DL ACKNAK(DL TFI %d)",pdan_info_ptr->pdan_elem.tfi);
      }
      ...
      else
      {
        ...
        if(geran_get_nv_recovery_restart_enabled())
        {
          MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart encode PDAN");
          geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
        }
        else
        {
...
void gmac_transfer_dl_pdan_handler(rm_dl_pkt_acknack_req_t *acknack_msg_ptr, boolean ul_present)
{
  ...
  if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
  {
      gmac_tranfer_dl_egprs_pdan_handler(acknack_msg_ptr);
  }
  else
  {
      ...
      (transfer_data_ptr->t_flags.gmac_local_ul_tbf_est_pending)  )
  {
    ...
    if(transfer_data_ptr->t_flags.gmac_transfer_dl_fai_set)
    {
      ...
      gmac_transfer_dl_store_pdan_info();
      gmac_transfer_dl_qual_meas();
    ...
    else if (global_data_ptr->gmac_local_pccn_params.pccn_pending == PCCN_PENDING_DL)
      {
       ...
       MSG_GERAN_LOW_0_G("MAC info Send PCCN not PDAN");
  ...
  else
  {
    if(transfer_dl_data_ptr->gprs_can_send_pmr_pccf == TRUE)
    {
      ...
      {
        /* Send EDTM Packet CS Request to network */
        MSG_GERAN_LOW_0_G("MAC info Send EDTM PKT CS REQ not PDAN");
      ...
      {
        MSG_GERAN_LOW_0_G("MAC info Send PCCF not PDAN");
      ...
      else if (global_data_ptr->gmac_local_pss_params.pss_pending == TRUE)
      {
        ...
        MSG_GERAN_LOW_0_G("MAC info Send PKT SI STATUS not PDAN");
      ...
      else if (global_data_ptr->gmac_local_pmr_params.pmr_pending)
      {
        if(transfer_dl_data_ptr->gmac_transfer_dl_ack_nack_count_max_2 == PDAN_COUNT_FOR_PMR_MAX)
        {
          MSG_GERAN_LOW_0_G("MAC info Send PMR not PDAN");
          /* Send PMR to network */
      ...
      else if (transfer_data_ptr->gmac_local_pts_params.pts_pending == TRUE)
      {
        ...
        MSG_GERAN_LOW_0_G("MAC info Send PTS not PDAN");
...
===
===========================================================================*/
void gmac_transfer_dl_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    ...
    case PACKET_UPLINK_ASSIGNMENT:
      ...
      if( rex_get_timer(&mac_timers_ptr->timer_t3168) != 0 )
      {
        ...
        if (gmac_transfer_dl_handle_pua())
        {
          gmac_stop_t3168();

          gmac_tran_log_state_change(TRANSFER_DL_WITH_DYNAMIC_ALLOC, __LINE__);
    ...
    case PACKET_DOWNLINK_ASSIGNMENT:
      ...
      if (gmac_transfer_handle_pda(GMAC_MAC_MODE_DYNAMIC_ALLOC))
      {
    ...
    case PACKET_PWR_CTRL_TMNG_ADV:
      gmac_transfer_handle_ppcta(FALSE);
      break;
    ...
    case PACKET_TBF_RELEASE:
      gmac_transfer_dl_tbf_release(FALSE);
      break;

    case PACKET_PDCH_RELEASE:
      gmac_tran_handle_pdch_release(((packet_pdch_release_t *)global_data_ptr->csn_dec_data_ptr)->timeslots_available,DL_ONLY);
      break;
...
===
===========================================================================*/
void gmac_transfer_dl_reassign_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
...
===
===========================================================================*/
void gmac_transfer_dl_dynamic_alloc_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
  ...
  switch (global_data_ptr->csn_dec_hdr_ptr->message_type)
  {
    ...
    case PACKET_TBF_RELEASE:
      gmac_transfer_dl_tbf_release(TRUE);
      break;
...
===
===========================================================================*/
void gmac_transfer_dl_reassign_dynamic_alloc_data_ind(void)
{
#ifdef FEATURE_GSM_EDTM
  gprs_mac_transfer_substates_t tran_state_change = TRANSFER_INACTIVE;
  char                         *new_state_str           = NULL;
...
static void gmac_tranfer_dl_egprs_pdan_handler(rm_dl_pkt_acknack_req_t *egprs_acknack_msg_ptr)
{
      ...
      (transfer_data_ptr->t_flags.gmac_local_ul_tbf_est_pending)  )
  {
    ...
    /***************** CSN encode and send PDAN  **********************/
        gmac_transfer_build_egprs_pdan(egprs_acknack_msg_ptr);
      /*when there is no UL tbf and the DL is being released (ie PDAN with FAI set)*/
      /*only PDAN can be sent on the rrbp so any pending PMR will not be sent. need*/
      /*to inform GRR that sending of PMR failed */
    ...
    else if (global_data_ptr->gmac_local_edtm_pcr_params.edtm_pcr_pending == TRUE)
    {
        ...
        MSG_GERAN_LOW_0_G("MAC info Send EDTM PACKET CS REQUEST not PDAN");
...
static void gmac_transfer_build_egprs_pdan(rm_dl_pkt_acknack_req_t *egprs_acknack_msg_ptr)
{
  ...
  MSG_GERAN_MED_5_G("MAC info frame number = 0X%x, TS =%d, rrbp =%d, len present =%d, acknack size =%d",
    egprs_acknack_msg_ptr->frame_number,
    egprs_acknack_msg_ptr->time_slot,
    egprs_acknack_msg_ptr->rrbp,
  ...
  if(egprs_acknack_msg_ptr->esp == 3)
  {
     ...
     {
       MSG_GERAN_LOW_0_G("MAC info E-PDAN to included qual rpt but no rpt rcvd");
     }
  }
  else
  {
  ...
  {
     MSG_GERAN_LOW_0_G("MAC info EGPRS ACK/NACK does NOT fill msg");
  }
  else
  {
     MSG_GERAN_LOW_0_G("MAC info EGPRS ACK/NACK does fill msg ");
  }
  ...
  if (gcsn1_encoder(EGPRS_PACKET_DOWNLINK_ACK_NACK, &(ml_pacch_rrbp_sig_ptr->pacch_ab_data[1]), &global_data_ptr->csn_encoder_ul_msg) == PASS)
  {
    ...
    ml_pacch_rrbp_sig_ptr->absolute_fn =  gmac_rrbp_absolute_fn(egprs_acknack_msg_ptr->frame_number, egprs_acknack_msg_ptr->rrbp);
...
/*log PDAN*/  /*use   &(ml_pacch_rrbp_sig_ptr->pacch_ab_data[1]) . Need to revisit it with length and type not hardcoded*/
    mac_log_dl_acknack(1 /* EGPRS type*/,  22,  &(ml_pacch_rrbp_sig_ptr->pacch_ab_data[1])); /*PDAN always has 22 bytes.*/
  ...
  {
    MSG_GERAN_LOW_0_G("MAC info CSN.1 error in encoding E-PDAN");
  }
...
/*===========================================================================
===
===  FUNCTION      new_gmac_trans_calc_e_pdan_size_dyn
===
===  DESCRIPTION
===
...
void new_gmac_trans_calc_e_pdan_size_dyn
(
...
void gmac_tran_check_dl_tbf_status(void)
{
     ...
     && global_data_ptr->current_tbf_mode == TBF_MODE_GPRS )
  {
    ...
    gmac_transfer_dl_pkt_acknack(&(transfer_data_ptr->gmac_tran_saved_qual_meas));
