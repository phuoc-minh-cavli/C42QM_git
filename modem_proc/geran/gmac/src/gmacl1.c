...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacl1.c#3 $
***
...
void mac_put_cmd_l1_mac(l1_mac_sig_t *sig_ptr);
...
void gprs_mac_l1_acc_func (l1_mac_sig_t *common_msg_ptr, uint8 *pacch_data)
{
  ...
  /* If dynamic_memory_type == TRUE then allocate dynamic memory */
  {
    ...
    if (l1_mac_sig_ptr != NULL)
    {
      ...
      {
        mac_put_cmd_l1_mac(l1_mac_sig_ptr);
      }
    }
  }
}
...
void mac_put_cmd_l1_mac(l1_mac_sig_t *sig_ptr)
{
  ...
  switch (sig_ptr->sig_id)
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("MAC error CFA LOG unknown message :0x%x",sig_ptr->sig_id);
      break;
  }
