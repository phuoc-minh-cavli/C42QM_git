...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacrb.c#5 $
***
...
static ctrl_ack_value_t gmac_check_for_rrbp_clash(uint32 absolute_rrbp_fn, uint8 ts);
static void xlate_mac_hdr_1st_optional_octet(uint8 *src, mac_hdr_1st_opt_t *mac_1st_opt_oct_ptr);
static uint8 retrieve_dl_logical_channel_msg(l1_mac_sig_t *msg_ptr);
static uint8 set_t3200(void);
static void clear_t3200(uint8 idle_timer_id);

#ifdef FEATURE_GSM_EDTM
static void xlate_mac_hdr_ext_seg_optional_octet(uint8 *src, mac_hdr_ext_seg_opt_t *mac_ext_seg_opt_oct_ptr);
static boolean gmac_segmented_msg_complete(uint16 current_rcvd_seg_bit_map, uint8  rbsne_of_last_seg);
static uint8 gmac_retrieve_completed_segmented_msg(uint8 *ptr_completed_dl_msg,  dl_msg_buffer_t *ptr_current_seg_msg);
static void gmac_update_segmented_vars(mac_ph_data_ind_t *ptr_mac_ph_data, dl_msg_buffer_t *ptr_current_seg_msg);
static void gmac_initialise_segmented_vars(dl_msg_buffer_t *ptr_current_seg_msg,uint8 ts,uint8 rti);
#else
static void xlate_mac_hdr_2nd_optional_octet(uint8 *src, mac_hdr_2nd_opt_t *mac_2nd_opt_oct_ptr);
#endif /*FEATURE_GSM_EDTM*/
static gmac_rb_data_t* gmac_get_rb_data_ptr(void);
...
uint8 dl_channel_msg_to_channel_type(void);
void gmac_log_dl_channel_msg(uint8 msg_type, uint16 msg_length);
...
===
===========================================================================*/
void gmac_init_rb_data(void)
{
  gmac_rb_data_t *rb_data_ptr = gmac_get_rb_data_ptr();
...
===
===========================================================================*/
void gmac_init_rb_data_non_zero(void)
{
  gmac_rb_data_t *rb_data_ptr = gmac_get_rb_data_ptr();
...
===
===========================================================================*/
static uint8 retrieve_dl_logical_channel_msg(l1_mac_sig_t *lm_sig_ptr)
{
  ...
  xlate_mac_hdr_1st_optional_octet(ptr_opt_oct_1,  ptr_mac_1st_opt_oct);
  ...
  if(rb_data_ptr->mac_1st_opt_oct.fs == 0 && rb_data_ptr->mac_1st_opt_oct.rbsn == 1)
  {
    ...
    MSG_GERAN_HIGH_2_G("MAC info ac bit = %d, ext seg octet = x%x",
      rb_data_ptr->mac_1st_opt_oct.ac, *ptr_opt_oct_ext_seg);

    ptr_mac_ext_seg_opt_oct= &mac_ext_seg_opt_oct;    
    xlate_mac_hdr_ext_seg_optional_octet(ptr_opt_oct_ext_seg,  ptr_mac_ext_seg_opt_oct);

    MSG_GERAN_HIGH_2_G("GMAC info fse =%d, rbsne =%d", 
    ...
    for(i=1;i<8;i++)
    {
      if(rb_data_ptr->first_in_list.ptr_to_next_buff != NULL)
      {
        ...
        ************************************************************************************/
        ...
        {
          /* rti found in list*/
          MSG_GERAN_LOW_0_G("GMAC info list NOT full, segment belongs to segmented msg in list,add this segment to it");
          ...
          if(msg_complete == TRUE)
          {
            ...
            clear_t3200(rb_data_ptr->ptr_to_current_in_list->timer_id);
    
            /*retrive completed segment msg and decode and decoded it */   
            msg = gmac_retrieve_completed_segmented_msg(global_data_ptr->dl_msg,rb_data_ptr->ptr_to_current_in_list);
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              MSG_GERAN_LOW_0_G("GMAC info list NOT full, segment does NOT belongs to segmented msg in list, New segmented msg to add to list");
      ...
      else
      {
        ...
        ******************************************************/
        ...
        else
        {
          ...
          /*initialsize*/
          MSG_GERAN_LOW_0_G("GMAC info list EMPTY, New segmented msg added to list");
    ...
    for(i=1;i<9;i++)
    {
      ...
      *  rti found in list,get other segment and combine all the segments in dl_msg
        ******************************************************************************/
        ...
        MSG_GERAN_LOW_0_G("GMAC info list FULL, segment belongs to segmented msg in list, add this segment to it");
        ...
        {
          MSG_GERAN_LOW_3_G( "GMAC info incomplete DL msg RTI=%d TS=%d list pos =%d", 
                              msg_buffer_wanted, lm_sig_ptr->msg.data_ind.timeslot, i );
        }
        ...
        if(i > 7)
        {
          ...
          *********************************************/
          ...
          else
          {
            ...
            /*initialsize*/
            MSG_GERAN_LOW_0_G("GMAC info list FULL, no match found,remove oldest from list and add new segmented msg to list");
...
#else /*FEATURE_GSM_EDTM*/
uint8 retrieve_dl_logical_channel_msg(l1_mac_sig_t *lm_sig_ptr)
{
  uint8   i;
  uint8   msg_buffer_wanted;
  ...
  else
  {
    ...
    xlate_mac_hdr_2nd_optional_octet(ptr_opt_oct_2,  ptr_mac_2nd_opt_oct);
  }
  msg_buffer_wanted = rb_data_ptr->mac_1st_opt_oct.rti;
    ...
    for(i=1;i<8;i++)
    {
      if(rb_data_ptr->first_in_list.ptr_to_next_buff != NULL)
      {
          ...
          && (rb_data_ptr->ptr_to_current_in_list->ts_received_on == lm_sig_ptr->msg.data_ind.timeslot))
        {
          ...
          {
            MSG_GERAN_LOW_1_G("Bad no_header_octets:%d, reject DL message",rb_data_ptr->ptr_to_current_in_list->no_header_octets);
          ...
          gmac_log_dl_channel_msg(msg, global_data_ptr->csn_msg_bit_length);
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              rb_data_ptr->ptr_to_current_in_list->timer_id = set_t3200();
...
static uint8 set_t3200(void)
{
  ...
  rr_read_public_store(RRPS_CELL_OPTIONS_BS_CV_MAX,&bs_cv_max);
...
boolean rb_block_handler(void)
{
  ...
  if(rb_data_ptr->mac_header.payload == 2)
  {
    ...
    if((rbsn_bit == 0)  && (fs_bit == 1))
    {
      ...
      gmac_log_dl_channel_msg(global_data_ptr->channel_msg, global_data_ptr->csn_msg_bit_length);
    }
    else
    {
      global_data_ptr->channel_msg = retrieve_dl_logical_channel_msg(&(global_data_ptr->msg_ptr->l1_mac_sig));
...
void t3200_expiry_handler(mac_mac_sig_id_t expired_timer)
{
  uint8 i;
  gmac_rb_data_t *rb_data_ptr = gmac_get_rb_data_ptr();
  ...
  {
    MSG_GERAN_LOW_1_G("GMAC expired timer out of range: 0x%x", expired_timer);
    return;
  }
  if(rb_data_ptr->first_in_list.ptr_to_next_buff != NULL)
  {
...
uint32 gmac_rrbp_absolute_fn(uint32 dl_frame_number, uint8 rrbp_value)
{
...
void rrbp_control_block_handler_2(rrbp_msg_t pca_direction)
{
  ...
  {
    rr_read_public_store(RRPS_CELL_OPTIONS_ACCESS_BURST_TYPE,&access_burst_type);
    rr_read_public_store(RRPS_CELL_OPTIONS_CONTROL_ACK_TYPE,&ctrl_ack_type);
    ...
    MSG_GERAN_MED_5_G("payload %d channel_msg %d ctrl_ack_value %d access_burst_type %d ctrl_ack_type %d",
      rb_data_ptr->mac_header.payload,
    ...
    {
      MSG_GERAN_MED_5_G("MAC info optional ctrl headers rbsn_bit %d fs_bit %d, [0x%x, 0x%x, 0x%x]",
        rbsn_bit,
        fs_bit,      
      ...
      {
        MSG_GERAN_LOW_0_G( "ERROR: MAC control header 2nd optional octet not present");
      ...
      if((global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[1] & 0x01) == 1)
      {
        ...
        if((global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[2] & 0x01) == 0)
        {
          if (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid == TRUE)
          {
            ...
            {
              MSG_GERAN_LOW_2_G("ERROR: MAC ctrl header ul tfi missmatch. ctrl header tfi =%d, current ul tfi =%d",
          ...
          {
            MSG_GERAN_LOW_0_G("MAC info ERROR SEG msg using ul tfi when there is no valid UL TFI");
            error_in_sending_pca = TRUE;
          }
        }
        else
        {        
          ...
          {            
            ...
            {
              MSG_GERAN_LOW_2_G( "ERROR: MAC ctrl header dl tfi missmatch. ctrl header tfi =%d, current dl tfi =%d",
                                 ((global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.dl_ctrl_block[2] & 0x3e) >> 1),
                                 global_data_ptr->gmac_local_tfi_store.dl_tfi);
              error_in_sending_pca = TRUE;
            }
          ...
          {
            MSG_GERAN_LOW_0_G("MAC info ERROR SEG msg using dl tfi when there is no valid DL TFI");
            error_in_sending_pca = TRUE;
          }          
        }
      }
    ...
    else if(global_data_ptr->channel_msg == PACKET_UPLINK_ACK_NACK)
    {
      ...
      {
        ctrl_ack_value = gmac_check_for_rrbp_clash(rrbp_value,global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.timeslot);
    ...
    if(ctrl_ack_type == ACCESS_BURST)
    {
        ...
        {
           MSG_GERAN_LOW_0_G("MAC info burst type is AB, invalid for polled ext segment msg");
           error_in_sending_pca = TRUE;
        }
        else
#endif /*FEATURE_GSM_EDTM*/
...
void gmac_rrbp_send_pca(uint32 fn,uint8 ts,boolean rrbp_present,uint8 rrbp_value,rrbp_msg_t direction)
{
  ...
  if (rrbp_present)
  {
    ml_pacch_rrbp_sig_ptr->absolute_fn =  gmac_rrbp_absolute_fn(fn,rrbp_value);
  }
  else
  {
  ...
  gprs_l1_mac_acc_func(&local_mac_l1_sig);
}
#ifdef FEATURE_GSM_EDTM
...
)
{
  ...
  for(i=0;i<rb_data_ptr->ptr_to_current_in_list->final_segment_rbsn;i++)
  {
    ...
    { 
      MSG_GERAN_LOW_1_G("Bad no_header_octets:%d, reject DL message",ptr_current_seg_msg->num_header_octs[i]);
      memset(global_data_ptr->dl_msg,0,SIZE_OF_CONTROL_RADIO_BLOCK);
      global_data_ptr->dl_msg[0] = PACKET_UNUSED_ID << 2;
      break;
    }
  ...
  gmac_log_dl_channel_msg(dl_ctrl_msg_type, csn_msg_length_in_bits);
...
{  
  ...
  MSG_GERAN_HIGH_3_G("GMAC info ext_seg_rcvd_bit_map = 0x%x sp_bit = %d rcvd segments bit map = 0x%x", 
    ptr_current_seg_msg->ext_seg_rcvd_bit_map, sp_bit,rb_data_ptr->gmac_pca_for_ext_seg_msg);
}
...
uint8  dl_channel_msg_to_channel_type(void)
{
  ...
  switch(global_data_ptr->channel_msg)
  {
    ...
    case PACKET_PSI13:
      ...
      break;
    default:
      MSG_GERAN_LOW_1_G("MAC INfo LOG ON DEMAND: OTA not part of logged msg_type %d",global_data_ptr->channel_msg);
...
void gmac_log_dl_channel_msg(uint8 msg_type, uint16 msg_length)
{
  ...
  switch(global_data_ptr->channel_msg)
  {
    case PACKET_DOWNLINK_DUMMY_CNTRL_BLK:
      break;
    default:
      (void)mac_log_signalling_message(dl_channel_msg_to_channel_type(),
                                       msg_type,
