...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacidle.c#4 $
***
...
***
*****************************************************************************/
gmac_idle_data_t gmac_idle_data;
...
*****************************************************************************/
   ...
   static void gmac_idle_ul_release_confirm_handler(void);

   static void gmac_idle_handle_pua_error_result(void);

   static void gmac_idle_simulate_freq_cnf_for_ul_states(void);

   static void gmac_idle_handle_no_freq_condition_in_pua(void);

   static void gmac_idle_dl_release_confirm_handler(void);
...
static void gmac_idle_log_state_change(gprs_mac_idle_substates_t new_state, uint32 line)
{
  gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();

  MSG_GERAN_MED_3_G("GMAC State Idle %d -> %d, line %d", idle_data_ptr->mac_idle_substate, new_state, line);
}
...
===
===  DESCRIPTION
===  Returns a pointer to idle data space
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
gmac_idle_data_t* gmac_get_idle_data_ptr(void)
{
  return &gmac_idle_data;
}

void idle_mode_handler(uint8 task_id, uint8 signal_id)
{
...
/*CR67833*/
     ...
     )
  {
    ...
    MSG_GERAN_MED_0_G( "DL msgs are ignored while performing GRR_INITIATE_NO_ACCESS release procedure");
    return;
  }
  ...
  if (task_id == MS_MAC_L1)
  {
    ...
    {
      MSG_GERAN_MED_3_G("MAC Rcvd IDLE mode sig %d, task %d, substate %d", signal_id, task_id, idle_data_ptr->mac_idle_substate);
    }
  }
  else
  {
  ...
  ******************************************************************/
         ...
         if(task_id == MS_MAC_L1)
         {
           ...
           switch (signal_id)
           {
             ...
             case MAC_PH_DATA_IND :
                switch (global_data_ptr->channel_msg)
                {
                    ...
                    case PACKET_DOWNLINK_ASSIGNMENT:
                         ...
                         idle_data_ptr->pda_data_result = gmac_idle_pda_handler(&idle_data_ptr->dl_ass_ind_sig,FALSE,PDA_CHECK_TLLI_ONLY);
                         ...
                         if( idle_data_ptr->pda_data_result == NO_ERRORS)
                         {
                           ...
                           MSG_GERAN_LOW_0_G( "MAC sent MAC_GRR_MAC_STATUS");
                           gmac_util_send_mac_grr_mac_status(&idle_data_ptr->mac_grr_sig,MAC_IDLE_BLOCKED);
                         }
                         else
                         {
                           ...
                           if(gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__)== UNBLOCKED)
                           {
                           gmac_util_block_rlc(NO_DEL,GMAC_IDLE,__LINE__);
                           MSG_GERAN_LOW_1_G( "GMAC should have already blocked rlc at this point mac action is %d",
                                                global_data_ptr->mac_action);
                           }
                    ...
                    case PACKET_CELL_CHANGE_ORDER :
                         ...
                         if(gmac_idle_check_pcco_addressing())
                         {
                           rrbp_control_block_handler_2(PCA_UL_MSG);

                           MSG_GERAN_LOW_0_G("MAC forwards PCCO to GRR");
                    ...
                    case PACKET_MEASUREMENT_ORDER:
                         ...
                         if ( gmac_util_check_pcco_pmo_addressing() )
                         {
                           rrbp_control_block_handler_2(PCA_UL_MSG);

                           MSG_GERAN_LOW_0_G("MAC forwards PMO to GRR");
                           gmac_send_data_ind_to_grr();
             ...
             case L1_MAC_51_SB_FAILURE:
                    ...
                    MSG_GERAN_LOW_0_G("MAC Rcvd L1_MAC_51_SB_FAILURE");

                    gmac_util_send_grr_mac_status(MAC_IDLE);

                    gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF,MAC_INITIATED,GMAC_IDLE,__LINE__);

                    /*In donwlink single block case, GMAC already blocked RLC so unblocking*/
                    gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_IDLE,__LINE__);
             ...
             case L1_MAC_TBF_REL_CONFIRM :
                  ...
                  gmac_idle_ul_release_confirm_handler();
         ...
         else if(task_id == MS_MAC_RR)
         {
           ...
           switch (signal_id)
           {
               case GRR_MAC_NO_PS_ACCESS_IND:
                   ...
                   MSG_GERAN_LOW_3_G("GMAC tbf_release_pending_flg %d,release_cause %d, waiting_for_dl_sb_control_msg %d",
                       ...
                       || (global_data_ptr->c_flags.waiting_for_dl_sb_control_msg == TRUE) )
                   {
                     ...
                     gmac_util_block_rlc(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_no_ps_access_ind.delete_pdu,GMAC_IDLE,__LINE__);
                     gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, DL_TBF_ABNORMAL,MAC_LOG_UL_UNKNOWN_ERROR,MAC_LOG_DL_NORMAL_RELEASE);
                   ...
                   else
                   {
                     ...
                     ***********************************************************************/
                     ...
                     gmac_util_complete_pcco_process(PCCO_FAILURE_NO_RESPONSE,GRR_INITIATED,GMAC_IDLE,__LINE__);
                     gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF,GRR_INITIATED,GMAC_IDLE,__LINE__);
                     ...
                     gmac_idle_send_mac_internal_sig(&idle_data_ptr->mac_int_sig,MAC_IN_NO_SERVICE);

                     display_mac_action(global_data_ptr->mac_action);
               ...
               case GRR_MAC_SEND_PCCF:
                    MSG_GERAN_LOW_0_G("GMAC Rxd Send PCCF when waiting for DL SB (51MF)");
                    gmac_util_start_pccf_process(SENDING_VIA_SB,GMAC_IDLE,__LINE__,(uint8)idle_data_ptr->mac_idle_substate);
               break;
               ...
               case GRR_MAC_PSHO_REQ:
                 MSG_GERAN_LOW_0_G("GMAC Rejected GRR_MAC_PSHO_REQ in idle mode");
         ...
         else if(task_id == MS_MAC_MAC)
         {
           ...
           switch (signal_id)
           {
             case MAC_NI_UL_1P_ASS:
               ...
               gmac_idle_log_state_change(UL_1P_CONTENTION_RES, __LINE__);
             ...
             case MAC_NI_UL_2P_ASS:
               ...
               rr_read_public_store(RRPS_CELL_OPTIONS_T3168,&idle_data_ptr->t3168_timeout);
               gmac_start_t3168(idle_data_ptr->t3168_timeout);

               gmac_idle_log_state_change(UL_2P_ASS, __LINE__);
             ...
             case MAC_NI_DL_ASS:
               ...
               gmac_idle_log_state_change(DL_ASS, __LINE__);
               ...
               case MAC_TI_RE_ESTABLISH:
                    ...
                    ***************************************************************/
                    ...
                    gmac_idle_log_state_change(UL_1P_ASS, __LINE__);
         ...
         if(task_id == MS_MAC_L1)
         {
           ...
           switch (signal_id)
           {
                ...
                switch (global_data_ptr->channel_msg)
                {
                    ...
                    case PACKET_UPLINK_ASSIGNMENT:
                        ...
                        **************************************************************/
                        ...
                        if(gmac_idle_check_pua_addressing(global_data_ptr->idle_mode_ul_tbf_release_action,idle_data_ptr->mac_idle_substate) == TRUE)
                        {
                          /*************************************************************
                          * DL msg address MS correctly, check if pca should be sent   *
                          **************************************************************/
                          MSG_GERAN_MED_3_G("MAC Rcvd PUA substate %d, action %d, rel_pend %d",
                            idle_data_ptr->mac_idle_substate,
                            global_data_ptr->mac_action,
                            global_data_ptr->c_flags.tbf_release_pending_flg);
                          ...
                          if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                          {
                            ...
                            if (gmac_dl_msg_data_valid() == FALSE)
                            {
                               ...
                               MSG_GERAN_LOW_2_G("GMAC DEBUG 1P_ASS access_requested:%d, access_granted:%d",global_data_ptr->access_requested,
                              ...
                              */
                               ...
                               if(idle_data_ptr->pua_data_result == NO_ERRORS)
                               {
                                  ...
                                  *******************************************************************/
                                  ...
                                  {
                                     idle_data_ptr->pua_data_result = gmac_util_frequency_param_ie(FALSE,TRUE);

                                     if ( idle_data_ptr->pua_data_result == NO_ERRORS )
                                     {
                                       gmac_idle_simulate_freq_cnf_for_ul_states();
                                     }
                                  }
                                  else
                                  {
                                    gmac_idle_handle_no_freq_condition_in_pua();
                                  }
                                  ...
                                  ***********************************************/
                                  if ( idle_data_ptr->pua_data_result != NO_ERRORS )
                                  {
                                    gmac_idle_handle_pua_error_result();
                                  }
                               }
                               else
                               {
                            ...
                            else
                            {
                              ...
                              MSG_GERAN_LOW_0_G("MAC CSN.1 decode error in PUA ");
                            }
                          }
                        }
                        else
...

                      {
                         ...
                         *********************************************************************/
                         ...
                         if(idle_data_ptr->pda_data_result == NO_ERRORS)
                         {
                           ...
                           ************************************************************/
                           if(gmac_idle_ignore_pda(global_data_ptr->mac_action,&idle_data_ptr->mac_l1_sig) == FALSE)
                           {
                           ...
                           else
                           {
                             ...
                             MSG_GERAN_LOW_1_G("MAC info PDA ignored reselection %d",global_data_ptr->mac_action);
                           }
                         }
                         else
                         {
             ...
             case L1_MAC_FREQ_ASS_CNF:
                  MSG_GERAN_LOW_0_G("GMAC Info L1_MAC_FREQ_ASS_CNF rxd, Ignored for freq cnf tidy up");
             ...
             case L1_MAC_QUAL_MEASURMENTS:
                  ...
                  {
                    MSG_GERAN_LOW_0_G("GMAC releasing L1, so ignoring L1_MAC_QUAL_MEASURMENTS signal");
                  }
             break;
                    ...
                    switch(idle_data_ptr->access_granted)
                    {
                      ...
                      default:
                           ...
                           if(global_data_ptr->mac_action == UL_DATA_EST)
                           {
                             ...
                             gmac_idle_send_no_access_to_rlc();
                           }

                           MSG_GERAN_ERROR_3_G( "Unknown access granted in state %d task id %d access granted %d. release_cause : ACCESS_RETRY",
                             idle_data_ptr->mac_idle_substate,
                             task_id,
                             idle_data_ptr->access_granted );
             ...
             case L1_MAC_SINGLE_BLOCK_SENT:
                  MSG_GERAN_MED_2_G("MAC Rcvd L1_MAC_SINGLE_BLOCK_SENT, rel_pend: %d, action:%d",
                    global_data_ptr->c_flags.tbf_release_pending_flg,
                    global_data_ptr->mac_action);
                  ...
                  if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                  {
                    if((global_data_ptr->mac_action == PMR_EST) || (global_data_ptr->mac_action == PCCF_EST))
                    {
                      ...
                      {
                        MSG_GERAN_LOW_0_G( "MAC sent MAC_GRR_MEAS_RPT_CNF");
                       ...
                       gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_IDLE,__LINE__);
                        ...
                        gmac_idle_reset_parameters(idle_data_ptr->mac_idle_substate);
                      }
                      else if(global_data_ptr->mac_action == PCCF_EST)
                      {
                        MSG_GERAN_LOW_0_G( "MAC sent MAC_GRR_PCCF_SENT");
                        /*sending PCCF_CNF with successful signal to GRR*/
                        gmac_util_complete_pccf_process(SUCCESSFULLY_SENT_PCCF,MAC_INITIATED,GMAC_IDLE,__LINE__);
                      ...
                      MSG_GERAN_LOW_0_G( "MAC sent MAC_GRR_MAC_STATUS");
                      gmac_util_send_mac_grr_mac_status(&idle_data_ptr->mac_grr_sig,MAC_IDLE);

                      global_data_ptr->mac_action = NONE;
                      display_mac_action(global_data_ptr->mac_action);

                      gmac_idle_log_state_change(PASSIVE, __LINE__);
             ...
             case L1_MAC_51_SB_FAILURE:
                  MSG_GERAN_MED_1_G("MAC Rcvd L1_MAC_51_SB_FAILURE, rel_pend:%d",
                    global_data_ptr->c_flags.tbf_release_pending_flg);
                  ...
                  if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                  {
                    ...
                    MSG_GERAN_LOW_0_G("MAC_GRR_MAC_STATUS");
                    gmac_util_send_mac_grr_mac_status(&idle_data_ptr->mac_grr_sig,MAC_IDLE);
                    ...
                    gmac_util_complete_pcco_process(PCCO_FAILURE_NO_RESPONSE,MAC_INITIATED,GMAC_IDLE,__LINE__);
         ...
         **********************************************************/
         else if(task_id == MS_MAC_RR)
         {
           ...
           switch (signal_id)
           {
               ...
               case GRR_MAC_SEND_PCCF:
                    MSG_GERAN_LOW_0_G("GMAC received GRR_MAC_SEND_PCCF from GRR");
                    ...
                    {
                      gmac_util_start_pccf_process(SENDING_VIA_UL_PACCH,GMAC_IDLE,__LINE__,(uint8)idle_data_ptr->mac_idle_substate);
                    }
                    else
                    {
                      MSG_GERAN_LOW_0_G("CSN.1 encode failure of PCCF, wait in state till t3176 exp");
                    }
               break;
         ...
         else if(task_id == MS_MAC_RLC)
         {
           ...
           switch (signal_id)
           {
               case RM_UL_EST_TBF_REQ :
                    MSG_GERAN_LOW_1_G("Unexpected RM_UL_EST_TBF_REQ, action: %d",
                                             global_data_ptr->mac_action);

                    (void)gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__);
               break;
         ...
         else if(task_id == MS_MAC_MAC)
         {
           ...
           switch (signal_id)
           {
               case MAC_TIMER_T3168:
                    ...
                    MSG_GERAN_LOW_1_G( "MAC info timer t3168 expired ",0 );
                    ...
                    /*GMAC sends block signal to RLC and will be unblocked rlc once gmac received release conf from L1*/
                    gmac_idle_send_no_access_to_rlc();
                    gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, UL_TBF_ABNORMAL,MAC_LOG_UL_EXPIRED_T3168,MAC_LOG_DL_UNKNOWN_ERROR);
               break;
               ...
               case MAC_TIMER_T3200_8:
                    MSG_GERAN_LOW_1_G("MAC info timer t3200 expired si_id %d",signal_id );
      ...
      case UL_2P_ASS:
           ...
           switch (signal_id)
           {
                ...
                switch (global_data_ptr->channel_msg)
                {
                                  ...
                                  ************************************************/
                                  ...
                                  if(idle_data_ptr->pua_data_result == NO_ERRORS)
                                  {
                                           ...
                                           idle_data_ptr->temp_egprs_2p_sb_mb_alloc == MB_ALLOC_OF_1_BLOCK ) )
                                    {
                                      ...
                                      gmac_idle_egprs_rac_procedure_2(TRUE,global_data_ptr->c_flags.egprs_rac_retran_req,TRUE,FALSE);
                                    }
                                    gmac_idle_simulate_freq_cnf_for_ul_states();
                                  }
                                  else
                    ...
                    case PACKET_ACCESS_REJECT:
                       ...
                       **********************************************************/
                         ...
                         if(check_idle_par_addressing(idle_data_ptr->access_granted) == TRUE)
                         {
                           ...
                           if( rex_get_timer(&mac_timers_ptr->timer_t3172) != 0 )
                           {
                             ...
                             */
                             gmac_idle_send_del_one_or_no_del_to_rlc();
                           }
                           else
                           {
                             idle_data_ptr->release_cause = ACCESS_RETRY;
                             ...
                             if ((global_data_ptr->mac_action == UL_DATA_EST) &&
                                 (gmac_util_get_pcco_state() == PCCO_ACTIVE))
                             {
                               if(global_data_ptr->gmac_util_data_req_cause == GRR_CAUSE_GMM_CELL_UPDATE)
                               {
                                 ...
                                 gmac_util_block_rlc(DEL_ONE,GMAC_IDLE,__LINE__);
                               }
                               else
                               {
                           ...
                           gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, UL_TBF_ABNORMAL,MAC_LOG_UL_ACCESS_REJECT,MAC_LOG_DL_UNKNOWN_ERROR);
                         }
                    break;
             ...
             case MAC_PH_CONNECT_IND:
                  ...
                  if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                  {
                    if(global_data_ptr->tbf_type == (uint8)dynamic_alloc_e)
                    {
                      MSG_GERAN_LOW_0_G( "MAC sent MAC_IT_UL_DYNAMIC");

                      memset(&(idle_data_ptr->mac_int_sig.msg.mac_it_ul_dynamic),0,sizeof(mac_it_ul_dynamic_t));

                      gmac_idle_send_mac_internal_sig(&idle_data_ptr->mac_int_sig,MAC_IT_UL_DYNAMIC);
                      ...
                      MSG_GERAN_MED_0_G("MAC sent MAC_IT_UL_DYNAMIC & MAC mode TRANSFER");
                    ...
                    {
                      MSG_GERAN_LOW_0_G( "2p rac retran completed, wait in state for DYN alloc");
                    }
                    else
                    {
                      MSG_GERAN_LOW_1_G( "MAC info ERROR tbf type not relevent for 2p ass tbf_type %d",global_data_ptr->tbf_type);
                    }
                  }
                  else
                  {
...
#ifdef FEATURE_GSM_EDTM
               case GRR_MAC_EDTM_PKT_CS_EST_REQ:
                 MSG_GERAN_LOW_1_G("GMAC Accepted GRR_MAC_EDTM_PKT_CS_EST_REQ rr_est_cause: 0x%x in 2 Phase Cont Res",
         ...
         **********************************************************/
         else if(task_id == MS_MAC_RLC)
         {
           ...
           {
               case RM_UL_EST_TBF_REQ :
                    MSG_GERAN_LOW_1_G( "MAC Rcvd RM_UL_EST_TBF_REQ and mac action is %d",
                      global_data_ptr->mac_action);

                    gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__);
               break;

               case RM_UL_RELEASE_IND:
                    gmac_stop_t3168();
                    /*exception handler:- RLC has detected an expection case*/
                    MSG_GERAN_MED_1_G("MAC Rcvd RM_UL_RELEASE_IND, exception recovery, cause:%d",
                      idle_data_ptr->release_cause);
                    ...
                    /*GMAC block RLC and will unblock RLC once GMAC received rel cnf from L1*/
                    gmac_util_block_rlc(NO_DEL,GMAC_IDLE,__LINE__);
                    gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, UL_TBF_ABNORMAL,MAC_LOG_UL_UNKNOWN_ERROR,MAC_LOG_DL_UNKNOWN_ERROR);
               break;
         ...
         **********************************************************/
         else if(task_id == MS_MAC_MAC)
         {
           ...
           {
               case MAC_TIMER_T3168:
                    MSG_GERAN_MED_1_G( "MAC info timer t3168 expired, rel_pend:%d",
                      global_data_ptr->c_flags.tbf_release_pending_flg);
                    ...
                    if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                    {
                      ...
                      gmac_idle_send_no_access_to_rlc();
                      gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, UL_TBF_ABNORMAL,MAC_LOG_UL_EXPIRED_T3168,MAC_LOG_DL_UNKNOWN_ERROR);
                      gmac_send_mac_grr_abnormal_release(RR_CONTENTION_FAILED);
                    }
               break;
           ...
           switch (signal_id)
           {
             case MAC_PH_DATA_IND :
                ...
                switch (global_data_ptr->channel_msg)
                {
                    case PACKET_MEASUREMENT_ORDER :
                         ...
                         {
                           rr_read_public_store(RRPS_TLLI,&idle_data_ptr->rrps_tlli);
                         ...
                         {
                           MSG_GERAN_LOW_0_G("MAC info pmo rcvd not addressed by TLLI in 1p cont res");
                         }
                    break;
                    ...
                    case PACKET_UPLINK_ASSIGNMENT:
                         ...
                         *******************************************************************/
                         ...
                         if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                         {
                           ...
                           {
                             idle_data_ptr->egprs_pua_contention_result = gmac_idle_egprs_pua_contention_res_checking(idle_data_ptr->mac_idle_substate);
                             ...
                             else
                             {
                               if(gmac_idle_check_pua_addressing(global_data_ptr->idle_mode_ul_tbf_release_action,idle_data_ptr->mac_idle_substate ) == TRUE )
                               {
                                 if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
                                 {
                                   if(idle_data_ptr->egprs_pua_contention_result == PUA_CONT_RES_PASSED)
                                   {
                                     ...
                                     if (global_data_ptr->tbf_type == (uint8)dynamic_alloc_e)
                                     {
                                       ...
                                       MSG_GERAN_MED_1_G
                                         ("MAC sent MAC_IT_UL_DYN_EGPRS_PUA_RCVD_IN_CONT_RES, UL re-alloc attempts%d, set idle_ul_realloc_state = IDLE_REALLOC_NO_ACTION",
                                     ...
                                     gmac_util_complete_pcco_process(PCCO_SUCCESS,MAC_INITIATED,GMAC_IDLE,__LINE__);
                                     global_data_ptr->mac_action = NONE;
                                     display_mac_action(global_data_ptr->mac_action);
                                     gmac_idle_log_state_change(PASSIVE, __LINE__);
                                     idle_data_ptr->mac_idle_substate = PASSIVE;
                                 ...
                                 {
                                   /* ignore pua msg*/
                                   MSG_GERAN_LOW_0_G( "MAC Info ERROR GPRS PUA rcvd error");
                                 }
                               }
                               else
                               {
                                  /* ignore pua msg*/
                                 MSG_GERAN_LOW_0_G( "MAC Rcvd PUA addressing error");
                               }
                             }
                           ...
                           {
                             MSG_GERAN_LOW_0_G(" PUA rcvd before 1st data block sent ignore");
                           }
                         }
                         else
                         {
                           MSG_GERAN_LOW_0_G("GMAC Ignore PUA when releasing L1");
                         }
                    break;
                    ...
                    case PACKET_UPLINK_ACK_NACK:
                         ...
                         *************************************************************/
                         ...
                         if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                         {
                           idle_data_ptr->contention_result =  gmac_idle_puan_handler();
                           ...
                           if( idle_data_ptr->contention_result == CON_RES_TRANSFER )
                           {
                              ...
                              *****************************************************************/
                              MSG_GERAN_MED_3_G( "MAC Rcvd PUAN ,m_s %d,m_a %d, tbf:%d",
                                idle_data_ptr->mac_idle_substate,global_data_ptr->mac_action, (uint8)global_data_ptr->tbf_type);
                              ...
                              **********************************************************************/
                              ...
                              if (global_data_ptr->tbf_type == (uint8)dynamic_alloc_e)
                              {
                                 MSG_GERAN_LOW_1_G( "MAC sent MAC_IT_UL_DYNAMIC, contention res UL re-alloc attempts count %d",
                                                    idle_data_ptr->idle_realloc_attempts_count);
                                 ...
                                 {
                                   MSG_GERAN_LOW_0_G("tlx PUAN rcvd before connect ind for unsolicited was PUA not rcvd");
                           ...
                           else if(idle_data_ptr->contention_result == CON_RES_RELEASE)
                           {
                              ...
                              ************************************************************/
                              ...
                              if (global_data_ptr->mac_action == PMR_EST || global_data_ptr->mac_action == PCCF_EST)
                              {
                                MSG_GERAN_LOW_1_G("MAC abnormal allocation for mac_action %d",global_data_ptr->mac_action);
                              }
                              ...
                              (void)gmac_normal_ul_tbf_release_proc( FALSE,FALSE,FALSE);
                           }
                    ...
                    case PACKET_TBF_RELEASE :
                          idle_data_ptr->tbf_rel = gmac_idle_packet_tbf_release(TRUE);

                          MSG_GERAN_MED_3_G( "MAC Rcvd PKT_TBF_REL ,m_s %d, m_a %d, tbf_rel:%d",
                            idle_data_ptr->mac_idle_substate,global_data_ptr->mac_action, idle_data_ptr->tbf_rel);
                          ...
                          {
                            MSG_GERAN_LOW_0_G( "MAC Rcvd PKT_TBF_REL UL Normal rel");
                          }
                          else
                          {
                            MSG_GERAN_LOW_0_G("MAC Info P_TBF_REL, Invalid release");
                          }
                    break;
                    ...
                    case PACKET_DOWNLINK_ASSIGNMENT:
                      ...
                      if(global_data_ptr->c_flags.tbf_release_pending_flg == TRUE)
                      {
                       ...
                       MSG_GERAN_LOW_0_G("MAC Rcvd PACKET_DOWNLINK_ASSIGNMENT");
                       /*********************************************************************
                       * FOR any cell selection/reselection activities ignore PDA           *
                       *********************************************************************/
                       ...
                       if (idle_data_ptr->release_cause == CONT_RES_COMPLETE)
                       {
                         /* Contention has passed and MAC is releasing for PUAN FAI=1, so address check using TFI
                            as it is now valid */
                         idle_data_ptr->pda_data_result = gmac_idle_pda_handler(&idle_data_ptr->dl_ass_ind_sig,FALSE,PDA_CHECK_TFI_AND_TLLI);
                       }
                       else
                       {
                         idle_data_ptr->pda_data_result = gmac_idle_pda_handler(&idle_data_ptr->dl_ass_ind_sig,FALSE,PDA_CHECK_TLLI_ONLY);
             ...
             case MAC_PH_CONNECT_IND:
                  ...
                  if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                  {
                    ...
                    if(idle_data_ptr->i_flags.idle_ul_realloc_state == IDLE_REALLOC_WAITING_TO_CONNECT)
                    {
                      MSG_GERAN_LOW_0_G( "MAC sent MR_UL_ALLOC_CNF");
                      ...
                      gmac_idle_send_mr_ul_alloc_cnf(idle_data_ptr->mac_rlc_sig_ptr,&idle_data_ptr->mr_ul_alloc_cnf_sig);
                      idle_data_ptr->i_flags.idle_ul_realloc_state = IDLE_REALLOC_NO_ACTION;
                      MSG_GERAN_MED_0_G("MAC sent MR_UL_ALLOC_CNF & set idle_ul_realloc_state = IDLE_REALLOC_NO_ACTION");
                      ...
                      if(idle_data_ptr->i_flags.pending_realloc_req == TRUE)
                      {
                        ...
                        MSG_GERAN_MED_0_G("MAC Sent MAC_L1_QUALITY_RPT & set idle_ul_realloc_state = IDLE_REALLOC_REQUESTED");
                        ...
                        gprs_l1_mac_acc_func(&idle_data_ptr->mac_l1_sig);
                    ...
                    else
                    {
                      ...
                      if (idle_data_ptr->i_flags.rcvd_unsolicited_pua_waiting_for_connected_ind == TRUE)
                      {
                        ...
                        MSG_GERAN_LOW_0_G("tlx connect ind for unsolicited PUA not rcvd before PUAN that completes contention res");
             ...
             case L1_MAC_USF_DETECTED:
                  ...
                  {
                    MSG_GERAN_LOW_0_G("GMAC releasing L1, so ignoring L1_MAC_USF_DETECTED signal");
                  }
             break;
             ...
             case L1_MAC_1ST_DATA_BLOCK_SENT:
                  ...
                  if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                  {
                    MSG_GERAN_LOW_2_G( "MAC Rcvd L1_MAC_1ST_DATA_BLOCK_SENT, mac state =%d mac_act %d",
                                       idle_data_ptr->mac_idle_substate ,global_data_ptr->mac_action);
                    /*******************************************************************************
                    ...
                    ********************************************************************************/
                    ...
                    {
                      MSG_GERAN_LOW_1_G("MAC error TBF type not valid for 1 phase, tbf_type =%d",global_data_ptr->tbf_type);
                    }
                  }
                  else
                  {
                    MSG_GERAN_LOW_0_G("GMAC releasing L1, so ignoring L1_MAC_1ST_DATA_BLOCK_SENT signal");
                  }
             break;
             ...
             case L1_MAC_SINGLE_BLOCK_SENT:
                  MSG_GERAN_LOW_1_G("L1_MAC_SINGLE_BLOCK_SENT rcvd. ul re-alloc on UL PACCH gmac_prr_on_pacch_state %d",
         ...
         else if(task_id == MS_MAC_RR)
         {
           ...
           switch (signal_id)
           {
               ...
               case GRR_MAC_EDTM_PKT_CS_EST_REQ:
                 ...
                 {
                   MSG_GERAN_LOW_0_G("GMAC Rejected GRR_MAC_EDTM_PKT_CS_EST_REQ in 1 phase Cont Res, T3166 NOT running");
                   gmac_send_mac_grr_edtm_pkt_cs_est_rej(EDTM_CS_REJ_CANNOT_SERVICE_REQUEST);
                 }
               break;
#endif /*FEATURE_GSM_EDTM*/
         ...
         else if(task_id == MS_MAC_RLC)
         {
           ...
           switch (signal_id)
           {
               case RM_UL_N3104_EXPIRY_IND :
                 if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
                 {
                    ...
                    ***************************************************************/
                    ...
                    gmac_idle_send_l1_release_sig(&idle_data_ptr->mac_l1_sig, UL_TBF_ABNORMAL,MAC_LOG_UL_EXPIRED_N3104,MAC_LOG_DL_UNKNOWN_ERROR );
                 }
               break;
               ...
               case RM_UL_EST_TBF_REQ :
                    if (global_data_ptr->mac_action == UL_DATA_EST)
                    {
                      MSG_GERAN_LOW_1_G( "MAC Rcvd Unexpected RM_UL_EST_TBF_REQ and mac action is %d",global_data_ptr->mac_action);
                    }
                    else
                    {
                      if(gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__)== UNBLOCKED)
                      {
                        MSG_GERAN_LOW_1_G( "GMAC should have already blocked rlc at this point mac action is %d",global_data_ptr->mac_action);
                      }
                    }
               break;
               ...
               case RM_UL_REALLOC_REQ:
                    ...
                    MSG_GERAN_MED_1_G("MAC Rcvd RM_UL_REALLOC_REQ, ul_realloc_state:%d",
               ...
               case RM_UL_RELEASE_IND:
                    if(global_data_ptr->msg_ptr->rlc_mac_sig.msg.ul_release_ind.cause == REL_UL_ABNORMAL_WITH_ACCESS_RETRY)
                    {
                      ...
                      MSG_GERAN_MED_1_G("MAC Rcvd RM_UL_RELEASE_IND, abnormal release UL TBF, cause:%d",
                        idle_data_ptr->release_cause);
                      /*GMC block RLC and will unblock RLC once GMAC received rel cnf from L1*/
                      gmac_util_block_rlc(NO_DEL,GMAC_IDLE,__LINE__);
         ...
         else if(task_id == MS_MAC_MAC)
         {
           ...
           switch (signal_id)
           {
               ...
               case MAC_TIMER_T3180:
                     ...
                     **********************************************************************/
                     MSG_GERAN_MED_2_G("MAC Rcvd MAC_TIMER_%d, rel_cause:%d", signal_id, idle_data_ptr->release_cause);

                     if(idle_data_ptr->release_cause != GRR_INITIATED_NO_ACCESS )
                     {
                       MSG_GERAN_LOW_0_G( "MAC wont send MR UL CONTENTION IND to RLC. Instead of this gmac sends no access ind to RLC");
      ...
      ******************************************************************/
      case DL_ASS:
         ...
         if(task_id == MS_MAC_L1)
         {
           ...
           switch (signal_id)
           {
             case MAC_PH_DATA_IND :
                ...
                switch (global_data_ptr->channel_msg)
                {
                    ...
                    case PACKET_DOWNLINK_ASSIGNMENT :
                         ...
                         ***************************************************************/
                         idle_data_ptr->pda_data_result = gmac_idle_pda_handler(&idle_data_ptr->dl_ass_ind_sig,TRUE,PDA_CHECK_TLLI_ONLY);
                         /**************************************************************
                         ...
                         **************************************************************/
                         if( idle_data_ptr->pda_data_result != NO_ERRORS)
                         {
                           ...
                           {
                              MSG_GERAN_LOW_1_G("PDA addressing error ignore pda_data_result = %d",idle_data_ptr->pda_data_result);
                           }
                         }
                         else
                         {
                            ...
                            MSG_GERAN_MED_2_G( "MAC Rcvd PDA ,m_s %d,m_a %d. sent MAC_L1_DL_TBF_CONFIG",
                              idle_data_ptr->mac_idle_substate, global_data_ptr->mac_action);
                            rrbp_control_block_handler_2(PCA_DL_MSG);
                            MSG_GERAN_LOW_0_G( "MAC sent MAC_L1_DL_TBF_CONFIG");
             ...
             case L1_MAC_FREQ_ASS_CNF:
                  MSG_GERAN_LOW_0_G("MAC info Rcvd Freq cnf no action");
             ...
             case MAC_PH_CONNECT_IND:
                  ...
                  ******************************************************************/
                   ...
                   {
                     {
                       ...
                       if (idle_data_ptr->mac_rlc_sig_ptr != NULL)
                       {
                         ...
                         grlc_put_cmd_mac(idle_data_ptr->mac_rlc_sig_ptr);
                       }
                       else
                       {
                       ...
                       global_data_ptr->mac_mode = TRANSFER_MODE;
                       MSG_GERAN_MED_0_G("MAC mode TRANSFER. sent MAC_IT_DL_DYNAMIC");
                       ...
                       MSG_GERAN_LOW_0_G( "MAC sent MAC_IT_DL_DYNAMIC");
                   ...
                   {
                     MSG_GERAN_LOW_0_G("GMAC releasing L1, so ignoring MAC_PH_CONNECT_IND signal");
                   }
             ...
             case L1_MAC_TBF_REL_CONFIRM :
                  ...
                  gmac_idle_dl_release_confirm_handler();
               ...
               */
               ...
               case GRR_MAC_SEND_PCCF:
                    ...
                    if (gmac_util_encode_ul_control_msg_handler(PCCF,FALSE) == PASS)
                    {
                      ...
                      MSG_GERAN_LOW_0_G("GMAC PCCF encoding is successful and GMAC gives pccf sets pccf sending to true");
                    }
                    else
                    {
                      MSG_GERAN_LOW_0_G("CSN.1 encode failure of PCCF, wait in state till t3176 exp");
               ...
               case GRR_MAC_DL_TBF_EST:
                    MSG_GERAN_LOW_0_G("MAC info Can't get this GRR_MAC_DL_TBF_EST in UL_1P_ASS");
               break;
         ...
         else if(task_id == MS_MAC_RLC)
         {
           ...
           switch (signal_id)
           {
               case RM_UL_EST_TBF_REQ :
                    if(gmac_util_is_rlc_blocked(GMAC_IDLE,__LINE__)== UNBLOCKED)
                    {
                        MSG_GERAN_LOW_0_G( "GMAC should have already blocked rlc at this point ");
                    }
               break;
         ...
         else if(task_id == MS_MAC_MAC)
         {
           ...
           switch (signal_id)
           {
               ...
               case MAC_TIMER_T3172:
                    ...
                    **********************************************************************/
                    MSG_GERAN_LOW_1_G( "MAC info timer t3172 expired ",0 );
                    /*GMAC will send access ind once it is connected the downlink tbf*/
                    MSG_GERAN_LOW_1_G( "MAC will send access ind once it is connected the downlink tbf",0 );
      ...
      default:
         ...
         {
           MSG_GERAN_ERROR_1_G( "Unknown substate %d, initiated Recovery Restart", idle_data_ptr->mac_idle_substate);
           geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
         }
         else
         {
           ERR_GERAN_FATAL_1_G("Unknown mac substate %d", idle_data_ptr->mac_idle_substate);
         }
      break;
...
)
{
  ...
  idle_data_ptr->release_cause = GRR_INITIATED_NO_ACCESS;
  /*GMC block RLC and will unblock RLC once GMAC received rel cnf from L1*/
  gmac_util_block_rlc(delete_pdu_flag,GMAC_IDLE,__LINE__);
  /*Depenidng upon the gmac substate, GMC stops all access timers */
  gmac_idle_stop_access_timers(idle_data_ptr->mac_idle_substate);

  if(current_mac_state == DL_ASS )
  {
    gmac_idle_send_l1_release_sig(mac_l1_sig_ptr, DL_TBF_NORMAL,MAC_LOG_UL_UNKNOWN_ERROR,MAC_LOG_DL_CELL_RESELECTION );
  }
  else
  {
    gmac_idle_send_l1_release_sig(mac_l1_sig_ptr, UL_TBF_NORMAL,MAC_LOG_UL_CELL_RESELECTION,MAC_LOG_DL_UNKNOWN_ERROR );
  }
}
/*===========================================================================
===
...
static boolean gmac_idle_ignore_pda(gprs_mac_action_t mac_act,mac_l1_sig_t *mac_l1_sig_ptr)
{
  ...
  {
    if(gmac_util_get_pcco_state() == PCCO_ACTIVE)
    {
        ignore_pda = TRUE;
    }
    else
    {
      gmac_idle_send_l1_release_sig(mac_l1_sig_ptr, UL_TBF_ABNORMAL,MAC_LOG_UL_PDA_ABORT,MAC_LOG_DL_UNKNOWN_ERROR);
    }
  }
  else if(mac_act == PMR_EST)
  {
    gmac_idle_send_pmr_cnf_to_grr(MAC_GRR_FAILURE);
    gmac_idle_send_l1_release_sig(mac_l1_sig_ptr, UL_TBF_ABNORMAL,MAC_LOG_UL_PDA_ABORT,MAC_LOG_DL_UNKNOWN_ERROR);
  }
...
static void gmac_idle_ul_release_confirm_handler(void)
{
  ...
  {
    MSG_GERAN_LOW_0_G("GMAC EDTM T3196 running on release from Idle");
    gmac_stop_t3196();
  ...
  switch(idle_data_ptr->release_cause)
  {
      ...
      case CONT_RES_UL_RE_ALLOC_FAILURE:
           ...
           gmac_idle_abnormal_release_handler(global_data_ptr->mac_action,idle_data_ptr->mac_idle_substate);
           gmac_idle_reset_parameters(idle_data_ptr->mac_idle_substate);
           global_data_ptr->mac_action = NONE;
           gmac_idle_log_state_change(PASSIVE, __LINE__);
           idle_data_ptr->mac_idle_substate = PASSIVE;
      ...
      case PARTIAL_SYS_ACQ:
           /* For this release cause send abnormal release to GRR if pcco is not active and give
              ...
              to PASSIVE*/
           if(gmac_util_get_pcco_state() == NO_PCCO_ACTIVE)
           {
             gmac_send_mac_grr_abnormal_release(RR_PART_SYS_INFO_REQ);
           }
           gmac_idle_abnormal_release_handler(global_data_ptr->mac_action,idle_data_ptr->mac_idle_substate);
           gmac_idle_reset_parameters(idle_data_ptr->mac_idle_substate);
           global_data_ptr->mac_action = NONE;
      ...
      case PDA_RCVD_IN_DL_SB_TBF:
           ...
           gmac_idle_csn1_decode_stored_pda_data(idle_data_ptr->csn1_encoded_pda_msg);
           ...
           idle_data_ptr->pda_data_result = gmac_idle_pda_handler(&idle_data_ptr->dl_ass_ind_sig,TRUE,PDA_NO_ADDRESS_CHECK);
  ...
  MSG_GERAN_LOW_2_G("MAC info current idle sub state %d current mac action",idle_data_ptr->mac_idle_substate,global_data_ptr->mac_action);
}
  ...
  switch(idle_data_ptr->release_cause)
  {
      ...
      ********************************************************/
      case REL_DL_TBF:
           if( rex_get_timer(&mac_timers_ptr->timer_t3172) == 0 )
           {
             gmac_util_unblock_rlc(NO_CELL_CHANGE_NO_PURGE,GMAC_IDLEUTIL,__LINE__);
           }
           gmac_idle_reset_parameters(idle_data_ptr->mac_idle_substate);
           global_data_ptr->mac_action = NONE;
           gmac_idle_log_state_change(PASSIVE, __LINE__);
...
{
  MSG_GERAN_LOW_4_G("MAC Rcvd %d, ln %d, m_state %d, m_act %d ", dl_chan_msg, line_no, mac_state, current_action );
}
...
{
  MSG_GERAN_MED_5_G( "MAC Rcvd sig %d, task %d, ln %d, m_state %d, m_act %d",
    sig_id, task_id, line_no, mac_state, current_action );
}
...
static void display_mac_action(gprs_mac_action_t mac_act)
{
  MSG_GERAN_HIGH_1_G("MAC Info mac_action:%d", mac_act);
}
...
===
===========================================================================*/
void gmac_init_idle_flags(void)
{
  gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();
...
===
===========================================================================*/
void gmac_init_idle_data_non_zero(void)
{
  gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();
  idle_data_ptr->mac_idle_substate = PASSIVE;
...
static void gmac_idle_simulate_freq_cnf_for_ul_states(void)
{
  ...
  if ( global_data_ptr->tbf_type == (uint8)dynamic_alloc_e )
  {
    switch ( idle_data_ptr->mac_idle_substate )
    {
      case UL_1P_ASS:
        ...
        else
        {
          if (gmac_util_get_pccf_state() == SENDING_VIA_UL_PACCH)
          {
             ...
             gmac_send_mac_l1_delete_ul_ctrl_msg(PACKET_CELL_CHANGE_FAILURE);
#endif/*FEATURE_GSM_EDTM*/
          ...
          if( (idle_data_ptr->access_granted == one_phase) &&
              ...
              (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)  )
          {
             gmac_idle_egprs_rac_procedure_2(FALSE,global_data_ptr->c_flags.egprs_rac_retran_req,FALSE,FALSE);
          }
          ...
          MSG_GERAN_MED_0_G( "MAC sent MAC_L1_DYNAMIC_TBF_CONFIG & MR_UL_ALLOC_CNF");
      ...
      case UL_1P_CONTENTION_RES:
        ...
        else
        {
          ...
          if(idle_data_ptr->i_flags.idle_ul_realloc_state == IDLE_REALLOC_REQUESTED)
          {
            ...
            MSG_GERAN_MED_0_G("MAC info performing UL re-alloc, wait for con ind before sending MR_UL_ALLOC_CNF. sent MAC_L1_DYNAMIC_TBF_CONFIG");
          ...
          else
          {
            ...
            MSG_GERAN_LOW_0_G("MAC info performing unsolicited pua UL re-alloc, wait for con ind before sending MR_UL_ALLOC_CNF ");
      ...
      default:
          ...
          MSG_GERAN_LOW_1_G("GMAC INFO dynamic in BAD mac idle substate:0x%x",idle_data_ptr->mac_idle_substate);
  ...
  else if ( global_data_ptr->tbf_type == (uint8)single_blk_alloc )
  {
    ...
    {
      case UL_1P_ASS:
        MSG_GERAN_MED_1_G( "MAC sent MAC_L1_SINGLE_BLOCK_CONFIG, action:%d", global_data_ptr->mac_action);
        ...
        if(global_data_ptr->mac_action == PMR_EST)
        {
           MSG_GERAN_LOW_0_G( "MAC sent MAC_PH_DATA_REQ with PMR");
           gmac_util_send_ul_control_msg_handler(PMR, UL_PRESENT, GRR_CNF_NOT_NEEDED);
        }
        else if (global_data_ptr->mac_action == PCCF_EST)
        {
           MSG_GERAN_LOW_0_G( "MAC sent MAC_PH_DATA_REQ with PCCF");
           gmac_util_send_ul_control_msg_handler(PCCF, UL_PRESENT, GRR_CNF_NOT_NEEDED);
        }
      break;
      ...
      default:
        ...
        MSG_GERAN_LOW_1_G("GMAC INFO single block in BAD mac idle substate:0x%x",idle_data_ptr->mac_idle_substate);
  ...
  else if ( global_data_ptr->tbf_type == (uint8)multi_blk_allocation )
  {
    switch ( idle_data_ptr->mac_idle_substate )
    {
      ...
      case UL_2P_ASS:
        ...
        MSG_GERAN_LOW_1_G( "MAC info ARAC retran via MB alloc",0 );
        gmac_send_add_rac_info_msg(NO_RE_EST);
      break;

      default:

        MSG_GERAN_LOW_1_G("GMAC INFO multi block in BAD mac idle substate:0x%x",idle_data_ptr->mac_idle_substate);
  ...
  else
  {
    ...
    MSG_GERAN_LOW_1_G("MAC info bad tbf_type: 0x%x",global_data_ptr->tbf_type);
...
static void gmac_idle_handle_no_freq_condition_in_pua(void)
{
  ...
  switch ( idle_data_ptr->mac_idle_substate )
  {
    case UL_1P_ASS:
      ...
      {
         MSG_GERAN_LOW_0_G("MAC error No Freq params in SB alloc");
      }
    break;
       ...
       *  only need to send L1 the new Pdtch config                         */
       if(global_data_ptr->tbf_type == (uint8)dynamic_alloc_e )
       {
         ...
         if (global_data_ptr->mac_action == PMR_EST || global_data_ptr->mac_action == PCCF_EST)
         {
           if(geran_get_nv_recovery_restart_enabled())
           {
             MSG_GERAN_ERROR_1_G("MAC abnormal allocation for mac_action %d, initiated Recovery Restart",
               global_data_ptr->mac_action);
             geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
           }
           else
           {
             ERR_GERAN_FATAL_1_G("MAC abnormal allocation for mac_action %d", global_data_ptr->mac_action);
           }
         }
         else
         {
           ...
           {
             /* dont send MR_UL_ALLOC_CNF do this if PH CONNECT IND is rcvd in UL_1P_CONTENTION_RES*/
             MSG_GERAN_LOW_0_G("MAC info performing UL re-alloc, wait for con ind before sending MR_UL_ALLOC_CNF ");
             idle_data_ptr->mac_l1_sig.mac_l1_dynamic_tbf_config.message_header.message_set = MS_MAC_L1;
             idle_data_ptr->mac_l1_sig.mac_l1_dynamic_tbf_config.message_header.message_id = (uint8)MAC_L1_DYNAMIC_TBF_CONFIG;
             gprs_l1_mac_acc_func(&idle_data_ptr->mac_l1_sig);
           ...
           MSG_GERAN_MED_1_G("MAC sent MAC_L1_DYNAMIC_TBF_CONFIG, gmac_local_ul_ts_alloc =%d",
             global_data_ptr->gmac_local_ul_ts_alloc);
         }
       }
       else if(global_data_ptr->tbf_type == (uint8)extension_alloc)
       {
         MSG_GERAN_LOW_0_G("PUA with extension_alloc, not supported ");
         idle_data_ptr->pua_data_result = ASSIGNMENT_DATA_ERROR;
       }
       else
       {

         MSG_GERAN_LOW_1_G("MAC info Unexpected tbf_type %d ",global_data_ptr->tbf_type);
         idle_data_ptr->pua_data_result = ASSIGNMENT_DATA_ERROR;
       }
     break;
    ...
    default:
      if(geran_get_nv_recovery_restart_enabled())
      {
        MSG_GERAN_ERROR_1_G("MAC Info, called in wrong IDLE state:0x%x, initiated Recovery Restart",
          idle_data_ptr->mac_idle_substate);
        geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
      }
      else
      {
        ERR_GERAN_FATAL_1_G("MAC Info, called in wrong IDLE state:0x%x", idle_data_ptr->mac_idle_substate);
      }
    break;
  }
}
...
===
===========================================================================*/
gprs_mac_idle_substates_t gmac_idle_get_idle_state(void)
{
  gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();
...
===
===========================================================================*/
boolean idle_ul_realloc_in_progress(void)
{
 boolean status;
 gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();
