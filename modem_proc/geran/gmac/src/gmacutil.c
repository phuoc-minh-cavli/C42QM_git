...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacutil.c#7 $
***
...
gmac_util_data_t gmac_util_data;
...
#error code not present
#else
static void modify_nw_list_for_1800_1900_band_combination_for_egprs( uint8 band_count, uint8 *nw_bands_ptr);
static void modify_nw_list_for_1800_1900_band_combination_for_gprs( uint8 band_count, uint8 *nw_bands_ptr);
#endif /*PERLUTF*/
...
===========================================================================*/
static boolean gmac_util_get_eda_from_dtm_ass( gm_dtm_channel_assign_req_t *dtm_ass_ptr, uint8 *eda_ptr );
...
static void gmac_nacc_release_all_containers(void);
...
===
===  DESCRIPTION
===  Returns a pointer to util data space 
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
static gmac_util_data_t* gmac_get_util_data_ptr(void)
{
  return &gmac_util_data;
}
...
===
===========================================================================*/
void gmac_init_util_data_non_zero(void)
{
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
...
void gmac_complete_reset(void)
{
  ...
  gmac_init_null_data();
  gmac_init_idle_data();
  gmac_init_idle_util_data();
  gmac_init_transfer_data();
  gmac_init_mac_proc_data();
  gmac_init_rb_data();
  gmac_init_task_data();
  gmac_init_tran_ul_data();
  gmac_init_tran_dl_data();
...
===
===========================================================================*/
void reset_tbf_parameters(void)
{
...
static boolean gmac_util_compare_freq_params(frequency_information_T *freq_list_A,frequency_information_T *freq_list_B)
{
      ...
      (freq_list_A->frequency_list.no_of_items == freq_list_B->frequency_list.no_of_items))
  {
   ...
   for(i = 0; i< freq_list_B->frequency_list.no_of_items; i++)
   {
     ...
     {
        MSG_GERAN_MED_4_G("Info a_list freq %x  b_list freq %x, a_list band %x  b_list band %x",
          freq_list_A->frequency_list.channel[i].num,
          freq_list_B->frequency_list.channel[i].num,
          freq_list_A->frequency_list.channel[i].band,
          freq_list_B->frequency_list.channel[i].band);
        return (FALSE);
     }
   }
  }
                                                      ...
                                                      frequency_information_T *ml_freq_config_info_ptr)
{
  ...
  if (csn_freq_param_ptr->indir_enc.ma_number == 14)
  {
    rr_read_public_store(RRPS_VALID_MA_BIT_MAP,&valid_ma_numbers_bit_map);

    /* check MA-number is stored list */
    if (((valid_ma_numbers_bit_map) & (ma_mask << csn_freq_param_ptr->indir_enc.ma_number)) !=0)
    {
      if(gmac_util_change_mark_1_and_2_valid(csn_freq_param_ptr) == TRUE)
      {
        ...
        if(frequency_list_ptr == NULL)
        {
          ...
          MSG_GERAN_LOW_0_G("GMAC error: No frequency list in public store");
        }
        else
        {
      ...
      {
        return_value = FREQ_ERROR_PARTIAL_SYS_ACQ;
        MSG_GERAN_LOW_0_G("MAC error change_mark_1 or 2 mismatch ");
      }
    }
    else
    {
  ...
  else
  {
    ...
    {
      frequency_list_ptr = rr_read_public_store_freq_list(15);
...
static boolean gmac_util_change_mark_1_and_2_valid(freq_params_t *csn_freq_param_ptr)
{
  ...
  /* MA_NUMBER = 14 */
  rr_read_public_store(RRPS_CHANGE_MARK_SI13,&change_mark);
...
===
===========================================================================*/
void gmac_send_data_ind_to_grr(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
#error code not present
  ...
  gprs_grr_mac_acc_func(&global_data_ptr->grr_sig);

  /* CSN memory will be freed by GRR, so set the flag to FALSE */
...
===
===========================================================================*/
void gmac_util_send_dl_ass_to_rlc(void)
{
  mac_rlc_msg_t *mac_rlc_sig_ptr;
  ...
  rr_read_public_store(RRPS_TLLI_OLD,&old_local_rrps_tlli);

  mac_rlc_sig_ptr = grlc_get_cmd_buf_mac();
  if (mac_rlc_sig_ptr != NULL)
  {

    MSG_GERAN_HIGH_7_G("MAC Sent MR_DL_ASS_IND tbf_mode = %d, ws = %d, dl_ass_tlli %x,L_tlli %x o_tlli %x dl_ts = 0x%x , num_dl_ts %d",
      global_data_ptr->current_tbf_mode,
      global_data_ptr->current_egprs_dl_ws,
      global_data_ptr->gmac_store_dl_ass_tlli,
      local_rrps_tlli,
      old_local_rrps_tlli,
      global_data_ptr->gmac_local_dl_ts_alloc,
      gmac_utils_calculate_nts(global_data_ptr->gmac_local_dl_ts_alloc));
    ...
    else
    {
       ...
       {
         MSG_GERAN_ERROR_1_G("Invalid TBF mode = %d, initiated Recovery Restart",global_data_ptr->current_tbf_mode);
         geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
       }
       else
       {
         ERR_GERAN_FATAL_1_G("Invalid TBF mode = %d", global_data_ptr->current_tbf_mode);
       }
    }
...
===
===========================================================================*/
void gmac_ul_dummy_blk_to_l1(void)
{
  ...
  rr_read_public_store(RRPS_TLLI,&tlli_store);
  ...
  else
  {
    if(geran_get_nv_recovery_restart_enabled())
    {
      MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart for encode PDDCB");
      geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
    }
    else
    {
...
uint8 gmac_utils_get_second_lowest_ts_allocated(uint8 ts_alloc)
{
  ...
  if ( gmac_utils_calculate_nts(ts_alloc) > 1 )
  {
    for ( i=0; i<8; i++ )
    {
      if ( ( ( ts_alloc << i ) & 0x80 ) == 0x80 )
  ...
  {
    MSG_GERAN_LOW_0_G("GMAC Error in Function, Less than 2 timeslots allocated");
  }
...
===
===========================================================================*/
uint8 gmac_utils_calculate_nts(uint8 ts_alloc)
{
  uint8 num_ts = 0;
  uint8 i;
...
boolean gmac_pkt_tmslt_reconfig_dynamic_alloc_ie(mac_l1_ts_reconfig_t *ts_reconfig_ptr)
{
  ...
  {
   if (gmac_util_tbf_starting_ie(&(dyn_alloc_ptr->tbf_start_frm_num), &(ts_reconfig_ptr->starting_time), TRUE))
   {
...
#endif /*FEATURE_GPRS_PS_HANDOVER*/
{
...
#endif /*FEATURE_GSM_DTM*/
  {
    ...
    if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
    {
      MSG_GERAN_LOW_0_G("EGPRS_MULTISLOT checking");
  ...
  if ( dtm_ms_class_check == TRUE )
  {
    ...
    num_cs_ts = gmac_utils_calculate_nts(cs_ts_alloc);

    /* Check that the same DL TS has not been allocated to CS and PS */
    if (num_cs_ts != 1 ||
        (dl_ts_alloc & cs_ts_alloc) != 0 ||
        (ul_ts_alloc & cs_ts_alloc) != 0)
    {
      MSG_GERAN_HIGH_4_G("MAC ERR MS Class, UL slot alloc:0x%x, DL slot alloc:0x%x, CS TS alloc:0x%x, num_cs_ts %d",
        ul_ts_alloc,dl_ts_alloc,cs_ts_alloc, num_cs_ts);
  ...
  if (eda == GMAC_EDA_IND_EXTENDED_DYNAMIC_ALLOC)
  {
    ...
    if (gmac_utils_calculate_ts_alloc_ws(ul_ts_alloc) == GMAC_SHIFTED_USF_NUM_OF_UL_TS)
    {
      /* Shifted USF, monitor is on second lowest timeslot */
      total_dl_ts_alloc = 0x80 >> gmac_utils_get_second_lowest_ts_allocated(ul_ts_alloc);
  }
  else
#endif /*FEATURE_GSM_GPRS_MSC33*/
  {
    if (ul_ts_alloc != 0)
    {
      ...
      total_dl_ts_alloc = 0x80 >> gmac_utils_get_lowest_ts_allocated(ul_ts_alloc);
    }
  }
  ...
  if (gmac_utils_calculate_nts(total_dl_ts_alloc) > local_ms_class_max_ts_rx)
  {
  ...
  if (gmac_utils_calculate_nts(ul_ts_alloc) > local_ms_class_max_ts_tx)
  {
  ...
  if ( gmac_utils_calculate_ts_alloc_ws(total_dl_ts_alloc) > local_ms_class_max_ts_rx )
  {
    /* fail */
  ...
  if ( gmac_utils_calculate_ts_alloc_ws(ul_ts_alloc) > local_ms_class_max_ts_tx )
  {
    /* fail */
  ...
  /***************************** SUM Check ****************************************/
  if ( ( gmac_utils_calculate_nts(ul_ts_alloc) + gmac_utils_calculate_nts(total_dl_ts_alloc) ) > local_ms_class_sum )
  {
  ...
  if ( ( gmac_utils_calculate_ts_alloc_ws(ul_ts_alloc) == UL_EXT_DYN_ALLOC_NUM_TS ) &&
  ...
  {
    {
      ...
      {
        if(idle_ul_realloc_in_progress() == TRUE)
        {
                   ...
                   boolean                           idle_realloc_req)
{
  ...
  ***************************************/
       ...
       ( global_data_ptr->mac_mode == NULL_MODE ) )
  {
    ...
    {
      gmac_idle_egprs_rac_procedure_2(FALSE,FALSE,FALSE,TRUE);
  ...
  *********************************/
  ...
  rr_read_public_store(RRPS_MS_LOW_PRIO, &ms_low_prio);
#endif /* FEATURE_GERAN_IMPLICIT_PS_REJECT */
  prr_ptr->lap_signalling = ms_low_prio;
  ...
  ***************************************/
  ...
  if(prr_encoding == PASS)
  {
    ...
    else
    {
      ...
      mac_log_prr_event(global_data_ptr->access_requested);
    }
    ...
    if((re_est_action == NO_RE_EST) || (re_est_action == RE_EST_WAIT_FOR_SB_ALLOC))
    {
      ...
      gprs_l1_mac_acc_func(&ml_ph_data_sig);
    }
  ...
  else
  {
    if(geran_get_nv_recovery_restart_enabled())
    {
      MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart for encode PRR");
      geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
    }
    else
    {
...
)
{
  ...
  gprs_grr_mac_acc_func(&mac_grr_sig);
}
...
===
===========================================================================*/
void gmac_util_send_dl_release_to_rlc(void)
{
  mac_rlc_msg_t  *mac_rlc_sig_ptr;
...
===
===========================================================================*/
void gmac_util_send_grr_reset_cnf(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
pua_data_error_t gmac_util_frequency_param_ie(boolean compare_flag,boolean send_freq_config)
{
  ...
  *********************************************************************************/
  ...
  switch (csn_freq_param_ptr->encoding_type)
  {
    ...
    case direct2_valid:
      ...
      if(frequency_list_ptr != NULL)
      {
        return_value = gmac_util_load_freq_sig_data(frequency_list_ptr,ml_freq_config_info_ptr,direct2_valid);
      }
      else
      {
        return_value = FREQ_ERROR_PARTIAL_SYS_ACQ;
    ...
    case indirect_valid:
      ...
      ml_freq_config_info_ptr->tsc = csn_freq_param_ptr->tsc ;
      return_value = gmac_util_freq_indirect_valid(csn_freq_param_ptr,ml_freq_config_info_ptr);
      break;
  ...
  *********************************************************************************/
  if ((compare_flag == TRUE) &&
      (gmac_util_compare_freq_params(&global_data_ptr->gmac_store_freq_params_in_use,ml_freq_config_info_ptr)) == FALSE)
  {
  ...
  */
  ...
  if( (return_value == NO_ERRORS) && (send_freq_config == TRUE))
  {
    ...
    gprs_l1_mac_acc_func(&freq_config_sig);
  }
  return (return_value);
}
...
===
===========================================================================*/
void gmac_util_send_ul_reset_req_to_rlc(void)
{
  ...
  if (mr_ul_reset_req_ptr != NULL)
  {
    ...
    grlc_put_cmd_mac(mr_ul_reset_req_ptr);
  }
  else
  {
    ERR_GERAN_FATAL_0_G("MAC error MALLOC failed");
...
===
===========================================================================*/
void gmac_util_send_dl_reset_req_to_rlc(void)
{
  ...
  if (mr_dl_reset_req_ptr != NULL)
  {
    ...
    grlc_put_cmd_mac(mr_dl_reset_req_ptr);
  }
  else
  {
    ERR_GERAN_FATAL_0_G("MAC error MALLOC failed");
...
{
  gmac_store_trace_send_msg_buf_dyn(MS_MAC_RR,common_msg_ptr);
...
/*===========================================================================
===
===  FUNCTION      gprs_l1_mac_acc_func_dyn
===
===  DESCRIPTION
...
===
===========================================================================*/
void gprs_l1_mac_acc_func_dyn(mac_l1_sig_t *common_msg_ptr)
{
  gmac_store_trace_send_msg_buf_dyn(MS_MAC_L1,common_msg_ptr);
  ...
  switch (common_msg_ptr->mac_l1_freq_config.message_header.message_id)
  {
    ...
    case MAC_L1_EXT_TBF_START_IND:
      PUT_IMH_LEN( sizeof( mac_l1_ext_tbf_start_ind_t ) - sizeof( IMH_T ),
        &common_msg_ptr->mac_l1_ext_tbf_start_ind.message_header );
      break;
...
#error code not present
  ...
  /* Send message to L1 */
  (void)gs_send_message(GS_QUEUE_LAYER_1, (void *)common_msg_ptr, TRUE);
...
===
===========================================================================*/
access_type_e  determine_access_requested(void)
{
  ...
  {
    rr_read_public_store( RRPS_CELL_OPTIONS_EGPRS_PKT_CHAN_REQ, &egprs_pkt_chan_req );
...
===
===========================================================================*/
void gmac_idle_pkt_polling_request_handler(void)
{
  ...
  **************************************************************/
  ...
  if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
  {
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
                ...
                (((packet_polling_request_t *)global_data_ptr->csn_dec_data_ptr)->global_tfi.direction == dwnlink_valid))
        {
          ...
          if (gmac_util_global_tfi(((packet_polling_request_t *)global_data_ptr->csn_dec_data_ptr)->global_tfi) == FALSE)
          {
  ...
  if(ppr_for_ms == TRUE)
  {
    ...
    rr_read_public_store(RRPS_CELL_OPTIONS_ACCESS_BURST_TYPE,&ab_type);
...
===
===========================================================================*/
void gmac_util_pmo_handler(void)
{
  uint32 local_rrps_tlli;
    ...
    if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
    {
        ...
        switch (pmo_data_ptr->tlli_gtfi_pkt_ref_valid)
        {
          ...
          case global_tfi_valid:
            ...
            addr_match = gmac_util_global_tfi(pmo_data_ptr->global_tfi);
            break;
...
===
===========================================================================*/
void gmac_util_pkt_paging_on_pacch(void)
{
  mac_grr_sig_t                 mac_grr_sig;
  ...
  while ((cs_page_found == FALSE) && (repeated_page_idx < ppr_data_ptr->number_of_pages))
  {
    switch (ppr_data_ptr->repeated_page[repeated_page_idx].ptmsi_tmsi_mi_flag)
    {
      ...
      case tmsi_valid:
        ...
        rr_read_public_store(RRPS_TMSI,&local_rrps_tmsi);
      ...
      case tmsi_mi_valid:
        ...
        rr_read_public_store(RRPS_SIM_IMSI,local_rrps_imsi);
...
===
===========================================================================*/
boolean gmac_util_check_pcco_pmo_addressing(void)
{
    ...
    if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
    {
      ...
      if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
      {
        ...
        switch (pmo_data_ptr->tlli_gtfi_pkt_ref_valid)
        {
          ...
          case global_tfi_valid:
            /* if stored TFI = received TFI then match else ignore message */
            addresses_ms = gmac_util_global_tfi(pmo_data_ptr->global_tfi);
            break;
 ...
 else if(global_data_ptr->csn_dec_hdr_ptr->message_type == PACKET_CELL_CHANGE_ORDER)
 {
     ...
     switch (global_data_ptr->mac_mode)
     {
      ...
      case IDLE_MODE:
        ...
        addresses_ms = gmac_idle_check_pcco_addressing();
        break;
      ...
      case TRANSFER_MODE:
        ...
        if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
        {
          ...
          if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
          {
            ...
            {
              case tlli_valid:
                addresses_ms = gmac_match_old_or_new_tlli(pcco_data_ptr->tlli);
                break;

              case global_tfi_valid:
                /* if stored TFI = received TFI then match else ignore message */
                addresses_ms = gmac_util_global_tfi(pcco_data_ptr->global_tfi);
                break;
...
===
===========================================================================*/
mac_header_tfi_t gmac_mac_control_header_tfi_checking(void)
{
...
uint16 gmac_egprs_decode_ul_window_size(uint8 encoded_ws, uint8 usf_bitmap)
{
  ...
  num_ts_allocated = gmac_utils_calculate_nts(usf_bitmap);
  ...
  */
  ...
  else
  {
    ...
    if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_tx,assigned_egprs_ws) == FAIL_RESULT)
    {
...
/*cr134062*/
uint16 gmac_egprs_decode_dl_window_size(uint8 encoded_ws, uint8 dl_ts_alloc)
{
  uint16 assigned_egprs_ws = WS_ALLOCATION_INVALID;
  uint8 num_ts_allocated = 0;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  num_ts_allocated = gmac_utils_calculate_nts(dl_ts_alloc);
  ...
  */
  ...
  else
  {
    ...
    if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_rx,assigned_egprs_ws) == FAIL_RESULT)
    {
      assigned_egprs_ws = WS_ALLOCATION_INVALID;
...
#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
    /* Add space for Release 10 content (Low Access Priority Signalling) 
     * 3 bits: Release 5 present + G-RNTI extension & Iu mode indicator flag + HFN_LSB indicator flag
     * 2 bits: Release 6 present + Extended Channel Request Description indicator flag
     * 4 bits: Release 7 present + EARLY_TBF_ESTABLISHMENT + EGPRS BEP LQM Type 2 indicator flag + EGPRS Timeslot LQM Type 2 indicator flag
     * 2 bits: Release 10 present + Low Access Priority Signalling */
    prr_size = prr_size + 11;
#endif /* FEATURE_GERAN_IMPLICIT_PS_REJECT */
  }
  else
  {
...
void gmac_send_add_rac_info_msg( idle_mode_ul_tbf_release_action_t re_est_action)
{
  ...
  ***************************************/
  ...
  else
  {
    if(geran_get_nv_recovery_restart_enabled())
    {
      MSG_GERAN_ERROR_0_G("GMAC initiated Recovery Restart for encode Add RAC msg");
      geran_initiate_recovery_restart(GERAN_CLIENT_GMAC);
    }
    else
    {
...
===
===========================================================================*/
void gmac_retran_arac_msg(void)
{
  mac_l1_sig_t        ml_ph_data_sig;
  mac_ph_data_req_t   *data_req_msg = &(ml_ph_data_sig.mac_ph_data_req);
  gmac_util_data_t    *util_data_ptr = gmac_get_util_data_ptr();
...
static boolean supported_band_valid(boolean inhibit_pgsm, sys_band_T band)
{
  ...
  if ( rr_gapi_is_supported_band(band) )
  {
...
static sys_band_T convert_rac_band_to_rr_format(uint8 nw_band)
{
  ...
  switch ( nw_band )
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("GMAC cannot convert RR band: %d",nw_band);
      rr_band = SYS_BAND_NONE;
    break;
  }
                                       ...
                                       uint8 nw_band_list_size)
{
  ...
  rr_read_public_store(RRPS_MS_BCCH_BAND,nw_band_list_ptr);
...
static void check_for_bcch_inclusion( uint8 *nw_bands_count_ptr, uint8 *nw_bands_ptr)
{
  ...
  {
     rr_band = convert_rac_band_to_rr_format(nw_bands_ptr[i]);

     if ( rr_gapi_is_supported_band(rr_band) )
     {
  ...
  {
    rr_read_public_store(RRPS_MS_BCCH_BAND,nw_bands_ptr);
...
#endif /*PERLUTF*/
{
  ...
  if ( gsm_1800_supp_in_nw_list && gsm_1900_supp_in_nw_list )
  {
    ...
    rr_read_public_store(RRPS_MS_BCCH_BAND,&bcch_band);
    ...
    {
      rr_read_public_store(RRPS_NW_BAND_IND,&nw_band_indicator);
...
===
===========================================================================*/
static byte gmac_get_8psk_power_class(void)
{
  ...
  rr_read_public_store( RRPS_CELL_OPTIONS_EGPRS_CAPABLE_CELL, &egprs_supported );

  if (egprs_supported)
  {
     rr_read_public_store(RRPS_MS_EGPRS_8PSK_UL_ENABLED,&egprs_8psk_ul_supported);
     ...
     */
     ...
     {
       rr_read_public_store(RRPS_MS_EGPRS_8PSK_POWER_CAP,&egprs_8psk_power_capability);
...
static byte gmac_get_gpsk_power_class(byte access_tech)
{
  ...
  rr_band = convert_rac_band_to_rr_format(access_tech);
  rf_power_class = rr_gapi_get_band_rf_power_class((rr_internal_band_T)rr_band);
...
)
{
   ...
   {
     sys_band_T rr_band = convert_rac_band_to_rr_format(access_tech[i]);

     if ( supported_band_valid(inhibit_pgsm, rr_band) == TRUE )
     {
       num_supported_bands++;
     }
   }
   ...
   MSG_GERAN_LOW_3_G("RR formatting %d RAC's in Additional RAC format, avail size %d, rac length %d",
   ...
   for (i=0; i<acc_tech_num; i++)
   {
     ...
     if ( supported_band_valid(inhibit_pgsm, rr_band) == TRUE )
     {
       ...
       /*GSMK Power Class*/
       csn_encode_bits(encoder, 3, gmac_get_gpsk_power_class(access_tech[i]));

       /*8PSK Power Class*/
       csn_encode_bits(encoder, 2, gmac_get_8psk_power_class());
     }
   }
...
)
{
  ...
  if(arac_retran == FALSE)
  {
    ...
    **************************************************************************************/
    ...
    if (format_gprs_rac)
    {
      get_additional_rac_nw_list(util_data_ptr->nw_bands,&util_data_ptr->nw_bands_count,sizeof(util_data_ptr->nw_bands));

      modify_nw_list_for_1800_1900_band_combination_for_gprs( util_data_ptr->nw_bands_count, util_data_ptr->nw_bands );
    }
    else if(global_data_ptr->access_tech_req_present == TRUE) /* work out the NW list, number of bands requested */
    {
      ...
      /* if none of the bands requested by the NW are supported then include
         the BCCH band*/
      check_for_bcch_inclusion( &util_data_ptr->nw_bands_count,util_data_ptr->nw_bands );

      modify_nw_list_for_1800_1900_band_combination_for_egprs( util_data_ptr->nw_bands_count, util_data_ptr->nw_bands );
    }
    else
    {
      MSG_GERAN_LOW_0_G("MAC info, No bands req by NW, using BCCH band");
      /* No bands have been requested so use the BCCH band currently camped on*/
      rr_read_public_store(RRPS_MS_BCCH_BAND,util_data_ptr->nw_bands);
      util_data_ptr->nw_bands_count = 1;
    }

    if ( modify_nw_list_for_pgsm_inhibit( util_data_ptr->nw_bands_count, util_data_ptr->nw_bands ) )
    {
      MSG_GERAN_LOW_0_G("GMAC INFO, inhibiting PGSM in the nw_band list");
      inhibit_pgsm = TRUE;
    }

    /* work out free space avail in the PRR */
    max_prr_size_avail = 176 - gmac_calc_pkt_resource_req_size(TRUE,format_gprs_rac);
    ...
    **************************************************************************************/
    for ( i=0; i<util_data_ptr->nw_bands_count; i++ )
    {
      ...
      {
        rr_band = convert_rac_band_to_rr_format(util_data_ptr->nw_bands[i]);

        if ( supported_band_valid(inhibit_pgsm, rr_band) )
        {
          ...
          if (!prr_full)
          {
            ...
            truncated = gmac_util_set_access_tech_info(&prr_encoder,
                                                       util_data_ptr->nw_bands[i],
                                                       rr_gapi_get_band_rf_power_class((rr_internal_band_T)rr_band),
                                                       &hw_classmark,
                                                       &a5_already_included,
                                                       &gprs_msclass_already_included,
                                                       truncate_length );
            ...
            if ( util_data_ptr->prr_ie.ms_ra_cap_ie_len_bits > max_prr_size_avail )
            {
              ...
              MSG_GERAN_HIGH_2_G("GMAC RAC band:%d not included in PRR not enough space, ms_ra_cap len %d",
                util_data_ptr->nw_bands[i],
                util_data_ptr->prr_ie.ms_ra_cap_ie_len_bits);
      ...
      if ( ( prr_full ) || ( reformat_arac ) )
      {
        ...
        if ( supported_band_valid(inhibit_pgsm, rr_band) )
        {
          ...
          (void)gmac_util_set_access_tech_info(&arac_encoder,
                                               ...
                                               &a5_already_included,
                                               &gprs_msclass_already_included,
                                               0); /* truncation not allowed in ARAC */
      ...
      if(two_phase_access == TRUE)
      {
        if ( reformat_arac == FALSE )
        {
          ...
          else
          {
            ...
            gmac_send_rac_prr(NO_RE_EST,FALSE,two_phase_access);
...
===
===========================================================================*/
void gmac_reset_rac_params(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
                                                            ...
                                                            freq_params_t *csn_pua_freq_param_ptr)
{
  ...
  *********************************************************************************/
  ...
  {
    case arfcn_valid:
      return_value = gmac_util_freq_arfcn_valid(csn_pua_freq_param_ptr,ml_freq_config_info_ptr);
      break;
...
===========================================================================*/
static void gmac_csn_data_ind_to_grr(csn_decoder_ptrs_t *csn_decoder_ptr, ARFCN_T source_arfcn)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
{
  gmac_csn_data_ind_to_grr(csn_result_ptr, arfcn);
...
===========================================================================*/
static void gmac_util_init_container(cell_data_t *cell_data_ptr)
{
  uint8 i;
...
static void gmac_util_extract_msg_types(cell_data_t *cell_data_ptr)
{
  ...
  if (msg_type_ptr)
  {
    ...
    for (segment = 0; segment < num_msgs; segment++, msg_type_ptr++)
    {
      ...
      msg_inst = gmac_util_extract_psi_index(msg_type, msg_inst);
...
static void gmac_util_forward_packet_si_data(uint8 container_id, cell_data_t *cell_data_ptr, msg_type_t *msg_type_ptr)
{
  ...
  if (num_octets)
  {
    ...
    {
      csn_data = gcsn1_decoder(si_buffer, &dummy, sizeof(si_buffer));
    }
    else
    {
      csn_data = gcsn1_decoder(si_buffer, &dummy, (num_octets + 2));
    }
    ...
    {
      gmac_nacc_ncell_data_ind_to_grr(&csn_data, container_id, BCCH_PD);
    }
  }
...
static void gmac_util_check_for_new_messages_and_dispatch(uint8 container_id, cell_data_t *cell_data_ptr)
{
   ...
   */
  new_msg_list = gmac_util_determine_new_msgs(&(cell_data_ptr->si_container), &count);
  if (new_msg_list)
  {
   ...
   */
  new_msg_list = gmac_util_determine_new_msgs(&(cell_data_ptr->psi_container), &count);
  if (new_msg_list)
  {
    uint16 num_bits = 0;
    uint16 num_octets;
    ...
    for (index = 0; index < count; index++, msg_type_ptr++)
    {
      ...
      if (num_octets)
      {
        ...
        {
          csn_data = gcsn1_decoder(psi_octets, &num_bits, sizeof(psi_octets));
        }
        else
        {
          csn_data = gcsn1_decoder(psi_octets, &num_bits,num_octets);
        }
        ...
        {
          gmac_nacc_ncell_data_ind_to_grr(&csn_data, container_id, PBCCH_PD);
        }
      }
    }
    GPRS_MEM_FREE(new_msg_list);
...
static void gmac_util_trigger_cell_data(uint8 container_id, boolean bReleaseAfter)
{
  ...
  if (cell_data_ptr)
  {
    ...
    gmac_util_extract_msg_types(cell_data_ptr);

    /* check for new messages since last call to the trigger, and dispatch them to GRR */
    gmac_util_check_for_new_messages_and_dispatch(container_id, cell_data_ptr);
  }

  if (bReleaseAfter)
  {
    gmac_util_release_container(container_id);
  }
                                              ...
                                              packet_ncell_data_t *pncd_ptr)
{
      ...
      (pncd_ptr->arfcn_bsic_included             == TRUE))
  {
    ...
    if (ncell_info_ptr->cell_data.container_valid == FALSE)
    {
      ...
      gmac_util_release_container(pncd_ptr->container_id);
    }
  }
                                                      ...
                                                      boolean *release_ptr)
{
  ...
  gmac_validate_container_sequence(&(ncell_info_ptr->cell_data), container_id, container_index);
  gmac_nacc_validate_arfcn_and_bsic(ncell_info_ptr, pncd_ptr);
  ...
  instance_ptr = gmac_nacc_extract_pncd_instance_data(pncd_ptr);
  ...
  if (instance_ptr)
  {
    ...
    if (ncell_info_ptr->cell_data.instances_rcvd & 0x00000001)
    {
      ...
      msgs_built = gmac_util_build_container(&(ncell_info_ptr->cell_data), release_ptr);
    }
  ...
  if ((was_empty == TRUE) && (msgs_built == 0))
  {
    ...
    gmac_nacc_ncell_data_ind_to_grr(NULL, container_id, DUMMY_PD);
  }
                                                      ...
                                                      boolean *release_ptr)
{
  ...
  gmac_validate_container_sequence(&util_data_ptr->scell_info, SERVING_CELL_CONTAINER, container_index);
  ...
  /* extract the container repetition structure from the incoming message */
  instance_ptr = gmac_util_extract_pscd_instance_data(pscd_ptr);
  ...
  if (instance_ptr)
  {
    ...
    if (util_data_ptr->scell_info.instances_rcvd & 0x00000001)
    {
      /* once the first instance is available,   */
      /* try and build the container data stream */
      msgs_built = gmac_util_build_container(&util_data_ptr->scell_info, release_ptr);
    }
  }
  else
  {
...
===========================================================================*/
void gmac_util_init_cell_data(void)
{
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
  ...
  for (i = 0; i < MAX_NUMBER_OF_NACC_NCELLS; i++, ncell_info_ptr++)
  {
    ...
    gmac_util_init_container(&(ncell_info_ptr->cell_data));
  }

  /* initialise the serving cell container */
  gmac_util_init_container(&util_data_ptr->scell_info);
...
void gmac_util_release_container(uint8 container_id)
{
  ...
  if (cell_data_ptr)
  {
    ...
    gmac_util_free_instance_ptrs(cell_data_ptr);

    /* free container bytestream, segment and msg type pointers */
    gmac_util_free_container_ptrs(cell_data_ptr);

    /* re-initialise the cell data contents */
    gmac_util_init_container(cell_data_ptr);
  }
...
void gmac_nacc_respond_to_release_req(mac_signals_u *signal_ptr)
{
  ...
  if (release_req_ptr->release_flags == NACC_RELEASE_ONE)
  {
    ...
    gmac_util_release_container(release_req_ptr->container_id);
  }
  else
  {
    /* GRR has requested that MAC now releases all NCELL containers */
    gmac_nacc_release_all_containers();
  }
                                    ...
                                    ARFCN_T bcch_arfcn)
{
  ...
  if (gmac_util_check_tfi(msg_type))
  {
  ...
  if (trigger_msg)
  {
    ...
    gmac_util_trigger_cell_data(container_id, bRelease);
  }
...
===========================================================================*/
boolean gmac_util_check_tfi(uint8 message_type)
{
  boolean addresses_ms = FALSE;
  ...
  if (message_type == PACKET_SERVING_CELL_DATA)
  {
    ...
    addresses_ms = gmac_util_global_tfi(pscd_ptr->global_tfi);
  }
  else if (message_type == PACKET_CELL_CHANGE_CONTINUE)
  {
    ...
    addresses_ms = gmac_util_global_tfi(pccc_ptr->global_tfi);
  }
  else if (message_type == PACKET_NEIGHBOUR_CELL_DATA)
  {
    ...
    addresses_ms = gmac_util_global_tfi(pncd_ptr->global_tfi);
  }
...
void gmac_send_mac_l1_freq_config(mac_l1_freq_config_t *sig_data_ptr)
{
  ...
  gprs_l1_mac_acc_func(&global_data_ptr->l1_sig);
...
static boolean gmac_util_dtm_ms_class_check( gm_dtm_channel_assign_req_t *dtm_ass_ptr)
{
  ...
  if ( gmac_util_get_eda_from_dtm_ass( dtm_ass_ptr, &local_eda ) == TRUE )
  {
    ...
    switch(dtm_ass_ptr->dtm_tbf_type)
    {
      case UL_TBF_PRESENT:
        ...
        {
          local_ul_ts_alloc = gmac_util_dtm_reverse_ts_bitmap(local_dyn_ptr->usf_bitmap);
        }
        else
        {
          local_ul_ts_alloc = gmac_util_dtm_reverse_ts_bitmap(local_dyn_ptr->gamma_bitmap);
        }
    ...
    if ( result == TRUE )
    {
      ...
      if ( gmac_util_multislot_class_check(local_ul_ts_alloc,
                                           ...
                                           TRUE, PSHO_UPDATE_PARAMS_NOT_REQUIRED ) == TRUE )
#else
      if ( gmac_util_multislot_class_check(local_ul_ts_alloc,
                                           local_dl_ts_alloc,
                                           local_eda,
...
)
{
  ...
  {
    /*Multi slot checking*/
    if ( gmac_util_dtm_ms_class_check( dtm_ass_ptr ) == FALSE )
    {
#ifdef FEATURE_GSM_EDTM
      if ( global_data_ptr->gmac_edtm_enabled )
      {
  ...
  if(dtm_ass_status == NO_DTM_ASS_ERROR)
  {
    dtm_ass_status = gmac_dtm_check_for_egprs_ass_clash(dtm_ass_ptr);
  ...
  if(dtm_ul_ass_ptr->allocation_type == GMAC_RR_PUA_DYNAMIC_ALLOCATION )
  {
    ...
    ****************************************************************************/
    ...
    if( dtm_ul_ass_ptr->egprs_flag == valid )
    {
      ...
      if ( gmac_util_egprs_mcs_valid(dtm_ul_ass_ptr->egprs_mcs_mode) == FALSE )
      {
    ...
    *******************************************************************************/
    ...
    gmac_dtm_ul_dyn_alloc_ie(dtm_ul_dyn_ptr,ul_tbf_ptr);
...
)
{
  ...
  ******************************************************************************/
  ...
  /*get TLLI */
  rr_read_public_store(RRPS_TLLI,&rrps_tlli);
...
***********************************************************************************/
     ...
     */
    else if ( (assigned_arfcn.num <= GMAC_EGSM_MAX_ARFCN_LOWER) ||
...
***********************************************************************************/
      ...
      */
      ...
      {
        error = TRUE;
        MSG_GERAN_ERROR_4_G("GMAC Freq Err Diff Bands in Assignment ARFCN %d, Band %d, Current ARFCN %d, Band %d",
          check_arfcn.num, check_arfcn.band, assigned_arfcn.num, assigned_arfcn.band);
      }
...
===
===========================================================================*/
void gmac_transfer_send_edtm_pcr_on_pacch_rrbp(uint8 ts, uint8 rrbp, uint32 fn)
{
  mac_l1_sig_t        ml_rrbp_sig;
...
===
===========================================================================*/
boolean gmac_util_check_pkt_cs_cmd_addressing(void)
{
  boolean addr_match = FALSE;
                                              ...
                                              boolean dl_only_present)
{
  ...
  switch(msg_type)
  {
    ...
    case PMR:
        ...
        {
            MSG_GERAN_LOW_1_G("Unknown report type: %d", global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_send_meas_rpt.msg_type);
        }
  ...
  if (csn_encoding != PASS)
    MSG_GERAN_MED_1_G("CSN.1 encode failure of %d", msg_type);
    ...
    case PCCN:
        ...
        else
        {
            if(global_data_ptr->gmac_local_pccn_params.pccn_pending == PCCN_PENDING_UL)
            {
                    ...
                    transfer_flags = gmac_tran_get_t_flags();
                           ...
                           uint32 linenumber)
{
  ...
  if (util_data_ptr->gmac_rlc_blocked_flag == BLOCKED)
  {
    ...
    gmac_send_mr_ps_access_req(action);
  }

  MSG_GERAN_MED_4_G("GMAC rlc_blocked_flag = %d, action: %d, gmac file %d line: %d",
    util_data_ptr->gmac_rlc_blocked_flag, action, filename, linenumber);
}
                         ...
                         uint32             linenumber)
{
  ...
  MSG_GERAN_MED_4_G("GMAC block RLC flag %d, send MR_NO_PS_ACCESS_IND, cause %d, file %d, line: %d",
    util_data_ptr->gmac_rlc_blocked_flag, delete_pdu_cause, filename,linenumber);

  gmac_send_mr_no_ps_access_ind(delete_pdu_cause);
}
...
static void gmac_send_mr_no_ps_access_ind(delete_pdu_cause_t delete_pdu_cause)
{
  ...
  if (mac_rlc_sig_ptr != NULL)
  {
    MSG_GERAN_MED_1_G("GMAC Sent MR_NO_PS_ACCESS_IND, del pdu: %d", delete_pdu_cause);
...
===
===========================================================================*/
void gmac_send_mac_grr_ps_access_cnf(void)
{
  mac_grr_sig_t grr_sig;
...
===
===========================================================================*/
gmac_pcco_state_t gmac_util_get_pcco_state(void)
{
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
...
===
===========================================================================*/
gmac_pccf_state_t gmac_util_get_pccf_state(void)
{
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
...
===
===========================================================================*/
void gmac_util_initialise_page_mode(void)
{
  gmac_util_data_t *util_data_ptr = gmac_get_util_data_ptr();
...
===
===========================================================================*/
void gmac_util_page_mode_handler(void)
{
  ...
  if (global_data_ptr->msg_ptr->l1_mac_sig.msg.data_ind.gprs_l1_page_group_flag == TRUE)
  {
     ...
     (util_data_ptr->gmac_current_page_mode != global_data_ptr->csn_dec_hdr_ptr->page_mode))
    {
      ...
      gprs_l1_mac_acc_func(&send_mac_l1_sig);
    }
  }
}
#ifdef FEATURE_GPRS_PS_HANDOVER
...
===
===========================================================================*/
void gmac_util_send_grlcul_psho_start_ind(void)
{
  mac_rlc_msg_t  *mac_rlc_sig_ptr;
...
===
===========================================================================*/
void gmac_util_send_grlcdl_psho_start_ind(void)
{
  mac_rlc_msg_t  *mac_rlc_sig_ptr;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  mac_rlc_sig_ptr = grlc_get_cmd_buf_mac();
                                                  ...
                                                  uint8                          *ts_alloc)
{
  ...
  *ts_alloc = 0;
  ...
  if ( gmac_utils_calculate_nts(timeslot_description_ptr->ms_timeslot_allocation)
                                          ...
                                          mac_l1_psho_req_t              *psho_req_ptr)
{
  ...
  if ( gmac_get_ul_ts_alloc_from_psho_req(ps_handover_resources_ptr, &ul_ts_alloc) == FALSE )
  {
                                                                ...
                                                                boolean                        *control_ack_ptr)
{
  ...
  if (gmac_get_usf_from_psho_req(ps_handover_resources_ptr,
                                 &(mac_l1_msg.mac_l1_psho_req)) == FALSE )
  {
  ...
  if ( gmac_util_multislot_class_check(ul_ts_alloc,
                                       ...
                                       local_eda,
                                       0,
                                       FALSE, PSHO_UPDATE_PARAMS_REQUIRED ) == FALSE )
  {
  ...
  if(global_data_ptr->gmac_ps_ho.psho_control_params.psho_tbf_type == PSHO_UL_DL_TBF)
  {
    ...
    if(ps_handover_resources_ptr->type_of_mode == PSHO_TBF_MODE_EGPRS)
    {
          ...
          ||(global_data_ptr->gmac_ps_ho.psho_control_params.psho_initiated_tran_substate == TRANSFER_INACTIVE))
      {
        ...
        if ( gmac_get_dl_ws_from_psho_req(ps_handover_resources_ptr, &dl_egprs_ws) == FALSE )
        {
        ...
        if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_rx,decoded_dl_ws) == FAIL_RESULT)
          {
      ...
      else
      {
        ...
        if ( gmac_get_dl_ws_from_psho_req(ps_handover_resources_ptr, &dl_egprs_ws) == TRUE )
        {
  ...
  if(ps_handover_resources_ptr->type_of_mode == PSHO_TBF_MODE_EGPRS)
  {
        ...
        ||(global_data_ptr->gmac_ps_ho.psho_control_params.psho_initiated_tran_substate == TRANSFER_INACTIVE))
    {
      ...
      if ( gmac_get_ul_ws_from_psho_req(ps_handover_resources_ptr, &ul_egprs_ws) == FALSE )
      {
      ...
      if(gmac_check_assigned_egprs_ws(global_data_ptr->gmac_egprs_ms_class_max_ts_tx,decoded_ul_ws) == FAIL_RESULT)
        {
    ...
    else
    {
      /*network commanded no reset of RLC or there is an active ul tbf
        in the mobile. if there is no WS in the assignment then use the
        existing ws otherwise validate the given WS from the assignment*/
      if ( gmac_get_ul_ws_from_psho_req(ps_handover_resources_ptr, &ul_egprs_ws) == TRUE )
      {
    ...
    if (gmac_get_egprs_mcs_from_psho_req(ps_handover_resources_ptr,&coding_scheme) == FALSE)
    {
  ...
  {
    /* UL GPRS CS checking */
    if (gmac_get_cs_from_psho_req(ps_handover_resources_ptr,&coding_scheme) == FALSE)
    {
                                                         ...
                                                         mac_l1_psho_req_t              *psho_req_ptr )
{
       ...
       ( psho_req_ptr->psho_tbf_type == PSHO_UL_TBF))
  {
    ...
    (void)gmac_get_ul_ts_alloc_from_psho_req(ps_handover_resources_ptr, &ul_ts_alloc);
    ...
    {
      /* return already checked in validate funciton*/
      (void)gmac_get_cs_from_psho_req(ps_handover_resources_ptr,&coding_scheme);
    ...
    /**** num_timeslots ****/
    global_data_ptr->gmac_ps_ho.psho_config_params.psho_ul_alloc_info.num_timeslots = gmac_utils_calculate_nts(ul_ts_alloc);
  ...
  if(psho_req_ptr->psho_tbf_type == PSHO_UL_DL_TBF)
  {
    ...
    /**** tlli****/
    rr_read_public_store(RRPS_TLLI,&(global_data_ptr->gmac_ps_ho.psho_config_params.psho_dl_ass_info.tlli));


    MSG_GERAN_MED_2_G("ps HO  map dl ts = 0x%x ,  num dl ts %d ",
                      global_data_ptr->gmac_local_dl_ts_alloc,
                      gmac_utils_calculate_nts(global_data_ptr->gmac_local_dl_ts_alloc));
    global_data_ptr->gmac_ps_ho.psho_config_params.psho_dl_ass_info.num_dl_ts = gmac_utils_calculate_nts(global_data_ptr->gmac_local_dl_ts_alloc);
...
boolean gmac_util_ps_ho_req_handler(gprs_mac_transfer_substates_t mac_transfer_substate)
{
  ...
  if(global_data_ptr->msg_ptr->grr_mac_sig.msg.gm_psho_req.psho_type == PSHO_GTOG)
  {
    if(ps_handover_ptr->target_type == CSN_PSHO_GSM_TARGET_CELL)
    {
      ...
      gmac_util_initialise_psho_structure();
    ...
    #error code not present
      ...
      gmac_pkt_ps_ho_result = gmac_util_psho_validate_psho_req(&(a_gb_ptr->ps_handover_radio_resources),&ctrl_ack_set);
      ...
      if ( gmac_pkt_ps_ho_result == PSHO_VALIDATION_PASS )
      {
        ...
        gmac_util_psho_populate_psho_req_config_msgs(&(a_gb_ptr->ps_handover_radio_resources),psho_req_ptr);
        ...
        if(ctrl_ack_set == TRUE)
        {
          ...
          gmac_util_send_mac_l1_release(DL_TBF_NORMAL);
        ...
        rr_read_public_store(RRPS_CELL_OPTIONS_ACCESS_BURST_TYPE,&mac_l1_msg.mac_l1_psho_req.psho_access_burst_type);
        ...
        gprs_l1_mac_acc_func(&mac_l1_msg);
#ifdef PERLUTF
        #error code not present
#endif
        return TRUE;
...
===
===========================================================================*/
void gmac_util_initialise_psho_structure(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
===
===========================================================================*/
void gmac_util_reset_psho_structure(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
===
===========================================================================*/
mac_header_tfi_t gmac_util_mac_control_header_pkt_phy_info_tfi_checking(void)
{
  mac_header_tfi_t match_found = MAC_HEADER_TFI_NOT_VALID;
  uint8 dl_ctrl_block_tfi = 0;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
boolean gmac_util_check_pkt_phy_info_global_tfi(global_tfi_t rxd_global_tfi)
{
  ...
  mac_header_tfi_check = gmac_util_mac_control_header_pkt_phy_info_tfi_checking();
  if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
  {
    if (rxd_global_tfi.direction == uplink_valid)
    {
...
===
===========================================================================*/
void gmac_util_send_grr_mac_rach_rej(void)
{
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
...
===
===========================================================================*/
uint8 gmac_access_func_get_num_dl_timeslots (void)
{
  uint8 num_ts = 0;

  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();

  num_ts = gmac_utils_calculate_nts(global_data_ptr->gmac_local_dl_ts_alloc);
...
boolean gmac_util_egprs_mcs_valid(uint8 egprs_mcs)
{
  ...
  rr_read_public_store( RRPS_CELL_OPTIONS_EGPRS_CAPABLE_CELL, &egprs_capable_cell );
  rr_read_public_store( RRPS_MS_EGPRS_8PSK_UL_ENABLED, &eight_psk_ul_capable);
                                  ...
                                  )
{
  ...
  geran_eng_mode_data_write( ENG_MODE_TBF_INFO,(void*)&tbf_info,TRUE);
}
  ...
  **************************************************************/
   ...
   **************************************************************/
   ...
   if(start_time_elapsed == FALSE)
   {
     ...
     grlc_ul_restart_await_ass_guard( time_in_ms);
   }
   else
   {
...
/*===========================================================================
===
===  FUNCTION      gmac_util_send_mac_l1_ext_tbf_start_ind
===
===  DESCRIPTION
===
===  These changes are for adding PEO support for EDRX in GERAN. GMAC
===  stores last FN when PUAN was received. When GRLC detects that EXT_UL_TBF
===  period has started, GRLC sends RM_UL_EXT_TBF_START_IND signal to GMAC
===  and GMAC further sends the stored PUAN FN to GL1 under signal 
===  MAC_L1_EXT_TBF_START_IND
===
===  DEPENDENCIES
===
===  RETURN VALUE
...
===
===========================================================================*/
void gmac_util_send_mac_l1_ext_tbf_start_ind(uint32 frame_no )
{
  mac_l1_sig_t   send_mac_l1_sig;

  MSG_GERAN_MED_1_G("MAC Sent MAC_L1_EXT_TBF_START_IND FN: %d", frame_no);
  send_mac_l1_sig.mac_l1_ext_tbf_start_ind.message_header.message_id  = (uint8)MAC_L1_EXT_TBF_START_IND;
  send_mac_l1_sig.mac_l1_ext_tbf_start_ind.message_header.message_set = MS_MAC_L1;
  send_mac_l1_sig.mac_l1_ext_tbf_start_ind.FN                         = frame_no;

  gprs_l1_mac_acc_func(&send_mac_l1_sig);
} /* gmac_util_send_mac_l1_ext_tbf_start_ind */
#endif /* FEATURE_GERAN_POWER_EFFICIENT_OPERATION */
