...
$Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmaclog.c#4 $
...
===========================================================================*/
static void mac_log_packet_init(gsmdiag_pkt_info_t *info_ptr, log_code_type pkt_id, unsigned int pkt_size)
{
  if (info_ptr != NULL)
  {
...
void _mac_log_init(void)
{
  ...
  {
      DEBUGMSG( "_mac_log_init> already initialised", 0,0,0 );
      return;
  }
  else
  {
    ...
    DEBUGMSG( "_mac_log_init> register log-on-demand", 0,0,0 );
    (void)log_on_demand_register(LOG_GPRS_MAC_STATE_C, mac_log_on_demand);

    DEBUGMSG( "_mac_log_init> finished", 0,0,0 );
  }
}

/*===========================================================================

FUNCTION  _mac_log_deinit

DESCRIPTION
  This function de-initializes the log_on_demand packets.
...
void _mac_log_deinit(void)
{
  if (mac_log_initialized)
  {
    ...
    DEBUGMSG( "_mac_log_deinit> de-register log-on-demand", 0,0,0 );
    (void)log_on_demand_unregister( LOG_GPRS_MAC_STATE_C);
    mac_log_initialized = FALSE;

    DEBUGMSG( "_mac_log_deinit> finished", 0,0,0 );
  }
}

/*===========================================================================

FUNCTION  mac_log_on_demand_dyn
...
===========================================================================*/
log_on_demand_status_enum_type mac_log_on_demand_dyn(log_code_type log_code)
{
...
boolean _mac_log_state(void)
{
  ...
  mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_STATE_C, sizeof(LOG_GPRS_MAC_STATE_C_type));
  ...
  if (log_ptr == NULL)
  {
    DEBUGMSG( "_mac_log_state> log_alloc failed", 0,0,0 );
  }
  else
  {
    log_ptr->mac_mode               = (byte) global_data_ptr->mac_mode;
    log_ptr->mac_null_fsm_state     = (byte) gmac_null_get_null_state();
    log_ptr->mac_idle_fsm_state     = (byte) gmac_idle_get_idle_state();
    log_ptr->mac_transfer_fsm_state = (byte) gmac_tran_get_transfer_state();

    gsmdiag_log_commit(&pkt_info);

    DEBUGMSG( "_mac_log_state> packet logged", 0,0,0 );
  }
                                   ...
                                   const byte *message_ptr)
{
  ...
  if (message_ptr == NULL)
  {
    ...
    mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_SIGNALLING_MESSAGE_C, sizeof(LOG_GPRS_MAC_SIGNALLING_MESSAGE_C_type) - 1);
  ...
  else
  {
    /* Log an event if we have a payload */
      ...
      payload[0] = geran_get_diag_sub_id(); /* Subscription Id */
    ...
    mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_SIGNALLING_MESSAGE_C, (sizeof(LOG_GPRS_MAC_SIGNALLING_MESSAGE_C_type) - 1) + message_length);
...
FUNCTION  _mac_log_camped_on_cell
...
#define GPRS_TBF 0 //for keeping GPRS one compatable with both old and new.
    ...
    mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_UL_TBF_ESTABLISH_C, sizeof(LOG_GPRS_MAC_UL_TBF_ESTABLISH_C_type));
...
FUNCTION  _mac_log_ul_tbf_release
...
)
{
    ...
    mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_UL_TBF_RELEASE_C, sizeof(LOG_GPRS_MAC_UL_TBF_RELEASE_C_type));
...
)
{
    ...
    mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_DL_TBF_ESTABLISH_C, sizeof(LOG_GPRS_MAC_DL_TBF_ESTABLISH_C_type));
...
FUNCTION  _mac_log_dl_tbf_release
 ...
 )
{
  ...
  mac_log_packet_init(&pkt_info, LOG_GPRS_MAC_DL_TBF_RELEASE_C, sizeof(LOG_GPRS_MAC_DL_TBF_RELEASE_C_type));
 ...
 )
{
  ...
  mac_log_packet_init(&pkt_info, LOG_EGPRS_MAC_DL_ACKNACK_C, sizeof(LOG_EGPRS_MAC_DL_ACKNACK_C_type) + acknack_byte_len-1);
...
)
{
  ...
  mac_log_packet_init(&pkt_info, LOG_EGPRS_MAC_UL_ACKNACK_C, sizeof(LOG_EGPRS_MAC_UL_ACKNACK_C_type) + acknack_byte_len-1);
...
)
{
  ...
  mac_log_ul_acknack_with_byte_len(acknack_byte_len, ul_acknak);
...
FUNCTION  _mac_log_gprs_pua_event
...
void _mac_log_gprs_pua_event( uint8 ts_alloc, uint8 cs )
{
  ...
  pua_event[0] = geran_get_diag_sub_id();
...
FUNCTION  _mac_log_gprs_pda_event
...
void _mac_log_gprs_pda_event( uint8 ts_alloc, uint8 tfi )
{
  ...
  pda_event[0] = geran_get_diag_sub_id();
...
FUNCTION  _mac_log_packet_tbf_rel_event
...
void _mac_log_ptr_event(uint8 ul_ts, uint8 dl_ts, uint8 ul_tfi, uint8 dl_tfi)
{
  ...
  ptr_event[0] = geran_get_diag_sub_id();
  ptr_event[1] = ul_ts;
  ptr_event[2] = dl_ts;
  ptr_event[3] = ul_tfi;
  ptr_event[4] = dl_tfi;
