...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gmac/src/gmacidleutil.c#5 $
***
...
***
*****************************************************************************/
gmac_idleutil_data_t gmac_idleutil_data;
...
***
*****************************************************************************/
static pua_data_error_t gmac_tbf_starting_ie(start_frame_number_t *start_time_ptr, uint32  *des_signal_data_ptr);
static boolean  gmac_1phase_contention_res(void);
static pua_data_error_t pua_dynamic_handler(mr_ul_alloc_cnf_t *mr_ul_alloc_cnf_sig_ptr);
...
static boolean gmac_pua_tfi_info(mr_ul_alloc_cnf_t *mr_ul_alloc_ptr);
...
static pua_data_error_t pua_mb_handler(gprs_mac_action_t current_mac_action);
...
===
===  DESCRIPTION
===  Returns a pointer to idleutil data space 
===
===  DEPENDENCIES
===
===
...
===
===========================================================================*/
static gmac_idleutil_data_t* gmac_get_idleutil_data_ptr(void)
{
  return &gmac_idleutil_data;
}
...
===
===========================================================================*/
void gmac_init_idle_util_data(void)
{
  gmac_idleutil_data_t *global_idleutil_ptr = gmac_get_idleutil_data_ptr();
...
===
===========================================================================*/
cont_res_success_t gmac_idle_puan_handler(void)
{
   /*csn decoded dl msg */
   ...
   puan_data_ptr = (packet_uplink_ack_nack_t *)global_data_ptr->csn_dec_data_ptr;
    ...
    /**********************         set up FAI         *********************************/
    MSG_GERAN_MED_2_G("MAC info PUAN type %d rcvd in %d", puan_data_ptr->egprs_puan, global_data_ptr->current_tbf_mode);
   ...
   fai_set = gmac_egprs_fai_set(puan_data_ptr);

   if(gmac_mac_control_header_tfi_checking() == MAC_HEADER_TFI_MATCH_NOT_FOUND)
   {
   ...
   else if(puan_data_ptr->uplink_tfi != global_data_ptr->gmac_local_tfi_store.ul_tfi)
   {
      ...
      contention_resolved = IGNORE_PUAN;
      MSG_GERAN_LOW_2_G("MAC Rcvd PUAN add err. puan tfi %d,ass tfi %d ",
   ...
   else
   {
     ...
     /*check for fai and s/p bit*/
      ...
      {
        if( gmac_1phase_contention_res() == TRUE)
        {
          gmac_stop_t3166();
          ...
          **************************************************************************************/
          MSG_GERAN_MED_2_G("MAC Sent MAC_L1_CONTENTION_RES, tbf_mode%d, fai%d",
            global_data_ptr->current_tbf_mode, fai_set);
...
/*          gmac_local_cs_setting = puan_data_ptr->chan_coding_cmd;*/
          ...
          ***************************************************************************************/
            ...
            || puan_data_ptr->pwr_ctrl_flag == valid)
          {
            ...
            if(gmac_update_local_ta_params(&(puan_data_ptr->pkt_timing_adv)) == TRUE)
            {

              /* TA Parameters, send to L1 if there is a change */
              if ((puan_data_ptr->pkt_timing_adv_flag == valid) &&
                  (gmac_util_handle_power_ctrl_TA_IE(puan_data_ptr->pkt_timing_adv,&(pwr_ctrl_msg_ptr->global_packet_ta))))
              {
            ...
            if(puan_data_ptr->pwr_ctrl_flag == valid)
            {
              ...
              gmac_power_control_params_ie(&(puan_data_ptr->pwr_ctrl_paramtrs), &(pwr_ctrl_msg_ptr->power_control));
            ...
            if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
            {
              ...
              if(fai_set == FALSE )
              {
                ...
                MSG_GERAN_LOW_0_G("MAC Sent MAC_L1_POWER_CONTROL");
          ...
          {
            MSG_GERAN_LOW_0_G( "Unexpected Reallocation during contention res");
          }
          ...
          if (puan_data_ptr->r99_additions_flag == valid)
          {
            ...
            {
              MSG_GERAN_LOW_0_G( "Unexpected R99 additions during contention res");
            }
            if(puan_data_ptr->tbf_est !=0)
            {
              MSG_GERAN_LOW_0_G( "Unexpected tbf_est during contention res");
            }
          }
      ...
      ************************************************************************/
      ...
      if((puan_data_ptr->egprs_puan == valid) && (global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS))
      {
        ...
        {
          MSG_GERAN_LOW_0_G("MAC info PUAN request for PRR retrans");
          gmac_retran_prr_msg();
        }
        else
        {
        ...
        {
          MSG_GERAN_LOW_0_G("MAC info PUAN request for ARAC retrans");
          gmac_retran_arac_msg();
        }
        else
        {
          MSG_GERAN_MED_2_G("MAC info arac_retran_req %d, egprs_arac_sent %d",
        ...
        if(global_data_ptr->gmac_local_tfi_store.ul_tfi == puan_data_ptr->uplink_tfi  )
        {
          ...
          else
          {
            ...
            ****************************************************************************/
            ...
            MSG_GERAN_LOW_0_G("MAC Rcvd PUAN without contention res TLLI field msg ignored");
          }
        }
        else
        {
                                       ...
                                       pda_address_check_t address_check )
{
    ...
    ************************************************************/
    ...
    {
      MSG_GERAN_LOW_0_G("PDA address already checked");
    }
    else
    {
      if (global_data_ptr->csn_dec_hdr_ptr->error_code != ADDRESS_INFO_ERROR)
      {
            ...
            if(mac_header_tfi_check == MAC_HEADER_TFI_NOT_VALID)
            {
                 if(pda_data_ptr->tlli_gtfi_pkt_ref_valid == global_tfi_valid)
                 {
                   ...
                   if (pda_data_ptr->global_tfi.direction == uplink_valid)
                   {
                           ...
                           && (global_data_ptr->gmac_local_tfi_store.ul_tfi_valid))
                      {
                        ...
                        MSG_GERAN_LOW_0_G("MAC Info  PDA, UL_TFI mismatch");
                   ...
                   {
                      MSG_GERAN_LOW_0_G("ERROR :- PDA incorrectly address MS");
                      return(ASSIGNMENT_ADDRESSING_ERROR);
                   }
                 }
                 else  if(pda_data_ptr->tlli_gtfi_pkt_ref_valid == tlli_valid)
                 {
                    ...
                    if(pda_data_ptr->tlli != global_data_ptr->gmac_store_ul_est_tlli)
                    {
                       ...
                       {
                         MSG_GERAN_LOW_0_G("MAC Info  PDA, TLLI mismatch");
                         return(ASSIGNMENT_ADDRESSING_ERROR);
                       }
                    }
                    else
        ...
        else
        {
          ...
          {
            MSG_GERAN_LOW_0_G("ERROR :- PDA incorrectly add by control header DL_TFI MS");
            return(ASSIGNMENT_ADDRESSING_ERROR);
          }
          else if (mac_header_tfi_check == MAC_HEADER_TFI_MATCH_NOT_FOUND)
          {
    ...
    ******************************************************************/
    if (global_data_ptr->csn_dec_hdr_ptr->error_code != MESSAGE_ESCAPE_ERROR)
    {
        ...
        *****************************************************************/
        ...
        {
          MSG_GERAN_LOW_0_G("GMAC PDA does not contain a timeslot allocation, ASSIGNMENT_DATA_ERROR");
          return(ASSIGNMENT_DATA_ERROR);
        }
        ...
        dl_ass_ind_sig_ptr->num_dl_ts = gmac_utils_calculate_nts(global_data_ptr->gmac_local_dl_ts_alloc);

        /*TLLI in DL CNF to RLC*/
        MSG_GERAN_HIGH_4_G("MAC loaded MR_DL_ASS_IND with tlli %x,L_tlli %x o_tlli %x, gmac_store_dl_ass_tlli %x",
          dl_ass_ind_sig_ptr->tlli,local_rrps_tlli,old_local_rrps_tlli, global_data_ptr->gmac_store_dl_ass_tlli);
        ...
        *****************************************************************/
        ...
        {
           MSG_GERAN_LOW_0_G("MAC error no DL TFI");
           return(ASSIGNMENT_DATA_ERROR);
        }
        ...
        ******************************************************************/
        ...
        if(pda_data_ptr->egprs_options_flag ==  valid)
        {
          ...
          *****************************************************************/
          ...
          {
             MSG_GERAN_LOW_2_G("MAC error in DL WS assignment ignored.DL ws bitmap 0x%x ts alloc 0x%x", 
        ...
        *****************************************************************/
        if(pda_data_ptr->freq_param_flag == valid )
        {
          ...
          **********************************************************************************/
            ...
            freq_check_result = gmac_util_frequency_param_ie(FALSE,send_frq_config);
        ...
        else
        {
           if(idle_data_ptr->i_flags.pda_ul_pacch == FALSE)
           {
              ...
              MSG_GERAN_LOW_0_G("MAC info, No frequency params, Ignore PDA");
...
static pua_data_error_t pua_dynamic_handler(mr_ul_alloc_cnf_t *mr_ul_alloc_cnf_sig_ptr)
{
  ...
  MSG_GERAN_MED_2_G("MAC info rcvd PUA type %d in tbf mode %d",
    global_data_ptr->current_tbf_mode, pua_data_ptr->egprs_pua);
  ...
  ***************************************************************/
  ...
  gmac_include_tlli_in_rlc_block(idle_data_ptr->access_granted, mr_ul_alloc_cnf_sig_ptr);
  ...
  ***************************************************************/
  if ((gmac_pua_tfi_info(mr_ul_alloc_cnf_sig_ptr)) == FALSE)
  {
  ...
  ***************************************************************/
  gmac_ul_rlc_tbf_type(idle_data_ptr->access_granted,mr_ul_alloc_cnf_sig_ptr);
 
  /***************************************************************
  *        calculate number of ts allocated for the ul tbf       *
  ***************************************************************/
  gmac_pua_rlc_nts(mr_ul_alloc_cnf_sig_ptr);
  ...
  *  usf ganularity                                              *
  ***************************************************************/
  gmac_global_dynamic_alloc_ie(ml_dynamic_tbf_info_ptr, pua_data_ptr);
  ...
  ****************************************************************/
  ...
  {
    MSG_GERAN_LOW_0_G("GMAC PUA contains no timeslot allocation, ASSIGNMENT_DATA_ERROR");
    return(ASSIGNMENT_DATA_ERROR);
  }
...
#endif /*FEATURE_GPRS_PS_HANDOVER*/
  {
     ...
     MSG_GERAN_LOW_2_G("MAC Info Error PUA multislot: rx_ts=%d, tx_ts=%d", ts_alloc_rx_bitmap ,ts_alloc_tx_bitmap);
     return(MULTISLOT_ERROR_ACCESS_RETRY);
  }
  ...
  ***************************************************************/
  ...
  /*update local "gmac_update_local_ta_params" with ta info */
  (void)gmac_update_local_ta_params(&(pua_data_ptr->pkt_timing_adv));
  ...
  ***************************************************************/
  ...
  if(pua_data_ptr->egprs_pua == valid)
  {
    ...
    ****************************************************************************/
    ...
    if (gmac_util_egprs_mcs_valid(pua_data_ptr->egprs_chan_coding_cmd) == TRUE)
    {
      mr_ul_alloc_cnf_sig_ptr->mcs = pua_data_ptr->egprs_chan_coding_cmd;
    }
    else
    {
      MSG_GERAN_LOW_0_G("GMAC PUA EGPRS MCS Invalid, Ignore Msg");
      return(ASSIGNMENT_DATA_ERROR);
    }
...
/*event logging changes*/
  mac_log_gprs_pua_event( global_data_ptr->gmac_local_ul_ts_alloc, pua_data_ptr->chan_coding_cmd );
...
static pua_data_error_t pua_sb_handler(gprs_mac_action_t current_mac_action )
{
  ...
  *******************************************************************/
  ...
  gmac_pua_dl_pwr_control(ml_sb_tbf_info_ptr);
...
/*event logging changes*/
  mac_log_gprs_pua_event( mac_log_convert_single_ts_to_bitmp( pua_data_ptr->sngl_blk_alloc.timeslot_num), pua_data_ptr->chan_coding_cmd );
...
static boolean gmac_pua_tfi_info(mr_ul_alloc_cnf_t *mr_ul_alloc_ptr)
{
  ...
  else if (pua_data_ptr->allocation_flag == dynamic_alloc_e)
  {
    ...
    {    
      ...
      if(idle_data_ptr->i_flags.idle_ul_realloc_state == IDLE_REALLOC_REQUESTED)
      {
          MSG_GERAN_LOW_0_G("GMAC info Idle UL re-alloc ");
        }
...
static void gmac_pua_rlc_nts( mr_ul_alloc_cnf_t *mr_ul_alloc_ptr)
{
   ...
   if(pua_data_ptr->allocation_flag == fixed_alloc )
   {
     ...
     {
       mr_ul_alloc_ptr->num_timeslots = gmac_utils_calculate_nts(pua_data_ptr->ts_allocation);
     }
     else
     {
...
===
===========================================================================*/
boolean gmac_dl_msg_data_valid(void)
{
  boolean error = FALSE;
  gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
  ...
  {
    error = TRUE;
    MSG_GERAN_ERROR_0_G("MAC Info CSN.1 non fatal msg data error");
  }
  return(error);
}
...
)
{
   ...
   if(l1_sig_id == MAC_L1_FIXED_TBF_CONFIG)
   {
      ...
      gprs_l1_mac_acc_func(l1_sig_ptr);
   }
                                   ...
                                   mac_log_dl_tbf_release_t dl_failure_reason)
{
   ...
   if(global_data_ptr->c_flags.tbf_release_pending_flg == FALSE)
   {
    ...
    {                     
       MSG_GERAN_LOW_0_G("GMAC info set idle_ul_realloc_state = IDLE_REALLOC_NO_ACTION");
      idle_data_ptr->i_flags.idle_ul_realloc_state = IDLE_REALLOC_NO_ACTION;
      idle_data_ptr->i_flags.pending_realloc_req = FALSE;
    }

     MSG_GERAN_MED_4_G("MAC sent MAC_L1_RELEASE type:%d, ul:%d, dl:%d, ul_realloc_state:%d",
       release_type, ul_failure_reason, dl_failure_reason, idle_data_ptr->i_flags.idle_ul_realloc_state);
    ...
    /*logging*/
    mac_log_packet_tbf_rel_event(release_type);

    if(release_type == UL_TBF_NORMAL || release_type == UL_TBF_ABNORMAL)
    {
       mac_log_ul_tbf_release(ul_failure_reason);
    }
    else if (release_type == DL_TBF_NORMAL || release_type == DL_TBF_ABNORMAL)
    {
       mac_log_dl_tbf_release(dl_failure_reason);
    }
    else
    {
  ...
  {
     MSG_GERAN_LOW_0_G( "MAC has already sent MAC_L1_RELEASE don't send subsequent");
  }
}
...
)
{
  ...
  switch(ul_tbf_type)
  {
   ...
   case dynamic_alloc_e:  tbf_assignment_result =  pua_dynamic_handler(mr_ul_alloc_cnf_sig_ptr);
   break;

   case single_blk_alloc:
        tbf_assignment_result =  pua_sb_handler(current_action);
   break;
   ...
   case multi_blk_allocation:
        ...
        MSG_GERAN_LOW_0_G("MAC info EGPRS 2p access initiated using EGPRS_PRACH");
        tbf_assignment_result =  pua_mb_handler(current_action);
   break;
...
===
===========================================================================*/
boolean gmac_idle_check_pcco_addressing(void)
{
...
void gmac_idle_send_pmr_cnf_to_grr(success_failure_t result)
{
  ...
  gprs_grr_mac_acc_func(&local_mac_grr_sig);
}
/*===========================================================================
===
...
)
{
  ...
  switch (current_mac_action)
  {
    case UL_DATA_EST :
         ...
         {
           gmac_util_complete_pcco_process(PCCO_FAILURE_NO_RESPONSE,MAC_INITIATED,GMAC_IDLEUTIL,__LINE__);
           gmac_util_complete_pccf_process(FAILED_TO_SEND_PCCF,MAC_INITIATED,GMAC_IDLEUTIL,__LINE__);
         }
...
pua_cont_res_t gmac_idle_egprs_pua_contention_res_checking(gprs_mac_idle_substates_t idle_substate)
{
  ...
  if(global_data_ptr->current_tbf_mode == TBF_MODE_EGPRS)
  {
    if(gmac_idle_check_pua_addressing(global_data_ptr->idle_mode_ul_tbf_release_action,idle_substate ) == TRUE)
    {
      ...
      if(pua_data_ptr->cont_res_tlli_flag == valid)
      {
        ...
        if (pua_data_ptr->cont_res_tlli == global_data_ptr->gmac_store_ul_est_tlli)
        {
          ...
          if (mr_sig_ptr != NULL)
          {
            ...
            **************************************************************************************/
            ...
            MSG_GERAN_LOW_0_G("MAC Sent MAC_L1_CONTENTION_RES");
            idle_data_ptr->mac_l1_sig.mac_l1_contention_res.message_header.message_id = (uint8)MAC_L1_CONTENTION_RES;
            idle_data_ptr->mac_l1_sig.mac_l1_contention_res.message_header.message_set = MS_MAC_L1;
            gprs_l1_mac_acc_func(&idle_data_ptr->mac_l1_sig);
        ...
        {
          MSG_GERAN_LOW_2_G("MAC info PUA contention TLLI mismatch. PUA TLLI:0x%x, MS TLLI:0x%x",
      ...
      {
        MSG_GERAN_LOW_0_G("MAC Rcvd PUA without contention res TLLI field");
        return_value = PUA_CONT_RES_NOT_APPLICABLE;
      }         
    }
    else
    {
      MSG_GERAN_LOW_0_G("MAC info PUA address checking failed");
    ...
    switch(return_value)
    {
      ...
      case PUA_CONT_RES_NOT_APPLICABLE:
        ...
        if (pua_data_ptr->arac_retran_req == 1)
        {
          ...
          {
            MSG_GERAN_LOW_0_G("GMAC info PUA request for ARAC retrans");
            gmac_retran_arac_msg();
          }
          else
          {
            MSG_GERAN_LOW_2_G("GMAC PUA req ARAC retran, ARAC not sent before, access_tech_req_present:%d, c_flags.egprs_arac_sent:%d ",
  ...
  ******************************************************************************/
  ...
  if(global_data_ptr->c_flags.egprs_arac_sent == FALSE)
  {
    ...
    if(pua_data_ptr->arac_retran_req == 0)
    {
      ...
      {
        MSG_GERAN_LOW_1_G("MAC Error in MB alloc,PUA ignored. Reserved blocks allocated %d",
    ...
    {
       MSG_GERAN_LOW_1_G("MAC Error: Incorrect req for ARAC retran during EGPRS 2p access.PUA ignored. arac_retran_req %d",
                          pua_data_ptr->arac_retran_req);
       return(ASSIGNMENT_DATA_ERROR);
    }
  }
  else
  {
    ...
    MSG_GERAN_LOW_0_G("MAC info NW did not RCVD ARAC. when ARAC was sent as a MB alloc for 2 blocks");
    ...
    else
    {
      if(pua_data_ptr->arac_retran_req == 0)
      {
        MSG_GERAN_LOW_1_G("ERROR in ARAC retrans request,PUA ignored. arac_retran_req %d ",pua_data_ptr->arac_retran_req);
      }
...
===
===========================================================================*/
void gmac_idle_send_no_access_to_rlc(void)
{
    gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
    gmac_idle_data_t *idle_data_ptr = gmac_get_idle_data_ptr();
    ...
    if(global_data_ptr->mac_action == UL_DATA_EST)
    {
      ...
      {
        MSG_GERAN_MED_1_G("MAC maximum number of access attempts = 4 and access_attempts_count = %d",
          idle_data_ptr->access_attempts_count);

        if(idle_data_ptr->access_attempts_count < 4)
        {
          idle_data_ptr->access_attempts_count ++;
          gmac_util_block_rlc(NO_DEL,GMAC_IDLEUTIL,__LINE__);
        }
        else
        {
          MSG_GERAN_LOW_0_G( "MAC has reached maximum number of access attempts so resetting access_attempts_count value is ");
          idle_data_ptr->access_attempts_count = 0;
          gmac_util_block_rlc(DEL_ONE,GMAC_IDLEUTIL,__LINE__);
        }
      }
      else
      {
...
{
  MSG_GERAN_MED_1_G("GMAC IDLE stop access timer in state %d", mac_idle_substate);
  switch(mac_idle_substate)
  {
      case UL_1P_ASS:
           MSG_GERAN_LOW_0_G( "GMAC is in UL_1P_ASS state so stopping T3164 timer ");
           gmac_stop_t3164();
      break;

      case UL_2P_ASS:
           MSG_GERAN_LOW_0_G( "GMAC is in UL_2P_ASS state so stopping T3168 timer ");
           gmac_stop_t3168();
      break;

      case UL_1P_CONTENTION_RES:
           MSG_GERAN_LOW_0_G( "GMAC is in UL_1P_CONTENTION_RES state so stopping T3164, T3166 and T3180 timers ");
           gmac_stop_t3164();
           gmac_stop_t3166();
           gmac_stop_t3180();
           gmac_stop_t3168();
      break;
      case DL_ASS:
           MSG_GERAN_LOW_0_G( "GMAC no need to stop any timers in DL_ASS state");
      break;

      default:
           MSG_GERAN_LOW_1_G( "NO action in state %d", mac_idle_substate);
      break;
  }
}
...
===
===========================================================================*/
void gmac_idle_send_del_one_or_no_del_to_rlc(void)
{
    gmac_global_data_t *global_data_ptr = gmac_get_global_data_ptr();
