...
*/
static void gsn_enter_recovery(  gsn_nsapi_e_t * );

static void gsn_ul_suspend_dataflow(  gsn_sapi_t, const uint8 );

static void gsn_clr_nsapi_buf( gsn_nsapi_t );

static void gsn_rel_nsapi_buf(   gsn_nsapi_t nsapi );

static void gsn_clr_sapi_buf(   gsn_sapi_t );

static void gsn_ul_prepare_retr(  gsn_nsapi_t );

static uint8  gsn_find_nsapi( const gsn_nsapi_q_t*, const gsn_nsapi_t * );

static void gsn_activate_nsapi(  gsnsm_prim_type, const void * );

static void gsn_activate_sapi(   gsn_sapi_t, uint32 );

static void gsn_map_nsapi_on_sapi(  gsn_nsapi_t, gsn_sapi_t );

static void gsn_init_sapi( gsn_sapi_e_t *sapi_ptr );

#if (defined(FEATURE_GSM_GPRS_SNDCP_PCOMP) || defined(FEATURE_GSM_GPRS_SNDCP_DCOMP))
static void gsn_pcomp_dcomp_validity_check( 
...
static void gsn_init_nsapi(         
                           gsn_nsapi_e_t   *nsapi_ptr,
                           gsnsm_prim_type  prim_type,
                           const void      *snsm_msg_ptr );

static void gsn_deactv_nsapi(   gsn_nsapi_e_t *nsapi_ptr );

static void gsn_deactv_sapi(   gsn_sapi_e_t *sapi_ptr );

static boolean gsn_release_sapi(  gsn_nsapi_t, gsn_sapi_t, boolean );

static void gsn_remove_nsapi_from_sapi(gsn_nsapi_t, gsn_sapi_t );

static boolean gsn_ack_nsapis_on_sapi(  gsn_nsapi_t, gsn_sapi_t );
...
static void gsn_process_pdp_dl_data(   gsn_nsapi_t, dsm_item_type **  );

static boolean gsn_dle_reas_ack_data
(
    gsn_nsapi_t, dsm_item_type **, const gsn_data_pdu_hdr_t *
);

static boolean gsn_dle_reas_unack_data
(
   gsn_nsapi_t, dsm_item_type **, const gsn_unitdata_pdu_hdr_t *
);
...
void gsn_unack_retr_npdu_hdlr(   gsn_nsapi_t nsapi );


/* LL primitive processing functions
*/
static void gsn_send_ll_est_res(  gsn_sapi_e_t *, dsm_item_type *);

static void gsn_send_ll_rel_req(  gsn_sapi_e_t *, boolean, boolean);

static void gsn_send_ll_xid_res(  gsn_sapi_e_t *, dsm_item_type *);

static void gsn_process_ll_rst_ind(  gsn_sapi_e_t *, ll_reset_ind_t  *);

static void gsn_process_ll_est_ind(  gsn_sapi_e_t *, ll_establish_ind_t  *);

static void gsn_process_ll_est_cnf(  gsn_sapi_e_t *, ll_establish_cnf_t  *);

static void gsn_process_ll_rel_ind(  gsn_sapi_e_t *, const ll_release_ind_t  *);

static void gsn_process_ll_rel_cnf(  gsn_sapi_e_t *, ll_release_cnf_t  *);

static void gsn_process_ll_xid_ind(  gsn_sapi_e_t *, ll_xid_ind_t  *);

static void gsn_process_ll_xid_cnf(  gsn_sapi_e_t *, ll_xid_cnf_t  *);

static void gsn_process_ll_stat_ind(  gsn_sapi_e_t *, const ll_status_ind_t  *);

static void gsn_process_ll_data_cnf(  gsn_sapi_e_t *, ll_data_cnf_t *);

void gsn_process_ll_udata_cnf(  gsn_sapi_e_t *, ll_unitdata_cnf_t *);


/* SNSM primitive processing functions
*/
static void gsn_process_snsm_actv_ind(  const gsnsm_activ_ind_type * );

static void gsn_process_snsm_deactv_ind(  const gsnsm_deactiv_ind_type * );

static void gsn_process_snsm_mod_ind(  const gsnsm_modify_ind_type * );

static void gsn_process_snsm_seq_ind(  const gsnsm_seq_ind_type * );

static void gsn_process_snsm_enter_psm_ind(  const gsnsm_enter_psm_ind_type * );

static void gsn_send_snsm_actv_res(  gsn_nsapi_t );

static void gsn_send_snsm_deactv_res(  gsn_nsapi_t );

static void gsn_send_snsm_mod_res(  gsn_nsapi_t );

static void gsn_send_snsm_seq_res(  gsn_nsapi_t, gsn_nsapi_mode_t );
...
===
===========================================================================*/
static void gsn_send_ll_activate_ind(   gsn_sapi_e_t *sapi_ptr, boolean psm_restore )
{
...
/*-------------------------------------------------------------------------------*/
  case ( IDLE ):
    ...
    ul_mes->mes.act_ind_mes.psm_restore = psm_restore;
    ...
    else
    {
      ...
      ul_mes->mes.act_ind_mes.radio_priority  = 1;
      ul_mes->mes.act_ind_mes.pfi             = 0;
      gsn_debug_print_invalid_sapi(sapi, GSN_GHDLR, __LINE__);
    }

    /* Post message to LLC and log for call flow analysis. */
    MSG_GERAN_LOW_1_G("GSN ""LL %d: Sent LL_ACT_IND",gsn_extern_sapi[sapi]);
...
===
===========================================================================*/
static void gsn_send_ll_deactivate_ind(   const gsn_sapi_e_t *sapi_ptr )
{
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
...
#ifndef FEATURE_GERAN_REDUCED_DEBUG
  /* Update diag */
  gsn_encode_state_diag_rpt(  (uint8) nsapi_ptr->nsapi );
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */
}
...
)
{
  ...
  nsapi_e_ptr = actv_nsapis[nsapi];
  if( nsapi_e_ptr == NULL )
  {
    gsn_debug_print_invalid_nsapi(nsapi, GSN_GHDLR, __LINE__);
    return( reas_comp );
  }
  ...
  case DL_RCV_1ST_SEQ:
    {
      ...
      if( !sn_hdr->f )
      {
        ...
        */
        ...
        /* Post message to LLC */
        gsn_send_ll_est_req(  nsapi_e_ptr->sapi_ptr, NULL, FALSE );
  ...
  default:
    ...
    MSG_GERAN_LOW_0_G("GSN ""Invalid sate during reassembly in Unack mode");
    break;
  }
  ...
  */
  ...
  case DL_RCV_SUBS_SEQ:
    ...
    */
        ...
        (nsapi_e_ptr->dle.prv_seg                       != sn_hdr->seg)    )
    {
      ...
      **  discard the SN-PDU.
      */
      MSG_GERAN_LOW_3_G("GSN ""NS %d: NPDU %d:%d out of SEQ",
    ...
    */
    if( sn_hdr->f )
    {
          ...
          (nsapi_e_ptr->dle.first_seg_info.pcomp != sn_hdr->pcomp)    )
      {
        ...
        MSG_GERAN_LOW_1_G("GSN ""NS %d: Info HDR dif from 1st seg", GSN_EXTERN_NSAPI( nsapi ));
        GPRS_PDU_FREE( sn_pdu );
        GPRS_PDU_FREE( &nsapi_e_ptr->dle.reas_buff );
  ...
  case DL_DISCARD:
    ...
    if( !sn_hdr->m )
    {
      ...
      /* Ignore SN-PDU */
      MSG_GERAN_LOW_1_G("GSN ""NS %d: Info Seg rcved in DISCARD state", GSN_EXTERN_NSAPI( nsapi ));
      GPRS_PDU_FREE( sn_pdu );
    }
    else if ( sn_hdr->f )
    {
...
)
{
  ...
  gsn_nsapi_e_t       *nsapi_ptr         = actv_nsapis[sn_hdr->nsapi];
  ...
  */
  npdu_ref_item_ptr = gsn_dle_is_npdu_ref_item_in_queue( 
                                                         nsapi_ptr,
                                                         sn_hdr->num
                                                       );
  ...
  if( npdu_ref_item_ptr )
  {
    ...
    */
    is_npdu_reordered = gsn_dle_add_npdu_seg_item( 
                                                   sn_pdu,
                                                   npdu_ref_item_ptr,
                                                   sn_hdr
                                                 );
  ...
  else
  {
    ...
    */
    npdu_ref_item_ptr = gsn_dle_add_npdu_ref_item(  nsapi_ptr, sn_hdr );
  ...
  if( is_npdu_reordered )
  {
    ...
    */
    gsn_dle_remove_npdu_ref_item (  nsapi_ptr, npdu_ref_item_ptr );
  }
  else
  {
...
)
{
  ...
  else
  {
    ...
    MSG_GERAN_MED_0_G("GSN ""Invalid NSAPI");
  ...
  else if ( npdu_ref_item_ptr->seg_reorder_buff_valid )
  {
    ...
    */
    ...
    if( q_cnt( &npdu_ref_item_ptr->seg_reorder_buff ) >= MAX_NPDU_SEG_ITEM_CNT )
    {
      ...
      /* Remove NPDU ref item from queue. */
      gsn_dle_remove_npdu_ref_item(  actv_nsapis[nsapi], npdu_ref_item_ptr );

      /* Free NPDU ref item. */
      gsn_dle_free_npdu_ref_item(  actv_nsapis[nsapi], npdu_ref_item_ptr );
    ...
    else
    {
      ...
      is_npdu_reordered =
        gsn_dle_is_npdu_reordered(  actv_nsapis[nsapi], npdu_ref_item_ptr );

    }
  }
  else
  {
    MSG_GERAN_LOW_0_G("GSN ""Seg_item_buff is invalid");
  }
...
)
{
    ...
    )
  {
    ...
    */
    if( q_cnt( &nsapi_ptr->dle.npdu_ref_buff ) >= MAX_NPDU_REF_ITEM_CNT )
    {
      ...
      /* Free NPDU ref item. */
      gsn_dle_free_npdu_ref_item(  nsapi_ptr, cur_npdu_ref_item_ptr );
    }
  ...
  {
    MSG_GERAN_LOW_1_G("GSN ""NS %d: npdu_ref_buff is invalid",
...
)
{
  ...
  if( ( nsapi_ptr == NULL ) || ( npdu_ref_item_ptr == NULL ) )
  {
    ...
    MSG_GERAN_MED_0_G("GSN "" NSAPI ptr and/or NPDU ref item ptr invalid ");
  ...
  else if( nsapi_ptr->dle.npdu_ref_buff_valid == TRUE )
  {
        ...
        ( q_cnt( &nsapi_ptr->dle.npdu_ref_buff )  == 0    )    )
    {
      ...
      MSG_GERAN_LOW_1_G("GSN ""NS %d: No NPDU ref items in queue ",
        GSN_EXTERN_NSAPI( nsapi_ptr->nsapi ));
...
)
{
  ...
  if( ( nsapi_ptr == NULL ) || ( npdu_ref_item_ptr == NULL ) )
  {
    ...
    MSG_GERAN_MED_0_G("GSN ""NSAPI ptr and/or NPDU ref item ptr is NULL ");
  ...
  else
  {
    ...
    MSG_GERAN_LOW_1_G("GSN ""NS: %d: NPDU seg reorder buff invalid ",
                    GSN_EXTERN_NSAPI( nsapi_ptr->nsapi ));
                                   ...
                                   gsn_npdu_ref_item_t *npdu_ref_item_ptr )
{
           ...
           ( q_cnt( &npdu_ref_item_ptr->seg_reorder_buff ) > 15 ) )
  {
    ...
    */
    ...
    MSG_GERAN_LOW_2_G("GSN ""NS %: Invalid number of seg items, free NPDU ref item %d",
      GSN_EXTERN_NSAPI( nsapi_ptr->nsapi ), npdu_ref_item_ptr->npdu_num);

    gsn_dle_remove_npdu_ref_item(  nsapi_ptr, npdu_ref_item_ptr );

    gsn_dle_free_npdu_ref_item(  nsapi_ptr, npdu_ref_item_ptr );
...
/* This function suspends the Up-Link PDP data flow and sets the indicated event */
void gsn_set_suspend_event(gsn_nsapi_t nsapi, uint8 event )
{
  if(GSN_VALID_NSAPI(GSN_EXTERN_NSAPI(nsapi)) && (actv_nsapis[nsapi] != NULL))
  {
    ...
    GSN_LOCK();
    actv_nsapis[nsapi]->ule.susp_flag |= event;
    GSN_UNLOCK();
    if( actv_nsapis[nsapi]->pdp_ul_suspend_fnc )
    {
      actv_nsapis[nsapi]->pdp_ul_suspend_fnc(
        NULL, actv_nsapis[nsapi]->ds_context);
    }
...
/* This function clears the indicated event. If all events are cleared
...
void gsn_clr_suspend_event(gsn_nsapi_t nsapi, uint8 event )
{
  if(GSN_VALID_NSAPI(GSN_EXTERN_NSAPI(nsapi)) && (actv_nsapis[nsapi] != NULL))
  {
    MSG_GERAN_LOW_3_G("GSN NS %d: ENTER clr_susp: s_event=%x susp_flag=%x",
      GSN_EXTERN_NSAPI(nsapi), (event), actv_nsapis[nsapi]->ule.susp_flag);

    GSN_LOCK();
    actv_nsapis[nsapi]->ule.susp_flag &= ~event;
    suspend_flag = actv_nsapis[nsapi]->ule.susp_flag;
    GSN_UNLOCK();
    ...
    if( suspend_flag )
    {
          ...
          (!(suspend_flag & GSN_UL_SUSP_MAX_NPDU_MASK)) )
      {
        ...
        ** is set, start retransmiting N-PDU. */
        if( actv_nsapis[nsapi]->ule.retr )
          gsn_send_retr_req(nsapi);
      }
    }
    else
    {
      ...
      ** then this can now be called prior to resumption. */
      if( (actv_nsapis[nsapi]->pdp_reg_cnf_fnc           ) &&
          (actv_nsapis[nsapi]->deactv_resp_pend == FALSE )    )
      {
        actv_nsapis[nsapi]->pdp_reg_cnf_fnc(
          actv_nsapis[nsapi]->ds_context, TRUE);
        actv_nsapis[nsapi]->pdp_reg_cnf_fnc = NULL;
      }
      else if( (actv_nsapis[nsapi]->pdp_reg_cnf_fnc           ) &&
               (actv_nsapis[nsapi]->deactv_resp_pend == TRUE  )    )
      {
        actv_nsapis[nsapi]->pdp_reg_cnf_fnc(
          actv_nsapis[nsapi]->ds_context, FALSE);
        actv_nsapis[nsapi]->pdp_reg_cnf_fnc = NULL;

        MSG_GERAN_MED_1_G( "GSN NS %d: PDP Reg failed", GSN_EXTERN_NSAPI(nsapi));
      }
      /* Resume PDP data flow. If retransmit event is set,
      ** start retransmiting bufferd N-PDUs. */
      if(actv_nsapis[nsapi]->pdp_ul_resume_fnc)
      {
        actv_nsapis[nsapi]->pdp_ul_resume_fnc(
          NULL, actv_nsapis[nsapi]->ds_context);
      }
      MSG_GERAN_HIGH_1_G("GSN NS %d: PDP UL Resume Req", GSN_EXTERN_NSAPI(nsapi));
      gsndcp_check_data_in_smd_to_sndcp_wm(nsapi);
      if( actv_nsapis[nsapi]->ule.retr )
        gsn_send_retr_req(nsapi);
    }
    MSG_GERAN_LOW_3_G("GSN NS %d: EXIT clr_susp: susp_flag=%x, retr_flag=%d",
      GSN_EXTERN_NSAPI(nsapi),actv_nsapis[nsapi]->ule.susp_flag,
      actv_nsapis[nsapi]->ule.retr);
  }
  else
  {
...
===
===========================================================================*/
static void gsn_ul_suspend_dataflow(   gsn_sapi_t sapi, const uint8 event )
{
  gsn_sapi_e_t    *sapi_e_ptr = actv_sapis[sapi];
  ...
  if( sapi_e_ptr != NULL )
  {
    if ( sapi_e_ptr->is_nsapi_ptr_q_valid )
    {
      ...
      if( item_ptr )
      {
        ...
        {
          /* Set Suspend flag  */
          gsn_set_suspend_event((item_ptr->nsapi_ptr->nsapi), event );
...
void gsn_ul_clr_est_susp_dyn(  uint8 sapi)
{
  ...
  sapi_e_ptr = actv_sapis[ sapi_int ];
  ...
  if( sapi_e_ptr != NULL )
  {
    if ( sapi_e_ptr->is_nsapi_ptr_q_valid )
    {
      ...
      if( item_ptr )
      {
        ...
        {
          /* Set Suspend flag  */
          gsn_clr_suspend_event(item_ptr->nsapi_ptr->nsapi, GSN_UL_SUSP_EST_PEND);
...
static void gsn_clr_nsapi_buf(   gsn_nsapi_t nsapi )
{
  gsn_nsapi_e_t       *nsapi_e_ptr = actv_nsapis[nsapi];
  ...
  if( nsapi_e_ptr != NULL )
  {
    ...
    */
    ...
    {
      MSG_GERAN_LOW_0_G("GSN ""No npdu buff");
    }
    ...
    */
    ...
    if ( nsapi_e_ptr->dle.npdu_ref_buff_valid )
    {
      ...
      while( npdu_ref_item_ptr )
      {
        ...
        gsn_dle_free_npdu_ref_item(  nsapi_e_ptr, npdu_ref_item_ptr );
...
static void gsn_clr_sapi_buf(   gsn_sapi_t sapi )
{
  gsn_sapi_e_t    *sapi_ptr = actv_sapis[sapi];
  gsn_nsapi_q_t   *item_ptr;
      ...
      for(alg = 0; alg <= T2_ALG_MAX; alg++ )
      {
        ...
        while( t2_item_ptr )
        {
              ...
              (t2_item_ptr->pcomp_e_ptr->t2_alg == T2_ALG_RFC1144) )
          {
            ...
            MSG_GERAN_LOW_2_G("GSN ""XID %d: Rmv PCOMP entity %d",
              gsn_extern_sapi[sapi],t2_item_ptr->pcomp_e_ptr->ent_num);

            gsn_slhc_free_mem( (slhc_handle_type*)t2_item_ptr->pcomp_e_ptr->comp_e );
          }
          else
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
                  ...
                  (t2_item_ptr->pcomp_e_ptr->t2_alg == T2_ALG_RFC2507) )
          {
            ...
            gsn_iphc_free( t2_item_ptr->pcomp_e_ptr->comp_e);
          }
          else 
#endif /* FEATURE_GSM_SNDCP_IPHC */
#ifdef FEATURE_GSM_SNDCP_ROHC
          if ((alg == T2_ALG_RFC3095) &&
                   (t2_item_ptr->pcomp_e_ptr->t2_alg == T2_ALG_RFC3095) )
          {
            ...
            gsn_rohc_free( t2_item_ptr->pcomp_e_ptr->comp_e);
          }
          else
#endif /* FEATURE_GSM_SNDCP_ROHC */
            MSG_GERAN_LOW_2_G("GSN "" Invalid T2 algorithm alg %d, t2_alg %d",
      ...
      for( alg = 0; alg <= T1_ALG_MAX; alg++ )
      {
        ...
        while( t1_item_ptr )
        {
              ...
              (t1_item_ptr->dcomp_e_ptr->t1_alg == T1_ALG_V42BIS) )
          {
            ...
            gsn_v42_dealloc_tables(  &t1_item_ptr->dcomp_e_ptr->comp_e, GSN_V42_BOTH_DIR );

            MSG_GERAN_LOW_2_G("GSN ""XID %d: Rmv DCOMP entity %d",
            ...
            MSG_GERAN_LOW_2_G("GSN "" Invalid T1 algorithm alg %d, t1_alg %d",
...
===
===========================================================================*/
static void gsn_ul_prepare_retr(   gsn_nsapi_t nsapi )
{
  gsn_nsapi_e_t       *nsapi_e_ptr = actv_nsapis[nsapi];
  gsn_seg_ref_item_t  *ref_item_ptr;
  gsn_npdu_item_t     *npdu_item_ptr;
...
static void gsn_activate_nsapi (   gsnsm_prim_type prim_type, const void *smsn_mes_ptr )
{
  gsn_nsapi_t   nsapi;
  ...
  if( actv_nsapis[nsapi] == NULL )
  {
    ...
    GSN_ASSERT( actv_nsapis[nsapi] != NULL );

    memset(actv_nsapis[nsapi], 0, sizeof(gsn_nsapi_e_t));
    ...
    actv_nsapis[nsapi]->to_gsndcp_sig = comp_reserve_to_comp_tx_sig();

    /* Store NSAPI params */
    actv_nsapis[nsapi]->nsapi      = nsapi;

    if( prim_type == GSNSM_ACTIVATE_IND )
    {
      actv_nsapis[nsapi]->radio_pr   =
        ((gsnsm_activ_ind_type*)smsn_mes_ptr)->radio_priority;
      actv_nsapis[nsapi]->qos        =
...
#endif
      actv_nsapis[nsapi]->pdp_type  =
        ((gsnsm_activ_ind_type*)smsn_mes_ptr)->pdp_type;

      actv_nsapis[nsapi]->data_comp = dcomp_enabled;
      actv_nsapis[nsapi]->pci_comp  = pcomp_enabled;

      actv_nsapis[nsapi]->pfi        =
    ...
    if( (actv_nsapis[nsapi]->qos.reliability_class == 0x1) ||
        (actv_nsapis[nsapi]->qos.reliability_class == 0x2)
      )
    {
      actv_nsapis[nsapi]->op_mode = ACK;
    }
    else
    {
      actv_nsapis[nsapi]->op_mode = UNACK;
    }
  }
  else
  {
...
===
===========================================================================*/
static void gsn_activate_sapi(gsn_sapi_t sapi, uint32 gsn_tlli )
{
  ...
  */
  if( actv_sapis[sapi] == NULL )
  {
    ...
    GSN_ASSERT( actv_sapis[sapi] != NULL );
    ...
    /* Heap memory allocated for sapi is initialised to avoid mutex creation
       ...
       actv_sapis[sapi]->nsapi_ptr_q.mutex into q_init(). */

    memset(actv_sapis[sapi], 0, sizeof(gsn_sapi_e_t));

    /* Store SAPI's number */
    actv_sapis[sapi]->sapi = sapi;

    /* Initialise SAPI entity */
    gsn_init_sapi(  actv_sapis[sapi] );

    /* Store SAPI's tlli */
    actv_sapis[sapi]->tlli   = gsn_tlli;

  }
  else
  {
    if( actv_sapis[sapi]->tlli != gsn_tlli )
    {
...
===
===========================================================================*/
static void gsn_map_nsapi_on_sapi(   gsn_nsapi_t nsapi, gsn_sapi_t sapi)
{
  ...
  */
  ...
  if( actv_sapis[sapi] != NULL )
  {
    if( actv_sapis[sapi]->is_nsapi_ptr_q_valid )
    {
      actv_nsapis[nsapi]->sapi_ptr = actv_sapis[sapi];
      nsapi_q_ptr = GPRS_MEM_MALLOC( sizeof(gsn_nsapi_q_t) );
      GSN_ASSERT( nsapi_q_ptr != NULL );

      nsapi_q_ptr->nsapi_ptr = actv_nsapis[nsapi];
      (void) q_link( nsapi_q_ptr, &(nsapi_q_ptr->link) );
      q_put( &(actv_sapis[sapi]->nsapi_ptr_q), &(nsapi_q_ptr->link) );
    }
    else
    {
      gsn_debug_print_invalid_sapi(sapi, GSN_GHDLR, __LINE__);
...
#endif
#ifdef FEATURE_GSM_SNDCP_IPHC
    if((hdr_comp_alg == SYS_PDP_HEADER_COMP_RFC2507)&&
          ((pdp_type == GSNSM_PDP_IPV4) ||
...
#endif /* FEATURE_GSM_SNDCP_IPHC */
    if(hdr_comp_alg == SYS_PDP_HEADER_COMP_OFF)
    {
                           ...
                           const void      *snsm_msg_ptr )
{
  ...
  */
  ...
  (
    ...
    gsn_timer_callback,
    (uint32)( ( (nsapi_ptr->nsapi << 16) & 0xFF0000 ) | SGSN_INITIATED_LINK_EST_FAIL )
  );
}
...
static void gsn_deactv_nsapi(   gsn_nsapi_e_t *nsapi_ptr )
{
  if( nsapi_ptr != NULL )
  {
    ...
    comp_unreserve_to_comp_tx_sig(
        actv_nsapis[nsapi_ptr->nsapi]->to_gsndcp_sig);
    ...
    gsn_clr_nsapi_buf(  nsapi_ptr->nsapi );

    /* Release npdu buffer, i.e. destroy queue. */
    gsn_rel_nsapi_buf(  nsapi_ptr->nsapi );

    /* Dealocate NSAPI's entity memory  */
    actv_nsapis[ nsapi_ptr->nsapi ] = NULL;
...
static void gsn_deactv_sapi(   gsn_sapi_e_t *sapi_ptr )
{
  if( sapi_ptr != NULL )
  {
    ...
    /* Inform LLC */
    gsn_send_ll_deactivate_ind(  sapi_ptr );

    /* Clear SAPI's buffers/queues */
    gsn_clr_sapi_buf(  sapi_ptr->sapi );

    /* Dealocate memory used for SAPI entity  */
    actv_sapis[ sapi_ptr->sapi ] = NULL;
...
===
===========================================================================*/
static void gsn_remove_nsapi_from_sapi(   gsn_nsapi_t nsapi, gsn_sapi_t sapi)
{
  gsn_nsapi_q_t   *nsapi_q_ptr;
  gsn_sapi_e_t    *sapi_ptr  = actv_sapis[sapi];
...
/*===========================================================================
===
===  FUNCTION      GSN_CHECK_LL_REL_NEEDED()
===
===  DESCRIPTION
===
===   It checks the if a release req is needed to be send to LLC. 
===   Release is not needed when NSAPIs attached to SAPI are in UNACK mode and SAPI is not in IDLE state.
===
...
===  RETURN VALUE
===
===    Boolean. TRUE if LL RELEASE is needed
===
===  SIDE EFFECTS
===
===    None
===
===========================================================================*/
static boolean gsn_check_ll_rel_needed(gsn_nsapi_t nsapi, gsn_sapi_t sapi )
{
  boolean      ll_rel_needed = FALSE;
  gsn_sapi_e_t *sapi_ptr = actv_sapis[sapi];

  /* Check if pointer to SAPI entity is valid */
  GSN_ASSERT(sapi_ptr != NULL);
  ...
  if (sapi_ptr->is_nsapi_ptr_q_valid)
  {
    ...
    if (q_cnt(&(sapi_ptr->nsapi_ptr_q)) > 1)
    {
       ...
       *  Check if any of them uses Ack operation before we release the LLC link (if established). */
      boolean sapi_uses_ack_op = gsn_ack_nsapis_on_sapi( nsapi, sapi);
    ...
    {
      /*  There is no other NSAPIs mapped on the SAPI. So release of
       *  the Ack LLC operation on this SAPI, if established, is possible. */
      if (sapi_ptr->state != IDLE)
      {
        ll_rel_needed = TRUE;
      }
...
===
===========================================================================*/
static boolean gsn_release_sapi(  gsn_nsapi_t nsapi, gsn_sapi_t sapi, boolean local)
{
  boolean      ll_rel_needed;
  gsn_sapi_t   temp_sapi;
  gsn_sapi_e_t *sapi_ptr = actv_sapis[sapi];
  ...
  ll_rel_needed = gsn_check_ll_rel_needed(nsapi, sapi);

  /* If LL_RELEASE_REQ is not needed, remove NSAPI from SAPI's list.
   * Otherwise, wait until LL_RELEASE_CNF to remove the NSAPI. */
  if (ll_rel_needed)
  {
    gsn_send_ll_rel_req( sapi_ptr, local, TRUE);
  }
  else
  {
    gsn_remove_nsapi_from_sapi( nsapi, sapi);
  }

  /* There is a possibility that two SAPIs can be attached to same NSAPI during SMSN_MOD_IND.
   * So, perform sanity check to remove such links from other SAPI. */
  for (temp_sapi = GSN_SAPI_3; temp_sapi < GSN_SAPI_MAX ; temp_sapi++)
  {
    sapi_ptr = actv_sapis[temp_sapi];
    
    if ((temp_sapi != sapi) && (sapi_ptr != NULL))
    {
      /* Check if NSAPI is connected to the SAPI */
      if(sapi_ptr->is_nsapi_ptr_q_valid && 
         q_linear_search(&(sapi_ptr->nsapi_ptr_q), (q_compare_func_type)gsn_find_nsapi, &nsapi) != NULL)
      {
        /* SAPI need to be released locally and we do not need a REL_CNF from LLC. 
         * We do not need CNF from LLC because it is possible that the NSAPI might have deactivated after removing the original SAPI */
        if (gsn_check_ll_rel_needed(nsapi, temp_sapi))
        {
          gsn_send_ll_rel_req( sapi_ptr, TRUE, FALSE);
        }

        /* Remove the NSAPI link from this SAPI */
        gsn_remove_nsapi_from_sapi( nsapi, temp_sapi);
      }
    }
  }
...
===
===========================================================================*/
static boolean gsn_ack_nsapis_on_sapi(   gsn_nsapi_t nsapi, gsn_sapi_t sapi )
{
  gsn_sapi_e_t *sapi_ptr = actv_sapis[sapi];
...
===
===========================================================================*/
static void gsn_send_ll_est_res(   gsn_sapi_e_t *sapi_ptr, dsm_item_type *xid_neg_ptr )
{
  gsn_nsapi_q_t         *nsapi_q_ptr;
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  case ( ESTABLISHED):
    {
      ...
      gsn_print_xid_params (  xid_neg_ptr, TRUE );
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_LOW_1_G("GSN ""LL %d: Sent LL_ESTABLISH_RES",gsn_extern_sapi[sapi]);
  ...
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd LL_EST_RSP in state: %d",
...
===
===========================================================================*/
static void gsn_send_ll_rel_req(  gsn_sapi_e_t *sapi_ptr, boolean local, boolean cnf_needed)
{
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  case ( ESTABLISHED ):
    {
      ...
      ul_mes->mes.rel_req_mes.cnf_needed = cnf_needed;
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_LOW_3_G("GSN ""LL %d: Sent LL_RELEASE_REQ (local=%d, cnf_needed=%d)", gsn_extern_sapi[sapi], local, cnf_needed);
...
/*-------------------------------------------------------------------------------*/
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd REL_REQ in state: %d",gsn_extern_sapi[sapi],sapi_ptr->state);
    break;
...
===
===========================================================================*/
static void gsn_send_ll_xid_res(   gsn_sapi_e_t *sapi_ptr, dsm_item_type  *xid_neg )
{
  gsn_nsapi_q_t         *nsapi_q_ptr;
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  case ( ESTABLISHED):
    {
      ...
      /* Print XID parameters in F3 message */
      gsn_print_xid_params (  xid_neg, TRUE );
...
===
===========================================================================*/
static void gsn_send_ll_reset_res(   gsn_sapi_e_t *sapi_ptr )
{
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  case ( ESTABLISHED):
    {
      ...
      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_LOW_1_G("GSN ""LL %d: Sent LL_RESET_RES",gsn_extern_sapi[sapi]);
...
/*-------------------------------------------------------------------------------*/
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd RESET_RES in state: %d",gsn_extern_sapi[sapi],sapi_ptr->state);
    break;
...
/*===========================================================================
===
===  FUNCTION      GSN_SEND_LL_ENTER_PSM_IND()
===
===  DESCRIPTION
===
===   On receiving GSNSM_ENTER_PSM_IND, SNDCP sends LL_ENTER_PSM_IND to LLC.
===
===  DEPENDENCIES
===
===   None.
...
===
===========================================================================*/
static void gsn_send_ll_enter_psm_ind(void)
{
  gsn_sapi_t            sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
  ...
  /* Load msg */
  ul_mes->sapi                          = 1;
  ul_mes->mes.enter_psm_ind.prim        = LL_ENTER_PSM_IND;

  for (sapi = GSN_SAPI_3; sapi < GSN_SAPI_MAX; sapi++)
  {
    if (actv_sapis[sapi])
    {
      ul_mes->mes.enter_psm_ind.psm_info[sapi].sapi_active = TRUE;
    }
    else
    {
      ul_mes->mes.enter_psm_ind.psm_info[sapi].sapi_active = FALSE;
    }

    /* Send L3 XID info so that LLC will store it in EFS. 
     * Currently set to NULL and will be used in future */
    ul_mes->mes.enter_psm_ind.psm_info[sapi].l3_xid_info = NULL;    
  }

  /* Post message to LLC and log for call flow analysis. */
  MSG_GERAN_MED_0_G("GSN ""LL : Sent LL_ENTER_PSM_IND");

  cfa_log_packet_ex
  (
    GS_QUEUE_LLC, MS_LLC_SNDCP, (uint8) LL_ENTER_PSM_IND,
    sizeof(ll_enter_psm_ind_t), (uint8 *)&ul_mes->mes.enter_psm_ind
  );
...
===
===========================================================================*/
static void gsn_process_ll_rst_ind(   gsn_sapi_e_t *sapi_ptr, ll_reset_ind_t  *ll_mes )
{
  gsn_sapi_t         sapi = sapi_ptr->sapi;
  gsn_npdu_item_t    *npdu_item_ptr;
...
/*-------------------------------------------------------------------------------*/
  ...
  case( DISC_PEND ):
    {
      ...
      if( (ll_mes != NULL) && sapi_ptr->is_nsapi_ptr_q_valid )
      {
          ...
          */
          ...
          for( alg = 0; alg <= T2_ALG_MAX; alg++ )
          {
            ...
            while( q_item != NULL )
            {
              ...
              {
#endif /* FEATURE_GPRS_PS_HANDOVER */
                gsn_del_pcomp_e(  q_item->pcomp_e_ptr->ent_num, sapi_ptr );
#ifdef FEATURE_GPRS_PS_HANDOVER
              }
              else
              {
                ...
                case T2_ALG_RFC1144:
                  ...
                  gsn_slhc_free_mem( q_item->pcomp_e_ptr->comp_e);
                  /* create a new instance and init it with existing params */
                  (void)gsn_1144_comp_init( q_item->pcomp_e_ptr,
                ...
                case T2_ALG_RFC2507:
                  ...
                  gsn_iphc_free( q_item->pcomp_e_ptr->comp_e);
                  /* create a new instance and init it with existing params */
                ...
                case T2_ALG_RFC3095:
                  ...
                  gsn_rohc_free( q_item->pcomp_e_ptr->comp_e);
                  /* create a new instance and init it with existing params */
                  (void)gsn_rohc_comp_init( q_item->pcomp_e_ptr->comp_e,
                    (gsn_xid_pcomp_3095_t *) (&q_item->pcomp_e_ptr->alg.xid_rfc3095));
                  break;
#endif
                default:
                  MSG_GERAN_LOW_0_G("GSN " "Invalid T2 alg");
                  break;
                }
              }
#endif /* FEATURE_GPRS_PS_HANDOVER */
          ...
          */
          ...
          for( alg = 0; alg <= T1_ALG_MAX; alg++ )
          {
            ...
            while( q_item != NULL )
            {
              ...
              {
#endif /* FEATURE_GPRS_PS_HANDOVER */
                gsn_del_dcomp_e(  q_item->dcomp_e_ptr->ent_num, sapi_ptr );
#ifdef FEATURE_GPRS_PS_HANDOVER
              }
              else
              {
                ...
                case T1_ALG_V42BIS:
                  ...
                  gsn_v42_dealloc_tables(  &q_item->dcomp_e_ptr->comp_e,
                    q_item->dcomp_e_ptr->alg.xid_v42bis.p0 );
                  /* Create a new dcomp entity and init it with existing dcomp
                     XID params */
                  (void)gsn_v42bis_comp_init(  q_item->dcomp_e_ptr);
                  break;

                default:
                  MSG_GERAN_LOW_0_G("GSN ""Invalid T1 alg");
                  break;
                }
              }
#endif /* FEATURE_GPRS_PS_HANDOVER */
        ...
        */
        ...
        if( item_ptr )
        {
          while( item_ptr != NULL )
          {
            ...
            if( item_ptr->nsapi_ptr->op_mode == ACK )
            {
              gsn_enter_recovery(  item_ptr->nsapi_ptr );
        ...
        {
          gsn_debug_print_invalid_sapi(sapi_ptr->sapi, GSN_GHDLR, __LINE__);
        }

        /* Initialise LLC SAPIs N201_I and N201_U */
        gsn_n201_i[sapi] = gllc_ll_get_default_N201_I( gsn_extern_sapi[sapi] );
        gsn_n201_u[sapi] = gllc_ll_get_default_N201_U( gsn_extern_sapi[sapi] );
        ...
        */
        ...
        /* Read Anite GCF flag */
        rr_read_public_store( RRPS_MS_ANITE_GCF_FLAG, &b_anite_gcf_flag );
        ...
        while( item_ptr != NULL )
        {
          if( item_ptr->nsapi_ptr->op_mode == UNACK )
          {
            ...
            if (!b_anite_gcf_flag)
            {
              ...
              gsn_unack_retr_npdu_hdlr(  item_ptr->nsapi_ptr->nsapi );
            }
          }
  ...
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Invalid SAPI state: %d",
      gsn_extern_sapi[sapi],sapi_ptr->state);
  ...
  gsn_send_ll_reset_res(  sapi_ptr );
}
...
===
===========================================================================*/
static void gsn_process_ll_est_ind(   gsn_sapi_e_t *sapi_ptr, ll_establish_ind_t  *ll_mes )
{
  ...
  case( IDLE ):
    {
      ...
      if( sapi_ptr->is_nsapi_ptr_q_valid )
      {
        ...
        */
        ...
        gsn_n201_u[sapi]  = ll_mes->n201_u;
        gsn_n201_i[sapi]  = ll_mes->n201_i;
        ...
        */
        ...
        {
          xid_neg_ptr = gsn_process_req_xid(  sapi_ptr, &ll_mes->l3_xid_req,
                                             collision, TRUE );

          /* Post message to LLC */
          gsn_send_ll_est_res(  sapi_ptr, xid_neg_ptr );
        }
        else
          sapi_ptr->state = ESTABLISHED;
        ...
        */
        ...
        if( item_ptr )
        {
          while( item_ptr != NULL )
          {
            ...
            if( collision )
            {
              gsn_clr_suspend_event(item_ptr->nsapi_ptr->nsapi, GSN_UL_SUSP_XID_PEND );
            }
            ...
            if( item_ptr->nsapi_ptr->actv_resp_pend )
            {
              ...
              gsn_send_snsm_actv_res(  item_ptr->nsapi_ptr->nsapi );
            }
                ...
                (item_ptr->nsapi_ptr->modif_est_resp_pend)    )
            {
              ...
              */
              ...
              if(!(item_ptr->nsapi_ptr->modif_rel_resp_pend) )
              {
                ...
                gsn_send_snsm_mod_res(  item_ptr->nsapi_ptr->nsapi);
          ...
          while( item_ptr != NULL )
          {
                ...
                (sapi_ptr->re_establish ) )
            {
              ...
              */
              sapi_ptr->re_establish = FALSE;
              gsn_enter_recovery(  item_ptr->nsapi_ptr );
              gsn_ul_prepare_retr(  item_ptr->nsapi_ptr->nsapi );
            }
        ...
        if( item_ptr )
        {
          while( item_ptr != NULL )
          {
            ...
            if( item_ptr->nsapi_ptr->op_mode == ACK )
            {
              ...
              */
              gsn_ack_remove_fback_npdus_and_renumber(  item_ptr->nsapi_ptr );
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */
...
/*-------------------------------------------------------------------------------*/
  case( DISC_PEND ):
  default:
    MSG_GERAN_ERROR_2_G("GSN ""LL %d: Rcvd LL_EST_IND in state: %d", gsn_extern_sapi[sapi], sapi_ptr->state);
    break;
...
===
===========================================================================*/
static void gsn_process_ll_est_cnf(   gsn_sapi_e_t *sapi_ptr, ll_establish_cnf_t  *ll_mes )
{
  gsn_sapi_t    sapi = sapi_ptr->sapi;
  ...
  case( EST_PEND ):
    {
      ...
      */
      ...
      if( gsn_decode_neg_xid(  &ll_mes->l3_xid_neg, sapi_ptr ) )
      {
      ...
      else
      {
        ...
        */
        ...
        if( sapi_ptr->xid_req_cnt < XID_REQ_MAX )
        {
          ...
          /* Post message to LLC */
          gsn_send_ll_est_req(  sapi_ptr, gsn_xid_req, FALSE );
        }
        else
        {
          /* initialise counter */
...
/*-------------------------------------------------------------------------------*/
  ...
  case( DISC_PEND ):
  default:
    MSG_GERAN_ERROR_2_G("GSN ""LL %d LL_EST_CNF received in SAPI state: %d", gsn_extern_sapi[sapi], sapi_ptr->state);
    break;
...
===
===========================================================================*/
static void gsn_process_ll_rel_ind(   gsn_sapi_e_t *sapi_ptr, const ll_release_ind_t  *ll_mes )
{
  gsn_sapi_t     sapi      = sapi_ptr->sapi;
...
/*-------------------------------------------------------------------------------*/
  ...
  case( IDLE ):
    ...
    MSG_GERAN_LOW_0_G("GSN ""REL_IND received while SAPI in state: IDLE");
    break;
  ...
  case( ESTABLISHED):
    {
      ...
      */
      ...
      case GPRS_LL_NORMAL_RELEASE:
        {
          ...
          */
          if( sapi_ptr->state == EST_PEND )
          {
            MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_REL_IND Normal release while EST_PEND",
          ...
          */
          ...
          if( sapi_ptr->xid_state == XID_EST_CNF_PEND )
          {
            ...
            gsn_del_all_selected_comp_e(  sapi_ptr );
#endif
          }
              ...
              while( item_ptr != NULL )
              {
                    ...
                    (item_ptr->nsapi_ptr->modif_rel_resp_pend) )
                {
                      ...
                      item_ptr->nsapi_ptr->init_nsapi     )
                  {
                    ...
                    gsn_remove_nsapi_from_sapi(  item_ptr->nsapi_ptr->nsapi,
                                                sapi);
                  }

                  if ( actv_nsapis[nsapi] != NULL )
                  {
                    if( actv_nsapis[nsapi]->init_nsapi )
                    {
                      actv_nsapis[nsapi]->init_nsapi = FALSE;

                      /* Deactivate NSAPI entity */
                      gsn_deactv_nsapi(  actv_nsapis[nsapi] );
                    }
                    /* Leave while loop */
                    break;
                  }
      ...
      case GPRS_LL_NO_PEER_RESPONSE:
        ...
        if( sapi_ptr->link_estab_originator == TRUE)
        {
          ...
          */
          ...
          if( sapi_ptr->xid_req_dup_ptr )
          {
            dsm_item_type *xid_ptr = sapi_ptr->xid_req_dup_ptr;
...
#if (defined(FEATURE_GSM_GPRS_SNDCP_PCOMP) || defined(FEATURE_GSM_GPRS_SNDCP_DCOMP))
          gsn_send_ll_est_req(  sapi_ptr, sapi_ptr->xid_req_dup_ptr, FALSE );
#else /* (FEATURE_GSM_GPRS_SNDCP_PCOMP ||FEATURE_GSM_GPRS_SNDCP_DCOMP) */
          gsn_send_ll_est_req(  sapi_ptr, NULL, FALSE );
#endif /* !(FEATURE_GSM_GPRS_SNDCP_PCOMP ||FEATURE_GSM_GPRS_SNDCP_DCOMP) */
        }
        break;
      ...
      default:
        MSG_GERAN_LOW_1_G("GSN ""Invalid LL_RELEASE_IND cause: %d", ll_mes->cause );
        break;
      }
      break;
    }
...
/*-------------------------------------------------------------------------------*/
  case( DISC_PEND ):
    ...
    MSG_GERAN_LOW_0_G("GSN ""REL_IND received while SAPI in state: DISC_PEND");
    break;

/*-------------------------------------------------------------------------------*/
  default:

    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd REL_IND in SAPI state: %d",
...
===
===========================================================================*/
static void gsn_process_ll_rel_cnf(   gsn_sapi_e_t *sapi_ptr, ll_release_cnf_t *mes )
{
  gsn_sapi_t          sapi = sapi_ptr->sapi;
...
/*-------------------------------------------------------------------------------*/
  ...
  case( EST_PEND ):
  case( ESTABLISHED ):
    MSG_GERAN_LOW_1_G("GSN ""REL_CNF received while SAPI in state: %d",sapi_ptr->state);
    break;
...
/*-------------------------------------------------------------------------------*/
  case( DISC_PEND ):
    {
      gsn_nsapi_q_t      *item_ptr = NULL;
      gsn_nsapi_q_t      *next_item_ptr = NULL;
      gsn_nsapi_t         nsapi;
      uint8               qcnt;
      ...
      if (sapi_ptr->is_nsapi_ptr_q_valid)
      {
        /* get queue count and head of the queue */
        qcnt = (uint8) q_cnt(&(sapi_ptr->nsapi_ptr_q));
        item_ptr = (gsn_nsapi_q_t*)q_check(&(sapi_ptr->nsapi_ptr_q));
        ...
        ** and check if a response is pending to SM
        */
        while((qcnt > 0) && (item_ptr != NULL))
        {
          /* check if active NSAPI entity has a valid pointer */
          GSN_ASSERT(item_ptr->nsapi_ptr != NULL);

          /* Store next item in the queue. Will be useful if current item is deleted in the process. */
          next_item_ptr = (gsn_nsapi_q_t*) q_next(&(sapi_ptr->nsapi_ptr_q), &item_ptr->link);
          MSG_GERAN_LOW_2_G("Temp_DBG:: qcnt=%d, NSAPI=%d", qcnt, item_ptr->nsapi_ptr->nsapi);
               ...
               (item_ptr->nsapi_ptr->modif_rel_resp_pend)))
          {
            ...
            if (item_ptr->nsapi_ptr->deactv_resp_pend)
            {
              ...
              gsn_send_snsm_deactv_res( item_ptr->nsapi_ptr->nsapi);
            }
            if ((item_ptr->nsapi_ptr->modif_resp_pend) &&
                (item_ptr->nsapi_ptr->modif_rel_resp_pend))
            {
                ...
                item_ptr->nsapi_ptr->init_nsapi)
            {
              ...
              gsn_remove_nsapi_from_sapi( item_ptr->nsapi_ptr->nsapi, sapi);
            }
          ...
          /* Decrement queue count and assign the next item in the list */
          qcnt --;
...
/*-------------------------------------------------------------------------------*/
  default:
    ...
    MSG_GERAN_LOW_1_G("GSN ""REL_CNF received in invalid SAPI state: %d", sapi_ptr->state);
    break;
...
===
===========================================================================*/
static void gsn_process_ll_xid_ind(   gsn_sapi_e_t *sapi_ptr, ll_xid_ind_t  *ll_mes )
{
  gsn_sapi_t          sapi = sapi_ptr->sapi;
  ...
  case( ESTABLISHED ):
    {
      ...
      */
      if( ll_mes->l3_xid_req != NULL )
      {
        ...
        gsn_send_ll_xid_res(  sapi_ptr, xid_neg_ptr);
...
===
===========================================================================*/
static void gsn_process_ll_xid_cnf(   gsn_sapi_e_t *sapi_ptr, ll_xid_cnf_t  *ll_mes )
{
  gsn_nsapi_q_t         *item_ptr;
  dsm_item_type         *gsn_xid_req;
...
/*-------------------------------------------------------------------------------*/
  ...
  case( DISC_PEND ): /* XID Request to remove compression entity from SAPI before disconnecting */
    ...
    /* Store primitive parameters */
    sapi_ptr->tlli                      = ll_mes->tlli;
    gsn_n201_u[sapi_ptr->sapi]  = ll_mes->n201_u;
    gsn_n201_i[sapi_ptr->sapi]  = ll_mes->n201_i;
...
#endif
          /* Send message */
          gsn_send_ll_xid_req(  sapi_ptr, gsn_xid_req, FALSE);
        }
        else
        {
          /* initialise counter */
          ...
          */
          ...
          while( item_ptr != NULL )
          {
            ...
            if(item_ptr->nsapi_ptr->deactv_resp_pend)
            {
              ...
              if(!gsn_release_sapi( 
                                    item_ptr->nsapi_ptr->nsapi,
                                    actv_nsapis[item_ptr->nsapi_ptr->nsapi]->sapi_ptr->sapi,
                                    FALSE))
              {
                gsn_send_snsm_deactv_res(  item_ptr->nsapi_ptr->nsapi );
                item_ptr->nsapi_ptr->deactv_resp_pend = FALSE;
                /* Deactivate NSAPI entity */
                gsn_deactv_nsapi(  actv_nsapis[item_ptr->nsapi_ptr->nsapi] );
              }
            }
    ...
    {
      MSG_GERAN_LOW_1_G("GSN ""LL %d: invalid LL_XID_CNF ",
        gsn_extern_sapi[sapi_ptr->sapi]);
    }
    break;
  ...
  default:
    ...
    MSG_GERAN_LOW_1_G("GSN ""LL %d: LL_XID_CNF in inval state ",
        gsn_extern_sapi[sapi_ptr->sapi]);
...
===
===========================================================================*/
static void gsn_process_ll_stat_ind(   gsn_sapi_e_t *sapi_ptr, const ll_status_ind_t  *ll_mes )
{
  gsn_nsapi_q_t       *item_ptr;
...
/*-------------------------------------------------------------------------------*/
  case( IDLE ):
    {
      ...
      if( sapi_ptr->xid_state == XID_CNF_PEND )
      {
        ...
        default:
          MSG_GERAN_LOW_2_G("GSN ""ME: Rcvd Invalid LL_STATUS_IND cause: %d, state: %d",
  ...
  case( ESTABLISHED ):
    ...
    */
    MSG_GERAN_LOW_1_G("GSN ""STATUS_IND received while SAPI in state: %d", sapi_ptr->state);
    break;

/*-------------------------------------------------------------------------------*/
    default:

    MSG_GERAN_LOW_1_G("GSN ""STATUS_IND received in invalid SAPI state: %d", sapi_ptr->state);
    break;
...
===
===========================================================================*/
static void gsn_process_snsm_actv_ind(   const gsnsm_activ_ind_type  *smsn_mes_ptr )
{
      ...
      GSN_VALID_NSAPI( smsn_mes_ptr->nsapi) )
  {
       ...
       (smsn_mes_ptr->pcomp_alg <= SYS_PDP_HEADER_COMP_RFC3095))
    {
      ...
      */
      MSG_GERAN_LOW_0_G("GSN " "ME: PCI compression is not enabled");
       ...
       (smsn_mes_ptr->dcomp_alg == SYS_PDP_DATA_COMP_V42_BIS))
    {
      ...
      ** feature is not switched on
      */
      MSG_GERAN_LOW_0_G("GSN " "ME: Data compression is not enabled");
    ...
    if ( actv_nsapis[nsapi] == NULL )
    {
      gsn_activate_nsapi(  GSNSM_ACTIVATE_IND, (void *)smsn_mes_ptr );
    }
    else
    {
    ...
    gsn_activate_sapi (  new_sapi, smsn_mes_ptr->tlli );

    /* link NSAPI with SAPI */
    gsn_map_nsapi_on_sapi(  nsapi, new_sapi );
    ...
    */
        ...
        (nsapi_e_ptr->sapi_ptr->state == IDLE) )
    {
      ...
      gsn_xid_req = gsn_encode_req_xid(  nsapi_e_ptr );

      /* Post message to LLC */
      gsn_send_ll_est_req( nsapi_e_ptr->sapi_ptr, gsn_xid_req, smsn_mes_ptr->psm_restore  );
              ...
              (nsapi_e_ptr->sapi_ptr->state == IDLE))  )
    {
      ...
      */
      if( gsn_xid_req != NULL )
      {
        gsn_send_ll_xid_req(  nsapi_e_ptr->sapi_ptr, gsn_xid_req, smsn_mes_ptr->psm_restore );
      }
      else
      {
        /* Inform LLC */
        gsn_send_ll_activate_ind(  nsapi_e_ptr->sapi_ptr, smsn_mes_ptr->psm_restore );

        if( nsapi_e_ptr->actv_resp_pend )
        {
          gsn_send_snsm_actv_res(  nsapi_e_ptr->nsapi );
    ...
    else
    {
      ...
      */
      MSG_GERAN_MED_2_G("GSN ""ME: NSAPI %d, SAPI %d in Inval state",
       GSN_EXTERN_NSAPI( nsapi ),gsn_extern_sapi[nsapi_e_ptr->sapi_ptr->sapi]);
    }
  }
  else
  {
    gsn_debug_print_invalid_sapi(smsn_mes_ptr->sapi, GSN_GHDLR, __LINE__);
    gsn_debug_print_invalid_nsapi(smsn_mes_ptr->nsapi, GSN_GHDLR, __LINE__);
  }
}
...
===
===========================================================================*/
static void gsn_process_snsm_deactv_ind(   const gsnsm_deactiv_ind_type  *mes_ptr )
{
  uint8               i;
  gsn_nsapi_t         nsapi;
  ...
  for( i = 0; i < mes_ptr->num_nsapi; i++ )
  {
    ...
    if( (!GSN_VALID_NSAPI( mes_ptr->nsapi[i] ) )  ||
        (actv_nsapis[GSN_INTERN_NSAPI( mes_ptr->nsapi[i])] == NULL) )
    {
      gsn_debug_print_invalid_nsapi(GSN_INTERN_NSAPI( mes_ptr->nsapi[i]), GSN_GHDLR, __LINE__);
      gsn_send_snsm_deactv_res(  GSN_INTERN_NSAPI( mes_ptr->nsapi[i]) );
    }
    else
    {
      ...
      actv_nsapis[nsapi]->init_nsapi = TRUE;
      ...
      */
      if( !mes_ptr->local )
      {
        req_xid_ptr = gsn_encode_rmv_nsapi_xid(  actv_nsapis[nsapi] );

        if( req_xid_ptr != NULL )
          gsn_send_ll_xid_req(  actv_nsapis[nsapi]->sapi_ptr, req_xid_ptr, FALSE );
      }
      else
      {
        /* Release SAPI Ack LLC operation if SAPI is used only by this NSAPI  */
        if(!gsn_release_sapi( 
                              nsapi,
                              actv_nsapis[nsapi]->sapi_ptr->sapi,
                              mes_ptr->local) )
        {
          gsn_send_snsm_deactv_res(  nsapi );
          actv_nsapis[nsapi]->deactv_resp_pend = FALSE;
          /* Deactivate NSAPI entity */
          gsn_deactv_nsapi(  actv_nsapis[nsapi] );
        }
...
===
===========================================================================*/
static void gsn_process_snsm_mod_ind(   const gsnsm_modify_ind_type  *smsn_mes_ptr )
{
  gsn_nsapi_e_t       *nsapi_e_ptr;
  ...
  {
    /* Activate NSAPI and SAPI entities */
    gsn_activate_nsapi(  GSNSM_MODIFY_IND, (void*) smsn_mes_ptr );
    gsn_activate_sapi (  new_sapi, smsn_mes_ptr->tlli );
    ...
    */
    gsn_process_lowater(  new_sapi, nsapi_e_ptr->op_mode, GSN_UL_SUSP_CTS_OFF );
    ...
    if( (nsapi_e_ptr->op_mode == ACK)   &&
        (actv_sapis[new_sapi]->state == IDLE) )
    {
      ...
      gsn_ul_suspend_dataflow(  new_sapi, GSN_UL_SUSP_EST_PEND );
    }
  ...
  else
  {
    ...
    if( new_sapi != old_sapi )
    {
      ...
      */
      if( (actv_sapis[old_sapi]->state == ESTABLISHED) &&
          (!gsn_ack_nsapis_on_sapi(  nsapi, old_sapi )) )
      {
        ...
        gsn_remove_nsapi_from_sapi(  nsapi, old_sapi);
      ...
      */
      ...
      {
        gsn_enter_recovery(  nsapi_e_ptr );
                ...
                (actv_sapis[new_sapi]->state == IDLE) )
      {
        ...
        */
         ...
         */
         ...
         /* Modification procedure is completed, resume data flow. */
         gsn_clr_suspend_event(  nsapi_e_ptr->nsapi, GSN_UL_SUSP_EST_PEND );
         gsn_clr_suspend_event(  nsapi_e_ptr->nsapi, GSN_UL_SUSP_SEQ_PEND );
      }
    }
    else
    { /* No change of SAPI */
      ...
      if( actv_nsapis[nsapi]->sapi_ptr->sapi != new_sapi )
      {
        MSG_GERAN_LOW_0_G("GSN ""ME: Rcvd Inval Modify Indication");
      }
      else
      {
        /* Use the new TLLI */
        nsapi_e_ptr->sapi_ptr->tlli = smsn_mes_ptr->tlli;
        ...
        if( nsapi_unack_to_ack )
        {
          ...
          */
          if( actv_sapis[new_sapi]->state == IDLE )
          {
            nsapi_e_ptr->modif_est_resp_pend = TRUE;
        ...
        else if( nsapi_ack_to_unack )
        {
          ...
          **  need to wait for LL_RELEASE_IND. */
          ...
          if ( ( !gsn_ack_nsapis_on_sapi(  nsapi, new_sapi )          ) ||
               ( (  gsn_ack_nsapis_on_sapi(  nsapi, new_sapi  )) &&
                 (  actv_sapis[new_sapi]->state == IDLE       )      )
             )
          {
      ...
      (nsapi_e_ptr->modif_rel_resp_pend == FALSE) )
  {
    ...
    gsn_send_snsm_mod_res(  nsapi_e_ptr->nsapi );
  }
}
...
===
===========================================================================*/
static void gsn_process_snsm_seq_ind(   const gsnsm_seq_ind_type  *smsn_mes_ptr )
{
  gsn_nsapi_e_t       *nsapi_e_ptr;
  ...
  if( (!GSN_VALID_NSAPI(smsn_mes_ptr->nsapi) )   ||
      (actv_nsapis[GSN_INTERN_NSAPI( smsn_mes_ptr->nsapi )] == NULL) )
  {
    gsn_debug_print_invalid_nsapi(GSN_INTERN_NSAPI(smsn_mes_ptr->nsapi), GSN_GHDLR, __LINE__);
  }
  else
  {
    /* Get NSAPI number */
    ...
    if(actv_nsapis[GSN_INTERN_NSAPI( smsn_mes_ptr->nsapi )]->op_mode == ACK)
    {
      /* Resume transmission of SN-PDUs and all buffered N-PDUs
      ** If suspended due to LL_RESET_IND.
      */
      gsn_clr_suspend_event(  nsapi, GSN_UL_SUSP_SEQ_PEND );
      ...
      */
      ...
      gsn_ack_remove_fback_npdus_and_renumber(  nsapi_e_ptr );
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */
      ...
      */
      ...
      gsn_send_snsm_seq_res(  nsapi_e_ptr->nsapi, (gsn_nsapi_mode_t) ACK);
    }
    else
    {
      /* No action is taken when NSAPI in UNACK mode. Just respond to SM */
      gsn_send_snsm_seq_res(  nsapi_e_ptr->nsapi, (gsn_nsapi_mode_t) UNACK);
    }
  }
}
...
/*===========================================================================
===
===  FUNCTION      GSN_PROCESS_SNSM_ENTER_PSM_IND()
===
===  DESCRIPTION
===
===   Processes the SNSM_ENTER_PSM indication primitive send by SM while
===   entering PSM mode.
===
===  DEPENDENCIES
===
...
===
===========================================================================*/
static void gsn_process_snsm_enter_psm_ind(   const gsnsm_enter_psm_ind_type *smsn_mes_ptr )
{
  /* Send indication to LLC */
  gsn_send_ll_enter_psm_ind();
}
...
===
===========================================================================*/
static void gsn_send_snsm_actv_res(   gsn_nsapi_t nsapi )
{
  ...
  snsm_dl_mes->cmd.sndcp_activate_resp.tlli = actv_nsapis[nsapi]->sapi_ptr->tlli;
...
===
===========================================================================*/
static void gsn_send_snsm_deactv_res(   gsn_nsapi_t nsapi )
{
  sm_cmd_type  *snsm_dl_mes;
  ...
  if ( actv_nsapis[nsapi] != NULL )
  {
    snsm_dl_mes->cmd.sndcp_deactivate_resp.tlli  =
      actv_nsapis[nsapi]->sapi_ptr->tlli;
  }
  else
...
===
===========================================================================*/
static void gsn_send_snsm_mod_res(   gsn_nsapi_t nsapi )
{
  sm_cmd_type  *snsm_dl_mes;
  ...
  snsm_dl_mes->cmd.sndcp_modify_resp.tlli   = actv_nsapis[nsapi]->sapi_ptr->tlli;
...
===
===========================================================================*/
static void gsn_send_snsm_seq_res(   gsn_nsapi_t nsapi, gsn_nsapi_mode_t nsapi_mode )
{
  sm_cmd_type  *snsm_dl_mes;
  ...
  snsm_dl_mes->cmd.sndcp_sequence_resp.tlli = actv_nsapis[nsapi]->sapi_ptr->tlli;
  snsm_dl_mes->cmd.sndcp_sequence_resp.nsapi= (uint8) GSN_EXTERN_NSAPI(nsapi);

  if( nsapi_mode == ACK )
  {
    snsm_dl_mes->cmd.sndcp_sequence_resp.rec_n_pdu = actv_nsapis[nsapi]->dle.ack_rcv_npdu;
  }
  else
  {
    snsm_dl_mes->cmd.sndcp_sequence_resp.rec_n_pdu = actv_nsapis[nsapi]->dle.unack_rcv_npdu;
  }
...
)
{
  ...
  while( len )
  {
    ...
    */
    n201_i  = gsn_n201_i[nsapi_e_ptr->sapi_ptr->sapi];
    ...
    */
    ...
    if( !gprs_pdu_append_head( &data_mes->l3_pdu, ext_hdr, (uint16)hdr_len) )
    {
      ...
      */
      ...
      MSG_GERAN_LOW_1_G("GSN ""NS %d: gprs_pdu_append_head() failed. Force NSAPI deactivation",
          GSN_EXTERN_NSAPI(nsapi));
    ...
    */
    ...
    MSG_GERAN_MED_5_G
    (
      "LL %: Sent LL_DATA_REQ %d:%d:%d, PFI %d",
      nsapi_e_ptr->sapi_ptr->sapi,
      sn_hdr.nsapi,
    ...
    gllc_ll_ul_dat_put_mes_buf(  ll_mes );
...
===
===========================================================================*/
static void gsn_process_pdp_dl_data(   gsn_nsapi_t nsapi, dsm_item_type **npdu  )
{
      ...
      (actv_nsapis[nsapi]->from_gsndcp_wm != NULL))
  {
    ...
    */
    ...
    if( actv_nsapis[nsapi]->dle.first_seg_info.dcomp != 0 )
      gsn_data_decompress(  actv_nsapis[nsapi], npdu );
    ...
    /* Check if packet is compressed. If so, decompress packet */
    if( actv_nsapis[nsapi]->dle.first_seg_info.pcomp != 0 )
    {
      if( !gsn_pci_decompress(  actv_nsapis[nsapi], npdu ) )
      {
        /* Free mem conveying the N-PDU */
        if( *npdu != NULL )
          GPRS_PDU_FREE( npdu );

        MSG_GERAN_LOW_1_G("GSN ""NS %d: DeComp failed. DL N-PDU is dropped",
          GSN_EXTERN_NSAPI(nsapi));
      }
    }
    ...
    if( *npdu != NULL )
    {
...
#ifndef FEATURE_GERAN_REDUCED_DEBUG
      /* Update Diag statistics */
      actv_nsapis[nsapi]->diag_pdu_stat_rsp.npdu_octet_rx_cnt +=
                                           GPRS_PDU_LENGTH( *npdu );
      actv_nsapis[nsapi]->diag_pdu_stat_rsp.npdu_rx_cnt++;
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */
      ...
      *npdu      = NULL;
      ...
      dsm_enqueue(actv_nsapis[nsapi]->from_gsndcp_wm, &temp_npdu);

      /* Call the registered SNDCP Rx function ptr */
      actv_nsapis[nsapi]->from_gsndcp_post_rx_func_ptr (0,0,actv_nsapis[nsapi]->ds_context);
    }
#ifdef DEBUG_GSM_GPRS_DS_UNIT_TEST
    #error code not present
#endif
  ...
  case( ESTABLISHED ):
    {
      ...
      /* Print XID parameters in F3 message */
      gsn_print_xid_params (  xid_req, TRUE );
      ...
      ul_mes->mes.est_req_mes.psm_restore = psm_restore;
      ...
      gsn_ul_suspend_dataflow(  sapi, GSN_UL_SUSP_EST_PEND );

      /* Post message to LLC and log for call flow analysis. */
      MSG_GERAN_LOW_1_G("GSN ""LL %d: Sent LL_ESTABLISH_REQ",gsn_extern_sapi[sapi]);
...
/*-------------------------------------------------------------------------------*/
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd LL_EST_REQ in state: %d",
      gsn_extern_sapi[sapi],sapi_ptr->state);
    break;
...
void gsn_unack_retr_npdu_hdlr(   gsn_nsapi_t nsapi )
{
  ...
  if( nsapi_e_ptr->op_mode != UNACK )
  {
    MSG_GERAN_LOW_1_G("GSN "" ReTr in Ack NSAPI %d",GSN_EXTERN_NSAPI(nsapi));
  }
  else
  {
    ...
    if( npdu_item_ptr )
    {
      for( i = 0; i < cnt; i++ )
      {
        ...
        /* Retransmit N-PDU. */
        gsn_process_sn_udata_req (  nsapi, &npdu_item_ptr->npdu_ptr, FALSE, npdu_item_ptr, pdu_priority );
    ...
    {
      MSG_GERAN_LOW_1_G("GSN ""NSAPI %d: No N-PDUs to re-transmit",GSN_EXTERN_NSAPI(nsapi));
    }
  }
}
...
void gsn_retr_npdu_hdlr(   gsn_nsapi_t nsapi )
{
  ...
  {
    MSG_GERAN_LOW_1_G("GSN "" ReTr in Unack NSAPI %d",GSN_EXTERN_NSAPI(nsapi));
  }
  else
  {
    /* Check if there are any N-PDUs in the buffer */
      ...
      for( i = 0; i < cnt; i++ )
    {
        if( npdu_item_ptr != NULL )
      {
          ...
          */
          ...
          else
          {
            ...
            */
            ...
            pcomp = gsn_pci_compress(  nsapi_e_ptr, &dup_npdu_ptr );
            ...
            if( dup_npdu_ptr != NULL )
            {
              ...
              dcomp = gsn_data_compress(  nsapi_e_ptr, &dup_npdu_ptr );
            }
#endif /* FEATURE_GSM_GPRS_SNDCP_DCOMP */
            ...
            */
            if( dup_npdu_ptr == NULL )
            {
              ...
              MSG_GERAN_LOW_2_G("GSN ""NS %d: comp failed. No comp for N-PDU %d",
                GSN_EXTERN_NSAPI(nsapi), npdu_item_ptr->ack_send_npdu);
            }
          }
        ...
        */
        ...
        {
          MSG_GERAN_LOW_1_G("GSN ""Invalid NPDU in npdu_buff of nsapi %d",
                                                GSN_EXTERN_NSAPI(nsapi));
          break;
        }
      }
...
)
{
  ...
  if( actv_sapis[sapi] != NULL )
  {
    ...
    case LL_RESET_IND:
        ...
        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_RESET_IND",gsn_extern_sapi[sapi]);

      gsn_process_ll_rst_ind(  actv_sapis[sapi], &mes_ptr->rst_ind_mes);

      break;

    case LL_ESTABLISH_IND:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_EST_IND",gsn_extern_sapi[sapi]);

      gsn_process_ll_est_ind(  actv_sapis[sapi], &mes_ptr->est_ind_mes);

      break;

    case LL_ESTABLISH_CNF:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_EST_CNF",gsn_extern_sapi[sapi]);

      gsn_process_ll_est_cnf(  actv_sapis[sapi], &mes_ptr->est_cnf_mes);

      break;

    case LL_RELEASE_IND:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_REL_IND",gsn_extern_sapi[sapi]);

      gsn_process_ll_rel_ind(  actv_sapis[sapi], &mes_ptr->rel_ind_mes);

      break;

    case LL_RELEASE_CNF:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_REL_CNF",gsn_extern_sapi[sapi]);

      gsn_process_ll_rel_cnf(  actv_sapis[sapi], &mes_ptr->rel_cnf_mes);

      break;

    case LL_XID_IND:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_XID_IND",gsn_extern_sapi[sapi]);

      gsn_process_ll_xid_ind(  actv_sapis[sapi], &mes_ptr->xid_ind_mes);

      break;

    case LL_XID_CNF:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_XID_CNF",gsn_extern_sapi[sapi]);

      gsn_process_ll_xid_cnf(  actv_sapis[sapi], &mes_ptr->xid_cnf_mes);

      break;

    case LL_STATUS_IND:

        MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_STATUS_IND",gsn_extern_sapi[sapi]);

      gsn_process_ll_stat_ind(  actv_sapis[sapi], &mes_ptr->status_ind_mes);

      break;

    case LL_DATA_CNF:

      MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_DATA_CNF",gsn_extern_sapi[sapi]);

      gsn_process_ll_data_cnf(  actv_sapis[sapi], &mes_ptr->data_cnf_mes);

      break;

    case LL_UNITDATA_CNF:

      MSG_GERAN_LOW_1_G("GSN ""LL %d: Rcvd LL_UDATA_CNF",gsn_extern_sapi[sapi]);

      gsn_process_ll_udata_cnf(  actv_sapis[sapi], &mes_ptr->udata_cnf_mes);

      break;

    default:

      MSG_GERAN_LOW_2_G("GSN ""LL %d: Inval control primitive %d",
  ...
  if( actv_sapis[sapi] != NULL )
  {
    ...
    if( actv_sapis[sapi]->is_nsapi_ptr_q_valid )
    {
      item_ptr = (gsn_nsapi_q_t*)q_check( &(actv_sapis[sapi]->nsapi_ptr_q) );
      ...
      if( item_ptr )
      {
        ...
        {
#ifndef FEATURE_GERAN_REDUCED_DEBUG
          gsn_encode_cntx_diag_rpt(  (uint8) item_ptr->nsapi_ptr->nsapi );
          gsn_encode_state_diag_rpt(  (uint8) item_ptr->nsapi_ptr->nsapi );
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */

          /* Get next NSAPI from the list */
          item_ptr = (gsn_nsapi_q_t*)q_next( &(actv_sapis[sapi]->nsapi_ptr_q),
                                             &item_ptr->link );
        }
      }
    }
...
===
===========================================================================*/
void gsn_sm_prim_hdlr(   const gsmsn_mes_type * mes_ptr)
{
  switch( mes_ptr->mes.prim )
  {
  case GSNSM_ACTIVATE_IND:

    MSG_GERAN_MED_3_G("GSN ""ME: Rcvd SNSM_ACTIV_IND for NSAPI%d, SAPI%d, psm_restore=%d",
        mes_ptr->mes.snsm_activ_ind.nsapi, mes_ptr->mes.snsm_activ_ind.sapi, mes_ptr->mes.snsm_activ_ind.psm_restore);

    /* Process primitive */
    gsn_process_snsm_actv_ind(  &mes_ptr->mes.snsm_activ_ind );

#ifndef FEATURE_GERAN_REDUCED_DEBUG
    /* Update diag */
    gsn_encode_actv_nsapis_diag_rpt();
    gsn_encode_cntx_diag_rpt(  mes_ptr->mes.snsm_activ_ind.nsapi );
    gsn_encode_state_diag_rpt(  mes_ptr->mes.snsm_activ_ind.nsapi );
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */
  ...
  case GSNSM_DEACTIVATE_IND:
    ...
    /* Process primitive */
    gsn_process_snsm_deactv_ind(  &mes_ptr->mes.snsm_deactiv_ind );
  ...
  case GSNSM_MODIFY_IND:
    ...
    MSG_GERAN_MED_3_G("GSN ""ME: Rcvd SNSM_MODIFY_IND for NSAPI%d, SAPI%d, psm_restore=%d",
      mes_ptr->mes.snsm_modify_ind.nsapi, mes_ptr->mes.snsm_modify_ind.sapi, mes_ptr->mes.snsm_modify_ind.psm_restore);

    /* Process primitive */
    gsn_process_snsm_mod_ind(  &mes_ptr->mes.snsm_modify_ind );
  ...
  case GSNSM_SEQUENCE_IND:
    ...
    /* Process primitive */
    gsn_process_snsm_seq_ind(  &mes_ptr->mes.snsm_seq_ind );
  ...
  default:
    ...
    MSG_GERAN_LOW_1_G("GSN ""ME: Rcvd Invalid prim: %d",mes_ptr->mes.prim);
...
===
===========================================================================*/
void gsn_int_cmd_hdlr(   const gsn_internal_msg_t *mes_ptr)
{
  gsn_sapi_t     sapi;
  gsn_nsapi_t    gnsapi;
  ...
  case( SEND_XID_REQ ):
    ...
    */
    ...
    if( actv_sapis[sapi] != NULL )
    {
      if( actv_sapis[sapi]->state != IDLE )
      {
        /* If ACK SAPI, then re-establish link and include XID Req
        ** in the LL_ESTABLISH_REQ primitive.
        */
        gsn_send_ll_est_req(  actv_sapis[sapi],
          mes_ptr->gsn_cmd.xid_req.xid_pdu, FALSE );
      }
      else
      {
        /* If UNACK SAPI, then send XID Req primitive.
        */
        gsn_send_ll_xid_req(  actv_sapis[sapi],
          mes_ptr->gsn_cmd.xid_req.xid_pdu, FALSE );
      }
    }
    else
  ...
  case( PDP_REG_REQ ):
    ...
    if( sndcp_init == FALSE )
    {
      MSG_GERAN_MED_1_G("GSN " "ME: Un-initialised NSAPI %d",mes_ptr->gsn_cmd.pdp_reg_req.nsapi);
      valid_reg = FALSE;
    ...
    else
    {
           ...
           actv_nsapis[gnsapi] != NULL                            )
      {
        ...
        actv_nsapis[gnsapi]->pdp_ul_suspend_fnc =
          mes_ptr->gsn_cmd.pdp_reg_req.pdp_ul_suspend_fnc_ptr;
        actv_nsapis[gnsapi]->pdp_ul_resume_fnc =
          mes_ptr->gsn_cmd.pdp_reg_req.pdp_ul_resume_fnc_ptr;
        actv_nsapis[gnsapi]->pdp_frw_dl_data_fnc =
          mes_ptr->gsn_cmd.pdp_reg_req.pdp_dl_fnc_ptr;
        actv_nsapis[gnsapi]->pdp_reg_cnf_fnc =
          mes_ptr->gsn_cmd.pdp_reg_req.pdp_reg_cnf_fnc_ptr;
        actv_nsapis[gnsapi]->ds_context =
          mes_ptr->gsn_cmd.pdp_reg_req.ds_context;
        actv_nsapis[gnsapi]->cipher =
          mes_ptr->gsn_cmd.pdp_reg_req.cipher;

        if ( actv_nsapis[gnsapi]->ule.susp_flag )
        {
          /* Reinforce flow control using the newly registered parameters. */
          gsn_set_suspend_event(  gnsapi, actv_nsapis[gnsapi]->ule.susp_flag );
        }
        else
        {
          ...
          if ( ( actv_nsapis[gnsapi]->pdp_reg_cnf_fnc  != NULL  ) &&
               ( actv_nsapis[gnsapi]->deactv_resp_pend == FALSE )
             )
          {
            actv_nsapis[gnsapi]->pdp_reg_cnf_fnc(
              actv_nsapis[gnsapi]->ds_context, valid_reg);
          }
          else if ( ( actv_nsapis[gnsapi]->pdp_reg_cnf_fnc  != NULL ) &&
                    ( actv_nsapis[gnsapi]->deactv_resp_pend == TRUE )
                  )
          {
            actv_nsapis[gnsapi]->pdp_reg_cnf_fnc(
              actv_nsapis[gnsapi]->ds_context, FALSE );

            MSG_GERAN_LOW_1_G("GSN " "NS %d: PDP Reg failed",
                           GSN_EXTERN_NSAPI(gnsapi));
          }

          actv_nsapis[gnsapi]->pdp_reg_cnf_fnc = NULL;
        }
      }
      else
      {
        valid_reg = FALSE;
        MSG_GERAN_LOW_1_G("GSN " "ME: Invalid registration NSAPI %d",mes_ptr->gsn_cmd.pdp_reg_req.nsapi);
  ...
  case (RE_TX_CMD):
    ...
    if(actv_nsapis[ mes_ptr->gsn_cmd.re_tx.nsapi ]->ule.retr)
    {
      gsn_retr_npdu_hdlr( mes_ptr->gsn_cmd.re_tx.nsapi);
    }
    break;
    ...
    if ( GSN_VALID_NSAPI ( GSN_EXTERN_NSAPI(gnsapi) ) )
    {
      if ( actv_nsapis[gnsapi] != NULL )
      {
        ...
        */
        ...
        {
          gsn_send_snsm_stat_req( 
                                  actv_nsapis[gnsapi]->sapi_ptr->sapi,
                                  actv_nsapis[gnsapi]->sapi_ptr->tlli,
                                  SNSM_MODIFY_FAILURE
                                );
        }
        else
        {
          MSG_GERAN_LOW_1_G("GSN " "Invalid timer id %d",
                          mes_ptr->gsn_cmd.timer.tmr_id);
        }
      }
      else
      {
        gsn_debug_print_invalid_nsapi(gnsapi, GSN_GHDLR, __LINE__);
      }
    }
    else
    {
  ...
  default:
    ...
    MSG_GERAN_LOW_1_G("GSN ""Invalid SNDCP cmd id %d",
      mes_ptr->cmd_id);
...
===
===========================================================================*/
void gsn_process_ll_data_cnf(   gsn_sapi_e_t *sapi_ptr, ll_data_cnf_t *ll_mes)
{
  ...
  if(GSN_VALID_NSAPI(ll_mes->sndcp_ref.nsapi ) == FALSE ||
     actv_nsapis[nsapi] == NULL)
  {
    gsn_debug_print_invalid_nsapi(nsapi, GSN_GHDLR, __LINE__);
    return;
  }

  if(actv_nsapis[nsapi]->sapi_ptr->sapi != sapi_ptr->sapi)
  {
    MSG_GERAN_ERROR_1_G("GSN "" Invalid SAPI %d", sapi_ptr->sapi); 
    return;
  }

  MSG_GERAN_MED_4_G("GSN ""LL %d: Rcvd LL_DATA_CNF %d:%d:%d",
    sapi_ptr->sapi, GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
        ...
        */
        ...
        if( npdu_item_ptr->ack_send_npdu == npdu_num )
        {
          if( npdu_item_ptr->seg_ref_buff_valid == TRUE )
          {
...
#ifndef FEATURE_GERAN_REDUCED_DEBUG
                        /* Update diag */
                        gsn_encode_state_diag_rpt(  (uint8) nsapi_e_ptr->nsapi );
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */
                      }
                      GSN_UNLOCK();
                    }
            ...
            {
              MSG_GERAN_LOW_0_G("GSN ""LL_DATA_CNF received while no CNF required");
            }
          }
          else
          {
...
/*-------------------------------------------------------------------------------*/
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: DATA_CNF Rcved in invalid SAPI state: %d",
...
===
===========================================================================*/
void gsn_process_ll_udata_cnf(   gsn_sapi_e_t *sapi_ptr, ll_unitdata_cnf_t *ll_mes)
{
  ...
  if(actv_nsapis[nsapi]->sapi_ptr->sapi != sapi_ptr->sapi)
  {
    MSG_GERAN_ERROR_2_G("GSN Inactive NSAPI %d or invalid SAPI %d", ll_mes->sndcp_ref.nsapi, sapi_ptr->sapi);
    return;
  }

  MSG_GERAN_MED_4_G("GSN ""LL %d: Rcvd LL_UDATA_CNF %d:%d:%d",
    sapi_ptr->sapi, GSN_EXTERN_NSAPI(nsapi), npdu_num, seg_num );
  ...
  while( npdu_item_ptr != NULL )
  {
    ...
    */
    ...
    if( npdu_item_ptr->unack_send_npdu == npdu_num )
    {
      ...
      {
        MSG_GERAN_LOW_0_G("GSN ""No seg ref buff");
      }
    }
...
{
  gsn_npdu_item_t     *item_ptr;
  gsn_nsapi_e_t       *nsapi_ptr = actv_nsapis[nsapi];
  uint8               npdu_num;
  dsm_item_type       *dup_npdu_ptr = NULL;
  uint8               pcomp = 0;
  uint8               dcomp = 0;
  ...
  */
  ...
  else if ( !fback_pdu_flag )
  {
    ...
    */
    ...
    /* Compress PCI of the N-PDU */
    pcomp = gsn_pci_compress(  nsapi_ptr, &dup_npdu_ptr );
    ...
    {
      /* Compress data of the N-PDU */
      dcomp = gsn_data_compress(  nsapi_ptr, &dup_npdu_ptr );
    }
#endif /* FEATURE_GSM_GPRS_SNDCP_DCOMP */
  ...
  *npdu = NULL;
  ...
  if( q_cnt( &(nsapi_ptr->ule.npdu_buff)) >= GPRS_MAX_RETR_NPDUS )
  {
    ...
    */
    gsn_set_suspend_event(  nsapi, GSN_UL_SUSP_MAX_NPDU );
  }
  ...
  gsn_process_ul_ack_npdu(   nsapi, &dup_npdu_ptr, item_ptr, FALSE, pcomp, dcomp, pdu_priority );
...
)
{
  ...
  if( nsapi_ptr->data_comp || nsapi_ptr->pci_comp )
  {
    ...
    */
    ...
    else if ( !fback_pdu_flag )
    {
      ...
      /* Compress PCI of the N-PDU */
      sn_udata_hdr.pcomp = gsn_pci_compress(  nsapi_ptr, &cp_npdu );
      ...
      {
        /* Compress data of the N-PDU */
        sn_udata_hdr.dcomp = gsn_data_compress(  nsapi_ptr, &cp_npdu );
      }
  ...
  */
  ...
  while( len )
  {
    ...
    **  from the head of the N-PDU to form the SN-PDU
    */
    n201_u  = gsn_n201_u[nsapi_ptr->sapi_ptr->sapi];
    ...
    if( udata_msg->l3_pdu == NULL )
    {
      ...
      */
      ...
      if( nsapi_ptr->pci_comp )
      {
        ...
        gsn_unack_npdu_dropped(  nsapi_ptr );
      }
#endif
    ...
    */
    ...
    gsn_encode_unitdata_pdu_hdr(  &sn_udata_hdr, ext_hdr);
    ...
    MSG_GERAN_HIGH_6_G("GSN LL %d: Sent LL_UDATA_REQ %d,%d,%d,%d,%d",
      nsapi_ptr->sapi_ptr->sapi,
      sn_udata_hdr.nsapi,
      sn_udata_hdr.num,
      sn_udata_hdr.seg,
      nsapi_ptr->qos.peak_throughput,
      nsapi_ptr->qos.reliability_class);

    /* Send Primitive to LLC */
    gllc_ll_ul_dat_put_mes_buf(  ll_msg );
  ...
  if( gprs_pdu_strip_head( &ll_mes_ptr->l3_pdu, hdr_pkd, ONE_BYTE) )
  {
    if( hdr_pkd[0] & (0x1 << F_BIT_POS) )
    {
                  ...
                  &hdr_pkd[1], ACK_PDU_HDR_1ST_SEG - ONE_BYTE) )
      {
        ...
        MSG_GERAN_LOW_1_G("GSN ""LL %d: Invalid pdu length in LL_DATA_IND",
                      gsn_extern_sapi[sapi]);
    ...
    */
    ...
    {
      gsn_debug_print_invalid_nsapi(GSN_INTERN_NSAPI(temp_nsapi), GSN_GHDLR, __LINE__);
    ...
    /* temp_nsapi >= 5 is already checked by GSN_VALID_NSAPI above but
       Klocwork disagrees */
    if((temp_nsapi >= 5) &&
     (actv_nsapis[GSN_INTERN_NSAPI(temp_nsapi)] != NULL))
    {
      if( hdr_pkd[0] & (0x1 << F_BIT_POS) )
      {
      ...
      else
      {
        if( hdr_pkd[0] & (0x1 << M_BIT_POS) )
        {
          ...
          actv_nsapis[GSN_INTERN_NSAPI(temp_nsapi)]->dle.first_seg_info.ack_npdu_num );
        }
        else
        {
    ...
    if( gsn_decode_data_pdu_hdr(  hdr_pkd, &sn_hdr) )
    {
      ...
      */
      if( actv_nsapis[sn_hdr.nsapi] != NULL )
      {
#ifndef FEATURE_GERAN_REDUCED_DEBUG
        /* Update Diag statistics */
        actv_nsapis[sn_hdr.nsapi]->diag_pdu_stat_rsp.snpdu_rx_cnt++;
        if( sn_hdr.f == SN_FIRST_SEG )
          actv_nsapis[sn_hdr.nsapi]->diag_pdu_stat_rsp.snpdu_octet_rx_cnt +=
        ...
        n_sapi = actv_nsapis[sn_hdr.nsapi]->sapi_ptr->sapi;

        if( n_sapi == sapi )
        {
          /* Reassemble acknowledged data */
          if( gsn_dle_reas_ack_data(  (gsn_nsapi_t) sn_hdr.nsapi,
                                      &ll_mes_ptr->l3_pdu, &sn_hdr ) )
          {
            /* Check if N-PDU should be forwarded to PDP */
            switch( actv_nsapis[sn_hdr.nsapi]->dle.state )
            {
              case DL_NORMAL:
                ...
                if( GMODULO_LT
                         (
                            ACK_NPDU_NUM_MAX,
                            actv_nsapis[sn_hdr.nsapi]->dle.first_seg_info.ack_npdu_num,
                            actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu
                         )
                  )
                {
                  /* NSAPI has received a duplicated N-PDU */
                  MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd duplicated N-PDU %d",
                  gsn_extern_sapi[sapi],
                  actv_nsapis[sn_hdr.nsapi]->dle.first_seg_info.ack_npdu_num);

                  GPRS_PDU_FREE( &actv_nsapis[sn_hdr.nsapi]->dle.reas_buff );

                  actv_nsapis[sn_hdr.nsapi]->dle.reas_buff = NULL;
                }
                else
                {

                  /* Increment receive N-PDU number */
                  actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu += 1;

                  /* Send N-PDU to PDP */
                  MSG_GERAN_LOW_2_G("GSN " "NS %d: Sent to PDP SN_DATA_IND NPDU %d ",
                                GSN_EXTERN_NSAPI(sn_hdr.nsapi),
                                actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu);

                  gsn_process_pdp_dl_data(  (gsn_nsapi_t) sn_hdr.nsapi,
                                        &actv_nsapis[sn_hdr.nsapi]->dle.reas_buff );
                }
              ...
              case DL_RECOVERY:
                ...
                if( actv_nsapis[sn_hdr.nsapi]->dle.first_seg_info.ack_npdu_num ==
                      actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu )
                {
                  ...
                  if( actv_nsapis[sn_hdr.nsapi]->first_dl_npdu == TRUE )
                    actv_nsapis[sn_hdr.nsapi]->first_dl_npdu = FALSE;

                  /* Back to normal operation */
                  actv_nsapis[sn_hdr.nsapi]->dle.state = DL_NORMAL;

#ifndef FEATURE_GERAN_REDUCED_DEBUG
                  /* Update diag */
                  gsn_encode_state_diag_rpt(  (uint8) sn_hdr.nsapi );
#endif /* !FEATURE_GERAN_REDUCED_DEBUG */
                       ...
                       )
                {
                  ...
                  */
                  ...
                  {
                    actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu =
                    actv_nsapis[sn_hdr.nsapi]->dle.first_seg_info.ack_npdu_num + 1;
                ...
                else
                {
                  ...
                  */
                  MSG_GERAN_LOW_2_G("GSN " "NS %d: N_PDU number %d out of sequence",
                    GSN_EXTERN_NSAPI(sn_hdr.nsapi),
                    actv_nsapis[sn_hdr.nsapi]->dle.ack_rcv_npdu);
              ...
              default:
                ...
                MSG_GERAN_LOW_2_G("GSN ""NS %d: Invalid sate %d of DL Entity ",
                  actv_nsapis[sn_hdr.nsapi]->dle.state,
                    GSN_EXTERN_NSAPI(sn_hdr.nsapi));
                break;

            } /* switch( actv_nsapis[sn_hdr.nsapi].nsapi_e.dle.state ) */
      ...
      {
        gsn_debug_print_invalid_nsapi(sn_hdr.nsapi, GSN_GHDLR, __LINE__);
      }
    }
    else
    {
      /* invalid header ignore PDU without error notification */
      MSG_GERAN_LOW_1_G("GSN ""LL %d: Invalid header of DL LL_DATA PDU ",
                      gsn_extern_sapi[sapi]);
    }
  }
  else
                ...
                hdr_pkd, UNACK_PDU_HDR_NOT_1ST_SEG ) )
  {
    ...
    if( hdr_pkd[0] & (0x1 << F_BIT_POS ) )
    {
                  ...
                  &hdr_pkd[3], ONE_BYTE) )
      {
        ...
        MSG_GERAN_LOW_1_G("GSN ""LL %d: Invalid pdu length in LL_UDATA_IND",
                          gsn_extern_sapi[sapi]);
    ...
    if( gsn_decode_unitdata_pdu_hdr(  hdr_pkd, &sn_hdr) )
    {
      /* Check if NSAPI is active and mapped to the SAPI. In case of modified NSAPI
      ** We might receiving some LL_DATA.IND, LL_DATA.CNF, LL_UNITDATA.IND
      ** on the old SAPI. In this case the above primitives should be ignored
      */
      if( actv_nsapis[sn_hdr.nsapi] != NULL )
      {
        ...
        MSG_GERAN_MED_4_G("GSN LL %d: Rcvd LL_UDATA_IND %d:%d:%d",
          sapi, GSN_EXTERN_NSAPI(sn_hdr.nsapi), sn_hdr.num, sn_hdr.seg );
        ...
        if( n_sapi == sapi )
        {
          ...
          */
          npdu_ref_item_ptr = gsn_dle_reorder_npdu_seg(  &ll_mes_ptr->l3_pdu, &sn_hdr );
          ...
          */
          if( npdu_ref_item_ptr != NULL )
          {
            ...
            while( seg_item_ptr != NULL )
            {
              ...
              if( gsn_dle_reas_unack_data(  
                                            (gsn_nsapi_t) seg_item_ptr->sn_hdr.nsapi,
                                            &seg_item_ptr->seg_ptr,
                                            &seg_item_ptr->sn_hdr ))
              {
                /* Send N-PDU to PDP */
                MSG_GERAN_MED_2_G("GSN " "NS %d: Sent to PDP SN_UNITDATA_IND NPDU %d",
                ...
                if(
                    seg_item_ptr->sn_hdr.num !=
                    actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->dle.unack_rcv_npdu
                  )
                {
                  actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->dle.unack_rcv_npdu
                    = seg_item_ptr->sn_hdr.num;
...
#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP
                  if(
                      actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->pci_comp &&
                      seg_item_ptr->sn_hdr.pcomp
                    )
                  {
                    /* Inform PCI compressor/decompressor about the dropped frame */
                    gsn_unack_npdu_dropped(
                       actv_nsapis[seg_item_ptr->sn_hdr.nsapi] );
                  }
#endif
                }
                ...
                actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->dle.unack_rcv_npdu =
                  ((actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->dle.unack_rcv_npdu + 1) %
                   UNACK_NPDU_NUM_MAX);

                gsn_process_pdp_dl_data
                (
                   (gsn_nsapi_t) seg_item_ptr->sn_hdr.nsapi,
                  &actv_nsapis[seg_item_ptr->sn_hdr.nsapi]->dle.reas_buff
                );
    ...
    {
      /* invalid header ignore PDU without error notification */
      MSG_GERAN_LOW_1_G("GSN ""LL %d: Invalid header of DL SN-UNITDATA PDU  ",
                          gsn_extern_sapi[sapi]);
      if( ll_mes_ptr->l3_pdu != NULL )
        /* Release pdu */
        GPRS_PDU_FREE( &ll_mes_ptr->l3_pdu );
...
===
===========================================================================*/
void gsn_send_ll_xid_req(   gsn_sapi_e_t *sapi_ptr, dsm_item_type  *xid_req, boolean psm_restore )
{
  gsn_nsapi_q_t         *nsapi_q_ptr;
  gsn_sapi_t            sapi = sapi_ptr->sapi;
  gprs_ll_ul_sig_mes_t  *ul_mes;
...
/*-------------------------------------------------------------------------------*/
  ...
  case ( ESTABLISHED):
    ...
    ul_mes->mes.xid_req_mes.psm_restore = psm_restore;
    ...
    if( ul_mes->mes.xid_req_mes.l3_xid_req != NULL )
    {
      ...
      gsn_ul_suspend_dataflow(  sapi, GSN_UL_SUSP_XID_PEND );
    }
...
/*-------------------------------------------------------------------------------*/
  default:
    ...
    MSG_GERAN_LOW_2_G("GSN ""LL %d: Rcvd XID_REQ in state: %d",gsn_extern_sapi[sapi],sapi_ptr->state);
    break;
...
===
===========================================================================*/
void gsn_send_snsm_stat_req(   gsn_sapi_t sapi, uint32 tlli, uint8 cause)
{
  sm_cmd_type  *snsm_dl_mes;
...
void gsn_encode_cntx_diag_rpt(   uint8 nsapi )
{
  ...
  if ( nsapi_cntx_rpt_fnc != NULL )
  {
    ...
    if( GSN_VALID_NSAPI(nsapi))
    {
      ...
      gsn_nsapi_ptr = actv_nsapis[gnsapi];
      ...
      if( gsn_nsapi_ptr != NULL )
      {
        ...
        nsapi_cntx_rpt.n201_i     = gsn_n201_i[gsn_nsapi_ptr->sapi_ptr->sapi];
        nsapi_cntx_rpt.n201_u     = gsn_n201_u[gsn_nsapi_ptr->sapi_ptr->sapi];
