...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gsndcp/src/gsn_comp.c#5 $
***
...
uint8 gsn_1144_compress( pcomp_e_t *, dsm_item_type ** );
#endif

#ifdef FEATURE_GSM_SNDCP_IPHC
uint8 gsn_iphc_compress( pcomp_e_t *pci_e, dsm_item_type **npdu);
#endif

#ifdef FEATURE_GSM_SNDCP_ROHC
uint8 gsn_rohc_compress( pcomp_e_t *pci_e, dsm_item_type **npdu);
#endif
/*****************************************************************************
***
                       ...
                       &pkt_type ) == SLHC_SUCCESS )
  {
    switch(pkt_type )
    {
      ...
      default:
        ...
        MSG_GERAN_LOW_3_G("GSN ""GSN 1144_comp: Bad IP Packet, pkt type %d", pkt_type,
                                                                  0, 
                                                                  0);
        pcomp_val = 0;
        break;
...
)
{
  ...
  {
    init_succ = TRUE;
    MSG_GERAN_LOW_1_G("GSN ""1144 instance created for pcomp entity %d",pcomp_e->ent_num);
  }
  else
  {
    MSG_GERAN_LOW_1_G("GSN ""1144 instance not created for entity %d",pcomp_e->ent_num);
  }
  return( init_succ );
}
...
===  
===========================================================================*/
void gsn_slhc_free_mem(   slhc_handle_type *comp_e )
{

  if( comp_e != NULL )
  {
    slhc_free( (slhc_handle_type *)comp_e );
    MSG_GERAN_LOW_0_G("GSN ""1144 instance freed");
  }
  else
  {
    MSG_GERAN_LOW_0_G("GSN " "1144 entity do not exist, cannot be freed");
  }
}
...
)
{
  ...
  if(result != 0)
  {
    MSG_GERAN_LOW_1_G( "iphc_compress() failed %d",result);
  }
  ...
  switch(pkt_type)
  {
    ...
    default:
      ...
      MSG_GERAN_LOW_1_G("GSN ""GSN 2507_comp: Bad Packet %d", pkt_type);
      pcomp_val = 0;
      break;
  }
...
void gsn_iphc_free(   void *handle)
{
  iphc_free(handle);
  MSG_GERAN_LOW_0_G("GSN ""IPHC instance freed");
}
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("GSN ""iphc memory alloc failed ");
    return FALSE;
  }
...
{
  uint8 pcomp_val = 0;
  MSG_GERAN_LOW_0_G("GSN ""ROHC comp disbled");
  return( pcomp_val );
}
...
void gsn_rohc_free(  void *handle)
{
  MSG_GERAN_LOW_0_G("GSN ""ROHC comp disbled ");
}
    ...
    if(pcomp_e_ptr != NULL)
    {
         ...
         (!pcomp_e_ptr->alg.xid_rfc2507.pcomp5))
      {
        if((pcomp_e_ptr->alg.xid_rfc2507.pcomp1) &&
           ...
           (pcomp_e_ptr->alg.xid_rfc2507.pcomp5))
        {
          MSG_GERAN_LOW_2_G("GSN ""LL %d: Invalid PCOMP entity %d",
            gsn_extern_sapi[nsapi_e_ptr->sapi_ptr->sapi],comp_ent_num);
        }
        else
        {
          MSG_GERAN_LOW_1_G("GSN ""LL %d",
            gsn_extern_sapi[nsapi_e_ptr->sapi_ptr->sapi]);

          MSG_GERAN_LOW_5_G("GSN ""LL %d: No Hdr comp PCOMP vals = %d/%d/%d%d/%d",
            pcomp_e_ptr->alg.xid_rfc2507.pcomp1,
            pcomp_e_ptr->alg.xid_rfc2507.pcomp2,
            pcomp_e_ptr->alg.xid_rfc2507.pcomp3,
            pcomp_e_ptr->alg.xid_rfc2507.pcomp4,
      ...
      {
        /* Compress N-PDU */
        pcomp_val = gsn_iphc_compress(  pcomp_e_ptr, npdu );
  ...
  if(nsapi_e_ptr->pcomp_e_num[T2_ALG_RFC1144] != NO_COMP_ENT )
  {
    ...
    if ( pcomp_e_ptr != NULL )
    {
           ...
           (!pcomp_e_ptr->alg.xid_rfc1144.pcomp2)    )
      {
        ...
        {
          MSG_GERAN_LOW_3_G("GSN ""LL %d: No Hdr comp PCOMP vals = %d/%d",
            gsn_extern_sapi[nsapi_e_ptr->sapi_ptr->sapi],
            pcomp_e_ptr->alg.xid_rfc1144.pcomp1,
            pcomp_e_ptr->alg.xid_rfc1144.pcomp2);
        }
      ...
      {
        /* Compress N-PDU */
        pcomp_val = gsn_1144_compress(  pcomp_e_ptr, npdu );
      }
    }
  }
  else
...
)
{
  ...
  */
  ...
  if( pcomp_ptr != NULL )
  {
    ...
    {
      MSG_GERAN_LOW_1_G("GSN ""Invalid T2 algorthm %",pcomp_ptr->alg);
    }
    else
    {
      ...
      {
        MSG_GERAN_LOW_0_G("GSN ""Invalid PCOMP entity");
      }
      else
      {
        ...
        switch( pcomp_ptr->alg )
        {
          ...
          case T2_ALG_RFC1144:
          {
            ...
            {          
              ...
              {
                MSG_GERAN_LOW_1_G("GSN " "RFC 1144:Err Uncompressing Packet,pkt type %d", 
                               SLHC_PACKET_TYPE_UNCOMPRESSED_TCP);
              }
              else
                decomp_succ = TRUE;
            ...
            {
              MSG_GERAN_LOW_1_G("GSN ""Inval PCOMP value %d for RFC 1144 ",
          ...
          case T2_ALG_RFC2507:
          {
                        ...
                        pcomp_e_ptr->alg.xid_rfc2507.pcomp1 )
            {
              ...
              if(iphc_decompress(pcomp_e_ptr->comp_e,
                                 ...
                                 &snd_pkt) < 0)
              {
                MSG_GERAN_LOW_0_G("GSN " "RFC 2507:Err Decompressing Packet");
              }
              else
                decomp_succ = TRUE;
            }
            ...
            {
              MSG_GERAN_LOW_1_G("GSN ""Inval PCOMP value % for RCF 2507 ",
                            nsapi_e_ptr->dle.first_seg_info.pcomp);
            }
            ...
            if( snd_pkt != NULL )
            {
              ...
              {
                gsn_process_sn_data_req(  nsapi_e_ptr->nsapi, &snd_pkt, TRUE, pdu_priority );
              }
              else if ( nsapi_e_ptr->op_mode == UNACK ) 
              {
                gsn_process_sn_udata_req(  nsapi_e_ptr->nsapi, &snd_pkt, TRUE, NULL, pdu_priority );
              }
              else
              {
                MSG_GERAN_LOW_1_G("GSN ""Inval op mode %d ",nsapi_e_ptr->op_mode);
              }
            }
            break;
          }
          ...
          default:
            MSG_GERAN_LOW_0_G("GSN ""Invalid T2/PCI algorthm");
            break;
        } /* Switch on algorithm type */
      }
    }
  ...
  {
    MSG_GERAN_LOW_1_G("GSN ""Unallocated PCOMP value %",
                  nsapi_e_ptr->dle.first_seg_info.pcomp);
  }
  ...
  */
  ...
  if( pcomp_ptr != NULL )
  {
    ...
    if( ( pcomp_e_ptr == NULL ) ||
        ( pcomp_e_ptr->state != ASSIGNED) )
    {
      MSG_GERAN_LOW_1_G("GSN ""Invalid PCOMP entity: %d",ent_num);
      return;
    }
  }
  else
  ...
  switch( pcomp_ptr->alg )
  {
    ...
    case T2_ALG_RFC2507:
      ...
      MSG_GERAN_LOW_0_G("GSN ""RFC2507 decompression recovers from dropped PDUs");
      break;
#endif
#ifdef FEATURE_GSM_SNDCP_ROHC
    case T2_ALG_RFC3095:
      /* SNDCP need not do anything as RFC3095 algorithm recovers by itself  */
      MSG_GERAN_LOW_0_G("GSN ""RFC3095 decompression recovers from dropped PDUs");
      break;
#endif
    default:
...
===  
===========================================================================*/
void gsn_ack_remove_fback_npdus_and_renumber(   gsn_nsapi_e_t *nsapi_e_ptr)
{
  ...
  if ( nsapi_e_ptr == NULL )
  {
    MSG_GERAN_LOW_0_G("GSN ""nsapi entity ptr is NULL");
    return;
  }
    ...
    if ( !seq_pend_susp )
    {
      ...
      */
      ...
      while( item_ptr != NULL )
      {
        ...
        if ( item_ptr->fback_pdu_flag )
        {
          MSG_GERAN_LOW_0_G("GSN ""Unexpected fback PDU in re-trans buffer");
        }
  ...
  {
    MSG_GERAN_LOW_2_G("GSN ""Fback PDU proc not done: LLC mode %d, Hdr Comp %d ",
    ...
    */
    ...
    if ( dcomp_e_ptr != NULL )
    {
         ...
         )
      {
        ...
        {
          MSG_GERAN_LOW_2_G("GSN ""LL %d: Invalid DCOMP entity %d",
          gsn_extern_sapi[nsapi_e_ptr->sapi_ptr->sapi],comp_ent_num);
        }
        else
        {
          ...
          */
          ...
          if ( V42_OK == gsn_v42_main( &dcomp_e_ptr->comp_e )  )
          {
            ...
            if( (len_comp >= len_decomp) && (nsapi_e_ptr->op_mode == UNACK) )
            {
              ...
              */
              dcomp_val = 0;
              *npdu = gprs_pdu_construct( &dcomp_e_ptr->comp_e.tx_data_buffer.tx_data[0], len_decomp );

              if(*npdu == NULL)
              {
                MSG_GERAN_HIGH_1_G("GSN ""gprs_pdu_construct() failed for len %d",len_decomp);
                return ( 0);
              }
            ...
            else
            {
              ...
              *npdu = gprs_pdu_construct( dcomp_e_ptr->comp_e.TxBuffer.buf, len_comp );

              if(*npdu == NULL)
              {
                MSG_GERAN_HIGH_1_G("GSN ""gprs_pdu_construct() failed for len %d",len_comp);
                return ( 0);
              }
            }
          }
          ...
          if( nsapi_e_ptr->op_mode == UNACK || init_flag == TRUE )
          {
            ...
            */
            ...
            if(  nsapi_e_ptr->op_mode == ACK )
            {
              ...
              MSG_GERAN_LOW_0_G("GSN "" V42bis comp failed in ACK mode. Re-EST SAPI");
            }
          }
...
)
{
  ...
  */
  ...
  if( dcomp_ptr != NULL )
  {
    ...
    {
      MSG_GERAN_LOW_1_G("GSN ""Invalid T1 algorthm %",dcomp_ptr->alg);
      return;
    }
    else
    {
    ...
    {
      MSG_GERAN_LOW_0_G("GSN ""Invalid DCOMP entity");
      return;
    }
  }
  else
  {
    MSG_GERAN_LOW_1_G("GSN ""Unallocated DCOMP value %",
      ...
      (dcomp_e_ptr->alg.xid_v42bis.p0 == GSN_V42_BOTH_DIR) )
  {
    ...
    {  
      len = V42COMP_BUF_LEN;
      MSG_GERAN_LOW_1_G("GSN ""Truncated length to %d",len);
    }
    ...
    *npdu = gprs_pdu_construct( dcomp_e_ptr->comp_e.rx_data_buffer.rx_data, len );

    if(*npdu == NULL)
    {
      MSG_GERAN_HIGH_1_G("GSN ""gprs_pdu_construct() failed for len %d",len);
    }
    ...
    MSG_GERAN_LOW_0_G("GSN ""DCOMP SGSN to MS dir is disabled");
}
