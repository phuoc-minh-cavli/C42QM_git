...
$Header: //components/rel/geran.mpss/5.2.0/gsndcp/src/gsndcplog.c#4 $
...
SNDCP_PACKET_TO_LOG gsn_packet_wanted = SNDCP_PACKET_LOG_ALL;

#define SNDCP_NSAPI_LOG_ALL 0
uint8 nsapi_wanted = SNDCP_NSAPI_LOG_ALL;
...
void sndcp_log_init(void)
{
  ...
  {
      /* Call this only once */
      sndcp_log_initialized = TRUE;
      gsn_packet_wanted     = SNDCP_PACKET_LOG_ALL;
      nsapi_wanted          = SNDCP_NSAPI_LOG_ALL;
...
===========================================================================*/
log_on_demand_status_enum_type sndcp_log_on_demand_dyn(log_code_type log_code)
{
  boolean retVal = TRUE;
...
===========================================================================*/
void sndcp_log_packet_request_dyn(uint16 log_code,
                              uint8  zero_stats,
...
void sndcp_log_active_nsapis_cb(const gsndcp_diag_actv_nsapis_rpt_t *rpt_ptr)
{
  ...
  if ((gsn_packet_wanted != SNDCP_PACKET_LOG_ALL) &&
       (gsn_packet_wanted != SNDCP_PACKET_ACTIVE_PDP_CONTEXTS) )
  {
...
void sndcp_log_nsapi_pdp_context_cb(const gsndcp_diag_nsapi_cntx_rpt_t *rpt_ptr)
{
  ...
  /* Do we want this packet? */
  if ((gsn_packet_wanted != SNDCP_PACKET_LOG_ALL) &&
       (gsn_packet_wanted != SNDCP_PACKET_PDP_CONTEXT) )
  {
  ...
  if (nsapi_wanted != SNDCP_NSAPI_LOG_ALL)
  {
    if (rpt_ptr->nsapi_addr != nsapi_wanted)
    {
      DEBUGMSG("sndcp_log_nsapi_pdp_context_cb> Discard NSAPI %d (%d)",
                rpt_ptr->nsapi_addr, nsapi_wanted, 0 );
      return;
    }
  }
...
void sndcp_log_nsapi_state_cb(const gsndcp_diag_nsapi_state_rpt_t *rpt_ptr)
{
  ...
  /* Do we want this packet? */
  if ((gsn_packet_wanted != SNDCP_PACKET_LOG_ALL) &&
       (gsn_packet_wanted != SNDCP_PACKET_NSAPI_STATE) )
  {
...
void sndcp_log_pdu_stats(uint8 nsapi, uint8 zero_stats)
{
    ...
    if (zero_stats)
    {
        ...
        gsndcp_diag_reset_pdu_stats_dyn(nsapi);
    }

    /* Obtain the latest statistics for this SAPI */
    rsp_ptr = gsndcp_diag_get_pdu_stats_dyn(nsapi);
    if (rsp_ptr == NULL)
    {
