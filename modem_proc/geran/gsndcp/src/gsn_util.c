...
***
*** $Header: //components/rel/geran.mpss/5.2.0/gsndcp/src/gsn_util.c#6 $
***
...
void gsn_get_nsapis_on_sapi(  gsn_sapi_e_t *, uint8 * );
...
void gsn_del_pcomp_e_mapped_to_alg(  gsn_sapi_e_t *, uint8);
...
boolean gsn_check_pcomp_e(  const pcomp_e_t *, gsn_sapi_e_t * );

void gsn_reset_pcomp_ent(   pcomp_e_t *pcomp_e_ptr );

pcomp_e_t *gsn_create_pcomp_e( const gsn_xid_t2_t *, gsn_sapi_e_t * );

void gsn_del_all_pcomp_e(  gsn_sapi_e_t *);

#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */

#ifdef FEATURE_GSM_GPRS_SNDCP_DCOMP

void gsn_del_dcomp_e_mapped_to_alg(   gsn_sapi_e_t *, uint8);
...
boolean gsn_check_dcomp_e(  const dcomp_e_t *, gsn_sapi_e_t * );

void gsn_reset_dcomp_ent(  dcomp_e_t * );

dcomp_e_t *gsn_create_dcomp_e( const gsn_xid_t1_t *, gsn_sapi_e_t * );

void gsn_del_all_dcomp_e(  gsn_sapi_e_t *);
  ...
  if( pcomp_e_ptr != NULL  )
  {
    ...
    if( pcomp_e_ptr ->t2_alg == T2_ALG_RFC1144 )
    {
      ...
      else
      {
        ...
        {
          comp_e_consistent = FALSE;
          gsn_debug_print_invalid_sapi(sapi_ptr->sapi, GSN_GUTIL, __LINE__);
        }
      }
    }
    else
...
#ifdef FEATURE_GSM_SNDCP_IPHC
    if( pcomp_e_ptr ->t2_alg == T2_ALG_RFC2507 )
    {
    ...
    if (pcomp_e_ptr ->t2_alg == T2_ALG_RFC3095 )
    {
    ...
    {
      MSG_GERAN_LOW_1("GSN "" Invalid PCI algorithm %d", pcomp_e_ptr ->t2_alg);
      comp_e_consistent = FALSE;
    }
  }
  else
...
)
{
  ...
  if( dcomp_e_ptr != NULL  )
  {
    ...
    {
      MSG_GERAN_LOW_1("GSN "" Invalid DATA algorithm %d", dcomp_e_ptr->t1_alg);
      comp_e_consistent = FALSE;
    }
  }
  else
...
===
===========================================================================*/
void gsn_reset_dcomp_ent(   dcomp_e_t *dcomp_e_ptr )
{
  if( dcomp_e_ptr->state == ASSIGNED )
  {
    gsn_V42_initialise
...
void gsn_reset_pcomp_ent(   pcomp_e_t *pcomp_e_ptr )
{
  if( pcomp_e_ptr->state == ASSIGNED )
  {
    ...
    if(pcomp_e_ptr->t2_alg == T2_ALG_RFC1144)
    {
      ...
      if(result < 0)
      {
        MSG_GERAN_LOW_1("GSN ""slhc reinit failed: %d",result);
      }
    }
    else
#endif /* FEATURE_GSM_SNDCP_SLHC */
    ...
    if(pcomp_e_ptr->t2_alg == T2_ALG_RFC2507)
    {
      ...
      result = gsn_iphc_ioctl(
                              (void *)pcomp_e_ptr->comp_e,
                              IPHC_IOCTL_COMPRESSOR_V4_SET_OPTIONS,
                              &ioctl_data);
      if(result < 0)
      {
        MSG_GERAN_LOW_1("GSN ""iphc reinit failed: %d",result);
...
#endif /* FEATURE_GSM_SNDCP_IPHC */
    {
      MSG_GERAN_LOW_1("GSN ""Invalid t2 alg %d",pcomp_e_ptr->t2_alg);
    }
  }
}
...
)
{
  ...
  */
  ...
  if( *xid_neg != NULL )
  {
    ...
    {
      MSG_GERAN_LOW_0("GSN ""XID RSP append XID block failed");
    }
  }
  else
  {
    /* Create a pdu item to carry the XID block */
    *xid_neg = gprs_pdu_construct( xid_ex, (uint16)xid_neg_len );
  }
}
...
===
===========================================================================*/
void gsn_del_all_dcomp_e(   gsn_sapi_e_t *sapi_ptr)
{
  ...
  */
  ...
  for( t1_alg = 0; t1_alg <= T1_ALG_MAX; t1_alg++ )
  {
    if( q_cnt( &sapi_ptr->t1_alg_q[t1_alg] ) )
      gsn_del_dcomp_e_mapped_to_alg(  sapi_ptr, (uint8)t1_alg );
  }
}
#endif /* FEATURE_GSM_GPRS_SNDCP_DCOMP */
...
===
===========================================================================*/
void gsn_del_dcomp_e_mapped_to_alg(   gsn_sapi_e_t *sapi_ptr, uint8 t1_alg )
{
        ...
        while( nsapi_q_item != NULL )
        {
              ...
              (q_item->dcomp_e_ptr->t1_alg == T1_ALG_V42BIS) )
          {
                ...
                (0x01 << GSN_EXTERN_NSAPI(nsapi_q_item->nsapi_ptr->nsapi)) )
            {
              ...
              actv_nsapis[nsapi_q_item->nsapi_ptr->nsapi]->
...
)
{
  ...
  */
...
#else
  xid_i->type_field.xid_t2.hdr.alg_type   = T2_ALG_NONE;
#endif
  ...
  {
    if( xid_neg_len != (xid_i->length + XID_TYPE_HDR_LEN) )
      MSG_GERAN_LOW_0("GSN ""XID encode. Error encoding T2 XID params");
  }
  else
  {
    if( xid_neg_len != (APP_NSAPIS_LEN + XID_FIELD_HDR_P0_LEN ) )
...
pcomp_e_t *gsn_create_pcomp_e( const gsn_xid_t2_t *t2, gsn_sapi_e_t *sapi_ptr )
{
...
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
    if( t2->hdr.alg_type == T2_ALG_RFC2507 )
    {
...
#endif /* FEATURE_GSM_SNDCP_IPHC */
#ifdef FEATURE_GSM_SNDCP_ROHC
    if (t2->hdr.alg_type == T2_ALG_RFC3095)
    {
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
    {
      MSG_GERAN_LOW_2("GSN ""XID %d: Inval Comp Algorithm %d",
  ...
  {
    MSG_GERAN_LOW_2("GSN ""XID %d: Inval Entity %d",
                    gsn_extern_sapi[sapi_ptr->sapi],ent_num);
    return( NULL );
  }
...
===
===========================================================================*/
void gsn_del_all_pcomp_e(   gsn_sapi_e_t *sapi_ptr )
{
  uint8       t2_alg;
  ...
  for( t2_alg = 0; t2_alg <= T2_ALG_MAX; t2_alg++ )
  {
    if( q_cnt( &sapi_ptr->t2_alg_q[t2_alg] ) )
      gsn_del_pcomp_e_mapped_to_alg(  sapi_ptr, t2_alg );
  }
}
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */
...
===
===========================================================================*/
void gsn_del_pcomp_e_mapped_to_alg(   gsn_sapi_e_t *sapi_ptr, uint8 t2_alg )
{
    ...
    if( q_cnt( &sapi_ptr->t2_alg_q[t2_alg] ) )
    {
      ...
      */
      ...
      while( q_item != NULL )
      {
...
#endif
#ifdef FEATURE_GSM_SNDCP_IPHC
          if( (q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC2507) &&
...
#endif
#ifdef FEATURE_GSM_SNDCP_ROHC
          if( (q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC3095) &&
        ...
        if( t2_alg == T2_ALG_RFC1144 )
        {
          gsn_slhc_free_mem( (slhc_handle_type *)q_item->pcomp_e_ptr->comp_e );
        }
        else
#endif
#ifdef FEATURE_GSM_SNDCP_IPHC
...
#define XID_BLK_MIN_SIZE 2 /* one byte for XID type and one byte for length */
#define XID_BLK_SIZE_CHECK(curr_bit_pos, bytes_needed, xid_len)                                  \
  if ((curr_bit_pos / XID_BYTE_SIZE + bytes_needed) > xid_len)                                   \
  {                                                                                              \
    MSG_GERAN_LOW_3("GSN ""XID block insufficient data (msg_pos=%d, bytes_needed=%d, len=%d)",   \
      curr_bit_pos, bytes_needed, xid_len);                                                      \
    return FALSE;                                                                                \
  }
...
)
{
  ...
  else
  {
    ...
    MSG_GERAN_LOW_1("GSN ""XID block len %d > XID_BLOCK_LEN_MAX",
                    xid_blck_len);

    return ( FALSE );
  }

  while(xid_blck_len > (msg_pos / XID_BYTE_SIZE + XID_BLK_MIN_SIZE))
  {
    ...
    MSG_GERAN_HIGH_2("GSN xid blk: type = %d, length = %d",dxid_tmp.type, dxid_tmp.length);
    ...
    if( (msg_pos / XID_BYTE_SIZE + dxid_tmp.length) > xid_blck_len )
    {
      /* XID Type length greater than XID block */
      MSG_GERAN_LOW_2("GSN ""XID type len %d > XID block len %d", dxid_tmp.length,xid_blck_len);
    ...
    case XID_TYPE_0:
      ...
      */
      ...
      XID_BLK_SIZE_CHECK(msg_pos, GSN_VER_NUM_LEN, xid_blck_len);
    ...
    case XID_TYPE_1:
      {
        ...
        */
        if( dxid_tmp.length )
        {
          ...
          {
            XID_BLK_SIZE_CHECK(msg_pos, 1, xid_blck_len);
            ...
            */
            ...
            if( dec_dcomp_ent[ent_num] == ent_num )
            {
              ...
              {
                XID_BLK_SIZE_CHECK(msg_pos, (dxid_tmp.type_field.xid_t1.hdr.p ? 2 : 1), xid_blck_len);
            ...
            */
            ...
            XID_BLK_SIZE_CHECK(msg_pos, (ent_xid->hdr.p ? 2 : 1), xid_blck_len);
            if( ent_xid->hdr.p )
            {
            ...
            XID_BLK_SIZE_CHECK(msg_pos, ent_xid->hdr.length, xid_blck_len);
                ...
                (ent_end_pos > t1_end_pos ) )
            {
              ...
              */
              MSG_GERAN_LOW_2("GSN ""Inval T1 Param len %d, T1 blck %d",
            ...
            if( ent_xid->hdr.p && (ent_xid->hdr.alg_type > T1_ALG_MAX) )
            {
              ...
              MSG_GERAN_LOW_1("GSN ""T1 unsupported alg %d",
                ent_xid->hdr.alg_type);
            ...
            ** and the P bit. See TS04.65 table 7(2nd table).*/
            ...
            if( ( ent_xid->hdr.p  &&
                ...
                ( ent_xid->hdr.length > 6)) )
            {
              MSG_GERAN_LOW_1("GSN ""Inval T1 field len %d",
               ent_xid->hdr.length);
        ...
        else
        {
          ...
          MSG_GERAN_LOW_0("GSN "" XID T1 body not included");
    ...
    case XID_TYPE_2:
      {
        ...
        */
          ...
          while(( t2_end_pos > msg_pos ) && ( num_of_ent < ENT_NUM_MAX ))
          {
            ...
            */
            ...
            if( dec_pcomp_ent[ent_num] == ent_num )
            {
              ...
              {
                XID_BLK_SIZE_CHECK(msg_pos, (dxid_tmp.type_field.xid_t2.hdr.p ? 2 : 1), xid_blck_len);
                /* Entity has been decoded. Read length of parameters body first
                ** and advance position of the read pointer to the next entity.
                */
            ...
            */
            ...
            MSG_GERAN_HIGH_3("GSN xid hdr: p-bit = %d, x1-bit = %d, entity num = %d",
              ent_xid->hdr.p,
              ent_xid->hdr.x1,
              ent_xid->hdr.ent_num);
            ...
            if( ent_xid->hdr.p )
            {
              ...
              */
               ...
               MSG_GERAN_HIGH_2("GSN xid hdr: x2-bit = %d, alg type = %d",
                 ent_xid->hdr.x2,
                 ent_xid->hdr.alg_type);
            }
#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP
            ...
            if( ent_end_pos > t2_end_pos )
            {
              ...
              */
              MSG_GERAN_LOW_2("GSN ""Inval T2 Param len %d, T2 blck %d",
            ...
            switch(alg_type)
            {
              ...
              case T2_ALG_RFC1144:
              {
                ...
                if(( ent_xid->hdr.p    &&
                   ...
                   ( ent_xid->hdr.length > 3)))
                {
                  MSG_GERAN_LOW_1("GSN ""Inval T2 len %d", ent_xid->hdr.length);
                ...
                if( ent_xid->hdr.p && ent_xid->hdr.length )
                {
                  ...
                  {
                    MSG_GERAN_LOW_2("GSN ""Inval PCOMP Value (RFC1144) %d, %d",
                      ent_xid->alg.xid_rfc1144.pcomp1,
                      ent_xid->alg.xid_rfc1144.pcomp2);
              ...
              case T2_ALG_RFC2507:
              {
                ...
                if( ent_xid->hdr.p && (ent_xid->hdr.length >= 3))
                {
                  ...
                  {
                    MSG_GERAN_LOW_3("GSN ""Inval PCOMP Val (RFC2507) 1-%d, 2-%d, 3- %d",
                      ent_xid->alg.xid_rfc2507.pcomp1,
                      ent_xid->alg.xid_rfc2507.pcomp2,
                      ent_xid->alg.xid_rfc2507.pcomp3 );

                    MSG_GERAN_LOW_2("GSN ""Inval PCOMP Val (RFC2507) 4-%d, 5-%d",
                      ent_xid->alg.xid_rfc2507.pcomp4,
                      ent_xid->alg.xid_rfc2507.pcomp5);
              ...
              case T2_ALG_RFC3095:
              {
                ...
                if( ent_xid->hdr.p && ent_xid->hdr.length )
                {
                  ...
                  MSG_GERAN_LOW_3("GSN "" xid : pcomp1 %d, pcomp2 %d",
                                          ent_xid->alg.xid_rfc3095.pcomp1,
                                          ent_xid->alg.xid_rfc3095.pcomp2,
                                          0);
                  ...
                  {
                    MSG_GERAN_LOW_2("GSN ""Inval PCOMP Value (RFC3095) %d, %d",
                      ent_xid->alg.xid_rfc3095.pcomp1,
                      ent_xid->alg.xid_rfc3095.pcomp2);
                    ...
                    ( ent_xid->hdr.length >= 16)) )
                {
                  ...
                  MSG_GERAN_LOW_0("Unexpected profiles");
                }
              ...
              default:
              {
                ...
                MSG_GERAN_LOW_1("GSN ""Unsupported T2 alg %d",
                  ent_xid->hdr.alg_type);
        ...
        else
        {
          ...
          MSG_GERAN_LOW_0("GSN "" XID T2 body not included");
    ...
    default:
      ...
      else
      {
        ...
        MSG_GERAN_LOW_1("GSN ""Invalid XID param type : %d ",dxid_tmp.type);
...
boolean gsn_decode_data_pdu_hdr(   uint8 *src, gsn_data_pdu_hdr_t *sn_data)
{
  ...
  if ( sn_data->t )
  {
    ...
    valid_hdr = FALSE;
    MSG_GERAN_LOW_0("GSN ""DL SN-UDATA rcvd while SN-DATA expected");
  }
  ...
  if( GSN_VALID_NSAPI( sn_data->nsapi ) )
  {
    ...
    if( (actv_nsapis[sn_data->nsapi] == NULL) ||
        (actv_nsapis[sn_data->nsapi]->op_mode != ACK) ||
        (actv_nsapis[sn_data->nsapi]->sapi_ptr == NULL) )
    {
      /* Invalid NSAPI */
      valid_hdr = FALSE;
      gsn_debug_print_invalid_nsapi(sn_data->nsapi, GSN_GUTIL, __LINE__);
    }
  }
  else
  {
    ...
    valid_hdr = FALSE;
    gsn_debug_print_invalid_nsapi(GSN_INTERN_NSAPI(sn_data->nsapi), GSN_GUTIL, __LINE__);
  }
  ...
  if(( sn_data->f ) && ( valid_hdr ))
  {
    ...
    */
    ...
    if( actv_nsapis[sn_data->nsapi]->dle.reas_state == DL_RCV_1ST_SEQ )
    {
      if( sn_data->pcomp >= PCOMP_VAL_MAX )
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN ""DL LL-DATA rcvd inval PCOMP %d ", sn_data->pcomp);
      }

#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP

      else if( (sn_data->pcomp ) &&
          (actv_nsapis[sn_data->nsapi]->sapi_ptr->pcomp_pool[sn_data->pcomp] == NULL) )
      {
        valid_hdr = FALSE;
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN " "DL LL-DATA rcvd with unallocated PCOMP %d", sn_data->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN ""DL LL-DATA rcvd inval DCOMP %d",
                      sn_data->dcomp);
      }
          ...
          (actv_nsapis[sn_data->nsapi]->sapi_ptr->dcomp_pool[sn_data->dcomp] == NULL) )
      {
        valid_hdr = FALSE;
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN " "DL LL-UDATA rcvd with unallocated DCOMP %d", sn_data->dcomp);
      }
...
boolean gsn_decode_unitdata_pdu_hdr(   uint8 *src, gsn_unitdata_pdu_hdr_t *sn_udata)
{
  uint16    msg_pos   = 0;
  boolean   valid_hdr = TRUE;
  ...
  **  Get SN-UNITDATA PDU header fields
  ...
  if( !(sn_udata->t) )
  {
    ...
    valid_hdr = FALSE;
    MSG_GERAN_LOW_0("GSN ""DL SN-DATA rcvd while SN-UDATA expected");
  }
  ...
  if( GSN_VALID_NSAPI( sn_udata->nsapi ) )
  {
    ...
    if( (actv_nsapis[sn_udata->nsapi] == NULL) ||
        (actv_nsapis[sn_udata->nsapi]->op_mode != UNACK) ||
        (actv_nsapis[sn_udata->nsapi]->sapi_ptr == NULL) )
    {
      /* Invalid NSAPI number */
      valid_hdr = FALSE;
      gsn_debug_print_invalid_nsapi(sn_udata->nsapi, GSN_GUTIL, __LINE__);
    }
  }
  else
  {
    /* Invalid NSAPI number */
    valid_hdr = FALSE;
    gsn_debug_print_invalid_nsapi(GSN_INTERN_NSAPI(sn_udata->nsapi), GSN_GUTIL, __LINE__);
  }
  ...
  if( sn_udata->f )
  {
    ...
    */
    ...
    if( actv_nsapis[sn_udata->nsapi]->dle.reas_state == DL_RCV_1ST_SEQ )
    {
      if( sn_udata->pcomp >=  PCOMP_VAL_MAX )
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN ""DL LL-UDATA rcvd inval PCOMP %d ",sn_udata->pcomp);
      }

#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP

      else if( (sn_udata->pcomp ) &&
          (actv_nsapis[sn_udata->nsapi]->sapi_ptr->pcomp_pool[sn_udata->pcomp] == NULL) )
      {
        valid_hdr = FALSE;
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN " "DL LL-UDATA rcvd with unallocated PCOMP %d", sn_udata->pcomp);
      }
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN ""DL LL-UDATA rcvd inval DCOMP %d ",sn_udata->dcomp);
      }

#ifdef FEATURE_GSM_GPRS_SNDCP_DCOMP

      else if( (sn_udata->dcomp ) &&
          (actv_nsapis[sn_udata->nsapi]->sapi_ptr->dcomp_pool[sn_udata->dcomp] == NULL) )
      {
        valid_hdr = FALSE;
      ...
      {
        valid_hdr = FALSE;
        MSG_GERAN_LOW_1("GSN " "DL LL-UDATA rcvd with unallocated DCOMP %d", sn_udata->dcomp);
      }
...
void gsn_encode_unitdata_pdu_hdr(   const gsn_unitdata_pdu_hdr_t *hdr_ptr, uint8 *dst)
{
  uint16 msg_pos = 0;    /* offset from the beginning of the message to the current item */
...
)
{
  ...
  gsn_get_nsapis_on_sapi(  sapi_ptr, nsapis_on_sapi );


  /* Print XID parameters in F3 message */
  gsn_print_xid_params (  *l3_xid, FALSE );
  ...
  else
  {
    ...
    if( !gsn_decode_peer_xid(  l3_xid, &dec_xid, sapi_ptr, TRUE ) )
    {
  ...
  if( dec_xid.t2.status !=  XID_ABSENT )
  {
    ...
    if( dec_xid.t2.status == XID_VALID )
    {
      for( i = 0; i < dec_xid.t2.ent_in_blk; i++ )
      {
        ...
        {
          uint16 pcomp_nsapis = 0;

#ifdef FEATURE_GSM_SNDCP_SLHC
          pcomp_nsapis = dec_xid.t2.ent[i].xid.alg.xid_rfc1144.nsapis;
#endif

#ifdef FEATURE_GSM_SNDCP_IPHC
          if (pcomp_nsapis == 0)
            pcomp_nsapis = dec_xid.t2.ent[i].xid.alg.xid_rfc2507.nsapis;
#endif

#ifdef FEATURE_GSM_SNDCP_ROHC
          if (pcomp_nsapis == 0)
            pcomp_nsapis = dec_xid.t2.ent[i].xid.alg.xid_rfc3095.nsapis;
#endif
            ...
            */
            ...
            if( pcomp_e_ptr->t2_alg == T2_ALG_RFC1144 )
            {
              ...
              if( !dec_ent->xid.alg.xid_rfc1144.nsapis )
              {
                ...
                gsn_del_pcomp_e(  pcomp_e_ptr->ent_num, sapi_ptr );
                MSG_GERAN_LOW_2("GSN ""XID %d: Rmv PCOMP entity %d",
                    gsn_extern_sapi[sapi_ptr->sapi],ent_num);
                  ...
                  */
                  for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
                  {
                    ...
                    if( app_nsapis & ( 0x1 << GSN_EXTERN_NSAPI(nsapi) ) )
                    {
                      ...
                      if( actv_nsapis[nsapi] )
                        actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC1144]= NO_COMP_ENT;
                    }
                  }
                }
              }
              ...
              */
              if( pcomp_e_ptr->state == SELECTED )
              {
                ...
                else
                {
                  ...
                  ** entity.
                  */
                  MSG_GERAN_LOW_1("GSN "" XID %d: PCOMP requested on different alg ",
                    gsn_extern_sapi[sapi_ptr->sapi]);
                  return( FALSE );
                }
              ...
              */
              for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
              {
                    ...
                    (0x1 << GSN_EXTERN_NSAPI(nsapi)) )
                {
                  if( actv_nsapis[nsapi] != NULL )
                    actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC1144] =
                      pcomp_e_ptr->ent_num;
                  else
                    gsn_debug_print_invalid_nsapi(nsapi, GSN_GUTIL, __LINE__);
                }
              }

              /* Before start using the entity perform the consistency test*/
              if(!gsn_check_pcomp_e(  pcomp_e_ptr, sapi_ptr ) )
              {
              ...
              if( !gsn_1144_comp_init( pcomp_e_ptr,
                   (uint8) (pcomp_e_ptr->alg.xid_rfc1144.s0_1 + 1) ) )
              {
                ...
                */
                ...
                xid_i.length = 0;
                gsn_bld_rej_t2_xid(  &xid_i, xid_ex, &xid_req );
            ...
            if(pcomp_e_ptr->t2_alg == T2_ALG_RFC2507)
            {
                  ...
                  */
                  for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
                  {
                    ...
                    if( app_nsapis & ( 0x1 << GSN_EXTERN_NSAPI(nsapi) ) )
                    {
                      /* Remove comp entity from NSAPI if it has been using the entity */
                      if( actv_nsapis[nsapi] )
                        actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC2507]= NO_COMP_ENT;
                    }
                  }
                }
              }
              ...
              */
              for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
              {
                ...
                {
                  if( actv_nsapis[nsapi] != NULL )
                    actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC2507] =
                      pcomp_e_ptr->ent_num;
                  else
                    gsn_debug_print_invalid_nsapi(nsapi, GSN_GUTIL, __LINE__);
                }
            ...
            if(pcomp_e_ptr->t2_alg == T2_ALG_RFC3095)
            {
                  ...
                  */
                  for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
                  {
                    ...
                    if( app_nsapis & ( 0x1 << GSN_EXTERN_NSAPI(nsapi) ) )
                    {
                      /* Remove comp entity from NSAPI if it has been using the entity */
                      if( actv_nsapis[nsapi] )
                        actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC3095]= NO_COMP_ENT;
                    }
                  }
                }
              }
              ...
              */
              for( i = 0; i < (uint8) GSN_NSAPI_MAX; i++ )
              {
                ...
                {
                  if( actv_nsapis[nsapi] != NULL )
                    actv_nsapis[nsapi]->pcomp_e_num[T2_ALG_RFC3095] =
                      pcomp_e_ptr->ent_num;
                  else
                    gsn_debug_print_invalid_nsapi(nsapi, GSN_GUTIL, __LINE__);
                }
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
            {
              MSG_GERAN_LOW_0("GSN ""Invalid T2 alg on selected comp entity ");
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_PCOMP */
      ...
      while( q_item != NULL )
      {
        if( q_item->pcomp_e_ptr->state == SELECTED )
        {
          ...
          if(q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC1144)
          {
            ...
            {
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc1144.pcomp1]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc1144.pcomp1]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc1144.pcomp2]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc1144.pcomp2]->state = ASSIGNED;
              }
              else
              {
                /* PCOMP values are not SELECTED.
                */
              MSG_GERAN_LOW_1("GSN "" XID %d: PCOMP values are't SELECTED ",
                  gsn_extern_sapi[sapi_ptr->sapi]);
                return( FALSE );
              }
              /* Entity number is assigned and can be used by the App NSAPIs */
          ...
          if (q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC2507)
          {
            ...
            {
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp1]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp1]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp2]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp2]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp3]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp3]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp4]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp4]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp5]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc2507.pcomp5]->state = ASSIGNED;
              }
              else
              {
                /* PCOMP values are not SELECTED. */
          ...
          if (q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC3095)
          {
            ...
            {
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc3095.pcomp1]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc3095.pcomp1]->state = ASSIGNED;
              }
              if(sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc3095.pcomp2]->state == SELECTED )
              {
                sapi_ptr->pcomp_pool[q_item->pcomp_e_ptr->alg.xid_rfc3095.pcomp2]->state = ASSIGNED;
              }
              else
              {
                /* PCOMP values are not SELECTED. */
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
          {
            MSG_GERAN_LOW_0("GSN ""Invalid algorithm type");
          }
          ...
          /* Before start using the entity perform the consistency test*/
          if(!gsn_check_pcomp_e(  q_item->pcomp_e_ptr, sapi_ptr ) )
          {
            MSG_GERAN_LOW_2("GSN "" XID: PCI comp entity %d params not consistant",
          ...
          {
            /* Initialise RFC 1144 compression entity */
            if(!gsn_1144_comp_init( q_item->pcomp_e_ptr,
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
          {
            MSG_GERAN_LOW_0("GSN ""Invalid T2 alg");
          }
        }
...
#else
          {
            ...
            */
            if( dcomp_e_ptr->t1_alg == T1_ALG_V42BIS )
            {
              ...
              {
                /* No App NSAPIs are set to "1". Delete the comp entity */
                gsn_del_dcomp_e(  dcomp_e_ptr->ent_num, sapi_ptr );
                  ...
                  */
                  for( i = 0; i < (uint8)GSN_NSAPI_MAX; i++ )
                  {
                    ...
                    if( app_nsapis & ( 0x1 << GSN_EXTERN_NSAPI(nsapi) ) )
                    {
                      /* Remove comp entity from NSAPI, if it has been using the entity */
                      if( actv_nsapis[nsapi] )
                        actv_nsapis[nsapi]->dcomp_e_num[T1_ALG_V42BIS]= NO_COMP_ENT;
                    }
                  }
                }
                /* Update entity's parameter value */
              ...
              */
              if( dcomp_e_ptr->state == SELECTED )
              {
                ...
                else
                {
                  ...
                  ** entity.
                  */
                  MSG_GERAN_LOW_1("GSN "" XID %d: DCOMP1 requested on different alg ",
                    gsn_extern_sapi[sapi_ptr->sapi]);
                  return( FALSE );
                }
              ...
              */
              for( i = 0; i < (uint8)GSN_NSAPI_MAX; i++ )
              {
                ...
                {
                  if( actv_nsapis[nsapi] != NULL )
                    actv_nsapis[nsapi]->dcomp_e_num[T1_ALG_V42BIS] =
                      dcomp_e_ptr->ent_num;
                  else
                    gsn_debug_print_invalid_nsapi(nsapi, GSN_GUTIL, __LINE__);
                }
              ...
              /* Before start using the entity perform the consistency test*/
              if(!gsn_check_dcomp_e(  dcomp_e_ptr, sapi_ptr ) )
              {
                MSG_GERAN_LOW_2("GSN "" XID: Data comp entity %d params not consistant",
                    gsn_extern_sapi[sapi_ptr->sapi], dcomp_e_ptr->ent_num);
                  return( FALSE );
              }

              /* Initialise V42 bis compression entity */
              if( !gsn_v42bis_comp_init(  dcomp_e_ptr ) )
              {
                /* Heap Exhaustion. Remove compression entity */
                ...
                */
                ...
                /* Do not include Type header. More entities might follow */
                xid_i.length = 0;
                gsn_bld_rej_t1_xid(  &xid_i, xid_ex, &xid_req );
            ...
            {
              MSG_GERAN_LOW_0("GSN ""Invalid T1 alg on selected comp entity ");
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_DCOMP */
        ...
        if( q_item->dcomp_e_ptr->state == SELECTED )
        {
          ...
          if( (sapi_ptr->dcomp_pool[q_item->dcomp_e_ptr->alg.xid_v42bis.dcomp1] != NULL) )
          {
            ...
            else
            {
              ...
              */
              MSG_GERAN_LOW_1("GSN "" XID %d: DCOMP values is not SELECTED ",
                gsn_extern_sapi[sapi_ptr->sapi]);
              return( FALSE );
            }
          ...
          */
          ...
          /* Before start using the entity perform the consistency test*/
          if(!gsn_check_dcomp_e(  q_item->dcomp_e_ptr, sapi_ptr ) )
          {
          ...
          /* Initialise V42 bis compression entity */
          if( !gsn_v42bis_comp_init(  q_item->dcomp_e_ptr ) )
          {
            /* Heap Exhaustion. Remove compression entity */
...
)
{
  ...
  if( !gsn_decode_peer_xid(  l3_xid, &dec_xid, sapi_ptr, FALSE ) )
  {
  ...
  if( dec_xid.t2.status !=  XID_ABSENT )
  {
        ...
        (dec_xid.t2.ent_in_blk > ENT_NUM_MAX +1 ) )
    {
      ...
      xid_i.length = XID_FIELD_HDR_P0_LEN + APP_NSAPIS_LEN;
      gsn_bld_rej_t2_xid(  &xid_i, xid_ex, &xid_rsp );
    }
          ...
          */
          ...
          else
          {
            ...
            if( sapi_ptr->pcomp_e_pool[dec_xid.t2.ent[i].xid.hdr.ent_num] != NULL )
              gsn_del_pcomp_e(  dec_xid.t2.ent[i].xid.hdr.ent_num, sapi_ptr);
...
#else
          {
                ...
                if( sapi_ptr->pcomp_e_pool[ent_num]->state == SELECTED )
                {
                  ...
                  */
                    ...
                    if( sapi_ptr->pcomp_e_pool[ent_num]->t2_alg == T2_ALG_RFC1144 )
                    {
                      ...
                      */
                      ...
                      for( idx=0; idx < (uint8) GSN_NSAPI_MAX; idx++ )
                      {
                        ...
                        if( actv_nsapis[nsapis[idx]] )/* NSAPIs can use the comp entity */
                          actv_nsapis[nsapis[idx]]->pcomp_e_num[T2_ALG_RFC1144] = ent_num;
                      }
                    }
                    else
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
                    if( sapi_ptr->pcomp_e_pool[ent_num]->t2_alg == T2_ALG_RFC2507 )
                    {
                      if( (sapi_ptr->pcomp_pool[sapi_ptr->pcomp_e_pool[ent_num]->
                      ...
                      {
                        MSG_GERAN_ERROR_5("GSN "" Inval PCOMP1 %d, PCOMP2 %d PCOMP3 %d PCOMP4 %d, PCOMP5 %d",
                      ...
                      */
                      ...
                      for( idx=0; idx < (uint8) GSN_NSAPI_MAX; idx++ )
                      {
                        ...
                        if( actv_nsapis[nsapis[idx]] )/* NSAPIs can use the comp entity */
                          actv_nsapis[nsapis[idx]]->pcomp_e_num[T2_ALG_RFC2507] = ent_num;
                      }
                    }
                    else
#endif /* FEATURE_GSM_SNDCP_IPHC */
#ifdef FEATURE_GSM_SNDCP_ROHC
                    if( sapi_ptr->pcomp_e_pool[ent_num]->t2_alg == T2_ALG_RFC3095 )
                    {
                      if( (sapi_ptr->pcomp_pool[sapi_ptr->pcomp_e_pool[ent_num]->
                      ...
                      */
                      ...
                      for( idx=0; idx < (uint8) GSN_NSAPI_MAX; idx++ )
                      {
                        ...
                        if( actv_nsapis[nsapis[idx]] )/* NSAPIs can use the comp entity */
                          actv_nsapis[nsapis[idx]]->pcomp_e_num[T2_ALG_RFC3095] = ent_num;
                      }
                    }
                    else
#endif /* FEATURE_GSM_SNDCP_ROHC */
                    {
                      MSG_GERAN_LOW_3("GSN "" Invalid t2 alg %d",
                  ...
                  else  /* p == 1 */
                  {
                    ...
                    gsn_del_pcomp_e(  ent_num, sapi_ptr );
...
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
                if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC2507)
                {
                  if((sapi_ptr->pcomp_pool
                  ...
                  if((sapi_ptr->pcomp_pool
                     ...
                     (sapi_ptr->pcomp_pool
                      [dec_ent->xid.alg.xid_rfc2507.pcomp2]->alg    !=
                       dec_ent->xid.hdr.alg_type) )
                  {
                    gsn_del_pcomp_e_mapped_to_alg(  sapi_ptr,
                                   sapi_ptr->pcomp_pool
                                   [dec_ent->xid.alg.xid_rfc2507.pcomp2]->alg );
                  }
...
#endif /* FEATURE_GSM_SNDCP_IPHC */
#ifdef FEATURE_GSM_SNDCP_ROHC
                if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC3095)
                {
                  if((sapi_ptr->pcomp_pool
            ...
            if( dec_ent->xid.hdr.p && dec_ent->xid.hdr.length )
            {
              ...
              if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC1144)
              {
                    ...
                    [dec_ent->xid.alg.xid_rfc1144.pcomp2]->state == SELECTED) ) )
                {
                  ...
                  MSG_GERAN_LOW_1("GSN ""LL %d: PCOMP val SELECTED after collision res",
                        gsn_extern_sapi[sapi_ptr->sapi]);
                }
            ...
            */
                     ...
                     (sapi_ptr->pcomp_e_pool[ent_num]->state   == SELECTED) )
            {
              ...
              MSG_GERAN_LOW_1("GSN ""LL %d: Ent SELECTED after collision res",
                      gsn_extern_sapi[sapi_ptr->sapi]);
            }
            ...
            if(dec_ent->xid.hdr.alg_type == T2_ALG_RFC1144)
            {
                ...
                if( app_nsapis )
                {
                  ...
                  else
                  {
                    for( idx = 0; idx < (uint8) GSN_NSAPI_MAX; idx++ )
                    {
                      ...
                      {
                        /* Remove comp entity from NSAPI if it has been using the entity */
                        if( actv_nsapis[nsapis_on_sapi[idx]] )
                          actv_nsapis[nsapis_on_sapi[idx]]->
                           pcomp_e_num[T2_ALG_RFC1144]= NO_COMP_ENT;
                      }
                    }
                  }
                  ...
                  for( idx = 0; idx < (uint8) GSN_NSAPI_MAX; idx++ )
                  {
                    ...
                    if( app_nsapis & ( 0x1 << GSN_EXTERN_NSAPI(nsapis_on_sapi[idx]) ) )
                    {
                      ...
                      */
                      if( actv_nsapis[nsapis_on_sapi[idx]] )
                      {
                        if( (actv_nsapis[nsapis_on_sapi[idx]]->op_mode ==
                              sapi_ptr->pcomp_e_pool[ent_num]->op_mode) &&
                          (actv_nsapis[nsapis_on_sapi[idx]]->pci_comp) )
                        {
                          actv_nsapis[nsapis_on_sapi[idx]]->
              ...
              else
              {
                ...
                */
                    ...
                    ((uint8)nsapis[idx] != GSN_NSAPI_MAX) ; idx++ )
                {
                  ...
                  if( (actv_nsapis[nsapis[idx]] != NULL)  &&
                      (actv_nsapis[nsapis[idx]]->pci_comp) )
                  {
                    if( actv_nsapis[nsapis[idx]]->op_mode == ACK)
                      ack_nsapis = TRUE;
                    else
                      unack_nsapis = TRUE;
                  }
                ...
                */
                     ...
                     sapi_ptr->pcomp_e_pool[ent_num]->alg.xid_rfc1144.s0_1) )
                {
                  ...
                  else if( sapi_ptr->pcomp_e_pool[ent_num]->op_mode == UNACK )
                  {
                    ...
                    gsn_reset_pcomp_ent(  sapi_ptr->pcomp_e_pool[ent_num] );
                  }
                }
              }
              else
                  ...
                  [dec_ent->xid.alg.xid_rfc1144.pcomp2]  == NULL)  )
              {
                ...
                if( dec_ent->xid.alg.xid_rfc1144.pcomp1 )
                {
                  ...
                  {
                    MSG_GERAN_LOW_2("GSN ""XID %d: Inval PCOMP val %d",
                      ...
                      [dec_ent->xid.alg.xid_rfc1144.pcomp2]  == NULL)  )
              {
                 ...
                 */
                MSG_GERAN_LOW_0("GSN "" Invalid state of PCOMP values");
                rmv_ent = TRUE;
              }
              ...
              /* Before start using the entity perform the consistency test*/
              if(!gsn_check_pcomp_e(  sapi_ptr->pcomp_e_pool[ent_num], sapi_ptr ) )
              {
                MSG_GERAN_LOW_2("GSN "" XID: PCI comp entity %d params not consistant",
                  gsn_extern_sapi[sapi_ptr->sapi], ent_num);
              }
              ...
              {
                /* Create a pdu item to carry the XID block */
                xid_rsp = gprs_pdu_construct( xid_ex, xid_neg_len );
              }
            }
            else
#endif /* FEATURE_GSM_SNDCP_SLHC */
              ...
              if( sapi_ptr->pcomp_e_pool[ent_num]  == NULL )
              {
                ...
                if( gsn_create_pcomp_e( &xid_i.type_field.xid_t2, sapi_ptr ) )
                {
                  ...
                  */
                  ...
                  /* Initialise RFC 3095 compression entity */
                  if(!gsn_rohc_comp_init(
                      (void *)sapi_ptr->pcomp_e_pool[ent_num],
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
            {
              MSG_GERAN_LOW_1("GSN ""Invalid t2 alg %d",
                    sapi_ptr->pcomp_e_pool[ent_num]->t2_alg);
            }
          }
#endif /*  FEATURE_GSM_GPRS_SNDCP_PCOMP */
  ...
  if( dec_xid.t1.status !=  XID_ABSENT )
  {
        ...
        (dec_xid.t1.ent_in_blk > ENT_NUM_MAX + 1) )
    {
      ...
      xid_i.length = XID_FIELD_HDR_P0_LEN + APP_NSAPIS_LEN;
      gsn_bld_rej_t1_xid(   &xid_i, xid_ex, &xid_rsp );
    }
          ...
          */
          ...
          else
          {
            ...
            if( sapi_ptr->dcomp_e_pool[dec_xid.t1.ent[i].xid.hdr.ent_num] != NULL )
              gsn_del_dcomp_e(  dec_xid.t1.ent[i].xid.hdr.ent_num, sapi_ptr);
                ...
                if( sapi_ptr->dcomp_e_pool[ent_num]->state == SELECTED )
                {
                  ...
                  */
                  if( dec_ent->xid.hdr.p == 0 )
                  {
                    ...
                    if( sapi_ptr->dcomp_e_pool[ent_num]->t1_alg == T1_ALG_V42BIS )
                    {
                            ...
                            [sapi_ptr->dcomp_e_pool[ent_num]->alg.xid_v42bis.dcomp1] != NULL )
                      {
                        ...
                        MSG_GERAN_LOW_1("GSN "" Invalid DCOMP value %d", sapi_ptr->dcomp_e_pool[ent_num]->
                      ...
                      */
                      ...
                      for( idx=0; idx < (uint8)GSN_NSAPI_MAX; idx++ )
                      {
                        ...
                        if( actv_nsapis[nsapis[idx]] )/* NSAPIs can use the comp entity */
                          actv_nsapis[nsapis[idx]]->dcomp_e_num[T1_ALG_V42BIS] = ent_num;
                      }
                    }
                  }
                  else  /* p == 1 */
                  {
                    ...
                    gsn_del_dcomp_e(  ent_num, sapi_ptr );
            ...
            */
            ...
            if( dec_ent->xid.hdr.p && dec_ent->xid.hdr.length  )
            {
              ...
              */
              ...
              {
                /* After collision reslolution there shouldn't be any SELECTED DCOMP values */
                MSG_GERAN_LOW_1("GSN ""LL %d: DCOMP val SELECTED after collision res",
                        gsn_extern_sapi[sapi_ptr->sapi]);
              }
            ...
            */
                    ...
                    */
                    if( actv_nsapis[nsapis_on_sapi[idx]] )
                    {
                      if( (actv_nsapis[nsapis_on_sapi[idx]]->op_mode ==
                          ...
                          (actv_nsapis[nsapis_on_sapi[idx]]->data_comp) )
                      {
                        actv_nsapis[nsapis_on_sapi[idx]]->
            ...
            else
            {
              ...
              */
              ...
              for( idx=0; (idx < (uint8)GSN_NSAPI_MAX) && ((uint8)nsapis[idx] != GSN_NSAPI_MAX) ; idx++ )
              {
                ...
                if( (actv_nsapis[nsapis[idx]] != NULL)  &&
                    (actv_nsapis[nsapis[idx]]->data_comp) )
                {
                  if( actv_nsapis[nsapis[idx]]->op_mode == ACK)
                    ack_nsapis = TRUE;
                  else
                ...
                ( dec_ent->xid.hdr.length >= 3)) )
            {
              ...
              */
                     ...
                     sapi_ptr->dcomp_e_pool[ent_num]->alg.xid_v42bis.p0) )
              {
                ...
                {
                  /* Reset localy the comp entity */
                  gsn_reset_dcomp_ent(  sapi_ptr->dcomp_e_pool[ent_num] );
                }
            ...
            if( sapi_ptr->dcomp_e_pool[ent_num]  == NULL )
            {
              ...
              if( gsn_create_dcomp_e( &xid_i.type_field.xid_t1, sapi_ptr ) )
              {
                ...
                */
                ...
                /* Initialise V42 bis compression entity */
                if( !gsn_v42bis_comp_init(  sapi_ptr->dcomp_e_pool[ent_num] ) )
                {
                ...
                [dec_ent->xid.alg.xid_v42bis.dcomp1]  == NULL )
            {
              ...
              if( dec_ent->xid.alg.xid_v42bis.dcomp1 )
              {
                ...
                {
                  MSG_GERAN_LOW_2("GSN ""XID %d: Inval DCOMP val %d",
  ...
  else
  {
    switch( xid_src->type )
    {
      ...
      case XID_TYPE_1:
        ...
        default:
          MSG_GERAN_LOW_0("GSN ""XID encode. Invalid XID T1 algorithm type ");
          break;
        }
      ...
      case XID_TYPE_2:
        ...
        */
        ...
        default:
          MSG_GERAN_LOW_0("GSN ""XID encode. Invalid XID T2 algorithm type ");
          break;
        }
      ...
      default:
        MSG_GERAN_LOW_0("GSN ""XID encode. Invalid XID type");
        break;
    }
...
===
===========================================================================*/
dsm_item_type* gsn_encode_req_xid(   const gsn_nsapi_e_t *nsapi_ptr )
{
...
#if( (defined(FEATURE_GSM_GPRS_SNDCP_PCOMP)) || (defined(FEATURE_GSM_GPRS_SNDCP_DCOMP)) )
  {
    ...
    if( ( nsapi_ptr->pci_comp == TRUE) || ( nsapi_ptr->data_comp == TRUE) )
    {
      ...
      */
      ...
      ver_xid_req_ptr = gprs_pdu_construct( t0_xid_par, XID_T0_LEN_MAX );
    }
  }
      ...
      */
      ...
      else
      {
        ...
        */
        ...
        if( nsapi_ptr->sn_xid_req.t2_alg == T2_ALG_RFC1144 )
        {
          ...
          */
          if( q_cnt(&nsapi_ptr->sapi_ptr->t2_alg_q[T2_ALG_RFC1144]) )
          {
            ...
            {
              MSG_GERAN_LOW_0("GSN ""Invalid PCI comp entity in T2 queue");
...
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
        if( nsapi_ptr->sn_xid_req.t2_alg == T2_ALG_RFC2507 )
        {
...
#endif /* FEATURE_GSM_SNDCP_IPHC */
#ifdef FEATURE_GSM_SNDCP_ROHC
        if( nsapi_ptr->sn_xid_req.t2_alg == T2_ALG_RFC3095 )
        {
          /* Check if the PCOMP values for the algorithm are
          ** already assigned on the SAPI.
          */
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
        {
          MSG_GERAN_LOW_1("GSN ""LL %d: Inval t2 algorithm",
          ...
          */
          ...
          if( pcomp_e_ptr != NULL )
          {
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
            {
              MSG_GERAN_LOW_0("Invalid pcomp alg");
            }
            ...
            if( t2_len != (pxid_src.length + XID_TYPE_HDR_LEN) )
              MSG_GERAN_LOW_0("GSN ""XID Error encoding T2 XID params");

            /* Allocate dsm item to carry the XID block */
            pxid_req_ptr = gprs_pdu_construct( t2_xid_ex, (uint16)t2_len );
          }
          else
          {
        ...
        {
          MSG_GERAN_LOW_0("GSN ""PCI comp entity has not been created");
          pxid_req_ptr = NULL;
        }
      }
    }
      ...
      if( op_mode_match && t1_q_item )
      {
        if( t1_q_item->dcomp_e_ptr->state != UNASSIGNED )
        {
          ...
          */
          ...
          /* Allocate dsm item to carry the XID block */
          dxid_req_ptr = gprs_pdu_construct( t1_xid_ex, (uint16)t1_len );
        ...
        */
        ...
        else
        {
          ...
          */
          if( nsapi_ptr->sn_xid_req.t1_alg == T1_ALG_V42BIS )
          {
            ...
            */
            if( q_cnt(&nsapi_ptr->sapi_ptr->t1_alg_q[T1_ALG_V42BIS]) )
            {
              ...
              {
                MSG_GERAN_LOW_0("GSN ""Invalid Data comp entity in T1 queue");
          ...
          {
            MSG_GERAN_LOW_1("GSN ""LL %d: Inval T1 algorithm",
                      gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
              ...
              */
              if( !q_cnt(&nsapi_ptr->sapi_ptr->t1_alg_q[xid_t1.hdr.alg_type]) )
              {
                if( xid_t1.hdr.alg_type == T1_ALG_V42BIS )
                {
                  ...
                  MSG_GERAN_LOW_0("GSN ""XID Invalid T1 algorithm");
              }
            }
          }
          else
  ...
  */
  ...
  if( !pxid_req_ptr && !dxid_req_ptr )
  {
    ...
    MSG_GERAN_LOW_1("GSN ""LL %d: No PCOMP or DCOMP xid req",
                      gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
  } /*lint -restore*/
  else
  {
    ...
    */
    ...
    if( xid_req_ptr )
    {
      if( nsapi_ptr->sapi_ptr->xid_req_dup_ptr == NULL )
      {
        ...
        MSG_GERAN_LOW_1("GSN ""LL %d: Inval xid_req_dup_ptr",
                        gsn_extern_sapi[nsapi_ptr->sapi_ptr->sapi]);
    }
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP || FEATURE_GSM_GPRS_SNDCP_DCOMP */
  }
...
===
===========================================================================*/
void gsn_del_all_selected_comp_e(   gsn_sapi_e_t *sapi_ptr )  /* SAPI in use */
{
...
===
===========================================================================*/
void gsn_del_dcomp_e(   uint8 ent_num, gsn_sapi_e_t *sapi_ptr )
{
  t1_alg_q_t *q_item;
  t1_alg_q_t *q_next_item;
  uint8      nsapi;
  ...
  if( sapi_ptr->dcomp_e_pool[ent_num] == NULL )
  {
    MSG_GERAN_MED_1("GSN ""Invalid Data compression entity %d", ent_num);
    return;
  }
  ...
  /* Change the check to T1_ALG_MAX when more algorithms are added and T1_ALG_MAX != 0 */
  if( alg == T1_ALG_V42BIS )
  {
    if( q_cnt( &sapi_ptr->t1_alg_q[alg] ) )
    {
      ...
      while( q_item != NULL )
      {
        ...
        if( q_item->dcomp_e_ptr->ent_num == ent_num )
        {
          ...
          for( i = 0; i < (uint8)GSN_NSAPI_MAX; i++ )
          {
            ...
            /* Exit the for loop if NSAPI is invalid */
            if (!GSN_VALID_NSAPI(GSN_EXTERN_NSAPI( nsapi )))
              break;
          ...
          if( alg == T1_ALG_V42BIS )
          {
            ...
            gsn_v42_dealloc_tables
              (  &q_item->dcomp_e_ptr->comp_e, q_item->dcomp_e_ptr->alg.xid_v42bis.p0 );
          }

          MSG_GERAN_LOW_2("GSN ""XID %d: Rmv DCOMP entity %d",
            gsn_extern_sapi[sapi_ptr->sapi],q_item->dcomp_e_ptr->ent_num);
    ...
    {
      /* At least one item should be in the queue */
      MSG_GERAN_LOW_1("GSN ""XID %d: Resp inval T1 alg queue",
                  gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  else
    MSG_GERAN_LOW_1("GSN ""XID %d: Inval T1 alg",
                  gsn_extern_sapi[sapi_ptr->sapi]);
...
===
===========================================================================*/
void gsn_del_pcomp_e(   uint8 ent_num, gsn_sapi_e_t *sapi_ptr )
{
  ...
  if( alg <= T2_ALG_MAX )
  {
        ...
        if( q_item->pcomp_e_ptr->ent_num == ent_num )
        {
...
#endif /* FEATURE_GSM_SNDCP_ROHC */
            {
              MSG_GERAN_LOW_2("GSN ""XID %d: Inval Algorithm type",
          ...
          if( q_item->pcomp_e_ptr->t2_alg == T2_ALG_RFC1144 )
          {
            if( !q_cnt( &sapi_ptr->t2_alg_q[T2_ALG_RFC1144] ) )
            {
              ...
              {
                gsn_slhc_free_mem(
                  sapi_ptr->pcomp_e_pool[q_item->pcomp_e_ptr->ent_num]->comp_e);
              }
...
#endif
          {
            MSG_GERAN_LOW_0("GSN ""Inval Algorithm type");
          }
    ...
    {
      /* At least one item should be in the queue */
      MSG_GERAN_LOW_1("GSN ""XID %d: Resp inval T2 alg queue",
                  gsn_extern_sapi[sapi_ptr->sapi]);
    }
  }
  else
   MSG_GERAN_LOW_1("GSN "" Inval T2 alg",
                gsn_extern_sapi[sapi_ptr->sapi]);
}
#endif /* FEATURE_GSM_GPRS_SNDCP_PCOMP */
...
===
===========================================================================*/
void gsn_reset_comp_ent(   gsn_sapi_e_t *sapi_ptr )
{
#ifdef FEATURE_GSM_GPRS_SNDCP_PCOMP
{
  t2_alg_q_t        *q_item;
  ...
  for( alg = 0; alg <= T2_ALG_MAX; alg++ )
  {
    ...
    while( q_item != NULL )
    {
      ...
      {
        gsn_reset_pcomp_ent(  q_item->pcomp_e_ptr );
      }
#endif
  ...
  for( alg = 0; alg <= T1_ALG_MAX; alg++ )
  {
    ...
    while( q_item != NULL )
    {
      if( (q_item->dcomp_e_ptr->op_mode == ACK) &&
          (q_item->dcomp_e_ptr->t1_alg == T1_ALG_V42BIS) )
      {
        gsn_reset_dcomp_ent(  q_item->dcomp_e_ptr );
      }
      /* Get next queue item */
      q_item = q_next( &sapi_ptr->t1_alg_q[alg], &q_item->link );
    }
...
===
===========================================================================*/
dsm_item_type *gsn_encode_rmv_nsapi_xid(   const gsn_nsapi_e_t *nsapi_ptr )
{
  dsm_item_type  *xid_req_ptr     = NULL; /* xid block to be send to SGSN SNDCP */
        ...
        while( t2_q_item )
        {
          ...
          if( alg == T2_ALG_RFC1144 )
          {
                ...
                (uint16)(0x01 << GSN_EXTERN_NSAPI(nsapi_ptr->nsapi)) )
            {
              ...
              /* Create a pdu item to carry the XID block */
              pxid_req_ptr = gprs_pdu_construct( xid_ex, t2_len );
            }
          }
#endif /* FEATURE_GSM_SNDCP_SLHC */
#ifdef FEATURE_GSM_SNDCP_IPHC
        ...
        while( t1_q_item )
        {
          if( alg == T1_ALG_V42BIS )
          {
                ...
                (0x01 << GSN_EXTERN_NSAPI(nsapi_ptr->nsapi)) )
            {
              ...
              /* Create a pdu item to carry the XID block */
              dxid_req_ptr = gprs_pdu_construct( xid_ex, t1_len );
            }
          }
...
dsm_item_type *gsn_encode_def_xid ( void )
{
  ...
  /* Create a pdu item to carry the XID block */
  xid = gprs_pdu_construct( (void *)xid_all_types_def, XID_DEF_ALL_TYPE_LEN_BLK );
...
===
===========================================================================*/
void gsn_print_xid_params (   dsm_item_type  *xid_pdu, boolean ul )
{
  ...
  if( xid_pdu )
  {
    ...
    else
    {
      ...
      MSG_GERAN_MED_5("GSN ""UL %d XID len %d, %08x %08x %08x", ul, len ,a, b, c );
      ...
      if( len > 12 )
      {
        ...
        MSG_GERAN_MED_4("GSN UL %d XID cont %08x %08x %08x", ul, a, b, c );
      }
    }
  }
  else
...
void gsn_debug_print_invalid_sapi(gsn_sapi_t sapi, gsn_filenames_t file, uint32 line)
{
  gsn_sapi_e_t*  sapi_e_ptr;
  gsn_nsapi_q_t* item_ptr;

  if (GSN_VALID_SAPI(gsn_extern_sapi[sapi]) == FALSE)
  {
    MSG_GERAN_ERROR_3_G("GSN ""LL %d: Invalid SAPI, file %d, line %d", gsn_extern_sapi[sapi], file, line);
  }
  else
  {
    sapi_e_ptr = actv_sapis[sapi];

    if (sapi_e_ptr == NULL)
    {
      MSG_GERAN_ERROR_3_G("GSN ""LL %d: SAPI ptr NULL, file %d, line %d", gsn_extern_sapi[sapi], file, line);
    }
    else
    {
      if (!sapi_e_ptr->is_nsapi_ptr_q_valid)
      {  
        MSG_GERAN_ERROR_4_G("GSN ""LL %d: No mapped NSAPIs, state %d, file %d, line %d",
          gsn_extern_sapi[sapi], sapi_e_ptr->state, file, line);
      }
      else
      {
        ...
        if (item_ptr == NULL)
        {
          MSG_GERAN_ERROR_3_G("GSN ""LL %d: No mapped NSAPIs, file %d, line %d", gsn_extern_sapi[sapi], file, line);
        }
      }
    }
  }
...
void gsn_debug_print_invalid_nsapi(gsn_nsapi_t nsapi, gsn_filenames_t file, uint32 line)
{
  if (GSN_VALID_NSAPI(GSN_EXTERN_NSAPI(nsapi)) == FALSE)
  {
    MSG_GERAN_ERROR_3_G("GSN Invalid NSAPI %d:, file %d, line %d", GSN_EXTERN_NSAPI(nsapi), file, line);
  }
  else
  {
    if (actv_nsapis[nsapi] == NULL)
    {  
      MSG_GERAN_ERROR_3_G("GSN NULL NSAPI %d, file %d, line %d", GSN_EXTERN_NSAPI(nsapi), file, line);
    }
    else if (actv_nsapis[nsapi]->sapi_ptr == NULL)
    {
      MSG_GERAN_ERROR_3_G("GSN NULL SAPI ptr, file %d, line %d", GSN_EXTERN_NSAPI(nsapi), file, line);
    }
  }
}
