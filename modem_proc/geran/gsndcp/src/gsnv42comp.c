...
/*===========================================================================
  ...
  $Header: //components/rel/geran.mpss/5.2.0/gsndcp/src/gsnv42comp.c#3 $ $DateTime: 2018/10/15 04:18:03 $ $Author: pwbldsvc $
...
void  gsn_v42_compress(comp_ent_V42bis_t *comp_e)
{
   ...
   switch (comp_e->ds_v42_c_state)
   {
      ...
      */
      ...
      default:
          MSG_GERAN_LOW_1("GSN ""V42 Comp: state=%d",comp_e->ds_v42_c_state);
         break;
   }
...
/*lint -e(716) Info -- while(1) ... */
   while (1)
   {
      ...
      {
         comp_e->ds_v42_c_buffer_write_error = FALSE;
         MSG_GERAN_LOW_0("v42encode: buffer write error\n");
         return;
      }
