...
$Header: //components/rel/geran.mpss/5.2.0/gmdsp6/inc/mdsp_cmd.h#2 $
...
void mdsp_issue_qdsp6_command (GfwCmdBufType *cmd, GfwMdspCmdType  cmdId  );

uint16 mdsp_get_current_seq_num (void);
void mdsp_apps_log (void);
boolean mdsp_allow_reset(void);
void mdsp_sleep_init(void);
...
uint8 mdsp_burst_current_buffer (mdsp_burst_type burst  );
#ifdef FEATURE_GSM_MDSP_IR
void mdsp_dm_disable (void);
void mdsp_dm_pdtch_decode_done (boolean completed);
#endif
uint16 mdsp_ftsm (void);
...
boolean mdsp_validate_rx_metrics (GfwBurstMetricsStruct *metrics  );
boolean mdsp_validate_fcch_results (GfwFcchResultStruct*data  );
boolean mdsp_validate_decoded_data (GfwDecodedDataStruct *data, boolean validate_rx_qual  );

boolean mdsp_validate_power_measure (uint32 num_rssi, uint32 *rssi  );
boolean mdsp_validate_sch_results (mdsp_sch_results_struct *data   );
