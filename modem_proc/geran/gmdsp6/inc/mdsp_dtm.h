...
$Header: //components/rel/geran.mpss/5.2.0/gmdsp6/inc/mdsp_dtm.h#2 $
$DateTime: 2018/10/10 02:31:27 $ $Author: pwbldsvc $

when         who     what, where, why
----------  -------- ------------------------------------------------------
31/10/17     hd      CR2085612 FR 42579: Enable Single Burst Decode with PEO
...
typedef struct
{
   ...
   /* Variables used to indicate PEO is enabled to GFW */
   boolean                   peo_enabled;
#endif
...
void mdsp_issue_async_msg(GfwAsyncCmd cmdType);
void mdsp_wakeup_issue_async_msg(GfwAsyncCmd cmdType,   uint16 fn);
...
SIDE EFFECTS
  None
===========================================================================*/
uint16 mdsp_random_timer_cmd(void);
...
void mdsp_dtm_abort_rx (uint16 data_tag);
void mdsp_dtm_reset_rx (void);

#ifdef  FEATURE_GSM_DTM
#ifdef  FEATURE_GSM_EDTM

void  mdsp_dtm_reset_rx_cs_only(void);
...
void mdsp_dtm_tx (mdsp_dtm_tx_params_type *params, ARFCN_T arfcn, uint16 pa_index[] );
boolean mdsp_dtm_get_tx_schedule (uint16 tag, boolean discard_untxed  );
boolean mdsp_dtm_abort_tx (uint16 tag  );
...
void mdsp_dtm_init (void);
void mdsp_dtm_process_frame_tick (void);
