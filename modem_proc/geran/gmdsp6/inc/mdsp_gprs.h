...
$Header: //components/rel/geran.mpss/5.2.0/gmdsp6/inc/mdsp_gprs.h#3 $
 ...
 ****************************************************************************
 ***************************************************************************/
boolean mdsp_gprs_get_rx_burst_schedule (GfwSchedulerResultStruct *data  );
boolean mdsp_gprs_get_rx_usfs (GfwUsfBufStruct *data  );
uint16  mdsp_gprs_get_tx_burst_schedule (void);

void mdsp_gprs_set_usf_list(uint16  list[8]  );
boolean mdsp_gprs_get_tx_schedule (uint16 tag, boolean discard_untxed );
void mdsp_gprs_convert_usf_to_pdch(uint16 tag, mdsp_timeslot_type pdch  );
...
boolean mdsp_gprs_get_power_measure_results
 (
   ...
   uint16  *decstat,
   uint16  *decstat2
#endif /*GL1_GMSK_MOD_DET*/
 );
void mdsp_gprs_abort_power_measure (void);


void mdsp_gprs_set_medium_access_mode(mdsp_access_mode_type mode  );

boolean mdsp_gprs_is_power_measure_results_available (void);

#ifdef FEATURE_GSM_MDSP_EGPRS
/* Enable EGPRS mode */
void mdsp_gprs_set_egprs_mode(boolean egprs  );

#ifdef FEATURE_GSM_EGPRS_IR_WS_REDUCTION
uint16 mdsp_gprs_get_oper_win_size(void);
#endif /* FEATURE_GSM_EGPRS_IR_WS_REDUCTION */

#ifdef FEATURE_GSM_MDSP_IR
/* Incremental Redundancy */
void mdsp_gprs_IR_enable(boolean enable  );
void mdsp_gprs_IR_clear_memory (uint16 starting_bsn, uint16 ending_bsn  );
void mdsp_gprs_IR_set_active_window(uint16 starting_bsn, uint16 ending_bsn  );
void mdsp_gprs_IR_set_tfi (uint16 tfi  );

#endif
#endif

/* enable ACI CCI detection in mdsp */
void mdsp_gprs_set_aci_detection(boolean enable  );

void mdsp_gprs_reset_rx_seq_nums(void);
void mdsp_gprs_init(void);
void mdsp_gprs_inact(void);
void mdsp_gprs_process_frame_tick(void);
...
mdsp_gprs_public_data_t* get_mdsp_gprs_public_dataspace_ptr( );
