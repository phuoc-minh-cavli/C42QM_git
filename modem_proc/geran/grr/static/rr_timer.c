...
$Header: //components/rel/geran.mpss/5.2.0/grr/static/rr_timer.c#7 $ $DateTime: 2021/08/21 02:53:36 $ $Author: pwbldsvc $
...
#define RR_TIMER_INVALID_HANDLE   0xFFUL
 ...
 * - unique ID (uid);
 */
typedef uint8 rr_timer_handle_t;
 ...
 *
 ****************************************************************************/
static rr_timer_ptr_t rr_timer_find_entry(rr_timer_id_T timer_id, rr_timer_handle_t *handle_ptr)
{
  ...
  for (i = 0; i < RR_TIMER_POOL_SIZE; i++, curr_ptr++)
  {
    ...
    tid = (rr_timer_id_T)(timer_handle);
...
void rr_timer_expiry_callback(unsigned long cb_param)
{
  ...
  if (timer_handle != RR_TIMER_INVALID_HANDLE)
  {
    ...
    timer_cmd.timer_id        = (timer_id_T)(timer_handle);
...
)
{
  ...
  for (i = 0; i < RR_TIMER_POOL_SIZE; i++, rr_timer_ptr++)
  {
    ...
    {
      if (list_ptr[j] == (rr_timer_id_T)(rr_timer_ptr->rr_timer_hndl))
      {
 ...
 *
 ****************************************************************************/
static boolean rr_timer_run(rr_timer_id_T timer_id, rex_timer_cnt_type millisecs)
{
  rr_timer_handle_t timer_handle;
  rr_timer_ptr_t    rr_timer_ptr;
  boolean           bStarted;
  ...
  rr_timer_ptr = rr_timer_find_entry(timer_id, &timer_handle);
  ...
  else
  {
    ...
    timer_handle = (rr_timer_handle_t)timer_id;
  }
 ...
 *
 ****************************************************************************/
boolean rr_timer_start(rr_timer_id_T timer_id, rex_timer_cnt_type millisecs )
{
  return(rr_timer_run(timer_id, millisecs));
}
 ...
 *
 ****************************************************************************/
static boolean rr_timer_clear(rr_timer_id_T timer_id)
{
  rr_timer_ptr_t rr_timer_ptr;
  ...
  do
  {
    ...
    rr_timer_ptr = rr_timer_find_entry(timer_id, NULL);
    if (rr_timer_ptr != NULL)
    {
...
{
  return(rr_timer_clear(timer_id));
}
...
)
{
  ...
  return(rr_timer_start(timer_id, millisecs));
}
...
)
{
  ...
  if (bStartNew == TRUE)
  {
    ...
    bExtended = rr_timer_run(timer_id, millisecs);
  }
 ...
 *
 *   Parameters:     timer ID, maximum duration
 *   -----------
 *
...
/****************************************************************************
 *   Function name:  rr_timer_create_expired_entry
 *   --------------
 *
 *   Description:    Creates a timer pool entry that looks like it has expired
 *
 *   Parameters:     timer ID, context ID, and optional output pointer
 *   -----------
 *
 *   Returns:        Pointer to expired entry if created successfully, or NULL
 *   --------
 *
 ****************************************************************************/
static rr_timer_ptr_t rr_timer_create_expired_entry(rr_timer_id_T timer_id, rr_timer_handle_t *handle_ptr)
{
  rr_timer_handle_t expired_handle;
  rr_timer_ptr_t    expired_ptr;

  /* check whether the pool has any handle for this timer already */
  expired_ptr = rr_timer_find_entry(timer_id, &expired_handle);
  ...
  if (expired_ptr != NULL)
  {
    ...
    /* to avoid collisions between timer service callbacks and this function */
    expired_handle = expired_handle ^ RR_TIMER_HANDLE_UID_BIT;
  }
  else
  {
    /* new instance, generate a new handle and find a free entry for it */
    expired_handle = (rr_timer_handle_t)timer_id;
  ...
  if (expired_ptr != NULL)
  {
    ...
    expired_ptr->rr_timer_hndl = expired_handle;
    expired_ptr->expiry_posted = RR_TIMER_EXPIRY_POSTED_AND_VALIDATED;

    /* caller can optionally pass an output pointer to retrieve the handle */
    if (handle_ptr != NULL)
    {
      *handle_ptr = expired_handle;
    }
  }
  else
  {
    MSG_GERAN_ERROR_0("Unable to create expired timer, pool full");
  }
...
/****************************************************************************
 *   Function name:  rr_timer_force_timer_expiry
 *   --------------
 *
 *   Description:    Generates a timer expiry
 *
 *   Parameters:     timer ID
 *   -----------
 *
 *   Returns:        TRUE if the expiry was generated successfully
 *   --------
 *
 ****************************************************************************/
boolean rr_timer_force_timer_expiry(rr_timer_id_T timer_id  )
{
  rr_timer_handle_t expired_handle;
  rr_timer_ptr_t    expired_ptr;
  ...
  /* use internal utility function to create an expired timer entry */
  expired_ptr = rr_timer_create_expired_entry(timer_id,  &expired_handle);
  if (NULL != expired_ptr)
  {
    /* even expiry posted, need to do below in order to post the timer expiry message in RR queue*/
    expired_ptr->expiry_posted = RR_TIMER_EXPIRY_NOT_POSTED;
  }
  else
  {
    expired_handle = RR_TIMER_INVALID_HANDLE;
  }

  RR_TIMER_UNLOCK();

  if (expired_handle != RR_TIMER_INVALID_HANDLE)
  {
    /* and then trigger the callback (which uses a context lock of its own) */
    rr_timer_expiry_callback((unsigned long)expired_handle);
    return(TRUE);
  }
...
)
{
  ...
  RR_TIMER_LOCK();
  expired_ptr = rr_timer_create_expired_entry(timer_id, NULL);

  RR_TIMER_UNLOCK();

  return((boolean)(expired_ptr != NULL));
}
...
{
  if(!rr_timer_is_running(RR_CELL_BARRED_TIMER))
  {
    MSG_GERAN_HIGH_0_G("Barred cell timer started");
    (void)rr_timer_start(RR_CELL_BARRED_TIMER,RR_CELL_BARRED_TIMEOUT);
  }
  else
  {
 ...
 * authentication and ciphering failure.
 */
void rr_stop_barred_cell_timer(void)
{
  if (rr_timer_cancel(RR_CELL_BARRED_TIMER))
  {
...
void rr_start_dmm_resumption_timer(rex_timer_cnt_type millisecs)
{
  if (!rr_timer_is_running(RR_DMM_RESUMPTION_TIMER))
  {
    MSG_GERAN_HIGH_1_G("DMM resume timer : RR_DMM_RESUMPTION_TIMER started for %d ms", millisecs);
    (void)rr_timer_start(RR_DMM_RESUMPTION_TIMER, millisecs);
  }
  else
  {
    MSG_GERAN_HIGH_0_G("DMM Resume timer is already running");
  }
} /* rr_start_dmm_resumption_timer */

void rr_stop_dmm_resumption_timer()
{
  if (rr_timer_cancel(RR_DMM_RESUMPTION_TIMER))
  {
    MSG_GERAN_HIGH_0_G("DMM resume timer stopped");
  }
  else
  {
    MSG_GERAN_HIGH_0_G("DMM Resume timer is not running");
  }
} /* rr_start_dmm_resumption_timer */

void rr_start_unloaded_nas_msg_guard_timer(rex_timer_cnt_type millisecs)
{
  if (!rr_timer_is_running(RR_UNLOADED_NAS_MSG_GUARD_TIMER))
  {
    MSG_GERAN_HIGH_1_G("rr_unloaded nas msg guard timer : RR_UNLOADED_NAS_MSG_GUARD_TIMER started for %d ms", millisecs);
    (void)rr_timer_start(RR_UNLOADED_NAS_MSG_GUARD_TIMER, millisecs);
  }
  else
  {
    MSG_GERAN_HIGH_0_G("rr_unloaded nas msg guard timer is already running");
  }
} /* rr_start_unloaded_nas_msg_guard_timer */

void rr_stop_unloaded_nas_msg_guard_timer()
{
  if (rr_timer_cancel(RR_UNLOADED_NAS_MSG_GUARD_TIMER))
  {
    MSG_GERAN_HIGH_0_G("rr_unloaded nas msg guard timer stopped");
  }
  else
  {
    MSG_GERAN_HIGH_0_G("rr_unloaded nas msg guard timer is not running");
  }
...
)
{
  switch (timer_id)
  {
   ...
   case RR_MM_ACTIVATION_RSP_TIMER:
    ...
    case T3236:
      return "T3236 expired";
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/


#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
    case RR_RMC_CCCH_READ_TIMER:
      return "RR_RMC_CCCH_READ_TIMER ";
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/

    case RR_DMM_RESUMPTION_TIMER:
      return "RR_DMM_RESUMPTION_TIMER ";

	case RR_UNLOADED_NAS_MSG_GUARD_TIMER:
	  return "RR_UNLOADED_NAS_MSG_GUARD_TIMER";
