...
$Header: //components/rel/geran.mpss/5.2.0/grr/inc/rr_l1_g.h#7 $    $DateTime: 2020/06/17 10:30:20 $ $Author: pwbldsvc $
...
typedef enum {
   ...
   RR_L1_SI_TYPE_21_EXT        = 0x01000000,
...
/*************************************************************
 * The Layer_1_edrx_peo_params structure  passed to layer 1
...
typedef struct
{
   ...
   boolean                 reduced_ncell_monitor;
   word                    BS_ePA_MFRMS;
   word                    BS_ePA_MFRMS_DSC;
   boolean                 imsi_valid;
   imsi_mod_10000000_T     imsi_mod_10000000;
   uint8                   rcc;   
} layer1_edrx_peo_params_T;
#endif /* FEATURE_GERAN_EXTENDED_DRX || FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
...
/******************************************************************************
 * MPH_SERVING_MEAS_REQ primitive
 * FROM : RR
 * TO   : LAYER 1
 * DESCRIPTION :
 * RR requests L1 to start sending serv meas_ind
 ******************************************************************************/
 
typedef struct
{
  IMH_T    message_header;
} mph_serving_meas_req_T;
...
/******************************************************************************
 * MPH_PSM_ENTRY_IND primitive
 * FROM : RR
 * TO   : LAYER 1
 * DESCRIPTION :
 * Indicates L1 of PSM_ENTRY received from NAS in RR_DEACT_REQ
 ******************************************************************************/
typedef struct
{
  IMH_T                 message_header;
...
#define MAX_SI_SCHED_POSITIONS 2
#define MAX_SI_SCHED_INFO      12
...
typedef struct
{
  normal_extended_t bcch_type;
  uint8             modulus;
  uint16            relative_pos;  /* This is valid only when modulus is not zero */
} si_position_T;
...
typedef struct
{
  si_type_T     si;
  uint8         pos_count;
  si_position_T positions[MAX_SI_SCHED_POSITIONS];
} si_sched_info_T;

typedef struct
{
  uint8           si9_sched_where;  /* This is valid only if si_mask contains SI9 */
  uint8           si_sched_info_count;
  si_sched_info_T si_sched_info[MAX_SI_SCHED_INFO];
} rr_l1_si_sched_info_T;
...
#if defined (FEATURE_GERAN_EXTENDED_DRX) || defined(FEATURE_GERAN_POWER_EFFICIENT_OPERATION)   
  layer1_edrx_peo_params_T layer1_edrx_peo_params;
#endif /* FEATURE_GERAN_EXTENDED_DRX || FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
...
typedef enum
{
  ...
  /* add any new priority levels after this line */
...
/******************************************************************************
 * MPH_PROC_REQ primitive
 * FROM : RR
 * TO   : LAYER 1
 * DESCRIPTION :
 * GRR instruct GL1 about the start of a procedure
 ******************************************************************************/
typedef struct
{
   IMH_T       message_header;
...
/******************************************************************************
 * MPH_PROC_CNF primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * Response to MPH_PROC_REQ
 ******************************************************************************/
typedef struct
{
   IMH_T         message_header;
...
/******************************************************************************
 * MPH_PROC_IND primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * GL1 instruct GRR about the start of a procedure
 ******************************************************************************/
typedef struct
{
   IMH_T       message_header;
   proc_type_e proc;
   uint32      rem_drx_time_ms;
} mph_proc_ind_t;
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION   
  mph_serving_meas_req_T                     mph_servig_meas_req;
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
...
{
   IMH_T       message_header;
} mph_bsic_change_ind_t; 
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/

#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
/******************************************************************************
 * MPH_SIMULATE_R13_PARAM_FROM_DIAG_IND primitive
 * FROM : LAYER 1
 * TO   : RR
 * DESCRIPTION :
 * L1 instructs RR about the R13  value to be used given from  QXDM
 *******************************************************************************/
typedef struct
{
  IMH_T    message_header;
  uint8    cmd_type;
  uint8    cmd;
} mph_simulate_r13_param_from_diag_ind_T;
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION   
  mph_bsic_change_ind_t                   bsic_change_ind;  
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION */
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK  
  mph_simulate_r13_param_from_diag_ind_T  simulate_r13_param_from_diag;
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/
} l1_rr_messages_u;
