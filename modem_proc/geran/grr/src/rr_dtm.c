...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_dtm.c#5 $ */
...
static rr_dtm_static_data_t  rr_dtm_data;
 ...
 */
static rr_dtm_static_data_t *rr_dtm_get_data_ptr( void )
{
  return &rr_dtm_data;
}
 ...
 * \param 
 */
static void log_dtm_event(rr_dtm_debug_event_e_t event )
{
  rr_dtm_debug_buf_entry_t *entry_ptr;

  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
  ...
  entry_ptr->timestamp_fn = GSTMR_GET_FN_GERAN();
...
=============================================================================*/
static void start_t3148( )
{
  MSG_GERAN_MED_0_G("Start T3148 (4 sec)");
  (void)rr_timer_start(T3148, GRR_TIMEOUT_1_SECOND * 4);
...
=============================================================================*/
static void stop_t3148( )
{
  if (rr_timer_cancel(T3148))
  {
...
=============================================================================*/
static boolean is_t3148_running( )
{
  return rr_timer_is_running(T3148);
...
=============================================================================*/
static void save_mac_grr_ul_tbf_req(mac_grr_ul_tbf_req_t *mac_grr_ul_tbf_req )
{
...
=============================================================================*/
static mac_grr_ul_tbf_req_t *get_mac_grr_ul_tbf_req( )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
...
=============================================================================*/
static void invalidate_dtm_sys_info( )
{
...
=============================================================================*/
static void set_default_dtm_sys_info_if_invalid( )
{
...
=============================================================================*/
static uint8 *get_layer3_message_buffer(uint8 msg_type, uint8 *msg_len )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
...
=============================================================================*/
static void clear_previous_assignment_details( )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
...
=============================================================================*/
static void send_dtm_request_ota(rr_channel_req_info_t *channel_req_info )
{
  uint32 tlli32;
  ...
  log_dtm_event(RR_DTM_DBG_DTM_REQUEST_SENT_OTA);
...
)
{
  ...
  log_dtm_event(RR_DTM_DBG_DTM_ASSIGN_FAIL_SENT_OTA);
...
=============================================================================*/
static void send_assignment_complete_ota( )
{
  byte msg_len;
  uint8 * layer3_message;
  ...
  log_dtm_event(RR_DTM_DBG_ASSIGN_COMPLETE_SENT_OTA);
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_dtm_data_ptr = rr_dtm_get_data_ptr();
  ...
  if (llc_pdu_container->llc_pdu_len <= MAX_GTTP_LLC_PDU_SIZE)
  {
    ...
    log_dtm_event(RR_DTM_DBG_GTTP_DATA_SENT_OTA);
  }
  else
  {
...
=============================================================================*/
static void send_dtm_channel_assign_req( )
{
  ...
  gprs_cell_options = rr_si_get_gprs_cell_options();
  //RR_NULL_CHECK_FATAL(gprs_cell_options);

  power_control_params = rr_si_get_power_control_params();
  ...
  (void)rr_timer_start(RR_GRR_CHN_ASSIGN_REQ_GUARD_TIMER, GRR_TIMEOUT_5_SECONDS);
  ...
  log_dtm_event(RR_DTM_DBG_DTM_CHAN_ASSIGN_REQ_SENT);
...
=============================================================================*/
rr_event_T rr_dtm_send_edtm_assignment_to_gmac( )
{
  ...
  if (rr_dtm_data_ptr->dtm_assignment_details.cs_assign_present == TRUE)
  {
    ...
    rr_gprs_edtm_store_channel_needed(cs_assign_ptr->channel_description.channel_type);
 ...
 * \param  
 */
static void send_grr_mac_ps_access_req( void )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
...
{
  rr_dtm_send_omsg_cs_release_cnf(ce_rel_mode);

  log_dtm_event(RR_DTM_DBG_CS_REL_CNF_SENT);
}
...
=============================================================================*/
static rr_event_T process_packet_assignment( )
{
  rr_event_T  return_event;
  ...
  if ( decoding_ok )
  {
    ...
    log_dtm_event(RR_DTM_DBG_PKT_ASSIGN_RCVD);
  ...
  else
  {
    ...
    log_dtm_event(RR_DTM_DBG_PKT_ASSIGN_RCVD_ERR);
...
=============================================================================*/
static rr_event_T process_dtm_reject( )
{
  ie_descriptor_T * current_IE;
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(DTM_REJECT);
  ...
  if ( decoding_ok )
  {
    ...
    {
      grr_start_t3142(t3122_t3142_timeout_value);

      return_event = EV_DTM_ACCESS_REJECT_T3142_RUNNING;
    }

    log_dtm_event(RR_DTM_DBG_DTM_REJECT_RCVD);
  }
  else
  {
    log_dtm_event(RR_DTM_DBG_DTM_REJECT_RCVD_ERR);
  }
...
=============================================================================*/
static boolean process_dtm_information( )
{
  ie_descriptor_T * current_IE;
  boolean decoding_ok;
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(DTM_INFORMATION);
  decoding_ok = FALSE;
  ...
  if (decoding_ok)
  {
    ...
    log_dtm_event(RR_DTM_DBG_DTM_INFO_RCVD);

    return TRUE;  // The message was decoded successfully
  }

  log_dtm_event(RR_DTM_DBG_DTM_INFO_RCVD_ERR);
...
=============================================================================*/
static void process_packet_notification( )
{
  ie_descriptor_T * current_IE;
  boolean decoding_ok;
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(PACKET_NOTIFICATION);
  decoding_ok = FALSE;
  ...
  if ( current_IE )
  {
    ...
    if ( current_IE->start_IE )
    {
      ...
      if ( current_IE->IEI == 0x10 )
      {
        ...
        if (rr_ce_compare_ota_ptmsi(current_IE->start_IE))
        {
...
=============================================================================*/
static uint8 get_max_LAPDm_frames( )
{
  gprs_scell_info_t *cell_info = rr_gprs_get_scell_info();
  uint8              max_LAPDm;
...
=============================================================================*/
static boolean gttp_criteria_met(uint16 num_of_octets )
{
  ...
  */
  ...
  max_LAPDm_frames = get_max_LAPDm_frames();
...
=============================================================================*/
static rr_event_T dtm_packet_assign_control(rr_event_T event_in, void *message_ptr )
{
  byte message_id;
  byte message_set;
      ...
      case DTM_PKT_ASSIGN_AWAITING_ASSIGN_CNF:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_CS_RELEASE_REQUIRED:
          {
            ...
            log_dtm_event(RR_DTM_DBG_AWAITING_L1_CNF);
            break;
        ...
        switch ( event_in )
        {
          ...
          case EV_INPUT_MESSAGE:
          {
                 ...
                 (message_id == (byte) MPH_DTM_CHANNEL_ASSIGN_CNF) )
            {
              ...
              rr_send_grr_mac_dtm_channel_release_ind(NORMAL_REL);
...
=============================================================================*/
static rr_event_T respond_to_dtm_assignment_command( )
{
  ...
  {
    rr_set_current_dedicated_band((rr_internal_band_T)new_band);
 ...
 * @return rr_event_T
 */
static rr_event_T dtm_assign_command_control(rr_event_T event_in, void *message_ptr )
{
  byte               message_id;
  byte               message_set;
  rr_event_T         event_out = EV_NO_EVENT;
      ...
      case DTM_ASSIGN_CMD_INACTIVE:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_ASSIGNMENT_READY_IN_DTM:
          {
            ...
            rr_dtm_data_ptr->data_ptr->old_band = rr_get_current_dedicated_band();
            ...
            rr_send_dl_suspend_req(RR_L1_GSM_GSM);
        ...
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
            ...
            if ( (message_set == MS_RR_L1) &&
                 (message_id == (byte) MPH_DTM_CHANNEL_ASSIGN_CNF) )
            {
              if (rr_timer_cancel(RR_GRR_CHN_ASSIGN_REQ_GUARD_TIMER))
              {
              ...
              rr_preserve_current_dedicated_config();

              /* Update RR's channel information with the new chanel assignment data */
              /* which is now using */
              rr_dtm_start_using_new_channel_spec();

              /* Resume L2 link and send "ASSIGNMENT COMPLETE" OTA to the network */
              send_assignment_complete_ota();

              /* Update the L2 link status tracking */
              rr_set_link_status(SAPI0_LINK_IN_PROGRESS);

              if (!rr_bands_equal(rr_dtm_data_ptr->data_ptr->old_band, rr_get_current_dedicated_band()))
              {
                MSG_GERAN_MED_0_G("Frequency band changed after DTM_ASSIGNMENT_COMMAND, clearing SACCH system information");
                rr_clear_si_sacch();
                rr_ded_set_measurement_reports_invalid(TRUE);
              }
              else
              {
        ...
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
                 ...
                 (message_id == (byte) DL_RESUME_CNF) )
            {
              ...
              {
                rr_band_avoid_ded_freq_upd_send_imsg(&rr_dtm_data_ptr->dtm_assignment_details.cs_assign.channel_assignment.channel_description.frequency_list);
              }
            ...
            else if (dtm_msg_indicates_l2_establishment_failure(message_set, message_id, message_ptr))
            {
              ...
              rr_send_grr_mac_dtm_channel_release_ind(NORMAL_REL);
              rr_send_grr_mac_no_ps_access_ind(NO_DEL);

              /* Reconnect L1 to the previous channel */
              rr_send_mph_reconnect_channel_req();
        ...
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
                 ...
                 (message_id == (byte) MPH_RECONNECT_CHANNEL_CNF) )
            {
              ...
              rr_restore_previous_dedicated_config();
    ...
    switch ( rr_dtm_data_ptr->data_ptr->substate.cell_capability_monitor )
    {
      ...
      case DTM_CELL_DTM_NOT_SUPPORTED:
      {
        switch ( event_in )
        {
          ...
          {
            boolean dtm_support = rr_cell_supports_dtm();
            ...
            if (dtm_support)
            {
              ...
              grr_gprs_cell_update_required_when_gmm_resumed();
          ...
          {
            set_default_dtm_sys_info_if_invalid();
            rr_dtm_data_ptr->data_ptr->substate.cell_capability_monitor = DTM_CELL_DTM_SUPPORTED;
            break;
          }
#endif /* FEATURE_GSM_EDTM */
 ...
 */
static boolean is_ps_transfer_in_progress( )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
 ...
 */
static boolean is_ps_ul_assign_allowed( )
{
  if (is_ps_transfer_in_progress())
  {
    return TRUE;
  }
  else
 ...
 * \param 
 */
static void check_if_mm_gprs_sys_info_changed( void )
{
  boolean mm_gprs_sys_info_changed;
  mm_gprs_sys_info_T mm_gprs_sys_info;

  (void)rr_gprs_get_sys_info_for_mm(&mm_gprs_sys_info);

  mm_gprs_sys_info_changed = rr_mm_diff_gprs_sys_info(&mm_gprs_sys_info);
 ...
 * 
 */
static void rr_dtm_transition_to_null( void )
{
  rr_dtm_discard_data();
}
...
/**
  ...
  sets initial values
 */
void rr_dtm_activate_init( void )
{
  // Obtain a pointer to the module data
  rr_dtm_static_data_t *rr_dtm_data_ptr = rr_dtm_get_data_ptr();
  ...
  rr_dtm_data_ptr->data_ptr->old_dtm_support = rr_cell_supports_dtm();

  log_dtm_event(RR_DTM_DBG_ACTIVATE);
...
void rr_dtm_discard_data( )
{
  ...
  if (rr_dtm_data_ptr->data_ptr != NULL)
  {
    ...
    log_dtm_event(RR_DTM_DBG_DEACTIVATE);
  }
...
=============================================================================*/
void rr_dtm_gttp_message_received(dl_data_ind_T *dl_data_ind )
{
  /*
  ...
  */
  ...
  if ( dl_data_ind->layer3_message[1] == GPRS_INFORMATION )
  {
    ...
    */
    ...
    log_dtm_event(RR_DTM_DBG_GTTP_DATA_RCVD_OTA);
  }
  else
  {
...
=============================================================================*/
void rr_dtm_start_using_new_channel_spec( void )
{
  ...
  dedic_ptr              = rr_get_dedicated_data_ptr();
  rr_channel_spec_ptr    = rr_get_channel_spec(dedic_ptr);
  ...
  rr_ciphering_has_started(rr_channel_spec_ptr->cipher_flag);
...
)
{
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(DTM_ASSIGNMENT_COMMAND);
  ...
  if ( decoding_ok )
  {
    ...
    if (rr_channel_type_supported(channel_description.channel_type) == FALSE)
    {
      error_cause = RR_CHANNEL_MODE_UNACCEPTABLE;
    ...
    if ( error_cause == RR_NORMAL_EVENT )
    {
      ...
      if ( channel_description.hopping_flag )
      {
        ...
        if ( frequency_list_present )
        {
          ...
          if (rr_check_frequency_list(&channel_description.frequency_list))
          {
    ...
    if ( error_cause == RR_NORMAL_EVENT )
    {
      ...
      {
        channel_spec_T *channel_spec_ptr = rr_get_channel_spec(dedic_ptr);
    ...
    if ( error_cause == RR_NORMAL_EVENT )
    {
      if (cipher_mode_setting_present)
      {
        if (start_ciphering)
        {
          if (rr_get_ciphering_mode_command_received() == FALSE)
          {
            MSG_GERAN_HIGH_0_G("DTM ASSIGNMENT COMMAND cipher mode start with no previous cipher mode set");
            error_cause = RR_PROTOCOL_ERROR_UNSPECIFIED;
          }

          if (rr_cipher_mode_supported(cipher_algorithm) == FALSE)
          {
    ...
    if ( error_cause == RR_NORMAL_EVENT )
    {
      ...
      else
      {
        ...
        if (rr_si_diff_gprs_cell_options(&gprs_broadcast_information.gprs_cell_options))
        {
          log_gprs_cell_options = TRUE;
        }
      }
      ...
      {
        (void)rr_log_gprs_cell_options(&gprs_broadcast_information.gprs_cell_options);
      }
#endif
      /* Update the system information database */
      ...
      log_dtm_event(RR_DTM_DBG_DTM_ASSIGN_CMD_RCVD);
    }
    else
    {
  ...
  else
  {
    ...
    log_dtm_event(RR_DTM_DBG_DTM_ASSIGN_CMD_RCVD_ERR);
  }
...
=============================================================================*/
rr_event_T rr_dtm_control(rr_event_T event_in, void *message_ptr )
{
  byte message_id;
  byte message_set;
  ...
  if ((event_in == EV_INPUT_MESSAGE) && (message_ptr != NULL))
  {
        ...
        switch ( message_type )
        {
          ...
          case PACKET_ASSIGNMENT:
          {
            ...
            if (rr_get_nv_dtm_enabled())
            {
              ...
              if (rr_thermal_ps_allowed() == FALSE)
              {
    ...
    if (message_set == MS_RR_RR)
    {
      ...
      {
        log_dtm_event(RR_DTM_DBG_CS_REL_REQ_RCVD);
      }
    }
    else
    if ((message_set == MS_MAC_RR) &&
    ...
    {
      save_mac_grr_ul_tbf_req((mac_grr_ul_tbf_req_t *)message_ptr);
      event_in = EV_DTM_UL_TBF_REQ;

      log_dtm_event(RR_DTM_DBG_UL_TBF_REQ);
    }
    else
    if (message_set == MS_RR_L1)
    {
      ...
      {
        log_dtm_event(RR_DTM_DBG_DTM_CHAN_ASSIGN_CNF_RCVD);
      }
      else
      if (message_id == (byte) MPH_DTM_CHANNEL_RELEASE_IND)
      {
        log_dtm_event(RR_DTM_DBG_DTM_CHAN_REL_IND_RCVD);
      }
    }
  }
  else
  ...
  {
    log_dtm_event(RR_DTM_DBG_NO_PS_ACCESS_REQ_RCVD);
  }
  else
  if ((EV_DTM_PS_ACCESS_REQ == event_in) ||
      (EV_DTM_PS_ACCESS_REQ_PURGE == event_in))
  {
    log_dtm_event(RR_DTM_DBG_PS_ACCESS_REQ_RCVD);
  }
  else
  if (EV_INTRA_CELL_HANDOVER == event_in)
  {
    log_dtm_event(RR_DTM_DBG_HANDOVER);
  }
    ...
    switch ( rr_dtm_data_ptr->state )
    {
      case DTM_NULL:
      {
        switch ( event_in )
        {
          ...
          {
#ifdef FEATURE_GSM_EDTM
            if (rr_gprs_edtm_control(event_in, NULL) == EV_EDTM_ESTABLISHMENT_STARTED)
            {
          ...
          case EV_DTM_W2G_HO_PS_ATTACHED:
          {
            ...
            */
            (void) dtm_cell_capability_monitor(event_in);
          ...
          case EV_EDTM_ESTABLISHMENT_COMPLETED:
          {
            ...
            (void) dtm_cell_capability_monitor(EV_EDTM_ESTABLISHMENT_COMPLETED);
          ...
          case EV_DTM_NO_PS_ACCESS_REQ_DEL_ALL:
          {
            ...
            CALL_RR_CB_FN((rr_cb_fn_t)message_ptr);
            break;
          }
      ...
      case DTM_RR_CS_EST:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_RR_EST_CNF:
          {
            ...
            if (dtm_cell_capability_monitor(EV_DTM_RR_EST_CNF) == EV_DTM_CELL_SUPPORTS_DTM)
            {
      ...
      case DTM_DTM_POSSIBLE:
      {
        switch ( event_in )
        {
                ...
                switch (rr_rr_msg_ptr->header.rr_message_set)
                {
                  case RR_DTM_IMSG:
                  {
                    switch (rr_rr_msg_ptr->header.imh.message_id)
                    {
                      case RR_DTM_IMSG_CS_RELEASE_REQ:
                      {
                        (void) dtm_cell_capability_monitor(EV_RESET_SOFTWARE);
          ...
          case EV_DTM_UL_TBF_REQ:
          {
            mac_grr_ul_tbf_req_t *ul_tbf_req = get_mac_grr_ul_tbf_req();
            ...
            if (ul_tbf_req != NULL)
            {
              ...
              */
              ...
              if (gttp_criteria_met(ul_tbf_req->gttp_gmm_sig_octets))
              {
                rr_send_grr_mac_dtm_ul_dcch_ind();
              ...
              else
              {
                ...
                {
                  mac_grr_ul_tbf_req_t *mac_grr_ul_tbf_req = get_mac_grr_ul_tbf_req();
          ...
          {
            boolean ul_requested = is_t3148_running();
            ...
            if (EV_DTM_ASSIGNMENT_COMMAND_CHECKED == respond_to_dtm_assignment_command())
            {
          ...
          case EV_DTM_PACKET_ASSIGNMENT_RECEIVED:
          {
            ...
            //  - EV_DTM_ASSIGNMENT_ERROR if there are errors
            if (EV_DTM_PACKET_ASSIGNMENT_CHECKED == process_packet_assignment())
            {
          ...
          case EV_DTM_ASSIGNMENT_COMMAND_CHECKED:
          {
            ...
            check_if_mm_gprs_sys_info_changed();

            // Start the procedure to configure MAC/L1 with the new DTM Assignment
            (void) dtm_assign_command_control(EV_DTM_ASSIGNMENT_READY, NULL);
          ...
          case EV_DTM_PACKET_ASSIGNMENT_CHECKED:
          {
            ...
            // Start the procedure to configure MAC/L1 with the new DTM Assignment
            (void) dtm_packet_assign_control(EV_DTM_ASSIGNMENT_READY, NULL);
          ...
          case EV_DTM_REJECT_RECEIVED:
          {
            if (is_t3148_running())
            {
               ...
               *  blocked
               */
              rr_event_T  process_result = process_dtm_reject();
          ...
          {
            log_dtm_event(RR_DTM_DBG_T3148_EXPIRY);
          ...
          case EV_DTM_SI6_RECEIVED:
          {
            ...
            if (dtm_cell_capability_monitor(event_in) ==
                EV_DTM_CELL_DOES_NOT_SUPPORT_DTM)
            {
              // Stop T3148 if running
              stop_t3148();
      ...
      case DTM_PROCESS_PACKET_ASSIGNMENT:
      {
        switch ( event_in )
        {
            ...
            switch (message_set)
            {
                ...
                switch (rr_rr_msg_ptr->header.rr_message_set)
                {
                  case RR_DTM_IMSG:
                  {
                    switch (rr_rr_msg_ptr->header.imh.message_id)
                    {
                      case RR_DTM_IMSG_CS_RELEASE_REQ:
                      {
                        new_event = dtm_packet_assign_control(EV_DTM_CS_RELEASE_REQUIRED, NULL);
                        break;
                      }
              ...
              case MS_MAC_RR:
              {
                switch (message_id)
                {
                  ...
                  {
                    new_event = dtm_packet_assign_control(EV_INPUT_MESSAGE, message_ptr);
                    break;
                  }
          ...
          case EV_DTM_UL_TBF_REQ:
          {
            ...
            (void) dtm_packet_assign_control(EV_DTM_UL_TBF_REQ, NULL);
            break;
          }
          ...
          case EV_DTM_PERFORM_CSPS_RELEASE:
          {
            ...
            (void) dtm_packet_assign_control(EV_RESET_SOFTWARE, NULL);
            (void) dtm_cell_capability_monitor(EV_RESET_SOFTWARE);
      ...
      case DTM_PROCESS_ASSIGNMENT_COMMAND:
      {
        switch ( event_in )
        {
          case EV_INPUT_MESSAGE:
          {
            ...
            switch (message_set)
            {
                ...
                switch (rr_rr_msg_ptr->header.rr_message_set)
                {
                  case RR_DTM_IMSG:
                  {
                    switch (rr_rr_msg_ptr->header.imh.message_id)
                    {
                      case RR_DTM_IMSG_CS_RELEASE_REQ:
                      {
                        new_event = dtm_assign_command_control(EV_DTM_CS_RELEASE_REQUIRED, NULL);
                        break;
                      }
              ...
              case MS_RR_L2:
              {
                ...
                new_event = dtm_assign_command_control(EV_INPUT_MESSAGE, message_ptr);
                break;
              }
          ...
          case EV_DTM_UL_TBF_REQ:
          {
            ...
            (void) dtm_assign_command_control(EV_DTM_UL_TBF_REQ, NULL);
            break;
          }
          ...
          case EV_DTM_PERFORM_CSPS_RELEASE:
          {
            ...
            (void) dtm_assign_command_control(EV_RESET_SOFTWARE, NULL);
            (void) dtm_cell_capability_monitor(EV_RESET_SOFTWARE);
      ...
      case DTM_DTM_TRANSFER:
      {
        switch ( event_in )
        {
                    ...
                    switch (rr_rr_msg_ptr->header.imh.message_id)
                    {
                      case RR_DTM_IMSG_CS_RELEASE_REQ:
                      {
                        ...
                        if (imsg_ptr->edtm_cs_release_allowed &&
                            (rr_gprs_edtm_ps_release_required() == FALSE))
                        {
                          /* PS is not being released, use the hybrid EDTM mode */
                          rr_gprs_edtm_signal_cs_release();
          ...
          case EV_DTM_ASSIGNMENT_COMMAND_RECEIVED:
          {
            ...
            {
              // Start the procedure to configure MAC/L1 with the new DTM Assignment
              (void) dtm_assign_command_control(EV_DTM_ASSIGNMENT_READY_IN_DTM, NULL);
          ...
          case EV_INTRA_CELL_HANDOVER:
          {
            ...
            rr_send_grr_mac_dtm_channel_release_ind(HANDOVER_REL);
      ...
      case DTM_GTTP_TRANSFER:
      {
        switch ( event_in )
        {
              ...
              case MS_RLC_RR:
              {
                switch (message_id)
                {
                  case RG_UL_DCCH_REQ:
                  {
                    ...
                    send_gttp_data_ota(ul_dcch_req->tlli, &ul_dcch_req->gttp_llc_pdu_container);
                    break;
                  }
          ...
          case EV_DTM_L2_DATA_ACK_IND:
          {
            ...
            rr_send_grr_mac_dtm_ul_dcch_cnf(NO_ERROR);
            rr_dtm_data_ptr->state = DTM_DTM_POSSIBLE;
            break;
          }
      ...
      case DTM_DED_PROC_IN_PROGRESS:
      {
        switch ( event_in )
        {
                  ...
                  case RR_PS_OMSG:
                  {
                    switch (rr_rr_msg_ptr->header.imh.message_id)
                    {
                      case RR_PS_OMSG_NO_PS_ACCESS_CNF:
                      {
                         ...
                         */
                        CALL_RR_CB_FN(rr_dtm_data_ptr->data_ptr->no_ps_access_confirm_cb_fn);
              ...
              case MS_MAC_RR:
              {
                switch (message_id)
                {
                  case MAC_GRR_PS_ACCESS_CNF:
                  {
                    ...
                    CALL_RR_CB_FN(rr_dtm_data_ptr->data_ptr->ps_access_confirm_cb_fn);
                  ...
                  {
                    rr_ps_access_control((const rr_cmd_bdy_type *)message_ptr);
                    break;
                  }
          ...
          case EV_HANDOVER_COMPLETED:
          {
            ...
            SI6 / DTM INFORMATION
            */
            if (dtm_cell_capability_monitor(EV_HANDOVER_COMPLETED) == EV_DTM_CELL_SUPPORTS_DTM)
            {
              // Resume GMM (if required)
              (void) rr_gmm_resume_gprs();
      ...
      case DTM_NO_PS_DOMAIN:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_INFORMATION_RECEIVED:
          {
            (void) dtm_cell_capability_monitor(EV_DTM_INFORMATION_RECEIVED);
            break;
          }

          case EV_DTM_SI6_RECEIVED:
          {
            if (EV_DTM_CELL_DOES_NOT_SUPPORT_DTM == dtm_cell_capability_monitor(EV_DTM_SI6_RECEIVED))
            {
      ...
      case DTM_CELL_DOES_NOT_SUPPORT_DTM:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_SI6_RECEIVED:
          {
            if (EV_DTM_CELL_SUPPORTS_DTM == dtm_cell_capability_monitor(event_in))
            {
              // Resume GPRS in GMM
              (void) rr_gmm_resume_gprs();
      ...
      case DTM_NO_PS_ACCESS_UL_BLOCKED:
      {
        switch ( event_in )
        {
          ...
          case EV_DTM_SI6_RECEIVED:
          {
            new_event = dtm_cell_capability_monitor(event_in);
            break;
          }
          ...
          case EV_DTM_PACKET_ASSIGNMENT_RECEIVED:
          {
            ...
            {
              process_result = respond_to_dtm_assignment_command();
            }
            else
            {
              process_result = process_packet_assignment();
            }
      ...
      case DTM_G2X_HANDOVER:
      {
        switch ( event_in )
        {
          ...
          case RR_EV_G2W_HO_FAIL:
          {
            ...
            (void) dtm_cell_capability_monitor(RR_EV_G2W_HO_FAIL);
            rr_dtm_data_ptr->released_in_gttp = FALSE;
            rr_dtm_data_ptr->state            = DTM_DED_PROC_IN_PROGRESS;
            break;
          }
 ...
 * \param
 */
void rr_dtm_w2g_ho(boolean ps_attached )
{
  ...
  if (ps_attached)
  {
    (void) rr_dtm_control(EV_DTM_W2G_HO_PS_ATTACHED, NULL);
  }
  else
  {
    (void) rr_dtm_control(EV_DTM_W2G_HO, NULL);
  }
 ...
 * \param 
 */
void rr_dtm_send_imsg_cs_est_ind( void )
{
  if (rr_get_nv_dtm_enabled())
  {
    (void) rr_dtm_control(EV_RR_EST_IND, NULL);
  }
 ...
 * \param  
 */
void rr_dtm_send_imsg_cs_conn_ind( void )
{
  if (rr_get_nv_dtm_enabled())
  {
    (void) rr_dtm_control(EV_DTM_RR_EST_CNF, NULL);
  }
 ...
 * \param 
 */
void rr_dtm_send_imsg_cs_rel_ind( void )
{
  if (rr_get_nv_dtm_enabled())
  {
    (void) rr_dtm_control(EV_DTM_CS_RELEASED, NULL);
  }
 ...
 * \param 
 */
void rr_dtm_send_imsg_l2_gttp_ack_ind( void )
{
  (void) rr_dtm_control(EV_DTM_L2_DATA_ACK_IND, NULL);
 ...
 * \param 
 */
void rr_dtm_dual_transfer_confirmed(rr_mac_status_t mac_status )
{
   ...
   */
  grr_controlled = rr_is_control_state(RR_GRR);
  if ((grr_controlled == TRUE) || (mac_status != MAC_TRANSFER))
  {
#ifdef FEATURE_GSM_EDTM
    rr_gprs_edtm_dual_transfer_started(grr_controlled);
#else
 ...
 */
boolean rr_dtm_can_process_msc_change_ind( void )
{
 ...
 */
boolean rr_dtm_can_process_no_ps_access_req( void )
{
  boolean process = TRUE;
  rr_dtm_static_data_t *rr_dtm_data_ptr;
 ...
 * \return boolean - TRUE if the message can be handled, FALSE otherwise
 */
boolean rr_dtm_can_process_ps_access_req( void )
{
  boolean process = TRUE;
  rr_dtm_static_data_t *rr_dtm_data_ptr;
 ...
 * \return boolean - TRUE if the message can be handled, FALSE otherwise
 */
boolean rr_dtm_can_process_cs_release_req( void )
{
  boolean process = TRUE;
  rr_dtm_static_data_t *rr_dtm_data_ptr;
 ...
 */
uint16 rr_dtm_get_max_LAPDm_octets_per_subs_dyn( void )
{
  gprs_scell_info_t *cell_info = rr_gprs_get_scell_info();

  if (rr_in_dedicated() && cell_info->dtm_sys_info_valid)
  {
...
} /* end rr_dtm_get_max_LAPDm_octets_per_subs_dyn */
 ...
 * \return uint16 - number of octets
 */
uint16 rr_dtm_get_max_LAPDm_octets_dyn(void)
{
  return rr_dtm_get_max_LAPDm_octets_per_subs_dyn();
}
