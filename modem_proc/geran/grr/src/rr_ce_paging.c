...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_ce_paging.c#4 $ */
/* $DateTime: 2018/10/11 08:21:00 $$Author: pwbldsvc $ */
...
extern void ghdi_ds_read_imeisv(byte *imeisv);
#endif
...
static rr_paging_data_t rr_paging_data;
 ...
 */
static rr_paging_data_t *rr_ce_paging_get_data_ptr( void )
{
  return &rr_paging_data;
}
...
)
{
  ...
  *decoding_ok = TRUE;
  ...
  switch (type_of_identity)
  {
    case IMSI :
    {
      if (rr_siminfo_get_imsi(imsi,sizeof(imsi)))
      {
        current_mobile = TRUE;
...
static boolean rr_ce_paging_decode_rest_octet(ie_descriptor_T *current_IE,
                                                    uint8 msg_type,
                                                    csn_rr_decoder_msg_id_t decoder_type,
                                                    void *dst )
{
  ...
  {
    remaining_length = rr_length_to_end_of_ota_msg(msg_type, current_IE->start_IE, decoder_type);

    (void)gcsn1_rr_decoder(decoder_type, current_IE->start_IE, dst, remaining_length);
    return TRUE;
  }
  return FALSE;
}
...
)
{
  ...
  ie_descriptor_T *temp_current_IE;
  ...
  /*Decode the IR fied and PEO BCCH CHANGE MARK /RCC*/
  temp_current_IE = current_IE->next_IE;
  if (rr_ce_paging_decode_rest_octet(temp_current_IE, 
    ...
    )
  {
    ...
    {
      p1_rest_octets.page_rest_octets_rel13_params.rcc = rr_nv_get_peo_rcc_from_efs();
      p1_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark = rr_nv_get_peo_bcch_change_mark_from_efs();
      p1_rest_octets.page_rest_octets_rel13_params.r13_addition_flag = TRUE;
    }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

    /*Store the peo bcch change mark if received in PAGING REQUEST TYPE 1*/
    if (p1_rest_octets.page_rest_octets_rel13_params.r13_addition_flag)
    {
      if (rr_process_rcc_or_bcch_change(p1_rest_octets.page_rest_octets_rel13_params.rcc, 
                                        p1_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark))
      {
        return FALSE;
      }    
    }
    ...
    rr_ps_update_implicit_reject(p1_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);

#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
   if (rr_nv_is_ir_eab_testing_params_present()) 
   {
     p1_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject = rr_nv_get_implicit_reject_during_rach_from_efs();
   }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

    /*Act on PS IR reject */
    if (p1_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject)
    {
    ...
    MSG_GERAN_HIGH_3_G("PEO_BCCH_CHANGE_MARK = %d, RCC= %d, IR= %d",
                        p1_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark,
                        p1_rest_octets.page_rest_octets_rel13_params.rcc,
                        p1_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);
    memset(&p1_rest_octets, 0, sizeof(p1_rest_octets));
  }
  else
  {
    MSG_GERAN_HIGH_0_G("Page rest octet == NULL");
  }
...
)
{
  ...
  rr_ce_check_for_page_mode_update(own_paging_block, paging_mode);
  ...
  /*Decode the IR fied and PEO BCCH CHANGE MARK /RCC well ahead of matching page*/
  temp_current_IE = current_IE->next_IE;
  if (rr_ce_paging_decode_rest_octet(temp_current_IE, 
     ...
     )
  {
    ...
    if (rr_nv_is_peo_testing_params_present())
    {
      p2_rest_octets.page_rest_octets_rel13_params.rcc = rr_nv_get_peo_rcc_from_efs();
      p2_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark = rr_nv_get_peo_bcch_change_mark_from_efs();
      p2_rest_octets.page_rest_octets_rel13_params.r13_addition_flag = TRUE;
    }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/ 

    /*Store the peo bcch change mark if received in PAGING REQUEST TYPE 2*/
    if (p2_rest_octets.page_rest_octets_rel13_params.r13_addition_flag)
    {
      if (rr_process_rcc_or_bcch_change(p2_rest_octets.page_rest_octets_rel13_params.rcc, 
                                        p2_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark))
      {
        return FALSE;
      }  
...
#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
    /* Update the Implicit PS reject */
    rr_ps_update_implicit_reject(p2_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
   if (rr_nv_is_ir_eab_testing_params_present() )
   {
     p2_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject = rr_nv_get_implicit_reject_during_rach_from_efs();
   }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

    /*Act on PS IR reject */
    if (p2_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject)
    {
      if (rr_ps_is_low_priority())
      {
        if (grr_tbf_est_in_progress())
    ...
    MSG_GERAN_HIGH_3_G("PEO_BCCH_CHANGE_MARK = %d, RCC= %d, IR= %d",
                        p2_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark,
                        p2_rest_octets.page_rest_octets_rel13_params.rcc,
                        p2_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);
    memset(&p2_rest_octets, 0, sizeof(p2_rest_octets));
  }
  else
  {
...
)
{
  ...
  ie_descriptor_T   *temp_current_IE;  
  ie_descriptor_T *identity_IE = NULL;
  p3_rest_octets_t p3_rest_octets;
  paging_mode_T paging_mode;
  boolean current_mobile = FALSE;
  boolean rr_sim_info_available;
  byte index;
  ...
  if (current_IE == NULL)
  {
   ...
   temp_current_IE = temp_current_IE->next_IE;  
      ...
      )
   {
    ...
    if (rr_nv_is_peo_testing_params_present())
    {
      p3_rest_octets.page_rest_octets_rel13_params.rcc = rr_nv_get_peo_rcc_from_efs();
      p3_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark = rr_nv_get_peo_bcch_change_mark_from_efs();
      p3_rest_octets.page_rest_octets_rel13_params.r13_addition_flag = TRUE;
    }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/  
     /*Store the peo bcch change mark if received in PAGING REQUEST TYPE 1*/
     if (p3_rest_octets.page_rest_octets_rel13_params.r13_addition_flag)
     {
       if (rr_process_rcc_or_bcch_change(p3_rest_octets.page_rest_octets_rel13_params.rcc, 
                                         p3_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark))
       {
         return FALSE;
       }  
     }
...
#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
     /* Update the Implicit PS reject */
     rr_ps_update_implicit_reject(p3_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
     if (rr_nv_is_ir_eab_testing_params_present())
     {
       p3_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject = rr_nv_get_implicit_reject_during_rach_from_efs();
     }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

     /*Act on PS IR reject */
     if (p3_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject)
     {
       if (rr_ps_is_low_priority())
       {
         if (grr_tbf_est_in_progress())
     ...
     MSG_GERAN_HIGH_3_G("PEO_BCCH_CHANGE_MARK = %d, RCC= %d, IR= %d",
                         p3_rest_octets.page_rest_octets_rel13_params.peo_bcch_change_mark,
                         p3_rest_octets.page_rest_octets_rel13_params.rcc,
                         p3_rest_octets.page_rest_octets_rel10_params.implicit_ps_reject);
     memset(&p3_rest_octets, 0, sizeof(p3_rest_octets));
   }
   else
   {
     MSG_GERAN_HIGH_0_G("Page rest octet == NULL");
 ...
 */
static void rr_ce_update_l1_page_mode(paging_mode_T page_mode )
{
  // Obtain the pointer to the module data
  rr_paging_data_t *rr_paging_data_ptr = rr_ce_paging_get_data_ptr();
...
boolean rr_ce_compare_ota_ptmsi(byte *IE )
{
...
void rr_ce_process_page_request(byte message_type, boolean own_paging_block )
{
  rr_page_info_t page_info;
...
void rr_ce_save_page(rr_page_info_t *page_info_ptr )
{
  if (page_info_ptr != NULL)
  {
    // Obtain the pointer to the module data
...
void rr_ce_discard_saved_pages( )
{
  // Obtain the pointer to the module data
  rr_paging_data_t *rr_paging_data_ptr = rr_ce_paging_get_data_ptr();
...
void rr_ce_send_saved_pages_to_nas( )
{
  // Obtain the pointer to the module data
  rr_paging_data_t *rr_paging_data_ptr = rr_ce_paging_get_data_ptr();
  ...
  if (rr_paging_data_ptr->saved_ps_page_info_ptr != NULL)
  {
    rr_ce_send_page_to_nas(rr_paging_data_ptr->saved_ps_page_info_ptr);
...
void rr_ce_send_page_to_nas(rr_page_info_t *page_info_ptr )
{
         ...
         (page_mode != rr_paging_data_ptr->ota_page_mode) )
    {
...
#endif /*FEATURE_GSM_GPRS_READ_CCCH_IN_XFER */      
      {
        ...
        if (!rr_ue_in_peo_mode())
        {
          ...
          if (!((page_mode == PAGING_REORGANIZATION)&& (get_waiting_for_surround_response())))
          {
        ...
        {
          MSG_GERAN_HIGH_0_G("PageMode Ignored , UE in PEO mode");         
        }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/ 
      }
      /*
...
paging_mode_T rr_ce_get_ota_page_mode( )
{
  // Obtain the pointer to the module data
  rr_paging_data_t *rr_paging_data_ptr = rr_ce_paging_get_data_ptr();
...
void rr_ce_set_ota_page_mode(paging_mode_T new_page_mode )
{
  // Obtain the pointer to the module data
  rr_paging_data_t *rr_paging_data_ptr = rr_ce_paging_get_data_ptr();
...
void rr_ce_paging_task_start_init( void )
{
