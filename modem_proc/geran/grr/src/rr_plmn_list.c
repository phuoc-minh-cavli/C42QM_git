...
typedef struct
{
  ...
  sys_found_plmn_list_u_type   available_plmn_list;
  uint64                       gps_plmn_search_scan_ref_time;
...
static rr_plc_data_t  rr_plc_data;
 ...
 */
static rr_plc_data_t *rr_plc_get_data_ptr( void )
{
  return &rr_plc_data;
}
...
static const char * rr_plc_gsm_state_name(rr_plc_gsm_state_e state)
{
  switch (state)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_1("Unexpected state: %d", state);
      return "?";
    }
  }
}
...
void rr_plc_task_start_init( void )
{
  ...
  rr_plc_data.band_filter_mask = SYS_BAND_MASK_EMPTY;
 ...
 */
sys_rat_pri_list_info_s_type *rr_plc_get_rat_pri_list_info( )
{
 ...
 * \return boolean
 */
static boolean rr_plc_gsm_only( )
{
  return rr_rat_pri_list_info_gsm_only(
    rr_plc_get_rat_pri_list_info()
  );
}
 ...
 * \return boolean
 */
static boolean rr_plc_is_gsm_supported( )
{
...
/*!
 * \brief Return TRUE if the indicated RAT is present in the RAT Priority List, FALSE otherwise.
 *
 * \param rat_id (in)
 *
 * \return boolean
 */
static boolean rr_plc_is_rat_supported(const rr_rat_e rat_id  )
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  if (!geran_is_multi_mode_gas_id())
  {
    /* current multimode mappings do not support this operation */
    MSG_GERAN_ERROR_1_G("This is not the multimode subscription, ignore RAT ID %d", (int)rat_id);
    return(FALSE);
  }
  else if (rr_ms_other_sim_in_background_traffic())
  {
    ...
    /* unless the specified RAT also supports X2G tuneaway (e.g. WCDMA/TDSCDMA)          */
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
    if (geran_g2w_tuneaway_supported_by_device_mode(geran_get_multi_sim_mode()))
    {
      switch (rat_id)
      {
#ifdef FEATURE_WCDMA
    ...
    MSG_GERAN_ERROR_1_G("PLMN search: no G2X during BG_TRAFFIC, ignore RAT ID %d", (int)rat_id);
    return(FALSE);
  }
 ...
 * \return boolean
 */
static boolean rr_plc_is_tds_supported( )
{
  if (!rr_plc_is_rat_supported(RR_RAT_UTRAN_TDD))
  {
    return(FALSE);
  }
  ...
  {
    MSG_GERAN_MED_0_G("TDSCDMA segment not loaded");
    return(FALSE);
  }
 ...
 * \return boolean
 */
static boolean rr_plc_is_wcdma_supported( )
{
  if (!rr_plc_is_rat_supported(RR_RAT_WCDMA))
  {
    return(FALSE);
  }
  ...
  {
    MSG_GERAN_MED_0_G("WCDMA segment not loaded");
    return(FALSE);
  }
 ...
 * \return boolean
 */
static boolean rr_plc_is_lte_supported( )
{
  if (!rr_plc_is_rat_supported(RR_RAT_LTE))
  {
    return(FALSE);
  }
 ...
 * \return boolean
 */
static sys_band_mask_type rr_plc_pscan_band_pref( )
{
 ...
 * \return sys_band_mask_type
 */
static sys_band_mask_type rr_plc_band_pref( )
{
  sys_band_cap_u_type band_cap_u = {0};
 ...
 * \return boolean
 */
static boolean rr_passed_service_search_csg_check( )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
...
#ifdef FEATURE_WCDMA
  wcdma_supported = rr_plc_is_wcdma_supported();
#endif /* FEATURE_WCDMA */

#ifdef FEATURE_LTE
  lte_supported = rr_plc_is_lte_supported();
#endif /* FEATURE_LTE */
...
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS) || defined (FEATURE_LTE)
static boolean rr_plc_should_available_plmn_list_be_filtered( )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info = rr_get_plmn_list_req_info_ptr();
  boolean filter = TRUE;
...
)
{
  ...
  if (rr_plc_should_available_plmn_list_be_filtered())
  {
    uint32 i = 0;
    ...
    for (i = 0;
         ...
         ++i)
    {
      if (rr_plc_find_plmn_in_request_list(&available_plmn_list->info[i]))
      {
...
)
{
  ...
  if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_CELL_SEARCH)
  {
    uint8 num_cells_to_search = rr_plmn_list_req_info_ptr->cell_search_num_cells;

    if (num_cells_to_search == 0 || num_cells_to_search > SYS_PCI_SCAN_MAX_NUM_PCI)
    {
      num_cells_to_search = SYS_PCI_SCAN_MAX_NUM_PCI;
    }

    return rr_candidate_db_found_best_pci_cells(num_cells_to_search);
  }
      ...
      (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_BACKGROUND_AUTOMATIC))
  {
    ...
    if (rr_plmn_list_get_first_gsm_plmn(&plmn_to_compare))
    {
     compare = TRUE;
    }
  }
 ...
 */
static boolean rr_plc_w2g_bplmn_check_if_hplmn_found( )
{
  uint32 i = 0;
  ...
  // Obtain a pointer to the module data
  rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);

  rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  //RR_NULL_CHECK_FATAL(rr_plmn_list_req_info_ptr);
...
static rr_plc_rat_search_info_t * rr_plc_rat_search_info_next( )
{
  uint32 i;
  rr_plc_data_t *rr_plc_data_ptr;
...
static rr_plc_rat_search_info_t * rr_plc_rat_search_info_find(rr_rat_e rat  )
{
  uint32 i;
  rr_plc_data_t *rr_plc_data_ptr;
...
#ifdef FEATURE_SGLTE
static void rr_plc_rat_search_set_to_search_done( )
{
  uint32 i;
  rr_plc_data_t *rr_plc_data_ptr;
...
/*!
  @brief This function stores filtered LTE band_pref. Filtered band_pref contains only LTE-TDD bands. 
 */
static void rr_plc_set_band_pref_LTDD(sys_band_mask_type band_pref  )
{
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);

  MSG_GERAN_HIGH_1_G("LTE-TDD band_pref=%#016X saved in the context", band_pref);
  rr_plc_data_ptr->band_pref_LTDD = band_pref;
}

/*!
  @brief This function returns filtered LTE band_pref. Filtered band_pref contains only LTE-TDD bands. 
 */
static sys_band_mask_type rr_plc_get_band_pref_LTDD( )
{
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);

  MSG_GERAN_HIGH_1_G("LTE-TDD band_pref %#016X retrieved from the context", rr_plc_data_ptr->band_pref_LTDD);
  return rr_plc_data_ptr->band_pref_LTDD;
}

/*!
  @brief This function filters out LTE-FDD bands from a given band mask. 
         Returned band mask contains only LTE-TDD bands.
 */
static sys_band_mask_type rr_plc_band_pref_LTDD(sys_band_mask_type band_pref  )
{
  uint8              band_idx = 0;
  sys_band_mask_type lte_band_chk = 0;

  /* Filter out the LTE-FDD bands */
  for (band_idx = 0; band_pref && (band_idx < SYS_SBAND_LTE_EUTRAN_BAND_MAX); band_idx++)
  {
    lte_band_chk = (sys_band_mask_type)(((uint64)1) << band_idx);
    if (lte_band_chk & band_pref)
    {
      if (!lte_rrc_lte_tdd_is_supported(lte_band_chk))
      {
        band_pref &= ~lte_band_chk;
        MSG_GERAN_MED_1_G("Removing LTE-FDD band %d", band_idx+1);
      }
    }
  }
...
/* currently this is called only for LTE band validation hence putting it under FEATURE_LTE flag. */
#ifdef FEATURE_LTE
/*!
  @brief This function checks if band_pref passed to GRR in RR_PLMN_LIST_REQ is valid or not.
 */
static boolean rr_plc_validate_plmn_search_band(rr_rat_e rat  )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  sys_band_cap_u_type band_cap_u = {0};
  ...
  switch(rat)
  {
    case RR_RAT_GSM:
    {
      ...
      band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_GSM);
      break;
    }
    ...
    case RR_RAT_WCDMA:
    {
      ...
      band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_WCDMA);
      break;
    }
#endif /* FEATURE_WCDMA */
    ...
    case RR_RAT_LTE:
    {
      ...
      band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_LTE);
      break;
    }
#endif /* FEATURE_LTE */
    ...
    case RR_RAT_UTRAN_TDD:
    {
      ...
      band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_TDS);
      break;
    }
#endif /*FEATURE_GSM_TDS*/
 ...
 */
static void rr_plc_start_plmn_search( )
{
        ...
        switch (rat)
        {
          ...
          case SYS_RAT_TDS_RADIO_ACCESS:
          {
            if (sys_rat_tds_radio_access_included == FALSE)
            {
              ...
              {
#ifdef FEATURE_SGLTE
                if (rr_sglte_g2x_bplmn_search_active() &&
                    rr_sglte_inhibit_tds_bplmn_search())
                {
...
#ifdef FEATURE_SGLTE
              if (rr_sglte_g2x_bplmn_search_active() &&
                  rr_sglte_inhibit_lte_bplmn_search())
              {
...
#endif /*FEATURE_SGLTE*/
              if (!rr_plc_validate_plmn_search_band(RR_RAT_LTE))
              {
   ...
   */
  if (rr_plmn_db_is_valid(available_plmn_db_ptr, rr_plc_pscan_band_pref()))
  {
    rr_plc_rat_search_info_t * rat_search_info = rr_plc_rat_search_info_find(RR_RAT_GSM);
  ...
  if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_CSG)
  {
    ...
    {
      rr_log_plmn_list_started(RR_LOG_PLMN_LIST_MANUAL_CSG_LIST_SEARCH);
    }
    else
    {
      rr_log_plmn_list_started(RR_LOG_PLMN_LIST_AUTOMATIC_CSG_LIST_SEARCH);
    }
  }
  else
#endif /* FEATURE_FEMTO_CSG */
 ...
 * @Params:  N/A
 */
boolean rr_plc_search_in_foreground( )
{
  // Obtain a pointer to the module data
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
...
)
{
  ...
  result = rr_msgr_send(&msg.msg_hdr, sizeof(msg));
...
)
{
  ...
  rr_get_inter_rat_bplmn_prtl_result_dyn(&plmn_srch_rsp_ptr->available_plmn_list);

  plmn_srch_rsp_ptr->srch_done = FALSE;
  plmn_srch_rsp_ptr->hplmn_found = FALSE;
  plmn_srch_rsp_ptr->partial_srch = TRUE;

  rr_plc_detected_frequency_list_populate(&plmn_srch_rsp_ptr->detected_frequency_list);
 ...
 */
static void rr_plc_update_l1_on_completion( )
{
  ...
  if (SYS_MODEM_DEVICE_MODE_DUAL_SIM_DUAL_ACTIVE == geran_get_multi_sim_mode())
  {
    ...
    rr_send_mph_device_mode_ind(GERAN_DEVICE_MODE_CAUSE_PLMN);
  }
  else
 ...
 */
static void rr_plc_search_complete( )
{
  uint32 i;
...
#ifdef FEATURE_MPLMN_PARTIAL_UPDATING
  rr_timer_cancel(RR_PLMN_LIST_SEARCH_PARTIAL_RESULTS_TIMER);
#endif // FEATURE_MPLMN_PARTIAL_UPDATING
  rr_timer_cancel(RR_PLMN_LIST_SEARCH_GUARD_TIMER);
  ...
  *filtered_available_plmn_list_ptr = *available_plmn_list_ptr;
  ...
  switch (rr_plc_data_ptr->search_type)
  {
    ...
    case RR_PLC_BACKGROUND_SEARCH:
    {
      rr_plc_update_l1_on_completion();
    ...
    case RR_PLC_WTOG_SEARCH:
    {
      ...
      else
      {
        rr_send_interrat_plmn_search_cnf(
          TRUE,                                            // srch_done
          rr_plc_w2g_bplmn_check_if_hplmn_found(),   // hplmn_found
    ...
    case RR_PLC_LTOG_SEARCH:
    {
      ...
      else
      {
        ...
        if (rr_plmn_list_req_info_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC)
        {
          plmn_srch_rsp_ptr->hplmn_found = rr_plc_w2g_bplmn_check_if_hplmn_found();
        }
    ...
    case RR_PLC_SGLTE_X2G_BACKGROUND_SEARCH:
    {
      ...
      if (rr_plmn_list_req_info_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC)
      {
        hplmn_found = rr_plc_w2g_bplmn_check_if_hplmn_found();
      }

      rr_plc_detected_frequency_list_populate(&detected_frequency_list);
    ...
    default:
      MSG_GERAN_LOW_1_G("Unexpected search_type %d", rr_plc_data_ptr->search_type);
      break;
  }
...
===========================================================================*/
static boolean rr_ms_x2g_search_preempted( )
{
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  ...
  while (event != EV_NO_EVENT)
  {
    ...
    switch (rr_plc_data_ptr->gsm_state)
    {
      ...
      case RR_PLC_GSM_STATE_NULL:
      {
        if (event == EV_PLMN_SEARCH_START)
        {
          ...
          if ((rr_plmn_list_req_info_ptr->scan_scope == SYS_SCAN_SCOPE_ACQ_DB) &&
              (rr_num_of_arfcn_in_acq_db(rr_plc_band_pref()) == 0))
          {
          ...
          {
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
            if(rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH &&
               !(rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.is_gsm_blind_scan_req) &&
                rr_is_any_frequency_band_supported() == FALSE)
            {
               MSG_GERAN_HIGH_0_G("[GPS search] : All the requested arfcn(s) are from unsupported band");
               rr_plc_data_ptr->gsm_state = RR_PLC_GSM_STATE_NULL;
            ...
            {
              rr_pscan_send_start_ind(RR_SEARCH_BACKGROUND, rr_plc_band_pref());
                  ...
                  case RR_PS_OMSG:
                  {
                    switch (message_ptr->rr.ps.omsg.header.imh.message_id)
                    {
                      case RR_PS_OMSG_NO_PS_ACCESS_CNF:
                      {
                        ...
                        {
                          rr_pscan_send_start_ind(RR_SEARCH_NORMAL, rr_plc_band_pref());
                          rr_plc_data_ptr->gsm_state = RR_PLC_GSM_STATE_PSCAN;
                        }
                        break;
        ...
        switch(event)
        {
          case EV_INPUT_MESSAGE:
          {
            ...
            switch (message_set)
            {
                  ...
                  case RR_PSCAN_OMSG:
                  {
                    switch (message_ptr->rr.pscan.omsg.header.imh.message_id)
                    {
                      case RR_PSCAN_OMSG_FINISHED_IND:
                      {
                        ...
                        (void) rr_pscan_process_power_scan_results(RR_NORMAL_SEARCH_ORDER);

                        num_freqs = rr_candidate_db_get_num_frequencies(candidate_db_ptr);

                        if (num_freqs != 0)
                        {
                          sys_band_mask_type filter_band_mask = rr_plc_band_pref();
              ...
              case MS_RR_L1:
              {
                rr_pscan_control(event, message_ptr);
                break;
              } /* MS_RR_L1 */
      ...
      case RR_PLC_GSM_STATE_BCCH_LIST_DECODE:
      {
        switch(event)
        {
            ...
            switch (message_set)
            {
              case MS_RR_RR:
              {
                switch (message_ptr->rr.header.rr_message_set)
                {
                  ...
                  {
                    rr_decode_bcch_list_control(event, message_ptr);
                    break;
                  } /* RR_DECODE_BCCH_IMSG */
          ...
          case RR_EV_CANDIDATE_CELL_SI_RECEIVED:
          {
            ...
            if (rr_plc_plmn_search_should_stop(available_plmn_db_ptr))
            {
      ...
      default:
      {
        ...
        {
          MSG_GERAN_LOW_1_G("Unexpected state: %d, triggering recovery", rr_plc_data_ptr->state);
        ...
        case MS_MSGR_RR:
        {
          switch (message_ptr->rr_msgr_msg.message.hdr.id)
          {
            ...
            case GERAN_GRR_LTE_GET_PLMN_PRTL_RESULTS_REQ:
            {
              ...
              rr_plc_send_geran_grr_lte_irat_plmn_srch_rsp_partial();
    ...
    switch (rr_plc_data_ptr->x2g_state)
    {
      case RR_PLC_X2G_STATE_NULL:
      {
        if (event == EV_PLMN_SEARCH_START)
        {
           ...
           */
          if (rr_plc_band_pref() == SYS_BAND_MASK_EMPTY)
          {
            ...
            rr_l1_gsm_stop_reason_set(rr_mode_get_l1_stop_reason());
          ...
          {
            if (rr_num_of_arfcn_in_acq_db(rr_plc_band_pref()) == 0)
            {
            ...
            {
              new_event = rr_mode_control(EV_ACTIVATE_GSM_MODE_FOR_BPLMN, message_ptr);
        ...
        switch (event)
        {
          case EV_GSM_MODE_DEACTIVATED:
          {
            switch (rr_plc_data_ptr->search_type)
            {
              ...
              case RR_PLC_WTOG_SEARCH:
              {
                 rr_send_interrat_plmn_search_cnf(FALSE, FALSE, NULL);
                break;
              } /* RR_PLC_WTOG_SEARCH */
#endif /* FEATURE_WCDMA */
      ...
      case RR_PLC_X2G_STATE_PSCAN:
      {
        switch (event)
        {
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
                    if (rr_ms_x2g_search_preempted())
                    {
                      /* GL1 can be resumed immediately */
                      next_state = RR_PLC_X2G_STATE_PSCAN_RESUMING;
                      rr_send_mph_resume_gsm_mode_for_bplmn_req(rr_plc_data_ptr->x2g_update_timer_val);
                    }
                    else
#endif /*  FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
                    {
                       ...
                       case RR_PSCAN_OMSG_FINISHED_IND:
                       {
                         if (rr_pscan_process_power_scan_results(RR_NORMAL_SEARCH_ORDER) != 0)
                         {
                           ...
                           else
                           {
                             ...
                             if (rr_plc_search_in_foreground() == TRUE)
                             {
                               (void) rr_candidate_db_send_decode_bcch_start_ind(
                                 RR_SEARCH_NORMAL,   // bcch_decode_type
                                 FALSE,              // si3_only
          ...
          case EV_PLMN_SEARCH_ABORT:
          {
            ...
            rr_pscan_control(EV_RESET_SOFTWARE, NULL);
          ...
          case EV_PLMN_SEARCH_SUSPEND:
          {
            ...
            rr_send_mph_suspend_gsm_mode_req();
            break;
          }
        ...
        switch(event)
        {
          ...
          case RR_EV_CANDIDATE_CELL_SI_RECEIVED:
          {
            ...
            if (rr_plc_plmn_search_should_stop(available_plmn_db_ptr))
            {
              ...
              rr_decode_bcch_list_control(EV_RESET_SOFTWARE, NULL);
...
)
{
  ...
  if (plmn_srch_req.search_type == SYS_SERVICE_SEARCH_IRAT_PLMN)
  {
    /* Use the band_pref that is saved while validating PLMN_LIST_REQ */
    plmn_srch_req.band_pref = rr_plc_get_band_pref_LTDD();
  }
  ...
  result = rr_msgr_send(&plmn_srch_req.msg_hdr, sizeof(plmn_srch_req));
  if (result != E_SUCCESS)
  {
    MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
  }
...
)
{
  ...
  switch (rat)
  {
    ...
    case RR_RAT_UTRAN_TDD:
    {
      ...
      rr_send_tds_interrat_plmn_search_abort_req();
      (void)rr_timer_start(RR_IRAT_PROC_TIMER,RR_IRAT_PROC_TIMEOUT);
    ...
    case RR_RAT_WCDMA:
    {
      ...
      rr_send_interrat_plmn_search_abort_req();
      (void)rr_timer_start(RR_IRAT_PROC_TIMER,RR_IRAT_PROC_TIMEOUT);
    ...
    case RR_RAT_LTE:
    {
      ...
      result = rr_msgr_send(&abort_plmn_srch_req.msg_hdr, sizeof(abort_plmn_srch_req));
      if (result != E_SUCCESS)
      {
        MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
      }
        ...
        switch (event)
        {
          case EV_PLMN_SEARCH_START:
          {
            switch (rr_plc_data_ptr->rat_search_info_current->rat)
            {
              ...
              case RR_RAT_UTRAN_TDD:
              {
                sys_band_cap_u_type band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_TDS);
                MSG_GERAN_HIGH_0_G("Starting IRAT PLMN search on TDSCDMA");
                rr_update_band_preference_for_tdsrrc(band_cap_u.chgwt_band_cap);
                break;
              } /* RR_RAT_UTRAN_TDD */
#endif /* FEATURE_GSM_TDS */
              ...
              case RR_RAT_WCDMA:
              {
                sys_band_cap_u_type band_cap_u = rr_plc_band_pref_sys_mode(SYS_SYS_MODE_WCDMA);
      ...
      case RR_PLC_G2X_STATE_FOREGROUND_DEACTIVATING_L1:
      {
        switch (event)
        {
          case EV_GSM_MODE_DEACTIVATED:
          {
            ...
            switch (rr_plc_data_ptr->rat_search_info_current->rat)
            {
              ...
              case RR_RAT_LTE:
              {
                ...
                rr_plc_send_lte_rrc_g_plmn_srch_req(RR_MAX_PLMN_LIST_SEARCH_TIME, &detected_frequency_list);
          ...
          case EV_PLMN_SEARCH_ABORT:
          {
            ...
            rr_timer_cancel(RR_PLMN_LIST_SEARCH_GUARD_TIMER);
            new_event = rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
                ...
                switch (message_id)
                {
                  case TDSRRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    ...
                    else 
                    {
                      ...
                      rr_plc_g2x_send_abort_req(rr_plc_data_ptr->rat_search_info_current->rat);
            ...
            switch (message_set)
            {
              ...
              case MS_TDSRRC_RR:
              {
                switch (message_id)
                {
                  ...
                  case TDSRRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    (void)rr_timer_cancel(RR_IRAT_PROC_TIMER);
                    new_event = rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
                    rr_plc_data_ptr->g2x_state = RR_PLC_G2X_STATE_FOREGROUND_ACTIVATING_L1;
                    break;
                    ...
                    if (message_ptr->mph.suspend_gsm_mode_cnf.suspended)
                    {
                      if (message_ptr->mph.suspend_gsm_mode_cnf.timer_value >= MIN_REQUIRED_TIME_FOR_IRAT_PLMN_SEARCH)
                      {
                        ...
                        switch (rr_plc_data_ptr->rat_search_info_current->rat)
                        {
                          ...
                          case RR_RAT_UTRAN_TDD:
                          {
                            ...
                            if (rr_is_limited_plmn_selection_mode() == FALSE &&
                                rr_cell_selection_is_camped_cell_suitable())
                            {
                      ...
                      else
                      {
                        MSG_GERAN_MED_1_G("Insufficient time %d indicated by L1, resume and try again",
                                message_ptr->mph.suspend_gsm_mode_cnf.timer_value);
                        rr_send_mph_resume_gsm_mode_req();
...
===========================================================================*/
static boolean rr_passed_service_search_irat_plmn_check( )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr;
  ...
  if (plmn_check_pass)
  {
    ...
    if (plmn_check_pass)
    {
      ...
      if (rr_priority_and_threshold_information_available(
          ...
          && (rr_is_lte_l1_measurements_configured())
         )
      {
...
#ifdef FEATURE_SGLTE
static boolean rr_plc_sglte_g2x_start_search( )
{
  rr_PLMN_list_req_info_T      *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  rr_plc_data_t                *rr_plc_data_ptr;
...
)
{
  ...
  for (index=0; index<plmn_db_ptr->num_plmn; index++)
  {
    ...
    for (cell_index = 0; cell_index < plmn_db_ptr->plmn_info[index].num_frequencies;++cell_index)
    {
      ...
      {
        if (!rr_check_lai_in_reject_list(&plmn_db_ptr->plmn_info[index].cell_info[cell_index].lai))
        {
          break;
        }
      }
      ...
      rr_plmn_list_req_info_ptr->network_selection_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)
  {
    ...
    meas = rr_gprs_get_serving_measurements();
  ...
  {
    {
      ...
      {
        rr_l1_freq_power_T meas;
        gprs_scell_info_t *other_scell_info_ptr = rr_gprs_get_scell_info();
        meas = rr_gprs_get_serving_measurements();

        //RR_NULL_CHECK_FATAL(other_scell_info_ptr);

        MSG_GERAN_HIGH_0_G("Adding PLMN to search results");
 ...
 * @Outputs:
 */
boolean rr_plc_is_automatic_search( )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  //RR_NULL_CHECK_FATAL(rr_plmn_list_req_info_ptr);
 ...
 */
uint32 rr_plc_get_wtog_search_timer_value( )
{
  // Obtain a pointer to the module data
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
...
boolean rr_plc_l2g_search_in_progress( )
{
  // Obtain a pointer to the module data
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
...
)
{
  ...
  if ((ps_service_enable) && (rr_mac_ul_tbf_est_is_allowed()))
  {
 ...
 */
boolean rr_plc_get_ps_service_priority( )
{
  // Obtain a pointer to the module data
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
 ...
 */
rr_action_after_plc_e rr_plc_get_action_after_plmn_search_complete( )
{
    ...
    switch (rr_plc_data_ptr->state)
    {
        ...
        switch(event)
        {
          case EV_PLMN_SEARCH_START:
          {
            ...
            if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_CSG)
            {
              ...
              service_search_proceed = rr_passed_service_search_csg_check();
            }
#endif /* FEATURE_FEMTO_CSG */

            rr_plc_start_plmn_search();

            rr_plc_data_ptr->rat_search_info_current = rr_plc_rat_search_info_next();

#ifdef FEATURE_LTE
            if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)
            {
              service_search_proceed = rr_passed_service_search_irat_plmn_check();
            }
#endif /* FEATURE_LTE */
            ...
            if (rr_plc_data_ptr->rat_search_info_current != NULL && service_search_proceed)
            {
              ...
              if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_CELL_SEARCH &&
                  rr_plmn_list_req_info_ptr->cell_search_num_cells == 1)
              {
                max_search_timeout_ms = RR_PCI_SINGLE_CELL_GUARD_DURATION;
                MSG_GERAN_HIGH_1_G(
                  "Max search timer %d secs for single cell scan",
              ...
              if (rr_plmn_list_req_info_ptr->service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH)
              {
                MSG_GERAN_HIGH_0_G("[GPS search] : updating timer for GPS search ");
                max_search_timeout_ms = CONVERT_SECONDS_TO_MILLISECONDS(
                  rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.max_scan_duration_gsm_sec
                );
              }
              else
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
              ...
              switch (rr_plc_data_ptr->rat_search_info_current->rat)
              {
                case RR_RAT_GSM:
                {
                  new_event = rr_plc_gsm_control(EV_PLMN_SEARCH_START, NULL);
                ...
                case RR_RAT_UTRAN_TDD:
                {
                  ...
                  new_event = rr_plc_g2x_control(EV_PLMN_SEARCH_START, NULL);
...
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
          {
            ...
            if (rr_plc_data_ptr->rat_search_info_current)
            {
              ...
              new_event = rr_plc_x2g_control(EV_PLMN_SEARCH_START, NULL);
      ...
      case RR_PLC_STATE_GSM:
      {
        switch (event)
        {
          ...
          {
            rr_plc_data_ptr->abort = TRUE;
            new_event = rr_plc_gsm_control(EV_PLMN_SEARCH_ABORT, NULL);
            break;
          ...
          default:
          {
            new_event = rr_plc_gsm_control(event, message_ptr);
          }
        }
        break;
      } /* RR_PLC_STATE_GSM */
      ...
      case RR_PLC_STATE_G2X:
      {
        switch (event)
        {
          ...
          case EV_PLMN_LIST_SEARCH_GUARD_TIMER_EXPIRED:
          {
            ...
            new_event = rr_plc_g2x_control(EV_PLMN_SEARCH_ABORT, NULL);
            break;
          } /* EV_PLMN_LIST_SEARCH_GUARD_TIMER_EXPIRED */
          ...
          case EV_PLMN_LIST_SEARCH_PARTIAL_RESULTS_TIMER_EXPIRED:
          {
            ...
            if (rat == RR_RAT_LTE)
            {
              rr_plc_send_lte_rrc_g_get_plmn_prtl_results_req();
            }
            else
#endif /* FEATURE_LTE */
            {
          ...
          default:
          {
            new_event = rr_plc_g2x_control(event, message_ptr);
          }
        }
        break;
      } /* RR_PLC_STATE_G2X */
      ...
      case RR_PLC_STATE_X2G:
        switch (event)
        {
          ...
          default:
            new_event = rr_plc_x2g_control(event, message_ptr);
            break;
        }
        break;
#endif /* FEATURE_WCDMA || FEATURE_LTE || FEATURE_GSM_TDS */
...
===========================================================================*/
boolean rr_plc_wtog_decoding_bcch( )
{
  rr_plc_data_t *rr_plc_data_ptr;
  ...
  if (rr_plc_data_ptr != NULL)
  {
    ...
    return(rr_background_plmn_search_in_progress() &&
           (rr_plc_data_ptr->state == RR_PLC_STATE_X2G) &&
           (rr_plc_data_ptr->x2g_state == RR_PLC_X2G_STATE_BCCH_LIST_DECODE) );
  }
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
boolean rr_plc_bplmn_pscan_active( )
{
  // Obtain a pointer to the module data
  rr_plc_data_t *rr_plc_data_ptr = rr_plc_get_data_ptr();
  rr_PLMN_list_req_info_T * rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_UE_MODE_CHANGE_CNF:
                  {
                    ...
                    if (rr_plc_sglte_g2x_start_search())
                    {                      
                      uint32 max_search_timeout_ms = 0;
                  ...
                  case TDSRRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    ...
                    if (plmn_srch_cnf->srch_done)
                    {
                      ...
                      if (plmn_srch_cnf->hplmn_found)
                      {
                        ...
                        rr_plc_rat_search_set_to_search_done(); /*HPLMN found stop searching*/
                      }
                      else
                      {
                        MSG_GERAN_HIGH_0_G("HPLMN not found");
                ...
                switch (message_id)
                {
                  case TDSRRC_INTERRAT_PLMN_SRCH_CNF:
                  {
                    if ((rr_plc_data_ptr->aborting_no_multirat_activation == FALSE) &&
                        (get_rr_state() != RR_GOING_INACTIVE))
                    {
...
boolean rr_is_sglte_plmn_srch_active( )
{
  rr_plc_data_t *rr_plc_data_ptr;
...
boolean rr_x2g_bplmn_search_active( )
{
  rr_plc_data_t *rr_plc_data_ptr;
  boolean search_active = FALSE;
 ...
 */
boolean rr_get_inter_rat_bplmn_prtl_result_sub_id_dyn(
  sys_detailed_plmn_list_s_type *available_plmn_list_ptr
)
{
#ifdef FEATURE_MPLMN_PARTIAL_UPDATING
  ...
  sys_detailed_plmn_list_s_type *plmn_list_ptr;
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  rr_available_plmn_db_T *rr_available_plmn_db_ptr = rr_get_available_plmn_db_ptr();
 ...
 * \return boolean - TRUE if results valid, FALSE otherwise
 */
boolean rr_get_inter_rat_bplmn_prtl_result_dyn(
  sys_detailed_plmn_list_s_type *available_plmn_list_ptr
)
{

  // Redirect to multi-SIM version indicating sub
  return rr_get_inter_rat_bplmn_prtl_result_sub_id_dyn(available_plmn_list_ptr);
}
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
...
===========================================================================*/
void rr_ms_set_plc_gsm_aborted( )
{
  rr_plc_data_t *rr_plc_data_ptr;
  rr_plc_data_ptr = rr_plc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_plc_data_ptr);
...
boolean rr_cell_search_in_progress(void)
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info = rr_get_plmn_list_req_info_ptr();

  return (rr_is_control_state(RR_PLMN_LIST_SEARCH) && rr_plmn_list_req_info->service_search == SYS_SERVICE_SEARCH_CELL_SEARCH);
}

boolean rr_gps_advance_search_in_progress( void )
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info = rr_get_plmn_list_req_info_ptr();

#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
  return (rr_is_control_state(RR_PLMN_LIST_SEARCH) && rr_plmn_list_req_info->service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH);
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
...
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
void rr_set_gps_plmn_search_scan_ref_time(uint64 scan_ref_time)
{
  rr_plc_data_t *rr_plc_data_ptr;
  rr_plc_data_ptr = rr_plc_get_data_ptr();

  rr_plc_data_ptr->gps_plmn_search_scan_ref_time = scan_ref_time;
}

uint64 rr_get_gps_plmn_search_scan_ref_time(void)
{
  return rr_plc_get_data_ptr()->gps_plmn_search_scan_ref_time;
}
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
/* EOF */
