...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_decode_ie.c#8 $ $DateTime: 2019/03/12 05:52:37 $ $Author: pwbldsvc $
...
Jan/28/04    VT     QuadBand: In rr_decode_channel_description() getting the
                      channel band using rr_get_band_dyn(), instead of getting from
...
static void rr_decode_set_band(ARFCN_T *arfcn_list_ptr, byte num_entries );
...
)
{
  ...
  *EXT_IND = (byte) (IE[0] & 0x20) >> 5;
  ...
  rr_decode_cell_channel_description(IE, &cell_channel_description,decoding_ok);
...
)
{
  ...
  cell_description->ARFCN.band = (sys_band_T) rr_get_band_dyn(cell_description->ARFCN.num);
}

#if defined(FEATURE_GERAN_CS_SUPPORT) || defined(FEATURE_GERAN_CBCH_SUPPORT)
/*
...
)
{
  ...
  */
...
#endif // FEATURE_VAMOS
    MSG_GERAN_LOW_0_G("channel_type=FULL_RATE_TRAFFIC, subchannel=n/a");
  }
  else
  /* TCH/H */
#ifdef FEATURE_VAMOS
...
#endif // FEATURE_VAMOS
  {
    ...
    TSC_S = (channel & 0x10) >> 4;  // 0x10 = 1 0 0 0 0  TSC_S = {0,1}
#endif // FEATURE_VAMOS
    MSG_GERAN_LOW_1_G("channel_type=HALF_RATE_TRAFFIC, subchannel=%x",(int)(channel & 0x01));
  }
  else
  /* SDCCH/4 */
  if ((channel & 0x1c) == 0x04)    // 0x1c = 1 1 1 0 0   0x04 = 0 0 1 0 0
  {
    ...
    MSG_GERAN_LOW_1_G("channel_type=SDCCH_4, subchannel=%x",(int)(channel & 0x03));
  }
  else
  /* SDCCH/8 */
  if ((channel & 0x18) == 0x08)    // 0x18 = 1 1 0 0 0   0x08 = 0 1 0 0 0
  {
    ...
    MSG_GERAN_LOW_1_G("channel_type=SDCCH_8, subchannel=%x",(int)(channel & 0x07));
  }
  else
  {
  ...
  if (*decoding_ok)
  {
    MSG_GERAN_HIGH_2_G("channel_type=%d and subchannel=%d in channel description2", 
      channel_information->channel_type,
      channel_information->subchannel);
  }
  ...
  else
  {
    ...
    channel_information->frequency_list.channel[0].band = (sys_band_T) rr_get_band_dyn(ARFCN);
  }

#ifdef FEATURE_VAMOS
  // If TSC2 is specified but VAMOS support is disabled, this channel mode is invalid
  if ((TSC_S == 1) && (!rr_get_nv_vamos_enabled()))
  {
...
#endif /* FEATURE_GERAN_CS_SUPPORT || FEATURE_GERAN_CBCH_SUPPORT */
  ...
  *decoding_ok = TRUE;
      ...
      for (bit_index = 0; bit_index < 8; bit_index++)
      {
        ...
        if ((IE[index] & ARFCN_mask) == ARFCN_mask)
        {
          if (ca_index < CA_MAX_LEN)
          {
            ...
            // WARNING: Need to resolve band information using SI
            cell_allocation->channel_number[ca_index].band = (sys_band_T) rr_get_band_dyn(CA_ARFCN);
  ...
  /*lint -restore */
  {
    ...
    if (F0 == 1)
    {
      ...
      // WARNING: Need to resolve band information using SI
      cell_allocation->channel_number[0].band = (sys_band_T) rr_get_band_dyn(0);
    ...
    if (cell_allocation->no_of_entries == 1)
    {
      for (index = 1; index < (size + 1); index++)
      {
        ...
        // WARNING: Need to resolve band information using SI
        cell_allocation->channel_number[index].band = (sys_band_T) rr_get_band_dyn(offset[index-1]);
      }
      qsort_arfcn(cell_allocation->channel_number,size + 1);
    }
    ...
    else
    {
      for (index = 0; index < size; index++)
      {
        ...
        // WARNING: Need to resolve band information using SI
        cell_allocation->channel_number[index].band = (sys_band_T) rr_get_band_dyn(offset[index]);
      }

      qsort_arfcn(cell_allocation->channel_number, size);
    }
  ...
  /*lint -restore */
  {
    ...
    // WARNING: Need to resolve band information using SI
    cell_allocation->channel_number[0].band = (sys_band_T) rr_get_band_dyn(ORIG_ARFCN);
    ...
    for (index = 1; index < (size + 1); index++)
    {
      ...
      // WARNING: Need to resolve band information using SI
      cell_allocation->channel_number[index].band = (sys_band_T) rr_get_band_dyn(
        cell_allocation->channel_number[index].num
      );
    }
  ...
  /*lint -restore  */
  {
    ...
    // WARNING: Need to resolve band information using SI
    cell_allocation->channel_number[ca_index].band = (sys_band_T) rr_get_band_dyn(ORIG_ARFCN);
    ca_index++;
    ...
    for (index = 2; index < 16; index++)
    {
      while (RRFCN_mask != 0 && (ca_index < CA_MAX_LEN))
      {
        if ((IE[index] & RRFCN_mask) != 0)
        {
          ...
          cell_allocation->channel_number[ca_index].band = (sys_band_T) rr_get_band_dyn(
            cell_allocation->channel_number[ca_index].num
          );
          ca_index++;
        }
    ...
    qsort_arfcn(cell_allocation->channel_number, cell_allocation->no_of_entries);
  }
  else
  {
    *decoding_ok = FALSE;
...
)
{
  ...
  *decoding_ok = TRUE;
  ...
  rr_decode_set_band(frequency_list->channel, frequency_list->no_of_items);
}
...
)
{
  ...
  *decoding_ok = FALSE;
  ...
  if ((byte)(IE_scr[1] & 0xC0) == 0)
  {
    ...
    rr_decode_set_band(frequency_list->channel_number, frequency_list->no_of_entries);
  }
  else if (((IE_scr[1] & 0xC0) == 0x80) && ((IE_scr[1] & 0x08) == 0x00) && (support == TRUE))
  {
    ...
    qsort_arfcn(frequency_list->channel_number,size + frequency_list->no_of_entries);
  ...
  else if (((IE_scr[1] & 0xC0) == 0x80) && ((IE_scr[1] & 0x0E) == 0x08) && (support == TRUE))
  {
    ...
    qsort_arfcn(frequency_list->channel_number, size + 1);
  }
  ...
  else if (((IE_scr[1] & 0xC0) == 0x80) && ((IE_scr[1] & 0x0E) == 0x0E) && (support == TRUE))
  {
    ...
    qsort_arfcn(frequency_list->channel_number, frequency_list->no_of_entries);
...
/*MR version 001 and 010 are supported*/
  ...
  rr_log_multirate_parameters(pConfig, num_modes);
#endif
  pConfig->valid = TRUE;
  *decoding_ok = TRUE;
...
static boolean rr_received_all_instances(instances_T *instances, byte array_length, byte COUNT)
{
  ...
  {
    MSG_GERAN_LOW_1("array_length > MAX_INSTANCES, truncating to %d", MAX_INSTANCES);
  ...
  {
    MSG_GERAN_LOW_1("COUNT >= MAX_INSTANCES, truncating to %d", MAX_INSTANCES - 1);
...
static void rr_reset_SI2q_MI_changed(instances_T *instances, byte array_length)
{
  ...
  if (array_length > MAX_INSTANCES)
  {
    MSG_GERAN_LOW_0("array_length > MAX_INSTANCES!!");
    array_length = MAX_INSTANCES;
  }
...
boolean rr_SI2q_MI_changed (instances_T *instances)
{
  ...
  if (instances == NULL)
  {
    MSG_GERAN_LOW_0("NULL ptr passed to rr_SI2q_MI_changed!!");
    return FALSE;
  }
...
)
{
  ...
  if (gs_bit_stream_extract_field(bit_stream_ptr,1))
  {
    ...
    MSG_GERAN_LOW_1("Extracted a BSIC: %d", current_entry->bsic[0]);
    ...
    while (num_remaining_bsic--)
    {
      ...
      if (current_entry != NULL && current_entry->size < MAX_BSIC_PER_FREQUENCY)
      {
        ...
        MSG_GERAN_LOW_1("Extracted a BSIC: %d", current_entry->bsic[current_entry->size]);
        current_entry->size++;
      }
      else
      {
...
boolean rr_received_all_si2ter_instances(si2ter_instances_T *instances, byte array_length, byte COUNT)
{
  ...
  {
    MSG_GERAN_LOW_1("array_length > MAX_SI2TER_INSTANCES, truncating to %d",
  ...
  {
    MSG_GERAN_LOW_1("COUNT >= MAX_SI2TER_INSTANCES, truncating to %d",
...
)
{
  ...
  while (gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    eutran_frequency.earfcn = (uint16) gs_bit_stream_extract_field(bitstream, 16);
    if (gs_bit_stream_extract_field(bitstream, 1))
    {
...
)
{
  ...
  if (gs_bit_stream_extract_field(bitstream, 1))
  {
    MSG_GERAN_LOW_0("E-UTRAN Measurement Parameters Description present");
  ...
  {
    MSG_GERAN_LOW_0("E-UTRAN Measurement Parameters Description not present");
  }

  if (gs_bit_stream_extract_field(bitstream, 1))
  {
    MSG_GERAN_LOW_0("GPRS E-UTRAN Measurement Parameters Description present");
  ...
  {
    MSG_GERAN_LOW_0("GPRS E-UTRAN Measurement Parameters Description not present");
  }
  ...
  while (gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    MSG_GERAN_LOW_0("Repeated E-UTRAN Neighbour Cells present");
  ...
  MSG_GERAN_LOW_0("Repeated E-UTRAN Neighbour Cells not present");
...
)
{
  ...
  if (gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    RR_ASSERT_FATAL(dst->serving_cell_priority_params != NULL);
  ...
  if (gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    RR_ASSERT_FATAL(dst->utran_priority_params != NULL);
...
)
{
  ...
  */
  ...
  MSG_GERAN_HIGH_2("Decoding SI2quater instance %d, max=%d",(int) temp_common_params.INDEX, (int) temp_common_params.COUNT );
       ...
       rr_MP_changed(SI2quater, &temp_common_params) )
  {
    ...
    MSG_GERAN_HIGH_4("3G_BA_IND old: <%d>, new: <%d> and MP_CHANGE_MARK old: <%d>, new: <%d>in SI2quater", \
    ...
    {
      MSG_GERAN_LOW_0("SI2Q MP changed flush SI cache");
#ifdef FEATURE_RR_SI_CACHE
...
/**************************************************************************************
Function
rr_allocate_SI21_instance

Description
Allocate memory for one SI21 instance and initialize to the default values.

Parameters
[IN] *inst_ptr: Pointer to global storage for the instances.
[IN] index    : Index to the instance for which memory is going to be allocated

Returns
None
**********************************************************************************************/
static void rr_allocate_SI21_instance(si21_instances_T *inst_ptr, byte index )
{
  si21_rest_T *instance;

  //RR_NULL_CHECK_FATAL(inst_ptr);
  //we do have sanity check before passing index value to this function.
  //RR_NULL_CHECK_FATAL(index < MAX_SI21_INSTANCES);

  if (inst_ptr->instance[index] == NULL)
  {
    inst_ptr->instance[index] = GPRS_MEM_MALLOC(sizeof(si21_rest_T));
    RR_NULL_CHECK_FATAL(inst_ptr->instance[index]);
    MSG_GERAN_HIGH_1_G("Memory allocated for instance %d of SI21", index);
  }
  else
  {
    MSG_GERAN_ERROR_1_G("Memory was not freed for instance %d of SI21", index);
  }

  instance = inst_ptr->instance[index];
  memset((void *) instance, 0, sizeof(si21_rest_T));
}

/**************************************************************************
Function:
rr_received_all_si21_instances

Description:
Checks if all the instances of SI21 are received.

Parameters:
[IN] *inst_ptr: Pointer to global storage for the instances.
[IN]  COUNT   : Max instance number broadcasted.

Returns:
TRUE if all instances have been received; FALSE otherwise
***************************************************************************/
static boolean rr_received_all_si21_instances(si21_instances_T *inst_ptr, byte COUNT )
{
  int i;

  //RR_NULL_CHECK_FATAL (inst_ptr);
  
  if (inst_ptr->num_instances > MAX_SI21_INSTANCES)
  {
    MSG_GERAN_LOW_2_G("num_instances(%d) > MAX_SI21_INSTANCES, truncating to %d", inst_ptr->num_instances, MAX_SI21_INSTANCES);
    inst_ptr->num_instances = MAX_SI21_INSTANCES;
  }
  
  if (COUNT >= MAX_SI21_INSTANCES)
  {
    MSG_GERAN_LOW_2_G("COUNT(%d) >= MAX_SI21_INSTANCES, truncating to %d", COUNT, MAX_SI21_INSTANCES - 1);
    COUNT = MAX_SI21_INSTANCES - 1;
  }

  if (inst_ptr->num_instances == COUNT+1) /* For one instance SI21 count = 0, hence the +1 */
  {
    for (i = 0; i < inst_ptr->num_instances; i++)
    {
      if (inst_ptr->instance[i] == NULL)
      {
        /* Something is wrong. Some instance is missing. */
        MSG_GERAN_ERROR_1_G("SI21 instance %d is missing", i);
        return FALSE;
      }
    }
    
    MSG_GERAN_HIGH_0_G("SI21 all instance are received");
    return TRUE;
  }

  MSG_GERAN_HIGH_0_G("SI21 all instance are not received");
  return FALSE;
}
...
Description:
Checks if there is any change in CHANGE_MARK.

Parameters:
[IN] *inst_ptr: Pointer to global storage for the instances.

Returns:
TRUE if CHANGE_MARK is changed between instances; FALSE otherwise
***************************************************************************/
static boolean rr_si21_changed(si21_instances_T *inst_ptr, uint8 change_mark)
{
  byte i;
  si21_rest_T *inst;
  ...
  /* need to loop through all the instances since the instances may be received
  out of order and the last received instance is not remembered */
  for (i = 0; (i < inst_ptr->num_instances) && (i < MAX_SI21_INSTANCES); i++)
  {
    inst = inst_ptr->instance[i];

    if (inst && (inst->si21_change_mark != change_mark))
    {
      return TRUE;
    }
  }
...
Description:
Decode the neighbour cell description 21 Rest Octets IE.

Parameters:
[IN] *IE - pointer to the SI 2ter rest octets
[IN] IE_length - (used to initialize the bitstream with gs_bit_stream_initialize)
[IN] *si21_instances - pointer to global database for storing SI21 instances
[OUT] *si21_received - set to TRUE if all instances are received
[OUT] *decoding_ok - indicates whether the decoding process was successful.
[IN] *channel_number - ARFCN on which SI is received
[IN] *sys_info_db_ptr - pointer to system info database of serving/pending cell

Returns:
None
***************************************************************************/
void rr_decode_si21_rest_octets
(
  byte                          *IE,
  byte                           IE_length,
  si21_instances_T              *si21_instances,
  boolean                       *si21_received,
  boolean                       *decoding_ok,
  ARFCN_T                       *channel_number,
  system_information_database_T *sys_info_db_ptr
)
{
  si21_rest_T this_si21_instance;
  ...
  //RR_NULL_CHECK_FATAL (IE);
  //RR_NULL_CHECK_FATAL (si21_instances);
  //RR_NULL_CHECK_FATAL (si21_received);
  //RR_NULL_CHECK_FATAL (decoding_ok);
  //RR_NULL_CHECK_FATAL (channel_number);
  //RR_NULL_CHECK_FATAL (sys_info_db_ptr);

  memset((void *)&this_si21_instance, 0, sizeof(si21_rest_T));
  ...
  *si21_received = FALSE;
  ...
  /* Start Extracting information into local storage                   */
  /*********************************************************************/
  this_si21_instance.si21_change_mark = (byte) gs_bit_stream_extract_field(bit_stream_ptr, 2);
  this_si21_instance.si21_index       = (byte) gs_bit_stream_extract_field(bit_stream_ptr, 3);
  this_si21_instance.si21_count       = (byte) gs_bit_stream_extract_field(bit_stream_ptr, 3);

  /* Check if the instance number is valid */
  if (this_si21_instance.si21_index > this_si21_instance.si21_count)
  {
    MSG_GERAN_HIGH_2_G("SI21_INDEX(%d) > SI21_COUNT(%d)", this_si21_instance.si21_index, this_si21_instance.si21_count);
    *decoding_ok = FALSE;
    return;
  }

  /* if the instance was already received */
  if (si21_instances->instance[this_si21_instance.si21_index])
  {
    /* Check if there is no change in CHANGE_MARK */
    if (rr_si21_changed(si21_instances, this_si21_instance.si21_change_mark) == FALSE)
    {
      MSG_GERAN_LOW_2_G("Received SI21(Index=%d, Count=%d) - Repeat Instance", this_si21_instance.si21_index, this_si21_instance.si21_count);

      /* If all instances are received then the si21_received is set to TRUE.
       * This is done in order to set the SI21 bit in the system information database or pending channel data */
      if (rr_received_all_si21_instances(si21_instances, this_si21_instance.si21_count))
      {
        *si21_received = TRUE;
      }
      return;
  ...
  if (gs_bit_stream_extract_field(bit_stream_ptr,1) == TRUE)
  {
    this_si21_instance.EAB_info_flag = TRUE;
    this_si21_instance.EAB_info.authorization_mask = (word) gs_bit_stream_extract_field(bit_stream_ptr, 10);
    this_si21_instance.EAB_info.subcategory = (byte) gs_bit_stream_extract_field(bit_stream_ptr, 2);
  }
  else
  {
    this_si21_instance.EAB_info_flag = FALSE;
  }

  MSG_GERAN_HIGH_2_G("Received SI21(Index=%d, Count=%d)", this_si21_instance.si21_index, this_si21_instance.si21_count);
  MSG_GERAN_HIGH_3_G("EAB:: EAB_info_flag(%d), Auth_mask(0x%x), Subcat(%d)", 
    this_si21_instance.EAB_info_flag, this_si21_instance.EAB_info.authorization_mask, this_si21_instance.EAB_info.subcategory);
  
  /* Check if there is change in CHANGE_MARK */
  if (rr_si21_changed(si21_instances, this_si21_instance.si21_change_mark))
  {
    MSG_GERAN_HIGH_0_G("SI21 CHANGE_MARK changed");
    
    /* Re-read all instances of SI21 */
    rr_reset_SI21_instances(si21_instances);
  ...
  /* If it is a newly received instance then allocate memory for it  */
  if (si21_instances->instance[this_si21_instance.si21_index] == NULL)
  {
    rr_allocate_SI21_instance(si21_instances, this_si21_instance.si21_index);
    si21_instances->num_instances++;
  }

  /* copy the current instance information into the global instances structure */
  *(si21_instances->instance[this_si21_instance.si21_index]) = this_si21_instance;

  /* If all instances are received then the si21_received is set to TRUE.
   * If current instance has EAB info then set si21_received to TRUE as we are interested only in EAB info.
   * This is done in order to set the SI21 bit in the system information database or pending channel data */
  if (rr_received_all_si21_instances(si21_instances, this_si21_instance.si21_count) ||
      this_si21_instance.EAB_info_flag)
  {
    *si21_received = TRUE;
  }
}
...
)
{
  ...
  while (gs_bit_stream_extract_field(bitstream, 1))
  {
    ...
    for (i = 0; i < earfcn_count && i < MAX_RR_EUTRAN_NEIGHBOUR_LIST_SIZE; ++i)
    {
      ...
      {
        eutran_params.neighbour_cells[eutran_params.neighbour_cells_count].earfcn = earfcns[i];
...
)
{
  ...
  *mi_received = FALSE;
  ...
  if (gs_bit_stream_extract_HL(bit_stream_ptr))
  {
    ...
    if (gs_bit_stream_extract_field(bit_stream_ptr, 1))
    {
      MSG_GERAN_LOW_0("3G Additional Measurement Parameters Description 2 present");
      rr_extract_3g_addl_meas_params_desc_2(bit_stream_ptr, &temp_mi_params);
    }
    else
    {
      MSG_GERAN_LOW_0("3G Additional Measurement Parameters Description 2 not present");
    }
  ...
  */
  if (temp_mi_params.MI_3G_WAIT_PI)
  {
    ...
    {
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
      MSG_GERAN_LOW_2("3G_WAIT(%d)<MI_COUNT(%d)",(int)temp_mi_params.MI_3G_WAIT,(int)temp_common_params.COUNT);
#endif
  ...
  if (rr_received_all_instances(MeasInfo,MeasInfo->num_instances,temp_common_params.COUNT))
  {
    *mi_received = TRUE;
    ...
    if (rr_SI2q_MI_changed(MeasInfo))
    {
      *send_wcdma_neighbor_list_to_L1 = TRUE;
#ifdef FEATURE_NONBLIND_HANDOVER_DEBUG
    MSG_GERAN_LOW_0("Sending WCDMA NCELL List to L1");
#endif
      /* reset changed flags; */
...
)
{
  ...
  boolean         invalid_band = FALSE;

  if ((IE == NULL) || (cell_sel_indicator == NULL))
  {
    MSG_GERAN_LOW_0_G("NULL pointer parameter");
    return FALSE;
  }
  ...
  if (target_type == GSM_TARGET)
  {
            ...
            (num_of_cells < MAX_GSM_CELLS_IN_CELL_SELECTION_INDICATOR) )
    {
      ...
      if (BAND_NONE == gsm_target->arfcn[num_of_cells].band)
      {
        invalid_band = TRUE; 
        MSG_GERAN_HIGH_0_G("Invalid Band detected,reset GSM CSI Info. ");
        break;
      }
    ...
    /* Set Valid parameter to FALSE */
    if (TRUE == invalid_band) 
    {
      gsm_target->num_of_cells = 0;
...
* @return Returns a pointer to the first IE of the specified message
*/
ie_descriptor_T *rr_get_first_ie_of_message(byte msg_type )
{
  return (rr_get_first_ie_and_length(msg_type, NULL));
    ...
    else
    {
      switch (csn_ie_type)
      {
        ...
        *  as the last part MSG which are not variable in length in the msg    */
        ...
        case SI9_REST_OCTETS  : length = 17;
          break;
...
static void reset_ota_message_at_index(byte message_index )
{
...
)
{
  ...
  *message_ok   = TRUE;
  ...
  if (message_index == *max_message_index_ptr)
  {
    ...
    *message_ok = FALSE;
    (void) rr_log_protocol_error(message_type, RR_PROTOCOL_ERROR_UNKNOWN_TYPE);
  }
  else
  {
    ...
    reset_ota_message_at_index(message_index);
      ...
      if (message_length < message_list_ptr[message_index].imperative_length)
      {
        ...
        if (message_list_ptr[message_index].message_type != CHANNEL_RELEASE)
        {
          ...
          *message_ok = FALSE;
          ...
          (void) rr_log_protocol_error(message_type, RR_PROTOCOL_ERROR_MESSAGE_TOO_SHORT);
        }
        else
        {
          /*****************************************/
    ...
    else
    {
      ...
      *message_ok = FALSE;
      (void) rr_log_protocol_error(message_type, RR_PROTOCOL_ERROR_OTHER);
    }
  }
}
