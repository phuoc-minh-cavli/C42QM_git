...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_psho.c#2 $
***
*** DESCRIPTION
***
...
static rr_psho_data_t rr_psho_data;
 ...
 */
static rr_psho_data_t *rr_psho_get_data_ptr(void)
{
  return &rr_psho_data;
}
...
)
{
  ...
  rr_psho_data_t *rr_psho_data_ptr = rr_psho_get_data_ptr();
...
)
{
  ...
  scell_ptr = rr_pcell_becomes_scell();
  ...
  rr_set_current_band((rr_internal_band_T)GET_ARFCN_BAND(scell_ptr->gsm.BCCH_ARFCN));
  ...
  {
    rr_gprs_build_ba_gprs_from_si2(scell_ptr);
    ...
    {
      (void)rr_gprs_send_sysinfo_status(0);
    }
    else
    {
...
{
  gprs_scell_info_t *p_scell = rr_gprs_get_scell_info();
  ...
  rr_gprs_build_ba_gprs_from_si2(p_scell);
...
static void rr_psho_state_transition(
  const psho_control_state_t next_psho_states
...
)
{
  ...
  rr_psho_state_exit(rr_psho_data_ptr->psho_state);
  rr_psho_data_ptr->psho_state = next_psho_state;
  rr_psho_state_entry(rr_psho_data_ptr->psho_state);
}
    ...
    switch (rr_psho_data_ptr->psho_state)
    {
      case RR_PSHO_NULL:
      {
        switch (psho_event)
        {
          case EV_PSHO_STARTED:
            rr_psho_state_transition(RR_PSHO_COLLECT_SYS_INFO);
            break;
      ...
      case RR_PSHO_COLLECT_SYS_INFO:
      {
        ...
        if (psho_event == EV_PSHO_CONTINUE)
        {
          ...
          rr_send_grr_mac_psho_req(PSHO_GTOG);

          rr_psho_state_transition(RR_PSHO_WAIT_FOR_MAC_CNF);
        ...
        else if (psho_event == EV_PSHO_REJECTED)
        {
          ...
          rr_psho_camp_on_old_cell();
          rr_psho_state_transition(RR_PSHO_NULL);
          event_out = EV_PSHO_COMPLETE;
        }
        else
        {
          /* all other messages and events should be forward to NACC procedure */
          new_psho_event = rr_resel_control(psho_event, message_ptr);
        }
      }
      break;
        ...
        if (psho_event == EV_INPUT_MESSAGE && message_ptr != NULL)
        {
          if ((message_set == MS_MAC_RR) && (message_id == (byte)MAC_GRR_PSHO_CNF))
          {
            ...
            if (p_mac_grr_psho_cnf->psho_status == PSHO_SUCCESS)
            {
              ...
              rr_send_ps_handover_ind();
              rr_psho_state_transition(RR_PSHO_WAIT_FOR_NAS_CNF);
            }
      ...
      case RR_PSHO_WAIT_FOR_NAS_CNF:
      {
        if (psho_event == EV_INPUT_MESSAGE && message_ptr != NULL)
        {
          if ((message_set == MS_MM_RR) && (message_id == (byte)RR_PSHO_CNF))
          {
            ...
            rr_send_grr_mac_psho_complete(PSHO_SUCCESS);

            rr_psho_camp_on_new_cell();
            rr_psho_state_transition(RR_PSHO_NULL);
            event_out = EV_PSHO_COMPLETE;
          }
          else
