...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_tds_send.c#3 $
...
)
{
  ...
  rr_debug_log_tdsrrc_rr_omsg(tds_cmd);

#ifdef FEATURE_TDS_SUB_ID_INTERFACE
  tds_cmd->cmd_hdr.as_id = GERAN_ACCESS_STRATUM_ID_1;
...
)
{
  ...
  rr_dedicated_priority_info_get(&tds_cmd->cmd.interrat_reselection_req.dedicated_priority_info);
#endif /*FEATURE_TDSCDMA_DEDICATED_PRI_INFO*/

  rr_send_message_to_tds_rrc(tds_cmd);
}
...
#ifdef FEATURE_TDSCDMA_DEDICATED_PRI_INFO
  rr_dedicated_priority_info_get(&tds_cmd->cmd.interrat_redirect_req.dedicated_priority_info);
#endif /*FEATURE_TDSCDMA_DEDICATED_PRI_INFO*/
...
)
{
  ...
  if (tds_cmd)
  {
    ...
    if (rr_individual_priorities_valid())
    {
      ...
      tds_cmd->cmd.dedicated_priorities_rsp.plmn = rr_dedicated_priority_info_get_plmn();
