...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_mac_send.c#3 $
...
/*==============================================================================
  Variables
==============================================================================*/
static rr_mac_send_data_t rr_mac_send_data;
 ...
 */
static rr_mac_send_data_t *rr_mac_send_get_data_ptr( void )
{
  return &rr_mac_send_data;
}
...
)
{
  ...
  rr_mac_send_data_t *rr_mac_send_data_ptr = rr_mac_send_get_data_ptr();
 ...
 * \param ps_access_status
 */
static rr_mac_ps_access_status_t rr_mac_send_get_ps_access_status( void )
{
  // Obtain a pointer to the module data
  rr_mac_send_data_t *rr_mac_send_data_ptr = rr_mac_send_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_mac_send_data_ptr);
...
)
{
  if (mac_cmd_ptr->task_id == MS_MAC_RR)
  {
    ...
    rr_show_output_message("MAC", NULL, MS_MAC_RR, (int) mac_cmd_ptr->sig_id);
    rr_debug_log_rr_mac_omsg(mac_cmd_ptr->sig_id);

    mac_put_cmd_grr_mac( mac_cmd_ptr);
  }
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_mac_send_data_ptr = rr_mac_send_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_mac_send_data_ptr);
...
=============================================================================*/
void rr_mac_send_reset_last_cell_info( )
{
  // Obtain a pointer to the module data
  rr_mac_send_data_t *rr_mac_send_data_ptr = rr_mac_send_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_mac_send_data_ptr);
...
)
{
  ...
  if (sig != NULL)
  {
    ...
    sig->msg.gm_ps_access_req.action = get_ps_access_req_action(purge_gmm_signalling);

    grr_send_message_to_mac(sig);

    /* Start a 5 second guard timer to make sure that we get a response. */
    grr_start_guard_timer(5000, EV_MAC_PS_ACCESS_GUARD_TIMER_EXPIRED);

    // Update current PS access status
    rr_mac_send_set_ps_access_status(MAC_PS_ACCESS);
  }
...
)
{
  ...
  if (sig_ptr != NULL)
  {
    ...
    grr_send_message_to_mac(sig_ptr);
    ...
    {
      rr_mac_send_set_ps_access_status(MAC_NO_PS_ACCESS_DEL_ALL);
    }
    else
    {
      rr_mac_send_set_ps_access_status(MAC_NO_PS_ACCESS);
    }
  }
...
=============================================================================*/
static void rr_find_best_fit_pmr(packet_measurement_report_t *msg_ptr )
{
  if (msg_ptr != NULL)
  {
...
)
{
  ...
  if ( grr_mac_sig != NULL )
  {
    ...
    grr_send_message_to_mac(grr_mac_sig);
  }
...
)
{
  ...
  if ( grr_mac_sig != NULL )
  {
    ...
    grr_mac_sig->msg.gm_ul_tbf_est.tlli =  rr_gmminfo_get_tlli();
    ...
    /* notify EDTM module */
    rr_gprs_edtm_gmac_uplink_pending(TRUE);
...
===========================================================================*/
boolean rr_send_grr_mac_reset_req( )
{
  grr_mac_sig_t *grr_mac_sig;
...
===========================================================================*/
void rr_send_grr_mac_dtm_ul_dcch_ind( )
{
  grr_mac_sig_t *grr_mac_sig;
...
===========================================================================*/
void rr_send_grr_mac_edtm_channel_release_ind( )
{
  grr_mac_sig_t *grr_mac_sig;
...
)
{
  ...
  psho_cmd = (ps_handover_command_t *)rr_psho_get_details();
...
*/
boolean rr_mac_ps_access_granted( void )
{
  return (rr_mac_send_get_ps_access_status() == MAC_PS_ACCESS);
}
...
*/
boolean rr_mac_ps_access_blocked( void )
{
  return (rr_mac_send_get_ps_access_status() != MAC_PS_ACCESS);
}
...
*/
boolean rr_mac_ps_access_blocked_del_all( void )
{
  return (rr_mac_send_get_ps_access_status() == MAC_NO_PS_ACCESS_DEL_ALL);
}
