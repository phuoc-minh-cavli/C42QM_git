...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_public_store.c#6 $ */
...
#define GRR_LOCK_PUBLIC_STORE() rex_enter_crit_sect(&grr_public_store_crit_sect)
#define GRR_UNLOCK_PUBLIC_STORE() rex_leave_crit_sect(&grr_public_store_crit_sect)
#else
#error code not present
#endif /* DEBUG_NO_RR */
...
rex_crit_sect_type grr_public_store_crit_sect;
#endif /* DEBUG_NO_RR */
 ...
 *
 * Note: This may call itself, so does not utitise a mutex lock (see rr_read_public_store_dyn()).
 *
...
)
{
  ...
  rr_public_store_data_t *rr_public_store_data_ptr = rr_public_store_get_data_ptr();
    ...
    switch ( param_id )
    {
      ...
      */
      case RRPS_CELL_OPTIONS:
      {
        ...
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_T3168, &cell_options->t3168);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_T3192, &cell_options->t3192);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_BS_CV_MAX, &cell_options->bs_cv_max);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_PAN_DEC, &cell_options->pan_dec);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_PAN_INC, &cell_options->pan_inc);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_PAN_MAX, &cell_options->pan_max);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_ACCESS_BURST_TYPE, &cell_options->access_burst_type);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_CONTROL_ACK_TYPE, &cell_options->control_ack_type);

        rr_read_public_store_internal(RRPS_CELL_OPTIONS_EGPRS_CAPABLE_CELL, &cell_options->egprs_capable_cell);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_EGPRS_PKT_CHAN_REQ, &cell_options->egprs_pkt_chan_req);

        rr_read_public_store_internal(RRPS_CELL_OPTIONS_NW_EXT_UL_TBF, &cell_options->nw_ext_ul_tbf);
        rr_read_public_store_internal(RRPS_CELL_OPTIONS_EXT_UTBF_NODATA, &cell_options->ext_utbf_nodata);

        rr_read_public_store_internal(RRPS_CELL_OPTIONS_PFC_FEATURE_MODE, &cell_options->pfc_feature_mode);
        break;
      }
      ...
      case RRPS_POWER_CTRL:
      {
        ...
        rr_read_public_store_internal(RRPS_POWER_CTRL_ALPHA, &pwr_control->alpha);
        rr_read_public_store_internal(RRPS_POWER_CTRL_T_AVG_W, &pwr_control->t_avg_w);
        rr_read_public_store_internal(RRPS_POWER_CTRL_T_AVG_T, &pwr_control->t_avg_t);
        rr_read_public_store_internal(RRPS_POWER_CTRL_PB, &pwr_control->pb);
        rr_read_public_store_internal(RRPS_POWER_CTRL_PC_MEAS_CHAN, &pwr_control->pc_meas_chan);
        rr_read_public_store_internal(RRPS_POWER_CTRL_INT_MEAS_CHAN_LIST_AV, &pwr_control->int_meas_channel_list_avail);
        rr_read_public_store_internal(RRPS_POWER_CTRL_N_AVG_I, &pwr_control->n_avg_i);
      ...
      case RRPS_SIM_DATA:
      {
        ...
        rr_read_public_store_internal(RRPS_SIM_IMSI, &sim_data->imsi[0]);
      ...
      case RRPS_GMM_DATA:
      {
        ...
        rr_read_public_store_internal(RRPS_GMM_STATE, &gmm_data->gmm_state);
        rr_read_public_store_internal(RRPS_READY_TIMER_FORCE_TO_STANDBY, &gmm_data->ready_timer_force_to_standby);
        rr_read_public_store_internal(RRPS_PTMSI, &gmm_data->ptmsi);
        rr_read_public_store_internal(RRPS_TMSI, &gmm_data->tmsi);
        rr_read_public_store_internal(RRPS_TLLI, &gmm_data->tlli);
        rr_read_public_store_internal(RRPS_TLLI_OLD, &gmm_data->tlli_old);
        rr_read_public_store_internal(RRPS_MS_RA_CAP_IE, &gmm_data->ms_ra_cap_ie);
      ...
      case RRPS_MS_DATA:
      {
        ...
        rr_read_public_store_internal(RRPS_MS_NON_DRX_TIMER, &ms_data->non_drx_timer);
        rr_read_public_store_internal(RRPS_MS_SPLIT_PAGE_CYCLE, &ms_data->split_page_cycle);
        rr_read_public_store_internal(RRPS_MS_BCCH_BAND,&ms_data->ms_bcch_band);
        rr_read_public_store_internal(RRPS_MS_EGPRS_MULTISLOT_CLASS,&ms_data->egprs_multislot_class);
        rr_read_public_store_internal(RRPS_MS_EGPRS_8PSK_UL_ENABLED,&ms_data->egprs_8psk_ul_enabled);
        rr_read_public_store_internal(RRPS_MS_EGPRS_8PSK_POWER_CAP,&ms_data->egprs_8psk_power_capability);

#ifdef FEATURE_GSM_DTM
        rr_read_public_store_internal(RRPS_MS_DTM_ENABLED,&ms_data->dtm_enabled);
        rr_read_public_store_internal(RRPS_MS_DTM_EGPRS_MULTISLOT_SUBCLASS,&ms_data->dtm_egprs_multislot_subclass);
#ifdef FEATURE_GSM_EDTM
        rr_read_public_store_internal(RRPS_MS_EDTM_ENABLED,&ms_data->edtm_enabled);
#endif /* FEATURE_GSM_EDTM */
#endif /* FEATURE_GSM_DTM */

        rr_read_public_store_internal(RRPS_MS_GPRS_GEA_SUPPORT,&ms_data->gea_supported);
#ifdef FEATURE_GSM_GPRS_MSC33
        rr_read_public_store_internal(RRPS_MS_HMSC_GPRS_CODED_MS_CLASS,&ms_data->hmsc_gprs_coded_ms_class);
        rr_read_public_store_internal(RRPS_MS_HMSC_EGPRS_CODED_MS_CLASS,&ms_data->hmsc_egprs_coded_ms_class);
      ...
      case RRPS_MS_LOW_PRIO:
      {
        *((boolean *) param_copy) = rr_public_store_data_ptr->rrps_store_data.ms_data.low_priority;
  ...
  {
    MSG_GERAN_LOW_1_G("Public Store id out of range (id=%d)",param_id);
    *((uint8 *)param_copy) = 0; /* To prevent KW error */
  }
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_public_store_data_t *data_ptr = rr_public_store_get_data_ptr();
  //RR_NULL_CHECK_FATAL(data_ptr);

  MSG_GERAN_LOW_0_G("GERAN public store data:");

  ms_data_ptr = &data_ptr->rrps_store_data.ms_data;

  MSG_GERAN_HIGH_7_G("split_page_cycle=%d,anite_gcf_flag=%d,multislot_class=%d,gea_supported=0x0%x, egprs_multislot_class=%d egprs_8psk_ul_enabled=%d egprs_8psk_power_capability=%d",
    (int)ms_data_ptr->split_page_cycle,
    (int)ms_data_ptr->anite_gcf_flag,
    (int)ms_data_ptr->multislot_class,
    (int)ms_data_ptr->gea_supported,
...
#ifdef FEATURE_GSM_GPRS_MSC33
  MSG_GERAN_HIGH_2_G("hmsc_gprs_coded_ms_class=%d, hmsc_egprs_coded_ms_class=%d",
  (int)ms_data_ptr->hmsc_gprs_coded_ms_class,
  (int)ms_data_ptr->hmsc_egprs_coded_ms_class);
#endif /* FEATURE_GSM_GPRS_MSC33 */
...
{
#ifndef DEBUG_NO_RR
  rex_init_crit_sect(&grr_public_store_crit_sect);
#endif /* DEBUG_NO_RR */
...
)
{
  ...
  if (rex_is_in_irq_mode())
  {
    ...
    rr_read_public_store_internal(param_id, param_copy);
  }
  else
#endif /* !DEBUG_NO_RR */
  {
 ...
 */
rrps_frequency_list_t *rr_read_public_store_freq_list_dyn(
  uint8 ma_number
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_public_store_data_ptr = rr_public_store_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_public_store_data_ptr);
  ...
  return(ret_ptr);
} /* end rr_read_public_store_freq_list_dyn */
...
#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
  if ( update_bitmap & RRPS_UPD_MS_LOW_PRIO )
  {
    rr_public_store_data_ptr->rrps_store_data.ms_data.low_priority = data->ms_low_priority;
  }
#endif /* FEATURE_GERAN_IMPLICIT_PS_REJECT */
 ...
 */
rrps_frequency_list_t *rr_packet_assignment_mobile_alloc_dyn(
...
)
{
  ...
  if ( decode_ok )
  {    
    uint8 no_of_entries = decoded_mobile_alloc_ptr->num_of_arfcns;
    /***************************************************************************/
    ...
    /****************************************************************************/
    if ((no_of_entries > 1) && (decoded_mobile_alloc_ptr->arfcns[no_of_entries - 1].num == 0))
    {
       uint32 ch_index;
       ARFCN_T arfcn0 = decoded_mobile_alloc_ptr->arfcns[no_of_entries - 1];
       for (ch_index = no_of_entries - 1; ch_index > 0; --ch_index)
       {
         decoded_mobile_alloc_ptr->arfcns[ch_index] = decoded_mobile_alloc_ptr->arfcns[ch_index-1];
       }
       decoded_mobile_alloc_ptr->arfcns[0] = arfcn0;      
    }
    ...
    freq_list_p = rr_read_public_store_freq_list_dyn(
      MA_NUMBER_15               // ma_number
    );
  }
...
} /* end rr_packet_assignment_mobile_alloc_dyn */
...
)
{
  ...
  ms_data_ptr->split_page_cycle = rr_get_nv_split_page_cycle();
  ms_data_ptr->multislot_class = rr_get_nv_multislot_class();
  ms_data_ptr->anite_gcf_flag = rr_get_nv_anite_gcf_flag();
  ms_data_ptr->egprs_multislot_class = rr_get_nv_egprs_multislot_class();
  ms_data_ptr->egprs_8psk_ul_enabled = rr_get_nv_egprs_8psk_ul_enabled();
  ms_data_ptr->egprs_8psk_power_capability = rr_get_nv_egprs_8psk_power_capability();
#ifdef FEATURE_GSM_DTM
  ms_data_ptr->dtm_enabled = rr_get_nv_dtm_enabled();
  ms_data_ptr->dtm_egprs_multislot_subclass = rr_get_nv_dtm_egprs_multislot_subclass();
#ifdef FEATURE_GSM_EDTM
  ms_data_ptr->edtm_enabled = rr_get_nv_edtm_enabled();
#endif /* FEATURE_GSM_EDTM */
#endif /* FEATURE_GSM_DTM */
  ms_data_ptr->gea_supported = rr_get_nv_gea_supported();
#ifdef FEATURE_GSM_GPRS_MSC33
  ms_data_ptr->hmsc_gprs_coded_ms_class = rr_get_nv_hmsc_gprs_coded_ms_class();
  ms_data_ptr->hmsc_egprs_coded_ms_class = rr_get_nv_hmsc_egprs_coded_ms_class();
#endif /* FEATURE_GSM_GPRS_MSC33 */

  GRR_UNLOCK_PUBLIC_STORE();

  rr_public_store_dump_to_diag();
