...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_general.c#17 $ $DateTime: 2021/03/10 11:45:48 $ $Author: pwbldsvc $
...
Mar/05/04    VT     In rr_get_band_dyn(): Initializing band to BAND_NONE instead of 0.
...
Jan/09/04    VT     QuadBand: To get the correct Band in the PCS1900/DCS1800 overlap range,
                      modified rr_get_band_dyn() and rr_is_allowed_band().
...
extern sd_ss_band_pref_e_type sd_misc_get_subs_supp_band_pref(void);
...
typedef struct
{
  ...
  uint8   num_ps_supported_cells_in_pscan;
  uint8   num_ps_cells_si_received;
  uint16  num_frequencies_in_pscan;
#endif /* FEATURE_PS_PREFERRED_CELL */
} rr_general_data_t;

static rr_general_data_t  rr_general_data;

static usat_system_information_database_T  usat_system_information_database;
 ...
 */
static rr_general_data_t *rr_general_get_data_ptr(void)
{
  return &rr_general_data;
}
...
static boolean rr_camped_in_forbidden_LAI( void )
{
...
void rr_set_rr_using_pseudo_si2q_data( boolean rr_using_pseudo_si2q_data  )
{
    // Obtain a pointer to the module data
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
...
boolean rr_is_pseudo_si2q_data_used( void )
{
    // Obtain a pointer to the module data
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_general_data_ptr);

  MSG_GERAN_HIGH_1_G("rr_is_pseudo_si2q_data_valid,= %d", rr_general_data_ptr->rr_using_pseudo_si2q_data );
...
**/
void geran_grr_dtx_override_on_per_subs_dyn(void)
{
#ifdef FEATURE_GERAN_CS_SUPPORT
  ...
  {
    rr_l1_update_dedicated_mode(&scell_info_ptr->gsm);
  }
#else /* FEATURE_GERAN_CS_SUPPORT */
rr_callback.c  NOTUSED();
#endif /* !FEATURE_GERAN_CS_SUPPORT */
}

// N.B. Public API (Called from NAS)
void geran_grr_dtx_override_on_dyn(void)
{
  // Assume SingleSIM API is for Sub1
  geran_grr_dtx_override_on_per_subs_dyn();
}

void rr_dtx_override_on_dyn( void )
{
  // Assume legacy API is for Sub1
  geran_grr_dtx_override_on_per_subs_dyn();
}

void geran_grr_dtx_override_off_per_subs_dyn( void )
{
#ifdef FEATURE_GERAN_CS_SUPPORT
...
void geran_grr_dtx_override_off_dyn(
  void
)
{
  // Assume SingleSIM API is for Sub1
...
void rr_dtx_override_off_dyn(void)
{
  // Assume legacy API is for Sub1
  geran_grr_dtx_override_off_per_subs_dyn();
}
...
{
  multiband_info_T * multiband_info = rr_get_multiband_info();
 ...
 * @return Max output power in dBm
 */
byte rr_get_maximum_power_output_for_frequency_dyn(
  ARFCN_T frequency
)
{
  return
    rr_get_maximum_power_output(
       rr_get_band_rf_power_class_dyn(GET_ARFCN_BAND(frequency)),
...
)
{
      ...
      (radio_link_timeout > RR_MAX_RLT_COUNTER))
  {
    ...
    MSG_GERAN_LOW_1_G("RLT counter out of range, using %d", (int)radio_link_timeout);
#ifndef FEATURE_GERAN_REDUCED_DEBUG
    (void)rr_log_cell_options(&(scell_info_ptr->gsm.cell_options));
#endif
  }
...
)
{
  ...
  rr_general_data_ptr->S_counter = rr_get_radio_link_timeout();
...
{
  uint8 max_rlt_counter = rr_get_radio_link_timeout();
...
)
{
  ...
  {
    channel_spec_T *chan_spec_ptr = rr_get_channel_spec(NULL);
...
)
{
  ...
  ms_operation_mode_T ms_operation_mode = rr_siminfo_get_ms_opmode();
...
)
{
     ...
     )
  {
    ...
    */
    if (rr_plc_l2g_search_in_progress())
    {
      bands_allowed |= SYS_BAND_MASK_GSM_EGSM_900 |
...
void mb_initialise_dyn(
  void
)
{
  ;
} /* end mb_initialise_dyn */
...
)
{
  ...
  {
    if (rr_gapi_is_supported_band_dyn(SYS_BAND_EGSM_900))
    {
      return(BAND_EGSM);
    }
  }
   ...
   ****************************************************************/
  if (rr_gapi_is_supported_band_dyn((sys_band_T)actual_band) == FALSE)
  {
...
)
{
...
#ifdef FEATURE_GERAN_CS_SUPPORT
  if (rr_is_control_state(RR_W2G_INTERRAT_HANDOVER_PROGRESS))
  {
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
  {
	...
	if (grr_get_l1_in_test_mode())
    {
      ...
      curr_band = grr_get_test_mode_band();
    }
    else
    {
      curr_band = rr_gapi_get_current_band();
 ...
 *         BAND_NONE if the ARFCN is unknown or not supported
 */
rr_internal_band_T rr_get_band_dyn(
  word local_arfcn
...
)
{
  ...
  result = rr_get_band_using_band_indicator(local_arfcn, band_ind);
  ...
  if ((result < MAXNO_BANDS) && grr_get_l1_in_test_mode())
  {
...
)
{
  ...
  if (rr_gapi_is_supported_band_dyn((sys_band_T)band) && (band < MAXNO_BANDS))
  {
    /*
     ...
     */
    if ((rr_get_band_dyn(local_arfcn) == BAND_EGSM) && (band == BAND_PGSM))
    {
      if (rr_gapi_is_supported_band_dyn(SYS_BAND_EGSM_900))
      {
        in_band = TRUE;
...
)
{
  ...
  for (i = 0; i < frequency_list->no_of_items; i++)
  {
    ...
    else
    {
      ...
      if (!rr_check_in_band(band,frequency_list->channel[i].num))
      {
        mixed_or_unsupported_bands = TRUE;
        break;
      }
...
)
{
  ...
  if (band >= MAXNO_BANDS)
  {
    MSG_GERAN_LOW_1_G("Invalid band (%d)",(int)band);
    return FALSE;
  }
 ...
 */
boolean rr_gapi_is_supported_band_dyn(sys_band_T band)
{
  multiband_info_T * multiband_info = rr_get_multiband_info();
  ...
  {
    MSG_GERAN_LOW_1_G("Invalid band (%d)", (int)rr_band);
    return FALSE;
  }
...
boolean rr_is_supported_band_dyn(
  rr_internal_band_T band
)
{
  return rr_gapi_is_supported_band_dyn((sys_band_T)band);
}
 ...
 */
rr_internal_band_T rr_gapi_get_current_band(void)
{
  multiband_info_T * multiband_info = rr_get_multiband_info();
...
rr_internal_band_T rr_get_current_band_dyn(void)
{
  return rr_gapi_get_current_band();
}
...
{
  if (rr_gapi_is_supported_band_dyn(SYS_BAND_PGSM_900))
  {
    rr_set_current_band(BAND_PGSM);
  }
  else if (rr_gapi_is_supported_band_dyn(SYS_BAND_EGSM_900))
  {
    rr_set_current_band(BAND_EGSM);
  }
  else if (rr_gapi_is_supported_band_dyn(SYS_BAND_DCS_1800))
  {
    rr_set_current_band(BAND_DCS);
  }
  else if (rr_gapi_is_supported_band_dyn(SYS_BAND_PCS_1900))
  {
    rr_set_current_band(BAND_PCS);
  }
  else
  {
    MSG_GERAN_ERROR_0_G("No GSM bands are supported");
    rr_set_current_band(BAND_NONE);
  }
...
{
  byte power_class;
  rr_internal_band_T current_band = rr_gapi_get_current_band();
  multiband_info_T * multiband_info = rr_get_multiband_info();
...
byte rr_get_current_rf_power_class_dyn(
  void
)
{

  return rr_gapi_get_current_rf_power_class();
}

byte rr_gapi_get_band_rf_power_class_dyn(
  rr_internal_band_T band
 ...
 */
byte rr_get_band_rf_power_class_dyn(
  rr_internal_band_T band
)
{

  return rr_gapi_get_band_rf_power_class_dyn(band);
}

byte rr_gapi_get_supported_band_count(void)
{
  uint8 count = 0;
  multiband_info_T *multiband_info_ptr = rr_get_multiband_info();
 ...
 */
byte rr_get_supported_band_count_dyn(void)
{
  return rr_gapi_get_supported_band_count();
 ...
 * @return number of bands currently supported
 */
byte rr_ds_get_supported_band_count_dyn( void )
{
  return rr_gapi_get_supported_band_count();
...
)
{
  ...
  switch(band)
  {
    ...
    default:
      MSG_GERAN_LOW_1("RR does not support band %d",band);
...
)
{
  ...
  {
    rr_set_preferred_band(BAND_PCS, TRUE);
    MSG_GERAN_HIGH_0_G("band_pref bit-mask for PCS1900 is allowed");
  }
  else
  {
    rr_set_preferred_band(BAND_PCS, FALSE);
  ...
  {
    rr_set_preferred_band(BAND_EGSM, TRUE);
    rr_set_preferred_band(BAND_PGSM, TRUE);
    MSG_GERAN_HIGH_0_G("band_pref bit-masks for EGSM and PGSM900 are allowed");
  }
  else
  {
    rr_set_preferred_band(BAND_EGSM, FALSE);
    ...
    {
      rr_set_preferred_band(BAND_PGSM, FALSE);
...
)
{
  ...
  for (cell_index_1 = 0; cell_index_1 < no_of_entries; cell_index_1++)
  {
    ...
    if (!present_flag)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("Too many neighbour cell entries, rest ignored");
        break;
      }
    }
  ...
  qsort_arfcn(target_ba_list_ptr->BCCH_ARFCN, target_ba_list_ptr->no_of_entries);
} /* rr_add_ba_list */
...
)
{
  ...
  rr_reset_dedicated_BA_lists(&scell_info_ptr->gsm);
...
)
{
  ...
  if (band < MAXNO_BANDS)
  {
    ...
    rf_power_capability = rr_gapi_get_band_rf_power_class_dyn(band);
    ...
    maximum_rf_power_output = rr_get_maximum_power_output(rf_power_capability, band);
  }
  else
  {
    MSG_GERAN_LOW_1_G("Invalid band: %d", (int)band);
...
)
{
  ...
  if (!rr_cell_forbidden(loc_arfcn))
  {
 ...
 */
void rr_initialise_forbidden_cell_list( )
{
...
)
{
     ...
     )
  {
    ...
    for (index = 0; index < RR_MAX_NUM_OF_BARRED_CELLS; index++)
    {
      ...
      {
        rr_static_general_data_ptr->rr_barred_cell_list[index].arfcn = barred_cell_ptr->arfcn;
        rr_static_general_data_ptr->rr_barred_cell_list[index].bsic = barred_cell_ptr->bsic;
        rr_static_general_data_ptr->rr_barred_cell_list[index].remaining_time =
    ...
    if (cell_added == FALSE)
    {
      index = rr_find_oldest_barred_cell();
      if (index < RR_MAX_NUM_OF_BARRED_CELLS)
      {
        rr_static_general_data_ptr->rr_barred_cell_list[index].arfcn = barred_cell_ptr->arfcn;
        rr_static_general_data_ptr->rr_barred_cell_list[index].bsic = barred_cell_ptr->bsic;
        rr_static_general_data_ptr->rr_barred_cell_list[index].remaining_time = barred_cell_ptr->remaining_time;
        cell_added = TRUE;
      }
    }
...
)
{
  ...
  for(index =0; index < RR_MAX_NUM_OF_BARRED_CELLS; index++)
  {
    if(rr_static_general_data_ptr->rr_barred_cell_list[index].remaining_time <= shortest_remaining_time)
    {
      shortest_remaining_time = rr_static_general_data_ptr->rr_barred_cell_list[index].remaining_time;
      index_of_shortest_time = index;
    }
  }

  MSG_GERAN_HIGH_3_G("Oldest barred cell,arfcn: %d, ncc: %d, bcc: %d",
              rr_static_general_data_ptr->rr_barred_cell_list[index_of_shortest_time].arfcn.num,
              rr_static_general_data_ptr->rr_barred_cell_list[index_of_shortest_time].bsic.PLMN_colour_code,
              rr_static_general_data_ptr->rr_barred_cell_list[index_of_shortest_time].bsic.BS_colour_code);
...
)
{
  ...
  for(index =0; index < RR_MAX_NUM_OF_BARRED_CELLS; index++)
  {
    if(ARFCNS_EQUAL(rr_static_general_data_ptr->rr_barred_cell_list[index].arfcn, arfcn) &&
       rr_same_BSIC(&rr_static_general_data_ptr->rr_barred_cell_list[index].bsic, &bsic) &&
       (rr_static_general_data_ptr->rr_barred_cell_list[index].remaining_time != RR_INVALID_REMAINING_TIME))
    {
      MSG_GERAN_HIGH_3_G("Cell in barred list, arfcn: %d, ncc: %d, bcc: %d",
                               rr_static_general_data_ptr->rr_barred_cell_list[index].arfcn.num,
                               rr_static_general_data_ptr->rr_barred_cell_list[index].bsic.PLMN_colour_code,
                               rr_static_general_data_ptr->rr_barred_cell_list[index].bsic.BS_colour_code);
      return_value = TRUE;
      break;
    }
  }
...
)
{
      ...
      (cell_bar_timer_value <= CONVERT_HOURS_TO_MILLISECONDS(12)))
  {
    ...
    rr_add_to_barred_cell_list(&barred_cell);
  }
  else
  {
...
void rr_authn_fail_bar_cell_basedon_ARFCN_BSIC(uint32 cell_bar_timer_value, ARFCN_T arfcn, uint8 bsic  )
{
  rr_barred_cell_info_T barred_cell;
  BSIC_T                local_bsic;
 ...
 */
void rr_compute_BSIC_from_SCH_block_dyn(
...
)
{
  ...
  for (cell_index = 0; (cell_index < no_of_entries) && (cell_index < BA_MAX_LEN); ++cell_index)
  {
    ...
    if (!remove_serving_cell || (serving_cell_arfcn.num != BA_ARFCN.num))
    {
       ...
       */
      rr_internal_band_T band = rr_get_band_dyn(BA_ARFCN.num);

      if (!rr_gapi_is_supported_band_dyn((sys_band_T)band))
      {
 ...
 *
 */
void rr_notify_deep_sleep_to_other_rats(boolean in_deep_sleep  )
{
  ...
  if (geran_is_multi_mode_gas_id())
  {
    ...
    if (rr_is_rat_segment_loaded(RR_SEG_LOAD_TDSCDMA))
    {
      ...
      tdsrrc_set_ue_in_deep_sleep(in_deep_sleep, SYS_MODEM_AS_ID_1);
#else
...
)
{
  ...
  /******************************/
  imsi_valid = rr_get_imsi_mod_1000(&imsi_mod_1000);
  ...
  if (early_camping_performed)
  {
    ...
    rr_send_mph_read_sys_info_req(RR_L1_SI_TYPE_NONE, cell_info->gsm.si_sched_info);
  }
  else
  {
  ...
  (void) rr_log_cell_info(sys_info_source);
  (void) rr_log_cell_options(&(sys_info_source->cell_options));
  (void) rr_log_cell_selection_parameters(&(sys_info_source->cell_selection_parameters));
  (void) rr_log_control_channel_description(&(sys_info_source->control_channel_description));
  (void) rr_log_rach_control_parameters(&(sys_info_source->RACH_control_parameters));
#endif
#ifdef FEATURE_GSM_RR_CELL_CALLBACK
  {
    best_six_ncells_t *best_six_ncells_ptr = rr_get_best_six_ncells_ptr();
  ...
  rr_notify_deep_sleep_to_other_rats(FALSE);
...
)
{
  ...
  else
  {
    ...
    if (rr_gsm_only())
    {
      ...
      rr_send_mph_wcdma_cell_update_list_req(NULL);
    }
    else
    {
         ...
         )
      {
        ...
        rr_gprs_send_mph_wcdma_cell_update_list_req();
      }
      else
      {
            ...
            scell_info_ptr->gsm.wcdma_neighbor_list_si2ter.valid_data)
        {
          ...
          rr_send_mph_wcdma_cell_update_list_req(scell_info_ptr);
        }
#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
        else if (rr_is_pseudo_si2q_data_used() == TRUE )
        {
...
)
{
  ...
  if (wcdma_ded_neighbor_list_ptr->mi_params.MI_3G_MP_Desc_PI == FALSE)
  {
    ...
    {
      /*+452757- ASR */
      if (rr_si2quater_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE)))//All SI2qs  received
      {
      /*-452757- ASR */
        wcdma_ded_neighbor_list_ptr->mi_params.QSearch_C = QSEARCH_I_ALWAYS;
...
#ifdef FEATURE_SGLTE
#ifdef FEATURE_GSM_TDS
boolean rr_sglte_is_l1_tds_measurements_configured( )
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_general_data_ptr);
...
void rr_sglte_set_l1_tds_measurements_configured(boolean is_configured  )
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_general_data_ptr);
...
#ifdef FEATURE_LTE
boolean rr_is_lte_l1_measurements_configured( )
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_general_data_ptr);
...
void rr_set_lte_l1_measurements_configured(boolean is_configured  )
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_general_data_ptr);
...
)
{
  ...
  rr_generate_complete_dedicated_BA_list(si_db_ptr);
  ...
  DTX_indicator = rr_get_DTX_indicator();
 ...
 */
void rr_reset_forbidden_lai_list( )
{
  int cell_index;

  // Obtain a pointer to the forbidden LAI list (stored by ASID)
  forbidden_lai_list_T *lai_list_ptr = rr_get_forbidden_lai_list_ptr();
  //RR_NULL_CHECK_FATAL(lai_list_ptr);
...
)
{
  ...
  ignored_timers[0] = RR_MM_ACTIVATION_RSP_TIMER;
  num_entries       = 1;
  ...
  ignored_timers[num_entries] = RR_DMM_RESUMPTION_TIMER;
  num_entries++;
#endif /* FEATURE_DYNAMIC_LOADING_GERAN || FEATURE_EXTERNAL_PAGING_GERAN */

  ignored_timers[num_entries] = RR_CELL_BARRED_TIMER;
  num_entries++;

  /* pass the array to a function that will reset all timers except these */
  rr_timer_reset_all(ignored_timers, num_entries);
...
)
{
  ...
  if (rr_l1_gsm_deact_reason_get() != RR_L1_MODE_CHANGE)
  {
  ...
  {
    rr_log_reselect_failure(RR_LOG_RES_FAILURE_ABORT_DUE_TO_MM_REQ);
  }
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
  (void) rr_mode_control(EV_RESET_SOFTWARE,NULL);
#ifdef FEATURE_GERAN_CS_SUPPORT
...
#ifdef FEATURE_GERAN_CS_SUPPORT
#ifdef FEATURE_WCDMA
  (void) rr_g2w_handover_control(EV_RESET_SOFTWARE, NULL);
#endif /* FEATURE_WCDMA */
#endif /* FEATURE_GERAN_CS_SUPPORT */
...
)
{
  ...
  */
  ...
  if (rr_siminfo_get_access_class(access_class_membership, sizeof(access_class_membership)))
  {
    boolean is_hplmn;
...
access_result_T rr_check_access_class_for_raching_for_tbf( )
{
  return rr_check_access_class_for_raching(MM_RR_MO_FULL_SPEECH);

} /* end rr_check_access_class_for_raching_for_tbf() */

#ifdef FEATURE_GERAN_EAB
boolean rr_is_extended_access_barred(rr_pkt_est_cause_t pkt_est_cause )
{
  boolean access_barred = FALSE;
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
  byte access_class[2];
  uint16 access_class_15_0;

  /* Get the access class membership from the SIM. Access control bit-mapping:
   * octet 0         b8 b7 b6 b5 b4 b3 b2 b1
   * access class =  15 14 13 12 11 10  9  8

   * octet 1         b8 b7 b6 b5 b4 b3 b2 b1
   * access class =   7  6  5  4  3  2  1  0 */
  (void) rr_siminfo_get_access_class(access_class, sizeof(access_class));

  /* Access class in 15 - 0 format. i.e. MSB represents Access Class 15 and LSB represents Access Class 0. */
  access_class_15_0 = (access_class[0] < 8) | access_class[1];
  
  MSG_GERAN_HIGH_1_G("UE configured for EAB (%d)", rr_ps_is_eab_enabled());
  if (rr_ps_is_eab_enabled())
  {
    /* Access is allowed if it is for PAGE RESPONSE */
    if (pkt_est_cause == RR_PKT_EST_CAUSE_PAGE_RESP)
    {
      MSG_GERAN_HIGH_0_G("EAB check not applicable for PAGE RESPONSE");
    }
    /* Access is allowed if UE is a member of special access class (Access Class: 11 - 15) */
    else if((access_class_15_0 & 0xF800) != 0x00)
    {
      MSG_GERAN_HIGH_0_G("EAB check not applicable for Special Access Class");
    }
    /* Access is allowed if SI21 is not broadcast */
    else if((scell->bcch_sys_info.si_messages_not_broadcast & MASK_SI_21) != 0x00)
    {
      MSG_GERAN_HIGH_0_G("EAB check not applicable when SI21 is not broadcast");
    }
    /* Access is barred if SI21 is broadcast but not received. wait untill SI21 is received. */
    else if((scell->gsm.update_message_received & MASK_SI_21) == 0x00)
    {
      MSG_GERAN_HIGH_0_G("Extended Access Barred - SI21 not yet received");
      access_barred = TRUE;
    }
    /* Access is allowed if SI21 is received but does not include EAB info */
    else if(scell->gsm.EAB_info_valid == FALSE)
    {
      MSG_GERAN_HIGH_0_G("EAB check not applicable when SI21 does not include EAB info");
    }
    /* Chcek if UE's access class is barred. authorization_mask format:
     *                 b10 b9 b8 b7 b6 b5 b4 b3 b2 b1
     * access class =    9  8  7  6  5  4  3  2  1  0 */
    else if((scell->gsm.EAB_info.authorization_mask & access_class_15_0 & 0x3FF) != 0)
    {
	  MSG_GERAN_HIGH_1_G("Extended Access Barred - Subcategory %d", scell->gsm.EAB_info.subcategory);
      /* Bar access based on subcategory */
      switch (scell->gsm.EAB_info.subcategory)
      {
        /* The EAB Authorization mask is applicable to all mobile stations configured for EAB. */
        case 0:
        {
          MSG_GERAN_LOW_0_G("Extended Access Barred - Subcategory 0");
          access_barred = TRUE;
          break;
        }
        /* The EAB Authorization mask is only applicable to mobile stations configured for EAB 
         * and neither in their HPLMN nor in a PLMN that is equivalent to it. */
        case 1:
        {
          if(rr_ps_is_eab_cat_b_barred())
          {
            MSG_GERAN_LOW_0_G("Extended Access Barred - Subcategory 1");
            access_barred = TRUE;
          }
          break;
        }
        /* The EAB Authorization mask is only applicable to mobile stations configured for EAB 
         * and neither in the PLMN listed as most preferred PLMN of the country where the UE is 
         * roaming in the operator-defined PLMN selector list on the SIM/USIM, nor in their HPLMN 
         * nor in an PLMN that is equivalent to it */
        case 2:
        {
          if(rr_ps_is_eab_cat_c_barred())
          {
            MSG_GERAN_LOW_0_G("Extended Access Barred - Subcategory 2");
            access_barred = TRUE;
          }
          break;
        }
        default:
          MSG_GERAN_LOW_1_G("Invalid EAB Subcategory (%d)", scell->gsm.EAB_info.subcategory);
      }
    }
  }

  MSG_GERAN_HIGH_1_G("EAB check result = %d", access_barred);
...
)
{
  ...
  access_result = rr_check_access_class_for_camping(si_db_ptr);
  ...
  switch (access_result)
  {
    ...
    default:
      MSG_GERAN_LOW_1_G("Unexpected value=%d for access result",(int) access_result);
      mm_as_access = MM_AS_ACCESS_ALL_CALLS;
      break;
  }
...
boolean rr_is_forbidden_LAI(LAI_T *lai_ptr  )
{
  int index;
  int no_of_entries;
  forbidden_lai_list_T *lai_list_ptr;
...
byte rr_convert_rxlex_to_absolute_dBm(byte rxlev)
{
  ...
  if (rxlev > 63)
  {
    MSG_GERAN_LOW_1("Invalid signal strength=%d",(int) rxlev);
    rxlev = 63;
  }
 ...
 */
void rr_process_rr_data_req(rr_data_req_T *rr_data_req )
{
  ...
  if (protocol_discriminator == SMS_MESSAGES)
  {
    ...
    /********************************************/
    (void) rr_control_SAPI3_link(rr_data_req);
  }
  else
  {
...
)
{
  ...
  rr_get_ms_capabilities(&rr_ms_capability, NULL);
  ...
  if (channel_info_ptr->hopping_flag == TRUE)
  {
    ...
    for (index = 0; index < no_of_entries; index++)
    {
      ...
      else /* not first time through loop */
      {
        ...
        /*****************************************************/
        if (!rr_check_in_band(band,channel_number.num))
        {
          /*****************************************************/
    ...
    if ((result == TRUE) && (no_of_entries > 1))
    {
      ...
      {
         uint32 ch_index;
         ARFCN_T arfcn0 = channel_info_ptr->frequency_list.channel[no_of_entries - 1];
         ...
         channel_info_ptr->frequency_list.channel[0] = arfcn0;
      }
    }
...
)
{
  ...
  for (index=0; index < no_of_items;++index)
  {
    ...
    {
      if (rr_check_in_band(band,channel_number.num) == FALSE)
      {
...
)
{
  if (si_db_ptr == NULL)
  {
    MSG_GERAN_LOW_0("si_db_ptr = NULL");
    return;
  }
...
)
{
  ...
  rr_add_ba_list(&si_db_ptr->complete_BA_list,&si_db_ptr->SI2_BA_list);
     ...
     )
  {
    ...
    rr_add_ba_list(&si_db_ptr->complete_BA_list,&si_db_ptr->SI2bis_BA_list);
  }
  ...
  rr_gprs_reassign_band(IDLE_BA_FREQ_LIST, camped_on_flag);
...
{
  boolean        old_si5_ba_lists_valid = rr_get_status_of_old_cell_si5_ba_lists();
  rr_ba_lists_t *rr_ba_lists_ptr = rr_get_ba_lists_ptr();
  ...
  if (ARE_SET(MASK_SI_5, si_db_ptr->update_message_received))
  {
    ...
    rr_add_ba_list(&rr_ba_lists_ptr->complete_BA_sacch, &rr_ba_lists_ptr->SI5_BA_list);
  }
  else if (old_si5_ba_lists_valid)
  {
    BCCH_frequency_list_T old_list = rr_get_old_si5_ba_list(MASK_SI_5);
    ...
    rr_add_ba_list(&rr_ba_lists_ptr->complete_BA_sacch, &old_list);
  }
      ...
      si_db_ptr->decode_SI5bis == RR_SI_REQUIRED)
  {
    ...
    rr_add_ba_list(&rr_ba_lists_ptr->complete_BA_sacch, &rr_ba_lists_ptr->SI5bis_BA_list);
  }
  else if (old_si5_ba_lists_valid)
  {
    BCCH_frequency_list_T old_list = rr_get_old_si5_ba_list(MASK_SI_5_BIS);
  ...
  else
  {
    ...
    {
      BCCH_frequency_list_T old_list = rr_get_old_si5_ba_list(MASK_SI_5_TER);
...
)
{
  ...
  usat_system_info_database_ptr = rr_get_usat_system_information_database_ptr();
 ...
 */
boolean rr_ds_get_nmr_bcch_dyn(uint8 *nmr,
                           rr_bcch_list_type *bcch_list)
{
 MSG_GERAN_LOW_0_G("Depreciated API will be removed");
 return FALSE;
}

boolean rr_ds_api_get_nmr_bcch_dyn(uint8 *nmr,
                               uint8 len_of_nmr,
                               rr_bcch_list_type *bcch_list)
{
  usat_system_information_database_T *usat_system_info_database_ptr;
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
  {
    ...
    usat_system_info_database_ptr->cell_options     = scell_info_ptr->gsm.cell_options;
    usat_system_info_database_ptr->reselection_info = scell_info_ptr->gsm.reselection_info;
    usat_system_info_database_ptr->best_six_info    = *rr_get_last_best_six();
    dedicated = FALSE;
  }

  GRR_GENERAL_UNLOCK();

  rr_get_ba_list_for_measurement_report(bcch_list, dedicated);
  rr_get_last_meas_results(nmr, len_of_nmr, dedicated);
 ...
 * @param bcch_list - ba list
 */
boolean rr_get_nmr_bcch_dyn(uint8 *nmr, rr_bcch_list_type *bcch_list)
{
 MSG_GERAN_LOW_0_G("Depreciated API will be removed");
 return FALSE;
}

boolean rr_api_get_nmr_bcch_dyn(uint8 *nmr, uint8 len_of_nmr,  rr_bcch_list_type *bcch_list)
{
 return rr_ds_api_get_nmr_bcch_dyn(nmr, len_of_nmr, bcch_list);
}
 ...
 * @param cell_id - pointer where cell id is returned
 */
boolean rr_get_cell_id_dyn(
  uint8 *cell_id
)
{
  return rr_ds_get_cell_id_dyn(cell_id);
}
 ...
 *                         returned.
 */
boolean rr_ds_get_timing_advance_dyn(
  uint8                  *me_status,
...
)
{
  ...
  /**************************************************************************/
  timing_advance[0] = rr_general_data.rr_current_timing_advance>>4;
  timing_advance[1] = rr_general_data.rr_current_timing_advance & 0x0F;
 ...
 */
boolean rr_get_timing_advance_dyn(
  uint8 *me_status,
  uint8* timing_advance
)
{
  return rr_ds_get_timing_advance_dyn(me_status, timing_advance);
}
...
)
{
  ...
  {
    rr_sapi3_process_dl_data_ack_ind(l3_message_number);
    return;
  }

  if (IS_RR_SIGNALLING_ACK(new_message->SAPI, l3_message_number))
  {
    rr_handle_signalling_acknowledgement(l3_message_number);
    return;
  }
  ...
  {
    rr_dtm_send_imsg_l2_gttp_ack_ind();
    return;
  }
#endif // FEATURE_GSM_DTM

  /* SAPI0 NAS message - remove the L3 message from the NAS message buffer */
  if (rr_remove_from_nas_message_buffer(l3_message_number) == FALSE)
  {
...
{
  uint32 i;
  rr_nas_message_buffer_T **rr_nas_msg_buf_ptr_ptr = rr_get_nas_message_buffer_ptr_ptr();
...
)
{
  ...
  {
    MSG_GERAN_LOW_1_G("Exceeded NAS Message Buffer limit (%d)", (int) MAX_NAS_MESSAGE_BUFFER_SIZE);
...
)
{
  ...
  {
     MSG_GERAN_LOW_1_G("Invalid l3_message_number(%d)", (int) l3_message_number);
    return FALSE;
  }

  if (rr_nas_msg_buf_ptr_ptr[index] == NULL)
  {
     MSG_GERAN_LOW_1_G("No data buffered at index(%d)", (int) index);
    return FALSE;
  }
...
)
{
  ...
  for (index = 0; index < num; index++, loop_ptr++)
  {
    ...
    loop_ptr->band = (sys_band_T) rr_get_band_dyn(loop_ptr->num);
  }
}
...
void rr_initialize_SI2ter_instances( )
{
  si2ter_instances_T *si2ter_inst_ptr;

  si2ter_inst_ptr = rr_get_SI2ter_instances_ptr();
...
)
{
  ...
  si2q_inst_ptr = rr_get_instances_ptr(SI2Q_INSTANCE);
...
)
{
  ...
  mi_inst_ptr = rr_get_instances_ptr(MI_INSTANCE);
...
/**************************************************************************************
Function
rr_reset_SI21_instances

Description
Frees memorry allocated for all SI21 instances.

Parameters
[IN] *inst_ptr: Pointer to global storage for the instances.

Returns
None
**********************************************************************************************/
void rr_reset_SI21_instances(si21_instances_T *inst_ptr  )
{
  byte i;

  //RR_NULL_CHECK_FATAL(inst_ptr != NULL);

  MSG_GERAN_HIGH_1_G("Clearing SI21 instances (num_instances=%d)!", inst_ptr->num_instances);

  for (i = 0; i < MAX_SI21_INSTANCES; i++)
  {
    if(inst_ptr->instance[i] != NULL)
    {
      GPRS_MEM_FREE(inst_ptr->instance[i]);
      inst_ptr->instance[i] = NULL;
    }
  }

  inst_ptr->num_instances = 0;
}
...
None
**********************************************************************************************/
void rr_initialize_SI21_instances( )
{
  si21_instances_T *si21_inst_ptr;

  si21_inst_ptr = rr_get_SI21_instances_ptr();
  rr_reset_SI21_instances(si21_inst_ptr);
}
#endif /* FEATURE_GERAN_EAB */
...
)
{
  if (dedic_ptr != NULL)
  {
    ...
    rr_ciphering_has_started(dedic_ptr->curr_channel_spec.cipher_flag);
#ifdef FEATURE_LTE
    if (rr_ce_access_reason() == MM_RR_MO_FULL_SPEECH ||
        ...
        rr_ce_is_periodic_lau() == TRUE)
    {
      if ((sync_reason == MM_RR_RES_ASS) && (rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled()))
      {
 ...
 */
void rr_hz_deregister_state_change_cb_dyn(
  void
)
{
  // Obtain a pointer to the module data
  ...
  MSG_GERAN_LOW_0("De-registering Home Zone callback function");
  rr_general_data_ptr->rr_hz_info.state_change_cb_ptr = NULL;
 ...
 */
boolean rr_hz_register_state_change_cb_dyn(
...
)
{
  ...
  if (rr_general_data_ptr->rr_hz_info.state_change_cb_ptr != NULL)
  {
    ...
    MSG_GERAN_LOW_0("ERROR: Home Zone callback is already registered");
    return(FALSE);
  }

  MSG_GERAN_LOW_0("Registering Home Zone callback function");
...
)
{
  ...
  if (bChanged && bNotify)
  {
    rr_hz_notify_state_change(RR_HZ_STATE_IDLE);
  }
...
)
{
  ...
  if (bPSActive)
  {
    ...
    {
      rr_hz_notify_state_change(RR_HZ_STATE_PS_CONNECTION);
    }
  }
  else
  {
...
)
{
  ...
  if (rr_x2g_redir_search_for_emergency_call_in_progress())
  {
...
#ifdef FEATURE_GERAN_CS_SUPPORT
  if (rr_saved_msg_is_rr_est_req_emergency_call())
  {
...
*/
void gprs_odm_get_plmn_lac_list_dyn(
...
)
{
  ...
  rr_candidate_cell_db_ptr = rr_get_candidate_cell_db_ptr();
 ...
 */
void gprs_odm_set_plmn_lac_list_dyn(
...
)
{
  ...
  scell_info.band = (eng_mode_band_t)rr_get_band_dyn(scell_info_ptr->gsm.BCCH_ARFCN.num);
...
#define ENG_MODE_SGSNR_REV_99  1
  ...
  nw_params.nc_mode = rr_gprs_get_network_control_order();
  nw_params.nmo = rr_si_get_nmo();
  ...
  if (rr_in_dedicated())
  {
    gprs_params_available = rr_get_nv_dtm_enabled() && rr_cell_supports_dtm();
...
#endif /* FEATURE_GSM_DTM */
  {
    ...
    */
    gprs_params_available =
      rr_cell_supports_gprs() && cell_info_ptr->bcch_sys_info.si13_data_valid_flag;
 ...
 */
usat_system_information_database_T *rr_get_usat_system_information_database_ptr(void)
{
  return &usat_system_information_database;
}
...
)
{
  if (rr_is_camped())
  {
    ...
    rr_individual_priorities_t ** individual_priorities = rr_individual_priorities();
    ...
    else
    {
      ...
      rr_utran_frequency_list_create(&utran_frequency_list);
...
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
...
sys_band_mask_type rr_band_pref( )
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  sys_band_cap_u_type band_cap_u = {0};
...
sys_band_mask_type rr_search_band_pref( )
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  sys_band_cap_u_type band_cap_u = {0};
...
#ifdef FEATURE_WCDMA
sys_band_mask_type rr_band_pref_wcdma( )
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  sys_band_cap_u_type band_cap_u = {0};
...
#endif
#ifdef FEATURE_GSM_TDS
sys_band_mask_type rr_band_pref_utran_tdd( )
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  sys_band_cap_u_type band_cap_u = {0};
...
#endif
#ifdef FEATURE_LTE
sys_lte_band_mask_e_type rr_band_pref_lte( )
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  sys_band_cap_u_type band_cap_u = {0};
...
)
{
  switch (sys_sband)
  {
    ...
    default:
      MSG_GERAN_LOW_1("Unknown sys_sband: 0x%08x", sys_sband);
...
)
{
  switch (sys_band)
  {
    ...
    default:
      MSG_GERAN_LOW_1("Unknown sys_band_T: 0x%08x", sys_band);
...
)
{
  ...
  if (individual_priorities_ie->t3230_timeout_value_valid)
  {
    ...
    (void)rr_timer_start(T3230, rr_t3230_timeout_ms(individual_priorities_ie->t3230_timeout_value));
  }

  lte_rrc_dedicated_pri_set_owner(LTE_RRC_DEDICATED_PRI_OWNER_GERAN);
  rr_individual_priorities_dump();
}

void rr_individual_priorities_clear(void)
{
  if (geran_is_multi_mode_gas_id())
  {
    rr_individual_priorities_t ** individual_priorities = rr_individual_priorities();
...
boolean rr_individual_priorities_valid( )
{
  rr_individual_priorities_t ** individual_priorities = rr_individual_priorities();
...
*/
void rr_individual_priorities_dump( )
{
  rr_individual_priorities_t ** individual_priorities = rr_individual_priorities();
...
)
{
  ...
  if (rr_dedicated_priority_info_valid(dedicated_priority_info))
  {
    ...
    if (dedicated_priority_info->rem_validity_timer_is_present)
    {
      ...
      (void) rr_timer_start(T3230, (uint32) dedicated_priority_info->rem_validity_timer);
    }
  }
  else
  {
...
void rr_dedicated_priority_info_get(lte_irat_dedicated_priority_info_s * dst )
{
  rr_individual_priorities_t ** individual_priorities = rr_individual_priorities();
  ...
  if (*individual_priorities != NULL)
  {
   dst->priority_list = (*individual_priorities)->dedicated_priority_list;
    ...
    if (rr_timer_is_running(T3230))
    {
      dst->rem_validity_timer_is_present = TRUE;
      dst->rem_validity_timer = rr_timer_get_time_remaining(T3230);
    }
  }
}
...
)
{
  ...
  switch (owner)
  {
    case LTE_RRC_DEDICATED_PRI_OWNER_LTE:
    {
      ...
      result = rr_msgr_send(&ind.msg_hdr, sizeof(lte_rrc_clear_dedicated_pri_ind_s));
      if (result != E_SUCCESS)
      {
    ...
    {
      rr_send_rrc_clear_dedicated_priorities_ind();
      break;
    ...
    {
      rr_send_tds_clear_dedicated_priorities_ind();
      break;
    }
#endif /*FEATURE_TDSCDMA_DEDICATED_PRI_INFO*/
    default:
...
void rr_dedicated_priorities_take_ownership(void)
{
  if (geran_is_multi_mode_gas_id())
  {
    if (rr_individual_priorities_valid())
    {
      ...
      if (dedicated_pri_owner != LTE_RRC_DEDICATED_PRI_OWNER_GERAN)
      {
        ...
        rr_dedicated_priorities_request_owner_clears(dedicated_pri_owner);
      }
    }
  }
  else
...
)
{
  if (geran_is_multi_mode_gas_id())
  {
    ...
    MSG_GERAN_MED_1_G("rr_dedicated_priorities_request_from_owner: Existing owner=%d",dedicated_pri_owner);
    ...
    if (dedicated_pri_owner != LTE_RRC_DEDICATED_PRI_OWNER_GERAN)
    {
      switch (dedicated_pri_owner)
      {
        case LTE_RRC_DEDICATED_PRI_OWNER_LTE:
        {
          ...
          result = rr_msgr_send(&req.msg_hdr, sizeof(lte_rrc_get_dedicated_pri_req_s));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case LTE_RRC_DEDICATED_PRI_OWNER_WCDMA:
        {
          ...
          rr_send_rrc_dedicated_priorities_req();
          break;
        } /* LTE_RRC_DEDICATED_PRI_OWNER_WCDMA */
#endif /* FEATURE_WCDMA */
        ...
        case LTE_RRC_DEDICATED_PRI_OWNER_TDSCDMA:
        {
          ...
          rr_send_tds_dedicated_priorities_req();
          break;
        }
#endif /*FEATURE_TDSCDMA_DEDICATED_PRI_INFO*/
        default:
 ...
 */
void rr_general_task_start_init(void)
{
  ...
  rr_general_data.S_counter = RR_DEF_RLT_COUNTER;
}

void rr_plmn_selection_triggered( )
{
  rr_params_mods_t ** params_mods = rr_params_mods();
  rr_params_mods_t ** pcco_params_mods = rr_pcco_params_mods();
...
void rr_cell_selection_triggered( )
{
  rr_params_mods_t ** params_mods = rr_params_mods();
  rr_params_mods_t ** pcco_params_mods = rr_pcco_params_mods();
 ...
 */
boolean rr_eplmn_list_equivalent_plmn(sys_plmn_id_s_type plmn )
{
 ...
 */
boolean rr_ehplmn_list_equivalent_plmn(sys_plmn_id_s_type plmn )
{
#ifdef FEATURE_EQUIVALENT_HPLMN
 ...
 */
boolean rr_is_ehplmn_or_hplmn(sys_plmn_id_s_type plmn  )
{
...
)
{
  ...
  MSG_LOW("rcvd  AMR type = %d rcvd converted AMR type = %d",(int) rcvd_channel_mode,
            (int)rcvd_channel_mode_1,0);

  MSG_LOW("current AMR type = %d current converted  AMR type = %d",(int) current_chan_mode,
...
#endif
#ifdef FEATURE_SGLTE
void rr_sglte_dedicated_priority_info_store(sglte_l2g_resel_req_ded_pri_info_t * sglte_dedicated_pri_info_ptr )
{
  ...
  if (!plmn_ok)
  {
    plmn_ok = rr_eplmn_list_equivalent_plmn(requested_PLMN_id) &&
              rr_eplmn_list_equivalent_plmn(sglte_dedicated_pri_info_ptr->requested_PLMN);
  }
...
boolean rr_check_and_populate_default_irat_prior_params(  )
{
  ...
  serving_cell_ptr = rr_gprs_get_scell_info();
  pseudo_si2q_data_p = rr_get_efs_nv_fast_g2l_pseudo_si2q_data();
  ...
  if ( !(rr_is_efs_nv_fast_g2l_using_pseudo_si2q_enabled() || rr_is_efs_nv_fast_g2l_using_pseudo_si2q_data_valid() ))
  {
    MSG_GERAN_HIGH_0_G("GRR Fast G2L Reselection Feature NOT enabled or EFS Data NOT VALID");    
    return FALSE;
  }

  if ( rr_is_pseudo_si2q_data_used() && (serving_cell_ptr->lte_neighbour_list.count > 0) )
  {
  ...
  for (i = 0; i < nas_earfcn.count; i++)
  {
    ...
    if (lte_rrc_earfcn_is_supported(nas_earfcn.earfcn_nas[i], rr_band_pref_lte()))
    {
  ...
  if ( pseudo_si2q_data_p != NULL ) 
  {
    ...
    rr_restore_default_serv_utran_pseudo_si2q_params( serving_cell_ptr );

    if ( (serving_cell_ptr->lte_neighbour_list.count == 0) || (rr_is_pseudo_si2q_data_used() == FALSE ) )
    {
      rr_lte_neighbour_list_entry_t * dst;
      ...
      rr_set_rr_using_pseudo_si2q_data(TRUE);         
      return TRUE;               
    }
  }
...
===========================================================================*/
boolean rr_default_irat_params_update_required( )
{
...
===========================================================================*/
void rr_restore_default_serv_utran_pseudo_si2q_params(gprs_scell_info_t *serving_cell_ptr )
{
  rr_efs_pseudo_si2q_def_data_t *pseudo_si2q_data_p = NULL;
  pseudo_si2q_data_p = rr_get_efs_nv_fast_g2l_pseudo_si2q_data();
...
===========================================================================*/
void rr_flush_default_irat_prior_params(  )
{

  gprs_scell_info_t *serving_cell_ptr = rr_gprs_get_scell_info();
 ...
 * \return uint8
 */
uint8 rr_scell_rxlev_average( void )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
 ...
 * \return PLMN_id_T
 */
PLMN_id_T rr_scell_plmn_id( void )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
 ...
 * \return PLMN_id_T
 */
sys_srv_capability_e_type rr_scell_service_capability( void )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
...
uint32 rr_get_T3236_value( )
{
  /*When a mobile station starts an implicit reject timer it selects a value randomly drawn
      from a uniform probability distribution within the set {10.0, 10.1, 10.2, …200.0} seconds (inclusive).*/   
  uint8 random;
  uint32 t3236_timeout;
  ...
  if (random > 190)
  {
    /*As random is 8 bit so it can be from 0-255 but what we are looking here is from 0-190*/
    random = random/2;
  }
  t3236_timeout = 10000 + (random *1000);
  MSG_GERAN_HIGH_1_G("T3236 time out in second is %d", t3236_timeout/1000);
  return t3236_timeout;
}
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/

#ifdef FEATURE_PS_PREFERRED_CELL
boolean rr_ps_preferred_allowed()
{
  return (!rr_get_nv_anite_gcf_flag() && 
          rr_nv_ps_preferred_enabled() && 
          grr_is_ps_allowed_by_service_domain());
}

uint8 rr_num_ps_supported_cells_in_pscan()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

  return rr_general_data_ptr->num_ps_supported_cells_in_pscan;
}

void rr_set_num_ps_supported_cells_in_pscan(uint8 num_cells)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

  MSG_GERAN_MED_1_G("[PS preferred] Number of PS supported cells in pscan set to %d", num_cells);
  rr_general_data_ptr->num_ps_supported_cells_in_pscan = num_cells;
}

void rr_inc_num_ps_supported_cells_in_pscan()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  rr_general_data_ptr->num_ps_supported_cells_in_pscan++;

//  MSG_GERAN_MED_1_G("[PS preferred] Number of PS supported cells in pscan incremented (count = %d)", rr_general_data_ptr->num_ps_supported_cells_in_pscan);
}

void rr_dec_num_ps_supported_cells_in_pscan()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  rr_general_data_ptr->num_ps_supported_cells_in_pscan--;

//  MSG_GERAN_MED_1_G("[PS preferred] Number of PS supported cells in pscan decremented (count = %d)", rr_general_data_ptr->num_ps_supported_cells_in_pscan);
}

uint8 rr_num_ps_cells_si_received()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

  return rr_general_data_ptr->num_ps_cells_si_received;
}

void rr_reset_num_ps_cells_si_received()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

//  MSG_GERAN_MED_0_G("[PS preferred] Number of PS cells SI received reset to zero");
  rr_general_data_ptr->num_ps_cells_si_received = 0;
}

void rr_inc_num_ps_cells_si_received()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();
  rr_general_data_ptr->num_ps_cells_si_received++;

//  MSG_GERAN_MED_1_G("[PS preferred] Number of PS cells SI received incremented (count = %d)", rr_general_data_ptr->num_ps_cells_si_received);
}

uint16 rr_num_frequencies_in_pscan()
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

  return rr_general_data_ptr->num_frequencies_in_pscan;
}

void rr_set_num_frequencies_in_pscan(uint16 num_freqs)
{
  rr_general_data_t *rr_general_data_ptr = rr_general_get_data_ptr();

  MSG_GERAN_MED_1_G("[PS preferred] Number of frequencies in pscan set to %d", num_freqs);
  rr_general_data_ptr->num_frequencies_in_pscan = num_freqs;
}
#endif /* FEATURE_PS_PREFERRED_CELL */

void rr_perform_backup_for_scell(void)
{
  rr_static_general_data_t *rr_static_general_data_ptr = rr_static_general_get_data_ptr();
  rr_static_general_data_ptr->rr_scell_backup = (rr_scell_backup_t*)GPRS_MEM_MALLOC(sizeof(rr_scell_backup_t));

  if (rr_static_general_data_ptr->rr_scell_backup != NULL)
  {
    uint8 count = 0;
    rr_scell_backup_t            *rr_scell_backup_ptr = rr_static_general_data_ptr->rr_scell_backup;
    rr_PLMN_request_details_T    *rr_PLMN_request_details_ptr = rr_get_PLMN_request_details_ptr();
    gprs_scell_info_t            *scell = rr_gprs_get_scell_info();
    inter_task_BCCH_allocation_T *ba_gprs_list = rr_gprs_get_ba_gprs();

    /* Back up SI cache data for serving cell */
    count = rr_csi_backup_scell_info(scell->gsm.BCCH_ARFCN,
                                     &(scell->gsm.location_area_identification),
                                     &(scell->gsm.cell_identity),
                                     rr_scell_backup_ptr->scell_csi);
    rr_scell_backup_ptr->scell_csi_count = count;
    rr_scell_backup_ptr->RXLEV_average = scell->gsm.reselection_info.RXLEV_average;
    rr_scell_backup_ptr->bsic = scell->gsm.BSIC;

    /* Store BA-GPRS list for resel */
    memscpy(&rr_scell_backup_ptr->BA_gprs,
            sizeof(inter_task_BCCH_allocation_T),
            ba_gprs_list,
            sizeof(inter_task_BCCH_allocation_T));

    /* Store PLMN_req details into backup */
    memscpy(&rr_scell_backup_ptr->plmn_req_details,
            sizeof(rr_PLMN_request_details_T),
            rr_PLMN_request_details_ptr,
            sizeof(rr_PLMN_request_details_T));

    MSG_GERAN_MED_5_G("SCell backed-up:: CSI_count: %d, RXLEV_average: %d, BSIC [%d, %d], ncells: %d",
     rr_scell_backup_ptr->scell_csi_count,
     rr_scell_backup_ptr->RXLEV_average,
     rr_scell_backup_ptr->bsic.PLMN_colour_code,
     rr_scell_backup_ptr->bsic.BS_colour_code,
     rr_scell_backup_ptr->BA_gprs.no_of_entries);
  }
  else
  {
    ERR_GERAN_FATAL_1_G("Failed to allocate heap memory of size %d for SCELL backup", sizeof(rr_scell_backup_t));
  }
}

#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH

boolean rr_is_frequency_band_supported(ARFCN_T *arfcn, sys_band_mask_type band_pref)
{
  MSG_GERAN_HIGH_2(" [GPS search] : Band reported for  ARFCN : %d is %d", arfcn->num, arfcn->band);
  ...
  switch(arfcn->band)
  {
    ...
    default:
       MSG_GERAN_HIGH_0("[GPS search] : Invalid Band Reported");
       break;
  }
...
boolean rr_is_any_frequency_band_supported(void)
{
  rr_PLMN_list_req_info_T *rr_plmn_list_req_info_ptr = rr_get_plmn_list_req_info_ptr();
  uint32 i;
  sys_band_cap_u_type band_cap_u = rr_rat_pri_list_info_band_pref(rr_plc_get_rat_pri_list_info(), SYS_SYS_MODE_GSM);

  MSG_GERAN_HIGH_2("[GPS search] : SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH found with num_gsm_ad_element = %d, is_gsm_blind_scan_req = %d",
    rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.num_gsm_ad_element,
    rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.is_gsm_blind_scan_req);

  if(rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.num_gsm_ad_element != 0)
  {
     for (i = 0;
          i < rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.num_gsm_ad_element && 
          i < SYS_MAX_EARFCN_SCAN_REQ; 
          i++)
     {
        if(rr_is_frequency_band_supported(
             &rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.gsm_ad_elems[i].arfcn,
             band_cap_u.chgwt_band_cap))
        {
           MSG_GERAN_HIGH_0("[GPS search] : Atleast an ARFCN present from supported band(s) in list search request, hence proceeding further");
           return TRUE;
        }
     }
   }
