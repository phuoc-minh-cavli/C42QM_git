...
==============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sglte_g2x_conn_resel.c#2 $ */
...
static rr_sglte_g2x_resel_conn_data_t  rr_sglte_g2x_resel_conn_data_store;
 ...
 * -------------------------------------------------------------------------*/
     ...
     */
static rr_sglte_g2x_resel_conn_data_t *rr_sglte_g2x_resel_conn_get_data_ptr(void)
{
   return &rr_sglte_g2x_resel_conn_data_store;
}
...
static void rr_sglte_g2x_resel_conn_log_start(rr_irat_cell_t * target  )
{
...
static void rr_sglte_g2x_resel_conn_log_end(rr_irat_cell_t * target  )
{
  RR_NULL_CHECK_RETURN_VOID(target);
 ...
 * 
 */
static void rr_sglte_g2x_resel_conn_ps_blocked_cb_fn( )
{
  rr_sglte_rr_conn_g2x_resel_control(EV_G2X_CONN_RESEL_PS_BLOCKED, NULL);
}
 ...
 * 
*/
static void rr_sglte_g2x_resel_conn_suspend_mac( )
{
   /**
...
static void rr_sglte_g2x_resel_conn_cache_rat_pri_list_info(void)
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  rr_sglte_g2x_resel_conn_data_t* rr_sglte_g2x_resel_conn_data_store_ptr = rr_sglte_g2x_resel_conn_get_data_ptr();
...
static void rr_sglte_g2x_resel_restore_rat_pri_list_info(void)
{
  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();
  rr_sglte_g2x_resel_conn_data_t* rr_sglte_g2x_resel_conn_data_store_ptr = rr_sglte_g2x_resel_conn_get_data_ptr();
  RR_NULL_CHECK_RETURN_VOID(plmn_request_details);
...
void rr_sglte_rr_conn_g2x_resel_init( )
{
  rr_sglte_g2x_resel_conn_data_t* rr_sglte_g2x_resel_conn_data_p =  rr_sglte_g2x_resel_conn_get_data_ptr();
...
rr_event_T rr_sglte_rr_conn_g2x_resel_control(rr_event_T control_event, rr_cmd_bdy_type *new_message  )
{
  rr_sglte_g2x_resel_conn_data_t* rr_sglte_g2x_resel_conn_data_p =  rr_sglte_g2x_resel_conn_get_data_ptr();
  rr_event_T out_event = control_event;
  rr_event_T new_control_event = EV_NO_EVENT ;

  if( rr_get_nv_sglte_g2x_conn_mode_resel_enabled(GERAN_ACCESS_STRATUM_ID_1) == FALSE )
  {
    ...
    case EV_G2X_CONN_RESEL_START_RESEL_EVAL:
        {
          ...
          if (rr_is_lte_supported() && rr_serving_cell_priority_parameters_ie_received(rr_gprs_get_scell_info()))
          {
          ...
          { /* RUN PRIORITY BASED ALGORITHM */
            ...
            if (rr_irat_check_for_priority_based_reselection(TRUE,FALSE))
            {
        ...
        case EV_G2X_CONN_RESEL_START_RESELECTION:
        {
          ...
          else
          {
            ...
            rr_sglte_g2x_resel_conn_suspend_mac();
        ...
        case MS_RR_L1:
        {
          switch ((new_message->rr.rr.imsg.header.imh.message_id))
          {
            case MPH_UE_MODE_CHANGE_CNF:
            {
              ...
              rr_sglte_g2x_resel_conn_data_p->target = rr_irat_resel_target_get();
              rr_sglte_g2x_resel_conn_cache_rat_pri_list_info();

              MSG_GERAN_HIGH_0_G("MSG MPH_UE_MODE_CHANGE_CNF Received BY SGLTE G2X Connected Mode Reselection Controller");
              rr_sglte_g2x_resel_conn_log_start( &rr_sglte_g2x_resel_conn_data_p->target );
              ...
              switch ( rr_sglte_g2x_resel_conn_data_p->target.rat )
              {
                ...
                case RR_RAT_UTRAN_TDD:
                {
                 ...
                 case RR_RAT_LTE:
                 {
                   ...
                   resel_req.nas_params.rat_pri_list = *rr_rat_pri_list_info();
                   ...
                   rr_dedicated_priority_info_get(&resel_req.dedicated_priority_info);
                   MSG_GERAN_HIGH_0_G("MSG LTE RRC RR RESELECTION REQUEST Sent FROM SGLTE G2X Connected Mode Reselection Controller");

                   result = rr_msgr_send(&resel_req.msg_hdr, sizeof(resel_req));
                   if (result != E_SUCCESS)
                   {
        ...
        switch (new_message->message_header.message_set)
        {
            ...
            switch (new_message->rr.header.rr_message_set)
            {
              case RR_IMSG:
              {
                if ( new_message->rr.rr.imsg.header.imh.message_id == RR_IMSG_SGLTE_SWITCH_TO_CS_ONLY_GSM_ONLY_REQ)
                {
                  ...
                  rr_g2wr_reset_wcdma_surrounding_database();
                  rr_sglte_g2x_resel_conn_log_end( &rr_sglte_g2x_resel_conn_data_p->target );
                  out_event = control_event;
                  rr_sglte_g2x_resel_conn_data_p->state = RR_SGLTE_G2X_RESEL_CONN_NULL;
                  break;
                  }
         ...
         { /* Received ABort in the middle of Reselection */
           ...
           switch (rr_sglte_g2x_resel_conn_data_p->target.rat)
           {
             case RR_RAT_WCDMA:
             {
                rr_send_interrat_reselection_abort_req();
                break;
             } /* RR_RAT_WCDMA */
             ...
             case RR_RAT_UTRAN_TDD:
             {
               rr_send_tds_interrat_reselection_abort_req();
               break;
             } /* RR_RAT_TDSCDMA */
#endif /*FEATURE_GSM_TDS*/
             ...
             case RR_RAT_LTE:
             {
               ...
               result = rr_msgr_send(&abort_resel_req.msg_hdr, sizeof(abort_resel_req));
               if (result != E_SUCCESS)
               {
                 MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
               }
            ...
            switch (new_message->message_header.message_id)
            {
              ...
              case RRC_INTERRAT_RESELECTION_REJ:
              {
                ...
                /* Restore the previously-cached RAT Priority List as it may have
                 ...
                 rr_sglte_g2x_resel_restore_rat_pri_list_info();
                 rr_send_mph_ue_mode_change_req(rr_gsm_only());
                 rr_sglte_g2x_resel_conn_data_p->state = RR_SGLTE_G2X_RESEL_CONN_L1_MULTIRAT_ACTIVATION;
                 out_event = EV_NO_EVENT;
                break;
              ...
              case RR_PS_OMSG:
              {
                switch (new_message->rr.header.imh.message_id)
                {
                  case RR_PS_OMSG_PS_ACCESS_CNF:
                  {
                    ...
                    CALL_RR_CB_FN(rr_sglte_g2x_resel_conn_data_p->ps_access_confim_cb_fn);
                    ...
                    if (rr_sglte_g2x_resel_conn_data_p->is_abort_in_progress )
                    {
                      ...
                      out_event = EV_G2X_CONN_RESEL_ABORT_CNF;
                      rr_sglte_g2x_conn_send_omsg(RR_SGLTE_G2X_RESEL_CONN_OMSG_ABORT_CNF );
  ...
  if (rr_sglte_g2x_resel_conn_data_p->old_state != rr_sglte_g2x_resel_conn_data_p->state)
  {
    RR_LOG_STATE_TRANSITION("rr_sglte_rr_conn_g2x_resel_control",
                            ...
                            rr_sglte_g2x_resel_conn_data_p->state,
                            rr_sglte_g2x_resel_conn_state_name);
...
/*!
  ...
  */
boolean rr_sglte_g2x_resel_conn_inhibit_plmn_search( )
{
  ...
  if (rr_sglte_multimode_using_g_only_device())
  {
...
/*!
  ...
  * \return boolean, TRUE - inhibit PLMN search, FALSE - opposite
  */
boolean rr_sglte_g2x_conn_mode_resel_can_process_plmn_search( )
{
  rr_sglte_g2x_resel_conn_data_t* rr_sglte_g2x_resel_conn_data_p =  rr_sglte_g2x_resel_conn_get_data_ptr();
