...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_procedure.c#6 $ */
...
static rr_procedure_data_t rr_procedure_data =
 ...
 */
rr_procedure_data_t *rr_procedure_get_data_ptr( void )
{
  return &rr_procedure_data;
}
...
{
  rr_procedure_data_t *rr_procedure_data_ptr = rr_procedure_get_data_ptr();
...
)
{
  ...
  rr_procedure_data_ptr = rr_procedure_get_data_ptr();
  ...
  if ( proc_dispatch_event == EV_RESET_SOFTWARE )
  {
    ...
    (void) rr_cipher_control(EV_RESET_SOFTWARE, NULL);
  }
  else if ((proc_dispatch_event == EV_INPUT_MESSAGE) && (new_message != NULL))
  {
        ...
        (message_header->message_id == (byte) DL_DATA_IND))
    {
      ...
      {
        proc_dispatch_event = rr_inter_cell_handover_control(EV_INPUT_MESSAGE, new_message);
      }

      else if (message_type == ASSIGNMENT_COMMAND)
      {
        proc_dispatch_event = rr_intra_cell_handover_control(EV_INPUT_MESSAGE, new_message);
      }
      ...
      else if (message_type == CHANNEL_MODE_MODIFY)
      {
        ...
        rr_handle_channel_mode_modify_msg();
        proc_dispatch_event = EV_NO_EVENT;
      }

      else if (message_type == CIPHERING_MODE_COMMAND)
      {
        proc_dispatch_event = rr_cipher_control(EV_INPUT_MESSAGE, new_message);
      }
      ...
      {
        proc_dispatch_event = rr_handle_frequency_redefinition_msg();
      }

#ifdef FEATURE_WCDMA
      else if (message_type == INTERSYSTEM_TO_UTRAN_HANDOVER_COMMAND)
      {
        proc_dispatch_event = rr_g2w_handover_control(RR_EV_G2W_HO_STARTED, new_message);
      }
#endif /* FEATURE_WCDMA */
      ...
      {
        proc_dispatch_event = rr_dtm_control(EV_INPUT_MESSAGE, new_message);
      }
#endif /* FEATURE_GSM_DTM */
    }
    ...
    {
      proc_dispatch_event = rr_handle_frequency_redefinition_ind(new_message);
    }

    else if ((message_header->message_set == MS_RR_L1) &&
             (message_header->message_id == (byte) MPH_CHANNEL_MODE_MODIFY_CNF))
    {
      rr_handle_channel_mode_modify_cnf(new_message);
      proc_dispatch_event = EV_NO_EVENT;
    }
    ...
    else if (message_header->message_set == MS_GHDI_EXPORT)
    {
      ...
      {
        rr_perform_classmark_sending(RR_CLASSMARK_CLIENT_GHDI);
        proc_dispatch_event = EV_NO_EVENT;
      }
    }
    ...
    switch (rr_procedure_data_ptr->dispatch_state)
    {
      case DISP_INACTIVE_STATE :
      {
        if (proc_dispatch_event == EV_INTER_CELL_HANDOVER)
        {
          ...
          (void) rr_dtm_control(EV_INTRA_CELL_HANDOVER, NULL);
          #endif /* FEATURE_GSM_DTM */
        ...
        else if (proc_dispatch_event == EV_G2W_HANDOVER)
        {
          #ifdef FEATURE_GSM_DTM
          (void) rr_dtm_control(EV_INTRA_CELL_HANDOVER, NULL);
          (void) rr_dtm_control(EV_G2W_HANDOVER, NULL);
          #endif /* FEATURE_GSM_DTM */
        ...
        {
          (void) rr_dtm_control(proc_dispatch_event, NULL);
        }
        #endif /* FEATURE_GSM_DTM */
        ...
        else if (proc_dispatch_event == EV_NEW_MESSAGE)
        {
             ...
             )
          {
               ...
               )
            {
              ...
              if (SAPI == SAPI3)
              {
                rr_event = rr_control_SAPI3_link(new_message);
              }
              else
              {
                rr_event = EV_RADIO_LINK_FAILURE;
                   ...
                   (message_header->message_id == (byte) DL_RELEASE_IND))
          {
            ...
            else
            {
              ...
              rr_event = rr_connection_release(EV_INPUT_MESSAGE, new_message);
            }
          }
      ...
      case INTRA_CELL_HANDOVER_STATE :
      {
        ...
        {
          new_proc_dispatch_event = rr_intra_cell_handover_control(EV_INPUT_MESSAGE, new_message);
        }
        ...
        else if (proc_dispatch_event == EV_HANDOVER_TERMINATED)
        {
          ...
          #ifdef FEATURE_GSM_DTM
          (void) rr_dtm_control(EV_HANDOVER_COMPLETED, NULL);
          #endif /* FEATURE_GSM_DTM */
      ...
      case INTER_CELL_HANDOVER_STATE :
      {
        if (proc_dispatch_event == EV_NEW_MESSAGE)
        {
          new_proc_dispatch_event = rr_inter_cell_handover_control(EV_INPUT_MESSAGE, new_message);
        }

        else if (proc_dispatch_event == EV_T3124_TIMEOUT)
        {
          new_proc_dispatch_event = rr_inter_cell_handover_control(EV_T3124_TIMEOUT, NULL);
        }
        ...
        else if (proc_dispatch_event == EV_INTER_CELL_HANDOVER)
        {
          ...
          (void) rr_dtm_control(EV_INTER_CELL_HANDOVER, NULL);
        }
        else if (proc_dispatch_event == EV_DTM_SI6_RECEIVED)
        {
          ;  /* Ignore SI6 reception until handover completed & L2 resumed */
        ...
        {
          new_proc_dispatch_event = rr_inter_cell_handover_control(proc_dispatch_event, NULL);

          #ifdef FEATURE_GSM_DTM
          (void) rr_dtm_control(RR_EV_G2W_HO_FAIL, NULL);
          #endif /* FEATURE_GSM_DTM */
        }
        ...
        else if (proc_dispatch_event == RR_EV_G2W_HO_SUCCESS)
        {
          rr_procedure_data_ptr->dispatch_state = DISP_INACTIVE_STATE;
          #ifdef FEATURE_GSM_DTM
          (void) rr_dtm_control(RR_EV_G2W_HO_SUCCESS, NULL);
          #endif /* FEATURE_GSM_DTM */
        }
      ...
      case CIPHERING_STATE :
      {
        if (proc_dispatch_event == EV_NEW_MESSAGE)
        {
          ...
          #endif /* FEATURE_GSM_DTM */
          new_proc_dispatch_event = rr_cipher_control(EV_INPUT_MESSAGE, new_message);
        }
      ...
      case DTM_ASSIGNMENT_STATE:
      {
        if (proc_dispatch_event == EV_NEW_MESSAGE)
        {
          new_proc_dispatch_event = rr_dtm_control(EV_INPUT_MESSAGE, new_message);
        }
      ...
      default:
        MSG_GERAN_LOW_1_G("Unexpected state(%d) for procedure control",(int) rr_procedure_data_ptr->dispatch_state);
        break;
    } /* switch (rr_procedure_data_ptr->dispatch_state) */
...
)
{
  ...
  case RR_MSG_ID_CIPHER_MODE_COMPLETE:
    rr_confirm_ciphering_completed();
    break;
