...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_cell_reestablish.c#7 $ */
...
static rr_reest_api_data_t rr_reest_api_data;
static rr_reest_data_t *rr_reest_data_ptrs;
 ...
 */
static rr_reest_api_data_t *rr_reest_api_get_data_ptr( void )
{
  return &rr_reest_api_data;
}
 ...
 */
static rr_reest_data_t *rr_reest_get_data_ptr( void )
{
  return rr_reest_data_ptrs;
}
 ...
 */
static rr_reest_data_t *rr_reest_alloc_and_init_data( void )
{
  rr_reest_data_t *rr_reest_data_ptr = rr_reest_data_ptrs;

  if (rr_reest_data_ptr == NULL)
  {
    rr_reest_data_ptr = GPRS_MEM_CALLOC(sizeof(rr_reest_data_t));

    rr_reest_data_ptrs = rr_reest_data_ptr;
  }
 ...
 */
static void rr_reest_free_data( void )
{
  rr_reest_data_t *rr_reest_data_ptr = rr_reest_data_ptrs;
  ...
  if (rr_reest_data_ptr != NULL)
  {
    ...
    rr_reest_data_ptrs = NULL;
  }

} /* rr_reest_free_data */

static rr_reest_state_e rr_reest_get_state( void )
{
  rr_reest_data_t *rr_reest_reest_data_ptr = rr_reest_get_data_ptr();
 ...
 )
{
  ...
  if (rr_ce_for_emergency_call())
  {
    any_cell_selection_mode = TRUE;
  }
  else
  {
    any_cell_selection_mode = rr_is_any_cell_selection_mode();
  }

  automatic_plmn_selection_mode = rr_is_automatic_plmn_selection_mode();
  ...
  else
  {
    MSG_GERAN_HIGH_3_G("Cell %d not suitable (candidate cell check failed any_cell:%d auto_plmn:%d)",
             (int)candidate_cell_ptr->meas.arfcn_num, any_cell_selection_mode, automatic_plmn_selection_mode);
  }
 ...
 */
static rr_candidate_cell_T *rr_reest_find_cell_suitable_for_idle_camp( void )
{
    ...
    while ( (num_of_unprocessed_cells > 0) && (cell_suitable == FALSE) )
    {
      ...
      if (cell_found && (candidate_cell_ptr != NULL))
      {
        if (candidate_cell_ptr->type == RR_CANDIDATE_VALID_BCCH)
        {
          ...
          cell_suitable = rr_reest_cell_suitable_camp_cb_fn(candidate_cell_ptr);
        }
        else
        {
 ...
 )
{
  ...
  if (candidate_cell_ptr != NULL)
  {
    ...
    rr_reest_data_t *rr_reest_data_ptr = rr_reest_get_data_ptr();
    RR_NULL_CHECK_FATAL(rr_reest_data_ptr);
 ...
 */
static boolean rr_reest_is_channel_config_ok_for_reest( void )
{
  boolean channel_config_ok_for_reest = FALSE;

  // Obtain a pointer to the module API data
  rr_reest_api_data_t *rr_reest_api_data_ptr = rr_reest_api_get_data_ptr();
 ...
 */
static boolean rr_reest_discard_rr_est_req( void )
{
 ...
 * \param 
 */
static void rr_reest_send_rr_est_req_to_rr_control( void )
{
  // Obtain a pointer to the module data
  rr_reest_data_t *rr_reest_data_ptr = rr_reest_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_reest_data_ptr);
  ...
  {
    rr_save_rr_est_req(&rr_reest_data_ptr->rr_est_req);
 ...
 */
static boolean rr_reest_check_if_nas_reest_req_pending( void )
{
  // Obtain a pointer to the module data
  rr_reest_data_t *rr_reest_data_ptr = rr_reest_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_reest_data_ptr);
 ...
 * \param 
 */
static void rr_reest_abort_nas_reest_req_if_pending(mm_rr_failure_cause_e_type MM_fail_cause)
{
  if (rr_reest_check_if_nas_reest_req_pending())
  {
    rr_send_abort_ind(OTHER_REASON, MM_fail_cause);

    // The RR_EST_REQ is not needed now so discard it
    (void) rr_reest_discard_rr_est_req();
  }
...
static void start_gsm_idle( )
{
  ...
  rr_set_current_band(GET_ARFCN_BAND(scell_info_ptr->gsm.BCCH_ARFCN));
...
void rr_cell_reest_rlf_occurred( )
{
  ...
  dedicated_data_ptr = rr_get_dedicated_data_ptr();
 ...
 )
{
  ...
  rr_reest_state = rr_reest_get_state();
  ...
  switch (rr_reest_state)
  {
    ...
    case RR_REEST_NULL:
    {
      switch (event_in)
      {
        ...
        case EV_CHOOSE_CELL_FOR_CAMP:
        {
          ...
          rr_reest_data_ptr = rr_reest_alloc_and_init_data();
          break;
        }
    ...
    default:
    {
      rr_reest_data_ptr = rr_reest_get_data_ptr();
    }
  }
  ...
  while (event_in != EV_NO_EVENT)
  {
    ...
    else if (event_in == EV_INPUT_MESSAGE && message_set == MS_RR_L1 && message_id == MPH_PROC_IND)
    {
      // Abort any pending RR_EST_REQ (for re-establishment) from NAS
      rr_reest_abort_nas_reest_req_if_pending(MM_RR_NO_RESOURCE_AVAILABLE);
      event_in = RR_EV_MM_REQ_RECEIVED_ABORT;
    }
    ...
    switch (rr_reest_state)
    {
      ...
      case RR_REEST_NULL:
      {
        switch (event_in)
        {
          ...
          case EV_CHOOSE_CELL_FOR_CAMP:
          {
            ...
            the serving cell
            */
            surrounding_measurement_ptr = rr_get_surrounding_measurement_ptr();
                  ...
                  case RR_SELECT_BCCH_OMSG:
                  {
                    switch (message_ptr->rr.select_bcch.omsg.header.imh.message_id)
                    {
                      case RR_SELECT_BCCH_OMSG_SELECT_CNF:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("Minimal GSM sys info received");
                        ...
                        if (rr_reest_check_if_nas_reest_req_pending())
                        {
                          ...
                          */
                          start_gsm_idle();
#ifdef FEATURE_IDLE_DRX_SCALING
                          rr_idle_drx_control(RR_EV_CAMPED_ON_CELL, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
                        ...
                        else
                        {
                          ...
                          candidate_cell_ptr = rr_reest_find_cell_suitable_for_idle_camp();
                          ...
                          if (candidate_cell_ptr != NULL)
                          {
                            ...
                            else
                            {
                              ...
                              rr_reest_select_bcch_for_idle_camp(candidate_cell_ptr);
                      ...
                      case RR_SELECT_BCCH_OMSG_SELECT_FAIL_IND:
                      {
                        ...
                        // Abort any pending RR_EST_REQ (for re-establishment) from NAS
                        rr_reest_abort_nas_reest_req_if_pending(MM_RR_OTHER_FAILURE);
#ifdef FEATURE_LTE
                        if ( rr_connection_release_get_g2l_blind_redir_flag() || 
                             rr_connection_release_get_g2l_blind_redir_after_csfb_flag()) 
                        {
              ...
              case MS_MM_RR:
              {
                switch (message_id)
                {
                  case RR_EST_REQ:
                  {
                    ...
                    rr_reest_save_rr_est_req(&message_ptr->mm.rr_est_req);
      ...
      case RR_REEST_SELECT_CELL_CAMP:
      {
        switch (event_in)
        {
          ...
          {
            // If there is a RR_EST_REQ saved, discard it */
            if (rr_reest_discard_rr_est_req())
            {
              rr_send_abort_ind(OTHER_REASON, MM_RR_OTHER_FAILURE);
            }
 ...
 */
void rr_cell_reest_task_start_init( void )
{
