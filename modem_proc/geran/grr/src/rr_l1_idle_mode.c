...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_l1_idle_mode.c#6 $ */
/* $DateTime: 2018/11/28 04:48:52 $ */
...
static rr_l1_idle_data_t rr_l1_idle_data;
 ...
 */
static rr_l1_idle_data_t *rr_l1_idle_get_data_ptr( void )
{
  return &rr_l1_idle_data;
}
...
)
{
  ...
  {
    cell_info = rr_pcell_becomes_scell();
  }
...
*/
static void rr_send_internal_start_idle_cnf( )
{
  gprs_scell_info_t *cell_info = rr_gprs_get_scell_info();

  rr_l1_idle_send_omsg_start_idle_cnf(cell_info->gsm.gprs_indicator);
...
*/
static void rr_l1_idle_confirmed( )
{
  ...
  rr_send_internal_start_idle_cnf();
...
boolean  rr_is_l1_idle_state_start_idle( )
{

  rr_l1_idle_data_t *rr_l1_idle_data_ptr;

  // Obtain a pointer to the module data
  rr_l1_idle_data_ptr = rr_l1_idle_get_data_ptr();
...
*/
boolean rr_l1_idle_can_process_start_idle_req( )
{
      ...
      switch (msg_ptr->rr.header.rr_message_set)
      {
        case RR_L1_IDLE_IMSG:
        {
          switch (msg_ptr->message_header.message_id)
          {
            ...
            {
              rr_l1_idle_status_e l1_idle_status = rr_l1_send_get_l1_idle_status();
            ...
            case RR_L1_IDLE_IMSG:
            {
              switch (msg_ptr->message_header.message_id)
              {
                ...
                case RR_L1_IDLE_IMSG_START_IDLE_REQ:
                {
                  ...
                  if (rr_l1_send_get_l1_idle_status() == RR_L1_IDLE_STATUS_EARLY_CAMP_IDLE)
                  {
                  ...
                  rr_l1_start_idle(is_early_camped);
        ...
        case MS_RR_L1:
        {
          switch (msg_ptr->message_header.message_id)
          {
            ...
            case MPH_START_IDLE_MODE_CNF:
            {
              ...
              rr_l1_idle_send_omsg_start_early_camp_cnf();
              rr_l1_idle_data_ptr->state = RR_L1_STATE_NULL;
              break;
...
*/
void rr_l1_idle_init( void )
{
  // Obtain a pointer to the module data
  rr_l1_idle_data_t *rr_l1_idle_data_ptr = rr_l1_idle_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_l1_idle_data_ptr);
