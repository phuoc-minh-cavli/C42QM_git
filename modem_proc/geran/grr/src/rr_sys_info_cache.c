...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sys_info_cache.c#6 $ 
...
#define RR_MSG_IS_SI9(mt)        (SYSTEM_INFORMATION_TYPE_9 == (mt))
#define RR_MSG_IS_SI13(mt)       (SYSTEM_INFORMATION_TYPE_13 == (mt))
#define RR_MSG_IS_SI21(mt)       (SYSTEM_INFORMATION_TYPE_21 == (mt))
...
#define SI21_INDEX(octet_ptr) ((uint8)((octet_ptr[3] & 0x38) >> 3))

#define NUM_SI_TYPES_CACHED 10
#define NUM_SI_TYPES_FAST_ACQ 2

static rr_csi_pend_store_t pend_store;
 ...
 */
static rr_csi_pend_store_t *rr_sys_info_cache_get_pend_store_ptr( void )
{
  return &pend_store;
}
...
===========================================================================*/
static csi_entry_t *rr_csi_delete_oldest_entry(uint16 offset  )
{
  uint16 i;
...
static csi_entry_t *rr_csi_find_multi_instance_entry(csi_entry_t *target_ptr)
{
  ...
  if (IS_MULTI_INSTANCE_SI(target_ptr->msg_type))
  {
    ...
    else if (target_ptr->msg_type == SYSTEM_INFORMATION_TYPE_21)
    {
      req_si_index = SI21_INDEX(target_ptr->octet_ptr);
    }
    ...
    for (i = 0; i < MAX_NUMBER_CACHED_SI; i++, entry_ptr++)
    {
            ...
            rr_csi_is_cgi_equal(entry_ptr->cgi,target_ptr->cgi))
      {
        ...
        else if (entry_ptr->msg_type == SYSTEM_INFORMATION_TYPE_21)
        {
          cached_si_index = SI21_INDEX(entry_ptr->octet_ptr);
        }
                                            ...
                                            uint16             *ci_ptr)
{
  ...
  rr_csi_pend_store_t *pend_store_ptr = rr_sys_info_cache_get_pend_store_ptr();

  //RR_NULL_CHECK_FATAL(pend_store_ptr);

  // Check if sys info cache functionality enabled
  if (rr_nv_sys_info_cache_is_enabled() == FALSE)
  {
    return FALSE;
  }
       ...
       (ci_ptr == NULL) )
  {
    ...
    else
    {
      ...
      MSG_HIGH("LAI, Cell Id not supplied ignore SI, lai ptr:0x%x, cell_id ptr:0x%x", lai_ptr, ci_ptr,0);
      return FALSE;
    }
  }
  ...
  if (entry_ptr != NULL)
  {
    if (ARFCNS_EQUAL(arfcn, entry_ptr->si_arfcn) == FALSE)
    {
      ...
      rr_csi_flush_data(lai_ptr, ci_ptr, CSI_FLUSH_ARFCN_CHANGE);
  ...
  {    
    ...
    rr_log_csi_update_entry(entry_ptr,CSI_ENTRY_UPDATED);
    cached_ok = TRUE;
  }
  else
  {
    ...
    if (entry_ptr == NULL)
    {
      entry_ptr = rr_csi_delete_oldest_entry(current_counter);
    }
    ...
    if (entry_ptr != NULL)
    {
      ...
      {        
        ...
        *entry_ptr = new_entry;
        ...
        rr_log_csi_update_entry(entry_ptr,CSI_ENTRY_ADDED);
        cached_ok = TRUE;
      }
      else
      {
...
===========================================================================*/
static void rr_csi_create_sys_info_message(csi_entry_t *entry_ptr, dl_unit_data_ind_T *msg_ptr)
{
...
===========================================================================*/
static uint32 rr_csi_map_msg_type_to_mask(byte msg_type)
{
  switch (msg_type)
  {
    ...
    case SYSTEM_INFORMATION_TYPE_9:          return MASK_SI_9;
    case SYSTEM_INFORMATION_TYPE_21:         return MASK_SI_21;
    ...
    for (i = 0; i < MAX_NUMBER_CACHED_SI; i++, entry_ptr++)
    {
      if (IS_VALID_OTA_ENTRY(entry_ptr))
      {
        if (rr_csi_is_cgi_equal(entry_ptr->cgi,requested_cgi))
        {
          ...
          {
            (void)rr_log_csi_flush_data(entry_ptr,csi_flush_cause);
...
FUNCTION  rr_sys_info_cache_flush_data

DESCRIPTION
  Discards all entries from system information cache
...
===========================================================================*/
void rr_sys_info_cache_flush_data( void )
{
  csi_entry_t *entry_ptr;
  int          i;

  entry_ptr = sys_info_cache;
  MSG_GERAN_HIGH_0_G("DMMSUSREM Debug:: iinside rr_sys_info_cache_flush_data ");
...
void rr_csi_init_cache(void)
{
  ...
  pend_store.active = FALSE;

  for (j = 0; j < RR_CSI_PEND_STORE_SIZE; j++)
  {
    pend_store.store[j].msg_type = 0xFF;
    pend_store.store[j].octet_ptr = NULL;
  }
...
===========================================================================*/
rr_si_cache_SIs_to_retrieve_T rr_csi_is_retrieval_for_fast_acquisition( )
{
 
#ifdef FEATURE_LTE
  if (rr_is_csfb_call_in_progress() &&
      rr_get_nv_fast_si_acq_during_csfb_enabled() &&
      rr_nv_sys_info_cache_is_enabled())
  {
...
===========================================================================*/
uint32 rr_csi_retrieve_all_SIs(ARFCN_T arfcn, LAI_T *lai_ptr, uint16 *cell_id_ptr, rr_si_cache_SIs_to_retrieve_T SIs_to_retrieve )
{
  csi_entry_t *entry_ptr;
  ...
  MSG_GERAN_HIGH_2_G("RR retrieve all SIs for arfcn=%d, SIs_to_retrieve=%d", arfcn.num, num_si_types_to_retrieve);
    ...
    if (IS_MULTI_INSTANCE_SI(si_retrieval_order_ptr[i]))
    {
          ...
          ARFCNS_EQUAL(arfcn,entry_ptr->si_arfcn) == FALSE)
      {
        MSG_GERAN_ERROR_2_G("Requested ARFCN diff from cached on retrieve, cached:%d, requested:%d",entry_ptr->si_arfcn.num,
                                                                                                    arfcn.num);
        rr_csi_flush_data(lai_ptr, cell_id_ptr, CSI_FLUSH_SI_CHANGE);
        return FALSE;
      }

      while (entry_ptr != NULL)
      { 
        MSG_GERAN_HIGH_3_G("rr_csi_retrieve_all_SIs Send entry entry_ptr:0x%x ARFCN:%d msg_type:%d",entry_ptr,entry_ptr->si_arfcn.num,entry_ptr->msg_type);
        /* if a match was found, create a new inter-task signal with the data */
        rr_csi_create_sys_info_message(entry_ptr, &new_msg);

        if(!retrieve_cache_logged) /* Event Logging done only once per retrieval .*/
        {
          rr_log_csi_retrieve_entry(entry_ptr);
  ...
  {
    MSG_GERAN_MED_0_G("RR_IMSG_CSI_MULTIPLE_INSTANCES_SIS_RETRIEVED_IND sent");
    rr_send_imsg(RR_IMSG_CSI_MULTIPLE_INSTANCES_SIS_RETRIEVED_IND);
  }
...
uint8 rr_csi_backup_scell_info(ARFCN_T arfcn, LAI_T *lai_ptr, uint16 *cell_id_ptr, csi_entry_t* scell_csi)
{
  csi_entry_t *entry_ptr;
  csi_entry_t  csi_entry;
  uint16 i = 0;
  ...
  MSG_GERAN_HIGH_0_G("DMMSUSREM Debug:: rr_csi_backup_scell_info");
  if (rr_nv_sys_info_cache_is_enabled() == FALSE)
  {
    return count;
  }
  ...
  csi_entry.cgi.lai  = *lai_ptr;
  csi_entry.cgi.ci   = *cell_id_ptr;
  csi_entry.octet_ptr = NULL;
  csi_entry.counter = 0;

  for (i=0, count = 0; i < num_si_types_to_retrieve && count < MAX_NUM_SI_PER_CELL; i++)
  {

    MSG_GERAN_HIGH_0_G("DMMSUSREM Debug:: rr_csi_backup_scell_info save SIs");
    if (IS_MULTI_INSTANCE_SI(si_retrieval_order_ptr[i]))
    {
      offset = 0;
      csi_entry.msg_type = si_retrieval_order_ptr[i];
      ...
      if (entry_ptr != NULL &&
          ARFCNS_EQUAL(arfcn,entry_ptr->si_arfcn) == FALSE)
      {
        MSG_GERAN_ERROR_2_G("Requested ARFCN diff from cached, cached:%d, requested:%d", entry_ptr->si_arfcn.num, arfcn.num);
        continue;
      }

      while (entry_ptr != NULL && count < MAX_NUM_SI_PER_CELL)
      { 
        MSG_GERAN_HIGH_3_G("rr_csi_backup_scell_info saved entry_ptr:0x%x ARFCN:%d msg_type:%d",entry_ptr,entry_ptr->si_arfcn.num,entry_ptr->msg_type);

        scell_csi[count] = csi_entry;
        scell_csi[count].octet_ptr = (uint8*)GPRS_MEM_MALLOC(N201_MAX);
        if (scell_csi[count].octet_ptr != NULL)
        {        
          /* store the new entry in the cache */
          memscpy(scell_csi[count].octet_ptr, N201_MAX, entry_ptr->octet_ptr, N201_MAX);
          scell_csi[count].counter = entry_ptr->counter;
          count++;
        }
        else
        {
          ERR_GERAN_FATAL_0_G("RR_CSI unable to allocate heap memory");
        }
    ...
    else
    {
      ...
      if (entry_ptr != NULL &&
          ARFCNS_EQUAL(arfcn,entry_ptr->si_arfcn) == FALSE)
      {
        MSG_GERAN_ERROR_2_G("Requested ARFCN diff from cached, cached:%d, requested:%d",entry_ptr->si_arfcn.num,
                                                                                                    arfcn.num);
        continue;
      }
    ...
    if (entry_ptr != NULL)
    {
      si_mask |= rr_csi_map_msg_type_to_mask(scell_csi[count].msg_type);
    }
  }

  MSG_GERAN_MED_2_G("SIs backed up from cache; mask = %x, count = %d", si_mask, count);
  return count;
}

void rr_csi_restore_from_backup(csi_entry_t* scell_csi, uint8 count)
{
  uint8 i = 0;
  for (i = 0; i < count; i++)
  {
    (void)rr_csi_update_entry_internal(
      scell_csi[i].msg_type, 
      scell_csi[i].si_arfcn,
      scell_csi[i].octet_ptr,
      0x00,    //tc, only used for checking with 0xcc
      &(scell_csi[i].cgi.lai),
      &(scell_csi[i].cgi.ci));
  }
}
...
===========================================================================*/
boolean rr_csi_retrieve_SI3_4(ARFCN_T arfcn, LAI_T *lai_ptr, uint16 *cell_id_ptr )
{
  csi_entry_t *entry_ptr;
  csi_entry_t  csi_entry;
...
===========================================================================*/
void rr_csi_activate_pend_store( )
{
  rr_csi_pend_store_t *pend_store_ptr = rr_sys_info_cache_get_pend_store_ptr();
...
===========================================================================*/
void rr_csi_deactivate_pend_store( )
{
  rr_csi_pend_store_t *pend_store_ptr = rr_sys_info_cache_get_pend_store_ptr();
                                     ...
                                     uint16             *ci_ptr)
{
  ...
  MSG_GERAN_HIGH_1_G("RR copy pend store to cache for arfcn=%d",arfcn.num);
