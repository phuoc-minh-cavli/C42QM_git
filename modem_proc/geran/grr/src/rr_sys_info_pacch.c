...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sys_info_pacch.c#4 $ */
...
static rr_sys_info_pacch_data_t rr_sys_info_pacch_data;
 ...
 */
rr_sys_info_pacch_data_t *rr_sys_info_pacch_get_data_ptr( void )
{
  return &rr_sys_info_pacch_data;
}
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("PSI13 contents ignored");
    return FALSE;
  }
  ...
  MSG_GERAN_HIGH_0_G("PSI13:R13 not supported , reset the params");
  memset(&(decoded_psi13->peo_params), 0, sizeof(peo_params_t));
...
)
{
  ...
  if (rr_is_this_the_scell(bcch_arfcn))
  {
    ...
    if (decode_psi13(psi13_ptr, &decoded_psi13))
    {
      ...
      rr_gprs_process_psi13_ccch_camped(&decoded_psi13, scell_info);
    }
  }
  else
  {
    MSG_GERAN_LOW_1("PSI13 from unknown (arfcn=%d)",CONVERT_FROM_ARFCN_T(bcch_arfcn));
  }
} /* end process_psi13() */
...
)
{
  if (rr_is_this_the_scell(bcch_arfcn))
  {
    ...
    process_psi14_as_psi13(psi14_ptr, scell_info);
  }
  else if (rr_is_this_the_pcell(bcch_arfcn))
  {
    ...
    rr_gprs_process_psi14_as_si13(psi14_ptr, pcell_info);
  }
  else
  {
    MSG_GERAN_LOW_1("PSI14 from unknown (arfcn=%d)",CONVERT_FROM_ARFCN_T(bcch_arfcn));
  }
...
)
{
  ...
  bCancel = (rr_macinfo_get_mac_status() == MAC_TRANSFER);
  ...
  if (bCancel)
  {
    ...
    if (rr_timer_cancel(GRR_SYS_INFO_STATUS_TIMER_2))
    {
      MSG_GERAN_MED_2_G("Cancelled resend timer 0x%02X (format %1d)",
...
)
{
  ...
  if (si_status_ptr != NULL)
  {
    ...
    si_status_ptr->ps_rel_req = rr_gprs_get_sysinfo_status_params(NULL);
 ...
 */
static uint32 rr_gprs_retrieve_si_status_mask(
...
)
{
  ...
  if (rr_in_dedicated())
  {
    bsi_mask = rr_gprs_edtm_retrieve_bsi_status_mask();
  }
  else
...
#ifdef FEATURE_GERAN_EAB
  /* SI21 - multiple instance */
  si21_inst_ptr = rr_get_SI21_instances_ptr();
  if (si21_inst_ptr != NULL)
  {
    msg_ptr->msg_type = SYSTEM_INFORMATION_TYPE_21;
    if (si21_inst_ptr->num_instances)
    {
      si21_rest_T *valid_instance = NULL;
      uint8 i = 0;

      /*find the first valid instance*/
      for (i = 0; i < MAX_SI21_INSTANCES; i++)
      {
        if (si21_inst_ptr->instance[i])
        {
          valid_instance = si21_inst_ptr->instance[i];
          break;
        }
      }
      ...
      if (valid_instance == NULL)
      {
        MSG_GERAN_ERROR_1_G("RR error in SI21 instances, num_inst:%d, no valid data",si21_inst_ptr->num_instances);
        msg_ptr->mess_rec = SI_MSG_NOT_RECEIVED; /* message not received */
      }
      else
      {
        /* build the change mark */
        msg_ptr->six_chng_mrk = 0x00;
        if (valid_instance->si21_change_mark)
        {
          msg_ptr->six_chng_mrk |= 0x01;
        }

        if (valid_instance->si21_count == (si21_inst_ptr->num_instances-1))
        {
          msg_ptr->mess_rec = SI_MSG_FULL_MULTI_INSTANCE;
        }
        else
        {
          ...
          msg_ptr->mess_rec  = SI_MSG_PART_MULTI_INSTANCE;
          msg_ptr->six_count = valid_instance->si21_count;

          for (msg_index = 0; msg_index < MAX_SI21_INSTANCES; msg_index++)
          {
            if (si21_inst_ptr->instance[msg_index])
            {
              bitmap |= (uint16) (1U << si21_inst_ptr->instance[msg_index]->si21_index);
            }
          }
  ...
  else
  {
    ...
    switch (header->message_type)
    {
      case PACKET_PSI13:
        ...
        process_psi13((packet_psi13_t *)data, bcch_arfcn);
        break;

      case PACKET_PSI14:

        process_psi14((packet_psi14_t *)data, bcch_arfcn);
        break;
...
)
{
  ...
  if (rr_in_dedicated())
  {
    rr_gprs_edtm_process_pacch_sys_info(mac_data_ind_ptr);
  }
  else
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
  {
    ...
    rr_gprs_check_status_resend_timers(msg_type);
    rr_gprs_nacc_notify_bcch_msg_received(msg_ptr, msg_type);
  }

  /* for SIs received on the PACCH, extract the data and route it into RR */
  rr_gprs_extract_si_data(mac_data_ind_ptr);
...
)
{
  ...
  *******************************************************************/
  ...
  {
    MSG_GERAN_LOW_1_G("MAC not in transfer: current mac Status = %d",mac_status);
    return;
  }
  ...
  *********************************************************************/
  ...
  {
    if (rr_timer_is_running(GRR_SYS_INFO_STATUS_TIMER_1))
    {
      MSG_GERAN_MED_0_G("GRR_SYS_INFO_STATUS_TIMER_1 is running");
      return;
    }

    if (rr_timer_is_running(GRR_SYS_INFO_STATUS_TIMER_2))
    {
  ...
  *******************************************************************/
  if (rr_gprs_is_nacc_active() == TRUE)
  {
...
)
{
  if ((rr_macinfo_get_mac_status()            == MAC_TRANSFER) && /* GPRS transfer in progress */
      (rr_get_nv_anite_gcf_flag() == FALSE) && /* GCF flag is turned OFF */
      (scell_ptr                                    != NULL))
  {
    /* reset the defaults for a single sys info status message */
    rr_gprs_set_sysinfo_status_params(
...
)
{
  ...
  {
    MSG_GERAN_LOW_1_G("Unknown sysinfo status timer ID %d", (int)timer_id);
    return(rr_event);
  }

  // Obtain a pointer to the module data
  rr_sys_info_pacch_data_ptr = rr_sys_info_pacch_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_sys_info_pacch_data_ptr);

  /* explicitly clear the expired timer */
  (void)rr_timer_cancel(timer_id);
  ...
  if (bStartSecondary == TRUE)
  {
    ...
    if (rr_sys_info_pacch_data_ptr->num_sysinfo_status_retrans > 0)
    {
      ...
      (void)rr_timer_start(GRR_SYS_INFO_STATUS_TIMER_2, GRR_TIMEOUT_2_SECONDS);
...
)
{
  ...
  if (rr_gprs_initialise_si_status(&si_status))
  {
    uint32 bsi_mask;

    /* retrieve a mask of the SIs that should be reported */
    bsi_mask = rr_gprs_retrieve_si_status_mask();
  ...
  if ((bSent == TRUE) && (resend_timeout != 0))
  {
    ...
    (void)rr_timer_start(GRR_SYS_INFO_STATUS_TIMER_1, (dword)resend_timeout);
  }
 ...
 *
 */
void rr_sys_info_pacch_init( void )
{
  rr_sys_info_pacch_data_t *rr_sys_info_pacch_data_ptr;
 ...
 */
void rr_sys_info_pacch_task_start_init( void )
{
  // Zero the control/data structure(s)
