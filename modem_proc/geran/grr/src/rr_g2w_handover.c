...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_g2w_handover.c#3 $ $DateTime: 2018/10/16 07:03:13 $ $Author: pwbldsvc $
...
static rr_g2w_handover_static_data_t  rr_g2w_handover_data;
 ...
 */
static rr_g2w_handover_static_data_t *rr_g2w_handover_get_data_ptr(void)
{
  return &rr_g2w_handover_data;
}

/**
 * Initialisation function called when this module is activated
 */
static void rr_g2w_handover_activate_init( void )
{
  // Obtain a pointer to the module data
  rr_g2w_handover_static_data_t *rr_g2w_handover_data_ptr = rr_g2w_handover_get_data_ptr();
 ...
 */
void rr_g2w_handover_discard_data( void )
{
  // Obtain a pointer to the module data
  rr_g2w_handover_static_data_t *rr_g2w_handover_data_ptr = rr_g2w_handover_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2w_handover_data_ptr);
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_g2w_handover_data_ptr = rr_g2w_handover_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2w_handover_data_ptr);
        ...
        if (message_set == MS_MM_RR)
        {
          ...
          else if (message_id == (byte) RR_DATA_REQ)
          {
            if (rr_g2w_handover_data_ptr->g2w_ho_state != RR_G2W_HO_AWAITING_COMPLETION)
            {
              ...
              rr_process_rr_data_req(&message_ptr->rr_data_req);
              g2w_handover_event = EV_NO_EVENT;
            }
          }
        ...
        if (g2w_handover_event == RR_EV_G2W_HO_STARTED)
        {
          ...
          if (message_ptr != NULL)
          {
            ...
            */
            ...
            rr_send_dl_suspend_req(RR_L1_GSM_WCDMA);
      ...
      case RR_G2W_HO_AWAITING_L2_SUSPENSION:
      {
        if ((g2w_handover_event == EV_INPUT_MESSAGE) && (message_ptr!=NULL))
        {
          if ((message_set == MS_RR_L2) && (message_id == (byte) DL_SUSPEND_CNF))
          {
            ...
            /*********************************************************************************/
            rr_set_handover_type(RR_L1_GSM_WCDMA);
            ...
            new_g2w_handover_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
          }
          else if ((message_set == MS_RR_L2) && (message_id == (byte) DL_RELEASE_IND || message_id == (byte) MDL_ERROR_IND))
          {
            ...
            rr_log_handover_failure(RR_LOG_G2WHO_CANNOT_SUSPEND_GL2);
            rr_g2w_handover_discard_data();
            rr_g2w_handover_data_ptr->g2w_ho_state = RR_G2W_HO_INACTIVE;
          ...
          {
            rr_log_handover_failure(RR_LOG_HO_FAILURE_HANDOVER_ABORTED);
            rr_g2w_handover_discard_data();
            rr_g2w_handover_data_ptr->g2w_ho_state = RR_G2W_HO_INACTIVE;
      ...
      case RR_G2W_HO_GOING_INACTIVE:
      {
        if ((g2w_handover_event== EV_INPUT_MESSAGE) && (message_ptr!=NULL))
        {
          if ((message_set == MS_MM_RR) && (message_id == (byte) RR_ABORT_REQ))
          {
            ...
            new_g2w_handover_event = rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
        ...
        else if (g2w_handover_event == EV_GSM_MODE_DEACTIVATED)
        {
...
#ifdef FEATURE_LTE
          csfb_call_status = rr_connection_release_get_g2l_blind_redir_after_csfb_flag();
#endif /* FEATURE_LTE */

          /* L1 is deactivated; Set channel_configuration to indicate that */
          rr_ded_set_l1_chan_is_not_connected();
          ...
          rr_send_interrat_handover_req(
            rr_get_ciphering_mode_command_received(),
      ...
      case RR_G2W_HO_AWAITING_RRC_RESPONSE:
      {
        if ((g2w_handover_event== EV_INPUT_MESSAGE) && (message_ptr!=NULL))
        {
          ...
          else if ((message_set == MS_MM_RR) && (message_id == (byte) RR_ABORT_REQ))
          {
            ...
            rr_g2w_handover_data_ptr->data_ptr->abort_req_received = TRUE;
            rr_send_interrat_handover_abort_req();
