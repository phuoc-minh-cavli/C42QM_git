...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_l2_send.c#3 $ $DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
...
)
{
  ...
  if (msg_ptr != NULL)
  {
    ...
    switch (protocol_discriminator)
    {
      ...
      case SMS_MESSAGES:
      {
        msg_num = rr_sapi3_get_next_message_number();
        break;
      }
...
)
{
  if ((layer3_message != NULL) && (length >= 2) && (length <= MAX_OCTETS_L3_MESSAGE))
  {
    ...
    RR_PREPARE_L2_MESSAGE(dl_data_req, DL_DATA_REQ);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_establish_req, DL_ESTABLISH_REQ);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_random_access_req, DL_RANDOM_ACCESS_REQ);
  ...
  (void)geran_get_traffic_reason(&(dl_random_access_req.rach_for_ps_traffic));
...
)
{
  ...
  dl_random_access_req.egprs_ra_content = ( ( egprs_access_reference & 0x7f8 ) >> 3 ) + ( ( egprs_access_reference & 0x07 ) << 8 );
   ...
   MSG_GERAN_LOW_1_G("Send EGPRS content as (0x%02X) ",(int)egprs_access_reference);
  ...
  if ((rr_ue_in_peo_mode())
       ...
       && (rr_nv_get_allow_peo_rach_from_efs())
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/
      )
  {
    dl_random_access_req.egprs_training_seq = 4; /* TS 4 - For PEO case*/
  }
  else 
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
  if ( rr_get_nv_egprs_8psk_ul_enabled() )
  {
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_reconnect_req, DL_RECONNECT_REQ);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_release_req, DL_RELEASE_REQ);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(mdl_release_req, MDL_RELEASE_REQ);
...
static l2_channel_type_T rr_get_resumption_channel_type( )
{
  l2_channel_type_T l2_channel_type;
  ...
  channel_spec_ptr = rr_get_channel_spec(NULL);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_resume_req, DL_RESUME_REQ);

  dl_resume_req.l3_message_length = message_length;
  dl_resume_req.SAPI = SAPI0;
  dl_resume_req.l2_channel_type = rr_get_resumption_channel_type();
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_suspend_req, DL_SUSPEND_REQ);
...
)
{
  ...
  RR_PREPARE_L2_MESSAGE(dl_unit_data_req, DL_UNIT_DATA_REQ);
...
{
  rr_send_dl_reconnect_req(l3_msg, l3_msg_len);
}
...
)
{
  switch (method)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_1_G("Invalid rr_l2_resumption_method (%d)",method);
    }
  }
}
