...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_conn_establish.c#5 $
***
*** DESCRIPTION
***
...
static rr_tbf_est_static_data_t  rr_tbf_est_static_data;
 ...
 */
static rr_tbf_est_static_data_t *rr_tbf_est_get_data_ptr( void )
{
  return &rr_tbf_est_static_data;
}
...
===========================================================================*/
void grr_init_gprs_conn_establishment( )
{
  // Obtain a pointer to the module data
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr = rr_tbf_est_get_data_ptr();
...
)
{
  ...
  egprs_multislot_class = rr_get_nv_hmsc_egprs_coded_ms_class();
  #else
  egprs_multislot_class = rr_get_nv_egprs_multislot_class();
  ...
  else if (access_reason == GMAC_EGPRS_PEO_ONE_PHASE_ACCESS)
  { 

#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT  
   boolean peo_priority    = !(rr_ps_is_low_priority());
#else
    boolean peo_priority   = FALSE;
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/
    uint8   access_cause =  rr_ce_determine_pkt_est_cause_for_peo();
    boolean egprs_capability = rr_get_nv_egprs_enabled();
      
    /* bit 10-8     = 0  0 0< EGPRS PACKET CHANNEL REQUEST message content for 'PEO One Phase Access Request' */
    /* bit 7-6       = Access Cause : bit (2) */
    /* bit 5          = PEO Priority : bit (1) */
    /* bit 4-2       = Random Bits : bit (3) */
    /* bit 1          = EGPRS Capability : bit (1) */
    /* bit 0          = Spare : bit (1) > ;*/

    /* set bits 10-8 for EGPRS PEO one-phase access */
    access_ref |= 0x000;    

    /* set bits 7-6  for access cause */
    access_ref |= access_cause << 6;

    /* set bits 5  for PEO priority */
    access_ref |= peo_priority << 5;

    /* set a 3 bit random element at bits 4-2  */
    random = random & 0x07;
    access_ref |= random << 2;

    /* set bits 1  for EGPRS capability */
    access_ref |= egprs_capability << 1;

    /* set a 10th bit as spare */
    *random_ref_len = 3;
  }
...
===========================================================================*/
static boolean check_tlli(dword dl_ass_tlli )
{
  uint32 tlli;

  // 1st check is against normal TLLI
  tlli = rr_gmminfo_get_tlli();
...
===========================================================================*/
static boolean match_tlli(ie_descriptor_T *orig_current_IE  )
{
  ia_rest_octets_t ia_rest_octets;
  ...
  {
    MSG_GERAN_LOW_0_G("Trying to match TLLI, no Rest Octets");
    return(FALSE);
  }
  ...
  if ((ia_rest_octets.contents_type == packet_downlink_ass) &&
      (check_tlli(ia_rest_octets.contents.packet_downlink_ass.tlli)))
  {
    return(TRUE);
  }
...
)
{
  ...
  *decoding_ok_ptr = TRUE;
  ...
  cell_allocation_temp = scell_info_ptr->gsm.cell_channel_description;

  rr_move_zero_in_frequency_list(
     scell_info_ptr->gsm.cell_channel_description.channel_number,
     MIN(scell_info_ptr->gsm.cell_channel_description.no_of_entries,CA_MAX_LEN),
  ...
  if ((*decoding_ok_ptr && has_arfcn_zero) && (frequency_list_ptr->no_of_items > 1))
  {
    byte no_of_entries = frequency_list_ptr->no_of_items;
    /***************************************************************************/
    ...
    /****************************************************************************/
    if (frequency_list_ptr->channel[no_of_entries - 1].num == 0)
    {
       uint32 ch_index;
       ARFCN_T arfcn0 = frequency_list_ptr->channel[no_of_entries - 1];
       for (ch_index = no_of_entries - 1; ch_index > 0; --ch_index)
       {
         frequency_list_ptr->channel[ch_index] = frequency_list_ptr->channel[ch_index-1];
       }
  
       frequency_list_ptr->channel[0] = arfcn0;
    }
  }
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_tbf_est_data_ptr = rr_tbf_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_tbf_est_data_ptr);
...
void rr_gprs_two_part_assignment_failure( )
{
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr;
  ...
  {
    boolean in_mm_non_drx = rr_gprs_non_drx_mode_requested();
...
void rr_gprs_two_part_assignment_complete( )
{
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr;
...
)
{
  ...
  if (*waiting_for_2nd_msg == TRUE)
  {
    ...
    (void)rr_timer_cancel(TMULTIFRM);
    ...
    if (tbf_info_ptr->tma == 0)
    {
      ...
      *single_IA_msg = FALSE;
      *waiting_for_2nd_msg = FALSE;
      rr_gprs_two_part_assignment_complete();
    }
    else
    {
  ...
  else
  {
    ...
    else
    {
      ...
      *single_IA_msg = FALSE;
      ...
      (void)rr_timer_start(TMULTIFRM, (dword)TMULTIFRM_value);
    }
  }
...
{
  boolean bValid = TRUE;
  boolean egprs_capable_cell = rr_get_nv_egprs_enabled() && rr_cell_supports_egprs();
  ...
  )
{
  ...
  /*Before proceeding look for PS reject, PEO BCCH change or RCC change*/
  {
    ...
    /*Store the Rel 13 paramtre*/
    if ((temp_ia_rest_octets.r13_addition_flag)
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
         ||
        ((rr_nv_is_peo_testing_params_present()) || (rr_nv_is_ir_eab_testing_params_present()))
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/
       )
    {
      MSG_GERAN_HIGH_0_G("R13 params present in IA");
      ...
      {
        temp_ia_rest_octets.rcc = rr_nv_get_peo_rcc_from_efs();
        temp_ia_rest_octets.peo_bcch_change_mark = rr_nv_get_peo_bcch_change_mark_from_efs();
      }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/ 

      /*Act on  PEO BCCH CHNAGE MARK and RCC if required */
      if (rr_ue_in_peo_mode() && rr_process_rcc_or_bcch_change(temp_ia_rest_octets.rcc, temp_ia_rest_octets.peo_bcch_change_mark))
      {
        return EV_NO_EVENT;
      }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
       ...
       rr_ps_update_implicit_reject(temp_ia_rest_octets.implicit_ps_reject);
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
       if (rr_nv_is_ir_eab_testing_params_present())
       {
         temp_ia_rest_octets.implicit_ps_reject = rr_nv_get_implicit_reject_during_rach_from_efs();
       }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

      if (temp_ia_rest_octets.implicit_ps_reject)
      {    
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
      MSG_GERAN_HIGH_2_G("PEO_BCCH_CHNAGE_MARK = %d, RCC= %d",temp_ia_rest_octets.peo_bcch_change_mark,temp_ia_rest_octets.rcc);     
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/

      MSG_GERAN_HIGH_1_G(" IR = %d",temp_ia_rest_octets.implicit_ps_reject);

    }
    else
    {
      MSG_GERAN_HIGH_0_G("R13 params not present in IA");
    }
    memset(&temp_ia_rest_octets, 0, sizeof(temp_ia_rest_octets));
  }
  ...
  if (tbf_info.downlink == 0)
  {
   ...
   if (rr_ce_egprs_access_valid())
   {
     ...
     if ( extended_ra_found )
     {
       ...
       {
         MSG_GERAN_HIGH_4_G("IA with acc_refe= %d, Sent acc ref[0]= %d, acc ref[1]= %d, acc ref[2]= %d", 
                                 extended_ra, 
                                 request_references_array[0].egprs_access_reference,
                                 request_references_array[1].egprs_access_reference,
                                 request_references_array[2].egprs_access_reference);
       }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
  ...
  else
  {
    ...
    /*******************************************/
    if (!match_against_tlli(waiting_for_2nd_msg, current_IE))
    {
      MSG_GERAN_MED_1_G("Ignored IA msg - no match (dnl=%d)", tbf_info.downlink);
      return EV_NO_EVENT;
    }
    else
    {
  ...
  rr_classmark_set_irat_ho_info_compressed(compressed_irat_ho_info);
 ...
 /*****************************/
  if (((*waiting_for_2nd_msg) && (tbf_info.tma == 1)) || single_IA_msg)
  {
    ...
    if ((current_IE->start_IE != NULL) && (decoding_ok))
    {
      ...
      if (rr_gprs_validate_immediate_assignment(&temp_ia_rest_octets) == FALSE)
      {
  ...
  if ((*waiting_for_2nd_msg == FALSE) && (decoding_ok))
  {
    ...
    {
      MSG_GERAN_LOW_0_G("Clearing IA EGPRS RACH references");
    ...
    {
      MSG_GERAN_LOW_0_G("Clearing IA RACH references");
      rr_reset_rach_references();
    ...
    else if (rr_tbf_est_data_ptr->data_ptr->ia_rest_octets.contents_type == packet_downlink_ass) /* Downlink */
    {
      ...
      else
      {
        ...
        */
        ...
        {
          if (rr_mac_dl_tbf_est_is_allowed())
          {
        ...
        {
          if ((grr_is_idle_camped() || grr_tbf_est_in_progress())
              && !rr_resel_reselection_is_pending())
          {
            /**
...
{
#if defined(FEATURE_GERAN_EAB) || defined (FEATURE_GERAN_IMPLICIT_PS_REJECT)
  boolean eab_ir_access_barred = FALSE;
#endif /* FEATURE_GERAN_EAB || FEATURE_GERAN_IMPLICIT_PS_REJECT */
  ...
  */
...
#endif /* FEATURE_EDGE_CAPABILITY_NOT_SUPPORTED */
  {
    ...
    access_result_T access_result = rr_check_access_class_for_raching_for_tbf();
  ...
  /* Extended Access Barring check */
  if (rr_is_extended_access_barred(mac_grr_ul_tbf_req->channel_req_info.pkt_est_cause))
  {
    MSG_GERAN_HIGH_0_G("Extended Access Barring check failed, block PS");
    eab_ir_access_barred = TRUE;
    event_out = EV_GPRS_ACCESS_NOT_POSSIBLE;
    /* Inform a default wait_time of 30 secs. 
     * However, if there is change in SI21 we will again send a indication with wait_time=0 */
    rr_send_ps_implicit_reject_ind(GRR_TIMEOUT_30_SECONDS);
  }
#endif /* FEATURE_GERAN_EAB */

#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
  if ((rr_ps_is_low_priority()) && 
      (rr_ps_is_implicit_reject_rcvd()) && 
      (RR_PKT_EST_CAUSE_PAGE_RESP != mac_grr_ul_tbf_req->channel_req_info.pkt_est_cause)
     )
  {
    ...
    MSG_GERAN_HIGH_0_G("Implicit PS reject by N/W , block PS");
    eab_ir_access_barred = TRUE;
    event_out = EV_GPRS_ACCESS_NOT_POSSIBLE; 
  ...
  {
#if defined(FEATURE_GERAN_EAB) || defined(FEATURE_GERAN_IMPLICIT_PS_REJECT)
    /* Disable PS access. No need to delete any PDU if access is barred due to EAB or IR.
     * PDU deletion will take place based on RLC stale timer. */
    if (eab_ir_access_barred)
    {
      ...
      {
        /* For non-data case delete curr PDU */
...
#if defined(FEATURE_GERAN_EAB) || defined(FEATURE_GERAN_IMPLICIT_PS_REJECT)
    /* No need to enable PS access if access is barred due to EAB/IR */
    if (eab_ir_access_barred == FALSE)
#endif /* FEATURE_GERAN_EAB || FEATURE_GERAN_IMPLICIT_PS_REJECT */
    {
...
===========================================================================*/
static rr_event_T abort_tbf_est( )
{
  rr_event_T rr_event;
  rr_tbf_est_static_data_t *rr_tbf_est_data_ptr;
...
)
{
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION        
       ( event_out == EV_BCCH_CHANGE_DETECTED) ||
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/       
  ...
  {
    rr_ce_reset_egprs_access();

    // Check if expecting 2nd part of IA
    if (rr_timer_is_running(TMULTIFRM) == FALSE)
    {
...
void grr_start_t3146( )
{
  if (!rr_timer_is_running(T3146))
  {
    gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
    ...
    (void)rr_timer_start(T3146, (dword)(t3146_timeout_ms));
  }
...
boolean grr_return_tbf_data_structure_pointer( )
{
