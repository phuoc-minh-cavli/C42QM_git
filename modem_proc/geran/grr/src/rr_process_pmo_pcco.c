...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_process_pmo_pcco.c#4 $ */
...
static pmo_pcco_store_t  pmo_pcco_store;
...
/**
  ...
  @return
*/
static pmo_pcco_store_t *rr_process_pmo_pcco_get_data_ptr( void )
{
  return &pmo_pcco_store;
...
)
{
  ...
  pmo_pcco_store_t *rr_process_pmo_pcco_data_ptr = rr_process_pmo_pcco_get_data_ptr();
...
pmo_pcco_data_t *rr_psi_get_pmo_store_ptr( )
{
  // Obtain a pointer to the module data
  pmo_pcco_store_t *rr_process_pmo_pcco_data_ptr = rr_process_pmo_pcco_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_process_pmo_pcco_data_ptr);
...
*/
static void rr_psi_clear_pmo_data( )
{
  uint32 i;
  pmo_pcco_store_t *pmo_pcco_store_ptr = rr_process_pmo_pcco_get_data_ptr();
  pmo_pcco_data_t *pmo_store_ptr = rr_process_pmo_pcco_get_pmo_data_ptr();
...
)
{
  ...
  if ( num_of_added_ncells > 0 )
  {
    ...
    if ((bHCS_scell == FALSE) && (bHCS_ncell == TRUE) && rr_gprs_camped_on_ccch())
    {
...
)
{
  ...
  {
    MSG_GERAN_LOW_0("NCC_PERMITTED is unhandled!");
  }
...
)
{
  ...
  pmo_pcco_data_t *pcco_store_ptr = rr_process_pmo_pcco_get_pcco_data_ptr();
...
boolean rr_get_pmo_priority_and_eutran_params_valid( )
{
  ...
  if (pmo_pcco_store_ptr->pmo_store_ptr != NULL)
  {
    ...
    for (i = 0; i < MAX_PMO_INSTANCES; ++i)
    {
      if (pmo_pcco_store_ptr->pmo_store_ptr->priority_and_eutran_params.utran_priority_params_valid_bitmap & (1 << i))
      {
        ...
        if (src == NULL)
        {
          MSG_GERAN_LOW_1_G("src == NULL for index %d", i);
          continue;
        }
...
)
{
  ...
  pmo_pcco_store_ptr = rr_process_pmo_pcco_get_data_ptr();
  //RR_NULL_CHECK_FATAL(pmo_pcco_store_ptr);
        ...
        if (pcco->cell.gsm.nc_meas_params.nc_frequency_list.num_of_added_freqs > 0)
        {
          ...
          for (i = 0; i < pcco->cell.gsm.nc_meas_params.nc_frequency_list.num_of_added_freqs; i++)
          {
            if (i < PMO_PCCO_MAX_NUM_OF_ADDED_NCELLS)
            {
              ...
              dst->bcch_arfcn.band = (sys_band_T) rr_get_band_dyn(
                src->frequency   // arfcn
              );
...
)
{
        ...
        if (pmo->nc_meas_params->nc_frequency_list.num_of_added_freqs > 0)
        {
          ...
          for (i = 0; i < pmo->nc_meas_params->nc_frequency_list.num_of_added_freqs; i++)
          {
            if (j < PMO_PCCO_MAX_NUM_OF_ADDED_NCELLS)
            {
              ...
              dst->bcch_arfcn.band = (sys_band_T) rr_get_band_dyn(src->frequency);
              CONVERT_TO_BSIC_T(dst->bsic, src->bsic);
              if (src->cell_selection_params_valid)
              {
                dst->params_not_present_mask = decode_pmo_pcco_cell_sel_params(
  ...
  if (pmo->priority_and_eutran_params_valid)
  {
    ...
    if (pmo->priority_and_eutran_params.utran_priority_params != NULL)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("Memory allocation failure, unable to store UTRAN Priority Parameters");
      }
    }
    ...
    if (pmo->priority_and_eutran_params.eutran_params != NULL)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("Memory allocation failure, unable to store E-UTRAN Priority Parameters");
      }
    }
  }
...
/*CR550108*/
boolean  rr_is_pmo_nc_frequency_list_valid( void )
{
  pmo_pcco_store_t *pmo_pcco_store_ptr = rr_process_pmo_pcco_get_data_ptr();
  //RR_NULL_CHECK_FATAL(pmo_pcco_store_ptr);
...
pmo_pcco_data_t *rr_get_pmo_data( void )
{
  pmo_pcco_store_t *pmo_pcco_store_ptr = rr_process_pmo_pcco_get_data_ptr();
  //RR_NULL_CHECK_FATAL(pmo_pcco_store_ptr);
...
void rr_psi_clear_pmo_data_going_inactive( )
{
  pmo_pcco_store_t *pmo_pcco_store_ptr = rr_process_pmo_pcco_get_data_ptr();
  pmo_pcco_store_ptr->pmo_nc_freq_list_valid = FALSE;
  if (pmo_pcco_store_ptr->pmo_nc_freq_list != NULL)
