...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_irat_overlap_detect.c#2 $ */
...
static void rr_construct_lte_earfcn_to_phys_freq_map(uint8 earfcn_count, rr_l1_eutran_frequency_t* earfcn_list  )
{
  uint8 index;
  uint16 phy_lte_freq;
...
static void rr_flush_earfcn_phys_freq_map(  )
{  
...
void rr_detect_and_remove_overlap_earfcn(mph_wcdma_cell_update_list_req_T* idle_msg_ptr, mph_dedicated_wcdma_cell_update_list_req_T* ded_msg_ptr   )
{
  uint8 index;
  uint8 inner_index;
  ...
  rr_construct_lte_earfcn_to_phys_freq_map( eutran_frequency_count, eutran_freq_list );
  ...
  if ( overlap_detected )
  {
   ...
   /* Now clean up both the lists before sending to GL1 */
    ...
    rr_tidy_up_invalid_earfcn_from_wcdma_cell_update_list(idle_msg_ptr, ded_msg_ptr);
  }

  /* Job Done, Flush the map created at the entry of this function */
  rr_flush_earfcn_phys_freq_map();
}
...
static void rr_invalidate_wcdma_cells_from_cell_list(wcdma_cell_list_T* wcdma_cell_list, uint32 uarfcn   )
{
  uint8 index;
...
static void rr_construct_wcdma_uarfcn_to_phys_freq_map(wcdma_reselection_freq_list_T* wcdma_reselection_freq_list  )
{
  ...
  for ( index = 0; ((index < wcdma_reselection_freq_list->num_of_frequencies) && (index < MAX_RR_MPH_FREQ_UPDATE_LIST)); index++ )
  {
    if ( E_SUCCESS == wcdma_derive_freq_from_uarfcn(wcdma_reselection_freq_list->uarfcn[index], scell_info_ptr->gsm.location_area_identification.plmn_id,  &wcdma_phy_freq) )
    {
      ...
      MSG_GERAN_HIGH_2_G("rr_construct_wcdma_uarfcn_to_phys_freq_map WCDMA Freq %d, PHYSICAL FREQ= %d",
                        (uint32) rr_wcdma_freq_info_list.uarfcn[index], wcdma_phy_freq );       
    }
    else
    {
...
static void rr_flush_uarfcn_phys_freq_map(  )
{  
 ...
 */
static void rr_tidy_up_invalid_uarfcn_from_wcdma_cell_list ( wcdma_cell_list_T* wcdma_cell_list_ptr  )
{
  uint8 index, real_index;
...
static void rr_tidy_up_invalid_uarfcn_from_wcdma_cell_update_list_idle( mph_wcdma_cell_update_list_req_T* idle_msg_ptr  )
{
  ...
  rr_tidy_up_invalid_uarfcn_from_wcdma_cell_list(&idle_msg_ptr->wcdma_cell_list );
}
...
static void rr_construct_wcdma_reselection_freq_list( wcdma_reselection_freq_list_T* wcdma_reselection_freq_list_ptr, wcdma_cell_list_T* wcdma_cell_list   )
{
...
void rr_detect_and_remove_overlap_uarfcn_in_idle( mph_wcdma_cell_update_list_req_T* idle_msg_ptr )
{
  uint8 index;
  uint8 inner_index;
  ...
  rr_construct_wcdma_uarfcn_to_phys_freq_map( wcdma_reselection_freq_list_ptr );
      ...
      for (inner_index = 0; inner_index < index; inner_index++ )
      {
        if ( (utra_phy_freq == rr_wcdma_freq_info_list.wcdma_phy_freq[inner_index]) && (wcdma_reselection_freq_list_ptr->uarfcn[inner_index] != (word)INVALID_UARFCN) )
        {
          ...
          if ( wcdma_cell_list_ptr->num_of_cells > 0 )
          {
            ...
            rr_invalidate_wcdma_cells_from_cell_list( wcdma_cell_list_ptr, overlap_uarfcn );        
          }
...
void rr_detect_and_remove_overlap_uarfcn_in_dedicated( mph_dedicated_wcdma_cell_update_list_req_T* ded_msg_ptr )
{
  uint8 index;
  uint8 inner_index;
  uint32 uarfcn, overlap_uarfcn;
  ...
  rr_construct_wcdma_reselection_freq_list(wcdma_reselection_freq_list_ptr, &ded_msg_ptr->wcdma_cell_list );
  ...
  if ( overlap_detected)
  {
    rr_tidy_up_invalid_uarfcn_from_wcdma_cell_list(wcdma_cell_list_ptr);
  }
