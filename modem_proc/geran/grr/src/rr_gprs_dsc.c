...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_dsc.c#4 $
$DateTime: 2018/10/11 08:21:00 $
$Author: pwbldsvc $
...
/*===================================================================
            LOCAL DATA
=====================================================================*/
static rr_gprs_dsc_data_t rr_gprs_dsc_data;
 ...
 */
static rr_gprs_dsc_data_t *rr_gprs_dsc_get_data_ptr( void )
{
  return &rr_gprs_dsc_data;
}
 ...
 *
 ****************************************************************************/
int rr_gprs_get_max_dsc_value(const boolean use_scaling )
{
  ...
  if (scell_info_ptr && scell_info_ptr->gsm.valid_data_flag)
  {
    ...
    if (bs_pa_mfrms == 0)
    {
      ...
      MSG_GERAN_LOW_0_G("DSC: default to BS_PA_MFRMS=2");
    }
  }
  else
  {
  ...
  if (use_scaling)
  {
    extended_dsc = rr_idle_drx_get_extended_dsc();
  }
#else
  NOTUSED(use_scaling);
#endif /* FEATURE_IDLE_DRX_SCALING */
  ...
  if (extended_dsc != 0)
  {
    MSG_GERAN_LOW_0_G("Using scaled DSC");
    return extended_dsc;
  }
  else
  {
    MSG_GERAN_LOW_0_G("Using normal DSC");
 ...
 *
 ****************************************************************************/
int rr_gprs_return_gprs_dsc_counter_value( )
{
  // Obtain a pointer to the module data
  rr_gprs_dsc_data_t *rr_gprs_dsc_data_ptr = rr_gprs_dsc_get_data_ptr();
 ...
 *
 ****************************************************************************/
void rr_gprs_set_gprs_dsc_counter_value(const int dsc )
{
  // Obtain a pointer to the module data
  rr_gprs_dsc_data_t *rr_gprs_dsc_data_ptr = rr_gprs_dsc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_dsc_data_ptr);
 ...
 *
 ****************************************************************************/
static boolean rr_gprs_serving_cell_resync_allowed(const l2_channel_type_T l2_channel_type )
{
 ...
 *
 ****************************************************************************/
static rr_l1_epd_preference_t rr_gprs_epd_preference( )
{
  if (rr_ms_sys_info_refresh_blocked())
  {
    if (rr_timer_has_expired(RR_EPD_TIMER))
    {
      /* timer has expired at some point in the past and not been cleared yet */
      (void)rr_timer_cancel(RR_EPD_TIMER);
      return(RR_L1_EPD_OFF_PREFERRED);
    }

    if (FALSE == rr_timer_is_running(RR_EPD_TIMER))
    {
      if (rr_timer_start(RR_EPD_TIMER, RR_EPD_TIMER_DURATION))
      {
 ...
 *
 ****************************************************************************/
rr_event_T rr_gprs_check_dsc_in_idle(const gl1_block_quality_t block_quality, const l2_channel_type_T l2_channel_type )
{
  ...
  /* Obtain a pointer to the module data */
  rr_gprs_dsc_data_ptr = rr_gprs_dsc_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_dsc_data_ptr);

  /* Retrieve the maximum value of the counter for the serving cell */
  maximum_value = rr_gprs_get_max_dsc_value(TRUE);
  ...
  {
    MSG_GERAN_LOW_2_G("DSC %d is greater than max %d, reinitialise now", rr_gprs_dsc_data_ptr->gprs_dsc_counter, maximum_value);
  ...
  switch(block_quality)
  {
    case GL1_BLOCK_QUALITY_GOOD:
    {
      ...
      if (rr_gprs_dsc_data_ptr->l1_updated)
      {
        MSG_GERAN_HIGH_0_G("Update L1 to switch back to regular eDRX");
        rr_gprs_dsc_data_ptr->l1_updated = FALSE;
        //update Gl1 to move to lowest eRDX
        rr_edrx_update_l1_params(RR_DSC_INCREMENT) ;       
      }
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
      rr_rmc_update_ncell_refresh_time_duration();
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
      ...
      rr_ms_reset_block_quality();
      epd_pref = rr_gprs_epd_preference();
    ...
    case GL1_BLOCK_QUALITY_BAD:
    {
      ...
      if (rr_ue_in_peo_mode())
      {
        /* decrement the DSC by 1 for each bad block (xCCH failure) for PEO */
        rr_gprs_dsc_data_ptr->gprs_dsc_counter = rr_gprs_dsc_data_ptr->gprs_dsc_counter - 1;

        if (rr_gprs_dsc_data_ptr->l1_updated == FALSE)
        {
          MSG_GERAN_HIGH_0_G("Update L1 to use lowest eDRX");         
          rr_gprs_dsc_data_ptr->l1_updated = TRUE;
          //update Gl1 to move to lowest eRDX
          rr_edrx_update_l1_params(RR_DSC_DECREMENT );
        }
        rr_rmc_update_ncell_refresh_time_duration();
      ...
      if (rr_get_edrx_indicator())
      {
        ...
        if (rr_gprs_dsc_data_ptr->l1_updated == FALSE)
        {
          MSG_GERAN_HIGH_0_G("Update L1 to use lowest eDRX for EDRX only case");         
          rr_gprs_dsc_data_ptr->l1_updated = TRUE;
          //update Gl1 to move to lowest eRDX
          rr_edrx_update_l1_params(RR_DSC_DECREMENT );
        }
      ...
      if (rr_gprs_dsc_data_ptr->denial_period_ms > rr_nv_get_scell_resync_time_millisecs())
      {
        /* set flag to generate the appropriate output event below */
        resync_serving_cell = rr_gprs_serving_cell_resync_allowed(l2_channel_type);
      }
    ...
    case GL1_BLOCK_QUALITY_UNKNOWN:
    {
      ...
      if (GL1_BLOCK_QUALITY_BAD == rr_ms_adjust_block_quality(l2_channel_type))
      {
   ...
   default:
     MSG_GERAN_LOW_2_G("Unexpected BQ value %d, channel %d", (int)block_quality, (int)l2_channel_type);
     break;
  }

  /* always log the current value of the DSC */
  rr_log_dsc_counter(rr_gprs_dsc_data_ptr->gprs_dsc_counter);
  ...
  if (maximum_value)
  {
    rr_idle_drx_check_dsc(maximum_value, rr_gprs_dsc_data_ptr->gprs_dsc_counter);
  }
#endif /* FEATURE_IDLE_DRX_SCALING */
  ...
  else if (maximum_value)
  {
    ...
    rr_send_mph_dsc_threshold_ind(percent_dsc, l2_channel_type, epd_pref);
  }
#endif /* !PERLUTF */
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
void rr_gprs_initialise_dsc( )
{
  ...
  rr_gprs_dsc_data_ptr->gprs_dsc_counter = rr_gprs_get_max_dsc_value(TRUE);
  rr_gprs_dsc_data_ptr->ms_blk_watermark = 0;
  rr_gprs_dsc_data_ptr->denial_period_ms = 0;
  
#if defined(FEATURE_GERAN_POWER_EFFICIENT_OPERATION) || defined (FEATURE_GERAN_EXTENDED_DRX)
  rr_gprs_dsc_data_ptr->l1_updated       = FALSE;
  MSG_GERAN_HIGH_1_G("DSC:= %d ",(int)rr_gprs_dsc_data_ptr->gprs_dsc_counter);  
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION || FEATURE_GERAN_EXTENDED_DRX*/
 ...
 *
 ****************************************************************************/
boolean rr_gprs_is_dsc_below_max_value( )
{
  rr_gprs_dsc_data_t *rr_gprs_dsc_data_ptr;
  ...
  return(rr_gprs_dsc_data_ptr->gprs_dsc_counter < rr_gprs_get_max_dsc_value(TRUE));
}

#if defined(FEATURE_GERAN_POWER_EFFICIENT_OPERATION) || defined (FEATURE_GERAN_EXTENDED_DRX)

/****************************************************************************
 *   Function name:  rr_gprs_is_l1_updated_with_lowest_edrx
 *   --------------
 *
 *   Description: This function is used to detect if in PEO or EDRX mode , l1 has been updated with lowest DSC or not
 *
 *   Parameters: NONE
 *   -----------
 *
 ...
 *
 ****************************************************************************/
boolean rr_gprs_is_l1_updated_with_lowest_edrx( )
{
  rr_gprs_dsc_data_t *rr_gprs_dsc_data_ptr;
  ...
  return(rr_gprs_dsc_data_ptr->l1_updated);
}

/****************************************************************************
 *   Function name:  rr_gprs_set_l1_updated_with_lowest_edrx
 *   --------------
 *
 *   Description: This function is used to set l1_updated flag so that it can be rused to reset Gl1 back to normal edrx
 *
 *   Parameters: NONE
 *   -----------
 *
 ...
 *
 ****************************************************************************/
void rr_gprs_set_l1_updated_with_lowest_edrx( )
{
  rr_gprs_dsc_data_t *rr_gprs_dsc_data_ptr;
  ...
  //RR_NULL_CHECK_FATAL(rr_gprs_dsc_data_ptr);
  rr_gprs_dsc_data_ptr->l1_updated = TRUE;
  MSG_GERAN_HIGH_0_G("Set update l1 flag to restore negotiated edrx ");
}
