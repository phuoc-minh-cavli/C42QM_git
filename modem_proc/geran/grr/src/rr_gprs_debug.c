...
*** TITLE
***
***  $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_debug.c#9 $
***
*** DESCRIPTION
***
...
static rr_gprs_debug_data_t  rr_gprs_debug_data;
 ...
 */
static rr_gprs_debug_data_t *rr_gprs_debug_get_data_ptr( void )
{
    return(rr_static_gprs_debug_get_data_ptr());
}
...
)
{
  ...
  uint32 timestamp_fn = GSTMR_GET_FN_GERAN();
  timetick_type timestamp_clk = timetick_get();

  // Obtain a pointer to the module data
  rr_gprs_debug_data_t *rr_gprs_debug_data_ptr = rr_gprs_debug_get_data_ptr();
...
)
{
  ...
  uint32 timestamp = GSTMR_GET_FN_GERAN();
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_gprs_debug_data_ptr = rr_gprs_debug_get_data_ptr();
  ...
  entry->timestamp = GSTMR_GET_FN_GERAN();
  entry->sm_id = sm_id;
  entry->state = state;
  entry->event = event;
      ...
      case MS_MM_RR:
      {
        switch (message_id)
        {
          ...
          case RR_MTC_PSM_EDRX_PARAMS_UPDATE_IND:
            s= "RR_MTC_PSM_EDRX_PARAMS_UPDATE_IND";
          break;          
          ...
          case RR_GMM_READY_TIMER_STATUS_IND:
            s= "RR_GMM_READY_TIMER_STATUS_IND";
          break;
      ...
      case MS_RR_L1:
      {
        switch (message_id)
        {
         ...
         case MPH_STOP_GSM_MODE_CNF:
          ...
          case MPH_SERVING_MEAS_REQ:
            s = "MPH_SERVING_MEAS_REQ";
            break;

          case MPH_BSIC_CHANGE_IND:
            s = "MPH_BSIC_CHANGE_IND";
            break;
          ...
          case MPH_PROC_IND:
           ...
           case MPH_SIMULATE_R13_PARAM_FROM_DIAG_IND:
             s="MPH_SIMULATE_R13_PARAM_FROM_DIAG_IND";
            break;
      ...
      case MS_RR_RR_EXT:
      {
        if (msg_ptr != NULL)
        {
          switch (msg_ptr->rr.header.rr_message_set)
          {
            ...
            {
              s = rr_rmc_imsg_name((rr_resel_rmc_imsg_e) message_id);
              break;
            }            
            case RR_RESEL_RMC_OMSG:
            {
              s = rr_rmc_omsg_name((rr_resel_rmc_omsg_e) message_id);
              break;
            }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
    ...
    switch(msg_level)
    {
      case MSG_LEGACY_LOW:
        MSG_SPRINTF_2(
          MSG_SSID_DFLT,
          MSG_LEGACY_LOW,
          "gs1:Timer %s expired in state %s",
          message_name,
          state_name
        );
        break;
...
)
{
  ...
  rr_cmd_bdy_type message_algn __attribute__((aligned(4)));
  rr_cmd_bdy_type *message_tmp = &message_algn;

  if (message_ptr != NULL) {
    message_algn.message_header.message_set = message_ptr->message_set;
    message_algn.message_header.message_id = message_ptr->message_id;
    message_algn.message_header.message_len_lsb = message_ptr->message_len_lsb;
    message_algn.message_header.message_len_msb = message_ptr->message_len_msb;
    message_algn.rr.header.rr_message_set = ((rr_cmd_bdy_type*)((void *)message_ptr))->rr.header.rr_message_set;
    message_algn.rr.grr.imsg.header.imh.message_id = ((rr_cmd_bdy_type*)((void *)message_ptr))->rr.grr.imsg.header.imh.message_id;
  }
  ...
  else
  {
    message_name = rr_gprs_message_name(
      (rr_cmd_bdy_type *) ((void *) message_tmp),
      message_set,
      message_id,
      &msg_level
    );
  ...
  switch(msg_level)
  {
    case MSG_LEGACY_LOW:
      MSG_SPRINTF_2(
        MSG_SSID_DFLT,
        MSG_MASK_5,
        "gs1:OMsg: %s sent to %s",
        message_name,
        destination
      );

      if ((message_set == MS_RR_RR) && (message_ptr != NULL))
      {
        MSG_3(
          MSG_SSID_DFLT,
          MSG_LEGACY_LOW,
          "gs1:OMsg: (%d,%d,%d)",
          ((rr_cmd_bdy_type *) ((void *) message_tmp))->message_header.message_set,
          ((rr_cmd_bdy_type *) ((void *) message_tmp))->rr.header.rr_message_set,
          ((rr_cmd_bdy_type *) ((void *) message_tmp))->message_header.message_id
        );
      }
      else
      {
 ...
 * \param msg_id (in)
 */
void rr_debug_log_rr_mac_omsg(grr_mac_sig_id_t msg_id  )
{
 ...
 * 
 */
void rr_debug_log_panic_reset( void )
{
  uint32 timestamp = GSTMR_GET_FN_GERAN();
