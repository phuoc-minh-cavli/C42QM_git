...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_mode.c#4 $ */
...
static rr_mode_data_t rr_mode_data = INITIAL_VALUE(_RR_MODE_DATA_INIT);
 ...
 */
rr_mode_data_t *rr_mode_get_data_ptr( void )
{
  return &rr_mode_data;
}
...
===========================================================================*/
boolean rr_mode_gsm_active_or_starting( void )
{
  rr_mode_data_t *rr_mode_data_ptr = rr_mode_get_data_ptr();
...
)
{
  ...
  {
    /* start a guard timer */
    if (rr_timer_start(RR_DUAL_SIM_INACTIVATE_TIMER, GRR_TIMEOUT_1_SECOND * RR_DUAL_SIM_INACTIVATE_SECONDS))
    {
...
void rr_mode_deactivation_guard_expiry( )
{
...
rr_l1_gsm_stop_reason_T rr_mode_get_l1_stop_reason( )
{
...
void rr_mode_set_l1_stop_reason(rr_l1_gsm_stop_reason_T l1_stop_reason  )
{
  // Obtain a pointer to the module data
  rr_mode_data_t *rr_mode_data_ptr = rr_mode_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_mode_data_ptr);
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_mode_data_ptr = rr_mode_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_mode_data_ptr);
  ...
  switch (event)
  {
    ...
    {
      rr_timer_cancel(RR_L1_GUARD_TIMER);
      rr_mode_data_ptr->state = RR_MODE_INACTIVE;
      rr_mode_data_ptr->pending_request = EV_NO_EVENT;
    ...
    switch (rr_mode_data_ptr->state)
    {
      case RR_MODE_INACTIVE:
      {
        switch (event)
        {
          ...
          {
            rr_send_mph_start_gsm_mode_req(TRUE, FALSE);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;
            break;
          }
          ...
          {
            rr_send_mph_start_gsm_mode_req(FALSE, TRUE);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;
            break;
          }
          ...
          {
            uint32 timer_value = rr_plc_get_wtog_search_timer_value();
            rr_send_mph_start_gsm_mode_for_bplmn_req(timer_value);
            rr_mode_data_ptr->stop_reason = RR_L1_GSM_STOP_REASON_UNDEFINED;
            rr_mode_data_ptr->state = RR_MODE_GOING_ACTIVE;
            rr_timer_start(RR_L1_GUARD_TIMER, RR_L1_GUARD_TIMEOUT);
            break;
          ...
          case EV_DEACTIVATE_GSM_MODE:
          {
            ...
            {
              if (rr_mode_data_ptr->stop_reason != rr_l1_gsm_stop_reason_get())
              {
      ...
      case RR_MODE_ACTIVE:
      {
        switch (event)
        {
          ...
          case EV_DEACTIVATE_GSM_MODE:
          {
            ...
            rr_mode_going_inactive(TRUE);
#endif /* FEATURE_DUAL_SIM */
            rr_mode_data_ptr->stop_reason = rr_l1_gsm_stop_reason_get();
            ...
            rr_timer_start(RR_L1_GUARD_TIMER, RR_L1_GUARD_TIMEOUT);
#ifdef FEATURE_IDLE_DRX_SCALING
            rr_idle_drx_control(EV_DEACTIVATE_GSM_MODE, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
            break;
          }
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_STOP_GSM_MODE_CNF:
                  {
                    ...
                    (void)rr_timer_cancel(RR_L1_GUARD_TIMER);
