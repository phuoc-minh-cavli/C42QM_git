/*============================================================================
 $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_classmark.c#4 $
...
#define RR_CLASSMARK_WCDMA_INACTIVE    FALSE
#define RR_CLASSMARK_LTE_IS_ACTIVE     TRUE
...
static rr_classmark_data_t rr_classmark_data;
 ...
 */
rr_classmark_data_t *rr_classmark_get_data_ptr( void )
{

  return &rr_classmark_data;
}
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS*/
...
)
{
...
#endif /* FEATURE_SGLTE */
  {
    wcdma_support = rr_is_wcdma_enabled();
  }
#else
  wcdma_support = FALSE;
#endif /* FEATURE_WCDMA */
  ...
  cm_params.wcdma_in_use    = RR_CLASSMARK_WCDMA_INACTIVE;
      ...
      (cm_type == RR_CLASSMARK_2_AND_3))
  {
    ...
    {
      MSG_GERAN_LOW_0_G("Unable to output Classmark 2 to NULL ptr");
    }
  }
...
/*===========================================================================
 ...
 ===========================================================================*/
static void rr_send_utran_classmark_change( void )
{
  byte index = 0;
  byte c_index;
  ...
  if (rr_is_utran_tdd_enabled() &&
      rr_is_wcdma_enabled() )
  {
    send_utran_tdd_classmark = rr_is_utran_tdd_enabled_in_nw_plmn();
  ...
  rr_classmark_data_ptr = rr_classmark_get_data_ptr();
...
boolean rr_classmark_get_irat_ho_info_compressed( )
{
  rr_classmark_data_t *rr_classmark_data_ptr;
...
)
{
  ...
  /*****************************************************/
  current_IE = rr_get_first_ie_of_message(CLASSMARK_ENQUIRY);
...
)
{
  ...
  case RR_CLASSMARK_CLIENT_ECSC:
    if (ES_IND == CECS_IMPLEMENTED)
    {
      ...
      {
        if (rr_ce_access_reason() == (byte)MM_RR_IMSI_DETACH)
        {
  ...
  case RR_CLASSMARK_CLIENT_NETW:
    ...
    rr_decode_classmark_enquiry(&send_geran, &send_utran, &send_cdma2k);
    break;
  ...
  if (send_geran == TRUE)
  {
    rr_send_geran_classmark_change();
  }

#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
  /* UTRAN Classmark is sent if WCDMA support is currently enabled. */
  if ((send_utran == TRUE) && ( rr_is_wcdma_enabled() || rr_is_utran_tdd_enabled() ))
  {
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
    rr_send_utran_classmark_change();
#else
  ...
  if (send_cdma2k == TRUE)
  {
    ...
    MSG_GERAN_LOW_0_G("CDMA2000 Classmark Change will not be sent");
  }
...
)
{
  ...
  /* "wcdma_in_use" flag is interpreted differently for LTE use */
  cm_params.wcdma_in_use    = RR_CLASSMARK_LTE_IS_ACTIVE;
...
)
{
  if (req_ptr != NULL)
  {
     ...
     */
    rr_send_gmm_gprs_ms_ra_cap_req();
  }
...
void rr_send_lte_ps_capabilities_rsp(rr_gmm_gprs_ms_ra_cap_ie_ind_T *ind_ptr )
{
  ...
  rsp.geran_dtm_is_supported  = rr_get_nv_dtm_enable());
