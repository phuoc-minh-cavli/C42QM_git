...
/*===========================================================================
  ...
  $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_meas_rpt_control.c#6 $
...
/*===========================================================================
                         LOCAL FUNCTIONS
===========================================================================*/
static rr_event_T rr_gprs_read_meas_rpt_sysinfo( );
static rr_event_T rr_gprs_process_nc_measurement_params( );
static boolean rr_gprs_format_meas_report(pmr_send_t  );
static void rr_gprs_start_nc_measurement_process(boolean  );
static void rr_gprs_store_nc_measurements(mph_nc_measurement_ind_t *nc_measurements );
static void rr_gprs_store_serving_measurements(mph_serving_meas_ind_t *serving_measurements );
static void rr_gprs_stop_nc_measurement_process( );
static rr_event_T rr_gprs_change_nc_measurement_process(cause_t );
static t3158_action_t rr_gprs_start_t3158(boolean mac_status_changed );
static void rr_gprs_meas_rpt_control_force_t3158_restart( );
static void rr_gprs_load_best_six_for_each_band(nc_measurement_data_t * );
...
static void rr_gprs_store_3g_measurements_idle(mph_surround_wcdma_idle_meas_ind_T * meas  );
static void rr_gprs_store_3g_measurements_xfer(mph_surround_wcdma_xfer_meas_ind_T * meas );
static void rr_gprs_3g_measurements_sort( );
...
#ifdef FEATURE_GSM_TDS
static void rr_gprs_tdd_3g_measurements_sort( );
...
/*===========================================================================
                         LOCAL VARIABLES
===========================================================================*/
static rr_gprs_meas_rpt_control_data_t  rr_gprs_meas_rpt_control_data;
 ...
 */
 static rr_gprs_meas_rpt_control_data_t *rr_gprs_meas_rpt_control_get_data_ptr( void )
{
  return &rr_gprs_meas_rpt_control_data;
}
...
static void rr_gprs_meas_rpt_control_reset_measurements( void )
{
  // Obtain a pointer to the module data
  rr_gprs_meas_rpt_control_data_t *rr_gprs_meas_rpt_control_data_ptr;
  rr_gprs_meas_rpt_control_data_ptr = rr_gprs_meas_rpt_control_get_data_ptr();
...
static boolean grr_nc_params_set_by_pmo_pcco( )
{
...
byte rr_gprs_get_network_control_order( )
{
...
*/
static void rr_network_control_order_reset( )
{
  gprs_scell_info_t * scell = rr_gprs_get_scell_info();

  //RR_NULL_CHECK_RETURN_VOID(scell);

  MSG_GERAN_LOW_0_G("Resetting network_control_order to 0");
...
)
{
  ...
  {
    byte nc_mode = rr_gprs_get_network_control_order();

    MSG_GERAN_LOW_3_G("PCCO target %d differs from NACC target %d (scell %d)",
             (int)pcco_arfcn.num,
             (int)nacc_arfcn.num,
    ...
    {
      bcch_ncell_info_t *ncell_db_ptr = rr_gprs_get_surround_ncell_sys_info(nacc_arfcn);
      ...
      {
        MSG_GERAN_LOW_0_G("NACC target not in surround cell database");
      }
    }
    else
    {
...
static byte rr_gprs_get_nc_non_drx_period( )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
...
static byte rr_gprs_get_nc_reporting_period_i( )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
  ...
  {
    MSG_GERAN_LOW_0_G("nc_reporting_period_i out of range, returning 0");
    return 0;
  }
}

static byte rr_gprs_get_nc_reporting_period_t( )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
        ...
        (scell_info_ptr->gsm.two_quater_position != SI2Q_POSITION_NONE))
  {
      ...
      MSG_GERAN_LOW_1_G("Using initial NC_REPORTING_PERIOD_T=%d", (int)nc_reporting_period_t);
  }
  else
  {
...
static void rr_params_mods_dump(rr_params_mods_t * params_mods )
{
  ...
  {
    MSG_GERAN_MED_1_G("  NC Measurement Parameters: network_control_order: %d", params_mods->nc_meas_params.network_control_order);
    if (params_mods->nc_meas_params.nc_period_valid)
    {
      MSG_GERAN_MED_3_G("    nc_non_drx_period: %d nc_reporting_period_i: %d nc_reporting_period_t: %d ",
        params_mods->nc_meas_params.nc_non_drx_period, params_mods->nc_meas_params.nc_reporting_period_i,
        params_mods->nc_meas_params.nc_reporting_period_t);
    }
  }
  else
  {
...
=============================================================================*/
static rr_event_T rr_gprs_meas_rpt_control_handle_pmo_message( )
{
  ...
  rr_params_mods_dump(*params_mods);

  /* merge in any changes from the PMO to our current set of modifications */
  rr_nc_meas_params_merge_mods(&(*params_mods)->nc_meas_params, rr_gprs_get_pmo_nc_meas_params());
  rr_gprs_meas_params_merge_mods(&(*params_mods)->gprs_meas_params, rr_gprs_get_pmo_gprs_meas_params());

  rr_gprs_nacc_params_merge_mods(&(*params_mods)->nacc_params, rr_gprs_get_pmo_nacc_params());

#ifdef FEATURE_WCDMA
  rr_gprs_3g_meas_params_merge_mods(&(*params_mods)->gprs_3g_meas_params, rr_gprs_get_pmo_gprs_3g_meas_params());
#endif /* FEATURE_WCDMA */

  if (rr_get_pmo_priority_and_eutran_params_valid())
  {
    if (rr_get_pmo_serving_cell_priority_params(&serving_cell_priority_params))
    {
    ...
    if (rr_get_pmo_utran_priority_params(&utran_priority_params))
    {
    ...
    if (rr_get_pmo_eutran_params(&eutran_params))
    {
  ...
  rr_gprs_update_meas_params();

  /* set our NC mode based on the new global measurement params */
  output_event = rr_gprs_process_nc_measurement_params();
...
=============================================================================*/
static rr_event_T rr_gprs_meas_rpt_control_handle_pcco_message( )
{
  ...
  rr_nc_meas_params_merge_mods(&(*pcco_params_mods)->nc_meas_params, rr_gprs_get_pcco_nc_meas_params());
  rr_gprs_meas_params_merge_mods(&(*pcco_params_mods)->gprs_meas_params, rr_gprs_get_pcco_gprs_meas_params());

  rr_gprs_nacc_params_merge_mods(&(*pcco_params_mods)->nacc_params, rr_gprs_get_pcco_nacc_params());

#ifdef FEATURE_WCDMA
  rr_gprs_3g_meas_params_merge_mods(&(*pcco_params_mods)->gprs_3g_meas_params, rr_gprs_get_pcco_gprs_3g_meas_params());
#endif /* FEATURE_WCDMA */

  /* clear the PCCO data store in rr_sys_info_psi.c ready for the one */
  rr_psi_discard_pcco_data();
...
uint8 rr_gprs_get_nc_mode( )
{
  ...
  default:
    MSG_GERAN_LOW_0_G("Unknown nc mode: %d");
    output_nc_mode = NC_MEAS_NC0;
  }
...
)
{
  ...
  if (rr_is_control_state(RR_GRR))
  {
    if (rr_mac_ps_access_blocked())
    {
      ...
      proceed = FALSE;
     (void)rr_gprs_start_t3158(FALSE);
    }
    else
    {
...
)
{
  ...
  if (!found)
  {
    ...
    else
    {
      ...
      see if the current cell is better than the weakest stored cell.  If it is,
      replace it with the new one. */
      rr_gprs_tdd_3g_measurements_sort();
    ...
    for (i = grr_3g_measurements->fdd_count; i > 0; --i)
    {
      ...
      if (entry->uarfcn == uarfcn)
      {
        ...
        if (!found)
        {
          ...
          grr_3g_measurements_delete_fdd_index(grr_3g_measurements, i-1);
        }
      }
    }
...
)
{
       ...
       ++i)
  {
    ...
    {
      MSG_GERAN_LOW_1("Overflow merging UTRAN Priority Parameters (%d)", i);
    }
  }
}
...
)
{
  ...
  for (i = 0; i < src->earfcn_count && i < CSN_EUTRAN_NEIGHBOUR_CELLS_MAX_EARFCNS; ++i)
  {
    ...
    {
      dst->eutran_frequency_list[dst->eutran_frequency_list_count].earfcn = src->earfcns[i].earfcn;
...
)
{
  ...
  */
       ...
       ++i)
  {
    ...
    {
      MSG_GERAN_LOW_1("Overflow merging E-UTRAN Neighbour Cells (%d)", i);
    }
  }
       ...
       ++i)
  {
    ...
    {
      MSG_GERAN_LOW_1("Overflow merging E-UTRAN Not Allowed Cells (%d)", i);
    }
  }
}
...
// Obtain a pointer to the module data
      ...
      nc_meas_event == RR_EV_MM_REQ_RECEIVED_ABORT)
  {
    if (rr_gprs_meas_rpt_control_data_ptr->state != NC_MEAS_NULL)
    {
      rr_gprs_stop_nc_measurement_process();
      rr_gprs_meas_rpt_control_data_ptr->state = NC_MEAS_NULL;
    }
    rr_gprs_meas_rpt_control_reset();
  ...
  /***************************************************************/
    case NC_MEAS_NULL:
        ...
        if (message_set == MS_MM_RR)
        {
          ...
          if (mm_rr_message_id == RR_GMM_GPRS_STATE_CHANGE_REQ)
          {
            ...
            if(rr_gmm_gprs_state_change_req->new_state == GMM_READY_STATE)
            {
              ...
              new_nc_meas_event = rr_gprs_read_meas_rpt_sysinfo();
            }
            else
            {
              new_nc_meas_event = rr_gprs_change_nc_measurement_process(GMM_STANDBY_CAUSE);
            }
          }
          else
          {
      ...
      {
        new_nc_meas_event = rr_gprs_meas_rpt_control_handle_pmo_message();
      }
      ...
      {
        new_nc_meas_event = rr_gprs_meas_rpt_control_handle_pcco_message();
      }
      ...
      else if (nc_meas_event == EV_NCO_MODE_1)
      {
        ...
        rr_gprs_start_nc_measurement_process(FALSE);
...
#ifdef FEATURE_IDLE_DRX_SCALING
        rr_idle_drx_control(EV_NCO_MODE_1, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
      }
...
#ifdef FEATURE_IDLE_DRX_SCALING
        rr_idle_drx_control(EV_NCO_MODE_2, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
      }
      ...
      else if (nc_meas_event == EV_CAMP_ON_PCCF_OLD_SCELL)
      {
        ...
        new_nc_meas_event = rr_gprs_change_nc_measurement_process(OLD_SCELL_CAUSE);
      }
    ...
    case NC_MEAS_ACQUIRE:
      ...
      if ((nc_meas_event == EV_INPUT_MESSAGE) && (new_message != NULL))
      {
        ...
        else if (message_set == MS_RR_L1)
        {
          ...
          {
            rr_gprs_store_nc_measurements((mph_nc_measurement_ind_t*) new_message);
          }
          else if (l1_message_id == MPH_SERVING_MEAS_IND)
          {
            rr_gprs_store_serving_measurements((mph_serving_meas_ind_t*) new_message);
          }
          else if (l1_message_id == MPH_SERVING_IDLE_MEAS_IND)
          {
            ...
            rr_gprs_store_serving_measurements(&mph_serving_meas_ind);
          }
#ifdef FEATURE_WCDMA
      ...
      {
        if (rr_gprs_meas_report_can_proceed())
        {
          /* compile the measurement report */
          if (rr_gprs_format_meas_report(send_to_mac))
          {
      ...
      {
        if (!grr_nc_params_set_by_pmo_pcco())
        {
          /*The measurement reporting system information has changed,
...
#ifdef FEATURE_IDLE_DRX_SCALING
        rr_idle_drx_control(EV_NCO_MODE_0, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
      }
      ...
      else if (nc_meas_event == EV_NCO_MODE_1)
      {
        ...
        /*send the command to layer 1 to begin the measurement reporting*/
        rr_gprs_start_nc_measurement_process(TRUE);
                ...
                (nc_meas_event == EV_MAC_TRANSFER) )
      {
        ...
        if ( rr_gprs_meas_rpt_control_data_ptr->pending_pmr_store.valid )
        {
          ...
          /*****************************************************************/
          if ( ((nc_meas_event == EV_MAC_IDLE) &&
                (rr_gprs_get_nc_reporting_period_i() > THRESHOLD_REPORTING_PERIOD))
               ||
               ((nc_meas_event == EV_MAC_TRANSFER) &&
                (rr_gprs_get_nc_reporting_period_t() > THRESHOLD_REPORTING_PERIOD)) )
          {
            ...
            rr_send_grr_mac_send_meas_rpt(
              ...
              rr_gprs_get_network_control_order(),
              rr_gprs_get_nc_non_drx_period()
            );
            ...
            rr_gprs_meas_rpt_control_force_t3158_restart();
          }
          else
          {
  ...
  /***************************************************************/
    case NC_MEAS_SEND:
      ...
      if ((nc_meas_event == EV_INPUT_MESSAGE) && (new_message != NULL))
      {
        ...
        if (message_set == MS_MM_RR)
        {
          ...
          if (mm_rr_message_id == RR_GMM_GPRS_STATE_CHANGE_REQ)
          {
            ...
            else
            {
              ...
              new_nc_meas_event =
                rr_gprs_change_nc_measurement_process(GMM_STANDBY_CAUSE);
            }
          }
          else
          {
        ...
        else if (message_set == MS_MAC_RR)
        {
          ...
          if (mac_message_id == MAC_GRR_MEAS_RPT_CNF)
          {
            ...
            rr_mac_status = rr_macinfo_get_mac_status();
      ...
      else if (nc_meas_event == EV_T3158_TIMEOUT)
      {
        ...
        (void)rr_gprs_format_meas_report(send_to_mac_forbidden);
        /* Start the nc measurement timer again */
        (void)rr_gprs_start_t3158(FALSE);
      }
  ...
  /***************************************************************/
    case NC_MEAS_SUSPEND:
      ...
      else if (nc_meas_event == EV_PMO_MESSAGE)
      {
        ...
        if (rr_resel_is_in_ccn_reselection_state())
        {
          new_nc_meas_event = rr_gprs_meas_rpt_control_handle_pmo_message();
        }
      }
...
static rr_event_T rr_gprs_read_meas_rpt_sysinfo( )
{
  return rr_gprs_process_nc_measurement_params();
}
...
static rr_event_T rr_gprs_process_nc_measurement_params( )
{
  ...
  switch (rr_gprs_get_network_control_order())
  {
  case NC_MEAS_NC0:
    /**********************************************************************/
  ...
  case NC_MEAS_NC1:
    ...
    /**********************************************************************/
    gmm_state = rr_gmminfo_get_gmm_status();

#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
    if (rr_ue_in_peo_mode())
    {
      MSG_GERAN_MED_0_G("UE in PEO mode , ignore moving to NC2 mode");
      output_event = EV_NCO_MODE_0;
    }
    else
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
  ...
  case NC_MEAS_RESET:
    ...
    if (!rr_network_control_order_valid(rr_gprs_get_network_control_order()))
    {
      MSG_GERAN_ERROR_1_G("Unexpected network_control_order %d, resetting to NC0",
                rr_gprs_get_network_control_order());

      rr_network_control_order_reset();
    }
  ...
  default:
    MSG_GERAN_LOW_0_G("NC mode unrecognised !!");
  }
 ...
 *
 ****************************************************************************/
static void rr_gprs_start_nc_measurement_process(boolean bActive  )
{
  ...
  t3158_action = rr_gprs_start_t3158(FALSE);

  if ( t3158_action == t3158_restarted )
  {
    /* Send the request to L1 */
    rr_send_mph_nc_measurements_req(ba_list);

    /* clean out the nc_measurement data structure */
    rr_gprs_meas_rpt_control_reset_measurements();
  }
  else if ( t3158_action == t3158_allow_to_run &&
            rr_timer_is_running(T3174)
          )
  {
 ...
 *
 ****************************************************************************/
static rr_event_T rr_gprs_change_nc_measurement_process(cause_t cause  )
{
  rr_event_T output_event = EV_NO_EVENT;
  ...
  switch ( cause )
  {
    ...
    case DSF_CAUSE:
    {
      rr_gprs_revert_ba_gprs();
      rr_gprs_meas_rpt_control_reset_modifications();
      output_event = rr_gprs_read_meas_rpt_sysinfo();
    }
    break;
    ...
    default:
      MSG_GERAN_LOW_1_G("Unknown cause: %d", cause);
  }
  return output_event;
}
...
static void rr_gprs_stop_nc_measurement_process( )
{
  ...
  (void)rr_timer_cancel(T3158);
  MSG_GERAN_LOW_0_G("Stop the NC timer T3158");

  /* Send the stop request to L1 */
  rr_send_mph_stop_nc_measurements_req(ba_list);
 ...
 *
 ****************************************************************************/
rr_l1_freq_power_T rr_gprs_get_serving_measurements( )
{
  ...
  meas.arfcn_num  = rr_gprs_meas_rpt_control_data_ptr->nc_measurement_data.scell_arfcn.num;
  meas.arfcn_band = rr_gprs_meas_rpt_control_data_ptr->nc_measurement_data.scell_arfcn.band;
...
static boolean rr_gprs_complete_gprs_bsic_description_received( )
{
 ...
 */
static byte get_reporting_threshold(sys_band_T band  )
{
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
...
)
{
       ...
       ++i)
  {
    ...
    {
      band_matches = TRUE;
      MSG_GERAN_LOW_0_G("yes");
    }
    else
    {
      MSG_GERAN_LOW_0_G("no");
    }
...
static boolean rr_gprs_band_in_ba_list(sys_band_T band )
{
  gprs_ncells_info_t *ncells = rr_gprs_get_ncells_info();
  uint32 i;
...
static byte rr_get_scale_for_pemr( )
{
  byte scale = 0;
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
...
)
{
  ...
  ncell = rr_gprs_get_ncell_info(arfcn, NULL);
...
)
{
  ...
  boolean gsm_850_in_ba_list = rr_gprs_band_in_ba_list(SYS_BAND_CELL_850);
  boolean gsm_900_in_ba_list = rr_gprs_band_in_ba_list(SYS_BAND_EGSM_900) ||
                               rr_gprs_band_in_ba_list(SYS_BAND_PGSM_900);
  boolean gsm_1800_in_ba_list = rr_gprs_band_in_ba_list(SYS_BAND_DCS_1800);
  boolean gsm_1900_in_ba_list = rr_gprs_band_in_ba_list(SYS_BAND_PCS_1900);
  ...
  rr_gprs_meas_rpt_control_data_ptr->pemr->scale = rr_get_scale_for_pemr();
  ...
  {
    sys_band_T serving_band = rr_gprs_get_scell_arfcn().band;
    ...
    uint32 num_of_cells_added = add_cells_to_pemr(
      ...
      get_reporting_threshold(serving_band)
    );
    ...
    case SYS_BAND_PCS_1900:
      ...
      break;
    default:
      MSG_GERAN_LOW_1_G("Unknown serving cell band: %d", serving_band);
    }
  }
  ...
  if (gsm_850_in_ba_list)
  {
    uint32 num_of_cells_added = add_cells_to_pemr(
      ...
      get_reporting_threshold(SYS_BAND_CELL_850)
    );
  ...
  if (gsm_900_in_ba_list)
  {
    uint32 num_of_cells_added = add_cells_to_pemr(
      ...
      get_reporting_threshold(SYS_BAND_EGSM_900)
    );
  ...
  if (gsm_1800_in_ba_list)
  {
    uint32 num_of_cells_added = add_cells_to_pemr(
      rr_gprs_meas_rpt_control_data_ptr->pemr,
                             SYS_BAND_DCS_1800,
                             scell->gprs_meas_params.multiband_reporting + unused_positions,
      get_reporting_threshold(SYS_BAND_DCS_1800)
    );
  ...
  if (gsm_1900_in_ba_list)
  {
    uint32 num_of_cells_added = add_cells_to_pemr(
      rr_gprs_meas_rpt_control_data_ptr->pemr,
                             SYS_BAND_PCS_1900,
                             scell->gprs_meas_params.multiband_reporting + unused_positions,
      get_reporting_threshold(SYS_BAND_PCS_1900)
    );
  ...
  if (!rr_gsm_only() && rr_gprs_meas_rpt_control_data_ptr->grr_3g_measurements.tdd_count > 0)
  {
    uint32 num_of_tdd_cells_added;

    rr_gprs_tdd_3g_measurements_sort();

    num_of_tdd_cells_added = grr_add_tdd_cells_to_pemr(rr_gprs_meas_rpt_control_data_ptr->pemr, scell);
  ...
  if (!rr_gsm_only() && rr_gprs_meas_rpt_control_data_ptr->grr_3g_measurements.fdd_count > 0)
  {
    uint32 num_of_fdd_cells_added;

    rr_gprs_3g_measurements_sort();

    num_of_fdd_cells_added = grr_add_fdd_cells_to_pemr(rr_gprs_meas_rpt_control_data_ptr->pemr, scell);
  ...
  if (rr_gprs_get_network_control_order() == NC_MEAS_NC1)
  {
  ...
  rr_gprs_meas_rpt_control_data_ptr->pemr->tlli = rr_gmminfo_get_tlli();
  ...
  if ((rr_macinfo_get_mac_status() == MAC_IDLE || rr_macinfo_get_mac_status() == MAC_TRANSFER) &&
...
)
{
  ...
  packet_measurement_report->tlli = rr_gmminfo_get_tlli();

  {
    ...
    case 3:
      ...
      for ( meas_index = 0; meas_index < 6; meas_index++ )
      {
        ...
        else
        {
          ...
          case BAND_GSM_850:
            ...
            break;
          default:
            MSG_GERAN_LOW_1_G("Invalid band (%d) in format_meas_report", scell_band);
          }
        }
        ...
        if (ptr_to_best_cell != NULL)
        {
          ...
          packet_measurement_report->enc_nc_meas_rprt_strct.num_meas[meas_index].freq_n =
            rr_gprs_find_ba_gprs_index(ptr_to_best_cell->bcch_arfcn, &ptr_to_best_cell->bsic);
      ...
      while (num_meas_left)
      {
        ptr_to_best_cell = pmr_find_next_best_meas();
        if (ptr_to_best_cell)
        {
          /* load the data into the meas report */
          packet_measurement_report->enc_nc_meas_rprt_strct.num_nc_meas++;
  ...
  if (packet_measurement_report->ba_or_psi3_and_pmo_included == valid)
  {
    ...
    if (rr_gprs_3g_meas_report_encode(&packet_measurement_report->three_g_measurement_report))
    {
  ...
  if (((rr_macinfo_get_mac_status() == MAC_IDLE) ||
       (rr_macinfo_get_mac_status() == MAC_TRANSFER))
      && (pmr_send_instruction == send_to_mac))
  {
    rr_send_grr_mac_send_meas_rpt(
      REPORT_TYPE_PMR,
  ...
  else
  {
    ...
    rr_gprs_meas_rpt_control_data_ptr->pending_pmr_store.valid    = TRUE;
     ...
     MSG_GERAN_MED_1_G("Storing NC meas rpt - send to mac = %d and MAC not in IDLE or TRANSFER or waiting for response ",
 ...
 */
static boolean rr_gprs_format_meas_report(pmr_send_t pmr_send_instruction  )
{
  gprs_scell_info_t * scell = rr_gprs_get_scell_info();

  if (scell->gprs_meas_params.report_type == REPORT_TYPE_ENHANCED)
  {
    if (rr_gprs_complete_gprs_bsic_description_received())
    {
...
static t3158_action_t rr_gprs_start_t3158(boolean mac_status_changed )
{
  ...
  {
    timer_setting_ms = nc_meas_rep_period_ms[rr_gprs_get_nc_reporting_period_t()];
  }
  else
  {
    gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
  ...
  else
  {
    ...
    timer_setting_ms = (uint16)
      ((nc_meas_rep_period_ms[rr_gprs_get_nc_reporting_period_i()]/drx_period_ms) * drx_period_ms);
  ...
  /************************************************************************/
  is_running = rr_timer_is_running(T3158);

  if ((mac_status_changed == TRUE) && (rr_macinfo_get_mac_status() == MAC_IDLE))
  {
    ...
    if ((is_running == TRUE) && (rr_gprs_meas_rpt_control_data_ptr->meas_rpt_sent_in_txfer == TRUE))
    {
      ...
      (void)rr_timer_start(T3158, timer_setting_ms);
  ...
  else
  {
    ...
    time_left = (uint16)rr_timer_get_time_remaining(T3158);
...
*/
void rr_gprs_meas_rpt_control_force_t3158_restart( )
{
  uint16         timer_setting_ms;
  ...
  rr_timer_start(T3158, timer_setting_ms);
}
...
uint8                     index;
  ...
  for (index = 0; index < results_src->no_of_ncell_entries; index++)
  {
    ...
    if ((meas_ptr->valid_info) && (meas_ptr->rxlev_count > 0))
    {
      ...
      default:
        MSG_GERAN_LOW_1_G("Invalid band (%d) in load_best_six", band);
        break;
      }  /* end switch */
    } /* end if */
  } /* end for */
...
*/
byte rr_gprs_3g_meas_get_rep_quant(int meas_index  )
{
  gprs_scell_info_t *scell_ptr = rr_gprs_get_scell_info();
  uint16             noof_meas;
  byte               rep_quant;
...
static void rr_gprs_3g_measurements_sort_fdd( )
{
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
  ...
  rr_gprs_average_3g_measurements();
...
static void rr_gprs_3g_measurements_sort( )
{
  rr_gprs_3g_measurements_sort_fdd();
}
#endif /* FEATURE_WCDMA */
...
static void rr_gprs_3g_measurements_sort_tdd( )
{
  // Obtain a pointer to the module data
  rr_gprs_meas_rpt_control_data_t *rr_gprs_meas_rpt_control_data_ptr;
  rr_gprs_meas_rpt_control_data_ptr = rr_gprs_meas_rpt_control_get_data_ptr();
  ...
  rr_gprs_average_tdd_3g_measurements();
...
static void rr_gprs_tdd_3g_measurements_sort( )
{
  rr_gprs_3g_measurements_sort_tdd();
}
...
void rr_gprs_clear_3g_measurement( )
{
  // Obtain a pointer to the module data
  rr_gprs_meas_rpt_control_data_t *rr_gprs_meas_rpt_control_data_ptr;
  rr_gprs_meas_rpt_control_data_ptr = rr_gprs_meas_rpt_control_get_data_ptr();
...
boolean rr_meas_reporting_is_active( )
{
  // Obtain a pointer to the module data
  rr_gprs_meas_rpt_control_data_t *rr_gprs_meas_rpt_control_data_ptr;
  rr_gprs_meas_rpt_control_data_ptr = rr_gprs_meas_rpt_control_get_data_ptr();
...
void rr_gprs_meas_rpt_control_task_start_init(void)
{
  ...
  memset(&rr_gprs_meas_rpt_control_data, 0, sizeof(rr_gprs_meas_rpt_control_data));

  // Setup any specific values
  rr_gprs_meas_rpt_control_data.state     = NC_MEAS_NULL;
  rr_gprs_meas_rpt_control_data.old_state = NC_MEAS_NULL;
