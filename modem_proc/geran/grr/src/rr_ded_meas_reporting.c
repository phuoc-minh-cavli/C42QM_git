...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_ded_meas_reporting.c#7 $
...
#else
#define RR_DEDICATED_MODE_IRAT_MEASUREMENT_LIST_SIZE MAX_NUMBER_OF_SURROUND_CELLS
 ...
 * -------------------------------------------------------------------------*/
#ifdef FEATURE_WCDMA
static rr_log_3g_meas_report_T  log_3g_meas_report;
#endif /* FEATURE_WCDMA */
#endif /* FEATURE_GERAN_CS_SUPPORT */
...
/*****************************************/
static rr_ded_meas_reporting_data_t rr_ded_meas_reporting_data;
...
)
{
  ...
  MSG_GERAN_MED_7("GSM Measurement Parameters: PWRC:=%d, REPORT_TYPE:=%d, REPORTING_RATE:=%d, INVALID_BSIC_REPORTING:=%d, SERVING_BAND_REPORTING:=%d, MULTIBAND_REPORTING:=%d, SCALE_ORD:=%d",
    gsm_meas_params->pwrc, gsm_meas_params->report_type, gsm_meas_params->reporting_rate,
    gsm_meas_params->invalid_bsic_reporting, gsm_meas_params->serving_band_reporting, gsm_meas_params->multiband_reporting,
    gsm_meas_params->scale_ord);
  
  MSG_GERAN_MED_8("  850_REPORTING_OFFSET:=%d, 850_REPORTING_THRESHOLD:=%d, 900_REPORTING_OFFSET:=%d, 900_REPORTING_THRESHOLD:=%d, 1800_REPORTING_OFFSET:=%d, 1800_REPORTING_THRESHOLD:=%d, 1900_REPORTING_OFFSET:=%d, 1900_REPORTING_THRESHOLD:=%d",
    gsm_meas_params->reporting_offset_850, gsm_meas_params->reporting_threshold_850, gsm_meas_params->reporting_offset_900,
    gsm_meas_params->reporting_threshold_900,gsm_meas_params->reporting_offset_1800, gsm_meas_params->reporting_threshold_1800,
    gsm_meas_params->reporting_offset_1900,gsm_meas_params->reporting_threshold_1900);
}
 ...
 */
static rr_ded_meas_reporting_data_t *rr_ded_meas_reporting_get_data_ptr( void )
{
  return &rr_ded_meas_reporting_data;
}
 ...
 */
static rr_log_3g_meas_report_T *rr_log_3g_meas_report_get_ptr( void )
{
  return &log_3g_meas_report;
}
...
*/
uint32 rr_dedicated_mode_ba_list_length( )
{
...
*/
static uint32 rr_dedicated_mode_eutran_neighbour_list_length( )
{
...
static uint8 rr_reporting_threshold(sys_band_T band )
{
  ...
  switch (band)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_1("Unexpected band: %d", band);
      return RR_REPORTING_THRESHOLD_DEFAULT;
    }
  }
}

static uint8 rr_reporting_offset(sys_band_T band )
{
  // Obtain a pointer to the module data
  gprs_scell_info_t * scell = rr_gprs_get_scell_info();
...
)
{
  ...
  usat_system_information_database_ptr = rr_get_usat_system_information_database_ptr();
...
)
{
  ...
  BSIC_T             local_bsic;
  byte               arfcn_index = rr_find_frequency_index(arfcn, &arfcn_found, in_dedicated);
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
...
===========================================================================*/
static boolean rr_gsm_bsic_received_for_each_ncell( )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
  rr_ba_lists_t     *rr_ba_lists_ptr = rr_get_ba_lists_ptr();
...
*******************************************************************************************/
static byte rr_get_absolute_index_start_emr( void )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
...
*******************************************************************************************/
static byte rr_get_scale_for_emr( )
{
  byte               scale = 0; 
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
...
{
  uint8 reporting_threshold = rr_reporting_threshold(band);
...
)
{
  ...
  for (i = 0; i < gsm_measurements->no_of_entries && i < ARR_SIZE(gsm_measurements->measurement); ++i)
  {
    ...
    if (cell_index_valid)
    {
      ...
      if (rr_reported_value_equal_or_beyond_threshold(rxlev, gsm_measurements->measurement[i].BCCH_ARFCN.band))
      {
        if (irat_measurement_list->count < ARR_SIZE(irat_measurement_list->entries))
        {
          ...
          entry->sort_value                  = rxlev - rr_decode_xxx_reporting_threshold(rr_reporting_threshold(arfcn.band)) + rr_decode_xxx_reporting_offset(rr_reporting_offset(arfcn.band));
...
)
{
       ...
       ++i)
  {
    ...
    {
      cell_index += rr_get_absolute_index_start_emr();
      if (rssi_index_valid)
      {
        rssi_index += rr_get_absolute_index_start_emr();
      }
    }
...
static int16 rr_compute_modified_offset_for_earfcn(rr_l1_lte_measurements_t* meas_entry_p, rr_eutran_meas_params_t* eutran_meas_params_p, rr_l1_eutran_type_e eutran_type )
{
  uint32  cell;
...
static uint8 rr_compute_modified_threshold_for_earfcn(rr_l1_lte_measurements_t* meas_entry_p, rr_eutran_meas_params_t* eutran_meas_params_p, rr_l1_eutran_type_e eutran_type  )
{
  uint32  cell;
  int16 max_rsrp;
       ...
       ++f)
  {
    ...
    if ( filter_cells_based_on_rsrp )
    {
      if ( report_type == REPORT_TYPE_ENHANCED )
      {
        ...
        MSG_GERAN_HIGH_0_G( "rr_compute_modified_threshold_for_earfcn Invoked" );
        modified_threshold = rr_compute_modified_threshold_for_earfcn( lte_measurements->entries[f], eutran_meas_params, eutran_type );
      }
      else
      {
        MSG_GERAN_HIGH_0_G( "rr_compute_modified_offset_for_earfcn Invoked" );      
        modified_offset = rr_compute_modified_offset_for_earfcn( lte_measurements->entries[f], eutran_meas_params, eutran_type );
      }
    }
...
)
{
  ...
  for (i = 0; i < irat_measurement_list->count && i < ARR_SIZE(irat_measurement_list->entries); ++i)
  {
    ...
    switch (cell->rat)
    {
      ...
      default:
      {
        MSG_GERAN_LOW_1("Unexpected RAT: %d", cell->rat);
      }
    }
  }
}
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
  wcdma_ncell_list_data_ptr = rr_get_wcdma_ncell_list_data_ptr();
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */
...
#ifdef FEATURE_WCDMA
  if (rr_dedicated_mode_is_wcdma_enabled() && wcdma_measurements != NULL)
  {
...
#ifdef FEATURE_GSM_TDS
  if (rr_dedicated_mode_is_tdscdma_enabled() && tdscdma_measurements != NULL)
  {
...
#ifdef FEATURE_LTE
  if (rr_dedicated_mode_is_lte_enabled() && lte_measurements != NULL)
  {
...
)
{
       ...
       ++i)
  {
    switch (measurement_report->entries[i].rat)
    {
      ...
      default:
      {
        MSG_GERAN_LOW_2("[%d]: Unexpected RAT: %d",
                            i,
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
  {
         ...
         ++i)
    {
      ...
      switch (measurement_report->entries[i].rat)
      {
        ...
        default:
        {
          MSG_GERAN_LOW_2("[%d]: Unexpected RAT: %d", i, measurement_report->entries[i].rat);
          continue;
        }
      }
...
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
// Obtain a pointer to the module data
  rr_ded_meas_reporting_data_t *rr_ded_meas_reporting_data_ptr = rr_ded_meas_reporting_get_data_ptr();

  boolean invalidate_measurement_reports = FALSE;
  ...
  if (in_dedicated)
  {
    ...
    gsm_measurements                            = *rr_get_surrounding_measurement_ptr();
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
  {
    usat_system_information_database_T * usat_system_information_database_ptr = rr_get_usat_system_information_database_ptr();
  ...
  */
  if (rr_dedicated_mode_ba_list_length() <= 31)
  {
#ifdef FEATURE_WCDMA
    wcdma_measurements = rr_get_wcdma_ded_meas_ptr();
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_GSM_TDS
    tdscdma_measurements = rr_get_tds_ded_meas_ptr();
#endif /* FEATURE_GSM_TDS */
  }

#ifdef FEATURE_LTE
  if (rr_dedicated_mode_ba_list_length() <= (31 - rr_dedicated_mode_eutran_neighbour_list_length()))
  {
...
)
{
  ...
  switch (ncell->rat)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_1("Unexpected RAT: %d", ncell->rat);
      return FALSE;
    }
  }
}
...
)
{
  ...
  MSG_GERAN_MED_5("Enhanced Measurement Report: BA_USED:= %d, 3G_BA_USED:= %d, BSIC_Seen:=%d, SCALE=%d, Serving Cell Data valid :=%d",
    emr->ba_used, emr->ba_3g_used, emr->bsic_seen, emr->scale,emr->serving_cell_data_valid);

  if (emr->serving_cell_data_valid)
  {
    MSG_GERAN_MED_6("DTX_USED:=%d, RXLEV_VAL:=%d, RX_QUAL_FULL:= %d, MEAN_BEP:=%d, CV_BEP:=%d, NBR_RCVD_BLOCKS:=%d",
      emr->serving_cell_data.dtx_used,emr->serving_cell_data.rxlev_val,
      emr->serving_cell_data.rx_qual_full, emr->serving_cell_data.mean_bep,emr->serving_cell_data.cv_bep,
...
)
{
  ...
  surrounding_measurement_T                  gsm_measurements;
#ifdef FEATURE_WCDMA
  surr_wcdma_ded_measurements_T            * wcdma_measurements = rr_get_wcdma_ded_meas_ptr();
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_GSM_TDS
  rr_ded_irat_measurements_t               * tdscdma_measurements = rr_get_tds_ded_meas_ptr();
#endif /* FEATURE_GSM_TDS */
#ifdef FEATURE_LTE
  ...
  {
    usat_system_information_database_T * usat_si_db = rr_get_usat_system_information_database_ptr();
  ...
  emr.bsic_seen = rr_is_invalid_bsic_in_top_six(&gsm_measurements);
  emr.scale = rr_get_scale_for_emr();
      ...
      for (i = 0; i < irat_measurement_list->count && i < ARR_SIZE(irat_measurement_list->entries); ++i)
      {
        if (irat_measurement_list->entries[i].reported == FALSE &&
            ...
            rr_is_bsic_valid(irat_measurement_list->entries[i].data.gsm.bsic,
                             irat_measurement_list->entries[i].data.gsm.arfcn))
        {
...
)
{
 ...
 // Obtain a pointer to the module data
  ...
  if (report_type == REPORT_TYPE_N0RMAL ||
     (report_type == REPORT_TYPE_ENHANCED && (rr_gsm_bsic_received_for_each_ncell() == FALSE)))
  {
    MSG_GERAN_HIGH_0_G("REPORT_TYPE=Normal Measurement Reporting");
    if ((report_type == REPORT_TYPE_ENHANCED) && (rr_gsm_bsic_received_for_each_ncell() == FALSE))
    {
  ...
  else if ((report_type == REPORT_TYPE_ENHANCED) && (rr_gsm_bsic_received_for_each_ncell() == TRUE))
  {
...
#ifdef FEATURE_GSM_BAND_AVOIDANCE
  rr_band_avoid_meas_rpt_sent_send_imsg();
#endif /*FEATURE_GSM_BAND_AVOIDANCE*/
...
)
{
  ...
  surr_wcdma_ded_measurements_T *stored_wcdma_ded_meas_ptr = rr_get_wcdma_ded_meas_ptr();
  ...
  for (i = 0; i < stored_wcdma_ded_meas_ptr->num_entries; ++i)
  {
    ...
    {
      byte num_uarfcns = rr_get_num_uarfcns_sent_to_l1_in_dedicated();
    ...
    if (earfcn_found == FALSE)
    {
      for (j = 0; j < MAX_RR_L1_EUTRAN_FREQUENCIES; ++j)
      {
        if (rr_ded_meas_reporting_data_ptr->lte_measurements.entries[j] == NULL)
        {
          ...
          if (rr_ded_meas_reporting_data_ptr->lte_measurements.entries[j] != NULL)
          {
            MSG_GERAN_HIGH_2_G("Allocated %d bytes for LTE EARFCN %d", sizeof(rr_l1_lte_measurements_t), earfcn);
            rr_ded_meas_reporting_data_ptr->lte_measurements.entries[j]->earfcn = earfcn;
          }
          else
          {
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_ded_meas_reporting_data_ptr = rr_ded_meas_reporting_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_ded_meas_reporting_data_ptr);

#ifdef FEATURE_WCDMA
  log_3g_meas_report_ptr = rr_log_3g_meas_report_get_ptr();
  ...
  if (rr_ded_meas_reporting_data_ptr->mph_surround_wcdma_dedicated_meas_ind_received == FALSE)
  {
    ...
    rr_update_pn_position_validity_count(NULL);
    rr_delete_invalid_wcdma_ded_meas();
  }
  else
  {
  ...
  rr_generate_measurement_report(meas_valid);

#ifdef FEATURE_WCDMA
  memset (log_3g_meas_report_ptr, 0, sizeof(rr_log_3g_meas_report_T));
  rr_prepare_3g_cell_list_report(log_3g_meas_report_ptr);
  (void) rr_log_3g_cell_list_report (log_3g_meas_report_ptr);
#endif /* FEATURE_WCDMA */
...
)
{
  ...
  gprs_scell_info_t             *scell_info_ptr = rr_gprs_get_scell_info();
  rr_ded_irat_measurements_t    *stored_tds_ded_meas_ptr = rr_get_tds_ded_meas_ptr();
#ifdef FEATURE_GSM_TDS_PSEUDO_FAST_RETURN
  rr_dedicated_data_T           *dedic_ptr = rr_get_dedicated_data_ptr();
#endif /*FEATURE_GSM_TDS_PSEUDO_FAST_RETURN*/
  wcdma_ded_neighbor_list_T     *wcdma_ded_nlist_ptr = NULL;
  ...
  {
    rr_tds_pseudo_fr_set_freq_to_measured(new_utran_tdd_measurements->uarfcn);
  }
#endif /*FEATURE_GSM_TDS_PSEUDO_FAST_RETURN*/
...
)
{
  ...
  if (!rr_validate_mph_surround_wcdma_dedicated_meas_ind(received_message))
  {
    return;
  }
  ...
  rr_update_pn_position_validity_count(received_message);
    ...
    if (i == stored_wcdma_ded_meas_ptr->num_entries)
    {
      ...
      else
      {
       /* In case of overflow update the an entry whose meas is poor */
        ...
        index_found = rr_get_index_to_replace_wcdma_ded_meas(rcvd_meas, &i);
...
void rr_ded_meas_reporting_l1_in_dedicated( )
{
  ...
  {
    MSG_GERAN_HIGH_1_G("Allocated %d bytes for irat_measurement_list",
                       sizeof(*rr_ded_meas_reporting_data_ptr->irat_measurement_list));
  }
  else
  {
    MSG_GERAN_ERROR_1_G("Failed to allocate %d bytes for irat_measurement_list",
                        sizeof(*rr_ded_meas_reporting_data_ptr->irat_measurement_list));
  }
...
void rr_ded_meas_reporting_discard_data( )
{
  ...
  for (i = 0; i < MAX_RR_L1_EUTRAN_FREQUENCIES; ++i)
  {
    if (rr_ded_meas_reporting_data_ptr->lte_measurements.entries[i] != NULL)
    {
      MSG_GERAN_HIGH_2_G("Freeing %d bytes for LTE EARFCN %d",
                         sizeof(rr_l1_lte_measurements_t),
                         rr_ded_meas_reporting_data_ptr->lte_measurements.entries[i]->earfcn);
 ...
 * initialisation.
 */
void rr_ded_meas_reporting_task_start_init( void )
{
  ...
  memscpy(
    &rr_ded_meas_reporting_data.rr_last_meas_results[0],
    sizeof(rr_ded_meas_reporting_data.rr_last_meas_results),
...
)
{
  ...
  si2quater_instances = rr_get_instances_ptr(SI2Q_INSTANCE);
...
)
{
  ...
  {
    rr_dedicated_mode_process_initial_gsm_params(scell, si2quater_instances);
  }
  ...
  {
    rr_dedicated_mode_process_initial_utran_params(scell, si2quater_instances);
  }
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */
...
boolean rr_dedicated_mode_is_wcdma_enabled( )
{
#ifdef FEATURE_WCDMA
#ifdef FEATURE_SGLTE
...
boolean rr_dedicated_mode_is_tdscdma_enabled( )
{
#ifdef FEATURE_GSM_TDS
#ifdef FEATURE_SGLTE
  if (rr_is_sglte_mode_active())
  ...
  {
    return rr_is_utran_tdd_enabled();
  }
#else
  return rr_is_utran_tdd_enabled();
#endif /* FEATURE_SGLTE */
...
boolean rr_dedicated_mode_is_lte_enabled( )
{
#ifdef FEATURE_LTE
#ifdef FEATURE_SGLTE
  if (rr_is_sglte_mode_active())
