...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_freq_lists.c#4 $
***
*** DESCRIPTION
***
...
)
{
  ...
  {
    arfcn_ptr->band = (sys_band_T)rr_get_band_dyn(arfcn_ptr->num);
  }
...
)
{
  ...
  if ( mobile_alloc && cell_alloc )
  {
    ...
    if ( ma_store->ma_bitmap_valid && ( cell_alloc->no_of_entries > 0 ) )
    {
      ...
      while ( ma_bitmap && (rfl_index < cell_alloc->no_of_entries) )
      {
                                    ...
                                    uint8                  len )
{
  ...
  switch (range)                /* specific initialisations     */
  {
    ...
    default:
      MSG_GERAN_LOW_1("Unknown freq format (%d)",range);
      return 0;
  }
