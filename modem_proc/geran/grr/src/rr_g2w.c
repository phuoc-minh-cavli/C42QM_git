...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_g2w.c#3 $
...
static rr_g2w_data_t rr_g2w_data_store;
...
static boolean rr_irat_lte_arfcn_deprioritized(rr_irat_measurement_lte_t lte_data_earfcn);
#endif /*FEATURE_LTE_REL11*/

static void rr_irat_check_and_delete_prev_resel_target( rr_irat_measurement_t * measurement, boolean cell_id_to_check  );

static uint32 rr_irat_get_arfcn_for_meas_entry(rr_irat_measurement_t* measurement )
{
  uint32 arfcn;
 ...
 */
static rr_g2w_data_t *rr_g2w_get_data_ptr( void )
{
  return &rr_g2w_data_store;
}
 ...
 * @return void
 */
static boolean rr_ms_g2x_build_allowed_list(boolean *rat_allowed, const int num_rats )
{
  boolean resel_possible = TRUE;

#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_SGLTE) && defined(FEATURE_SGLTE_G2X_CONN_MODE_RESEL) && defined(FEATURE_GERAN_CS_SUPPORT)
  /* if SGLTE is active and all stacks are in dedicated mode, reselection is not possible */
  resel_possible = !(rr_is_sglte_mode_active() && rr_ms_all_stacks_in_state(RR_DATA_TRANSFER));
#else
  /* always allow reselection to proceed */
  resel_possible = TRUE;
#endif /* FEATURE_DUAL_SIM && FEATURE_SGLTE && FEATURE_SGLTE_G2X_CONN_MODE_RESEL && FEATURE_GERAN_CS_SUPPORT */
      ...
      else
      {
        ...
        if (is_multimode)
        {
          ...
          if (geran_in_multi_standby_mode())
          {
            ...
            {
              /* by default GPRS background traffic is not pre-emptable by another RAT */
            ...
            else
            {
              ...
              {
                /* G2X can be allowed if it won't coexist with GPRS on some OTHER stack */
                rat_allowed[rat_id] = (boolean)(gas_ready == GERAN_ACCESS_STRATUM_ID_1);
              }
            }

#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
            /* additional logic for WCDMA/TDSCDMA here that can override the checks above */
            if (geran_g2w_tuneaway_supported_by_device_mode(geran_get_multi_sim_mode()))
            {
              switch (rat_id)
              {
                ...
                {
                  rat_allowed[rat_id] = TRUE;
                  MSG_GERAN_HIGH_0_G("G2W reselection allowed, using G2W tuneaway");
                  break;
                }
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_GSM_TDS
                ...
                {
                  rat_allowed[rat_id] = TRUE;
                  MSG_GERAN_HIGH_0_G("G2T reselection allowed, using G2T tuneaway");
                  break;
                }
#endif /* FEATURE_GSM_TDS */
              }
...
#ifdef FEATURE_WCDMA
    if (rat_allowed[RR_RAT_WCDMA])
    {
      if (FALSE == rr_is_rat_segment_loaded(RR_SEG_LOAD_WCDMA))
      {
        rat_allowed[RR_RAT_WCDMA] = FALSE;
        MSG_GERAN_HIGH_0_G("WCDMA segment is not loaded, G2W blocked");
      }
    }
#endif /* FEATURE_WCDMA */

#ifdef FEATURE_GSM_TDS
    if (rat_allowed[RR_RAT_UTRAN_TDD])
    {
      if (FALSE == rr_is_rat_segment_loaded(RR_SEG_LOAD_TDSCDMA))
      {
        rat_allowed[RR_RAT_UTRAN_TDD] = FALSE;
        MSG_GERAN_HIGH_0_G("TDSCDMA segment is not loaded, G2T blocked");
      }
    }
#endif /* FEATURE_GSM_TDS */
  }/* rat_allowed != NULL */
...
)
{
  ...
  rr_g2w_data_ptr = rr_g2w_get_data_ptr();
...
)
{
  ...
  if (measurements->no_of_entries < RR_IRAT_MEASUREMENTS_PER_RAT)
  {
    ...
    rr_irat_measurement_allocate_timers(measurement, TRUE);
...
)
{
  ...
  if (measurements->no_of_entries < RR_IRAT_MEASUREMENTS_PER_RAT)
  {
    ...
    rr_irat_measurement_allocate_timers(measurement, FALSE);
...
)
{
  ...
  gprs_better_cell_list_T       *local_better_cell_list_ptr = rr_get_better_cell_list_ptr();
...
#ifdef FEATURE_PS_PREFERRED_CELL
  if (rr_ps_preferred_allowed() && !rr_cell_supports_gprs())
  {
    MSG_GERAN_MED_0_G("Scell Rxlev comparison ignored as cell does not support PS");
    ignore_serving_cell_rxlev = TRUE;
  }
  ...
  {
    boolean ignore_ncell_rxlev = FALSE;

#ifdef FEATURE_PS_PREFERRED_CELL
    ignore_ncell_rxlev = rr_ps_preferred_allowed() && 
      (rr_gprs_get_surround_ncell_sys_info(better_cell_ptr->reselection_data.BCCH_ARFCN)->gprs_supported == FALSE);
#endif /* FEATURE_PS_PREFERRED_CELL */
    ...
    if (!(measurement->data.wcdma.rscp > RxLev) && !ignore_ncell_rxlev)
    {
      surrounding_wcdma_better = FALSE;
      MSG_GERAN_HIGH_4_G("WCDMA cell RSCP:%d, NCell(%d) RxLev: %d, ignore_ncell_rxlev=%d", \
        measurement->data.wcdma.rscp, better_cell_ptr->reselection_data.BCCH_ARFCN.num, RxLev, ignore_ncell_rxlev);
...
)
{
     ...
     )
  {
    ...
    if (rr_timer_cancel(five_second_timer_id))
    {
#ifdef FEATURE_WCDMA
...
static boolean rr_irat_check_if_highest_priority_cell(rr_irat_priority_list_t*  priority_list_ptr, rr_irat_priority_list_entry_t *input_entry )
{
...
@param  (a) rr_irat_measurement_t IRAT Meas Entry (b) cell_id_to_chec (c) 
@ returns NONE
...
#ifdef FEATURE_LTE
static void rr_irat_adjust_all_running_low_priority_timer_values(rr_irat_priority_list_t*  priority_list_ptr, rr_irat_priority_list_entry_t *input_entry )
{
  uint32 i;
  rr_g2w_data_t                      *rr_g2w_data_ptr;
  uint8                              t_reselection;
       ...
       ++i)
  {
    ...
    if ( entry->priority < input_entry->priority )
    {
      if ( rr_timer_is_running(entry->measurement->thresh_high_timer)) 
      {
        rr_timer_extend(entry->measurement->thresh_high_timer, t_reselection_ms );
        timer_adjusted= TRUE;
      }
      else if ( rr_timer_is_running(entry->measurement->thresh_low_timer))
      {
        rr_timer_extend(entry->measurement->thresh_low_timer, t_reselection_ms );        
        timer_adjusted= TRUE;
      }
      else if ( rr_timer_is_running(entry->measurement->h_prio_timer))
      {
        rr_timer_extend(entry->measurement->h_prio_timer, t_reselection_ms );        
        timer_adjusted= TRUE;
      }
...
)
{
  ...
  if (i < measurements->no_of_entries && i < RR_IRAT_MEASUREMENTS_PER_RAT)
  {
    ...
    {
      (void)rr_timer_cancel(measurements->entries[i].resel_timer);
...
#ifdef FEATURE_LTE
    (void)rr_timer_cancel(measurements->entries[i].thresh_high_timer);
    (void)rr_timer_cancel(measurements->entries[i].thresh_low_timer);
    (void)rr_timer_cancel(measurements->entries[i].h_prio_timer);
   ...
   rr_irat_check_and_delete_prev_resel_target(&measurements->entries[i], TRUE );
...
)
{
  ...
  for (i = measurements->no_of_entries; i > 0; --i)
  {
    ...
    {
      rr_irat_measurements_delete_index(measurements, i-1);
    }
  }
}
    ...
    for (i = wcdma_measurements->no_of_entries; i > 0; --i)
    {
      if (wcdma_measurements->entries[i-1].data.wcdma.uarfcn == new_wcdma_measurements->uarfcn)
      {
        ...
        if (j == new_wcdma_measurements->no_of_entries)
        {
          ...
          rr_irat_measurements_delete_index(wcdma_measurements, i-1);
        }
      }
    }
    ...
    for (i = utran_tdd_measurements->no_of_entries; i > 0; --i)
    {
      if (utran_tdd_measurements->entries[i-1].data.utran_tdd.uarfcn == new_utran_tdd_measurements->uarfcn)
      {
        ...
        if (j == new_utran_tdd_measurements->no_of_entries)
        {
          /* The WCDMA ncell is not present in the new measurements from L1, so
          should be deleted from the store. */
          rr_irat_measurements_delete_index(utran_tdd_measurements, i-1);
        }
      }
    }
    ...
    for (i = lte_measurements->no_of_entries; i > 0; --i)
    {
      if (lte_measurements->entries[i-1].data.lte.earfcn == new_lte_measurements->earfcn)
      {
        ...
        if (j == new_lte_measurements->no_of_entries)
        {
          ...
          should be deleted from the store. */
          rr_irat_measurements_delete_index(lte_measurements, i-1);
        }
      }
    }
...
#ifdef FEATURE_WCDMA
static void rr_irat_dump_wcdma_measurements( )
{
  rr_irat_measurements_t * measurements;
  uint32 i;
  rr_g2w_data_t *rr_g2w_data_ptr;
...
#ifdef FEATURE_GSM_TDS
static void rr_irat_dump_utran_tdd_measurements( )
{
  rr_irat_measurements_t * measurements;
  uint32 i;
  rr_g2w_data_t *rr_g2w_data_ptr;
...
#ifdef FEATURE_LTE
static void rr_irat_dump_lte_measurements( )
{
  rr_irat_measurements_t * measurements;
  uint32 i;
  rr_g2w_data_t *rr_g2w_data_ptr;
...
)
{
  ...
  for (i = 0; i < wcdma_measurements->no_of_entries; ++i)
  {
    ...
    if (rr_g2x_resel_to_irat_cell_is_allowed(&cell))
    {
      ...
      if (priority != RR_RESEL_PRIORITY_UNDEFINED)
      {
        ...
        priority_list_entry->s_value = measurement->data.wcdma.rscp - rr_get_qrxlevmin_for_uarfcn(measurement->data.wcdma.uarfcn, RR_UTRAN_FDD);
        priority_list_entry->thresh_high = rr_get_thresh_utran_high_for_uarfcn(measurement->data.wcdma.uarfcn, RR_UTRAN_FDD);
        priority_list_entry->thresh_low = rr_get_thresh_utran_low_for_uarfcn(measurement->data.wcdma.uarfcn, RR_UTRAN_FDD);
  ...
  for (i = 0; i < lte_measurements->no_of_entries; ++i)
  {
   ...
   if (rr_g2x_resel_to_irat_cell_is_allowed(&cell))
   {
      ...
      if (priority != RR_RESEL_PRIORITY_UNDEFINED)
      {
        ...
        if (rr_irat_lte_arfcn_deprioritized(measurement->data.lte) == TRUE)
        {
        ...
        priority_list_entry->s_value = measurement->data.lte.rsrp - rr_get_qrxlevmin_for_earfcn(measurement->data.lte.earfcn);
        priority_list_entry->thresh_high = rr_get_thresh_high_for_earfcn(measurement->data.lte.earfcn);
        priority_list_entry->thresh_low = rr_get_thresh_low_for_earfcn(measurement->data.lte.earfcn);
  ...
  for (i = 0; i < tds_measurements->no_of_entries; ++i)
  {
    ...
    if (rr_g2x_resel_to_irat_cell_is_allowed(&cell))
    {
      ...
      if (priority != RR_RESEL_PRIORITY_UNDEFINED)
      {
        ...
        priority_list_entry->measurement = measurement;
        priority_list_entry->priority = priority;
        priority_list_entry->s_value = measurement->data.utran_tdd.rscp - rr_get_qrxlevmin_for_uarfcn(measurement->data.utran_tdd.uarfcn, RR_UTRAN_TDD);
        priority_list_entry->thresh_high = rr_get_thresh_utran_high_for_uarfcn(measurement->data.utran_tdd.uarfcn, RR_UTRAN_TDD);
        priority_list_entry->thresh_low = rr_get_thresh_utran_low_for_uarfcn(measurement->data.utran_tdd.uarfcn, RR_UTRAN_TDD);
  ...
  rr_irat_priority_list_entries_sort_geran_priority = rr_geran_priority();
...
static void rr_irat_priority_list_dump(rr_irat_priority_list_t * priority_list )
{
  uint32 i;
    ...
    switch (priority_list->entries[i].measurement->rat)
    {
      ...
      case RR_RAT_WCDMA:
      {
        MSG_8(MSG_SSID_DFLT,
              ...
              rr_timer_is_running(priority_list->entries[i].measurement->thresh_high_timer) << 16 |
              rr_timer_is_running(priority_list->entries[i].measurement->thresh_low_timer)  << 8  |
              rr_timer_is_running(priority_list->entries[i].measurement->h_prio_timer));
        break;
      }
#endif /* FEATURE_WCDMA */
...
)
{
  ...
  rr_log_packet_init(&pkt_info, LOG_GSM_RR_IRAT_PRIORITY_LIST_C, sizeof(rr_log_irat_priority_list_t));
         ...
         ++i)
    {
      ...
      switch (priority_list->entries[i].measurement->rat)
      {
        ...
        case RR_RAT_WCDMA:
        {
          ...
          entry->rank_resel_timer_running   = rr_timer_is_running(priority_list->entries[i].measurement->resel_timer);
          break;
        } /* RR_RAT_WCDMA */
#endif /* FEATURE_WCDMA */
      ...
      entry->thresh_high_timer_running = rr_timer_is_running(priority_list->entries[i].measurement->thresh_high_timer);
      entry->thresh_low_timer_running  = rr_timer_is_running(priority_list->entries[i].measurement->thresh_low_timer);
      entry->h_prio_timer_running      = rr_timer_is_running(priority_list->entries[i].measurement->h_prio_timer);
...
{
  // Obtain a pointer to the module data
  rr_g2w_data_t *rr_g2w_data_ptr = rr_g2w_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2w_data_ptr);
  ...
  for (i = 0; i < measurements->no_of_entries && i < RR_IRAT_MEASUREMENTS_PER_RAT; ++i)
  {
    ...
    else if (timer_id == measurement->resel_timer)
    {
      if (!found)
      {
        ...
        RR_ASSERT_RETURN_VOID(rr_timer_has_expired(timer_id));
...
)
{
  ...
  (void) rr_log_gprs_3g_reselection_measurements();
...
)
{
  ...
  rr_log_irat_measurements(surround_irat_meas_ind);
  ...
  switch (surround_irat_meas_ind->rat)
  {
    ...
    case RR_L1_IRAT_RAT_WCDMA:
    {
      ...
      {
        rr_irat_store_wcdma_measurements(&surround_irat_meas_ind->measurements.wcdma);
        rr_irat_dump_wcdma_measurements();
      }
      else
      {
    ...
    case RR_L1_IRAT_RAT_LTE:
    {
      ...
      {
        rr_irat_store_lte_measurements(&surround_irat_meas_ind->measurements.lte);
        rr_irat_dump_lte_measurements();
      }
      else
      {
    ...
    case RR_L1_IRAT_RAT_UTRAN_TDD:
    {
      ...
      {
        rr_irat_store_utran_tdd_measurements(&surround_irat_meas_ind->measurements.utran_tdd);
        rr_irat_dump_utran_tdd_measurements();
      }
      else
      {
...
)
{
  ...
  if((rr_is_sglte_mode_active()) && (TRUE == immediate_reselection ))
  {
  ...
  resel_possible = rr_ms_g2x_build_allowed_list(allowed_rat_list, (int)RR_RAT_MAX);
  ...
  for (count = 0; count < no_of_neighbor_wcdma_cells; ++count)
  {
    ...
    if (wcdma_info_ptr->rat == RR_RAT_WCDMA)
    {
      ...
      if (rr_wcdma_cell_rscp_is_better(
          ...
          rr_gprs_wcdma_cell_ecno_is_suitable(wcdma_info_ptr) &&
          rr_wcdma_cell_rscp_is_suitable(wcdma_info_ptr))
      {
        ...
        if (rr_g2x_resel_to_irat_cell_is_allowed(&cell) == FALSE)
        {
        ...
        if (surrounding_wcdma_better)
        {
          ...
          timer_state = rr_timer_get_state(wcdma_info_ptr->resel_timer);
          ...
          if (timer_state == RR_TIMER_EXPIRED)
          {
            ...
            */
            ...
            rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_GSM_TO_WCDMA);
          ...
          else if (timer_state == RR_TIMER_STOPPED)
          {
            ...
            /*************************************************************/
            (void)rr_timer_start(wcdma_info_ptr->resel_timer, RR_FIVE_SEC_TIMEOUT);
            rr_send_mph_specific_irat_freq_update_req(
              rr_irat_get_arfcn_for_meas_entry(wcdma_info_ptr),
              wcdma_info_ptr->rat,
              RR_FIVE_SEC_TIMEOUT,
        ...
        else
        {
          ...
          rr_g2wr_stop_wcdma_five_second_timer(wcdma_info_ptr);
        }
      }
      else
      {
    ...
    if (utran_tdd_info_ptr->rat == RR_RAT_UTRAN_TDD)
    {
         ...
         )
      {
        ...
        if (surrounding_utran_tdd_better)
        {
          ...
          /* Check if it's 5 Sec timer expired, else start it. */
          timer_state = rr_timer_get_state(utran_tdd_info_ptr->resel_timer);
          ...
          if (timer_state == RR_TIMER_EXPIRED)
          {
            ...
            */
            ...
            else
            {
              ...
              rr_g2w_data_ptr->resel_target.valid = FALSE;   
              rr_g2w_data_ptr->resel_target.cell.rat = RR_RAT_UNDEFINED;
              (void)rr_timer_start(utran_tdd_info_ptr->resel_timer, RR_FIVE_SEC_TIMEOUT);
          ...
          else if (timer_state == RR_TIMER_STOPPED)
          {
            ...
            /* better for more than five seconds                              */
            /******************************************************************/
            rr_timer_start(utran_tdd_info_ptr->resel_timer, RR_FIVE_SEC_TIMEOUT);
            rr_send_mph_specific_irat_freq_update_req(
              rr_irat_get_arfcn_for_meas_entry(utran_tdd_info_ptr),
        ...
        else
        {
          ...
          // Stop the Five Sec timer, it's ok to stop it even it's not running.
          rr_g2wr_stop_wcdma_five_second_timer(utran_tdd_info_ptr);
        }
      }
      else
      {
  ...
  if (rr_timer_is_running(RR_W2G_SERVICE_REDIRECTION_WAIT_TIMER))
  {
    if (resel_triggered)
    {
  ...
  if (rr_g2w_data_ptr->resel_target.valid)
  {
    if (grr_verify_saved_mm_message(RR_DEACT_REQ) ||
        grr_verify_saved_mm_message(RR_STOP_GSM_MODE_REQ))
    {
...
)
{
  ...
  switch (entry->measurement->rat)
  {
    ...
    case RR_RAT_WCDMA:
    {
      if (rr_gprs_wcdma_cell_ecno_is_suitable(entry->measurement))
      {
        return TRUE;
      }
      else
...
static boolean rr_set_reselection_target(rr_irat_priority_list_entry_t *entry )
{
...
)
{
  ...
  scell = rr_gprs_get_scell_info();
  geran_priority = rr_geran_priority();
  ...
  gprs_reselection_list = rr_get_gprs_reselection_list_ptr();
  ...
  rr_irat_priority_list_dump(&priority_list);
  ...
  for (i = 0; i < priority_list.length; ++i)
  {
    ...
    else if (entry->priority > geran_priority)
    {
      ...
      if (entry->s_value > entry->thresh_high && rr_irat_priority_list_entry_suitable(entry))
      {
        if (rr_timer_is_running(entry->measurement->thresh_high_timer))
        { 
           /* No action necessary. */
        }
        else if (rr_timer_has_expired(entry->measurement->thresh_high_timer))
        {        
          rr_set_reselection_target(entry ); 
          rr_g2w_data_ptr->prev_resel_target.valid = FALSE;
          ...
          if (!resel_possible)
          {
            ...
            (void)rr_timer_start(entry->measurement->thresh_high_timer, t_reselection_ms);          
          }
        ...
        else
        {
          ...
          (void)rr_timer_start(entry->measurement->thresh_high_timer, t_reselection_ms);
          rr_send_mph_specific_irat_freq_update_req(
            rr_irat_get_arfcn_for_meas_entry(entry->measurement),
            entry->measurement->rat,
            t_reselection_ms,
            specific_irat_freq_update_list,
            RR_IRAT_PRIORITY_LIST_LENGTH);

          if ( (rr_get_nv_anite_gcf_flag() == FALSE) )
          {
            if ( rr_irat_check_if_highest_priority_cell(&priority_list, entry) && !(rr_g2w_data_ptr->prev_resel_target.valid) )
            { 
              ...
              rr_irat_adjust_all_running_low_priority_timer_values(&priority_list, entry);
            }
          }
        }
      }
      ...
      {
        if (rr_timer_cancel(entry->measurement->thresh_high_timer))
        {
        ...
        /* This indicates that the High Priority Cell no longer exists */
          ...
          {
            rr_irat_check_and_delete_prev_resel_target(entry->measurement, TRUE );
          }          
        }
      }

      if (rr_timer_cancel(entry->measurement->thresh_low_timer))
      {
        MSG_GERAN_MED_2_G("Cell %d: Cancelled thresh_low_timer (0x%02x)", i, entry->measurement->thresh_low_timer);
      }

      if (rr_timer_cancel(entry->measurement->h_prio_timer))
      {
    ...
    else
    {
      ...
      if (priority_resel_to_lower_layers_permitted)
      {
        if (entry->s_value > entry->thresh_low &&  rr_irat_priority_list_entry_suitable(entry))
        {
          if (rr_timer_is_running(entry->measurement->thresh_low_timer))
          {
            /* No action necessary. */
          }
          else if (rr_timer_has_expired(entry->measurement->thresh_low_timer))
          {
            ...
            if (!resel_possible)
            {
              ...
              (void)rr_timer_start(entry->measurement->thresh_low_timer, t_reselection_ms);            
            }
        ...
        else
        {
          ...
          if (h_prio != 0 && entry->s_value > (s_gsm + (RR_CONVERT_H_PRIO_TO_DB(h_prio)))
              &&  rr_irat_priority_list_entry_suitable(entry))
          {
            if (rr_timer_is_running(entry->measurement->h_prio_timer))
            {
              /* No action necessary. */
            }
            else if (rr_timer_has_expired(entry->measurement->h_prio_timer))
            {
              rr_set_reselection_target(entry );            
              rr_g2w_data_ptr->prev_resel_target.valid = FALSE;
              ...
              if (!resel_possible)
              {
                ...
                (void)rr_timer_start(entry->measurement->h_prio_timer, t_reselection_ms);
              }
    ...
    for (i = 0; i < priority_list.length; ++i)
    {
      ...
      else if (entry->priority > geran_priority)
      {
        ...
        if (entry->s_value > entry->thresh_high && rr_irat_priority_list_entry_suitable(entry))
        {
          if (rr_timer_is_running(entry->measurement->thresh_high_timer))
          {                    
            if(rr_set_reselection_target(entry ))
            {              
              rr_g2w_data_ptr->prev_resel_target.valid = FALSE;
              break;
            }
...
void rr_irat_lte_measurements_delete( )
{
  rr_irat_measurements_t * lte_measurements;
  uint32 i;
  ...
  {
    rr_irat_measurements_delete_index(lte_measurements, 0);
  }
}
#endif /* FEATURE_LTE */
...
rr_irat_cell_t rr_irat_resel_target_get( )
{
  // Obtain a pointer to the module data
  rr_g2w_data_t *rr_g2w_data_ptr = rr_g2w_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2w_data_ptr);
...
extern void rr_g2w_depri_update_lte_about_requested_plmn(PLMN_id_T *requested_plmn_id )
{
       ...
       || (rr_g2w_data_ptr->depri_lte_earfcn_list.depri_all_freq ==TRUE) )
  {
    ...
    result = rr_msgr_send(&lte_rrc_depri_requested_plmn_ind_ptr->msg_hdr, sizeof(lte_rrc_depri_requested_plmn_ind_s));
    if (result != E_SUCCESS)
    {
