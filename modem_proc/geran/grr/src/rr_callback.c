...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_callback.c#4 $
***
*** DESCRIPTION
***
...
static rr_cell_info_t  rr_callback_cell_info;
...
static rr_cell_change_cb_t rr_callback_list = {NULL};
 ...
 */
static rr_cell_info_t *rr_callback_get_data_ptr(void )
{
  return &rr_callback_cell_info;
}
...
===========================================================================*/
void rr_init_cell_change_cb_data(void)
{
  uint8 i;
  rr_cell_info_t *rr_callback_data_ptr;

  // Obtain a pointer to the module data
  rr_callback_data_ptr = rr_callback_get_data_ptr();
...
boolean rr_register_cell_change_ind_cb_dyn(RR_CELL_CHANGE_IND_CB_FUNC_TYPE *cb_ptr)
{
...
boolean rr_ds_register_cell_change_ind_cb_dyn( RR_CELL_CHANGE_IND_CB_FUNC_TYPE *cb_ptr)
{
  ...
  if (entry_ptr != NULL)
  {
    ...
    MSG_GERAN_HIGH_1("Registered callback 0x%08X",(int)(cb_ptr));
    return(TRUE);
  }
#else
  (void)cb_ptr;
  MSG_GERAN_ERROR_0_G("rr_ds_register_cell_change_ind_cb_dyn() called but FEATURE_DUAL_SIM not def");
...
boolean rr_deregister_cell_change_ind_cb_dyn(RR_CELL_CHANGE_IND_CB_FUNC_TYPE *cb_ptr)
{
  if (rr_callback_list.cb_fn_ptr == cb_ptr)
  {
    MSG_GERAN_HIGH_1("RR Deregistering rr_serving_cell_change_ind_cb cb: 0x%x",cb_ptr);
    rr_callback_list.cb_fn_ptr = NULL;
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
boolean rr_ds_deregister_cell_change_ind_cb_dyn(RR_CELL_CHANGE_IND_CB_FUNC_TYPE *cb_ptr)
{
  (void)cb_ptr;
  MSG_GERAN_ERROR_0("rr_ds_deregister_cell_change_ind_cb_dyn() called but FEATURE_DUAL_SIM not def");
  return FALSE;
}
#endif /* FEATURE_DUAL_SIM */
/*===========================================================================
...
===========================================================================*/
static void rr_callbacks_execute(const rr_callback_trigger_t cb_trigger)
{
  ...
  cb_ptr = rr_callback_list.cb_fn_ptr;
  ...
  if (num_exec != 0)
  {
    MSG_GERAN_HIGH_1_G("RR has executed registered callbacks id is = %d", cb_trigger);
  }
...
{
  // Obtain a pointer to the module data
  rr_cell_info_t *rr_callback_data_ptr = rr_callback_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_callback_data_ptr);
  ...
  if (cell_id != rr_callback_data_ptr->cell_id)
  {
    ...
    rr_callbacks_execute(RR_CALLBACK_CELLID_TRIGGER);
  }
...
void rr_handle_ncell_callback(best_six_ncells_t best_six)
{
  ...
  if (rr_diff_ncells(converted_best_six,rr_callback_data_ptr->neighbour_cell) == TRUE)
  {
    ...
    rr_callbacks_execute(RR_CALLBACK_NCELLS_TRIGGER);
  }
