...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_channel_mode.c#4 $ */
...
static rr_channel_mode_support_t rr_channel_mode_support;
 ...
 * \return uint8
 */
static uint8 rr_channel_mode_get_data_space_index(void)
{
 ...
 * \return rr_nv_data_t*
 */
static rr_channel_mode_support_t *rr_channel_mode_get_data_ptr(void)
{
  return &rr_channel_mode_support;
}
...
)
{
  ...
  /*****************************************************/
  current_IE = rr_get_first_ie_of_message(CHANNEL_MODE_MODIFY);
    ...
    if ((chan_spec_ptr != NULL) && IS_AMR_CHANNEL_MODE(cmm_ptr->channel_mode))
    {
               ...
               (IS_AMR_CHANNEL_MODE(chan_spec_ptr->channel_mode_1)))
      {
        ...
        else
        {
          ...
          MSG_GERAN_LOW_0_G("No multirate config in CHANNEL MODE MODIFY");
        }
      }
      else
      {
...
)
{
  ...
  rr_channel_mode_support_ptr = rr_channel_mode_get_data_ptr();
 ...
 * \param  
 */
void rr_handle_channel_mode_modify_msg( void )
{
  rr_dedicated_data_T   *dedic_ptr;
  channel_mode_modify_T *cmm_ptr;
  ...
  if (rr_decode_channel_mode_modify(dedic_ptr) == TRUE)
  {
    channel_spec_T *chan_spec_ptr;
    ...
    /*****************************/
    else if (rr_channel_mode_supported(cmm_ptr->channel_mode) == FALSE)
    {
    ...
    rr_send_channel_mode_modify_ack(cmm_ptr);
...
)
{
  ...
  else
  {
    ...
    {
      rr_band_avoid_ded_freq_upd_send_imsg(&dedic_ptr->curr_channel_spec.channel_info_1_after.frequency_list);
    }
...
===========================================================================*/
void rr_set_traffic_reason_from_channel_mode(const channel_mode_T channel_mode )
{
  switch(channel_mode)
  {
    case SIGNALLING_ONLY:
...
uint32 rr_eng_mode_get_speech_sample_frequency(sys_speech_codec_e_type speech_codec)
{
  ...
  MSG_LOW("Speech_enc_samp_freq set to:%d",speech_enc_samp_freq,0,0);
  ...
  if (cmd == SYS_SPEECH_CODEC_INFO)
  {
    ...
    switch (new_chan_mode)
    {
      case SPEECH_V1:
        ...
        {
          MSG_GERAN_LOW_1_G("RR SPEECH_V1 unexpected channel type:%d",new_chan_type);
          ho_codec_info.speech_codec = SYS_SPEECH_CODEC_GSM_HR;
        }
      break;
 ...
 */
void rr_channel_mode_task_start_init(void)
{
