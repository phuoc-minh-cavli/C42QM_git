...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_tdsrrc_if.c#2 $
...
*/
void tdsrr_send_rrc_message_to_rr_dyn(
  void *                message_ptr,
...
*/
sys_band_T tdsrr_get_arfcn_band_sub_id_dyn(
  uint16                       arfcn,
...
/**
  ...
  @return:         The band that ARFCN is in, or INVALID_BAND if it is in a
                   band that is not currently supported by RR.
*/
sys_band_T tdsrr_get_arfcn_band_dyn(
  uint16                       arfcn,
  tdsrr_frequence_band_e_type  band_ind
)
{
  ...
  return tdsrr_get_arfcn_band_sub_id_dyn(arfcn, band_ind );
}
...
*/
void tdsrr_set_band_pref_sub_id_dyn(sys_band_mask_type band_pref)
{

  rr_PLMN_request_details_T * plmn_request_details = rr_get_PLMN_request_details_ptr();

  MSG_GERAN_HIGH_2_G("TDSRRC sets band_pref from 0x%08x to 0x%08x", rr_band_pref(), band_pref);
...
/**
  ...
  @param band_pref: The band_pref for RR to set.
*/
void tdsrr_set_band_pref_dyn(sys_band_mask_type band_pref)
{
#ifdef FEATURE_TDS_SUB_ID_INTERFACE
  MSG_GERAN_ERROR_0("SS API called - use tdsrr_set_band_pref_sub_id_dyn()");
#endif // FEATURE_TDS_SUB_ID_INTERFACE

  tdsrr_set_band_pref_sub_id_dyn(band_pref, SYS_MODEM_AS_ID_1);
  return;
}
...
*/
void tdsrr_set_bst_band_pref_sub_id_dyn(sys_band_mask_type band_pref, sys_modem_as_id_e_type as_id)
{
  return;
}
...
/**
  ...
  @param band_pref: The band_pref for RR to set.
*/
void tdsrr_set_bst_band_pref_dyn(sys_band_mask_type band_pref)
{
#ifdef FEATURE_TDS_SUB_ID_INTERFACE
  MSG_GERAN_ERROR_0("SS API called - use tdsrr_set_bst_band_pref_sub_id_dyn()");
#endif // FEATURE_TDS_SUB_ID_INTERFACE

  tdsrr_set_bst_band_pref_sub_id_dyn(band_pref, SYS_MODEM_AS_ID_1);
  return;
}
...
*/
PLMN_id_T tdsrr_get_rr_curr_camped_plmn_sub_id_dyn(sys_modem_as_id_e_type as_id)
{
  PLMN_id_T          plmn_id;
...
/**
  ...
  @return: PLMN ID of the cell that RR is camped on, or (0xFF 0xFF 0xFF) is
           returned if RR is not camped.
*/
PLMN_id_T tdsrr_get_rr_curr_camped_plmn_dyn(void)
{
#ifdef FEATURE_TDS_SUB_ID_INTERFACE
  MSG_GERAN_ERROR_0("SS API called - use tdsrr_get_rr_curr_camped_plmn_sub_id_dyn()");
#endif // FEATURE_TDS_SUB_ID_INTERFACE

  return tdsrr_get_rr_curr_camped_plmn_sub_id_dyn(SYS_MODEM_AS_ID_1);
}
...
*/
void tdsrr_acq_db_save_sub_id_dyn(sys_modem_as_id_e_type as_id)
{
...
/**
  ...
  RR's acquisition database to NV.
*/
void tdsrr_acq_db_save_dyn(void)
{
#ifdef FEATURE_TDS_SUB_ID_INTERFACE
  MSG_GERAN_ERROR_0("SS API called - use tdsrr_acq_db_save_sub_id_dyn()");
#endif // FEATURE_TDS_SUB_ID_INTERFACE

  tdsrr_acq_db_save_sub_id_dyn(SYS_MODEM_AS_ID_1);
  return;
}
