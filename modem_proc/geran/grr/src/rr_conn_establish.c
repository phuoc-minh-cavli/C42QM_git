...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_conn_establish.c#9 $ $DateTime: 2019/12/02 06:22:48 $ $Author: pwbldsvc $
...
/* CR2324640 As per system recommendation, DEL_ONE Signaling only
** PDU if IA reject has happened and wait_time < 10 sec */
#define IA_REJ_WAIT_THRES       10
...
static rr_conn_est_info_T rr_conn_est_info;

#define RRCE_SET_CAMPED_FLAG(cf) {\
 ...
 */
static rr_conn_est_info_T *rr_conn_est_get_data_ptr( void )
{
  return &rr_conn_est_info;
}
...
/**
  ...
  a PCCO procedure.
*/
static boolean rr_ce_pcco_in_progress( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  return rr_conn_est_info_ptr->pcco_in_progress;
}

/*!
 * \brief Called when a connection is established and confirmed to NAS (i.e. RR_EST_CNF/RR_EST_IND)
 */
static void rr_ce_conn_confirmed_to_nas( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
...
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK  
    if (rr_nv_get_allow_peo_rach_from_efs())
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/    
    {
      /*In PEO mode */
       MSG_GERAN_HIGH_0_G("PEO is supported use PEO one phase");
       access_reason = GMAC_EGPRS_PEO_ONE_PHASE_ACCESS;
    }
  }
...
)
{
  ...
  switch(pkt_est_cause)
  {
    ...
    case RR_PKT_EST_CAUSE_PAGE_RESP:
      geran_set_traffic_reason(GERAN_TRAFFIC_PS_DATA);
      break;

    case RR_PKT_EST_CAUSE_CELL_UPD:
    case RR_PKT_EST_CAUSE_MM:
      geran_set_traffic_reason(GERAN_TRAFFIC_PS_SIGNALLING);
      break;

    case RR_PKT_EST_CAUSE_UNKNOWN:
    default:
      geran_set_traffic_reason(GERAN_TRAFFIC_PS_OTHER);
      break;
  }
 ...
 */
rr_pkt_est_cause_t rr_ce_get_pkt_est_cause( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
boolean rr_ce_egprs_access_valid( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
#ifdef FEATURE_GERAN_CS_SUPPORT
boolean rr_ce_for_emergency_call( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
)
{
  ...
  paging_response[index] = rr_get_cipher_key_sequence_number();
  index++;
 ...
 */
static byte rr_construct_paging_response(byte *l3_msg_ptr )
{
  classmark_2_ie_T classmark_2;
  byte             msg_len = 0;
  ...
  /**************************************************/
   if ((l3_msg_ptr == NULL) ||
       (rr_encode_classmark(RR_CLASSMARK_2_ONLY, &classmark_2, NULL) == FALSE))
   {
     return(0);
   }
 ...
 */
static void rr_ce_send_first_message( void )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
  ...
  if (rr_ce_access_reason() == (byte) MM_RR_ANSWER_TO_PAGING)
  {
    byte layer3_message[N201_MAX];
    byte message_length;

    /* RR should send a PAGE RESPONSE message to the network */
    message_length = rr_construct_paging_response(layer3_message);
  ...
  /*******************************************************/
  rr_set_ciphering_mode_command_received(FALSE);
 ...
 */
static rr_event_T rr_ce_handle_communication_establishment( void )
{
  if (rr_ce_access_reason() == (byte) MM_RR_ANSWER_TO_PAGING)
  {
  ...
  rr_perform_classmark_sending(RR_CLASSMARK_CLIENT_ECSC);

#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
  if (rr_gprs_edtm_cs_establishment_in_progress())
  {
    rr_gprs_edtm_complete_cs_establishment();

    if (rr_ce_conn_is_requested_by_nas())
    {
 ...
 *
 */
boolean rr_ce_can_process_ps_abort( )
{
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
  ...
  {
    if (rr_ce_for_gprs_access() && rr_timer_is_running(GRR_GUARD_TIMER))
    {
...
)
{
  ...
  {
    rr_send_abort_ind(RR_abort_cause, MM_fail_cause);
  }
...
static void rr_ce_handle_scell_dsf_event( const rr_event_T con_est_event )
{
  if (con_est_event != EV_DSF_FOR_DENIALS)
  {   
    rr_add_forbidden_cell(rr_gprs_get_scell_arfcn());
...
static void rr_start_t3126( )
{
  if (!rr_timer_is_running(T3126))
  {
    gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
    ...
    (void)rr_timer_start(T3126,(dword)(t3126_timeout_ms));
  }
 ...
 * @return VOID
 */
static void rr_reset_t3122_info( void )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
)
{
  ...
  rr_get_ms_capabilities(&rr_ms_capability, &rr_hr_data_supported);
  ...
  else if (access_reason == (byte) MM_RR_CALL_REEST)
  {
    ...
    else /* num_of_ded_channels != 1 or 2 */
    {
      ...
      MSG_GERAN_LOW_1_G("num_of_ded_channels=%d",(int)num_of_ded_channels);
    }
  } /* MM_RR_CALL_REEST */
...
static l2_channel_type_T rr_ce_get_rach_channel_type( )
{
  ...
  if (rr_ce_for_gprs_access())
  {
    if ((rr_conn_est_info_ptr->channel_request_counter == 1) &&
        (rr_ce_get_pkt_est_cause() != RR_PKT_EST_CAUSE_PAGE_RESP))
    {
 ...
 * See GSM 04.18 9.1.8
 */
static void rr_ce_initiate_random_access( )
{
  rr_event_T guard_timer_expiry_event = EV_NO_EVENT;
  byte       access_reason;
  byte       access_reference = 0x00;
  byte       rand_length = 0;
  ...
  rr_conn_est_info_ptr->t3122_t3142_timeout_value = 0;
  ...
  else
  {
    ...
    /*************************************************************/
    access_reference = rr_ce_generate_access_reference(access_reason, &rand_length);
    ...
    case MM_RR_MO_FULL_SPEECH:
      geran_set_traffic_reason(GERAN_TRAFFIC_CS_VOICE);
    ...
    case MM_RR_MO_DATA_9600:
      geran_set_traffic_reason(GERAN_TRAFFIC_CS_DATA);
    ...
    case MM_RR_SUPPLEMENTARY_SERVICE:
      geran_set_traffic_reason(GERAN_TRAFFIC_CS_SIGNALLING);
    ...
    default:
      geran_set_traffic_reason(GERAN_TRAFFIC_DEFAULT);
  ...
  {
    rr_hz_notify_state_change(RR_HZ_STATE_CS_CONNECTION);
  }
#endif /* FEATURE_GERAN_CS_SUPPORT */
  ...
  grr_start_guard_timer(GRR_TIMEOUT_1_SECOND * 2, guard_timer_expiry_event);
  ...
  rr_gprs_odm_info_rach_attempt();

  return;
} /* rr_ce_initiate_random_access() */

static boolean rr_ce_max_rach_bursts_sent( void )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
...
)
{
  ...
  if (rr_ce_max_rach_bursts_sent() == FALSE)
  {
    word random;
    ...
    if (rr_conn_est_info_ptr->egprs_access_valid)
    {
      ...
      switch (rr_conn_est_info_ptr->random_reference_length)
      {
        case 3:
          ...
          if (rr_ue_in_peo_mode())
          { 
            egprs_access_ref &= 0xFE3;
            random = random & 0x7;
            egprs_access_ref |= random << 2;
          }
          else
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/ 
          {
        ...
        default:
          MSG_GERAN_LOW_1_G("Bad EGPRS rand ref len %d", rr_conn_est_info_ptr->random_reference_length);
          break;
      }
...
)
{
  ...
  first_msg_length = rr_ce_store_first_message(message_ptr);
  ...
  /*******************************************/
  access_result = rr_check_access_class_for_raching(access_reason);
 ...
 )
{
  ...
  chan_spec_ptr = rr_get_channel_spec(dedic_ptr);
  ...
  rr_ciphering_has_started(FALSE);

#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
  if (RR_CE_EDTM_REQUEST(rr_conn_est_info_ptr->request_references[0]))
  {
    rr_gprs_edtm_store_channel_needed(channel_info_after_ptr->channel_type);
  }
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
...
)
{
      ...
      (first_IE_ptr      != NULL))
  {
    ...
    if (after_params_ptr->hopping_flag == FALSE)
    {
      ...
      if (rr_gapi_is_supported_band_dyn((sys_band_T)cs_band) == FALSE)
      {
    ...
    if (is_for_mobile == TRUE)
    {
      ...
      if ((rr_channel_type_supported(after_params_ptr->channel_type) == FALSE) ||
          (decoding_ok == FALSE))
      {
...
)
{
  ...
  current_IE = rr_get_first_ie_of_message(IMMEDIATE_ASSIGNMENT);
  ...
  {
#ifdef FEATURE_GERAN_CS_SUPPORT
    if (grr_cs_conn_est_in_progress())
    {
    ...
    if (rr_macinfo_get_mac_status() == MAC_TRANSFER) /* GPRS transfer in progress */
    {
...
#endif /* FEATURE_GSM_GPRS_READ_CCCH_IN_XFER */
    {
      ...
      if ( (rr_conn_est_info_ptr->waiting_for_2nd_msg == TRUE &&
                 rr_timer_is_running(TMULTIFRM) == FALSE) )
      {
      ...
      if ((rr_conn_est_info_ptr->waiting_for_2nd_msg == TRUE) && 
          (rr_timer_is_running(TMULTIFRM) == FALSE) && 
          (grr_return_tbf_data_structure_pointer()== TRUE)
         )
      {
...
)
{
  ...
  uint8             feature_indicator = 0x00;
  boolean           implict_ps_reject = FALSE;
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(IMMEDIATE_ASSIGNMENT_EXTENDED);
  ...
  //Extract the Feature indicator 
  feature_indicator = (uint8)(*(current_IE->start_IE) & 0xF0);
  ...
  if (rr_ue_in_peo_mode())
  {
     ...
     iax_rest_octets_t  iax_rest_octets;
      uint8             peo_bcch_change_mark = 0; 
     uint8 new_rcc = INVALID_RCC;
     peo_bcch_change_mark = (uint8)(feature_indicator >> 6);

     temp_IE_ptr = current_IE;

     /* advance to the IA Rest Octets IE*/
     temp_IE_ptr = temp_IE_ptr->next_IE; /*Channel description 1*/
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Request Ref 1 */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Timing advance 1 */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Channel description 2 */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Request Ref 2 */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Timing advance 2 */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Mobile allocation */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* Starting time */
     temp_IE_ptr = temp_IE_ptr->next_IE; /* IAX rest octet */


     remaining_length = rr_length_to_end_of_ota_msg(
                             IMMEDIATE_ASSIGNMENT_EXTENDED, // msg_type
                             temp_IE_ptr->start_IE, // csn1_start_ie_ptr
                             IAX_REST_OCTETS // csn_ie_type
                            );

     (void)gcsn1_rr_decoder(
            IAX_REST_OCTETS,  // message_type
            temp_IE_ptr->start_IE,   // csn1_ie_src
            &iax_rest_octets,
            remaining_length   // len_to_end_of_buf
            );

     /*Act on  PEO BCCH CHNAGE MARK and RCC if required */
     if (iax_rest_octets.r13_addition_flag)
     {
       new_rcc = iax_rest_octets.rcc;
     }

#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
    if (rr_nv_is_peo_testing_params_present())
    {
      new_rcc = rr_nv_get_peo_rcc_from_efs();
      peo_bcch_change_mark =  rr_nv_get_peo_bcch_change_mark_from_efs();
    }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/ 

     if (rr_process_rcc_or_bcch_change(new_rcc, peo_bcch_change_mark))
     {
       memset(&iax_rest_octets, 0, sizeof(iax_rest_octets_t));
       return EV_NO_EVENT;
     }  
  ...
  if ( IS_GPRS_ACCESS_REASON(rr_conn_est_info_ptr->access_reason))
  {    
    implict_ps_reject    = (boolean)((feature_indicator >> 4) & 0x01);

    /* Update the Implicit PS reject */
    rr_ps_update_implicit_reject(implict_ps_reject);   
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
    if (rr_nv_is_ir_eab_testing_params_present()) 
    {
      implict_ps_reject = rr_nv_get_implicit_reject_during_rach_from_efs();
    }
#endif /*FEATURE_GERAN_R13_TEST_FRAMEWORK*/

    if (implict_ps_reject)
    {      
      if (rr_ps_is_low_priority())
      {
        if (grr_tbf_est_in_progress())
        {
          rr_ce_send_imsg_ps_ir_rcvd_ind();
        }
      }
      else
      {
        MSG_GERAN_HIGH_0_G("PS Implict Reject ignored as UE in high priorty");
      }
...
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
        rr_classmark_set_irat_ho_info_compressed(irat_ho_info_compressed);
#endif /*  FEATURE_WCDMA || FEATURE_GSM_TDS*/
...
)
{
  ...
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
   ...
   /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(IMMEDIATE_ASSIGNMENT_REJECT);
  ...
  // Extract the PEO BCCH change mark 
  ...
  if (rr_ue_in_peo_mode())
  {
     ...
     /*Act on  PEO BCCH CHNAGE MARK and RCC if required */
     if (rr_process_rcc_or_bcch_change(iar_rest_octets.rcc, peo_bcch_change_mark))
     {
  ...
  if (!rr_in_idle)
  {
    ...
    for (request_index = 0; request_index < 4; request_index++)
    {
      ...
      if (match_reference == TRUE)
      {
        ...
        {
          rr_conn_est_info_ptr->t3122_t3142_timeout_value = *(current_IE->start_IE);
          ...
          {
            grr_start_t3146();
            grr_start_t3142(rr_conn_est_info_ptr->t3122_t3142_timeout_value);
          }
#ifdef FEATURE_GERAN_CS_SUPPORT
          else
          {
            rr_start_t3126();
            rr_start_t3122(rr_conn_est_info_ptr->t3122_t3142_timeout_value, rr_conn_est_info_ptr->access_reason);
          }
#endif /* FEATURE_GERAN_CS_SUPPORT */
        }
...
static char * rr_con_est_state_name(rr_con_est_state_T state)
{
  switch (state)
  {
    ...
    case RR_CP_IMPLICT_REJECT_RECEIVED   :       return "IMPLICT_REJECT_RECEIVED";
...
)
{
  ...
  if (release_cause == RELEASE_CAUSE_IMPLICIT_NAS_REQUEST)
  {
    if (can_return_to_idle)
    {
      ...
      if (grr_examine_saved_mm_message(&message_id))
      {
        switch (message_id)
        {
          case RR_EST_REQ:
  ...
  else if (TRUE == rr_static_get_suspend_in_progress())
  {
    MSG_GERAN_MED_0_G("DMMSUSREM:: CE rr_conn_inform_mm_of_channel_release");
 ...
 *
 */
static void rr_ce_l1_in_dedicated_mode(rr_l1_message_id_enum_T message_id )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
...
#ifdef FEATURE_GSM_EDTM
  rr_gprs_edtm_dedicated_mode_started(message_id);
#endif /* FEATURE_GSM_EDTM */
...
void rr_initialise_connection_establishment( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
boolean rr_ce_can_process_mm_message( )
{
...
boolean rr_ce_can_process_rr_grr_gprs_ia_ind( )
{
  boolean process = TRUE;
...
/*!
 * \brief Indicates  the access cause is page response.
 * 
 * \return TRUE is YES , FALSE otherwise
 */

static boolean rr_ce_is_pkt_est_cause_for_page_rsp( )
{

  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);

  if (RR_PKT_EST_CAUSE_PAGE_RESP == rr_conn_est_info_ptr->pkt_est_cause) 
  {
    return TRUE;
  }

  return FALSE;
} /* rr_ce_is_pkt_est_cause_for_page_rsp */
...
)
{
  ...
  while (con_est_event != EV_NO_EVENT)
  {
    ...
    if (con_est_event == EV_RESET_SOFTWARE)
    {
      ...
      rr_conn_est_info_ptr->immd_assign_rej_counter = 0;
      rr_gprs_notify_sys_info_ia_reject(FALSE);
    }
    else if (con_est_event == EV_GPRS_RESET_RR_CE_STATE)
    {
      ...
      if (rr_timer_cancel(TMULTIFRM))
      {
        MSG_GERAN_HIGH_0_G("Stopped TMULTIFRM timer");
        rr_conn_est_info_ptr->waiting_for_2nd_msg = FALSE;
        rr_gprs_two_part_assignment_failure();
    ...
    else if (con_est_event == EV_INPUT_MESSAGE)
    {
      if (message_ptr != NULL)
      {
        ...
        if ( ( message_set == MS_MM_RR ) && ( message_id == (byte) RR_ABORT_REQ ) )
        {
          ...
          if (RR_CE_EDTM_REQUEST(rr_conn_est_info_ptr->request_references[0]))
          {
            (void)rr_gprs_edtm_control(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
          }
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
    ...
    switch(rr_conn_est_info_ptr->con_est_state)
    {
      ...
      case RR_CP_INACTIVE :
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          ...
          else if ((message_set == MS_MM_RR) && (message_id == (byte) RR_EST_REQ))
          {
            ...
            /******************************************************/
            new_con_est_event = rr_ce_process_est_req(&message_ptr->rr_est_req, NULL);

            #ifdef FEATURE_INTERRAT_REDIRECTION_WTOG
            (void)rr_timer_cancel(RR_W2G_SERVICE_REDIRECTION_WAIT_TIMER);
        ...
        else if (con_est_event == EV_INIT_ACCESS_PROCEDURE)
        {
          ...
          /* been started                                                  */
          /*****************************************************************/
          if (!rr_is_control_state(RR_CONNECTION_PENDING))
          {
          ...
          {
            rr_ce_send_omsg_x2g_cco_status_ind(RR_CCO_STATUS_SUCCESS);
          }
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
        ...
        else if (con_est_event == RR_EV_MM_REQ_RECEIVED_ABORT)
        {
          ...
          if (rr_ce_for_gprs_access() == FALSE)
          {
            if (rr_x2g_cco_in_progress() == FALSE)
            {
          ...
          new_con_est_event = rr_ce_complete_mm_abort_processing(TRUE);
        }
#endif /* FEATURE_GERAN_CS_SUPPORT */
        ...
        else if (con_est_event == EV_EDTM_DTM_ASSIGNMENT_RECEIVED)
        {
          ...
          rr_ce_l1_in_dedicated_mode(MPH_DTM_CHANNEL_ASSIGN_CNF);
        ...
        else if (con_est_event == EV_T3174_EXPIRY)
        {
          if (rr_x2g_cco_in_progress())
          {
            rr_ce_send_omsg_x2g_cco_status_ind(RR_CCO_STATUS_FAILURE_CCO_TIMER_EXPIRY);
          }
        }
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
      ...
      case RR_CP_SENDING_RANDOM_ACCESS:
      {
          ...
          if (message_set == MS_RR_RR)
          {
            switch (message_ptr->rr.header.rr_message_set)
            {
              ...
              case RR_PS_OMSG:
              {
                switch (message_ptr->rr.header.imh.message_id)
                {
                  case RR_PS_OMSG_NO_PS_ACCESS_CNF:
                  {
                    ...
                    rr_conn_est_failed(FALSE);
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
                    if (rr_conn_est_info_ptr->return_rr_event != EV_BCCH_CHANGE_DETECTED)
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
              ...
              case RR_CE_IMSG:
              {
                switch (message_ptr->rr.header.imh.message_id)
                {
                  case RR_CE_IMSG_PS_IR_RCVD_IND:
                  {                    
                    if (rr_ce_is_pkt_est_cause_for_page_rsp())
                    {
                      MSG_GERAN_HIGH_0_G("Implicit Reject ignored for Page response");
                    }
                    else
                    {                      
                      new_con_est_event = EV_IMPLICIT_REJECTION;
                      rr_conn_est_info_ptr->con_est_state = RR_CP_IMPLICT_REJECT_RECEIVED;
                    }
                    break;                    
                  }
                  default:
              ...
              {
                new_con_est_event = rr_reduced_mobility_control(EV_INPUT_MESSAGE, message_ptr);
                break;
              }

              case RR_RESEL_RMC_OMSG:
              {
                switch (message_ptr->rr.resel_rmc.imsg.header.imh.message_id)
                {
                  case RR_RESEL_RMC_OMSG_FIND_CELL_IND:
                  {
                    MSG_GERAN_HIGH_0_G("RCC changed while waiting for IA, for reselection");
                    new_con_est_event = EV_DSF_RESELECTION;
                    break;
                  }
                  default:;
                }
          ...
          if (message_set == MS_RR_L2)
          {
            ...
            else if (message_id == (byte) DL_RANDOM_ACCESS_CNF)
            {
              ...
              if (GERAN_ACCESS_GRANTED != cnf_ptr->result)
              {
                ...
                new_con_est_event = rr_ce_handle_access_failure(cnf_ptr);
              }
              else
              {
                /************************************************************/
                ...
                /************************************************************/
                new_con_est_event = rr_ce_handle_access_confirm(cnf_ptr);
              }
            } /* DL_RANDOM_ACCESS_CNF */
        ...
        else if((con_est_event == EV_DSF_RESELECTION) || (con_est_event == EV_DSF_FOR_DENIALS))
        {
          ...
          if (rr_timer_is_running(T3174))
          {
            ...
            rr_ce_handle_scell_dsf_event(con_est_event);
        ...
        else if (con_est_event == EV_CONNECTION_ABORTION)
        {
          ...
          if (RR_CE_EDTM_REQUEST(rr_conn_est_info_ptr->request_references[0]))
          {
           ...
           (void)rr_gprs_edtm_control(EV_CONNECTION_ABORTION, NULL);
          }
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
        ...
        else if ( con_est_event == EV_T3174_EXPIRY )
        {
          ...
          if (rr_ce_for_gprs_access())
          {
            RR_ASSERT_WARN(rr_timer_is_running(T3146) == FALSE);
          ...
          {
            RR_ASSERT_WARN(rr_timer_is_running(T3126) == FALSE);
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
        else if (con_est_event == EV_BCCH_CHANGE_DETECTED)
        {          
          if (rr_ce_for_gprs_access())
          {
            MSG_GERAN_HIGH_0_G("PEO BCCH  changed while waiting for IA, abort PS procedure");
            if (rr_timer_cancel(T3176))
            {
              MSG_GERAN_HIGH_0_G("Stopped T3176 timer");
            }
          ...
          {
            MSG_GERAN_ERROR_0_G("Incorerct event detected, reset RMC software");
      ...
      case RR_CP_MAX_RETRIES_ATTEMPTED :
      {
        ...
        else if (con_est_event == EV_T3126_EXPIRY)
        {
          ...
          else
          {
            ...
            if(rr_ce_access_reason() == MM_RR_EMERGENCY_CALL)
            {
            ...
            /* store the reselection trigger */
            rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_T3126_TIMEOUT);
...
#endif 
            if (rr_timer_has_expired(GRR_SURROUND_MEAS_TIMER))
            {
      ...
      /********************************************************/
      /********************************************************/
      /* For case RR_CP_IMPLICT_REJECT_RECEIVED RR received an IR */
      /* and is waiting for either the network to send an     */
      /* immediate assignment message for the mobile or for   */
      /* T3146 to expire                                      */
      /********************************************************/
      ...
      case RR_CP_IMPLICT_REJECT_RECEIVED:
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          if ((message_set == MS_RR_L2) && (message_id == (byte) DL_UNIT_DATA_IND))
        ...
        else if((con_est_event == EV_DSF_RESELECTION) || (con_est_event == EV_DSF_FOR_DENIALS))
        {
          ...
          else
          {
            ...
            if (rr_ce_for_gprs_access())
            {
              ...
              /* Cancel T3236, if it's running */
              if (rr_timer_cancel(T3236))
              {
                MSG_GERAN_HIGH_0_G("Stopped T3236 timer");
                rr_conn_est_info_ptr->implicit_reject = FALSE;
              }
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/
        ...
        else if (con_est_event == EV_GPRS_IMMEDIATE_ASSIGNMENT)
        {
          ...
          MSG_GERAN_HIGH_0_G("Stopping T3236 if running");
        ...
        else if (con_est_event == EV_T3126_EXPIRY)
        {
          ...
          /*******************************************************************/
          rr_gprs_notify_sys_info_ia_reject(TRUE);
        ...
        else if (con_est_event == EV_T3146_EXPIRY)
        {
          ...
          {
            delete_pdu_cause_t del_pdu = DEL_ONE;
...
#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
            if (rr_conn_est_info_ptr->implicit_reject == TRUE)
            {
              uint32 rand_value = rr_get_T3236_value(); 
              if (rr_timer_start(T3236, rand_value))
              {        
                MSG_GERAN_HIGH_1_G("Started T3236 for %dms", rand_value);
                rr_send_ps_implicit_reject_ind(rand_value); 
              }
              rr_conn_est_info_ptr->implicit_reject = FALSE;
            }
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/

            if (rr_conn_est_info_ptr->t3122_t3142_timeout_value < IA_REJ_WAIT_THRES)
            {
              del_pdu = DEL_ONE_SIG;
            }
        ...
        /*****************************************************************/
        /* EV_IMPLICIT_REJECTION                                              */
        /*****************************************************************/
        else if (con_est_event == EV_IMPLICIT_REJECTION)
        {
          /****************************************************************/
          /* RR should not send any more RACH messages after the Implicit Reject is */
          /* received. Ask L1 to abort any that are still pending.        */
          /****************************************************************/
          rr_send_mph_abort_ra_req();
          grr_start_t3146();        
          rr_conn_est_info_ptr->implicit_reject = TRUE;
        }
        else
        {
          RR_CE_LOG_UNEXPECTED_EVENT(con_est_event);
        }
      } /* RR_CP_ASSIGNMENT_REJECTION_RECEIVED|| RR_CP_IMPLICT_REJECT_RECEIVED */
      break;
      ...
      case RR_CP_AWAITING_PHYSICAL_LINK:
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          ...
          if ((message_set == MS_RR_L1) && (message_id == (byte) MPH_IMMED_ASSIGN_CNF))
          {
            ...
            {
              /* L1 has successfully set up the physical channel   */
              rr_ce_l1_in_dedicated_mode(MPH_IMMED_ASSIGN_CNF);
        ...
        else if (con_est_event == EV_T3174_EXPIRY)
        {
          ...
          if (rr_x2g_cco_in_progress())
          {
            rr_conn_set_release_cause(RELEASE_CAUSE_X2G_CCO_FAILURE);
          }
          else
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
          {
            rr_conn_set_release_cause(RELEASE_CAUSE_PCCO_FAILURE);
          }
        ...
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
          if (message_set == MS_RR_L2)
          {
            if (message_id == (byte) DL_ESTABLISH_CNF)
            {
              ...
              /**********************************************/
              new_rr_event = rr_ce_handle_communication_establishment();
              ...
              */
              if (grr_determine_if_gprs_suspension_should_be_sent_ota())
              {
            ...
            else if (message_id == (byte) DL_RELEASE_IND)
            {
              ...
              if (rr_x2g_cco_in_progress())
              {
                ...
                rr_ce_perform_ia_connection_release(FALSE);
                rr_conn_est_info_ptr->repeat_assignment_procedure = FALSE;
              }
              else
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
              ...
              else /* connection establishment procedure has not been repeated */
              {
                 ...
                 */
                rr_ce_perform_ia_connection_release(TRUE);
...
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
              {
...

                {
                  /*flush the SI cache as the L2 link failed and this might be due to stale SI data*/
                   ...
                   {
                     MSG_GERAN_LOW_0_G("L2 link failed flush the SI cache");
              ...
              else /* IA procedure should not be repeated */
              {
                if (rr_timer_is_running(T3174))
                {
                  if (rr_x2g_cco_in_progress() == FALSE)
                  {
                    ...
                    rr_send_abort_ind(rr_conn_est_info_ptr->release_reason, MM_RR_OTHER_FAILURE);
                  }
      ...
      case RR_CP_IA_ABORTED:
      {
        if ((con_est_event == EV_INPUT_MESSAGE) && (message_ptr != NULL))
        {
              ...
              case RR_L1_IDLE_OMSG:
              {
                switch (message_ptr->rr.header.imh.message_id)
                {
                  case RR_L1_IDLE_OMSG_START_IDLE_CNF:
                  {
                    ...
                    if (rr_ce_should_confirm_conn_rel_to_nas())
                    {
                      rr_send_service_ind(
                        MM_AS_SERVICE_AVAILABLE,
                        &(scell_info_ptr->gsm.location_area_identification),
          ...
          if ((message_set == MS_RR_L1) && (message_id == (byte) MPH_CHANNEL_RELEASE_CNF))
          {
            if (rr_x2g_cco_in_progress() || rr_ce_pcco_in_progress())
            {
              ...
              else
              {
                ...
                else /* RR was not able to camp on serving cell */
                {
                  ...
                  new_con_est_event = rr_ce_complete_mm_abort_processing(FALSE);
                }
              }
            }
            else
        ...
        else if ( con_est_event == EV_REPEAT_ASSIGNMENT_PROCEDURE )
        {
          ...
          rr_conn_inform_mm_of_channel_release(TRUE);
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
#ifdef FEATURE_SGLTE
                      if (rr_x2g_cco_in_progress() && !rr_is_sglte_mode_active())
#else
                      if (rr_x2g_cco_in_progress())
#endif /* FEATURE_SGLTE */
                      {
                        ...
                        rr_ce_send_omsg_x2g_cco_status_ind(rr_conn_est_info_ptr->x2g_cco_status);
                      }
                      else
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS || FEATURE_LTE */
                      {
              ...
              case MS_RR_L2:
              {
                switch (message_id)
                {
                  case DL_UNIT_DATA_IND:
                  {
                     ...
                     */
                    MSG_GERAN_LOW_0_G("L2 message ignored - returning to Packet Idle");
                    break;
                  } // case DL_UNIT_DATA_IND
                ...
                switch (message_ptr->rr.header.rr_message_set)
                {
                  case RR_PS_OMSG:
                  {
                    switch (message_ptr->rr.header.imh.message_id)
                    {
                      case RR_PS_OMSG_NO_PS_ACCESS_CNF:
                      {
                        if (rr_timer_is_running(T3142))
                        {
                          MSG_GERAN_HIGH_0_G("T3146 expiry after IAR - T3142 running");
                        }

#ifdef FEATURE_GERAN_IMPLICIT_PS_REJECT
                        else if (rr_timer_is_running(T3236))
                        {
                          MSG_GERAN_HIGH_0_G("T3146 expiry after Implict Reject - T3236 running");
                        }
#endif /*FEATURE_GERAN_IMPLICIT_PS_REJECT*/

                        else
                        {
                          MSG_GERAN_HIGH_0_G("T3146 expiry after IAR - no T3142 or T3236");
                        }
...
)
{
  if (t3122_timeout_secs != 0)
  {
    ...
    // Obtain a pointer to the module data
    rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
    //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
    ...
    (void)rr_timer_start(T3122,(dword)t3122_timeout_ms);
...
static boolean rr_check_t3122_running( )
{
  boolean t3122_running = rr_timer_is_running(T3122);
  ...
  {
    MSG_GERAN_MED_0_G("T3122 not running");
  }
...
boolean rr_get_t3122_running_for_nas( )
{
#ifdef FEATURE_GERAN_CS_SUPPORT
  boolean t3122_running = rr_check_t3122_running();
...
#ifdef FEATURE_GERAN_CS_SUPPORT
void rr_check_nas_t3122_status( )
{
  boolean t3122_running = rr_check_t3122_running();
 ...
 * \param requested_by_nas (in) - TRUE if NAS requested this connection, FALSE if not (e.g. E-DTM IA)
 */
void rr_ce_init_conn_info_after_x2g_ho( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
 ...
 */
boolean rr_ce_conn_is_requested_by_nas( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
 ...
 */
boolean rr_ce_should_confirm_conn_rel_to_nas( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
 ...
 */
boolean rr_ce_conn_originated_on_other_rat( )
{
  // Obtain a pointer to the module data
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);
...
/*!
 * \brief Indicates  the access cause used while sending RACH.
 * 
 * \return access cause
 */

uint8 rr_ce_determine_pkt_est_cause_for_peo( )
{
  // Obtain a pointer to the module data
  uint8 access_cause= 0x03;
  rr_conn_est_info_T *rr_conn_est_info_ptr = rr_conn_est_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_conn_est_info_ptr);

  switch (rr_conn_est_info_ptr->pkt_est_cause)
  {
    case RR_PKT_EST_CAUSE_PAGE_RESP:
    case RR_PKT_EST_CAUSE_CELL_UPD:
    case RR_PKT_EST_CAUSE_MM:
    {
      MSG_GERAN_HIGH_0_G("Access cause is SIGNALLING");
      access_cause = 0x00;
      break;
    }    
    /*TO DO new cause for SINGLE BLOCK si it require d ??*/
    case RR_PKT_EST_CAUSE_USER_DATA:
    {      
      MSG_GERAN_HIGH_0_G("Access cause is User Data ");
      access_cause = 0x02;
      break;
    }
