...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_g2x.c#2 $ */
...
static rr_g2x_data_t   rr_g2x_data;
 ...
 */
static rr_g2x_data_t *rr_g2x_get_data_ptr(void)
{
  return &rr_g2x_data;
}
...
)
{
  ...
  rr_g2x_data_ptr = rr_g2x_get_data_ptr();
...
)
{
  ...
  if (list_size(&rr_g2x_data_ptr->barred_frequency_db) == RR_G2X_BARRED_FREQUENCY_DB_SIZE)
  {
    ...
    if (entry != NULL)
    {
      (void) rr_timer_cancel(entry->timer);
    }
    else
    {
  ...
  else
  {
    ...
    entry->timer = rr_g2x_barred_frequency_db_allocate_timer();
  ...
  rr_timer_start(entry->timer, time_to_reselect);
...
*/
static void rr_g2x_barred_frequency_db_dump( )
{
  uint32                        i = 0;
  rr_g2x_barred_frequency_db_entry_t * entry;
  rr_g2x_data_t *rr_g2x_data_ptr;
...
)
{
  ...
  else
  {
    ...
    entry->timer = rr_g2x_barred_cell_db_allocate_timer();
  ...
  if (barred_time > 0)
  {
    rr_timer_start(entry->timer, barred_time);
  }
  else
  {
    switch (cell->rat)
...
static void rr_g2x_barred_cell_db_dump( )
{
  uint32                          i = 0;
  rr_g2x_barred_cell_db_entry_t * entry;
  rr_g2x_data_t *rr_g2x_data_ptr;
    ...
    switch (entry->cell.rat)
    {
      ...
      case RR_RAT_WCDMA:
      {
        MSG_5(MSG_SSID_DFLT,
              ...
              entry->reason,
              rr_timer_get_time_remaining(entry->timer));
        break;
      } /* RR_RAT_WCDMA */
#endif /* FEATURE_WCDMA */
...
)
{
  ...
  else
  {
    ...
    rr_g2x_barred_frequency_db_dump();
  }
}
#endif /* FEATURE_LTE */
...
{
  // Obtain a pointer to the module data
  rr_g2x_data_t *rr_g2x_data_ptr = rr_g2x_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_data_ptr);
...
*/
static void rr_g2x_cell_quality_data_dump( )
{
  // Obtain a pointer to the module data
  rr_g2x_data_t *rr_g2x_data_ptr = rr_g2x_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_data_ptr);
...
*/
void rr_g2x_barred_frequency_db_clear( )
{
  rr_g2x_barred_frequency_db_entry_t * entry;
  rr_g2x_data_t *rr_g2x_data_ptr;
...
*/
void rr_g2x_barred_cell_db_clear( )
{
  rr_g2x_barred_cell_db_entry_t * entry;
  rr_g2x_data_t *rr_g2x_data_ptr;
...
*/
void rr_g2x_cell_quality_data_clear( )
{
  // Obtain a pointer to the module data
  rr_g2x_data_t *rr_g2x_data_ptr = rr_g2x_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_data_ptr);
  ...
  switch (cause)
  {
    ...
    case RR_G2X_RESEL_FAILURE_CAUSE_FORBIDDEN_TA:
    {
      switch (cell->rat)
      {
        ...
        case RR_RAT_LTE:
        {
          rr_g2x_update_barred_db_after_forbidden_ta(cell, data);
          break;
         } /* RR_RAT_LTE */
#endif /* FEATURE_LTE */
    ...
    case RR_G2X_RESEL_FAILURE_CAUSE_CELL_BARRED:
    {
      ...
      The LTE/UMTS/TDSCDMA network actually provides the time to bar the LTE cell, so
...
)
{
  ...
  if (tdsrrc_is_arfcn_valid_sub_id(uarfcn))
#else
...
)
{
        ...
        cell) == NULL)
  {
    ...
    switch (cell->rat)
    {
      ...
      case RR_RAT_LTE:
      {
        resel_allowed = rr_g2x_resel_to_lte_cell_is_allowed(&cell->data.lte);
        break;
      }
#endif /* FEATURE_LTE */
      ...
      case RR_RAT_WCDMA:
      {
        resel_allowed = rr_g2x_resel_to_utran_cell_is_allowed(cell);
        break;
      }
#endif /* FEATURE_WCDMA */
...
*/
void rr_g2x_cell_selection_triggered( )
{
...
/**
  @brief Clear cell quality data and do anything that is done on normal
         cell selection.
*/
void rr_g2x_plmn_selection_triggered( )
{
...
boolean rr_g2x_lock_exchange_allowed( const rr_g2x_operation_type_e rr_g2x_operation_type, const rr_rat_e target_rat )
{
...
/**
  ...
  @brief Called when the Forbidden LAI List is updated, this function will
         ...
         no longer forbidden.
*/
void rr_g2x_forbidden_lai_list_updated( )
{
  rr_g2x_barred_frequency_db_entry_t *entry;
  ...
  while (entry != NULL)
  {
    ...
    if (entry->reason == RR_G2X_RESEL_FAILURE_CAUSE_FORBIDDEN_LA &&
        !rr_is_forbidden_LAI(&entry->data.lai))
    {
      list_pop_item(&rr_g2x_data_ptr->barred_frequency_db, &entry->list_link);
      (void) rr_timer_cancel(entry->timer);
      GPRS_MEM_FREE(entry);
...
*/
void rr_g2x_equivalent_plmn_list_updated( )
{
  rr_g2x_barred_frequency_db_entry_t *entry;
  boolean                             barred_frequency_db_updated = FALSE;
  rr_g2x_data_t                      *rr_g2x_data_ptr;
  ...
  while (entry != NULL)
  {
    ...
    if (entry->reason == RR_G2X_RESEL_FAILURE_CAUSE_PLMN_MISMATCH)
    {
      ...
      database. */
      else if (rr_eplmn_list_equivalent_plmn(scell->gsm.location_area_identification.plmn_id) &&
               rr_eplmn_list_equivalent_plmn(entry->data.plmn_id))
      {
        list_pop_item(&rr_g2x_data_ptr->barred_frequency_db, &entry->list_link);
        (void) rr_timer_cancel(entry->timer);
        GPRS_MEM_FREE(entry);
...
*/
void rr_g2x_csg_white_list_updated( )
{
  rr_g2x_barred_cell_db_entry_t *entry = NULL;
