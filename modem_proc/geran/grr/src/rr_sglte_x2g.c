...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sglte_x2g.c#2 $ */
...
static rr_sglte_x2g_data_t * rr_sglte_x2g_data_ptrs;

/*----------------------------------------------------------------------------
 * Static Function Declarations and Definitions
 *--------------------------------------------------------------------------*/

static rr_sglte_x2g_data_t * rr_sglte_get_x2g_data_ptr( void )
{
  return(rr_sglte_x2g_data_ptrs);
}
...
void rr_sglte_x2g_init( void )
{
  ...
  rr_sglte_x2g_data_ptrs = local_ptr;

  return;
}

void rr_sglte_x2g_deinit( void )
{

  GPRS_MEM_FREE(rr_sglte_x2g_data_ptrs);
  rr_sglte_x2g_data_ptrs = NULL;

  return;
}

void rr_sglte_x2g_control(rr_cmd_bdy_type *message  )
{
  rr_sglte_x2g_data_t *rr_sglte_x2g_data_ptr = rr_sglte_get_x2g_data_ptr();
            ...
            case RR_SGLTE_X2G_IMSG:
            {
              switch (message->message_header.message_id)
              {
                case RR_SGLTE_X2G_IMSG_RESEL_REQ:
                {
                  ...
                  rr_send_activation_ind(resel_req->activation_reason);
                ...
                {
                  rr_sglte_x2g_send_omsg(RR_SGLTE_X2G_OMSG_ABORT_CNF);
                  break;
                }
            ...
            case RR_SGLTE_X2G_IMSG:
            {
              switch (message->message_header.message_id)
              {
                ...
                case RR_SGLTE_X2G_IMSG_REJECT_REQ:
                {
                  ...
                  rr_sglte_x2g_data_ptr->state = NULL;
                  rr_sglte_x2g_send_omsg(RR_SGLTE_X2G_OMSG_REJECT_CNF);
				  break;
                }
        ...
        case MS_MM_RR:
        {
          switch (message->message_header.message_id)
          {
            case RR_ACTIVATION_RSP:
            {
              ...
              rr_sglte_dedicated_priority_info_store(&rr_sglte_x2g_data_ptr->lte_dedicated_pri);


              if (rr_gsm_only() == FALSE)
              {
              ...
              else
              {
                ...
                rr_sglte_x2g_send_omsg(RR_SGLTE_X2G_OMSG_RESEL_CNF);
              }              
              break;
            } /* RR_ACTIVATION_RSP */
            ...
            case MPH_UE_MODE_CHANGE_CNF:
            {
...
#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
              {
                ...
                if ( (scell_info_ptr->gsm.two_quater_position == SI2Q_POSITION_NONE) ||
                     (rr_si2quater_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE))) )
                {
