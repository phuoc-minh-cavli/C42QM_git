...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_pscan.c#13 $ */
/* $DateTime: 2021/01/27 23:18:37 $$Author: pwbldsvc $ */
...
static rr_pscan_data_t          rr_pscan_data;
 ...
 */
rr_pscan_data_t *rr_pscan_data_get_data_ptr( void )
{
  return &rr_pscan_data;
}
...
)
{
  ...
  rr_pscan_data_t *rr_pscan_data_ptr = rr_pscan_data_get_data_ptr();
...
)
{
  ...
  MSG_GERAN_LOW_0_G("PSCAN data capture enabled");
...
{
  MSG_GERAN_LOW_0_G("PSCAN data capture disabled");
...
)
{
      ...
      (rr_pscan_data_capture_data.buffer_ptr != NULL))
  {
    ...
    */
    ...
    {
      arfcn = (uint16) pscan_db_ptr->pscan_results.meas[pscan_db_index].arfcn_num;
      band = (uint8) pscan_db_ptr->pscan_results.meas[pscan_db_index].arfcn_band;
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_pscan_data_ptr = rr_pscan_data_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_pscan_data_ptr);
  ...
  {
    if (blocked_arfcn.num == rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j].arfcn_num)
    {
...
static rr_l1_band_bitmap_T rr_pscan_get_band_bitmap_for_arfcn(ARFCN_T arfcn)
{
  switch(arfcn.band)
  {
    ...
    default:
      MSG_GERAN_LOW_1("Unknown band %d in power scan setup", arfcn.band);
      break;
  }
...
)
{
  ...
  uint16                 index;
  rr_pscan_results_db_T *pscan_results_db_ptr = rr_get_pscan_results_db_ptr();

  for (index = 0; index < no_of_entries; index++)
  {
    pscan_results_db_ptr->pscan_results.meas[index].arfcn_num= arfcn_list[index].num;
    pscan_results_db_ptr->pscan_results.meas[index].arfcn_band= arfcn_list[index].band;
...
)
{
       ...
       counter++)
  {
    ...
    {
      word num = pscan_db_ptr->pscan_results.meas[counter + power_scan_offset].arfcn_num;
      rr_internal_band_T band = (rr_internal_band_T) pscan_db_ptr->pscan_results.meas[counter + power_scan_offset].arfcn_band;
      ...
      else if(band == BAND_DCS)
      {      
        dcs_counter++;
      }
      else if(band == BAND_PCS)
      {
        pcs_counter++;
      }
      else if(band == BAND_GSM_850)
      {
        g850_counter++;
      }
      else
...
)
{
  ...
  for (index = 0; index < no_of_entries; ++index)
  {
    ...
    {
      if (((pscan_db_ptr->pscan_results.meas[index].arfcn_num) ==
          frequency_list[listIndex].num)
          && ((pscan_db_ptr->pscan_results.meas[index].arfcn_band) ==
 ...
 */
void rr_invalidate_power_scan_results( )
{
  if (rr_timer_cancel(RR_INVALIDATE_PSCAN_RESULTS_TIMER))
  {
 ...
 *
 */
void rr_pscan_discard_common_results( )
{
  rr_pscan_data_t *rr_pscan_data_ptr;
 ...
 *
 */
static void rr_start_pscan_validity_timer( )
{
  rex_timer_cnt_type timeout_ms = rr_nv_get_pscan_results_reuse_time_millisecs();
...
)
{
  ...
  uint16                  num_freq_to_monitor = RR_NUM_FREQ_MONITOR;
  uint16                  pscan_result_offset;
  uint16                  num_frequencies_found = 0;
  ...
  if (candidate_db_ptr != NULL)
  {
    ...
    rr_set_num_frequencies_in_pscan(0);
    rr_reset_num_ps_cells_si_received();
#endif
    ...
    rr_pscan_data_capture(pscan_results_db_ptr, pscan_result_offset);
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("EGSM 900 and PGSM 900 specified in band_group, removing PGSM 900");
  ...
  if ((rr_gps_advance_search_in_progress()) &&
      rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.num_gsm_ad_element != 0 &&
      !(rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.is_gsm_blind_scan_req))
  {
    MSG_GERAN_HIGH_1_G("[GPS search] : ARFCN(s) in WWAN req = %d",
    ...
    for (i = 0;
         ...
         i++)
    {
       arfcn.num = rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.gsm_ad_elems[i].arfcn.num;
       arfcn.band = rr_plmn_list_req_info_ptr->gps_wwan_search_info.assist_param.wwan_rat_scan_param.gsm_params.gsm_ad_elems[i].arfcn.band;

      if(rr_is_frequency_band_supported(&arfcn, band_group) == TRUE)
      {
        MSG_GERAN_HIGH_2_G("[GPS search] : ARFCN (%d, %d) supported, hence adding to power scan request", arfcn.num, arfcn.band);

        pscan_results_db_ptr->pscan_results.meas[count].arfcn_num = arfcn.num;
        pscan_results_db_ptr->pscan_results.meas[count].arfcn_band = arfcn.band;
        count++;
      }
      else
      {
        MSG_GERAN_HIGH_2_G("[GPS search] : ARFCN (%d, %d) not supported", arfcn.num, arfcn.band);
      }
    }
  }
  else
      ...
      if (band_group & band_data->sys_band_mask)
      {
             ...
             arfcn.num++)
        {
          if (filter_acq_db_freqs && rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs_valid)
          {
            ...
            for (j = 0; j < rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.num_frequencies; j++)
            {
              if (arfcn.num == rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j].arfcn_num &&
                  arfcn.band == rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j].arfcn_band)
              {
                filter_freq = TRUE;
                break;
              }
...
)
{
  ...
  if (pscan_db->pscan_results.num_frequencies > 0)
  {
    ...
    {
       ARFCN_T arfcn;
       arfcn.num = pscan_db->pscan_results.meas[i].arfcn_num;
       arfcn.band = pscan_db->pscan_results.meas[i].arfcn_band;

    
      band_bitmap |= rr_pscan_get_band_bitmap_for_arfcn(arfcn);
    }
 ...
 *
 */
static boolean rr_pscan_find_valid_data_sources(boolean *data_sources_ptr)
{
 ...
 *
 */
boolean rr_pscan_arfcn_present(rr_l1_pscan_results_T *pscan_request_ptr, uint16 num_freq, ARFCN_T arfcn  )
{  
  word meas_index = 0;
  ...
  {
    if (arfcn.num == pscan_request_ptr->meas[meas_index].arfcn_num &&
        arfcn.band == pscan_request_ptr->meas[meas_index].arfcn_band)
    {
      match_found = TRUE;
      break;
    }
...
static boolean rr_pscan_find_meas(rr_l1_freq_power_T *meas_ptr, boolean *valid_data_sources)
{
  ...
  if (TRUE == *valid_data_sources)
  {
    /* get the results database pointer for this space and search it */
    results_db_ptr = &(rr_pscan_data.rr_pscan_results_db);
    if (results_db_ptr != NULL)
    {
      meas_count = results_db_ptr->pscan_results.num_frequencies;
      for (meas_index = 0; meas_index < meas_count; meas_index++)
      {
        if (meas_ptr->arfcn_num == results_db_ptr->pscan_results.meas[meas_index].arfcn_num &&
            meas_ptr->arfcn_band == results_db_ptr->pscan_results.meas[meas_index].arfcn_band)
...
void rr_pscan_prepare_common_results(mph_power_scan_req_T *req_ptr)
{
  ...
  if (!rr_pscan_find_valid_data_sources(&valid_data_sources))
  {
  ...
  if (rr_pscan_data_ptr->common_results.meas != NULL)
  {
    ...
    arfcn.num = arfcn_result.arfcn_num;
    arfcn.band = arfcn_result.arfcn_band;
    req_ptr->requested_bands = rr_pscan_get_band_bitmap_for_arfcn(arfcn);
...
)
{
  ...
  while (event != EV_NO_EVENT)
  {
    switch (rr_pscan_data_ptr->rr_pscan_control_data.state)
    {
                    ...
                    switch (message_ptr->rr.pscan.imsg.header.imh.message_id)
                    {
                      case RR_PSCAN_IMSG_ABORT_REQ:
                      {
                        ...
                        else if (rr_pscan_data_ptr->rr_pscan_control_data.search_mode == RR_SEARCH_BACKGROUND)
                        {
                          ...
                          rr_send_mph_power_scan_abort_req();
            ...
            switch (message_set)
            {
              case MS_RR_L1:
              {
                switch ((rr_l1_message_id_enum_T) message_id)
                {
                  case MPH_POWER_SCAN_CNF:
                  {
                    MSG_GERAN_LOW_0_G("Received MPH_POWER_SCAN_CNF while in PSCAN_ABORT");
#ifdef FEATURE_DUAL_SIM
      ...
      default:
        MSG_GERAN_LOW_2_G("UnexEV. Ev(%d), State(%d)",(int) event,(int)rr_pscan_data_ptr->rr_pscan_control_data.state);
        break;
    }
 ...
 * \return boolean
 */
boolean rr_pscan_data_capture_enabled( )
{
  return(rr_pscan_data_capture_data.enabled);
}
...
)
{
  ...
  if (rr_timer_is_running(RR_INVALIDATE_PSCAN_RESULTS_TIMER))
  {
    ...
    pscan_results_db_ptr = rr_get_pscan_results_db_ptr();
 ...
 */
boolean geran_start_capture_scan_data_dyn(uint32 *memptr, uint16 memsize)
{
  ...
  */
  ...
  {
    rr_pscan_enable_pscan_data_capture(memptr, max_num_of_entries);
 ...
 * \return boolean - TRUE if successful; FALSE otherwise
 */
boolean geran_stop_capture_scan_data_dyn(void)
{
...
#ifdef FEATURE_PS_PREFERRED_CELL
void rr_pscan_remove_unprocessed_freq_from_last_pscan(const rr_candidate_cell_db_T *candidate_db_ptr)
{
  uint16 index = 0;
  int16 j;
  rr_pscan_data_t *rr_pscan_data_ptr = rr_pscan_data_get_data_ptr();
  ...
  for (index = 0; index < candidate_db_ptr->num_frequencies; index++)
  {
    ...
    {
      for (j = rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.num_frequencies - 1; j >= 0; --j)
      {
        if ((candidate_db_ptr->candidate_cells[index].meas.arfcn_num == rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j].arfcn_num) &&
            (candidate_db_ptr->candidate_cells[index].meas.arfcn_band == rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j].arfcn_band))
        {
          uint16 size_to_move = sizeof(rr_l1_freq_power_T) *  (rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.num_frequencies - 1 - j);

          if (size_to_move > 0)
          {
            /* remove entry from list of last pscan frequencies so that they can be included in full band scan */
            memsmove(&rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j],
              size_to_move,
              &rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.meas[j+1],
              size_to_move);
          }

          rr_pscan_data_ptr->rr_pscan_last_acq_db_freqs.num_frequencies--;
          break;
        }
      }
    }
