...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_x2g_cgi.c#5 $ */
...
)
{
  ...
  event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
            ...
            case RR_X2G_CGI_IMSG:
            {
              switch (message->message_header.message_id)
              {
                case RR_X2G_CGI_IMSG_CGI_REQ:
                {
                  ...
                  if (time_available > 0)
                  {
                    rr_timer_start(RR_X2G_CGI_TIMER, time_available);
                    rr_x2g_cgi_activate_l1();
            ...
            case RR_PSCAN_OMSG:
            {
              switch (message->rr.header.imh.message_id)
              {
                case RR_PSCAN_OMSG_FINISHED_IND:
                {
                  ...
                  if (rr_x2g_cgi_select_first_arfcn())
                  {
                    rr_x2g_cgi_data_ptr->state = RR_X2G_CGI_STATE_SELECTING;
                  }
                  else
          ...
          switch (message->message_header.message_id)
          {
            case MPH_SELECT_SPECIFIC_BCCH_CNF:
            {
              if (message->mph.select_specific_bcch_cnf.BCCH_found)
              {
                ...
                {
                  uint32 notify_on_si_mask = 0;
                  uint32 wait_for_si_mask = 0;
                  ...
                  if ((rr_x2g_cgi_data_ptr->info.mask & RR_X2G_CGI_INFO_MASK_RXLEV) == 0)
                  {
                    ...
                    the CGI data. */
                    rr_x2g_cgi_data_ptr->cgi_arfcn.num = cell->arfcn_num;
                    rr_x2g_cgi_data_ptr->cgi_arfcn.band = cell->arfcn_band;
                ...
                else
                {
                  ...
                  if (rr_x2g_cgi_select_next_arfcn())
                  {
          ...
          switch (message->rr.header.rr_message_set)
          {
            ...
            case RR_X2G_CGI_IMSG:
            {
              switch (message->message_header.message_id)
              {
                ...
                default:
                {
                  rr_select_bcch_control(message);
                }
              }
              break;
            } /* RR_X2G_CGI_IMSG */
