...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_log.c#7 $ */
...
static rr_log_data_t rr_log_data;
 ...
 */
static rr_log_data_t *rr_log_get_data_ptr( void )
{
  return &rr_log_data;
}
...
static uint16 rr_arfcn_to_word(ARFCN_T arfcn)
{
   ...
   switch (ret_band)
   {
      ...
      default:
        MSG_GERAN_LOW_2("Unknown band value %d for ARFCN %d", ret_band, ret_arfcn);
        break;
   }
...
)
{
  ...
  switch (log_code)
  {
    ...
    {
      success = rr_log_gprs_grr_state(get_grr_state());
      break;
    }
    ...
    {
      success = rr_log_channel_configuration();
      break;
    }
    ...
    {
      success = rr_log_gprs_3g_reselection_measurements();
      break;
    }
...
)
{
  ...
  if (rr_log_on_demand_handler(log_code))
  {
 ...
 * \param 
 */
 void rr_log_on_demand_init( void )
{
  // Obtain a pointer to the module data
  rr_log_data_t *rr_log_data_ptr = rr_log_get_data_ptr();
 ...
 * \param 
 */
void rr_log_on_demand_deinit( void )
{
  // Obtain a pointer to the module data
  rr_log_data_t *rr_log_data_ptr = rr_log_get_data_ptr();
...
)
{
  ...
  if (NULL != log_ptr)
  {
    ...
    switch (state)
    {
      ...
      default:
        MSG_GERAN_LOW_1_G("State=%d not mapped to log packet",(int) state);
        break;
    } /* switch */
...
)
{
      ...
      != 0)
  {
    ...
    (void) rr_log_cell_selection_parameters(csp_ptr);
  }
...
)
{
  ...
  {
    // they are different, log the new values
    (void) rr_log_rach_control_parameters(rcp_ptr);
  }
...
)
{
  ...
  {
    // they are different, log the new values
    (void) rr_log_control_channel_description(ccd_ptr);
  }
...
)
{
  ...
  {
    // they are different, log the new values
    (void) rr_log_cell_options(cellop_ptr);
  }
 ...
 * \return boolean
 */
boolean rr_log_channel_configuration( void )
{
  ...
  if (NULL != log_ptr)
  {
    ...
    for (i=0; i < num_plmns; ++i)
    {
      ...
      for (j=0; j < num_cells; ++j)
      {
        ...
        meas_arfcn.num = plmn_info_ptr->cell_info[j].meas.arfcn_num;
        meas_arfcn.band = plmn_info_ptr->cell_info[j].meas.arfcn_band;

        entry_ptr->cells[j].ARFCN           = rr_arfcn_to_word(meas_arfcn);
 ...
 * \return boolean
 */
boolean rr_log_gprs_3g_reselection_measurements( void )
{
  ...
  if (NULL != log_ptr)
  {
    rr_irat_populate_gprs_3g_reselection_measurements_log_packet(log_ptr);
 ...
 * \param 
 */
void rr_log_rrc_interrat_reselect_rej( void )
{
...
)
{
  ...
  if (log_ptr != NULL)
  {
    ...
    {
      MSG_GERAN_LOW_2_G("index(%d) != packet_size(%d)",(int) index,(int) packet_size);
    }
    gsmdiag_log_commit(&pkt_info);
  }
...
)
{
  ...
  if (log_ptr != NULL)
  {
    ...
    if (index != packet_size)
    {
      MSG_GERAN_LOW_2_G("index(%d) != packet_size(%d)", index, packet_size);
    }
    gsmdiag_log_commit(&pkt_info);
  }
...
**      log_ptr->grr_state = 7;
      ...
      default:
        MSG_GERAN_LOW_0_G("GRR state not defined");
...
)
{
  ...
  if (scell_info_ptr->gsm.reselection_info.five_second_timer_id != 0)
  {
    log_ptr->serving_five_second_timer_state =
      (byte)rr_timer_get_state(scell_info_ptr->gsm.reselection_info.five_second_timer_id);
  }
  else
  {
  ...
  {
    MSG_GERAN_LOW_0_G("Some meas may be dropped since gprs_reselection_info_ptr->no_of_entries > RR_LOG_MAX_NUM_NEIGHBORS");
  }
 ...
 */
rr_log_reselection_trigger_T rr_log_get_reselection_trigger( )
{
...
)
{
  ...
  payload.sub_id        = geran_get_diag_sub_id();
...
static void rr_log_handover_end(byte failure_cause  )
{
  typedef PACKED struct PACKED_POST
  {
    byte sub_id;
 ...
 *
 */
void rr_log_handover_success( )
{
  rr_log_handover_end(0xFF);
 ...
 */
void rr_log_handover_failure(rr_log_handover_failure_T failure_reason )
  {
  rr_log_handover_end(failure_reason);
 ...
 */
void rr_log_reselect_start(rr_log_reselection_trigger_T trigger )
{
  // Obtain a pointer to the module data
  rr_log_data_t *rr_log_data_ptr = rr_log_get_data_ptr();
...
#endif
    payload_v2.sub_id         = geran_get_diag_sub_id();
...
{
  byte payload = geran_get_diag_sub_id();
...
{
  rr_log_g2w_redir_end(RR_LOG_G2W_REDIR_STATUS_SUCCESS);
  return;
}
...
{
  rr_log_g2w_redir_end(RR_LOG_G2W_REDIR_STATUS_ABORTED);
  return;
}
...
{
  rr_log_g2w_redir_end(RR_LOG_G2W_REDIR_STATUS_FAILURE);
  return;
}
...
{
  rr_log_g2t_redir_end(RR_LOG_G2T_REDIR_STATUS_SUCCESS);
  return;
}
...
{
  rr_log_g2t_redir_end(RR_LOG_G2T_REDIR_STATUS_ABORTED);
  return;
}
...
{
  rr_log_g2t_redir_end(RR_LOG_G2T_REDIR_STATUS_FAILURE);
  return;
}
...
{
  rr_log_g2l_reselect_end(earfcn, pcid, RR_LOG_G2L_RESEL_STATUS_SUCCESS);
  return;
}
...
{
  rr_log_g2l_reselect_end(earfcn, pcid, RR_LOG_G2L_RESEL_STATUS_ABORTED);
  return;
}
...
{
  rr_log_g2l_reselect_end(earfcn, pcid, status);
  return;
}
...
{
  rr_log_g2l_redir_end(RR_LOG_G2L_REDIR_STATUS_SUCCESS);
  return;
}
...
{
  rr_log_g2l_redir_end(RR_LOG_G2L_REDIR_STATUS_ABORTED);
  return;
}
...
{
  rr_log_g2l_redir_end(RR_LOG_G2L_REDIR_STATUS_FAILURE);
  return;
}
...
void rr_log_g2t_reselect_started(word uarfcn, word cpid )
{
  typedef PACKED struct PACKED_POST
  {
    byte sub_id;
...
void rr_log_g2t_reselect_success(word uarfcn, word cpid  )
{
  rr_log_g2t_reselect_end(uarfcn, cpid, RR_LOG_G2T_RESEL_STATUS_SUCCESS);
  return;
}

void rr_log_g2t_reselect_failure(word uarfcn, word cpid, rr_log_reselection_failure_T failure_reason )
{

  rr_log_g2t_reselect_end(uarfcn, cpid, failure_reason);
  return;
}
...
{
  rr_log_camping_attempt_end(failure_reason);
  return;
...
{
  rr_log_camping_attempt_end(0xFF);
  return;
...
{
  rr_log_power_scan_status(RR_LOG_PSCAN_STARTED);
  return;
...
{
  rr_log_power_scan_status(RR_LOG_PSCAN_COMPLETED);
  return;
...
{
  rr_log_power_scan_status(RR_LOG_RESELECTION_PSCAN_STARTED);
  return;
} /* rr_log_power_scan_started */
...
{
  rr_log_power_scan_status(RR_LOG_RESELECTION_PSCAN_COMPLETED);
  return;
} /* rr_log_power_scan_completed */
#endif
/*!
...
{
  rr_log_gprs_surround_search_end(failure_reason);
  return;
...
{
  rr_log_gprs_surround_search_end(0xFF);
  return;
...
)
{
  ...
  if (log_ptr != NULL)
  {
    ...
    if( (MAC_IDLE == rr_macinfo_get_mac_status()) || (MAC_IDLE_BLOCKED == rr_macinfo_get_mac_status()) )
    {
...
)
{
  ...
  switch (irat_measurements->rat)
  {
    ...
    {
      rr_log_wcdma_measurements(&irat_measurements->measurements.wcdma);
      break;
    } /* RR_IRAT_RAT_WCDMA */
#endif /* FEATURE_WCDMA */
    ...
    {
      rr_log_lte_measurements(&irat_measurements->measurements.lte);
      break;
    } /* RR_IRAT_RAT_LTE */
#endif /* FEATURE_LTE */
 ...
 */
void rr_log_task_start_init( void )
{
