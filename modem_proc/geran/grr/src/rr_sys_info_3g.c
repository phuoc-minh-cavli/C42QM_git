...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sys_info_3g.c#2 $
...
/*==============================================================================
  Variables
==============================================================================*/
static rr_sys_info_3g_data_t  rr_sys_info_3g_data;
 ...
 */
static rr_sys_info_3g_data_t *rr_sys_info_3g_get_data_ptr(void)
{
  return &rr_sys_info_3g_data;
}
...
)
{
  ...
  rr_sys_info_3g_data_t *rr_sys_info_3g_data_ptr = rr_sys_info_3g_get_data_ptr();
    ...
    if (dst->valid)
    {
      if (entry->instance_received_in > dst->instance_received_in)
      {
        if (entry->frequency_only)
        {
...
#endif /*FEATURE_GSM_TDS*/
          {
            MSG_GERAN_MED_3("[%d]: FDD UARFCN %d, index for RSSI reporting",
            index,
            entry->uarfcn,
            0);
        }
          ...
          {
            MSG_GERAN_MED_3("[%d]: TDD UARFCN %d, index not valid for reprting",
              index,
              entry->uarfcn,
              0);
          }
        ...
        else
        {
...
#endif /*FEATURE_GSM_TDS*/
          {
            MSG_GERAN_MED_3("[%d]: FDD UARFCN %d, Scrambling Code: %d",
            index,
            entry->uarfcn,
          ...
          {
            MSG_GERAN_MED_3("[%d]: TDD UARFCN %d, CPID: %d",
              index,
              entry->uarfcn,
      ...
      {
        MSG_GERAN_MED_0("Entry already present is from higher instance, not adding");
        return FALSE;
      }
    }
    else
  ...
  for (i = 0; i < fdd_cells->utran_fdd_neighbour_cell_count; i++)
  {
    ...
    */
    if (nr_of_fdd_cells == 0 && cells->fdd_indic0 == 0)
    {
      ...
      (void) rr_gprs_3g_nlist_add(list, &entry, *index_start_3g);
      *index_start_3g += 1;
    }
...
)
{
  ...
  rr_sys_info_3g_data_ptr = rr_sys_info_3g_get_data_ptr();
  pmo_store_ptr = rr_psi_get_pmo_store_ptr();
...
#endif /* FEATURE_WCDMA */
  { 
	MSG_HIGH("No PMO recvd before SI2q",0, 0, 0);
...
{
  return rr_sys_info_3g_get_data_ptr();
}
