...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_intra_ho.c#4 $ $DateTime: 2018/10/11 08:21:00 $ $Author: pwbldsvc $
...
static rr_intra_ho_data_t rr_intra_ho_data = RR_INTRA_HO_DATA_INIT;
 ...
 */
static rr_intra_ho_data_t *rr_intra_ho_get_data_ptr( void )
{
  return &rr_intra_ho_data;
}
 ...
 *                  the old channel
 */
static void rr_trigger_assignment_failure_response( )
{
...
)
{
  ...
  current_IE = rr_get_first_ie_of_message(ASSIGNMENT_COMMAND);
  ...
  if (current_IE != NULL)
  {
    ...
    if (decoding_ok == TRUE)
    {
      ...
      if (assignment_details_ptr->channel_info_1_after.frequency_list.no_of_items == 1)
      {
        ...
        if (rr_get_band_dyn(ARFCN.num) == BAND_NONE)
        {
  ...
  if ((decoding_ok == TRUE) && (assignment_failure == FALSE))
  {
    ...
    /******************************************************************/
    if (rr_channel_mode_supported(assignment_details_ptr->channel_mode_1) == FALSE)
    {
      assignment_failure = TRUE;
      RR_cause = RR_CHANNEL_MODE_UNACCEPTABLE;
    ...
    {
      if (rr_channel_mode_supported(assignment_details_ptr->channel_mode_2) == FALSE)
      {
        assignment_failure = TRUE;
        RR_cause = RR_CHANNEL_MODE_UNACCEPTABLE;
        ...
        (multirate_ie_present == FALSE))
    {
          ...
          (dedic_ptr->curr_amr_config.valid == TRUE))
      {
        ...
        else
        {
          ...
          assignment_failure = TRUE;
          RR_cause = RR_CHANNEL_MODE_UNACCEPTABLE;
          MSG_GERAN_LOW_2_G("current AMR type = %d New AMR type = %d",(int) dedic_ptr->curr_channel_spec.channel_mode_1,
    ...
    if ((assignment_failure == FALSE) && (decoding_ok == TRUE))
    {
      ...
      if (!rr_check_assignment_frequency_lists_supported(assignment_details_ptr))
      {
...
)
{
  ...
  rr_intra_ho_data_ptr = rr_intra_ho_get_data_ptr();
        ...
        if ( (assignment_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          if ((message_set == MS_RR_L2 ) && (message_id == (byte) DL_DATA_IND))
          {
            ...
            if (dl_data_ind_ptr->layer3_message[1] == ASSIGNMENT_COMMAND)
            {
              ...
              rr_intra_ho_data_ptr->old_band = rr_get_current_dedicated_band();
        ...
        if ( (assignment_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          ...
          if ( message_set == MS_RR_L1)
          {
            if (message_id == (byte) MPH_CHANNEL_ASSIGN_CNF)
            {
              ...
              {
                rr_complete_channel_assignment(dedic_ptr);
        ...
        if ( (assignment_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          if (message_set == MS_RR_L2 )
          {
            if  (message_id == (byte) DL_RESUME_CNF)
            {
               ...
               */
              if (!rr_bands_equal(rr_intra_ho_data_ptr->old_band, rr_get_current_dedicated_band()))
              {
              ...
              {
                rr_band_avoid_ded_freq_upd_send_imsg(&assignment_details_ptr->channel_info_1_after.frequency_list);
              }
