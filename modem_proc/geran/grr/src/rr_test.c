...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_test.c#3 $ */
...
static rr_test_data_t rr_test_data;

/*----------------------------------------------------------------------------
 * Static Function Declarations and Definitions
 * -------------------------------------------------------------------------*/

static rr_test_data_t *rr_test_get_data_ptr( void )
{
  return &rr_test_data;
}
...
)
{
  ...
  rr_test_data_ptr = rr_test_get_data_ptr();
  ...
  ms_operation_mode = rr_siminfo_get_ms_opmode();
    ...
    if (rr_test_data_ptr->loopback_type == GL1_DEFS_NO_LOOPBACK)
    {
      ...
      */ 
      ...
      if (loopback_valid)
      {
        ...
        gl1_msg_tch_loopback(loopback_type);
  ...
  switch ( msg->message_set )
  {
    ...
    case MS_RR_L2:
      ...
      */
      ...
      {
        case DL_DATA_IND:
          rr_tm_handle_data_ind((dl_data_ind_T*) new_message);
          break;
        default:
          break;
      }
...
{
  rr_test_data_t *rr_test_data_ptr = rr_test_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_test_data_ptr);
  ...
  if (rr_test_data_ptr->loopback_type != GL1_DEFS_NO_LOOPBACK)
  {
    ...
    gl1_msg_tch_loopback(GL1_DEFS_NO_LOOPBACK);

    if (rr_test_data_ptr->loopback_type == GL1_DEFS_LOOPBACK_TYPE_C)
    {
      // Do not send if L2 link not present
      if (rr_get_link_status() != NO_LINK)
      {
        rr_tm_send_open_tch_ack();
      }
    }
