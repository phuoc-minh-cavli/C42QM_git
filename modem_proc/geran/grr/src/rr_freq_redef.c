...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_freq_redef.c#3 $ $DateTime: 2018/10/10 08:50:54 $ $Author: pwbldsvc $
...
)
{
  ...
  current_IE_ptr = rr_get_first_ie_of_message(FREQUENCY_REDEFINITION);
...
rr_event_T rr_handle_frequency_redefinition_msg( void )
{
  ...
  ded_ptr = rr_get_dedicated_data_ptr();
  if (ded_ptr == NULL)
  {
    MSG_GERAN_LOW_0_G("No dedicated mode data is available for Freq Redef");
    return(EV_NO_EVENT);
  }
  else
  {
  ...
  if (rr_decode_frequency_redefinition(frd_ptr))
  {
...
===========================================================================*/
rr_event_T rr_handle_frequency_redefinition_ind(void *message_ptr )
{
  ...
  if (redef_ind_ptr != NULL)
  {
    if (redef_ind_ptr->channels_in_same_band == FALSE)
    {
      ...
      {
        rr_send_status(RR_FREQUENCY_NOT_IMPLEMENTED);
      }
    }
    else
    {
      ...
      /***********************************************/
      chan_spec_ptr = rr_get_channel_spec(ded_ptr);
