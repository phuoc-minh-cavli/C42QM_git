...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_control.c#29 $ $DateTime: 2021/07/28 09:37:24 $ $Author: pwbldsvc $
...
static rr_control_data_t  rr_control_data = INITIAL_VALUE(_RR_CONTROL_DATA_INIT);
...
static void rr_retry_msg_after_panic_reset_restart(void);
static rr_event_T rr_process_panic_reset_saved_msg(rr_cmd_bdy_type **message_ptr_ptr);
static rr_gsm_as_deactivation_complete_action_e rr_gsm_as_deactivation_complete(void);
static rr_event_T rr_process_rr_stop_gsm_mode_req(boolean rr_deact_gsm);
static void rr_handle_nw_sel_mode_reset_ind(const rr_nw_sel_mode_reset_ind_T * p_nw_sel_mode);

#ifdef FEATURE_SGLTE
static boolean rr_start_sglte_plmn_list_search(rr_plmn_list_req_T * plmn_list_req_ptr );
static void rr_sglte_set_lte_blind_redir_in_data_transfer( );
#endif /* FEATURE_SGLTE */
 ...
 */
static rr_control_data_t *rr_control_get_data_ptr( void )
{
  return &rr_control_data;
}
...
)
{
  ...
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  ...
  if (for_new_search == TRUE)
  {
    ...
    rr_control_data_ptr->bplmn_dsc_max = rr_gprs_get_max_dsc_value(FALSE);
...
boolean rr_is_pending_mode_change( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
void rr_set_pending_mode_change(boolean mode_change_status )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
static rr_x2g_resel_type_e rr_get_panic_reset_x2g_cco_flag( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_pending_mode_change_data_action( )
{
  ...
  if (rr_control_data_ptr->pending_mode_change_data.valid)
  {
    ...
    {
      rr_store_service_domain(rr_control_data_ptr->pending_mode_change_data.service_domain );
    }
#endif /*FEATURE_SGLTE*/
...
static void rr_report_invalid_band_pref_received( )
{
  ...
  MSG_GERAN_ERROR_1_G(
    "RR band_pref (0x%08x) includes unsupported (RR/L1/RF) bands or Check NV item 1877 - update of automation script",
    rr_control_data_ptr->rr_invalid_band_pref
  );
  (void)rr_timer_start(RR_INVALID_BAND_PREF_TIMER, RR_INVALID_BAND_PREF_TIMEOUT);
}
...
void rr_control_init( )
{
  ...
  set_rr_deact_reason(RR_DEACT_REASON_STOP_GSM_MODE_REQ);
  ...
  rr_ms_bplmn_initialise_dsc(FALSE);
#else
...
static void rr_control_recovery_init( )
{
  ...
  (void) rr_selection_control(EV_RESET_SOFTWARE, NULL);
#ifdef FEATURE_GERAN_CS_SUPPORT
  (void) rr_handle_procedures(EV_RESET_SOFTWARE, NULL);
  (void) rr_inter_cell_handover_control(EV_RESET_SOFTWARE, NULL);
  ...
  (void) rr_w2g_service_redirection_control(EV_RESET_SOFTWARE, NULL);
#endif /* FEATURE_WCDMA || FEATURE_LTE  || FEATURE_GSM_TDS */
...
#ifdef FEATURE_IDLE_DRX_SCALING
  rr_idle_drx_control(EV_RESET_SOFTWARE, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
...
{
  set_rr_deact_reason(deact_reason);

  return rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);

} /* end rr_trigger_gsm_as_deactivation() */

static boolean rr_wcdma_stop_cnf_required( )
{
#ifdef FEATURE_WCDMA
...
static boolean rr_tds_stop_cnf_required( )
{
#ifdef FEATURE_GSM_TDS
  if (!geran_is_multi_mode_gas_id())
  {
...
static rr_gsm_as_deactivation_complete_action_e rr_gsm_as_deactivation_complete( )
{
  ...
  rr_send_ps_implicit_reject_ind(0);
  ...
  switch ( deact_reason )
  {
    ...
    {
      rr_l1_gsm_deact_reason_T l1_deact_reason = rr_l1_gsm_deact_reason_get();
      ...
      if (l1_deact_reason == RR_L1_PSM_ENTER)
      {
        rr_send_mph_psm_entry_ind();
      }

#if defined(FEATURE_DYNAMIC_LOADING_GERAN) || defined(FEATURE_EXTERNAL_PAGING_GERAN)
     /* DEACT CNF to NAS will be sent once memory is unloaded */
      rr_static_set_dyn_unload_pending(TRUE);
#else
    ...
    {
      rr_init_geran_stopped();
#ifdef FEATURE_SGLTE
      if (rr_sglte_internal_stop_gsm_being_processed() &&
          rr_saved_l2g_msg_present())
      {
      ...
      {
#if defined(FEATURE_DYNAMIC_LOADING_GERAN) || defined(FEATURE_EXTERNAL_PAGING_GERAN)
        if (get_rr_mm_gsm_stop_reason() == RR_MM_DEEP_SLEEP
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
            || get_rr_mm_gsm_stop_reason() == RR_SUSPEND_STACK
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
           )
        {
          rr_static_send_proc_req(PROC_NONE, NULL);
          rr_send_stop_gsm_mode_cnf(MM_AS_SUCCESS);
        }
        else
        {
         /* STOP CNF to NAS will be sent once memory is unloaded */
          rr_static_set_dyn_unload_pending(TRUE);
        }
#else
        /* Confirm to MM */
    ...
    {
	  if( rr_static_get_stop_gsm_mode_cnf_pending() == TRUE)
	  {
	     /* Fix for CR 2876405 
	      * responce for RR_STOP_GSM_MODE_REQ is not send as
	      * GRR recieved MPH_PROC_IND while handling & deact reason changed to STOP_CAMPING
	      * Hence sending from here if RR_STOP_GSM_MODE_REQ  is in saved message 
	      * or setting dyn_unload_pending as true										
	      * also clearing saved rr_stop_gsm_mode_req 										*/
	     MSG_GERAN_HIGH_0_G("stop_gsm_mode_cnf is pending");
	 	 if (get_rr_mm_gsm_stop_reason() == RR_MM_DEEP_SLEEP
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
			 || get_rr_mm_gsm_stop_reason() == RR_SUSPEND_STACK
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
	     ...
	     if(grr_verify_saved_mm_message(RR_STOP_GSM_MODE_REQ))
	     {
		    rr_control_data_ptr->rr_saved_msg.valid = FALSE;
	     }
	  }
    ...
    case RR_DEACT_REASON_X2G_BPLMN_SEARCH_COMPLETE:
    {
      rr_grr_control_init();
      (void) rr_resel_control(EV_RESET_SOFTWARE, NULL);
      rr_cell_acq_control(EV_RESET_SOFTWARE, NULL);
      ...
      (void) rr_gprs_bsi_acquisition_control(EV_RESET_SOFTWARE, NULL);
      (void) rr_plmn_list_control( EV_RESET_SOFTWARE, NULL);
      /* Do-not Inform CB task about CBCH Info */
      send_cb_info = FALSE;
      break;
    }
...
boolean rr_in_connection_pending( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_background_plmn_search_in_progress( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
#ifdef FEATURE_GERAN_CS_SUPPORT
boolean rr_is_acquiring_si_after_conn_rel( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_is_control_state(const rr_state_T check_state )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_reject_msg(rr_cmd_bdy_type* msg_ptr )
{
  ...
  MSG_GERAN_HIGH_3_G(
    "reject message (%d, %d) and msg_rejected=%d",
...
static void rr_delete_saved_msg( )
{
  ...
  {
    rr_reject_msg(&rr_control_data_ptr->rr_saved_msg.msg);
  }
...
)
{
  ...
  if (rr_control_data_ptr->rr_saved_msg.valid == TRUE)
  {
    ...
    {
      if(rr_gprs_edtm_prioritise_saved_msg(new_msg_ptr) == TRUE)
      {
...
boolean rr_save_l2g_msg(rr_cmd_bdy_type *new_msg_ptr )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
 ...
 * @return FALSE if the buffer is empty or the message was not copied, TRUE if success
 */
boolean rr_get_saved_l2g_msg(rr_cmd_bdy_type ** message_ptr_ptr )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_saved_l2g_msg_present( )
{
...
rr_event_T rr_process_saved_l2g_message(rr_cmd_bdy_type **new_message_ptr_ptr )
{
  /* if there is a saved L2G message */
  if (rr_get_saved_l2g_msg(new_message_ptr_ptr))
  {
...
boolean rr_sglte_internal_stop_gsm_being_processed( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  ...
  if (rr_control_data_ptr->sglte_processng_intrnl_stop_gsm)
  {
    ...
    return(rr_is_sglte_mode_active());
  }

  return(FALSE);
}

boolean rr_sglte_saved_l2g_ho_being_processed( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_sglte_reinit_processing_l2g_ho_related_parameter( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_reject_nas_msg_during_intrn_shutdown(rr_cmd_bdy_type* msg_ptr )
{
  if (msg_ptr == NULL)
  {
    MSG_GERAN_ERROR_0_G("msg_ptr=NULL");
...
static void rr_sglte_send_lte_ho_failed_rsp( )
{
  ...
  result = rr_msgr_send(&rsp.hdr, sizeof(rsp));
 ...
 */
boolean rr_saved_msg_is_rr_est_req( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
 ...
 */
boolean rr_saved_msg_is_rr_est_req_emergency_call( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
 ...
 */
boolean rr_save_msg_for_edtm(rr_cmd_bdy_type *message_ptr )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
 ...
 */
void rr_delete_saved_mac_ul_tbf_req( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  ...
  {
    MSG_GERAN_HIGH_0_G("Saved MAC_GRR_UL_TBF_REQ deleted");
    rr_control_data_ptr->rr_saved_msg.valid = FALSE;
  }
...
)
{
  ...
  if (rr_get_saved_msg(new_message_ptr_ptr))
  {
...
static void rr_validate_rat_priority_for_plmn_search(sys_rat_pri_list_info_s_type *rat_pri_list_ptr )
{
  ...
  {
    sys_modem_device_mode_e_type device_mode = geran_get_multi_sim_mode();
    ...
    {
      if (SYS_SYS_MODE_GSM == rat_pri_list_ptr->priority_list_info[i].acq_sys_mode)
      {
        allowed_rat = TRUE;
      }
      else
      {
        allowed_rat = geran_is_multi_mode_gas_id();

        if (allowed_rat)
        {
          /* perform further checks for multi-SIM */
          if ((SYS_MODEM_DEVICE_MODE_DUAL_SIM_DUAL_STANDBY     == device_mode) ||
              (SYS_MODEM_DEVICE_MODE_TRIPLE_SIM_TRIPLE_STANDBY == device_mode))
          {
            if (rr_ms_other_sim_in_background_traffic())
            {
              if (((SYS_SYS_MODE_WCDMA == rat_pri_list_ptr->priority_list_info[i].acq_sys_mode) ||
                   (SYS_SYS_MODE_TDS == rat_pri_list_ptr->priority_list_info[i].acq_sys_mode)) &&
                  (geran_g2w_tuneaway_supported_by_device_mode(device_mode)))
              {
                allowed_rat = TRUE;
                MSG_GERAN_HIGH_0_G("PLMN list search may use G2W Tuneaway");
              }
              else
              {
                /* Background traffic implies other RAT is somehow active on the other stack */
                /* or that GPRS Tuneaway is currently active, so GL1 cannot share the radio. */
                allowed_rat = FALSE;
              }
            }
          }
...
)
{
  ...
  rr_validate_rat_priority_for_plmn_search(&(plmn_list_req->rat_pri_list_info));

#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
  MSG_GERAN_HIGH_5_G("PLMN search can start (network_selection_mode:%d trans_id:%d gsm_only:%d service_search:%d cell_search_num_cells:%d)",
    ...
    plmn_list_req->cell_search_num_cells
  );
#else
  MSG_GERAN_HIGH_4_G("PLMN search can start (network_selection_mode:%d trans_id:%d service_search:%d cell_search_num_cells:%d)",
    plmn_list_req->network_selection_mode,
    plmn_list_req->trans_id,
    plmn_list_req->service_search,
    plmn_list_req->cell_search_num_cells
   ...
   */
  rr_save_msg_for_recovery_after_panic_reset((rr_cmd_bdy_type *)plmn_list_req);
...
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
  rr_plmn_list_req_info_ptr->gps_wwan_search_info = plmn_list_req->gps_wwan_search_info;
  if (plmn_list_req->service_search == SYS_SERVICE_SEARCH_GPS_ADVANCE_SEARCH)
  {
    rr_set_gps_plmn_search_scan_ref_time(geran_get_time_ms());
  }
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */
  ...
  band_cap_u = rr_rat_pri_list_info_band_pref(rr_plc_get_rat_pri_list_info(), SYS_SYS_MODE_GSM);
  rr_set_preferred_bands(band_cap_u.chgwt_band_cap);

#ifdef FEATURE_WCDMA
  band_cap_u = rr_get_plmn_list_req_band_pref(plmn_list_req, SYS_SYS_MODE_WCDMA);
  rr_update_band_preference_for_rrc(band_cap_u.chgwt_band_cap);
#endif /* FEATURE_WCDMA */

  rr_plmn_list_req_info_ptr->service_search = plmn_list_req->service_search;
  rr_plmn_list_req_info_ptr->cell_search_num_cells = plmn_list_req->cell_search_num_cells;
  ...
  if (!foreground)
  {
    ...
    if (rr_ce_compare_and_update_l1_page_mode(PAGING_REORGANIZATION, NORMAL_PAGING))
    {
   ...
   */
  rr_plc_setup(foreground?RR_PLC_FOREGROUND_SEARCH:RR_PLC_BACKGROUND_SEARCH, 0);
  ...
  {
    rr_ms_bplmn_initialise_dsc(TRUE);
  }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
static void rr_sglte_x2g_plmn_list_search_results_clear( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(foreground ? RR_PLC_SGLTE_X2G_FOREGROUND_SEARCH : RR_PLC_SGLTE_X2G_BACKGROUND_SEARCH, 0);
  ...
  rr_sglte_x2g_plmn_list_search_results_clear();
  ...
  if ( plmn_srch_req->foreground_search == FALSE )
  {
    ...
    result = rr_msgr_send(&plmn_srch_rsp_ptr->msg_hdr, sizeof(lte_irat_plmn_srch_rsp_s));
    if (result != E_SUCCESS)
    {
      MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
    }
...
)
{
  ...
  rr_plmn_list_req_info_ptr->rat_pri_list_info.num_items = 1;
  rr_plmn_list_req_info_ptr->rat_pri_list_info.next_acq_sys_index = 0;
  rr_plmn_list_req_info_ptr->rat_pri_list_info.priority_list_info[0].band_cap.chgwt_band_cap = rr_band_pref();
  rr_plmn_list_req_info_ptr->rat_pri_list_info.priority_list_info[0].acq_sys_mode = SYS_SYS_MODE_GSM;
  rr_plmn_list_req_info_ptr->rat_pri_list_info.priority_list_info[0].acq_sys_time_interval = 0;
  ...
  if (plmn_srch_req->is_foreground_search == FALSE)
  {
    ...
    rr_send_tds_interrat_plmn_search_cnf(FALSE, FALSE, NULL);
  }
  else
  {
...
)
{
  ...
  plc_band_filter = rr_plc_get_band_filter();
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_TTOG_SEARCH, msg_ptr->timer_val);
}
#endif /*FEATURE_GSM_TDS*/
...
)
{
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_WTOG_SEARCH, msg_ptr->timer_val);
}
 ...
 */
static rr_event_T rr_process_lte_geran_activity_ind(geran_grr_lte_geran_activity_ind_t *ind_ptr )
{
  if (ind_ptr == NULL)
  {
    ...
    {
      {
        ...
        activity_mask = geran_get_lte_activity_status();
        ...
        geran_set_lte_activity_status(activity_mask);
        MSG_GERAN_HIGH_1("LTE activity mask updated to 0x%08X", activity_mask);

        if (rr_mode_gsm_active_or_starting())
        {
          /* if GL1 is active, trigger it to resubmit any acquisition requests */
          rr_send_mph_set_priority_ind(GERAN_PRIORITY_NOT_CHANGED, FALSE );
        }
      }
    }
  }
...
)
{
  ...
  rr_plc_set_band_filter(plc_band_filter);
   ...
   * setup rr_plmn_list_control()
   */
  rr_plc_setup(RR_PLC_LTOG_SEARCH, msg_ptr->timer_val);
}
...
)
{
  ...
  {
    S_counter = rr_increment_S_counter();
  }
  else if (block_quality == GL1_BLOCK_QUALITY_BAD)
  {
    S_counter = rr_decrement_S_counter();
  }
  else
  {
    S_counter = 0;
...
)
{
  ...
  if (next_state == RR_DEDICATED)
  {
    ...
    rr_dedicated_mode_process_initial_params(
      scell_info_ptr,
      rr_get_instances_ptr(SI2Q_INSTANCE)
    );
...
)
{
  ...
  rr_save_msg_for_recovery_after_panic_reset(new_message_ptr);
  ...
  switch (new_message_ptr->rr_deact_req.deact_reason)
  {
    ...
    {
      rr_l1_gsm_deact_reason_set(RR_L1_POWER_OFF);
      rr_l1_gsm_stop_reason_set(RR_L1_DEEP_SLEEP);
      break;
    }
    ...
    case RR_MM_PSM:
    {
      rr_l1_gsm_deact_reason_set(RR_L1_PSM_ENTER);
      rr_l1_gsm_stop_reason_set(RR_L1_DEEP_SLEEP);
      break;
    }
    ...
    default:
    {
      rr_l1_gsm_deact_reason_set(RR_L1_OFF_LINE);
      rr_l1_gsm_stop_reason_set(RR_L1_RAT_CHANGE);
    }
  }
  ...
  else if (rr_w2g_redirection_search_in_progress())
  {
  ...
  processed later. */
            ...
            rr_control_data_ptr->rr_state == RR_INACTIVE) &&
            rr_mode_get_l1_stop_reason() != rr_l1_gsm_stop_reason_get() &&
            rr_l1_gsm_stop_reason_get() == RR_L1_DEEP_SLEEP)
  {
    MSG_GERAN_HIGH_3_G(
      "rr_control() state:%d last L1 stop reason:%d new L1 stop reason:%d",
      rr_control_data_ptr->rr_state,
      rr_mode_get_l1_stop_reason(),
      rr_l1_gsm_stop_reason_get()
    );
  ...
  if (pass_to_rr_control == FALSE)
  {
    if ( rr_control_data_ptr->rr_state == RR_INACTIVE  )
    {
      ...
      set_rr_deact_reason(RR_DEACT_REASON_DEACT_REQ);
    ...
    else if ( rr_control_data_ptr->rr_state != RR_RESET )
    {
       ...
       */
      new_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
...
static void rr_cancel_idle_timers( )
{
  ...
  (void)rr_timer_cancel(SIGNAL_STRENGTH_TIMER);
  rr_cancel_all_five_second_timers();
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_LTE) || defined (FEATURE_GSM_TDS)
  rr_g2x_barred_frequency_db_clear();
  rr_g2x_barred_cell_db_clear();
  rr_g2x_cell_quality_data_clear();
#endif /* FEATURE_WCDMA || FEATURE_LTE || FEATURE_GSM_TDS*/
 ...
 *
 */
byte rr_ms_get_bplmn_failed_id( void )
{
  rr_control_data_t *rr_control_data_ptr;
  rr_control_data_ptr = rr_control_get_data_ptr();
 ...
 *
 */
void rr_ms_set_bplmn_failed( void )
{
  rr_PLMN_list_req_info_T *list_req_ptr;
  rr_control_data_t       *rr_control_data_ptr;

  rr_control_data_ptr = rr_control_get_data_ptr();
  list_req_ptr        = rr_get_plmn_list_req_info_ptr();
 ...
 */
static boolean rr_ms_last_bplmn_failed(rr_plmn_list_req_T *new_req_ptr )
{
...
)
{
  ...
  if (bq_ptr != NULL)
  {
    ...
    if (rr_plc_bplmn_pscan_active())
    {
      ...
      plc_event = rr_ms_bplmn_dsc_check(adjusted_bq);
    }
    else
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
    {
  ...
  switch(msg_set)
  {
    case MS_RR_L1:
      switch (msg_id)
      {
        ...
        case MPH_FCCH_SCH_DECODE_IND:
          rr_candidate_update_FCCH_SCH(&(new_message_ptr->mph_fcch_sch_decode_ind));
          new_rr_event = EV_NO_EVENT;
          break;
        ...
        case MPH_PROC_IND:
        {
          MSG_GERAN_MED_0_G("DMMSUSREM:: perform back before aborting plmn list search");
          rr_perform_backup_for_scell();
          new_rr_event = EV_PLMN_SEARCH_ABORT;

          MSG_GERAN_MED_1_G("DMMSUSREM:: PLMN search event: %d", new_rr_event);
        }
          break;
#endif
    ...
    case MS_RR_L2:
      switch (msg_id)
      {
        case DL_UNIT_DATA_IND:
          ...
          if (rr_prepare_ota_msg_for_parsing(&(new_message_ptr->dl_unit_data_ind)))
          {
            /* forward the message to connection establishment control for processing */
            new_rr_event = rr_connection_establishment_control(EV_INPUT_MESSAGE, new_message_ptr);
          }
          break;
    ...
    case MS_MM_RR:
      switch (msg_id)
      {
        ...
        case RR_EST_REQ:
        {
          ...
          else
          {
            ...
            */
            ...
            {
              proceed_with_rr_est_req = rr_plc_is_automatic_search();
            }
            else
            {
            ...
            if (proceed_with_rr_est_req)
            {
              ...
              (void)rr_save_msg(new_message_ptr);
              new_rr_event = EV_PLMN_SEARCH_ABORT;
            }
            else
            {
        ...
        case RR_PLMN_LIST_REQ:
        case RR_DEACT_REQ:
          if (rr_save_msg(new_message_ptr))
          {
            new_rr_event = EV_PLMN_SEARCH_ABORT;
          }
          break;
    ...
    case MS_MAC_RR:
      switch(msg_id)
      {
        case MAC_GRR_UL_TBF_REQ:
          if (rr_save_msg(new_message_ptr))
          {
            if (rr_plc_get_ps_service_priority())
            {
        ...
        case MAC_GRR_PS_ACCESS_CNF:
        {
          ...
          rr_ps_access_control(new_message_ptr);
          break;
        }
...
boolean rr_x2g_cco_with_nacc_in_progress( )
{
  ...
  {
    return rr_x2g_resel_with_nacc_in_progress();
  }
  else
  {
    return FALSE;
...
#ifdef FEATURE_SGLTE
  else if ((rr_is_sglte_plmn_srch_active() == TRUE) &&
           (rr_is_sglte_mode_active() == TRUE))
  {
    MSG_GERAN_HIGH_0_G("No G2X measurements allowed, SGLTE BPLMN search active");
    return FALSE;
  }
  else if (rr_sglte_g2l_blind_redirection_active())
  {
  ...
  switch (request->message_header.message_set)
  {
    case MS_RRC_RR:
    {
      switch (request->message_header.message_id)
      {
        ...
        {
          rr_send_interrat_reselection_abort_cnf();
          break;
        ...
        {
          rr_send_interrat_redirect_abort_cnf(RRC_RR_SUCCESS);
          break;
        } /* RR_INTERRAT_REDIRECT_ABORT_REQ */

        case RR_INTERRAT_HANDOVER_REQ:
        {
          rr_send_interrat_ho_cnf(RRC_RR_FAILURE, RRC_RR_INVALID_STATE, NULL);
          break;
        } /* RR_INTERRAT_HANDOVER_REQ */

        case RR_INTERRAT_HANDOVER_ABORT_REQ:
        {
          rr_send_interrat_ho_abort_cnf(RRC_RR_SUCCESS);
          break;
        } /* RR_INTERRAT_HANDOVER_ABORT_REQ */

        case RR_INTERRAT_CC_ORDER_REQ:
        {
          if (rr_get_x2g_cco_act_rsp_rcvd())
          {
          ...
          rr_set_x2g_cco_act_rsp_rcvd(FALSE);
        ...
        {
          rr_send_interrat_cc_order_abort_cnf(RRC_RR_SUCCESS);
          break;
        } /* RR_INTERRAT_CC_ORDER_ABORT_REQ */

        case RR_INTERRAT_PLMN_SRCH_REQ:
        {
          rr_send_interrat_plmn_search_cnf(TRUE, FALSE, NULL);
          break;
        } /* RR_INTERRAT_PLMN_SRCH_REQ */

        case RR_INTERRAT_PLMN_SRCH_ABORT_REQ:
        {
          rr_send_interrat_plmn_search_abort_cnf();
          break;
    ...
    case MS_MSGR_RR:
    {
      switch (request->rr_msgr_msg.message.hdr.id)
      {
        case GERAN_GRR_LTE_RESEL_REQ:
        {
          ...
          result = rr_msgr_send(&lte_resel_failed_rsp.hdr, sizeof(lte_resel_failed_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_ABORT_RESEL_REQ:
        {
          ...
          result = rr_msgr_send(&lte_abort_resel_rsp, sizeof(lte_abort_resel_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_ABORT_REDIR_REQ:
        {
          ...
          result = rr_msgr_send(&lte_abort_redir_rsp, sizeof(lte_abort_redir_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_ABORT_PLMN_SRCH_REQ:
        {
          ...
          result = rr_msgr_send(&abort_plmn_srch_rsp.msg_hdr, sizeof(abort_plmn_srch_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_GET_CGI_REQ:
        {
          ...
          result = rr_msgr_send(&lte_get_cgi_rsp.msg_hdr, sizeof(lte_get_cgi_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_ABORT_CGI_REQ:
        {
          ...
          result = rr_msgr_send(&lte_abort_cgi_rsp.msg_hdr, sizeof(lte_abort_cgi_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_CCO_REQ:
        {
          ...
          rr_set_x2g_cco_act_rsp_rcvd(FALSE);
          result = rr_msgr_send(&lte_cco_rsp.hdr, sizeof(lte_cco_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
        ...
        case GERAN_GRR_LTE_ABORT_CCO_REQ :
        {
          ...
          result = rr_msgr_send(&lte_abort_cco_rsp, sizeof(lte_abort_cco_rsp));
          if (result != E_SUCCESS)
          {
            MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
          }
  ...
  switch (request->message_header.message_set)
  {
    case MS_TDSRRC_RR:
    {
      switch (request->message_header.message_id)
      {
        ...
        {
          rr_send_tds_interrat_reselection_abort_cnf();
          break;
        ...
        {
          rr_send_tds_interrat_redirect_abort_cnf(TDSRRC_RR_SUCCESS);
          break;
        } /* TDSRR_INTERRAT_REDIRECT_ABORT_REQ */

        case TDSRR_INTERRAT_HANDOVER_REQ:
        {
          rr_send_tds_interrat_ho_cnf(TDSRRC_RR_FAILURE, TDSRRC_RR_INVALID_STATE, NULL);
          break;
        } /* TDSRR_INTERRAT_HANDOVER_REQ */

        case TDSRR_INTERRAT_HANDOVER_ABORT_REQ:
        {
          rr_send_tds_interrat_ho_abort_cnf(TDSRRC_RR_SUCCESS);
          break;
        ...
        {
          rr_send_tds_interrat_cc_order_abort_cnf(TDSRRC_RR_SUCCESS);
          break;
        } /* TDSRR_INTERRAT_CC_ORDER_ABORT_REQ */

        case TDSRR_INTERRAT_PLMN_SRCH_REQ:
        {
          rr_send_tds_interrat_plmn_search_cnf(TRUE, FALSE, NULL);
          break;
        } /* TDSRR_INTERRAT_PLMN_SRCH_REQ */

        case TDSRR_INTERRAT_PLMN_SRCH_ABORT_REQ:
        {
          rr_send_tds_interrat_plmn_search_abort_cnf();
          break;
...
*/
boolean rr_is_sglte_mode_active(void)
{
...
)
{
  ...
  rr_store_service_domain(service_domain);
...
*/
static boolean rr_sglte_serving_cell_meas_valid( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
boolean rr_is_sglte_x2g_resel_in_progress( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
boolean rr_sglte_is_full_service_available( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
void rr_sglte_store_serving_cell_info( 
{
  uint8 rxlev ;
  rr_measured_signal_data_t * measured_signal_data = rr_get_measured_signal_data_ptr();
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
  ...
  rr_control_data_ptr->sglte_store_serv_cell.serving_ARFCN = rr_gprs_get_scell_arfcn();

  rr_control_data_ptr->sglte_store_serv_cell.serving_BSIC = rr_gprs_get_scell_bsic();
...
static void rr_sglte_delete_store_serving_cell_info( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_sglte_get_stored_serv_gprs_ind( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
ARFCN_T rr_sglte_get_stored_serv_ARFCN( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
BSIC_T rr_sglte_get_stored_serv_BSIC( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
int16 rr_sglte_get_stored_serv_RSSI( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
===========================================================================*/
boolean rr_limited_service_in_effect( )
{
  rr_PLMN_request_details_T *rr_PLMN_request_details_ptr = rr_get_PLMN_request_details_ptr();
  rr_control_data_t         *rr_control_data_ptr         = rr_control_get_data_ptr();
 ...
 * \param new_state 
 */
static void rr_control_transition_to_state(rr_state_T new_state )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
)
{
  ...
  if (get_waiting_for_surround_response() && 
      match_waiting_for_surround_response(su_fail_ptr->BCCH_ARFCN))
  {
    bcch_ncell_info_t *bcch_ncell = rr_gprs_get_surround_ncell_sys_info(su_fail_ptr->BCCH_ARFCN);
    ...
    rr_log_gprs_surround_search_failure(RR_LOG_GPRS_NCELL_NOT_FOUND_BY_L1);
#endif
  }
}
 ...
 */
void rr_control(rr_event_T rr_event, rr_cmd_bdy_type* new_message_ptr )
{
  byte                        message_id;
  byte                        message_set;
#ifdef FEATURE_GERAN_CS_SUPPORT
  ...
  while (rr_event != EV_NO_EVENT)
  {
           ...
           message_set == MS_MM_RR)
       {
         switch (message_id)
         {
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
           {
             ...
             rr_reject_nas_msg_during_intrn_shutdown(new_message_ptr);
    ...
    if (rr_control_data_ptr->sglte_g2x_plmn_list_search_in_progress == TRUE)
    {
          ...
          message_set == MS_MM_RR)
      {
        switch (message_id)
        {
          ...
          case RR_PLMN_LIST_REQ:
          {
            ...
            rr_event = rr_plc_sglte_g2x_control(EV_PLMN_SEARCH_ABORT,new_message_ptr);
            break;
          }
          case RR_BPLMN_SEARCH_ABORT_REQ:
          {
               ...
               (rr_control_data_ptr->rr_state == RR_INACTIVE ||rr_control_data_ptr->rr_state == RR_GOING_INACTIVE))
      {
        ...
        rr_event = rr_plc_sglte_g2x_control(EV_PLMN_SEARCH_ABORT_NO_MULTIRAT_ACT,new_message_ptr);
      }
      ...
      {
        rr_event = rr_plc_sglte_g2x_control(rr_event,new_message_ptr);
      }
      ...
      if (rr_event == EV_PLMN_SEARCH_COMPLETE)
      {
        ...
        {
          rr_event_T  saved_event = rr_process_saved_message(&new_message_ptr);
              ...
              message_id == RR_PLMN_LIST_REQ)
          {
            ...
            if (rr_start_sglte_plmn_list_search(((rr_plmn_list_req_T *)new_message_ptr)))
            {
              ...
              rr_start_plmn_list_search((rr_plmn_list_req_T *)new_message_ptr, TRUE);
              rr_event = rr_plc_sglte_g2x_control(EV_PLMN_SEARCH_START,new_message_ptr);
        ...
        else
        {
          ...
          rr_saved_event = rr_examine_saved_message( &new_message_ptr );
    ...
    if (rr_event == EV_INPUT_MESSAGE)
    {
      if (message_set == MS_MM_RR)
      {
        switch ((mm_rr_message_id_enum_T) message_id)
        {
          case RR_DEACT_REQ:
          {
            ...
            rr_event = rr_process_rr_deact_req(new_message_ptr);
            break;
          } /* RR_DEACT_REQ */
          ...
          case RR_STOP_GSM_MODE_REQ:
          {
            ...
            {
              rr_l1_gsm_stop_reason_set(RR_L1_DEEP_SLEEP);
              rr_notify_deep_sleep_to_other_rats(TRUE);
            }
            else
            {
              rr_l1_gsm_stop_reason_set(RR_L1_RAT_CHANGE);
...
#ifdef FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH
            if (new_message_ptr->rr_stop_gsm_mode_req.rr_mm_gsm_stop_reason == RR_SUSPEND_STACK && rr_is_camped())
            {
              MSG_GERAN_HIGH_0_G("GPS FR : Perform scell back up as we received suspend stack from NAS");
              rr_perform_backup_for_scell();
            }
#endif /* FEATURE_GERAN_GPS_PLMN_SCAN_SEARCH */

			/* Fix for CR 2876405*/
			rr_static_set_stop_gsm_mode_cnf_pending(TRUE);
          ...
          {
#if defined(FEATURE_WCDMA) || defined(FEATURE_LTE) || defined(FEATURE_GSM_TDS)
            rr_g2x_equivalent_plmn_list_updated();
#endif /* FEATURE_WCDMA || FEATURE_LTE || FEATURE_GSM_TDS */
          ...
          case RR_LTE_RESELECTION_ALLOWED_STATUS_IND:
          {
            ...
            if (rr_control_data_ptr->g2l_resel_allowed != g2l_resel_allowed)
            {
              ...
              necessary. */
              rr_send_imsg(RR_IMSG_G2L_RESEL_ALLOWED_UPDATED_IND);
            }
          ...
          case RR_CSFB_CALL_STATUS_IND:
          {
            ...
            MSG_GERAN_HIGH_2_G("RR_CSFB_CALL_STATUS_IND changed: %d -> %d",
            ...
            if (rr_g2l_blind_redir_after_srvcc_enabled())
            {
            ...
            if ((get_rr_state()== RR_DATA_TRANSFER) && (rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled()))
            {
              MSG_GERAN_HIGH_0_G("UltraFlasg CSFB in progress set the flag for FRLTE");            
              rr_connection_release_set_g2l_blind_redir_flag();
            }

#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
            /* notify the Priority Inversion Manager of the new status */
            rr_ms_csfb_status_changed(is_csfb_call_in_progress);
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
            break;
          ...
          {
            boolean    update_11;
            rr_mtc_nw_config_param_update_ind_T *rr_mtc_nw_config_param_update_ind = &new_message_ptr->mm.rr_mtc_nw_config_param_update_ind;

#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
            boolean    prev_peo_mode = rr_ue_in_peo_mode();
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/ 
            
            rr_set_psm_edrx_support(rr_mtc_nw_config_param_update_ind, &update_11);

            if (TRUE == rr_control_data_ptr->ready_timer_running)
            {
#ifdef FEATURE_GERAN_EXTENDED_DRX             
              if (FALSE == rr_get_edrx_indicator())
              {
                MSG_GERAN_HIGH_0_G(" Edrx disabled");
              }
              else
              ...
              MSG_GERAN_HIGH_1_G("Ready timer running update l1=%d",update_11);              
            }

#if defined (FEATURE_GERAN_EXTENDED_DRX) || defined(FEATURE_GERAN_POWER_EFFICIENT_OPERATION) 
            if (update_11)
            {
              rr_edrx_update_l1_params(RR_MM_PSM_EDRX_PARAMS_UPDATE);
            }
            ...
            {
              /*Initialise DSC with PEO DSC now*/
              rr_gprs_initialise_dsc();

              /*Restart Sys infor refersh timer */
              rr_gprs_start_sys_info_refresh_timer(); 
              rr_resel_send_imsg_reduced_mobility_required_ind(RR_REDUCED_MOBILITY_MAX);
            }
            else
            {
              if (prev_peo_mode != rr_ue_in_peo_mode())
              {
                rr_resel_send_imsg_reduced_mobility_stop_ind();
              }
            }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
          ...
          { 
#ifdef FEATURE_GERAN_EXTENDED_DRX
            MSG_GERAN_HIGH_1_G("Ready timer = %d",new_message_ptr->mm.rr_gmm_ready_timer_status_ind.timer_active);
            if (new_message_ptr->mm.rr_gmm_ready_timer_status_ind.timer_active)
            {
              rr_control_data_ptr->ready_timer_running = TRUE;
              rr_edrx_update_l1_params(RR_MM_READY_TIMER_START);            
            }
            else
            {
              rr_control_data_ptr->ready_timer_running = FALSE;
              rr_edrx_update_l1_params(RR_MM_READY_TIMER_STOP);
            }
#endif /* FEATURE_GERAN_EXTENDED_DRX*/
            rr_event = EV_NO_EVENT;
            break;
      ...
      else if (message_set == MS_TDSRRC_RR)
      {
        ...
        {
#ifdef FEATURE_SGLTE
          if (rr_is_sglte_mode_active() == FALSE)
#endif /* FEATURE_SGLTE */
          {
        ...
        else if (message_id == (byte) TDSRR_INTERRAT_TL1_STOP_IND)
        {
          ...
          {
            rr_control_data_ptr->waiting_for_tdsrr_interrat_tl1_stop_ind = FALSE;
            rr_send_imsg(RR_IMSG_X2G_CCO_COMPLETE_IND);
            rr_event = EV_NO_EVENT;
          }
        }
        ...
        else if (message_id == (byte) TDSRR_CIPHER_SYNC_IND)
        {
          if (rr_control_data_ptr->rr_state == RR_INACTIVE)
          {
            rr_siminfo_activate_new_cipher_keys();
          }
          else
          {
        ...
        else if (message_id == (byte) TDSRRC_DEDICATED_PRIORITIES_RSP)
        {
          ...
          if (!plmn_ok)
          {
            plmn_ok = rr_eplmn_list_equivalent_plmn(requested_PLMN_id) &&
                      rr_eplmn_list_equivalent_plmn(dedicated_priorities_rsp->plmn);
          }
          ...
          if (plmn_ok)
          {
            ...
            {
              ARFCN_T arfcn = rr_gprs_get_scell_arfcn();
              ...
              rr_dedicated_priorities_take_ownership();

              rr_send_imsg(RR_IMSG_INDIVIDUAL_PRIORITIES_UPDATED_IND);
            }
            else
            {
          ...
          else
          {
            if (geran_is_multi_mode_gas_id())
            {
              ...
              rr_dedicated_priorities_request_owner_clears(owner);
            }
            else
            {
      ...
      else if ((message_set == MS_TIMER) && (message_id == MID_TIMER_EXPIRY))
      {
            ...
            timer_id == RR_RESET_FORBIDDEN_CELL_TIMER_7)
        {
          ...
          /*********************************************/
          rr_clear_forbidden_cell_timer(timer_id);
                 ...
                 (timer_id == RR_G2WR_FIVE_SEC_TIMER_6))
        {
          ...
          rr_g2wr_update_wcdma_five_second_timer_state(timer_id);
#endif /* FEATURE_WCDMA */
        ...
        {
          rr_g2x_barred_frequency_timer_expired(timer_id);
        }
        ...
        {
          rr_g2x_barred_cell_timer_expired(timer_id);
        }
        ...
        {
          timer_handled = rr_gprs_general_timer_expiry_handler(timer_id, &new_rr_event);
        }
      ...
      else if (message_set == MS_MSGR_RR)
      {
        switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
        {
          ...
          case LTE_RRC_GET_DEDICATED_PRI_RSP:
          {
            ...
            {
              plmn_ok = rr_eplmn_list_equivalent_plmn(requested_PLMN_id) &&
                        rr_eplmn_list_equivalent_plmn(get_dedicated_pri_rsp->plmn);
            }
          ...
          case GERAN_GRR_LTE_SGLTE_SERVING_MEAS_REQ:
          {
            ...
            if (rr_is_sglte_mode_active())
            {
              if (rr_sglte_is_full_service_available() && rr_sglte_serving_cell_meas_valid() && rr_cell_supports_gprs())
              {
#ifdef FEATURE_GERAN_CS_SUPPORT
                if ( (rr_cell_supports_dtm() == FALSE) && (rr_is_voice_call_active()) )
                {
              ...
              if ( serving_cell_meas_valid )
              {
                ...
                RR_ASSERT_WARN(rr_sel_get_service_domain() == SYS_SRV_DOMAIN_CS_ONLY);
                RR_ASSERT_WARN(rr_gsm_only() == TRUE);

                rr_send_lte_sglte_serving_meas_rsp(
                  rr_gprs_get_scell_arfcn(),
                  TRUE,
                  rr_gprs_get_scell_bsic(),
                  measured_signal_data->current_signal_strength
                );
              }
              else
              {
                rr_send_lte_sglte_serving_meas_rsp_no_service();
              }
            }
            else
            {
          ...
          case GERAN_GRR_TDS_SGLTE_SERVING_MEAS_REQ:
          {
            ...
            if (rr_is_sglte_mode_active())
            {
              ...
              {
                rr_send_tds_sglte_serving_meas_rsp_no_service();
              }
            }
            else
            {
          ...
          {
            if (rr_is_sglte_mode_active() &&
                rr_sglte_serving_cell_meas_valid() &&
                rr_sglte_is_full_service_available())
            {
              rr_measured_signal_data_t * measured_signal_data = rr_get_measured_signal_data_ptr();
             ...
             else
             {
               ...
               rr_send_lte_sglte_neighbor_meas_rsp_no_service();
             }
             rr_event = EV_NO_EVENT;
             break;
          ...
          case GERAN_GRR_LTE_HO_REQ:
          {
            if (rr_is_sglte_mode_active())
            {
              switch (get_rr_state())
              {
                case RR_INACTIVE:
                {
                  if((!rr_sglte_saved_l2g_ho_being_processed()) && (!rr_saved_l2g_msg_present()))
                  {
                ...
                case RR_CELL_SELECTION:
                {
                  ...
                  geran_eng_mode_data_write(ENG_MODE_CODEC_HO_INFO,&ho_codec_info,TRUE);

                  rr_sglte_send_lte_ho_failed_rsp();
                  rr_send_srvcc_handover_failure_ind();
                  rr_event = EV_NO_EVENT;
                  break;
                }
                ...
                default:
                {
                  if(rr_is_limited_plmn_selection_mode())
                  {
                    sys_codec_ho_info ho_codec_info = {0};
                    ho_codec_info.cmd = SYS_HO_START;
                    ho_codec_info.ho_type = SYS_HO_TYPE_LTE_GSM_SRVCC;
                  ...
                  else if(rr_save_l2g_msg(new_message_ptr))
                  {
                    ...
                    rr_mm_send_message( &rr_rr_stop_gsm_mode_req.message_header, GS_QUEUE_RR );
                    rr_event = EV_NO_EVENT;
                  }
              else
                  {
          ...
          {
            rr_event = rr_process_lte_geran_activity_ind(&(new_message_ptr->rr_msgr_msg.message.geran_grr_msg_ind.lte_geran_activity_ind));
            break;
          }
#endif /* FEATURE_LTE */
          ...
          {
            rr_band_avoidance_control(new_message_ptr);
            rr_event = EV_NO_EVENT;
            break;
          }
#endif /*FEATURE_GSM_BAND_AVOIDANCE*/
          ...
          {
            rr_idle_drx_control(EV_INPUT_MESSAGE, new_message_ptr);
            rr_event = EV_NO_EVENT;
            break;
          }
#endif /* FEATURE_IDLE_DRX_SCALING */
            ...
            case MS_MM_RR:
            {
              switch ((mm_rr_message_id_enum_T) message_id)
              {
                ...
                case RR_PLMN_LIST_REQ:
                {
                  ...
                  if (rr_ms_plmn_list_search_valid(RR_INACTIVE) == FALSE)
                  {
                    rr_plmn_list_req_T *req_ptr = &new_message_ptr->rr_plmn_list_req;

                    /* reject the request immediately */
                    rr_send_plmn_list_cnf(NULL, MM_AS_CONNECTED, req_ptr);
                    new_rr_event = EV_NO_EVENT;
                    break;
                  }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
                  ...
                  {
                    MSG_GERAN_LOW_0_G("must be able to start PLMN search in INACTIVE");
                  }
                  break;
                }
                ...
                case RR_STOP_GSM_MODE_REQ:
                {
                  ...
                  if ((new_message_ptr->rr_stop_gsm_mode_req.rr_mm_gsm_stop_reason == RR_MM_DEEP_SLEEP) ||
                      (new_message_ptr->rr_stop_gsm_mode_req.rr_mm_gsm_stop_reason == RR_MM_DUAL_SWITCH))
                  {
                    if (rr_mode_get_l1_stop_reason() != RR_L1_DEEP_SLEEP)
                    {
                      rr_mode_set_l1_stop_reason(RR_L1_DEEP_SLEEP);
                      rr_l1_send_mph_deep_sleep_ind();
                    }
                  }
                  (void)rr_process_rr_stop_gsm_mode_req(FALSE);
                  (void)rr_gsm_as_deactivation_complete();
                ...
                case RR_DEACT_REQ:
                {
                  ...
                  if ((rr_mode_get_l1_stop_reason() != rr_l1_gsm_stop_reason_get()) &&
                      (rr_l1_gsm_stop_reason_get() == RR_L1_DEEP_SLEEP))
                  {
                    ...
                    rr_ps_no_ps_access_del_all();
#ifndef FEATURE_DYNAMIC_LOADING_GERAN
                    rr_init_modem_offline();
#endif
                  }
#if defined(FEATURE_DYNAMIC_LOADING_GERAN) || defined(FEATURE_EXTERNAL_PAGING_GERAN)
                 /* DEACT CNF to NAS will be sent once memory is unloaded */
                  ...
                  if (rr_l1_gsm_deact_reason_get() == RR_L1_PSM_ENTER)
                  {
                    rr_send_mph_psm_entry_ind();
                  }
              ...
              switch (new_message_ptr->rr.header.rr_message_set)
              {
                case RR_IMSG:
                {
                  switch (new_message_ptr->rr.header.imh.message_id)
                  {
                    case RR_IMSG_RR_EVENT_IND:
                    {
                      MSG_GERAN_LOW_0_G("RR_IMSG_RR_EVENT_IND");
                      break;
                    }
                    ...
                    case RR_IMSG_GERAN_NV_REFRESH_IND:
                    {
                      ...
                      rr_nv_process_nv_refresh_ind();

                      /* Send message to GL1 to refresh GL1 EFS/NV items */
                      rr_l1_send_mph_nv_refresh_req();
                      break;
                    } /* RR_IMSG_GERAN_NV_REFRESH_IND */
#endif /* FEATURE_MODEM_CONFIG_REFRESH */
                    ...
                    {
                      rr_scell_backup_t   *rr_scell_backup_ptr = rr_static_get_scell_backup_ptr();
                      MSG_GERAN_MED_0_G("DMMSUSREM:: Activating GSM");
                      rr_ps_domain_restore();
#ifdef PERLUTF
                      #error code not present
#endif
                      /* Restore PLMN_req data from backup here, will be used to send START_GSM_MODE */
                      /* If CSI data available in static store, try camping on old scell */
                      if (rr_scell_backup_ptr != NULL)
                      {
                        rr_PLMN_request_details_T *rr_PLMN_request_details_ptr = rr_get_PLMN_request_details_ptr();
                        gprs_scell_info_t         *scell_info_ptr = rr_gprs_get_scell_info();

                        /* Update scell_info (used during rps) and mm_send info(used for sending service_ind) */
                        scell_info_ptr->gsm.BCCH_ARFCN = rr_scell_backup_ptr->scell_csi[0].si_arfcn;
                        rr_mm_send_update_old_scell(
                          rr_scell_backup_ptr->scell_csi[0].si_arfcn,
                          rr_scell_backup_ptr->bsic,
                          rr_scell_backup_ptr->scell_csi[0].cgi.lai);

                        memscpy(rr_PLMN_request_details_ptr,
                                sizeof(rr_PLMN_request_details_T),
                                &rr_scell_backup_ptr->plmn_req_details,
                                sizeof(rr_PLMN_request_details_T));

                        MSG_GERAN_MED_0_G("DMMSUSREM:: CSI available, activate gsm & move to DMM_RESUMING");
                        rr_control_data_ptr->rr_state = RR_DMM_RESUMING;
                        new_rr_event = rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
                      }
                      else
                      {
              ...
              switch ((tdsrr_rrc_cmd_e_type) message_id)
              {
                ...
                case TDSRR_INTERRAT_RESELECTION_REQ:
                {
...
#endif /* FEATURE_SGLTE */
                  {
                    ...
                    rr_store_plmn_id(&rr_interrat_reselect_req->requested_PLMN);
                    ...
                    rr_x2g_resel_send_imsg_resel_req(
                      ...
                      rr_is_any_cell_reselection_mode(),
                      &rr_interrat_reselect_req->requested_PLMN,
                      NULL
                    );
                ...
                case TDSRR_INTERRAT_CC_ORDER_REQ:
                {
...
#endif /* FEATURE_SGLTE */
                   {
                     ...
                     (void)rr_timer_start(T3174, req->T309_value * 1000);
                ...
                case TDSRR_INTERRAT_HANDOVER_REQ:
                {
                  ...
                  rr_get_band_dyn() will result in the RR state being checked to
                ...
                case TDSRR_INTERRAT_PLMN_SRCH_REQ:
                {
                  ...
                  rr_start_ttog_plmn_search((tdsrr_interrat_plmn_srch_req_type *)new_message_ptr);
                  new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_START_TTOG, NULL);
                ...
                {
                  rr_send_tds_dedicated_priorities_rsp();
                  break;
...
//                  rr_store_plmn_id(&req->requested_PLMN);
                ...
                case RR_INTERRAT_PLMN_SRCH_REQ:
                {
                  ...
                  rr_start_wtog_plmn_search((rr_interrat_plmn_srch_req_type *)new_message_ptr);
                  new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_START_WTOG, NULL);
                  rr_control_data_ptr->rr_state = RR_PLMN_LIST_SEARCH;
                  break;
                }
                ...
                {
                  rr_send_rrc_dedicated_priorities_rsp();
                  break;
              ...
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
                ...
                case GERAN_GRR_LTE_CCO_REQ:
                {
                  ...
                  if (req->t304 < RR_T304_MINIMUM_DURATION)
                  {
                    ...
                    (void) rr_timer_start(T3174, RR_T304_MINIMUM_DURATION);
                  }
                  else
                  {
                    (void) rr_timer_start(T3174, req->t304);
                  }
                ...
                case GERAN_GRR_LTE_PLMN_SRCH_REQ:
                {
                  ...
                  new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_START_LTOG, NULL);
                  rr_control_data_ptr->rr_state = RR_PLMN_LIST_SEARCH;
                  break;
                } /* GERAN_GRR_LTE_PLMN_SRCH_REQ */
                ...
                case GERAN_GRR_LTE_DEDICATED_PRIORITIES_REQ:
                {
                  ...
                  if (rr_individual_priorities_valid())
                  {
                    lte_dedicated_priorities_ptr->dedicated_pri_valid = TRUE;
                    lte_dedicated_priorities_ptr->plmn = rr_dedicated_priority_info_get_plmn();
                    rr_dedicated_priority_info_get(
                      &(lte_dedicated_priorities_ptr->dedicated_pri_info)
                    );
#ifdef FEATURE_LTE_REL11
        ...
        else if (rr_event == EV_PANIC_RESET)
        {
          ...
          (void)rr_send_grr_mac_reset_req();
      ...
      case RR_W2G_INTERRAT_HANDOVER_PROGRESS:
      {
        ...
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          if (message_set == MS_RR_L2)
          {
            ...
            {
              new_rr_event = rr_inter_cell_handover_control(EV_INPUT_MESSAGE, new_message_ptr);
            }
            else if (message_id == (byte) MDL_ERROR_IND)
            {
              ...
              if (mdl_error_ptr->error_code == UNSOLICITED_DM_RESPONSE_PAR)
              {
                ...
                new_rr_event = rr_inter_cell_handover_control(EV_INPUT_MESSAGE, (rr_cmd_bdy_type *)&release_ind);
              }
              else
              {
            ...
            else if (message_id == (byte) DL_DATA_IND)
            {
                 ...
                 )
              {
                ...
                {
                  rr_perform_classmark_sending(RR_CLASSMARK_CLIENT_NETW);
                }
                else
                {
                  ...
                  rr_send_status(RR_MESSAGE_TYPE_NOT_COMPATIBLE_WITH_PROT_STATE);
                }
              }
              else
              {
          ...
          else if ((message_set == MS_TIMER) && (message_id == (byte) MID_TIMER_EXPIRY))
          {
            switch (new_message_ptr->mid_timer_expiry.timer_id)
            {
              case T3124:
              {
                ...
                new_rr_event = rr_inter_cell_handover_control(EV_T3124_TIMEOUT, NULL);
                break;
              }
        ...
        {
          new_rr_event = rr_inter_cell_handover_control(rr_event, NULL);
        }
        else if (rr_event == EV_GSM_MODE_DEACTIVATED)
        {
          new_rr_event = rr_inter_cell_handover_control(rr_event, NULL);
        ...
        else if (rr_event == RR_EV_W2G_HO_COMPLETED)
        {
          ...
          boolean g2l_blind_redir_required = 
            rr_connection_release_get_g2l_blind_redir_flag();
#endif /* FEATURE_LTE */
          gprs_scell_info_t         * scell_info_ptr = rr_gprs_get_scell_info();
          surr_wcdma_ded_measurements_T *wcdma_ded_meas_ptr = rr_get_wcdma_ded_meas_ptr();
#ifdef FEATURE_GSM_TDS
...
#ifdef FEATURE_SGLTE
         rr_sglte_delete_store_serving_cell_info();
         rr_sglte_reinit_processing_l2g_ho_related_parameter();
#endif /*FEATURE_SGLTE*/
          ...
          (void)rr_connection_establishment_control(EV_CONNECTION_EXISTS, NULL);
          (void)rr_connection_release(EV_CONNECTION_EXISTS, NULL);
          ...
          {
            rr_connection_release_set_g2l_blind_redir_after_csfb_flag();
          }
        ...
        {
          rr_control_data_ptr->rr_state = RR_INACTIVE;
          new_rr_event = rr_inter_cell_handover_control(EV_RESET_SOFTWARE, NULL);

#ifdef FEATURE_SGLTE

          new_rr_event = rr_process_saved_message( &new_message_ptr );

          if((rr_is_sglte_mode_active()) && (rr_sglte_saved_l2g_ho_being_processed()) && (new_rr_event != EV_INPUT_MESSAGE) )
          {
             rr_sglte_srvcc_send_no_service_ind(MM_AS_NO_SERVICE);
          }
          rr_sglte_reinit_processing_l2g_ho_related_parameter();
#endif /*FEATURE_SGLTE*/
...
#endif /*FEATURE_SGLTE*/
          rr_event = EV_NO_EVENT;
          new_rr_event = rr_inter_cell_handover_control(EV_PANIC_RESET_REQ, NULL);
        }
        else if (rr_event == EV_PANIC_RESET_CNF)
        {
          if (rr_verify_panic_reset_saved_message_set(MS_RRC_RR)
#ifdef FEATURE_GSM_TDS
              // TODO DSDA
              || rr_verify_panic_reset_saved_message_set(MS_TDSRRC_RR)
#endif /*FEATURE_GSM_TDS*/
              )
          {
      ...
      case RR_GOING_ACTIVE:
      {
        ...
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          if (message_set == MS_MM_RR)
          {
            ...
            else if (message_id == (byte) RR_EST_REQ)
            {
              ...
              MSG_GERAN_LOW_0_G("Cannot establish RR connection");
              rr_send_abort_ind(RR_CONNECTION_FAILURE, MM_RR_OTHER_FAILURE);
            }
#endif /* FEATURE_GERAN_CS_SUPPORT */
            else if (message_id == (byte) RR_STOP_GSM_MODE_REQ)
            {
              new_rr_event = rr_process_rr_stop_gsm_mode_req(TRUE);
              rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
            }
#ifdef FEATURE_GERAN_CS_SUPPORT
            /******************************************************/
          ...
          else if (message_set == MS_RR_L1 && message_id == MPH_PROC_IND)
          {
            /* start deactivation procedure and goto RR_GOING_INACTIVE */
            rr_delete_saved_msg();
            set_rr_deact_reason(RR_DEACT_REASON_STOP_CAMPING);
            new_rr_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
            rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
          }
#endif /* FEATURE_EXTERNAL_PAGING_GERAN */
        ...
        else if (rr_event == EV_GSM_MODE_ACTIVATED)
        {
          ...
          /* cell selection */
          if (rr_process_saved_message(&new_message_ptr) == EV_INPUT_MESSAGE)
          {
            rr_control_data_ptr->rr_state = RR_NO_CHANNELS;
            new_rr_event = EV_INPUT_MESSAGE;
          }
          else
          {
            ...
            new_rr_event = rr_selection_control(EV_TRIGGER_CELL_SELECTION, NULL);
          }
        }
      ...
      case RR_GOING_INACTIVE:
      {
        ...
        else if (rr_event == EV_GSM_MODE_DEACTIVATED)
        {
          ...
          next action. */
          rr_gsm_as_deactivation_complete_action_e action = rr_gsm_as_deactivation_complete();

          rr_l1_idle_init();//Clear l1 idle init state
          ...
          switch (action)
          {
            ...
            case RR_GSM_AS_DEACTIVATION_COMPLETE_ACTION_RECOVER_FROM_PANIC_RESET:
            {
              ...
              if (rr_verify_panic_reset_saved_message_set(MS_RRC_RR))
              {
                ...
                if (rr_process_panic_reset_saved_msg(&new_message_ptr) == EV_INPUT_MESSAGE)
                {
                  ...
                  if (rr_verify_panic_reset_saved_msg(MS_RRC_RR, RR_INTERRAT_CC_ORDER_REQ))
                  {
...
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_LTE
              if (rr_verify_panic_reset_saved_message_set(MS_MSGR_RR))
              {
...
#ifdef FEATURE_GSM_TDS
              // TODO DSDA
              if (rr_verify_panic_reset_saved_message_set(MS_TDSRRC_RR))
              {
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
              if (rr_get_panic_reset_x2g_cco_flag() != RR_X2G_RESEL_TYPE_UNDEFINED)
              {
                ...
                {
                  switch (rr_get_panic_reset_x2g_cco_flag())
                  {
#ifdef FEATURE_WCDMA
                    case RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER:
                    {
                    ...
                    default:
                    {
                      MSG_GERAN_ERROR_1_G(
                        ...
                        rr_get_panic_reset_x2g_cco_flag()
                      );
                    }
                  }
                ...
                rr_control_data_ptr->rr_panic_reset_info.cco_activation_ind_sent = FALSE;
                rr_set_panic_reset_x2g_cco_flag(RR_X2G_RESEL_TYPE_UNDEFINED);
            ...
            case RR_GSM_AS_DEACTIVATION_COMPLETE_ACTION_GO_TO_INACTIVE:
            {
              ...
              {
                /* Set Dynamic Unloading Pending Flag */
            ...
            case RR_GSM_AS_SGLTE_INTERNAL_DEACTIVATION_COMPLETE_ACTION_PROCESS_L2G_HO_REQ:
            {
              ...
              rr_saved_event = rr_examine_saved_message( &message_ptr );
                  ...
                  )
              {
                ...
                if(rr_saved_l2g_msg_present())
                {
                  rr_delete_saved_l2g_msg();
              ...
              {
                new_rr_event = rr_process_saved_l2g_message( &new_message_ptr );
      ...
      case RR_NO_CHANNELS:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          if (message_set == MS_MM_RR)
          {
            ...
            if (message_id == (byte) RR_PLMN_SELECT_REQ)
            {
              ...
              new_rr_event = rr_selection_control(EV_INPUT_MESSAGE, new_message_ptr);
            }
            ...
            else if (message_id == (byte) RR_PLMN_LIST_REQ)
            {
              ...
              if (rr_mm_get_service_state() != MM_AS_NO_SERVICE)
              {
                rr_send_no_service_ind();
              }

              rr_start_plmn_list_search((rr_plmn_list_req_T *)new_message_ptr, TRUE);
              new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_START, NULL);
              rr_control_data_ptr->rr_state = RR_PLMN_LIST_SEARCH;
            }
            /****************************************************************/
            ...
            else if (message_id == (byte) RR_STOP_GSM_MODE_REQ)
            {
              ...
              if ( rr_x2g_cco_in_progress() )
              {
                ...
                switch (rr_control_data_ptr->x2g_resel_type)
                {
                  ...
                  case RR_X2G_RESEL_TYPE_W2G_CELL_CHANGE_ORDER:
                  {
                    rr_send_interrat_cc_order_cnf(CCO_SUCCESS, RR_GSM_CCO_OTHER_REASON);
                    break;
                  }
#endif /* FEATURE_WCDMA */
        ...
        else if (rr_event == EV_NO_BCCH_FOUND)
        {
          ...
          rr_save_msg_for_recovery_after_panic_reset(NULL);
          rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
          new_rr_event = rr_trigger_gsm_as_deactivation( RR_DEACT_REASON_STOP_CAMPING);
        }
      ...
      case RR_CELL_SELECTION:
      {
          ...
          switch (message_set)
          {
            case MS_RR_L1:
            {
              switch ((rr_l1_message_id_enum_T) message_id)
              {
                ...
                case MPH_PROC_IND:
                {
                  /* Abort the cell selection */
                  new_rr_event = rr_selection_control(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
                  break;
                }
#endif /* FEATURE_EXTERNAL_PAGING_GERAN */
                default:
              ...
              switch ((mm_rr_message_id_enum_T) message_id)
              {
                case RR_PLMN_SELECT_REQ:
                {
                  ...
                  if ((rr_PLMN_request_details_ptr->search_reason == RR_PLMN_SELECTION) &&
                      (rr_compare_plmn_select_req_info(rr_plmn_select_req)))
                  {
                ...
                case RR_PLMN_LIST_REQ:
                {
                  ...
                  if (rr_plmn_list_req->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC)
                  {
                    ...
                    rr_send_plmn_list_cnf(NULL, MM_AS_ABORTED, rr_plmn_list_req);
                  }
                  else
                  {
                    /* save the message, abort the cell selection, then process the message */
                ...
                case RR_EST_REQ:
                {
                      ...
                      (byte)MM_RR_EMERGENCY_CALL)
                  {
                     ...
                     * valid entries.
                     */
                    new_rr_event = rr_selection_control(RR_EV_REPROCESS_CANDIDATE_CELL_DB, NULL);
                  }
                  else
                  if (new_message_ptr->rr_est_req.establishment_cause ==
                ...
                case RR_ABORT_REQ:
                {
                  ...
                  {                  
                    MSG_GERAN_LOW_1_G("RR_ABORT_REQ: Unexpected when No connection Exists in RR State %d", rr_control_data_ptr->rr_state );
                    /* no connection exists, respond with success */
                    rr_send_abort_cnf(MM_AS_SUCCESS);
                ...
                )
        {
          ...
          new_rr_event = rr_selection_control(rr_event, NULL);
...
#ifdef FEATURE_LTE
          rr_dedicated_priorities_request_from_owner();
#endif /* FEATURE_LTE */
...
#ifdef FEATURE_IDLE_DRX_SCALING
          if (rr_idle_drx_start_scaling_suspend_timer())
          {
      ...
      case RR_PLMN_LIST_SEARCH:
      {
        switch (rr_event)
        {
          case EV_INPUT_MESSAGE:
          {
            switch (message_set)
            {
              case MS_TIMER:
              {
                switch (new_message_ptr->mid_timer_expiry.timer_id)
                {
                  case T3122:
                  {
                    if (rr_plc_get_action_after_plmn_search_complete() == RR_PLC_COMPLETE_START_IDLE)
                    {
              ...
              case MS_MSGR_RR:
              {
                switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
                {
                  ...
                  case GERAN_GRR_LTE_ABORT_PLMN_SRCH_REQ:
                  {
                    ...
                    new_rr_event = rr_control_plmn_search_state_msg_handler(message_set, message_id, new_message_ptr);
                    break;
                  } /* GERAN_GRR_LTE_ABORT_PLMN_SRCH_REQ */
            ...
            {
              new_rr_event = rr_plmn_list_control(EV_INPUT_MESSAGE, new_message_ptr);
            }
            break;
          }
...
#endif
              {
                next_action = rr_plc_get_action_after_plmn_search_complete();
                switch (next_action)
                {
                  case RR_PLC_COMPLETE_START_IDLE:
                    ...
                    rr_grr_send_imsg_camped_on_cell_ind();
                    rr_control_data_ptr->rr_state = RR_GRR;
                    break;
                  ...
                  case RR_PLC_COMPLETE_GOTO_INACTIVE:
                    ...
                    if (rr_sglte_internal_stop_gsm_being_processed())
                    {
                      ...
                      new_rr_event = rr_trigger_gsm_as_deactivation(RR_DEACT_REASON_STOP_GSM_MODE_REQ);
                    }
                    else
#endif /* FEATURE_SGLTE */
                    {
                      new_rr_event = rr_trigger_gsm_as_deactivation(RR_DEACT_REASON_X2G_BPLMN_SEARCH_COMPLETE);
                      rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
                    }
                    break;
          ...
          case EV_PLMN_SEARCH_ABORT:
            new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_ABORT, new_message_ptr);
            break;
          ...
          default:
            new_rr_event = rr_plmn_list_control(rr_event, new_message_ptr);
            break;
        }
      ...
      case RR_GRR:
      {
              ...
              switch (new_message_ptr->rr.header.rr_message_set)
              {
                case RR_IMSG:
                {
                  switch (message_id)
                  {
                    case RR_IMSG_PS_ACCESS_IND:
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_PS_ACCESS_IND);
                      break;
                    } /* RR_IMSG_PS_ACCESS_IND */
                    ...
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_INDIVIDUAL_PRIORITIES_UPDATED_IND);
                      break;
                    } /* RR_IMSG_INDIVIDUAL_PRIORITIES_UPDATED_IND */

                    case RR_IMSG_G2L_RESEL_ALLOWED_UPDATED_IND:
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_G2L_RESEL_ALLOWED_UPDATED_IND);
                      break;
                    } /* RR_IMSG_G2L_RESEL_ALLOWED_UPDATED_IND */

                    case RR_IMSG_IRAT_SYSTEM_INFORMATION_UPDATED_IND:
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_IRAT_SYSTEM_INFORMATION_UPDATED_IND);
                      break;
                    ...
                    case RR_IMSG_MSC_CHANGE_IND:
                    {
                      rr_grr_send_imsg_msc_change_ind();
                      break;
                    }
#endif
                    case RR_IMSG_PAGE_IND:
                    {
                      ...
                      rr_grr_send_imsg_page_ind(&imsg->page_info);
                      break;
                    }
                    ...
                    {
                      rr_grr_send_imsg(RR_GRR_IMSG_L1_IDLE_MODE_CHANGE_REQUIRED);
                      break;
                    ...
                    case RR_IMSG_SGLTE_SWITCH_TO_CS_ONLY_GSM_ONLY_REQ:
                    {
                      ...
                      if(rr_x2g_cco_in_progress())
                      {
                        rr_grr_send_imsg_switch_to_cs_only_gsm_only_req(
                          rr_sglte_x2g_cco_nas_split_cb_fn
                        );
                      }
                      else
                      {
                        rr_grr_send_imsg_switch_to_cs_only_gsm_only_req(
                          rr_sglte_confirm_rat_change_cb_fn
                        );
                      }
                      break;
                  ...
                  switch (message_id)
                  {
                    ...
                    case RR_GRR_OMSG_X2G_CCO_STATUS_IND:
                    {
                      ...
                      switch (new_message_ptr->rr.grr.omsg.x2g_cco_status_ind.status)
                      {
...
#ifdef FEATURE_SGLTE
                              rr_x2g_send_x2g_cco_rsp_success(RR_RAT_WCDMA);
#else
                              rr_send_interrat_cc_order_cnf(CCO_SUCCESS, RR_GSM_CCO_OTHER_REASON);
#endif /* FEATURE_SGLTE */
...
#ifdef FEATURE_SGLTE
                              rr_x2g_send_x2g_cco_rsp_success(RR_RAT_UTRAN_TDD);
#else
                              rr_send_tds_interrat_cc_order_cnf(
                                TDSCCO_SUCCESS,
                                TDSRR_GSM_CCO_OTHER_REASON
...
#ifdef FEATURE_SGLTE
                              rr_x2g_send_x2g_cco_rsp_success(RR_RAT_LTE);
#else
                              geran_grr_lte_cco_rsp_t lte_cco_rsp;
                              errno_enum_type         result;
                        ...
                        {
                          RR_ASSERT_WARN(rr_is_sglte_mode_active());
                        ...
                        default:
                        {
                          ...
                          set_rr_deact_reason(RR_DEACT_REASON_SWITCH_TO_OTHER_RAT);
                          new_rr_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
                    ...
                    {
                      rr_x2g_send_x2g_IRAT_rej(rr_control_data_ptr->x2g_resel_type);
                      MSG_GERAN_MED_0_G("Clearing x2g_resel_type / x2g_resel_abort_cause");
                      rr_control_data_ptr->x2g_resel_type = RR_X2G_RESEL_TYPE_UNDEFINED;
                      rr_control_data_ptr->x2g_resel_abort_cause = RR_X2G_RESEL_ABORT_CAUSE_UNDEFINED;
                      rr_control_data_ptr->x2g_cco_status = RR_CCO_STATUS_UNDEFINED;
                ...
                default:
                {
                  new_rr_event = grr_control(EV_INPUT_MESSAGE, new_message_ptr);
                }
              }
              break;
            } /* MS_RR_RR */
            ...
            case MS_MSGR_RR:
            {
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
                ...
                case GERAN_GRR_LTE_RESEL_REQ:
                {
                  if (rr_is_sglte_mode_active())
                  {
                    if (rr_sglte_is_full_service_available() && rr_cell_supports_gprs())
                    {
                ...
                case GERAN_GRR_LTE_ABORT_CCO_REQ:
                {
                  rr_grr_send_imsg(RR_GRR_IMSG_SGLTE_X2G_ABORT_REQ);
                  break;
                } /* GERAN_GRR_LTE_ABORT_CCO_REQ */
#endif /* FEATURE_LTE */
        ...
        else if (rr_event == RR_EV_PROCESS_SAVED_MESSAGE)
        {
          ...
          if ( rr_event == EV_INPUT_MESSAGE )
          {
            ...
            if ( (message_set == MS_MM_RR) && (message_id == (byte) RR_PLMN_LIST_REQ) )
            {
              ...
              if (rr_is_camped() && (scell_ptr->l1_idle_mode != GRR_L1_IDLE_MODE_NONE))
              {
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
                if (geran_in_multi_standby_mode() && rr_limited_service_in_effect())
                {
...
#ifdef FEATURE_GERAN_CS_SUPPORT
                else if (geran_in_multi_standby_mode() && rr_ms_other_stack_in_dedicated())
                {
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
                else if (rr_is_forbidden_LAI(&(scell_ptr->gsm.location_area_identification)))
                {
                ...
                else if (rr_ms_last_bplmn_failed(req_ptr))
                {
              ...
              rr_start_plmn_list_search(req_ptr, foreground_search);
              new_rr_event = rr_plmn_list_control(EV_PLMN_SEARCH_START, NULL);
              rr_control_data_ptr->rr_state = RR_PLMN_LIST_SEARCH;
            }
        ...
        {
          rr_control_data_ptr->rr_state = RR_CELL_SELECTION;
          new_rr_event = rr_selection_control(RR_EV_PROCESS_PLMN_SELECT_REQ, NULL);
        }
        ...
        if ( rr_event == EV_RR_EST_REQ )
        {
          ...
          rr_leave_idle_mode(RR_CONNECTION_ESTABLISHMENT);
        }
#endif /* FEATURE_GERAN_CS_SUPPORT */
        ...
        if ( rr_event == EV_NO_BCCH_FOUND )
        {
          ...
          {
            rr_scell_backup_t* rr_scell_backup_ptr = rr_static_get_scell_backup_ptr();
            /* Clear any saved message */
            rr_delete_saved_msg();

            /* Send service_ind only if GRR is unable to return to old s/ncell */
            if (rr_scell_backup_ptr == NULL)
            {
              rr_send_no_service_ind();
            }
        ...
        {
          new_rr_event = grr_control(rr_event, NULL);
        }
        ...
        else if(rr_event == EV_ACCESS_CLASS_NOT_ALLOWED)
        {
          ...
          /* GRR to GRR_CAMPED*/
          new_rr_event = grr_control(RR_EV_CAMPED_ON_CELL, NULL);
        }
      ...
      case RR_CONNECTION_PENDING:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          else if (message_set == MS_RR_L1)
          {
            ...
            else if (message_id == (byte) MPH_SURROUND_UPDATE_FAILURE_IND)
            {
              ...
              rr_handle_surround_update_failure_ind(su_fail_ptr, FALSE, FALSE);
            }

            else if (message_id == (byte) MPH_IMMED_ASSIGN_CNF    ||
                     message_id == (byte) MPH_CHANNEL_RELEASE_CNF ||
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
                     message_id == (byte) MPH_PROC_IND            ||
#endif /* FEATURE_EXTERNAL_PAGING_GERAN */
                     message_id == (byte) MPH_BLOCK_QUALITY_IND)
            {
          ...
          else if (message_set == MS_MM_RR)
          {
            ...
            else if (message_id == (byte) RR_PLMN_LIST_REQ)
            {
              ...
              {
                MSG_GERAN_LOW_0_G("Reject MM request while RR connected");
                rr_reject_msg(new_message_ptr);
              }
            }
            else if (message_id == (byte) RR_STOP_GSM_MODE_REQ)
            ...
            {
              if (rr_ce_conn_is_requested_by_nas() == FALSE)
              {
#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
                (void) rr_gprs_edtm_prioritise_saved_msg(new_message_ptr);
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
                /* Queue the request and handle it after conn est has been aborted */
                (void)rr_save_msg(new_message_ptr);
            ...
            {
              boolean redir = rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled();
              boolean redir_after_csfb = rr_g2l_blind_redir_after_csfb_enabled();
          ...
          else if ( message_set == MS_CGPS_RR )
          {
            switch ( message_id )
            {
              ...
              {
                rr_cgps_process_cgps_event_req(new_message_ptr);
                break;
              }
          ...
          else if ((message_set == MS_TIMER) && (message_id == MID_TIMER_EXPIRY))
          {
            ...
            /*Optimisation: When SGLTE mode is active and T3126 expires
              ...
              tries to reselect*/
                (void)rr_sglte_reject_pending_x2g_req(FALSE);
#endif /*FEATURE_SGLTE*/
                new_rr_event = rr_connection_establishment_control(EV_T3126_EXPIRY, NULL);
                break;
              }

              case T3174:
              {
                new_rr_event = rr_connection_establishment_control(EV_T3174_EXPIRY, NULL);
                break;
              }
              ...
              default:
              {
                LOG_UNEXPECTED_TIMER_EXPIRY(
                  new_message_ptr->mid_timer_expiry.timer_id,
                  grr_control_current_state_name()
                );
              }
            }
          }
          ...
          else if (message_set == MS_MAC_RR)
          {
            switch (message_id)
            {
              ...
              {
#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
                new_rr_event = rr_gprs_edtm_control(EV_INPUT_MESSAGE, new_message_ptr);
#endif /* FEATURE_GSM_DTM && FEATURE_GSM_EDTM */
                break;
              } /* MAC_GRR_MAC_STATUS */
              ...
              case GERAN_GRR_LTE_PLMN_SRCH_REQ:
              {
                    ...
                    new_message_ptr->rr_msgr_msg.message.geran_grr_msg_req.plmn_srch_req.network_select_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)
                {
                  ...
                  rr_sglte_send_lte_plmn_srch_rsp_in_dedicated(
                    rr_scell_plmn_id(),
                    measured_signal_data->current_signal_strength,
                    rr_cell_supports_gprs() ? SYS_SRV_CAPABILITY_CS_PS_SERVICE : SYS_SRV_CAPABILITY_CS_SERVICE_ONLY
                  );
                }
                else
                {
                 ...
                 (rr_event == RR_EV_START_RESELECTION_POWER_SCAN))
        {
          ...
          if (rr_log_reselection_trigger == RR_RESELECTION_TRIGGER_SERVING_BARRED)
          {
            ...
            rr_send_abort_ind(RANDOM_ACCESS_FAILURE, MM_RR_OTHER_FAILURE);
          }

          #ifdef FEATURE_GSM_DTM
          (void) rr_dtm_control(EV_DTM_RR_EST_FAIL, NULL);
          #endif /* FEATURE_GSM_DTM */
          ...
          if ((rr_connection_release_get_g2l_blind_redir_flag() || 
              ...
              rr_is_lte_enabled())
          {
            rr_chan_rel_cell_selection_indicator_t *csi = rr_get_cell_select_indicator_ptr();
            ...
            longer required. */
            rr_init_cell_selection_indicator();
        ...
        else if (rr_event == EV_RR_EST_L1_IN_DED)
        {
          ...
          rr_ded_meas_reporting_l1_in_dedicated();
          rr_clear_si_sacch();
#ifdef FEATURE_RR_SI_CACHE
          rr_clear_dedicated_sys_info_cache();
#endif
          ...
          rr_set_status_of_old_cell_si5_ba_lists(TRUE);
        ...
        else if (rr_event == EV_RR_EST_CNF)
        {
          ...
          rr_leave_idle_mode(RR_DEDICATED);

          (void) rr_connection_release(EV_CONNECTION_EXISTS, NULL);
          (void) rr_cell_reest_control(EV_CONNECTION_EXISTS, NULL);
          (void) rr_resel_control(EV_RR_EST_IND, NULL);
          ...
          if (rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled())
          {
            boolean cs_is_for_voice_or_periodic_lau = FALSE;

            if (rr_ce_access_reason() == MM_RR_MO_FULL_SPEECH ||
                rr_ce_access_reason() == MM_RR_MO_HALF_SPEECH ||
                rr_ce_access_reason() == MM_RR_SUPPLEMENTARY_SERVICE ||
                rr_ce_access_reason() == MM_RR_CALL_REEST)
            {
              cs_is_for_voice_or_periodic_lau = TRUE;
            }
            else if (rr_ce_access_reason() == MM_RR_LOCATION_UPDATE)
            {
              if (rr_ce_is_periodic_lau())
              {
                cs_is_for_voice_or_periodic_lau = TRUE;
              }
            }
        ...
        else if ( rr_event == EV_CONNECTION_ABORTION)
        {
          ...
          geran_eng_mode_data_write(ENG_MODE_GSM_CIPHER_INFO,NULL, FALSE); /*cipher mode invalid, connection released*/

          rr_control_transition_to_state(RR_CONNECTION_RELEASE);
        }
        ...
        else if ( rr_event == RR_EV_PROCESS_SAVED_MESSAGE )
        {
          ...
          if (TRUE == rr_static_get_suspend_in_progress())
          {
            MSG_GERAN_MED_0_G("DMMSUSREM:: CP process_saved_msg, go to INACTIVE");
            rr_perform_backup_for_scell();
            set_rr_deact_reason(RR_DEACT_REASON_STOP_CAMPING);
            new_rr_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
            rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
        ...
        else if ( rr_event == EV_PANIC_RESET )
        {
          ...
          rr_conn_set_release_cause(RELEASE_CAUSE_CONN_EST_FAILURE);
      ...
      case RR_CELL_REESTABLISH:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
            ...
            switch (new_message_ptr->rr.header.rr_message_set)
            {
              case RR_IMSG:
              {
                switch (new_message_ptr->rr.header.imh.message_id)
                {
                  ...
                  default:
                  {
                    new_rr_event = rr_cell_reest_control(EV_INPUT_MESSAGE, new_message_ptr);
                  }
                }
                break;
              }
          ...
          else if (message_set == MS_MM_RR)
          {
            ...
            {
              (void) rr_cell_reest_control(EV_INPUT_MESSAGE, new_message_ptr);
            }
            ...
            */
            else if (message_id == (byte) RR_PLMN_LIST_REQ)
            {
              ...
              if ( rr_plmn_list_req->network_selection_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC )
              {
                MSG_GERAN_LOW_0_G("Rejecting HPLMN search during reest");
                rr_reject_msg(new_message_ptr);
              }
              else
              {
                ...
                new_rr_event = rr_cell_reest_control(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
            ...
            */
            ...
            {
              MSG_GERAN_LOW_0_G("Reest aborted for RR_PLMN_SELECT_REQ");
            ...
            */
            ...
            {
              MSG_GERAN_LOW_0_G("Reest aborted for RR_STOP_GSM_MODE_REQ");
            ...
            */
            ...
            {
              MSG_GERAN_LOW_0_G("Reest aborted for RR_DEACT_REQ");
        ...
        {
          new_rr_event = rr_cell_reest_control(rr_event, NULL);
        }
        ...
        */
        else if (rr_event == EV_NO_BCCH_FOUND)
        {
          ...
          else
          {
            ...
            (void) rr_selection_control(EV_TRIGGER_CELL_SELECTION, NULL);
            rr_control_data_ptr->rr_state = RR_CELL_SELECTION;
          }
        }
        ...
        */
        else if (rr_event == RR_EV_CAMPED_ON_CELL)
        {
          ...
          rr_event = rr_examine_saved_message(&message_ptr);
               ...
               (message_ptr->rr_est_req.establishment_cause == (byte) MM_RR_CALL_REEST) )
          {
            ...
            (void) rr_connection_establishment_control( EV_INPUT_MESSAGE, message_ptr);
        ...
        else if (rr_event == EV_G2L_BLIND_REDIR_REQUIRED)
        {
          ...
          else
          {
            ...
            rr_ps_no_ps_access_del_all();
            rr_g2x_redir_init();
            rr_g2x_redir_send_imsg_redir_req(csi);  
            rr_init_cell_selection_indicator();
            rr_control_data_ptr->rr_state = RR_G2X_REDIR;
          }    
        ...
        else if (rr_event == RR_EV_PROCESS_SAVED_MESSAGE)
        {
               ...
               (new_message_ptr != NULL) )
          {
            ...
            if ( (message_set == MS_MM_RR) && (message_id == (byte) RR_PLMN_LIST_REQ) )
            {
              ...
              plc_event = rr_plmn_list_control(EV_PLMN_SEARCH_START, NULL);
            ...
            {
              MSG_GERAN_LOW_0_G("TODO: Handle RR_PLMN_SELECT_REQ properly");
        ...
        {
          /* Reset cell selection ready for restart */
          (void) rr_cell_reest_control(EV_RESET_SOFTWARE, NULL);
        }
      ...
      case RR_DATA_TRANSFER:
      {
          ...
          switch (message_set)
          {
            case MS_RR_RR:
            {
              switch (new_message_ptr->rr.header.rr_message_set)
              {
                case RR_IMSG:
                {
                  switch (new_message_ptr->rr.rr.imsg.header.imh.message_id)
                  {
                    case RR_IMSG_PS_ACCESS_REQ:
                    {
                     /* FR - 19574  Pass this message to Our Reselection Control,where, it will send a request to MAC, via PS ACCESS FSM*/
                      ...
                      if ( rr_is_sglte_mode_active() && rr_get_nv_sglte_g2x_conn_mode_resel_enabled() )
                      {
                        ...
                        rr_event = rr_sglte_rr_conn_g2x_resel_control(rr_event, new_message_ptr );
                      }
#endif
#endif
                      if (rr_event == EV_INPUT_MESSAGE)
                    ...
                    case RR_IMSG_X2G_CCO_COMPLETE_IND:
                    {
                      ...
                      if (rr_is_sglte_mode_active() && rr_control_data_ptr->sglte_pending_rat_pri_list_valid)
                      {
                         /* CS is connected.  RR_CHANGE_MODE_IND may change service
                        domain and/or RAT priority list. */
                        rr_data_transfer_handle_rr_change_mode_ind(
                        ...
                        rr_sglte_set_lte_blind_redir_in_data_transfer();
                    ...
                    {
#ifdef FEATURE_GSM_DTM
                      (void) rr_dtm_control(EV_DTM_MSC_CHANGE_IND, NULL);
                    ...
                    case RR_IMSG_SGLTE_SWITCH_TO_CS_ONLY_USER_GSM_ONLY_REQ:
                    {
                      ...
                      if ( rr_is_sglte_mode_active() && rr_get_nv_sglte_g2x_conn_mode_resel_enabled() )
                      {
                        ...
                        /* Inject it to Connected Mode Reselection FSM */
                        new_rr_event = rr_sglte_rr_conn_g2x_resel_control(EV_G2X_CONN_RESEL_ABORT_REQ, new_message_ptr );
                ...
                case RR_CE_IMSG:
                {
                  new_rr_event = rr_connection_release(EV_INPUT_MESSAGE, new_message_ptr);
                  break;
                }
 ...
 #endif
                  (void)rr_dtm_control(EV_INPUT_MESSAGE, new_message_ptr);
                  break;
                }
            ...
            case MS_RR_L1:
            {
              switch ((rr_l1_message_id_enum_T) message_id)
              {
                ...
                case MPH_SURROUND_IRAT_MEAS_IND:
                {
                 ...
                 if ( rr_is_sglte_mode_active() && rr_get_nv_sglte_g2x_conn_mode_resel_enabled() )
                 {
                     if ( rr_sel_get_service_domain() == SYS_SRV_DOMAIN_CS_PS )
                     {
                       /* First invoke G2W function to save the measurement results */
                       rr_irat_process_surround_irat_meas_ind(&new_message_ptr->mph.surround_irat_meas_ind);
                       rr_sglte_rr_conn_g2x_resel_control(EV_IRAT_SURROUND_MEAS_IND, new_message_ptr );
                     }
                 }
#endif
            ...
            case MS_RR_L2:
            {
              switch ((rr_l2_message_id_enum_T) message_id)
              {
                ...
                {
                  new_rr_event = rr_handle_procedures(EV_INPUT_MESSAGE, new_message_ptr);
                  break;
                }
                     ...
                     )
                  {
                    ...
                    if (rr_gsm_only())
                    {
                      ...
                      new_rr_event = EV_NO_EVENT;
                             ...
                             (FALSE == rr_is_wcdma_enabled()))
                    {
                      MSG_GERAN_MED_0_G("Rx UTRAN_HO_CMD: WCDMA not supported");
                      send_handover_failure_ota(RR_PROTOCOL_ERROR_UNSPECIFIED);
                      new_rr_event = EV_NO_EVENT;
                    }
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS */
                    else
                       ...
                       )
                    {
                      ...
                      switch (message_type)
                      {
                        ...
                        case APPLICATION_INFORMATION:
                        {
                          ...
                          rr_cgps_process_apdu_data_seg(new_message_ptr);
                ...
                case DL_UNIT_DATA_IND:
                {
                  ...
                  if (rr_prepare_ota_msg_for_parsing(dl_unit_data_ind))
                  {
                  ...
                  {
                    MSG_GERAN_LOW_0_G("ProtocolError for UnitData");
                  }
                  break;
                }
                ...
                {
                  rr_process_dl_data_ack_ind(&new_message_ptr->dl_data_ack_ind);
                  break;
                }
            ...
            case MS_MM_RR:
            {
              switch ((mm_rr_message_id_enum_T) message_id)
              {
                case RR_ABORT_REQ:
                {
                  ...
                  if ( rr_is_sglte_mode_active() && rr_get_nv_sglte_g2x_conn_mode_resel_enabled() )
                  {
                    new_rr_event = rr_sglte_rr_conn_g2x_resel_control(
                      EV_G2X_CONN_RESEL_ABORT_REQ, new_message_ptr
                    );
                ...
                case RR_DATA_REQ:
                {
                  rr_process_rr_data_req(&(new_message_ptr->rr_data_req));
                  break;
                }
                ...
                case RR_STOP_GSM_MODE_REQ:
                {
                  ...
                  will be generated. */
                  new_rr_event = rr_connection_release(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
                  break;
                }
                  ...
                  if (rr_is_sglte_mode_active())
                  {
                    ...
                    rr_data_transfer_handle_rr_change_mode_ind(
                      &new_message_ptr->mm.rr_change_mode_ind.rat_pri_list_info,
                       new_message_ptr->mm.rr_change_mode_ind.service_domain
                    );
                  }
                  else
#endif /* FEATURE_SGLTE */
                  {
                    rr_pending_mode_change_data_store(&new_message_ptr->mm.rr_change_mode_ind);
                  }
                  break;
                } /* RR_CHANGE_MODE_IND */
                  ...
                  if (rr_is_sglte_mode_active())
                  {
                    ...
                    switch (rr_control_data_ptr->x2g_resel_type)
                    {
                      ...
                      case RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER:
                      {
                        ...
                        rr_send_tds_interrat_cc_order_cnf(TDSCCO_SUCCESS, TDSRR_GSM_CCO_OTHER_REASON);
                        rr_control_data_ptr->waiting_for_tdsrr_interrat_tl1_stop_ind = TRUE;
                        break;
                      } /* RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER */
                    ...
                    if (rr_control_data_ptr->x2g_resel_type == RR_X2G_RESEL_TYPE_UNDEFINED)
                    {
                      ...
                      rr_sglte_dedicated_priority_info_store(&rr_control_data_ptr->sglte_ded_pri_info);
            ...
            case MS_CGPS_RR:
            {
              switch (message_id)
              {
                case RR_APDU_REQ:
                {
                  rr_cgps_process_rr_apdu_req(RR_APDU_ID_RRLP, new_message_ptr);
                  break;
                }
            ...
            case MS_TIMER:
            {
              switch (new_message_ptr->mid_timer_expiry.timer_id)
              {
                case T3124:
                {
                  new_rr_event = rr_handle_procedures(EV_T3124_TIMEOUT, NULL);
                  break;
                }
            ...
            case MS_MAC_RR:
            {
              ...
              (void) rr_handle_procedures(EV_INPUT_MESSAGE, new_message_ptr);
              break;
            } /* MS_MAC_RR */
            ...
            case MS_MSGR_RR:
            {
              switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
              {
                ...
                case GERAN_GRR_LTE_CCO_REQ:
                {
                  if (rr_is_sglte_mode_active())
                  {
                    if (rr_sglte_is_full_service_available())
                    {
                      ...
                      rr_send_activation_ind(MM_AS_LTOG_CCO);
                      rr_control_data_ptr->x2g_resel_type = RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER;
                    }
                    else
                    {
                ...
                case GERAN_GRR_LTE_ABORT_CCO_REQ:
                {
                  rr_sglte_x2g_cco_aborted_cb_fn();
                  break;
                } /* GERAN_GRR_LTE_ABORT_CCO_REQ */

                case GERAN_GRR_LTE_ABORT_RESEL_REQ:
                {
                  rr_sglte_x2g_resel_aborted_cb_fn();
                  break;
                } /* GERAN_GRR_LTE_ABORT_RESEL_REQ */

                case GERAN_GRR_LTE_ABORT_REDIR_REQ:
                {
                  rr_sglte_x2g_redir_aborted_cb_fn();
                  break;
                } /* GERAN_GRR_LTE_ABORT_REDIR_REQ */
                ...
                case TDSRR_INTERRAT_CC_ORDER_REQ:
                {
                  if (rr_is_sglte_mode_active())
                  {
                    if (rr_sglte_is_full_service_available())
                    {
                      ...
                      rr_send_activation_ind(MM_AS_WTOG_INTERRAT_CC_ORDER);
                      rr_control_data_ptr->x2g_resel_type = RR_X2G_RESEL_TYPE_T2G_CELL_CHANGE_ORDER;
                    }
                    else
                    {
        ...
        else if (rr_event == EV_RADIO_LINK_FAILURE)
        {
          ...
          new_rr_event = rr_connection_release(EV_RADIO_LINK_FAILURE, NULL);
        }
        else if (rr_event == EV_PROCEDURE_COMPLETE)
        {
          ...
          new_rr_event = rr_connection_release(EV_PROCEDURE_COMPLETE, NULL);
        }
        else if (rr_event == RR_EV_GSM_SI_RECEIVED)
        {
        ...
        else if (rr_event == RR_EV_CONNECTION_RELEASE_IN_PROGRESS)
        {
          ...
          (void) rr_inter_cell_handover_control(EV_RESET_SOFTWARE, NULL);
          (void) rr_intra_cell_handover_control(EV_RESET_SOFTWARE, NULL);
          (void) rr_handle_procedures(EV_RESET_SOFTWARE, NULL);
          rr_eng_mode_update_dtx_ul_status(FALSE); /*Connection is being released default to DTX FALSE*/
          geran_eng_mode_data_write(ENG_MODE_GSM_CIPHER_INFO,NULL, FALSE); /*cipher mode invalid, connection released*/
        ...
        else if (rr_event == EV_PANIC_RESET)
        {
          ...
          (void) rr_connection_establishment_control(EV_RESET_SOFTWARE, NULL);
          (void) rr_connection_release(EV_RESET_SOFTWARE, NULL);
          (void) rr_inter_cell_handover_control(EV_RESET_SOFTWARE, NULL);
          (void) rr_intra_cell_handover_control(EV_RESET_SOFTWARE, NULL);
          (void) rr_handle_procedures(EV_RESET_SOFTWARE, NULL);
          rr_reset_sapis_due_to_failure();
        ...
        {
          (void)rr_handle_procedures(EV_MAC_IDLE_MODE, NULL);
        }
      ...
      case RR_CONNECTION_RELEASE:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          if (message_set == MS_RR_RR)
          {
            switch (new_message_ptr->rr.header.rr_message_set)
            {
              ...
              case RR_CE_IMSG:
              {
                rr_connection_release(EV_INPUT_MESSAGE, new_message_ptr);
                break;
              }
                  ...
                  case RR_CE_OMSG_CS_RELEASE_IND:
                  {
                    ...
                    switch (imsg_ptr->status)
                    {
                      case RR_CE_REL_STATUS_CAMP_ON_CELL:
                      {
                        ...
                        rr_control_transition_to_state(RR_GRR);
                        break;
                      }
                      ...
                      case RR_CE_REL_STATUS_ACQUIRE_SI:
                      {
                        ...
                        MSG_GERAN_HIGH_1_G("SI not valid: invalidate_si_reason = %d", rr_gprs_get_scell_info()->gsm.invalidate_si_reason);

#ifdef FEATURE_EXTERNAL_PAGING_GERAN
                        if (rr_static_get_suspend_in_progress() == FALSE)
#endif
                        {
                          ...
                          rr_control_transition_to_state(RR_CELL_SELECTED_ACQUIRE_SI);
                   ...
                   (message_id == (byte) MPH_SURROUND_MEAS_IND))
          {
            ...
            rr_store_surround_meas(&mph_surround_meas_ind->best_six_info, TRUE);
          }
          ...
          else if (message_set == MS_RR_L2)
          {
            ...
            else if (message_id == (byte) DL_DATA_ACK_IND)
            {
              ...
              rr_process_dl_data_ack_ind((dl_data_ack_ind_T *)new_message_ptr);
            }
            else
            {
              MSG_GERAN_ERROR_3_G(
          ...
          else if ((message_set == MS_TIMER) && (message_id == (byte) MID_TIMER_EXPIRY))
          {
            switch (new_message_ptr->mid_timer_expiry.timer_id)
            {
              case T3110:
              {
                new_rr_event = rr_connection_release(EV_T3110_TIMEOUT, NULL);
                break;
              }
          ...
          if (rr_cell_select_indicator_ptr->valid)
          {
              ...
              if (rr_is_wcdma_enabled())
              {
                ...
                */
                     ...
                     ++i)
                {
                  ...
                  rr_g2x_redir_triggered(frequency);

                  if (rr_g2x_is_uarfcn_valid(uarfcn, scell->gsm.location_area_identification.plmn_id))
                  {
                    bands_ok = TRUE;
                  }
                }
                ...
                if (bands_ok)
                {
                  ...
                  rr_control_transition_to_state(RR_G2X_REDIR);
                }
                else
                {
                  MSG_GERAN_MED_0_G(
            ...
            else if (rr_cell_select_indicator_ptr->type == LTE_TARGET)
            {
                   ...
                   ++i)
              {
                ...
                if (lte_rrc_earfcn_is_supported(frequency.data.lte, rr_band_pref_lte()))
                {
                  bands_ok = TRUE;
                }
              }
        ...
        else if (rr_event == EV_G2T_PSEUDO_FR_REQUIRED)
        {
          ...
          rr_tds_pseudo_fr_populate_freq_list(&(csi->target.tdd_target.target));
        ...
        {
          rr_control_transition_to_state(RR_CELL_SELECTION);
        }
        else if (rr_event == RR_EV_CELL_REEST_TRIGGERED)
        {
          rr_control_transition_to_state(RR_CELL_REESTABLISH);
        }
        ...
        else if ( rr_event == RR_EV_PROCESS_SAVED_MESSAGE )
        {
          ...
          if (TRUE == rr_static_get_suspend_in_progress())
          {
            MSG_GERAN_MED_0_G("DMMSUSREM:: CR process_saved_msg, go to INACTIVE");
            rr_perform_backup_for_scell();
            set_rr_deact_reason(RR_DEACT_REASON_STOP_CAMPING);
            new_rr_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
            rr_control_transition_to_state(RR_GOING_INACTIVE);
          }
          else
#endif /* FEATURE_EXTERNAL_PAGING_GERAN */
          {
            rr_control_transition_to_state(RR_NO_CHANNELS);
            new_rr_event = rr_process_saved_message(&new_message_ptr);
        ...
        if ( rr_event == EV_NO_BCCH_FOUND )
        {
          ...
          new_rr_event = rr_trigger_gsm_as_deactivation(RR_DEACT_REASON_STOP_CAMPING_THEN_RESTART);
        }
        ...
        else if ( rr_event == EV_X2G_CCO_FAILURE_CCO_TIMER_EXPIRY )
        {
          ...
          rr_control_transition_to_state(RR_X2G_CCO_FAILED);
      ...
      case RR_EARLY_CAMPED_WAIT_FOR_SI:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          if (message_set == MS_RR_RR)
          {
            switch (new_message_ptr->rr.header.rr_message_set)
            {
              ...
              {
                rr_l1_idle_control(new_message_ptr);
                break;
              }
          ...
          else if (message_set == MS_RR_L1)
          {
            ...
            else if (message_id == (byte) MPH_SERVING_IDLE_MEAS_IND)
            {
              ...
              /* Set the measurement timestamp for scell */
              rr_set_scell_meas_timestamp(GSTMR_GET_FN_GERAN());
            }
          ...
          else if (message_set == MS_MM_RR)
          {
            ...
            else if (message_id == (byte) RR_PLMN_LIST_REQ)
            {
              ...
              */
              ...
              {
                (void)rr_save_msg(new_message_ptr);
              MSG_GERAN_LOW_0_G("Saved RR_PLMN_SELECT_REQ in EARLY_CAMP_WAIT_FOR_SI");
              }
            }
            else if (message_id == (byte) RR_PLMN_SELECT_REQ)
            {
               ...
               */
              (void)rr_save_msg(new_message_ptr);
              MSG_GERAN_LOW_0_G("Saved RR_PLMN_SELECT_REQ during EARLY_CAMP_WAIT_FOR_SI");
            }
            else if (message_id == (byte) RR_STOP_GSM_MODE_REQ)
            {
          ...
          else if ((message_set == MS_TIMER) && (message_id == (byte) MID_TIMER_EXPIRY))
          {
            switch (new_message_ptr->mid_timer_expiry.timer_id)
            {
              case RR_GSM_SYS_INFO_TIMER:
              {
                ...
                if (rr_abort_pending_rr_est_req(RR_ABORT_CON_EST_NOT_ATTEMPTED))
                {
                  ...
                  (rr_event == EV_TRIGGER_CELL_SELECTION) )
        {
          ...
          rr_leave_idle_mode(RR_NOT_CAMPED);
          ...
          else if (grr_verify_saved_mm_message(RR_PLMN_SELECT_REQ))
          {
            /**
             * Layer 1 will be stopped and restarted - RR will check for saved
            ...
            else
            {
               ...
               /* c) Band not supported                                 */
               {
                  MSG_GERAN_HIGH_3_G(
                    ...
                    (int) rr_is_any_cell_reselection_mode(),
      ...
      case RR_G2W_INTERRAT_HANDOVER_PROGRESS:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          {
            new_rr_event = rr_g2w_handover_control(EV_INPUT_MESSAGE, new_message_ptr);
          }
          else if ((message_set == MS_RR_L1) &&
                   ((message_id == (byte) MPH_SERVING_DED_MEAS_IND) ||
                    (message_id == (byte) MPH_SURROUND_MEAS_IND)))
        ...
        else if ((rr_event == EV_GSM_MODE_ACTIVATED) || (rr_event == EV_GSM_MODE_DEACTIVATED))
        {
          ...
          new_rr_event = rr_g2w_handover_control(rr_event, NULL);
        }
        else if (rr_event == EV_RADIO_LINK_FAILURE)
        {
        ...
        else if (rr_event == RR_EV_G2W_HO_ABORTED)
        {
          ...
          (void) rr_handle_procedures(RR_EV_G2W_HO_ABORTED, NULL);
        ...
        {
          new_rr_event = rr_handle_procedures(RR_EV_G2W_HO_FAIL, NULL);
        ...
        else if (rr_event == RR_EV_G2W_HO_SUCCESS)
        {
          ...
          (void) rr_handle_procedures(RR_EV_G2W_HO_SUCCESS, NULL);
      ...
      case RR_W2G_SERVICE_REDIRECTION_IN_PROGRESS:
      {
        if (rr_event == EV_INPUT_MESSAGE)
        {
          if (message_set == MS_RRC_RR)
          {
            ...
            switch (message_id)
            {
              case RR_INTERRAT_REDIRECT_ABORT_REQ:
              {
                ...
                new_rr_event = rr_w2g_service_redirection_control(rr_event, new_message_ptr);
                break;
              } /* RR_INTERRAT_REDIRECT_ABORT_REQ */
          ...
          else if (message_set == MS_MM_RR)
          {
            ...
            else if ((mm_rr_message_id_enum_T) message_id == RR_PLMN_LIST_REQ)
            {
              if (rr_save_msg(new_message_ptr))
              {
                new_rr_event = rr_w2g_service_redirection_control(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
              }
              else
              {
            ...
            else if ((mm_rr_message_id_enum_T) message_id == RR_EST_REQ)
            {
              ...
              rr_send_abort_ind(REASON_NOT_AVAILABLE, MM_RR_OTHER_FAILURE);
            }
#endif /* FEATURE_GERAN_CS_SUPPORT */
            ...
            switch (new_message_ptr->rr.header.rr_message_set)
            {
              ...
              case RR_L1_IDLE_OMSG:
              {
                switch (new_message_ptr->message_header.message_id)
                {
                  case RR_L1_IDLE_OMSG_START_EARLY_CAMP_CNF:
                  {
                    rr_w2g_service_redirection_control(EV_INPUT_MESSAGE, new_message_ptr);
                    break;
                  }
        ...
        else if (rr_event == EV_PANIC_RESET)
        {
          ...
          rr_event = EV_NO_EVENT;
          new_rr_event = rr_w2g_service_redirection_control(EV_PANIC_RESET_REQ, NULL);
        }
        else if (rr_event == EV_PANIC_RESET_CNF)
        {
      ...
      case RR_RESET:
      {
        if ((rr_event == EV_INPUT_MESSAGE) && (new_message_ptr != NULL))
        {
          ...
          {
            new_rr_event = rr_mode_control(EV_INPUT_MESSAGE, new_message_ptr);
          }
          else
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
          ...
          {
            new_rr_event = rr_mode_control(EV_GSM_MODE_DEACTIVATED, NULL);
          }
          else
          if ( (message_set == MS_MAC_RR) && (message_id == (byte) MAC_GRR_RESET_CNF) )
          {
            ...
            if (rr_verify_panic_reset_saved_message_set(MS_RRC_RR) ||
                rr_verify_panic_reset_saved_message_set(MS_TDSRRC_RR) ||
                rr_verify_panic_reset_saved_message_set(MS_MSGR_RR) ||
                (rr_get_panic_reset_x2g_cco_flag() != RR_X2G_RESEL_TYPE_UNDEFINED))
            {
              MSG_GERAN_HIGH_0_G("Panic reset during X2G, restarting GL1 to stop it for RAT change");
              (void) rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
              new_rr_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
            }
            else
#endif /* FEATURE_WCDMA */
        ...
        {
          if (rr_send_grr_mac_reset_req() == FALSE)
          {
            rr_control_data_ptr->rr_state = RR_GOING_INACTIVE;
            new_rr_event = EV_GSM_MODE_DEACTIVATED;
          }
      ...
      */
      case RR_X2G_RESEL:
      {
        switch (rr_event)
        {
                ...
                switch (new_message_ptr->rr.header.rr_message_set)
                {
                  case RR_IMSG:
                  {
                    switch (new_message_ptr->message_header.message_id)
                    {
                      ...
                      default:
                      {
                        rr_x2g_resel_control(new_message_ptr);
                      }
                    }
                    break;
                  }
                      ...
                      case RR_X2G_RESEL_OMSG_RESEL_CNF:
                      {
                        ...
                        if (resel_status == RR_X2G_RESEL_STATUS_SUCCESS)
                        {
                          if (rr_x2g_cco_in_progress())
                          {
                            ...
                            rr_set_panic_reset_x2g_cco_flag(rr_control_data_ptr->x2g_resel_type);
                            ...
                            if (rr_dedicated_priority_info_valid(&rr_control_data_ptr->dedicated_priority_info))
                            {
                              ...
                              rr_PLMN_request_details_T * rr_PLMN_request_details_ptr =
                                rr_get_PLMN_request_details_ptr();
              ...
              case MS_TDSRRC_RR:
              {
                switch (new_message_ptr->message_header.message_id)
                {
                  case TDSRR_INTERRAT_RESELECTION_ABORT_REQ:
                  {
                    ...
                    rr_x2g_resel_send_imsg_abort_req(RR_L1_RAT_CHANGE);
              ...
              case MS_MM_RR:
              {
                switch (new_message_ptr->message_header.message_id)
                {
                  ...
                  case RR_PLMN_SELECT_REQ:
                  {
                    ...
                    rr_x2g_resel_send_imsg_abort_req(rr_l1_gsm_stop_reason_get());
                    rr_control_data_ptr->rr_state = RR_X2G_RESEL_ABORTED;
          ...
          {
            rr_x2g_resel_send_imsg(RR_X2G_RESEL_IMSG_L1_ACTIVATED_IND);
            break;
          } /* EV_GSM_MODE_ACTIVATED */

          case EV_GSM_MODE_DEACTIVATED:
          {
            rr_x2g_resel_send_imsg(RR_X2G_RESEL_IMSG_L1_DEACTIVATED_IND);
            break;
          } /* EV_GSM_MODE_DEACTIVATED */

          case EV_PANIC_RESET:
          {
            rr_x2g_resel_send_imsg(RR_X2G_RESEL_IMSG_PANIC_RESET_REQ);
      ...
      */
                        ...
                        switch (rr_control_data_ptr->x2g_resel_abort_cause)
                        {
                          ...
                          case  RR_X2G_RESEL_ABORT_CAUSE_CCO_ABORT:
                          {
                            ...
                            if (new_message_ptr->rr.x2g_resel.omsg.abort_cnf.activation_ind_sent)
                            {
                               ...
                               rr_send_interrat_cc_order_cnf(CCO_FAILURE_AFTER_ACT_IND, RR_GSM_CCO_OTHER_REASON);
                            }
                            else
                            {
                               MSG_HIGH("CCO ABORT CNF before act ind is sent ", 0, 0, 0);  
                               rr_send_interrat_cc_order_cnf(CCO_FAILURE_BEFORE_ACT_IND, RR_GSM_CCO_OTHER_REASON);                         
                            }
                            rr_control_data_ptr->rr_state = RR_INACTIVE;
#endif /* FEATURE_WCDMA */
                            break; 
      ...
      case RR_G2X_REDIR:
      {
        switch (rr_event)
        {
                ...
                switch (new_message_ptr->rr.header.rr_message_set)
                {
                  case RR_IMSG:
                  {
                    switch (new_message_ptr->message_header.message_id)
                    {
                      ...
                      default:
                      {
                        rr_g2x_redir_control(new_message_ptr);
                      }
                    }
                    break;
                  }
                        ...
                        switch (new_message_ptr->rr.g2x_redir.omsg.redir_cnf.status)
                        {
                          ...
                          case RR_G2X_REDIR_STATUS_FAILURE:
                          {
                            switch (new_message_ptr->rr.g2x_redir.omsg.redir_cnf.failure_action)
                            {
                              ...
                              {
                                rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
                                rr_control_data_ptr->rr_state = RR_GOING_ACTIVE;
                                break;
                ...
                switch (new_message_ptr->message_header.message_id)
                {
                  ...
                  case RR_EST_REQ:
                  {
                    switch (new_message_ptr->rr_est_req.establishment_cause)
                    {
                      ...
                      {
                        rr_send_abort_ind(RR_ABORT_GTOW_REDIRECTION_IN_PROGRESS, MM_RR_OTHER_FAILURE);
                        break;
                      } /* MM_RR_LOCATION_UPDATE */

                      default:
                      {
                        set_rr_deact_reason(RR_DEACT_REASON_STOP_CAMPING_THEN_RESTART);
                        (void) rr_save_msg(new_message_ptr);
                        rr_g2x_redir_send_imsg(RR_G2X_REDIR_IMSG_ABORT_REQ);
                      }
                    }
                    break;
                  } /* RR_EST_REQ */
          ...
          case EV_GSM_MODE_DEACTIVATED:
          {
            rr_g2x_redir_send_imsg(RR_G2X_REDIR_IMSG_L1_DEACTIVATED_IND);
            break;
          } /* EV_GSM_MODE_DEACTIVATED */
      ...
      */
      ...
      case RR_X2G_CGI:
      {
        switch (rr_event)
        {
                ...
                switch (new_message_ptr->rr.header.rr_message_set)
                {
                  case RR_IMSG:
                  {
                    switch (message_id)
                    {
                      ...
                      default:
                      {
                        rr_x2g_cgi_control(new_message_ptr);
                      }
                    }
                    break;
                  }
              ...
              case MS_MSGR_RR:
              {
                switch (new_message_ptr->rr_msgr_msg.message.hdr.id)
                {
                  case GERAN_GRR_LTE_ABORT_CGI_REQ:
                  {
                    ...
                    rr_x2g_cgi_send_imsg(RR_X2G_CGI_IMSG_ABORT_REQ);
                    break;
                  } /* GERAN_GRR_LTE_ABORT_CGI_REQ */
          ...
          case EV_GSM_MODE_ACTIVATED:
          {
            rr_x2g_cgi_send_imsg(RR_X2G_CGI_IMSG_L1_ACTIVATED_IND);
            break;
          } /* EV_GSM_MODE_ACTIVATED */

          case EV_GSM_MODE_DEACTIVATED:
          {
            rr_x2g_cgi_send_imsg(RR_X2G_CGI_IMSG_L1_DEACTIVATED_IND);
            break;
          } /* EV_GSM_MODE_DEACTIVATED */
          ...
          case EV_PANIC_RESET:
          {
            ...
            rr_timer_cancel(RR_X2G_CGI_TIMER);
            rr_x2g_cgi_deinit();
            break;
          } /* EV_PANIC_RESET */
                      ...
                      case RR_X2G_RESEL_TYPE_L2G_CELL_CHANGE_ORDER:
                      {
                        ...
                        switch (new_message_ptr->rr_stop_gsm_mode_req.rr_mm_gsm_stop_reason)
                        {
                          ...
                          default:
                          {
                            ...
                            result = rr_msgr_send(&hdr, sizeof(hdr));
                            if (result != E_SUCCESS)
                            {
                              MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
                            }
      ...
      */
             ...
             switch (new_message_ptr->message_header.message_set)
             {
               case MS_RR_RR:
               {
                 switch (new_message_ptr->rr.header.rr_message_set)
                 {
                   case RR_IMSG:
                   {
                     switch (new_message_ptr->rr.header.imh.message_id)
                     {
                       ...
                       case RR_IMSG_PS_ACCESS_REQ:
                       {
                         ...
                         // Call the call-back function (if one was supplied)
                         CALL_RR_CB_FN(imsg_ptr->confirm_cb_fn);
                         break;
                       }
                   ...
                   case RR_L1_IDLE_OMSG:
                   {
                     rr_select_bcch_control(new_message_ptr);
                     break;
               ...
               case MS_MM_RR:
               {
                 switch (new_message_ptr->mm.header.message_id)
                 {
                   case RR_EST_REQ:
                   {
                     MSG_GERAN_LOW_0_G("Save RR_EST_REQ");
                   ...
                   case RR_PLMN_LIST_REQ:
                   {
                     ...
                     {
                       MSG_GERAN_LOW_0_G("Save RR_PLMN_LIST_REQ (Manual)");
                   ...
                   case RR_PLMN_SELECT_REQ:
                   {
                     MSG_GERAN_LOW_0_G("Save RR_PLMN_SELECT_REQ");
                   ...
                   case RR_DEACT_REQ:
                   {
                     MSG_GERAN_LOW_0_G("Save RR_STOP_GSM_MODE_REQ/RR_DEACT_REQ");
                     (void)rr_save_msg(new_message_ptr);
      ...
      case RR_DMM_RESUMING:
      {
        switch (rr_event)
        {
            ...
            switch (new_message_ptr->message_header.message_set)
            {
              ...
              case MS_RR_L1:
              {
                 switch (message_id)
                 {
                   case MPH_SELECT_SPECIFIC_BCCH_CNF:
                   {
                     mph_select_specific_bcch_cnf_T *message_ptr = (mph_select_specific_bcch_cnf_T *)new_message_ptr;
                     rr_scell_backup_t              *rr_scell_backup_ptr = rr_static_get_scell_backup_ptr();
                     inter_task_BCCH_allocation_T   *ba_gprs_list = rr_gprs_get_ba_gprs();

                     MSG_GERAN_MED_1_G("DMMSUSREM:: rcvd SELECT_SPECIFIC_BCCH_CNF [%d], restore GRR context", message_ptr->BCCH_found);
                     ...
                     if (message_ptr->BCCH_found)
                     {
                       ...
                       if (rr_scell_backup_ptr->bsic.PLMN_colour_code != PLMN_colour_code ||
                           rr_scell_backup_ptr->bsic.BS_colour_code != BS_colour_code)
                       {
                         MSG_GERAN_MED_0_G("DMMSUSREM:: BSIC mismatch, move to resel");
                         /* failed to camp on previous scell, try doing reselection on saved ncells */
                         new_rr_event = EV_FORCE_CELL_RESELECTION;
                         rr_control_data_ptr->rr_state = RR_GRR;
                       }
                       else
                       {
                         /* Specific channel selected, retrieve all SIs from cache and move to CELL SELECTION */
                         if (rr_scell_backup_ptr->scell_csi_count > 0)
                         {
                           rr_l1_freq_power_T        meas_info;
                           gprs_scell_info_t*        pcell_info_ptr = rr_gprs_get_pcell_info();
                           rr_candidate_cell_db_T*   candidate_db_ptr = rr_get_candidate_cell_db_ptr();

                           MSG_GERAN_MED_0_G("DMMSUSREM:: BSIC matched, Send stored SIs");

                           if (FALSE != rr_csi_retrieve_all_SIs(rr_scell_backup_ptr->scell_csi[0].si_arfcn,
                                          &(rr_scell_backup_ptr->scell_csi[0].cgi.lai),
                                          &(rr_scell_backup_ptr->scell_csi[0].cgi.ci),
                                          CSI_RETRIEVE_ALL_SIS))
                           {
                             MSG_GERAN_HIGH_0_G("DMMSUSREM:: SIs retrieved after RR_CELL_ACQ_OMSG_RESELECTED_IND");
                             pcell_info_ptr->gsm.BCCH_ARFCN = rr_scell_backup_ptr->scell_csi[0].si_arfcn;
                             pcell_info_ptr->gsm.BSIC = rr_scell_backup_ptr->bsic;
                             pcell_info_ptr->gsm.reselection_info.RXLEV_average = rr_scell_backup_ptr->RXLEV_average;
  
                             pcell_info_ptr->gsm.valid_data_flag = FALSE;
                             pcell_info_ptr->gsm.generate_si_received_event = TRUE;
                             pcell_info_ptr->gsm.update_message_received = 0;
  
                             meas_info.arfcn_num = pcell_info_ptr->gsm.BCCH_ARFCN.num;
                             meas_info.arfcn_band = pcell_info_ptr->gsm.BCCH_ARFCN.band;
                             meas_info.RXLEV_average = rr_scell_backup_ptr->RXLEV_average;
  
                             rr_candidate_db_add_frequency(candidate_db_ptr, &meas_info);
  
                             MSG_GERAN_HIGH_0_G("Starting GSMSysInfo timer");
                             (void)rr_timer_start(RR_GSM_SYS_INFO_TIMER, RR_GSM_SYS_INFO_TIMEOUT);

                             new_rr_event = rr_selection_control(RR_EV_DMM_SPECIFIC_CHANNEL_SEARCH, NULL);
                             rr_control_data_ptr->rr_state = RR_CELL_SELECTION;
                           }
                           else
                           {
                             MSG_GERAN_MED_0_G("DMMSUSREM:: SI retrieval error, move to resel");
                             new_rr_event = EV_FORCE_CELL_RESELECTION;
                             rr_control_data_ptr->rr_state = RR_GRR;
                           }
                         }
                         ...
                         {
                           MSG_GERAN_MED_0_G("DMMSUSREM:: CSI backup not available, move to resel");
                           new_rr_event = EV_FORCE_CELL_RESELECTION;
                           rr_control_data_ptr->rr_state = RR_GRR;
                         }
                       }
                     ...
                     {
                       MSG_GERAN_MED_0_G("DMMSUSREM:: No BCCH found, move to resel");
                       new_rr_event = EV_FORCE_CELL_RESELECTION;
                       rr_control_data_ptr->rr_state = RR_GRR;
                     }

                     /* Restore saved ncells to do resel, trigerred here or if cell selection above fails */
                     if (rr_scell_backup_ptr->BA_gprs.no_of_entries > 0)
                     {
                       uint8 i = 0;
                       gprs_ncells_info_t* ncells_info = rr_gprs_get_ncells_info();

                       MSG_GERAN_MED_0_G("DMMSUSREM:: restore available ncell data");
                       /* Store the BA_GPRS list info ncell info to be detected during resel */
                       ncells_info->gsm_neighbour_cell_list.num_of_ncells = rr_scell_backup_ptr->BA_gprs.no_of_entries;
                       for (i= 0; i < ncells_info->gsm_neighbour_cell_list.num_of_ncells; i++)
                       {
                         ncells_info->gsm_neighbour_cell_list.ncells[i].arfcn = rr_scell_backup_ptr->BA_gprs.BCCH_ARFCN[i];
                         ncells_info->gsm_neighbour_cell_list.ncells[i].status = NCELL_ORIGINAL;
                       }
                     }
                   ...
                   default:
                     LOG_UNEXPECTED_MESSAGE(new_message_ptr, "RR_DMM_RESUMING");
                     break;
                 }
                 break;
              }
          ...
          {
            rr_scell_backup_t* rr_scell_backup_ptr = rr_static_get_scell_backup_ptr();
            MSG_GERAN_MED_0_G("DMMSUSREM:: GSM_MODE_ACTIVATED");

            if (rr_scell_backup_ptr && rr_scell_backup_ptr->scell_csi_count > 0)
            {
              /* Restore any saved system information to SI cache if available */
              MSG_GERAN_MED_0_G("DMMSUSREM:: restore available CSI data");

              rr_csi_restore_from_backup(
                rr_scell_backup_ptr->scell_csi,
                rr_scell_backup_ptr->scell_csi_count);

              rr_send_mph_select_specific_channel(
                rr_scell_backup_ptr->scell_csi[0].si_arfcn,
                rr_scell_backup_ptr->bsic);

              /*rr_resel_control(EV_INPUT_MESSAGE, new_message_ptr);*/
            }
            else
            {
              /* Dont expect code to come here */
              /* No CSI data saved, try cell selection. If ncell data was available we would've already tried resel */
              MSG_GERAN_MED_0_G("DMMSUSREM:: no CSI data available, move to cell_selection");
              rr_static_resume_completed();
              rr_control_data_ptr->rr_state = RR_GOING_ACTIVE;
              new_rr_event = EV_GSM_MODE_ACTIVATED;
            }
            break;
          }
    ...
    if ( rr_event == EV_PANIC_RESET )
    {
      ...
      {
        set_rr_deact_reason(RR_DEACT_REASON_LOWER_LAYER_FAILURE);
      }

      if (rr_control_data_ptr->rr_panic_reset_info.l1_reset)
      {
        rr_event = rr_mode_control(EV_GSM_MODE_DEACTIVATED, NULL);
        rr_control_data_ptr->rr_panic_reset_info.l1_reset = FALSE;
      }
      else
      {
        rr_event = rr_mode_control(EV_PANIC_RESET, NULL);
      }

      rr_control_recovery_init();
    ...
    if (rr_control_data_ptr->rr_state != rr_control_data_ptr->old_rr_state)
    {
      ...
      if (rr_control_data_ptr->rr_state == RR_INACTIVE)
      {
        if (rr_mode_get_l1_stop_reason() == RR_L1_RAT_CHANGE)
        {
#ifdef FEATURE_SGLTE
          if(rr_sglte_saved_l2g_ho_being_processed())
          {
...
const char *rr_main_current_state_name( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  ...
  if (rr_control_data_ptr->rr_state == RR_GRR)
  {
    return grr_control_current_state_name();
  }
  else
  {
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("p_nw_sel_mode=NULL");
    return;
  }
...
boolean rr_can_process_resel_abort_req( )
{
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      process = grr_can_process_rr_resel_abort_req();
    }
    break;
...
#if defined(FEATURE_LTE) && defined(FEATURE_GERAN_CS_SUPPORT)
boolean rr_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled( )
{
  if (rr_is_lte_enabled())
  {
    return rr_nv_get_g2l_blind_redir_after_voice_ss_or_periodic_lau_enabled();
  }
  else
  {
    return FALSE;
...
boolean rr_g2l_blind_redir_after_csfb_enabled( )
{
  if (rr_is_lte_enabled())
  {
    return rr_nv_get_g2l_blind_redir_after_csfb_enabled();
  }
  else
  {
    return FALSE;
...
boolean rr_g2l_blind_redir_after_srvcc_enabled( )
{
  if (rr_is_lte_enabled())
  {
    return rr_nv_get_g2l_blind_redir_after_srvcc_enabled();
  }
  else
  {
    return FALSE;
 ...
 */
boolean rr_can_process_saved_msg_ind( void )
{
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_rr_event_ind( void )
{
  boolean process = TRUE;
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_mm_message( void )
{
  boolean process = TRUE;
  ...
  if (
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
       rr_static_get_suspend_in_progress() == TRUE ||
#endif
      (rr_control_data_ptr->rr_saved_msg.valid &&
  ...
  else
  {
    switch (rr_control_data_ptr->rr_state)
    {
      ...
      case RR_GRR:
      {
        process = grr_can_process_mm_message();
        break;
      }
      ...
      case RR_CONNECTION_PENDING:
      {
        process = rr_ce_can_process_mm_message();
        break;
      } /* RR_CONNECTION_PENDING */
#endif /* FEATURE_GERAN_CS_SUPPORT */
  ...
  switch (timer_id)
  {
    ...
    default:
    {
      switch (rr_control_data_ptr->rr_state)
      {
        case RR_GRR:
        {
          process = grr_can_process_timer_expiry();
          break;
        }
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_serving_meas_ind( void )
{
  boolean process = TRUE;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      process = grr_can_process_serving_meas_ind();
      break;
    }
...
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
boolean rr_can_process_proc_ind_with_suspend(void)
{
  boolean process = TRUE;
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      process = grr_can_process_proc_ind_with_suspend();
      break;
    }
    default: ;
  }
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_surround_meas_ind( void )
{
  boolean process = TRUE;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      process = grr_can_process_surround_meas_ind();
      break;
    }
...
#if defined(FEATURE_LTE) && defined(FEATURE_GERAN_CS_SUPPORT)
boolean rr_can_process_csfb_blind_redir_to_lte_req( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_can_process_change_mode_ind( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      ...
      states so RR does not get one of these signals 'stuck' in it's queue if
      it missed somehow in other states. */
      return grr_can_process_change_mode_ind();
    } /* RR_GRR */
...
#ifdef FEATURE_GERAN_CS_SUPPORT
boolean rr_can_process_rr_abort_req( )
{
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_EARLY_CAMPED_WAIT_FOR_SI:
    {
        ...
        (rr_control_data_ptr->rr_saved_msg.msg.message_header.message_set == MS_MM_RR))
      {
         ...
         process_message = rr_saved_msg_is_rr_est_req();
      }        
      break;
    }
    ...
    case RR_GRR:
    {
      ...
      if ( process_message) 
      {
        process_message = grr_can_process_rr_abort_req();
      }
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_msc_change_ind( void )
{
  boolean process = TRUE;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_DATA_TRANSFER:
    {
      process = rr_dtm_can_process_msc_change_ind();
      break;
    }
#endif
...
boolean rr_can_process_stop_gsm_mode_req_sglte_rat_change( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  boolean            process;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_GRR:
    {
      process = grr_can_process_stop_gsm_mode_req_sglte_rat_change();
      break;
    } /* RR_GRR */
...
boolean rr_can_process_sglte_x2g_req( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  boolean            process;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_GRR:
    {
      process = grr_can_process_sglte_x2g_req();
      break;
    } /* RR_GRR */
...
boolean rr_can_process_sglte_l2g_ho_req( )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  boolean process= TRUE;
...
boolean rr_can_process_sglte_band_coex_omsg_supported_bands_updated_ind( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  boolean            process;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_GRR:
    {
      process = grr_can_process_sglte_band_coex_omsg_supported_bands_updated_ind();
      break;
    } /* RR_GRR */
...
#ifdef FEATURE_LTE
static boolean rr_is_fast_si_acq_during_x2g_allowed()
{
  rr_PLMN_request_details_T *rr_PLMN_request_details_ptr = rr_get_PLMN_request_details_ptr();

  if (rr_nv_x2g_fast_si_acq_feature_is_enabled() &&
      rr_nv_x2g_fast_si_acq_is_plmn_match(rr_PLMN_request_details_ptr->requested_PLMN_id))
  {
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_no_ps_access_req( )
{
  boolean process = TRUE;

#ifdef FEATURE_GERAN_CS_SUPPORT
  switch (rr_control_get_data_ptr()->rr_state)
  {
#ifdef FEATURE_GSM_DTM
    case RR_DATA_TRANSFER:
    {
      process = rr_dtm_can_process_no_ps_access_req();
      break;
    }
#endif
 ...
 * @return boolean TRUE if the message can be processed
 */
boolean rr_can_process_ps_access_req( )
{
  boolean process = TRUE;
  ...
  switch (rr_control_get_data_ptr()->rr_state)
  {
    ...
    case RR_DATA_TRANSFER:
    {
      process = rr_dtm_can_process_ps_access_req();
      break;
    }
#endif
...
boolean rr_get_ue_oos_flag( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
void rr_set_ue_oos_flag(boolean ue_oos )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
#ifdef FEATURE_LTE
boolean rr_is_csfb_call_in_progress( )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
)
{
  ...
  boolean anite_gcf_flag = rr_get_nv_anite_gcf_flag();
  ...
  MSG_GERAN_HIGH_2_G("Fast SI Acq : anite_gcf_flag=%d and service domain = %d",(int)anite_gcf_flag, rr_sel_get_service_domain() );

  #ifdef FEATURE_GSM_DTM
  nv_dtm_enabled = rr_get_nv_dtm_enabled();
#else
...
#ifdef FEATURE_LTE
  MSG_GERAN_HIGH_3_G(
    "is_csfb_call_in_progress=%d, nv_dtm_enabled=%d, nv_fast_si_acq_during_csfb=%d",
    rr_is_csfb_call_in_progress(),
    nv_dtm_enabled,
    rr_get_nv_fast_si_acq_during_csfb_enabled()
  );
#endif
  ...
  MSG_GERAN_HIGH_2_G(
    ...
    rr_t2g_resel_in_progress(),
    rr_t2g_cco_in_progress()
  );
#endif
  ...
  if (rr_x2g_redir_with_si_in_progress())
  {
    ...
    {
     if (rr_get_nv_fast_si_acq_during_csfb_enabled() && !anite_gcf_flag)
     {
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
  if (rr_x2g_cco_with_nacc_in_progress())
  {
    return TRUE;
  }
  else
...
#ifdef FEATURE_GSM_TDS
  if ((rr_t2g_resel_in_progress() || rr_t2g_cco_in_progress() || rr_t2g_redirection_in_progress() ) && !nv_dtm_enabled && !anite_gcf_flag)
  {
...
#ifdef FEATURE_LTE
	if ( (rr_l2g_resel_in_progress() || rr_l2g_cco_in_progress()|| rr_l2g_redirection_in_progress()) && 
		 (
#ifdef FEATURE_GSM_DTM
		  (!rr_get_nv_dtm_enabled()) && 
#endif /* FEATURE_GSM_DTM */
		  (!rr_get_nv_anite_gcf_flag()) && (rr_is_fast_si_acq_during_x2g_allowed())) )
	{
  ...
  else if (rr_is_csfb_call_in_progress())
  {
    if (rr_get_nv_fast_si_acq_during_csfb_enabled() && !anite_gcf_flag)
    {
      /* For CSFB calls, we do not wait for SI13. */
...
#endif /*FEATURE_LTE*/
  if ((SYS_SRV_DOMAIN_CS_ONLY == rr_sel_get_service_domain()) &&
      rr_nv_fast_si_acq_when_cs_only_is_enabled())
  {
...
{
  if (rr_nv_c1_relaxation_is_plmn_match(plmn))
  {
    return TRUE;
  }
  else
...
boolean rr_g2w_measurements_enabled_in_nv( void )
{
  return rr_nv_get_g2w_measurements_enabled();
}

boolean rr_g2t_measurements_enabled_in_nv(void)
{
  return rr_nv_get_g2t_measurements_enabled();
}

boolean rr_g2l_measurements_enabled_in_nv( void )
{
  return rr_nv_get_g2l_measurements_enabled();
}
#endif
#ifdef FEATURE_GSM_TDS
/**
...
*/
boolean rr_t2g_resel_in_progress( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
*/
boolean rr_t2g_cco_in_progress( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
*/
boolean rr_l2g_resel_in_progress( void )
{
...
*/
boolean rr_l2g_cco_in_progress( void )
{
...
*/
void rr_sglte_x2g_cco_nas_split_cb_fn( void )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
*/
void rr_sglte_x2g_cco_failed_cb_fn( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
*/
void rr_sglte_x2g_cco_aborted_cb_fn( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
*/
void rr_sglte_x2g_redir_aborted_cb_fn( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
*/
void rr_sglte_x2g_resel_aborted_cb_fn( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_sglte_g2x_bplmn_search_active( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_sglte_inhibit_tds_bplmn_search( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
boolean rr_sglte_inhibit_lte_bplmn_search( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
#ifdef FEATURE_SGLTE
static boolean rr_control_can_process_sglte_plmn_search( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    ...
    case RR_GRR:
      search_handled = grr_can_process_sglte_plmn_search();
    break;

#ifdef FEATURE_GERAN_CS_SUPPORT
   case RR_DATA_TRANSFER:
 #ifdef FEATURE_SGLTE_G2X_CONN_MODE_RESEL
      search_handled = rr_sglte_g2x_conn_mode_resel_can_process_plmn_search();
 #else
     search_handled  = TRUE;
 #endif /*FEATURE_SGLTE_G2X_CONN_MODE_RESEL*/
    break;   
 ...
 *         if not.
 */
static boolean rr_sglte_irat_neighbour_inhibit_plmn_list(boolean g2x_resel_in_ded_mode )
{
  ...
  {
    inhibit_plmn_list_search = rr_sglte_g2x_resel_conn_inhibit_plmn_search();
  }
...
static boolean rr_start_sglte_plmn_list_search(rr_plmn_list_req_T * plmn_list_req_ptr )
{
      ...
      plmn_list_req_ptr->service_search == SYS_SERVICE_SEARCH_IRAT_PLMN)
  {
    ...
    {
#ifdef FEATURE_SGLTE_G2X_CONN_MODE_RESEL
      if (rr_get_nv_sglte_g2x_conn_mode_resel_enabled() )
      {
...
#endif /* FEATURE_GERAN_CS_SUPPORT */
    {
      ...
      {
        if (rr_is_lte_in_neighbour_list() == TRUE)
        {
          rr_control_data_ptr->sglte_g2x_plmn_list_search_inhibit_lte_search = rr_sglte_irat_neighbour_inhibit_plmn_list(is_sglte_irat_ded_meas_possible);
        }
      ...
      if (rr_is_rat_in_plmn_search_list(plmn_list_req_ptr,SYS_RAT_TDS_RADIO_ACCESS) == TRUE)
      {
        if ((nw_supp_rats == RR_NW_SUPP_UTRAN_TDD ||
            nw_supp_rats == RR_NW_SUPP_UTRAN_TDD_FDD) && rr_sglte_is_l1_tds_measurements_configured())
        {
          rr_control_data_ptr->sglte_g2x_plmn_list_search_inhibit_tds_search = rr_sglte_irat_neighbour_inhibit_plmn_list(is_sglte_irat_ded_meas_possible);
        }
    ...
    if ( rr_gprs_check_if_any_g2x_resel_timer_is_running(   ) )
    ...
    if (rr_control_can_process_sglte_plmn_search() &&
...
boolean rr_is_lte_in_neighbour_list( void )
{
  gprs_scell_info_t *scell = rr_gprs_get_scell_info();
  boolean rat_is_neighbour = FALSE;

  if (  rr_priority_and_threshold_information_available(rr_get_instances_ptr(SI2Q_INSTANCE)) &&
...
boolean rr_sglte_perform_g2l_blind_redir( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
static void rr_sglte_set_lte_blind_redir_in_data_transfer( void )
{
  // Obtain a pointer to the module data
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
...
/**
  ...
  @return rr_rat_e
*/
rr_rat_e rr_get_x2g_resel_source_rat( void )
{
  rr_control_data_t *rr_control_data_ptr = rr_control_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_control_data_ptr);
...
/**
 * Checks if the rr state machine is in correct state to handle
 * Reduced mobility related message signal.
 *
 * @return - boolean
 *
 */
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
boolean rr_can_process_resel_reduced_mobility_msg( void )
{
  boolean process = FALSE;
  ...
  switch (rr_control_data_ptr->rr_state)
  {
    case RR_GRR:
    {
      process = grr_can_process_rr_resel_reduced_mobility_msg();
    }
    break;
