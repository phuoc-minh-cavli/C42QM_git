...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_resel.c#12 $ */
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
  /*PEO related states*/
  RR_RESEL_STATE_REDUCED_MOBILITY,
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
 ...
 * Local Variables
 *--------------------------------------------------------------------------*/
static rr_resel_data_t rr_resel_data_spaces;
 ...
 */
static rr_resel_data_t *rr_resel_get_data_ptr( void )
{
  return &rr_resel_data_spaces;
}
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
    case RR_RESEL_STATE_REDUCED_MOBILITY:
      return "REDUCED_MOBILITY STATE";
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
...
)
{
  ...
  {
    MSG_GERAN_LOW_2_G("rr_resel_data_ptr (%p) == NULL or dst (%p) == NULL", rr_resel_data_ptr, dst);
    return;
  }
...
)
{
  if (rr_resel_data_ptr)
  {
    ...
    rr_gprs_restore_serving_cell_details(rr_resel_data_ptr, pcell_info_ptr);
    ...
    if (scell_info_ptr)
    {
      ...
      for (index = 0; index < no_of_entries; index++)
      {
        ...
        */
        ...
        if(include_entry && rr_cell_forbidden(ARFCN))
        {
          include_entry = FALSE;
        }
      ...
      {
        /* Do not add forbidden cells to Reselection Power Scan list */
        if(FALSE == rr_cell_forbidden(ARFCN))       
        {
...
)
{
  gprs_scell_info_t             *scell_info        = rr_gprs_get_scell_info();
   rr_available_plmn_db_T        *available_plmn_db = rr_get_available_plmn_db_ptr();
  rr_candidate_cell_db_T        *candidate_cell_db = rr_get_candidate_cell_db_ptr();
  gprs_better_cell_list_T       *better_cell_list_ptr = rr_get_better_cell_list_ptr();

  // Obtain a pointer to the module data
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_resel_data_ptr);
  ...
  if (!rr_resel_data_ptr->rps_data.pscan_cell_list.valid_data)
  {
    ...
    {
      rr_reselection_build_pscan_list_from_cell_select_indicator(rr_resel_data_ptr);
    }
    else
    {
      rr_reselection_build_power_scan_cell_list(rr_resel_data_ptr);
    }
  }

  /* invalidate the available PLMN database and candidate cell database */
  rr_plmn_db_clear(available_plmn_db);
...
)
{
  ...
  {
    MSG_GERAN_LOW_2_G("Overflow detected attempting to add ARFCN (%d,%d) to RPS cell list",
              arfcn.num,
              arfcn.band);
  }
...
)
{
  ...
  {
    MSG_GERAN_LOW_3_G("L1 res-> arfcn:%d band:%d rxlev:%d",
            rr_pscan_results_db_ptr->pscan_results.meas[i].arfcn_num,
            rr_pscan_results_db_ptr->pscan_results.meas[i].arfcn_band,
...
)
{
  ...
  rr_plmn_db_clear(rr_get_available_plmn_db_ptr());
  ...
  (void)rr_timer_cancel(RR_INVALIDATE_AVAILABLE_PLMN_DB_TIMER);
  ...
  rr_pscan_send_start_ind(RR_SEARCH_RESELECTION, rr_band_pref());
  ...
  if( num_frequencies_found != 0 )
  {
    ...
    */
         ...
         ++index)
    {
      ...
      bcch_ncell_info_t *new_ncell;

      channel_number.num = results_buffer[index].arfcn_num;
      channel_number.band = results_buffer[index].arfcn_band;
      /* See if the entry exists already. It should always be there on bcch */
      new_ncell = rr_gprs_get_surround_ncell_sys_info(channel_number);
      ...
      if ( new_ncell == NULL )
      {
        ...
        rr_gprs_add_ncell_to_ba_gprs(channel_number, NULL, NULL);
      ...
      */
      ...
      if (new_ncell != NULL)
      {
        rr_gprs_add_to_best_six(channel_number, new_ncell->gsm.BSIC, FALSE);
        ...
        if ( new_ncell->gsm.valid_data_flag == TRUE )
        {
          ...
          {
            MSG_GERAN_LOW_0_G("rr_candidate_db_get_entry() returned NULL");
          }
          else
          {
...
)
{
  ...
  if (resel_type == RESEL_COMPLETE_C1)
  {
    ...
    if (rr_resel_data_ptr->cause != RR_RESEL_CAUSE_RESELECTION_TRM_DENIAL)
    {
      if (!ARFCNS_EQUAL(new_arfcn, old_arfcn) && ARFCN_VALID_CHANNEL(old_arfcn))
      {
        ...
        rr_add_forbidden_cell(old_arfcn);
        /*Start the forbidden list timer*/
        rr_start_forbidden_timer();
      }
    }
  ...
  */
  ...
  if (rr_gprs_parameters_available())
  {
    (void)rr_timer_start(PATH_LOSS_TIMER, GPRS_PATH_LOSS_TIMEOUT);
    MSG_GERAN_LOW_0_G("Start the PATH LOSS TIMER");
  }

  /* If the recent reselection timer is still running cancel it */
  (void)rr_timer_cancel(RESELECTION_HYSTERESIS);

  /* If the surround measurement timer is still running cancel it */
  (void)rr_timer_cancel(GRR_SURROUND_MEAS_TIMER);

  /* now restart the recent reselection timer again from the start */
  (void)rr_timer_start(RESELECTION_HYSTERESIS, RESELECTION_HYSTERESIS_TIMEOUT);
...
)
{
  ...
  /**************************************************************/
  if (rr_gprs_parameters_available() == FALSE)
  {
...
#ifdef FEATURE_IDLE_DRX_SCALING
  if (rr_idle_drx_is_nmeas_off() && (rr_macinfo_get_mac_status() == MAC_IDLE))
  {
    measurement_expected = RR_MASK_GSM_SERV_MEAS;
  }
  else
#endif /* FEATURE_IDLE_DRX_SCALING */
  if (rr_gsm_only() || rr_macinfo_get_mac_status() == MAC_TRANSFER)
  {
...
)
{
  ...
  rr_gprs_cache_current_LAI();
}


boolean resel_type_C2_based( )
{
   rr_resel_data_t   *rr_resel_data_ptr = rr_resel_get_data_ptr();
...
)
{
  ...
  {
    MSG_GERAN_LOW_2("NULL pointer (%p, %p)", ptr1, ptr2);
    return 0;
  }
}
...
)
{
  if (ptr1 != NULL && ptr2 != NULL)
  {
    ...
    uint8 cell1_priority_class = grr_get_ncell_priority_class((*cell1)->reselection_data.BCCH_ARFCN);
    uint8 cell2_priority_class = grr_get_ncell_priority_class((*cell2)->reselection_data.BCCH_ARFCN);
...
)
{
  ...
  {
    MSG_GERAN_LOW_2_G("NULL pointer (%p, %p)", ptr1, ptr2);
    return 0;
  }
}
...
)
{
  ...
  gprs_scell_info_t             *scell_info = rr_gprs_get_scell_info();
  gprs_reselection_structure_T  *gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();
  ...
  if (create_gprs_reselection_list)
  {
    if (rr_gprs_parameters_available())
    {
      ...
      (void) rr_gprs_build_candidate_cell_list_GPRS();
    }
    else
    {
      /* Place into the better cell list C2 values with hysteresis. */
      (void) rr_gprs_build_candidate_cell_list_GSM();
    }
  }
  ...
  rr_resel_data_ptr->rr_gprs_target_cell = rr_gprs_get_best_cell(TRUE, check_forbidden_cells);
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
     rr_update_c1_delta(RR_C1_DELTA_AFTER_SCELL_NCELL_MEAS);
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
...
)
{
  ...
  if (rr_gprs_nacc_is_allowed(NACC_GSM_GSM) == FALSE)
  {
  ...
  if (rr_in_dedicated())
  {
    ...
    MSG_GERAN_HIGH_0_G("NACC prohibited during dedicated mode");
    return(FALSE);
  }
...
)
{
  ...
  rr_resel_data_ptr->rr_gprs_target_cell = rr_gprs_get_best_cell(TRUE, FALSE);
...
)
{
  ...
  MSG_GERAN_LOW_0_G("Invalidating RPS Data");
...
===========================================================================*/
static void rr_resel_handle_no_service( )
{
  gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
  ...
  if (scell_info_ptr->gsm.camped_on_flag == FALSE)
  {
    ...
    bcch_ncell = rr_gprs_get_surround_ncell_sys_info(pcell_info_ptr->gsm.BCCH_ARFCN);
 ...
 *
 ****************************************************************************/
static void grr_start_nacc_reselection_process( void)
{
  rr_resel_data_t               *rr_resel_data_ptr = rr_resel_get_data_ptr();
  gprs_scell_info_t             *scell_info_ptr = rr_gprs_get_scell_info();
  ...
  /* initialise 2G NACC */
  rr_gprs_nacc_initialise(NACC_GSM_GSM);
  ...
  (void)rr_timer_start(T3208, T3206_TIMEOUT);

  /* send the PCCN message to MAC */
  rr_send_grr_mac_send_pccn_req(&rr_resel_data_ptr->pccn);
}
...
)
{
  ...
  if (rr_gprs_resel_suitable_cell(surround_db_entry_ptr))
  {
    if (rr_gprs_parameters_available() == FALSE)
    {
      /***********************************************************************/
      ...
      /* criteria                                         */
      /****************************************************/
      if (C1 >= 0 || rr_allow_negative_c1(surround_db_entry_ptr->gsm.location_area_identification.plmn_id))
      {
        /*calculate C2 for the target cell*/
        C2 = rr_gprs_compute_surrounding_C2(C1, resel_candidate);
        ...
        surrounding_better_than_serving = rr_gprs_compare_serving_and_surrounding_C2(
          ...
          ,resel_candidate->ncell_data->rxlev
    ...
    else
    {
      ...
      /* criteria                                         */
      /****************************************************/
      if (local_fresh_reselection_values.C1 >= 0 || rr_allow_negative_c1(surround_db_entry_ptr->gsm.location_area_identification.plmn_id))
      {
        /********************************************/
...
)
{
  ...
  {
    rr_gprs_nacc_store_trigger_info(container_id);
  }
  ...
  /**************************************************************/
  {
    ...
    if (new_surround_db_entry_ptr)
    {
      new_surround_db_entry_ptr->gsm.BSIC = rr_gprs_get_scell_bsic();
    ...
    MSG_GERAN_LOW_0_G("NACC transition 51 -> 51");
  ...
  rr_init_sys_info_3g();
#else
  pcell_info->gsm.utran_neighbor_list.valid_data = FALSE;
#endif /* FEATURE_WCDMA */
 ...
 *
 ****************************************************************************/
static boolean rr_gprs_is_pncd_enough_to_camp(void)
{
  gprs_scell_info_t *pcell_info = rr_gprs_get_pcell_info();
...
)
{
  ...
  if (pcco->gsm_3g_cell_type == CSN_PCCO_GSM_TARGET_CELL)
  {
    ...
    rr_resel_data_ptr->pcco_data.pccf.tlli = rr_gmminfo_get_tlli();
  }
...
)
{
  ...
  rr_resel_data_ptr->rr_gprs_old_scell.reselection_data.BCCH_ARFCN = rr_gprs_get_scell_arfcn();
}
...
)
{
  ...
  rr_gprs_load_pccf_from_pcco(pcco);
  rr_gprs_store_serving_cell_details();
  rr_gprs_cache_current_LAI();
  ...
  if (pcco->gsm_3g_cell_type == CSN_PCCO_GSM_TARGET_CELL)
  {
    ...
    /****************************************************/
    if (!rr_gapi_is_supported_band_dyn((sys_band_T)GET_ARFCN_BAND(pcco->cell.gsm.arfcn)))
    {
      MSG_GERAN_HIGH_1_G("Band not allowed for arfcn %d - PCCF",pcco->cell.gsm.arfcn.num);
      rr_gprs_send_pccf(FREQ_NOT_IMPLEMENTED);
    }
    else
    {
      ...
      if (!rr_gprs_is_cell_in_sync_list(pcco->cell.gsm.arfcn))
      {
      ...
      else
      {
        ...
        if (pcco->cell.gsm.container_flag == valid)
        {
          ...
          if (rr_gprs_nacc_container_empty(container_id))
          {
            ...
            (void)rr_timer_cancel((rr_timer_id_T)(container_id + GRR_NACC_CONTAINER_TIMER_0));
            rr_send_grr_mac_nacc_release_req(container_id, NACC_RELEASE_ONE);
...
)
{
  ...
  rr_gprs_load_pccf_from_psho(psho);

  if (!rr_gapi_is_supported_band_dyn((sys_band_T)GET_ARFCN_BAND(psho_target->arfcn)))
  {
    MSG_GERAN_HIGH_1_G("Band not allowed for arfcn %d - PCCF",psho_target->arfcn.num);
    rr_gprs_send_pccf(PSHO_FAILURE_REASON_OTHERS);
    return EV_NO_EVENT;
  }

  if (rr_gmminfo_get_gmm_status() != GMM_READY_STATE)
  {
  ...
  if (rr_gprs_nacc_container_empty(psho->container_id))
  {
    MSG_GERAN_ERROR_1_G("PSHO includes container ID (%d), but no data is available",
              (int)psho->container_id);

    /* cancel container timer and discard any stored data in GMAC */
    (void)rr_timer_cancel((rr_timer_id_T)(psho->container_id + GRR_NACC_CONTAINER_TIMER_0));
    rr_send_grr_mac_nacc_release_req(psho->container_id, NACC_RELEASE_ONE);
    rr_gprs_send_pccf(PSHO_FAILURE_REASON_OTHERS);
    return EV_NO_EVENT;
  }

  CONVERT_TO_BSIC_T(bsic, psho_target->bsic);
  rr_prepare_sidb_for_camping(&pcell_info_ptr->gsm, psho_target->arfcn, bsic, 0xFF);
  ...
  (void)rr_gprs_nacc_trigger_ncell_data(psho->container_id, psho_target->arfcn, TRUE);
  ...
  rr_gprs_prepare_for_reselection_with_PNCD(INVALID_CONTAINER_ID);
...
*/
const char *rr_resel_current_state_name(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
 ...
 *
 ****************************************************************************/
static void rr_gprs_set_parameters_available(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
  ...
  if (rr_gprs_pmo_pcco_ncells_are_present() == TRUE ||
...
)
{
  ...
  if ((bcch_ncell_info_ptr != NULL) && (!rr_is_any_cell_reselection_mode()))
  {
...
*/
static boolean rr_resel_to_lower_priority_layers_permitted(void)
{
  gprs_scell_info_t * scell = rr_gprs_get_scell_info();
  ...
  {
    if (rr_resel_greatest_gsm_s_value(TRUE) < (thresh_gsm_low * 2))
    {
      return TRUE;
    }
    else
...
{
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_1);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_2);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_3);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_4);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_5);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_6);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_7);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_8);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_9);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_10);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_11);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_12);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_1);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_2);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_3);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_4);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_5);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_6);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_7);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_8);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_9);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_10);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_11);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_12);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_1);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_2);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_3);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_4);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_5);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_6);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_7);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_8);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_9);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_10);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_11);
  (void)rr_timer_cancel(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_12);
}
...
{
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_1);
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_2);
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_3);
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_4);
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_5);
  (void)rr_timer_cancel(RR_G2WR_FIVE_SEC_TIMER_6);
}
...
void rr_resel_init(void)
{
  ...
  grr_reset_reselection_power_scan();
  ...
  rr_clear_better_cell_list();
  
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION
  rr_resel_rmc_init();
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/

  MSG_GERAN_LOW_0_G("Clearing PCCC/PCCO flags");
 ...
 */
void rr_resel_set_pccf_cause_to_no_response_on_target(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
 ...
 */
boolean rr_pcco_in_progress(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();

  if (rr_timer_is_running(T3174))
  {
    ...
    {
      MSG_GERAN_LOW_0_G("T3174 running and rr_resel_control() state is PCCO / PCCO_ABORTING_CELL_ACQ / PCCO_PENDING_L1_IDLE_CNF, PCCO in progress");
      return TRUE;
    }
    else if (rr_resel_data_ptr->state == RR_RESEL_STATE_IDLE)
    {
      MSG_GERAN_LOW_0_G("T3174 running and rr_resel_control() state is IDLE, PCCO in progress");
      return TRUE;
    }
    else
    {
 ...
 */
boolean rr_failed_pcco_in_progress(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();

  if (rr_timer_is_running(T3176))
  {
...
)
{
  ...
  rr_send_grr_mac_send_pccf(rr_resel_data_ptr->pcco_data.pccf);
 ...
 *
 ****************************************************************************/
boolean rr_gprs_parameters_available(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
 ...
 *
 ****************************************************************************/
void rr_gprs_reset_reselection_list(void)
{
  rr_resel_data_t              *rr_resel_data_ptr = rr_resel_get_data_ptr();
  gprs_reselection_structure_T *gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();
 ...
 *
 ****************************************************************************/
void rr_gprs_reset_nmeas_in_reselection_list(void )
{
  rr_resel_data_t              *rr_resel_data_ptr = rr_resel_get_data_ptr();
  gprs_reselection_structure_T *gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();
...
boolean get_recent_reselection_flag_value(void)
{
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
...
/**
  ...
  @return FALSE if the reselection state machine is not in it's CCN reselection
          state.
*/
boolean rr_resel_is_in_ccn_reselection_state(void)
{
  // Obtain a pointer to the module data
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_resel_data_ptr);
...
boolean rr_gprs_nacc_check_pccn_flag(void)
{
  // Obtain a pointer to the module data
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_resel_data_ptr);
...
===========================================================================*/
boolean rr_resel_can_process_rr_resel_abort_req(void)
{
...
boolean rr_resel_reselection_is_pending(void)
{
  // Obtain a pointer to the module data
  rr_resel_data_t *rr_resel_data_ptr = rr_resel_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_resel_data_ptr);
  ...
  else if (rr_resel_data_ptr->state == RR_RESEL_STATE_C1 && rr_cell_acq_pending_mac_suspension())
  {
    MSG_GERAN_MED_0_G("C1 reselection is pending");
    return TRUE;
  }
  else if (rr_resel_data_ptr->state == RR_RESEL_STATE_NACC && rr_cell_acq_pending_mac_suspension())
  {
  ...
  while (event != EV_NO_EVENT)
  {
    ...
    switch (event)
    {
          ...
          case MS_RR_L1:
          {
            switch ((rr_l1_message_id_enum_T) message_id)
            {
              ...
              case MPH_SELECT_SPECIFIC_BCCH_CNF:
                rr_cell_acq_control(event, message);
                return output_event;
      ...
      case EV_RECENT_RESELECTION_TIMEOUT:
      {
        ...
        event = EV_NO_EVENT;
        rr_resel_data_ptr->recent_reselection_penalty_flag = FALSE;
        rr_cell_acq_control(EV_RECENT_RESELECTION_TIMEOUT, NULL);
        return EV_NO_EVENT;
      }
        ...
        switch (event)
        {
            ...
            switch (message_set)
            {
              case MS_RR_L1:
              {
                switch ((rr_l1_message_id_enum_T) message_id)
                {
                  ...
                  {
                    rr_resel_data_ptr->measurements_expected = rr_cell_resel_set_meas_expected();

                    /* stop and restart the measurement timer */
                    (void)rr_timer_restart(GRR_SURROUND_MEAS_TIMER, GRR_SURROUND_MEAS_TIMEOUT);

                    /* Process the message */
                    rr_gprs_store_surround_meas(&message->mph_surround_meas_ind);
                    ...
                    */
                    if ( (rr_gprs_camped_on_ccch() || rr_gsm_camped_on_ccch() )
                       && ( rr_gprs_parameters_available() == FALSE))
                    {
                  ...
                  case MPH_SERVING_MEAS_IND:
                  {
                    ...
                    if (rr_gprs_parameters_available())
                    {
                      if (!rr_timer_is_running(PATH_LOSS_TIMER))
                      {
                        (void)rr_timer_start( PATH_LOSS_TIMER, GPRS_PATH_LOSS_TIMEOUT);
                        MSG_GERAN_LOW_0_G("Start the PATH LOSS TIMER");
                      }
                  ...
                  case MPH_SERVING_IDLE_MEAS_IND:
                  {
                    ...
                    if (rr_gprs_is_scell_bsic_unchanged(&message->mph_serving_idle_meas_ind))
                    {
                  ...
                  {
                    if (rr_meas_reporting_is_active())
                    {
                      rr_resel_data_ptr->measurements_expected = rr_cell_resel_set_meas_expected();
                      ...
                      rr_gprs_process_nc_meas_ind(&message->mph_nc_measurement_ind);
                  ...
                  case MPH_SURROUND_WCDMA_XFER_MEAS_IND:
                  {
                    ...
                    if (rr_gprs_camped_on_ccch() && (rr_gprs_parameters_available() == FALSE))
                    {
                      /* received the wcdma surrounding measurements */
                      rr_resel_data_ptr->measurements_received |= RR_MASK_WCDMA_SURROUND_MEAS;
                ...
                switch (message->rr.header.rr_message_set)
                {
                  case RR_RESEL_IMSG:
                  {
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      ...
                      case RR_RESEL_IMSG_REQUIRED_IND:
                      {
                        ...
                        switch (message->rr.resel.imsg.required_ind.cause)
                        {
                          ...
                          case RR_RESEL_CAUSE_RESELECTION_TRM_DENIAL:
                          {
                            ...
                            {
#ifdef FEATURE_LTE
                              if (rr_irat_check_for_priority_based_reselection(rr_resel_to_lower_priority_layers_permitted(),TRUE))
                              {
...
#endif /* FEATURE_LTE */
#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS)
                              if (rr_irat_check_for_rank_based_reselection(FALSE, TRUE, TRUE, FALSE, FALSE))
                              {
                          ...
                          default:
                          {
                            MSG_GERAN_LOW_1_G("RR bad resel_cause: 0x%x",message->rr.resel.imsg.required_ind.cause);
                          }
                        }
                        break;
                      ...
                      {
                        rr_resel_algo_e resel_algo = rr_resel_algo();

                        switch (resel_algo)
                        {
                          case RR_RESEL_ALGO_RANK_BASED:
                          {
                            rr_resel_cancel_all_priority_based_resel_timers();
                            break;
                          } /* RR_RESEL_ALGO_RANK_BASED */

                          case RR_RESEL_ALGO_PRIORITY_BASED:
                          {
                            rr_resel_cancel_all_rank_based_resel_timers();
                            break;
                      ...
                      case RR_RESEL_IMSG_G2L_RESEL_ALLOWED_UPDATED_IND:
                      {
                        ...
                        if (!rr_g2l_resel_allowed())
                        {
                          rr_irat_lte_measurements_delete();
                        }
                        break;
                      ...
                      case RR_RESEL_IMSG_START_RPS_IND:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("RR_RESEL_IMSG_START_RPS_IND rxd");
                        ...
                        rr_perform_rps(&rr_resel_data_ptr->rps_data.pscan_cell_list);
                      ...
                      {
                        MSG_GERAN_HIGH_1_G("RR_RESEL_IMSG_REDUCED_MOBILITY_START_IND start type:0x%x rxd",message->rr.resel.imsg.start_reduced_mobility.reason);
                        rr_resel_rmc_send_imsg_start_ind(message->rr.resel.imsg.start_reduced_mobility.reason);
                        rr_resel_data_ptr->state = RR_RESEL_STATE_REDUCED_MOBILITY;
                        break;
                      }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
              ...
              case MS_MAC_RR:
              {
                switch ((mac_grr_sig_id_t) message_id)
                {
                  case MAC_GRR_DATA_IND:
                  {
                    ...
                    switch (mac_grr_data_ind->data_ptr.header_ptr->message_type)
                    {
                      case PACKET_CELL_CHANGE_ORDER:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("PACKET_CELL_CHANGE_ORDER rxd");
                        ...
                        {
                          new_event = rr_gprs_process_PCCO(pcco);
                        ...
                        else
                        {
                          ...
                          if (rr_gmminfo_get_neg_ready_timer_zero())
                          {
                            rr_gprs_send_pccf(FORCED_TO_STANDBY);
                          }
                          else
                          {
                            rr_gprs_send_pccf(MS_IN_GMM_STANDBY);
                          }
                        }
                      }
                      break;
                      ...
                      case PACKET_PS_HANDOVER:
                      {
                        ...
                        if (rr_gprs_reselection_process_psho_command(psho) == EV_ACTION_PSHO)
                        {
                  ...
                  case MAC_GRR_PCCF_SENT:
                  {
                    ...
                    MSG_GERAN_LOW_0_G("MAC_GRR_PCCF_SENT rxd");
                    ...
                    if (rr_timer_is_running(T3176))
                    {
                      ...
                      {
                        MSG_GERAN_LOW_0_G("PCCF successfully sent, cancelling T3176");
                        (void)rr_timer_cancel(T3176);
                      }
                      else
                      {
                        MSG_GERAN_HIGH_0_G("PCCF not successfully sent, retrying");
                        rr_gprs_send_pccf(USE_PRELOADED_CAUSE);
                      }
                    }
#ifdef FEATURE_GPRS_PS_HANDOVER
                    else
                    ...
                    {
                      MSG_GERAN_ERROR_1_G("MAC_GRR_PCCF_SENT(pccf_status=%d) received but T3176 not running",
                        pccf_sent->pccf_status);
                    }
#endif /* FEATURE_GPRS_PS_HANDOVER */
                  }
                  ...
                  case MAC_GRR_CELL_RESEL_STATUS:
                  {
                    ...
                    switch (message->mac_grr_sig.mac_grr_cell_resel_status.status)
                    {
                      ...
                      default:
                      {
                        MSG_GERAN_LOW_1_G("Unknown status %d in MAC_GRR_CELL_RESEL_STATUS, setting PCCF cause to 'no response'",
                    ...
                    if (message->mac_grr_sig.mac_grr_cell_resel_status.status != PCCO_SUCCESS)
                    {
                      ...
                      if (rr_gmminfo_get_gmm_status() == GMM_READY_STATE)
                      {
                        (void)rr_timer_start(T3176, T3176_TIMEOUT);
                        rr_reselect_old_scell_after_pcco_failure(rr_resel_data_ptr);
          ...
          case EV_PATH_LOSS_TIMEOUT:
          {
            (void) rr_log_gprs_cell_reselection_measurements(
              ...
              get_recent_reselection_flag_value()
            );
                ...
                )
            {
              ...
              switch (resel_algo)
              {
                case RR_RESEL_ALGO_RANK_BASED:
                {
                  ...
                  /*****************************************************/
                  if (rr_gprs_check_for_path_loss())
                  {
                    rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_C1);
                  ...
                  else if ( rr_gprs_check_for_bad_scell() )
                  {
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
                      /*Optimisation: 
                ...
                case RR_RESEL_ALGO_PRIORITY_BASED:
                {
                  if (rr_irat_check_for_priority_based_reselection(
                        rr_resel_to_lower_priority_layers_permitted(),
                        FALSE           
                      ))
                  {
                    rr_resel_data_ptr->resel_type = RESEL_G2W_NORMAL;
                  ...
                  else if (rr_gprs_check_for_path_loss())
                  {
                    rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_C1);
                    /*Optimisation: 
          ...
          case EV_CELL_RESELECTION:
          {
            ...
            switch (rr_resel_data_ptr->resel_type)
            {
              ...
              case RESEL_C1_CHOOSE_CELL:
              {
                ...
                c1_resel_status = rr_resel_start_c1(TRUE, TRUE, TRUE);
                ...
                switch (c1_resel_status)
                {
                  case START_C1_RESEL_FAILED:
                    ...
                    rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_NORMAL);
                  ...
                  case START_C1_RESEL_CONTINUE:
                    rr_resel_prepare_for_resel_from_path_loss(TRUE);
                    rr_resel_send_cell_acq_start_ind(
                  ...
                  default:
                    MSG_GERAN_LOW_1_G("RR Resel Invalid start_c1: 0x%x",c1_resel_status);
                  break;
                }
              }
              break;
              ...
              case RESEL_C2_CHOOSE_CELL:
              {
                ...
                c2_resel_status = rr_resel_start_c2(START_C2_TARGET_BEST_CELL);
                ...
                switch (c2_resel_status)
                {
                  case NACC_RESEL_FROM_BCCH_CELL:
                  {
                    ...
                    switch( surround_SI3_SI4_result )
                    {
                      ...
                      case NACC_CELL_SUITABLE_FOR_RESEL:
                        grr_start_nacc_reselection_process();
                      ...
                      default:
                        MSG_GERAN_LOW_1_G("RR RESEL Handle SI3,4 unexpected return event:0x%x",surround_SI3_SI4_result);
                      break;
                    }
                  }
                  break;
                  ...
                  case RESEL_FROM_BCCH_CELL:
                    MSG_GERAN_LOW_0_G("Prepare for path loss without clearing SI2q instance");
                    rr_resel_prepare_for_resel_from_path_loss(FALSE);
                  ...
                  default:
                    MSG_GERAN_LOW_1_G("RR Resel Invalid c2_resel_status: 0x%x",c2_resel_status);
                  break;
                }
              }
              break;
              ...
              case RESEL_C2_SELECT_NEXT_BEST_CELL:
              {
                ...
                switch (c2_resel_status)
                {
                  ...
                  case RESEL_NO_CELLS_VALID_FOR_RESEL:
                    ...
                    {
                      rr_resel_send_omsg_finished_ind(RR_RESEL_STATUS_IN_SERVICE);
                    }
                  break;
              ...
              case RESEL_G2W_PCCO:
              {
                ...
                rr_resel_data_ptr->rr_resel_c2_pend_abort_req_rxd = FALSE;
                rr_resel_data_ptr->g2x_target = rr_irat_resel_target_get();
                ...
                rr_resel_data_ptr->old_scell_arfcn = rr_gprs_get_scell_arfcn();
                ...
                rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_G2W);
              ...
              default:
                MSG_GERAN_LOW_1_G("RR Resel Invalid rr_resel_data_ptr->resel_type: 0x%x",rr_resel_data_ptr->resel_type);
              break;
            }
          ...
          case EV_T3174_TIMEOUT:
          {
            ...
            rr_gprs_deactivate_timed_sys_info_refresh();
            rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_PCCO_FAILED);
            if (rr_gmminfo_get_gmm_status() == GMM_READY_STATE)
            {
              (void)rr_timer_start(T3176, T3176_TIMEOUT);
              MSG_GERAN_HIGH_0_G("RR_RESEL_STATE_IDLE: T3174 expired, starting T3176");
              MSG_GERAN_LOW_0_G("Suspending MAC (if necessary) before reselecting old serving cell");
              rr_reselect_old_scell_after_pcco_failure(rr_resel_data_ptr);
          ...
          case EV_PS_ACCESS_IND:
          {
            ...
            MSG_GERAN_LOW_0_G("GMM is resumed");
            if (rr_timer_is_running(T3176))
            {
          ...
          {
            rr_resel_send_omsg_mac_idle_ind();
          }
          break;

          case EV_CR_G2W_TIMER_EXPIRED:
          {
            MSG_GERAN_LOW_0_G("EV_CR_G2W_TIMER_EXPIRED rxd");
            ...
            {
              MSG_GERAN_LOW_0_G("no autonomous reselection in NC2");
              new_event = EV_NO_CELL_RESELECTION;
            }
      ...
      **/
      case RR_RESEL_STATE_C1:
      {
        ...
        switch (event)
        {
            ...
            switch (message_set)
            {
              case MS_RR_RR:
              {
                switch (message->rr.header.rr_message_set)
                {
                  case RR_CELL_ACQ_IMSG:
                  {
                    rr_cell_acq_control(EV_INPUT_MESSAGE, message);
                    break;
                  } /* RR_CELL_ACQ_IMSG */
                    ...
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        ...
                        switch (message->rr.cell_acq.omsg.finished_ind.cell_acq_status)
                        {
                          ...
                          {
                            rr_gprs_cell_reselection_complete(RESEL_COMPLETE_C1);
                          ...
                          default:
                          {
                            grr_perform_reselection_power_scan();

                            if (rr_resel_data_ptr->rps_data.pscan_cell_list.no_of_entries != 0)
                            {
                              rr_perform_rps(&rr_resel_data_ptr->rps_data.pscan_cell_list);

                              (void)rr_timer_start(CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT);
                            ...
                            {
                              rr_resel_send_omsg_finished_ind(RR_RESEL_STATUS_OUT_OF_SERVICE);
                              ...
                              rr_log_reselect_failure(RR_LOG_RES_FAILURE_RESELECTION_ABORT_TIMEOUT);
                      ...
                      case RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND:
                      {
                        ...
                        rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_NORMAL);
                        rr_gprs_prepare_for_reselection_MFRM51(TRUE, FALSE);
                        (void)rr_timer_start( CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT);
                    ...
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_MAC_UL_TBF_EST:
                          {
                          ...
                          case RR_RESEL_ABORT_REASON_MM_REQ:
                          {
                            ...
                            rr_cell_acq_send_imsg_abort_req();
                          ...
                          default:
                          {
                            MSG_GERAN_LOW_1_G("RR bad abort type: 0x%x",message->rr.resel.imsg.abort_req.reason);
                          }
                        }
                        break;
                      ...
                      case RR_RESEL_IMSG_REQUIRED_IND:
                      {
                        MSG_GERAN_LOW_1_G("RR_RESEL_IMSG_REQUIRED_IND cause:0x%x rxd",message->rr.resel.imsg.required_ind.cause);
          ...
          case EV_PS_ACCESS_BLOCKED:
          {
            rr_cell_acq_control(EV_MAC_IDLE_MODE, NULL);
            break;
          } /* EV_MAC_IDLE_MODE */
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG_ABORT_CNF:
                      {
                        ...
                        {
                          MSG_GERAN_LOW_0_G("RR_CELL_ACQ_OMSG_ABORT_CNF rxd");
                        }
                        ...
                        switch (rr_resel_data_ptr->abort_cell_acq_cause)
                        {
                          ...
                          case ABORT_CELL_ACQ_RESEL_REQUEST:
                          {
                            ...
                            MSG_GERAN_LOW_1_G("RR Unexpected abort_cell_acq_cause: 0x%x", rr_resel_data_ptr->abort_cell_acq_cause);
                          }
                          break;
                ...
                switch (message->rr.header.rr_message_set)
                {
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        ...
                        switch (message->rr.cell_acq.omsg.finished_ind.cell_acq_status)
                        {
                          ...
                          case RR_CELL_ACQ_STATUS_SUCCESS_BCCH_EARLY_CAMPED:
                          {
                            ...
                            MSG_GERAN_LOW_1_G("RR Cannot handle success status:0x%x",message->rr.cell_acq.omsg.finished_ind.cell_acq_status);
                            break;
                          } /* RR_CELL_ACQ_STATUS_SUCCESS_BCCH ... */
                      ...
                      case RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND:
                      {
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND rxd");
                        rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_NORMAL);
                        rr_gprs_prepare_for_reselection_MFRM51(TRUE, FALSE);
                        (void)rr_timer_start( CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT);
                        MSG_GERAN_HIGH_0_G("RR Started CELL_RESELECTION_TIMER");
                      ...
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_NACC_NC2:
                          {
                            ...
                            MSG_GERAN_LOW_0_G("RR NACC_NC2 cause not expected");
                            break;
                          } /* RR_RESEL_ABORT_REASON_NACC_NC2 */
                ...
                switch (message->rr.header.rr_message_set)
                {
                      ...
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_MAC_UL_TBF_EST:
                             ...
                             */
                             MSG_GERAN_LOW_1_G("RR Unexpected Resel Abort Req, abort_type: 0x%x:",message->rr.resel.imsg.abort_req.reason);
                          break;
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      ...
                      {
                        MSG_GERAN_HIGH_1_G("RR_CELL_ACQ_OMSG_FINISHED_IND status:0x%x rxd",message->rr.cell_acq.omsg.finished_ind.cell_acq_status);
                        if( resel_type_C2_based())
                        {
                          MSG_GERAN_LOW_0_G("C2 based reselction finished now clear SI2q insnatce");						
                          rr_initialize_SI2q_instances();
                          rr_initialize_SI2ter_instances();
#ifdef FEATURE_GERAN_EAB
                          rr_initialize_SI21_instances();
                        ...
                        switch(message->rr.cell_acq.omsg.finished_ind.cell_acq_status)
                        {
                          ...
                          case RR_CELL_ACQ_STATUS_SUCCESS_BCCH:
                          {
                            rr_gprs_cell_reselection_complete(RESEL_COMPLETE_C2);
                            rr_l1_idle_send_imsg_start_idle_req();
                      ...
                      case RR_CELL_ACQ_OMSG_ABORT_CNF:
                      {
                        ...
                        switch (rr_resel_data_ptr->abort_cell_acq_cause)
                        {
                          case ABORT_CELL_ACQ_RESEL_TIMEOUT:
                          {
                            ...
                            if (rr_log_reselection_log_complete() == 0)
                            {
                              rr_log_reselect_failure(RR_LOG_RES_FAILURE_RESELECTION_ABORT_TIMEOUT);
                            }
                          ...
                          case ABORT_CELL_ACQ_RESEL_REQUEST:
                          {
                            ...
                            if (rr_log_reselection_log_complete() == 0)
                            {
                              rr_log_reselect_failure(RR_LOG_RES_FAILURE_CELL_SELECTION);
                            }
                          ...
                          default:
                            MSG_GERAN_LOW_1_G("RR RESEL Unknown abort cause: 0x%x", rr_resel_data_ptr->abort_cell_acq_cause);
                          break;
                        }
                      }
                      break;
      ...
      case RR_RESEL_STATE_NACC:
      {
        switch (event)
        {
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND:
                      {
                        ...
                        {
                          rr_log_reselect_start(RR_RESELECTION_TRIGGER_C2);
                        }
                      ...
                      case RR_CELL_ACQ_OMSG_RESELECTED_IND:
                      {
                        ...
                        pncd_ok = rr_gprs_is_pncd_enough_to_camp();

                        if (pncd_ok)
                        {
                          rr_cell_acq_send_imsg_nacc_acq_complete_ind();
                        }
                        else
                        {
                      ...
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        ...
                        MSG_GERAN_LOW_2_G("Clearing PCCC/PCCO flags (%d, %d)", rr_resel_data_ptr->nacc_pccc_received, rr_resel_data_ptr->nacc_pcco_received);
                        rr_resel_data_ptr->nacc_pccc_received = FALSE;
                        rr_resel_data_ptr->nacc_pcco_received = FALSE;
                    ...
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_MAC_UL_TBF_EST:
                          case RR_RESEL_ABORT_REASON_NACC_NC2:
                            MSG_GERAN_LOW_1_G("RR Unexpected Abort Cause: 0x%x",message->rr.resel.imsg.abort_req.reason);
                          break;
          ...
          case EV_T3174_TIMEOUT:
          {
            ...
            rr_log_reselect_failure(RR_LOG_RES_FAILURE_T3174_TIMEOUT);
      ...
      **/
      case RR_RESEL_STATE_NACC_PENDING_NW_RESPONSE:
      {
        switch (event)
        {
              ...
              case MS_RR_L1:
              {
                switch ((rr_l1_message_id_enum_T) message_id)
                {
                  case MPH_SERVING_MEAS_IND:
                  {
                    ...
                    local_gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();

                    rr_gprs_store_serving_meas(&message->mph_serving_meas_ind);

                    if (local_gprs_reselection_list_ptr->serving_values.C1 < 0 &&
                        !rr_allow_negative_c1(rr_gprs_get_scell_plmn_id()))
                    {
                      MSG_GERAN_HIGH_1_G("Serving cell C1 value: %d", local_gprs_reselection_list_ptr->serving_values.C1);
                      rr_resel_send_imsg(RR_RESEL_IMSG_C1_LT_ZERO_IND);
                    }
                    break;
                  ...
                  case MAC_GRR_DATA_IND:
                  {
                    ...
                    switch (mac_grr_data_ind->data_ptr.header_ptr->message_type)
                    {
                      case PACKET_CELL_CHANGE_ORDER:
                      {
                        ...
                        (void)rr_timer_cancel(T3208);
                        (void)rr_timer_cancel(T3210);

                        MSG_GERAN_LOW_1_G("A PCCO has been received in the CCN state(%d)",(int)nacc_target.num);
                        ...
                        */
                        ...
                        if (rr_gmminfo_get_gmm_status() == GMM_READY_STATE)
                        {
                          rr_gprs_nacc_notify_event(CCN_RESPONSE_FROM_NW);
                          ...
                          if (new_event == EV_ACTION_PCCO_WITH_PNCD)
                          {
                            ...
                            rr_gprs_nacc_check_pcco_reject(pcco->cell.gsm.arfcn, nacc_target);
                            ...
                            (void)rr_timer_start(T3174, T3174_TIMEOUT);
                          }
                      ...
                      case PACKET_CELL_CHANGE_CONTINUE:
                      {
                        ...
                        if ( pccc->data_valid == valid )
                        {
                          ...
                          if (rr_gprs_nacc_container_empty(pccc->container_id))
                          {
                          ...
                          else
                          {
                            ...
                            rr_gprs_prepare_for_reselection_with_PNCD(pccc->container_id);
                        ...
                        else
                        {
                          ...
                          exit_ccn_event = rr_gprs_nacc_exit_ccn_mode();
                          ...
                          switch (exit_ccn_event)
                          {
                            ...
                            default:
                            {
                              MSG_GERAN_LOW_1_G("Unexpected CCN exit event: %d", exit_ccn_event);
                            }
                          }
                        }
                      }
                    ...
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          ...
                          case RR_RESEL_ABORT_REASON_NACC_NC2:
                          {
                            MSG_GERAN_LOW_0_G("A PMO has changed the NC mode to 2 - leave CCN state");
                            rr_gprs_nacc_notify_event(CCN_RESET);
          ...
          {
            MSG_GERAN_LOW_0_G("EV_T3210_EXPIRY rxd");
          ...
          case EV_ALL_NACC_PNCD_DISPATCHED:
          {
            ...
            {
              if (rr_gprs_is_pncd_enough_to_camp() &&
                  !rr_is_forbidden_LAI(&(pcell_info_ptr->gsm.location_area_identification)))
              {
...
#endif /* FEATURE_GPRS_PS_HANDOVER */
            {
              ...
              if (rr_timer_start( CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT))
              {
                    ...
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG_ABORT_CNF:
                      {
                        ...
                        switch (rr_resel_data_ptr->abort_cell_acq_cause)
                        {
                          ...
                          default:
                            MSG_GERAN_LOW_1_G("RR Resel invalid cell acq cause: 0x%x",rr_resel_data_ptr->abort_cell_acq_cause);
                          break;
                        }
                      }
                      break;
      ...
      **/
      case RR_RESEL_STATE_RPS:
      {
        switch (rr_resel_data_ptr->rps_data.rps_sub_state)
        {
          case RR_RESEL_RPS_IDLE:
          {
            switch ( event )
            {
              case EV_INPUT_MESSAGE:
              {
                if (message == NULL)
                {
                  MSG_GERAN_LOW_0_G("message == NULL");
                  break;
                }
                ...
                switch (message_set)
                {
                  case MS_RR_RR:
                  {
                    switch (message->rr.header.rr_message_set)
                    {
                      case RR_PSCAN_IMSG:
                      {
                        rr_pscan_control(EV_INPUT_MESSAGE, message);
                        break;
                      }
              ...
              case EV_PS_ACCESS_BLOCKED:
              {
                ...
                rr_log_reselect_start(RR_RESELECTION_TRIGGER_DSF);
                rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_NORMAL);
            ...
            switch ( event )
            {
              case EV_INPUT_MESSAGE:
              {
                ...
                switch (message_set)
                {
                      ...
                      case RR_PSCAN_OMSG:
                      {
                        switch (message->rr.pscan.omsg.header.imh.message_id)
                        {
                          case RR_PSCAN_OMSG_FINISHED_IND:
                          {
                            ...
                            MSG_GERAN_LOW_0_G("RR_PSCAN_OMSG_FINISHED_IND rxd");

                            if (grr_place_results_in_best_six_list() == 0)
                            {
                              MSG_GERAN_HIGH_0_G("No BA cells in pscan, RR lost service");

#ifdef RR_DEBUG_EXTRA_INFO
                              /* Print out the results database */
                              rr_gprs_print_pscan_results();
#endif
                            ...
                            {
                              if (grr_start_bcch_decode_list_procedure() == TRUE)
                              {
                              ...
                              else
                              {
                                ...
                                */
                                ...
                                rps_resel_status = rr_resel_start_c1(TRUE, TRUE, FALSE);
                                ...
                                switch (rps_resel_status)
                                {
                                  ...
                                  default:
                                  {
                                    MSG_GERAN_LOW_1_G("RR Resel Invalid start_c1: 0x%x",rps_resel_status);
                                  }
                                }
                              }
                            }
                  ...
                  case MS_RR_L1:
                  {
                    rr_pscan_control(event, message);
                    break;
                  } /*MS_RR_L1*/
                ...
                switch (message_set)
                {
                  case MS_RR_RR:
                  {
                    switch (message->rr.header.rr_message_set)
                    {
                      ...
                      {
                        rr_decode_bcch_list_control(EV_INPUT_MESSAGE, message);
                        break;
                      }
                      ...
                      case RR_DECODE_BCCH_OMSG:
                      {
                        switch (message->rr.decode_bcch.omsg.header.imh.message_id)
                        {
                          case RR_DECODE_BCCH_OMSG_FINISHED_IND:
                          {
                            ...
                            MSG_GERAN_LOW_0_G("RR_DECODE_BCCH_OMSG_FINISHED_IND rxd");
                            ...
                            rps_resel_status = rr_resel_start_c1(TRUE, FALSE, FALSE);
                            ...
                            switch (rps_resel_status)
                            {
                              case START_C1_RESEL_FAILED:
                                    ...
                                    ))
                                {
                                  ...
                                  if (!rr_timer_is_running(CELL_RESELECTION_TIMER))
                                  {
                                    if (rr_timer_start(CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT))
                                    {
                                  ...
                                  rr_pscan_send_finish_ind(FALSE);
                                  rr_resel_data_ptr->rps_data.rps_sub_state = RR_RESEL_RPS_POWER_SCAN;
                                }
                                else
                                {
                  ...
                  case MS_RR_L1:
                  {
                    rr_decode_bcch_list_control(event, message);
                    break;
                  } /*MS_RR_L1*/
                  ...
                  case MS_TIMER:
                  {
                    switch (message_id)
                    {
                      case MID_TIMER_EXPIRY:
                      {
                        ...
                        {
                          MSG_GERAN_LOW_1_G("Unexpected timer expiry, timer_id=%d",timer_expiry->timer_id);
                        }
                        break;
                      }
                    ...
                    switch (message->rr.header.rr_message_set)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG:
                      {
                        switch (message->rr.cell_acq.omsg.header.imh.message_id)
                        {
                          case RR_CELL_ACQ_OMSG_FINISHED_IND:
                          {
                            ...
                            switch(message->rr.cell_acq.omsg.finished_ind.cell_acq_status)
                            {
                              ...
                              default:
                              {
                                ...
                                rps_resel_status = rr_resel_start_c1(FALSE, TRUE, FALSE);
                                ...
                                switch (rps_resel_status)
                                {
                                  ...
                                  case START_C1_RESEL_CONTINUE:
                                  {
                                    ...
                                    if (!rr_timer_is_running(CELL_RESELECTION_TIMER))
                                    {
                                      rr_timer_start(CELL_RESELECTION_TIMER, CELL_RESELECTION_TIMEOUT);
                          ...
                          case RR_CELL_ACQ_OMSG_RESELECTED_IND:
                          {
                            MSG_GERAN_LOW_0_G("RR_CELL_ACQ_OMSG_RESELECTED_IND rxd");
                            /* ignore this signal */
                          ...
                          case RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND:
                          {
                            MSG_GERAN_LOW_0_G("RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND rxd");
                            rr_resel_send_omsg_started_ind(RR_RESEL_TYPE_NORMAL);
                            rr_gprs_prepare_for_reselection_MFRM51(TRUE, TRUE);
                            break;
                          } /* RR_CELL_ACQ_OMSG_MAC_SUSPENDED_IND */
          ...
          default:
          {
            MSG_GERAN_LOW_1_G("Unexpected state: %d", rr_resel_data_ptr->rps_data.rps_sub_state);
          }
        }
        break;
      } /* RR_RESEL_STATE_RPS */
      ...
      **/
      case RR_RESEL_STATE_RPS_ABORTED:
      {
        switch (event)
        {
                  ...
                  case RR_PSCAN_OMSG:
                  {
                    switch (message->rr.pscan.omsg.header.imh.message_id)
                    {
                      ...
                      case RR_PSCAN_OMSG_ABORT_CNF:
                      {
                        ...
                        {
                          MSG_GERAN_LOW_0_G("RR_PSCAN_OMSG_ABORT_CNF rxd");
                        }
          ...
          case EV_MAC_GUARD_TIMER_EXPIRED:
          {
            rr_cell_acq_control(event, NULL);
            break;
          } /* EV_MAC_IDLE_MODE ... */
      ...
      **/
      ...
      case RR_RESEL_STATE_PCCO:
      {
        switch ( event )
        {
          ...
          case EV_ACTION_IMMEDIATE_PCCO:
          {
            ...
            {
              MSG_GERAN_LOW_0_G("EV_ACTION_IMMEDIATE_PCCO rxd");
            }

            rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_PCCO);

            MSG_GERAN_LOW_0_G("Deactivating timed sys info refresh...");
            rr_gprs_deactivate_timed_sys_info_refresh();
          ...
          case EV_ALL_NACC_PNCD_DISPATCHED:
          {
            ...
            rr_log_reselect_start(RR_RESELECTION_TRIGGER_PCCO);
                ...
                switch (message->rr.header.rr_message_set)
                {
                  ...
                  case RR_CELL_ACQ_OMSG:
                  {
                    switch (message->rr.cell_acq.omsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        MSG_GERAN_LOW_1_G("RR_CELL_ACQ_OMSG_FINISHED_IND status:0x%x rxd",message->rr.cell_acq.omsg.finished_ind.cell_acq_status);
          ...
          case EV_T3174_TIMEOUT:
          {
            MSG_GERAN_LOW_0_G("EV_T3174_TIMEOUT rxd");
          ...
          case EV_GMM_LEFT_READY_STATE:
          {
            MSG_GERAN_LOW_0_G("RR_RESEL_STATE_PCCO: GMM has left READY state, continue with NC0 reselection");
                      ...
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        switch (message->rr.resel.imsg.abort_req.reason)
                        {
                          case RR_RESEL_ABORT_REASON_MM_REQ:
                          {
                            ...
                            MSG_GERAN_LOW_0_G("Abort received during PCCO, cancelling T3174");
                            (void)rr_timer_cancel(T3174);
      ...
      case RR_RESEL_STATE_PCCO_FAILED_ABORTING_CELL_ACQ:
      {
        switch ( event )
        {
          ...
          case EV_T3176_TIMEOUT:
          {
            MSG_GERAN_LOW_0_G("EV_T3176_TIMEOUT rxd");
      ...
      case RR_RESEL_STATE_G2W:
      {
        switch (event)
        {
                    ...
                    switch (message->rr.header.imh.message_id)
                    {
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        if (rr_resel_data_ptr->g2x_type == RR_G2X_RESEL_TYPE_RESELECTION)
                        {
                          ...
                          {
                            rr_resel_g2w_send_imsg_abort_ind(RR_RESEL_G2W_ABORT_CAUSE_MM_REQ);
                          }
                          else if (abort_req->reason == RR_RESEL_ABORT_REASON_MAC_RR_EST ||
                                   abort_req->reason == RR_RESEL_ABORT_REASON_PAGE_RESPONSE ||
                                   abort_req->reason == RR_RESEL_ABORT_REASON_MAC_UL_TBF_EST)
                          {
                            rr_resel_g2w_send_imsg_abort_ind(RR_RESEL_G2W_ABORT_CAUSE_RR_EST_REQ);
                          }
                        }
                        else
                        {
                      ...
                      {
                        rr_resel_g2w_send_imsg(RR_RESEL_G2W_IMSG_STOP_GSM_MODE_REQ_SGLTE_RAT_CHANGE);
                        break;
                      ...
                      case RR_RESEL_G2W_OMSG_RESEL_CNF:
                      {
                        ...
                        switch (resel_cnf->status)
                        {
                          ...
                          case RR_RESEL_G2W_STATUS_RESELECTION_SUCCESS:
                          {
                            ...
                            rr_resel_data_ptr->state = RR_RESEL_STATE_IDLE;
                            rr_resel_send_omsg_finished_ind(RR_RESEL_STATUS_G2W_RESELECTION_SUCCESS);
                            break;
                          }
                          ...
                          case RR_RESEL_G2W_STATUS_RESELECTION_FAILURE:
                          {
                            ...
                            if (rr_is_sglte_mode_active())
                            {
                              ...
                              rr_g2wr_reset_wcdma_surrounding_database();
                              rr_resel_data_ptr->state = RR_RESEL_STATE_IDLE;
                              rr_resel_send_omsg_finished_ind(RR_RESEL_STATUS_G2W_RESELECTION_FAILURE);                                                                                          
                            }
                            else
#endif
                            {
                              ...
                              if (rr_resel_data_ptr->old_scell_arfcn_valid)
                              {
                                ...
                                rr_log_reselect_start(RR_RESELECTION_TRIGGER_GSM_TO_WCDMA_FAILED);
                          ...
                          case RR_RESEL_G2W_STATUS_PCCO_SUCCESS:
                          {
                            ...
                            scell_info_ptr->gsm.camped_on_flag = FALSE;
                            rr_resel_data_ptr->state = RR_RESEL_STATE_IDLE;
                            rr_resel_send_omsg_finished_ind(RR_RESEL_STATUS_G2W_PCCO_SUCCESS);
                            break;
                          }
          ...
          case EV_GSM_MODE_ACTIVATED:
          {
            ...
            rr_resel_g2w_send_imsg_l1_activated_ind();
            break;
          ...
          case EV_GSM_MODE_DEACTIVATED:
          {
            ...
            rr_resel_g2w_send_imsg_l1_deactivated_ind();
            break;
                    ...
                    switch (message->rr.header.imh.message_id)
                    {
                      ...
                      case RR_CELL_ACQ_OMSG_FINISHED_IND:
                      {
                        ...
                        switch (finished_ind->cell_acq_status)
                        {
                          ...
                          case RR_CELL_ACQ_STATUS_SUCCESS_BCCH:
                          {
                            rr_gprs_cell_reselection_complete(RESEL_COMPLETE_REACQ_AFTER_G2W_FAILURE);
                            rr_l1_idle_send_imsg_start_idle_req();
      ...
      case RR_RESEL_STATE_REDUCED_MOBILITY:
      {
        switch (event)
        {
            ...
            switch (message->message_header.message_set)
            {
              case MS_RR_L1:
              {
                switch ((rr_l1_message_id_enum_T) message_id)                
                {
                  ...
                  case MPH_SURROUND_MEAS_IND:
                  {
                    rr_reduced_mobility_control(EV_INPUT_MESSAGE, message);
                  }
                  break;

                  case MPH_BSIC_CHANGE_IND:
                  {
                    MSG_GERAN_HIGH_0_G("MPH_BSIC_CHANGE_IND rxd trigger reselection");
                    new_event = rr_reduced_mobility_control(EV_START_RESELECTION, NULL);
                  }
                  break;
              ...
              case MS_RR_RR:
              {
                ...
                {                  
                  ...
                  {
                    new_event = rr_reduced_mobility_control(EV_INPUT_MESSAGE, message);
                    break;
                  }
                  ...
                  {                    
                    switch (message->rr.resel.imsg.header.imh.message_id)
                    {
                      case RR_RESEL_IMSG_ABORT_REQ:
                      {
                        ...
                        if ((message->rr.resel.imsg.abort_req.reason == RR_RESEL_ABORT_REASON_MAC_UL_TBF_EST))
                        {
                          rr_resel_rmc_send_imsg_ps_data_req();
                        }
                        else if ((message->rr.resel.imsg.abort_req.reason == RR_RESEL_ABORT_REASON_MAC_RR_EST))
                        {
                          rr_resel_rmc_send_imsg_cs_data_req();
                        }
                        else
                        {
                          rr_resel_send_omsg_abort_cnf();                        
                      ...
                      case RR_RESEL_IMSG_REDUCED_MOBILITY_START_IND:
                      {
                        MSG_GERAN_HIGH_1_G("RR_RESEL_IMSG_REDUCED_MOBILITY_START_IND start when already in RMC type:0x%x rxd",message->rr.resel.imsg.start_reduced_mobility.reason);
                        rr_resel_rmc_send_imsg_start_ind(message->rr.resel.imsg.start_reduced_mobility.reason);
                      }                      
                      break;                      
                      ...
                      {
#ifdef FEATURE_GERAN_EXTENDED_DRX 
                        (void)rr_init_psm_edrx_params();
#endif /* FEATURE_GERAN_EXTENDED_DRX */
                        if (rr_rmc_ps_eval_in_prgrs())
                        {
                          rr_resel_send_omsg_abort_cnf(); 
                        }
          ...
          case EV_BCCH_CHANGE_DETECTED:
          {
            /*This is done to ensure that SI13 is read again*/
            (void)rr_expire_refresh_timer();
            break;
          }
          case EV_MAC_IDLE_MODE:
          {
      ...
      default:
      {
        MSG_GERAN_LOW_1_G("Unexpected state in rr_resel_control(): %d",
          rr_resel_data_ptr->state);
      }
    }
    ...
    if (rr_resel_data_ptr->state != rr_resel_data_ptr->old_state)
    {
      ...
      if (rr_resel_control_state_is_transitory(rr_resel_data_ptr->old_state))
      {
        ...
        rr_timer_cancel(RR_RESEL_CONTROL_STATE_TRANSITION_GUARD_TIMER);
      }
    ...
    else if (rr_resel_data_ptr->state == RR_RESEL_STATE_IDLE)
    {
      ...
      if (rr_timer_is_running(RR_RESEL_CONTROL_STATE_TRANSITION_GUARD_TIMER))
      {
...
*/
void rr_resel_task_start_init( void )
{
  ...
  rr_resel_data_spaces.state = RR_RESEL_STATE_IDLE;
  rr_resel_data_spaces.old_state = RR_RESEL_STATE_IDLE;
}
...
*/
boolean rr_inhibit_utran_reselection( void )
{
  ...
  if (rr_resel_algo() == RR_RESEL_ALGO_RANK_BASED)
  {
    if (scell_info_ptr->gsm.two_quater_position != SI2Q_POSITION_NONE)
    {
      if (rr_si2quater_all_instances_received(rr_get_instances_ptr(SI2Q_INSTANCE)))
      {
