...
*** TITLE
***
***  $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_main.c#4 $
***
*** DESCRIPTION
***
...
static rr_gprs_main_data_t rr_gprs_main_data;
...
static rr_event_T rr_gprs_message_from_mac(rr_cmd_bdy_type * message  );
static rr_event_T rr_gprs_message_from_l1(rr_cmd_bdy_type * message  );
 ...
 */
static rr_gprs_main_data_t *rr_gprs_main_get_data_ptr( void )
{
  return &rr_gprs_main_data;
}
...
rr_event_T rr_gprs_process_gprs_messages(rr_cmd_bdy_type * message )
{
  rr_event_T  rr_event;
  ...
  {
    case MS_MAC_RR:
      rr_event = rr_gprs_message_from_mac(message);
      break;

    case MS_RR_L1:
      rr_event = rr_gprs_message_from_l1(message);
      break;
...
rr_mac_status_t rr_macinfo_get_mac_status( )
{
  // Obtain a pointer to the module data
  rr_gprs_main_data_t *rr_gprs_main_data_ptr = rr_gprs_main_get_data_ptr();
...
)
{
  ...
  if (rr_cmd_ptr != NULL)
  {
    ...
    if (pcsc_ptr->container_length <= (CSN_PKT_CS_COMMAND_MAX_OCTETS+1))
    {
      ...
      rr_control(EV_INPUT_MESSAGE, rr_cmd_ptr);
    }
    else
    {
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_gprs_main_data_ptr = rr_gprs_main_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_main_data_ptr);
  ...
  if ( message->message_header.message_id == (byte)MAC_GRR_DATA_IND )
  {
    ...
    switch (header_ptr->message_type)
    {
      ...
      case PACKET_PSI15:
        ...
        if (grr_get_l1_in_test_mode() == FALSE)
        {
          rr_gprs_new_psi(
            header_ptr,
      ...
      default:
        ...
        rr_control(EV_INPUT_MESSAGE, message);
        break;
    }
  ...
  if ( message->message_header.message_id == (byte)MAC_GRR_MAC_STATUS )
  {
...
#ifdef FEATURE_IDLE_DRX_SCALING
        rr_idle_drx_control(EV_MAC_IDLE_MODE, NULL);
#endif /* FEATURE_IDLE_DRX_SCALING */
      }
    }
    else
    {
      ...
      rr_hz_notify_ps_status(ps_active);

#ifdef FEATURE_GERAN_BSR_PS_STATUS
      if (ps_active)
      {
        /* Stop any recent activity timer, if it is running */
  ...
  {
    rr_event = rr_gprs_nacc_handle_ncell_data_cnf(message);
  }
#if defined(FEATURE_GSM_DTM) && defined(FEATURE_GSM_EDTM)
...
static rr_event_T rr_gprs_message_from_l1(rr_cmd_bdy_type * message )
{
  ...
  switch ( message->message_header.message_id )
  {
    ...
    case MPH_DTM_CHANNEL_ASSIGN_CNF:
      ...
      rr_dtm_dual_transfer_confirmed(rr_gprs_main_data_ptr->mac_status);
      rr_event = EV_INPUT_MESSAGE;
      break;
    ...
    case MPH_DTM_CHANNEL_RELEASE_IND:
      ...
      rr_gprs_edtm_cache_dtm_release(message);
      rr_event = EV_INPUT_MESSAGE;
      break;
#endif /* FEATURE_GSM_EDTM */
...
boolean grr_get_l1_in_test_mode( )
{
  // Obtain a pointer to the module data
  rr_gprs_main_data_t *rr_gprs_main_data_ptr = rr_gprs_main_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_main_data_ptr);
...
===========================================================================*/
void grr_set_test_mode_band(rr_internal_band_T curr_band )
{
  // Obtain a pointer to the module data
  rr_gprs_main_data_t *rr_gprs_main_data_ptr = rr_gprs_main_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_main_data_ptr);
...
===========================================================================*/
rr_internal_band_T grr_get_test_mode_band( )
{
  // Obtain a pointer to the module data
  rr_gprs_main_data_t *rr_gprs_main_data_ptr = rr_gprs_main_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_gprs_main_data_ptr);
