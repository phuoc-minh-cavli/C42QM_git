...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_g2x_redir.c#2 $ */
...
static rr_g2x_redir_data_t * rr_g2x_redir_data_ptrs;
 ...
 */
static rr_g2x_redir_data_t *rr_g2x_redir_get_data_ptr(void)
{
  return rr_g2x_redir_data_ptrs;
}
...
*/
static void rr_g2x_redir_deactivate_l1( )
{
  rr_rat_e target_rat;
  // Obtain a pointer to the module data
  rr_g2x_redir_data_t* rr_g2x_redir_data_ptr = rr_g2x_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_redir_data_ptr);

  target_rat = rr_g2x_redir_map_redir_type_to_target_rat(rr_g2x_redir_data_ptr->type);
  
  if (rr_g2x_lock_exchange_allowed(RR_G2X_OPERATION_TYPE_REDIRECTION, target_rat))
  {
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_g2x_redir_data_ptr = rr_g2x_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_redir_data_ptr);
...
)
{
  ...
  sys_rat_pri_list_info_s_type * rat_pri_list_info = rr_rat_pri_list_info();
  uint32                         i;
  ...
  rr_dedicated_priority_info_get(&redir_req.dedicated_priority_info);
  ...
  result = rr_msgr_send(&redir_req.msg_hdr, sizeof(redir_req));
  if (result != E_SUCCESS)
  {
...
/**
  @brief Sends an appropriate diag event to indicate that the redirection
         has been successful.
*/
static void rr_g2x_redir_log_success( )
{
  // Obtain a pointer to the module data
  rr_g2x_redir_data_t *rr_g2x_redir_data_ptr = rr_g2x_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_redir_data_ptr);
...
/**
  @brief Sends an appropriate diag event to indicate that the redirection
         has been aborted.
*/
static void rr_g2x_redir_log_aborted( )
{
  // Obtain a pointer to the module data
  rr_g2x_redir_data_t *rr_g2x_redir_data_ptr = rr_g2x_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_redir_data_ptr);
...
/**
  @brief Sends an appropriate diag event to indicate that the redirection
         has failed.
*/
static void rr_g2x_redir_log_failure( )
{
  // Obtain a pointer to the module data
  rr_g2x_redir_data_t *rr_g2x_redir_data_ptr = rr_g2x_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_g2x_redir_data_ptr);
...
void rr_g2x_redir_init(void)
{
  if (rr_g2x_redir_data_ptrs == NULL)
  {
    ...
    rr_g2x_redir_data_ptrs = local_ptr;
  }
  else
  {
    MSG_GERAN_ERROR_0_G("rr_g2x_redir_data already allocated, clearing");
    memset(rr_g2x_redir_data_ptrs, 0, sizeof(rr_g2x_redir_data_t));
  }
}

void rr_g2x_redir_deinit( void )
{

  if (rr_g2x_redir_data_ptrs != NULL)
  {
    GPRS_MEM_FREE(rr_g2x_redir_data_ptrs);
    rr_g2x_redir_data_ptrs = NULL;
  }
}
          ...
          switch (message->rr.header.rr_message_set)
          {
            case RR_G2X_REDIR_IMSG:
            {
              switch (message->message_header.message_id)
              {
                case RR_G2X_REDIR_IMSG_L1_DEACTIVATED_IND:
                {
                  geran_abandon_lock_exchange(GERAN_EXCHANGE_GTOX);
                  rr_g2x_redir_send_omsg(RR_G2X_REDIR_OMSG_ABORT_CNF);
                  rr_g2x_redir_data_ptr->state = RR_G2X_REDIR_STATE_NULL;
                  break;
                } /* RR_G2X_REDIR_IMSG_L1_DEACTIVATED_IND */
              ...
              switch (message->message_header.message_id)
              {
                case RR_G2X_REDIR_IMSG_ABORT_REQ:
                {
                  ...
                  switch (rr_g2x_redir_data_ptr->type)
                  {
                    ...
                    case FDD_TARGET:
                    {
                      rr_send_interrat_gtow_redirection_abort_req();
                    ...
                    case TDD_TARGET:
                    {
                      rr_send_tds_interrat_redirection_abort_req();
                      rr_g2x_redir_data_ptr->state = RR_G2X_REDIR_STATE_REDIRECTING_ABORTED;
                      break;
                    } /* FDD_TARGET */
#endif /*FEATURE_GSM_TDS_G2T_REDIRECTION*/
                    ...
                    case LTE_TARGET:
                    {
                      ...
                      result = rr_msgr_send(&abort_redir_req.msg_hdr, sizeof(abort_redir_req));
                      if (result != E_SUCCESS)
                      {
                        MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
                      }
    ...
    */
      ...
      switch (message->message_header.message_set)
      {
        ...
        case MS_TDSRRC_RR:
        {
          switch (message->message_header.message_id)
          {
            ...
            {
              geran_abandon_lock_exchange(GERAN_EXCHANGE_XTOG);
              rr_g2x_redir_send_omsg(RR_G2X_REDIR_OMSG_ABORT_CNF);
              rr_g2x_redir_data_ptr->state = RR_G2X_REDIR_STATE_NULL;
              break;
    ...
    case RR_G2X_REDIR_STATE_SGLTE_ABORTED_MODE_CHANGE:
    {
          ...
          message->message_header.message_id == MPH_UE_MODE_CHANGE_CNF)
      {
         ...
         rr_g2x_redir_send_omsg(RR_G2X_REDIR_OMSG_SGLTE_ABORT_CNF);
         rr_g2x_redir_data_ptr->state = RR_G2X_REDIR_STATE_NULL;
...
#ifdef FEATURE_SGLTE
boolean rr_sglte_g2l_blind_redirection_active( )
{
  rr_g2x_redir_data_t *rr_g2x_redir_data_ptr;
