...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sglte.c#2 $ */
...
)
{
  ...
  result = rr_msgr_send(&sglte_serving_meas_rsp.msg_hdr, sizeof(sglte_serving_meas_rsp));
  if (result != E_SUCCESS)
  {
...
)
{
  ...
  result = rr_msgr_send(&sglte_neighbor_meas_rsp.msg_hdr, sizeof(sglte_neighbor_meas_rsp));
  if (result != E_SUCCESS)
  {
    MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
  }
...
#ifdef FEATURE_LTE
void rr_send_lte_sglte_serving_meas_rsp_no_service( )
{
...
void rr_send_lte_sglte_neighbor_meas_rsp_no_service( )
{
...
void rr_send_tds_sglte_serving_meas_rsp_no_service( )
{
  rr_send_irat_sglte_serving_meas_rsp_no_service(
...
void rr_sglte_set_rat_priority_list_gsm_only( )
{
  rr_PLMN_request_details_T * rr_plmn_request_details = rr_get_PLMN_request_details_ptr();
...
#ifdef FEATURE_LTE
void rr_sglte_reject_lte_resel_req( )
{
...
void rr_sglte_reject_lte_redir_req( )
{
  rr_x2g_send_lte_redir_failure();
  return;
}

void rr_sglte_reject_lte_plmn_srch_req( )
{
...
void rr_sglte_reject_tds_resel_req( )
{
...
void rr_sglte_reject_tds_redir_req( )
{
...
#ifdef FEATURE_GSM_TDS
void rr_sglte_reject_tds_plmn_srch_req( )
{
  MSG_GERAN_HIGH_0_G("Rejecting L2G PLMN List Search");
...
===========================================================================*/
boolean rr_sglte_multimode_using_g_only_device( void )
{
  ...
  if (rr_is_sglte_mode_active() &&
      ...
      (TRM_DENIAL != grm_get_granted(TRM_GSM1, &trm_query)))
  {
