...
==============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_resel_nacc.c#5 $ */
...
static rr_resel_nacc_data_t rr_resel_nacc_data =
 ...
 */
static rr_resel_nacc_data_t *rr_resel_nacc_get_data_ptr( void )
{
  return &rr_resel_nacc_data;
}
...
)
{
  switch (event)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_1("Unexpected event: %d", event);
      return "?";
    }
  }
}
...
)
{
  ...
  rr_resel_nacc_data_t *rr_resel_nacc_data_ptr = rr_resel_nacc_get_data_ptr();
...
{
  inter_task_BCCH_allocation_T *ba_ptr = rr_gprs_get_ba_gprs();
      ...
      (rr_resel_nacc_data_ptr->nacc_RAT_type         != NACC_WCDMA_GSM))
  {
    ...
    rr_send_mph_read_sys_info_req(RR_L1_SI_TYPE_2, rr_gprs_get_scell_info()->gsm.si_sched_info);
  }
  else
  {
    MSG_GERAN_LOW_2_G("NACC status: RAT=%1d, BA=%d",
...
)
{
  ...
  if (rr_gprs_send_sysinfo_status(NACC_STATUS_TIMEOUT))
  {
  ...
  {
    next_state = rr_gprs_nacc_force_sys_info_refresh();
  }
...
)
{
  ...
  memscpy(si_buffer+2,sizeof(si_buffer)-2, msg_ptr, num_octets);
    ...
    wrapper_ptr->data_ptr = gcsn1_decoder(si_buffer, &dummy,N201_MAX+2);
...
)
{
  ...
  if (container_id != INVALID_CONTAINER_ID)
  {
    ...
    else
    {
      ...
      MSG_GERAN_LOW_1_G("NACC: Invalid container ID %d", container_id);
      rr_resel_nacc_data_ptr->nacc_RAT_type  = NACC_GSM_GSM;
    }
  }
  else
...
)
{
  ...
  for (index = 0; index < MAX_NUM_NACC_STORED_MSGS; index++, nacc_msg_ptr++)
  {
    if (index < rr_resel_nacc_data_ptr->nacc_msg_count)
    {
      ...
      if (container_id < NUM_NACC_CONTAINERS)
      {
        ...
        {
          (void)rr_timer_cancel(NACC_CONTAINER_TIMER(container_id));
  ...
  rr_gprs_nacc_store_trigger_info(INVALID_CONTAINER_ID);
...
)
{
  ...
  if (rr_timer_is_running(GRR_NACC_CONTAINER_TIMER_0) ||
      rr_timer_is_running(GRR_NACC_CONTAINER_TIMER_1) ||
      rr_timer_is_running(GRR_NACC_CONTAINER_TIMER_2) ||
      rr_timer_is_running(GRR_NACC_CONTAINER_TIMER_3))
  {
    /* ask GMAC to release neighbour cell data for ALL containers */
    rr_send_grr_mac_nacc_release_req(0, NACC_RELEASE_ALL);
  }
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("Null ptr supplied as NACC data");
    return;
  }
  ...
  if (rr_resel_nacc_data_ptr->nacc_msg_count < MAX_NUM_NACC_STORED_MSGS)
  {
    ...
    else if (rr_resel_nacc_data_ptr->nacc_msg_count > 0)
    {
      ...
      if (rr_gprs_nacc_container_info(container_id) != CONTAINER_EMPTY)
      {
    ...
    if (container_id < NUM_NACC_CONTAINERS)
    {
      ...
      (void)rr_timer_restart(timer_id, GRR_TIMEOUT_30_SECONDS);
...
)
{
  ...
  else if (data_type == GERAN_INFO_TYPE_SI)
  {
    ...
    if (rr_gprs_nacc_wrap_si_data(&wrapper, msg_ptr, num_octets))
    {
  ...
  if (wrapper.protocol != DUMMY_PD)
  {
    ...
    rr_gprs_nacc_store_ncell_data(&wrapper);
  }
  ...
  for (container_id = 0; container_id < NUM_NACC_CONTAINERS; container_id++)
  {
    if (timer_id == NACC_CONTAINER_TIMER(container_id))
    {
      MSG_GERAN_HIGH_2_G("NACC timer expiry for container ID %1d (%d)",
               (int)container_id, (int)(rr_resel_nacc_data_ptr->nacc_msg_count));
      rr_gprs_nacc_free_container_data(container_id, TRUE);
...
)
{
      ...
      (nacc_msg_ptr->protocol == BCCH_PD))
  {
    ...
    pcell_ptr = rr_gprs_get_pcell_info();
...
)
{
  ...
  else
  {
    ...
    for (index = 0; index < rr_resel_nacc_data_ptr->nacc_msg_count; index++, nacc_msg_ptr++)
    {
      ...
      if (nacc_msg_ptr->container_id == container_id)
      {
        ...
        {
          rr_gprs_nacc_si1_detection(nacc_msg_ptr);
        }

        if (rr_gprs_nacc_data_ind(nacc_msg_ptr, target_arfcn))
        {
  ...
  (void)rr_gprs_nacc_trigger_confirm(container_id, &target_arfcn, num_stored);
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (rr_gprs_nacc_check_pccn_flag())
      {
        /* start the PCCN resend timer */
        (void)rr_timer_start(T3210, T3210_TIMEOUT);

        /* restart T3208 to guard the entire NACC procedure */
        (void)rr_timer_restart(T3208, T3208_TIMEOUT);
      }
      else
      {
...
)
{
  ...
  {
    rr_gprs_nacc_notify_event(CCN_MAC_IN_TRANSFER);
  }
  else if (mac_status == MAC_IDLE)
  {
    rr_gprs_nacc_notify_event(CCN_MAC_IN_IDLE);
  }
...
)
{
  ...
  case CCN_MAC_IN_TRANSFER:
    if (rr_resel_nacc_data_ptr->nacc_state == NACC_COMPLETE)
    {
      ...
      rr_resel_nacc_data_ptr->nacc_state = rr_gprs_nacc_send_status();
    }
    break;
  ...
  default:
    MSG_GERAN_LOW_2_G("NACC: invalid event %1d in state %1d", (int)event_in, (int)(rr_resel_nacc_data_ptr->nacc_state));
    break;
  }
...
)
{
  ...
  if (rr_resel_nacc_data_ptr->nacc_RAT_type == NACC_WCDMA_GSM)
  {
    ...
    MSG_GERAN_LOW_0_G("Unexpected NACC RAT type, resume reselection");
    rr_gprs_nacc_reset();
    rr_resel_nacc_data_ptr->nacc_RAT_type = NACC_GSM_GSM;
  }
  ...
  else if (rr_resel_nacc_data_ptr->nacc_RAT_type == NACC_GSM_WCDMA)
  {
    ...
    MSG_GERAN_LOW_0_G("Leaving CCN state, continue G2W reselection");
  }
...
)
{
  ...
  bAllowed = rr_get_nv_geran_feature_pack_1_enabled();

#ifdef FEATURE_WCDMA
  if ((bAllowed == TRUE) && (rat_type != NACC_GSM_GSM))
  {
    uint8 irat_support = rr_get_nv_interrat_nacc_support();
...
{
  container_info_t info = rr_gprs_nacc_container_info(container_id);
