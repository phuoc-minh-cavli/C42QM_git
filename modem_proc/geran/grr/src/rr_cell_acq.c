...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_cell_acq.c#7 $ */
...
static rr_cell_acq_data_t rr_cell_acq_data;
 ...
 */
static rr_cell_acq_data_t *rr_cell_acq_get_data_ptr( void )
{
  return &rr_cell_acq_data;
}
 ...
 * \param 
 */
static void rr_cell_acq_init( )
{
  // Obtain the pointer to the module data
  rr_cell_acq_data_t *rr_cell_acq_data_ptr = rr_cell_acq_get_data_ptr();
...
static void rr_cell_acq_reset_parameters( )
{
 ...
 */
static void rr_cell_acq_suspend_gprs( )
{
 ...
 *
 ****************************************************************************/
static void rr_gprs_surround_update_SI3_SI4_for_ARFCN(ARFCN_T BCCH_ARFCN )
{
  ...
  if ((bcch_ncell_info = rr_gprs_get_surround_ncell_sys_info(BCCH_ARFCN)) != NULL)
  {
  ...
  (void)rr_timer_start(RR_GPRS_SYS_INFO_TIMER, BCCH_SI3_SI4_TIMEOUT);
  ...
  /*Send request to L1*/
  si_from_cache = rr_send_mph_gprs_surround_update_req(&gprs_surround_data, TRUE, bcch_ncell_info);

  /* Set up the surround response handler */
  set_waiting_for_surround_response( BCCH_ARFCN, si_from_cache );
 ...
 *
 ****************************************************************************/
static void rr_cell_acq_early_camping_confirmed( )
{
  rr_cell_acq_data_t *rr_cell_acq_data_ptr = rr_cell_acq_get_data_ptr();
...
)
{
      ...
      !sys_plmn_id_is_undefined(rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification.plmn_id))
  {
    ...
    if (!plmn_ok)
    {
      ...
      plmn_ok = rr_eplmn_list_equivalent_plmn(rr_cell_acq_data_ptr->requested_plmn) &&
                rr_eplmn_list_equivalent_plmn(rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification.plmn_id);
    }
  ...
  if (rr_is_any_cell_reselection_mode() == FALSE)
  {
    if (rr_is_forbidden_LAI(&rr_cell_acq_data_ptr->gprs_cell_info->gsm.location_area_identification))
    {
...
)
{
  ...
  surround_db_entry_ptr = rr_gprs_get_surround_ncell_sys_info(rr_cell_acq_data_ptr->target_cell);
  ...
  if (rr_gprs_resel_suitable_cell(surround_db_entry_ptr))
  {
    if (rr_gprs_parameters_available() == FALSE)
    {
      ...
      if (C1 >= 0 || rr_allow_negative_c1(surround_db_entry_ptr->gsm.location_area_identification.plmn_id))
      {
        ...
        C2 = rr_gprs_compute_surrounding_C2(C1, rr_resel_candidate);
        ...
        surrounding_better_than_serving = rr_gprs_compare_serving_and_surrounding_C2(
          ...
          ,rr_resel_candidate->ncell_data->rxlev
    ...
    else
    {
      ...
      else if (local_fresh_reselection_values.C1 >= 0 || rr_allow_negative_c1(surround_db_entry_ptr->gsm.location_area_identification.plmn_id))
      {
        /***************************************************/
 ...
 *
 ****************************************************************************/
static void rr_gprs_action_cell_reselection( )
{
  gprs_surround_data_t gprs_surround_data;
  ...
  switch (rr_cell_acq_data_ptr->action)
  {
    ...
    case RR_CELL_ACQ_ACTION_RESELECT_BCCH_PCCO_FAILED:
    {
      ...
      */
      if (rr_cell_acq_data_ptr->sync_flag )
      {
        ...
        {
          rr_send_mph_select_specific_channel(rr_cell_acq_data_ptr->target_cell,BSIC);
        }
        else
        {
          rr_send_mph_gprs_reselection_req(&gprs_surround_data, FALSE);
        }
      ...
      else
      {
        ...
        if (rr_cell_acq_data_ptr->action == RR_CELL_ACQ_ACTION_SELECT_BCCH)
        {
          if ( rr_log_reselection_log_complete() )
          {
            ...
            rr_log_reselect_start(RR_RESELECTION_TRIGGER_NONE);
          }
          rr_send_mph_select_specific_channel(rr_cell_acq_data_ptr->target_cell,BSIC);
        ...
        {
          rr_send_mph_gprs_reselection_req(&gprs_surround_data, TRUE);
        }
      }
    ...
    default:
    {
      MSG_GERAN_LOW_1_G("Unexpected action: %d", rr_cell_acq_data_ptr->action);
      break; 
    }
  }
  ...
  rr_gprs_nacc_notify_event(CCN_REQUESTED_TO_L1);
}
...
)
{
  if (ARFCNS_EQUAL(msg->BCCH_ARFCN, target_arfcn))
  {
    ...
    bcch_ncell_db_ptr = rr_gprs_get_surround_ncell_sys_info(target_arfcn);
    ...
    resel_candidate = rr_resel_best_six_find(target_arfcn, NULL);
    ...
    /*********************************************************************/
    if ((rr_gprs_camped_on_ccch() || rr_gsm_camped_on_ccch()) && bcch_ncell_db_ptr)
    {
      if (msg->state_changed)
      {
        bcch_ncell_db_ptr->gsm.valid_data_flag = TRUE;

        (void)rr_timer_create_expired_timer(resel_candidate->reselection_info.five_second_timer_id);
...
======================================================================*/
static void rr_cell_acq_check_sys_info_timer(const rr_l1_message_id_enum_T gl1_msg_id )
{
  ...
  {
    (void)rr_timer_cancel(RR_GPRS_SYS_INFO_TIMER);
    if (rr_timer_start(RR_GPRS_SYS_INFO_TIMER, SI3_SI4_RESTART_TIMEOUT))
    {        
  ...
  if (FALSE == rr_timer_is_running(RR_GPRS_SYS_INFO_TIMER))
  {
  ...
  while (event_in != EV_NO_EVENT)
  {
    switch (rr_cell_acq_data_ptr->state)
    {
                  ...
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_START_IND:
                      {
                        ...
                        {
                          rr_gprs_surround_update_SI3_SI4_for_ARFCN(rr_cell_acq_data_ptr->target_cell);
                      ...
                      case RR_CELL_ACQ_IMSG_ABORT_REQ:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_ABORT_REQ in NULL state");
                        rr_cell_acq_send_omsg_abort_cnf();
                        break;
                      }
            ...
            switch (message_set)
            {
                  ...
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_SURROUNDING_SI_RECEIVED_IND:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_SURROUNDING_SI_RECEIVED_IND rxd");
                      ...
                      case RR_CELL_ACQ_IMSG_ABORT_REQ:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_ABORT_REQ rxd");
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_SURROUND_UPDATE_FAILURE_IND:
                  {
                    MSG_GERAN_LOW_0_G("MPH_SURROUND_UPDATE_FAILURE_IND rxd");
        ...
        switch (event_in)
        {
                ...
                switch (message->rr.header.rr_message_set)
                {
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_ABORT_REQ:
                      {
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_ABORT_REQ rxd in state RR_CELL_ACQ_STATE_SUSPENDING_MAC");
          ...
          case EV_MAC_IDLE_MODE:
          {
            ...
            if (rr_mac_ps_access_blocked())
            {
              ...
              rr_cell_acq_send_omsg_mac_suspended_ind();

              rr_gprs_action_cell_reselection();
            }
              ...
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  case MPH_RESELECTION_CNF:
                  {
                    ...
                    {
                      rr_cell_acq_check_sys_info_timer(MPH_RESELECTION_CNF);
                      rr_cell_acq_send_omsg_reselected_ind();
                  ...
                  case MPH_SELECT_SPECIFIC_BCCH_CNF:
                  {
                    ...
                    bcch_ncell_info_t *ptr_surround_database;                   
                    ptr_surround_database = rr_gprs_get_surround_ncell_sys_info(rr_cell_acq_data_ptr->target_cell);
                    ...
                    if (message_ptr->BCCH_found)
                    {
                      ...
                      rr_cell_acq_check_sys_info_timer(MPH_SELECT_SPECIFIC_BCCH_CNF);
                      ...
                      if (rr_cell_acq_data_ptr->gprs_cell_info->gsm.control_channel_description_valid)
                      {
                        ...
                        {
                          rr_cell_acq_send_omsg_finished_ind(status);
                          rr_cell_acq_reset_parameters();
                          rr_cell_acq_data_ptr->state = RR_CELL_ACQ_STATE_NULL;
                          break;
                        }
                      ...
                      else
                      {
                        rr_cell_acq_send_omsg_reselected_ind();
#ifdef FEATURE_RR_SI_CACHE
                        rr_csi_activate_pend_store(); /* CGI of target cell is unknown, store any SI's before SI3 is received
                                                               in the pend store */
#endif
                        rr_cell_acq_data_ptr->state = RR_CELL_ACQ_STATE_ACQUIRING_BSI_GSM;
                        rr_cell_acq_data_ptr->early_camping_status = RR_CELL_ACQ_EARLY_CAMPING_NOT_STARTED;
                ...
                switch (message->rr.header.rr_message_set)
                {
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_ABORT_REQ:
                      {
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_ABORT_REQ rxd, wait for GL1 response");
            ...
            switch (message_set)
            {
              case MS_RR_L1:
              {
                switch (message_id)
                {
                  ...
                  case MPH_SELECT_SPECIFIC_BCCH_CNF:
                  {
                    MSG_GERAN_LOW_1_G("Received message 0x%02X from GL1",(int)message_id);
                  ...
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_EARLY_CAMPING_BSI_RECEIVED_IND:
                      {
                        rr_cell_acq_status_e status = RR_CELL_ACQ_STATUS_UNDEFINED;
                        boolean              cell_suitable;
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_EARLY_CAMPING_BSI_RECEIVED_IND rxd");
                        ...
                        if (cell_suitable)
                        {
                          ...
                          x2g_resel_with_nacc = rr_x2g_resel_with_nacc_in_progress(); 
                      ...
                      case RR_CELL_ACQ_IMSG_GSM_BSI_RECEIVED_IND:
                      {
                        ...
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_GSM_BSI_RECEIVED_IND rxd");
                        ...
                        if (cell_suitable)
                        {
                          ...
                          if (rr_cell_acq_data_ptr->gprs_cell_info->gsm.gprs_indicator)
                          {
                            ...
                            else
                            {
                              ...
                              (void)rr_timer_start(RR_GPRS_SYS_INFO_TIMER, BCCH_SI13_TIMEOUT);

                              if (is_fast_si_acq_enabled && !wait_for_si13)
                              {
                                MSG_GERAN_LOW_0_G("Fast SI Acq enabled, SI13 not yet received, start GSM idle mode");
                                if(rr_cell_acq_data_ptr->early_camping_status == RR_CELL_ACQ_EARLY_CAMPING_NOT_STARTED)
                                {
                                  rr_cell_acq_send_omsg_finished_ind(RR_CELL_ACQ_STATUS_SUCCESS_BCCH);
                                  rr_cell_acq_reset_parameters();
                                  rr_cell_acq_data_ptr->state = RR_CELL_ACQ_STATE_NULL;
                                }
                                else if(rr_cell_acq_data_ptr->early_camping_status == RR_CELL_ACQ_EARLY_CAMPING_COMPLETED)
                                {
                                  rr_cell_acq_send_omsg_finished_ind(RR_CELL_ACQ_STATUS_SUCCESS_BCCH_EARLY_CAMPED);
                                  rr_cell_acq_reset_parameters();
                                  rr_cell_acq_data_ptr->state = RR_CELL_ACQ_STATE_NULL;
                                }
                                else if(rr_cell_acq_data_ptr->early_camping_status == RR_CELL_ACQ_EARLY_CAMPING_REQ_SENT)
                      ...
                      {
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_NACC_ACQ_COMPLETE_IND rxd");
                  ...
                  case RR_L1_IDLE_OMSG:
                  {
                    switch (message->rr.l1_idle.omsg.header.imh.message_id)
                    {
                      case RR_L1_IDLE_OMSG_START_EARLY_CAMP_CNF:
                      {
                        ...
                        rr_cell_acq_early_camping_confirmed();
                        rr_cell_acq_check_sys_info_timer(MPH_START_IDLE_MODE_CNF);
                ...
                switch (message->rr.header.rr_message_set)
                {
                  case RR_CELL_ACQ_IMSG:
                  {
                    switch (message->rr.cell_acq.imsg.header.imh.message_id)
                    {
                      case RR_CELL_ACQ_IMSG_GPRS_BSI_RECEIVED_IND:
                      {
                        MSG_GERAN_LOW_0_G("RR_CELL_ACQ_IMSG_GPRS_BSI_RECEIVED_IND rxd");
      ...
      **/
      ...
      default:
      {
        ...
        MSG_GERAN_LOW_1_G("In unknown state (%d)", rr_cell_acq_data_ptr->state);
        break;
      }
    }
    ...
    if (rr_cell_acq_data_ptr->state != rr_cell_acq_data_ptr->old_state)
    {
      ...
      if (rr_cell_acq_control_state_is_transitory(rr_cell_acq_data_ptr->old_state))
      {
        ...
        rr_timer_cancel(RR_CELL_ACQ_CONTROL_STATE_TRANSITION_GUARD_TIMER);
      }
    ...
    else if (rr_cell_acq_data_ptr->state == RR_CELL_ACQ_STATE_NULL)
    {
      ...
      if (rr_timer_is_running(RR_CELL_ACQ_CONTROL_STATE_TRANSITION_GUARD_TIMER))
      {
