...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_resel_utils.c#8 $ */
...
static sync_cell_data_t sync_cell_data;
 ...
 */
static sync_cell_data_t *sync_cell_get_data_ptr( void )
{
  return &sync_cell_data;
}
...
{
  rr_resel_candidate_t * resel_candidate = rr_resel_best_six_find(arfcn, NULL);
  ...
  if (resel_candidate && resel_candidate->ncell_data->bcch_or_pmo == BCCH_FORMAT)
  {
    ...
    (void)rr_timer_cancel(resel_candidate->penalty_timer_id);
...
)
{
  ...
  if (best_six_ncells_ptr->no_of_entries < MAX_NUMBER_OF_BEST_CELLS)
  {
    ...
    if ( best_six_ncells_ptr->ncell_entry[best_six_ncells_ptr->no_of_entries].ncell_data == NULL )
    {
      ...
      {
        MSG_GERAN_LOW_1_G("No matching ncell entry found %d",CONVERT_FROM_ARFCN_T(arfcn));
      }
    ...
    else
    {
      ...
      if (resel_candidate->ncell_data->bcch_or_pmo == BCCH_FORMAT)
      {
        ...
        resel_candidate->penalty_timer_id = rr_gprs_get_gprs_penalty_timer_id();
        resel_candidate->reselection_info.five_second_timer_id = rr_gprs_get_five_second_timer_id();
...
)
{
       ...
       ++i)
  {
        ...
        SAME_BSIC(resel_candidate->ncell_data->bsic, bsic))
    {
      ...
      si_from_cache = FALSE;
      if (match_surround_response_with_cache_check(arfcn, &si_from_cache))
      {
        if (!si_from_cache)
      {
 ...
 *
 ****************************************************************************/
rr_timer_id_T rr_gprs_get_gprs_penalty_timer_id( )
{
  best_six_ncells_t *best_six_ncells_ptr = rr_get_best_six_ncells_ptr();
    ...
    if (penalty_timer_used[index] == FALSE)
    {
      switch (index)
      {
        ...
        case 5:
          ...
          break;
        default:
          MSG_GERAN_LOW_0_G("No free penalty timer, reusing number 6");
          penalty_timer_id = GRR_PENALTY_TIMER_6;
      }
    ...
    if (five_sec_timer_used[index] == FALSE)
    {
      switch (index)
      {
        ...
        case 5:
          ...
          break;
        default:
          MSG_GERAN_LOW_0_G("No free 5 sec timer, reusing number 6");
          five_sec_timer_id = RR_FIVE_SEC_C2_TIMER_6;
      }
      /* Stop any more iterations, a timer has been found */
      break;
...
)
{
  ...
  {
    MSG_GERAN_LOW_0_G("Null PTR Assigned");
  ...
  /**********************************************************************/
  if (!rr_is_any_cell_reselection_mode())
  {
...
#ifndef FEATURE_GERAN_CS_SUPPORT
  gprs_indicator = ptr_surround_database->gprs_supported;
#endif /* !FEATURE_GERAN_CS_SUPPORT */
...
#ifndef FEATURE_GERAN_CS_SUPPORT
  else if (gprs_indicator == FALSE)
  {
    MSG_GERAN_MED_1_G("ncell(%d) GPRS not supported", CONVERT_FROM_ARFCN_T(ptr_surround_database->gsm.BCCH_ARFCN));
  }
#endif /* !FEATURE_GERAN_CS_SUPPORT */
...
)
{
  ...
  {
    p_timer_id = rr_gprs_return_penalty_timer_id(index);

    if ( *p_timer_id == penalty_timer_id )
    {
      (void)rr_timer_create_expired_timer(*p_timer_id);
...
)
{
  ...
  for (index = 0; index < best_six_ncells_ptr->no_of_entries; ++index)
  {
    ...
    {
      (void)rr_timer_create_expired_timer(five_sec_timer_id);
      ...
      )
{
  ...
  for (index = 0; index < best_six_ncells_ptr->no_of_entries; ++index)
  {     
    rr_timer_state = rr_timer_get_state(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id);
    ...
    if ((rr_timer_state == RR_TIMER_EXPIRED) || (rr_timer_state == RR_TIMER_RUNNING))    
    {
      ...
      /* reselection attempt.                                                     */
      /****************************************************************************/
      (void)rr_timer_cancel(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id);      
    }
  }
}
...
{
  if (rr_timer_start(rr_gprs_reselection_meas_ptr->five_second_timer_id, RR_FIVE_SEC_TIMEOUT))
  {
    MSG_GERAN_HIGH_1_G("Started 5 second timer (%d)",
             rr_gprs_reselection_meas_ptr->five_second_timer_id);
  }
  else
{
    MSG_GERAN_LOW_1_G("Failed to start 5 second timer (%d)", rr_gprs_reselection_meas_ptr->five_second_timer_id);
  }
...
)
{
  if (rr_gprs_reselection_meas_ptr != NULL)
  {
    ...
    {
        MSG_GERAN_HIGH_1_G("Stopped 5 second timer:%d",
                 (int)five_second_timer_id);
      }
    }
    }
    else
    {
      MSG_GERAN_LOW_0_G("NULL pointer for stopping 5 second timer");
    }
...
)
{
  ...
  {
    (void)rr_timer_create_expired_timer(resel_candidate->penalty_timer_id);
  }
}
  ...
  for (index = 0; (index < measurement_entries) && (index < MAX_NUMBER_OF_SURROUND_CELLS) ; index++)
  {
    ...
    if (ARFCNS_EQUAL(ncell_ptr->arfcn, BCCH_ARFCN))
    {
      ...
      else
      {
        ...
        /**************************************************************/
        MSG_GERAN_LOW_1("Meas info not valid for cell: %d",(int) CONVERT_FROM_ARFCN_T(BCCH_ARFCN));
        /***********************************************************/
...
)
{
  ...
  {
    MSG_GERAN_LOW_2("measurement_report_entries (%d) out of range, truncating to %d",
...
{
  sync_cell_data_t *sync_cell_data_ptr = sync_cell_get_data_ptr();
  //RR_NULL_CHECK_FATAL(sync_cell_data_ptr);

  if(!rr_gprs_is_cell_in_sync_list(arfcn))
  {
...
)
{
  ...
  grr_event = rr_gprs_nc_measurement_control(RR_EV_MEASUREMENT_PARAMS_UPDATED, NULL);

  /* Pass active/inactive indication into reselection control */
  (void) rr_resel_control(grr_event, NULL);
...
)
{
  ...
  {
    MSG_GERAN_LOW_0("pscan_cell_list_ptr == NULL");
  }
...
)
{
  ...
  {
    MSG_GERAN_LOW_2("reselection_list->no_of_entries (%d) out of range, truncating to %d",
...
{
  uint8                          i;
  gprs_reselection_structure_T * reselection_list = rr_get_gprs_reselection_list_ptr();
 ...
 *
 ****************************************************************************/
boolean grr_any_other_reselection_candidates(bcch_ncell_info_t *ref_entry)
{
  ...
  uint8                      index;
  ARFCN_T                    ref_arfcn = ref_entry->gsm.BCCH_ARFCN;
  boolean                    ref_gprs_supported = ref_entry->gprs_supported;
  ...
  for (index = 0; index < best_six_ncells_ptr->no_of_entries; index++)
  {
    ...
    ref_values = ptr_ncell_reselection_list(ref_arfcn);
    candidate_values = ptr_ncell_reselection_list(surround_db_entry_ptr->gsm.BCCH_ARFCN);
    ...
    else
    {
      ...
      {
        if (ref_gprs_supported == TRUE)
        {
          /* Reference entry supports CS+PS.
           * Check for better C2 ncell having CS+PS support and its timer is running. */
          if (surround_db_entry_ptr->gprs_supported == TRUE && 
              rr_timer_is_running(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id) &&
              (ref_values && candidate_values && candidate_values->C2 > ref_values->C2))
          {
        ...
        {
          /* Reference entry supports only CS.
           * i)  Check for any ncell having CS+PS support and its timer is running or expired or
           * ii) Check for better C2 ncell having CS only support and its timer is running. */
          if (surround_db_entry_ptr->gprs_supported == TRUE && 
              (rr_timer_is_running(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id) ||
               rr_timer_get_state(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id) == RR_TIMER_EXPIRED))
          {
            better_candidate_found = TRUE;
            break;
          }
          else if (surround_db_entry_ptr->gprs_supported == FALSE && 
                   rr_timer_is_running(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id) &&
                   (ref_values && candidate_values && candidate_values->C2 > ref_values->C2))
          {
            better_candidate_found = TRUE;
...
#endif
      {
        if (rr_timer_is_running(best_six_ncells_ptr->ncell_entry[index].reselection_info.five_second_timer_id))
        {
 ...
 *
 ****************************************************************************/
boolean rr_gprs_check_if_any_g2x_resel_timer_is_running(  )
{
  boolean g2x_timer_running = FALSE;

#if defined(FEATURE_WCDMA) || defined(FEATURE_GSM_TDS) || defined(FEATURE_LTE)
  if (
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_1) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_2) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_3) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_4) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_5) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_6) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_7) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_8) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_9) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_10) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_11) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_HIGH_TIMER_12) ||
        
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_1) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_2) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_3) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_4) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_5) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_6) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_7) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_8) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_9) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_10) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_11) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_THRESH_LOW_TIMER_12) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_1) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_2) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_3) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_4) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_5) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_6) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_7) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_8) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_9) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_10) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_11) ||
        rr_timer_is_running(RR_IRAT_PRIORITY_RESEL_H_PRIO_TIMER_12) ||
 
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_1) ||
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_2) ||
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_3) ||
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_4) ||
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_5) ||
        rr_timer_is_running(RR_G2WR_FIVE_SEC_TIMER_6) 
  )
 {
