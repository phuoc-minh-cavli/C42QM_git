...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_band_avoid_if.c#2 $ */
 ...
 */
static sys_band_class_e_type rr_band_avoid_gsm_band_to_sys_band(sys_band_T band)
{
   switch(band)
   {
...
void rr_band_avoid_send_cxm_blist_rsp(uint16 blist_id )
{
  ...
  result = rr_msgr_send(&blist_rsp.msg_hdr, sizeof(blist_rsp));
  if (result != E_SUCCESS)
  {
...
void rr_band_avoid_send_cxm_freq_ind(frequency_list_T *freq )
{
  ...
  {    
    freq_ind.serv_freq_data.freq_list[i].frequency = rr_band_avoid_arfcn_to_centre_freq_kHz(freq->channel[i]);
    freq_ind.serv_freq_data.freq_list[i].band = rr_band_avoid_gsm_band_to_sys_band(freq->channel[i].band);
  ...
  result = rr_msgr_send(&freq_ind.msg_hdr, sizeof(freq_ind));
  if (result != E_SUCCESS)
  {
    MSG_GERAN_ERROR_1_G("rr_msgr_send failed: %d", result);
  }
 ...
 * \return void
 */
void rr_band_avoid_meas_rpt_sent_send_imsg( )
{
