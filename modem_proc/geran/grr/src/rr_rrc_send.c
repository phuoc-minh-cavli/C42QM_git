...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_rrc_send.c#6 $
...
#define RRC_DS_INSERT_AS_ID(msg) {msg.as_id = SYS_MODEM_AS_ID_1;}
...
)
{
  ...
  rr_show_output_message("RRC", NULL_PTR, MS_RRC_RR, rrc_id);
  rr_debug_log_rrc_rr_omsg(rrc_cmd_ptr);
  ...
  for (i = 0; i < candidate_db_ptr->num_frequencies && gsm_arfcn_list->size < MAX_RR_RRC_GSM_CELL_LIST_SIZE; i++)
  {
        ...
        (entry->type == RR_CANDIDATE_BCCH_WITH_DECODE_FAILURES))
    {
      MSG_GERAN_LOW_3_G("Adding cell (%d, %d) to GSM cell list, index %d",
        entry->meas.arfcn_num,
        entry->meas.arfcn_band,
        gsm_arfcn_list->size);

      gsm_arfcn_list->cells[gsm_arfcn_list->size].num = entry->meas.arfcn_num;
      gsm_arfcn_list->cells[gsm_arfcn_list->size].band = entry->meas.arfcn_band;
...
)
{
  ...
  rr_send_message_to_rrc(rrc_cmd);
}
...
void rr_send_rrc_interrat_cc_order_abort_req( )
{
  rrc_rr_cmd_type *rrc_cmd = NULL;
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION
  rr_build_gsm_arfcn_list(&(rrc_cmd->cmd.interrat_plmn_srch_req.gsm_cell_list));
#endif /* FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION */
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION
    rr_build_gsm_arfcn_list(&(rrc_cmd->cmd.interrat_redirect_rej.gsm_cell_list));
#endif /* FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION */
  ...
  for (i = 0; (i < available_plmn_db->num_plmn) && (i < SYS_PLMN_LIST_MAX_LENGTH); i++)
  {
    ...
    if (rr_w2g_oos_search_in_progress())
    {
      for (j = 0; j < available_plmn_db->plmn_info[i].num_frequencies; j++)
      {
        ...
        if (mm_rr_plmn_lai_forbidden_per_subs(inter_task_lai, GERAN_ACCESS_STRATUM_ID_1))
#else
        ...
        if (rr_is_forbidden_LAI(&available_plmn_db->plmn_info[i].cell_info[j].lai))
        {
          continue;
        }
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION
  rr_build_gsm_arfcn_list(&rrc_cmd->cmd.interrat_plmn_srch_cnf.gsm_cell_list);
#endif /* FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION */
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION
  rr_build_gsm_arfcn_list(&rrc_cmd->cmd.interrat_redirect_req.gsm_cell_list);
#endif /* FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION */

  rrc_cmd->cmd.interrat_redirect_req.rat_pri_list_info = *rat_pri_list_info;

#ifdef FEATURE_LTE
  rr_dedicated_priority_info_get(&rrc_cmd->cmd.interrat_redirect_req.dedicated_priority_info);
#endif /* FEATURE_LTE */
...
)
{
  ...
  if (rrc_cmd_ptr)
  {
    ...
    if (rr_individual_priorities_valid())
    {
      ...
      rrc_cmd_ptr->cmd.dedicated_priorities_rsp.plmn = rr_dedicated_priority_info_get_plmn();
      rr_dedicated_priority_info_get(
