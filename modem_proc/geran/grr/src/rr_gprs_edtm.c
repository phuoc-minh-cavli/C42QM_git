...
*** TITLE
***
*** $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_gprs_edtm.c#3 $
***
*** DESCRIPTION
***
...
static edtm_data_t rr_gprs_edtm_data =
             INITIAL_VALUE(EDTM_DATA_INIT) ;


#define EDTM_STATE_TRANSITION(vs,cs) if ((vs) != (cs))\
{\
  rr_gprs_edtm_log_state_change((vs),(cs));\
  (vs) = (cs);\
}
 ...
 * \return edtm_data_t*
 */
static edtm_data_t *rr_gprs_edtm_get_data_ptr( void )
{
  return &rr_gprs_edtm_data;
}
...
)
{
  ...
  edtm_data_t *rr_gprs_edtm_data_ptr = rr_gprs_edtm_get_data_ptr();
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_gprs_edtm_data_ptr = rr_gprs_edtm_get_data_ptr();
  ////RR_NULL_CHECK_FATAL(rr_gprs_edtm_data_ptr);
...
{
  gprs_cell_options_t *gco_ptr = rr_si_get_gprs_cell_options();
...
)
{
  ...
  ue_support = rr_get_nv_dtm_enabled() && rr_get_nv_edtm_enabled();
  nw_support = rr_gprs_edtm_available_in_cell();
...
)
{
  ...
  if ((rr_macinfo_get_mac_status()     == MAC_TRANSFER) &&
      (rr_mac_ps_access_granted()      == TRUE )        &&
      (rr_in_dedicated()               == TRUE)         &&
      (rr_power_scan_on_edtm_release() == FALSE)        &&
      (rr_gprs_edtm_supported()        == TRUE))
  {
...
)
{
  ...
  if ((rr_gprs_edtm_lai_has_changed() == TRUE) && (nmo == NETWORK_OP_MODE_1))
  {
    ps_rel_req = TRUE;
  }
  else
 ...
 *
 ****************************************************************************/
 static boolean rr_gprs_edtm_initiate_enhanced_release(void)
{
  boolean release_started;

  if (rr_gprs_edtm_enhanced_release_allowed())
  {
    /* set SI Status parameters */
    rr_gprs_edtm_set_sysinfo_status_params();

    /* initiate the enhanced release by sending the first */
    /* SI Status, and starting a retransmission timer */
    release_started = rr_gprs_send_sysinfo_status(EDTM_STATUS_TIMEOUT);
  }
  else
  {
...
)
{
  ...
  if ((rr_gprs_edtm_allowed_by_gmac() == TRUE) &&
      (rr_gprs_edtm_supported()       == TRUE))
  {
#ifdef FEATURE_GPRS_PS_HANDOVER
    if (rr_psho_in_progress() == TRUE)
    {
...
)
{
  ...
  if (rr_gprs_edtm_enhanced_establishment_allowed())
  {
    rr_cmd_bdy_type *rr_cmd_ptr;

    /* reset all the establishment data in this module */
    rr_gprs_edtm_reset_est_data(TRUE);

    /* there should be a saved RR_EST_REQ from MM - process it now */
    if (rr_process_saved_message(&rr_cmd_ptr) == EV_INPUT_MESSAGE)
    {
          ...
          (rr_cmd_ptr->message_header.message_id  == (byte)RR_EST_REQ))
      {
        ...
        if (EDTM_INVALID_ESTABLISHMENT_CAUSE(rr_est_req_ptr->establishment_cause))
        {
          ...
          (void)rr_save_msg_for_edtm(rr_cmd_ptr);
      ...
      //Replace memcpy with memscpy
          ...
          geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_EDTM, TRUE);
          ...
          (void)rr_timer_start(T3196, T3196_GUARD_TIMEOUT);

          /* and then ask GMAC to transmit the Pkt CS Request */
          rr_send_grr_mac_edtm_pkt_cs_est_req(access_ref);
        }
      }
    }
...
)
{
  ...
  if (rr_timer_cancel(GRR_SYS_INFO_STATUS_TIMER_1))
  {
  ...
  if (rr_gprs_edtm_data_ptr->final_status_sent == FALSE)
  {
    ...
    {
      (void)rr_timer_start(GRR_SYS_INFO_STATUS_TIMER_1, EDTM_STATUS_TIMEOUT);
    }
    ...
    (void)rr_timer_start(GRR_SYS_INFO_STATUS_TIMER_2, 1);
  }
...
)
{
      ...
      (pkt_msg_type == (uint8)PACKET_SI_DATA))
  {
    ...
    if (rr_gprs_edtm_check_sys_info_complete())
    {
      rr_gprs_edtm_send_final_sys_info_status();
    }
  }
 ...
 *
 ****************************************************************************/
uint32 rr_gprs_edtm_retrieve_bsi_status_mask(
...
)
{
  ...
  if (rr_gprs_edtm_data_ptr->pending_l3_msg_ptr != NULL)
  {
    ...
    //Replace mecpy with memscpy
      ...
      msg_saved = rr_save_msg_for_edtm(rr_cmd_ptr);
...
)
{
  ...
  if (rr_ce_allow_cs_connection(rr_gprs_edtm_data_ptr->establishment_cause))
  {
    rr_cmd_bdy_type *rr_cmd_ptr;
    uint16          msg_len;
    ...
    if (rr_cmd_ptr != NULL)
    {
      ...
      if (restore_type == RESTORE_AS_SAVED_MSG)
      {
        ...
        bRestored = rr_save_msg_for_edtm(rr_cmd_ptr);
      }
      else
      {
...
)
{
  ...
  if (rr_gprs_edtm_data_ptr->pending_l3_msg_typ == IMMEDIATE_ASSIGNMENT)
  {
    ...
    /*********************************************************/
    first_IE_ptr = rr_get_first_ie_of_message(IMMEDIATE_ASSIGNMENT);
...
)
{
  ...
  else if (rr_gprs_edtm_data_ptr->pending_l3_msg_typ != INVALID_L3_MSG_TYPE)
  {
    ...
    {
      decoded_ok = rr_gprs_edtm_validate_pending_ccch_message();
    }
    else
    {
      decoded_ok = FALSE;
...
)
{
  ...
  switch(dl_data_ind_ptr->layer3_message[1])
  {
    ...
    case IMMEDIATE_ASSIGNMENT_REJECT:
      ...
      else if (rr_timer_is_running(T3122) == TRUE)
      {
...
/*lint -restore*/
  ...
  if (store_msg == TRUE)
  {
    ...
    //Replace memcpy with memscpy
      ...
      return_event = rr_gprs_edtm_parse_pending_message();
    }
    else /* memory allocation has failed */
    {
      ...
      rr_gprs_edtm_reset_est_data(FALSE);
...
)
{
  ...
  if (error_cause == RR_NORMAL_EVENT)
  {
    ...
    if (rr_gprs_edtm_available_in_cell() == FALSE)
    {
...
)
{
  if (tmsi_ptr != NULL)
  {
    rr_siminfo_get_tmsi((byte *)tmsi_ptr,destsize);
...
)
{
  ...
  if (rr_gprs_edtm_tmsi_valid(&local_tmsi,sizeof(local_tmsi)) == TRUE)
  {
  ...
  else if (rr_siminfo_get_imsi(local_id, sizeof(local_id)) == FALSE)
  {
  ...
  rr_ce_edtm_set_paged_identity(local_id);
  ...
  return(rr_gprs_edtm_enhanced_establishment_allowed());
}
...
)
{
  ...
  (void)rr_gprs_edtm_page_response_allowed();
...
)
{
  ...
  if (rr_event != EV_NO_EVENT)
  {
    if (rr_event != EV_RESET_SOFTWARE)
    {
      ...
      rr_ce_set_edtm_access_parameters(rr_gprs_edtm_data_ptr->establishment_cause);
    }

    (void)rr_connection_establishment_control(rr_event, NULL);
  }
...
)
{
  ...
  (void)rr_gprs_edtm_control(EV_RR_EST_CNF, NULL);
  ...
  if (new_msg_ptr->message_header.message_set == MS_MM_RR)
  {
    ...
    else
    {
      if (new_msg_ptr->message_header.message_id == (byte)RR_PLMN_LIST_REQ)
      {
        ...
        rr_send_plmn_list_cnf(NULL, MM_AS_ABORTED, req_ptr);
      }
...
)
{
  ...
  if (rr_gprs_edtm_data_ptr->cs_request_sent == TRUE)
  {
    ...
    return_event = rr_gprs_edtm_control(next_event, NULL);
  }
  else
  {
...
)
{
  ...
  if (pkt_cs_release_ptr != NULL)
  {
    ...
    {
      return_event = EV_NO_EVENT;
      rr_gprs_edtm_data_ptr->cs_release_started = rr_gprs_edtm_initiate_enhanced_release();
...
)
{
  ...
  rr_gprs_edtm_cache_dtm_release(NULL);
...
)
{
  ...
  if (mode == RELEASE_MODE_EDTM)
  {
    ...
    else
    {
      ...
      (void)rr_dtm_control(EV_EDTM_RELEASE_ABORTED, NULL);
    }
  }
  else
  {
...
)
{
  if (message_id == MPH_IMMED_ASSIGN_CNF)
  {
    ...
    rr_gprs_edtm_store_cell_parameters();
  }
...
)
{
      ...
      (EDTM_CS_ESTABLISHMENT != new_state))
  {
    ...
    geran_api_set_tuneaway_blocked(GERAN_API_CLIENT_EDTM, FALSE);
  }
...
)
{
  ...
  if (event_in == EV_INPUT_MESSAGE)
  {
    if ( (message_set == MS_RR_L2)           &&
         ...
         (message_ptr != NULL) )
    {
      event_in = rr_gprs_edtm_process_dl_data_ind((dl_data_ind_T *)message_ptr);
    }
  ...
  {
    EDTM_STATE_TRANSITION(rr_gprs_edtm_data_ptr->state, EDTM_NULL);
    event_in = EV_NO_EVENT;
  }
...
/****************************************************************************/
      case EDTM_NULL:
        ...
        if (event_in == EV_EDTM_DTM_ASSIGNMENT_RECEIVED)
        {
          ...
          if (rr_gprs_edtm_page_response_allowed() == TRUE)
          {
            MSG_GERAN_MED_0_G("Unsolicited DTM Assignment: respond to EDTM paging");

            /* transition to the DTM establishment state */
            EDTM_STATE_TRANSITION(rr_gprs_edtm_data_ptr->state, EDTM_DTM_ESTABLISHMENT);

            /* and handle the response there */
            new_event = rr_gprs_edtm_respond_to_dtm_assignment();
        ...
        else if (event_in == EV_EDTM_IMM_ASSIGNMENT_RECEIVED)
        {
          ...
          if ((rr_gprs_edtm_page_response_allowed()     == TRUE) &&
              (rr_gprs_edtm_save_pending_ccch_message() == TRUE))
          {
            ...
            rr_gprs_edtm_connection_event(event_in);

            /* push DTM control into the CS establishment state */
            (void)rr_dtm_control(event_in, NULL);
        ...
        {
          if (rr_gprs_edtm_initiate_enhanced_establishment())
          {
            /* return a valid event to the caller */
            event_out = EV_EDTM_ESTABLISHMENT_STARTED;

            /* then transition to the establishment state */
            EDTM_STATE_TRANSITION(rr_gprs_edtm_data_ptr->state, EDTM_CS_ESTABLISHMENT);
          }
          else
          {
...
/****************************************************************************/
      case EDTM_CS_ESTABLISHMENT:
        ...
        {
          /* save the pending Immediate Assignment message */
          if (rr_gprs_edtm_save_pending_ccch_message())
          {
             /* disable system information refresh */
             rr_gprs_suspend_camped_sys_info_handling(FALSE);
        ...
        else if (event_in == EV_EDTM_IMM_ASSIGNMENT_REJECT_RECEIVED)
        {
          ...
          rr_send_abort_ind(RR_ABORT_IA_REJECT_RECEIVED, MM_RR_OTHER_FAILURE);
        ...
        else if (event_in == EV_EDTM_ESTABLISHMENT_REJECTED)
        {
          ...
          if (rr_gprs_edtm_restore_rr_est_req(RESTORE_AS_SAVED_MSG))
          {
            /* tell GRR-CONTROL to block PS access (it will then process the saved message) */
            rr_grr_send_imsg_no_ps_access_req(
              FALSE,   // del_all_pdus
        ...
        {
          /* try to restore the RR_EST_REQ that was saved earlier */
          if (rr_gprs_edtm_restore_rr_est_req(RESTORE_IN_RR_QUEUE))
          {
...
/****************************************************************************/
      case EDTM_DTM_ESTABLISHMENT:
        ...
        {
          new_event = rr_dtm_send_edtm_assignment_to_gmac();
        }
        /* EV_DTM_ASSIGNMENT_ERROR
        ...
        else if (event_in == EV_DTM_ASSIGNMENT_COMPLETE)
        {
          ...
          rr_gprs_edtm_connection_event(EV_EDTM_DTM_ASSIGNMENT_RECEIVED);
        ...
        else if (event_in == EV_MAC_IDLE)
        {
          ...
          rr_gprs_edtm_connection_event(EV_RESET_SOFTWARE);

          if (rr_is_control_state(RR_CONNECTION_PENDING))
          {
            /* push DTM control back to PS_ALLOWED */
            (void)rr_dtm_control(EV_EDTM_ESTABLISHMENT_ABORTED, NULL);
            ...
            rr_conn_set_release_cause(RELEASE_CAUSE_L2_LINK_RESET);
