...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sys_info_si.c#8 $ */
...
static rr_sys_info_si_data_t rr_sys_info_si_data =
 ...
 */
static rr_sys_info_si_data_t *rr_sys_info_si_get_data_ptr( void )
{
  return &rr_sys_info_si_data;
}
...
)
{
  ...
  rr_sys_info_si_data_ptr = rr_sys_info_si_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_sys_info_si_data_ptr);

  if ( rr_ce_get_ota_page_mode() == PAGING_REORGANIZATION )
  {
 ...
 * \return uint16
 */
static uint32 bsi_determine_messages_missing_after_camping(
  gprs_scell_info_t *cell_info,
  const boolean include_3G
   
)
{
  uint32 messages_missing = MASK_SI_NONE;
 ...
 * \return uint16
 */
static uint32 get_remaining_si_bitmask(
  gprs_scell_info_t *cell_info
)
{
  uint32 remaining_si_bitmask;
...
static rr_l1_si_type_T  map_rr_si_bitmask_to_l1_si_bitmask(
  uint32 rr_si_bitmask,
  uint32 si_bcch_ext_bitmask
)
{
  ...
  if ( rr_si_bitmask & MASK_SI_9 )
  {
    l1_si_bitmask |= (uint32)RR_L1_SI_TYPE_9;
  }
  ...
  if ( rr_si_bitmask & MASK_SI_21 )
  {
    if ( si_bcch_ext_bitmask & MASK_SI_21 )
    {
      l1_si_bitmask |= (uint32)RR_L1_SI_TYPE_21_EXT;
    }
    else
    {
      l1_si_bitmask |= (uint32)RR_L1_SI_TYPE_21_NORM;
    }
  }
...
)
{
  ...
  if (si_required != MASK_SI_NONE)
  {
    ...
    {
      uint32 new_si_bitmask = MASK_SI_NONE;
      uint8 i = 0;
  ...
  rr_send_mph_read_sys_info_req(l1_si_required, rr_gprs_get_scell_info()->gsm.si_sched_info);
...
)
{
  ...
  rr_gprs_start_sys_info_acquisition_timer(GRR_TIMEOUT_BCCH_SI_ACQUISITION);
...
)
{
  ...
  */
  siev = rr_gprs_camped_sys_info_acquisition_complete();
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
    /*If in PEO mode inform RMC that SIs has been acquired*/
    if (rr_ue_in_peo_mode())
    {
      rr_resel_rmc_send_imsg_bsi_acquition_complete_ind();
    }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
...
)
{
  ...
  uint32             required_mask = MASK_FULL_BSI_ACQUISITION;
  uint32             low_pri;
  ...
  */
  ...
  {
    uint32 not_required_mask = SI_TYPE_TO_MASK(last_si);
...
{
  setup_full_refresh_acquisition();
  continue_l1_bsi_acquisition();
...
)
{
  ...
  {
    setup_si13_refresh_acquisition();
  }
  else
  {
    setup_full_refresh_acquisition();
...
)
{
  ...
  pcell_info = rr_gprs_get_pcell_info();
       ...
       pcell_info->bcch_sys_info.si13_expected_flag )
  {
    ...
    rr_cell_acq_send_imsg_gprs_bsi_received_ind();
    ...
    {
      rr_send_priority_imsg_rr_event_ind(RR_EV_GPRS_SI13_RECEIVED);
...
)
{
  ...
  scell_info = rr_gprs_get_scell_info();
  ...
  Handler is inactive
  */
  rr_event = rr_gprs_bsi_acquisition_control(EV_SI_SYS_INFO_MESSAGE_RECEIVED, NULL);
  ...
  */
  ...
  {
    rr_send_priority_imsg_rr_event_ind(rr_event);
  }
...
)
{
  ...
  decoded_msg->si_change_alt = si13.si_change_alt; 
  
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
  if (si13.peo_params.peo_supported_flag)
  {
     decoded_msg->peo_params    = si13.peo_params;
     MSG_GERAN_HIGH_4_G("PEO supported by N/W, DSC= %d, C1_DELTA_MIN = %d,, C1_DELTA_MAX = %d, RCC= %d",
                           decoded_msg->peo_params.peo_dsc,
                           decoded_msg->peo_params.c1_delta_min,
                           decoded_msg->peo_params.c1_delta_max,
                           decoded_msg->peo_params.rcc);

  }
  else
  {
    MSG_GERAN_HIGH_0_G("Release 13, PEO is not supported");
    memset(&(decoded_msg->peo_params), 0, sizeof(peo_params_t));
  }
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
 ...
 * \return uint16
 */
static uint32 process_bcch_change_mark(
...
)
{
  ...
  uint32 messages_changed = 0;   /* Assume no messages are changed */
  uint32 messages_required = 0;  /* Assume no messages are changed */
  ...
  if ( bcch_change_mark == ( ( stored_bcch_change_mark + 1 ) & 7 ) )
  {
    ...
    */
    ...
    if ( si_change_field == 0x00 )
    {
      ...
      /* Clear SI9 and 21 from received list. They will be read based on SI3 content */
      CLEAR_BIT(MASK_SI_9|MASK_SI_21, cell_info->gsm.update_message_received);
 ...
 */
static uint32 check_if_bcch_change_mark_changed(
  gprs_scell_info_t *cell_info,
  uint8 bcch_change_mark,
  uint8 si_change_field
)
{
  uint32 messages_required = 0;
...
)
{
  ...
  if ( scell_info->bcch_sys_info.si13_data_valid_flag )
  {
    ...
    if ( messages_changed != MASK_SI_NONE )
    {
      ...
      rr_send_sysinfo_status_on_bcch_change_mark_update(scell_info);
    }
  }
...
)
{
...
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION  
  boolean                     peo_param_change = FALSE;
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
  ...
  if ( scell_info->bcch_sys_info.si13_data_valid_flag )
  {
    ...
    if ( si13_data->mobile_allocation_status == IE_PRESENT )
    {
      ...
      {
        MSG_GERAN_LOW_0_G("SI13 mobile alloc changed");
    ...
    */
    ...
    scell_info->bcch_sys_info.si_change_alt       = si13_data->si_change_alt;
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION

    if (scell_info->bcch_sys_info.peo_params.peo_supported_flag != si13_data->peo_params.peo_supported_flag )
    {
      MSG_GERAN_HIGH_0_G("N/W peo support changed..update Gl1");
      peo_param_change = TRUE;            
    }

    if (si13_data->peo_params.peo_supported_flag)
    {
      scell_info->bcch_sys_info.peo_params          = si13_data->peo_params;
      MSG_GERAN_HIGH_0_G("Release 13, PEO is supported");
      if (rr_is_camped())
      {
        if (rr_process_rcc_or_bcch_change(scell_info->bcch_sys_info.peo_params.rcc,INVALID_PEO_BCCH_CHANGE_MARK))
        {
          peo_param_change = FALSE;
          update_l1 = FALSE;
        }
      }
    }
    ...
    {
      MSG_GERAN_HIGH_0_G("Release 13, PEO is not supported");
      memset(&(scell_info->bcch_sys_info.peo_params), 0, sizeof(peo_params_t));
    }
...
)
{
  ...
  if (si2q_extend > GRR_TIMEOUT_BCCH_SI_ACQUISITION)
  {
    ...
    if (rr_extend_timer(GRR_SYS_INFO_ACQUISITION_TIMER, si2q_extend))
    {
 ...
 */
static uint32 rr_si_messages_on_bcch_ext(gprs_scell_info_t *cell_info_ptr)
{
...
)
{
  ...
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_13);
  ...
  if (current_IE)
  {
    ...
    if (current_IE->start_IE)
    {
      ...
      error_code = decode_si13_ro((void *)current_IE->start_IE, si13_data_ptr);
      ...
      if ( error_code == CSN_DECODE_NO_ERROR )
      {
        ...
        if ( rr_is_this_the_scell( bcch_arfcn ) )
        {
          ...
          process_si13_or_psi13(si13_data_ptr);
        ...
        /* required */
        if ( rr_is_this_the_pcell( bcch_arfcn ) )
        {
        ...
        if (rr_gprs_is_this_a_surround_ncell(bcch_arfcn))
        {
          MSG_GERAN_HIGH_1_G("SI13 from ncell (%d) - ignored",(int)bcch_arfcn.num);
        }
        else
        {
          MSG_GERAN_LOW_1_G("SI13 from unknown cell (%d)",(int)bcch_arfcn.num);
        }
      }
      else
      {
...
)
{
  ...
  */
  ...
  {
    rr_gprs_pcco_apply_ba_gprs_deltas();
  }
  ...
  {
    instances_T *si2q_inst_ptr = rr_get_instances_ptr(SI2Q_INSTANCE);
...
)
{
  ...
  rr_gprs_build_ba_gprs_from_si2(rr_gprs_get_scell_info());
...
)
{
  ...
  */
  #ifdef FEATURE_GSM_EDTM
  if ( rr_in_dedicated() && (!rr_gprs_edtm_enhanced_release_started()) )
  #else
  if ( rr_in_dedicated() )
  #endif /* FEATURE_GSM_EDTM */
  {
...
#endif /* FEATURE_GSM_DTM */
  {
    process_si13_or_psi13(decoded_psi13);
    ...
    siev = bsi_acquisition_control(SIEV_PSI13_RECEIVED, NULL);

    if ( siev != SIEV_NO_EVENT )
    {
      rr_send_priority_imsg_rr_event_ind(grr_siev_to_rrev(siev));
    }
  }
...
)
{
  ...
  if ( si13_data->mobile_allocation_status == IE_PRESENT )
  {
    MSG_GERAN_LOW_0_G("SI13 mobile alloc present");
  ...
  /* Store Rel13 paramtre*/
  cell_info->bcch_sys_info.si_change_alt       = si13_data->si_change_alt;
#ifdef FEATURE_GERAN_POWER_EFFICIENT_OPERATION 
  if (si13_data->peo_params.peo_supported_flag)
  {
    cell_info->bcch_sys_info.peo_params		   = si13_data->peo_params;
  }
  else
  {
    MSG_GERAN_HIGH_0_G("Release 13, PEO is not supported");
    memset(&(cell_info->bcch_sys_info.peo_params), 0, sizeof(peo_params_t));
  }
...
)
{
  if (rr_is_this_the_pcell(bcch_arfcn))
  {
    bsi_non_camped_post_processing();
  }
  else
  if ( rr_is_this_the_scell(bcch_arfcn))
  {
    bsi_camped_post_processing(si_message_received);
  }
...
)
{
  ...
  event_out = bsi_acquisition_control(event_in, event_in_data);
...
)
{
  ...
  {
    event_out = bsi_acquisition_control(event_in, NULL);
  }
  else
  {
    MSG_GERAN_LOW_1_G("No SIEV for timer %d",timer_id);
  }
 ...
 */
static boolean bsi_paging_reorg_allowed( )
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
...
{
  (void) bsi_acquisition_control(SIEV_EXIT_PAGING_REORG, NULL);
...
{
  gprs_scell_info_t *scell_info = rr_gprs_get_scell_info();
  boolean            require_3G = rr_ms_3g_sys_info_mandatory();
  ...
  */
  ...
  bsi_set_messages_required_for_acquisition(
    bsi_determine_messages_missing_after_camping(scell_info, require_3G),// required
    MASK_SI_NONE,                                                                // low_priority
    FALSE                                                                       // reset_history
  );
  ...
  (void) bsi_acquisition_control(SIEV_ACTIVATE_SYS_INFO_ACQUISITION, NULL);
...
void bsi_update_messages_being_broadcast(
  ...
  uint32             added_bcch_norm,
  uint32             added_bcch_ext,
  uint32             removed
)
{
...
)
{
  ...
  {
    scell_info = rr_gprs_get_pcell_info();
  }
...
)
{
  ...
  /*****************************************************************/
  if (grr_get_l1_in_test_mode() && (event_in != SIEV_STOP_AND_RESET_SOFTWARE))
  {
    MSG_GERAN_MED_0_G("L1 in test mode, no BSI acquition allowed");
    return event_out;
  }
  /*****************************************************************/
  /* SIEV_INPUT_MESSAGE                                            */
  ...
  if ( event_in == SIEV_INPUT_MESSAGE )
  {
    ...
    event_in = bsi_common_signal_handler(event_in_data);
  }
    ...
    switch ( rr_sys_info_si_data_ptr->state )
    {
      case BSIST_ACQ_NOT_REQUESTED:
      {
        ...
        if ( event_in == SIEV_SYS_INFO_REFRESH_TIMER_EXPIRY )
        {
          ...
          if (rr_ue_in_peo_mode())
          {
            MSG_GERAN_MED_0_G("Acquiring SI (for periodic refresh or PEO-BCCH CHANGE detected)");
          }
          else
#endif /*FEATURE_GERAN_POWER_EFFICIENT_OPERATION*/
          {
        ...
        if ( event_in == SIEV_SYS_INFO_FULL_REFRESH )
        {
          ...
          start_l1_full_refresh_acquisition();
        ...
        if ( event_in == SIEV_SYS_INFO_MESSAGE_RECEIVED )
        {
          ...
          new_event = grr_align_sys_info_acquisition_with_cell_capabilities();
        ...
        if ( event_in == SIEV_PSI13_RECEIVED )
        {
          ...
          MSG_GERAN_LOW_0_G("PSI13 received from PACCH");
        ...
        {
          new_event = bsi_determine_action_after_activation();
        }
        ...
        if ( event_in == SIEV_ENTER_PAGING_REORG )
        {
          ...
          */
          rr_send_mph_read_sys_info_req((rr_l1_si_type_T)rr_l1_mask, rr_gprs_get_scell_info()->gsm.si_sched_info);
      ...
      case BSIST_ACQ_REQUESTED:
      {
        ...
        if ( event_in == SIEV_SYS_INFO_MESSAGE_RECEIVED )
        {
          ...
          if ( new_event == SIEV_NO_EVENT )
          {
            if ( check_if_bsi_acquisition_complete() )
            {
        ...
        if ( event_in == SIEV_SYS_INFO_ACQUISITION_COMPLETE )
        {
          ...
          */
          grr_siev_t siev = bsi_acquisition_complete();
        ...
        {
          gprs_scell_info_t * cell_info = rr_gprs_get_scell_info();
          uint32 remaining_si = get_remaining_si_bitmask( cell_info );
          ...
          if ( remaining_si & MASK_SI_13 )
          {
            ...
            Setup SI masks to perform full SI refresh
            */
            bsi_reset_message_tracking_data();
            setup_full_refresh_acquisition();
        ...
        {
          rr_gprs_start_sys_info_retry_timer(GRR_TIMEOUT_1_SECOND);
        }
      ...
      */
      ...
      case BSIST_ACQ_SUSPENDED:
      {
        ...
        if ( event_in == SIEV_SYS_INFO_RETRY_TIMER_EXPIRY )
        {
          ...
          if ( new_event != SIEV_NO_EVENT )
          {
            rr_gprs_nsi_notify_scell_si_acquisition_required();
          }
        }
             ...
             (event_in == SIEV_SI13_REMOVED) )
        {
          ...
          {
            rr_gprs_invalidate_sys_info_during_cs_conn(RR_SI_INCONSISTENT);
          }
          else
      ...
      default:
        ...
        MSG_GERAN_LOW_1_G("In unknown state (%d)",rr_sys_info_si_data_ptr->state);
        break;
    }
...
)
{
  ...
  {
    cell = rr_gprs_get_scell_info();
  }
  else if (rr_is_this_the_pcell(channel_number))
  {
    cell = rr_gprs_get_pcell_info();
  }
  ...
  if (cell)
  {
    ...
    if (params_mods != NULL && *params_mods != NULL)
    {
      rr_nc_meas_params_merge(nc_meas_params, &(*params_mods)->nc_meas_params);
      rr_gprs_meas_params_merge(gprs_meas_params, &(*params_mods)->gprs_meas_params);
#ifdef FEATURE_WCDMA
      rr_gprs_3g_meas_params_merge(gprs_3g_meas_params,  &(*params_mods)->gprs_3g_meas_params);
#endif /* FEATURE_WCDMA */
      rr_gprs_merge_nc_nacc_into_cell_nacc_params(gprs_cell_options, &(*params_mods)->nacc_params);
      ...
      {
        MSG_GERAN_LOW_0_G("Using UTRAN Priority Parameters from PMO");
    ...
    {
      rr_nc_meas_params_merge(nc_meas_params, &(*pcco_params_mods)->nc_meas_params);
      rr_gprs_meas_params_merge(gprs_meas_params, &(*pcco_params_mods)->gprs_meas_params);
#ifdef FEATURE_WCDMA
      rr_gprs_3g_meas_params_merge(gprs_3g_meas_params,  &(*pcco_params_mods)->gprs_3g_meas_params);
#endif /* FEATURE_WCDMA */
      rr_gprs_merge_nc_nacc_into_cell_nacc_params(gprs_cell_options, &(*pcco_params_mods)->nacc_params);
    }
  }
}
...
)
{
  ...
  {
    MSG_GERAN_LOW_0("E-UTRAN Measurement Parameters present");
  ...
  {
    MSG_GERAN_LOW_0("GPRS E-UTRAN Measurement Parameters present");
...
)
{
  ...
  if (si2q_inst_ptr != NULL)
  {
    for (i = 0; i < MAX_INSTANCES; i++)
    {
      ...
      if ( si2quater_instance->si2q_mi.si2q_params.ccn_support_flag && rr_gprs_camped_on_ccch() )
      {
        rr_gprs_merge_ccn_support_into_ba_list(
      &si2quater_instance->si2q_mi.si2q_params.ccn_support_desc
        );
      ...
      if (si2quater_instance->si2q_mi.si2q_params.priority_and_eutran_params_desc != NULL)
      {
        ...
        {
          MSG_GERAN_HIGH_0_G("Merging Serving Cell Priority Parameters from SI2quater to SCELL");
        ...
        {
          MSG_GERAN_HIGH_0_G("Merging UTRAN Priority Parameters from SI2quater to SCELL");
          rr_utran_priority_params_merge_mods(
            &cell->utran_priority_params,
        ...
        {
          MSG_GERAN_HIGH_0_G("Merging E-UTRAN Parameters from SI2quater to SCELL");
...
#ifdef FEATURE_FAST_G2L_PSEUDO_SI2Q
    if ( rr_is_pseudo_si2q_data_used() && cell->lte_neighbour_list.count > 0 )
    {
      ...
      rr_restore_default_serv_utran_pseudo_si2q_params(cell);  
    }
#endif /* FEATURE_FAST_G2L_PSEUDO_SI2Q */
...
)
{
  ...
  if (rr_gprs_camped_on_ccch() && update_nc_meas_control)
  {
    rr_send_priority_imsg_rr_event_ind(RR_EV_MEASUREMENT_PARAMS_UPDATED);
  }
}
...
)
{
  ...
  if ((cell != NULL) && (si2q_inst_ptr != NULL))
  {
    ...
    for (i = 0; i < MAX_SIZE_OF_GSM_NEIGHBOUR_CELL_LIST; i++)
    {
      ...
      {
        MSG_GERAN_LOW_1_G("GPRS BSIC Description for index: %d", i);
      }

      for (j = 0; j < cell->gprs_bsic_description.bsics[i].num_of_bsics; j++)
      {
        MSG_GERAN_LOW_3_G("  (%d, %d)",
    ...
    {
      rr_gprs_build_ba_gprs_from_si2(cell);
    }
  }
  MSG_GERAN_HIGH_0_G("Completed decoding of GPRS BSIC Description");
}
...
)
{
  ...
  {
    ncell = rr_gprs_get_ncell_info(ba_list->BCCH_ARFCN[index], NULL);
...
{
  // Obtain a pointer to the module data
  rr_sys_info_si_data_t *rr_sys_info_si_data_ptr = rr_sys_info_si_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_sys_info_si_data_ptr);
...
{
  rr_l1_si_type_T l1_si_required = RR_L1_SI_TYPE_NONE;
  uint32 si_messages_on_bcch_ext = rr_si_messages_on_bcch_ext(rr_gprs_get_pcell_info());
  ...
  rr_send_mph_read_sys_info_req(l1_si_required, rr_gprs_get_pcell_info()->gsm.si_sched_info);
 ...
 */
uint32 rr_si_messages_not_broadcast(gprs_scell_info_t *cell_info_ptr)
{
  //RR_NULL_CHECK_RETURN_PARAM(cell_info_ptr, MASK_SI_NONE);
 ...
 * \param si_bitmask (in)
 */
void rr_si_print_bitmask(char *descr_text, uint32 si_bitmask)
{
//#ifdef PERLUTF
#if 1
  if (descr_text != NULL)
  ...
  MSG_GERAN_HIGH_1_G("SI mask 0x%x", (int)si_bitmask);
 ...
 */
uint32 rr_si_calc_sys_info_acquisition_time(uint32 si_required, uint32 si_on_bcch_ext)
{
