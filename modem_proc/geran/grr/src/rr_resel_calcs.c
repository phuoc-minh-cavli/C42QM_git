...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_resel_calcs.c#8 $ */
...
#ifdef FEATURE_GSM_SCE_REDUCED_NCELL_LIST
#define RECVD_REDUCED_NCELL_LIST_SIZE   6
...
#define C2_PS_SUPPORT_THRESHOLD 6  //System recommended -105 dbm rxlev
#endif
...
/**
  ...
  @see
  */
static best_six_ncells_t  best_six_ncells;

/*----------------------------------------------------------------------------
 * Local Variables
 *--------------------------------------------------------------------------*/

static rr_gprs_resel_calc_data_t  rr_resel_calcs_data;
...
/* added to the list                                                   */
/***********************************************************************/
static gprs_reselection_structure_T gprs_reselection_list;

/* This type will hold a list of potential target cells for reselection */
static gprs_better_cell_list_T better_cell_list;
 ...
 */
static rr_gprs_resel_calc_data_t *rr_resel_calcs_get_data_ptr( void )
{
  return &rr_resel_calcs_data;
}
...
)
{
  ...
  rr_resel_calcs_data_ptr = rr_resel_calcs_get_data_ptr();
  ...
  if (rr_gprs_camped_on_ccch() && local_gprs_reselection_list_ptr->transfer_serving_snr_is_bad)
  {
    ...
    */
    ...
    while (local_gprs_reselection_list_ptr->no_of_entries && rr_gprs_is_snr_counter_zero())
    {
      ...
      resel_candidate = rr_resel_best_six_find(ptr_best_ncell_c2->BCCH_ARFCN, NULL);
      ...
      else if (rr_cell_forbidden(ptr_best_ncell_c2->BCCH_ARFCN))
      {
      ...
      else if (rr_timer_is_running(resel_candidate->reselection_info.five_second_timer_id))
      {
    ...
    if (!force_bad_scell_reselection)
    {
      ...
      *local_gprs_reselection_list_ptr = temp_ncell_scell_reselection_values;
      ...
      while ( local_gprs_reselection_list_ptr->no_of_entries && rr_gprs_is_snr_counter_zero() )
      {
        ...
        ptr_bcch_surround_db = rr_gprs_get_surround_ncell_sys_info(ptr_best_ncell_c2->BCCH_ARFCN);
        ...
        /* Make sure that the cell is suitable before bothering to test it further */
        else if ( rr_gprs_resel_suitable_cell( ptr_bcch_surround_db ) && ( ptr_best_ncell_c2->C1 > 0 || rr_allow_negative_c1(ptr_bcch_surround_db->gsm.location_area_identification.plmn_id) )
) {
          /*
          ...
          */
              ...
              (rr_same_lai(&ptr_bcch_surround_db->gsm.location_area_identification,&scell_info->gsm.location_area_identification)))
          {
            ...
            rr_log_set_reselection_trigger(RR_XFER_SNR_RESELECTION_TRIGGER);
            break;
          }
          else
...
)
{
  ...
  */
       ...
       better_ncell_index++)
  {
    ...
    {
      MSG_GERAN_MED_3_G("ncell ARFCN:%d, nC2:%d sc2:%d",
  ...
  MSG_GERAN_LOW_1_G("reduced_ncell_list: %d",reduced_ncell_list);
...
)
{
  ...
  // Obtain a pointer to the module data
  rr_gprs_resel_calc_data_t *rr_resel_calcs_data_ptr = rr_resel_calcs_get_data_ptr();
  //RR_NULL_CHECK_FATAL(rr_resel_calcs_data_ptr);
  ...
  */
  ...
  ncell_better = rr_gprs_test_for_slow_snr_reselection(hysteresis_adjust);
 ...
 *
 ****************************************************************************/
static boolean rr_gprs_check_for_bad_scell_in_idle(void)
{
      ...
      rr_gprs_is_dsc_below_max_value())
  {
    ...
    */
    ...
    if (rr_gprs_is_nC2_near_sC2_and_nC2inst_greater_than_sC2inst())
    {
      rr_gprs_init_bad_snr_counter();
      bad_scell_reselection = TRUE;

      rr_clear_all_best_six_snr_markers();
      rr_log_set_reselection_trigger(RR_FAST_SNR_RESELECTION_TRIGGER);
    }
    else
    {
    ...
    /***************************************************************************/
      ...
      if ( rr_gprs_is_nC2_greater_than_sC2() )
      {
        rr_gprs_init_bad_snr_counter();
        bad_scell_reselection = TRUE;

        rr_clear_all_best_six_snr_markers();
        rr_log_set_reselection_trigger(RR_SLOW_SNR_RESELECTION_TRIGGER);
      }
      else
      {
        /* restore the C2 values potentially stripped out of the reselection list by the
 ...
 *
 ****************************************************************************/
static boolean rr_gprs_check_scell_path_loss_GPRS(void)
{
  ...
  if (local_gprs_reselection_list_ptr->serving_values_valid == TRUE &&
    (local_gprs_reselection_list_ptr->serving_values.C1 < 0 && !rr_allow_negative_c1(rr_gprs_get_scell_plmn_id())))
  {
 ...
 *
 ****************************************************************************/
static boolean rr_gprs_check_scell_path_loss_GSM(void)
{
  boolean                        path_loss_too_high = FALSE;
  gprs_scell_info_t             *scell_info_ptr = rr_gprs_get_scell_info();
  gprs_reselection_structure_T  *local_gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();

  if ( local_gprs_reselection_list_ptr->serving_values.C1 < 0 && !rr_allow_negative_c1(rr_gprs_get_scell_plmn_id()))
  {
#ifdef FEATURE_GERAN_EXTENDED_DRX 
    if (rr_get_edrx_indicator())
    {      
      MSG_GERAN_HIGH_0_G("Serving cell C1 too low, force reselection immediately for eDRX case");
      path_loss_too_high = TRUE;      
    }
    else
#endif /*FEATURE_GERAN_EXTENDED_DRX*/
    if (rr_timer_get_state(RR_SERV_FIVE_SEC_C1_TIMER) == RR_TIMER_STOPPED)
    {
      /*Serving C1 below 0, must start 5s timer*/
      scell_info_ptr->gsm.reselection_info.five_second_timer_id = RR_SERV_FIVE_SEC_C1_TIMER;
      (void)rr_timer_start(RR_SERV_FIVE_SEC_C1_TIMER, RR_FIVE_SEC_TIMEOUT);
      MSG_GERAN_MED_0_G("Serv 5 sec timer started");
    }
    else if (rr_timer_get_state(RR_SERV_FIVE_SEC_C1_TIMER) == RR_TIMER_EXPIRED)
    {
  ...
  else
  {
    if (rr_timer_get_state(RR_SERV_FIVE_SEC_C1_TIMER) != RR_TIMER_STOPPED)
    {
      ...
      */
      scell_info_ptr->gsm.reselection_info.five_second_timer_id = RR_SERV_FIVE_SEC_C1_TIMER;
      (void)rr_timer_cancel(RR_SERV_FIVE_SEC_C1_TIMER);
...
/***************************************************************/
  ...
  {
    rr_mac_status_t mac_status = rr_macinfo_get_mac_status();
...
)
{
  ...
  resel_candidate = rr_resel_best_six_find(BCCH_ARFCN, NULL);
  ...
  ptr_bcch_surround_db = rr_gprs_get_surround_ncell_sys_info(BCCH_ARFCN);
  ...
  else if (rr_gprs_parameters_available() == TRUE)
  {
    /*************************************************/
    ...
    /* this ARFCN                                    */
    /*************************************************/
    if ((ptr_pbcch_ncell_info = rr_gprs_get_pmo_pcco_ncell_sys_info(BCCH_ARFCN, &BSIC)) != NULL)
    {
      /************************************************************************/
      ...
      if (scell_info->bcch_sys_info.pmo_gprs_serving_cell_params.hcs_params_present)
      {
        C31 = rr_gprs_calculate_C31(
          ...
          RXLEV_average,
          rr_timer_get_state(resel_candidate->penalty_timer_id)
        );
      }
      else
      {
...
)
{
  ...
  gprs_reselection_structure_T *local_gprs_reselection_list_ptr = rr_get_gprs_reselection_list_ptr();
  best_six_ncells_t            *local_best_six_ncells_ptr = rr_get_best_six_ncells_ptr();
  ...
  for (cell_index = 0; cell_index < p_best_six_info->no_of_entries; cell_index++)
  {
    ...
    if (rr_gprs_get_best_six_by_arfcn(meas_chan_info_ptr->BCCH_ARFCN, &recieved_bsic) != NULL)
    {
       /*Add the channel to the list of cells that L1 is sync'ed to*/
       rr_gprs_add_sync_cell(BCCH_ARFCN);
    }
    else
    {
...
)
{
  ...
  *dest_gprs_reselection_values = *src_gprs_reselection_values;
  ...
  /****************************************************************/
  if ((ptr_surround_bcch = rr_gprs_get_surround_ncell_sys_info(src_gprs_reselection_values->BCCH_ARFCN)) != NULL)
  {
  ...
  /******************************************************************/
  else if ((ptr_surround_pbcch = rr_gprs_get_pmo_pcco_ncell_sys_info(src_gprs_reselection_values->BCCH_ARFCN,NULL)) != NULL)
  {
  ...
  {
    if (same_routing_area && (rr_gmminfo_get_gmm_status() == GMM_STANDBY_STATE))
    {
      /**********************************************************************/
    ...
    else if (same_routing_area && (rr_gmminfo_get_gmm_status() == GMM_READY_STATE))
    {
      /*********************************************************************/
    ...
    else if (!same_routing_area && (rr_gmminfo_get_gmm_status() != GMM_IDLE_STATE))
    {
      /********************************************************************************/
...
)
{
       ...
       sys_index++)
  {
    ...
    resel_candidate = rr_resel_best_six_find(surround_db_entry_ptr->gsm.BCCH_ARFCN, NULL);
...
#ifdef FEATURE_PS_PREFERRED_CELL
            ,local_gprs_reselection_list_ptr->surrounding_values[cell_index].RXLEV_average
#endif /* FEATURE_PS_PREFERRED_CELL */
          );

          /* Also the ncell must have C1 > 0 to be better */
          if (local_gprs_reselection_list_ptr->surrounding_values[cell_index].C1 < 0 && !rr_allow_negative_c1(surround_db_entry_ptr->gsm.location_area_identification.plmn_id))
          {
            surrounding_better = FALSE;
          }
    ...
    if (surrounding_better == TRUE)
    {
      ...
      if (rr_timer_get_state(resel_candidate->reselection_info.five_second_timer_id) == RR_TIMER_EXPIRED)
      {
        ...
        else if ( rr_cell_forbidden( surround_db_entry_ptr->gsm.BCCH_ARFCN ) )
        {
        ...
        else
        {
          ...
          if (grr_any_other_reselection_candidates(surround_db_entry_ptr))
          {
          ...
          else
          {
            ...
            /* reselection attempt.                                                     */
            /****************************************************************************/
            (void)rr_timer_cancel(resel_candidate->reselection_info.five_second_timer_id);
            ...
            /****************************************************************************/
            MSG_GERAN_MED_1_G("ncell better scell, %d in list, force reselection",
                    (int)(local_better_cell_list_ptr->no_of_entries));
               ...
               ) == RR_TIMER_STOPPED)
      {
        ...
        /************************************************************************/
        rr_gprs_start_five_second_timer(&resel_candidate->reselection_info);
      }
    }
    else
    {
      ...
      {
        rr_gprs_stop_five_second_timer(&resel_candidate->reselection_info);
      }
...
)
{
       ...
       index++)
  {
    ...
    if (ncell->C1 < 0 && !rr_allow_negative_c1(rr_gprs_get_scell_plmn_id()))
    {
    ...
    if (rr_cell_forbidden(ncell->BCCH_ARFCN))
    {
    ...
    if ((ptr_ncell_bcch_data = rr_gprs_get_surround_ncell_sys_info(ncell->BCCH_ARFCN)) != NULL)
    {
      if (!rr_gprs_resel_suitable_cell(ptr_ncell_bcch_data))
      {
...
)
{
  ...
  if (rr_gprs_parameters_available())
  {
    ...
    Path_Loss_Reselection = rr_gprs_check_scell_path_loss_GPRS();
  }
  else
  {
    /*Use the GSM path loss algorithm in GSM05.08 6.6.2*/
    Path_Loss_Reselection = rr_gprs_check_scell_path_loss_GSM();
  }
...
)
{
  ...
  if (rr_macinfo_get_mac_status() == MAC_TRANSFER)
  {
    ...
    #error code not present
#else
    bad_scell_reselection = rr_gprs_check_for_bad_scell_in_xfer();
#endif
  }
  else
  {
    ...
    #error code not present
#else
    bad_scell_reselection = rr_gprs_check_for_bad_scell_in_idle();
#endif
  }
...
)
{
  ...
  if (rr_gprs_parameters_available())
  {
    rr_gprs_store_C31_C32_serving_measurement(RXLEV, RXQUAL);
  }
  else
  {
    rr_gprs_store_C1_C2_serving_measurement(RXLEV, RXQUAL);
    //transfer SNR
...
)
{
  ...
  {
    MSG_GERAN_LOW_1_G("MPH_SURROUND_MEAS_IND: %d entries rcvd, capping at 6",
  ...
  if ( rr_gprs_camped_on_ccch() || rr_gsm_camped_on_ccch() )
  {
    ...
    rr_gprs_store_surrounding_measurement_MFRM51(&mph_surround_meas_ind->best_six_info);
  }
  else
  {
    /*Store the surrounding measurements and use them to calculate C1, C31 and C32*/
    rr_gprs_store_surrounding_measurement_MFRM52(&mph_surround_meas_ind->best_six_info);
  }

#ifdef FEATURE_GSM_RR_CELL_CALLBACK
  rr_handle_ncell_callback(*local_best_six_ncells_ptr);
#endif /* FEATURE_GSM_RR_CELL_CALLBACK */
...
uint8 index;
  ...
  if(mph_nc_measurement_ind->no_of_entries > 0)
  {
    ...
    rr_gprs_store_surround_meas(&mph_surround_meas_ind);
  }
  else
  {
...
/*##################################################################################*/
  ...
  for (cell_index = 0; cell_index < measurement_report_entries; cell_index++)
  {
    ...
    /* new cell on the best 6 list so allocates a penalty timer to it and starts it */
    /********************************************************************************/
    channel_present = rr_gprs_is_ncell_in_best_six(meas_chan_info_ptr);
    ...
    if ( meas_chan_info_ptr->valid_info &&
         ( rr_gprs_camped_on_ccch() || rr_gsm_camped_on_ccch() ) )
    {
      ...
      if( best_six_member && best_six_member->bcch_or_pmo == BCCH_FORMAT )
      {
        ...
        rr_gprs_ncell_bcch_read_manager(best_six_member, (boolean)((cell_index == 0) ? TRUE : FALSE));
      }
    }
  ...
  if(p_best_six_info->reduced_ncell_list_allowed)
  {
    ...
    if ( !reduced_ncell_list_current_mode)
    {
      ...
      if ( entry_index >= RECVD_REDUCED_NCELL_LIST_SIZE)
      {
        ...
        reduced_ncell_list_current_mode = rr_gprs_compare_serv_and_surr_C2(local_gprs_reselection_list_ptr, MIN_SERV_C2_DIFF_TOP6_MODE);
      }
    }
    else
    {
      /* GL1 in TOP2 mode so do 3dB check*/
      reduced_ncell_list_current_mode = rr_gprs_compare_serv_and_surr_C2(local_gprs_reselection_list_ptr, MIN_SERV_C2_DIFF_TOP2_MODE);
    }

    /* send indication to GL1 if mode needs to be changed */
    if ( reduced_ncell_list_current_mode != reduced_ncell_list_l1_view)
    {
      rr_l1_send_reduced_ncell_meas_ind(reduced_ncell_list_current_mode);
    }
  } /* if(p_best_six_info->reduced_ncell_list_allowed) */
#endif /* FEATURE_GSM_SCE_REDUCED_NCELL_LIST*/     
...
)
{
  ...
  if (resel_candidate == NULL)
  {
    MSG_GERAN_LOW_0_G("resel_candidate == NULL");
    return 0;
  }
  ...
  if (surround_db_entry_ptr->cell_selection_parameters.CELL_RESELECT_PARAM_IND)
  {
    ...
    if (PENALTY_TIME != 31)
    {
      ...
      if (rr_timer_is_running(resel_candidate->penalty_timer_id))
      {
...
)
{
      ...
      index++)
  {
    ...
    if (ptr_surround_database != NULL)
    {
      ...
      if (local_gprs_reselection_list_ptr->surrounding_values[index].C1 > 0 || rr_allow_negative_c1(ptr_surround_database->gsm.location_area_identification.plmn_id))
      {
        local_better_cell_list_ptr->candidate_cell[candidate_index].cell_reselection_attempted = FALSE;

        if( rr_gprs_resel_suitable_cell(ptr_surround_database) )
        {
...
)
{
      ...
      index++)
  {
    ...
    /* and check, if the cell is on a supported Band 
     ...
     if (rr_gapi_is_supported_band_dyn((sys_band_T)band) == FALSE)
     {
    ...
    if ( (local_gprs_reselection_list_ptr->surrounding_values[index].C1 > 0 || rr_allow_negative_c1(rr_gprs_get_scell_plmn_id())) &&
...
boolean output = FALSE;
    ...
    if (rr_gprs_parameters_available() == FALSE)
  {
    output = rr_gprs_check_for_better_GSM_ncell(recent_reselection_flag);
    if(output)
    {
      rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_C2);
    }
  }
  else
    if(rr_gprs_parameters_available() == TRUE)
  {
    output = rr_gprs_check_for_better_GPRS51_ncell(recent_reselection_flag);
    if(output)
    {
      rr_log_set_reselection_trigger(RR_RESELECTION_TRIGGER_C32);
    }
  }
        ...
        )
{
  ...
  MSG_GERAN_HIGH_4_G("scell_threshold_db=%d scell_threshold_rxlev=%d, scell_rxlev_valid=%d, scell_rxlev=%d",
...
)
{
  ...
  boolean             same_ra = TRUE;
    ...
    {
      /* If service cell is CS only and we get a suitable ncell supporting PS,
         having Rxlev >= PS_SUPPORT threshold, we can start the timer */
      if (scell_info->gsm.gprs_indicator == FALSE && ptr_ncell_info->gprs_supported == TRUE)
      {
        if( ncell_RXLEV_avg >= C2_PS_SUPPORT_THRESHOLD)
        {
          MSG_GERAN_MED_1_G("Mark PS supporting ncell (ARFCN=%d) better than CS only scell", ptr_ncell_info->gsm.BCCH_ARFCN.num);
          return TRUE;
        }
      }
      /* If currently camped on PS SUPPORTED cell and ncell is cs only, check for other cell
         only if serving cell c1 < 0; Hence returning FALSE from here */
      else if (scell_info->gsm.gprs_indicator == TRUE && ptr_ncell_info->gprs_supported == FALSE)
      {
        MSG_GERAN_MED_1_G("Camped on CS+PS cell, dont go to CS only ncell (ARFCN=%d)", ptr_ncell_info->gsm.BCCH_ARFCN.num);
        return FALSE;
      }
    }
#endif /* FEATURE_PS_PREFERRED_CELL */
  ...
  MSG_GERAN_MED_3_G("sur (ARFCN=%d) C2:%d srv C2:%d", ptr_ncell_info->gsm.BCCH_ARFCN.num, surroundingC2, servingC2);
  ...
  if (rr_get_nv_anite_gcf_flag() == FALSE)
  {
    ...
    if (add_crh_enabled)
    {
      ...
      if (rr_is_serving_rx_lev_greater_threshold(add_crh_scell_threshold_db))
      {
    ...
    {
      MSG_GERAN_LOW_0_G("Additional CRH adjustment not enabled");
    }
  }
...
int GPRS_TEMPORARY_OFFSET_dB;
  ...
  /*********************************************************/
  /* if the penalty timer for the current cell is running */
  if (rr_timer_is_running(penalty_timer_id))
  {
    if (GPRS_TEMPORARY_OFFSET == 7)
    {
 ...
 *
 ****************************************************************************/
gprs_reselection_structure_T *rr_get_gprs_reselection_list_ptr( void )
{
  return &gprs_reselection_list;
}
 ...
 *
 ****************************************************************************/
gprs_better_cell_list_T *rr_get_better_cell_list_ptr( void )
{
  return &better_cell_list;
}
 ...
 *
 ****************************************************************************/
best_six_ncells_t *rr_get_best_six_ncells_ptr( void )
{
  return &best_six_ncells;
}
...
{
  gprs_better_cell_list_T      * better_cell_list = rr_get_better_cell_list_ptr();
  int                            greatest_s_value = 0;
  uint32                         i;

  if (include_serving_cell)
  {
    gprs_reselection_structure_T * gprs_reselection_list = rr_get_gprs_reselection_list_ptr();
