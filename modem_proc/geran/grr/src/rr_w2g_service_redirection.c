...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_w2g_service_redirection.c#5 $
...
static rr_x2g_redir_data_t rr_x2g_redir_data_store =
                                       INITIAL_VALUE(_RR_X2G_REDIRECTION_INIT) ;
#undef _RR_X2G_REDIRECTION_INIT

static rr_reselection_pscan_cell_list_T rr_w2g_redir_cell_list;
...
static rr_x2g_redir_data_t *rr_x2g_redir_get_data_ptr( )
{
  return &rr_x2g_redir_data_store;
}

static rr_reselection_pscan_cell_list_T *rr_x2g_redir_get_cell_list_ptr( )
{
  return &rr_w2g_redir_cell_list;
}
...
)
{
  ...
  rr_x2g_redir_data_ptr = rr_x2g_redir_get_data_ptr();
  RR_NULL_CHECK_FATAL(rr_x2g_redir_data_ptr);

  rr_w2g_redir_cell_list_ptr = rr_x2g_redir_get_cell_list_ptr();
...
)
{
  ...
  rr_PLMN_request_details_ptr->service_req_cause = RR_MM_NORMAL_ACQ;
#endif

  rr_store_plmn_id(&req->PLMN);
...
)
{
  ...
  rr_store_plmn_id(&lte_redir_req->requested_PLMN);
  rr_PLMN_request_details_ptr->search_reason = RR_W2G_SERVICE_REDIRECTION;
...
)
{
  ...
  check_forbidden_lai = rr_w2g_redirection_get_forbidden_lai_check_rule();

  if (check_forbidden_lai == CHECK_FORBIDDEN_LAI_LIST)
  {
    forbidden_lai = rr_is_forbidden_LAI(lai_ptr);
  }
  else if (check_forbidden_lai == CHECK_REJECT_LAI_LIST)
  {
    forbidden_lai = rr_check_lai_in_reject_list(lai_ptr);
  }
  else
  {
    forbidden_lai = FALSE;
  ...
  check_forbidden_plmn = rr_w2g_redirection_should_check_forbidden_plmn();

  if (check_forbidden_plmn && (!forbidden_lai))
  {
    inter_task_lai_T inter_task_lai = rr_convert_rr_lai_to_inter_task_lai(lai_ptr);
#ifdef FEATURE_DUAL_SIM
    forbidden_lai = mm_rr_plmn_lai_forbidden_per_subs(inter_task_lai, GERAN_ACCESS_STRATUM_ID_1);
#else
  ...
  if (check_plmn_mismatch)
  {
    ...
    PLMN_ok = rr_cell_selection_PLMN_match(rr_PLMN_request_details_ptr, lai_ptr);
...
)
{
  ...
  if (C1 <= 0 && !rr_allow_negative_c1(pcell_info_ptr->gsm.location_area_identification.plmn_id))
  {
    return RR_EV_W2G_NO_SUITABLE_CELL_FOUND;
  }
...
)
{
       ...
       ++i)
  {
    if (ARFCNS_EQUAL(rr_x2g_redir_data_ptr->cell_info_list[i]->arfcn, arfcn))
    {
      ...
      if (rr_x2g_redir_is_si1_available(arfcn))
      {
        tc = 8;
      }
      else
...
s          dl_unit_data_ind.short_header = FALSE;
...
{
  // Obtain a pointer to the module data
  rr_x2g_redir_data_t *rr_x2g_redir_data_ptr = rr_x2g_redir_get_data_ptr();
...
boolean rr_l2g_redirection_in_progress( )
{

  // Obtain a pointer to the module data
  rr_x2g_redir_data_t *data = rr_x2g_redir_get_data_ptr();
...
boolean rr_t2g_redirection_in_progress( )
{
  // Obtain a pointer to the module data
  rr_x2g_redir_data_t *data = rr_x2g_redir_get_data_ptr();
...
)
{
  ...
  if (pscan_results->num_frequencies > 1)
  {
         ...
         --i)
    {
      ...
      if (rr_x2g_redir_are_si_messages_available(arfcn))
      {
        MSG_GERAN_MED_2_G("Prioritising ARFCN (%d, %d) with tunnelled SI information available",
                tmp.arfcn_num,
                tmp.arfcn_band);
...
)
{
       ...
       ++i)
  {
    ...
    arfcn.num = cell->meas.arfcn_num;
    arfcn.band = cell->meas.arfcn_band;

    if (rr_x2g_redir_send_specific_si(arfcn, SYSTEM_INFORMATION_TYPE_3))
    {
      si_found = TRUE;
      MSG_GERAN_MED_2_G("Sent tunnelled SI3 for ARFCN %d (band %d)",
              cell->meas.arfcn_num, cell->meas.arfcn_band);
    }
    else if (rr_x2g_redir_send_specific_si(arfcn, SYSTEM_INFORMATION_TYPE_4))
    {
      si_found = TRUE;
    ...
    if (si_found)
    {
      ...
      {
        if (rr_x2g_redir_data_ptr->cell_info_list[j]->arfcn ==  arfcn.num &&
            rr_x2g_redir_data_ptr->cell_info_list[j]->band == arfcn.band)
        { 
...
)
{
  ...
  while (event != EV_NO_EVENT)
  {
    ...
    if (event == EV_RESET_SOFTWARE)
    {
      ...
      (void)rr_timer_cancel(RR_MM_ACTIVATION_RSP_TIMER);	  
      event = EV_NO_EVENT;
    }
    ...
    switch (rr_x2g_redir_data_ptr->state)
    {
        ...
        if (event == EV_INPUT_MESSAGE && new_message != NULL)
        {
          if (message_set == MS_RRC_RR)
          {
            ...
            if (rrc_rr_cmd == RR_INTERRAT_REDIRECT_REQ)
            {
              ...
              if (rr_search_band_pref() != SYS_BAND_MASK_EMPTY)
              {
      ...
      case STATE_GOING_ACTIVE:
      {
        ...
        {
          new_event = rr_selection_control(RR_EV_TRIGGER_CELL_SELECTION_W2G, NULL);
        }
      ...
      case STATE_PERFORMING_CELL_SELECTION_REDIRECTION_SEARCH_1:
      {
        if (event == EV_INPUT_MESSAGE && new_message != NULL)
        {
          ...
          if (!message_handled)
          {
            new_event = rr_selection_control(event, new_message);
          }
        }
        ...
        else if (event == RR_EV_CHANNEL_FOUND)
        {
          ...
          new_event = rr_selection_control(RR_EV_CHANNEL_FOUND, NULL);
        }
        ...
        else if (event == EV_NO_BCCH_FOUND)
        {
          ...
          (void) rr_selection_control(EV_NO_BCCH_FOUND, NULL);
          ...
          {
            if (rr_timer_has_expired(RR_W2G_SERVICE_REDIRECTION_WAIT_TIMER))
            {
              new_event = rr_mode_control(EV_DEACTIVATE_GSM_MODE, NULL);
              rr_x2g_redir_data_ptr->state = STATE_GOING_INACTIVE_DUE_TO_TIMER_EXPIRY;
            }
        ...
        {
          new_event = rr_selection_control(RR_EV_ABORT_PROCEDURE, NULL);
        ...
        else if (event == EV_NO_BCCH_FOUND)
        {
          ...
          if (rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_REDIRECTION_AFTER_CONNECTION_RELEASE)
          {
            ...
            else
            {
              ...
              rr_ms_x2g_procedure_failure(RR_X2G_REDIRECTION_FAILURE);
      ...
      case STATE_PERFORMING_CELL_SELECTION_SERVICE_SEARCH:
      {
        ...
        {
          new_event = rr_selection_control(RR_EV_MM_REQ_RECEIVED_ABORT, NULL);
          rr_x2g_redir_data_ptr->state = STATE_PERFORMING_CELL_SELECTION_ABORTED_BY_MM;
        }
      ...
      case STATE_ACQUIRING_SYSTEM_INFO:
      {
        if (event == RR_EV_W2G_SUITABLE_CELL_FOUND)
        {
          ...
          (void) rr_selection_control(EV_GO_TO_IDLE, NULL);
...
#endif /*FEATURE_SGLTE*/
          {          
            rr_send_activation_ind(MM_AS_INTERRAT_RESELECTION);
          }
        ...
        else if (event == RR_EV_W2G_NO_SUITABLE_CELL_FOUND)
        {
          ...
          to take. */
          new_event = rr_selection_control(RR_EV_ACQUISITION_FAILURE, NULL);
        }
        ...
        else if (event == RR_EV_GSM_SI_RECEIVED)
        {
          ...
          if (pcell_info_ptr->gsm.gprs_indicator)
          {
            if (pcell_info_ptr->bcch_sys_info.si13_data_valid_flag)
            {
              ...
              new_event = process_pending_channel_data();
            }
            else
            {
        ...
        {
          if((rr_timer_is_running(RR_W2G_SERVICE_REDIRECTION_WAIT_TIMER) == TRUE) && 
		  	 ((rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_REDIRECTION_SEARCH) ||
              (rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_REDIRECTION_AFTER_CONNECTION_RELEASE)
             )
            )  
      ...
      case STATE_WAITING_FOR_ACTIVATION_RSP:
      {
          ...
          if (message_set == MS_MM_RR)
          {
            ...
            if ((mm_rr_message_id_enum_T) message_id == RR_ACTIVATION_RSP)
            {
              ...
              else
              {
                ...
                process_rr_activation_rsp((rr_activation_rsp_T *) new_message);
        ...
        {
          MSG_GERAN_HIGH_0_G("Unable to abort interrat redirection after sending RR_ACTIVATION_IND");
#ifdef FEATURE_WCDMA
          if (rr_x2g_redir_data_ptr->source_rat == SYS_RAT_UMTS_RADIO_ACCESS)
          {            
            rr_send_interrat_redirect_abort_cnf(
              RRC_RR_FAILURE   // abort_status
            );
          }
#endif /* FEATURE_WCDMA */
#ifdef FEATURE_LTE
        ...
        else if (event == RR_EV_T2G_REDIR_ABORT_PROCEDURE)
        {
          MSG_GERAN_HIGH_0_G("Unable to abort interrat redirection after sending RR_ACTIVATION_IND");
          rr_send_tds_interrat_redirect_abort_cnf(
            TDSRRC_RR_FAILURE    // abort_status
          );
        }
#endif /*FEATURE_GSM_TDS*/
        else if (event == RR_EV_MM_REQ_RECEIVED_ABORT)
      ...
      case STATE_GOING_INACTIVE_DUE_TO_RRC:
      {
        if (event == EV_GSM_MODE_DEACTIVATED)
        {
          ...
          else if (rr_x2g_redir_data_ptr->source_rat == SYS_RAT_LTE_RADIO_ACCESS)
          {
            ...
            rr_msgr_send(&lte_abort_redir_rsp, sizeof(lte_abort_redir_rsp));
          }
#endif /* FEATURE_LTE */
...
#endif /* FEATURE_LTE || FEATURE_GSM_TDS*/
          {
                ...
                rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_OUT_OF_SERVICE_SEARCH)
            {
              rr_send_interrat_redirect_rej(
                ...
                rr_get_available_plmn_db_ptr()           // available_plmn_db
              );
            }
#ifdef FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION
            else if (rr_x2g_redir_data_ptr->type == RR_X2G_REDIR_TYPE_SERVICE_SEARCH)
            {
              ...
              {
                rr_send_interrat_redirect_rej_service_search_success();
              }
              else
              {
                rr_send_interrat_redirect_rej_service_search_failure();
              }
            }
#endif /* FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION */
#endif /* FEATURE_WCDMA */
    ...
    if (rr_x2g_redir_data_ptr->old_state != rr_x2g_redir_data_ptr->state)
    {
      ...
      {
        if(!rr_timer_is_running(RR_MM_ACTIVATION_RSP_TIMER))
        {
          MSG_GERAN_HIGH_0_G("Start a timer for RR_ACTIVATION_RSP ");
		  (void)rr_timer_start(RR_MM_ACTIVATION_RSP_TIMER, RR_MM_ACTIVATION_RSP_TIMEOUT);          
        }		  
      }
	  else if(rr_timer_is_running(RR_MM_ACTIVATION_RSP_TIMER))
	  {
...
void rr_x2g_send_lte_redir_failure( )
{
  ...
  result = rr_msgr_send(&lte_redir_failed_rsp.hdr, sizeof(lte_redir_failed_rsp));
  if (result != E_SUCCESS)
  {
...
{
  return rr_x2g_redir_get_cell_list_ptr();
}
