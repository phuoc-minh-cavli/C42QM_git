...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sys_info.c#9 $ $DateTime: 2019/07/26 10:11:08 $ $Author: pwbldsvc $
...
static rr_sys_info_data_t  rr_sys_info_data;

static boolean rr_should_ignore_si2ter( );
static boolean rr_handle_si2ter_indicator(boolean, byte, gprs_scell_info_t *  );
static boolean rr_handle_si2quater_indicator(boolean, boolean, gprs_scell_info_t *  );
#ifdef FEATURE_GERAN_EAB
static void rr_handle_si21_indicator(boolean, uint8, boolean, gprs_scell_info_t *);
 ...
 */
static rr_sys_info_data_t *rr_sys_info_get_data_ptr( void )
{
  return &rr_sys_info_data;
}
...
)
{
  ...
  if (dedic_ptr == NULL)
  {
    MSG_GERAN_LOW_0_G("Unable to restore dedicated sys info");
    return;
  }

  MSG_GERAN_LOW_1_G("Restoring dedicated sys info for ARFCN %d...",
  ...
  for (index = 0; index < RR_MAX_DED_SIS; index++)
  {
    ...
    if (dedic_ptr->sys_info_cache.msg_rcvd[index] == TRUE)
    {
      ...
      if (ARFCNS_EQUAL(msg_ptr->channel_number, serving_ARFCN))
      {
        ...
        {
          MSG_GERAN_LOW_0_G("Unable to restore dedicated SI in RR task queue");
          break;
        }
      }
      else
      {
        MSG_GERAN_LOW_3_G("Cached sys info 0x%02X 0x%02X is for different ARFCN (%d)",
...
)
{
  ...
  rr_sys_info_data_t *rr_sys_info_data_ptr = rr_sys_info_get_data_ptr();
...
)
{
  ...
  MSG_GERAN_LOW_0_G("Swapping old SI5 and SI5ter BA lists");
...
)
{
  ...
  switch(si5_variant)
  {
    ...
    default:
    {
      MSG_GERAN_LOW_0_G("Wrong ba list type");
...
)
{
  ...
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_1);

  if (current_IE == NULL)
  {
    MSG_GERAN_LOW_0_G("current_IE == NULL");
    decoding_ok = FALSE;
  }
  else
  {
  ...
  if (decoding_ok == TRUE)
  {
    ...
    current_IE = current_IE->next_IE;
    decode_si1_ro(current_IE->start_IE, &nch_pos, &band_ind);
    ...
    else if (pcell_info_ptr->gsm.BCCH_ARFCN.num == channel_number.num)
    {
      ...
      /* Store R99 band indicator in pending SI database. This should be done prior
         to reassigning band ind in cell allocation list since rr_get_band_dyn() uses
       ...
       */
      rr_gprs_reassign_band(CELL_ALLOC_FREQ_LIST, FALSE);
...
)
{
  ...
  if (rr_gapi_is_supported_band_dyn(SYS_BAND_PGSM_900) && (rr_gapi_get_supported_band_count() == 1))
  {
    /*
  ...
  else /* mobile station does not only support PGSM band */
  {
    ...
    if (rr_gapi_is_supported_band_dyn(SYS_BAND_DCS_1800) && (rr_gapi_get_supported_band_count() == 1))
    {
  ...
  if (si_db_ptr->valid_data_flag)
  {
    ...
    else /* BA_IND has not changed */
    {
      ...
      if (ba_change)
      {
        ...
        rr_generate_complete_idle_BA_list(si_db_ptr, IGNORE_RCVD_SI_MASK);
      }
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_2);
  ...
  if (decoding_ok == TRUE)
  {
        ...
        )
    {
      ...
      if (ba_changed)
      {
        ...
        rr_generate_complete_idle_BA_list(&scell_info_ptr->gsm, IGNORE_RCVD_SI_MASK);
      }
      ...
      /********************************************************************************/
      {
        ...
        {
          rr_gprs_gsm_ba_list_updated( (boolean) ( ! notify_L1 ) );
        }
      }
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_2_BIS);
  ...
  if (decoding_ok == TRUE)
  {
        ...
        )
    {
      ...
      boolean notify_L1  = FALSE;
      boolean ba_changed = FALSE;
      uint32  ba_change_flags;

      MSG_GERAN_HIGH_1_G("Store SI2bis(%d) for serving cell",channel_number.num);
      rr_gprs_nacc_check_ba_flags(&(scell_info_ptr->gsm.SI2bis_BA_list));
#ifndef FEATURE_GERAN_REDUCED_DEBUG
      // Log RACH params if different from SIDB
      rr_log_diff_rach_control_parameters(
        &RACH_control_parameters,
...
)
{
  ...
  MSG_GERAN_MED_1_G("SI2ter received, tc=%02X", message_ptr->tc);

  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_2_TER);
  ...
  */
  ...
  if (rr_si2ter_changed(si2ter_inst_ptr))
  {
    ...
    rr_gprs_update_3g_nlist_from_si2ter_instances(si2ter_inst_ptr);
...
#ifdef FEATURE_LTE
        rr_send_imsg(RR_IMSG_IRAT_SYSTEM_INFORMATION_UPDATED_IND);
#endif /* FEATURE_LTE */
    } /* rr_is_camped */
#endif /* FEATURE_WCDMA */
  } /* Si2ter changed */
  ...
  if (process_2g_BA == TRUE)
  {
        ...
        ARFCNS_EQUAL(channel_number, scell_info_ptr->gsm.BCCH_ARFCN))
    {
      ...
      rr_gprs_nacc_check_ba_flags(&(scell_info_ptr->gsm.SI2ter_BA_list));
...
)
{
  ...
  MSG_GERAN_MED_1_G("SI2quater received, tc=%02X", message_ptr->tc);

  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_2_QUATER);
  ...
  ********************************************************************/
  ...
  if (rr_SI2q_MI_changed(si2q_inst_ptr))
  {
    ...
    rr_gprs_update_3g_nlist_from_si2quater_instances(si2q_inst_ptr);
#else
    sys_info_db_ptr->utran_neighbor_list.valid_data = TRUE;
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS*/

    /* LTE neighbour list is generated by the following function. */
    rr_gprs_update_meas_params_from_bcch(channel_number, TRUE);

    rr_gprs_update_gprs_bsic_description_from_si2quater_instances(channel_number);
...
#if defined(FEATURE_WCDMA) || defined(FEATURE_LTE) || defined(FEATURE_GSM_TDS)
  if (rr_is_camped() && (update_l1 == TRUE))
  {
    /* Send Cell_update_list_req to GL1 only for OTA instances.*/
#ifdef FEATURE_RR_SI_CACHE
    if (message_ptr->tc != CACHED_SYS_INFO_TC)
...
)
{
  ...
  /* get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(MEASUREMENT_INFORMATION_TYPE);
  ...
  if (decoding_ok == TRUE)
  {
    ...
    MSG_GERAN_LOW_0_G("Completed Decoding Measurement Information instance(s)");
    ...
    */
    ...
    if ((set_MI_received == TRUE) && (send_wcdma_neighbor_list_to_L1 == TRUE))
    {
      ...
      rr_gsm_update_gsm_bsic_description_from_mi_instances(mi_inst_ptr);
    }
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
    else if ((set_MI_received == FALSE) && (send_wcdma_neighbor_list_to_L1 == TRUE))
    {
      ...
      rr_populate_ded_neighbor_list(&scell_info_ptr->gsm.wcdma_ded_neighbor_list, mi_inst_ptr);
    }
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS*/

    if (send_wcdma_neighbor_list_to_L1 == TRUE)
    {
      scell_info_ptr->gsm.wcdma_ded_neighbor_list.valid_data = TRUE;
    }
 ...
 */
static rr_l1_si_type_T rr_sys_info_required_after_si3(gprs_scell_info_t *cell_info_ptr )
{
...
#ifdef FEATURE_GERAN_EAB
  if (rr_nv_get_eab_enabled() == FALSE)
  {
    norm_mask = norm_mask & (~(RR_L1_SI_TYPE_9 | RR_L1_SI_TYPE_21_NORM));
  }
#endif
  ...
  if (cell_info_ptr != NULL)
  {
    ...
    if ((SI2Q_POSITION_NONE == cell_info_ptr->gsm.two_quater_position) ||
        (FALSE == rr_ms_3g_sys_info_mandatory()))
    {
...
)
{
  ...
  boolean si21_pi = FALSE;
  uint8   si21_pos = 0;
  boolean siaw_flag = FALSE;
  uint8   where = 0;
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_3);
  ...
  {
    MSG_GERAN_LOW_0_G("message_list parsing error");
  }
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("LOCATION AREA IDENTIFICATION missing");
      }
    }
    else
    {
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("CONTROL CHANNEL DESCRIPTION missing");
      }
    }
    else
    {
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("CELL OPTIONS missing");
      }
    }
    else
    {
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("CELL SELECTION PARAMETERS missing");
      }
    }
    else
    {
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
      ...
      {
        MSG_GERAN_LOW_0_G("RACH CONTROL PARAMETERS missing");
      }
    }
    else
    {
    ...
    if (current_IE)
    {
      ...
      if (current_IE->start_IE)
      {
        ...
        if (error_code == CSN_DECODE_NO_ERROR)
        {
          ...
          if (si3ro.power_offset_flag == valid)
          {
            ...
            */
            if (((band == BAND_DCS) || (band == BAND_PCS)) &&
                (rr_gapi_get_band_rf_power_class_dyn(band) == (byte) CLASS_3))
            {
          ...
          /* SI21 Indicator */
          si21_pi = si3ro.si21_indicator_flag;
          si21_pos = si3ro.si21_position;
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
          if (rr_nv_is_ir_eab_testing_params_present())
          {
            si21_pi = rr_nv_get_SI21_indicator();
            si21_pos = rr_nv_get_SI21_position();
          }
#endif /* FEATURE_GERAN_R13_TEST_FRAMEWORK */

          /* Scheduling Info If and Where */
          siaw_flag = si3ro.siaw_flag;
          where = si3ro.where;
#ifdef FEATURE_GERAN_R13_TEST_FRAMEWORK
          if (rr_nv_is_ir_eab_testing_params_present())
          {
            siaw_flag = rr_nv_get_SI9_where_indicator();
            where = rr_nv_get_SI9_where();
          }
#endif /* FEATURE_GERAN_R13_TEST_FRAMEWORK */
          MSG_GERAN_HIGH_4_G("EAB:: SI21_Indicator(%d), SI21_Position(%d), SIAW_Flag(%d), Where(%d)", si21_pi, si21_pos, siaw_flag, where);
          ...
          if (rr_nv_get_eab_enabled() == FALSE)
          {
            si21_pi = FALSE;
            siaw_flag = FALSE;
            MSG_GERAN_HIGH_0_G("EAB:: EFS not enabled - SI21_Indicator and SIAW_Flag reset to FALSE");
          }
#endif /* FEATURE_GERAN_EAB */
        }
      }
  ...
  if (decoding_ok == TRUE)
  {
    ...
    /* system_information_database store              */
    /**************************************************/
    if (rr_is_this_the_scell(channel_number) && (! rr_background_plmn_search_in_progress()))
    {
      gprs_scell_info_t *scell_info_ptr = rr_gprs_get_scell_info();
      boolean notify_MM = FALSE;
      boolean notify_L1 = FALSE;
      ...
      if (control_channel_description.BS_PA_MFRMS != scell_info_ptr->gsm.control_channel_description.BS_PA_MFRMS)
      {
        ...
        old_dsc_max = rr_gprs_get_max_dsc_value(TRUE);
#endif /* FEATURE_IDLE_DRX_SCALING */
      }
      ...
      scell_info_ptr->gsm.si_sched_info.si9_sched_where = where;

      if (bs_pa_mfrms_changed)
      {
#ifdef FEATURE_IDLE_DRX_SCALING
        rr_idle_drx_bs_pa_mfrms_changed(old_dsc_max);
      ...
      if (log_cell_info == TRUE)
      {
        ...
        (void) rr_log_cell_info(&scell_info_ptr->gsm);
#endif 
#ifdef FEATURE_GSM_RR_CELL_CALLBACK
      ...
      send_wcdma_cell_update =
        rr_handle_si2ter_indicator(two_ti, tc, rr_gprs_get_scell_info());
      send_wcdma_cell_update |=
        rr_handle_si2quater_indicator(si2q_pi, si2q_pos, rr_gprs_get_scell_info());
...
#ifdef FEATURE_GERAN_EAB
      /* Make sure SI21 indicator handling is done after setting SI3 mask. */
      rr_handle_si21_indicator(si21_pi, si21_pos, siaw_flag, rr_gprs_get_scell_info());
#endif /* FEATURE_GERAN_EAB */
    ...
    /* pending_channel_data store                     */
    /**************************************************/
    else if (rr_is_this_the_pcell(channel_number))
    {
      gprs_scell_info_t *pcell_info_ptr = rr_gprs_get_pcell_info();
      ...
      pcell_info_ptr->gsm.si_sched_info.si9_sched_where = where;
       ...
       */
      if (rr_is_acquiring_si_after_conn_rel())
      {
        if (rr_ce_conn_originated_on_other_rat() &&
            rr_equivalent_plmn_is_undefined() &&
            (rr_is_limited_plmn_selection_mode() == FALSE))
        {
          rr_set_equivalent_plmn(&location_area_identification.plmn_id);
        }
      }
#endif /* FEATURE_GERAN_CS_SUPPORT */
      ...
      else
      {
        ...
        if (rr_gprs_nacc_pncd_in_progress())
        {
      ...
      (void)rr_handle_si2ter_indicator(two_ti, tc, rr_gprs_get_pcell_info());
      (void)rr_handle_si2quater_indicator(si2q_pi, si2q_pos, rr_gprs_get_pcell_info());

      if (pcell_info_ptr->gsm.two_quater_position == SI2Q_POSITION_BCCH_Ext)
      {
        if (rr_ms_3g_sys_info_mandatory())
        {
          /*lint -save -e655 :Suppress Lint Warning for Enum Bitwise operation */
...
#ifdef FEATURE_GERAN_EAB
      /* Make sure SI21 indicator handling is done after setting SI3 mask. */
      rr_handle_si21_indicator(si21_pi, si21_pos, siaw_flag, rr_gprs_get_pcell_info());
#endif /* FEATURE_GERAN_EAB */
    ...
    if (rr_gprs_is_this_a_surround_ncell(channel_number))
    {
      bcch_ncell_info_t *ncell_info_ptr;

      ncell_info_ptr = rr_gprs_get_surround_ncell_sys_info(channel_number);
      ...
      if (ncell_info_ptr)
      {
        ...
        rr_ncell_si3_si4_received(channel_number, ncell_info_ptr);
    ...
    if (rr_candidate_db_is_candidate_cell(channel_number))
    {
      ...
      candidate_cell_ptr = rr_candidate_db_store_si_info(channel_number, &si_info);
      if (candidate_cell_ptr != NULL)
      {
        rr_plmn_db_update(candidate_cell_ptr, channel_number);
      }
      ...
      (void)rr_acq_db_add_arfcn(
        ...
        ,(rr_ps_support_status_t)gprs_indicator_flag
#endif
       );
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_4);
  ...
  if (decoding_ok)
  {
    ...
    if (current_IE)
    {
#ifdef FEATURE_GERAN_CBCH_SUPPORT
...
#endif /* FEATURE_GERAN_CBCH_SUPPORT */
      {
        decoding_ok = TRUE;
      }
    }
  ...
  if (decoding_ok == TRUE)
  {
    ...
    if (rr_is_this_the_scell(channel_number) && (! rr_background_plmn_search_in_progress()))
    {
      ...
      if( cbch_updated )
      {
        ...
        rr_send_cb_info(cbch_channel_information_valid);
      }
    ...
    if (rr_gprs_is_this_a_surround_ncell(channel_number))
    {
      ...
      if (ncell_info_ptr)
      {
        ...
        else
        {
          ...
          /*******************************************************************/
          if (!rr_is_this_the_pcell(channel_number))
          {
            ...
            (void)rr_send_mph_gprs_surround_update_req(&l1_surround_data, FALSE, NULL);
          }
        }
      }
      else
    ...
    if (rr_candidate_db_is_candidate_cell(channel_number))
    {
      ...
      (void)rr_acq_db_add_arfcn(
        ...
        location_area_identification.plmn_id
#ifdef FEATURE_PS_PREFERRED_CELL
        ,(gprs_indicator_flag ? PS_SUPPORTED : PS_SUPPORT_UNKNOWN)
#endif
       );
 ...
 )
{
  ...
  if (rr_ba_lists_ptr->complete_BA_sacch.BA_IND != BCCH_frequency_list_ptr->BA_IND)
  {
    ...
    /* When the BA_IND changes, the complete BA list must be rebuilt from scratch */
    /******************************************************************************/
    rr_reset_dedicated_BA_lists(si_db_ptr);
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_5);
  ...
  if (decoding_ok == TRUE)
  {
    ...
    if (rr_get_status_of_old_cell_si5_ba_lists())
    {
      if (rr_get_si5_count() <= RR_SI5_COUNT_BEFORE_RESET)
      {
        rr_increment_si5_count();
      }
      else
      {
        ...
        rr_reset_si5_count();
        rr_set_status_of_old_cell_si5_ba_lists(FALSE);
    ...
    MSG_GERAN_LOW_0_G("SI5 received");
    if (ba_changed)
    {
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_5_BIS);
  ...
  if (decoding_ok == TRUE)
  {
    ...
    MSG_GERAN_LOW_0_G("SI5bis received");
    if (ba_changed)
    {
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_5_TER);
  ...
  if (decoding_ok == TRUE)
  {
    ...
    MSG_GERAN_LOW_0_G("SI5ter received");
    if (ba_changed)
    {
    ...
    if (multiband_reporting != scell_info_ptr->gsm.multiband_reporting)
    {
      ...
      list. */
      rr_generate_complete_dedicated_BA_list(&scell_info_ptr->gsm);
      notify_L1 = TRUE;
    }
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_6);
  ...
  if (decoding_ok)
  {
    ...
    if (log_cell_info == TRUE)
    {
      ...
      (void)rr_log_cell_info(&scell_info_ptr->gsm);
#endif //FEATURE_GERAN_REDDUCED_DEBUG
#ifdef FEATURE_GSM_RR_CELL_CALLBACK
      if (best_six_ncells_ptr != NULL)
      {
        rr_notify_cell_id_change(
    ...
    if (si6_ro_present == TRUE)
    {
      ...
      if (rr_is_camped() == FALSE)
      {
        ...
        /* so calls to rr_get_band_dyn will use the indicator in pending_channel_data. */
      ...
      else if (scell_info_ptr->gsm.band_indicator != prev_indicator)
      {
        ...
        pcell_info_ptr->gsm.band_indicator = (si_psi_band_ind_T)decoded_si6_ro.band_ind;
        rr_gprs_reassign_band(0xffff, TRUE);
        notify_L1 = TRUE;
#ifdef FEATURE_GSM_DTM
    ...
    if (rr_get_nv_dtm_enabled())
    {
      ...
      DISP_INACTIVE_STATE.
      */
      (void) rr_handle_procedures(EV_DTM_SI6_RECEIVED, NULL);
    }

#else /* FEATURE_GSM_DTM */

    if (cell_identity_valid_flag == FALSE) 
    {
      /* Send RR_SERVICE_IND_FROM_OTA_MSG to MM */
      grr_update_mm_with_rr_service_ind();
    }
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_7);
  ...
  if (current_IE)
  {
    if (current_IE->start_IE)
    {
      ...
      if (rr_gprs_is_this_a_surround_ncell(channel_number))
      {
        ...
        else
        {
          ...
          MSG_GERAN_LOW_1_G("RR SI7(%d) ncell_info_ptr == NULL",channel_number.num);
        }
      }
      else
      {
        if (!rr_gprs_nacc_pncd_in_progress())
        {
          MSG_GERAN_LOW_1_G("SI7(%d) ignored, wrong channel",(int) channel_number.num);
        }
        error_code = UNKNOWN_MESSAGE_TYPE;
      }
      ...
      if (error_code == CSN_DECODE_NO_ERROR)
      {
        ...
        MSG_GERAN_LOW_1_G("Store SI7(%d) in surrounding",(int) channel_number.num);
        ...
        if (ncell_info_ptr) /* to satisfy Lint */
        {
          ...
          rr_ncell_si7_si8_received(channel_number, ncell_info_ptr);
    ...
    {
      MSG_GERAN_LOW_0_G("SI7 rest octets not present");
    }
  }
  else
  {
...
)
{
  ...
  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_8);
  ...
  if (current_IE)
  {
    if (current_IE->start_IE)
    {
      ...
      if (rr_gprs_is_this_a_surround_ncell(channel_number))
      {
        ...
        {
          error_code = UNKNOWN_MESSAGE_TYPE;
          MSG_GERAN_LOW_1_G("RR SI8(%d) ncell_info_ptr == NULL",channel_number.num);
        }
      }
      else
      {
        if (!rr_gprs_nacc_pncd_in_progress())
        {
          MSG_GERAN_LOW_1_G("SI8(%d) ignored, wrong channel",(int) channel_number.num);
        }
        error_code = UNKNOWN_MESSAGE_TYPE;
      }
      ...
      if (error_code == CSN_DECODE_NO_ERROR)
      {
        ...
        MSG_GERAN_LOW_1_G("Store SI8(%d) in surrounding",(int) channel_number.num);
    ...
    {
      MSG_GERAN_LOW_0_G("SI8 rest octets not present");
    }
  }
  else
  {
...
#endif /* FEATURE_HANDLE_SI7_SI8 */
#ifdef FEATURE_GERAN_EAB
static void rr_process_si_scheduling_info(
  gprs_scell_info_t  *cell_info_ptr
)
{
  /* process scheduling info only if SI3 is read to avoid unnecessary addition/deletion of SIs from the received mask */
  if (ARE_SET(MASK_SI_3, cell_info_ptr->gsm.update_message_received))
  {
    /* Check if SI21 scheduling info is present */
    if (cell_info_ptr->gsm.si_sched_info.si_sched_info_count > 0)
    {
      /* Set position to BCCH_Norm. GL1 will pick exact position from bcch_type present in sched_info. */
      cell_info_ptr->gsm.si21_position = SI21_POSITION_BCCH_Norm;
      if (!ARE_SET(MASK_SI_21, cell_info_ptr->gsm.update_message_received))
      {
        bsi_set_messages_required_for_acquisition(MASK_SI_21, MASK_SI_NONE, FALSE);
      }
    }
    else
    {
      /* SI21 scheduling info is not present in SI9.
       * Check if SI21 position is not present in SI3 then SI21 is not broadcasted. */
      if (cell_info_ptr->gsm.si21_position == SI21_POSITION_UNKNOWN)
      {
        /* SI21 is not broadcast. Inform NAS so that they may retry n/w access, if previously blocked due to SI21 not received. */
        if ((cell_info_ptr->gsm.update_message_received & MASK_SI_21) == 0x00)
        {
          MSG_GERAN_HIGH_0_G("EAB:: SI21 not broadcast. Send indication to NAS with wait_time=0");
          rr_send_ps_implicit_reject_ind(0);
        }
        /* Mark SI21 as not being broadcast. This also marks SI21 as received, and prevents further acquisitions */
        cell_info_ptr->gsm.si21_position = SI21_POSITION_NONE;
        bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_NONE, MASK_SI_NONE, MASK_SI_21);
        rr_initialize_SI21_instances();
      }
    }
  }

  MSG_GERAN_HIGH_1_G("EAB:: si21_position: %d", cell_info_ptr->gsm.si21_position);
}

static void rr_dump_si9ro(si9_rest_octets_t *si9ro_ptr )
{
  uint8 i, j;
  
  MSG_GERAN_LOW_1_G("EAB:: SI9 Scheduling Info Count = %d", si9ro_ptr->scheduling_info_count);
  for (i = 0; (i < si9ro_ptr->scheduling_info_count) && (i < MAX_SCHEDULING_INFO); i++)
  {
    MSG_GERAN_HIGH_4_G("[%d] Info type flag: %d, Info type:%d, Position Count: %d", 
      i, 
      si9ro_ptr->scheduling_info[i].info_type_flag, 
      si9ro_ptr->scheduling_info[i].info_type, 
      si9ro_ptr->scheduling_info[i].position_count
    );
    for (j = 0; (j < si9ro_ptr->scheduling_info[i].position_count) && (j < MAX_POSITIONS); j++)
    {
      MSG_GERAN_HIGH_4_G("  [%d] modulus: %d, relative_position: %d, bcch_type: %d",
        j,
        si9ro_ptr->scheduling_info[i].positions[j].modulus,
        si9ro_ptr->scheduling_info[i].positions[j].relative_position,
        si9ro_ptr->scheduling_info[i].positions[j].bcch_type
      );
    }
  }
}

static void rr_update_si_scheduling_info(
  si9_rest_octets_t  *si9ro_ptr,
  gprs_scell_info_t  *cell_info_ptr
)
{
  uint8 i, j, si_cnt;

  rr_dump_si9ro(si9ro_ptr);

  for (i = 0, si_cnt = 0; (i < si9ro_ptr->scheduling_info_count) && (si_cnt < MAX_SI_SCHED_INFO); i++)
  {
    /* info_type_4 and info_type_6 are reserved for future use.
     * Currently we are only interested with SI21. So, ignore scheduling info of other SIs. */
    if ((si9ro_ptr->scheduling_info[i].info_type_flag == info_type_5) &&
        (si9ro_ptr->scheduling_info[i].info_type == SI_TYPE_21))
    {
      cell_info_ptr->gsm.si_sched_info.si_sched_info[si_cnt].si = si9ro_ptr->scheduling_info[i].info_type;
      cell_info_ptr->gsm.si_sched_info.si_sched_info[si_cnt].pos_count = si9ro_ptr->scheduling_info[i].position_count;

      for (j = 0; (j < si9ro_ptr->scheduling_info[i].position_count) && (j < MAX_SI_SCHED_POSITIONS); j++)
      {
        si_position_T *dst = &cell_info_ptr->gsm.si_sched_info.si_sched_info[si_cnt].positions[j];
        position_t    *src = &si9ro_ptr->scheduling_info[i].positions[j];
        dst->bcch_type    = src->bcch_type;
        dst->modulus      = src->modulus;
        dst->relative_pos = src->relative_position;
      }
  ...
  cell_info_ptr->gsm.si_sched_info.si_sched_info_count = si_cnt;
  rr_process_si_scheduling_info(cell_info_ptr);
}

/**
 * Called to process a System Information 9 message received from
 * the network. SI9 is expected for the serving and pending
 * cells only
 *
 * @param channel_number       channel number where the message was received
 *
 */
static void rr_handle_system_info_9(
  ARFCN_T              channel_number,
  dl_unit_data_ind_T  *message_ptr
)
{
  ...
  boolean                    decoding_ok = FALSE;
  RACH_control_parameters_T  RACH_control_parameters;
  si9_rest_octets_t          si9ro;

  RR_ZERO_STRUCTURE(RACH_control_parameters);

  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_9);

  if (current_IE == NULL)
  {
    MSG_GERAN_LOW_0_G("Message parsing error");
  }
  else if (current_IE->start_IE)
  {
    /* Decode Mandatory IE - RACH Control Parameters */
    rr_decode_RACH_control_parameters(current_IE->start_IE, &RACH_control_parameters);
    ...
    if (current_IE)
    {
      /* Decode Mandatory IE - SI9 Rest Octets */
      if (current_IE->start_IE)
      {
      ...
      {
        MSG_GERAN_ERROR_0_G("Mandatory IE - SI9 Rest Octets missing");
      }
    }
    else
    {
      MSG_GERAN_ERROR_0_G("Message parsing error");
    }
  }
  else
  {
    MSG_GERAN_ERROR_0_G("Mandatory IE - RACH Control Parameters missing");
  }

  if (decoding_ok)
  {
    if (rr_is_this_the_scell(channel_number))
    {
      boolean notify_MM = FALSE;
      boolean notify_L1 = FALSE;
      LAI_T  *lai_ptr = NULL;
      ...
      MSG_GERAN_HIGH_1_G("Store SI9 for scell (%d)", channel_number.num);
#ifndef FEATURE_GERAN_REDUCED_DEBUG
      rr_log_diff_rach_control_parameters(&RACH_control_parameters, &scell_info_ptr->gsm);
#endif
      rr_check_for_rach_control_parameters_change(&RACH_control_parameters, &scell_info_ptr->gsm, &notify_L1, &notify_MM);

      scell_info_ptr->gsm.RACH_control_parameters = RACH_control_parameters;
      rr_update_si_scheduling_info(&si9ro, scell_info_ptr);
      SET_BIT(MASK_SI_9, scell_info_ptr->gsm.update_message_received);

      rr_perform_MM_or_L1_notification_on_serving_SI_change(notify_L1, notify_MM);

      if (scell_info_ptr->gsm.cell_identity_valid_flag)
      {
        lai_ptr = &scell_info_ptr->gsm.location_area_identification;
        ci_ptr  = &scell_info_ptr->gsm.cell_identity;
      }
#ifdef FEATURE_RR_SI_CACHE
      (void)rr_csi_update_entry(SYSTEM_INFORMATION_TYPE_9, channel_number, message_ptr, lai_ptr, ci_ptr);
#endif
    }
    else if (rr_is_this_the_pcell(channel_number))
    {
      ...
      MSG_GERAN_HIGH_1_G("Store SI9 for pcell (%d)", channel_number.num);

      pcell_info_ptr->gsm.RACH_control_parameters = RACH_control_parameters;
      rr_update_si_scheduling_info(&si9ro, pcell_info_ptr);
      SET_BIT(MASK_SI_9, pcell_info_ptr->gsm.update_message_received);

      if (pcell_info_ptr->gsm.cell_identity_valid_flag)
      {
        lai_ptr = &pcell_info_ptr->gsm.location_area_identification;
        ci_ptr  = &pcell_info_ptr->gsm.cell_identity;
      }
#ifdef FEATURE_RR_SI_CACHE
      (void)rr_csi_update_entry(SYSTEM_INFORMATION_TYPE_9, channel_number, message_ptr, lai_ptr, ci_ptr);
#endif
    ...
    {
      MSG_GERAN_HIGH_1_G("SI9 ignored - wrong channel number (%d)", channel_number.num);
    }
  }
  else
  {
    MSG_GERAN_ERROR_1_G("Cannot decode SI9(%d)", channel_number.num);
  }

  return;
} /* rr_handle_system_info_9 */

static void rr_update_EAB_info(
  si21_instances_T               *si21_instances,
  system_information_database_T  *sys_info_db_ptr,
  boolean                        *EAB_changed
)
{
  ...
  *EAB_changed = TRUE;
  ...
  /* Assign the one with highest INDEX if there are multiple EAB info received. */
  for (i = si21_instances->num_instances - 1; i >= 0; i--)
  {
    if (si21_instances->instance[i] && si21_instances->instance[i]->EAB_info_flag)
    {
      if ((sys_info_db_ptr->EAB_info_valid &&
           sys_info_db_ptr->EAB_info.authorization_mask == si21_instances->instance[i]->EAB_info.authorization_mask &&
           sys_info_db_ptr->EAB_info.subcategory == si21_instances->instance[i]->EAB_info.subcategory))
      {
        *EAB_changed = FALSE;
      }
      else
      {
        sys_info_db_ptr->EAB_info_valid = TRUE;
        sys_info_db_ptr->EAB_info = si21_instances->instance[i]->EAB_info;
      }

      MSG_GERAN_HIGH_4_G("EAB:: SysInfoDB - EAB_valid(%d), Auth_mask(0x%x), Subcat(%d), EAB_changed(%d)",
        sys_info_db_ptr->EAB_info_valid, sys_info_db_ptr->EAB_info.authorization_mask, sys_info_db_ptr->EAB_info.subcategory, *EAB_changed);
      return;
    }
  }
  
  sys_info_db_ptr->EAB_info_valid = FALSE;
}

/**
 * Called to process a System Information 21 message received from
 * the network. SI21 is expected for the serving and pending
 * cells only
 *
 * @param channel_number       channel number where the message was received
 *
 */
static void rr_handle_system_info_21(
  ARFCN_T              channel_number,
  dl_unit_data_ind_T  *message_ptr
)
{
  system_information_database_T *sys_info_db_ptr = NULL;
  si21_instances_T              *si21_instances  = NULL;
  ie_descriptor_T               *current_IE;
  boolean                        decoding_ok = FALSE;
  boolean                        si21_rcvd   = FALSE;

  /* obtain pointer to global SI21 instances store */
  si21_instances = rr_get_SI21_instances_ptr();

  /* Get a pointer to the first IE of the OTA message */
  current_IE = rr_get_first_ie_of_message(SYSTEM_INFORMATION_TYPE_21);

  if (rr_is_this_the_scell(channel_number))
  {
    sys_info_db_ptr = &(rr_gprs_get_scell_info()->gsm);
    MSG_GERAN_HIGH_1_G("Store SI21 for scell (%d)", channel_number.num);
  }
  else if (rr_is_this_the_pcell(channel_number))
  {
    sys_info_db_ptr = &(rr_gprs_get_pcell_info()->gsm);
    MSG_GERAN_HIGH_1_G("Store SI21 for pcell (%d)", channel_number.num);
  }
  else
  {
    MSG_GERAN_HIGH_1_G("SI21(%d) ignored: wrong channel number", channel_number.num);
    return;
  }
  ...
  else if (current_IE->start_IE)
  {
    ...
    if (decoding_ok == FALSE)
    {
      MSG_GERAN_ERROR_0_G("Error decoding SI21 Rest Octets");
      return;
    }
    ...
    if (si21_rcvd)
    {
      ...
      rr_update_EAB_info(si21_instances, sys_info_db_ptr, &EAB_changed);
      SET_BIT(MASK_SI_21, sys_info_db_ptr->update_message_received);

      /* If there is change in EAB content send indication to NAS. So that they may retry n/w access. */
      if (EAB_changed == TRUE)
      {
        MSG_GERAN_HIGH_0_G("EAB:: SI21 received. Send indication to NAS with wait_time=0");
        rr_send_ps_implicit_reject_ind(0);
      }
    }
    else
    {
      /* Mark SI21 as not received */
      CLEAR_BIT(MASK_SI_21, sys_info_db_ptr->update_message_received);

      /* Request SI21 acquisition */
      bsi_set_messages_required_for_acquisition(MASK_SI_21, MASK_SI_NONE, FALSE);
    }

    if (sys_info_db_ptr->cell_identity_valid_flag)
    {
      lai_ptr = &sys_info_db_ptr->location_area_identification;
      ci_ptr  = &sys_info_db_ptr->cell_identity;
    }
#ifdef FEATURE_RR_SI_CACHE
    (void)rr_csi_update_entry(SYSTEM_INFORMATION_TYPE_21, channel_number, message_ptr, lai_ptr, ci_ptr);
#endif
  }
  else
  {
    MSG_GERAN_ERROR_0_G("Mandatory IE - SI21 Rest Octets missing");
  }

  return;
} /* rr_handle_system_info_21 */
#endif /* FEATURE_GERAN_EAB */
...
)
{
  ...
  if (message_ok == TRUE)
  {
    ...
    if (get_rr_state() == RR_W2G_INTERRAT_HANDOVER_PROGRESS)
    {
    ...
    switch (message_type)
    {
      ...
      {
        rr_handle_system_info_1(channel_number, message_ptr);
      ...
      {
        rr_handle_system_info_2(channel_number, message_ptr);
      ...
      case SYSTEM_INFORMATION_TYPE_2_BIS:
      {
        ...
        /**********************************************************************/
        if ((rr_gapi_get_supported_band_count() == 1) && rr_gapi_is_supported_band_dyn(SYS_BAND_PGSM_900))
        {
        ...
        else
        {
          ...
          rr_handle_system_info_2_bis(channel_number, message_ptr);
      ...
      {
        rr_handle_system_info_2_ter(channel_number, message_ptr);
      ...
      {
        (void)rr_handle_system_info_2_quater(channel_number, message_ptr);
      ...
      case MEASUREMENT_INFORMATION_TYPE:
      {
        ...
        MSG_GERAN_LOW_1_G("Received Measurement Information Message (%d)",(int) channel_number.num);
        rr_handle_measurement_information();
        break;
      }
#endif /* FEATURE_GERAN_CS_SUPPORT */
      ...
      {
        rr_handle_system_info_3(channel_number, message_ptr->tc, message_ptr);
      ...
      {
        rr_handle_system_info_4(channel_number, message_ptr);
      ...
      case SYSTEM_INFORMATION_TYPE_5_TER:
      {
        ...
        /***********************************************************************/
        if ((rr_gapi_get_supported_band_count() == 1)  &&
             (rr_gapi_is_supported_band_dyn(SYS_BAND_PGSM_900) ||
               rr_gapi_is_supported_band_dyn(SYS_BAND_DCS_1800)
             )
           )
        {
      ...
      {
        rr_handle_system_info_7(channel_number);
      ...
      {
        rr_handle_system_info_8(channel_number);
      ...
      {
        if (rr_nv_get_eab_enabled())
        {
          rr_handle_system_info_9(channel_number, message_ptr);
          si_message_received = SI_TYPE_SI_9;
        }
        else
        {
          MSG_GERAN_HIGH_0_G("EAB:: EFS not enabled - SI9 ignored");
        }
        break;
      }
#endif /* FEATURE_GERAN_EAB */
      ...
      {
        rr_process_si13_from_bcch(channel_number, message_ptr);
      ...
      case SYSTEM_INFORMATION_TYPE_21:
      {
        if (rr_nv_get_eab_enabled())
        {
          rr_handle_system_info_21(channel_number, message_ptr);
          si_message_received = SI_TYPE_SI_21;
        }
        else
        {
          MSG_GERAN_HIGH_0_G("EAB:: EFS not enabled - SI21 ignored");
        }
        break;
      }
#endif /* FEATURE_GERAN_EAB */
    ...
    if (!dedicated_si_message)
    {
      ...
      if (!rr_is_camped() && rr_is_this_the_pcell(channel_number))
      {
        /*******************************************************************************/
        ...
        tracking */
        rr_select_bcch_send_imsg_bsi_received_ind(si_message_received);
        ...
        if (pcell_info_ptr->gsm.generate_si_received_event)
        {
          ...
          if (ARE_SET(si_required_mask,
                      pcell_info_ptr->gsm.update_message_received))
          {
            rr_send_priority_imsg_rr_event_ind(RR_EV_GSM_SI_RECEIVED);

            rr_cell_acq_send_imsg_gsm_bsi_received_ind();
            ...
            rr_generate_complete_idle_BA_list(&pcell_info_ptr->gsm, IGNORE_RCVD_SI_MASK);
          }
          else
          {
          ...
          {
            rr_send_priority_imsg_rr_event_ind(RR_EV_EARLY_CAMP_SI_RECEIVED);
            rr_cell_acq_send_imsg_early_camping_bsi_received_ind();
          }
        }
      } /* RR is not camped */
      ...
      if (rr_candidate_db_is_candidate_cell(channel_number))
      {
        ...
        if ((message_type == SYSTEM_INFORMATION_TYPE_3)||(message_type == SYSTEM_INFORMATION_TYPE_4))
        {
...
#else
          rr_send_priority_imsg_rr_event_ind(RR_EV_CANDIDATE_CELL_SI_RECEIVED);
          ...
          // rr_decode_bcch_send_candidate_cell_si_ind();
          rr_decode_bcch_list_control(RR_EV_CANDIDATE_CELL_SI_RECEIVED, NULL);
#endif // FEATURE_FEMTO_GSM_NL
        }
      }
      ...
      surround_db_entry_ptr = rr_gprs_get_surround_ncell_gsm_sys_info(channel_number);
      ...
      if (surround_db_entry_ptr != NULL)
      {
           ...
           )
        {
          ...
          {
            rr_cell_acq_send_imsg_surrounding_si_received_ind();
       ...
       )
    {
      ...
      if (scell_info_ptr->gsm.valid_data_flag && !dedicated_si_message)
      {
        ...
        if (cell_priority == RR_CELL_BARRED)
        {
          ...
          rr_send_priority_imsg_rr_event_ind(EV_FORCE_CELL_RESELECTION);
        }
      }
      ...
      if (dedicated_si_message)
      {
        ...
        if (!rr_inter_ho_handover_in_progress())
        {
#ifdef FEATURE_RR_SI_CACHE
           ...
           )
        {
          ...
          rr_ded_set_measurement_reports_invalid(FALSE);
...
)
{
  ...
  if (cell_info_ptr == NULL)
  {
    MSG_GERAN_LOW_0_G("NULL pointer passed to handle_si2ter()");
    return FALSE;
  }

  /* Check whether RR should process SI2ter or ignore it */
  if (rr_should_ignore_si2ter())
  {
  ...
  else
  {
    ...
    if (&cell_info_ptr->gsm == &pcell_info_ptr->gsm)
    {
      ...
      else if (tc < 8) /* TC range is 0..7 on BCCH (TC=8 used for PACCH sys info) */
      {
         ...
         */
        if (rr_timer_is_running(RR_GSM_SYS_INFO_TIMER))
        {
          ...
          time_remaining_ms = (uint32)rr_timer_get_time_remaining(RR_GSM_SYS_INFO_TIMER);
          ...
          if (time_remaining_ms <= max_time_to_si2ter)
          {
            ...
            (void)rr_timer_extend(RR_GSM_SYS_INFO_TIMER, extension_ms);
...
)
{
  ...
  if (cell_info_ptr == NULL)
  {
    MSG_GERAN_LOW_0_G("NULL pointer passed to handle_si2quater()");
    return FALSE;
  }
  ...
  if ( si2q_pi == FALSE )
  {
    MSG_GERAN_MED_0_G("No SI2quater");
...
#ifdef FEATURE_GERAN_EAB
static void rr_handle_si21_indicator(
  boolean            si21_pi,
  uint8              si21_pos,
  boolean            siaw_flag,
  gprs_scell_info_t *cell_info_ptr
...
{
  /* SI3 indicates SI21 position */
  if (si21_pi == TRUE)
  {
    si21_position_T  si21_position;

    if (si21_pos == 0)
    {
      si21_position = SI21_POSITION_BCCH_Norm;
      
      /* Mark that SI21 is being broadcast on BCCH-Norm */
      bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_21, MASK_SI_NONE, MASK_SI_NONE);
    }
    else
    {
      si21_position = SI21_POSITION_BCCH_Ext;

      /* Mark that SI21 is being broadcast on BCCH-Ext */
      bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_NONE, MASK_SI_21, MASK_SI_NONE);
    }

    cell_info_ptr->gsm.si21_position = si21_position;

    /* set SI21 expected to ask L1 to receive SI21 later */
    if (!ARE_SET(MASK_SI_21, cell_info_ptr->gsm.update_message_received))
    {
      bsi_set_messages_required_for_acquisition(MASK_SI_21, MASK_SI_NONE, FALSE);
    }

    /* Mark SI9 as not being broadcast. */
    bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_NONE, MASK_SI_NONE, MASK_SI_9);
  }
  /* SI21 position is not present. We need to read SI9 to get SI21 scheduling info. */
  else
  {
    /* If SI9 is already read process the scheduling info */
    if (ARE_SET(MASK_SI_9, cell_info_ptr->gsm.update_message_received))
    {
      rr_process_si_scheduling_info(cell_info_ptr);
    }
    /* Otherwise ask GL1 to read it (only if SI9 is sent).*/
    else if (siaw_flag)
    {
      /* Mark SI9 is being broadcast */
      bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_9, MASK_SI_NONE, MASK_SI_NONE);
      bsi_set_messages_required_for_acquisition(MASK_SI_9, MASK_SI_NONE, FALSE);
    }
    else
    {
      /* Mark SI9 and SI21 as not being broadcast. */
      bsi_update_messages_being_broadcast(cell_info_ptr, MASK_SI_NONE, MASK_SI_NONE, MASK_SI_9 | MASK_SI_21);
      rr_initialize_SI21_instances();
    }
  }
}
...
boolean rr_ms_3g_sys_info_mandatory( )
{
#ifndef PERLUTF
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
  if (rr_multi_sim_mode_enabled())
  {
    ...
    return(geran_is_multi_mode_gas_id());
  }
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
