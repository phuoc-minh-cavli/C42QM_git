...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_x2g_resel.c#2 $ */
 ...
 *--------------------------------------------------------------------------*/
static rr_x2g_resel_data_t *rr_x2g_resel_data_ptrs = 0;
...
{
  uint8 as_index = GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX();
...
)
{
  ...
  rr_x2g_resel_data_ptr = rr_x2g_resel_get_data_ptr();
  ...
  switch (rr_x2g_resel_data_ptr->activation_reason)
  {
    ...
    {
      event = rr_mode_control(EV_ACTIVATE_GSM_MODE_FOR_X2G_RESEL, NULL);
      break;
    ...
    case MM_AS_LTOG_CCO:
    {
      event = rr_mode_control(EV_ACTIVATE_GSM_MODE_FOR_X2G_CCO, NULL);
      break;
...
static void rr_x2g_resel_deactivate_l1( )
{
  rr_x2g_resel_data_t *rr_x2g_resel_data_ptr;
  ...
  rr_l1_gsm_stop_reason_set(rr_x2g_resel_data_ptr->l1_stop_reason);

  /* advise the multi-SIM controller of this failure */
  rr_ms_x2g_procedure_failure(RR_X2G_RESELECTION_FAILURE);
  ...
  else
  {
    ...
    if (!plmn_ok)
    {
      ...
      plmn_ok = rr_eplmn_list_equivalent_plmn(requested_plmn_id) &&
                rr_eplmn_list_equivalent_plmn(target_cell_plmn_id);
    }
                    ...
                    case RR_CELL_ACQ_STATUS_SUCCESS_BCCH_EARLY_CAMPED:
                    {
                      ...
                      if (rr_is_sglte_mode_active())
                      {
                        ...
                        { /*note CCO does not have SGLTE specifc cause, hence no conversion*/
                          rr_send_activation_ind(rr_x2g_resel_data_ptr->activation_reason); 
                        }
                      }
                      else
#endif /*FEATURE_SGLTE*/
        ...
        case MS_MM_RR:
        {
          switch (message->message_header.message_id)
          {
            case RR_ACTIVATION_RSP:
            {
              ...
              rr_set_x2g_cco_act_rsp_rcvd(TRUE);
              plmn_suitable = rr_x2g_resel_is_plmn_suitable(
                rr_x2g_resel_data_ptr,
                rr_gprs_get_pcell_info()->gsm.location_area_identification.plmn_id,
              ...
              if (plmn_suitable)
              {
                ...
                rr_store_service_domain(message->rr_activation_rsp.service_domain);
                plmn_request_details->search_reason = RR_AUTONOMOUS_CELL_SELECTION;

                rr_store_plmn_id(&message->rr_activation_rsp.PLMN_id);
                ...
                {
                  rr_x2g_resel_send_omsg_resel_cnf(RR_X2G_RESEL_STATUS_SUCCESS);
          ...
          switch (message->rr.header.rr_message_set)
          {
            case RR_X2G_RESEL_IMSG:
            {
              switch (message->message_header.message_id)
              {
                case RR_X2G_RESEL_IMSG_L1_DEACTIVATED_IND:
                {
                  rr_x2g_resel_send_omsg_abort_cnf(rr_x2g_resel_data_ptr->activation_ind_sent);
                  rr_x2g_resel_data_ptr->state = RR_X2G_RESEL_STATE_NULL;
                  break;
                }
