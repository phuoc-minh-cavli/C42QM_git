 ...
 */
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_msc.c#5 $ */
...
static rr_msc_module_data_t rr_msc_module_data;
 ...
 */
static rr_msc_module_data_t *rr_msc_get_data_ptr(void)
{


  return &rr_msc_module_data;
}
 ...
 * 
 */
static void determine_new_msc(void)
{
  uint8 i;


  // Obtain a pointer to the module data
  rr_msc_module_data_t *module_data_ptr = rr_msc_get_data_ptr();
 ...
 */
boolean compare_msc_to_gmm_msc(rr_msc_data_t *msc_data_ptr)
{
...
)
{
      ...
      (module_data_ptr->valid))
  {
    ...
    if (compare_msc_to_gmm_msc(&module_data_ptr->new_msc_data))
    {
 ...
 * \return boolean - TRUE if the MSC has changed, FALSE otherwise
 */
boolean rr_msc_reduce_tx_slots(rr_msc_client_e_t msc_client)
{
  rr_msc_data_t *msc_data_ptr;
 ...
 * \return boolean - TRUE if the MSC has changed, FALSE otherwise
 */
boolean rr_msc_revert_to_orig(rr_msc_client_e_t msc_client)
{
...
void rr_msc_nv_ready(void)
{
  ...
  msc_data_ptr->gprs_multislot_class.msc_val = rr_get_nv_multislot_class();
  ...
  msc_data_ptr->egprs_multislot_class.msc_val = rr_get_nv_egprs_multislot_class();
...
#ifdef FEATURE_GSM_DTM
  msc_data_ptr->dtm_multislot_class.msc_val = rr_get_nv_dtm_multislot_class();
 ...
 * \param msg_ptr (in)
 */
void rr_msc_process_l1_coex_msc_change_ind(mph_coex_msc_change_ind_T *msg_ptr )
{
  boolean msc_changed;
 ...
 */
void rr_msc_update(boolean can_send_service_ind )
{
  ...
  rr_thermal_update(can_send_service_ind);
#endif
 ...
 * 
 */
void rr_msc_send_rr_imsg_msc_change_ind(void)
{
