...
============================================================================*/
/* $Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_band_avoid.c#2 $ */
 ...
 *--------------------------------------------------------------------------*/
static rr_band_avoid_t rr_band_avoid;

/*----------------------------------------------------------------------------
 * Static Function Declarations and Definitions
 *--------------------------------------------------------------------------*/
static void rr_band_avoid_update_blacklist(coex_ba_blist_data *blist_ptr );
static ARFCN_T rr_get_arfcn(uint32 freq, sys_band_T band );
static void rr_add_arfcns_to_blacklist(uint16 lower_arfcn, uint16 upper_arfcn, sys_band_T band );
 ...
 */
static rr_band_avoid_t *rr_band_avoid_get_data_ptr(void)
{
  return &rr_band_avoid;
}
 ...
 * \return void
 */
static void rr_band_avoid_start_avoidance( void )
{
  rr_band_avoid_t *rr_band_avoid_ptr = rr_band_avoid_get_data_ptr();
 ...
 */
static boolean rr_band_avoid_valid_ncells(sys_band_T bl_band )
{
  ...
  for (i=0; i<surrounding_measurement_ptr->no_of_entries; i++)
  {
    ...
    if (surrounding_measurement_ptr->measurement[i].BCCH_ARFCN.band != bl_band &&
        surrounding_measurement_ptr->measurement[i].RXLEV_average > 
        rr_get_band_avoid_ncell_threshold())
    {
      ncells_not_bl++;
    }
  }
 ...
 */
static boolean rr_band_avoid_is_ded_freq_blacklisted(sys_band_T *band, frequency_list_T *ded_freq )
{
  uint16 i, j = 0;
 ...
 * \return sys_band_T
 */
static sys_band_T rr_band_avoid_sys_band_to_internal_band(sys_band_class_e_type band )
{
   switch(band)
   {
 ...
 * \return void
 */
static void rr_band_avoid_check_frequencies(uint32 *lower_freq, uint32 *upper_freq, sys_band_T band )
{
...
static void rr_band_avoid_update_blacklist(coex_ba_blist_data *blist_ptr )
{
  ...
  for (i=0; i < blist_ptr->num_entries; i++)
  {
    ...
    blist_ptr->freq_list[i].band = rr_band_avoid_sys_band_to_internal_band(blist_ptr->freq_list[i].band);
        ...
        blist_ptr->freq_list[i].freq_upper == 0xffffffff)
    {
      ...
      {
        case SYS_BAND_EGSM_900:
          rr_add_arfcns_to_blacklist(EGSM_LOW_RANGE_LOWER_ARFCN, EGSM_LOW_RANGE_UPPER_ARFCN, SYS_BAND_EGSM_900);
          rr_add_arfcns_to_blacklist(EGSM_HIGH_RANGE_LOWER_ARFCN, EGSM_HIGH_RANGE_UPPER_ARFCN, SYS_BAND_EGSM_900);
        break;

        case SYS_BAND_PGSM_900:
          rr_add_arfcns_to_blacklist(PGSM_LOWER_ARFCN, PGSM_UPPER_ARFCN, SYS_BAND_PGSM_900);
        break;

        case SYS_BAND_PCS_1900:
          rr_add_arfcns_to_blacklist(PCS_LOWER_ARFCN, PCS_UPPER_ARFCN, SYS_BAND_PCS_1900);
        break;

        case SYS_BAND_DCS_1800:
          rr_add_arfcns_to_blacklist(DCS_LOWER_ARFCN, DCS_UPPER_ARFCN, SYS_BAND_DCS_1800);
        break;

        case SYS_BAND_CELL_850:
          rr_add_arfcns_to_blacklist(GSM_850_LOWER_ARFCN, GSM_850_UPPER_ARFCN, SYS_BAND_CELL_850);
        break;
      }
    }
    else /*only blacklist ARFCN's within specified band*/
    {
      ...
      lower_freq_arfcn = rr_get_arfcn(blist_ptr->freq_list[i].freq_lower, blist_ptr->freq_list[i].band);

      /*get ARFCN of upper frequency*/
      upper_freq_arfcn = rr_get_arfcn(blist_ptr->freq_list[i].freq_upper, blist_ptr->freq_list[i].band);
          ...
          upper_freq_arfcn.band != INVALID_BAND)
      {
        ...
        if (blist_ptr->freq_list[i].band == SYS_BAND_EGSM_900)
        {
          ...
          {
            rr_add_arfcns_to_blacklist(EGSM_LOW_RANGE_LOWER_ARFCN, lower_arfcn, blist_ptr->freq_list[i].band);
            rr_add_arfcns_to_blacklist(upper_arfcn, EGSM_HIGH_RANGE_UPPER_ARFCN, blist_ptr->freq_list[i].band);
          }
          else
          {
            /*lower and upper ARFCN are in same range*/
            rr_add_arfcns_to_blacklist(lower_arfcn, upper_arfcn, blist_ptr->freq_list[i].band);
          }
        }
 ...
 * \return void
 */
void rr_band_avoidance_control(rr_cmd_bdy_type * message)
{
  rr_band_avoid_t *rr_band_avoid_ptr = rr_band_avoid_get_data_ptr();
  ...
  if (rr_is_band_avoidance_enabled() == FALSE)
  {
    if (message->message_header.message_set == MS_MSGR_RR &&
        message->rr_msgr_msg.message.hdr.id == MCS_CXM_BAND_AVOID_BLIST_GSM1_IND)
    {
      MSG_GERAN_HIGH_0_G("RR_BA Band avoidance disabled send NACK"); 
      rr_band_avoid_send_cxm_blist_rsp(message->rr_msgr_msg.message.band_avoid_blacklist_ind.blacklist_data.blacklist_id);
    }
    return;
  }
        ...
        case MS_MSGR_RR:
        {
          switch (message->rr_msgr_msg.message.hdr.id)
          {
            ...
            case MCS_CXM_BAND_AVOID_BLIST_GSM2_IND:
            {
              ...
              {
                rr_band_avoid_update_blacklist(&message->rr_msgr_msg.message.band_avoid_blacklist_ind.blacklist_data);

                if (rr_band_avoid_is_ded_freq_blacklisted(&rr_band_avoid_ptr->blacklist.band, &rr_band_avoid_ptr->ded_freq))
                {

                  if (rr_band_avoid_valid_ncells(rr_band_avoid_ptr->blacklist.band))
                  {
                    MSG_GERAN_HIGH_0_G("RR_BA Current TCH is in blacklist, Ncells available, start avoiding");
                    rr_band_avoid_start_avoidance();
                  ...
                  else
                  {
                    ...
                    rr_band_avoid_send_cxm_blist_rsp(rr_band_avoid_ptr->blacklist_id);
                  }
                }
                else
                {
                  ...
                  rr_band_avoid_send_cxm_freq_ind(&rr_band_avoid_ptr->ded_freq);
                }
              }
              else if (message->rr_msgr_msg.message.band_avoid_blacklist_ind.blacklist_data.serv_freq_state == SERV_FREQ_OK)
              {
                ...
                rr_band_avoid_update_blacklist(NULL);
              }
              break;
            }
            ...
            case RR_BAND_AVOID_IMSG:
            {
              switch (message->message_header.message_id)
              {
                ...
                { 
                  ...
                  if (rr_band_avoid_ptr->meas_rpt_count > rr_get_band_avoid_max_meas_rpts())
                  {
 ...
 */
uint32 rr_band_avoid_arfcn_to_centre_freq_kHz(ARFCN_T arfcn )
{
   int32 freq = 0;
