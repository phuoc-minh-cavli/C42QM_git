...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_inter_ho.c#5 $ $DateTime: 2018/10/16 07:03:13 $ $Author: pwbldsvc $
...
static rr_inter_ho_data_t rr_inter_ho_data =
...
static void rr_ho_reset_internal_variables(handover_details_T *handover_details_ptr  );
#if defined (FEATURE_WCDMA) || defined (FEATURE_GSM_TDS)
static void rr_w2g_process_stored_nas_messages( );
static void rr_w2g_store_rrc_message(const rr_data_req_T *  );
static void rr_w2g_store_mm_message(const rr_data_req_T *  );
static void rr_w2g_empty_rrc_message_queue( );
static void rr_w2g_empty_mm_message_queue( );
#endif /* FEATURE_WCDMA || FEATURE_GSM_TDS*/

#ifdef FEATURE_WCDMA
static void rr_w2g_store_gsm_commands(const rr_interrat_handover_req_type *irat_ho_req );
...
static boolean rr_store_nas_data_ind(const void *  );
#endif /* FEATURE_WCDMA */

#ifdef FEATURE_LTE
static void rr_l2g_store_gsm_commands(const geran_grr_lte_ho_req_t *lte_ho_req  );
static geran_grr_lte_ho_failure_cause_type_e rr_convert_rr_x2g_ho_failure_cause_to_lte(rr_x2g_ho_failure_e rr_x2g_cause);
#endif /* FEATURE_LTE */

#ifdef FEATURE_GSM_TDS
static void rr_t2g_store_gsm_commands(tdsrr_interrat_handover_req_type *tds_ho_req  );
...
static boolean rr_store_tds_nas_data_ind(const void *message_ptr  );
#endif /*FEATURE_GSM_TDS*/
 ...
 */
static rr_inter_ho_data_t *rr_inter_ho_get_data_ptr( void )
{
  return &rr_inter_ho_data;
}
...
)
{
  switch (rr_l1_handover_type)
  {
    ...
    default:
    {
      MSG_LOW("Unexpected handover type: %d", 0, 0, 0);
 ...
 */
static rr_event_T deactivate_during_x2g_handover( void )
{
  /* advise the multi-SIM controller of the failure */
  rr_ms_x2g_procedure_failure(RR_X2G_HANDOVER_FAILURE);
...
)
{
  ...
  rr_inter_ho_data_ptr = rr_inter_ho_get_data_ptr();
...
)
{
  if (dedic_ptr != NULL)
  {
    ...
    rr_set_traffic_reason_from_channel_mode(dedic_ptr->curr_channel_spec.channel_mode_1);
...
static void rr_send_lte_abort_ho_rsp( )
{
  ...
  result = rr_msgr_send(&rsp, sizeof(rsp));
...
)
{
  ...
  if ((decoding_ok == TRUE) && (handover_failure == FALSE))
  {
    ...
    /******************************************************************/
    if (rr_channel_mode_supported(handover_details_ptr->channel_spec.channel_mode_1) == FALSE)
    {
    ...
    else if (handover_details_ptr->channel_spec.number_of_ded_channels == 2)
    {
      ...
      if (rr_channel_mode_supported(handover_details_ptr->channel_spec.channel_mode_2) == FALSE)
      {
    ...
    switch (handover_details_ptr->handover_type)
    {
      case RR_L1_GSM_GSM:
      {
        ...
        if (start_ciphering == TRUE)
        {
          if (rr_get_ciphering_mode_command_received())
          {
            rr_ciphering_has_started(TRUE);
          }
          else
          {
      ...
      case RR_L1_LTE_GSM:
      {
        ...
        rr_set_ciphering_mode_command_received(TRUE);
    ...
    if (decoding_ok &&
        (rr_check_frequency_lists_supported(&(handover_details_ptr->channel_spec)) == FALSE)
       )
    {
    ...
    if (handover_failure || !decoding_ok)
    {
      ...
      if (decoding_ok)
      {
        ...
        rr_log_handover_failure(RR_LOG_HO_FAILURE_CONFIGURATION_NOT_SUPPORTED);
      }
      else /* decoding was not OK */
      {
        /* must be a protocol error */
        rr_log_handover_failure(RR_LOG_HO_FAILURE_PROTOCOL_ERROR);
      }
      /****************************************************/
      ...
      switch (handover_details_ptr->handover_type)
      {
        ...
        default:
        {
          MSG_GERAN_LOW_1_G("Invalid handover type=%d",(int) handover_details_ptr->handover_type);
          break;
        }
      }
    }
...
*/
boolean rr_inter_ho_handover_in_progress( )
{
  // Obtain a pointer to the module data
  rr_inter_ho_data_t *rr_inter_ho_data_ptr = rr_inter_ho_get_data_ptr();
...
*/
boolean rr_inter_ho_w2g_starting( )
{
  // Obtain a pointer to the module data
  rr_inter_ho_data_t *rr_inter_ho_data_ptr = rr_inter_ho_get_data_ptr();
...
)
{
  ...
  while (handover_event != EV_NO_EVENT)
  {
    ...
    else if (handover_event == EV_RESET_SOFTWARE)
    {
      ...
      rr_ho_reset_internal_variables(handover_details_ptr);
    } /* EV_RESET_SOFTWARE */
        ...
        if ( (handover_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          if ((message_set == MS_RR_L2) && (message_id == (byte) DL_DATA_IND))
          {
            ...
            if (dl_data_ind->layer3_message[1] == HANDOVER_COMMAND)
            {
              ...
              rr_inter_ho_data_ptr->old_band = rr_get_current_dedicated_band();

              new_handover_event = rr_respond_to_handover_command(dedic_ptr, FALSE);
            }
          } /* MS_RR_L2 */
#ifdef FEATURE_WCDMA
          else if ((message_set == MS_RRC_RR) && (message_id == (byte) RR_INTERRAT_HANDOVER_REQ))
          {
            ...
            rr_w2g_store_gsm_commands(rr_interrat_handover_req);
            ...
            if (rr_x2g_ho_gsm_cmd_ptr->payload[1] == HANDOVER_COMMAND)
            {
              ...
              if (message_ok == TRUE)
              {
                ...
                {
                  new_handover_event = rr_respond_to_handover_command(dedic_ptr, TRUE);
                }
                else
                {
                  new_handover_event = rr_respond_to_handover_command(dedic_ptr, FALSE);
          ...
          else if ((message_set == MS_MM_RR) && (message_id == (byte) RR_ABORT_REQ))
          {
            ...
            rr_send_abort_cnf(MM_AS_INTERRAT_HANDOVER);
            rr_inter_ho_data_ptr->handover_state = HANDOVER_INACTIVE;
            rr_ho_reset_internal_variables(handover_details_ptr);
          }
#endif /* FEATURE_WCDMA */
          ...
          else if ((message_set == MS_TDSRRC_RR) && (message_id == (byte) TDSRR_INTERRAT_HANDOVER_REQ))
          {
            ...
            /* Store the GSM command info from the inter-RAT handover request */
            rr_t2g_store_gsm_commands(tdsrr_interrat_handover_req);
          ...
          else if (message_set == MS_MSGR_RR)
          {
            switch (new_message->rr_msgr_msg.message.hdr.id)
            {
              ...
              case GERAN_GRR_LTE_HO_REQ:
              {
                ...
                /* Perform the handover details store initialisation */
                /*****************************************************/
                rr_init_handover_details(handover_details_ptr, RR_L1_LTE_GSM);
                ...
                /* Store the GSM command info from the inter-RAT handover request */
                rr_l2g_store_gsm_commands(lte_ho_req);
        ...
        else if (handover_event == EV_HANDOVER_COMMAND_CHECKED)
        {
          switch (handover_details_ptr->handover_type)
          {
            ...
            case RR_L1_LTE_GSM:
            {
              ...
              new_handover_event = rr_mode_control(EV_ACTIVATE_GSM_MODE, NULL);
    ...
    case RR_W2G_HO_GOING_ACTIVE :
      ...
      if ( (handover_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
      {
        if ((message_set == MS_MM_RR) && (message_id == (byte) RR_ABORT_REQ))
        {
          ...
          new_handover_event = deactivate_during_x2g_handover();
      ...
      {
        // Suspend the L2 link
        rr_send_dl_suspend_req(handover_details_ptr->handover_type);
        rr_inter_ho_data_ptr->handover_state = AWAITING_LOCAL_END_SUSPENSION;
        ...
        if ( (handover_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          if (message_set == MS_RR_L2)
          {
                     ...
                     (message_id == (byte) MDL_ERROR_IND))
            {
              ...
              rr_log_handover_failure(RR_LOG_HO_FAILURE_PHYSICAL_CHANNEL_FAILURE);
          ...
          if ( message_set == MS_RR_L1 )
          {
            if (message_id == (byte) MPH_HANDOVER_IND)
            {
              ...
              if ( mph_handover_ind->error_code == NO_ERROR )
              {
                switch (handover_details_ptr->handover_type)
                {
                  case RR_L1_GSM_GSM:
                  {
                    if ((handover_details_ptr->BCCH_ARFCN.num !=
                      ...
                      !rr_bands_equal(rr_inter_ho_data_ptr->old_band, rr_get_current_dedicated_band()))
                    {
                      rr_clear_si_sacch();
                 ...
                 */
                rr_process_handover_indication(dedic_ptr);
            ...
            if (message_id == (byte) DL_RANDOM_ACCESS_CNF)
            {
              ...
              else
              {
                    ...
                    channel_info_1_after.channel_type == SDCCH_8)
                {
                  ...
                  (void)rr_timer_start(T3124, (dword)T3124_DURATION_SDCCH);
                }
                else
                {
                  /**********************************************/
                  ...
                  (void)rr_timer_start(T3124, (dword)T3124_DURATION_OTHER);
                }
              ...
              if (mph_handover_cnf->error_code == NO_ERROR)
              {
                ...
                switch (handover_details_ptr->handover_type)
                {
                  case RR_L1_GSM_GSM:
                  {
                    ...
                    rr_complete_handover(handover_details_ptr);
                  ...
                  default:
                  {
                    MSG_LOW("Invalid Handover Type = %d",(int)handover_details_ptr->handover_type,0,0);
                    break;
                  }
                }
              }
     ...
     case AWAITING_L2_RESUME_CONFIRM:
            ...
            if (message_id == (byte) DL_RESUME_CNF)
            {
              ...
              switch (handover_details_ptr->handover_type)
              {
                case RR_L1_GSM_GSM:
                {
                  ...
                  /***********************************************************************/
                  rr_send_handover_sync_ind(dedic_ptr);
                ...
                case RR_L1_LTE_GSM:
                {
                  ...
                  {
                    rr_send_activation_ind(MM_AS_LTOG_SGLTE_SRVCC_CS_HANDOVER);
                  }
                  else
#endif /*FEATURE_SGLTE*/
                  {
                    rr_send_activation_ind(MM_AS_LTOG_SRVCC_CS_HANDOVER);
                  }
          ...
          if (message_set == MS_RR_L2)
          {
            if (message_id == (byte) DL_UNIT_DATA_IND)
            {
              ...
              if (dl_unit_data_ind->layer3_message[1] == PHYSICAL_INFORMATION)
              {
                ...
                rr_send_mph_stop_handover_access_req(timing_advance);

                MSG_GERAN_HIGH_0_G("Stopping T3124");
                (void)rr_timer_cancel(T3124);
        ...
        if (handover_event == EV_HANDOVER_COMPLETED)
        {
          ...
          */
            ...
            case RR_L1_GSM_GSM:
            {
                ...
                !rr_bands_equal(rr_inter_ho_data_ptr->old_band, rr_get_current_dedicated_band()))
              {
                ...
                */
                ...
                if (!rr_bands_equal(rr_inter_ho_data_ptr->old_band, rr_get_current_dedicated_band()))
                {
                  ...
                  */
                  rr_swap_old_si5_and_si5ter_ba_lists();
                }
              }
              else
              {
            ...
            case RR_L1_LTE_GSM:
            {
              ...
              */
              ...
              if( rr_is_sglte_mode_active() && 
                  (ARFCNS_EQUAL(handover_details_ptr->BCCH_ARFCN,rr_sglte_get_stored_serv_ARFCN())) && 
                   rr_sglte_saved_l2g_ho_being_processed()
                )  
              {
                MSG_GERAN_HIGH_0_G("SGLTE SRVCC on same cell, set ded mode param to SI2q"); 
                rr_dedicated_mode_process_initial_params(scell_info_ptr,rr_get_instances_ptr(SI2Q_INSTANCE));
              }
              else
 #endif /*FEATURE_SGLTE*/
              {
                MSG_GERAN_HIGH_0_G("Set ded mode param to default value"); 
                rr_dedicated_mode_process_initial_params(scell_info_ptr, NULL);
...

          {
            ...
            {
              rr_band_avoid_ded_freq_upd_send_imsg(&handover_details_ptr->channel_spec.channel_info_1_after.frequency_list);
            }
      ...
      case HO_AWAITING_PHYSICAL_RECONNECTION :
        ...
        if ( (handover_details_ptr->handover_type != RR_L1_GSM_GSM) &&
               (handover_details_ptr->handover_type != RR_L1_GSM_WCDMA))
        {
          MSG_GERAN_LOW_0_G("Unknown handover_type!!!");
        }
            ...
            */
            ...
            if (message_id == (byte) MPH_RECONNECT_CHANNEL_CNF)
            {
              ...
              if (handover_details_ptr->handover_type == RR_L1_GSM_WCDMA)
              {
                ...
                rr_inter_ho_data_ptr->handover_cause = rr_g2w_map_handover_failure_cause();
        ...
        if ( (handover_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
        {
          if (message_set == MS_RRC_RR)
          {
            ...
            if (message_id == (byte) RR_NAS_DATA_IND)
            {
              ...
              /*************************************************************/
              if (rr_store_nas_data_ind(new_message) == FALSE)
              {
                rr_send_activation_ind(MM_AS_INTERRAT_HANDOVER);
                rr_inter_ho_data_ptr->handover_state = RR_W2G_HO_WAITING_FOR_ACTIVATION_RSP;
              }
            }
          ...
          else if (message_set == MS_TDSRRC_RR)
          {
            if (message_id == (byte) TDSRR_NAS_DATA_IND)
            {
              ...
              /* NAS messages to be sent                                   */
              /*************************************************************/
              if (rr_store_tds_nas_data_ind(new_message) == FALSE)
              {
                rr_send_activation_ind(MM_AS_INTERRAT_HANDOVER);
                rr_inter_ho_data_ptr->handover_state = RR_W2G_HO_WAITING_FOR_ACTIVATION_RSP;
              }
      ...
      if ( (handover_event == EV_INPUT_MESSAGE) && (new_message != NULL) )
      {
        ...
        else if (message_set == MS_MM_RR)
        {
          if (message_id == (byte) RR_DATA_REQ)
          {
            ...
            rr_w2g_store_mm_message(&rr_data_req);
          }
          else if (message_id == (byte) RR_ACTIVATION_RSP)
          {
            ...
            rr_w2g_process_stored_nas_messages();
            ...
            rr_store_service_domain(rr_activation_rsp->service_domain);
...
)
{
  ...
  */
  ...
  if (rr_inter_ho_data_ptr->rr_w2g_rrc_msg_queue != NULL)
  {
    ...
    rr_w2g_empty_rrc_message_queue();
  }
...
static void rr_w2g_process_stored_nas_messages( )
{
  ...
  if (rr_inter_ho_data_ptr->rr_w2g_rrc_msg_queue != NULL)
  {
    ...
    {
      rr_process_rr_data_req(&rrc_message->nas_message);
  ...
  if (rr_inter_ho_data_ptr->rr_w2g_nas_msg_queue != NULL)
  {
    ...
    {
      rr_process_rr_data_req(&nas_message->nas_message);
...
void rr_set_handover_type(rr_l1_handover_type_T ho_type  )
{
  rr_dedicated_data_T *dedic_ptr;
