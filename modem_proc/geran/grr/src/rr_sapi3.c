...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_sapi3.c#4 $ $DateTime: 2018/10/11 08:21:00 $ $Author: pwbldsvc $
...
static rr_sapi3_data_t rr_sapi3_data =
...
===========================================================================*/
static rr_sapi3_data_t *rr_sapi3_get_data_ptr( void )
{
  return &rr_sapi3_data;
}
...
===========================================================================*/
static rr_sapi3_callback_entry_T * rr_sapi3_get_free_list_entry( void )
{
  rr_sapi3_data_t           *rr_sapi3_data_ptr = rr_sapi3_get_data_ptr();
...
)
{
  ...
  rr_set_link_status(SAPI0_AND_SAPI3_LINK);
...
)
{
  ...
  rr_sapi3_process_outstanding_callbacks();
...
)
{
  ...
  {
    rr_sapi3_callback_entry_T *entry_ptr = rr_sapi3_get_free_list_entry();
...
)
{
  ...
  if (rr_sapi3_data_ptr->pending_l3_message_count < MAX_RR_SAPI3_REQUESTS)
  {
    ...
    if (pend_msg->msg != NULL)
    {
      ...
      MSG_GERAN_LOW_1_G("L3 message stored in pending list (count=%d)", rr_sapi3_data_ptr->pending_l3_message_count);
    }
    else
    {
  ...
  {
    MSG_GERAN_LOW_0_G("Overflow detected!!! Not possible to store L3 message");
  }
}
...
)
{
  ...
  rr_sapi3_remove_pending_l3_messages();
}
...
)
{
  ...
  if (sapi == SAPI3)
  {
    ...
    access_reason = rr_ce_access_reason();
...
)
{
  ...
  l2_channel_type = rr_get_l2_channel_type();
  ...
  else if ((message_set == MS_RR_L2) && (message_id == (byte) MDL_ERROR_IND))
  {
    ...
    /**************************************************/
    rr_sapi3_remove_pending_l3_messages();
    return(rr_sapi3_handle_mdl_error(error_sapi, error_code, error_chan));
  }
  ...
  switch (rr_sapi3_data_ptr->SAPI3_link_state)
  {
    case NO_SAPI3_LINK:
    {
      if ((message_set == MS_MM_RR) && (message_id == (byte) RR_DATA_REQ))
      {
        ...
        rr_sapi3_initialise_message_tracking();
        
        rr_sapi3_add_pending_l3_message(rr_data_req);
    ...
    case SAPI3_LINK_PENDING:
    {
      if ((message_set == MS_RR_L2) && (message_id == (byte) DL_ESTABLISH_CNF))
      {
        ...
        rr_sapi3_process_pending_l3_messages(l2_channel_type);
      }
    ...
    case SAPI3_LINK_ESTABLISHED:
    {
      ...
      else if ((message_set == MS_MM_RR) && (message_id == (byte) RR_DATA_REQ))
      {
        ...
        l3_message_length = rr_sapi3_process_rr_data_req(rr_data_req);
    ...
    default:
      MSG_GERAN_LOW_1_G("Unknown SAPI3 State: %d",(int)rr_sapi3_data_ptr->SAPI3_link_state);
      break;
  } /* switch (rr_sapi3_data_ptr->SAPI3_link_state) */
...
)
{
  ...
  entry_ptr = rr_sapi3_find_list_entry_for_message(l3_message_number);
...
)
{
  ...
  (void)rr_control_SAPI3_link(&reset_msg);
...
)
{
      ...
      (rr_sapi3_data_ptr->SAPI3_link_state == SAPI3_LINK_PENDING))
  {
    rr_send_dl_release_req(
      ...
      SAPI3,                            // SAPI
      rr_get_l2_channel_type()   // l2_channel_type
    );
