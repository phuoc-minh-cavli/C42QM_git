...
$Header: //components/rel/geran.mpss/5.2.0/grr/src/rr_cipher.c#4 $ $DateTime: 2018/10/11 08:21:00 $ $Author: pwbldsvc $
...
static cipher_state_data_t  cipher_state_data =
 ...
 */
static cipher_state_data_t *rr_cipher_get_data_ptr( void )
{
  return &cipher_state_data;
}
...
===========================================================================*/
boolean rr_get_ciphering_mode_command_received( void )
{
  rr_dedicated_data_T *dedic_ptr;
...
byte rr_get_cipher_key_sequence_number( )
{
  ...
  if(!rr_siminfo_get_cipher_key_seq(&ciph_key_seq_no))
  {
    /*******************************************************/
...
{
  if ((rr_get_cipher_key_sequence_number() != RR_CIPHER_KEY_SEQ_NOT_AVAILABLE) &&
      (ciph_key_ptr != NULL))
  {
    return(rr_siminfo_get_cipher_key(ciph_key_ptr,destsize_64,cipher_key_128,destsize_128));
  }
...
)
{
  ...
  if (rr_algorithm_support_status(cipher_mode) == RR_NORMAL_EVENT)
  {
    supported = TRUE;
  }
  else
...
static rr_event_T rr_respond_to_cipher_command( )
{
  ...
  current_IE = rr_get_first_ie_of_message(CIPHERING_MODE_COMMAND);
  ...
  if (current_IE != NULL)
  {
    ...
    if (current_IE->start_IE != NULL)
    {
      ...
      if ((*(current_IE->start_IE) & 0x01) == 0x01)
      {
        ...
        rr_status = rr_algorithm_support_status(algorithm_identifier);

        if (rr_status == RR_NORMAL_EVENT)
        {
          if (TRUE == geran_ciphering_logging_enabled())
          {
...

        {
          ...
          geran_eng_mode_data_write(ENG_MODE_GSM_CIPHER_INFO,(void*)&gsm_cipher_info, TRUE);
        }
      }
      else
      {
      ...
      if (decoding_ok)
      {
        ...
        rr_ciphering_has_started(start_ciphering);
             ...
             (start_ciphering == FALSE)))
        {
          ...
          key_available = rr_get_cipher_key(cipher_key,sizeof(cipher_key),cipher_key_128,sizeof(cipher_key_128));

          if (key_available == FALSE)
          {
            if (start_ciphering == TRUE)
            {
              rr_send_status(RR_PROTOCOL_ERROR_UNSPECIFIED);
            }
          ...
          {
            (void)rr_log_ciphering_mode((byte) start_ciphering,(byte) algorithm_identifier);

            rr_send_mph_set_ciphering_req(start_ciphering,algorithm_identifier,
                                           cipher_key,cipher_key_128);
...
static boolean rr_complete_ciphering( )
{
  ...
  if (rr_ce_access_reason() == (byte)MM_RR_SHORT_MESSAGE)
  {
    if (rr_send_signalling_msg_with_ack(layer3_message, msg_len, RR_MSG_ID_CIPHER_MODE_COMPLETE))
    {
  ...
  else
  {
    ...
    (void)rr_send_signalling_msg_with_ack(layer3_message, msg_len, 0);
  }
...
)
{
  ...
  cipher_state_data_t *cipher_state_data_ptr = rr_cipher_get_data_ptr();
        ...
        if (cipher_event == EV_INPUT_MESSAGE)
        {
          if ((message_set == MS_RR_L2) && (message_id == (int) DL_DATA_IND))
          {
            ...
            if (data_ind_ptr->layer3_message[1] == CIPHERING_MODE_COMMAND)
            {
              ...
              /**********************************************/
              new_cipher_event = rr_respond_to_cipher_command();
            }
          }
        }
