...
$Header: //components/rel/geran.mpss/5.2.0/gdiag/src/gsmdiag.c#7 $
...
void L1_send_MPH_SIMULATE_R13_PARAM_FROM_DIAG_IND(uint8 simu_pro, uint8 simu_param_t );
boolean gl1_simulate_rr_r13_peo_params(void);
boolean gl1_simulate_rr_r13_eab_params(void);
boolean gl1_simulate_gl1_ptm_drx_params(void);
extern void gl1_set_ps_page_in_empty_page(void);
...
void gsmdiag_set_geran_state(const gsm_state_t gsm_state )
{
  gsmdiag_cached_gsm_state[GERAN_ACCESS_STRATUM_ID_1] = gsm_state;
}
    ...
    gprs_log_packet_request_desc *desc_ptr =
      &gprs_log_packet_request_dispatch_table[entry];
        ...
        if (desc_ptr->log_code == log_code)
        {
            ...
            {
                MSG_GERAN_HIGH_1( "lpr_reg> code %x already has same handler", log_code);
            }
            else
            {
                if (handler == NULL)
    ...
    gprs_log_packet_request_desc *desc_ptr =
      &gprs_log_packet_request_dispatch_table[entry];
        ...
        if (desc_ptr->log_code == 0x0000)
        {
            ...
            MSG_GERAN_HIGH_2( "lpr_reg> code %x handler offset %d", log_code, entry);
            return(TRUE);
        }
    }
...
void gsmdiag_deinit (void)
{
  DIAGPKT_DISPATCH_TABLE_DEREGISTER((word)DIAG_SUBSYS_GSM, gsmdiag_tbl);
}
