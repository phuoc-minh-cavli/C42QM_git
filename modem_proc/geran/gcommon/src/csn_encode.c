...
$Header: //components/rel/geran.mpss/5.2.0/gcommon/src/csn_encode.c#2 $
  ...
  packet_resource_request_t *msg_ptr)
{
  ...
  #define REL_10_EXT_WITH_LOW_PRIO_ONLY 11
  ...
  if(msg_ptr->include_egprs == TRUE)
  {
    ...
    if (prr_size_ok)
    {
      ...
      /* sanity check to include Rel 10 Low Access Priority Signalling */
      if (*pos_ptr <= (MAX_PRR_SIZE - REL_10_EXT_WITH_LOW_PRIO_ONLY))
      {
        /* set Additions for Rel-5 */
        gpackb(1, src, pos_ptr, 1);
        /* G-RNTI extension & Iu mode indicator flag */
        gpackb(0, src, pos_ptr, 1);
        /* HFN_LSB indicator flag */
        gpackb(0, src, pos_ptr, 1);

        /* set Additional contents for Release 6 */
        gpackb(1, src, pos_ptr, 1);
        /* Extended Channel Request Description indicator flag */
        gpackb(0, src, pos_ptr, 1);

        /* set Additional contents for Release 7 */
        gpackb(1, src, pos_ptr, 1);
        /* EARLY_TBF_ESTABLISHMENT */
        gpackb(0, src, pos_ptr, 1);
        /* EGPRS BEP Link Quality Measurements Type 2 indicator flag */
        gpackb(0, src, pos_ptr, 1);
        /* EGPRS Timeslot Link Quality Measurements Type 2 indicator flag */
        gpackb(0, src, pos_ptr, 1);
        
        /* set Additional contents for Release 10 */
        gpackb(1, src, pos_ptr, 1);
        /* Low Access Priority Signalling */
        gpackb(msg_ptr->lap_signalling, src, pos_ptr, 1);
        MSG_GERAN_HIGH_1("Low Access Priority Signalling(%d) added to PRR", msg_ptr->lap_signalling);
      }
      else
      {
        MSG_GERAN_ERROR_1("Less space to add Low Access Priority Signalling, curr position=%d", *pos_ptr);
      }
    }
  }
  else
  ...
  channel_req_desc_t *msg_ptr)
{
  ...
  {
    MSG_GERAN_LOW_0("CSN1: RADIO PRIORITY INVALID - 0");
