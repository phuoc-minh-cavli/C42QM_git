...
$Header: //components/rel/geran.mpss/5.2.0/gcommon/src/csn_decode.c#4 $
...
***
*****************************************************************************/
#define CSN_MCS_UNDEF 11

#define CSN_BAND_INITIALISER(arfcn_struct) \
  {(arfcn_struct).band = (sys_band_T)rr_get_band((arfcn_struct).num);}
...
static csn_decoder_error_t csn_decode_iax_rest_octets(uint8 *src, iax_rest_octets_t *dst);
...
)
{
  ...
  {
    MSG_GERAN_LOW_1("CSN PSI Type=0x%02X",mess_type);
  }
  ...
  if (return_status == CSN_DECODE_NO_ERROR)
  {
    ...
    if (!(gunpackb(src, pos_ptr, (int16)1)))
    {
      ...
      if (gunpackb(src, pos_ptr, (int16)1))
      {
        ...
        return_status = csn_frequency_params_ie(src, pos_ptr, &(msg_ptr->freq_params));
...
static void csn_decode_cdma2000_cell(uint8 src[], uint16 *pos_ptr, cdma2000_cell_t *msg_ptr)
{
  ...
  if (gunpackb(src, pos_ptr, (uint16) 1))
  {
    ...
    case 6:
      ...
      msg_ptr->aux_walsh_len = gunpackw(src, pos_ptr, (uint16)(msg_ptr->walsh_len_c + 6));
      if (gunpackb(src, pos_ptr, (uint16) 1))
      {
        ...
        msg_ptr->aux_pilot_walsh1 = gunpackw(src, pos_ptr, (uint16)(msg_ptr->walsh_length1 + 6));
      }
      else
      {
      ...
      if (gunpackb(src, pos_ptr, (uint16) 1))
      {
        ...
        msg_ptr->aux_pilot_walsh2 = gunpackw(src, pos_ptr, (uint16)(msg_ptr->walsh_length2 + 6));
      }
      else
      {
...
)
{
  ...
  if (src == NULL)
  {
    MSG_LOW("csn_decode_gsm_priority_params_ie: src == NULL", 0, 0, 0);
    return;
  }

  if (pos_ptr == NULL)
  {
    MSG_LOW("csn_decode_gsm_priority_params_ie: pos_ptr == NULL", 0, 0, 0);
    return;
  }
...
)
{
  ...
  if (src == NULL)
  {
    MSG_LOW("csn_decode_utran_priority_params_ie: src == NULL", 0, 0, 0);
    return;
  }

  if (pos_ptr == NULL)
  {
    MSG_LOW("csn_decode_utran_priority_params_ie: pos_ptr == NULL", 0, 0, 0);
    return;
  }
...
)
{
  ...
  if (src == NULL)
  {
    MSG_LOW("src == NULL", 0, 0, 0);
    return;
  }

  if (pos_ptr == NULL)
  {
    MSG_LOW("pos_ptr == NULL", 0, 0, 0);
    return;
  }
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    {
      MSG_GERAN_LOW_1("Invalid PCID: %d", pcid);
    }
  }
  ...
  if (gunpackb(src, pos_ptr, 1))
  {
    ...
    for (n = 1; n < 7; ++n)
    {
      ...
      if (pcid_bitmap_group & (1 << (n - 1)))
      {
        uint32 lower = (n - 1) * 84;
        uint32 upper = (n * 84) - 1;
        MSG_LOW("PCID Bitmap Group %d is in PCID group (from %d to %d inclusive)", n, lower, upper);
...
)
{
  if (src == NULL)
  {
    MSG_LOW("csn_decode_eutran_params_ie: src == NULL", 0, 0, 0);
    return;
  }

  if (pos_ptr == NULL)
  {
    MSG_LOW("csn_decode_eutran_params_ie: pos_ptr == NULL", 0, 0, 0);
    return;
  }

  if (dst == NULL)
  {
    MSG_LOW("csn_decode_eutran_params_ie: dst == NULL", 0, 0, 0);
    return;
  }
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    while (gunpackb(src, pos_ptr, 1))
    {
      ...
      {
        MSG_LOW("Discarding EARFCN, overflow", 0, 0, 0);
      }
    }
    ...
    {
      MSG_LOW("Discarding Repeated E-UTRAN Neighbour Cells, overflow", 0, 0, 0);
    }
  }
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    while (gunpackb(src, pos_ptr, 1))
    {
      ...
      {
        MSG_GERAN_LOW_1("Invalid eutran_frequency_index: %d", eutran_frequency_index);
      }
    }
    ...
    {
      MSG_GERAN_LOW_0("Overflow decoding E-UTRAN Not Allowed Cells");
    }
  }
...
)
{
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    {
      MSG_GERAN_LOW_1("Invalid PSC: %d", psc);
    }
  }
...
)
{
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    {
      MSG_GERAN_LOW_0("Overflow decoding UTRAN CSG Description");
    }
  }
...
)
{
  ...
  while (gunpackb(src, pos_ptr, 1))
  {
    ...
    {
      MSG_GERAN_LOW_0("Overflow decoding E-UTRAN CSG Description");
    }
  }
...
)
{
  ...
  if (gunpackb(src, pos_ptr, 1))
  {
    ...
    do
    {
      ...
      {
        MSG_GERAN_LOW_1("Invalid eutran_frequency_index: %d", frequency_index);
      }
    } while (gunpackb(src, pos_ptr, 1));
  }
  ...
  if (gunpackb(src, pos_ptr, 1))
  {
    ...
    do
    {
      ...
      {
        MSG_GERAN_LOW_1("Invalid utran_frequency_index: %d", frequency_index);
      }
    } while (gunpackb(src, pos_ptr, 1));
  }
  ...
  if (!(gunpackb(src, pos_ptr, (int16)2)))
  {
    ...
    else
    {
        ...
        if (!(gunpackb(src, pos_ptr, (int16)2)))
        {
          ...
          if (msg_ptr->egprs_chan_coding_cmd >= CSN_MCS_UNDEF)
          {
            return NON_DISTRIBUTION_ERROR;
          }
...
static void csn_decode_optional_extension_info(uint8 src[], uint16 *pos_ptr, optional_extension_info_t *msg_ptr)
{
  ...
  MSG_GERAN_LOW_3("Rel6 flags MTBF:%1d EXTULTBF:%1d EDTM:%1d",
   ...
   */
  remaining_length = length + padding; 
  while (remaining_length > 0)
  {
    if (remaining_length >= 16)
    {
      (void) gunpackw(src, pos_ptr, 16);
      remaining_length -= 16;
    }
    else if(remaining_length > 8)
    {
     (void) gunpackw(src, pos_ptr, remaining_length);
      remaining_length = 0;
    }
    else
    {
      (void) gunpackb(src, pos_ptr, remaining_length);
      remaining_length = 0;
    }
  }
}
...
)
{
  ...
  switch (message_type)
  {
    ...
    case P1_REST_OCTETS:
      return csn_decode_p1_rest_octets(src, (p1_rest_octets_t *) dst);
    case P2_REST_OCTETS:
    ...
    case PACKET_CHANNEL_DESCRIPTION:
      return csn_decode_packet_channel_description(src, (packet_channel_description_t *) dst);
    case IA_REST_OCTETS:
    ...
    case IAX_REST_OCTETS:
      return csn_decode_iax_rest_octets(src, (iax_rest_octets_t *) dst);	  
#ifdef FEATURE_GSM_DTM
...
static csn_decoder_error_t csn_decode_si3_rest_octets(uint8 *src, si3_rest_octets_t *dst)
{
 ...
 /* <Iu Indicator> is conditional and is included if and only if GPRS is not supported and Iu mode is supported in the cell.
  * It makes difficult to get "Iu mode" as it comes in a different IE and to get that value here additional changes needed.
  * If <Iu Indicator> is present but we are not decoding then SI21_POSITION will be decoded with wrong value. 
  * This should be OK as later we will not camp on this cell as GPRS won't be supported in this cell. 
  * Hence, completely ignoring <Iu Indicator> decoding and assuming it is not present. */

 /* decode <System Information 21 Indicator> */
 dst->si21_indicator_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->si21_indicator_flag)
 {
   dst->si21_position = gunpackb(src, &msg_pos, CSN_SI21_POSITION_LEN);
 }
    ...
    if (dst->sgsnr_flag == TRUE)
    {
      ...
      if (dst->r4_additions_flag == TRUE)
      {
        ...
        if (dst->r6_additions_flag == TRUE)
        {
          ...
          /*RELEASE 11 addition: SI_CHANGE_ALT support*/
          dst->si_change_alt = GUNPACKS_LH(src, &msg_pos);
                  
          dst->peo_params.peo_supported_flag = FALSE;
          dst->peo_params.c1_delta_present   = FALSE;

           /*RELEASE 13 addition: PEO support*/  
          if (gunpackb(src, &msg_pos, 1) == 1)
          {
            dst->peo_params.peo_supported_flag = TRUE;
            dst->peo_params.peo_dsc            = gunpackb(src, &msg_pos, CSN_PEO_DSC_LEN);          
            /*RELEASE 13 addition: RCC support*/
            dst->peo_params.rcc                  = gunpackb(src, &msg_pos, CSN_RCC_LEN);
          }
  
          /*RELEASE 13 addition: C1 DELTA support*/
          if (gunpackb(src, &msg_pos, 1) == 1)
          {
            MSG_GERAN_HIGH_0_G("C1 Delta params are present");
            dst->peo_params.c1_delta_present   = TRUE;
            dst->peo_params.c1_delta_min       = gunpackb(src, &msg_pos, CSN_C1_DELTA_MIN_LEN);
            dst->peo_params.c1_delta_max       = gunpackb(src, &msg_pos, CSN_C1_DELTA_MAX_LEN);
          }     
...
)
{
 ...
 {
  csn_decode_chan_desc(src, msg_pos, &(dst->group_chan_desc.chan_desc));
...
/*===========================================================================
===
===  FUNCTION      csn_mbms_channel_params_ie
===
===  DESCRIPTION
===
===    This function extracts the information from the Frequency Parameters
...
static csn_decoder_error_t csn_mbms_channel_params_ie(
  uint8 src[],
  uint16 *msg_pos,
  mbms_notification_params_t *mbms_notify
)
{
  if (gunpackb(src, msg_pos, 1))
  {
    /*MBMS Channel Params IE present*/	
    if (gunpackb(src, msg_pos, 1))
    {
      /*MBMS PRACH_DSC present*/		   
      if (gunpackb(src, msg_pos, 1))
      {
        /*FREQ PARAMS present*/
        (void)csn_frequency_params_ie(src, msg_pos, &(mbms_notify->prach_desc.freq_params));   
      }
      /*MPRACH Time slot numnber*/
       mbms_notify->prach_desc.ts_num = gunpackb(src, msg_pos, 3);
      /*MPRACH USF */
      mbms_notify->prach_desc.usf    = gunpackb(src, msg_pos, 3);
    }
  }
  else
  {
    if (gunpackb(src, msg_pos, 1))
    {
      /*MBMS PTM/Session present*/
      if (gunpackb(src, msg_pos, 1))
      {
        /*FREQ PARAMS present*/
        (void)csn_frequency_params_ie(src, msg_pos, &(mbms_notify->ptm_info.freq_params));
      }
      /*DL TIME SLOT Aloocation */
      mbms_notify->ptm_info.dl_ts_allocation = gunpackb(src, msg_pos, 3); 

      /*Repeatitive MBMS session params list*/
      while (gunpackb(src, msg_pos, 1))
      {
        /*As of now these paramtres re not used but decoding is still required hence not forming the array but keep on updating the
              same value, when later this is required have two define array fo size MBMS Session Parmas IE and store accordnigy
              Hence this a a TODO things for future when support for MBMS become avail*/
        /*MBMS session params*/
        mbms_notify->session_info.len_of_bid = gunpackb(src, msg_pos, 3);
        mbms_notify->session_info.bid_value  = gunpackb(src, msg_pos, mbms_notify->session_info.len_of_bid);
        mbms_notify->session_info.esd = gunpackb(src, msg_pos, 8);
        if (gunpackb(src, msg_pos, 1))
        {
          mbms_notify->session_info.rbst = gunpackb(src, msg_pos, 16);
        }

        if (gunpackb(src, msg_pos, 1))
        {
          mbms_notify->session_info.egprs_ws = gunpackb(src, msg_pos, CSN_EGPRS_WINDOW_SIZE_LEN);
        }

        if (gunpackb(src, msg_pos, 1))
        {
          mbms_notify->session_info.npm_tt = gunpackb(src, msg_pos, 5);
        }
      }//End of Session info
    } //End of PTM session
  }	//End of MBMS channel Paramtres IE	   
...
/*****************************************************************************
***
***     12.08 - Information Element -  MBMS Session IE
***
*****************************************************************************/

/*===========================================================================
===
===  FUNCTION      csn_mbms_session_list_ie
===
===  DESCRIPTION
===
===    This function extracts the information from the MBMS session list IE Parameters
===    IE.
===
===  DEPENDENCIES
===
...
static csn_decoder_error_t csn_mbms_session_list_ie(
  uint8 src[],
  uint16 *msg_pos,
  mbms_session_list_t *mbms_session_list
)
{
  while (gunpackb(src, msg_pos, 1))
  {
    ...
    {
      //MBMS Service ID
      mbms_session_list->mbms_service_id = gunpackb(src, msg_pos, 24);
      // MBMS MCC and MNC
      mbms_session_list->mcc = gunpackb(src, msg_pos, 12);
      mbms_session_list->mnc = gunpackb(src, msg_pos, 12);
    }
    else
    {
      // Without MCC and MNC paramtre
      mbms_session_list->mbms_service_id = gunpackb(src, msg_pos, 24);
    }
    /*MBMS session ID*/
    if (gunpackb(src, msg_pos, 1))
    {
      mbms_session_list->mbms_session_id = gunpackb(src, msg_pos, 8);
    }
  }//MBMS session IE      
...
)
{
 ...
 {
  csn_decode_group_call_info(src, &msg_pos, &(dst->group_call_info));
 }
 ...
 /*Addition in Release 6*/
 dst->p1_rest_octets_rel6_params.r6_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->p1_rest_octets_rel6_params.r6_addition_flag)
 {
   mbms_related_params_t *mbms_prams;
   /*Cell global related params*/
   cell_global_params_t *cell_global_param = &(dst->p1_rest_octets_rel6_params.cell_global_param);
   
   cell_global_param->valid = gunpackb(src, &msg_pos, 1); 
   if (cell_global_param->valid)
   {
     uint8 cgp = gunpackb(src, &msg_pos, 2);
     switch (cgp)
     {
       case 0x00:
       {
         cell_global_param->cell_global_count = gunpackb(src, &msg_pos, 2);
         break;
       }
       case 0x01:
       {
         cell_global_param->cell_global_count = gunpackb(src, &msg_pos, 2);
         cell_global_param->vstk_rand         = gunpackb(src, &msg_pos, 36);
         break;
       }
       case 0x10:
       {
         cell_global_param->reduced_gcr       = gunpackb(src, &msg_pos, 28);
         cell_global_param->vstk_rand         = gunpackb(src, &msg_pos, 36);
         break;
       }
       case 0x11:
   ...
   }//End of decoding cell global param
   
   /*MBMS related Params*/
   mbms_prams         = &(dst->p1_rest_octets_rel6_params.mbms_related_param);
   mbms_prams->valid  = gunpackb(src, &msg_pos, 1); 
   if ( mbms_prams->valid)
   {
     /*MBMS params is included */
     if (gunpackb(src, &msg_pos, 1))
     {
        mbms_notification_params_t *mbms_notify = &(mbms_prams->mbms_notify_1);  
       /*MBMS notifcation1 present, decode channel params*/        
       (void)csn_mbms_channel_params_ie(src, &msg_pos, mbms_notify);
     }//End of MBMS notfication 1

     if (gunpackb(src, &msg_pos, 1))
     {
       if (gunpackb(src, &msg_pos, 1))
       {
         mbms_notification_params_t *mbms_notify = &(mbms_prams->mbms_notify_2);  
         /*MBMS notifcation2 present, decode channel params*/
         (void)csn_mbms_channel_params_ie(src, &msg_pos, mbms_notify);         
       }
       else
       {
         mbms_prams->mbms_notify_2 = mbms_prams->mbms_notify_1;
       }
     } // end of MBMS notifcation 2

     /*Ideally all session list and notification with MBMS channel paramtre  should ahve been stored in separte structure but as G is not 
            making any use of the structe , for now we are   re-suing the structure and overwriting the contents ,
            later when it becomes mandate we need to store it separately */
            
     // Check for MBMS session info
     if (gunpackb(src, &msg_pos, 1))
     {
       /*MBMS sesssion info IE is present*/  
       (void)csn_mbms_session_list_ie(src, &msg_pos, &(mbms_prams->mbms_session_info.mbms_session_list));

       /*Notifications with same MBMS Channel Parameters as in Notification 1 or Notification 2*/
       if (gunpackb(src, &msg_pos, 1))
       {
         if (gunpackb(src, &msg_pos, 1))
         {
           /*Same as channel paramtre of  notifcation 2*/
           mbms_prams->mbms_session_info.mbms_session_list_notification = mbms_prams->mbms_notify_2;
         }
         else
         {
           /*Same as channel paramtre of notification 1*/
           mbms_prams->mbms_session_info.mbms_session_list_notification = mbms_prams->mbms_notify_1;
         }

         (void)csn_mbms_session_list_ie(src, &msg_pos, &(mbms_prams->mbms_session_info.mbms_session_list));
       }
       /*Notifications with specific MBMS Channels Parameters*/
       while (gunpackb(src, &msg_pos, 1))
       {
         (void)csn_mbms_channel_params_ie(src, &msg_pos,&(mbms_prams->mbms_session_info.mbms_session_list_notification));
         (void)csn_mbms_session_list_ie(src, &msg_pos, &(mbms_prams->mbms_session_info.mbms_session_list));  
       }
  
     }//End of MBMS session info        
   }   //MBMS params are present
 }//Release 6 

 /*Addition in Release 7*/
 dst->p1_rest_octets_rel7_params.r7_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->p1_rest_octets_rel7_params.r7_addition_flag)
 {
   if (gunpackb(src, &msg_pos, 1))
   {
     dst->p1_rest_octets_rel7_params.amr_config_present = TRUE;
     dst->p1_rest_octets_rel7_params.amr_config         = gunpackb(src, &msg_pos, 4);
   }
 }

 /*Addition in Release 8*/
 dst->p1_rest_octets_rel8_params.r8_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->p1_rest_octets_rel8_params.r8_addition_flag)
 {
   dst->p1_rest_octets_rel8_params.priority_uplink_access = gunpackb(src, &msg_pos, 1);
   if (gunpackb(src, &msg_pos, 1))
   {
     dst->p1_rest_octets_rel8_params.etws_params.valid  = TRUE;
     dst->p1_rest_octets_rel8_params.etws_params.not_first_segment = gunpackb(src, &msg_pos, 1);
     if (dst->p1_rest_octets_rel8_params.etws_params.not_first_segment)
     {
       dst->p1_rest_octets_rel8_params.etws_params.segment_number = gunpackb(src, &msg_pos, 4);
     }
     else
     {
       dst->p1_rest_octets_rel8_params.etws_params.total_segment = gunpackb(src, &msg_pos, 4);
     }

     dst->p1_rest_octets_rel8_params.etws_params.pni= gunpackb(src, &msg_pos, 1);
     dst->p1_rest_octets_rel8_params.etws_params.length_of_segment = gunpackb(src, &msg_pos, 7);
     dst->p1_rest_octets_rel8_params.etws_params.data_segment = gunpackb(src, &msg_pos, dst->p1_rest_octets_rel8_params.etws_params.length_of_segment);
   }
 }
 
 /*Addition in Release 10*/
 dst->page_rest_octets_rel10_params.r10_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->page_rest_octets_rel10_params.r10_addition_flag)
 {
   dst->page_rest_octets_rel10_params.implicit_cs_reject = gunpackb(src, &msg_pos, 1);
   dst->page_rest_octets_rel10_params.implicit_ps_reject = gunpackb(src, &msg_pos, 1); 
 }

 /*Addition in Release 11*/
 dst->page_rest_octets_rel11_params.r11_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->page_rest_octets_rel11_params.r11_addition_flag)
 {
   dst->page_rest_octets_rel11_params.ipa_support = gunpackb(src, &msg_pos, 1);
 }

 /*Addition in Release 13*/
 dst->page_rest_octets_rel13_params.r13_addition_flag = GUNPACKS_LH(src, &msg_pos);
 if (dst->page_rest_octets_rel13_params.r13_addition_flag)
 {
   dst->page_rest_octets_rel13_params.peo_bcch_change_mark = gunpackb(src, &msg_pos, CSN_PEO_BCCH_CHANGE_MARK_LEN);
   dst->page_rest_octets_rel13_params.rcc = gunpackb(src, &msg_pos, CSN_RCC_LEN);   
 }
...
static csn_decoder_error_t csn_decode_p2_rest_octets(uint8 *src, p2_rest_octets_t *dst)
{
 ...
 /*Addition in Release 6*/
 dst->p2_rest_octets_rel6_params.r6_addition_flag = gunpackb_lh(src, &msg_pos);
 if (dst->p2_rest_octets_rel6_params.r6_addition_flag)
 {
   /*Release 6 present */
   if (gunpackb(src, &msg_pos, 1))
   {
     /*MBMS params are included */
     if (gunpackb(src, &msg_pos, 1))
     {
       /*MBMS Notification present*/
       mbms_notification_params_t *mbms_notify = &(dst->p2_rest_octets_rel6_params.mbms_notify_3);  
       /*MBMS notifcation2 present, decode channel params*/
       (void)csn_mbms_channel_params_ie(src, &msg_pos, mbms_notify);  
     }
     /*MBMS Information*/
     if (gunpackb(src, &msg_pos, 1))
     {
       ...
       (void)csn_mbms_session_list_ie(src, &msg_pos, &(dst->p2_rest_octets_rel6_params.mbms_session_info.mbms_session_list));
        if (gunpackb(src, &msg_pos, 1))   
        {
          /*Same tsructure is getting used but when in actual this params is required we need to update the structure*/
          dst->p2_rest_octets_rel6_params.mbms_session_info.mbms_session_list_notification = dst->p2_rest_octets_rel6_params.mbms_notify_3;
         (void)csn_mbms_session_list_ie(src, &msg_pos, &(dst->p2_rest_octets_rel6_params.mbms_session_info.mbms_session_list));
        }

        while (gunpackb(src, &msg_pos, 1))
        {
          (void)csn_mbms_channel_params_ie(src, &msg_pos, &(dst->p2_rest_octets_rel6_params.mbms_session_info.mbms_session_list_notification));
          (void)csn_mbms_session_list_ie(src, &msg_pos, &(dst->p2_rest_octets_rel6_params.mbms_session_info.mbms_session_list));
        }
     }
 ...
 /*Addition in Release 10*/
 dst->page_rest_octets_rel10_params.r10_addition_flag = gunpackb_lh(src, &msg_pos);
 if (dst->page_rest_octets_rel10_params.r10_addition_flag)
 {
   dst->page_rest_octets_rel10_params.implicit_cs_reject = gunpackb(src, &msg_pos, 1);
   dst->page_rest_octets_rel10_params.implicit_ps_reject = gunpackb(src, &msg_pos, 1); 
 ...
 /*Addition in Release 11*/
 dst->page_rest_octets_rel11_params.r11_addition_flag = gunpackb_lh(src, &msg_pos);
 if (dst->page_rest_octets_rel11_params.r11_addition_flag)
 {
   dst->page_rest_octets_rel11_params.ipa_support = gunpackb(src, &msg_pos, 1);
 }
 ...
 /*Addition in Release 13*/
 dst->page_rest_octets_rel13_params.r13_addition_flag = gunpackb_lh(src, &msg_pos);
 if (dst->page_rest_octets_rel13_params.r13_addition_flag)
 {
   dst->page_rest_octets_rel13_params.peo_bcch_change_mark = gunpackb(src, &msg_pos, CSN_PEO_BCCH_CHANGE_MARK_LEN);
   dst->page_rest_octets_rel13_params.rcc = gunpackb(src, &msg_pos, CSN_RCC_LEN); 
...
static csn_decoder_error_t csn_decode_egprs_packet_uplink_ass(uint8 *src, uint16 *msg_pos, ia_rest_octets_egprs_packet_uplink_ass_t *dst)
{
 ...
 case BLOCK_ALLOC_FLAG_SINGLE:
  {
   ...
   /*Addition of release 7*/
   dst->r7_supported_flag = GUNPACKS_LH(src, msg_pos);
   if (dst->r7_supported_flag)
   {
     /*Uplink BTTI is received*/
     if ((gunpackb(src, msg_pos, (word) 1) == 0))
     {
        /*If FANR is activated*/
        if (gunpackb(src, msg_pos, (word) 1) == 1)
        {
          /*As of now UE dont require to store the data  so just skip this*/
          /* Time based encoding*/
          if (gunpackb(src,msg_pos, (word) 1) == 1)
          {
            dst->ia_ro_r7.reported_timeslot =  gunpackb(src, msg_pos, 8);
            dst->ia_ro_r7.tsh =  gunpackb(src, msg_pos, 3);

          }
        }
     }
     /* Uplink RTTIS is used*/
     else
     {
       dst->ia_ro_r7.rtti_usf_mode = gunpackb(src, msg_pos, 1);
       dst->ia_ro_r7.pdch_pair     = gunpackb(src, msg_pos, 3);
       while (gunpackb(src, msg_pos, 1))
       {
         dst->ia_ro_r7.additional_usf = gunpackb(src, msg_pos, 3);  
       }

       if ((gunpackb(src, msg_pos, (word) 1) == 1))
       {
         dst->ia_ro_r7.usf2  = gunpackb(src, msg_pos, 2);
         while (gunpackb(src, msg_pos, 1))
         {
           dst->ia_ro_r7.additional_usf2 = gunpackb(src, msg_pos, 3);  
         }
       }
 ...
 case BLOCK_ALLOC_FLAG_MULTI:
  ...
  /*Addition of release 6*/
  dst->r6_supported_flag = GUNPACKS_LH(src,msg_pos);
  if (dst->r6_supported_flag)
  {
    if ((gunpackb(src, msg_pos, (word) 1) == 1))
    {
      dst->ia_ro_r6_alloc.pfi = gunpackb(src, msg_pos, 7);
    }
  } 
 }
 return CSN_DECODE_NO_ERROR;
 ...
 */
 ...
 {    
   if ((gunpackb(src, msg_pos, (word) 1) == 1))
   {
     dst->ia_ro_r7_pda_alloc.npn_starting_time = gunpackb(src, msg_pos, 5);
   }
   ...
   if ((gunpackb(src, msg_pos, (word) 1) == 0))
   {
     if ((gunpackb(src,msg_pos, (word) 1) == 1))
     {
       dst->ia_ro_r7_pda_alloc.event_based_fnr = gunpackb(src, msg_pos, (word) 1);
     }
   }
   else
   {
     /*DL RTTI is used */
     dst->ia_ro_r7_pda_alloc.event_based_fnr = gunpackb(src, msg_pos, (word) 1);
     dst->ia_ro_r7_pda_alloc.pdcsh_pair = gunpackb(src,msg_pos, 3);
   }
   
   dst->ia_ro_r7_pda_alloc.egprs_level = gunpackb(src, msg_pos, 2);
 }
 return CSN_DECODE_NO_ERROR;
}
...
static csn_decoder_error_t csn_decode_iax_rest_octets(uint8 *src, iax_rest_octets_t *dst)
{
  ...
  memset(dst, 0, sizeof(iax_rest_octets_t));

  dst->compressed_irat_ho_info = GUNPACKS_LH(src, &msg_pos);  

  /*Addition in release 13*/
  dst->r13_addition_flag = GUNPACKS_LH(src, &msg_pos);
  if (dst->r13_addition_flag)
  {
    dst->rcc = gunpackb(src, &msg_pos, CSN_RCC_LEN);
  }
...
#define HH 3
 ...
 case LL:
  ...
  if (dst->contents.compressed_irat_ho_info)
  {
    /*Addition of Release 13*/
    dst->r13_addition_flag = GUNPACKS_LH(src, &msg_pos);
    if (dst->r13_addition_flag)
    {
      dst->implicit_ps_reject   = gunpackb(src, &msg_pos, 1); 
      dst->peo_bcch_change_mark = gunpackb(src, &msg_pos, CSN_PEO_BCCH_CHANGE_MARK_LEN);
      dst->rcc                  = gunpackb(src, &msg_pos, CSN_RCC_LEN);
    }       
  }

  MSG_GERAN_HIGH_3_G("IA rest octet for LL type has RCC = %d, PEO BCCH = %d, IR %d",dst->rcc,dst->peo_bcch_change_mark, dst->implicit_ps_reject);
 ...
 case LH:
  ...
  MSG_GERAN_HIGH_3_G("IA rest octet for LH type has RCC = %d, PEO BCCH = %d, IR %d",dst->rcc,dst->peo_bcch_change_mark, dst->implicit_ps_reject);
  break;
 ...
 case HL:
  ...
  MSG_GERAN_HIGH_3_G("IA rest octet for HL type  has RCC = %d, PEO BCCH = %d, IR %d",dst->rcc,dst->peo_bcch_change_mark, dst->implicit_ps_reject);
  retval  = CSN_DECODE_NO_ERROR;
  break;
 ...
 case HH:
  ...
  dst->r10_addition_flag = GUNPACKS_LH(src, &msg_pos);  
  /*Addition of Release 10 */
  if (dst->r10_addition_flag)
  {
    dst->implicit_cs_reject = gunpackb(src, &msg_pos, 1); 
    dst->implicit_ps_reject = gunpackb(src, &msg_pos, 1); 
  ...
  MSG_GERAN_HIGH_3_G("IA rest octet for HH type has RCC = %d, PEO BCCH = %d, IR %d",dst->rcc,dst->peo_bcch_change_mark, dst->implicit_ps_reject);
  break;
...
/*===========================================================================
===
===  FUNCTION     csn_decode_si9_rest_octets
===
===  DESCRIPTION
===
===    Decode CSN1 encoded SI 9 Rest Octets.
===
===
===  DEPENDENCIES
===
...
static csn_decoder_error_t csn_decode_si9_rest_octets(uint8 *src, si9_rest_octets_t *dst)
{
  ...
  if (GUNPACKS_LH(src, &msg_pos))
  {
    do
    {
      scheduling_info_t *sched_ptr = &dst->scheduling_info[dst->scheduling_info_count++];
      ...
      if (gunpackb(src, &msg_pos, (word) 1))
      {
        if (gunpackb(src, &msg_pos, (word) 1))
        {
          /* info_type_6 - this is reserved for future use */
      ...
      {
        /* info_type_4 - this is reserved for future use */
      ...
      /* decode positions */
      do {
        position_t *position_ptr = &sched_ptr->positions[sched_ptr->position_count++];
      ...
      }while ((sched_ptr->position_count < MAX_POSITIONS) && gunpackb(src, &msg_pos, (word) 1));
