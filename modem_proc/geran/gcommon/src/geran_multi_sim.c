...
*/
/* $Header: //components/rel/geran.mpss/5.2.0/gcommon/src/geran_multi_sim.c#5 $ */
/* $DateTime: 2019/07/05 02:39:06 $ */
/* $Author: pwbldsvc $ */
...
static const gs_queue_id_T gl1_queue_ids = {GS_QUEUE_LAYER_1};
...
static gas_id_t                           as_id_to_gas_id; /* ASID to GAS ID mapping */
static sys_modem_as_id_e_type             gas_id_to_as_id; /* GAS ID to ASID mapping */
static trm_grant_return_enum_type         geran_chain_exchanged[2];
static geran_exchange_direction_t         geran_exchange_on_release;
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
#ifdef FEATURE_LTE
static uint32 geran_lte_activity_status;
#endif /* FEATURE_LTE */
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
 ...
 */
sys_modem_as_id_e_type ftm_sub( void )
{
...
===========================================================================*/
byte geran_get_diag_sub_id( void )
{
  byte nas_id;

  nas_id = geran_map_gas_id_to_nas_id();
#ifdef FEATURE_TRIPLE_SIM
...
int geran_get_data_space_index( void )
{
...
static int geran_map_asid_to_index(const sys_modem_as_id_e_type as_id)
{
  ...
  {
    MSG_GERAN_LOW_1("ASID %d is out of range", (int)as_id);
    as_index = NUM_GERAN_DATA_SPACES;
  }
...
boolean geran_api_get_tuneaway_blocked(const sys_modem_as_id_e_type as_id)
{
  ...
  if (as_index < NUM_GERAN_DATA_SPACES)
  {
    ...
    {
      if (TRUE == gta_veto_status.client_vote[vote_index])
      {
...
static int geran_get_client_vote_index(const geran_api_client_id_t client_id)
{
  ...
  MSG_GERAN_LOW_1("Unexpected GTA client ID %d", (int)client_id);
  return(GERAN_NUM_GTA_VETO_CLIENTS);
}
 ...
 * \return void
 */
void geran_api_set_tuneaway_blocked(const geran_api_client_id_t client_id, const boolean gta_blocked)
{
  int      as_index;
  ...
  /* convert inbound ASID to data space index */
  as_index = geran_map_asid_to_index(SYS_MODEM_AS_ID_1);
  ...
  if (as_index < NUM_GERAN_DATA_SPACES)
  {
    ...
    if (GERAN_API_CLIENT_RESET == client_id)
    {
      ...
      for (vote_index = 0; vote_index < GERAN_NUM_GTA_VETO_CLIENTS; vote_index++)
      {
        ...
        gta_veto_status.client_vote[vote_index] = gta_blocked;
      }
      GTA_VOTE_UNLOCK();

      MSG_GERAN_LOW_1_G("GTA veto status reset to %d (1=GTA blocked)", (int)gta_blocked);
    }
    else
    {
      ...
      if (vote_index < GERAN_NUM_GTA_VETO_CLIENTS)
      {
        ...
        MSG_GERAN_LOW_2_G("GTA veto client %d vote set to %d (1=GTA blocked)", (int)client_id, (int)gta_blocked);
      }
    }
  }
  else
  {
    MSG_GERAN_LOW_1_G("Unable to apply GTA veto for client %d, ASID 0x%02X", (int)client_id);
  }
...
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
/*!
 * \brief Determine whether G2W tuneaway is supported for specified device mode.
 */
boolean geran_g2w_tuneaway_supported_by_device_mode(const sys_modem_device_mode_e_type dev_mode)
{
  sys_modem_dual_standby_pref_e_type mode_pref = geran_get_sys_multi_sim_pref();

  MSG_GERAN_HIGH_2("Checking device mode %d, standby pref %d", (int)dev_mode, (int)mode_pref);

#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
#if defined(FEATURE_WCDMA) && defined(FEATURE_G2X_TUNEAWAY)
  /* make changes here if G2W TA is supported in any other mode e.g. TSTS */
  if (GERAN_TA_SUPPORTED == geran_nv_g2w_ta_supported())
  {
    return(TRUE);
  }
#endif /* FEATURE_WCDMA && FEATURE_G2X_TUNEAWAY */
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
                                         ...
                                         const trm_grant_return_enum_type        trm_grant)
{
  ...
  {
    MSG_GERAN_LOW_0_G("Invalid parameter to geran_set_chain_exchanged()");
  }
...
boolean geran_was_lock_exchanged(const geran_exchange_direction_t direction)
{
  if (GERAN_EXCHANGE_NULL == direction)
  {
    MSG_GERAN_LOW_0_G("Invalid parameter to geran_was_lock_exchanged()");
  }
...
===========================================================================*/
void geran_clear_lock_exchanged(const geran_exchange_direction_t direction)
{
  /* just call the local utility function to reset the chain */
  (void)geran_set_chain_exchanged(direction, TRM_DENIAL);
...
===========================================================================*/
static boolean geran_lock_exchange_allowed( void )
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
...
===========================================================================*/
boolean geran_exchange_lock(const geran_exchange_direction_t direction)
{
  if (GERAN_EXCHANGE_NULL == direction)
  {
    MSG_GERAN_LOW_0_G("Invalid parameter to geran_exchange_lock()");
    return(FALSE);
  }
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
#ifndef PERLUTF
  else
  {
    ...
    if (FALSE == geran_lock_exchange_allowed())
    {
    ...
    if (TRM_DENIAL != trm_granted)
    {
      ...
      return(geran_set_chain_exchanged(direction, trm_granted ));
    }
    else
    {
...
===========================================================================*/
void geran_set_exchange_on_release(const geran_exchange_direction_t direction)
{
  {
    MSG_GERAN_LOW_1("Exchange direction %d not supported for GAS ID 1", (int)direction);
  }
...
===========================================================================*/
geran_exchange_direction_t geran_get_exchange_on_release( void )
{
  if (geran_lock_exchange_allowed())
  {
    {
      MSG_GERAN_LOW_0("No inter-RAT lock exchange info available");
    }
  }
 ...
 */
void geran_set_traffic_reason(const geran_traffic_reason_t traffic_reason)
{
 ...
 */
geran_traffic_reason_t geran_get_traffic_reason(boolean *ps_ptr)
{
...
void geran_abandon_lock_exchange(const geran_exchange_direction_t direction)
{
#if defined(FEATURE_DUAL_SIM) || defined(FEATURE_TRIPLE_SIM)
#ifndef PERLUTF
  if (geran_is_multi_mode_gas_id())
  {
    /*explicitly release the inter-RAT client */
    grm_release(TRM_IRAT );
    geran_clear_lock_exchanged(direction );
  }
#endif /*!PERLUTF */
...
void geran_multi_sim_init(void)
{
  ...
  geran_sys_subs_feature   = SYS_SUBS_FEATURE_MODE_NORMAL;
    ...
    (void)geran_set_chain_exchanged(GERAN_EXCHANGE_XTOG, TRM_DENIAL );
    (void)geran_set_chain_exchanged(GERAN_EXCHANGE_GTOX, TRM_DENIAL );
#ifdef FEATURE_LTE
    geran_lte_activity_status[GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX()] = LTE_ACTIVITY_NONE;
#endif /* FEATURE_LTE */
  ...
  if (GERAN_TRAFFIC_DEFAULT != geran_get_traffic_reason(NULL))
  {
...
===========================================================================*/
void geran_set_lte_activity_status(const uint32 activity_mask)
{
  geran_lte_activity_status[GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX()] = activity_mask;
  return;
}
...
===========================================================================*/
uint32 geran_get_lte_activity_status( void )
{
  return(geran_lte_activity_status[GERAN_MAP_GAS_ID_TO_DATA_SPACE_INDEX()]);
}
#endif /* FEATURE_LTE */
#endif /* FEATURE_DUAL_SIM || FEATURE_TRIPLE_SIM */
...
FUNCTION     geran_q_destroy

DESCRIPTION  Frees the memory allocated for each queue element and then destroys the queue

PARAMS       Pointer to the queue to be destroyed
...
===========================================================================*/
void geran_q_destroy(q_type *q_ptr)
{
  void *element_ptr;
  int   cnt = q_cnt(q_ptr);
  ...
  if (cnt > 0)
  {
    MSG_GERAN_HIGH_2("Freeing %d elements of the queue (0x%x)", cnt, q_ptr);
    
    while ((element_ptr = q_get(q_ptr)) != NULL)
    {
      GPRS_MEM_FREE(element_ptr);
    }
  }
