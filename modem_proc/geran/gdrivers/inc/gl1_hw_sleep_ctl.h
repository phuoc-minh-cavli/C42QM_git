...
$ $DateTime: 2020/05/13 04:13:16 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
15/04/20    sal     CR2663867 Increased the sleep duration by a frame in case of Rude wake-up.
21/09/18    ng      CR2320290 During wakeup 'gap_to_ols' must be at least greater than time needed for ccs write, else do error recovery
...
#define FORCE_WAKEUP_THRESHOLD               (5000)     /* 5ms *
/*
...
typedef struct
{
  ...
  /* Stored samples used in the algorithm.
     Each sample (in usecs) represents the sum of two quantities:
       1) the ACTUAL time FW/RF/MCPM took to perform warmup procedure (NOT time allocated)
       2) the CCS_Write_time */
  uint16  samples[ADAPTIVE_TMLN_SAMPLE_COUNT];
  
  uint8 num_samples; /* How many samples have been collected starting at the zeroth index of the above buffer */
  uint8 index;       /* The index of the most recently collected sample. Will loop around, allowing the rolling-buffer concept. */
  
} adaptive_warmup_tmln_struct_type;

void gl1_hw_save_ccs_write_time_estimate(uint32 CCS_write_time);
...
void   gl1_sleepctl_error_feedback( sclk_enum_type sclkRat, sys_err_t sysclk_error);
...
ustmr_type gl1_sleepctl_get_ustmr_at_ols( void );

#ifdef FEATURE_GL1_SLEEP_QDSS_LOG
void QDSS_logging( tracer_event_id_t event_id );
#endif /* FEATURE_GL1_SLEEP_QDSS_LOG */

#if defined ( FEATURE_DUAL_SIM ) || defined ( FEATURE_TRIPLE_SIM )
void gl1_hw_sleep_clear_wakeup_cancel_status_flags(void);
void gl1_hw_sleep_handle_deferred_wakeup_conflict(void);
boolean gl1_hw_sleep_wup_cancel_reprogram_allowed(void);
void gl1_hw_sleep_set_any_pending_deferred_wmgr_event( boolean event_pending);
boolean gl1_hw_sleep_get_any_pending_deferred_wmgr_event(void);
void gl1_hw_sleep_clear_any_pending_deferred_wmgr_event(void);
...
void gl1_hw_sleep_set_bg_traffic_active(  boolean bg_traffic_active );
boolean gl1_hw_sleep_get_bg_traffic_active( void );
#endif
#ifdef FEATURE_EXTERNAL_PAGING_GERAN
void gl1_hw_sleep_handle_wakeup_conflict_dmm(void);
extern uint32 sleep_duration_frames;
extern uint64 sleep_duration_gsm_clks;
/* ----------------------------------------------------------------------- */
/* GSM warmup time in micro seconds.                                       */
/* ----------------------------------------------------------------------- */
extern uint32 gl1_hw_sleep_warmup_time_us;
#endif
...
extern boolean entered_warmup_recovery;

#ifdef FEATURE_GSM_ADAPTIVE_TMLN
extern adaptive_warmup_tmln_struct_type adaptive_warmup_tmln;
#endif

gl1_hw_sleep_state_T gl1_hw_get_sleep_state(void);
void gl1_hw_rf_result_monitor_timer_init(void );
void gl1_hw_rf_result_monitor_timer_deactivate( void );
void gl1_hw_rf_result_monitor_timer_start(uint32 time_to_wait_for_rf_cnf );
void gl1_hw_rf_result_monitor_timer_deinit(void );
void gl1_hw_timer_sleep_commit ( void );
void gl1_hw_abort_idle_act_prog(void );
uint16 gl1_hw_get_frame_number_mdsp_wakeup(void);
uint16 gl1_hw_get_frame_number_ols(void);
void gl1_hw_sleep_update_scheduled_activity(void);

extern void gl1_hw_sleep_gsm_warmup_gas_1_cb( void );

#ifdef FEATURE_IDLE_DRX_SCALING
void gl1_hw_sleep_timing_error_add_sample(int16 timing_offset_qs);   
#endif /* FEATURE_IDLE_DRX_SCALING */

#ifdef FEATURE_GL1_SLEEP_DEBUG_STRUCT

#define NUM_OF_GL1_DEBUG_CYCLES         (4)
...
extern gl1_hw_sleep_type gl1_hw_sleep_struct[NUM_OF_GL1_DEBUG_CYCLES];

void gl1_hw_sleep_rf_wakeup_end(void);
