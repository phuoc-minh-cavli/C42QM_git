...
$Header: //components/rel/geran.mpss/5.2.0/gdrivers/inc/gl1_hw_lif.h#6 $
$DateTime: 2019/03/20 20:27:54 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     -------------------------------------------------------- 
19/03/19    ktg     CR2418661 Reverting 2381037 for LIF log metrics
07/03/19    kh      CR2381037 Realigning data structures for Padding reduction
22/02/19    ktg     CR2403664 Updating LIF metrics Log packet version to 5
24/09/18    ktg     CR2317283 : FR 46672: Jammer detection and mitigation in GSM 
...
#define MAX_NUMBER_OF_RX_SLOTS           6
...
typedef struct
{
  ...
  uint32     ilpc_i2q2_average;
  boolean    nb_jdet_result_valid;
  boolean    nb_jammer_detected;
  uint32     unused1;
  uint32     unused2;
...
boolean  gl1_lif_force_enable;
uint8    gl1_lif_force_sign;
...
uint32 gl1_hw_get_lif_feature_flag(void);
void gl1_hw_get_lif_status(gl1_hw_lif_table_t *gl1_hw_lif_table,uint32 index);
void  gl1_hw_log_lif_metrics(gl1_log_lif_metrics_t  *log_lif_metrics);
void gl1_hw_update_lif_metrics(gl1_defs_metrics_rpt *metrics_ptr,gl1_defs_rx_pkt_metrics_type **metrics_info,
                                   ...
                                   uint32 fn,gl1_hw_update_lif_metrics_t update_metrics);

void gl1_hw_populate_lif_metrics(uint32 fn);
uint32 gl1_hw_get_arfcn_index(ARFCN_T arfcn );
void gl1_hw_update_lif_table(uint32 index, boolean enable_lif, boolean lif_freq_offset );
void gl1_hw_reset_lif_table(void);
void gl1_hw_lif_data_init(void);
void gl1_hw_reset_lif_dtm_metrics(void);
void gl1_hw_efs_read_lif_feature_flag(void);
void gl1_hw_enable_lif_in_ftm_cal(boolean enable_lif );
boolean gl1_hw_get_enable_lif_in_ftm_cal(void);
