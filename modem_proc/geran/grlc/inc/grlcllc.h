...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/inc/grlcllc.h#2 $
***
...
#define GRLCLLC_LOWATER_OCTETS()                                    \
  MAX(RLC_FLOOR_FCTRL_VALUE,                                              \
      (                                                                   \
        (ul.state == US_ACK_XFER || ul.state == US_UNACK_XFER) ?          \
        (                                                                 \
          (uint32)                                                        \
          ( ((RM_RLC_WINDOW_SIZE + (BS_CV_CONST * ul.cur_res.num_timeslots)) * \
      ...
      ((IS_TX_BLANKING_EXCEEDED_THRESHOLD() ?                       \
...
#define GRLCLLC_HIWATER_OCTETS()    ( GRLCLLC_LOWATER_OCTETS() + GRLC_UL_PDU_MAX_LEN )
...
*/
#define ERLCLLC_LOWATER_OCTETS()                                    \
  MAX(RLC_FLOOR_FCTRL_VALUE,                                              \
      (                                                                   \
        (ul.state == US_ACK_XFER || ul.state == US_UNACK_XFER) ?          \
        (                                                                 \
          (uint32)                                                        \
          ( MIN((MOD_DIF_ESNS(ul.vs, ul.va)),ul.cur_res.ws) + (BS_CV_CONST * ul.cur_res.num_timeslots)) *    \
          E_GRLC_DATA_BLOCK_LEN[ul.cur_res.cs] +                          \
          ERLCLLC_MARGIN                                                  \
        ) :                                                               \
        /* maximum level for GPRS mode */                                 \
        MAX_FLOW_CONTROL_NUM_OCTETS                                       \
...
**===============================================================================
*/
#define ERLCLLC_HIWATER_OCTETS()    (ERLCLLC_LOWATER_OCTETS() + GRLC_UL_PDU_MAX_LEN)
...
extern uint32 grlc_llc_get_current_octet_count(void);
