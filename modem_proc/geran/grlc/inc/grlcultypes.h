...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/inc/grlcultypes.h#3 $
***
...
*/
#undef  GSM_GPRS_SHOW_FIRST_LAST_PSNS

/* Turn on to show every PDU being freed. Default off
*/
#define  GSM_GPRS_GRLU_SHOW_FREE_PDU
...
/* Turn on to show resource reallocation activitiies. Default off
*/
...
/* Turn on to show segmentation process. Default on
*/
...
*/
#define IS_EXT_UTBF_ACTIVE()	(sysinfo.ext_utbf == 1)
...
typedef enum
{
  ...
  ** cur_pdu_ptr->psn. So freeing was done upto and including the PDU previous
...
typedef struct
{
  ...
  */
  ...
  /* To indicate timer expiry only when GERAN in unloaded */
  boolean      timer_stale_pdu_delete_expired;
...
#define IS_TX_BLANKING_EXCEEDED_THRESHOLD()           \
        ((ul.usf_info.blanking_percentage_average) > TX_BLANKING_PERCENTAGE_MAX)
...
#define IS_CV_ACTIVE()    (ul.cv != BS_CV_CONST)
#define IS_CV_INACTIVE()  (!IS_CV_ACTIVE())
#define IS_CV_NOT_ZERO()  (ul.cv != 0)
...
**-------------------------------------------------------------------------
*/
#define IS_TX_CV_ACTIVE()    (ul.ptx_cv != BS_CV_CONST || ul.pak_cv != BS_CV_CONST)
#define IS_TX_CV_INACTIVE()  (!IS_TX_CV_ACTIVE())
#define IS_TX_CV_ZERO()      (ul.ptx_cv == 0 || ul.pak_cv == 0)
#define IS_TX_CV_NOT_ZERO()  (!IS_TX_CV_ZERO())

/*--------------------------------------------------------------
** Macro to return TRUE if the network supports PFC procedures.
**--------------------------------------------------------------
*/
#define IS_PFC_NW_SUPPORTED()  (sysinfo.pfc_feature_mode != 0)
...
**-----------------------------------------------------------------
*/
#define SEND_REALLOC_REQ( mptr, pdu_ptr, num_octets, cv_status)                  \
  {                                                                                     \
    ...
    ul.prev_realloc_psn = (pdu_ptr)->psn;                                       \
    TIMER_START_MAC_UL_RESP();                                                    \
    mac_put_cmd_rlc( mptr);                                                      \
...
#define INC_VS()       INC_MODSNS(ul.vs)

#define E_INC_VS()     INC_MOD((ul.vs),ESNS)
...
#define SET_BLOCK_COUNT_LIMIT( bsn)                                                                \
        {                                                                                                 \
          TX_ARR(bsn).blk_cnt = grlc_ul_block_counter+(uint16)(MAX(BS_CV_MAX(),1));  \
        }

#define E_SET_BLOCK_COUNT_LIMIT( bsn)                                                               \
        {                                                                                                  \
          TX_ARR((bsn)).blk_cnt = grlc_ul_block_counter+(uint16)(MAX(BS_CV_MAX(),1)); \
        }
...
*/
  ...
  #define TIMER_START_T3182()                                                            \
          grlc_ul_start_timer_T3182()

  #define TIMER_START_AWAIT_UL_ACKNACK()                                                                \
          {                                                                                                   \
            (void)rex_set_timer(&(ul.timer_await_ul_acknack.rex_timer), RM_TMR_AWAIT_UL_ACKNACK_DUR); \
            ul.timer_await_ul_acknack.state = TMR_ACTIVE;                                             \
          }
...
#error code not present
  ...
  #define TIMER_START_MAC_UL_RESP()
  #define TIMER_STOP_MAC_UL_RESP()
...
#define   TIMER_STOP_AWAIT_UL_ACKNACK()                                             \
          {                                                                               \
            if (ul.timer_await_ul_acknack.state == TMR_ACTIVE)                    \
            {                                                                             \
  ...
  #define   TIMER_START_AWAIT_ASS_GUARD()                                  \
            {                                                                    \
              (void)rex_set_timer(&(ul.timer_await_ass_guard.rex_timer), \
                                    RM_TMR_AWAIT_ASS_GUARD_DUR);                 \
              ul.timer_await_ass_guard.state = TMR_ACTIVE;               \
              MSG_MED("GRLU start await_ass_guard timer",0,0,0);   \
            }


  #define   TIMER_STOP_AWAIT_ASS_GUARD()                                       \
            {                                                                        \
              if (ul.timer_await_ass_guard.state == TMR_ACTIVE)              \
              {                                                                      \
                (void)rex_clr_timer(&(ul.timer_await_ass_guard.rex_timer));  \
                ul.timer_await_ass_guard.state = TMR_INACTIVE;               \
                MSG_MED("GRLU stop await_ass_guard timer",0,0,0);        \
              }                                                                      \
            }

  #define   TIMER_START_STALE_PDU_DELETE()                                     \
            {                                                                        \
              if ( (ul_static.timer_stale_pdu_delete.state == TMR_INACTIVE) &&      \
                   (!ul_static.b_ul_pdu_delete_after_stale_tmr_expiry)              \
                 )                                                                   \
              {                                                                      \
                (void)rex_set_timer(&(ul_static.timer_stale_pdu_delete.rex_timer),  \
                                      RM_TMR_STALE_PDU_DELETE_DUR);                  \
                ul_static.timer_stale_pdu_delete.state = TMR_ACTIVE;                \
                MSG_MED("GRLU stale_tmr started",0,0,0);                 \
              }                                                                      \
              else                                                                   \
              {                                                                      \
                MSG_MED("GRLU stale_tmr not started, state: %d, flag: %d",      \
                         0,                                                   \
                         ul_static.timer_stale_pdu_delete.state,                    \
                         ul_static.b_ul_pdu_delete_after_stale_tmr_expiry);         \
              }                                                                      \
            }
...
**-------------------------------------------------------------
*/
#define SEND_BLOCK_CV_0()   (void)send_block_to_pl1_fifo( ul.bsn_of_cv_0_block, TXA_UNACK)
...
**-------------------------------------------------------------
*/
#define E_SEND_BLOCK_CV_0() (void)e_send_block_to_pl1_fifo(   \
                                       ul.e_bsn_of_cv_0_block, 0xffff, TXA_UNACK)
...
#define PUT_UL_RLC_HDR_OCTET_1(bp,sp,tf)                                                 \
        {                                                                                \
          (bp)->tfi = (tf);                                                              \
          (bp)->ti = (IS_TLLI_REQUIRED() ? RM_TLLI_PRESENT : RM_TLLI_NOT_PRESENT); \
          (bp)->pi = (IS_PFI_REQUIRED() ? RM_PFI_PRESENT : RM_PFI_NOT_PRESENT);    \
        }
...
**      cur_pdu_ptr->tlli (passed in by LLC)
...
**-----------------------------------------------------------------
*/
#define NEW_DATA_BLOCK(bp)      grlc_ul_init_gprs_data_block(bp)
...
**-----------------------------------------------------------------
*/
#define E_NEW_DATA_BLOCK(bp)  grlc_ul_init_egprs_data_block(bp)

#define IS_E_DATA_BLOCK_PRECISE_FIT(bsn)                    \
        (                                                          \
          TX_ARR((bsn)).blk.num_ext_octets >= 1 &&          \
          TX_ARR((bsn)).blk.ext == RM_EXT_OCTET_FOLLOWS &&  \
          TX_ARR((bsn)).blk.ext_octet[0].egprs.li == 0      \
        )

#define E_RESTORE_DATA_BLOCK_PRECISE_FIT()                              \
        {                                                                     \
          cur_blk.num_ext_octets = 1;                                 \
          cur_blk.ext = RM_EXT_OCTET_FOLLOWS;                         \
          cur_blk.ext_octet[0].egprs.ext = RM_EXT_NO_OCTET_FOLLOWS;   \
          cur_blk.ext_octet[0].egprs.li = 0;                          \
          cur_blk.rem_cnt--;                                          \
          ul.b_li_0_exists = TRUE;                                    \
        }
        ...
        {                                                   \
          ...
          {                                                 \
            ...
            case RM_MCS_9:                                  \
              ...
              break;                                        \
            default:                                        \
              MSG_ERROR("GRLU Invalid MCS %d",(mcs),0,0);   \
              break;                                        \
          }                                                 \
        }
...
**-----------------------------------------------------------------------
*/
#define GET_CPS( frst_bsn, scnd_bsn, cps )                                           \
        {                                                                                   \
          if (TX_ARR(frst_bsn).blk.cs <= RM_MCS_4)                                   \
          {                                                                                 \
            GET_CPS_HDR_3( (frst_bsn), (cps));                                       \
          }                                                                                 \
          else if (TX_ARR(frst_bsn).blk.cs <= RM_MCS_6)                              \
          {                                                                                 \
            GET_CPS_HDR_2( (frst_bsn), (cps) );                                      \
          }                                                                                 \
          else if (TX_ARR(frst_bsn).blk.cs <= RM_MCS_9)                              \
          {                                                                                 \
            if ((scnd_bsn) == 0xffff)                                                       \
            {                                                                               \
              MSG_GERAN_ERROR_2_G("GRLU Invalid MCS %d or 2nd BSN %d",                      \
               TX_ARR(frst_bsn).blk.cs, (scnd_bsn));                                 \
              (scnd_bsn) = (frst_bsn);                                                      \
            }                                                                               \
            GET_CPS_HDR_1( (frst_bsn), (scnd_bsn), (cps));                           \
          }                                                                                 \
          else                                                                              \
          {                                                                                 \
            MSG_GERAN_ERROR_1_G("GRLU Invalid MCS %d",                                      \
                TX_ARR(frst_bsn).blk.cs);                                            \
          }                                                                                 \
        }

/* Uses table E_GRLC_HDR1_CPS[] to index the right CPS value.
*/
#define GET_CPS_HDR_1( frst_bsn, scnd_bsn, cps )                            \
        {                                                                          \
          grlc_ul_data_block_t *tx_blk_frst_bsn_p = &TX_ARR( frst_bsn).blk; \
          grlc_ul_data_block_t *tx_blk_scnd_bsn_p = &TX_ARR( scnd_bsn).blk; \
                                                                                   \
          ...
          {                                                                        \
            MSG_GERAN_ERROR_3_G("GRLU Invalid MCS %d or 1st/2nd PS %d/%d ",        \
...
*/
#define GET_CPS_HDR_2(  bsn, cps )                                                              \
        {                                                                                              \
          grlc_ul_data_block_t *tx_blk_p = &TX_ARR( bsn).blk;                                   \
          ...
          {                                                                                            \
            ROTATE_PS_MCS_CHANGE(ul.cur_res.cs, tx_blk_p->cs, tx_blk_p->ps, &(tx_blk_p->ps));  \
            MSG_GERAN_MED_3_G("EGRLU cmd_mcs %d, new_mcs %d force PS to %d",                           \
              ul.cur_res.cs, tx_blk_p->cs, tx_blk_p->ps);                                      \
          }                                                                                            \
        }

/* Uses table E_GRLC_HDR3_CPS[] to index the right CPS value.
*/
#define GET_CPS_HDR_3(  bsn, cps )                                             \
        {                                                                             \
          grlc_ul_data_block_t *tx_blk_p = &TX_ARR( bsn).blk;                  \
          ...
          {                                                                           \
            MSG_GERAN_ERROR_3_G("GRLU Invalid MCS %d or PS %d",                  \
              0, tx_blk_p->cs,tx_blk_p->ps);                                   \
            ERR_GERAN_FATAL_0_G("GRLU Invalid MCS or PS");                            \
          }                                                                           \
        }
...
**----------------------------------------------------------------------
*/
#define GET_TX_WINDOW_SIZE()    MOD_DIF(ul.vs, ul.va, SNS)
#define GET_E_TX_WINDOW_SIZE()  MOD_DIF_ESNS(ul.vs, ul.va)

/*-------------------------------------------------------------
** Macro to return TRUE if the transmit window is full (stall)
**-------------------------------------------------------------
*/
#define IS_TX_WINDOW_FULL()  IS_GE_MODULO_COMP(ul.vs, (ul.va+RM_RLC_WINDOW_SIZE), SNS)
...
**-------------------------------------------------------------------------
*/
#define SET_TBF_MODE_EGPRS()                                 \
        {                                                          \
          ul.cur_res.tbf_mode = TBF_MODE_EGPRS;            \
          grlc_ul_fifo.tbf_mode = TBF_MODE_EGPRS;          \
          MSG_MED("ERLU GPRS to EGPRS",0,0,0);         \
        }

#define SET_TBF_MODE_GPRS()                                  \
        {                                                          \
          ul.cur_res.tbf_mode = TBF_MODE_GPRS;             \
          grlc_ul_fifo.tbf_mode = TBF_MODE_GPRS;           \
          MSG_MED("ERLU EGPRS to GPRS",0,0,0);         \
        }
...
**-------------------------------------------------------------
*/
#define IS_E_TX_WINDOW_FULL()	(MOD_DIF_ESNS(ul.vs, ul.va) >= ul.cur_res.ws)
...
**--------------------------------------------------------------------------
*/
#define GET_ATTACH_WINDOW_SIZE()      MOD_DIF(ul.seg_ind, ul.va, SNS)
...
**-----------------------------------------------------------------------
*/
#define GET_UNACK_TX_WINDOW_SIZE()    MOD_DIF(ul.seg_ind, ul.vs, SNS)

#define GET_UNACK_TX_WINDOW_SPACE()   (RM_RLC_WINDOW_SIZE - GET_UNACK_TX_WINDOW_SIZE())
...
**-------------------------------------------------------------
*/
#define GET_ATTACH_WINDOW_SPACE()    (uint8)(RM_RLC_WINDOW_SIZE - GET_ATTACH_WINDOW_SIZE())

#define E_GET_ATTACH_WINDOW_SPACE()  (ul.cur_res.ws - MOD_DIF_ESNS(ul.seg_ind, ul.va))

#define E_GET_UNACK_WINDOW_SPACE()   (ul.cur_res.ws - MOD_DIF_ESNS(ul.seg_ind, ul.vs))
...
**------------------------------------------------------------------------
*/
#define IS_UL_BSN_VALID_IN_WINDOW(bsn)                                             \
          (IS_GE_MOD((bsn),(ul.va),(ul.cur_res.ws)) &&                    \
            IS_GE_MOD((MOD_ESNS(ul.vs + ESNS -1)),(bsn),(ul.cur_res.ws)))
...
**--------------------------------------------------------------
*/
#define IS_TLLI_REQUIRED()                                        \
          (ul.cur_res.access_phase == GMAC_ONE_PHASE_ACCESS &&  \
           ul.cur_res.ti == RM_TLLI_PRESENT &&                  \
...
**------------------------------------------------------------------
*/
#define IS_NEXT_PDU_PFI_DIFF_TO_CURRENT(bp)                                          \
        (                                                                                   \
          ((bp)->pi   == RM_PFI_PRESENT) &&                                                 \
          (((bp)->pfi != ul.pfi) && (ul.pfi != RM_PFI_INVALID))  &&         \
          ((next_pdu_ptr != NULL) && (next_pdu_ptr->pfi == ul.pfi)) \
        )

#define IS_NEXT_EGPRS_PDU_PFI_DIFF_TO_CURRENT(bp)                                    \
        (                                                                                   \
...
**--------------------------------------------------------------
*/
#define IS_PFI_REQUIRED()                                     \
        (                                                           \
          IS_TLLI_REQUIRED() && IS_PFC_NW_SUPPORTED()   \
        )
...
#define IS_DYNAMIC_OPEN()                                   \
        (                                                         \
          (ul.cur_res.access_mode == RM_DYNAMIC_ALLOC ||  \
           ul.cur_res.access_mode == RM_EXT_DYNAMIC_ALLOC \
          ) &&                                                    \
          ul.cur_res.tbf_type == RM_OPEN_ENDED_TBF        \
        )

#define IS_DYNAMIC_CLOSE()                                     \
        (                                                            \
          (ul.cur_res.access_mode == RM_DYNAMIC_ALLOC ||     \
            ul.cur_res.access_mode == RM_EXT_DYNAMIC_ALLOC   \
          ) &&                                                       \
          ul.cur_res.tbf_type == RM_CLOSE_ENDED_TBF          \
        )
...
**------------------------------------------------------------
*/
#define GET_PL1_FREE_SPACE() (uint8)(RLC_PL1_UL_TOTAL_FIFO_SIZE - grlc_ul_fifo_get_cnt_dyn())
...
**---------------------------------
*/
#define PAN_INC()	sysinfo.pan_inc
#define PAN_DEC()	sysinfo.pan_dec
#define PAN_MAX()	sysinfo.pan_max

/*------------------------------------------------------------------------
** If any of the below parameters are 0 then N3102 operation is disabled
**------------------------------------------------------------------------
*/
#define IS_N3102_ENABLED()		(PAN_INC() != 0 && PAN_DEC() != 0)
...
**-------------------------------------------------------------------------
*/
#define DECODE_PAN_MAX()    { ul.pan_max_value = (int8)(4*(PAN_MAX()+1));  }
...
#define INC_N3102()                                                     \
        {                                                                     \
          if (IS_N3102_ENABLED() &&                                     \
              (ul.n3102+PAN_INC()) <= ul.pan_max_value  \
             )                                                                \
          {                                                                   \
            ul.n3102 += (int8)PAN_INC();                        \
            UPD_UL_N3102();                                             \
          }                                                                   \
        }
...
**--------------------------------------------------------------
*/
#define IS_N3102_EXPIRED() (ul.n3102 <= 0)
...
**-------------------------------
*/
#define N3104_MAX()           (3 * (BS_CV_MAX() + 3) * ul.cur_res.num_timeslots)

/* Has n3104 reached its maximum defined value
*/
#define IS_N3104_EXPIRED()    (ul.n3104 >= N3104_MAX())
...
#define UPD_UL_STATE()                                                  \
        {                                                               \
          ul_diag_stats_st.grlc_ul_state = (uint32)ul.state;            \
          rlc_log_ul_statistics(FALSE);                                 \
        }
#define UPD_UL_SUBSTATE()                                               \
        {                                                               \
          ul_diag_stats_st.grlc_ul_substate = (uint32)ul.realloc_state; \
          rlc_log_ul_statistics(FALSE);                                 \
        }

#define UPD_UL_VA()           ul_diag_stats_st.ul_seq_va = (uint8)ul.va;
#define UPD_UL_VS()           ul_diag_stats_st.ul_seq_vs = (uint8)ul.vs;
#define UPD_UL_NSEG()         ul_diag_stats_st.ul_seq_nseg = (uint8)ul.seg_ind;

#define UPD_UL_VSF()          ul_diag_stats_st.ul_seq_vsf = (uint8)ul.vsf;
#define UPD_UL_PACK_VSF()     ul_diag_stats_st.ul_seq_pack_vsf = (uint8)ul.pack_vsf;
#define UPD_UL_NACK_VSF()     ul_diag_stats_st.ul_seq_nack_vsf = (uint8)ul.nack_vsf;

#define UPD_UL_N3102()        ul_diag_stats_st.ul_n3102 = (int8)ul.n3102;
#define UPD_UL_N3104()        ul_diag_stats_st.ul_n3104 = (int8)ul.n3104;
...
#define UPD_UL_NSEG()
#define UPD_UL_VSF()
#define UPD_UL_PACK_VSF()
#define UPD_UL_NACK_VSF()
#define UPD_UL_N3102()
#define UPD_UL_N3104()
