...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcgrp.c#3 $
***
...
)
{
  ...
  */
  blk_size = get_blocks_payload_size(cs, tlli_cs, ti);
...
)
{
  ...
  /* (payload) block size given coding scheme, and tlli indicator (no MAC header)
  */
  blk_size = get_blocks_payload_size(mcs, tlli_cs, ti);
  ...
  */
  if (ul.b_li_0_exists)
  {
    ...
    */
    ...
    {
      MSG_GERAN_LOW_1_G("ELRU e_pg_calc_init octets = pad_octets(%u)",octets);
    ...
    */
    if (octets > tbci_ptr->pad_octets)
    {
      MSG_GERAN_LOW_3_G("ELRU e_pg_calc_init octets(%u) > pad_octets(%u)",
              octets,
              tbci_ptr->pad_octets,
              0);
    ...
    */
    ...
    MSG_GERAN_LOW_0_G("ERLU e_pg_calc_init clr b_li_o_exists");
...
)
{
  ...
  */
  ...
  ul.tbc = pg.tbci.num_blks;
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_ADJ
  MSG_GERAN_LOW_3_G("GRLU pgadj tbc=%u new=%u abs=%u",
    ul.tbc,pg.tbci.num_blks,ul.abs_bsn);
#endif
...
===
===========================================================================*/
static boolean check_tx_cv_and_reprocess (void)
{
  ...
  */
  if (((next_pdu_ptr != NULL) &&
       (cur_pdu_ptr != NULL)  &&
        next_pdu_ptr->rlc_mode != cur_pdu_ptr->rlc_mode
      ) ||
      ul.b_forced_release
     )
  {
    ...
    */
    if(cur_pdu_ptr != NULL)
    {
      ...
      */
      ...
      if ( next_pdu_ptr != NULL)
      {
        MSG_GERAN_HIGH_3_G("GRLU pg close %u new grp %u tbc=%u",
  ...
  else if (IS_EXT_UTBF_ACTIVE())
  {
    if (IS_CV_NOT_ZERO())
    {
      /* Action (1): group PDU as normal. Reduced message level to LOW
      */
      MSG_GERAN_LOW_3_G("GRLU ch_grp (1) cv=%u ptx_cv=%u pak_cv=%u",
              ul.cv,
              ul.ptx_cv,
              ul.pak_cv
             );
    ...
    else /* ul.cv is zero */
    {
      ...
      */
      if ( IS_TX_CV_NOT_ZERO() ||
           ((IS_TX_CV_ZERO()) &&
            (
              (grlc_ul_fifo_is_area_full(TXA_PTX)) ||
              ((ul.rlc_mode == GRLC_MODE_UNACK) && (grlc_ul_fifo_is_area_full(TXA_UNACK)))
             )
           )
         )
      {
        ...
        */
        MSG_GERAN_MED_3_G("GRLU ch_grp (2) cv=%u ptx_cv=%u pak_cv=%u",
                ul.cv,
                ul.ptx_cv,
                ul.pak_cv
               );
      ...
      else
      {
        ...
        */
        MSG_GERAN_LOW_3_G("GRLU ch_grp (3) cv=%u ptx_cv=%u pak_cv=%u",
                ul.cv,
                ul.ptx_cv,
                ul.pak_cv
               );
  ...
  else if (IS_TX_CV_INACTIVE())
  {
    if (IS_CV_INACTIVE())
    {
      /* segment CV and transmit CV are both inactive hence group PDU(1)
      */
      MSG_GERAN_LOW_3_G("GRLU ch_grp (1) both CV NOT active ptx_cv=%u pak_cv=%u cv=%u grp",
            ul.ptx_cv,
            ul.pak_cv,
            ul.cv);
      return TRUE;
    ...
    {
      MSG_GERAN_LOW_3_G("GRLU ch_grp (2) rep, grp ptx_cv=%u pak_cv=%u cv=%u grp",
            ul.ptx_cv,
            ul.pak_cv,
            ul.cv);
  ...
  {
    MSG_GERAN_LOW_3_G("GRLU ch_grp (4) no rep, no grp ptx_cv=%u pak_cv=%u cv=%u !grp",
          ul.ptx_cv,
          ul.pak_cv,
          ul.cv);
...
)
{
  ...
  */
  ...
  if (IS_GT_MODULO_COMP(cur_pdu_ptr->psn,pdu_ptr->pg_last_psn,32768))
  {
...
)
{
  ...
  */
  if (next_ptr->rlc_mode == cur_pdu_ptr->rlc_mode &&
     ...
     )
  {
    ...
    */
    cur_pdu_ptr->pg_pdus++;
    cur_pdu_ptr->pg_octets += next_ptr->len;

    /* Mark the last PSN of this open group, for now
    */
    cur_pdu_ptr->pg_last_psn = next_ptr->psn;

#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_GENERAL
    MSG_GERAN_HIGH_3_G("GRLU pg add %u open tbc=%u num_blks=%u",
              cur_pdu_ptr->pg_last_psn,
              ul.tbc,
...
===
===========================================================================*/
static void group_pdus_on_queue (void)
{
  ...
  if (ul.llc_pdus > 0)
  {
    ...
    */
    ...
    if (next_ptr != NULL)
    {
      ...
      */
      next_ptr = seek_next_pdu_in_group(cur_pdu_ptr);
    }
    ...
    do
    {
      if (next_ptr != NULL)
      {
        /* Group PDU to cur_pdu_ptr and set cur_pdu_ptr->pg_state
        */
        if (check_tx_cv_and_reprocess() && cur_pdu_ptr->pg_state != PGS_CLOSE)
        {
          if (group_or_close_pdu( next_ptr))
          {
            if( ul.cur_res.tbf_mode == TBF_MODE_GPRS )
              pg_adjust_tbc( &pg.tbci, next_ptr, ul.cur_res.cs, ul.tlli_cs,
                            (uint8)IS_TLLI_REQUIRED());
            else
              e_pg_adjust_tbc( &pg.tbci, next_ptr, ul.cur_res.cs, ul.tlli_cs,
                              (uint8)IS_TLLI_REQUIRED());

#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
            MSG_GERAN_MED_3_G("GRLU pgxf ONQ add psn %u len = %u blks=%u",
                    next_ptr->psn,
                    next_ptr->len,
                    pg.tbci.num_blks);
#endif
    ...
    } while (next_ptr != NULL && cur_pdu_ptr->pg_state != PGS_CLOSE);
  }
} /* group_pdus_on_queue */
...
===
===========================================================================*/
static void group_pdus_on_queue_for_reprocessing (void)
{
  gprs_grr_ul_mes_t*    next_ptr;
...
)
{
  ...
  */
  ...
  if (blk_size == 0)
  {
    MSG_GERAN_LOW_3_G("GRLU blk_size=0, cs=%d tlli_cs=%d ti=%d",cs, tlli_cs, ti);
...
)
{
  ...
  */
  ...
  if (blk_size == 0)
  {
    MSG_GERAN_LOW_3_G("GRLU blk_size=0, mcs=%d tlli_cs=%d ti=%d",mcs, tlli_cs, ti);
...
)
{
  if (ul.cur_res.tbf_mode == TBF_MODE_EGPRS)
  {
    e_get_num_blks_ex( octets, ul.cur_res.cs,ul.tlli_cs,ti,b_next_pdu_avail);
  }
  else
  {
    get_num_blks_ex( octets,ul.cur_res.cs,ul.tlli_cs,ti,b_next_pdu_avail);
  }
...
)
{
  ...
  */
  ...
  /* whole number of blocks
  */
  ul.num_blks = octets / blk_size;

  /* Number of octets in the partial block (remained) -- avoid using the % operator
  */
  ul.partial_octets = octets - (uint16)(ul.num_blks * blk_size);
  ...
  */
  ...
  if (ul.partial_octets == 0)
  {
    ...
    else
    {
      ...
      */
      ul.pad_octets = blk_size;
      ul.b_ext_octet = FALSE;
    }
  }
  else
  {
    ...
    ** an extension
    */
    ul.pad_octets = (blk_size - ul.partial_octets) - 1;
...
===
===========================================================================*/
void pg_clr_tbc_info (  grlc_ul_tbc_info_t *tbci_ptr)
{
  ...
  */
  ...
  MSG_GERAN_HIGH_0_G("ul.ptx_cv = BS_CV_CONST in pg_clr_tbc_info()");
}
...
void pg_calc_xfer_value (void)
{
  ...
  ** there a next PDU !
  */
  pg_calc_initial_tbc( &pg.tbci,
                      cur_pdu_ptr,
                      ul.cur_res.cs,
                      ul.tlli_cs,
                      ti,
          (uint16)IS_GT_MODULO_COMP(cur_pdu_ptr->pg_last_psn,cur_pdu_ptr->psn,32768));
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
  MSG_GERAN_MED_3_G("GRLU pgxf 1st psn %u len = %u blks=%u",
          cur_pdu_ptr->psn,
          cur_pdu_ptr->len,
          pg.tbci.num_blks);
#endif
  ...
  */
  if (IS_GT_MODULO_COMP(cur_pdu_ptr->pg_last_psn,cur_pdu_ptr->psn,32768))
  {
    ...
    if (next_pdu_ptr != NULL)
    {
      ...
      */
      pg_adjust_tbc( &pg.tbci, next_pdu_ptr, ul.cur_res.cs, ul.tlli_cs, ti);
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
      MSG_GERAN_MED_3_G("GRLU pgxf add psn %u len = %u blks=%u",
              next_pdu_ptr->psn,
              next_pdu_ptr->len,
              pg.tbci.num_blks);
#endif
  ...
  */
  ...
  if (pdu_ptr != NULL)
  {
    do
    {
      if (IS_GE_MODULO_COMP(cur_pdu_ptr->pg_last_psn,pdu_ptr->psn,32768))
      {
        pg_adjust_tbc( &pg.tbci, pdu_ptr, ul.cur_res.cs, ul.tlli_cs, ti);
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
        MSG_GERAN_MED_3_G("GRLU pgxf ONQ add psn %u len=%u blks=%u",
                pdu_ptr->psn,
                pdu_ptr->len,
      ...
      pdu_ptr = q_next(&ULQ(), &pdu_ptr->link);
             ...
             IS_GE_MODULO_COMP(cur_pdu_ptr->pg_last_psn,pdu_ptr->psn,32768)
            );
  }
  ...
  */
  ul.tbc = pg.tbci.num_blks;
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
  MSG_GERAN_MED_3_G("GRLU pgxf tbc=%u new=%u abs=%u",ul.tbc,pg.tbci.num_blks,ul.abs_bsn);
#endif
...
===
===========================================================================*/
void e_pg_calc_xfer_value (void)
{
  gprs_grr_ul_mes_t*  pdu_ptr;
  uint8               ti;
  ...
  ** there a next PDU !
  */
  e_pg_calc_initial_tbc( &pg.tbci, cur_pdu_ptr, ul.cur_res.cs, ul.tlli_cs, ti,
    (uint16)IS_GT_MODULO_COMP(cur_pdu_ptr->pg_last_psn,cur_pdu_ptr->psn,32768));
  ...
  */
  if (IS_GT_MODULO_COMP(cur_pdu_ptr->pg_last_psn, cur_pdu_ptr->psn, 32768))
  {
    ...
    if (next_pdu_ptr != NULL)
    {
      /* Next_pdu is loaded so add its length to TBC-info
      */
      e_pg_adjust_tbc( &pg.tbci, next_pdu_ptr, ul.cur_res.cs, ul.tlli_cs, ti);
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
      MSG_GERAN_MED_3_G("GRLU pgxf add psn %u len = %u blks=%u",
              next_pdu_ptr->psn,
              next_pdu_ptr->len,
  ...
  */
  ...
  if (pdu_ptr != NULL)
  {
    do
    {
      if (IS_GE_MODULO_COMP(cur_pdu_ptr->pg_last_psn, pdu_ptr->psn, 32768))
      {
        e_pg_adjust_tbc( &pg.tbci, pdu_ptr, ul.cur_res.cs, ul.tlli_cs, ti);
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
        MSG_GERAN_MED_3_G("GRLU pgxf ONQ add psn %u len=%u blks=%u",
                pdu_ptr->psn,
                pdu_ptr->len,
  ...
  */
  ul.tbc = pg.tbci.num_blks;
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_XFER
  MSG_GERAN_MED_3_G("ERLU pgxf tbc=%u new=%u abs=%u",ul.tbc,pg.tbci.num_blks,ul.abs_bsn);
#endif
...
===
===========================================================================*/
void group_pdu_init (  gprs_grr_ul_mes_t* pdu_ptr)
{
  INC_MOD(pg.psn,32768);
  pdu_ptr->psn = pg.psn;
  pdu_ptr->pg_last_psn = pg.psn;
  pdu_ptr->pg_state = PGS_INIT;
...
void group_pdus (void)
{
  ...
  */
  ...
  if (cur_pdu_ptr == NULL)
  {
    MSG_GERAN_HIGH_1_G("GRLU pg cur pdu NULL (tbc=%u)",ul.tbc);
    return;
  }

  switch (cur_pdu_ptr->pg_state)
  {
    case PGS_INIT:
      /*---------------------------------------------
      ...
      */
      cur_pdu_ptr->pg_pdus = 1;
      cur_pdu_ptr->pg_octets = cur_pdu_ptr->len;

      /* The state is now PGS_OPEN
      */
      cur_pdu_ptr->pg_state = PGS_OPEN;

#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_GENERAL
      MSG_GERAN_HIGH_3_G("GRLU pg INIT psn %u tbc=%u last_p=%u",
                cur_pdu_ptr->psn,
                ul.tbc,
                cur_pdu_ptr->pg_last_psn);
#endif
    ...
    case PGS_OPEN:
      ...
      */
      ...
      if (next_pdu_ptr != NULL)
      {
        if (IS_GT_MODULO_COMP(next_pdu_ptr->psn,cur_pdu_ptr->pg_last_psn,32768))
        {
          /* Group the 'next_pdu_ptr'
          */
          next_ptr = next_pdu_ptr;

          /* If reprocessing takes place then this is set
          */
          b_check = check_tx_cv_and_reprocess();

          if (b_check && cur_pdu_ptr->pg_state != PGS_CLOSE)
          {
            if (group_or_close_pdu( next_ptr))
            {
              /* Group only if check_tx_cv_and_reprocess() had not performed PDU grouping
              */
              if (ul.b_ext_utbf_init_group)
              {
                if( ul.cur_res.tbf_mode == TBF_MODE_GPRS )
                  pg_calc_initial_tbc
                    (
                      ...
                      ul.tlli_cs,
                      (uint8)IS_TLLI_REQUIRED(),
                      (uint16)IS_GT_MODULO_COMP(cur_pdu_ptr->pg_last_psn,
                                                cur_pdu_ptr->psn,
                                                32768
                                               )
                    );
                ...
                */
                ul.tbc = pg.tbci.num_blks;
                MSG_GERAN_MED_3_G("GRLU pgxf tbc=%u new=%u abs=%u",
                        ul.tbc,
                        pg.tbci.num_blks,
                        ul.abs_bsn);
          ...
          {
            MSG_GERAN_LOW_3_G("GRLU pg check_tx_cv ret FALSE psn=%u tbc=%u abs=%u",
                    cur_pdu_ptr->psn,
                    ul.tbc,
                    ul.abs_bsn);
          }
      ...
      */
      if (b_check)
      {
        MSG_GERAN_LOW_1_G("group_pdus calls group_pdus_onq b_check=%u",b_check);
        group_pdus_on_queue();
      }
      break;
...
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_GENERAL
      MSG_GERAN_HIGH_3_G("GRLU PGS close last_psn %u tbc=%u num_blks=%u",
                cur_pdu_ptr->pg_last_psn,
                ul.tbc,
                cur_pdu_ptr->pg_num_blks);
#endif
    ...
    default:
      ...
      MSG_GERAN_LOW_1_G("GRLU PGS bad state %d",cur_pdu_ptr->pg_state);
      break;

  } /* switch pg_state */

#ifdef  GSM_GPRS_GRLU_SHOW_PDU_GROUPING_GENERAL
  MSG_GERAN_MED_3_G("GRLU gp end tbc=%u psn=%u last_psn=%u",
          ul.tbc,
          cur_pdu_ptr->psn,
          cur_pdu_ptr->pg_last_psn);
#endif
...
===
===========================================================================*/
void ungroup_pdu (  gprs_grr_ul_mes_t*  pdu_ptr)
{
  ...
  */
  ...
  if (cur_pdu_ptr == NULL)
  {
    MSG_GERAN_LOW_0_G("GRLU unpg pdu NULL");
    return;
  }
  ...
  */
  ...
  else
  {
    ...
    MSG_GERAN_MED_1_G("GRLU PGS ungp neg pg_pdus=%u",pdu_ptr->pg_pdus);
#endif
  }
  ...
  */
  ...
  {
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_UNGROUPING
    MSG_GERAN_MED_2_G("GRLU PGS ungp neg pg_octs=%u len=%u",pdu_ptr->pg_octets, pdu_ptr->len);
#endif
  }
  ...
  */
  ...
  {
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_UNGROUPING
    MSG_GERAN_MED_1_G("GRLU ungp pg_close psn=%u",cur_pdu_ptr->psn);
#endif
  }
  else
  {
    ...
    */
    if (next_pdu_ptr == NULL)
    {
      ...
      */
      ...
      {
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_UNGROUPING
        MSG_GERAN_MED_2_G("GRLU ungp xfer psn(%u) to psn(%u)",pdu_ptr->psn, next_ptr->psn);
#endif
    ...
    else
    {
      ...
      */
#ifdef  GSM_GPRS_GRLU_SHOW_PDU_UNGROUPING
      MSG_GERAN_MED_2_G("GRLU ungp xfer psn(%u) to psn(%u)",pdu_ptr->psn, next_pdu_ptr->psn);
#endif
      next_pdu_ptr->pg_state = pdu_ptr->pg_state;
      next_pdu_ptr->pg_pdus = pdu_ptr->pg_pdus;
      next_pdu_ptr->pg_octets = pdu_ptr->pg_octets;
      next_pdu_ptr->pg_octets_rem = pdu_ptr->pg_octets_rem;
      next_pdu_ptr->pg_last_psn = pdu_ptr->pg_last_psn;
    }
  }
} /* ungroup_pdu() */
...
===
===========================================================================*/
void group_pdus_for_reprocessing (void)
{
  /* Points to result of the search
  */
  gprs_grr_ul_mes_t*    next_ptr;
  ...
  switch (cur_pdu_ptr->pg_state)
  {
    ...
    default:
      ...
      MSG_GERAN_HIGH_1_G("GRLU PGS bad state %d",cur_pdu_ptr->pg_state);
      break;
