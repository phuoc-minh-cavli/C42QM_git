...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcmac.c#4 $
***
...
===
===========================================================================*/
mac_rlc_msg_t* grlc_get_cmd_buf_mac_dyn(void)
{
...
===
===========================================================================*/
void grlc_put_cmd_mac_dyn(  mac_rlc_msg_t* msg_ptr)
{
  if (msg_ptr == NULL)
  {
  ...
  {
    gmac_store_trace_send_msg_buf( MS_MAC_RLC, msg_ptr);
    switch(msg_ptr->msg.sig)
    {
      ...
      default:
        MSG_GERAN_LOW_0_G("GRLC error CFA LOG unknown message");
        break;
    } /* switch on sig */
    ...
    */
    switch (msg_ptr->msg.sig)
    {
      ...
      */
      case MR_UL_PKT_ACKNACK_IND:
        msg_ptr->msg.ul_pkt_acknack_ind.blk_cnt = grlc_ul_block_counter;
        /* add command to rlc ul MAC queue and set the appropriate signal
        */
        GRLC_UL_MSG_LOCK();

        q_put(&UMQ(), &msg_ptr->link);
        RLC_UL_SET_MAC_SIG();
      ...
      case MR_UL_CONTENTION_IND:
        msg_ptr->msg.ul_tlli_ind.blk_cnt = grlc_ul_block_counter;
        /* add command to rlc ul MAC queue and set the appropriate signal
        */
        GRLC_UL_MSG_LOCK();
...
********************************************************/
      ...
      */
      ...
      default:
        ...
        */
        MSG_GERAN_LOW_1_G("Invalid MAC sig %d",msg_ptr->msg.sig);
        break;
    } /* end switch */
  } /* msg_ptr OK */
}
...
===
===========================================================================*/
boolean grlc_ul_is_cv_active_dyn(void)
{
...
===
===========================================================================*/
void grlc_ul_restart_await_ass_guard_dyn(  uint32 time_ms)
{
  if (TMR_ACTIVE == ul.timer_await_ass_guard.state)
  {
    (void)rex_set_timer(&(ul.timer_await_ass_guard.rex_timer), time_ms);
    MSG_GERAN_MED_1_G("GRLU re-start await_ass_guard timer. Time in ms %d", time_ms);
  }
...
===
===========================================================================*/
void grlc_ul_stop_await_ass_guard_dyn(void)
{
