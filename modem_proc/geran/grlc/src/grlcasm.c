...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcasm.c#3 $   
***
...
)
{
  ...
  */
  if (cur_dl_pdu.len == 0)
  {
    cur_dl_pdu.first_bsn = bsn;
    cur_dl_pdu.last_bsn = bsn;
  }
  ...
  **----------------------------------------------------
  */
  if ((cur_dl_pdu.len + len) <= GRLC_DL_PDU_MAX_LEN)
  {
    ...
    */
    if (blk_len == len)
    {
      gprs_pdu_augment_tail(&(cur_dl_pdu.dsm_ptr), dsm_ptr_ptr);
      cur_dl_pdu.len += len;
    }
    else if ( blk_len > len )
    ...
    */
    {
      ...
      */
      ...
      gprs_pdu_augment_tail(&(cur_dl_pdu.dsm_ptr), &temp_dsm_ptr);
    ...
    {
      MSG_GERAN_ERROR_2_G("ERLD append_pdu blk_len=%d < len=%d",blk_len,len);

      /* Append the pdu conveyed by dsm_ptr_ptr */
      gprs_pdu_augment_tail(&(cur_dl_pdu.dsm_ptr), dsm_ptr_ptr);
      cur_dl_pdu.len += blk_len;
    }
  ...
  else
  {
    ...
    */
    rem_space = GRLC_DL_PDU_MAX_LEN - cur_dl_pdu.len;
    ...
    */
    ...
    cur_dl_pdu.len = GRLC_DL_PDU_MAX_LEN;
...
===  SIDE EFFECTS
===
===    cur_dl_pdu.status is changed to CUR_PDU_UNINIT after the pdu is sent.
===  
===========================================================================*/
static void send_llc_pdu (void)
{
  ...
  */
  ...
  if (cur_dl_pdu.len == 0)
  {
    MSG_GERAN_HIGH_0_G("ERLD req sending zero-length PDU");
    return;
  }
  ...
  else
  {
    ...
    */
    if (cur_dl_pdu.rlc_mode == GRLC_MODE_ACK)
    {
      ...
      */
      dl_diag_stats_st.ack_rx_pdu_cnt++;
      dl_diag_stats_st.ack_rx_oct_cnt += (uint32)cur_dl_pdu.len;

#ifdef  GRLD_SHOW_PDU_SENT
      MSG_GERAN_HIGH_3_G("ERLD send_pdu pnum=%d plen=%d octs=%d",
        dl_diag_stats_st.ack_rx_pdu_cnt,
        cur_dl_pdu.len,
        dl_diag_stats_st.ack_rx_oct_cnt
       );
#endif
#else /* !FEATURE_GERAN_REDUCED_DEBUG */
#ifdef  GRLD_SHOW_PDU_SENT
      MSG_GERAN_HIGH_1_G("ERLD send_pdu plen=%d", cur_dl_pdu.len);
#endif
    ...
    else
    {
      ...
      */
      dl_diag_stats_st.unack_rx_pdu_cnt++;
      dl_diag_stats_st.unack_rx_oct_cnt += (uint32)cur_dl_pdu.len;
      if (dl_diag_stats_st.unack_oct_filled_cnt != 0)
      {
        dl_diag_stats_st.unack_pdu_filled_cnt++;
      }
      ...
      MSG_GERAN_HIGH_3_G("ERLD send_pdu pnum=%d plen=%d octs=%d",
        dl_diag_stats_st.unack_rx_pdu_cnt,
        cur_dl_pdu.len,
        dl_diag_stats_st.unack_rx_oct_cnt
       );
#endif
#else /* !FEATURE_GERAN_REDUCED_DEBUG */
#ifdef  GRLD_SHOW_PDU_SENT
    ...
    dl_msg_ptr -> ll_pdu = cur_dl_pdu.dsm_ptr;
    ...
    */
    dl_msg_ptr->tlli = dl.dl_ass.tlli;
#endif

    /* mark the pdu as used and requires init again
    */
    cur_dl_pdu.status = DL_PDU_STATUS_UNINIT;

#ifdef  GRLD_SHOW_PDU_SENT
    MSG_GERAN_HIGH_3_G("dpdu=0x%x tlli=0x%x FrstLast=%08x",
              dl_msg_ptr->ll_pdu,
              dl_msg_ptr->tlli,
              (((uint32)(cur_dl_pdu.first_bsn & 0x07ff) << 16) | 
               (uint32)(cur_dl_pdu.last_bsn  & 0x07ff)
              )
            );
#endif
  ...
  */
  if (blk_ptr->ext == RM_EXT_NO_OCTET_FOLLOWS)
  {
    ...
    */
    ...
    {
      (void)append_pdu(  &(blk_ptr->dsm_payload_ptr),
      ...
      MSG_GERAN_HIGH_0_G("GRLD Info Sent PDU FBI");
#endif
      cur_dl_pdu.last_bsn = blk_ptr->bsn;
      send_llc_pdu();
    }
    else /* FBI is clear */
    {
      ...
      */
      if (append_pdu(  &(blk_ptr->dsm_payload_ptr), 
                      (uint16)(GRLC_DATA_BLOCK_LEN[blk_ptr->cs] - 
                               RM_RLC_HDR_LEN),
                      blk_ptr->bsn                
         ...
         )
      {
        ...
        ** as per 9.1.11
        */
        dl.b_pdu_truncated = TRUE;

        /* pdu is now full so send it off
        */
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
        MSG_GERAN_HIGH_0_G("GRLD Info Sent PDU FBI=0 no-ext trnctd");
#endif
        cur_dl_pdu.last_bsn = blk_ptr->bsn;
        send_llc_pdu();
      }
  ...
  {
    /*MSG_LOW("GRLD asm ext bsn=%d",dl.bsn,0,0);*/
      ...
      switch(me)
      {
        ...
        case RM_ME_IGNORE:
          ...
          */
          ...
          MSG_GERAN_HIGH_0_G("GRLD Asm Error Unexp ME=00");
          break;
        ...
        case RM_ME_NO_PDU_FOLLOWS_CURRENT_NO_EXT_FOLLOWS_01:
          ...
          */
          ...
          {
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
            MSG_GERAN_HIGH_0_G("GRLD Info Sent PDU no-pdu no-ext (01) li=0");
#endif
            ...
            */
               ...
               )
            {
              ...
              ** send off pdu
              */
              MSG_GERAN_LOW_0_G("GRLD Info Sent PDU p0 e0 01 li=0 trnctd");
              cur_dl_pdu.last_bsn = blk_ptr->bsn;
              send_llc_pdu();
          ...
          {
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
            MSG_GERAN_HIGH_1_G("GRLD Info Sent PDU p0 e0 01 li=%d",blk_ptr->ext_octet[b].gprs.li);
#endif
            /* case 2: 
        ...
        case RM_ME_PDU_FOLLOWS_EXT_FOLLOWS_10:
          ...
          */
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
          MSG_GERAN_HIGH_1_G("GRLD Info Sent p1 e1 10 li=%d",blk_ptr->ext_octet[b].gprs.li);
#endif
          cur_dl_pdu.last_bsn = blk_ptr->bsn;
          send_llc_pdu();
        ...
        case RM_ME_PDU_FOLLOWS_NO_EXT_FOLLOWS_11:
          ...
          */
          ...
          /* pdu is now full so send it off
          */
          MSG_GERAN_LOW_3_G("GRLD Sent p1 e0 11 num_octs=%d li=%d plen=%d",
              num_octets,
              blk_ptr->ext_octet[b].gprs.li,
              GPRS_PDU_LENGTH(cur_dl_pdu.dsm_ptr));
          ...
          */
          ...
          MSG_GERAN_LOW_3_G("GRLD 11 b_l=%d n_octs=%d n_e=%d",
                  blk_ptr->len,
                  num_octets,
          ...
          */
          if (blk_ptr->fbi == RM_FBI_FINAL)
          {
            MSG_GERAN_LOW_1_G("GRLD 11 FBI p_len=%d", GPRS_PDU_LENGTH(blk_ptr->dsm_payload_ptr));
            cur_dl_pdu.last_bsn = blk_ptr->bsn;
            send_llc_pdu();
          }
        ...
        default:
          ...
          MSG_GERAN_ERROR_1_G("GRLD Asm Error Unexp ME=%d",me);
          break;
...
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
      MSG_GERAN_HIGH_1_G("ERLD Info Sent PDU FBI last-bsn=%u", blk_ptr->bsn);
#endif
      cur_dl_pdu.last_bsn = blk_ptr->bsn;
      send_llc_pdu();
    }
    else /* FBI is clear */
    {
      ...
      */
         ...
         )
      {
        ...
        */
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
        MSG_GERAN_MED_1_G("ERLD Info Sent PDU FBI=0 no-ext trnctd last-bsn=%u bsn2=%u", blk_ptr->bsn);
#endif
        cur_dl_pdu.last_bsn = blk_ptr->bsn;
        send_llc_pdu();
      } /* truncated */
    ...
    for (b=next_ext_octet_index; b<blk_ptr->num_ext_octets; b++)
    {
      ...
      */
      ...
      /* b must be 0 when li=0. Already validated */
      {
        ...
        */
        cur_dl_pdu.last_bsn = MOD_ESNS(blk_ptr->bsn-1);
        send_llc_pdu();
               ...
               blk_ptr->ext_octet[b].egprs.ext == 1)
      {
        ...
        */
        if (cur_dl_pdu.status == DL_PDU_STATUS_INIT)
        {
#ifdef  GRLD_SHOW_BLOCK_ASSEMBLED
          MSG_GERAN_HIGH_1_G("ERLD Info Sent PDU last-bsn=%u", blk_ptr->bsn);
#endif
      ...
      MSG_GERAN_HIGH_1_G("ERLD Info Sent PDU FBI last-bsn=%u",blk_ptr->bsn);
        ...
        */
        if (blk_ptr->dsm_payload_ptr != NULL)
        {
          MSG_GERAN_LOW_1_G("ERLD Asm Free trailing %u octets", GPRS_PDU_LENGTH(blk_ptr->dsm_payload_ptr));
          GPRS_PDU_FREE(&(blk_ptr->dsm_payload_ptr));
        }
      } /* end ext == 1 */
      else
      {
        MSG_GERAN_HIGH_2_G("GRLD Asm Error Unexp LI/E %d %d", 
                  blk_ptr->ext_octet[b].egprs.li,
                  blk_ptr->ext_octet[b].egprs.ext);
      }
    }   /* for loop to process the extension octets */
