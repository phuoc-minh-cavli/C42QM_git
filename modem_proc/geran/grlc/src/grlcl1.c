...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcl1.c#4 $
***
...
static pl1_rlc_msg_t  grlc_l1isr_mes_buf[ GRLC_L1ISR_MAX_BUFS ];
static q_type         grlc_l1isr_free_list;
...
static pl1_rlc_msg_t *gprs_rlc_l1isr_get_buffer(void);
...
void gprs_rlc_l1isr_init_buffers(void)
{
  uint32 ctr;

  (void)q_init( &grlc_l1isr_free_list );

  for ( ctr = 0; ctr < GRLC_L1ISR_MAX_BUFS; ctr++ )
  {
    (void)q_link( &grlc_l1isr_mes_buf[ctr], &(grlc_l1isr_mes_buf[ctr].link) );
    q_put( &grlc_l1isr_free_list, &(grlc_l1isr_mes_buf[ctr].link) );
  }
}
...
===
===========================================================================*/
pl1_rlc_msg_t *grlc_get_cmd_buf_pl1_dyn(void)
{
  ...
  */
  ...
  pl1_rlc_msg_ptr = gprs_rlc_l1isr_get_buffer();
...
===
===========================================================================*/
void grlc_put_cmd_pl1_dyn(pl1_rlc_msg_t* msg_ptr)
{
  if (msg_ptr == NULL)
  {
  ...
  else
  {
    ...
    */
    if (msg_ptr->sig == PR_DL_EPDAN_REQ)
    {
      ...
      q_put(&DPQ(), &(msg_ptr->link));
      RLC_DL_SET_PL1_EPDAN_REQ_SIG();
    ...
    else
    {
      ...
      */
      MSG_GERAN_LOW_1_G("Invalid PL1 sig %d",msg_ptr->sig);
    }
  }
}
...
===
===========================================================================*/
boolean e_grlc_dl_get_window_info_dyn(  e_grlc_dl_window_info_t* info_ptr)
{
  boolean   is_win_info_valid = FALSE;

  if( (dl.state == DS_RX_ACK) || (dl.state == DS_RX_ACK_PENDING_RELEASE) )
  {
    /* Get the beginning of the DL window */
    info_ptr->start_bsn = dl.vq;

    /* Get the end of the DL window. This is ( V(Q) + dl_window_size ) % 2048 */
    info_ptr->end_bsn   = MOD_ESNS(dl.vq + dl.ws);
...
===
===========================================================================*/
void grlc_dl_set_oper_win_size_dyn(uint16 oper_win_sz)
{
  dl.ir_oper_win_size = oper_win_sz;
  MSG_GERAN_MED_1_G("ERLD EDGE IR operational window size %d",dl.ir_oper_win_size);
}
...
===
===========================================================================*/
void e_grlc_dl_copy_pld_for_dump_dyn(uint16* hdr_ptr, uint8* pld1_ptr, uint8* pld2_ptr)
{
  static uint8 idx = 0; /* static_dual_spaced_ignore - debug fn */

  /* Copy the header */
  (void) memscpy( (void*) dl.dump_buff[idx].hdr, sizeof(dl.dump_buff[idx].hdr), (void*) hdr_ptr, 6 ); /* 3 of uint16  */

  /* Copy the first DUMP_PLD_LEN octets of the DL blocks   */
  (void) memscpy( dl.dump_buff[idx].pld1, sizeof(dl.dump_buff[idx].pld1), pld1_ptr, DUMP_PLD_LEN );
  (void) memscpy( dl.dump_buff[idx].pld2, sizeof(dl.dump_buff[idx].pld2), pld2_ptr, DUMP_PLD_LEN );
...
===
===========================================================================*/
void grlc_send_ph_data_ind_dyn(void)
{
  if (!RLC_DL_IS_PL1_SIG_SET())
  {
    RLC_DL_SET_PL1_SIG();
  }
}
...
===
===========================================================================*/
void grlc_send_ph_ready_to_send_ind_dyn(void)
{
  if (!RLC_UL_IS_PL1_SIG_SET())
  {
    RLC_UL_SET_PL1_SIG();
  }
}
...
static pl1_rlc_msg_t *gprs_rlc_l1isr_get_buffer(void)
{
  pl1_rlc_msg_t *sig_ptr = q_get( &grlc_l1isr_free_list );

  if ( q_cnt( &grlc_l1isr_free_list ) <= GRLC_L1ISR_LOWATER_BUFS )
  {
    MSG_GERAN_LOW_0_G("GRLC: Get Static Mes Buf: Lo Water!");
  }
...
void gprs_rlc_l1isr_free_buffer(   pl1_rlc_msg_t *sig_ptr )
{
  if ( sig_ptr != NULL )
  {
    (void)q_link( sig_ptr, &sig_ptr -> link );
    q_put( &grlc_l1isr_free_list, &sig_ptr -> link );
  }
}
