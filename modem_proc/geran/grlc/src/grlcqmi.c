...
$Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcqmi.c#4 $
...
===
===========================================================================*/
static uint32 grlc_current_get_tx_data_rate (void)
{
  if (ul.cur_res.tbf_mode == TBF_MODE_EGPRS)
  {
    return
      grlc_get_data_rate(ul.cur_res.num_timeslots,
                         (ul.cur_res.cs > RM_MCS_6) ?
                         (uint16)(2 * E_GRLC_DATA_BLOCK_LEN[ul.cur_res.cs]) :
                         (uint16)(E_GRLC_DATA_BLOCK_LEN[ul.cur_res.cs])
                        );
  }
  else
  {
    return
      grlc_get_data_rate(ul.cur_res.num_timeslots,
                         (GRLC_DATA_BLOCK_LEN[ul.cur_res.cs] - RM_RLC_HDR_LEN)
                        );
  }
...
===
===========================================================================*/
static uint32 grlc_current_get_rx_data_rate(void)
{
  if ( dl.tbf_mode == TBF_MODE_EGPRS)
  {
    return
      grlc_get_data_rate(gmac_access_func_get_num_dl_timeslots(),
                         (dl.cs > RM_MCS_6) ?
                         (uint16)(2 * E_GRLC_DATA_BLOCK_LEN[dl.cs]) :
                         (uint16)(E_GRLC_DATA_BLOCK_LEN[dl.cs])
                        );
  }
  else
  {
    return
      grlc_get_data_rate(gmac_access_func_get_num_dl_timeslots(),
                         (GRLC_DATA_BLOCK_LEN[dl.cs] - RM_RLC_HDR_LEN)
                        );
  }
...
===
===========================================================================*/
void geran_get_bearer_data_rates_dyn
(
  geran_bearer_data_rate_t  *p_data_rates
)
{
  geran_get_bearer_data_rates_as_id_dyn(p_data_rates, SYS_MODEM_AS_ID_1);
}

void geran_get_bearer_data_rates_as_id_dyn
(
  geran_bearer_data_rate_t  *p_data_rates,
  ...
  */
  if (egprs_capable_cell == TRUE)
  {
    p_data_rates->max_rx_bearer_data_rate = (int32)
      grlc_get_data_rate(max_data_rate[sysinfo.max_egprs_multislot_class][0],
                          (uint16)(E_GRLC_DATA_BLOCK_LEN[RM_MCS_9]*2) );

    p_data_rates->max_tx_bearer_data_rate = (int32)
      grlc_get_data_rate(max_data_rate[sysinfo.max_egprs_multislot_class][1],
                          (uint16)(E_GRLC_DATA_BLOCK_LEN[RM_MCS_9]*2) );
  }
  else
  {
    ...
    */
    ...
    p_data_rates->max_rx_bearer_data_rate = (int32)
      grlc_get_data_rate(max_data_rate[sysinfo.max_gprs_multislot_class][0],
                          (uint16)(GRLC_DATA_BLOCK_LEN[RM_CS_4] - RM_RLC_HDR_LEN));

    p_data_rates->max_tx_bearer_data_rate = (int32)
      grlc_get_data_rate(max_data_rate[sysinfo.max_gprs_multislot_class][1],
                          (uint16)(GRLC_DATA_BLOCK_LEN[RM_CS_4] - RM_RLC_HDR_LEN));
  }


  if ( ul.state == US_ACK_XFER || ul.state == US_UNACK_XFER )
  {
    p_data_rates->current_tx_bearer_data_rate =
      (int32)grlc_current_get_tx_data_rate();
  }
  else
  {
  ...
  if ( dl.state == DS_RX_ACK || dl.state == DS_RX_UNACK )
  {
    p_data_rates->current_rx_bearer_data_rate =
      (int32)grlc_current_get_rx_data_rate();
  }
  else
  {
...
)
{
  ...
  */
  rr_read_public_store(RRPS_MS_MULTISLOT_CLASS,
                       &sysinfo.max_gprs_multislot_class);

  p_multislot_classes->gprs_msc =
    (uint8) sysinfo.max_gprs_multislot_class;


  /* Read the MAX multislot class for EGPRS
  */
  rr_read_public_store(RRPS_MS_EGPRS_MULTISLOT_CLASS,
                       &sysinfo.max_egprs_multislot_class);
  ...
  */
  rr_read_public_store(RRPS_MS_DTM_EGPRS_MULTISLOT_SUBCLASS,
                       &sysinfo.max_dtm_egprs_multislot_subclass);
  p_multislot_classes->dtm_msc =
    (uint8) sysinfo.max_dtm_egprs_multislot_subclass;
