...
***
***
***  grlc_dl_fifo_one_time_init(void)
***  grlc_dl_fifo_clear(void)
***  grlc_dl_fifo_get_cnt(void)
***  grlc_dl_fifo_get_block(void)
***  grlc_dl_fifo_put_block(void)
***
***
*** INITIALIZATION AND SEQUENCING REQUIREMENTS
***
***  grlc_dl_fifo_one_time_init() uses dynamic memory allocation for the data area
***  and therfore should just be called once. Use grlc_dl_fifo_clear() to clear
...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcdlfifo.c#4 $
***
...
#else
static grlc_dl_pl1_fifo_t  grlc_dl_fifo;
#endif
...
rex_crit_sect_type grlc_dl_fifo_crit_sec;
...
===
===========================================================================*/
void grlc_dl_fifo_one_time_init (void)
{
  uint8  b;
  ...
  rex_init_crit_sect(&grlc_dl_fifo_crit_sec);


  GRLC_DL_FIFO_LOCK();

  grlc_dl_fifo.in_ind = 0;
  grlc_dl_fifo.out_ind = 0;
  grlc_dl_fifo.ctr = 0;

  for (b=0; b<GRLC_DL_PL1_FIFO_SIZE; b++)
  {
    grlc_dl_fifo.pl1_pyld_ptrs[b]= NULL;
    grlc_dl_fifo.pl1_blk_hdr[b].loop_dl_blk = 0;
  }

  for (b=0; b<GRLC_DL_PL1_FIFO_SIZE; b++)
  {
    grlc_dl_fifo.arr_elm[b].b_block_put = FALSE;
    grlc_dl_fifo.arr_elm[b].loop_dl_blk = 0;
  }
...
void grlc_dl_fifo_clear (void)
{
  ...
  if( dl.tbf_mode == TBF_MODE_EGPRS )
  {
    do
    {
      res = e_grlc_dl_fifo_get_block ( &pl1_blk);
      if (res == DL_FIFO_GET_OK)
      {
        if( pl1_blk.payload_1_ptr )
        {
          MSG_GERAN_LOW_3_G("GRLD fifo_clr in=%d out=%d ctr=%d",
                  grlc_dl_fifo.in_ind,
                  grlc_dl_fifo.out_ind,
                  grlc_dl_fifo.ctr);
    ...
    */
    if (grlc_dl_fifo.ctr > 0 ||
        (grlc_dl_fifo.ctr == 0 &&
         grlc_dl_fifo.in_ind != grlc_dl_fifo.out_ind
        )
       )
    {
      MSG_GERAN_LOW_3_G("GRLD FIFO re-init (pre-re-init values) in=%u out=%u ctr=%u",
              grlc_dl_fifo.in_ind,
              grlc_dl_fifo.out_ind,
              grlc_dl_fifo.ctr
             );
  ...
  */
  {
    do
    {
      res = grlc_dl_fifo_get_block ( &hdr_ptr, &pyld_ptr);
      if (pyld_ptr != NULL)
      {
        MSG_GERAN_LOW_3_G("GRLD fifo_clr in=%d out=%d ctr=%d",
                grlc_dl_fifo.in_ind,
...
===
===========================================================================*/
uint8 grlc_dl_fifo_get_cnt (void)
{
  uint8 b;
...
===
===========================================================================*/
grlc_dl_fifo_op_result_t grlc_dl_fifo_put_block_dyn
(
...
)
{
  ...
  */
  ...
  if( dl.tbf_mode == TBF_MODE_EGPRS )
  {
    MSG_GERAN_LOW_3_G("GRLD FIFO PL1 put GPRS block whilst in EGPRS mode ctr=%u in=%u out=%u",
                grlc_dl_fifo.ctr,
                grlc_dl_fifo.in_ind,
                grlc_dl_fifo.out_ind
             );
  ...
  {
    if (grlc_dl_fifo.ctr >= GRLC_DL_PL1_FIFO_SIZE)
    {
      res = DL_FIFO_FULL;
    }
    else
    {
      ...
      grlc_dl_fifo.pl1_blk_hdr[grlc_dl_fifo.in_ind]   = *pl1_blk_hdr_ptr_ptr;
      grlc_dl_fifo.pl1_pyld_ptrs[grlc_dl_fifo.in_ind] = pl1_blk_pyld_ptr_ptr;

      /* Delink pointer */
      pl1_blk_pyld_ptr_ptr = NULL;

      INC_MOD(grlc_dl_fifo.in_ind, GRLC_DL_PL1_FIFO_SIZE);
      grlc_dl_fifo.ctr++;
...
)
{
  ...
  else
  {
    ...
    *pl1_blk_hdr_ptr_ptr  = &grlc_dl_fifo.pl1_blk_hdr[grlc_dl_fifo.out_ind];
    *pl1_blk_pyld_ptr_ptr = grlc_dl_fifo.pl1_pyld_ptrs[grlc_dl_fifo.out_ind];
    INC_MOD(grlc_dl_fifo.out_ind, GRLC_DL_PL1_FIFO_SIZE);
    grlc_dl_fifo.ctr--;
...
===
===========================================================================*/
grlc_dl_fifo_op_result_t  e_grlc_dl_fifo_put_block_dyn
(
...
)
{
  ...
  */
  ...
  if( dl.tbf_mode == TBF_MODE_GPRS )
  {
    MSG_GERAN_LOW_3_G("ERLD FIFO PL1 put EGPRS block whilst in GPRS mode ctr=%u in=%u out=%u",
                grlc_dl_fifo.ctr,
                grlc_dl_fifo.in_ind,
                grlc_dl_fifo.out_ind
             );
  ...
  else
  {
    ...
    */
    grlc_dl_fifo.arr_elm[grlc_dl_fifo.in_ind] = *pl1_blk_ptr;

    /* Check block's status */
    if( !grlc_dl_fifo.arr_elm[grlc_dl_fifo.in_ind].b_block_put )
    {
      grlc_dl_fifo.arr_elm[grlc_dl_fifo.in_ind].b_block_put = TRUE;
    }
    else
    {
      MSG_GERAN_LOW_3_G("ERLD FIFO put block with status UNREAD ctr=%d, in_ind=%d, out_ind=%d",
        grlc_dl_fifo.ctr,grlc_dl_fifo.in_ind,grlc_dl_fifo.out_ind);
    }
    ...
    */
    ...
    if( pl1_blk_ptr->payload_1_ptr )
    {
          ...
          (pl1_blk_ptr->payload_1_ptr->references != 1)   )
      {
        ...
        dl_diag_stats_st.dl_gen_purpose_count_b &= 0xffff0000;

        dl_diag_stats_st.dl_gen_purpose_count_b |=
          E_GET_RAW_DL_BSN1(&(pl1_blk_ptr->hdr_arr[0])) & 0xffff07ff;

        dl_diag_stats_st.dl_gen_purpose_count_b |=  (uint32)(pl1_blk_ptr->crc_1 << 12);

        if( !GPRS_PDU_LENGTH(pl1_blk_ptr->payload_1_ptr) )
        {
          dl_diag_stats_st.dl_gen_purpose_count_b |= GRLC_INVAL_1ST_PLD_DSM_LEN;
          MSG_GERAN_ERROR_0_G("ERLD FIFO put block with 1st pld len=0");
        }

        if( pl1_blk_ptr->payload_1_ptr->references != 1)
        {
          dl_diag_stats_st.dl_gen_purpose_count_b |= GRLC_INVAL_1ST_DSM_REF_CNT;
          MSG_GERAN_ERROR_1_G("ERLD FIFO put block with 1st pld dsm ref_cnt=%d",
    ...
    if( pl1_blk_ptr->mcs > RM_MCS_6 )
    {
      ...
      */
      if( pl1_blk_ptr->payload_2_ptr )
      {
        ...
        {
          /* Clear flag before logging the info */
          dl_diag_stats_st.dl_gen_purpose_count_b  &= 0x0000ffff;
          dl_diag_stats_st.dl_gen_purpose_count_b  |= (uint32)
           ( ( MOD_ESNS(E_GET_RAW_DL_BSN1(&(pl1_blk_ptr->hdr_arr[0])) +
                           E_GET_RAW_DL_BSN2(&(pl1_blk_ptr->hdr_arr[0])) ) << 16 ));

          dl_diag_stats_st.dl_gen_purpose_count_b  |= (uint32)(pl1_blk_ptr->crc_2 << 28);

          if( !GPRS_PDU_LENGTH(pl1_blk_ptr->payload_2_ptr) )
          {
            dl_diag_stats_st.dl_gen_purpose_count_b |= GRLC_INVAL_2ND_PLD_DSM_LEN;
            MSG_GERAN_ERROR_0_G("ERLD FIFO put block with 2nd pld len=0");
          }

          if( pl1_blk_ptr->payload_2_ptr->references != 1)
          {
            dl_diag_stats_st.dl_gen_purpose_count_b |= GRLC_INVAL_2ND_DSM_REF_CNT;
            MSG_GERAN_ERROR_1_G("ERLD FIFO put block with 2nd pld dsm ref_cnt=%d",
...
)
{
  ...
  else
  {
    ...
    ** the Portable Layer 1 radio block
    */
    *pl1_blk_ptr = grlc_dl_fifo.arr_elm[grlc_dl_fifo.out_ind];

    /* Check block's status */
    if( pl1_blk_ptr->b_block_put )
    {
      grlc_dl_fifo.arr_elm[grlc_dl_fifo.out_ind].b_block_put = FALSE;
    }
    else
    {
      MSG_GERAN_LOW_3_G("ERLD FIFO get block with status READ ctr=%d, in_ind=%d, out_ind=%d",
        grlc_dl_fifo.ctr,grlc_dl_fifo.in_ind,grlc_dl_fifo.out_ind);
    }
    ...
    */
    ...
    if( pl1_blk_ptr->payload_1_ptr )
    {
      ...
      {
        /* Clear flag before logging the info */
        dl_diag_stats_st.dl_gen_purpose_count_a &= 0xffff0000;

        dl_diag_stats_st.dl_gen_purpose_count_a |=
          E_GET_RAW_DL_BSN1(&(pl1_blk_ptr->hdr_arr[0])) & 0xffff07ff;

        dl_diag_stats_st.dl_gen_purpose_count_a  |= (uint32)(pl1_blk_ptr->crc_1 << 12);

        if( !GPRS_PDU_LENGTH(pl1_blk_ptr->payload_1_ptr) )
        {
          dl_diag_stats_st.dl_gen_purpose_count_a |= GRLC_INVAL_1ST_PLD_DSM_LEN;
          MSG_GERAN_ERROR_0_G("ERLD FIFO get block with 1st pld len=0");
        }

        if( pl1_blk_ptr->payload_1_ptr->references != 1)
        {
          dl_diag_stats_st.dl_gen_purpose_count_a |= GRLC_INVAL_1ST_DSM_REF_CNT;
          MSG_GERAN_ERROR_1_G("ERLD FIFO get block with 1st pld dsm ref_cnt=%d",
              pl1_blk_ptr->payload_1_ptr->references);
        }
      }
    }
    ...
    if( pl1_blk_ptr->mcs > RM_MCS_6 )
    {
      ...
      */
      if( pl1_blk_ptr->payload_2_ptr )
      {
        ...
        {
          /* Clear flag before logging the info */
          dl_diag_stats_st.dl_gen_purpose_count_a  &= 0x0000ffff;
          dl_diag_stats_st.dl_gen_purpose_count_a  |= (uint32)
           ( ( MOD_ESNS(E_GET_RAW_DL_BSN1(&(pl1_blk_ptr->hdr_arr[0])) +
                           E_GET_RAW_DL_BSN2(&(pl1_blk_ptr->hdr_arr[0])) ) << 16 ));

          dl_diag_stats_st.dl_gen_purpose_count_a |= (uint32)(pl1_blk_ptr->crc_2 << 28);

          if( !GPRS_PDU_LENGTH(pl1_blk_ptr->payload_2_ptr) )
          {
            dl_diag_stats_st.dl_gen_purpose_count_a |= GRLC_INVAL_2ND_PLD_DSM_LEN;
            MSG_GERAN_ERROR_0_G("ERLD FIFO get block with 2nd pld len=0");
          }

          if( pl1_blk_ptr->payload_2_ptr->references != 1)
          {
            dl_diag_stats_st.dl_gen_purpose_count_a |= GRLC_INVAL_2ND_DSM_REF_CNT;
            MSG_GERAN_ERROR_1_G("ERLD FIFO get block with 2nd pld dsm ref_cnt=%d",
              pl1_blk_ptr->payload_2_ptr->references);
          }
        }
      }
...
===
===========================================================================*/
boolean e_grlc_is_rlc_dl_required_context_dyn
(
  uint8     crc_1,
  uint8     crc_2,
  uint8     fbi_e_1,
...
)
{
  ...
  **---------------------------------------------------------------------
  */
  return ((dl.b_first_block_ind_sent == FALSE)
           ||
          (crc_1 && ( (fbi_e_1 & 0x2) ||
                      ((fbi_e_1 & 0x1) == 0)
                    )
          ...
          (esp != ESP_NO_POLL)
           ||
          (grlc_dl_fifo.ctr >= GRLC_DL_FIFO_HIGH_WATER)
         );
...
===
===========================================================================*/
boolean grlc_is_rlc_dl_required_context_dyn
(
...
===
===========================================================================*/
void  grlc_dl_free_excess_dl_blocks(void)
{
  while (grlc_dl_fifo_get_cnt() > 0)
  {
    ...
    {
      e_pl1_dl_blk = grlc_dl_fifo.arr_elm[grlc_dl_fifo.out_ind];

     /* Check block's status */
     if( e_pl1_dl_blk.b_block_put )
     {
       e_pl1_dl_blk.b_block_put = FALSE;
     }
    ...
    {
      pl1_blk_dsm_ptr = grlc_dl_fifo.pl1_pyld_ptrs[grlc_dl_fifo.out_ind];

      if( pl1_blk_dsm_ptr != NULL )
      {
        GPRS_PDU_FREE( &pl1_blk_dsm_ptr);
        pl1_blk_dsm_ptr = NULL;
      }
    }
...
===
===========================================================================*/
void show_bsn_and_fifo_vars (void)
{
  uint32 u=0UL;

  /* Only for monitoring purposes hence do not require mutex lock for accessing FIFO */

  u =((uint32) (((uint32)grlc_dl_fifo.ctr & 0xff) << 16)    |
      (uint32) (((uint32)grlc_dl_fifo.in_ind & 0xff) << 8)  |
      (uint32) ((uint32)grlc_dl_fifo.out_ind & 0xff)
     );

  MSG_GERAN_LOW_3_G("G/ERLD A/U BSN=%d FBI=%d FIFO 00CtInOu=%08x",dl.bsn,dl.fbi,u);
...
)
{
  ...
  */
  ...
  u3 = ( (uint32) (((uint32)grlc_dl_fifo.ctr & 0xff) << 16)    |
         (uint32) (((uint32)grlc_dl_fifo.in_ind & 0xff) << 8)  |
         (uint32) ((uint32)grlc_dl_fifo.out_ind & 0xff)
       );

  if( (dl.state != DS_RX_UNACK_PENDING_RELEASE) &&
      (dl.state != DS_RX_UNACK )                   )
  {
    MSG_GERAN_MED_3_G("ERLD A blk CFEsCFEe=%08x M0bs1bs2=%08x FIFO 00CtInOu=%08x",u1, u2, u3);
  }
  else
  {
    MSG_GERAN_MED_3_G("ERLD U blk CFEsCFEe=%08x M0bs1bs2=%08x FIFO 00CtInOu=%08x",u1, u2, u3);
  }
  /* avoid compiler and lint warnings about unused variables when MSG
     is defined away to nothing */
  ...
  /* Usefull to see RLC Block contents */
    ...
    for (l=0; l<((len/12)+1); l++)
    {
      ...
      MSG_GERAN_HIGH_3_G("%08x %08x %08x",u1,u2,u3);
    }
#endif
