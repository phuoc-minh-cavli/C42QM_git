...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlcdli.h#3 $   
***
...
typedef struct
{
  ...
  */
  ...
  /* Latch value of S/P field in the current downlink data block (cur_dl_blk).
  ** Required since the assemble function may re-initialise cur_dl_blk
...
#define GRLC_MAX_DL_BLCKS_PER_FR()  (2 * dl.dl_ass.num_dl_ts)
...
#define RM_TMR_T3192_DUR() (t3192_dur_ms[0x07 & sysinfo.t3192_index])
...
/* default until defined by sys info */
  ...
  #define TIMER_START_T3190()                                                      \
          {                                                                           \
            (void)rex_set_timer(&(dl.timer_t3190.rex_timer), RM_TMR_T3190_DUR);  \
            dl.timer_t3190.state = TMR_ACTIVE;                                   \
          }

  #define TIMER_START_T3192()                                                          \
          {                                                                               \
            (void)rex_set_timer(&(dl.timer_t3192.rex_timer), RM_TMR_T3192_DUR()); \
            dl.timer_t3192.state = TMR_ACTIVE;                                       \
          }
...
#define TIMER_STOP_T3190()                                       \
        {                                                           \
          if (dl.timer_t3190.state == TMR_ACTIVE)              \
          {                                                         \
            (void)rex_clr_timer(&(dl.timer_t3190.rex_timer));  \
            dl.timer_t3190.state = TMR_INACTIVE;               \
          }                                                         \
        }

#define TIMER_STOP_T3192()                                       \
        {                                                           \
          if (dl.timer_t3192.state == TMR_ACTIVE)              \
          {                                                         \
            (void)rex_clr_timer(&(dl.timer_t3192.rex_timer));  \
...
**-----------------------------------------------------------------------------
*/
#define IS_BLOCK_SPLIT()                                \
        ( (cur_dl_blk.cs <= RM_MCS_4)  &&             \
...
#define UPD_DL_STATE() {                                    \
          dl_diag_stats_st.grlc_dl_state = (uint32)dl.state;\
          rlc_log_dl_statistics(FALSE);                     \
        }

#define UPD_DL_ACKNACK()  dl_diag_stats_st.ack_nack_req_cnt++;
#define UPD_DL_VR()       dl_diag_stats_st.dl_seq_vr = (uint8)dl.vr;
#define UPD_DL_VQ()       dl_diag_stats_st.dl_seq_vq = (uint8)dl.vq;
#define UPD_DL_SSN()      dl_diag_stats_st.dl_seq_ssn = (uint8)dl.ssn;
#else  /* !FEATURE_GERAN_REDUCED_DEBUG */
#define UPD_DL_STATE()
#define UPD_DL_ACKNACK()
#define UPD_DL_VR()
#define UPD_DL_VQ()
#define UPD_DL_SSN()
