...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlct4decode.c#2 $   
***
...
)
{
  ...
  MSG_GERAN_ERROR_0_G("T4SeekFreeNode Bin Table Full");
...
void T4ShowORLBinaryTree (void)
{
  ...
  MSG_GERAN_LOW_0_G("T4 one run length binary tree");
  MSG_GERAN_LOW_0_G("     Index   Left  Right  Runlength");
  for (u=0; u<T4_ORL_BIN_TABLE_LEN; u++)
  {
    MSG_GERAN_LOW_3_G("{ /* %3u */  %3u,   %3u, ",
                u,
...
void T4ShowZRLBinaryTree (void)
{
  ...
  MSG_GERAN_LOW_0_G("T4 zero run length binary tree");
  MSG_GERAN_LOW_0_G("     Index   Left  Right  Runlength");
  ...
  */
  for (uRawIndex = 0; uRawIndex<RAW_TABLE_LEN(); uRawIndex++)
  {
    ...
    */
    for (u=0; u<uLen; u++)
    {
      ...
      */
      ...
      {
        MSG_GERAN_HIGH_2_G("T4DecodeORLTest failed at char %u uLen = %u", u, uLen);
        break;
      }
      else
      {
    ...
    */
    ...
    {
      MSG_GERAN_ERROR_3_G("T4VerifyORLTable FAILED raw ind = %3u, RL = %3u decoded RL = %3u",
                uRawIndex, 
    ...
    {
      MSG_GERAN_MED_3_G("T4VerifyORLTable PASS raw ind %3u, RL = %3u decoded RL = %3u",
                uRawIndex, 
                T4RawORLTable[uRawIndex].uRunLen, 
                t4_orl_binary_tree[uInd].run_len
             );
  ...
  */
  for (uRawIndex = 0; uRawIndex<RAW_TABLE_LEN(); uRawIndex++)
  {
    ...
    */
    for (u=0; u<uLen; u++)
    {
      ...
      */
      ...
      if (uInd == INVALID_INDEX)
      {
        MSG_GERAN_HIGH_2_G("T4DecodeZRLTest failed at char %u uLen = %u", u, uLen);
        break;
      }
      else
      {
    ...
    */
    ...
    {
      MSG_GERAN_ERROR_3_G("T4VerifyZRLTable FAILED raw ind = %3u, RL = %3u decoded RL = %3u",
                uRawIndex, 
    ...
    {
      MSG_GERAN_MED_3_G("T4VerifyZRLTable PASS raw ind %3u, RL = %3u decoded RL = %3u",
                uRawIndex, 
                T4RawZRLTable[uRawIndex].uRunLen, 
                t4_zrl_binary_tree[uInd].run_len
             );
...
void T4DecodeORLTest (void)
{
  ...
  for (u=0; u<uLen; u++)
  {
    ...
    */
    ...
    else
    {
      if (t4_orl_binary_tree[uInd].run_len != INVALID_RUN_LEN)
      {
        MSG_GERAN_HIGH_3_G("T4DecodeORLTest ORL = %u at char %u of uLen = %u",
                  t4_orl_binary_tree[uInd].run_len, 
                  u, 
                  uLen);
        break;
  ...
  if (u != (uLen-1))
  {
    MSG_GERAN_HIGH_2_G("T4DecodeORLTest ORL failed u = %u uLen = %u", u, uLen);
  }
} /* T4DecodeORLTest */
...
{
  MSG_GERAN_MED_2_G("T4AFn Pos = %u Ack = %u", uPosRelSSN, uAckValue);
...
void T4DecoderTest (void)
{
  ...
  */
  ...
  (void)grlc_t4_decode ( &(test_crbb[0]), T4ActionFunction, cbscc, uLen, &bits_decoded);
...
(
  ...
  */
  void (*action_func) (uint16 position, uint16 ack_value),
...
)
{
  ...
  {
    MSG_GERAN_ERROR_0_G("ERLU bad CRBB length 0");
    return FALSE;
  }
  ...
  *bits_decoded = 0;
  ...
  do
  {
    ...
    */
    if (t4_current_node != INVALID_INDEX)
    {
      ...
      */
      if (tree_ptr[t4_current_node].run_len != INVALID_RUN_LEN)
      {
        /* Codeword found
        */
        MSG_GERAN_LOW_3_G("ERLU t4_dec run length=%u #bits=%u bit=%u",
        ...
        */
        ...
        {
          (*action_func)( (uint16)(tot_run_len + u), cbscc);
        }
    ...
    {
      MSG_GERAN_ERROR_3_G("ERLU bad node CurInd=%u bit=%u bit_cnt=%u",
                t4_current_node, 
                bit-1, 
                bit_cnt);
    ...
    {
      MSG_GERAN_ERROR_3_G("ERLU t4_dec search failed at node %u after %u bits", 
                t4_current_node, 
                bit_cnt, 
                0);
  ...
  *bits_decoded = tot_run_len;
  ...
  MSG_GERAN_LOW_1_G("#decoded = %u",*bits_decoded);
  return TRUE;
