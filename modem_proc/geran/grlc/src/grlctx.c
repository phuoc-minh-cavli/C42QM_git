...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlctx.c#5 $
***
...
*/
///HV-grlc_ul_tx_array_t  (*p_tx_arr)[];
grlc_ul_tx_array_t  (*(p_tx_arr))[];
...
===
============================================================================*/
static void indicate_n3104_expiry (void)
{
...
===
===========================================================================*/
boolean is_any_nacked_blks (void)
{
  byte b, ind;
  byte tx_win_size;
  ...
  */
  ...
  tx_win_size = (byte)GET_TX_WINDOW_SIZE();
...
)
{
  ...
  ** Clocwork throws a warning if ul.cur_res.cs is used directly
  */
  uint8 cmd_mcs = ul.cur_res.cs;
  ...
  */
  ...
  for (u = 0; u < range; u++)
  {
    ...
    init_mcs = TX_ARR( ind).blk.init_mcs;

    if ((init_mcs <= RM_MCS_9) && (cmd_mcs <= RM_MCS_6_9 ))
    {
      bsn2_mcs = GET_MCS_FOR_RETX( init_mcs, cmd_mcs, ul.cur_res.resegment);
    }
    else
    {
      MSG_GERAN_LOW_2_G("GRLU init_mcs %u or cur_res.cs %u is oor",init_mcs, cmd_mcs);
      return E_GRLC_INVALID_BSN;
    }
    ...
    */
    ...
    else if( TX_ARR( ind).vb == block_type)
      return E_GRLC_INVALID_BSN;
  }
#if 0
  if( block_type == TXA_PACK )
  {
    range = MOD_DIF_ESNS(start, ul.va);
    ...
    */
    for (u = 0; u < range; u++)
    {
      ind = MOD_ESNS(ul.va + u);

      bsn2_mcs = GET_MCS_FOR_RETX( TX_ARR( ind).blk.init_mcs, cmd_mcs, ul.cur_res.resegment);
...
{
  (void)rex_set_timer(&(ul.timer_t3182.rex_timer), RM_TMR_T3182_DUR);
  ul.timer_t3182.state = TMR_ACTIVE;
...
void grlc_ul_demote_mcs_modify_li( uint16 ind )
{
  ...
  pre_demotion_mcs = TX_ARR(  ind ).blk.cs;

  /* TX_ARR( tx_arr_index ).blk.cs will contain demoted MCS after call to macro
  ** below.
  */
  DEMOTE_MCS_FOR_TX( TX_ARR(  ind ).blk.cs );

  /* Modify Length Indicator(s) as approriate. */
  if ( ( TX_ARR( ind).vb     == TXA_PTX  ) &&
       ( TX_ARR( ind).blk.cs == RM_MCS_6 ) &&
       ( pre_demotion_mcs   == RM_MCS_8 )
     )
  {
    /* Precise fit and no ext octets in MCS-8 block. */
    if ( ( TX_ARR( ind).blk.ext == RM_EXT_NO_OCTET_FOLLOWS ) &&
         ( TX_ARR( ind).blk.num_ext_octets == 0            )
       )
    {
      /* 2 new ext octets required. */
      TX_ARR( ind).blk.num_ext_octets = ( TX_ARR( ind).blk.num_ext_octets + 2 );
      TX_ARR( ind).blk.ext = RM_EXT_OCTET_FOLLOWS;
      ext_oct_cnt = TX_ARR( ind).blk.num_ext_octets;

      /* First ext octet. */
      TX_ARR( ind).blk.ext_octet[0].egprs.li  = TX_ARR( ind).blk.data_len;
      TX_ARR( ind).blk.ext_octet[0].egprs.ext = RM_EXT_OCTET_FOLLOWS;

      /* Second ext octet. */
      TX_ARR( ind).blk.ext_octet[1].egprs.li  = RM_FILLER_OCTET_LEN;
      TX_ARR( ind).blk.ext_octet[1].egprs.ext = RM_EXT_NO_OCTET_FOLLOWS;
    }
           ...
           RM_FILLER_OCTET_LEN )
      {
        ...
        */
        if ( total_blk_len == E_GRLC_DATA_BLOCK_LEN[ RM_MCS_8 ] )
        {
          ...
          TX_ARR( ind).blk.num_ext_octets = (TX_ARR( ind).blk.num_ext_octets + 1);
          ext_oct_cnt = TX_ARR( ind).blk.num_ext_octets;

          TX_ARR( ind).blk.ext_octet[ext_oct_cnt - 2].egprs.ext =
            RM_EXT_OCTET_FOLLOWS;

          TX_ARR( ind).blk.ext_octet[ext_oct_cnt - 1].egprs.li =
            RM_FILLER_OCTET_LEN;
          TX_ARR( ind).blk.ext_octet[ext_oct_cnt - 1].egprs.ext =
            RM_EXT_NO_OCTET_FOLLOWS;
        }
        /* Not a precise fit. */
        else
        {
          ...
          TX_ARR( ind).blk.ext_octet[ext_oct_cnt - 3].egprs.ext =
            RM_EXT_OCTET_FOLLOWS;

          TX_ARR( ind).blk.ext_octet[ext_oct_cnt - 2].egprs.li =
...
)
{
  ...
  MSG_GERAN_LOW_3_G("GRLU pdu-txed psn=%u vs=%u seg_ind=%u",
           psn,
           ul.vs,
           ul.seg_ind
          );
  ...
  if (ul.cur_res.tbf_mode == TBF_MODE_EGPRS)
  {
    ...
    */
    if (IS_GE_MODULO_COMP(ul.seg_ind, bsn, ESNS) &&
        IS_GE_MODULO_COMP(bsn, ul.va, ESNS)
       )
    {
      if (IS_GT_MODULO_COMP(TX_ARR( bsn).blk.last_psn, psn, 32768))
      {
        MSG_GERAN_LOW_2_G("GRLU pdu-txed TRUE bsn=%u last_psn=%u",
                bsn,
                TX_ARR( bsn).blk.last_psn
               );

        return TRUE;
      }
      else if (TX_ARR( bsn).blk.last_psn == psn)
      {
        /* check for last block
        */
        if (TX_ARR( bsn).blk.hdr.cv == 0)
        {
          MSG_GERAN_LOW_0_G("GRLU pdu-txed TRUE cv=0");
          return TRUE;
        }
        ...
        */
        if ((IS_GT_MODULO_COMP(ul.seg_ind, ul.vs, ESNS) &&
             IS_GT_MODULO_COMP(TX_ARR( (bsn+1)%ESNS).blk.first_psn, psn, 32768)
            )
           )
        {
          if(IS_E_DATA_BLOCK_PRECISE_FIT( (bsn+1) % ESNS))
          {
            MSG_GERAN_LOW_0_G("GRLU pdu-txed FALSE - will fit in next bsn with LI=0");
            return FALSE;
          }
          else
          {
            MSG_GERAN_LOW_1_G("GRLU pdu-txed TRUE psn=%d fits with L1",psn);
            return TRUE;
          }
        }
      }
  ...
  else if (ul.cur_res.tbf_mode == TBF_MODE_GPRS)
  {
    DEC_MOD(bsn, SNS);
    if (IS_GE_MODULO_COMP(ul.seg_ind, bsn, SNS) &&
        IS_GE_MODULO_COMP(bsn, ul.va, SNS)
       )
    {
      if (IS_GT_MODULO_COMP(TX_ARR( bsn).blk.last_psn, psn, 32768))
      {
      ...
      else if (TX_ARR( bsn).blk.last_psn == psn)
      {
        /* check for last block
        */
        if (TX_ARR( bsn).blk.mac_hdr.type.ul_data.cv == 0 ||
            (IS_GT_MODULO_COMP(ul.seg_ind, ul.vs, SNS) &&
             IS_GT_MODULO_COMP(TX_ARR( (bsn+1)%SNS).blk.first_psn, psn, 32768)
            )
           )
        {
          MSG_GERAN_LOW_0_G("GRLU pdu-txed TRUE cv=0 or good fit");
          return TRUE;
        }
      }
    }
  ...
  {
    MSG_GERAN_LOW_0_G("GRLU pdu-txed bad tbf_mode");
  }

  MSG_GERAN_MED_3_G("GRLU pdu-txed FALSE ",
          psn,
          ul.vs,
          ul.seg_ind
         );
...
)
{
  ...
  if (grlc_ul_fifo_is_area_full( ack_state))
  {
    return FALSE;
  }
  GRLC_UL_SHOW_BLOCK_SENT( ind, ack_state);

  /*====================================================================
  ** Refresh the fields in MAC header before sending block to the FIFO
  **====================================================================
  */
  TX_ARR( ind).blk.mac_hdr.type.ul_data.retry = ul.cur_res.retry;
  ...
  */
  ...
  {
    TX_ARR( ind).blk.mac_hdr.type.ul_data.si = ul.si;
  }
  ...
  **====================================================================
  */
  TX_ARR( ind).blk.tfi = ul.cur_res.ul_tfi;
  ...
  */
  if (ul.rlc_mode == GRLC_MODE_ACK)
  {
    ...
    */
    ...
    {
      TX_ARR( ind).blk.dup_dsm_ptr =
        gprs_pdu_duplicate(TX_ARR( ind).blk.dsm_ptr, 0, TX_ARR( ind).blk.data_len);
      if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
      {
        MSG_GERAN_ERROR_3_G("GRLU A ##Dup Exhaustion %d vb=%d ofs=%u",
                    ind,
                    TX_ARR( ind).vb,
                    TX_ARR( ind).blk.offset);
        u = (uint16)GPRS_PDU_LENGTH(TX_ARR( ind).blk.dsm_ptr);
        MSG_GERAN_HIGH_3_G("dsm_ptr=%x a_len=%u dup=%x",
                  TX_ARR( ind).blk.dsm_ptr,
                  u,
                  TX_ARR( ind).blk.dup_dsm_ptr);
    ...
    */
    if (TX_ARR( ind).blk.mac_hdr.type.ul_data.cv == 0)
    {
      ...
      */
      if (TX_ARR( ind).blk.dup_dsm_ptr != NULL)
      {
        MSG_GERAN_LOW_1_G("GRLU U ##dup not NULL bsn=%d",ind);
      ...
      if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
      {
        ...
        if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
        {
          MSG_GERAN_ERROR_3_G("GRLU U ##Dup Exhaustion %d vb=%d ofs=%u",
                    ind,
                    TX_ARR( ind).vb,
                    TX_ARR( ind).blk.offset);
          u = (uint16)GPRS_PDU_LENGTH(TX_ARR( ind).blk.dsm_ptr);
    ...
    else
    {
      ...
      */
      if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
      {
        TX_ARR( ind).blk.dup_dsm_ptr = TX_ARR( ind).blk.dsm_ptr;
        TX_ARR( ind).blk.dsm_ptr = NULL;
      }
    } /* end block CV not 0 */
  } /* un-ack mode */
  ...
  **======================================================================
  */
  put_res = grlc_ul_fifo_put_block( &(TX_ARR( ind).blk), ack_state);
  if (put_res != UL_FIFO_PUT_OK)
  {
    MSG_GERAN_LOW_2_G("GRLU send_b bad put result %d bsn=%d",put_res, ind);
  ...
  */
  if (ul.b_first_fifo_load)
  {
    ...
    if (ul.rlc_mode == GRLC_MODE_ACK)
    {
      ...
      */
      b_block_timers_service_active = TRUE;
    }
    else
    {
...
)
{
  ...
  while( payload < 2 )
  {
    ...
    grlc_ul_show_block_sent( ind,TX_ARR( ind).vb);
#endif
    /*====================================================================
    ** Refresh the fields in MAC header before sending block to the FIFO
    **====================================================================
    */
    TX_ARR( ind).blk.hdr.retry = ul.cur_res.retry;
    ...
    **====================================================================
    */
    TX_ARR( ind).blk.hdr.tfi = ul.cur_res.ul_tfi;
    ...
    if (ul.rlc_mode == GRLC_MODE_ACK)
    {
      ...
      */
      if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
      {
        ...
        if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
        {
          MSG_GERAN_ERROR_3_G("GRLU A Potential Dup Exhaustion %d vb=%d ofs=%u",
                    ind,
                    TX_ARR( ind).vb,
                    TX_ARR( ind).blk.offset
                   );
          ...
          */
          ...
          {
            MSG_GERAN_HIGH_9_G("BSN=%u vb=%u data_len=%u dsm_ptr=%p dup=%p rem_cnt=%u mcs=%u ptx_miss_cnt=%u nack_miss_cnt=%u",
                      u,
                      TX_ARR( u).vb,
                      TX_ARR( u).blk.data_len,
                      TX_ARR( u).blk.dsm_ptr,
                      TX_ARR( u).blk.dup_dsm_ptr,
                      TX_ARR( u).blk.rem_cnt,
                      TX_ARR( u).blk.cs,
                      TX_ARR( u).blk.ptx_miss_cnt,
                      TX_ARR( u).blk.nack_miss_cnt
                    );

            MSG_GERAN_HIGH_3_G(" 1st_psn=%u last_psn=%u rlc_mode=%u",
                      TX_ARR( u).blk.first_psn,
                      TX_ARR( u).blk.last_psn,
                      ul.rlc_mode
                    );
          }
#endif /* debug */
    ...
    */
        ...
        */
        ...
        if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
        {
          ...
          if (TX_ARR( ind).blk.dup_dsm_ptr == NULL)
          {
            ...
            MSG_GERAN_MED_8_G("GRLU U Potential Dup Exhaustion ind %d vb=%d ofs=%u dsm_ptr=%x a_len=%u dup=%x dup free = %u dp=%x",
              ind,
              TX_ARR( ind).vb,
              TX_ARR( ind).blk.offset,
              TX_ARR( ind).blk.dsm_ptr,
    ...
    if( (TX_ARR( ind).blk.cs == RM_MCS_7  ||
         TX_ARR( ind).blk.cs == RM_MCS_8  ||
         TX_ARR( ind).blk.cs == RM_MCS_9 )&&
         (ind2 != 0xffff)
      )
    {
  ...
  */
  if( payload )
    put_res = e_grlc_ul_fifo_put_block( &(TX_ARR( ind1).blk),
                                       &(TX_ARR( ind2).blk),
                                       ack_state);
  else
    put_res = e_grlc_ul_fifo_put_block( &(TX_ARR( ind1).blk), NULL, ack_state);
...
void attach_block_to_tx_array(  grlc_ul_data_block_t *bp)
{
  ...
  */
  if (TX_ARR( ul.seg_ind).blk.dsm_ptr != NULL)
  {
    MSG_GERAN_HIGH_2_G("GRLU ##atta_blk[%d] dsm not NULL 0x%x",
              ul.seg_ind,
              TX_ARR( ul.seg_ind).blk.dsm_ptr
              );
    GPRS_PDU_FREE(&TX_ARR( ul.seg_ind).blk.dsm_ptr);
  }

  /* Check and warn before we over-write an item
  */
  if (TX_ARR( ul.seg_ind).blk.dup_dsm_ptr != NULL)
  {
    MSG_GERAN_HIGH_2_G("GRLU ##atta_blk[%d] dup not NULL 0x%x",
              ul.seg_ind,
              TX_ARR( ul.seg_ind).blk.dup_dsm_ptr
              );
    GPRS_PDU_FREE(&TX_ARR( ul.seg_ind).blk.dup_dsm_ptr);
  }

  /* copy of the data block in internal format to attach window
  */
  TX_ARR( ul.seg_ind).blk = *bp;

  /* update the cv as block is now FULLY segmented */
  ul.cv = (bp)->mac_hdr.type.ul_data.cv;
  ...
  */
  if (cur_pdu_ptr->rlc_mode == GRLC_MODE_ACK)
    TX_ARR( ul.seg_ind).vb = TXA_PTX;
  else
    TX_ARR( ul.seg_ind).vb = TXA_UNACK;

  TX_ARR( ul.seg_ind).blk.ptx_miss_cnt = 0;

  /* Mark block as part of the current PSN
  */
  TX_ARR( ul.seg_ind).blk.last_psn = cur_pdu_ptr->psn;
  TX_ARR( ul.seg_ind).blk.bsn = ul.seg_ind;

#ifdef  GSM_GPRS_SHOW_FIRST_LAST_PSNS
  MSG_GERAN_HIGH_3_G("GRLU atta bsn=%d 1st_psn=%u last_psn=%u",
    TX_ARR( ul.seg_ind).blk.bsn,
    TX_ARR( ul.seg_ind).blk.first_psn,
    TX_ARR( ul.seg_ind).blk.last_psn);
#endif

  /* advance the index with wrapround
  */
  INC_MOD(ul.seg_ind,SNS);
...
void e_attach_block_post_init(grlc_ul_data_block_t *bp)
{
  ...
  */
  ...
  TX_ARR( ul.seg_ind).blk.ptx_miss_cnt  = 0;
  TX_ARR( ul.seg_ind).blk.nack_miss_cnt = 0;
...
#ifdef  GSM_GPRS_SHOW_FIRST_LAST_PSNS
  MSG_GERAN_HIGH_3_G("GRLU atta bsn=%d 1st_psn=%u last_psn=%u", TX_ARR( ul.seg_ind).blk.bsn,
                                                      TX_ARR( ul.seg_ind).blk.first_psn,
                                                      TX_ARR( ul.seg_ind).blk.last_psn);
#endif
...
===
===========================================================================*/
static void e_tx_blks ( uint8  num_blks )
{
  ...
  */
  ...
  if (num_blks == 0)
  {
    MSG_GERAN_LOW_3_G("ERLU e_tx num_blks = 0 va=%d vs=%d vsf=%d",ul.va,ul.vs,ul.vsf);
    return;
  }
  ...
  */
  ...
  /* As the value of VS (ul.vs) is refreshed by the ISR in post_tx_actions() we can take
  ...
  */
  if (ul.va != ul.inst_vs)
  {
    ...
    **----------------------------------------------------------------------
    */
    if ( IS_GE_MOD(ul.nack_vsf, ul.inst_vs, ul.cur_res.ws) ||
         IS_GT_MOD(ul.va,ul.nack_vsf, ul.cur_res.ws)
       )
    {
      ul.nack_vsf = ul.va;
      UPD_UL_NACK_VSF();
    ...
    */
    ind1 = ul.nack_vsf;
    num_blks_to_search = MOD_DIF_ESNS(ul.inst_vs, ul.nack_vsf);

    while (num_blks_to_search && num_blks)
    {
      if (TX_ARR( ind1).vb == TXA_NACK)
      {
        if( grlc_ul_fifo_get_nack_cnt_dyn() == RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE )
        {
          /* No enough space in the PL1 FIFO to send any NACK blocks.
          ** Exit this function and try again next time around.
          */
          MSG_GERAN_LOW_0_G("GRLU PL1 FIFO full for NACK blks");
          return;
        }
        ...
        */
        if( TX_ARR( ind1).blk.init_mcs == RM_MCS_UNDEF )
        {
          SET_MCS_FOR_TX( TX_ARR( ind1).blk.init_mcs, ul.cur_res.cs );
          MSG_GERAN_LOW_1_G( "ERLU init_msc of NACK block bsn %d is UNDEF!!",ind1);
        }
        ...
        */
        TX_ARR( ind1).blk.cs = GET_MCS_FOR_RETX( TX_ARR( ind1).blk.init_mcs,
                                                ul.cur_res.cs,
                                                ul.cur_res.resegment);

        if( IS_SPLIT_REQUIRED(TX_ARR( ind1).blk.init_mcs,
                              TX_ARR( ind1).blk.cs,
                              ul.cur_res.resegment
                             )
          )
        {
          if (num_blks < 2 || ((RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE - grlc_ul_fifo_get_nack_cnt_dyn()) < 2))
          {
          ...
          */
            MSG_GERAN_MED_1_G("GRLU PL1 FIFO full for split NACK bsn=%u",ind1);
            return;
          }
        ...
        TX_ARR( ind1).blk.mcs_demoted_ps = NO_MCS_DEMOTION;

        if( IS_FORMING_DUAL_PAYLOAD_REQUIRED(TX_ARR( ind1).blk.cs) )
        {
          ...
          ** payload.
          */
          ind2 = e_find_2nd_payload(  (uint16)MOD_ESNS(ind1+1), ul.inst_vs,
                                     TX_ARR( ind1).blk.cs, TXA_NACK );
          ...
          if( ind2 == E_GRLC_INVALID_BSN )
          {
            ...
            */
            if( ul.vsf != ul.seg_ind )
            {
              /* Check if vsf is inside V(S) and seg_ind and work out the offset
              ** relative to V(S)
              */

              /* As the value of VS (ul.vs) is refreshed by the ISR in post_tx_actions()
              ** we can take a snapshot and use it throughout processing of PTX blocks
              ** without reading again
              */
              ul.inst_vs = ul.vs;

              if (IS_GT_MOD(ul.seg_ind, ul.vsf, ul.cur_res.ws))
              {
                if (IS_GT_MOD(ul.inst_vs, ul.vsf, ul.cur_res.ws))
                {
                ...
                */
                if( TX_ARR( ind1).blk.cs == TX_ARR( ul.vsf).blk.cs )
                  ind2 = ul.vsf;
              }
              else
              {
                MSG_GERAN_ERROR_2_G("GRLU ptx vsf(%d) >= seg_ind(%d)",ul.vsf, ul.seg_ind);
              }
            }
            ...
            if( ind2 == E_GRLC_INVALID_BSN )
            {
              ...
              */
              ...
              if( ul.segment_state == SEGMENT_DONE )
              {
                ...
                ** searching from V(A)
                */
                if( !( IS_GT_MOD(ul.inst_vs, ul.pack_vsf, ul.cur_res.ws) &&
                       IS_GE_MOD(ul.pack_vsf, ul.va, ul.cur_res.ws)         )
                   )
                {
                ...
                **   with CV=0
                */
                ind2 = e_find_2nd_payload (  ul.pack_vsf,
                                            ul.inst_vs,
                                            TX_ARR( ind1).blk.cs,
                                            TXA_PACK );

                if( (ind2 == E_GRLC_INVALID_BSN) ||
                    ( !ul.pre_emptive_tx && ul.e_bsn_of_cv_0_block != ind2 )
                  )
                {
                  /* No valid PACK found to pair the PTX block. Downgrade MCS and send
                  ** PTX block in a single payload block
                  */
                  grlc_ul_demote_mcs_modify_li(  ind1 );
                }
              }
            ...
            */
            TX_ARR( ind1).blk.mcs_demoted_ps = TX_ARR( ind1).blk.old_ps+1;
#endif /* if 0 */
          }
        ...
        ** function
        */
        if( TX_ARR( ind1).blk.old_mcs != TX_ARR( ind1).blk.cs )
        {
          /* MCS has changed. Use TS 04.60 table 9.3.2.1.1 to determine the new PS value
          */
          ROTATE_PS_MCS_CHANGE(TX_ARR( ind1).blk.old_mcs, TX_ARR( ind1).blk.cs,
                               TX_ARR( ind1).blk.old_ps, &(TX_ARR( ind1).blk.ps)
                              );
        }
        ...
        if( ind2 != E_GRLC_INVALID_BSN )
        {
          ...
          TX_ARR( ind1).blk.hdr.bsn2 = MOD_DIF_ESNS( ind2 , ind1);
          ...
          */
          if( TX_ARR( ind2).vb == TXA_NACK )
          {
            TX_ARR( ind2).blk.cs = TX_ARR( ind1).blk.cs;

            if( TX_ARR( ind2).blk.old_mcs != TX_ARR( ind2).blk.cs )
            {
              /* MCS has changed. Use TS 04.60 table 9.3.2.1.1 to determine the new PS
              ** value
              */
              ROTATE_PS_MCS_CHANGE( TX_ARR( ind2).blk.old_mcs, TX_ARR( ind2).blk.cs,
                                    TX_ARR( ind2).blk.old_ps, &(TX_ARR( ind2).blk.ps)
                                  );
            }
          }
        }
        ...
        */
          ...
          if( ind2 != E_GRLC_INVALID_BSN )
          {
            ...
            */
            if( TX_ARR( ind2).vb == TXA_NACK )
            {
              ...
              ** and we will be sending nack as pending pack if needed.
              */
              TX_ARR( ind2).blk.old_mcs = TX_ARR( ind2).blk.cs;
              TX_ARR( ind2).blk.old_ps = TX_ARR( ind2).blk.ps;
              ROTATE_PS_NO_MCS_CHANGE( TX_ARR( ind2).blk.cs,
                TX_ARR( ind2).blk.old_ps, TX_ARR( ind2).blk.ps );
              ...
              */
              if (IS_GT_MOD(ul.pack_vsf, ind2, ul.cur_res.ws))
              {
                ul.pack_vsf = ind2;
              }
            }
            else if( TX_ARR( ind2).vb == TXA_PTX )
            {
              ul.vsf = ind2 + 1;
            ...
            else if( TX_ARR( ind2).vb == TXA_PACK )
            {
              ul.pack_vsf = ind2 + 1;
            ...
            TX_ARR( ind2).vb = TXA_PENDING_PACK;
          }
          else
          {
            if( IS_FORMING_DUAL_PAYLOAD_REQUIRED(TX_ARR( ind1).blk.cs) )
            ...
            ** The NACK block is now P_ACK and that is where pack_vsf should point at
            */
            if (IS_GT_MOD(ul.pack_vsf, ind1, ul.cur_res.ws))
            {
              ul.pack_vsf = ind1;
            }
          }

          INC_MOD(ul.nack_vsf, ESNS);
          if ( IS_GE_MOD(ul.nack_vsf, ul.inst_vs, ul.cur_res.ws) ||
               IS_GT_MOD(ul.va,ul.nack_vsf, ul.cur_res.ws)
             )
          {
            /* Reset nack_vsf to V(A) if NOT (ul.va <= nack_vsf < ul.inst_vs) */
            ul.nack_vsf = ul.va;
          }
        ...
        {
          MSG_GERAN_LOW_2_G("GRLU Info tx NAK FIFO full ind1=%d num_blks=%d",ind1, num_blks);
  ...
  MSG_GERAN_LOW_1_G("ERLU e_tx petx = %u",ul.pre_emptive_tx);
  ...
  */
  {
    ...
    */
    if ( (num_blks > 0) && (ul.pre_emptive_tx) )
      b_repeat_sending_p_ack = TRUE;

#ifdef  GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
    MSG_GERAN_MED_3_G("ERLU e_tx ptx/pack w full va=%u vs=%u repeat_sending_p_ack=%u",
            ul.va,
            ul.vs,
            b_repeat_sending_p_ack);
#endif

    if (ul.si == RM_TX_NOT_STALLED)
    {
      ...
      */
      ...
      MSG_GERAN_MED_3_G("ERLU e_tx SI=1 va=%d vs=%d seg=%d",ul.va,ul.vs,ul.seg_ind);
#endif
    }
  } /* end window stall */
  else
  ...
  */
  {
#ifdef  GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
    MSG_GERAN_MED_3_G("ERLU e_tx ptx/pack w ! full va=%u vs=%u p_vsf=%u",
            ul.va,
            ul.vs,
            ul.pack_vsf);
#endif
    ...
    */
       ...
       )
    {
      ...
      */
      ...
      /* As the value of VS (ul.vs) is refreshed by the ISR in post_tx_actions() we
      ...
      */
      while ( num_blks &&
              IS_GT_MOD(ul.seg_ind, ind1, ul.cur_res.ws)
            )
      {
        if (TX_ARR( ind1).vb == TXA_PTX)
        {
          if (grlc_ul_fifo_get_ptx_cnt_dyn() == RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE)
          {
            /* No enough space in the PL1 FIFO to send any PTX blocks.
            ** Exit this function and try again next time around.
            */
            MSG_GERAN_MED_0_G("GRLU PL1 FIFO full for PTX blks");
            return;
          }
          ...
          */
          TX_ARR( ind1).blk.init_mcs = RM_MCS_UNDEF;
          ...
          */
          if( IS_FORMING_DUAL_PAYLOAD_REQUIRED( TX_ARR( ind1).blk.cs ) )
          {
            ...
            ** two PTX blocks.
            */
            if( ( IS_GT_MOD(ul.seg_ind, MOD_ESNS(ind1+1), ul.cur_res.ws)) ||
                ( !E_GET_ATTACH_WINDOW_SPACE() )
              )
            {
              if( TX_ARR( MOD_ESNS(ind1 + 1)).blk.cs == TX_ARR( ind1).blk.cs &&
                  TX_ARR( MOD_ESNS(ind1 + 1)).vb == TX_ARR( ind1).vb
                )
              {
                ind2 = MOD_ESNS(ind1 + 1);

                /* Assign PS 1 to the PTX data block before is sent to PL1 FIFO */
                TX_ARR( ind2).blk.ps = GRLC_PS_1;

                /* The MCS for the initial transmission will be set to the block's
                ** MCS after the block has been confirmed as sent by L1 in
                ** post_tx_actions()
                */
                TX_ARR( ind2).blk.init_mcs = RM_MCS_UNDEF;
              }
              else
              {
            ...
            else
            {
              ...
              if( ul.segment_state == SEGMENT_DONE )
              {
                ...
                **   with CV=0
                */
                ind2 = e_find_2nd_payload (  ul.pack_vsf, ul.inst_vs,
                                            TX_ARR( ind1).blk.cs, TXA_PACK );
          ...
          if (TX_ARR( ind1).blk.hdr.cv == 0)
          {
            MSG_GERAN_HIGH_1_G("GRLU tx CV=0 bsn=%d",ind1);
            ul.e_bsn_of_cv_0_block = ind1;
            ul.re_tx_cv_0_cnt = 0;
          }
          else
          {
          ...
          {
            /* Mark the bsn with CV = 0 */
            if (TX_ARR( ind2).blk.hdr.cv == 0)
            {
              MSG_GERAN_HIGH_1_G("GRLU tx ind2 CV=0 bsn=%d",ind2);
              ul.e_bsn_of_cv_0_block = ind2;
              ul.re_tx_cv_0_cnt = 0;
            }
            else
            {
          ...
          */
          if (e_send_block_to_pl1_fifo( ind1,ind2, TXA_PTX))
          {
...
#ifdef  GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
            MSG_GERAN_HIGH_2_G("GRLU Info PTX FIFO full ind1=%d ind2=%d",ind1,ind2);
#endif
            return;
          }
    ...
    */
    if ( num_blks > 0 &&
         ul.pre_emptive_tx &&
         ((! E_GET_ATTACH_WINDOW_SPACE()) || ul.segment_state == SEGMENT_DONE)
       )
    {
      b_repeat_sending_p_ack = TRUE;
#ifdef  GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
      MSG_GERAN_MED_3_G("ERLU e_tx ena-PACKs p=%u vs=%d in=%d",
              cur_pdu_ptr->psn,
              ul.vs,
              ul.seg_ind);
#endif
      ...
      if (! E_GET_ATTACH_WINDOW_SPACE())
      {
        if (ul.si == RM_TX_NOT_STALLED)
        {
          ...
          */
          ul.si = RM_TX_STALLED;
#ifdef  GSM_GPRS_GRLU_SHOW_SI
          MSG_GERAN_MED_3_G("ERLU tx SI=1 va=%d vs=%d seg=%d",ul.va,ul.vs,ul.seg_ind);
#endif
        }
      }
    }
    ...
    {
      MSG_GERAN_MED_3_G("ERLU e_tx num_blks=%u WINDOW_FULL=%u segment_state=%u",
                num_blks,
                IS_E_TX_WINDOW_FULL(),
                ul.segment_state
             );
  ...
  */
  if( (grlc_ul_fifo.pack_ctr > grlc_ul_fifo.pack_read_ctr) && ul.b_puan)
  {
    grlc_ul_fifo_disable_pl1_access();

    /* Clear PACK FIFO */
    grlc_ul_fifo.pack_in_ind = 0;
    grlc_ul_fifo.pack_out_ind = 0;
    grlc_ul_fifo.pack_ctr = 0;
    grlc_ul_fifo.pack_read_out_ind = 0;
    grlc_ul_fifo.pack_read_ctr = 0;

    MSG_GERAN_LOW_0_G("ERLU Pack FIFO Cleared");
  ...
  */
  ...
  /* As the value of VS (ul.vs) is refreshed by the ISR in post_tx_actions() we can
  ...
  */
  ...
  if (b_repeat_sending_p_ack)
  {
    ...
    */
    if( !( IS_GE_MOD(ul.inst_vs, ul.pack_vsf, ul.cur_res.ws) &&
           IS_GE_MOD(ul.pack_vsf, ul.va, ul.cur_res.ws)         )
       )
    {
      /* pack_vsf outside V(A) and V(S) so reset it to V(A)
    ...
    */
    ...
    if( ul.pack_vsf == ul.inst_vs && ul.inst_vs != ul.vsf )
    {
      return;
    }
    else if (ul.pack_vsf == ul.inst_vs && ul.inst_vs == ul.vsf)
    {
      ul.pack_vsf = ul.va;
    }

    num_blks_to_search = MOD_DIF_ESNS(ul.inst_vs, ul.va);
    ...
    */
          ...
          )
    {
      ...
      **-------------------------------------------------------------------------
      */
      if (TX_ARR( ind1).vb == TXA_PACK ||
          TX_ARR( ind1).vb == TXA_PENDING_PACK
        )
      {
        if( grlc_ul_fifo_get_pack_cnt() == RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE )
        {
          /* No enough space in the PL1 FIFO to send any PACK blocks.
          ** Exit this function and try again next time around.
          */
          MSG_GERAN_LOW_0_G("GRLU PL1 FIFO full for PACK blks");
          return;
        }
        ...
        if (TX_ARR( ind1).vb == TXA_PENDING_PACK)
        {
          if (++TX_ARR( ind1).blk.nack_miss_cnt > MAX_MISS_CNT)
          {
#ifdef  GSM_GPRS_GRLU_SHOW_DATA_XFER
            MSG_GERAN_MED_1_G("ERLU tx NACK %d not acked from PL1",ind1);
#endif
            TX_ARR( ind1).blk.nack_miss_cnt = 0;
          }
        }
        ...
        */
        ...
        {
          SET_MCS_FOR_TX( TX_ARR( ind1).blk.init_mcs, ul.cur_res.cs );
          MSG_GERAN_LOW_1_G( "ERLU init_msc of PACK block bsn %d is UNDEF!!",ind1);
        }
        ...
        */
        ...
        {
          if(num_blks < 2 || ((RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE - grlc_ul_fifo_get_pack_cnt()) < 2))
          {
            /* No enough space in the PL1 FIFO to send the two blocks created after
            ** the split. Exit this function and try again next time around.
            */
            MSG_GERAN_LOW_1_G("GRLU PL1 FIFO full for split PACK bsn=%u",ind1);
            return;
          }
          /* one less block to send as split means 2 blocks*/
          num_blks--;
        ...
        if( IS_FORMING_DUAL_PAYLOAD_REQUIRED(TX_ARR( ind1).blk.cs) )
        {
          ...
          */
          ...
          {
            ind2 = e_find_2nd_payload (  (uint16)MOD_ESNS(ind1+1), ul.inst_vs,
                                        TX_ARR( ind1).blk.cs, TXA_PENDING_PACK );
          }
        ...
        if( ind2 != E_GRLC_INVALID_BSN )
        {
          if (TX_ARR( ind2).vb == TXA_PENDING_PACK)
          {
            if (++TX_ARR( ind2).blk.nack_miss_cnt > MAX_MISS_CNT)
            {
#ifdef  GSM_GPRS_GRLU_SHOW_DATA_XFER
              MSG_GERAN_LOW_1_G("ERLU tx NACK %d not acked from PL1",ind2);
#endif
              TX_ARR( ind2).blk.nack_miss_cnt = 0;
            }
          }
        }
        ...
        if (e_send_block_to_pl1_fifo( ind1, ind2, TXA_PACK))
        {
          /* one less block to send
          */
          num_blks--;
          ...
          */
          ...
          {
            if (IS_GE_MOD(ul.pack_vsf, ul.vsf, ul.cur_res.ws))
            {
              ul.pack_vsf = ul.va;
            }
            UPD_UL_PACK_VSF();
...
#ifdef GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
          MSG_GERAN_LOW_2_G("ERLU e_tx PAK FIFO full ind1=%d ind2=%d",ind1,ind2);
#endif
      ...
      else
      {
        if(TX_ARR( ind1).vb == TXA_PTX)
        {
          ...
          */
          if (++TX_ARR( ind1).blk.ptx_miss_cnt > MAX_MISS_CNT)
          {
#ifdef  GSM_GPRS_GRLU_SHOW_DATA_XFER
            MSG_GERAN_LOW_1_G("ERLU tx PTX %d not acked from PL1",ind1);
#endif
            TX_ARR( ind1).blk.ptx_miss_cnt = 0;

#if 0  /* Forcing TXA_PTX blocks to TXA_PTX
   ** have some undesirable effect on vsf pointers
   */
            TX_ARR( ind1).vb = TXA_PACK;
            if (IS_GE_MOD(ind1,ul.vs,ul.cur_res.ws))
            {
              ul.vs = ind1;
              E_INC_VS();
            }
  ...
  else
  {
    ...
    */
    while( num_blks                                                &&
           ul.e_bsn_of_cv_0_block != E_GRLC_INVALID_BSN            &&
           IS_GE_MOD( ul.e_bsn_of_cv_0_block,ul.va,ul.cur_res.ws)  &&
           IS_GT_MOD( ul.vs, ul.e_bsn_of_cv_0_block,ul.cur_res.ws) &&
           ( TX_ARR( ul.e_bsn_of_cv_0_block).vb == TXA_PACK         ||
             TX_ARR( ul.e_bsn_of_cv_0_block).vb == TXA_PENDING_PACK
           )
         )
    {
      if( grlc_ul_fifo_get_pack_cnt() == RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE )
      ...
      if( TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_mcs !=
            TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs
        )
      {
        /* MCS has changed. Use TS 04.60 table 9.3.2.1.1 to determine the new
        ** PS value
        */
        ROTATE_PS_MCS_CHANGE ( TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_mcs,
                               TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs,
                               TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_ps,
                               &(TX_ARR( ul.e_bsn_of_cv_0_block).blk.ps)
                             );
      }

      if( IS_SPLIT_REQUIRED(TX_ARR( ul.e_bsn_of_cv_0_block).blk.init_mcs,
                            TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs,
                            ul.cur_res.resegment
                           )
        )
      {
        ...
        if( num_blks < 2 || ((RLC_PL1_UL_INDIVIDUAL_FIFO_SIZE - grlc_ul_fifo_get_pack_cnt()) < 2))
        {
          ...
          ** the split. Exit this function and try again next time around.
          */
          MSG_GERAN_MED_1_G("GRLU PL1 FIFO full for split PACK with CV=0 bsn=%u",
            ul.e_bsn_of_cv_0_block);
          return;
        }
        /* one less block to send as split means 2 blocks
        */
      ...
      ** payload MCS
      */
      if( IS_FORMING_DUAL_PAYLOAD_REQUIRED(TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs) )
      {
      ...
      GET_CPS( 
               ul.e_bsn_of_cv_0_block,
               ind2,
               TX_ARR( ul.e_bsn_of_cv_0_block).blk.hdr.cps
             );

      MSG_GERAN_MED_3_G("ELRU e_tx petx=%u sending CV_0 bsn =%u num_blks=%u",
              ul.pre_emptive_tx,
              ul.e_bsn_of_cv_0_block,
              num_blks);
      ...
      */
      (void)e_send_block_to_pl1_fifo( ul.e_bsn_of_cv_0_block, ind2, TXA_PACK);
      ...
      */
      ...
      /* Update old MCS/PS values and rotate PS to be ready for next re-tx */
      TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_mcs = TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs;
      TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_ps = TX_ARR( ul.e_bsn_of_cv_0_block).blk.ps;
      ROTATE_PS_NO_MCS_CHANGE( TX_ARR( ul.e_bsn_of_cv_0_block).blk.cs,
                               TX_ARR( ul.e_bsn_of_cv_0_block).blk.old_ps,
                               TX_ARR( ul.e_bsn_of_cv_0_block).blk.ps );
    }
...
===
===========================================================================*/
static void e_tx_blks_unack_mode (uint8 num_blks)
{
  ...
  */
  if (num_blks == 0)
  {
    ...
    MSG_GERAN_MED_3_G("ERLU U blks=0 PL1space=%u seg=%u vs=%u",
            GET_PL1_FREE_SPACE(),
            ul.seg_ind,
            ul.vs);
#endif
  ...
  */
  ...
  if (IS_GT_MOD(ul.seg_ind, ul.pack_vsf, ul.cur_res.ws))
  {
    if (IS_GT_MOD(ul.inst_vs, ul.pack_vsf, ul.cur_res.ws))
    {
      /* vsf behind vs so take it to vs value
      */
      ul.pack_vsf = ul.inst_vs;
      UPD_UL_VSF();
    }
  }
  else if( IS_GT_MOD(ul.pack_vsf, ul.seg_ind, ul.cur_res.ws) )
  {
    MSG_GERAN_ERROR_2_G("ERLU U pack vsf(%d) >= seg_ind(%d)",ul.pack_vsf,ul.seg_ind);
  }

  ind1 = ul.pack_vsf;
  max_to_send = MIN(num_blks, MOD_DIF_ESNS(ul.seg_ind, ul.pack_vsf));

  b = 0;
  while ( (ind1 != ul.seg_ind) && (b < max_to_send) )
  {
    /* Initialise 2nd payload index to invalid block. */
    ind2 = E_GRLC_INVALID_BSN;
    ...
    */
    if (e_send_block_to_pl1_fifo( ind1, ind2, TXA_UNACK))
    {
      ...
      */
      if (TX_ARR( ind1).blk.hdr.cv == 0)
      {
        MSG_GERAN_HIGH_1_G("ERLU U tx CV=0 bsn=%d",ind1);
        ...
        */
        ...
        {
          if (TX_ARR( ul.e_bsn_of_cv_0_block).blk.dsm_ptr != NULL &&
              ind1 != ul.e_bsn_of_cv_0_block
             )
          {
            MSG_GERAN_HIGH_3_G("ERLU free prev CV_0 bsn (%u)",
                    ul.e_bsn_of_cv_0_block,
                    0,
                    0
                   );
            GPRS_PDU_FREE(&TX_ARR( ul.e_bsn_of_cv_0_block).blk.dsm_ptr);
          }
        }
        MSG_GERAN_HIGH_1_G("ERLU new ind1 e_bsn_of_cv_0_block = %u", ul.e_bsn_of_cv_0_block);
        ul.e_bsn_of_cv_0_block = ind1;
        ul.re_tx_cv_0_cnt = 0;
      }
      else
      ...
      if( (ind2 != E_GRLC_INVALID_BSN) )
      {
        ...
        if (TX_ARR( ind2).blk.hdr.cv == 0)
        {
          MSG_GERAN_HIGH_1_G("GRLU tx CV=0 bsn=%d",ind2);
          ...
          */
          ...
          ul.e_bsn_of_cv_0_block = ind2;
          ul.re_tx_cv_0_cnt = 0;
          MSG_GERAN_HIGH_1_G("ERLU new ind2 e_bsn_of_cv_0_block = %u", ul.e_bsn_of_cv_0_block);
        }
        else
        {
          ul.e_bsn_of_cv_0_block = E_GRLC_INVALID_BSN;
    ...
    {
#ifdef  GSM_GPRS_GRLU_SHOW_E_TX_PTX_PACK_BLKS
      MSG_GERAN_HIGH_2_G("ERLU Info UNACK PACK FIFO full ind1=%d ind2=%d",ind1,ind2);
#endif
        break;
    }
  ...
  */
  if (ul.vs != 0)
  {
    ind1 = (ul.vs + ESNS - 1)% ESNS;
    psn = TX_ARR( ind1).blk.first_psn;
    ...
    */
    ...
    if (IS_GT_MODULO_COMP(psn,ul.first_psn_to_free,32768))
    {
      /* Successful transmission of PDU , set the flag to TRUE
      ** to start ready timer
      */
      grlc_ul_del_free_paq_upto_psn( psn,TRUE);
      ul.first_psn_to_free = (psn+1)%32768;
      MSG_GERAN_MED_3_G("ERLU U post free %u NpOctets=%08x 1st_to_free=%u",
              psn,
...
===
===========================================================================*/
static void tx_blks ( uint8  num_blks )
{
  /* loop count and array index */
  uint8       ind, ofs;
  ...
  */
  ...
  if (num_blks == 0)
  {
    MSG_GERAN_LOW_3_G("GRLU tx num_blks = 0 va=%d vs=%d vsf=%d",ul.va, ul.vs, ul.vsf);
    return;
  }
  ...
  */
  if (ul.va != ul.inst_vs)
  {
    ...
    **----------------------------------------------------------------------
    */
    if (IS_GE_MODULO_COMP(ul.nack_vsf, ul.inst_vs, SNS) ||
        IS_GT_MODULO_COMP(ul.va,ul.nack_vsf, SNS)
       )
    {
      ul.nack_vsf = ul.va;
      UPD_UL_NACK_VSF();
    ...
    else
    {
      ...
      */
      ofs = (uint8)MOD_DIF(ul.nack_vsf, ul.va, SNS);
    }
    ...
    ** is between nack_vsf and V(S)
    */
    ind = MOD_SNS(ul.va+ofs);
    num_blks_to_search = (uint8)MOD_DIF(ul.inst_vs, ul.nack_vsf, SNS);
    ...
    while (num_blks_to_search && num_blks)
    {
      if (TX_ARR( ind).vb == TXA_NACK)
      {
        /* send block to FIFO and update vars if FIFO is not full
        */
        if (send_block_to_pl1_fifo( ind, TXA_NACK))
        {
          TX_ARR( ind).vb = TXA_PENDING_PACK;
          ...
          */
          ul.nack_vsf = ind;
          INC_MOD(ul.nack_vsf, SNS);
          UPD_UL_NACK_VSF();

          if (IS_GT_MODULO_COMP(ul.nack_vsf, ul.pack_vsf, SNS))
          {
            ul.pack_vsf = ul.nack_vsf;
            UPD_UL_PACK_VSF();
          }
        }
        else
      ...
      */
      ...
      if (ul.si == RM_TX_NOT_STALLED)
      {
        ...
        */
        ul.si = RM_TX_STALLED;
#ifdef  GSM_GPRS_GRLU_SHOW_SI
        MSG_GERAN_MED_3_G("GRLU tx SI=1 va=%d vs=%d seg=%d",ul.va,ul.vs,ul.seg_ind);
#endif
      }
    } /* end window stall */
    else
    ...
    */
    {
      ...
      */
         ...
         )
      {
        ...
        */
        ...
        if (IS_GT_MODULO_COMP(ul.seg_ind, ul.vsf, SNS))
        {
          if (IS_GE_MODULO_COMP(ul.vsf, ul.inst_vs, SNS))
          {
            ofs = (uint8)MOD_DIF(ul.vsf, ul.inst_vs, SNS);
          }
          else
          {
            /* vsf behind vs so take it to vs value
        ...
        /* index to first block to search
        */
        ind = MOD_SNS(ul.inst_vs + ofs);

        /* Send loop. Terminate when num_blks runs out or the vsf reaches the
        ** seg_ind where segmentation stops
        */
        while ( num_blks &&
                IS_GT_MODULO_COMP(ul.seg_ind, ind, SNS)
              )
        {
          if (TX_ARR( ind).vb == TXA_PTX)
          {
            /* send block to FIFO with current status as PTX
            */
            if (send_block_to_pl1_fifo( ind, TXA_PTX))
            {
              /* increment VSF. No need to check for wrap-round or overflow
              ** because 'num_blks_to search' puts a limit at seg_ind
              */
      ...
      */
      if ((num_blks > 0) &&
          ...
          */
           ((ul.segment_state == SEGMENT_DONE) &&
            (IS_DYNAMIC_OPEN() || IS_DYNAMIC_CLOSE())
           )
          )
         )
      {
        b_repeat_sending_p_ack = TRUE;
        MSG_GERAN_LOW_3_G("GRLU tx ena-PACKs p=%u vs=%d in=%d",cur_pdu_ptr->psn,ul.vs,ul.seg_ind);

        /* TX Window stall , set the stall indicator
        */
        if (! GET_ATTACH_WINDOW_SPACE())
        {
          if (ul.si == RM_TX_NOT_STALLED)
          {
            /* Don't start T3182 timer here since the block will sit in the FIFO.
    ...
    if (!b_ret)
    {
      ...
      */
      ...
      if (b_repeat_sending_p_ack)
      {
        ...
        ** pending_ack from V(A)
        */
        if (IS_GT_MODULO_COMP(ul.inst_vs, ul.pack_vsf, SNS) &&
            IS_GE_MODULO_COMP(ul.pack_vsf, ul.va, SNS)
           )
        {
          /* Find where that block should be relative to V(A)
          */
          ofs = (uint8)MOD_DIF(ul.pack_vsf, ul.va, SNS);
        }
        else
        {
          /* pack_vsf outside V(A) and V(S) so reset it to V(A)
        ...
        */
        ...
        num_blks_to_search = GET_TX_WINDOW_SIZE();
        ...
        */
              ...
              )
        {
          ...
          **-------------------------------------------------------------------------
          */
          if (TX_ARR( ind).vb == TXA_PACK)
          {
            if (send_block_to_pl1_fifo( ind, TXA_PACK))
            {
              /* update the bsn of the next pending_ack block
              */
              ul.pack_vsf = ind;
              INC_MOD(ul.pack_vsf, SNS);
              ...
              {
                if (IS_GT_MODULO_COMP(ul.pack_vsf, ul.vsf,SNS))
                {
                  ul.pack_vsf = ul.va;
                }
                UPD_UL_PACK_VSF();
          ...
          {
            if (TX_ARR( ind).vb == TXA_PENDING_PACK)
            {
#ifdef  GSM_GPRS_GRLU_SHOW_DATA_XFER
              MSG_GERAN_LOW_1_G("GRLU P_PACK %d becomes PACK (PL1 mis-ack)",ind);
#endif
              TX_ARR( ind).vb = TXA_PACK;
            }
            else if(TX_ARR( ind).vb == TXA_PTX)
            {
              /* when post_tx_actions() misses acknowleging PTX blocks and cause
              ** the BSN not to be continuous with VS then make these PTX blocks
              ** in PACK range PACK blocks. Otherwise they don't get re-transmitted
              ** and get stuck in PTX state. Also update VS in the process
              */
              if (++(TX_ARR( ind).blk.ptx_miss_cnt) > MAX_MISS_CNT)
              {
#ifdef  GSM_GPRS_GRLU_SHOW_DATA_XFER
                MSG_GERAN_MED_1_G("GRLU tx PTX %d becomes PACK (PL1 mis-ack)",ind);
#endif
                TX_ARR( ind).vb = TXA_PACK;
                if (IS_GE_MODULO_COMP(ind,ul.vs,SNS))
                {
                  TX_ARR( ind).blk.ptx_miss_cnt = 0;
          ...
          */
          ...
          if (IS_GE_MODULO_COMP((ul.va+ofs),ul.vsf,SNS))
          {
            ofs = 0;
          }
...
===
===========================================================================*/
static void tx_blks_unack_mode (uint8 num_blks)
{
  /* loop count and common array index
  */
  ...
  */
  ...
  {
#ifdef  GSM_GPRS_GRLU_SHOW_NO_TX_UNACK
    MSG_GERAN_MED_3_G("GRLU U blks=0 PL1space=%d seg=%d vs=%d",
            GET_PL1_FREE_SPACE(),
            ul.seg_ind,
            ul.vs);
#endif
  ...
  */
  ...
  if (IS_GT_MODULO_COMP(ul.seg_ind, ul.pack_vsf, SNS))
  {
    if (IS_GE_MODULO_COMP(ul.pack_vsf, ul.inst_vs, SNS))
    {
      /* pack_vsf is inside V(S) and seg_ind
      */
      ofs = (uint8)MOD_DIF(ul.pack_vsf, ul.inst_vs, SNS);
    }
    else
    {
      /* pack_vsf is behind V(S).
      */
      MSG_GERAN_HIGH_0_G("GRLU tx_u pack_vsf < V(S)");
      ul.pack_vsf = ul.inst_vs;
      UPD_UL_PACK_VSF();
  ...
  max_to_send = (uint8)MIN(num_blks, MOD_DIF(ul.seg_ind, ul.pack_vsf, SNS));
  if (max_to_send > SNS)
    max_to_send -= SNS;

  b = 0;
  while ( (ind != ul.seg_ind) && (b < max_to_send) )
  {
    /* Update CV, MAC header (SI..) and send block to FIFO.
    ** Over-ride ack status with TXA_UNACK to ensure block goes to PACK area
    */
    if (send_block_to_pl1_fifo( ind, TXA_UNACK))
    {
      /* Update the BSN of the block with CV = 0.
      ** During un-ack release this block may need to be re-sent
      */
      if (TX_ARR( ind).blk.mac_hdr.type.ul_data.cv == 0)
      {
        MSG_GERAN_HIGH_1_G("GRLU U tx CV=0 bsn=%d",ind);

        /* Before marking the BSN value of the block with CV 0, try to free the
        ** dsm_ptr of the previous BSN with CV 0. Since in un-ack mode, we free
        ** all PDUs behind the current PDU
        */
        if (ul.bsn_of_cv_0_block != GRLC_INVALID_BSN)
        {
          if (TX_ARR( ul.bsn_of_cv_0_block).blk.dsm_ptr != NULL &&
              ind != ul.bsn_of_cv_0_block
             )
          {
            MSG_GERAN_HIGH_3_G("GRLU free prev CV_0 bsn (%u)",
                    ul.bsn_of_cv_0_block,
                    0,
                    0
                   );
            GPRS_PDU_FREE(&TX_ARR( ul.bsn_of_cv_0_block).blk.dsm_ptr);
          }
        }

        ul.bsn_of_cv_0_block = ind;
        ul.re_tx_cv_0_cnt = 0;
        MSG_GERAN_HIGH_1_G("GRLU new bsn_of_cv_0_block = %u",ul.bsn_of_cv_0_block);
      }
      else
      {
    ...
    */
    ...
    if (IS_GT_MODULO_COMP(psn,ul.first_psn_to_free,32768))
    {
      ...
      */
      grlc_ul_del_free_paq_upto_psn( psn,TRUE);
      ul.first_psn_to_free = (psn+1)%32768;
      MSG_GERAN_MED_3_G("GRLU U post free %u NpOctets=%08x 1st_to_free=%u",
              psn,
              grlc_ul_pack_queue_info(),
              ul.first_psn_to_free);
    }
...
{
  ul.free_space = GET_PL1_FREE_SPACE();
  ul.max_blks = MAX_BLOCKS_TO_SEND;
  if (ul.cur_res.tbf_mode == TBF_MODE_GPRS)
  {
    if(ul.rlc_mode == GRLC_MODE_ACK)
    {
      tx_blks( (uint8)MIN3(ul.free_space,
                          ul.max_blks,
                          GET_ATTACH_WINDOW_SIZE()
                         )
             );
    }
    else
  ...
  else
  {
    if(ul.rlc_mode == GRLC_MODE_ACK)
    {
      e_tx_blks( (uint8)MIN(ul.free_space,ul.max_blks));
    }
    else
    {
      e_tx_blks_unack_mode( 
...
)
{
  ...
  */
  ...
  /* Get MSByte
  */
  tcb = (uint32)ul.timer_t3182.rex_timer.tcb >> 24;
  ...
  */
  ...
  if (ul.cur_res.tbf_mode == TBF_MODE_EGPRS)
  {
    ...
    */
    ...
    while( loop_cnt )
    {
      ...
      */
      ...
      if (TX_ARR( bsn).vb == TXA_NACK ||
          TX_ARR( bsn).vb == TXA_PENDING_PACK
         )
      {
        GRLC_UL_QUEUE_BLK_FOR_SHOW( bsn, TX_ARR( bsn).vb);

        /* Status becomes PACK
        */
        TX_ARR( bsn).vb = TXA_PACK;
        ul.nack_cnt++;
        ...
        if (TX_ARR( bsn).vb == TXA_PTX)
        {
          ...
          */
          if (IS_EXT_UTBF_ACTIVE() &&
              TX_ARR( bsn).blk.hdr.cv == 0 &&
              ul.timer_t3182.state == TMR_ACTIVE &&
              (seek_block_type( ul.va, ul.vs, TXA_NACK)==E_GRLC_INVALID_BSN)
             )
          {
  
            TIMER_START_T3182();
  #ifdef  GSM_GPRS_GRLU_SHOW_T3182
            MSG_GERAN_MED_3_G("ERLU EXT_UTBF T3182 started CV=0 or stalled bsn=%u e_bsn_of_cv_0=%u",
                    bsn,
                    ul.e_bsn_of_cv_0_block,
                    0);
  #endif
          ...
          */
          if (ul.timer_t3182.state != TMR_ACTIVE)
          {
            if (((ul.guard_timer_state == GUARD_OFF) ||
                 (ul.guard_timer_state == GUARD_ON)  ||
                 (ul.guard_timer_state == CV_ZERO)
                ) &&
                (IS_EXT_UTBF_ACTIVE()) &&
                (TX_ARR( bsn).blk.hdr.cv == 0)
               )
            {
              ...
              ul.va_old = ul.va;
              MSG_GERAN_HIGH_2_G("ERLU XTimer started CV=0 bsn=%u e_bsn_of_cv_0=%u",
                      bsn,
                      ul.e_bsn_of_cv_0_block);
            }
          }
          ...
          */
          if (((ul.guard_timer_state == CV_ZERO) ||
               (ul.guard_timer_state == GUARD_ON)
              ) &&
              (IS_EXT_UTBF_ACTIVE()) &&
              (TX_ARR( bsn).blk.hdr.cv != 0)
             )
          {
            ...
            MSG_GERAN_HIGH_2_G("ERLU XTimer stopped CV not 0 bsn=%u e_bsn_of_cv_0=%u",
                    bsn,
                    ul.e_bsn_of_cv_0_block);
          }
          ...
          */
          ...
          {
            MSG_GERAN_LOW_3_G("ERLU post_tx bsn(%u) != vs(%u) vb[vs]=%u",
                      bsn,
                      ul.vs,
                      TX_ARR( ul.vs).vb);
  
            if (IS_GT_MOD(bsn,ul.vs,ul.cur_res.ws))
            {
              ...
              */
              ...
              for (; ind != bsn;)
              {
                MSG_GERAN_MED_3_G("ERLU post_tx ind(%d) becomes PACK bsn(%d) (PL1 mis-ack)",
                        ind,
                        bsn,
                        0);
                TX_ARR( ind).vb = TXA_PACK;
                ...
                */
                E_SET_BLOCK_COUNT_LIMIT( ind);
  
                /* Initiliaze to the values as they were segmented               */
                TX_ARR( ind).blk.old_mcs = TX_ARR( ind).blk.cs;
                TX_ARR( ind).blk.init_mcs = TX_ARR( ind).blk.cs;
                TX_ARR( ind).blk.old_ps = TX_ARR( ind).blk.ps;
                ROTATE_PS_NO_MCS_CHANGE( TX_ARR( ind).blk.cs,
                                         TX_ARR( ind).blk.old_ps,
                                         TX_ARR( ind).blk.ps );
          ...
          **===========================================================================
          */
          if (ul.realloc_state == RALS_AWAIT_TX_COMPLETE)
          {
            if (TX_ARR( bsn).blk.hdr.cv == 0)
            {
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
              MSG_GERAN_MED_1_G("ERLU post bsn=%u RA_AT_TX_COMPLETE aborted cv=0",bsn);
  #endif
              ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
            }
            else if (is_pdu_transmitted( ul.realloc_psn))
            {
              ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
              MSG_GERAN_MED_1_G("ERLU post bsn=%u RA_AT_TX_COMPLETE",bsn);
  #endif
              /* Set internal signal to trigger sending PRR type 2 in task time
              */
              if (!RLC_UL_IS_ITN_SIG_SET())
  #ifndef PERLUTF
              {
                RLC_UL_SET_ITN_SIG();
              }
          ...
          **----------------------------------------------------------------------------
          */
          if ( (IS_TLLI_REQUIRED()) && (loop_cnt == 1) )
          {
            INC_N3104();
  #ifdef  GSM_GPRS_GRLU_SHOW_N3104
            MSG_GERAN_MED_3_G("ERLU A post TLLI IncN3104=%u max=%u vsf=%u",
                    ul.n3104,
                    N3104_MAX(),
                    ul.vsf);
  #endif
          ...
          TX_ARR( bsn).blk.old_mcs = TX_ARR( bsn).blk.cs;
          TX_ARR( bsn).blk.init_mcs = TX_ARR( bsn).blk.cs;
  
          TX_ARR( bsn).blk.old_ps = TX_ARR( bsn).blk.ps;
          ROTATE_PS_NO_MCS_CHANGE(  TX_ARR( bsn).blk.cs,
                                    TX_ARR( bsn).blk.old_ps,
                                    TX_ARR( bsn).blk.ps );
        }
        else
        /*--------------------------------
        ...
        if (TX_ARR( bsn).vb == TXA_UNACK)
        {
          ...
          */
          if (ul.realloc_state == RALS_AWAIT_TX_COMPLETE)
          {
            ...
            {
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
              MSG_GERAN_MED_1_G("ERLU U post bsn=%u RA_AT_TX_COMPLETE aborted cv=0",bsn);
  #endif
              ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
            }
            else if (is_pdu_transmitted( ul.realloc_psn))
            {
              ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
              MSG_GERAN_MED_1_G("ERLU U post bsn=%u RA_AT_TX_COMPLETE",bsn);
  #endif
              /* Set internal signal to trigger sending PRR type 2 in task time
              */
              if (!RLC_UL_IS_ITN_SIG_SET())
      ...
      */
      if (ul.rlc_mode == GRLC_MODE_ACK)
      {
        ...
        */
        if (ul.timer_t3182.state != TMR_ACTIVE)
        {
          if ( IS_TX_COMPLETE()                                           &&
               (seek_block_type( ul.va, ul.vs, TXA_NACK)==E_GRLC_INVALID_BSN)
               &&
               ( (!IS_EXT_UTBF_ACTIVE() ) || (ul.suspend_state == GRLC_SUSPEND) )
             )
          {
            TIMER_START_T3182();
#if defined(GSM_GPRS_GRLU_SHOW_T3182) || defined(GSM_GPRS_GRLU_SHOW_SI)
            MSG_GERAN_MED_3_G("ERLU post T3182 start CV=0 NoNack va=%u vs=%u seg=%u",
                    ul.va,
                    ul.vs,
                    ul.seg_ind);
#endif
        ...
        */
        E_SET_BLOCK_COUNT_LIMIT( bsn);

        /* MSG_GERAN_HIGH_3_G("ERLU post bsn=%u global=%u blk_cnt[]=%u",bsn,
        ** grlc_ul_block_counter,TX_ARR( (bsn)).blk_cnt);
        */
      }
      else
      /* GRLC_MODE_UNACK
      */
      {
        ...
        */
        if ( ( ( TX_ARR( bsn).blk.hdr.cv == 0 )
             ...
             ||
             ( ( GET_E_TX_WINDOW_SIZE() >= ul.cur_res.ws ) &&
               ( ul.timer_t3182.state != TMR_ACTIVE      )
             )
           )
        {
          if( ul.state != US_TEST_MODE )
          {
            TIMER_START_T3182();
#ifdef  GSM_GPRS_GRLU_SHOW_T3182
            MSG_GERAN_MED_3_G("ERLU U T3182 started CV=0 or stalled bsn=%u e_bsn_of_cv_0=%u",
                      bsn,
                      ul.e_bsn_of_cv_0_block,
                      0);
#endif
      ...
      */
      if ( (ul.rlc_mode == GRLC_MODE_ACK) && IS_E_TX_WINDOW_FULL() )
      {
        ...
        if (ul.timer_t3182.state != TMR_ACTIVE)
        {
          TIMER_START_T3182();
#if defined(GSM_GPRS_GRLU_SHOW_T3182) || defined(GSM_GPRS_GRLU_SHOW_SI)
          MSG_GERAN_MED_3_G("ERLU post WinFull start T3182 va=%u vs=%d SI=%u",
                  ul.va,
                  ul.vs,
                  ul.si);
#endif
    ...
    */
    rlc_log_ul_header( (uint8)(GET_HEADER_TYPE(blk_ptr->blk_ptr->elm.egprs.mcs) + 1),
  ...
  { /* GPRS */
    ...
    else
    {
      ...
      */
      if (TX_ARR( bsn).vb == TXA_PTX)
      {
        ...
        */
        if ( (IS_EXT_UTBF_ACTIVE()) &&
             (TX_ARR( bsn).blk.mac_hdr.type.ul_data.cv == 0) &&
             (ul.timer_t3182.state == TMR_ACTIVE) &&
             ( !is_any_nacked_blks() )
           )
        {
          TIMER_START_T3182();
  #ifdef  GSM_GPRS_GRLU_SHOW_T3182
          MSG_GERAN_MED_1_G("GRLU EXT_TBF:T3182 start bsn cv 0 = %d",bsn);
  #endif
        }
        ...
        */
        if (ul.timer_t3182.state != TMR_ACTIVE)
        {
             ...
             )
          {
            ...
            ul.va_old = ul.va;
            TIMER_START_AWAIT_UL_ACKNACK();
            MSG_GERAN_HIGH_2_G("GRLU XTimer started CV=0 bsn=%u bsn_of_cv_0=%u",
                    bsn,
                    ul.bsn_of_cv_0_block);
          }
        }
        ...
        */
           ...
           )
        {
          ul.guard_timer_state = GUARD_OFF;
          TIMER_STOP_AWAIT_UL_ACKNACK();
          MSG_GERAN_HIGH_2_G("GRLU XTimer stopped CV not 0 bsn=%u bsn_of_cv_0=%u",
                  bsn,
                  ul.bsn_of_cv_0_block);
        }
        ...
        */
        ...
        {
          MSG_GERAN_LOW_3_G("GRLU post_tx bsn(%u) != vs(%u) vb[vs]=%u",bsn,ul.vs,TX_ARR( ul.vs).vb);
  
          if (IS_GT_MODULO_COMP(bsn,ul.vs,SNS))
          {
            /* Make blocks from VS to bsn inclusive PACK blocks to account for PL1 not
            ** acknowledging certain PTX blocks
            */
            ...
            for (; ind != bsn;)
            {
              MSG_GERAN_LOW_2_G("GRLU post_tx ind(%d) becomes PACK bsn(%d) (PL1 mis-ack)",ind,bsn);
              TX_ARR( ind).vb = TXA_PACK;
        ...
        */
        if (ul.realloc_state == RALS_AWAIT_TX_COMPLETE)
        {
          ...
          {
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
            MSG_GERAN_MED_1_G("GRLU post bsn=%u RA_AT_TX_COMPLETE aborted cv=0",bsn);
  #endif
            ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
          }
          else if (is_pdu_transmitted( ul.realloc_psn))
          {
            ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
            MSG_GERAN_MED_1_G("GRLU post bsn=%u RA_AT_TX_COMPLETE",bsn);
  #endif
            /* Set internal signal to trigger sending PRR type 2 in task time
            */
            if (!RLC_UL_IS_ITN_SIG_SET())
        ...
        */
        if (IS_TLLI_REQUIRED())
        {
          INC_N3104();
  #ifdef  GSM_GPRS_GRLU_SHOW_N3104
          MSG_GERAN_MED_3_G("GRLU A post TLLI IncN3104=%u max=%d vsf=%u",ul.n3104, N3104_MAX(), ul.vsf);
  #endif
        }
      }
      else
      ...
      if (TX_ARR( bsn).vb == TXA_UNACK)
      {
        ...
        */
        if (ul.realloc_state == RALS_AWAIT_TX_COMPLETE)
        {
          ...
          {
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
            MSG_GERAN_MED_1_G("GRLU U post bsn=%u RA_AT_TX_COMPLETE aborted cv=0",bsn);
  #endif
            ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
          }
          else if (is_pdu_transmitted( ul.realloc_psn))
          {
            ul.realloc_state = RALS_CURRENT_PDU_TX_COMPLETED;
  #ifdef  GSM_GPRS_GRLU_SHOW_REALLOC
            MSG_GERAN_MED_1_G("GRLU U post bsn=%u RA_AT_TX_COMPLETE",bsn);
  #endif
            /* Set internal signal to trigger sending PRR type 2 in task time
            */
            if (!RLC_UL_IS_ITN_SIG_SET())
        ...
        */
        if (IS_TLLI_REQUIRED())
        {
          INC_N3104();
  #ifdef  GSM_GPRS_GRLU_SHOW_N3104
          MSG_GERAN_MED_3_G("GRLU U post IncN3104=%d max=%d vsf=%d",ul.n3104, N3104_MAX(), ul.vsf);
  #endif
        }
      } /* tx_unack */
    }
      ...
      */
      if (ul.timer_t3182.state != TMR_ACTIVE)
      {
           ...
           )
        {
          TIMER_START_T3182();
#if defined(GSM_GPRS_GRLU_SHOW_T3182) || defined(GSM_GPRS_GRLU_SHOW_SI)
          MSG_GERAN_MED_3_G("GRLU post T3182 start CV=0 NoNack va=%d vs=%d seg=%d",
                  ul.va,
                  ul.vs,
                  ul.seg_ind);
#endif
    ...
    */
    {
      ...
      */
      if ( ( ( TX_ARR( bsn).blk.mac_hdr.type.ul_data.cv == 0 )
           ...
           ||
           ( ( GET_TX_WINDOW_SIZE() >= RM_RLC_WINDOW_SIZE     )  &&
             ( ul.timer_t3182.state != TMR_ACTIVE             )
           )
         )
      {
        if( ul.state != US_TEST_MODE )
        {
          TIMER_START_T3182();
#ifdef  GSM_GPRS_GRLU_SHOW_T3182
          MSG_GERAN_MED_3_G("GRLU U T3182 started CV=0 or stalled bsn=%u bsn_of_cv_0=%u",
                      bsn,
                      ul.bsn_of_cv_0_block,
                      0);
#endif
    ...
    /* Check Window Full condition (Stalled) again now that VS might have changed
    */
    if ( (ul.rlc_mode == GRLC_MODE_ACK) && IS_TX_WINDOW_FULL() )
    {
      if (ul.si != RM_TX_STALLED)
      {
        ul.si = RM_TX_STALLED;
      ...
      if (ul.timer_t3182.state != TMR_ACTIVE)
      {
        TIMER_START_T3182();
#if defined(GSM_GPRS_GRLU_SHOW_T3182) || defined(GSM_GPRS_GRLU_SHOW_SI)
        MSG_GERAN_MED_3_G("GRLU post WinFull start T3182 va=%u vs=%u SI=%u", ul.va, ul.vs, ul.si);
#endif
      }
    }
    ...
    */
    ...
    rlc_log_ul_header( (uint8)GPRS_HEADER_TYPE, buf);
...
void ack_mode_xfer (void)
{
  ...
  {
    MSG_GERAN_LOW_1_G("GRLU A_X s=%d return",ul.state);
    return;
  }
  ...
  */
  if (IS_TLLI_REQUIRED())
  {
    if (MOD_DIF(ul.seg_ind, ul.vsf, SNS) < MAX_BLOCKS_TO_SEGMENT)
    {
      /*-----------------------------------------------------------------
    ...
    */
    if (IS_N3104_EXPIRED())
    {
      ...
      **------------------------------------------------------------------------
      */
      MSG_GERAN_MED_2_G("GRLU A Expy N3104=%d N3104_MAX=%d",ul.n3104, N3104_MAX());
  ...
  */
  {
    ...
    */
    if (MOD_DIF(ul.seg_ind, ul.vsf, SNS) < MAX_BLOCKS_TO_SEGMENT)
    {
      ...
      ** short PDU
      */
      if (cur_pdu_ptr->pg_state != PGS_CLOSE)
      {
        MSG_GERAN_LOW_0_G("A_X calls group_pdus");
        group_pdus();
      }
...
void e_ack_mode_xfer (void)
{
  ...
  if (ul.state != US_ACK_XFER)
  {
    MSG_GERAN_LOW_1_G("ERLU A_X s=%d return",ul.state);
    return;
  }
  ...
  */
  if (IS_TLLI_REQUIRED())
  {
    if (MOD_DIF_ESNS(ul.seg_ind, ul.vsf) < MAX_BLOCKS_TO_SEGMENT)
    {
      /*-----------------------------------------------------------------
      ** If segment state is not complete then segment a few more blocks
      **-----------------------------------------------------------------
    ...
    */
    if (MOD_DIF_ESNS(ul.seg_ind, ul.vsf) < MAX_BLOCKS_TO_SEGMENT)
    {
      ...
      */
      if (cur_pdu_ptr->pg_state != PGS_CLOSE)
      {
        MSG_GERAN_LOW_0_G("E_A_X calls group_pdus");
        group_pdus();
      }
...
void unack_mode_xfer (void)
{
  ...
  {
    MSG_GERAN_LOW_1_G("GRLU U_X s=%d return",ul.state);
  }
  ...
  **----------------------------------------------------------------------
  */
  if (MOD_DIF(ul.seg_ind, ul.pack_vsf, SNS) < MAX_BLOCKS_TO_SEGMENT)
  {
    /* Do this before segmentation starts in case the PDU in the group is a
    ** short PDU
    */
    if (cur_pdu_ptr->pg_state != PGS_CLOSE)
    {
      MSG_GERAN_LOW_0_G("U_X calls group_pdus");
      group_pdus();
    }
...
void e_unack_mode_xfer (void)
{
  ...
  if (ul.state != US_UNACK_XFER)
  {
    MSG_GERAN_LOW_1_G("ERLU U_X s=%d return",ul.state);
  }
  ...
  **----------------------------------------------------------------------
  */
  if (MOD_DIF_ESNS(ul.seg_ind, ul.pack_vsf) < MAX_BLOCKS_TO_SEGMENT)
  {
    /* Do this before segmentation starts in case the PDU in the group is a
    ** short PDU
    */
    if (cur_pdu_ptr->pg_state != PGS_CLOSE)
    {
      MSG_GERAN_LOW_0_G("E_U_X calls group_pdus");
      group_pdus();
    }

    seg_res = e_segment_pdu();
