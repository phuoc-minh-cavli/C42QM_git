...
***
*** $Header: //components/rel/geran.mpss/5.2.0/grlc/src/grlct4encode.c#1 $   
***
...
)
{
  ...
  */
  ...
  {
    MSG_GERAN_ERROR_2("ERLD t4enc OOR run=%u run_length=%u", run, run_length);
  ...
  */
  ...
  MSG_GERAN_LOW_2("ERLD t4enc run=%u run_length=%u", run, run_length);
  MSG_GERAN_LOW_2("ERLD       mc=x%03x mclen=%u", res_ptr->mcode, res_ptr->mcode_len);
  MSG_GERAN_LOW_2("ERLD       tc=x%03x tclen=%u", res_ptr->tcode, res_ptr->tcode_len);
