...
/*==============================================================================
  ...
  $Header: //components/rel/avs.adsp/2.8.6.c2/api/listen/inc/adsp_lsm_session_commands.h#1 $
...
typedef struct lsm_session_detection_engine_generic_event
lsm_session_detection_engine_generic_event_t;

/** @h2xmlp_parameter   {"LSM_SESSION_DETECTION_ENGINE_GENERIC_EVENT",
                          LSM_SESSION_DETECTION_ENGINE_GENERIC_EVENT}
    @h2xmlp_description {Detection event raised by the Detection Engine module.}
    @h2xmlp_toolPolicy  {NO_SUPPORT} */
#include "adsp_begin_pack.h"
struct lsm_session_detection_engine_generic_event
{
	uint16_t status;
	/**< @h2xmle_description {Status returned by the algorithm.}
       @h2xmle_rangeList   {success=LSM_DETECTION_STATUS_DETECTED;
                            failed=LSM_DETECTION_STATUS_REJECTED} */

	uint16_t payload_size;
	/**< @h2xmle_description {Size of the additional configuration sent to
                            the Detection Engine module. \n
                            \n
                            If payload_size is not 4-byte aligned, the
                            caller must fill padding bytes to ensure that the
                            entire calibration is 4-byte aligned. \n
                            \n
                            Immediately following this variable is the payload of 
							size payload_size.this is followed by 
							detection_event_info_header_t structures and different key 
							structures based on key_id mode received }
...
/** IDs for keyword indices information and confidence values information.*/
enum detection_engine_event_key_id_info_t
{
	KEY_ID_CONFIDENCE_LEVELS = 0x2000,
	/**<  Key id used to send confidence levels information in detection engine
          event payload.*/

	KEY_ID_KEYWORD_POSITION_STATS,
	/**< Key id used to send keyword start and end index information in detection 
	     engine event payload.*/

	KEY_ID_TIME_STAMP_INFO 
	/**< Key id used to send time stamp information in detection engine
         event payload.*/
}; 
...
/** @h2xmlp_subStruct */
typedef struct detection_event_info_header_t 
...
{
	uint32_t key_id;
	/**< @h2xmle_description {IDs for different keys in detection event structure}*/

	uint32_t payload_size;
	/**< @h2xmle_description {payload size of particular key in detection event structure.}
