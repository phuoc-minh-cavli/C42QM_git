...
$Header: //components/rel/lte.mpss/4.7.2/api/lte_pdcp_msg.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/07/21   ve      CR 2909375 : Conn sus/resume fixes for OOS, MARB and PSM
07/13/20   ss      CR2730546: FR 63445: Rel.13 LTE Cat.M1 Suspend resume
...
08/09/18   ss      CR2294218: CatM Missing eMBMS and UDC featurization
...
typedef enum
{
  LTE_PDCP_DEFAULT_INIT_DEINIT = 0, 
  LTE_PDCP_PSM_INIT_DEINIT = 1,
  LTE_PDCP_SHIM_CXT_INIT_DEINIT = 2,
  LTE_PDCP_INIT_DEINIT_NONE
} lte_pdcp_init_deinit_type_e;
...
typedef struct
{
  msgr_hdr_struct_type   hdr;            /*!< common message router header */
  lte_eps_id_t eps_id;                   /*!< eps bearer for which info is requested*/
  lte_pdcp_qci_enum_type  qci_value ; /* QOS information of this EPS bearer. */
...
typedef struct
{
  ...
  lte_pdcp_init_deinit_type_e   pdcp_init_deinit_type;/*!< Init type message */
...
{
  msgr_hdr_struct_type                hdr;    /*!< message header */
} lte_pdcpul_contex_store_req_msg_s;
  
typedef struct
{
  boolean psm_entered;
  // TODO:
  //lte_rb_rohc_ctx_s       lte_rb_rohc_ctx; /*!< TODO: ROHC */
} lte_pdcpul_psm_db_s;
...
typedef struct
{
  ...
  lte_pdcpul_psm_db_s *               lte_pdcpul_psm_db_restore;
      // TODO: ROHC Context store
} lte_pdcpul_contex_store_restore_cnf_msg_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,PDCPUL,REQ,CONN_REESTAB,0x10,
                   lte_pdcpul_conn_reestab_req_msg_s),
  MSGR_DEFINE_UMID( LTE,PDCPUL,REQ,CXT_STORE,0x11,
                   lte_pdcpul_contex_store_req_msg_s),
  LTE_PDCPUL_MAX_REQ,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,PDCPUL,CNF,RB_STATS,0x0a,
                   lte_pdcpul_rb_stats_cnf_msg_s),
  MSGR_DEFINE_UMID(LTE,PDCPUL,CNF,CXT_STORE_RESTORE,0x0b,
                   lte_pdcpul_contex_store_restore_cnf_msg_s),
  LTE_PDCPUL_MAX_CNF,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,PDCPUL,IND,RAB_REG, 0x02, 
                   lte_pdcpul_rab_reg_ind_msg_s),
  MSGR_DEFINE_UMID(LTE,PDCPUL,IND,AMR_CODEC_INFO,0x03,
                   lte_pdcpul_amr_codec_info_ind_msg_s),
