...
$Header: //components/rel/lte.mpss/4.7.2/api/lte_mac_rrc.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/16/20   ve      CR 2767340: Changes to copy complete ConnResumeReq message
09/12/18   ss      CR 2312915: FR 45229: Release Assistance Indication Support
01/17/17   ss      CR 1112344: CatM CE Level RACH Group A/B Support
06/06/16   el      CATM1 MAC layer implementation
...
*/
#define LTE_MAC_SR_PROHIBIT_INFINITY  0xFFFFFFFF
...
/*! @brief defined size of the MSG3 connection request SDU = 9 bytes
*/
#define LTE_MAC_MAX_CONN_REQ_SDU_EXTN_SIZE   (9)
...
enhanced coverage mode, ce level N is for normal coverage 
*/
typedef enum
{
  LTE_MAC_CE_LEVEL_0        =      0,
  LTE_MAC_CE_LEVEL_1,
  LTE_MAC_CE_LEVEL_2,
  LTE_MAC_CE_LEVEL_3,
  LTE_MAC_CE_LEVEL_N,
  LTE_MAC_CE_LEVEL_MAX
} lte_mac_ce_level_e;
...
typedef struct
{
  ...
  /*! Size of RSRP threshold list size 
    If the value is 0, MAC should delete previously configured values */
  ...
  lte_cphy_conn_meas_rsrp_t rsrp_thresh_prach_list[LTE_MAC_RSRP_THRESH_PRACH_LIST_SIZE];

  /*! Size of MAC RACH/PRACH param list for each CE level
    If the value is 0, MAC should delete previously configured values */
  uint8 rach_prach_list_ce_size;

  /*! List of MAC RACH/PRACH parameters per CE level, the last two levels
  are reserved for MAC internal usage, RRC do not need to fill in the content */
  lte_mac_rach_prach_param_ce_s rach_prach_ce_list[LTE_MAC_CE_LEVEL_MAX];

  /*! Indicates if max_preamble_tx_per_ce is valid. If set to FALSE 
    MAC should delete the previously configured value */
  boolean preamble_trans_max_ce_valid;

  /*!< MAX number of preamble transmissions while operating in enhanced coverage */
  uint16  preamble_trans_max_ce;   
...
typedef struct
{
  ...
  lte_mac_ce_level_e ce_level;/*!< CE level  */
