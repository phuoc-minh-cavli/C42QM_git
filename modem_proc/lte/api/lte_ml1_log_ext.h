...
$Header: //components/rel/lte.mpss/4.7.2/api/lte_ml1_log_ext.h#1 $
...
#define LTE_ML1_LOG_MAX_NCELLS_PER_REQ  8
...
typedef PACK(struct)
{
  ...
  ///< flag to indicate if serv cell is in normal mode or enhance coverage mode
...
typedef struct
{
  ...
  /*! Starting Paging Narrow Band PNB */
  uint8                      start_page_nb;

  uint16         edrx_page_hyperframe_offset;

  uint16         edrx_page_window_start_offset;
  uint16         edrx_page_window_end_offset;
...
typedef struct
{
  ...
  /* Fourth Word and Fisrt half of fifth word */
  struct
  {
  ...
  /* Second half of Fifth Word */
  uint32  tpc_dci_rnti_type:4;
  uint32  tpc_dci_tpc_cmd:3;
  uint32  dci_info_reserved:9;                  //32 bits

  /* Sixth Word */
  /** TX Repetition Number   */
  uint32 ul_grant_rep_num:12;
  /** Narrowband Index   */
  uint32 ul_grant_nb_idx_modeA:4;
  /** harq ack resource offset used to calc n1pucch */ 
  uint32 dl_grant_harq_ack_res_offset:8;
  /** MPDCCH PRB Set, 2,4, or 2+4 PRB sets */ 
  uint32 mpdcch_prb:3;
  /** The passing Hypothesis in 2+4 PRB set */ 
  uint32 prb_idx:3;
  /** padding bits   */
  uint32 reserved4:2;                           // 32 bits
...
typedef struct
{
  ...
  uint32 mcs_index:5;
  /**Total Num Rep*/
  uint32 total_num_rep:12;
  /**Current Rep Num*/
  uint32 current_rep_num:12;
  uint32 padding:2;
...
/*! @brief DLM pdsch stat record log structure  = 4+4+(3*4*2)+4=36 bytes
*/
typedef struct
{
   ...
   /*! ********** PDSCH stat per record **************/
	...
	//first word
  /* OTA Tx time stamp */
  uint32    hsfn_now:12;                      //32bit word1 start
  uint32    reserved6:4;
  uint32    subframe_num:4;                   ///< 0 ~9     
  uint32    frame_num:12;                     ///< radio frame number 0-1023//32bit word1 end

  //second word
  uint32 num_rbs:8;                           ///< number or rbs allocated//32bit word2 start
  uint32 num_layers:8;                        ///< number of layers 
                                              

  /*! Num of transport blocks present or valid */
  uint32    num_trbs_present:8;
  uint32    serving_cell_index:3;
  uint32    reserved0:5;             //32bit word2 end

  /*! FIXED SIZE ALLOACTION based on "LTE_LL1_MAX_PDSCH_TRANMISSIONS_PER_TTI" */
  struct                                                //32bit word 2 - 5.5  start
  {
    //third word
    uint32     harq_id:4;                       ///< (up to 8 for FDD, up to 15 for TDD)//32bit word3 start
    uint32     rv:2;                            ///< (0, 1, 2, 3)
    uint32     ndi:1;                           ///< (0 or 1)
    uint32     crc_result:1;

    uint32     rnti_type:4;
    uint32     tb_index:1;                      ///< (0 or  1 (MIMO))
    uint32     pdsch_result_discard_retx:1;
    uint32    did_recomb:1;
    uint32     reserved1:1;

    uint32    trblk_size:14;                 /// Size in bytes
    uint32    reserved2:2;                                  //32 bit word3 end

	//fourth word
    uint32    mcs:8;                          ///< ( MCS index 0 - 31) //32bit word4 start
    uint32    num_rbs:8;                      ///< number of rbs allocated to this tb
    uint32    ce_mode:1;                      // when ce_mode bit is set repetition_idx,repetition_total, 
    										  //nb_idx parameters wouldn't be displayed in log packet
    uint32    reserved3:3;
    uint32    repetition_idx:12;               //32 bit word4 end

	//fifth word
    uint32    repetition_total:12;            //32bit word5 start
    uint32    reserved4:4;
    uint32    nb_idx:8;  
    uint32    reserved5:8;                   //32bit word5 end

    //Sixth word
    uint32    bundling_enabled:1;            //32bit word6 start
    uint32    delay_value:5;
    uint32    bundled_ack_nak:2;  
    uint32    end_tb_of_bundle:1;  
    uint32    reserved6:23;                 //32bit word6 end
  }tb[2];

  //Seventh word
  uint32      pmch_id:8;     ///< PMCH id: only applicable to pmch decodes //32bit word7 start
  uint32      area_id:8;     ///< Area id used: only applicable to pmch decodes  //32bit word 2 - 6  end
  uint32      reserved7:16;   //32bit word7 end
