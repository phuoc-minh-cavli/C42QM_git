...
$Header: //components/rel/lte.mpss/4.7.2/api/lte_rrc_ext_msg.h#1 $
...
typedef enum
{
  ...
  LTE_RRC_EST_CAUSE_MO_EXCEPTION_DATA, /*!< 7 */
...
typedef enum
{
  ...
  LTE_RRC_SERVICE_REQ_TYPE_PSM, /* <3 */
  LTE_RRC_SERVICE_REQ_TYPE_PLMN, /*<4 *///to get the PLMN information
...
/*! @brief Data structure for power measurement
*/
typedef struct
{
   boolean is_rx_power_meas_valid;
   /* the measurement time to be the delta of the current uptime milliseconds 
    * (at the time when sending the results back to GPS) minus last serving cell 
    * measurement uptime milliseconds */
   uint64  delta_meas_time; 
   int16   rsrp; /* -140 to -44 dBm */
} lte_rrc_rx_power_measurement_s_type;
...
typedef struct
{
  lte_rrc_ue_id_s ue_id[LTE_RRC_MAX_UE_IDENTITY_TYPES]; /* UE identity list */
...
{
  lte_rrc_plmn_s  plmn;
}lte_rrc_shared_plmn_info_s;

typedef struct
{
  uint32                     num_plmn;
  lte_rrc_shared_plmn_info_s shared_plmn[LTE_RRC_MAX_PLMN_LIST_SIZE];
}lte_rrc_shared_plmn_list_s;
...
typedef struct
{
  lte_rrc_plmn_s selected_plmn; /*!< Selected PLMN - req PLMN or its equivalent
                                     ...
                                     service */
  lte_rrc_shared_plmn_list_s  shared_plmn_list; /* PLMN list from SIB1 */
  ...
  boolean edrx_support;  /* Flag to indicate support for eDRX in camped cell*/
  boolean attachWithoutPDNConnectivity;  /* Support for attachWithoutPDN-Connectivity */
...
enum
{
  ...
  */
  ...
  MSGR_DEFINE_UMID(LTE, RRC, REQ, EDRX_CFG,      0x2E, lte_rrc_edrx_cfg_req_s),
  MSGR_DEFINE_UMID(LTE, RRC, REQ, WWAN_SET_PRIORITY, 0x2F, lte_rrc_wwan_set_priority_req_s),
  MSGR_DEFINE_UMID(LTE, RRC, REQ, CELL_SELECT_INFO, 0x30, lte_rrc_cell_select_info_req_s),
  MSGR_DEFINE_UMID(LTE, RRC, REQ, PSM_READY, 0x31, lte_rrc_psm_ready_req_s), 
  MSGR_DEFINE_UMID(LTE, RRC, REQ, PSM_ENTER, 0x32, lte_rrc_psm_enter_req_s),
  #ifdef FEATURE_FMD_SPEED_INFO
  MSGR_DEFINE_UMID(LTE, RRC, REQ, FMD_SPEED_INFO, 0x33, lte_rrc_fmd_speed_info_req_s),  
...
/*! @brief Structure for UE CIOT Optimizations
*/
typedef struct
{
  boolean attach_without_pdn;         /* whether Attach w/o PDN Connection */
  boolean cp_ciot_eps_optimization;   /* whether Supports CP CIOT EPS Optimization */
  boolean up_ciot_eps_optimization;   /* whether Supports UP CIOT EPS Optimization */
  boolean s1u_supported;              /* whether Supports S1-U interface */
}lte_rrc_ue_ciot_opts_s;
...
typedef struct
{
  ...
  sys_lte_reestab_calltype_e actual_calltype; /*!< New: call type to indicate the purpose of the call,  "est_calltype" denoted above doesn't indicate the actual purpose of the call */
  ...
  lte_rrc_ue_ciot_opts_s ue_ciot_opts; /* CIOT Optimization Informations */
								 
  boolean resume_req; /*NAS will set this to TRUE to request connection resume when in EMM IDLE state with suspend indication.*/
  boolean send_on_resume; /*LRRC needs to look at this filed only if it is a resume request. And NAS will set it to FALSE/TRUE depending 
                                            on whether the pending message need not/ need to be sent out OTA after resumption is successful. 
                                            If it is resumption fallback, LRRC will need to send pending message irrespective of this flag. */

  boolean include_dcn_id; /*Should DCN ID be included in Setup Complete*/
  uint16 dcn_id; /* if above field is TRUE then include dcn id in setup complete. */
...
typedef struct
{
  ...
  boolean esm_data_transport; /*!< NAS will set this flag to TRUE if data 
                                   is for ESM (Extended Session Management) */                                
...
/*! @brief enumeration to identify Service request cause type
*/
typedef enum
{
  /*! Service request triggered by NAS */
  LTE_RRC_SERVICE_REQ_CAUSE_BY_NAS,

  /*! Service request triggered by SHIM layer due to MARB LOAD */
  LTE_RRC_SERVICE_REQ_CAUSE_BY_SHIM,
} service_req_cause_e;
...
typedef struct
{
  ...
  /*!< boolean to indicator whether snr level 2 acq db timer expired or not */
...
/*! @brief Enumeration for frequency list type
*/
typedef enum
{
  LTE_RRC_FREQ_LIST_DEFAULT,    /*!< 0 */
  LTE_RRC_FREQ_LIST_CPFL,           /*!< 1 */
  LTE_RRC_FREQ_LIST_MAX            /*!< 2 */
} lte_rrc_freq_list_type_e;
...
typedef struct
{
  ...
  /*!< Home: CPFL type, Roaming: Default type */
  lte_rrc_freq_list_type_e  freq_list_type; 

  boolean rplmn_info_is_valid; /*!< Is registered PLMN info available?  */    

  /* 0 : Use of enhanced coverage is not restricted 1 : Use of enhanced coverage is restricted*/
  boolean ce_restriction; 
} lte_rrc_system_update_req_s;
...
typedef enum
{
  ...
  LTE_RRC_DRX_CN_COEFFICIENT_S1_T512 = 10,   /*!< T = 512 */
  LTE_RRC_DRX_CN_COEFFICIENT_S1_T1024 = 11,  /*!< T = 1024 */
...
/*! @brief Message for eDRX Configuration from NAS to LTE RRC
    Sent by NAS when it obtains the parameters from network
	Sent only when UE is camped on to a valid cell
*/
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header LTE_RRC_EDRX_CFG_REQ*/

  /* Following eDRX parameters are valid only if set to TRUE. eDRX disabled if set to FALSE*/
  boolean edrx_params_valid;
  
  /*eDRX params*/
  /* bits |    7   |    6   |    5   |   4   |   3   |   2   |   1    |    0   | */
  /*        |<- page time window -> |<- edrx cycle length  ->| */
  uint8 edrx_params;

} lte_rrc_edrx_cfg_req_s;

/*! @brief External confirmations that RRC sends
*/

/*! @brief enumeration to identify PLMN search request cause type
*/
typedef enum
{
  /*! PLMN search request triggered by NAS */
  PLMN_SEARCH_REQ_CAUSE_LTE_RRC_PLMN_REQ,

  /*! PLMN search request triggered by SHIM layer due to MARB LOAD */
  LTE_RRC_PLMN_SEARCH_REQ_CAUSE_BY_SHIM,
} plmn_search_req_cause_e;
...
typedef struct
{
  ...
  ** in bplmn search by lower layers*/
  ...
  /* PLMN Search Request triggered cause (NAS/SHIM) */
  plmn_search_req_cause_e search_req_cause;
  ...
  ** 0 Indicates no restrictions on the number of cells. */  
  ...
  /* Location technology needs assistance data and it needs to scan signals and then compute 
   * a position using those signals.   To get the required data, Location technology will send 
   * a request to WWAN. Then WWAN will do the search and give the required information to
   * Location Technology. Location Technology will use this information and compute the position.*/
  /* sys_service_search_e_type : new type will be added by MMCP for WWAN search */
  sys_wwan_adavnce_scan_pram_s_type wwan_adv_search_req_param;
#endif
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, CNF, WWAN_SET_PRIORITY, 0x17, lte_rrc_wwan_set_priority_cnf_s),  
};
...
typedef struct
{
  ...
  */
  ...
  boolean suspended; /*LRRC will set this to TRUE when after connection resumption failure, 
                                    UE is still in suspended state. For successful resumption/ fallback cases, this flag will be of no significance.*/
  boolean resumption_success; /*Flag applicable only when conn est is a success and conn est req is for resumption. 
                                                   Set to TRUE when resumption is a success. False if Fallbacked to normal connection.*/
  boolean deprior_is_active; /*!< Applicable when est_status is not SUCCESS, TRUE if Call failure led to Cell Deprior, FALSE when the cell deprior is not active */
...
{
  msgr_hdr_s msg_hdr; /*!< Message router header */
  uint8 call_type_bitmask;/*!< Bit mask (based on lte_rrc_est_cause_e) indicating for which calls the barring timer needs to be stopped*/
} lte_rrc_ab_update_ind_s;
...
typedef struct
{
  ...
  sys_found_plmn_list_u_type found_plmn_rat_list; /*!< Found PLMN List */

} lte_rrc_plmn_search_cnf_s;

/*! @brief Message to cnf WWAN tech loaded after high prio req
*/
typedef struct
{
  msgr_hdr_s          msg_hdr; /*!< Message router header */

} lte_rrc_wwan_set_priority_cnf_s;
...
/*! @brief Enumeration for Reason of Image switch
*/
typedef enum
{
  LTE_RRC_IMAGE_SWITCH_REASON_NONE = 0,  /*!< No Image switch */
  LTE_RRC_IMAGE_SWITCH_REASON_PAGE,      /*!< full image load trigger due to Page */
  LTE_RRC_IMAGE_SWITCH_REASON_RESEL,     /*!< full image load trigger due to Resel */
  LTE_RRC_IMAGE_SWITCH_REASON_OOS,       /*!< full image load trigger due to OOS */
  LTE_RRC_IMAGE_SWITCH_REASON_MAX
} lte_rrc_image_switch_reason_e;
...
/*Msg sent by LTE RRC for eDRX status Info*/
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header LTE_RRC_EDRX_CFG_STATUS_IND */

  /* Following edrx_configured is set to TRUE if lower layers are configured for eDRX and UE is in Idle state.
FALSE if UE is not configured for eDRX or when it is in connected state*/
  boolean edrx_configured;
  
  /*eDRX cycle length value in ms. Valid only in above parameter is set to TRUE*/
 uint32 edrx_cycle_length;

} lte_rrc_edrx_cfg_status_ind_s;

/*! @brief 
  LTE TX start info provided to Ext GNSS
*/
typedef struct{

   /*! boolean tx_on status */
   boolean                 tx_on;
   
   /*! LTE Tx Band information */
   sys_sband_lte_e_type   tx_band;
   
} lte_rrc_tx_stat_ext_info_s;
...
typedef struct{
  ...
  /*! Message header */
   ...
   /*! Information regarding TX status */
   lte_rrc_tx_stat_ext_info_s   tx_stat_info;
   
} lte_rrc_tx_stat_ext_ind_s;

/*! @brief enumeration to identify Service request cause type
*/
typedef enum
{
  /*! Priority request triggered by NAS */
  LTE_RRC_PRIORITY_REQ_PROC_NAS,

  /*! Priority request triggered by SHIM */
  LTE_RRC_PRIORITY_REQ_PROC_SHIM,
} priority_req_proc_e;

/*! @brief Message for informing WWAN tech is prioritized over GPS for time critical procedure otherwise FALSE
*/
typedef struct
{
  msgr_hdr_s          msg_hdr; /*!< Message router header */

  /* TRUE if WWAN tech is prioritized over GPS for time critical procedure otherwise FALSE */
  boolean             high_priority;   

  /* Priority Request Triggered Proc (NAS/SHIM) */
  priority_req_proc_e req_proc;
} lte_rrc_wwan_set_priority_req_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, IND, EDRX_CFG_STATUS,      0x2A, lte_rrc_edrx_cfg_status_ind_s),
  MSGR_DEFINE_UMID(LTE, RRC, IND, TX_STATUS_EXT, 0x2B, lte_rrc_tx_stat_ext_ind_s),
  MSGR_DEFINE_UMID(LTE, RRC, IND, AB_UPDATE, 0x2C, lte_rrc_eab_update_ind_s),
  MSGR_DEFINE_UMID(LTE, RRC, IND, DS_SERVICE, 0x2D, lte_rrc_ds_service_ind_s),
  MSGR_DEFINE_UMID(LTE, RRC, IND, CELL_DEPRIOR_STATUS, 0x36, lte_rrc_cell_deprior_status_ind_s),
};
...
typedef struct
{
  ...
  boolean oos_search_done; /*!< TRUE if at least one scan completed before 
                                OOS timer expires */
  ...
  /* boolean flag to indicate whether the no service is due to repeated RACH fail */
  boolean oos_due_to_rach_optmz;

  boolean up_ciot_supported; /*This will indicate the cell support for UP CIOT. If LRRC camps in a non-updated cell that does not support UP CIOT,
 								and NAS is in suspended state already, NAS will assume that connection state is IDLE (without suspension) &
 								start legacy connection establishment request (instead of resumption request) while triggering TAU. */

} lte_rrc_service_ind_s;

/*! @brief Enumeration for Active Rat Type
*/
typedef enum
{
  LTE_ACTIVE_RAT_TYPE_M1,
  LTE_ACTIVE_RAT_TYPE_NB1
} lte_active_rat_type_e;
...
typedef struct
{
  ...
  lte_active_rat_type_e act_rat_type; /*!< Active RAT */

  lte_rrc_svc_status_e svc_status; /*!< Service status */
} lte_rrc_ds_service_ind_s;
...
typedef struct
{
  ...
  uint16 extended_wait_time;  /*!< Extended Wait time in seconds for Delay Tolerant Access requests; Valid Range (1 to 1800); 0 indicates value not applicable */
   uint16 extended_wait_time_for_cp;		/*!<RRC layer provides this to NAS when NB-IoT UE only supports the Control Plane CIoT EPS optimization and RRCConnectionRelease-NB message is received with extendedWaitTime-CPdata*/

 boolean suspended;  /*LRRC will set this to TRUE to indicate that connection is suspended. */

 /* LRRC will set it to TRUE for all the RLF causes that we want to track, 
    such that NAS will trigger TAU if required as a UE workaround fix for frequent RACH failures */
...
/*! @brief External responses that RRC receives/sends
 */
enum
{
    ...
    MSGR_DEFINE_UMID(LTE, RRC, RSP, CELL_SELECT_INFO, 0x05, lte_rrc_cell_select_info_rsp_s),
    #ifdef FEATURE_FMD_SPEED_INFO    
    MSGR_DEFINE_UMID(LTE, RRC, RSP, FMD_SPEED_INFO, 0x06, lte_rrc_fmd_speed_info_rsp_s),
    #endif /* FEATURE_FMD_SPEED_INFO */
};
...
/*! @brief Message for LTE RRC Cell Select Info Request
 */
typedef struct
{
  /*!< Message router header */
  msgr_hdr_s msg_hdr; 
}lte_rrc_cell_select_info_req_s;


/*! @brief
  RRC SIB1 Cell Select Info Response S-criteria information
*/
typedef struct {
  /*!< Message router header */
  msgr_hdr_s msg_hdr; 
  ...
  /*!< UE camped or not.
       If is_ue_camped = FALSE 
       then none of the below fields would be valid */
  boolean is_ue_camped;

  /*!< Active RAT Type */
  lte_active_rat_type_e act_rat_type; 

  /*! Hysteresis for cell reselection (range :0-24dB).
      Applicable only for CATM1. 
      Default value = 0xFF (Invalid/network not configured) */
  uint8 q_hyst;

  /*! Minimum required quality level in the cell in dB, Range (-34 .. -3).
      Applicable only for CATM1. 
      Default value = -128(LTE_CPHY_INT8_NEG_INFINITY) when Invalid/not signaled */
  int8 q_qualmin;
  
  /*!< Minimum Rx level to camp on cell (-70..-22) * 2, 
       Default value = 0 (Invalid/network not configured).
       Applicable for both CATM1, NB1 */
  lte_l1_q_rxlevmin_t q_rxlevmin;

  /*!< Minimum Rx level to camp on cell (-70..-22) * 2,
       Applicable only for CE Mode A CATM1. 
       Default value = 0 (Invalid/network not configured) */
  lte_l1_q_rxlevmin_t qrxlevmin_ce;

  /*!< Minimum Rx level to camp on cell (-70..-22) * 2, 
       Applicable only for CE Mode B CATM1. 
       Default value = 0 (Invalid/network not configured) */
  lte_l1_q_rxlevmin_t qrxlevmin_ce1;
}lte_rrc_cell_select_info_rsp_s;


#ifdef FEATURE_FMD_SPEED_INFO
/*! @brief Message for FMD (Flight Mode Decision) Speed Info Request (QMI to RRC)
*/
typedef struct
{
  /*!< Message router header */
  msgr_hdr_s msg_hdr; 
}lte_rrc_fmd_speed_info_req_s;

/*! @brief Message for FMD (Flight Mode Decision) Speed Info Response (RRC to QMI)
*/
typedef struct {
  /*!< Message router header */
  msgr_hdr_s msg_hdr; 
  ...
  uint64 abs_time_sec; 
}lte_rrc_fmd_speed_info_rsp_s;

#endif /* FEATURE_FMD_SPEED_INFO */

/*! @brief Message for PSM Ready Request */
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header */
} lte_rrc_psm_ready_req_s;

/*! @brief Message for PSM Enter Request */
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header */
} lte_rrc_psm_enter_req_s;
...
typedef enum
{
  LTE_RRC_CELL_DEPRIOR_START,
  LTE_RRC_CELL_DEPRIOR_END
} lte_rrc_cell_deprior_sts_e;

/*! @brief Message for Cell Deprior Status Ind
*/
typedef struct
{
  msgr_hdr_s msgr_hdr; /*!< Message router header */

  lte_rrc_cell_deprior_sts_e status; /*!< Status */

} lte_rrc_cell_deprior_status_ind_s;
