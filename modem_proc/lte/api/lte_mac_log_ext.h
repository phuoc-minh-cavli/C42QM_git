...
--------   ---     ----------------------------------------------------------
01/22/21   ve      CR 2853472: RACH logging radio condn fix
11/06/20   ve      CR 2813039: Fix to correct length to log resume req
09/18/16   ss      CR 1067471: CatM New RACH Log Message
...
*/
#define LTE_MAC_LOG_MSG3_RACH_MAC_HDR_MAX_LEN (11)


#define LTE_MAC_LOG_MAX_CE_LEVEL LTE_CPHY_MAX_CE_LEVEL

/*! @brief Size of RSRP thresholds PRACH Info List Size */
#define LTE_MAC_LOG_RSRP_THRESH_PRACH_LIST_SIZE LTE_MAC_RSRP_THRESH_PRACH_LIST_SIZE
...
typedef PACK(struct)
{
  uint8               first_preamble;                     /*!< First Preamble, range (0..63) */
  uint8               last_preamble;                      /*!< Last Preamble, range (0..63) */
  uint8               max_preamble_tx_attempt_per_ce;     /*!< Max number of premable transmission attempts per CE */
  uint16              contention_resol_tmr;               /*!< Contention timer in ms values: 80, 100, 120, 160, 200, 240 */
  uint8               prach_cfg_index;                    /*!< Prach-ConfigurationIndex, Range (0..63) */
  uint8               ra_rsp_win_size;                    /*!< RA RSP Win Size for this CE level in ms */
} lte_mac_log_sub_pkt_cell_prach_param_ce_s;

typedef PACK(struct)
{
  uint8               rsrp_thresh_prach_list_size;        /*!< Size of RSRP threshold list size */
  uint8               rsrp_thresh_prach_list[LTE_MAC_LOG_RSRP_THRESH_PRACH_LIST_SIZE];      /*!< UE to select PRACH resource set. Up to 3 RSRP threshold */
  uint8               prach_param_ce_list_size;           /*!< Size of prach_param_ce_list */
  lte_mac_log_sub_pkt_cell_prach_param_ce_s prach_param_ce_list[LTE_MAC_LOG_MAX_CE_LEVEL];  /*!< PRACH parameter list for enhanced coverage */
  uint16              initial_ce_level;                   /*!< Initial CE level, range (1..LTE_CPHY_MAX_CE_LEVEL) */
  uint16              preamble_trans_max_ce;              /*!< MAX number of preamble transmissions in enhanced coverage */
}lte_mac_log_sub_pkt_cell_prach_cfg_r13_s;
...
typedef PACK(struct)
{
  ...
  uint8               prach_cfg_r13_present;
  lte_mac_log_sub_pkt_cell_prach_cfg_r13_s    prach_cfg_r13; /*!< PRACH Rel-13 parameters */
