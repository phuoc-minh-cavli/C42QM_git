...
$Header: //components/rel/lte.mpss/4.7.2/api/lte_rrc_int_msg.h#1 $
...
#include <lte.h>
#include "../common/inc/lte_assert.h"
#include "../RRC/src/lte_rrc_cfg.h"
...
#include "../RRC/ASN1/R14/on_target/lte_rrc_osys_asn1.h"
#else
...
/*! @brief NULL HSFN, i.e. indicates invalid or unused value
*/
#define LTE_RRC_NULL_HSFN LTE_CPHY_NULL_HYPER_SFN
...
*/
#ifdef FEATURE_LTE_CATM
/* for saving memory, support 5 SIB DB at most in CATM. */
#define LTE_RRC_SIB_DB_MAX_CELLS 5
#else
#define LTE_RRC_SIB_DB_MAX_CELLS 10
#endif
#define LTE_RRC_SIB_MAX_SIS 11
...
#define LTE_RRC_PENDQ_MAX_QUEUE_LEN     5
...
*/
#define LTE_RRC_INVALID_SI_VALUE_TAG 0xFF
...
/*  Duration of UE info validity 1 timer tick value,
    that seems to be 24 hours in ms */
...
#define LTE_RRC_CSP_ACQ_DB_EFS_FILENAME "/nv/reg_files/modem/lte/rrc/csp/acq_db"

/*! EFS file path for PSM acq db */
#define LTE_RRC_CSP_PSM_ACQ_DB_EFS_FILENAME "/nv/item_files/modem/lte/rrc/csp/psm_acq_db"

/*! EFS file permissions */
#define LTE_RRC_CSP_ACQ_DB_EFS_FILE_PERMISSIONS 0777

/*! EFS file path and permission for PSM SIB db */
#define LTE_RRC_SIB_DB_PSM_EFS_FILENAME "/nv/reg_files/modem/lte/rrc/sib/psm_sib_db"
#define LTE_RRC_SIB_DB_PSM_EFS_PERMISSION 0777
...
#define LTE_RRC_PSM_POI_ENCODED_PAGE_SIZE 70

#define LTE_RRC_ASSERT( exp ) \
    /*lint -save -e506 -e774 */ \
    if(!(exp)) \
    { \
       ERR_FATAL( "Assert!", 0, 0, 0 ); \
    } \
    /*lint -restore */ \
...
typedef struct
{
  ...
  lte_rrc_csp_efs_acq_entry_s efs_db;
}lte_rrc_csp_psm_acq_db_efs;
...
/* Guard timer for LTE-RRC to send LTE_RRC_FMD_SPEED_INFO_RSP to QMI (msec) */
#define LTE_RRC_FMD_SPEED_INFO_REQ_GUARD_TMR_VAL 5000 

/* FMD Speed Info Default Validity Time (msec) */
#define LTE_RRC_FMD_SPEED_INFO_DEF_VALIDITY_TIME_VAL 2000 

/* Macro for FMD Speed Info Invalid Value */
#define LTE_RRC_FMD_SPEED_INFO_INVALID_SPEED_VAL 0xFFFF 

/* Macro for FMD Speed Info Invalid Time */
#define LTE_RRC_FMD_SPEED_INFO_INVALID_TIME_VAL 0

/*! @brief The structure for FMD (Flight Mode Decision) Speed EFS Info
*/
typedef struct
{
  /*!< Boolean to indicate if FMD Speed Info Feature Enabled */
  boolean feature_enable;

  /*!< FMD Speed Info Validity Timer (Seconds),
       Range:[0, 5], 
       default = 2-sec, 
       value = 0 means always trigger fmd_speed_info_req to ML1 */
  uint8 validity_time;
}lte_rrc_fmd_speed_efs_info_s;

/*! @brief The structure for FMD (Flight Mode Decision) Speed Info
*/
typedef struct
{
  /* !< FMD (Flight Mode Decision) Speed Info Req Guard timer, 
        to send fmd_speed_rsp back to QMI */
  lte_rrc_tmr_s guard_tmr; 

  /*!< Speed estimated absolute time in seconds since epoch(Jan6, 1980) */
  uint64 abs_time_sec;
  
  /*!< Provides the absolute speed in kmph as estimated by the modem firmware.
       Invalid_value = 0xFFFF (Failure Case) */
  uint16 speed;

  /*!< FMD Speed EFS Info */
  lte_rrc_fmd_speed_efs_info_s efs_info;

  /*!< Bollean to indicate, sending FMD Speed Info Req to ML1 pending */
  boolean fmd_speed_info_req_to_ml1_pending;
} lte_rrc_fmd_speed_info_s;
...
typedef enum
{
  LTE_RRC_OSYS_MEM_NONE,  
  LTE_RRC_OSYS_MEM_MALLOC,
  LTE_RRC_OSYS_MEM_REALLOC,
  LTE_RRC_OSYS_MEM_FREE, 
} lte_rrc_osys_mem_alloc_type_e;
#endif
...
typedef enum
{
  ...
  LTE_RRC_CAP_DISABLE_CE_PUSCH_NB_MAX_TBS_R14 = 3,
  LTE_RRC_CAP_DISABLE_INTRA_FREQ_CE_MODE_A_HO = 4,   
  LTE_RRC_CAP_DISABLE_INTRA_FREQ_CE_MODE_A_MEAS = 5,  
  LTE_RRC_CAP_DISABLE_EXTENDED_POLL_BYTE_R14 = 6,
  LTE_RRC_CAP_DISABLE_R14_RAI = 7,
  LTE_RRC_CAP_DISABLE_POWER_CLASS_PER_BAND = 8,  
  LTE_RRC_CAP_DISABLE_R14_DATA_INAC_TMR = 9,
  LTE_RRC_CAP_DISABLE_CE_PDSCH_PUSCH_ENH_R14 = 10,
  LTE_RRC_CAP_DISABLE_IE_CE_MEAS_R14 = 11,
  LTE_RRC_CAP_DISABLE_CE_PDSCH_TEN_PROCESSES_R14=12,
  LTE_RRC_CAP_DISABLE_CE_RETUNING_SYMBOLS_R14 = 13,
  //LTE_RRC_CAP_DISABLE_LOGICAL_CH_SR_PROHIBIT_TMR_R12 = 14,  
  //LTE_RRC_CAP_DISABLE_CE_HARQ_ACK_BUNDLING_R14 = 15,
  //LTE_RRC_CAP_DISABLE_CE_SCHEDULING_ENHANCEMENT_R14 = 16,
  ...
  LTE_RRC_CAP_TM8_TDD = 71, 
  LTE_RRC_CAP_ENABLE_IE_OTDOA_RSTD_TDD = 72,
  LTE_RRC_CAP_ENABLE_IE_OTDOA_RSTD_FDD = 73,
  LTE_RRC_CAP_ENABLE_LOGICAL_CH_SR_PROHIBIT_TMR_R12 = 74, 
  LTE_RRC_CAP_ENABLE_IE_UNICAST_FREQ_HOP = 75,
  LTE_RRC_CAP_ENABLE_CE_HARQ_ACK_BUNDLING_R14 = 76,
  LTE_RRC_CAP_ENABLE_CE_SCHEDULING_ENHANCEMENT_R14 = 77,  
...
typedef enum
{
  ...
  LTE_RRC_SUSPENDED_DUE_TO_TX_UNAVAILABILITY, /*!< 24 */

  /* Following new causes added for X2L TRM exchange and reserve */
  LTE_RRC_ACTIVATED_DUE_TO_PSHO_FROM_W_TO_LTE, /*!< 25 */
  LTE_RRC_ACTIVATED_DUE_TO_REDIR_FROM_W_TO_LTE, /*!< 26 */
  LTE_RRC_ACTIVATED_DUE_TO_PSHO_FROM_TDS_TO_LTE, /*!< 27 */
  LTE_RRC_ACTIVATED_DUE_TO_REDIR_FROM_TDS_TO_LTE, /*!< 28 */
  LTE_RRC_ACTIVATED_DUE_TO_RESEL_FROM_TDS_TO_LTE, /*!< 29 */
  LTE_RRC_ACTIVATED_DUE_TO_REDIR_FROM_G_TO_LTE, /*!< 30 */
  LTE_RRC_ACTIVATED_DUE_TO_RESEL_FROM_G_TO_LTE, /*!< 31 */

  /*No TRM exchange, only for SSIM*/
  LTE_RRC_ACTIVATED_DUE_TO_RESEL_FROM_1X_TO_LTE, /*!< 32 */
...
typedef enum
{
  ...
  LTE_RRC_GET_SIBS_CAUSE_EDRX_REFRESH, /*!< 19 eDRX Refresh*/
  LTE_RRC_GET_SIBS_CAUSE_EDRX_SIB_UPDATE, /*!< 20 eDRX Sib update*/
  LTE_RRC_GET_SIBS_CAUSE_CONN_EST_FAIL,
...
typedef enum
{
  ...
  #endif
  LTE_RRC_CFG_SIB1,            /*!< 13 used for Sib1 Common config */
  LTE_RRC_CFG_RESEL_CANCEL,    /*!< 14 Resel cancel config */
...
typedef enum
{
  ...
  LTE_RRC_SIB_READ_ERROR_CAUSE_NON_CATM,         /*!< 3 */
...
typedef enum
{
  LTE_RRC_CEP_USE_RRC_SETUP_FOR_SRB1,     /*!< 0 - ConnectionSetup OTA Config */
  LTE_RRC_CEP_USE_RRC_RESUME_FOR_SRB1,    /*!< 1 - ConnectionResume OTA Config */
  LTE_RRC_CEP_USE_SAVED_CONTEXT_FOR_SRB1, /*!< 2 - Configure SRB1 before sending ConnectionResumeRequest OTA */
} lte_rrc_srb1_cfg_source_e ;
...
typedef struct
{
  ...
  boolean t301_r13_valid;

  uint32 t301_r13;  /*!< Timer R13 T311 value */  
...
typedef struct
{
  lte_rrc_srb1_cfg_source_e srb1_config_src; 
...
typedef enum
{
  ...
  LTE_RRC_INITIATE_CELL_SEL_DMM_SUSPEND,    /*!< 2 */
  LTE_RRC_INITIATE_CELL_SEL_DMM_RESUME,     /*!< 3 */
  LTE_RRC_INITIATE_CELL_SEL_MAX             /*!< 4 */
...
typedef enum
{
  ...
  #endif
  LTE_RRC_CONN_MODE_FAILURE_CAUSE_SUSPEND_FAILURE, /*!< 8 - Suspend Failures */
  LTE_RRC_CONN_MODE_FAILURE_CAUSE_OTHER,        /*!< 9 - Other Failures */
  LTE_RRC_CONN_MODE_FAILURE_CAUSE_DMM_SUSPEND,
  LTE_RRC_CONN_MODE_FAILURE_CAUSE_MAX          /*!< 10 Invalid */
...
/*! @brief enumeration to identify the cause of Connection Reestablishment. 
    ...
    * nb1_rrc_cre_cause_e ENUM definition as well, To log correct  *        
...
typedef enum
{
  ...
  LTE_RRC_CRE_CAUSE_RESUME_ON_LTE_FAILURE, /*!< 14 - Resume on lte failure */
  LTE_RRC_CRE_CAUSE_SUSPEND_LTE_FAILURE,   /*!< 15 - Suspend LTE failure */
  LTE_RRC_CRE_CAUSE_DMM_SUSPEND,           /*!< 16 - DMM Suspend for unload */
  LTE_RRC_CRE_CAUSE_RESERVED_1,            /*!< 17 - Reserved NB cause */
  LTE_RRC_CRE_CAUSE_LOCAL_RELEASE,         /*!< 18 - Local Release by MAC */  
} lte_rrc_cre_cause_e;
...
typedef enum
{
  LTE_RRC_CELL_TYPE_NB1,    /*!< 0 - NB1 cell */
  LTE_RRC_CELL_TYPE_CATM1,    /*!< 1 - CATM1 cell */
  LTE_RRC_CELL_TYPE_LTE         /*!< 2 - LTE cell */
} lte_rrc_cell_type_event_e;
...
typedef struct
{
  ...
  boolean conn_rel_during_cre_rach; /*!< Rel cfg while CRE RACH in progress */
  lte_rrc_ml1_status_type_e ml1_status; /*!< ml1 state */
} lte_rrc_llc_rel_cfg_s;

/*! @brief Data structure for resel cancel configuration info
*/
typedef struct
{
/*! current serving cell info */
  ...
  lte_cphy_resel_cancel_info_s resel_cancel_info;
}lte_rrc_llc_resel_cancel_cfg_s;
...
typedef struct
{
  ...
  lte_cphy_sib1_common_cfg_mask_t    sib1_common_cfg_mask;
  ...
  /*! For BL and UEs in CE, indicates the OFDM starting symbol for any MPDCCH, 
    PDSCH scheduled on the same cell except the PDSCH carrying SystemInformationBlockType1-BR, 
    see TS 36.213 [21]. Values 1, 2, and 3 are applicable for dl-Bandwidth greater than 10 
    resource blocks. Values 2, 3, and 4 are applicable otherwise.*/
...
} lte_cphy_sib1_common_cfg_req_param_s;
...
typedef struct
{
  ...
  lte_cphy_sib1_common_cfg_req_param_s sib1_common_cfg_param;

} lte_cphy_sib1_common_cfg_s;
...
typedef struct
{
  ...
  union {
    ...
    lte_cphy_sib1_common_cfg_s sib1_common_cfg;
    lte_rrc_llc_resel_cancel_cfg_s resel_cancel_cfg;
    #ifdef FEATURE_LTE_ELS_ENABLED
...
typedef enum
{
  ...
  /*! Start autonomous meas id removal */
  LTE_RRC_MEAS_CFG_REQI_AUTONOMOUS_MEAS_ID_REMOVAL,
  #endif
...
typedef struct
{
  ...
  /*! (Optional) If PCI is provided, (applicable to GERAN IRAT only) 
...
typedef enum
{
  ...
  RRC_REQI_REVOKE_SUSPEND                          = 0x46,
  RRC_REQI_DMM_RESUME                              = 0x47,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, REQI, REVOKE_SUSPEND, RRC_REQI_REVOKE_SUSPEND, 
                  lte_rrc_revoke_suspend_reqi_s),  

#if defined (FEATURE_DYNAMIC_LOADING_LTE) || defined (FEATURE_EXTERNAL_PAGING)
  MSGR_DEFINE_UMID(LTE, RRC, REQI, DMM_RESUME, RRC_REQI_DMM_RESUME, 
				  lte_rrc_dmm_resume_reqi_s), 
#endif
...
typedef struct
{
  ...
  lte_l1_sfn_t sib1_sfn; //SIb1 SFN 
  lte_rrc_osys_BCCH_DL_SCH_Message *sib1_ptr; /*!< Sib1 decoded ptr for populating parameters in service Indi */
...
#if defined (FEATURE_DYNAMIC_LOADING_LTE) || defined (FEATURE_EXTERNAL_PAGING)
/*! @brief Data structure for marb event notify
*/
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router Header */
  marb_mem_ind_enum_t marb_mem_ind;	/*!< Indicates the memory load status 
										of the active client group */
  marb_client_group_enum_t active_client_group;	/*! Active client group */
}lte_rrc_marb_event_notify_indi_s;
...
typedef struct
{
  ...
  uint64 min_wait;
  uint64 load_wait;
}lte_rrc_marb_unload_indi_s;

typedef struct
{
  msgr_hdr_s msg_hdr; 
  boolean status;
}lte_rrc_dmm_suspend_cnfi_s;

typedef struct
{
  msgr_hdr_s msg_hdr; 

  boolean trigger_reest; /*!< True = to trigger reestablishment, False = to trigger IDLE mode */
}lte_rrc_dmm_resume_reqi_s;
#endif

/*! @brief Message for Internal Deactivate Confirm
*/
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header */
...
typedef enum
{
  ...
  #endif
  RRC_CNFI_REVOKE_SUSPEND        = 0x3f,
  RRC_CNFI_DEACTIVATE            = 0x40,
  RRC_CNFI_DMM_SUSPEND		     = 0x41,
  RRC_CNFI_RRC_CONNECTION_RESUME_COMPLETE = 0x42,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, CNFI, MBMS_INTEREST_INDICATION,
                   ...
                   lte_rrc_ul_msg_cnfi_s),
  MSGR_DEFINE_UMID(LTE, RRC, CNFI, RRC_CONNECTION_RESUME_COMPLETE,
                   RRC_CNFI_RRC_CONNECTION_RESUME_COMPLETE,
                   lte_rrc_ul_msg_cnfi_s),  
  ...
  MSGR_DEFINE_UMID(LTE, RRC, CNFI, REVOKE_SUSPEND, RRC_CNFI_REVOKE_SUSPEND,
                 lte_rrc_revoke_suspend_cnfi_s),  

  MSGR_DEFINE_UMID(LTE, RRC, CNFI, DEACTIVATE, RRC_CNFI_DEACTIVATE,
                 lte_rrc_deactivate_cnfi_s),  

#if defined (FEATURE_DYNAMIC_LOADING_LTE) || defined (FEATURE_EXTERNAL_PAGING)
  MSGR_DEFINE_UMID(LTE, RRC, CNFI, DMM_SUSPEND, RRC_CNFI_DMM_SUSPEND,
	  		     lte_rrc_dmm_suspend_cnfi_s),
...
typedef enum
{
  ...
  LTE_RRC_TRM_PRIORITY_REQ_MAX                     /*!< 11 */    
...
typedef struct
{
  uint32 earfcn;    /*!< EARFCN for E-UTRA */
  uint8 priority; /*!< Cell reselection priority */
} lte_eutra_reselection_priority_s;

typedef struct
{
  uint32 num_eutra_freq;          /*!< Number of EUTRA frequencies */
  lte_eutra_reselection_priority_s eutra_freq_list[LTE_CPHY_MEAS_MAX_NUM_FREQ]; 
}lte_eutra_dedicated_priority_list_s;
...
typedef struct
{
  ...
  lte_eutra_dedicated_priority_list_s freq_list;      /*!< list of frequencies and their priority */
...
/*! @brief Structure to hold OTA Deprioritization info and Deprioritization
...
/*! @brief Structure for Saving Context during PSM
*/
typedef struct
{
  ...
  lte_rrc_meas_t320_status t320_running; //can be removed after proper testing but required for now

  uint64 t320_remaining_time;

  /*! The owner of dedicated priorities */
  lte_rrc_dedicated_pri_owner_e dedicated_pri_owner;

  lte_eutra_dedicated_priority_list_s freq_list; 

}lte_rrc_meas_psm_dedicated_prio_s;

typedef struct
{
  uint64 curr_time_before_psm;
  
  uint64 t325_remaining_time;

  /*! Only LTE Network Deprioritization info are saved*/
  lte_rrc_lte_ota_depri_info_s lte_ota_depri_info;
  
}lte_rrc_meas_psm_deprio_s;

typedef struct
{
  lte_rrc_meas_psm_dedicated_prio_s dedicated_prio_efs;
  lte_rrc_meas_psm_deprio_s         deprio_efs;
}lte_rrc_meas_psm_efs_s;

/* Structure to store ueinfo rpt */
typedef struct
{
  /*! Flag indicates if RLF Report present */
  boolean rlf_rpt_present;

  /*! Valid UEINFO Reports Mask */
  lte_cphy_ueinfo_rpt_mask_e ueinfo_rpt_mask;

  /*! RLF Info Reported PLMN */
  lte_rrc_plmn_s stored_rplmn;

  /*! Con Est Failure Reported PLMN */
  lte_rrc_plmn_s failed_pcell_info_selected_plmn;

  /*! RLF Report Strucure */
  lte_cphy_ueinfo_rlf_rpt_s cphy_rlf_rpt;
}lte_static_rrc_m1_ueinfo_s; 

/* Structure to store mdt rpt in efs*/
#ifdef FEATURE_LTE_MDT

typedef struct
{
  boolean meas_rpt_present;

  lte_rrc_plmn_s stored_rplmn;

  lte_rrc_plmn_s logged_mdt_cfg_trace_ref_selected_plmn;
}lte_rrc_mdt_psm_efs_s;
#endif
...
typedef enum
{
  ...
  RRC_INDI_TX_UNAVAILABLE = 0x48,

  RRC_INDI_TX_AVAILABLE = 0x49,
  
  RRC_INDI_MARB_EVENT_NOTIFY = 0x4A,
  
  RRC_INDI_MARB_ACCEPT = 0x4B,

  RRC_INDI_MARB_UNLOAD = 0x4C,

  RRC_INDI_PENDED_SIB_UPDATED = 0x4D,

  RRC_INDI_SMC_CONFIGURE = 0x4E,
...
typedef struct
{
  ...
  /*!< SI value tag */
  uint8 si_value_tag[LTE_CPHY_MAX_SI_MSGS];
  ...
  /*!< SFN at which Sib1 is received*/
  lte_l1_sfn_t sib1_sfn;
  ...
  /*!< 24 hrs or 3hr validity in 10 min counter i.e 144 or 18*/
  uint8 si_validity;
} lte_rrc_sib_cell_s;
...
typedef struct
{
  ...
  lte_rrc_sib_bitmask_t sibs_in_si_bitmask[LTE_RRC_SIB_MAX_SIS];
  /*!< Array of encoded SIs */
  uint32 encoded_si_size[LTE_RRC_SIB_MAX_SIS];
  /*!< Indicates whether decoded SIs are present or not. */
} lte_rrc_sib_psm_all_encoded_si_s;
...
{  
  ...
  /*!< Encoded & decoded SIs */
  lte_rrc_sib_psm_all_encoded_si_s	 all_encoded_si;

  //Current time before PSM
  uint64 curr_time_before_psm;

  lte_l1_sfn_t psm_sfn;
  
  lte_l1_hyper_sfn_t psm_hyper_sfn;

  /*SIB1 + SIz pointer */
  uint8 encoded_si_ptr[0];  //variable length structure
}lte_rrc_sib_db_psm_efs;
...
typedef struct
{
 ...
 uint8  encoded_page_msg[LTE_RRC_PSM_POI_ENCODED_PAGE_SIZE];
 lte_sfn_s sfn; /*!< SFN on which the msg was received */
 
}lte_rrc_psm_poi_page_pdu_data_s;


typedef union
{
 /*! LTE specific cell info for reselection candidate*/
 lte_cphy_resel_poi_params_s  target_cell_params;
 /* MPDCCH DII Ind*/
 lte_cphy_poi_mpdcch_direct_ind_info_ind_s poi_dii_ind;
 /*Paging DLM size and pdu */
 lte_rrc_psm_poi_page_pdu_data_s page_pdu;
}lte_rrc_psm_poi_data_u;

typedef struct
{
  /* MSGR data */
  lte_ml1_poi_img_switch_rsn_e img_switch_reason;
  lte_rrc_psm_poi_data_u   msgr_data;
} lte_rrc_psm_img_switch_data_s;
...
typedef struct
{
  ...
  lte_rrc_sec_as_security_context_s saved_as_security_context;

  /*!< Current AS Security Context index */
  uint8 saved_current_as_security_context_index; 
  
  /*!< Current NAS Security Context index */
  uint8 saved_current_nas_security_context_index;

  /*!<Key indices are 3*n, 3*n+1 and 3*n+2 */
  uint8 saved_a2_key_index_n; 
  
  lte_rrc_sec_nas_security_context_s saved_nas_security_context;
}lte_rrc_psm_saved_sec_db;

/* context save for PSM */

typedef struct
{
  boolean has_valid_saved_context;
  lte_rrc_saved_cfg_db_type saved_cfg_db;
  lte_rrc_osys_ResumeIdentity_r13 resume_id; /*!<  redume identity from RRC Connection release DLM */
}lte_rrc_psm_saved_cfg_db;
...
typedef struct
{
  ...
  lte_rrc_psm_img_switch_data_s img_switch_data;
  /* Used by POI */
  boolean eab_enabled;
  lte_rrc_ue_id_efs_s    ue_id;
  lte_rrc_meas_psm_efs_s psm_meas_db;
  lte_static_rrc_m1_ueinfo_s psm_ueinfo_db;
  #ifdef FEATURE_LTE_MDT
  lte_rrc_mdt_psm_efs_s psm_mdt_db;
  #endif
  lte_rrc_sib_db_psm_efs psm_sib_db; //this should be the last variable in this structure
  /* Any new variable should be added before psm_sb_db */
}lte_rrc_psm_context_save_s;
...
typedef struct
{
  ...
  lte_rrc_ue_id_efs_s    ue_id;
  lte_rrc_meas_psm_efs_s psm_meas_db;
  lte_rrc_psm_saved_cfg_db psm_cfg_db;
  lte_rrc_psm_saved_sec_db psm_sec_db;
  lte_static_rrc_m1_ueinfo_s psm_ueinfo_db;
  #ifdef FEATURE_LTE_MDT
  lte_rrc_mdt_psm_efs_s psm_mdt_db;
  #endif
  lte_rrc_sib_db_psm_efs psm_sib_db; //this should be the last variable in this structure
  /* Any new variable should be added before psm_sb_db */
}lte_rrc_edrx_psm_context_save_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, INDI, PENDED_SIB_UPDATED, 
                   RRC_INDI_PENDED_SIB_UPDATED,
                   lte_rrc_sib_updated_indi_s),
  ...
  MSGR_DEFINE_UMID(LTE, RRC, INDI, OOS, 
                   RRC_INDI_OOS,
                   lte_rrc_oos_indi_s),
  MSGR_DEFINE_UMID(LTE, RRC, INDI, TX_UNAVAILABLE, 
                   RRC_INDI_TX_UNAVAILABLE,
                   lte_rrc_tx_unavailable_indi_s),
  MSGR_DEFINE_UMID(LTE, RRC, INDI, TX_AVAILABLE, 
                   RRC_INDI_TX_AVAILABLE,
                   lte_rrc_tx_available_indi_s),
  /*Paging*/
 ...
 /*CSP*/
  ...
  MSGR_DEFINE_UMID(LTE, RRC, INDI, CONFIG_SAVED_SEC,
                   RRC_INDI_SMC_CONFIGURE,
                   lte_rrc_smc_config_indi_s),
...
#if defined (FEATURE_DYNAMIC_LOADING_LTE) || defined (FEATURE_EXTERNAL_PAGING)              
  MSGR_DEFINE_UMID(LTE, RRC, INDI, MARB_EVENT_NOTIFY, RRC_INDI_MARB_EVENT_NOTIFY,
                   lte_rrc_marb_event_notify_indi_s),

  MSGR_DEFINE_UMID(LTE, RRC, INDI, MARB_ACCEPT, RRC_INDI_MARB_ACCEPT,
                   lte_rrc_marb_accept_indi_s),

  MSGR_DEFINE_UMID(LTE, RRC, INDI, MARB_UNLOAD, RRC_INDI_MARB_UNLOAD,
                   lte_rrc_marb_unload_indi_s),
#endif
...
typedef enum
{
  ...
  #endif
  RRC_DLM_RRC_CONNECTION_RESUME = 0x13,
  /*RRC Controller*/
  /*SIB*/
  /*CEP*/
  /*Paging*/
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, DLM, RRC_CONNECTION_RESUME, 
                   RRC_DLM_RRC_CONNECTION_RESUME,
                   lte_rrc_mh_dlm_s),
...
typedef enum
{
  ...
  RRC_TMRI_CEP_TX_UNAVAILABLE = 0x42, /* do not change */
  RRC_TMRI_CTLR_TX_UNAVAILABLE = 0x43, /* do not change */
  RRC_TMRI_ARB_WAIT = 0x44, /* do not change */
  RRC_TMRI_MARB_DEADLOCK = 0x45, /* do not change */  
  RRC_TMRI_MARB_HIGH_PRIORIY_HOLD = 0x46, /* do not change */  
  RRC_TMRI_PLMN_SHIM_ENVELOP = 0x47, /* do not change */
  RRC_TMRI_FMD_SPEED_INFO_REQ_GUARD = 0x48, /* do not change */
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, CEP_TX_UNAVAILABLE, 
                     RRC_TMRI_CEP_TX_UNAVAILABLE, lte_rrc_tmri_s),
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, CTLR_TX_UNAVAILABLE, 
                     RRC_TMRI_CTLR_TX_UNAVAILABLE, lte_rrc_tmri_s),
  ...
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, PLMN_SEARCH_PARTIAL, 
...
#if defined (FEATURE_DYNAMIC_LOADING_LTE) || defined (FEATURE_EXTERNAL_PAGING)              
  /* ARB wait timer expiry */
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, ARB_WAIT, 
				   RRC_TMRI_ARB_WAIT, lte_rrc_tmri_s),

  /* MARB deadlock timer expiry to assert */
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, MARB_DEADLOCK, 
				 RRC_TMRI_MARB_DEADLOCK, lte_rrc_tmri_s),

    /*PLMN SHIM Envelop timer */
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, PLMN_SHIM_ENVELOP, 
                   RRC_TMRI_PLMN_SHIM_ENVELOP, lte_rrc_tmri_s),
#endif
                    
  /* MARB high priority hold timer */
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, MARB_HIGH_PRIORIY_HOLD, 
                   RRC_TMRI_MARB_HIGH_PRIORIY_HOLD, lte_rrc_tmri_s),

#ifdef FEATURE_FMD_SPEED_INFO
  /* FMD (Flight Mode Decision) Seed Info Req Guard timer expiry */
  MSGR_DEFINE_UMID(LTE, RRC, TMRI, FMD_SPEED_INFO_REQ_GUARD, 
                   RRC_TMRI_FMD_SPEED_INFO_REQ_GUARD, lte_rrc_tmri_s),
#endif /* FEATURE_FMD_SPEED_INFO */
...
typedef enum
{
  LTE_RRC_PLMN_SEARCH_ACQ_DB_TMR,  /*!< 1 */ 
  LTE_RRC_PLMN_SEARCH_MSEARCH_BSACN_SEP_TMR, /*!< 2 */ 
  LTE_RRC_SIB_THREE_HOUR_TICK_TMR,   /*!< 3*/
  LTE_RRC_UEINFO_RLF_VALID_TMR,      /*!< 4 */
  LTE_RRC_TMR_MAX /*!< 5*/
} lte_rrc_tmr_type_e;

/*! @brief Enum for deprior removal types
*/
typedef enum
{
  LTE_RRC_CELL_DEPRIOR_REMOVAL_TYPE_ALL,
  LTE_RRC_CELL_DEPRIOR_REMOVAL_TYPE_FREQ,
  LTE_RRC_CELL_DEPRIOR_REMOVAL_TYPE_PCI,
  LTE_RRC_CELL_DEPRIOR_REMOVAL_TYPE_MAX
} lte_rrc_cell_deprior_removal_type_e;
...
extern lte_rrc_csp_efs_acq_db_v2_s* _lte_rrc_csp_get_efs_acq_db(void);

/* Returns a pointer to lte_rrc_sib_db_s*/
extern lte_rrc_sib_db_s* _lte_rrc_sib_get_sib_db(void);

extern void _lte_rrc_sib_copy_sib_db_from_shim_layer
(
  uint8 db_index,  /*!< Index of cell in sib_db */
  lte_rrc_sib_cell_s *cell_ptr
);
