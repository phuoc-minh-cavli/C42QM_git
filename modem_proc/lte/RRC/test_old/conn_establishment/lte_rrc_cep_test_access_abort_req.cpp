/*!
  @file lte_rrc_cep_test_access_abort_req.cpp


  @brief
  Unit test code for testing handling of ConnAbortReq 
...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_access_abort_req.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/10/08   dd     Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPAccessConnAbortReqTest1);

void LteRrcCEPAccessConnAbortReqTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPAccessConnAbortReqTest1::Test()
{
...
//  #if 0
  conn_req_msg_ptr = ul_ccch_msg_ptr->message.u.c1->u.rrcConnectionRequest; 
  ...
  TF_ASSERT(stm_get_state(LTE_RRC_CEP_SM) == LTE_RRC_CEP_SM__ACCESS);
   ...
   // On receiving ConnAbortReq , CEP should transition to inactive  state
  ...
  TF_ASSERT(LTE_RRC_CONN_EST_FAILURE_ABORTED == conn_est_cnf_ptr->est_status); 
...
void LteRrcCEPAccessConnAbortReqTest1::Teardown()
{
