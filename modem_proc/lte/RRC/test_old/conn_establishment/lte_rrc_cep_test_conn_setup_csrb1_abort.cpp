...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_conn_setup_csrb1_abort.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupCSRB1AbortTest1);

void LteRrcCEPConnSetupCSRB1AbortTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupCSRB1AbortTest1::Test()
{
...
//  #if 0
  ...
  *(cfg_req_ptr->trans_id_ptr)= lte_rrc_cep_test.cfg_transaction_id;
  ...
  /*
  result = memcmp(cfg_req_ptr->cfg_info[0].cfg_u.ded_cfg.msg_ptr,
                  lte_rrc_cep_test.conn_setup_dl_ccch_ptr, 
   				  sizeof(lte_rrc_osys_DL_CCCH_Message)) ;
  TF_ASSERT(result == 0);
  */
  ...
  lte_rrc_utf_wait_for_done();
   ...
   /*Expect a Config Req (Release) from CEP*/
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = 
    (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  ...
  TF_ASSERT((cfg_req_ptr->msg_hdr.id) ==  
            LTE_RRC_CFG_REQI);
  /*Assign a transaction ID to the CFG req*/
  lte_rrc_cep_test.cfg_transaction_id = 4;
  TF_ASSERT(cfg_req_ptr->trans_id_ptr !=NULL);
  *(cfg_req_ptr->trans_id_ptr)= lte_rrc_cep_test.cfg_transaction_id;
  TF_ASSERT(cfg_req_ptr->cnfi_required == TRUE);
  TF_ASSERT(cfg_req_ptr->num_cfg == 1);
  TF_ASSERT(cfg_req_ptr->cfg_info[0].cfg_type == LTE_RRC_CFG_CONN_REL);
  TF_ASSERT(cfg_req_ptr->cfg_info[0].cfg_u.rel_cfg.ml1_status == LTE_RRC_ML1_STATUS_NORMAL);


  TF_MSG("Checking that the CEP SM is in CSRB1 state");
  TF_ASSERT(stm_get_state(LTE_RRC_CEP_SM) == LTE_RRC_CEP_SM__CONFIGURING_SRB1);

  /*Send CFG_CNFI with success*/
  lte_rrc_init_default_hdr(&cfg_cnf.msg_hdr, LTE_RRC_CFG_CNFI);
  cfg_cnf.status = E_SUCCESS ; 
  cfg_cnf.proc= LTE_RRC_PROC_CEP;
  cfg_cnf.cfg_type = LTE_RRC_CFG_DED;
  cfg_cnf.trans_id = 3;
 ...
 /*Send CFG_CNFI with success*/
  ...
  cfg_cnf.proc= LTE_RRC_PROC_CEP;
  cfg_cnf.trans_id = 4;
  cfg_cnf.cfg_type = LTE_RRC_CFG_CONN_REL;
  lte_rrc_utf_send_msg((byte*)&cfg_cnf, sizeof(lte_rrc_cfg_cnfi_s));
...
void LteRrcCEPConnSetupCSRB1AbortTest1::Teardown()
{
