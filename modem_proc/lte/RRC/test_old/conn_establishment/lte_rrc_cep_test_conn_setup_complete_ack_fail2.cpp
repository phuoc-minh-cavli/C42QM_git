...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_conn_setup_complete_ack_fail2.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupCompleteAckFailTest2);

void LteRrcCEPConnSetupCompleteAckFailTest2::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupCompleteAckFailTest2::Test()
{
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  ul_msg_cnfi.status = LTE_RRC_UL_MSG_MAYBE;
  ul_msg_cnfi.fail_cause =LTE_RRC_UL_MSG_FAIL_CAUSE_RLF; 
  lte_rrc_utf_send_msg((byte*)&ul_msg_cnfi, sizeof(ul_msg_cnfi));

  lte_rrc_utf_wait_for_done();

  /*We will not be sending NACK to NAS since Conn rel will be followed */


  /*No need to check TA/PLMN stuff when there is a failure*/
  /*
...
void LteRrcCEPConnSetupCompleteAckFailTest2::Teardown()
{
