/*!
  @file lte_rrc_cep_test_conn_setup_aclist_failure.cpp
...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_conn_setup_aclist_failure_mo_data.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/14/12   kp     Added AC barring TC for internal AC barring algo.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPAccessListFailureMODataTest1);

void LteRrcCEPAccessListFailureMODataTest1::Setup()
{
  int ext_count = 0;   
...
void LteRrcCEPAccessListFailureMODataTest1::Test()
{
  ...
  encoded_pdu.value =  cep_basic_sib2_1;
  lte_rrc_BCCH_DL_SCH_Message *bcch_dl_sch_ptr = NULL;
  */
  ...
  //lte_rrc_AccessClassBarringInformation *access_class_barring_info = NULL;
  lte_rrc_osys_AC_BarringConfig *access_class_barring_info = NULL;
  //struct lte_rrc_AccessClassBarringList_ *ac_list_ptr = NULL;
  ...
  /*
  bcch_dl_sch_ptr =
  (lte_rrc_BCCH_DL_SCH_Message *) 
   lte_rrc_asn1_decode_pdu(encoded_pdu.value, lte_rrc_osys_BCCH_DL_SCH_Message_PDU, 
                           encoded_pdu.length);
  */
  ...
  lte_rrc_test_cep_set_controller_access_class(1 << LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS);
  ...
  sib2->ac_BarringInfo.m.ac_BarringForMO_DataPresent = 1;
  access_class_barring_info = &(sib2->ac_BarringInfo.ac_BarringForMO_Data);
  access_class_barring_info->ac_BarringFactor = 
    lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p00;
  access_class_barring_info->ac_BarringTime = 
    lte_rrc_osys_AC_BarringConfig_ac_BarringTime_s4;
  access_class_barring_info->ac_BarringForSpecialAC.numbits = 5;
  value_ptr = access_class_barring_info->ac_BarringForSpecialAC.data;
...
/*
  access_class_barring_info->accessClassBarringList = 
    (struct lte_rrc_AccessClassBarringList_ *)lte_rrc_mem_calloc(sizeof(struct lte_rrc_AccessClassBarringList_ ));
  ac_list_ptr = access_class_barring_info->accessClassBarringList;
  ac_list_ptr->next = NULL;
  
  for(i=11;i<15;i++)
  {
    TF_ASSERT(ac_list_ptr != NULL);
    if(i== LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS)
    {
      ac_list_ptr->value.accessClassBarring = TRUE;
    ...
    ac_list_ptr->next = 
      (struct lte_rrc_AccessClassBarringList_ *)lte_rrc_mem_calloc(sizeof(struct lte_rrc_AccessClassBarringList_ ));
    ac_list_ptr = ac_list_ptr->next;
    ac_list_ptr->next = NULL;
  }
  if(15 == LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS)
  {
    ac_list_ptr->value.accessClassBarring = TRUE;
...
*/
  ...
  conn_est_req.ac_barring_info_for_special_valid = FALSE;
 ...
 TF_ASSERT((1<< (uint8)LTE_RRC_EST_CAUSE_MO_DATA) == 
 ...
 /*Expect T303 tmri*/
 TF_MSG("Checking for T303 tmr expiry");
 lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
...
//#if 0
  ...
  ul_ccch_msg_ptr = 
    (lte_rrc_UL_CCCH_Message *) lte_rrc_asn1_decode_pdu(access_req_ptr->access_info.msg_ptr,
...
//  #if 0
  ...
  *(ulm_ptr->encode_status) = 0; 
  ...
  conn_setup_complete_ptr = (lte_rrc_osys_RRCConnectionSetupComplete *)
  (ul_dcch_msg_ptr->message).u.c1->u.rrcConnectionSetupComplete;
   ...
   r8_ie_ptr =
    (lte_rrc_osys_RRCConnectionSetupComplete_r8_IEs *)
  (&conn_setup_complete_ptr->criticalExtensions.u.c1->u.
...
//#endif
  /*
  TF_ASSERT(0 == lte_rrc_asn1_free_pdu(lte_rrc_osys_BCCH_DL_SCH_Message_PDU,
                                       bcch_dl_sch_ptr));
  */
   /*Free the AC list*/
  ...
  /*
  while(NULL != ac_list_ptr)
  { 
    ac_list_ptr_next = ac_list_ptr->next;
    lte_rrc_mem_free(ac_list_ptr);
    ac_list_ptr = ac_list_ptr_next;
  } 
  */
  lte_rrc_utf_reset();
}

void LteRrcCEPAccessListFailureMODataTest1::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring);

void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring::Setup()
{
  int ext_count = 0;   
  ...
  lte_rrc_cep_test.ext_umid_list[1] = LTE_RRC_CONN_EST_CNF;
  lte_rrc_cep_test.ext_umid_count = ++ext_count;
  lte_rrc_cep_test.ext_umid_list[2] = LTE_RRC_BARRING_UPDATE_IND;
  lte_rrc_cep_test.ext_umid_count = ++ext_count;
...
void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring::Test()
{
  lte_rrc_conn_est_cnf_s        *conn_est_cnf_ptr = NULL;
   //lte_rrc_conn_establishment_failure_indi_s *conn_est_failure_indi_ptr = NULL;
   lte_rrc_conn_establishment_started_indi_s *conn_est_started_indi_ptr = NULL;
  ...
  uint8 *value_ptr = NULL;
   ...
   //struct lte_rrc_AccessClassBarringList_ *ac_list_ptr = NULL;
   //struct lte_rrc_AccessClassBarringList_ *ac_list_ptr_next = NULL;
  ...
  /*Fill in the appropriate bit mask*/
  *(value_ptr) = (0x00 >>(LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS -11));
  ...
  conn_est_req.ac_barring_info_for_special_valid = TRUE;
  conn_est_req.ac_barring_info_for_special.access_barring_time = 100;

  for (i = 0; i < 16; i++)
  {
    conn_est_req.ac_barring_info_for_special.access_barring_prob[i] = 0;
  }
...
void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring_CSFB);

void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring_CSFB::Setup()
{
  int ext_count = 0;   
...
void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring_CSFB::Test()
{
  lte_rrc_conn_est_cnf_s        *conn_est_cnf_ptr = NULL;
   //lte_rrc_conn_establishment_failure_indi_s *conn_est_failure_indi_ptr = NULL;
   lte_rrc_conn_establishment_started_indi_s *conn_est_started_indi_ptr = NULL;
   ...
   //#endif
   /*Set the Est cause to MO_DATA*/
   lte_rrc_est_cause_e est_cause = LTE_RRC_EST_CAUSE_MO_CSFB;
  lte_rrc_osys_SystemInformationBlockType2 *sib2 = NULL;
  ...
  *(value_ptr) = (0x00 >>(LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS -11));
  ...
  conn_est_req.ac_barring_info_for_special_valid = TRUE;
  conn_est_req.ac_barring_info_for_special.access_barring_time = 0;
...
void LteRrcCEPAccessListFailureMODataTest_for_internal_AC_Barring_CSFB::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}
