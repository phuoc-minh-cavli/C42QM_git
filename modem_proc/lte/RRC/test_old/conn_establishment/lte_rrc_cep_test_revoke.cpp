...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_revoke.cpp#1 $
...
static lte_rrc_ml1_revoke_cnfi_s revoke_cnfi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupRevokeTest1);

void LteRrcCEPConnSetupRevokeTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupRevokeTest1::Test()
{
  ...
  lte_rrc_ml1_revoke_reqi_s *revoke_reqi_ptr = NULL;;
  ...
  lte_rrc_test_cep_set_controller_state(LTE_RRC_CONTROLLER_SM__SUSPENDED);  
  ...
  TF_MSG("Expecting Revoke Reqi from CEP");

 /*Expect the Revoke Reqi from CEP*/
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_ml1_revoke_reqi_s) == buf_len);
  revoke_reqi_ptr = 
    (lte_rrc_ml1_revoke_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(revoke_reqi_ptr !=NULL);
  TF_ASSERT((revoke_reqi_ptr->msg_hdr.id) ==  
            LTE_RRC_ML1_REVOKE_REQI);

  TF_MSG("Checking that the CEP SM is in Pending Retry state");
...
/*Set the RRC controller in Camped state*/
  ...
  /*Send Revoke cnfi*/
  TF_MSG("Sending ML Revoke Cnfi");
  lte_rrc_init_default_hdr(&revoke_cnfi.msg_hdr, LTE_RRC_ML1_REVOKE_CNFI);
  lte_rrc_utf_send_msg((byte*)&revoke_cnfi, sizeof(revoke_cnfi));
...
//  #if 0
  conn_req_msg_ptr = 
    (lte_rrc_osys_RRCConnectionRequest *)(ul_ccch_msg_ptr->message.u.c1->u.rrcConnectionRequest); 
  ...
  *(ulm_ptr->mu_id) = mu_id;
  ...
  conn_setup_complete_ptr = 
   ...
   if(0 == conn_est_req.registered_mme_is_present)
   {
...
void LteRrcCEPConnSetupRevokeTest1::Teardown()
{
