...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_ssac_idle.cpp#1 $
...
static boolean skip_acb_for_volte_call;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPSSACIdleModeTest1);

void LteRrcCEPSSACIdleModeTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPSSACIdleModeTest1::Test()
{
  ...
  lte_rrc_ul_msg_cnfi_s ul_msg_cnfi;
  lte_rrc_ul_data_cnf_s *ul_data_cnf_ptr = NULL;

 

  uint8 mu_id = 34;
  uint32 nas_trans_id = 534;
  ...
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_barring_update_ind_s *ssac_ind;
  int decode_status = 0; 
  ...
  access_class_mask = (1 << LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS_12);
  lte_rrc_test_cep_set_controller_access_class(access_class_mask);
  ...
  sib2->m.ssac_BarringForMMTEL_Video_r9Present = TRUE;
  sib2->m.ssac_BarringForMMTEL_Voice_r9Present = TRUE;
  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringFactor = 			lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p10;

  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringTime = 
  lte_rrc_osys_AC_BarringConfig_ac_BarringTime_s16;

  sib2->ssac_BarringForMMTEL_Voice_r9.ac_BarringFactor = 
	  lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p70;

  sib2->ssac_BarringForMMTEL_Voice_r9.ac_BarringTime =
	 lte_rrc_osys_AC_BarringConfig_ac_BarringTime_s128; 
  
  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringForSpecialAC.numbits = 5;
  //Enable special AC bit 11 for barring
  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringForSpecialAC.data[0] = 0xff;
  
  sib2->ssac_BarringForMMTEL_Voice_r9.ac_BarringForSpecialAC.numbits = 5;

  //Disable all special AC bits
  sib2->ssac_BarringForMMTEL_Voice_r9.ac_BarringForSpecialAC.data[0] = 0xff;
  
  TF_ASSERT( E_SUCCESS == lte_rrc_test_cep_stub_get_selected_plmn(&camped_plmn, &plmn_idx) );

  lte_rrc_cep_get_ssac_params(&ssac_params, &skip_acb_for_volte_call);

 //TF_ASSERT(barring_factor_mmtel_voice == lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p70);
 TF_ASSERT(ssac_params.barring_factor_mmtel_voice == 70);
 TF_ASSERT(ssac_params.barring_factor_mmtel_video == 10);
 //TF_ASSERT(barring_time_mmtel_voice == 
 //		   lte_rrc_osys_AC_BarringConfig_ac_BarringTime_s128);
 TF_ASSERT(ssac_params.barring_time_mmtel_voice == 128);
 TF_ASSERT(ssac_params.barring_time_mmtel_video == 16);


   // We expect a ssac_ind message from RRC
  lte_rrc_utf_get_next_ext_msg((byte**)&ssac_ind, &buf_len);

  //TF_ASSERT(service_ind->trans_id == 1);
  TF_ASSERT(ssac_ind->msg_hdr.id == LTE_RRC_BARRING_UPDATE_IND);

  
  TF_ASSERT(buf_len == sizeof(lte_rrc_barring_update_ind_s));
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_video_valid == TRUE);
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_voice_valid == TRUE);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_video == 10);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_voice == 70);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_video == 16);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_voice == 128);

  sib2->m.ssac_BarringForMMTEL_Video_r9Present = FALSE;
  sib2->m.ssac_BarringForMMTEL_Voice_r9Present = FALSE;
  ...
  TF_ASSERT(buf_len == sizeof(lte_rrc_barring_update_ind_s));
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_video_valid == FALSE);
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_voice_valid == FALSE);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_video == 255);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_voice == 255);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_video == 65535);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_voice == 65535);
  ...
  conn_est_req.transaction_id = nas_trans_id;
...
//  #if 0
  ...
  *(ulm_ptr->mu_id) = mu_id;
  ...
  conn_setup_complete_ptr = (ul_dcch_msg_ptr->message).u.c1->u.rrcConnectionSetupComplete;
   ...
   {
     TF_ASSERT(0 == r8_ie_ptr->m.nonCriticalExtensionPresent && 0 == r8_ie_ptr->m.registeredMMEPresent);
   ...
   //TF_ASSERT(r8_ie_ptr->nas_DedicatedInformation.length == 
   ...
   {
     //TF_ASSERT(0 == (memcmp(r8_ie_ptr->nas_DedicatedInformation.value, 
#if 0
  ...
  /*Send UL MSG CNF for ConnSetupComplete*/
  TF_MSG("Sending the UL MSG CNF for Conn SetupComplete");
  lte_rrc_init_default_hdr(&ul_msg_cnfi.msg_hdr, 
                            LTE_RRC_RRC_CONNECTION_SETUP_COMPLETE_CNFI); 
  ul_msg_cnfi.mu_id = mu_id;
  ul_msg_cnfi.status = LTE_RRC_UL_MSG_ACK;
  lte_rrc_utf_send_msg((byte*)&ul_msg_cnfi, sizeof(ul_msg_cnfi));

  lte_rrc_utf_wait_for_done();

  /*Expect UL DATA CNF from CEP */
  lte_rrc_utf_get_next_ext_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_ul_data_cnf_s) == buf_len);
  ul_data_cnf_ptr = 
    (lte_rrc_ul_data_cnf_s *)(void*) buf_ptr;
  TF_ASSERT(ul_data_cnf_ptr !=NULL);
  TF_ASSERT((ul_data_cnf_ptr->msg_hdr.id) ==  
            LTE_RRC_UL_DATA_CNF);
  TF_ASSERT(1 == ul_data_cnf_ptr->num_trans_ids);
  TF_ASSERT(conn_est_req.transaction_id == ul_data_cnf_ptr->trans_id[0]);
  ...
  TF_ASSERT(ul_data_cnf_ptr->ta.tac == test_state_ptr->tac);

  TF_ASSERT(0 == memcmp(&(ul_data_cnf_ptr->ta.plmn),&(test_state_ptr->plmn),
                        sizeof(lte_rrc_plmn_s)));
...
void LteRrcCEPSSACIdleModeTest1::Teardown()
{
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPSSACIdleModeTest2);

void LteRrcCEPSSACIdleModeTest2::Setup()
{
  int ext_count = 0;
  int result;
  uint8 ac_barring_restr = 1;

  qtf_efs_initialize("LteRrcCEPSSACIdleModeTest2 - ACB restriction with EFS test case");

  result = efs_put("/nv/item_files/modem/lte/rrc/cep/ac_barring_restriction_param",
                   &ac_barring_restr,
...
void LteRrcCEPSSACIdleModeTest2::Test()
{
  lte_rrc_conn_est_cnf_s        *conn_est_cnf_ptr = NULL;
  //lte_rrc_conn_establishment_failure_indi_s *conn_est_failure_indi_ptr = NULL;
  lte_rrc_conn_establishment_started_indi_s *conn_est_started_indi_ptr = NULL;
  ...
  access_class_mask = (1 << LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS);
  lte_rrc_test_cep_set_controller_access_class(access_class_mask);
  ...
  lte_rrc_init_default_hdr(&conn_est_req.msg_hdr, LTE_RRC_CONN_EST_REQ);
  conn_est_req.est_cause = est_cause;
  conn_est_req.barring_restr_for_special_ac_enabled = TRUE;
  /*No registered MME*/
  conn_est_req.registered_mme_is_present= FALSE;
...
//#if 0
  ...
  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringForSpecialAC.numbits = 5;
  //Enable special AC bit 11 for barringL
  sib2->ssac_BarringForMMTEL_Video_r9.ac_BarringForSpecialAC.data[0] = 0xff;
  ...
  value_ptr = sib2->ssac_BarringForMMTEL_Voice_r9.ac_BarringForSpecialAC.data;
  *(value_ptr) = (0x80 >>(LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS -11));

  TF_ASSERT( E_SUCCESS == lte_rrc_test_cep_stub_get_selected_plmn(&camped_plmn, &plmn_idx) );

  lte_rrc_cep_get_ssac_params(&ssac_params, &skip_acb_for_volte_call);

  /*! This is FR 27133 - where if lower access class is not barred, we do not bar higher access class too*/
 //TF_ASSERT(barring_factor_mmtel_voice == lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p70);
 TF_ASSERT(ssac_params.barring_factor_mmtel_voice == 100);
 TF_ASSERT(ssac_params.barring_factor_mmtel_video == 100);
 //TF_ASSERT(barring_time_mmtel_voice == 
 //		   lte_rrc_osys_AC_BarringConfig_ac_BarringTime_s128);
 TF_ASSERT(ssac_params.barring_time_mmtel_voice == 0);
 TF_ASSERT(ssac_params.barring_time_mmtel_video == 0);
  ...
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_video_valid == TRUE);
  TF_ASSERT(ssac_ind->ssac_params.ssac_param_voice_valid == TRUE);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_video == 100);
  TF_ASSERT(ssac_ind->ssac_params.barring_factor_mmtel_voice == 100);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_video == 0);
  TF_ASSERT(ssac_ind->ssac_params.barring_time_mmtel_voice == 0);
...
void LteRrcCEPSSACIdleModeTest2::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}
