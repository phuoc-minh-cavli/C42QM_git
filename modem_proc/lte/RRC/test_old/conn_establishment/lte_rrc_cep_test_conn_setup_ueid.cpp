...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_conn_setup_ueid.cpp#1 $
...
static lte_rrc_ue_id_list_s ue_id_list;
/*===========================================================================
...
*/     
  TF_PROTO_STUB( const lte_rrc_ue_id_list_s *, lte_rrc_controller_get_ue_id_list, void);
}
...
/*!
    @brief
    Stub function for lte_rrc_controller_get_ue_id_list

    @detail

    @return
    Returns TRUE -- i.e.UE ID list is valid. 
    

*/
/*=========================================================================*/

const lte_rrc_ue_id_list_s * lte_rrc_test_cep_stub_controller_get_ue_id_list2(void)
{
  
  lte_rrc_s_tmsi_s *s_tmsi_ptr = NULL;     
  
  //return &(lte_rrc_cep_test.ue_id_list);

  
  ue_id_list.num_ue_identities = 2;
  ue_id_list.ue_id[0].type = LTE_RRC_UE_ID_IMSI;
  ue_id_list.ue_id[1].type = LTE_RRC_UE_ID_STMSI;
  
  s_tmsi_ptr = &(ue_id_list.ue_id[1].value.s_tmsi);
  /*
  s_tmsi_ptr->m_tmsi= 0x2f29ad7a;
  s_tmsi_ptr->mmec = 0x01;
  */
  s_tmsi_ptr->m_tmsi= 0x29ad7a01;
  s_tmsi_ptr->mmec = 0x2f;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupUEIDTest1);

void LteRrcCEPConnSetupUEIDTest1::Setup()
{
  int ext_count = 0;
  ...
  TF_STUB(lte_rrc_controller_get_ue_id_list) = 
    lte_rrc_test_cep_stub_controller_get_ue_id_list2;
  
  
}

void LteRrcCEPConnSetupUEIDTest1::Test()
{
...
//  #if 0
  ...
  //TF_ASSERT(0x01 == ue_identity_ptr->u.s_TMSI->mmec.data[0]);
  TF_ASSERT(0x2f == ue_identity_ptr->u.s_TMSI->mmec.data[0]);
  TF_ASSERT(8 == ue_identity_ptr->u.s_TMSI->mmec.numbits);

  TF_ASSERT(32 == ue_identity_ptr->u.s_TMSI->m_TMSI.numbits);
  //TF_ASSERT(0x2f == ue_identity_ptr->u.s_TMSI->m_TMSI.data[0]);
  TF_ASSERT(0x29 == ue_identity_ptr->u.s_TMSI->m_TMSI.data[0]);
  //TF_ASSERT(0x29 == ue_identity_ptr->u.s_TMSI->m_TMSI.data[1]);
  TF_ASSERT(0xad == ue_identity_ptr->u.s_TMSI->m_TMSI.data[1]);
  //TF_ASSERT(0xad == ue_identity_ptr->u.s_TMSI->m_TMSI.data[2]);
  TF_ASSERT(0x7a == ue_identity_ptr->u.s_TMSI->m_TMSI.data[2]);
  //TF_ASSERT(0x7a == ue_identity_ptr->u.s_TMSI->m_TMSI.data[3]);
  TF_ASSERT(0x01 == ue_identity_ptr->u.s_TMSI->m_TMSI.data[3]);
...
void LteRrcCEPConnSetupUEIDTest1::Teardown()
{
