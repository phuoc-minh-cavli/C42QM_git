...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_bad_conn_setup.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupErrTest1);

void LteRrcCEPConnSetupErrTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupErrTest1::Test()
{
  ...
  lte_rrc_osys_DL_CCCH_Message *dl_ccch_ptr = NULL;
...
//  #if 0
  ...
  TF_ASSERT(conn_req_msg_ptr->criticalExtensions.u.rrcConnectionRequest_r8->establishmentCause
             == lte_rrc_cep_est_cause_to_asn1(est_cause));
  ...
  lte_rrc_test_cep_build_connection_setup2_dlm(&conn_setup_dlm, &lte_rrc_cep_test);
  dl_ccch_ptr = (lte_rrc_osys_DL_CCCH_Message *)(conn_setup_dlm.dl_msg_ptr);

  dl_ccch_ptr->message.t = 
    T_lte_rrc_osys_DL_CCCH_MessageType_messageClassExtension; 
  ...
  /*Expect a CPHY Cancel ConnReq from CEP*/
  lte_rrc_utf_get_next_ext_msg (&buf_ptr, &buf_len);
  ...
  TF_ASSERT((cphy_cancel_conn_req_ptr->msgr_hdr.id) ==  
   ...
   /*Expect a MAC Cancel ConnReq from CEP*/
  lte_rrc_utf_get_next_ext_msg (&buf_ptr, &buf_len);
  ...
  /*Expect a Conn Establishment Failure Indi from CEP*/
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  ...
  TF_ASSERT(LTE_RRC_CONN_EST_FAILURE == conn_est_cnf_ptr->est_status);

  /*Verify that the state of the CEP SM is Inactive*/
  TF_MSG("Verifying that the CEP SM is in Inactive state");
  ...
  *(ulm_ptr->mu_id) = mu_id;
  ...
  (&conn_setup_complete_ptr->criticalExtensions.u.c1->u.
   ...
   {
     TF_ASSERT(0!= ((lte_rrc_registeredMME_present) & (r8_ie_ptr->bit_mask)));
   }
...
void LteRrcCEPConnSetupErrTest1::Teardown()
{
