/*!
  @file lte_rrc_cep_test_t302_mod_barred.cpp


  @brief
  Unit test code for MOData call barred when T302 is running.
...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_t302_mod_barred.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/04/08   dd     Fixed bitmask of ConnEstCnf after ConnReject
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPT302MoDBarredTest1);

void LteRrcCEPT302MoDBarredTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPT302MoDBarredTest1::Test()
{
...
//  #if 0
  ...
  if(conn_est_cnf_ptr->barring_timer_value >5)
  {
    /*Sleep less than the Access Barring time as given by T303*/
     ...
     TF_MSG("Sending the ConnEstReq");
     lte_rrc_init_default_hdr(&conn_est_req.msg_hdr, LTE_RRC_CONN_EST_REQ);
     conn_est_req.est_cause = LTE_RRC_EST_CAUSE_MO_DATA;
     lte_rrc_utf_send_msg((byte*)&conn_est_req, sizeof(conn_est_req));
     lte_rrc_utf_wait_for_done();
  ...
  TF_SLEEP(5);
    ...
    /*Expect T306 tmri*/
  TF_MSG("Checking for T306 tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
...
void LteRrcCEPT302MoDBarredTest1::Teardown()
{
