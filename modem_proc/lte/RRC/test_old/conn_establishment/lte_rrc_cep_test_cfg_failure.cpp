...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_cfg_failure.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPCfgFailureTest1);

void LteRrcCEPCfgFailureTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPCfgFailureTest1::Test()
{
...
//  #if 0
  ...
  *(cfg_req_ptr->trans_id_ptr)= lte_rrc_cep_test.cfg_transaction_id;
  ...
  /*Setthe status to a non-zero (error) value*/
 ...
 TF_MSG("Received MAC CancelConnReq from CEP");
  ...
  TF_ASSERT(LTE_RRC_CONN_EST_FAILURE_CONFIG_FAILURE 
...
void LteRrcCEPCfgFailureTest1::Teardown()
{
