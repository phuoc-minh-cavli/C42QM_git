...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_rand_conn_setup.cpp#1 $
...
#define LTE_RRC_CEP_TEST_PROB_FACTOR 5
...
*/     
  TF_PROTO_STUB( boolean, lte_rrc_controller_ue_id_list_is_valid, void);
}
...
//TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPRandConnSetupTest1);
TF_DEFINE_TEST_CASE(xyz, LteRrcCEPRandConnSetupTest1);

void LteRrcCEPRandConnSetupTest1::Setup()
{
  int ext_count = 0;
  ...
  lte_rrc_utf_setup(LTE_RRC_CEP_SM, 
                    ...
                    ext_count);
  lte_rrc_test_cep_init();
  TF_STUB(lte_rrc_controller_ue_id_list_is_valid) = 
    lte_rrc_test_cep_stub_controller_ue_id_list_is_valid2;

}

void LteRrcCEPRandConnSetupTest1::Test()
{
  ...
  uint16 barred_count = 0;
  uint16 not_barred_count = 0;
  ...
  sib2 = (lte_rrc_osys_SystemInformationBlockType2 *)
    lte_rrc_test_cep_stub_get_cell_sib(LTE_RRC_SIB2_BITMASK,0);
  ...
  sib2->ac_BarringInfo.ac_BarringForMO_Data.ac_BarringFactor = 
   lte_rrc_osys_AC_BarringConfig_ac_BarringFactor_p05; 
  ...
  for(call_count = 0; call_count < 100; call_count ++)
  {
    ...
    if(i_ptr->dd_ptr->random_number < LTE_RRC_CEP_TEST_PROB_FACTOR)
    {
      ...
      /*
      Commenting the UE ID part for now...because we are sending 
      no S_TMSI (i.e. UE ID is random #)
...
//      if(conn_est_req.nas_msg_container.msg_size >0)
      ...
      /*Verify that the CEP SM has transitioned to INACTIVE state*/

      TF_MSG("After sending ConnReleased indication: "
             "Verifying that the CEP SM is in Inactive state");
  ...
  TF_MSG("Barred count = %d; Not barred count = %d ", barred_count, not_barred_count);
  lte_rrc_utf_reset();
}

void LteRrcCEPRandConnSetupTest1::Teardown()
{
