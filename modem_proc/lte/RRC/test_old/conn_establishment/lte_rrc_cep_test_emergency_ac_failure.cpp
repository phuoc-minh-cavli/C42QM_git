/*!
  @file lte_rrc_cep_test_emergency_ac_failure.cpp


  @brief
  Unit test code for CEP Connnection Setup for Emergency Calls AC Barring
  Failure .
...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_emergency_ac_failure.cpp#1 $
...
#include "lte_rrc_cep_test_ota_msg.h"
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupACListEmergencyFailureTest1);

void LteRrcCEPConnSetupACListEmergencyFailureTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupACListEmergencyFailureTest1::Test()
{
  ...
  lte_rrc_est_cause_e est_cause = LTE_RRC_EST_CAUSE_EMERGENCY;
  ...
  //sib2->accessBarringInformation.accessBarringForEmergencyCalls = TRUE;
  sib2->ac_BarringInfo.ac_BarringForEmergency = TRUE;
...
/*Expect ConnEstCnf from CEP */
  ...
  TF_ASSERT((1<< (uint8)LTE_RRC_EST_CAUSE_EMERGENCY) == 
            conn_est_cnf_ptr->call_type_bitmask);

  TF_ASSERT(((uint64)LTE_RRC_INDEFINITE_BARRING_TIME) == 
...
void LteRrcCEPConnSetupACListEmergencyFailureTest1::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupACListEmergencyFailureTest1_internal_ac_barring);

void LteRrcCEPConnSetupACListEmergencyFailureTest1_internal_ac_barring::Setup()
{
  int ext_count = 0;
  //TF_STUB(lte_rrc_controller_get_state) = lte_rrc_test_cep_stub_controller_get_state;
  //lte_rrc_test_cep_init();
...
void LteRrcCEPConnSetupACListEmergencyFailureTest1_internal_ac_barring::Test()
{
  lte_rrc_conn_est_cnf_s        *conn_est_cnf_ptr = NULL;
   //lte_rrc_conn_establishment_failure_indi_s *conn_est_failure_indi_ptr = NULL;
   lte_rrc_conn_establishment_started_indi_s *conn_est_started_indi_ptr = NULL;
  ...
  sib2->ac_BarringInfo.ac_BarringForMO_Data.ac_BarringForSpecialAC.numbits = 5;
  value_ptr = sib2->ac_BarringInfo.ac_BarringForMO_Data.ac_BarringForSpecialAC.data;
...
void LteRrcCEPConnSetupACListEmergencyFailureTest1_internal_ac_barring::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupACListEmergencyFailureTest2);

void LteRrcCEPConnSetupACListEmergencyFailureTest2::Setup()
{
  int ext_count = 0;
  //TF_STUB(lte_rrc_controller_get_state) = lte_rrc_test_cep_stub_controller_get_state;
  //lte_rrc_test_cep_init();
  ...
  /*Initialize the BCCH_DL_SCH ptr... for this test specific SIB2*/
  lte_rrc_test_cep_sib2_init(cep_custom_sib2_1, sizeof(cep_custom_sib2_1));
}

void LteRrcCEPConnSetupACListEmergencyFailureTest2::Test()
{
  lte_rrc_conn_est_cnf_s        *conn_est_cnf_ptr = NULL;
  lte_rrc_conn_establishment_failure_indi_s *conn_est_failure_indi_ptr = NULL;
  /* DSM pointer */
  ...
  /*Set the Access class in the USIM to invalid*/
  lte_rrc_test_cep_set_controller_access_class(1 << LTE_RRC_CEP_TEST_USIM_ACCESS_CLASS_1);

  sib2 = (lte_rrc_osys_SystemInformationBlockType2 *)lte_rrc_test_cep_stub_get_cell_sib(LTE_RRC_SIB2_BITMASK,0);

  /*Current test specific SIB2 basic validation*/
  TF_ASSERT(1==sib2->m.ac_BarringInfoPresent);
  TF_ASSERT(TRUE==sib2->ac_BarringInfo.ac_BarringForEmergency);
  TF_ASSERT(FALSE==sib2->ac_BarringInfo.m.ac_BarringForMO_DataPresent); 
...
void LteRrcCEPConnSetupACListEmergencyFailureTest2::Teardown()
{
  lte_rrc_test_cep_teardown();
  lte_rrc_utf_teardown();
}
