...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_access_resel_conn_setup_success.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPAccessReselectionTest4);
/*!@todo: Fix this test with the reseelction stuff*/
//TF_DEFINE_TEST_CASE(XYZ, LteRrcCEPAccessReselectionTest1);

void LteRrcCEPAccessReselectionTest4::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPAccessReselectionTestSendConnReq
(
  void
)
{
...
void LteRrcCEPAccessReselectionTest4::Test()
{
  ...
  //{
    ...
    LteRrcCEPAccessReselectionTestSendConnReq();
    ...
    //{
      ...
      /* Send Camped indication to indicate reselection has completed */
      TF_MSG("Sending CampedIndi");
      lte_rrc_init_default_hdr(&camped_indi.msg_hdr, 
                               LTE_RRC_CAMPED_INDI);
      ...
      *(ulm_ptr->encode_status) = 0; 
      ...
      i_ptr = (lte_rrc_cep_s *)stm_get_user_data(LTE_RRC_CEP_SM);
      TF_ASSERT(FALSE == i_ptr->rach_in_progress);
      ...
      i_ptr = (lte_rrc_cep_s *)stm_get_user_data(LTE_RRC_CEP_SM);
      TF_ASSERT(FALSE == i_ptr->rach_in_progress);
      TF_ASSERT(TRUE == i_ptr->resel_timer_is_running);
...
void LteRrcCEPAccessReselectionTest4::Teardown()
{
