...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/conn_establishment/lte_rrc_cep_test_t302_mt_barred.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPT302MTBarredTest1);

void LteRrcCEPT302MTBarredTest1::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPT302MTBarredTest1::Test()
{
...
//  #if 0
  ...
  if(conn_est_cnf_ptr->barring_timer_value >5)
  {
    /*Sleep less than the Access Barring time as given by T303*/
     ...
     TF_MSG("Sending the ConnEstReq");
     lte_rrc_init_default_hdr(&conn_est_req.msg_hdr, LTE_RRC_CONN_EST_REQ);
     conn_est_req.est_cause = LTE_RRC_EST_CAUSE_MT_ACCESS;
     lte_rrc_utf_send_msg((byte*)&conn_est_req, sizeof(conn_est_req));
     lte_rrc_utf_wait_for_done();
     ...
     TF_ASSERT(LTE_RRC_CONN_EST_FAILURE_ACCESS_BARRED == 
      ...
      TF_ASSERT(((1<< (uint8)LTE_RRC_EST_CAUSE_MT_ACCESS)& 
...
void LteRrcCEPT302MTBarredTest1::Teardown()
{
