/*!
  @file lte_rrc_cep_test_conn_setup_dlm_during_rach_abort.cpp


  @brief
  Unit test code for CEP that tests the handling of Connnection Setup DLM when
  RACH abort is in progress due to Reselection.
...
$Header: //source/qcom/qct/modem/lte/RRC/main/latest/test/conn_establishment/ 
lte_rrc_cep_test_conn_setup_dlm_during_rach_abort.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/12/11   mm     Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCEPConnSetupDLMDuringRachAbortTest);

void LteRrcCEPConnSetupDLMDuringRachAbortTest::Setup()
{
  int ext_count = 0;
...
void LteRrcCEPConnSetupDLMDuringRachAbortTest::Test()
{
  ...
  for(i =0; i<2; i++)
  {
    ...
    if (reselect == 1)
    {
      ...
      /* While CEP is waiting for RACH abort, send the Conn Setup DLM */
      ...
      /* Verify that the CEP SM still is in Access state */
      TF_ASSERT(stm_get_state(LTE_RRC_CEP_SM) == LTE_RRC_CEP_SM__ACCESS);
      ...
      /*Expect a DLM processed indication from CEP*/
      lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
...
void LteRrcCEPConnSetupDLMDuringRachAbortTest::Teardown()
{
