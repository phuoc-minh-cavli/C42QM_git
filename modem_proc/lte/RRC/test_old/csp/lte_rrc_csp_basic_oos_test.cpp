...
01/12/09   sureshs Initial Revision
...
static lte_cphy_oos_ind_s oos_ind;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicOosTest);

void LteRrcCspBasicOosTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicOosTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  TF_MSG("Sending LTE_CPHY_OOS_IND");
  msgr_init_hdr(&oos_ind.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_OOS_IND);
  oos_ind.serv_cell_info.dl_cell_frequency = 1;
  oos_ind.serv_cell_info.phy_cell_id = 1;
  lte_rrc_utf_send_msg((byte*)&oos_ind, sizeof(oos_ind));
...
void LteRrcCspBasicOosTest::Teardown()
{
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicOosTestFWResetAcq);

void LteRrcCspBasicOosTestFWResetAcq::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicOosTestFWResetAcq::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  TF_MSG("Checking for LTE_RRC_CFG_REQI for ACQ REQ");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);
  ...
  oos_ind.serv_cell_info.dl_cell_frequency = 1;
  oos_ind.serv_cell_info.phy_cell_id = 1;
  oos_ind.oos_reason = LTE_CPHY_OOS_REASON_RESET_5;
  lte_rrc_utf_send_msg((byte*)&oos_ind, sizeof(oos_ind));
...
void LteRrcCspBasicOosTestFWResetAcq::Teardown()
{
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicOosTestFWResetBandscan);

void LteRrcCspBasicOosTestFWResetBandscan::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicOosTestFWResetBandscan::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  TF_MSG("Checking for LTE_RRC_CFG_REQI for Band Scan REQ");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);
...
void LteRrcCspBasicOosTestFWResetBandscan::Teardown()
{
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicOosTestFWResetSystemscan);

void LteRrcCspBasicOosTestFWResetSystemscan::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicOosTestFWResetSystemscan::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
...
void LteRrcCspBasicOosTestFWResetSystemscan::Teardown()
{
  lte_rrc_utf_teardown();
 }
