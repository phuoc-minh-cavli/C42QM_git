/*!
  @file lte_rrc_csp_cell_reselection_failure_test.cpp
  ...
  Scan is started. After camping, reselection is triggered but S criteria 
  check fails. Reselection is canceled and UE camps again.
...
12/17/08   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspCellReSelectionFailureTest);

void LteRrcCspCellReSelectionFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspCellReSelectionFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp_test_set_num_tx_antennas(2);
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  TF_ASSERT(lte_rrc_csp_get_num_tx_antennas() == 2);
  ...
  resel_ind.target_cell_info.rat_cell_info.lte_cell_info.dl_cell_frequency = 1;
  resel_ind.target_cell_info.rat_cell_info.lte_cell_info.phy_cell_id = 2;
  ...
  lte_rrc_csp_test_set_num_tx_antennas(4);
  ...
  TF_MSG("Sending LTE_CPHY_CELL_SELECT_CNF failure");
  msgr_init_hdr(&cell_sel_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_CELL_SELECT_CNF);
  ...
  TF_ASSERT(LTE_CPHY_RESEL_CANCEL_CAUSE_S_FAILURE == 
            resel_cancel_req_ptr->cancel_info.cancel_cause);
  TF_ASSERT(1 == resel_cancel_req_ptr->serv_cell_info.dl_cell_frequency);
  TF_ASSERT(1 == resel_cancel_req_ptr->serv_cell_info.phy_cell_id);
  TF_ASSERT(LTE_RAT_EUTRAN == resel_cancel_req_ptr->cancel_info.
            target_cell_info.rat_type);
  TF_ASSERT(1 == resel_cancel_req_ptr->cancel_info.target_cell_info.
            rat_cell_info.lte_cell_info.dl_cell_frequency);
  TF_ASSERT(2 == resel_cancel_req_ptr->cancel_info.target_cell_info.
            rat_cell_info.lte_cell_info.phy_cell_id);
...
void LteRrcCspCellReSelectionFailureTest::Teardown()
{
