/*!
  @file lte_rrc_csp_basic_system_scan_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Controller is in Inactive state and there is one entry in the Acq DB. System
  Scan is started
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBandChangeOnFly1);

void LteRrcCspBandChangeOnFly1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBandChangeOnFly1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp_test_construct_basic_sib1_bandChgFly(SYS_SBAND_LTE_EUTRAN_BAND25, TRUE, TRUE);

  lte_rrc_utf_set_sync_cfg_reqi_params(LTE_RRC_CSP_TEST_CFG_TRANS_ID);

  TF_MSG("Creating Acq DB entry");
  lte_rrc_csp.sd_ptr->acq_db.db[0].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  lte_rrc_csp.sd_ptr->acq_db.db[0].bandwidth = LTE_BW_NRB_6;
  lte_rrc_csp.sd_ptr->acq_db.db[0].dl_earfcn = 650;
  lte_rrc_csp.sd_ptr->acq_db.db[0].phy_cell_id = 1;
  lte_rrc_csp.sd_ptr->acq_db.db[0].cell_identity = 1;
  ...
  lte_rrc_csp.sd_ptr->acq_db.acq_db_index[0] = 0;
  lte_rrc_csp.sd_ptr->acq_db.acq_db_count = 1;
  ...
  svc_req.rat_pri_list.priority_list_info[0].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_LTE_BAND25;
  ...
  get_sib_cnf.status = LTE_RRC_GET_SIBS_BANDCHG_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 650;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  get_sib_cnf.new_dl_freq_list.num_freqs = 1;
  get_sib_cnf.new_dl_freq_list.new_dl_freq[0] = 8090;
  ...
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND25;
  cfg_req_param.earfcn = 8090;

  send_band_scan_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  send_band_scan_cnf.candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND25;
  send_band_scan_cnf.candidates[0].system.earfcn = 8090;
  
  validate_system_scan_req[0].band = SYS_SBAND_LTE_EUTRAN_BAND25;
  validate_system_scan_req[0].earfcn = 8090;
  ...
  get_sib_cnf.cell_info.cell_id.freq = 8090;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  get_sib_cnf.new_dl_freq_list.num_freqs = 1;
  get_sib_cnf.new_dl_freq_list.new_dl_freq[0] = 8090;
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND25 == cell_sel_req_ptr->band);
  TF_ASSERT(8090 == cell_sel_req_ptr->earfcn);
  ...
  cell_sel_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND25;
  cell_sel_cnf.earfcn = 8090;
...
void LteRrcCspBandChangeOnFly1::Teardown()
{
  lte_rrc_utf_teardown();
}


TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBandChangeOnFly2);

void LteRrcCspBandChangeOnFly2::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBandChangeOnFly2::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[1].dl_earfcn = 1;
  ...
  lte_rrc_csp.sd_ptr->acq_db.acq_db_index[0] = 0;
  lte_rrc_csp.sd_ptr->acq_db.acq_db_index[1] = 1;
  ...
  validate_system_scan_req[1].earfcn = 1;
  ...
  send_band_scan_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND25;
  send_band_scan_cnf.candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND25;
  send_band_scan_cnf.candidates[0].system.earfcn = 8090;
  ...
  lte_rrc_csp_test_construct_basic_sib1_bandChgFly(SYS_SBAND_LTE_EUTRAN_BAND1, TRUE, TRUE);
  ...
  get_sib_cnf.cell_info.sib_db_key = 1;
  get_sib_cnf.new_dl_freq_list.num_freqs = 1;
  get_sib_cnf.new_dl_freq_list.new_dl_freq[0] = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
...
void LteRrcCspBandChangeOnFly2::Teardown()
{
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBandChangeOnFly3);

void LteRrcCspBandChangeOnFly3::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBandChangeOnFly3::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  lte_rrc_service_ind_s *svc_ind_ptr;
  lte_rrc_sib_abort_reqi_s *sib_abort_ptr;
  lte_cphy_cell_bar_req_s *cell_bar_req_ptr;
  lte_rrc_csp_test_cfg_req_param_s		  cfg_req_param;
  lte_rrc_csp_validate_system_scan_req_s  validate_system_scan_req[LTE_CPHY_SYSTEMSCAN_MAX_SYSTEMS_TO_SCAN];
  lte_rrc_csp_send_band_scan_cnf_s		  send_band_scan_cnf;
  ...
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  get_sib_cnf.new_dl_freq_list.num_freqs = 2;
  get_sib_cnf.new_dl_freq_list.new_dl_freq[0] = 8090;
  get_sib_cnf.new_dl_freq_list.new_dl_freq[1] = 8091;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  cell_bar_req_ptr = (lte_cphy_cell_bar_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_CELL_BAR_REQ == cell_bar_req_ptr->msgr_hdr.id);
  TF_ASSERT(2 == cell_bar_req_ptr->bar_info.num_cells);
  TF_ASSERT(LTE_RAT_EUTRAN == cell_bar_req_ptr->bar_info.cell_info_list[0].rat_type);
  TF_ASSERT(8090 == cell_bar_req_ptr->bar_info.cell_info_list[0].rat_info.lte_cell_info.dl_cell_frequency);
  TF_ASSERT(1 == cell_bar_req_ptr->bar_info.cell_info_list[0].rat_info.lte_cell_info.phy_cell_id);
  TF_ASSERT(LTE_RAT_EUTRAN == cell_bar_req_ptr->bar_info.cell_info_list[1].rat_type);
  TF_ASSERT(8091 == cell_bar_req_ptr->bar_info.cell_info_list[1].rat_info.lte_cell_info.dl_cell_frequency);
  TF_ASSERT(1 == cell_bar_req_ptr->bar_info.cell_info_list[1].rat_info.lte_cell_info.phy_cell_id);
...
void LteRrcCspBandChangeOnFly3::Teardown()
{
  lte_rrc_utf_teardown();
}
