/*!
  @file lte_rrc_csp_intralte_redirection_failure_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Send Connection release with intraLTE frequency redirection present.
  Aquisition is started. Send Acq failure.
  Check service_ind not_available to NAS.
...
05/27/09   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIntraLteRedirectionFailureTest);

void LteRrcCspIntraLteRedirectionFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIntraLteRedirectionFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_start_internal_cell_selection_reqi_s *internal_reqi_ptr;
  ...
  bs_req_ptr = (lte_cphy_band_scan_req_s *) &(cfg_req_ptr->cfg_info[0].cfg_u.band_scan_cfg);		   
  ...
  lte_rrc_csp_test_stub_controller_set_state(LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  TF_MSG("Checking for INITIAL state");
  //TF_ASSERT(stm_get_state(LTE_RRC_CSP_SM) ==    LTE_RRC_CSP_SM__INITIAL);
  printf ("\nstm_get_state(LTE_RRC_CSP_SM) [%d]    LTE_RRC_CSP_SM__INITIAL[%d]\n",stm_get_state(LTE_RRC_CSP_SM) ,   LTE_RRC_CSP_SM__INITIAL);
  ...
  TF_MSG("Checking for LTE_RRC_START_INTERNAL_CELL_SELECTION_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_start_internal_cell_selection_reqi_s) == buf_len);
  internal_reqi_ptr = (lte_rrc_start_internal_cell_selection_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_START_INTERNAL_CELL_SELECTION_REQI == internal_reqi_ptr->msg_hdr.id);
...
void LteRrcCspIntraLteRedirectionFailureTest::Teardown()
{
