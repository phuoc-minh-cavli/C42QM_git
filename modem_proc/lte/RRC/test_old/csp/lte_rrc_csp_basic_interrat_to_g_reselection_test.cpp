/*!
  @file lte_rrc_csp_basic_interrat_to_g_reselection_test.cpp
  ...
  Scan is started. After camping, reselection to W is triggered and is
  successful.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicInterratToGReSelectionTest);

void LteRrcCspBasicInterratToGReSelectionTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicInterratToGReSelectionTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  svc_req.rat_pri_list.priority_list_info[1].acq_sys_mode = SYS_SYS_MODE_GSM;
  svc_req.rat_pri_list.priority_list_info[1].band_cap.lte_band_cap.bits_1_64 = 0;
  svc_req.rat_pri_list.priority_list_info[1].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_GSM_DCS_1800;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  resel_ind.target_cell_info.rat_type = LTE_RAT_GERAN;
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_G_RESEL_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  ...
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_G_RESEL_REQI == 
...
void LteRrcCspBasicInterratToGReSelectionTest::Teardown()
{
