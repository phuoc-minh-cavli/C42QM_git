/*!
  @file lte_rrc_csp_basic_interrat_to_1x_reselection_failure_test.cpp
  ...
  Scan is started. After camping, reselection to 1x is triggered and fails.
...
TF_DEFINE_TEST_CASE(COMPONENT,LteRrcCspBasicInterratTo1xReSelectionFailureTest);

void LteRrcCspBasicInterratTo1xReSelectionFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicInterratTo1xReSelectionFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  resel_ind.target_cell_info.rat_type = LTE_RAT_CDMA1XRTT;
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_1X_RESEL_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  ...
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_1X_RESEL_REQI == 
  ...
  TF_MSG("Sending IRAT Reselection Failed Ind");
  lte_rrc_init_default_hdr(&resel_failed_indi.msg_hdr,
                           LTE_RRC_IRAT_FROM_LTE_RESEL_FAILED_INDI);
  resel_failed_indi.fail_cause = LTE_RRC_IRAT_FROM_LTE_RESEL_FAILURE_CAUSE_UNKNOWN;
...
void LteRrcCspBasicInterratTo1xReSelectionFailureTest::Teardown()
{
