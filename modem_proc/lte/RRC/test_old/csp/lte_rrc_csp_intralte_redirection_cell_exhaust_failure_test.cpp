/*!
  @file lte_rrc_csp_intralte_redirection_cell_exhaust_failure_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Send Connection release with intraLTE frequency redirection present.
  System aquisition is started. Send Acq success.
  Force the SIB1 plmn mismatch. 
  System aquisition starts again and is successful (plmn match).
  Follow basic camping steps.
  Check service_ind available to NAS.
...
06/03/09   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIntraLteRedirectionCellExhaustFailureTest);

void LteRrcCspIntraLteRedirectionCellExhaustFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIntraLteRedirectionCellExhaustFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  svc_req.ehplmn_list.num_plmns = 2;
  ...
  svc_req.ehplmn_list.plmn[1].mcc[0] = 25;
  svc_req.ehplmn_list.plmn[1].mcc[1] = 26;
  svc_req.ehplmn_list.plmn[1].mcc[2] = 27;
  svc_req.ehplmn_list.plmn[1].num_mnc_digits = 3;
  svc_req.ehplmn_list.plmn[1].mnc[0] = 28;
  svc_req.ehplmn_list.plmn[1].mnc[1] = 29;
  svc_req.ehplmn_list.plmn[1].mnc[2] = 30;
  memset(&svc_req.forbidden_ta_list,0,
         sizeof(svc_req.forbidden_ta_list));
  svc_req.forbidden_ta_list.num_entries = 2;
  svc_req.forbidden_ta_list.tracking_area[0].tac = 5;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[0] = 31;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[1] = 32;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[2] = 33;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.num_mnc_digits = 3;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[0] = 34;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[1] = 35;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[2] = 36;
  svc_req.forbidden_ta_list.tracking_area[1].tac = 6;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mcc[0] = 37;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mcc[1] = 38;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mcc[2] = 39;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.num_mnc_digits = 3;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mnc[0] = 40;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mnc[1] = 41;
  svc_req.forbidden_ta_list.tracking_area[1].plmn.mnc[2] = 42;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  connection_released_indi.redir_info.rat_type = LTE_RRC_REDIR_RAT_LTE;
  connection_released_indi.redir_info.dl_arfcn = 20;
  connection_released_indi.redir_info.band = SYS_BAND_CLASS_NONE;
  ...
  printf("\nLTE_CPHY_ACQ_REQ[%d] acq_req_ptr->msgr_hdr.id[%d]\n",LTE_CPHY_ACQ_REQ , acq_req_ptr->msgr_hdr.id);
  TF_ASSERT(LTE_CPHY_ACQ_REQ == acq_req_ptr->msgr_hdr.id);
  TF_ASSERT(3 == acq_req_ptr->trans_id);
  TF_ASSERT(20 == acq_req_ptr->earfcn);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == acq_req_ptr->band);
  TF_ASSERT(0 == acq_req_ptr->num_black_cells);
  TF_ASSERT(LTE_CPHY_PARTIAL_ROAMING_ALLOW_TARGET_FREQ ==
			  acq_req_ptr->restriction_cfg);
  ...
  acq_cnf.acquired_cell.phy_cell_id = 2;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 20;
  ...
  //------------- Aquisition Attepmt 2 --------------------------

  cfg_req_param.trans_id = 4;
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  cfg_req_param.earfcn = 20;
  cfg_req_param.bandwidth = LTE_BW_NRB_6;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  get_sib_cnf.cell_info.sib_db_key = 3;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
  TF_ASSERT(20 == cell_sel_req_ptr->earfcn);
...
void LteRrcCspIntraLteRedirectionCellExhaustFailureTest::Teardown()
{
