/*!
  @file lte_rrc_csp_basic_interrat_to_W_reselection_failure_test.cpp
  ...
  Scan is started. After camping, reselection to W is triggered and fails.
...
{ 
  ...
  LTE_CPHY_CELL_RESEL_CANCEL_REQ
};
...
static lte_rrc_irat_from_lte_resel_failed_indi_s resel_failed_indi;
...
TF_DEFINE_TEST_CASE(COMPONENT,LteRrcCspBasicInterratToWReSelectionFailureTest);

void LteRrcCspBasicInterratToWReSelectionFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,7);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicInterratToWReSelectionFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_irat_from_lte_resel_reqi_s *irat_resel_reqi_ptr;
  lte_cphy_cell_resel_cancel_req_s *resel_cancel_req_ptr;
  lte_rrc_cell_resel_canceled_indi_s *resel_canceled_ind_ptr;
  ...
  svc_req.rat_pri_list.num_items = 3;
  ...
  svc_req.rat_pri_list.priority_list_info[1].acq_sys_mode = SYS_SYS_MODE_CDMA;
  svc_req.rat_pri_list.priority_list_info[1].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_BC0_A;
  svc_req.rat_pri_list.priority_list_info[1].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_BC1;
    svc_req.rat_pri_list.priority_list_info[2].acq_sys_mode = SYS_SYS_MODE_WCDMA;
  svc_req.rat_pri_list.priority_list_info[2].band_cap.lte_band_cap.bits_1_64 |=  SYS_BAND_MASK_WCDMA_V_850;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  TF_MSG("Sending LTE_CPHY_CELL_RESEL_IND, resel to UTRAN");
  msgr_init_hdr(&resel_ind.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_CELL_RESEL_IND);
  resel_ind.serv_cell_info.dl_cell_frequency = 1;
  resel_ind.serv_cell_info.phy_cell_id = 1;
  resel_ind.target_cell_info.rat_type = LTE_RAT_UTRAN;
  memset(&resel_ind.target_cell_info.rat_cell_info,0,
         sizeof(resel_ind.target_cell_info.rat_cell_info));
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_W_RESEL_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_irat_from_lte_resel_reqi_s) == buf_len);
  irat_resel_reqi_ptr = (lte_rrc_irat_from_lte_resel_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_W_RESEL_REQI == 
            irat_resel_reqi_ptr->msg_hdr.id);
  ...
  TF_MSG("Sending IRAT Reselection Failed Ind with Cell Barred and Override bar time");
  lte_rrc_init_default_hdr(&resel_failed_indi.msg_hdr,
                           LTE_RRC_IRAT_FROM_LTE_RESEL_FAILED_INDI);
  resel_failed_indi.fail_cause = LTE_RRC_IRAT_FROM_LTE_RESEL_FAILURE_CAUSE_CELL_BARRED;
  resel_failed_indi.override_bar_time = TRUE;
  resel_failed_indi.bar_time = 1500;
  lte_rrc_utf_send_msg((byte*)&resel_failed_indi, sizeof(resel_failed_indi));
  ...
  TF_ASSERT(LTE_RAT_UTRAN == cphy_cell_bar_req_ptr->bar_info.cell_info_list[0].rat_type );
  TF_ASSERT(1500 == cphy_cell_bar_req_ptr->bar_info.t_barred );

  TF_MSG("Checking for LTE_CPHY_CELL_RESEL_CANCEL_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_cell_resel_cancel_req_s) == buf_len);
  resel_cancel_req_ptr = (lte_cphy_cell_resel_cancel_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_CELL_RESEL_CANCEL_REQ == 
            resel_cancel_req_ptr->msgr_hdr.id);
  ...
  TF_MSG("Checking for LTE_RRC_CELL_RESEL_CANCELED_INDI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cell_resel_canceled_indi_s) == buf_len);
  resel_canceled_ind_ptr = (lte_rrc_cell_resel_canceled_indi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CELL_RESEL_CANCELED_INDI == 
            resel_canceled_ind_ptr->msg_hdr.id);
...
void LteRrcCspBasicInterratToWReSelectionFailureTest::Teardown()
{
