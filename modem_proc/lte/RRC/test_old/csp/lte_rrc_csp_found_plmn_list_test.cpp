/*!
  @file lte_rrc_csp_found_plmn_test.cpp

  @brief
  Unit test code for checking if failed cells make it to found PLMN list.
  ...
  logic. This happens a second time and then no service indication is sent to
  NAS with the found PLMN ID.
...
10/24/08   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspFoundPlmnTest);

void LteRrcCspFoundPlmnTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspFoundPlmnTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspFoundPlmnTest::Teardown()
{
