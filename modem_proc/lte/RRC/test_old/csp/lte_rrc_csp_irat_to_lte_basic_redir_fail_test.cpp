/*!
  @file lte_rrc_csp_irat_to_lte_basic_redir_fail_test.cpp
  ...
  LTE redirection request comes in and cell selection proceeds. Cell is found
  to be barred.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIratToLteBasicRedirFailTest);

void LteRrcCspIratToLteBasicRedirFailTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,2);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIratToLteBasicRedirFailTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspIratToLteBasicRedirFailTest::Teardown()
{
