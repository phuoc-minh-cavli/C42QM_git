/*!
  @file lte_rrc_csp_acq_db_lru_test.cpp

  @brief
  Unit test code for taking CSP to camped state and starting reselection.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. After camping, reselection is triggered and camping occurs
  again. This is repeated till the acq db is full and the LRU entry is replaced
...
02/17/09   vatsac Initial Revision
...
static lte_cphy_cell_resel_ind_s resel_ind;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspAcqDbLruTest);

void LteRrcCspAcqDbLruTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspAcqDbLruTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_cell_resel_started_indi_s *resel_started_ind_ptr;
  uint16 iter;
  uint16 lru_index;
  ...
  validate_system_scan_req[0].earfcn = -60;
  ...
  TF_ASSERT(stm_get_state(LTE_RRC_CSP_SM) == LTE_RRC_CSP_SM__CAMPED);

  lte_rrc_csp_test_stub_controller_set_state(
    LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  for (iter = 0; iter < LTE_RRC_CSP_MAX_ACQ_DB_ENTRIES; iter++)
  {
      TF_MSG("Sending LTE_CPHY_CELL_RESEL_IND");
      msgr_init_hdr(&resel_ind.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_CELL_RESEL_IND);
      resel_ind.serv_cell_info.dl_cell_frequency = iter+1;
      resel_ind.serv_cell_info.phy_cell_id = 1;
      resel_ind.target_cell_info.rat_type = LTE_RAT_EUTRAN;
      resel_ind.target_cell_info.rat_cell_info.lte_cell_info.dl_cell_frequency = 
        iter+2;
      resel_ind.target_cell_info.rat_cell_info.lte_cell_info.phy_cell_id = 1;
      ...
      TF_MSG("Checking for LTE_RRC_CELL_RESEL_STARTED_INDI");
      lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
      TF_ASSERT(sizeof(lte_rrc_cell_resel_started_indi_s) == buf_len);
      resel_started_ind_ptr = (lte_rrc_cell_resel_started_indi_s *)(void*) buf_ptr;
      TF_ASSERT(LTE_RRC_CELL_RESEL_STARTED_INDI == 
                resel_started_ind_ptr->msg_hdr.id);

      lte_rrc_utf_wait_for_done();

      TF_MSG("Checking for WT_SIB_CNF state");
      TF_ASSERT(stm_get_state(LTE_RRC_CSP_SM) == LTE_RRC_CSP_SM__WT_SIB_CNF);

      TF_MSG("Sending Get Sibs Cnf");
      lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
      get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
      get_sib_cnf.cell_info.cell_id.freq = iter+2;
      get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
      get_sib_cnf.cell_info.sib_db_key = iter+2;
      lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
      ...
      TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
      TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
      TF_ASSERT(iter+2 == cell_sel_req_ptr->earfcn);
      TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
      ...
      TF_MSG("Sending LTE_CPHY_CELL_SELECT_CNF");
      msgr_init_hdr(&cell_sel_cnf.msgr_hdr,MSGR_LTE_CPHY,
                    LTE_CPHY_CELL_SELECT_CNF);
      cell_sel_cnf.status = E_SUCCESS;
      cell_sel_cnf.failure_reason = 0;
      cell_sel_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND1;
      cell_sel_cnf.earfcn = iter+2;
      cell_sel_cnf.phy_cell_id = 1;
	  cell_sel_cnf.rsrp = 0;
      lte_rrc_utf_send_msg((byte*)&cell_sel_cnf, sizeof(cell_sel_cnf));
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  lru_index = iter % LTE_RRC_CSP_MAX_ACQ_DB_ENTRIES;

  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].band == 
            SYS_SBAND_LTE_EUTRAN_BAND1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].bandwidth == LTE_BW_NRB_6);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].dl_earfcn == iter+1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].phy_cell_id == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].cell_identity == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].tac == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.num_plmns == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mcc[0] == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mcc[1] == 2);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mcc[2] == 3);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].num_mnc_digits ==
             3);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mnc[0] == 4);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mnc[1] == 5);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].plmn_list.plmn[0].mnc[2] == 6);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].sib_db_key == iter+1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mcc[0] == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mcc[1] == 2);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mcc[2] == 3);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.num_mnc_digits == 3);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mnc[0] == 4);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mnc[1] == 5);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn.mnc[2] == 6);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lru_index].selected_plmn_index == 0);
...
void LteRrcCspAcqDbLruTest::Teardown()
{
