/*!
  @file lte_rrc_csp_iterative_band_scan_exhaust_freq_test.cpp
  ...
  Scan returns two frequencies. The timer is made to expire before cell select
  cnf. Then another Service Request is sent with use_timer = TRUE and 
...
--------   ---     ----------------------------------------------------------
06/16/11   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIterativeBandScanExhaustFreqTest);

void LteRrcCspIterativeBandScanExhaustFreqTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIterativeBandScanExhaustFreqTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  //lte_rrc_csp_set_iterative_band_scan_enabled(TRUE);
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  svc_req.rat_pri_list.priority_list_info[0].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_LTE_BAND2;
  ...
  bs_cnf.band_ptr[0].num_candidates = 3;
  ...
  bs_cnf.band_ptr[0].candidates[2].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  bs_cnf.band_ptr[0].candidates[2].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[0].candidates[2].system.earfcn = 3;
  bs_cnf.band_ptr[0].candidates[2].energy = -60;
  ...
  TF_MSG("Sending LTE_RRC_SIB_READ_ERROR_INDI");
  lte_rrc_init_default_hdr(&sib_read_error_ind.msg_hdr,
                           LTE_RRC_SIB_READ_ERROR_INDI);
  sib_read_error_ind.cell_id.freq = 1;
  sib_read_error_ind.cell_id.phy_cell_id = 1;
  sib_read_error_ind.csg_indication = FALSE;
  sib_read_error_ind.intra_freq_resel_allowed = TRUE;
  sib_read_error_ind.cause = LTE_RRC_SIB_READ_ERROR_CAUSE_SIB_TIMEOUT;
  lte_rrc_utf_send_msg((byte*)&sib_read_error_ind, sizeof(sib_read_error_ind));
...
lte_rrc_csp_test_handle_rrc_cfg_req (&cfg_req_param, validate_system_scan_req,&send_band_scan_cnf);
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  for (i = 0; i < 4; i++)
  {
      ...
      TF_ASSERT(5+i == acq_req_ptr->trans_id);
      TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == acq_req_ptr->band);
      TF_ASSERT(2 == acq_req_ptr->earfcn);
      TF_ASSERT(1+i == acq_req_ptr->num_black_cells);
      ...
      acq_cnf.trans_id = 5+i;
      acq_cnf.status = E_SUCCESS;
      acq_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND40;
      acq_cnf.earfcn = 2;
      ...
      acq_cnf.acquired_cell.phy_cell_id = 2+i;
      ...
      get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
      get_sib_cnf.cell_info.cell_id.freq = 2;
      get_sib_cnf.cell_info.cell_id.phy_cell_id = 2+i;
      get_sib_cnf.cell_info.sib_db_key = 1;
      lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
	  ...
	  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
	  TF_ASSERT(2 == cell_sel_req_ptr->earfcn);
	  TF_ASSERT(2+i == cell_sel_req_ptr->phy_cell_id);
  ...
  TF_ASSERT(1 == band_scan_cfg_ptr->req_bands.num_bands);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.band[0]);
  TF_ASSERT(9 == band_scan_cfg_ptr->trans_id);
  TF_ASSERT(2 == band_scan_cfg_ptr->req_bands.unusable_systems.num_systems);
 
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.unusable_systems.system[0].band);
  TF_ASSERT(LTE_BW_UNKNOWN == band_scan_cfg_ptr->req_bands.unusable_systems.system[0].bandwidth);
  TF_ASSERT(1 == band_scan_cfg_ptr->req_bands.unusable_systems.system[0].earfcn);
  
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.unusable_systems.system[1].band);
  TF_ASSERT(LTE_BW_NRB_6 == band_scan_cfg_ptr->req_bands.unusable_systems.system[1].bandwidth);
  TF_ASSERT(2 == band_scan_cfg_ptr->req_bands.unusable_systems.system[1].earfcn);
  ...
  bs_cnf.band_ptr[0].candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  bs_cnf.band_ptr[0].candidates[0].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[0].candidates[0].system.earfcn = 3;
  bs_cnf.band_ptr[0].candidates[0].energy = -60;
  bs_cnf.band_ptr[0].candidates[1].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  bs_cnf.band_ptr[0].candidates[1].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[0].candidates[1].system.earfcn = 4;
  bs_cnf.band_ptr[0].candidates[1].energy = -60;
  bs_cnf.band_ptr[0].candidates[2].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  bs_cnf.band_ptr[0].candidates[2].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[0].candidates[2].system.earfcn = 5;
  bs_cnf.band_ptr[0].candidates[2].energy = -60;
  lte_rrc_utf_send_msg((byte*)&bs_cnf, sizeof(bs_cnf));


  
  cfg_req_param.trans_id = 10;
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  cfg_req_param.earfcn = 3;
  cfg_req_param.bandwidth = LTE_BW_NRB_6;
  cfg_req_param.phy_cell_id = 1;
  cfg_req_param.num_black_cells = 0;
  cfg_req_param.energy = -60;
  ...
  cfg_req_param.trans_id = 11;
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  cfg_req_param.earfcn = 4;
  cfg_req_param.bandwidth = LTE_BW_NRB_6;
  cfg_req_param.phy_cell_id = 1;
  cfg_req_param.num_black_cells = 0;
  cfg_req_param.energy = -60;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 4;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
  TF_ASSERT(4 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
  ...
  TF_ASSERT(1 == band_scan_cfg_ptr->req_bands.num_bands);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.band[0]);
  TF_ASSERT(13 == band_scan_cfg_ptr->trans_id);
  TF_ASSERT(4 == band_scan_cfg_ptr->req_bands.unusable_systems.num_systems);
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.unusable_systems.system[2].band);
  TF_ASSERT(LTE_BW_UNKNOWN == band_scan_cfg_ptr->req_bands.unusable_systems.system[2].bandwidth);
  TF_ASSERT(3 == band_scan_cfg_ptr->req_bands.unusable_systems.system[2].earfcn);
 
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.unusable_systems.system[3].band);
  TF_ASSERT(LTE_BW_NRB_6 == band_scan_cfg_ptr->req_bands.unusable_systems.system[3].bandwidth);
  TF_ASSERT(4 == band_scan_cfg_ptr->req_bands.unusable_systems.system[3].earfcn);
  ...
  bs_cnf.num_bands = 1;
  bs_cnf.band_ptr[0].band = SYS_SBAND_LTE_EUTRAN_BAND1;
  bs_cnf.band_ptr[0].num_candidates = 0;
  lte_rrc_utf_send_msg((byte*)&bs_cnf, sizeof(bs_cnf));
  ...
  TF_ASSERT(1 == cfg_req_ptr->num_cfg);
   ...
   TF_ASSERT(LTE_CPHY_BAND_SCAN_REQ == band_scan_cfg_ptr->msgr_hdr.id);
   TF_ASSERT(1 == band_scan_cfg_ptr->req_bands.num_bands);
   TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == band_scan_cfg_ptr->req_bands.band[0]);
   TF_ASSERT(14 == band_scan_cfg_ptr->trans_id);
   TF_ASSERT(0 == band_scan_cfg_ptr->req_bands.unusable_systems.num_systems);
  ...
  bs_cnf.num_bands = 1;
  bs_cnf.band_ptr[0].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  bs_cnf.band_ptr[0].num_candidates = 1;
  bs_cnf.band_ptr[0].candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  bs_cnf.band_ptr[0].candidates[0].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[0].candidates[0].system.earfcn = 601;
  bs_cnf.band_ptr[0].candidates[0].energy = -60;
  lte_rrc_utf_send_msg((byte*)&bs_cnf, sizeof(bs_cnf));


  cfg_req_param.trans_id = 15;
...
void LteRrcCspIterativeBandScanExhaustFreqTest::Teardown()
{
