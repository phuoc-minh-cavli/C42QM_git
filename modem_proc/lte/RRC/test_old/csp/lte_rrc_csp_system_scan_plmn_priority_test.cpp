/*!
  @file lte_rrc_csp_system_scan_plmn_priority_test.cpp

  @brief
  Controller is in Inactive state and there are 2 entries in the Acq DB. 
  THe second entry in acq_db has the same plmn as in service_req. System
  Scan is started. The system_scan_cnf is sent for both entries present
  in acq_db. THe entries in acq_db are reordered and acq_req is received
  for the entry that was originally second in acq_db. PLMN match succeeds
  and successful camping occurs.
...
06/30/09   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspSystemScanPlmnPriorityTest);

void LteRrcCspSystemScanPlmnPriorityTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspSystemScanPlmnPriorityTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[1].plmn_list.plmn[0].mcc[0] = 9; // plmn diff from NAS svc_req
  ...
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lte_rrc_csp.sd_ptr->acq_db.acq_db_index[0]].dl_earfcn == 1);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lte_rrc_csp.sd_ptr->acq_db.acq_db_index[1]].dl_earfcn == 2);
  ...
  // Acq DB reordered
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lte_rrc_csp.sd_ptr->acq_db.acq_db_index[0]].dl_earfcn == 2);
  TF_ASSERT(lte_rrc_csp.sd_ptr->acq_db.db[lte_rrc_csp.sd_ptr->acq_db.acq_db_index[1]].dl_earfcn == 1);
...
void LteRrcCspSystemScanPlmnPriorityTest::Teardown()
{
