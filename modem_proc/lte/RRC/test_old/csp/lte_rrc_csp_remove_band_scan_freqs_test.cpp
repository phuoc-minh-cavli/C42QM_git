/*!
  @file lte_rrc_csp_remove_band_scan_freqs_test.cpp

  @brief
  Controller is in Inactive state and there are 2 entries in the Acq DB. System
  Scan is started. Then cell SIB1 PLMN match fails. Also tests blacklist
  logic. Exhaust the systems scans by failing 4 times. CSP sends band scan req 
  to CPHY, which returns 3 candidates, of which first one has the same earfcn
  as an acq db entry, and is removed. PLMN now matches for the next candidate,
  the band scan succeeds and CSP goes to CAMPED state.
...
05/27/09   sureshs  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspRemoveBandScanFreqsTest);

void LteRrcCspRemoveBandScanFreqsTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspRemoveBandScanFreqsTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  send_band_scan_cnf.unusable_systems.num_systems = 2;
  ...
  send_band_scan_cnf.unusable_systems.system[1].band = SYS_SBAND_LTE_EUTRAN_BAND1 ;
  send_band_scan_cnf.unusable_systems.system[1].bandwidth= LTE_BW_NRB_6;
  send_band_scan_cnf.unusable_systems.system[1].earfcn= 2;
  ...
  send_band_scan_cnf.candidates[1].system.earfcn = 4;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 3;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
  TF_ASSERT(3 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
...
void LteRrcCspRemoveBandScanFreqsTest::Teardown()
{
