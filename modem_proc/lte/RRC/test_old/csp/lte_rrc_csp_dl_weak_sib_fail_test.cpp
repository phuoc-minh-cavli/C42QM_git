/*!
  @file lte_rrc_csp_dl_weak_sib_fail_test.cpp

  @brief
  Unit test code for cell barring.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. Then SIB reading fails due to DL Weak ind. Also test 
  blacklist logic. This happens a second time but now SIB reading succeeds, 
  and we camp succesfully.
...
11/10/08   sureshs Initial Revision
...
static lte_cphy_dl_weak_ind_s dl_weak_ind;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspDlWeakSibFailTest);

void LteRrcCspDlWeakSibFailTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspDlWeakSibFailTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG("Sending DL Weak indication");
  msgr_init_hdr(&dl_weak_ind.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_DL_WEAK_IND);
  dl_weak_ind.freq = 1;
  dl_weak_ind.phy_cell_id = 1;
  lte_rrc_utf_send_msg((byte*)&dl_weak_ind, sizeof(dl_weak_ind));
  ...
  TF_ASSERT(1 == acq_req_ptr->num_black_cells);
...
void LteRrcCspDlWeakSibFailTest::Teardown()
{
