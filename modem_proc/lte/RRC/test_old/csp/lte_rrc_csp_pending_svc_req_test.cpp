/*!
  @file lte_rrc_csp_pending_svc_req_test.cpp

  @brief
  Unit test code for taking CSP to camped state with pending Svc request.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. Then a new Svc request is injected, and it is checked that
  SM goes back to INITIAL state and processes the request from there.
...
11/18/08   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspPendingSvcReqTest);

void LteRrcCspPendingSvcReqTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspPendingSvcReqTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG("Setting Controller state to Idle Not Camped");
  ...
  TF_MSG("Sending Service Req again");
  msgr_init_hdr(&svc_req.msg_hdr,MSGR_NAS_EMM,LTE_RRC_SERVICE_REQ);
  svc_req.trans_id = 2;
  svc_req.network_select_mode = SYS_NETWORK_SELECTION_MODE_AUTOMATIC;
  svc_req.csg_id = SYS_CSG_ID_INVALID;  svc_req.req_plmn_info_is_valid = TRUE;
  ...
  TF_ASSERT(1 == band_scan_cfg_ptr->req_bands.num_bands);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == band_scan_cfg_ptr->req_bands.band[0]);
  TF_ASSERT(2 == band_scan_cfg_ptr->trans_id);
 ...
 {
   bs_cnf.band_ptr[0].candidates[index].system.band 	 = send_band_scan_cnf.candidates[index].system.band;//band;//SYS_SBAND_LTE_EUTRAN_BAND1;
   bs_cnf.band_ptr[0].candidates[index].system.bandwidth = send_band_scan_cnf.candidates[index].system.bandwidth;//LTE_BW_NRB_6;
   bs_cnf.band_ptr[0].candidates[index].system.earfcn	 = send_band_scan_cnf.candidates[index].system.earfcn;//10;
   bs_cnf.band_ptr[0].candidates[index].energy			 = send_band_scan_cnf.candidates[index].energy;//-60;
}
...
void LteRrcCspPendingSvcReqTest::Teardown()
{
