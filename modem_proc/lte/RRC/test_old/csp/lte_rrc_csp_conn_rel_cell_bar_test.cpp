/*!
  @file lte_rrc_csp_conn_rel_cell_bar_test.cpp

  @brief
  Unit test code for taking CSP to camped state and then Connection Closed 
  indication has cell barring required set.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspConnRelCellBarTest);

void LteRrcCspConnRelCellBarTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspConnRelCellBarTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  conn_rel_ind.cell_barring_is_reqd = TRUE;
...
void LteRrcCspConnRelCellBarTest::Teardown()
{
