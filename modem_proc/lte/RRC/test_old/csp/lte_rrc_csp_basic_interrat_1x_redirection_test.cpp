/*!
  @file lte_rrc_csp_basic_interrat_1x_redirection_test.cpp
  ...
  message for 1X is sent. The resulting no_service_ind is verified for 
  redirection information and neighbor list.
...
09/21/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicInterRat1xRedirectionTest);

void LteRrcCspBasicInterRat1xRedirectionTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicInterRat1xRedirectionTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  connection_released_indi.redir_info.rat_type = 
    LTE_RRC_REDIR_RAT_CDMA2000_1xRTT;
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_1X_REDIR_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  ...
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_1X_REDIR_REQI ==irat_redir_reqi_ptr->msg_hdr.id);
  TF_ASSERT(LTE_RRC_REDIR_RAT_CDMA2000_1xRTT == 
            irat_redir_reqi_ptr->redir_info.rat_type);
...
void LteRrcCspBasicInterRat1xRedirectionTest::Teardown()
{
