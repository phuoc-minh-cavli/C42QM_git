/*!
  @file lte_rrc_csp_handover_stress_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. After camping, handover related messages are injected
  5 times. The connection is then released.
...
02/19/09   vatsac  Initial Revision
...
static lte_rrc_serving_cell_changed_indi_s serving_cell_changed_indi;
static lte_rrc_handover_completed_ind_s handover_completed_ind;
static lte_rrc_get_sibs_cnfi_s get_sibs_cnfi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspHandoverStressTest);

void LteRrcCspHandoverStressTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspHandoverStressTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  {
      TF_MSG("Sending Serving Cell Changed Indication");
      lte_rrc_init_default_hdr(&serving_cell_changed_indi.msg_hdr, 
                               LTE_RRC_SERVING_CELL_CHANGED_INDI);
      serving_cell_changed_indi.serving_cell_id.phy_cell_id = iter + 2;
      serving_cell_changed_indi.serving_cell_id.freq = 1;
      lte_rrc_utf_send_msg((byte*)&serving_cell_changed_indi,
                           sizeof(serving_cell_changed_indi));
      ...
      handover_completed_ind.phy_cell_id = iter + 2;
      ...
      TF_MSG("Checking for LTE_RRC_GET_SIBS_REQ");
      lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
      TF_ASSERT(sizeof(lte_rrc_get_sibs_reqi_s) == buf_len);
      get_sibs_reqi_ptr = (lte_rrc_get_sibs_reqi_s *)(void*) buf_ptr;
      TF_ASSERT(LTE_RRC_GET_SIBS_REQI == get_sibs_reqi_ptr->msg_hdr.id);

      TF_MSG("Sending Get SIBs CNFI");
      lte_rrc_init_default_hdr(&get_sibs_cnfi.msg_hdr, LTE_RRC_GET_SIBS_CNFI);
      get_sibs_cnfi.status = LTE_RRC_GET_SIBS_SUCCESS;
      get_sibs_cnfi.cell_info.cell_id.freq = 1;
      get_sibs_cnfi.cell_info.cell_id.phy_cell_id = iter + 2;
      get_sibs_cnfi.cell_info.sib_db_key = iter + 2;
      lte_rrc_utf_send_msg((byte*)&get_sibs_cnfi, sizeof(lte_rrc_get_sibs_cnfi_s));
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
      ...
      TF_ASSERT(iter+2 == svc_ind_ptr->camped_svc_info.phy_cell_id);
      TF_ASSERT(1 == svc_ind_ptr->camped_svc_info.earfcn);
  ...
  conn_rel_ind.rel_reason = LTE_RRC_CONN_REL_NORMAL;
  conn_rel_ind.cell_barring_is_reqd = FALSE;
  conn_rel_ind.earfcn = 0;
  conn_rel_ind.phy_cell_id = 0;
  ...
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 1;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = iter + 1;
  get_sib_cnf.cell_info.sib_db_key = iter + 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
  TF_ASSERT(1 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(iter + 1 == cell_sel_req_ptr->phy_cell_id);
  ...
  cell_sel_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  cell_sel_cnf.earfcn = 1;
  cell_sel_cnf.phy_cell_id = iter + 1;
  cell_sel_cnf.rsrp = 0;
  lte_rrc_utf_send_msg((byte*)&cell_sel_cnf, sizeof(cell_sel_cnf));
...
void LteRrcCspHandoverStressTest::Teardown()
{
