/*!
  @file lte_rrc_csp_system_scan_band_scan_test.cpp
  ...
  to CPHY, which returns 2 candidates. PLMN now matches for the first candidate,
...
02/10/09   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspSystemScanBandScanTest);

void LteRrcCspSystemScanBandScanTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspSystemScanBandScanTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  cfg_req_param.num_black_cells = 0;
  cfg_req_param.energy = -60;
  cfg_req_param.num_candidates = 2;
  cfg_req_param.status = E_SUCCESS;
...
void LteRrcCspSystemScanBandScanTest::Teardown()
{
