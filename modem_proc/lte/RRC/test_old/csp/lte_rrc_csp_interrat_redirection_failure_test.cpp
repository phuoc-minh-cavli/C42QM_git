/*!
  @file lte_rrc_csp_interrat_redirection_failure_test.cpp
  ...
  message is sent. The resulting no_service_ind is verified for redirection
  information and neighbor list.
...
08/29/09   vatsac  Initial Revision
...
static lte_rrc_irat_from_lte_redir_failed_indi_s redir_failed_indi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspInterRatRedirectionFailureTest);

void LteRrcCspInterRatRedirectionFailureTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspInterRatRedirectionFailureTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  svc_req.rat_pri_list.priority_list_info[1].acq_sys_mode = SYS_SYS_MODE_HDR;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  connection_released_indi.redir_info.rat_type = LTE_RRC_REDIR_RAT_CDMA2000_HRPD;
  connection_released_indi.redir_info.dl_arfcn = 20;
  connection_released_indi.redir_info.band = SYS_BAND_BC1;
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_DO_REDIR_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  ...
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_DO_REDIR_REQI == irat_redir_reqi_ptr->msg_hdr.id);
  TF_ASSERT(LTE_RRC_REDIR_RAT_CDMA2000_HRPD == irat_redir_reqi_ptr->redir_info.rat_type);
  TF_ASSERT(SYS_BAND_BC1 == irat_redir_reqi_ptr->redir_info.band);
  TF_ASSERT(20 == irat_redir_reqi_ptr->redir_info.dl_arfcn);
  TF_ASSERT(&redir_opt_info == irat_redir_reqi_ptr->redir_info.opt_info_ptr);

  // verify sib8 info
  TF_ASSERT(1 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band_count);
  TF_ASSERT(SYS_BAND_BC0 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].band_class);
  TF_ASSERT(2 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].freq_count);
  TF_ASSERT(1900 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[0].freq);
  TF_ASSERT(2 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[0].cell_count);
  TF_ASSERT(200 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[0].cell[0].id);
  TF_ASSERT(300 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[0].cell[1].id);
  TF_ASSERT(1800 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[1].freq);
  TF_ASSERT(2 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[1].cell_count);
  TF_ASSERT(100 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[1].cell[0].id);
  TF_ASSERT(150 == irat_redir_reqi_ptr->redir_info.opt_info_ptr->rat_u.cdma2000_neighbors.band[0].frequency[1].cell[1].id);
  ...
  TF_MSG("Sending IRAT Redirection failed Indication");
  lte_rrc_init_default_hdr(&redir_failed_indi.msg_hdr, 
                           LTE_RRC_IRAT_FROM_LTE_REDIR_FAILED_INDI);
  lte_rrc_utf_send_msg((byte*)&redir_failed_indi, 
                       sizeof(redir_failed_indi));
  ...
  mode_change_req_ptr = (lte_rrc_mode_change_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_REQI == mode_change_req_ptr->msg_hdr.id);
  TF_ASSERT(LTE_RRC_MODE_CHANGE_DEACTIVATE == mode_change_req_ptr->mode);
  ...
  /* free them here because deinit will happen after this*/
...
void LteRrcCspInterRatRedirectionFailureTest::Teardown()
{
