/*!
  @file lte_rrc_csp_acq_db_efs_test.cpp

  @brief
  Write an acq db entry into EFS. Make sure it is read. Change it, and make
  sure changed version is written back.
...
03/24/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspOverlappingEarfcnRemovalTest);

void LteRrcCspOverlappingEarfcnRemovalTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,7);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspOverlappingEarfcnRemovalTest::Test()
{
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[0].tac = 1;
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[0].plmn_list.plmn[0].mnc[0] = 5;
  lte_rrc_csp.sd_ptr->acq_db.db[0].plmn_list.plmn[0].mnc[1] = 4;
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[0].selected_plmn.mnc[0] = 5;
  lte_rrc_csp.sd_ptr->acq_db.db[0].selected_plmn.mnc[1] = 4;
  lte_rrc_csp.sd_ptr->acq_db.db[0].selected_plmn.mnc[2] = 6;
  lte_rrc_csp.sd_ptr->acq_db.db[0].selected_plmn_index = 0;
  lte_rrc_csp.sd_ptr->acq_db.db[0].csg_id = 0xffffffff;
  ...
  svc_req.rat_pri_list.priority_list_info[0].band_cap.lte_band_cap.bits_1_64 |= (SYS_BAND_MASK_LTE_BAND1 | SYS_BAND_MASK_LTE_BAND4);
  ...
  TF_MSG("Changing field");
  lte_rrc_csp.sd_ptr->acq_db.db[0].bandwidth = LTE_BW_NRB_15;

  TF_ASSERT(efs_get("/nv/reg_files/modem/lte/rrc/csp/acq_db",
                    &lte_rrc_csp.sd_ptr->efs_acq_db,sizeof(lte_rrc_csp.sd_ptr->efs_acq_db)) == 
                    sizeof(lte_rrc_csp.sd_ptr->efs_acq_db));

  TF_ASSERT(lte_rrc_csp.sd_ptr->efs_acq_db.efs_db[0].bandwidth == LTE_BW_NRB_6);

  TF_MSG("End of test");
}

void LteRrcCspOverlappingEarfcnRemovalTest::Teardown()
{
	lte_rrc_csp_test_free_sib1_pdu();
