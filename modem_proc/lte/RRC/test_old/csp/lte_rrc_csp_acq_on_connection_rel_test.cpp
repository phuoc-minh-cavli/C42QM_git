...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspAcqOnConnectionReleasePciDisableTest);

void LteRrcCspAcqOnConnectionReleasePciDisableTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspAcqOnConnectionReleasePciDisableTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp.sd_ptr->pci_lock_enabled = FALSE;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  connection_released_indi.redir_info_is_present = FALSE;
  connection_released_indi.is_acq_needed = TRUE;
  connection_released_indi.acq_cell_info.bandwidth = LTE_BW_NRB_6;
  connection_released_indi.acq_cell_info.dl_cell_frequency = 1;
  connection_released_indi.acq_cell_info.phy_cell_id = 2;
  connection_released_indi.acq_cell_info.ref_time = 0;
...
void LteRrcCspAcqOnConnectionReleasePciDisableTest::Teardown()
{
  lte_rrc_utf_teardown();
}


TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspAcqOnConnectionReleasePciEnableTest);

void LteRrcCspAcqOnConnectionReleasePciEnableTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspAcqOnConnectionReleasePciEnableTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  lte_rrc_csp.sd_ptr->pci_lock_enabled = TRUE;
  lte_rrc_csp.sd_ptr->pci_lock_efs_info.dl_earfcn = 1;
  lte_rrc_csp.sd_ptr->pci_lock_efs_info.pci = 1;
...
void LteRrcCspAcqOnConnectionReleasePciEnableTest::Teardown()
{
  lte_rrc_utf_teardown();
}

  /* This test case makes sure that No Service Ind is sent to NAS,when
     Acq on conn rel is requested with time_out value set to 0 in acq req ,when the
     scan type is INTRA LTE REDIR */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspAcqOnConnectionReleaseNoServiceInd);

void LteRrcCspAcqOnConnectionReleaseNoServiceInd::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspAcqOnConnectionReleaseNoServiceInd::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
   ...
   TF_MSG("Sending Connection Released Indication");
   lte_rrc_init_default_hdr(&connection_released_indi.msg_hdr,LTE_RRC_CONN_RELEASED_INDI);
   connection_released_indi.rel_reason = LTE_RRC_CONN_REL_ABORTED;
   connection_released_indi.cell_barring_is_reqd = FALSE;
   connection_released_indi.earfcn = 1;
   connection_released_indi.phy_cell_id = 1;
   connection_released_indi.is_acq_needed = TRUE;
   connection_released_indi.acq_cell_info.bandwidth = LTE_BW_NRB_6;
   connection_released_indi.acq_cell_info.dl_cell_frequency = 1;
   connection_released_indi.acq_cell_info.phy_cell_id = 2;
   connection_released_indi.acq_cell_info.ref_time = 0;
   lte_rrc_utf_send_msg((byte*)&connection_released_indi, sizeof(connection_released_indi));
  ...
  cfg_req_param.trans_id = 3;
  cfg_req_param.phy_cell_id = 2;
  cfg_req_param.status = E_NOT_AVAILABLE;
  lte_rrc_csp_test_handle_rrc_cfg_req (&cfg_req_param, validate_system_scan_req,&send_band_scan_cnf);
...
void LteRrcCspAcqOnConnectionReleaseNoServiceInd::Teardown()
{
  lte_rrc_utf_teardown();
}
