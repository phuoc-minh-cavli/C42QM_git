/*!
  @file lte_rrc_csp_simple_cell_barring_test.cpp

  @brief
  Unit test code for cell barring.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. Then cell SIB1 indicates it is barred. Also test blacklist
  logic. This happens a second time but now SIB1 indicates cell is not 
  barred, and we camp succesfully.
...
10/31/08   sureshs Added receiving of cell bar request
10/23/08   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspSimpleCellBarringTest);

void LteRrcCspSimpleCellBarringTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspSimpleCellBarringTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspSimpleCellBarringTest::Teardown()
{
