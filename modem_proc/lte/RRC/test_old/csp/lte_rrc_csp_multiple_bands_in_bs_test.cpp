/*!
  @file lte_rrc_csp_multiple_bands_in_bs_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started but since 2 bands are supported, they go out together in
  band scan request.
...
--------   ---     ----------------------------------------------------------
03/10/10   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(NO_RUN, LteRrcCspMultipleBandsInBsTest);

void LteRrcCspMultipleBandsInBsTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspMultipleBandsInBsTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp_test_set_max_bands_bs_req(2);
  ...
  TF_ASSERT(2 == bs_req_ptr->req_bands.num_bands);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == bs_req_ptr->req_bands.band[0]);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == bs_req_ptr->req_bands.band[1]);
  ...
  bs_cnf.band_ptr[1].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  bs_cnf.band_ptr[1].num_candidates = 1;
  bs_cnf.band_ptr[1].candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  bs_cnf.band_ptr[1].candidates[0].system.bandwidth = LTE_BW_NRB_6;
  bs_cnf.band_ptr[1].candidates[0].system.earfcn = 601;
  bs_cnf.band_ptr[1].candidates[0].energy = -60;
...
void LteRrcCspMultipleBandsInBsTest::Teardown()
{
