/*!
  @file lte_rrc_csp_rlf2_test.cpp
  ...
  Connected mode and we recamp. Then CRE fails, connection is closed, and 
  camping occurs on the last cell.
...
08/24/09   sureshs Initial Revision
...
static lte_rrc_conn_released_indi_s conn_rel_ind;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspRlf2Test);

void LteRrcCspRlf2Test::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspRlf2Test::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
#if 0
  TF_MSG("Checking for LTE_CPHY_BAND_SCAN_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_band_scan_req_s) == buf_len);
  bs_req_ptr = (lte_cphy_band_scan_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_BAND_SCAN_REQ == bs_req_ptr->msgr_hdr.id);
  TF_ASSERT(1 == bs_req_ptr->req_bands.num_bands);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == bs_req_ptr->req_bands.band[0]);
  TF_ASSERT(1 == bs_req_ptr->trans_id);
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  lte_rrc_utf_send_msg((byte*)&svc_req, sizeof(svc_req));
#if 0
  TF_MSG("Checking for LTE_CPHY_SYSTEM_SCAN_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_system_scan_req_s) == buf_len);
  ss_req_ptr = (lte_cphy_system_scan_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_SYSTEM_SCAN_REQ == ss_req_ptr->msgr_hdr.id);
  TF_ASSERT(3 == ss_req_ptr->trans_id);
  TF_ASSERT(1 == ss_req_ptr->req_systems.num_systems);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == ss_req_ptr->req_systems.system[0].band);
  TF_ASSERT(LTE_BW_NRB_6 == ss_req_ptr->req_systems.system[0].bandwidth);
  TF_ASSERT(1 == ss_req_ptr->req_systems.system[0].earfcn);

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for WT_SYSTEM_SCAN_CNF state");
  TF_ASSERT(stm_get_state(LTE_RRC_CSP_SM) == 
            LTE_RRC_CSP_SM__WT_SYSTEM_SCAN_CNF);

  TF_MSG("Sending LTE_CPHY_SYSTEM_SCAN_CNF");
  msgr_init_hdr(&ss_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_SYSTEM_SCAN_CNF);
  ss_cnf.trans_id = 3;
  ss_cnf.sys_scan_results.num_systems = 1;
  ss_cnf.sys_scan_results.system[0].energy = -60;
  ss_cnf.sys_scan_results.system[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  ss_cnf.sys_scan_results.system[0].system.bandwidth = LTE_BW_NRB_6;
  ss_cnf.sys_scan_results.system[0].system.earfcn = 1;
  ...
  acq_req_ptr = (lte_cphy_acq_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_ACQ_REQ == acq_req_ptr->msgr_hdr.id);
  TF_ASSERT(4 == acq_req_ptr->trans_id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == acq_req_ptr->band);
  TF_ASSERT(1 == acq_req_ptr->earfcn);
  TF_ASSERT(0 == acq_req_ptr->num_black_cells);
  ...
  lte_rrc_csp_test_stub_controller_set_state(LTE_RRC_CONTROLLER_SM__CLOSING);

  TF_MSG("Sending Connection Released Indication");
  lte_rrc_init_default_hdr(&conn_rel_ind.msg_hdr,LTE_RRC_CONN_RELEASED_INDI);
  conn_rel_ind.rel_reason = LTE_RRC_CONN_REL_CRE_FAILURE;
  conn_rel_ind.cell_barring_is_reqd = FALSE;
  conn_rel_ind.earfcn = 1;
  conn_rel_ind.phy_cell_id = 1;
  lte_rrc_utf_send_msg((byte*)&conn_rel_ind, sizeof(conn_rel_ind));
...
void LteRrcCspRlf2Test::Teardown()
{
