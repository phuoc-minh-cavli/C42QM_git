/*!
  @file lte_rrc_csp_oos_system_scan_timeout_test.cpp

  @brief
  Controller is in Inactive state and there is one entry in the Acq DB. System
  Scan is started. Before System Scan confirmation arrives, OOS timer expires.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspOosSystemScanTimeoutTest);

void LteRrcCspOosSystemScanTimeoutTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,3);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspOosSystemScanTimeoutTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  printf("\nLTE_RRC_CFG_SYSTEM_SCAN == band_scan_cfg_ptr->msgr_hdr.id[%d]\n",LTE_RRC_CFG_SYSTEM_SCAN , band_scan_cfg_ptr->msgr_hdr.id);
  TF_ASSERT(LTE_CPHY_SYSTEM_SCAN_REQ == band_scan_cfg_ptr->msgr_hdr.id);
...
void LteRrcCspOosSystemScanTimeoutTest::Teardown()
{
