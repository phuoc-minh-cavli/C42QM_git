/*!
  @file lte_rrc_csp_band_pref_opt_test.cpp

  @brief
  Controller is in Inactive state and there are 2 entries in the Acq DB, 
  belonging to BAND2. Service request specifies BAND1 and BAND2.
  System Scan is started. Then cell SIB1 PLMN match fails. 
  Also tests blacklist logic. Exhaust the systems scans by failing 4 times. 
  CSP sends band scan req to CPHY on BAND2 due to optimization, which returns 2 
  candidates. PLMN now matches for the first candidate, the band scan succeeds 
  and CSP goes to CAMPED state.
...
05/28/09   sureshs  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBandPrefOptTest);

void LteRrcCspBandPrefOptTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBandPrefOptTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_csp_test_set_max_bands_bs_req(1);
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[1].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  lte_rrc_csp.sd_ptr->acq_db.db[1].bandwidth = LTE_BW_NRB_6;
  lte_rrc_csp.sd_ptr->acq_db.db[1].dl_earfcn = 601;
  ...
  lte_rrc_csp.sd_ptr->acq_db.db[0].dl_earfcn = 602;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  send_band_scan_cnf.candidates[0].system.earfcn = 601;
  
  validate_system_scan_req[0].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  validate_system_scan_req[0].bandwidth = LTE_BW_NRB_6;
  validate_system_scan_req[0].earfcn = 601;

  validate_system_scan_req[1].band = SYS_SBAND_LTE_EUTRAN_BAND2;
  validate_system_scan_req[1].bandwidth = LTE_BW_NRB_6;
  validate_system_scan_req[1].earfcn = 602;
  ...
  get_sib_cnf.cell_info.cell_id.freq = 601;
  ...
  TF_MSG("Check Checking for LTE_CPHY_CELL_SELECT_REQ");
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == cell_sel_req_ptr->band);
  TF_ASSERT(601 == cell_sel_req_ptr->earfcn);
  ...
  cell_sel_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  cell_sel_cnf.earfcn = 601;
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 602;
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == cell_sel_req_ptr->band);
  TF_ASSERT(602 == cell_sel_req_ptr->earfcn);
  ...
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 602;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 4;
  get_sib_cnf.cell_info.sib_db_key = 2;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == cell_sel_req_ptr->band);
  TF_ASSERT(602 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(4 == cell_sel_req_ptr->phy_cell_id);
  ...
  TF_MSG("System Scan over and unsuccessful");
  ...
  send_band_scan_cnf.candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  send_band_scan_cnf.candidates[0].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[0].system.earfcn = 603;
  
  send_band_scan_cnf.candidates[1].energy = -60;
  send_band_scan_cnf.candidates[1].system.band = SYS_SBAND_LTE_EUTRAN_BAND2;
  send_band_scan_cnf.candidates[1].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[1].system.earfcn = 604;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 603;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND2 == cell_sel_req_ptr->band);
  TF_ASSERT(603 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
...
void LteRrcCspBandPrefOptTest::Teardown()
{
