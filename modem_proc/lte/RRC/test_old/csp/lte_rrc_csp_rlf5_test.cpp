/*!
  @file lte_rrc_csp_rlf5_test.cpp
  ...
  Connected mode. Meanwhile, NAS sends Connection Abort and connection is 
  closed. Cell selection continues, but NAS now sends Deactivate with Mode
  Change option.
...
08/31/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspRlf5Test);

void LteRrcCspRlf5Test::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,7);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspRlf5Test::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspRlf5Test::Teardown()
{
