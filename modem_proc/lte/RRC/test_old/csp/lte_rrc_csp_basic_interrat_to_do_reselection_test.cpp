/*!
  @file lte_rrc_csp_basic_interrat_to_do_reselection_test.cpp
  ...
  Scan is started. After camping, reselection to DO is triggered and is
  successful.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicInterratToDoReSelectionTest);

void LteRrcCspBasicInterratToDoReSelectionTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicInterratToDoReSelectionTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  resel_ind.target_cell_info.rat_type = LTE_RAT_CDMAHRPD;
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_FROM_LTE_TO_DO_RESEL_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  ...
  TF_ASSERT(LTE_RRC_IRAT_FROM_LTE_TO_DO_RESEL_REQI == 
...
void LteRrcCspBasicInterratToDoReSelectionTest::Teardown()
{
