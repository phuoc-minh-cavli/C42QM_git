/*!
  @file lte_rrc_csp_simple_rlf_test.cpp

  @brief
  Unit test code for taking CSP to camped state.
  Controller is in Inactive state and there is nothing in the Acq DB. Band
  Scan is started. After camping, Connection Released indication is sent
  indicating RLF as cause.
...
08/26/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspSimpleRlfTest);

void LteRrcCspSimpleRlfTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspSimpleRlfTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspSimpleRlfTest::Teardown()
{
