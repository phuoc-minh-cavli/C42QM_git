/*!
  @file lte_rrc_csp_iterative_mutli_band_scan_test.cpp

  @brief
  NAS Initiates service request with for both FDD and TDD bands.
  FDD band scan request returns three frequencies and 1st candidate
  acquisition fails, 2nd candidate acquisitions fails due to plmn mismatch
  between NAS requsted and SIB parameter.Then CSP sends the 
  another band scan request with 1 and 2 unsuable systems and
  and band scan cnf repsonds with the same 3rd candidate but 3rd candidate aquisition
  fails and the candidates will get exhausted in FDD band and then
  CSP sends the band scan request for the TDD band. The same scenario of FDD 
  will be repeated for first two candidates but second band scan request(iterative)
  will not be initiated from CSP. And cell selection will be successful and UE camps successfully on 
  3rd candidate and CSP sends service indication with status available 
...
--------   ---     ----------------------------------------------------------
08/19/11   gpotnuru  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIterativeMultiBandScanTest);

void LteRrcCspIterativeMultiBandScanTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIterativeMultiBandScanTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  svc_req.rat_pri_list.priority_list_info[0].band_cap.lte_band_cap.bits_1_64 |= SYS_BAND_MASK_LTE_BAND40;
  ...
  cfg_req_param.num_candidates = 3;
  ...
  send_band_scan_cnf.candidates[1].energy = -60;
  send_band_scan_cnf.candidates[1].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  send_band_scan_cnf.candidates[1].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[1].system.earfcn = 2;
  send_band_scan_cnf.candidates[2].energy = -60;
  send_band_scan_cnf.candidates[2].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  send_band_scan_cnf.candidates[2].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[2].system.earfcn = 3;
  ...
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  cfg_req_param.earfcn = 2;
  cfg_req_param.bandwidth = LTE_BW_UNKNOWN;
  cfg_req_param.phy_cell_id = 1;
  ...
  send_band_scan_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  send_band_scan_cnf.num_candidates = 2;

  send_band_scan_cnf.unusable_systems.num_systems = 0;
  send_band_scan_cnf.candidates[0].energy = -60;
  send_band_scan_cnf.candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  send_band_scan_cnf.candidates[0].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[0].system.earfcn = 3;
  ...
  cfg_req_param.num_systems = 2;
  cfg_req_param.band = SYS_SBAND_LTE_EUTRAN_BAND40;
  cfg_req_param.earfcn = 38650;
  cfg_req_param.bandwidth = LTE_BW_NRB_6;
  cfg_req_param.phy_cell_id = 1;
  cfg_req_param.num_black_cells = 0;
  cfg_req_param.energy = -60;
  ...
  send_band_scan_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND1;
  send_band_scan_cnf.num_candidates = 3;

  send_band_scan_cnf.unusable_systems.num_systems = 0;

  send_band_scan_cnf.candidates[0].energy = -60;
  send_band_scan_cnf.candidates[0].system.band = SYS_SBAND_LTE_EUTRAN_BAND40;
  send_band_scan_cnf.candidates[0].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[0].system.earfcn = 38650;

  send_band_scan_cnf.candidates[1].energy = -60;
  send_band_scan_cnf.candidates[1].system.band = SYS_SBAND_LTE_EUTRAN_BAND40;
  send_band_scan_cnf.candidates[1].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[1].system.earfcn = 38651;

  send_band_scan_cnf.candidates[2].energy = -60;
  send_band_scan_cnf.candidates[2].system.band = SYS_SBAND_LTE_EUTRAN_BAND40;
  send_band_scan_cnf.candidates[2].system.bandwidth = LTE_BW_NRB_6;
  send_band_scan_cnf.candidates[2].system.earfcn = 38652;
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 38651;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  cell_sel_req_ptr = (lte_cphy_cell_select_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND40 == cell_sel_req_ptr->band);
  TF_ASSERT(38651 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
  ...
  cell_sel_cnf.status = E_SUCCESS;
  cell_sel_cnf.failure_reason = 0;
  cell_sel_cnf.band = SYS_SBAND_LTE_EUTRAN_BAND40;
  cell_sel_cnf.earfcn = 38651;
  cell_sel_cnf.phy_cell_id = 1;
  cell_sel_cnf.rsrp = 0;
  lte_rrc_utf_send_msg((byte*)&cell_sel_cnf, sizeof(cell_sel_cnf));
...
lte_rrc_csp_test_handle_rrc_cfg_req (&cfg_req_param, validate_system_scan_req,&send_band_scan_cnf);
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  TF_MSG("Checking for LTE_RRC_GET_SIBS_REQI");
	...
	lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
	get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
	get_sib_cnf.cell_info.cell_id.freq = 38652;
	get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
	get_sib_cnf.cell_info.sib_db_key = 1;
	lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(LTE_CPHY_CELL_SELECT_REQ == cell_sel_req_ptr->msgr_hdr.id);
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND40 == cell_sel_req_ptr->band);
  TF_ASSERT(38652 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
...
void LteRrcCspIterativeMultiBandScanTest::Teardown()
{
