/*!
  @file lte_rrc_csp_resel_during_conn_est_test.cpp
  ...
  Scan is started. After camping, controller is moved to Connecting state. 
  Reselection is triggered, but is stalled till RACH is aborted. Then, camping 
  occurs again.
...
03/18/09   sureshs Initial Revision
...
static lte_rrc_conn_establishment_failure_indi_s conn_est_fail_ind;
static lte_rrc_proceed_with_resel_indi_s proc_with_resel_ind;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspReselDuringConnEstTest);

void LteRrcCspReselDuringConnEstTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspReselDuringConnEstTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
lte_rrc_test_integration_verify_req_send_cnf(NULL, LTE_RRC_BARRING_UPDATE_IND, LTE_RRC_TEST_CELL_0, FALSE, NULL);
  ...
  TF_MSG("Setting controller state to Connecting");
  ...
  TF_MSG("Sending LTE_RRC_CONN_ESTABLISHMENT_FAILURE_INDI");
  lte_rrc_init_default_hdr(&conn_est_fail_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_FAILURE_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_fail_ind, sizeof(conn_est_fail_ind));
  ...
  TF_MSG("Sending Proceed with Reselection(true)");
  lte_rrc_init_default_hdr(&proc_with_resel_ind.msg_hdr,
                           LTE_RRC_PROCEED_WITH_RESEL_INDI);
  proc_with_resel_ind.status = TRUE;
  lte_rrc_utf_send_msg((byte*)&proc_with_resel_ind, 
                       sizeof(proc_with_resel_ind));
  ...
  TF_MSG("Setting controller state to Idle Camped");
  lte_rrc_csp_test_stub_controller_set_state(
...
void LteRrcCspReselDuringConnEstTest::Teardown()
{
