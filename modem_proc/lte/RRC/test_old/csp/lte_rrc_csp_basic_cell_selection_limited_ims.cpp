/*!
  @file lte_rrc_csp_basic_cell_selection_limited_ra_test.cpp
...
--------   ---     ----------------------------------------------------------
08/10/11   vatsac  Initial Revision
...
static void send_service_req()
{
	TF_MSG("Sending Service Req");
	msgr_init_hdr(&svc_req.msg_hdr,MSGR_NAS_EMM,LTE_RRC_SERVICE_REQ);
	svc_req.trans_id = 1;
	svc_req.csg_id = SYS_CSG_ID_INVALID;
	svc_req.network_select_mode = SYS_NETWORK_SELECTION_MODE_LIMITED;
	svc_req.req_plmn_info_is_valid = FALSE;
	...
	svc_req.emc_srv_pending = TRUE;
}

/*
Limited mode service request for E911 call
SIB1 supporting IMS
LTE will send service ind with use_oos set to false
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test1);

void LteRrcCspBasicCellSelectionLimitedE911Test1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_barring_update_ind_s *ssac_update_ind_ptr;
  ...
  lte_rrc_csp_test_stub_set_3gpp_release_version(LTE_3GPP_REL9);
  ...
  TF_MSG("Setting PLMN reserved for operator use");
  ...
  TF_MSG("Setting IMS call support to TRUE");
  sib1_pdu_ptr = (lte_rrc_osys_BCCH_DL_SCH_Message*) 
                  lte_rrc_csp_test_get_sib1_pdu_ptr();
  TF_ASSERT(sib1_pdu_ptr != NULL);

  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->cellAccessRelatedInfo.plmn_IdentityList.elem->plmn_Identity.mcc.n = 3;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->m.nonCriticalExtensionPresent = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->nonCriticalExtension.m.nonCriticalExtensionPresent = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->nonCriticalExtension.nonCriticalExtension.m.ims_EmergencySupport_r9Present = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->nonCriticalExtension.nonCriticalExtension.ims_EmergencySupport_r9 = 
                                                              lte_rrc_osys_SystemInformationBlockType1_v920_IEs_ims_EmergencySupport_r9_true;
  ...
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_barring_update_ind_s) == buf_len);
  ssac_update_ind_ptr = (lte_rrc_barring_update_ind_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_BARRING_UPDATE_IND == ssac_update_ind_ptr->msg_hdr.id);
  ...
  TF_ASSERT(LTE_RRC_CELL_ACCESS_ALL_CALLS == svc_ind_ptr->camped_svc_info.cell_access_status);
  TF_ASSERT(FALSE == svc_ind_ptr->use_oos);
...
void LteRrcCspBasicCellSelectionLimitedE911Test1::Teardown()
{
...
/*
Limited mode service request for E911 call
SIB1 not supporting IMS
LTE will send no service ind with use_oos set to false
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test2);

void LteRrcCspBasicCellSelectionLimitedE911Test2::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test2::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->m.nonCriticalExtensionPresent = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->nonCriticalExtension.m.nonCriticalExtensionPresent = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->nonCriticalExtension.nonCriticalExtension.m.ims_EmergencySupport_r9Present = 0;
  ...
  cfg_req_param.num_systems = 0;

  lte_rrc_csp_test_handle_rrc_cfg_req (&cfg_req_param, validate_system_scan_req,&send_band_scan_cnf);

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for LTE_RRC_BARRING_UPDATE_IND");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_barring_update_ind_s) == buf_len);
  ssac_update_ind_ptr = (lte_rrc_barring_update_ind_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_BARRING_UPDATE_IND == ssac_update_ind_ptr->msg_hdr.id);
  ...
  TF_ASSERT(LTE_RRC_CELL_ACCESS_NORMAL_ONLY == svc_ind_ptr->camped_svc_info.cell_access_status);
  TF_ASSERT(FALSE == svc_ind_ptr->use_oos);
...
void LteRrcCspBasicCellSelectionLimitedE911Test2::Teardown()
{
  lte_rrc_utf_teardown();
}
...
Limited mode service request for E911 call
No LTE cell present
LTE will send no service ind with use_oos set to TRUE
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test3);

void LteRrcCspBasicCellSelectionLimitedE911Test3::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test3::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  TF_ASSERT(LTE_RRC_SVC_STATUS_NOT_AVAILABLE == svc_ind_ptr->svc_status);
  TF_ASSERT(LTE_RRC_CELL_ACCESS_NORMAL_ONLY == svc_ind_ptr->camped_svc_info.cell_access_status);
  TF_ASSERT(TRUE == svc_ind_ptr->use_oos);
...
void LteRrcCspBasicCellSelectionLimitedE911Test3::Teardown()
{
  lte_rrc_utf_teardown();
}

/*
Automatic service request with forbidden TA
SIB contain forbidden PLMN 
No found PLMN 
LTE will send no service ind with use_oos set to TRUE
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test4);

void LteRrcCspBasicCellSelectionLimitedE911Test4::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test4::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  svc_req.req_plmn_info_is_valid = TRUE;
  ...
  memset(&svc_req.forbidden_ta_list,0,
         sizeof(svc_req.forbidden_ta_list));
  svc_req.forbidden_ta_list.num_entries= 1;
  svc_req.forbidden_ta_list.tracking_area[0].tac = 1;

  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[0]= 1;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[1]= 2;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mcc[2]= 3;

  svc_req.forbidden_ta_list.tracking_area[0].plmn.num_mnc_digits = 3;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[0]= 4;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[1]= 5;
  svc_req.forbidden_ta_list.tracking_area[0].plmn.mnc[2]= 6;
...
void LteRrcCspBasicCellSelectionLimitedE911Test4::Teardown()
{
  lte_rrc_utf_teardown();
}
...
/*
Automatic service request with forbidden TA
SIB contain forbidden same PLMN as presdnt in forbidden TA
Second SIB contain diff PLMN but PLMN will be added in found PLMN list
LTE will send no service ind with use_oos set to FALSE
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test5);

void LteRrcCspBasicCellSelectionLimitedE911Test5::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test5::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  *(sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
  ...
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 1;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 2;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(SYS_SBAND_LTE_EUTRAN_BAND1 == cell_sel_req_ptr->band);
  TF_ASSERT(1 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(2 == cell_sel_req_ptr->phy_cell_id);
  ...
  cfg_req_param.bandwidth = LTE_BW_NRB_6;
  cfg_req_param.phy_cell_id = 1;
  cfg_req_param.num_black_cells = 2;
  cfg_req_param.energy = -60;
  cfg_req_param.status = E_FAILURE;
...
void LteRrcCspBasicCellSelectionLimitedE911Test5::Teardown()
{
  lte_rrc_utf_teardown();
}

/*
Limited mode service request not for E911 call
SIB1 supporting IMS
LTE will send service ind with use_oos set to false
*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicCellSelectionLimitedE911Test6);

void LteRrcCspBasicCellSelectionLimitedE911Test6::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicCellSelectionLimitedE911Test6::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
...
void LteRrcCspBasicCellSelectionLimitedE911Test6::Teardown()
{
  lte_rrc_utf_teardown();
}
