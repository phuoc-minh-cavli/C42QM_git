/*!
  @file lte_rrc_csp_irat_to_lte_basic_redir_fail_blacklist_test.cpp
  ...
  LTE redirection request comes in with a maximum-size blacklist and cell 
  selection proceeds. Cell is found to be barred. The blacklist being full 
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIratToLteBasicRedirFailBlacklistTest);

void LteRrcCspIratToLteBasicRedirFailBlacklistTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,2);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIratToLteBasicRedirFailBlacklistTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  redir_reqi.earfcn_info[0].blacklisted_cells_count = 4;
  for (i = 0; i < 4; i++)
  {
    redir_reqi.earfcn_info[0].blacklisted_cells[i] = 10 + i;
  }
  ...
  cfg_req_param.num_black_cells = 4;
  ...
  TF_ASSERT(E_FAILURE == redir_cnfi_ptr->status);
...
void LteRrcCspIratToLteBasicRedirFailBlacklistTest::Teardown()
{
