/*!
  @file lte_rrc_csp_irat_to_lte_resel_abort_test.cpp
...
static lte_rrc_irat_to_lte_resel_abort_reqi_s resel_abort_reqi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspIratToLteReselAbortTest);

void LteRrcCspIratToLteReselAbortTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,2);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspIratToLteReselAbortTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_irat_to_lte_resel_abort_cnfi_s *abort_cnfi_ptr;
  ...
  TF_MSG("Sending LTE_RRC_IRAT_TO_LTE_RESEL_ABORT_REQI");
  lte_rrc_init_default_hdr(&resel_abort_reqi.msg_hdr,
                           LTE_RRC_IRAT_TO_LTE_RESEL_ABORT_REQI);
  lte_rrc_utf_send_msg((byte*)&resel_abort_reqi, sizeof(resel_abort_reqi));
  ...
  TF_MSG("Checking for LTE_RRC_IRAT_TO_LTE_RESEL_ABORT_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_irat_to_lte_resel_abort_cnfi_s) == buf_len);
  abort_cnfi_ptr = (lte_rrc_irat_to_lte_resel_abort_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_IRAT_TO_LTE_RESEL_ABORT_CNFI == abort_cnfi_ptr->msg_hdr.id);
...
void LteRrcCspIratToLteReselAbortTest::Teardown()
{
