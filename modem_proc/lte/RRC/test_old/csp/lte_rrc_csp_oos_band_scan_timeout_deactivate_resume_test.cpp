/*!
  @file lte_rrc_csp_oos_band_scan_timeout_deactivate_resume_test.cpp
  ...
  timer is made to expire before band scan cnf. Deactivate request is sent.
  Then another Service Request is sent with use_timer = TRUE and scan_is_new = 
  FALSE. Scan resumes from where it left off on the same band even though one 
  more band is available and UE camps
...
04/15/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspOosBandScanTimeoutDeactivateResumeTest);

void LteRrcCspOosBandScanTimeoutDeactivateResumeTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,6);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspOosBandScanTimeoutDeactivateResumeTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_ASSERT(LTE_RRC_CFG_BAND_SCAN == cfg_req_ptr->cfg_info[0].cfg_type);
...
void LteRrcCspOosBandScanTimeoutDeactivateResumeTest::Teardown()
{
