/*!
  @file lte_rrc_csp_oos_band_scan_timeout_resume_test.cpp
  ...
  timer is made to expire before band scan cnf. Then another Service Request is
  sent with use_timer = TRUE and scan_is_new = FALSE. Scan resumes from where
  it left off on the same band even though one more band is available and UE 
  camps
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspOosBandScanTimeoutResumeTest);

void LteRrcCspOosBandScanTimeoutResumeTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspOosBandScanTimeoutResumeTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcCspOosBandScanTimeoutResumeTest::Teardown()
{
