/*!
  @file lte_rrc_csp_oos_cfg_timeout_test.cpp
  ...
  Scan returns two frequencies. The timer is made to expire before cfg
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspOosCfgTimeoutContinueFreqTest);

void LteRrcCspOosCfgTimeoutContinueFreqTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,7);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspOosCfgTimeoutContinueFreqTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_ASSERT(1 == cell_bar_req_ptr->bar_info.num_cells);
  TF_ASSERT(1 == cell_bar_req_ptr->bar_info.cell_info_list[0].rat_info.lte_cell_info.dl_cell_frequency);
...
lte_rrc_csp_test_handle_rrc_cfg_req (&cfg_req_param, validate_system_scan_req,&send_band_scan_cnf);
  ...
  TF_ASSERT(1 == cfg_req_ptr->num_cfg);
   ...
   bs_cnf.band_ptr[0].candidates[0].system.bandwidth = LTE_BW_NRB_6;
   bs_cnf.band_ptr[0].candidates[0].system.earfcn    = 600;
   bs_cnf.band_ptr[0].candidates[0].energy		   = -60;
  ...
  lte_rrc_init_default_hdr(&get_sib_cnf.msg_hdr,LTE_RRC_GET_SIBS_CNFI);
  get_sib_cnf.status = LTE_RRC_GET_SIBS_SUCCESS;
  get_sib_cnf.cell_info.cell_id.freq = 600;
  get_sib_cnf.cell_info.cell_id.phy_cell_id = 1;
  get_sib_cnf.cell_info.sib_db_key = 1;
  lte_rrc_utf_send_msg((byte*)&get_sib_cnf, sizeof(get_sib_cnf));
  ...
  TF_ASSERT(600 == cell_sel_req_ptr->earfcn);
  TF_ASSERT(1 == cell_sel_req_ptr->phy_cell_id);
...
void LteRrcCspOosCfgTimeoutContinueFreqTest::Teardown()
{
