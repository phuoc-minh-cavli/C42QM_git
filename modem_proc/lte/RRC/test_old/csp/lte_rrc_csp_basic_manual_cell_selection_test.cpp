/*!
  @file lte_rrc_csp_basic_manual_cell_selection_test.cpp

  @brief
  Unit test code for taking CSP to camped state with MANUAL mode.
...
05/21/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCspBasicManualCellSelectionTest);

void LteRrcCspBasicManualCellSelectionTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CSP_SM,ext_umid_list,5);
  lte_rrc_csp_test_stub_init();
}

void LteRrcCspBasicManualCellSelectionTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  svc_req.trans_id = 1;
  svc_req.network_select_mode = SYS_NETWORK_SELECTION_MODE_MANUAL;
  ...
  /* add an entry to forbidden TA list */
...
void LteRrcCspBasicManualCellSelectionTest::Teardown()
{
