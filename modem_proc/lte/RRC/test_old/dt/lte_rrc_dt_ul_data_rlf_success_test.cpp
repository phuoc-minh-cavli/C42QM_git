/*!
  @file lte_rrc_dt_ul_data_rlf_success_test.cpp
  ...
  4. Send UL data req
  5. Send UL data req
  6. Send Service Indication, should be ignored
  7. Send SRB2 Resumed Indication, should be handled
  8. Receive UL data confirmation failure with reason RLF and 2 transactions
  9. Check if DT is in READY state
...
static lte_rrc_srb2_resumed_indi_s srb2_resumed_indi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcDtULDataRlfSuccessTest);

void LteRrcDtULDataRlfSuccessTest::Setup()
{
    lte_rrc_utf_setup(LTE_RRC_DT_SM, ext_umid_list, 3);
    lte_rrc_test_dt_init();
}

void LteRrcDtULDataRlfSuccessTest::Test()
{
    byte *buf_ptr;
    uint32 buf_len;
    ...
    /* DT should ignore the service indication and still be in Blocked state */
    TF_MSG("Checking for BLOCKED state");
    TF_ASSERT(stm_get_state(LTE_RRC_DT_SM) == LTE_RRC_DT_SM__BLOCKED);

    TF_MSG("Sending SRB2 Resumed Indication");
    lte_rrc_init_default_hdr(&srb2_resumed_indi.msg_hdr, LTE_RRC_SRB2_RESUMED_INDI);
    lte_rrc_utf_send_msg((byte*)&srb2_resumed_indi, sizeof(srb2_resumed_indi));
    
    /* SRB2_RESUMED_INDI should come before UL_DATA_CNF,
    ...
    TF_ASSERT(ext_cnf_ptr->status == LTE_RRC_UL_DATA_CNF_FAILURE_RLF); 
    */
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        else if (((msgr_hdr_s *)(void *)buf_ptr)->id == LTE_RRC_IRAT_TUNNEL_UL_MSG_CNF)
        {
            ...
            TF_ASSERT(ext_irat_cnf_ptr->status == LTE_RRC_TUNNEL_UL_MSG_CNF_FAILURE_RLF); 
        }
        else
        {
            TF_ASSERT(0);
...
void LteRrcDtULDataRlfSuccessTest::Teardown()
{
