/*!
  @file lte_rrc_dt_dl_data_test.cpp
...
--------   ---     ----------------------------------------------------------
12/03/08   vatsac  Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcDtDLDataTest);

void LteRrcDtDLDataTest::Setup()
{
    lte_rrc_utf_setup(LTE_RRC_DT_SM, ext_umid_list, 1);
    lte_rrc_test_dt_init();
}

void LteRrcDtDLDataTest::Test()
{
    byte *buf_ptr;
    uint32 buf_len;
    int decode_status = 0;
    lte_rrc_dl_data_ind_s *ext_ind_ptr;
    lte_rrc_dlm_processed_indi_s *ind_ptr;
    lte_rrc_osys_DedicatedInfoNAS *nas_di;
		unsigned char *nas_msg;
		unsigned char nas_msg_ptr[10];
    ...
    TF_MSG("Sending DL Information Transfer Message");
    ...
    dcch_msg_ptr->message.u.c1->u.dlInformationTransfer->
      criticalExtensions.u.c1->u.dlInformationTransfer_r8->
      dedicatedInfoType.t = 
        T_lte_rrc_osys_DLInformationTransfer_r8_IEs_dedicatedInfoType_dedicatedInfoNAS;

    dcch_msg_ptr->message.u.c1->u.dlInformationTransfer->criticalExtensions.u.c1->
      u.dlInformationTransfer_r8->dedicatedInfoType.u.dedicatedInfoNAS = 
      (lte_rrc_osys_DedicatedInfoNAS *)
          lte_rrc_asn1_osys_IE_malloc(sizeof(lte_rrc_osys_DedicatedInfoNAS));
    nas_di = dcch_msg_ptr->message.u.c1->u.dlInformationTransfer->criticalExtensions.u.c1->
      u.dlInformationTransfer_r8->dedicatedInfoType.u.dedicatedInfoNAS;

    nas_di->numocts = 10; //LTE_RRC_MAX_NAS_MSG_CONTAINER_SIZE;
    nas_msg = (unsigned char*)lte_rrc_asn1_osys_IE_malloc(10);
    memset(nas_msg, 'A', 10);
		memset(nas_msg_ptr, 'A',10);
    nas_di->data = nas_msg;
    dl_msg.dl_msg_ptr = dcch_msg_ptr;
    
    /*
    unsigned char byte_str[] = {0x10, 0x01, 0x00, 0x07, 0x79, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00};
    LTE_RRC_OSYS_ASN1_DECODE_PDU(byte_str, sizeof(byte_str), 
                                 dl_msg.dl_msg_ptr,
                                 DL_DCCH_Message,
                                 &decode_status);
    */
    ...
    TF_MSG("Checking for LTE_RRC_DL_DATA_IND");
    lte_rrc_utf_get_next_ext_msg(&buf_ptr, &buf_len);
    TF_ASSERT(buf_len == sizeof(lte_rrc_dl_data_ind_s));
    ext_ind_ptr = (lte_rrc_dl_data_ind_s *)(void*) buf_ptr;
    TF_ASSERT(LTE_RRC_DL_DATA_IND == ext_ind_ptr->msg_hdr.id); 

    /* See if DT module has sent the DT message as a DSM item by checking for attachments */
    num_attach = msgr_get_num_attach((msgr_hdr_struct_type *)ext_ind_ptr);

    if ( num_attach == 1 )
    {
      unsigned char rcvd_nas_value[10];
      
      att_ptr = msgr_get_attach((msgr_hdr_struct_type *)ext_ind_ptr, 0);
      msgr_get_dsm_attach(att_ptr, &dsm_ptr);
      LTE_ASSERT(dsm_ptr != NULL);

      dsm_packet_length = dsm_length_packet(dsm_ptr);
      TF_ASSERT(dsm_packet_length == 10);

      TF_ASSERT ( 10 == dsm_pullup(&dsm_ptr, 
                  rcvd_nas_value, 
                  (uint16)dsm_packet_length) );

      TF_ASSERT(0 == 
            memcmp (rcvd_nas_value, nas_msg_ptr, 10));
...
void LteRrcDtDLDataTest::Teardown()
{
