...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigProxCfgRelTest);

void LteRrcConfigProxCfgRelTest::Setup()
{
...
void LteRrcConfigProxCfgRelTest::Test()
{
  ...
  TF_MSG("\nSending 2nd reconfig message with Prox Indi UTRA disabled \n");
  ...
  /* Setting the encoded message to the byte string corresponding to radio 
     resource configuration. Please refer to lte_rrc_config_ota_msg.h for 
     all Config OTA message definitions */ 
  encoded_pdu.length = sizeof(reconfig_meas_info_with_prox_cfg_msg1);
  encoded_pdu.value = reconfig_meas_info_with_prox_cfg_msg1;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;
  ...
  TF_ASSERT(prox_cfg_ind_ptr->msg_hdr.id == LTE_RRC_PROX_CFG_INDI);
  TF_ASSERT(prox_cfg_ind_ptr->eutra_configured == 1);
  TF_ASSERT(prox_cfg_ind_ptr->utra_configured == 0);
  ...
  TF_MSG("\nSending 3rd reconfig message with Prox Indi EUTRA disabled \n");
  ...
  /* Setting the encoded message to the byte string corresponding to radio 
     resource configuration. Please refer to lte_rrc_config_ota_msg.h for 
     all Config OTA message definitions */ 
  encoded_pdu.length = sizeof(reconfig_meas_info_with_prox_cfg_msg2);
  encoded_pdu.value = reconfig_meas_info_with_prox_cfg_msg2;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;
  ...
  /* Make sure the processed indication message is valid */
  TF_ASSERT(prox_cfg_ind_ptr->msg_hdr.id == LTE_RRC_PROX_CFG_INDI);
  TF_ASSERT(prox_cfg_ind_ptr->eutra_configured == 0);
  TF_ASSERT(prox_cfg_ind_ptr->utra_configured == 1);
  ...
  TF_MSG("\nSending 4th reconfig message with Prox Indi EUTRA and UTRA disabled \n");
  ...
  /* Setting the encoded message to the byte string corresponding to radio 
     resource configuration. Please refer to lte_rrc_config_ota_msg.h for 
     all Config OTA message definitions */ 
  encoded_pdu.length = sizeof(reconfig_meas_info_with_prox_cfg_msg);
  encoded_pdu.value = reconfig_meas_info_with_prox_cfg_msg;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;
...
void LteRrcConfigProxCfgRelTest::Teardown()
{
  lte_rrc_utf_teardown();
}


TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigWithProxCfgOnlyTest);

void LteRrcConfigWithProxCfgOnlyTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
  TF_STUB(lte_rrc_sec_security_is_configured) = lte_rrc_config_meas_test_is_sec_activated;
...
void LteRrcConfigWithProxCfgOnlyTest::Test()
{
  lte_rrc_mh_dlm_s dl_msg;
  lte_rrc_pdu_buf_s encoded_pdu;
  lte_rrc_dlm_processed_indi_s *dlm_processed_ind_ptr = NULL;
  ...
  /* Setting the encoded message to the byte string corresponding to radio 
     resource configuration. Please refer to lte_rrc_config_ota_msg.h for 
     all Config OTA message definitions */ 
  encoded_pdu.length = sizeof(reconfig_prox_indi_only_msg);
  encoded_pdu.value = reconfig_prox_indi_only_msg;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;
...
void LteRrcConfigWithProxCfgOnlyTest::Teardown()
{
  lte_rrc_utf_teardown();
}
