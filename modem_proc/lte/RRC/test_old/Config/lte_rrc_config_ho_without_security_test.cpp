/*!
  @file lte_rrc_config_ho_without_security_test.cpp

  @brief
  Unit test code for the Config procedure that tests handling of a Reconfig 
  message containing HO related information without security configuration being 
  present.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigHOWithoutSecurityTest);

void LteRrcConfigHOWithoutSecurityTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigHOWithoutSecurityTest::Test()
{
  ...
  encoded_pdu.length = sizeof(reconfig_msg_ho_without_security);
  encoded_pdu.value = reconfig_msg_ho_without_security;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nConfig: HO without security Test Begins \n");
  ...
  /* Security configuration being absent would cause validation failure and 
    Config would call the stub function for lte_rrc_config_start_reestablishment */
  ...
  TF_MSG("\nConfig: HO without security Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigHOWithoutSecurityTest::Teardown()
{
