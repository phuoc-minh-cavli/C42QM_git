/*!
  @file lte_rrc_config_nas_no_rrc_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message containing NAS dedicated info but no Radio Resource 
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigNasNoRrcTest);

void LteRrcConfigNasNoRrcTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigNasNoRrcTest::Test()
{
  ...
  i_ptr->sd_ptr->nas_pdu_in_recfg_chk_enabled = 1;
  ...
  encoded_pdu.length = sizeof(invalid_reconfig_nas_wo_drb_msg);
  encoded_pdu.value = invalid_reconfig_nas_wo_drb_msg;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG( "\n NAS Ded. Info without DRB to Add/Modify/Release Test begins\n" );
  ...
  TF_MSG("\nNAS Ded. Info without DRB to Add/Modify/Release Test Ends \n");
...
void LteRrcConfigNasNoRrcTest::Teardown()
{
