/*!
  @file lte_rrc_config_lte_stopped_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  an LTE_RRC_STOPPED_INDI from RRC Controller while processing a Reconfig 
  message containing radio resource configuration.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigLteStoppedTest);

void LteRrcConfigLteStoppedTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigLteStoppedTest::Test()
{
  lte_rrc_mh_dlm_s dl_msg;
  lte_rrc_pdu_buf_s encoded_pdu;
  lte_rrc_cfg_reqi_s* cfg_req_msg_ptr = NULL;
  lte_rrc_stopped_indi_s stopped_ind_msg;
  uint32 buf_len = 0;
  int decode_status = 0;
  /* ---------------------------------------------------------------------- */
  ...
  TF_MSG("\nConfig: LTE Stopped Test Begins \n");
  ...
  /* Setting the header for internal RRC messages */
  lte_rrc_init_default_hdr(&stopped_ind_msg.msg_hdr,LTE_RRC_STOPPED_INDI);
  /* Send the STOPPED_INDI to Config module there by mimicing the action of Controller */
  lte_rrc_utf_send_msg((byte*)&stopped_ind_msg, sizeof(stopped_ind_msg));
  lte_rrc_utf_wait_for_done();
  /* After Config completes processing, assert its in INITIAL state */
  TF_ASSERT(lte_rrc_config_get_state() == LTE_RRC_CONFIG_SM__INITIAL);
  TF_MSG("\nConfig: LTE Stopped Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigLteStoppedTest::Teardown()
{
