/*!
  @file lte_rrc_config_empty_ies_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message with empty IEs.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigEmptyIEsTest);

void LteRrcConfigEmptyIEsTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigEmptyIEsTest::Test()
{
  ...
  encoded_pdu.length = sizeof(reconfig_with_empty_ies);
  encoded_pdu.value = reconfig_with_empty_ies;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nReconfig with empty IEs Test Begins \n");
  ...
  TF_MSG("\nReconfig with empty IEs Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigEmptyIEsTest::Teardown()
{
