/*!
  @file lte_rrc_config_from_lte_to_g_mobility_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Mobility from EUTRA message that contains CCO to GERAN.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigFromLteToGMobilityTest);

void LteRrcConfigFromLteToGMobilityTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigFromLteToGMobilityTest::Test()
{
  ...
  encoded_pdu.length = sizeof(mobility_from_eutra_cco);
  encoded_pdu.value = mobility_from_eutra_cco;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nMobility from EUTRA - CCO Test Begins \n");
  ...
  TF_MSG("\nMobility from EUTRA - CCO Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigFromLteToGMobilityTest::Teardown()
{
