/*!
  @file lte_rrc_config_from_lte_to_w_mobility_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Mobility from EUTRA message that contains PS HO to W.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigFromLteToWMobilityTest);

void LteRrcConfigFromLteToWMobilityTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigFromLteToWMobilityTest::Test()
{
  ...
  TF_MSG("\nMobility from EUTRA to W msg dispatch Test Begins \n");
  ...
  TF_MSG("\nMobility from EUTRA to W msg dispatch Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigFromLteToWMobilityTest::Teardown()
{
