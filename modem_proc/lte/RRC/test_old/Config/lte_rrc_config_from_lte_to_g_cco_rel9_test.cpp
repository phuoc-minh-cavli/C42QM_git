/*!
  @file lte_rrc_config_from_lte_to_g_cco_rel9_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Mobility from EUTRA message that contains CCO to GERAN for Rel9.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigFromLteToGCCORel9Test);

void LteRrcConfigFromLteToGCCORel9Test::Setup()
{
...
void LteRrcConfigFromLteToGCCORel9Test::Test()
{
  lte_rrc_mh_dlm_s dl_msg;
  lte_rrc_pdu_buf_s encoded_pdu;
  lte_rrc_irat_from_lte_to_g_mobility_indi_s *lte_to_g_mobility_indi_ptr;
  ...
  encoded_pdu.length = sizeof(mobility_from_eutra_cco_rel9);
  encoded_pdu.value = mobility_from_eutra_cco_rel9;
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nMobility from EUTRA - CCO for Rel 9 Test Begins \n");
  ...
  /* Catch LTE_RRC_IRAT_FROM_LTE_TO_G_MOBILITY_INDI sent by Config to G IRAT MGR */
  lte_rrc_utf_get_next_int_msg((byte **)&lte_to_g_mobility_indi_ptr,&buf_len);
  /* Make sure the Config request is valid */
  TF_ASSERT(lte_to_g_mobility_indi_ptr->msg_hdr.id == LTE_RRC_IRAT_FROM_LTE_TO_G_MOBILITY_INDI);
  TF_ASSERT(lte_to_g_mobility_indi_ptr->dl_msg_ptr != NULL);
  dl_dcch_msg_ptr = (lte_rrc_osys_DL_DCCH_Message *) lte_to_g_mobility_indi_ptr->dl_msg_ptr;
  ...
  TF_MSG("\nMobility from EUTRA - CCO for Rel 9 Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigFromLteToGCCORel9Test::Teardown()
{
