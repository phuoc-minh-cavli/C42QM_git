/*!
  @file lte_rrc_config_msg_class_ext_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message containing invalid message class extension.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigMsgClassExtTest);

void LteRrcConfigMsgClassExtTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigMsgClassExtTest::Test()
{
  ...
  encoded_pdu.length = 1;
  encoded_pdu.value = invalid_msg_class_ext;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG(
  "\nReconfig - Invalid Message Class Extension Test - Begins \n");
  ...
  TF_MSG("\nReconfig - Invalid Message Class Extension Test Ends \n");
...
void LteRrcConfigMsgClassExtTest::Teardown()
{
