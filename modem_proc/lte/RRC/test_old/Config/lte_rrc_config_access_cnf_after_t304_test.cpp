/*!
  @file lte_rrc_config_access_cnf_after_t304_test.cpp

  @brief
  Unit test code for the Config procedure that tests the handling of ACCESS_CNF
  from MAC when T304 has just expired.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigAccessCnfAfterT304Test);

void LteRrcConfigAccessCnfAfterT304Test::Setup()
{
...
void LteRrcConfigAccessCnfAfterT304Test::Test()
{
  ...
  lte_mac_access_abort_req_msg_s *mac_access_abort_req_msg_ptr = NULL;
  ...
  /* Catch the ACCESS_ABORT_REQ sent by Config to MAC */
  lte_rrc_utf_get_next_ext_msg((byte **)&mac_access_abort_req_msg_ptr,&buf_len); 
  /* Make sure the Access Abort Request message is valid */
  TF_ASSERT(sizeof(lte_mac_access_abort_req_msg_s) == buf_len);

  /* After Config sends ACCESS_ABORT_REQ, assert its still in WAIT_FOR_MAC_ACCESS_CNF state */
  TF_ASSERT(lte_rrc_config_get_state() == LTE_RRC_CONFIG_SM__WAIT_FOR_MAC_ACCESS_CNF);

  /* Now send MAC_ACCESS_CNF with SUCCESS */
...
void LteRrcConfigAccessCnfAfterT304Test::Teardown()
{
