/*!
  @file lte_rrc_config_invalid_release_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message containing IEs from unsupported release.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigInvalidReleaseTest);

void LteRrcConfigInvalidReleaseTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigInvalidReleaseTest::Test()
{
  ...
  encoded_pdu.length = 2;
  encoded_pdu.value = invalid_release_msg;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nReconfig: Invalid Release Test Begins \n");
  ...
  TF_MSG("\nReconfig: Invalid Release Test Ends \n");
  lte_rrc_utf_reset(); 
...
void LteRrcConfigInvalidReleaseTest::Teardown()
{
