/*!
  @file lte_rrc_config_crit_ext_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message containing invalid critical extensions.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigCritExtTest);

void LteRrcConfigCritExtTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigCritExtTest::Test()
{
  ...
  TF_MSG("\nReconfig: Invalid Crit Extension Test Ends \n");
...
void LteRrcConfigCritExtTest::Teardown()
{
