/*!
  @file lte_rrc_config_sec_no_mobility_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a Reconfig message containing security configuration but no mobility Info
  being present.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigSecurityWithoutHOTest);

void LteRrcConfigSecurityWithoutHOTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigSecurityWithoutHOTest::Test()
{
  ...
  encoded_pdu.length = sizeof(invalid_reconfig_sec_no_mobility_msg);
  encoded_pdu.value = invalid_reconfig_sec_no_mobility_msg;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nSecurity without Mobility Info Test Begins \n");
  ...
  /* Since Security configuration is present without the mobility info being
     ...
     module to send Conn mode failure indication */
  ...
  TF_MSG("\nSecurity without Mobility Info Test Ends \n");
...
void LteRrcConfigSecurityWithoutHOTest::Teardown()
{
