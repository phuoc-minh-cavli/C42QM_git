/*!
  @file lte_rrc_config_non_reconfig_test.cpp

  @brief
  Unit test code for the Config procedure that tests the processing of 
  a DL DCCH message that is not a Reconfig message.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcConfigNonReconfigTest);

void LteRrcConfigNonReconfigTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONFIG_SM, NULL, 0);
  lte_rrc_config_test_stub_init();
}

void LteRrcConfigNonReconfigTest::Test()
{
  ...
  encoded_pdu.length = sizeof(non_reconfig_msg);
  encoded_pdu.value = non_reconfig_msg;

  /* Initialise the currently used encoded message*/
  lte_rrc_config_test_encoded_pdu = encoded_pdu;

  TF_MSG("\nNon Reconfig Message Test - Begins \n");
  ...
  TF_MSG("\nNon Reconfig Message Test Ends \n");
...
void LteRrcConfigNonReconfigTest::Teardown()
{
