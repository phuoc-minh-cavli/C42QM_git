/*!
  @file lte_rrc_llc_invalid_rnti_test.cpp
...
lte_rrc_llc_invalid_rnti_test.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/15/09   da      Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidRntiTest);

void LteRrcLlcInvalidRntiTest::Setup()
{
...
void LteRrcLlcInvalidRntiTest::Test()
{
  ...
  encoded_pdu.length = sizeof(llc_invalid_crnti_msg);
  encoded_pdu.value = llc_invalid_crnti_msg;

  /* Set the RNTI to invalid */
  TF_MSG("\nConnection Reconfiguration Invalid RNTI Test Begins \n");
  lte_rrc_llc_test_ho_validation_failure(encoded_pdu);
  TF_MSG("\nConnection Reconfiguration Invalid RNTI Test Test Ends \n");
  lte_rrc_utf_reset(); 
}

void LteRrcLlcInvalidRntiTest::Teardown()
{
