/*!
  @file lte_rrc_llc_invalid_ho_fdd_to_tdd_no_tdd_ack_nak_feedback_mode_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the handling of HO mesg from
  FDD to TDD that does not include tdd_ack_nak_feedback_mode, that is mandatory 
  to include.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidHOFDDtoTDDnoAckNakFeedbackModeTest);

void LteRrcLlcInvalidHOFDDtoTDDnoAckNakFeedbackModeTest::Setup()
{
...
void LteRrcLlcInvalidHOFDDtoTDDnoAckNakFeedbackModeTest::Test()
{
  lte_rrc_pdu_buf_s encoded_pdu;
  lte_rrc_osys_RadioResourceConfigCommon *radio_resrc_common_ptr = NULL;
  int decode_status = 0;
  TF_MSG("\nConnection Reconfiguration Invalid HO - FDD to TDD without TDD Config Test Begins \n");
  ...
  /* Mask off tdd_AckNackFeedbackMode bitmask */
  lte_rrc_llc_test_common_data.dl_dcch_msg_ptr->message.u.c1->u.rrcConnectionReconfiguration->
    criticalExtensions.u.c1->u.rrcConnectionReconfiguration_r8->radioResourceConfigDedicated.
    physicalConfigDedicated.pucch_ConfigDedicated.m.tdd_AckNackFeedbackModePresent = 0;
  ...
  TF_MSG("\nConnection Reconfiguration Invalid HO - FDD to TDD without TDD Config Test Test Ends \n");
  lte_rrc_llc_test_free_dl_ded_msg_ptr();
  lte_rrc_llc_test_free_sib_ptr();
  lte_rrc_utf_reset(); 
}

void LteRrcLlcInvalidHOFDDtoTDDnoAckNakFeedbackModeTest::Teardown()
{
