/*!
  @file lte_rrc_llc_rlc_um_sn_length_modify_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of a reconfig 
  message that reconfigigures RLC SN length.
...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/llc/lte_rrc_llc_rlc_um_sn_length_modify_test.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcRlcUmSnLengthModifyTest);

void LteRrcLlcRlcUmSnLengthModifyTest::Setup()
{
...
void LteRrcLlcRlcUmSnLengthModifyTest::Test()
{
  ...
  int decode_status;
  /* ---------------------------------------------------------------------- */
  TF_MSG("\nInvalid RLC UM sn length Change Test Begins \n");
  ...
  dcch_ptr = (lte_rrc_osys_DL_DCCH_Message *)lte_rrc_osys_asn1_decode_pdu(encoded_pdu.value, 14, 
                               DL_DCCH_Message, &decode_status);
  TF_ASSERT(decode_status == 0);
  TF_ASSERT(dcch_ptr != NULL);
  /* Change the RLC SN length */
  ...
  reconfig_msg->criticalExtensions.u.c1->u.rrcConnectionReconfiguration_r8->radioResourceConfigDedicated.
    drb_ToAddModList.elem[0].rlc_Config.u.um_Bi_Directional->ul_UM_RLC.sn_FieldLength = lte_rrc_osys_SN_FieldLength_size10;
  ...
  TF_MSG("\nInvalid RLC UM sn length modify test Ends \n");
...
void LteRrcLlcRlcUmSnLengthModifyTest::Teardown()
{
