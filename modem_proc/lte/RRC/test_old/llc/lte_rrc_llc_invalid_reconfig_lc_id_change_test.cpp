/*!
  @file lte_rrc_llc_invalid_reconfig_lc_id_change_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of a reconfig 
  message that tries to change LC id of an RB that is already established.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidReconfigLCIDChangeTest);

void LteRrcLlcInvalidReconfigLCIDChangeTest::Setup()
{
...
void LteRrcLlcInvalidReconfigLCIDChangeTest::Test()
{
  lte_rrc_pdu_buf_s encoded_pdu;
  lte_rrc_osys_DL_DCCH_Message *dl_dcch_msg_ptr = NULL;
  int decode_status = 0;
  /* ---------------------------------------------------------------------- */
  TF_MSG("\nInvalid Reconfig for LC ID Change Test Begins \n");
  ...
  dl_dcch_msg_ptr->message.u.c1->u.rrcConnectionReconfiguration->criticalExtensions.
    ...
    drb_ToAddModList.elem->m.logicalChannelIdentityPresent = 1;
  /* Set the LC id of the existing DRB to 1 */
  dl_dcch_msg_ptr->message.u.c1->u.rrcConnectionReconfiguration->criticalExtensions.
    u.c1->u.rrcConnectionReconfiguration_r8->radioResourceConfigDedicated.
    drb_ToAddModList.elem->logicalChannelIdentity = 1;
  ...
  TF_MSG("\nInvalid Reconfig for LC ID Change Test Ends \n");
...
void LteRrcLlcInvalidReconfigLCIDChangeTest::Teardown()
{
