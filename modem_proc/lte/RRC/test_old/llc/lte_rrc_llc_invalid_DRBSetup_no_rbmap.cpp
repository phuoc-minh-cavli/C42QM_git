/*!
  @file lte_rrc_llc_invalid_drbsetup_no_rbmap_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of 
  a Reconfig message setting up DRB without providing the RB Mapping Info
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidDRBSetupNoRBMapTest);

void LteRrcLlcInvalidDRBSetupNoRBMapTest::Setup()
{
...
void LteRrcLlcInvalidDRBSetupNoRBMapTest::Test()
{
  lte_rrc_pdu_buf_s encoded_pdu;
  /* ---------------------------------------------------------------------- */
  TF_MSG("\nInvalid Reconfig for DRB Setup without RB mapping Test Begins \n");

  lte_rrc_llc_test_basic_sib2_update();
  /* Doing connection setup */
  lte_rrc_llc_test_rrc_connection_setup_default();

  /* Setting the encoded message to the byte string in lte_rrc_llc_ota_msg.h */ 
  encoded_pdu.length = sizeof(llc_invalid_drb_no_rbmap_msg);
  encoded_pdu.value = llc_invalid_drb_no_rbmap_msg;
  ...
  TF_MSG("\nInvalid Reconfig for DRB Setup without RB mapping Test Ends \n");
...
void LteRrcLlcInvalidDRBSetupNoRBMapTest::Teardown()
{
