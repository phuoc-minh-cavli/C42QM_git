/*!
  @file lte_rrc_llc_invalid_conn_setup_rbid_test.cpp
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidConnSetupRBIDTest);

void LteRrcLlcInvalidConnSetupRBIDTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_LLC_SM, NULL, 0);
  lte_rrc_llc_test_stub_init();
}

void LteRrcLlcInvalidConnSetupRBIDTest::Test()
{
  lte_rrc_pdu_buf_s encoded_pdu;
  /* ---------------------------------------------------------------------- */
  TF_MSG("\nInvalid Connection Setup RB ID Test Begins \n");

  /* Setting the encoded message to the byte string in lte_rrc_llc_ota_msg.h */  
  encoded_pdu.length = sizeof(llc_invalid_conn_setup_id);
  encoded_pdu.value = llc_invalid_conn_setup_id;
  ...
  TF_MSG("\nInvalid Connection Setup RB ID Test Ends \n");
...
void LteRrcLlcInvalidConnSetupRBIDTest::Teardown()
{
