/*!
  @file lte_rrc_llc_invalid_multiple_rb_single_eps_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of 
  a Connection reconfiguration message that tries to setup a DRB mapped to an
  EPS bearer ID that is already in use.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidMultipleRbSingleEPSTest);

void LteRrcLlcInvalidMultipleRbSingleEPSTest::Setup()
{
...
void LteRrcLlcInvalidMultipleRbSingleEPSTest::Test()
{
  ...
  TF_MSG("\nInvalid Reconfig for Multiple DRBs mapped to a single EPS bearer ID Test Begins \n");

  /* Setup SRB1, SRB2, DRB3, DRB4 and DRB5 */
  ...
  /* Now setup a new DRB mapped to an EPS bearer id that is already in use */
  ...
  /* Set the DRB id of the new DRB to 1 */
  ...
  TF_MSG("\nInvalid Reconfig for Multiple DRBs mapped to a single EPS bearer ID Test Ends \n");
...
void LteRrcLlcInvalidMultipleRbSingleEPSTest::Teardown()
{
