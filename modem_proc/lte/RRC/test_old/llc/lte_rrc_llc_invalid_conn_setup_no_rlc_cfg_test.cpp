/*!
  @file lte_rrc_llc_invalid_conn_setup_no_rlc_cfg_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of 
  a Connection Setup message with no RLC Config for SRB1.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcInvalidConnSetupNoRLCCfgTest);

void LteRrcLlcInvalidConnSetupNoRLCCfgTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_LLC_SM, ext_umid_list, 1);
  lte_rrc_llc_test_stub_init();
}

void LteRrcLlcInvalidConnSetupNoRLCCfgTest::Test()
{
 ...
 /* Setting the encoded message to the byte string in lte_rrc_llc_ota_msg.h */  
  ...
  /* Modify message so that RLC Config for SRB1 is not present */
  TF_ASSERT(ccch_msg_ptr != NULL);
  ccch_msg_ptr->message.u.c1->u.rrcConnectionSetup->criticalExtensions.u.c1->u.
    rrcConnectionSetup_r8->radioResourceConfigDedicated.srb_ToAddModList.elem->m.rlc_ConfigPresent = 0;
...
void LteRrcLlcInvalidConnSetupNoRLCCfgTest::Teardown()
{
