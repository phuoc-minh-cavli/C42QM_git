/*!
  @file lte_rrc_llc_ho_with_rlc_pdcp_modify_test.cpp
  ...
  information along with RLC and PDCP reconfiguration. To start receives a 
  Connection Setup followed by Connection Reconfiguration message that sets up 
  an AM DRB, before handover.
...
lte_rrc_llc_ho_with_rlc_pdcp_modify_test.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcHOWithRLCPDCPModifyTest);

void LteRrcLlcHOWithRLCPDCPModifyTest::Setup()
{
...
void LteRrcLlcHOWithRLCPDCPModifyTest::Test()
{
  ...
  encoded_pdu.length = sizeof(llc_reconfig_ho_rlc_pdcp_modify_msg);
  encoded_pdu.value = llc_reconfig_ho_rlc_pdcp_modify_msg;
  lte_rrc_llc_test_intra_freq_mobility_only(encoded_pdu, TRUE);
  lte_rrc_utf_reset(); 
}

void LteRrcLlcHOWithRLCPDCPModifyTest::Teardown()
{
