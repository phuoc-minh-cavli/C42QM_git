/*!
  @file lte_rrc_llc_ack_nak_with_pucch_format_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of an invalid 
  Connection Setup message that includes dedicated PUCCH Config with
  TDD ACK/NAK feedback mode and PUCCH REL 10 Format, in a TDD cell.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcAckNakWithPucchFormatTest);

void LteRrcLlcAckNakWithPucchFormatTest::Setup()
{
...
void LteRrcLlcAckNakWithPucchFormatTest::Test()
{
  ...
  encoded_pdu.length = sizeof(llc_conn_setup_with_ack_nak_and_pucch_fmt);
  encoded_pdu.value = llc_conn_setup_with_ack_nak_and_pucch_fmt;
...
void LteRrcLlcAckNakWithPucchFormatTest::Teardown()
{
