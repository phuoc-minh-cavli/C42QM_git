/*!
  @file lte_rrc_llc_ho_intra_freq_mobility_loop_test.cpp

  @brief
  Unit test code for the LLC procedure that tests the processing of back to back
  HOs - reconfig messages with intra freq mobility. To start, UE receives a 
  Connection Setup followed by Connection Reconfiguration message that sets up 
  a UM Bi-directional DRB, before handover.
...
lte_rrc_llc_ho_intra_freq_mobility_loop_test.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcLlcIntraFreqMobilityLoopTest);

void LteRrcLlcIntraFreqMobilityLoopTest::Setup()
{
...
void LteRrcLlcIntraFreqMobilityLoopTest::Test()
{
...
void LteRrcLlcIntraFreqMobilityLoopTest::Teardown()
{
