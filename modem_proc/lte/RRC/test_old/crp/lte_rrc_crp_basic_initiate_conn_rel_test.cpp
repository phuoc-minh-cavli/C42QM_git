/*!
  @file lte_rrc_crp_basic_initiate_conn_rel_test.cpp

  @brief
  Unit test code for taking CRP from Inactive -> Wait for LLC Config -> Inactive
  for Initiate Connection Release indication
...
--------   ---     ----------------------------------------------------------
05/16/09   sureshs Initial Revision
...
//message structures
static lte_rrc_initiate_conn_rel_indi_s initiate_conn_rel_indi;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCrpBasicInitiateConnRelTest);

void LteRrcCrpBasicInitiateConnRelTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRP_SM,ext_umid_list,1);
  lte_rrc_crp_test_stub_init();
}

void LteRrcCrpBasicInitiateConnRelTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG("Sending Initiate Conn Rel indication");
  lte_rrc_init_default_hdr(&initiate_conn_rel_indi.msg_hdr,
                           LTE_RRC_INITIATE_CONN_REL_INDI);
  initiate_conn_rel_indi.rel_reason = LTE_RRC_CONN_REL_RLF;
  lte_rrc_utf_send_msg((byte*)&initiate_conn_rel_indi, 
                       sizeof(initiate_conn_rel_indi));
  ...
  TF_ASSERT(LTE_RRC_CONN_REL_RLF == conn_rel_ind_ptr->rel_reason);
  ...
  TF_ASSERT(LTE_RRC_CONN_REL_RLF == ext_conn_rel_ind_ptr->rel_reason);
...
void LteRrcCrpBasicInitiateConnRelTest::Teardown()
{
