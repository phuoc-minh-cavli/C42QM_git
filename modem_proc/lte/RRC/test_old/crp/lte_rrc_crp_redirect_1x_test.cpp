/*!
  @file lte_rrc_crp_redirect_1x_test.cpp

  @brief
  Unit test code for taking CRP from Inactive -> Wait for Timer ->
  Wait for LLC confirm -> Inactive with redirect information to 1X
...
--------   ---     ----------------------------------------------------------
11/03/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCrpRedirect1xTest);

void LteRrcCrpRedirect1xTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRP_SM,ext_umid_list,1);
  lte_rrc_crp_test_stub_init();
}

void LteRrcCrpRedirect1xTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_crp_test_stub_csp_set_1x_is_supported(TRUE);

  TF_MSG("Sending Conn Release DLM");
  lte_rrc_crp_test_construct_conn_rel_msg(&conn_rel_dlm,FALSE,TRUE,FALSE);
  conn_rel_r8_ptr = ((lte_rrc_osys_DL_DCCH_Message *)(conn_rel_dlm.dl_msg_ptr))->message.u.c1->u.rrcConnectionRelease->
  ...
  conn_rel_r8_ptr->redirectedCarrierInfo.t = 
      T_lte_rrc_osys_RedirectedCarrierInfo_cdma2000_1xRTT;
  conn_rel_r8_ptr->redirectedCarrierInfo.u.cdma2000_1xRTT->arfcn = 2000;
  ...
  TF_ASSERT(conn_rel_ind_ptr->rel_reason ==
            LTE_RRC_CONN_REL_NORMAL);
  TF_ASSERT(conn_rel_ind_ptr->redir_info_is_present == TRUE);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.rat_type == 
            LTE_RRC_REDIR_RAT_CDMA2000_1xRTT);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.band == SYS_BAND_BC0);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.dl_arfcn == 2000);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.opt_info_ptr != NULL);
  ...
  TF_ASSERT( ext_conn_rel_ind_ptr->redir_info.is_redir );
  TF_ASSERT( LTE_RRC_REDIR_RAT_CDMA2000_1xRTT == ext_conn_rel_ind_ptr->redir_info.redir_rat );
...
void LteRrcCrpRedirect1xTest::Teardown()
{
