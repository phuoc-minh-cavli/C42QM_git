/*!
  @file lte_rrc_crp_redirect_conn_release_band12_test.cpp

  @brief
  Unit test code for taking CRP from Inactive -> Wait for Timer ->
  Wait for LLC confirm -> Inactive with redirect information
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCrpRedirectConnRelBand12Test);

void LteRrcCrpRedirectConnRelBand12Test::Setup()
{
...
void LteRrcCrpRedirectConnRelBand12Test::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_crp_test_stub_csp_set_do_is_supported(TRUE);

  TF_MSG("Sending Conn Release DLM");
  lte_rrc_crp_test_construct_conn_rel_msg(&conn_rel_dlm,FALSE,TRUE,FALSE);

  /* Change Connection release message to include redirection info for band 12*/
  dcch_msg_ptr = (lte_rrc_osys_DL_DCCH_Message *) conn_rel_dlm.dl_msg_ptr;
  conn_rel_r8_ptr = dcch_msg_ptr->message.u.c1->u.rrcConnectionRelease->
                     criticalExtensions.u.c1->u.rrcConnectionRelease_r8;
  conn_rel_r8_ptr->redirectedCarrierInfo.t = T_lte_rrc_osys_RedirectedCarrierInfo_eutra;
  conn_rel_r8_ptr->redirectedCarrierInfo.u.eutra = 5100;
  ...
  TF_ASSERT(conn_rel_ind_ptr->redir_info.rat_type == LTE_RRC_REDIR_RAT_LTE);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.band == SYS_BAND_CLASS_NONE);
  TF_ASSERT(conn_rel_ind_ptr->redir_info.dl_arfcn == 5770);
...
void LteRrcCrpRedirectConnRelBand12Test::Teardown()
{
