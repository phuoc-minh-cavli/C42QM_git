/*!
  @file lte_rrc_crp_tau_conn_release_test.cpp

  @brief
  Unit test code for taking CRP from Inactive -> Wait for Timer ->
  Wait for LLC confirm -> Inactive with TAU required
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCrpTauConnRelTest);

void LteRrcCrpTauConnRelTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRP_SM,ext_umid_list,1);
  lte_rrc_crp_test_stub_init();
}

void LteRrcCrpTauConnRelTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_crp_test_construct_conn_rel_msg(&conn_rel_dlm,TRUE,FALSE,FALSE);
  ...
  TF_ASSERT(TRUE == ext_conn_rel_ind_ptr->load_balancing_tau_is_reqd);
...
void LteRrcCrpTauConnRelTest::Teardown()
{
