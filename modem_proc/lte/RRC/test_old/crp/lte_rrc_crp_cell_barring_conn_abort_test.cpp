/*!
  @file lte_rrc_crp_cell_barring_conn_abort_test.cpp

  @brief
  Unit test code for taking CRP from Inactive -> Wait for LLC Config -> Inactive
  with cell barring required
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCrpCellBarringConnAbortTest);

void LteRrcCrpCellBarringConnAbortTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRP_SM,ext_umid_list,1);
  lte_rrc_crp_test_stub_init();
}

void LteRrcCrpCellBarringConnAbortTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  conn_abort_req.cell_barring_is_reqd = TRUE;
  ...
  TF_ASSERT(TRUE == conn_rel_ind_ptr->cell_barring_is_reqd);
...
void LteRrcCrpCellBarringConnAbortTest::Teardown()
{
