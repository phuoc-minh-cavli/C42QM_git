/*!
  @file lte_rrc_plmn_search_interleaved_manual_hplmn_search_test.cpp
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchInterleavedManualHplmnSearchTest);

void LteRrcPlmnSearchInterleavedManualHplmnSearchTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
}

void LteRrcPlmnSearchInterleavedManualHplmnSearchTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  search_req.plmn_rat_list.info[1].plmn.identity[0] = 0x22;
  search_req.plmn_rat_list.info[1].plmn.identity[1] = 0x63;
  search_req.plmn_rat_list.info[1].plmn.identity[2] = 0x54;
  search_req.plmn_rat_list.info[1].rat = SYS_RAT_LTE_RADIO_ACCESS;
  search_req.rplmn_info_is_valid = TRUE;
  search_req.rplmn.mcc[0] = 7;
  search_req.rplmn.mcc[1] = 8;
  search_req.rplmn.mcc[2] = 9;
  ...
  TF_ASSERT(search_cnf_ptr->trans_id == 2);
  ...
  TF_ASSERT(search_cnf_ptr->status == LTE_RRC_PLMN_SEARCH_ABORTED);
  ...
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[0].plmn.identity[0] == 0x22);
  ...
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[0].signal_strength == 110);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[1].plmn.identity[0] == 0x21);
...
void LteRrcPlmnSearchInterleavedManualHplmnSearchTest::Teardown()
{
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchInterleavedManualAbortHplmnSearchTest);

void LteRrcPlmnSearchInterleavedManualAbortHplmnSearchTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
...
void LteRrcPlmnSearchInterleavedManualAbortHplmnSearchTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
  ...
  //At this point, RRC sends an extra plmn_srch_cnf to NAS which is incorrect and is a BUG.
  TF_MSG("Sending LTE_CPHY_BPLMN_CELL_CNF");
  msgr_init_hdr(&cell_cnf.msgr_hdr, MSGR_LTE_CPHY, LTE_CPHY_BPLMN_CELL_CNF);
  cell_cnf.earfcn = 1; cell_cnf.freq = 1;
  cell_cnf.cell_id = 1;
...
void LteRrcPlmnSearchInterleavedManualAbortHplmnSearchTest::Teardown()
{
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchInterleavedManualAbort2HplmnSearchTest);

void LteRrcPlmnSearchInterleavedManualAbort2HplmnSearchTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
  lte_rrc_test_integration_common_init_all( UNIT );
...
void LteRrcPlmnSearchInterleavedManualAbort2HplmnSearchTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_rrc_mode_change_reqi_s *mode_change_req_ptr;
...
void LteRrcPlmnSearchInterleavedManualAbort2HplmnSearchTest::Teardown()
{
  lte_rrc_test_integration_common_deinit_all();
  lte_rrc_utf_teardown();
}
