/*!
  @file lte_rrc_plmn_search_manual_search_test.cpp
...
TF_DEFINE_TEST_CASE(DISABLED, LteRrcPlmnSearchManualSearchTest);

void LteRrcPlmnSearchManualSearchTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
}

void LteRrcPlmnSearchManualSearchTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_ASSERT(start_req_ptr->num_bands == 5);
  TF_ASSERT(start_req_ptr->bands[0] == SYS_SBAND_LTE_EUTRAN_BAND1);
  TF_ASSERT(start_req_ptr->bw_masks[0] == LTE_BW_BMASK_1_4_MHZ | LTE_BW_BMASK_3_MHZ | LTE_BW_BMASK_5_MHZ | LTE_BW_BMASK_10_MHZ | LTE_BW_BMASK_15_MHZ | LTE_BW_BMASK_20_MHZ);
  TF_ASSERT(start_req_ptr->bands[4] == SYS_SBAND_LTE_EUTRAN_BAND12);
  TF_ASSERT(start_req_ptr->bw_masks[4] == LTE_BW_BMASK_1_4_MHZ | LTE_BW_BMASK_3_MHZ | LTE_BW_BMASK_5_MHZ | LTE_BW_BMASK_10_MHZ);
  ...
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
      ...
      elem[0] = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->freqBandIndicator = 1;
  ...
  unsolicited_sib_indi.cell_id.freq = 225;
  unsolicited_sib_indi.cell_id.phy_cell_id = 4;
  ...
  TF_ASSERT(cell_req_ptr->freq == 225);
  TF_ASSERT(cell_req_ptr->updated_freq == 225);
  TF_ASSERT(cell_req_ptr->cell_id == 4);
  ...
  cell_cnf.earfcn = 225; cell_cnf.freq = 21325;
  cell_cnf.cell_id = 4;
  ...
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
      cellAccessRelatedInfo.plmn_IdentityList.elem->plmn_Identity.mcc.
      elem[0] = 2;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->freqBandIndicator = 4;
  ...
  TF_ASSERT(LTE_CPHY_BPLMN_CELL_REQ == cell_req_ptr->msgr_hdr.id);
  TF_ASSERT(cell_req_ptr->freq == 225);
  TF_ASSERT(cell_req_ptr->updated_freq == 2175);
  ...
  TF_MSG("Sending LTE_CPHY_BPLMN_CELL_CNF");
  msgr_init_hdr(&cell_cnf.msgr_hdr, MSGR_LTE_CPHY, LTE_CPHY_BPLMN_CELL_CNF);
  cell_cnf.earfcn = 2175; cell_cnf.freq = 21325;
  cell_cnf.cell_id = 1;
  cell_cnf.bw = LTE_BW_NRB_100;
  cell_cnf.rsrp = -110; //LTE_RRC_CSP_HIGH_CELL_QUALITY_THRESHOLD;
  cell_cnf.srxlev_calculated = TRUE;
  ...
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->
      cellAccessRelatedInfo.plmn_IdentityList.elem->plmn_Identity.mcc.
      elem[0] = 1;
  sib1_pdu_ptr->message.u.c1->u.systemInformationBlockType1->freqBandIndicator = 17;
  ...
  TF_MSG("Sending LTE_RRC_UNSOLICITED_SIB_INDI");
  lte_rrc_init_default_hdr(&unsolicited_sib_indi.msg_hdr, LTE_RRC_UNSOLICITED_SIB_INDI);
  unsolicited_sib_indi.cell_id.freq = 5120;
  ...
  cell_req_ptr = (lte_cphy_msg_bplmn_cell_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_BPLMN_CELL_REQ == cell_req_ptr->msgr_hdr.id);
  TF_ASSERT(cell_req_ptr->freq == 5120);
  TF_ASSERT(cell_req_ptr->updated_freq == 5790);
  ...
  TF_MSG("Sending LTE_CPHY_BPLMN_CELL_CNF");
  msgr_init_hdr(&cell_cnf.msgr_hdr, MSGR_LTE_CPHY, LTE_CPHY_BPLMN_CELL_CNF);
  cell_cnf.earfcn = 5790; cell_cnf.freq = 7400;
  cell_cnf.cell_id = 2;
  cell_cnf.bw = LTE_BW_NRB_50;
  cell_cnf.rsrp = -90; //LTE_RRC_CSP_HIGH_CELL_QUALITY_THRESHOLD;
  cell_cnf.srxlev_calculated = TRUE;
  cell_cnf.srxlev = 0;  
  cell_cnf.squal = 1;
  lte_rrc_utf_send_msg((byte*)&cell_cnf, sizeof(cell_cnf));
  ...
  TF_ASSERT(search_cnf_ptr->search_type == SYS_SERVICE_SEARCH_NONE);
  TF_ASSERT(search_cnf_ptr->status == LTE_RRC_PLMN_SEARCH_COMPLETED);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.length == 2);
  ...
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[0].signal_strength == 90);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[1].plmn.identity[0] == 0x22);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[1].plmn.identity[1] == 0x63);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[1].plmn.identity[2] == 0x54);
  TF_ASSERT(search_cnf_ptr->found_plmn_rat_list.info[1].rat == SYS_RAT_LTE_RADIO_ACCESS);
...
void LteRrcPlmnSearchManualSearchTest::Teardown()
{
