...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchGuardTimerExpiryTest);

void LteRrcPlmnSearchGuardTimerExpiryTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
}

void LteRrcPlmnSearchGuardTimerExpiryTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_plmn_search_data.s_ptr->control.guard_tmri_val = 500; // change guard timer value to 500 sec
  ...
  TF_SLEEP((int)(LTE_RRC_PLMN_SEARCH_GUARD_TMRI_VAL));

  TF_MSG("Checking for Guard Timer expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr->msg_hdr.id ==  LTE_RRC_PLMN_SEARCH_GUARD_TMRI);
  ...
  TF_ASSERT(search_cnf_ptr->status == LTE_RRC_PLMN_SEARCH_TIMED_OUT);
...
void LteRrcPlmnSearchGuardTimerExpiryTest::Teardown()
{
  lte_rrc_utf_teardown();
}
void populate_plmn_search_req_guardtimerexpiry()
{
	...
	plmn_search_req -> forbidden_ta_list.num_entries = 3;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchGuardTimerExpiryAbortTest);

void LteRrcPlmnSearchGuardTimerExpiryAbortTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
...
void LteRrcPlmnSearchGuardTimerExpiryAbortTest::Test()
{
	TF_MSG("Start of test");
	...
	lte_rrc_test_integration_common_send_umid (LTE_RRC_PLMN_SEARCH_REQ, &populate_plmn_search_req_guardtimerexpiry, LTE_RRC_TEST_MAX_CELL, LTE_RRC_TEST_MAX_SIB_UPDATE, 0, FALSE);
	...
	lte_rrc_test_integration_verify_req_send_cnf (NULL, LTE_CPHY_BPLMN_CELL_REQ, LTE_RRC_TEST_CELL_0, TRUE, NULL);
	...
	lte_rrc_test_integration_verify_req_send_cnf (NULL, LTE_CPHY_BPLMN_CELL_REQ, LTE_RRC_TEST_CELL_1, TRUE, NULL);
	...
	//check expected behavior from Raghu and update. Currently we need to verify which message is RRC sending out when it receives abortreq followed by complete_ind
...
void LteRrcPlmnSearchGuardTimerExpiryAbortTest::Teardown()
{
