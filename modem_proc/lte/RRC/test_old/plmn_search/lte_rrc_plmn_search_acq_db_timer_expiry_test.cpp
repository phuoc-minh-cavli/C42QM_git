/*!
  @file lte_rrc_plmn_search_acq_db_timer_expiry_test.cpp
...
extern "C"
{
extern lte_rrc_plmn_search_data_s lte_rrc_plmn_search_data;
}
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcPlmnSearchAcqDbTimerExpiryTest);

void LteRrcPlmnSearchAcqDbTimerExpiryTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_PLMN_SEARCH_SM,ext_umid_list,4);
  lte_rrc_plmn_search_test_stub_init();
}

void LteRrcPlmnSearchAcqDbTimerExpiryTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  lte_rrc_plmn_search_data.s_ptr->control.acq_db_tmri_val = 50; // change acq db timer value to 50 sec
  ...
  /*Sleep for the Guard Timer time */
  TF_SLEEP(lte_rrc_plmn_search_data.s_ptr->control.acq_db_tmri_val);
  ...
  TF_ASSERT(start_req_ptr->new_req == TRUE);
  TF_ASSERT(start_req_ptr->req_type == LTE_CPHY_BPLMN_TYPE_LTE_LTE);
  TF_ASSERT(start_req_ptr->start_now == FALSE);
  TF_ASSERT(start_req_ptr->num_chans == 1);
  TF_ASSERT(start_req_ptr->chans[0].chan == 1);
  TF_ASSERT(start_req_ptr->chans[0].bw == LTE_BW_NRB_100);
  TF_ASSERT(start_req_ptr->num_bands == 1);
  TF_ASSERT(start_req_ptr->bands[0] == SYS_SBAND_LTE_EUTRAN_BAND1);
  TF_ASSERT(start_req_ptr->num_black_freqs == 0);
  TF_ASSERT(start_req_ptr->irat_lte_plmn_time_allowed == LTE_CPHY_BPLMN_UNLIMITED_TIME);
...
void LteRrcPlmnSearchAcqDbTimerExpiryTest::Teardown()
{
