/*!
  @file lte_rrc_controller_start_cnf_fail_test.cpp

  @brief
  Unit test code for basic RRC controller state transitions. Take it from
  Inactive -> Inactive by sending Start Cnf with failure status.
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerStartCnfFailTest);

void LteRrcControllerStartCnfFailTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 6);
}

void LteRrcControllerStartCnfFailTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG("Sending Start Cnf with FALSE status");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_FAILURE;
  ...
  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI with FALSE status");
  ...
  TF_ASSERT(FALSE == mc_cnf_ptr->status);
...
void LteRrcControllerStartCnfFailTest::Teardown()
{
