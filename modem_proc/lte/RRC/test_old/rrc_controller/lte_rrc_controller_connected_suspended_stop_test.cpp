/*!
  @file lte_rrc_controller_connected_suspended_stop_test.cpp

  @brief
  Take controller from Inactive -> Connected. Then send Mode Change Request
  with Suspend and type = Stop, and change to Suspended. Then send Mode Change
  Request with Resume.
...
08/17/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerConnectedSuspendedStopTest);

void LteRrcControllerConnectedSuspendedStopTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 7);
}

void LteRrcControllerConnectedSuspendedStopTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcControllerConnectedSuspendedStopTest::Teardown()
{
