/*!
  @file lte_rrc_controller_basic_test.cpp

  @brief
  Unit test code for RRC controller deadlock timer expiry. Tests put for trapping expiry upon
  suspend req, resume req, start req(s) and stop req(s)

*/

/*===========================================================================

  Copyright (c) 2008 Qualcomm Technologies Incorporated. All Rights Reserved

  Qualcomm Proprietary

  Export of this technology or software is regulated by the U.S. Government.
  Diversion contrary to U.S. law prohibited.

  All ideas, data and information contained in or disclosed by
  this document are confidential and proprietary information of
  Qualcomm Technologies Incorporated and all rights therein are expressly reserved.
  By accepting this material the recipient agrees that this material
  and the information contained therein are held in confidence and in
  trust and will not be used, copied, reproduced in whole or in part,
  nor its contents revealed in any manner to others without the express
  written permission of Qualcomm Technologies Incorporated.

===========================================================================*/

/*===========================================================================

                        EDIT HISTORY FOR MODULE

This section contains comments describing changes made to the module.
Notice that changes are listed in reverse chronological order.

$Header: 

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/10/12   ap      Initial Revision
           
===========================================================================*/

/*===========================================================================

                           INCLUDE FILES

===========================================================================*/
extern "C"
{
#include <lte_rrc_int_msg.h>
#include <lte_rrc_controller.h>
#include <lte_rrc_utils.h>
#include <lte_cphy_msg.h>
#include <lte_mac_msg.h>
#include <lte_mac_rrc.h>
#include <lte_pdcp_msg.h>
#include <lte_rlc_msg.h>
#include <lte_rlc_rrc.h>
#include <lte_rrc_controlleri.h>
}

#include <TestFramework.h>
#include <lte_rrc_utf.h>
#include <tf_stub.h> 

extern "C"
{
  TF_PROTO_STUB(void, lte_rrc_throw_err_fatal, lte_rrc_err_fatal_e);
  extern lte_rrc_ctlr_s lte_rrc_ctlr_data;
}

//external messages generated by code under test
static uint32 ext_umid_list[] = 
{ 
  LTE_CPHY_START_REQ,
  LTE_CPHY_STOP_REQ,
  LTE_MAC_START_REQ,
  LTE_MAC_STOP_REQ,
  LTE_RLCDL_START_REQ,
  LTE_RLCDL_STOP_REQ,
  LTE_RLCUL_START_REQ,
  LTE_RLCUL_STOP_REQ,
  LTE_PDCPDL_START_REQ,
  LTE_PDCPDL_STOP_REQ,
  LTE_PDCPUL_START_REQ,
  LTE_PDCPUL_STOP_REQ,
  LTE_CPHY_SUSPEND_REQ,
  LTE_CPHY_RESUME_REQ,
};

//message structures
static lte_rrc_mode_change_reqi_s mc_req;
static lte_rrc_cfg_cnfi_s cfg_cnfi;
static lte_cphy_start_cnf_s start_cnf;
static lte_cphy_stop_cnf_s stop_cnf;
static lte_rrc_camped_indi_s camped_ind;
static lte_rrc_conn_establishment_started_indi_s conn_est_started_ind;
static lte_rrc_connected_indi_s connected_ind;
static lte_rrc_conn_rel_started_indi_s conn_rel_started_ind;
static lte_mac_start_cnf_msg_s mac_start_cnf;
static lte_mac_stop_cnf_msg_s mac_stop_cnf;
static lte_rlc_start_cnf_s rlc_start_cnf;
static lte_rlc_stop_cnf_s rlc_stop_cnf;
static lte_pdcp_start_cnf_msg_s pdcp_start_cnf;
static lte_pdcp_stop_cnf_msg_s pdcp_stop_cnf;
static lte_cphy_suspend_cnf_s suspend_cnf;
static lte_cphy_resume_cnf_s resume_cnf;
static lte_rrc_not_camped_indi_s not_camped_ind;

/*===========================================================================

                                FUNCTIONS

===========================================================================*/
/*===========================================================================

  FUNCTION:  lte_rrc_ctlr_test_deadlock_tmri_ef

===========================================================================*/
/*!
    @brief
    This function simulates the RRC CTLR API that calls error fatal.

    @return
    void
*/
/*=========================================================================*/
void lte_rrc_ctlr_test_deadlock_tmri_ef
(
  lte_rrc_err_fatal_e reason
)
{
  TF_ASSERT(LTE_RRC_ERR_FATAL_CTLR_DEADLOCK_TMR_EXPIRY == reason);
  TF_MSG("\nCtlr Deadlock timer expired and Handler called successfully\n");
} /* lte_rrc_ctlr_test_deadlock_tmri_ef() */


/*===========================================================================

                                UNIT TESTS

===========================================================================*/
//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving start confirm from MAC */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStartCnfFromMAC);

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromMAC::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}
void LteRrcControllerDeadlockTmrExpiryForStartCnfFromMAC::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
	lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromMAC::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving all start confirm form RLC */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStartCnfFromRLC);

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromRLC::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromRLC::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
	lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromRLC::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving all start confirm form PDCP */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStartFromPDCP);

void LteRrcControllerDeadlockTmrExpiryForStartFromPDCP::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStartFromPDCP::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
	lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStartFromPDCP::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving all stop confirm from MAC */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStopCnfFromMAC);

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromMAC::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromMAC::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_rrc_cfg_reqi_s *cfg_req_ptr;
  lte_rrc_stopped_indi_s *stopped_ind_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_mac_stop_req_msg_s *mac_stop_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_rlc_stop_req_s *rlc_stop_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_pdcp_stop_req_msg_s *pdcp_stop_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG("Sending Connection Release Started indication");
  lte_rrc_init_default_hdr(&conn_rel_started_ind.msg_hdr,
                           LTE_RRC_CONN_REL_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_rel_started_ind, 
                       sizeof(conn_rel_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CLOSING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CLOSING);

  TF_MSG("Sending Mode Change Req to Dectivate");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST; /* don't care */
  mc_req.mode = LTE_RRC_MODE_CHANGE_DEACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_RRC_CFG_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);

  TF_MSG("Checking for LTE_RRC_STOPPED_INDI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_stopped_indi_s) == buf_len);
  stopped_ind_ptr = (lte_rrc_stopped_indi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_STOPPED_INDI == stopped_ind_ptr->msg_hdr.id);

  TF_MSG("Sending cfg_cnfi");
  lte_rrc_init_default_hdr(&cfg_cnfi.msg_hdr,LTE_RRC_CFG_CNFI);
  cfg_cnfi.proc = LTE_RRC_PROC_CONTROLLER;
  cfg_cnfi.trans_id = 0;
  lte_rrc_utf_send_msg((byte*)&cfg_cnfi, sizeof(cfg_cnfi));

  TF_MSG("Checking for LTE_PDCPUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_MAC_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_stop_req_msg_s) == buf_len);
  mac_stop_req_ptr = (lte_mac_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_STOP_REQ == mac_stop_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending PDCPUL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending PDCPDL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending RLCUL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending RLCDL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending Stop Cnf");
  msgr_init_hdr(&stop_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_STOP_CNF);
  stop_cnf.status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&stop_cnf, sizeof(stop_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();
  
  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromMAC::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving all stop confirm from RLC */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStopCnfFromRLC);

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromRLC::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromRLC::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_rrc_cfg_reqi_s *cfg_req_ptr;
  lte_rrc_stopped_indi_s *stopped_ind_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_mac_stop_req_msg_s *mac_stop_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_rlc_stop_req_s *rlc_stop_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_pdcp_stop_req_msg_s *pdcp_stop_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG("Sending Connection Release Started indication");
  lte_rrc_init_default_hdr(&conn_rel_started_ind.msg_hdr,
                           LTE_RRC_CONN_REL_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_rel_started_ind, 
                       sizeof(conn_rel_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CLOSING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CLOSING);

  TF_MSG("Sending Mode Change Req to Dectivate");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST; /* don't care */
  mc_req.mode = LTE_RRC_MODE_CHANGE_DEACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_RRC_CFG_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);

  TF_MSG("Checking for LTE_RRC_STOPPED_INDI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_stopped_indi_s) == buf_len);
  stopped_ind_ptr = (lte_rrc_stopped_indi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_STOPPED_INDI == stopped_ind_ptr->msg_hdr.id);

  TF_MSG("Sending cfg_cnfi");
  lte_rrc_init_default_hdr(&cfg_cnfi.msg_hdr,LTE_RRC_CFG_CNFI);
  cfg_cnfi.proc = LTE_RRC_PROC_CONTROLLER;
  cfg_cnfi.trans_id = 0;
  lte_rrc_utf_send_msg((byte*)&cfg_cnfi, sizeof(cfg_cnfi));

  TF_MSG("Checking for LTE_PDCPUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_MAC_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_stop_req_msg_s) == buf_len);
  mac_stop_req_ptr = (lte_mac_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_STOP_REQ == mac_stop_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending PDCPUL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending PDCPDL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending RLCUL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending MAC Stop Cnf");
  msgr_init_hdr(&mac_stop_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_STOP_CNF);
  mac_stop_cnf.status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&mac_stop_cnf, sizeof(mac_stop_cnf));

  TF_MSG("Sending Stop Cnf");
  msgr_init_hdr(&stop_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_STOP_CNF);
  stop_cnf.status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&stop_cnf, sizeof(stop_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();
  
  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromRLC::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving all stop confirm from PDCP */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStopCnfFromPDCP);

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromPDCP::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromPDCP::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_rrc_cfg_reqi_s *cfg_req_ptr;
  lte_rrc_stopped_indi_s *stopped_ind_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_mac_stop_req_msg_s *mac_stop_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_rlc_stop_req_s *rlc_stop_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_pdcp_stop_req_msg_s *pdcp_stop_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG("Sending Connection Release Started indication");
  lte_rrc_init_default_hdr(&conn_rel_started_ind.msg_hdr,
                           LTE_RRC_CONN_REL_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_rel_started_ind, 
                       sizeof(conn_rel_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CLOSING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CLOSING);

  TF_MSG("Sending Mode Change Req to Dectivate");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST; /* don't care */
  mc_req.mode = LTE_RRC_MODE_CHANGE_DEACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_RRC_CFG_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);

  TF_MSG("Checking for LTE_RRC_STOPPED_INDI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_stopped_indi_s) == buf_len);
  stopped_ind_ptr = (lte_rrc_stopped_indi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_STOPPED_INDI == stopped_ind_ptr->msg_hdr.id);

  TF_MSG("Sending cfg_cnfi");
  lte_rrc_init_default_hdr(&cfg_cnfi.msg_hdr,LTE_RRC_CFG_CNFI);
  cfg_cnfi.proc = LTE_RRC_PROC_CONTROLLER;
  cfg_cnfi.trans_id = 0;
  lte_rrc_utf_send_msg((byte*)&cfg_cnfi, sizeof(cfg_cnfi));

  TF_MSG("Checking for LTE_PDCPUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_MAC_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_stop_req_msg_s) == buf_len);
  mac_stop_req_ptr = (lte_mac_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_STOP_REQ == mac_stop_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending PDCPUL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending RLCUL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending RLCDL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending MAC Stop Cnf");
  msgr_init_hdr(&mac_stop_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_STOP_CNF);
  mac_stop_cnf.status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&mac_stop_cnf, sizeof(mac_stop_cnf));

  TF_MSG("Sending Stop Cnf");
  msgr_init_hdr(&stop_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_STOP_CNF);
  stop_cnf.status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&stop_cnf, sizeof(stop_cnf));

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();
  
  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromPDCP::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving stop confirm from ML1 */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStopCnfFromML1);

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromML1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromML1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;  

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG(
   "Sending Mode Change Req to Suspend with dest of Connected and type = Stop");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.mode = LTE_RRC_MODE_CHANGE_SUSPEND;
  mc_req.dest_scenario = LTE_RRC_SUSPENDED_DUE_TO_RLF;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStopCnfFromML1::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving start confirm from ML1 */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForStartCnfFromML1);

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromML1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromML1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG(
   "Sending Mode Change Req to Suspend with dest of Connected and type = Stop");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.mode = LTE_RRC_MODE_CHANGE_SUSPEND;
  mc_req.dest_scenario = LTE_RRC_SUSPENDED_DUE_TO_RLF;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG("Sending Stop Cnf");
  msgr_init_hdr(&stop_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_STOP_CNF);
  stop_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&stop_cnf, sizeof(stop_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG(
   "Sending Mode Change Req to Resume");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.mode = LTE_RRC_MODE_CHANGE_RESUME;
  memset(&mc_req.dest_scenario,0,sizeof(mc_req.dest_scenario));
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForStartCnfFromML1::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving suspend confirm from ML1 */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForSuspendCnfFromML1);

void LteRrcControllerDeadlockTmrExpiryForSuspendCnfFromML1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForSuspendCnfFromML1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_cphy_suspend_req_s *suspend_req_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);
  TF_ASSERT(LTE_RRC_MODE_CHANGE_ACTIVATE == mc_cnf_ptr->mode);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Mode Change Request for Suspend");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_SUSPENDED_DUE_TO_RESEL_FROM_LTE_TO_DO;
  mc_req.mode = LTE_RRC_MODE_CHANGE_SUSPEND;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_SUSPEND_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_suspend_req_s) == buf_len);
  suspend_req_ptr = (lte_cphy_suspend_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_SUSPEND_REQ == suspend_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);
  
  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();
  
  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForSuspendCnfFromML1::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer expiry for not receiving resume confirm from ML1 */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrExpiryForResumeCnfFromML1);

void LteRrcControllerDeadlockTmrExpiryForResumeCnfFromML1::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = 
	lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrExpiryForResumeCnfFromML1::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_cphy_suspend_req_s *suspend_req_ptr;
  lte_cphy_resume_req_s *resume_req_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);
  TF_ASSERT(LTE_RRC_MODE_CHANGE_ACTIVATE == mc_cnf_ptr->mode);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Mode Change Request for Suspend");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_SUSPENDED_DUE_TO_RESEL_FROM_LTE_TO_DO;
  mc_req.mode = LTE_RRC_MODE_CHANGE_SUSPEND;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_SUSPEND_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_suspend_req_s) == buf_len);
  suspend_req_ptr = (lte_cphy_suspend_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_SUSPEND_REQ == suspend_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);
  
  TF_MSG("Sending Suspend Cnf");
  msgr_init_hdr(&suspend_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_SUSPEND_CNF);
  suspend_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&suspend_cnf, sizeof(suspend_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);
  TF_ASSERT(LTE_RRC_MODE_CHANGE_SUSPEND == mc_cnf_ptr->mode);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG("Sending Mode Change Request for Resume");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = (lte_rrc_dest_scenario_e) 0;
  mc_req.mode = LTE_RRC_MODE_CHANGE_RESUME;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_RESUME_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_resume_req_s) == buf_len);
  resume_req_ptr = (lte_cphy_resume_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_RESUME_REQ == resume_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);
  
  /*Expire the ctlr deadlock timer*/
  TF_MSG("Expiring the timer");
  TF_SLEEP(LTE_RRC_CTLR_DEADLOCK_TIME);
  /* Deadlock timer expiry indication would call the stub function for 
    lte_rrc_llc_deadlock_tmri_handler */
  
  /*Expect CTLR_DEADLOCK tmri*/
  TF_MSG("Checking for Ctlr_Deadlock tmr expiry");
  lte_rrc_utf_get_next_int_msg (&buf_ptr, &buf_len);
  TF_ASSERT(sizeof(lte_rrc_tmri_s) == buf_len);
  tmri_ptr = (lte_rrc_tmri_s *)(void*) buf_ptr;
  TF_ASSERT(tmri_ptr != NULL);
  TF_ASSERT((tmri_ptr->msg_hdr.id) ==  
		  LTE_RRC_CTLR_DEADLOCK_TMRI);
  lte_rrc_utf_reset();

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrExpiryForResumeCnfFromML1::Teardown()
{
  lte_rrc_utf_teardown();
}

//-----------------------------------------------------------------------------
/* Test to verify ctlr deadlock timer starting/stopping */

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeadlockTmrStartStop);

void LteRrcControllerDeadlockTmrStartStop::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 14);
  TF_STUB(lte_rrc_throw_err_fatal) = lte_rrc_ctlr_test_deadlock_tmri_ef;
}

void LteRrcControllerDeadlockTmrStartStop::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  lte_cphy_stop_req_s *stop_req_ptr;
  lte_rrc_mode_change_cnfi_s *mc_cnf_ptr;
  lte_rrc_cfg_reqi_s *cfg_req_ptr;
  lte_rrc_stopped_indi_s *stopped_ind_ptr;
  lte_mac_start_req_msg_s *mac_start_req_ptr;
  lte_mac_stop_req_msg_s *mac_stop_req_ptr;
  lte_rlc_start_req_s *rlc_start_req_ptr;
  lte_rlc_stop_req_s *rlc_stop_req_ptr;
  lte_pdcp_start_req_msg_s *pdcp_start_req_ptr;
  lte_pdcp_stop_req_msg_s *pdcp_stop_req_ptr;
  lte_rrc_tmri_s *tmri_ptr	= NULL;

  TF_MSG("Start of test");

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Mode Change Req to Activate with dest of Idle Not Camped");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.dest_scenario = LTE_RRC_ACTIVATED_DUE_TO_NAS_REQUEST;
  mc_req.mode = LTE_RRC_MODE_CHANGE_ACTIVATE;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_start_req_s) == buf_len);
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 

  TF_MSG("Checking for LTE_MAC_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_start_req_msg_s) == buf_len);
  mac_start_req_ptr = (lte_mac_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_START_REQ == mac_start_req_ptr->hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_start_req_s) == buf_len);
  rlc_start_req_ptr = (lte_rlc_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_START_REQ == rlc_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPUL_START_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_start_req_msg_s) == buf_len);
  pdcp_start_req_ptr = (lte_pdcp_start_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_START_REQ == pdcp_start_req_ptr->msg_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending Start Cnf");
  msgr_init_hdr(&start_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_START_CNF);
  start_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&start_cnf, sizeof(start_cnf));

  TF_MSG("Sending MAC Start Cnf");
  msgr_init_hdr(&mac_start_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_START_CNF);
  mac_start_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_start_cnf, sizeof(mac_start_cnf));

  TF_MSG("Sending RLCDL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending RLCUL Start Cnf");
  msgr_init_hdr(&rlc_start_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_START_CNF);
  rlc_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&rlc_start_cnf, sizeof(rlc_start_cnf));

  TF_MSG("Sending PDCPDL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));
  
  lte_rrc_utf_wait_for_done();
  /* Checking that the timer is not stopped yet */
  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Sending PDCPUL Start Cnf");
  msgr_init_hdr(&pdcp_start_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_START_CNF);
  pdcp_start_cnf.cnf_status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&pdcp_start_cnf, sizeof(pdcp_start_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for IDLE_NOT_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_NOT_CAMPED);

  TF_MSG("Sending Idle Camped indication");
  lte_rrc_init_default_hdr(&camped_ind.msg_hdr,LTE_RRC_CAMPED_INDI);
  memset(&camped_ind.camped_cell,0,sizeof(camped_ind.camped_cell));
  camped_ind.reason = LTE_RRC_CAMPED_CELL_SELECTION;
  lte_rrc_utf_send_msg((byte*)&camped_ind, sizeof(camped_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for IDLE_CAMPED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__IDLE_CAMPED);

  TF_MSG("Sending Conn Est Started indication");
  lte_rrc_init_default_hdr(&conn_est_started_ind.msg_hdr,
                           LTE_RRC_CONN_ESTABLISHMENT_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_est_started_ind, 
                       sizeof(conn_est_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTING state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTING);

  TF_MSG("Sending Connected indication");
  lte_rrc_init_default_hdr(&connected_ind.msg_hdr,LTE_RRC_CONNECTED_INDI);
  lte_rrc_utf_send_msg((byte*)&connected_ind, 
                       sizeof(connected_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for CONNECTED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__CONNECTED);

  TF_MSG(
   "Sending Mode Change Req to Suspend with dest of Connected and type = Stop");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
  mc_req.mode = LTE_RRC_MODE_CHANGE_SUSPEND;
  mc_req.dest_scenario = LTE_RRC_SUSPENDED_DUE_TO_RLF;
  lte_rrc_utf_send_msg((byte*)&mc_req, sizeof(mc_req));

  TF_MSG("Checking for LTE_CPHY_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_cphy_stop_req_s) == buf_len);
  stop_req_ptr = (lte_cphy_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_STOP_REQ == stop_req_ptr->msgr_hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG("Sending Stop Cnf");
  msgr_init_hdr(&stop_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_STOP_CNF);
  stop_cnf.status = E_SUCCESS;
  lte_rrc_utf_send_msg((byte*)&stop_cnf, sizeof(stop_cnf));

  TF_MSG("Checking for LTE_RRC_MODE_CHANGE_CNFI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_mode_change_cnfi_s) == buf_len);
  mc_cnf_ptr = (lte_rrc_mode_change_cnfi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG("Sending Connection Release Started indication");
  lte_rrc_init_default_hdr(&conn_rel_started_ind.msg_hdr,
                           LTE_RRC_CONN_REL_STARTED_INDI);
  lte_rrc_utf_send_msg((byte*)&conn_rel_started_ind, 
                       sizeof(conn_rel_started_ind));

  lte_rrc_utf_wait_for_done();

  TF_MSG("Checking for SUSPENDED state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__SUSPENDED);

  TF_MSG("Sending Idle Not Camped indication");
  lte_rrc_init_default_hdr(&not_camped_ind.msg_hdr,LTE_RRC_NOT_CAMPED_INDI);
  lte_rrc_utf_send_msg((byte*)&not_camped_ind, sizeof(not_camped_ind));

  TF_MSG("Checking for LTE_RRC_CFG_REQI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_cfg_reqi_s) == buf_len);
  cfg_req_ptr = (lte_rrc_cfg_reqi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_CFG_REQI == cfg_req_ptr->msg_hdr.id);

  TF_MSG("Checking for LTE_RRC_STOPPED_INDI");
  lte_rrc_utf_get_next_int_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rrc_stopped_indi_s) == buf_len);
  stopped_ind_ptr = (lte_rrc_stopped_indi_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RRC_STOPPED_INDI == stopped_ind_ptr->msg_hdr.id);

  TF_MSG("Sending cfg_cnfi");
  lte_rrc_init_default_hdr(&cfg_cnfi.msg_hdr,LTE_RRC_CFG_CNFI);
  cfg_cnfi.proc = LTE_RRC_PROC_CONTROLLER;
  cfg_cnfi.trans_id = 0;
  lte_rrc_utf_send_msg((byte*)&cfg_cnfi, sizeof(cfg_cnfi));

  TF_MSG("Checking for LTE_PDCPUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPUL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_PDCPDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_pdcp_stop_req_msg_s) == buf_len);
  pdcp_stop_req_ptr = (lte_pdcp_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_PDCPDL_STOP_REQ == pdcp_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCUL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCUL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_RLCDL_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_rlc_stop_req_s) == buf_len);
  rlc_stop_req_ptr = (lte_rlc_stop_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_RLCDL_STOP_REQ == rlc_stop_req_ptr->msg_hdr.id); 

  TF_MSG("Checking for LTE_MAC_STOP_REQ");
  lte_rrc_utf_get_next_ext_msg(&buf_ptr,&buf_len);
  TF_ASSERT(sizeof(lte_mac_stop_req_msg_s) == buf_len);
  mac_stop_req_ptr = (lte_mac_stop_req_msg_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_MAC_STOP_REQ == mac_stop_req_ptr->hdr.id); 

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("Sending PDCPUL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPUL,LTE_PDCPUL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending PDCPDL Stop Cnf");
  msgr_init_hdr(&pdcp_stop_cnf.msg_hdr,MSGR_LTE_PDCPDL,LTE_PDCPDL_STOP_CNF);
  pdcp_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&pdcp_stop_cnf, sizeof(pdcp_stop_cnf));

  TF_MSG("Sending RLCUL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCUL,LTE_RLCUL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  TF_MSG("Sending RLCDL Stop Cnf");
  msgr_init_hdr(&rlc_stop_cnf.msg_hdr,MSGR_LTE_RLCDL,LTE_RLCDL_STOP_CNF);
  rlc_stop_cnf.cnf_status = E_SUCCESS; 
  lte_rrc_utf_send_msg((byte*)&rlc_stop_cnf, sizeof(rlc_stop_cnf));

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(TRUE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(TRUE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Sending MAC Stop Cnf");
  msgr_init_hdr(&mac_stop_cnf.hdr,MSGR_LTE_MAC,LTE_MAC_STOP_CNF);
  mac_stop_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&mac_stop_cnf, sizeof(mac_stop_cnf));

  lte_rrc_utf_wait_for_done();

  TF_ASSERT(FALSE == lte_rrc_tmr_is_running(&lte_rrc_ctlr_data.dd_ptr->deadlock_tmr));
  TF_ASSERT(FALSE == lte_rrc_ctlr_data.dd_ptr->deadlock_timer_is_running);

  TF_MSG("Checking for INACTIVE state");
  TF_ASSERT(lte_rrc_controller_get_state() == 
            LTE_RRC_CONTROLLER_SM__INACTIVE);

  TF_MSG("End of test");
}

void LteRrcControllerDeadlockTmrStartStop::Teardown()
{
  lte_rrc_utf_teardown();
}
