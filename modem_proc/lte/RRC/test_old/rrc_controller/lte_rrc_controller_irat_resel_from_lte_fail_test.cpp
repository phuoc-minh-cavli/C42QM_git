/*!
  @file lte_rrc_controller_irat_resel_from_lte_fail_test.cpp

  @brief
  Unit test code for basic RRC controller state transitions. Take it from
  Inactive -> Idle Camped -> Idle Camped -> Suspended -> Idle Camped
...
--------   ---     ----------------------------------------------------------
12/04/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerIratReselFromLteFailTest);

void LteRrcControllerIratReselFromLteFailTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 8);
}

void LteRrcControllerIratReselFromLteFailTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG("Sending Resume Cnf");
  msgr_init_hdr(&resume_cnf.msgr_hdr,MSGR_LTE_CPHY,LTE_CPHY_RESUME_CNF);
  resume_cnf.status = E_SUCCESS;  
  lte_rrc_utf_send_msg((byte*)&resume_cnf, sizeof(resume_cnf));
...
void LteRrcControllerIratReselFromLteFailTest::Teardown()
{
