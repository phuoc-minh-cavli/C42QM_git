/*!
  @file lte_rrc_controller_idle_not_camped_suspended_stop_test.cpp

  @brief
  Take controller from Inactive -> IdleNotCamped. Then send Mode Change Request
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerIdleNotCampedSuspendedStopTest);

void LteRrcControllerIdleNotCampedSuspendedStopTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 7);
}

void LteRrcControllerIdleNotCampedSuspendedStopTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  TF_MSG(
 "Sending Mode Change Req to Suspend,type = Stop");
  lte_rrc_init_default_hdr(&mc_req.msg_hdr,LTE_RRC_MODE_CHANGE_REQI);
...
void LteRrcControllerIdleNotCampedSuspendedStopTest::Teardown()
{
