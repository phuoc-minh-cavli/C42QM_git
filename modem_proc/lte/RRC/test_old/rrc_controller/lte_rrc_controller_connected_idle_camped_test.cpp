/*!
  @file lte_rrc_controller_connected_idle_camped_test.cpp

  @brief
  Unit test code for basic RRC controller state transitions. Take it from
  Inactive -> Connected -> Idle Camped 
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerConnectedIdleCampedTest);

void LteRrcControllerConnectedIdleCampedTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 6);
}

void LteRrcControllerConnectedIdleCampedTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcControllerConnectedIdleCampedTest::Teardown()
{
