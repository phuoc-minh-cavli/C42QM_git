/*!
  ...
  Inactive -> Connected -> Closing -> Idle Camped -> Inactive
...
09/03/08   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerBasicTest);

void LteRrcControllerBasicTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 12);
}

void LteRrcControllerBasicTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  ...
  mc_req.lte_band_pref.bits_1_64 = 1;
  ...
  TF_ASSERT(LTE_RRC_MODE_CHANGE_CNFI == mc_cnf_ptr->msg_hdr.id);
  TF_ASSERT(TRUE == mc_cnf_ptr->status);
  TF_ASSERT(LTE_RRC_MODE_CHANGE_DEACTIVATE == mc_cnf_ptr->mode);
...
void LteRrcControllerBasicTest::Teardown()
{
  lte_rrc_utf_teardown();
}

TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerBasicTestCat4);

void LteRrcControllerBasicTestCat4::Setup()
{
  int oflag, mode;
	uint8 ca_disable = 0x01;
  oflag = O_RDWR | O_AUTODIR | O_TRUNC;
  mode = ACCESSPERMS;  
    TF_ASSERT(efs_put("/nv/item_files/modem/lte/common/ca_disable"  , 
				  &ca_disable, sizeof(uint8), oflag, mode) >= 0); 
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 12);
}

void LteRrcControllerBasicTestCat4::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
  lte_cphy_start_req_s *start_req_ptr;
  ...
  start_req_ptr = (lte_cphy_start_req_s *)(void*) buf_ptr;
  TF_ASSERT(LTE_CPHY_START_REQ == start_req_ptr->msgr_hdr.id); 
	TF_ASSERT(start_req_ptr->cap_info.valid==TRUE);
	TF_ASSERT(start_req_ptr->cap_info.ue_cat==LTE_UE_CATEGORY-2);
...
void LteRrcControllerBasicTestCat4::Teardown()
{
  lte_rrc_utf_teardown();
}
