/*!
  @file lte_rrc_controller_deact_idle_not_camped_test.cpp

  @brief
  Unit test code for basic RRC controller state transitions. Take it from
  Inactive -> Idle Not Camped -> Inactive
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerDeactIdleNotCampedTest);

void LteRrcControllerDeactIdleNotCampedTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 12);
}

void LteRrcControllerDeactIdleNotCampedTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcControllerDeactIdleNotCampedTest::Teardown()
{
