/*!
  @file lte_rrc_controller_connected_suspended_stop_closing_test.cpp

  @brief
  Take controller from Inactive -> Connected. Then send Mode Change Request
  with Suspend and type = Stop, and change to Suspended. Then close the 
  connection
...
08/20/09   sureshs Initial Revision
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcControllerConnectedSuspendedStopClosingTest);

void LteRrcControllerConnectedSuspendedStopClosingTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CONTROLLER_SM, ext_umid_list, 12);
}

void LteRrcControllerConnectedSuspendedStopClosingTest::Test()
{
  byte *buf_ptr;
  uint32 buf_len;
...
void LteRrcControllerConnectedSuspendedStopClosingTest::Teardown()
{
