/*!
  @file lte_rrc_cre_rach_problem_and_t301_expiry_test.cpp

  @brief
  Unit test code for CRE procedure that tests the handling of Random
  access problem indication followed by T301 expiry. Also checks CRE validation
  by sending a Random access problem indication for a reason not relevant to CRE.
...
lte_rrc_cre_rach_problem_and_t301_expiry_test.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/24/09   mm     Changed the test to verify T301 expiry handling
...
===========================================================================*/
#define LTE_RRC_CONFIG_TEST_T301_TMRI_VAL 100
/*===========================================================================
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCreRachProblemT301ExpiryTest);

void LteRrcCreRachProblemT301ExpiryTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRE_SM, ext_umid_list, 3);
  lte_rrc_cre_test_stub_init();
}

void LteRrcCreRachProblemT301ExpiryTest::Test()
{
  ...
  lte_rrc_tmri_s *t301_tmri_ptr = NULL;
  ...
  TF_MSG("\nCRE Random access problem and T301 expiry test begins \n");

  /* Make sure CRE is in initial state on receiving a RLF indication */
  TF_ASSERT(lte_rrc_cre_get_state() == LTE_RRC_CRE_SM__INITIAL);

  /* Setting the header for Random access problem indication for a reason
    not relevant to CRE. Should be ignored */
  lte_rrc_init_default_hdr(&access_prob_ind.hdr, LTE_MAC_RANDOM_ACCESS_PROBLEM_IND);
  access_prob_ind.access_reason = LTE_MAC_ACCESS_REASON_HAND_OVER;
  ...
  TF_MSG("T301 timer expires");
  TF_SLEEP(LTE_RRC_CONFIG_TEST_T301_TMRI_VAL);

  /* Catch the T301 Timer expiry */
  lte_rrc_utf_get_next_int_msg((byte **)&t301_tmri_ptr,&buf_len);
  ...
  TF_MSG("\nCRE Random access problem and T301 expiry test ends \n");
  lte_rrc_utf_reset(); 

  /* ---------------------------------------------------------------------- */
}

void LteRrcCreRachProblemT301ExpiryTest::Teardown()
{
