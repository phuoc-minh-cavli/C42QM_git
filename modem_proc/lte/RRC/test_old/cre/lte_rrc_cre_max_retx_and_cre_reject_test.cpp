/*!
  @file lte_rrc_cre_max_retx_and_cre_reject_test.cpp

  @brief
  Unit test code for CRE procedure that tests the handling of max retransmission 
  indication from RLC followed by Connection reestablishment which results in
  ReestablishmentReject being received from the network.
...
lte_rrc_cre_max_retx_and_cre_reject_test.cpp#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/24/09   mm     Changed the test to verify handling of CRE Reject from the NW
                  caused by max retransmission indication from RLC
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCreMaxRetxAndCRERejectTest);

void LteRrcCreMaxRetxAndCRERejectTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRE_SM, ext_umid_list, 2);
  lte_rrc_cre_test_stub_init();
}

void LteRrcCreMaxRetxAndCRERejectTest::Test()
{
  lte_rlcul_max_retx_ind_s max_retx_ind;
  ...
  TF_MSG("\nCRE Max retx and ReestablishmentReject test begins \n");
  ...
  lte_rrc_init_default_hdr(&max_retx_ind.msg_hdr,LTE_RLCUL_MAX_RETX_IND);
  max_retx_ind.rb_cfg_idx = 2;
  lte_rrc_utf_send_msg((byte*)&max_retx_ind, sizeof(max_retx_ind));
  ...
  lte_rrc_init_default_hdr(&mh_dlm.msg_hdr,
                           LTE_RRC_RRC_CONNECTION_REESTABLISHMENT_REJECT_DLM);
  ...
  dl_ccch_msg_ptr->message.u.c1->t = T_lte_rrc_osys_DL_CCCH_MessageType_c1_rrcConnectionReestablishmentReject;
  dl_ccch_msg_ptr->message.u.c1->u.rrcConnectionReestablishmentReject->criticalExtensions.
    t = T_lte_rrc_osys_RRCConnectionReestablishmentReject_criticalExtensions_rrcConnectionReestablishmentReject_r8;
  dl_ccch_msg_ptr->message.u.c1->u.rrcConnectionReestablishmentReject->criticalExtensions.
    u.rrcConnectionReestablishmentReject_r8 = (lte_rrc_osys_RRCConnectionReestablishmentReject_r8_IEs *)
  lte_rrc_asn1_osys_IE_malloc(sizeof(lte_rrc_osys_RRCConnectionReestablishmentReject_r8_IEs));
  ...
  LTE_RRC_OSYS_ASN1_COPY_PDU(dl_ccch_msg_ptr,
							 mod_dl_ccch_msg_ptr, DL_CCCH_Message);
  ...
  dl_ccch_msg_ptr->message.u.c1->u.rrcConnectionReestablishmentReject->criticalExtensions.t
    = T_lte_rrc_osys_RRCConnectionReestablishmentReject_criticalExtensions_rrcConnectionReestablishmentReject_r8;
  ...
  TF_MSG("\nsizeof(lte_rrc_initiate_conn_rel_indi_s) = %d \n", sizeof(lte_rrc_initiate_conn_rel_indi_s));
  ...
  TF_MSG("\nCRE Max retx and ReestablishmentReject test ends \n");
  lte_rrc_utf_reset(); 

  /* ---------------------------------------------------------------------- */
}

void LteRrcCreMaxRetxAndCRERejectTest::Teardown()
{
