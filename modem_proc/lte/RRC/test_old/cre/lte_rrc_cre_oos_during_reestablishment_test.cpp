/*!
  ...
  access problem indication followed by OOS during reestablishment. 
...
lte_rrc_cre_oos_during_reestablishment_test.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcCreOOSDuringReestablishmentTest);

void LteRrcCreOOSDuringReestablishmentTest::Setup()
{
  lte_rrc_utf_setup(LTE_RRC_CRE_SM, ext_umid_list, 3);
  lte_rrc_cre_test_stub_init();
}

void LteRrcCreOOSDuringReestablishmentTest::Test()
{
  ...
  lte_cphy_oos_ind_s cphy_oos_ind;
  ...
  TF_MSG("\nOOS during reestablishment test begins \n");
  ...
  /* At this point, send LTE_CPHY_OOS_IND */
  lte_rrc_init_default_hdr(&cphy_oos_ind.msgr_hdr, LTE_CPHY_OOS_IND);
  cphy_oos_ind.serv_cell_info.phy_cell_id = 500;
  cphy_oos_ind.serv_cell_info.dl_cell_frequency = 10689;
  /* Send the OOS_IND to CRE module there by mimicing the action of ML1 */
  lte_rrc_utf_send_msg((byte*)&cphy_oos_ind, sizeof(cphy_oos_ind));
  ...
  TF_MSG("\nOOS during reestablishment test ends \n");
  lte_rrc_utf_reset(); 

  /* ---------------------------------------------------------------------- */
}

void LteRrcCreOOSDuringReestablishmentTest::Teardown()
{
