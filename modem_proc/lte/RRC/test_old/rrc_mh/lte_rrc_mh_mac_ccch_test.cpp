...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/rrc_mh/lte_rrc_mh_mac_ccch_test.cpp#1 $
...
===========================================================================*/
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcMHMacCcchTest);

void LteRrcMHMacCcchTest::Setup()
{
...
void LteRrcMHMacCcchTest::Test()
{
  ...
  msgr_init_hdr_attach(&dl_msg_ind.hdr, MSGR_LTE_MAC, LTE_MAC_RRC_CCCH_DL_DATA_IND, 0, 1);

  length = 2;
  encoded_msg = (unsigned char *)lte_rrc_mem_calloc(2);
  *encoded_msg = 0x40;
  *(encoded_msg + 1) = 0x60;
  ...
  TF_ASSERT(lte_rrc_mh.dd_ptr->dl_hist[0].encoded_msg[0] == 0x40);
  TF_ASSERT(lte_rrc_mh.dd_ptr->dl_hist[0].encoded_msg[1] == 0x60);
  TF_ASSERT(lte_rrc_mh.dd_ptr->dl_hist[0].length == 2);
  TF_ASSERT(lte_rrc_mh.dd_ptr->dl_hist[0].pdu_num == lte_rrc_osys_DL_CCCH_Message_PDU);
  ...
  TF_ASSERT(dlm->pdu_num == lte_rrc_osys_DL_CCCH_Message_PDU);

  lte_rrc_utf_reset();
}


void LteRrcMHMacCcchTest::Teardown()
{
