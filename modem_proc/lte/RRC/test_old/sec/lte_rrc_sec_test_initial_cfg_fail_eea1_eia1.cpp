...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/sec/lte_rrc_sec_test_initial_cfg_fail_eea1_eia1.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcSecInitialSecurityConfigFailTest1);

static uint32 ext_umid_list[20];


void LteRrcSecInitialSecurityConfigFailTest1::Setup()
{
  //uint8 ext_count = 0;
  uint8 ext_count = 1;
...
void LteRrcSecInitialSecurityConfigFailTest1::Test()
{
  ...
  /*Set the flush NAS security context to TRUE*/
  ...
  /*The following is valid for BEARER == 1*/
  /*
 ...
 *((uint8 *)(ulm_ptr->encode_status)) = 0; 
 ...
 //Let the SMC_FAIL_TIMER expire
 TF_SLEEP(LTE_RRC_SEC_SMC_FAIL_TIME+1);


 lte_rrc_utf_reset();
 }

void LteRrcSecInitialSecurityConfigFailTest1::Teardown()
{
