...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/sec/lte_rrc_sec_test_sim_update1.cpp#1 $
...
DECLARE_LTE_RRC_SEC_SMC_EEA0_EIA0;
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcSecSimUpdateTest1);

static uint32 ext_umid_list[20];


void LteRrcSecSimUpdateTest1::Setup()
{
  uint8 ext_count = 0;
...
void LteRrcSecSimUpdateTest1::Test()
{
  ...
  lte_rrc_sec_test_encoded_pdu.value = sec_smc_1;
  lte_rrc_sec_test_encoded_pdu.length = sizeof(sec_smc_1);

  smc_dlm.dl_msg_ptr = (lte_rrc_osys_DL_DCCH_Message *)lte_rrc_osys_asn1_decode_pdu(sec_smc_1, sizeof(sec_smc_1),
                               DL_DCCH_Message,
                               &decode_status);
 ...
 *((uint8 *)(ulm_ptr->encode_status)) = 0; 
 ...
 TF_ASSERT(sec_mode_complete_ptr->criticalExtensions.u.securityModeComplete_r8->
           m.nonCriticalExtensionPresent == 0);
  
 lte_rrc_utf_reset();
 }

void LteRrcSecSimUpdateTest1::Teardown()
{
