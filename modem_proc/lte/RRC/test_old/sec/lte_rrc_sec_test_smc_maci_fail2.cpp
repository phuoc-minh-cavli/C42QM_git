...
$Header: //components/rel/lte.mpss/4.7.2/RRC/test_old/sec/lte_rrc_sec_test_smc_maci_fail2.cpp#1 $
...
TF_DEFINE_TEST_CASE(COMPONENT, LteRrcSecSmcMACIFailTest2);

static uint32 ext_umid_list[20];


void LteRrcSecSmcMACIFailTest2::Setup()
{
  uint8 ext_count = 1;
...
void LteRrcSecSmcMACIFailTest2::Test()
{
  ...
  */
  ...
  /*Set the flush NAS flag to FALSE*/
...
/*Expect a Config Req from CEP*/
 ...
 TF_ASSERT((pdcpdl_reset_req_ptr->hdr.id) ==  
  ...
  TF_MSG("Sending the ConnReleased");
  ...
  conn_mode_fail_indi_ptr =  
    (lte_rrc_conn_mode_failure_indi_s *)buf_ptr;
...
void LteRrcSecSmcMACIFailTest2::Teardown()
{
