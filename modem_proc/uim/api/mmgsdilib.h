...
Copyright (c) 2004 - 2019, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.  All
...
$Header: //components/rel/uim.mpss/3.3.1/api/mmgsdilib.h#2 $$ $DateTime: 2023/06/21 11:32:16 $ $Author: pwbldsvc $
...
/**
  ...
  @param[in] refresh_enforcement_policy Enforcement policy sent by card
...
/*===========================================================================
  ...
  This API is used to get plmn name by directly accessing SE13 table for given PLMN Id
  Caller should allocate the memory for obtaining plmn name info.
  After returning from the function, it is the responsibility of the caller
  to free the memory.

  @param[in]   client_id:           Client ID
  @param[in]   plmn_id:             PLMN id
  @param[out]  plmn_name_info_ptr:  Contains PLMN name information. This info is valid when
...
/**
  ...
  disabled, otherwise.  If a test card MCC/MNC of the GW IMSI is present
  for the subscription, it will always return ok. If it is a commercial card,
  it will take into account the value in the protected NV item for test mode
  logging. SYS_MODEM_AS_ID_1 means mmgsdi will extract info from PRI session
  and SYS_MODEM_AS_ID_2 means from SEC session info.  

  @param[in] sub_id: The subscription ID number for this query
...
/**
  ...
  the protected NV item for test mode logging. 
  

  @param[in] slot_id: The slot ID number for this query
...
=========================================================================== */
/**
  We are deprecating the below functions. Currently, we are including them 
  in this header to support clients whose changes are not yet fully completed
*/
