...
Copyright (c) 2008-2020, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/api/gstk_exp_v.h#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/30/23   ac      Added Satellite E-UTRAN RAT
07/17/20   dt      Added support for VZW PCO command
03/29/18   dt      Change default CAT version to 12
...
10/04/16   dt      Fixed kw error:changed GSTK_CM_NUM_STACKS
...
/* Max. possible MSISDN length in a PCO envelope */
#define GSTK_MSISDN_LEN                          0x78
...
#define GSTK_CM_NUM_STACKS                       CM_NO_STACKS

#define GSTK_DS_SYS_MAX_AVAIL_SYS                DS_SYS_MAX_AVAIL_SYS
/*--------------------------------------------------------------------------
                             Type Defs
--------------------------------------------------------------------------*/
...
--  Enum: gstk_nmr_utran_qualifier_enum_type
      ...
      GSTK_NMR_LTE_INTRA_FREQ_MEAS      E-UTRAN/Satellite E-UTRAN Intra-frequency measurements
      GSTK_NMR_LTE_INTER_FREQ_MEAS      E-UTRAN/Satellite E-UTRAN Inter-frequency measurements
      GSTK_NMR_LTE_INTER_RAT_GERAN_MEAS E-UTRAN/Satellite E-UTRAN Inter-RAT (GERAN) measurements
      GSTK_NMR_LTE_INTER_RAT_UTRAN_MEAS E-UTRAN/Satellite E-UTRAN Inter-RAT (UTRAN) measurements
===========================================================================*/
typedef enum
{
...
typedef enum {
  ...
  /* Update below whenever new CAT version is added */
  GSTK_CFG_CAT_VER_LATEST              = GSTK_CFG_CAT_VER12,
...
/**
  @brief This is the type that is used to different network
         types. These values are used within GSTK and are mapped
         from ds_sys_network_enum_type
*/
typedef enum
{
  GSTK_DS_SYS_NETWORK_3GPP,     /**< Network type 3gpp */
  GSTK_DS_SYS_NETWORK_MAX
} gstk_ds_sys_network_enum_type;

/**
  @brief This is the type that is used to define different RATs
         types. These values are used within GSTK and are mapped
         from ds_sys_rat_ex_enum_type
*/
typedef enum
{
  GSTK_DS_SYS_RAT_EX_NULL_BEARER           = 0x0,
  /* 3GPP RAT Values */
  GSTK_DS_SYS_RAT_EX_3GPP_WCDMA             = 1,
  GSTK_DS_SYS_RAT_EX_3GPP_GERAN             = 2,
  GSTK_DS_SYS_RAT_EX_3GPP_LTE               = 3,
  GSTK_DS_SYS_RAT_EX_3GPP_TDSCDMA           = 4,
  GSTK_DS_SYS_RAT_EX_3GPP_WLAN              = 5,
  GSTK_DS_SYS_RAT_EX_3GPP_MAX               = 100,

  /* 3GPP2 RAT Values */
  GSTK_DS_SYS_RAT_EX_3GPP2_1X               = 101,
  GSTK_DS_SYS_RAT_EX_3GPP2_HRPD             = 102,
  GSTK_DS_SYS_RAT_EX_3GPP2_EHRPD            = 103,
  GSTK_DS_SYS_RAT_EX_3GPP2_WLAN             = 104,
  GSTK_DS_SYS_RAT_EX_3GPP2_MAX              = 200,

  /* WLAN RAT Values */
  GSTK_DS_SYS_RAT_EX_WLAN                   = 201,
  GSTK_DS_SYS_RAT_EX_WLAN_MAX               = 300,

  GSTK_DS_SYS_RAT_MAX
} gstk_ds_sys_rat_ex_enum_type;
...
typedef struct
{
  /* Do not change the sequence of first 4 variables in cnf_type of any new proactive command */
  gstk_hdr_type                      message_header;
...
typedef struct
{
  ...
  boolean                     time_data_avail; /* Used for G/W/L only */
...
{
  gstk_hdr_type                        message_header;
  gstk_poll_interval_evt_ind_type      poll_interval_evt_info;
}
gstk_envelope_poll_interval_evt_ind_type;

typedef struct
{
  gstk_hdr_type                         message_header;
  gstk_msisdn_info_type                msisdn_info;
}
gstk_envelope_pco_info_ind_type;
...
{ 
  ...
  boolean                       exit_from_psm_status; /**< psm mode enable status. */
}gstk_tp_dl_evt_info_type;
...
typedef struct
{
  ...
  union
  {
    ...
    gstk_tp_dl_evt_info_type                  terminal_profile;
...
typedef struct
{
  gstk_ds_sys_network_enum_type            technology; /**< technology type*/
  gstk_ds_sys_rat_ex_enum_type             rat_value;  /**< rat type*/
...
typedef struct
{
  ...
  gstk_ds_sys_system_status_info_type   avail_sys[GSTK_DS_SYS_MAX_AVAIL_SYS];
  /**< Array of current system status, the first element in the array will
       contain the preferred system */
}
gstk_ds_system_status_type;

typedef struct
{
   gstk_hdr_type                        message_header;
   gstk_ds_system_status_type           ds_sys_status_info;
...
{
   gstk_hdr_type                        message_header;
   ds_sys_event_msisdn_info_type        ds_msisdn_info;
   ds_sys_subscription_enum_type        subscription_id;   
}
gstk_ds_msisdn_change_ind_type;
...
typedef union
{
    ...
    gstk_envelope_poll_interval_evt_ind_type       poll_interval_envelope_ind;
    gstk_envelope_pco_info_ind_type                   pco_info_envelope_ind;    
    /* Envelope Response */
    ...
    /* ds MSISDN change notification */
    gstk_ds_msisdn_change_ind_type                 ds_msisdn_change_ind;
