...
$Header: //components/rel/uim.mpss/3.3.1/api/mmgsdisessionlib.h#1 $$ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $
...
FUNCTION:       MMGSDI_SESSION_OPEN_EXT_SYNC
  ...
  Caller should allocate the memory for obtaining session info based on the number of
  requested sessions. After returning from the function, it is the responsibility of the caller
  to free the memory.
  ...
  @param[in] evt_cb_ptr:          Application event callback. A client must provide
                                  an event callback if it is to receive session-related event
                                  notifications. If the pointer is NULL, the client receives no
                                  event notifications.
  @param[out] mmgsdi_session_info_ptr: Contains session info. It is valid only when return value
                                       of this function is MMGSDI_SUCCESS.
...
..@dependencies
  ...
  @limitations
  If any one of the requested session fails, all other opened sessions will be cleaned up and will  
...
FUNCTION:       MMGSDI_SESSION_REGISTER_FOR_REFRESH_SYNC
  ...
  vote_for_init:        Vote to start the refresh
  proceed_with_refresh: Vote to bring down the call stack.
...
FUNCTION:       MMGSDI_SESSION_GET_OPERATOR_NAME_EXT_SYNC
  ...
  operator name info based on the number of number of plmn ids passed.
