...
Copyright (c) 2008 - 2017, 2020 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/api/uim_v.h#2 $
...
03/10/17   ku      Support two physical slots for IoE PLs
...
#elif defined (FEATURE_UIM_SUPPORT_DUAL_SLOTS) || defined (FEATURE_UIM_ENABLE_SLOT2)
#define UIM_MAX_INSTANCES        2
#else
#define UIM_MAX_INSTANCES        1
...
/*Resume token size */
#define UIM_RESUME_TOKEN_LENGTH         0x08
...
typedef enum
{
  ...
  UIM_SUSPEND_R,                       /*UIM suspend response*/
  UIM_RESUME_R,                        /*UIM Resume response*/
  UIM_MAX_R
...
typedef enum {
  ...
  SW2_APPLN_SPECIFIC_SECURITY_SESSION_EXPIRED  = 0x63,
  SW2_APPLN_SPECIFIC_MIN_SUSP_TIME_TOO_LONG    = 0x64,
...
typedef struct {
  ...
  uint8                    t_actual; /* actualsupend time */
  uint8   token[UIM_RESUME_TOKEN_LENGTH];/*token  */
} uim_suspend_rpt_type;
...
typedef struct
{
  ...
  union
  {
    ...
    uim_suspend_rpt_type                 suspend_data;
...
typedef enum {
  ...
  /*0x600 => 1536 8*/
  UIM_SUSPEND_F           =0x0600, /*Suspend  UIM*/
  UIM_RESUME_F,                    /*Resume UIM*/ 
...
typedef union {
  ...
  uim_suspend_cmd_type                  suspend_cmd;          /*To set suspend command to card*/
  uim_resume_token_type                 resume_cmd;
} uim_cmd_type;
