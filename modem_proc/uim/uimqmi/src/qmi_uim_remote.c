...
Copyright (c) 2013 - 2018, 2022 QUALCOMM Technologies, Incorporated and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/uimqmi/src/qmi_uim_remote.c#2 $ $DateTime: 2022/04/22 09:52:56 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/22/22   ykr     Fix to integer overflow issue in uim_remote_apdu handler
01/24/19   ng      Invalidate the slot on receiving conn_unavail req
12/17/18   bcho    Remove use of service_enable NV as already featurized.
10/12/18   bcho    RCINIT Optimization
02/15/18   ku      Fix KW errors 
01/09/18   vr      Initialize secure decrypted apdu response buffer
11/01/17   ku      Fix Compilation error on enabling Remote flag
09/27/17   nc      UIM RMT Secure Channel 
03/11/16   vr      Initialize the client_register slot value to invalid
...
#define QMI_UIM_EXTRA_ENC_BYTES            100

/* Maximum UINT32 value */
#define UIM_UINT_MAX                       0xFFFFFFFF
...
/*===========================================================================
   FUNCTION:      QMI_UIM_REMOTE_NV_SECURE_CHANNEL_INIT

   DESCRIPTION:
     This function initializes the NV secure channle for QMI UIM REMOTE
   ...
   SIDE EFFECTS:
     None
===========================================================================*/
static void qmi_uim_remote_nv_secure_channel_init (
  void
)
{
  ...
  if (qmi_uim_remote_state_ptr == NULL ||
      qmi_uim_remote_state_ptr->encryption_mandatory != QMI_UIM_REMOTE_NV_STATUS_NOT_SET)
  {
    return;
  }

  if (UIM_COMMON_EFS_SUCCESS == uim_common_efs_read(UIM_COMMON_EFS_UIMQMI_QMI_UIM_REMOTE_SECURE_CHANNEL,
                                                    UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                                    UIM_COMMON_EFS_DEVICE,
                                                   (uint8 *)&(is_enabled),
                                                    sizeof(is_enabled)))
  {
    qmi_uim_remote_state_ptr->encryption_mandatory = (is_enabled == FALSE)? QMI_UIM_REMOTE_NV_STATUS_FALSE:QMI_UIM_REMOTE_NV_STATUS_TRUE;
  }
  else
  {
    /* When NV item on encryption is not set, the default behavior is
       to enable encryption. This would prevent potential manipulation of the
       messages exchanged, in case encryption is not enforced in the modem. */
    qmi_uim_remote_state_ptr->encryption_mandatory = QMI_UIM_REMOTE_NV_STATUS_TRUE;
  }
} /* qmi_uim_remote_nv_secure_channel_init */
...
)
{
  ...
  memset(&evt, 0x0, sizeof(qmi_uim_remote_event_data_type));
  memset(&resp, 0x0, sizeof(uim_remote_event_resp_msg_v01));
       ...
       card_client_index++)
  {
          ...
          card_client_handle_ptr->client_handle)
    {
       ...
       if (evt.evt == QMI_UIM_REMOTE_EVT_CONNECTION_AVAILABLE)
       {
         ...
         /* Check the NV for enc/not enc configuration.
            1. If NV enforces encryption and QMI connect request does not,
            reject the request
            2. If NV enforces encryption and QMI connect request has encryption TLV,
            set the flag in global and allow connection
            3. If NV does not enforce encryption, then whatever HLOS requests
            is stored in global and handle accordingly. */

         /* Check if encryption is enforced by the NV */
         qmi_uim_remote_nv_secure_channel_init();

         if (qmi_uim_remote_state_ptr->encryption_mandatory == QMI_UIM_REMOTE_NV_STATUS_TRUE) 
         {
           /* Check if the encryption TLV is present */
           if (req_msg_ptr->encryption_valid &&
               req_msg_ptr->encryption == UIM_REMOTE_ENCRYPTION_ALL_APDUS_V01) 
           {
             qmi_uim_remote_state_ptr->card_client_registry[card_client_index].encryption_used = TRUE;
           }
           else
           {
             /* Reject connection */
             UIM_MSG_HIGH_0("Encryption enforced by NV. No valid TLV. Reject connection");
             return QMI_CSI_CONN_REFUSED;
           }
        }
        else
        {
          /* Do what the TLV says */
          if (req_msg_ptr->encryption_valid) 
          {
            if (req_msg_ptr->encryption == UIM_REMOTE_ENCRYPTION_NONE_V01) 
            {
              qmi_uim_remote_state_ptr->card_client_registry[card_client_index].encryption_used = FALSE;
            }
            else if (req_msg_ptr->encryption == UIM_REMOTE_ENCRYPTION_ALL_APDUS_V01) 
            {
              qmi_uim_remote_state_ptr->card_client_registry[card_client_index].encryption_used = TRUE;
            }
            else
...
)
{
  ...
  qmi_uim_remote_rsp_apdu_data_type        dec_rsp;
  ...
  memset(&dec_rsp, 0x00, sizeof(dec_rsp));
  ...
  /* Check for Interger overflow in APDU response data */
  if(req_msg_ptr->response_apdu_info.response_apdu_segment_offset >
                UIM_UINT_MAX - req_msg_ptr->response_apdu_segment_len)
  {
    UIM_MSG_ERR_2("qmi_uim_remote_apdu_req_handler: Integer Overflow in response data, offset: 0x%x, len: 0x%x",
	               req_msg_ptr->response_apdu_info.response_apdu_segment_offset, req_msg_ptr->response_apdu_segment_len);
    resp.resp.error  = QMI_ERR_INTERNAL_V01;
    resp.resp.result = QMI_RESULT_FAILURE_V01;
    goto send_resp;
  }
  ...
  /* If the APDU segment is encrypted, decrypt the payload
       and copy the segment into the allocated buffer */
  if (card_client_ptr->encryption_used) 
  {
    uint32 decrypted_apdu_len = req_msg_ptr->response_apdu_info.total_response_apdu_size;

    /* The decrypted buffer is smaller than the encrypted buffer. So, it is
       safe to allocate a buffer of the same size for decryption. */
    dec_rsp.rsp_apdu_ptr =
       modem_mem_calloc(1, decrypted_apdu_len, MODEM_MEM_CLIENT_UIM);

    if (dec_rsp.rsp_apdu_ptr == NULL) 
    {
    ...
    if(!qmi_uim_remote_decrypt_payload(rsp_ptr->rsp_apdu_ptr,
                                       req_msg_ptr->response_apdu_info.total_response_apdu_size,
                                       dec_rsp.rsp_apdu_ptr,
                                       &decrypted_apdu_len))
    {
      UIM_MSG_ERR_0("Payload decryption failed");
      resp.resp.error  = QMI_ERR_INTERNAL_V01;
      resp.resp.result = QMI_RESULT_FAILURE_V01;
      goto free_mem;
    }

    dec_rsp.rsp_apdu_len = decrypted_apdu_len;
  }
...
callback_and_send_resp:
  if (card_client_ptr->curr_apdu.rsp_apdu_cb_ptr)
  {
     if (card_client_ptr->encryption_used) 
     {
       dec_rsp.result = rsp_ptr->result;
       dec_rsp.slot = rsp_ptr->slot;

       (card_client_ptr->curr_apdu.rsp_apdu_cb_ptr)(&dec_rsp);
     }
     else
     {
...
free_mem:
  ...
  if (dec_rsp.rsp_apdu_ptr) 
  {
    modem_mem_free(dec_rsp.rsp_apdu_ptr, MODEM_MEM_CLIENT_UIM);
    dec_rsp.rsp_apdu_ptr = NULL;
  }
...
)
{
  ...
  *ind_c_struct_ptr =
  ...
  /* If encryption is enforced, encrypt the APDU IND */
  if (qmi_uim_remote_state_ptr->card_client_registry[card_client_index].encryption_used) 
  {
    mmgsdi_data_type encrypted_apdu_ind = {0, NULL};
    mmgsdi_data_type plain_apdu_ind     = {0, NULL};

    plain_apdu_ind.data_len = cmd_ptr->cmd_data.send_apdu_req.cmd_apdu_len;
    plain_apdu_ind.data_ptr = cmd_ptr->cmd_data.send_apdu_req.cmd_apdu_ptr;

    if(!qmi_uim_remote_encrypt_payload(plain_apdu_ind, &encrypted_apdu_ind))
    {
      UIM_MSG_ERR_0("Encrypt APDU IND failed");
      return QMI_CSI_INTERNAL_ERR;
    }

    if ((encrypted_apdu_ind.data_len > QMI_UIM_REMOTE_MAX_ENC_COMMAND_APDU_LEN_V01) ||
         encrypted_apdu_ind.data_ptr == NULL) 
    {
      UIM_MSG_ERR_1("Invalid encrypted APDU IND length 0x%x ",
                    encrypted_apdu_ind.data_len);

      /* Free the encrypted IND since we do not need it anymore.*/
      if (encrypted_apdu_ind.data_ptr) 
      {
        modem_mem_free(encrypted_apdu_ind.data_ptr, MODEM_MEM_CLIENT_UIM);
        encrypted_apdu_ind.data_ptr = NULL;
      }
    ...
    ((uim_remote_apdu_ind_msg_v01 *)(*ind_c_struct_ptr))->encrypted_command_apdu_len =
              encrypted_apdu_ind.data_len;

    ((uim_remote_apdu_ind_msg_v01 *)(*ind_c_struct_ptr))->encrypted_command_apdu_valid = TRUE;

    memscpy(((uim_remote_apdu_ind_msg_v01 *)(*ind_c_struct_ptr))->encrypted_command_apdu,
              QMI_UIM_REMOTE_MAX_ENC_COMMAND_APDU_LEN_V01,
              encrypted_apdu_ind.data_ptr,
              encrypted_apdu_ind.data_len);

    modem_mem_free(encrypted_apdu_ind.data_ptr, MODEM_MEM_CLIENT_UIM);
    encrypted_apdu_ind.data_ptr = NULL;

    /* When APDUs are encrypted, the legacy TLV has length 0 */
    ((uim_remote_apdu_ind_msg_v01 *)(*ind_c_struct_ptr))->command_apdu_len = 0;
  }
  else
  {
...
)
{
  if(qmi_uim_remote_state_ptr == NULL)
  {
    /* We consider it as command queue is empty and don't process the command */
    return TRUE;
  }
