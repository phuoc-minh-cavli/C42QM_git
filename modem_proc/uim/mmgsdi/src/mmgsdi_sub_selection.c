...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_sub_selection.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/13/16   sn      Support for CDMALess Device
05/02/16   ar      Removing the mmgsdi_cnf_map_req_to_cnf_enum_type()
04/01/16   bcho    gsdi_efs_read_file() replaced gsdi_efs_read_file_with_len()
...
)
{
  ...
  if(mmgsdi_subs_prov_info.is_efs_content_valid == FALSE)
  {
    ...
    file_length = sizeof(mmgsdi_subs_prov_data_type) * MMGSDI_MAX_NUM_SLOTS;
    mmgsdi_status = gsdi_efs_read_file((const char *)mmgsdi_efs_subs_prov_info,
                                       &file_length,
                                       (uint8 *)mmgsdi_subs_prov_info.subs_prov_data,
                                       UIM_COMMON_EFS_DEVICE);
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  if(cnf != MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ ||
     cnf_ptr == NULL)
  {
    return;
  }
