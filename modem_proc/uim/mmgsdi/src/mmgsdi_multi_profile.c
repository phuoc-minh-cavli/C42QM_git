...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_multi_profile.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/24/18   rps     Refragment alloc extra param code
11/15/16   vdc     Change parsing logic to get current active profile
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_SET_SIM_PROFILE_REQ,
                                                           (void*)set_sim_profile_req_ptr,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
