...
Copyright (c) 2007 - 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_refresh.c#1 $$ $DateTime: 2020/07/22 10:53:27 $

when       who      what, where, why
--------   ---      ---------------------------------------------------------- 
06/26/19   bcho     Added caching support for EF-ACL
06/13/19   bcho     Rel 12 refresh enforcement policy support
05/01/19   bcho     Remove EF-HPPLMN from call stack file list
04/10/19   bcho     Fix KW P1 issues
...
10/16/17   srm      Check app is prov type before finding call stack files
09/04/17   ra       Don't allow write if REFRESH_INIT is in progress
11/10/16   me       Handle FCN refresh for EF-NASCONFIG
07/14/16   ar       Merge refresh retry nv functions
...
05/04/16   vdc      Return error if provided session is invalid for refresh reg
...
/* Refresh file list for GW */
static mmgsdi_file_enum_type mmgsdi_refresh_gw_reg_file_list[] =
{
  ...
  MMGSDI_USIM_OCSGT,    MMGSDI_TELECOM_ADN,    MMGSDI_USIM_MSISDN,    MMGSDI_USIM_ACL
};

/* Refresh file list for 1X */
#ifdef FEATURE_MMGSDI_3GPP2
static mmgsdi_file_enum_type mmgsdi_refresh_1x_reg_file_list[] =
{
...
static uint8 mmgsdi_refresh_retry_table_index = 0;
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  if (status != MMGSDI_SUCCESS)
  {
    ...
    if(cnf == MMGSDI_SESSION_APP_RESET_ACTIVATE_REQ)
    {
  ...
  const mmgsdi_request_header_type          *request_hdr_ptr)
{
  ...
  while(node_ptr)
  {
       ...
       node_ptr->stage == extra_info_ptr->stage)
    {
      ...
      evt_info_ptr->data.refresh.enforcement_policy                        = mmgsdi_refresh_info_ptr[slot_index]->enforcement_policy;
  ...
  mmgsdi_file_enum_type     file)
{
  switch(file)
  {
    ...
    /* It is decided to change Refresh mode from FCN to INIT 
       by adding these files in call stack file list instead.
       So that client does not need to read these files during Refresh FCN.
    */
    ...
    case MMGSDI_REFRESH_STAGE_START:
      ...
      switch(event_ptr->data.refresh.mode)
      {
        ...
        case MMGSDI_REFRESH_NAA_FCN:
        {
          ...
          {
            uint8 mmgsdi_gw_refresh_file_count = 0;
            ...
            mmgsdi_gw_refresh_file_count = sizeof(mmgsdi_refresh_gw_reg_file_list) /
                                             sizeof(mmgsdi_refresh_gw_reg_file_list[0]);

            for(j = 0; j < mmgsdi_gw_refresh_file_count; j++)
            {
              if(event_ptr->data.refresh.refresh_files.file_list_ptr[i] ==
                 mmgsdi_refresh_gw_reg_file_list[j])
              {
                break;
              }
            }
            if(j == mmgsdi_gw_refresh_file_count)
            {
              file_1x_found = FALSE;

#ifdef FEATURE_MMGSDI_3GPP2
              for(j = 0;
                  j < sizeof(mmgsdi_refresh_1x_reg_file_list) /sizeof(mmgsdi_refresh_1x_reg_file_list[0]);
                  j++)
              {
                if(event_ptr->data.refresh.refresh_files.file_list_ptr[i] ==
                   mmgsdi_refresh_1x_reg_file_list[j])
                {
                  file_1x_found = TRUE;
                  break;
                }
                    ...
                    MMGSDI_GSM_CPHSI)
            {
              ...
              mmgsdi_status = mmgsdi_cphs_post_pin1_init(
                event_ptr->session_id,
                event_ptr->data.refresh.slot,
                event_ptr->data.refresh.aid.app_type,
                read_cphs_efs);
            ...
            {
              mmgsdi_status = mmgsdi_hidden_key_refresh_process_fcn(
                                event_ptr,
                                event_ptr->data.refresh.refresh_files.file_list_ptr[i]);
            }
            else if(event_ptr->data.refresh.refresh_files.file_list_ptr[i] ==
                     ...
                     (mmgsdi_util_is_imsi_plmn_cust_plmn(event_ptr->data.refresh.slot)))
            {
              ...
              MMGSDIUTIL_TMC_MEM_FREE(file_buffer_ptr);
                     ...
                     event_ptr->data.refresh.refresh_files.file_list_ptr[i] == MMGSDI_TELECOM_ADN)
            {
...
)
{
  ...
  {
    refresh_files.num_files     = sizeof(mmgsdi_refresh_gw_reg_file_list) /
                                    sizeof(mmgsdi_refresh_gw_reg_file_list[0]);
    refresh_files.file_list_ptr = mmgsdi_refresh_gw_reg_file_list;
  }
  ...
  {
    refresh_files.num_files     = sizeof(mmgsdi_refresh_1x_reg_file_list) /
                                    sizeof(mmgsdi_refresh_1x_reg_file_list[0]);
    refresh_files.file_list_ptr = mmgsdi_refresh_1x_reg_file_list;
  }
#endif /* FEATURE_MMGSDI_3GPP2 */
  else
  {
  ...
  (void)mmgsdi_session_register_for_refresh_sync(session_id,
                                            refresh_files,
                                            TRUE,
                                            FALSE);
  ...
  if((mmgsdi_refresh_info_ptr[slot_index]->aid.app_type == MMGSDI_APP_USIM) ||
     ...
     ((mmgsdi_refresh_info_ptr[slot_index]->aid.app_type == MMGSDI_APP_UNKNOWN) &&
        mmgsdi_util_is_isim_aid(mmgsdi_refresh_info_ptr[slot_index]->aid.aid.data_ptr,
                                (uint8)mmgsdi_refresh_info_ptr[slot_index]->aid.aid.data_len)))
  {
...
)
{
  ...
  if(mmgsdi_util_get_session_app_info(session_ptr->session_id,
                                      &slot_id,
                                      NULL,
                                      NULL,
                                      NULL,
  ...
  /* Check if the app is provisioning app */
  if(mmgsdi_util_is_prov_app(&mmgsdi_refresh_info_ptr[slot_index]->aid,
                              mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id,
                              &is_prov) != MMGSDI_SUCCESS)
  {
    return MMGSDI_ERROR;
  }
  ...
  if(!is_reg_by_path)
  {
    ...
    if(session_ptr->refresh_data.refresh_files.num_files > 0)
    {
      ...
      {
        /* check if the file is not a call stack file or session is not
           provisioning */
        if(mmgsdi_refresh_is_call_stack_file(
             refresh_files_ptr->file_list_ptr[i]) && is_prov)
        {
          continue;
        }
...
)
{
  ...
  do
  {
    ...
    /* When a INIT+FFCN or INIT refresh is underway, return TRUE so that the
       writes to ALL the EFs can be blocked */
    if(mmgsdi_refresh_data_info.refresh_mode == MMGSDI_REFRESH_NAA_INIT_FULL_FCN ||
       mmgsdi_refresh_data_info.refresh_mode == MMGSDI_REFRESH_NAA_INIT)
    {
      is_present = TRUE;
      break;
    }
  ...
  uint32                                 aid_index)
{
  ...
  mmgsdi_refresh_info_ptr[slot_index]->enforcement_policy = req_ptr->enforcement_policy;
  ...
  if(!is_reg_by_path)
  {
    if(session_ptr->refresh_data.refresh_files.num_files > 0)
    {
      for(j = 0; j < refresh_files_ptr->num_files; j++)
      {
        ...
        if(mmgsdi_refresh_is_call_stack_file(
              refresh_files_ptr->file_list_ptr[j])  && is_prov)
        {
           continue;
        }
        for(k=0;k<session_ptr->refresh_data.refresh_files.num_files;k++)
  ...
  mmgsdi_refresh_mode_enum_type        mode)
{
  ...
  if(req_ptr->refresh_files.data_len > 0 && mode == MMGSDI_REFRESH_3G_SESSION_RESET)
  {
    ...
    {
      if(!(mmgsdi_refresh_is_call_stack_file(
             refresh_files.file_list_ptr[k]) && is_prov))
      {
        non_call_stack_file_present = TRUE;
        break;
      }
  ...
  mmgsdi_client_req_extra_info_type   * extra_param_ptr)
{
  ...
  UIM_MSG_LOW_0("FUNCTION mmgsdi_refresh_process_steering");
  ...
  mmgsdi_status = mmgsdi_util_get_slot_index(slot_id,
                                             &slot_index);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    return mmgsdi_status;
  ...
  mmgsdi_status = mmgsdi_util_get_prov_session_id_for_app(mmgsdi_refresh_info_ptr[slot_index]->request_header.slot_id,
                                                          &mmgsdi_refresh_info_ptr[slot_index]->aid,
                                                          &session_id);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
  ...
  /* Do not send event as no client is registered for steering of roaming refresh */
  extra_param_ptr->refresh_data.send_evt = FALSE;
  ...
  do
  {
    ...
    if(!reg_sim_mmgsdi_steering_of_roaming_refresh_ind(&req_ptr->plmn_list))
    {
      UIM_MSG_ERR_0("Steering of Roaming failed from NAS side");
      mmgsdi_refresh_info_ptr[slot_index]->refresh_status = MMGSDI_CAN_NOT_REFRESH;
      break;
    }
  }while(0);

  if(mmgsdi_refresh_info_ptr[slot_index]->refresh_status == MMGSDI_REFRESH_SUCCESS)
  {
    extra_param_ptr->refresh_data.stage    = MMGSDI_REFRESH_STAGE_END_SUCCESS;
    slot_ptr->refresh_state                = MMGSDI_REFRESH_READY_STATE;
  }

  return mmgsdi_refresh_info_ptr[slot_index]->refresh_status;
...
/* =============================================================================
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_refresh_req_process_reg(
...
/* =============================================================================
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
 mmgsdi_return_enum_type mmgsdi_refresh_req_process_dereg(
  mmgsdi_refresh_req_type           *req_ptr)
{
  mmgsdi_return_enum_type      mmgsdi_status            = MMGSDI_ERROR;
  mmgsdi_session_info_type    *session_ptr              = NULL;
  ...
  mmgsdi_refresh_req_type *req_ptr)
{
  ...
  /* When the MMGSDI state is not initialized and we receive REFRESH event (this
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  */
  if((MMGSDI_SESSION_ACT_OR_SWITCH_PROV_REQ != cnf) ||
     (mmgsdi_refresh_info_ptr[slot_index_cnf]->request_header.client_data ==
...
)
{
  ...
  if(refresh_mode == MMGSDI_REFRESH_RESET)
  {
    ...
    new_table_entry_ptr->card_req.enforcement_policy      = mmgsdi_refresh_info_ptr[slot_index]->enforcement_policy;
...
)
{
  ...
  {
    memset(mmgsdi_refresh_retry_settings_ptr[slot_index], 0, sizeof(mmgsdi_refresh_retry_settings_type));
  }
  ...
  {
    UIM_MSG_ERR_1("Could not read refresh retry settings on Slot index 0x%x", slot_index);
...
/*===========================================================================
  ...
  DESCRIPTION
    This function deletes the OPL-PNN cache and re-cache OPL-PNN file.
    If the flag "delete_old_pnn" is FALSE, it means we are calling this
    function for PNN file as part of REFRESH on OPL file to cache the new
    refrenced records of PNN file. In that case there is no need to delete
    the existing PNN cache.
  ...
  boolean                delete_old_pnn)
{
  ...
  mmgsdi_int_app_info_type           *app_info_ptr          = NULL;
  mmgsdi_file_enum_type               pnn_file_enum         = MMGSDI_USIM_PNN;

  UIM_MSG_HIGH_4("eons refresh file enum: 0x%x client id: 0x%x session id: 0x%x delete old pnn: 0x%x ",
                 file_enum, client_id, session_id, delete_old_pnn);
  ...
  {
    pnn_file_enum = MMGSDI_USIM_PNN;
  }
  else
  {
    pnn_file_enum = MMGSDI_GSM_PNN;
  }

  /* Skip this block if the incoming request is for caching newly referenced
     PNN records as part of OPL refresh (delete_old_pnn == FALSE). In this
     case we don't want to delete PNN cache and there is no need to get file
     attributes for PNN again as refresh happened on OPL file */
  if (delete_old_pnn)
  {
  ...
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
      UIM_MSG_ERR_1("eons get attr error status : 0x%x", mmgsdi_status);
    return mmgsdi_status;
  }
    }
  ...
  if(file_enum == MMGSDI_USIM_OPL)
    ...
    if(mmgsdi_status != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_1("get opl uicc search failed status : 0x%x", mmgsdi_status);
      return mmgsdi_status;
    }
    }
  else if (file_enum == MMGSDI_USIM_PNN || file_enum == MMGSDI_GSM_PNN)
    {
  ...
  else if (file_enum == MMGSDI_GSM_OPL)
    {
    mmgsdi_data_type   data;

    memset(&data, 0x00, sizeof(data));

    /* In case of GSM_OPL, all OPL records needs to be read from card as there
       is no search performed on 2G cards for invalid records. In that case as
       per MMGSDI cache, all OPL records are valid and cached. */
  ...
  /* If all OPL records are invalid, in that case for 2G cards we need to cache
     complete OPL file but for 3G cards next_valid_rec_num and next_valid_rec_len
     will be returned as 0. So, we skip reading the next record and also no need
     to cache the PNN file again as it was already cached and no new record is
     now referenced by OPL file */
  if(mmgsdi_status != MMGSDI_SUCCESS ||
  ...
  /* When refresh FCN is triggered on EF-OPL, we not only need to cache
     EF-OPL but we also need to recache EF-PNN because valid EF-PNN cache
     is derived from the EF-OPL. */
  if(mmgsdi_status == MMGSDI_SUCCESS &&
     (file_enum == MMGSDI_USIM_OPL ||
      file_enum == MMGSDI_GSM_OPL))
  {
    /* We are calling PNN caching as part of REFRESH on OPL, so PNN cache
       should not be deleted. So passing delete_old_pnn flag as FALSE */
    mmgsdi_status = mmgsdi_refresh_cache_opl_pnn(session_id,
                                                 client_id,
                                                 pnn_file_enum,
                                                 FALSE);
  }
