...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_se13.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
...
===========================================================================*/
/* Maximum number of PLMN info set corresponding to 1 name */
#define MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME              10
...
#ifndef FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY
/* ===========================================================================
...
#endif/* FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY */
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  number of entries in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return number of entries.
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  MCC in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return MCC.
...
)
{
  ...
  {
    num_of_entries = mmgsdi_se13_file_info.se13_header.num_entries;
    mcc            = MMGSDI_SE13_MCC_INVALID_VAL;
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  MNC in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return MNC.
...
)
{
  ...
  {
    num_of_entries = mmgsdi_se13_file_info.se13_header.num_entries;
    mnc            = MMGSDI_SE13_MNC_INVALID_VAL;
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  network mask of specified index in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return network mask of specified index.
...
)
{
  ...
  {
    num_of_entries    = mmgsdi_se13_file_info.se13_header.num_entries;
    network_type_mask = MMGSDI_SE13_NET_MASK_INVALID_VAL;
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  GSMA entry of specified index in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return GSMA entry of specified index.
...
DESCRIPTION:
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  PLMN entry of specified index in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return PLMN entry of specified index.
...
)
{
  ...
  if (mmgsdi_se13_check_if_file_present())
  {
    ...
    /* Reset network info data */
    memset(info_ptr, 0x00, sizeof(mmgsdi_eons_network_info_data_type));
    short_name_len = 0;
    full_name_len  = 0;
    short_name_ptr = NULL;
    full_name_ptr  = NULL;
  ...
  /* Now get the name length & addresses
    ...
    UIM_MSG_HIGH_0("PLMN Entry fetched from SE13 present in EFS");
  }
#endif /* FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY */
...
/*===========================================================================
FUNCTION MMGSDI_EONS_RSP_GET_SE13_PLMN_NAMES

DESCRIPTION
  This function handle response for get se13 plmn names.
  ...
  SIDE EFFECTS:
    None
===========================================================================*/
mmgsdi_return_enum_type  mmgsdi_eons_rsp_get_se13_plmn_names (
...
)
{
  ...
  memset(&status_word, 0x00, sizeof(status_word));
  ...
  return mmgsdi_status;
}/* mmgsdi_eons_rsp_get_se13_plmn_names */
...
)
{
  ...
  {
    switch(data_ptr[loop_count])
    {
      case '$':
        data_ptr[loop_count] = 0x02;
        break;
      case '@':
        data_ptr[loop_count] = 0x00;
        break;
      case '_':
        data_ptr[loop_count] = 0x11;
        break;
      case '`':
        data_ptr[loop_count] = 0x27;
        break;
      default :
        break;
    }
...
/*===========================================================================
  ...
  This function is used to get plmn name by directly accessing SE13 table for given PLMN Id
...
)
{
  ...
  uint8                              length        = 0;
  mmgsdi_eons_network_info_data_type network_info;

  memset(&network_info, 0x00, sizeof(mmgsdi_eons_network_info_data_type));
  ...
  /*set lac to 0xFFFF*/
  plmn_name_info_ptr->lac = 0xFFFF;
  
  mmgsdi_memscpy(plmn_name_info_ptr->plmn_id.plmn_id_val,
    sizeof(plmn_name_info_ptr->plmn_id.plmn_id_val),
    plmn_id.plmn_id_val,
    MMGSDI_PLMN_ID_SIZE);
  
  /*copy RAT type in output*/
  plmn_name_info_ptr->plmn_id.rat = plmn_id.rat;

  sys_mode = mmgsdi_eons_map_rat_type_to_sys_mode(plmn_id.rat);

  if ((mmgsdi_eons_get_network_info_from_se13(plmn_id, sys_mode, &network_info))  &&
      ((network_info.full_name_ptr  != NULL) && (network_info.short_name_ptr != NULL)))
  {
    length = strlen(network_info.full_name_ptr);
    ...
    if(length > 0)
    {
      mmgsdi_memscpy(plmn_name_info_ptr->plmn_long_name.plmn_data,
                     sizeof(plmn_name_info_ptr->plmn_long_name.plmn_data),
                     (uint8 *)network_info.full_name_ptr,
                     length);
     
      mmgsdi_eons_convert_ascii_to_7bit_unpacked(
        plmn_name_info_ptr->plmn_long_name.plmn_data,
        plmn_name_info_ptr->plmn_long_name.plmn_data_len);
    }

    length = strlen(network_info.short_name_ptr);

    plmn_name_info_ptr->plmn_short_name.plmn_data_len = length;
    ...
    if(length > 0)
    {
      mmgsdi_memscpy(plmn_name_info_ptr->plmn_short_name.plmn_data,
                     sizeof(plmn_name_info_ptr->plmn_short_name.plmn_data),
                     (uint8 *)network_info.short_name_ptr,
                     length);

      mmgsdi_eons_convert_ascii_to_7bit_unpacked(
        plmn_name_info_ptr->plmn_short_name.plmn_data,
  ...
  MMGSDIUTIL_TMC_MEM_FREE_NULL_OK(network_info.full_name_ptr);
  MMGSDIUTIL_TMC_MEM_FREE_NULL_OK(network_info.short_name_ptr);

  return mmgsdi_status;
}/* mmgsdi_proc_get_se13_plmn_name */
...
)
{
  ...
  memset(se13_plmn_info_ptr,
         0x00,
         sizeof(mmgsdi_se13_plmn_static_info_type) * MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME);

  num_se13_entries = mmgsdi_se13_table_get_num_entries();

  sys_mode = mmgsdi_eons_map_rat_type_to_sys_mode(rat_type);
  ...
  for(index = 0; index < num_se13_entries; index++)
  {
       ...
       (plmn_long_name.data_ptr != NULL))
    {
      ...
      {
        if(memcmp(plmn_long_name.data_ptr,
                  network_info_data.full_name_ptr,
                  plmn_long_name.data_len))
        {
          match_found = FALSE;
       ...
       (plmn_short_name.data_ptr != NULL))
    {
      ...
      {
        if(memcmp(plmn_short_name.data_ptr,
                  network_info_data.short_name_ptr,
                  plmn_short_name.data_len))
        {
          match_found = FALSE;
    ...
    if(match_found)
    {
      if(node_count < MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME)
      {
        se13_plmn_info_ptr[node_count].mcc = network_info_data.mcc;
        se13_plmn_info_ptr[node_count].mnc = network_info_data.mnc;
        se13_plmn_info_ptr[node_count].network_mask = network_info_data.network_type_mask;

        /* Copy long name */
        if(network_info_data.full_name_ptr != NULL)
        {
          se13_plmn_info_ptr[node_count].plmn_long_name_len =
            strlen(network_info_data.full_name_ptr);
          mmgsdi_memscpy(se13_plmn_info_ptr[node_count].plmn_long_name,
            sizeof(se13_plmn_info_ptr[node_count].plmn_long_name),
            network_info_data.full_name_ptr,
            strlen(network_info_data.full_name_ptr));
        }
        ...
        if(network_info_data.short_name_ptr != NULL)
        {
          se13_plmn_info_ptr[node_count].plmn_short_name_len =
            strlen(network_info_data.short_name_ptr);
          mmgsdi_memscpy(se13_plmn_info_ptr[node_count].plmn_short_name,
                         sizeof(se13_plmn_info_ptr[node_count].plmn_short_name),
                         network_info_data.short_name_ptr,
                         strlen(network_info_data.short_name_ptr));
        }
        node_count++;
  ...
  return node_count;
}/* mmgsdi_eons_get_se13_plmn_info_by_name_sync */
...
DESCRIPTION
  ...
  If feature FEATURE_MMGSDI_SE13_FROM_HARDCODED_ONLY is enabled then retrieve
  SE13 table release data and source in static SE13 table.
  If feature FEATURE_MMGSDI_SE13_FROM_EFS_ONLY is enabled then retrieve
  SE13 from EFS and return SE13 table release data and source.
...
)
{
  ...
  if (mmgsdi_se13_check_if_file_present())
  {
    memset(se13_table_info_ptr ,0x00, sizeof(mmgsdi_se13_table_details_info));
