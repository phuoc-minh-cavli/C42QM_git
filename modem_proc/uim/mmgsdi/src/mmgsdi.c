...
Copyright (c) 2001 - 2019, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi.c#3 $$ $DateTime: 2023/06/21 11:32:16 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/23/23   tq	   Modem base IMSI switch
10/30/20   tq      Remove compilation error when FEATURE_LTE undefined
08/22/19   sch     Send SELECT ISDR to determine if card is eUICC or not
08/21/19   sch     No need to send PDOWN if EF-DIR fails for eUICC card
...
02/06/19   kb      Clear ATR related globals at card error/link est
12/11/18   sg      Reduce F3 logging
...
08/20/18   bcho    Migration to new CM interface
03/23/18   me      Check LTE feature before accessing LTE version NV
03/12/18   me      Set Byte11 - Bit7 in EF TST based on the lte_3gpp_release_ver NV
11/17/17   me      Support for FEATURE_UIM_JO_3_0_3_CAPABILITIES 
10/13/17   rps     Remove old  perso support
06/06/17   me      Added support for CIOT LTE EFS with values CAT1 and NB1
03/29/17   nr      Add support to cache PIN1 of the SIM card when PSM mode enabled
03/27/17   nr      Send context save response in case of NO SIM
...
12/01/16   me      Added support for default values for enabling PSM and EDRX 
10/10/16   shr     Use UIM common utility to determine LTE support
10/06/16   ar      Remove unused variables
09/13/16   me      Added support for Byte 10,11,12 mmgsdi_set_ens_support_flag
09/13/16   sn      Support for CDMALess Device
09/08/16   vdc     Extend full and expired service policies to any slot
07/04/16   ar      Don't RESET card if EF-DIR req is not processed by UIMDRV
...
05/31/16   bcho    EONS F3 reduction
...
05/13/16   vdc     Remove excessive logging
...
05/04/16   vdc     Removed logging for refresh registration
...
04/25/16   dd      Proper task name is saved as client name
04/18/16   vdc     Send act resp and session changed evt at same time
04/14/16   na      Remove cmd_rsp_cb_ptr from mmgsdi_client_id_reg_table_type
04/14/16   vdc     Added external personlization files for IOE flavor
04/11/16   vdc     Feature guard 1X refresh files with FEATURE_MMGSDI_3GPP2
...
01/12/16   bcho    Don't delete attr cache if not re-cached during NAA Refresh
...
mmgsdi_is_test_mode_logging_enabled_type mmgsdi_is_test_mode_logging_enabled;
...
/* CM SS event srv timer for IMSI switch logic */
rex_timer_type                        mmgsdi_imsi_switch_timer;
  ...
  mmgsdi_task_cmd_type           * task_cmd_ptr)
{
  ...
  switch (task_cmd_ptr->cmd.cmd_enum)
  {
    ...
    case MMGSDI_PSM_CONTEXT_SAVE_REQ:
...
)
{
  ...
  /*---------------------------------------------------------------------------
    ...
    4. slot_id           - Slot that the request is to be performed on
  ...
  mmgsdi_return_enum_type                mmgsdi_error_status)
{
  ...
  index to free
  -------------------------------------------------------------------------*/
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(task_cmd_data_ptr->cmd_enum,
                                                           (void*)&task_cmd_data_ptr->cmd,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
...
)
{
  ...
  switch(task_cmd_ptr->cmd.cmd_enum)
  {
    ...
    case MMGSDI_ACTIVATE_ONCHIP_SIM_REQ:
      mmgsdi_status = mmgsdi_onchip_activate(&task_cmd_ptr->cmd.cmd.act_onchip_sim_req,
    ...
    case MMGSDI_PIN_OPERATION_REQ:
      mmgsdi_status = mmgsdi_common_pin_operation(&task_cmd_ptr->cmd.cmd.pin_operation_req, protocol);
    ...
    case MMGSDI_SRV_AVAILABLE_REQ:
      ...
      {
        mmgsdi_status = mmgsdi_common_is_service_available(&task_cmd_ptr->cmd.cmd.srv_available_req);
      } /* protocol is MMGSDI_ICC */
    ...
    case MMGSDI_SEARCH_REQ:
      ...
      mmgsdi_status = mmgsdi_uicc_search(&task_cmd_ptr->cmd.cmd.search_req);
      break;
    ...
    case MMGSDI_SESSION_ENABLE_SERVICE_REQ:
      ...
      {
        mmgsdi_status = mmgsdi_uicc_process_enable_or_disable_srv((void*)&task_cmd_ptr->cmd.cmd.session_enable_service_req,
    ...
    case MMGSDI_SESSION_DISABLE_SERVICE_REQ:
      ...
      {
        mmgsdi_status = mmgsdi_uicc_process_enable_or_disable_srv((void*)&task_cmd_ptr->cmd.cmd.session_disable_service_req,
    ...
    case MMGSDI_SESSION_SUB_OK_REQ:
      mmgsdi_status = mmgsdi_external_simlock_process_sub_ok_req(
  ...
  case MMGSDI_NOTIFY_SIM_BUSY_REQ:
    ...
    case MMGSDI_SESSION_UPDATE_ACSGL_RECORD_REQ:
      mmgsdi_status = mmgsdi_csg_process_update_acsgl_record_request(&task_cmd_ptr->cmd.cmd.update_acsgl_record_req);
    ...
    case MMGSDI_IMSI_SWITCH_REQ:
      mmgsdi_process_imsi_switch(&task_cmd_ptr->cmd.cmd.switch_imsi_profile_req);
      break;
    ...
    case MMGSDI_GET_M2M_EID_REQ:
      mmgsdi_status = mmgsdi_process_get_m2m_eid(task_cmd_ptr);

      /* It is possible that the command is re-queued, if a previous
         command of the same type is already being executed */
...
-----------------------------------------------------------------------------*/
  ...
  switch(cmd_req_enum)
  {
    ...
    case MMGSDI_PIN_OPERATION_REQ:
      mmgsdi_status = mmgsdi_common_rsp_pin_op(&task_cmd_ptr->cmd.cmd.uim_report_rsp, protocol, NULL, FALSE);
      break; /* end case - MMGSDI_PIN_OPERATION_REQ */
    ...
    case MMGSDI_GET_SE13_PLMN_NAMES_REQ:
      mmgsdi_status = mmgsdi_eons_rsp_get_se13_plmn_names(
                        &task_cmd_ptr->cmd.cmd.uim_report_rsp);
      break; /* end case - MMGSDI_GET_SE13_PLMN_NAMES_REQ */
    ...
    case MMGSDI_SEARCH_REQ:
      ...
      {
        mmgsdi_status = mmgsdi_uicc_rsp_search(
                          &task_cmd_ptr->cmd.cmd.uim_report_rsp,
                            NULL, FALSE);
      }
      else
    ...
    case MMGSDI_SESSION_SUB_OK_REQ:
      mmgsdi_status = mmgsdi_external_simlock_process_sub_ok_rsp(
                        &task_cmd_ptr->cmd.cmd.uim_report_rsp);
      break;
...
/*===========================================================================
FUNCTION MMGSDI_HANDLE_QMI_UIM_QCSI_SIG

DESCRIPTION
  MMGSDI main task calls this function when QCSI framework sets QCSI sig for QMI UIM
  service. It will result in the calling of one of the QMI UIM handlers.

DEPENDENCIES
  MMGSDI main task should be properly initialized prior to receiving this signal
...
SIDE EFFECTS
  NONE
===========================================================================*/
static void mmgsdi_handle_qmi_uim_qcsi_sig (
  void
)
{
  /* Clear signal */
  (void) rex_clr_sigs(UIM_GSDI_TCB,
                      (rex_sigs_type)MMGSDI_QMI_UIM_QCSI_SIG);

  UIM_MSG_HIGH_0("Got MMGSDI_HANDLE_QMI_UIM_QCSI_SIG");
  
  mmgsdi_process_qmi_uim_sig = TRUE;

  qmi_uim_handle_qcsi_evt(UIM_GSDI_TCB, MMGSDI_QMI_UIM_QCSI_SIG);

  mmgsdi_process_qmi_uim_sig = FALSE;
} /* mmgsdi_handle_qmi_uim_qcsi_sig */
...
void mmgsdi_cm_init (void)
{
  ...
  else
  {
    ...
    bit_mask_256_clear_all(&bit_mask);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_ORIG);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_ANSWER);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_END_REQ);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_END);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_SUPS);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_INCOM);
    (void)bit_mask_256_set_bit(&bit_mask, CM_CALL_EVENT_CONNECT);
    ...
    bit_mask_256_clear_all(&bit_mask);
    (void)bit_mask_256_set_bit(&bit_mask, CM_SS_EVENT_SRV_CHANGED);

    (void) cm_mm_client_ss_reg(mmgsdi_generic_data_ptr->mmgsdi_cm_id,
    ...
    bit_mask_256_clear_all(&bit_mask);
    (void)bit_mask_256_set_bit(&bit_mask, CM_PH_EVENT_OPRT_MODE);
    (void)bit_mask_256_set_bit(&bit_mask, CM_PH_EVENT_TEST_CONTROL_TYPE);
    (void)bit_mask_256_set_bit(&bit_mask, CM_PH_EVENT_SYS_SEL_PREF);
...
)
{
  ...
  if(!(mmgsdi_generic_data_ptr->ready_tasks & (0x00000001 << (uint8) MMGSDI_CM_TYPE)))
  {
    ...
    /* Register with CM for PSM voting */
...
)
{
  ...
  if(!(mmgsdi_generic_data_ptr->ready_tasks & (0x00000001 << (uint8) MMGSDI_QMI_MODEM_TYPE)))
  {
    ...
    qmi_uim_init_post_startup(UIM_GSDI_TCB,
                              MMGSDI_QMI_UIM_QCSI_SIG);
...
)
{
  ...
  /* Register for CM/QMIMODEM ready signals and publish task ready evt for
    ...
    /* In case NVRUIM task is not present,
       initialized NVRUIM Stub by calling its init function directly */
    nvruim_mmgsdi_init();
  }
  ...
  ** ----------------------------------------------------------------------*/
  for (;;)
  {
    ...
    /*-----------------------------------------------------------------------------
            Process Signal - MMGSDI_QMI_UIM_QCSI_SIG
    -----------------------------------------------------------------------------*/
    else if((sigs_rcvd & sigs & MMGSDI_QMI_UIM_QCSI_SIG) &&
            (running_task == MMGSDI_TASK_MAIN))
    {  
      mmgsdi_handle_qmi_uim_qcsi_sig();
    }
...
void mmgsdi_evt_cb (const mmgsdi_event_data_type *event_ptr)
{
  ...
  switch(event_ptr->evt)
  {
    case MMGSDI_SESSION_CHANGED_EVT:
      ...
      /* Reinitialize sub based test_mode_logging globals when session change
         The values will be populated after read IMSI */
      if (FALSE == mmgsdi_is_test_mode_logging_enabled.always_enable &&
          (event_ptr->data.session_changed.app_info.app_data.app_type == MMGSDI_APP_USIM ||
           event_ptr->data.session_changed.app_info.app_data.app_type == MMGSDI_APP_SIM)) 
      {
        switch(session_type)
        {
          case MMGSDI_GW_PROV_PRI_SESSION:
            mmgsdi_is_test_mode_logging_enabled.subs[0] = FALSE;
            break;
          case MMGSDI_GW_PROV_SEC_SESSION:
            mmgsdi_is_test_mode_logging_enabled.subs[1] = FALSE;
            break;
          case MMGSDI_GW_PROV_TER_SESSION:
            mmgsdi_is_test_mode_logging_enabled.subs[2] = FALSE;
            break;
          default:
            break;
        }
    ...
    case MMGSDI_PIN1_EVT:
      ...
      /* Notify PSM module about PIN event */
      mmgsdi_psm_pin_evt(&event_ptr->data.pin);
      break;
    ...
    case MMGSDI_CARD_INSERTED_EVT:
      ...
      if(gstk_slot != GSTK_SLOT_ID_MAX && !mmgsdi_nv_is_ens_enabled(slot))
      {
        gstk_gsdi_set_fetch_status(gstk_slot, TRUE);
      }
      ...
      if(mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        /* Reinitialize test mode logging for this slot */
        mmgsdi_is_test_mode_logging_enabled.slots[slot_index] = FALSE;
      }
    ...
    case MMGSDI_CARD_ERROR_EVT:
      UIM_MSG_HIGH_2("MMGSDI_CARD_ERROR, condition: 0x%x, slot: 0x%x",
...
#if defined(FEATURE_MMGSDI_3GPP2) && !defined(FEATURE_UIM_TEST_FRAMEWORK)
/*===========================================================================
...
#endif /* FEATURE_MMGSDI_3GPP2 && !FEATURE_UIM_TEST_FRAMEWORK*/
...
void mmgsdi_init_rtre_config(void)
{
  ...
  if((mmgsdi_generic_data_ptr->ready_tasks & (00000001 << (uint8) MMGSDI_CM_TYPE)))
  {
    ...
    if ( FALSE == cm_ph_cmd_change_rtre_config(mmgsdi_cm_rtre_cb,
                                               NULL,
                                               mmgsdi_generic_data_ptr->mmgsdi_cm_id,
                                               mmgsdi_util_convert_nv_to_cm_rtre(nv_rtre_config->rtre_config)))
    {
  ...
  else
  {
    ...
    mmgsdi_util_send_rtre_command_to_nv(nv_rtre_config);
  }
...
void mmgsdi_open_base_sessions(void)
{
  ...
  if(mmgsdi_generic_data_ptr->pri_gw_session_id == 0)
  {
    ...
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->pri_gw_session_id);
  ...
  if(mmgsdi_generic_data_ptr->sec_gw_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->sec_gw_session_id);
  ...
  if(mmgsdi_generic_data_ptr->ter_gw_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->ter_gw_session_id);
  ...
  if(mmgsdi_generic_data_ptr->pri_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->pri_1x_session_id);
  ...
  if(mmgsdi_generic_data_ptr->sec_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->sec_1x_session_id);
  ...
  if(mmgsdi_generic_data_ptr->ter_1x_session_id == 0)
  {
    ...
    /*Register for Refresh*/
    (void)mmgsdi_refresh_register(mmgsdi_generic_data_ptr->ter_1x_session_id);
...
)
{
  ...
  build_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_CLIENT_ID_AND_EVT_REG_REQ,
                                                          (void*)req_ptr,
                                                          NULL,
                                                          &index);
  ...
  build_status = mmgsdi_build_and_queue_client_reg_rsp(index,
                                                       mmgsdi_status,
...
)
{
  ...
  build_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_CLIENT_ID_AND_EVT_DEREG_REQ,
                                                          (void*)req_ptr,
                                                          NULL,
                                                          &client_req_index);
  if(build_status != MMGSDI_SUCCESS)
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_TP_DOWNLOAD_COMPLETE_REQ,
                                                           (void*)req_ptr,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_RECOVERY_IND_REQ,
                                                           (void*)req_ptr,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
...
)
{
  ...
  (void)uim_set_proactive_uim_slot(TRUE, uim_slot);
...
)
{
       ...
       req_ptr->refresh_req_type == MMGSDI_REFRESH_ORIG_PUP_REQ)
  {
    ...
    msg_ptr->refresh.card_req.enforcement_policy =
      req_ptr->refresh.card_req.enforcement_policy;
...
/*===========================================================================
   ...
   PARAMETERS:
     ...
     refresh_enforcement_policy    Enforcement policy for refresh from card
...
)
{
  ...
  req.refresh.card_req.enforcement_policy  = refresh_enforcement_policy;
...
mmgsdi_return_enum_type mmgsdi_get_me_config_params_from_nv(void)
{
  ...
  /* We support FDN, ACL and BDN on all devices so removing NV_MMGSDI_ME_CONFIG_PARAM_I
     (NV 4395) read/write operation. Use default ME config value instead. */
  config_value = MMGSDI_DEFAULT_ME_CONFIG_VALUE;
...
)
{
...
#ifdef FEATURE_LTE
  boolean                             lte_available               = FALSE;
  mmgsdi_session_type_enum_type       session_type                = MMGSDI_MAX_SESSION_TYPE_ENUM;
  lte_3gpp_release_ver_e              mmgsdi_3gpp_rel_ver_value   = LTE_3GPP_REL11;
#endif /*FEATURE_LTE*/
 ...
 /* Count how many RATs are enabled in the modem_cap mask */
  temp_modem_cap = modem_cap;
  while(temp_modem_cap)
  {
    num_of_rats += temp_modem_cap & 1;
    temp_modem_cap >>= 1;
  }

  /* Devices that support only one Access Tech or support CAT M1/NB1 
     shall not support RAT balancing and for these cases RAT balancing is set to 0 */
 if (num_of_rats > 1 && 
     (!((modem_cap & UIM_MODEM_CAP_FEATURE_LTE_M1_MASK) || (modem_cap & UIM_MODEM_CAP_FEATURE_LTE_NB1_MASK)))) 
  {
    nv_item.nv_item_enum = MMGSDI_NV_RAT_BALANCING_SUPPORT_I;
    mmgsdi_status = mmgsdi_get_cached_nv_item(&nv_item);
    if ((mmgsdi_status == MMGSDI_SUCCESS) &&
  ...
  #ifdef FEATURE_LTE
      if(lte_available && MMGSDI_APP_SIM == app_type)
      {
        req_ptr->data.data_ptr[4] = 0x01;
      }
  #endif /*FEATURE_LTE*/
...
#else
  /* Set CSG byte to 0x01 if WCDMA RAT is available.
     Default value is CSG support for W is enabled, for LTE is disabled */
  if (modem_cap & UIM_MODEM_CAP_FEATURE_WCDMA_MASK)
  {
  ...
  if ((req_ptr->data.data_len >= 12) && mmgsdi_is_iot_device())
  {
    mmgsdi_iot_category_enum            mmgsdi_iot_category         = MMGSDI_IOT_UNKNOWN;

    /* MTC support category, byte 10 in EF TST */
    mmgsdi_iot_category = mmgsdi_get_iot_category();
    if (mmgsdi_iot_category != MMGSDI_IOT_UNKNOWN)
    {
      req_ptr->data.data_ptr[9] = (uint8)mmgsdi_iot_category;
    }
  }
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(
