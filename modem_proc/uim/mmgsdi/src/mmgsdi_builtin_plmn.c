...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_builtin_plmn.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
  ...
  uint8 ** file_buffer_pptr)
{
  ...
  *file_buffer_pptr = NULL;
  ...
  mmgsdi_status = gsdi_fs_get_file_size(file_handle_ptr, file_size_ptr,
                                        UIM_COMMON_EFS_DEVICE);
...
mmgsdi_return_enum_type mmgsdi_store_builtin_plmn_list(
  const mmgsdi_builtin_plmn_list_type      *set_builtin_plmn_list_ptr
)
{
  mmgsdi_return_enum_type mmgsdi_status = MMGSDI_ERROR;
  char  * file_handle_ptr               = MMGSDI_BUILTIN_PLMN_LIST_FILE;
  ...
  MMGSDIUTIL_RETURN_IF_NULL(set_builtin_plmn_list_ptr);
       ...
       MMGSDI_FEATURE_ENABLED)
  {
    ...
    num_of_plmn_ids = set_builtin_plmn_list_ptr->num_of_plmn_ids;
    if(num_of_plmn_ids > 0)
    {
      ...
      for(loop_cnt = 0; loop_cnt < num_of_plmn_ids; loop_cnt++)
      {
        mmgsdi_memscpy(
          ...
          set_builtin_plmn_list_ptr->plmn_list_ptr[loop_cnt].plmn_id_val,
          MMGSDI_PLMN_ID_SIZE);
        ...
        mmgsdi_memscpy(&builtin_plmn_data_ptr[(loop_cnt * MMGSDI_OPLMNWACT_RECORD_SIZE) + MMGSDI_PLMN_ID_SIZE],
          ...
          &set_builtin_plmn_list_ptr->plmn_list_ptr[loop_cnt].access_tech,
          sizeof(uint16));
      }
...
{
  uint32 dummy_file_size = 0;
  
  if(uim_common_efs_get_file_size(UIM_COMMON_EFS_MMGSDI_BUILT_IN_PLMN_LIST,
                                   UIM_COMMON_EFS_DEVICE,
                                   &dummy_file_size) != UIM_COMMON_EFS_SUCCESS)
  {
    return MMGSDI_ERROR
  }
