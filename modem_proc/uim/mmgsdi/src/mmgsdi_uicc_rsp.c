...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_uicc_rsp.c#1 $$ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/26/19   sch     Don't do protocol switch if card returns 69 99 for ISDR select
12/11/18   sg      Remove read ARR print while handling response
05/02/18   srm     Send correct status words for VERIFY PIN command.
05/26/17   me      Updated the final return status for mmgsdi_uicc_rsp_sim_profile
...
05/30/16   lm      Do not switch protocol when multi profile AID is not present
05/16/16   vdc     Fill enum data when error resp is timeout for get file attr
...
05/19/16   ar      Fix read/write of cache files when cache is not_init
...
)
{
  ...
  /* If cmd_transacted is TRUE and status is WRONG_CLASS, then it means card
     ...
     to card.
     In multi profile AID case, do not switch the protocol since the
     purpose of the request is to determine whether the card is single or multi
     profile card.
     In case the AID is ISDR AID, do not switch the protocol since the
     decision of switching the protocol is taken in the card_init function */
    if(report_status == MMGSDI_WRONG_CLASS &&
       uim_rsp_ptr->cmd_transacted &&
     is_multi_profile_aid == FALSE &&
     FALSE == mmgsdi_euicc_is_isdr_aid(
                &(extra_param_ptr->get_file_attr_data.access.file.app_id)))
    {
...
)
{
  ...
  if(sync_process)
  {
    MMGSDIUTIL_RETURN_IF_NULL(pin_operation_cnf_pptr);
  }
  ...
  if (report_status != MMGSDI_SUCCESS)
  {
   ...
   MMGSDIUTIL_DEBUG_MSG_ERROR_3("UIM_FAIL sw1:%x sw2:%x status:0x%x",
    ...
    if (extra_param_ptr->pin_data.orig_pin_op != MMGSDI_PIN_OP_NONE)
    {
      ...
      /* Some buggy cards require PIN1 to be verified after PIN1 value is enabled in order to work correctly.
      ...
      /* Final status words should be sent only after getting verify and unblock retries from the card. 
         In this case, the retry type would be set to MMGSDI_INTERNAL_VERIFY_AFTER_PIN_CHANGE or                                MMGSDI_GET_NUM_RETRIES */
      if(extra_param_ptr->pin_data.retry_type == MMGSDI_INTERNAL_VERIFY_AFTER_PIN_CHANGE ||
         extra_param_ptr->pin_data.retry_type == MMGSDI_GET_NUM_RETRIES)
      {
  ...
  else
  {
    /* Obtain memory for confirmation data */
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(
      (*pin_operation_cnf_pptr),
      sizeof(mmgsdi_pin_operation_cnf_type));
    if (*pin_operation_cnf_pptr == NULL)
    {
      return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
    }

    cnf_ptr = *pin_operation_cnf_pptr;

    mmgsdi_status = mmgsdi_cnf_build_pin_operation_data(
                                      &req_header,
                                      cnf_ptr,
                                      report_status,
                                      extra_param_ptr);
...
)
{
  ...
  {
    UIM_MSG_HIGH_1("Auth response first byte: 0x%x", uim_rsp_ptr->rsp_data.data_ptr[0]);
  }
...
)
{
  ...
  if (report_status != MMGSDI_SUCCESS)
  {
    ...
    Log Error Message
    ----------------------------------------------------------------------*/
    UIM_MSG_ERR_3("UIM Response parsing failed for Search, sw1:0x%x sw2:0x%x status:0x%x",
                  uim_rsp_ptr->status_word.sw1,
                  uim_rsp_ptr->status_word.sw2,
                  report_status);
