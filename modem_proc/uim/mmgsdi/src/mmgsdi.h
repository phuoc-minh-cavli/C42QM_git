...
Copyright (c) 2004 - 2019, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi.h#2 $$ $DateTime: 2023/06/21 11:32:16 $
...
09/22/16   me      Increased number of files in common cache   
...
04/14/16   na      Compilation eror removed
...
#define MMGSDI_UNUSED_SIG                     0x00000100 /* UNUSED SIG */
...
#define MMGSDI_QMI_UIM_QCSI_SIG               0x00004000 /* QCSI SIGNAL */
...
#define MMGSDI_NUM_FILES_IN_USIM_CACHE       43

#ifdef FEATURE_MMGSDI_3GPP2
#define MMGSDI_NUM_FILES_IN_RUIM_CACHE       26
#define MMGSDI_NUM_FILES_IN_CSIM_CACHE       30
#endif /* FEATURE_MMGSDI_3GPP2 */

#define MMGSDI_NUM_FILES_IN_COMMON_CACHE     6
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     aram_aid_select_failed:
                          This flag indicate Select ARA-M AID attempt is already failed. This
                          is then used to ensure app reselection is not attempted.
     is_euicc:            Whether the card is eUICC or not
     is_eid_in_process:   Boolean to check whether the current eid request is in progress or not
-------------------------------------------------------------------------------*/
typedef struct {
  ...
  boolean                               aram_aid_select_failed;
  boolean                               is_euicc;
  boolean                               is_eid_in_process;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     request_type:   Request Command Type
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_GET_M2M_EID_REQ_TYPE

   DESCRIPTION:
     This structure is used when handling GET_EID request
     for M2M devices in MMGSDI task
------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type         request_header;
} mmgsdi_get_m2m_eid_req_type;
...
/* --------------------------------------------------------------------------
   ...
   MEMBERS:
     query_type         : contains query type
     query_session_id   : contains session id
     query_session_type : contains session type
 ---------------------------------------------------------------------------*/
typedef struct
{
  mmgsdi_session_info_enum_type  query_type;
  mmgsdi_session_id_type         query_session_id;
  mmgsdi_session_type_enum_type  query_session_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_GENERATE_KEY_EXTRA_INFO_TYPE

   DESCRIPTION:
     The structure contains vpm first octet and vpm last octet
------------------------------------------------------------------------------*/
typedef struct
{
  mmgsdi_single_vpm_octet_type    vpm_first_octet;
  mmgsdi_single_vpm_octet_type    vpm_last_octet;
}mmgsdi_generate_key_extra_info_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SESSION_ISIM_AUTH_EXTRA_INFO_TYPE

   DESCRIPTION:
     The structure contains reference data number for ISIM authentication. 
------------------------------------------------------------------------------*/
typedef struct
{
  mmgsdi_autn_context_enum_type    auth_context;
}mmgsdi_session_isim_auth_extra_info_type;
...
typedef union
{
  ...
  mmgsdi_generate_key_extra_info_type             generate_key;
  mmgsdi_session_isim_auth_extra_info_type        isim_auth;
...
/* ----------------------------------------------------------------------------
   ...
   DESCRIPTION:
     ...
     enforcement_policy   : Enforcement policy from card
-------------------------------------------------------------------------------*/
typedef struct{
  mmgsdi_request_header_type              request_header;
  mmgsdi_refresh_mode_enum_type           mode;
...
/* --------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SWITCH_IMSI_PROFILE_REQ_TYPE

   DESCRIPTION:
     This structure is used use to prepare request to switch imsi profile

     request_header:                      Switch IMSI profile request Header information
     sys_srv_status_e_type                camped_srv
     mmgsdi_imsi_switch_nw_enum_type      network_type
...
typedef struct {
  ...
  mmgsdi_slot_id_enum_type          slot;
  mmgsdi_imsi_switch_src_enum_type  src;
}mmgsdi_switch_imsi_profile_req_type;
...
SIDE EFFECTS
  NONE
===========================================================================*/
mmgsdi_return_enum_type mmgsdi_process_imsi_switch(   mmgsdi_switch_imsi_profile_req_type *switch_imsi_profile_req_ptr);
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_PSM_CONTEXT_SAVE_REQ_TYPE

   DESCRIPTION:
     This structure is used when CM notifies MMGSDI about MMGSDI_PSM_CONTEXT_SAVE_REQ
     to save the context into SFS.
     request_header :  Request Header information
-------------------------------------------------------------------------------*/
typedef struct {
  mmgsdi_request_header_type     request_header;
} mmgsdi_psm_context_save_req_type;
...
typedef struct {
  ...
  union {
    ...
    mmgsdi_get_m2m_eid_req_type                     get_m2m_eid_req;
    /* Response */
    mmgsdi_uim_report_rsp_type                      uim_report_rsp;
    mmgsdi_client_id_reg_rsp_type                   client_id_reg_rsp;
    ...
    mmgsdi_psm_context_save_req_type                psm_context_save_req;
    mmgsdi_switch_imsi_profile_req_type             switch_imsi_profile_req;
  } cmd;
...
/* ----------------------------------------------------------------------------
   STRUCTURE: MMGSDI_IS_TEST_MODE_LOGGING_ENABLED_TYPE

   DESCRIPTION:
      Used for storing information related to test mode logging
-------------------------------------------------------------------------------*/
typedef struct {
  boolean                    always_enable;
  boolean                    slots[MMGSDI_MAX_NUM_SLOTS];
  boolean                    subs[MMGSDI_NUM_ATTRIBUTES];
} mmgsdi_is_test_mode_logging_enabled_type;
...
extern mmgsdi_is_test_mode_logging_enabled_type mmgsdi_is_test_mode_logging_enabled;
...
extern rex_timer_type                        mmgsdi_imsi_switch_timer;
