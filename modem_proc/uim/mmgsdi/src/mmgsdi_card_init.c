...
Copyright (c) 2011 - 2019, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_card_init.c#4 $$ $DateTime: 2023/07/24 02:41:58 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/24/23   tq      Making EF_PSLOCI and EF_FPLMN as optional file for buggy card
07/06/23   tq	   halt initialization if get profile failied during init
...
01/08/17   ra      Optimize write activity during PSM context save request processing
...
05/12/16   vdc     Added stub file for hidden key funcationality
...
01/12/16   bcho    parameter type changed for function mmgsdi_cache_delete_item
...
)
{
  ...
  if(MMGSDI_SUCCESS == mmgsdi_util_convert_uim_slot(slot_id, &uim_slot))
  {
    (void)uim_set_proactive_uim_slot(FALSE, uim_slot);
  }
...
/*=============================================================================
FUNCTION: MMGSDI_CARD_INIT_TEST_MODE_LOGGING_ENABLED

DESCRIPTION
  This function to populate the mmgsdi_is_test_mode_logging_enabled global
...
SIDE EFFECTS
  NONE
=============================================================================*/
static void mmgsdi_card_init_test_mode_logging_enabled(
  mmgsdi_slot_id_enum_type           slot_id,
  ...
  mmgsdi_imsi_data_type              imsi)
{
  ...
  /* Check if this is a commercial card by IMSI value */
  ...
  {
    mmgsdi_is_test_mode_logging_enabled.slots[slot_index] = is_enabled;
  }

  switch(session_type)
  {
    case MMGSDI_GW_PROV_PRI_SESSION:
      mmgsdi_is_test_mode_logging_enabled.subs[0] = is_enabled;
      break;
    case MMGSDI_GW_PROV_SEC_SESSION:
      mmgsdi_is_test_mode_logging_enabled.subs[1] = is_enabled;
      break;
    case MMGSDI_GW_PROV_TER_SESSION:
      mmgsdi_is_test_mode_logging_enabled.subs[2] = is_enabled;
      break;
    default:
      break;
  }
} /* mmgsdi_card_init_is_test_mode_logging_enabled */
...
)
{
  ...
  /* Updating is_test_mode_logging_enabled flag based on the EFS item settings and
     resevered set of MCC table */
  if (FALSE == mmgsdi_is_test_mode_logging_enabled.always_enable)
  {
    ...
    {
      mmgsdi_card_init_test_mode_logging_enabled(slot_id,
                                                 session_type,
                                                 imsi);
    }
  }
...
)
{
  ...
  service = MMGSDI_USIM_SRV_EPS_MMI - MMGSDI_USIM_SRV;
  offset = service/8;
  ...
  if(offset < int32touint32(data.data_len))
  {
    ...
    /* Set EPS MMI service status bit to 0 */
...
)
{
  ...
  {
    /* Disable EPS MMI service 85 when EF-EPSNSC length is less than 54 */
    if(data_ptr->data_len < 54)
    {
...
)
{
  ...
  if (mmgsdi_status == MMGSDI_SUCCESS &&
      ...
      data_ptr->data_ptr != NULL)
  {
    /* Disable EPS MMI service 85 when EF-EPSLOCI length is not 18 */
    if(data_ptr->data_len != 18)
    {
      return mmgsdi_card_init_disable_eps_mmi_srv(session_id);
    }
  }
    ...
    else
    {
      if ( mmgsdi_status == MMGSDI_NOT_SUPPORTED )
      {
        /* do not reset status here because validation function may require the
           info */
        UIM_MSG_HIGH_1("Init Optional file 0x%x not supported, reset mmgsdi status",
                       app_init_table_ptr[i].mmgsdi_file);
      }
      else if ( mmgsdi_status == MMGSDI_NOT_FOUND )
      {
        /* do not reset status here because validation function may require the
           info */
        UIM_MSG_HIGH_1("Init Optional file 0x%x not found, reset mmgsdi status",
                       app_init_table_ptr[i].mmgsdi_file);
      }
      else if ( mmgsdi_status == MMGSDI_EF_INCONSISTENT )
      {
        /* do not reset status here because validation function may require the
           info */
        UIM_MSG_HIGH_1("Init Optional file 0x%x has zero size, reset mmgsdi status",
                       app_init_table_ptr[i].mmgsdi_file);
      }
...
)
{
  ...
  {
    UIM_MSG_ERR_0("NULL Input Paramter usage_type_ptr");
    return;
  }
  ...
  if (!nv_meid_avail)
  {
    usage_ind = (data & GSDI_RUIM_ESN_USAGE_MASK);
...
)
{
  ...
  nv_meid_avail = meid_nv_data.meid_available;
  ...
  switch(app_type)
  {
    ...
    case MMGSDI_APP_CSIM:
      ...
      break;
    default:
      UIM_MSG_ERR_1("Invalid app_type 0x%x for store esn procedure", app_type);
      return MMGSDI_ERROR;
  }
  ...
  store_esn_req_ptr->request_header.client_id         = mmgsdi_generic_data_ptr->client_id;
  ...
  store_esn_req_ptr->request_header.slot_id           = slot;
  store_esn_req_ptr->request_header.client_data       = 0;
  store_esn_req_ptr->request_header.response_cb       = NULL;

  if(card_meid_avail && nv_meid_avail)
  {
    store_esn_req_ptr->esn_usage  = GSDI_RUIM_STORE_MEID_USE_EUIM_ID;
    store_esn_req_ptr->esn_length = GSDI_RUIM_STORE_MEID_LENGTH;
    store_esn_req_ptr->meid_available = TRUE;
    qw_equ(store_esn_req_ptr->meid, meid_nv_data.meid);
  }
  else
  {
    store_esn_req_ptr->esn_usage  = GSDI_RUIM_STORE_ESN_USE_UIM_ID;
    store_esn_req_ptr->esn_length = GSDI_RUIM_STORE_ESN_LENGTH;
    store_esn_req_ptr->meid_available = FALSE;
    /* Get the HW ESN from NV */
    nv_read_status = mmgsdi_nv_get_item(NV_ESN_ME_I, &mmgsdi_nv_data,
                                        MMGSDI_NV_CONTEXT_DEVICE_SPECIFIC);
    ...
    mmgsdi_memscpy(&store_esn_req_ptr->esn,
                   sizeof(store_esn_req_ptr->esn),
                   &mmgsdi_nv_data.item_value.esn,
                   sizeof(store_esn_req_ptr->esn));
  }

  /* Assign this in case the featurization for CDMA and CSIM for the persective
     app_type is off and we need to build an error response */
  ...
  if ((mmgsdi_status != MMGSDI_SUCCESS) || (cnf_ptr == NULL) ||
  ...
  MMGSDIUTIL_TMC_MEM_FREE_NULL_OK(data_buf.data_ptr);
  return mmgsdi_status;
...
)
{
  ...
  mmgsdi_cache_init_order_type usim_pup_init_table[] =
    ...
    {{MMGSDI_USIM_NETPAR,    FALSE,        mmgsdi_card_init_cache_binary,
     ...
     /* Making FPLMN as optional for buggy card where FPLMN file access 
        denied. If FPLMN not accessible then NAS will try to camp on forbidden 
        network which lead to increase power cunsumption but it is
        acceptable in case of buggy card */
     {MMGSDI_USIM_FPLMN,     is_mandatory, mmgsdi_card_init_cache_binary, NULL},
     ...
     {MMGSDI_USIM_MSISDN,    FALSE,        mmgsdi_card_init_cache_record, NULL},
     {MMGSDI_USIM_ACL,       FALSE,        mmgsdi_card_init_cache_binary, NULL},
    };
...
)
{
  ...
  mmgsdi_cache_init_order_type csim_pup_init_pre_esn_table[] =
    ...
    {{MMGSDI_CSIM_CST,               TRUE,  mmgsdi_card_init_cache_binary, NULL},
     ...
     {MMGSDI_CSIM_AD,                TRUE,  mmgsdi_card_init_cache_binary, NULL},
...
)
{
  ...
  if ((app_type == MMGSDI_APP_USIM) || (app_type == MMGSDI_APP_SIM))
  {
    ...
    if(MMGSDI_SUCCESS != mmgsdi_cphs_post_pin1_init(session_id,
                                                    slot_id,
                                                    app_type,
                                                    read_cphs_efs))
    {
...
)
{
  ...
  mmgsdi_cache_init_order_type sim_pre_perso_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
     ...
     {MMGSDI_GSM_ACC,       is_mandatory, mmgsdi_card_init_cache_binary,
      mmgsdi_card_init_validate_acc_data                                     },
     {MMGSDI_GSM_KC,        is_mandatory, mmgsdi_card_init_cache_binary, NULL},
     {MMGSDI_GSM_LOCIGPRS,  FALSE,        mmgsdi_card_init_cache_binary, NULL},
     {MMGSDI_GSM_KCGPRS,    FALSE,        mmgsdi_card_init_cache_binary, NULL},
  ...
  mmgsdi_cache_init_order_type usim_pre_perso_init_table[] =
    /* file name,        mandatory, init func,               validation func */
    {
     ...
     /* Making PSLOCI as optional for buggy card where PSLOCI file access 
        denied. If PSLOCI not accessible then NAS will read this file 
        from NV NV_GSM_LOCIGPRS_I */
     {MMGSDI_USIM_PSLOCI,    is_mandatory, mmgsdi_card_init_cache_binary, NULL},
     {MMGSDI_USIM_EPSLOCI,   FALSE,        mmgsdi_card_init_cache_binary,
      mmgsdi_card_init_validate_epsloci                                       },
     {MMGSDI_USIM_KEYS,      is_mandatory, mmgsdi_card_init_cache_binary, NULL},
     {MMGSDI_USIM_KEYSPS,    FALSE,        mmgsdi_card_init_cache_binary, NULL},
     ...
     {MMGSDI_USIM_KC,        FALSE,        mmgsdi_card_init_cache_binary, NULL},
     {MMGSDI_USIM_KCGPRS,    FALSE,        mmgsdi_card_init_cache_binary, NULL},
  ...
  if(mmgsdi_switch_imsi_enable)
  {
    if(mmgsdi_switch_imsi_get_current_profile(&mmgsdi_imsi_switch_crnt_active_profile, &mmgsdi_imsi_switch_crnt_nw_type ) != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_2("not able to get imsi switch profile mmgsdi_imsi_switch_crnt_active_profile 0x%x, mmgsdi_imsi_switch_crnt_nw_type 0x%x",
                     mmgsdi_imsi_switch_crnt_active_profile, mmgsdi_imsi_switch_crnt_nw_type);
      mmgsdi_status = MMGSDI_HALT_INIT_ERROR;
    }
  }
...
/*===========================================================================
FUNCTION:      MMGSDI_CARD_INIT_REMOVE_APPS_AVAILABLE_INFO
  ...
  switch(internal_pup_status)
  {
    case MMGSDI_SUCCESS:
      ...
      if (int_app_info_ptr->app_state == MMGSDI_APP_STATE_READY)
      {
        ...
        (void)mmgsdi_eons_cache_opl(session_id);
...
)
{
     ...
     mmgsdi_generic_data_ptr->onchip_act_data[slot_index].auto_activation_enabled != FALSE)
  {
    /* EFS item mandates that Onchip be auto activated. So, MMGSDI will try to
       ...
       succeeds */
    if(MMGSDI_SUCCESS == mmgsdi_onchip_auto_activate(
                           req_ptr->request_header.slot_id))
    {
