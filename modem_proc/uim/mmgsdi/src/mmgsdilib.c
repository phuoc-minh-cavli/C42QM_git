...
12/16/16   dd      Add test mode logging enabled APIs for sensitive info control
...
{
  return mmgsdi_onchip_create_onchip_request(client_id,
                                             card_slot,
                                             onchip_sim_config,
...
/* ============================================================================
  ...
  @param[out] atr_data_ptr    this will contain atr data. ATR data is valid 
                              only if return value is MMGSDI_SUCCESS.

  @return
  MMGSDI_SUCCESS          -- Request processed sucessfully
  MMGSDI_INCORRECT_PARAMS -- Parameters are not within the appropriate ranges.
  MMGSDI_ERROR            -- Error has occured.
...
mmgsdi_return_enum_type mmgsdi_get_atr_sync(
  mmgsdi_client_id_type    client_id,
  mmgsdi_slot_id_enum_type card_slot,
  mmgsdi_atr_info_type    *atr_data_ptr
)
{
  mmgsdi_return_enum_type   mmgsdi_status = MMGSDI_SUCCESS;
  mmgsdi_slot_data_type    *slot_data_ptr = NULL;
  ...
  { 
    ...
    atr_data_ptr->atr_length = slot_data_ptr->card_atr.atr_length;
 
    mmgsdi_memscpy(atr_data_ptr->atr_data,
                   sizeof(atr_data_ptr->atr_data),
...
/*===========================================================================
   ...
   PARAMETERS:
     ...
     refresh_enforcement_policy_type             Refresh enforcement policy information
...
/*=============================================================================
   FUNCTION:      MMGSDI_CLIENT_ID_AND_EVT_REG_SYNC
  ...
  Upon successful completion of this command, client will receive a client ID.

  @param[in] evt_cb_ptr:      Pointer to callback for event notifications.
  ...
  @return
  mmgsdi_client_id_type:      In case of Failure it will return 0.
                              Otherwise Valid Client ID will be returned.
  ...
  None
=========================================================================== */
mmgsdi_client_id_type  mmgsdi_client_id_and_evt_reg_sync
(
...
)
{
  ...
  memset(&rex_task_name, 0x00, sizeof(rex_task_name));
  ...
  {
    strlcpy(client_name, client_name_ptr, sizeof(client_name));
  }
  else
  {
    /* If the client did not provide any name, copy task_name as both client_name */
    strlcpy(client_name, rex_task_name, sizeof(client_name));
  }
  ...
  if(mmgsdi_status == MMGSDI_SUCCESS)
  {
    mmgsdi_status = mmgsdi_util_get_client_id_index(client_id, &client_index);
    if (mmgsdi_status == MMGSDI_SUCCESS)
    {
      /* mmgsdi_util_get_client_id_index success guarantees this dereference is ok */
      mmgsdi_client_id_reg_table[client_index]->evt_cb_ptr = evt_cb_ptr;
      strlcpy(mmgsdi_client_id_reg_table[client_index]->client_task_name, rex_task_name, MMGSDI_TASK_NAME_LEN);
      /* Set the event reg cnf sent as true for clients so that events can be sent */
      ...
      for(slot_id = MMGSDI_SLOT_1; slot_id <= MMGSDI_SLOT_3; slot_id++)
      {
        ...
        if (slot_data_ptr != NULL)
        {
          mmgsdi_evt_build_and_notify_card_evt_from_state(slot_id, client_index);
        }
      }
    }
  }
  ...
  return client_id;
}/* mmgsdi_client_id_and_evt_reg_sync */
...
/*===========================================================================
   FUNCTION:      MMGSDI_CLIENT_ID_AND_EVT_DEREG_SYNC
=========================================================================== */
/**
  This sync function deregisters the client for event callbacks, and deallocates 
  the client ID and any sessions owned by that client.

  @param[in] client_id        Client ID.

  @return
  MMGSDI_SUCCESS          -- client deregistered successfully
  MMGSDI_INCORRECT_PARAMS -- Parameters are not within the appropriate ranges.
  MMGSDI_ERROR            -- Not able to process request
  ...
  None
============================================================================ */
mmgsdi_return_enum_type  mmgsdi_client_id_and_evt_dereg_sync (
  mmgsdi_client_id_type      client_id
)
{
  mmgsdi_return_enum_type mmgsdi_status    = MMGSDI_ERROR;
  ...
  return mmgsdi_status;
} /* mmgsdi_client_id_and_evt_dereg_sync */
...
/*===========================================================================
  FUNCTION:  MMGSDI_GET_SE13_PLMN_NAMES_SYNC

  This API is used to get plmn name by directly accessing SE13 table
  for given PLMN Id caller should allocate the memory for obtaining plmn name info.
  After returning from the function, it is the responsibility of the caller
  to free the memory.
  ...
  None
=========================================================================== */
mmgsdi_return_enum_type   mmgsdi_get_se13_plmn_name_sync
(
  mmgsdi_client_id_type              client_id,
...
)
{
  ...
  UIM_MSG_HIGH_3("Get ONS for plmn id: 0x%x, 0x%x, 0x%x",
                 plmn_id.plmn_id_val[0],
                 plmn_id.plmn_id_val[1],
                 plmn_id.plmn_id_val[2]);

  mmgsdi_status = mmgsdi_proc_get_se13_plmn_name(plmn_id, plmn_name_info_ptr);

  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

  return mmgsdi_status;
}/* mmgsdi_get_se13_plmn_name_sync */


/*====================================================================
FUNCTION:  MMGSDI_GET_SE13_PLMN_INFO_BY_NAME_SYNC
=========================================================================== */
/**
  ...
  should be valid. MMGSDI returns info related to all SE13 table entries 
  where plmn name matches.
  This function can return limited ( upto 10 i.e MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME)
  number of SE13 table entry.

  Upon successful completion, this function return with MCC, MNC, RAT type,
  PLMN long name and PLMN short name.
  ...
  @param[in] rat_type            RAT type, if valid RAT type is
                                 ...
                                 matching the RAT.
  @param[out] se13_plmn_info_ptr It contains MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME set of PLMN info
                                 (MCC, MNC, NETWORK MASK, SHORT and LONG names).
                                 Caller has to allocate Memory (MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME * mmgsdi_se13_plmn_static_info_type).

  @dependencies
  PLMN name string must be in Default 7 bit coded alphabet encoding.

  @limitations
  Maximum MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME number of PLMN IDs is allowed corresponding to any name.

  @return 
  This function will return 0 if some error has occured or no matched is found for provided name.
  otherwise it will return number of PLMN IDs matched. Maximum MMGSDI_MAX_PLMN_ID_FOR_ONE_NAME
  number of PLMN IDs is allowed corresponding to one name.

  @sideeffects
  None 
====================================================================== */
uint16 mmgsdi_get_se13_plmn_info_by_name_sync(
  mmgsdi_client_id_type              client_id,
...
)
{
  ...
  /*---------------------------------------------------------------------------
    Input Parameters Check
    1) rat type validity check
    2) se13_plmn_info_ptr validity check
    3) plmn long name validity check
    4) plmn short name validity Check
  ---------------------------------------------------------------------------*/
  if(rat_type < MMGSDI_RAT_NONE && rat_type > MMGSDI_RAT_TDS)
  {
    return number_of_plmn_ids;
  }
  ...
  if(se13_plmn_info_ptr        == NULL                                                                   ||
     (plmn_long_name.data_ptr != NULL  &&
      (plmn_long_name.data_len <= 0 || plmn_long_name.data_len > MMGSDI_EONS_MAX_PLMN_LONG_NAME_LEN))    ||
     (plmn_long_name.data_len > 0 && plmn_long_name.data_ptr == NULL)                                    ||
     (plmn_short_name.data_ptr != NULL &&
      (plmn_short_name.data_len <= 0 || plmn_short_name.data_len > MMGSDI_EONS_MAX_PLMN_SHORT_NAME_LEN)) ||
  ...
  number_of_plmn_ids = mmgsdi_eons_get_se13_plmn_info_by_name_sync(
                         plmn_long_name,
                         plmn_short_name,
                         rat_type,
                         se13_plmn_info_ptr);
  ...
  return number_of_plmn_ids;
}/* mmgsdi_get_se13_plmn_info_by_name_sync */
...
/* ==========================================================================
FUNCTION:       MMGSDI_SET_BUILTIN_PLMN_LIST_SYNC

  This function is used to set builtin PLMN list which is received from network.
  MMGSDI stores this list in EFS and gives it precedence over EF-OPLMNwACT.
  This function include EFS write operation.
  ...
  MMGSDI_INCORRECT_PARAMS.-- The parameters supplied to the API are not
                             within appropriate ranges.
  MMGSDI_ERROR.  --          Some Error has occured

  @dependencies
  The client must have a valid client ID.
=========================================================================== */
mmgsdi_return_enum_type  mmgsdi_set_builtin_plmn_list_sync (
  mmgsdi_client_id_type                  client_id,
...
)
{
  ...
  mmgsdi_status = mmgsdi_store_builtin_plmn_list(
                    &builtin_plmn_list);
  
  MMGSDIUTIL_LEAVE_CLIENT_APP_DATA_CRIT_SECT;

  return mmgsdi_status;
}/* mmgsdi_set_builtin_plmn_list_sync */
...
{
  return mmgsdi_pkcs_queue_start_potential_silent_file_change(client_id, slot);
...
/* ==========================================================================
  FUNCTION:      MMGSDI_SWITCH_IMSI_TN_NTN

  DESCRIPTION:
    This function will be called by NAS whenever network switch between TN and NTN

  PARAMETERS:
  mmgsdi_imsi_switch_nw_enum_type switch_to_network
  ...
  SIDE EFFECTS:
    None
==========================================================================*/
void mmgsdi_switch_imsi_tn_ntn
(
  mmgsdi_imsi_switch_nw_enum_type switch_to_network,
  sys_plmn_id_s_type              plmn
)
{
  ...
  /* As per NAS, At this point, Device wont be in full service but exact service status will be unknown*/
   mmgsdi_status = mmgsdi_switch_imsi_profile(mmgsdi_generic_data_ptr->client_id,
                                              ...
                                              MMGSDI_IMSI_SWITCH_SRC_API,
                                              0); 
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_1("mmgsdi_switch_imsi_tn_ntn queue mmgsdi_status 0x%x", mmgsdi_status);
  }
}/* mmgsdi_switch_imsi_tn_ntn */


/* ==========================================================================
  FUNCTION:      MMGSDI_IMSI_SWITCH_GET_CRNT_PROFILE_ID

  DESCRIPTION:
    This function will return currently activated IMSI profile 
  ...
  LIMITATIONS:
    Should be call after SUBREADY, else Profile Id unknown can be return 
  ...
  SIDE EFFECTS:
    None
==========================================================================*/
mmgsdi_imsi_switch_profile_id_enum_type mmgsdi_imsi_switch_get_crnt_profile_id(void)
{

  return mmgsdi_imsi_switch_crnt_active_profile;
}
...
/* ===========================================================================
   FUNCTION: MMGSDI_IS_TEST_MODE_LOGGING_ENABLED_FOR_SUBSCRIPTION
 
   DESCRIPTION:
     This function is used to get whether test mode logging is enabled for
     subscription. If enabled, logging of keys is allowed in the caller. If
     disabled, otherwise.  If a test card is in use, it will always return ok. 
     If it is a commercial card, it will take into account the value in the 
     protected NV item for test mode logging. SYS_MODEM_AS_ID_1 means mmgsdi 
     will extract info from PRI session and SYS_MODEM_AS_ID_2 means from SEC 
     session info.  

   PARAMETERS:
     sub_id: The subscription ID number for this query
   ...
   RETURN VALUE:
     TRUE  : Test mode logging allowed. It is ok to log security sensitive keys
     FALSE : Otherwise

   SIDE EFFECTS:
     None
=========================================================================== */
boolean mmgsdi_is_test_mode_logging_enabled_for_subscription(
  sys_modem_as_id_e_type sub_id
)
{
  if (mmgsdi_is_test_mode_logging_enabled.always_enable) 
  {
    return TRUE;
  }
  switch(sub_id)
  {
    case SYS_MODEM_AS_ID_1:
      return mmgsdi_is_test_mode_logging_enabled.subs[0];
    case SYS_MODEM_AS_ID_2:
      return mmgsdi_is_test_mode_logging_enabled.subs[1];
    case SYS_MODEM_AS_ID_3:
      return mmgsdi_is_test_mode_logging_enabled.subs[2];
    default:
      break;
  }

  return FALSE;
}/*mmgsdi_is_test_mode_logging_enabled_for_subscription*/


/* ===========================================================================
   FUNCTION: MMGSDI_IS_TEST_MODE_LOGGING_ENABLED_FOR_SLOT
 
   DESCRIPTION:
     This function is used to get whether test mode logging is enabled. If 
     enabled, logging of keys is allowed in the caller. If disabled, otherwise.  
     If a test card is in use, it will always return ok. If it is a commercial 
     card, it will take into account the value in the protected NV item for test
     mode logging.  

   PARAMETERS:
     slot_id: The slot ID number for this query
   ...
   SIDE EFFECTS:
     None
=========================================================================== */
boolean mmgsdi_is_test_mode_logging_enabled_for_slot(
  mmgsdi_slot_id_enum_type slot_id
)
{
  uint8 slot_index = 0;
  ...
  {
    return mmgsdi_is_test_mode_logging_enabled.slots[slot_index];
  }

  return FALSE;
}/*mmgsdi_is_test_mode_logging_enabled_for_slot*/
...
)
{
  ...
  return client_id;
} /* mmgsdi_get_global_client_id */


/*=============================================================================
FUNCTION        MMGSDI_GET_M2M_EID

DESCRIPTION
  This function allows the user to retrieve the EID of the M2M eUICC compliant with SGP.02.
...
RETURN VALUE
  mmgsdi_return_enum_type
===============================================================================*/
mmgsdi_return_enum_type mmgsdi_get_m2m_eid(
  mmgsdi_slot_id_enum_type     slot_id,
  mmgsdi_callback_type         response_cb_ptr,
  mmgsdi_client_data_type      client_ref
)
{
  mmgsdi_task_cmd_type                 *task_cmd_ptr  = NULL;
  mmgsdi_return_enum_type               mmgsdi_status = MMGSDI_ERROR;
  mmgsdi_get_m2m_eid_req_type          *msg_ptr       = NULL;
  ...
  /* --------------------------------------------------------------------------
      No need to Align pointers of the structures members
     ------------------------------------------------------------------------ */
  msg_ptr = &task_cmd_ptr->cmd.cmd.get_m2m_eid_req;
  /*
   * Populate header.
   */
  task_cmd_ptr->cmd.cmd_enum                 = MMGSDI_GET_M2M_EID_REQ;
  msg_ptr->request_header.client_data        = client_ref;
  msg_ptr->request_header.slot_id            = slot_id;
  msg_ptr->request_header.response_cb        = response_cb_ptr;

  msg_ptr->request_header.orig_request_type  = MMGSDI_GET_M2M_EID_REQ;
  msg_ptr->request_header.request_type       = MMGSDI_GET_M2M_EID_REQ;
  ...
  UIM_MSG_HIGH_2("Queue of MMGSDI command: MMGSDI_GET_M2M_EID_REQ status 0x%x for slot: 0x%x",
