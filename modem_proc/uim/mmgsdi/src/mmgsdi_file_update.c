...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_file_update.c#1 $$ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_SESSION_REGISTER_FILE_UPDATE_REQ,
                                                           (void*)req_ptr,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  ...
  for (client_index = 0; client_index < MMGSDI_MAX_CLIENT_INFO; client_index++)
  {
          ...
          (session_index < MMGSDI_MAX_SESSION_INFO); session_index++)
    {
      ...
      if(send_evt)
      {
        ...
        mmgsdi_file_update_build_and_send_evt(notify_type, file_access, write_extra_ptr);
      }
