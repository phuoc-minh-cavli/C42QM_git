...
#ifndef FEATURE_SKIP_SEARCH_FOR_PB_FILES
...
#endif /* FEATURE_SKIP_SEARCH_FOR_PB_FILES */
...
)
{
  ...
  for(loop_cnt = 0; loop_cnt < num_files; loop_cnt++)
  {
    ...
    mmgsdi_file_node_ptr = mmgsdi_pb_cache_read_file_node(file_path,
                                                          mmgsdi_pb_cache_tbl_ptr[slot_index],
                                                          session_type);
    if(mmgsdi_file_node_ptr != NULL)
    {
      /* Freeing phonebook cache record data for the file being refreshed currently */
      mmgsdi_pb_cache_delete_all_records(mmgsdi_file_node_ptr);

      /* Delete file attributes and search data also as the content may change as part of REFRESH FCN */
      mmgsdi_pb_cache_delete_file_node(&mmgsdi_pb_cache_tbl_ptr[slot_index],
                                       mmgsdi_file_node_ptr);
