...
Copyright (c) 2014 - 2016, 2019, 2021 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_simlock.c#4 $$ $DateTime: 2021/07/07 00:29:01 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/07/21   tq      Enable SIMLOCK on CT
05/11/21   tq      Reverted simlock CR changes as SIMLOCK is disabled on CT
01/28/21   tq      Update category based on slot policy during initialization
03/11/19   vdc     Move SIMLOCK algo message type to heap
...
)
{
  ...
  for (slot = 0; slot < MMGSDI_MAX_NUM_SLOTS && slot < SIMLOCK_SLOT_COUNT_MAX; slot++)
  {
        ...
        app++)
    {
      ...
      /* EF-IMSI has a file size of 9 while EF-IMSI_M and EF-IMSI_T have size 10 */
      if (simlock_algo_data_ptr->card_data[slot].subscription_data[sub].sub_type == SIMLOCK_SUBSCRIPTION_3GPP)
      {
        data_len = SIMLOCK_IMSI_FILE_LEN - 1;
      }
      else
      {
        data_len = SIMLOCK_IMSI_FILE_LEN;
      }

      mmgsdi_status = mmgsdi_cache_read_item(imsi,
                                             mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type,
                                             session_type,
                                             data_len,
                                             0,
      ...
      if (mmgsdi_status != MMGSDI_SUCCESS)
      {
        UIM_MSG_ERR_1("Failed to read cached IMSI: 0x%x", mmgsdi_status);
        /* Applications where the IMSI has not been initalized will
           not proceed with algorithm */
        continue;
      }

      simlock_algo_data_ptr->card_data[slot].subscription_data[sub].imsi.imsi_len = (uint8)data_len;
      ...
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        if(data_len > SIMLOCK_MAX_GID_LEN)
        {
          data_len = SIMLOCK_MAX_GID_LEN;
        }

        (void)mmgsdi_cache_read_item(gid1,
                                     mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type,
                                     session_type,
                                     data_len,
                                     0,
                                     ...
                                     NULL);
        simlock_algo_data_ptr->card_data[slot].subscription_data[sub].gid1.gid_len = data_len;
      }
      ...
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        (void)mmgsdi_cache_read_item(gid2,
                                     mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type,
                                     session_type,
                                     data_len,
                                     0,
                                     ...
                                     NULL);
        simlock_algo_data_ptr->card_data[slot].subscription_data[sub].gid2.gid_len = data_len;
      }
          ...
          mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type == MMGSDI_APP_USIM)
      {
        ...
        data_len = SIMLOCK_AD_FILE_LEN;
        mmgsdi_status = mmgsdi_cache_read_item(ad,
                                               mmgsdi_data_slot_ptr[slot]->app_info_pptr[app]->app_data.app_type,
                                               session_type,
                                               data_len,
                                               0,
        ...
        if (mmgsdi_status == MMGSDI_SUCCESS)
        {
          simlock_algo_data_ptr->card_data[slot].subscription_data[sub].ad.ad_len = (uint8)data_len;
        }
      }
...
)
{
  ...
  switch (failure_reason)
  {
    ...
    case SIMLOCK_SUBSCRIPTION_INVALID_REASON_APP_REQUIRED_BUT_LOCKED:
      /* Not checking the return value here. Nothing else do, if the
         function doesn't return a success */
      if(update_app_type == MMGSDI_APP_CSIM)
      {
        (void)mmgsdi_simlock_get_locked_feature_for_app(mmgsdi_slot, MMGSDI_APP_USIM, feature_ptr);
      }
      else if (update_app_type == MMGSDI_APP_USIM)
      {
        (void)mmgsdi_simlock_get_locked_feature_for_app(mmgsdi_slot, MMGSDI_APP_CSIM, feature_ptr);
      }
      break;
...
)
{
  ...
  uint8                                current_slot          = 0;
  boolean                              use_slot1_config      = FALSE;
  ...
  use_slot1_config = simlock_policy_uses_slot1_config_for_all_slots(simlock_status_ptr->slot_policy);

  for (slot = 0; slot < SIMLOCK_SLOT_COUNT_MAX; slot++)
  {
    /* If use_slot1_config is true, then for all the slots ,mmgsdi_simlock_status_table will be updated
       with slot 1 config. Else mmgsdi_simlock_status_table will be updated by corresponding slot wise config */
    if(!use_slot1_config)
    {
      current_slot = slot;
    }
    for (category = 0; category < SIMLOCK_CATEGORY_COUNT_MAX; category++)
    {
      mmgsdi_simlock_status_table[slot][category].category =
        simlock_status_ptr->lock_info[current_slot].category_info[category].category;

      mmgsdi_simlock_status_table[slot][category].category_status =
        simlock_status_ptr->lock_info[current_slot].category_info[category].category_status;
    }
  }
...
)
{
  ...
  simlock_run_algorithm_msg_req_type    *simlock_algo_data_ptr   = NULL;
  ...
  MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(simlock_algo_data_ptr,
    sizeof(simlock_run_algorithm_msg_req_type));
  if(simlock_algo_data_ptr == NULL)
  {
    MMGSDIUTIL_TMC_MEM_FREE(session_id_table_ptr);
    return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
  }

  mmgsdi_status = mmgsdi_simlock_build_algo_data(simlock_algo_data_ptr,
                                                 session_id_table_ptr);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    UIM_MSG_ERR_1("Failed to save SIM Lock algo data: 0x%x", mmgsdi_status);
    MMGSDIUTIL_TMC_MEM_FREE(session_id_table_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(simlock_algo_data_ptr);
      ...
      ignore_result)
  {
    ...
    simlock_status = simlock_run_algo(simlock_algo_data_ptr,
                                      mmgsdi_simlock_psm_init_cb,
                                      session_id_table_ptr);
