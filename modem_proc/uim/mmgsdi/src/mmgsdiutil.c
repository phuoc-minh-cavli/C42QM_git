...
Copyright (c) 2004 - 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdiutil.c#2 $$ $DateTime: 2023/06/21 11:32:16 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/21/20   tq     Check app type while allocating entry in channel_ptr_table for icc
08/26/19   sch     Manage channel open gracefully if EF-DIR not present
...
12/08/18   sg      Removed F3 Message
...
08/17/17   tq      Remove select aid failed linklist
06/13/17   vdc     Don't update the card error cause in RESET AUTO mode
...
03/01/17   vdc     Send verify PIN command after enabling the PIN for buggy card
02/07/17   vdc     Block select AID only for ARA-M AID
09/12/16   kv      Add access check for MMSS EFs
...
06/22/16   nr      featurize 3GPP2 functionality
...
05/25/16   dd      Skip Search Req for PB files
05/24/16   ar      Review of macros used by MMGSDI
05/19/16   tkl     F3 log prints cleanup
05/16/16   sp      Extend SIMLOCK featurization in mmgsdi_util_tmsi_plmn_check
...
04/25/16   dd      Remove the extra debug loggings
...
void mmgsdi_util_free_response_ptr(mmgsdi_cnf_type * response_ptr)
{
  ...
  {
    cmd_type = response_ptr->response_header.response_type;
    switch(cmd_type)
    {
...
)
{
  ...
  MMGSDIUTIL_RETURN_IF_NULL_2(request_hdr_ptr, extra_param_pptr);
  MMGSDIUTIL_RETURN_IF_OUT_OF_RANGE(index, 0, MMGSDI_MAX_CLIENT_REQ_INFO - 1);
...
)
{
  ...
  /* Check if MMSS file access is allowed*/
  if ((mmgsdi_access_ptr->file.file_enum == MMGSDI_TELECOM_MLPL ||
       mmgsdi_access_ptr->file.file_enum == MMGSDI_TELECOM_MSPL ||
       mmgsdi_access_ptr->file.file_enum == MMGSDI_TELECOM_MMSSMODE) &&
      !(mmgsdi_file_access_allowed_for_mmss_ef(session_id)))
  {
...
)
{
  ...
  task_cmd_len = sizeof(mmgsdi_task_cmd_type) + msg_ptr->rsp_data.data_len;
...
/* ==========================================================================
   FUNCTION:      MMGSDI_SEND_CMD_TO_UIM_SERVER_SYNCH
...
)
{
  ...
  /* UIMDRV makes a copy of the command, so we no longer needs it and we
...
/* ==========================================================================
   FUNCTION:      MMGSDI_SEND_CMD_TO_UIM_SERVER
...
)
{
  ...
  /* Check if there is a channel assigned to the session or not */
  ...
  *channel_index_ptr = MMGSDI_MAX_CHANNEL_INFO;
  ...
  for (i = 0; i < MMGSDI_MAX_CHANNEL_INFO; i++)
  {
    ...
    if ((app_type == MMGSDI_APP_SIM || app_type == MMGSDI_APP_RUIM) &&
        (mmgsdi_channel_info_ptr_table[i]->app_info_ptr == NULL ||
         mmgsdi_channel_info_ptr_table[i]->app_info_ptr->app_data.app_type != app_type))
    {
      /* SIM or RUIM apps (because slot_id and channel_id could be same for
         both these apps)... so, continue looping through */
      continue;
    }
...
/* ==========================================================================
   FUNCTION:      MMGSDI_UTIL_ALIGN_PTR
  ...
  mmgsdi_client_req_extra_info_type **extra_param_pptr)
{
  ...
  case MMGSDI_PIN_OPERATION_REQ:
    ...
    if(((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_id != MMGSDI_HIDDENKEY)
    {
      ...
      if(((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_puk.data_len != 0)
      {
           ...
           (((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_op == MMGSDI_PIN_OP_DISABLE_AND_REPLACE))
        {
             ...
             (((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_puk.data_len <= MMGSDI_PIN_MAX_LEN))
          {
            ...
            /* Cache the PIN data into extra parameter pointer for verification of PIN after enabling it */
            if(mmgsdi_nv_get_feature_status(MMGSDI_FEATURE_VERIFY_PIN_AFTER_CHANGE_OR_ENABLE, MMGSDI_NV_CONTEXT_DEVICE_SPECIFIC) == MMGSDI_FEATURE_ENABLED)
            {
      ...
      /* Cache the user entered PIN data into extra parameter pointer from the request for verification
         of PIN after enabling or changing it */
      else if((mmgsdi_nv_get_feature_status(MMGSDI_FEATURE_VERIFY_PIN_AFTER_CHANGE_OR_ENABLE, MMGSDI_NV_CONTEXT_DEVICE_SPECIFIC) == MMGSDI_FEATURE_ENABLED) &&
              ((((mmgsdi_pin_operation_req_type*)cmd_ptr)->pin_op == MMGSDI_PIN_OP_ENABLE) ||
  ...
  case MMGSDI_SRV_AVAILABLE_REQ:
    ...
    (*extra_param_pptr)->srv_available_data.service = ((mmgsdi_srv_available_req_type*)cmd_ptr)->service;
    break;
  ...
  case MMGSDI_SESSION_GET_INFO_REQ:
    ...
    (*extra_param_pptr)->session_get_info_data.query_type =
                   ((mmgsdi_session_get_info_req_type*)cmd_ptr)->session_info_query.query_type;
    (*extra_param_pptr)->session_get_info_data.query_session_id =
                   ((mmgsdi_session_get_info_req_type*)cmd_ptr)->session_info_query.query_key.session_id;
    (*extra_param_pptr)->session_get_info_data.query_session_type =
                   ((mmgsdi_session_get_info_req_type*)cmd_ptr)->session_info_query.query_key.session_type;
    break;
  ...
  case MMGSDI_SESSION_DISABLE_SERVICE_REQ:
    ...
    (*extra_param_pptr)->enable_service_data.service_type  = ((mmgsdi_session_enable_service_req_type*)cmd_ptr)->service;
    (*extra_param_pptr)->enable_service_data.client_data   = ((mmgsdi_session_enable_service_req_type*)cmd_ptr)->request_header.client_data;
    (*extra_param_pptr)->enable_service_data.orig_callback = ((mmgsdi_session_enable_service_req_type*)cmd_ptr)->request_header.response_cb;
    break;
  ...
  case MMGSDI_GENERATE_KEY_VPM_REQ:
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(*extra_param_pptr,
      sizeof(mmgsdi_generate_key_extra_info_type));
    if ((*extra_param_pptr) == NULL)
    {
      return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
    }

    (*extra_param_pptr)->generate_key.vpm_first_octet = ((mmgsdi_generate_key_req_type*)cmd_ptr)->first_octet;
    (*extra_param_pptr)->generate_key.vpm_last_octet  = ((mmgsdi_generate_key_req_type*)cmd_ptr)->last_octet;
    break;

  case MMGSDI_ISIM_AUTH_REQ:
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(*extra_param_pptr,
      sizeof(mmgsdi_session_isim_auth_extra_info_type));
    if ((*extra_param_pptr) == NULL)
    {
      return MMGSDI_MEMORY_ERROR_HEAP_EXHAUSTED;
    }

    (*extra_param_pptr)->isim_auth.auth_context = ((mmgsdi_auth_req_type*)cmd_ptr)->auth_context;
    break;
  ...
  const mmgsdi_cnf_type   *cnf_ptr)
{
  ...
  switch(cnf)
  {
    ...
    case MMGSDI_SESSION_ENABLE_SERVICE_REQ:
      ...
      callback_cnf_ptr->mmgsdi_cnf.response_header.response_type = MMGSDI_SESSION_ENABLE_SERVICE_REQ;
    ...
    case MMGSDI_SESSION_DISABLE_SERVICE_REQ:
      ...
      callback_cnf_ptr->mmgsdi_cnf.response_header.response_type = MMGSDI_SESSION_DISABLE_SERVICE_REQ;
      callback_cnf_ptr->client_req_cb = extra_param_ptr->enable_service_data.orig_callback;
  ...
  boolean                            sync_process)
{
  ...
  {
    mmgsdi_status = mmgsdi_uim_common_unblock_verify_pin(pin_req_ptr,
  ...
  boolean                            sync_process)
{
  ...
  mmgsdi_status = mmgsdi_uim_icc_select(req_ptr, sync_process, NULL);
...
)
{
  ...
  if(slot_data_ptr != NULL)
  {
    ...
    slot_data_ptr->aram_aid_select_failed            = FALSE;
...
{
  uint32 mf_child_path     = 0;

  switch (file_access.access_method)
  {
    case MMGSDI_EF_ENUM_ACCESS:
      UIM_MSG_HIGH_1("For ef_enum: 0x%x", file_access.file.file_enum);
      break;
  ...
  uint8                          *app_index_ptr)
{
  ...
  *app_index_ptr = app_index;
  return MMGSDI_SUCCESS;
} /* mmgsdi_util_get_app_index_for_app */
...
DESCRIPTION
  This function checks if the input Client ID is nonzero or not.
...
/*===========================================================================
FUNCTION MMGSDI_UTIL_PREINIT_MMGSDI
...
)
{
  ...
  return mmgsdi_status;
} /* mmgsdi_util_select_adf_before_pin2_op */
...
/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_GET_PROV_SESSION_ID_FOR_APP
...
/*===========================================================================
  FUNCTION MMGSDI_UTIL_READ_CACHE_FILE
...
/* ==========================================================================
FUNCTION MMGSDI_UTIL_GET_EXPECTED_APP_TYPE

DESCRIPTION
  This function determines the app type  from session type passed.
...
SIDE EFFECTS
  NONE
==========================================================================*/
static mmgsdi_return_enum_type mmgsdi_util_get_expected_app_type(
  mmgsdi_slot_id_enum_type        slot_id,
  mmgsdi_session_id_type          session_id,
  mmgsdi_app_enum_type           *app_type_ptr
)
{
  ...
  switch(session_type)
  {
    ...
    case MMGSDI_GW_PROV_SEC_SESSION:
    case MMGSDI_GW_PROV_TER_SESSION:
      *app_type_ptr = (protocol == MMGSDI_UICC)? MMGSDI_APP_USIM : MMGSDI_APP_SIM;
      break;

    case MMGSDI_1X_PROV_PRI_SESSION:
    case MMGSDI_1X_PROV_SEC_SESSION:
    case MMGSDI_1X_PROV_TER_SESSION:
      *app_type_ptr = (protocol == MMGSDI_UICC)? MMGSDI_APP_CSIM : MMGSDI_APP_RUIM;
      break;
  ...
  return MMGSDI_SUCCESS;
} /* mmgsdi_util_get_expected_app_type */
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_get_expected_app_type(slot_id,
                                                    session_id,
                                                    &app_type);
  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
...
/* =============================================================================
  FUNCTION:      MMGSDI_UTIL_FILL_AND_ALLOC_REQUEST_TABLE

  DESCRIPTION:
    This function is used to alloc client index table and fill the client table
    with header data.
  ...
  SIDE EFFECTS:
    None
=============================================================================*/
mmgsdi_return_enum_type mmgsdi_util_fill_and_alloc_request_table(
  mmgsdi_cmd_enum_type                cmd_type,
  const void                         *cmd_ptr,
  mmgsdi_client_req_extra_info_type **out_extra_param_pptr,
  uint8                              *index_ptr
)
{
  mmgsdi_return_enum_type                mmgsdi_status           = MMGSDI_SUCCESS;
  uint8                                  loop_index              = 0;
  mmgsdi_uim_client_req_user_data_type **client_index_table_pptr = NULL;

  MMGSDIUTIL_RETURN_IF_NULL_2(index_ptr, cmd_ptr);

  if(mmgsdi_util_get_current_task() >= MMGSDI_NUM_TASKS)
  {
    /* Note: The only non-MMGSDI task usage of this function could be from
       MMGSDI cancel API in client context. Hence, instead of returning error,
       make sure that the API call entered the task sync critical section */
  ...
  for (loop_index = 0; loop_index < MMGSDI_MAX_CLIENT_REQ_INFO; loop_index++)
  {
    if (mmgsdi_client_req_table_info_ptr[loop_index] == NULL)
    {
      client_index_table_pptr = &mmgsdi_client_req_table_info_ptr[loop_index];
      MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(*client_index_table_pptr,
                                         sizeof(mmgsdi_uim_client_req_user_data_type));
  ...
  if (client_index_table_pptr == NULL ||
      *client_index_table_pptr == NULL)
  {
    UIM_MSG_ERR_0("Unable to get free client index");
    return MMGSDI_ERROR;
  }

  mmgsdi_status = mmgsdi_util_alloc_and_populate_extra_param(cmd_type,
                                                             cmd_ptr,
                                                             &(*client_index_table_pptr)->extra_param_ptr);

  if(mmgsdi_status != MMGSDI_SUCCESS)
  {
    MMGSDIUTIL_TMC_MEM_FREE((*client_index_table_pptr)->extra_param_ptr);
    MMGSDIUTIL_TMC_MEM_FREE(*client_index_table_pptr);
    return mmgsdi_status;
  }

  (*client_index_table_pptr)->client_data        = ((mmgsdi_request_header_type*)cmd_ptr)->client_data;
  (*client_index_table_pptr)->client_id          = ((mmgsdi_request_header_type*)cmd_ptr)->client_id;
  (*client_index_table_pptr)->session_id         = ((mmgsdi_request_header_type*)cmd_ptr)->session_id;
  (*client_index_table_pptr)->request_type       = ((mmgsdi_request_header_type*)cmd_ptr)->request_type;
  (*client_index_table_pptr)->orig_request_type  = ((mmgsdi_request_header_type*)cmd_ptr)->orig_request_type;
  (*client_index_table_pptr)->response_cb        = ((mmgsdi_request_header_type*)cmd_ptr)->response_cb;
  (*client_index_table_pptr)->slot_id            = ((mmgsdi_request_header_type*)cmd_ptr)->slot_id;
  (*client_index_table_pptr)->task_id            = mmgsdi_util_get_current_task();
  (*client_index_table_pptr)->uim_rsp_status     = MMGSDI_UIM_RSP_INBOUND;

  if(out_extra_param_pptr != NULL)
  {
    *out_extra_param_pptr = (*client_index_table_pptr)->extra_param_ptr;
  }

  return MMGSDI_SUCCESS;
}/* mmgsdi_util_fill_and_alloc_request_table */
