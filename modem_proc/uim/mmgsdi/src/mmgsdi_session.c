...
Copyright (c) 2004 - 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_session.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
...
12/14/18   vgd     Reduce F3 log prints
...
01/19/17   vr      Fix compilation warnings
09/13/16   sn      Support for CDMALess Device
06/07/16   ar      Get client_id and session_id indices when failing session open
...
04/15/16   av      Clean up the index when rejecting session open request
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_populate_client_request_table_info(
                    index, &request_header, extra_param_ptr);
  if (mmgsdi_status != MMGSDI_SUCCESS)
  {
  ...
  const mmgsdi_uim_report_rsp_type *uim_rsp_ptr)
{
      ...
      uim_rsp_ptr->rsp_data.data_len > offset)
  {
    ...
    {
      if (extra_param_ptr != NULL &&
          (extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_CONTEXT ||
           extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_HTTP_DIGEST_CONTEXT))
      {
        uint8 max_resp_elements = 0;

        if (extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_CONTEXT)
        {
      ...
      }/* if extra_param_ptr->isim_auth.auth_context ==
          MMGSDI_AUTN_ISIM_CONTEXT || MMGSDI_AUTN_ISIM_HTTP_DIGEST_CONTEXT */
      else if (extra_param_ptr != NULL &&
               (extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_GBA_BOOTSTRAPPING_CONTEXT ||
                extra_param_ptr->isim_auth.auth_context == MMGSDI_AUTN_ISIM_GBA_NAF_DERIVATION_CONTEXT))
      {
        ...
        else
        {
          ...
          MMGSDI_AUTN_ISIM_GBA_BOOTSTRAPPING_CONTEXT || MMGSDI_AUTN_ISIM_GBA_NAF_DERIVATION_CONTEXT */
...
)
{
  ...
  /*---------------------------------------------------------------------------
    ...
    3. slot_id         - Slot that the request is to be performed on
    4. client_data     - Pointer to Client Data
    ...
    if (report_status == MMGSDI_SUCCESS)
    {
      ...
      if (mmgsdi_status == MMGSDI_SUCCESS)
      {
        ...
        return mmgsdi_cnf_build_and_queue(mmgsdi_status,
             ...
             req_header.orig_request_type != MMGSDI_SESSION_SELECT_AID_REQ)
    {
      /* SELECT AID failed becase AID not found. Lets check if this AID is ARA-M
         AID then we set our global which can then be used at a later time to 
         ensure a SELECT AID is never attempted again on this AID.
      ...
      if (MMGSDI_SUCCESS == mmgsdi_util_is_aram_aid(&select_aid_cnf_info.app_info, &is_aram_aid) &&
          is_aram_aid)
      {
        MMGSDIUTIL_ENTER_CLIENT_APP_DATA_CRIT_SECT;

        slot_data_ptr = mmgsdi_util_get_slot_data_ptr(uim_rsp_ptr->request_header.slot_id);

        if (slot_data_ptr)
        {
          slot_data_ptr->aram_aid_select_failed = TRUE;
        }
...
/* ----------------------------------------------------------------------------
   ...
   PARAMETERS:
     @param[in] msg_ptr                   msg_ptr conatins request.
     @param[out] caller_session_info_ptr  if it is not null thats means it is
                                          sync call and no need to build cnf.
     @param[in] num_sessions_open         number of session open.
...
mmgsdi_return_enum_type mmgsdi_session_proc_open_ext(
  ...
  mmgsdi_session_open_info_type    *caller_session_info_ptr,
  uint8                            *num_sessions_open
)
{
  mmgsdi_return_enum_type                mmgsdi_status           = MMGSDI_SUCCESS;
  ...
  if(caller_session_info_ptr == NULL)
  {
    MMGSDIUTIL_RETURN_IF_NULL_2(msg_ptr,
                                msg_ptr->request_header.response_cb);
  }
  ...
  { 
    MMGSDIUTIL_RETURN_IF_NULL(msg_ptr);
    /* no need to allocate memory we as aller already alocated the memory */
    session_info_ptr = caller_session_info_ptr;
  }
 ...
 /* -----------------------------------------------------------
  ...
  UIM_MSG_HIGH_1("session mask = 0x%x", session_mask);
  ...
  /* if cller has not allocated the memory then allocate 
     memory for session info for all requested sessions */
  if(session_info_ptr == NULL)
  {
    MMGSDIUTIL_MEM_MALLOC_AND_VALIDATE(session_info_ptr,
  ...
  while(num_sessions_requested)
  {
    ...
    switch(session_mask)
    {
      ...
      /* Unexpected here ! */
        ...
        mmgsdi_session_open_ext_cleanup(session_info_ptr ,
                                        num_sessions_opened);
        if(session_info_ptr != NULL && num_sessions_opened <= MMGSDI_SESSION_MAX_PROV_SESSIONS)
        {
          MMGSDIUTIL_TMC_MEM_FREE( session_info_ptr);
        }
        return MMGSDI_ERROR;
    ...
    do
    {
      ...
      (session_info_ptr[session_curr_index]).session_id =
        session_ptr->session_id;
      (session_info_ptr[session_curr_index]).session_type =
        session_ptr->session_type;

      num_sessions_opened++;
      num_session = num_sessions_opened;
    } while (0);
    ...
    if (mmgsdi_util_is_prov_session(session_ptr->session_type))
    {
        ...
        for(j=0; j<MMGSDI_MAX_SESSION_INFO; j++)
        {
            ...
            session_ptr->session_type))
          {
               ...
               (mmgsdi_channel_info_ptr_table[session_ptr->channel_info_index] != NULL))
            {
              ...
              (session_info_ptr[session_curr_index]).channel_id =
                mmgsdi_channel_info_ptr_table[session_ptr->channel_info_index]->channel_id;
              (session_info_ptr[session_curr_index]).slot_id =
      ...
      {
        mmgsdi_memscpy((void *)&session_info_ptr[session_curr_index].app_info,
          sizeof(session_info_ptr[session_curr_index].app_info),
    ...
    else
    {
      ...
      mmgsdi_session_open_ext_cleanup(session_info_ptr, num_sessions_opened);
      if(session_info_ptr != NULL && num_sessions_opened <= MMGSDI_SESSION_MAX_PROV_SESSIONS)
      {
        MMGSDIUTIL_TMC_MEM_FREE(session_info_ptr);
      }
  ...
  if(caller_session_info_ptr == NULL)
  {
    session_cnf_ext_info.session_info_ptr = session_info_ptr;
    session_cnf_ext_info.num_sessions = num_session;
  ...
  else if(num_sessions_open)
  {
    *num_sessions_open = num_session;
  }
  ...
  uint8                          num_sessions_opened)
{
     ...
     num_sessions_opened <= MMGSDI_SESSION_MAX_PROV_SESSIONS)
  {
    ...
    {
      session_id =
        session_info_ptr[curr_session_index].session_id;
  ...
  {
    UIM_MSG_ERR_2("Num of opened sessions 0x%x session info ptr 0x%x",
                  num_sessions_opened,
                  session_info_ptr);
  }
...
)
{
  ...
  else if (mmgsdi_util_is_non_prov_session(session_ptr->session_type))
  {
    ...
    /* Check if this non-zero AID is ARA-M AID and a SELECT on this AID failed
       earlier. If so, we should just return failure here and not re-attempt 
       the MANAGE CHANNEL followed by a SELECT AID */
    if (slot_data_ptr &&
        aid.aid.data_len > 0)
    {
      boolean                                is_aram_aid = FALSE;

      if (MMGSDI_SUCCESS == mmgsdi_util_is_aram_aid(&aid, &is_aram_aid) &&
          is_aram_aid &&
          slot_data_ptr->aram_aid_select_failed)
      {
        ...
        status_word.sw1 = SW1_WRONG_PARAMS;
        status_word.sw2 = SW2_FILE_NOT_FOUND;
  ...
  else if (mmgsdi_util_is_non_prov_session(session_info_ptr->session_type))
  {
    ...
    else if(channel_info_ptr->num_connections == 1)
    {
      ...
      if(msg_ptr->non_prov_terminate)
      {
        ...
        mmgsdi_status = mmgsdi_util_get_client_request_table_free_index(&client_table_index);
        if (mmgsdi_status != MMGSDI_SUCCESS)
        {
          MMGSDIUTIL_TMC_MEM_FREE_NULL_OK(extra_param_ptr);
          return mmgsdi_status;
        ...
        if (mmgsdi_status != MMGSDI_SUCCESS)
        {
          mmgsdi_util_free_client_request_table_index(client_table_index);
          MMGSDIUTIL_TMC_MEM_FREE_NULL_OK(extra_param_ptr);
          return mmgsdi_status;
        }
        ...
        mmgsdi_status = mmgsdi_session_build_deactivate(
                          ...
                          msg_ptr->request_header.orig_request_type,
                          mmgsdi_session_internal_deact_cb,
                          (mmgsdi_client_data_type)client_table_index);
      }
      else
      {
      ...
      mmgsdi_status = mmgsdi_util_queue_mmgsdi_uim_report_rsp(
                      client_table_index, MMGSDI_SESSION_CLOSE_REQ, MMGSDI_SUCCESS);
...
)
{
  ...
  switch (cmd)
  {
    ...
    case MMGSDI_SESSION_OPEN_EXT_REQ:
      mmgsdi_status = mmgsdi_session_proc_open_ext(&cmd_ptr->cmd.session_open_ext_req, NULL, NULL);
      break;
