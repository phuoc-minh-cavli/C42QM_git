...
Copyright (c) 2012 - 2018, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_task_init.c#2 $$ $DateTime: 2023/06/21 11:32:16 $
...
03/08/16   vr      Void obd_init API call's return value
03/07/16   kv      UIM PSM handling updates
...
)
{
  ...
  /* Check EFS item is_test_mode_logging_enabled for the whole device */
  if(UIM_COMMON_EFS_SUCCESS != uim_common_efs_read(UIM_COMMON_EFS_MMGSDI_TEST_MODE_LOGGING_ENABLED,
                                                   UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                                   UIM_COMMON_EFS_DEVICE,
                                                   &mmgsdi_is_test_mode_logging_enabled.always_enable,
                                                   sizeof(mmgsdi_is_test_mode_logging_enabled.always_enable)))
  {
    mmgsdi_is_test_mode_logging_enabled.always_enable = FALSE;
  }
  ...
  ** ----------------------------------------------------------------------*/
  ...
  for(i = 0; i < MMGSDI_MAX_NUM_SLOTS && i < mmgsdi_generic_data_ptr->num_slots; i++)
  {
    ...
    if (task_index > MMGSDI_TASK_MAIN && task_index < MMGSDI_NUM_TASKS)
    {
      ...
      mmgsdi_onchip_init_auto_activation_data(i);
