...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_efs.c#1 $$ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/16   bcho    file size should not be greater than input buffer size
...
)
{
  ...
  {
    UIM_MSG_ERR_0("NULL Returned Data Len Pointer");
    return MMGSDI_ERROR;
  }
  ...
  if(mcfg_fs_stat((const char *)file_handle_ptr, &file_stat,
         ...
         == MCFG_FS_STATUS_OK)
  {
    *returned_data_len_ptr = file_stat.st_size;
...
)
{
  ...
  /* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------*/
  if (file_handle_ptr       == NULL ||
      data_buffer_len_ptr   == NULL ||
      data_buffer_ptr       == NULL)
  {
    UIM_MSG_ERR_0("Input parameter is NULL");
    return MMGSDI_ERROR;
  }
  ...
  if(mcfg_fs_stat((const char *)file_handle_ptr, &file_stat,
         ...
         != MCFG_FS_STATUS_OK)
  {
    UIM_MSG_ERR_1("efs stat procedure failed: %x",
                  file_stat.st_size);
    return MMGSDI_ERROR;
  }

  if(*data_buffer_len_ptr < file_stat.st_size)
  {
    UIM_MSG_ERR_2("file size %x in greater than input buffer size %x",
                  file_stat.st_size, *data_buffer_len_ptr);
    return MMGSDI_ERROR;
  }
  ...
  {
    UIM_MSG_ERR_0("efs open Procedure Failed ");
    return MMGSDI_ERROR;
  }
  ...
  /* ------------------------------------------------------------------------
     ...
     ----------------------------------------------------------------------*/
  if(mcfg_fread(file_descriptor, (void *)data_buffer_ptr, file_size, MCFG_FS_TYPE_EFS) != (int)file_size)
  {
    UIM_MSG_ERR_0("efs read procedure failed ");
    return MMGSDI_ERROR;
  }

  /* ------------------------------------------------------------------------
     Data has been successfully copied in data_buffer_ptr. Need to assign the
     data_buffer_len_ptr; it is used as input-ouput param. Also close this file.
     ----------------------------------------------------------------------*/
  *data_buffer_len_ptr = file_size;
...
)
{
  ...
  if ( file_handle_ptr == NULL )
  {
    UIM_MSG_ERR_0 ("NULL Handle Provided");
    return MMGSDI_ERROR;
  }
  /* --------------------------------------------------------------------------
  ...
  {
    UIM_MSG_ERR_0("efs stat procedure failed ");
    return MMGSDI_ERROR;
  }
} /* gsdi_efs_name_test*/
...
)
{
  ...
  {
    UIM_MSG_ERR_0("efs mkdir procedure failed");
    return MMGSDI_ERROR;
  }
...
)
{
  ...
  /* --------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------*/
  if(mcfg_fwrite(file_descriptor, (void *)data_buffer_ptr, bytes_to_write, MCFG_FS_TYPE_EFS) != (int)bytes_to_write)
  {
    UIM_MSG_ERR_0("efs write failed ");
...
)
{
  ...
  if(mcfg_fs_delete(file_handle_ptr, MCFG_FS_TYPE_EFS,
         ...
         != MCFG_FS_STATUS_OK)
  {
    UIM_MSG_ERR_0("Failure to remove file");
    return MMGSDI_ERROR;
  }
