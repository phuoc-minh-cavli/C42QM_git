...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_file.c#1 $
...
06/21/16   ar      Featurizing GSM related tables
...
const mmgsdi_file_enum_table_type mmgsdi_file_gsm_enum_table[] =
{
  ...
  {MMGSDI_GSM_EXT6,                         0x6FC8, MMGSDI_NONE /* should it be MBDN */},
  {MMGSDI_GSM_MBI,                          0x6FC9, MMGSDI_NONE},
...
const mmgsdi_file_enum_table_type mmgsdi_file_usim_enum_table[] =
{
  ...
  /* 60 */
  {MMGSDI_USIM_EXT6,                        0x6FC8, MMGSDI_NONE},
...
/* ============================================================================
  FUNCTION MMGSDI_FILE_ACCESS_ALLOWED_FOR_MMSS_EF

  DESCRIPTION
    This function checks if mmss related files can be accessed.
    3GPP Spec 31.102 sec 4.6 indicates MMSS EFs are not applicable to 3GPP only ME.

  DEPENDENCIES
    None

  LIMITATIONS
    Valid only for MMSS EFs

  RETURN VALUE
    boolean
    TRUE:          Access not allowed.
    FALSE:        Access allowed.
    
  SIDE EFFECTS
    None
============================================================================*/
boolean mmgsdi_file_access_allowed_for_mmss_ef(
  mmgsdi_session_id_type session_id
)
{
  ...
  do
  {
    ...
    /* - Get RTRE configuration 
       - Allow access if RTRE is not defined - by this time RTRE cache should be available */
    nv_result = mmgsdi_nv_get_item(NV_RTRE_CONFIG_I, &mmgsdi_nv_data, MMGSDI_NV_CONTEXT_DEVICE_SPECIFIC);
    if(nv_result != NV_DONE_S)
    {
      break;
    }

    /* Do not block access if there is problem accessing slot info */
    ...
    /* If RTRE is RUIM and there is no CDMA app on card - do not allow MMSS EFs */
    if (mmgsdi_nv_data.item_value.rtre_config == NV_RTRE_CONFIG_RUIM_ONLY &&
        (slot_data_ptr->card_apps_available & MMGSDI_RUIM_APP_MASK) == 0  &&
        (slot_data_ptr->card_apps_available & MMGSDI_CSIM_APP_MASK) == 0)
    {
      UIM_MSG_ERR_0("MMSS EF access blocked");
      allow_access = FALSE;
    }
  }while (0);
...
#endif
  return allow_access;
} /* mmgsdi_file_access_allowed_for_mmss_ef */
