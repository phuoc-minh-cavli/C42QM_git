...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_icc.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
...
02/17/16   bcho    Make function static which has scope in this file
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_SESSION_ENABLE_SERVICE_REQ,
                                                            (void*)req_ptr,
                                                            &extra_param_ptr,
                                                            &index);
  ...
  if(req_ptr->service == MMGSDI_GSM_SRV_BDN)
  {
    ...
    set_file_status_req.request_header.client_data  = index;
...
)
{
  ...
  mmgsdi_status = mmgsdi_util_fill_and_alloc_request_table(MMGSDI_GET_ALL_PIN_STATUS_REQ,
                                                           (void*)req_ptr,
                                                           NULL,
                                                           &index);
  if (mmgsdi_status != MMGSDI_SUCCESS)
...
)
{
  ...
  /* Update FDN service */
  mmgsdi_status = mmgsdi_icc_update_fdn_status(session_id, slot);
  if ( mmgsdi_status != MMGSDI_SUCCESS )
  {
...
/*===========================================================================
  ...
  SIDE EFFECTS:
     None
===========================================================================*/
static mmgsdi_return_enum_type mmgsdi_icc_onchip_run_gsm_algo (
...
)
{
  ...
  if (slot_data_ptr->onchip_sim_data.state == MMGSDI_ONCHIP_SIM_INIT)
  {
    ...
    mmgsdi_status = mmgsdi_onchip_icc_populate_run_gsm_data(
