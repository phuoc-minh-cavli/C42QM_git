...
Copyright (c) 2009 - 2017, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/mmgsdi/src/mmgsdi_evt.c#1 $ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $
...
10/17/16   sn      send card error if prev card error was due to SAP connect
  ...
  mmgsdi_card_err_info_enum_type prev_err_cause)
{
 ...
 /* --------------------------------------------------------------------------
    ...
    (4)Send CARD_ERROR event when previous card error was due to SAP connect 
    ------------------------------------------------------------------------*/
  if(notify_all)
  {
    ...
    if(((prev_err_cause != MMGSDI_CARD_ERR_PWR_DN_CMD_NOTIFY) ||
       ...
       (prev_err_cause != MMGSDI_CARD_ERR_SAP_CONNECTED))
    {
      UIM_MSG_HIGH_3("MMGSDI_CARD_ERROR_EVT already sent out for slot: 0x%x, cause: 0x%x, prev cause: 0x%x",
                     slot, slot_data_ptr->card_error_condition, prev_err_cause);
      return;
    }
  }
  ...
  mmgsdi_client_id_type            client_id)
{
  ...
  mmgsdi_evt_free_data(evt_info_ptr);
  MMGSDIUTIL_TMC_MEM_FREE(evt_info_ptr);
#else /* FEATURE_SIMLOCK */
  (void)notify_all;
  (void)temp_unlock_info;
  (void)client_id;
