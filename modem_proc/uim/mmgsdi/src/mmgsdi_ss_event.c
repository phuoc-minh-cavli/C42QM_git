...
Copyright (c) 2014 - 2016, 2018, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
12/14/18   vgd     EONS selective logging
...
10/06/16   vdc     Resolve KW issue in accessing stack info 
...
)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_EONS, "Queue of MMGSDI command: MMGSDI_EONS_SS_EVENT_REQ status 0x%x",
                 mmgsdi_status);
...
)
{
  ...
  if(MMGSDI_3GPP_SYS_MODE(mm_ss_info_ptr->sys_mode))
  {
    session_type   = MMGSDI_GW_PROV_PRI_SESSION;
  }
  else if(MMGSDI_3GPP2_SYS_MODE(mm_ss_info_ptr->sys_mode))
  {
    session_type   = MMGSDI_1X_PROV_PRI_SESSION;
  }
  srv_status       = mm_ss_info_ptr->srv_status;
...
)
{
  ...
  if ((nitz_info_ptr == NULL) || (mm_ss_info_ptr == NULL) || (sys_mode_ptr == NULL))
  {
    return;
  }
  ...
  */
  ...
  mmgsdi_eons_copy_ss_event_mm_info(&(mm_ss_info_ptr->mm_information),
                                    nitz_info_ptr);

  if((mm_ss_info_ptr->changed_fields & CM_SS_EVT_MOBILITY_MGMT_MASK) &&
     mm_ss_info_ptr->mm_information.plmn_avail &&
     MMGSDI_EONS_PLMN_ID_VALIDITY_CHECK(mm_ss_info_ptr->mm_information.plmn.identity))
  {
    mmgsdi_memscpy(&sys_id_ptr->id.plmn,
                   sizeof(sys_plmn_id_s_type),
                   &mm_ss_info_ptr->mm_information.plmn,
                   sizeof(sys_plmn_id_s_type));
    sys_id_ptr->id.plmn_lac.lac = mm_ss_info_ptr->mm_information.lac_id;
    mmgsdi_rat = mmgsdi_eons_map_cm_rat_to_mmgsdi_rat(mm_ss_info_ptr->mm_information.rat);
  ...
  else
  {
    mmgsdi_memscpy(sys_id_ptr,
                   sizeof(sys_sys_id_s_type),
                   &mm_ss_info_ptr->sys_id,
                   sizeof(sys_sys_id_s_type));
    *sys_mode_ptr = mm_ss_info_ptr->sys_mode;
  }
  ...
  /* CSG is not supported on Chiltepin modem PLs, hence setting id to invalid */
...
)
{
  ...
  if (mm_ss_info_ptr->sys_mode == SYS_SYS_MODE_LTE)
  {
    UIM_MSG_HIGH_5("CM SS Event, PLMN: 0x%x, Service Status: 0x%x Sys Mode: 0x%x Changed fields: 0x%x, TAC: 0x%x",
  ...
  {
    UIM_MSG_HIGH_6("CM SS Event, PLMN: 0x%x, Service Status: 0x%x Sys Mode: 0x%x Changed fields: 0x%x, LAC: 0x%x Sys id type: 0x%x",
                   mm_ss_info_ptr->sys_id.id.plmn.identity[0] * 0x10000 +
                   mm_ss_info_ptr->sys_id.id.plmn.identity[1] * 0x100 +
                   mm_ss_info_ptr->sys_id.id.plmn.identity[2],
                   mm_ss_info_ptr->srv_status,
                   mm_ss_info_ptr->sys_mode,
                   mm_ss_info_ptr->changed_fields,
                   mm_ss_info_ptr->sys_id.id.plmn_lac.lac,
                   mm_ss_info_ptr->sys_id.id_type);
  }
  ...
  if(MMGSDI_SS_INFO_CHANGED(mm_ss_info_ptr->changed_fields) &&
     MMGSDI_SRV_STATUS_CHANGED(mm_ss_info_ptr->srv_status))
  {
    if(MMGSDI_3GPP_SYS_MODE(mm_ss_info_ptr->sys_mode) &&
       MMGSDI_3GPP_SS_INFO_CHANGED(mm_ss_info_ptr->changed_fields))
    {
      ...
      ss_info.first_stack_ss.srv_status   = mm_ss_info_ptr->srv_status;
      ss_info.first_stack_ss.roam_status  = mm_ss_info_ptr->roam_status;
      ...
      mmgsdi_eons_copy_ss_event_data(
         ...
         mm_ss_info_ptr);
            ...
            MMGSDI_3GPP2_SS_INFO_CHANGED(mm_ss_info_ptr->changed_fields) &&
            (mm_ss_info_ptr->srv_status == SYS_SRV_STATUS_SRV))
    {
      ...
      ss_info.first_stack_ss.is_valid     = TRUE;
      ss_info.first_stack_ss.srv_status   = mm_ss_info_ptr->srv_status;
      ss_info.first_stack_ss.sys_mode     = mm_ss_info_ptr->sys_mode;
      ss_info.first_stack_ss.roam_status  = mm_ss_info_ptr->roam_status;
      ss_info_changed                     = TRUE;
    }
  }
  ...
  {
    UIM_MSG_HIGH_2("Changed: Primary sub First stack GW SS info 0x%x 1X SS info 0x%x",
...
)
{
  ...
  {
    mmgsdi_imsi_switch_cm_ss_evt_cb(ss_event, mm_ss_info_ptr);
  }
...
)
{
  ...
  UIM_SELECTIVE_MSG_HIGH_1(UIM_LOG_EONS, "MMGSDI PROCESS SS EVENT RESPONSE, status: 0x%x",
  ...
  UIM_SELECTIVE_MSG_HIGH_4(UIM_LOG_EONS, "App Info ptr 0x%x Session type 0x%x first stack valid 0x%x second stack valid 0x%x",
