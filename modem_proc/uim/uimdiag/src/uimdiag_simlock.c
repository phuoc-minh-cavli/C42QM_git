...
$Header: //components/rel/uim.mpss/3.3.1/uimdiag/src/uimdiag_simlock.c#1 $$ $DateTime: 2020/07/22 10:53:27 $

when       who   what, where, why
--------   ---   -----------------------------------------------------------
04/17/17   tq    Decrement uimdiag command length
05/25/16   vdc   Remove F3 messages for memory allocation failure
03/07/16   bcho  Support for whitelist and blacklist codes in same category
  ...
  uint32                                    diag_req_len)
{
  ...
  if(category > UIMDIAG_SIMLOCK_CATEGORY_3GPP_CP_EXT)
  {
  ...
  for (i = 0; i < SIMLOCK_SLOT_COUNT_MAX; i++)
  {
    for (j = 0; j < SIMLOCK_CATEGORY_COUNT_MAX; j++)
    {
      uimdiag_get_status_ptr->lock_info[i].category_info[j].category =
        simlock_get_status_ptr->lock_info[i].category_info[j].category;
...
)
{
  ...
  return num_of_codes;
} /* uimdiag_get_number_of_codes_from_resp*/
...
)
{
  ...
  if(uimdiag_simlock_get_category_from_resp(simlock_get_category_data_ptr,
                                            &uimdiag_get_category_data_ptr->category_type) != UIMDIAG_SUCCESS)
  {
    UIMDIAG_MSG_ERROR_0("uimdiag_simlock_get_category_from_resp failed!");
    return UIMDIAG_ERROR;
  }

  switch (uimdiag_get_category_data_ptr->category_type)
  {
    case SIMLOCK_CATEGORY_3GPP_NW:
    ...
    case SIMLOCK_CATEGORY_3GPP2_CP:
    default:
      UIMDIAG_MSG_ERROR_1("Invalid category type: 0x%x", uimdiag_get_category_data_ptr->category_type);
      return UIMDIAG_ERROR;
  }
...
)
{
  ...
  uimdiag_get_category_data_ext_ptr->category_type = (uint8)category;
