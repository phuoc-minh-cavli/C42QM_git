...
Copyright (c) 2003-2018, 2021 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_provide_local_info.c#2 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
10/12/18   ks      Split ME and slot specific NV
04/16/18   gs      Provide right date and time when same is not retrieved from NITZ
11/15/17   vr      PLI Date and Time from NITZ/CM SS event for Data card
01/19/17   vr      Fixed compiler warning
06/17/16   gs      Fix the PLI TR
...
extern gstk_status_enum_type gstk_populate_tr_additional_res(gstk_additional_info_ptr_type *adtnl_inf_ptr, 
...
/* Constant used to determine if current year
   is atleast greater than a minimum */
#define GSTK_VALID_MIN_YEAR 2000
    ...
    */
    ...
    case GSTK_DATE_TIME_ZONE_INFO:
      ...
      */
      ...
      /* Populate the date and time retrieved from the core module
         this make sure that available information being sent to UICC */
      ...
      /* If the device is Data card, use NITZ time if available 
         and user did not set the time */
      if(rtc_julian.year < GSTK_VALID_MIN_YEAR &&
         (gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DATA_CARD) == TRUE))
      {
        for (i = 0; i < GSTK_CM_NUM_STACKS; ++i)
        {
          if ((gstk_curr_inst_ptr->gstk_next_curr_rat ==
               gstk_curr_inst_ptr->tz_info[i].access_tech) &&
              gstk_curr_inst_ptr->tz_info[i].time_data_avail)
          {
            local_info_ptr->info_data.time.day = ((uint8)gstk_curr_inst_ptr->tz_info[i].day/10);
            local_info_ptr->info_data.time.day += (((uint8)gstk_curr_inst_ptr->tz_info[i].day%10) << 4);
            local_info_ptr->info_data.time.hour = ((uint8)gstk_curr_inst_ptr->tz_info[i].hour/10);
            local_info_ptr->info_data.time.hour += (((uint8)gstk_curr_inst_ptr->tz_info[i].hour%10) << 4);
            local_info_ptr->info_data.time.minute = ((uint8)gstk_curr_inst_ptr->tz_info[i].minute/10);
            local_info_ptr->info_data.time.minute += (((uint8)gstk_curr_inst_ptr->tz_info[i].minute%10) << 4);
            local_info_ptr->info_data.time.month = ((uint8)gstk_curr_inst_ptr->tz_info[i].month/10);
            local_info_ptr->info_data.time.month += (((uint8)gstk_curr_inst_ptr->tz_info[i].month%10) << 4);
            local_info_ptr->info_data.time.second = ((uint8)gstk_curr_inst_ptr->tz_info[i].second/10);
            local_info_ptr->info_data.time.second += (((uint8)gstk_curr_inst_ptr->tz_info[i].second%10) << 4);
            local_info_ptr->info_data.time.year = ((uint8)gstk_curr_inst_ptr->tz_info[i].year/10);
            local_info_ptr->info_data.time.year += (((uint8)gstk_curr_inst_ptr->tz_info[i].year%10) << 4);
            break;
          }
        }
      }
...
)
{
  ...
  gstk_populate_tr_additional_res(
			  &(task_cmd_ptr->cmd.provide_local_info_term_rsp_cnf.result_additional_info), 
  ...
  else
  {
    ...
    case GSTK_CSG_CELL_INFO:
      ...
      (void)gstk_memcpy(&(task_cmd_ptr->cmd.provide_local_info_term_rsp_cnf.local_info_rsp.info_data.csg_info),
