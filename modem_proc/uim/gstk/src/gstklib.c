...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstklib.c#1 $ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/05/19   gs      Compiler warning fixes
...
05/18/17   vr      Handle invalid refresh command with no alpha
...
06/28/16   gs      Control the IoE related functionlity
...
)
{
      ...
      if (!gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_PCMD_SELECTIVE_PARALLEL_PROCESSING))
      {
        ...
        switch(data[cmd_bit])
        {
          ...
          case GSTK_CMD_STK_REFRESH:
            ...
            {
              if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_ESTK_QMI_SEND_NO_AND_NULL_ALPHA_IND) == TRUE)
              {
  ...
  if(gstk_status != GSTK_SUCCESS)
  {
    ...
    if(gstk_shared_data.ui_nw_proactive_session_slot_in_progress == (gstk_slot_id_enum_type)slot)
    {
...
)
{
  ...
  /* Wait until CM, DS task is ready because we initialize ESTK (a GSTK client)
     only after CM, DS is ready. QMI is the other GSTK client we wait for to come
  ...
  {
    UIM_MSG_HIGH_1("All GSTK clients are not up yet! ready tasks:0x%x fetch command and process when tasks ready",
                                                                                      gstk_shared_data.ready_tasks);
  }
...
)
{
...
#ifdef FEATURE_CDMA    
  gstk_access_technology_type                             gstk_curr_rat        = GSTK_ACCESS_NONE;
#endif  
  ...
  case GSTK_IO_GET_CURR_ACCESS_TECH:
    ...
    UIM_MSG_HIGH_3("The current rat of slot 0x%x is %d, next xurrent rat is %d",
                   slot,
                   gstk_instances_ptr[(uint8)slot-1]->gstk_curr_rat,
                   gstk_instances_ptr[(uint8)slot - 1]->gstk_next_curr_rat);
#ifdef FEATURE_CDMA  
    if(gstk_instances_ptr[(uint8)slot - 1]->gstk_next_curr_rat == GSTK_ACCESS_TECH_LTE)
    {
      if(gstk_instances_ptr[(uint8)slot - 1]->mode_change_cmd_ptr != NULL)
      {
        if((gstk_instances_ptr[(uint8)slot - 1]->mode_change_cmd_ptr->mode_change_ind.cm_stack[0].loc_status
            == GSTK_NORMAL_SERVICE) &&
           (gstk_instances_ptr[(uint8)slot - 1]->mode_change_cmd_ptr->mode_change_ind.cm_stack[0].access_tech
            == GSTK_ACCESS_TECH_CDMA))
        {
          UIM_MSG_HIGH_0("LTE and CDMA are in service - send curr rat as CDMA");
          gstk_curr_rat = GSTK_ACCESS_TECH_CDMA;
          (void)gstk_memcpy(param_out_ptr,
                            &gstk_curr_rat,
                            len,
                            len,
  ...
  case GSTK_IO_GET_DS_SYSTEM_RAT_INFO:
    ...
    GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_instances_ptr[(uint8)slot - 1], GSTK_ERROR);
    *(gstk_ds_system_status_type**)param_out_ptr = &gstk_instances_ptr[(uint8)slot-1]->ds_sys_status_info;
    break;
...
PARAMETERS
  ...
  exit_from_psm_status
               : gives info if system\ME have exited from PSM mode or not.
...
)
{
  ...
  if(task_cmd != NULL)
  {
    ...
    task_cmd->cmd.mmgsdi_evt.evt.terminal_profile.exit_from_psm_status
                                                          = exit_from_psm_status;
