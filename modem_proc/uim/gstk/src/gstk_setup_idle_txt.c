...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_setup_idle_txt.c#1 $
...
{
  if (message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.text != NULL) {
    gstk_free(message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.text);
    message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.text_string.text = NULL;
  }
  if ((message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.icon.present) &&
    (message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.icon.data != NULL)) {
    gstk_free(message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.icon.data);
    message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req.icon.data = NULL;
  }
  memset(&(message_ptr->cmd.setup_idle_mode_txt_pro_cmd_req),
  	   0x00,
