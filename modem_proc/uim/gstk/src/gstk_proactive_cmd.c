...
Copyright (c) 2003-2018, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_proactive_cmd.c#2 $
...
03/07/16   gs      Handling of DTMF and Activate commands
03/07/16   gs      Handling of Unsupported Proactive commands
  ...
  gstk_proactive_cmd_type *cmd_ptr )
{
  ...
  */
...
#endif /* FEATURE_CDMA */    
    {
      UIM_MSG_HIGH_0("FEATURE_CDMA is not enabled");
      gstk_status = gstk_open_ch_req(cmd_ptr);
    }
    break;
  ...
  if(gstk_status == GSTK_UNSUPPORTED_COMMAND)
  {
    gstk_status = gstk_unknown_unsupported_proactive_cmd_req(cmd_ptr,
                                GSTK_COMMAND_BEYOND_THE_ME_CAPABILITIES);
  }  
