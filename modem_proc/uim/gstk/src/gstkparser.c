...
Copyright (c) 2003-2016, 2019, 2022 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstkparser.c#7 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/19/22   dt      Modify checks in parsing logic of APN
08/08/22   shp     Fix OOB issue while decoding ucs2 alpha value
...
03/07/22   dt      Fix security issue in alpha identifier parsing
    ...
    for (i = 0; i< tlv_length; i++) {
      switch(orig_cmd_data[count]) {
        ...
        case GSTK_IMS_REGISTRATION:
          ...
          break;
        case GSTK_POLL_INTERVAL:
          evt_list_mask[GSTK_POLL_INTERVAL] = 0x01;
          UIM_MSG_HIGH_0("Poll Interval");
          break; 
        default:
          if (tag_comprehension_req) {
            UIM_MSG_ERR_1("unknown event list, 0x%x", orig_cmd_data[count]);
...
)
{
    ...
    *more_tlvs = TRUE;
      ...
      if(orig_cmd_data[count+length_of_length_field-1] == 0) {
        ...
        if(orig_cmd_data[count+length_of_length_field-1] == 1) {
          ...
          icon_id_list->num_items = orig_cmd_data[count+length_of_length_field-1]
                                  - 1;
          if(icon_id_list->num_items >= GSTK_MAX_MENU_ITEMS)
          {
            icon_id_list->num_items = GSTK_MAX_MENU_ITEMS;
          }
        }
        ...
        count++;
        while ( ( loop_control < icon_id_list->num_items ) && (loop_control < GSTK_MAX_MENU_ITEMS) &&
                ( gstk_status == GSTK_SUCCESS ) )
        {
    ...
    if (gstk_status != GSTK_SUCCESS) {
        UIM_MSG_ERR_2("cmd_data_len=0x%x, final count=0x%x",
                      cmd_data_len, count);
        for ( loop_control = 0; loop_control < icon_id_list->num_items && loop_control < GSTK_MAX_MENU_ITEMS; loop_control++ ) {
          if (icon_id_list->icon_list[loop_control] != NULL) {
                                    ...
                                    uint16             *alphabet_out)
{
  ...
  if((in_len > 0) && (ucs2_default_in != NULL)) /*lint !e774 */
  {
    ...
    memset(alphabet_out, 0x00, sizeof(uint16)*int32touint32(in_len));

    for (i = 0; i < in_len-2; i++) { /* for loop to the second last uint16 */
                                    ...
                                    uint16             *alphabet_out)
{
  ...
  if((in_len > 0) && (ucs2_default_in != NULL)) /*lint !e774 */
  {
    ...
    for (i = 0; i < in_len - 1; i++) { /* for loop to the last byte in input array */
                       ...
                       const uint32                  *ref_id)
{
    ...
    *more_tlv = FALSE;
    ...
    if(orig_alpha_len >=  cmd_data_len)
    {
      return GSTK_INVALID_LENGTH;
    }
    /* position count to the url location in the tlv */
    ...
    if (orig_alpha_len > 0) { /* Card provides a url address */
      ...
      case 0x80:
        ...
        if (alpha->alphabet != NULL) {
          (void)gstk_decode_h80_usc2_alphabet(alpha->length, &orig_cmd_data[count], alpha->alphabet);
      ...
      case 0x81:
        ...
        alpha->alphabet = GSTK_CALLOC(sizeof(uint16) * int32touint32(alpha->length));
        if (alpha->alphabet != NULL) {
          (void)gstk_decode_h81_usc2_alphabet(alpha->length, &orig_cmd_data[count], alpha->alphabet);
      ...
      case 0x82:
        ...
        alpha->alphabet = GSTK_CALLOC(sizeof(uint16) * int32touint32(alpha->length));
        if (alpha->alphabet != NULL) {
          (void)gstk_decode_h82_usc2_alphabet(alpha->length, &orig_cmd_data[count], alpha->alphabet);
      ...
      default: /* GSM 7 bit default with bit 8 == 0 */
        ...
        if (alpha->length > 0) {
          alpha->alphabet = GSTK_CALLOC(sizeof(uint16) * int32touint32(alpha->length));
          if (alpha->alphabet != NULL) {
            (void)gstk_convert_gsm_default_to_gstk_alphabet(alpha->length, &orig_cmd_data[count], alpha->alphabet);
                       ...
                       const uint32                  *ref_id)
{
    ...
    */
    ...
    if(nw_access_name->length > (cmd_data_len - count))
    {
      return GSTK_INVALID_LENGTH;
    }
    ...
    */
    ...
    if(gstk_status != GSTK_SUCCESS || (nLen > (cmd_data_len - count)))
    {
      UIM_MSG_ERR_3("Null bytes ! or nLen %d is greater than command length %d, %d", nLen, cmd_data_len, count);
      nw_access_name->length = 0;
      nw_access_name->alphabet_given = FALSE;
      if(tag_comprehension_req)
      {
    ...
    nw_access_name->alphabet = (uint16*)GSTK_CALLOC((nLen)*sizeof(uint16));
    ...
    memset(nw_access_name->alphabet, 0x00, (nLen) * sizeof(uint16));
...
/*===========================================================================
FUNCTION gstk_parser_refresh_enforcement_policy_tlv

DESCRIPTION
    This function parses the refresh enforcement policy TLV
    It also returns the index slot for next TLV's tag if available. If there is
    no more TLVs to be parsed, the more tlv will be set to FALSE
...
PARAMETERS
  ...
  orig_offset:                     [Input] Offset in the original command data where the parsing
                                       should begin
  refresh_enforcement_policy_para: [Input/Output] Pointer to the refresh enforcement policy
  next_tlv_offset:                 [Input/Output] Pointer to the index offset for the next tlv
                                       if applicable.
  more_tlv:                        [Input/Output] Pointer to boolean to indicate whether there are more
                                       TLV to be parse after exiting this function
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_parser_refresh_enforcement_policy_tlv(
                       uint8                                *orig_cmd_data,
                       int32                                 cmd_data_len,
                       int32                                 orig_offset,
                       gstk_refresh_enforcement_policy_type *refresh_enforcement_policy_para,
                       int32                                *next_tlv_offset,
                       boolean                              *more_tlv,
                       boolean                               man_opt_specifier,
                       const uint32                         *ref_id)
{
    ...
    UIM_MSG_HIGH_0("** Parsing refresh enfocement policy parameter");
    ...
    *more_tlv = FALSE;
    ...
    if (refresh_enforcement_policy_para == NULL) {
        UIM_MSG_ERR_0("Null Input Param");
        return GSTK_MEMORY_ERROR;
    }
    ...
    /* Refresh enforcement policy TLV */
    tag_parsing_result = gstk_is_tag_valid(ref_id, orig_cmd_data[count], (uint8)GSTK_REFRESH_ENFORCEMENT_POLICY_TAG,
                                           man_opt_specifier, orig_cmd_data, &count, cmd_data_len);

    if (!gstk_continue_parsing(GSTK_REFRESH_ENFORCEMENT_POLICY_TAG, tag_parsing_result, man_opt_specifier, &gstk_status))
    {
      if (gstk_status == GSTK_SUCCESS) {
        /* Was optional tag, go through the parser_get_next_tlv_offset */
        gstk_status = gstk_parser_get_next_tlv_offset(cmd_data_len, count, next_tlv_offset, more_tlv);
    ...
    /* valid Refresh enforcement policy data Tag */
    count++;
    ...
    /* position count to the refresh enforcement policy data in the tlv */
    count = count + length_of_length_field;

    refresh_enforcement_policy_para->length = tlv_length;

    refresh_enforcement_policy_para->refresh_enforcement_policy = 
      GSTK_CALLOC(refresh_enforcement_policy_para->length);
      
    if(refresh_enforcement_policy_para->refresh_enforcement_policy != NULL)
    {
      /* copy the data */
      gstk_memcpy(refresh_enforcement_policy_para->refresh_enforcement_policy, 
                  &orig_cmd_data[count],
                  refresh_enforcement_policy_para->length,
                  refresh_enforcement_policy_para->length,
                  refresh_enforcement_policy_para->length);
    }
    
    count += refresh_enforcement_policy_para->length;

    /* next TLV offset */
    gstk_status = gstk_parser_get_next_tlv_offset(cmd_data_len, count, next_tlv_offset, more_tlv);

    return gstk_status;
} /* gstk_parser_refresh_enforcement_policy_tlv */