...
Copyright (c) 2003-2018, 2020, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_envelope_rsp.c#5 $
...
05/09/16   gm      Combine envelope retry processing in envelope response
...
void gstk_uim_envelope_report (uim_rpt_type *report_ptr)
{
  ...
  task_cmd_ptr->cmd.envelope_response.uim_original_sw1 = task_cmd_ptr->cmd.envelope_response.sw1;
...
)
{
  ...
  if (!GSTK_IS_VALID_SLOT_ID(task_cmd->cmd.envelope_response.message_header.sim_slot_id))
  {
	  return GSTK_ERROR;
  }

  if (gstk_shared_data.envelope_cmd_ref_table[env_ref_id].expected_cmd_rsp_type == GSTK_DATA_AVAIL_EVT_IND_RSP ||
      gstk_shared_data.envelope_cmd_ref_table[env_ref_id].expected_cmd_rsp_type == GSTK_CH_STATUS_EVT_IND_RSP  ||
      gstk_shared_data.envelope_cmd_ref_table[env_ref_id].expected_cmd_rsp_type == GSTK_CC_IND_RSP)
  {
    gstk_instances_ptr[task_cmd->cmd.envelope_response.message_header.sim_slot_id - 1]->gstk_bip_env_in_progress = FALSE;
  }
...
/*===========================================================================
FUNCTION gstk_envelope_rsp_process_retry

DESCRIPTION
  This is the common function used to process envelope retry envelope response

PARAMETERS
  type_of_command: [Input] Type of envelope command
  rsp_status     : [Input] Response status
...
SEE ALSO
  None
===========================================================================*/
static gstk_status_enum_type gstk_envelope_rsp_process_retry(
  ...
  const gstk_cmd_type * cmd)
{
  ...
  if(gstk_envelope_rsp_process_retry(cmd) == GSTK_SUCCESS)
  {
    return GSTK_SUCCESS;
  }
  ...
  if (cmd->envelope_response.rsp_status == GSTK_ENVELOPE_CMD_FAIL)
  {
               ...
               sizeof(gstk_curr_inst_ptr->attach_ctrl_p->mcc_mnc)) == 0)
    {
	  ...
	  {
	    ULOG_RT_PRINTF_2(gstk_get_ulog_handle(),"=GSTK= gstk_loc_info_envelope_rsp 0x%x, 0x%x",
...
/*===========================================================================
FUNCTION gstk_evt_dl_uim_polling_report
...
SEE ALSO
  None
===========================================================================*/
static void gstk_evt_dl_uim_polling_report (uim_rpt_type *report_ptr)
{
  (void)report_ptr;
} /* gstk_evt_dl_uim_polling_report */

/*===========================================================================
FUNCTION gstk_evt_dl_poll_interval_rsp

DESCRIPTION

  This function:
  - Processes the Poll Interval event download response
  - Sends Poll Interval Command to UIMDRV if needed
  - For decoded client, builds the event download rsp and sends it to client

PARAMETERS
  envelope_response_ptr: [Input] Pointer to the envelope rsp to be processed
  response_ptr         : [Input] Response to be sent to decoded client
...
SEE ALSO
  None
===========================================================================*/
static gstk_status_enum_type gstk_evt_dl_poll_interval_rsp(
  const gstk_envelope_resp_type * envelope_response_ptr,
  gstk_cmd_from_card_type       * response_ptr)
{
  gstk_duration_type                  poll_duration;
  gstk_general_envelope_rsp_enum_type general_resp       = GSTK_ENVELOPE_CMD_SUCCESS;
  gstk_control_result_enum_type       result             = GSTK_CONTROL_RESULT_MAX;
  boolean                             poll_cmd_to_uimdrv = FALSE;
  gstk_status_enum_type               gstk_status        = GSTK_SUCCESS;

  if(envelope_response_ptr == NULL)
  {
    return GSTK_ERROR;
  }

  memset(&poll_duration, 0x00, sizeof(poll_duration));
  general_resp = envelope_response_ptr->rsp_status;
  
  if (general_resp == GSTK_ENVELOPE_CMD_SUCCESS)
  {
    /* If there is no response data, Poll Interval request is allowed */
    result = GSTK_ALLOWED_NO_MOD;
    
    /* Response data present */
    if(envelope_response_ptr->length > 0)
    {
      /* Get the Poll Interval result */
      result = (gstk_control_result_enum_type)envelope_response_ptr->data[0];
    }
      
    switch(result)
    {
      case GSTK_ALLOWED_NO_MOD:
        if(envelope_response_ptr->length <= GSTK_GEN_RESULT_LEN)
        {
          /* Need to send Poll Interval Duration request to UIMDRV */
          poll_cmd_to_uimdrv = TRUE;
          /* Get the duration from the original request */
          poll_duration.units =
            gstk_shared_data.envelope_cmd_ref_table[envelope_response_ptr->message_header.user_data].poll_interval_evt_info.units;
          poll_duration.length = 
            gstk_shared_data.envelope_cmd_ref_table[envelope_response_ptr->message_header.user_data].poll_interval_evt_info.length;    
        }
        /* Per ETSI 102 223 , only Result TLV is expected with no modification */
        else
        {
          general_resp = GSTK_ENVELOPE_CMD_FAIL;
        }        
        break;
      case GSTK_NOT_ALLOWED:
        /* If Envelope response has Result and Duration */
        if(envelope_response_ptr->length ==
           (GSTK_GEN_RESULT_LEN + GSTK_TAG_LENGTH_LEN + GSTK_DURATION_LEN))
        {
          poll_duration.present = TRUE;
          poll_duration.units = envelope_response_ptr->data[3];
          poll_duration.length = envelope_response_ptr->data[4];
        }        
        break;
      case GSTK_ALLOWED_BUT_MOD:
        ...
        /* Per ERSI 102 223, poll Duration is madatory for modified response */
        else
        {
          general_resp = GSTK_ENVELOPE_CMD_FAIL;
        }
  ...
  /* Send updated Poll Duration to UIMDRV */
  if(poll_cmd_to_uimdrv)
  {
    uint32        poll_ms     = 0;
    ...
    if(uim_cmd_ptr != NULL)
    {
      uim_cmd_ptr->hdr.command                  = UIM_POLLING_INTERVAL_F;
      ...
      uim_cmd_ptr->hdr.rpt_function             = gstk_evt_dl_uim_polling_report;
  ...
  /* Send response to decoded clients */
  if(response_ptr != NULL)
  {
    response_ptr->hdr_cmd.command_id = GSTK_POLL_INTERVAL_EVT_IND_RSP;
    response_ptr->hdr_cmd.cmd_detail_reference = 0; /* doesn't matter */
    response_ptr->cmd.poll_interval_evt_rsp.general_resp = general_resp;
    response_ptr->cmd.poll_interval_evt_rsp.result = result;
    if(poll_duration.present)
    {
      response_ptr->cmd.poll_interval_evt_rsp.duration = poll_duration;
    }
    
    return gstk_util_send_response_to_clients(
             response_ptr,
             envelope_response_ptr->message_header.user_data);
  }
  return GSTK_SUCCESS;
} /* gstk_evt_dl_poll_interval_rsp */

/*===========================================================================
FUNCTION gstk_pco_info_envelope_rsp

DESCRIPTION

  This function handles the PCO envelope response
...
SEE ALSO
  None
===========================================================================*/
static gstk_status_enum_type gstk_pco_info_envelope_rsp(
  const gstk_cmd_type * cmd)
{
  uint32 env_ref_id = GSTK_MAX_ENVELOPE_COMMAND;
  ...
  UIM_MSG_HIGH_1("In gstk_pco_info_envelope_rsp, rsp_status 0x%x",
     ...
     (gstk_curr_inst_ptr->gstk_curr_envelope_info.gstk_env_state == GSTK_SUB_ST_PENDING_ENV_RSP_FROM_CARD))
  {
    ...
    {
      if(gstk_start_env_retry_timer(GSTK_PCO_INFO_ENVELOPE_RSP) != GSTK_SUCCESS)
      {
        gstk_util_clear_retry_envelope_data(
          gstk_util_get_sim_slot_id(gstk_curr_inst_ptr->client_id));
        memset(gstk_curr_inst_ptr->gstk_last_loc_info_sent, 0xFF, 3);
  ...
  /* Record that PCO envelope has been successfully sent to card */
  if(cmd->envelope_response.rsp_status == GSTK_ENVELOPE_CMD_SUCCESS)
  {
    gstk_curr_inst_ptr->pco_info.is_pco_resp_rcvd = TRUE;
  }

  return GSTK_SUCCESS;
} /* gstk_pco_info_envelope_rsp */
  ...
  const gstk_cmd_type * cmd_ptr)
{
  ...
  boolean                                     send_end_session = FALSE;
  ...
  if(evt_list == GSTK_POLL_INTERVAL)
  {
    (void)gstk_evt_dl_poll_interval_rsp(&cmd_ptr->envelope_response, NULL);   
  }
  ...
  if((gstk_shared_data.envelope_cmd_ref_table[cmd_ptr->envelope_response.message_header.user_data].is_menu_selection == TRUE) 
     && (cmd_ptr->envelope_response.uim_original_sw1 != SW1_END_FETCH))
  {
  ...
  /* If Menu Selection Envelope does not lead to a Proactive Command,
     simulate End Session, so that UI clears the screen */
  if(send_end_session == TRUE)
  {              
  ...
  const gstk_cmd_type * cmd)
{
    ...
    switch( type_of_command )
    {
      ...
      case GSTK_POLL_INTERVAL_EVT_IND_RSP:
        /* build the command */
        gstk_status = gstk_evt_dl_poll_interval_rsp(&cmd->envelope_response,
                                                    response_ptr);
        break;   
      default:
        /* do nothing */
    ...
    gstk_cmd_type* cmd_ptr )
{
    ...
    switch( type_of_command )
    {
      ...
      case GSTK_LOC_INFO_ENVELOPE_RSP:
        ...
        break;
      case GSTK_PCO_INFO_ENVELOPE_RSP:
        gstk_status = gstk_pco_info_envelope_rsp(cmd_ptr);
        break;  
      default:
  ...
  const gstk_envelope_sms_pp_download_ind_type * sms_pp_cmd)
{
   ...
   response.hdr_cmd.cmd_detail_reference = 0; /* doens't matter */
   if(sms_pp_cmd->message_header.user_data < GSTK_MAX_ENVELOPE_COMMAND) {
     response.hdr_cmd.user_data =
  ...
  const gstk_envelope_cc_ind_type * cc_cmd)
{
   ...
   if(cc_cmd->message_header.user_data < GSTK_MAX_ENVELOPE_COMMAND) {
     response.hdr_cmd.user_data =
  ...
  const gstk_envelope_mo_sms_ctrl_ind_type * mo_sms_ctrl_cmd)
{
   ...
   if(mo_sms_ctrl_cmd->message_header.user_data < GSTK_MAX_ENVELOPE_COMMAND) {
     response.hdr_cmd.user_data =
  ...
  const gstk_raw_envelope_ind_type * raw_env_ptr)
{
  ...
  if(raw_env_ptr->message_header.user_data < GSTK_MAX_ENVELOPE_COMMAND) {
    response.hdr_cmd.user_data =
