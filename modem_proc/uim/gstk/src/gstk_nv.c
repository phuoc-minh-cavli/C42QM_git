...
Copyright (c) 2014-2017, 2019, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_nv.c#2 $$ $DateTime: 2020/08/19 06:36:45 $

when       who      what, where, why
--------   ---    --------------------------------------------------------------------
07/17/20   dt      Added support for VZW PCO command
07/16/19   dt      Enable GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT for GCF
10/12/18   ks      Remove NV reads in GSTK NV task initialization
10/12/18   ks      Split ME and slot specific NV
08/31/18   dt     Change default CAT version to 12
08/31/18   dt      9x05 interface changes for memory reduction
04/23/18   dt      Deprecate GSTK_CFG_FEATURE_SLOT_ESTK_BIP_IP_DEFAULT
08/09/17   shr     Enabled GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT for GCF
...
05/12/16   vr      Make NV cache read print less lines
...
static void gstk_nv_access_post_process_feature_bmask_2(
 uint8 cache_index
);

static void gstk_nv_access_post_process_me_feature_bmask(void);
...
/*===========================================================================
FUNCTION gstk_nv_access_post_process_feature_bmask_2

DESCRIPTION
  This function parses the raw feature bitmask 2 read from NV and updates
  the NV cache
...
{
  gstk_nv_items_data_type read_data  = {0};

  /* Set default value */
  if(gstk_nv_cache.slot_items_ptr[cache_index]->feature_bmask_2.state == GSTK_NV_STATE_NOT_FOUND)
  {
    gstk_nv_cache.slot_items_ptr[cache_index]->feature_bmask_2.value = 0;
  }
  ...
  if(read_data.gcf_gstk == TRUE)
  {
    /* Enabled GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT by default
       if GCF */
    gstk_nv_cache.slot_items_ptr[cache_index]->feature_bmask_2.value |=
                        GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT;
  }
} /* gstk_nv_access_post_process_feature_bmask_2 */

/*===========================================================================
FUNCTION gstk_nv_access_post_process_me_feature_bmask

DESCRIPTION
  Function caches the ME specific sub-features of NV 73760
...
static void gstk_nv_access_post_process_me_feature_bmask(void)
{
  ...
  if(gstk_nv_cache.me_items.me_spec_feature_bmask.state != GSTK_NV_STATE_INIT)
  {
    gstk_nv_cache.me_items.me_spec_feature_bmask.value = 0;
  }

  if(!(gstk_nv_cache.me_items.me_spec_feature_bmask.value & GSTK_CFG_FEATURE_ME_REPORT_SW1_SW2))
  {
    ...
    {
      gstk_nv_cache.me_items.me_spec_feature_bmask.value |= GSTK_CFG_FEATURE_ME_REPORT_SW1_SW2;
    }
  }

  /* If QMI CAT is not supported or QMI CAT mode is disabled, enable DATA_CARD and ESTK_IGNORE_ALPHA config */
  if(qmi_cat_check_if_supported() == FALSE ||
  	 read_data.qmi_cat_mode == CATI_CONFIG_MODE_DISABLED)
  {
    /* Set Data Card bits */
    gstk_nv_cache.me_items.me_spec_feature_bmask.value |= GSTK_CFG_FEATURE_ME_DATA_CARD;
    gstk_nv_cache.me_items.me_spec_feature_bmask.value |= GSTK_CFG_FEATURE_ME_ESTK_IGNORE_ALPHA;
  }

  UIM_MSG_HIGH_2("GSTK NV me_spec_feature_bmask, value: 0x%x, state: 0x%x",
                 gstk_nv_cache.me_items.me_spec_feature_bmask.value,
                 gstk_nv_cache.me_items.me_spec_feature_bmask.state);
} /* gstk_nv_access_post_process_me_feature_bmask */
    ...
    case GSTK_NV_EXT_SUB_START:
         ...
         op_categ == GSTK_NV_OP_CATEG_INIT)
      {
        gstk_status = gstk_util_get_sub_id_from_slot(
                        slot_id,
                        (sys_modem_as_id_e_type*)&item_params_ptr->nv_index);
  ...
  switch (item_type)
  {
    ...
    case GSTK_NV_SLOT_CAT_VERSION:
      ...
      if(is_item_read &&
         *item_params_ptr->cache_state_ptr == GSTK_NV_STATE_NOT_FOUND)
      {
        *(uint8*)item_params_ptr->cache_data_ptr = GSTK_CFG_CAT_VER_LATEST;
      }
    ...
    case GSTK_NV_SLOT_FEATURE_BMASK_2:
      ...
      if(is_item_read)
      {
        gstk_nv_access_post_process_feature_bmask_2(item_params_ptr->cache_index);
      }
...
DESCRIPTION
  This function allocates memory for ME and slot specific NV items during
...
)
{
  ...
  switch (cmd_ptr->nv_refresh_cmd.slot_index)
  {
    ...
    case MCFG_REFRESH_INDEX_2:
      ...
      break;
    default:
      (void)mcfg_refresh_done(cmd_ptr->nv_refresh_cmd.reference_id);
      return GSTK_BAD_PARAM;
  }
  ...
  if(mcfg_refresh_done(cmd_ptr->nv_refresh_cmd.reference_id))
  {
    return GSTK_SUCCESS;
  }
  else
...
)
{
  ...
  if(gstk_nv_cache.slot_items_ptr[(uint8)(slot_id - 1)]->feature_bmask.state == GSTK_NV_STATE_NOT_INIT)
  {
    (void)gstk_nv_access_read(GSTK_NV_SLOT_FEATURE_BMASK,
                              slot_id,
                              &nv_data);
  }
...
)
{
  ...
  if(gstk_nv_cache.slot_items_ptr[(uint8)(slot_id - 1)]->feature_bmask_2.state == GSTK_NV_STATE_NOT_INIT)
  {
    (void)gstk_nv_access_read(GSTK_NV_SLOT_FEATURE_BMASK_2, 
                              slot_id,
                              &nv_data);
  }
  ...
  switch(mask_value)
  {
    ...
    case GSTK_CFG_FEATURE_2_SLOT_DISABLE_ENH_RAT_AND_LOC_EVT:
    case GSTK_CFG_FEATURE_2_SLOT_ENABLE_PCO_ENV:
...
)
{
  ...
  if(gstk_nv_cache.me_items.me_spec_feature_bmask.state == GSTK_NV_STATE_NOT_INIT)
  {
    (void)gstk_nv_access_read(GSTK_NV_ME_FEATURE_BMASK,
                              GSTK_SLOT_ID_MAX,
                              &nv_data);
  }
