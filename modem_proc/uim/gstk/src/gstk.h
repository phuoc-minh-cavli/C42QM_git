...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk.h#4 $ $DateTime: 2023/02/01 21:55:39 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
01/29/23   ac      Added Satellite E-UTRAN RAT
10/23/20   dt      Fix tech type and timing issue in PCO envelope
07/17/20   dt      Remove TON/NPI for PCO Envelope and from incoming MSISDN
...
#define GSTK_PCO_INFO_ENVELOPE_TAG               0xEF
...
#define GSTK_MAX_EVENT_LIST_LEN         0x1D /* based on Standard, there are only 0x1D events
...
#define GSTK_RCEVT_DS_TASK_READY_SIG          0x00000004 /* To receive notification from RCINIT
                                                            that DS task is up */
...
#define GSTK_CLASH_WITH_REX_SLEEP_GRP_ACK_SIG 0x00010000 /* RESERVED SIGNAL */
#ifdef FEATURE_LTE
#define GSTK_RRC_MSG_Q_SIG                    0x00020000
#endif /*FEATURE_LTE*/
#define GSTK_QMI_CAT_QCSI_SIG                0x00040000 /* RESERVED SIGNAL */
...
#define GSTK_REFRESH_ENFORCEMENT_NAVIGATING_MENU_MASK   0x01
#define GSTK_REFRESH_ENFORCEMENT_DATA_CALL_MASK         0x02
#define GSTK_REFRESH_ENFORCEMENT_VOICE_CALL_MASK        0x04
...
typedef enum {
    ...
    GSTK_TR_TIMER5,
    GSTK_TR_TIMER6,
    GSTK_LOC_INFO_TIMER   = 0x00000009,
    GSTK_TR_DELAY_TIMER,
...
typedef enum
{
    ...
    GSTK_REFRESH_ENFORCEMENT_POLICY_TAG                       = 0x3A,    
...
typedef struct
{
   uint8                         tag;                     /* GSTK_MSISDN_TAG */
   uint8                         length;
   uint8                         msisdn[GSTK_MSISDN_LEN]; /* based on PCO req. spec. */
}
gstk_msisdn_tlv_type;
...
typedef enum
{
  ...
  GSTK_RAT_LTE_IDX                    = 0x1, /* E-UTRAN/Satellite E-UTRAN RAT */
  GSTK_RAT_MAX_IDX
...
{
  gstk_envelope_event_dl_cmd_header_type      event_download_hdr;
  gstk_duration_tlv_type                      request;
}
gstk_envelope_poll_interval_evt_dl_command_type;
...
typedef struct
{
   uint8                                    tag;                 /* GSTK_PCO_INFO_ENVELOPE_TAG */
   uint8                                    length;
   /* Mandatory */
   gstk_device_identities_tag_tlv_type      device_id;
   /* Mandatory - contains TON, NPI and dialing number */
   gstk_msisdn_tlv_type                     msisdn_info;
}
gstk_envelope_pco_info_command_type;
...
/*===========================================================================
  STRUCT: GSTK_PCO_INFO_TYPE

  PURPOSE:
    The structure is used to hold information related to the
    Protocol Configuration Options (PCO)
==========================================================================*/
typedef struct
{
  /* DS SUB ID on which PCO MSISDN event is registered */
  ds_sys_subscription_enum_type sub_id;
  /* Attach APN corresponding to the DS SUB */
  uint8                         apn_string_len;
  char                          apn_string[DS_SYS_MAX_APN_LEN];
  /* Is PCO envelope sent to card and response received*/
  boolean                       is_pco_resp_rcvd;
  /* MSISDN recevied in PCO event from DS */
  gstk_msisdn_info_type         msisdn_info;
      /* Is PCO envelope sent to card */
  boolean                       is_pco_sent_to_card;
}
gstk_pco_info_type;
...
typedef struct
{
  ...
  gstk_terminal_profile_cmd_type   * client_tp_ptr[1];
  ...
  uint32                             gstk_curr_bearer_buffer_size;
  ...
  /* Indicates if Polling Off has been requested by card */
  boolean                            polling_off;
  gstk_pco_info_type                 pco_info;
}
gstk_priv_data_type;
...
typedef struct
{
  ...
  gstk_env_cmd_ref_table_type           envelope_cmd_ref_table[GSTK_MAX_ENVELOPE_COMMAND];
  /* client id from cm */
