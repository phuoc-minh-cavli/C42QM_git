...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_send_ussd.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/09/16   dt      KW fixes array out of bound in estk_gstk_init
...
{  
  ...
  if (other_info_ptr->present) {
    /* CC */
    if (other_info_ptr->extra_param.send_ussd_extra_param.has_cc_type_modification) {
  
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.has_cc_type_modification = TRUE;
  
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.length = 0;
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.additional_info_ptr = NULL;
  
      switch (gstk_shared_data.cc_req_action.call_type) {
      case GSTK_VOICE:
      case GSTK_SS:
        if (other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.length != 0) {
          task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.additional_info_ptr =
            GSTK_CALLOC(other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.length);
          if (task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.additional_info_ptr == NULL) {
            /* free data and return */
            /* free the additional result */
            if (task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
              gstk_free(task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr);
              task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr = NULL;
            }
            /* free the alpha */
            if (gstk_shared_data.cc_req_action.alpha.text != NULL) {
              gstk_free(gstk_shared_data.cc_req_action.alpha.text);
          ...
          task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.length =
            other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.length;
          (void)memscpy(task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.additional_info2_ptr.additional_info_ptr,
                  other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.length,
                  other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.additional_info_ptr,
                  other_info_ptr->extra_param.send_ussd_extra_param.additional_info2_ptr.length);
        }
        break;
      default:
      ...
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.command_result2 =
        other_info_ptr->extra_param.send_ussd_extra_param.command_result2;
    } /* has cc */
    else
    {
      /* text */
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.length =
        other_info_ptr->extra_param.send_ussd_extra_param.data.length;
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.text = NULL;
  
      if (task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.length > 0) {
        task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.text = (uint8*)GSTK_CALLOC(
                         task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.length);
        if (task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.text == NULL) {
          /* free the additional result */
          if (task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
            gstk_free(task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr);
            task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.result_additional_info.additional_info_ptr = NULL;
        ...
        (void)memscpy(task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.text,
               other_info_ptr->extra_param.send_ussd_extra_param.data.length,
               other_info_ptr->extra_param.send_ussd_extra_param.data.text,
               other_info_ptr->extra_param.send_ussd_extra_param.data.length);
      }
      task_cmd_ptr->cmd.send_ussd_term_rsp_cnf.ussd_extra_param.data.dcs =
        other_info_ptr->extra_param.send_ussd_extra_param.data.dcs;
    }
  } /* has extra param*/
  else {
    /* just to make sure */
...
DESCRIPTION
  This function check whether the USSD capabilites are enabled
