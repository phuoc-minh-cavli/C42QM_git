...
Copyright (c) 2003-2005, 2008-2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_s_idle.c#1 $ $Author: pwbldsvc $ $DateTime: 2020/07/22 10:53:27 $
  ...
  gstk_cmd_type* cmd_ptr)
{
  ...
  switch(cmd_ptr->general_cmd.message_header.command_group )
  {
      ...
      else
      {
              ...
              gstk_curr_inst_ptr->slot_id) == TRUE)
        {
          ...
          gstk_find_normal_service_rat_when_mode_ch(&serv_access_tech,&(cmd_ptr->mode_change_ind));
            ...
            /* exit_from_psm_mode is false as PSM is not applicable to 2G cards */ 
       	    gstk_status = gstk_pack_tp_cmd_to_uim_with_df(df, FALSE);
    ...
    case GSTK_CM_CSG_CHANGE_IND:
      ...
      {
        gstk_status = gstk_csg_process_cm_csg_change_ind(cmd_ptr);
      }
      else
      {
      ...
      if(cat_version >= GSTK_CFG_CAT_VER6)
      {
        switch (cmd_ptr->cm_ph_ind.cm_ph_event)
        {
          ...
          case GSTK_CM_PH_EVENT_INFO:
            ...
            {
              gstk_csg_process_cm_ph_event(local_info_ptr);
            }
    ...
    case GSTK_TIME_ZONE_CHANGE_IND:
      ...
      {
        if ((cmd_ptr->time_zone_ind.cm_stack[i].tz_avail != GSTK_TZ_NO_CHANGE) ||
             cmd_ptr->time_zone_ind.cm_stack[i].time_data_avail)
        {
          switch (cmd_ptr->time_zone_ind.cm_stack[i].access_tech)
          {
            ...
            case GSTK_ACCESS_TECH_LTE:
              gstk_curr_inst_ptr->tz_info[i] = cmd_ptr->time_zone_ind.cm_stack[i];
              break;
            default:
              /* no action needed */
              break;
