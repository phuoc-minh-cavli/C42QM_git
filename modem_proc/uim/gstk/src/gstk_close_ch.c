...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_close_ch.c#1 $
...
)
{
  ...
  */
  ...
  /* Check if this CLOSE CH wants to keep the link open */
  /* Command details, qualifier bit 1 (0x01) */
  if(command_details_ref_table_ptr[GSTK_CMD_REF_TABLE_INDEX(cmd_details_ref_id)].command_details.qualifier & 0x01)
  {
    request.cmd.close_ch_pro_cmd_req.keep_ch_open = TRUE;
  }
  else
  {
    request.cmd.close_ch_pro_cmd_req.keep_ch_open = FALSE;
  }
  UIM_MSG_HIGH_1(" keep_ch_open = %d", request.cmd.close_ch_pro_cmd_req.keep_ch_open);
