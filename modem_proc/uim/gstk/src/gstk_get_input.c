...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_get_input.c#1 $
  ...
  gstk_get_input_cnf_type* get_input_cnf_ptr)
{
  ...
  if(get_input_cnf_ptr->data_present) {
    ...
    if (get_input_cnf_ptr->returned_data_len > 0) {
      ...
      if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_TRANSCODING_ENABLE) == TRUE)
      {
...
)
{
  ...
  if(gstk_status == GSTK_SUCCESS) 
  {
    ...
    {
      task_cmd_ptr->cmd.get_input_term_rsp_cnf.data_present = TRUE;
      task_cmd_ptr->cmd.get_input_term_rsp_cnf.returned_data_len = other_info_ptr->extra_param.user_get_input.length;
      if (task_cmd_ptr->cmd.get_input_term_rsp_cnf.returned_data_len > 0) {
        task_cmd_ptr->cmd.get_input_term_rsp_cnf.data = (uint8*)GSTK_CALLOC(
                         other_info_ptr->extra_param.user_get_input.length);
        if (task_cmd_ptr->cmd.get_input_term_rsp_cnf.data == NULL) {
          if (task_cmd_ptr->cmd.get_input_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
            gstk_free(task_cmd_ptr->cmd.get_input_term_rsp_cnf.result_additional_info.additional_info_ptr);
            task_cmd_ptr->cmd.get_input_term_rsp_cnf.result_additional_info.additional_info_ptr = NULL;
          }
          return GSTK_MEMORY_ERROR;
        }
        (void)memscpy(task_cmd_ptr->cmd.get_input_term_rsp_cnf.data,
               other_info_ptr->extra_param.user_get_input.length,
               other_info_ptr->extra_param.user_get_input.text,
               other_info_ptr->extra_param.user_get_input.length);
      }
      task_cmd_ptr->cmd.get_input_term_rsp_cnf.data_coding_scheme = other_info_ptr->extra_param.user_get_input.dcs;
    }
    else 
    {
