...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_polling.c#1 $
    ...
    if (uim_cmd_ptr != NULL )
    {
      ...
      if (gstk_status == GSTK_SUCCESS && uim_cmd_ptr)
      {
        ...
        if(!is_polling_off) {
          ...
          gstk_status = gstk_util_get_duration_in_ms(poll_duration.units,
      ...
      { /* send to UIM */
        /* Update Polling Off Status */
        if(is_polling_off)
        {
          gstk_curr_inst_ptr->polling_off = TRUE;
        }
        else
        {
