...
FUNCTIONS
 ...
 gstk_util_is_valid_func_req
  ...
  gstk_util_map_ds_to_gstk_sys_status
    This function will map DS system status enums to its GSTK counterparts
...
Copyright (c) 2003-2020, 2022-2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstkutil.c#8 $ $DateTime: 2023/06/08 10:12:54 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
06/08/23   ac      Fixed geo loc terminal response result code 
03/27/23   dt      Adding ULOG for attach control crash issue
08/08/22   shp     Fix OOB issues in APN parsing
03/09/22   dt      Fix security issue while parsing Network Access name tag
03/15/22   dt      Resolve security issue in item icon parsing
...
12/19/16   gs      Ignore NULL RATs while checking for 1X only availability
12/09/16   dt      KW fixes array out of bound in gstk_get_input.c
09/12/16   shr     Add support for Poll Interval Envelope
08/05/16   gs      Adapt the common functionality for getting RAT capabilities
06/28/16   gs      Fix compilation issue
...
05/12/16   vr      Make util dump print less lines
...
void gstk_util_env_cmd_ref_table_free_slot(uint32 ref_id) {
  ...
  if (ref_id < GSTK_MAX_ENVELOPE_COMMAND)
  {
    /* Set the free slot boolean to TRUE, so that the slot is avaliable next time */
    ...
    gstk_shared_data.envelope_cmd_ref_table[ref_id].is_menu_selection = FALSE;
  }
  else
  {
...
)
{
  ...
  switch(env_cmd_type)
  {
    ...
    case GSTK_POLL_INTERVAL_EVT_IND:
      env_rsp_type = GSTK_POLL_INTERVAL_EVT_IND_RSP;
      break; 
    case GSTK_PCO_INFORMATION_IND:
      env_rsp_type = GSTK_PCO_INFO_ENVELOPE_RSP;
      break; 
    default:
      break;
  }
  ...
  *env_ref_ptr = gstk_util_env_cmd_ref_table_get_next_free_slot();
  ...
  if ((GSTK_CMD_DETAILS_REF_TABLE_FULL == *env_ref_ptr)
      || (*env_ref_ptr >= (GSTK_MAX_ENVELOPE_COMMAND - 1)))
  {
      ...
      if(gstk_shared_data.gstk_client_table[client_id - 1].client_cb != NULL)
      {
        ...
        gstk_shared_data.envelope_cmd_ref_table[*env_ref_ptr].env_cb.new_client_env_cb =
          (gstk_toolkit_evt_cb_func_type)gstk_shared_data.gstk_client_table[client_id - 1].client_cb;
              ...
              (gstk_shared_data.gstk_client_table[client_id - 1].client_type == GSTK_UIMDRV_TYPE))
      {
  ...
  uint32                  env_ref_id)
{
  ...
  if ((env_ref_id == GSTK_CMD_DETAILS_REF_TABLE_FULL) ||
      (env_ref_id >= GSTK_MAX_ENVELOPE_COMMAND))
  {
...
{
  if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_GSTK_CUST_ENH0) == FALSE)
  {
    ...
    gstk_cmd_from_card_type * message_p )
{
  ...
  case GSTK_REFRESH_REQ:
    ...
    if (message_p->cmd.refresh_pro_cmd_req.refresh_enforcement_policy.refresh_enforcement_policy!= NULL)
    {
      gstk_free(message_p->cmd.refresh_pro_cmd_req.refresh_enforcement_policy.refresh_enforcement_policy);
      message_p->cmd.refresh_pro_cmd_req.refresh_enforcement_policy.refresh_enforcement_policy = NULL;
    }    
  ...
  case GSTK_SETUP_MENU_REQ:
    ...
    {
      for(i = 0; i< message_p->cmd.setup_menu_pro_cmd_req.icon_id_list.num_items && i < GSTK_MAX_MENU_ITEMS; i++) {
        /* array of pointer to icon type */
  ...
  case GSTK_SETUP_IDLE_TEXT_REQ:
  	gstk_release_setup_idle_text_memory(message_p);
    break;
  ...
  case GSTK_POLL_INTERVAL_EVT_IND_RSP:
    /* nothing to free */
    break;
  ...
  const gstk_evt_dl_ind_type  * evt_list)
{
  ...
  case GSTK_POLL_INTERVAL_EVT_IND:
    if ((inst_ptr->gstk_evt_list[GSTK_POLL_INTERVAL] & 0x01) == 0x01) {

      UIM_MSG_HIGH_2("Poll Interval Evt, Length: 0x%x Units: 0x%x",
                     evt_list->evt_ind.poll_interval_evt_ind.length,
                     evt_list->evt_ind.poll_interval_evt_ind.units);
      ...
      cmd->cmd.poll_interval_envelope_ind.message_header.command_group = GSTK_ENVELOPE_CMD;
      gstk_set_command_id(&cmd->cmd.poll_interval_envelope_ind.message_header,
                          GSTK_POLL_INTERVAL_EVT_IND);
      cmd->cmd.poll_interval_envelope_ind.message_header.user_data = env_ref_id;
      cmd->cmd.poll_interval_envelope_ind.poll_interval_evt_info =
        evt_list->evt_ind.poll_interval_evt_ind;
    }
    else {
      UIM_MSG_ERR_0("Poll Interval Evt is not set");
      return GSTK_UNSUPPORTED_COMMAND;
    }
    break;
...
)
{
  ...
  if((alphabet_in == NULL)||(alphabet_length == 0) || (alphabet_out == NULL) ||
  ...
  */
  ...
  case GSTK_APN_NET_ID:
    ...
    */
    ...
    /*label_len */
    for( i = *ind,j=1; j <= alphabet_in[0] && i <= (*ind+lb_len) && i < alphabet_length;j++,i ++)
    {
  ...
  case GSTK_APN_OP_ID:
    ...
    while(val < 3)
    {
      ...
      {
        for( i = *ind, j = init+1; i <= (*ind+3) && j <= init+3 && i < alphabet_length; i ++, j++)
        {
          alphabet_out[i] =  alphabet_in[j];
        }
        for(j=*ind + GSTK_MAX_OP_LABEL_LEN, k = init+lb_len; (j> 0) && (k > init+3) && (j < alphabet_length);k--,j--)
        {
          alphabet_out[j] = alphabet_in[k];
        }
	if(*ind+7 < alphabet_length)
      ...
      {
        for( i = *ind,j=init; j <= init+lb_len && i <= (*ind+lb_len) && i < alphabet_length;j++,i ++)
        {
          alphabet_out[i] =  alphabet_in[j];
        }
...
gstk_status_enum_type gstk_util_retry_envelope_cmd(void)
{
  ...
  if ((gstk_curr_inst_ptr->gstk_curr_envelope_info.env_ref_id == GSTK_CMD_DETAILS_REF_TABLE_FULL) ||
      (gstk_curr_inst_ptr->gstk_curr_envelope_info.env_ref_id >= GSTK_MAX_ENVELOPE_COMMAND))
  {
...
boolean gstk_util_get_rat_from_ds(
  ps_iface_ioctl_bearer_tech_type *rat_ptr
)
{
...
void gstk_util_send_attach_ctrl_res()
{
  ...
  {
	ULOG_RT_PRINTF_2(gstk_get_ulog_handle(),"=GSTK= gstk_util_send_attach_ctrl_res : 0x%x 0x%x",
	                 gstk_curr_inst_ptr->attach_ctrl_res,
                     gstk_curr_inst_ptr->attach_ctrl_p);
  }
  ...
  gstk_cmd_check_orig_enum_type         cmd_origin)
{
...
#else
    UIM_MSG_HIGH_0("MM IDLE PTR is NULL");
#endif /* FEATURE_CDMA */
    return TRUE;
  }
  ...
  UIM_MSG_HIGH_2("Lowest = Highest MM IDLE LS 0x%x Mode Change LS 0x%x",
                 mm_idle_ls, mode_change_ls);
...
boolean gstk_util_requeue_prio_mm(gstk_mm_idle_state_type* new_mm_ptr)
{
  ...
  /* Compare LS */
  UIM_MSG_HIGH_2("MM IDLE arbitration, prev stack RAT: 0x%x, new stack RAT: 0x%x",
                                         ...
                                         gstk_tag_enum_type     tag)
{
  ...
  switch(tag)
  {
    ...
    case GSTK_TAG_POWER_DOWN:
      ...
      /* Clean-up the PCO envelope info cache */
      gstk_curr_inst_ptr->pco_info.is_pco_resp_rcvd = FALSE;
      gstk_curr_inst_ptr->pco_info.is_pco_sent_to_card = FALSE;
      memset(&gstk_curr_inst_ptr->pco_info.msisdn_info, 0x00, sizeof(gstk_msisdn_info_type));

      /* Clear Polling Off Status */
      gstk_curr_inst_ptr->polling_off = FALSE;
      break;
    default:
      /*Do nothing*/
      break;
...
/*===========================================================================
FUNCTION gstk_util_map_ds_to_gstk_sys_status

DESCRIPTION
  This function will map DS system status enums to its GSTK counterparts

PARAMETERS
  ds_sys_status_ptr    [input] : DS system status structure
  gstk_sys_status_ptr  [output]: GSTK version of DS sys status structure
...
RETURN VALUE
  ...
  GSTK_SUCCESS - Mapping was successful
  GSTK_ERROR - Mapping was not successful
===========================================================================*/
extern gstk_status_enum_type gstk_util_map_ds_to_gstk_sys_status(
   ds_sys_status_type                  *ds_sys_status_ptr,
  gstk_ds_sys_system_status_info_type  *gstk_sys_status_ptr
)
{
  if (!ds_sys_status_ptr || !gstk_sys_status_ptr)
  {
    return GSTK_ERROR;
  }

  gstk_sys_status_ptr->so_mask = ds_sys_status_ptr->so_mask;

  switch (ds_sys_status_ptr->rat_value)
  {
    case DS_SYS_RAT_EX_NULL_BEARER:
      gstk_sys_status_ptr->rat_value = GSTK_DS_SYS_RAT_EX_NULL_BEARER;
      break;
    case DS_SYS_RAT_EX_3GPP_GERAN:
      gstk_sys_status_ptr->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_GERAN;
      break;
    case DS_SYS_RAT_EX_3GPP_LTE:
      gstk_sys_status_ptr->rat_value = GSTK_DS_SYS_RAT_EX_3GPP_LTE;
      break;
    case DS_SYS_RAT_MAX:
      gstk_sys_status_ptr->rat_value = GSTK_DS_SYS_RAT_MAX;
      break;
    default:
      return GSTK_ERROR;
  }

  switch (ds_sys_status_ptr->technology)
  {
    case DS_SYS_NETWORK_3GPP:
      gstk_sys_status_ptr->technology = GSTK_DS_SYS_NETWORK_3GPP;
      break;
    case DS_SYS_NETWORK_MAX:
      gstk_sys_status_ptr->technology = GSTK_DS_SYS_NETWORK_MAX;
      break;
    default:
      return GSTK_ERROR;
  }
...
/*===========================================================================
FUNCTION gstk_util_get_duration_in_ms

DESCRIPTION
  This function returns the duration in msec

PARAMETERS
  units:         [Input]        Unit of time
                                - GSTK_DURATION_MINUTES (0)
                                - GSTK_DURATION_SECONDS (1)
                                - GSTK_DURATION_TENTHS_OF_SECONDS (2)
  length:        [Input]        Time interval
  time_msec_ptr: [Input/Output] Pointer to which the time in msec
                                 can be populated
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_util_get_duration_in_ms(
  uint8   units,
  uint8   length,
  uint32 *time_msec_ptr        
)
{
  gstk_status_enum_type gstk_status = GSTK_SUCCESS;

  if(time_msec_ptr == NULL)
  {
    return GSTK_ERROR;
  }
  ...
  case GSTK_DURATION_MINUTES:
    *time_msec_ptr = length * GSTK_MILISECONDS_IN_A_MINUTE;
    break;
  case GSTK_DURATION_SECONDS:
    *time_msec_ptr = length * GSTK_MILISECONDS_IN_A_SECOND;
    break;
  case GSTK_DURATION_TENTHS_OF_SECONDS:
    *time_msec_ptr = length * GSTK_MILISECONDS_IN_ONE_TENTH_SEC;
    break;
  default:
    UIM_MSG_MED_1("Bad Duration: 0x%x",units);
    gstk_status = GSTK_ERROR;
    break;
  }
  return gstk_status;
} /* gstk_util_get_duration_in_ms */

/*===========================================================================
FUNCTION gstk_util_is_only_1x_ds_cellular_rat_present

DESCRIPTION
  Checks if 1X is the only DS Cellular RAT present
...
SEE ALSO
  None
===========================================================================*/
boolean gstk_util_is_only_1x_ds_cellular_rat_present(void)
{
  uint32  sys_index          = 0;
  boolean is_1x_present      = FALSE;
  boolean other_rats_present = FALSE;
    
  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_curr_inst_ptr, FALSE);

  /* If valid RATs are not available */
  if(gstk_curr_inst_ptr->ds_sys_status_info.num_avail_sys == 0)
  {
    return FALSE;
  }

  for(sys_index = 0; sys_index < gstk_curr_inst_ptr->ds_sys_status_info.num_avail_sys; sys_index++)
  {
    /* Check the DS RATs available */
    switch(gstk_curr_inst_ptr->ds_sys_status_info.avail_sys[sys_index].rat_value)
    {
  ...
  /* Check if 1X is the only RAT available */
  if((is_1x_present) &&
  ...
  return FALSE;
} /* gstk_util_is_only_1x_ds_cellular_rat_present */
