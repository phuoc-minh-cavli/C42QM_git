...
FUNCTIONS
  gstk_sms_pp_dl_ind
    This function builds the sms pp download command and sends it to IM
...
Copyright (c) 2003-2017, 2020, 2023 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_envelope_cmd.c#5 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
03/27/23   dt      Adding ULOG for attach control crash issue
10/23/20   dt      Fix tech type and timing issue in PCO envelope
09/21/20   dt      Fix compilation issues on NBGENIA flavour
...
05/05/16   vr      Send Cached TR only if the delay TR timer is running
05/02/16   gm      Removal of CSG feature for IOE devices
03/07/16   gs      Call Control mechanism reverted back
03/07/16   gs      Envelope commands moved to corresponding new file
...
extern ULogHandle gstk_get_ulog_handle(void);
  ...
  const gstk_raw_envelope_ind_type * raw_env_cmd_ptr)
{
  ...
  if (raw_env_cmd_ptr->env_data_ptr[0] == GSTK_EVENT_DOWNLOAD_TAG)
  {
    ...
    if(event_list_type == GSTK_POLL_INTERVAL)
    {
      /* Do not send Poll Interval to card if Polling is Off */
      if(gstk_curr_inst_ptr->polling_off)
      {
        return GSTK_UNSUPPORTED_COMMAND;
      }
      
      /* Skip to Unit byte of Duration TLV */
      offset = offset + 7;
      if(offset >= raw_env_cmd_ptr->env_len || offset >= UIM_MAX_ENVELOPE_BYTES)
      {
        return GSTK_BAD_PARAM;
      ...
      /* Cache the Poll Interval unit */
      gstk_shared_data.envelope_cmd_ref_table[raw_env_cmd_ptr->message_header.user_data].poll_interval_evt_info.units =
        raw_env_cmd_ptr->env_data_ptr[offset];

      /* Skip to duration of Duration TLV */
      offset++;
      if(offset >= raw_env_cmd_ptr->env_len || offset >= UIM_MAX_ENVELOPE_BYTES)
      {
        return GSTK_BAD_PARAM;
      ...
      /* Copy the Poll Interval Duration */
      gstk_shared_data.envelope_cmd_ref_table[raw_env_cmd_ptr->message_header.user_data].poll_interval_evt_info.length =
      raw_env_cmd_ptr->env_data_ptr[offset];

      gstk_status = gstk_util_get_duration_in_ms(
                      gstk_shared_data.envelope_cmd_ref_table[raw_env_cmd_ptr->message_header.user_data].poll_interval_evt_info.units,
                      gstk_shared_data.envelope_cmd_ref_table[raw_env_cmd_ptr->message_header.user_data].poll_interval_evt_info.length,
                      &poll_ms);
      
      /* Poll Interval Duration of less than 5 sec is not supported by UIM Driver */
      if((gstk_status != GSTK_SUCCESS) || (poll_ms < 5000))
      {
        return GSTK_BAD_PARAM;
      }      
    }
    ...
    if(raw_env_cmd_ptr->message_header.user_data < GSTK_MAX_ENVELOPE_COMMAND)
    {
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        ...
        if(raw_env_cmd_ptr->env_data_ptr[0] == GSTK_MENU_SELECTION_TAG)
        {
          gstk_shared_data.envelope_cmd_ref_table[raw_env_cmd_ptr->message_header.user_data].is_menu_selection = TRUE;
...
#endif /*(FEATURE_WCDMA) || defined(FEATURE_LTE)*/
    }
    /* Valid location info has been passed to the function */
    else
    {
...
)
{
   ...
   */
   ...
   UIM_MSG_HIGH_0("Sending Network Search Mode envelope to UIM");
   gstk_send_cmd_to_uim_server(uim_cmd_ptr); /* clear signal and call uim_cmd */
   return gstk_status;
}
...
/*===========================================================================
FUNCTION gstk_pco_info_ind

DESCRIPTION
  This function builds the PCO info envelope and sends it to UIM

PARAMETERS
  pco_info_cmd: [Input] Pointer to the envelope command to be processed
...
SEE ALSO
  None
===========================================================================*/
static gstk_status_enum_type gstk_pco_info_ind(
  const gstk_envelope_pco_info_ind_type * pco_info_cmd)
{
  gstk_status_enum_type                gstk_status = GSTK_SUCCESS;
  int                                  offset      = 0;
  uint32                               cmd_ref_id  = 0;
  ...
  uim_cmd_type                        *uim_cmd_ptr = NULL;
  gstk_envelope_pco_info_command_type  STK_envelope_cmd;

  GSTK_RETURN_ERROR_IF_NULL_PTR(gstk_curr_inst_ptr, GSTK_ERROR);

  if(pco_info_cmd == NULL)
  {
    UIM_MSG_ERR_0("pco_info_cmd ERR : NULL");
    return GSTK_BAD_PARAM;
  }
  ...
  /* Initialize STK_envelope_cmd */
  memset(&STK_envelope_cmd, 0,
         sizeof(gstk_envelope_pco_info_command_type));

  /* Pack various TLVs */
  STK_envelope_cmd.tag = GSTK_PCO_INFO_ENVELOPE_TAG;
  ...
  /* MSISDN info */
  gstk_status = gstk_packer_msisdn_tlv(&STK_envelope_cmd.msisdn_info,
                                       &pco_info_cmd->msisdn_info);
  ...
  /* Total data length: device id data + 2 (2 is for the tag and length field)
                       + PCO data + 2 (tag and length) */
  STK_envelope_cmd.length = STK_envelope_cmd.device_id.device_tag_length +
                            STK_envelope_cmd.msisdn_info.length +
                            4;
  ...
  if (uim_cmd_ptr != NULL )
  {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL))
    {
      /* Required so that uim will grab the data from index 0 */
      uim_cmd_ptr->envelope.offset = 0;

      /* Set user_data */
      uim_cmd_ptr->hdr.user_data = pco_info_cmd->message_header.user_data;
      ...
      if(gstk_byte_offset_memcpy(uim_cmd_ptr->envelope.data,
         ...
         (size_t)(STK_envelope_cmd.device_id.device_tag_length + GSTK_TAG_LENGTH_LEN))
      {
        UIM_MSG_ERR_0("gstk_pco_info_ind(): gstk_byte_offset_memcpy failed");
        gstk_status = GSTK_MEMORY_ERROR;
      }
      /* Copy MSISDN information */
      else if(gstk_byte_offset_memcpy(uim_cmd_ptr->envelope.data,
                                      &STK_envelope_cmd.msisdn_info,
                                      offset +=
                                      (int)(STK_envelope_cmd.device_id.device_tag_length +
                                            GSTK_TAG_LENGTH_LEN),
                                      (size_t)(STK_envelope_cmd.msisdn_info.length +
                                               GSTK_TAG_LENGTH_LEN),
                                      sizeof(uim_cmd_ptr->envelope.data),
                                      sizeof(gstk_msisdn_tlv_type)) <
              (size_t)(STK_envelope_cmd.msisdn_info.length + GSTK_TAG_LENGTH_LEN))
      {
        UIM_MSG_ERR_0("gstk_pco_info_ind(): gstk_byte_offset_memcpy failed");
        gstk_status = GSTK_MEMORY_ERROR;
      }
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        offset = (int)(offset + STK_envelope_cmd.msisdn_info.length +
                     GSTK_TAG_LENGTH_LEN);

        UIM_MSG_MED_1("Env: PCO Length 0x%x", uim_cmd_ptr->envelope.num_bytes);
        gstk_util_dump_byte_array("Env: PCO Info Data",
                                  uim_cmd_ptr->envelope.data, 
                                  uim_cmd_ptr->envelope.num_bytes);

        /* Envelope Retry logic: only if envelope state is GSTK_NONE,
           change state to RSP_FROM_CARD and store the env payload */                      
        if(gstk_curr_inst_ptr->gstk_curr_envelope_info.retry_info.retry_flag == TRUE)
        {
          gstk_status = gstk_process_and_store_env_cmd_for_retry(
                          uim_cmd_ptr->envelope.data,
        ...
        gstk_curr_inst_ptr->pco_info.is_pco_sent_to_card = TRUE;
      }
      else
      {
        gstk_send_cmd_to_uim_free_q(uim_cmd_ptr);
  ...
  /* nothing to free for PCO envelope */
  return gstk_status;
} /* gstk_pco_info_ind */
...
)
{
  ...
  if((GSTK_MEMORY_ERROR == gstk_status)
     && ((GSTK_MAX_ENVELOPE_COMMAND - 1) == env_ref_id))
  {
  ...
  switch( type_of_command )
  {
    ...
    case GSTK_LOCATION_INFORMATION_IND:
      ...
      if (gstk_get_ulog_handle() != NULL)
      {
        ULOG_RT_PRINTF_1(gstk_get_ulog_handle(),"=GSTK= Rec'd envelope GSTK_LOCATION_INFORMATION_IND user_data %d", 
	...
	if (gstk_get_ulog_handle() != NULL)
	{
          ULOG_RT_PRINTF_5(gstk_get_ulog_handle(),"=GSTK= Rec'd attach ctrl sig_to_wait 0x%x,  task_tcb 0x%x MCC/MNC: 0x%x 0x%x 0x%x", 
		                 cmd->attach_ctrl_ind.attach_ctrl.sig_to_wait,
                         cmd->attach_ctrl_ind.attach_ctrl.task_tcb,
					     cmd->attach_ctrl_ind.attach_ctrl.mcc_mnc[0],
                         cmd->attach_ctrl_ind.attach_ctrl.mcc_mnc[1],
        ...
        else
        {
	      ...
	      if (gstk_get_ulog_handle() != NULL)
              {
                ULOG_RT_PRINTF_0(gstk_get_ulog_handle(),"=GSTK= attach control result is sent - REFRESH command in progress");
	      }
            }
            else
            {
	        ...
	        if (gstk_get_ulog_handle() != NULL)
	        {
	          ULOG_RT_PRINTF_0(gstk_get_ulog_handle(),"=GSTK= gstk_last_loc_info_sent and attach_ctrl_p->mcc_mnc doesn't match");
	        }
		...
		if (gstk_get_ulog_handle() != NULL)
                {
                  ULOG_RT_PRINTF_1(gstk_get_ulog_handle(),"=GSTK= same loc info env is already sent loc_info_env_res 0x%x", 
				                   gstk_curr_inst_ptr->loc_info_env_res);
                }
    ...
    case GSTK_IMS_REG_EVT_IND:
      ...
      break;
      case GSTK_POLL_INTERVAL_EVT_IND:
      UIM_MSG_HIGH_0(" Rec'd Poll Interval Evt");  
      if(error_env_flg == FALSE)
      {
        /* Cache Polling Duration info */
        gstk_shared_data.envelope_cmd_ref_table[env_ref_id].poll_interval_evt_info =
          cmd->poll_interval_envelope_ind.poll_interval_evt_info;
        gstk_status = gstk_poll_interval_evt_ind(&(cmd->poll_interval_envelope_ind));
      }
      if((error_env_flg == TRUE) || (gstk_status != GSTK_SUCCESS))
      {
        gstk_status = gstk_build_error_envelope_report(cmd->poll_interval_envelope_ind.message_header.user_data,
                                                       cmd->poll_interval_envelope_ind.message_header.command_id,
                                                       GSTK_ENVELOPE_CMD_FAIL);
      }
      break;
   case GSTK_PCO_INFORMATION_IND:
      UIM_MSG_HIGH_0(" Rec'd Location Statuc Evt:GSTK_PCO_INFORMATION_IND");
      if(error_env_flg == FALSE && gstk_curr_inst_ptr->pco_info.is_pco_sent_to_card != TRUE)
      {
        gstk_status = gstk_pco_info_ind(&(cmd->pco_info_envelope_ind));
      }
      if((error_env_flg == TRUE) || (gstk_status != GSTK_SUCCESS))
      {
        gstk_status = gstk_build_error_envelope_report(cmd->pco_info_envelope_ind.message_header.user_data,
                                                       cmd->pco_info_envelope_ind.message_header.command_id,
                                                       GSTK_ENVELOPE_CMD_FAIL);
      }
      break;
...
/*===========================================================================
FUNCTION   gstk_build_pco_envelope

DESCRIPTION
  This function builds the PCO envelope.

PARAMETERS
  msisdn_info_ptr : MSISDN info.
  slot_id         : SIM slot ID.
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_build_pco_envelope(
  ds_sys_msisdn_info_type *msisdn_info_ptr,
...
)
{
  ...
  /* Return if MSISDN is invalid */
  if((msisdn_info_ptr == NULL) ||
     (msisdn_info_ptr->msisdn_length == 0))
  {
    UIM_MSG_ERR_0("gstk_build_pco_envelope, Invalid MSISDN info");
    return GSTK_ERROR;
  }

  UIM_MSG_HIGH_2("gstk_build_pco_envelope, MSISDN length: 0x%x, slot: 0x%x",
                 msisdn_info_ptr->msisdn_length,
                 slot_id);

  /* If MSISDN is same as the one stored in cache and has already been sent
     to the SIM card, no need to build a new envelope */
  if((gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.is_pco_resp_rcvd == TRUE) &&
     (gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn_length ==
      msisdn_info_ptr->msisdn_length) &&
     (memcmp(gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn,
             msisdn_info_ptr->msisdn,
             msisdn_info_ptr->msisdn_length) == 0))
  {
    UIM_MSG_HIGH_0("MSISDN is same as before");
    return GSTK_ERROR;
  }

  /* Cache the MSISDN */
  gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn_length =
  gstk_memcpy((void*)gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn,
              (void*)msisdn_info_ptr->msisdn,
              msisdn_info_ptr->msisdn_length,
              sizeof(gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn),
              sizeof(msisdn_info_ptr->msisdn));

  gstk_util_dump_byte_array("MSISDN from DS cached",
                            (byte *)gstk_curr_inst_ptr->pco_info.msisdn_info.msisdn,
                            (uint32)gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info.msisdn_length);

  /* Need to send PCO envelope to card */
  gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.is_pco_resp_rcvd = FALSE;
  ...
  /* Successfully obtained command buffer - build the envelope command */
  task_cmd->cmd.pco_info_envelope_ind.message_header.client_id = gstk_instances_ptr[(uint32)slot_id - 1]->client_id;
  task_cmd->cmd.pco_info_envelope_ind.message_header.sim_slot_id = slot_id;
  task_cmd->cmd.pco_info_envelope_ind.message_header.command_group = GSTK_ENVELOPE_CMD;
  task_cmd->cmd.pco_info_envelope_ind.message_header.command_id = (int)GSTK_PCO_INFORMATION_IND;
  task_cmd->cmd.pco_info_envelope_ind.message_header.user_data = 0x00;

  /* Copy MSISDN info */
  (void)gstk_memcpy((void*)&task_cmd->cmd.pco_info_envelope_ind.msisdn_info,
                    (void*)&gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info,
                    sizeof(gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info),
                    sizeof(task_cmd->cmd.pco_info_envelope_ind.msisdn_info),
                    sizeof(gstk_instances_ptr[(uint32)slot_id - 1]->pco_info.msisdn_info));

  /* set GSTK_CMD_Q_SIG and put on GSTK queue */
  gstk_task_put_cmd_buf(task_cmd);
  return GSTK_SUCCESS;
}/* gstk_build_pco_envelope */
