...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_select_item.c#1 $
...
)
{
  ...
  if (gstk_status == GSTK_SUCCESS)
  {
    if (other_info_ptr->present) {
      task_cmd_ptr->cmd.select_item_term_rsp_cnf.data_present = TRUE;
      task_cmd_ptr->cmd.select_item_term_rsp_cnf.chosen_item_identifier =
          other_info_ptr->extra_param.select_item_chosen_item;
    }
  }
  else {
    if (task_cmd_ptr->cmd.select_item_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
      gstk_free(task_cmd_ptr->cmd.select_item_term_rsp_cnf.result_additional_info.additional_info_ptr);
      task_cmd_ptr->cmd.select_item_term_rsp_cnf.result_additional_info.additional_info_ptr = NULL;
    }
  }  
  return gstk_status;
}
