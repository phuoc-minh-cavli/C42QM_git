...
Copyright (c) 2003-2016, 2020 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_evt_dl.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/17/20   dt      Added support for VZW PCO command
03/28/18   dt      From Device identity of IMS evt dl changed to network
...
03/07/16   gs      Event downloads moved to corresponding new file
  ...
  uint32      ref_id)
{
  ...
  /* Per PCO requirement, attempt only one retry for PCO envelope */
  if(data_ptr[0] == GSTK_PCO_INFO_ENVELOPE_TAG)
  {
    gstk_curr_inst_ptr->gstk_curr_envelope_info.retry_info.retry_counter =
      nv_data.env_retry_ext.max_retries - 1;
  }

  if ((ref_id == GSTK_CMD_DETAILS_REF_TABLE_FULL) ||
      (ref_id >= GSTK_MAX_ENVELOPE_COMMAND))
  {
...
#ifdef FEATURE_ESTK
   if(estk_bip_is_channel_closed(ch_status_evt_cmd->ch_status_info.ch_id,
                                 ch_status_evt_cmd->message_header.sim_slot_id))
   {
     UIM_MSG_HIGH_1("BIP channel 0x%x is closed, channel status indication is not sent",
                    ch_status_evt_cmd->ch_status_info.ch_id);
     return GSTK_ERROR;
   }   
#endif /* FEATURE_ESTK */
   STK_envelope_cmd.event_download_hdr.tag = GSTK_EVENT_DOWNLOAD_TAG;
...
/*===========================================================================
FUNCTION   gstk_poll_interval_evt_ind

DESCRIPTION
  This function builds the envelope of Poll Interval event

PARAMETERS
  poll_interval_evt_cmd: [Input] The struct of Poll Interval
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_poll_interval_evt_ind(
  gstk_envelope_poll_interval_evt_ind_type *poll_interval_cmd)
{
  gstk_status_enum_type                            gstk_status         = GSTK_SUCCESS;
  uim_cmd_type*                                    uim_cmd_ptr         = NULL;
  int                                              offset              = 0;
  gstk_duration_type                               duration;
  gstk_envelope_poll_interval_evt_dl_command_type  STK_envelope_cmd;

  if (poll_interval_cmd == NULL)
  {
    return GSTK_BAD_PARAM;
  }
  ...
  duration.length = poll_interval_cmd->poll_interval_evt_info.length;
  duration.units = poll_interval_cmd->poll_interval_evt_info.units;
  gstk_status = gstk_packer_duration_tlv(
     &duration,
     &STK_envelope_cmd.request);
  if (gstk_status != GSTK_SUCCESS)
  {
    return gstk_status;
  }
  ...
  /* total data length:  evt id data + 2
                       + device id data + 2 (2 is for the tag and length field)
                       + poll interval data + 2
  */
  STK_envelope_cmd.event_download_hdr.length =
                             STK_envelope_cmd.event_download_hdr.evt_list.length +
                             STK_envelope_cmd.event_download_hdr.device_id.device_tag_length +
                             STK_envelope_cmd.request.length +
                             6;
  ...
  if (uim_cmd_ptr != NULL ) {
    ...
    if ((gstk_status == GSTK_SUCCESS) && (uim_cmd_ptr != NULL)) {
      ...
      /* set user_data */
      uim_cmd_ptr->hdr.user_data = poll_interval_cmd->message_header.user_data;
      ...
      if(gstk_byte_offset_memcpy(uim_cmd_ptr->envelope.data,
                                 &(STK_envelope_cmd.request),
                                 offset,
                                 (size_t)(STK_envelope_cmd.request.length +
                                          GSTK_TAG_LENGTH_LEN),
                                 sizeof(uim_cmd_ptr->envelope.data)-offset,
                                 sizeof(STK_envelope_cmd.request)) <
              (size_t)(STK_envelope_cmd.request.length + GSTK_TAG_LENGTH_LEN))
      {
        gstk_status = GSTK_MEMORY_ERROR;
      }
      ...
      if (gstk_status == GSTK_SUCCESS)
      {
        gstk_util_dump_byte_array("Poll Interval Env",
                                  &uim_cmd_ptr->envelope.data[uim_cmd_ptr->envelope.offset],
                                  uim_cmd_ptr->envelope.num_bytes);
