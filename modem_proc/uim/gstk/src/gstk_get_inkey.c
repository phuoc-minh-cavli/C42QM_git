...
Copyright (c) 2003-2016, 2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_get_inkey.c#1 $
...
/*===========================================================================
  FUNCTION gstk_populate_get_inkey_tr_cnf
  ...
  SEE ALSO
     None
  ===========================================================================*/
gstk_status_enum_type gstk_populate_get_inkey_tr_cnf(
...
)
{
  ...
  task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data_present = FALSE;
  task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.returned_data_len = 0;
  if(gstk_status == GSTK_SUCCESS) {
    if (other_info_ptr->extra_param.get_inkey_extra_param.user_get_inkey_input.length != 0 &&
        other_info_ptr->extra_param.get_inkey_extra_param.user_get_inkey_input.length <= GSTK_GET_INKEY_USER_INPUT_DATA_LEN) {
      task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data_present = TRUE;
      task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.returned_data_len = other_info_ptr->extra_param.get_inkey_extra_param.user_get_inkey_input.length;
      memset(task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data, 0x00, GSTK_GET_INKEY_USER_INPUT_DATA_LEN);
      (void)memscpy(task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data,
             sizeof(task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data),
             other_info_ptr->extra_param.get_inkey_extra_param.user_get_inkey_input.text,
             task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.returned_data_len);
      task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.data_coding_scheme = other_info_ptr->extra_param.get_inkey_extra_param.user_get_inkey_input.dcs;
    }
    else {
      task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.returned_data_len = 0;
    }
    if(cat_version >= GSTK_CFG_CAT_VER6)
    {
      ...
      if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_GSTK_CUST_ENH0) == TRUE) &&
         (other_info_ptr->present &&
          other_info_ptr->extra_param.get_inkey_extra_param.duration_of_exec.present))
      {
        task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.duration_of_exec
            = other_info_ptr->extra_param.get_inkey_extra_param.duration_of_exec;
      }
      else if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_GSTK_CUST_ENH0) == FALSE) &&
              other_info_ptr->extra_param.get_inkey_extra_param.duration_of_exec.present)
      {
        task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.duration_of_exec
            = other_info_ptr->extra_param.get_inkey_extra_param.duration_of_exec;
      ...
      {
        task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.duration_of_exec.present = FALSE;
      }
    }
    else
    {
    ...
    if (task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.result_additional_info.additional_info_ptr != NULL) {
      gstk_free(task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.result_additional_info.additional_info_ptr);
      task_cmd_ptr->cmd.get_inkey_term_rsp_cnf.result_additional_info.additional_info_ptr = NULL;
    }
  }
  return gstk_status;
}
