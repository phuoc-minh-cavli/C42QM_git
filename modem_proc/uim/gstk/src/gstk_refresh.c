...
Copyright (c) 2001-2016, 2019 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstk_refresh.c#1 $
...
static gstk_status_enum_type gstk_send_mmgsdi_refresh_req(
  uint32                         refresh_mode,
  const gstk_refresh_req_type   *refresh_req_ptr,
...
gstk_status_enum_type gstk_refresh_req (gstk_proactive_cmd_type* command)
{
  ...
  /* initialize request */
  memset(request_ptr, 0x00, sizeof(gstk_cmd_from_card_type));
        ...
        switch(0x7F & cmd_data[curr_offset])
        {
          ...
          case GSTK_PLMNWACT_LIST_TAG:
                ...
                expected_tag ==GSTK_PLMNWACT_LIST_TAG)
            {
              ...
              expected_tag = GSTK_REFRESH_ENFORCEMENT_POLICY_TAG;
              plmnwact_list_tag_needed = FALSE;
            }
            break;
            
          case GSTK_REFRESH_ENFORCEMENT_POLICY_TAG:
            if (file_list_tag_needed)
            {
              gstk_status = GSTK_INVALID_COMMAND;
              break;
            ...
            if (expected_tag == GSTK_FILE_LIST_TAG || expected_tag == GSTK_AID_TAG ||
                expected_tag == GSTK_ALPHA_IDENTIFIER_TAG || expected_tag == GSTK_ICON_IDENTIFIER_TAG ||
                expected_tag == GSTK_TEXT_ATTRIBUTE_TAG || expected_tag == GSTK_FRAME_IDENTIFIER_TAG  ||
                expected_tag ==GSTK_PLMNWACT_LIST_TAG || expected_tag == GSTK_REFRESH_ENFORCEMENT_POLICY_TAG)
            {
              UIM_MSG_HIGH_0("GSTK_REFRESH_ENFORCEMENT_POLICY_TAG parsing");
              /* Parse refresh enforcement policy List */
              gstk_status = gstk_parser_refresh_enforcement_policy_tlv( cmd_data,
                                                 cmd_data_len,
                                                 curr_offset,
                                                 &(request_ptr->cmd.refresh_pro_cmd_req.refresh_enforcement_policy),
                                                 &next_tlv_offset,
                                                 &more_tlv,
                                                 GSTK_OPTIONAL,
                                                 &cmd_details_ref_id);         
  ...
  if(gstk_status == GSTK_SUCCESS)
  {
    ...
    else
    {
      ...
      gstk_status = gstk_send_mmgsdi_refresh_req(refresh_mode,
                                                 &(request_ptr->cmd.refresh_pro_cmd_req),
                                                 cmd_details_ref_id);
...
PARAMETERS
  ...
  refresh_req_ptr     [Input] : Refresh request pointer
  ...
  uint32                         cmd_details_ref_id)
{
  ...
  gstk_file_list_type                   *file_list_ptr       = NULL;
  gstk_app_id_type                       app_id;
  gstk_generic_data_type                *plmn_list_ptr       = NULL;
  mmgsdi_refresh_enforcement_policy_type gstk_refresh_enforcement_policy;

  if (refresh_req_ptr == NULL)
  {
    UIM_MSG_ERR_0("Refresh pointer is NULL");
    return GSTK_ERROR;
  }

  file_list_ptr = (gstk_file_list_type *)&(refresh_req_ptr->file_list);
  plmn_list_ptr = (gstk_generic_data_type *)&(refresh_req_ptr->plmn_wact_list);
  app_id = refresh_req_ptr->app_id;
  ...
  memset(&gstk_refresh_enforcement_policy, 0x00, sizeof(mmgsdi_refresh_enforcement_policy_type));
  ...
  if(refresh_req_ptr->refresh_enforcement_policy.length > 0)
  {
    gstk_refresh_enforcement_policy.is_enforcement_present = TRUE;
    if(refresh_req_ptr->refresh_enforcement_policy.refresh_enforcement_policy[0] & GSTK_REFRESH_ENFORCEMENT_NAVIGATING_MENU_MASK)
    {
      gstk_refresh_enforcement_policy.data.force_refresh_navigating_menu = TRUE;
    }
    if(refresh_req_ptr->refresh_enforcement_policy.refresh_enforcement_policy[0] & GSTK_REFRESH_ENFORCEMENT_DATA_CALL_MASK)
    {
      gstk_refresh_enforcement_policy.data.force_refresh_data_call = TRUE;
    }
    if(refresh_req_ptr->refresh_enforcement_policy.refresh_enforcement_policy[0] & GSTK_REFRESH_ENFORCEMENT_VOICE_CALL_MASK)
    {
      gstk_refresh_enforcement_policy.data.force_refresh_voice_call = TRUE;
    }
  }
  ...
  if(mmgsdi_refresh_mode != MMGSDI_REFRESH_STEERING_OF_ROAMING)
  {
    mmgsdi_status = mmgsdi_naa_refresh(gstk_shared_data.gstk_mmgsdi_client_id,
                                       ...
                                       gstk_refresh_enforcement_policy,
                                       option,
                                       gstk_mmgsdi_cmd_cb,
                                       cmd_details_ref_id);
  }
