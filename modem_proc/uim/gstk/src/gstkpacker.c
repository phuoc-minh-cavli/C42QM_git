...
$Header: //components/rel/uim.mpss/3.3.1/gstk/src/gstkpacker.c#2 $
...
SEE ALSO
  None
===========================================================================*/
gstk_status_enum_type gstk_packer_msisdn_tlv(
...
)
{
  ...
  UIM_MSG_HIGH_0("** Packing MSISDN");

  if(msisdn_tlv_ptr == NULL || msisdn_info_ptr == NULL)
  {
     UIM_MSG_ERR_0("MSISDN Err: NULL");
     return GSTK_BAD_PARAM;
  }

  /* Truncate MSISDN if length exceeds max. supported, skip
     first byte of MSISDN from NW (TON/NPI) */
  if((msisdn_info_ptr->msisdn_length -1) > GSTK_MSISDN_LEN)
  {
    copy_len = GSTK_MSISDN_LEN;
  }
  else
  {
    copy_len = msisdn_info_ptr->msisdn_length - 1;
  }

  msisdn_tlv_ptr->tag = (int)GSTK_MSISDN_TAG;
  msisdn_tlv_ptr->length = copy_len;

  /* Skip first byte of MSISDN from NW (TON/NPI) and copy the rest */
  if(gstk_memcpy(msisdn_tlv_ptr->msisdn,
                 &msisdn_info_ptr->msisdn[1],
                 copy_len,
                 GSTK_MSISDN_LEN,
                 GSTK_MAX_MSISDN_LEN - 1) <
     copy_len)
  {
    return GSTK_MEMORY_ERROR;
  }
