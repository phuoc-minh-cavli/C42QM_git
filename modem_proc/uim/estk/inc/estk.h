...
GENERAL DESCRIPTION:
    ESTK generic interface header include file
        ...
        Copyright © 2008 - 2015, 2018 QUALCOMM Technologies, Inc(QTI).
        All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/estk/inc/estk.h#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/17/20   dt      Fixing offtarget TC failures on CT
03/13/20   dt      User data to be made dynamic
12/26/19   dt      Adding support for DNS in BIP
...
/*===========================================================================
  ...
  STRUCT: ESTK_BIP_SOCK_CB_USER_DATA

  PARAMETERS:
    slot_id      : Slot_id on which BIP channel is established
    ch_index    : BIP channel table index
  PURPOSE:
    The structure contains network cb user data information
==========================================================================*/
typedef struct
{
  gstk_slot_id_enum_type slot_id; 
  uint8                  ch_index;
}estk_bip_sock_cb_user_data;

/*===========================================================================

  STRUCT: ESTK_BIP_NET_CB_USER_DATA
...
/*===========================================================================
  ...
  PARAMETERS:
    ...
    user_data    : user data received in net_cb
  PURPOSE:
    The structure contains network cb information
==========================================================================*/
typedef struct
{
  sint15             dss_nethandle;
  sint15             error_num;
  estk_bip_net_cb_user_data  user_data;
}
...
/*===========================================================================
  ...
  PARAMETERS:
    ...
    result                  : CM call id
    estk_raw_evt_cb         : Raw wvent call back type
