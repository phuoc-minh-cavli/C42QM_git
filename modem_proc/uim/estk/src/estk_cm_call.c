...
GENERAL DESCRIPTION : ENHANCED STK layer SetUp call and Send DTMF support
          ...
          Copyright © 2008-2020 QUALCOMM Technologies, Inc (QTI).
          All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/estk/src/estk_cm_call.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/03/20   dt      Do not disconnect calls before user conf is received      
02/03/20   ks      Handle setup call command with alpha as not busy
01/07/20   ks      Fill right call_id from CM before DTMF digit is sent to CM
10/12/18   ks      Split ME and slot specific NV
10/08/18   gs      Remove the usage of cm_mm_call_cmd_get_call_info_list
08/31/18   gs      9x05 interface changes for memory reduction
11/16/17   gs      Clean up redial values
06/03/16   shr     GSTK/ESTK globals memory optimization
05/27/16   gs      F3 Logging inprovements
05/25/16   shr     F3 reduction: Malloc Failure, Memcpy, Null Ptr checks
03/08/16   gs      Init and activation with CM for Supplementary services
03/07/16   gs      Supplementary services init movement
03/07/16   gs      ESTK CM Init is moved
...
/* Maximum of 6 (0-5) calls are supported */
#define ESTK_MAX_CALL_BMASK              0x3F
...
static void estk_populate_call_info();
...
void estk_cm_init( void )
{
  ...
  if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == FALSE) ||
      (gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == FALSE))
  {
  ...
  /* NOTE - An error in any of the function calls below will cause a return from this function.
          Please make sure no mandatory to execute code is below CMs registartion
  */
  if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == FALSE)
  {
    ...
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_ORIG);
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_CONNECT);
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_END);
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_INCOM);
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_MNG_CALLS_CONF);
    (void)bit_mask_256_set_bit(&bmask, CM_CALL_EVENT_ORIG_MOD_TO_SS);
  ...
  gstk_cmd_from_card_type      *gstk_req_ptr)
{
  ...
  switch (gstk_req_ptr->cmd.setup_call_pro_cmd_req.call_setup_requirement)
  {
    ...
    case GSTK_DISCONNECT_ACTIVE_CALLS:
      /* End all calls only if ignore alpha NV is enabled or 
    ...
    case GSTK_NO_OTHER_CALLS:
      /* Check if there is any active calls */
      if((call_bitmask = cm_get_call_ids_allocated()) == 0)
      {
      ...
      /* If the active call bit mask can not be greater than 32, as call_id ranges from 0 to 5
         this means that only 6 calls can be active at a time */
      else if(call_bitmask > ESTK_MAX_CALL_BMASK)
      {
        send_err_rsp_to_gstk = TRUE;
      }
      else 
...
)
{
  ...
  *cmd_has_alpha_ptr = FALSE;
  ...
  else if (gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_ESTK_QMI_SEND_NO_AND_NULL_ALPHA_IND) == TRUE)
  {
    *cmd_has_alpha_ptr = TRUE;
    return estk_send_alpha_to_display_client(gstk_req_ptr, FALSE);
  }
   ...
   gstk_cmd_from_card_type   *gstk_req_ptr)
{
  ...
  switch(gstk_req_ptr->cmd.setup_call_pro_cmd_req.call_setup_requirement)
  {
    ...
    case GSTK_DISCONNECT_ACTIVE_CALLS:
      ...
      if(estk_end_all_calls() == FALSE)
      {
        UIM_MSG_ERR_0("estk_end_all_calls failed");
        return ESTK_ERROR;
      }
      estk_curr_inst_ptr->state = ESTK_PREPROCESS_COMMAND_ST;
      return ESTK_SUCCESS;
...
void estk_setup_call_cleanup(void)
{
  ...
  gstk_free(estk_shared_info.redial_cmd_ptr);
  estk_shared_info.redial_cmd_ptr = NULL;
  ...
  /* Clean up redial values */
  UIM_MSG_HIGH_0("Cleaning up redial values");
...
void estk_handle_cm_cmd_reattempt_timer(uint32 dummy)
{
  ...
  UIM_MSG_HIGH_3("Reattempt Timer Expired: redial_delayed_cm_cmd=0x%x, redial_in_progress=0x%x, redial_cmd_ptr=0x%x",
                 estk_shared_info.redial_delayed_cm_cmd,
                 estk_shared_info.redial_in_progress,
                 estk_shared_info.redial_cmd_ptr);
  ...
  if(estk_shared_info.redial_cmd_ptr == NULL)
  {
    return;
  }
     ...
     (estk_shared_info.redial_in_progress == TRUE))
  {
    ...
    if(estk_cm_orig_call(estk_shared_info.redial_cmd_ptr) == FALSE)
    {
      UIM_MSG_ERR_0("estk_cm_orig_call failed in case of redial");
    }
  }
  else if(estk_shared_info.redial_in_progress == FALSE)
  {
    ...
    (void)estk_send_terminal_response(
            estk_shared_info.redial_cmd_ptr->hdr_cmd.cmd_detail_reference, /* command_details_ref */
            GSTK_SETUP_CALL_CNF,                                      /* command response */
            GSTK_NETWORK_CURRENTLY_UNABLE_TO_PROCESS_COMMAND,         /* general_result */
  ...
  gstk_cmd_from_card_type  *gstk_req_ptr)
{
  ...
  return estk_process_DTMF(gstk_req_ptr);
...
===========================================================================*/
estk_result_enum_type estk_process_DTMF(gstk_cmd_from_card_type        *gstk_req_ptr)
{
  ...
  uint8                          call_bitmask         = 0;
  uint8                          additional_info_data = 0;
  gstk_general_result_enum_type  gstk_result          = GSTK_ME_CURRENTLY_UNABLE_TO_PROCESS_COMMAND;
  boolean                        send_err_rsp_to_gstk = FALSE;
  uint32                         cmd_ref_id           = 0;
  ...
  gstk_cmd_from_card_type  *gstk_req_ptr)
{
     ...
     (!estk_shared_info.redial_in_progress))
  {
    ...
    estk_shared_info.redial_cmd_ptr = GSTK_CALLOC(sizeof(gstk_cmd_from_card_type));
    if(estk_shared_info.redial_cmd_ptr == NULL)
    {
      return ESTK_ERROR;
    }
    (void)memscpy(estk_shared_info.redial_cmd_ptr,
...
/*===========================================================================
FUNCTION: estk_map_cm_sys_mode_to_call_mode
...
/*===========================================================================
FUNCTION: estk_populate_call_info

DESCRIPTION:
  This function populates the call information from CM
...
===========================================================================*/
static void estk_populate_call_info(void)
{
  uint8                          i               = 0;
  cm_mm_call_info_s_type        *call_info_ptr   = NULL;
  
  call_info_ptr = (cm_mm_call_info_s_type *)GSTK_CALLOC(sizeof(cm_mm_call_info_s_type));
  ...
  for(i = 0; i < ESTK_CALL_ID_MAX; i++)
  {
    /* get info of each call id */
    if(cm_call_info_get_call_id(i, call_info_ptr))
    {
      if(call_info_ptr->call_type == CM_CALL_TYPE_VOICE)
      {
        if((call_info_ptr->call_state == CM_CALL_STATE_IDLE) ||
           (call_info_ptr->call_state == CM_CALL_STATE_NONE) ||
           (call_info_ptr->call_state == CM_CALL_STATE_MAX))
        {
          continue;
        }
        else
        {
          ...
          estk_shared_info.call_info.call_id = call_info_ptr->call_id;
          estk_shared_info.call_info.call_sys_mode = call_info_ptr->sys_mode;
          break;
        }
      }
    }
  }
  gstk_free(call_info_ptr);
  call_info_ptr = NULL;
} /* estk_populate_call_info */
