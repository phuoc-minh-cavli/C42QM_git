...
GENERAL DESCRIPTION:ENHANCED STK layer
  ...
  functionality
          ...
          Copyright © 2009 - 2016, 2018 QUALCOMM Technologies, Inc(QTI).
          All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/estk/src/estk.c#2 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
09/17/20   dt      Fixing offtarget TC failures on CT
10/12/18   ks      Split ME and slot specific NV
10/11/18   gs      KW Fix that dereference NULL Pointer
08/24/18   dt      Fix compilation errors
08/21/18   dt      9x05 interface changes for memory reduction
...
03/07/16   gs      estk_cm_init movement
06/15/15   vr      Remove TR delay logic for CLOSE CHANNEL
01/14/15   vr      Intialize call list info
...
void estk_register_proactive_cmds(void)
{
  ...
  if(!(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == TRUE))
  {
    bitmask = bitmask | GSTK_REG_SEND_USSD_EVT;
    bitmask = bitmask | GSTK_REG_SEND_SS_EVT;
  }
  if(!(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == TRUE))
  {
     bitmask = bitmask | GSTK_REG_SETUP_CALL_EVT;
     bitmask = bitmask | GSTK_REG_SEND_DTMF_EVT;
  }
  if(!(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_WMS_INTERFACE) == TRUE))
  {
...
void estk_gstk_init(uint8 gstk_num_of_instances)
{
  ...
  for (i = 0; ((i < estk_num_of_instances) && (i < GSTK_MAX_NUM_OF_INSTANCES)); ++ i)
  {
...
)
{
  ...
  {
    UIM_MSG_ERR_0("estk_gstk_evt_cb(): estk_curr_inst_ptr is NULL");
    return;
  }

  UIM_MSG_HIGH_1("estk_gstk_evt_cb(): estk_curr_inst_ptr command_id=0x%x",
  ...
  gstk_terminal_rsp_extra_param_type         *other_info_ptr)
{
  ...
  */
  ...
  if (terminal_response_type == GSTK_CLOSE_CH_CNF &&
      estk_curr_inst_ptr->keep_ch_open)
  {
    if (estk_curr_inst_ptr->ch_open_id >= ESTK_MAX_BIP_CH)
    {
      /*
        We cannot do clean up in this case as we don't know which channel is being reused.
        This case should not occur as both the keep_ch_open flag and ch_open_id are set
          at the same place.
        */
      UIM_MSG_ERR_0("Invalid ch_open_id during close CH (reuse)");
    }
    else if (estk_curr_inst_ptr->
          bip_channel_table[estk_curr_inst_ptr->ch_open_id].keep_ch_open_timer_p != NULL)
    {
      UIM_MSG_HIGH_0("Starting channel reuse timer");
      (void)rex_clr_timer(estk_curr_inst_ptr->
                            bip_channel_table[estk_curr_inst_ptr->ch_open_id].keep_ch_open_timer_p);
      (void)rex_set_timer(estk_curr_inst_ptr->
                          bip_channel_table[estk_curr_inst_ptr->ch_open_id].keep_ch_open_timer_p,
                          ESTK_BIP_KEEP_CH_OPEN_DURATION);
    }
    else
    {
      UIM_MSG_ERR_0("Sending CLOSE CH(reuse) TR but keep_ch_open_timer not allocated, "
                    "this case should not occur");
      (void)estk_bip_clean_up_ch_table(estk_curr_inst_ptr->ch_open_id);
  }
  }
...
static void estk_disable_tp_and_registration(void)
{
  ...
  for(i = 0; i < n; i++)
  {
    ...
    {
      /* Disable SS in TP either based on the NV setting or
         respective feature enabled */
      case GSTK_SEND_SS_SUPPORT:
        if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == TRUE) ||
           (gstk_check_ss_capability() == FALSE))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      /* Disable USSD in TP either based on the NV setting or
         respective feature enabled */
      case GSTK_SEND_USSD_SUPPORT:
        if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == TRUE) ||
           (gstk_check_ussd_capability() == FALSE))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      /* Disables Voice in TP either based on the NV setting or
         respective feature enabled */
      case GSTK_SET_UP_CALL_SUPPORT:
        if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == TRUE) ||
           (gstk_check_setup_call_capability() == FALSE))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      /* Disables DTMF in TP either based on the NV setting or
         respective feature enabled */
      case GSTK_SEND_DTMF_SUPPORT:
        if((gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == TRUE) ||
           (gstk_check_dtmf_capability() == FALSE))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      /* Disables SMS in TP based on the respective feature enabled*/
      case GSTK_CDMA_SEND_SMS_SUPPORT:
        if(!(modem_cap & UIM_MODEM_CAP_FEATURE_1X_MASK) ||
            (gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_ESTK_WMS_INTERFACE) == TRUE))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      ...
      case GSTK_UDP_SUPPORT:
        if((!cmd_enable) || (is_estk_bip_disable))
        {
          estk_profile_value_list[i].value = 0;
        }
        break;
      case GSTK_TCP_SERVER_MODE_SUPPORT:
        if((!cmd_enable) || (is_estk_bip_disable) ||
           (gstk_nv_get_proactive_feature_enable(GSTK_FEATURE_ENABLE_UICC_SERVER_MODE,&uicc_cmd_enable) == GSTK_SUCCESS && !uicc_cmd_enable) ||
           (estk_check_scws_capability() == FALSE))
        {
           estk_profile_value_list[i].value = 0;
        }
        break;
