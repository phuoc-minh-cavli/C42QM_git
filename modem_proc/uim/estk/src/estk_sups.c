...
Copyright (c) 2008 - 2014, 2016, 2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/3.3.1/estk/src/estk_sups.c#1 $
...
)
{
  ...
  /* Always have 1 byte of additional result */
  ...
  additional_result.additional_info_ptr = (uint8*)GSTK_CALLOC(sizeof(additional_result_data));
  (void)gstk_memcpy(additional_result.additional_info_ptr,
                 &additional_result_data,
                 additional_result.length,
                 additional_result.length,
  ...
  switch (estk_curr_inst_ptr->curr_cmd.hdr_cmd.command_id)
  {
    case GSTK_SEND_SS_REQ:
      if (TRUE == estk_curr_inst_ptr->extra_param.present)
      {
        ...
        extra_param_ptr->additional_info2_ptr.additional_info_ptr= additional_result.additional_info_ptr;
        if(estk_curr_inst_ptr->state == ESTK_WAIT_FOR_DISPLAY_ALPHA_CNF_ST)
        {
  ...
  {
    gstk_free(additional_result.additional_info_ptr);
