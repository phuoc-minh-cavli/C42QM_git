...
$Header: //components/rel/uim.mpss/3.3.1/simlock/src/simlock_category.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
...
#define SIMLOCK_CATEGORY_GET_CATEGORY(category)                                \
              (simlock_category_enum_type)(category & 0x0F)
...
)
{
  ...
  out_category_data_ptr->category_header.category_type    = SIMLOCK_CATEGORY_GET_CATEGORY(header_data.category_type_and_version);
  ...
  if(num_of_codes > 0)
  {
    ...
    {
      if(file_version == SIMLOCK_CATEGORY_FILE_VERSION_2)
      {
...
)
{
  ...
  if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_CP ||
  ...
  if(category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_CP ||
     category_data_ptr->category_header.category_type == SIMLOCK_CATEGORY_3GPP_SP)
  {
    header_data.category_type_and_version = SIMLOCK_CATEGORY_ADD_VERSION_INFO(category_data_ptr->category_header.category_type,
  ...
  switch(msg_type_ptr->category.category_type)
  {
    case SIMLOCK_CATEGORY_3GPP_NW:
      ...
      for(code_cnt = 0; code_cnt < msg_type_ptr->category.code_data.nw_3gpp_code_list.num_nw_codes;
          code_cnt++)
      {
        SIMLOCK_MSG_LOW_3("From req - mcc[0]: 0x%x, mcc[1]: 0x%x, mcc[2]: 0x%x",
        ...
        SIMLOCK_MSG_LOW_3("From req - mnc[0]: 0x%x, mnc[1]: 0x%x, mnc[2]: 0x%x",
    ...
    case SIMLOCK_CATEGORY_3GPP2_RUIM:
          ...
          code_cnt++)
      {
        ...
        SIMLOCK_MSG_LOW_4("From req - msin[0]: 0x%x, msin[1]: 0x%x, msin[2]: 0x%x, msin[3]: 0x%x",
        ...
        SIMLOCK_MSG_LOW_4("From req - msin[4]: 0x%x, msin[5]: 0x%x, msin[6]: 0x%x, msin[7]: 0x%x",
        ...
        SIMLOCK_MSG_LOW_2("From req - msin[8]: 0x%x, msin[9]: 0x%x",
