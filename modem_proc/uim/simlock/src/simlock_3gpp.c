...
$Header: //components/rel/uim.mpss/3.3.1/simlock/src/simlock_3gpp.c#1 $$ $DateTime: 2020/07/22 10:53:27 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/08/16   vdc     Extend full and expired service policies on any slot
09/06/16   vdc     Apply same config for VALID_CARD_MUST_ON_ANY_SLOT policy
...
)
{
  ...
  SIMLOCK_MSG_MED_0("simlock_3gpp_run_algorithm_sim");
  ...
  /* if autolock is enabled, update the lock data in the category
     file */
  if(category_data_ptr->category_header.auto_lock)
  {
    ...
    return (simlock_status == SIMLOCK_SUCCESS ? SIMLOCK_CODE_IN_WHITELIST : SIMLOCK_CODE_IN_NOLIST);
  }
...
)
{
  ...
  SIMLOCK_MSG_MED_0("simlock_3gpp_run_algorithm_cp");
  ...
  SIMLOCK_MSG_LOW_2("From SIM - GID1 Len: 0x%x, GID2 Len: 0x%x",
       ...
       code_index++)
  {
    ...
    SIMLOCK_MSG_LOW_2("From file - gid1 Len: 0x%x, gid2 Len: 0x%x",
...
)
{
  ...
  SIMLOCK_MSG_MED_0("simlock_3gpp_run_algorithm_sp");
  ...
  SIMLOCK_MSG_LOW_1("From SIM - gid1 Len: 0x%x", sim_code_data.gid1.gid_len);
       ...
       code_index++)
  {
    ...
    SIMLOCK_MSG_LOW_1("From file - gid1 Len: 0x%x", category_data_ptr->code_data_ptr[code_index].gid1.gid_len);
...
)
{
  ...
  SIMLOCK_MSG_MED_0("simlock_3gpp_run_algorithm_ns");
  ...
  SIMLOCK_MSG_LOW_2("From IMSI - d6: 0x%x, d7: 0x%x",
       ...
       code_index++)
  {
    ...
    SIMLOCK_MSG_LOW_2("From file - d6: 0x%x, d7: 0x%x",
                      category_data_ptr->code_data_ptr[code_index].msin.msin_data[0],
                      category_data_ptr->code_data_ptr[code_index].msin.msin_data[1]);
...
)
{
  ...
  SIMLOCK_MSG_MED_0("simlock_3gpp_run_algorithm_nw");
