...
$Header: //components/rel/uim.mpss/3.3.1/simlock/src/simlock_rsu_key.c#1 $$ $DateTime: 2020/07/22 10:53:27 $
...
)
{
  ...
  simlock_status = simlock_efs_name_test(simlock_efs_rsu_pubkey_file);
  if(SIMLOCK_SUCCESS == simlock_status) 
  {
    simlock_status = simlock_config_create_simlock_dir_if_needed();
    if(simlock_status == SIMLOCK_SUCCESS)
    {
      simlock_status = simlock_file_secure(simlock_efs_rsu_pubkey_file,
