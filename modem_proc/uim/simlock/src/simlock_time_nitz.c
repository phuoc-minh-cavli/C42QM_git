...
)
{
  ...
  if( SYS_SRV_STATUS_SRV != mm_ss_info_ptr->srv_status)
  {
    return;
  }

  if(SIMLOCK_TIME_3GPP_SYS_MODE(mm_ss_info_ptr->sys_mode))
  {
    cm_ss_evt_info.src_data.cm_time.sys = SIMLOCK_TIME_SYS_3GPP;

    if(mm_ss_info_ptr->mm_information.univ_time_and_time_zone_avail)
    {
      post_command = TRUE;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.is_time_present = 
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone_avail;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.year =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.year;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.month =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.month;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.day =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.day;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.hours =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.hour;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.minutes =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.minute;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.seconds =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.second;
      cm_ss_evt_info.src_data.cm_time.nitz_info.gw_nitz_info.time_zone =
        mm_ss_info_ptr->mm_information.univ_time_and_time_zone.time_zone;
    }
  }
  else
  {
    SIMLOCK_MSG_LOW_1("Unhandled sys mode 0x%x of CM SS event", mm_ss_info_ptr->sys_mode);
  }
