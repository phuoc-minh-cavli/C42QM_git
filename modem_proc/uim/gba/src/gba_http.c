...
$Header: //components/rel/uim.mpss/3.3.1/gba/src/gba_http.c#1 $ $DateTime: 2020/07/22 10:53:27 $ $Author: pwbldsvc $
...
#define GBA_HTTP_STATUS_500_SVR_ERR           500
...
)
{
  ...
  switch(gba_http_global_data_ptr->pending_cmd)
  {
    case GBA_HTTP_PENDING_401_UNAUTH:
      callback_data.cmd_type = GBA_HTTP_RESPONSE_UNAUTHORIZED_CMD;
      callback_data.data.auth.server_http_response = rsp_data.http_status;
      break;
    case GBA_HTTP_PENDING_200_OK:
      callback_data.cmd_type = GBA_HTTP_RESPONSE_FINAL_CMD;
      callback_data.data.http_200_ok.server_http_response = rsp_data.http_status;
      break;
    default:
      break;
  }
...
)
{
     ...
     gba_http_global_data_ptr->callback == NULL)
  {
    ...
    switch(rsp_data.event_type)
    {
       case GBA_HTTP_BLOCK_SSL_CERTIFICATE_ALERT:
         ...
         {
           callback_data.cmd_type = GBA_HTTP_RESPONSE_UNAUTHORIZED_CMD;
           callback_data.data.auth.server_http_response = GBA_HTTP_STATUS_500_SVR_ERR;
           (gba_http_global_data_ptr->callback)(GBA_SERVER_ERROR,
                                                &callback_data,
                                                gba_http_global_data_ptr->reference_id);
         }
  ...
  callback_data.cmd_type = GBA_HTTP_RESPONSE_UNAUTHORIZED_CMD;
  callback_data.data.auth.server_http_response = GBA_HTTP_STATUS_200OK;
