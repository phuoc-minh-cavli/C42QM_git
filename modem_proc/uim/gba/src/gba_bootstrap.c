...
/*===========================================================================
FUNCTION GBA_BOOTSTRAP_BUILD_AND_SEND_SERVER_ERROR_RESPONSE

DESCRIPTION
  Function builds the bootstrap GBA_SERVER_ERROR response including corresponding
  http error code to the client.
...
SIDE EFFECTS
  None
===========================================================================*/
static gba_result_enum_type gba_bootstrap_build_and_send_server_error_response (
...
)
{
  ...
  switch(gba_state)
  {
    case GBA_STATE_INITIAL_HTTP:
      bootstrap_resp_ptr->resp_data.server_error.error_code = cmd_data_ptr->data.auth.server_http_response;
      break;
    case GBA_STATE_FINAL_HTTP:
      bootstrap_resp_ptr->resp_data.server_error.error_code = cmd_data_ptr->data.http_200_ok.server_http_response;
      break;
    default:
  ...
  {
    gba_send_response(GBA_SERVER_ERROR, bootstrap_resp_ptr);
  ...
  return gba_status;
} /* gba_bootstrap_build_and_send_server_error_response */
...
)
{
  ...
  else if(gba_status == GBA_SERVER_ERROR)
  {
    gba_status = gba_bootstrap_build_and_send_server_error_response(&task_cmd_ptr->cmd_data);
  }
