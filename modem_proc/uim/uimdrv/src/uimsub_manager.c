...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uimsub_manager.c#1 $
...
12/12/17   ku      NULL check for uim_ptr
12/27/16   ks      Enabling subscription manager
12/27/16   ks      Added api to query current active slot
07/01/16   na      Remove memory free in subscription manager report callback
...
/*===========================================================================
FUNCTION   uim_sm_get_current_active_slot

DESCRIPTION
  If UIM CARD MANAGER is enabled this function returns the current active slot

PARAMETERS
 slot: logical slot ID. This should be UIM_SLOT_1
DEPENDENCIES
  None

RETURN VALUE
  slot: Current active slot ID is returned

SIDE EFFECTS
  NONE

===========================================================================*/
uim_slot_type uim_sm_get_current_active_slot
(
  uim_slot_type slot
)
{
        ...
        instance_counter++ )
  {
    if(uim_sm_data.uim_slot_info[instance_counter].activity_status ==
         UIM_SLOT_STATE_ACTIVE)
    {
      active_slot = (uim_slot_type)(instance_counter+1);
      break;
    }
  }
  return active_slot;
...
uim_rpt_status uim_sm_get_logical_slot_status_sync (
  ...
  uim_phy_slot_status_type   *curr_slot_status_ptr
)
{
#ifdef FEATURE_UIM_DS_SUBSCRIPTION_MANAGER
   uim_instance_global_type *uim_ptr          = NULL;
   ...
   if(curr_slot_status_ptr == NULL || num_phy_slots_ptr == NULL)
   {
   ...
   *num_phy_slots_ptr = UIM_NUM_PHY_SLOTS;
   ...
   uim_memscpy(curr_slot_status_ptr, sizeof(uim_sm_data.uim_slot_info),
               uim_sm_data.uim_slot_info, sizeof(uim_sm_data.uim_slot_info));
   rex_leave_crit_sect(&uim_sm_data.uim_sm_data_crit_sect);
   return UIM_PASS;
#else
   (void) num_phy_slots_ptr;
   (void) curr_slot_status_ptr;
   return UIM_FAIL;
#endif /* FEATURE_UIM_DS_SUBSCRIPTION_MANAGER */
...
)
{
      ...
      uim_ptr->command.cmd_ptr->hdr.command == UIM_RESET_F)
   {
     ...
     if (UIM_COMMON_EFS_SUCCESS == uim_common_efs_write(UIM_COMMON_EFS_UIMDRV_ACTIVE_SLOT,
                                                       ...
                                                       (char *)&(uim_active_slot_configuration),
