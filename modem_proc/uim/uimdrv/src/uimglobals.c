...
Copyright (c) 2013-2015, 2019 - 2020 by QUALCOMM Technologies, Inc.
All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uimglobals.c#2 $
...
08/22/16   ks      Polling alignment with eDRX
...
{
  if(id >= UIM_INSTANCE_INVALID || id < UIM_INSTANCE_1 || id >= UIM_MAX_INSTANCES )
  {
    return NULL;
  }
...
{
  ASSERT(id < UIM_INSTANCE_INVALID  || id < UIM_MAX_INSTANCES);
...
/* reset the voltage_class_known_from_atr varialbe */
  ...
  uim_ptr->edrx.ef_ad_value = UIM_EF_AD_NOT_READ;

  /* Re-initialize the value with the NV value */  
