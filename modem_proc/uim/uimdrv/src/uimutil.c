...
Copyright (c) 2003,2009-2018 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uimutil.c#1 $
...
01/09/18   gm      Align Poll timer with eDRX
08/28/17   ks      Fix KW error
11/25/16   gm      Storing edrx configuration and applying when data exchange is not happening
...
{
  byte   usim_aid[] = {0xA0, 0x00, 0x00, 0x00, 0x87, 0x10, 0x02};

  if (aid.aid_length < sizeof(usim_aid) || aid.aid_length > UIM_AID_MAX_NAME_LENGTH )
  {
    return FALSE;
  }

  if(0 == memcmp(aid.data,usim_aid,sizeof(usim_aid)))
  {
    return TRUE;
  }
  return FALSE;
...
{
  byte aram_aid[] = {0xA0, 0x00, 0x00, 0x01, 0x51, 0x41, 0x43, 0x4C, 0x00};

 if (aid.aid_length < sizeof(aram_aid) || aid.aid_length > UIM_AID_MAX_NAME_LENGTH )
  {
    return FALSE;
  }


  if(0 == memcmp(aid.data,aram_aid,sizeof(aram_aid)))
  {
    return TRUE;
  }
  return FALSE;
...
{
  uim_path_type ef_ad_path   = {0x3, {0x3F00,0x7FFF,0x6FAD}};

  boolean       is_ef_ad = FALSE;
  
  if(0 == memcmp(cmd_path_ptr->path, ef_ad_path.path, ef_ad_path.len * 2))
  {
    is_ef_ad = TRUE;
  }

  return (is_ef_ad);
} /* uim_is_read_for_ef_ad() */
...
{
#ifdef FEATURE_GSTK
  gstk_evt_dl_ind_type          *gstk_event_indication_ptr  = NULL;

  gstk_event_indication_ptr = uim_malloc(sizeof(gstk_evt_dl_ind_type));

  if(NULL == gstk_event_indication_ptr)
  {
    return;
  }  

  gstk_event_indication_ptr->evt_type = GSTK_POLL_INTERVAL_EVT_IND;

  /* If poll Interval is more than 255 minutes, dont send poll negotiation request */
  if(time_interval_in_ms > (UIM_MILLISECONDS_IN_A_MINUTE * 0xFF))
  {
    return;
  }

  if(time_interval_in_ms > (UIM_MILLISECONDS_IN_A_SECOND * 0xFF))
  {
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.units = 0;
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.length = (time_interval_in_ms / UIM_MILLISECONDS_IN_A_MINUTE);
  }
  else if(time_interval_in_ms > (UIM_MILLISECONDS_IN_ONE_TENTH_SEC * 0xFF))
  {
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.units = 1;
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.length = (time_interval_in_ms / UIM_MILLISECONDS_IN_A_SECOND);
  }
  else
  {
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.units = 2;
    gstk_event_indication_ptr->evt_ind.poll_interval_evt_ind.length = (time_interval_in_ms / UIM_MILLISECONDS_IN_ONE_TENTH_SEC);
  }
  
  gstk_send_envelope_evt_download_command(uim_ptr->uimdrv_client_id.uimdrv_gstk_client_id, 
                                          0,
                                          gstk_event_indication_ptr);

  UIM_FREE(gstk_event_indication_ptr);
#else
  (void)time_interval_in_ms;
  (void)uim_ptr;
#endif /* FEATURE_GSTK */
}/* uim_util_send_poll_interval_negotiation_req */
...
{
  static rex_timer_cnt_type     restore_poll_time_delay                             = 0;
  static rex_timer_cnt_type     restore_poll_time_offset                            = 0;
  errno_enum_type               e_status                                            = E_SUCCESS;
  static boolean                is_poll_interval_negotiation_req_need_to_send       = TRUE;
  static boolean                is_register_for_page                                = FALSE;
  static boolean                is_poll_need_to_send                                = TRUE;

  UIMDRV_MSG_HIGH_3(uim_ptr->id,"uim_util_set_proactive_poll_timer poll_change_req 0x%x is_poll_interval_negotiation_req_need_to_send 0x%x is_register_for_page 0x%x", 
                    poll_change_req, 
                    is_poll_interval_negotiation_req_need_to_send, 
                    is_register_for_page);                

  switch (poll_change_req)
  {
    case UIM_CHANGE_PROACTIVE_POLLING_INTERVAL_INTERNAL_REQ:
      uim_set_poll_timer( poll_interval_time,
                          wake_up_poll_offset,
                          uim_ptr );
      
      /* Return if presence detection polling is ON */
      if(((int)uim_ptr->state.ctl & UIM_TRAF_CHAN) &&
         (uim_ptr->poll.presence_detection_state != UIM_PD_POLLING_SUSPENDED))
      {
        return;
      }
      /* If edrx is enabled, then run eDRX algorithm 
       * If eDRX > poll_interval, Set the poll interval as edrx_cycle and send the poll 
         negotiation request with poll_interval as edrx_cycle to GSTK
       * If eDRX < poll_interval, Set the poll interval as poll_interval and send the poll 
         negotiation request with poll_interval as (edrx_cycle * n) to GSTK 
       */
      if(uim_ptr->edrx.cycle_length != 0)
      {
        is_poll_interval_negotiation_req_need_to_send = FALSE;
        is_poll_need_to_send = FALSE;
        if(uim_ptr->edrx.cycle_length >= poll_interval_time)
        {
          uim_util_set_proactive_poll_timer( uim_ptr->edrx.cycle_length,
                              UIM_DO_NOT_START_POLL_WAKE_UP_TIMER,
                              UIM_CHANGE_PROACTIVE_POLLING_INTERVAL_REQ_EDRX_GREATER_THAN_POLLING,
                              uim_ptr );
        }
        else
        {
          uim_util_set_proactive_poll_timer( uim_ptr->setting.poll_time_delay,
                              UIM_DO_NOT_START_POLL_WAKE_UP_TIMER,
    ...
    case UIM_CHANGE_PROACTIVE_POLLING_INTERVAL_REQ_EDRX_GREATER_THAN_POLLING:
      /* Store the  poll_timer value to reset when edrx is OFF */        
      restore_poll_time_delay  = uim_ptr->setting.poll_time_delay;
      restore_poll_time_offset = uim_ptr->setting.wakeup_poll_offset; 

      /* Set the poll interval with the eDRX cycle */
      uim_set_poll_timer( poll_interval_time,
                          wake_up_poll_offset,
                          uim_ptr );

      if(is_poll_need_to_send == TRUE)
      {
        /* Triggering poll when recieved edrx indication */
        (void) rex_set_sigs(uim_ptr->tcb_ptr, UIM_POLL_TIMER_SIG);
      }

      is_poll_need_to_send = TRUE;

      /* De-register for ML1 page if already registered */
      if(is_register_for_page)
      {
        e_status = msgr_deregister(MSGR_UIM_DRIVER,
                                   &uim_ptr->edrx.msgr_rrc_ml1_client,
                                   LTE_CPHY_EDRX_UIM_PAGE_IND) ;
        if (e_status != E_SUCCESS)
        {
          UIMDRV_MSG_ERR_0(uim_ptr->id,"msgr deregisteration failed");
        }
        else
        {
          is_register_for_page = FALSE;
        }
      }

      /* Send poll negotiation request if request came when eDRX kick-in 
          If request came from GSTK then dont send poll negotiation request */
      if(is_poll_interval_negotiation_req_need_to_send && 
         uim_nv_is_feature_enabled(UIMDRV_FEATURE_PROACTIVE_POLLING_NEGOTIATION_WITH_CARD, uim_ptr) &&
         uim_ptr->poll.is_proactive_polling_enabled)
      {
        uim_util_send_poll_interval_negotiation_req(poll_interval_time, uim_ptr);
      }
      else
      {
        is_poll_interval_negotiation_req_need_to_send = TRUE;      
      }
      break;

    case UIM_CHANGE_PROACTIVE_POLLING_INTERVAL_REQ_EDRX_LESS_THAN_POLLING:
      /* Store the  poll_timer value to reset when edrx is OFF */        
      restore_poll_time_delay  = uim_ptr->setting.poll_time_delay;
      restore_poll_time_offset = uim_ptr->setting.wakeup_poll_offset; 
      ...
      /* Dont register of ML1 page indication if POLL_INTERVAL is multiple of edrx cycle */
      if((((poll_interval_time + UIM_POLL_OFFSET) % uim_ptr->edrx.cycle_length) > 100) && 
         (uim_ptr->poll.is_proactive_polling_enabled))
      {
        ...
        if (e_status != E_SUCCESS)
        {
           UIMDRV_MSG_ERR_0(uim_ptr->id,"msgr registeration failed");
        }
        else
        {
          is_register_for_page = TRUE;
          UIMDRV_MSG_HIGH_0(uim_ptr->id,"Wait for the ML1 indications as eDRX value is less than the POLL timer");                
        }
        ...
        if(is_poll_interval_negotiation_req_need_to_send && 
           ...
           uim_ptr->poll.is_proactive_polling_enabled)
        {
          uint32 poll_interval_multiple_of_edrx = 0;
          poll_interval_multiple_of_edrx = (((poll_interval_time + UIM_POLL_OFFSET) / uim_ptr->edrx.cycle_length) +1) * uim_ptr->edrx.cycle_length;
          uim_util_send_poll_interval_negotiation_req(poll_interval_multiple_of_edrx, uim_ptr);
        }
        else
        {
          is_poll_interval_negotiation_req_need_to_send = TRUE;
      ...
      {
        /* de-register for ML1 page indication if already register */
        if(is_register_for_page)
        {
          e_status = msgr_deregister(MSGR_UIM_DRIVER,
                                     &uim_ptr->edrx.msgr_rrc_ml1_client,
    ...
    case UIM_CHANGE_PROACTIVE_POLLING_OFF_INTERNAL_REQ:      
    case UIM_CHANGE_PROACTIVE_POLLING_INTERVAL_REQ_EDRX_OFF:
      /* Re-store the poll interval if edrx is off */
      if(restore_poll_time_delay != 0)
      {        
        uim_set_poll_timer( restore_poll_time_delay,
                            restore_poll_time_offset,
                            uim_ptr);
      }
      restore_poll_time_delay = 0;
      restore_poll_time_offset = 0;
      is_poll_interval_negotiation_req_need_to_send = TRUE;
...
} /* uim_util_set_proactive_poll_timer() */