...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/enumeration/uimdrv_enumeration.c#1 $
...
#ifdef FEATURE_UIM_ENABLE_SLOT2 
  {DALCHIPINFO_FAMILY_MDM9x07,      NUM_UIM_SLOTS_AVAILABLE_ON_9x07,  FALSE, FALSE, TRUE, TRUE},
#else
  {DALCHIPINFO_FAMILY_MDM9x07,      NUM_UIM_SLOTS_AVAILABLE_ON_9x07,  FALSE, TRUE, TRUE, TRUE},
#endif /*  FEATURE_UIM_ENABLE_SLOT2 */
  {DALCHIPINFO_FAMILY_MDM9205,      NUM_UIM_SLOTS_AVAILABLE_ON_9x05,  FALSE, TRUE, TRUE, TRUE},
...
uim_hw_slot_info_type uimdrv_hw_nv_configuration(void)
{
  ...
  if (FALSE == uimdrv_hw_nv_config_info.efs_file_read_completed)
  {
    ...
    */
    ...
    if (TRUE == uim_hotswap_disabled_in_sw())
    {
      for (j=0;j < UIM_INSTANCES_CONFIGURABLE;j++)
      {
        m_HWConfig.uim_hotswap_flaky_switch[j] = FALSE;
        m_HWConfig.uim_slot_config[j].enableUimHotswap = FALSE;
      }
    }
    uim_set_uicc_usb_feature();
    uim_set_hotswap_flaky_switch_support();
