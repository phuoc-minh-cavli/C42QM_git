...
Copyright(c) 2001,2007 - 2020, 2022 by Qualcomm Technologies, Inc. All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uimgen.c#4 $
$DateTime: 2022/04/19 05:27:18 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/19/22   ykr     Avoid setting poll timer for POWER_DOWN command
09/07/20   ykr     Fixing overwriting of ICC protocol
07/28/20   rkg     Adding suspend resume feature
04/04/19   ng      Fix to allow only Telecomm CLA for Get Response in Passive mode
03/06/19   vgd     GET EID for M2M cards where LPA is not supported
03/14/19   ku      Reducing the recovery attempts for the remote SIM
03/05/19   nr      Extracting TA3 byte from ATR to get clock stop info
02/27/19   ng      Fixing stream APDU with select using file ID issue
10/12/18   ku      FR 49111: Reducing RCInit Timeline
04/06/18   ku      Initializing status as UIM_UNINITIALIZED_S
01/23/18   ku      Enabling Passive mode and remote client parallely
02/09/17   ks      Send 91 xx to client incase of stream envelope command
02/08/17   ks      Workaround for non spec complaint card's poll error
02/07/17   ks      Fix for non spec complaint card for poll error under 7F10
11/28/16   ku      UIMDRV KW errors fix
10/26/16   ks      Using appropriate global to check if AID is selected
...
06/24/16   ks      Avoid selection of 7FFF file on non-provisioning application
...
06/08/16   vdc     Workaround to select MF/ADF on open channels after a succesful
                   close of channel on which ARA-M app selection failed.
06/07/16   gm      Remove the test code of Simulate Null byte
06/06/16   ssr     F3 Optimization on curr path
05/24/16   dd      F3 Optimization
05/24/16   dd      Reduce number of ATR prints under 20 F3s/ms
...
05/17/16   lm      Fix compiler errors
05/16/16   lm      Fix compiler warnings
05/16/16   ks      Code optimisation to reduce text memory
...
03/08/16   na      Do not update current path for streaming apdu in ICC mode
03/08/16   na      Removal of UIM_CLEAR_CACHE_F handling
...
/*  UIM_SUSPEND_F,   Suspend UIM if inactivity timer expires */
static const uim_generic_state_type UIM_SUSPEND_STATES[] =
{ UIM_SUSPEND_ST,
  UIM_POWER_DOWN_ST,
  UIM_DONE_ST
};

/*UIM_RUSME_F, Resume UIM */
static const uim_generic_state_type UIM_RESUME_STATES[] = 
{UIM_POWER_UP_ST,
  UIM_RESET_ST,
  UIM_DELAY_AFTER_ATR_ST,
  UIM_PPS_ST,
  UIM_UPDATE_OP_PARAMS_ST,
#if defined( FEATURE_UIM_T_1_SUPPORT )
  UIM_IFS_NEG_ST,
#endif /* FEATURE_UIM_T_1_SUPPORT */
  UIM_TERMINAL_CAPABILITY_ST,
...
extern void uim_process_passive_mode_command
(
  uim_instance_global_type *uim_ptr
);
extern void uim_process_passive_mode_response
(
...
{
  if (cmd_ptr == NULL ||  uim_ptr->id >= UIM_MAX_INSTANCES)
  {
    return;
  }
  ...
  if (cmd_ptr->hdr.rpt_function != NULL)
  {
    ...
    switch(cmd_ptr->hdr.command)
    {
      ...
      case UIM_SILENT_RECOVER_UICC_F:
	   ...
	   case UIM_SUSPEND_F:
        {
          uim_ptr->command.rpt_buf.rpt_type = UIM_SUSPEND_R;
          ...
          /* Report to requesting task */
          (*(cmd_ptr->hdr.rpt_function))( &uim_ptr->command.rpt_buf );
        } /* end case - UIM_SUSPEND_F */
        break;

      case UIM_RESUME_F:
        {
          uim_ptr->command.rpt_buf.rpt_type = UIM_RESUME_R;
...
)
{
  ...
  {
    uim_process_passive_mode_command(uim_ptr);
  ...
  case UIM_HOTSWAP_CARD_REM_F:
    ...
    case UIM_STREAM_ISO7816_APDU_F:     /* Stream an APDU - extended version */
    {
      ...
      if(uim_ptr->command.cmd_ptr->stream_iso7816_apdu.cmd_data[UIM_7816_APDU_INSTRN_OFFSET] == SUSPEND_UICC)
      {
        UIMDRV_MSG_HIGH_0(uim_ptr->id, "Currently stream APDU is not supported for Suspend/Resume commands");
        status = FALSE;
        break;
      }
	...
	{
      if((uim_ptr->command.cmd_ptr->hdr.protocol != UIM_UICC)|| 
         (!(uim_ptr->setting.suspend_resume_enabled)))
         {          
           UIMDRV_MSG_ERR_2(uim_ptr->id, "Device/card doesn't support suspend - 0x%x OR protocol - 0x%x",
                            uim_ptr->setting.suspend_resume_enabled, uim_ptr->command.cmd_ptr->hdr.protocol);
           status = FALSE;
           break;
           }
      uim_ptr->command.generic_state_ptr = UIM_SUSPEND_STATES;
    }
      break;
    case UIM_RESUME_F:
      {
        uim_ptr->command.generic_state_ptr = UIM_RESUME_STATES;
      }
      break;
 ...
 if (uim_ptr->command.cmd_ptr == NULL || rsp_ptr == NULL)
  ...
  switch (uim_ptr->command.cmd_ptr->hdr.command)
  {
    ...
    case UIM_INTERNAL_WAKE_UP_F:        /* Power up due to Power management */
      ...
      break;
    case UIM_SUSPEND_F:                  /* command to suspend the card*/
	  {
        ...
        UIMDRV_MSG_HIGH_0(uim_ptr->id, "Received suspend response");
      }
      break;
    case UIM_RESUME_F:                  /* command to suspend the card*/
      {
        /* Report back the status as success */
        if (uim_ptr->command.cmd_ptr->hdr.rpt_function != NULL)
        {
        ...
        UIMDRV_MSG_HIGH_0(uim_ptr->id, "Received resume response");
      }
      break;
    case UIM_READ_REC_CMD_F:
    case UIM_READ_BIN_CMD_F:
  ...
  case UIM_RESET_SWITCH_UIM_PASSIVE_F:
    {
      uim_process_passive_mode_response(rsp_ptr, uim_ptr);
    }
    break;
  case UIM_HOTSWAP_CARD_REM_F:
  case UIM_POWER_DOWN_F:
...
)
{
  ...
  switch(*uim_ptr->command.generic_state_ptr)
  {
    ...
    case UIM_POWER_UP_ST:                     /* Power Up state */
      {
        ...
        if(nv_cmd_buf_ptr == NULL || uim_nv_data_buf_ptr == NULL)
        {
          if(nv_cmd_buf_ptr != NULL)
          {
            UIM_FREE(nv_cmd_buf_ptr);
          }
          else if(uim_nv_data_buf_ptr != NULL)
          {
            UIM_FREE(uim_nv_data_buf_ptr);
          }
          return;
        }
        uim_read_nv(uim_ptr, NV_UIM_FIRST_INST_CLASS_I, nv_cmd_buf_ptr, uim_nv_data_buf_ptr);

        /* Check NV read status */
        if (nv_cmd_buf_ptr->status != NV_DONE_S || 
         (!uim_is_gsm_supported() && !uim_is_cdma_supported()))
         {
        ...
        /* If cmd_ptr has protocol as ICC, power-up card in ICC protocol */
    ...
    case UIM_SELECT_ST:
     ...
     case UIM_READ_UMPC_ST:
      {
        if(UIM_INTERNAL_WAKE_UP_F == uim_ptr->command.cmd_ptr->hdr.command &&
          FALSE == uim_ptr->flag.is_rel_12_uicc)
        {
          UIMDRV_MSG_MED_0(uim_ptr->id, "Skipping read_umpc during recovery for non rel 12 card");
          uim_ptr->command.generic_state_ptr++;
          uim_generic_command (uim_ptr);
          break;
        }
        if (uim_ptr->command.cmd_ptr->hdr.protocol == UIM_UICC)
        {
          UIMDRV_MSG_MED_0(uim_ptr->id, "read EF-UMPC");
          uim_req_buf_static_ptr->apdu_hdr.instrn    = READ_BINARY;
          uim_req_buf_static_ptr->instrn_case        = UIM_INSTRN_CASE_2;
        ...
        {
          UIMDRV_MSG_MED_0(uim_ptr->id, "Skipping read EF-UMPC for ICC card");
          uim_ptr->command.generic_state_ptr++;
          uim_generic_command (uim_ptr);
        }
      } /* end case - UIM_READ_UMPC_ST */
      break;
     ...
     case UIM_TERMINAL_CAPABILITY_ST:
      {
        ...
        if((uim_ptr->command.cmd_ptr->hdr.command == UIM_RESUME_F) && 
           !(uim_ptr->suspend_resume.is_terminal_capability_sent))
        {
          UIMDRV_MSG_HIGH_0(uim_ptr->id, "Skipping sending terminal capability for resume command");
          ++uim_ptr->command.generic_state_ptr;
          uim_generic_command (uim_ptr);
          return;
        }
    ...
    case UIM_POLLING_INTERVAL_ST:
      {
        ...
        uim_ptr->command.rpt_buf.rpt.polling_interval.actual_uim_polling_interval = uim_ptr->setting.proactive_time_delay;
        (void)uim_power_control_ds((uim_slot_type)(uim_ptr->id + 1), UIM_NONE, TRUE, UIM_CALL_TYPE_NOT_APPLICABLE );
    ...
    case UIM_DONE_ST:                         /* Done State */
    {
      ...
      if((uim_ptr->setting.suspend_resume_enabled) && (q_check( & uim_ptr->task.cmd_q )== NULL)
         && (uim_ptr->suspend_resume.is_card_suspended !=TRUE))
      {
        timer_set( &uim_ptr->suspend_resume.uim_inactivity_timer,
                    UIM_MAX_CARD_INACTIVITY_TIME,
                    0, T_MSEC );
        
        timer_def(&uim_ptr->suspend_resume.uim_resume_timer, &uim_ptr->task.always_on_timer_group,
                 uim_ptr->tcb_ptr, 0,
                 (timer_t1_cb_type)uim_resume_timer_expiry_cb, (timer_cb_data_type)uim_ptr);
  ...
  case UIM_RECOVERY_VERIFY_UPIN_FOR_UICC_ST:
    ...
    case UIM_MANAGE_CHANNEL_ST:            /* Manage Channel State */
      ...
      case UIM_SUSPEND_ST:
        {
          uim_req_buf_static_ptr = &uim_ptr->command.req_buf;
          if(uim_req_buf_static_ptr == NULL)
          {
            return;
          }
          UIMDRV_MSG_HIGH_0(uim_ptr->id, "Sending suspend command to card");
          uim_req_buf_static_ptr->apdu_hdr.instrn     = SUSPEND_UICC;
          uim_req_buf_static_ptr->instrn_case         = UIM_INSTRN_CASE_4;
          uim_req_buf_static_ptr->apdu_hdr.p1         = 0x00;
          uim_req_buf_static_ptr->apdu_hdr.p2         = 0x00;
          uim_req_buf_static_ptr->apdu_hdr.p3         = 0x04;
          uim_req_buf_static_ptr->apdu_hdr.uim_class  = UIM_INSTRUCTION_CLASS2;
          

          uim_req_buf_static_ptr->req.data[0]         = (byte) uim_ptr->suspend_resume.suspend_time.t_unit_min;
          uim_req_buf_static_ptr->req.data[1]         = (byte) uim_ptr->suspend_resume.suspend_time.t_min;
          uim_req_buf_static_ptr->req.data[2]         = (byte) uim_ptr->suspend_resume.suspend_time.t_unit_max;
          uim_req_buf_static_ptr->req.data[3]         = (byte) uim_ptr->suspend_resume.suspend_time.t_max;
      ...
      case UIM_RESUME_ST:
        {
          if((uim_ptr->command.cmd_ptr->hdr.protocol != UIM_UICC)||
             (!(uim_ptr->suspend_resume.is_card_suspended)) || 
             (!(uim_ptr->setting.suspend_resume_enabled)))
          {
          UIMDRV_MSG_HIGH_0(uim_ptr->id, "Resume command failed");
            return;
          }
          uim_req_buf_static_ptr = &uim_ptr->command.req_buf;
          if(uim_req_buf_static_ptr == NULL)
          ...
          UIMDRV_MSG_HIGH_0(uim_ptr->id, "Sending resume command to card");
          uim_req_buf_static_ptr->apdu_hdr.instrn     = SUSPEND_UICC;
          uim_req_buf_static_ptr->instrn_case         = UIM_INSTRN_CASE_3;
          uim_req_buf_static_ptr->apdu_hdr.p1         = 0x01;
          uim_req_buf_static_ptr->apdu_hdr.p2         = 0x00;
          uim_req_buf_static_ptr->apdu_hdr.p3         = 0x08;
          uim_req_buf_static_ptr->apdu_hdr.uim_class  = UIM_INSTRUCTION_CLASS2;
          
          uim_memscpy(uim_req_buf_static_ptr->req.data,
                 sizeof(uim_req_buf_static_ptr->req.data),
                 &uim_ptr->suspend_resume.resume_token,
                 sizeof(uim_ptr->suspend_resume.resume_token)); 
          uim_req_buf_static_ptr->rsp_ptr = &uim_ptr->command.rsp_buf;
          uim_send_command(uim_req_buf_static_ptr, uim_ptr);
        }
      break;
...
)
{
  ...
  /* Holds the XI (clock stop indicator) value found in the first TA for T=15 */
  byte clock_stop_indicator;
 ...
 /*  Boolean variable to indicate the Bit 5 of the TA2 byte.
  ...
  /* Byte to store the clock stop bits extracted from either TA3 or TA4 byte */
  byte uim_ta3_ta4_clock_stop_bits = 0;
  byte mnc1 = 0, mnc2 = 0;
 ...
 /* Create a ptr to current channel to increase code readability */
  ...
  switch (curr_uim_generic_state)
  {
    case UIM_POWER_DOWN_ST:
      {
        ...
        if (uim_ptr->command.cmd_ptr->hdr.command == UIM_SUSPEND_F)
        {
          uim_ptr->suspend_resume.is_card_suspended = TRUE;
          UIMDRV_MSG_HIGH_0(uim_ptr->id,"Card is suspended");
          uim_set_resume_timer(uim_ptr);
         }
      }
      break;
    case UIM_POWER_UP_ST:                     /* Power Up state */
    ...
    case UIM_RESET_ST:                        /* Reset State */
      {
        ...
        for ( i = 0; i < rsp_ptr->cmd_rsp_size; i+=4)
        {
          ...
          UIMDRV_MSG_HIGH_3(uim_ptr->id, "atr byte[%d-%d]:%x",
                            i, i+j-1, tempATR);
        }
        ...
        for (i = 0;i < uim_ptr->atr.atr_buf.num_bytes;i++)
        {
          ...
          if ( voltage_class_indicator_scan == TRUE)
          {
            ...
            /* -- Clock stop indicator processing -- */
            clock_stop_indicator = rx_value >> UIM_TA3_TA4_CLOCK_STOP_FIELD_SHFT;

            UIMDRV_MSG_HIGH_1(uim_ptr->id, "clock_stop_indicator 0x%x", clock_stop_indicator);

            /* Check if the UIM clock can be stopped */
            if(clock_stop_indicator == UIM_TA3_TA4_CLOCK_STOP_NOT_SUPPORTED)
            {
              /* UIM clock stop not allowed */
              uim_ptr->state.clock_stop_cntrl = UIM_CLOCK_STOP_NOT_ALLOWED;
            }
            else if((clock_stop_indicator == UIM_TA3_TA4_CLOCK_STOP_LOW) ||
		    (clock_stop_indicator == UIM_TA3_TA4_CLOCK_STOP_NO_PREF))
            {
        ...
        uim_read_nv(uim_ptr, NV_UIM_PREF_PROTOCOL_I, nv_cmd_buf_ptr, uim_nv_data_buf_ptr);
...
#endif /* FEATURE_UIM_T_1_SUPPORT */
        {
              ...
              (UIM_TD_PRESENT))
          {
            ...
            UIMDRV_MSG_HIGH_1(uim_ptr->id, "uim_atr_td1_index 0x%x", uim_atr_td1_index);
          ...
          if (uim_ptr->atr.atr_buf.data[UIM_ATR_T0_BYTE] & (UIM_TD_PRESENT))
          {
            ...
            UIMDRV_MSG_HIGH_1(uim_ptr->id, "uim_atr_td_index 0x%x", uim_atr_td_index);
                                                              ...
                                                              (UIM_TD_PRESENT))
            {
              ...
              UIMDRV_MSG_HIGH_2(uim_ptr->id, "uim_atr_td_index 0x%x toggle_instrn_class 0x%x", uim_atr_td_index, uim_ptr->flag.toggle_instrn_class);
                  ...
                  (!uim_ptr->flag.toggle_instrn_class))
              {
                ...
                if (uim_ptr->atr.atr_buf.data[uim_atr_td_index] &
                     UIM_ATR_TA_PRESENT)
                {
                  /* Extract the clock stop bits from TA3 */
                  uim_ta3_ta4_clock_stop_bits =
                      (uim_ptr->atr.atr_buf.
                         data[++uim_proc_atr_byte_index])>>
                                 UIM_TA3_TA4_CLOCK_STOP_FIELD_SHFT;

                  UIMDRV_MSG_HIGH_1(uim_ptr->id, "uim_ta3_ta4_clock_stop_bits 0x%x", uim_ta3_ta4_clock_stop_bits);

                  /* Check if the UIM clock can be stopped */
                  if(uim_ta3_ta4_clock_stop_bits == UIM_TA3_TA4_CLOCK_STOP_NOT_SUPPORTED)
                  {
                    /* UIM clock stop not allowed */
                    uim_ptr->state.clock_stop_cntrl = UIM_CLOCK_STOP_NOT_ALLOWED;
                  }
                  else if((uim_ta3_ta4_clock_stop_bits == UIM_TA3_TA4_CLOCK_STOP_LOW) ||
                           (uim_ta3_ta4_clock_stop_bits == UIM_TA3_TA4_CLOCK_STOP_NO_PREF))
                  {
                    /* UIM clock stop at LOW */
                    uim_ptr->state.clock_stop_cntrl = UIM_CLOCK_STOP_LOW;
                  }
    ...
    case UIM_READ_UMPC_ST:
      {
        ...
        else
        {
          ...
          uim_ptr->setting.card_supports_suspend_resume                        =  (rsp_ptr->rsp.data[2] >> 1) & 1;
          UIMDRV_MSG_MED_4(uim_ptr->id, "ef_umpc - umpc: 0x%x, t_op: 0x%x, idle_current: 0x%x, is_suspend_resume_supported: 0x%x",
                           ...
                           uim_ptr->setting.card_supports_suspend_resume);
          ...
          uim_ptr->setting.suspend_resume_enabled             = (uim_ptr->setting.card_supports_suspend_resume 
                                                                 && uim_ptr->setting.device_supports_suspend_resume);
        ...
        if (uim_ptr->setting.suspend_resume_enabled)
        {
          timer_def(&uim_ptr->suspend_resume.uim_inactivity_timer, &uim_ptr->task.always_on_timer_group,
                     uim_ptr->tcb_ptr, 0,
                     (timer_t1_cb_type)uim_inactivity_timer_expiry_cb, (timer_cb_data_type)uim_ptr);
        }
		if (!uim_ptr->setting.is_terminal_capability_supported)
		{
		  uim_ptr->command.generic_state_ptr +=2;
		}
		else
    ...
    case UIM_TERMINAL_CAPABILITY_ST:
          ...
          uim_ptr->suspend_resume.is_terminal_capability_sent = TRUE;
          ++uim_ptr->command.generic_state_ptr;
      break;
    ...
    case UIM_CHECK_CHARACTERISTICS_ST:
      {
        ...
        if (rsp_ptr->cmd_status != UIM_DONE)
        {
          ...
          if(0x0000 == uim_check_characteristics_get_next_df(uim_ptr, uim_ptr->command.cmd_ptr->select.path.path[0]))
          {
            ...
            /* Skip EF-UMPC and  the terminal capability state and continue to select and read the ICCID */
            uim_ptr->command.generic_state_ptr+=3;
        ...
        else /* The select was success.  Check the characteristics. */
        {
          ...
          else if (uim_ptr->command.cmd_ptr->hdr.protocol == UIM_UICC)
          {
            ...
            {
              uim_ptr->setting.is_terminal_capability_supported = FALSE;
            }
            else 
            {
              uim_ptr->setting.is_terminal_capability_supported = TRUE;
            }
#ifdef FEATURE_UIM_USB_UICC
            if(!uim_usb_is_usb_interface_selected(uim_ptr))
#endif /* FEATURE_UIM_USB_UICC */
    ...
    case UIM_READ_BIN_MEM_ST:                  /* Read Binary State */
      {
        ...
        else
        {
          ...
          if (uim_ptr->command.rd_wr_info.num_bytes == 0)
          {
           ...
           if ((uim_ptr->command.cmd_ptr->hdr.command == UIM_READ_BIN_CMD_F)    &&
               (uim_is_read_for_ef_ad(&uim_ptr->command.cmd_ptr->read_bin.path))&&
               (uim_is_usim_aid(uim_ptr->state.current_path[curr_ch].curr_app)))
           {
              /* Check if EFAD is programmed to support polling alignment with eDRX */
              if((rsp_ptr->cmd_rsp_size >= 3) && 
                 (rsp_ptr->rsp.data[UIM_USIM_EFAD_EDRX_SUPPORT_OFFSET] & 0x08))
              {
                UIMDRV_MSG_HIGH_0(uim_ptr->id,"EF AD is programmed to support polling alignment");
                /* Disable return FCP for STATUS */
                uim_ptr->edrx.ef_ad_value = UIM_EF_AD_SUPPORTS_POLLING_ALIGNMENT;
                uim_ptr->edrx.allow_poll_alignment_with_edrx = TRUE;
              }
              else
              {
                UIMDRV_MSG_HIGH_0(uim_ptr->id,"EF AD is programmed not to support polling alignment");
                uim_ptr->edrx.ef_ad_value = UIM_EF_AD_DO_NOT_SUPPORT_POLLING_ALIGNMENT;
                uim_ptr->edrx.allow_poll_alignment_with_edrx = uim_nv_is_feature_enabled(UIMDRV_FEATURE_ALIGN_PROACTIVE_POLLING_WITH_EDRX,uim_ptr);
              } 
            }
            /* Skip to the next state. */
            ...
            if(uim_ptr->command.cmd_ptr->read_bin.sfi != UIM_INVALID_SFI)
            {
              UIMDRV_MSG_HIGH_0(uim_ptr->id,"UIM_READ_BIN_MEM_ST - SFI used successfully");
    ...
    case UIM_MANAGE_CHANNEL_ST:
            ...
            for(i=0; i<UIM_MAX_CHANNELS; i++)
            {
                   ...
                   (i != uim_ptr->state.aram_app.channel) )
               {
                 if(0 != uim_ptr->state.current_path[i].curr_app.aid_length)
                 {
                   uim_ptr->state.adf_selection.required  = TRUE;
                   uim_ptr->state.adf_selection.channel   = i;
                   UIMDRV_MSG_HIGH_1(uim_ptr->id,"ADF channel 0x%x",i);
                   uim_explicit_reselect_mf_aid(uim_ptr);
                 }
                 else
                 {
                   uim_ptr->state.mf_selection.required  = TRUE;
                   uim_ptr->state.mf_selection.channel   = i;
                   UIMDRV_MSG_HIGH_1(uim_ptr->id,"MF channel 0x%x",i);
                   uim_explicit_reselect_mf_aid(uim_ptr);
                 }  
              }
            }
      ...
      case UIM_SUSPEND_ST:
        {
          if ((rsp_ptr->cmd_status == UIM_APPLN_SPECIFIC_MIN_SUSP_TIME_TOO_LONG)
         ...
         {
           if (rsp_ptr->cmd_rsp_size == 0x0A)
           {
           uim_ptr->suspend_resume.t_unit_actual  =rsp_ptr->rsp.data[0];
           uim_ptr->suspend_resume.t_actual       =rsp_ptr->rsp.data[1];
           UIMDRV_MSG_HIGH_2(uim_ptr->id,"Actual resume timer:  time unit :%x, time :%x", 
                             rsp_ptr->rsp.data[0], rsp_ptr->rsp.data[1]);
           uim_memscpy(uim_ptr->suspend_resume.resume_token,
                       sizeof(uim_ptr->suspend_resume.resume_token),
                       rsp_ptr->rsp.data+2,
                       rsp_ptr->cmd_rsp_size-2*sizeof(byte));
         
                      /* copy the data into the report buffer  */
            uim_memscpy(uim_ptr->command.rpt_buf.rpt.suspend_data.token,
                        sizeof(uim_ptr->command.rpt_buf.rpt.suspend_data.token),
                        rsp_ptr->rsp.data+2,
                        rsp_ptr->cmd_rsp_size-2*sizeof(byte));
         
           ++uim_ptr->command.generic_state_ptr;
         
           UIMDRV_MSG_MED_8(uim_ptr->id, "Resume Token bytes: %x %x %x %x %x %x %x %x",
                            rsp_ptr->rsp.data[2], rsp_ptr->rsp.data[3], rsp_ptr->rsp.data[4],
                            rsp_ptr->rsp.data[5],rsp_ptr->rsp.data[6],rsp_ptr->rsp.data[7],
                            rsp_ptr->rsp.data[8],rsp_ptr->rsp.data[9]);
           }
           else
           {
            uim_ptr->command.generic_state_ptr +=2; 
            UIMDRV_MSG_HIGH_0(uim_ptr->id,"Suspend cmd rsp size is not 0x0A");
           }
      ...
      case UIM_RESUME_ST:
        {  
          if (rsp_ptr->cmd_status == UIM_DONE )
          {
            uim_ptr->suspend_resume.is_card_suspended   = FALSE;
            ++uim_ptr->command.generic_state_ptr;
          }
          if (rsp_ptr->cmd_status == UIM_SEC_STATUS_NOT_SATISFIED )
          {
            uim_ptr->suspend_resume.is_card_suspended   = FALSE;
            UIMDRV_MSG_HIGH_0(uim_ptr->id,"Invalid token, performing recovery");
            uim_force_recovery(uim_ptr);
          }
          if (rsp_ptr->cmd_status == UIM_USE_CONDNS_NOT_SATISFIED )
          {
            uim_ptr->suspend_resume.is_card_suspended   = FALSE;
            uim_force_recovery(uim_ptr);
          }
  ...
  (void)uim_ta3_ta4_clock_stop_bits;
  return(status);
...
)
{
...
#ifdef FEATURE_UIM_DS_SUBSCRIPTION_MANAGER
  if(cmd_ptr->hdr.rpt_function == uim_sm_rpt_function)
  {
    UIM_FREE(cmd_ptr->hdr.user_data);
  }
#endif  /* FEATURE_UIM_DS_SUBSCRIPTION_MANAGER */

  /* Change for SILENT RECOVER to recover the SWP (NFC) interface */
  if (cmd_ptr->hdr.command == UIM_SILENT_RECOVER_UICC_F)
  {
    /*Copying command buffer to report buffer to have it freed */
    uim_ptr->command.rpt_buf.user_data = uim_ptr->command.cmd_ptr->hdr.user_data;
    uim_silent_recovery_cb(&uim_ptr->command.rpt_buf);
  }
  if (cmd_ptr->hdr.cmd_hdr.done_q_ptr)
  {
  ...
  /* Frreing the toolkit command memory */
  UIM_FREE(cmd_ptr);
} /* uim_remove_toolkit_cmds */
