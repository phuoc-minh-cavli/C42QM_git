...
Copyright (c) 2001-2020, 2022 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uim.c#5 $
...
09/16/20   rkg     Start inactivity timer for cmd error after rpt
...
03/05/19   vm      Remove redundant error print message
02/20/19   ng      Delay local card power up in Modem SSR in BTSAP mode
01/30/19   ng      Handling BTSAP connection unavailable signal
10/25/18   ng      Added deep copy of INCREASE data sent by mmgsdi 
10/12/18   ku      FR49111: Reducing RCInit Timeline
03/16/18   ku      Removing unneccesary inclusions.
01/09/18   gm      Clear poll timer before setting poll timer
12/28/17   dt      Do not pet the dog after the task power down
12/27/16   ks      Added api to query current active slot
11/21/16   gm      Initiating presence detection polling only if data exchange is happening
10/14/16   gm      Added support for Poll interval negotiation
10/03/16   ks      Polling alignment with eDRX
08/29/16   na      Passing slot1 to gstk when subscription manage is enabled 
...
05/30/16   ks      Check for command transacted flag when the internal 
                   open channel command fails
05/24/16   dd      F3 Optimization
05/24/16   sam     Cleanup unused reocvery logic - uim_wait_filter_non_recovery_cmds
05/23/16   ssr     Remove msg for malloc failure
05/19/16   sam     Remove EFS logging feature
                   Remove unused legacy NVs
05/17/16   na      Removed the handling of command UIM_PREF_SLOT_CHANGE_F
05/17/16   lm      Fix compiler warnings
...
03/29/16   ks      Fixing issue with presence detection polling
03/09/16   vdc     Remove bcast related code
03/08/16   na      Removal of UIM_CLEAR_CACHE_F handling
03/07/16   ssr     Adding uimgsm,uimcdma stubs for IOE variant
...
extern boolean uim_check_cdma_df_select_before_tp (void);

#ifdef FEATURE_UIM_TEST_FRAMEWORK
#error code not present
#endif /*FEATURE_UIM_TEST_FRAMEWORK*/

extern boolean uim_check_explicit_select_df_cdma_before_telecom_access
(
  uim_instance_global_type     *uim_ptr
);

/*===========================================================================
FUNCTION uim_msgr_init

DESCRIPTION
  Register uim drivers as client of Message Router.
...
SEE ALSO
  None
===========================================================================*/
LOCAL static void uim_msgr_init
(
  uim_instance_global_type *uim_ptr
)

{
  ...
  uim_ptr->edrx.allow_poll_alignment_with_edrx = 
  	 uim_nv_is_feature_enabled(UIMDRV_FEATURE_ALIGN_PROACTIVE_POLLING_WITH_EDRX,uim_ptr);

  do {
    /* initialize msg queue */
    q_ptr = q_init(&uim_ptr->edrx.msgr_cmd_q);
    if (q_ptr == NULL)
    {
      e_status = E_FAILURE;
      break;
    ...
    e_status = msgr_client_create(
                 &uim_ptr->edrx.msgr_rrc_ml1_client);
    if (e_status != E_SUCCESS)
    {
      UIMDRV_MSG_ERR_0(uim_ptr->id,"UIM failure while creating client of the msgr");
      break;
    }
    ...
    e_status = msgr_client_set_rex_q(
                 &uim_ptr->edrx.msgr_rrc_ml1_client,
                 uim_ptr->tcb_ptr,
                 UIM_RRC_ML1_MSG_Q_SIG,
                 &uim_ptr->edrx.msgr_cmd_q,
                 MSGR_NO_QUEUE,
                 STD_OFFSETOF( uim_edrx_msg_type, edrx_msgr_cmd ),
                 MSGR_NO_OFFSET,
                 (uint16)sizeof(uim_edrx_msg_type));
    if (e_status != E_SUCCESS)
    {
      break;
    }

    e_status = msgr_register(MSGR_UIM_DRIVER,
                             &uim_ptr->edrx.msgr_rrc_ml1_client,
                             MSGR_ID_NONE,
                             LTE_RRC_EDRX_CFG_STATUS_IND) ;
    if (e_status != E_SUCCESS)
    {
      UIMDRV_MSG_ERR_0(uim_ptr->id,"UIM failure while registering with the msgr");
      break;
    }
  } while(0);
  ...
  if (e_status != E_SUCCESS)
  {
    UIM_MSG_ERR_0("uim_msgr_init failed");
  }
}/* uim_msgr_init */
...
#ifdef FEATURE_GSTK
/*===========================================================================
FUNCTION: uim_gstk_init_cb

DESCRIPTION:
  Callback function from GSTK providing UIMDRV with client id if successful.
...
)
{
  ...
  uim_ptr = uim_get_instance_ptr((uim_instance_enum_type) user_data);
  ...
  uim_ptr->uimdrv_client_id.uimdrv_gstk_client_id = client_id;
  UIM_LEAVE_CRIT_SECTION(&uim_ptr->crit_sect.clk_ctl, uim_ptr);  
}/* uim_gstk_init_cb */


/*===========================================================================
FUNCTION UIM_GSTK_INIT

DESCRIPTION
  Register uim drivers as client of GSTK
...
SEE ALSO
  None
===========================================================================*/
LOCAL static void uim_gstk_init
(
  uim_instance_global_type *uim_ptr
)
...
{
  gstk_status_enum_type             gstk_status            = GSTK_SUCCESS;
  gstk_slot_id_enum_type            gstk_slot_id           = GSTK_SLOT_AUTO;
  ...
  {
    case UIM_INSTANCE_1:
      gstk_slot_id = GSTK_SLOT_1;
      break;
      
    case UIM_INSTANCE_2:
      gstk_slot_id = GSTK_SLOT_2;
      break;
      
    case UIM_INSTANCE_3:
      gstk_slot_id = GSTK_SLOT_3;
      break;
  ...
  gstk_status = gstk_slot_client_init_ext(gstk_slot_id, 
                                          GSTK_UIMDRV_TYPE,
                                          uim_gstk_init_cb,
                                          (gstk_client_ref_data_type) uim_ptr->id);

  if (gstk_status != E_SUCCESS)
  {
    UIM_MSG_ERR_1("uim_gstk_init failed for slot : ox%x", uim_ptr->id+1);
  }
}/* uim_gstk_init */
#endif /* FEATURE_GSTK */
...
)
{
  ...
  if (uim_ptr->poll.uim_disable_polling == UIM_DISABLE_POLLING_UNKNOWN                    &&
      (UIM_COMMON_EFS_SUCCESS == uim_common_efs_read(UIM_COMMON_EFS_UIMDRV_DISABLE_POLLING,
                                                    ...
                                                    sizeof(disable_poll))))
  {
    ...
    {
      uim_ptr->poll.uim_disable_polling = UIM_DISABLE_POLLING_ON;
    }
    else
    {
      uim_ptr->poll.uim_disable_polling = UIM_DISABLE_POLLING_OFF;
    }
  }
  else if(uim_ptr->poll.uim_disable_polling == UIM_DISABLE_POLLING_UNKNOWN)
  {
  ...
  if ( uim_ptr->poll.uim_disable_polling == UIM_DISABLE_POLLING_ON )
  {
  ...
  {
    UIMDRV_MSG_ERR_0(uim_ptr->id,"Not setting poll timer as slot is in error state");
    return;
  }

  if ( uim_ptr->suspend_resume.is_card_suspended)
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "Clearing poll timer as Card is suspended");
  ...
  /* Do nothing if timer value is UIM_MAX_POLL_DURATION */
  ...
  else
  {
    ...
    if(rex_get_sigs(uim_ptr->tcb_ptr) & UIM_POLL_TIMER_SIG)
    {
       ...
       */
      if(wake_up_poll_offset != UIM_DO_NOT_START_POLL_WAKE_UP_TIMER)
      {
...
FUNCTION UIM_SET_RESUME_TIMER

DESCRIPTION
  This function should be used to set the time to trigger resume.
...
SIDE EFFECTS
  .Thisfunction is used to set resume timer. 

===========================================================================*/
void uim_set_resume_timer(
  uim_instance_global_type *uim_ptr
)
{
  timetick_type        time;
  ...
  UIMDRV_MSG_HIGH_2(uim_ptr->id,"Setting resume timer:  time unit :%d, time :%d", 
                    uim_ptr->suspend_resume.t_unit_actual, 
                    uim_ptr->suspend_resume.t_actual);

  switch(uim_ptr->suspend_resume.t_unit_actual)
  {
    case UIM_SECONDS:
    {
      time = uim_ptr->suspend_resume.t_actual * MILLISECONDS_PER_SECOND; 
    }
    break;
    case UIM_MINUTES:
    {
      time = uim_ptr->suspend_resume.t_actual * MILLISECONDS_PER_MINUTE; 
    }
    break;
    case UIM_HOURS:
    {
      time = uim_ptr->suspend_resume.t_actual * MILLISECONDS_PER_HOUR; 
    }
    break;
    case UIM_DAYS:
    {
      time = uim_ptr->suspend_resume.t_actual * MILLISECONDS_PER_DAY; 
    }
    break;
    case UIM_10_DAYS:
    {
      time = uim_ptr->suspend_resume.t_actual * MILLISECONDS_PER_10DAYS; 
    }
    break;
    default:
    {
      UIMDRV_MSG_HIGH_0(uim_ptr->id,"Invalid time unit");
      return;
    }
  }

  timer_set(&uim_ptr->suspend_resume.uim_resume_timer, time,
            0, T_MSEC );
}/* uim_set_resume_timer */
...
)
{
  ...
  /*resetting the procative polling flag*/
...
DESCRIPTION
  This UIM power control function is the voting interface for modules (MMGSDI/GSTK) to 
  indicate their pref as to whether they would like to keep the UIM powered ON or OFF.
  Also, this function sets the new polling timer value.

DEPENDENCIES
  Poll time delay to be used.

RETURN VALUE
  Returns new or existing poll time delay.
...
{
  uim_instance_global_type      *uim_ptr                   = NULL;
  dword                          new_uim_poll_time_delay   = 0;
  uim_slot_type                  active_slot               = slot;
  boolean                        uim_send_poll_signal      = FALSE;


#ifdef FEATURE_UIM_DS_SUBSCRIPTION_MANAGER
  active_slot = uim_sm_get_current_active_slot(slot);
#endif

  uim_ptr = uim_get_instance_ptr((uim_instance_enum_type)(active_slot - 1));

  if(uim_ptr == NULL)
  {
    UIM_MSG_ERR_1("uim_power_control_ds: Incorrect uim_slot 0x%x, returning default poll delay",
                  slot);
    return (UIM_POLL_TIME);
  }
  ...
  if (new_uim_poll_time_delay != uim_ptr->setting.poll_time_delay)
  {
    ...
    uim_util_set_proactive_poll_timer( new_uim_poll_time_delay,
  ...
  UIMDRV_MSG_MED_0(uim_ptr->id, "Setting State toggle signal");
  ...
  if ((int)uim_ptr->state.ctl & UIM_TRAF_CHAN )
  {
    ...
    {
      /* Set the poll timer signal either we have NON_PS_CALL is ongoing
          or PS_CALL is connected and data exchange is happening */
      if((uim_ptr->state.uim_call_info.call_mask & MMGSDI_MASK_NON_PS_CALL_ONGOING) ||
         ((uim_ptr->state.uim_call_info.call_mask & MMGSDI_MASK_PS_CALL_ONGOING) &&
          uim_is_ps_call_data_exchange_happening(uim_ptr)))
      {
        (void) rex_set_sigs(uim_ptr->tcb_ptr, UIM_POLL_TIMER_SIG);
        uim_send_poll_signal = TRUE;
      }
...
)
{
  ...
  /* Set the protocol information for slot */
  link_est_data.protocol = uim_ptr->state.dfs_1.protocol;
  ...
  /* This global will be update after EF-AD read command from mmgsdi */
  uim_ptr->edrx.allow_poll_alignment_with_edrx = 
      uim_nv_is_feature_enabled(UIMDRV_FEATURE_ALIGN_PROACTIVE_POLLING_WITH_EDRX,uim_ptr);
...
)
{
  ...
  if(status == UIM_CMD_FETCH &&
     !uim_sm_gstk_uim_fetch_allow((gstk_slot_id_enum_type)cmd_slot))
  {
  ...
  if ((status == UIM_ICCID_MISMATCH) || (status == UIM_VOLTAGE_MISMATCH))
  {
    ...
    *i_mask |= UIM_CMD_Q_SIG;
    ...
    if (uim_ptr->extended_recovery.extended_recovery_enabled == UIM_EXTENDED_RECOVERY_UNKNOWN)
    {
    ...
    if((uim_ptr->extended_recovery.extended_recovery_enabled == UIM_EXTENDED_RECOVERY_ON) &&
  ...
  else if (status == UIM_CMD_ERR)
  {
    ...
    /* In case of command error, UIM state machine will not go to UIM_DONE_ST.
       Initiacity timer will not start, so starting here   */
    if((uim_ptr->setting.suspend_resume_enabled) && (q_check( & uim_ptr->task.cmd_q )== NULL)
        && (uim_ptr->suspend_resume.is_card_suspended !=TRUE))
    {
      UIMDRV_MSG_HIGH_0(uim_ptr->id, "Inactivity timer started");
...
)
{
  ...
  do
  {
    ...
    if ((sigs_received & UIM_DOG_HB_RPT_SIG) && 
         uim_ptr != NULL && uim_ptr->task.dog_rpt_var != 0)
    {
...
void uim_start_initial_powerup(uim_instance_global_type *uim_ptr, rex_sigs_type *iMask_ptr)
{
  ...
  ** ----------------------------------------------------- */
       ...
       (UIM_CONNECTION_BTSAP == uim_ptr->state.last_connection_mode))
  {
    ...
    {
      UIMDRV_MSG_HIGH_0(uim_ptr->id, "Remote Connect request received, Powering up remote SIM");
  ...
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "UIM Power up in ISO Connection Mode");
    uim_btswap_save_current_mode_to_efs(uim_ptr,UIM_CONNECTION_ISO);
  }
#endif /* FEATURE_UIM_TEST_FRAMEWORK */
#endif /* FEATURE_UIM_BTSAP_CLIENT */
...
)
{
  ...
  /* Makes sure that dog will not be pet again after task stop */
  uim_ptr->task.dog_rpt_var = 0;
...
rex_sigs_type get_rex_sigs_mask(uim_instance_global_type *uim_ptr, rex_sigs_type i_mask)
{
  ...
  /* The command q sig is set in the mask and there is a command in
     the command queue, then set rex signals mask as command q sig */
  if (((i_mask & UIM_CMD_Q_SIG) != 0) &&
       (q_cnt(& uim_ptr->task.cmd_q) != 0))
  {
...
)
{
  ...
  ** ------------------------------------------------------------------ */
  ...
  uim_msgr_init(uim_ptr);

  /* Register with GSTK as a client */
  uim_gstk_init(uim_ptr);
...
)
{
  ...
  switch(src_cmd_ptr->hdr.command)
  {
    ...
    case UIM_INC_CMD_F:
	  ...
	  dest_cmd_ptr->inc.data = (byte*)dest_cmd_ptr + sizeof(uim_cmd_type);

      if((dest_cmd_size - sizeof(uim_cmd_type)) >= src_cmd_ptr->inc.len)
	  {
        uim_memscpy((void*)(dest_cmd_ptr->inc.data),
                    (dest_cmd_size - sizeof(uim_cmd_type)),
                    (void*)src_cmd_ptr->inc.data,
                    src_cmd_ptr->inc.len);
        return TRUE;
      }
	  else
      {
        UIMDRV_MSG_ERR_2(uim_ptr->id,
                         ...
                         (dest_cmd_size - sizeof(uim_cmd_type)), src_cmd_ptr->inc.len);
        return FALSE;
      }	
    ...
    case UIM_SUSPEND_F:
      uim_memscpy((void*)dest_cmd_ptr, dest_cmd_size, (void*)src_cmd_ptr,
                   sizeof(uim_cmd_type));
      if (src_cmd_ptr->suspend_cmd.t_unit_max != UIM_SECONDS 
          && src_cmd_ptr->suspend_cmd.t_max != 0x00
          && src_cmd_ptr->suspend_cmd.t_min != 0x00 )
      {
        uim_ptr->suspend_resume.suspend_time.t_unit_max  = src_cmd_ptr->suspend_cmd.t_unit_max;
        uim_ptr->suspend_resume.suspend_time.t_unit_min  = src_cmd_ptr->suspend_cmd.t_unit_min;
        uim_ptr->suspend_resume.suspend_time.t_max       = src_cmd_ptr->suspend_cmd.t_max;
        uim_ptr->suspend_resume.suspend_time.t_min       = src_cmd_ptr->suspend_cmd.t_min;
      }
      return TRUE;
    default:
...
)
{
  ...
  if ((cmd_ptr == NULL) || (cmd_ptr->hdr.channel >= UIM_MAX_CHANNELS) ||
  ...
  */
  ...
  uim_ptr = uim_get_instance_ptr(cmd_ptr->hdr.slot - 1);
  ...
  /*handle suspend on suspended card and resume on non suspended card*/
  if ((uim_ptr->suspend_resume.is_card_suspended) && 
      (uim_task_cmd_ptr->hdr.command == UIM_SUSPEND_F))
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "Suspend on suspended card");
    UIM_FREE(uim_task_cmd_ptr);
    return UIM_INCOMPATIBLE_STATE_ERR_S;
  }
  if ((!uim_ptr->suspend_resume.is_card_suspended) && 
      (uim_task_cmd_ptr->hdr.command == UIM_RESUME_F))
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "Resume on already resumed card");
    UIM_FREE(uim_task_cmd_ptr);
    return UIM_INCOMPATIBLE_STATE_ERR_S;
  }
  ...
  switch(evt)
  {
    case UIM_EXTENDED_RECOVERY_EVT_NV_INIT:
      ...
      if(0 != uim_ptr->extended_recovery.recovery_config.no_of_extended_powerups)
      {
        ...
        uim_ptr->extended_recovery.extended_recovery_enabled = UIM_EXTENDED_RECOVERY_ON;
      }
      else
      {
        uim_ptr->extended_recovery.extended_recovery_enabled = UIM_EXTENDED_RECOVERY_OFF;
      }
      break;
