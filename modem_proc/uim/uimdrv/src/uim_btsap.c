...
Copyright (c) 2013-2016, 201-19 by QUALCOMM Technologies, Inc. All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uim_btsap.c#1 $
...
01/30/19   ng      Adding report call bcak for remote/local cards power-up
01/30/19   ng      Handling BTSAP connection unavailable signal
01/30/19   ng      Clearing the uim_hotswap_notify_card_removed flag
                   while switching between remote card to local card
10/12/18   ku      FR 49111: Reducing RCInit Timeline
06/11/18   dd      Fix MOB compilation error
...
uint16 uim_btsap_get_work_waiting_time(uim_instance_global_type *uim_ptr)
{
  if(uim_ptr == NULL)
  {
    return UIM_BTSAP_TRANSACTION_TIME_DURATION;
  }
...
void uim_btsap_init (uim_instance_global_type *uim_ptr)
{
  ...
  if(uim_ptr == NULL || TRUE == m_HWConfig.uim_slot_config[uim_ptr->id].disableUim)
  {
    /* Slot is disabled or pointer is NULL */
    return;
  }
...
void uim_btsap_power_down_local_sim_cb( uim_rpt_type *report_ptr)
{
  ...
  if (uim_ptr == NULL ||((uim_cmd_ptr = (uim_cmd_type*)uim_malloc(sizeof(uim_cmd_type))) == NULL))
  {
    return;
  }
...
void uim_btsap_event_callback(const qmi_uim_remote_event_data_type *evt_ptr)
{
  ...
  memset(&uim_remote_cmd_rsp_timer_nv, 0x00, sizeof(uim_remote_cmd_rsp_timer_nv_type));
  ...
  switch(evt_ptr->evt)
  {
    case QMI_UIM_REMOTE_EVT_CONNECTION_AVAILABLE :
    {
      ...
      if (uim_ptr->remote.uim_remote_timer_read == FALSE  &&  
                                                        ...
                                                        sizeof(uim_remote_cmd_rsp_timer_nv))))
      {
        ...
        uim_ptr->remote.uim_remote_timer_read           = TRUE;
        UIMDRV_MSG_LOW_1(uim_ptr->id,"BTSAP:Successfully read the remote command response timer from NV, 0x%x ",
