...
Copyright (c) 1999 - 2016, 2018-2020 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uimdrv.c#3 $
...
11/14/19   rkg     setting maximum possible voltage to class C
03/14/19   nr      Workaround to avoid glitch on UIM lines
10/04/18   nr      9X05 BU changes
04/04/18   ku      No GET_RESPONSE for SEEK command and 62 82 status words
04/03/18   ku      No GET_RESPONSE for SEEK command
06/22/16   gm      Fix KW warning
06/15/16   ks      Avoid uart register access in force_Recovery when 
                   remote card is active
06/10/16   na      Removed F3 traces
06/03/16   sam     Remove unused function uim_print_uim_config
05/24/16   dd      F3 Optimization
05/16/16   sam     Stub out UIM supply voltage indication code for IOE flavour
05/17/16   na      Feature guarding USB UICC API
05/19/16   sam     Remove unused legacy NVs
05/13/16   ssr     Reduce the UIM_MAX_INSTANCES to 1 for single slot build
05/02/16   dd      Do not turn on CLK when remote SIM is connected
...
boolean data_drivestrength_modified[UIM_MAX_INSTANCES] = {FALSE};
boolean clk_drivestrength_modified[UIM_MAX_INSTANCES] = {FALSE};
boolean reset_drivestrength_modified[UIM_MAX_INSTANCES] = {FALSE};

void uim_update_gpio_drive_strength(uim_instance_global_type   *uim_ptr)
{
  DalDeviceHandle            *phChipInfo_ptr = NULL;
  ...
  if (DAL_DeviceAttach(DALDEVICEID_CHIPINFO, &phChipInfo_ptr) != DAL_SUCCESS)
  {
    UIMDRV_MSG_ERR_0(uim_ptr->id, "DAL_DeviceAttach failed in uim_update_gpio_drive_strength");
    return;
  }

  DalChipInfo_GetChipFamily(phChipInfo_ptr, &chipFamily);
  if(chipFamily != DALCHIPINFO_FAMILY_MDM9205)
  {
    return;
  }

  UIMDRV_MSG_HIGH_0(uim_ptr->id, "uim_update_gpio_drive_strength enter");

  if(m_HWConfig.uim_slot_config[uim_ptr->id].uimDataGpioDrvStrength < DS_10MA)
  {
    /* Increase Drive Strength to avoid glitch on UIM DATA GPIO */
    uim_hw_if.gpio[uim_ptr->id].m_DataGpioConfigId.eDriveStrength = (DALGpioDriveType)DS_10MA;

    data_drivestrength_modified[uim_ptr->id] = TRUE;
    
    /* get GPIO configs and protocal setting */
    res = DalTlmm_GetGpioIdSettings(uim_hw_if.gpio[uim_ptr->id].m_TlmmHandle_ptr,
                                    uim_hw_if.gpio[uim_ptr->id].m_DataGpioId,
                                    &GpioSettings);
    if(GpioSettings.nFunctionSelect == 0)
    {
      UIM_CONFIGURE_DATA_DEFAULT_GPIO(uim_ptr);
    }
    else
    {
      UIMDRV_MSG_ERR_2(uim_ptr->id, "Wrong Mode set for DataGpio 0x%x res 0x%x", GpioSettings.nFunctionSelect, res);
    }
  }

  if(m_HWConfig.uim_slot_config[uim_ptr->id].uimClkGpioDrvStrength < DS_10MA)
  {
    /* Increase Drive Strength to avoid glitch on UIM CLK GPIO */
    uim_hw_if.gpio[uim_ptr->id].m_ClkGpioConfigId.eDriveStrength = (DALGpioDriveType)DS_10MA;

    clk_drivestrength_modified[uim_ptr->id] = TRUE;
    
    /* get GPIO configs and protocal setting */
    res = DalTlmm_GetGpioIdSettings(uim_hw_if.gpio[uim_ptr->id].m_TlmmHandle_ptr,
                                    uim_hw_if.gpio[uim_ptr->id].m_ClkGpioId,
                                    &GpioSettings);
    if(GpioSettings.nFunctionSelect == 0)
    {
    ...
    {
      UIMDRV_MSG_ERR_2(uim_ptr->id, "Wrong Mode set for ClkGpio 0x%x res 0x%x", GpioSettings.nFunctionSelect, res);
    }
  }

  if(m_HWConfig.uim_slot_config[uim_ptr->id].uimResetGpioDrvStrength < DS_10MA)
  {
    /* Increase Drive Strength to avoid glitch on UIM RST GPIO */
    uim_hw_if.gpio[uim_ptr->id].m_ResetGpioConfigId.eDriveStrength = (DALGpioDriveType)DS_10MA;

    reset_drivestrength_modified[uim_ptr->id] = TRUE;

    /* get GPIO configs and protocal setting */
    res = DalTlmm_GetGpioIdSettings(uim_hw_if.gpio[uim_ptr->id].m_TlmmHandle_ptr,
                                    uim_hw_if.gpio[uim_ptr->id].m_ResetGpioId,
                                    &GpioSettings);
    if(GpioSettings.nFunctionSelect == 0)
    {
    ...
    {
      UIMDRV_MSG_ERR_2(uim_ptr->id, "Wrong Mode set for ResetGpio 0x%x res 0x%x", GpioSettings.nFunctionSelect, res);
    }
  }

  UIMDRV_MSG_HIGH_0(uim_ptr->id, "uim_update_gpio_drive_strength exit");
} /* uim_update_gpio_drive_strength */


void uim_revert_gpio_drive_strength(uim_instance_global_type   *uim_ptr)
{
  DalDeviceHandle            *phChipInfo_ptr  = NULL;
  DalChipInfoFamilyType       chipFamily      = DALCHIPINFO_FAMILY_UNKNOWN;
  DALResult                   res             = DAL_ERROR;
  ...
  if (DAL_DeviceAttach(DALDEVICEID_CHIPINFO, &phChipInfo_ptr) != DAL_SUCCESS)
  {
    UIMDRV_MSG_ERR_0(uim_ptr->id, "DAL_DeviceAttach failed in uim_revert_gpio_drive_strength");
    return;
  }
  ...
  UIMDRV_MSG_HIGH_0(uim_ptr->id, "uim_revert_gpio_drive_strength enter");

  if(data_drivestrength_modified[uim_ptr->id])
  {
    uim_hw_if.gpio[uim_ptr->id].m_DataGpioConfigId.eDriveStrength = (DALGpioDriveType)m_HWConfig.uim_slot_config[uim_ptr->id].uimDataGpioDrvStrength;

    data_drivestrength_modified[uim_ptr->id] = FALSE;

    /* get GPIO configs and protocal setting */
    res = DalTlmm_GetGpioIdSettings(uim_hw_if.gpio[uim_ptr->id].m_TlmmHandle_ptr,
                                    uim_hw_if.gpio[uim_ptr->id].m_DataGpioId,
                                    &GpioSettings);
    if(GpioSettings.nFunctionSelect == 1)
    {
  ...
  if(clk_drivestrength_modified[uim_ptr->id])
  {
    uim_hw_if.gpio[uim_ptr->id].m_ClkGpioConfigId.eDriveStrength = (DALGpioDriveType)m_HWConfig.uim_slot_config[uim_ptr->id].uimClkGpioDrvStrength;
    
    clk_drivestrength_modified[uim_ptr->id] = FALSE;
  ...
  if(reset_drivestrength_modified[uim_ptr->id])
  {
    uim_hw_if.gpio[uim_ptr->id].m_ResetGpioConfigId.eDriveStrength = (DALGpioDriveType)m_HWConfig.uim_slot_config[uim_ptr->id].uimResetGpioDrvStrength;
    reset_drivestrength_modified[uim_ptr->id] = FALSE;
  }

  UIMDRV_MSG_HIGH_0(uim_ptr->id, "uim_revert_gpio_drive_strength exit");
} /* uim_revert_gpio_drive_strength */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else
  {
	...
	/* 9X05 only supports only 1.8V class, hence making max possible voltage to 1.8V*/
    uim_ptr->state.current_voltage_class = UIM_VOLTAGE_CLASS_C;
    uim_ptr->state.max_possible_voltage_class = UIM_VOLTAGE_CLASS_C;
  }
    ...
    case UIM_VOLTAGE_CLASS_C:
      {
        ...
        /* Check and update drive strength to 10mA if the original DriveSterength
           is configured is below 10mA to avoid HW issue
           Glitch on UIM DATA/CLK/RESET Gpio's */
        uim_update_gpio_drive_strength(uim_ptr);
        UIMDRV_MSG_HIGH_6 ( uim_ptr->id, "Data_eDriveStrength 0x%x Clk_eDriveStrength 0x%x Reset_eDriveStrength 0x%x data_drivestrength_modified 0x%x clk_drivestrength_modified 0x%x reset_drivestrength_modified 0x%x",
                            uim_hw_if.gpio[uim_ptr->id].m_DataGpioConfigId.eDriveStrength,
                            uim_hw_if.gpio[uim_ptr->id].m_ClkGpioConfigId.eDriveStrength,
                            uim_hw_if.gpio[uim_ptr->id].m_ResetGpioConfigId.eDriveStrength,
                            data_drivestrength_modified[uim_ptr->id],
                            clk_drivestrength_modified[uim_ptr->id],
                            reset_drivestrength_modified[uim_ptr->id]);
...
*/
  ...
  /* Check and revert to original value if drive strength increased
     to avoid HW issue Glitch on UIM DATA/CLK/RESET Gpio's */
  uim_revert_gpio_drive_strength(uim_ptr);
...
static void uim_suspend_card_cb(uim_rpt_type *rpt)
  {
    /* Do nothing */
    (void)rpt;
   }/* uim_suspend_card_cb */
...
FUNCTION UIM_INACTIVITY_TIMER_EXPIRY_CB

DESCRIPTION
  This procedure is registered as a callback associated with the expiry of a
  timer. When the timer expires and the callback is called the UIM CMD Q
  signal is set to send suspend command to  the interface, if needed.
...
===========================================================================*/
void uim_inactivity_timer_expiry_cb (timer_cb_data_type param )
{
   uim_instance_global_type *uim_ptr = (uim_instance_global_type*)param;
   ...
   if(uim_ptr == NULL)
   {
     UIM_ERROR_MSG_0("NULL UIM PTR");
     return;
   }

   rex_clr_timer(&uim_ptr->suspend_resume.uim_inactivity_timer);
   if(uim_ptr->suspend_resume.is_card_suspended)
   {
     UIMDRV_MSG_HIGH_0(uim_ptr->id, "Card is already in suspended state no need to queue it again");
     return;
   }
   
   if ((uim_cmd_ptr = (uim_cmd_type*)uim_malloc(sizeof(uim_cmd_type)))== NULL)
   {
     UIMDRV_MSG_HIGH_0(uim_ptr->id, "Returning as memory is not allocated to uim_cmd_ptr");
     return;
   }
   ...
   if ( q_cnt( &uim_ptr->task.cmd_q ) == 0 )
   {
     ...
     ---------------------------------------------------------------------------*/
     uim_cmd_ptr->hdr.command                = UIM_SUSPEND_F;
     ...
     uim_cmd_ptr->hdr.rpt_function           = uim_suspend_card_cb;
     uim_cmd_ptr->hdr.user_data              = (uint32)uim_cmd_ptr;

     UIMDRV_MSG_HIGH_0(uim_ptr->id, "queue a suspend command to empty queue");
...
static void uim_resume_card_cb(uim_rpt_type *rpt)
  {
    /* Do nothing */
    (void)rpt;
   }/* uim_suspend_card_cb */
...
FUNCTION UIM_RESUME_TIMER_EXPIRY_CB

DESCRIPTION
  This procedure is registered as a callback associated with the expiry of a
  timer. When the timer expires and the callback is called the Resume 
  command is sent interface, if needed.
...
===========================================================================*/
void uim_resume_timer_expiry_cb (timer_cb_data_type param)
{
  uim_instance_global_type *uim_ptr = (uim_instance_global_type*)param;
  uim_cmd_type *uim_cmd_ptr = NULL;
  ...
  rex_clr_timer(&uim_ptr->suspend_resume.uim_resume_timer);
    
  if (!uim_ptr->suspend_resume.is_card_suspended)
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "Card is not in suspended state, can't process resume");
    return; 
  }
  if ((uim_cmd_ptr = (uim_cmd_type*)uim_malloc(sizeof(uim_cmd_type)))== NULL)
  {
  ...
  Populate UIM Cmd pointer data
  ---------------------------------------------------------------------------*/
  uim_cmd_ptr->hdr.command            = UIM_RESUME_F;
  uim_cmd_ptr->hdr.protocol           = uim_ptr->card_cmd.uimdrv_protocol;;
  uim_cmd_ptr->hdr.slot               = uim_ptr->id + 1 ;
  uim_cmd_ptr->hdr.cmd_hdr.task_ptr   = uim_ptr->tcb_ptr;
  uim_cmd_ptr->hdr.cmd_hdr.sigs       = 0;
  uim_cmd_ptr->hdr.options            = UIM_OPTION_ALWAYS_RPT;
  uim_cmd_ptr->hdr.cmd_hdr.done_q_ptr = NULL;
  uim_cmd_ptr->hdr.channel            = 0x00;
  
  uim_cmd_ptr->hdr.rpt_function       = uim_resume_card_cb;
  uim_cmd_ptr->hdr.user_data          = (uint32)uim_cmd_ptr;

  
  UIMDRV_MSG_HIGH_0(uim_ptr->id, "queue a resume command");
...
)
{
  ...
  if((SEEK == uim_ptr->debug.last_sent_apdu.instrn) &&
     ...
     (SW2_EOF_REACHED == sw2))
  {
    /* Don't allow GET RESPONSE, since some commercial
