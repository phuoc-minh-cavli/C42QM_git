...
Copyright (c) 2001-2018, 2020 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/uimdrv/src/uim_sigs.c#3 $
$DateTime: 2020/09/08 11:00:18 $
$Author: pwbldsvc $
...
01/25/19   ks      Not clearing the card parameters during UIM_POWER_UP_UIM_PASSIVE_F
...
04/06/17   gm      Return the buffer back to free queue 
04/04/17   ks      Re-start polling after cold reset 
11/25/16   gm      Storing edrx configuration and applying when data exchange is not happening
11/21/16   gm      Updated F3 message
10/14/16   gm      Added support for Poll interval negotiation
10/04/16   ks      Ignore eDRX indications during presence detection polling
10/03/16   ks      Polling alignment with eDRX
08/09/16   ks      Clearing timeout signal before processing response
...
05/26/16   ks      Do not clear voltage class upon receipt of RESET_F
...
05/02/16   dd      Remove check for channel ID of previous command when  
...
#endif /* FEATURE_UIM_BTSAP_CLIENT */
extern boolean  uim_passive_mode_is_not_allowed(uim_instance_global_type *uim_ptr);
extern void uim_handle_cmd_rsp_timeout_sig_in_passive_mode(rex_sigs_type *sig_mask_ptr, rex_sigs_type *imask_ptr, uim_instance_global_type *uim_ptr);
static boolean uim_handle_rrc_ml1_ind_sig(rex_sigs_type *sig_mask_ptr, rex_sigs_type *imask_ptr, uim_instance_global_type *uim_ptr);
...
static uim_sig_map_type uim_sig_map[] =
{
  ...
  {UIM_RRC_ML1_MSG_Q_SIG, uim_handle_rrc_ml1_ind_sig}
};
                                        ...
                                        uim_instance_global_type *uim_ptr)
{
  ...
  if((UIM_EXTENDED_RECOVERY_ON == uim_ptr->extended_recovery.extended_recovery_enabled) &&
                                    ...
                                    uim_instance_global_type *uim_ptr)
{
  ...
  if ((uim_ptr->command.cmd_ptr = (uim_cmd_type *)q_get( &uim_ptr->task.cmd_q )) != NULL)
  {
    ...
    if ((uim_ptr->suspend_resume.is_card_suspended) && 
        (uim_ptr->command.cmd_ptr->hdr.command  == UIM_SUSPEND_F))
    {
      UIMDRV_MSG_HIGH_0(uim_ptr->id,  "Suspend command is not possible ");
      return TRUE;
    }
    if ((uim_ptr->suspend_resume.is_card_suspended) && 
        (uim_ptr->command.cmd_ptr->hdr.command  != UIM_RESUME_F))
    {
      if ( q_cnt( &uim_ptr->task.cmd_q ) == 0 )
      {
      ...
      UIMDRV_MSG_HIGH_1(uim_ptr->id,  "Resuming card for the command, 0x%x ", uim_ptr->command.cmd_ptr->hdr.command);
      uim_resume_timer_expiry_cb((unsigned long) uim_ptr);
      return TRUE;
    }
    if (!(uim_ptr->suspend_resume.is_card_suspended) &&
         (uim_ptr->command.cmd_ptr->hdr.command  == UIM_RESUME_F))
    {
      UIMDRV_MSG_HIGH_0(uim_ptr->id,  "Resume command is not possible ");
      return TRUE;
    }
    ...
    if ( ( (uim_ptr->state.status == UIM_ERR_S) &&
           ...
           (TRUE ==  uim_passive_mode_is_not_allowed(uim_ptr))
           )
       )
    {
      if (uim_ptr->state.status == UIM_ERR_S)
    ...
    else
    {
      if  ( (uim_ptr->state.status == UIM_ERR_S) &&
           ...
           (uim_ptr->command.cmd_ptr->hdr.command  == UIM_RESET_F)))
      {
        if((UIM_EXTENDED_RECOVERY_ON == uim_ptr->extended_recovery.extended_recovery_enabled) && (TRUE == uim_ptr->extended_recovery.extended_recovery_in_progress))
        {
                                              ...
                                              uim_instance_global_type *uim_ptr)
{
  ...
  else
  {
    ...
    {
      uim_handle_cmd_rsp_timeout_sig_in_passive_mode( sig_mask_ptr, imask_ptr, uim_ptr);
    }
    ...
    else if (!uim_ptr->flag.static_buffer_used)
    {
      ...
      if (uim_ptr->command.cmd_ptr != NULL)
      {
        ...
        else
        {
          ...
          else if(uim_ptr->command.cmd_ptr->hdr.command == UIM_SUSPEND_F)
          {
            /*reset the command transacted flag as response 
            is not succesfully received from card*/
            uim_ptr->command.rpt_buf.cmd_transacted = FALSE;
            /*clear suspend globals */
            uim_ptr->suspend_resume.suspend_time.t_unit_max  = NULL;
            uim_ptr->suspend_resume.suspend_time.t_unit_min  = NULL;
            uim_ptr->suspend_resume.suspend_time.t_max       = NULL;
            uim_ptr->suspend_resume.suspend_time.t_min       = NULL;

            uim_send_err_rpt(uim_ptr->command.cmd_ptr, uim_ptr);
          }

          else if(uim_ptr->command.cmd_ptr->hdr.command == UIM_RESUME_F)
          {
            /*reset the command transacted flag as response 
            is not succesfully received from card*/
            uim_ptr->command.rpt_buf.cmd_transacted        = FALSE;
            ...
            uim_ptr->suspend_resume.t_unit_actual          = NULL;
            uim_ptr->suspend_resume.t_actual               = NULL;
            memset(&uim_ptr->suspend_resume.resume_token, 0x00, sizeof(uint8)*UIM_RESUME_TOKEN_LENGTH);
                                              ...
                                              uim_instance_global_type *uim_ptr)
{
  ...
  (void)mcfg_refresh_done(uim_ptr->state.nv_refresh_reference_id);
...
/**
*  This function handles the UIM_RRC_ML1_MSG_Q_SIG  signal.
*
*  @param sig_mask_ptr  Pointer to the OR's set of signals that UIM task needs tohandle.
                        Unused in this function.
*  @param imask_ptr     Pointer to the mask of signals that the uim task is interested
...
*          going back to the top of the main loop is desired.
*/
static boolean uim_handle_rrc_ml1_ind_sig(rex_sigs_type *sig_mask_ptr,
                                            rex_sigs_type *imask_ptr,
                                            uim_instance_global_type *uim_ptr)
{
  uim_edrx_msg_type            *msg_ptr	                = NULL;
  rex_timer_cnt_type            time_left_for_poll      = 0;
  errno_enum_type               e_status                = E_SUCCESS;
  
  (void)sig_mask_ptr;
  (void)imask_ptr;
  
  uim_ptr->debug.indications_received++;
  UIMDRV_MSG_HIGH_0(uim_ptr->id, "Received edrx indication signal");
  (void) rex_clr_sigs(uim_ptr->tcb_ptr, UIM_RRC_ML1_MSG_Q_SIG );

  if((uim_ptr->state.status == UIM_ERR_S) || 
     (uim_ptr->edrx.allow_poll_alignment_with_edrx == FALSE) || 
     uim_ptr->poll.uim_disable_polling == UIM_DISABLE_POLLING_ON)
  {
    UIMDRV_MSG_HIGH_0(uim_ptr->id, "Not handling the indication as UIM status or NV doesn't allow to do so");
    msg_ptr = (uim_edrx_msg_type *)q_get(&uim_ptr->edrx.msgr_cmd_q);
    if( msg_ptr != NULL)
    {
  ...
  while((msg_ptr = (uim_edrx_msg_type *)q_get(
                                        &uim_ptr->edrx.msgr_cmd_q)) != NULL)
  {
    switch (msg_ptr->edrx_msgr_cmd.msg_hdr.id)
    {
      case LTE_RRC_EDRX_CFG_STATUS_IND:
      {
        UIMDRV_MSG_HIGH_2(uim_ptr->id,"LTE_RRC_EDRX_CFG_STATUS_IND received. edrx_configured = 0x%x, edrx_cycle 0x%x",
					   msg_ptr->edrx_msgr_cmd.rrc_edrx_config_ind.edrx_configured,
					   msg_ptr->edrx_msgr_cmd.rrc_edrx_config_ind.edrx_cycle_length);

        /* Set POLL TIMER SIGNAL to send status command immediately */
        (void) rex_set_sigs(uim_ptr->tcb_ptr, UIM_POLL_TIMER_SIG);

        if (msg_ptr->edrx_msgr_cmd.rrc_edrx_config_ind.edrx_configured == FALSE)
        {
          uim_ptr->edrx.cycle_length = 0;

          /* The subsequent POLL commands will be sent with original time/offset values  */
        ...
        uim_ptr->edrx.configured   = msg_ptr->edrx_msgr_cmd.rrc_edrx_config_ind.edrx_configured;
        uim_ptr->edrx.cycle_length = msg_ptr->edrx_msgr_cmd.rrc_edrx_config_ind.edrx_cycle_length;

        if(uim_ptr->edrx.cycle_length >= uim_ptr->setting.poll_time_delay)
        {
      ...
      {
        time_left_for_poll =  rex_get_timer(&uim_ptr->poll.no_wake_up_poll_timer);
        UIMDRV_MSG_HIGH_3(uim_ptr->id,"LTE_CPHY_EDRX_UIM_PAGE_IND: time_left_for_poll 0x%x, time_to_next_pulse_ml1 = 0x%x, is_page_start = 0x%x",
                                           time_left_for_poll,
					   msg_ptr->edrx_msgr_cmd.ml1_page_info.next_page_start_time_ms,
					   msg_ptr->edrx_msgr_cmd.ml1_page_info.is_page_start);

        if(time_left_for_poll > msg_ptr->edrx_msgr_cmd.ml1_page_info.next_page_start_time_ms 
                               + UIM_POLLING_ALIGNMENT_SW_LATENCY)
        {
          UIMDRV_MSG_HIGH_0(uim_ptr->id,"Wait for the next indication from ML1");
        }
        else
        {
          UIMDRV_MSG_HIGH_0(uim_ptr->id,"Send POLL command now, setting the POLL signal");
          (void) rex_set_sigs(uim_ptr->tcb_ptr, UIM_POLL_TIMER_SIG); 
        }
      }
      break;
      ...
      default:
        UIMDRV_MSG_ERR_1(uim_ptr->id,"eDRX msg 0x%x is unhandled!",
                                      msg_ptr->edrx_msgr_cmd.msg_hdr.id);
    } /* switch (msg_ptr->edrx_msgr_cmd.msg_hdr.id) */
    /* Return the buffer back to free Q */
    msgr_rex_free_msg_buf(&(msg_ptr->link));
  }
  return FALSE;
}/* uim_handle_rrc_ml1_ind_sig */
