...
$Header: //components/rel/uim.mpss/3.3.1/pbm/src/pbmuim.h#1 $
...
typedef PACKED struct PACKED_POST
{
  ...
  /* Last TxId used to sent to WMS for WMS ready query */
  uint8	                              last_txid;
  uint8                               slot_id;  
  boolean                             isActive;
...
/*============================================================================
   STRUCTURE:      PBM_REFRESH_INFO

   DESCRIPTION:
     The data structure binds refresh informations and maintains file list
     that are currently being refreshed by the card.
=============================================================================*/
typedef PACKED struct PACKED_POST
{
...
/* Save refresh information in PBM for each slot or subscription (1X/GW) */
extern PBM_REFRESH_INFO pbm_refresh_info[PBM_SESSION_MAX];
...
pbm_return_type pbm_seek_unused_records(pbm_session_enum_type session_type,
                                               pbm_file_type file_id);
