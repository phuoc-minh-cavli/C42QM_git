...
Copyright (c) 2004 - 2017 QUALCOMM Technologies, Inc(QTI).
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/3.3.1/pbm/src/qmi_pbm_if.c#1 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
08/29/17    tq     Check email, grp, anr and sne count(KW error)
      ...
      for( j=0;j < PBM_MAX_USIM_FIELDS_IN_PB_SET; j++ )
      {
        switch( info->pbm_all_usim_capibilities[i][j].field_id )
        {
          case PBM_FIELD_EMAIL :
            if(email_cnt < QMI_PBM_MAX_FIELDS_IN_SAME_USIM_TYPE)          
            {
          ...
          case PBM_FIELD_ADDITIONAL_NUMBER_EXT :
            if(anr_cnt < QMI_PBM_MAX_FIELDS_IN_SAME_USIM_TYPE)
            {
          ...
          case PBM_FIELD_OTHERNAME :
            if(sne_cnt < QMI_PBM_MAX_FIELDS_IN_SAME_USIM_TYPE)
            {
          ...
          case PBM_FIELD_GRPNAME_ID :
            if(grp_cnt < QMI_PBM_MAX_FIELDS_IN_SAME_USIM_TYPE)
            {
        ...
        case PBM_FIELD_GRPNAME_ID:
          if(fields[i].num_items > 0)
          {
            ...
            else
            {
              ...
              UIM_MSG_HIGH_2("Group Caps:  Max Grps %d, Max GrpName length %d",
                                fields[i].num_items, type3_recs_info.text_len);
