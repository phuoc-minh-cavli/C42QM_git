...
$Header: //components/rel/uim.mpss/3.3.1/pbm/src/pbmcache.c#1 $
...
03/10/16    nr     UID out of bounds access when uid records are less than adn records
...
SIDE EFFECTS
===========================================================================*/
pbm_addr_cache_s_type *pbm_cache_record_get(
...
)
{
  ...
  PBM_CHECK_PTR2_RET(cache, cache->pb_cache_array, NULL);
...
pbm_return_type pbm_cache_init(void)
{
  ...
  pbm_ret = pbm_efs_init();
  pbm_init_info.pbm_efs_init_done = TRUE;
  PBM_CHECK_ERR(pbm_ret, "EFS init failed");
...
uint16 pbm_usim_get_first_available(PB_USIM_CACHE *usim_cache)
{
  ...
  PBM_CHECK_PTR2_RET(usim_cache, usim_cache->pb_cache_array, 0);
                                      ...
                                      uint16 location, boolean init)
{
  ...
  PBM_CHECK_PTR2_RET(usim_cache, usim_cache->pb_cache_array, PBM_ERROR);
  ...
  PBM_CHECK_PTR2_RET(pb_cache, record, FALSE);
   ...
   do
   {
      ...
      if (m_usim_cache && !pbm_gsm_modified(rec_id))
      {
        ...
        if (!usim_cache                   ||
            !usim_cache->pb_cache_array   ||
            !m_usim_cache->index_array    ||
            !m_usim_cache->index_array[i])
        {
               continue;
        }
    ...
    PBM_CHECK_PTR_BREAK(node, ret, FALSE);
      ...
      j = 0;
      while(((uint16*)record->sim_text)[j++] != 0XFFFF
             && field_lengths[i] <= PBM_MAX_TEXT_LENGTH)
      {
        field_lengths[i] += 2;
      }
   ...
   if (m_sync_cache && m_sync_cache->sync_present)
   {
      ...
      } else
      {
         ...
         if (cc == 0 || ((uid == 0) && generate_uid))
         {
            ...
            for (i = 1; i <= cache->num_of_records; i++)
            {
              if (cache->pb_cache_array != NULL && cache->pb_cache_array[i] && cache->pb_cache_array[i]->num_fields)
              {
...
pbm_return_type pbm_delete_hidden_contacts_from_adn_cache(pbm_session_enum_type session_type)
{
  ...
  PBM_CHECK_PTR3_RET(cache, cache->pb_cache_array, cache_hidden, PBM_ERROR);
...
pbm_return_type pbm_merge_hidden_contacts(pbm_session_enum_type session_type)
{
  ...
  PBM_CHECK_PTR3_RET(cache, cache_hidden, cache_hidden->pb_cache_array, PBM_ERROR);
...
SIDE EFFECTS
===========================================================================*/
void pbm_session_phonebook_init(
  pbm_session_enum_type session_type
)
{
  ...
  PBM_VALIDATE_SESSION_RET(session_type,VOID);
...

   {
     ...
     if (USE_USIM_LOCAL_SUPPORT(session_type))
     {
       ...
       {
         /* Failure */
         UIM_MSG_HIGH_1("Failed mmgsdi_session_is_service_available %d ",session_type);
       }
     }
     ...
     {
       UIM_MSG_HIGH_1("PBM queried for local phonebook %d",session_type);
       return ;
     }
   }
...
SIDE EFFECTS
===========================================================================*/
void pbm_handle_app_init_completed(
...
)
{
   ...
   {
     UIM_MSG_HIGH_3("PBM pbm_handle_app_init_completed q_id 0x%x pbm_session 0x%x curr_q[q_id] 0x%x",
