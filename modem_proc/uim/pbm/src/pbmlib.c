...
$Header: //components/rel/uim.mpss/3.3.1/pbm/src/pbmlib.c#1 $
...
pbm_return_type pbm_session_clear_phonebook(pbm_phonebook_type pb_id)
{
   ...
   if ((cache->status == PBM_STATUS_READY) && (cache->records_in_use > 0))
   {
      ...
      } else
      {
         ...
         {
            if (!cache->pb_cache_array || !cache->pb_cache_array[i])
               continue;
                                    ...
                                    int num_fields)
{
   ...
   pbm_field_s_type *ptyped_fields = (pbm_field_s_type *)data_buf;
                                 ...
                                 PBM_FIND_CB_FUNC proc_func)
{
   ...
   for (i = 1; i <= cache->num_of_records; i++)
   {
    ...
    if (cache->pb_cache_array != NULL && (record_ptr = cache->pb_cache_array[i]) != NULL)
      {
...
)
{
  ...
  UIM_MSG_HIGH_6("Checking num as emergency.  Len: 0x%x Num: 0x%x 0x%x 0x%x 0x%x 0x%x",
  ...
  *ecc_category_ptr = 0;
  ...
  if (PBM_SUCCESS == ret_val)
  {
    while (PBM_SUCCESS == pbm_session_enum_next_rec_id_ext(&rec_id, handle))
    {
      ...
      if (pbm_find_ecc_cat_in_rec(rec_id, &found_category,
                                  &found_field,&emergency_mode))
      {
        //we found a category
  ...
  UIM_MSG_HIGH_2("Emergency number status 0x%x cat 0x%x",is_emergency_num,*ecc_category_ptr);

  return is_emergency_num;
}/* pbm_session_emergency_number_cat */
