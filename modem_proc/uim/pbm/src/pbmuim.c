...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2002 - 2016, 2018 by QUALCOMM Technologies, Inc(QTI).
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/3.3.1/pbm/src/pbmuim.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/25/18   dd      Check file type before accessing cache for PB files
...
03/10/16   nr      Fix to ignore invalid tags present in EF-PBR record for non spec cards
      ...
      if (file_id != PBM_FILE_MAX_REQUESTS)
      {
         switch(pbm_file_id_to_cache_type(file_id))
         {
            case PBM_CACHE_TYPE_PB:
               pb_cache = (PB_CACHE *)pbm_file_id_to_cache(pbm_sess_type, file_id);
            ...
            case PBM_CACHE_TYPE_USIM:
               usim_cache = (PB_USIM_CACHE *)pbm_file_id_to_cache(pbm_sess_type, file_id);
               if (usim_cache)
               {
   ...
   * entries */
       ...
       i++)
   {
      ...
      switch (pbm_file_id_to_cache_type(file_id))
      {
         ...
         case PBM_CACHE_TYPE_PB:
            {
               PB_CACHE *pb_cache = (PB_CACHE *) cache;
...
)
{
  ...
  pbm_refresh_info[session_type].pbm_refresh_num_files = 0;
  ...
  memset(pbm_refresh_info[session_type].pbm_refresh_file_list,
                        0x00,sizeof(pbm_refresh_info[session_type].pbm_refresh_file_list));
...
void pbm_init_entries(pbm_session_enum_type session_type)
{
   ...
   */
   ...
   {
      UIM_MSG_HIGH_3("PBM starts init of PBM file index %d, File id %d session %d",
                   pbm_initialize_index[session_type],
                      pbm_initialize_list[pbm_initialize_index[session_type]],
                      session_type);
                         ...
                         const mmgsdi_cnf_type  *data_ptr)                         
{
   ...
   rex_enter_crit_sect(&pbm_crit_sect);
  if(cache->pb_cache_array != NULL && cache->pb_cache_array[l_delete_link_index] != NULL)
   {
     uint16 cnt;
...
boolean pbm_uim_delete_links(uint32 buffer_index, pbm_return_type *ret,uint16 adn_index)
{
   ...
   *ret   = PBM_ERROR;
   ...
   pb_id.device_type = PBM_ADN;
   cache = pbm_pb_id_to_cache(pb_id);
   PBM_CHECK_PTR2_RET(cache, cache->pb_cache_array, FALSE);
   /* for cache arrays */
   rex_enter_crit_sect(&pbm_crit_sect);
      ...
      switch (file_id[buffer->findex])
      {
         ...
         case PBM_FILE_UID:
            ...
            } else
            {
               ...
               if(pb_cache == NULL || pb_cache->pb_cache_array == NULL)
               { 
                 *ret = PBM_ERROR;
                 return FALSE;
               }
                                        ...
                                        PBM_WRITE_CB_FUNC proc_func)
{
   ...
   {
      usim_cache = m_usim_cache->usim_cache_array[i];
      if (!usim_cache || !usim_cache->pb_cache_array)
      {
         continue;
      }
...
)
{
  ...
  {
     UIM_MSG_ERR_1("invalid confirmation type 0x%x in pbm_service_info_cb",
                    data_type);
     return;
  }
  ...
  if (pbm_session_restart & PBM_SESSIONID_TO_SESSION_MASK(session_type))
  {
     UIM_MSG_HIGH_1("PBM restarting init of session_type 0x%x", session_type);
     pbm_restart_fileinfo_internal(session_type);
     return;
  }
  ...
  if (!(pbm_session_initializing & PBM_SESSIONID_TO_SESSION_MASK(session_type)))
  {
     UIM_MSG_HIGH_1("PBM stopping init of SIM 0x%x", session_type);
     return;
  }
  ...
  {
    /* Could not get file attributes or service is disabled, so this
        file will not be read */
    if(pbm_file_id_to_cache_type(file_id) == PBM_CACHE_TYPE_PB)
    {
      cache_ptr = pbm_file_id_to_cache(session_type, (pbm_file_type)file_id);
      if (cache_ptr != NULL && 
          cache_ptr->num_of_records==0)
      { 
         cache_ptr->status = PBM_STATUS_NOT_AVAILABLE;
      }
    }
    /* After we process the results, put the next request in. */
    pbm_make_fileinfo_request(session_type, (1 + (uint32) file_id));
