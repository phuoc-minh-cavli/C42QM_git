...
typedef enum
{
  PM_MEGA_XO_MANUAL_MODE_CMD,               // configure the controller to be operable in
                                            // MANUAL mode wherein the switching is done in
                                            // software using APIs provided in this file
  PM_MEGA_XO_AUTOMATIC_MODE_CMD,            // configure the controller to be operable in
                                            // AUTOMATIC mode wherein the switching is done
                                            // in hardware using TCXO_EN pin

  PM_MEGA_XO_BUFFER_ON_CMD,                 // enable TCXO buffer
  PM_MEGA_XO_BUFFER_OFF_CMD,                // disable TCXO buffer
  PM_MEGA_XO_A_PREBUFF_FOLLOW_D_PREBUFF_ON_CMD, // AOUT prebuff follow DOUT prebuff
  PM_MEGA_XO_A_PREBUFF_FOLLOW_D_PREBUFF_OFF_CMD // AOUT prebuff not follow DOUT prebuff
}pm_mega_xo_cmd_type;


pm_err_flag_type pm_mega_xo_set_cmd(int externalResourceIndex, pm_mega_xo_cmd_type cmd)
{
