...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/rpm.bf/2.1.3/core/bsp/rpm/build/RPM.py#2 $
...
def generate(env):
   ...
   if do_local_files:
      ...
      if env['MSM_ID'] == '9x07':
         ...
         env.Append(CPPDEFINES = 'MODEM_PROXY_VOTE_CX_NOM')
      if env['MSM_ID'] == '9x05':
         env.Append(CPPDEFINES = 'MDM9x05_IMAGE_LAYOUT')
         env.Append(CPPDEFINES = 'PMIC_S1_PC_PROXY_VOTE')
         env.Append(CPPDEFINES = 'MODEM_PROXY_VOTE_CX_NOM')
         env.Append(CPPDEFINES = 'MDM9x05_TARGET_CONFIG')
         env.Append(CPPDEFINES = 'MDM9x05_STUBS')
         env.Append(CPPDEFINES = 'NO_STACK_CHECK')
         env.Append(CPPDEFINES = 'MDM9x05_TARGET')
   ...
   if do_link:
      ...
      if env['MSM_ID'] in ['9x05']:
          sectools_signed_mbn = env.SectoolBuilder(
             ...
             soc_vers = "0x2013",
             config= '${BUILD_ROOT}/tools/build/scons/sectools/config/integration/secimage_eccv3.xml',		 
