...
$Header: //components/rel/rpm.bf/2.1.3/core/power/mpm/hal/source/MPM_V4/HALmpmPlatform.c#6 $
...
#define XO_SHUTDOWN_VOTE 	0
#define XO_WAKEUP_VOTE 		1
...
**
** ======================================================================== */
uint16_t CxoWarmupSclks;

void HAL_mpm_SetCXOWarmup (uint16_t warmupSclks)
{
  CxoWarmupSclks = warmupSclks;
...
#ifdef MPM_INTERNAL_VREF
  HWIO_OUTF(TCSR_LDO_OBIAS_CTRL, LDO_OBIAS_ON, 1);
  HWIO_OUTF(TCSR_LDO_IB_CONFIG, LDO_IB_CONFIG, 3);
  HWIO_OUTF(TCSR_LDO_VREF_CONFIG, LDO_VREF_CONFIG, 0x8);
#else
  // use external vref
...
void HAL_mpm_SWDone(uint64_t sleep_duration)
{
  ...
  //on 9x05, requirement is to turnoff BOB GPIO rbsc
  gpio_bob_toggle_low();
  message_ram_calc_xo_accumulation(XO_SHUTDOWN_VOTE);
  /* invoke sleep */
  ...
  //on 9x05, requirement is to turnon BOB GPIO after rbsc
  gpio_bob_toggle_high();
  message_ram_calc_xo_accumulation(XO_WAKEUP_VOTE);
...
uint32 HAL_mpm_GetDeepSleepExitLatency(void)
{
  ...
  delay += ((MAX(mpmPlatform.mpmCfg->mpmCfg.wakeupDelays.cxoDelay,CxoWarmupSclks) + HAL_mpm_GetTCXOLatency()) * 586);
