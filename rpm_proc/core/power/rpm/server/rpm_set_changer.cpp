    ...
    case CLEANUP:
      {
        ...
        if(from_sleep)
        {
          ...
          if(SPM_SLEEPING == rpm->ees[ee_].subsystem_status)
          {
			...
			//read back wakeup_reason and wakeup_ind to ensure msg_ram flush is done.
            (message_ram_master_stats(ee_))->wakeup_reason = 1;
			//make sure wakeup_ind is written into msg_ram locatoin.
			memory_barrier();
			uint32 wakeup_tmp = (message_ram_master_stats(ee_))->wakeup_reason;
            rpm->ees[ee_].stats->wakeup_ind = now;
			wakeup_ind_tmp = rpm->ees[ee_].stats->wakeup_ind;
			
			
            rpm_spm_trigger_wakeup_int(ee_);
            
			if((ee_ == RPM_EE_MPSS) && deadline_valid && old_deadline && (old_deadline > now) && (old_deadline - now) > 57692)
				early_wakeup_count++;
				//CORE_VERIFY(0);
            ...
            if(deadline_valid && old_deadline && (now > old_deadline) && ((now - old_deadline) > MAX_WAKEUP_DEADLINE_EXCEEDANCE))
            {
                ...
                {
                    ERR_FATAL("Deadline latency exceeded", (uint32_t)this, wakeup_ind_tmp, wakeup_tmp);
                    CORE_VERIFY(0);
                }
