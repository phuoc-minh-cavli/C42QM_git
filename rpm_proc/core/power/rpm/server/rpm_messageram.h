...
$Header: //components/rel/rpm.bf/2.1.3/core/power/rpm/server/rpm_messageram.h#3 $
$Date: 2019/01/28 $
...
typedef struct
{
  ...
  uint32_t reserved[6];   //Reduced to 2 as Glink RPM Fifos need 0x6F8 * 2 bytes memory. If reserved is needed more, can reduce glink fifo sizes
} rpm_master_stats_t;
...
typedef struct
{
  ...
  vmpm_data_t vmpm;
  rpm_xo_drift_compensation_t xo_drift;
  uint32_t    large_alloc_space[892];
...
void message_ram_update_xo_accumulation(void);
