...
$Header: //components/rel/rpm.bf/2.1.3/core/power/rpm/server/rpm_spm_handshake.cpp#4 $
$Date: 2019/07/15 $
...
inline void rpm_acknowledge_spm_handshakes(unsigned ee)
{
    ...
    if (rpm->ees[ee].pending_bringups)
    {
      ...
      // read back from msg_ram to make sure flush is happen
	  uint64 tmp = rpm->ees[ee].stats->bringup_ack;
	  rpm->ees[ee].stats->bringup_ack = tmp;
    }
...
static void rpm_spm_bringup_isr(void) __irq
{
    ...
    //read back bringup_req to ensue msg_ram flush is done.
    memory_barrier();
	uint64 tmp = ee_state->stats->bringup_req;
	ee_state->stats->bringup_req = tmp;
...
static void rpm_spm_shutdown_high_isr(void) __irq
{
    ...
    if (ee_state->num_active_cores == 0)
    {
	  ...
	  message_ram_update_modem_xo_drift(MODEM_SHUTDOWN);
    }
