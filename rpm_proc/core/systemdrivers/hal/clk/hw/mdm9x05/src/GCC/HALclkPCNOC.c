...
$Header: //components/rel/rpm.bf/2.1.3/core/systemdrivers/hal/clk/hw/mdm9x05/src/GCC/HALclkPCNOC.c#2 $
...
void HAL_clk_EnableULTAUDIOClocks( boolean bEnable )
{
  //turning off/on below clocks for ddr ready state when modem is down/up.
  if(bEnable == TRUE)
  {
    HWIO_OUTF( GCC_ULTAUDIO_AHBFABRIC_IXFABRIC_CBCR, CLK_ENABLE, bEnable);
    while(HWIO_INF(GCC_ULTAUDIO_AHBFABRIC_IXFABRIC_CBCR , CLK_OFF ) != 0);
    
    HWIO_OUTF( GCC_ULTAUDIO_PCNOC_SWAY_CBCR , CLK_ENABLE, bEnable);
    while(HWIO_INF(GCC_ULTAUDIO_PCNOC_SWAY_CBCR , CLK_OFF ) != 0);
  }
  else
  {
    HWIO_OUTF( GCC_ULTAUDIO_AHBFABRIC_IXFABRIC_CBCR, CLK_ENABLE, bEnable);
