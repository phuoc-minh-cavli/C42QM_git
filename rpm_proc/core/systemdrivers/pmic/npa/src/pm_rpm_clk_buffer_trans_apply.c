...
$Header: //components/rel/rpm.bf/2.1.3/core/systemdrivers/pmic/npa/src/pm_rpm_clk_buffer_trans_apply.c#2 $  
...
{ 
    ...
    if((info->id > clk_ptr->num_of_peripherals))
    {
        return;  
    }
...
int pm_rpm_clk_buffer_aggregate(rpm_application_info *info)
{
    ...
    if(cb_data->clkInfo[pmic_index][info->id].AccessAllowed == 0)
    {
    ...
    if(cb_data->clkInfo[pmic_index][info->id].AlwaysOn)
    {
