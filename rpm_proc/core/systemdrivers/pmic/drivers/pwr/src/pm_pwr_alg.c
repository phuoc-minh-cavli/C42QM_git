...
$Header: //components/rel/rpm.bf/2.1.3/core/systemdrivers/pmic/drivers/pwr/src/pm_pwr_alg.c#1 $
...
pm_err_flag_type pm_pwr_pull_down_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, pm_on_off_type on_off)
{
    ...
    else
    {
        ...
        if((pwr_data->pwr_specific_info[peripheral_index].is_periph_unified_reg  == TRUE) &&(!(pwr_data->pwr_specific_info[peripheral_index].pmic4_flag)))
        {
            reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->PMIC5_PD_CTL );
        }
        else
        {
...
pm_err_flag_type pm_pwr_pin_ctrl_mode_alg(pm_pwr_data_type *pwr_data, pm_comm_info_type *comm_ptr, uint8 peripheral_index, uint8 select_pin)
{
    ...
    if ((peripheral_index < pwr_data->num_of_peripherals) && (pwr_data->pwr_specific_info[peripheral_index].periph_type != 0))  
    {
        ...
        if (pwr_data->pwr_specific_info[peripheral_index].is_periph_unified_reg ==  TRUE) //PMIC5
        {
            reg = (pm_register_address_type)(pwr_data->pwr_specific_info[peripheral_index].periph_base_address + pwr_data->pwr_reg_table->FOLLOW_HWEN);
            mask = 0xF0;
	                                           ...
	                                           pm_on_off_type on_off , uint8 select_pin)
{
    ...
    if ((peripheral_index < pwr_data->num_of_peripherals) && (pwr_data->pwr_specific_info[peripheral_index].periph_type != 0))
    {
        ...
        else if((periph_type == PM_HW_MODULE_BOOST)  && (perph_revision <=2))
        {
                                                      ...
                                                      pm_volt_level_type req_volt_level, uint32 *calculated_vset, pm_volt_level_type *ceiling_uv)
{
    ...
    else
    {
        ...
        {
            // calculate VSET 
			calculated_volt = ((req_volt_level - vmin)/vStep);
			 if (pwr_data->pwr_specific_info[peripheral_index].is_periph_unified_reg == TRUE)
            {
                vset = calculated_volt;
            }
            else
            {
				vset =(pm_register_data_type)(calculated_volt);
			}
