...
typedef enum {
	...
	TUI_SECURE_INDICATOR_ERROR         = -5,  /**< Error in setting the secure indicator for the local application. */
...
/** @xreflabel{hdr:CustomSecTouchCmdCallback}
  ...
  @param[out]  final_status           Upon a failure during the callback, pointer
...
/** Defines a general buffer parameter.
  ...
  For instructions on how to create and reference a font file, see the
   <i>QSEE Trusted User Interface Customization Tool User Guide</i> @xhyperref{80-P3696-1,(80-P3696-1)}.
 */
typedef struct{
