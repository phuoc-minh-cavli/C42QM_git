...
/*===========================================================================
   Copyright (c) 2010-2017, 2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
...
/* Only allow TAs with the right privilege to access SECURE DISPLAY memory */
#define SHMB_CP_SECDISP          (1 << 4)
...
/**
 * Add reclaimed memory as additional heap spaces
 */
void tzbsp_add_heap_section(void);
...
/**
 * Returns the base address of the TZ heap.
 *
 * @return The address where the primary TZ heap region begins.
 */
uintptr_t tzbsp_get_heap_base(void);

/**
 * Returns the size of the TZ heap.
 *
 * @return The size of the primary TZ heap region.
 */
size_t tzbsp_get_heap_size(void);

/**
 * Tests whether all of the range [\c start, \c start + \c len]
 * is owned by VM. This is a convenience function to access
 * \c ACIsMemoryOwnedByVMs. \c NULL is a valid value for \c
 * start, because physical addressing is used.
 *
 * @param [in] start Start of the memory range, physical address,
 * included in the range.
 *
 * @param [in] len Length of the memory range in bytes.
 *
 * @param [in] vmMask Bit mask to represnt VMs
 *
 * @param [in] bExclusive Exclusive ownership state
 *
 * @return \c TRUE if the entire area is owned by VM.
 * \c FALSE if the area contains secure memory.
 */
boolean tzbsp_is_vm_range(uintnt start, uint64 len, uint64 vmMask, boolean bExclusive);

/**
 * Convert the virtual address from the MMU to the corresponding virtual address
 * on the SMMU Secure Context Bank.
 *
 * @param [in]  va_mmu Virtual address in the MMU.
 *
 * @param [out] p_va_smmu Pointer to the corresponding virtual address in the SMMU.
 *
 * @return \c E_SUCCESS on success.
           \c Negative otherwise.
 * */
int tzbsp_mmu_to_smmu(uintptr_t va_mmu, uintptr_t *p_va_smmu);
