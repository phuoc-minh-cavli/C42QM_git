...
*/
/*===========================================================================
  Copyright (c) 2011-2016 , 2018-2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
  ===========================================================================*/
...
extern int smplap_heap_debugging_test(void);
extern int tz_app_crypto_test(uint32_t src, uint32_t dst, uint32_t length);
extern int tz_app_crypto_func_test(void);
extern int tz_app_crypto_perf_test(void);
extern int smplap_sfs_basic_ops(uint32_t taData);
...
extern int run_core_test(void* cmd, uint32_t cmdlen, void* rsp, uint32_t rsplen, void* data, uint32_t len);
extern int run_iris_test(void* cmd, uint32_t cmdlen,
                        void* rsp, uint32_t rsplen);
extern int run_misc_test(void);
extern int run_securechannel_test(void* data, uint32_t len);
extern int tz_app_cmnlib_test(void);
extern int run_message_passing_test(void);
extern int run_buses_test(uint32_t I2cDeviceId, uint32_t slaveaddr);
extern int run_fuse_test(void);
extern int run_sw_fuse_test(void);
extern int run_stor_test(void);
extern int run_buses_spi_test(uint32_t spiDeviceId);
...
extern int run_sharedbuffer_test(void* data, uint32_t len);
...
extern int run_oem_buf_test(size_t requestedOemBufLen, bool requestedOemBufLenValid);
...
extern int run_persistent_obj_dep_tests(void);
extern int run_persistent_obj_func_tests(void);
extern int run_persistent_obj_perf_tests(void);
extern int run_fips_services_test(void);
                        ...
                        void* rsp, uint32_t rsplen)
{
   ...
   switch (cmd_id) {
         ...
         */
         {
            ...
            do {
               ...
               ptr = (unsigned char *)(((cmd_ptr->data) + (__4KB__)) & ~((__4KB__) - 1));
      ...
      case CLIENT_CMD120_RUN_DYNAMIC_GP_LISTENER_SIZE_TEST:
         if(smplap_sfs_basic_ops((uint32_t)cmd_ptr->data)){
            rsp_ptr->status = FAILED;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : SFS dynamic gp listener size Test");
            break;
         }
         rsp_ptr->status = SUCCESS;
         qsee_log(QSEE_LOG_MSG_ERROR, "****SUCCESS : SFS dynamic gp listener size Test");
         break;
      ...
      case CLIENT_CMD11_RUN_SECURECHANNEL_TEST :
         ...
         retval = run_securechannel_test((void*)cmd_ptr->data, cmd_ptr->len);
         if (retval)
         {
            rsp_ptr->status = FAILED;
      ...
      case CLIENT_CMD16_RUN_BUSES_TEST:
		 qsee_log(QSEE_LOG_MSG_DEBUG, "DeviceId: %u,  SlaveAddr:%u", (uint32_t)cmd_ptr->data, (uint32_t)cmd_ptr->data2);
		 
         retval = run_buses_test((uint32_t)cmd_ptr->data, (uint32_t)cmd_ptr->data2);
         if (retval) {
            rsp_ptr->status = FAILED;
			qsee_log(QSEE_LOG_MSG_ERROR, "DeviceId: %u,  SlaveAddr:%u", (uint32_t)cmd_ptr->data, (uint32_t)cmd_ptr->data2);
      ...
      case CLIENT_CMD17_RUN_STOR_TEST :
         ...
         if (retval == QSEE_STOR_RPMB_NOT_PROVISIONED_ERROR) {
           qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : Store Test QSEE_STOR_RPMB_NOT_PROVISIONED_ERROR");
           rsp_ptr->status =  FAIL_DEVICE_NOT_PROVISIONED_ERROR;
         } else if (retval) {
            rsp_ptr->status = FAILED;
      ...
      case CLIENT_CMD19_RUN_BUSES_SPI_TEST:
         retval = run_buses_spi_test((uint32_t)cmd_ptr->data);
         if (retval) {
            rsp_ptr->status = FAILED;
  ...
  case CLIENT_CMD56_RUN_SP_KEYMASTER_TEST:
    ...
    break;
      ...
      case CLIENT_CMD50_RUN_LISTENER_FRAMEWORK_TEST:
         ...
         list_test_req = ((uint32_t)cmd_ptr->data) * 100;
         qsee_log(QSEE_LOG_MSG_DEBUG, "cmd_ptr->data: %u,  list_test_req:%u", cmd_ptr->data, list_test_req);
         retval = qsee_request_service(0x100, &list_test_req, sizeof(uint32_t), &list_test_rsp, sizeof(uint32_t));
         if (retval) {
      ...
      {
         rsp_ptr->status = run_oem_buf_test(0, false);
      ...
      case CLIENT_CMD101_RUN_SHAREDBUFFER_TEST:
         ...
         retval =  run_sharedbuffer_test((void*)cmd_ptr->data, cmd_ptr->len);
         if (retval) {
            rsp_ptr->status = FAILED;
     ...
     case CLIENT_CMD60_RUN_HDCP_KEY_WRITE_TEST:
      ...
      case CLIENT_CMD58_RUN_SP_IRIS_TEST:
        ...
        break;
      case CLIENT_CMD115_RUN_PERSISTENT_OBJ_DEP_TESTS:
         qsee_log(QSEE_LOG_MSG_ERROR, "****Start : CLIENT_CMD115_RUN_GP_PERSISTENT_OBJ_DEP_TESTS");
         retval = run_persistent_obj_dep_tests();
         if (retval) {
            rsp_ptr->status = retval;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : GP_PERSISTENT_OBJ_DEP_TESTS, retval = %d", retval);
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED :GP_PERSISTENT_OBJ_DEP_TESTS");
         }
         break;
      case CLIENT_CMD116_RUN_PERSISTENT_OBJ_FUNC_TESTS:
         qsee_log(QSEE_LOG_MSG_ERROR, "****Start : CLIENT_CMD116_RUN_GP_PERSISTENT_OBJ_FUNC_TESTS");
         retval = run_persistent_obj_func_tests();
         if (retval) {
            rsp_ptr->status = retval;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : GP_PERSISTENT_OBJ_FUNC_TESTS, retval = %d", retval);
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED :GP_PERSISTENT_OBJ_FUNC_TESTS");
         }
         break;
      case CLIENT_CMD117_RUN_PERSISTENT_OBJ_PERF_TESTS:
         qsee_log(QSEE_LOG_MSG_ERROR, "****Start : CLIENT_CMD117_RUN_GP_PERSISTENT_OBJ_PERF_TESTS");
         retval = run_persistent_obj_perf_tests();
         if (retval) {
            rsp_ptr->status = retval;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : GP_PERSISTENT_OBJ_PERF_TESTS, retval = %d", retval);
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED :GP_PERSISTENT_OBJ_PERF_TESTS");
         }
         break;
      case CLIENT_CMD118_RUN_FIPS_SERVICES_TESTS:
         qsee_log(QSEE_LOG_MSG_ERROR, "****Start : CLIENT_CMD118_RUN_FIPS_SERVICES_TESTS");
         retval = run_fips_services_test();
         if (retval) {
            rsp_ptr->status = retval;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : RUN_FIPS_SERVICES_TESTS, retval = %d", retval);
         } else {
            rsp_ptr->status = SUCCESS;
            qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED :RUN_FIPS_SERVICES_TESTS");
         }
         break;

      case CLIENT_CMD119_RUN_OEM_BUF_CHIPSET_TEST:
      {
         rsp_ptr->status = run_oem_buf_test((size_t)cmd_ptr->data, true);
      ...
      case CLIENT_CMD40_RUN_TA_HEAP_DEBUGGING_TEST :

         if (smplap_heap_debugging_test()) {
            rsp_ptr->status = FAILED;
            qsee_log(QSEE_LOG_MSG_ERROR, "****FAILED : SAMPLE APP heap debugging test");
            break;
         }

         rsp_ptr->status = SUCCESS;
         qsee_log(QSEE_LOG_MSG_DEBUG, "****PASSED : SAMPLE APP heap debugging test");
         break;
...
{
   /*Adding a kernel service in shutdown sequence.*/
   unsigned long long test_time = qsee_get_uptime();
