...
uint8_t red_dot[DOT_SIZE*BYTES_PER_PIXEL];
uint8_t green_dot[DOT_SIZE*BYTES_PER_PIXEL];
uint8_t image_dot[DOT_SIZE*BYTES_PER_PIXEL];
...
int run_touch_test( void )
{
  ...
  {
    ( ( uint32_t * ) red_dot   ) [ i ] = RED_PIXEL;
    ( ( uint32_t * ) green_dot ) [ i ] = GREEN_PIXEL;
  }
