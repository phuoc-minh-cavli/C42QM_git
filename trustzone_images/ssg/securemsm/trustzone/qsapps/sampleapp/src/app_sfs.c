/*
  @file app_sfs.c
  @brief This file is a sample code that show how QTEE Filesystem and SFS APIs can be used.
...
static int sfs_clean_existing_files(void)
{
  ...
  qsee_log(QSEE_LOG_MSG_DEBUG, "-- Start %s TESTS ---------------------------------", __func__);
...
//basic sfs operations to test dynamic GP Listener size
int smplap_sfs_basic_ops(uint32_t taData)
{
  // sfs operation size should be greater than listener size registered from
  // cmd_id CLIENT_CMD120_RUN_DYNAMIC_GP_LISTENER_SIZE_TEST to test listener size
  // change detection and re-request for read/write
  size_t len = GP_LISTENER_BUF_SIZE;
  size_t bufLen = 0;
  int i = 0, ret = 0, fd = 0;
  void* writeBuf = NULL;
  void* readBuf = NULL;

  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: TEST started --", __func__);
  ...
  if (NULL == (readBuf = qsee_malloc(bufLen))) {
    GPLOG_ERROR(bufLen);
    ret = FAILED;
    goto Exit;
  }

  if (NULL == (writeBuf = qsee_malloc(bufLen))) {
    GPLOG_ERROR(bufLen);
    ret = FAILED;
    goto Exit;
  }

  memset(writeBuf, 0x77, bufLen);
  ...
  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: SFS_OPEN SUCCESS --", __func__);
  if (len != (ret = qsee_sfs_write(fd, (char *)writeBuf, len))) {
    GPLOG_ERROR(ret);
    ret = FAILED;
    goto Exit;
  }

  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s:  SFS_WRITE SUCCESS --", __func__);
  if (0 != (ret = qsee_sfs_seek(fd, 0, SEEK_SET))) {
    GPLOG_ERROR(ret);
    ret = FAILED;
    goto Exit;
  }
  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: SFS_SEEK SUCCESS --", __func__);

  if (bufLen!= (ret = qsee_sfs_read(fd, (char *)readBuf, bufLen))) {
    GPLOG_ERROR(ret);
    ret = FAILED;
    goto Exit;
  }
  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: SFS_READ SUCCESS --", __func__);

  //Compare readBuf and writeBuf
  ret = memcmp(readBuf, writeBuf, bufLen);
  if (ret) {
    qsee_log(QSEE_LOG_MSG_ERROR, "%s:data mismatch ret:%d ", __func__, ret);
    ret = FAILED;
    goto Exit;
  }
  qsee_log(QSEE_LOG_MSG_DEBUG, "%s: data read PASSED", __func__);

  if (SUCCESS != (ret = qsee_sfs_close(fd))) {
    GPLOG_ERROR(ret);
    ret = FAILED;
    goto Exit;
  } else {
  ...
  qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: SFS_CLOSE SUCCESS --", __func__);
...
Exit:
  ...
  if (ret != SUCCESS) {
    qsee_log(QSEE_LOG_MSG_ERROR, "-- %s: TEST FAILED --", __func__);
    qsee_sfs_rm(FILE_NAME);
  }
  return  ret;
}
...
int smplap_sfs_unit_open(void)
{
  ...
  if(SUCCESS != (ret = sfs_clean_existing_files()))
  {
    GPLOG_ERROR(ret);
    goto Exit;
  }
  ...
  /* Clean Up File */
  if (SUCCESS != (ret = qsee_sfs_rm(FILE_NAME)))
  {
    GPLOG_ERROR(ret);
    ret = -1;
    goto Exit;
...
int smplap_sfs_unit_read_write(void)
{
   ...
   if (0 <= (ret = qsee_sfs_write(fd, (char *)buffer, -10)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   }
   if (0 <= (ret = qsee_sfs_read(fd, (char *)buffer, -10)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   ...
   for (i = 1; i < 10; i++)
   {
      ...
      while (total < FILE_SIZE)
      {
         ...
         if (writeSize != (ret = qsee_sfs_write(fd, (char *)data, writeSize)))
         {
            GPLOG_ERROR(ret);
            ret = -1;
            goto Exit;
      ...
      while (total < FILE_SIZE)
      {
         ...
         if (readSize != (ret = qsee_sfs_read(fd, (char *)data, readSize)))
         {
            GPLOG_ERROR(ret);
            ret = -1;
            goto Exit;
...
int smplap_sfs_unit_get_size(void)
{
   ...
   if (SUCCESS == (ret = qsee_sfs_getSize(0, &size)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   ...
   if (SUCCESS == (ret = qsee_sfs_getSize(fd, NULL)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   ...
   for (i = GET_SIZE_WRITE_SIZE; i < FILE_SIZE; i += GET_SIZE_WRITE_SIZE)
   {
      ...
      if (SUCCESS != (ret = qsee_sfs_getSize(fd, &size)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
      ...
      if (GET_SIZE_WRITE_SIZE != (ret = qsee_sfs_write(fd, (char *)data, GET_SIZE_WRITE_SIZE)))
      {
         GPLOG_ERROR(GET_SIZE_WRITE_SIZE);
         GPLOG_ERROR(ret);
         ret = -1;
...
int smplap_sfs_unit_seek(void)
{
   ...
   {
      if (SEEK_SEGMENT_SIZE != (ret = qsee_sfs_write(fd, (char *)data, SEEK_SEGMENT_SIZE)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
   ...
   if (SEEK_WRITE_SIZE != (ret = qsee_sfs_write(fd, (char *)data, SEEK_WRITE_SIZE)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   ...
   if (SEEK_WRITE_SIZE != (ret = qsee_sfs_read(fd, (char *)data, SEEK_WRITE_SIZE)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
...
int smplap_sfs_unit_rm(void)
{
   ...
   if (SUCCESS == (ret = qsee_sfs_rm("/persist/data/im/not/there")))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
...
int smplap_sfs_unit_file_list(void)
{
   ...
   qsee_log(QSEE_LOG_MSG_DEBUG, "-- smplap_sfs_unit_file_list Negative Tests ----------------------");
    ...
    /*Null Inputs*/
   if (SUCCESS == (ret = qsee_sfs_get_file_list(NULL, &size)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   }
   if (SUCCESS == (ret = qsee_sfs_get_file_list(&list, NULL)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
   ...
   if (SUCCESS != (ret = qsee_sfs_get_file_list(&list, &size)))
   {
      GPLOG_ERROR(ret);
      ret = -1;
      goto Exit;
...
int smplap_sfs_basic_use_cases(void)
{
   ...
   if (NULL == (obj1 = (dataObj *)qsee_malloc(sizeof(dataObj))))
   {
      GPLOG_ERROR(obj1);
      ret = -1;
      goto Exit;
   }
   if (NULL == (obj2 = (dataObj *)qsee_malloc(sizeof(dataObj))))
   {
      GPLOG_ERROR(obj2);
      ret = -1;
      goto Exit;
  ...
  /*USE CASE 1 (Marshall and Unmarshall Structs)*/
   ...
   for (i = 0; i < BASIC_NUM_OF_WRITES; i++)
   {
      ...
      if (FILE_SIZE / BASIC_NUM_OF_WRITES != (ret = qsee_sfs_write(fd, (char *)data, FILE_SIZE / BASIC_NUM_OF_WRITES)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
   ...
   for (i = 0; i < BASIC_NUM_OF_WRITES; i++)
   {
      if (FILE_SIZE / BASIC_NUM_OF_WRITES != (ret = qsee_sfs_read(fd, (char *)data, FILE_SIZE / BASIC_NUM_OF_WRITES)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
   ...
   for (i = 0; i < size; i++)
   {
      ...
      if (SUCCESS != (ret = qsee_sfs_getSize(fd, &fileSize)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
...
int smplap_sfs_perf_test(void)
{
   ...
   for (i = 0; i < ITERS; i++)
   {
      ...
      if (PERF_FILE_SIZE != (ret = qsee_sfs_write(fd, (char *)data, PERF_FILE_SIZE)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
   ...
   {
      start = qsee_get_uptime();
      if (PERF_FILE_SIZE != (ret = qsee_sfs_read(fd, (char *)data, PERF_FILE_SIZE)))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
   ...
   {
      start = qsee_get_uptime();
      if (SUCCESS != qsee_sfs_getSize(fd, &size))
      {
         GPLOG_ERROR(ret);
         ret = -1;
         goto Exit;
...
int smplap_sfs_unit_max_files_test(void)
{
  ...
  uint32_t size = 0;
  sfs_file_entry* list = NULL;
  char *data = (char *)("Test data for SFS");
  ...
  do
  {
    for(i=0; i < TA_MAX_STORAGE_FILES; i++)
    {
      ...
      bytes_written = qsee_sfs_write(fd, (char *)data, strlen(data));
    ...
    for(i=0; i<size; i++ )
    {
      ...
      if(SUCCESS != qsee_sfs_rm(c_const_cast(char*, list[i].file_name)))
      {
...
int smplap_sfs_file_size_stress_test(void)
{
  ...
  do
  {
    data_write = (char *)qsee_malloc(FILE_SIZE);
    if(data_write == NULL)
    {
    ...
    {
      data_written = qsee_sfs_write(fd, (char *)data_write, FILE_SIZE);
      if(data_written == 0)
      {
    ...
    if(qsee_sfs_getSize(fd, &file_size) != SUCCESS)
    {
   ...
   if((res = qsee_sfs_write(fd, (char *)data_write, FILE_SIZE)) == -1)
   {
...
#ifndef OFFTARGET
  #define CHECK(x) do { if(!(x)) { return FAILED; } } while(0)
#else
  #define CHECK(x) do { if(!(x)) { qsee_log(QSEE_LOG_MSG_ERROR, "Failed: "#x); } } while(0)
#endif
int smplap_sfs_unit_test_suite(void)
{
  CHECK(SUCCESS == smplap_sfs_unit_open());
  CHECK(SUCCESS == smplap_sfs_unit_rm());
  CHECK(SUCCESS == smplap_sfs_unit_read_write());
  CHECK(SUCCESS == smplap_sfs_unit_get_size());
  CHECK(SUCCESS == smplap_sfs_unit_seek());
  CHECK(SUCCESS == smplap_sfs_unit_file_list());
  CHECK(SUCCESS == smplap_sfs_perf_test());
  CHECK(SUCCESS == smplap_sfs_basic_use_cases());
  #ifndef IS_sdm710_TARGET
    CHECK(SUCCESS == smplap_sfs_file_size_stress_test()); // https://orbit/CR/2315512
  #endif
...
int tz_apps_test_dir_ops(void)
{
  ...
  int fd = 0;
  DIR *dp = NULL;
  struct tzDirent dirp ={0};
  struct tzDirent *dir = NULL;
  size_t i = 0;
  do
  {
    for(i = 0; i < 5; i++) {
      snprintf(dir_name, sizeof(dir_name), "/data/misc/qsee/sampleapp/fs%zu/", i);
      snprintf(filename, sizeof(filename), "sampleapp%zu.txt", i);
      strlcpy(full_file_name , dir_name, sizeof(full_file_name));
      strlcat(full_file_name , "/", sizeof(full_file_name));
      strlcat(full_file_name , filename, sizeof(full_file_name));
      ret = mkdir(dir_name, 0755);
      if (ret != 0) {
        qsee_log(QSEE_LOG_MSG_ERROR, "file system mkdir() FAILED! errno = %d", ret);
        ret = FAILED;
        break;
      } else {
        qsee_log(QSEE_LOG_MSG_DEBUG, "file system mkdir() PASSED for dir %s", dir_name);
      }

      fd = open(full_file_name, O_RDWR | O_CREAT | O_TRUNC);
      if (fd == -1) {
         qsee_log(QSEE_LOG_MSG_ERROR, "file system open() FAILED! errno = %d", fd);
         ret = FAILED;
         break;
      } else {
         qsee_log(QSEE_LOG_MSG_DEBUG, "file system open( O_RDWR | O_CREAT | O_TRUNC ) PASSED for %s", filename);
      }
      ...
      if (ret != strlen(ETEST_DATA)) {
         qsee_log(QSEE_LOG_MSG_ERROR, "file system write() FAILED! errno = %d", ret);
         break;
      }
      //close
      ret = close(fd);
      if (ret != 0) {
         qsee_log(QSEE_LOG_MSG_ERROR, "file system close FAILED! returned = %d", ret);
         break;
      }
      fd = -1;
    }
    ...
    for(i=0 ;i < 5; i++){
      ...
      dp = opendir(dir_name);
      if(dp == NULL) {
        qsee_log(QSEE_LOG_MSG_ERROR, "file system opendir() FAILED! errno = %d", get_error_number());
        ret = FAILED;
        break;
      }else {
        qsee_log(QSEE_LOG_MSG_DEBUG, "Opend Dir passed for %s", dir_name);
      }

      /*readdir*/
      while((dir = readdir(dp, &dirp)) != NULL) {
        if (strncmp(dir->d_name, ".", 1) && strncmp(dir->d_name, "..", 2)){
          qsee_log(QSEE_LOG_MSG_ERROR, "File name is %s", dir->d_name);

          if(strncmp(dir->d_name, filename, strlen(filename))) {
            qsee_log(QSEE_LOG_MSG_ERROR," File name mismatch in directory %s"
                     "expected filename %s, retrieved filename %s", dir_name, filename, dir->d_name);
             ret = FAILED;
             break;
          }
        }
      ...
      if(ret != 0){
         qsee_log(QSEE_LOG_MSG_ERROR,
                  "file system closedir() FAILED! ret = %d, errno = %d", ret, get_error_number());
         break;
      }
      else {
        qsee_log(QSEE_LOG_MSG_DEBUG, "Close Dir passed for %s", dir_name);
      }
      dir = NULL;
      dp = NULL;
    }
  ...
  for(i = 0; i< 5; i++) {
    ...
    if (status != 0) {
      qsee_log(QSEE_LOG_MSG_ERROR, "file system rmdir() FAILED! errno = %d", status);
      if(get_error_number() != ENOENT) {
        ret = status;
      }
    }
  }
...
int tz_app_fs(void) {
   ...
   unsigned long long time_2;
   DIR *dp;
   struct tzDirent dirp;
   struct tzDirent *dir;

   qsee_log(QSEE_LOG_MSG_DEBUG, "-- Start %s TESTS ---------------------------------", __func__);

   do {
      if((ret = tz_apps_test_dir_ops()) != 0) {
        qsee_log(QSEE_LOG_MSG_ERROR, "Directory operations test failed");
        break;
      }
      len = (strlen(ETEST_DATA) + strlen(ETEST_DATA2) + 1);
      buf = (uint8_t *) qsee_malloc(sizeof(uint8_t) * len);
...
//fctnl
      ...
      dp = opendir(etest_fs_dir);
      if(dp == NULL) {
        qsee_log(QSEE_LOG_MSG_ERROR, "file system opendir() FAILED! errno = %d", ret);
        ret = FAILED;
        break;
      }else {
        qsee_log(QSEE_LOG_MSG_DEBUG, "Opend Dir passed");
      }
      qsee_log(QSEE_LOG_MSG_ERROR, "get_error_number %d",get_error_number());
      ...
      if(ret != 0){
         qsee_log(QSEE_LOG_MSG_ERROR,
                  "file system closedir() FAILED! errno = %d", ret);
         break;
      } 
      else{
        qsee_log(QSEE_LOG_MSG_DEBUG, "file system closedir PASSED.");
      }
      //remdir
...
int tz_app_sfs_anti_rollback(void)
{
   ...
   do
   {
      ...
      if (write_file)
      {
         ...
         ret = qsee_sfs_write(fd_sfs, (char *)test_buffer, len);
         if(ret != len)
         {
      ...
      else    /* read result */
      {
         ...
         ret = qsee_sfs_read(fd_sfs, (char *)test_buffer, len);
         if(ret != len)
         {
            qsee_log(QSEE_LOG_MSG_ERROR, "qsee_sfs_write() %s FAILED! ",anti_rollback_file );
            break;
