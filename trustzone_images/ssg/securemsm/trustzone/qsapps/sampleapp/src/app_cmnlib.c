...
/*===========================================================================
  ...
  $Header: //components/rel/ssg.tz/1.1.3/securemsm/trustzone/qsapps/sampleapp/src/app_cmnlib.c#1 $
...
int32_t copy_S_BIGINT(uint8_t *dst, uint32_t dst_len, QSEE_S_BIGINT *src, uint32_t max_len)
{
   uint32_t i = 0;
   ...
   {
      *((uint32_t *)(uintnt)dst + i) =  htonl(src->bi.a[array_length - 1 - i]);
   }
   return E_SUCCESS;
}
...
int test_crypto_ecc_complementary(void)
{
	...
	uint16_t pkcs8_ecc_key_len = sizeof(pkcs8_ecc_key)/sizeof(uint8_t);
	int32_t ret = E_SUCCESS;

    pkcs8_prv_ecc_key.algo = QSEE_PKCS8_ECC_ENCRYPTION;
    ret = (int32_t)qsee_secpkcs8_parse(pkcs8_ecc_key, (uint16_t)pkcs8_ecc_key_len, &pkcs8_prv_ecc_key);
    if (ret != E_SUCCESS) {
    ...
    ret = (int32_t)qsee_secpkcs8_parse(pkcs8_ecc_key2, (uint16_t)pkcs8_ecc_key_len, &pkcs8_prv_ecc_key);
    if (ret != E_SUCCESS) {
      qsee_log(QSEE_LOG_MSG_ERROR, "Secpkcs8 Parser failed: %d.", ret);
    } else {
        qsee_log(QSEE_LOG_MSG_DEBUG, "Secpkcs8 (ECC) Parser succeeded.");
...
int test_crypto_rsa_complementary(void)
{
   ...
   uint16_t pkcs8_key_len = sizeof(pkcs8_key)/sizeof(uint8_t);
   ...
   do {
      ...
      ret = (int32_t)qsee_secpkcs8_parse(pkcs8_key, (uint16_t)pkcs8_key_len, &pkcs8_prv_key);
      if (ret != E_SUCCESS) {
