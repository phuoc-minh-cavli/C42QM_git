...
/*===========================================================================
  ...
  $Header: //components/rel/ssg.tz/1.1.3/securemsm/trustzone/qsapps/sampleapp/src/app_bulletin_board.c#1 $
...
// current application name, taken from metadata
extern const char *TA_APP_NAME;

// length of IETF UUID represented as a string
#define UUID_STR_LEN     ((4 + 2 + 2 + 2 + 6)*2 + 4 + 1)
...
static bool simple_note_passing(void)
{
  ...
  size_t const cmp_name_len = MIN(strlen(TA_APP_NAME), sizeof(g_sender.name));
  size_t const cmp_uuid_len = MIN(UUID_STR_LEN, sizeof(g_sender.name));
  BB_ASSERT_EQUAL(timesafe_strncmp(g_sender.name, TA_APP_NAME, cmp_name_len) &&
                  timesafe_strncmp(g_sender.name, "53616D70-6C65-4170-7000-000000000032", cmp_uuid_len) &&
                  timesafe_strncmp(g_sender.name, "53616D70-6C65-4170-7000-000000000064", cmp_uuid_len), 0);
  return true;
}
