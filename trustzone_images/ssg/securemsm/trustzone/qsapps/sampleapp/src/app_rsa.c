...
int tzt_test_crypto_rsa_enc_dec_vect(  uint8_t                   operation,
                                       uint32_t                  hashidx,
                                       uint32_t                  padding_type,
                                       void*                   padding_info,
                                       uint32_t                  padding_info_len,
                                       uint32_t*                 _key,
                                       ...
                                       uint32_t                  emsg_len)
{
   ...
   uint32_t                  enc_msg_len = 256;
   uint8_t                   *dec_msg = NULL;
   uint32_t                  dec_msg_len = 256;
   int                     result;
   const uint32_t            MSG_LEN = 256;
   ...
   enc_msg = (uint8_t*)qsee_malloc(MSG_LEN);
   if( NULL == enc_msg )
   {
      return -E_FAILURE;
   }

   dec_msg = (uint8_t*)qsee_malloc(MSG_LEN);
   if( NULL == dec_msg )
   {
      return -E_FAILURE;
   }
...
int test_crypto_rsa_enc_dec(void)
{
   ...
   uint32_t                  pass_test_count = 0;
   uint32_t                  fail_test_count = 0;
   int                     ret = CE_SUCCESS;
...
int tzt_test_crypto_rsa_sign_verify_vect(uint8_t operation,
                                         ...
                                         uint32_t                  sign_len)
{
   ...
   uint32_t                  signature_len = 384;
   uint8_t                   hash[QSEE_SHA256_HASH_SZ] = {0};
   uint32_t                  hash_len = QSEE_SHA256_HASH_SZ;
   int                     result;
   ...
   signature = (uint8_t*)qsee_malloc(signature_len);
   if( NULL == signature )
   {
      return -E_FAILURE;
   }
...
int tzt_test_crypto_rsa_sign_verify_ex(uint8_t operation,
                                       QSEE_HASH_IDX hashidx,
                                       uint32_t padding_type,
                                       void* padding_info,
                                       uint32_t padding_info_len,
                                       ...
                                       uint32_t sign_len)
{
   ...
   do
   {
      if( operation == 0 )
      {
         ...
         key->type = QSEE_RSA_KEY_PRIVATE;
         //val = qsee_get_uptime();
         result = qsee_rsa_sign_ex(key, (QSEE_RSA_PADDING_TYPE)padding_type, padding_info, hashidx,
                                   (unsigned char *)msg, msg_len, signature, (int*)(uintnt)&signature_len);
         if( CE_SUCCESS != result )
         {
            break;
         }
         ...
         */
         key->type = QSEE_RSA_KEY_PUBLIC;
         result = qsee_rsa_verify_signature_ex(key, (QSEE_RSA_PADDING_TYPE)padding_type, padding_info, hashidx,
                                               (unsigned char *) msg, msg_len, signature, signature_len);
         if( CE_SUCCESS != result )
         {
            qsee_log(QSEE_LOG_MSG_ERROR, "verify Signature fail");
            break;
      ...
      else
      {
         ...
         key->type = QSEE_RSA_KEY_PUBLIC;
         result = qsee_rsa_verify_signature_ex(key, (QSEE_RSA_PADDING_TYPE)padding_type, padding_info, hashidx,
                                               (unsigned char *) msg, msg_len, sign, sign_len);
         if( CE_SUCCESS != result )
         {
            break;
         }
...
int test_crypto_rsa_sign_verify(void)
{
   ...
   for (k = 0; k < sizeof(pss_key_list) / sizeof(test_rsa_key_t *); k++)
   {
     ...
     for (operation = 2; operation; operation--)
     {
       ...
       if (operation == 2)
       {
         ret = tzt_test_crypto_rsa_sign_verify_ex(0,
                                                  pss_test_vectors[k].hashidx,
                                                  padding_type,
                                                  padding_info,
                                                  sizeof(padding_info),
       ...
       {
         ret = tzt_test_crypto_rsa_sign_verify_ex(1,
                                                  pss_test_vectors[k].hashidx,
                                                  padding_type,
                                                  padding_info,
                                                  sizeof(padding_info),
...
int test_crypto_rsa_CRT_verify(void)
{
   ...
   QSEE_RSA_KEY              *key = NULL;
   uint8_t                   *enc_msg_1 = NULL;
   uint32_t                  enc_msg_len_1 = 256;
   uint8_t                   *dec_msg = NULL;
   //uint32_t                  dec_msg_len = 256;
   uint8_t                   *dec_msg_1 = NULL;
   //uint32_t                  dec_msg_len_1 = 256;
   int                     result;
   const uint32_t            MSG_LEN = 256;
   uint32_t len = 128;
   uint32_t msg_len_act = 10;
   ...
   enc_msg_1 = (uint8_t*)qsee_malloc(MSG_LEN);
   if( NULL == enc_msg_1 )
   {
   ...
   dec_msg_1 = (uint8_t*)qsee_malloc(MSG_LEN);
   if( NULL == dec_msg_1 )
   {
...
static int test_crypto_rsa_key_gen(void)
{
  int                     ret = E_SUCCESS;
  unsigned long long      total_time;
  QSEE_RSA_KEY            *key = NULL;
  ...
  do {
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 2K key gen %llu",total_time);
    */
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 2K CRT key gen %llu",total_time);
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 3K key gen %llu",total_time);
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 3K CRT key gen %llu",total_time);
    */
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 4K key gen %llu",total_time);
    */
    ...
    total_time = qsee_get_uptime() - total_time;
    qsee_log(QSEE_LOG_MSG_DEBUG,"time for RSA 4K CRT key gen %llu",total_time);
    */
  ...
  qsee_log(QSEE_LOG_MSG_DEBUG,(E_SUCCESS == ret)?"test_crypto_rsa_key_gen Success":"test_crypto_rsa_key_gen failed");
...
int tz_app_crypto_rsa_test(void)
{
   ...
   status |= test_crypto_rsa_key_gen();
