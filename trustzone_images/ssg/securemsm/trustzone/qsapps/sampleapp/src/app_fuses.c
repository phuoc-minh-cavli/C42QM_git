...
/*===========================================================================
  ...
  $Header: //components/rel/ssg.tz/1.1.3/securemsm/trustzone/qsapps/sampleapp/src/app_fuses.c#1 $
...
/*This function will flip the least significant unset bit
  ...
  output: 000101011
*/
int flip_next_bit(uint32_t* input)
{
   int index = 0;
   uint32_t data = *input;
...
*/
int blow_LSB_fuse(uint32_t lsb_addr, bool MSB)
{
   uint32_t qfprom_status1;
   uint32_t qfprom_status2;
   uint32_t qfprom_status3;

   uint64_t row_data_read1;
   uint32_t row_data_read1_32;
   uint64_t row_data_read2;

   uint64_t row_data_write;

   /*Read the fuse value*/
   qsee_fuse_read(lsb_addr, QFPROM_ADDR_SPACE_RAW, (uint32_t*)(uintnt)(uintnt)&row_data_read1, &qfprom_status1);
   ...
   {
      qsee_log(QSEE_LOG_MSG_DEBUG, "Initial read value: %x %x", (uint32_t)(row_data_read1 >> 32), (uint32_t)row_data_read1);
   }

   /*Extract the lower (or upper) 32 bits, then increment the that fuse value by flipping a bit*/
   if(MSB)
   {
      row_data_read1_32 = (uint32_t)(row_data_read1>>32);  //Grab upper 32 bits
   }
   else
   {
      row_data_read1_32 = (uint32_t)row_data_read1;    //Grab lower 32 bits
   }
   ...
   if(MSB)
   {
      row_data_write = (uint64_t)row_data_read1_32 << 32;
   }
   else
   {
      row_data_write = (uint64_t)row_data_read1_32;
   }
   qsee_log(QSEE_LOG_MSG_DEBUG, "After flipping a bit, we are writing:: %x %x", (uint32_t)(row_data_write >> 32), (uint32_t)row_data_write);
   qsee_fuse_write( lsb_addr, (uint32_t*)(uintnt)&row_data_write, BUS_FREQUENCY, &qfprom_status2);
   ...
   qsee_fuse_read(lsb_addr, QFPROM_ADDR_SPACE_RAW, (uint32_t*)(uintnt)&row_data_read2, &qfprom_status3);
   ...
   qsee_log(QSEE_LOG_MSG_DEBUG, "After second read we get: %x %x", (uint32_t)(row_data_read2 >> 32), (uint32_t)row_data_read2);

   if(MSB)
   {
      if((uint32_t)(row_data_read2 >> 32) != (uint32_t)(row_data_write >> 32))
      {
         qsee_log(QSEE_LOG_MSG_ERROR, "Fuse value read does not match what was written. %x != %x", (uint32_t)(row_data_read2 >> 32), (uint32_t)(row_data_write >> 32));
         return -1;
      }
   }
   else
   {
      if((uint32_t)(row_data_read2) != (uint32_t)(row_data_write))
      {
         qsee_log(QSEE_LOG_MSG_ERROR, "Fuse value read does not match what was written. %x != %x",  (uint32_t)row_data_read2, (uint32_t)row_data_write);
         return -1;
      }
   }
