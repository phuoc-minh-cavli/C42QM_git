...
/*===========================================================================
  ...
  $Header: //components/rel/ssg.tz/1.1.3/securemsm/trustzone/qsapps/sampleapp/src/app_iris.c#1 $
...
int run_reenroll_test(uint64_t addr, uint32_t len) {
   sp_iris_result_e ret;
   uint8_t tamplates_buf[TWO_TEMPLATES_SIZE];
   uint32_t reenroll_id = (qsee_get_uptime())%MAX_IDS_NUM;
   uint32_t score1, score2;
 ...
 * Global function
 * ----------------------------------------------------------------------*/
int run_iris_test(void* cmd, uint32_t cmdlen,
