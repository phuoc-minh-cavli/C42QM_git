...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/ssg.tz/1.1.3/bsp/build/scripts/pil_splitter_builder.py#1 $
...
def splitter_generate(env):
   ...
   action = 'python ' + os.path.join(THIS_DIR, 'pil_splitter.py') + ' $${SOURCE.abspath} $${TARGET.base}'
   
   splitter_bld = env.Builder(action  = action,
...
def splitter_helper(env, mbns, pil_base_dir, split_bin_count = None):
   ...
   for signed_mbn in mbns:
      ...
      sign_path_index = pil_dir.find(env.subst("${SHORT_BUILDPATH}")) \
            + len(env.subst("${SHORT_BUILDPATH}")) + 1
