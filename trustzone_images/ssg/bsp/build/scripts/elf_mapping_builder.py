import struct, os, shutil, copy, re
...
def getSymbolAddress(symbols, symbol_list):
  ...
  for i in range(len(symbol_list)):
    if len(symbol_list[i]) < 3:
      continue
    for j in range(len(symbols)):
      s = symbols[j]
      if symbol_list[i][-1] == s:
...
def round_up(value, size):
  if value is None:
    raise TypeError("round_up: value is None")

  if size is None:
    raise TypeError("round_up: size is None")
...
def get_builds_file_value(env, val_name, symbol_list):
  ...
  if val_name in env['GLOBAL_DICT']:
    ...
    # if the symbol has not been found in the 'GLOBAL_DICT' or in the symbol
    # file, check the CPPDEFINES list
    for x in env['CPPDEFINES']:
      keyval = x.split('=')
      if 2 == len(keyval) and keyval[0] == val_name:
          return get_builds_file_value(env, keyval[1], symbol_list)

  return None

def eval_symbol_expr(env, expr, symbol_list, required=False):
  result = re.findall('[a-zA-Z][a-zA-Z0-9_$]*', expr)
  for x in result:
    expr = expr.replace(x, str(get_builds_file_value(env, x, symbol_list)))

  try:
    result = eval(expr)
  except:
    msg = "{}: no target.builds value for '{}'".format(os.path.basename(__file__), expr)
    if required:
...
def elf_mapping_builder (target, source, env):
  ...
  levelwise_max_tts_node = xml_root.find('pt_pool_dtls').find('levelwise_max_tts')
  if levelwise_max_tts_node == None:
    raise ValueError(
        "Invalid XML - missing <levelwise_max_tts> tag in {}".format(
        source_xml_str))

  levelwise_max_tts_symbols = (levelwise_max_tts_node.text).strip().split(" ")
  ppt_size_symbols = getSymbolAddress(levelwise_max_tts_symbols, symbol_list)

  assert {} not in ppt_size_symbols
  levelwise_max_tts_values = [x['address'] for x in ppt_size_symbols if x != {}]

  symbol_info = [{} for x in arch.mapping_sizes]
  
  for i in range(len(arch.mapping_sizes)):
   symbol_info[i]['address'] =  pt_pool_addr
   symbol_info[i]['size'] = arch.table_sizes[i] * levelwise_max_tts_values[i]
  ...
  va_offset = xml_root.find('va_offset')
  if va_offset is None:
    va_offset = 0
  else:
    va_offset = eval_symbol_expr(env, va_offset.text, symbol_list)

  # Static mappings
  for m in xml_root.find('map_list').findall('mapping'):
    temp = {
      'vaddr' : eval_symbol_expr(env, m.attrib['vaddr'], symbol_list),
      'paddr' : eval_symbol_expr(env, m.attrib['paddr'], symbol_list),
      'flags' : m.attrib['flags'].split(',')}

    if 'bytes' in m.attrib:
      temp['sz_in_kb'] = B2KB(round_up(eval_symbol_expr(env, m.attrib['bytes'], symbol_list, True), arch.page_size))
    else:
      temp['sz_in_kb'] = B2KB(round_up(eval_symbol_expr(env, m.attrib['end_vaddr'], symbol_list, True) - temp['vaddr'], arch.page_size))

    if 0 == temp['sz_in_kb']:
      continue

    min_mapping_size = arch.mapping_sizes[-1]
    if temp['vaddr'] is None or temp['paddr'] is None or not is_valid_mapping(temp, arch):
      raise Exception("Invalid mapping values VA %s(%s), PA %s(%s), size %s(%s)" % (m.attrib['vaddr'], temp['vaddr'], m.attrib['paddr'], temp['paddr'], ('bytes' in m.attrib and m.attrib['bytes'] or m.attrib['end_vaddr']), temp['sz_in_kb']))

    if 'va_offset' in m.attrib and "true" == m.attrib["va_offset"].lower():
      temp['vaddr'] += va_offset
  ...
  for i in range(elf_header.e_phnum):
    ...
    if is_valid_segment(phdr_entry) and phdr_entry.p_memsz > 0:
      ...
      for i in range(len(arch.mapping_sizes)):
        ...
        if addr >= phdr_entry.p_paddr and addr < (phdr_entry.p_paddr + phdr_entry.p_memsz):
          symbol_seek_offsets[i] = (addr - phdr_entry.p_paddr) + phdr_entry.p_offset
      ...
      if (PF_R) == perms:
        ...
        pass
      mappings.append({
        'vaddr' : phdr_entry.p_vaddr + va_offset,
