...
/*=========================================================================
  ...
  $Header: //components/rel/ssg.tz/1.1.3/api/securemsm/trustzone/qsee/qsee_log.h#1 $
...
/**
  ...
  This function accepts bitmask values in range QSEE_LOG_MSG_LOW
  to QSEE_LOG_MSG_FATAL. Any value outside this range is ignored.

  @param[in] pri_flags   OR(ed) bitmask for desired log levels.
...
/**
  ...
  @return Bitmask value of the log levels set.

 */
extern uint8_t qsee_log_get_mask(void);

/**
  Collects a set of logs in internal buffers
  and flushes the logs to a rolling log file at predetermined thresholds.

  @param [in] pri  Priority of message to be logged.
  @param [in] fmt  Pointer to a string describing the message format.
...
/**
  ...
  @param [in] fmt  Pointer to string describing message format.
  @param [in] ...  Variable argument list.
 */
