...
# SSL/TLS related command line arguments.
...
class QtiSigningServerOptionParser(CoreOptionParser):
    ...
    def c_add_options(self):
        ...
        # Optional arguments for setting up an HTTPS server.
        output_group = self.add_option_group('Starting an HTTPS server')
        output_group.add_option(HTTPS_CERTIFICATE, metavar='<file_path>',
                                help="The certificate used to authenticate this server to clients when using HTTPS. "
                                     "If provided, the server will use HTTPS.")
        output_group.add_option(HTTPS_KEY, metavar='<file_path>',
                                help="The private key used by the server to make HTTPS connections. If provided, the "
...
class QtiSigningServer(HTTPServer):
    ...
    def __init__(self, host, port, signer, cass_capability, config, output_dir, verbose=False, debug=False, quiet=False,
        ...
        if bool(ssl_key) != bool(ssl_cert):
            raise RuntimeError("Must provide both {0} and {1} when starting the server with "
                               "HTTPS.".format(HTTPS_KEY, HTTPS_CERTIFICATE))
        HTTPServer.__init__(self, host, port, https=(ssl_cert and ssl_key), ssl_cert=ssl_cert, ssl_key=ssl_key)
        ...
        # Create a lock to serialize handling of signing requests and avoid race conditions.
        ...
        with self.lock:
            ...
            try:
                ...
                il = launch_secimage(config=temp_config_file, output_dir=output_dir,
                                     ...
                                     verbose=self.verbose, debug=True, quiet=self.quiet,
                                     cfg_params={"cass_capability": self.cass_capability}, authority=authority)
                ...
                sig_pkg['signature'] = \
                    BinString(c_path.load_data_from_file(il[0].dest_image.debug_file_signer_signature))
...
def main(args):
    ...
    # Start the server.
    server = QtiSigningServer(host=args.host, port=args.port, signer=args.signer, cass_capability=args.cass_capability,
                              config=args.config, output_dir=args.output_dir, verbose=args.verbose, debug=args.debug,
                              quiet=args.quiet, ssl_cert=args.https_certificate, ssl_key=args.https_key)
    server.start()
...
def start(host=DEFAULT_HOST, port=DEFAULT_PORT,
          ...
          https_certificate=None, https_key=None):
    qti_signing_server = QtiSigningServer(host, port, signer, cass_capability, config, output_dir,
                                          ssl_cert=https_certificate, ssl_key=https_key)
