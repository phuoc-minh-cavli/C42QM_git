...
sectools_root_dir = os.path.realpath(os.path.join(os.path.dirname(__file__), *([".."] * 4)))
trusted_ca_certificates_file = os.path.join(sectools_root_dir, "sectools", "features", "isc", "signer",
                                            "qti_remote_signer_client_truststore", "qti_ca_file.pem")
...
class QtiRemoteSigner(LocalV2Signer):
    ...
    def initialize(self, imageinfo, **kwargs):
        ...
        # qti_ca_file.pem should be updated in advance of the server's certificate being issued by a new CA certificate.
        client = QtiRemoteClient(host=remote_host, port=remote_port, ssl_cert=trusted_ca_certificates_file)
