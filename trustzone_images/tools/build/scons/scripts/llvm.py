...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.tz/2.1.3/build/scons/scripts/llvm.py#1 $
#  $DateTime: 2019/05/23 05:36:07 $
#  $Change: 19292089 $
...
def smart_link(source, target, env, for_signature):
    cplusplus = imp.load_source('cplusplus' ,env['BUILD_ROOT']+"/tools/build/scons/SCons/Tool/c++.py")
    has_cplusplus = cplusplus.iscplusplus(source)

    if has_cplusplus:
      if env['PROC'] == 'A53_64':
        return "${TEMPFILE('$LINK $LINKFLAGS $_LIBDIRFLAGS -L $LLVMROOT/aarch64-none-elf/libc/lib -L $LLVMROOT/aarch64-none-elf/lib --start-group -lunwind -lc++ -lc++abi $_LISTFILES $_LIBFLAGS $SOURCES  $LINKFLAGSAN  --end-group  -static --start-group -lc $LLVMROOT/lib/clang/$clang_version/lib/baremetal/libclang_rt.builtins-aarch64.a --end-group -lm -lc $LINKFLAGS2 $ARMLD_OUTPUT_CMD $TARGET')}"
      else :
        return "${TEMPFILE('$LINK $LINKFLAGS $_LIBDIRFLAGS -L $LLVMROOT/armv7-none-eabi/libc/lib -L $LLVMROOT/armv7-none-eabi/lib --start-group -lunwind -lc++ -lc++abi $_LISTFILES $_LIBFLAGS $SOURCES  $LINKFLAGSAN  --end-group  -static --start-group -lc $LLVMROOT/lib/clang/$clang_version/lib/baremetal/libclang_rt.builtins-armv7.a --end-group -lm -lc $LINKFLAGS2 $ARMLD_OUTPUT_CMD $TARGET')}"
    else:
      return "${TEMPFILE('$LINK $LINKFLAGS $_LIBDIRFLAGS --start-group $_LISTFILES $_LIBFLAGS $SOURCES  $LINKFLAGSAN  --end-group  $LINKFLAGS2 $ARMLD_OUTPUT_CMD $TARGET')}"
...
def generate(env):
   ...
   env.Replace(LLVMROOT = os.environ['LLVMROOT'])
   env.Replace(clang_version = os.environ['LLVM_VERSION'])
   ...
   env.PrintInfo('LLVMROOT         = ${LLVMROOT}')
   ...
   if ARGUMENTS.get('deterministic',0) == 1:
      ...
      llvm_cpp_flags_tz.append(' -D__TIME__=\\"ss:mm:hh\\"')
      llvm_cpp_flags_tz.append(' -D__DATE__=\\"mmmddyyyy\\"')
      llvm_cpp_flags_tz.append(' -D__FILE__=\\"${SOURCE.file}\\"')
      llvm_cpp_flags_tz.append('-Wno-builtin-macro-redefined')
   LLVMFlags = ""
   LLVMCPPFlags = ""
   ...
   if do_64:
      ...
      llvm_cpp_flags_tz.extend([
           "-target aarch64-none-elf",                                # Generate code for the given target.
           "-mcpu=cortex-a53",                                         # Specify CPU.  
           "-fno-exceptions",
           "-fno-rtti",
       ])
      ...
      LLVMCPPFlags = " ".join(llvm_cpp_flags_tz)
      ...
      env.Replace(TCPP = LLVMBIN + "clang++${EXE_EXT} " + LLVMCPPFlags)
      ...
      llvm_cpp_flags_tz.extend([
        "-target armv7-none-eabi",                                      # Generate code for the given target. 
        "-mfloat-abi=softfp",                                          # Specifies which floating-point ABI to use.
        "-mno-unaligned-access",                                       # Force all memory accesses to be aligned.       
        "-fno-exceptions ",
        "-fno-rtti",
      ...
      env.Replace(TCPP = LLVMBIN + "clang++${EXE_EXT} -mthumb " + LLVMCPPFlags)
      if 'USES_NO_DEBUG' not in env:
         env.Replace(ARM_DBG = "-g ")
         ASM_DBG = "-g "
   ...
   env.Replace(ARMCPP = LLVMBIN + "clang++${EXE_EXT} " + LLVMCPPFlags)
...
#       8916
   ...
   # TODO: need to remove CCFLAGS from CXXCOM it is couseing a lot of warnings, a lot of files in MULTIMEDIA need to be fixed first
   if do_64:
     #Appending the LLVMINC to include the CPP specific paths as well
     LLVMCPPINC = ""
     env.Append(LLVMCPPINC = " $LLVMROOT/aarch64-none-elf/include/c++/v1")
     CXXCOM = "${TEMPFILE('$CXX $CXXFLAGS $CCFLAGS $_CCCOMCOM $INCPATHS -isystem$LLVMINC -isystem$LLVMCPPINC  $ARMCC_COMPILE_CMD $SOURCES $ARMCC_OUTPUT_CMD $TARGET')}"
   else:
     #Appending the LLVMINC to include the CPP specific paths as well
     LLVM32CPPINC = ""
     env.Append(LLVM32CPPINC = " $LLVMROOT/armv7-none-eabi/include/c++/v1")
     CXXCOM = "${TEMPFILE('$CXX $CXXFLAGS $CCFLAGS $_CCCOMCOM $INCPATHS -isystem$LLVM32INC -isystem$LLVM32CPPINC  $ARMCC_COMPILE_CMD $SOURCES $ARMCC_OUTPUT_CMD $TARGET')}"
