...
$Header: //components/rel/audio.tx/2.0/audio/audcal/acdb/src/acdb_command.c#1 $
...
*--------------------------------------------------------------------------- */
#define ACDB_SOFTWARE_VERSION_MAJOR 0x00000007
#define ACDB_SOFTWARE_VERSION_MINOR                      0x00000000
#define ACDB_SOFTWARE_VERSION_REVISION 0x00000003
...
int32_t AcdbCmdGetVocProcStatInfo(uint32_t queryType,uint8_t *pIn,uint8_t *pOut)
{
	...
	else
	{
		...
		ACDB_MEM_CPY(&ofTbloffset,sizeof(uint32_t),&lutTbl.pLut[index].nOffTableOffset,sizeof(uint32_t));// get offset table offset from lut table
		ACDB_MEM_CPY(&cvdTbloffset,sizeof(uint32_t),&lutTbl.pLut[index].nCommonTblOffset,sizeof(uint32_t));// get commoncvd table offset from lut table
		...
		if(queryType==TABLE_CMD)// write lut size for the first time
		{
			...
			if( heapsize>0)
			{
				for(lut_index=0;lut_index<(int)lutLength;lut_index++)
				{
					...
					isheapentryfound=0;
					//datatable calculation
					cdotTbl.nLen = *((uint32_t *)(pInput->nBufferPointer+cdot_start + 4+GetAt(lut_index,&lut_cdot_new)));
					...
					cdefTbl.nLen = *((uint32_t *)(pInput->nBufferPointer+cdft_start +4+ GetAt(lut_index,&lut_cdft_new)/*offsettbleinstance->nCDEFTblOffset*/));
...
int32_t AcdbCmdGetVocStream2Info(uint32_t queryType,uint8_t *pIn,uint8_t *pOut)
{
	...
	else
	{
		...
		for(lut_index=(int)index;lut_index<(int)lutTbl.nLen;lut_index++)
		{
			...
			if(!update(lutTbl.pLut[lut_index].nCDOTTblOffset,lut_index,&lut_cdot_new))
			{
				result= ACDB_INSUFFICIENTMEMORY;
				goto end;
			}
			if(!update(lutTbl.pLut[lut_index].nCDEFTblOffset,lut_index,&lut_cdft_new))
			{
				result= ACDB_INSUFFICIENTMEMORY;
				goto end;
			}
			...
			if(Find(lutTbl.pLut[lut_index].nCDEFTblOffset,&lut_cdft,&i))
				{
					lut_cdftfound=1;
			}
			if(!lut_cdftfound)
			{
				if(!update(lutTbl.pLut[lut_index].nCDEFTblOffset,j,&lut_cdft))
				{
					...
					if(!update(0,0,&lut_cdot_buffer))
			{
					...
					if(Find(lutTbl.pLut[lut_index].nCDOTTblOffset,&lut_cdot,&i))
				{
					lut_cdotfound=1;
			}
			if(!lut_cdotfound)
			{
						if(!update(lutTbl.pLut[lut_index].nCDOTTblOffset,k,&lut_cdot))
						{
							result= ACDB_INSUFFICIENTMEMORY;
							goto end;
						}
		...
		if(queryType==TABLE_CMD)// write lut size for the first time
		{
			...
			if(/*isheapentryfound==1 &&*/ heapsize>0)
			{
				for(lut_index=0;lut_index<(int)lutTbl.nLen;lut_index++)
				{
					...
					isheapentryfound=0;
					//datatable calculation
					cdotTbl.nLen = *((uint32_t *)(pInput->pBufferPointer+cdot_start + 4+GetAt(lut_index,&lut_cdot_new)));
					...
					cdefTbl.nLen = *((uint32_t *)(pInput->pBufferPointer+cdft_start +4+ GetAt(lut_index,&lut_cdft_new)/*offsettbleinstance->nCDEFTblOffset*/));
