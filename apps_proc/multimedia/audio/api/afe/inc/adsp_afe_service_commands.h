...
$Header: //components/rel/audio.tx/2.0/audio/api/afe/inc/adsp_afe_service_commands.h#1 $
...
#define AFE_PORT_ID_INVALID                                         0xFFFF
  ...
  @detdesc
	...
	This command is accepted only on ports with sampling rate 
	AFE_PORT_SAMPLE_RATE_8K or AFE_PORT_SAMPLE_RATE_16K or AFE_PORT_SAMPLE_RATE_48K. 
...
#include "adsp_end_pack.h"
  ...
  /** ID of the parameter used by #AFE_MODULE_AUDIO_DEV_INTERFACE to
      configure the LPASS core shared clock for register access to lpass core.
      Only #AFE_PORT_CMD_SET_PARAM_V2 can use this ID.
  
      @apr_hdr_fields
      Opcode -- AFE_PARAM_ID_LPASS_CORE_SHARED_CLK_CONFIG @vertspace{-3}
  
      @msg_payload{afe_param_id_lpass_core_shared_clk_cfg_t}
      @wktable{weak__afe__param__id__lpass__core__shared____clk__cfg__t}
  */
...
#include "adsp_begin_pack.h"
  ...
  @{ */
    ...
    struct afe_param_id_lpass_core_shared_clk_cfg_t
    {
      ...
      uint32_t                  enable;
      /**< Specifies whether the lpass core shared clock is enabled (1) or disabled (0). */
    }
#include "adsp_end_pack.h"
    ;
...
#include "adsp_end_pack.h"
  ...
  @msg_payload{afe_port_cmd_devices_restart_t}
    @wktable{weak__afe__port__cmd__devices__restart__t}
...
@{ */
  /** ID of the software duty cycle module.

    This module supports following parameter IDs:
     - #AFE_PARAM_ID_SW_DUTY_CYCLE_CFG
     - #AFE_PARAM_ID_ENABLE

    To configure the duty cycle,  client must use the
    #AFE_PORT_CMD_SET_PARAM_V2 command, and fill the module ID with the
    respective parameter IDs as listed above.

    AFE_PARAM_ID_SW_DUTY_CYCLE_CFG, AFE_PARAM_ID_ENABLE (enable =1) must be sent
    before AFE port starts.

    After receiving AFE_PARAM_ID_ENABLE (enable =0), there wont be any data exchange
    between codec and AFE.

    @apr_hdr_fields
    Opcode -- AFE_MODULE_SW_DUTYCYCLE @newpage
   */
#define AFE_MODULE_SW_DUTY_CYCLE                                   0x00010258

  /** ID of the parameter used by #AFE_MODULE_SW_DUTYCYCLE to
    configure the duty cycle parameters. 

    AFE_PARAM_ID_SW_DUTY_CYCLE_CFG command must be sent before
    AFE port starts. It wont accept if port is in RUN state.

    @apr_hdr_fields
    Opcode -- AFE_PARAM_ID_SW_DUTY_CYCLE_CFG

    @msg_payload{afe_param_id_sw_dutycycle_cfg_v1_t}
    @table{weak__afe__param__id__sw__dutycycle__cfg__v1__t}
   */
#define AFE_PARAM_ID_SW_DUTY_CYCLE_CFG                            0x00010259

  /** Version information used to handle future additions to
    configuration parameters(for backward compatibility).
   */
#define AFE_API_VERSION_SW_DUTY_CYCLE_CFG                         0x1

  /** @} */ /* end_addtogroup afe_module_group_device */
...
@{ */
  ...
  {
    uint32_t                  minor_version;
    /**< Minor version used for tracking. 

        @values #AFE_API_VERSION_SW_DUTY_CYCLE_CFG */

    uint32_t                  sleep_time_ms;
    /**< For sleep_time_ms amount of time AFE wont process any data. Units of time
...
@{ */
/** ID of the feedback module, which supports the following parameter ID:
    - #AFE_PARAM_ID_FEEDBACK_PATH_CFG
 
    @apr_hdr_fields
    Opcode -- AFE_MODULE_FEEDBACK @newpage
*/
...
/** ID of the parameter used by #AFE_MODULE_CLOCK_SET to
    configure the audio clock. #AFE_SVC_CMD_SET_PARAM can use this 
    parameter ID.
...
#define AFE_CLOCK_SET_CLOCK_ID_MI2S_RANGE_END                         0x115
...
struct afe_param_id_clock_set_t
{
    ...
    /**< Minor version used to track clock settings.
    ...
    uint16_t                 clk_attri;
    /**< Specifies a divider for two clocks that are coupled, if necessary:
         ...
         - #AFE_CLOCK_SET_CLOCK_ATTRIBUTE_COUPLE_DIVISOR
         @tablebulletend */
