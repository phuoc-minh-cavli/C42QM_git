 ...
 * Qualcomm Technologies, Inc. Confidential and Proprietary. 
  ...
  $Header: //components/rel/audio.tx/2.0/apr/utils/src/apr_memmgr.c#1 $
  $Author: pwbldsvc $
*/
...
$Header: //components/rel/audio.tx/2.0/apr/utils/src/apr_memmgr.c#1 $ $DateTime: 2018/12/25 22:39:07 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
08/01/13   sg      Merged latest APR code from Mainline and updated Copyright.
...
/*===========================================================================
FUNCTION APR_MEMMGR_HEAP_GET_ALLOC_INFO
...
===========================================================================*/
void
apr_memmgr_heap_get_alloc_info(
  apr_memmgr_type                 *heap_ptr,
  unsigned int                     num_blocks,
  apr_memmgr_heap_alloc_info_type *buf
)
{
  apr_memmgr_heap_block_iterator_type heap_iter;
  apr_memmgr_block_header_type the_block;
  char *alloc_ptr;
  uint32 i = 0;

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  APR_MEMMGR_ASSERT( heap_ptr );
  APR_MEMMGR_ASSERT( num_blocks >= 1 );

  BEGIN_CRITICAL_SECTION( heap_ptr );

  memset( buf, 0, num_blocks * sizeof( apr_memmgr_heap_alloc_info_type ) );

  apr_memmgr_heap_block_iterator_init( &heap_iter, heap_ptr );

  alloc_ptr = (char *) heap_ptr->first_block;

  while ( apr_memmgr_heap_block_iterator_next( &heap_iter, &the_block, 1 ) )
  {
    ...
    {
      buf[i].ptr                = alloc_ptr;
      buf[i].size               = apr_memmgr_get_block_logical_size( &the_block );
...
#endif /* FEATURE_APR_MEMMGR_DEBUG */
