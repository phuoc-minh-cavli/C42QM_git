 ...
 * Qualcomm Technologies, Inc. Confidential and Proprietary. 
  ...
  $Header: //components/rel/audio.tx/2.0/apr/domain/variant/app/src/apr_log.c#3 $
  $Author: pwbldsvc $
*/
...
#ifndef LOG_APR_APPS_C
  #define LOG_APR_APPS_C ( ( 0x4D1 ) + LOG_1X_BASE_C )
#endif /* LOG_APR_APPS_C */

#ifndef MSG_SSID_APR_APPS
  #define MSG_SSID_APR_APPS ( MSG_SSID_AVS )
  #define MSG_BUILD_MASK_MSG_SSID_APR_APPS ( MSG_BUILD_MASK_MSG_SSID_AVS )
#endif /* MSG_SSID_APR_APPS */
...
APR_INTERNAL int32_t apr_log_init ( void )
{
  ...
  for ( ;; )
  {
    rc = apr_lock_create( APR_LOCK_TYPE_MUTEX, &apr_log_printf_mutex );
    if ( rc ) break;
    checkpoint = 1;

    rc = apr_lock_create( APR_LOCK_TYPE_MUTEX, &apr_log_history_mutex );
    if ( rc ) break;
    checkpoint = 2;

    ( void ) mmstd_memcpy( apr_log_history.start_marker, sizeof( apr_log_history.start_marker ), "APRAPPS1", 8 );
    apr_log_history.size = APR_LOG_HISTORY_BYTE_SIZE_V;
    ( void ) mmstd_memset( apr_log_history.log, 0xFF, sizeof( apr_log_history.log ) );
    ( void ) mmstd_memcpy( apr_log_history.end_marker, sizeof( apr_log_history.end_marker ), "APRAPPS2", 8 );
    checkpoint = 3;

#ifdef APR_ERROR_LOGGING
    rc = apr_lock_create( APR_LOCK_TYPE_MUTEX, &apr_log_error_mutex );
    if ( rc ) break;
    checkpoint = 4;

    ( void ) mmstd_memcpy( apr_log_error_history.start_marker, sizeof( apr_log_error_history.start_marker ), "APRADSP3", 8 );
    ...
    ( void ) mmstd_memcpy( apr_log_error_history.end_marker, sizeof( apr_log_error_history.end_marker ), "APRAPPS4", 8 );
#endif /* APR_ERROR_LOGGING */
...
APR_INTERNAL void apr_log_printf ( const char * fmt, ... )
{
  ...
  MSG_SPRINTF_1( MSG_SSID_APR_APPS, MSG_LVL_LOW, "%s", apr_log_printf_buffer );
...
APR_INTERNAL void apr_log_panic_printf ( const char * fmt, ... )
{
  ...
  ( void ) apr_lock_enter( apr_log_printf_mutex );
  vsnprintf( apr_log_printf_buffer, sizeof( apr_log_printf_buffer ), fmt, args );
  MSG_SPRINTF_1( MSG_SSID_APR_APPS, MSG_LVL_FATAL, "%s", apr_log_printf_buffer );
  ( void ) apr_lock_leave( apr_log_printf_mutex );
  va_end( args );
...
)
{
  ...
  log_ptr = ( ( uint8_t* ) log_alloc( LOG_APR_APPS_C,
