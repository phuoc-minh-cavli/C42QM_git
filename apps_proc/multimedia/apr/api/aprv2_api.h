...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //components/rel/audio.tx/2.0/apr/api/aprv2_api.h#1 $
  $Author: pwbldsvc $
...
static inline int32_t __aprv2_cmd_register2 (
  ...
  @param[out] ret_handle     Returns the registered service handle.
  @param[in] name            Service name string to register. A NULL name means
                             the service does not have a name.
  @param[in] name_size       Service name string size in bytes.
  @param[in] addr            Address to register. A zero value means to
                             autogenerate a free dynamic address. A nonzero
                             value means to directly use the statically
                             assigned address.
  @param[in] dispatch_fn     Dispatch function to register.
  @param[in] dispatch_data   Client-supplied data pointer for the dispatch
                             function.
  @param[out] ret_addr       Returns the actual registered address.
