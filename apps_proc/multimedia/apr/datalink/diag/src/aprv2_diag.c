 ...
 * Qualcomm Technologies, Inc. Confidential and Proprietary. 
   ...
   $Header: //components/rel/audio.tx/2.0/apr/datalink/diag/src/aprv2_diag.c#1 $
   $Author: pwbldsvc $
*/
  ...
  do
  {
    ...
    { /* There is something to send */
      ...
      tx = ( ( aprv2_diag_rsp_pkt_t* ) diagpkt_subsys_alloc( DIAG_SUBSYS_APR, 0,
             ( sizeof( aprv2_diag_rsp_pkt_t ) + size ) ) );
      if ( tx )
...
)
{
  ...
  if ( tx_qitem != NULL )
  {
    ( void ) mmstd_memcpy( tx_qitem->buf, sizeof( tx_qitem->buf ), packet, size );
...
// Using this DIAG_SUBSYS_APR temporarily for audio debug
#if 0
...
APR_INTERNAL int32_t aprv2_diag_init ( void )
{
  ...
  for ( ;; )
  {
    ...
    rc = apr_lock_create( APR_LOCK_TYPE_MUTEX, &aprv2_diag.lock );
    if ( rc ) break;
    check_point = 1;
