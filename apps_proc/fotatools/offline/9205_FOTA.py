...
META_1 = Clade1SegSections + Clade2SegSections
# Meta 2 is comprised of singleton data
META_2 = CLADE1_Uncompressed_Key + CLADE2_Uncompressed_Key + CLADE2_Config
...
class CLADE_Extractor:
  ...
  def find_mbn_offsets(self):
    ...
    '''
    #depricated not applicable to CT.2.0
    ...
    with open(self.out_dir + 'offsets.txt', 'w') as out:
      out.write("CLADE1 OFFSET: {}\n".format(
        self.mbn_offsets['.clade.comp'][0]))
      out.write("CLADE2 OFFSET: {}\n".format(
        self.mbn_offsets['.clade2.comp'][0]))
      '''
      #depricated for CT.2.0
      out.write("EXTPL STATIC OFFSET: {}\n".format(
        self.extpl_meta['static_offset']))
      out.write("EXTPL PAGING OFFSET: {}\n".format(
        self.extpl_meta['paging_offset']))
      '''  
  ...
  def make_fota_image(self, clade1, clade2, dump_dir):
    ...
    with open(self.out_dir + '9205_fota_image.mbn', 'wb') as out:
      ...
      self.mbn_handle.seek(clade2end)
...
def main(argv):
  ...
  if not pe(args.old_mbn_dir + '/qdsp6sw.mbn') and pe(args.old_mbn_dir + '/qdsp6sw_paging.mbn'):
    sys.stderr.write('Could not find final images in build path %s\n' % old_mbn_dir)
    return 1

  if not pe(args.new_mbn_dir + '/qdsp6sw.mbn') and pe(args.new_mbn_dir + '/qdsp6sw_paging.mbn'):
    sys.stderr.write('Could not find final images in build path %s\n' % new_mbn_dir)
    return 1

  import_build_scripts(args.build_path)

  oldbuild = CLADE_Extractor(args.old_orig_elf, args.old_mbn_dir + '/qdsp6sw.mbn', args.old_mbn_dir + '/qdsp6sw_paging.bin', "old/")
  ...
  newbuild = CLADE_Extractor(args.new_orig_elf, args.new_mbn_dir + '/qdsp6sw.mbn', args.new_mbn_dir + '/qdsp6sw_paging.bin', "new/")
