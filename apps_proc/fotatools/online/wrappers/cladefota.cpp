...
static uint32_t write_to_file(void *buf, uint32_t size, FILE *fp) {
	unsigned rw_size;
...
static uint32_t read_from_file(void *buf, int off, uint32_t size, FILE *fp) {
	unsigned rw_size;
	if (off != -1) {
...
int clade_high_read_cb(void *buf, long int offset, unsigned size) {
  return read_from_file(buf, offset, size, cladehifp);
}

int clade_low_read_cb(void *buf, long int offset, unsigned size) {
  return read_from_file(buf, offset, size, cladelofp);
}

void init_clade_compress_callbacks(const char *high, const char *low) {
	// open high and low input files
	cladehifp = open_file(high, "rb");
	cladelofp = open_file(low, "rb");
}
...
static void clade1_decompress_9205(const char *metafile, const char *image, const char *high, const char* low, const char* dict) {
	// read metadata file
	FILE *fdst, *metafp, *imgfp;

	metafp = open_file(metafile, "rb");
	int *meta = (int*)malloc(NUM_METADATA*sizeof(int));
	read_from_file(meta, 0, NUM_METADATA*sizeof(int), metafp);
	fclose(metafp);
	...
	imgfp = open_file(image, "rb");

	// read and setup dictionaries
	uint32_t *cladedict = (uint32_t*) malloc(meta[CLADE_DICT_SZ]);
	read_from_file(cladedict, meta[CLADE_DICT_OFF], meta[CLADE_DICT_SZ], imgfp);
	...
	read_from_file(ex_hi, meta[CLADE_EX_HI_OFF], meta[CLADE_EX_HI_SZ], imgfp);

	// decompress high priority section
	dprintf("Starting hi prioritiy decompression...\n");
	init_HP_decompression((char *) ex_hi, dictionary, meta[CLADE_HIGH_SZ]);
	fdst = open_file(high, "wb");
	...
	while (curr < meta[CLADE_HIGH_SZ]) {
		readsize = (BLOCK_SZ < meta[CLADE_HIGH_SZ] - curr) ? 
			BLOCK_SZ : meta[CLADE_HIGH_SZ] - curr;
		if (readsize%BYTES_PER_LINE != 0) //align to cache line if it is not, fix case 04518131
			readsize = (readsize / BYTES_PER_LINE + 1) * BYTES_PER_LINE;
		memset(comp_buf, 0, BLOCK_SZ);
		memset(uncomp_buf, 0, BLOCK_SZ);
		read_from_file(comp_buf, meta[CLADE_COMP_OFF] + curr, readsize, imgfp);
		ret_size = clade_HP_decompress_next_blk(comp_buf, uncomp_buf, BLOCK_SZ);
		if (ret_size == 0)
			break;
		write_to_file(uncomp_buf, ret_size, fdst);
		curr += BLOCK_SZ;
	}
	fclose(fdst);
	free(ex_hi);
	dprintf("Finished hi prioritiy decompression...\n");
	char *ex_lo = (char*) malloc(meta[CLADE_EX_LO_LG_SZ] + meta[CLADE_EX_LO_SM_SZ]);
	char *ex_lo_ptr = (char*) ex_lo + meta[CLADE_EX_LO_LG_SZ];
	read_from_file(ex_lo, meta[CLADE_EX_LO_LG_OFF], meta[CLADE_EX_LO_LG_SZ], imgfp);
	read_from_file(ex_lo_ptr, meta[CLADE_EX_LO_SM_OFF], meta[CLADE_EX_LO_SM_SZ], imgfp);
	...
	while(curr_out < meta[CLADE_LOW_SZ]) {
		...
		read_from_file(comp_buf, meta[CLADE_COMP_OFF] + curr_in, readsize, imgfp);
		curr_in += readsize;
	...
	fdst = open_file(dict, "wb");
	write_to_file(cladedict, meta[CLADE_DICT_SZ], fdst);
...
static void clade1_compress_9205(
...
// Note the callback used for clade1_compress_9205. This can be reimplemented as 
// needed, but is used as FILE I/O for the simplest use case (Linux filesystem)
static void clade1_compress_9205_wrapper(
	const char *metafile, 
	const char *high, 
	const char* low, 
	const char* dict, 
	const char *out)
{
	...
	outfp = open_file(out, "wb");
	write_to_file(clade_buf, clade_seg_sz, outfp);
...
int main(int argc, const char** argv) {
	...
	if (strcmp(argv[1], "-compress") == 0) {
		...
		clade1_compress_9205_wrapper(argv[2], argv[3], argv[4], argv[5], out1);
	}
	if (strcmp(argv[1],"-decompress") == 0) {
		const char *hi = "high.bin";
		const char *lo = "low.bin";
		const char *d = "dict.bin";
		clade1_decompress_9205(argv[2], argv[3], hi, lo, d);
	}
	return 0;
