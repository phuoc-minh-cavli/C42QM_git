...
static void location_motion_tracking_callback(qapi_Location_Motion_Info_t location);
...
qapi_Loc_Start_Batching
qapi_Loc_Stop_Batching
qapi_Loc_Add_Geofences
qapi_Loc_Remove_Geofences
qapi_Loc_Get_Single_Shot
qapi_Loc_Cancel_Single_Shot
qapi_Loc_Start_Get_Gnss_Data
qapi_Loc_Stop_Get_Gnss_Data
...
The parameters for all the functions used in this application are 
initialized in the following function. The values can be changed as desired.
parameters_init

The application will execute the following functions in succession:
process_tracking
process_batching
process_geofence
process_singleshot
...
Each function exemplifies a functionality
Here is a detailed description of each:

process_tracking

This function illustrates tracking. It invokes the following APIs:
qapi_Loc_Start_Tracking
qapi_Loc_Stop_Tracking
...
This function illustrates batching. It invokes the following APIs:
qapi_Loc_Start_Batching
qapi_Loc_Stop_Batching
...
This function illustrates geofencing. It invokes the following APIs:
qapi_Loc_Add_Geofences
qapi_Loc_Remove_Geofences
...
This function illustrates single shot operation. It invokes the following APIs:
qapi_Loc_Get_Single_Shot
qapi_Loc_Cancel_Single_Shot


process_data
This function illustrates getting data operation. It invokes the following APIs:
qapi_Loc_Start_Get_Gnss_Data
qapi_Loc_Stop_Get_Gnss_Data
...
B <minInterval> <minDistance> <numberOfBatches> <timeout>
This command illustrates batching. It invokes the following APIs:
qapi_Loc_Start_Batching
qapi_Loc_Stop_Batching

G <breachTypeMask> <responsiveness> <dwellTime> <latitude> <longitude> <radius> <numberOfBreaches> <timeout>
This command illustrates geofencing. It invokes the following APIs:
qapi_Loc_Add_Geofences
qapi_Loc_Remove_Geofences

S <powerLevel>
This command illustrates single shot operation. It invokes the following APIs:
qapi_Loc_Get_Single_Shot
qapi_Loc_Cancel_Single_Shot

D <numberOfFixes> <timeout>
This command illustrates getting data operation. It invokes the following APIs:
qapi_Loc_Start_Get_Gnss_Data
qapi_Loc_Stop_Get_Gnss_Data
...
void location_app()
{
    ...
    LOC_LOG_QFLOG("15 seconds delay");
    timer_handle = start_app_timer(0, 20, &timer_init_cb);

    LOC_LOG_QFLOG("Waiting for Init (15 seconds)...");
    ...
    qapi_FS_Open("loc_app_data", QAPI_FS_O_RDONLY_E, &fd);
    if (fd >= 0) {
...
static int location_init()
{
    ...
    sLocationCallbacks.motionTrackingCb = location_motion_tracking_callback;
...
//        (UINT)clientId, (UINT)&locationOptions, (UINT)&id);
    ...
    motionOptions.motionTypeMask = 0xFF;
    ret = (qapi_Location_Error_t)qapi_Loc_Start_Motion_Tracking(
        (UINT)clientId, (UINT)&motionOptions, (UINT)&id);
...
static void location_batching_callback(
    ...
    for (i = 0; i < count; ++i) {
        ...
        memscpy(&location, sizeof(qapi_Location_t), pLocation, sizeof(location));
        log_location(location);
        pLocation++;
    }
...
/* MOTION TRACKING CALLBACK */
static void location_motion_tracking_callback(qapi_Location_Motion_Info_t info)
{
    LOC_LOG_QFLOG("LOC API CALLBACK : MOTION TRACKING");
    // todo: log_motion_info(info);
    if (gGpsVerifyTestInProgress) {
        tx_event_flags_set(gSignalLocation, 1, TX_OR);
    }
}
