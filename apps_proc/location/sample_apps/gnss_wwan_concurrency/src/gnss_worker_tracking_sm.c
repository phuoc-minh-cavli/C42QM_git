...
/***************************************************************************
    INTERNAL STATE DATA
****************************************************************************/
typedef struct gnss_worker_tracking_sm_request_params
{
    uint32 tbf;
    uint32 dbf;
    qapi_Location_Accuracy_Level_t accuracy;
    uint32 allowPriorityInversion;

} gnss_worker_tracking_sm_request_params;

typedef struct gnss_worker_tracking_sm_state_data
{
    ...
    // Mutex for blocking
    TX_MUTEX* mutex;
    TX_EVENT_FLAGS_GROUP* trackingSignal;

    // Ongoing request parameters
    gnss_worker_tracking_sm_request_params trackingRequestParams;
...
} gnss_worker_tracking_sm_state_data;

// Internal state
static gnss_worker_tracking_sm_state_data _StateData = {};

/***************************************************************************
    Internal Method Declarations
****************************************************************************/
static void _gnss_worker_tracking_sm_request_tracking_start();

static void gnss_worker_tracking_sm_backoff_timer_cb_in_timer_context(uint32 cbData);

// State change methods
// Return 0 if event is handled, -1 otherwise
static int _gnss_worker_tracking_sm_state_idle(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_waiting_for_gnss_load(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_waiting_for_ad_dl_complete(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_ad_dl_complete(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_gnss_loaded(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_backoff_bad_wwan_connection(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_backoff_weak_gnss_signals(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_backoff_wwan_high_priority(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_gnss_retry(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);
static int _gnss_worker_tracking_sm_state_gnss_fix_generated(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);

// Returns 0 if event is handled by some state, or -1 otherwise
static int _gnss_worker_tracking_sm_report_event(
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status);

static void _gnss_worker_tracking_sm_start_backoff_timer(uint32 durationSec);
static void _gnss_worker_tracking_sm_cancel_backoff_timer();

static void _gnss_worker_tracking_sm_enter_state(gnss_worker_state state);

/***************************************************************************
    INTERNAL METHODS
****************************************************************************/
static void _gnss_worker_tracking_sm_backoff_timer_cb(uint32 cbData)
{
    // stop the recurring timer
    _gnss_worker_tracking_sm_cancel_backoff_timer();

    // Report backoff expiry to be handled in current backoff state
    _gnss_worker_tracking_sm_report_event(
...
static void _gnss_worker_tracking_sm_start_backoff_timer(uint32 durationSec)
{
    ...
    if (_StateData.backoffTimerHandle == NULL)
    {
        app_utils_timer_init(
                ...
                gnss_worker_tracking_sm_backoff_timer_cb_in_timer_context,
                (uint32)&_StateData);
    }
...
void _gnss_worker_tracking_sm_update_dl_ongoing(qapi_Location_Meta_Data_Engine_Status_t status)
{
    switch (status)
    {
...
static void _gnss_worker_tracking_sm_request_tracking_start()
{
    APP_LOGEVENT_STR("Triggering Tracking request with [tbf %d] [dbf %d] [%s]",
            _StateData.trackingRequestParams.tbf,
            _StateData.trackingRequestParams.dbf,
            app_utils_accuracy_level_str(_StateData.trackingRequestParams.accuracy));

    qapi_loc_wrapper_tracking_start(
            _StateData.trackingRequestParams.tbf,
            _StateData.trackingRequestParams.dbf,
            _StateData.trackingRequestParams.accuracy);

    _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_WAITING_FOR_GNSS_LOAD);
}

// CANCEL SINGLESHOT SESSION
static void _gnss_worker_tracking_sm_request_tracking_stop()
{
    APP_LOGEVENT("Stopping Tracking Session");

    qapi_loc_wrapper_tracking_stop();
}
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    else if (event == GNSS_WORKER_EVENT_TRACKING_START_REQUEST)
    {
        APP_LOGEVENT("Trigger new tracking request in IDLE state");
        _gnss_worker_tracking_sm_request_tracking_start();
        ret = 0;
    }
    ...
    if (event == GNSS_WORKER_EVENT_ENGINE_STATUS_UPDATE)
    {
        ...
        case QAPI_LOCATION_ENGINE_STATUS_UNLOADED:
            if (gnss_worker_utils_is_gnss_high_priority())
            {
                ...
                _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_BACKOFF_BAD_WWAN_CONNECTION);
            }
            else
            {
                _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_BACKOFF_WWAN_HIGH_PRIORITY);
            }
            break;

        case QAPI_LOCATION_ENGINE_STATUS_WEAK_GNSS_SIGNALS:
        case QAPI_LOCATION_ENGINE_STATUS_FIX_FAILED:
            _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_BACKOFF_WEAK_GNSS_SIGNALS);
            break;

        case QAPI_LOCATION_ENGINE_STATUS_LOAD_SUCCESS:
            _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_GNSS_LOADED);
            break;
        ...
        case QAPI_LOCATION_ENGINE_STATUS_NTP_DL_START:
        case QAPI_LOCATION_ENGINE_STATUS_GTPWWAN_POS_START:
            _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_WAITING_FOR_AD_DL_COMPLETE);
            break;

        case QAPI_LOCATION_ENGINE_STATUS_FIX_SUCCESS:
            _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_GNSS_FIX_GENERATED);
            break;
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    if (event == GNSS_WORKER_EVENT_ENGINE_STATUS_UPDATE)
    {
        ...
        case QAPI_LOCATION_ENGINE_STATUS_LOAD_FAILED:
        case QAPI_LOCATION_ENGINE_STATUS_UNLOADED:
            if (_gnss_worker_tracking_sm_is_xtra_or_ntp_dl_ongoing() ||
                    _gnss_worker_tracking_sm_is_xtra_or_ntp_dl_failed())
            {
        ...
        case QAPI_LOCATION_ENGINE_STATUS_NTP_DL_SUCCESS:
        case QAPI_LOCATION_ENGINE_STATUS_GTPWWAN_POS_SUCCESS:
            if (!_gnss_worker_tracking_sm_is_any_ad_dl_ongoing() &&
                    !_gnss_worker_tracking_sm_is_any_ad_dl_failed())
            {
                _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_AD_DL_COMPLETE);
            }
            break;
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    if (event == GNSS_WORKER_EVENT_STATE_ENTRY)
    {
        ...
        _gnss_worker_tracking_sm_start_backoff_timer(BACKOFF_DELAY_SEC);

        _gnss_worker_tracking_sm_request_tracking_stop();
    ...
    if (event == GNSS_WORKER_EVENT_BACKOFF_TIMER_EXPIRY)
    {
        ...
        _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_GNSS_RETRY);
        ret = 0;
    }
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    if (event == GNSS_WORKER_EVENT_BACKOFF_TIMER_EXPIRY)
    {
        ...
        if (_StateData.trackingRequestParams.allowPriorityInversion)
        {
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    if (event == GNSS_WORKER_EVENT_STATE_ENTRY)
    {
        ...
        if (_StateData.backoffCount > MAX_BACKOFF_COUNT)
        {
            ...
            _gnss_worker_tracking_sm_enter_state(GNSS_WORKER_STATE_IDLE);
            return 0;
        }
        else
        {
        ...
        gnss_worker_event event, qapi_Location_Meta_Data_Engine_Status_t status)
{
    ...
    _gnss_worker_tracking_sm_update_dl_ongoing(status);

    switch(_StateData.state)
    {
    case GNSS_WORKER_STATE_IDLE:
        ret = _gnss_worker_tracking_sm_state_idle(event, status);
        break;

    case GNSS_WORKER_STATE_WAITING_FOR_GNSS_LOAD:
        ret = _gnss_worker_tracking_sm_state_waiting_for_gnss_load(event, status);
        break;

    case GNSS_WORKER_STATE_WAITING_FOR_AD_DL_COMPLETE:
        ret = _gnss_worker_tracking_sm_state_waiting_for_ad_dl_complete(event, status);
        break;

    case GNSS_WORKER_STATE_AD_DL_COMPLETE:
        ret = _gnss_worker_tracking_sm_state_ad_dl_complete(event, status);
        break;

    case GNSS_WORKER_STATE_GNSS_LOADED:
        ret = _gnss_worker_tracking_sm_state_gnss_loaded(event, status);
        break;

    case GNSS_WORKER_STATE_BACKOFF_BAD_WWAN_CONNECTION:
        ret = _gnss_worker_tracking_sm_state_backoff_bad_wwan_connection(event, status);
        break;

    case GNSS_WORKER_STATE_BACKOFF_WEAK_GNSS_SIGNALS:
        ret = _gnss_worker_tracking_sm_state_backoff_weak_gnss_signals(event, status);
        break;

    case GNSS_WORKER_STATE_BACKOFF_WWAN_HIGH_PRIORITY:
        ret = _gnss_worker_tracking_sm_state_backoff_wwan_high_priority(event, status);
        break;

    case GNSS_WORKER_STATE_GNSS_RETRY:
        ret = _gnss_worker_tracking_sm_state_gnss_retry(event, status);
        break;

    case GNSS_WORKER_STATE_GNSS_FIX_GENERATED:
        ret = _gnss_worker_tracking_sm_state_gnss_fix_generated(event, status);
        break;
...
static void _gnss_worker_tracking_sm_enter_state(gnss_worker_state state)
{
    ...
    {
    case GNSS_WORKER_STATE_IDLE:
        _gnss_worker_tracking_sm_state_idle(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_WAITING_FOR_GNSS_LOAD:
        _gnss_worker_tracking_sm_state_waiting_for_gnss_load(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_WAITING_FOR_AD_DL_COMPLETE:
        _gnss_worker_tracking_sm_state_waiting_for_ad_dl_complete(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_AD_DL_COMPLETE:
        _gnss_worker_tracking_sm_state_ad_dl_complete(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_GNSS_LOADED:
        _gnss_worker_tracking_sm_state_gnss_loaded(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_BACKOFF_BAD_WWAN_CONNECTION:
        _gnss_worker_tracking_sm_state_backoff_bad_wwan_connection(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_BACKOFF_WEAK_GNSS_SIGNALS:
        _gnss_worker_tracking_sm_state_backoff_weak_gnss_signals(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_BACKOFF_WWAN_HIGH_PRIORITY:
        _gnss_worker_tracking_sm_state_backoff_wwan_high_priority(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_GNSS_FIX_GENERATED:
        _gnss_worker_tracking_sm_state_gnss_fix_generated(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    case GNSS_WORKER_STATE_GNSS_RETRY:
        _gnss_worker_tracking_sm_state_gnss_retry(GNSS_WORKER_EVENT_STATE_ENTRY, 0);
        break;
    default:
...
void gnss_worker_tracking_sm_init()
{
    ...
    app_utils_init_signal(&_StateData.trackingSignal, "_StateData.trackingSignal");
...
// REQUEST HANDLER :: Tracking start
void gnss_worker_tracking_sm_request_tracking_start(
        uint32 tbf, uint32 dbf, qapi_Location_Accuracy_Level_t accuracy,
        uint32 allowPriorityInversion)
{
    _StateData.trackingRequestParams.tbf = tbf;
    _StateData.trackingRequestParams.dbf = dbf;
    _StateData.trackingRequestParams.accuracy = accuracy;
    _StateData.trackingRequestParams.allowPriorityInversion = allowPriorityInversion;

    int ret = _gnss_worker_tracking_sm_report_event(GNSS_WORKER_EVENT_TRACKING_START_REQUEST, 0);
    if (ret != 0)
    {
        APP_LOGEVENT_STR("Ignoring new tracking request in current state [%s], one request already ongoing",
...
{
    memset(&_StateData.trackingRequestParams, 0, sizeof(_StateData.trackingRequestParams));
...
{
    APP_LOGM("gnss_worker_tracking_sm_meta_data_cb flags %p engineStatus %d",
...
{
    _gnss_worker_tracking_sm_report_event(
            GNSS_WORKER_EVENT_NMEA_CB, 0);
}

/***************************************************************************
    Out of context calls - not in GNSS Worker thread context
****************************************************************************/
void gnss_worker_tracking_sm_backoff_timer_cb_in_timer_context(uint32 cbData)
{
    APP_LOGM("Backoff timer expiry");

    gnss_worker_send_msg_process_cb(_gnss_worker_tracking_sm_backoff_timer_cb, cbData);
}
