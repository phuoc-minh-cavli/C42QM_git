...
$Header: //components/rel/build.tn/4.0/cust/custdrivers.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/20/09   gfr     Fix MSMC1 boot up voltage for turbo mode.
02/17/09    mk     Added FEATURE_DMOV_ADM_CLK_GATING_SUPPORT and MM HEAP
                   Cache maintanence exclusion
01/13/09   abp     Removed FEATURE_WINCE around FEATURE_SEC_TIME_MU so that
                   FEATURE_SEC_TIME_MU is defined for WM platform
...
/* Define the boot-up value for MSMC1. */
#ifdef FEATURE_TURBOMODE
  #define PM_VREG_MSMC1_LEVEL          1350
#else
  #if defined(FEATURE_ROC)
     #define PM_VREG_MSMC1_LEVEL       1275
  #elif defined(FEATURE_SROC)
     #define PM_VREG_MSMC1_LEVEL       1300
  #endif
#endif
...
/* If we are booting at 528 MHz we need to increase the initial MSMC2 from
   ...
   #elif defined(FEATURE_SROC)
     #define PM_VREG_MSMC2_LEVEL     1350
   #endif
#endif
...
---------------------------------------------------------------------------*/
  ...
  /* Enable ADM clock gating for the modem processor */
    ...
    #define FEATURE_DMOV_ADM_CLK_GATING_SUPPORT
  #endif
