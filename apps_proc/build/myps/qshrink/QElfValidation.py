...
Copyright (c) 2015-2020 by QUALCOMM, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/build.tn/4.0/myps/qshrink/QElfValidation.py#2 $
$Date: 2020/09/09 $
...
def validateStructs(outputFile, hashFile,qdbFile):
  import QShrinkElf
  global qversion
  global qdbStringList,newHashStringDict
      ...
      if pointer & 0x3 == 0:
        ...
        if (pointerOffset >= 0 and 
          ...
          if __qsrnewHash__:
            ...
            string = re.sub(r'\\n', '\x0a', string)
            if (string in QShrinkElf.ElfShrinkState.stringHashDict):
              hash = QShrinkElf.ElfShrinkState.stringHashDict[string]
            else:
              print string
              print "Error, Some strings are not hashed yet"
              exit()
            string = re.sub('\x0a', r'\\n', string)
            hashstring = "".join([
                      str(QShrinkElf.convertFromElf('Elf32_Word', hash, endian=False)),
                      ":",
                      string, "\n"])
            hash=QShrinkElf.convertFromElf('Elf32_Word', hash, endian=False)
            if hash not in newHashStringDict:
              qdbStringList.append(hashstring)    
              newHashStringDict[hash] = string
          if __debug__:
            if struct['version'] == 3:
        ...
        if __qsrnewHash__:
            string = QShrinkElf.ElfShrinkState.hashStringDict[hash]
            string = re.sub('\x0a', r'\\n', string)
            hashstring = "".join([
                      str(QShrinkElf.convertFromElf('Elf32_Word', hash, endian=False)),
                      ":",
...
#=========================================================================
# Function : ReadNewHashFile 
#
# Arguments
#   file: New Hash file
#
# Description : Read New Hash file
...
def ReadNewHashFile():
  import QShrinkElf
  global newHashStringDict
  
  entryRE = re.compile('(\d+):(.*)')
  entryREsearch = entryRE.search
  re.sub = re.sub
  import os
  if os.path.exists('msg_hash_new.txt') :    
    try:
      input = open('msg_hash_new.txt', 'r+')
    except IOError:
      ...
      for line in input :
        ...
        hash = QShrinkElf.convertToElf(4, hashNum, endian=False)
        string = match.group(2)
        # if string has '\n', treat as Windows and Unix (0d 0a and 0a)
        # only save Unix version in ElfShrinkState.hashStringDict
        string = re.sub(r'\\n', '\x0a', string)
        newHashStringDict[hashNum] = string
      input.close()
#=========================================================================
# Function : CreateNewHashFile
#
# Arguments
#
# Description
#   Creates New Qshrink database file (Intermediete step for QShrink 4.0)
#
# Comments:
#  Only for testing
#=========================================================================
def CreateNewHashFile():

  global qdbStringList
  import QShrinkElf

  #Get Database GUID
  m = hashlib.sha1()
  m.update("".join(qdbStringList))
  crc = m.digest()[-2:]
  
 #write Hash File
  Hashfile_name = "".join(["msg_hash_new.txt"])  
  contentStr = [ \
    " ".join(["##################################################################","\n"]),
    ...
    " ".join(["<Date>datetime.date</Date>","\n"]),
    " ".join(["<CRC>",str(QShrinkElf.convertFromElf('Elf32_Half', str(crc), endian=True)),"</CRC>","\n"]),
    "".join(qdbStringList)
  ]
  if os.path.exists(Hashfile_name) :
    Hashfile = open(Hashfile_name, 'a')
    contentStr = ["".join(qdbStringList)]
  else:
...
def createLCSFile(lcsFile):
  ...
  try:
    ...
    input.close()
    print "Updated linker script,",lcsFile,", with latest entries from map file."
  ...
  for line in input :
     ...
     if (start == 1):
       ...
       if match is None : 
         ...
         # string = match.group(2)
         ssid=line.split("	")[0]
         ssid=ssid.replace("QSR_4_0_MSG.fmt.rodata.","")
         ssid=ssid.split(".")
         string=ssid[1]
         ssid=ssid[0]
         # print string
