...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/build.tn/4.0/myps/qshrink/elfStructs.py#1 $
...
class elfStruct(object):
    ...
    def __init__ (self, elf, offset, size, idx=0, verbose=0):
        ...
        self.idx = idx
        ## Verbosity level
        self.verbose = verbose # rmandal disable verbose by default
            ...
            if utils.isPowerOfTwo(self.p_align):
                if ((self.p_filesz != 0) and
                    ...
                    utils.raiseElfManipulatorWarning(
                        "Elf32_Phdr %i failed p_align validation. self.p_align  %d			self.p_vaddr %d		self.p_offset %d" %
                        (self.idx,self.p_align,self.p_vaddr,self.p_offset))
            else:
                utils.raiseElfManipulatorWarning(
                    "Elf32_Phdr %i failed p_align validation.  self.p_align  %d			self.p_vaddr %d		self.p_offset %d" %
                    (self.idx,self.p_align,self.p_vaddr,self.p_offset))
...
class Elf32_Shdr(elfStruct):
    ...
    def setAndValidateStructMembers(self):
        ...
        print "Section Name  Section type",self.sh_name,self.sh_type
