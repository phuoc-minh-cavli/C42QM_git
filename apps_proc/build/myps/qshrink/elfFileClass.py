...
#
#-------------------------------------------------------------------------------
#  $Header: //components/rel/build.tn/4.0/myps/qshrink/elfFileClass.py#1 $
#  $DateTime: 2020/09/09 22:06:25 $
#  $Author: pwbldsvc $
#  $Change: 26083944 $
    ...
    def moveupElfOffsetSectionByName(self, sectionName):
        ...
        if section != const.RC_ERROR:
            ...
            if segment != const.RC_ERROR:
                ...
                for ph in self.programHeaderTable:
                    if ph.p_offset < segment.p_offset and ph.p_memsz > 0 and ph.p_type != 0:
                ...
                if previousSegment:
                    ...
                    print "moveupElfOffsetSectionByName before: segment.p_offset,section.sh_offset,elfOffset %x %x %x" %(segment.p_offset,section.sh_offset,elfOffset)
                    segment.p_offset -= elfOffset
                    section.sh_offset -= elfOffset
                    print "moveupElfOffsetSectionByName after: segment.p_offset,section.sh_offset,elfOffset %x %x %x" %(segment.p_offset,section.sh_offset,elfOffset)
    ...
    def moveupSectionByName(self, sectionName, align=0):
        ...
        if section != const.RC_ERROR:
            ...
            if segment != const.RC_ERROR:
                ...
                # Find the closest segment preceding the segment to move up
                for ph in self.programHeaderTable:
                    if ph.p_vaddr < segment.p_vaddr and ph.p_type != 0:
                ...
                if previousSegment:
                    ...
                    print "moveupSectionByName before: segment.p_vaddr, segment.p_paddr, section.sh_addr offset %x %x %x %x" %(segment.p_vaddr, segment.p_paddr, section.sh_addr, offset)
                    segment.p_vaddr -= offset
                    segment.p_paddr -= offset
                    section.sh_addr -= offset
                    print "moveupSectionByName after: segment.p_vaddr, segment.p_paddr, section.sh_addr offset %x %x %x %x" %(segment.p_vaddr, segment.p_paddr, section.sh_addr, offset)
