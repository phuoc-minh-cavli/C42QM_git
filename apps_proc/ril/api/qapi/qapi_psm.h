...
*/
/*=======================================================================
  Copyright (c) 2017-2021 Qualcomm Technologies, Inc. and/or its
  subsidiaries. All rights reserved.
  Confidential - Qualcomm Technologies, Inc. and/or its affiliated
  companies - May Contain Trade Secrets
 ======================================================================*/
/*======================================================================
 *
...
============================================================================*/
/**
 *  Makes application known to the PSM server as a PSM-aware application.
 *
 ...
 *
 *  @detdesc
 *  This is the first API every PSM-aware application must call. Every application
 *  that needs network related-functionality must call this API.
 *  @par
 *  Registering a client enables the PSM-aware application to vote 
 *  for the PSM time and readiness when it is required. The PSM server uses
 *  the callback to inform the application of all PSM events.
 ...
 *  - QAPI_ERR_ESPIPE -- Some file descriptors (like pipes and FIFOs) are not seekable. @newpage
 */
...
============================================================================*/
/**
 *  Unregisters the PSM-aware application with the PSM server.
 *
 ...
 *  @par
 *  Unregistered applications cannot vote for PSM. Reregistration
 ...
 *  - #QAPI_ERR_PSM_GENERIC_FAILURE -- Communication with the server failed. @newpage
 */
 ...
 *                        active time, the next wake up time (time in PSM), and the next
 *                        wake up type. Based on the wake up type, the server decides whether
 *                        to load the modem as part of bootup.
 *
 *  @detdesc
 *  The application must pass active_time in seconds, time in PSM mode,
 *  and determine whether the next wake up is for measurement purposes or
 ...
 *
 *  @detdesc
 *  The duration for which the application wants to enter PSM mode is 
 ...
 *  @par
 *  A call to backoff will overwrite any valid PSM vote of the same client.
 *  If a call to backoff is made when there is no network, out-of-service
 *  will be treated as a vote to PSM with a default time as set in NV73784.
 *
 *  @return
 *  Returns QAPI_OK on success or a -ve error code on failure. \n
 *  - #QAPI_ERR_PSM_APP_NOT_REGISTERED -- Invalid client ID.
...
============================================================================*/
/**
 *  Requests the PSM server to load the modem if it is not already 
 *  loaded (PIL-based flavors only).
 *
 *  @param[in] client_id  Client ID obtained during registration.
 *
 *  @detdesc
 *  PSM-aware applications can load the modem dynamically based on
...
============================================================================*/
/**
 *  Application health check acknowledge API. 
 *
 *  @param[in] client_id  Client ID obtained during registration.
 *
 *  @detdesc
 ...
 *  @par
 *  This API ensures that every registered PSM-aware application is
 ...
 *  If the application fails to respond to the Health Check, it is treated
 *  as a dead application and the server votes for PSM on behalf of the dead
 *  application.
 *  @par
 *  Time in PSM is configured in NV setting NV73784 (psm_duration_due_to_oos).
 *
 *  @return
 *  Returns QAPI_OK on success or a -ve error code on failure. \n
 *  - #QAPI_ERR_PSM_APP_NOT_REGISTERED -- Invalid client ID.
 ...
 * }
 *  Calculates the optimal range of time to vote for PSM from a given set of
 *  intervals.
 *
 *  @param[out] optimal_interval  Pointer to optmial interval.
 *                                Memory must be allocated by client
 *  @param[in] client_id          Client ID obtained during registration.
 *  @param[in] interval_group     Structure containing the array of intervals for
 *                                which clients want the optimal interval.
 *
 *  @detdesc
 *  PSM Clients frequently come up with different ranges of intervals 
 *  during which the device must be online.
 *  This gives clients a way to rely on the PSM Daemon to calculate the optimal
 ...
 *  - #QAPI_ERR_PSM_APP_NOT_REGISTERED -- Invalid client ID.
 *  - #QAPI_ERR_PSM_GENERIC_FAILURE -- Future reserved error status. @newpage
 */
 ...
 * }
 *  Used by the application to indicate its intent to enter PSM mode
 *  with a time interval, that is, a start and end time for the next wake up.
 *
 *  @param[in] client_id  Client ID obtained during registration.
 *  @param[in] psm_info   Pointer to a psm_info_type structure consisting of
 *                        active time, the next wake up time (time in PSM),
 *                        and the next wake up type. Based on the wake up type,
 *                        the server decides whether to load the modem as part of bootup.
 *
 *  @detdesc
 *  The application must pass active_time in seconds, time interval for PSM,
 *  and indicate whether the next wake up is for measurement purposes or access
 *  to the network.
 *  @par
 *  The PSM Time interval is the time range within which the application needs
 *  a device to be online. The actual absolute time when the device shall be
 *  online will be within this interval. The PSM Daemon shall optimally calculate
 *  the exact time based upon votes of all the clients.
 *  @par
 *  PSM time can be accepted in either a broken down format or in seconds.
 *  A PSM-aware application blocks PSM entry if either of the Enter PSM APIs are not 
 *  called indefinitely.
 *
 *  @return
 *  Returns QAPI_OK on success or a -ve error code on failure. \n
