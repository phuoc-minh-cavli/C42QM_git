...
static qurt_mutex_t  *pDalTimetickLegacySyncObj = NULL;
/*=============================================================================
...
static void timetick_attach(void)
{
  ...
  if (phDalTimetickLegacyHandle == NULL)
  {
    ...
    if(DAL_SUCCESS == DalTimetick_Attach("SystemTimer", &phTempTimetick))
    {
       ...
       if(pDalTimetickLegacySyncObj == NULL)
       {
          pDalTimetickLegacySyncObj = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
          if (pDalTimetickLegacySyncObj == NULL)
          {
            ERR_FATAL
	  ...
	  qurt_mutex_init(pDalTimetickLegacySyncObj);
       }
...
timetick_type timetick_get_ms( void )
{
  ...
  timetick_attach(); 
  qurt_mutex_lock(pDalTimetickLegacySyncObj);  
  ticks = timetick_get_sclk64();
  ms = (timetick_type) cvt_timetick64_to_ms(ticks);
  qurt_mutex_unlock(pDalTimetickLegacySyncObj);
  return ms;
