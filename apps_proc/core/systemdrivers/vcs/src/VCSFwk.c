...
/*  
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/vcs/src/VCSFwk.c#1 $
...
)
{
  ...
  eResult = VCS_GetPropertyValue("VCSStubConfig", &PropVal);
  if (eResult == DAL_SUCCESS)
  {
    pVCSStubConfig = (VCSStubType *)PropVal;
    PlatformInfo = DalPlatformInfo_Platform();

    if ((PlatformInfo == DALPLATFORMINFO_TYPE_RUMI) &&
        (pVCSStubConfig->bRUMI == TRUE))
    {
      VCS_GLOBAL_FLAG_SET(STUB_API);

      ULOG_RT_PRINTF_0(
        pDrvCtxt->hVCSLog,
        "DALLOG Device VCS: VCS_GLOBAL_FLAG_STUB_API set - stubbing out on RUMI.");
    }
  }
}
