...
$Header: //components/rel/core.tx/6.0/systemdrivers/hal/clk/hw/v1/HALclkStromerPLL.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------- 
12/28/18   shm     Separate APCS PLL control for updating POST_DIV
...
/*
 * HAL_clk_StromerAPCSPLLControl
 *
 * Control function array for the Stromer/Tesla PLLs.
 */
HAL_clk_SourceControlType HAL_clk_StromerAPCSPLLControl =
{
  /* .Enable          = */ HAL_clk_StromerPLLEnable,
  /* .Disable         = */ HAL_clk_StromerPLLDisable,
  /* .IsEnabled       = */ HAL_clk_StromerPLLIsEnabled,
  /* .Config          = */ HAL_clk_StromerPLLConfig,
  /* .ConfigPLL       = */ HAL_clk_StromerAPCSPLLConfigPLL,
  /* .DetectPLLConfig = */ HAL_clk_StromerPLLDetectPLLConfig,
  /* .EnableVote      = */ HAL_clk_StromerPLLEnableVote,
  /* .DisableVote     = */ HAL_clk_StromerPLLDisableVote,
  /* .IsVoteEnabled   = */ HAL_clk_StromerPLLIsVoteEnabled,
...
/* ===========================================================================
**  HAL_clk_StromerAPCSPLLConfigPLLSlew
**    This is a separate function to support POST_DIV update while slewing.
**    Only a switch between div-1 and div-2 is glitch-free and is supported.
**    If POST_DIV is increasing, it will applied before L_VAL update.
**    If POST_DIV is decreasing, it will applied after L_VAL update.
**
**  Returns:
**    boolean - TRUE if successfully configured.
**              FALSE if unable to detect that the PLL successfully locked.
...
static boolean HAL_clk_StromerAPCSPLLConfigPLLSlew
(
  HAL_clk_SourceDescType      *pSource,
  const HAL_clk_PLLConfigType *pConfig,
  boolean                      bTriggerUpdate
...
{
  uint32                  nModeVal, nUserVal, nPrevPostDiv;
  HAL_clk_PLLContextType *pCtxt;
  boolean                 bResult;
  ...
  /*
   * Get the current POST_DIV value
   */
  nUserVal = HAL_CLK_PLL_HWIO_IN(PLL_USER_CTL, pCtxt);
  nPrevPostDiv = ((nUserVal & HWIO_FMSK(PLL_USER_CTL, POST_DIV_RATIO))
    >> HWIO_SHFT(PLL_USER_CTL, POST_DIV_RATIO)) + 1;

  /*
   * Support only a switch between div 1 and 2
   */
  if (nPrevPostDiv > 2 || pConfig->nPostDiv > 2)
  {
    return FALSE;
  }
  else
  {
    nUserVal &= ~HWIO_FMSK(PLL_USER_CTL, POST_DIV_RATIO);
    nUserVal |=
      ((pConfig->nPostDiv - 1) << HWIO_SHFT(PLL_USER_CTL, POST_DIV_RATIO));
  }

  /*
   * If POST_DIV is increasing
   */
  if (nPrevPostDiv == 1 && pConfig->nPostDiv == 2)
  {
  ...
  /*
   * If POST_DIV is decreasing
   */
  if (nPrevPostDiv == 2 && pConfig->nPostDiv == 1)
  {
    HAL_CLK_PLL_HWIO_OUT(PLL_USER_CTL, pCtxt, nUserVal);
  }
...
} /* END HAL_clk_StromerAPCSPLLConfigPLLSlew */
...
/* ===========================================================================
**  HAL_clk_StromerAPCSPLLConfigPLL
**
**  Returns:
**    boolean - TRUE if successfully configured. FALSE otherwise.
** ======================================================================== */

static boolean HAL_clk_StromerAPCSPLLConfigPLL
(
  HAL_clk_SourceDescType      *pSource,
  const HAL_clk_PLLConfigType *pConfig,
  HAL_clk_SourceConfigMode     eMode
)
{
  ...
  {
    bResult = HAL_clk_StromerAPCSPLLConfigPLLSlew(pSource, pConfig, TRUE);
  }
  else if (eMode == HAL_CLK_SOURCE_CONFIG_MODE_PENDING)
  {
    bResult = HAL_clk_StromerAPCSPLLConfigPLLSlew(pSource, pConfig, FALSE);
  }

  return bResult;

} /* END HAL_clk_StromerAPCSPLLConfigPLL */
