...
$Header: //components/rel/core.tx/6.0/systemdrivers/hal/qgic/src/HALqgic.c#1 $
...
void HAL_qgic_Reset (void)
{
  ...
  uint32 n,nBanks=0;
   ...
   nBanks =  HWIO_INF(GICD_TYPER,IT_LINES);
...
void HAL_qgic_ClearInterrupts
(
  void 
)
{
  uint32 *nAddr,nBanks=0;
  uint32 i, nMask = 0xFFFFFFFF;
  ...
  nBanks =  HWIO_INF(GICD_TYPER,IT_LINES);
    ...
    nAddr = (uint32*)HAL_QGIC_INT2ADDR(GICD_ICPENDRn, 0);
