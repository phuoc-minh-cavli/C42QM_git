...
-------------------------------------------------------------------------------
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/icb/src/common/ul_arb.c#1 $
 ...
 --------   ---     -----------------------------------------------------------
 06/15/16   kk      Replacing DalSys Heap APIs with Kernel APIs
 03/03/16   sds     Make the /icb/arbiter interface NPA vector compliant
 01/18/16   sds     Allow remoting.
 12/15/15   sds     Fix KW errors.
                                          ...
                                          npa_client_handle  client )
{
   ...
   uint64 uRequest = (uint64)(NPA_PENDING_REQUEST( client ).state) * sizeof(npa_resource_state)/ 
                                          ...
                                          void *reference )
{
   ...
   aMasterSlave = (ICBArb_MasterSlaveType *)reference;
   u32NumMasterSlave = data / sizeof(ICBArb_MasterSlaveType);
   
   /* Allocate client vector of master client handles */
   client->resource_data = malloc( ALIGN8( sizeof(ICBArb_MasterRouteVectorType) ) + 
...
static void _ul_destroy_client( npa_client *client )
{
   ...
   if ( pMasterRouteVector )
   {
      ...
      free( (void *)pMasterRouteVector );
      pMasterRouteVector = NULL;
   }
                                       ...
                                       uint32 u32NumRequest )
{
   ...
   npa_issue_vector_request( client, 
                             (u32NumRequest * sizeof(ICBArb_RequestType))/sizeof(npa_resource_state),
...
/*==============================================================================
  ...
  FUNCTION   icbarb_fill_client_vector

  DESCRIPTION
     Wrapper function for client creation.
     See also ICBARB_CREATE_CLIENT_VECTOR macro in icbarb.h

     Note:
     This function doesn't do much and the prototype no longer makes any sense.
     Given that it's part of the interface, the prototype can't really change.
     We could have handled the unused items in the macro itself, but this
     presented two problems. Clients had to create the psVector structure
     on the stack, and if we don't use it in some way, it'll generate compiler
     warnings for them. We could get the clients to change, but that's more
     work than it's worth for existing code.
     This is part of a deprecated API path in any case, and should not be used.

  PARAMETERS
     psVector - unused
     aMasterSlave - a pointer to the master/slave request array
     npa_callback - unused
...
==============================================================================*/
void *icbarb_fill_client_vector
(
...
)
{
   return npa_create_sync_client_ex( "/icb/arbiter", 
                                     pszClientName,
                                     NPA_CLIENT_VECTOR, 
                                     u32NumMasterSlave * sizeof(ICBArb_MasterSlaveType),
                                     (void *)aMasterSlave );
}
...
)
{
   ...
   aClkNameArray = (const char **)malloc( uNumClocks * sizeof(*aClkNameArray) );
   if ( NULL == aClkNameArray )
   {
   ...
   else
   {
       ...
       */
      if ( *puNumResource == uNumClocks && uNumClocks != 0)
      {
...
)
{
   ...
   if( NULL != notify_list )
   {
      icb_notify_type *notify = (icb_notify_type *)malloc( sizeof(icb_notify_type) ); 
      if( NULL != notify )
      {
