 ...
 * 
 * $Header: //components/rel/core.tx/6.0/systemdrivers/icb/src/common/axicfgint.c#1 $ 
 * $DateTime: 2020/01/30 22:49:35 $ 
 * $Author: pwbldsvc $ 
 * 
 * when         who     what, where, why
 * ----------   ---     ---------------------------------------------------
 * 06/15/2016   kk      Replacing DalSys Heap APIs with Kernel APIs
 * 05/03/2016   sds     Add support for power domains during configuration
 * 01/05/2016   sds     Correct BIMC configuration in QosSetLocal().
 * 12/14/2015   sds     Clear the correct size for NoC QoS overrides.
...
void _AxiCfgInt_InitCb( void *context, unsigned int event_type, void *data, unsigned int data_size )
{
       ...
       bus_idx++ )
  {
    ...
    for( pwr_idx = 0;
         pwr_idx < SocInfo.pBusList[bus_idx]->uNumPwrDomains;
         pwr_idx++ )
    {
      AxiCfgOs_GetPowerDomainId( pBus->aPwrDomainNames[pwr_idx],
                                 &pBus->aPwrDomainIds[pwr_idx] );
    }
    for( clk_idx = 0;
...
void _AxiCfgInt_EnableAllConfigClocks( void )
{
       ...
       bus_idx++ )
  {
    ...
    for( pwr_idx = 0;
         ...
         pwr_idx++ )
    {
      AxiCfgOs_EnablePowerDomain( pBus->aPwrDomainIds[pwr_idx] );
    }
    for( clk_idx = 0;
         clk_idx < SocInfo.pBusList[bus_idx]->uNumEnCfgClks;
         clk_idx++ )
...
void _AxiCfgInt_DisableAllConfigClocks( void )
{
       ...
       bus_idx++ )
  {
    ...
    for( pwr_idx = 0;
         ...
         pwr_idx++ )
    {
      AxiCfgOs_DisablePowerDomain( pBus->aPwrDomainIds[pwr_idx] );
    }
  }
}
...
)
{
  ...
  switch( pMaster->pBus->busType )
  {
    ...
    case ICB_BUS_NOC:
    {
      ...
      memset(pOverride, 0, sizeof(icb_noc_master_defaults_type));
  ...
  do
  {
        ...
        ( NULL != pMaster->pQosPortIds ) )
    {
      ...
      if( NULL == pQosPolicy )
      {
        ...
        {
          free(pMaster->pQosPolicyOverride);
      ...
      else
      {
        ...
        * Otherwise, just overwrite it. */
        if( NULL == pMaster->pQosPolicyOverride )
        {
          ...
          pMaster->pQosPolicyOverride = malloc(size);
          if( NULL == pMaster->pQosPolicyOverride )
          {
            break;
          }
