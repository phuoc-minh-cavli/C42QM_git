 ...
 *
 * $Header: //components/rel/core.tx/6.0/systemdrivers/icb/src/common/ul_int.c#1 $
...
)
{
  ...
  if( pRequestList->uEntryCount + 1 >= pRequestList->uListSize )
  {
    ...
    pRequestList->aRequestList = (ul_bw_request_type **)malloc((pRequestList->uListSize + UL_BW_REQUEST_LIST_SIZE) * sizeof(void*));
    if(NULL == pRequestList->aRequestList)
    {
