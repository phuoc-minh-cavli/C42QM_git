...
/*
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/clock/src/ClockFwk.c#1 $
 ...
 */
static qurt_mutex_t hClockLiteMutex;

static boolean bClockLiteMutexIsInit = FALSE;
...
)
{
  ...
  if (eRes != DAL_SUCCESS)
  {
    ERR_FATAL("Clock_TransferReferenceCount failed.",0,0,0);
  }
...
{
  /*-----------------------------------------------------------------------
   * To enable/disable the PRNG clock, the PRNG driver may call this API
   * (via the Clock_EnablePRNG()) at any time before, during or after the DAL
   * Init. The PRNG competes with DAL for exclusive access to the clock
   * driver context. The mutex thus needs to be initialized during the first
   * call.
   *
   * The atomic section for mutex init disables interrupts temporarily.
   * The double layered check for hClockLiteMutexIsInit ensures interrupts
   * are disabled only once during the initialization. Also ensures that
   * the mutex is initialized only once.
   * Every subsequent call checks bClockLiteMutexIsInit and avoids needless
   * disabling of interrupts.
   *-----------------------------------------------------------------------*/
   unsigned long    interrupt_flag=0;
   if(!bClockLiteMutexIsInit)
   {
     INTLOCK_SAVE(interrupt_flag);
     if(!bClockLiteMutexIsInit)
     {
       qurt_mutex_init(&hClockLiteMutex);
       bClockLiteMutexIsInit = TRUE;
     }
     INTFREE_RESTORE(interrupt_flag);
   }

   qurt_mutex_lock(&hClockLiteMutex);
...
void Clock_ReleaseDrvCtxt(void)
{
  ...
  /*-----------------------------------------------------------------------*/
	...
	qurt_mutex_unlock(&hClockLiteMutex);
