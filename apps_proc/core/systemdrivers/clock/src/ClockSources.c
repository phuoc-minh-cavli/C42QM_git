...
/*
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/clock/src/ClockSources.c#1 $
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
  /* If don't have any active frequency config, use the calibration config */
  /*-----------------------------------------------------------------------*/

  if(pSource->pActiveFreqConfig == NULL)
  {
    pSource->pActiveFreqConfig = pCalibrationFreqConfig;
  }
...
)
{
  ...
  {
    ERR_FATAL("Clock_GetPropertyValue failed.",0,0,0);
  ...
  {
    ERR_FATAL("Clock_InitSources found 0 sources.",0,0,0);
    return DAL_ERROR;
  }
  ...
  pDrvCtxt->aSources = (ClockSourceNodeType*)malloc(nSize);

  if (pDrvCtxt->aSources == NULL)
  {
    ERR_FATAL("Unable to allocate %d bytes for %d sources.",
      nSize, pDrvCtxt->nNumSources,0);
    return eResult;
  }
  ...
  for (nSourceIndex = 0; nSourceIndex < pDrvCtxt->nNumSources; nSourceIndex++)
  {
    ...
    if (pSourceFreqConfig)
    {
      ...
      {
        ERR_FATAL("Unable to detect calibration frequency config for source[%s].",
          pSource->szName,0,0);
...
)
{
      ...
      (pSource->nFlags & CLOCK_FLAG_SOURCE_CALIBRATED) == 0)
  {
    ...
    if (eResult != DAL_SUCCESS)
    {
      ERR_FATAL("Unable to calibrate and slew source[%s]",
        pSource->szName,0,0);
  ...
  else
  {
    ...
    else if (pSource->pBSPConfig->nConfigMask & CLOCK_CONFIG_PLL_FSM_MODE_ENABLE)
    {
      if (bStub != TRUE)
      {
        ...
        if (bResult != TRUE)
        {
          ERR_FATAL("Unable to enable source[%s] vote",
            pSource->szName,0,0);
    ...
    else
    {
      ...
      if (bStub != TRUE)
      {
        ...
        if (bResult != TRUE)
        {
          ERR_FATAL("Unable to enable source[%s]",
            pSource->szName,0,0);
