...
/*
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/clock/src/ClockDriver.c#1 $
...
)
{
  ...
  /*-----------------------------------------------------------------------*/
   ...
   pDrvCtxt->LockObj = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
   if(pDrvCtxt->LockObj == NULL)
      {
         ERR_FATAL("Failed to allocate memory to the synchronization object", 0, 0, 0);
      }
   qurt_mutex_init(pDrvCtxt->LockObj);
  /*-----------------------------------------------------------------------*/
  /* Initialize the log.                                                   */
  /*-----------------------------------------------------------------------*/
  ...
  {
    eResult = ULogFront_RealTimeInit (&pDrvCtxt->hClockLog, "Clock Log",
  ...
  if (eResult != DAL_SUCCESS)
  {
    ...
    ERR_FATAL("Clock_InitImage failed.",0,0,0);
    return eResult;
  }
...
)
{
  ...
  if (eResult != DAL_SUCCESS)
  {
    ERR_FATAL("ClockStub_InitImage failed.",0,0,0);
    return eResult;
  }
...
)
{
  ...
  if (pDrvCtxt->nNumClockDomains > 0)
  {
    ...
    pDrvCtxt->aClockDomains = (ClockDomainNodeType*)malloc(nSize);
	
    if(pDrvCtxt->aClockDomains == NULL)
    {
      ERR_FATAL("Unable to allocate %lu bytes for %lu clock domains.",
        nSize, pDrvCtxt->nNumClockDomains,0);
      return eResult;
    }
  ...
  else
  {
    ...
    {
      ERR_FATAL("HAL_clk_GetNumberOfClocks returned 0.",0,0,0);
      return DAL_ERROR;
    }

    nSize = pDrvCtxt->nNumClocks * sizeof(*pDrvCtxt->aClocks);

    pDrvCtxt->aClocks = (ClockNodeType*)malloc(nSize);

    if (pDrvCtxt->aClocks == NULL)
    {
      ERR_FATAL("Unable to allocate %lu bytes for %lu clocks.",
        nSize, pDrvCtxt->nNumClocks,0);
      return eResult;
    }
  ...
  if (pDrvCtxt->nNumPowerDomains > 0)
  {
    ...
    pDrvCtxt->aPowerDomains = (ClockPowerDomainNodeType*)malloc(nSize);

    if (pDrvCtxt->aPowerDomains == NULL)
    {
      ERR_FATAL("Unable to allocate %lu bytes for %lu power domains.",
        nSize, pDrvCtxt->nNumPowerDomains,0);
      return eResult;
    }
         ...
         (nClockDomainIndex < pDrvCtxt->nNumClockDomains))
  {
    ...
    if (pHALClock != NULL)
    {
      ERR_FATAL("Clock_InitCore exceeded its memory allocation for clocks.",0,0,0);
      return DAL_ERROR;
    }
  ...
  {
    ERR_FATAL("Clock_InitCore exceeded its memory allocation for clock domains.",0,0,0);
    return DAL_ERROR;
  }
  ...
  {
    ERR_FATAL("Clock_InitCore received too few clock domains from the HAL.",0,0,0);
    return DAL_ERROR;
  }
  ...
  {
    ERR_FATAL("Clock_InitCore exceeded its memory allocation for power domains.",0,0,0);
    return DAL_ERROR;
  }
  ...
  {
    ERR_FATAL("Clock_InitCore received too few power domains from the HAL.",0,0,0);
    return DAL_ERROR;
  }
  ...
  if (eResult != DAL_SUCCESS)
  {
    ...
    ERR_FATAL("Clock_InitVoltage failed.",0,0,0);
    return eResult;
  }
...
)
{
  ...
  free (pDrvCtxt->aClocks);
  pDrvCtxt->aClocks = NULL;

  free (pDrvCtxt->aClockDomains);
  pDrvCtxt->aClockDomains = NULL;

  free (pDrvCtxt->aSources);
  pDrvCtxt->aSources = NULL;

  free (pDrvCtxt->aPowerDomains);
  ...
  qurt_mutex_destroy(pDrvCtxt->LockObj);
  ...
  if ((pClockDomain->nReferenceCount + pClockDomain->nReferenceCountSuppressible) == 1)
  {
    if (CLOCK_FLAG_IS_SET(pClockDomain, DOMAIN_WAIT_FOR_ROOT_OFF))
    {
      if (HAL_clk_IsClockDomainSupported(pClockDomain->HALHandle, HAL_CLK_CLOCK_DOMAIN_API_ISON))
      {
        ...
        if (nTimeout == 150)
        {
          ERR_FATAL("Clock domain root did not turn off after all domain clocks are off",0,0,0);
...
)
{
  ...
  if (pClock->nReferenceCount == 0)
  {
    ...
    if(HAL_clk_IsClockSupported (pClock->HALHandle, HAL_CLK_CLOCK_API_ISON))
    {
      ...
      {
        ULOG_RT_PRINTF_1 (pDrvCtxt->hClockLog,
          "WARNING:Unable to turn ON clock: %s.", pClock->szName);
      }
    }
  }
...
)
{
  ...
  if (nSrcIdx != 0xFF)
  {
    ...
    pClockUARTConfig = (ClockMuxConfigType*)malloc(nSize);
    if (pClockUARTConfig == NULL)
    {
      ERR_FATAL("Unable to allocate %lu bytes for UART clock configuration.",nSize,0,0);
    ...
    else
    {
       ...
       */
      free (pClockUARTConfig);
      pClockUARTConfig = NULL;
      nResultFreqHz = 0;
      eResult = DAL_ERROR_INTERNAL;
    }
  }
