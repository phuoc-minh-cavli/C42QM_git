...
============================================================================
  $Header: //components/rel/core.tx/6.0/systemdrivers/ipcint/src/DalIPCInt.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
============================================================================*/
...
DALResult IPCInt_GetTargetData(IPCIntClientCtxt *pCtxt)
{
  ...
  {
    ERR_FATAL("IPCInt_GetTargetData: Device could not get SourceProc",0,0,0);
    return DAL_ERROR;
  ...
  {
     ERR_FATAL("IPCInt_GetTargetData: Device could not get IPCINT_PHYSICAL_ADDRESS",0,0,0);
    return DAL_ERROR;
  }
  ...
  {
    ERR_FATAL("IPCInt_GetTargetData: Device could not get IPCINT_OFFSET",0,0,0);
    return DAL_ERROR;
  }
  ...
  if(DAL_DeviceAttach(DALDEVICEID_HWIO,&pHWIOHandler) == DAL_SUCCESS)
  {
    ...
    {
       ERR_FATAL("IPCInt_GetTargetData: could not get the virtual address",0,0,0);
      return DAL_ERROR; 
    }    
...
IPCInt_DeviceInit(IPCIntClientCtxt *pCtxt)
{
  ...
  pDevice->IPCIntSyncObject = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
  if(pDevice->IPCIntSyncObject == NULL)
  {
    ERR_FATAL(
      "IPCInt_DeviceInit: Device could not allocate memory to the sync object"
      , 0, 0, 0);  }
  qurt_mutex_init(pDevice->IPCIntSyncObject);
