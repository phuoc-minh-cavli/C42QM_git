...
$Header: //components/rel/core.tx/6.0/systemdrivers/chippower/src/DalChipPower.c#1 $
=============================================================================*/
...
)
{
  ...
  if(pNode != NULL)
  {
    ...
    {
      pChipPowerEventObj->cbFunc(pChipPowerEventObj->cbFuncCtx,
                                      eventType,
                                      NULL,NULL);
      pNode = ChipPowerDequeueNode(&pDrvCtxt->eventQueue);
      if(pNode == NULL)
...
void
ChipPower_PrepareForPowerEvent (void)
{
  qurt_mutex_t*    mSyncObj;
  unsigned long    interrupt_flag=0;
  unsigned int     fiq_flag=0;
  ...
  /*
   * Need to disable FIQ as well or the dog bark ISR hits us.  TODO - Need
   * either DALSYS or dog API to disable FIQ or the dog bark.
   */
   fiq_flag = qurt_fiq_lock_save();
   //To avoid compiler warning for unused variables
   if(fiq_flag){}

  /* Indicate to reset logger that is a normal reset. */
  err_reset_detect_disable();

  /* Ensure that the dog is running. */
  dog_autokick(FALSE);

  /*
   * Select boot partition 0 to prevent going to the SDI image upon reset.
   */
  req.disable_wd_dbg = 1;
  req.boot_partition_sel = 0;
  tz_config_hw_for_offline_ram_dump_STM(&req);

} /* END ChipPower_PrepareForPowerEvent */


static void
ChipPower_InitInterface(ChipPowerClientCtxt* pclientCtxt)
{
...
)
{
  ...
  static ChipPowerDrvCtxt drvCtxt = {
    {ChipPower_DriverInit, ChipPower_DriverDeInit},
    1,
