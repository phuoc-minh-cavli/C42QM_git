...
$Header: //components/rel/core.tx/6.0/systemdrivers/pmic/npa/src/pm_npa_device_vs.c#1 $
...
void pm_npa_vs_kvps_create_client_fcn(npa_client *client)
{
  ...
  memset((void *)(client->resource_data), 0, sizeof(pm_npa_vs_kvps));
}
...
npa_resource_state pm_npa_vs_kvps_update_fcn(npa_resource *resource, npa_client_handle client)
{
  ...
  memscpy((void *)client_req, (uint32)sizeof(pm_npa_vs_kvps), (const void*)NPA_PENDING_REQUEST(client).pointer.vector, (uint32)sizeof(pm_npa_vs_kvps));
  ...
  memscpy((void *)kvps, (uint32)sizeof(pm_npa_vs_kvps), (const void*)client_req, (uint32)sizeof(pm_npa_vs_kvps));

    /* Reqd request Only*/
  reqd_kvps = (pm_npa_vs_kvps *)resource->required_state_vector;
  if (req_type == NPA_CLIENT_VECTOR)
  {
    memscpy((void *)reqd_kvps, (uint32)sizeof(pm_npa_vs_kvps), (const void*)client_req, (uint32)sizeof(pm_npa_vs_kvps));
  }
  ...
  PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s ActiveSet: enable = %d, pin_ctrl_value = %d, uV = %d, current = %d", 
            client->resource_name, client->name, kvps->sw_enable_value, kvps->pin_ctrl_enable_value, 
            kvps->uv_value, kvps->current_value);
            
  //Print final aggregated required set values   
   PM_LOG_MSG_DEBUG("ResName=%s ClientName=%s RequiredSet: enable = %d, pin_ctrl_value = %d, uV = %d, current = %d", 
            client->resource_name, client->name, reqd_kvps->sw_enable_value, reqd_kvps->pin_ctrl_enable_value, 
            reqd_kvps->uv_value, reqd_kvps->current_value);
