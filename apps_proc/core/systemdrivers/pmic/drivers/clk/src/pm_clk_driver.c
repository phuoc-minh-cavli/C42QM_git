...
$Header: //components/rel/core.tx/6.0/systemdrivers/pmic/drivers/clk/src/pm_clk_driver.c#1 $
...
void pm_clk_driver_init(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint8 pmic_index)
{
    ...
    /* Added this check to avoid initializing the clock driver for CLOCK_CALRC2 peripheral. */
    /*YODA CALRC clocks does not need  driver initialization*/
    ...
    if (clk_ptr == NULL)
    {
        ...
        memset((void *)(clk_ptr->periph_subtype), 0, sizeof(clk_ptr->periph_subtype));
...
void pm_clk_periph_map_init(pm_clk_data_type *clk_ptr)
{
   ...
   memset((void *)(clk_ptr->clk_perph_index), PM_CLK_INVALID_DATA, sizeof(clk_ptr->clk_perph_index));
