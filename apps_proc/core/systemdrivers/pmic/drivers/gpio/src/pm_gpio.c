...
$Header: //components/rel/core.tx/6.0/systemdrivers/pmic/drivers/gpio/src/pm_gpio.c#1 $ 


when        who          what, where, why
--------    ---          ----------------------------------------------------------     
04/10/12        hs               Changed the type for gpio from int to uint8.
...
pm_err_flag_type pm_gpio_config_bias_voltage(uint8 pmic_chip, pm_gpio_perph_index gpio, pm_gpio_volt_src_type voltage_source)
{
    ...
    // select GPIO type
    errFlag = pm_gpio_configure_type(pmic_chip, gpio, PM_GPIO_DIG_OUT);
    ...
    //errFlag = pm_gpio_set_ext_pin_config( me, PM_GPIO_EXT_PIN_ENABLE, gpio );//no need to do this, since we will enable the GPIO at the end.
    //if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
                                                  ...
                                                  pm_gpio_src_config_type source)
{
    ...
    mask = is_periph_unified_reg==0 ? 0xF : 0x87;
    ...
    //errFlag = pm_gpio_set_ext_pin_config(PM_GPIO_EXT_PIN_ENABLE, gpio);
    //if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag;
    ...
    switch (source)
    {
        case PM_GPIO_SOURCE_DTEST1:
            ...
            //select data based on subtype for backward compatibility 
            break;
...
pm_err_flag_type pm_gpio_set_inversion_configuration(uint8 pmic_chip, pm_gpio_perph_index gpio, boolean  inversion)
{
    ...
    {
         dig_out_src_ctl = gpio_ptr->gpio_register->base_address + (pm_register_address_type)(gpio_ptr->gpio_register->peripheral_offset*gpio) +
...
static pm_err_flag_type pm_gpio_status_get_v2(uint8 pmic_chip, pm_gpio_perph_index gpio , pm_gpio_status_type *gpio_status)
{
    ...
    if ( gpio >= gpio_ptr->num_of_peripherals )
    {
        return PM_ERR_FLAG__PAR1_OUT_OF_RANGE;
    }
...
pm_err_flag_type pm_gpio_get_pin_status(uint8 pmic_chip, pm_gpio_perph_index gpio, pm_pin_status *pin_status)
{
    pm_err_flag_type         errFlag = PM_ERR_FLAG__SUCCESS;
    pm_gpio_data_type *gpio_ptr = pm_gpio_get_data(pmic_chip);
    pm_register_data_type    data;
    ...
    STATUS = gpio_ptr->gpio_register->base_address + (pm_register_address_type)(gpio_ptr->gpio_register->peripheral_offset*(uint8)gpio) +
              gpio_ptr->gpio_register->status;
              
    errFlag = pm_comm_read_byte_mask(gpio_ptr->comm_ptr->slave_id, STATUS, 0xFF, &data,0);

    if(errFlag != PM_ERR_FLAG__SUCCESS) return errFlag ;

    *pin_status = (data & 0x1) ? PM_PIN_STATUS_HIGH : PM_PIN_STATUS_LOW;
