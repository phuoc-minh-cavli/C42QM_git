...
$Header: //components/rel/core.tx/6.0/systemdrivers/pmic/drivers/pon/src/pm_pon.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/06/16   kcs     Added pm_pon_trigger_enable (CR - 1079165) 
06/27/16   rk      MSM9206: API porting from DALsys to Threadx (CR - 1034553) 
07/08/15   aab     Updated to support PON GEN2 peripheral
...
pm_pon_init(void)
{
  ...
  pm_malloc( sizeof(pm_pon_data_type), (void**)&pm_pon_data_arr);

  pon_peripheral_info.base_address = pon_peripheral_index * 0x0100;

  //enabled for 1st pmic only for now. Need to update if more than one pmic supported.
  for(pmic_index = 0; pmic_index < 1; pmic_index++)
  {
  ...
  /* Read Warm Reset reason */
     err_flag = pm_comm_read_byte_array(PMIC_A_SLAVEID_PRIM, PMIO_PON_WARM_RESET_REASON1_ADDR, 2, warm_reset_reason, 0);  //0x080a ===>WARM_RESET_REASON1 : 0x0800 + 0x000A (0x080A) on 9x35
                                                                                                                          //this Reg does not exist on 9x55
  if (err_flag != PM_ERR_FLAG__SUCCESS)
  {
    return err_flag;
  }
...
pm_pon_overtemp_reset_cfg(uint8 pmic_device_index, pm_pon_reset_cfg_type overtemp_cfg)
{
  ...
  if (  (pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_type     != PM_HW_MODULE_PON) || 
        (pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_subtype  != PM_HW_MODULE_PON_LV_PON) )
  {
...
pm_pon_get_fsm_current_state(uint8 pmic_device_index, pm_pon_fsm_status_type *pon_fsm_status)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;
  pm_register_data_type fsm_status_reg = NULL;
  ...
  err_flag = pm_comm_read_byte(slave_id, PMIO_PON_GEN2_FSM_STATUS_ADDR, (uint8*)fsm_status_reg, 0);

  //Check FSM State
  pon_fsm_status = (pm_pon_fsm_status_type*)(fsm_status_reg & PMIO_PON_GEN2_FSM_STATUS_BMSK);  
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  pm_register_address_type pon_reg_addr = NULL;
  uint32 slave_id = 0;
  ...
  if ( pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_subtype  == PM_HW_MODULE_PON_LV_PON )
  {
     pon_reg_addr = PMIO_PON_PON_REASON1_ADDR;
			    ...
			    (pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_subtype  == PM_HW_MODULE_PON_LV_PON_GEN2_NO_XVDD_RB))      //GEN2
  {
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  pm_register_address_type poff_reg_addr = NULL;
  uint32 slave_id = 0;
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  pm_register_address_type warm_reset_reason_reg_addr = NULL;
  uint32 slave_id = 0;
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  pm_register_address_type soft_reset_reason_reg_addr = NULL;
  uint32 slave_id = 0;
...
//Applicable only for PON GEN2 
pm_err_flag_type 
pm_pon_get_on_reason(uint8 pmic_device_index, pm_pon_on_state_reason_type* reason)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;
...
//Applicable only for PON GEN2
pm_err_flag_type 
pm_pon_get_off_reason(uint8 pmic_device_index, pm_pon_off_state_reason_type* reason)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;
...
//Applicable only for PON GEN2
pm_err_flag_type 
pm_pon_get_fault_reason(uint8 pmic_device_index, pm_pon_fault_state_reason_type* reason)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;
...
//Applicable only for PON GEN2
pm_err_flag_type 
pm_pon_get_s3_reset_reason(uint8 pmic_device_index, pm_pon_s3_reset_reason_type* reason)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;
  ...
  if (err_flag != PM_ERR_FLAG__SUCCESS)
  {
    return err_flag =  PM_ERR_FLAG__PAR1_OUT_OF_RANGE;
  }
...
//Applicable only for PON GEN2
pm_err_flag_type 
pm_pon_gen2_get_all_pon_reasons(uint8 pmic_device_index, uint64* pon_reasons1, uint64* pon_reasons2)  
{
  pm_err_flag_type err_flag = PM_ERR_FLAG__SUCCESS;
  uint32 slave_id = 0;

  if ((pon_reasons1 == NULL) || (pon_reasons1 == NULL) )
  {
    return PM_ERR_FLAG__INVALID_POINTER;
  }
  ...
  err_flag |= pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_PON_REASON1_ADDR, 8, (uint8*) pon_reasons1, 0);
  err_flag |= pm_comm_read_byte_array(slave_id, PMIO_PON_GEN2_FAULT_REASON1_ADDR, 4, (uint8*) pon_reasons2, 0);
...
//Applicable only for PON GEN1/GEN2: Added needed update
pm_err_flag_type 
...
pm_pon_config_uvlo(uint8 pmic_device_index, uint32 uvlo_rising_thresh_voltage_in_mv, uint32 hyst_voltage_in_mv)
{
  ...
  if ( (pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_type     != PM_HW_MODULE_PON) || 
       (pm_pon_data_arr[pmic_device_index]->preph_info.peripheral_subtype  != PM_HW_MODULE_PON_LV_PON) )
  {
      return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;  //This API is only applicable for peripheral type: PM_HW_MODULE_PON_LV_PON
  }
