...
$Header: //components/rel/core.tx/6.0/systemdrivers/GPIOInt/src/GPIOIntFwk.c#1 $
...
{               
    ...
    pclientCtxt = (GPIOIntClientCtxt*)malloc(sizeof(GPIOIntClientCtxt));
    if(pclientCtxt == NULL)
    {
      ERR_FATAL(
             "Failed to allocate memory to GPIO Client Context"
             , 0, 0, 0);
    }
    memset(pclientCtxt,0,sizeof(GPIOIntClientCtxt));
