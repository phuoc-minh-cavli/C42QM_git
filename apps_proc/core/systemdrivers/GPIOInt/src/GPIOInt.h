...
$Header: //components/rel/core.tx/6.0/systemdrivers/GPIOInt/src/GPIOInt.h#1 $
 ...
 */
#define MAX_INTERRUPT_LOG_ENTRIES  50

/*
 * As an upper limit we keep the maximum number of GPIOs as 63.
 * This number needs to be updated if the GPIO number exceeds 63.
 */
#define MAX_NUMBER_OF_GPIOS        63
...
struct GPIOIntDevCtxt
{
  ...
  uint32                         Reserved[16];
  qurt_mutex_t                   *GPIOIntSyncObject;
...
/*=============================================================================
  ...
  DESCRIPTION   This function registers the isr routine to a given GPIO 
  interrupt. Whenever an interrupt is fired the isr routine registered by the 
  user is called.  This API is an extension to the traditional RegisterIsr API
  to allow for priority to be set.  If there are no direct connects available,
  this API will fail.
  ...
  uint32 param :               The uint32 parameter given to the isr routine 
                                when called.
  uint32 nFlags :              The uint32 flag passed in as part of the extension.
                               Flag supported:
                               GPIOINT_PRIORITY_FLAG - Add priority to be set in
  ...
  to it which is different from the current one in attempt at event registration. 
  An error is also returned if the GPIO pin is out of bounds or not supported.
  Also returns DAL_ERROR if there are no available direct connects if PRIO flag
  is set.
  DAL_SUCCESS :
  Returns a success if the ISR registration was a success.
...
DALResult GPIOIntr_RegisterIsrEx
(
  GPIOIntClientCtxt * pCtxt,
  uint32 gpio,
  GPIOIntTriggerType trigger,
