...
$Header: //components/rel/core.tx/6.0/systemdrivers/tlmm/hw/v2/HALtlmm.c#1 $
...
void HAL_tlmm_SetPull( uint32 nGpioNumber, HAL_tlmm_PullType ePull )
{
  ...
  if( nGpioNumber < tHALTLMMInit.nNumGpio )
  {
    ...
    nValue |= ((ePull << HWIO_TLMM_GPIO_CFGn_GPIO_PULL_SHFT) & HWIO_TLMM_GPIO_CFGn_GPIO_PULL_BMSK);
...
)
{
  ...
  nValue |= ((eDirection <<  HWIO_TLMM_GPIO_CFGn_GPIO_OE_SHFT) & HWIO_TLMM_GPIO_CFGn_GPIO_OE_BMSK);
...
)
{
  ...
  nValue |= ((eDrive << HWIO_TLMM_GPIO_CFGn_DRV_STRENGTH_SHFT) & HWIO_TLMM_GPIO_CFGn_DRV_STRENGTH_BMSK);
...
)
{
  ...
  nValue |= ((nFuncVal << HWIO_TLMM_GPIO_CFGn_FUNC_SEL_SHFT) & HWIO_TLMM_GPIO_CFGn_FUNC_SEL_BMSK);
