...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/systemdrivers/tlmm/src/DALTLMM.c#2 $
...
/*==========================================================================
  ...
  Static functions for key generation

  ==========================================================================*/
static uint32 DALGpioSetKey
(
  uint32        nGpioNumber,
  DALGpioIdType nGpioId
)
{
  if(gnGpioAdder >= UINT16_MAX)
  {
    gnGpioAdder = 0;
  }
  return (nGpioNumber | (((0x0000FFFF & nGpioId) + (++gnGpioAdder)) << 16));
}

static uint32 DALGpioCreateKey
(
  uint32 nPin
)
{
  if(g_Value >= UINT32_MAX)
  {
    g_Value = 1;
  }
  return (((((uint16)~(nPin) & 0xFF) << 8)) | g_Value++);
}
...
DALResult TLMM_InitPlatformIO(TLMMClientCtxt *pCtxt)
{
  ...
  eResult = DALSYS_GetPropertyValue( pCtxt->pDevCtxt->hProp, "TlmmPlatformGroups", 0, &tPropVar );
            ...
            (PlatformInfo.fusion == pDeviceInfo[nIdx].Platform.fusion))
        {
          if (pDeviceInfo[nIdx].nKVPS != 0)
          {
            if (DalPlatformInfo_GetKeyValue(hPlatformInfo, 
                                            (DalPlatformInfoKeyType)pDeviceInfo[nIdx].nKVPS,
                                            &nKVPSType) == DAL_SUCCESS)
            {
    ...
    if (FALSE == bHasPlatIO)
    {
      ...
      ULOG_RT_PRINTF_1 (pCtxt->hTLMMLog,
    ...
    if (NULL != pPlatform)
    {
       ...
       */
      eResult = DALSYS_GetDALPropertyHandleStr(pPlatform->pszPlatform, pCtxt->pDevCtxt->hPlatIO);
      ...
      if( DAL_SUCCESS == eResult )
      {
        ...
        {
          eResult = DALSYS_GetDALPropertyHandleStr(pPlatform->pszPlatConfigs, pCtxt->pDevCtxt->hPlatIOCfgs);
...
DALResult TLMM_DeviceInit(TLMMClientCtxt *pCtxt)
{
  ...
  //ULog Initialization
  if(DAL_SUCCESS == ULogFront_RealTimeInit (&pCtxt->hTLMMLog, "TLMM Log",
                            TLMM_LOG_SIZE,
                            ULOG_MEMORY_LOCAL, ULOG_LOCK_OS))
  {
    ULogCore_HeaderSet (pCtxt->hTLMMLog,
  ...
  if( !pgtState )
  {
     ...
     */
   pCtxt->pDevCtxt->hTlmmSyncObj = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
   if (pCtxt->pDevCtxt->hTlmmSyncObj == NULL)
      {
         ERR_FATAL(
             "Failed to allocate memory to synchronization object"
             , 0, 0, 0);
      }
	qurt_mutex_init(pCtxt->pDevCtxt->hTlmmSyncObj);
    /* 
     * Ensure synchronization for critical initialization parameters.
     */
    qurt_mutex_lock(pCtxt->pDevCtxt->hTlmmSyncObj);
    /* 
    ...
    if( DAL_SUCCESS != eResult )
    {
      ...
      qurt_mutex_unlock(pCtxt->pDevCtxt->hTlmmSyncObj);
      return(DAL_ERROR);
    }
     ...
     */
    ganGpioIdUsers = (DALGpioSecAccessType*)malloc((gnTotalGPIO*sizeof(DALGpioSecAccessType)));
    if(ganGpioIdUsers == NULL)
    {
      ERR_FATAL("TLMM_DeviceInit: Unable to create global array for checking GPIO ID usage.",0,0,0);
      qurt_mutex_unlock(pCtxt->pDevCtxt->hTlmmSyncObj);
      return(DAL_ERROR);
    }
    ...
    {
      ERR_FATAL(
          "TLMM_DeviceInit: Failed to retrieve the register base offset.",0,0,0);
    ...
    if( DAL_SUCCESS != eResult )
    {
      ULOG_RT_PRINTF_0 (pCtxt->hTLMMLog,
    ...
    {  
      ERR_FATAL(
          "TLMM_DeviceInit: Failed to initialize shared memory data structure.",0,0,0);
...
/*==========================================================================
  ...
  DALResult TLMM_ConfigGpioInternal
(
 TLMMClientCtxt*     pCtxt,   
...
DALResult  TLMM_GpioOutInternal
( 
 TLMMClientCtxt*    pCtxt,   
 DALGpioSignalType  eGpioConfig,   
 DALGpioValueType   eValue,
 ...
 )
{
  ...
  if( bHasPlatIO )
  {
          ...
          &tPropVar ))
    {
      ...
      {
        ganGpioIdUsers[nGpioNumber].nGpioKey = DALGpioSetKey(nGpioNumber, nTempGpioId);
...
/*==========================================================================
  ...
  FUNCTION      TLMM_GetGpioPinId

  DESCRIPTION   See DALTLMM.h

  ==========================================================================*/

DALResult TLMM_GetGpioPinId
( 
 TLMMClientCtxt*   pCtxt,
 uint32            nGpioNumber,
 DALGpioIdType*    pnGpioId
 )
{
  ...
  if(ganGpioIdUsers[nGpioNumber].nGpioKey == 0)
  {
    ganGpioIdUsers[nGpioNumber].nGpioKey = DALGpioCreateKey(nGpioNumber);
    *pnGpioId = ganGpioIdUsers[nGpioNumber].nGpioKey;
    qurt_mutex_unlock(pCtxt->pDevCtxt->hTlmmSyncObj);
    return(DAL_SUCCESS);
  }
  else
  {
    *pnGpioId = NULL;
    ULOG_RT_PRINTF_1 (pCtxt->hTLMMLog,
        "TLMM_GetGpioPinId:  GPIO ID for GPIO %d is already allocated to another entity.",
        nGpioNumber);
  }
  qurt_mutex_unlock(pCtxt->pDevCtxt->hTlmmSyncObj);
...
DALResult TLMM_ReleaseGpioPinId
(
 TLMMClientCtxt*          pCtxt, 
 uint32                   nGpioNumber, 
 DALGpioIdType            nGpioId
