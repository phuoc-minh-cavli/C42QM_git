...
#----------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tx/6.0/storage/efs/build/fs_build_config.py#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $ $Change: 22400605 $
#
...
def fs_get_build_defines (env):
  ...
  if ('USES_RMTS_GOLDEN_COPY_FROM_FILES_LIST' in env) :
    cpp_flags += ['FEATURE_EFS_RMTS_GOLDEN_COPY_FROM_FILES_LIST']

  if ('USES_APPS_CONTROLS_EFS_SYNC' in env):
    cpp_flags += ['FEATURE_EFS_APPS_CONTROLS_EFS_SYNC']
	
  if ('USES_ENABLE_EFS_COMPRESSION' in env):
    cpp_flags += ['FEATURE_EFS_ENABLE_EFS_COMPRESSION']

  if ('USES_APPS_CONTROLS_EFS_SYNC' in env) or \
     ('USES_EFS_MFSE' in env):
    cpp_flags += ['FEATURE_EFS_MFSE']

  if ('USES_RMTS_ERASE_EFS_ON_EFS_RESET_API' in env):
    cpp_flags += ['FEATURE_EFS_RMTS_ERASE_EFS_ON_EFS_RESET_API']
  ...
  if ('USES_STORAGE_NAND' in env)     or \
    ...
    if not env.has_key('USES_THREADX'):
      cpp_flags += ['FEATURE_FS_BENCHMARK']

  if ('USES_EFS_QUOTA_SETUP_FOR_CLIENTS' in env):
    cpp_flags += ['FEATURE_EFS_QUOTA_SETUP_FOR_CLIENTS']
