...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/inc/fs_zip.h#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2015-02-23   dks   Add support to compress EFS RAM into shared RAM on sync
2015-01-17   dks   Add API to compress/decompress to output buffer fully.
...
enum fs_zip_return_status
{
  ...
  FS_ZIP_INSUFFICIENT_BUF       = -7, /* Output buffer insufficient */
  FS_ZIP_INVALID_PARAM          = -8, /* Incorrect parameters supplied */
};
...
/**************************************************************************//**
 * @par Name:
 *   fs_zip_compress_complete - Compress the data in input buffer fully into
 *   the output buffer is output buffer has enough space. Since it works on
 *   block input, and not stream, it does not require a set_input call.
 *
 * @par Synopsis:
 *  @code
 *    #include "fs_zip.h"
 *    enum fs_zip_return_status
 *    fs_zip_compress_complete (fs_zip_handle handle, void *input,
 *                              uint32 input_size, void *output,
 *                              uint32 output_buff_size,
 *                              uint32 *available_output);
 *  @endcode
 *
 * @par Description:
 *  Compress the data set in input buffer and return the output if possible
 *  in the output buffer.
 *
 * @param handle
 *  The zip stream handle pointer returned by fs_zip_open.
 ...
 * @return
 * Returns a status message of type enum fs_zip_return_status;
 * On succes to store all of compressed data in the output buffer returns
 * FS_ZIP_STATUS_FINISHED.
 *
******************************************************************************/
enum fs_zip_return_status
fs_zip_compress_complete (fs_zip_handle handle, void *input, uint32 input_size,
                          void *output, uint32 output_buff_size,
                          uint32 *available_output);
...
/**************************************************************************//**
 * @par Name:
 *   fs_zip_decompress_complete - Decompress the data in input buffer fully
 *   into the output buffer is output buffer has enough space. Since it works
 *   on block input, and not stream, it does not require a set_input call.
 *
 *
 * @par Synopsis:
 *  @code
 *    #include "fs_zip.h"
 *    enum fs_zip_return_status
 *    fs_zip_decompress_complete (fs_zip_handle handle, void *input,
 *                  uint32 input_size, void *output, uint32 output_buff_size,
 *                  uint32 *available_output);
 *
 *  @endcode
 *
 ...
 * @return
 * Returns a status message of type enum fs_zip_return_status;
 * On succes to store all of decompressed data in the output buffer returns
 * FS_ZIP_STATUS_FINISHED.
******************************************************************************/
enum fs_zip_return_status
fs_zip_decompress_complete (fs_zip_handle handle, void *input,
...
/**************************************************************************//**
 * @par Name:
 *   fs_zip_get_heap_usage - Get the current size of heap in use.
 *
 * @par Synopsis:
 *  @code
 *    #include "fs_zip.h"
 *    enum fs_zip_return_status fs_zip_get_heap_usage (fs_zip_handle handle,
 *                                                     uint32 *heap_used);
 *  @endcode
 *
 * @par Description:
 *  Get the current size of heap in use.
 *
 * @param buffer
 *  The zip stream handle pointer returned by fs_zip_open.
 *
 * @param heap_used
 *  Current heap usage for the handle.
 *
 * @return
 *  FS_ZIP_STATUS_SUCCESS on success to get the heap usage; else error code.
 *
******************************************************************************/
enum fs_zip_return_status fs_zip_get_heap_usage (fs_zip_handle handle,
                                                 uint32 *heap_used);
