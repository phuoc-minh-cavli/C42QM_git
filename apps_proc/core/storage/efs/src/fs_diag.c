 ...
 *
 *
 *  Copyright (C) 2002-2016, 2019  QUALCOMM Technologies, Incorporated.
 *  All Rights Reserved.
 *
 ...
 *====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
 ...
 /*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_diag.c#2 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/23/19   eo      fix signed and unsigned arithmetic diag read/write handler.
04/14/16   rp      Add EFS golden copy creation from files list.
04/12/16   rp      Check for buffer overreads in FS Diag handlers.
04/12/16   rp      revert SPC code check in FS diag handlers.
10/21/15   mj      Added SPC code check in FS diag handlers.
...
DECLARE_DIAG_FN (fs_diag_make_golden_copy_v2_handler);
DECLARE_DIAG_FN (fs_diag_make_golden_copy_v2_get_status_handler);
...
static const diagpkt_user_table_entry_type fs_diag_tbl[] =
{
  ...
  {EFS2_DIAG_MAKE_GOLDEN_COPY_V2,
   EFS2_DIAG_MAKE_GOLDEN_COPY_V2,
   fs_diag_make_golden_copy_v2_handler},
  {EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS,
   EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS,
   fs_diag_make_golden_copy_v2_get_status_handler},
};
...
fs_diag_read_handler (VOID_PACKED_PTR req_ptr, uint16 pkt_len)
{
  ...
  desc_entry *slot;
  uint32 base_length = offsetof (efs2_diag_read_rsp_type, data);
  uint32 rsp_len = base_length, new_rsp_len;
  uint32 bytes_left, bytes_read;
  fs_off_t offset, new_offset;
  fs_ssize_t result;
  ...
  if (slot->file == NULL) {
    /* Unbuffered. */
    new_offset = efs_lseek (slot->fd, offset, SEEK_SET);
    ...
    result = efs_read (slot->fd, (void *) response->data, bytes_left);
  } else {
    ...
    result = efs_fread ((void *) response->data, 1, bytes_left, slot->file);
  }
  ...
  if (result < 0) {
    ...
    bytes_read = (uint32)result;
    response->bytes_read = result;
    response->diag_errno = 0;
    if (bytes_read < bytes_left) {
      new_rsp_len = base_length + bytes_read;
      diagpkt_shorten (response, new_rsp_len);
    }
  }
...
fs_diag_write_handler (VOID_PACKED_PTR req_ptr, uint16 pkt_len)
{
  ...
  fs_off_t offset, new_offset;
  uint32 nbyte, bytes_written = 0, bytes_left;
  char      *write_locn;
  desc_entry *slot;
  uint32 recd_pkt_len, base_pkt_len;
  fs_ssize_t result;
  ...
  recd_pkt_len = (uint32)pkt_len;

  base_pkt_len = offsetof (efs2_diag_write_req_type, data);
  if (recd_pkt_len < base_pkt_len) {
    response->diag_errno = fs_get_diag_errno (EINVAL);
    response->bytes_written = 0;
    goto write_end;
  }

  nbyte = recd_pkt_len - base_pkt_len;
  ...
  while (bytes_left > 0) {
    if (slot->file == NULL) {
      result = efs_write (slot->fd, (void *) write_locn, bytes_left);
    } else {
      result = efs_fwrite ((void *) write_locn, 1, bytes_left,
          slot->file);
    }
...
VOID_PACKED_PTR
fs_diag_make_golden_copy_v2_handler (VOID_PACKED_PTR req_ptr, uint16 pkt_len)
{
  efs2_diag_make_golden_copy_v2_req_type *request;
  efs2_diag_make_golden_copy_v2_rsp_type *response;
  uint16 rsp_len = sizeof (efs2_diag_make_golden_copy_v2_rsp_type);
  enum fs_golden_copy_type golden_copy_type;
  const char *request_path;
  int result;

  EFS_BAB_DEBUG_PRINTF ("_GOLDEN_COPY_V2");

  FS_MSG_HIGH_1("make fsg : %d", pkt_len);
  ...
  /* Check for minimum length of packet */
  if (!fs_diag_is_valid_pkt_len (pkt_len,
          sizeof (efs2_diag_make_golden_copy_v2_req_type)))
  {
    return diagpkt_err_rsp (DIAG_BAD_LEN_F, req_ptr, pkt_len);
  }

  FSD_MEMOIZED_PACKET_CHECK (req_ptr, pkt_len, EFS2_DIAG_MAKE_GOLDEN_COPY_V2);

  request = (efs2_diag_make_golden_copy_v2_req_type *) req_ptr;
  request_path = LONG_STRING_CAST (request->path);

  /* Check for a NULL char */
  FSD_CHECK_NULLS (request, efs2_diag_make_golden_copy_v2_req_type,
                   path, pkt_len, 1);

  response = (efs2_diag_make_golden_copy_v2_rsp_type *)
    diagpkt_subsys_alloc (FS_SUBSYS,
                          EFS2_DIAG_MAKE_GOLDEN_COPY_V2,
                          rsp_len);
  if (response == NULL)
  ...
  golden_copy_type = (enum fs_golden_copy_type)request->golden_copy_type;

  if (fs_rmts_can_create_golden_copy() == 1)
  {
    result = efs_make_golden_copy_no_wait (request_path, golden_copy_type);
    if (result == 0)
    {
      response->golden_copy_status = 1;
      response->diag_errno = 0;
    }
    else
    {
      response->golden_copy_status = 0;
      response->diag_errno = fs_get_diag_errno (efs_errno);
      /* Since we failed to make the golden copy, erase the item file to
       * allow future packets to create a golden copy. */
      result = efs_unlink (MADE_GOLDEN_COPY_PATH);
  ...
  {
    FS_MSG_ERROR_0("make fsg error, fsg was already made");
  }
...
VOID_PACKED_PTR
fs_diag_make_golden_copy_v2_get_status_handler (VOID_PACKED_PTR req_ptr,
                                                uint16 pkt_len)
{
  efs2_diag_make_golden_copy_v2_get_status_req_type *request;
  efs2_diag_make_golden_copy_v2_get_status_rsp_type *response;
  uint16 rsp_len = sizeof (efs2_diag_make_golden_copy_v2_get_status_rsp_type);
  enum fs_golden_copy_status status;
  const char *request_path;
  int result;

  EFS_BAB_DEBUG_PRINTF ("_GOLDEN_COPY_V2_GET_STATUS");
  FS_MSG_HIGH_1("fsg get status: %d", pkt_len);
  ...
  /* Check for minimum length of packet */
  if (!fs_diag_is_valid_pkt_len (pkt_len,
          sizeof (efs2_diag_make_golden_copy_v2_get_status_req_type)))
  {
    return diagpkt_err_rsp (DIAG_BAD_LEN_F, req_ptr, pkt_len);
  }

  FSD_MEMOIZED_PACKET_CHECK (req_ptr, pkt_len,
    EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS);

  request = (efs2_diag_make_golden_copy_v2_get_status_req_type *) req_ptr;
  request_path = LONG_STRING_CAST (request->path);

  /* Check for a NULL char */
  FSD_CHECK_NULLS (request, efs2_diag_make_golden_copy_v2_get_status_req_type,
                   path, pkt_len, 1);

  response = (efs2_diag_make_golden_copy_v2_get_status_rsp_type *)
    diagpkt_subsys_alloc (FS_SUBSYS,
                          EFS2_DIAG_MAKE_GOLDEN_COPY_V2_GET_STATUS,
                          rsp_len);
  if (response == NULL)
  ...
  result = efs_golden_copy_no_wait_get_status (request_path, &status);
  if (result == 0)
  {
    response->golden_copy_status = (uint8)status;
    ASSERT (response->golden_copy_status == 1 ||
            response->golden_copy_status == 2);
    response->diag_errno = 0;
  }
  else
  {
