 ...
 * Copyright (C) 2001-2013,2015-2016 QUALCOMM Technologies, Inc.
 *
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_buffer.h#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
struct fsb_implementation {
  ...
  int (*sync_start) (fsb_t imp, int do_force_sync, int is_shutdown_sync);
  ...
  int (*get_buf_for_fsg) (fsb_t imp, uint8 **buf, uint32 *buf_size,
                          uint32 *hdr_offset);

  /* Called to write an buffer to Golden Copy */
  int (*write_to_fsg) (fsb_t imp, uint8 *buf, uint32 buf_size,
                       uint32 partition);
};
