...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_lib.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when          who     what, where, why
--------      ---     ------------------------------------------------------
2016-06-01    rp      Avoid delete and recreate of EFS private directories.
2016-06-01    rp      Fix critical section logic in efs_iter.
 ...
 *--------------------------------------------------------------------------*/
#define SAFE_DIR_NAME       "/safe"
#define SFS_DIR_NAME        "sfs"
#define SAFE_DIR_NAME_LEN   5
#define SFS_DIR_NAME_LEN    3
          ...
          const void *param)
{
  ...
  int callback_fail_cnt = 0, callback_efs_errno = 0, local_efs_errno;
  dword symlink_nbr = 0;
  int make_callback;
  EFSDIR *dirp;
  struct fs_dirent *direntp;
  ...
  if (result < 0)
  {
    local_efs_errno = efs_errno;
    FS_MSG_ERROR_2 ("%d,%d priv-dir mkdir failed", result, local_efs_errno);
    FS_PATH_BUFF_UNLOCK ();
    return -1;
  }
...
Cleanup:
  ...
  {
    local_efs_errno = efs_errno;
    FS_MSG_ERROR_2 ("%d,%d priv-dir deltree failed",
                    deltree_result, local_efs_errno);
  }
                       ...
                       int ignore_device_specific_files)
{
  ...
  do
  {
    ...
    if (ignore_device_specific_files)
    {
      if ((dent != NULL) &&
          (strlen (dent->d_name) == SFS_DIR_NAME_LEN) &&
          (strlen (rec_dirp->parent_dir) == SAFE_DIR_NAME_LEN))
      {
        if ((memcmp (dent->d_name, SFS_DIR_NAME, SFS_DIR_NAME_LEN) == 0) &&
            (memcmp (rec_dirp->parent_dir, SAFE_DIR_NAME,
                     SAFE_DIR_NAME_LEN) == 0))
        {
          /* Skip Device Specific folders */
          continue;
        }
      }
    }
...
/* Reads full file in one shot given that max transaction size is bigger than
 * supplied buffer size and buffer can hold the full file data.
 */
int
efs_read_full_file (const char *path, void *data, fs_size_t length)
{
  struct fs_stat fbuf;
  struct fs_statvfs fsbuf;
  int result;

  result = efs_statvfs (path, &fsbuf);
  if (result != 0)
  {
    goto End;
  }

  result = efs_stat (path, &fbuf);
  if (result != 0)
  {
    goto End;
  }

  if (length <= fsbuf.f_maxwrite && length >= fbuf.st_size)
  {
    result = efs_get (path, data, fbuf.st_size);
  }
  else
  {
    efs_errno = FS_ERANGE;
...
int
efs_is_dir_exists (const char *path)
{
