...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_device_efs_flash.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
static fs_os_signal_handle_type fs_device_efs_flash_signal;
...
fs_device_efs_flash_init (void)
{
  ...
  fs_os_signal_init(&fs_device_efs_flash_signal);
}
...
fs_device_flash_suspend_erase (efs_device_t base_dev)
{
  ...
  if (result == FS_DEVICE_NOT_SUPPORTED)
  {
    ...
    do
    {
      fs_util_wait (&fs_device_efs_flash_signal, 20);
