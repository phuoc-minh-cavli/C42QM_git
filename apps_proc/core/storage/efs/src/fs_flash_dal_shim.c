 ...
 *
 * Copyright (c) 2008-2013, 2016 QUALCOMM Technologies, Inc.
 * All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_flash_dal_shim.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when         who     what, where, why
--------     ---     ----------------------------------------------------------
2016-09-22   sb      Clean-Up
2016-08-02   svl     DAL SYS and DAL FW to QURT porting
...
struct flash_client_device_data
{
  ...
  /* Memory buffers for page operations */
  struct flash_mem_desc_buf page_buff_desc[FLASH_MAX_NUM_BUFS];

  uint32 buff_desc_initialized;
};
...
efs_flash_init_buffer_descriptor (uint32 op_type, uint32 page_buff_addr,
                              ...
                              struct flash_mem_desc_buf *buffer_desc,
                              struct flash_page_iovec  *page_data_iovec)
{
  struct flash_mem_desc_buf *main_desc_buff;
  struct flash_mem_desc_buf *spare_desc_buff;

  main_desc_buff = &buffer_desc[MAIN_BUFF_DESC];

  ASSERT (main_desc_buff != NULL);
  main_desc_buff->VirtualAddr = page_buff_addr;
  main_desc_buff->user = page_data_iovec->total_page_count;

  spare_desc_buff = &buffer_desc[SPARE_BUFF_DESC];
  ...
  switch (op_type)
  {
    case FLASH_READ_SPARE:
      ...
      main_desc_buff->VirtualAddr = FLASH_MEM_ADDR_NOT_SPECIFIED;
    ...
    case FLASH_READ_MAIN:
      ...
      spare_desc_buff->VirtualAddr = FLASH_MEM_ADDR_NOT_SPECIFIED;
...
flash_nand_is_page_erased (fs_device_t self, page_id page)
{
  ...
  efs_flash_init_buffer_descriptor(read_type,
                          (uint32)(self->page_buf), (uint32) NULL,
                           &client_datap->page_buff_desc[0],
                          &page_data_iovec);
  desc_buff = &client_datap->page_buff_desc[MAIN_BUFF_DESC];
  ...
  uint32 spare_udata_blen)
{
  ...
  efs_flash_init_buffer_descriptor(read_type, (uint32) NULL,
                           (uint32) spare_udata,
                            &client_datap->page_buff_desc[0],
                           &page_data_iovec);
  spare_desc_buff = &client_datap->page_buff_desc[SPARE_BUFF_DESC];
  ASSERT (spare_desc_buff != NULL);
...
flash_shim_open (char *partition_name, int whichflash)
{
  ...
  qurt_mem_region_attr_t mem_info;    /* HAL Workspace mem handle */
  qurt_mem_region_t handle_mem;  /* HAL Workspace mem info */
  (void) whichflash;
                                       ...
                                       (const unsigned char *)partition_name))
  {
    ...
    mem_info.virtaddr=FLASH_MEM_ADDR_NOT_SPECIFIED;
    mem_info.physaddr=FLASH_MEM_ADDR_NOT_SPECIFIED;
    if(QURT_EOK == qurt_mem_region_create(  &handle_mem,
                                            fs_device->info.partition.total_page_size_bytes,
                                            ...
                                            &mem_info))
      {
        ...
        nand_page_buf = (uint32) mem_info.virtaddr;
