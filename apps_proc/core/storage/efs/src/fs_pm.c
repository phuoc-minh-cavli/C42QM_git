...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_pm.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when          who     what, where, why
--------      ---     ------------------------------------------------------
08/08/16      rp      Enhance fs-osal to support both rex and qurt.
05/23/16      rp      Support OEMs to create their own EFS-golden-copy.
04/14/16      rp      Add EFS golden copy creation from files list.
12/27/15      mj      Remove function calls with ASSERT macro.
04/23/15      vm      Force the EFS Sync during shutdown.
    ...
    efs_device_t   dev)
{
  ...
  fs_os_signal_init(&gc->timer_wait_sigh);
      ...
      case FS_PM_ERASE:
      {
        ...
        else if (allow_erase_pending == FALSE)
        {
          if(gc->erase_state == FS_PM_ERASE_SUSPEND)
          {
            ...
            {
              fs_util_wait(&gc->timer_wait_sigh, 20);
  ...
  if( gc->erase_state == FS_PM_ERASE_BUSY)
  {
    ...
    if(fs_device_erase_status (gc->dev) == FS_DEVICE_DONE)
    {
      ...
      if(do_work && (work > 0))
      {
        do
        {
          fs_util_wait(&gc->timer_wait_sigh, FS_NOR_ERASE_INCREMENT);
...
static int
fs_pm_sync_start (fs_pm_t pm_data, int do_force_sync,
...
fs_pm_nor_finish_current_erase (fs_pm_flash_t pm)
{
  ...
  if (pm->erase_state == FS_PM_ERASE_BUSY)
  {
    do
    {
      fs_util_wait (&pm->timer_wait_sigh, 20);
...
static int
fs_pm_invalid_get_buf_for_fsg (fs_pm_t pm_data, uint8 **buf, uint32 *buf_size,
                               uint32 *hdr_offset)
{
  fs_pm_flash_t gc = (fs_pm_flash_t) pm_data;

  (void) buf; (void) buf_size; (void) hdr_offset;
...
static int
fs_pm_invalid_write_to_fsg (fs_pm_t pm_data, uint8 *buf, uint32 buf_size,
                            uint32 partition)
{
  fs_pm_flash_t gc = (fs_pm_flash_t) pm_data;

  (void) buf; (void) buf_size; (void) partition;
...
fs_pm_init_read_only_ops (struct fs_pm_ops *ops)
{
  ...
  ops->get_buf_for_fsg = fs_pm_invalid_get_buf_for_fsg;
  ops->write_to_fsg = fs_pm_invalid_write_to_fsg;
}
