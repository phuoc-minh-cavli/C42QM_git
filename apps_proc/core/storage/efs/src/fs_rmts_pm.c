...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_rmts_pm.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
2016-04-06   rp    Fix klockwork errors.
2015-12-27   mj    Remove function calls with ASSERT macro.
2015-04-23   vm    Force the EFS Sync during shutdown.
2015-03-12   dks   Fix logic of detecting compression in existing image.
2015-02-23   vm    Add support to compress EFS RAM into shared RAM on sync
2015-02-17   dks   Support shared ram of size different from modem ram size.
...
#ifdef FEATURE_EFS_ENABLE_EFS_COMPRESSION
  #ifndef SCL_SHARED_EFS_RAM_SIZE
    #error "EFS compression should not be enbled in standalone mode"
  #endif

  /* Min size + buffer of 1K for any future need like alignmment etc. */
  #define FS_RMTS_ZIP_COMPRESSION_MEMORY (FS_ZIP_COMPRESSION_MEMORY + 1024)

  /* Heap for compression/decompression operations */
  static uint8 fs_rmts_pm_heap [FS_RMTS_ZIP_COMPRESSION_MEMORY];
#else
  static uint32 fs_rmts_pm_heap[1];
#endif
...
static void fs_rmts_pm_hookup_ram_addr (struct fs_rmts_pm *pm,
                                       struct fs_rmts_superblock *sb);
...
static void fs_rmts_pm_copy_fsg_to_fs2(struct fs_rmts_pm *pm);
...
fs_rmts_pm_compute_sync_iovec_sizes (struct fs_rmts_pm *pm)
{
  ...
  pm->full_sync_size_bytes =
                      ((sb->data_region.start_sector -
                        FS_RMTS_PM_IMG_HEADER_START_SECTOR) *
                        sb->sector_size) + pm->shared_data_region_size_bytes;
}
...
fs_rmts_pm_init (efs_device_t dev)
{
  ...
  pm->shared_ram_size = pm->dev->priv.rmts_dev.shared_ram_size;
  if (pm->shared_ram_size > pm->ram_size)
  {
    FS_ERR_FATAL ("%d %d Shared RAM size > Moedm RAM size",
                  pm->shared_ram_size, pm->ram_size, 0);
  }
  ...
  pm->is_golden_copy_from_files_list_supported = 1;
#endif

#ifdef FEATURE_EFS_RMTS_ERASE_EFS_ON_EFS_RESET_API
  pm->erase_efs_image_on_efs_reset_api = 1;
#endif
...
static void
fs_rmts_pm_hookup_ram_addr (struct fs_rmts_pm *pm,
                            struct fs_rmts_superblock *sb)
{
  uint32 offset;

  offset = sb->write_cache_region.start_sector * sb->sector_size;
  pm->modem_copy.write_cache_region = pm->modem_copy.ram_addr + offset;
  pm->shared_copy.write_cache_region = pm->shared_copy.ram_addr + offset;

  offset = sb->data_region.start_sector * sb->sector_size;
  pm->modem_copy.data_region = pm->modem_copy.ram_addr + offset;
  pm->shared_copy.data_region = pm->shared_copy.ram_addr + offset;

  pm->shared_data_region_size_bytes = pm->shared_ram_size -
                (pm->shared_copy.data_region - pm->shared_copy.ram_addr);
}
...
fs_rmts_fresh_start (struct fs_rmts_pm *pm)
{
  ...
  pm->modem_copy.superblock->is_image_compressed = pm->is_compression_enabled;
  ...
  fs_rmts_pm_hookup_ram_addr (pm, pm->modem_copy.superblock);
...
static void
fs_rmts_pm_compress_efs_copy (struct fs_rmts_pm *pm)
{
  void *src, *dest;
  uint32 src_len, dest_len, heap_used;
  enum fs_zip_return_status status;
  fs_zip_handle zip_handle = NULL;
  uint64 start_time, end_time;
  struct fs_rmts_superblock *sb;

  pm->curr_sync_timing.compress_time = 0;
  sb = pm->modem_copy.superblock;

  if (pm->is_compression_enabled == 1)
  {
    start_time = fs_timetick_get_timetick ();

    src = pm->modem_copy.data_region;
    src_len = pm->modem_data_region_size_bytes;

    dest = pm->shared_copy.data_region;
    dest_len = pm->shared_data_region_size_bytes;

    zip_handle = fs_zip_open (FS_ZIP_MODE_COMPRESSION, fs_rmts_pm_heap,
                              sizeof (fs_rmts_pm_heap));

    pm->compressed_data_bytes = 0;
    status = fs_zip_compress_complete (zip_handle, src, src_len, dest,
                                       dest_len, &pm->compressed_data_bytes);
    if (status != FS_ZIP_STATUS_FINISHED)
    {
      FS_ERR_FATAL ("%d %d EFS Compression failed [status,outlen]",
                     status, pm->compressed_data_bytes, 0);
    }
    FS_ASSERT (pm->compressed_data_bytes <= dest_len);

    status = fs_zip_get_heap_usage (zip_handle, &heap_used);
    FS_ASSERT (status == FS_ZIP_STATUS_SUCCESS);

    fs_zip_close (zip_handle);

    /* Can be just compressed bytes, but easier to debug when size is fixed */
    sb->data_region.encrypted_data_size_bytes = dest_len;

    end_time = fs_timetick_get_timetick ();
    pm->curr_sync_timing.compress_time =
                  fs_timetick_timetick_diff_to_us (end_time - start_time);
    FS_MSG_HIGH_3("%d,%d,%d EFS Compression done[time_us,compressed_len,heap]",
                  (uint32) pm->curr_sync_timing.compress_time,
                   pm->compressed_data_bytes, heap_used);
  }
  else
  {
    /* Compression not enabled, so return the data reg size to be encrypted */
    sb->data_region.encrypted_data_size_bytes =
                                        pm->modem_data_region_size_bytes;
  }
}

static void
fs_rmts_prepare_address_for_encryption (struct fs_rmts_pm *pm, uint8 **src,
                                        uint8 **dest, uint32 *len)
{
  if (pm->is_compression_enabled == 0)
  {
    *src = pm->modem_copy.data_region;
    *dest = pm->shared_copy.data_region;
  }
  ...
  {
    *src = pm->shared_copy.data_region;
    *dest = pm->shared_copy.data_region;
  }

  *len = pm->modem_copy.superblock->data_region.encrypted_data_size_bytes;
}
...
fs_rmts_encrypt_efs_copy (struct fs_rmts_pm *pm)
{
  ...
  {
    fs_rmts_prepare_address_for_encryption (pm, &src, &dest, &len);
  ...
  pm->curr_sync_timing.enc_time =
              fs_timetick_timetick_diff_to_us (end_time - start_time);

  FS_MSG_MED_2 ("%d,%d, EFS: Encrypt complete for sync [time_us,age]",
      (uint32)pm->curr_sync_timing.enc_time, pm->modem_copy.superblock->age);
}
                         ...
                         uint32 sector_size, uint8 *dest_addr)
{
  ...
  uint32 read_size, available_ram_size;
  uint8 *shared_ram_addr;

  shared_ram_addr = pm->shared_copy.ram_addr + (start_sector * sector_size);
  read_size = num_sectors * sector_size;
  available_ram_size = pm->shared_ram_size -
                      (shared_ram_addr - pm->shared_copy.ram_addr);
  FS_ASSERT (read_size <= available_ram_size);

  iovec.sector_id = start_sector;
  iovec.size = read_size;
                         ...
                         enum fs_rmts_partition_id which_partition)
{
  ...
  /* Count write cache sectors */
  start_sector = sb->write_cache_region.start_sector;
  num_sectors = sb->write_cache_region.num_sectors;
  /* Add to that data reg sectors. */
  num_sectors += (sb->data_region.encrypted_data_size_bytes +
                 (sb->sector_size - 1)) / sb->sector_size;
...
/* Check for image layout validity by verifying offsets do not overlap or do
   not overflow the available memory. */
static void
fs_rmts_pm_verify_image_layout (struct  fs_rmts_superblock *sb,
                                struct fs_rmts_pm *pm)
{
  uint32 write_cache_region_end, data_region_end;
  uint32 encrypted_write_cache_end, encrypted_data_end;
  uint32 write_cache_start, data_region_start;

  if (fs_rmts_super_get_img_compression_state (sb) !=
      pm->is_compression_enabled)
  {
    FS_ERR_FATAL ("%d,%d,Build & FS Img compression config mismatch. Erase FS",
                   sb->is_image_compressed, pm->is_compression_enabled, 0);
  }

  /* Check regions fit inside Modem RAM region */
  if ((sb->write_cache_region.start_sector +
      sb->write_cache_region.num_sectors) > sb->data_region.start_sector)
  {
    FS_ERR_FATAL ("%d,%d,%d Bad image layout. Write cache overlaps data",
                  sb->write_cache_region.start_sector,
                  sb->write_cache_region.num_sectors,
                  sb->data_region.start_sector);
  }

  write_cache_start = sb->write_cache_region.start_sector * sb->sector_size;
  write_cache_region_end = write_cache_start +
                      (sb->write_cache_region.num_sectors * sb->sector_size);
  if (write_cache_region_end > pm->ram_size)
  {
    FS_ERR_FATAL ("%d,%d,%d Bad FS img. Write cache > Modem RAM, Erase FS?",
                  sb->write_cache_region.start_sector,
                  sb->write_cache_region.num_sectors, pm->ram_size);
  }

  data_region_start = sb->data_region.start_sector * sb->sector_size;
  data_region_end = data_region_start +
                   (sb->data_region.num_sectors * sb->sector_size);
  if (data_region_end  > pm->ram_size)
  {
    FS_ERR_FATAL ("%d,%d,%d Bad FS img. Data reg end > Modem RAM, Erase FS?",
                  sb->data_region.start_sector, sb->data_region.num_sectors,
                  pm->ram_size);
  }

  /* Check encrypted region bytes fit inside Shared RAM region */
  encrypted_write_cache_end = write_cache_start +
                              sb->write_cache_region.encrypted_data_size_bytes;
  if (encrypted_write_cache_end > pm->shared_ram_size)
  {
    FS_ERR_FATAL ("%d,%d,%d Bad FS img. Enc Write cache >Shared RAM,Erase FS?",
                  sb->write_cache_region.start_sector,
                  sb->write_cache_region.encrypted_data_size_bytes,
                  pm->shared_ram_size);
  }

  encrypted_data_end = data_region_start +
                       sb->data_region.encrypted_data_size_bytes;
  if (encrypted_data_end > pm->shared_ram_size)
  {
    FS_ERR_FATAL ("%d,%d,%d Bad FS img. Enc Data > Shared RAM, Erase FS?",
                  sb->data_region.start_sector,
                  sb->data_region.encrypted_data_size_bytes,
                  pm->shared_ram_size);
  }
                                     ...
                                     enum fs_rmts_partition_id which_partition)
{
  ...
  if (result == 0)
  {
    ...
    if (result == 0)
    {
      ...
      fs_rmts_pm_verify_image_layout (dest_sb, pm);
    }
  }
...
static void
fs_rmts_pm_decompress_efs_copy (struct fs_rmts_pm *pm)
{
  void *src, *dest;
  uint32 src_len, dest_len, heap_used;
  enum fs_zip_return_status status;
  fs_zip_handle zip_handle = NULL;
  uint64 start_tick, stop_tick;
  struct fs_rmts_superblock *sb = pm->modem_copy.superblock;

  pm->decompress_time_us = 0;

  if (pm->is_compression_enabled == 0 || pm->fresh_start)
  {
    return;
  }
  ...
  /* Copy modem data region into shared region to decompress back to modem */
  fs_memscpy (pm->shared_copy.data_region, pm->shared_data_region_size_bytes,
              pm->modem_copy.data_region,
              sb->data_region.encrypted_data_size_bytes);

  zip_handle = fs_zip_open (FS_ZIP_MODE_DECOMPRESSION, fs_rmts_pm_heap,
                            sizeof (fs_rmts_pm_heap));
  ASSERT (zip_handle != NULL);

  src = pm->shared_copy.data_region;
  dest = pm->modem_copy.data_region;
  src_len = pm->shared_data_region_size_bytes;
  dest_len = sb->data_region.num_sectors * sb->sector_size;

  status = fs_zip_decompress_complete (zip_handle, src, src_len, dest,
                                       dest_len, &pm->decompressed_data_bytes);
  if (status != FS_ZIP_STATUS_FINISHED)
  {
    FS_ERR_FATAL ("%d %d EFS Decompress failed [status,out_len]",
                   status, pm->decompressed_data_bytes, 0);
  }

  FS_ASSERT (pm->decompressed_data_bytes <= dest_len);
  ...
  stop_tick = fs_timetick_get_timetick ();
  pm->decompress_time_us =
      fs_timetick_timetick_diff_to_us (stop_tick - start_tick);

  FS_MSG_HIGH_3("%d,%d,%d,EFS Decompression done[time_us,decompress_len,heap]",
                (uint32)pm->decompress_time_us, pm->decompressed_data_bytes,
                heap_used);
}
                                ...
                                struct fs_rmts_pm_efs_copy *dest_copy)
{
  ...
  region_size = sb->data_region.encrypted_data_size_bytes;
                                 ...
                                 uint8 *dr_decrypt_failed)
{
  ...
  *wc_decrypt_failed = *dr_decrypt_failed = 1;
  ...
  fs_rmts_pm_hookup_ram_addr (pm, superblock);
  ...
  if (which_partition == FS_RMTS_PARTITION_ID_FS2)
  {
    if (pm->modem_type == FS_RMTS_MODEM_TYPE_FUSION)
      fs_memscpy (pm->modem_copy.ram_addr, pm->ram_size,
                  pm->shared_copy.ram_addr, pm->shared_ram_size);
...
Decompress:
  /* Now the active efs copy is present in Modem RAM. We might need to
     decompress it if not already */

  fs_rmts_pm_decompress_efs_copy (pm);
}
                                ...
                                enum fs_rmts_partition_id partition_to_boot)
{
  ...
  uint32 img_hdr_ram_size, image_size, sector_size;
  ...
  sector_size = pm->modem_copy.superblock->sector_size;
  img_hdr_ram_size = FS_RMTS_PM_IMG_HEADER_NUM_SECTORS * sector_size;
  image_size = pm->shared_ram_size - img_hdr_ram_size;
...
static enum fs_rmts_device_errno
fs_rmts_pm_write_fsg_partition (struct fs_rmts_pm *pm,
                                uint8 *ram_addr, uint32 ram_size,
                                enum fs_rmts_partition_id which_partition,
                                enum fs_rmts_partition_id partition_to_boot)
{
  struct fs_iovec iovec;
  ...
  sector_size = pm->modem_copy.superblock->sector_size;
  img_hdr_ram_size = FS_RMTS_PM_IMG_HEADER_NUM_SECTORS * sector_size;
  ASSERT (ram_size >= img_hdr_ram_size);
  image_size = ram_size - img_hdr_ram_size;
                                   ...
                                   uint32 buffer_size)
{
  ...
  fs_memscpy (pm->shared_copy.ram_addr, pm->shared_ram_size,
              buffer, buffer_size);
...
static int
fs_rmts_pm_read_efs_copy (struct fs_rmts_pm *pm,
                          enum fs_rmts_partition_id parti,
                          void *dest_addr, uint32 dest_size)
{
  uint32 num_sectors;

  num_sectors = pm->shared_ram_size / FS_RMTS_SECTOR_SIZE;

  if (dest_size < pm->shared_ram_size)
  {
    num_sectors = dest_size / FS_RMTS_SECTOR_SIZE;
  }

  return fs_rmts_pm_read_sectors (pm, parti, 0, num_sectors,
                                  FS_RMTS_SECTOR_SIZE, dest_addr);
}


static void
fs_rmts_pm_copy_fsg_to_fs2(struct fs_rmts_pm *pm)
{
  int result;
  ...
  switch (pm->modem_type)
  {
    case FS_RMTS_MODEM_TYPE_STANDALONE:
    {
      result = fs_rmts_pm_read_efs_copy (pm, FS_RMTS_PARTITION_ID_FSG,
                                         pm->shared_copy.ram_addr,
                                         pm->shared_ram_size);
      if (result != 0)
      {
    ...
    case FS_RMTS_MODEM_TYPE_FUSION:
    {
      ...
      fs_memscpy (pm->shared_copy.ram_addr, pm->shared_ram_size,
                  pm->golden_copy.ram_addr, pm->shared_ram_size);
      break;
    }
...
fs_rmts_pm_try_golden_copy (struct fs_rmts_pm *pm)
{
  ...
  switch (pm->modem_type)
  {
    case FS_RMTS_MODEM_TYPE_STANDALONE:
    {
      /* Read the whole image into modem ram */
      result = fs_rmts_pm_read_efs_copy (pm, FS_RMTS_PARTITION_ID_FSG,
                                         pm->modem_copy.ram_addr,
                                         pm->ram_size);
      if (result != 0)
      {
        FS_ERR_FATAL ("%d, FSG Read failed", result, 0, 0);
                             ...
                             struct fs_rmts_superblock *sb)
{
  ...
  data_region_size = sb->data_region.num_sectors * sb->sector_size;
  pm->modem_data_region_size_bytes = data_region_size;
  ...
  if (sb->data_region.sync_counter == 0)
  {
    ...
    for (i = 0; i < pm->write_cache_hdr->write_cache_ctable_num_entries; i++)
    {
      ...
      if (cluster_num != FS_INVALID_PAGE_ID)
      {
        ...
        dest_size = pm->modem_data_region_size_bytes -
                   (cluster_num * pm->page_size);
                       ...
                       void *buffer, uint16 tag)
{
  ...
  dest_size = pm->modem_data_region_size_bytes - (cluster * pm->page_size);
  fs_memscpy (dest_addr, dest_size, buffer, pm->page_size);
 ...
 */
static int
fs_rmts_sync_start_check (struct fs_rmts_pm *pm, int force_sync)
{

  if ((pm->txns_since_sync == 0) && (force_sync == 0))
  {
...
static int
fs_rmts_pm_sync_start (fs_pm_t pm_data, int do_force_sync,
                       int is_shutdown_sync)
{
  ...
  result = fs_rmts_sync_start_check (pm, do_force_sync);
  if (result != 0)
    return result;
  ...
  pm->is_shutdown_sync = is_shutdown_sync;
  ...
  /* Compress the data before encrypting if requried */
  fs_rmts_pm_compress_efs_copy (pm);
...
{
  struct fs_rmts_pm *pm = (struct fs_rmts_pm *)pm_data;
  uint64 start_time, end_time, time_diff;
  ...
  if (write_result == FS_RMTS_ERRNO_SUCCESS)
  {
    FS_MSG_MED_2 ("[%d,%d], EFS: write_iovec for sync [parti,time (ms)]",
                  active_efs_partition,
                  (uint32) pm->curr_sync_timing.write_iovec_time);
    pm->sync_result = 0;

    if (fs_enable_read_back_verify_sync != 0)
    {
    #ifndef EFS_READ_BACK_ALLOW_ANY_TASK
    if (fs_os_self () != fs_task_tcb)
    {
    ...
    else
    {
      ...
      time_diff = fs_timetick_timetick_diff_to_us (end_time - start_time);
...
fs_rmts_pm_sync_end (fs_pm_t pm_data)
{
  ...
  timings->efs_sync_timings.compression_time =
                           (uint32) pm->curr_sync_timing.compress_time;
  timings->efs_sync_timings.encryption_time  =
                           (uint32) pm->curr_sync_timing.enc_time;
...
fs_rmts_pm_sync_no_wait (fs_pm_t pm_data, fs_sync_token_t *sync_token)
{
  ...
  *sync_token = fs_sync_get_completed_token (&pm->sync_data);
  ...
  result = fs_rmts_sync_start_check (pm, 0);
  if (result != 0)
    return result;
...
fs_rmts_pm_golden_copy_sync (struct fs_rmts_pm *pm)
{
  ...
  uint8 original_partition, original_is_curr_sync_full_sync;
  ...
  original_is_curr_sync_full_sync = pm->is_curr_sync_full_sync;
  ...
  pm->is_curr_sync_full_sync = original_is_curr_sync_full_sync;
...
fs_rmts_pm_fs_init_complete (fs_pm_t pm_data)
{
  ...
  if (pm->is_golden_copy_from_files_list_supported)
  {
    fs_rmts_pm_copy_fsg_to_fs2 (pm);
  }

  sector_size = pm->modem_copy.superblock->sector_size;

  img_hdr_ram = (pm->shared_copy.ram_addr +
                (FS_RMTS_PM_IMG_HEADER_START_SECTOR * sector_size));
  img_hdr_ram_size = FS_RMTS_PM_IMG_HEADER_NUM_SECTORS * sector_size;
  image_size = pm->shared_ram_size - img_hdr_ram_size;
...
static int
fs_rmts_pm_get_buf_for_fsg (fs_pm_t pm_data, uint8 **buf, uint32 *buf_size,
                            uint32 *hdr_offset)
{
  struct fs_rmts_pm *pm = (struct fs_rmts_pm *)pm_data;
  ...
  ASSERT (hdr_offset != NULL);

  *buf = pm->shared_copy.ram_addr;
  *buf_size = pm->shared_ram_size;
  *hdr_offset = FS_RMTS_PM_TAR_START_SECTOR * FS_RMTS_SECTOR_SIZE;;

  return 0;
}

static enum fs_rmts_partition_id
fs_rmts_pm_convert_to_fsg_partition (enum fs_golden_copy_partition_type parti)
{
  enum fs_rmts_partition_id rmts_partition;

  rmts_partition = FS_RMTS_PARTITION_ID_FSC;

  switch (parti)
  {
    case FS_GOLDEN_COPY_PARTITION_TYPE_DEFAULT:
      rmts_partition = FS_RMTS_PARTITION_ID_FSG;
      break;

    case FS_GOLDEN_COPY_PARTITION_TYPE_OEM_1:
      rmts_partition = FS_RMTS_PARTITION_ID_OEM_1;
      break;

    case FS_GOLDEN_COPY_PARTITION_TYPE_OEM_2:
      rmts_partition = FS_RMTS_PARTITION_ID_OEM_2;
      break;

    case FS_GOLDEN_COPY_PARTITION_TYPE_MAX:
    default:
      ASSERT (0);
      break;
  }

  return rmts_partition;
}

static int
fs_rmts_pm_write_to_fsg (fs_pm_t pm_data, uint8 *buf, uint32 buf_size,
                         uint32 partition_arg)
{
  struct fs_rmts_pm *pm = (struct fs_rmts_pm *)pm_data;
  enum fs_rmts_device_errno result;
  enum fs_golden_copy_partition_type partition;
  enum fs_rmts_partition_id rmts_partition;

  ASSERT (pm != NULL);
  ASSERT (buf_size <= pm->ram_size);
  partition = (enum fs_golden_copy_partition_type )partition_arg;
  ASSERT (partition <= FS_GOLDEN_COPY_PARTITION_TYPE_MAX);

  rmts_partition = fs_rmts_pm_convert_to_fsg_partition (partition);

  result = fs_rmts_pm_write_fsg_partition (pm, buf, buf_size, rmts_partition,
                                           FS_RMTS_PARTITION_ID_FSG);
  if (result != FS_RMTS_ERRNO_SUCCESS)
  {
    FS_ERR_FATAL ("%d,%d,%d EFS: make golden write failed [parti, result]",
                   FS_RMTS_PARTITION_ID_FSG, result, rmts_partition);
  }
                                         ...
                                         uint32 total_size)
{
  ...
  if (total_size > pm->shared_ram_size)
  {
    FS_ERR_FATAL ("TAR size %d bigger than shared RAM Size %d. Cannot copy",
                   total_size, pm->shared_ram_size, 0);
  }

  /* Sanity check the offset and size before copying. */
  ASSERT (src_offset + total_size <= pm->ram_size);
  ASSERT (dest_offset + total_size <= pm->shared_ram_size);
                                        ...
                                        uint32 total_size)
{
  ...
  if (total_size > pm->shared_ram_size)
  {
    FS_ERR_FATAL ("ZIP size %d bigger than shared RAM Size %d. Cannot copy",
                   total_size, pm->shared_ram_size, 0);
  }

  /* Sanity check the offset and size before copying. */
  FS_ASSERT (src_offset + total_size <= pm->ram_size);
  FS_ASSERT (dest_offset + total_size <= pm->shared_ram_size);
  ...
  zlib_heap_size  = pm->shared_ram_size - (src_offset + total_size);
...
fs_rmts_pm_unzip_from_shared_ram (struct fs_rmts_pm *pm, uint32 start_offset)
{
  ...
  zlib_heap_size  = pm->shared_ram_size - pm->untar_size;
...
void fs_rmts_pm_erase_efs (void)
{
  struct fs_rmts_pm *pm = (struct fs_rmts_pm *)&rmts_pm_inst;
  struct fs_rmts_superblock *sb;
  enum fs_rmts_device_errno result;
  uint8 original_is_curr_sync_full_sync, *shared_buf_addr, *modem_buf_addr;
  uint32 hdr_size, overwrite_size;

  ASSERT (pm != NULL);

  if (!pm->erase_efs_image_on_efs_reset_api)
  {
    FS_ERR_FATAL ("Corrupted EFS-image.", 0, 0, 0);
  }

  FS_MSG_ERROR_0 ("Erasing EFS begin");

  sb = pm->modem_copy.superblock;
  original_is_curr_sync_full_sync = pm->is_curr_sync_full_sync;
  pm->is_curr_sync_full_sync = 1;

  hdr_size = (FS_RMTS_PM_SUPERBLOCK_START_SECTOR * sb->sector_size);
  overwrite_size = (100 * FS_RMTS_SECTOR_SIZE); /* 100 sectors overwrite */
  shared_buf_addr = pm->shared_copy.ram_addr + hdr_size;
  modem_buf_addr = pm->shared_copy.ram_addr + hdr_size;
  memset (shared_buf_addr, 0xEF, overwrite_size);

  result = fs_rmts_pm_write_efs_partition (pm, FS_RMTS_PARTITION_ID_FS1,
                                           FS_RMTS_PARTITION_ID_FS1);
  if (result != FS_RMTS_ERRNO_SUCCESS)
  {
    FS_ERR_FATAL ("%d,%d, erase EFS failed[parti, result]",
                   FS_RMTS_PARTITION_ID_FS1, result, 0);
  }

  result = fs_rmts_pm_write_efs_partition (pm, FS_RMTS_PARTITION_ID_FS2,
                                           FS_RMTS_PARTITION_ID_FS2);
  if (result != FS_RMTS_ERRNO_SUCCESS)
  {
    FS_ERR_FATAL ("%d,%d, erase EFS failed[parti, result]",
                   FS_RMTS_PARTITION_ID_FS2, result, 0);
  }
  pm->is_curr_sync_full_sync = original_is_curr_sync_full_sync;

  FS_MSG_ERROR_0 ("Erasing EFS done");

  FS_ERR_FATAL ("EFS-image erased, requires restart.", 0, 0, 0);
}
...
fs_rmts_pm_init_ops (fs_pm_t pm_data)
{
  ...
  pm_data->ops.get_buf_for_fsg = fs_rmts_pm_get_buf_for_fsg;
  pm_data->ops.write_to_fsg = fs_rmts_pm_write_to_fsg;
}
