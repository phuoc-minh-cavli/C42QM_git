 ...
 * Copyright (C) 2003-2013,2015-2016 QUALCOMM Technologies, Inc.
 */
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_buffer.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
static int b_sync_start (fsb_t imp, int do_force_sync,
...
static int b_get_buf_for_fsg (fsb_t imp, uint8 **buff, uint32 *buff_size,
                              uint32 *hdr_offset);
static int b_write_to_fsg (fsb_t imp, uint8 *buff, uint32 buff_size,
                           uint32 partition);
...
static int
b_sync_start_invalid (fsb_t imp, int do_force_sync, int is_shutdown_sync)
{
  (void) imp;
  (void) do_force_sync;
...
static int
b_get_buf_for_fsg_invalid (fsb_t imp, uint8 **buff, uint32 *buff_size,
                           uint32 *hdr_offset)
{
  (void) imp; (void) buff; (void) buff_size; (void) hdr_offset;
  return -EPERM;
}

static int
b_write_to_fsg_invalid (fsb_t imp, uint8 *buff, uint32 buff_size,
                        uint32 partition)
{
  (void) imp; (void) buff; (void) buff_size; (void) partition;
  return -EPERM;
}
                           ...
                           uint32 cluster_size)
{
  ...
  buf->imp.get_buf_for_fsg = b_get_buf_for_fsg_invalid;
  buf->imp.write_to_fsg = b_write_to_fsg_invalid;
...
fsb_buffer_initialize (fsb_nand_t buf, fs_pm_t gc_data, uint32 cluster_size)
{
  ...
  buf->imp.get_buf_for_fsg = b_get_buf_for_fsg;
  buf->imp.write_to_fsg = b_write_to_fsg;
...
static int
b_sync_start (fsb_t imp, int do_force_sync, int is_shutdown_sync)
{
  fsb_nand_t buf = (fsb_nand_t) imp;
  return (buf->gc->ops.sync_start (buf->gc, do_force_sync, is_shutdown_sync));
}
...
static int
b_get_buf_for_fsg (fsb_t imp, uint8 **buff, uint32 *buff_size,
                   uint32 *hdr_offset)
{
  fsb_nand_t buf = (fsb_nand_t) imp;

  return (buf->gc->ops.get_buf_for_fsg (buf->gc, buff, buff_size, hdr_offset));
}

static int
b_write_to_fsg (fsb_t imp, uint8 *buff, uint32 buff_size, uint32 partition)
{
  fsb_nand_t buf = (fsb_nand_t) imp;

  return (buf->gc->ops.write_to_fsg (buf->gc, buff, buff_size, partition));
}
