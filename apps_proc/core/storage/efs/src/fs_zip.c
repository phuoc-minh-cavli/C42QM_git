...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_zip.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
                 ...
                 uint32 *available_output)
{
  ...
  if (handle->state == FS_ZIP_DONE)
  {
    *available_output = 0;
    return FS_ZIP_STATUS_FINISHED;
  }

  if ((output == NULL) || (output_buff_size == 0))
  {
    return FS_ZIP_INSUFFICIENT_BUF;
  }
...
fs_zip_compress_complete (fs_zip_handle handle, void *input, uint32 input_size,
                          ...
                          uint32 *available_output)
{
  uint32 remaining_bytes, written_bytes, output_available;
  enum fs_zip_return_status status = FS_ZIP_STATUS_SUCCESS;

  if ((handle == NULL) || (input == NULL) || (input_size == 0) ||
      (output == NULL) || (output_buff_size == 0))
  {
    return FS_ZIP_INVALID_PARAM;
  }

  status = fs_zip_set_input (handle, input, input_size);
  if (status != FS_ZIP_STATUS_SUCCESS)
  {
    goto Error;
  }

  remaining_bytes = output_buff_size;
  written_bytes = 0;
  while (status == FS_ZIP_STATUS_SUCCESS)
  {
    status = fs_zip_compress (handle, ((uint8 *)output) + written_bytes,
                              remaining_bytes, &output_available);
    if (status == FS_ZIP_STATUS_SUCCESS)
    {
      ASSERT (output_available > 0 && output_available <= remaining_bytes);
      written_bytes += output_available;
      remaining_bytes -= output_available;
    }
  }

  /* If buffer was fully consumed, no more space left */
  if (status == FS_ZIP_STATUS_SUCCESS && remaining_bytes == 0)
  {
    status = FS_ZIP_INSUFFICIENT_BUF;
    goto Error;
  }

  /* Bad state */
  if (status != FS_ZIP_STATUS_NEED_MORE_INPUT)
  {
    goto Error;
  }

  status = fs_zip_set_input (handle, NULL, 0);
  if (status != FS_ZIP_STATUS_SUCCESS)
  {
    goto Error;
  }
  ...
  *available_output = written_bytes;
  return status;

Error:
  memset (output, 0x0, output_buff_size);
  *available_output = 0;
  return status;
}
                    ...
                    uint32 *available_output)
{
  ...
  if ((handle->state != FS_ZIP_PROCESS_TRAILER) &&
      ((output == NULL) || (output_buff_size == 0)))
  {
    return FS_ZIP_INSUFFICIENT_BUF;
  }
...
{
  enum fs_zip_return_status status = FS_ZIP_STATUS_SUCCESS;
  uint32 output_available, remaining_bytes, written_bytes;
  ...
  while (status == FS_ZIP_STATUS_SUCCESS)
  {
    status = fs_zip_decompress (handle, ((uint8 *)output) + written_bytes,
                                remaining_bytes, &output_available);
    if (status == FS_ZIP_STATUS_SUCCESS)
    {
      ASSERT (output_available > 0 && output_available <= remaining_bytes);
...
enum fs_zip_return_status
fs_zip_get_heap_usage (fs_zip_handle handle, uint32 *heap_used)
{
  if (handle == NULL)
  {
    return FS_ZIP_INVALID_PARAM;
  ...
  *heap_used = fs_zip_malloc_get_allocated_heap (handle->malloc_handle);
