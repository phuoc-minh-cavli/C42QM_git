...
/*==========================================================================
  ...
  Copyright (c) 2002-2013,2016 by QUALCOMM Technologies, Incorporated.
  All Rights Reserved.
===========================================================================*/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_diag_i.h#1 $
...
/* Request packet to create a golden copy. */
typedef PACKED struct PACKED_POST {
  diagpkt_subsys_header_type hdr;
  uint16 sequence_number; /* Number for Synchronization / Correlation     */
  uint8  golden_copy_type; /* Golden copy type */
  char   path[1];          /* Which file system. */
} efs2_diag_make_golden_copy_v2_req_type;

/* Make Golden Copy Response. */
typedef PACKED struct PACKED_POST {
  diagpkt_subsys_header_type hdr;
  uint16 sequence_number;   /* Acknowledge Synchronization / Correlation num */
  uint8 golden_copy_status;
  int32  diag_errno;         /* Error code if error, 0 otherwise             */
} efs2_diag_make_golden_copy_v2_rsp_type;


/* golden_copy_get_status request packet.
 *
 * This packet is used to query the status of a preceeding request for
...
typedef PACKED struct PACKED_POST {
  ...
  uint16 sequence_number;
  char path[1];
} efs2_diag_make_golden_copy_v2_get_status_req_type;

/* golden_copy_get_status response packet. */
typedef PACKED struct PACKED_POST {
  diagpkt_subsys_header_type hdr;
  uint16 sequence_number;
  uint8 golden_copy_status;
  int32 diag_errno;       /* errno if any error or 0 otherwise. */
} efs2_diag_make_golden_copy_v2_get_status_rsp_type;
