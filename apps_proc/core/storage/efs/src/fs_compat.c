...
Copyright (C) 2002-2007, 2009-2013, 2015-2016 by QUALCOMM Technologies, Inc.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_compat.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when       who    what, where, why
--------   ---    ----------------------------------------------------------
08/08/16   rp     Enhance fs-osal to support both rex and qurt.
03/16/15   vm     Add support to new RCINIT framework.
...
fs_os_signal_handle_type fs_compat_signal;
...
fs_compat_process_commands (void)
{
  ...
  while ((cmd_ptr = (fs_cmd_type *) q_get (&fs_compat_queue)) != NULL) {
    ...
    {
      fs_os_set_sigs (task_ptr, &fs_compat_signal, task_sigs);
    }
...
fs_compat_task
(
  fs_os_task_func_arg_type parm            /* from REX - ignored. */
)
{
  fs_os_sigs_type task_sigs, wait_sigs;

  (void) parm;

  // FS_MSG_MED ("Starting FS Compatibility task", 0, 0, 0);

  fs_os_def_timer (&fs_compat_rpt_timer, &fs_compat_tcb, &fs_compat_signal,
      FS_COMPAT_RPT_TIMER_SIG);
  fs_os_def_timer (&fs_cache_sync_timer, &fs_compat_tcb, &fs_compat_signal,
      FS_COMPAT_CACHE_SYNC_SIG);

  wait_sigs = (FS_COMPAT_RPT_TIMER_SIG |
               FS_COMPAT_CACHE_SYNC_SIG |
               FS_CMD_Q_SIG |
               TASK_STOP_SIG |
               TASK_OFFLINE_SIG);
  ...
  while (1) {
    fs_os_clr_sigs (&fs_compat_tcb, &fs_compat_signal, FS_CMD_Q_SIG);
    fs_compat_process_commands ();

    task_sigs = fs_os_wait (&fs_compat_signal, wait_sigs);

    /* Check if we need to kick. */
    if ((task_sigs & FS_COMPAT_RPT_TIMER_SIG) != 0)
    {
      fs_os_clr_sigs (&fs_compat_tcb, &fs_compat_signal,
                      FS_COMPAT_RPT_TIMER_SIG);
    }
    ...
    {
      fs_os_clr_sigs (&fs_compat_tcb, &fs_compat_signal, TASK_OFFLINE_SIG);
    ...
    {
      fs_os_clr_sigs (&fs_compat_tcb, &fs_compat_signal, TASK_STOP_SIG);
#ifdef FEATURE_EFS_FTL
      ftl_cache_force_sync_all ();
#endif
      task_stop ();
...
fs_compat_ops_cmd (fs_cmd_type *cmd_ptr)
{
  ...
  fs_os_set_sigs (&fs_compat_tcb, &fs_compat_signal, FS_CMD_Q_SIG);
...
fs_compat_init (void)
{
  ...
  RCINIT_PRIO fs_compat_task_prio = 0;
  unsigned long fs_compat_stack_size = 0;
  ...
  if (!init_complete)
  {
    ...
    fs_os_signal_init(&fs_compat_signal);
  ...
  info_handle = rcinit_lookup ("FS_Compat_Task");
  if (info_handle == NULL)
  {
    FS_ERR_FATAL ("FS Compat task info not found", 0, 0, 0);
  }
  else
  {
    fs_compat_task_prio = rcinit_lookup_prio_info (info_handle);
    fs_compat_stack_size = rcinit_lookup_stksz_info (info_handle);
    if ((fs_compat_task_prio > 255) || (fs_compat_stack_size == 0))
    {
      FS_ERR_FATAL ("[%d,%d,%d] Invalid Task Priority/Stack Size:[prio,size]",
                     fs_compat_task_prio, fs_compat_stack_size, 0);
    }
  }
#else
  (void) info_handle;
  fs_compat_task_prio = FS_COMPAT_PRI;
  fs_compat_stack_size = FS_COMPAT_STACK_SIZ;
#endif /* FEATURE_CENTRALIZED_THREAD_PRIORITY */

    /* Start up the fs_compat task */
    fs_os_def_task_ext (&fs_compat_tcb,
                        fs_compat_stack,
                        fs_compat_stack_size,
                        fs_compat_task_prio,
                        fs_compat_task,
                        0,
                        "FS_Compat_Task",
