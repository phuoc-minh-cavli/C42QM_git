...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_sync.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 ...
 * phone boot. */
uint8 fs_sync_lock_init_complete = 0;
...
fs_sync_timer_init (struct fs_sync_data *sync_data)
{
  ...
  fs_os_def_timer (&sync_data->sync_timer, fs_task_tcb, fs_task_signal,
                   FS_EFS_SYNC_SIG);
}
...
fs_sync_start_timer (struct fs_sync_data *sync_data)
{
  ...
  if (msecs != 0)
  {
    fs_os_clr_timer (&sync_data->sync_timer);
    fs_os_set_timer (&sync_data->sync_timer, msecs);
    result = 0;
  }
...
fs_sync_signal_task (struct fs_sync_data *sync_data)
{
  ...
  fs_os_set_sigs (fs_task_tcb, fs_task_signal,
                  FS_EFS_SYNC_SIG);
}
