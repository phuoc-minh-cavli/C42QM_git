...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_benchmark.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2016-08-08   rp    Enhance fs-osal to support both rex and qurt.
2015-03-16   vm    Add support to new RCINIT framework.
2015-01-27   dks   Remove usage of ran.h
...
static fs_os_signal_handle_type fs_benchmark_signal;
...
#define BENCH_BUFFER_DWORDS (BENCH_BUFFER_BYTES / sizeof(uint32))

/* The +32 is to allow an intentionally misaligned start address of 0
 * to 31 bytes */
#define BENCH_BUFFER_ACTUAL_BYTES (BENCH_BUFFER_BYTES + 32)

static uint32 *fs_bench_buffer = NULL;
static uint32 *fs_bench_read_buffer = NULL;
...
static fs_os_signal_handle_type fs_cpu_hog_signal;
...
static uint32 fs_benchmark_seed = 0xDECAFBAD;

/* Fill the benchmark buffer with a fresh block of psuedo-random data */
static void
fill_bench_buffer_with_pattern (uint32 seed)
{
  unsigned int i;

  ASSERT (fs_bench_buffer != NULL);

  fs_benchmark_seed  = seed;

  for (i = 0; i < BENCH_BUFFER_DWORDS; i++)
  {
    fs_bench_buffer[i] = fs_benchmark_seed++;
  }
}

#define FS_BENCH_MAX_PSEUDO_RAND (32767)
static uint64 fs_bench_r_seed = 1;
void
fs_bench_set_pseudo_rand_seed (uint32 seed)
{
  fs_bench_r_seed = seed;
}

/* Return a pseudo random number of max value FS_BENCH_MAX_PSEUDO_RAND */
uint32
fs_bench_get_next_pseudo_rand (void)
{
  fs_bench_r_seed = fs_bench_r_seed * 1103515245 + 12345;
  /* FS_BENCH_MAX_PSEUDO_RAND is largest number it returns */
  return ((uint32)(fs_bench_r_seed / 65536) % FS_BENCH_MAX_PSEUDO_RAND);
}

uint32
fs_bench_get_next_pseudo_rand_in_range (uint32 min, uint32 max)
{
  max++; // to get a number in a range inclusive of
  return ((fs_bench_get_next_pseudo_rand () % (max - min)) + min);
}
...
{
  fs_os_clr_sigs (&fs_benchmark_tcb, &fs_benchmark_signal,
                  FS_BENCHMARK_SLEEP_TIMER_SIG);
  fs_os_set_timer(&fs_benchmark_sleep_timer, ms);
  fs_os_wait (&fs_benchmark_signal,
              FS_BENCHMARK_SLEEP_TIMER_SIG); /* Ignoring other signals */
}
...
{
  uint32 seed = (unsigned)args[0] << 16 | args[1];
  ...
  dword_count = (byte_count + sizeof(uint32) - 1) / sizeof(uint32);
  if (dword_count > BENCH_BUFFER_DWORDS)
    dword_count = BENCH_BUFFER_DWORDS;

  fs_bench_set_pseudo_rand_seed (seed);
  for (i = 0; i < dword_count; i++)
    fs_bench_buffer[i] = fs_bench_get_next_pseudo_rand ();
  return 0;
}
...
fs_test_check_pattern (int32 *args, char *path)
{
  ...
  for (i = 0; i < dword_count; i++) {
    uint32 expect;
    expect = fs_bench_get_next_pseudo_rand ();
...
/* similar to fs_test_write, but writes psedu-random sizes each time */
int
fs_test_random_write (int32 *args, char *path)
{
  ...
  fs_bench_set_pseudo_rand_seed (fs_benchmark_seed);
  write_count = fs_bench_get_next_pseudo_rand_in_range (write_count_min,
                                                   write_count_max);
  ...
  while (write_count--)
  {
    ...
    write_size = fs_bench_get_next_pseudo_rand_in_range (write_size_min,
                                                    write_size_max);
    written = efs_write (fd, fs_bench_buffer, write_size);
    if (written != write_size)
    {
  ...
  fs_benchmark_seed = fs_bench_get_next_pseudo_rand ();
...
/* Write pseudo-randomly through hotplug */
int
fs_test_hotplug_random_write (int32 *args, char *path)
{
  ...
  {
    int lba, count;
    count = fs_bench_get_next_pseudo_rand_in_range (block_count_min,
                                                    block_count_max);
    if (count > remaining_blocks)
      count = remaining_blocks;

    lba = fs_bench_get_next_pseudo_rand_in_range (lba_min, lba_max - count);
...
fs_test_safe_write (int32 *args, char *path)
{
  ...
  fill_bench_buffer_with_pattern (fs_benchmark_seed);
...
fs_safe_read_prepare (int32 *args, char *path)
{
  ...
  fs_util_wait (&fs_benchmark_signal, DELAY_BEFORE_READ_MS);
...
fs_benchmark_put_cmd (fs_benchmark_test_cmd *cmd)
{
  ...
  fs_os_set_sigs (&fs_benchmark_tcb, &fs_benchmark_signal,
...
static void
fs_benchmark_task (fs_os_task_func_arg_type parm)
{
  ...
  while (1)
  {
    task_sigs = fs_os_wait (&fs_benchmark_signal, FS_BENCHMARK_CMD_Q_SIG);
...
fs_benchmark_init (void)
{
  ...
  info_handle = rcinit_lookup ("FS_Benchmark");
  if (info_handle == NULL)
  {
    FS_ERR_FATAL ("FS Benchmark task info not found", 0, 0, 0);
  }
  else
  {
    fs_benchmark_task_prio = rcinit_lookup_prio_info (info_handle);
    fs_benchmark_stack_size = rcinit_lookup_stksz_info (info_handle);
    if ((fs_benchmark_task_prio > 255) || (fs_benchmark_stack_size == 0))
    {
      FS_ERR_FATAL ("[%d,%d,%d] Invalid Task Priority/Stack Size:[prio,size]",
                     fs_benchmark_task_prio, fs_benchmark_stack_size, 0);
    }
  }
#else
  (void) info_handle;
  fs_benchmark_task_prio = FS_BENCHMARK_PRI;
  fs_benchmark_stack_size = FS_BENCHMARK_STACK_SIZE;
#endif /* FEATURE_CENTRALIZED_THREAD_PRIORITY */

  fs_os_def_task_ext (&fs_benchmark_tcb,
                      fs_benchmark_stack,
                      fs_benchmark_stack_size,
                      fs_benchmark_task_prio,
                      fs_benchmark_task,
                      0,
                      "FS_Benchmark",
                      0,
...
static void
fs_cpu_hog_task (fs_os_task_func_arg_type parm)
{
  ...
  while (1) {
    ...
    /* Then sleep */
    fs_os_clr_sigs (&fs_cpu_hog_tcb, &fs_cpu_hog_signal, FS_CPU_HOG_TIMER_SIG);
    fs_os_set_timer (&fs_cpu_hog_timer, fs_cpu_hog_sleep_ms);
    fs_os_wait (&fs_cpu_hog_signal, FS_CPU_HOG_TIMER_SIG);
...
fs_cpu_hog_init (void)
{
  ...
  RCINIT_INFO info_handle = NULL;
  RCINIT_PRIO cpu_hog_task_prio = 0;
  unsigned long cpu_hog_stack_size = 0;

  fs_os_signal_init(&fs_cpu_hog_signal);

#ifdef FEATURE_CENTRALIZED_THREAD_PRIORITY
  info_handle = rcinit_lookup ("FS_CPU_Hog");
  if (info_handle == NULL)
  {
    FS_ERR_FATAL ("FS_CPU_Hog task info not found", 0, 0, 0);
  }
  else
  {
    cpu_hog_task_prio = rcinit_lookup_prio_info (info_handle);
    cpu_hog_stack_size = rcinit_lookup_stksz_info (info_handle);
    if ((cpu_hog_task_prio > 255) || (cpu_hog_stack_size == 0))
    {
      FS_ERR_FATAL ("[%d,%d,%d] Invalid Task Priority/Stack Size:[prio,size]",
                     cpu_hog_task_prio, cpu_hog_stack_size, 0);
    }
  }
#else
  (void) info_handle;
  cpu_hog_task_prio = FS_CPU_HOG_PRI;
  cpu_hog_stack_size = FS_CPU_HOG_STACK_SIZ;
#endif /* FEATURE_CENTRALIZED_THREAD_PRIORITY */
