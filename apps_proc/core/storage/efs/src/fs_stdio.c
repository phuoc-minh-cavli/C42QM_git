 ...
 * Copyright (C) 2004-2007,2009-2011,2013,2015,2016 QUALCOMM Technologies, Inc.
 */
 ...
 *
 *  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_stdio.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
...
static fs_os_signal_handle_type fs_stdio_signal;
...
efs_stdio_init (void)
{
  ...
  fs_os_signal_init(&fs_stdio_signal);
}
...
efs_fwrite (void *ptr, fs_size_t size, fs_size_t nitems, EFS_FILE *stream)
{
  ...
  if (is_async_stream)
    fs_os_clr_sigs (stream->caller_tcb, &fs_stdio_signal, stream->caller_sigs);
  ...
  while (offset < full_size) {
    ...
    if (EFS_BUF_FULL (stream->head, tail, stream->bufsize)) {
      if (is_async_stream) {
        fs_os_set_sigs (stream->writer_tcb, &fs_stdio_signal, stream->writer_sigs);
        while (EFS_BUF_FULL (stream->head, tail, stream->bufsize)) {
          fs_os_wait (&fs_stdio_signal, stream->caller_sigs);
          fs_os_clr_sigs (stream->caller_tcb, &fs_stdio_signal, stream->caller_sigs);
          tail = stream->tail;
          if (stream->error)
            break;
...
flush_write_buffer (EFS_FILE *stream)
{
  ...
  while (head != tail) {
    ...
    if (is_async_stream)
      fs_os_set_sigs (stream->caller_tcb, &fs_stdio_signal, stream->caller_sigs);
  }
...
#ifndef FEATURE_EFS_OSAL_IS_QURT
