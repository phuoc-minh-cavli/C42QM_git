...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_efs2.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
2016-03-24    wek     Update the mode bits when converting an item file.
...
static int v_efs_item_utime_item (struct fs_vnode *vp,
                                  const struct fs_utimbuf *times);
static int v_efs_sync_start (struct fs_vnode *vp, int do_force_sync,
...
static int v_efs_store_data_in_sb (struct fs_vnode *vp, const void *data,
                                   uint32 data_size);
static int v_efs_get_buf_for_fsg (struct fs_vnode *vp, uint8 **buf,
                                  uint32 *buf_size, uint32 *hdr_offset);
static int v_efs_write_to_fsg (struct fs_vnode *vp, uint8 *buf,
...
efs2_init_vops (void)
{
  ...
  efs_dir_vops.get_buf_for_fsg  = v_efs_get_buf_for_fsg;
  efs_dir_vops.write_to_fsg     = v_efs_write_to_fsg;
  ...
  efs_file_vops.store_data_in_sb = v_efs_store_data_in_sb;
  efs_file_vops.get_buf_for_fsg  = v_efs_get_buf_for_fsg;
  efs_file_vops.write_to_fsg     = v_efs_write_to_fsg;
  ...
  efs_symlink_vops.store_data_in_sb = v_efs_store_data_in_sb;
  efs_symlink_vops.get_buf_for_fsg  = v_efs_get_buf_for_fsg;
  efs_symlink_vops.write_to_fsg     = v_efs_write_to_fsg;
  ...
  efs_item_vops.store_data_in_sb = v_efs_store_data_in_sb;
  efs_item_vops.get_buf_for_fsg  = v_efs_get_buf_for_fsg;
  efs_item_vops.write_to_fsg     = v_efs_write_to_fsg;
  ...
  efs_devspecial_vops.store_data_in_sb = v_efs_store_data_in_sb;
  efs_devspecial_vops.get_buf_for_fsg  = v_efs_get_buf_for_fsg;
  efs_devspecial_vops.write_to_fsg     = v_efs_write_to_fsg;
}
                         ...
                         unsigned value_length)
{
  ...
  switch (val[0])
  {
    ...
    default:
      /* ASSSERT to catch corruption, return error if ASSERT is disabled. */
      ASSERT(val[0] == FS_VAL_ITEM2 || val[0] == FS_VAL_ITEM);
      return -EBADF;
  }
    ...
    const void *buf, fs_size_t count, int mode)
{
  ...
  switch (value[0])
  {
    ...
    default:
      /* ASSSERT to catch corruption, return error if ASSERT is disabled. */
      ASSERT(value[0] == FS_VAL_ITEM2 || value[0] == FS_VAL_ITEM);
      return -EBADF;
  }
...
rename_item (struct nameidata *ndp, struct nameidata *ndp2)
{
  ...
  if (result == 0) {
    ...
    if (result == 0)
    {
      const uint8 *key;
      unsigned key_length;
      struct fs_vnode *vp = ndp2->vp;

      ndp2->dvp->p.inode.size++;
      ndp2->dvp->dirty = 1;

      /* Get the name of the item file as it is stored in the DB. For long
       * file name the filename is not stored contiguously. */
      result = fs_efs2_name_lookup (mp->db, ndp2->next, ndp2->length,
                                    dvp->inum);
      fs_efs2_name_last_key (&key, &key_length);
      fs_memscpy (vp->p.named.key, sizeof (vp->p.named.key), key, key_length);
      vp->p.named.key_length = key_length;
...
static int
v_efs_sync_start (struct fs_vnode *vp, int do_force_sync,
                  int is_shutdown_sync)
{
  struct fs_mount_efs *mp = (struct fs_mount_efs *) vp->mp;

  return mp->buf->sync_start (mp->buf, do_force_sync, is_shutdown_sync);
}
...
static int
v_efs_get_buf_for_fsg (struct fs_vnode *vp, uint8 **buf, uint32 *buf_size,
                       uint32 *hdr_offset)
{
  struct fs_mount_efs *mp = (struct fs_mount_efs *) vp->mp;

  return mp->buf->get_buf_for_fsg (mp->buf, buf, buf_size, hdr_offset);
}

static int
v_efs_write_to_fsg (struct fs_vnode *vp, uint8 *buf, uint32 buf_size,
                    uint32 partition)
{
  struct fs_mount_efs *mp = (struct fs_mount_efs *) vp->mp;

  return mp->buf->write_to_fsg (mp->buf, buf, buf_size, partition);
}
