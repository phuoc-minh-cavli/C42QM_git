...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_rmts_super.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
uint8
fs_rmts_super_get_img_compression_state (struct fs_rmts_superblock *sb)
{
  uint8 invalid = ~(0);
  ASSERT (sb != NULL);

  if (sb->is_image_compressed == 1)
  {
    return 1;
  }

  ASSERT ((sb->is_image_compressed == 0) ||
          (sb->is_image_compressed == invalid));
