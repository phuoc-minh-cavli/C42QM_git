 ...
 * fs_pm.h
 *
 * Copyright (C) 2003-2008,2010-2013,2015-2016 QUALCOMM Technologies, Inc.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_pm.h#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
struct fs_pm_ops
{
  ...
  int (*sync_start) (fs_pm_t pm_data, int do_force_sync,int is_shutdown_sync);
  ...
  int (*get_buf_for_fsg) (fs_pm_t gc, uint8 **buf, uint32 *buf_size,
                          uint32 *hdr_offset);

  /* Called to write to Golden Copy. */
  int (*write_to_fsg) (fs_pm_t gc, uint8 *buf, uint32 buf_size,
                       uint32 partition);
};
...
struct fs_pm_flash_data {
  ...
  fs_os_signal_handle_type timer_wait_sigh;
