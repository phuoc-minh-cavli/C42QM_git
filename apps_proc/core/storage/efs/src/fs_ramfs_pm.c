 ...
 * Copyright (C) 2010-2013,2015-2016 QUALCOMM Technologies, Inc.
 *
 * Page Manager for RamFS.
 *
 ***********************************************************************/
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs/src/fs_ramfs_pm.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
static int
fs_ramfs_pm_sync_start (fs_pm_t pm_data, int do_force_sync,
...
static int
fs_ramfs_pm_get_buf_for_fsg (fs_pm_t pm_data, uint8 **buf, uint32 *buf_size,
                             uint32 *hdr_offset)
{
...
static int
fs_ramfs_pm_write_to_fsg (fs_pm_t pm_data, uint8 *buf, uint32 buf_size,
                          uint32 partition)
{
...
fs_ramfs_pm_init_ops (fs_pm_t pm_data)
{
  ...
  pm_data->ops.get_buf_for_fsg = fs_ramfs_pm_get_buf_for_fsg;
  pm_data->ops.write_to_fsg = fs_ramfs_pm_write_to_fsg;
}
