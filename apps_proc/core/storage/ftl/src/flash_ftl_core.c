 ...
 * ----------   ---     ------------------------------------------------
 * 2019-07-24   sl      correct flash result check in FTL
 * 2019-01-09   sl      Updated FTL pad calculation formula
 * 2017-03-10   sl      Added support to copy data of bad block to next
                                ...
                                uint32 *backed_up_block_ptr)
{
  ...
  buffer = (char*)flash_ftl_aligned_malloc(page_size);
  ...
  INIT_BUF_DESC(page_buff_desc, page_data_iovec, buffer);
