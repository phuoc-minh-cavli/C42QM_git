...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs_qmi_put_get/service/src/fs_qmi_service.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
static fs_os_signal_handle_type fs_qmi_service_signal;
                        ...
                        void *req_c_struct, int req_c_struct_len)
{
  ...
  if (fs_qmi_service_access_check (req_msg->path) == 1)
  {
    result = efs_read_full_file (req_msg->path, resp_msg->data,
                                 req_msg->data_length);
  }
  else
  {
...
fs_qmi_service_init (void)
{
  ...
  fs_os_signal_init (&fs_qmi_service_signal);

  /* Initialize os_params. */
#ifdef FEATURE_EFS_OSAL_IS_QURT
  fs_qmi_service_obj.os_params.signal = &fs_qmi_service_signal;
#else
...
fs_os_signal_handle_type* fs_qmi_service_get_signal_object(void)
{
  return &fs_qmi_service_signal;
}
