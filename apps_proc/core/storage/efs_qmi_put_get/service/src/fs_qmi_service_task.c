...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/storage/efs_qmi_put_get/service/src/fs_qmi_service_task.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
fs_qmi_service_task (dword parm)
{
  ...
  tcb = fs_os_self();
  sigh = fs_qmi_service_get_signal_object();
  ...
  for (;;)
  {
    task_sigs = fs_os_wait (sigh, FS_QMI_EFS_SERVICE_HANDLE_EVENT);

    if ((task_sigs & FS_QMI_EFS_SERVICE_HANDLE_EVENT) != 0)
    {
      FS_MSG_MED_0 ("FS_QMI_EFS_SERVICE_HANDLE_EVENT received");
      fs_os_clr_sigs (tcb, sigh, FS_QMI_EFS_SERVICE_HANDLE_EVENT);
