 ...
 *
 * Copyright (c) 2008-2013, 2015, 2016 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/hal/flash_hal_main.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
  ...
  struct flash_hal_predef_page_data *page_data, enum flash_hal_status *status)
{
  ...
  while (cw_count < dev_info->config.cw_cnt)
  {
    ...
    {
      *status = FLASH_HAL_ERASED_PAGE_BITFLIP;
    }
    else if (zero_bit_count > 4)
    {
      *status = FLASH_HAL_FAIL;
