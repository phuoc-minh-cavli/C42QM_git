 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/iface/flash_bam_wrapper.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 10/03/16     svl     change added qurt_mem_region_create
...
qurt_mem_region_t desc_buf_handle = 0;
...
int flash_bam_core_dump_mem_alloc(void)
{
  ...
  if (dump_buffer == NULL)
  {
    ...
    dump_buffer = (void *)malloc(dump_buf_size);
    if (NULL == dump_buffer)
    {
      return FLASH_BAM_FAIL;
    }
...
int flash_bam_init(void)
{
  ...
  /* Allocate a physically contiguous buffer for Descriptor FIFO - check 
    ...
    /* changed this from phys to virtual
        as we don't have pys memory reserved for our work */
    ...
    alloc_attr.virtaddr=0xFFFFFFFF;
    alloc_attr.physaddr=0xFFFFFFFF;
    if(QURT_EOK != qurt_mem_region_create(  &desc_buf_handle,
                                            desc_fifo_size,
  ...
  qurt_mem_region_attr_get(desc_buf_handle, &alloc_attr);
 ...
 /* NAND Command Pipe Initialization */ 
  flash_bam_pipe_cfg.desc_base_va = alloc_attr.virtaddr;
  flash_bam_pipe_cfg.desc_base_pa = alloc_attr.physaddr;
  ...
  struct flash_bam_xfer_desc_list *desc_list)
{
  ...
  for(i = 1; i <= desc_list->desc_count; i++)
  {
    ...
    if (bam_status != BAM_SUCCESS)
    {
      ...
      ERR_FATAL( "Flash: bam transfer failed.!", 0, 0, 0 );
      return FLASH_BAM_FAIL;
    }
    desc_ptr++;
  }
...
int flash_bam_poll_pipe(bam_handle pipe_handle, void* user_data)
{
  ...
  {      
      ...
      bam_poll_result.event == BAM_EVENT_OUT_OF_DESC))
    {
      ...
      {
        /* Error Fatal */
        ERR_FATAL( "Flash: bam error, bam core dump failed!", 0, 0, 0 );
      }
      else
      {
        ERR_FATAL( "Flash: bam pipe poll failed.!", 0, 0, 0 );
      }
  ...
  {      
    ...
    {
      /* Error Fatal */
      ERR_FATAL( "Flash: xfer fail, bam core dump failed!", 0, 0, 0 );
    }
    else
    {
      /* Error Fatal */
  ...
  struct flash_bam_xfer_desc_list *write_desc_list)
{
  ...
  {
      ERR_FATAL( "Flash: invalid bam input params!", 0, 0, 0 );
    return FLASH_BAM_FAIL;
  }
  ...
  if(result != FLASH_BAM_SUCCESS)
  {
    ...
    ERR_FATAL( "Flash: bam tx failed.!", 0, 0, 0 );
  }
  return result;
}
...
int flash_bam_deinit(void)
{
  ...
  if(desc_buf_handle)
  {
    qurt_mem_region_delete(desc_buf_handle);
    desc_buf_handle = NULL;
  }
