 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_dal_config.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
...
/*
 * This API checks a valid partition table for 0:EFS2 (or "efs2") partition.
*/
static int flash_dal_check_efs2(void)
{
  ...
  if (parti_ptr)
  {
    ...
    for (i = 0; i < entries; i++)
    {
      ...
      if ((strncmp(entry->name, (const char *)MIBIB_EFS2_MODEM_PARTI_NAME, 
          ...
          (strncmp(entry->name, (const char *)"efs2", 
          FLASH_PART_NAME_LENGTH) == 0))
      {
        return TRUE;
      }
  ...
  {
     ERR_FATAL( "Flash DAL: Error! partition tbl is NULL!", 0, 0, 0 );
   }
  return FALSE;
}
...
enum flash_dal_config_state flash_dal_drv_get_config(void)
{
  ...
  if (flash_dal_check_smem_parti() == FLASH_DEVICE_DONE)
  {
    switch (flash_dal_config.config)
    {
      ...
      default:
      {
        ERR_FATAL( "Flash DAL: Error! unknown flash dal config state!", 0, 0, 0 );
      }
    }
