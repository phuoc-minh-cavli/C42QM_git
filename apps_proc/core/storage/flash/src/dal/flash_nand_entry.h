...
{
    qurt_mem_region_t         handle_hal_mem;   /* HAL Workspace mem handle */
    qurt_mem_region_attr_t    hal_mem_info;     /* HAL Workspace mem info */
    qurt_mem_region_t         handle_drv_mem;   /* Driver Workspace mem handle */
    qurt_mem_region_attr_t    drv_mem_info;     /* Driver Workspace mem info */
};
...
struct nand_entry_client_data
{
  ...
  qurt_mem_region_t nand_phys_handle;
  /* Memory buffers for page operations */
  struct flash_mem_desc_buf nand_phys_buf_list[FLASH_MAX_NUM_BUFS];
};
...
#define DO_CACHE_MAINTENANCE \
  ((nand_entry_init_data.meminfo.hal_mem_info.cache_attribs != QURT_MEM_CACHE_OUTER_INNER_UNCACHED) \
  && (nand_entry_init_data.meminfo.hal_mem_info.permission != QURT_PERM_FULL))
