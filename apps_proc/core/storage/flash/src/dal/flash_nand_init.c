 ...
 *
 * Copyright (c) 2009-2015, 2016, 2018 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 12/21/18     sl      added SPI NAND init support
 ...
 * 11/03/14     sb      Read partition table from shared memory to avoid MPU violation
...
static int nand_mem_init(flash_client_ctxt *client_ctxt)
{
  ...
  if (TRUE == nand_entry_init_data.nand_mem_initialized)
  {
    ...
    hal_ws_buffer = (uint32) meminfo->hal_mem_info.virtaddr;
    hal_ws_buffer_size = meminfo->hal_mem_info.size;
    ...
    nand_pdata->raw_page_buffer =
      (uint8 *) meminfo->drv_mem_info.virtaddr;

    nand_pdata->drv_ws_buffer =
      (uint32 *) (meminfo->drv_mem_info.virtaddr +
      dev_ctxt->dev_info.u.nand.total_page_size_bytes);

    memset(nand_pdata->raw_page_buffer, 0xFF, meminfo->drv_mem_info.size);
...
int nand_predef_init(flash_client_ctxt *client_ctxt)
{
  ...
  qurt_mem_region_attr_t *hal_mem_info, *drv_mem_info;
  ...
  predef_init_data.buff_data.buff_vaddr = hal_mem_info->virtaddr;
  predef_init_data.buff_data.buff_paddr = hal_mem_info->physaddr;
  predef_init_data.buff_data.buff_size = hal_mem_info->size;

  predef_init_data.raw_buff_data.buff_vaddr = drv_mem_info->virtaddr;
  predef_init_data.raw_buff_data.buff_paddr = drv_mem_info->physaddr;
  predef_init_data.raw_buff_data.buff_size = drv_mem_info->size;
...
int nand_close(flash_client_ctxt *client_ctxt)
{
  ...
  if (client_ctxt->client_data.client_specific_data)
  {
    ...
    free(client_ctxt->client_data.client_specific_data);
...
static int nand_init_client_info(flash_client_ctxt *client_ctxt)
{
  ...
  if (client_ctxt->client_data.client_specific_data == 0)
  {
    ...
    nand_client_data 
	  = (struct nand_client_specific_data *)malloc(sizeof(struct nand_client_specific_data));
	  
    if (NULL == nand_client_data)
    {
    ...
    memset((void *) nand_client_data, 0x0,
...
int nand_deinit (flash_client_ctxt *client_ctxt)
{
  ...
  {
    free(client_ctxt->flash_dev_ctxt->dev_info.common.dev_specific_data);
  ...
  {
    /* Free any malloced extra data for client */
    free(client_ctxt->flash_dev_ctxt->parti_ptr);
  ...
  struct flash_nand_params *device)
{
  ...
  {
    dev_ctxt->dev_info.common.dev_specific_data 
       = (struct nand_dev_pdata *)malloc(sizeof(struct nand_dev_pdata));
    
    if (NULL == dev_ctxt->dev_info.common.dev_specific_data)
    {
      FLASHLOG(3, ("\nMalloc failed\n"));
  ...
  struct flash_nand_params *device)
{
  ...
  nand_pdata->raw_page_buffer =
    (uint8 *) flash_mem_data->drv_mem_info.virtaddr;

  nand_pdata->drv_ws_buffer =
    (uint32 *) (flash_mem_data->drv_mem_info.virtaddr +
    dev_ctxt->dev_info.u.nand.total_page_size_bytes);
...
static int nand_get_device_list_supported(struct flash_hal_dev_config *nand_cfg,
  enum flash_hal_dev_type hal_dev_type,
  enum flash_hal_status  *hal_status)
{
       ...
       FLASH_HAL_DEVICE_NONE); cfg_count++ )
  {
     ...
     */
    if (hal_dev_type == nand_cfgs[cfg_count].dev_type)
    {
      /* Basic configuration for flash HAL client init */
      nand_cfg->dev_type = hal_dev_type;
   ...
   uint8** ids)
{
  ...
  enum flash_hal_dev_type hal_dev_type = FLASH_HAL_DEVICE_NONE;

  FLASHLOG(3, ("\nProbing for NAND device ...\n"));

  switch(flash_get_boot_dev_type())
  {
    case FLASH_BOOT_DEV_NAND:
    {
      hal_dev_type = FLASH_HAL_DEVICE_NAND;
      FLASHLOG(3, ("\n Boot Device type is Parallel NAND ...\n"));
    }break;
    case FLASH_BOOT_DEV_NAND_SPI:
    {
      hal_dev_type = FLASH_HAL_DEVICE_NAND_SPI;
      FLASHLOG(3, ("\n Boot Device type is SPI NAND ...\n"));
    }break;
    default:
      hal_dev_type = FLASH_HAL_DEVICE_NONE;
      FLASHLOG(3, ("\nUnknown device type for boot...\n"));
  }

  /* Search the BSP to see if it has any NAND device configured to be
     probed on the first configured chip select */
  cfg_count = nand_get_device_list_supported(&nand_cfg, hal_dev_type, &hal_status);
...
int nand_probe(flash_client_ctxt *client_ctxt)
{
  ...
  *********************************************************************************/
  ...
  /* Allocate private data shared across clients */
  nand_pdata = (struct nand_dev_pdata *)malloc(sizeof(struct nand_dev_pdata));
