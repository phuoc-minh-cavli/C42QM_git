 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_dal_util.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 11/25/12     svl     Added Api to clean+invalidate cache
...
uint32 flash_vtop_get_physical(uint32 virt_addr)
{
  ...
  while (item_count < virt_phys_list.no_of_entries)
  {
    ...
    */
    if ((vitem->vstart != FLASH_MEM_ADDR_NOT_SPECIFIED) &&
...
{
  busywait(wait_time_us); //from systemdrivers
}

/* This function does cache clean on the given address */
void flash_dcache_clean(uint32 addr, uint32 len)
{
  qurt_mem_cache_clean((qurt_addr_t)addr, (qurt_size_t)len, QURT_MEM_CACHE_FLUSH, QURT_MEM_DCACHE);
}

/* This function does cache invalidation on the given address */
void flash_dcache_inval(uint32 addr, uint32 len)
{
  qurt_mem_cache_clean((qurt_addr_t)addr, (qurt_size_t)len, QURT_MEM_CACHE_INVALIDATE, QURT_MEM_DCACHE);
}

/*
 * This function does cache clean on the given address and
 * then invalidates that address
 */
void flash_dcache_clean_inval(uint32 addr, uint32 len)
{
  qurt_mem_cache_clean((qurt_addr_t)addr, (qurt_size_t)len, QURT_MEM_CACHE_FLUSH_INVALIDATE, QURT_MEM_DCACHE);
}
