 ...
 *
 * Copyright (c) 2008-2013, 2016, 2018-2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_nand_partition.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 05/08/19     sl      Update partition table from smem for every open call
 * 11/26/18     sb      Update to efs2apps
 * 10/03/18     eo      Set mibib search parameters based on mibib partition info
 * 10/03/18     eo      Support partition names without "0:"
 * 10/03/18     svl     Fix mibib age initialization
...
static struct flash_mem_desc_buf parti_buff_desc[2];
...
static __inline void init_buffer_descriptor(flash_client_ctxt *client_ctxt,
  uint32 buff_addr, uint32 page_count, struct flash_mem_desc_buf *buffer_desc)
{
  struct flash_mem_desc_buf *desc_buff;
  qurt_mem_region_attr_t memInfo;

  desc_buff = &buffer_desc[0];
  ...
  uint32 read_type, void *buffer)
{
  ...
  init_buffer_descriptor(client_ctxt, (uint32) buffer, 1, &parti_buff_desc[0]);
  ...
  result = nand_entry_read_pages(client_ctxt, (enum page_read_opcode) read_type,
     &parti_buff_desc[0], &page_data_iovec);
  ...
  const unsigned char *name)
{
  ...
  {
    if (strncmp((const char *)"0:EFS2APPS", (const char *)name, FLASH_PART_NAME_LENGTH) == 0)
    {
      if (strncmp(partentry->name, (const char *)"efs2apps", 
        FLASH_PART_NAME_LENGTH) == 0)
      {
        return TRUE;
      }
    ...
    if (DAL_SUCCESS != result)
    {
      if (FLASH_DEVICE_FAIL_PAGE_ERASED == result)
      {
        FLASHLOG (4, ("fnibm: MIBIB page 0x%x is erased. Filling buf w/ 0xFF\n",
          page));
        memset((void *)page_buf, fill_char,
          client_ctxt->flash_dev_ctxt->dev_info.u.nand.page_size_bytes);
      }
      else
      {
...
int smem_retrieve_mibib(flash_client_ctxt *client_ctxt)
{
  ...
  if (client_ctxt->flash_dev_ctxt->parti_ptr == NULL)
  {
    client_ctxt->flash_dev_ctxt->parti_ptr 
	  = (flash_partable_t *)malloc(client_ctxt->flash_dev_ctxt->dev_info.u.nand.page_size_bytes);
	  
    if (NULL == client_ctxt->flash_dev_ctxt->parti_ptr)
    {
      FLASHLOG(3, ("\nMalloc failed\n"));
  ...
  memscpy((void *)page_buf, sizeof(struct flash_partition_table), 
     (void *)smem_aarm_partition_table, sizeof(struct flash_partition_table));
      ...
      (parti_ptr->version == FLASH_PARTITION_VERSION) )
  {
    ...
    memscpy(client_ctxt->flash_dev_ctxt->parti_ptr, 
	  client_ctxt->flash_dev_ctxt->dev_info.u.nand.page_size_bytes,
      (const void *)page_buf,
      client_ctxt->flash_dev_ctxt->dev_info.u.nand.page_size_bytes);
...
int nand_retrieve_mibib(flash_client_ctxt *client_ctxt)
{
  ...
  int cur_block, age = 0;
  int mibib_parti_search_min = -1, mibib_parti_search_max = -1;
  int result = FLASH_DEVICE_DONE;
  ...
  {
    client_ctxt->flash_dev_ctxt->parti_ptr 
	   = (flash_partable_t *)malloc(FLASH_PARTITION_TABLE_SIZE);
  ...
  /* At this point, flash driver should have read the MIBIB partition table from SMEM as part of its
  ...
  if ((flash_dal_config.config == FLASH_DAL_CONFIG_NAND) &&
      (flash_dal_config.parti_tbl != NULL))
  {
    entry_ptr = nand_partition_find_entry(flash_dal_config.parti_tbl,
    ...
    if (entry_ptr != (flash_partentry_t)0)
    {
      ...
      mibib_parti_search_max = mibib_parti_search_min + entry_ptr->length - 1;
    }
    else
    {
  ...
  {
    FLASHLOG (4, ("Error: Flash driver is not configured as NAND. \n"));
    return FLASH_DEVICE_FAIL;
  }

  /* Start looking from first MIBIB block */
  cur_block = mibib_parti_search_min;
  
#else
  /* Start looking from first MIBIB block */
  cur_block = MIBIB_BLOCK_SEARCH_MIN;
  mibib_parti_search_max = MIBIB_BLOCK_SEARCH_MAX;
#endif

  while ((cur_block <= mibib_parti_search_max))
  {
  ...
  if (cur_block > mibib_parti_search_max)
  {
  ...
  {
    /*  This code should never execute, if it does, there's an issue */
...
#endif
          memscpy(client_ctxt->flash_dev_ctxt->parti_ptr, FLASH_PARTITION_TABLE_SIZE, 
  ...
  else /* if new_mibib_block == -1 */
  {
    FLASHLOG(5, ("Valid MIBIB block not found\n"));
    return FLASH_DEVICE_FAIL;
  ...
  const unsigned char *parti_name)
{
  ...
  if (strncmp((const char *)parti_name, "all", 4) == 0)
  {
    client_ctxt->client_data.partition_block_start   = 0;
    client_ctxt->client_data.partition_block_limit   =
      client_ctxt->flash_dev_ctxt->dev_info.u.nand.block_count;
  ...
  else
  {
    ...
    {
      FLASHLOG (5, (" Retrieving MIBIB from smem failed %x: partition name :%s\n",result, parti_name));
      return result;
    }
  ...
  } /* If partition being opened is not "all" */
