 ...
 *
 * Copyright (c) 2013, 2016, 2018 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_partition_services.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
...
int flash_partition_table_update (flash_handle_t handle, flash_partable_t parti_table)
{
  ...
  while (new_mibib_block <= (mibib_parti_search_min + mibib_parti_search_len))
  {
    ...
    else
    {
      ...
      memscpy(client_ctxt->flash_dev_ctxt->parti_ptr, sizeof(struct flash_partition_table),
  ...
  /* Error fatal if good blocks are not found in MIBIB partition */
   ERR_FATAL( "Flash: MIBIB blocks are bad.!", 0, 0, 0 );
   return FLASH_DEVICE_FAIL;
}
...
int flash_partition_table_get (flash_handle_t handle, flash_partable_t parti_table)
{
  ...
  memscpy((void *)parti_table, sizeof(struct flash_partition_table),
     (void *)client_ctxt->flash_dev_ctxt->parti_ptr, sizeof(struct flash_partition_table));
