 ...
 *
 * Copyright (c) 2008-2015, 2018 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_onfi.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 10/03/18     eo      Use 4 byte aligned read for BBM marker 
...
retry:
  ...
  {
    memscpy(buffer, num_bytes, nand_pdata->raw_page_buffer, num_bytes);
  }
  ...
  struct flash_nand_params **device_params, uint8 *is_onfi)
{
  ...
  *is_onfi = FALSE;
  ...
  {
    *device_params = (struct flash_nand_params *)(malloc(sizeof(struct flash_nand_params)));

    if (NULL != *device_params)
    {
      onfi_parameter_page = (byte *)malloc(ONFI_PARAMETER_PAGE_SIZE * ONFI_READ_PARAMETER_PAGE_MAX);

      if (NULL == onfi_parameter_page)
      {
        free(*device_params);
        *device_params=NULL;
      }
