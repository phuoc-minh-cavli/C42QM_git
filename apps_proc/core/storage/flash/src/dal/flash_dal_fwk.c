 ...
 *   order.
 *
 *  $Header: //components/rel/core.tx/6.0/storage/flash/src/dal/flash_dal_fwk.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
 *
 *
  ...
  DalDeviceHandle **flash_dal_handle)
{
  ...
  client_ctxt = (flash_client_ctxt *)malloc(sizeof(flash_client_ctxt));

  if (NULL != client_ctxt)
  {
    memset(client_ctxt, 0x0, sizeof(flash_client_ctxt));
  ...
  {
    err = DAL_ERROR_INSUFFICIENT_MEMORY;
  }
