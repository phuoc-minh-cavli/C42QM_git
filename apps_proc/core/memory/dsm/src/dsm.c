...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/memory/dsm/src/dsm.c#1 $
...
)
{
	       ...
	       DSMI_ITEM_HEAD(item_ptr)) >= size))
  {
    ...
    if(buf != NULL)
    {
      memscpy_i((*pkt_head_ptr)->data_ptr,size,buf,size);
    }
#ifdef FEATURE_DSM_MEM_CHK
    dsmi_touch_item(item_ptr, file, line);
#endif
...
)
{
		  ...
		  DSMI_ITEM_HEAD(item_ptr))) != 0)
  {
    ...
    if( buf != NULL )
    {
      memscpy_i((*pkt_head_ptr)->data_ptr,frag,
	  	   ((char*)buf) + size - frag,
		   frag);
    } /* if !buf */
    size -= frag;
...
)
{
  ...
  while(cnt != 0 && (item_ptr = *pkt_head_ptr) != NULL)
  {
    ...
    if(buf != NULL)
    {
      ...
      {
        memscpy_i((uint8 *)buf, bytes_this_item,item_ptr->data_ptr, bytes_this_item);
      }
...
)
{
  ...
  while(((*pkt_head_ptr) != NULL) && (cnt != 0))
  {
    ...
    if (buf != NULL)
    {
      memscpy_i(buf, bytes_this_item, (*pkt_head_ptr)->data_ptr + offset, bytes_this_item);
      buf = (uint8*) buf + bytes_this_item;
    }
    tot += bytes_this_item;
    cnt -= bytes_this_item;
...
)
{
  ...
  while(packet_ptr != NULL && len != 0)
  {
    ...
    memscpy_i(buf,n, packet_ptr->data_ptr + offset, n);
    copied += n;
    buf = (uint8*)buf + n;
    len -= n;
    offset = 0;
...
)
{
  ...
  while(item_ptr != NULL && to_copy > 0)
  {
    ...
    memscpy_i( buf_ptr,can_copy, item_ptr->data_ptr + (uint16)offset, can_copy);
    buf_ptr += can_copy;
    to_copy -= can_copy;
