...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/memory/dsm/src/dsm_queue.c#1 $

when        who    what, where, why
--------    ---    ---------------------------------------------------------- 
05/07/14    sko    Added dne_q_cnt
05/06/14    sh      Added dne_statistics
05/06/14    sh      Added dne_callback
...
#define DSMI_CHECK_WM_PTR(wm_ptr)\
  do { if( wm_ptr == NULL ){\
        ERR_FATAL("DSM WM POINTER IS NULL",0,0,0);\
       }\
                     ...
                     boolean enqueue)
{
 ...
 /* Update min_cnt if the current_cnt has gone below min_cnt for the
    current period */
 if(wm_ptr->current_cnt < wm_ptr->min_cnt)
 {
   wm_ptr->min_cnt = wm_ptr->current_cnt;
 }

 /* Update max_cnt if the current_cnt has exceeded max_cnt for the
    current period */
 if(wm_ptr->current_cnt > wm_ptr->max_cnt)
 {
   wm_ptr->max_cnt = wm_ptr->current_cnt;
 }
...
)
{
  ...
  if (wm_ptr->q_ptr->cnt > wm_ptr->dne_q_cnt)
  {
    ...
    wm_ptr->dne_q_cnt_discard_bytes_cnt += item_length;
    /*----------------------------------------------------------------------- 
      Display message, put item to free queue. 
      -----------------------------------------------------------------------*/
    MSG_4(MSG_SSID_DFLT, MSG_LEGACY_ERROR,
         "WM full - dne_Q_cnt hit,Pkt freed 0x%x:WM 0x%x: Discard cnt %d : dne_q_cnt %d",
          (int) *pkt_head_ptr,
          (int) wm_ptr,
  ...
  else 
  {
    ...
    wm_ptr->dne_event_cnt++;
    #endif
    wm_ptr->dne_discard_pkt_cnt++; 
    wm_ptr->dne_discard_bytes_cnt += item_length;
    ...
    -----------------------------------------------------------------------*/
    MSG_4(MSG_SSID_DFLT, MSG_LEGACY_ERROR,
         "WM full,freeing packet 0x%x:Watermark 0x%x:Discard cnt %d: dne %d",                  
          (int) *pkt_head_ptr,
          (int) wm_ptr,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    DSM_ASSERT(wm_ptr->current_cnt == 0);
  }
  DSMI_QUEUE_UNLOCK_WM(wm_ptr);
  return item_ptr;
...
)
{
  ...
  /* Copy all the counts */
  wm_cnts->min_cnt              =  wm_ptr->min_cnt;
  wm_cnts->max_cnt              =  wm_ptr->max_cnt;
  wm_cnts->hi_event_cnt         =  wm_ptr->hi_event_cnt;
  wm_cnts->lo_event_cnt         =  wm_ptr->lo_event_cnt;
  wm_cnts->dne_event_cnt        =  wm_ptr->dne_event_cnt;
  wm_cnts->empty_event_cnt      =  wm_ptr->empty_event_cnt;

  /* Reset the min, max and event counts to current_cnt for the next
     window period */
  wm_ptr->min_cnt               =  wm_ptr->current_cnt;
  wm_ptr->max_cnt               =  wm_ptr->current_cnt;
  wm_ptr->hi_event_cnt          =  0;
  wm_ptr->lo_event_cnt          =  0;
  wm_ptr->dne_event_cnt         =  0;
  wm_ptr->empty_event_cnt       =  0;
