...
Copyright (c) 2000, 2001, 2004, 2008 - 2010 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/memory/dsm/src/dsmutil.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
)
{
  ...
  if (table->free_count > threshold)
  {
    if (NULL == (allocated_dsm_ptr = dsmi_new_buffer(DSM_DS_LARGE_ITEM_POOL, file, line)))
    {
      ...
      if (fall_back && (table->free_count > threshold))
      {
        allocated_dsm_ptr = dsmi_new_buffer(DSM_DS_SMALL_ITEM_POOL, file, line);
      }
    }
  ...
  else /* allocate small Item */
  {
    ...
    else if (table->free_count <= threshold)
    {
