...
/*=============================================================================
Edit History
$Header: //components/rel/core.tx/6.0/debugtools/utils/src/tms_utils_fmt.c#1 $
...
static int32_t tms_utils_fmt_put(tms_chr* out_buf_p, const size_t out_buf_sz, tms_chr const* fmt_str, va_list argv)
{
   ...
   if (out_buf_p && out_buf_sz && fmt_str)
   {
      ...
      do
      {
         ...
         for (field_width.ul = NUMERIC_ZERO; ASCII_ZERO <= fmt_chr && ASCII_9 >= fmt_chr; fmt_chr = *fmt_str++)
         {
         ...
         switch (fmt_code)
         {
            ...
            case ASCII_CAP_S:                                                          // next field is template, %s
            {
               ...
               for (fmt_fill.ul = NUMERIC_ZERO; fmt_fill.ul <= out_buf_sz - (out_p - out_buf_p) && chrs_p && ASCII_NIL != *(chrs_p + fmt_fill.ul); fmt_fill.ul++)
                  /*NULL*/ ;
         ...
         if ((ASCII_D == fmt_code || ASCII_CAP_D == fmt_code) && NUMERIC_ZERO > (int32_t)field_arg.ul)
         {
            field_flag.ul |= FIELD_FLAG_SIGN;

            field_arg.ul = NUMERIC_ZERO - (int32_t)field_arg.ul;                                            // 2's compliment obtains absolute value
         }
