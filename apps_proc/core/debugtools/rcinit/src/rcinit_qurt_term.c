...
/*=============================================================================
Edit History
$Header: //components/rel/core.tx/6.0/debugtools/rcinit/src/rcinit_qurt_term.c#1 $
...
void rcinit_internal_unprocess_groups(void)
{
   ...
   timer_def_osal(&tmo, &timer_group, TIMER_NATIVE_OS_SIGNAL_TYPE, &signal, SIG_TMO);

   timer_def_osal(&bail, &timer_group, TIMER_NATIVE_OS_SIGNAL_TYPE, &signal, SIG_BAIL);
   ...
   if (0 != rcinit_term_timeout)                                                 // condition check used to start bailout timer
   {
      ...
      qurt_signal_clear(&signal, SIG_BAIL);                                        // clear signals
   ...
   do
   {
      ...
      if (0 != timetick)
      {
         ...
         qurt_signal_clear(&signal, SIG_TMO);
      ...
      qurt_signal_clear(&signal, SIG_ACK);                       // clear signals before waiting
      ...
      if (count > 0)                                                             // block only if count is positive
      {
         ...
         do
         {
            ...
            recv = qurt_signal_wait(&signal, SIG_ACK | SIG_TMO | SIG_BAIL, QURT_SIGNAL_ATTR_WAIT_ANY); // wait for HANDSHAKE ACK, GROUP TIMEOUT, BAILOUT TIMEOUT

            qurt_signal_clear(&signal, recv);                              // clear signals, then handle
            ...
            if (recv & SIG_BAIL)                                                 // BAIL OUT - MAXIMUM TIME
            {
               ...
               timetick = rcinit_term_tmo[RCINIT_GROUP_0];

               if (rcecb_signal_name_timetick(rcinit_term_rcecb[RCINIT_GROUP_0], &timetick)) {;} // signal registered functions of their event, returns when done
      ...
      qurt_signal_clear(&signal, SIG_ACK | SIG_TMO);             /* Clear any pending TIMEOUT or ack signals */
   ...
   if (0 != rcinit_term_latency_enable && 0 != rcinit_term_timeout)              // condition check used to start bailout timer
   {
      ...
      do
      {
         ...
         recv = qurt_signal_wait(&signal, SIG_TMO | SIG_BAIL, QURT_SIGNAL_ATTR_WAIT_ANY);             // artificial delay for bailout timer

         qurt_signal_clear(&signal, recv);                                 // clear signals
