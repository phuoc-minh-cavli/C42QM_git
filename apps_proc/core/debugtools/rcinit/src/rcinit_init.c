...
/*=============================================================================
Edit History
$Header: //components/rel/core.tx/6.0/debugtools/rcinit/src/rcinit_init.c#1 $
...
/* Signal object for RCINIT WORKER execution */
#define RCINIT_WORKER_TRIGGER     0x0000001
#define RCINIT_WORKER_ACK         0x0000002
#define RCINIT_HANDSHAKE_INIT     0x0000004
...
/*===========================================================================
 ...
 ===========================================================================*/
static void rcinit_worker_internal(void)
{
...
{
	/* Signal mask for procedure waits */
	uint32 sigs;

	while(1)
		{
		/* Wait for the trigger signal before starting RCINIT_WORKER */
		sigs = qurt_signal_wait(&rcinit_internal.worker_sigobj, 
							RCINIT_WORKER_TRIGGER, 
							QURT_SIGNAL_ATTR_WAIT_ANY);

		if(sigs & RCINIT_WORKER_TRIGGER)
			{
			/* Call the internal function to process init functions */
			rcinit_worker_internal();

			/* Clear the signal for the received qurt signal */
			qurt_signal_clear(&rcinit_internal.worker_sigobj, RCINIT_WORKER_TRIGGER);
			}

		/* Send the ACK signal */
		qurt_signal_set(&rcinit_internal.worker_sigobj, RCINIT_WORKER_ACK);
		}
...
void rcinit_create_worker_thread(void)
{
	static qurt_thread_attr_t qurt_attr;
	static qurt_thread_t qurt_tid;
	int ret = 0;
	
	/* Initialize the signal object to be used for the worker thread sync */
	qurt_signal_init(&rcinit_internal.worker_sigobj);

	/* Create the thread to handle init function processing for the RCINIT groups */
	qurt_thread_attr_init(&qurt_attr);
	qurt_thread_attr_set_name(&qurt_attr, RCINIT_WORKER);
	qurt_thread_attr_set_stack_size(&qurt_attr, rcinit_lookup_stksz(RCINIT_WORKER));
	qurt_thread_attr_set_priority(&qurt_attr, rcinit_lookup_prio(RCINIT_WORKER));
	
	ret = qurt_thread_create((qurt_thread_t*)(&qurt_tid), &qurt_attr, rcinit_worker, (void*)NULL);
...
void rcinit_handshake_init(void)                                                 // preferred API
{
   ...
   else
   {
          ...
          rcinit_process_group_init_fn == rcinit_internal.process_state)            // processing initfns (only)
      {
		...
		qurt_signal_set(&rcinit_internal.init_fn_spawn_sigobj, RCINIT_HANDSHAKE_INIT);
      }
...
void rcinit_initfn_spawn_task(RCINIT_INFO info_p, void* entry)
{
       ...
       RCINIT_NULL != entry)                                                     // must have an entry point argument
   {
      ...
      qurt_signal_init(&rcinit_internal.init_fn_spawn_sigobj);
      ...
      if (RCINIT_NULL != rcinit_p->handle && RCINIT_ENTRY_NONE != rcinit_p->handle->entry)
      {
         if (RCINIT_TASK_DALTASK == rcinit_p->type)
         {
            ...
            ERR_FATAL("DALTASKS are not available", 0, 0, 0);
         ...
         else if (RCINIT_TASK_POSIX == rcinit_p->type)
         {
			...
			#endif
			ERR_FATAL("POSIX is not available", 0, 0, 0);
      ...
      {
         // DAL Event Objects Are Protected on Context Allowed to Wait (Create/Wait/Destroy)
         qurt_signal_wait(&rcinit_internal.init_fn_spawn_sigobj,
         					RCINIT_HANDSHAKE_INIT, 
         					QURT_SIGNAL_ATTR_WAIT_ANY);
      }
      // Destroy the signal object
      qurt_signal_destroy(&rcinit_internal.init_fn_spawn_sigobj);
   }
   else
   {
   ...
   while (rcinit_internal.group_curr < RCINIT_GROUP_MAX)
   {
      ...
      for (rcinit_internal.group_curr_idx = RCINIT_GROUP_0; RCINIT_NULL != rcinit_internal.group_base[rcinit_internal.group_curr_idx]; rcinit_internal.group_curr_idx++)
      {
             ...
             rcinit_info_rcinit_p != rcinit_p)                                   // must not be rcinit, rcinit is the bootstrap
         {
            ...
            {
               // NO STACK ALLOCATIONS; ThreadX qurt_thread_create api allocates stack itself
            }
      ...
      for (rcinit_internal.group_curr_idx = RCINIT_GROUP_0; RCINIT_NULL != rcinit_internal.group_base[rcinit_internal.group_curr_idx]; rcinit_internal.group_curr_idx++)
      {
         ...
         if (IS_INITFN(rcinit_p))
         {
			...
			/* trigger rcinit_worker */
			qurt_signal_set(&rcinit_internal.worker_sigobj, RCINIT_WORKER_TRIGGER);
			...
			if (sigs & RCINIT_WORKER_ACK)
				{
				/* clear this signal */
				qurt_signal_clear(&rcinit_internal.worker_sigobj, RCINIT_WORKER_ACK);
				}
      ...
      for (rcinit_internal.group_curr_idx = RCINIT_GROUP_0; RCINIT_NULL != rcinit_internal.group_base[rcinit_internal.group_curr_idx]; rcinit_internal.group_curr_idx++)
      {
         ...
         if (IS_TASK(rcinit_p))
         {
            if (RCINIT_ENTRY_NONE != rcinit_p->handle->entry)
            {
               ...
               {
                  // NO STACK ALLOCATIONS; ThreadX qurt_thread_create api internally allocates
...
void rcinit_init(void)
{
   ...
   if (RCINIT_POLICY_NONE != rcinit_internal.policy_curr)
   {
      ...
      rcinit_create_worker_thread();                                           // internal worker thread
