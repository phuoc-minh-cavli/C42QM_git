...
$Header: //components/rel/core.tx/6.0/debugtools/sys_m/src/lib/9x65/apps/sys_m_diag.c#1 $
...
__packed void * sys_m_diag_control (__packed void *req_pkt, uint16 pkt_len)
{
    ...
    if ( (req->mode == MODE_RESET)|| (req->mode == MODE_SHUTDOWN)) //shutdown
    {
        ...
        MSG(MSG_SSID_TMS, MSG_LEGACY_HIGH,"SYS_M_DIAG received MODE change request");
