...
Copyright (c) 2011-2017, 2020 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/core.tx/6.0/debugtools/sys_m/src/lib/9205/apps/sys_m.c#3 $
...
/* Treat hibernation to be similar to shutdown until HW suports it with a difference that instead of
 * powering off chip using DAL API, send command to RPM to enter hibernation 
 */
#define HIBERNATION_AS_SHUTDOWN 1
...
/* Shutdown Type*/
shutdown_type sys_m_modem_shutdown_type = SYS_M_SHUTDOWN_DEFAULT;

#define SYS_M_BUFFER_LEN 50
char sys_m_buffer[SYS_M_BUFFER_LEN];
int sys_m_reset_in_progress = 0;
int sys_m_hibernation_in_progress = 0;
unsigned int sys_m_hibernation_time_ms = 0;
int sys_m_true_reset = 0;
int sys_m_apps_timeout = 0;
boolean sys_m_reset_over_ssctl_v02 = FALSE;

#if HIBERNATION_AS_SHUTDOWN
int sys_m_hibernation_is_shutdown = 0;
#endif
...
static qurt_signal_t sys_m_signal_obj;
static qurt_signal_t sys_m_hibernation_blocking_obj;
...
static void sys_m_hibernation_notify_internal(void);
static void sys_m_hibernation_send_to_rpm(unsigned int h_time_ms);
...
void sys_m_sio_rx_notify (dsm_item_type ** item_ptr)
{
    ...
    if (bytes_sent <= SYS_M_BUFFER_LEN)
    {
		...
		qurt_signal_set(&sys_m_signal_obj, SYS_M_AP_NOTIFY_RCV);
    }
    else
    {
...
{
    sys_m_true_reset = 1;
	qurt_signal_set(&sys_m_signal_obj, SYS_M_INITIATE_RESET);
}
...
{
  TMS_MSG_HIGH_1("sys_m_hibernation for time (ms) = %d", h_time_ms);
  
  if (h_time_ms <= sys_m_apps_timeout)
  {
    return SYS_M_HIBERNATION_ERR_INVALID_H_TIME;
  }
  else
  {
    sys_m_hibernation_time_ms = h_time_ms;
  }
#if !HIBERNATION_AS_SHUTDOWN
  qurt_signal_set(&sys_m_signal_obj, SYS_M_INITIATE_HIBERNATION);
#else
  sys_m_hibernation_is_shutdown = 1;
  qurt_signal_set(&sys_m_signal_obj, SYS_M_INITIATE_RESET);
#endif

  // Block client until hibernation completion
  qurt_signal_wait( &sys_m_hibernation_blocking_obj, 
                    SYS_M_BLOCKING_HIBERNATION, 
                    QURT_SIGNAL_ATTR_WAIT_ANY);

  return SYS_M_HIBERNATION_ERR_NONE;
}
...
{
	qurt_signal_set(&sys_m_signal_obj, SYS_M_COMPLETE_RESET);
}

/**
 *
 * @brief sys_m_complete_hibernation
 *
 * Triggers the complete hibernation sequence
 * 
*/
void sys_m_complete_hibernation(void)
{
  qurt_signal_set(&sys_m_signal_obj, SYS_M_COMPLETE_HIBERNATION);
}
...
void sys_m_task(unsigned long param)
{
  ...
  timer_type sys_m_reset_timer = {0};
  timer_type sys_m_hibernation_timer = {0};
  timer_group_type sys_m_timer_group = {0};
  ...
  qurt_signal_init(&sys_m_signal_obj);
  qurt_signal_init(&sys_m_hibernation_blocking_obj);

  /* Set up timer */
  timer_group_set_deferrable(&sys_m_timer_group, FALSE);
  timer_def_osal( &sys_m_reset_timer,
            			&sys_m_timer_group,
            			TIMER_NATIVE_OS_SIGNAL_TYPE,
            			&sys_m_signal_obj,
            			SYS_M_COMPLETE_RESET);
  timer_def_osal( &sys_m_hibernation_timer,
            			&sys_m_timer_group,
            			TIMER_NATIVE_OS_SIGNAL_TYPE,
            			&sys_m_signal_obj,
  ...
  //task forever loop
    for (;;)
    {
		sigs = qurt_signal_wait(&sys_m_signal_obj, 
					SYS_M_INITIATE_RESET | SYS_M_COMPLETE_RESET | SYS_M_AP_NOTIFY_RCV |
					SYS_M_INITIATE_HIBERNATION | SYS_M_COMPLETE_HIBERNATION, 
						QURT_SIGNAL_ATTR_WAIT_ANY);

    TMS_MSG_HIGH_1("event received at sys_m_task %d", sigs);

        //mode reset -- prepare modem for reset/shutdown
        if (sigs & SYS_M_INITIATE_RESET)
        {
			qurt_signal_clear( &sys_m_signal_obj, SYS_M_INITIATE_RESET );
			if (!sys_m_reset_in_progress)
            {
                sys_m_reset_in_progress = 1; /*once set this is never cleared by design*/
                if(sys_m_true_reset) /* APPS wants to do system reset/restart */
                {
                    ssctl_v02_send_req(MPSS_SSCTL_SVC, SSCTL_RESTART, NULL);
                }
                else /* APPS wants to do system shutdown */
                {
        ...
        else if (sigs & SYS_M_COMPLETE_RESET)
        {
			...
			qurt_signal_clear( &sys_m_signal_obj, SYS_M_COMPLETE_RESET );
    ...
    //hibernation -- prepare modem for hibernation
    if (sigs & SYS_M_INITIATE_HIBERNATION)
    {
      qurt_signal_clear( &sys_m_signal_obj, SYS_M_INITIATE_HIBERNATION );
      if (!sys_m_hibernation_in_progress)
      {
        sys_m_hibernation_in_progress = 1;
        ssctl_v02_send_req(MPSS_SSCTL_SVC, SSCTL_HIBERNATION, NULL);
        timer_set(&sys_m_hibernation_timer, sys_m_apps_timeout, 0, T_MSEC);
      }
    }

    //hibernation -- physically hibernation chip
    if (sigs & SYS_M_COMPLETE_HIBERNATION)
    {
      timer_clr(&sys_m_hibernation_timer, T_NONE);
      qurt_signal_clear( &sys_m_signal_obj, SYS_M_COMPLETE_HIBERNATION );
      //notify internally (blocks until complete)
      sys_m_hibernation_notify_internal();

      // Send hibernation cmd to rpm
      sys_m_hibernation_send_to_rpm(sys_m_hibernation_time_ms);

      // Un-block caller
      qurt_signal_set(&sys_m_hibernation_blocking_obj, SYS_M_BLOCKING_HIBERNATION);
    }

        //message received from AP over HSIC
        else if (sigs & SYS_M_AP_NOTIFY_RCV)
        {
			qurt_signal_clear( &sys_m_signal_obj, SYS_M_AP_NOTIFY_RCV );
...
/** sys_m_hibernation_notify_internal
 *
 * Signal interested modules to prepare for hibernation.
 */
static void sys_m_hibernation_notify_internal(void)
{
  ...
  TMS_MSG_HIGH("Hibernation: Notify clients start.");

  // RC-ECB Callbacks
  rcecb_signal_name(SYS_M_HIBERNATION);
  
  registrants_count = rcevt_getregistrants_name(SYS_M_HIBERNATION);
  ack_count = rcevt_getcount_name(SYS_M_HIBERNATION_ACK);

  // RC-EVT Signal & Blocking Wait for ACK
  rcevt_signal_name(SYS_M_HIBERNATION);
  rcevt_wait_count_name(SYS_M_HIBERNATION_ACK, registrants_count + ack_count);

  TMS_MSG_HIGH("Hibernation: Notify clients end.");
}

/** sys_m_hibernation_send_to_rpm
 * 
 * Send CMD to RPM for Hibernation.
 *
 * @param h_time_ms Total time for hibernation (in ms)
 *
 * CMD details:
 * Resource name: 0x6d6370  // resource name
 * Resource Id:   0
 * Key:           0x656d6974  //hibernation timeout key
 * Value:         <timeout value to set>
 */
#define HIBERNATION_RESOURCE 0x6d6370
#define HIBERNATION_TIME_KEY 0x656d6974
static void sys_m_hibernation_send_to_rpm(unsigned int h_time_ms)
{
  kvp_t *kvps = NULL;
  uint64 h_time_tick = 0;
  DalDeviceHandle *h_time_dal = NULL;

  TMS_MSG_HIGH("Hibernation: Sending command to RPM.");

  if(DAL_SUCCESS != DalTimetick_Attach("SystemTimer", &h_time_dal))
  {
    ERR_FATAL("DalTimetick_Attach to SystemTimer failed.",0,0,0);
    return ;
  }
  if(DAL_SUCCESS != DalTimetick_CvtToTimetick64(h_time_dal, h_time_ms, T_MSEC, &h_time_tick))
  {
    ERR_FATAL("DalTimetick_CvtToTimetick64 failed.",0,0,0);
    return ;
  }
  
  kvps = kvp_create(2 * sizeof(unsigned) + sizeof(h_time_tick));  //type+length+value
  kvp_clear(kvps);
  kvp_put(kvps, HIBERNATION_TIME_KEY, sizeof(h_time_tick), (void *)&h_time_tick);

  rpm_post_request(RPM_ACTIVE_SET, (rpm_resource_type)HIBERNATION_RESOURCE, 0, kvps);

  // kvp_destroy(kvps); Not required as system will die
}
...
static void sys_m_shutdown_terminate(timer_cb_data_type delay)
{
  if (delay)
  {
	...
	timer_def_osal(&shutdown_delay_timer,
		           &sd_delay_timer_group,
		           TIMER_FUNC1_CB_TYPE,
		           sys_m_shutdown_terminate,
		           0);
...
#if HIBERNATION_AS_SHUTDOWN
      if(sys_m_hibernation_is_shutdown == 1)
      {
        sys_m_hibernation_send_to_rpm(sys_m_hibernation_time_ms);
        // Loop forever while RPM kills us
        while(1);
      }
#endif
...
FUNCTION sys_m_notify_modem_before_shutdown
...
{
	sys_m_modem_shutdown_type = SYS_M_SHUTDOWN_NOTIFY_ONLY;
	ssctl_v02_send_req(MPSS_SSCTL_SVC, SSCTL_SHUTDOWN, NULL);
}
