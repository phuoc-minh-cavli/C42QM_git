...
Copyright (c) 2015-17 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/core.tx/6.0/debugtools/sys_m/src/lib/9205/apps/sys_m_smp2p.c#1 $
...
/* Variables used to notify clients for smp2p state change */
uint32 sys_m_modem_smp2p_bits_wait = 0;
sys_m_modem_smp2p_bit_notify_cb_type sys_m_modem_smp2p_bit_notify_cb = NULL;

extern shutdown_type sys_m_modem_shutdown_type;
...
uint32 sys_m_smp2p_get_modem_shutdwn_ack(void)
{
   ...
   /* read the data here */
   return_val = smp2p_read_ex(sys_m_smp2p_rx_context[SMEM_MODEM], &rx_state_received,SMP2P_FLAGS_STM);
...
int32 sys_m_smp2p_set(uint32 bits, smem_host_type host)
{
   if ((host <SMEM_NUM_HOSTS) && (sys_m_smp2p_tx_context[host] == 0))
	...
	/* Use STM safe API for writing to SMP2P */
	return smp2p_modify_ex(sys_m_smp2p_tx_context[host], 0xffffffff, bits, SMP2P_FLAGS_STM);
}
 ...
 *
*/
void sys_m_smp2p_task(unsigned long arg)

{
  ...
  while (1)
  {
	...
	if (host_idx < SMEM_NUM_HOSTS)
		...
		ERR_FATAL("host_idx is invalid  = %d",host_idx,0,0);
		return;
		}
	...
	if (rx_state_change & sys_m_modem_smp2p_bits_wait)
	{
		if((remote_id == SMEM_MODEM)){
          sys_m_modem_smp2p_bit_notify_cb(rx_state_change & sys_m_modem_smp2p_bits_wait);
        }
	}
       ...
       &&(remote_id ==  SMEM_MODEM))
    {
      ...
      MSG_1(MSG_SSID_TMS, MSG_LEGACY_HIGH,PROC_ID"SYS_M_SUBSYS2AP_QMI_REQ_ACK received. sys_m_shutdown=%d",sys_m_shutdown);
	  if(sys_m_modem_shutdown_type == SYS_M_SHUTDOWN_DEFAULT)
	  {
         sys_m_complete_reset();
	  }
	  else
	  {
	     /* Need to Reset sys_m_modem_shutdown_type to default as modem shutdown 
		    by pil is complete. So that after this if crash happens , we need to 
			reset the complete system */
		 sys_m_modem_shutdown_type = SYS_M_SHUTDOWN_DEFAULT;
	  }
    }
  }
}
...
/**
 *
 * @brief sys_m_modem_smp2p_notify_
 *
...
{
	boolean ret = FALSE;
	if( (fp != NULL) && (bits != 0) && (sys_m_modem_smp2p_bit_notify_cb == NULL))
	{
	   sys_m_modem_smp2p_bits_wait = bits;
	   sys_m_modem_smp2p_bit_notify_cb = fp;
	   ret = TRUE;
	}
	return ret;
}
...
{
   boolean ret = FALSE;
   if((fp != NULL) && (fp == sys_m_modem_smp2p_bit_notify_cb))
   {
      sys_m_modem_smp2p_bits_wait = 0;
	  sys_m_modem_smp2p_bit_notify_cb = NULL;
	  ret = TRUE;
   }
   return ret;
}
