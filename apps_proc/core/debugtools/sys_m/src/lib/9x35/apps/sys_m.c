...
$Header: //components/rel/core.tx/6.0/debugtools/sys_m/src/lib/9x35/apps/sys_m.c#1 $
...
int sys_m_ssr_msg_mpss = 0;
int sys_m_ssr_msg_lpass = 0;
...
#define MDM2AP_STATUS_GPIO_NUM              80
#define MDM2AP_ERR_FATAL_GPIO_NUM           15
#define AP2MDM_STATUS_GPIO_NUM              85
#define AP2MDM_ERR_FATAL_GPIO_NUM           14

#define MDM2AP_STATUS_GPIO_FUS7_NUM          21
#define MDM2AP_ERR_FATAL_GPIO_FUS7_NUM       23
#define AP2MDM_STATUS_GPIO_FUS7_NUM          63
#define AP2MDM_ERR_FATAL_GPIO_FUS7_NUM       25

#define AP2MDM_STATUS_GPIO_LIQUID_NUM       18

#define SYS_M_MDM2AP_STATUS_OUT_GPIO    DAL_GPIO_CFG(sys_m_gpio_map.MDM2AP_STATUS_GPIO, 0, DAL_GPIO_OUTPUT, DAL_GPIO_NO_PULL, DAL_GPIO_2MA)
#define SYS_M_MDM2AP_STATUS_OUT_GPIO_INACTIVE   DAL_GPIO_CFG_OUT(sys_m_gpio_map.MDM2AP_STATUS_GPIO, 0, DAL_GPIO_OUTPUT, DAL_GPIO_NO_PULL, DAL_GPIO_2MA, HAL_TLMM_LOW_VALUE)
...
static smdl_handle_type sys_m_port_MODEM_AP = NULL;
static smdl_handle_type sys_m_port_LPASS_AP = NULL;
 ...
 * 
*/
__packed void * sys_m_control (__packed void *req_pkt, uint16 pkt_len)
{
    sys_m_mode_control_req_type *req = NULL;
    sys_m_mode_control_req_type *rsp = NULL;
    const int rsp_len = sizeof (sys_m_mode_control_req_type);
    
    req = (sys_m_mode_control_req_type *) req_pkt;
    if (req->mode == MODE_RESET) //reset
        sys_m_reset();
    else if (req->mode == MODE_SHUTDOWN) //shutdown
        sys_m_shutdown();
    else
        return NULL;
    
    rsp = (sys_m_mode_control_req_type *)
 ...
 * 
*/
__packed void * sys_m_control_stub (__packed void *req_pkt, uint16 pkt_len)
{
    sys_m_mode_control_req_type *req = NULL;
    sys_m_mode_control_req_type *rsp = NULL;
    const int rsp_len = sizeof (sys_m_mode_control_req_type);
...
const diagpkt_user_table_entry_type sys_m_tbl[] = {
    {DIAG_CONTROL_F,    DIAG_CONTROL_F,     sys_m_control}
};

const diagpkt_user_table_entry_type sys_m_tbl_stub[] = {
    {DIAG_CONTROL_F,    DIAG_CONTROL_F,     sys_m_control_stub}
};
...
void sys_m_sio_rx_notify (dsm_item_type ** item_ptr)
{
    ...
    {
        memcpy(sys_m_buffer, (*item_ptr)->data_ptr, bytes_sent);
 ...
 * 
*/
void q6sw_smd_cb(smdl_handle_type port, smdl_event_type  event, void *data)
{
    if (event == SMDL_EVENT_READ)
    {
        smdl_read(port, SYS_M_BUFFER_LEN, sys_m_buffer, 0);
        //awaiting a SSR ack
        if (sys_m_ssr_msg_mpss)
        {
            if (strncmp(sys_m_buffer, SYS_M_ACK, SYS_M_BUFFER_LEN) == 0)
                rex_set_sigs(TASK_HANDLE(sys_m), SYS_M_ACK_SIG);
            else
                rex_set_sigs(TASK_HANDLE(sys_m), SYS_M_ACK_SIG); //ignore failure for now
        }
        //awaiting a shutdown ack
        else
        {
            if (strncmp(sys_m_buffer, SYS_M_SHUTDOWN_ACK, SYS_M_BUFFER_LEN) == 0)
                rex_set_sigs(TASK_HANDLE(sys_m), SYS_M_COMPLETE_RESET);
        }
    }
    return;
}
 ...
 * 
*/
void adsp_smd_cb(smdl_handle_type port, smdl_event_type  event, void *data)
{
    if (event == SMDL_EVENT_READ)
    {
        smdl_read(port, SYS_M_BUFFER_LEN, sys_m_buffer, 0);
        if (sys_m_ssr_msg_lpass)
        {
            if (strncmp(sys_m_buffer, SYS_M_ACK, SYS_M_BUFFER_LEN) == 0)
                rex_set_sigs(TASK_HANDLE(sys_m), SYS_M_ACK_SIG);
            else
...
{
    sys_m_true_reset = 1;
    rex_set_sigs(TASK_HANDLE(sys_m), SYS_M_INITIATE_RESET);
}
...
/**
 *
 * @brief sys_m_modem_shutdown_cb
 *
 * Callback for a reset request from modem
 * 
...
*/
void sys_m_modem_shutdown_cb(smsm_entry_type entry, uint32 prev_state, uint32 curr_state, void * cb_data)
{
...
/**
 *
 * @brief sys_m_modem_poweroff_cb
 *
 * Callback for a poweroff request from modem
 * 
 * @param[in] entry: SMSM type
 * @param[in] prev_state: SMSM's previous state
 * @param[in] curr_state: SMSM's current state
 * @param[in] *cb_data: callback data
*/
void sys_m_modem_poweroff_cb(smsm_entry_type entry, uint32 prev_state, uint32 curr_state, void * cb_data)
{
...
static void sys_m_pmapp_pon_cblpwr_cb(void)
{
    ...
    else
    {
      ...
      MSG_HIGH("pmapp_pon_cblpwr interrupt driving shutdown",0,0,0);
      sys_m_shutdown();
    }
}
    ...
    if (dRes != DAL_SUCCESS)
{
    ...
    if ( DAL_SUCCESS != GPIOInt_RegisterIsr(hGPIOInt, sys_m_gpio_map.AP2MDM_ERR_FATAL_GPIO, GPIOINT_TRIGGER_RISING, (GPIOINTISR) sys_m_AP_kernel_panic_isr, NULL)
      || DAL_SUCCESS != GPIOInt_RegisterIsr(hGPIOInt, sys_m_gpio_map.AP2MDM_STATUS_GPIO, GPIOINT_TRIGGER_FALLING, (GPIOINTISR) sys_m_wdog_reset_isr, NULL) )
    {
...
/**
 *
 * @brief sys_m_lpass_errfatal_cb
 *
 * Callback for LPASS error fatal
 * 
 * @param[in] entry: SMSM type
 * @param[in] prev_state: SMSM's previous state
 * @param[in] curr_state: SMSM's current state
 * @param[in] *cb_data: callback data
*/
void sys_m_lpass_errfatal_cb(smsm_entry_type entry, uint32 prev_state, uint32 curr_state, void * cb_data)
{
    char *rcv_buff;
    memset(sys_m_sfr_buffer, 0, SYS_M_SFR_LENGTH);
    strlcpy(sys_m_sfr_buffer, "ssr:return:MDM-LPASS:", SYS_M_SFR_LENGTH);
    ...
    sys_m_sfr_written = TRUE;
    ERR_FATAL("LPASS error fatal", 0, 0, 0);
}
 ...
 * @param[in] *cb_data: callback data
*/
void sys_m_mpss_errfatal_cb(smsm_entry_type entry, uint32 prev_state, uint32 curr_state, void * cb_data)
{
    char *rcv_buff;
    memset(sys_m_sfr_buffer, 0, SYS_M_SFR_LENGTH);
...
void sys_m_set_gpio(void)
{
    ...
    {
        //set ERR_FATAL high
        dRes = DalTlmm_GpioOut(hTlmm, (DALGpioSignalType) SYS_M_MDM2AP_ERR_FATAL_OUT_GPIO, DAL_GPIO_HIGH_VALUE);
        if (dRes != DAL_SUCCESS)
        {
            ERR_FATAL("System monitor could not set GPIO high", 0, 0, 0);
        }
 ...
 * @brief sys_m_halt
 *
 * System monitor halt function for fusion configs
 * 
*/
void sys_m_halt(void)
{    
...
void sys_m_init(void)
{
    ...
    sys_m_platform_info.subtype = DALPLATFORMINFO_SUBTYPE_UNKNOWN;
    ...
    sys_m_port_MODEM_AP = smdl_open(sys_m_smd_port, SMD_APPS_MODEM, 0, SMD_MIN_FIFO, q6sw_smd_cb, NULL);
    sys_m_port_LPASS_AP = smdl_open(sys_m_smd_port, SMD_APPS_QDSP, 0, SMD_MIN_FIFO, adsp_smd_cb, NULL);    
    ...
    DalInterruptController_RegisterISR(hInthandle, LPASS_DOG_ISR, (DALIRQ)lpass_wdog_bite_isr, 0, DALINTRCTRL_ENABLE_RISING_EDGE_TRIGGER);
    ...
    smsm_cb_register(SMSM_MODEM_STATE, SMSM_SYSTEM_REBOOT, sys_m_modem_shutdown_cb, NULL);
    smsm_cb_register(SMSM_MODEM_STATE, SMSM_SYSTEM_DOWNLOAD, sys_m_modem_poweroff_cb, NULL);
    
    //register for LPASS/Q6SW error fatals
    smsm_cb_register(SMSM_Q6_STATE, SMSM_RESET, sys_m_lpass_errfatal_cb, NULL);
    smsm_cb_register(SMSM_MODEM_STATE, SMSM_RESET, sys_m_mpss_errfatal_cb, NULL);
...
void sys_m_task(unsigned long param)
{
    ...
    for (;;)
    {
        ...
        if (sigs & SYS_M_INITIATE_RESET)
        {
            ...
            {
                sys_m_reset_in_progress = 1;
                smdl_write(sys_m_port_MODEM_AP, SYS_M_BUFFER_LEN, SYS_M_SHUTDOWN, SMDL_WRITE_FLAGS_NONE);
                rex_set_timer(&sys_m_reset_timer, sys_m_apps_timeout);
            }
        }
        //physically reset/shutdown chip
        else if (sigs & SYS_M_COMPLETE_RESET)
        {
            rex_clr_timer(&sys_m_reset_timer);
...
void sys_m_signal_modules(void)
{
    ...
    int req_count = 0;
    rex_timer_cnt_type time_remaining = 0;
    
    
    //NOTIFY MPSS
    rex_def_timer(&sys_m_ssr_timer, rex_self(), SYS_M_ACK_SIG);
    //prepare for mpss msg
    sys_m_ssr_msg_mpss = 1;
    smdl_write(sys_m_port_MODEM_AP, SYS_M_BUFFER_LEN, accepted_messages[sys_m_notification_id], SMDL_WRITE_FLAGS_NONE);
    rex_set_timer(&sys_m_ssr_timer, 150);
    rex_wait(SYS_M_ACK_SIG);
    time_remaining = rex_clr_timer(&sys_m_ssr_timer);
    (void) rex_clr_sigs(rex_self(), SYS_M_ACK_SIG);
    
    //timer expired
    if (time_remaining == 0)
    {
        ERR_FATAL("sys_m: SSR notification ACK fail", 0, 0, 0);
    }
    
    sys_m_ssr_msg_mpss = 0;
    
    //NOTIFY LPASS
    rex_def_timer(&sys_m_ssr_timer, rex_self(), SYS_M_ACK_SIG);
    //prepare for mpss msg
    sys_m_ssr_msg_lpass = 1;
    smdl_write(sys_m_port_LPASS_AP, SYS_M_BUFFER_LEN, accepted_messages[sys_m_notification_id], SMDL_WRITE_FLAGS_NONE);
    rex_set_timer(&sys_m_ssr_timer, 150);
    rex_wait(SYS_M_ACK_SIG);
    time_remaining = rex_clr_timer(&sys_m_ssr_timer);
    (void) rex_clr_sigs(rex_self(), SYS_M_ACK_SIG);
...
{
    smsm_state_set(SMSM_APPS_STATE, SMSM_RESET);
}
...
void sys_m_wait_peripherals(void)
{
    ...
    {
        if ( (smsm_state_get(SMSM_MODEM_STATE) & SMSM_RESET) &&
            (smsm_state_get(SMSM_Q6_STATE) & SMSM_RESET))
            return;
