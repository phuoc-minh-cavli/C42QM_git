...
/*=============================================================================
Edit History
$Header: //components/rel/core.tx/6.0/debugtools/rcevt/src/rcevt.c#1 $
...
/* Dummy signal to be used for QURT signal object */
#define RCEVT_WAIT_SIG_MASK  0x1
...
RCEVT_HANDLE rcevt_create_name(RCEVT_NAME const name)
{
   ...
   rce_mutex_lock_qurt(&rce_internal.mutex_create);
   ...
   rce_mutex_unlock_qurt(&rce_internal.mutex_create);
...
int rcevt_getregistrants_handle(RCEVT_HANDLE const handle)
{
   ...
   if (RCE_NULL != nde_p)
   {
	  ...
	  rce_mutex_lock_qurt(&nde_p->mutex);
	  ...
	  rce_mutex_unlock_qurt(&nde_p->mutex);
   }
      ...
      while (RCE_NULL != nfy_p)                                                  // iterate
      {
         ...
         switch (nfy_p_type)
         {
            ...
            case RCEVT_SIGEX_TYPE_SIGQURT:
            {
               ...
               if (nde_p->threshold >= nfy_p->threshold)
               {
                  qurt_signal_set(nfy_p_sigex_p->signal, nfy_p_sigex_p->mask);
                  rc = handle;
               }
            }
               break;
...
static RCEVT_HANDLE internal_wait_common(RCEVT_HANDLE const handle, RCEVT_THRESHOLD const threshold, RCEVT_SIGEX_TYPE const type, RCEVT_SIGEX const sigex)
{
   ...
   if (rc)
   {
      ...
      switch (nfy_p_type)
      {
         ...
         {
            RCEVT_SIGEX_SIGQURT* nfy_p_sigex_p = (RCEVT_SIGEX_SIGQURT*)&nfy_p->sigex;
            qurt_signal_wait(nfy_p_sigex_p->signal, nfy_p_sigex_p->mask, QURT_SIGNAL_ATTR_WAIT_ANY);
            qurt_signal_clear(nfy_p_sigex_p->signal, nfy_p_sigex_p->mask);
         }
            break;
#endif
...
RCEVT_HANDLE rcevt_wait_count_handle(RCEVT_HANDLE const handle, RCEVT_THRESHOLD const threshold)
{
   ...
   // DAL Event Objects Are Protected on Context Allowed to Wait (Create/Wait/Destroy)
   sig.signal = malloc(sizeof(qurt_signal_t));
   if(sig.signal)
   	{
   		qurt_signal_init(sig.signal);
		sig.mask = RCEVT_WAIT_SIG_MASK;

		// Waiting causes an implicit registration
		rc = internal_wait_common(handle, threshold, RCEVT_SIGEX_TYPE_SIGQURT, &sig);

		// Unregister the implicit addition from waiting
		if (internal_unregister_sigex(handle, RCEVT_SIGEX_TYPE_SIGQURT, &sig)) { ; }

		// De-register the qurt signal object & free the memory
		qurt_signal_destroy(sig.signal);
		free(sig.signal);
   	}
#endif
