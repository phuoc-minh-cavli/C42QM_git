...
/*=============================================================================
Edit History
$Header: //components/rel/core.tx/6.0/debugtools/rcevt/src/rcevt_common.c#1 $
...
rce_nfy_p rce_nfy_pool_alloc(void)
{
   ...
   rce_mutex_lock_qurt(&rce_internal.mutex);                                      // lock mutex
   ...
   rce_mutex_unlock_qurt(&rce_internal.mutex);                                    // unlock mutex
...
rce_nde_p rce_nde_pool_init(void)
{
   ...
   if (RCE_NULL != next_pool)                                                    // populate
   {
      ...
      for (i = 0; i < RCEVT_NODE_POOL_SIZ; i++)                                  // initialize pool internal
      {
         ...
         rce_mutex_init_qurt(&(next_pool->nde_pool[i].mutex));                    // initialize member
...
void rce_init(void)
{
   ...
   {
      rce_mutex_init_qurt(&rce_internal.mutex);                                   // initialize mutex

      rce_mutex_init_qurt(&rce_internal.mutex_create);
