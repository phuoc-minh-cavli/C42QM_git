...
def PrintConfig(ModName):
   ...
   #structure definition expressed in xml
...
#driver_entry driver_list[] = {{ /dev/buses/i2c1, 1234, {2,5,8 }, }
   ...
   CONFIG_DATA = open(DirName + "/" + 'devcfg_data.c', 'w')
   CONFIG_DATA.write ("#include \"DALSysTypes.h\" \n")
   CONFIG_DATA.write ("\nextern const void * DALPROP_StructPtrs[];\n")
   CONFIG_DATA.write ("\nextern const uint32 DALPROP_PropBin[];\n")
   
   if len(ConfigStructList)!=0:
      CONFIG_DATA.write ("\nextern StringDevice driver_list[];\n")
...
def PrintModDir(ModName, DriverList):
   # Adjust the String device offsets and print them 
   ConfigHeaderLen = 24 + 4*len(DeviceID)
   global UintSection, DevSection
...
ModSrcfilename = "DALModDir_" + ModName +".c"
if sys.argv[3] != ModSrcfilename:
   PrintConfig(ModName)
#Print_Debug()
