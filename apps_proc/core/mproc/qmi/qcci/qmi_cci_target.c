...
$Header: //components/rel/core.tx/6.0/mproc/qmi/qcci/qmi_cci_target.c#1 $
...
)
{
  ...
  if(!ptr->ext_signal)
  {
    qurt_signal_destroy(&ptr->signal);
  }
}
...
)
{
  ...
  if(ptr->ext_signal)
  {
    qurt_signal_clear(ptr->ext_signal, sig_mask);
  }
  else
  {
    qurt_signal_clear(&ptr->signal, sig_mask);
  }
}
...
)
{
  ...
  if(ptr->ext_signal)
  {
    qurt_signal_set(ptr->ext_signal, ptr->sig);
  }
  else
  {
    qurt_signal_set(&ptr->signal, ptr->sig);
  }
}
...
)
{
  ...
  if(timeout_ms && ptr->timer_inited)
  {
    if(ptr->ext_signal)
    {
      ...
      qurt_signal_wait(ptr->ext_signal, ptr->sig | ptr->timer_sig, QURT_SIGNAL_ATTR_WAIT_ANY);
    ...
    else
    {
      ...
      qurt_signal_wait(&ptr->signal, ptr->sig | ptr->timer_sig, QURT_SIGNAL_ATTR_WAIT_ANY);
  ...
  {
    qurt_signal_wait(ptr->ext_signal ? ptr->ext_signal : &ptr->signal, 
                        ptr->sig | ptr->timer_sig,
