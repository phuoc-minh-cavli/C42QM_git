...
$Header: //components/rel/core.tx/6.0/mproc/ipc_router/ipc_router_xal_glink_qurt.c#1 $
...
#ifndef IPC_ROUTER_OS_QURT_THREADX
...
static void ipc_router_xal_glink_worker_thread(void *priv)
{
  ...
  while(1)
  {
    ...
    qurt_signal_wait(&port->os_priv->signal, wait_sigs, QURT_SIGNAL_ATTR_WAIT_ANY);
    got_sigs = qurt_signal_get(&port->os_priv->signal);
    qurt_signal_clear(&port->os_priv->signal, got_sigs);
  }
...
)
{
  ...
  if(port->params.stack_size > 0)
  {
    ...
    qurt_signal_init(&os_priv->signal);
...
#endif
    qurt_signal_destroy(&port->os_priv->signal);
  }
  else
  {
...
)
{
  ...
  if(port->params.stack_size > 0)
  {
    qurt_signal_set(&port->os_priv->signal, EVENT_MASK(event));
  }
  else
  {
