...
#define IPCR_OS_SIGNAL_CLEAR(ptr) \
    qurt_signal_clear(&((ptr)->signal), IPCR_EVENT_SIG)

/* Sets an event on a signal */
#define IPCR_OS_SIGNAL_SET(ptr) \
  qurt_signal_set(&((ptr)->signal), IPCR_EVENT_SIG)
...
#define IPCR_OS_SIGNAL_WAIT(ptr, timeout_ms) \
  do { \
    if(IPCR_OS_SIGNAL_TIMED_OUT(ptr)) \
      break;  \
    qurt_signal_wait(&((ptr)->signal), IPCR_EVENT_SIG | IPCR_TIMER_SIG, QURT_SIGNAL_ATTR_WAIT_ANY); \
  } while(0)
