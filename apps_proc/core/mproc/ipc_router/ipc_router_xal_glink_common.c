...
$Header: //components/rel/core.tx/6.0/mproc/ipc_router/ipc_router_xal_glink_common.c#1 $
...
)
{
  ...
  if(ipc_enqueue_rx_intent(port, req_size, FALSE) != IPC_ROUTER_STATUS_SUCCESS)
  {
    return FALSE;
  }
  return TRUE;
...
)
{
  ...
  glink_err_type          err;
  static glink_err_type   ipcrtr_glink_err;
  ...
  do{
      ...
      if(err != GLINK_STATUS_SUCCESS)
      {
		...
		ipcrtr_glink_err = err;
		
		// To resolve warning
		ipcrtr_glink_err = ipcrtr_glink_err;
        break;
      }
