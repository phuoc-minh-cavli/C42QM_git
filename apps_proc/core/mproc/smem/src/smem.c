...
Copyright (c) 2004 - 2015 by Qualcomm Technologies, Incorporated.  All 
Rights Reserved.
...
$Header: //components/rel/core.tx/6.0/mproc/smem/src/smem.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/11/15   rv      Removed spin unlock call from smem_alloc_dynamic_locked()
06/26/15   an      set explicit function invocation order in smem_set_base_addr
05/27/15   db      Removed ERROR FATAL from smem_alloc_dynamic() function
04/08/15   db      Added smem_hw_mutex_base_ptr physical address in 
                   smem_set_spinlock_base_addr
04/07/15   db      Enabled smem_partition_init() in smem_init() for TZ
02/19/14   an      smem refactored to prevent deadlock in qnx
12/16/13   bt      Do not cast smem_ext_heap_info.base_virt, keep as char *.
...
/*===========================================================================
FUNCTION      smem_alloc_dynamic_locked
...
DEPENDENCIES  smem_set_ext_mem_region() must have been caled before trying to
              ...
              This function must be called from SMEM locked context.

RETURN VALUE  NULL if an error occurred.  Otherwise, pointer to the 
              requested buffer.
===========================================================================*/
static void *smem_alloc_dynamic_locked
(
  smem_mem_type         smem_type,
  uint32                buf_size,
  uint32                mem_bank
)
{
      ...
      SMEM_ALLOCATION_ENTRY_ALLOCATED )
  {
    ...
    if( buf_size == smem_init_info.smem_allocation_table[smem_type].size )
    {
      ...
      {
        result = (void*) ( smem_ext_heap_info.base_virt + 
    ...
    else
    {
      ...
      MSG_ERROR( "Invalid SMEM buffer size requested id=%d, requested size=%d, "
                 "actual size=%d.", smem_type, buf_size, 
                 smem_init_info.smem_allocation_table[smem_type].size );
      return result;
    }
  }
  else
  {
    ...
    else
    {
      ...
      MSG_ERROR( "Out of shared memory have %d requested %d for %d",
...
)
{
  ...
  result = smem_alloc_dynamic_locked(smem_type, buf_size, mem_bank);
...
void smem_set_base_addr( void )
{
  if (smem_init_info.smem_data_base_addr == SMEM_INVALID_ADDR) 
  {
    void *phys_base_addr = smem_get_phys_base_addr();
    uint32 smem_size = smem_get_size();

    smem_init_info.smem_data_base_addr = smem_map_base_va(
                                  phys_base_addr,
                                  smem_size);
  }
}/* smem_set_base_addr */
...
@dependencies  This function must be called from SMEM locked context.
...
)
{
  ...
  return smem_alloc_dynamic_locked(smem_type, buf_size, 0);
} /* smem_alloc_locked */
...
)
{
  ...
  result = smem_get_addr_locked(smem_type, buf_size);
...
uint32 smem_get_num_hosts( void )
{
  /* hard code the return value until bootloaders place it 
     ...
     (+1 in case it would need to be expanded in the near future) */
  return SMEM_NUM_HOSTS + 1;  
}
