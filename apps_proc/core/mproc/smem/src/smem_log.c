...
$Header: //components/rel/core.tx/6.0/mproc/smem/src/smem_log.c#1 $
                   ...
                   uint32 * events_remaining )
{
  ...
  memcpy(events, (void *)(smem_log_table[area].smem_log_events + read_idx),
      read_count1 * sizeof(smem_log_struct_type));
  if(read_count2 > 0)
  {
    /* If the read wraps around the beginning of the buffer */
    memcpy(events + read_count1, (void *)smem_log_table[area].smem_log_events,
  ...
  if(abs_write_idx > abs_read_idx + smem_log_table[area].smem_num_entries)
  {
    ...
    memmove(events, events + invalidated,
