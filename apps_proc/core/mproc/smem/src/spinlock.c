...
$Header: //components/rel/core.tx/6.0/mproc/smem/src/spinlock.c#1 $
...
/** Pointer to the spinlock array in SMEM */
volatile uint32 *spinlock_ptr = NULL;
...
void (*spin_lock_internal_fn_ptr)(uint32);
void (*spin_unlock_internal_fn_ptr)(uint32);
uint32 (*spin_lock_get_fn_ptr)(uint32);
...
void smem_spin_lock( uint32 lock ) 
{
  ...
  /* Acquire spinlock */
  spin_lock_internal_fn_ptr( lock );
}
...
void smem_spin_unlock( uint32 lock )
{
  ...
  spinlock_value = spin_lock_get_fn_ptr( lock );
  ...
  /* Release spinlock */
  spin_unlock_internal_fn_ptr( lock );
...
void smem_spin_locks_clear( smem_host_type host )
{
  ...
  for( i = 0; i < smem_num_spinlocks; i++ )
  {
    ...
    if( spinlock_ids[ host ] == spin_lock_get_fn_ptr( i ) )
    {
