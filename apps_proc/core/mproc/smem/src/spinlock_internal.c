...
$Header: //components/rel/core.tx/6.0/mproc/smem/src/spinlock_internal.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/12/16   rv      Same spinlock PID is used for APPS and TZ. TZ will not 
                   wait for hardware mutex when APPS is already acquired it.
                   Similarly for APPS, APPS will not wait for hardware 
                   mutex when TZ is already acquired it.
...
#if defined(SMEM_TZ_PROC) || defined(SMEM_APPS_PROC)
static boolean spinlock_locked[SPINLOCK_NUM_LOCKS] = {0};
#endif
...
void spin_lock_internal( uint32 lock )
{
  ...
  /* APPS and TZ spinlock PIDs are same, hence TZ will not wait for hardware 
   * mutex when APPS is already acquired it. Similarly for APPS, APPS will 
   * not wait for hardware mutex when TZ is already acquired it.
   * This is only applicable to single core APPS processor. */
...
void spin_unlock_internal( uint32 lock )
{
  ...
  if (spinlock_locked[lock] == TRUE)
  {
    spinlock_locked[lock] = FALSE;
  }
  else
  {
    return;
