...
$Header: //components/rel/core.tx/6.0/mproc/smd/src/smd_memcpy.c#1 $
...
01/22/13   bt      Remove unused variables to resolve compiler warnings.
...
#undef SMD_MEMCPY_FILTER_EVENTS_IDX       
#undef SMD_MEMCPY_FILTER_EVENTS_RMT_HOST  
#undef SMD_MEMCPY_LOG_EVENTS

#ifdef SMD_MEMCPY_FILTER_EVENTS_IDX
#define SMD_MEMCPY_LOG_EVENT(pid,id,d1,d2,d3)   \
  do { \
    if( PORT_GET_IDX(pid) == SMD_MEMCPY_FILTER_EVENTS_IDX && \
        PORT_GET_REMOTE(pid) == SMD_MEMCPY_FILTER_EVENTS_RMT_HOST ) \
    { \
...
)
{
  ...
  ASSERT(info != NULL && info->copy_from_fifo_fn != NULL);
  ...
  if( read_index + count > fifo_sz ) {
    ...
    info->copy_from_fifo_fn( dst, src + read_index, bytes );
    count -= bytes;
    read_index = 0;
  }
  else {
  ...
  info->copy_from_fifo_fn( dst + bytes, src + read_index, count );
...
)
{
  ...
  info->copy_to_fifo_fn( dst + write_index, src + bytes, count );
...
static void smd_memcpy_process_read( smd_stream_info_type *si )
{
  ...
  SMD_MEMCPY_LOG_EVENT( si->port_id, SMEM_LOG_EVENT_READ, 0, si->port_id, 0 );
  ...
  if( write_index == read_index ) {
    /* source buffer is empty - nothing to do */
    SMD_MEMCPY_LOG_EVENT( si->port_id, SMEM_LOG_EVENT_READ, 1, si->port_id, 0 );
    return;
  }
      ...
      if( mi->rx_buf_ptr == NULL )
      {
        ...
        if( si->rx_fctl_enabled )
        {
          ...
          SMD_MEMCPY_LOG_EVENT
            ( si->port_id, SMEM_LOG_EVENT_READ, 2, si->port_id, 0 );
  ...
  SMD_MEMCPY_LOG_EVENT( si->port_id,
                        SMEM_LOG_EVENT_READ, 4, si->port_id, 0 );
...
static void smd_memcpy_process_write( smd_stream_info_type *si )
{
  ...
  SMD_MEMCPY_LOG_EVENT( si->port_id, SMEM_LOG_EVENT_WRITE, 0, si->port_id, 0 );
  ...
  */
  ...
  if( mi->tx_buf_ptr == NULL ) {
    ...
    SMD_MEMCPY_LOG_EVENT( si->port_id, SMEM_LOG_EVENT_WRITE, 1, si->port_id, 0 );
    return;
  }
  ...
  if( byte_count >= si->max_queued_data )
  {
    ...
    SMD_MEMCPY_LOG_EVENT( si->port_id, SMEM_LOG_EVENT_WRITE, 3, si->port_id, 0 );
  ...
  do {
    ...
    if( si->tx_fctl_enabled )
    {
      ...
      if( 0 == si->prev_rts ) 
      {
        ...
        SMD_MEMCPY_LOG_EVENT( si->port_id,
                        SMEM_LOG_EVENT_WRITE, 2, si->port_id, 0 );
        break;
      }
    }
  ...
  SMD_MEMCPY_LOG_EVENT( si->port_id,
                        SMEM_LOG_EVENT_WRITE, 4, si->port_id, 0 );
...
SIDE EFFECTS  None
===========================================================================*/
smd_api_return_type smd_memcpy_open
(
