...
$Header: //components/rel/core.tx/6.0/mproc/smd/src/smdl_echo.c#1 $
...
)
{
  ...
  if(smd_malloc(sizeof(smdl_echo_type), (void **)&handle) != DAL_SUCCESS)
  {
    return NULL;
  }
  ...
  if(smd_malloc(handle->buffer_len, (void **)&handle->buffer) != DAL_SUCCESS)
  {
    smd_free(handle);
    return NULL;
  }
