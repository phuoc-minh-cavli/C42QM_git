...
sure that this library is loaded you would need to modify our current
platofrom_libs_list in 'proj/platfrom_libs_list/platform_libs_list.c',
the current list contains the following:

PL_DEP(fastrpc_smd)
PL_DEP(dlw)
PL_DEP(svfs)
PL_DEP(rpc_qmi_server)
PL_DEP(hashverify)
...
struct platform_lib* (*pl_list[])(void) = {
 ...
 PL_ENTRY(dlw),
 PL_ENTRY(svfs),
 PL_ENTRY(rpc_qmi_server),
 PL_ENTRY(hashverify),
 0
};

to add a library simple add a dependency and an entry in the list.
