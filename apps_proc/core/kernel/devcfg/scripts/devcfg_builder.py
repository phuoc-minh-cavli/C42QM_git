...
def check_and_ignore_xml_file_duplicates(env,rootenv,xml_file_list,each_xml_tag,target_image):
    exisiting_pp_xml_node_list = rootenv.get('DEVCFG_INFO_COLLECTOR',{}).get(target_image,{}).get('pp_'+each_xml_tag,[])
        ...
        for each_xml_tag in tag_list:     
            ...
            if each_xml_tag in thread_input_dict:            
                ...
                for file in xml_file_list:
                   ...
                   rootenv['DEVCFG_INFO_COLLECTOR'].setdefault(target_image,{}).setdefault(pp_xml_tag,[]).append(pp_xml_cl[0])
