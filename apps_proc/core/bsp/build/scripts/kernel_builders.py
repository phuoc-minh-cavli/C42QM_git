...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tx/6.0/bsp/build/scripts/kernel_builders.py#1 $
...
def pbn_complex_builder(env, target, source):
   ...
   if env['MSM_ID'] in ['6695','6615','8660','8960', '9x15', '9x25', '9x35','9x45','9x55','9x06']:
   ...
   if env['MSM_ID'] in ['8660','8960', '9x15', '9x25', '9x35','9x45','9x55','9x06']:
...
def quartz_constant_builder(target, source, env):
   ...
   if env.has_key('USES_L4_3X'):
      if env['MSM_ID'] in ['8064', '9x15', '9x25', '9x35','9x45','9x55','9x06']:
         virt_l4_re = re.compile(r'\"physical_kernel\"\s*>\s*<\s*region\s+base\s*=\"(\S+)\"\s*', re.M)
         virt_l4 = virt_l4_re.findall(xml_data)
         if len(virt_l4) == 0:
            env.PrintError("Cannot find kernel entry point in ["+ source_xml +"].")
            raise RuntimeError, 'Cannot find kernel entry point in XML data.'
         virt_l4_entry_point = virt_l4[0]
      elif env['MSM_ID'] in ['6695','6615']:
   ...
   if env.has_key('USES_L4_3X'):
      source_reloc_base, source_pbn_dummy_file = os.path.split(str(env['PBN_DATA_FILE']))
      virt_l4_entry_point = 0xFF000000
      kernel_file = source_reloc_base + "/kernel.reloc"
   #create copy of kernel.reloc in \build\ms  
      cpy_kernel_file = "kernel.reloc"
      shutil.copy2(kernel_file,cpy_kernel_file)
   else:
   ...
   data = data.replace('L4_ENTRY_POINT', hex(phys_l4_entry_point).rstrip("L"))
   data = data.replace('MMU_DATA_OFFSET', hex(mmu_data_offset).rstrip("L"))
...
#===============================================================================
# Customer Configuration build rules
#===============================================================================
# target = cust config object = target[0]
# source = cust config XML file = source[0]
#
...
def cust_config_generate(env):
   env.Replace(QDSP6_SCRIPT_DIR = "${COREBSP_ROOT}/kernel/blast/install/modemSWv4/scripts")
...
def cust_config_emitter(target, source, env):
   ...
   env.Depends(target, "${QDSP6_SCRIPT_DIR}")

   # clean side effects
   env.Clean(target, "${BUILDPATH}/blast_config.c")
...
def cust_config_builder(target, source, env):
   ...
   cmd = "${PYTHONCMD} ${QDSP6_SCRIPT_DIR}/blast_config.py update -T ${QDSP6_RELEASE_DIR} -o " + cust_config_obj + " " + cust_config_xml
