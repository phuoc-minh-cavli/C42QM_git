...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/core.tx/6.0/bsp/build/scripts/mbn_tools.py#1 $
...
def pboot_gen_elf(env, elf_in_file_name,
   ...
   if elf_out_file_name is not None:
     ...
     # Determine original start of the program headers 
     phdr_start = elf_header.e_phoff

     # Shift program headers if required
     if phdr_start > ELF_HDR_SIZE:
         phdr_start += hashtable_shift
     ...
     # After copying last ELF segment do a check for program header location
     data_end_offset = curr_phdr.p_offset + hashtable_shift + curr_phdr.p_filesz
    
     # If the program header start offset is greater than the end of all data segments, 
     # reassign the program header start to the end of the last data segment. 
     if phdr_start > data_end_offset:
        phdr_start = data_end_offset
     ...
     hash = generate_sha1_hash(elfhdr_buff + proghdr_buff)
