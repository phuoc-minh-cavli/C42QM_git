...
class Elf32_dynamic(object):
    s = struct.Struct('I' * 2)
    DT_NULL             =0   # Marks end of dynamic array */
    DT_NEEDED           =1   # Name of needed library (DT_STRTAB offset) */
    DT_PLTRELSZ         =2   # Size, in bytes, of relocations in PLT */
    DT_PLTGOT           =3   # Address of PLT and/or GOT */
    DT_HASH             =4   # Address of symbol hash table */
    DT_STRTAB           =5   # Address of string table */
    DT_SYMTAB           =6   # Address of symbol table */
    DT_RELA             =7   # Address of Rela relocation table */
    DT_RELASZ           =8   # Size, in bytes, of DT_RELA table */
    DT_RELAENT          =9   # Size, in bytes, of one DT_RELA entry */
    DT_STRSZ            =10  # Size, in bytes, of DT_STRTAB table */
    DT_SYMENT           =11  # Size, in bytes, of one DT_SYMTAB entry */
    DT_INIT             =12  # Address of initialization function */
    DT_FINI             =13  # Address of termination function */
    DT_SONAME           =14  # Shared object name (DT_STRTAB offset) */
    DT_RPATH            =15  # Library search path (DT_STRTAB offset) */
    DT_SYMBOLIC         =16  # Start symbol search within local object */
    DT_REL              =17  # Address of Rel relocation table */
    DT_RELSZ            =18  # Size, in bytes, of DT_REL table */
    DT_RELENT           =19  # Size, in bytes, of one DT_REL entry */
    DT_PLTREL           =20  # Type of PLT relocation entries */
    DT_DEBUG            =21  # Used for debugging; unspecified */
    DT_TEXTREL          =22  # Relocations might modify non-writable seg */
    DT_JMPREL           =23  # Address of relocations associated with PLT */
    DT_BIND_NOW         =24  # Process all relocations at load-time */
    DT_INIT_ARRAY       =25  # Address of initialization function array */
    DT_FINI_ARRAY       =26  # Size, in bytes, of DT_INIT_ARRAY array */
    DT_INIT_ARRAYSZ     =27  # Address of termination function array */
    DT_FINI_ARRAYSZ     =28  # Size, in bytes, of DT_FINI_ARRAY array*/
    DT_PROCESSOR        =0x70000001 #Proccessor specific tag
    DT_VERSYM           =0x6ffffff0 #ARM Symbol Versioning
    DT_VERDEFNUM        =0x6ffffffd #ARM Symbol Versioning
    DT_VERDEF           =0x6ffffffc #ARM Version Definition
    def __init__(self, data):
        unpacked_data       = (Elf32_dynamic.s).unpack(data)
        self.unpacked_data   = unpacked_data
        self.tag            = unpacked_data[0]
        self.val            = unpacked_data[1]
    def get_tag_str(self):
        tag_name = self.tag
        if(self.DT_NULL == tag_name):
            return "DT_NULL"
        elif(self.DT_NEEDED == tag_name):
            return "DT_NEEDED"
        elif(self.DT_PLTRELSZ == tag_name):
            return "DT_PLTRELSZ"
        elif(self.DT_PLTGOT == tag_name):
            return "DT_PLTGOT"
        elif(self.DT_HASH == tag_name):
            return "DT_HASH"
        elif(self.DT_STRTAB == tag_name):
            return "DT_STRTAB"
        elif(self.DT_SYMTAB == tag_name):
            return "DT_SYMTAB"
        elif(self.DT_RELA == tag_name):
            return "DT_RELA"
        elif(self.DT_RELASZ == tag_name):
            return "DT_RELASZ"
        elif(self.DT_RELAENT == tag_name):
            return "DT_RELAENT"
        elif(self.DT_STRSZ == tag_name):
            return "DT_STRSZ"
        elif(self.DT_SYMENT == tag_name):
            return "DT_SYMENT"
        elif(self.DT_INIT == tag_name):
            return "DT_INIT"
        elif(self.DT_FINI == tag_name):
            return "DT_FINI"
        elif(self.DT_SONAME == tag_name):
            return "DT_SONAME"
        elif(self.DT_RPATH == tag_name):
            return "DT_RPATH"
        elif(self.DT_SYMBOLIC == tag_name):
            return "DT_SYMBOLIC"
        elif(self.DT_REL == tag_name):
            return "DT_REL"
        elif(self.DT_RELSZ == tag_name):
            return "DT_RELSZ"
        elif(self.DT_RELENT == tag_name):
            return "DT_RELENT"
        elif(self.DT_PLTREL == tag_name):
            return "DT_PLTREL"
        elif(self.DT_DEBUG == tag_name):
            return "DT_DEBUG"
        elif(self.DT_TEXTREL == tag_name):
            return "DT_TEXTREL"
        elif(self.DT_JMPREL == tag_name):
            return "DT_JMPREL"
        elif(self.DT_BIND_NOW == tag_name):
            return "DT_BIND_NOW"
        elif(self.DT_INIT_ARRAY == tag_name):
            return "DT_INIT_ARRAY"
        elif(self.DT_FINI_ARRAY == tag_name):
            return "DT_FINI_ARRAY"
        elif(self.DT_INIT_ARRAYSZ == tag_name):
            return "DT_INIT_ARRAYSZ"
        elif(self.DT_PROCESSOR == tag_name):
            return "DT_PROCESSOR"
        elif(self.DT_VERSYM == tag_name):
            return "DT_VERSYM"
        elif(self.DT_VERDEF == tag_name):
            return "DT_VERDEF"
        elif(self.DT_VERDEFNUM == tag_name):
            return "DT_VERDEFNUM"
        else:
            return "UNKNOWN_TAG"
    def get_value(self):
    ...
    def print_values(self):
        ...
        print tag + "\t" + str(val)
    def get_packed_data(self):
        values = [self.tag , self.val]
        return (Elf32_dynamic.s).pack(*values)
#----------------------------------------------------------------------------
...
def merge_sections_by_name(out_elf_dict, sec_elf_dict, pri_elf_file_name, 
    ...
    return 0
def merge_dynamic_sections_by_name(out_elf_dict, sec_elf_dict, pri_elf_file_name, 
                                pri_section_name, sec_section_name):
    out_elf_fp     = out_elf_dict['elf_fp']
    out_elf_header = out_elf_dict['elf_header']
    out_phdr_table = out_elf_dict['phdr_table']
    ...
    total_sec_size = 0
    for shdr in sec_shdr_list:
        if (shdr.sh_type != Elf32_Shdr.SHT_PROGBITS and shdr.sh_type != Elf32_Shdr.SHT_DYNAMIC\
            and shdr.sh_type != Elf32_Shdr.SHT_STRTAB and shdr.sh_type != Elf32_Shdr.SHT_DYNSYM\
            and shdr.sh_type != Elf32_Shdr.SHT_HASH):
            err_str = 'Error: ELF file {0}\'s Section: "{1}" not marked as PROGBITS'    \
                        .format(sec_elf_file_name, shdr.str_name)
            raise RuntimeError, err_str
        if (shdr.sh_flags & out_shdr.sh_flags) != shdr.sh_flags:
