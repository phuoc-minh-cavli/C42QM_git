...
#===============================================================================
# Edit History
# $Header: //components/rel/core.tx/6.0/bsp/build/scripts/tms_builder_servreg.py#1 $
...
TOOL_KEY_DOMAIN_LIST        = ['modem','adsp','slpi', 'apps']
TOOL_KEY_SUBDOMAIN_NAME     = 'subdomain'
TOOL_KEY_SUBDOMAIN_LIST     = ['root_pd','modem_pd','audio_pd','sensor_pd']
...
def tool_action(target, source, env):
   ...
   Playbook_C_Output.append('   {"xxx/xxx", 0 , 0},\n')
   Playbook_C_Output.append('};\n\n')
   ...
   # Final Output
      ...
      Playbook_C_Output.append('unsigned int servreg_service_count = sizeof(servreg_local_services)/sizeof(servreg_local_services[0]) - 1;\n')
...
def tool_method_add_domain(env, build_ids, input_dict):
   ...
   if not input_dict[TOOL_KEY_DOMAIN_NAME] in TOOL_KEY_DOMAIN_LIST:
      tool_error(env, input_dict[TOOL_KEY_DOMAIN_NAME] + ' is not a valid ' + TOOL_KEY_DOMAIN_NAME + ' name. Get name from list = [modem, adsp, slpi, apps] ')
   ...
   if not input_dict[TOOL_KEY_SUBDOMAIN_NAME] in TOOL_KEY_SUBDOMAIN_LIST:
      tool_error(env, input_dict[TOOL_KEY_SUBDOMAIN_NAME] + ' is not a valid ' + TOOL_KEY_SUBDOMAIN_NAME + ' name. Get name from list = [modem_pd, audio_pd, sensor_pd, root_pd ] ')
