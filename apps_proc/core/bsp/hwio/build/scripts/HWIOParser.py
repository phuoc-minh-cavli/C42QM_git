...
#  ===========================================================================
#
#  $Header: //components/rel/core.tx/6.0/bsp/hwio/build/scripts/HWIOParser.py#1 $
  ...
  def CheckForDuplicates (self, symbol, fix=False):
    ...
    if isinstance(dup, HWIOModule) and isinstance(symbol, HWIOModule):
      if len(dup.registers) == 0:
        #LogWarning("Removing empty duplicate module (line %d): %s" % (dup.line_number, symbol.name), symbol.line_number)
    ...
    if dup:
      LogWarning("Duplicate symbol (line %d): %s" % (self.symbols[symbol.Symbol()].line_number, symbol.name), symbol.line_number)
      if not fix:
        raise SyntaxError
...
class HWIOField:
  ...
  def __init__(self, m, line_number, register, *args, **kwargs):
    ...
    extra_config = m.group(4)

    self.retention = False
    if re.search('RETENTION', extra_config, re.I):
        self.retention = True
        extra_config = re.sub('RETENTION', '', extra_config)

    self.unrecognized_params = extra_config.strip()

  def BitRangeStr(self):
    if self.bit_low == self.bit_high:
      return "%d" % (self.bit_high)
    return "%d:%d" % (self.bit_high, self.bit_low)

  def __str__(self):
    return "%s[%s]" % (self.name, self.BitRangeStr())
  ...
  def RetentionStr (self):
    if self.retention or self.register.retention:
      return 'Yes'
    else:
      return 'No'
...
class HWIORegister(object):
  ...
  def __init__(self, m, line_number, module):
    self.fields = []
    self.line_number = line_number
    self.access_str = None
    self.module = module
    self.dimensions = 0
    self.reset_value = None
    self.retention = False
    self.retention_str = None
    ...
    self.ParseExtraConfig(m.group(4))

  def ParseExtraConfig(self, extra_config):
    self.retention = False
    if re.search('RETENTION', extra_config, re.I):
        self.retention = True
        extra_config = re.sub('RETENTION', '', extra_config)
  ...
  def AccessStr (self):
    if not self.access_str:
      if self.Command():
        if self.Readable():
            self.access_str = "Read/Write (Command)"
        else:
            self.access_str = "Write (Command)"
      elif self.Readable() and self.Writeable():
        self.access_str = "Read/Write"
      elif self.WriteOnly():
        self.access_str = "Write"
      else:
        self.access_str = "Read"
    return self.access_str

  def RetentionStr (self):
    if not self.retention_str:
      if self.retention:
        self.retention_str = 'Yes'
      else:
        retention_fields = len(filter(lambda field: field.retention, self.fields))
        if retention_fields == 0:
          self.retention_str = 'No'
        elif retention_fields == len(self.fields):
          self.retention_str = 'Yes'
        else:
          self.retention_str = 'Partial'
...
class HWIORegisterArray (HWIORegister):
  ...
  def __init__(self, m, line_number, module):
    ...
    if array_m != None:
      ...
      self.stride = [int(formula_m.group(1), 0)]
    ...
    if array_m != None:
      ...
      self.stride = [int(mult0, 0), int(mult1, 0)]
  ...
  def __str__(self):
    ...
    for dim in range(self.dimensions):
      name += ', %s=[%d..%d]' % (self.arg[dim], self.min_index[dim], self.max_index[dim])
    return name

  def ArrayRange (self):
    if self.dimensions == 1:
      return [(x, -1) for x in range(self.min_index[0], self.max_index[0] + 1)]
    elif self.dimensions == 2:
      return [(x, y) for x in range(self.min_index[0], self.max_index[0] + 1) \
                     for y in range(self.min_index[1], self.max_index[1] + 1)]
  ...
  def Unroll (self):
    '''
    This function will "unroll" the array into a list of HWIORegister
    objects.  It does not change the array register itself.
    '''
    regs = []
    for idx in self.ArrayRange():
      reg = HWIORegister(None, self.line_number, self.module)
      reg.name = self.Symbol(idx[0], idx[1])
      reg.offset = self.Offset(idx[0], idx[1])
      reg.access = self.access
      reg.retention = self.retention
      reg.unrecognized_params = self.unrecognized_params
      reg.reset_value = self.reset_value
      reg.reset_mask = self.reset_mask

      # Create copies of the fields so they can be modified to point
      # back to the right register.  Note each field will share all
      # the same HWIOValue objects.
      for field in self.fields:
        new_field = copy.copy(field)
        new_field.register = reg
        reg.AddField(new_field)
  ...
  def CreateFromRegister (cls, base_register, array_offset, max_index, arg='a', arg_pos=None, **kwargs):
    ...
    array.retention = base_register.retention
    array.unrecognized_params = base_register.unrecognized_params
    array.reset_value = base_register.reset_value
    array.reset_mask = base_register.reset_mask
    array.min_index = [ 0 ]
...
class HWIOModule:
  ...
  def __init__(self, m, line_number, use_apre=False, *args, **kwargs):
    ...
    if not re.search(self.postfix + '$', '_' + self.name):
      ...
      self.postfix_module = self.postfix
    else:
      self.postfix_module = ''
  ...
  def SetPostfix (self, new_postfix):
    if self.postfix_override != None:
      postfix_len = len(self.postfix_override)
      postfix_module_len = len(self.postfix_override)
    else:
      postfix_len = len(self.postfix)
      postfix_module_len = len(self.postfix_module)

    if postfix_module_len == 0:
      self.name = self.name + new_postfix
    else:
      self.name = self.name[:-postfix_module_len] + new_postfix

    for register in self.registers:
      if postfix_len == 0:
        register.name = register.name + new_postfix
      else:
        register.name = register.name[:-postfix_len] + new_postfix

    self.postfix_override = new_postfix

  def RestorePostfix (self):
    if self.postfix_override == None:
      return
    postfix_len = len(self.postfix_override)

    if postfix_len == 0:
      self.name = self.name + self.postfix_module
    else:
      self.name = self.name[:-postfix_len] + self.postfix_module

    for register in self.registers:
      if postfix_len == 0:
        register.name = register.name + self.postfix
      else:
        register.name = register.name[:-postfix_len] + self.postfix
...
class HWIOBase:
  ...
  def __init__(self, m, line_number, *args, **kwargs):
    if type(m) == list:
      ...
      self.address = m[1]
      self.size = m[2]
      self.size_found = True if self.size else False
    else:
  ...
  def AutoSize (self, max_size=None, round_up=False):
    '''
    This function will attempt to automatically size the base to best
    fit the containing modules.  The size will be rounded up to the
    closest 4K, 64K or 1MB boundary if requested.
    Note if no modules are present the size will be set to 0.
    '''

    # Find the largest offset in the list of modules.
    min_size = 0
    for module in self.modules:
      size = module.offset + module.size
      if size > min_size:
        min_size = size
    ...
    if round_up:
      ...
      elif min_size < 0x100000:
        size = (min_size +  0xFFFF) &  ~0xFFFF
      else:
        size = (min_size + 0xFFFFF) & ~0xFFFFF
    else:
      size = min_size
      ...
      if line_type == HWIO_LINE_REGISTER:
        ...
        if next_register_is_array:
          ...
          if register:
            ...
            register.ParseExtraConfig(m.group(4))
        else:
          try:
