...
#  ===========================================================================
#
#  $Header: //components/rel/core.tx/6.0/bsp/hwio/build/scripts/HWIOGenRegs.py#1 $
...
def ProcessModule (module, regfiles):
  ...
  for regfile in regfiles:
    ...
    # Save the module name here so that when checking for postfix overrides
    # we can use it.  Otherwise if a prefix override is used we won't have
    # the correct module.name anymore.
    orig_module_name = module.name
    ...
    if orig_module_name in regfile['postfix-overrides']:
      module.SetPostfix(regfile['postfix-overrides'][orig_module_name])

    # Check for a base override.
    if orig_module_name in regfile['base-overrides']:
      base_name = regfile['base-overrides'][orig_module_name]
    else:
      base_name = module.base.Symbol()
    ...
    value  = '(%s      + %#010x)' % (base_name, module.offset)
    f.write( form % (define, value) )

    if regfile['output-phys']:
      define = '%s_PHYS' % module.Symbol()
      value  = '(%s_PHYS + %#010x)' % (base_name, module.offset)
      f.write( form % (define, value) )
    ...
    for register in module.registers:
      ...
      # Check if we have any definitions for unrolling arrays into
      # register definitions.
      if 'unroll-array' in regfile:
        for regname in regfile['unroll-array']:
          if re.search(regname, register.Symbol()):
            for reg in register.Unroll():
              ProcessRegister(reg, regfile)
            break

    module.RestorePostfix()
...
def InitRegisterFile (regfile):
  ...
  elif 'stringio' in regfile:
    regfile['file'] = regfile['stringio']
  else:
  ...
  if 'chip-release' in regfile:
    f.write('\n')
    f.write('  Reference chip release:\n')
    f.write('    %s\n ' % regfile['chip-release'])
...
def GenerateRegisters (bases, register_files, run_silently=False):
  ...
  for regfile in register_files:
    ...
    if not 'postfix-overrides' in regfile:
      regfile['postfix-overrides'] = {}
    if not 'base-overrides' in regfile:
      regfile['base-overrides'] = {}
...
postfix-overrides (dictionary of strings key'ed by module name):
  Use this parameter to override the expected module and register postfix
  for a selected module.  Used generally when creating definitions
  for a module that has moved between two chip versions but binary
  compatibility is required.
  Example:
    'postfix-overrides': {'SECURITY_CONTROL_CORE' : '_V2'}
 
base-overrides (dictionary of strings key'ed by module name):
  Use this parameter to override the basename for a selected module.
  This is generally only used in strange exceptional cases although
  it could be used to specify a variable name or other alternate base
  concept.
  Example:
...
unroll-array (list of strings):  Causes the given array registers to be 
  "unrolled" into individual registers.  
  Example:
    # Here HWIO_GICD_ISRn_ADDR would be unrolled into HWIO_GICD_ISR0_ADDR,
    # HWIO_GICD_ISR1_ADDR etc.
    'unroll-array': [ 'GICD_ISRn', 'GICD_ISENABLERn' ],
