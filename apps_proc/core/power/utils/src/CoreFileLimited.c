...
$Header: //components/rel/core.tx/6.0/power/utils/src/CoreFileLimited.c#1 $
============================================================================*/
 ...
 */
static void CoreFile_getSize( const char *filename, uint32 *filesize )
{
   int ret;
   struct fs_stat fs_stat_buf;

   /* if fail return non-zero value */
   ret = efs_stat(filename,&fs_stat_buf);
   if(ret)
   {
      CoreFile_Log( " %s failed: file has size zero or doesn't exist", __FUNCTION__ );
   }
   *filesize = fs_stat_buf.st_size;
...
static unsigned char CoreFile_LoadFile( const char *filename, int flags, int *fileHandle )
{
    ...
    /* Get the size of file,if fails return ZERO*/
    CoreFile_getSize( filename, &filesize );
    if(!filesize)
    {
      return FALSE;
    }
    ...
    for (i = 0; i < buffer_size_num; i++) {
        ...
        /* load file content */
        ret = CoreFile_LoadFileHelper(filename, file->dataBuf, filesize);
