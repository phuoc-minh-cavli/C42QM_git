...
$Header: //components/rel/core.tx/6.0/power/utils/src/CoreOpaqueHandle.c#1 $
==============================================================================*/
  ...
  unsigned int num_new_tables )
{
  ...
  CORE_VERIFY( new_tables = (void *) malloc( size ) );
  ...
  for( i = num_old_tables; i < num_new_tables; i++ )
  {
     ...
     CORE_DAL_VERIFY( new_tables[i] = (void  *) malloc( sizeof( CoreOpaqueHandleTable ) ) );
