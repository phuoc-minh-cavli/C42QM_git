...
$Header: //components/rel/core.tx/6.0/power/npa/src/framework/npa_resource.c#1 $
============================================================================*/
...
//    CORE_DAL_VERIFY( Core_WorkQueueAlloc( npa.event_queue, (void **)&action ) );
	action = (npa_event_action*) Core_Malloc (sizeof(npa_event_action));
...
#endif
    CORE_DAL_VERIFY( 			
      Core_ObjectEnqueue (npa.event_queue, action, in_stm_state ? TRUE : FALSE));
...
{
  if ( _npa_resource_has_attribute( 
                     resource, NPA_RESOURCE_ENABLE_PREPOST_CHANGE_EVENTS ) )
  {
...
void npa_enqueue_async_request( npa_client_handle client, int new_request )
{
  ...
  //CORE_DAL_VERIFY( Core_WorkQueueAlloc( npa.async_request_queue, (void **)&request_data ) );
  request_data = (npa_async_request_data*) Core_Malloc (sizeof(npa_async_request_data));
...
#endif
  /*CORE_DAL_VERIFY( 
     Core_WorkQueueInsert(request_data, FALSE, COREWORKLOOP_REQUEST_QUEUE ) );*/
	CORE_DAL_VERIFY(
	  Core_ObjectEnqueue (npa.async_request_queue, request_data, FALSE));
