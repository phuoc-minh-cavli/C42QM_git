...
$Header: //components/rel/core.tx/6.0/power/npa/src/framework/npa.c#1 $
============================================================================*/
                                      ...
                                      npa_resource_state state )
{
  if ( client->type != NPA_CLIENT_INITIALIZE && 0 == state )
  {
    ...
    // TODO: Replace Core_WorkQueueSignalWorkLoop() call with Core Object quueue call
...
{
  Core_CreateObjectQueue (&npa.event_queue, npa_async_event_handler);
  CORE_VERIFY_PTR( npa.event_queue );
  
  Core_CreateObjectQueue (&npa.async_request_queue, npa_issue_async_request);
  CORE_VERIFY_PTR( npa.async_request_queue );
                             ...
                             void *data,    unsigned int data_size )
{
  ...
  CORE_DAL_VERIFY( Core_ObjectEnqueue (npa.event_queue, data, FALSE) );
