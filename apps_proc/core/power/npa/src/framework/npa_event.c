...
$Header: //components/rel/core.tx/6.0/power/npa/src/framework/npa_event.c#1 $
============================================================================*/
                          ...
                          void                  *context )
{
 ...
 // CORE_DAL_VERIFY( Core_WorkQueueAlloc( npa.event_queue, (void **)&action ) );
  ...
  CORE_DAL_VERIFY ( Core_ObjectEnqueue (npa.event_queue, (void *)action, FALSE) );
  ...
  if ( trigger_sys_event )
  {
   // CORE_DAL_VERIFY( Core_WorkQueueAlloc( npa.event_queue, (void **)&action ) );
    ...
    CORE_DAL_VERIFY( 			
      Core_ObjectEnqueue (npa.event_queue, (void*)action, FALSE));
  }
...
//  CORE_DAL_VERIFY( Core_WorkQueueAlloc( npa.event_queue, (void **)&action ) );
  ...
  action->state.event = event;
  CORE_DAL_VERIFY( Core_ObjectEnqueue( npa.event_queue, action, FALSE ) );
...
#endif						  
  CORE_DAL_VERIFY( 			
      Core_ObjectEnqueue (npa.event_queue, (void*)action, in_stm_state ? TRUE : FALSE));
...
npa_custom_event_action_destroy( npa_event_action *action )
{
  ...
  if ( event->action )
  {
    //Todo: Call Object queue free function
