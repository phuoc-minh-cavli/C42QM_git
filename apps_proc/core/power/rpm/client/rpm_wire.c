...
static unsigned _rpm_wire_send(const unsigned msg_id, rpm_set_type set, const rpm_resource_type resource, unsigned id, rpm_resource_t *resource_data)
{
    ...
    {
        char * data = NULL;	    
        RPMMessageHeaderV1 msg = {{NULL}, {NULL}, {NULL}, {NULL}};
	msg.msg_id = msg_id;
    ...
    {	    
        ...
        if(msg.set == RPM_NEXT_ACTIVE_SET)
        {
            rpmclient_put_nas(&msg);
        }
        else
        {
            rpmclient_put(&msg);
...
void rpm_wire_recv_ack(RPMAckPacketV1 *msg, rpm_mq_t *mq)
{
    ...
    rpm_mq_completed(mq, msg->msg_id, true);
...
void rpm_wire_recv(kvp_t *message, rpm_mq_t *mq)
{
    ...
    rpm_mq_completed(mq, *message_id, true);
}
