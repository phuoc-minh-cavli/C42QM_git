...
qurt_signal_t glink_sync_event;
#define QURT_SIG_MASK 0x1
...
unsigned rpm_msg_to_misc_resource_to_abort(void)
{
  ...
  ULOG_RT_PRINTF_0(rpm_log, "rpm_msg_to_misc_resource_to_abort");

  data = 0xdeadbeef;
  kvp_put(test_data, 0x74726261, 4, (char *)&data);

  msg_id = rpm_post_request(RPM_ACTIVE_SET, RPM_MISC_REQ, 0, test_data);
...
unsigned rpm_force_sync(rpm_set_type set)
{
    ...
    {
	 unsigned oldest_msg_id = rpm_mq_get_oldest_in_flight_msg_id(rpm.rpm_mq);
        //if we are sending the NAS set, churn all the active/sleep messages so we have plenty of room
        while(oldest_msg_id)
        {
            rpm_churn_queue(oldest_msg_id);
           oldest_msg_id = rpm_mq_get_oldest_in_flight_msg_id(rpm.rpm_mq);
        }  
    }
...
static void rpmclient_glink_isr(char * rx_buffer, unsigned size)
{
    ...
    else if(version == 1)
    {
        ...
        type = MESSAGING_V1_UNPACK_TYPE(*(unsigned*)rx_buffer);
        switch(type)
        {
...
static void rpmclient_glink_notify_state_isr(glink_handle_type handle, void *data, glink_channel_event_type event)
{
    ...
    {
        rpm.rpm_is_up = true;
        qurt_signal_set((qurt_signal_t *)&glink_sync_event, QURT_SIG_MASK);
    }	    
}	
...
void rpmclient_init(void)
{
   ...
   glink_link_id_type link_cfg = {1, NULL, "rpm", (glink_link_state_notif_cb)rpmclient_glink_link_state_isr, NULL};

    if(rpm.rpmclient_initialized)
        return;
    rpm.rpmclient_initialized = 1;
    rpm_fifo_full_handler = &rpm_mq_force_churn_cb;
    qurt_mutex_init(&rpm_lock);
    qurt_signal_init((qurt_signal_t *)&glink_sync_event);
    ...
    link_cfg.handle = rpm.glink_link;
    ...
    if(!is_standalone())
    {   
        qurt_signal_wait((qurt_signal_t *)&glink_sync_event, QURT_SIG_MASK, QURT_SIGNAL_ATTR_WAIT_ANY);
        if(!rpm.glink_port)
