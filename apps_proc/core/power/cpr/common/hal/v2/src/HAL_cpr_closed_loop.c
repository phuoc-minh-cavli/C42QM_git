...
extern void HAL_config_timers_for_warmup(HAL_cpr_rail_t rail);
...
#endif
    //Program the roscs for ro number read from fuses
    uint8 gcnt = CPR_DEFAULT_GCNT;
    uint8 rosc = corner_params->rosc_targets.rosc_num;

    CORE_VERIFY (corner_params->rosc_targets.ro_target_quotient > 0);
    HAL_cpr_prgm_gcnt_target(&rail->rail_config->hal_handle, (HAL_cpr_rosc_type)rosc, gcnt, corner_params->rosc_targets.ro_target_quotient);
}
...
void cpr_init_rail_hw(cpr_closed_loop_rail_t rail)
{
    ...
    HAL_cpr_set_clamp_timer_interval(hal_handle, 0);
	
    /* Program the target quots and GCNT initially to 0  */
    for (int rosc = 0; rosc < CPR_ROSC_COUNT; rosc++)
    {
        HAL_cpr_prgm_gcnt_target(&rail->rail_config->hal_handle, (HAL_cpr_rosc_type)rosc, 0, 0);
    }	
}
...
void cpr_hal_setup_warmup_settings(cpr_closed_loop_rail_t rail)
{
    const cpr_closed_loop_rail_config_t* rail_config = rail->rail_config;
    const HAL_cpr_rail_s* hal_handle = &rail_config->hal_handle;

    HAL_cpr_write_consecutive_dn(hal_handle, 1);
    HAL_cpr_set_count_mode(hal_handle, HAL_CPR_ALL_AT_ONCE);
    
    HAL_config_timers_for_warmup(hal_handle);

}

void cpr_hal_exit_warmup_settings(cpr_closed_loop_rail_t rail)
{
    const cpr_closed_loop_rail_config_t* rail_config = rail->rail_config;
    const HAL_cpr_rail_s* hal_handle = &rail_config->hal_handle;
    HAL_cpr_write_consecutive_dn(hal_handle, rail_config->consecutive_dn);
...
void cpr_handler(cpr_closed_loop_rail_t rail)
{
    ...
    if (current_voltage != recommendation)
    {
        ...
        //"If you change the VDD by exactly one PMIC step you send ACK , otherwise NACK. " The new warmup code can take more than
        // one PMIC step so this check is needed. 
        if ((recommendation - current_voltage == rail->pmic_step_size) || (current_voltage - recommendation == rail->pmic_step_size))
        {
            ack_result = TRUE;
        }
        else {
...
/**
 * <!-- cpr_calc_aging_delta -->
 *
 * @brief calculates the aging delta from the DEBUG1 QUOT_SLOW and QUOTE_FAST resutls.
 *
 ...
 */
static int32 cpr_calc_aging_delta(cpr_closed_loop_rail_t rail, HAL_cpr_rail_t hal_handle, int32 initial_fuse_delta)
{
    int32 max_age_delta;
    ...
    uint16 sel_fast = HWIO_INXF(hal_handle->hw_base_address, RBCPR_RESULT_1, SEL_FAST);
    int32 current_delta = 0;

    if (age_min != 0)
	{
       //generate current_delta based on sel_fast in results_1
      if(sel_fast == 1)
      {
        current_delta = ( age_min - age_max );
      }
      else if(sel_fast == 0)
      {
        current_delta = ( age_max - age_min );
      }
      else
      {
        CORE_VERIFY(0); // sel_fast variable is wrong
      }
    
      // calculate the max_age_delta
      max_age_delta = current_delta - initial_fuse_delta;
    
      // if the max_age_delta is negative, set it to 0 as we dont want to decrease the margin
      if(max_age_delta < 0)
      {
        max_age_delta = 0;
      }
        ...
        CPR_LOGGING(0, "WARNING - age_min was 0, aging appears to non functional. A safe max aging will be used");
        max_age_delta = 10000;
    }

    CPR_LOGGING(4, "(age_min: %d) (age_max: %d) (initial_fuse_delta: %d) (max_age_delta: %d)", age_min, age_max, initial_fuse_delta, max_age_delta);
    
    return max_age_delta;
}
...
{
    static bool aging_poll_done = FALSE;
    const cpr_closed_loop_rail_config_t* rail_config = rail->rail_config;
    const HAL_cpr_rail_s* hal_handle = &rail_config->hal_handle;
    uint8 aging_sensor_count;
    int32 initial_fuse_delta;
    int32 max_aging_delta = 0;
    const cpr_rail_id_t rail_id = rail->rail_id;
    
    //already did aging poll
    if (aging_poll_done == TRUE)
    {
       return;
    }
    aging_poll_done = TRUE;
    ...
    {
        rail->max_aging_delta = 10000; //aging 10000 will apply all the aging margin for safety
    ...
    {
        int32 measured_aging_delta; 
        cpr_aging_hw_init_and_poll(rail, hal_handle, rail->rail_config->aging_sensor_ids[i]);
        measured_aging_delta = cpr_calc_aging_delta(rail, hal_handle, initial_fuse_delta);
        max_aging_delta = MAX(max_aging_delta, measured_aging_delta);
    }
    
    rail->max_aging_delta = max_aging_delta;

    CPR_LOGGING(1, "Finished cpr_aging_poll.  max_aging_delta: %d", max_aging_delta);
}
