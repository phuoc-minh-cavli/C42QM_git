...
/**
* <!-- cpr_fuse_uses_top_bit_as_sign -->
*
* @brief Indicates whether the selected type of fuse should use the top bit as sign. 
*
*/
static bool cpr_fuse_uses_top_bit_as_sign(cpr_fuse_type fuse_type){
...
*
* @brief This function is called to read an individual fuse value. 
*
*/
void cpr_fuses_individual_fuse_init(cpr_bsp_individual_fuse_config_t* rail_fuse_config)
{
    ...
    for(int i=0; i<rail_fuse_config->number_of_partial_fuse_configs; i++)
    {
        ...
        //An address of 0 indicates a test configuration.  The mask value is the test value
        if (part_fuse_config->fuse_address == 0){
            rail_fuse_config->cpr_fuse_value = part_fuse_config->mask;
            CPR_LOGGING(2, "%s value forced to %x", cpr_logging_get_ascii_fuse_type(rail_fuse_config->fuse_type), rail_fuse_config->cpr_fuse_value);
            return;
        }
...
#ifdef ENABLE_FUSE_LOGGING
        CPR_LOGGING(3, "Reading fuse type %s at %x value is %x", cpr_logging_get_ascii_fuse_type(rail_fuse_config->fuse_type), part_fuse_config->fuse_address, raw_fuse_value);
#endif
    ...
    if (cpr_fuse_uses_top_bit_as_sign(rail_fuse_config->fuse_type)){
        //Create a mask for the value part of the fuse. This mask is to pull out the bottom (rail_fuse_config->fuse_total_bits-1) bits.
        value_mask = (1 << (rail_fuse_config->fuse_total_bits - 1)) - 1;
        rail_fuse_config->cpr_fuse_value = (int16)(cpr_fuse_bits&value_mask);
    }  else {
        rail_fuse_config->cpr_fuse_value = (int16)cpr_fuse_bits;
    }

#ifdef ENABLE_FUSE_LOGGING
	CPR_LOGGING(2, "Fuse %s is %d", cpr_logging_get_ascii_fuse_type(rail_fuse_config->fuse_type), rail_fuse_config->cpr_fuse_value);
#endif    

    //Only check the sign bit for some fuses
    if (cpr_fuse_uses_top_bit_as_sign(rail_fuse_config->fuse_type)){
        // Check the top bit to see if we need to flip the sign.
...
* <!-- cpr_fuses_rail_init -->
*
* @brief For a given rail, loop through all the fuses and read in the values. 
*
*/
