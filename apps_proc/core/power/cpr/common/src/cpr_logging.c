...
#if defined(APSS_CPR_BUILD) || defined(WINSIM)
ULogHandle cpr_ulog_h;   
ULogHandle cpr_ulog_static_h;
#endif


void cpr_logging_init(void)
{
 #if defined(APSS_CPR_BUILD) || defined(WINSIM)
...
char* cpr_logging_get_ascii_fuse_type(cpr_fuse_type fuse_type){
    ...
    if (fuse_type == CPR_FUSE_SVS2_OFFSET){
        return "CPR_FUSE_SVS2_OFFSET";
    }
    if (fuse_type == CPR_FUSE_SVS_OFFSET){
        return "CPR_FUSE_SVS_OFFSET";
    }
    if (fuse_type == CPR_FUSE_NOMINAL_OFFSET){
        return "CPR_FUSE_NOMINAL_OFFSET";
    }
    if (fuse_type == CPR_FUSE_TURBO_OFFSET){
        return "CPR_FUSE_TURBO_OFFSET";
    }
    if (fuse_type == CPR_FUSE_SUTURBO_OFFSET){
        return "CPR_FUSE_SUTURBO_OFFSET";
    }
    if (fuse_type == CPR_FUSE_FUSING_REV){
        return "CPR_FUSE_FUSING_REV";
    }
    if (fuse_type == CPR_FUSE_RO_SEL_SVS){
        return "CPR_FUSE_RO_SEL_SVS";
    }
    if (fuse_type == CPR_FUSE_RO_SEL_NOMINAL){
        return "CPR_FUSE_RO_SEL_NOMINAL";
    }
    if (fuse_type == CPR_FUSE_RO_SEL_TURBO){
        return "CPR_FUSE_RO_SEL_TURBO";
    }
    if (fuse_type == CPR_FUSE_LOCAL_QUOT_SVS){
        return "CPR_FUSE_LOCAL_QUOT_SVS";
    }
    if (fuse_type == CPR_FUSE_LOCAL_QUOT_NOMINAL){
        return "CPR_FUSE_LOCAL_QUOT_NOMINAL";
    }
    if (fuse_type == CPR_FUSE_LOCAL_QUOT_TURBO){
        return "CPR_FUSE_LOCAL_QUOT_TURBO";
    }	
...
void cpr_logging_update_aging_stats(int32 aging_margin_contribution, int32 aging_static_margin_limit){
    if ((aging_margin_contribution == 0) && (aging_static_margin_limit == 0)){
        ULOG_RT_STATIC_PRINTF_0(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_AGING_INFO,"Aging not enabled.");
    }  else {
        ULOG_RT_STATIC_PRINTF_2(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_AGING_INFO,
            "Aging (aging_margin_contribution: %d)  (aging_static_margin_limit: %d)",
            aging_margin_contribution, aging_static_margin_limit);
    }
                            
}

void cpr_logging_update_therm_stats(char* label, uint32 region, uint32 thermal_margin_adjust) {
    static uint32 therm_change_count = 0;
    if ((label == NULL) && (region == 0) && (thermal_margin_adjust == 0)){
        ULOG_RT_STATIC_PRINTF_0(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_THERM_INFO,"Thermal changes not enabled");
    }
    else {
        ULOG_RT_STATIC_PRINTF_4(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_THERM_INFO,
                                "Thermal (state: %s) (current_region: %u) (therm margin adjust: %d) (change_count: %d)",
                                 label, region, thermal_margin_adjust, ++therm_change_count);
    }
}


void cpr_logging_update_fuse_stats(cpr_rail_id_t rail_id){
    ULOG_RT_STATIC_PRINTF_5(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_FUSE_INFO,
                            "Debug (SVS2: %d)  (SVS: %d) (NOM: %d) (TURBO: %d) (AGING: %d)",
...
void cpr_logging_update_fusing_version(cpr_rail_id_t rail_id, int16 fusing_version)
{
    ULOG_RT_STATIC_PRINTF_5(cpr_ulog_static_h, CPR_ULOG_STATIC_LOG_FUSING_VERSION,
        "Offset Debug  (Rev: %d) (SVS2: %d)  (SVS: %d) (NOM: %d) (TURBO: %d)",
        fusing_version,
        cpr_fuses_get_fuse_value(rail_id, CPR_FUSE_SVS2_OFFSET),
        cpr_fuses_get_fuse_value(rail_id, CPR_FUSE_SVS_OFFSET),
...
void cpr_logging_dump_cprRegistersToLog(cpr_closed_loop_rail_t rail)
{
  ...
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " CPR_SENSOR_BYPASS_READ0 : 0x%x", CPR_HWIO_IN(HWIO_CPR_SENSOR_BYPASS_READn_ADDR(hal_handle->hw_base_address, 0)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " CPR_SENSOR_MASK_READ0 : 0x%x", CPR_HWIO_IN(HWIO_CPR_SENSOR_MASK_READn_ADDR(hal_handle->hw_base_address, 0)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " MSS_CPR_RESULT0_0 : 0x%x", CPR_HWIO_IN(HWIO_CPR_RESULT0_t_ADDR(hal_handle->hw_base_address, 0)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " MSS_CPR_RESULT1_0 : 0x%x", CPR_HWIO_IN(HWIO_CPR_RESULT1_t_ADDR(hal_handle->hw_base_address, 0)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " MSS_CPR_RESULT2_0 : 0x%x", CPR_HWIO_IN(HWIO_CPR_RESULT2_t_ADDR(hal_handle->hw_base_address, 0)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " CPR_BIST_CHAIN_CHECK1 : 0x%x", CPR_HWIO_IN(HWIO_CPR_BIST_CHAIN_CHECK1_ADDR(hal_handle->hw_base_address)));
  ULogFront_RealTimePrintf(cpr_ulog_h, 1, " CPR_FSM_STA : 0x%x", CPR_HWIO_IN(HWIO_CPR_FSM_STA_ADDR(hal_handle->hw_base_address)));
