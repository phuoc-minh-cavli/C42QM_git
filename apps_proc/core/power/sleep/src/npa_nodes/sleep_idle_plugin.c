/*==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/power/sleep/src/npa_nodes/sleep_idle_plugin.c#1 $
============================================================================*/
...
/**
 * sleep_idle_entry_point_tbl
 *
 * @brief Function pointer table for different Sleep behavior based on various
 *        restrictions.
 */
static sleep_idle_entry_ptr_type sleep_idle_entry_point_tbl[] = 
{
                                                    ...
                                                    npa_client_handle client)
{
  ...
  /* Holds the intermediate value when input latency in sclk is
   * converted to ticks */
  uint64 temp_latency_budget = 0;
  ...
  switch( client->type )
  {
    ...
    case SLEEP_IDLE_PLUGIN_CLIENT_LATENCY:
    {
      ...
      /* Requests to /core/cpu/latency resources are in sclk (32768Hz) while the
       * sleep calculations are in ticks. Converting the sclk to ticks
       * and safely truncating it to 32 bit value.
       */
      temp_latency_budget = SCLK_TO_TICKS64(latency_budget);
      latency_budget = temp_latency_budget > UINT32_MAX ? 
                       UINT32_MAX : (uint32)temp_latency_budget;
      break;
    }
