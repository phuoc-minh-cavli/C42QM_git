/*==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/power/sleep/src/solver/lookup_table_solver.c#1 $
========================================================================*/
...
)
{
  ...
  pModeLUT  = SleepLPR_power_lookup_table[CORE_CURR_CORE][lutIdx];
    ...
    CLprRegistry              *registry)
{
  ...
  /* Get head pointer to fLUT */
  freqTable = g_sleep_fLUT[CORE_CURR_CORE];
  ...
  if(closest_freq_LUT != NULL)
  {
    ...
    if(closest_freq_LUT->frequency > cur_freq)
    {
      ...
      g_sleep_fLUT[CORE_CURR_CORE]  = newTable;
    }
    else
    {
  ...
  CLprm_add_data_elements(registry, newTable);
                                     ...
                                     sleep_freq_lookup_table **freq_LUT)
{
  ...
  sleep_freq_lookup_table *freqTable        = g_sleep_fLUT[CORE_CURR_CORE];
  sleep_freq_lookup_table *workingCachePtr  = cachedEntry[CORE_CURR_CORE];
  sleep_freq_lookup_table *closest          = freqTable;
  ...
  if(workingCachePtr == 0 || (freq != workingCachePtr->frequency))
  {
    while(freqTable != NULL)
    {
      ...
      if(freq == freqTable->frequency)
      {
        ...
        *freq_LUT                   = freqTable;
        cachedEntry[CORE_CURR_CORE] = freqTable;
        return (TRUE);
      }
