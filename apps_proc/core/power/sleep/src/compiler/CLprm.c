...
================================================================================
$Header: //components/rel/core.tx/6.0/power/sleep/src/compiler/CLprm.c#2 $
$DateTime: 2021/05/30 08:04:16 $
==============================================================================*/
#include <stdint.h>
#include <stdlib.h>
#include "CLprm.h"
                            ...
                            uint32                  cycle_id )
{
  ...
  {
    /* Need first check as pfreq_LUT may be just allocated and 
     * not have been added to list */
    if((data_idx < hCLprm->num_data_elements) &&
       (hCLprm->cached_exit_latency[data_idx] != 0))
   ...
   * the correct value for future calls. First condition is required since
   * data_idx is obtained from pfreq_LUT which could have been just allocated
   * and not added to list yet */
  if((data_idx < hCLprm->num_data_elements) && 
     (hCLprm->cached_exit_latency != NULL))
  {
...
void CLprm_update( CLprm *hCLprm, sleep_lprm *changed_mode )
{
  ...
  CORE_VERIFY( hCLprm->num_enabled_component_modes <= 
...
/**
 * CLprm_set_initial_minlprm 
 *  
 * @brief Sets initial minlprm value for a newly created frequency table 
 *  
 * @param registry: Global registry 
 * @param coreIdx:  Core index to work with 
 * @param pfreq_LUT: Newly added frequency LUT 
 */
static void CLprm_set_initial_minlprm(CLprRegistry            *registry,
                                      uint32                  coreIdx,
                                      sleep_freq_lookup_table *pfreq_LUT)
{
  uint32  nSynth;
  ...
  uint32  currLatency;
  uint32  data_idx;
  uint32  tempMinLatency = UINT32_MAX;
  
  CORE_VERIFY_PTR(registry);
  CORE_VERIFY(coreIdx < SLEEP_NUM_CORES);
  
  data_idx  = pfreq_LUT->data_idx;
  lpr       = registry->m_lprs[coreIdx];
  minlprm   = &registry->m_minLprm[coreIdx][data_idx];

  /* Initiall minlprm is NULL */
  *minlprm = NULL;

  /* Search for min enabled LPRM */
  for(nSynth = 0; nSynth < SLEEP_LPR_NUM_SYNTHESIZED_MODES; nSynth++ )
  {
    ...
    {
      /* If the synth mode is enabled and has constant latencies we can check
       * if it is the minimum mode */
                             ...
                             sleep_freq_lookup_table  *pfreq_LUT)
{
  ...
  for( coreIdx = 0; coreIdx < SLEEP_NUM_CORES; coreIdx++ )
  {
    ...
    CLprm_set_initial_minlprm(registry, coreIdx, pfreq_LUT);
