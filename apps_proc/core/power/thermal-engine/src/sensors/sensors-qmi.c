...
static qurt_mutex_t qmi_ts_info_mtx ;//= PTHREAD_MUTEX_INITIALIZER;

/* Work thread for handling modem restart and init */
static qurt_thread_t work_thread;
...
static qurt_thread_t fusion_qmi_register_thread;
...
static qurt_mutex_t ts_req_mtx; //= PTHREAD_MUTEX_INITIALIZER;
...
static void *fusion_qmi_register(void *data)
{
	...
	msg("qmi: Instance id %d for fusion TS", instance_id);
	while (1) {
	...
	msg("Modem thermal sensor service available.\n");
				 ...
				 void *error_cb_data)
{
	...
	if (clnt == fusion_modem_clnt) {
		...
		qurt_thread_join(fusion_qmi_register_thread, NULL);
		qurt_thread_create(&fusion_qmi_register_thread, NULL,
...
void qmi_ts_update_temperature(const char *sensor, int temperature)
{
	...
	qurt_mutex_lock(&qmi_ts_info_mtx);
	...
	qurt_mutex_unlock(&qmi_ts_info_mtx);
}
...
int qmi_ts_setup(struct sensor_info *sensor)
{
	...
	qurt_mutex_init(&(qmi_ts->mutex));
	qurt_cond_init(&(qmi_ts->condition));
...
void qmi_ts_shutdown(struct sensor_info *sensor)
{
	...
	qurt_mutex_destroy(&qmi_ts->mutex);
	qurt_cond_destroy(&qmi_ts->condition);
...
int modem_ts_qmi_init(void)
{
	...
	if (!ts_service_object) {
        ...
        qurt_mutex_init(&qmi_ts_info_mtx);
        qurt_mutex_init(&ts_req_mtx);
	/* start thread to register with QMI services */
	qurt_thread_create(&fusion_qmi_register_thread, NULL, fusion_qmi_register, NULL);
