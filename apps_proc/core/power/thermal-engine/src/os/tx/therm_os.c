...
============================================================================*/
/*=======================================================================
$Header: //components/rel/core.tx/6.0/power/thermal-engine/src/os/tx/therm_os.c#1 $
...
#define TASK_SIG_DEFAULT_OFFLINE 0x0000001

//extern rex_tcb_type thermal_tcb;
qurt_signal_t thermal_sigobj;
  ...
  )
{
  ...
  /* Initialize the signal object to be used for the main thread */
  qurt_signal_init(&thermal_sigobj);

  while (1)
  {
	  
    /* Wait for the trigger signal before starting RCINIT_WORKER */
    sigs = qurt_signal_wait(&thermal_sigobj, 
                            TASK_SIG_DEFAULT_OFFLINE, 
                            QURT_SIGNAL_ATTR_WAIT_ANY);
    if(sigs & TASK_SIG_DEFAULT_OFFLINE)							
    {
      exit_daemon = 1;
      /* Clear the signal for the received qurt signal */
      qurt_signal_clear(&thermal_sigobj, TASK_SIG_DEFAULT_OFFLINE);
      break;
    }
  }
