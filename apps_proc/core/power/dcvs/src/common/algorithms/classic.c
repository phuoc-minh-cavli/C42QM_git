...
/*=======================================================================
                        Edit History
$Header: //components/rel/core.tx/6.0/power/dcvs/src/common/algorithms/classic.c#1 $ 
...
typedef struct
{
  ...
  uint64 sleepStart, sleepStop;
  uint64 systemStart, systemStop;

  /* Timer handle for the classic algorithm. */
  DCVSKernelTimerHandle timerHandle;
...
static void DCVSAlgoClassicTimerStart(uint32 time_ms, DCVS_CONTEXT* context)
{
  ...
  algoDataContext->sleepStart = DCVSKernelSleepTickGet(context);
  algoDataContext->systemStart = DCVSKernelSystemTickGet(context);
  algoDataContext->timerBusy = 1;
  /* Start the DCVS sampling timer. */
  DCVSKernelTimerStart(algoDataContext->timerHandle, algoDataContext->timerPeriod, context);
...
static void DCVSAlgoClassicTimerStop(DCVS_CONTEXT* context)
{
  ...
  DCVSKernelTimerStop(algoDataContext->timerHandle, context);
...
static void IntervalSystemSleepTimeGet(uint32* delTime, uint32* delSleep, DCVS_CONTEXT* context)
{
  ...
  algoDataContext->sleepStop = DCVSKernelSleepTickGet(context);
  algoDataContext->systemStop = DCVSKernelSystemTickGet(context);

  /* Obtain the system ticks/ sleep ticks for the previous interval */
  DCVSKernelIdleInfo(&idleOverride);

  // System Tick Delta = (System Tick Stop - System Tick Start) in secs . 
  *delTime = (uint32)( (algoDataContext->systemStop - algoDataContext->systemStart) & 0xFFFFFFFF ); 

  // Sleep Tick Delta = (Sleep Tick Stop - Sleep Tick Start) in secs .
  *delSleep = (uint32)( (algoDataContext->sleepStop - algoDataContext->sleepStart) & 0xFFFFFFFF );
...
static void DCVSAlgoClassicMipsCalculate(DCVS_CONTEXT* context)
{
  ...
  ULogFront_RealTimePrintf(algoDataContext->logHandle, 
                           10,
                           "CLASSIC_ALGO (systemStart : %llu) (systemStop: %llu) (sleepStart: %llu) (sleepStop: %llu) (DeltaTime: %d) (DeltaSleep: %d)",                       
						   ULOG64_LOWWORD(algoDataContext->systemStart), ULOG64_HIGHWORD(algoDataContext->systemStart),
                           ULOG64_LOWWORD(algoDataContext->systemStop), ULOG64_HIGHWORD(algoDataContext->systemStop),
						   ULOG64_LOWWORD(algoDataContext->sleepStart), ULOG64_HIGHWORD(algoDataContext->sleepStart),
                           ULOG64_LOWWORD(algoDataContext->sleepStop), ULOG64_HIGHWORD(algoDataContext->sleepStop),
                           deltaTime,
                           deltaSleep
                           );
#endif
  ULogFront_RealTimePrintf(algoDataContext->logHandle, 
                           9,
                           "CLASSIC_ALGO (AvgMipsWPrec: %d) (SysTime: %llu) (SleepTime: %llu) (DeltaTime: %d) (DeltaSleep: %d) (CurMips: %d) (NewMips: %d)",
                           avgMipsWithPrec,
                           ULOG64_LOWWORD(algoDataContext->systemStop), ULOG64_HIGHWORD(algoDataContext->systemStop),
                           ULOG64_LOWWORD(algoDataContext->sleepStop), ULOG64_HIGHWORD(algoDataContext->sleepStop),
                           deltaTime,
...
static void DCVSAlgoClassicClkChange(DCVS_CONTEXT* context)
{
  ...
  if(algoDataContext->timerStarted == TRUE && algoDataContext->currState != DCVS_ALGO_CLASSIC_INVALID_CLK_STATE)
  {
    ...
    if (algoDataContext->currState != algoDataContext->prevState)
    {
      ...
      localTimeElapsed = DCVSKernelTicksToMsConvert(localTimeElapsed);
...
static uint32 DCVSAlgoClassicInitialize(void* param)
{
  ...
  context->algoDataContext = (DCVS_ALGO_CLASSIC *)Core_Malloc(sizeof(DCVS_ALGO_CLASSIC));
  if(NULL == context->algoDataContext ) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);   
  }
  ...
  algoDataContext->timerHandle = DCVSKernelTimerCreate(DCVSAlgoClassicTimerCB, context);
...
void DCVSAlgoClassicSleepCb(sleep_event event_id, void* server_data, void* client_data)
{
  ...
  switch (event_id)
  {
    case SLEEP_EVENT_POWER_REMOVE:
      ...
      {
        DCVSKernelTimerStop(algoDataContext->timerHandle); 
      }
      break;
    ...
    case SLEEP_EVENT_POWER_RESTORE:
      ...
      DCVSKernelTimerStart(algoDataContext->timerHandle, algoDataContext->timerPeriod);
...
static void DCVSAlgoClassicThresholdTableSetup(DCVS_CONTEXT* context)
{
  ...
  (DCVS_ALGO_CLASSIC_CLK_THRESHOLD *)Core_Malloc( sizeof(DCVS_ALGO_CLASSIC_CLK_THRESHOLD)*(algoDataContext->clockPlanMaxIndex+1) );
  
  if( NULL == algoDataContext->thresholdTable ) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);
  }
