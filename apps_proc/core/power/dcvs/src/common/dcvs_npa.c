...
/*=======================================================================
                        Edit History
$Header: //components/rel/core.tx/6.0/power/dcvs/src/common/dcvs_npa.c#1 $ 
...
static void ResDataMemoryAllocateAndInitialize(DCVS_CONTEXT* context)
{
  ...
  if(context->resDataContext == NULL)
  {
    ...
    resDataContext = (DCVS_RES_DATA *)Core_Malloc(sizeof(DCVS_RES_DATA));
	
    if(NULL == resDataContext) 
    {
      /* Error fatal */
      CORE_VERIFY(NULL);   
    }
                                                   ...
                                                   npa_client_handle client )
{
  ...
  switch (client->type)
  {
	...
	resDataContext->clkChangePriority = Core_GetThreadSelfPriority();
	//Core_SetThreadPriority(qurt_thread_get_id() ,0);
      }
  ...
  else
  {
    ...
    if( DALCHIPINFO_ID_MDM9206 == DalChipInfo_ChipId() )
    {	
       /* for 9206, fix that to NOM level */
      newMips = 800;
    }
    else
    {
      newMips = resDataContext->maxMips;
    }
  }
                                                     ...
                                                     npa_resource_state state )
{
  ...
  if (resource == NPA_RESOURCE( resource, 0))
  {
    ...
    index = DCVSCoreClockPlanIndexGet(state, context);
                               ...
                               unsigned int size )
{
  ...
  if ( TRUE == resDataContext->dcvsInit )
  {
    ...
    /* Notify all the registered clients who requested for the callbacks 
	      from this event */
    previousPriority = Core_GetThreadSelfPriority();
    Core_SetThreadPriority(qurt_thread_get_id(), resDataContext->clkChangePriority );
    dcvs_npa_events_notify(DCVS_NPA_EVENT_CLK_CHANGE, context);
    Core_SetThreadPriority(qurt_thread_get_id(), previousPriority);
  }
  return;
}
                                    ...
                                    unsigned int data_size )
{
  ...
  CORE_VERIFY( npa_create_event_cb( dcvsContext->clockDependency, 
                                    "Clock_Change_Event", 
  ...
  if( !(DALCHIPINFO_ID_MDM9206 == DalChipInfo_ChipId()) )
  {	
     //By pass the vote for 9206 target to avoid TURBO vote from startup client
...
void NodeCoreCpuCreate (npa_node_dependency* targetNodeDeps, uint32 numNodeDeps, const char** targetSysDeps, uint32 numSysDeps, uint32 coreId, DCVS_CONTEXT* context)
{
  ...
  context->resourceContext = (npa_resource_definition *)Core_Malloc(sizeof(npa_resource_definition));
  if( NULL == context->resourceContext) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);   
  }
  ...
  nodePtr = (npa_node_definition *)Core_Malloc(sizeof(npa_node_definition));
  if( NULL == nodePtr ) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);   
