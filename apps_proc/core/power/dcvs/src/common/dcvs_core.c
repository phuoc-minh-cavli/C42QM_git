...
/*============================================================================================
                        Edit History
$Header: //components/rel/core.tx/6.0/power/dcvs/src/common/dcvs_core.c#1 $ 
...
static void ClockPlanDataMemoryAllocateAndInitialize(DCVS_CONTEXT* context)
{
  ...
  clockPlanTable = 
  (DCVS_PRIMARY_CLOCK_PLAN_DATA_TYPE*)Core_Malloc(sizeof(DCVS_PRIMARY_CLOCK_PLAN_DATA_TYPE)*(coreDataContext->clockPlan->numLevels));
  if( NULL == clockPlanTable) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);   
  }
...
const DCVS_CORE_ALGO * DCVSCoreAlgoGet(const char * algName)
{
  ...
  while (dcvsTargetAlgList[count]!= NULL)
  {
    ...
    if (strncasecmp(algName, dcvsTargetAlgList[count]->DCVSAlgoName,
...
static void CoreDataMemoryAllocateAndInitialize(DCVS_CONTEXT* context)
{
  ...
  {
    coreDataContext = ( DCVS_CORE_DATA*)Core_Malloc(sizeof(DCVS_CORE_DATA));	  
    if( NULL == coreDataContext) 
    {
      /* Error fatal */
      CORE_VERIFY(NULL);   
    }
...
void DCVSCoreInitialize(DCVS_CONTEXT* context)
{
  ...
  CpuMonitorInitialize(context);
  
  /* Start the CPU monitor if enabled */
  CpuMonitorStart(context);

  /* Introduce a delay before starting the DCVS algorithm through a callback. */
  DCVSKernelCallbackCreate(DCVSCoreStart, DCVSKernelStartUpDelay(), context);
...
void DCVSCoreStart(DCVS_CONTEXT* context)
{
  ...
  if (strncmp("Disabled", coreDataContext->currentAlgorithm->DCVSAlgoName, 
      (DCVS_NV_STRING_LEN_LIMIT-1)) == 0)
  {
...
/**
  ...
  @return : index where the mips value is found in the table.

*/
uint32 DCVSCoreClockPlanIndexGet(uint32 mips, DCVS_CONTEXT* context)
{
  uint32 i;
