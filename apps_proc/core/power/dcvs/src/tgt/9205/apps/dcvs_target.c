...
/*============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/power/dcvs/src/tgt/9205/apps/dcvs_target.c#1 $
  $DateTime: 2020/01/30 22:49:35 $

  when       who     what, where, why
  --------   ----    ---------------------------------------------------
  11/14/12    nj     Bus arbiter client changed to type NPA_CLIENT_SUPPRESSIBLE_VECTOR
  10/26/12    nj     9x25 bus-arbiter changes  
  09/28/11    sg     Set non-zero default size for ulog buffer.
  08/31/11    sg     Initial version
========================================================================*/
...
static const DCVS_NV_PARAM_ENTRY dcvsTargetAlgParamsDefault[] =
{
  ...
  {NULL, DCVS_NV_PARAM_NULL, 0},
};

static const DCVS_NV_PARAM_ENTRY dcvsTargetAlgParamsStartup[] =
{
  {"Algorithm", DCVS_NV_PARAM_STRING, {"classic"}},
...
{
  {"Classic", DCVS_NV_CORE_ID_ALL, dcvsTargetAlgParamsDefault},  
...
extern DCVS_CORE_ALGO defaultAlgorithm; // "classic"
...
void DCVSTargetSpecificInit (void)
{
  ...
  dcvsContext.targetDataContext = Core_Malloc(sizeof(DCVS_TARGET_STRUCT));
  if( NULL == dcvsContext.targetDataContext) 
  {
    /* Error fatal */
    CORE_VERIFY(NULL);   
  }
