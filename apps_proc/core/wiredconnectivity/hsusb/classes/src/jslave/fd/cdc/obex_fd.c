...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/classes/src/jslave/fd/cdc/obex_fd.c#1 $
...
#if defined (T_UNDP) || defined (FEATURE_HS_USB_OS_DETECTION)
...
#endif /* T_UNDP || FEATURE_HS_USB_OS_DETECTION */
...
jresult_t cdc_init_obex(void *core_ctx)
{
    ...
    if (!ctx)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "CDC_OBEX, failed_to_allocate_ctx");  
        rc = JENOMEM;
        goto Error;
    }
    ...
    if (rc)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "CDC_OBEX, app_failed_to_init");
        goto Error;
    }
    
    cbs = ctx->cbs.basic;
    
    /* HSU Addition - Resolve compilation warning about the 'cbs' variable
    ** not being used in ARMPRG\DLOAD builds.
    */
    HSU_USE_PARAM(cbs);
    ...
    if(!(ctx->info.obex.basic.flags & CDC_INIT_OBEX_BULK_IN_ONLY))
    {
   ...
   /* Create descriptors */
    ...
    if (rc)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "CDC_OBEX, failed_to_allocate_descriptors");        
        goto Error;
    }
    ...
    if (rc)
    {
        HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "CDC_OBEX, cdc_init failed, rc %u", rc);           
        goto Error;
    }
