...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/classes/src/jslave/fd/fd_storage_usb.c#1 $
...
void send_csw_callback(request_t *request)
{
    ...
    else
    {
        ...
        if (rc)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_to_post_next_CBW, rc %u", rc);            
        }
    }
}
...
{
    HSU_ULOG_NOFN(ERROR_MSG, DBG_E_LOG, "MS_USB, ******** Fatal processing error *********");     
    core_stall_pipe(fd->core_ctx, fd->in_pipe, 1);
    core_stall_pipe(fd->core_ctx, fd->out_pipe, 1);
    clear_cmd_in_progress(fd); 
}
...
static jresult_t control_msg(void *context, void *buffer)
{
    ...
    case USB_BULK_RESET_REQUEST:
        ...
        {
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, invalid_MS_RESET_req");              
            break;
        }
        ...
        if (rc)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_post_CBW_on_RESET_REQUEST, rc %u", rc);             
        }
        /******* End HSU addition **************************/
        break;
    ...
    case USB_BULK_GET_MAX_LUN_REQUEST:
        ...
        {
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, invalid_MS_GET_MAX_LUN_req");            
            break;
        }
        /* HSU addition: CR 159898 */
        /* Clear Stalls from pipes */
        ...
        if (rc)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_post_CBW_on_GET_MAX_LUN_REQUEST, rc %u", rc);
        }
        /******* End HSU addition **************************/
...
#define ALLOC_MSFD_REQUEST(i) \
    do { \
        rc = jdma_alloc(fd->scsi_buffer_size, 32, &(fd->io_buffer[i].vaddr), \
            &(fd->io_buffer[i].dma_addr), 0, \
            &(fd->io_buffer[i].cookie)); \
        if (rc) \
        { \
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, enabled, failed_allocate_SCSI_buffer"); \                
            goto Exit; \
        } \
        ...
        if (!fd->io_request[i]) \
        { \
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, enabled, failed_allocate_SCSI_req"); \                 
            rc = JENOMEM; \
            goto Exit; \
        } \
    } while(0)
...
#define ALLOC_MSFD_LA_REQUEST(i) \
    do { \
        rc = jdma_alloc(fd->scsi_buffer_size, 32, &(fd->la_io_buffer[i].vaddr), \
            &(fd->la_io_buffer[i].dma_addr), 0, \
            &(fd->la_io_buffer[i].cookie)); \
        if (rc) \
        { \
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, enabled, "\
                " failed_allocate_SCSI_lookahead_buffer"); \                 
            goto Exit; \
        } \
        fd->la_io_request[i] = core_request_alloc(fd->scsi_buffer_size, \
            fd->la_io_buffer[i].vaddr); \
        if (!fd->la_io_request[i]) \
        { \
            HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, enabled, failed_allocate_SCSI_req"); \                 
            rc = JENOMEM; \
            goto Exit; \
...
#define FREE_MSFD_LA_REQUEST(i) \
    do { \
        if (fd->la_io_buffer[i].cookie) \
        { \
            jdma_free(fd->la_io_buffer[i].cookie); \
            fd->la_io_buffer[i].cookie = NULL; \
            if (fd->la_io_request[i]) \
            { \
                core_request_free(fd->la_io_request[i]); \
                fd->la_io_request[i] = NULL; \
            } \
        } \
    } while (0)
...
static jresult_t enable(context_t context)
{
    ...
    /* HSU addition: CR 159898 */
    {
        HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_post_CBW_on_enable, rc %u", rc); \
        goto Exit;
    }
    /******* End HSU addition **************************/
...
static jresult_t resume(void *context)
{
    ...
    if (fd->prev_state == STATE_ENABLED)
    {
        rc = enable(context);
        if (rc){
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_to_resume_FD, rc %u", rc);
        }
    }
    return rc;
}
...
jresult_t mass_storage_init(void *ctx)
{
    ...
    if (!fd->ep0_request)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_allocate_ep0_req");
        return JENOMEM;
    }
    ...
    if (!fd->cmd_request)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "MS_USB, failed_allocate_cmd_req");        
        return JENOMEM;
    }
