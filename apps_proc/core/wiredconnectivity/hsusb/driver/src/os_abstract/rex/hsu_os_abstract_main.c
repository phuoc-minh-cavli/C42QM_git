...
$Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/driver/src/os_abstract/rex/hsu_os_abstract_main.c#1 $
...
jresult_t hsu_os_init(void)
{
  ...
  */
  ...
  if (status != HSU_JSUCCESS)
  {
    HSU_MSG_ERROR_1("hsu_os_init: Failed. status: %d", status);
    hsu_os_uninit();
  }
  else
  {
    file_static_vars_ptr->os_abstract_is_initialized = TRUE;
    HSU_MSG_HIGH("hsu_os_init: Exit.");
  }
