...
$Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/hal/src/HAL_hsusb_impl_ChipIdea.c#1 $
...
#include "busywait.h"                  /* busywait etc functions */
...
HAL_HSUSB_ErrorType HAL_hsusb_ReadUlpiPhyChipIdea(uint8 instance, uint8 port, uint8 reg, uint8* data)
{
  ...
  if (HAL_HSUSB_CoresObj.CoreInfo[instance].PortInfo[port].PhyType.PhyInterfaceType != HAL_HSUSB_PHY_IF_ULPI
      && HAL_HSUSB_CoresObj.CoreInfo[instance].PortInfo[port].PhyType.PhyInterfaceType != HAL_HSUSB_PHY_IF_HS_INTERCHIP)
  {
    HAL_HSUSB_CoresObj.CoreInfo[instance].LastError = HAL_HSUSB_WrongPhyInterface;
    return HAL_HSUSB_WrongPhyInterface;
  }
  ...
  */
  ...
  HAL_HSUSB_CoresObj.CoreInfo[instance].LastError = HAL_HSUSB_TimeoutExpired;
  return HAL_HSUSB_TimeoutExpired;
}
