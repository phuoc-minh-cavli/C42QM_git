...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/core/inc/jdebug.h#1 $
...
#define DBG_FUNC hsu_os_dprintf
...
{                                                              \
  ...
  {                                                            \
    XX_MSG_CONST (MSG_SSID_HSUSB, MSG_LEGACY_ERROR, fmt);      \
                                                               \
    hsu_dbg_e_log_ext(fmt, ##__VA_ARGS__);                     \
    /*lint -e{571} */                                          \
...
/* Defined in debug_init.c */
/***************************/
char* jdebug_sprintf_to_static_buffer(const char *fmt, ...);

#ifdef FEATURE_HS_USB_USE_LEGACY_CIR_BUF

#define DBG_E(type, message) \
        do { \
            char* jdebug_output_buf_ptr; \
            \
            jdebug_output_buf_ptr = jdebug_sprintf_to_static_buffer message; \
            \
            valid_type(type); \
            if ((DL_ERROR) <= debug_bank[(jint_t)(type) - ((jint_t)(type) % 10)] && \
                (DL_ERROR) <= debug_bank[(jint_t)(type)]) \
            { \
                MSG_ERROR(jdebug_output_buf_ptr, 0, 0, 0); \
            } \
        } while (0)
#else //else FEATURE_HS_USB_USE_LEGACY_CIR_BUF
#define DBG_E(type, message)
...
#define DBG_W(type, message) \
        do { \
            ...
            \
            valid_type(type); \
            if ((DL_WARNING) <= debug_bank[(jint_t)(type) - ((jint_t)(type) % 10)] && \
                (DL_WARNING) <= debug_bank[(jint_t)(type)]) \
            { \
                MSG_HIGH(jdebug_output_buf_ptr, 0, 0, 0); \
            } \
        } while (0)
...
#define DBG_I(type, message) \
        do { \
            ...
            \
            valid_type(type); \
            if ((DL_INFO) <= debug_bank[(jint_t)(type) - ((jint_t)(type) % 10)] && \
                (DL_INFO) <= debug_bank[(jint_t)(type)]) \
            { \
                MSG_MED(jdebug_output_buf_ptr, 0, 0, 0); \
            } \
        } while (0)
...
#define DBG_V(type, message) \
        do { \
            ...
            \
            valid_type(type); \
            if ((DL_VERBOSE) <= debug_bank[(jint_t)(type) - ((jint_t)(type) % 10)] && \
                (DL_VERBOSE) <= debug_bank[(jint_t)(type)]) \
            { \
                MSG_LOW(jdebug_output_buf_ptr, 0, 0, 0); \
            } \
        } while (0)
...
#define DBG_X(type, message) \
        do { \
            ...
            \
            valid_type(type); \
            if ((DL_EX_VERBOSE) <= debug_bank[(jint_t)(type) - ((jint_t)(type) % 10)] && \
                (DL_EX_VERBOSE) <= debug_bank[(jint_t)(type)]) \
            { \
                MSG_LOW(jdebug_output_buf_ptr, 0, 0, 0); \
            } \
        } while (0)
