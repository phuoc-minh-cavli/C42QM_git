...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/core/src/jos/jos_sync.c#1 $
...
jresult_t jtask_init(jtask_h *task, jtask_type type)
{
    ...
    KASSERT((type < TASK_TYPE_COUNT), ("TASK: task_type out of bound"));
...
void jtask_uninit(jtask_h task)
{
    ...
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, req_uninit_task_pending_a_stop"); 
    }
#endif
...
#endif
{
    ...
    KASSERT((task->type < TASK_TYPE_COUNT), ("TASK: task_type out of bound"));

    if (task->type >= TASK_TYPE_COUNT)
    {
      // Avoid running into array out of bound in case ASSERT is disabled
      return JEINVAL;
    }
    ...
    /* HSU Fix - Call jget_time only after locking the spinlock */
    ...
    /* End of HSU Fix */
        ...
        case JTASK_STOPPING:
            ...
            HSU_ULOG(HIGH_MSG, DBG_E_LOG, "DJOS_SYNC, schedule_a_stopping_task"); 
            return JEINVAL;
        }
    ...
    {
      // Avoid running into array out of bound in case ASSERT is disabled
      HSU_ERR_FATAL("task_type out of bound %u", task->type, 0, 0);
      return JEINVAL;
    }
...
{   
    ...
    if (jget_context() == task_thread_arr[task->type].context)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, task_tying_to_stop_itself");
        return;
    }
#endif
    ...
    {
         HSU_ULOG_2(HIGH_MSG, DBG_E_LOG, "DJOS_SYNC,"
                " waiting_for_task 0x%08X, to stop %d (cont)", (uint32) task, task->state);
         HSU_ULOG_2(HIGH_MSG, DBG_E_LOG, "DJOS_SYNC,"
                " [0x%X: 0x%X]", jget_context(), task_thread_arr[task->type].context);
    ...
    jthread_func cb, void *arg)
{
    ...
    timeout = timeout / HSU_EMULATION_TIME_FACTOR;
...
static void task_signal(jtask_type type, jtime_t *time)
{
    ...
    {
      // Avoid running into array out of bound in case ASSERT is disabled
      HSU_ERR_FATAL("task_type out of bound %u", type, 0, 0);
      return;
    }

    ctx = &(task_thread_arr[type]);
    ...
    while (ctx->flags & TASK_THREAD_ACTIVE)
    {
       ...
       DBG_X(DJOS_SYNC, ("TASK: %d Task thread sleeping\n", type));
        ...
        jspinlock_lock(task_lock);
        {
            ...
            {
                jspinlock_unlock(task_lock);
                HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, "
                "task_on_list_not_in_pending_state_but_in_state %d", task->state);                
                KASSERT(task->state < JTASK_MAX_COUNT, ("Corrupt task->state %d", task->state)); 
                continue;
            }
#endif
            ...
            /* HSU Fix - Use TIME_OVER to avoid overflow issues */
            if (!TIME_OVER(time, task->timeout))
            {
                jspinlock_unlock(task_lock);    
#ifdef CONFIG_DATA_PATH_DEBUG
        ...
        default:
            jspinlock_unlock(task_lock);
            /* HSU Addition */
            j_panic((uint32)"TASK: Unexpected state of finished thread" == NULL);
            /* HSU Addition end */
#endif
        }
        jspinlock_unlock(task_lock);
...
static jresult_t task_thread_init(jtask_type index)
{
    ...
    if (index >= TASK_TYPE_COUNT)
    {
      // Avoid running into array out of bound in case ASSERT is disabled
      HSU_ERR_FATAL("task_type out of bound %u", index, 0, 0);
      return JEINVAL;
    }

    ctx = &(task_thread_arr[index]);
...
static void task_thread_uninit(jtask_type index)
{
    ...
    if (ctx->flags | TASK_THREAD_THREAD_INIT)
    {
        ...
        #define HSU_THREAD_FINISH_WAIT_MAX_LOOP_COUNT (1000000)
        juint32_t max_loop_cnt;
        max_loop_cnt = HSU_THREAD_FINISH_WAIT_MAX_LOOP_COUNT;

        /* Check if there are any pending tasks left */
        if (ctx->list.next)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, "
                "task_tread %u, have_task_left", index);            
...
static void task_list_dump(jtask_type index)
{
    ...
    HSU_ULOG_2(HIGH_MSG, DBG_E_LOG, "DJOS_SYNC, index %u , runs %u", index, ctx->task_run);

    for (i = 0, task = ctx->list.next; task; i++, task = task->next)
    {
      HSU_ULOG_5(NO_MSG, DBG_E_LOG, "TASK: %d $d : task 0x%X [0x%X:0x%X]",
        index, i, task, task->cb. task->arg);
      HSU_ULOG_STR_2(NO_MSG, DBG_E_LOG, "cont, file:%s, line %d, Time diff %d",
         task->file, TIME_DIFF(time, task->timeout), task->line);
...
static jresult_t task_monitor_check(jtask_type index)
{
    ...
    if (ctx->state == TASK_THREAD_RUNNING)
    {
        if (!ctx->curr_task)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, thread %u, running_thread_wo_task_set", index);
            return JEINVAL;
        }
        ...
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, task %u, possible_deadlock", index);
            ...
            HSU_ULOG_3(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, task 0x%08X [0x%08X:0x%08X]",
              (uint32) ctx->curr_task, (uint32) ctx->curr_task->cb, (uint32) ctx->curr_task->arg);
            HSU_ULOG_STR_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, from [%s:%u]", ctx->curr_task->file, 
                ctx->curr_task->line);
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, run %u msec", run_time);
    ...
    for (task = ctx->list.next; task; task = task->next)
    {
        if (TIME_DIFF(time, task->timeout) < -TASK_MAX_REASONABLE_RUN_TIME)
        {
            ...
            HSU_ULOG_3(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, task 0x%08X [0x%08X:0x%08X]",
             (uint32) task, (uint32) task->cb, (uint32) task->arg);
            HSU_ULOG_STR_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, from [%s:%u]",
              task->file, task->line);
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "DJOS_SYNC, waiting for %u msec", -(TIME_DIFF(time, task->timeout));
...
jresult_t jmutex_init_d(jmutex_h *mtx_h, char *file, int line)
{
    ...
    rc = hsu_os_mutex_init(&info->os_handle);
    if (rc)
    {
        jfree(info);
        return rc;
...
void jmutex_uninit_d(jmutex_h mtx_h, char *file, int line)
{
    ...
    hsu_os_mutex_uninit(info->os_handle);
    jfree(info);
}
...
void jmutex_lock_d(jmutex_h mtx_h, char *file, int line)
{
    ...
    hsu_os_mutex_lock(info->os_handle);
...
void jmutex_unlock_d(jmutex_h mtx_h, char *file, int line)
{
    ...
    hsu_os_mutex_unlock(info->os_handle);
}
