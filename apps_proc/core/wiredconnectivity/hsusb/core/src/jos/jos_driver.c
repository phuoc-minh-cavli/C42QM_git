...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/core/src/jos/jos_driver.c#1 $
...
void jdrivers_uninit(void)
{
    ...
    for (curr = driver_list.next; curr; curr = next)
    {
        ...
        if (curr->usage_count)
        {
            HSU_ULOG_STR(ERROR_MSG, DBG_E_LOG, "JDRIVER, unloading_driver $s (cont)", curr->name);
            HSU_ULOG_2(ERROR_MSG, DBG_E_LOG, "JDRIVER, type $d, used_by %d device",
              curr->type, curr->usage_count);
        }
    ...
    char *name, jint_t id, juint16_t size)
{
    ...
    if (!name)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "JDRIVER, name_is_null");        
        return JEINVAL;
    }

    if (!methods)
    {
        HSU_ULOG_STR(ERROR_MSG, DBG_E_LOG, "JDRIVER, jdriver_register %s, methods_is_null", name);            
        return JEINVAL;
    }
    
    if (!methods->attach || !methods->detach)
    {
        HSU_ULOG_STR(ERROR_MSG, DBG_E_LOG, "JDRIVER, jdriver_register %s, some_methods_is_null", name);      
        return JEINVAL;
    }

    if (type != DRIVER_CONTROLLER && type != DRIVER_USB)
    {
        HSU_ULOG_STR_1(ERROR_MSG, DBG_E_LOG, "JDRIVER, jdriver_register %s, unknown_driver_type %d", name, type);              
        return JEINVAL;
    }
#endif
...
void jdriver_detach(jdevice_t device)
{
    ...
    if (!device->jdrv)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "JDRIVER, driver_is_null");
        return;
    }
...
jresult_t jdriver_controller_attach(jdevice_t device, jint_t id, jbus_h bus)
{
    ...
    if (!device)
    {
        HSU_ULOG(ERROR_MSG, DBG_E_LOG, "JDRIVER, device_is_null");        
        return JEINVAL;
    }
...
jresult_t jdriver_usb_attach(jdevice_t device)
{
    ...
    if (best->softsize)
    {
        ...
        if (!device->softc)
        {
            HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "JDRIVER, cannot_create_softc_size %u", best->softsize);            
            j_device_set_nameunit(device, "unknown", unknown_unit++);
            return JENOMEM;
        }
    }
    ...
    if (rc)
    {
        HSU_ULOG_STR_1(ERROR_MSG, DBG_E_LOG, "JDRIVER, cannot_attach %s, rc %u",
          best->name, rc);
...
jresult_t jdriver_attach(jint_t type, jdevice_t device, jint_t id, void *arg)
{
    ...
    default:
        rc = JEINVAL;
        HSU_ULOG_1(ERROR_MSG, DBG_E_LOG, "JDRIVER, unknown_dev_type %u",
          type);      
    }
