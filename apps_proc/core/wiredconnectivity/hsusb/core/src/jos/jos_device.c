...
==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/wiredconnectivity/hsusb/core/src/jos/jos_device.c#1 $
...
void j_device_set_nameunit(jdevice_t dev, char *name, jint_t unit)
{
    ...
    DBG_V(DJOS_DEVICE, ("JDEV: Set nameunit of %p [%s] %d\n",
        dev, name, unit));
    KASSERT(((dev != NULL) && (name != NULL)), ("device_set_nameunit(NULL, ...)\n"));

    if (dev->nameunit)
        jfree(dev->nameunit);

    len = j_strlen(name) + MAXINT_LEGTH;
    KASSERT((len < MAXNAMEUNIT_LENGTH), ("device_set_nameunit, name is too long"));
...
void *j_device_get_softc(jdevice_t dev)
{
    ...
    KASSERT(dev && dev->softc, ("device_get_softc(NULL, ...)\n"));
    return dev->softc;
}

void *j_device_get_softc_no_null_check(jdevice_t dev)
{
#ifdef CONFIG_DATA_PATH_DEBUG
    DBG_V(DJOS_DEVICE, ("JDEV: Get softc %p\n", dev));
#endif /* CONFIG_DATA_PATH_DEBUG */
    KASSERT(dev, ("device_get_softc(NULL, ...)\n"));
    return dev->softc;
}
...
/* HSU ADDITION+++  unused function cleanup
