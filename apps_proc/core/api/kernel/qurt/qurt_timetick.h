...
/**@ingroup func_qurt_timetick_user_impl

   System call wrapper to call actual implementation based on the requested ID.

   @param[in] ID		   registered dispatcher ID 
   @param[in] time		   time value to convert. 
   @param[in] time_unit    time units that value is expressed in.
  
   @return 
   FAILURE	 -1 		
   PASS  system tick/time based on requested system call
   
   @dependencies
   None.
   
 */
static qurt_time_t qurt_timetick_user_impl(UINT id, qurt_time_t a, UINT b)
{
