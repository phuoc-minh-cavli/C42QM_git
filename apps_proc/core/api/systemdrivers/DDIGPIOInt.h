...
$Header: //components/rel/core.tx/6.0/api/systemdrivers/DDIGPIOInt.h#1 $
...
/** 
  This macro is used to tell GPIOInt that there is a priority
  flag set.
 */
#define GPIOINT_PRIORITY_FLAG  0x10
#define GPIOINT_PRIORITY_MASK  0xFFFF
#define GPIOINT_PRIORITY_SHIFT 16

/**
  This macro is used to configure the priority of a 
  GPIOInterrupt. The priority setting would be the same as in 
  DALInterruptController. Priority should be uint16. \n
 */
#define GPIOINT_CONFIG_PRIORITY( priority ) \
        ( ( ( priority & GPIOINT_PRIORITY_MASK ) << GPIOINT_PRIORITY_SHIFT ) | GPIOINT_PRIORITY_FLAG ) 

/**
  These macros are used for getting priorities from flags. 
 */

#define GPIOINT_PRIORITY_ISSET( nFlags ) ( nFlags & GPIOINT_PRIORITY_FLAG )

#define GPIOINT_GET_PRIORITY( nFlags ) \
        ( ( nFlags >> GPIOINT_PRIORITY_SHIFT ) & GPIOINT_PRIORITY_MASK )
...
struct GPIOInt
{
   ...
   DALResult (*RegisterIsrEx)(DalDeviceHandle * _h, uint32 gpio, GPIOIntTriggerType  trigger,
...
/**
  ...
  @param[in] nFlags    The Client specified flags to be given to the interrupt controller so that internally it
                       can configure the interrupt priority for direct connect.
...
GPIOINTISR isr,GPIOINTISRCtx param, uint32 nFlags)
{
  if(_h != NULL)
  {
   ...
   { 
     return ((GPIOIntHandle *)_h)->pVtbl->RegisterIsrEx( _h, gpio, trigger,isr,param, nFlags);  
   }
  }
  else
  {
