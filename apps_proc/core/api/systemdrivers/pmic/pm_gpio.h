...
$Header: //components/rel/core.tx/6.0/api/systemdrivers/pmic/pm_gpio.h#1 $
...
* @param[in] pmic_chip Each PMIC device in the systems is enumerated
*                              starting with zero.
* @param[in] gpio GPIO to configure as a digital input. See #pm_gpio_perph_index.
...
/**
 * @brief This function gets the current status of the selected GPIO
 * 
 * @param[in] pmic_device_index. Primary: 0. Secondary: 1
 * @param[in] gpio:  
 *                GPIO number. Refer enum pm_gpio_perph_index.
 * @param[out] pin_status:  
 *                Status of selected pin (LOW or HIGH)
 *
 *
 * @return  pm_err_flag_type 
 *          PM_ERR_FLAG__FEATURE_NOT_SUPPORTED = Feature not available on this
 ...
 *
 */
extern pm_err_flag_type pm_gpio_get_pin_status
(
  uint8 pmic_device_index, 
  pm_gpio_perph_index gpio, 
  pm_pin_status *pin_status
);

/**
*  @brief  Enable/disable GPIO.

* @param[in] pmic_device_index. Primary: 0. Secondary: 1
* @param[in] gpio:  
*                GPIO number. Refer enum pm_gpio_perph_index.
* @param[in] enable TRUE: turn on GPIO, FALSE: turn off GPIO.
 ...
 *
*/
extern pm_err_flag_type pm_gpio_enable
(
   uint8 pmic_device_index,
   pm_gpio_perph_index gpio,
   boolean enable
