...
$Header: //components/rel/core.tx/6.0/api/power/sclk.h#1 $
...
----------   ---   -------------------------------------------------------
06/29/2011   cab   added TDSCDMA client 
...
// Base Station Corrected XO frequency (frequency the XO should always run at, 
// but doesn't due to temperature drift)
#define SCLK_BSCXO_FREQ                           19200000uL 

// XO to SCLK clock frequency divisor
#define SCLK_XO_PER_SCLK                          586uL

// Default slow clock frequency values
// These values are off by 0.5 since 19200000/586 == ~32764.5, 
// but will be corrected in FEE
#define SCLK_DEFAULT_SLOW_CLOCK_NOM_VALUE         ( SCLK_BSCXO_FREQ / SCLK_XO_PER_SCLK )
#define SCLK_EXT_SLOW_CLOCK_NOM_VALUE             SCLK_DEFAULT_SLOW_CLOCK_NOM_VALUE
#define SCLK_TCXO_DIV_SLOW_CLOCK_NOM_VALUE        SCLK_DEFAULT_SLOW_CLOCK_NOM_VALUE
...
FUNCTION SCLK_BSCXO_FROM_SYSCLK

DESCRIPTION
  Determine the number of Base Station Corrected XOs which correspond to the
  given number of sysclks.
  
DEPENDENCIES
  None.

RETURN VALUE
  Base station corrected XOs
...
uint32 sclk_bscxo_from_sysclk
(
  sclk_sys_enum_type              sclk_system,
    /* System conversion is being done for */

  uint32                          duration_sysclks
    /* Duration to be converted into XOs */
);

/* <EJECT> */
/*===========================================================================

FUNCTION SCLK_SYSCLK_FROM_BSCXO

DESCRIPTION
  Determine the number of sysclks which correspond to the given number of
  Base Station Corrected XOs.
...
uint32 sclk_sysclk_from_bscxo
(
  sclk_sys_enum_type              sclk_system,
    /* System conversion is being done for */

  uint32                          duration_bscxo
    /* Duration to be converted into system clocks */
);

/* <EJECT> */
/*===========================================================================

FUNCTION SCLK_XO_FROM_BSCXO

DESCRIPTION
  Determine the number of uncorrected XOs which corresponds to the given
  number of base station corrected XOs.
...
uint32 sclk_xo_from_bscxo
(
  uint32                          duration_bscxo
    /* Duration to be converted into uncorrected XOs */
);

/* <EJECT> */
/*===========================================================================

FUNCTION SCLK_BSCXO_FROM_XO

DESCRIPTION
  Determine the number of base station corrected XOs which corresponds to
  the given number of uncorrected XOs.
...
uint32 sclk_bscxo_from_xo
(
  uint32                          duration_xo
    /* Duration to be converted into base station corrected XOs */
);
