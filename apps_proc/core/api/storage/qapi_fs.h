 ...
 */
 ...
 2022-09-28   sk    Update qapi_FS_Seek arguments to support larger offsets
...
/**
  This QAPI only works for files inside client's moduleid
  directory. The Path being passed must be valid absolute or 
  relative pathname excluding client's moduleid. Otherwise,
  invalid param error is returned.

  Example1 - File path is /xyz123/efsdir/demofile
  Argument should be efsdir/demofile or /efsdir/demofile

  Example2 - File path is /xyz123/dir/../dir2/./demofile
  Argument should be dir/../dir2/./demofile or 
  /dir/../dir2/./demofile

  xyz123 - is client's moduleid.

  Please note that '\\' is not same as '/'. Avoid usage of backward slash.
*/
...
/**
  ...
  Example - Old File path is /xyz123/efsdir/demofile and 
  New File path is /xyz123/efsdir/.././demofile2

  Argument should be old file path - efsdir/demofile or 
  /efsdir/demofile and New file path - efsdir/.././demofile2 
  or /efsdir/.././demofile2
...
#define qapi_FS_Seek(Fd, Offset, Whence, Actual_Offset_Ptr)\
    ...
    (ULONG) Fd,(ULONG)(((qapi_FS_Offset_t)Offset)&0xFFFFFFFF),(ULONG) Whence,\
    (ULONG) Actual_Offset_Ptr,(ULONG)(((qapi_FS_Offset_t)Offset)>>32),\
    (ULONG) 0,(ULONG) 0, (ULONG) 0,(ULONG) 0, (ULONG) 0, (ULONG) 0,(ULONG) 0))

/**
  This QAPI only works for directories inside client's moduleid
  directory. The Path being passed must be valid absolute or 
  relative pathname excluding client's moduleid. Otherwise,
  invalid param error is returned.

  Example1 - Directory path is /xyz123/efsdir/demodir/
  Argument should be efsdir/demodir/ or /efsdir/demodir/

  Example2 - Directory path is /xyz123/dir/../newdir/
  Argument should be dir/../newdir/ or /dir/../newdir/
...
/**
  This QAPI only works for handle associated with directories inside
  client's moduleid directory.
*/
 ...
 *            This QAPI when called from userspace only works for directories
 *            inside client's moduleid directory.
 */
 ...
 *            the parameters must be user space addresses, otherwise
 *            an invalid parameter error will be returned.
 *            This QAPI when called from userspace only works for handle
 *            associated with directories inside client's moduleid directory.
 */
 ...
 *  @return  Returns QAPI_OK on success and -ve error code is returned on failure.
 *
 * @note1hang This QAPI when called from userspace only works for handle
 *            associated with directories inside client's moduleid directory.
 */
