...
/*==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/api/wiredconnectivity/hsusb/uw_device_cdc_ecm.h#1 $
...
typedef jresult_t (*ecm_set_control_line_state_cb)(juint16_t ctrl_sig_bitmap,
                                                   cdc_appctx_t ctx);
/* End of HSU fix*/
/* FEATURE_HS_USB_MBIM Support */
/* Callbacks for passing the data to QBI Layer */
typedef void (*ecm_set_ntb_input_size_cb)(cdc_handle_t handle,juint32_t ntb_input_size,juint16_t max_datagram_in_ntb);
typedef void (*ecm_handle_reset_function)(cdc_handle_t handle);
typedef boolean (*ecm_is_mbim_callback_registered)(void);
...
/* End of HSU fix */
    ...
    cdc_io_complete_cb          notify_speed_change_complete_h; 
      /**< ECM notification completion handler. */
/* FEATURE_HS_USB_MBIM Support */
    ecm_handle_reset_function 	handle_reset_function_h;
    /**< ECM notification once RESET_FUNCTION is received */
	
    ecm_set_ntb_input_size_cb	set_ntb_input_size_h;
    /**< ECM notification once SetNtbSize Command is received  */

    ecm_is_mbim_callback_registered get_mbim_cb_status_h;
    /**<ECM has to check if MBIM is registered or not */
	
/* End of FEATURE_HS_USB_MBIM Support*/
   
} ecm_callbacks_t;

/*FEATURE_HS_USB_MBIM Support to handle callbacks from QBI*/
void set_ntb_input_size_complete(cdc_handle_t handle,jresult_t status);
void handle_reset_function_complete(cdc_handle_t handle,jresult_t status);
