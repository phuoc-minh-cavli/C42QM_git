...
/*==============================================================================
  ...
  $Header: //components/rel/core.tx/6.0/api/wiredconnectivity/hsusb/cdc_fd.h#1 $
...
#define USB_CDC_SUBTYPE_MBIM            0x1B
#define USB_CDC_SUBTYPE_MBIM_EXT        0x1C

#define USB_CDC_SUBCLASS_NCM            0x0D
#define USB_CDC_SUBCLASS_MBIM           0x0E

#define USB_CDC_PROTOCOL_MBIM           0x00
#define USB_CDC_PROTOCOL_MBIM_NTB       0x02
...
__PACKED_PRE__
struct cdc_mbb_fdesc
{
    juint8_t bLength;
    juint8_t bType;
    juint8_t bSubtype;
    juint16_t wMbbVersion;
    juint16_t wMaxControlMessage;
    juint8_t bNumberPowerFilters;
    juint8_t bMaxFilterSize;
    juint16_t wMaxSegmentSize;
    juint8_t bmNetworkCapabilities;
} __PACKED_POST__;

__PACKED_PRE__
struct cdc_mbb_extended_fdesc
{
    juint8_t bLength;
    juint8_t bType;
    juint8_t bSubtype;
    juint16_t wMbbExtVersion;
    juint8_t bMaxOutstandingCmdMsgs;
    juint16_t wMTU;
} __PACKED_POST__;
...
typedef struct _cdc_ctx_t
{
	...
	*/
    struct
    {
        ...
        /* HSU Addition +++
         * Used to add sequence number at the end of NOTIFY_RESPONSE_AVAILABLE
         * when FEATURE_HS_USB_ECM_CONTROL_SEQ_NUM is defined.  The sequence
         * number is used to detect the control packet loss from host
         */
        juint32_t seq_num;
#ifdef FEATURE_HS_USB_VERIFY_RX_TX_ZLP_COMPLETION
        enum
        {
          CDC_ZLP_CLIENT_UNKNOWN=0,
          CDC_ZLP_CLIENT_SEND_ENCAP_CMD,
          CDC_ZLP_CLIENT_GET_ENCAP_RESP,
        } zlp_client;
...
typedef struct
{
    ...
    jbool_t cc_ntf_supported;            /**< Control-if notification pipe 
                                              support. */
    jbool_t dc_in_supported;                /**< In Data-if support. */
    jbool_t dc_out_supported; 			   /**< Out Data-if support. */
    ...
    jbool_t is_hw_accelerated;           // New flag to indicate pipe is HW accelerated
} cdc_desc_init_t;
