...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/api/services/qapi_diag.h#1 $
...
02/06/18   gn      Exclusive control of DIAG_MSG_DISABLE_LVL_MASK in qapi_diag.h for User modules
01/22/18   gn      Moved FEATURE_MMU_ENABLED from target.builds to qapi_diag.h
				   as target files will not be shipped as part of SDK
...
04/21/17   leo     (Tech Comm) Edited/added Doxygen comments and markup.
...
extern uint32_t diag_user_space_dispatcher(uint32_t cb_id, void *app_cb, uint32_t cb_param);
#endif
...
/** 
   ...
   Returns QAPI status; see @xhyperref{hdr:QAPIStatust,QAPI Status Codes}.
@hideinitializer
*/
#define QAPI_DIAGPKT_DISPATCH_TABLE_REGISTER(xx_subsysid, xx_entry, inbuf, inbuf_len, outbuf, outbuf_len) \
   do { \
	 ...
	 inbuf, inbuf_len, outbuf, outbuf_len, 0 \
	 }; \
...
#ifdef DIAG_MSG_DISABLE_LVL_MASK
  #undef DIAG_MSG_DISABLE_LVL_MASK
...
#define qapi_user_space_tbl_reg_append_proc(tbl_ptr, user_space_tbl_ptr) \
		((qapi_Status_t) (_txm_module_system_call4) (TXM_QAPI_DIAG_DISPATCH_TABLE_REGISTER, (uint32_t)tbl_ptr, (uint32_t)user_space_tbl_ptr, (uint32_t)diag_user_space_dispatcher, (uint32_t) 0))
#else
#define qapi_user_space_tbl_reg_append_proc(tbl_ptr, user_space_tbl_ptr) \
...
/**  
   QAPI to deregister a user table with Diag. Provide the user_table pointer as an argument. 
*/
#define qapi_user_space_tbl_dereg(tbl_ptr) \
		((qapi_Status_t) (_txm_module_system_call4) (TXM_QAPI_DIAG_DISPATCH_TABLE_DEREGISTER, (uint32_t)tbl_ptr, (uint32_t)0, (uint32_t)0, (uint32_t) 0))
...
/**  
    ...
    @param[in] tbl_ptr             Address of the user table that is to be deregistered
								   with Diag.
