...
/*
Copyright (c) 1992-2014, 2017-2018 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/api/services/diagpkt.h#1 $
...
07/23/14   rh      Updated from 8kB commands to 16kB commands
06/03/14   rh      Updated to support 8kB commands
...
*/ 
  typedef struct
  {
    ...
    PACK(void *) (*func_ptr) (PACK(void *) req_pkt_ptr, uint16 pkt_len); /*!< Pointer to Function to handle the packet  
                                                                          ...
                                                                          cmd_code_hi  */
    uint32 (*user_func_ptr) (uint32 pkt_len); /*!< Pointer to user space client's handler when command code is in the range 
  ...
  /*! 
@ingroup packet_service
   An array of diagpkt_slave_user_table_entry_type is created by the remote client and registered with 
   diagnostics packet service. It must be declared 'const' (preferrably 'static const').  
   The function is called when an inbound packet matches subsystem ID and
   is within the command code range specified in the table.
*/ 
  ...
  diagpkt_slave_user_table_entry_type;
...
*/
  typedef struct
  { 
	uint32 *inbuf;     /*!< User space client's input buffer address */
	uint32 inbuf_len;  /*!< User space client's input buffer length */
	uint32 *outbuf;    /*!< User space client's output buffer address */
	uint32 outbuf_len; /*!< User space client's output buffer length */
  	uint32 (*user_func_ptr) (uint32 pkt_len); /*!< User space client's mapped function pointer 
	                                               provided by call back manager */
...
@ingroup packet_service
   ...
   @details
   This function is called by QAPI_DIAGPKT_DISPATCH_TABLE_REGISTER internally, registers the
   user's dispatch table with the packet service

   @param[in] tbl_ptr             Pointer to the diagpkt_master_table_type
   @param[in] user_space_tbl_ptr  Pointer to the diagpkt_user_space_table_type
   ...
   None 
*/
  void diagpkt_user_space_tbl_reg_append_proc (diagpkt_master_table_type * tbl_ptr, diagpkt_user_space_table_type * user_space_tbl_ptr);

/*! 
@ingroup packet_service

   @brief
   This function is called to deregsiter user space client's dispatch table with the packet service 

   @details
   This function deregisters the user's dispatch table from Diag's master table

   @param[in] tbl_ptr             Pointer to the diagpkt_user_table_entry_type
...
#define DIAG_MAX_RX_PKT_SIZ (16384) /* To support long command packets */
#endif
/*@}*/ /* start grouping of Packet Size Definitions */
...
@ingroup packet_service
   ...
   @brief
   Allocates the specified number of bytes in AMSS heap
