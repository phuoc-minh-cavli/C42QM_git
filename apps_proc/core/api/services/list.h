...
$Header: //components/rel/core.tx/6.0/api/services/list.h#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
typedef struct {
   ...
   qurt_mutex_t  pkcsListCriticalSection;
...
DESCRIPTION
====================================================================== */
#define LIST_LOCK_INIT(x) qurt_mutex_init(&(x->pkcsListCriticalSection))
#define LIST_LOCK(x) qurt_mutex_lock(&(x->pkcsListCriticalSection))
#define LIST_FREE(x) qurt_mutex_unlock(&(x->pkcsListCriticalSection))
