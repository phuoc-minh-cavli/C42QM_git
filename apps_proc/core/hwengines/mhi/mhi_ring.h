...
/* Ring priorities */
#define RING_PRIORITY_MAX 0 
#define RING_PRIORITY_CONTROL 1 
#define RING_PRIORITY_REST 2
#define RING_PRIORITY_MIN 0xFF
...
/* Debug counter tracking ring state */
typedef struct _ring_state_log
{
  uint64 count;
}ring_state_log_t;
...
typedef struct _ring
{
   ...
   list_t             linkReqWaitAckQ;     /**< Link requests waiting for ack (reliability) */

   rel_process_func   rel_process;         /**< Ring element processing function */ 
   boolean            interruptable;       /**< If ring is interruptable or not */
   boolean            reliable;            /**< If ring is realiable */

   ring_state_log_t   ring_state_log[RING_MAX]; /**< Array of debug counters tracking ring state */
   uint64             issued_tres_count;   /**< Counter tracking the number of TREs issued to the link for this ring */
}ring_t;
