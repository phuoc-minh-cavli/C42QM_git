...
typedef struct _ch
{
   ...
   uint32  total_payload_size;      /**< Total size of payload being processed */ 

   /* Processing state */
   boolean stalled;                 /**< Indicates if channel is in stalled state where host is not providing buffers. Used 
                                         only on inbound channels */
   uint32  wakeCount;               /**< Indicates amount of times AP was woken up due to a stalled channe  */

   /* Relability */
   boolean reliable;                /**< Channel is reliable or not */
   uint32  seq_host;                /**< Host Sequence number (either ack_host for outbound or seq_host for inbound) */
   uint32  ack_device;              /**< Ack Device number */
   uint32  seq_device;              /**< Device sequence number */
...
typedef struct _mhi_link_async_req
{
  ...
  mhi_op_t  op;           /**< Operation */
  uint32 size;            /**< Size of link operation in bytes */ 
  uint32 status;          /**< Status of operation */
  uint64 time_issued_ms;  /**< Timestamp in ms when the link request was issued */
  ...
  uint32 priority;        /**< link request priority */
...
typedef struct _mhi_ctx
{
  ...
  boolean            boot_enabled;      /**< TRUE if MHI was used in boot chain */
  boolean            is_syserr_pending; /**< Has SYSERR processing been deferred */
  ...
  /* Memory pools */
  mhi_osal_meminfo pool_uncached;               /**  Uncached pool used for infrequently used data structures that are shared with
                                          host such as * contexts */
  ...
  list_t waitAckRingQ;              /**< Queue of rings waiting on host ack  */  

  /* Reliability  */
  uint32 reliableEr[4];             /**< Bit mask of event rings which are realiable */ 
  ...
  volatile int32        num_completions_pending; /**< Number of issued link requests pending completions */
  ...
  /* Target info */
  const mhi_core_tgt_config_t *mhi_core_tgt_config; /**< MHI target specific configuration */
}mhi_ctx_t; 
 ...
 * @brief      mhi_core_isr
 *
 *             Handle mhi core interrupts 
 *              
 * @param[in]  mhiCtx - Pointer to MHI Context
 *
 * @dependencies
 *             None
 *              
