...
{
   if(mhiCtx.enumerated)
   {
      if(mhiCtx.boot_enabled)
         return MHI_BOOT_ENUMERATED;
      else
         return MHI_AMSS_ENUMERATED;
   }
   else
   {
      return MHI_DISABLED;
...
mhi_status_t mhi_init(void)
{
   ...
   /* Get platform specific info for core */
   mhiCtx.mhi_core_tgt_config=mhi_get_core_tgt_info();
   if (NULL == mhiCtx.mhi_core_tgt_config)
   {
      mhi_osal_debug(MSG_ERR, "mhi_init: Failed to retrieve target info"); 
      goto err; 
   }

   /* Initialize the MHI Sync object and set the MHI version we support */ 
   mhiCtx.mhiConfig.version = mhiCtx.mhi_core_tgt_config->mhiversion;
   ...
   /* Initialize MMIO */ 
   if(MHI_SUCCESS != mhi_mmio_init(mhiCtx.mhi_core_tgt_config->mmio_basephys, &mhiCtx.mhiConfig))
   {
...
mhi_status_t mhi_channel_open(uint32 chId, mhi_cb_t *cb)
{
   ...
   if(MHI_STATE_READY == mhi_sm_get_mhistate())
   {
      ...
      if(mhi_q_dequeue(&mhiCtx.ch[chId].request_free_q,(list_t**)&request))
      {
         mhi_osal_syncleave(&mhiCtx.mhiSync);
         ret=MHI_ERROR; 
         goto end; 
...
mhi_status_t mhi_channel_close(uint32 chId)
{
   ...
   if(!mhi_q_is_queue_empty(&ch->request_q_issued))
   {
      ...
      /* Leave sync and wait for the event */
      mhi_osal_syncleave(&ch->chSync); 
...
mhi_status_t mhi_channel_rw_multiple(uint32 chId, void** data_array, uint32* size_array, uint32 nElements, uint32 *nSubmitted, uint32* bytesrw, mhi_cb_t *cb, mhi_op_t op) 
{
   ...
   /* Consider a case where large payloads are sent back to back. If the total payload (nElements) is
    * greater than or equal to the channel request Q size - 1, request last_op may never be set in 
    * the pending requests Q, resulting in a MHI data stall. */
   if(nElements >= MAX_REQUESTS_PER_CH-1)
   {
      mhi_osal_debug(MSG_FATAL, "mhi_channel_rw_multiple: Payload cannot be bigger than channel request queue size"); 
      return MHI_INVALID_PARAM; 
   }
   ...
   *nSubmitted=0;  
   ...
   mhi_osal_debug(PROFILING_LOG|MSG_HIGH,"mhi_channel_rw_multiple: core_packet=start, ch_id=%d, req_userdata=0x%x",chId,request->cb.userData);
