...
void mhi_mmio_reset(void)
{
   ...
   MMIO_OUTF(MHICFG, NHWER, HWEVENTRINGNUM);
   MMIO_OUTF(MHICFG, NHWCH, HWCHANNELNUM);
...
mhi_status_t mhi_mmio_init(uint32 base, mhi_config_t *mhiConfig)
{
   ...
   mhi_osal_mallocmemregion(&mmio_mem, mmio_mem.size, MHI_DEVICE_MAPPING);
   ...
   mhiConfig->nHwEr = MMIO_INF(MHICFG, NHWER);
   mhiConfig->nHwCh = MMIO_INF(MHICFG, NHWCH);
...
/** 
 * @brief      mhi_mmio_is_mhictrlint_pending
 *
 *             Check if there are any pending control interrupts
 *              
 *
 *
 * @dependencies
 ...
 * @sideeffect None 
 */
boolean mhi_mmio_is_mhictrlint_pending(void)
{
   return (0 != MMIO_IN(MHI_MMIO_CTRL_INT_STATUS_A7));
}
...
/** 
 * @brief      mhi_mmio_get_mhiconfig
 *
 *             Get mhi configuration
 *              
 * 
 * @param[out]  *mhiConfig - Mhi Configuration
 *
 * @dependencies
 *             None
 *              
 ...
 * @sideeffect None 
 */
void mhi_mmio_get_mhiconfig(mhi_config_t *mhiConfig)
{   
   mhiConfig->mhiRegLen = MMIO_IN(MHIREGLEN);
   mhiConfig->nEr = MMIO_INF(MHICFG, NER);
   mhiConfig->nCh = SWCHEND;
...
/** 
 * @brief      mhi_mmio_clear_chdb_interrupt
 *
 *             Clear a pending chdb interrupt
 *              
 * 
 * @param[in]  chdb - Channel doorbell number
 *
 * @dependencies
 *             None
 *              
 ...
 * @sideeffect None 
 */
void mhi_mmio_clear_chdb_interrupt(uint32 chdb)
{
   MMIO_OUT(MHI_DB_INT_CLEAR_A7_n(chdb/32),1<<(chdb%32));
}
