...
===============================================================================
 ...
 $Header: //components/rel/core.tx/6.0/hwengines/bam/bam.c#2 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
11/17/16   PC      Removed Disabling 'bUsesDsr' in bam_init, if already bam init is done(CR1090645)
10/27/16   PC      Avoid mutex calls for QPIC BAM(CR 1081944)
04/12/16   SS      Optimized code for secure config
01/15/15   SS      Fixed BAM pipe empty toggle issue.
...
static bam_status_type bam_validate_pipe_cfg(bam_dev *bam, uint32 pipe_num, bam_pipe_config_type *pipe_cfg)
{
    ...
    if (pipe_cfg->mode == BAM_MODE_BAM2BAM)
    {
        ...
        if ( pipe_cfg->desc_base_pa == 0 || pipe_cfg->data_base_pa == 0 )
        {
            return BAM_FAILED;
        }
    }
...
BAM_API_NON_PAGED void bam_pipe_eot_handler(pipe_dev *pipe, bam_result_type *pollresult)
{
    ...
    bam_osal_debug(MSG_ERR, "bam_pipe_eot_handler entry - %d\n",pipe->pipe_num);
#endif
    ...
    if(!(pipe->bam->bam_tgt_cfg->options & BAM_TGT_CFG_USE_NO_SYNC_LOCK))
    {
...
void bam_pipe_isr(bam_dev *bam, uint32 pipe_num)
{
...
#ifdef FEATURE_BAM_EXTRA_LOGGING
    bam_osal_debug(MSG_ERR, "bam_pipe_isr entry - %d\n",pipe_num);
#endif
    ...
    if(!(bam->bam_tgt_cfg->options & BAM_TGT_CFG_USE_NO_SYNC_LOCK))
    {
        bam_osal_syncenter(&pipe->sync);
    }
    ...
    if (bam->bUsesDsr)
    {
       // If BAM uses DSR we use the saved interrupt status (since we cleared
       // the interrupt in ISR).
       status = pipe->saved_intr_status;
       pipe->saved_intr_status = 0x0;
    }
    else
    {
      // Check the pipe IRQ Status register
    ...
    if (bam->bUsesDsr)
    {
       // For level-triggered BAMs we use the saved interrupt mask (since we cleared the pipe mask in ISR).
       mask = pipe->saved_irq_mask;
    }
    else
    {
 ...
 bam_pipe_isr_enable_irq:
    if (bam->bUsesDsr)
    {
       bamcore_pipe_setirqmask(bam->bam_cfg.bam_va, pipe_num, pipe->saved_irq_mask);
    }
...
void bamIsrDsr(dsr_obj_t *pDsr, void *ctxt, void *pPayload)
{
    ...
    {
       //
       // Since this bam uses DSR we cleared the pending
       // interrupt for this BAM in ISR. The IRQ_SRCs register gets cleared.
       // So we need this loop to know which pipe(s) have raised interrupt.
       //
       irq_src = 0;
       pipe_num = 0;
       pipe_mask = 0x1;
       while (active_pipes_mask && (pipe_num < bam->bam_caps.num_pipes))
       {
         if (active_pipes_mask & 0x1)
         {
           pipe = bam->pipes[pipe_num];
           ...
           if (pipe->intr_disabled)
           {
              irq_src |= pipe_mask;
              pipe->intr_disabled = 0x0;
           }
    ...
    while(irq_src != 0)
    {
        ...
        //irq_src = bamcore_get_irq_sources(bam->bam_cfg.bam_va,bam->bam_tgt_cfg->ee,active_pipes_mask);
...
void bam_isr(void *pCtxt)
{
   bam_dev *bam = (bam_dev *)pCtxt;
    uint32 pipe_num = 0;
    pipe_dev* pipe;
    uint32 irq_src;

   if (bam->bUsesDsr == TRUE)
   {
      // Clear the interrupt and queue DSR

      irq_src = bamcore_get_irq_sources(bam->bam_cfg.bam_va,
            bam->bam_tgt_cfg->ee,
            bam->active_pipes_mask);

       while (irq_src) {
           if (irq_src & 1){
               pipe = bam->pipes[pipe_num];
               pipe->saved_irq_mask = bamcore_pipe_getirqmask(bam->bam_cfg.bam_va, pipe_num);
               pipe->saved_intr_status = bamcore_pipe_getandclearirqstatus(bam->bam_cfg.bam_va, pipe_num);  
               bamcore_pipe_setirqmask(bam->bam_cfg.bam_va, pipe_num, 0);
            pipe->intr_disabled = 0x1;
           }
           pipe_num++;
           irq_src >>= 1;
       }

      dsr_enqueue(bam->pDsr, NULL);
   }
   else
   {
      // Handle interrupt here
      bamIsrDsr(NULL, pCtxt, NULL);
   }
   //boolean bIsrPending = FALSE;

   //AdcLock();

   //if (pDevCtxt->bDsrInProgress)
   //{
   //   AdcUnlock();
   //   return;
   //}

   //pDevCtxt->bDsrInProgress = TRUE;

   //for ( ; ; )
   //{
   //   INTLOCK();
   //   bIsrPending = pDevCtxt->bIsrServicePending;
   //   pDevCtxt->bIsrServicePending = FALSE;
   //   INTFREE();

   //   if (!bIsrPending || pDevCtxt->pBufferingHandle == NULL)
   //   {
   //      break;
   //   }

   //   AdcDsrHandleDma(pDevCtxt);

   //   AdcDsrHandleFifoThresh(pDevCtxt);

   //   AdcDsrHandleAmpThresh(pDevCtxt);
   //}

   //pDevCtxt->bDsrInProgress = FALSE;
...
BAM_API_NON_PAGED bam_handle bam_init(bam_config_type *bam_cfg, bam_callback_type *bam_cb)
{
    ...
    if (install_isr && (BAM_OPT_SET(tgt_cfg->options, BAM_TGT_CFG_USE_DSR)))
    {
        bDsrResult = dsr_create(&bam->pDsr,
                                bamIsrDsr,
                                (void *)bam,
                                DSR_PRI_HIGH,
                                DSR_CONTEXT_ISR);
...
bam_handle bam_pipe_init(bam_handle bamhandle, uint32 pipe_num, bam_pipe_config_type *pipe_cfg, bam_callback_type *pipe_cb)
{
    ...
    if(status == BAM_SUCCESS)
    {
        ...
        // Check if BAM is configured to not use Sync locks
        if(!(bam->bam_tgt_cfg->options & BAM_TGT_CFG_USE_NO_SYNC_LOCK))
        {
...
bam_status_type bam_pipe_deinit(bam_handle pipehandle)
{
    ...
    if (bam_pipe_cfg_uses_pipemem(&pipe->pipe_cfg) && bam_osal_clock_ctl_disable(&pipe_mem_clock_ctl) != BAM_SUCCESS)
    {
...
BAM_API_NON_PAGED bam_status_type bami_pipe_transfer_no_lock(pipe_dev* pipe, bam_paddr buf_pa, uint16 buf_size, uint16 xfer_opts, void* user_data)
{
    ...
    *((bam_iovec_type *) (pipe->pipe_cfg.desc_base_va + pipe->desc_offset)) = iovec;
    ...
    {
        // Ideally we should operate with uncached memory. But some systems may not provision uncached memory
    ...
    else
    {
        ...
        else
        {
            ...
            if ( prod_next_offset == producer_sw_offset )
            {
                ...
                if(bamcore_pipe_verifyzlt(producer_bam,producer_pipe))
                {
                    if((bamcore_pipe_desc_published(consumer_bam, consumer_pipe)) && (bamcore_pipe_verifypipeempty(producer_bam,producer_pipe)))
                    {
...
/**
 * @brief bam_pipe_flush
 *
 * This function will flush out any pending descriptor on 
 * pipe.This function wont issue any callback 
 *
 * @param pipehandle[in]      Pointer to client's handle to the BAM Pipe. 
 *                              
 *
 * @return                   Status of the operation.
 *
 * @note  If pipe is used in callback,this is harmful operation.
 */
#define BAM_FLUSH_TIMEOUT 200
#define BAM_FLUSH_TIMEOUT_STEP 10
bam_status_type bam_pipe_flush(bam_handle pipehandle)
{
    pipe_dev *pipe = (pipe_dev *)pipehandle;
    uint32 endoffset, cnt_index;
    bam_status_type status = BAM_SUCCESS;
    ...
    for (cnt_index = 0; cnt_index < BAM_FLUSH_TIMEOUT; cnt_index++)
    {
        endoffset = bamcore_pipe_getdescreadoffset(pipe->bam->bam_cfg.bam_va, pipe->pipe_num);
        if (pipe->desc_offset == endoffset)
        {
            pipe->acked_offset = endoffset;
            pipe->size_since_eot = 0;
            break;
        }
        busywait(BAM_FLUSH_TIMEOUT_STEP);
    }
    if (cnt_index == BAM_FLUSH_TIMEOUT) 
        status = BAM_FAILED;
...
/* sumbit the current WP to the HW */       
 bam_status_type bam_pipe_submit_all(bam_handle pipehandle)     
 {      
         pipe_dev *pipe = (pipe_dev *)pipehandle;       
         ...
         if ( pipe->desc_offset == bamcore_pipe_getdescwriteoffset(pipe->bam->bam_cfg.bam_va,pipe->pipe_num))       
         {      
             if(!(pipe->bam->bam_tgt_cfg->options & BAM_TGT_CFG_USE_NO_SYNC_LOCK))
             {
                 bam_osal_syncleave(&pipe->sync);
         ...
         if(NULL == pipe->pipe_desc_mem.handle)     
         {      
                 bam_osal_cachecmd(BAM_CACHE_FLUSH,pipe->pipe_cfg.desc_base_va,pipe->pipe_cfg.desc_size);       
         }

         bam_osal_memorybarrier();      
         bamcore_pipe_setdescwriteoffset(pipe->bam->bam_cfg.bam_va, pipe->pipe_num,pipe->desc_offset, 0);       
         bam_osal_memorybarrier();
