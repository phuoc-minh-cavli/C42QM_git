...
--------  ---  ------------------------------------------------------------
01/24/17  ska  KW issues fixed
10/18/16  ska  Added the split for ISR and DSR 
...
static int32  build_descriptor_single(SpiDeviceInfoType            *qup_config,
                                      ...
                                      uint32                        write_length,
                                      SpiDevice_TransferDescriptor *descriptor);
static void   cancel_transfer(SpiDevice_DeviceCtxt *ctxt);
...
static int32 SpiDevice_SetRunState(uint8 *pQupAddr, 
...
DESCRIPTION:
   Cancel a block mode or BAM mode transfer.
...
static void cancel_transfer(SpiDevice_DeviceCtxt *ctxt)
{
   ...
   // Cancel any existing BAM transfers.  If the current transaction is block mode
   // this operation is not necessary but it should be harmless.
                             ...
                             SpiDevice_TransferDescriptor *descriptor, boolean eot)
{
   ...
   if (descriptor->bam_mode && ctxt->device_cfg.UseCmdDesc)
   {
      ...
      res = bam_handle->setQupDeviceParms  (bam_handle, &hal_device_parameters);
	  if (res != SPIDEVICE_RES_SUCCESS)
      {
         SPIDEVICE_LOG(ERROR, "start_transfer: setQupDeviceParms failed (0x%lx)", res);
         return res;
      }
      res = bam_handle->setQupTransferParms(bam_handle, &hal_transfer_parameters);
	  if (res != SPIDEVICE_RES_SUCCESS)
      {
         SPIDEVICE_LOG(ERROR, "start_transfer: setQupTransferParms failed (0x%lx)", res);
         return res;
      }
   }
   else
...
{ 
   ...
   SpiDeviceTransferType       *transferInfo = &(pDev->transfer_info);
   ...
   */
   while(pQupInfo->uOutFreeSlotReqCnt > pQupInfo->uOutFreeSlotServicedCnt)
   {
      ...
      if(SPIDEVICE_TRANSFER_GET_DATA_NONE != res)
      {
         ...
         HAL_qupe_PutWordIntoOutFifo(pDev->device_cfg.QupVirtAddr, writeData);
         pQupInfo->uOutFreeSlotServicedCnt++;
         if(SPIDEVICE_TRANSFER_GET_DATA_DONE == res)
         {
         	   pDev->transfer_info.bWriteBufferEmpty = TRUE;
         }
      } 
      else if(pQupInfo->uOutFreeSlotReqCnt - pQupInfo->uOutFreeSlotServicedCnt 
      	   < pDev->hw_info.uNumOutputBlockSlots)
      {
      ...
      {
        /*We should Error Fatal here or abort tranfer because we're provided with 
...
FUNCTION: QUP_PerformInputOutput
...
static uint32 QUP_PerformInputOutput(SpiDevice_DeviceCtxt *pDev)
{
   ...
   {
      res = SpiDevice_SetRunState(pDev->device_cfg.QupVirtAddr, 
      ...
      signal_completion(pDev,SPIDEVICE_RES_SUCCESS);
   }
   return TxRxCount;
}
/*==================================================================================================

FUNCTION: QUP_ServiceQUP
...
static uint32 QUP_ServiceQUP(SpiDevice_DeviceCtxt *pDev)
{
   ...
   pQupInfo->uQupErrors   = HAL_qupe_GetErrStatusFlags(pDev->device_cfg.QupVirtAddr);
   uOperationalFlags      = HAL_qupe_GetOpStatusFlags(pDev->device_cfg.QupVirtAddr);
   ...
   if(pQupInfo->uQupErrors & HAL_QUPE_ERRSTATE_FIELD)
   {
      ...
      HAL_qupe_Reset(pDev->device_cfg.QupVirtAddr);
	  wait_for_run_state_valid(pDev->device_cfg.QupVirtAddr);
   ...
   if((uOperationalFlags & HAL_QUPE_OPSTATE_OUTPUT_SERVICE))  
   {
	  ...
	  {
         pQupInfo->uOutFreeSlotReqCnt += pDev->hw_info.uNumOutputBlockSlots;
      }

      if(pQupInfo->uOperational & HAL_QUPE_OPSTATE_MAX_OUTPUT_DONE)
	  {
   ...
   if((uOperationalFlags & HAL_QUPE_OPSTATE_INPUT_SERVICE))  
   {
	  ...
	  {
         pQupInfo->uInFreeSlotReqCnt += pDev->hw_info.uNumInputBlockSlots;
      }
   ...
   HAL_qupe_ClearOpStatusFlags(pDev->device_cfg.QupVirtAddr, uSetServiceFlags);
   return (pQupInfo->uOutFreeSlotReqCnt + pQupInfo->uInFreeSlotReqCnt);
}
...
uint32 SpiDevice_Init(SpiDevice_Config *pConfig, SPIDEVICE_HANDLE *phDev)
{
   ...
   *phDev = NULL;  // in case of error below
   ...
   if(IBUSOS_Memcpy(&ctxt->device_cfg, sizeof(ctxt->device_cfg),pConfig, sizeof(SpiDevice_Config)) != SPIDEVICE_RES_SUCCESS)
   {
      return SPIDEVICE_ERR_MEM_CPY_FAILED;
   }
   ...
   IBUSPLAT_SetPowerState(ctxt->plat_handle, SPIDEVICE_POWER_STATE_0);  // turn off the clocks
...
FUNCTION: SpiDevice_SetGpioState

DESCRIPTION:
   This function sets the GPIO state for the device gpios.

ARGUMENTS PASSED:
   hDev        - device handle
   eGpioState - new GPIO state

RETURNS:
   status - SPIDEVICE_RES_SUCCESS on success, error code on error

==================================================================================================*/
uint32 SpiDevice_SetGpioState(SPIDEVICE_HANDLE hDev, SpiDevice_GpioStates eGpioState)
{
   SpiDevice_DeviceCtxt *ctxt = (SpiDevice_DeviceCtxt *)hDev;

   return (int32)IBUSPLAT_SetGpioState(ctxt->plat_handle, eGpioState);
}


/*==================================================================================================

FUNCTION: SpiDevice_SetPNOCVote
DESCRIPTION:
   This function Votes/devote the PCNOC 
ARGUMENTS PASSED:
   hDev        - device handle
   ePnocVote   - Vote state ADD/REMOVE
RETURNS:
   status - SPIDEVICE_RES_SUCCESS on success, error code on error
==================================================================================================*/
uint32 SpiDevice_SetPNOCVote(SPIDEVICE_HANDLE hDev, SpiDevice_PnocVotes ePnocVote)
{
   SpiDevice_DeviceCtxt *ctxt = (SpiDevice_DeviceCtxt *)hDev;
   return IBUSPLAT_SetPNOCVote(ctxt->plat_handle, ePnocVote);
}
/*==================================================================================================
...
/*==================================================================================================
FUNCTION: SpiDevice_SetRunState

DESCRIPTION:
   Gets the QUP to desired state

ARGUMENTS PASSED:
   pQupAddr    - QUP virtual base address
   cRunState   - HAL_qupe_RunStateType state type
...
uint32 SpiDevice_SetCSControl(SPIDEVICE_HANDLE hDev, uint32 uChipSelect, SpiCSControl eControl)
{
   ...
   switch (eControl)
   {
      ...
      default: return(SPIDEVICE_ERR_INVALID_DEV_PARAMETERS);
   }
