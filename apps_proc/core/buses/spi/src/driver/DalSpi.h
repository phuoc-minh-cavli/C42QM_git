...
typedef struct
{
   ...
   qapi_SPIM_Callback_Fn_t pCallback_fn;
...
struct SpiDevCtxt
{
   ...
   /* Mutex lock to synchronize multiple clients using the same device */   
   qurt_mutex_t  hDeviceSynchronization; 

   /* Workloop DSP handle */
   dsr_obj_t *pStartTransferWorkLoop;
...
DALResult Spi_StartTransferAsync( SpiClientCtxt *, SpiTransferType *, SpiDataAddrType *, int , SpiDataAddrType * , int , int * , uint32 , uint32 * , uint32 ,void * ,void *);
...
DALResult Spi_SendAndReceiveAsync(SpiClientCtxt *, SpiDataAddrType *, int ,uint32 ,SpiDataAddrType *, int , int *, uint32 ,void * ,void *);
