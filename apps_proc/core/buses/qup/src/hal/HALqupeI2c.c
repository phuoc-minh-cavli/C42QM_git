...
-------- ---    -----------------------------------------------------------
02/03/14 SB     Add add'l check for invalid HS divider value.
...
#define HAL_QUPE_MAX_HS_DIV_VAL                         7
...
)
{
   ...
   uNoiseRejection = (uI2cBusSDANoiseRejection << QUPE_SHFT(I2C_MASTER_CLK_CTL, SDA_NOISE_REJECTION)) |
                     (uI2cBusSCLNoiseRejection << QUPE_SHFT(I2C_MASTER_CLK_CTL, SCL_NOISE_REJECTION)); 
   ...
   if ( uI2cBusClockReqKhz <= HAL_QUPE_FS_FREQ_KHZ_MAX ) {
      ...
      if ( (uHsDivider < HAL_QUPE_MIN_HS_DIV_VAL) || (uHsDivider > HAL_QUPE_MAX_HS_DIV_VAL) ) {
