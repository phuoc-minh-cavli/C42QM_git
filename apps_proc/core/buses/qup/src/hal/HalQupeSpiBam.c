...
#include "HalQupeSpiBamSys.h"

#define HAL_QUPE_SPI_BAM_LOG
#include "QupLog.h"
...
#define HAL_QUPE_BAM_FILL_CE_BY_FIELD(QUP_ADDR,CE_BASE, INDEX,CMD,REG_NAME,FIELD, DATA)   \
        BAM_FILL_CE((uint32)CE_BASE, INDEX, \
...
)
{
   ...
   do
   {
      ...
      data = HAL_QUPE_SPI_BAM_CHECK_SET_DATA(mHalDeviceParameters->eLoopbackMode,
                                             HAL_QUPE_SPI_LOOPBACK_ENABLED);     

      HAL_QUPE_BAM_ADD_REG_INFO(SPI_CONFIG, LOOP_BACK, data, reg_data, mask);
