...
struct SpiTransferType
{
   ...
   qurt_signal_t *dalSysEvent;/**< If the client needs to start an asynchronous transfer, 
...
struct DalSpi
{
   ...
   DALResult (*SendAndReceiveAsync)(DalDeviceHandle * _h, SpiDataAddrType * outputAddr,  int  outputDataLen,  uint32  u32NumInputTransfers, SpiDataAddrType * inputAddr,  int  inputDataLen, int * inputDataLenReq, uint32 olen6, void *callback_fn, void *callback_ctxt);
...
@{ */
/**
*DalSpi_SendAndReceiveASync()
*
*@brief
* 	Conduct a bidirectional data transfer.
* 	The data transfer will be conducted for its desired SPI core with 
* 	its desired configuration.
* 	This function is asynchrous. It dosent block until all data are read or failure happens
*
*
*  @param _h - Pointer to a DAL device handle
*  @param outputAddr - Pointer to output data.
...
*  @param inputDataLen - Length of input data buffer.
*  @param inputDataLenReq - Length of input data received.
*  @param callback_fn - Pointer to Call back function
*  @param callback_ctxt - Pointer to Call back context
*
*@dependencies
*  None
*
*@return
*     SPI_QUEUED     The transfer is queued
*     SPI_COMPLETE   The transfer is successfully completed.
*     SPI_FAILURE    The transfer failed.
*     SPI_FULL       SPI DAL's working queue is full. It cannot process this request.
*     NOTE: SpiTransferStatus is casted DALResult and retured to client.
...
*/
static __inline DALResult
DalSpi_SendAndReceiveAsync(DalDeviceHandle * _h,  SpiDataAddrType*  outputAddr,  int  outputDataLen,  uint32  u32NumInputTransfers,  SpiDataAddrType*  inputAddr,  int  inputDataLen,  int*  inputDataLenReq,void *callback_fn, void *callback_ctxt)
{
   return ((DalSpiHandle *)_h)->pVtbl->SendAndReceiveAsync( _h, outputAddr, outputDataLen, u32NumInputTransfers, inputAddr, inputDataLen, inputDataLenReq,sizeof( int ),callback_fn, callback_ctxt);
}
