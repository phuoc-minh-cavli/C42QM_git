...
/*
Copyright (c) 1990-1993, 1997-2017,2019, 2022 Qualcomm Technologies Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/buses/api/sio/sio.h#2 $ $DateTime: 2022/04/26 04:42:20 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/26/22   rk      Added flow control support
5/24/19    ss      Memory optimisation on Threadx
03/08/18   rc      Added SIO_PORT_USB_SER6 port.
12/13/17   pr      Added SIO_PORT_USB_SER5 port.
03/22/17   rc      Added new IOCTL to get the current baud rate 
03/21/17   rc      Added the USB_MDM2 port 
11/24/16   rc      Added the SIO_BITRATE_1000000 
7/28/16    rc      Added new IOCTL to get the USB end point info
10/06/16   ska	   Ported for TN 
10/06/14   vv      Added MHI DCI port
07/25/14   vv      Added DPL port.
08/19/13   vv      Added MHI ports.
...
#define SIO_MAJOR_APS           5       /**< SIO Major for APS driver */
...
#define SIO_MAJOR_A2            8      /**< SIO Major for A2 driver */
#define SIO_MAJOR_DATA_MUX      9      /**< SIO Major for MUX driver  */
#define SIO_MAJOR_SDIOC_STREAM_DATA   10      /**< SIO Major for SDIO stream client driver */
#define SIO_MAJOR_SDIOC_PACKET_DATA   11      /**< SIO Major for SDIO packet client driver */
#define SIO_MAJOR_QSAP_FILTER   12      /**< SIO Major for QSAP filter driver */
#define SIO_MAJOR_SMUX          13      /**< SIO Major for serial mux driver */
#define SIO_MAJOR_QBI_DIAG      14      /**< SIO Major for QBI DIAG driver */
#define SIO_NULL_DRIVER         15      /**< SIO Major for NULL driver */
...
#define SIO_MINOR_USB_IPCR      22      /**< SIO Minor for USB IPC Router device */
#define SIO_MINOR_USB_DCI       23      /**< SIO Minor for USB DCI device */
#define SIO_MINOR_USB_LWIP      24      /**< SIO Minor for USB LWIP device */
#define SIO_MINOR_USB_DPL       25      /**< SIO Minor for USB DPL device */
#define SIO_MINOR_USB_AUDIO     26     /**< SIO Minor for USB Audio device */
#define SIO_MINOR_USB_MDM2      27     /**< SIO Minor for USB MDM2 device */
#define SIO_MINOR_USB_SER5      28      /**< SIO Minor for USB SER5 device */
#define SIO_MINOR_USB_SER6      29      /**< SIO Minor for USB SER6 device */
...
#define SIO_MINOR_HSUSB_IPCR      SIO_MINOR_USB_IPCR
#define SIO_MINOR_HSUSB_LWIP      SIO_MINOR_USB_LWIP
#define SIO_MINOR_HSUSB_DCI       SIO_MINOR_USB_DCI
#define SIO_MINOR_HSUSB_DPL       SIO_MINOR_USB_DPL
#define SIO_MINOR_HSUSB_MDM2      SIO_MINOR_USB_MDM2
...
typedef enum
{
  ...
  SIO_PORT_USB_SER4 = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_SER4 ),
      /** -- SIO Port id for USB serial 5 device */
  SIO_PORT_USB_SER5 = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_SER5 ),
    /** -- SIO Port id for USB serial 6 device */
  SIO_PORT_USB_SER6 = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_SER6 ),
  ...
  SIO_PORT_USB_SMCT = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_SMCT),
    /** -- SIO Port id for USB DCI device */
  SIO_PORT_USB_DCI = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_DCI),
    /** -- SIO Port id for USB IPC Router device */
  SIO_PORT_USB_IPCR   = DEVICE_ID(SIO_MAJOR_HSUSB_ECM , SIO_MINOR_USB_IPCR),
    /** -- SIO Port id for USB LWIP device */
  SIO_PORT_USB_LWIP   = DEVICE_ID(SIO_MAJOR_HSUSB_ECM , SIO_MINOR_USB_LWIP),
    /** -- SIO Port id for USB DPL device */
  SIO_PORT_USB_DPL = DEVICE_ID(SIO_MAJOR_HSUSB_ECM, SIO_MINOR_USB_DPL),
    
      /** -- SIO Port id for USB MDM2 device */
  SIO_PORT_USB_MDM2 = DEVICE_ID(SIO_MAJOR_USB , SIO_MINOR_USB_MDM2 ),
...
/*---------------------------------------------------------------------------*/
/** Identifies the different USB EP support.*/
/*---------------------------------------------------------------------------*/
...
typedef enum
{
  ...
  SIO_IOCTL_DISABLE_DTR_EVENT_EXT,   /**< -- De-register call-back for informing of
                                        ...
                                        indication       */
  SIO_IOCTL_ENABLE_CTS_EVENT_EXT,    /**< -- Register callback which informs caller
                                        of change in state of CTS Signal changed
                                          - callback gets port_id and user data as args */
  SIO_IOCTL_DISABLE_CTS_EVENT_EXT,   /**< -- De-register call-back for informing of
                                            change in state of CTS signal      */	
  ...
  SIO_IOCTL_SET_TYPE2_WCI2_MSG_INTR_MASK,/**< -- Sets the interrupt mask for the wci2 message 
                                                interrupt.*/
  SIO_IOCTL_ENABLE_RAT_ID_CXM,         /**< -- Enables the RAT ID packet feature starting Atlas modem */
  SIO_IOCTL_DISABLE_RAT_ID_CXM,        /**< -- Disables the RAT ID packet feature starting Atlas modem */
  SIO_IOCTL_SET_RAT_EVENTS,         /**< -- Modifies the broken RAT ID interrupt masks */ 
  SIO_IOCTL_SET_AUDIO_CB,              /**< -- Regsiter callbacks for Audio connect/disconnect*/
  SIO_IOCTL_SET_AUDIO_DL_CB,           /**< -- Regsiter callback for Audio DownLink transfer complete*/
  SIO_IOCTL_GET_EP_INFO,           /**< -- To return the peripheral_ep_info  structure which will be filled by USB driver*/
  SIO_IOCTL_GET_CURRENT_BAUD_RATE, /**< -- To return the current baud rate of the UART*/
} sio_ioctl_cmd_type;

/*---------------------------------------------------------------------------*/
/** These are the CXM UART RAT ID based events.*/
/*---------------------------------------------------------------------------*/

typedef enum 
{
  SIO_CXM_UART_BROKEN_RAT_ID = 0x1,
  SIO_CXM_UART_WRONG_RAT_ID  = 0x2,
  SIO_CXM_UART_TYPE0_MISSING = 0x4,
}sio_cxm_uart_rat_events;
...
/*---------------------------------------------------------------------------*/
/** Structure to be passed as SIO_IOCTL_ENABLE_CTS_EVENT_EXT ioctl parameter. */
/*---------------------------------------------------------------------------*/
typedef struct sio_ioctl_enable_cts_event_ext{
  sio_vpu_func_ptr_type cb_func;
  void *                cb_data;
} sio_ioctl_enable_cts_event_ext_type;
...
/*---------------------------------------------------------------------------*/
/** Structure to set the CXM RAT ID feature. */
/*---------------------------------------------------------------------------*/
typedef struct
{
  /** Input: RAT ID packet message type */
  uint8                   rat_id_msg_type;
  /** Input: Boolean to enable/disable RAT packet from entering FIFO */
  boolean                 enable_rat_msg_to_fifo;
  /** Input: Boolean to enable/disable TCM resend option on broken RATID */
  boolean                 enable_tcm_resend;
  /** Input: SAM for the different RATs*/
  uint32                  sam;
  /** Input: Callback function for RAT ID based events*/
  sio_vpu_func_ptr_type   rat_event_cb;
  /** Output: Status of the IOCTL */
  sio_status_type         *status_ptr; 
}uart_cxm_rat_id_param_type;

/*---------------------------------------------------------------------------*/
/** Call back for  Audio for notfying the connect status with SIO_IOCTL_AUDIO_SET_CB   */
/*---------------------------------------------------------------------------*/
typedef void (*sio_audio_status_cb_type)
(
	boolean is_read,
	boolean connect
);
...
/*---------------------------------------------------------------------------*/
/** Call back for  Audio for notfying the DL data trasnfer completion SIO_IOCTL_AUDIO_SET_DL_CB   */
/*---------------------------------------------------------------------------*/
typedef void (*sio_audio_dl_comp_cb_type)
(
	void
);
typedef struct
...
typedef union sio_ioctl_param {
  ...
  sio_ioctl_enable_cts_event_ext_type   enable_cts_ready_event_ext; /**< Enable DTE ready event ext */
  ...
  uart_cxm_rat_id_param_type     uart_cxm_rat_id_param;
  uint32                         rat_events_mask;
  sio_audio_status_cb_type      set_audio_status_cb;       /**< Callback for Audio stream connect/disconnect  */
  sio_audio_dl_comp_cb_type     set_audio_dl_xfer_comp;    /**< Callback for Audio DownLink stream transfer complete */
  sio_peripheral_ep_info        usb_peripheral_ep_info;    /*USB endpoint info*/
  sio_bitrate_type              *cur_bitrate;                   /*current baud/bit rate */
