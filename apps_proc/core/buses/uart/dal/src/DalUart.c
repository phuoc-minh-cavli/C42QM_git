...
DESCRIPTION: UART DAL (OS-independent layer)
                     ...
                     Copyright (c) 2009-2014, 2016,2018 Qualcomm Technologies Incorporated
                                        All Rights Reserved
...
$Header: //components/rel/core.tx/6.0/buses/uart/dal/src/DalUart.c#1 $
...
GLOBAL FUNCTIONS:
   DalUart_ClearBreak
   DalUart_Close
   DalUart_CxmTxDirectChar
   DalUart_DeInit
   DalUart_DumpRegs
   DalUart_GetCTS
   DalUart_GetCxmTxSticky
   DalUart_GetStatus
   DalUart_Init
   DalUart_LoopbackSet
   DalUart_Open
   DalUart_PowerCapabilities
   DalUart_PowerGet
   DalUart_PowerSet
   DalUart_PurgeRx
   DalUart_PurgeTx
   ...
   DalUart_RxActive
   DalUart_SetBreak
   DalUart_SetCharFormat
   DalUart_SetCtsControl
   DalUart_SetCxm
   DalUart_SetCxmMsgIntrMask
   DalUart_SetCxmType2Mode
   DalUart_SetRate
   DalUart_SetRtsControl
   DalUart_TxComplete
   DalUart_TxSingleChar
   DalUart_Write
...
#define TX_BUF_SIZE            2048 // size of the TX buffer
...
#define DMOV(uart_ctxt) (uart_ctxt->init_cfg.uart_type == UART_DM)
#define BAM(uart_ctxt)  (uart_ctxt->init_cfg.uart_type == UART_BAM)
...
typedef struct
{
   TAL_OS_PHYS         *rx_memobj;    // uncached buffer that contains RX data
   TAL_DMA_DESCRIPTOR  rx_descriptor; // buffer descriptor used for DMA operations
...
typedef struct
{
   UartInitConfig       init_cfg;            // configuration passed down during init
   ...
   void *               register_sync;       // protects UART Registers (enforces atomic access)
   ...
   uint32               prop_cxm_msg_irq;    // (from XML props) IRQ number assigned to the CXM
                                             // messages
   uint32               prop_cxm_rat_irq;    // (from XML props) IRQ number assigned to the CXM 
                                             // rat events
   TAL_OS_PHYS          *uart_mapping;       // contains virtual address mapping for UART block
   TAL_OS_PHYS          *gsbi_mapping;       // contains virtual address mapping for GSBI block
   ...
   void *               tx_sync;             // protects transmit operations
   uint32               tx_fifo_size;        // size of TXFIFO (in bytes)
   TAL_OS_PHYS          *tx_memobj;          // uncached buffer that contains TX data
   ...
   void *               rx_sync;             // protects receive operations
   ...
   TAL_INTERRUPT_ID     cxm_rat_intr;        // TAL_INTERRUPT_ID for CXM RAT ID events
   boolean              cxm_rat_isr_registered;   // flag to track ISR registration
   uint32               cxm_rat_stat;        // used to store the rat event type
} UART_CONTEXT;
...
static boolean       create_objects          (UART_CONTEXT *uart_ctxt);
#if 0
...
static void *        cxm_rat_dpc             (void *context);
static void *        cxm_rat_isr             (void *context);
#endif
...
static void          deregister_isr_for_cxm_rat(UART_CONTEXT *uart_ctxt);
...
static void          get_property_dword      (void *id, const char *property_name,
...
static void          notify_events           (UART_CONTEXT *uart_ctxt, UartLineEventType events);
...
static uint32        power_025               (UART_CONTEXT *uart_ctxt);
static uint32        power_off               (UART_CONTEXT *uart_ctxt);
static uint32        power_on                (UART_CONTEXT *uart_ctxt);
...
static boolean       register_isr_for_cxm_rat(UART_CONTEXT *uart_ctxt);
#endif
...
static boolean allocate_buffers(UART_CONTEXT *uart_ctxt)
{
   ...
   uart_ctxt->tx_memobj = tal_os_phys_alloc(TX_BUF_SIZE);
   if (uart_ctxt->tx_memobj == NULL) { goto error; }

   for (i = 0; i < RX_BUF_CNT; i++)
   {
      MSM_UART_RX_BUF *rx_buf = &uart_ctxt->rx_buf[i];

      rx_buf->rx_memobj = tal_os_phys_alloc(RX_BUF_SIZE);
      if (rx_buf->rx_memobj == NULL) { goto error; }
   ...
   return TRUE;
error:
   tal_os_log(id, ERROR, "allocate_buffers: failed");
   return FALSE;
}
...
static boolean check_rx_error(UART_CONTEXT *uart_ctxt)
{
   ...
   if ( status & HAL_UART_STATUS_OVERRUN )
   {
      ...
      rx_error = TRUE;
      tal_os_log(id, ERROR, "check_rx_error: overrun detected");
   }
   ...
   if ( status & HAL_UART_STATUS_PARITY_FRAME_ERROR )
   {
      ...
      rx_error = TRUE;
      tal_os_log(id, ERROR, "check_rx_error: parity/frame error detected");
   }
...
static boolean clock_enable(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      tal_os_log(id, ERROR, "clock_enable: failed");
      return FALSE;
   }
}
...
static void configure_gsbi(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      gsbi = uart_ctxt->gsbi_mapping->virt_addr;
...
static boolean configure_interrupts(UART_CONTEXT *uart_ctxt)
{
   ...
   // The UART IRQ is mandatory.  It can be caller-specified (passed into DalUart_Init)
   // or read from the DAL properties (caller-specified takes precedence).  A nonzero
   // value is considered specified.

   if (uart_ctxt->init_cfg.uart_irq)  // caller-specified
   {
      uart_ctxt->uart_interrupt.irq = uart_ctxt->init_cfg.uart_irq;
   }
   ...
   {
      //tal_os_log(id, ERROR, "configure_interrupts: UART IRQ unspecified");
      return FALSE;
   }
   ...
   if (uart_ctxt->init_cfg.rx_irq)  // caller-specified
   {
      uart_ctxt->rx_interrupt.irq = uart_ctxt->init_cfg.rx_irq;
   }
   ...
   if (uart_ctxt->prop_cxm_rat_irq)
   {
      uart_ctxt->cxm_rat_intr.irq = (void *)uart_ctxt->prop_cxm_rat_irq;
   }
...
static boolean create_objects(UART_CONTEXT *uart_ctxt)
{
   ...
   if ((uart_ctxt->tx_sync       = tal_os_sync_create()) == NULL) { goto error; }
   if ((uart_ctxt->rx_sync       = tal_os_sync_create()) == NULL) { goto error; }
   if ((uart_ctxt->register_sync = tal_os_sync_create()) == NULL) { goto error; }

   return TRUE;
error:
   tal_os_log(id, ERROR, "create_objects: failed");
   return FALSE;
}
#if 0
/*==================================================================================================
...
static void *cxm_misalign_dpc(void *context)
{
   ...
   UartInitConfig *init_cfg = &uart_ctxt->init_cfg;
   void *id = init_cfg->device_id;
   uint32 cxm_misalign_stat;

   tal_os_log(id, INFO, "+cxm_misalign_dpc");
   ...
   if (init_cfg->CxmMisalignNotif) { init_cfg->CxmMisalignNotif(init_cfg->Ctxt,
                                                                cxm_misalign_stat); }
...
static void *cxm_misalign_isr(void *context)
{
   ...
   tal_os_log(id, INFO, "+cxm_misalign_isr");
...
static void *cxm_msg_dpc(void *context)
{
   ...
   tal_os_log(id, INFO, "+cxm_msg_dpc");

   tal_interrupt_sync_enter(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_msg_intr);
   msg_status = uart_ctxt->wci2_msg_status;
   tal_os_memcpy(msgs, uart_ctxt->wci2_msgs, NUM_CXM_WCI2_CONTENT_REGS);
   tal_interrupt_sync_exit(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_msg_intr);

   if (init_cfg->CxmMsgNotif) { init_cfg->CxmMsgNotif(init_cfg->Ctxt, msg_status,
                                                      msgs, NUM_CXM_WCI2_CONTENT_REGS); }
...
static void *cxm_msg_isr(void *context)
{
   ...
   tal_os_log(id, INFO, "+cxm_msg_isr");
...
FUNCTION: cxm_rat_dpc

DESCRIPTION:
   This is the dpc for the cxm rat event interrupt. The client notification is done in this
   function.
...
static void *cxm_rat_dpc(void *context)
{
   ...
   tal_os_log(id, INFO, "+cxm_rat_dpc");

   tal_interrupt_sync_enter(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_rat_intr);
   cxm_rat_stat = uart_ctxt->cxm_rat_stat;
   tal_interrupt_sync_exit(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_rat_intr);
   
   if (init_cfg->CxmRatNotif) { init_cfg->CxmRatNotif(init_cfg->Ctxt, cxm_rat_stat); }
...
static void *cxm_rat_isr(void *context)
{
   ...
   tal_os_log(id, INFO, "+cxm_rat_isr");

   uart_ctxt->cxm_rat_stat = HALuart_GetCxmRatEventStatus(uart_hal);

   tal_interrupt_dpc_schedule(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_rat_intr);
...
FUNCTION: deregister_isr_for_cxm_rat
...
static void deregister_isr_for_cxm_rat(UART_CONTEXT *uart_ctxt)
{
   if (uart_ctxt->cxm_rat_isr_registered)
   {
      tal_interrupt_deregister(uart_ctxt->interrupt_handle, &uart_ctxt->cxm_rat_intr);
      uart_ctxt->cxm_rat_isr_registered = FALSE;
   }
}*/
#endif
/*==================================================================================================
...
static void deallocate_buffers(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->tx_memobj) { tal_os_phys_dealloc(uart_ctxt->tx_memobj); }

   for (i = 0; i < RX_BUF_CNT; i++)
   {
      memobj = uart_ctxt->rx_buf[i].rx_memobj;
      if (memobj) { tal_os_phys_dealloc(memobj); }
   }
}
...
static void destroy_objects(UART_CONTEXT *uart_ctxt)
{
   if (uart_ctxt->tx_sync)       { tal_os_sync_destroy(uart_ctxt->tx_sync);       }
   if (uart_ctxt->rx_sync)       { tal_os_sync_destroy(uart_ctxt->rx_sync);       }
   if (uart_ctxt->register_sync) { tal_os_sync_destroy(uart_ctxt->register_sync); }
}
...
static void dump_regs(UART_CONTEXT *uart_ctxt)
{
   ...
   if ( DMOV(uart_ctxt) )
   {
      tal_os_log(id, INFO, "UartDbg: MR1  (0x00) - 0x%08X", registers.mr1);
      tal_os_log(id, INFO, "UartDbg: MR2  (0x04) - 0x%08X", registers.mr2);
      tal_os_log(id, INFO, "UartDbg: SR   (0x08) - 0x%08X", registers.sr);
      tal_os_log(id, INFO, "UartDbg: MISR (0x10) - 0x%08X", registers.misr);
      tal_os_log(id, INFO, "UartDbg: ISR  (0x14) - 0x%08X", registers.isr);
      tal_os_log(id, INFO, "UartDbg: IPR  (0x18) - 0x%08X", registers.ipr);
      tal_os_log(id, INFO, "UartDbg: TFWR (0x1C) - 0x%08X", registers.tfwr);
      tal_os_log(id, INFO, "UartDbg: RFWR (0x20) - 0x%08X", registers.rfwr);
      tal_os_log(id, INFO, "UartDbg: HCR  (0x24) - 0x%08X", registers.hcr);
      tal_os_log(id, INFO, "UartDbg: DMRX (0x34) - 0x%08X", registers.dmrx);
      tal_os_log(id, INFO, "UartDbg: SNAP (0x38) - 0x%08X", registers.snap);
      tal_os_log(id, INFO, "UartDbg: DMEN (0x3C) - 0x%08X", registers.dmen);
      tal_os_log(id, INFO, "UartDbg: NCFT (0x40) - 0x%08X", registers.ncft);
      tal_os_log(id, INFO, "UartDbg: BADR (0x44) - 0x%08X", registers.badr);
      tal_os_log(id, INFO, "UartDbg: TXFS (0x4C) - 0x%08X", registers.txfs);
      tal_os_log(id, INFO, "UartDbg: RXFS (0x50) - 0x%08X", registers.rxfs);
      tal_os_log(id, INFO, "UartDbg: SIM  (0x80) - 0x%08X", registers.sim);
   }
   else if ( BAM(uart_ctxt) )
   {
      ...
      tal_os_log(id, INFO, "UartDbg: CSR  (0xA0) - 0x%08X", registers.csr);
      tal_os_log(id, INFO, "UartDbg: SR   (0xA4) - 0x%08X", registers.sr);
      tal_os_log(id, INFO, "UartDbg: MISR (0xAC) - 0x%08X", registers.misr);
      tal_os_log(id, INFO, "UartDbg: IMR  (0xB0) - 0x%08X", registers.imr);
      tal_os_log(id, INFO, "UartDbg: ISR  (0xB4) - 0x%08X", registers.isr);
      tal_os_log(id, INFO, "UartDbg: IRDA (0xB8) - 0x%08X", registers.irda);
      tal_os_log(id, INFO, "UartDbg: SNAP (0xBC) - 0x%08X", registers.snap);

      tal_os_log(id, INFO, "UartDbg: CCTL (0xC4) - 0x%08X", registers.cctl);
      tal_os_log(id, INFO, "UartDbg: BCR  (0xC8) - 0x%08X", registers.bcr);
      tal_os_log(id, INFO, "UartDbg: RCTL (0xCC) - 0x%08X", registers.rctl);
      tal_os_log(id, INFO, "UartDbg: DDBG (0xD0) - 0x%08X", registers.ddbg);
      tal_os_log(id, INFO, "UartDbg: FSM  (0xD4) - 0x%08X", registers.fsm);
      tal_os_log(id, INFO, "UartDbg: VER  (0xD8) - 0x%08X", registers.ver);
      tal_os_log(id, INFO, "UartDbg: GEN  (0xDC) - 0x%08X", registers.gen);
   }
}
...
static boolean get_properties(UART_CONTEXT *uart_ctxt)
{
   ...
   get_property_dword(id, "UartBase",       &uart_ctxt->prop_uart_physical,    0);
   get_property_dword(id, "GsbiBase",       &uart_ctxt->prop_gsbi_physical,    0);
   get_property_dword(id, "GsbiControl",    &uart_ctxt->prop_gsbi_control,     0);
   get_property_dword(id, "DetectBrk",      &uart_ctxt->prop_detect_break,     FALSE);
   get_property_dword(id, "IsCxm",          &uart_ctxt->prop_is_cxm,           FALSE);
   get_property_dword(id, "Irq",            &uart_ctxt->prop_uart_irq,         0);
   get_property_dword(id, "CxmMsgIrq",      &uart_ctxt->prop_cxm_msg_irq,      0);
   get_property_dword(id, "CxmMisalignIrq", &uart_ctxt->prop_cxm_misalign_irq, 0);
   get_property_dword(id, "CxmRatIrq",      &uart_ctxt->prop_cxm_rat_irq,       0);
   ...
   return TRUE;
error:
   tal_os_log(id, ERROR, "get_properties: failed");
   return FALSE;
}
...
static void get_property_dword(void *id, const char *property_name,
                               uint32 *property_value, uint32 default_value)
{
   if ( tal_os_get_property(id, property_name, property_value) == TAL_SUCCESS )
   {
      tal_os_log(id, INFO, "get_property_dword: %s = 0x%08X",
                                                property_name, *property_value);
   }
   else
   {
      *property_value = default_value;
      tal_os_log(id, INFO, "get_property_dword: %s = 0x%08X (defaulted)",
                                                property_name, *property_value);
   }
}
...
static void gpio_disable(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      tal_os_log(id, ERROR, "gpio_disable: failed");
   }
}
...
static boolean gpio_enable(UART_CONTEXT *uart_ctxt)
{
   ...
   if (result != TAL_SUCCESS)
   {
      tal_os_log(id, ERROR, "gpio_enable: failed");
      return FALSE;
   }
...
static void halt_line_detection(UART_CONTEXT *uart_ctxt, boolean skip_halt_edge)
{
   ...
   tal_os_log(id, INFO, "+halt_line_detection");
...
static void halt_rx_path(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+halt_rx_path");

   tal_os_sync_enter(uart_ctxt->rx_sync);
   uart_ctxt->rx_halt = TRUE;
   tal_os_sync_exit(uart_ctxt->rx_sync);
   ...
   if (uart_ctxt->use_dma)
   {
      ...
      tal_os_sync_enter(uart_ctxt->register_sync);
      HALuart_SetRxDmaEnable(uart_hal, FALSE);
      tal_os_sync_exit(uart_ctxt->register_sync);
...
static void halt_tx_path(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+halt_tx_path");

   tal_os_sync_enter(uart_ctxt->tx_sync);
   uart_ctxt->tx_halt = TRUE;
   tal_os_sync_exit(uart_ctxt->tx_sync);
...
static boolean map_hardware(UART_CONTEXT *uart_ctxt)
{
   ...
   //phys_addr = (void *)uart_ctxt->prop_uart_physical;
   uart_ctxt->uart_mapping = tal_os_phys_map(uart_ctxt->prop_uart_physical, UART_MEM_SIZE);
   if (uart_ctxt->uart_mapping == NULL) { goto error; }

   if (uart_ctxt->prop_gsbi_control)
   {
      //phys_addr = (void *)uart_ctxt->prop_gsbi_physical;
      uart_ctxt->gsbi_mapping = tal_os_phys_map(uart_ctxt->prop_uart_physical, GSBI_MEM_SIZE);
      if (uart_ctxt->gsbi_mapping == NULL) { goto error; }
   }

   return TRUE;
error:
   tal_os_log(id, ERROR, "map_hardware: failed");
   return FALSE;
}
...
static void mu_close(UART_CONTEXT *uart_ctxt)
{
   ...
   deregister_isr_for_cxm_rat(uart_ctxt);*/
...
DESCRIPTION:
   ...
   DalUart_Init) since we are not using a flag and critical section to control
   access.
...
static UART_CONTEXT *mu_init(UartInitConfig *init_cfg)
{
   ...
   HALUartType hal_type;
   HALUartObject *uart_hal;
   TAL_DMA_CONFIG dma_config;

   uart_ctxt = tal_os_malloc(sizeof(UART_CONTEXT));
   if (uart_ctxt == NULL) { goto error; }

   memset(uart_ctxt, 0, sizeof(UART_CONTEXT));

   if      (init_cfg->uart_type == UART_DM)  { hal_type = HAL_UART_DM; }
   else if (init_cfg->uart_type == UART_BAM) { hal_type = HAL_UART_BAM; }
   else                                      { goto error; }

   uart_ctxt->init_cfg    = *init_cfg;
   uart_ctxt->power_state = POWER_OFF;
   ...
   if ( uart_ctxt->use_dma )
   {
      ...
      dma_config.device_id   = init_cfg->device_id;
...
DESCRIPTION:
   The DAL client must ensure that they never call this function (via DalUart_Open)
   when the port is already open since we are not using a flag and critical section
   to control access.
...
static boolean mu_open(UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   if ( !register_isr_for_cxm_rat(uart_ctxt)      ) { goto error; }
#endif
...
static void notify_events(UART_CONTEXT *uart_ctxt, UartLineEventType events)
{
   ...
   tal_os_log(id, INFO, "+notify_events");

   if (init_cfg->LineEventNotif) { init_cfg->LineEventNotif(init_cfg->Ctxt, events); }
...
static void notify_rxdata_available(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+notify_rxdata_available");

   if (init_cfg->ReceiveReadyNotif) { init_cfg->ReceiveReadyNotif(init_cfg->Ctxt); }
...
static void notify_transmit_complete(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+notify_transmit_complete");

   if (init_cfg->TransmitReadyNotif) { init_cfg->TransmitReadyNotif(init_cfg->Ctxt); }
...
static boolean open_devices(UART_CONTEXT *uart_ctxt)
{
   ...
   result = tal_interrupt_open(&uart_ctxt->interrupt_handle, id);
   if (result != TAL_SUCCESS) { goto error; }

   result = tal_tlmm_open(&uart_ctxt->tlmm_handle, id);
   if (result != TAL_SUCCESS) { goto error; }

   result = tal_clock_open(&uart_ctxt->clock_handle, id);
   if (result != TAL_SUCCESS) { goto error; }

   return TRUE;
error:
   tal_os_log(id, ERROR, "open_devices: failed");
   return FALSE;
}
...
static uint32 power_025(UART_CONTEXT *uart_ctxt)
{
   uint32 result = UART_SUCCESS;

   switch (uart_ctxt->power_state)  // check current state
   {
      case POWER_OFF:  // POWER_OFF -> POWER_025
         halt_line_detection(uart_ctxt, FALSE);
         if ( !register_isr_for_rx(uart_ctxt, FALLING_EDGE) ) { result = UART_ERROR; break; }
         if ( !clock_enable(uart_ctxt) )                      { result = UART_ERROR; break; }
      ...
      case POWER_FULL:  // POWER_FULL -> POWER_025
         if ( !prepare_for_sleep(uart_ctxt) )                 { result = UART_ERROR; break; }
         halt_line_detection(uart_ctxt, FALSE);
         if ( !register_isr_for_rx(uart_ctxt, FALLING_EDGE) ) { result = UART_ERROR; break; }
         set_rts_control(uart_ctxt, RTS_ASSERT);
         clock_disable(uart_ctxt);
   ...
   if (result == UART_SUCCESS) { uart_ctxt->power_state = POWER_025; }
   return(result);
}
...
static uint32 power_off(UART_CONTEXT *uart_ctxt)
{
   ...
   if (result == UART_SUCCESS) { uart_ctxt->power_state = POWER_OFF; }
   return(result);
}
...
static uint32 power_on(UART_CONTEXT *uart_ctxt)
{
   ...
   switch (uart_ctxt->power_state)  // check current state
   {
      case POWER_OFF:  // POWER_OFF -> POWER_FULL
         if ( !clock_enable(uart_ctxt) )      { result = UART_ERROR; break; }
         if ( !resume_from_sleep(uart_ctxt) ) { result = UART_ERROR; break; }
         break;
   ...
   if (result == UART_SUCCESS) { uart_ctxt->power_state = POWER_FULL; }
   return(result);
}
...
static boolean prepare_for_sleep(UART_CONTEXT *uart_ctxt)
{
   ...
   if (!tx_quiescent)
   {
      tal_os_log(id, ERROR, "prepare_for_sleep: timeout waiting for TX idle");
      uart_ctxt->tx_wait = FALSE;
      return(FALSE);
   }
   ...
   tal_os_sleep_ms(sleep_duration);
...
static void read_rx_fifo(UART_CONTEXT *uart_ctxt, uint32 num_bytes)
{
   ...
   write_ptr = (uint8 *)rx_head->rx_memobj->virt_addr + rx_head->write_offset;
...
static void receive_chunk(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+receive_chunk");
...
static boolean reg_init(UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   tal_os_log(id, INFO, "RX FIFO size = %d", uart_ctxt->rx_fifo_size);
   tal_os_log(id, INFO, "TX FIFO size = %d", uart_ctxt->tx_fifo_size);
...
static boolean register_isr_for_cxm_misalign(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->prop_cxm_misalign_irq)
   {
      ...
      {
         tal_os_log(id, ERROR, "register_isr_for_cxm_misalign: failed");
      }
   }
...
static boolean register_isr_for_cxm_msg(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->prop_cxm_msg_irq)
   {
      ...
      {
         tal_os_log(id, ERROR, "register_isr_for_cxm_msg: failed");
      }
   }
...
FUNCTION: register_isr_for_cxm_rat
...
static boolean register_isr_for_cxm_rat(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->prop_cxm_rat_irq)
   {
      ...
      result = tal_interrupt_register(uart_ctxt->interrupt_handle,
                                      &uart_ctxt->cxm_rat_intr, TAL_INTERRUPT_TRIGGER_RISING,
                                      cxm_rat_isr, cxm_rat_dpc, uart_ctxt);

      if (result == TAL_SUCCESS)
      {
         uart_ctxt->cxm_rat_isr_registered = TRUE;
      }
      else
      {
         tal_os_log(id, ERROR, "register_isr_for_cxm_rat: failed");
      }
   }
...
static boolean register_isr_for_rx(UART_CONTEXT *uart_ctxt, RX_TRIGGER_TYPE rx_trigger)
{
   ...
   {
      tal_os_log(id, ERROR, "register_isr_for_rx: failed");
      return(FALSE);
   }
}
...
static boolean register_isr_for_uart(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      tal_os_log(id, ERROR, "register_isr_for_uart: failed");
      return(FALSE);
   }
}
...
static boolean resume_from_sleep(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->use_dma)
   {
      ...
      {
         tal_os_log(id, ERROR, "resume_from_sleep: dma_open() failed");
         return(FALSE);
      }
   }
...
static void rx_dma_callback(void *context)
{
   ...
   tal_os_log(id, INFO, "+rx_dma_callback");
...
static void rx_transfer_complete(UART_CONTEXT *uart_ctxt, boolean *rx_data, boolean *rx_error)
{
   ...
   tal_os_log(id, INFO, "+rx_transfer_complete");
...
static boolean rx_transfer_start(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+rx_transfer_start");

   if (uart_ctxt->rx_halt)
   {
      tal_os_log(id, INFO, "RX path is halted");
      goto exit;
   }
   ...
   if (uart_ctxt->rx_out_of_buffers)
   {
      ...
      tal_os_log(id, INFO, "ran out of buffers");
      goto exit;
   }
   ...
   if (uart_ctxt->use_dma)
   {
      ...
      rx_head->rx_descriptor.physical_address  = rx_head->rx_memobj->phys_addr;
      rx_head->rx_descriptor.bytes_to_transfer = RX_BUF_SIZE;
      rx_head->rx_descriptor.bytes_transferred = 0;
      rx_head->rx_descriptor.error             = 0;
      rx_head->rx_descriptor.transfer_complete = 0;

      if ( !dma_receive(uart_ctxt->dma_ctxt, &rx_head->rx_descriptor) )
      {
         tal_os_log(id, ERROR, "rx_transfer_start: dma transfer failed");
         goto exit;
      }
   }
...
static void service_rx_break_start(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_rx_break_start");
...
static void service_rx_dma(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_rx_dma");
   ...
   rx_head = &uart_ctxt->rx_buf[uart_ctxt->rx_head];
   dma_error = rx_head->rx_descriptor.error;
   bytes_transferred = DMOV(uart_ctxt) ? uart_ctxt->rx_xfer_size :
                                         rx_head->rx_descriptor.bytes_transferred;
...
static void service_rx_falling(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_rx_falling");
...
static void service_rx_rising(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_rx_rising");
...
static void service_rx_watermark(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_rx_watermark");
...
static void service_tx_dma(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_tx_dma");
   ...
   {
      tal_os_log(id, ERROR, "service_tx_dma: error detected");
   }
...
static void service_tx_ready(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_tx_ready");
...
static void service_tx_watermark(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+service_tx_watermark");
...
static boolean set_baud_rate(UART_CONTEXT *uart_ctxt, uint32 baud_rate)
{
   ...
   return TRUE;
error:
   tal_os_log(id, ERROR, "set_baud_rate: failed");
   return FALSE;
}
...
static boolean transmit_chunk(UART_CONTEXT *uart_ctxt)
{
   ...
   tal_os_log(id, INFO, "+transmit_chunk");
...
static void tx_dma_callback(void *context)
{
   ...
   tal_os_log(id, INFO, "+tx_dma_callback");
...
static void *uart_dpc(void *context)
{
   ...
   if ( (uart_events & IRQ_RXLEV) && (uart_events & IRQ_RXSTALE) )
   {
      ...
      tal_os_log(id, INFO, "RXLEV/RXSTALE both set - ignoring RXLEV");
   }
...
static void unmap_hardware(UART_CONTEXT *uart_ctxt)
{
   if (uart_ctxt->uart_mapping) { tal_os_phys_unmap(uart_ctxt->uart_mapping); }
   if (uart_ctxt->gsbi_mapping) { tal_os_phys_unmap(uart_ctxt->gsbi_mapping); }
}
...
uint32 DalUart_ClearBreak(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_ClearBreak(0x%08p)", h);

   HALuart_StopBreak(uart_hal);

   tal_os_log(id, INFO, "-DalUart_ClearBreak(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_Close
...
void DalUart_Close(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_Close(0x%08p)", h);

   mu_close(uart_ctxt);

   tal_os_log(id, INFO, "-DalUart_Close(void)");
}

/*==================================================================================================

FUNCTION: DalUart_CxmTxDirectChar
...
uint32 DalUart_CxmTxDirectChar(void *h, uint8 character)
{
   ...
   void *id = uart_ctxt->init_cfg.device_id;
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
   uint32 result = UART_ERROR;

   tal_os_log(id, INFO, "+DalUart_CxmTxDirectChar(0x%08p, 0x%02X)", h, character);
   ...
   if (uart_ctxt->prop_is_cxm)
   {
      ...
      {
         tal_os_log(id, ERROR, "DalUart_CxmTxDirectChar Failed");
      }
      tal_os_sync_exit(uart_ctxt->register_sync);
   }
   else
   {
      tal_os_log(id, ERROR, "DalUart_CxmTxDirectChar Failed Not Supported");
   }

   tal_os_log(id, INFO, "-DalUart_CxmTxDirectChar(%d)", result);
   return result;
}*/

/*==================================================================================================

FUNCTION: DalUart_DeInit
...
void DalUart_DeInit(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_DeInit(0x%08p)", h);

   mu_deinit(uart_ctxt);

   tal_os_log(id, INFO, "-DalUart_DeInit(void)");
}

/*==================================================================================================

FUNCTION: DalUart_DumpRegs
...
uint32 DalUart_DumpRegs(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_DumpRegs(0x%08p)", h);

   /* Read the GSBI register to double-check the protocol code */
   if (uart_ctxt->gsbi_mapping)
   {
      volatile uint32 *gsbi = uart_ctxt->gsbi_mapping->virt_addr;

      tal_os_log(id, INFO, "GSBIn_CTRL_REG=0x%08X", *gsbi);
   }
   ...
   {
      tal_os_log(id, INFO, "UART clocks are off");
   }

   tal_os_log(id, INFO, "-DalUart_DumpRegs(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_GetCTS
...
uint32 DalUart_GetCTS(void *h, uint32 *cts_state)
{
   ...
   tal_os_log(id, INFO, "+DalUart_GetCTS(0x%08p, 0x%08p)", h, cts_state);
   ...
   *cts_state = !(HALuart_GetInterruptStatus(uart_hal) & HAL_UART_CURRENT_CTS_IMR_FLAG);
   ...
   tal_os_log(id, INFO, "-DalUart_GetCTS(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_GetCxmTxSticky
...
uint32 DalUart_GetCxmTxSticky(void *h, uint32 *tx_sticky, uint32 clear)
{
   ...
   tal_os_log(id, INFO, "+DalUart_GetCxmTxSticky(0x%08p, 0x%08p, 0x%08X)", h, tx_sticky, clear);
   ...
   tal_os_log(id, INFO, "-DalUart_GetCxmTxSticky(%d)", result);
   return result;
}*/

/*==================================================================================================

FUNCTION: DalUart_GetStatus
...
uint32 DalUart_GetStatus(void *h, UartStatusType *status)
{
   ...
   tal_os_log(id, INFO, "+DalUart_GetStatus(0x%08p, 0x%08p)", h, status);
   tal_os_sync_enter(uart_ctxt->rx_sync);

   *status = (UartStatusType)0;
   ...
   *status |= uart_ctxt->rx_cached_status;  // get cached status (overrun/parity/framing)
   ...
   tal_os_sync_exit(uart_ctxt->rx_sync);
   tal_os_log(id, INFO, "-DalUart_GetStatus(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_Init
...
void *DalUart_Init(UartInitConfig *init_cfg)
{
   ...
   //tal_os_log(id, INFO, "+DalUart_Init(0x%08p)", init_cfg);

   uart_ctxt = mu_init(init_cfg);

   //tal_os_log(id, INFO, "-DalUart_Init(0x%08p)", uart_ctxt);
   return uart_ctxt;
}

/*==================================================================================================

FUNCTION: DalUart_LoopbackSet
...
uint32 DalUart_LoopbackSet(void *h, uint32 enabled)
{
   ...
   tal_os_log(id, INFO, "+DalUart_LoopbackSet(0x%08p, 0x%08X)", h, enabled);

   tal_os_sync_enter(uart_ctxt->register_sync);
   HALuart_LoopbackSet(uart_hal, enabled);
   tal_os_sync_exit(uart_ctxt->register_sync);

   tal_os_log(id, INFO, "-DalUart_LoopbackSet(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_Open
...
uint32 DalUart_Open(void *h, UartOpenConfig *open_cfg)
{
   ...
   tal_os_log(id, INFO, "+DalUart_Open(0x%08p, 0x%08p)", h, open_cfg);

   result = mu_open(uart_ctxt, open_cfg) ? UART_SUCCESS : UART_ERROR;

   tal_os_log(id, INFO, "-DalUart_Open(%d)", result);
   return result;
}

/*==================================================================================================

FUNCTION: DalUart_PowerCapabilities
...
uint32 DalUart_PowerCapabilities(void *h, uint32 *power_mask)
{
   ...
   tal_os_log(id, INFO, "+DalUart_PowerCapabilities(0x%08p, 0x%08p)", h, power_mask);

   *power_mask = POWER_FULL | POWER_025 | POWER_OFF;

   tal_os_log(id, INFO, "-DalUart_PowerCapabilities(UART_SUCCESS)");
   return(UART_SUCCESS);
}

/*==================================================================================================

FUNCTION: DalUart_PowerGet
...
uint32 DalUart_PowerGet(void *h, UartPowerType *power_state)
{
   ...
   tal_os_log(id, INFO, "+DalUart_PowerGet(0x%08p, 0x%08p)", h, power_state);

   *power_state = uart_ctxt->power_state;

   tal_os_log(id, INFO, "-DalUart_PowerGet(UART_SUCCESS)");
   return(UART_SUCCESS);
}

/*==================================================================================================

FUNCTION: DalUart_PowerSet
...
uint32 DalUart_PowerSet(void *h, UartPowerType power_state)
{
   ...
   tal_os_log(id, INFO, "+DalUart_PowerSet(0x%08p, 0x%08X)", h, power_state);
   ...
   switch (power_state)  // requested power state
   {
      ...
      case POWER_075:
      default:
         tal_os_log(id, ERROR, "DalUart_PowerSet: invalid power state");
         ret = UART_ERROR;
         break;
   }

   tal_os_log(id, INFO, "-DalUart_PowerSet(%d)", ret);
   return(ret);
}

/*==================================================================================================

FUNCTION: DalUart_PurgeRx
...
uint32 DalUart_PurgeRx(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_PurgeRx(0x%08p)", h);
   ...
   tal_os_log(id, INFO, "-DalUart_PurgeRx(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_PurgeTx
...
uint32 DalUart_PurgeTx(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_PurgeTx(0x%08p)", h);

   halt_tx_path(uart_ctxt);
   HALuart_SetTxEnable(uart_hal, TRUE);  // is this necessary after TX reset?
   uart_ctxt->tx_halt = FALSE;

   tal_os_log(id, INFO, "-DalUart_PurgeTx(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_Read
                    ...
                    uint32 *bytes_read, uint32 *overrun_events)
{
   ...
   tal_os_log(id, INFO, "+DalUart_Read(0x%08p, 0x%08p, 0x%08X, 0x%08p, 0x%08p)",
   ...
   *overrun_events = uart_ctxt->rx_overruns;  // report number of overrun events since last read
   ...
   for (;;)
   {
      ...
      src_ptr = (uint8 *)rx_tail->rx_memobj->virt_addr + rx_tail->read_offset;

      tal_os_memcpy(read_buffer, src_ptr, num_bytes);
   ...
   tal_os_log(id, INFO, "-DalUart_Read(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_RxActive
...
uint32 DalUart_RxActive(void *h, uint32 *rx_active)
{
   ...
   tal_os_log(id, INFO, "+DalUart_RxActive(0x%08p, 0x%08p)", h, rx_active);

   *rx_active = (HALuart_GetDmaRx(uart_hal) != 0) ? 1 : 0;

   tal_os_log(id, INFO, "-DalUart_RxActive(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_SetBreak
...
uint32 DalUart_SetBreak(void *h)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetBreak(0x%08p)", h);

   HALuart_StartBreak(uart_hal);

   tal_os_log(id, INFO, "-DalUart_SetBreak(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_SetCharFormat
...
uint32 DalUart_SetCharFormat(void *h, UartCharFormat *char_format)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetCharFormat(0x%08p, 0x%08p)", h, char_format);

   if ( !valid_char_format(char_format) ) { return UART_ERROR; }
   set_char_format(uart_ctxt, char_format);

   tal_os_log(id, INFO, "-DalUart_SetCharFormat(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_SetCtsControl
...
uint32 DalUart_SetCtsControl(void *h, UartCtsControlType cts_control)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetCtsControl(0x%08p, 0x%08X)", h, cts_control);

   tal_os_sync_enter(uart_ctxt->register_sync);
   HALuart_SetOutboundFlowCtl(uart_hal, (cts_control == CTS_ENABLE));
   tal_os_sync_exit(uart_ctxt->register_sync);

   tal_os_log(id, INFO, "-DalUart_SetCtsControl(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_SetRate
...
uint32 DalUart_SetRate(void *h, uint32 baud_rate)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetRate(0x%08p, %d)", h, baud_rate);

   if ( !set_baud_rate(uart_ctxt, baud_rate) ) { return UART_ERROR; }

   tal_os_log(id, INFO, "-DalUart_SetRate(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_SetRtsControl
...
uint32 DalUart_SetRtsControl(void *h, UartRtsControlType rts_control)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetRtsControl(0x%08p, %d)", h, rts_control);

   if ( !valid_rts_control(rts_control) ) { return UART_ERROR; }
   uart_ctxt->rts_control = rts_control;
   set_rts_control(uart_ctxt, uart_ctxt->rts_control);

   tal_os_log(id, INFO, "-DalUart_SetRtsControl(UART_SUCCESS)");
   return UART_SUCCESS;
}
#if 0
/*==================================================================================================

FUNCTION: DalUart_SetCxm
...
uint32 DalUart_SetCxm(void *h, uint32 enabled, uint32 sam)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetCxm(0x%08p, %d, %x)", h, enabled, sam);
   ...
   tal_os_log(id, INFO, "-DalUart_SetCxm(%d)", result);
   return result;
}*/

/*==================================================================================================

FUNCTION: DalUart_SetCxmMsgIntrMask
...
uint32 DalUart_SetCxmMsgIntrMask(void *h, uint32 intr_mask)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetCxmMsgIntrMask(0x%08p, %d)", h, intr_mask);
   ...
   tal_os_log(id, INFO, "-DalUart_SetCxmMsgIntrMask(%d)", result);
   return result;
}

/*==================================================================================================

FUNCTION: DalUart_SetCxmType2Mode
...
uint32 DalUart_SetCxmType2Mode(void *h, uint32 enabled)
{
   ...
   tal_os_log(id, INFO, "+DalUart_SetCxmType2Mode(0x%08p, %d)", h, enabled);
   ...
   tal_os_log(id, INFO, "-DalUart_SetCxmType2Mode(%d)", result);
   return result;
}

/*=================================================================================================

FUNCTION: DalUart_TxComplete
                                        ...
                                        uint32 sam, boolean enabled)
{
   ...
   tal_os_log(id, INFO, "+Uart_SetCxmRatIdMode(0x%08p, %d)", h, enabled);
   ...
   if (uart_ctxt->prop_cxm_rat_irq)
   {
      ...
      if (enabled)
      {
         HALuart_SetCxmRatIdEnable(uart_hal, rat_msg_type, enable_rat_msg_to_fifo,
                                   enable_tcm_resend, sam, TRUE);
         
      }
      else
      {
         HALuart_SetCxmRatIdEnable(uart_hal, rat_msg_type, enable_rat_msg_to_fifo,
                                   enable_tcm_resend, sam, FALSE);
   ...
   tal_os_log(id, INFO, "-Uart_SetCxmRatIdMode(%d)", result);
   return result;
}*/
#endif
/*==================================================================================================
...
uint32 DalUart_TxComplete(void *h, uint32 *tx_complete)
{
   ...
   tal_os_log(id, INFO, "+DalUart_TxComplete(0x%08p, 0x%08p)", h, tx_complete);

   *tx_complete = HALuart_GetStatus(uart_hal) & HAL_UART_STATUS_TX_EMPTY;

   tal_os_log(id, INFO, "-DalUart_TxComplete(UART_SUCCESS)");
   return UART_SUCCESS;
}

/*==================================================================================================

FUNCTION: DalUart_TxSingleChar
...
uint32 DalUart_TxSingleChar(void *h, uint8 character)
{
   ...
   tal_os_log(id, INFO, "+DalUart_TxSingleChar(0x%08p, 0x%02X)", h, character);
   ...
   for (i = 0; i < 10000; i++)
   {
      ...
      if (bytes_sent == 1) { result = UART_SUCCESS; break; }
   }

   tal_os_sync_exit(uart_ctxt->tx_sync);

   tal_os_log(id, INFO, "-DalUart_TxSingleChar(%d)", result);
   return result;
}

/*==================================================================================================

FUNCTION: DalUart_Write

DESCRIPTION:

==================================================================================================*/
uint32 DalUart_Write(void *h, const uint8 *write_buffer, uint32 buffer_len,
                     uint32 *bytes_written)
{
   ...
   uint32 result = UART_ERROR;
   boolean transmit_ok;
   TAL_OS_PHYS *tx_memobj = uart_ctxt->tx_memobj;
   uint32 num_bytes;

   tal_os_log(id, INFO, "+DalUart_Write(0x%08p, 0x%08p, 0x%08X, 0x%08p)",
   ...
   *bytes_written = 0;
   ...
   if (buffer_len == 0)
   {
      tal_os_log(id, ERROR, "DalUart_Write: No data to send");
   }
   else
   {
      tal_os_sync_enter(uart_ctxt->tx_sync);
      ...
      {
         tal_os_log(id, ERROR, "DalUart_Write: TX transfer in progress");
      }
      else if (uart_ctxt->tx_halt || uart_ctxt->tx_wait)
      {
         tal_os_log(id, ERROR, "DalUart_Write: TX halt/wait in progress");
      }
      else
      {
         ...
         tal_os_memcpy((uint8 *)tx_memobj->virt_addr, write_buffer, num_bytes);
         ...
         if (uart_ctxt->use_dma)
         {
            ...
            uart_ctxt->tx_descriptor.physical_address  = tx_memobj->phys_addr;
         ...
         else
         {
            ...
            uart_ctxt->tx_ptr        = (uint8 *)tx_memobj->virt_addr;
            uart_ctxt->tx_bytes_left = num_bytes;

            transmit_ok = transmit_chunk(uart_ctxt);
         }

         if (transmit_ok) { *bytes_written = num_bytes; result = UART_SUCCESS; }
   ...
   tal_os_log(id, INFO, "-DalUart_Write(%d)", result);
   return result;
}
