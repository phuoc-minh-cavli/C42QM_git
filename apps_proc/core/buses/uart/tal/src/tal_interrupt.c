...
DESCRIPTION: This module defines the reference implementation of the interrupt APIs for the
             Target Abstraction Layer.  It uses the DAL interrupt architecture.
                     ...
                     Copyright (c) 2012-2014,2020 Qualcomm Technologies Incorporated
                                        All Rights Reserved
...
$Header: //components/rel/core.tx/6.0/buses/uart/tal/src/tal_interrupt.c#2 $
...
#define MAX_IRQS       2     // (1) UART (2) RX GPIO (3) CXM Msg (4) CXM Misalignment
...
typedef struct int_object
{
   ...
   dsr_obj_t             *dsr_objext;       // signaling this dsr object will wake-up the callback
   boolean               event_queued;     // TRUE if the event is added to the workloop
   qurt_mutex_t          *isr_sync;         // mutex used to synchronize with the ISR
   boolean               isr_running;      // TRUE while the client ISR is running
   qurt_mutex_t          *dpc_sync;         // mutex that protects DPC variables
...
static uint32              atomic_compare_exchange(uint32 volatile *addr, uint32 new,
                                                   uint32 expected, qurt_mutex_t *mutex);
static uint32              atomic_decrement(uint32 volatile *addr, qurt_mutex_t* mutex);
static uint32              atomic_increment(uint32 volatile *addr, qurt_mutex_t *mutex);
static uint32              atomic_read(uint32 volatile *addr, qurt_mutex_t *mutex);
...
static TAL_RESULT          register_isr(INT_OBJECT *int_object, TAL_INTERRUPT_ID *interrupt, TAL_INTERRUPT_TRIGGER trigger, TAL_INTERRUPT_ISR isr, TAL_INTERRUPT_DPC dpc, void *ctxt);
static void                service_dpc(INT_OBJECT *int_object);
static void               *service_interrupt(void *context);
static void           service_workloop(dsr_obj_t *dsr_obj, void *ctxt, void *payload);
...
static DALResult array_create(TAL_INTERRUPT_HANDLE handle)
{
   ...
   int_array = malloc(array_size);
   if (int_array == NULL) { goto error1; }
   ...
   {
      int_array[i].isr_sync = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
      qurt_mutex_init(int_array[i].isr_sync);
      int_array[i].dpc_sync = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));	  
      qurt_mutex_init(int_array[i].dpc_sync);
      ...
      if(!dsr_create(&int_array[i].dsr_objext,service_workloop,&int_array[i],DSR_PRI_HIGH,DSR_CONTEXT_ISR))
      {
         goto error2;
      }
...
static void array_destroy(TAL_INTERRUPT_HANDLE handle)
{
   ...
   for (i = 0; i < handle->max_irqs; i++)
   {
      if (int_array[i].dsr_objext)     
	  { 
		  dsr_destroy(int_array[i].dsr_objext);
	  }
      if (int_array[i].isr_sync)       
	  { 
		  qurt_mutex_destroy(int_array[i].isr_sync);
		  free(int_array[i].isr_sync);
	  }
      if (int_array[i].dpc_sync)      
	  {
		  qurt_mutex_destroy(int_array[i].dpc_sync);    
		  free(int_array[i].dpc_sync);
	  }
   }
...
==================================================================================================*/
static void atomic_bit_clear(uint32 volatile *addr, uint32 bit_position, qurt_mutex_t *mutex)
{
...
==================================================================================================*/
static void atomic_bit_set(uint32 volatile *addr, uint32 bit_position, qurt_mutex_t *mutex)
{
...
static void service_dpc(INT_OBJECT *int_object)
{
   qurt_mutex_t *mutex = int_object->dpc_sync;
   uint32 expected;
   uint32 previous;
...
==================================================================================================*/
static void service_workloop(dsr_obj_t *dsr_obj, void *context, void *payload) 
{
...
void tal_interrupt_dpc_enable(TAL_INTERRUPT_HANDLE handle, TAL_INTERRUPT_ID *interrupt)
{
   ...
   if (dpc_disable_cnt == 0)
   {
      ...
      {
		 dsr_enqueue(int_object->dsr_objext,NULL);
      }
   }
}
...
void tal_interrupt_dpc_schedule(TAL_INTERRUPT_HANDLE handle, TAL_INTERRUPT_ID *interrupt)
{
   ...
   if(!dsr_enqueue(int_object->dsr_objext,NULL))
   {
     return;
   }
}
...
==================================================================================================*/
TAL_RESULT tal_interrupt_open(TAL_INTERRUPT_HANDLE *phandle, void *device_id)
{
   TAL_INTERRUPT_CONTEXT *ctxt;
   DALResult result;

   ctxt= malloc(sizeof(TAL_INTERRUPT_CONTEXT));
   if (ctxt == NULL) { goto error1; }

   ctxt->client_id = (DALDEVICEID)device_id;
...
void tal_interrupt_sync_enter(TAL_INTERRUPT_HANDLE handle, TAL_INTERRUPT_ID *interrupt)
{
   ...
   int_object = array_find(handle, interrupt);
   if (int_object) { qurt_mutex_lock(int_object->isr_sync); }
}
...
void tal_interrupt_sync_exit(TAL_INTERRUPT_HANDLE handle, TAL_INTERRUPT_ID *interrupt)
{
   ...
   int_object = array_find(handle, interrupt);
   if (int_object) { qurt_mutex_unlock(int_object->isr_sync); }
}
