...
$Header: //components/rel/core.tx/6.0/buses/uart/tal/src/tal.h#1 $
...
#define TAL_TIMEOUT  1  // operation timed out
...
typedef struct
{
   void *           device_id;    // handle to UART device
...
typedef enum
{
   ...
   WARNING,
   INFO
} TAL_OS_FLAG;  // log flags

typedef struct
{
   uint32 handle;     // opaque handle that may be needed when deallocating/unmapping
   void *phys_addr;  // physical address of contiguous memory
   void *virt_addr;  // virtual address of contiguous memory
   uint32 size;      // size of contiguous memory
} TAL_OS_PHYS;
...
TAL_RESULT tal_tlmm_open           (TAL_TLMM_HANDLE *phandle, void *device_id);
