...
Copyright (c) 1999-2000, 2007-2008, 2012, 2014,2018 Qualcomm Technologies Incorporated.  All Rights Reserved.
...
$Header: //components/rel/core.tx/6.0/buses/uart/mdd/src/uart_log.c#1 $

when          who        what, where, why
-----------   --------   -----------------------------------------------------
04-Dec-2018   ssatpati   Optimisation for Twizy
...
#define  MAX_TRACE_BUFFER_SIZE    (4 * 1024)
...
buf_log_init_locking( void )
{
  ...
  s_uartTraceLogInfo.bufferSync = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
  initialized = ( s_uartTraceLogInfo.bufferSync != NULL );

  qurt_mutex_init(s_uartTraceLogInfo.bufferSync);
...
buf_log_lock( void )
{
  qurt_mutex_lock(s_uartTraceLogInfo.bufferSync);
}
...
buf_log_unlock( void )
{
  qurt_mutex_unlock(s_uartTraceLogInfo.bufferSync);
}
