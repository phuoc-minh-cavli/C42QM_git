...
Copyright (c) 1999-2000, 2007-2008, 2012-2015,2016 Qualcomm Technologies Incorporated.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/buses/uart/mdd/src/siors232_mdd.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/24/16   rc      Added the SIO_BITRATE_1000000 
10/04/16   PR      Added port id Upper Array bound check to resolve KW issue
...
static void                handle_line_event_isr( sio_mdd_port_type*, UartLineEventType );
static void                handle_line_event( sio_mdd_port_type* );
static void                workloop_handle_line_event( dsr_obj_t *dsr_obj, void *ctxt, void *payload);
static void                workloop_handle_inactivity_event( dsr_obj_t *dsr_obj, void *ctxt, void *payload);
static void                workloop_handle_flush_event( dsr_obj_t *dsr_obj, void *ctxt, void *payload);
static void                initialize_device_state( uart_device_id_type device_id);
static void                initialize_device( uart_device_id_type device_id);
...
poll_tx_complete_isr( int32 timeMs, timer_cb_data_type data )
{
  ...
  qurt_mutex_lock( txd->lock_handle );
  ...
  if( is_tx_complete )
  {
    ...
    if(!dsr_enqueue(mdd_port->dsr[UART_WORKLOOP_FLUSH_EVENT],NULL))
    {
      UART_LOG_1( TRACE, "[%d] flush event dsr enqueu is failed", device_id );
    }
  }

  qurt_mutex_unlock( txd->lock_handle );
...
static void
 workloop_handle_flush_event( dsr_obj_t *dsr_obj, void *context, void *payload)
{
...
poll_tx_complete( sio_mdd_port_type* mdd_port )
{
  ...
  timer_set(&mdd_port->tx_data.sio_stream_close_cb_item,0,TX_COMPLETE_TIMEOUT,T_MSEC);
...
siors_mdd_to_dal_baud_rate( sio_bitrate_type bitrate )
{
  ...
  switch( bitrate )
  {
    ...
    case SIO_BITRATE_1000000:  baud_rate = 1000000; break;
...
static boolean
create_lock( qurt_mutex_t **handle )
{
  ...
  do
  {
    ...
    *handle = (qurt_mutex_t *)malloc(sizeof(qurt_mutex_t));
    if(*handle != NULL)
    {
      qurt_mutex_init(*handle);
      created = TRUE;
    }
  }while(0);
  return( created );
...
static void cxm_type2_misalign_isr(sio_mdd_port_type* mdd_port, uint32 cxm_type2_misalign_status)
{
  ...
  qurt_mutex_lock(mdd_port->rx_data.lock_handle );
  ...
  qurt_mutex_unlock(mdd_port->rx_data.lock_handle );
  UART_LOG_1( TRACE, "[%d] -cxm_type2_misalign_isr", device_id );
}
                                   ...
                                   uint32 wci2_msgs_length )
{
  ...
  // interrupt callback.    
  qurt_mutex_lock(rxd->lock_handle );
...
DESCRIPTION
  ...
  This is the ISR that is called when there is a RAT ID based events
  This feature is available on CXM cores starting from Atlas modem.
...
static void cxm_rat_event_isr(sio_mdd_port_type* mdd_port, uint32 cxm_rat_event_status)
{
  uint32 device_id = mdd_port->port_data.device_id;

  UART_LOG_1( TRACE, "[%d] +cxm_rat_event_isr", device_id );

  // Call the Client's rat event interrupt callback. 
  qurt_mutex_lock(mdd_port->rx_data.lock_handle );
  if (mdd_port->rx_data.rat_event_cb != NULL)
  {  
    mdd_port->rx_data.rat_event_cb((void *)cxm_rat_event_status); 
  }
...
static void
destroy_lock( qurt_mutex_t ** handle )
{
  if(*handle !=  NULL)
  {
    qurt_mutex_destroy(*handle);
    free(*handle);
    *handle = NULL;
  }
...
siors_mdd_update_flow_control( sio_mdd_port_type* mdd_port )
{
  ...
  qurt_mutex_unlock(txd->lock_handle );
  qurt_mutex_unlock(rxd->lock_handle );
...
initialize_device_state( uart_device_id_type device_id )
{
  ...
  result = timer_def_osal(&mdd_port->tx_data.sio_stream_close_cb_item,&uart_timer_group,
                         TIMER_FUNC2_CB_TYPE,poll_tx_complete_isr,(timer_cb_data_type)mdd_port);
  if(result != TE_SUCCESS)
  {
    UART_LOG_2( LOW, "[%d] failed to register for a close timer with error %d", device_id,result );
  }    

  result = timer_def_osal( &mdd_port->port_data.inactivity_timer,&uart_timer_group,
                          TIMER_FUNC2_CB_TYPE,siors_mdd_on_inactivity_isr,(timer_cb_data_type)mdd_port);
  if(result != TE_SUCCESS)
  {
    UART_LOG_2( LOW, "[%d] failed to register for a close timer with error %d", device_id,result );
  }
...
{
  sio_mdd_port_type*           mdd_port;
  boolean                      dsr_result = FALSE;
  int                          i;
  dsr_hdlr_t                   workloop_func = NULL;
  ...
  for( i = 0; i < UART_WORKLOOP_MAX_EVENTS; i++ )
  {
    ...
    switch(i)
    {
      case UART_WORKLOOP_LINE_EVENT:
        ...
        mdd_port->line_dsr_queued = FALSE;
        break;
    ...
    dsr_result = dsr_create(&mdd_port->dsr[i],workloop_func, mdd_port,DSR_PRI_MEDIUM,DSR_CONTEXT_WORKLOOP);

    if( dsr_result != TRUE)
    {
      UART_LOG_2( HIGH, "[%d] Error %d creating adding event to workloop.", device_id, dsr_result );
      goto FAILURE;
    }
    }
...
initialize_device( uart_device_id_type device_id )
{
  ...
  initConfig.device_id          = (void*)device_info.dal_device_id;
  initConfig.Ctxt               = mdd_port;
  initConfig.enable_dma         = device_info.enable_dma;
  initConfig.uart_type          = UART_BAM;
  ...
  initConfig.CxmRatNotif        = (DALCxmRatEventFunc) cxm_rat_event_isr;
  ...
  dal_device_handle =  DalUart_Init(&initConfig);
  if(dal_device_handle  == NULL)
  {
    UART_LOG_1( ERROR, "[%d] DalUart_Init failed.", device_id );
    goto FAILURE;
  }
...
static void
handle_line_event_isr( sio_mdd_port_type* mdd_port, UartLineEventType line_events )
{
  int device_id = mdd_port->port_data.device_id;
  ...
  UART_LOG_2( TRACE, "[%d] adding line events 0x%08x to queue.", device_id, (uint32)line_events );

  uart_event_queue_add_item( &mdd_port->event_queue, (uint32)line_events );

  if(!(mdd_port->line_dsr_queued))
  {
    if(dsr_enqueue(mdd_port->dsr[UART_WORKLOOP_LINE_EVENT], NULL))
    {
      mdd_port->line_dsr_queued = TRUE;
    }
    else
    {
        UART_LOG_1( TRACE, "[%d] -failed to enque the LINE event dsr", device_id );
    }
  }
...
static void 
workloop_handle_line_event( dsr_obj_t *dsr_obj, void *ctxt, void *payload)
{
  sio_mdd_port_type* mdd_port = (sio_mdd_port_type*)ctxt;
  int                device_id;
...
siors_mdd_on_inactivity_isr( int32 time_ms, timer_cb_data_type data )
{
  ...
  if(!(dsr_enqueue(mdd_port->dsr[UART_WORKLOOP_INACTIVITY_EVENT], NULL)))
  {
    UART_LOG_1( TRACE, "[%d] inactivity event dsr enqueue is failed", device_id );
  } 
...
static void
workloop_handle_inactivity_event( dsr_obj_t *dsr_obj, void *ctxt, void *payload)
{
  sio_mdd_port_type* mdd_port = (sio_mdd_port_type*)ctxt;
  int device_id;
  uint32 rx_active;
  ...
  qurt_mutex_lock(mdd_port->port_data.lock_handle);
  ...
  if (rx_active)
  {
    ...
    if(mdd_port->port_data.inactivity_ms > 0)
    {
      ...
      timer_set(&mdd_port->port_data.inactivity_timer,
                mdd_port->port_data.inactivity_ms,0,T_MSEC);
    }
  }
  else
  {
...
DONE:
  qurt_mutex_unlock(mdd_port->port_data.lock_handle);
...
siors_mdd_powerdown( sio_mdd_port_type* mdd_port, sio_pd_reason_type reason )
{
  ...
  qurt_mutex_lock(mdd_port->tx_data.lock_handle);
  tx_locked = TRUE;
  ...
  qurt_mutex_unlock(mdd_port->tx_data.lock_handle);
...
siors_mdd_api_dev_init( void )
{
  ...
  {
     UART_LOG( ERROR, "-siors_mdd_api_dev_init, DALSYS_GetPropertyHandleStr failed" );
     return;
  }
...
siors_mdd_api_open_stream( sio_open_type* open_ptr )
{
  ...
  dal_result = DalUart_Open( mdd_port->port_data.dal_device_handle, &open_config );
  if( dal_result != UART_SUCCESS )
  {
    UART_LOG_2( ERROR, "[%d] DalUart_Open failed: %d.", device_id, dal_result );
    return_val = (sio_status_type) SIO_NO_STREAM_ID;
    goto FAILURE;
  }
...
siors_mdd_api_close_stream( sio_stream_id_type stream_id, sio_close_func close_func )
{
  ...
  DalUart_Close( mdd_port->port_data.dal_device_handle );

  mdd_port->port_data.is_clock_on = FALSE;

  //  Release RX, TX resources AFTER calling DalUart_Close.
