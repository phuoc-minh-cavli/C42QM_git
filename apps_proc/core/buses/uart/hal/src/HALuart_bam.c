...
$Header: //components/rel/core.tx/6.0/buses/uart/hal/src/HALuart_bam.c#1 $
...
#define CXM_RAT_INDEX_CONFIG 0x1EC
...
/* CXM configuration */
#define UARTBAM_CXM_CONFIG_BYPASS_BOLT               0x00020000
#define UARTBAM_CXM_CONFIG_IRQ_BROKEN_RAT_ID_MASK    0x0001C000
#define UARTBAM_CXM_CONFIG_MWS_CA_WCI2_RX_EN         0x00002000
...
#define UARTBAM_CXM_TX_CONFIG_MWS_CA_WCI2_TX_EN      0x007E0000
#define UARTBAM_CXM_TX_CONFIG_SAM_MASK               0x000007FE
#define UARTBAM_CXM_TX_CONFIG_SAM_SHIFT                       1
#define UARTBAM_CXM_TX_CONFIG_T0_WU_CLR                       1   // TYPE0_WAKEUP_CLEAR

/* CXM RX status */
#define UARTBAM_CXM_RX_STAT_IRQ_BROKEN_RAT_ID_STAT       0x00001C00
#define UARTBAM_CXM_RX_STAT_IRQ_BROKEN_RAT_ID_STAT_SHIFT         10
...
#define UARTBAM_CXM_DIR_UART_TR_DIR_UART_COUNT_MSB_SHIFT           24
#define UARTBAM_CXM_DIR_UART_TR_DIR_UART_COUNT_MSB_MASK    0xFF000000
#define UARTBAM_CXM_DIR_UART_TR_DIR_UART_CA_ENABLE         0x00040000            
...
/* CXM RAT Index configuration*/
#define UARTBAM_CXM_RAT_INDEX_CONFIG_TYPE7_IRQ_TYPE_MASK            0x00000040
#define UARTBAM_CXM_RAT_INDEX_CONFIG_SEND_TYPE1_RAT_ID_BROKEN_MASK  0x00000020
#define UARTBAM_CXM_RAT_INDEX_CONFIG_SEND_TYPE1_RAT_ID_BROKEN_SHIFT          5
#define UARTBAM_CXM_RAT_INDEX_CONFIG_TYPE7_RXFIFO_TYPE_MASK         0x00000010
#define UARTBAM_CXM_RAT_INDEX_CONFIG_TYPE7_4_RXFIFO_MASK            0x00000008
#define UARTBAM_CXM_RAT_INDEX_CONFIG_RAT_INDEX_IDENTIFIER_MASK      0x00000007
...
static uint32 HALuart_bam_CxmTxDirectChar            (HALUartObject *uart, uint16 character);
...
static void   HALuart_bam_SetCxmRatIdEnable          (HALUartObject *uart, uint32 rat_msg_type, 
                                                         uint32 enable_rat_msg_to_fifo,
                                                         uint32 enable_tcm_resend,
                                                         uint32 sam, uint32 enabled);
static uint32 HALuart_bam_GetCxmRatEventStatus         (HALUartObject *uart);   
...
const HALUartFcnTable functions_uart_bam =
{
   ...
   HALuart_bam_SetCxmRatIdEnable,
   HALuart_bam_GetCxmRatEventStatus,
};
...
static uint32 HALuart_bam_CxmTxDirectChar(HALUartObject *uart, uint16 character)
{
   ...
   cxm_dir_uart_tr &= ~(UARTBAM_CXM_DIR_UART_TR_DIR_UART_COUNT_MSB_MASK | 
                        UARTBAM_CXM_DIR_UART_TR_DIR_UART_COUNT_MASK);
   cxm_dir_uart_tr |= ((character & 0xff) << 8);

   if (cxm_dir_uart_tr & UARTBAM_CXM_DIR_UART_TR_DIR_UART_CA_ENABLE)
   {
      cxm_dir_uart_tr |= (((character >> 8) & 0xff) << UARTBAM_CXM_DIR_UART_TR_DIR_UART_COUNT_MSB_SHIFT);
   }
...
static void HALuart_bam_SetCxmRatIdEnable (HALUartObject *uart, uint32 rat_msg_type, 
                                           uint32 enable_rat_msg_to_fifo, uint32 enable_tcm_resend,
                                           uint32 sam, uint32 enabled)
{
   uint32 cxm_config;
   uint32 cxm_tx_config;
   uint32 cxm_dir_uart_tr;
   uint32 cxm_rat_index_config = 0;
   ...
   if (enabled)
   {
      ...
      cxm_config |= UARTBAM_CXM_CONFIG_BYPASS_BOLT |
                    UARTBAM_CXM_CONFIG_MWS_CA_WCI2_RX_EN;       
      cxm_config &= ~UARTBAM_CXM_CONFIG_IRQ_BROKEN_RAT_ID_MASK;

      cxm_tx_config |= UARTBAM_CXM_TX_CONFIG_MWS_CA_WCI2_TX_EN;
      cxm_tx_config &= ~UARTBAM_CXM_TX_CONFIG_SAM_MASK;
      cxm_tx_config |= ((sam << UARTBAM_CXM_TX_CONFIG_SAM_SHIFT) & UARTBAM_CXM_TX_CONFIG_SAM_MASK);

      cxm_dir_uart_tr |= UARTBAM_CXM_DIR_UART_TR_DIR_UART_CA_ENABLE;

      cxm_rat_index_config |= ((enable_tcm_resend << UARTBAM_CXM_RAT_INDEX_CONFIG_SEND_TYPE1_RAT_ID_BROKEN_SHIFT)
                                & UARTBAM_CXM_RAT_INDEX_CONFIG_SEND_TYPE1_RAT_ID_BROKEN_MASK);
      if (enable_rat_msg_to_fifo)
      {
         cxm_rat_index_config |= UARTBAM_CXM_RAT_INDEX_CONFIG_TYPE7_RXFIFO_TYPE_MASK | 
                                 UARTBAM_CXM_RAT_INDEX_CONFIG_TYPE7_4_RXFIFO_MASK;
      }

      cxm_rat_index_config |= (rat_msg_type & UARTBAM_CXM_RAT_INDEX_CONFIG_RAT_INDEX_IDENTIFIER_MASK);
    
      REG_OUT(uart->base, CXM_CONFIG, cxm_config);
      REG_OUT(uart->base, CXM_TX_CONFIG, cxm_tx_config);
      REG_OUT(uart->base, CXM_DIR_UART_TR, cxm_dir_uart_tr);
      REG_OUT(uart->base, CXM_RAT_INDEX_CONFIG, cxm_rat_index_config);
   }
   else
   {
       REG_IN(uart->base, CXM_CONFIG, &cxm_config);
...
static uint32 HALuart_bam_GetCxmRatEventStatus(HALUartObject *uart)
{
   ...
   rat_events = (rx_stat & UARTBAM_CXM_RX_STAT_IRQ_BROKEN_RAT_ID_STAT) >>
                UARTBAM_CXM_RX_STAT_IRQ_BROKEN_RAT_ID_STAT_SHIFT;

   //To clear the event bits set.
   REG_OUT(uart->base, CXM_RX_STAT, rx_stat);
