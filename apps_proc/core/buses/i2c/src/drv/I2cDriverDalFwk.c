...
/*=========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/buses/i2c/src/drv/I2cDriverDalFwk.c#1 $
  $DateTime: 2020/01/30 22:49:35 $$Author: pwbldsvc $
 
  When     Who    What, where, why
  -------- ---    -----------------------------------------------------------
  09/02/16 PR     Replace DALSYS_Memset with I2CSYS_Memset   
...
)
{
  ...
  if (NULL == pDrvCtxt) {
     ...
     */
     ...
     uDrvCtxtSize = sizeof(I2CDrvCtxt) + (uNumI2cBuses - 1) * sizeof(I2CDevCtxt);
	 nErr = I2CSYS_Malloc((void **)&pDrvCtxt, uDrvCtxtSize);
	 ...
	 I2CSYS_Memset(pDrvCtxt, 0, uDrvCtxtSize);
  ...
  nErr = I2CSYS_Malloc((void **)&pclientCtxt, sizeof(I2CClientCtxt));
  

  *phDalDevice = NULL;

  if ((DAL_SUCCESS == nErr) && (NULL != pclientCtxt)) {
    I2CSYS_Memset(pclientCtxt, 0, sizeof(I2CClientCtxt));
