...
$Header: //components/rel/core.tx/6.0/buses/i2c/src/dev/I2cPlatSvc.c#1 $
$DateTime: 2020/01/30 22:49:35 $$Author: pwbldsvc $
When     Who    What, where, why
-------- ---    -----------------------------------------------------------
04/11/17 VG     Input validation of EnableClocks and DisableClocks API
01/09/16 VG     Added I2CPLATDEV_DeConfigGpios and I2CPLATDEV_ConfigGpios API
02/11/14 LK     Fixed Compiler warnings.
09/26/11 LK     Created
=============================================================================*/
...
typedef enum I2CPLATSVC_Error
{
   ...
   I2CPLATSVC_ERROR_INVALID_SDA_SCL_ID,
   I2CPLATSVC_ERROR_FAILED_TO_CONFIGURE_GPIO_SDA,
   I2CPLATSVC_ERROR_FAILED_TO_CONFIGURE_GPIO_SCL,
   I2CPLATSVC_ERROR_FAILED_TO_DEINIT_GPIO_SDA,
   I2CPLATSVC_ERROR_FAILED_TO_DEINIT_GPIO_SCL,
   I2CPLATSVC_ERROR_INVALID_AHB_APPSCLK_ID,
} I2CPLATSVC_Error;
...
typedef struct I2CPLATDEV_PlatPropertiesType
{
   ...
   DALGpioIdType  i2cqup_sdaid;
   DALGpioIdType  i2cqup_sclid;
   DalTlmm_GpioConfigIdType  i2cqup_sdacfg; 
   DalTlmm_GpioConfigIdType  i2cqup_sclcfg; 
...
)
{
   ...
   dalRes = DalTlmm_GetGpioId(pDev->hTlmm, pDev->platProps.pSCLGPIO, &pDev->platProps.i2cqup_sclid);
   if (DAL_SUCCESS != dalRes) {
      return I2CPLATSVC_ERROR_FAILED_TO_CONFIGURE_GPIO_0;
   }

   dalRes = DalTlmm_GetGpioId(pDev->hTlmm, pDev->platProps.pSDAGPIO, &pDev->platProps.i2cqup_sdaid);
   if (DAL_SUCCESS != dalRes) {
...
/** @brief Configure the scl,sda gpios.
...
{  
  int32 res = I2C_RES_SUCCESS; 
   ...
   do
   {  
          if (!(pDev->platProps.i2cqup_sdaid && pDev->platProps.i2cqup_sclid))
		  {
			res = I2CPLATSVC_ERROR_INVALID_SDA_SCL_ID;
			break;
		  }
		  
		 /* Config SDA Pin */
		  if (DAL_SUCCESS != DalTlmm_ConfigGpioId(pDev->hTlmm,
		  	                                      pDev->platProps.i2cqup_sdaid,
		  	                                      &pDev->platProps.i2cqup_sdacfg))
		  {
			 res = I2CPLATSVC_ERROR_FAILED_TO_CONFIGURE_GPIO_SDA;      
			 break;
		  }
		  /* Config SCL Pin */
		  if (DAL_SUCCESS != DalTlmm_ConfigGpioId(pDev->hTlmm,
		  										  pDev->platProps.i2cqup_sclid, 
		  										  &pDev->platProps.i2cqup_sclcfg))
		  {
			 res = I2CPLATSVC_ERROR_FAILED_TO_CONFIGURE_GPIO_SCL;      
			 break;
		  }
...
/** @brief De-configure the scl,sda gpios.
...
)
{
  ...
  do
   {     
		  ...
		  /* Config SDA Pin */
		  if (DAL_SUCCESS !=  DalTlmm_ConfigGpioIdInactive(pDev->hTlmm,
		  												   pDev->platProps.i2cqup_sdaid))
		  {
			 res = I2CPLATSVC_ERROR_FAILED_TO_DEINIT_GPIO_SDA;      
			 break;
		  }
		  ...
		  if (DAL_SUCCESS !=  DalTlmm_ConfigGpioIdInactive(pDev->hTlmm,
		  												   pDev->platProps.i2cqup_sclid))
		  {
			 res = I2CPLATSVC_ERROR_FAILED_TO_DEINIT_GPIO_SCL;      
			 break;
		  }
   }while(0);
...
)
{
   ...
   if (!(pDev->QupAppClkId && pDev->QupHClkId))
   {
       return I2CPLATSVC_ERROR_INVALID_AHB_APPSCLK_ID;
   }
...
)
{
   ...
   if(!dsr_enqueue(pIrq->dsr_objext,NULL))
   {
...
)
{
   ...
   phAddress = (uint32)pBamIoVec->pBuff;
...
)
{
   ...
   if ( DAL_SUCCESS != DALSYS_GetDALPropertyHandle((DALDEVICEID)uPlatDevId, hProp ) ) {
   ...
   do {
      ...
      if (DAL_SUCCESS != 
           DALSYS_GetPropertyValue(hProp, "GPIO_I2C_SDA_STR", 0, &PropVar )) {
         break;
      } 
      pDev->platProps.pSDAGPIO = PropVar.Val.pszVal;

      if (DAL_SUCCESS != 
           DALSYS_GetPropertyValue(hProp, "GPIO_I2C_SCL_STR", 0, &PropVar )) {
         break;
      } 
      pDev->platProps.pSCLGPIO = PropVar.Val.pszVal;

     if ( DAL_SUCCESS !=
			DALSYS_GetPropertyValue(hProp, "I2CQUP_GPIO_SDA_CFG", 0, &PropVar) ) {
	    break;
	  }
	  pDev->platProps.i2cqup_sdacfg = *(DalTlmm_GpioConfigIdType *)PropVar.Val.pStruct;
	  
      if ( DAL_SUCCESS !=
			DALSYS_GetPropertyValue(hProp, "I2CQUP_GPIO_SCL_CFG", 0, &PropVar) ) {
	    break;
	  }
	  pDev->platProps.i2cqup_sclcfg = *(DalTlmm_GpioConfigIdType *)PropVar.Val.pStruct;
...
)
{
   ...
   if ( NULL == pDev->hInterruptCtrl ) {
	  ...
	  dsr_result =  
	  (dsr_create(&pDev->irq.dsr_objext,IstHook,(void*)pIstData,DSR_PRI_MEDIUM,DSR_CONTEXT_ISR) == 1 ? I2C_SUCCESS : 1);

	  if(dsr_result == I2C_SUCCESS)
      {
        dalRes = DAL_InterruptControllerDeviceAttach(DALDEVICEID_INTERRUPTCONTROLLER,
                                              &pDev->hInterruptCtrl);
   ...
   pDev->irq.IstHook = NULL;
   pDev->irq.pIsrData = pIsrData;
   pDev->irq.pIstData = NULL;
