...
$Header: //components/rel/core.tx/6.0/buses/i2c/src/dev/I2cDeviceQup.c#1 $
$DateTime: 2020/01/30 22:49:35 $$Author: pwbldsvc $ 
 
When     Who    What, where, why
-------- ---    -----------------------------------------------------------  
02/22/17 VG     Added changes to support async apis
09/15/16 PR     Added API to know in which mode I2C Driver Running
09/01/16 PR     Changed IST and ISR interrupt context perameter based on qurt    
...
{
   return ( NULL != pDev->asyncCall.pCallbackFn );
}
...
)
{
   ...
   {
      osRes = I2CSYS_Wait(pDev->hQupEvt, pSeqInfo->uSeqTimeoutUs * SYSTEM_TICK_COUNT);
...
)
{
   ...
   do {
      ...
      if ( pDev->devProps.bBamAvailable ) {
         ...
         pDev->seqInfo.outSeqInfo.aOutBamBuff = (uint8 *)pDev->seqInfo.physMem.pVirtAddress;
...
/** @brief Query regarding mode.

    This Function Query is it in polling or interrupt mode.
	@param[in]     hDev            Device handle.
