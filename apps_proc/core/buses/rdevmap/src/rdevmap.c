...
EXTERNALIZED FUNCTIONS
  ...
  rdm_is_busy()
    Check if the RDM is in busy state.
...
Copyright (c) 2000-2017,2019,2020, 2022 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/buses/rdevmap/src/rdevmap.c#4 $ $DateTime: 2022/11/30 21:57:51 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/28/22   rk      Added support for Diag service over UART
03/17/22   rk      Initialize non-persistent flag after reading previous port mapping
06/25/20   ss      Added back USB_SER6 port support.
5/24/19    ss      Memory optimisation on Threadx
03/25/19   ss      Unable to send AT COmmands Over UART Issue Fix.
03/08/18   rc      Added USB_SER6 port.
12/13/17   pr      Added USB_SER4.
03/21/17   rc      Added USB_MDM2 device.
08/03/16   rc      Assigned the DIAG service to the USB_SER1_DEV as default 
                   service
07/10/16   rc      Ported the QURT specific API's for OS based services
10/10/14   vv      Made RDM_SYSM_SRVC default for USB_SER3 device
10/10/14   vv      Made RDM_SYSM_SRVC default for USB_SER3 device
10/06/14   vv      Added DCI service and device
10/13/14   vv      DUN service defaulted to MHI
07/25/14   vv      Added DPL device.
10/28/13   sb      Additions to support services over PCIe/USB.
08/19/13   vv      Added MHI devices.
...
/** RDM Ulog handle for logging.   */
ULogHandle           rdm_ulog;

/** Critical section for RDM */
//rex_crit_sect_type rdm_crit_sect;
qurt_mutex_t rdm_crit_sect;
#define rdm_enter_crit_section() qurt_mutex_lock(&rdm_crit_sect)
#define rdm_leave_crit_section() qurt_mutex_unlock(&rdm_crit_sect)
#define rdm_init_crit_section()  qurt_mutex_init(&rdm_crit_sect)
/*===========================================================================
                                Macros
===========================================================================*/
...
/*------------------------------------------------------------------*/
 ...
 rdm_map[RDM_USB_SER4_DEV].default_srvc_id    = RDM_DATA_SRVC;
#endif

 rdm_map[RDM_USB_SER5_DEV].default_srvc_id    = RDM_DATA_SRVC;

 rdm_map[RDM_USB_SER6_DEV].default_srvc_id    = RDM_RSP_SRVC;
 
#ifdef FEATURE_HS_USB_SMCT_PORT
  rdm_map[RDM_USB_SMCT_DEV].default_srvc_id   = RDM_DATA_SRVC;
#endif

  // IPC Router support anticipated to be available on all B-Family
  rdm_map[RDM_USB_IPCR_DEV].default_srvc_id   = RDM_DATA_SRVC;
  rdm_map[RDM_USB_DCI_DEV].default_srvc_id    = RDM_DATA_SRVC;
  rdm_map[RDM_USB_LWIP_DEV].default_srvc_id   = RDM_DATA_SRVC;

  // DPL is used by DATA
  rdm_map[RDM_USB_DPL_DEV].default_srvc_id   = RDM_DATA_SRVC;
  
  /* DIAG defaults to MHI (PCIE) */
  //rdm_map[RDM_MHI_DIAG_DEV].default_srvc_id  = RDM_DIAG_SRVC;

  /* ALT DIAG defaults to USB */
  #ifdef FEATURE_DIAG_SIO_UART
  rdm_map[RDM_UART1_DEV].default_srvc_id  = RDM_DIAG_SRVC;
  #else
  rdm_map[RDM_USB_DIAG_DEV].default_srvc_id  = RDM_DIAG_SRVC;
  #endif
 

  /* ALT DUN defaults to USB */
  rdm_map[RDM_USB_MDM_DEV].default_srvc_id  = RDM_DUN_SRVC;
  rdm_map[RDM_USB_MDM2_DEV].default_srvc_id  = RDM_DUN_SRVC;

  /* WWAN Service defaults to USB */
  rdm_map[RDM_USB_NET_WWAN_DEV].default_srvc_id   = RDM_DATA_WWAN_SRVC;

  /* System Monitor defaults to MHI (PCIE) */
  //rdm_map[RDM_MHI_SSR_DEV].default_srvc_id   = RDM_SYSM_SRVC;

  /* Remote EFS defaults to MHI (PCIE) */
  //rdm_map[RDM_MHI_EFS_DEV].default_srvc_id   = RDM_RMTS_SRVC;

  /* CSVT defaults to MHI (PCIE) */
  //rdm_map[RDM_MHI_CSVT_DEV].default_srvc_id  = RDM_CSVT_SRVC;

  /* DUN defaults to MHI (PCIE) */
  //rdm_map[RDM_MHI_DUN_DEV].default_srvc_id   = RDM_DUN_SRVC;
  //rdm_map[RDM_MHI_DUN_DEV].default_srvc_id   = RDM_ALT_DUN_SRVC;

  /* DCI defaults to MHI (PCIE)  */
  //rdm_map[RDM_MHI_DCI_DEV].default_srvc_id   = RDM_DIAG_DCI_SRVC;
...
@brief
  Make call to remote EFS entity to read in the last known port mappings.
...
FUNCTION RDM_IS_BUSY                                    EXTERNALIZED FUNCTION

@brief
  This function check the state of RDM and returns true if RDM is in IDLE
  state and ready to accept new request (ex: port switching)
...
@return
  Boolean
    True : if RDM is non idle state
    False: if RDM is ready/idle state

@sideeffects
===========================================================================*/
boolean rdm_is_busy (void)
{
   return (rdm_state.state==RDM_IDLE_STATE)?FALSE:TRUE;
}
...
/*------------------------------------------------------------------*/
     ...
     service < RDM_FIRST_SRVC || service >= RDM_SRVC_MAX)
  {
    ...
    ULogFront_RealTimePrintf(rdm_ulog,3,"rdm_assign_port(SRVC=%s, DEV=%s, CB=%x)",
                           rdm_service_names[service],rdm_device_names[device], cbptr);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ULogFront_RealTimePrintf(rdm_ulog, 2, "rdm_notify(SRVC=%s, STATUS=%d)", rdm_service_names[service], status);
...
/*------------------------------------------------------------------*/
  if(!rdm_non_persistent)
  {
	...
	config_item_buffer = malloc(RDM_CONFIG_ITEM_FILE_SIZE);
    if (config_item_buffer != NULL)
    {
...
/*------------------------------------------------------------------*/
      ...
      dev_index < RDM_DEV_MAX; dev_index++)
  {
    ...
    if(srv != RDM_NULL_SRVC)
    {
      ...
      if( !((srv >= RDM_FIRST_SRVC) && (srv < RDM_SRVC_MAX)) )
	  {
		  continue;
	  }
	  
	  if( rdm_service_info[srv].is_remote == is_remote )
	  {
...
void usbotg_turn_off_modem_isr_control(void)
{
  ...
  if(qurt_system_is_irq_mode())
  {
    usbotg_remote_vv_handler(usbotg_turn_on_apps_isr_control);
  }
  else
  {
...
static void rdm_register_all_devices()
{
 ...
 /*---RDM_USB_MDM_DEV---*/
  ...
  device_info.device = RDM_USB_MDM2_DEV;
  #ifdef FEATURE_DATA_RM_NET
    RDM_SET_DEV_NAME(device_info.device_name , "USB MDM2");
  #else
    RDM_SET_DEV_NAME(device_info.device_name , "USB CDC2");
  #endif
  #ifdef FEATURE_USB_CDC_ACM
    device_info.available = TRUE;
  #else
    device_info.available = FALSE;
  #endif
  device_info.sio_port = SIO_PORT_USB_MDM2;
  rdm_register_device(&device_info);
 /*---------------------*/
 ...
 /*---RDM_USB_SER6_DEV----*/
  device_info.device = RDM_USB_SER6_DEV;
  RDM_SET_DEV_NAME(device_info.device_name , "USB SER6");
  device_info.available = TRUE;
  device_info.sio_port = SIO_PORT_USB_SER6;
  rdm_register_device(&device_info);
 /*-----------------------*/
...
/*---RDM_USB_IPCR_DEV----*/
  device_info.device = RDM_USB_IPCR_DEV;
  RDM_SET_DEV_NAME(device_info.device_name , "USB IPCR");
  // IPC Router support anticipated to be available on all B-Family
  device_info.available = TRUE;
  device_info.sio_port = SIO_PORT_USB_IPCR;
  rdm_register_device(&device_info);
 /*-----------------------*/

 
/*---RDM_USB_LWIP_DEV----*/
  device_info.device = RDM_USB_LWIP_DEV;
  RDM_SET_DEV_NAME(device_info.device_name , "USB LWIP");
  device_info.available = TRUE;
  device_info.sio_port = SIO_PORT_USB_LWIP;
  rdm_register_device(&device_info);
 /*-----------------------*/

  /*---RDM_USB_DPL_DEV----*/
  device_info.device = RDM_USB_DPL_DEV;
  RDM_SET_DEV_NAME(device_info.device_name , "USB DPL");
  device_info.available = TRUE;
  device_info.sio_port = SIO_PORT_USB_DPL;
  rdm_register_device(&device_info);
 /*-----------------------*/
...
static void rdm_register_all_services()
{
  ...
  /*---RDM_DIAG_SRVC---*/
   ...
   #endif
   rdm_set_compatibility(RDM_DUN_SRVC,RDM_UART1_DEV);
   ...
   //rdm_set_compatibility(RDM_DIAG_SRVC, RDM_MHI_DIAG_DEV);
  ...
  /*---RDM_DATA_SRVC---*/
    ...
    rdm_set_compatibility(RDM_DATA_SRVC, RDM_USB_SER5_DEV);
    rdm_set_compatibility(RDM_RSP_SRVC, RDM_USB_SER6_DEV);
    rdm_set_compatibility(RDM_DATA_SRVC, RDM_USB_CSVT_DEV);
    rdm_set_compatibility(RDM_DATA_SRVC, RDM_USB_IPCR_DEV);
    rdm_set_compatibility(RDM_DATA_SRVC, RDM_USB_DCI_DEV);
    rdm_set_compatibility(RDM_DATA_SRVC, RDM_USB_LWIP_DEV);
  ...
  /*------------------------*/
   ...
   /*---RDM_DIAG_DCI_SRVC---*/
   //rdm_set_compatibility(RDM_DIAG_DCI_SRVC, RDM_MHI_DCI_DEV);
   rdm_set_compatibility(RDM_DIAG_DCI_SRVC, RDM_USB_DCI_DEV);

   /*---RDM_ALT_DIAG_SRVC--*/
   rdm_set_compatibility(RDM_ALT_DIAG_SRVC, RDM_USB_DIAG_DEV);
   //rdm_set_compatibility(RDM_ALT_DIAG_SRVC, RDM_MHI_DIAG_DEV);

   /*---RDM_ALT_DUN_SRVC--*/
   rdm_set_compatibility(RDM_ALT_DUN_SRVC, RDM_USB_MDM_DEV);
   //rdm_set_compatibility(RDM_ALT_DUN_SRVC, RDM_MHI_DUN_DEV);
...
/*------------------------------------------------------------------*/
  ...
  ULogFront_RealTimePrintf(rdm_ulog, 3, "rdm_send_service_cmd(SRV=%s, CMD=%d, DEV=%s)",
                           rdm_service_names[service], port_cmd, rdm_device_names[device]);
...
static void rdm_enum_to_string(void)
{
  ...
  RDM_DEV_NAME(RDM_USB_SER5_DEV);
  RDM_DEV_NAME(RDM_USB_SER6_DEV);
  RDM_DEV_NAME(RDM_USB_QDSS_DEV);
  RDM_DEV_NAME(RDM_USB_CSVT_DEV);
  RDM_DEV_NAME(RDM_USB_IPCR_DEV);
  RDM_DEV_NAME(RDM_USB_DCI_DEV);
  RDM_DEV_NAME(RDM_USB_LWIP_DEV);
  ...
  /*RDM_DEV_NAME(RDM_MHI_DIAG_DEV);
  RDM_DEV_NAME(RDM_MHI_SAHARA_DEV);
  RDM_DEV_NAME(RDM_MHI_SSR_DEV);
  RDM_DEV_NAME(RDM_MHI_EFS_DEV);
  RDM_DEV_NAME(RDM_MHI_QMI_DEV);
  RDM_DEV_NAME(RDM_MHI_MBIM_DEV);
  RDM_DEV_NAME(RDM_MHI_IPSW_0_DEV);
  RDM_DEV_NAME(RDM_MHI_IPSW_1_DEV);
  RDM_DEV_NAME(RDM_MHI_IPSW_2_DEV);
  RDM_DEV_NAME(RDM_MHI_IPSW_3_DEV);
  RDM_DEV_NAME(RDM_MHI_QDSS_DEV);
  RDM_DEV_NAME(RDM_MHI_SMCT_DEV);
  RDM_DEV_NAME(RDM_MHI_CSVT_DEV);
  RDM_DEV_NAME(RDM_MHI_DUN_DEV);
  RDM_DEV_NAME(RDM_MHI_DCI_DEV);*/
  ...
  RDM_SVC_NAME(RDM_SYSM_SRVC);
  RDM_SVC_NAME(RDM_RMTS_SRVC);
  RDM_SVC_NAME(RDM_DUN_SRVC);
  RDM_SVC_NAME(RDM_CSVT_SRVC);
  RDM_SVC_NAME(RDM_DIAG_DCI_SRVC);
  
  RDM_SVC_NAME(RDM_ALT_DIAG_SRVC);
  RDM_SVC_NAME(RDM_ALT_DUN_SRVC);
  RDM_SVC_NAME(RDM_RSP_SRVC);

  nbytes = snprintf(rdm_fversion, RDM_ROW_WIDTH,
...
//                 line++;
    ...
    else
    {
      ...
      MSG_HIGH("efs_get: errcode %d", efs_errno, 0, 0);
      retval = 0;
    }
...
FUNCTION RDM_LOG_INIT

@brief
  rdm_log_init() initializes the ULog handle.
...
{
 // Initialization 
  ULogFront_RealTimeInit(&rdm_ulog,       // The log to create
                         "RDM Log",       // The log name
