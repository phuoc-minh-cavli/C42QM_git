...
Copyright (c) 1998-2009,2014,2016 Qualcomm Technologies Incorporated.  All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/buses/sio/src/common/sio.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
5/24/19    ss      Memory optimisation on Threadx
04/10/16   PR      Added array bound checking COndition
...
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support open interface", open_ptr->port_id);
...
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %d  does not support close interafce", stream_id);
}
...
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support ioctl interface", port_id);
}
...
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support transmit interface", port_id);
...
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support flush interface", port_id);
}

static void sio_null_disable_device (sio_port_id_type port_id)
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support disable_device interface", port_id);
}

static sio_status_type sio_null_control_open (sio_open_type *open_ptr)
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support control open interface", open_ptr->port_id);
    return SIO_UNAVAIL_S;
}
...
static void sio_null_control_close (sio_stream_id_type stream_id,
                             sio_port_id_type      port_id,
                              void (*close_func_ptr)(void))
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x does not support control_close interface", port_id);
}
...
static void sio_null_control_ioctl (sio_stream_id_type stream_id,
                             ...
                             sio_ioctl_param_type *param)
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x  does not support control_ioctl interface", port_id);
}
...
static void sio_null_control_transmit (sio_stream_id_type stream_id,
                                sio_port_id_type      port_id,
                                 dsm_item_type     *tx_ptr)
{
    MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "SIO port %x does not support control_transmit interface", port_id);
}
...
FUNCTION SIO_LOG_INIT                                     INTERNAL FUNCTION

@brief
  Initializes uLog handle.
...
===========================================================================*/
void sio_log_init(void)
{
  // Initialization 
  ULogFront_RealTimeInit(&sio_ulog,       // The log to create
                         "SIO Log",       // The log name
...
)
{
  ...
  sio_port_id_type     saved_port_id = SIO_PORT_NULL;           /* Saved Port ID           */
...
//  smd_bridge_port_id = saved_port_id = open_ptr->port_id;
  ...
  ULogFront_RealTimePrintf(sio_ulog, 1, "+sio_open () - Port ID : 0x%08x", port_id);
  ...
  */
  ...
  ULogFront_RealTimePrintf(sio_ulog, 2, "-sio_open () - Port ID : 0x%08x, Stream ID : 0x%08x", port_id, stream_id);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ULogFront_RealTimePrintf(sio_ulog, 2, "+sio_close () - Port ID : 0x%08x, Stream ID : 0x%08x", port_id, stream_id);

  driver_iface = sio_get_driver_interface(port_id);
  ASSERT(driver_iface->HW_close);
  (driver_iface->HW_close) (stream_id,port_id, close_func_ptr);

  ULogFront_RealTimePrintf(sio_ulog, 2, "-sio_close () - Port ID : 0x%08x, close fn: 0x%08x", port_id, driver_iface->HW_close);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ( tx_ptr == NULL) 
  {
     MSG_1(MSG_SSID_DFLT, MSG_LEGACY_ERROR, "NULL dsm_item ptr for transmit, stream_id: %d", stream_id);
     return;
  }
...
//  smd_bridge_port_id = saved_port_id = open_ptr->port_id;
  ...
  ULogFront_RealTimePrintf(sio_ulog, 1, "+sio_control_open () - Port ID : 0x%08x", port_id);
  ...
  */
  ...
  ULogFront_RealTimePrintf(sio_ulog, 2, "-sio_control_open () - Port ID : 0x%08x, Stream ID : 0x%08x", port_id, stream_id);
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  ULogFront_RealTimePrintf(sio_ulog, 2, "+sio_control_close () - Port ID : 0x%08x, Stream ID : 0x%08x", port_id, stream_id);

  driver_iface = sio_get_driver_interface(port_id);
  ASSERT(driver_iface->HW_control_close);
  (driver_iface->HW_control_close) (stream_id,port_id, close_func_ptr);

  ULogFront_RealTimePrintf(sio_ulog, 2, "-sio_control_close () - Port ID : 0x%08x, close fn: 0x%08x", port_id, driver_iface->HW_control_close);
