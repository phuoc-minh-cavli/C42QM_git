...
/*===========================================================================
  ...
  $Header: //components/rel/core.tx/6.0/services/osal/qurt/src/osal_qurt.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
08/09/16     ph    Updated osal_thread_exit() API.
...
timer_group_type  diag_timer_group = {0};  /* non-deferrable to wakeup from sleep */
#endif
/*Contain last error returned by timer APIs())*/
timer_error_type osal_time_last_err=TE_SUCCESS;
/*
 *=========================
 * Synchronization
 * Critical sections
...
int osal_delete_mutex(osal_mutex_arg_t *mutex)
{
    ...
    if(mutex)
    {
        qurt_mutex_destroy(&(mutex->handle));
        ret_value = OSAL_SUCCESS;
    }
    return(ret_value);
}
...
void osal_unlock_mutex(osal_mutex_arg_t *mutex)
{
    if (mutex)
    {
        qurt_mutex_unlock(&(mutex->handle));
    }
}
...
int osal_set_sigs(osal_tcb_t *tcb_ptr, osal_sigs_t sig, osal_sigs_t *prev_sig)
{
    ...
    if (tcb_ptr != NULL )
    {
        ...
        qurt_signal_set(&(tcb_ptr->current_sigs), sig);
...
int osal_reset_sigs(osal_tcb_t *tcb_ptr, osal_sigs_t sig, osal_sigs_t *prev_sig)
{
    ...
    if (tcb_ptr != NULL)
    {
        qurt_signal_clear(&(tcb_ptr->current_sigs), sig);
...
int osal_get_sigs(osal_tcb_t *tcb_ptr, osal_sigs_t *current_sig)
{
    ...
    if (tcb_ptr && current_sig)
    {
        ...
        *current_sig = qurt_signal_get(&(tcb_ptr->current_sigs));
...
osal_sigs_t osal_thread_wait(osal_tcb_t *tcb_ptr, osal_sigs_t sigs)
{
    ...
    if(tcb_ptr)
    {
     return_value = qurt_signal_wait(&(tcb_ptr->current_sigs), sigs,QURT_SIGNAL_ATTR_WAIT_ANY);
    }
     return (return_value);
}
/*===========================================================================
...
int osal_create_thread(osal_tcb_t *tcb_ptr, osal_thread_attr_t *attr, int *pri)
{
  ...
  osal_sigs_t return_sigs;
    ...
    if(tcb_ptr && attr)
    {
        ...
        {
          qurt_signal_init(&(tcb_ptr->current_sigs));
          ...
          //Qurt it self allocates the stack memory
          ...
          qurt_mutex_lock(&tcb_map_table_lock);
          for( ctr=0; ctr < QURT_MAX_THREADS; ctr++ )
          {
            /* look up in the table for the thread ID corresponding to the
            tcb pointer */
            ...
            if( qurt_tcb_map_table[ctr].tcb_id == 0)
            {
              ...
              {
                ret_value = OSAL_FAILURE;
                qurt_mutex_unlock(&tcb_map_table_lock);
                return(ret_value);
              }
              else
              {
          ...
          qurt_mutex_init(&tcb_ptr->tcb_msg_lock);
...
void osal_update_tcb_map_table(int thread_id, osal_tcb_t *tcb_ptr )
{
    int ctr = 0;
    ...
    if(tcb_ptr)
    {
        ...
        for( ctr=0; ctr < QURT_MAX_THREADS; ctr++ )
        {
            ...
            if( qurt_tcb_map_table[ctr].thd_id == thread_id )
            {
                ...
                qurt_signal_destroy(&(tcb_ptr->current_sigs));
                qurt_mutex_destroy(&tcb_ptr->tcb_msg_lock);
...
void osal_thread_join(osal_tcb_t *tcb_ptr, int *status)
    {
        ...
        if(tcb_ptr)
        {
            ret_val= qurt_thread_join((osal_thread_t)(tcb_ptr->thd_id), NULL);
            if( ret_val!=0 )
            {
                MSG_1(MSG_SSID_DIAG, MSG_LEGACY_LOW, "thread_join failed %d",
                                                                        ret_val);
            }
...
void osal_init (void)
{
   ...
   {
      initialized = TRUE;
      qurt_mutex_init(&tcb_map_table_lock);
...
{
    return qurt_system_ints_are_locked();
}
/*===========================================================================
...
{
    return qurt_system_is_irq_mode();
}
...
/*===========================================================================
  ...
  SIDE EFFECTS
       None
  ===========================================================================*/
int osal_tls_key_create(osal_tls_key_t *key, void (*destructor)(void*))
{
...
/*===========================================================================
  ...
  SIDE EFFECTS
       None
  ===========================================================================*/
int osal_tls_key_delete(osal_tls_key_t key)
{
...
/*===========================================================================
  ...
  DEPENDENCIES
       Key must be initialized using osal_tls_key_create.
  ...
  SIDE EFFECTS
       None
  ===========================================================================*/
void *osal_tls_getspecific(osal_tls_key_t key)
{
...
/*===========================================================================
  ...
  SIDE EFFECTS
       None
===========================================================================*/
int osal_tls_setspecific(osal_tls_key_t key, const void* value)
{
