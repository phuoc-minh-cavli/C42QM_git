...
$Header: //components/rel/core.tx/6.0/services/diag/diagbuffer/src/ULogCore.c#1 $
                               ...
                               ULOG_INTERFACE_TYPE interfaceType )
{
  ...
  if ((memoryType & ULOG_ALLOC_LOCAL))
  {
    ...
    /* If maxMemory is more than the maximum size a ulog buffer can have, the size of maxMemory
       is forcefully trimmed to ULOG_MAXIMUM_LOG_SIZE */
    if(maxMemory>ULOG_MAXIMUM_LOG_SIZE)
    {
       maxMemory = ULOG_MAXIMUM_LOG_SIZE;
    }
    //=== Log Buffer ===
...
ULogResult ULogCore_HeaderSet(ULogHandle h, char * headerText)
{
...
#endif
  {
    ...
    log->header = malloc(headerTextLength);
    if (log->header != NULL)
    {
...
ULogResult ULogCore_Connect(ULogHandle * h,  const char * logName)
{
  ...
  if (tempLog == NULL)
  {
    tempLog = malloc(sizeof(ULOG_TYPE));
    if (tempLog == NULL)
    {
      return(DAL_ERROR);
    }
...
ULogHandle ULogCore_TeeAdd(ULogHandle h1, ULogHandle h2)
{
  ...
  {
	ulogContext.teePool = malloc(sizeof(CorePoolType));
    CORE_VERIFY(ulogContext.teePool);
    Core_PoolConfig(ulogContext.teePool, &ULogTeePoolConfig);

    ulogContext.teeLinkPool = malloc(sizeof(CorePoolType));
