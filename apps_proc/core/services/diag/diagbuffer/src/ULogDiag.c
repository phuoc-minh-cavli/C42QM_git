...
Copyright (c) 2003-2013,2016 Qualcomm Technologies Incorporated.
All Rights Reserved.
Qualcomm Confidential and Proprietary

$Header: //components/rel/core.tx/6.0/services/diag/diagbuffer/src/ULogDiag.c#1 $
============================================================================*/
...
static ULogHandle GetExternalHandle(ULogHandle handle)
{
  ...
  {
    newHandle = malloc( sizeof(external_ulog_handle));
    if (newHandle != NULL)
    {
      newHandle->next = NULL;
...
)
{
  ...
  {
    newHandle = malloc(sizeof(ulog_diag_plugin_handle));
    if (newHandle != NULL)
    {
      newHandle->next = NULL;
