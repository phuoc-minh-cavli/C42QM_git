...
Copyright (c) 1999-2016, 2019-2020 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.tx/6.0/services/diag/DCM/common/src/diagbuf.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/10/19   kdey    Porting of Qsockets
07/20/16   nk      Changes to support ThreadX
03/30/16   ph      Remove check for HDLC offloading for TN traffic.
03/10/16   gn      Avoided hdlc encoding if hdlc offloading is supported by
                   peripherals and packet is not a command response for
                   streaming mode also.
02/10/16   is      Add timeout timer when flushing diagbuf in diagbuf_flush_timeout()
02/10/16   tbg     Added null checks in HDLC disable function
08/17/15   xy      Added support for HDLC removal feature
06/19/14   sa      Added support to send an event to debug diag drops.
04/24/14   rh      Added registration to set immediate diagbuf drain during shutdown.
...
08/07/13   ph      Removed the assert in diagbuf_drain to exit gracefully without crash.
...
10/05/12   rh      Moved DIAGBUF_SIO_TX_ITEM_SIZ to diagbuf_v.h
...
extern void diagcomm_ctrl_send_buf_retry(diagcomm_enum_port_type port_num);
...
static void diagbuf_immediate_delivery(void)
{
  ...
  diagbuf_ctrl_commit_threshold(0);  /* If we set the threshold to 0, we never invoke the timer.*/
  osal_set_sigs(&diag_fwd_task_tcb, DIAG_FWD_DRAIN_TIMER_SIG, &return_sigs);
  diag_clr_fwd_drain_timer();        /* Timer length of 0 turns this timer off. */
  diag_ctrl_fwd_drain_timer_len(1);  /* Therefore we need to set it to something very short instead. */
  diag_tx_notify();
}
   ...
   byte stream_id )
{
  ...
  /* Currently, we support only version 0 and 1 pkts */
  if (version == 0)
  {
    return diagbuf_alloc(code, length, nice, diagbuf_alloc_request, chain, version);
  }
  else  /*Allocate chain diagbuf pkts for extended logs*/
  {
    ...
    ptr_start = (byte *)diagbuf_alloc(DIAG_LOG_F, buf_used, 0, DIAGBUF_ALLOC_REQUEST_LOG, 1, 1);
    ...
    switch (version)
    {
       case 1:
         ...
         ptr_payload->version = 1;
         ptr_payload->proc_id = DIAG_MY_PROC_ID;
         ptr_payload->id = id;
         break;
       default:
         diagbuf_shorten(ptr_start, 0);
         return NULL;
    }
...
)
{
  ...
  if (new_length == 0) {
    ...
    if( (pkt_hdr->chain == DIAGBUF_SECOND_PKT_CHAIN) && (pkt_hdr->version == DIAGBUF_VERSION_1))
    {
      /*Mark the first pkt in diagbuf as a hole*/
      pkt_hdr = ((diagbuf_header_type *)pkt_hdr) - diagbuf_ver_len[DIAGBUF_VERSION_1-1];
...
diagbuf_status_enum_type diagbuf_send_pkt_single_proc (diag_send_desc_type *desc)
{
  ...
  byte *outbound = NULL; /* outbound should contain the data_ptr of the current
                            dsm item. */
  dsm_item_type *outbound_dsm_ptr = NULL;
  uint64 tx_cnt = tx_cnt_sav;

/* Getting the cmd or data dsm item into outbound. */
  if(desc)
    outbound_dsm_ptr = diag_get_dsm_item_ptr(desc->is_cmd_rsp_type);

  if(outbound_dsm_ptr)
    outbound = (byte*)outbound_dsm_ptr->data_ptr;
  else
    outbound = NULL;
  ...
  if (!(diagcomm_status() && DIAG_CHECK_MASK (diag_cx_state, DIAG_CX_COMM_S)))
  {
    ...
    if (outbound != NULL)
    {
      diagcomm_send (item_ptr, 0, FALSE);
      outbound = NULL;
    }
  ...
  {
    /* This case is actually for force flush*/
    outbound_dsm_ptr = diag_get_dsm_item_ptr(FALSE);
    ...
    if (outbound != NULL)
    {
      item_ptr = outbound_dsm_ptr; // Updating item_ptr
      outbound_used = item_ptr->used;
      diagcomm_send (item_ptr, outbound_used, FALSE);
  ...
  {
    item_ptr = outbound_dsm_ptr;
    /* Updating enc.dest and enc.dest_last so that stale values are not used.
       This makes sure  that correct dsm item is being accessed in case of 
       switching between draining data and cmd rsp.*/
    if(item_ptr)
    {
      enc.dest = (void*)(item_ptr->data_ptr + item_ptr->used);
      enc.dest_last = (void*)(item_ptr->data_ptr + item_ptr->size - 1);
    }
    while (status == DIAGBUF_READY_S && desc->state < DIAG_SEND_STATE_COMPLETE)
    {
      ...
      if (outbound == NULL)
      {
        ...
        /*Do non HDLC encoding if not command response */
        if(!desc->priority)
        {
      ...
      else
      {
        ...
        if (outbound)
        {
          ...
          // updating used field of item_ptr after writing
          item_ptr->used = (uint32) enc.dest - (uint32) outbound;
        }
        ...
        if (((desc->terminate) && (new_chain || dsm_full) && (item_ptr)) || ((desc->priority) && (item_ptr))) {
                ...
                diagcomm_send (item_ptr, outbound_used, desc->is_cmd_rsp_type);

                tx_cnt += outbound_used;
                outbound = NULL;
                new_chain = FALSE;
                dsm_full = FALSE;
                /* Clearing enc.dest and enc.dest_last after draining cmd rsp
                   so that stale values are not used. */
                enc.dest = NULL;
                enc.dest_last = NULL;
...
DESCRIPTION
  This function is called when IPCRouter has signaled diag to resend the buffer if
  it was not able to send all the data successfully in first attempt
  
  This is needed only for control channel. It is not handled intentionally for
  command channel since command respones should be sent successfully at the first attempt
  i.e. no handling of flow control for command responses while forwarding the command to
  peripheral since for some reason if first few commands are delayed, the subsequent incoming
  commands are handled only after previous pending commands are sent causing delays.
  
  Since there is no outgoing data from APPS to Slave on data channel, retry mechanism
  need not be handled.
...
FUNCTION IS_HDLC_DISABLE_RSP_PKT

DESCRIPTION
  This function checks if the packet is the HDLC disable cmd rsp packet

PARAMETERS
  src_desc - packet that will be sent out to tools

RETURN VALUE
  TRUE - the packet is HDLC disable cmd rsp packet
  FALSE - otherwise

===========================================================================*/
booleanis_hdlc_disable_rsp_pkt(diag_send_desc_type *src_desc)
{
  uint32 pkt_len;
  diagdiag_hdlc_disable_rsp_type *cmd_rsp_pkt  = NULL;

  if (src_desc == NULL)
  {
    return FALSE;
  }
  /* packet length =  address of last byte - address of first byte + 1 */
  pkt_len = (uint32)(src_desc->last) - (uint32)(src_desc->pkt) + 1;
  if (pkt_len == sizeof(diagdiag_hdlc_disable_rsp_type) && (src_desc->pkt) != NULL)
  {
    cmd_rsp_pkt = (diagdiag_hdlc_disable_rsp_type *)(src_desc->pkt);
    if (((byte *)cmd_rsp_pkt)[0] == DIAG_SUBSYS_CMD_F && ((byte *)cmd_rsp_pkt)[1] == DIAG_SUBSYS_DIAG_SERV
         && (((uint16 *)cmd_rsp_pkt)[1] == DIAGDIAG_HDLC_DISABLE_APP))
    {
      if ((cmd_rsp_pkt->framing_version == DIAG_HDLC_DISABLE_VERSION) && (cmd_rsp_pkt->result == DIAG_HDLC_DISABLE_SUCCESS))
      {
        return (TRUE);
      }
    }
...
boolean diag_hdlc_disabled = FALSE ;
boolean diag_hdlc_disable_cmd_received = FALSE ;
  ...
  if (diag_hdlc_disabled == FALSE)
  {
    if (diag_hdlc_protocol)
    {
      ...
      if (src_desc && enc)
      {
        ...
        if (diag_hdlc_disable_cmd_received == TRUE && is_hdlc_disable_rsp_pkt(src_desc))
        {
          flag = TRUE;
        }
    ...
    if(dest && dest_last)
    {
      switch (diag_enhc_state)
      {
        case DIAG_STATE_START:
          /*write 7E + version*/
          *dest++ = DIAG_ASYNC_CONTROL_CHAR;
          *dest++ = DIAG_HDLC_DISABLE_VERSION;
          if (dest > dest_last)
          {
            /*no space available*/
            diag_enhc_state = DIAG_STATE_WRITE_FIRST_BYTE_LEN;
          ...
          /*have enough space, go to write the first byte*/
          diag_enhc_state = DIAG_STATE_WRITE_FIRST_BYTE_LEN;
        ...
        case DIAG_STATE_WRITE_FIRST_BYTE_LEN:
          ...
          if (dest_last - dest >= 1)
          {
            ...
            /* Had enough space - goto writing the packet*/
            diag_enhc_state = DIAG_STATE_WRITE_PACKET;
            goto write_packet_non_hdlc;
          }
          else if (dest_last - dest == 0)
          {
            /* Memorize location of the first byte of length field */
            diag_enhc_length1 = dest++;
            diag_enhc_state = DIAG_STATE_WRITE_SECOND_BYTE_LEN;
            break;
          }
        case DIAG_STATE_WRITE_SECOND_BYTE_LEN:
          ...
          /*have enough space, go to write the packet*/
          diag_enhc_state = DIAG_STATE_WRITE_PACKET;

        case DIAG_STATE_WRITE_PACKET:
        write_packet_non_hdlc:
        {
          int length_current_chunk = 0;
          diag_enhc_length  = src_last - src + 1;
          if (diag_enhc_length > (dest_last - dest + 1))
          {
            /*packet cannot fit in current item*/
            length_current_chunk = dest_last - dest + 1;
          }
          else
          {
          ...
          if (dest > dest_last)
          {
            if (diag_enhc_length == 0)
            {
              /*the whole packet has been copied*/
              diag_enhc_state = DIAG_STATE_END;
            }
            break;
          }
        ...
        case DIAG_STATE_END:
        {
          ...
          {
            /*data packet is not ended, there are more data needs to be copied*/
            diag_enhc_state = DIAG_STATE_WRITE_PACKET;
          }
        }
      }
...
diagbuf_status_enum_type diagbuf_drain (osal_sigs_t mask)
{
  ...
  static diag_send_desc_type send = {NULL, NULL, DIAG_SEND_STATE_COMPLETE, FALSE};
   byte *start_pos = NULL;
  ...
  /*Exit if context is not diag */
  if (osal_thread_self () != &diag_task_tcb)
  {
    MSG(MSG_SSID_DIAG, MSG_LEGACY_ERROR, "diagbuf_drain called in context other than diag");
...
void diagbuf_flush_timeout(unsigned int timeoutVal)
{
  ...
  /* Set timer to quit diagbuf draining after timeout value. */
  if( timeoutVal > 0 )
  {
  ...
  */
  ...
  {
    sigs = diag_wait (DIAG_TX_SIG | DIAG_DRAIN_TIMER_SIG | DIAG_SIO_TOUT_TIMER_SIG);
    if (sigs & DIAG_TX_SIG)
    {
    ...
    if (sigs & DIAG_SIO_TOUT_TIMER_SIG)
    {
      break;
    }
  }

  osal_reset_timer(&diag_sio_tout_timer);
  return_val = osal_reset_sigs(&diag_task_tcb, DIAG_SIO_TOUT_TIMER_SIG, &return_sigs);
  ASSERT(OSAL_SUCCESS == return_val);
