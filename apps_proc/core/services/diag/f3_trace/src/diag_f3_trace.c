...
Copyright (c) 2004-2014, 2020 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/

/*==========================================================================

                        EDIT HISTORY FOR MODULEW
$Header: //components/rel/core.tx/6.0/services/diag/f3_trace/src/diag_f3_trace.c#2 $
...
03/20/13   rh      Moved file from Error Services to Diag 
...
#define DIAG_F3_TRACE_IS_HEADER_VALID(x) ((x)==ERR_F3_TRACE_VALID_HEADER_FLAG)
...
extern uint8 err_f3_trace_wrap_flag; // from diag_f3_trace_nzi.c
extern uint8 err_f3_trace_buffer[DIAG_F3_TRACE_BUFFER_SIZE]; // from diag_f3_trace_nzi.c
extern uint32 err_f3_trace_buffer_head; // from diag_f3_trace_nzi.c
extern err_f3_trace_id_type err_f3_trace_buffer_id; // from diag_f3_trace_nzi.c

// these can be changed before diag_f3_trace_init runs, so defaults need to be 
// statically initialized 
uint8 diag_f3_trace_control_mask = DIAG_F3_TRACE_CONTROL_MASK_DEFAULT_VAL;
uint8 diag_f3_trace_detail_mask = DIAG_F3_TRACE_DETAIL_MASK_DEFAULT_VAL;
...
static uint32 diag_f3_trace_find_next_entry(uint32 *buffer_position)
{
   ...
   err_f3_trace_hdr_type record_hdr;
...
static int diag_f3_trace_calculate_entry_size(uint32 buffer_offset)
{
   ...
   diag_f3_trace_from_buffer((uint8*) &record_hdr, buffer_offset, sizeof(err_f3_trace_hdr_type));

   if (!DIAG_F3_TRACE_IS_HEADER_VALID(record_hdr.header_flag))
   {
      /* Expecting a valid header here, but we didn't get one.
       * Return error code. */
      return(ERR_F3_TRACE_BUFFER_CORRUPTED);
   }
   ...
   switch (type)
   {
   case ERR_F3_TRACE_MSG_BIT:
      /* Increment past header and const type, at least */
      entry_size = sizeof(err_f3_trace_hdr_type)+ sizeof(void*);

      details = DIAG_F3_TRACE_GET_DETAILS(record_hdr.config);

      if (details & ERR_F3_TRACE_SAVETIME)
      {
         entry_size += sizeof(err_f3_trace_tstamp_type);
      }

      if (details & ERR_F3_TRACE_SAVEVARS)
      {
   ...
   case ERR_F3_TRACE_EVT_BIT:
      /* Events no longer supported */
   case ERR_F3_TRACE_LOG_BIT:
   default:
      return(ERR_F3_TRACE_BUFFER_CORRUPTED);
   }
   ...
   if (found_tail)
   {
      ...
      *buffer_tail = current_position;
      do
      {
         ...
         if (size != ERR_F3_TRACE_BUFFER_CORRUPTED)
         {
...
void err_f3_save_msg(const void * constant_data_ptr, msg_arg_type * args, uint8 nargs, void* msg, uint32 type, uint8 version, void* param, uint8 arg_bytes)
{
   /* Basic data checks */
   ...
   else
   {
      ...
      //else if (type==DIAG_F3_TRACE_QSHRINK30 &&(((msg_v3_const_hash*)constant_data_ptr)->ssid_index < msg_v3_ss_num_table_entries))
      //{
      ...
      else if (type==DIAG_F3_TRACE_QSHRINK40)
      {
          if ((qsr_hash_v4 >= (uint32)qsr_4_0_msg_aligned_start_address) &&
                (qsr_hash_v4 < (uint32)qsr_4_0_msg_unaligned_start_address))
           {
			...
			// remove 8K aligment. we are  not doing in lcs
			// below is same as subtracting start of aligned address of section.
   ...
   /* Proceed with saving msg to buffer */
   {
      ...
      if (savevars)
      {
        ...
        /*If the message is Qshrink4 then use the arg_bytes to calculate
        byte_sized*/
        // use arg_bytes
        if(arg_bytes != 0)
        {
      ...
      if (savetime)
      {
        ...
        if (msg)
        {
           ...
           if (type == DIAG_F3_TRACE_STANDARD)
           {
      	   ...
      	   {
      	      qw_equ(tstamp, ((msg_v4_store_type*)msg)->hdr.ts);
      	   }
           else
           {
