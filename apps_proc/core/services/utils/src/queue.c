...
$Header: //components/rel/core.tx/6.0/services/utils/src/queue.c#1 $ $DateTime: 2020/01/30 22:49:35 $ $Author: pwbldsvc $
...
#else  
   #define q_lock( q ) qurt_mutex_lock( &q->mutex )
   #define q_unlock( q ) qurt_mutex_unlock( &q->mutex )
   #define q_lock_init( q ) qurt_mutex_init( &q->mutex )
   #define q_lock_delete( q ) qurt_mutex_destroy( &q->mutex )
