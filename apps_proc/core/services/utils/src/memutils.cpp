...
Copyright (c) 2004 - 20134   by Qualcomm Technologies Incorporated.  All Rights Reserved.
============================================================================ */
...
$Header: //components/rel/core.tx/6.0/services/utils/src/memutils.cpp#1 $ 
...
/* Mutex type object */
qurt_mutex_t mem_utils_mutex;

/* ------------------------------------------------------------------------
** Defines
** ------------------------------------------------------------------------ */
// For managing mutex related operations
#define MEMUTILS_MUTEX_INIT()  qurt_mutex_init(&mem_utils_mutex);
#define MEMUTILS_MUTEX_LOCK()  qurt_mutex_lock(&mem_utils_mutex);
#define MEMUTILS_MUTEX_FREE()  qurt_mutex_unlock(&mem_utils_mutex);
...
void HeapCacheFindMemoryOverwrite(void)
{
   ...
   MEMUTILS_MUTEX_FREE()
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"HeapCacheFindMemoryOverwrite(): "
          "FEATURE_MEMUTILS_HEAP_CACHE_ENHANCED_DEBUGGING NOT defined");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"HeapCacheLeakReport(): "
          "FEATURE_MEMUTILS_HEAP_CACHE_ENHANCED_DEBUGGING NOT defined");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"FEATURE_MEMUTILS_HEAP_CACHE_USAGE_STATISTICS not defined");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_leak_report(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_initialize(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_terminate(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_new(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_delete(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
}
...
#else
  MSG(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"memory_tracker_get_location(): FEATURE_MEMUTILS_MEMORY_TRACKING "
           "feature NOT enabled");
#endif
   return;
}
