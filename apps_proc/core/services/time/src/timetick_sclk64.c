...
$Header: //components/rel/core.tx/6.0/services/time/src/timetick_sclk64.c#1 $
...
#if defined(FEATURE_QDSP6) || defined(FEATURE_QUBE)
#include "qmutex.h"
#endif
...
void timetick_sclk64_mutex_init(void);

/*MACRO for locking/unlocking the mutex*/
#define ATS_INTLOCK()  \
  qurt_mutex_lock(&timetick_sclk64_mutex); \
  
#define ATS_INTFREE()  \
  qurt_mutex_unlock(&timetick_sclk64_mutex); \
...
void timetick_sclk64_mutex_init(void)
{
  qurt_mutex_init( &timetick_sclk64_mutex );
}
