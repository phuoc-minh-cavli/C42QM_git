...
$Header: //components/rel/core.tx/6.0/services/time/src/time.c#1 $
...
extern void timetick_sclk64_mutex_init(void);

#ifndef FEATURE_TIME_NO_RPC
extern void time_remote_mutex_init(void); 
#endif
...
timer_type heap_debug_timer;
extern mem_heap_type amss_mem_heap;

/*-----------------------------------------------------------------------------
  Time static information
-----------------------------------------------------------------------------*/
#if (((defined(IMAGE_MODEM_PROC)&& !defined T_MDM9X00) || defined FEATURE_QDSP6) && defined(FEATURE_TOD_MULTIPROC_SYNC))
/* Apps to modem offset For WM builds */
static int64 time_apps_to_modem_offset = 0;
#endif /*(((defined(IMAGE_MODEM_PROC)&& !defined T_MDM9X00) || defined FEATURE_QDSP6) && defined(FEATURE_TOD_MULTIPROC_SYNC))*/
...
static time_fn_map_fp_type time_fn_map_time_fps;
...
void time_mutex_init(void);

/*MACRO for locking/unlocking the mutex*/
#define ATS_INTLOCK()  \
  qurt_mutex_lock(&time_q6_mutex); \
  
#define ATS_INTFREE()  \
  qurt_mutex_unlock(&time_q6_mutex); \
...
=============================================================================*/
static void time_mutex_init(void)
  {
  qurt_mutex_init ( &time_q6_mutex );
}
...
void time_init( void )
{
  ...
  time_mutex_init(); 
  timetick_sclk64_mutex_init();
  #ifndef FEATURE_TIME_NO_RPC
  time_remote_mutex_init(); 
  #endif
...
)
{
  ...
  if(ATS_TOD == info_ptr->ats_genoff_update.base_type) 
  {
    ...
    MSG_1(MSG_SSID_ATS, MSG_LEGACY_HIGH,"time_remote_update_seq_num: Remote modem update seq num =  %d",
            time_remote_update_get_seq_num(),0,0);
    ...
    memscpy(&ts_value[0],sizeof(time_type),&info_ptr->ats_genoff_update.offset,sizeof(time_type));
        ...
        && !defined(BUILD_BOOT_CHAIN))
         ...
         MSG_3(MSG_SSID_ATS, MSG_LEGACY_HIGH,"mm_fusion_debug:7K:ts_val[0]=0x%x, ts_val[1]=0x%x, offset=0x%x",ts_value[0],ts_value[1],info_ptr->ats_genoff_update.offset);
...
{
   osal_enter_crit_sect((osal_crit_sect_t*) (((mem_heap_type *)ptr)->memheap_crit_sect));
...
{
   osal_exit_crit_sect((osal_crit_sect_t*) (((mem_heap_type *)ptr)->memheap_crit_sect));
  
} /* END mem_heap_leave_crit_sect */

void heap_debug_cb(int data)
{
	mem_heap_enter_crit_sect(&amss_mem_heap);
	
    MSG_6(MSG_SSID_DFLT, MSG_LEGACY_HIGH,"heap profile data total_bytes = %d used_bytes = %d heap_used_bytes = %d free_memory_bytes = %d max_used = %d  max_request = %d ",
               amss_mem_heap.total_bytes, amss_mem_heap.used_bytes, amss_mem_heap.heap_used_bytes, (amss_mem_heap.total_bytes - amss_mem_heap.heap_used_bytes), amss_mem_heap.max_used, amss_mem_heap.max_request);	
	
	mem_heap_leave_crit_sect(&amss_mem_heap);
}
...
void time_init_config( void )
{
  ...
  #endif /* IMAGE_APPS_PROC || (T_MDM9X00 && !FEATURE_QDSP6 */
  ...
  time_genoff_add_proxy_base ( ATS_GPS, ATS_TOD);
  ...
  time_genoff_add_proxy_base ( ATS_MFLO, ATS_TOD);
  ...
  timer_def_osal(&heap_debug_timer, NULL, TIMER_FUNC1_CB_TYPE, heap_debug_cb, 0);
  timer_set(&heap_debug_timer, 1, 1, T_SEC);
...
)
{
  ...
  temp_genoff_args.base        = (0 != time_genoff_get_offset(ATS_TOD)) ? ATS_TOD : ATS_USER;
...
#else
  MSG(MSG_SSID_ATS, MSG_LEGACY_HIGH,"time_set_ms(): Not implemented");
...
)
{
  ...
  time_sclk_get( uptime_ms, (uint64)-1);
...
void time_sync_time_of_day( void )
{
  ...
  if ( source != TIME_SOURCE_32KHZ )
  {
   ...
   {
     MSG(MSG_SSID_ATS, MSG_LEGACY_ERROR,"Invalid technology trying to set time.Setting of time ignored");
     return;
   }
...
#endif /* IMAGE_MODEM_PROC && !T_MDM9X00 || !FEATURE_QDSP6 */
...
) 
{
  ...
  MSG(MSG_SSID_ATS, MSG_LEGACY_HIGH,"mm_fusion_debug:Registered with Server Modem");
