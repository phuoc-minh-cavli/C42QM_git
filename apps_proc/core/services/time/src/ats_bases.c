...
$Header: //components/rel/core.tx/6.0/services/time/src/ats_bases.c#2 $
...
/* 1000 msec threshold for efs write */
#define TIMER_INACCURACY_UPDATE_THRESHOLD_MS 1000
...
)
{
  ...
  if(time_base == ATS_PSM)
  {
    time_genoff_updates_per_storage( ptime_genoff, f_name, 
		  TIMER_INACCURACY_UPDATE_THRESHOLD_MS );
  }
  else
  {
    time_genoff_updates_per_storage( ptime_genoff, f_name, 
