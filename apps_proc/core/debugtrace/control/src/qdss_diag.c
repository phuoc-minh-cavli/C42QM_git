...
PACK(void *) qdss_diag_pkt_handler(PACK(void *) pReq, uint16 pkt_len)
{
   ...
   if (NULL != pReq)    {
      ...
      case QDSS_FILTER_ETM:
         QDSS_HANDLE_DIAG_CMD(qdss_filter_etm);
         break;
...
PACK(void *) qdss_diag_pkt_handler_ext(PACK(void *) pReq, uint16 pkt_len)
{
   ...
   if (NULL != pReq)    {
      ...
      case QDSS_MAP_CTI:
         QDSS_HANDLE_DIAG_CMD(qdss_map_cti);
                 break;

      case QDSS_UNMAP_CTI:
         QDSS_HANDLE_DIAG_CMD(qdss_unmap_cti);
                 break;

      case QDSS_QUERY_CTI:
         QDSS_HANDLE_DIAG_CMD(qdss_query_cti);
         break;

      case QDSS_RESET_CTI:
         QDSS_HANDLE_DIAG_CMD(qdss_reset_cti);
         break;

      case QDSS_TRIGGER_CTI:
         QDSS_HANDLE_DIAG_CMD(qdss_trigger_cti);
         break;

      case QDSS_CORRELATE_GPIO:
         QDSS_HANDLE_DIAG_CMD(qdss_correlate_gpio);
         break;
      ...
      case QDSS_AGENT_TEST:
         QDSS_HANDLE_DIAG_CMD(qdss_agent_test);
         break;
...
static const diagpkt_user_table_entry_type qdss_diag_ext_tbl[] =
{
   ...
   { QDSS_DIAG_PROC_ID | QDSS_AGENT_TEST,    // range: from
     QDSS_DIAG_PROC_ID | QDSS_TEST_CMD,      // range: to
     qdss_diag_pkt_handler_ext }             // handler fcn
