...
int qdss_tpdm_set(uint8 tpdm_id,uint8 state) 
{
  ...
  TRY(nErr,DAL_TPDA_EnableDisable(qdss.hTPDA,tpda_name,tpdm_name,state));
...
int qdss_tpdm_get_gpr(uint8 tpdm_id, uint8 reg_num, uint32 reg_type,uint32 *pval)
{
  ...
  TRY(nErr,DAL_TPDM_Get_GPR(qdss.hTPDM,tpdm_name,reg_num,reg_type,pval));
                              ...
                              uint32 reg_type) 
{
   ...
   nErr = qdss_tpdm_get_gpr(pReq->tpdm_id,
                            pReq->reg_num,
                            reg_type,
                            &val);
   pRsp->val = val;
...
int qdss_tpdm_set_gpr(uint8 tpdm_id, uint8 reg_num, uint32 reg_type,uint32 val)
{
  ...
  TRY(nErr,DAL_TPDM_Set_GPR(qdss.hTPDM,tpdm_name,reg_num,reg_type,val));
                                    ...
                                    qdss_tpdm_dsb_get_param_rsp *pRsp)
{
   ...
   nErr = qdss_tpdm_dsb_get_param(pReq->tpdm_id,
                                  pReq->param_id,
                                  &val);
   pRsp->val = val;
   return nErr;
                                    ...
                                    qdss_tpdm_cmb_get_param_rsp *pRsp)
{
   ...
   nErr =  qdss_tpdm_cmb_get_param(pReq->tpdm_id,
                                  pReq->param_id,
                                  &val);
   pRsp->val = val;
   return nErr;
...
#define QDSS_HANDLE_TPDM_GPR_CMD(cmd)                                   \
   if (pkt_len < (sizeof(qdss_tpdm_req) + sizeof(cmd##_req) -1 )) {     \
      pRsp = diagpkt_err_rsp(DIAG_BAD_LEN_F, pReq, pkt_len);            \
   }                                                                    \
   else {                                                               \
                               ...
                               int pkt_len)
{
   ...
   case QDSS_TPDM_GET_GPR:
      QDSS_HANDLE_TPDM_GPR_CMD(qdss_tpdm_get_gpr);
      break;

   case QDSS_TPDM_SET_GPR:
      QDSS_HANDLE_TPDM_GPR_CMD(qdss_tpdm_set_gpr);
      break;
