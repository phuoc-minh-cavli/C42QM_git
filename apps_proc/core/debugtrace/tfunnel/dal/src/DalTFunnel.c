/*=============================================================================
  Copyright (c) 2009 Qualcomm Technologies Incorporated.
           All Rights Reserved.
                  ...
                  $Header: //components/rel/core.tx/6.0/debugtrace/tfunnel/dal/src/DalTFunnel.c#1 $
=============================================================================*/
...
TFunnel_DeviceInit(DalDeviceHandle *h)
{
   ...
   qurt_mem_pool_t TFunnel_Pool;
   qurt_mem_region_t TFunnel_region;
   qurt_mem_region_attr_t TFunnel_region_attr;
...
*/
   ...
   pTable = (uint32 *)malloc(NumFunnels*sizeof(uint32));
   if(pTable != NULL)	/* Need to check this condition */
   {
      HAL_qdss_tfunnel_MemAllocTable((uint32) pTable);
   }
   else
   {
      return DAL_ERROR;
   ...
   */
   ...
   for(i=0;i<PropVar.dwLen;i++) /*iterate through length of the sequence*/
   {
	  ...
	  TFunnel_region_attr.mapping_type = QURT_MEM_MAPPING_NONE;
      TFunnel_region_attr.region_type = QURT_MEM_REGION_DEVICE;
      TFunnel_region_attr.cache_attribs = QURT_MEM_CACHE_DEVICE;
      TFunnel_region_attr.permission = QURT_PERM_FULL;
      TFunnel_region_attr.virtaddr = (qurt_addr_t)-1;
      TFunnel_region_attr.size = 4096;
	  virtaddr_temp = (qurt_addr_t)-1;	  
	  TFunnel_region_attr.physaddr = pFunnelhysAddr[i];
	  
	  if ( QURT_EOK != qurt_mem_map_static_query(&virtaddr_temp, TFunnel_region_attr.physaddr, TFunnel_region_attr.size, QURT_MEM_CACHE_DEVICE_SHARED, QURT_PERM_READ_WRITE))
	  {
		nErr = qurt_mem_pool_attach("HWIO_POOL", &TFunnel_Pool);
		if (QURT_EOK != nErr)
		{
			return nErr;
		}
		nErr = qurt_mem_region_create(&TFunnel_region, 4096, TFunnel_Pool, &TFunnel_region_attr );
		if (QURT_EOK != nErr)
		{
			return nErr;
		}
		nErr = qurt_mem_region_attr_get(TFunnel_region, &TFunnel_region_attr);
		if (QURT_EOK == nErr)
		{
			nErr = qurt_mapping_create (TFunnel_region_attr.virtaddr, TFunnel_region_attr.physaddr, TFunnel_region_attr.size, TFunnel_region_attr.cache_attribs, TFunnel_region_attr.permission);
			if (QURT_EOK != nErr)	  
			{
				return nErr;
			}
			HalDevId = i;
			HAL_qdss_tfunnel_HalInit(TFunnel_region_attr.virtaddr,HalDevId);
		}
	  }
	  else
	  {
	    TFunnel_region_attr.virtaddr = (uint32) virtaddr_temp;
		HalDevId = i;
		HAL_qdss_tfunnel_HalInit(TFunnel_region_attr.virtaddr,HalDevId);
	  }	
   }
