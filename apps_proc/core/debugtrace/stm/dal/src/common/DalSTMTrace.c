/*=============================================================================
  Copyright (c) 2009 Qualcomm Technologies Incorporated.
           All Rights Reserved.
                  ...
                  $Header: //source/qcom/qct/core/debugtrace/main/latest/stm/dal/src/common/DalSTMTrace.c#4 $
=============================================================================*/
...
STMTrace_DeviceDeInit(DalDeviceHandle *h)
{
  ...
  /* also delete the list of ports here */
      ...
      qdss_osal_mutex_destroy(&stmMutex);
   

   if(pDevCtxt->portRangeDMA.pBitMask)
   {
      free(pDevCtxt->portRangeDMA.pBitMask);
   }

   if(pDevCtxt->portRange.pBitMask)
   {
      free(pDevCtxt->portRange.pBitMask);
   }
...
STMTrace_NewPort( DalDeviceHandle * h, uint32 * uPort, uint32 * uPortDMA)
{
	 ...
	 qurt_mem_region_t STMPort_region;
	 qurt_mem_region_attr_t STMPort_region_attr;
	 ...
	 currRange = (void *)malloc(sizeof(STMPortRangeItem));
	 if(currRange == NULL)	/* Need to check the condition */
	 {
			qdss_osal_mutex_unlock(&stmMutex);
			return DAL_ERROR;
	 }
	 ...
	 if (QURT_EOK != qurt_mem_map_static_query(&memInfo.VirtualAddr, pDevCtxt->uSTMSPBaseAddr +(currRange->portRange.uBasePort * STM_PORT_SIZE) , 4096, QURT_MEM_CACHE_DEVICE_SHARED, QURT_PERM_READ_WRITE))
	 {
		 qurt_mem_pool_t STMPort_Pool;	
		 STMPort_region_attr.mapping_type = QURT_MEM_MAPPING_NONE;
		 STMPort_region_attr.region_type = QURT_MEM_REGION_DEVICE;
		 STMPort_region_attr.cache_attribs = QURT_MEM_CACHE_DEVICE;
		 STMPort_region_attr.permission = QURT_PERM_FULL;
		 STMPort_region_attr.physaddr = pDevCtxt->uSTMSPBaseAddr + (currRange->portRange.uBasePort * STM_PORT_SIZE);
		 STMPort_region_attr.virtaddr = (qurt_addr_t)-1;
		 STMPort_region_attr.size = 4096;
		
		 nErr = qurt_mem_pool_attach("STATIC_POOL", &STMPort_Pool);	
		 if (QURT_EOK != nErr)
		 {
			_FreeBit(pMainPortRange->pBitMask, pMainPortRange->uNumPorts/MAX_STM_PORTS_PER_PAGE, uBitPos);
			free(currRange->portRange.pBitMask);
			free(currRange);
			qdss_osal_mutex_unlock(&stmMutex);
			return DAL_ERROR;
		 }
		 nErr = qurt_mem_region_create(&STMPort_region, 4096, STMPort_Pool, &STMPort_region_attr );
		 if (QURT_EOK != nErr)
		 {
			_FreeBit(pMainPortRange->pBitMask, pMainPortRange->uNumPorts/MAX_STM_PORTS_PER_PAGE, uBitPos);
			free(currRange->portRange.pBitMask);
		 ...
		 nErr = qurt_mem_region_attr_get(STMPort_region, &STMPort_region_attr);
		 if (QURT_EOK != nErr)
		 {
			_FreeBit(pMainPortRange->pBitMask, pMainPortRange->uNumPorts/MAX_STM_PORTS_PER_PAGE, uBitPos);	
			free(currRange->portRange.pBitMask);
		 ...
		 nErr = qurt_mapping_create (STMPort_region_attr.virtaddr, STMPort_region_attr.physaddr, STMPort_region_attr.size, STMPort_region_attr.cache_attribs, STMPort_region_attr.permission);
		 if (QURT_EOK != nErr)	  
		 {
			_FreeBit(pMainPortRange->pBitMask, pMainPortRange->uNumPorts/MAX_STM_PORTS_PER_PAGE, uBitPos);	
			free(currRange->portRange.pBitMask);
		...
		memInfo.VirtualAddr = STMPort_region_attr.virtaddr;
	 }
