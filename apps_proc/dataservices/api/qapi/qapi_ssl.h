...
/** TLS version 1.3. */
#define QAPI_NET_SSL_PROTOCOL_TLS_1_3                  0x34
...
/**< TLS1.3 Cipher AES 128 GCM SHA256. */
#define QAPI_NET_TLS13_AES_128_GCM_SHA256                   0x1301
/**< TLS1.3 Cipher AES 256 GCM SHA384. */
#define QAPI_NET_TLS13_AES_256_GCM_SHA384                   0x1302
/**< TLS1.3 Cipher CHACHA20 POLY1305 SHA256. */
#define QAPI_NET_TLS13_CHACHA20_POLY1305_SHA256             0x1303

/**< TLS1.3 Key Exchange group secp256r1. */
#define QAPI_NET_TLS13_KEY_EXCHANGE_GROUP_SECP256R1 23
/**< TLS1.3 Key Exchange group secp384r1. */
#define QAPI_NET_TLS13_KEY_EXCHANGE_GROUP_SECP384R1 24
/**< TLS1.3 Key Exchange group secp512r1. */
#define QAPI_NET_TLS13_KEY_EXCHANGE_GROUP_SECP521R1 25
/**< TLS1.3 Key Exchange group x25519. */
#define QAPI_NET_TLS13_KEY_EXCHANGE_GROUP_X25519    29
...
typedef struct __qapi_Net_SSL_Cert_List_s {
    ...
    /**< Certificate name. See #QAPI_NET_SSL_MAX_NUM_CERTS and
         #QAPI_NET_SSL_MAX_CERT_NAME_LEN. @newpagetable*/
 ...
 */
typedef struct __qapi_Net_SSL_RPK_Cert_s
{
    uint8_t *pubkey_Buf;
    /**< Public key buffer. */
...
typedef struct __qapi_Net_SSL_Cert_Info_s
{
    ...
    union
    {
       ...
       qapi_Net_SSL_RPK_Cert_t rpk_cert;  /**< Raw public key certificate. @newpagetable */
 ...
 *     one or more certificates. @newpage
 *
 *
 */
...
typedef enum
{
  ...
  QAPI_NET_SSL_EXTENDED_CONFIG_MIN_TLS_PROTOCOL = 4,
  /**< Sets minimum TLS protocol to support. \n
   * Data type is uint8, value range as TLS protocol version value.
   */

  QAPI_NET_SSL_EXTENDED_CONFIG_TLS13_KEY_EXCHANGE_GROUP = 5,
  /**< Sets key-exchange algorithm for TLS13. \n
   * Data type is uint8[], each item is the enum value of key-exchange group (QAPI_NET_TLS13_KEY_EXCHANGE_GROUP_xxx).
   */

  QAPI_NET_SSL_EXTENDED_CONFIG_TLS13_KEY_EXCHANGE_NEGOTIATE = 6,
  /**< Enable/disable KeyExchange negotiate in TLS13. \n
   * Data type is uint8, value range [0, 1]. \n
   * 1: Enables negotiate. \n
   * 0: Disables negotiate.
   */

  QAPI_NET_SSL_EXTENDED_CONFIG_ALLOW_SERVER_RPK = 7, 
  /**< Allows server to pick raw public key certificate. \n
   * For server authentication, data type is uint32, value range [0, 1].  \n
  ...
  /**< Option to accept or discard an alert with epoch=0 after handshake process is complete. \n
   * Per RFC specification, SSL should discard packets with epoch=0 after handshake,
   * but in some special network environments, a DTLS session may be lost and the server cannot fill the 
   * correct epoch and sequence number. In this case, the server sends an alert where epoch=0 to notify
   * session is down and the client must redo the handshake.
   * Note: There is a Denial of Service (DoS) risk to enable this option.
   * Data type is uint8, value range [0, 1].  \n
   * 1: SSL accepts alert packet with epoch=0 as a normal alert.  \n
   * 0: SSL discards the alert packet with epoch=0. */

  QAPI_NET_SSL_EXTENDED_CONFIG_RFC_COMPATIBLE_EXT_MASTER_SECRET_FEATURE = 9,
  /**< Option to configure compatibility of Extended Master Secret feature. \n
   * Per RFC7627, the master secret uses the Extended Master Mecret to bind
   * to the log of the full handshake that computes it, thus preventing 3shake attacks.\n
   * The Extended Master Secret feature is not applicable to TLS 1.3, but the configured value can
   * be used if the TLS version falls back to 1.2 or lower.

   * Data type is uint8, value range [0, 1].  \n
   * 1: Not compatible with server that does not support extended master secret.  \n
   * 0: Compatible with server that does not support extended master secret. */
   ...
   * Per RFC5746, the client can either continue a handshake if the server does not support
   * secure renegotiation, or abort the handshake with an alert if the client wishes to ensure
   * that renegotiation attacks are impossible.
   *
   * For TLS1.3, which does not support renegotiation, this option is a meaningful fallback
   * scenario.
   *
   * Data type is uint8, value range [0, 1]. \n
   * 1: Not compatible with server that does not support secure renegotiation. \n
   * 0: Compatible with server that does not support secure renegotiation. */

  QAPI_NET_SSL_EXTENDED_CONFIG_ENABLE_RENEG_EXTENSION = 11,
  /**< Enables use of Renegotiation_Info extension instead of the Signaling Cipher 
       Suite Value (SCSV).
   * Per RFC5746, the client can either include SCSV or an empty Renegotiaton_Info extension
   * to indicate support of secure renegotiation. SCSV is included by default.
   *
   * Data type is uint8, value range [0, 1]. \n
   * 1: Use Renegotiation_Info extension in ClientHello. \n
   * 0: Use SCSV in ClientHello. */

   QAPI_NET_SSL_EXTENDED_CONFIG_ENABLE_NON_SECURE_TLS12_OPTIONS = 12,

  /**<Option to enable Non-secure Cipher Suites, enables DHE Key Length less than 1024, and 
   * allows CA to use the signature algorithm as SHA1 for TLS1.2. \n
   *
   * Data type is uint8, value range [0, 1].   \n
   * 1: Enables non-secure options.  \n
   * 0: Disables non-secure options. \n  
  */
 ...
 *
 * If set max_frag_len is 0, the default configuration will be applied. 
 ...
 * the user can choose to configure max_frag_len and set max_frag_len_neg_disable to
 * 1 to disable negotiation and still get the benefits of a smaller fragment
 * length. 
 
 * When negotiation is disabled, any value < 16 KB can be configured for max_frag_len. 
 * When value @ge 16 KB, the max_frag_len is set to 16384 and the above limiations do not apply.
 *
 ...
 * Data\_Services 1.6.0  &  Add support for RPK certificate type. @tblendline
 * }
 *
 ...
 *                 The maximum length of the name allowed is #QAPI_NET_SSL_MAX_CERT_NAME_LEN,
 ...
 * @versiontable{2.0,2.45,
 * Data\_Services 1.2.0  &  Introduced. @tblendline
 * Data\_Services 1.6.0  &  Deprecated. @tblendline
 * }
 * @ingroup qapi_accept_ssl_conn
 * @deprecated API is deprecated in dataservices version 1.6.0.
 ...
 * @versiontable{2.0,2.45,
 * Data\_Services 1.2.0  &  Introduced. @tblendline
 * Data\_Services 1.6.0  &  Add option support for TLS1.3 and RPK. @tblendline
 * }
