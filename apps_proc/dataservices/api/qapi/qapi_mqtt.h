...
enum QAPI_NET_MQTT_CONN_STATE {
  ...
  QAPI_NET_MQTT_ST_DEAD_E, /**< MQTT connection is dead. @newpage */
};
...
typedef struct __qapi_Net_MQTT_Sock_Opts_s
{
    ...
    /**< Socket option value. */
    uint32_t opt_len;
    /**< Socket option length. @newpagetable */
...
struct qapi_Net_MQTT_config_s {
  ...
  qapi_Net_SSL_Cert_t cert; /**< SSL certificate details. @newpagetable */
#endif
} ;
...
typedef enum {
  ...
  QAPI_NET_MQTT_EXTENDED_CONFIG_PUBACK_TIMEOUT = 6,	/**< */
  QAPI_NET_MQTT_EXTENDED_CONFIG_MAX = QAPI_NET_MQTT_EXTENDED_CONFIG_PUBACK_TIMEOUT, 	/**< */
...
/**< Timeout for Acknowledgement of publish message (sec). */
...
/** Structure for topic string length in a publish message. */
typedef struct
{
  int len;		/**< Topic string length in publish message. */
  char* data;	/**< Topic string data in publish message. */
} qapi_Net_MQTTLenString_t;

/** Structure for a topic in a publish message. */
typedef struct
{
  char* cstring;						/**< Topic in publish message. */
  qapi_Net_MQTTLenString_t lenstring;	/**< Structure for topic string length in a publish message. */
 ...
 * 
 * @note1hang This macro is only used in DAM Spac.
 */
...
qapi_Status_t qapi_Net_MQTT_Handler(UINT id, UINT a1, UINT a2, UINT a3, UINT a4, UINT a5, UINT a6, UINT a7, UINT a8, UINT a9, UINT a10, UINT a11, UINT a12);
 ...
 *
 * @return
 * #QAPI_OK on success or < 0 on failure. @newpage
 */
 ...
 *
 * @param[in] hndl MQTT handle.
 * @param[in] allow_unsub_pub  Condition that allows this behavior.
 *
 * @return
 * #QAPI_OK on success or < 0 on failure.
 */
 ...
 * }
 *   Sets the extended configuration parameters.
 *   It is expected that the user will call this API
 *   before these parameters will be used
 *   in another API.
 *
 * @param[in] hndl MQTT handle.
 * @param[in] ext_option  Option type to be set.
 * @param[in] val         Values to be set.
 * @param[in] val_len     Values length.
 *
 * @return
 * #QAPI_OK on success or < 0 on failure.  @newpage
 */
 ...
 * @param[out] topic_n  Returned MQTTString, the MQTT topic in the publish.
 ...
 * - #QAPI_ERR_NO_ENTRY -- Failure to retreive the handle.
 * - #QAPI_ERR_INVALID_PARAM -- Pointer passed has a null value.
 * - #QAPI_ERR_NOT_SUPPORTED -- Unsupported extended configuration options.
 * - #QAPI_ERROR -- All other failures.
 */
