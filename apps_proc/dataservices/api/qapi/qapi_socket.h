/*===================================================================
Copyright (c) 2016-2023 Qualcomm Technologies, Inc. and/or its subsidiaries.
All rights reserved.
 ...
 * @details
 * The BSD socket interface API is a collection of standard functions
 * that allow the application to include internet communications capabilities.
 * The BSD socket interface relies on Client-Server architecture,
 ...
 * follow the BSD socket programming guidelines and check for
 * return values of each function.

 * There is a chance that an operation may fail because of resource limitations.
 * For example, the send function might be able to only send some data, but not
 * all of it in a single call. A subsequent call with the rest of the data is
 * required.
 
 * In other cases, an application thread might need to sleep to allow the system
 * to clear its queues, process data, etc.
 *
 * @}
 */
...
#define  TXM_QAPI_SOCKET_SELECT_V2       TXM_QAPI_SOCKET_BASE + 22
#define  TXM_QAPI_SOCKET_FD_ZERO_V2      TXM_QAPI_SOCKET_BASE + 23
#define  TXM_QAPI_SOCKET_GET_FD_CLR_V2   TXM_QAPI_SOCKET_BASE + 24
#define  TXM_QAPI_SOCKET_GET_FD_SET_V2   TXM_QAPI_SOCKET_BASE + 25
#define  TXM_QAPI_SOCKET_GET_FD_ISSET_V2 TXM_QAPI_SOCKET_BASE + 26
...
#define     SO_RST_SEM     0x1018      /**< Reset so_semaphore to socket my_semaphore. */
...
#define     TCP_MAX_RXT    0X2005        /**< Set the maximum number of retxn. */
...
#define		TCP_REASSMVAL  		       0x2011		 /**< Set timer to flush TCP reassembly queue. */
#define		TCP_MSL       		       0x2012		 /**< Set Maximum Segment Life. */
#define		TCP_PERSIST_PROBE	       0x2013		 /**< Maximum probes in case of zero window. */
#define		TCP_RTO_MIN	   		       0x2014		 /**< Set Minimum retransmission timeout value. */
#define		TCP_RTO_MAX	   			   0x2015		 /**< Set Maximum retransmission timeout value. */
#define		TCP_MAX_SACK_BLOCKS	   	   0x2016		 /**< Set SACK blocks in Selective Acknowledgement (SACK) option. */
...
#define		IP_IRE_TMO			18	   /**< Maximum lifetime of an IP reassembly timeout entry (IRE) entry. */
#define		IP_IPR_MAX_FRAGS	19	   /**< Maximum number of fragments that can be stored in a reassembly queue. */
...
#define     MSG_TRUNC           0x40      /**< Get MAX message length received  , to be set in qapi rcv/rcvfrom flag option . */
...
/** Pre-pending QAPI to internal socket structures' names to avoid conflict with internal
 *  structures. */
...
typedef struct
{
   ...
   /**< File descriptor array. @newpagetable */
} fd_set_v2;
...
#define FD_ISSET(handle, set)   qapi_fd_isset((handle), (set))  /**< Tests to see if a file descriptor is part of the set after select() returns. @newpage*/
...
#define qapi_select_v2(a,b,c,d)         ((int)           (_txm_module_system_call12)(TXM_QAPI_SOCKET_SELECT_V2     , (ULONG) a, (ULONG) b, (ULONG) c, (ULONG) d, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
#define qapi_fd_zero_v2(a)              ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_SOCKET_FD_ZERO_V2    , (ULONG) a, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
#define qapi_fd_clr_v2(a,b)             ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_SOCKET_GET_FD_CLR_V2 , (ULONG) a, (ULONG) b, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
#define qapi_fd_set_v2(a,b)             ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_SOCKET_GET_FD_SET_V2 , (ULONG) a, (ULONG) b, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
#define qapi_fd_isset_v2(a,b)           ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_SOCKET_GET_FD_ISSET_V2 , (ULONG) a, (ULONG) b, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
 ...
 * Errors:
 * - EINVAL: Invalid arguments passed to set the value. \n 
 * - #ENOTCONN: Tries to set socket options that require socket to be in a
 *              connected state for setting the same. \n
 ...
 *
 * @return
 * The number of bytes sent, or -1 if an error occurred and error number is set appropriately. \n
 *  Errors: \n
 *  - #ENOTCONN: Socket is in disconnected state. \n
 *  - #EWOULDBLOCK: If the socket is in non-blocking state, a send timeout is set, and
 *                no data is received for that duration. \n
 ...
 *@ingroup qapi_select
 *
 * Enhanced qapi_select with increased number of socket file descriptors.
 * Monitors multiple socket handles, waiting until one or more of them becomes "ready"
 * for some class of I/O operation, e.g., read, write, etc. 
 *
 * The call causes the calling process to block waiting for activity on any 
 * list of sockets. Arrays of socket handles are passed for read, write, 
 * and exception events. A timeout (in milliseconds) is also passed.
 *
 * @param[in] rd            Pointer to a list of read socket handles.
 ...
 *@ingroup qapi_fd_zero
 *
 * Enhanced qapi_fd_zero with with increased number of socket file descriptors.
 * Initializes a socket that is set to zero.
 *
 * @param[in] set  Pointer to a list of sockets.
 *
 ...
 * @ingroup qapi_fd_clr
 *
 * Enhanced qapi_fd_clr with with increased number of socket file descriptors.
 *
 * Removes a socket from the socket set.
 *
 * @param[in] handle    Socket handle returned from qapi_socket().
 ...
 * @ingroup qapi_fd_set
 *
 * Enhanced qapi_fd_set with with increased number of socket file descriptors.
 *
 * Adds a socket to the socket set.
 *
 * @param[in] handle    Socket handle returned from qapi_socket().
