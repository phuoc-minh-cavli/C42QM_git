...
#define QAPI_NET_ICMP_TIMEX_FRAG_ASSEMBLY_TIME_EXCEEDED 1    /**< Fragment reassembly time exceeded. */
 ...
 * Retrieves the physical address and physical address length of an
 *        interface.
 *
 * @details Note that all arguments must not be 0.
 * This function does not allocate space for the address,
...
typedef struct qapi_Ping_Info_Resp_R2_s
{
 ...
 uint8_t ttl;
 /**< Time to live (TTL) or hop limit is a mechanism that limits the lifespan or lifetime of data in a computer or network. @newpagetable */
