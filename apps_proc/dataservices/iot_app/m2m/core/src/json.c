...
/**
 * @fn static int prv_isReserved()
 * @brief This function is used to check for reserved char
 *
 * @param sign char to be verified if it is reserved
 *
 * @return  1 if sign is reserved
 *          0 if sign is not reserved
 *        
 */
...
/**
 * @fn static int prv_isWhiteSpace()
 * @brief This function is used to if given char is whitespace
 *
 * @param sign char to be verified if it is whitespace
 *
 * @return  1 if sign is whitespace
 *          0 if sign is not whitespace
 *        
 */
...
/**
 * @fn static size_t prv_skipSpace()
 * @brief This function is used to skip spaces
 *
...
/**
 * @fn static int prv_split()
 * @brief This function is used to split given buffer
 *
 * @param buffer    handle to the buffer
 * @param bufferLen length of the buffer
 * @param tokenStartP token start pointer
 * @param tokenLenP length of the token
 * @param valueStartP value start pointer
 * @param valueLenP length of the value
 *
 * @return   -1, on error
 *        
 */
...
/**
 * @fn static int prv_countItems()
 * @brief This function is used to count number of items in buffer
 *
 * @param buffer        pointer to the buffer
 * @param bufferLen     buffer length
 *
...
/**
 * @fn static int prv_parseItem()
 * @brief This function is used to parse item in buffer
 *
 * @param buffer        pointer to the buffer
 * @param bufferLen     buffer length
 * @param recordP       pointer of _record_t structure
 *
 * @return    0 on success
 *        -1 on failure
 */
      ...
      case 2:
        {
          ...
          switch (buffer[index+tokenStart])
          {
            ...
            { 
              ...
              /* Object link - "FFFF:FFFF" (only this format is supported) */              
              if (recordP->type != _TYPE_UNSET) 
                return -1;              
              ...
              /* Validate the object link to ensure ":" embedded within the link */
              while (link_len < index+valueStart+valueLen)
              {
                if (buffer[index+valueStart+link_len] == ':')
                   colon_found = true; 
                               
                link_len += 1; 
              }    

              /* Invalid object link format */ 
              if (!colon_found)
              {
                ds_iot_ulog_err("Invalid object link format - expected 65535:65535"); 
                return -1;  
              } 
              
              recordP->type = _TYPE_OBJLINK;
...
/**
 * @fn static bool prv_convertValue()
 * @brief This function is used to convert value and copy into 
 *        targetP structure
 *
 * @param recordP   pointer to the value to be converted
 * @param targetP   pointer target structure to store value
 *
    ...
    lwm2m_data_t * targetP)
{
  switch (recordP->type)
  {
    ...
    case _TYPE_OBJLINK:
    {
      ...
      /* Object ID should be a maximum size of 16-bit */
      if (obj_id > 65535)
      {
        ds_iot_ulog_err("Invalid object ID - expected to be within the range of (0-65535)"); 
        return false;
      }

      if ( 1 != utils_plainTextToInt64((recordP->value)+i + 1,
                recordP->valueLen -i -1,
                &inst_id))
      {
        return false;
      }

      /* Instance ID should be a maximum size of 16-bit */
      if (inst_id > 65535)
      {
        ds_iot_ulog_err("Invalid instance ID - expected to be within the range of (0-65535)");
        return false;
      }

      value = (obj_id << 16) | (inst_id);
      lwm2m_data_encode_objLink(value, targetP);
...
/**
 * @fn static lwm2m_data_t * prv_findDataItem()
 * @brief This function is used to find an item in the list
 *
 * @param listP     pointer to the list of type lwm2m_data_t
...
/**
 * @fn static uri_depth_t prv_decreaseLevel()
 * @brief This function is used to decrease the given depth level
 *
 * @param level   uri depth level to be decreased
 *
 * @return  uri depth 
 *        
 */
...
/**
 * @fn static lwm2m_data_t * prv_extendData()
 * @brief This function is used to extend the data of lwm2m_data_t structure
 *
 * @param parentP     pointer of lwm2m_data_t to be extended
 *
 * @return      pointer of lwm2m_data_t on success
 *        NULL on failure
 */
static lwm2m_data_t * prv_extendData(lwm2m_data_t * parentP)
{
  ...
  {
    LWM2M_MEMCPY(newP, (parentP->value.asChildren.count + 1) * sizeof(lwm2m_data_t), 
        parentP->value.asChildren.array, parentP->value.asChildren.count *
        sizeof(lwm2m_data_t));
...
/**
 * @fn static int prv_convertRecord()
 * @brief This function is used to convert records into lwm2m data
 *
 * @param uriP  handle to the resource uri
 * @param recordArray handle to record
 * @param count number of records
 * @param dataP handle to lwm2m data
 *
 * @return  -1 on failure
 */
...
/**
 * @fn static int prv_dataStrip()
 * @brief This function is used to strip data from given lwm2m data
 *
 * @param size size of the dataP
 * @param dataP handle to lwm2m data
 * @param resultP lwm2m data handle to store result 
 *
 * @return  -1 on failure
 */
    ...
    lwm2m_data_t ** resultP)
{
  ...
  *resultP = lwm2m_data_new(realSize);
  ...
  for (i = 0 ; i < size ; i++)
  {
    if (dataP[i].type != LWM2M_TYPE_UNDEFINED)
    {
      LWM2M_MEMCPY((*resultP) + j, sizeof(lwm2m_data_t), dataP + i, 
          sizeof(lwm2m_data_t));
          ...
          || dataP[i].type == LWM2M_TYPE_MULTIPLE_RESOURCE)
      {
        ...
        childLen = prv_dataStrip(dataP[i].value.asChildren.count, 
            dataP[i].value.asChildren.array, &((*resultP)[j].value.asChildren.array));
        if (childLen <= 0)
        {
          // skip this one
          j--;
...
/**
 * @fn int json_parse()
 * @brief This function is used to parse the json data
 *
 * @param uriP      pointer to uri
 * @param buffer    pointer to the buffer to be parsed
 * @param bufferLen length of the buffer
 * @param dataP     double pointer to hold result of type lwm2m_data_t
 *
 * @return       parsed count on success
 *        -1 on failure
 */
    ...
    lwm2m_data_t ** dataP)
{
  ...
  LOG_DEBUG("bufferLen: %d, buffer: \"%s\"", bufferLen, (char *)buffer);
  LOG_URI(uriP);
  *dataP = NULL;
  recordArray = NULL;
  parsedP = NULL;
  ...
  LOG_DEBUG("Parsing successful. count: %d", count);
  return count;
...
/**
 * @fn static int prv_serializeValue()
 * @brief This function is used to copy lwm2m data structure values 
 *        into given buffer
 *
 * @param tlvP      handle to lwm2m data
 * @param buffer    handle to store the result data
 * @param bufferLen length of the buffer
 *
 * @return   -1 on failure
 */
    ...
    size_t bufferLen)
{
  ...
  switch (tlvP->type)
  {
    case LWM2M_TYPE_STRING:
      ...
      LWM2M_MEMCPY(buffer, bufferLen, JSON_ITEM_STRING_BEGIN, JSON_ITEM_STRING_BEGIN_SIZE);
      head = JSON_ITEM_STRING_BEGIN_SIZE;

      if (bufferLen - head < tlvP->value.asBuffer.length) return -1;
      LWM2M_MEMCPY(buffer + head, bufferLen - head, tlvP->value.asBuffer.buffer, tlvP->value.asBuffer.length);
      head += tlvP->value.asBuffer.length;

      if (bufferLen - head < JSON_ITEM_STRING_END_SIZE) return -1;
      LWM2M_MEMCPY(buffer + head, bufferLen - head, JSON_ITEM_STRING_END, JSON_ITEM_STRING_END_SIZE);
    ...
    case LWM2M_TYPE_INTEGER:
      {
        ...
        LWM2M_MEMCPY(buffer, bufferLen, JSON_ITEM_NUM, JSON_ITEM_NUM_SIZE);
        ...
        LWM2M_MEMCPY(buffer + head, bufferLen - head, JSON_ITEM_NUM_END, JSON_ITEM_NUM_END_SIZE);
    ...
    case LWM2M_TYPE_BOOLEAN:
      {
        ...
        if (value == true)
        {
          ...
          LWM2M_MEMCPY(buffer, bufferLen, JSON_ITEM_BOOL_TRUE, JSON_ITEM_BOOL_TRUE_SIZE);
          head = JSON_ITEM_BOOL_TRUE_SIZE;
        }
        else
        {
          if (bufferLen < JSON_ITEM_BOOL_FALSE_SIZE) return -1;
          LWM2M_MEMCPY(buffer, bufferLen , JSON_ITEM_BOOL_FALSE, JSON_ITEM_BOOL_FALSE_SIZE);
...
/**
 * @fn int prv_serializeData()
 * @brief This function is used to copy lwm2m data structure data 
 *        into given buffer
 *
 * @param tlvP      handle to lwm2m data
 * @param parentUriStr handle to the parent uri
 * @param parentUriLen length of the parent uri
 * @param buffer    handle to store the result data
 * @param bufferLen length of the buffer
 *
 * @return   -1 on failure
    ...
    size_t bufferLen)
{
  ...
  switch (tlvP->type)
  {
    ...
    default:
      ...
      LWM2M_MEMCPY(buffer, bufferLen, JSON_RES_ITEM_URI, JSON_RES_ITEM_URI_SIZE);
...
//TODO: return value
/**
 * @fn static size_t prv_findAndCheckData()
 * @brief This function is used to find and check data in give lwm2m data
 *
 * @param uriP    handle to resource uri
 * @param level   depth of the uri
 * @param size    number of elements in lwm2m data
 * @param tlvP    handle to lwm2m data
 * @param targetp handle to store result
 *
 * @return   0 on failure
 */
...
/**
 * @fn size_t json_serialize()
 * @brief This function is used to copy given lwm2m data into given buffer
 *
 * @param uriP    handle to resource uri
 * @param size    number of elements in lwm2m data
 * @param tlvP    handle to lwm2m data
 * @param bufferP handle to store result
 *
 * @return   0, on failure
 */
    ...
    uint8_t ** bufferP)
{
  ...
  if (baseUriLen > 0)
  {
    LWM2M_MEMCPY(bufferJSON, PRV_JSON_BUFFER_SIZE, JSON_BN_HEADER_1, JSON_BN_HEADER_1_SIZE);
    head = JSON_BN_HEADER_1_SIZE;
    LWM2M_MEMCPY(bufferJSON + head, PRV_JSON_BUFFER_SIZE - head, baseUriStr, baseUriLen);
    head += baseUriLen;
    LWM2M_MEMCPY(bufferJSON + head, PRV_JSON_BUFFER_SIZE - head, JSON_BN_HEADER_2, JSON_BN_HEADER_2_SIZE);
    head += JSON_BN_HEADER_2_SIZE;
  }
  else
  {
    LWM2M_MEMCPY(bufferJSON, PRV_JSON_BUFFER_SIZE - head, JSON_HEADER, JSON_HEADER_SIZE);
  ...
  LWM2M_MEMCPY(bufferJSON + head - 1, PRV_JSON_BUFFER_SIZE - head + 1, JSON_FOOTER, JSON_FOOTER_SIZE);
  head = head - 1 + JSON_FOOTER_SIZE;

  *bufferP = (uint8_t *)lwm2m_malloc(head);
  if (*bufferP == NULL) return 0;
  LWM2M_MEMCPY(*bufferP, head, bufferJSON, head);
