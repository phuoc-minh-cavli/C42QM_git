                               ...
                               const void *sid)
{
  ...
  tx_byte_allocate(byte_pool_qcli, (VOID **) &topic_str, topic_length + 1, TX_NO_WAIT);
  if(topic_str == NULL)
  {
                               ...
                               uint16_t msg_id)
{
  ...
  MQTT_PRINTF("App %d : Publish Callback type %s, QOS %d msg_id %d\n",
...
void mqtt_cli_connect(int app_id)
{
  ...
  qapi_Net_MQTT_Pass_Pool_Ptr (app_mqttcli_ctx[app_id] ,byte_pool_qcli );
  ...
  MEMSET(&sin6->sin_addr.s_addr,0,sizeof(struct in6_addr));
#endif
  }
...
void mqtt_cli_disconnect(int app_id)
{
  ...
  if(ret) {
    // MQTT_PRINTF("MQTT Disconnect State %d\n", app_mqttcli_ctx->conn_state);
