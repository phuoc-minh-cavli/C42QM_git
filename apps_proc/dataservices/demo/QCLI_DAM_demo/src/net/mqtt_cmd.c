...
extern void mqtt_cli_connect_callback(qapi_Net_MQTT_Hndl_t mqtt,
                                      int32_t reason);
...
QCLI_Command_Status_t conn_mqtt(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
  ...
  if((Parameter_Count < 2) || (Parameter_List == NULL)) {
      MQTT_PRINTF("Usage: mqtt connect");
      MQTT_PRINTF("    [-app <app_id>] connect [-h <ip>] [-p <port>] [-k <time>]\n");
      MQTT_PRINTF("    [-q <Qos>] [-r <retain>] [-C <clean session>],[-e <get extended config>]\n");
      MQTT_PRINTF("    [--will-topic <topic>] [--will-message <message>]\n");
      MQTT_PRINTF("    [-i <Client_Id>] [-u <username>][-pubackt <publish ack timeout]\n");
      MQTT_PRINTF("    [-a <allow_unsub_pub 0/1>] [-nb <non-blocking 0/1>]\n");
      MQTT_PRINTF("    [-ct <connack time out in sec.>][-l <linger time in secs>]\n");
      MQTT_PRINTF("    [-secmode <security_mode>] [-psk <psk_table>]\n");
      MQTT_PRINTF("    [-time <time_validity 0/1>][-header <pass_hdr>], MQTT CLI Connect command\n");
  ...
  for(i = 0; i < Parameter_Count; i++) {
    if(strcmp((char *)Parameter_List[i].String_Value, "-h") == 0) {
      ...
      if(inet_pton(AF_INET, addr_ptr, &addr.a) != 0) {
        ...
        memscpy(&sin4->sin_addr.s_addr, sizeof(struct in_addr), &addr.a,
    ...
    } else if (strcmp((char*)Parameter_List[i].String_Value, "-e") == 0)
    {
      ...
      if(Parameter_List[i].Integer_Is_Valid) {
        if(Parameter_List[i].Integer_Value == 1) {
          retain = true;
        } else if(Parameter_List[i].Integer_Value > 1 ||
    ...
    } else if(strcmp((char *)Parameter_List[i].String_Value, "--will-topic") ==
              0) {
      ++i;
      if(Parameter_List[i].String_Value == NULL) {
        ...
        strlcpy(will_topic[app_id], (char *)Parameter_List[i].String_Value,
                MAX_TOPIC_LEN);
    ...
    } else if(strcmp((char *)Parameter_List[i].String_Value,
                     "--will-message") == 0) {
      ++i;
      if(Parameter_List[i].String_Value == NULL) {
        ...
        while(((char *)Parameter_List[i].String_Value) && (i < Parameter_Count)) {
          ...
          strlcpy(will_message[app_id] + will_message_len,
                  (char *)Parameter_List[i].String_Value,
                  (MAX_MSG - will_message_len));
        ...
        {
            strlcpy(client_id[app_id], (char *)Parameter_List[i].String_Value,
                    CLI_MAX_CLIENT_ID_LEN);
       ...
       QAPI_NET_MQTT_EXTENDED_CONFIG_PSK_TABLE_T))))
  {
    ...
    mqttdemo_cfg[app_id].keepalive_duration = 
         k_alive; //qurt_timer_convert_time_to_ticks(k_alive, QURT_TIME_SEC);
  ...
  if(pass_header == 1)
  {
    ...
    if (linger_time >= 0)
    {
        tx_byte_allocate(byte_pool_qcli, (VOID **) &(mqttdemo_cfg[app_id].sock_options), sizeof(qapi_Net_MQTT_Sock_Opts_t), TX_NO_WAIT);
        ...
        mqttdemo_cfg[app_id].sock_options->level = SOL_SOCKET;
        mqttdemo_cfg[app_id].sock_options->opt_name = SO_LINGER;
        mqttdemo_cfg[app_id].sock_options->opt_len = sizeof(struct linger);
        mqttdemo_cfg[app_id].sock_options->opt_value = &so_linger;
        mqttdemo_cfg[app_id].sock_options_cnt = 1;
    }
    ...
    if(mqttdemo_cfg[app_id].sock_options != NULL)
    {
      tx_byte_release(mqttdemo_cfg[app_id].sock_options);
      mqttdemo_cfg[app_id].sock_options = NULL;
    }
...
QCLI_Command_Status_t pub_mqtt(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
  ...
  if((Parameter_Count <= 4) || (Parameter_List == NULL)) {
    MQTT_PRINTF("Usage: [-app <app_id>] publish [-t <topic>] [-m <message>] [-q <QoS>] [-r "
                "<state>], MQTT CLI Publish command\n");    
    result = QCLI_STATUS_SUCCESS_E;
    goto end;
  }

  //msg = (char *)malloc(MAX_SUB_LEN);
  tx_byte_allocate(byte_pool_qcli, (VOID **) &msg, MAX_SUB_LEN, TX_NO_WAIT);
  if(msg == NULL) {
...
QCLI_Command_Status_t sub_mqtt(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
  ...
  if((Parameter_Count <= 2) || (Parameter_List == NULL)) {
    MQTT_PRINTF("Usage: [-app <app_id>] subscribe [-t <topic>] [-q <QoS>],  MQTT CLI Subscribe "
                "command\n");   
    result = QCLI_STATUS_SUCCESS_E;
    goto end;
  }
  ...
  for(i = 0; i < Parameter_Count; i++) {
    if(strcmp((char *)Parameter_List[i].String_Value, "-t") == 0) {
      ...
      if(i > Parameter_Count) {
        ...
        tx_byte_allocate(byte_pool_qcli, (VOID **) &topic, MAX_SUB_LEN, TX_NO_WAIT);
        if(topic == NULL) {
          MQTT_PRINTF("malloc failed while allocating MAX_SUB_LEN bytes for "
                      "the topic\n");
          result = QCLI_STATUS_SUCCESS_E;
          goto end;
        }
  ...
  command = (char*)Parameter_List[0].String_Value;
    ...
    if (0 == strcmp(command, "connect")) {
        ...
        result = pub_mqtt(Parameter_Count - 1, &Parameter_List[1], app_id);
         ...
         result = destroy_mqtt(Parameter_Count - 1, &Parameter_List[1], app_id);
    }
