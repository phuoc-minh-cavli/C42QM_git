...
const QCLI_Command_t psm_cmd_list[] =
{
    ...
    {psm_demo_register, false, "REGISTER APPLICATION", "", "Register as PSM aware Application. Max 20 clients"},
...
static QCLI_Command_Status_t psm_demo_register(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    ...
    QCLI_Printf(qcli_psm_handle, "ITX: %d\n", itx);

    if(itx == 20)
        return QCLI_STATUS_USAGE_E;


    result = qapi_PSM_Client_Register(&id, psm_demo_app_call_back_func, &msgs[itx]);
    clients[itx] = id;
