...
static qapi_Net_LWM2M_Flat_Data_t *data_malloc(int size)
{
  ...
  tx_byte_allocate(byte_pool_qcli, (VOID **)&data, size * sizeof(qapi_Net_LWM2M_Flat_Data_t), TX_NO_WAIT);
...
static void data_free(int size,
...
#define STR_LENGTH 32
  ...
  tx_byte_allocate(byte_pool_qcli, (VOID **)&(*buffer),
                   length, TX_NO_WAIT);
  if(*buffer == NULL)
  {
    return 0;
                     ...
                     size_t buffer_len)
{
  ...
  tx_byte_allocate(byte_pool_qcli, (VOID **)&(data->value.asBuffer.buffer), buffer_len, TX_NO_WAIT);
