...
void at_fwd_callback(boolean is_reg, char *atcmd_name, uint8* at_fwd_params, uint8 mask, uint32 at_handle)
{
    ...
    {
      QCLI_Printf(qcli_atfwd_cli_app_handle, "Successfully received callback of Atcmd %s\n",atcmd_name);
      e = qapi_atfwd_send_resp(atcmd_name, NULL, QCLI_AT_RESULT_ERROR_V01);
    }
    ...
    else if (!strncasecmp(atcmd_name, "+ex5", strlen(atcmd_name)))
    {
      ...
      memset (at_resp, 0, QCLI_AT_CMD_RESP_LEN_MAX_V01);
      snprintf(at_resp, QCLI_AT_CMD_RESP_LEN_MAX_V01, "81-abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz");
      e = qapi_atfwd_send_resp(atcmd_name, at_resp, QCLI_AT_RESULT_OK_V01);
	  if (at_resp)
      {
        atfwd_free(at_resp);
    ...
    else if (!strncasecmp(atcmd_name, "+ex21", strlen(atcmd_name)))
    {
      ...
      memset (at_resp, 0, QCLI_AT_CMD_RESP_LEN_MAX_V01);
      snprintf(at_resp, QCLI_AT_CMD_RESP_LEN_MAX_V01, "+extest");

	  e = qapi_atfwd_reg(at_resp, at_fwd_extest_callback);
      if (e != QAPI_OK)
      {
      ...
      e = qapi_atfwd_send_resp(atcmd_name, NULL, cmd_result);
	  if (at_resp)
      {
        atfwd_free(at_resp);
      }
