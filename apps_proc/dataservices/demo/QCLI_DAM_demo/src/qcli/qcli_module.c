...
#define BENCHRX_STR "benchrx"
#define BENCHTX_STR "benchtx"

static char qcli_benchrx_stack[THREAD_STACK_SIZE];
static char qcli_benchtx_stack[THREAD_STACK_SIZE];
static char qcli_stack[THREAD_STACK_SIZE];
...
static TX_THREAD *Thread_Handle_cmd;
...
static void Command_Thread(ULONG Thread_Parameter);
...
/**
   ...
   @param Thread_Parameter is the parameter specified when the thread was
          ...
          structure.
*/
void Command_Thread(ULONG Thread_Parameter)
{
   uint32_t              Index;
   ...
   int status;
      ...
      memscpy(&Thread_Info, sizeof(Thread_Info), &QCLI_Context_D.Thread_Info, sizeof(Thread_Info_t));
...
static void Execute_Command(uint32_t Command_Index, const QCLI_Command_t *Command, uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
   ...
   if(Command->Start_Thread)
   {
      ...
      if(QCLI_Context_D.Thread_Count < MAXIMUM_THREAD_COUNT)
      {
         ...
         /*
          Benchtx and Benchrx can run simultaneously. Hence we will need two
          seperate starting addresses for the thread stack.
          In future if there is any other operation which needs a thread creation,
          which needs to run with benchtx and benchrx, we can use a different
          stack start address (else case below).
         */
         if (!strncmp((char*)QCLI_Context_D.Thread_Info.Command->Command_String,BENCHRX_STR,strlen(BENCHRX_STR)))
         {
           cmd_stack = qcli_benchrx_stack;
         }
         else if (!strncmp((char*)QCLI_Context_D.Thread_Info.Command->Command_String,BENCHTX_STR,strlen(BENCHTX_STR)))
         {
           cmd_stack = qcli_benchtx_stack;
         }
         else
         {
           cmd_stack =  qcli_stack;
         }

         status = txm_module_object_allocate((void **)&Thread_Handle_cmd, sizeof(TX_THREAD));

		  if(status != TX_SUCCESS)
         {
            QCLI_Printf(MAIN_PRINTF_HANDLE, "Module object creation failed for Command Thread (%d).\n", status);
         }

         ret = tx_thread_create(Thread_Handle_cmd, "Command Thread", Command_Thread, 0,
...
qbool_t QCLI_Initialize(void)
{
   ...
   txm_module_object_allocate((void **)(&(QCLI_Context_D.CLI_Mutex)), sizeof(TX_MUTEX));
   tx_mutex_create((QCLI_Context_D.CLI_Mutex), "qcli_mutex", TX_INHERIT);

   /* Initialize the thread ready event.                                */
   txm_module_object_allocate((void **)(&(QCLI_Context_D.Thread_Info.Thread_Ready_Event)), sizeof(TX_EVENT_FLAGS_GROUP));
