...
TX_EVENT_FLAGS_GROUP gSignalGtpWwan;
...
QCLI_Group_Handle_t qcli_loc_handle;
...
static int location_enable_gtp_wwan(void);
...
static QCLI_Command_Status_t LocTest_location_enable_gtp_wwan(
    uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
    int res;

    // Invoke loc API
    res = location_enable_gtp_wwan();
    if (res == 0) {
        LOC_LOG_QCLI("location_enable_gtp_wwan() returned success");
    }
    else {
        LOC_LOG_QCLI("location_enable_gtp_wwan() returned failure !");
    }
...
const QCLI_Command_t loc_cmd_list[] =
{
    ...
    {LocTest_location_enable_gtp_wwan, false, "locenablegtpwwan", "id <id>",
            "Enable GTP WWAN", NULL},
...
static int location_init(uint32_t clientIdx, uint32_t cbMask)
{
    ...
    tx_event_flags_create(&gSignalLocation, "gSignalGtpWwan");
...
static int location_deinit(uint32_t clientIdx)
{
    ...
    tx_event_flags_delete(&gSignalGtpWwan);
...
static uint32_t location_start_tracking(uint32_t clientIdx, uint32_t minInterval, uint32_t minDistance)
{
    ...
    ret = (qapi_Location_Error_t)qapi_Loc_Start_Tracking(
            clientIdArr[clientIdx], &locationOptions, &id);
    ...
    uint32_t clientIdx, uint32_t powerLevel)
{
    ...
    ret = (qapi_Location_Error_t)qapi_Loc_Get_Single_Shot(
        clientIdArr[clientIdx], (qapi_Location_Power_Level_t)powerLevel, &id);
...
static int location_enable_gtp_wwan(void)
{
    ...
    LOC_LOG_QCLI("LOCATION ENABLE GTP WWAN");
