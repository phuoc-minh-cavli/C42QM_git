...
/*DTLS Certificate PEM file names*/
...
//extern  dss_net_hndl[NO_OF_APPS][QAPI_DSS_MAX_DATA_CALLS];
...
static qapi_Coap_Status_t coap_block1_handler(qapi_Coap_Session_Hdl_t hdl , qapi_Coap_Packet_t * message , int app_id);
...
{ \
  ...
  { \
    ...
    { \
      ...
      data = atoi((const char *)input_data); \
...
/*static void  process_ext_options(qapi_Coap_Session_Hdl_t hdl,qapi_Coap_Packet_t * messageP)
{
  ...
  ext_option = (qapi_Coap_Ext_Msg_Option_t *)malloc(sizeof(qapi_Coap_Ext_Msg_Option_t));
...
static qapi_Coap_Status_t coap_block1_handler(qapi_Coap_Session_Hdl_t handle , qapi_Coap_Packet_t * message , int app_id)
{
	...
	COAP_PRINTF("coap_block1_handler called\n");
	...
	if(message->code == QAPI_CONTINUE_2_31)
	{
       if(message->block1_more)	 
       {
          if(qapi_Coap_Init_Message(handle, &response , QAPI_COAP_TYPE_CON , QAPI_COAP_PUT) != QAPI_OK)
          {
             ...
             COAP_PRINTF("coap_block1_handler : CON : uri_path_block[appid] %s , len %d \n" , uri_path_block[app_id] , uri_path_block_len[app_id]);
    	  ...
    	  if(payload != NULL)
    	  {
		   ...
		   COAP_PRINTF("coap_block1_handler : payload to put %s : " , payload);
    	  }
    	  else
    	  {
           ...
           if(qapi_Coap_Set_Payload(handle,response , payload, payload_len) != QAPI_OK)
    	   {
    		...
    		else
    		{
             if(nextblock1ok == 1)
    	     {
			   ...
			   COAP_PRINTF("coap_block1_handler : CON : qapi_Coap_Send_Message returned success\n");
			   COAP_PRINTF("coap_block1_handler bytes put till now = %d \n" , send_block1_buffer_len[app_id]);
    	     }
...
static qapi_Coap_Status_t coap_block2_handler(qapi_Coap_Session_Hdl_t  handle , qapi_Coap_Packet_t * message , int app_id)
{
	...
	COAP_PRINTF("coap_block2_handler called\n");
    ...
    params.num = last_block2_num_rcvd[app_id]; 
	  ...
	  if(message->block2_more == 1)
	  {
         ...
         COAP_PRINTF("coap_block2_handler : CON : uri_path_block[appid] %s , len %d \n" , uri_path_block[app_id] , uri_path_block_len[app_id]);
	...
	{    
		...
		if(qapi_Coap_Send_Message(handle ,response, &message_param) != QAPI_OK)
		{
	       ...
	       goto error;
            ...
            {
               if(response->uri_path)
			     coap_free(response->uri_path);
		       coap_free(response);
		       response  = NULL;
            }
...
static qapi_Coap_Status_t coap_downlink_block1_handler(qapi_Coap_Session_Hdl_t handle , qapi_Coap_Packet_t * message , int app_id)
{
	    ...
	    COAP_PRINTF("coap_downlink_block1_handler\n");
		...
		if(message->code == QAPI_COAP_PUT)
		{
		   ...
		   if(message->payload_len > 0 && message->payload)
		   {
		      if(last_downlink_block1_num_rcvd[app_id] == message->block1_num)
			  {
				 ...
				 COAP_PRINTF("coap_downlink_block1_handler: Blockwise2 payload = %s\n" , get_block1_buffer[app_id]);
		   ...
		   else
		   {
		      if(message->block1_more == 0)
			   ...
			   if (qapi_Coap_Set_Header(handle,response ,QAPI_COAP_BLOCK1,&params,sizeof(qapi_Coap_Block_Params_t)) != QAPI_OK)
			   {
				  COAP_PRINTF("coap_block1_handler : qapi_Coap_Set_Header returned error");
			      ret =  QAPI_COAP_IGNORE;
	              goto end;			  
			   } 
 ...
 COAP_PRINTF("Reply recevied from Server\n");
   ...
   { 
	 if (NULL != coapMessage && QAPI_COAP_TYPE_RST != coapMessage->type)
	 {
	   ...
	   {
         payload = (char*)malloc((coapMessage->payload_len) * sizeof(char));
		 if(payload != NULL)
		 {
    ...
    )
{
  ...
  {
    COAP_PRINTF("COAP_LOG : Opening EFS file %s failed\n", name);
    return QAPI_ERROR;
  } 

  if(qapi_FS_Stat_With_Handle(efs_fd, &stat) < 0) {
    COAP_PRINTF("COAP_LOG : Reading EFS file %s failed\n", name);
    return QAPI_ERROR;
  }



  file_buf = calloc(1, stat.st_size + 4); 

  if(file_buf == NULL) 
  {
    COAP_PRINTF("COAP_LOG : calloc failed \n");
    return QAPI_ERROR;
  }
  ...
  if((bytes_read != stat.st_size) && (bytes_read == 0)) {
    coap_free(file_buf);
    COAP_PRINTF("COAP_LOG : Reading EFS file error\n");
    return QAPI_ERROR;
  }
...
int coap_configure_ssl(qapi_Sec_Mode mode , qapi_Net_SSL_Config_t  * cfg, qapi_Coap_Sec_Info_t * ssl_cfg )
{
  qapi_Status_t ret = QAPI_OK;
  char ssl_cert[MAX_CERT_NAME_LEN] = {0};
  char ssl_key[MAX_CERT_NAME_LEN]  = {0};
  char ca_list[MAX_CERT_NAME_LEN]  = {0};
  char di_cert[MAX_CERT_NAME_LEN]  = {0};
  qapi_Net_SSL_Cert_Info_t cert_info ;
  char psk_fname_raw[MAX_CERT_NAME_LEN] = {0};
  char cert_fname_bin[MAX_CERT_NAME_LEN] = {0};
  ...
  {
    COAP_PRINTF(" coap_configure_ssl :returned QAPI_ERROR : reason cfg == NULL || ssl_cfg == NULL\n");
    ret =  QAPI_ERROR;
	goto Error;
  }
	    ...
	    switch (message_code)
	    {
		   case QAPI_COAP_GET : 
            ...
            if(message_type == QAPI_COAP_TYPE_CON)
            {
			   ...
			   if(QAPI_IS_OPTION(messageP , QAPI_COAP_OPTION_BLOCK2) && is_request_uri_ok == 1 )
			   {
		   ...
		   case QAPI_COAP_PUT :
            ...
            if(message_type == QAPI_COAP_TYPE_CON)
            {
			  ...
			  if(QAPI_IS_OPTION(messageP , QAPI_COAP_OPTION_BLOCK1) && is_request_uri_ok == 1 )
			  {
		   ...
		   case QAPI_CONTINUE_2_31 :
		    ...
		    if((char *)messageP->payload &&  messageP->payload_len > 0)
            {
              payload = (char*)malloc((messageP->payload_len) * sizeof(char));
			  if(payload != NULL)
...
QCLI_Command_Status_t create_session(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
   ...
   {
     result = QCLI_STATUS_ERROR_E;
	 COAP_PRINTF("create_session : qapi_Coap_Register_Event returned error ");
	 goto end;
   }
   ...
   {
     result = QCLI_STATUS_ERROR_E;
	 COAP_PRINTF("create_session : Session on app id %d not created " , app_id);
   }
   else
   {
     result = QCLI_STATUS_SUCCESS_E;
...
QCLI_Command_Status_t create_connection(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    if((Parameter_List == NULL))
    {
		COAP_PRINTF("Usage: create_connection [-app <app_id>] [-m <sec mode> 0 MODE_PSK|1 MODE_CERTIFICATE|2 MODE_NON SEC] [-psk <psk_key id> e.g 1234_psk.bin(default) | 101_psk.bin , only for psk mode] [-p <protocol> |0 UDP|1 NIDD] [-i <handle_index for non ip >] [-s <source ip>] [-sp <source port>] [-d <server ip>] [-dp <server port>]"
    ...
    for (i = 0; i < Parameter_Count; i++)
    {
            ...
            ++i;
             if ((Parameter_List[i].Integer_Is_Valid) && connection_params.family_type == AF_NONIP)
             {
				 ...
				 if(connection_params.callHndl == 0)
				 {
				    ...
				    COAP_PRINTF(" create_connection call handle fot nonip : %p\n" , connection_params.callHndl);
        ...
        else if (strcmp((char*)Parameter_List[i].String_Value, "-s") == 0)
        {
			...
			if (inet_pton(AF_INET, addr_ptr, &addr.a) != 0)   
            {
                ...
                {
                    /* not IPv6 address */
                    COAP_PRINTF("Not a valid IPv4/IPv6 address of source\n");
                    result = QCLI_STATUS_USAGE_E;
                    goto end;
                }
                else
        ...
        else if (strcmp((char*)Parameter_List[i].String_Value, "-r") == 0)
        {
             ...
             if (Parameter_List[i].Integer_Is_Valid && Parameter_List[i].Integer_Value >= 0 && Parameter_List[i].Integer_Value <= 1 )
            {
               if(Parameter_List[i].Integer_Value)
				 ...
				 COAP_PRINTF(" Resumption enable value %d\n" , connection_params.resumption_enabled);
               	}
        ...
        else if(strcmp((char*)Parameter_List[i].String_Value, "-t") == 0)
        {
		    ...
		    if (Parameter_List[i].Integer_Is_Valid && connection_params.resumption_enabled == TRUE)
		    {
		       if(Parameter_List[i].Integer_Value)
		       {
				 ...
				 COAP_PRINTF(" Resumption enable value timeout %d\n" , connection_params.session_resumption_timeout);
    ...
    else if(connection_params.sec_Mode ==0 || connection_params.sec_Mode == 2)
    {
	  ...
	  COAP_PRINTF("Calling coap_configure_ssl for mode = %d " , connection_params.sec_Mode );
    ...
    {
		   result = QCLI_STATUS_ERROR_E;
		   COAP_PRINTF("qapi_Coap_Create_Connection returned error !!\n");
    }
    else
	{      
	...
	QCLI_Command_Status_t set_header(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
	{
        ...
        for (i = 0; i < Parameter_Count; i++)
        {
           ...
           else if ((strcmp((char*)Parameter_List[i].String_Value, "-v")) == 0)
           {
			...
			else if(header_type == QAPI_COAP_TOKEN || header_type == QAPI_COAP_ETAG)
			{
	        ...
	        else if(header_type == QAPI_COAP_OBSERVE || header_type == QAPI_COAP_SIZE1 || header_type ==  QAPI_COAP_SIZE || header_type == QAPI_COAP_CONTENT_TYPE || header_type == QAPI_COAP_ACCEPT)
			{
			...
			else if(header_type > QAPI_COAP_SIZE && (header_type == 80 || header_type == 258 || header_type == 120))
			{
			  if(Parameter_List[i].Integer_Is_Valid)
			  {
			    //val_len = set_extended_header_data(INTEGER, &Parameter_List[i].Integer_Value,
			    //                                    0, header_val);
			  ...
			  {
			    //val_len = set_extended_header_data(STRING, Parameter_List[i].String_Value,
	    ...
	    else if(header_type == QAPI_COAP_OBSERVE || header_type == QAPI_COAP_SET_STATUS || header_type == QAPI_COAP_SIZE1 || header_type == QAPI_COAP_SIZE || header_type == QAPI_COAP_CONTENT_TYPE || header_type == QAPI_COAP_ACCEPT)
		{    
        ...
        else 
        {
		 ...
		 else
		 {
		   if(header_type == QAPI_COAP_URI_PATH)
		   {
             if(uri_path_block_len[app_id] > 0 && uri_path_block_len[app_id] <= MAX_RESOURCE_NAME_LEN && header_val != NULL)
             { 
		   ...
		   if (strcmp((char*)Parameter_List[i].String_Value, "-t") == 0)
           {
        	...
        	if(header_type > QAPI_COAP_SIZE &&  (header_type == 80 || header_type == 258 || header_type == 120))
        	{
			  ...
			  else if(header_type == QAPI_COAP_CONTENT_TYPE || header_type == QAPI_COAP_OBSERVE || header_type == QAPI_COAP_SIZE1 ||  header_type == QAPI_COAP_SIZE)
              {
			    ...
			    {
                  COAP_PRINTF("get_header returned success for header type = %d\n " , header_type);
				  COAP_PRINTF("get_header : header value = %d\n " , *(header_value));
				  COAP_PRINTF("get_header : header length = %d\n " , val_len);
				}
...
QCLI_Command_Status_t parse_message(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
   uint32_t i,p,j;
   QCLI_Command_Status_t result = QCLI_STATUS_SUCCESS_E;
   qapi_Coap_Packet_t * packet = NULL;
   char *message_buff =NULL;
   char *recieved_buff =NULL;
   size_t buff_len = 0;
  
   if ((Parameter_Count < 2) || (Parameter_List == NULL))
   {
      COAP_PRINTF("Usage: parse_message [-buf <message_buffer>] [-len <length of message_buffer>]");
      result = QCLI_STATUS_SUCCESS_E;
      goto end;
   }
   ...
   {
      result = QCLI_STATUS_ERROR_E;
 	  COAP_PRINTF("parse_message  : coap hndl == NULL \n");
 	  goto end;
   }

   for (i = 0; i < Parameter_Count; i++)
   {
      if (strcmp((char*)Parameter_List[i].String_Value, "-buf") == 0)
      {
         ++i;
         if(Parameter_List[i].String_Value != NULL)
         {
 			recieved_buff = (char*)Parameter_List[i].String_Value ;               
         }
         else
         {
            COAP_PRINTF("Invalid message buffer string value\n");
 			result = QCLI_STATUS_USAGE_E;
 			goto end;
         }
      }
      else if (strcmp((char*)Parameter_List[i].String_Value, "-len") == 0)
	  {
	     ++i;
	     if(Parameter_List[i].Integer_Is_Valid)
	     {
		    buff_len = Parameter_List[i].Integer_Value;
			COAP_PRINTF("payload length = %d \n", buff_len);
   ...
   if (recieved_buff == NULL)
   { 
      result = QCLI_STATUS_ERROR_E;
	  goto end;
   }
   
   if (qapi_Coap_Init_Message(app_coapcli_hndle[app_id], &packet, QAPI_COAP_TYPE_CON, QAPI_CONTENT_2_05) != QAPI_OK)
   {
       COAP_PRINTF("qapi_Coap_Init_Message returned error\n");
       result = QCLI_STATUS_ERROR_E;
	   goto end;
   }
   if (packet == NULL)
   ...
   message_buff = (char *)malloc(buff_len/2);
   ...
   for(i=0,j=0;j<buff_len/2;i+=2,j++)
   {
	  message_buff[j] = ((recieved_buff[i] >'9' ? recieved_buff[i]-=7 : recieved_buff[i]) <<4) | ((recieved_buff[i+1] >'9' ? recieved_buff[i+1]-=7 : recieved_buff[i+1]) & 0x0F);
   }
   
   if (qapi_Coap_Parse_Message(app_coapcli_hndle[app_id], &packet, (uint8_t *)message_buff, (uint16_t)buff_len) != QAPI_OK)
   {
     result = QCLI_STATUS_ERROR_E;
     COAP_PRINTF("qapi_Coap_Parse_Message returned error \n");
     goto end;
   }
   COAP_PRINTF("--------CoAP parse message sucess ------- \n");
   COAP_PRINTF("type ---->%d\n" , packet->type);
   COAP_PRINTF("code ----> %d\n" , packet->code);
   COAP_PRINTF("version ----> %d\n" , packet->version );
   COAP_PRINTF("mid ----> %d\n" , packet->mid );
   COAP_PRINTF("token_len ----> %d\n" , packet->token_len );
   COAP_PRINTF("tocken ---->" );
   for (i = 0; i < packet->token_len ; i++)
   {
     COAP_PRINTF("%2x", packet->token[i]);
   }
   COAP_PRINTF("\n" );
   COAP_PRINTF("payload_len ----> %d\n" , packet->payload_len );
   COAP_PRINTF("----------Processing options-----------------\n");
   process_options(packet);	
   
end:
  if(packet != NULL )
  {
     qapi_Coap_Free_Message(app_coapcli_hndle[app_id], packet);
     packet = NULL;
  }
  if(message_buff != NULL )
  {
...
QCLI_Command_Status_t disconnect(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
	...
	{
       COAP_PRINTF("coap_disconnect : either coap handle on app_id %d doensn't exists or connection isn't created\n" ,app_id);
	   result=QCLI_STATUS_ERROR_E; 
	}
...
QCLI_Command_Status_t delete_session(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
	...
	QCLI_Command_Status_t result =  QCLI_STATUS_SUCCESS_E;
     ...
     {    
	    ...
	    {
             COAP_PRINTF("qapi_Coap_Destroy_Session ! = QAPI_OK\n");
		     result=QCLI_STATUS_ERROR_E;
		     goto end;
	    }	
	    else
...
#ifndef MINI_QCLI
  cmd_parser_temp->Command_Index = 28;
#else
...
QCLI_Command_Status_t get_last_error_coap(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
	...
	if((Parameter_Count > 2) || (Parameter_List == NULL)) 
	{
    	COAP_PRINTF("get_last_error_coap:  Invalid Params or More than necessary Params passed \n");
    	result = QCLI_STATUS_ERROR_E;
    	goto end;
    }
  	...
  	if(app_coapcli_hndle[app_id] != NULL) {
		...
		{
			COAP_PRINTF("get_last_error_coap : Memory allocation failed for Lasterror\n" ,app_id);
	   		result = QCLI_STATUS_ERROR_E; 
	   		goto end;
		}
    	...
    	if(ret != QAPI_OK)
    	{
      		COAP_PRINTF("get_last_error_coap:  COAP Get Last error Code failed Failed\n");
    	}
    	else
    	{
	...
	{
       COAP_PRINTF("get_last_error_coap : either coap handle on app_id %d doensn't exists or connection isn't created\n" ,app_id);
	   result = QCLI_STATUS_ERROR_E; 
	   goto end;
	}
...
proc_coap_command_handler(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    COAP_PRINTF("[-app <app_id>] create_session [-a <ack_random_factor>] [-m <max_retransmit>] [-M <max_age>] [-ml <max_latency>] [-t <transaction_timeout>] [-e <enable block config option>][-le <enable last error>]  \n");
	COAP_PRINTF("[-app <app_id>] create_connection  [-m <sec mode> 0 MODE_PSK|1 MODE_CERTIFICATE|2 MODE_NON SEC]  [-p <protocol> |0 UDP|1 NIDD] [-i <handle_index for non ip >]\n");
	COAP_PRINTF("[-s <source ip>] [-sp <source port>] [-d <server ip>] [-dp <server port>][-r <resumption enable> 0|1] [-t <resumption timout>]\n");
    COAP_PRINTF("[-app <app_id>] init_message [-t msg type | 0 CON | 1 NON CON | 2 ACK | 3 RST ] [-m coap_method_type | 1 GET |2 POST | 3 PUT |4 DELETE]\n");
    COAP_PRINTF("[-app <app_id>] set_header  [-t <header type >] [-v <value>] [-l <length>]\n");
	COAP_PRINTF("[-app <app_id>] get_header  [-t <header type >] \n");
    COAP_PRINTF("[-app <app_id>] send_message [-m <mid> max 16 bit unsigned int] [-t <token id>] [-l <token length>] [-p <payload>] [-L <payload length>]\n");
    COAP_PRINTF("[-app <app_id>] send_message_v2 [-m <mid> max 16 bit unsigned int] [-t <token id>] [-l <token length>] [-btype <block type>] [-bsize <block size>] [-p <payload>] [-L <payload length>]\n");
    COAP_PRINTF("[-app <app_id>] get_block_info \n");
	COAP_PRINTF("[-app <app_id>] parse_message [-buf <message_buffer>] [-len <length of message_buffer>]\n");
    ...
    if (0 == strcmp(command, "create_session")) {
        ...
        result = get_block_info(Parameter_Count - 1, &Parameter_List[1], app_id);
	}
	 ...
	 else if (0 == strcmp(command, "parse_message")) {
        result = parse_message(Parameter_Count-1, &Parameter_List[1], app_id);
    }
