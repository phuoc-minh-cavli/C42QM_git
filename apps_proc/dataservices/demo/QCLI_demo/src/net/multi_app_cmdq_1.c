...
{
  LOG_INFO("multi_app_set_signal_t1 \n");

  qurt_mutex_lock(&multi_app_param_t1.multi_app_mutex);

  MULTI_APP_TASK_SET_SIG(multi_app_param_t1.multi_app_signal,MULTI_APP_SIGNAL_MASK);

  qurt_mutex_unlock(&multi_app_param_t1.multi_app_mutex);
...
==============================================================================*/
static void multi_app_mutex_init_t1(void)
{
  LOG_INFO("multi_app_mutex_init_t1 \n");

  qurt_mutex_init(&multi_app_param_t1.multi_app_mutex);
  return;
}
...
==============================================================================*/
static void multi_app_signal_init_t1(void)
{
  LOG_INFO("multi_app_signal_init_t1 \n");

  qurt_signal_init(&multi_app_param_t1.multi_app_signal);
  return;
}
...
==============================================================================*/
static void multi_app_signal_wait_t1(void)
{
  uint32_t received_sigs = 0;

  LOG_INFO("multi_app_signal_wait_t1 \n");
...
=============================================================================*/
    received_sigs = qurt_signal_wait(&multi_app_param_t1.multi_app_signal, MULTI_APP_SIGNAL_MASK,
...
void multi_app_task_init_t1(void)
{
  ...
  LOG_INFO("multi_app_task_init_t1 \n");
...
void multi_app_task_entry_t1(void *thread_ctxt)
{
  ...
  LOG_INFO("multi_app_task_entry_t1 \n");

  multi_app_task_init_t1();

/*=============================================================================
Initialize Mutex Object
==============================================================================*/

  multi_app_mutex_init_t1();

/*=============================================================================
Initialize Signal 
==============================================================================*/

  multi_app_signal_init_t1 ();

/*=============================================================================
Wait on the Signal
==============================================================================*/
  multi_app_signal_wait_t1();
...
void multi_app_enqueue_cmd_t1(multi_app_cmd_t* cmd_ptr)
{
  LOG_INFO("Cmd_ptr: 0x%x with cmd_id: %d, posted to int Q," 
                             ...
                             q_cnt(&multi_app_cmdq_t1));

  q_put(&multi_app_cmdq_t1, &(cmd_ptr->cmd_hdr.link));
...
static multi_app_cmd_t* multi_app_dequeue_cmd_t1( void )
{
  ...
  cmd_ptr = (multi_app_cmd_t*)q_get( &multi_app_cmdq_t1 );

  LOG_INFO("Cmd ptr: 0x%x, num rem cmds: Multi App Q count: %d",
                           cmd_ptr, q_cnt(&multi_app_cmdq_t1));
...
--------------------------------------------------------------------------*/
        ...
        if(obj_buffer != NULL)
        {
          switch(obj_buffer->app_data.ul_op.msg_type)
          {
            ...
            case QAPI_NET_LWM2M_EXT_NOTIFY_E:
            {
              ...
              time_t timeout = 5;
              flag = true;
              ext_object_notify(0,current_time, &timeout);
            }
            break;
        ...
        if(obj_buffer_v2 != NULL)
        {
          switch(obj_buffer_v2->app_data.ul_op.msg_type)
          {
            ...
            case QAPI_NET_LWM2M_EXT_NOTIFY_v2_E:
            {
              ...
              time_t timeout = 5;
              flag = true;
              ext_object_notify_v2(0,current_time, &timeout);
            }
            break;
    ...
    break;
	 ...
	 case LWM2M_CELL_CON_SET_EVT_v2:
	    result = proc_cell_nw_set_resource_v2(multi_app_cmd_ptr->cmd_data.Parameter_Count,
												 multi_app_cmd_ptr->cmd_data.Parameter_List,
												 multi_app_cmd_ptr->cmd_data.app_id);
	    qcli_multi_app_handle = qcli_m2m_stub_handle_v2;
	    break;
