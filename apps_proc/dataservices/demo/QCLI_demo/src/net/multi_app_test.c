...
bool is_cmd_running[NO_OF_APPS] = {false};
...
multi_app_cmd_t* multi_app_get_cmd_buffer(void)
{
  ...
  cmd_ptr = (multi_app_cmd_t*)malloc(sizeof(multi_app_cmd_t));
...
multi_app_cmd_parser_func(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, boolean is_lwm2m_ext_cmd)
{
  ...
  cmd_parser = (multi_app_cmd_parser_t*)malloc(sizeof(multi_app_cmd_parser_t));
  ...
  if (k == 2)  /* Command given to be run on both Threads*/
  {
    ...
    {
      cmd_parser->app_data1.Parameter_List[i].String_Value = (uint8_t *)malloc(strlen((char *)Parameter_List[j].String_Value) + 1);
    ...
    {
      cmd_parser->app_data2.Parameter_List[i].String_Value = (uint8_t *)malloc(strlen((char *)Parameter_List[j].String_Value) + 1);
  ...
  {
    if (is_lwm2m_ext_cmd) /* LWM2M_EXT command */
    {
      if (Parameter_Count > 0)
      {
        if (Parameter_List[0].Integer_Is_Valid)
        {
          app_id[0] = Parameter_List[0].Integer_Value + 1;
        }
      }
      else
      {
...
QCLI_Command_Status_t multi_app_cmd_queue_func(multi_app_cmd_t* cmd_ptr)
{
  ...
  {
    if(thrd_running[0] == false)
    {
      LOG_INFO("Application 0 not registerd ");
      return QCLI_STATUS_SUCCESS_E;
    }

    if (is_cmd_running[0] == false)
    {
  ...
  {
    if(thrd_running[1] == false)
    {
      LOG_INFO("Application 1 not registerd ");
      return QCLI_STATUS_SUCCESS_E;
    }

    if (is_cmd_running[1] == false)
    {
    LOG_INFO("Enqueue the command into the command queue. ");
