...
#endif
static QCLI_Command_Status_t server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
static QCLI_Command_Status_t server6(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
static QCLI_Command_Status_t udp_server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List);
static void client(void *parameter);
static void client6(void *parameter);
static void udp_client(void *parameter);
#ifndef MINI_QCLI
...
int test_for_quit_s(void);

extern unsigned long inet_addr(char *str);
extern int32_t qapi_Net_Profile_Set_Custom(qapi_Net_Profile_Custom_Pool_t *pNet_buf, uint8_t net_bufq_size);
extern int32_t qapi_Net_Profile_Set_Active(qapi_Net_Profile_Type_t profile);
...
{cfg_netbuf, false, "profile", "[-app <app id: 0/1>] custom <pool_size> <buf_size> <num_buf> <buf_size> <num_buf>\n"
    ...
    {benchrx, false, "benchrx", "[-app <app id: 0/1>] {tcp|udp|ssl|raw} <port> [{v4|v6} [<multicast IP> [<exclude list size> [{0|1|2} [<server_addr> [<hdrincl>]]]]]]", "Run the receive traffic test, <Port> is reused as socket protocol, <mulitcase IP> is reused as local address for raw socket",NULL},
    {benchtx, false, "benchtx", "[-app <app id: 0/1>] <Rx IP> <port> {tcp|udp|ssl|raw} <msg size> {0|1} {<time to TX (sec)>|<num of msg to TX>} <delay between msg (ms)> [{v4|v6} [<Tx IP> [<hdrincl>]] {tx_port}]\n",
#else
    {benchrx, false, "benchrx", "[-app <app id: 0/1>] {tcp|udp|raw} <port> [v4|v6] [<multicast IP>]\n", "Run the receive traffic test, <Port> is reused as socket protocol for raw socket",NULL},
    {benchtx, false, "benchtx", "[-app <app id: 0/1>] <Rx IP> <port> {tcp|udp|raw} <msg size> {0|1} {<time to TX (sec)>|<num of msg to TX>} <delay between msg (ms)> [{v4|v6} [<Tx IP>]]\n",
#endif
                                "For raw socket <port> is used as socket protocol number\n"
...
#endif
    {server, false , "server","[-app <app id: 0/1>] <server ip> <client ip>","TCP IPv4 server-client",server_abort},
    {server6, false, "server6", "[-app <app id: 0/1>] <server ip> <client ip>", "TCP IPv6 server-client",server_abort},
    {udp_server, false , "udpserver","[-app <app id: 0/1>] <v4|v6> <server ip> <client ip>","UDP server-client",server_abort},
#ifndef MINI_QCLI
	...
	{profile_command_handler, false, "txprofile", "[start|status|reset] [all | specific thread name[e.g qcli,mqtt,netmain...]]\n",
        "Perform CPU profile Client operations\n Type command name to get more info on usage. For example \"txprofile\".\n", server_abort},
    ...
    {set_sock_opt, false, "set_sock_opt", "[-app <app.id: 0/1] [-e<enable/disable sack>] [-i <irt>] [-m <max_rxt>] [-o <overall_timer>] [-s <mss>] \n[-k<keep_alive>] [-a <keep_idle>] [-t <keep_intvl>] [-c <keep_cnt>] [-r<rcv_buf>] [-b<snd_buf>] [-d<benchtx_sleep in ms>][-l<linger>][-z delayacktime][-f<reassm_timer>][-g<msl>][-p<max_persist>][-h<min_rto>][-j<max_rto>][-w<win_scale>][-n<enable_timestamp>][-q<max_sack_block>][-u<ip_ire>][-v<ip_maxfrags>]","set socket option", NULL},
...
static int portnumber[NO_OF_APPS] = {1025};
...
static void client(void *parameter)
{
    ...
    unsigned short port = portnumber[app_id];

server_quit = 0;

/*TCP Socket creation*/
    sock_ds = qapi_socket(AF_INET, SOCK_STREAM, 0);
    QCLI_Printf(qcli_net_handle,"Client socket fd %d\n",sock_ds);
    if(sock_ds == -1)
    {
        QCLI_Printf(qcli_net_handle, "Socket creation error: %s\n", strerror(errno));
        return;
    }

    remote_addr.sin_family = AF_INET;
    remote_addr.sin_port = htons(port);
    remote_addr.sin_addr.s_addr =rclient[app_id].sin_addr.s_addr;


    QCLI_Printf(qcli_net_handle,"Calling Client connect\n");
    if(qapi_connect(sock_ds, (struct sockaddr*)&remote_addr, sizeof(remote_addr)) == -1)
    {
        QCLI_Printf(qcli_net_handle, "Connect failure(%s)\n", strerror(errno));
    }
    else
    {
        while(1)
...
//            timer_sleep(40, T_MSEC, FALSE);
            snprintf(buf,25,"Hello Server %d",a);
            if( qapi_send(sock_ds, buf, 128, 0) == -1)
            {
              QCLI_Printf(qcli_net_handle,"send failed \n");
              break;
            }
            a++;
        }
...

{
    ...
    uint16 port = portnumber[app_id];

    server_quit = 0;

/*TCP Socket creation*/
    sock_ds = qapi_socket(AF_INET6, SOCK_STREAM, 0);
    QCLI_Printf(qcli_net_handle,"Client socket fd %d\n",sock_ds);
    if(sock_ds == -1)
    {
        QCLI_Printf(qcli_net_handle, "Socket creation error: %s\n", strerror(errno));
    ...
    remote_addr.sin_port = htons(port);
    remote_addr.sin_flowinfo = 0;
    remote_addr.sin_scope_id= 1;
    inet_pton(AF_INET6 ,rclient6[app_id],&(remote_addr.sin_addr));
...
static QCLI_Command_Status_t server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
  ...
  cmd_parser_temp->cmd_id = SERVER_EVT;
...
proc_server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{

    int sock_ds = 0,errno = 0, acc_sock_ds = 0;
    unsigned short port = portnumber[app_id];
    char buf[1024]="server";
    ...
    if (Parameter_Count < 2 || Parameter_List == NULL)
    {
     ...
     host2 = (char *)Parameter_List[1].String_Value;

    qurt_thread_attr_init(&Thread_Attribte);
    qurt_thread_attr_set_name(&Thread_Attribte, "client");
    qurt_thread_attr_set_priority(&Thread_Attribte, 150);
    qurt_thread_attr_set_stack_size(&Thread_Attribte, 2048);


    /*TCP Socket creation*/
    sock_ds = qapi_socket(AF_INET, SOCK_STREAM, 0);
    QCLI_Printf(qcli_net_handle, "Server started with socket %d ", sock_ds);
    ...
    rclient[app_id].sin_addr.s_addr = inet_addr(host2);

    if(qapi_bind(sock_ds, (struct sockaddr*)&server_addr, sizeof(struct sockaddr_in)) == -1){
      QCLI_Printf(qcli_net_handle, "Address binding error\n");
      result = QCLI_STATUS_ERROR_E;
      goto end;
    }
    ...
    {
       QCLI_Printf(qcli_net_handle, "Listen call error: %s\n", strerror(errno));
        result = QCLI_STATUS_ERROR_E;
        goto end;
    }

    Thread_Result = qurt_thread_create(&Thread_Handle, &Thread_Attribte, client, NULL);

    if(Thread_Result != 0)
    {
        QCLI_Printf(qcli_net_handle, "Failed to create thread for command (%d).\n", Thread_Result);
        result = QCLI_STATUS_ERROR_E;
        goto end;
    }

    memset(&client_addr, 0, sizeof(client_addr));
    QCLI_Printf(qcli_net_handle,"Server accept ()\n");
    acc_sock_ds = qapi_accept(sock_ds, (struct sockaddr *)&client_addr, &len);

    QCLI_Printf(qcli_net_handle,"Accept Successfull\n");
    while(1)
     {
         ...
         if( qapi_recv(acc_sock_ds, buf, 128, 0) == -1)
         {
            QCLI_Printf(qcli_net_handle,"qapi_server_recv failed \n");
            break;
         }
          else
          {
            QCLI_Printf(qcli_net_handle,"Recv Data: %s\n",buf);
            memset(buf,0,sizeof(buf));
          }
    }

    qapi_socketclose(acc_sock_ds);
    qapi_socketclose(sock_ds);
    portnumber[app_id]++;
...
static QCLI_Command_Status_t server6(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
  ...
  cmd_parser_temp->cmd_id = SERVER6_EVT;
  cmd_parser_temp->Command = (QCLI_Command_t *)&net_cmd_list[0];
#ifndef MINI_QCLI
...
QCLI_Command_Status_t 
proc_server6(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    qurt_thread_attr_init(&Thread_Attribte);
    qurt_thread_attr_set_name(&Thread_Attribte, "client6");
    qurt_thread_attr_set_priority(&Thread_Attribte, 150);
    qurt_thread_attr_set_stack_size(&Thread_Attribte, 2048);
    ...
    if (Parameter_Count < 2 || Parameter_List == NULL)
    {
     ...
     host = (char *)Parameter_List[0].String_Value;
     rclient6[app_id] = (char *)Parameter_List[1].String_Value;
    ...
    /*Server address binding*/
    memset(&server_addr, 0, sizeof(server_addr));
    inet_pton(AF_INET6 ,host,&(server_addr.sin_addr));
    server_addr.sin_flowinfo = 0;
    server_addr.sin_family = AF_INET6;
    server_addr.sin_port = htons(port);
    server_addr.sin_scope_id = 1;

    if(qapi_bind(sock_ds, (struct sockaddr*)&server_addr, sizeof(struct sockaddr_in6)) == -1){
      QCLI_Printf(qcli_net_handle, "Address binding error\n");
      result = QCLI_STATUS_ERROR_E;
      goto end;
    }
 ...
 /*Server with passive socket*/
    ...
    Thread_Result = qurt_thread_create(&Thread_Handle, &Thread_Attribte, client6, NULL);
...
//    qapi_Net_DNS_Server_List_t svr_list;
    ...
    uint32_t addr=0, mask=0, gw=0;
    int i, err = -1;
    uint8_t * physical_address =  NULL;
    const uint8_t **paddr = NULL;
    uint32_t physical_address_length = 0;

    QCLI_Printf(qcli_net_handle, "%s:\n", interface_name);

    physical_address = (uint8_t *)malloc((sizeof (uint8_t)) *( QAPI_MAC_ADDR_LEN));
    ...
    memset (physical_address , 0 , QAPI_MAC_ADDR_LEN);
...
proc_dnsc(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    */
    ...
    else if (strncmp(cmd, "settimeout", 7) == 0)
    {
	    ...
	    {
	       QCLI_Printf(qcli_net_handle, "timeout value set to default 2000ms\n ");
		   result = QCLI_STATUS_SUCCESS_E;
           goto end;
	    }
	}
    ...
    else if (strncmp(cmd, "hostbyname", 3) == 0)
    {
        ...
        memset(res.h_name, 0, 31);

        res.h_addr_list = (char**)malloc(sizeof(char*)*5);
        ...
        memset(res.h_addr_list, 0, (sizeof(char*)*5));

        for(index = 0; index < 5; index++) 
        {
          if( ipaddr.type == AF_INET)
          {
            res.h_addr_list[index] = (char*)malloc(16);
          ...
          {
            res.h_addr_list[index] = (char*)malloc(48);
            if (res.h_addr_list[index] == NULL) 
            { 
              QCLI_Printf(qcli_net_handle,"tx_byte_allocate [h_addr_list[%d]] failed!", index); 
              result = QCLI_STATUS_ERROR_E;
     ...
     error:
      ...
      if(res.h_addr_list != NULL)
      {
        for(index = 0; index < 5; index++) 
        {
          if(res.h_addr_list[index] != NULL)
          {
             free(res.h_addr_list[index]);
          }
        }
        free(res.h_addr_list);
      }
  ...
  if (Parameter_List!= NULL)
  {
    for (q = 0; q<Parameter_Count; q++)
    {
      ...
      {
        free(Parameter_List[q].String_Value);
...
static QCLI_Command_Status_t udp_server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
  ...
  cmd_parser_temp->cmd_id = UDP_SERVER_EVT;
  cmd_parser_temp->Command = (QCLI_Command_t *)&net_cmd_list[0];
#ifndef MINI_QCLI
...
QCLI_Command_Status_t 
proc_udp_server(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    int sock_ds=0,errno=0;
    unsigned short port = portnumber[app_id];
    char buf[1024]="udp server";
    struct sockaddr_in server_addr, client_addr;
    ...
    host2 = (char *)Parameter_List[2].String_Value;
    rclient6[app_id] = (char *)Parameter_List[2].String_Value;

    if (strcmp((char *)Parameter_List[0].String_Value,"v4") == 0)
    {
        ip_type[app_id] = AF_INET;
    }
    else if (strcmp((char *)Parameter_List[0].String_Value,"v6") == 0)
    {
        ip_type[app_id] = AF_INET6;
    }


    qurt_thread_attr_init(&Thread_Attribte);
    qurt_thread_attr_set_name(&Thread_Attribte, "udp client");
    qurt_thread_attr_set_priority(&Thread_Attribte, 150);
    qurt_thread_attr_set_stack_size(&Thread_Attribte, 2048);

    /*UDP Socket creation*/
    sock_ds = qapi_socket(ip_type[app_id], SOCK_DGRAM, 0);
    QCLI_Printf(qcli_net_handle, "Server started with socket %d ", sock_ds);
    ...
    if (ip_type[app_id] == AF_INET)
    {
        ...
        from = (struct sockaddr *)&client_addr;
    ...
    else
    {
        ...
        inet_pton(AF_INET6 ,host,&(server_addr6.sin_addr));
        server_addr6.sin_flowinfo = 0;
        server_addr6.sin_family = AF_INET6;
        server_addr6.sin_port = htons(port);
        server_addr6.sin_scope_id = 1;
        addr_size = sizeof(client_addr6);
        from = (struct sockaddr *)&client_addr6;

        if(qapi_bind(sock_ds, (struct sockaddr*)&server_addr6, sizeof(struct sockaddr_in6)) == -1)
        {
            QCLI_Printf(qcli_net_handle, "Address binding error\n");
            result = QCLI_STATUS_ERROR_E;
            goto end;
    ...
    Thread_Result = qurt_thread_create(&Thread_Handle, &Thread_Attribte, udp_client, NULL);
    ...
    while(1)
    {
        ...
        if( qapi_recvfrom(sock_ds, buf, 128, 0,from, &addr_size) == -1)
        {
           QCLI_Printf(qcli_net_handle,"qapi_server_recv failed \n");
            break;
...
/*UDP Socket creation*/
    ...
    else
    {
        ...
        inet_pton(AF_INET6 ,rclient6[app_id],&(remote_addr6.sin_addr));
        remote_addr6.sin_flowinfo = 0;
        remote_addr6.sin_family = AF_INET6;
        remote_addr6.sin_port = htons(port);
        remote_addr6.sin_scope_id = 1;
        addr_size = sizeof(remote_addr6);
...
//        timer_sleep(40, T_MSEC, FALSE);
        snprintf(buf,25,"Hello Server %d",a);
        if( qapi_sendto(sock_ds, buf, 128, 0, to,addr_size) == -1)
        {
          QCLI_Printf(qcli_net_handle,"send failed \n");
          break;
        }
...
proc_ping3(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    for (i = 0; i < count; i++)
    {
       ...
       if(flag == FALSE)
       {
        ...
        ping_buf = (qapi_Net_Ping_V4_R2_t *)malloc(sizeof(qapi_Net_Ping_V4_R2_t));
        if(ping_buf == NULL)
        {
        ...
        ping_resp = (qapi_Ping_Info_Resp_R2_t *)malloc(sizeof(qapi_Ping_Info_Resp_R2_t));
        if(ping_resp == NULL)
        {
          QCLI_Printf(qcli_net_handle, "Failed to allocate memory\n");
          result = QCLI_STATUS_ERROR_E;
        ...
        else
        {
          ...
          {
             QCLI_Printf(qcli_net_handle, "ICMP Error is %s type %d and code %d\n",
                         ping_resp->perror, ping_resp->ptype, ping_resp->pcode);
          }
 ...
 * Ping6_3  FE80::865D:D7FF:FE40:19C8 [-c  100] [-s  1500]  [-I  wlan0] [-w 1000] [-B FE80:::D7FF:19C8:8540:FEC8] [-t 60]
 ************************************************************************************************************/
static QCLI_Command_Status_t ping6_3(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List)
{
  multi_app_cmd_parser_t *cmd_parser_temp = NULL;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if(flag == FALSE)
	   {
         timeout = PING_DEFAULT_TIMEOUT ;
          ...
          ping6_buf = (qapi_Net_Ping_V6_R2_t *)malloc(sizeof(qapi_Net_Ping_V6_R2_t));
          if(ping6_buf == NULL)
          {
            QCLI_Printf(qcli_net_handle, "Failed to allocate memory\n");
            result = QCLI_STATUS_ERROR_E;
          ...
          QCLI_IP6CPY(ping6_buf->ipv6_addr,ip6addr);
          ping6_buf->bitmask |= QAPI_NET_PING_V6_DST_ADDR_MASK;
		  if(ip6src)
            QCLI_IP6CPY(ping6_buf->ipv6_src,ip6src);
 ...
 * set_sock_opt  [-e<enable/disable SACK>][-i <irt>] [-m <max_rxt>] [-t <overall_timer>] [-s <mss>] [-k<keepalive>] 
                 [-a<keep_idle>] [-t<keep_intvl>] [-c<keep_cnt>] [-r <rcv_buf>] [-b <snd_buf>] [-d <benchtx_sleep>] [-z delay ack interval]
...
proc_set_sock_opt(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
   ...
   for (i = 0; i < Parameter_Count ; i++)
   {
      ...
      else if(strcmp((char*)Parameter_List[i].String_Value, "-z") == 0)
      {
        ++i;
        if (Parameter_List[i].Integer_Is_Valid)
        {
           option[app_id].delayacktime = Parameter_List[i].Integer_Value;
        }
      }
   ...
   QCLI_Printf(qcli_net_handle, "Going to set: \n  IRT = %d\n  MR = %d\n  OT = %d\n  MSS = %d\n DELACK %d \n",option[app_id].irt,
         option[app_id].max_rxt, option[app_id].overall_timer, option[app_id].mss,option[app_id].delayacktime);
   ...
   if(prot && ifind && op)
    {
		...
		buf = (nwstats_t*)malloc(sizeof(nwstats_t));
        ...
        if(strncmp(prot,"IP4",3)==0 && buf)
        { 
            
            buf->u_prot_stats.ip =  (qapi_ipv4_stats_t *)malloc(sizeof(qapi_ipv4_stats_t));				
			protocol = QAPI_NWSTATS_IPv4;
		...
		else if(strncmp(prot,"IP6",3)==0 && buf)
        {
            buf->u_prot_stats.ip6 =  (qapi_ipv6_stats_t *)malloc(sizeof(qapi_ipv6_stats_t));
		...
		else if(strncmp(prot,"TCP",3)==0 && buf)
        {
            buf->u_prot_stats.tcp =  (qapi_tcp_stats_t *)malloc(sizeof(qapi_tcp_stats_t));
		...
		else if(strncmp(prot,"UDP",3)==0 && buf)
        {
            buf->u_prot_stats.udp =  (qapi_udp_stats_t *)malloc(sizeof(qapi_udp_stats_t));
		...
		else if(strncmp(prot,"ICMP4",5)==0 && buf)
        {
            buf->u_prot_stats.icmp =  (qapi_icmp_stats_t *)malloc(sizeof(qapi_icmp_stats_t));
		...
		else if(strncmp(prot,"ICMP6",5)==0 && buf)
        {
            buf->u_prot_stats.icmp6 =  (qapi_icmp6_stats_t *)malloc(sizeof(qapi_icmp6_stats_t));
...
proc_benchtoolconfig(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    else if(isCat4 == 3)
    {
      ...
      QCLI_Printf(qcli_net_handle," Invalid argument");
