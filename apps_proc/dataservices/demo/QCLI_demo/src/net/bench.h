...
#define CFG_PACKET_SIZE_MAX_RX  (65535)
...
/*+RAW IPv4*/
#define CFG_PACKET_SIZE_MIN_RAW_TX (49)  /*+RAW IPv6*///Minial IPv6 packet is 49 = IP Header(40) + UDP Header(8) + 1 byte data
#define CFG_PACKET_SIZE_MAX_RAW_RX (4096)
/*-RAW IPv4*/
...
typedef struct transmit_params
{
    ...
    int tx_port;
	/*+RAW IPv4*/
	char src_raw_ip_address[256];
	char dst_raw_ip_address[256];
	int hdrincl;
	/*-RAW IPv4*/
} TX_PARAMS;
...
{     
	...
	uint8_t server_address6[16];  /*+RAW IPv6*/
	int hdrincl;
	int debug_print;
	/*-RAW IPv4*/
} RX_PARAMS;
...
enum test_type
{
    ...
    /*+RAW IPv4*/
    RAW_IP_RX, // Raw Ip (Donwlink Test)
    RAW_IP_TX  // Raw Ip (Uplink Test)
    /*-RAW IPv4*/
};
...
extern SSL_INST ssl_inst[NO_OF_APPS][MAX_SSL_INST];
#endif
...
extern uint8_t *cert_data_buf[NO_OF_APPS];
extern uint16_t cert_data_buf_len[NO_OF_APPS];
...
typedef struct socket_option
{
    ...
    int rcv_buf;
    int snd_buf;
    uint32_t benchtx_sleep; /*duration in ms for benchtx to sleep*/
    int linger_time;
    int linger_time_val0;
    int delayacktime;
	int reassm_timer;
	int t_msl;
	int persfails;
