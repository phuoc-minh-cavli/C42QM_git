...
QCLI_Command_Status_t covert_store_cert(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    if (0 == strcmp("cert", (char *)Parameter_List[0].String_Value))
    {
        ...
        result = ssl_store_read_from_EFS_file((char *)cert_name, (void **)&cert_Buf, (size_t *)&cert_Size);
        if (QAPI_OK != result) 
        {
        ...
        result = ssl_store_read_from_EFS_file((char *)key_name, (void **)&key_Buf, (size_t *)&key_Size);
        if (QAPI_OK != result) 
        {
        ...
        for (argc = 0; argc + 1 < Parameter_Count; ++argc)
        {
            if (0 == strcmp("ca_list", (const char *)Parameter_List[argc].String_Value))
            {
                for (argc += 1 ; argc < Parameter_Count; ++argc)
                {
                    ...
                    result = ssl_store_read_from_EFS_file((char *)Parameter_List[argc].String_Value, (void **)&calist_info[ca_list_cnt].ca_Buf, (size_t *)&calist_info[ca_list_cnt].ca_Size);
                    if (QAPI_OK != result) 
                    {
    ...
    else if (0 == strcmp("psk_table", (char *)Parameter_List[0].String_Value))
    {
        ...
        /* Read the client certificate information */
        result = ssl_store_read_from_EFS_file((char *)psk_Tbl, (void **)&psk_Buf, (size_t *)&psk_Size);
        if (QAPI_OK != result) 
        {
    ...
    else if (0 == strcmp("di_cert", (char *)Parameter_List[0].String_Value))
    {
      ...
      /* Read the client certificate information */
      result = ssl_store_read_from_EFS_file((char *)di_cert, (void **)&dicert_Buf, (size_t *)&dicert_Size);
      if (QAPI_OK != result) 
      {
          CERT_PRINTF("ERROR: Reading PSK table information from EFS failed!! \r\n");
          result = QCLI_STATUS_ERROR_E;
...
QCLI_Command_Status_t store_cert(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    if ( Parameter_Count > 3 ) {
        ...
        result = ssl_store_read_from_EFS_file(file_path, (void **)&cert_data_buf[app_id], (size_t *)&cert_data_buf_len[app_id]);
...
QCLI_Command_Status_t get_cert(uint32_t Parameter_Count, QCLI_Parameter_t *Parameter_List, int app_id)
{
    ...
    if (qapi_connect(socketHandle, to, tolen) == -1)
    {
    ...
    req = (CERT_HEADER_T*) malloc(reqLen);
    ...
    if ((buf = malloc(SSL_CERT_BUF_LEN)) == NULL) {
        ...
        if (res > 0)
        {
            ...
            if (res > 0)
            {
                if (cert_data_buf_len[app_id] == 0)
                {
                    ...
                    cert_data_buf[app_id] = malloc(header->length);
