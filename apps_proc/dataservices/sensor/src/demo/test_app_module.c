...
qapi_Net_LWM2M_Data_v2_t *object_presence_sensor = NULL;
...
uint16_t object_check_readable(qapi_Net_LWM2M_Data_v2_t *object,
...
uint16_t object_check_numeric(qapi_Net_LWM2M_Data_v2_t *object,
                             qapi_Net_LWM2M_Uri_t *uri)
{
  uint16_t result = QAPI_NET_LWM2M_IGNORE_E;
  qapi_Net_LWM2M_Data_v2_t *target = NULL;
...
qapi_Status_t create_object(int id)
{
  ...
  if(id == 0)
  {
    ...
    {
      tx_byte_allocate(byte_pool_sensor, (VOID **)&object, sizeof(qapi_Net_LWM2M_Data_v2_t), TX_NO_WAIT);
      if(object == NULL)
      {
...
qapi_Status_t create_object_instance_app()
{
  ...
  {
    result = qapi_Net_LWM2M_Create_Object_Instance_v2(test_object_handler_app, &lwm2m_data);
  }
...
qapi_Net_LWM2M_Instance_Info_v2_t* obj_create_presence_sensor(qapi_Net_LWM2M_Data_v2_t *lwm2m_data)
{
  int i =0;
  qapi_Net_LWM2M_Data_v2_t *object = NULL;
  qapi_Net_LWM2M_Instance_Info_v2_t *instance = NULL;
  ...
  tx_byte_allocate(byte_pool_sensor, (VOID **)&instance, sizeof(qapi_Net_LWM2M_Instance_Info_v2_t), TX_NO_WAIT);
...
qapi_Status_t exec_data_sen(uint16_t instanceid, qapi_Net_LWM2M_Data_v2_t *object)
{
  qapi_Net_LWM2M_Instance_Info_v2_t *target = NULL;
  qapi_Net_LWM2M_Resource_Info_t *resource = NULL;
...
qapi_Status_t sensor_auto_create()
{
  ...
  if(test_object_handler_app == NULL)
  {
    ...
    {
      QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Presence sensor application successfully registered ");
    }
  }
  ...
  {
    QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Presence sensor object created ");
  }
  else if(result == QAPI_ERR_EXISTS)
  {
    QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Presence sensor object already created ");
  }
  ...
  if(result != QAPI_OK)
  {
    ...
    QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Presence sensor object instance creation failed  with error %d", result);
    goto end;
  }
  else
  {
    QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Presence sensor object instance successfully created ");
  }
...
qapi_Status_t set_dig_cnt(uint16_t count)
{
  ...
  if(count == 0 && resource1)
  {
    ...
    QFLOG2_MSG(sen_hndl, MSG_SSID_DFLT,MSG_MASK_2, "Digital input counter value set to %lu ",count);
...
int sensor_dam_app_start(void)
{
  ...
  txm_module_object_allocate((void **)&byte_pool_sensor, sizeof(TX_BYTE_POOL));
  tx_byte_pool_create(byte_pool_sensor, "Sensor application pool", free_memory_sensor, SENSOR_BYTE_POOL_SIZE);

  ret = txm_module_object_allocate((void **)&app_thread_handle, sizeof(TX_THREAD));
  if(ret != TX_SUCCESS)
  {
  ...
  ret = qflog_app_register(&sen_hndl, "sensor_obj");
    if(ret != QAPI_OK)
    {
      LOG_INFO(" Not able to register handle.");
    }
