...
#
# Copyright 2015-2016, 2018, 2019 by Qualcomm Technologies, Incorporated.
#                 All Rights Reserved.
...
# --------   ---     -----------------------------------------------------------
# 09/09/19	rrawat   Added changes for flavors in perform_merges and build_single_flavor
# 02/23/18   svl     EEPROM tools Support for VT
# 02/20/18   svl     Nand tools Support for VT
...
# 08/03/16   yps     Changed perform merges output folder name to CLANG40
...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                'Tools','sectools','sectools','features','isc','parsegen'))
import mbn_tools
...
WIN_AARCH64_TOOL         = "C:\\Apps\\LLVM\\4.0.2\\bin"
WIN_AARCH64_TOOL_PATH    = "C:\\Apps\\Linaro\\gcc-linaro-aarch64-none-elf-4.9-2014.07"
WIN_ARM_TOOL             = "C:\\Apps\\LLVM\\4.0.2\\bin"
WIN_ARM_TOOL_PATH        = "C:\\Apps\\gcc-linaro-arm-linux-gnueabihf-4.9-2014.05_win32"
CLANG40LINUX_BIN         = "/pkg/qct/software/llvm/release/arm/4.0.2/bin/"
CLANG40WIN_BIN           = "C:\\Apps\\LLVM\\4.0.2\\bin\\"
...
COPY_IMAGES_AS_IS        = ["jtagprogrammer", "nandjtagtools", "norjtagtools"]
	  ...
	  for release_mode in options.release.split(','):
		...
		if("Mdm9x05Pkg" == flavor.target):
			...
			XBL_loader_map_path = os.path.join(os.environ["WORKSPACE"],
										   ...
										   +flavor.variant
										    + "_Loader",
										   release_mode + "_" + arch_to_compiler("ARM"),
										   "ARM","QcomPkg","XBLLoader","XBLLoader","OUTPUT")
			map_file = os.path.join(XBL_loader_map_path ,"XBLLoader.map")
			op_file = os.path.join(XBL_loader_map_path ,"XBLL_Memoy_Usage.txt")
			if os.path.exists(Mapit_script) is True:
				print "Done [Memory Usage - Section wise Summary]"              
				cmds = "python" + ' ' + Mapit_script + " -t " + os.path.join(Mapit_script_root,"Qcs405 ") + map_file + ' > ' + op_file
				subprocess.call(cmds,shell=True)
...
def build_single_flavor(flavor, release_mode, compiler_flags, build_flags, use_intermediate_core, use_intermediate_loader):
  ...
  for item in flavor.dsc_list:
	...
	if platform.system() == "Windows":
	  ...
	  if "CLANG40_BIN" not in os.environ:
		os.environ["CLANG40_BIN"] = CLANG40WIN_BIN
	  if "CLANG40_ARM_PREFIX" not in os.environ:
		clang40_path = re.search(r"(.*)(bin\b)",os.getenv("CLANG40_BIN"))
		if clang40_path:
		  os.environ["CLANG40_ARM_PREFIX"] = clang40_path.group(1) + "tools\\bin\\"
	  ...
	  if "CLANG40_BIN" not in os.environ:
		os.environ["CLANG40_BIN"] = CLANG40LINUX_BIN
	  if "CLANG40_ARM_PREFIX" not in os.environ:
		clang40_path = re.search(r"(.*)(bin\b)",os.getenv("CLANG40_BIN"))
		if clang40_path:
		  os.environ["CLANG40_ARM_PREFIX"] = clang40_path.group(1) + "tools/bin/"
	...
	if not os.path.isdir(os.environ["CLANG40_BIN"]):
	  raise NameError("ERROR: buildit::build_single_flavor: " + \
						"Missing LLVM tool chain." + \
						"Expected location " + os.environ["CLANG40_BIN"])
	if not os.path.isdir(os.environ["CLANG40_ARM_PREFIX"]):
	  raise NameError("ERROR: buildit::build_single_flavor: " + \
						"Missing LLVM tool chain." + \
						"Expected location " + os.environ["CLANG40_ARM_PREFIX"])
	  
	is_clang40_bin_exist = False
	for path in os.environ["PATH"].split(os.pathsep):
	  if path == os.getenv("CLANG40_BIN"):
		is_clang40_bin_exist = True
		break
		
	if not is_clang40_bin_exist:
	  os.environ["PATH"] = os.environ["CLANG40_BIN"]+ os.pathsep + os.environ["PATH"]
	...
	build_cmd+=['-D', "VAR="+flavor.variant]
...
def perform_merges(merge_entry, target, sub_target ,variant, release_mode, use_intermediate_loader):
  ...
  # Added VARIANT information in path    

  dlls  = [item.replace('$(VAR)',variant) for item in dlls]
  merge_base_dll  = [item.replace('$(VAR)',variant) for item in merge_base_dll]
...
def sign_image(input_image, output_image, sign_type, variant, release_mode):
	...
	sign_bin = os.path.join(output_dir_root, "sign")
	unsigned_bin = os.path.join(output_dir_root, "unsigned")
	target_base = os.path.splitext(str(input_image))[0]
	preamble_file_name = target_base + "_preamble.mbn"
	target_combined_unsigned = os.path.splitext(str(input_image))[0] + "_combined_unsigned.mbn"
	target_intermediate_elf = os.path.splitext(str(input_image))[0] + "_intermediate_elf.mbn"
	...
	if (sign_type == "sbl1"):
		mbn_tools.image_preamble([], preamble_file_name, None)
		
		files_to_cat_in_order = [target_combined_unsigned]
		files_to_cat_in_order.insert(0, preamble_file_name)
		mbn_tools.concat_files(target_intermediate_elf, files_to_cat_in_order)
		mbn_tools.insert_SBL1_magicCookie([], target_intermediate_elf)
		
		shutil.copy2(target_intermediate_elf, os.path.join(unsigned_bin, "sbl1.mbn"))			
	
		shutil.copy2(signed_output_elf, target_combined_unsigned)
		files_to_cat_in_order = [target_combined_unsigned]
		files_to_cat_in_order.insert(0, preamble_file_name)
		mbn_tools.concat_files(target_intermediate_elf, files_to_cat_in_order)
		mbn_tools.insert_SBL1_magicCookie([], target_intermediate_elf)
		
		shutil.copy2(target_intermediate_elf, os.path.join(output_dir_root, "sbl1.mbn"))
		shutil.copy2(target_intermediate_elf, os.path.join(sign_bin, "default", sign_type, "sbl1.mbn"))
