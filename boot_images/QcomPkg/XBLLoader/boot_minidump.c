...
--------   ---        -------------------------------------------------------
12/28/17   rohik   L2_TCM changed to BOOTIMEM for Vipertooth
...
{ 
  /* Include BOOTIMEM, to be able to dump encryption keys, if required */
  if((base > SCL_BOOTIMEM_BASE_ADDR) && (base < ddr_end_addr)
