...
{
 BL_VERIFY( boot_pbl_shared_data.pbl_flash_shared_data != NULL, 
             BL_ERR_NULL_PTR_PASSED );
	
  return ( ((boot_flash_shared_dev_info_type*)boot_pbl_shared_data.pbl_flash_shared_data)->type );
...
)
{
  ...
  return (boot_pbl_shared_data.pbl_flash_shared_data);
...
void boot_pbl_log_milestones(boot_pbl_shared_data_type * pbl_shared_data)
{
  ...
  //Skip first entry as it is PBL shared data version
  for (count = 0;
       count < ((sizeof(*(pbl_shared_data->timestamps)) / sizeof(uint32)) - 1);
       count++, pbl_timestamp++)
  {
    pbl_us_value = ( (uint64)(*(pbl_timestamp+1) ) * PS_PER_PBL_TIMESTAMP_TICK ) / 1000000;
