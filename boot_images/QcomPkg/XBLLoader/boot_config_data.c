...
static whitelst_tbl_entry_type cdt_table_whitelist[2];
...
void boot_config_data_table_init(bl_shared_data_type* bl_shared_data)
{
  ...
  cdt_table_whitelist[0].start_addr = (uintnt)(&config_data_table);
  cdt_table_whitelist[0].size       = (uintnt)(CONFIG_DATA_TABLE_MAX_SIZE);
  ...
  {
	  boot_update_eeprom_config_data_table(&config_data_table_info);
  }
