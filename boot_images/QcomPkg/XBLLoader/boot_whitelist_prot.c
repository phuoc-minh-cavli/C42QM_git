...
{
  whitelst_tbl_entry_type new_whtlst[]={{0,0},{0,0}};
  
  new_whtlst[0].start_addr = start_addr;
  new_whtlst[0].size       = size;
...
* 
*/
boolean boot_add_entry_to_active_img_whtlst(uintnt start_addr, uintnt size)
{
  if(!size || (((uintnt)start_addr + (uintnt)size) < (uintnt)(start_addr)) 
...
* 
*/
boolean boot_set_entry_as_hdr_meta_whitelist(uintnt start_addr, uintnt size)
{
  whitelst_tbl_entry_type new_whtlst[]={{0,0},{0,0}};
...
* 
*/
boolean boot_add_entry_to_hdr_meta_whtlst(uintnt start_addr, uintnt size)
{
  if(!start_addr || !size 
    || (((uintnt)start_addr + (uintnt)size) < (uintnt)(start_addr)) 
...
* 
*/
static boolean check_range_against_whtlst(uintnt start_addr,uintnt size,whitelist_table *whitelist)
{
  uintnt entry_num=0;
