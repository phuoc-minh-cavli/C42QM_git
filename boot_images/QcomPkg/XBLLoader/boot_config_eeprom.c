...
--------   ---     ----------------------------------------------------------
04/26/19   aus     Remove check for ddr cdb
08/11/15   kpa     Added Integer Overflow check
06/09/14   kpa     Update to support 64-bit MSM8996 different align syntax
...
/*buffer to hold 2 eeprom page, we need it to read the 
  ...
  ALIGN(4) static uint8 page_buf[EEPROM_MAX_PAGE_BUF_SIZE*INIT_READ_NUM_PAGE];
...
static boot_boolean boot_eeprom_dev_init(DALDEVICEID DeviceId)
{
  ...
  do
  {
    ...
    if(boot_flash_open_partition(eeprom_dev_handle,(const unsigned char*)"0:ALL") != DAL_SUCCESS)
    {
...
*   None
*/
void boot_update_eeprom_config_data_table(struct cdt_info *cdt_info_ptr)
{ 
  ...
  do
  {
    ...
    {
      /* Only eeror on read failure if flag is set */
    ...
    */
    ...
    while(((uint32)cdb_meta_ptr - (uint32)cdt_info_ptr->cdt_ptr) < cdb_start_offset)
    {
      ...
      BL_VERIFY((cdb_total_length <= (cdb_total_length + cdb_meta_ptr->size)), BL_ERR_INTEGER_OVERFLOW|BL_ERROR_GROUP_BOOT);
    ...
    /* Integer overflow check */
    BL_VERIFY((cdb_total_length <= (cdb_start_offset + cdb_total_length)), BL_ERR_INTEGER_OVERFLOW|BL_ERROR_GROUP_BOOT);
