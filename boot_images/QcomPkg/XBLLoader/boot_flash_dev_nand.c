...
--------   ---     ------------------------------------------------------------
06/11/14   kedara  Support 64 bit compilation.
...
/* Page buffer to be allocated in DDR. Since BAM cannot access
  it in L2 as TCM.
*/
uint8 ALIGN(32) bl_page_buf[BL_PAGE_NUM_BYTES];
...
)
{
  ...
  if((boot_device == NAND_FLASH) || (boot_device == SPI_NAND_FLASH))
  {	 
     ...
     BL_VERIFY( smem_aarm_partition_table != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT );
  
     /* Fill the partition table in SMEM */
     boot_flash_boot_ops_get_parti_tbl((uintnt *)smem_aarm_partition_table, (uintnt *)&smem_parti_size);
  
     /* Failed to update SMEM with partition table if smem_parti_size is 0 */
     BL_VERIFY( smem_parti_size != 0, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT );
  }
...
)
{
  ...
  BL_VERIFY( (mibib_partition_tbl.magic1 == FLASH_PART_MAGIC1) &&
             ...
             BL_ERR_PARTI_TABLE_NOT_FOUND|BL_ERROR_GROUP_BOOT );
...
)
{
  ...
  BL_VERIFY( partition_name != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT );
...
)
{
  ...
  BL_VERIFY( parti_name != NULL, BL_ERR_PARTI_NAME_NOT_FOUND|BL_ERROR_GROUP_BOOT );
  ...
  BL_VERIFY( parti_entry_ptr != NULL, BL_ERR_PARTI_ENTRY_NOT_FOUND|BL_ERROR_GROUP_BOOT );
...
)
{
  ...
  BL_VERIFY( (flash_close(dev_priv.nand_if) == 0 ), BL_ERR_BOOT_FLASH_ACCESS|BL_ERROR_GROUP_BOOT );
...
)
{
  ...
  BL_VERIFY( ( dev_priv.nand_if != FS_NO_DEVICE ), BL_ERR_BOOT_FLASH_ACCESS|BL_ERROR_GROUP_BOOT );
  ...
  BL_VERIFY( dev_priv.trans_fact_if != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT );
