...
Copyright 2009 - 2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
/* Function prototypes */
boolean boot_check_error_code( uint32      err_code );
...
extern bl_shared_data_type bl_shared_data;

/* Error codes which indicate corruption */
uint32 corrupt_err_code[] = {BL_ERR_IMG_SECURITY_FAIL,
 	BL_ERR_IMG_SIZE_TOO_BIG,
	BL_ERR_IMG_SIZE_INVALID,	
	BL_ERR_IMG_HDR_INVALID,
	BL_ERR_SUBSYS_BOOT_FAIL,
	BL_ERR_BOOT_FLASH_ACCESS,
	BL_ERR_CLOBBER_PROTECTION_VIOLATION,
	BL_ERR_INVALID_ELF_FORMAT,
	BL_ERROR_GROUP_SECURITY,
	BL_ERR_FLASH_READ_FAIL, 
	BL_ERR_ELF_INVAL_PARAM,
	BL_ERR_INVALID_HASH_PTR,
...
=============================================================================*/
/*=======================================?====================================
 **  Function :  boot_check_error_code
 ** =========================================?=================================
*/
boolean boot_check_error_code
(
uint32      err_code 
)
{
	...
	for(i = 0; i < (sizeof(corrupt_err_code)/sizeof(uint32)); i++)
	{
		if(err_code == corrupt_err_code[i])
		return TRUE;
	}
...
)
{
  ...
  #ifdef FEATURE_FOTA_BOOT
	boot_images_entry * qsee_interface_image_entry = NULL;
	uint32 curr_image = bl_shared_data.sbl_qsee_interface.number_images;
	#endif
  ...
  boot_pm_vib_off();
	...
	{	
	 qsee_interface_image_entry =
		&(bl_shared_data.sbl_qsee_interface.boot_image_entry[curr_image-1]);
	} 
	

	if(booting_from_storage_media()   &&  (boot_check_error_code(err_code) == TRUE) &&
	(qsee_interface_image_entry!=NULL) )
	{  
	    boolean reset = TRUE;
		
		if((qsee_interface_image_entry->image_id == SECBOOT_APPSBL_SW_TYPE) && 
		  boot_check_fota_partition_cookie(APPS_IMAGE_INDEX))
		{	
		    boot_log_message("Fota Apps image is corrupted");
			boot_clear_image_fota_cookie(APPS_IMAGE_INDEX); 
		}
		else if((qsee_interface_image_entry->image_id == SECBOOT_AMSS_HASH_TABLE_SW_TYPE ) &&
        boot_check_fota_partition_cookie(MPSS_IMAGE_INDEX))		
	    {
			boot_log_message("Fota MPSS image is corrupted");
			boot_clear_image_fota_cookie(MPSS_IMAGE_INDEX); 
		}		
		else
		{
			reset = FALSE;
		...
		if ((reset==TRUE) && boot_set_fota_partition_cookie() == TRUE)
		{	
		   boot_log_message("Rebooting..\n");
  ...
  if((MAX_BOOTLOG_SECONDARY_ENTRIES + 1) >= error_handler_entry_count)
  {
    if(error_handler_entry_count == 1) /* Entering Error Handler for first time */
    {
      ...
      qsnprintf(error_message, BOOT_ERROR_MSG_LEN, 
             "Error code %x at %s Line %d", err_code, filename_ptr, line);
...
{
  /* Check the interrupt occurred and execute its registered handler */	
  IrqInterruptHandler(0);
  return;
}
