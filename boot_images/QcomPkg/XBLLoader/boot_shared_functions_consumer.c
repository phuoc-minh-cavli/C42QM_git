...
boolean boot_set_entry_as_img_whitelist(uintnt start_addr, uintnt size)
{
  return shared_functions_table->boot_set_entry_as_img_whitelist(start_addr, size);
}

boolean boot_whitelist_address_range_check(uintnt start_addr,uintnt size,buffer_type buf_typ)
{
...
{
	return shared_functions_table->boot_set_active_img_whitelist_table(whtlst_tbl_array_ptr);
}
...
uint32 boot_auth_image_hashtable (uint32 image_id, sec_img_auth_elf_info_type *sbl_elf_info, 
                                            sec_img_auth_whitelist_area_param_t *sbl_white_list_param)
{
	return shared_functions_table->boot_auth_image_hashtable(image_id, sbl_elf_info, sbl_white_list_param);
}												
...
int boot_sw_cipher(void *in, size_t in_len, void *out, size_t out_len,
        void   *key, size_t  key_len, void   *nonce, size_t  nonce_len, void   *tag, size_t  tag_len,
        boolean    dir )
{
  return shared_functions_table->boot_sw_cipher(in, in_len, out, out_len, key, 
                    key_len, nonce, nonce_len, tag, tag_len, dir );
}
...
void qmemset (void* dst_ptr, uint32 val, uint32 len)
{
	return shared_functions_table->qmemset(dst_ptr, val, len);
}
...
{
  return shared_functions_table->dcache_flush_region(addr, length);	
}
...
{
  return shared_functions_table->pmapp_chg_usb_get_vbus_presence(status);		
}

pm_err_flag_type pmapp_chg_usb_get_usb_connector_type(pmapp_chg_usb_connector_type *usb_type)
{
  return shared_functions_table->pmapp_chg_usb_get_usb_connector_type(usb_type);			
}

/*uint64 boot_get_elf_entry_address( void )
{
	return shared_functions_table->boot_get_elf_entry_address();
}*/

boot_boolean boot_mmu_page_table_map_single_mem_block_L1(const struct mem_block *map_ptr)
{
	return shared_functions_table->boot_mmu_page_table_map_single_mem_block_L1(map_ptr);
}
