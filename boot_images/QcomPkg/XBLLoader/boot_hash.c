...
bl_error_boot_type boot_parallel_hashInit
(
  uint32 sec_img_id
)
{
  ...
  sec_img_auth_parallel_hash_cmd_et hash_cmd = SEC_IMG_AUTH_PARALLEL_HASH_INIT;
  
  indirect_arg_list.arg[0] = (uint32) 0;
  indirect_arg_list.arg[1] = (uint32) 0;
  indirect_arg_list.arg[2] = (uint32) 0;

  dcache_flush_region((void *)&indirect_arg_list, sizeof(indirect_arg_list));
  
  smc_status = boot_fastcall_tz (TZ_PIL_SEC_IMG_AUTH_VERIFY_HASH_IN_PARLLEL_ID,
                                 TZ_PIL_SEC_IMG_AUTH_VERIFY_HASH_IN_PARLLEL_ID_PARAM_ID,
                                 sec_img_id,
                                 hash_cmd,
                                 0,
                                 &indirect_arg_list,
...
bl_error_boot_type boot_parallel_hashUpdate
(
  uint32 sec_img_id,
  const uint8 * buffer,
  uint32 buffer_size,
...
)
{
  ...
  boot_tz_indirect_arg_list_t indirect_arg_list;
  boot_tzt_result_regs_t result;
  sec_img_auth_parallel_hash_cmd_et hash_cmd = SEC_IMG_AUTH_PARALLEL_HASH_UPDATE;
  
  indirect_arg_list.arg[0] = (uint32) buffer_size;
  indirect_arg_list.arg[1] = (uint32) segment_idx;
  indirect_arg_list.arg[2] = (uint32) 0;

  dcache_flush_region((void *)buffer, buffer_size);
  dcache_flush_region((void *)&indirect_arg_list, sizeof(indirect_arg_list));
...
bl_error_boot_type boot_parallel_hashFinal
(
  uint32 sec_img_id,
  const uint8 * buffer,
  uint32 buffer_size,
...
)
{
  ...
  boot_tz_indirect_arg_list_t indirect_arg_list;
  boot_tzt_result_regs_t result;
  sec_img_auth_parallel_hash_cmd_et hash_cmd = SEC_IMG_AUTH_PARALLEL_HASH_FINALIZE;
  
  /* buffer_size and buffer needs to be NULL or it needs to point to a buffer other 
     than the one sent in the hashUpdate call. Hence setting it to NULL in this API */
  indirect_arg_list.arg[0] = (uint32) 0;
  indirect_arg_list.arg[1] = (uint32) segment_idx;
  indirect_arg_list.arg[2] = (uint32) 0;
  ...
  if(smc_status == 0)
  {
    status = boot_parallel_hashDeInit(sec_img_id);
  }                     
  return status;    
}

bl_error_boot_type boot_parallel_hashDeInit
(
  uint32 sec_img_id
)
{
  ...
  boot_tz_indirect_arg_list_t indirect_arg_list;
  boot_tzt_result_regs_t result;
  sec_img_auth_parallel_hash_cmd_et hash_cmd = SEC_IMG_AUTH_PARALLEL_HASH_DEINIT;
