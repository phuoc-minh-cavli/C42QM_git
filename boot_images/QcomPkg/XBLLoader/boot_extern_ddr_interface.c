...
$Header: //components/rel/boot.xf/0.2/QcomPkg/XBLLoader/boot_extern_ddr_interface.c#2 $
$DateTime: 2019/04/23 17:39:55 $
$Author: pwbldsvc $
    
when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/12/16   sk      Added ddr_boot_funcs attribute to separate SBL and RPM functions 
...
extern boolean ddr_remapper(void);
extern boolean ddr_preinit_rpm(void);
...
void boot_ddr_test(uint32 clk_speed)
{
  ...
  static volatile ddr_test_suite    test_suite = DDR_TEST_SUITE_ACCESS_RAM_TEST;
  ...
  if (test_suite != DDR_TEST_SUITE_EMPTY)
  {
	...
	if (test_suite != DDR_TEST_SUITE_ACCESS_RAM_TEST)
		ddr_test_exit();
  }
 
  /* restore test iteration to 1 */
  test_iterations = 1;
  test_suite = DDR_TEST_SUITE_EMPTY;
} /* boot_ddr_test */
...
*/
__attribute__((section("ddr_boot_funcs")))
void boot_ddr_preinit_rpm(void)
{
  ddr_preinit_rpm();
}
