 ...
 *
 * Copyright 2010-2012, 2014-2015, 2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
...
--------     ---     ----------------------------------------------------------
08/14/19     aus     Added non-blocking flash APIs
...
flash_init_buffer_descriptor (uint32 op_type, uintnt page_buff_addr,
                              uintnt spare_buff_addr,
                              ...
                              struct flash_page_iovec  *page_data_iovec)
{
  ...
  BL_VERIFY(main_desc_buff != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT); 
  ...
  BL_VERIFY(spare_desc_buff != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT);
...
flash_nand_write_page (fs_device_t self, page_id page, void *data)
{
  ...
  flash_init_buffer_descriptor(write_type, (uintnt) data, (uintnt) NULL,
  ...
  void *page_data, void *spare_udata, uint32 spare_udata_blen)
{
  ...
  /* Initialize buffer descriptors */
  flash_init_buffer_descriptor(write_type, (uintnt) page_data,
                               (uintnt) spare_udata,
                               (DALSysMemDescList *) &page_buff_desc,
                               &page_data_iovec);
...
flash_nand_is_page_erased (fs_device_t self, page_id page)
{
  ...
  flash_init_buffer_descriptor(read_type,
                               (uintnt)(self->page_buf), (uintnt) NULL,
                               (DALSysMemDescList *) &page_buff_desc,
                               &page_data_iovec);
  ...
  desc_buff = boot_DALFW_MemDescBufPtr((DALSysMemDescList *) &page_buff_desc,
                                  MAIN_BUFF_DESC);
  BL_VERIFY(desc_buff != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT);                                  
...
flash_nand_read_page (fs_device_t self, page_id page, void *data)
{
  ...
  /* Initialize buffer descriptors */
  flash_init_buffer_descriptor(read_type, (uintnt) data, 0,
    (DALSysMemDescList *) &page_buff_desc, &page_data_iovec);
...
/***********************************************************************
**  Function :      flash_nand_start_read_page
...
*/
int
flash_nand_start_read_page (fs_device_t self, page_id page, void *data, page_id num_pages)
{
  int ret_status = FS_DEVICE_OK;     /* return status of read page operation */
  struct flash_page_iovec  page_data_iovec;
  uint32 read_type = FLASH_READ_MAIN;
  ...
  ret_status = boot_flash_start_read_pages(self->handle_flash,
                                     (enum page_read_opcode) read_type,
                                     (DALSysMemDescList *) &page_buff_desc,
                                     &page_data_iovec);
  ...
  return ret_status;
} /* End of flash_nand_start_read_page */

/***********************************************************************
**  Function :      flash_nand_wait_read_page
...
*/
int
flash_nand_wait_read_page (fs_device_t self)
{
  int ret_status = FS_DEVICE_OK;     /* return status of read page operation */
  //struct flash_page_iovec  page_data_iovec;
  //uint32 read_type = FLASH_READ_MAIN;
  ...
  ret_status = boot_flash_wait_read_pages(self->handle_flash);
  ...
  return ret_status;
} /* End of flash_nand_wait_read_page */
  ...
  uint32 spare_udata_blen)
{
  ...
  /* Initialize buffer descriptors */
  flash_init_buffer_descriptor(read_type, (uintnt) NULL,
                               (uintnt) spare_udata,
                               (DALSysMemDescList *) &page_buff_desc,
                               &page_data_iovec);
  ...
  void *spare, uint32 spare_udata_blen)
{
  ...
  page_data_iovec.total_page_count = 1;
    ...
    /* Initialize buffer descriptors */
  flash_init_buffer_descriptor(read_type,  (uintnt) data,
       (uintnt)spare, (DALSysMemDescList *) &page_buff_desc, &page_data_iovec);
  ...
  uint32 spare_offset, uint32 len, void *data)
{
  ...
  /* Initialize buffer descriptors */
  flash_init_buffer_descriptor(read_type, (uintnt)NULL,  (uintnt)data,
     (DALSysMemDescList *) &page_buff_desc, &page_data_iovec);
...
flash_nand_bad_block_check (fs_device_t self, block_id block)
{
  ...
  if (DAL_SUCCESS == boot_flash_block_get_state(self->handle_flash,
                                                block, &block_state))
  {
...
fs_device_t flash_open(char *partition_name, int whichflash)
{
                                       ...
                                       (const unsigned char *)partition_name))
  {
    ...
    /* Read Page                      */
    fs_device->start_read                = flash_nand_start_read_page;

    /* Read Page                      */
    fs_device->wait_for_read             = flash_nand_wait_read_page;    
    ...
    if (DAL_SUCCESS == boot_DALSYS_MemRegionAlloc(DALSYS_MEM_PROPS_UNCACHED |
        ...
        &handle_mem, NULL))
      {
        ...
        nand_page_buf = (uintnt) mem_info.VirtualAddr;
