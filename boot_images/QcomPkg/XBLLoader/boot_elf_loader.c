...
Copyright 2011-2019 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
...
--------   ---        -------------------------------------------------------
08/14/19   aus        Updated the flash read API for parallel load support
06/14/19   aus        Do ZI before segment load as parallel hash API locks the hashed regions
06/11/19   aus        Added support for UIE 
12/27/18   rohik      Disabled parallel loading and hashing feature for Tiny modem image
...
static Elf32_Phdr phdr_array_32[MI_PBT_MAX_SEGMENTS] SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
static Elf64_Phdr phdr_array[MI_PBT_MAX_SEGMENTS] SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));

/* ELF Header */
static Elf32_Ehdr ehdr_32 SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
static Elf64_Ehdr ehdr SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));

/* Output digest buffer for comparing hashes */
static uint8 verify_digest_buf[PBL_SHA_HASH_MAX_LEN] SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
extern hash_alg_t sbl_hash_algorithm SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
extern uint32 sbl_hash_len SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));

/* ELF format (object class), default to 64bit */
static uint8 elf_format SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));

static void *ehdr_ptr, *phdr_array_ptr SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
static uint32 ehdr_size, phdr_size SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
static uint16 phdr_num SECTION(".bss.BOOT_INTERNAL_HEAP") __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
...
static whitelst_tbl_entry_type elf_hdr_meta_whitelist[5];

boolean is_load_parallel = FALSE;
...
* 
*/
static void boot_elf_load_seg_generic(void * phdr, uint8 *dest_addr, uint32 sec_img_id, uint32 segment_idx, buffer_type bf_type)
{
  boolean success = FALSE;
  uint64 phdr_offset, phdr_filesz, phdr_memsz;
  uint32 rem_size, prev_copy_size, prev_rem_size;
  uint32 copy_size = 0;
  bl_error_boot_type hash_result = FALSE;
  uint8 *prev_data_ptr = NULL;
  ...
  if(phdr_memsz)
  {
    /* There is a whitelist check in the flash read logic but that will only
     ...
     be safe. */
  	BL_VERIFY(boot_whitelist_address_range_check((uintnt)dest_addr,(uintnt)phdr_memsz,bf_type),
            BL_ERR_CLOBBER_PROTECTION_VIOLATION);
  }
  rem_size = phdr_filesz;
  prev_rem_size = rem_size;
  read_parallel = READ_SEQUENTIAL;

  /* If uninitialized data exists, make sure to zero-init */
  if (phdr_memsz && (phdr_memsz > phdr_filesz) && ((dest_addr + phdr_filesz) >= dest_addr))
  {
  ...
  dcache_flush_region((void *)(dest_addr + phdr_filesz), (phdr_memsz - phdr_filesz));
  ...
  {
	  if(is_load_parallel == TRUE)
	  {
		  hash_result = boot_parallel_hashInit(sec_img_id);
          BL_VERIFY( hash_result == BL_ERR_NONE, BL_ERR_IMG_SECURITY_FAIL);
	  }
	  
	  while((copy_size < PARALLEL_LOAD_SIZE) && (rem_size > 0))
	  {
		  success = boot_flash_trans_read( trans_if,
                                     (void *)((uint32)(dest_addr) + phdr_filesz - rem_size),
                                     (uint32)(phdr_offset + phdr_filesz - rem_size),
                                     (uint32) rem_size,
                                     bf_type, &copy_size);
          BL_VERIFY( success, BL_ERR_FLASH_READ_FAIL|BL_ERROR_GROUP_BOOT );
		  
		  rem_size = prev_rem_size - copy_size;
	  }
		
		prev_data_ptr = dest_addr + (phdr_filesz - prev_rem_size);
		...
		while (rem_size>0)
	    {
			...
			if(prev_copy_size != 0 && (is_load_parallel == TRUE))
			{
				hash_result = boot_parallel_hashUpdate(sec_img_id, prev_data_ptr, prev_copy_size, segment_idx);
                BL_VERIFY( hash_result == BL_ERR_NONE, BL_ERR_IMG_SECURITY_FAIL); 
			}
		...
		if(is_load_parallel == TRUE)
		{
		    ...
		    hash_result = boot_parallel_hashFinal(sec_img_id, dest_addr, phdr_filesz, segment_idx);
            BL_VERIFY( hash_result == BL_ERR_NONE, BL_ERR_IMG_SECURITY_FAIL);
			
			BL_VERIFY(((read_parallel == READ_SEQUENTIAL) || (read_parallel == READ_SEQUENTIAL_WITH_CHECK)), BL_ERR_SBL);
		}
		else
	    {
			/* Flush the segment memory region after loading */
            dcache_flush_region((void *)dest_addr, phdr_filesz);
		}
  }
}
...
* 
*/
static void boot_elf_load_seg_in_chunks(void * phdr, buffer_type bf_type)
{
  boolean success = FALSE;  
  ...
  if(phdr_entry_memsz)
  {
    /* There is a whitelist check in the flash read logic but that will only
     check the elf file size length.  Need to ensure that zero init regions
  	are also checked.  So make an explicit whitelist check using mem size to
     be safe. */
  	BL_VERIFY(boot_whitelist_address_range_check((uintnt)phdr_entry_paddr,(uintnt)phdr_entry_memsz,bf_type),
...
* 
*/
static void boot_elf_load_seg(void * phdr, uint32 sec_img_id, uint32 segment_idx)
{
  ...
  if(boot_sec_img_auth_is_valid_segment(elf_format, phdr))
  {
    boot_elf_load_seg_generic(phdr, (uint8 *)(uintnt)phdr_entry_paddr, sec_img_id, segment_idx, IMAGE_BODY_TYPE);
  }
}
    ...
    else
  {
    ...
    */
    ...
    {  
      ...
      boot_elf_load_seg_in_chunks(phdr_entry_ptr, HDR_META_INFO_TYPE);
    }
                               ...
                               boot_boolean auth)
{
  ...
  for(index = 0; index < phdr_num; index++)
  {
    ...
    boot_elf_load_seg(phdr_entry_ptr, image_id, index);
  }

  if((auth == TRUE) && (is_load_parallel == FALSE))
  {
...
boolean boot_set_elf_and_prog_header(void *elf_hdr_ptr, void *prog_hdr_ptr)
{
  ...
  {  
	...
	dcache_inval_region((void *)ehdr_ptr, sizeof(Elf64_Ehdr));
	dcache_inval_region((void *)phdr_array_ptr, phdr_array_size);
                                               ...
                                               mi_boot_image_header_type * header_ptr)
{
  ...
  elf_hdr_meta_whitelist[0].start_addr = (uintnt)&ehdr_32;
  elf_hdr_meta_whitelist[0].size       = (uintnt)(sizeof(ehdr_32));
  elf_hdr_meta_whitelist[1].start_addr = (uintnt)(&phdr_array_32);
  elf_hdr_meta_whitelist[1].size       = (uintnt)(sizeof(phdr_array_32));
  elf_hdr_meta_whitelist[2].start_addr = (uintnt)&ehdr;
  elf_hdr_meta_whitelist[2].size       = (uintnt)(sizeof(ehdr));
  elf_hdr_meta_whitelist[3].start_addr = (uintnt)(&phdr_array);
  elf_hdr_meta_whitelist[3].size       = (uintnt)(sizeof(phdr_array));
  
  // set the elf header, prg header whietlist and add entry for hash buffer region
  BL_VERIFY(boot_set_active_hdr_meta_whitelist_table(elf_hdr_meta_whitelist),BL_ERR_IMG_SECURITY_FAIL);  
  ...
  while(rem_size < sizeof(Elf64_Ehdr))
  {
...
void boot_elf_load_init_hash_segment(uint8 * partition_id, mi_boot_image_header_type *header_ptr)
{
 ...
 boot_elf_open_and_parse_elf_header(partition_id,
  ...
  while(rem_size < phdr_array_size)
  {
    ...
    else
  {
    ...
    {
      rem_size = 0;
	  while(rem_size < sizeof(*header_ptr))
	  {
      ...
      boot_elf_load_seg_generic(phdr_entry_ptr, hash_segment_base_ptr, 0, index, HDR_META_INFO_TYPE);
...
void boot_load_elf_image(whitelst_tbl_entry_type * elf_segment_whitelist,
                         uint32 image_id, 
                         boot_boolean auth,
						 uint32 image_type, uint32 uie_enabled)
{
  /* Open the translation interface if Not already opened. (Usually its opened when elf header
     ...
     target ELF images to re-open and use the translation interface when they are loaded. 
  */
  if((image_type != CONFIG_IMG_ELF_SEG) && (uie_enabled != 0))
  {
      is_load_parallel = TRUE;
  }
