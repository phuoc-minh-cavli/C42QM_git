...
Copyright 2014-2016,2019 by Qualcomm Technologies, Incorporated.  
All Rights Reserved.
============================================================================*/
/*===========================================================================
...
/**
 * @brief This function will unlock IMEM region for image loading
 *
 * @param[in]
 *   None
 *
 * @return
 *   Boolean indicating if region is unlocked or not
 *
 * @sideeffects
 *   None
 *
 *
 */
boot_boolean boot_qsee_unlock_region(void)
{
  int smc_status;
  boot_boolean status = FALSE;
  boot_tzt_result_regs_t result;

  smc_status = boot_fastcall_tz(TZ_INIT_CMD,
                                TZ_INIT_PARAM_ID,
                                0,0,0,0, &result);
...
/**
 * @brief This function verifies if UIE is enabled
 *      
 *
 * @param[in,out] sec_img_id            SW Image ID
 *                sec_img_data          Pointer to ELF header         
 ...
 *
 */
uint32 boot_sec_img_auth_verify_uie(uint32 sec_img_id, 
               const sec_img_auth_elf_info_type *sec_img_data,
               sec_img_auth_whitelist_area_param_t *whitelist_area,
               sec_img_auth_verified_info_s *v_info)
{
  ...
  auth_error_type = boot_fastcall_tz (TZ_PIL_SEC_IMG_VERIFY_ENC_ID,
                                    TZ_PIL_SEC_IMG_VERIFY_ENC_ID_PARAM_ID,
                                    sec_img_id,
                                    sec_img_data,
                                    sizeof(*sec_img_data),
                                    &indirect_arg_list,
