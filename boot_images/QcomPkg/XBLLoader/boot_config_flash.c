...
Copyright 2012-2013, 2015-2017, 2019 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
============================================================================*/
...
boolean cdt_contains_ddr_cdb = FALSE;
...
{ 
  ...
  do
  {
    ...
    boot_set_entry_as_img_whitelist((uintnt)cdt_buf , (uintnt)sizeof(cdt_buf));
	
	while(rem_size < sizeof(cdt_buf))
	{
		success = boot_flash_trans_read(trans_if, cdt_buf, 0, sizeof(cdt_buf), IMAGE_BODY_TYPE, &rem_size);
	}
    ...
    */
         ...
         cdb_index++, cdb_meta_ptr++, cdb_meta_offset+=sizeof(struct cdb_meta))
    {
	  ...
	  boot_set_entry_as_img_whitelist((uintnt)cdb_ptr , (uintnt)cdb_size);	
	  rem_size = 0;
      
	  while(rem_size < cdb_meta_ptr->size)
	  {
		success = boot_flash_trans_read(trans_if,
                                cdb_ptr,
                                cdb_meta_ptr->offset,
                                cdb_meta_ptr->size, IMAGE_BODY_TYPE, &rem_size);
	  }
