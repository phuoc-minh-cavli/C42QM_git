...
class BaseCASSSigningRequest(SecPluginIntf_Abs):
    ...
    def _init_attribute_dict(self, hash_to_sign, signing_attributes):
        ...
        # SHA256
        if signing_attributes.hash_algorithm is None or signing_attributes.hash_algorithm.lower() == "sha256":
            self.attribute_dict.update({SignAttr.SHA256: "TRUE"})
        else:
            self.attribute_dict.update({SignAttr.SHA256: "FALSE"})
