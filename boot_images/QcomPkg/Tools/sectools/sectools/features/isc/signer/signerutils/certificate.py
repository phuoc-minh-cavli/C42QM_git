...
class Certificate(CertificateBase):
    ...
    def _repr_properties(self):
        properties = [(self.sign_attr.SW_ID, self.sw_id),
                      (self.sign_attr.HW_ID, self.hw_id)
                     ]

        if self.in_use_soc_hw_version is not None:
            properties += [(self.sign_attr.IN_USE_SOC_HW_VERSION, self.in_use_soc_hw_version)]

        if (self.in_use_soc_hw_version == 1 or
                (self.soc_hw_version is not None and self.in_use_soc_hw_version is None)):
            properties += [(self.sign_attr.SOC_HW_VERSION, self.soc_hw_version),
                           (self.sign_attr.MASK_SOC_HW_VERSION, self.mask_soc_hw_version)]

        if self.soc_vers is not None:
            properties += [(self.sign_attr.SOC_VERS, self.soc_vers)]

        if self.use_serial_number_in_signing is not None:
            properties += [(self.sign_attr.USE_SERIAL_NUMBER_IN_SIGNING, self.use_serial_number_in_signing)]
        if self.oem_id_independent is not None:
            properties += [(self.sign_attr.OEM_ID_INDEPENDENT, self.oem_id_independent)]

        properties += [(self.sign_attr.DEBUG, self.debug),
        ...
        if self.root_cert_sel is not None:
            properties += [(self.sign_attr.ROOT_CERT_SEL, self.root_cert_sel)]
        if self.revocation_enablement is not None:
            properties += [(self.sign_attr.REVOCATION_ENABLEMENT, self.revocation_enablement)]
        if self.activation_enablement is not None:
            properties += [(self.sign_attr.ACTIVATION_ENABLEMENT, self.activation_enablement)]
        if self.root_revoke_activate_enable is not None:
            properties += [(self.sign_attr.ROOT_REVOKE_ACTIVATE_ENABLE, self.root_revoke_activate_enable)]
        if self.uie_key_switch_enable is not None:
            properties += [(self.sign_attr.UIE_KEY_SWITCH_ENABLE, self.uie_key_switch_enable)]
