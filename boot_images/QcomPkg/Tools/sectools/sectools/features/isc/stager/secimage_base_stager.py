...
class SecimageBaseStager(BaseStager):
    ...
    def _create_imageinfo(self, img_config_parser, parsegen_config, sign_id, image_path,
                          gen_multi_image=False, src_image=None, dest_image=None,
                          crypto_params={}, imageinfo_class=ImageInfo):
        if not gen_multi_image and sign_id not in MULTI_IMAGE_SIGN_ID.values():
            ...
            try:
                ...
                raise RuntimeError(multi_image_string() + " images are not supported for chipset {0}"
        ...
        image_info = imageinfo_class(image_path, sign_id, img_config_block, img_config_parser,
                                     parsegen_config, gen_multi_image=gen_multi_image,
                                     authority=self.authority, crypto_params=crypto_params)
