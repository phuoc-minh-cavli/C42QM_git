...
Copyright 2010-2014, 2018-19 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
--------   ---     ------------------------------------------------------------
06/14/19   aus     Added a parameter in TX to distinguish between command and data packets
...
#define AC_PARALLEL_HASH_ALIGNMENT 0x1000
...
uint8 ALIGN(32) sahara_packet_buffer[SAHARA_MAX_PACKET_SIZE_IN_BYTES];
...
static uint32 sahara_parallel_buf_remaining_size = 0;
static uint32 missed_parallel_hash_cnt = 0;
...
)
{
  ...
  if( FALSE == sahara_dispatch_tbl->elf_optimized_segment_load() && (length > max_packet_size))
  {
  ...
  {
    //with Access Control (AC) enabled, for parallel hashing the buffers have
    //to be 4K aligned
    sahara_parallel_buf_remaining_size =
          ((uint32)sahara_parallel_buf + sahara_parallel_size) % AC_PARALLEL_HASH_ALIGNMENT;
    sahara_parallel_size -= sahara_parallel_buf_remaining_size;
    if (sahara_parallel_size != 0)
    {
  ...
  if (image_data_size_max >= image_data_size)
  {
           ...
           (image_transfer_size_left != 0))
    {
      ...
      if (sahara_state != SAHARA_STATE_WAIT_RESET)
      {
		...
		dcache_flush_region(image_dest, image_transfer_size);
        ...
        if (sahara_auth_elf && sahara_parallel_init)
        {
          ...
          sahara_parallel_buf = image_dest - sahara_parallel_buf_remaining_size;
          sahara_parallel_size =
                    image_transfer_size + sahara_parallel_buf_remaining_size;
        }
      }
      else
      {
...
static enum boot_sahara_status boot_sahara_receive_image_binary( void )
{
  ...
  do
  {
    ...
    if (! sahara_dispatch_tbl->valid_address_range((uintnt)(image_dest), 
                                                   (uintnt)(image_data_size_max),
												   IMAGE_BODY_TYPE))
    {
      if (sahara_dispatch_tbl->skip_image_load())
      {
...
static enum boot_sahara_status boot_sahara_receive_image_elf( void )
{
  ...
  do
  {
    ...
    dcache_flush_region(elf_header, sizeof(Elf32_Ehdr));	
    ...
    sahara_elf_image_info->hash_table_index = hash_table_index;
         ...
         phdr_index++)
    {
      ...
      // Only load the segments which are loadable and non-zero memsize
      if ( (boot_sahara_elf_segment_is_loadable(
            sahara_elf_phdrs[phdr_index].type,
            sahara_elf_phdrs[phdr_index].memory_attribute)) &&
           (sahara_elf_phdrs[phdr_index].mem_size > 0) )
      {
        ...
        // Validate destination address range
        // Take into account the added overflow due to buffer size
        // passed into H/W receive. image_data_size_max must be <= segment.mem_size to avoid corrupting other memory
        image_data_size_max = image_data_size;
        buff_ovf = image_data_size % max_packet_size;
        if (buff_ovf > 0)
        {
        ...
        if (! sahara_dispatch_tbl->valid_address_range((uint64)(image_dest),
                                                       (uint64)(sahara_elf_phdrs[phdr_index].mem_size),
													   IMAGE_BODY_TYPE))
        {
          // ERROR: Invalid destination address
          status = SAHARA_NAK_INVALID_DEST_ADDR;
        ...
        if(sahara_elf_phdrs[phdr_index].mem_size > 0)  
        {
          ...
          if (image_data_size != 0)
          {
                 ...
                 MI_PBT_HASH_SEGMENT))
            {
              ...
              if (!sahara_auth_tbl->init_hash(phdr_index))
              {
                // ERROR: Failed to verify hash for a given segment of ELF image
                 ...
                 MI_PBT_HASH_SEGMENT))
            {
              ...
              if(!sahara_auth_tbl->finalize_hash(sahara_parallel_buf,
                                                 sahara_parallel_size,
                                                 phdr_index))
              {
                sahara_parallel_hash = FALSE;
              ...
              sahara_parallel_hash = FALSE;
              sahara_parallel_buf_remaining_size = 0;
            }
          }
...
static void boot_sahara_process_packets( void )
{
  ...
  while (!sahara_done)
  {
    ...
    length = boot_sahara_handle_rx(
              (uint8 *)sahara_packet_buffer, 
              sahara_dispatch_tbl->get_max_packet_size(),
              SAHARA_MAX_PACKET_SIZE_IN_BYTES,
              &err_code,
