 ...
 * targets.
 *
 * Copyright (c) 2008-2010,2012,2016, 2018, 2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ------------------------------------------------
 * 2019-12-05   sl      New error to indicate op not failed but not done
 * 2019-01-30   sl      Add non-blocking page read support
 * 2018-02-20   svl     Support for VT
 * 2016-07-11   eo      Extend flash dal device ids
...
/** flash device ids */
#define FLASH_DEVICEID_1                0x1
#define FLASH_DEVICEID_2                0x2
#define FLASH_DEVICEID_3                0x3
#define FLASH_DEVICEID_4                0x4
#define FLASH_DEVICEID_5                0x5
#define FLASH_DEVICEID_6                0x6
#define FLASH_DEVICEID_7                0x7
#define FLASH_DEVICEID_8                0x8
...
#define FLASH_DEVICE_OP_NOT_DONE        (-12) /**< Error applicable only for
                                                   write/erase ops where NAND
                                                   didn't accept write/erase
                                                   request.
                                                   This should be considered as
                                                   request for retry,
                                                   the block should
                                                   not be marked as bad for this
                                                   error */
...
#define INHERIT_FLASH_VTBL(type) \
  ...
  \
  int (*start_page_read)(type* handle, enum page_read_opcode read_type, \
    dalsys_mem_desc_list *buffer_desc, struct flash_page_iovec *page_iovec); \
  \
  int (*wait_for_page_read)(type* handle)
...
/**
 * This API allows to start reading a single page within a block
 * in non blocking mode and return without waiting for the page read
 * to complete. flash_wait_for_page_read needs to be called after this
 * api to check the status of the page read.
 *
 * @param handle [IN]
 *   DAL interface handle
 *
...
/**
 * This argument points to a set of buffer descriptors. This
 * API assume that there are at least 2 buffer descriptors. A client can
 * have up to 128 buffer descriptors in an API call.
 * @par
 * The main buffer descriptor points to the main data buffer and the spare buffer
...
/**
 * @param page_iovec [IN]
 *   I/O vector for flash read. Each element in the vector
 *   contains size of main and spare data to operate,
 *   and logical start page with number of pages to read.
 *
 * @return int [OUT]
 *   Result of the operation.
 *
 * @note
 *   IMPORTANT: The 'page_iovec' allows clients to specify number of
 *   pages read from a given start page. Currently multi page read is
 *   not supported so total_page_count should always be 1.
 *   flash_wait_for_page_read needs to be called after this
 *   api to check the status of the page read, between flash_start_page_read
 *   and flash_wait_for_page_read, there should be no other flash api called
 *
 */
static __inline int flash_start_page_read(flash_handle_t handle,
  enum page_read_opcode read_type, dalsys_mem_desc_list *buffer_desc,
  struct flash_page_iovec *page_iovec)
{
  return GET_FLASH_HANDLE_VTBL(handle)->start_page_read((flash_handle *) handle,
    read_type, buffer_desc, page_iovec);
}

/**
 * This api waits for the previously started page read to complete.
 * api to check the status of the page read.
 *
 * @param handle [IN]
 *   DAL interface handle
 *
 * @note
 *   IMPORTANT: This api should only be called after a call to
 *   flash_start_page_read().
*/
static __inline int flash_wait_for_page_read(flash_handle_t handle)
{
  return GET_FLASH_HANDLE_VTBL(handle)->wait_for_page_read((flash_handle *) handle);
}
