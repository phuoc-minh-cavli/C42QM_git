...
/* EFI_ADC_PROCESS_CONVERSION_RESULT */
/** @ingroup efi_adc_process_conversion_result
  @par Summary
  Scales an ADC code to a useable value.

  @param[in]   pEfiChannel    ADC channel to scale;
                              see #EfiAdcDeviceChannelType for details.
  @param[in]   uAdcCode       ADC code to scale.
  @param[out]  pEfiAdcResult  Result of the ADC reading;
                              see #EfiAdcResultType for details.
...
typedef
EFI_STATUS
(EFIAPI *EFI_ADC_PROCESS_CONVERSION_RESULT)(
  IN EfiAdcDeviceChannelType *pEfiChannel,
  IN UINT32 uAdcCode,
  OUT EfiAdcResultType *pEfiAdcResult
);

/* EFI_ADC_PROCESS_CONVERSION_RESULT_INVERSE */
/** @ingroup efi_adc_process_conversion_result
  @par Summary
  Scales a physical value to an ADC code.

  @param[in]   pEfiChannel    ADC channel to scale;
                              see #EfiAdcDeviceChannelType for details.
  @param[in]   nPhysical      Value to scale to ADC code.
  @param[out]  pEfiAdcResult  Result of the ADC reading;
                              see #EfiAdcResultType for details.
...
typedef
EFI_STATUS
(EFIAPI *EFI_ADC_PROCESS_CONVERSION_RESULT_INVERSE)(
  IN EfiAdcDeviceChannelType *pEfiChannel,
  IN INT32 nPhysical,
  OUT EfiAdcResultType *pEfiAdcResult
);
...
struct _EFI_ADC_PROTOCOL {
  ...
  EFI_ADC_PROCESS_CONVERSION_RESULT AdcProcessConversionResult;
  EFI_ADC_PROCESS_CONVERSION_RESULT_INVERSE AdcProcessConversionResultInverse;
};
