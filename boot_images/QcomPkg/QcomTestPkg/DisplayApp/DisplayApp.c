...
static EFI_STATUS DisplaySetEnvVar(CHAR8* pName, const CHAR16 *pVarname, UINT32 uValue, UINT32 uMask, UINTN uSize, UINTN uUpdate);
  ...
  )
{
  ...
  else if (0==AsciiStriCmp("disabledisplay", Param))
  {
      ...
      DisplaySetEnvVar(Param, (CHAR16*)&EnvVarList[DISPLAY_VAR_DISABLEDISPLAY].sVarName, uValue, 0x0, EnvVarList[DISPLAY_VAR_DISABLEDISPLAY].nVarSize, HasValue);
  }
  else if (0==AsciiStriCmp("amoled", Param))
  {
      ...
      // Configure or display the environment variable
      DisplaySetEnvVar(Param, (CHAR16*)&EnvVarList[DISPLAY_VAR_DISPLAYFLAGS].sVarName, uFlag, uMask, EnvVarList[DISPLAY_VAR_DISPLAYFLAGS].nVarSize, HasValue);
  }
  else if (0==AsciiStriCmp("panelid", Param))
  {
      ...
      // Configure or display the environment variable
      DisplaySetEnvVar(Param, (CHAR16*)&EnvVarList[DISPLAY_VAR_DISPLAYID].sVarName, uValue, 0x0, EnvVarList[DISPLAY_VAR_DISPLAYID].nVarSize, HasValue);
  }
  else
  {
...
static EFI_STATUS DisplaySetEnvVar(CHAR8* pName, const CHAR16 *pVarname, UINT32 uValue, UINT32 uMask, UINTN uSize, UINTN uUpdate)
{
  ...
  if (uUpdate)
  {
    uVariable = (uVariable & uMask) | uValue;

    // Set value    
    if (EFI_SUCCESS == (eStatus = gRT->SetVariable((CHAR16*)pVarname, 
                                                    &gOSAVendorGuid,
                                                    ...
                                                    (void*)&uVariable)))
    {
      ...
      // Flatten value to 1 or 0 if a mask is present 
      if (uMask)
      {
        uVariable = (uValue>0)?1:0;
      }
    
      AsciiPrint("%a variable set to 0x%x\n", pName, uVariable);
    }
    else
    {
      AsciiPrint("%a variable unable to update (%d)\n", pName, eStatus);  
    }
  }
  else
  {
    ...
    if (uMask)
    {
      uVariable = (uVariable>0)?1:0;
    }
  
    if (EFI_SUCCESS == eStatus)
    {
      AsciiPrint("%a variable value is 0x%x\n", pName, uVariable);
    }
    else
    {
      AsciiPrint("%a variable does not exist or unable to read value\n", pName);
    }
  }
