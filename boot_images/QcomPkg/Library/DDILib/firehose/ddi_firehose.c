 ...
 * ----------   ---     ----------------------------------------------
 * 2019-02-25   zhz     DDI framwork re-architecture for Twizy
...
firehose_error_t handleDDIInfo()
{
    ...
    AppendToBuffer(fh.tx_buffer,str, FIREHOSE_TX_BUFFER_SIZE );
	snprintf(str, BUFFER_LENGTH," CommandNum=\"%d\"", itemNum);
    AppendToBuffer(fh.tx_buffer,str, FIREHOSE_TX_BUFFER_SIZE );
...
boolean cmdIsCompatible(ddi_firehose_item_p item)
{
    ...
    int ddr_type_mask = DDR_TYPE_MASK(ddr_get_device_type());
