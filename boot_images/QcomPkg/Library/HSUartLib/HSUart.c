...
--------   --------   ----------------------------------------------------------
05/27/2019  ss        Implemented RX Buffer flush in case of overrun error
04/04/2019  ss        KW Issues Fix
11/30/2018  ss        enablement of HSUart
============================================================================*/
/*==================================================================================================
                                            DESCRIPTION
====================================================================================================
...
GLOBAL FUNCTIONS:
   ...
   hsuart_read
   hsuart_write
   hsuart_rxBreakonDetected
   hsuart_get_max_packet_size
   hsuart_max_raw_data_transfer_size 
   hsuart_flush_rx_buffer
...
#define RX_FIFO_SIZE   256  // size 
#define MAX_PACKET_SIZE 4092 // size of the maximum packet that is supported
#define RFR_LEVEL 232  //size of the RFR level in bytes
...
/* MR1 Register for flow control */
#define UART_DM_MR1_CTS_BIT      0x00000006   // Enable CTS
#define UART_DM_MR1_RTS_BIT      0x00000007   // Enable RTS


/* Command Register */
#define UARTDM_MR1_CTS_CTL       (1<<UART_DM_MR1_CTS_BIT)   // Enable CTS wire
#define UARTDM_MR1_RX_RDY_CTL    (1<<UART_DM_MR1_RTS_BIT)   // Enable RTS wire
#define UARTDM_MR1_RFR_L0_SHFT            0                 // RFR LEVEL 0 SHIFT
#define UARTDM_MR1_RFR_L0_MASK   0x0000003F                 // RFR LEVEL 0 MASK
#define UARTDM_MR1_RFR_L1_SHFT            8                 // RFR LEVEL 1 SHIFT
#define UARTDM_MR1_RFR_L1_MASK   0xFFFFFFC0                 // RFR LEVEL 1 MASK
...
static void configure_clocks(void)
{
   ...
   else if (props->family == BEAR)
   {
      ...
      cfg.tlmm_vote      =  4;
      if(props->platform == 6200)
      {
          cfg.blsp2_ahb_vote =  20;  // 6200 have 2 blsp
          switch (props->uart_instance)
          {
             case 1: cfg.blsp_instance = 1; cfg.uart_offset = 0x6000; break;
             case 2: cfg.blsp_instance = 1; cfg.uart_offset = 0x2038; break;
             case 3: cfg.blsp_instance = 1; cfg.uart_offset = 0x3028; break;
             case 4: cfg.blsp_instance = 1; cfg.uart_offset = 0x4008; break;
			 case 5: cfg.blsp_instance = 2; cfg.uart_offset = 0xC038; break;
			 default: return;
          }
      }
	  else
...
static void reg_init(void)
{
   ...
   if(props->enable_flow_control)
   {
	   rfr = RFR_LEVEL >> 2;
	   MmioWrite32(REG(mr1),UARTDM_MR1_CTS_CTL|
	                        UARTDM_MR1_RX_RDY_CTL|
							(rfr & UARTDM_MR1_RFR_L0_MASK)|
							((rfr & UARTDM_MR1_RFR_L1_MASK)<<UARTDM_MR1_RFR_L1_SHFT));
   }  
   else
   {
   ...
   if(props->enable_loopback)
   {
	   MmioWrite32(REG(mr2),  0x5B4);          // set char format to 8N1 and set RX_BREAK_ZERO_CHAR_OFF and enable loopback
   }  
   else
   {
...
hsuart_initialize(HSUART_CONFIG *cf)
{
   
   props = hsuart_get_properties();
   if (props == NULL) { return RETURN_UNSUPPORTED; }
   props->enable_loopback = cf->enable_loopback;
   props->baud_rate = cf->baud_rate;
   props->enable_flow_control = cf->enable_flow_control;
...
hsuart_poll(void)
{
   if (props == NULL) { props = hsuart_get_properties(); }
...
==================================================================================================*/
UINTN EFIAPI
hsuart_read(OUT UINT8 *user_buffer, IN UINTN bytes_requested, UINT32 *error)
{
   UINT8 *ptr = user_buffer;
   ...
   sr = MmioRead32(REG(sr));
   
   if (sr & UARTDM_SR_OVR_ERR) // checking if overrun error occurs
   {
      *error = HSUART_OVERRUN_ERROR;
	  rx_error = TRUE;
   }

   if (sr & UARTDM_SR_PF_ERR) // checking if parity error occurs
   {
      *error = HSUART_PARITY_FRAMING_ERROR;
	  rx_error = TRUE;
   }
...
hsuart_write(IN UINT8 *user_buffer, IN UINTN bytes_to_send)
{
   UINT32 bytes_sent;
   UINT8* msg_buff;
   UINT32 bytes_remaining = bytes_to_send;
   msg_buff = (UINT8 *) user_buffer;

   if (props == NULL) { props = hsuart_get_properties(); }
   if (props == NULL) { return 0; }
   while (bytes_remaining)
   {
      bytes_sent = transmit_data(msg_buff, (UINT32)bytes_remaining);
	  msg_buff += bytes_sent;
...
FUNCTION: hsuart_rxBreakonDetection

DESCRIPTION: Check Break Detection Condition
            
================================================================================================*/
BOOLEAN EFIAPI
hsuart_rxBreakonDetected(void)
{
...
FUNCTION: hsuart_get_max_packet_size

DESCRIPTION: Returns the size of the maximum packet supported
            
================================================================================================*/
UINTN EFIAPI
hsuart_get_max_packet_size(void)
{
...
FUNCTION: hsuart_max_raw_data_transfer_size

DESCRIPTION: Returns the maximum raw data transfer size
            
================================================================================================*/
UINTN EFIAPI
hsuart_max_raw_data_transfer_size(void)
{
	return (MAX_PACKET_SIZE);
}
...
FUNCTION: hsuart_flush_rx_buffer

DESCRIPTION: Flushes/clears the whole RX buffer in case of overrun error caused
...
hsuart_flush_rx_buffer()
{
   UINT8 temp_buffer[1024];
   UINT32 bytes_requested = 1024;
   UINT32 error = HSUART_SUCCESS;
   while(hsuart_read(temp_buffer,bytes_requested,&error));
}
