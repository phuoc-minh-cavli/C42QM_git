...
sdcc_init_sdhci_mode (UINT32 driveno)
{
   ...
   else
   {
      ...
      HAL_sdhci_FIFOALTDisable(driveno);
   }
                                         ...
                                         UINT32 transfer_size_in_byte)
{
   ...
   {
      nWaitMask = HAL_SDHCI_STATUS_BUF_RD_READY_BMSK;
   }
   else if (SDCC_IS_WRITE_TRANSFER(p_command->flags))
   {
      nWaitMask = HAL_SDHCI_STATUS_BUF_WR_READY_BMSK;
   }
   else
   {
      return SDCC_ERR_UNKNOWN;
                                         ...
                                         UINT16 transfer_size)
{
   ...
   nWaitMask = HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK;
   ...
   {
      nWaitMask |= HAL_SDHCI_STATUS_ADMA_ERR_BMSK;
   }
#else
   nWaitMask = HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK | HAL_SDHCI_STATUS_ADMA_ERR_BMSK;
#endif
   ...
   if (SDCC_NO_ERROR != rc)
   {
      ...
      if (p_device->enable_dma && (nIntStatus & HAL_SDHCI_STATUS_ADMA_ERR_BMSK))
      {
         DEBUG ((EFI_D_ERROR,"SDCC: DATA ADMA ERROR %d\n",p_command->cmd));
         /* ADMA error */
         rc = SDCC_ERR_DMA;
      }
      else if((nIntStatus & HAL_SDHCI_STATUS_DATA_CRC_ERR_BMSK) || 
         (nIntStatus & HAL_SDHCI_STATUS_DATA_TIMEOUT_ERR_BMSK))
      {
   ...
   else if (nIntStatus &  HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK)
   {
      /* Data transfer succeeded. Clear Transfer Complete status */
      HAL_sdhci_ResetInterruptStatus (driveno, HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK);
                              ...
                              UINT32 *pnIntStatus)
{
   ...
   while (count < SDHCI_STATUS_POLL_MAX)
   {
      ...
      if ((status & mask) || (status & HAL_SDHCI_STATUS_ERR_INT_BMSK))
      {
...
SDCC_STATUS sdcc_command_sdhci_mode (sdcc_dev_type *sdcc_pdev, sdcc_cmd_type *cmd)
{
   ...
   rc = sdcc_hc_wait_interrupt_status(driveno, HAL_SDHCI_STATUS_CMD_COMPLETE_BMSK, &nIntStatus);
   if (rc != SDCC_NO_ERROR)
   {
      cmd->status = nIntStatus;
      if (sdcc_pdev->tuning_needed && sdcc_pdev->tuning_done)
      {
         if((cmd->status & HAL_SDHCI_STATUS_CMD_CRC_ERR_BMSK) || 
            (cmd->status & HAL_SDHCI_STATUS_DATA_CRC_ERR_BMSK))
         {
   ...
   HAL_sdhci_ResetInterruptStatus(driveno, HAL_SDHCI_STATUS_CMD_COMPLETE_BMSK);
   ...
   {
      rc = sdcc_hc_wait_interrupt_status(driveno, HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK, &nIntStatus);
      if (rc != SDCC_NO_ERROR)
      {
         cmd->status = nIntStatus;
...
static void sdcc_sdhci_print_error (UINT32 driveno)
{
   ...
   if (status & HAL_SDHCI_STATUS_ERR_INT_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI Error. Specific error below: \n"));
 
   if (status & HAL_SDHCI_STATUS_CMD_TIMEOUT_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_TIMEOUT_ERR \n"));
   else if (status & HAL_SDHCI_STATUS_CMD_CRC_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_CRC_ERR \n"));     
   else if (status & HAL_SDHCI_STATUS_CMD_END_BIT_ERR_BMSK)   
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_END_BIT_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_CMD_INDX_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_INDEX_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_DATA_TIMEOUT_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_DATA_TIMEOUT_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_DATA_CRC_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_DATA_CRC_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_DATA_END_BIT_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CMD_DATA_END_BIT_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_CURRENT_LIMIT_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver CURRENT_LIMIT_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_AUTO_CMD_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver AUTO_CMD_ERR \n")); 
   else if (status & HAL_SDHCI_STATUS_ADMA_ERR_BMSK)
      DEBUG ((EFI_D_INFO, "Storage SDHCI driver ADMA_ERR \n"));
   else if (status & HAL_SDHCI_STATUS_TUNING_ERR_BMSK)
...
)
{
   ...
   *rc = SDCC_NO_ERROR;
   ...
   {    
      if (nIntStatus & HAL_SDHCI_STATUS_TRANS_COMPLETE_BMSK)
      {
         ds = TRANSFER_DONE;
      }
      else
