...
--------   --------   ----------------------------------------------------------
05/14/19   SS         Fixed KW Issues.
04/29/19   SS         Deinitialize UART Port
01/11/19   SS         Enabled 4 wire Uart feature
01/04/18   PR         Added support for QCS6200 vipertooth.
...
#define MAX_PACKET_SIZE 256 // size of the maximum packet that is supported
#define RFR_LEVEL 8  //size of the RFR level
...
/* Command Register */
#define UARTDM_MR1_CTS_CTL       0x00000040   // Enable CTS wire
#define UARTDM_MR1_RX_RDY_CTL    0x00000080   // Enable RTS wire
#define UARTDM_MR1_RFR_L0_SHFT            0   // RFR LEVEL 0 SHIFT
#define UARTDM_MR1_RFR_L0_MASK   0x0000003F   // RFR LEVEL 0 MASK
#define UARTDM_MR1_RFR_L1_SHFT            2   // RFR LEVEL 1 SHIFT
...
static void   deconfigure_clocks(void);
static void   deconfigure_clocks_badger(CLOCK_CONFIG *cfg);
...
FUNCTION: deconfigure_clock

DESCRIPTION:
   This function disables the uart clock.
...
static void deconfigure_clocks(void)
{
   ...
   if (props->family == BEAR)
   {
      cfg.branch_vote_offset = 0x45004;
      cfg.blsp1_ahb_vote = 10;
      cfg.blsp2_ahb_vote =  0;  // BLSP2 vote does not exist on this target
	 
    if(props->platform == 9205)
      {
        switch (props->uart_instance)
      	{
            case 1: cfg.blsp_instance = 1; cfg.uart_offset = 0x2038; break;
...
static void deconfigure_clocks_badger(CLOCK_CONFIG *cfg)
{
   ...
   MmioWrite32(GCC_APCS_CLOCK_BRANCH_ENA_VOTE_ADDR, branch_enable);
   MmioWrite32(GCC_BLSP_UART_APPS_CBCR_ADDR,        0);  // Now disable the branch (and root)
...
static void reg_init(void)
{
   ...
   {
	   UINT32 rfr = RX_FIFO_SIZE - RFR_LEVEL;
...
uart_deinitialize()
{
	...
	while((MmioRead32(REG(sr)) & UARTDM_SR_TXEMT) != 1 )
	{
		busywait(100);
		if(retry_count++ == 50)
		{
