 ...
 * @file:  SpmiBusCfg.c
 * 
 * Copyright (c) 2013-2015, 2019 by Qualcomm Technologies Incorporated. All Rights Reserved.
 * 
 * $DateTime: 2019/07/19 00:38:23 $
 * $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/SPMILib/src/core/hal/bear/SpmiBusCfg.c#2 $
 * $Change: 19912616 $ 
 * 
 *                              Edit History
 * Date     Description
 * -------  -------------------------------------------------------------------
 * 5/10/18  Enable Auto channel Assignment to SPMI configuration.
                                  ...
                                  boolean autoAssignChan, boolean configHW, boolean legacySorted)
{
    ...
    for (i = 0; i < numEntries; i++)
    {
		...
		if(entry->intOwner >= SPMI_NUM_MASTERS(pDev) &&
			entry->intOwner != SPMI_OPEN_OWNER) 
		{
...
{
    return configChannels( SPMI_PDEV(pSpmiInfo,0), entries, numEntries, NULL, NULL, TRUE, TRUE, TRUE );
}
...
Spmi_Result SpmiBusCfg_ConfigurePvcPort(uint8 busId, const SpmiBusCfg_PvcPortCfg* portCfg)
{
    ...
    //Set priority of PVC port               
    SPMI_HWIO_OUT(HWIO_PMIC_ARB_PRIORITIESn_ADDR(base_addr,portCfg->priority),(portCfg->pvcPortId)+1);
