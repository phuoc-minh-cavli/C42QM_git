 ...
 *
 * Copyright (c) 2010-2014,2016 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/EEPROM/flash_extdrv_i2c_v2.c#4 $ $DateTime: 2018/11/08 00:59:30 $ $Author: pwbldsvc $
 *
 *
...
static uint8 eeprom_device_flags = I2C_DEVICE_FLAG_DFLT;
 ...
 */
int flash_i2c_init(uint32 slave_id, uint32 clk, uint32 flags)
{
  ...
  /* Init device specific flags */
  eeprom_device_flags = flags;
...
int flash_i2c_read(struct nor_i2c_xfer_buff_info *i2c_read_buf_info)
{
  ...
  while (byte_cnt > 0)
  {
    ...
    /* send out a command (4 or 6 byte cmd header), requesting 
           a read of total input_cnt bytes */
    if (eeprom_device_flags == I2C_DEVICE_FLAG_CUST_HDR)
    {
      if (FLASH_EXTDRV_I2C_DONE != i2c_qup_config(I2C_INPUT_MODE_BLOCK, 
        I2C_OUTPUT_MODE_BLOCK, input_cnt, 4))
      {
        return FLASH_EXTDRV_I2C_FAIL; 	
      }
    }
    ...
    if (eeprom_device_flags == I2C_DEVICE_FLAG_CUST_HDR)
    {
      *(i2c_reg_addr.qup.QUP_OUTPUT_FIFO) = ((I2C_BYTE_WRITE(addr & 0xFF) << 16) | \
        (I2C_START_WRITE(eeprom_slave_id) | ((addr & 0x300) >> 7)));
    }
    else
    {
