 ...
 *
 * Copyright (c) 2009-2012, 2014, 2019 Qualcomm Technologies, Inc. All rights reserved.
 *===========================================================================*/
 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/Core/flash_nand_core.c#2 $ $DateTime: 2019/06/06 02:14:18 $ $Author: pwbldsvc $
 *
 *
  ...
  struct nand_block_vector *block_vector, uint32 iovec_count)
{
  ...
  while (iovec_count--)
  {
    ...
    while (tmp_block_count < block_vector->block_count)
    {
      ...
      if (NAND_DEVICE_DONE != block_vector->result_vector[tmp_block_count])
      {
        result = NAND_DEVICE_FAIL;
      }
  ...
  struct flash_copy_iovec *copy_op, uint32 iovec_count)
{
  ...
  while(tmp_iovec_cnt < iovec_count)
  {
    ...
    while ((copy_count) && (NAND_DEVICE_DONE == result))
    {
      if (!copy_sync_state)
      {
        DALSYS_SyncEnter(client_ctxt->flash_dev_ctxt->handle_dev_sync);
      ...
      if (NAND_DEVICE_DONE == nand_read_mpages(client_ctxt, &page_data))
      {
      ...
      if ((NAND_DEVICE_DONE == result) && (copy_count))
      {
        if (multi_count == 1)
        {
          DALSYS_SyncLeave(client_ctxt->flash_dev_ctxt->handle_dev_sync);
