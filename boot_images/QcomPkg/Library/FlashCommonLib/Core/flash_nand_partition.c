 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/Core/flash_nand_partition.c#1 $ $DateTime: 2018/08/01 23:51:05 $ $Author: pwbldsvc $
 *
 *
...
int smem_retrieve_mibib(flash_client_ctxt *client_ctxt)
{
  ...
  {
    if (NAND_DEVICE_DONE != flash_malloc
      (client_ctxt->flash_dev_ctxt->dev_info.page_size_bytes,
  ...
  flash_memcpy((void *)page_buf, (void *)smem_aarm_partition_table,
    sizeof(struct flash_partition_table));
      ...
      (parti_ptr->version == FLASH_PARTITION_VERSION) )
  {
    ...
    flash_memcpy(client_ctxt->flash_dev_ctxt->parti_ptr,
      page_buf, client_ctxt->flash_dev_ctxt->dev_info.page_size_bytes);
...
int nand_retrieve_mibib(flash_client_ctxt *client_ctxt)
{
  ...
  int cur_block, age = 0;//<TODOS> add mibib related fix here
  int result = NAND_DEVICE_DONE;
  ...
  if (client_ctxt->flash_dev_ctxt->parti_ptr == NULL)
  {
    if (NAND_DEVICE_DONE != flash_malloc(FLASH_PARTITION_TABLE_SIZE,
      &(client_ctxt->flash_dev_ctxt->parti_ptr)))
    {
      FLASHLOG(3, ("\nMalloc failed\n"));
      return NAND_DEVICE_FAIL;
    ...
    if (age != -1)
    {
      ...
      if (NAND_DEVICE_DONE == result)
      {
          ...
          (parti_ptr->version == FLASH_PARTITION_VERSION) )
        {
          ...
          flash_memcpy(client_ctxt->flash_dev_ctxt->parti_ptr, 
            (void *)page_buf, FLASH_PARTITION_TABLE_SIZE);
