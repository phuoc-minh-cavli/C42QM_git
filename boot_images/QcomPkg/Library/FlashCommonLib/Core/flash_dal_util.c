 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/Core/flash_dal_util.c#1 $ $DateTime: 2018/08/01 23:51:05 $ $Author: pwbldsvc $
 *
 *
...
struct vtop_item
{
  uintnt vstart;   /* Start of the virtual addr */
  uint32 size;     /* Virtual address range     */
  uintnt pstart;   /* Start of the physical addr */
};
 ...
 */
void flash_vtop_add_item(uintnt vaddr, uintnt phys_addr, uint32 size)
{
 ...
 */
uintnt flash_vtop_get_physical(uintnt virt_addr)
{
  uint32 item_count = 0;
  uintnt phy_address = 0;
  struct vtop_item *vitem;
