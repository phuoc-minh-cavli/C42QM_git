 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/Core/flash_nand_init.c#1 $ $DateTime: 2018/08/01 23:51:05 $ $Author: pwbldsvc $
 *
 *
...
static int nand_mem_init(flash_client_ctxt *client_ctxt)
{
  ...
  if (TRUE == nand_entry_init_data.nand_mem_initialized)
  {
    ...
    hal_ws_buffer = (uint32) meminfo->hal_mem_info.virtual_addr;
    ...
    nand_pdata->raw_page_buffer =
      (uint8 *) meminfo->drv_mem_info.virtual_addr;

    nand_pdata->drv_ws_buffer =
      (uint32 *) (meminfo->drv_mem_info.virtual_addr +
  ...
  uint32 spare_size)
{
  ...
  flash_hal_get_predef_mem_usage (hal_id,
    nand_entry_init_data.multi_rw_support_count, (uint32*)&hal_mem_size);
...
int nand_predef_init(flash_client_ctxt *client_ctxt)
{
  ...
  predef_init_data.buff_data.buff_vaddr = hal_mem_info->virtual_addr;
  predef_init_data.buff_data.buff_paddr = hal_mem_info->physical_addr;
  predef_init_data.buff_data.buff_size = hal_mem_info->size;

  predef_init_data.raw_buff_data.buff_vaddr = drv_mem_info->virtual_addr;
  predef_init_data.raw_buff_data.buff_paddr = drv_mem_info->physical_addr;
  predef_init_data.raw_buff_data.buff_size = drv_mem_info->size;
  
  flash_hal_predef_init(client_ctxt->client_data.hal_id,
    &predef_init_data, (uint32*)&used_buff_size, &hal_status);
  ...
  struct flash_nand_params *device)
{
  ...
  nand_pdata->raw_page_buffer =
    (uint8 *) flash_mem_data->drv_mem_info.virtual_addr;

  nand_pdata->drv_ws_buffer =
    (uint32 *) (flash_mem_data->drv_mem_info.virtual_addr +
    dev_ctxt->dev_info.total_page_size_bytes);
   ...
   uint8** ids)
{
  ...
  flash_hal_dev_init(&nand_cfg, (uint32*)&hal_id, &hal_status);
