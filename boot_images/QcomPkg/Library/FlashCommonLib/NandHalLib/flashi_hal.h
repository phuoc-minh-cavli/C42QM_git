 ...
 *
 * Copyright (c) 2008-2012,2017-2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
...
struct flash_hal_ctlr_ops
{
  ...
  /* start reading a single page */
  void (*flash_hal_start_page_read) (flash_hal_dev_info *dev_info, struct
    flash_hal_predef_page_data *page_data, struct flash_hal_predef_op_list
    **op_list, enum flash_hal_status *status);

  /* wait for previously started page read request to complete */
  void (*flash_hal_wait_for_page_read) (flash_hal_dev_info *dev_info,
    enum flash_hal_status *status);
