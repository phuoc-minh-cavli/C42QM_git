 ...
 *
 * Copyright (c) 2008-2012, 2015, 2016, 2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
...
static uint32 flash_hal_read_queued_page_start;
static uint32 flash_hal_read_queued_page_total;
...
void flash_hal_dev_deinit(uint32 dev_id)
{
  ...
  {
    if(flash_hal_devices[dev_id].ctlr_ops.flash_hal_close)
    {
...
{
  flash_hal_devices[dev_id].config.spi_num_addr_cycles = spi_num_addr_cycles;
}
...
/*
 * Start flash HAL page read non-blocking
 */
  ...
  *status)
{
  ...
  dev_info->ctlr_ops.flash_hal_start_page_read(dev_info,
    page_data, op_list, status);

  if (FLASH_HAL_SUCCESS == *status)
  {
    flash_hal_read_queued_page_start = page_data->start_page;
    flash_hal_read_queued_page_total = page_data->total_pages;
  }
}

/*
 * Wait for previously started flash HAL read to complete
 */
void flash_hal_wait_for_page_read(uint32 dev_id,
                                    enum flash_hal_status *status)
{
  flash_hal_dev_info  *dev_info;
  struct flash_hal_predef_page_data raw_page_data;

  if(!flash_hal_read_queued_page_total)
  {
    *status = hal_status = FLASH_HAL_FAIL;
  }
  else
  {
    hal_status = FLASH_HAL_NO_ERROR;
    ...
    dev_info->ctlr_ops.flash_hal_wait_for_page_read(dev_info, status);
    ...
    if (FLASH_HAL_CHECK_ECC == *status)
    {
      ...
      raw_page_data.main_size_bytes = 
        dev_info->config.total_page_size;
      raw_page_data.spare_size_bytes = 0;
      raw_page_data.start_page = flash_hal_read_queued_page_start;
      raw_page_data.total_pages = flash_hal_read_queued_page_total;

      dev_info->ctlr_ops.flash_hal_read_page(dev_info,
        &raw_page_data, NULL, status);
  ...
  flash_hal_read_queued_page_total=0;
}
