 ...
 * MSMs.
 *
 * Copyright (c) 2008-2012,2015,2017-2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *  --------     ---     ---------------------------------------------------
 * 12/05/19      sl      New error type to indicate FLASH dev status errors.
 * 06/11/19      sl      Added support for pagescope/multipage.
 * 04/03/19      sl      Add non-blocking page read support
 * 11/27/18      sl      added support to configure SPI_NUM_ADDRESS_CYCLES
 * 10/30/18      sl      Added device ID for SPI NAND
...
enum flash_hal_status
{
  ...
  FLASH_HAL_OP_NOT_DONE,               /**< Error applicable only for
                                            write/erase ops where NAND
                                            didn't accept write/erase
                                            request. This error does not mean
                                            that op failed, it means that FLASH
                                            rejected the command right now.
                                            The op can be retried but,
                                            block should not be marked as bad
                                            for this error. */
...
enum flash_hal_dev_type
{
  ...
  FLASH_HAL_DEVICE_NAND_SPI,  /**< SPI NAND device */
...
/**
 *
 * Sets the number of address cycles for the SPI NAND device in flash HAL.
 *
 * @param dev_id [IN]
 *   -- Logical device ID, as described in \ref flash_hal_dev_id
 *
 * @param spi_num_addr_cycles [IN]
 *   -- refer below bit fields to fill correct val
      Bits        Field Name
      20:18       ERASE_ADDR_CYCLES
      17:15       PROG_PAGE_CYCLES
      14:12       PROG_DATA_X4_CYCLES
      11:9        PROG_DATA_X1_CYCLES
      8:6         READ_PAGE_CYCLES
      5:3         READ_DATA_X4_CYCLES
      2:0         READ_DATA_X1_CYCLES
 *
 */
void flash_hal_set_spi_num_addr_cycles(uint32 dev_id,
                                       uint32 spi_num_addr_cycles);
 ...
 */
uintnt flash_hal_get_cs_base_addr(uint32 dev_id, boolean addr_is_phys);
...
{
   uintnt reg_addr;           /**< Wait Mask Register Address */
 ...
 *
 */
void flash_hal_read_dev_buffer(uint32 dev_id, uintnt src_addr,
  uint32 data_len_bytes, uint8 *buffer);
 ...
 *
 */
void flash_hal_write_dev_buffer(uint32 dev_id, uintnt dst_addr,
...
#ifdef LOADER_CODE
  FLASH_HAL_EF_EE_FEATURE, 		  /**<	Set (EFh), Get (EEh) Feature cmd */
#endif
...
{
  uintnt buff_vaddr;  /**< Virtual address of the buffer */
  uintnt buff_paddr;  /**< Physical address of the buffer */
...
{
  uintnt start_addr;       /**< Starting address */
...
{
#ifdef LOADER_CODE
  uint16 xfer_reg_count;         /**< Number of transfer step registers */
  uint16 xfer_boost_mode;        /**< Enable boost mode in QPIC Nand ctrl */
#else
...
struct flash_hal_predef_feat_data
{
  ...
  uint32 feat_addr;                      /**< Feature address */
  uint32 feat_buff_addr;                 /**< Feature data buffer */
};
#endif
...
/**
 *
 * Start the Read of a flash page with predefined command sequence
 * but don't wait for it to complete. For non-DMA, this
 * this function is not supported.
 * NOTE: flash_hal_wait_for_page_read should be used to check
 * status of the started page read op
 *
 * @param dev_id [IN]
 *   -- Logical device ID, as described in \ref flash_hal_dev_id
 *
 ...
 *      FLASH_HAL_FAIL - On fail \n
 */
void flash_hal_start_page_read(uint32 dev_id, struct flash_hal_predef_page_data
  *page_data, struct flash_hal_predef_op_list **op_list, enum flash_hal_status
  *status);

/**
 *
 * Wait for previously started page reads to complete
 * Not supported for non-DMA mode.
 * NOTE: should only be used after flash_hal_start_page_read()
 *
 * @param dev_id [IN]
 *   -- Logical device ID, as described in \ref flash_hal_dev_id
 *
 ...
 *                              in non-DM mode
 */
void flash_hal_wait_for_page_read(uint32 dev_id, enum flash_hal_status *status);
