 ...
 * ----------   ---     ----------------------------------------------
 * 12/05/19     sl      updated device errors returned by SPI NAND
 * 02/08/19     sl      Enable SPI NAND quad speed support
 * 10/30/18     svl     Added SPI NAND support
...
#define FLASH_SPI_NAND_READ_STATUS_CLEAR      0x0
...
#define FLASH_NAND_CTRL_RD_DELAY_CNTR_VAL_SHFT \
          HWIO_NAND_CTRL_NANDC_READ_DELAY_COUNTER_VAL_SHFT
#endif
...
#define HAL_QPIC_NANDC_FLASH_DEV_CMD3_PADDR           HWIO_PHYS(FLASH_DEV_CMD3)
#define HAL_QPIC_NANDC_FLASH_DEV_CMD7_PADDR           HWIO_PHYS(FLASH_DEV_CMD7)
#define HAL_QPIC_NANDC_SPI_NUM_ADDR_CYCLES_PADDR      HWIO_PHYS(SPI_NUM_ADDR_CYCLES)
#define HAL_QPIC_NANDC_SPI_BUSY_CHECK_WAIT_CNT_PADDR  HWIO_PHYS(SPI_BUSY_CHECK_WAIT_CNT)
#define HAL_QPIC_NANDC_NAND_FLASH_FEATURES_PADDR      HWIO_PHYS(NAND_FLASH_FEATURES)
#define HAL_QPIC_NANDC_NAND_FLASH_SPI_CFG_PADDR       HWIO_PHYS(NAND_FLASH_SPI_CFG)
#define HAL_QPIC_NANDC_NAND_AUTO_STATUS_EN_PADDR      HWIO_PHYS(NAND_AUTO_STATUS_EN)
...
/*
    WINBOND SPI NAND Status Register 3 (0xC) has below bits:
    Bit0: BUSY
    Bit1: WEL
    Bit2: E-FAIL : If Program FAIL
    Bit3: P-FAIL : If erase FAIL
    ECC-0: Default 0, as we disable ECC in device this should be 0 always
    ECC-1: Default 0, as we disable ECC in device this should be 0 always
    LUT-FAIL: Default value 0, 1 if all 20 BB links are used
    (R): Reserved

    For Write Fail Status register will be : 1000b : 0x8
    For Erase Fail Status register will be : 0100b : 0x4
    For WEL set    Status register will be : 0010b : 0x2
*/
#define FLASH_SPI_NAND_DEV_STATUS_PFAIL_MASK (0x8 << HWIO_NAND_FLASH_STATUS_DEV_STATUS_SHFT)
#define FLASH_SPI_NAND_DEV_STATUS_EFAIL_MASK (0x4 << HWIO_NAND_FLASH_STATUS_DEV_STATUS_SHFT)
#define FLASH_SPI_NAND_DEV_STATUS_WEL_MASK   (0x2 << HWIO_NAND_FLASH_STATUS_DEV_STATUS_SHFT)

/*
 According to Winbond data sheet Protection Register/Status Register-1 is:
   __________________________________________________
  |       |     |     |     |     |    |      |      |
  | SRP-0 | BP3 | BP2 | BP1 | BP0 | TB | WP-E | SRP1 |
  |_______|_____|_____|_____|_____|____|______|______|

with SRP-0=0, SRP-1=0, WP-E=0 : there will be no /WP functionality
/WP pin will always function as IO2. This will be required for Quad mode.

Disable all Block protection by setting BP0-BP3 and TB to 0
*/
#define FLASH_SPI_NAND_SR1_ADDR        (0xA0) 
#define FLASH_SPI_NAND_SR1_DATA        (0x00) 


/*
 According to Winbond data sheet Configuration Register/Status Register-2 is:
 _______________________________________________________
|       |       |       |       |     |     |     |     |
| OTP-L | OTP-E | SR1-L | ECC-E | BUF | (R) | (R) | (R) |
|_______|_______|_______|_______|_____|_____|_____|_____|

According to WinBond data sheet :
  with BUF=1 and ECC-E=0 Data Output=2048+64
  with BUF=0 and ECC-E=0 Data Output=2048
  Also we don't need any OTP configuration.
So below are the values to be set in SR-2

OTP-L : set to 0 i.e. OTP pages are not locked.
OTP-E : as we are not entering OTP mode keep value to 0.
SR1-L : set to 0 so that values in protection register are not OTP locked.
ECC-E : set to 0 to disable In-built ECC
BUF   : set to 1 for Buffer Read Mode

*/
#define FLASH_SPI_NAND_SR2_ADDR       (0xB0)
#define FLASH_SPI_NAND_SR2_DATA       (0x8)
 ...
 ***************************************************************/
/* value for below register according to HPG : 0X005400D0 */
#define SPI_NAND_DEVn_CFG0_BASE_RESET                                         \
       (NAND_OPT_OFF << HWIO_SHFT(NAND_DEV0_CFG0, SET_RD_MODE_AFTER_STATUS) | \
   ...
   NAND_UD_BYTE_SIZE << HWIO_SHFT(NAND_DEV0_CFG0, UD_SIZE_BYTES)            | \
         ...
         NAND_OPT_ON << HWIO_SHFT(NAND_DEV0_CFG0, DISABLE_STATUS_AFTER_WRITE))

/* value for below register according to HPG : 0X87476B1 */
#define SPI_NAND_DEVn_CFG1_BASE_RESET                                         \
         (NAND_OPT_ON << HWIO_SHFT(NAND_DEVn_CFG1, ENABLE_BCH_ECC)          | \
                   58 << HWIO_SHFT(NAND_DEVn_CFG1, WR_RD_BSY_GAP)           | \
                    0 << HWIO_SHFT(NAND_DEVn_CFG1, BAD_BLOCK_IN_SPARE_AREA) | \
                  474 << HWIO_SHFT(NAND_DEVn_CFG1, BAD_BLOCK_BYTE_NUM)      | \
          NAND_OPT_ON << HWIO_SHFT(NAND_DEVn_CFG1, CS_ACTIVE_BSY)           | \
                    4 << HWIO_SHFT(NAND_DEVn_CFG1, NAND_RECOVERY_CYCLES)    | \
         NAND_OPT_OFF << HWIO_SHFT(NAND_DEVn_CFG1, WIDE_FLASH)              | \
          NAND_OPT_ON << HWIO_SHFT(NAND_DEVn_CFG1, ECC_DISABLE))

/* value for below register according to HPG : 0X02000001 */
#define SPI_NAND_DEVn_ECC_CFG_BASE_RESET                                      \
       (NAND_OPT_OFF << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_FORCE_CLK_OPEN)    | \
   NAND_UD_BYTE_SIZE << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_NUM_DATA_BYTES)    | \
                 0x0 << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_PARITY_SIZE_BYTES) | \
                 0x0 << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_MODE)              | \
         NAND_OPT_ON << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_DISABLE))

#define FLASH_CMD_ACC_MODE_SHFT      HWIO_SHFT(NAND_FLASH_CMD, CMD_ACCESS_MODE)

/*
  SPI TRANSFER MODE:
  for x1 mode: 0x1
  for x4 mode: 0x3
  x2 mode not supported currently in QPIC v2.0.1
*/
#define NAND_SPI_XFER_MODE_X1   (0x1)
#define NAND_SPI_XFER_MODE_X4   (0x3)

#define SPI_MODE_ENABLE_FOR_NON_RW_CMD                                        \
        (NAND_SPI_XFER_MODE_X1<<HWIO_SHFT(NAND_FLASH_CMD,SPI_TRANSFER_MODE)|  \
         NAND_OPT_ON << HWIO_SHFT(NAND_FLASH_CMD, SPI_WP)                  |  \
         NAND_OPT_ON << HWIO_SHFT(NAND_FLASH_CMD, SPI_HOLD))

#define SPI_MODE_ENABLE_FOR_RW_CMD                                            \
        (NAND_SPI_XFER_MODE_X4<<HWIO_SHFT(NAND_FLASH_CMD,SPI_TRANSFER_MODE)|  \
         NAND_OPT_ON << HWIO_SHFT(NAND_FLASH_CMD, SPI_WP)            |        \
         NAND_OPT_ON << HWIO_SHFT(NAND_FLASH_CMD, SPI_HOLD))

/* value for below register according to HPG : 0X1A5408D0 */
#define SPI_NAND_DEVn_CFG0_BASE                                               \
      (NAND_OPT_OFF << HWIO_SHFT(NAND_DEV0_CFG0, SET_RD_MODE_AFTER_STATUS)  | \
                  0 << HWIO_SHFT(NAND_DEV0_CFG0, NUM_ADDR_CYCLES)           | \
                  0 << HWIO_SHFT(NAND_DEV0_CFG0, SPARE_SIZE_BYTES)          | \
      NAND_ECC_SIZE << HWIO_SHFT(NAND_DEV0_CFG0, ECC_PARITY_SIZE_BYTES)     | \
...
/* value for below register according to HPG : 0X08287440 */
#define SPI_NAND_DEVn_CFG1_BASE                                               \
                   (20 << HWIO_SHFT(NAND_DEVn_CFG1, WR_RD_BSY_GAP)          | \
                     0 << HWIO_SHFT(NAND_DEVn_CFG1, BAD_BLOCK_IN_SPARE_AREA)| \
                     0 << HWIO_SHFT(NAND_DEVn_CFG1, BAD_BLOCK_BYTE_NUM)     | \
          NAND_OPT_OFF << HWIO_SHFT(NAND_DEVn_CFG1, CS_ACTIVE_BSY)          | \
                     0 << HWIO_SHFT(NAND_DEVn_CFG1, NAND_RECOVERY_CYCLES)   | \
          NAND_OPT_OFF << HWIO_SHFT(NAND_DEVn_CFG1, WIDE_FLASH)             | \
 ...
 */
#define SPI_NAND_DEVn_ECC_CFG_BASE                                            \
        (NAND_OPT_ON << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_FORCE_CLK_OPEN)    | \
               0x200 << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_NUM_DATA_BYTES)    | \
                 0x7 << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_PARITY_SIZE_BYTES) | \
                 0x0 << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_MODE)              | \
        NAND_OPT_OFF << HWIO_SHFT(NAND_DEV0_ECC_CFG, ECC_DISABLE))

/* value for below register according to HPG : 0X0000000D */
#define SPI_NAND_FLASH_DEV_CMD_VLD_BASE                                       \
          (NAND_OPT_OFF << HWIO_SHFT(FLASH_DEV_CMD_VLD, READ_STOP_VLD)      | \
           ...
           NAND_OPT_OFF << HWIO_SHFT(FLASH_DEV_CMD_VLD, SEQ_READ_START_VLD) | \
...
#define FLASH_NAND_ACC_FEATURE_CMD         0xE
