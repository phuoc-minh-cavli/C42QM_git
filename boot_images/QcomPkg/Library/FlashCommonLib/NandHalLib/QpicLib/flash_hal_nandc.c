 ...
 *
 * Copyright 2012-2016, 2019 Qualcomm Technologies, Inc. All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/NandHalLib/QpicLib/flash_hal_nandc.c#2 $ $DateTime: 2019/04/23 17:39:55 $ $Author: pwbldsvc $
 *
 *
...
static void hal_nandc_check_erased_page_dm(struct flash_hal_ctlr_data *priv_data,
  struct flash_hal_nand_buffer *spare_data, uintnt nandc_buffer,
...
static __inline void hal_nandc_check_erased_page(
  struct flash_hal_ctlr_data *priv_data,
  struct flash_hal_nand_buffer *data, uintnt nandc_buffer,
  enum flash_hal_status *status)
{
...
static void hal_nandc_set_addr(struct flash_hal_ctlr_data *priv_data,
  uintnt num_addr, uint8 *addr)
{
  uint32 i = 0;
  uintnt addr_val = 0, addr_mask = 0;
  ...
  uint32 mask, uint32 bail_out_time)
{
  if (FLASH_HAL_DMA_ENABLE == hal_dma_state)
  {
   ...
   while (status & 0x1F)
   {
    flash_hal_get_next_ws_addr((uint32*)&status, 1);
   }
  ...
  enum flash_hal_status *status)
{
  ...
  *status = tmp_status;
  ...
  nandc_buffer = (FLASH_HAL_DMA_ENABLE == hal_dma_state)?
    HAL_QPIC_NANDC_NANDC_BUFFER_PADDR : ((uintnt)HAL_QPIC_NANDC_NANDC_BUFFER_VADDR);
  ...
  enum flash_hal_status *status)
{
  ...
  */
  ...
  do
  {
    uint32 bytes_copied = 0, align_len = 0;
    uintnt temp_write_addr = 0;
    ...
    if (data->buf_len)
    {
      ...
      if (align_len < data->buf_len)
      {
        ...
        flash_hal_get_next_ws_addr((uint32*)&temp_write_addr, 1);
