 ...
 * 10/20/18     sa      Added support for pagescope/multipage.
...
volatile bam_result_type bam_poll_result;
void *desc_buf_handle = NULL;
uint32 flash_bam_poll_cnt = FLASH_BAM_MAX_POLL_COUNT;


void flash_bam_pipe_isr_cb(bam_result_type bam_result)
{
    if((uint8)NAND_BAM_CMD_PIPE == (uint8)bam_result.cb_data)
    {
      bam_poll_result = bam_result;
    }
}
  ...
  bam_pipe_config_type *bam_pipe_cfg)
{
  ...
  pipe_cb.func = flash_bam_pipe_isr_cb;
  pipe_cb.data = (void*)pipe_num;
  /* Initialize the BAM PIPE */  
  bam_pipe_handle = 
    bam_pipe_init(flash_bam_handle, pipe_num, bam_pipe_cfg, &pipe_cb);
  if(bam_pipe_handle == NULL)
  {
    return NULL;
  }

  /* Enable IRQ Mode for CMD pipe */
  bam_status = bam_pipe_setirqmode(bam_pipe_handle, (NAND_BAM_CMD_PIPE==pipe_num)?1:0, bam_pipe_cfg->options);
...
{
  uint32 result = FLASH_BAM_SUCCESS;
...
uint32 flash_bam_queue_desc(bam_handle bam_pipe_handle,
...
uint32 flash_bam_pipe_check_intr(bam_handle pipe_handle, void* user_data)
{
  uint32 result = FLASH_BAM_SUCCESS;
  ...
  // while(1)
  {
    if ((bam_poll_result.event == BAM_EVENT_ERROR)
  ...
  memset( (void*)&bam_poll_result, 0, sizeof(bam_result_type));
  return result;
}

/* Poll the pipe for transfer completion */
uint32 flash_bam_poll_pipe(bam_handle pipe_handle, void* user_data)
{
  bam_status_type bam_status;
  uint32 result = FLASH_BAM_SUCCESS;
  ...
  while(--flash_bam_poll_cnt)
  {      
    bam_status = bam_pipe_poll(pipe_handle, (bam_result_type *)&bam_poll_result);
  ...
  struct flash_bam_xfer_desc_list *status_desc_list)
{
  ...
  result =  flash_bam_pipe_check_intr(flash_bam_cmd_handle,
...
/*  Start the Transfer of command/data described by
    the descriptors to NAND using BAM */
...
uint32 flash_bam_pipe_deinit(bam_handle bam_pipe_handle)
{
  uint32 result = FLASH_BAM_SUCCESS;
...
{
  uint32 ret_status = FLASH_BAM_SUCCESS;
