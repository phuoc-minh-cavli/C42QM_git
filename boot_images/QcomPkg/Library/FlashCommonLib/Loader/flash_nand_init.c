 ...
 *
 * Copyright (c) 2009-2015, 2017-2019 QUALCOMM Technologies, Inc.
 * All Rights Reserved.
 ...
 * 03/23/19     sl      Move MIBIB search from nand probe to flash open.
 * 10/27/18     sl      added SPI NAND init support
...
int nand_close(flash_client_ctxt *client_ctxt)
{
  ...
  if (client_ctxt->client_data.client_specific_data)
  {
    ...
    if (client_data_handle->nand_phys_handle)
    {
    ...
    if (device->dev_xfer_steps_cfg != NAND_MAX_XFER_CFG)
    {
      ...
      if (xfer_steps)
      {
        ...
        if ( (timing_mode >= 4) && (FLASH_XFER_BOOST_MODE_DISABLED == FALSE) )
        {
          ...
          cfg_data.xfer_reg_data = (uint32 *)&xfer_steps->xfer_data[FLASH_MAX_TIMING_MODES - 1][0];
        }
        else
        {
          cfg_data.xfer_boost_mode = 0;
          cfg_data.xfer_reg_data = (uint32 *)&xfer_steps->xfer_data[timing_mode][0];
        }

        /*
          QPIC_SPI_NUM_ADDR_CYCLES is different for different SPI NAND devices
          It should be set as part of initialialization sequence.
          according to HPG Xfer steps and QPIC_SPI_NUM_ADDR_CYCLES should be set
          before read ID, but that's not possible as these settings change based
          on device ID. So clubbing QPIC_SPI_NUM_ADDR_CYCLES and Xfer steps settings
          so that its done immidiately after read ID */
        flash_hal_set_spi_num_addr_cycles(client_ctxt->client_data.hal_id, device->spi_num_addr_cycles);
