 ...
 *
 * Copyright (c) 2009-2012,2014,2017, 2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 12/05/19     sl      Handle HAL_FAIL and other errors separately.
 * 07/23/19     sl      enabled pagescope; avoid ps-mp call for non bam mode.
 * 06/15/19     sl      Added support for pagescope/multipage.
  ...
  struct flash_hal_predef_page_data *page_data)
{
  ...
  static uint8 ps_mp_en = 1; /* assume ps/mp is supported by default*/
  ...
  if(ps_mp_en && (nand_entry_init_data.dma_state != FLASH_HAL_DMA_ENABLE))
  {
    /*
      pagescope/multipage is a bam feature, so disabled it for non-bam mode
    */
    ps_mp_en = 0;
  }

  while ((page_count) && ((FLASH_DEVICE_DONE == result) || (FLASH_DEVICE_FAIL_PAGE_ERASED == result)))
  {
    if (!read_sync_state)
    {
    ...
    /* check if pagescope_multipage is enabled or not */
    if(ps_mp_en)
    {
      /* if enabled then go for multipage read api to read all pages*/
      page_data->total_pages = page_read_count;
      flash_hal_read_mpages(hal_id, page_data, &op_list, &hal_status);
      if(FLASH_HAL_MULTIPAGE_NOT_SUPPORTED == hal_status)
      {
        /*
          looks like pagescope multipage is not enabled
          so start over with normal page read api.
        */
        ps_mp_en = 0;
    ...
    {
      /* go for normal page by page read */
      page_read_count = 1;
      page_data->total_pages = page_read_count;
  ...
  struct flash_hal_predef_page_data *page_data)
{
  ...
  while ((page_count) && (FLASH_DEVICE_DONE == result))
  {
    ...
    if (FLASH_HAL_SUCCESS != hal_status)
    {
      ...
      {
        DALSYS_LogEvent(0, DALSYS_LOGEVENT_FATAL_ERROR, "Flash: hw error.!");
        result = FLASH_DEVICE_OP_NOT_DONE;
      }
  ...
  struct flash_block_vector *block_vector, uint32 iovec_count)
{
  ...
  while (iovec_count--)
  {
    ...
    while (tmp_block_count < block_vector->block_count)
    {
      ...
      {
        if (FLASH_DEVICE_FAIL == block_vector->result_vector[tmp_block_count])
        {
          result = FLASH_DEVICE_FAIL;
        }
        else
...
/*
 * start reading a page read but don't wait for the read to complete
 */
int nand_start_page_read(flash_client_ctxt *client_ctxt,
  enum page_read_opcode read_type, dalsys_mem_desc_list *buffer_desc,
  struct flash_page_iovec *page_data_iovec)
{
  int    result = FLASH_DEVICE_DONE;
  ...
  /* Get the buffer descriptor ptr */
  buf_desc = (struct flash_hal_buffer_desc*)DALFW_MemDescBufPtr(buffer_desc, 0);
  ...
  flash_hal_start_page_read(client_ctxt->client_data.hal_id,
                            &page_data, &op_list, &hal_status);
...
/*
 * wait for previously started page read to complete
 */
int nand_wait_for_page_read(flash_client_ctxt *client_ctxt)
{
  int    result = FLASH_DEVICE_DONE;
  enum   flash_hal_status hal_status = FLASH_HAL_FAIL;

  FLASH_CORE_PROFILE_START(FLASH_PROFILE_HAL_TRANSFER, 1);

  flash_hal_wait_for_page_read(client_ctxt->client_data.hal_id, &hal_status);
...
flash_vtbl* nand_get_base_tbl_default(void)
{
  ...
  nand_base_vtbl.start_page_read = nand_start_page_read;
  nand_base_vtbl.wait_for_page_read = nand_wait_for_page_read;
