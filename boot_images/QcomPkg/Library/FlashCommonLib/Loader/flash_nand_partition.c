 ...
 *
 * Copyright (c) 2008-2013,2017, 2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * 08/14/17     eo      Set min search mibib start block to 8 
  ...
  const unsigned char *parti_name)
{
  ...
  if (entry_ptr != (flash_partentry_t)0)
  {
    ...
    if(partition_name_len > (strlen((const char*)parti_name)+1))
    {
        partition_name_len = (strlen((const char*)parti_name)+1);
    }
    strlcpy(client_ctxt->client_data.partition_name,
            (const char*)parti_name, partition_name_len);
...
int nand_retrieve_mibib(flash_client_ctxt *client_ctxt)
{
  ...
  /* Start looking for first MIBIB at block 8 (= MIBIB_BLOCK_SEARCH_MIN) */
  ...
  const unsigned char *parti_name)
{
  ...
  if (strncmp((const char *)parti_name, "all", 6) == 0)
  {
    client_ctxt->client_data.partition_block_start   = 0;
    client_ctxt->client_data.partition_block_limit   =
      client_ctxt->flash_dev_ctxt->dev_info.u.nand.block_count;
  ...
  else
  {
    ...
    if (client_ctxt->flash_dev_ctxt->parti_ptr == NULL)
    {
      if ((nand_retrieve_mibib(client_ctxt)) != FLASH_DEVICE_DONE)
	  {
        return FLASH_DEVICE_FAIL;
	  }	  
    }
