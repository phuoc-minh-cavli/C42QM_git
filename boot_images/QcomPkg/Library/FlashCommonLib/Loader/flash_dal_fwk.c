 ...
 *
 * Copyright (c) 2008-2013,2016, 2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   Data structures
 ***********************************************************************/
#define FLASH_FWK_MAX_DEV_ID_ENTRIES     8

struct flash_device_id_tbl 
{
  uint32 dev_id;
  uint32 dal_dev_id;
  char *str_dev_id;
...
struct flash_device_id_tbl flash_str_dev_id[FLASH_FWK_MAX_DEV_ID_ENTRIES] = 
{
  {FLASH_DEVICEID_1, DALDEVICEID_FLASH_DEVICE_1, ""},
  {FLASH_DEVICEID_2, DALDEVICEID_FLASH_DEVICE_2, ""},
  {FLASH_DEVICEID_3, DALDEVICEID_FLASH_DEVICE_3, ""},
  {FLASH_DEVICEID_4, 0, "DALDEVICEID_FLASH_DEVICE_4"},
  {FLASH_DEVICEID_5, 0, "DALDEVICEID_FLASH_DEVICE_5"},
  {FLASH_DEVICEID_6, 0, "DALDEVICEID_FLASH_DEVICE_6"},
  {FLASH_DEVICEID_7, 0, "DALDEVICEID_FLASH_DEVICE_7"},
  {FLASH_DEVICEID_8, 0, "DALDEVICEID_FLASH_DEVICE_8"}
};
  ...
  DalDeviceHandle **flash_dal_handle)
{
  ...
  if ((DAL_SUCCESS != err) || (NULL == flash_dal_handle))
  {
    return DAL_ERROR;
  }
  ...
  {
    err = DALFW_AttachToStringDevice(pszArg,
      (DALDrvCtxt *)&drv_ctxt,(DALClientCtxt *)client_ctxt);
    if (DAL_SUCCESS != err)
    {
      goto error;
    }
    str = client_ctxt->flash_dev_ctxt->str_dev_id;
    for(idx = 0; idx < FLASH_FWK_MAX_DEV_ID_ENTRIES; idx++)
    {
      // if (memcmp(flash_str_dev_id[idx].str_dev_id, str, sizeof(str)) == 0)
      if (memcmp(flash_str_dev_id[idx].str_dev_id, str, strlen(str)) == 0)
      {
        client_ctxt->flash_dev_ctxt->internal_dev_id = flash_str_dev_id[idx].dev_id;
        break;
      }
    }
  }
...
/* This function starts a page read in non blocking mode and
 * return before the page read is completed
 * wait_for_page_read api should be used to check status of this op
 */
static int flash_fwk_start_page_read(
  flash_handle *pif, enum page_read_opcode read_type,
  dalsys_mem_desc_list *buffer_desc,
  struct flash_page_iovec *page_data_iovec)
{
  ...
  if (FLASH_DEV_ENTRY_VTBL(client_ctxt)->start_page_read)
  {
    status = FLASH_DEV_ENTRY_VTBL(client_ctxt)->start_page_read(
      client_ctxt, read_type, buffer_desc, page_data_iovec);
  }
...
/* This function waits for the previously started page read to complete
 * and returns status of that page read op.
 * start_page_read api should be used before calling this api
 */
static int flash_fwk_wait_for_page_read(
  flash_handle *pif)
{
  int status = FLASH_DEVICE_FAIL;
  flash_client_ctxt *client_ctxt = ((flash_handle *)pif)->flash_client_ctxt;

  if (FLASH_DEV_ENTRY_VTBL(client_ctxt)->wait_for_page_read)
  {
    status = FLASH_DEV_ENTRY_VTBL(client_ctxt)->wait_for_page_read(
      client_ctxt);
  }
...
static void flash_init_interface(flash_client_ctxt* client_ctxt)
{
  static const flash_handle_vtbl vtbl = {
    ...
    flash_fwk_start_page_read,
    flash_fwk_wait_for_page_read
  };
...
int flash_dev_attach(DALDEVICEID dev_id, DalDeviceHandle **handle)
{
  int idx;
  int status = FLASH_DEVICE_FAIL;

  if ((dev_id >= DALDEVICEID_FLASH_DEVICE_1) && \
    (dev_id <= DALDEVICEID_FLASH_DEVICE_3))
  {
	/* Attach using DAL Device Id */ 
    status = DAL_DeviceAttach(dev_id,handle);
  }
  else
  {
    for (idx = 0; idx < FLASH_FWK_MAX_DEV_ID_ENTRIES; idx++)
    {
      if (dev_id == flash_str_dev_id[idx].dev_id)
      {
        if (dev_id >= 4)
        {
          /* Attach using DAL String Id */ 
          status = DAL_StringDeviceAttach(flash_str_dev_id[idx].str_dev_id, handle);
          break;
        }
		else
        {
          /* Attach using DAL Device Id */ 
          status = DAL_DeviceAttach(flash_str_dev_id[idx].dal_dev_id, handle);
          break;
...
int flash_dev_close(DalDeviceHandle *handle)
{
  int status = FLASH_DEVICE_FAIL;
  
  status = DalDevice_Close(handle);
  return status;
}

int flash_dev_detach(DalDeviceHandle *handle)
{
  int status = FLASH_DEVICE_FAIL;
  
  status = DAL_DeviceDetach(handle);
  return status;
}
