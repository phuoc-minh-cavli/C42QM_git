 ...
 *   order.
 *
 *  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/FlashCommonLib/Loader/flash_nor_i2c_init.c#1 $ $DateTime: 2018/08/01 23:51:05 $ $Author: pwbldsvc $
 *
...
static int nor_i2c_init(flash_client_ctxt *client_ctxt)
{
  ...
  if (FLASH_PROPERTY_NOT_DEFINED != (uint32)i2c_tbl)
  {
    if (i2c_tbl && i2c_tbl->i2c_config)
    {
      ...
      result = nor_i2c_ops.init(nor_i2c_pdata->config_data->base_address, 
        nor_i2c_pdata->config_data->clk, nor_i2c_pdata->config_data->flags1);
...
int nor_i2c_probe(flash_client_ctxt *client_ctxt)
{
  ...
  for (cfg_count = 0; (FLASH_UNKNOWN != nor_cfgs[cfg_count].dev_type); cfg_count++)
  {
    ...
    if ((dev_ctxt->internal_dev_id == FLASH_DEVICEID_4) && (cfg_count == 2))
    {
      if (FLASH_NOR_I2C == nor_cfgs[cfg_count].dev_type) 
      {
        devices = nor_cfgs[cfg_count].dev_params;
