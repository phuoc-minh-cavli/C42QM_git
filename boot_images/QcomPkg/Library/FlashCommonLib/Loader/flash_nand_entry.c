 ...
 *
 * Copyright (c) 2008-2013,2015,2017, 2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 06/15/19     sl      Seperated R/W count to support pagescope/multipage.
...
static dalsys_mem_desc_list *flash_queued_buffer_desc;
static dalsys_mem_desc_list *flash_queued_phys_buf_list;
/*===================================================================
 *
  ...
  struct flash_block_vector *block_vector, uint32 iovec_count)
{
  ...
  if(flash_queued_phys_buf_list || flash_queued_buffer_desc)
  {
    result = FLASH_DEVICE_FAIL;
    return result;
  }
...
/*
 * This API calls the handle to start a page read within a partition
 * This is a non-blocking call and driver won't wait for page read to
 * complete before returning from this API
 */
int nand_entry_start_page_read(flash_client_ctxt *client_ctxt,
  enum page_read_opcode read_type, dalsys_mem_desc_list *buffer_desc,
  struct flash_page_iovec *page_data_iovec)
{
  int result = FLASH_DEVICE_DONE;
  ...
  if (FLASH_DEVICE_DONE == result)
  {
    ...
    if (FLASH_DEVICE_DONE == result)
    {
      result = nand_start_page_read(client_ctxt, read_type,
        (dalsys_mem_desc_list *)phys_buf_list, page_data_iovec);

      if(FLASH_DEVICE_DONE == result)
      {
        flash_queued_phys_buf_list = phys_buf_list;
        flash_queued_buffer_desc = buffer_desc;
      }
    }
  }
...
/*
 * This API calls should be called in continuation to nand_entry_start_page_read API
 * This API will wait for the previously initiated page read to complete.
 */
int nand_entry_wait_for_page_read(flash_client_ctxt *client_ctxt)
{
  int result = FLASH_DEVICE_DONE;
  ...
  result = nand_wait_for_page_read(client_ctxt);
  if (FLASH_DEVICE_DONE == result)
  {
    if (FLASH_DEVICE_DONE != flash_copy_dma_buffers(flash_queued_buffer_desc,
      (dalsys_mem_desc_list *) flash_queued_phys_buf_list))
    {
      result = FLASH_DEVICE_FAIL;
    }
  }

  flash_queued_phys_buf_list = NULL;
  flash_queued_buffer_desc = NULL;
...
flash_vtbl* nand_get_entry_vtbl_reduced(void)
{
  ...
  nand_entry_vtbl.start_page_read     = nand_entry_start_page_read;
  nand_entry_vtbl.wait_for_page_read  = nand_entry_wait_for_page_read;
