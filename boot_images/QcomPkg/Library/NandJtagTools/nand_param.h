 ...
 *
 * Copyright (c) 2008-2009, 2019 Qualcomm Technologies, Inc. All rights reserved.
 *=======================================================================*/
 ...
 *   order.
 *
 *   $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/NandJtagTools/nand_param.h#4 $ $DateTime: 2019/10/14 05:05:36 $ $Author: pwbldsvc $
 *
...
/* FLASH_PROG_BUFFER_SIZE
   = pagesize * max_num_pages_to_read
   = (4096+128) * 32
   = 4224 * 32
   = 0x21000
*/
#define FLASH_PROG_BUFFER_SIZE 0x21000
 ...
 * our FLASH plug-in. */
#if 0 
/* For T32 on 64-bit processsor(cortex A53 - vipertooth) */
struct flash_prog_param {
   UINT64 flashaddr;
   UINT64 flashwidth;
   UINT64 width;
   UINT64 offset;
   UINT64 addr;            // in: address of the flash to write/erase
   UINT64 size;            // in: size of data to write/erase
   UINT64 reserved;
   UINT64 status;          // in: 1=program, 2=erase, 5=erase,
...
/* For T32 on 32-bit processsor(cortex A7) */
struct flash_prog_param {
   uint32 flashaddr;
   uint32 flashwidth;
   uint32 width;
