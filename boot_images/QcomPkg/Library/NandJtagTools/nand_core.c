 ...
 *
 * Copyright (c) 2010-2015, 2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 12/05/19     sl      mark block bad only if error is FLASH_DEVICE_FAIL
 * 01/28/15     sb      Cleanup
...
// #endif
ALIGN(ALIGN_VAL) static uint32 magic_buffer[MAX_PAGE_SIZE/(sizeof(uint32))] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
ALIGN(ALIGN_VAL)static unsigned char page_buffer[MAX_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");

ALIGN(ALIGN_VAL)static unsigned char readback_spare_buf[FLASH_NAND_SPARE_NUM_BYTES] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
ALIGN(ALIGN_VAL)static unsigned char spare_buffer[FLASH_NAND_SPARE_NUM_BYTES] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
ALIGN(ALIGN_VAL)static unsigned char temp_spare_buff[FLASH_NAND_SPARE_NUM_BYTES] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
...
/* Partition tables from host, MIBIB1, and MIBIB2 and variables to
  ...
  */
ALIGN(ALIGN_VAL) unsigned char oldest_parti_buffer[MAX_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");

/* Buffer for reading user partition tbl out from flash */
ALIGN(ALIGN_VAL) unsigned char oldest_usr_parti_buffer[MAX_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
ALIGN(ALIGN_VAL) unsigned char partial_page_buffer[TOTAL_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
ALIGN(ALIGN_VAL) unsigned char factory_img_buffer[TOTAL_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
// #if !defined(BUILD_EHOSTDL) && !defined(BUILD_HOSTDL)
