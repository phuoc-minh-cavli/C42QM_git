 ...
 *
 * Copyright (c) 2008-2012, 2014, 2019 by Qualcomm Technologies, Inc. All Rights Reserved.
 *=======================================================================*/
 ...
 *   order.
 *
 *   $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/NandJtagTools/nand_entry.c#3 $ $DateTime: 2019/06/17 00:13:57 $ $Author: pwbldsvc $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 06/11/19     sl      updated dal heap init for multipage
...
static uint8 flash_tools_heap[FLASH_TOOLS_HEAP_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
...
#endif
{
  ...
  {
    DALSYS_InitMod(NULL);
    // as per current implementation of dalsys_malloc 3rd param
    // is for external memory and if ext. memory is available then
    // heap with memheap[0] is not used. so if we pass 2 heapinits here
    // one with 0 (DALHEAP_IMEM) and one with 1 (DALHEAP_EXT_MEM)
    // then the memory passed with 0 will not be
    // utilized as per current dalsys_malloc implementation
    // so making sure we call DALSYS_HeapInit just once with 0 as 3rd param
    DALSYS_HeapInit(flash_tools_heap, FLASH_TOOLS_HEAP_SIZE, DALHEAP_IMEM);
    (void)bam_drv_init();
    flash_tools_init = 1;
  }
