 ...
 * ----------   ---     ----------------------------------------------
 * 07/23/19     sl      fix spare buf size.
...
static __inline void init_buffer_descriptor(
   ...
   uint32 op_type, uint32 buff_addr, uint32 spare_buff_addr,
  ...
  uint32 page, uint32 read_type, void *buffer)
{
  ...
  init_buffer_descriptor(&page_data_iovec, read_type,
    (uint32) buffer, (uint32)NULL, (DALSysMemDescList *) &page_buff_desc);
  ...
  uint32 page, uint32 page_count, uint32 read_type, void *buffer)
{
  ...
  /* Initialize the page data IOVEC structure */
  init_page_param(dev_info, read_type, page, page_count, &page_data_iovec, 0);
  ...
  uint32 spare_udata_blen)
{
  ...
  /* Initialize the page data IOVEC structure */
  init_page_param(dev_info, read_type, page, page_count ,&page_data_iovec, spare_udata_blen);
