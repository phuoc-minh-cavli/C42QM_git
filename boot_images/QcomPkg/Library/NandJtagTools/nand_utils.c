 ...
 *
 * Copyright (c) 2008-2010,2012-2013,2017,2019 QUALCOMM Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *   $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/NandJtagTools/nand_utils.c#2 $
 *   $DateTime: 2019/06/17 00:13:57 $
 *   $Author: pwbldsvc $
 *
...
static uint32 flash_utils_init(int page_count)
{
  ...
  if (DAL_SUCCESS != DALSYS_Malloc(total_page_size_bytes*page_count,
    (void **)&page_buffer))
  {
    FLASHLOG(1, ("[%s,%d] Malloc failed\n", __func__, __LINE__));
    goto close_and_detach;
  }
...
uint32 flash_utils_read_page(void)
{
  ...
  if (FLASH_PROG_SUCCESS == result)
  {
    page = (uint32)prog_param.reserved;
    ...
    memset(page_buffer, 0xBD, total_page_size_bytes * no_of_pages);
    ...
    switch (read_type)
    {
      case FLASH_READ_MAIN:
      case FLASH_READ_RAW:
        op_result = flash_prog_read_mpages(handle_flash, &dev_info,
      ...
      case FLASH_READ_MAIN_SPARE:
        ...
        op_result = flash_prog_read_mpages_and_udata(handle_flash, &dev_info,
...
uint32 flash_utils_fill_page(void)
{
  ...
  {
    page = (uint32)prog_param.reserved;
    fill_value = (uint32)prog_param.size;
...
uint32 flash_utils_read_uuid (void)
{
  ...
  FLASHLOG(1, ("=======================\n"));
    ...
    if (DAL_SUCCESS != flash_get_uuid(handle_flash, &prog_param.data,
      FLASH_PROG_BUFFER_SIZE, (uint32 *)&prog_param.size))
    {
      result = FLASH_PROG_FAIL;
    }
