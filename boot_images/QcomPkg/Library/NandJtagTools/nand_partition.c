 ...
 *
 * Copyright (c) 2008-2010, 2012-2014,2017, 2019 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 * ----------   ---     ----------------------------------------------
 * 04/16/19     sl      improve partition layout output
 * 04/03/19     sl      allow pad size 0 for 1st partition having block size 1.
 * 03/23/19     sl      open MIBIB partition and ignore if it returns error.
...
// #endif
static ALIGN(ALIGN_VAL) unsigned char mibib_pg_buf[MAX_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
static ALIGN(ALIGN_VAL) unsigned char is_erased_pg_buf[MAX_PAGE_SIZE] SECTION(".bss.FLASH_TOOLS_HEAP_ZONE");
// #if !defined(BUILD_EHOSTDL) && !defined(BUILD_HOSTDL)
...
uint32 flash_prog_print_layout(void)
{
  ...
  {
    FLASHLOG ( 1,("  %-16s", (char *)&parti_sys->part_entry[i].name));
    offset = parti_sys->part_entry[i].offset;
    FLASHLOG ( 1,("\t\t0x%-5x", offset));
    limit = parti_sys->part_entry[i].length;
    if (limit == -1)
    {
      limit = mibib.flash_info.partition.block_count;
      FLASHLOG ( 1,("\t\t 0x%-5x", limit));
    }
    else
    {
      FLASHLOG ( 1,("\t\t 0x%-5x", (offset + limit)));
    }
...
int flash_prog_init_for_partition_image (parti_ctl_t pctl, char *pname)
{
  ...
  /*
    JTAG tools open partition with the name "all" and
    provide an offset w.r.t. block0 for flashing/any other operation on NAND.
    as we have moved MIBIB retrieve from probe to nand_update_limits,
    we won't be able to get MIBIB data with "all" partition name.
    JTAG scripts which print partition layout or print MIBIB details
    will fail in this case, so, added a call to open MIBIB partition.
    This way tools will work in the same way they worked before.
  */
  if (FLASH_DEVICE_DONE == flash_open_partition(pctl->flash_handle,
    (const unsigned char *)MIBIB_PARTI_NAME))
  {
  ...
  {
    /*
      If opening MIBIB partition fails tools should not be blocked and
      should be able to go ahead with rest of the flashing features.
      In some customer issues MIBIB partition had 2 copies of MIBIB with
      same age and so nand_mibib_retrieve failed causing JTAG tools to fail.
      So they were not able to recover the device even with JTAG tools.
      By ignoring this error here, tools won't be blocked.
      It will print the error and continue and customers or anyone
      will be able to recover the device.
    */
    FLASHLOG(3, ("flash_open_partition error for MIBIB \n"));
  }
  if (FLASH_DEVICE_DONE == flash_open_partition(pctl->flash_handle,
...
int flash_prog_find_nth_good_block(flash_handle_t flash_handle, int which_block)
{
  ...
  /* 
     This is done only for MIBIB programming
     partition layout is not available for mibib programming
     so we need to initialize curblk to starting block of MIBIB partition
  */
  if(FLASH_MI_BLOCK_MIBIB == which_block)
  {
    curblk = FLASH_MI_BLOCK_MIBIB;
    which_block = 0;
    endblk = (FLASH_MI_BLOCK_MIBIB+MIBIB_DEFAULT_PARTI_SIZE);
  }
  else
  {
    endblk = flash_info.partition.block_count;
  }
                                             ...
                                             flash_partable_t parti2)
{
  ...
  for (i=0; i<parti1->numparts; i++)
  {
    ...
    if (partition_pad == 0)
    {
      ...
      {
        /*
          As part of SW WA for a QPIC MPU HW bug,
          the first partition will be a dummy partition and not SBL partition
          and it will be of size 1 block i.e. pad blocks for first partition
          will be 0. So skipping pad block check for first partition.
        */
      }
      else
      {
