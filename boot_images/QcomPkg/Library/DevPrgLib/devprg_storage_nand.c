...
----------   ---   ---------------------------------------------------------
2022-02-11   sk    Fix erase of any partition with bad blocks
2022-01-17   sk    Fix bad block pattern fill issue while reading.
                                      ...
                                      uint64 lba, uint32 *pba)
{
  ...
  uint32 bb_count = 0, total_blocks = 0, num_erased_blocks = 0;
  ...
  total_blocks = dev->last_avail_pba-dev->start_pba+1;
  num_erased_blocks = total_blocks - bb_count;

  /* not enough good blocks in the given partition */
  DDP_LOGI(
  "[%d] out of good blocks, total blocks:0x%x badblocks:0x%x erased blocks:0x%x\n",
  __LINE__, total_blocks, bb_count, num_erased_blocks);
...
void devprg_storage_nand_init(void)
{
  ...
  DALSYS_InitMod(NULL);
  DALSYS_HeapInit(dp_flash_uncached_tools_heap, FLASH_TOOLS_UNCACHED_HEAP_SIZE, 2);
  DALSYS_HeapInit(dp_flash_tools_heap, (FLASH_TOOLS_HEAP_SIZE-FLASH_TOOLS_UNCACHED_HEAP_SIZE), 1);
}
...
void* devprg_storage_nand_open(uint32 slot, uint32 partition, dp_res_t *error)
{
  ...
  fresult = flash_open_partition( handle, (const unsigned char *)"all");
  if(fresult != DAL_SUCCESS)
  {
    *error = translate_flash_error(fresult);
                                  ...
                                  uint64 num_of_pages)
{
  ...
  uint32 actual_page_size = nand_page_size;
  ...
  if(dev->skip_bb == BAD_BLOCK_NO_SKIP)
  {
    pba = start_block_num;
                                    ...
                                    uint64 start_page, uint64 num_of_pages)
{
  ...
  /* Do not support write operation in case of BAD_BLOCK_NO_SKIP */
                                   ...
                                   uint64 num_of_pages)
{
  ...
  if(dev->skip_bb != BAD_BLOCK_SKIP && dev->skip_bb != BAD_BLOCK_NO_SKIP)
  {
    DDP_LOGE("[%d] skip_bb - %d not supported\n",
            __LINE__, dev->skip_bb);
    return DEVPRG_ERROR_NOT_SUPPORTED;
  ...
  for(i = 0; i < max_avail_blocks; i++)
  {
	...
	{
	  pba = erase_block;
      if (FALSE == DEVPRG_STORAGE_NAND_BLOCK_IS_BAD(dev->bbt, erase_block))
        error = DEVPRG_SUCCESS;
      else
	    error = DEVPRG_ERROR_DO_NOT_SKIP_BAD_BLOCKS;
	}
	else /* get actual block to erase in case of BAD_BLOCK_SKIP */
      error = nand_get_pba_from_lba(dev, erase_block, &pba);

    if(error != DEVPRG_SUCCESS)
    {
	  /* Bad block counted as erased block in case of BAD_BLOCK_NO_SKIP */
	  if (dev->skip_bb == BAD_BLOCK_NO_SKIP)
	  {
	    erase_block_count--;
        error = DEVPRG_SUCCESS;
