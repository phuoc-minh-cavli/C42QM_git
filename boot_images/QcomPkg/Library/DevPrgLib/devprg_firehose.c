...
/*===========================================================================
  ...
  $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/DevPrgLib/devprg_firehose.c#5 $
  $DateTime: 2022/02/21 11:41:08 $
  $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2022-02-14   sk    Rename BAD_BLOCK_FILL to BAD_BLOCK_NO_SKIP in skip_bb
2020-01-07   as    Add enhancements in hsuart transport layer
2019-07-15   sl    Added support for reset in devprg over HSUART.
...
cleanup:
  ...
  if(result == 0 && error == DEVPRG_SUCCESS)
    ack = 1;
  else if(result == 0 && error == DEVPRG_ERROR_READONLY)
  {
    DP_LOGE("Write failed, Probable writing to a read only partition", '.');
    ack = 1;
  }
  else
  {
...
static int handle_read(struct dp_xml_handle *xml_handle)
{
  ...
  /* Now that we have all the data from XML and it looks good,
     disable log and send an ACK.*/
  devprg_log_set_state(LOG_DISABLE);
  ack = 1; rawmode = 1;
  ...
  while (sectors_remaining != 0)
  {
    ...
    error = devprg_tx_blocking(data, read_size);
    if (error != DEVPRG_SUCCESS)
    {
      goto cleanup;
    }
...
cleanup:
  ...
  devprg_log_set_state(LOG_ENABLE);
...
cleanup:
  ...
  {
    DP_LOGE("An attempt to erase a read-only partitions was ignored, other partition if any are erased successfully", '.');
    ack = 1;
  }
  else
  {
