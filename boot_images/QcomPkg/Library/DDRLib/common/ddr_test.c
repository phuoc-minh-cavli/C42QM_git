...
$Header: //components/rel/boot.xf/0.2/QcomPkg/Library/DDRLib/common/ddr_test.c#1 $
$DateTime: 2018/08/01 23:51:05 $
$Author: pwbldsvc $
================================================================================
when       who     what, where, why
--------   ---     -------------------------------------------------------------
05/29/15   sk      Added changes for 1.5GB+1.5GB memmap issue
05/21/15   sk      Added changes for DDR timeout issue
...
================================================================================
                 Copyright 2010,2012-2015 Qualcomm Technologies Incorporated
                              All Rights Reserved
 ...
 */  
#define DDR_TEST_LOG_MESSAGE(MESSAGE, VALUE) \
          do \
          { \
            test_log.message = (MESSAGE); \
			test_log.value=(uint32) (VALUE); \
          } while (0)
...
{
  DDR_TEST_LOG_MESSAGE("Testing Memory Map.",0);

     return ((limit >= sizeof(uint32) - 1) &&
           ((base & limit) == 0x0) );
}
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Entering Data lines.",0);  
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Exiting Data lines.",0);  
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Entering Address lines.",0);    
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Exiting Address lines.",0);  
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Entering Own-address algorithm.",0);  
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Exiting Own-address algorithm.",0);    
  return TRUE;
}
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Entering Walking-ones algorithm.",0);  
  ...
  for (start_pattern = 0x1; start_pattern != 0x0; start_pattern <<= 1)
  {
	...
	DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Walking-ones algorithm : Write Pass.",0);	
	...
	DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Walking-ones algorithm : Read Pass.",0);		
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write: Exiting Walking-ones algorithm.",0);  
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Read/Write.",0);  
...
)
{
  ...
  DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh.",0);  
  ...
  /* write each word with its own address */
  DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh: Write pass.",0);  
  for (offset = 0; offset <= limit; offset++)
  {
    base_addr[offset] = offset;
  }
  ...
  while (iterations > 0)
  {
    ...
    /* enter self refresh */
    DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh: Entering self refresh.",0);	
    ddr_enter_self_refresh(interface, chip_select, clk_speed);

    /* delay for some time */
    DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh: In self refresh.",0);	
    DDR_TEST_DELAY(DDR_TEST_SELF_REFRESH_DELAY);

    /* exit self refresh */
    DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh: Exiting self refresh.",0);	
    ddr_exit_self_refresh(interface, chip_select, clk_speed);
  }

  /* check if same value is read back from each word */
  DDR_TEST_LOG_MESSAGE("Testing DDR Self Refresh: Read pass.",0);  
  for (offset = 0; offset <= limit; offset++)
  {
    if (base_addr[offset] != offset)
    {
...
{
  DDR_TEST_LOG_MESSAGE("Testing DDR Deep Power Down.",0);  
  ...
  while (iterations > 0)
  {
    ...
    DDR_TEST_LOG_MESSAGE("Testing DDR Deep Power Down: Entering deep power down.",0);		
    ddr_enter_deep_power_down(interface, chip_select);

    /* delay for some time */
    DDR_TEST_LOG_MESSAGE("Testing DDR Deep Power Down: In deep power down.",0);		
    DDR_TEST_DELAY(DDR_TEST_DEEP_POWER_DOWN_DELAY);

    /* exit deep power down */
    DDR_TEST_LOG_MESSAGE("Testing DDR Deep Power Down: Exiting deep power down.",0);		
...
#ifndef DDR_CLOCK_TEST_FRAME_WORK
/* =============================================================================
...
*   None
*/
static ddr_test_error_type ddr_test_clock_switching(void)
{
 uint32 nNumLevel=0x0;
 ClockPlanType   *pClockPlan;
 ddr_size_info ddr_size = ddr_get_size();
 volatile uint32 index1, index2;
 uint32 is_dual_channel=0; 
 
 DDR_TEST_LOG_MESSAGE("Testing DDR Clock Switching: Enter DDR Clock Switch.",0); 
 ...
 /* Call BIMCQuery to get the Num Perf Levels */
  ...
  if( pClockPlan == NULL ) 
  {
    ...
    return DDR_TEST_ERR_CLOCK_SWITCHING;
  }
  else
  {
   for(index1=0;index1<nNumLevel;index1++)
	{	 	 
	 DDR_TEST_LOG_MESSAGE("Testing BIMC Clock Frequency:.",(pClockPlan[index1].nFreqKHz));	 	 
	 //Switching BIMC clock frequency
     ddr_pre_clock_switch(0, ((pClockPlan[index1].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_0);
     if(is_dual_channel==1)	 
		ddr_pre_clock_switch(0, ((pClockPlan[index1].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_1);
	 boot_clock_set_bimcspeed(pClockPlan[index1].nFreqKHz);
	 ddr_post_clock_switch(0, ((pClockPlan[index1].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_0);
	 if(is_dual_channel==1)
		ddr_post_clock_switch(0, ((pClockPlan[index1].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_1);
	 //Test Read/Write cycles in DDR after clock switch
	 ddr_test_own_addr(ddr_size.sdram0_cs0_addr, 0x10000000);
	 
	 for (index2 = 0; index2 <nNumLevel; index2++) 
			 {
			  DDR_TEST_LOG_MESSAGE("Testing BIMC Clock Frequency:.", (pClockPlan[index2].nFreqKHz));			  
			  //Switching BIMC clock frequency
			  ddr_pre_clock_switch(0, ((pClockPlan[index2].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_0);
			  if(is_dual_channel==1)
				ddr_pre_clock_switch(0, ((pClockPlan[index2].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_1);
			  boot_clock_set_bimcspeed(pClockPlan[index2].nFreqKHz);
			  ddr_post_clock_switch(0, ((pClockPlan[index2].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_0);
			  if(is_dual_channel==1)
				ddr_post_clock_switch(0, ((pClockPlan[index2].nFreqKHz)*DDR_CLOCK_RATIO), SDRAM_INTERFACE_1);
			  //Test Read/Write cycles in DDR after clock switch
			  ddr_test_own_addr(ddr_size.sdram0_cs0_addr, 0x10000000);
 ...
 DDR_TEST_LOG_MESSAGE("Testing DDR Clock Switching: Exiting DDR Clock Switch.",0);  
 DALSYS_Free(pClockPlan);
...
)
{
  ...
  else if ((ddr_base + (ddr_size-1)) > min_base)
  {
    ...
    switch (test_suite)
    {
	  ...
	  case DDR_TEST_SUITE_ACCESS_RAM_TEST:
	  ...
	  case DDR_TEST_SUITE_CLOCK_SWITCHING:
	    #ifndef DDR_CLOCK_TEST_FRAME_WORK
		test_error_type = ddr_test_clock_switching();
		#endif
		break;
    default:
      break;
