...
ReInitPagePoolBuffer (VOID)
{
  ...
  Status = AllocPagePoolOperation (SET_PAGEPOOL, &pPagePool, (UINT64 *)&NumPages);
...
AllocatePages (IN UINTN Pages)
{
  ...
  Status = AllocPagePoolOperation (GET_PAGEPOOL_INFO, &pPagePool, (UINT64 *)&PageCount);
  if (Status == EFI_NOT_READY)
  {
    ...
    Status = AllocPagePoolOperation (SET_PAGEPOOL, &pPagePool, (UINT64 *)&PageCount);
    ASSERT (Status == EFI_SUCCESS);

    /* Update pool buffer for current allocation */
    PageCount = Pages;
    Status = AllocPagePoolOperation (SET_PAGEPOOL_ALLOC, &pPagePool, (UINT64 *)&PageCount);
    ASSERT (Status == EFI_SUCCESS);
