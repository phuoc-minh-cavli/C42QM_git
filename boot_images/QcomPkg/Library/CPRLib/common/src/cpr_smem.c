 ...
 * $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/CPRLib/common/src/cpr_smem.c#1 $
 * $Change: 16788488 $
 */
#include "cpr_smem.h"
#include "cpr_image.h"
...
static void cpr_smem_write_sub_modes(cpr_smem_hdr* hdr, uint32 *currOffset, cpr_mode_settings* modeSetting)
{
    for(int i = 0; i < modeSetting->subModesCount; i++)
    {
        ...
        INCREMENT_OFFSET(sizeof(cpr_submode_settings));
                ...
                submode->freq             = modeSetting->subModes[i].freq;
		submode->ceiling          = modeSetting->subModes[i].ceiling;
		submode->floor            = modeSetting->subModes[i].floor;
		submode->current          = modeSetting->subModes[i].current;
		submode->min              = modeSetting->subModes[i].min;
		submode->debug.floor      = modeSetting->subModes[i].debug.floor;
		submode->debug.ceiling    = modeSetting->subModes[i].debug.ceiling;
		submode->debug.up         = modeSetting->subModes[i].debug.up;
		submode->debug.down       = modeSetting->subModes[i].debug.down;
		submode->debug.ignored    = modeSetting->subModes[i].debug.ignored;
...
static void cpr_smem_read_sub_modes(cpr_smem_hdr* hdr, uint32 *currOffset, cpr_mode_settings* modeSetting)
{
    for(int i = 0; i < modeSetting->subModesCount; i++)
    {
        ...
        modeSetting->subModes[i].freq = submode->freq;
        modeSetting->subModes[i].ceiling  = submode->ceiling; 
        modeSetting->subModes[i].floor = submode->floor;
        modeSetting->subModes[i].current = submode->current;
        modeSetting->subModes[i].min = submode->min;
        modeSetting->subModes[i].debug.floor = submode->debug.floor;
        modeSetting->subModes[i].debug.ceiling = submode->debug.ceiling;
        modeSetting->subModes[i].debug.up = submode->debug.up;
        modeSetting->subModes[i].debug.down = submode->debug.down;
        modeSetting->subModes[i].debug.ignored = submode->debug.ignored;
