 ...
 *
 * $DateTime: 2019/06/06 03:29:44 $
 * $Header: //components/rel/boot.xf/0.2/QcomPkg/Library/CPRLib/image/xbl/src/cpr_image.c#3 $
 * $Change: 19440061 $
 */
#include <stdlib.h>
#include "cpr_image.h"
...
cpr_foundry_id cpr_image_get_foundry()
{
	...
	uint8 foundryID = (HWIO_IN(QFPROM_RAW_FEAT_CONFIG_ROW2_MSB) >> 0x7 ) & 0xF; 
...
void cpr_image_open_remote_cfg(void** cfg, uint32* size)
{
    *cfg = smem_get_addr( SMEM_CPR_CONFIG, size );
    ...
    if(!*cfg)
    {
        *size = 0xC00;     //Reserve 1.5KB for now
...
void cpr_externalize_state()
{
    ...
    //cpr_smem_serialize_config( cpr_utils_get_rail_state( CPR_RAIL_MX   ), false );
    //cpr_smem_serialize_config( cpr_utils_get_rail_state( CPR_RAIL_LPI_CX ), true );
	//cpr_smem_serialize_config( cpr_utils_get_rail_state( CPR_RAIL_LPI_MX ), true );
