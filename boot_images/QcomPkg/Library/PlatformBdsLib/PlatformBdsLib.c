...
UpdatePlatformVars(VOID)
{
  ...
  else if(boot_from_nand() == TRUE)
  {
    Status = HWIOProtocol->MapRegion(HWIOProtocol, "+QPIC_QPIC_XPU2", (UINT8**) &Addr);
    if (EFI_SUCCESS != Status)
		DEBUG ((EFI_D_ERROR, "ERROR: Failed to map QPIC_QPIC_XPU2 base address: 0x%08x\n", Status));
  }
  else
  {
