...
uintPtr HAL_DSI_GetRegBaseOffset(DSI_Device_IDType  eDeviceId)
{
  ...
  switch (eDeviceId)
  {
    ...
    case DSI_DeviceID_1:
    {
      uOffset = HWIO_MMSS_DSI_1_HW_VERSION_ADDR - HWIO_MMSS_DSI_0_HW_VERSION_ADDR;
      break;
    }
                           ...
                           uint32             *puDsiStatus)
{
  ...
  else
  {
    ...
    *puDsiStatus   =  0;
    ...
    /* Data lane 0 state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN0_STOPSTATE))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN0_STOPSTATE;
    }
  
    /* Data lane 1 state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN1_STOPSTATE))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN1_STOPSTATE;
    }

    /* Data lane 2 state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN2_STOPSTATE))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN2_STOPSTATE;
    }

    /* Data lane 3 state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN3_STOPSTATE))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN3_STOPSTATE;
    }

    /* Clock lane state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, CLKLN_STOPSTATE))
    {
      *puDsiStatus |= DSI_LANE_STATUS_CLKLN_STOPSTATE;
    }

    /* Data lane 0 ULPS state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN0_ULPS_ACTIVE_NOT))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN0_ULPS_NOT_ACTIVE;
    }

    /* Data lane 1 ULPS state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN1_ULPS_ACTIVE_NOT))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN1_ULPS_NOT_ACTIVE;
    }
    
    /* Data lane 2 ULPS state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN2_ULPS_ACTIVE_NOT))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN2_ULPS_NOT_ACTIVE;
    }

    /* Data lane 3 ULPS state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, DLN3_ULPS_ACTIVE_NOT))
    {
      *puDsiStatus |= DSI_LANE_STATUS_DLN3_ULPS_NOT_ACTIVE;
    }

    /* Clock lane ULPS state */
    if (0 != HWIO_GETVAL(DSI_0_LANE_STATUS, uDsiHWLaneStatusReg, CLKLN_ULPS_ACTIVE_NOT))
    {
