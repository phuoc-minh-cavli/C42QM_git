...
typedef enum
{
  ...
  DSI_DISPLAY_PROPERTYID_ULPSCLAMP,                     /**< DSI lane ULPS clamp control */
...
#define DSI_POLLING_CYCLE                          (1)          // 50 us

#define DSI_MAX_POLLING_TIME                       (50)          // 50 ms

/* Reference -  max configureable HW BTA timer (16 bit) value is 2^16 esc clks
 *              e.g:  escclk = 13.5MHz -> max_hw_bta_to = 4.8 ms  
 *                    escclk =  8 Mhz  -> max_hw_bta_to = 8.2 ms
 *
 * We typically use an escclk higher than the above examples. However, if the escclk is a lower value for 
 *  a platform, this value may need to be adjusted to account for it.
 */
#define DSI_MAX_BTA_DONE_POLL_TIME                 (10)           // 10 ms

#define DSI_LANE_T_WAKEUP_TIME                     1500          // 1500 us
