...
MDP_Status DSIDriver_Init(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  if (0 == (pPanelConfig->eDisplayFlags & MDP_PANEL_FLAG_DSI_DCS_FIFO_ONLY))
  {
    if (NULL == pDSIBuffer)
    {
      if (NULL == (pDSIBuffer = UncachedAllocateZeroPool(DSI_DMA_BUFFER_SIZE)))
      {
        DEBUG ((EFI_D_INFO, "DixplayDxe: UncachedAllocateZeroPool failed\n"));
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
    {
      ...
      {
        DEBUG ((EFI_D_INFO, "DixplayDxe: Invalid DSI Device ID %d\n", eDSIDeviceId));
        eStatus = MDP_STATUS_FAILED;
        break;
      }
      ...
      {
        DEBUG ((EFI_D_INFO, "DixplayDxe: DSIDriver_Init(MDP_CLOCKTYPE_DSI%d) failed\n", eDSIDeviceId));
        ...
        (NULL          != pPanelConfig->uAttrs.sDsi.sI2CConfig.pI2CInitSequence))
    {
      ...
      {
        DEBUG ((EFI_D_ERROR, "DixplayDxe: I2CDriver_Init() failed eStatus=%d \n", eStatus));
      }
    }
  }
...
void DSIDriver_Close(void)
{
  ...
  for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
  {
    ...
    {
      DEBUG ((EFI_D_INFO, "DixplayDxe: DSI_Display_Close() failed on device%d\n", uDevIdx ));
      break;
    }
    // Turn off DSI Clock turned on in DSIDriver_Init()
    if (MDP_STATUS_OK != MDPDisableClocks(DSI_CORE_CLOCK_MAPPING(uDevIdx)))
    {
      DEBUG ((EFI_D_INFO, "DixplayDxe: DSIDriver_Close(MDP_CLOCKTYPE_DSI%d) failed\n", uDevIdx));
      // Non fatal, only a warning .
    }
  }
  ...
  for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
  {
    ...
    {
      DEBUG ((EFI_D_INFO, "DixplayDxe: DSI_Device_Close() failed on device%d\n", uDevIdx ));
      break;
    }
  }
}
...
MDP_Status DSIDriver_SetMode(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
  {
    ...
    if (pPanelConfig->uAttrs.sDsi.uLaneRemap >= DSI_LANE_SWAPNUM)
    {
      ...
      DEBUG ((EFI_D_INFO, "DixplayDxe: DSIDriver_SetMode() Incorrect Lane remap parameter, defalting to 0\n" ));
    }
    ...
    if (pPanelConfig->ePhysConnect == MDP_DISPLAY_CONNECT_PRIMARY_DSI_VIDEO )
    {
      ...
      if (pPanelConfig->uAttrs.sDsi.bFBCEnable)
      {
        if (pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio)
        {
          sDisplayConfig.sDisplayInfo.iVisWidthPx                           /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sDisplayInfo.iBlankHeightPx                        /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sDisplayInfo.iBlankWidthPx                         /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iHsyncPulseWidthDclk  /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iHsyncBackPorchDclk   /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iHsyncFrontPorchDclk  /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iVsyncPulseWidthLines /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iVsyncBackPorchLine   /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
          sDisplayConfig.sVideoModeConfg.sVideoTiming.iVsyncFrontPorchLine  /= pPanelConfig->uAttrs.sDsi.uFBCCompressionRatio;
        }
        else
        {
    ...
    {
      sDisplayConfig.sDisplayInfo.iVisWidthPx    = pPanelConfig->uDisplayWidth;
      sDisplayConfig.sDisplayInfo.iBlankWidthPx  = 0 ;
      sDisplayConfig.sDisplayInfo.iVisHeightPx   = pPanelConfig->uDisplayHeight;
      sDisplayConfig.sDisplayInfo.iBlankHeightPx = 0;
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    if (DSI_Display_ClockLane_ForceHS == eClockLaneCfg)
    {
      ...
      for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)    
      {
        ...
        {
          DEBUG ((EFI_D_WARN, "DisplayDxe: Clock lane failed to enter HS pre-init\n"));
        }
      }
    }
    ...
    if (NULL != pPanelConfig->uAttrs.sDsi.sI2CConfig.pI2CInitSequence)
    {
      ...
      {
        DEBUG ((EFI_D_WARN, "DisplayDxe: Failed to send I2C command \n"));
      }
    }
    ...
    if (DSI_Display_ClockLane_ForceHS_PostInit == eClockLaneCfg)
    {
      ...
      for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)    
      {
        if (DSI_STATUS_SUCCESS != DSI_Display_SetProperty(pDriverCtx->hDisplayHandle[uDevIdx], DSI_DISPLAY_PROPERTYID_LANESTATE, &sDSIProp))
        {
          DEBUG ((EFI_D_WARN, "DisplayDxe: Clock lane failed to enter HS post-init\n"));
        }
      }
    }
  }
...
MDP_Status DSIDriver_MinimalInit(void)
{
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    if (MDP_STATUS_OK == eStatus)  
    {
      ...
      sDeviceConfig.bPhyDCDCMode                         = FALSE;
      ...
      else
      {
        ...
        sMDPPlatformParam.sPlatformPanelReset.uFlags |= MDDPLATFORM_PANEL_RESET_FLAG_USE_CUSTOM_DELAY;
        sMDPPlatformParam.sPlatformPanelReset.uCustomDelayMs  = 0;  // no wait after reset
        
        if (MDP_STATUS_OK != MDPPlatformConfigure(MDP_DISPLAY_PRIMARY, MDPPLATFORM_CONFIG_RESETPANEL, &sMDPPlatformParam))
        {
          DEBUG ((EFI_D_INFO, "DixplayDxe: MDPPlatformConfigure(MDPPLATFORM_CONFIG_RESETPANEL) failed\n" ));    
        }
...
MDP_Status DsiPanelInit(MDP_Panel_AttrType    *pPanelInfo)
{
  ...
  {
    DEBUG ((EFI_D_INFO, "DixplayDxe: DSIDriver_Init() failed\n" ));
    eStatus = MDP_STATUS_FAILED;
  }
  else 
  {
    ...
    {
      DEBUG ((EFI_D_INFO, "DixplayDxe: DSIDriver_SetMode() failed\n" ));
      eStatus = MDP_STATUS_FAILED;
    }
  }
    ...
    {    
      ...
      if (DSI_DeviceID_0 == uDevIdx)
      {
        ...
        if (MDP_DISPLAY_CONNECT_PRIMARY_DSI_VIDEO == pPanelConfig->ePhysConnect)
        {
          ...
          {
           DEBUG ((EFI_D_INFO, "DSI_Display_SetProperty(DSI_DISPLAY_PROPERTYID_LP11) Failed"));
          }
          ...
          {
           DEBUG ((EFI_D_INFO, "DSI_Display_SetProperty(DSI_DISPLAY_PROPERTYID_RESETCORE) Failed"));
          }
        }
        ...
        if (FALSE == pPanelConfig->uAttrs.sDsi.bForceCmdInVideoHS)
        {
          ...
          {
              DEBUG ((EFI_D_INFO, "DixplayDxe: SendPanelXMLSequence() failed on device%d\n", uDevIdx ));
              // Warn and continue
          }
        }
      }
...
MDP_Status DsiPanelDumpRegisters(void)
{
  ...
  DEBUG ((EFI_D_ERROR, "Offset   Data\n"));
  DEBUG ((EFI_D_ERROR, "-------+------------------------------------------------------------\n"));

  for (uIndex=0;uIndex<uReadSize;uIndex++)
  {
      uint16  uAddress     = uIndex<<1; // Align to even addresses only
      uint32  uReponseSize = sizeof(uResponse);
  
      if (0 == (uIndex % uDataWidth))
      {
        DEBUG ((EFI_D_ERROR, "0x%04x   ", uAddress));
      }
      ...
      {
        DEBUG ((EFI_D_ERROR, "0x%08x ", uResponse[0]));        
      }
      else
      {
    ...
    while (0 != sXMLBinaryData.uPacketLength)
    {
      ...
      else
      {
        ...
        for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
        {
          ...
          {
            DEBUG ((EFI_D_WARN, "DisplayDxe: Packet transmission failed CMD:0x%x Length:%d\n", sWritePacket.puPayload[0], sWritePacket.bPayloadLen));
            eStatus = MDP_STATUS_FAILED;
            break;
          }
        }
