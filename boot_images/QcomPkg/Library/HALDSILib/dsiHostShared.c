  ...
  else
  {
    ...
    else
    {
      ...
      if ((iPacketLen + psSharedData->pCurrentBufferSize) > HAL_DSI_MAX_DMAFIFOSIZE_BYTE)
      {
        ...
        psSharedData->pCurrentBufferSize = 0;
        eStatus = DSI_STATUS_FAILED_BUFFER_TOO_SMALL;
        DSI_Log_Message(DSI_LOGLEVEL_ERROR, "dsiShared_ClientWrite: Command is too long for FIFO, swtich to MDP_PANEL_FLAG_DSI_DCS_DMA_ONLY.");
      }
      else
      {
        /* Command data fetched via TPG FIFO */
    ...
    )
{
  ...
  if (DSI_STATUS_SUCCESS == eStatus)
  {
    ...
    {
      eStatus = dsiShared_WaitforInterrupt(eDeviceId, HAL_DSI_INTERRUPT_BTA_DONE, DSI_MAX_BTA_DONE_POLL_TIME);
      if (DSI_STATUS_SUCCESS == eStatus)
      {
                                                 ...
                                                 uint32                 uWaitTime)
{
    ...
    if (0 != uIntFlags)
    {
        ...
        for (uLoopCount=0;uLoopCount<(uWaitTime*1000/DSI_POLLING_CYCLE);uLoopCount++)
        {
