...
static const DSI_LaneMapType asDisplayLaneSetting[LANESWAPNUM][DSI_LaneMap_MAX] =
{
                            ...
                            DSI_LaneMapType        *paDisplayLane)
{
  ...
  bool32  bMatchSoFar       = FALSE;
  uint32  i, j;

  for(i = 0; i < LANESWAPNUM; i++)
  {
    for(j =0; j <DSI_LaneMap_MAX-1; j++)
    {
      if ( (paDisplayLane[j] != asDisplayLaneSetting[i][j] )&&
        (paDisplayLane[j] != DSI_LaneMap_NONE) )
      {
        bMatchSoFar  = FALSE;
        break;
      }
  ...
  if(FALSE == bMatchSoFar)
  {
    geDSIErrorCode = HAL_DSI_FAILED_INVALID_INPUT_PARAMETER;
  }
  else
  {
    uLaneSwapCtrlReg = HWIO_OUT_FLD(uLaneSwapCtrlReg, DSI_0_LANE_SWAP_CTRL, DLN_SWAP_SEL,  i);
    out_dword(uOffset + HWIO_MMSS_DSI_0_LANE_SWAP_CTRL_ADDR, uLaneSwapCtrlReg  );
  }
