 ...
 *  API implementations.
 *  
 *  &copy; Copyright 2014 - 2018, 2019 QUALCOMM Technologies Incorporated, All Rights Reserved
 */
...
static pm_err_flag_type pm_pbs_info_otp_read (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_ROM);
  ...
  err_flag = pm_pbs_disable_access(slave_id, PM_PBS_TYPE_ROM);
...
static pm_err_flag_type pm_pbs_info_ram_read (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM);
  ...
  /* Configuring to disable PBS core read access */
  err_flag = pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM);
...
pm_err_flag_type pm_pbs_info_rom_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    {
      /* Configuring to disable PBS core read access */
      pm_pbs_disable_access(slave_id, PM_PBS_TYPE_ROM);
...
pm_err_flag_type pm_pbs_info_ram_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    {
      /* Configuring to disable PBS core read access */
      pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM);
...
pm_err_flag_type pm_pbs_header_version_info(uint8 slave_id, pm_pbs_header_info_type *pbs_header_ptr)
{
	...
	err_flag |=  pm_pbs_mem_read(slave_id, pm_pbs_data->pbs_ram_start_addr + header_start_address, read_data, 2); 
...
pm_err_flag_type pm_pbs_header_dataset_info(uint8 slave_id, uint32 start_address, pm_pbs_header_dataset_info_type *pbs_header_dataset_ptr, pm_pbs_header_version_type pbs_header_version)
{
    ...
    err_flag |=  pm_pbs_mem_read(slave_id, pm_pbs_data->pbs_ram_start_addr + start_address, read_data, PM_PBS_HEADER_SIZE); 
    if (err_flag != PM_ERR_FLAG__SUCCESS)
    {
        return err_flag;
    }
    ...
    {
          pbs_header_dataset_ptr->sleep_dataset_address = ((read_data[sleep_data_set_index] & PM_PBS_DATASET_START_ADDRESS_MASK )) - (uint32)pm_pbs_data->pbs_ram_start_addr;
        }
    }
	 ...
	 if(pon_reason_dataset_index !=  PBS_DATASET_INVALID_INDEX)
    {
        ...
        else    
        {
          ...
          pbs_header_dataset_ptr->pon_reason_dataset_address = ((read_data[pon_reason_dataset_index] & PM_PBS_DATASET_START_ADDRESS_MASK)) - (uint32)pm_pbs_data->pbs_ram_start_addr ;
	...
	{
          pbs_header_dataset_ptr->rtrr_dataset_address = ((read_data[rtrr_dataset_index] & PM_PBS_DATASET_START_ADDRESS_MASK)) - (uint32)pm_pbs_data->pbs_ram_start_addr ;
        }
	}
        ...
        if(read_data[spare_reg_dataset_index] == PM_PBS_DATASET_INVALID_START_ADDRESS)
	{
	   ...
	   {
          pbs_header_dataset_ptr->spare_reg_dataset_address = ((read_data[spare_reg_dataset_index] & PM_PBS_DATASET_START_ADDRESS_MASK)) - (uint32)pm_pbs_data->pbs_ram_start_addr;
        }
	   }
    ...
    if(lpg_dataset_index !=  PBS_DATASET_INVALID_INDEX)
    {
	   ...
	   {
          pbs_header_dataset_ptr->lpg_dataset_address = ((read_data[lpg_dataset_index] & PM_PBS_DATASET_START_ADDRESS_MASK)) - (uint32)pm_pbs_data->pbs_ram_start_addr;
	   }	    
	}
...

{
    ...
    switch(dataset_type)
    {
      case PM_PBS_HEADER_LPG_DATASET:
           ...
           dataset_size  = PM_PBS_LPG_DATASET_SIZE_V1;
      ...
      case PM_PBS_HEADER_SPARE_REG_DATASET:
           ...
           dataset_size  = PM_PBS_SPARE_REG_DATASET_SIZE_V1;
      ...
      case PM_PBS_HEADER_PON_REASON:
           ...
           dataset_size  = PM_PBS_PON_DATASET_SIZE_V1;
      ...
      case PM_PBS_HEADER_SLEEP_DATASET:
           ...
           dataset_size  = PM_PBS_SLEEP_DATASET_SIZE_V1;
    ...
    err_flag = pm_pbs_config_access(slave_id, start_address , PM_PBS_ACCESS_READ_BURST ,PM_PBS_TYPE_RAM);
    if (err_flag != PM_ERR_FLAG__SUCCESS)
    {
        return err_flag;
    }
...

{
    ...
    err_flag = pm_pbs_config_access(slave_id, start_address , PM_PBS_ACCESS_WRITE_BURST,PM_PBS_TYPE_RAM);
    if (err_flag != PM_ERR_FLAG__SUCCESS)
    {
        return err_flag;
    }
...
pm_err_flag_type pm_pbs_ram_dataset_read(uint32 pmic_index, uint32* dataset_ptr, uint32 dataset_ptr_size, pm_pbs_header_dataset_type dataset_type)
{
	...
	/* Retrieve PBS info config data */
    pm_pbs_temp_data = (pm_pbs_info_data_type**)pm_target_information_get_specific_info(PM_PROP_PBS_INFO);
    if (dataset_ptr == NULL || dataset_type >= PM_PBS_HEADER_INVALID || pm_pbs_temp_data == NULL)
    {
       return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
    }
	...
	switch(dataset_type)
    {
      ...
      case PM_PBS_HEADER_PON_REASON_DATASET:
           //if PON RSNs are stored in SDAM , byte 3,2 will be 0x00.
    ...
    if(start_address != 0xFFFF)
    {
        ...
        err_flag |= pm_get_slave_id(pmic_index, 0, &slave_id);
        err_flag |= pm_pbs_mem_read(slave_id, pm_pbs_temp_data[pmic_index]->pbs_ram_start_addr + start_address, dataset_ptr, dataset_size);
...
pm_err_flag_type pm_pbs_ram_dataset_write(uint32 pmic_index, uint32* dataset_ptr, uint32 dataset_ptr_size, pm_pbs_header_dataset_type dataset_type)
{
    ...
    if (dataset_ptr == NULL || dataset_ptr_size == 0 || pm_pbs_temp_data == NULL)
	{
       return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
	}
    ...
    err_flag |= pm_get_slave_id(pmic_index, 0, &slave_id);
    err_flag |= pm_pbs_mem_write(slave_id, pm_pbs_temp_data[pmic_index]->pbs_ram_start_addr + start_address, (uint32*)local_dataset_ptr, data_size);
...
pm_err_flag_type pm_pbs_mem_read(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    if(data_ptr == NULL || data_size == 0 || pm_pbs_temp_data == NULL)
    {
       return PM_ERR_FLAG__FEATURE_NOT_SUPPORTED;
    }
               
    //check PBS RAM read address is within range
    if((start_address + data_size) > (pm_pbs_temp_data[pmic_index]->pbs_ram_start_addr + (4 * (pm_pbs_temp_data[pmic_index]->pbs_ram_mem_size))))
    {
...
pm_err_flag_type pm_pbs_mem_write(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    if((start_address + data_size) > (pm_pbs_temp_data[pmic_index]->pbs_ram_start_addr + (4 * (pm_pbs_temp_data[pmic_index]->pbs_ram_mem_size - PM_PBS_MEMORY_VER_LINE_FROM_END))) ||
	    ((start_address) < (pm_pbs_temp_data[pmic_index]->pbs_ram_start_addr)) )
    {
       return PM_ERR_FLAG__INVALID_PARAMETER;
    }
