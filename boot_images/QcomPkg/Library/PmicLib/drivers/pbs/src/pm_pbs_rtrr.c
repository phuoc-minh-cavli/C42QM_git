...
$Header: //components/rel/boot.xf/0.2/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_rtrr.c#2 $ 
$DateTime: 2018/11/08 00:51:28 $  $Author: pwbldsvc $
...
pm_pbs_rtrr_ram_dump(uint32 slave_id, pm_pbs_rtrr_dump_data_type *pm_pbs_rtrr_data)
{
    ...
    err_flag = pm_pbs_config_access(slave_id, pbs_ram_rtrr_dataset_start_addr_placeholder, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM);
    ...
    err_flag = pm_pbs_config_access(slave_id, rtrr_dataset_start_addr , PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM);
	if (err_flag != PM_ERR_FLAG__SUCCESS)
	{
		return err_flag;
	}
