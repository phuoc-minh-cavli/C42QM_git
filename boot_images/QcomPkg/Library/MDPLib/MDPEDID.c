...
XML_TagDefType sXmlTagsList[] = {
    ...
    XML_CREATE_TAG_ENTRY("DSIFBCEnable", XML_TAG_DATATYPE_BOOLEAN, gPanelConfig.uAttrs.sDsi.bFBCEnable),
    XML_CREATE_TAG_ENTRY("DSIFBCProfileID", XML_TAG_DATATYPE_INTEGER, gPanelConfig.uAttrs.sDsi.uFBCProfileID),
    ...
    else if (XML_PARSER_STATUS_OK == XML_Parser(sPlatformParams.sPlatformPanel.pPanelXMLConfig, sPlatformParams.sPlatformPanel.uConfigSize, sXmlTagsList, XML_TAGSLIST_LENGTH))
    {
        ...
        if (0 != pPanelInfo->uAttrs.sDsi.sI2CConfig.uI2CNumOfSlaves)
        {
          ...
          pPanelInfo->uAttrs.sDsi.sI2CConfig.pI2CTermSequence = gpI2CTermSequenceBuffer;
            ...
            if( TRUE == pPanelInfo->uAttrs.sDsi.bFBCEnable)
            {
                if (pPanelInfo->uAttrs.sDsi.uFBCProfileID < MDP_FBC_PROFILEID_MAX)
                {
                    pPanelInfo->uAttrs.sDsi.uFBCCompressionRatio = gFbcProfileModes[pPanelInfo->uAttrs.sDsi.uFBCProfileID].uCompressionRatio;
                }
                else
                {
                    eStatus = MDP_STATUS_BAD_PARAM;   
