...
const uint32 gMDP_FlushFlags[MDP_DISPLAY_MAX][MDP_DUALPIPE_NUM_MIXERS] =
{
  // MDP_DISPLAY_PRIMARY
  {
    (HAL_CONTROL_PATH_ATTACH_DETACH | HAL_CONTROL_PATH_FLUSH_VIG0 | HAL_CONTROL_PATH_FLUSH_LM0 | HAL_CONTROL_PATH_FLUSH_DSPP0),
        ...
        (pPanelConfig->uNumMixers))
    {
            ...
            (MDP_DUALPIPE_NUM_MIXERS   == pPanelConfig->uNumInterfaces))
        {
              ...
              ( MDP_PINGPONG_CAP_PINGPONG_SPLIT & pPingPongCaps->uPPFeatureFlags))
            {
                ...
                {
                    MDP_Log_Message(MDP_LOGLEVEL_WARN, "SplitDisplayInit() failed to configure PP Split! (PingPong ID = %d)\n", ePingPongId);
                    eStatus = MDP_STATUS_FAILED;
                }
            }
            else
            {
                MDP_Log_Message(MDP_LOGLEVEL_WARN, "SplitDisplayInit() failed: HW does not support ping-pong split (PingPong ID = %d)\n", ePingPongId);
...
MDP_Status  MDPStartPipe(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  else
  {
    ...
    if (MDP_STATUS_OK==eStatus)
    {
      ...
      {
        MDP_OSAL_MEMZERO(&sFlushType, sizeof(HAL_MDP_ControlPathFlushType));
        sFlushType.uFlushModules = HAL_CONTROL_PATH_FLUSH_TIMING_1;
...
#if 0 // sravan todo check: is anyone using this ? 
