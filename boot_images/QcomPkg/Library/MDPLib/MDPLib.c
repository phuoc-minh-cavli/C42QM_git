...
static const sMDSS_BaseAddressMappings asMDSS_BaseAddressMappings[MDSS_BASEADDRESSMAPINGS_MAX] = {  
    ...
    {EFICHIPINFO_FAMILY_QCS405,  0x01A00000},  //for QCS405(ViperTooth) family 
};
...
MDP_Status  MDPInit(MDP_InitParamsType *pMDPInitParams, uint32 uFlags)
{
    ...
    else
    {
        ...
        if (FALSE == bSWRender)
        {
          ...
          {
            if (HAL_MDSS_STATUS_SUCCESS != HAL_MDP_Init(DEAFAULT_MDP_INIT_FLAGS))
            {
            ...
            else if (HAL_MDSS_STATUS_SUCCESS != HAL_MDP_TrafficCtrl_Init(0))
            {
            ...
            else
            {
              ...
              /* TODO: commenting for BU now, need rework on this while enabling Cont Splash.
...
static bool32 MDPPlatformGetDisableDisplay(void)
{
  ...
  if (MDP_STATUS_OK == MDP_GetEnvironmentVariable(L"DISABLEDISPLAY", &uValue, sizeof(uValue)))
  {
    if (uValue)
    {
      bDisableDisp = TRUE;
...
static MDP_Status MDPPlatformSetMdssBase(EFIChipInfoFamilyType  sEFIChipSetFamily)
{
  ...
  {
    MDP_Log_Message (MDP_LOGLEVEL_WARN, "MDPLib:HAL_MDSS_SetBaseAddress - Chipset Family 0x%x not found , taking default base address !\n", sEFIChipSetFamily);
  ...
  {
    MDP_Log_Message (MDP_LOGLEVEL_WARN, "MDPLib: HAL_MDSS_SetBaseAddress() failed!\n");	
