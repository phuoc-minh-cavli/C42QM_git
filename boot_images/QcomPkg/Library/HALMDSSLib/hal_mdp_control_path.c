                                             ...
                                             uint32                                uFlags)
{
   ...
   uint32                  uMixerId             = (pControlPathMixerConfig->eMixerId - 1); // Count starts from 1

   uRegOffset  = HAL_MDP_Get_ControlPath_BaseOffset(eControlPath);

   uRegValue      = in_dword((HWIO_MMSS_MDP_CTL_0_LAYER_0_ADDR + uRegOffset + (uMixerId << 2)));
   uRegValueEx    = in_dword((HWIO_MMSS_MDP_CTL_0_LAYER_0_EXT_ADDR + uRegOffset + (uMixerId << 2)));
   ...
   if(NULL != pControlPathMixerConfig->psMixerZOrderConfig)
   {
          ...
          uBlendOrderIndex++  )
      {
         ...
         switch(pControlPathMixerConfig->psMixerZOrderConfig->eSourcePipe[uBlendOrderIndex])
         {
   ...
   out_dword((HWIO_MMSS_MDP_CTL_0_LAYER_0_ADDR + uRegOffset + (uMixerId << 2)), uRegValue);
   out_dword((HWIO_MMSS_MDP_CTL_0_LAYER_0_EXT_ADDR + uRegOffset + (uMixerId << 2)), uRegValueEx);
}
