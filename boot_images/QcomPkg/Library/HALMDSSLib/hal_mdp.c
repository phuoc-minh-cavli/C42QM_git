 ...
 *    [  0xFFB5,          0xFF6B,              0x00E0   ]
 *    [  0x00E0,          0xFF44,              0xFFDC   ]
 *
 * Matrix Value Packing
 *
 */
uint32   MDP_CSC_RgbToYuv_Matrix_Limited_Rec601[] =
{
   ...
   0xFFB50032,                // Hi-WORD Co-eff R2C1 - Low-WORD Co_eff R1C3
   0x00E0FF6B,                // Hi-WORD Co-eff R2C3 - Low-WORD Co_eff R2C2
   0xFF4400E0,                // Hi-WORD Co-eff R3C2 - Low-WORD Co_eff R3C1
   0x0000FFDC                 // Hi-WORD INVALID     - Low-WORD Co_eff R3C3
};
...
uintPtr  uMDPLayerRegBaseOffset[HAL_MDP_SOURCE_PIPE_MAX];                        // Source (SSPP) Layers (Scalar) register offsets
...
{
   // Use MDSS_MDP V1.0.0 as the base info
   HAL_MDP_ReadHardwareInfo_1_0_0(psHwInfo, psMdpHwInfo);

   // HW differences between MDP_1.0.0 and MDP_1.16.0 , MDP_1.17.0
   if ((0x01 == psVersionInfo->uMajorVersion) &&  
            (0x10 == psVersionInfo->uMinorVersion ||
             0x11 == psVersionInfo->uMinorVersion)) 
   { 
      HAL_MDP_ReadHardwareInfo_1_16_0(psHwInfo, psMdpHwInfo); 
   } 
...
* \param [in]   pIntrReg              - Interupt Register
...
*
* \param [out]  psInterruptStatus     - Interrupt status
...
* \param [out]  pHistIntrReg          - Pointer to histogram interrupt register
*
* \retval NONE
*
****************************************************************************/
                                                         ...
                                                         uint32                       uMDPHistIntrRegValue)
{
   ...
   case HAL_MDP_MODULE_SOURCE_PIPE:
      {
         HAL_MDP_SourcePipe_Interrupt_GetStatus(psIntrStatus, uMDPIntrRegValue, uMDPHistIntrRegValue);
      }
      break;

   case HAL_MDP_MODULE_DESTINATION_PIPE:
      {
         HAL_MDP_DestinationPipe_Interrupt_GetStatus(psIntrStatus, uMDPIntrRegValue, uMDPHistIntrRegValue);
      }
      break;
   ...
   case HAL_MDP_MODULE_PINGPONG:
      {
         HAL_MDP_PingPong_Interrupt_GetStatus(psIntrStatus, psIntrStatus->sIntrConfig.eModuleId.ePingPongId, uMDPIntrRegValue, uMDPHistIntrRegValue);
      }
      break;
...
*
****************************************************************************/
HAL_MDSS_ErrorType HAL_MDP_Init(uint32    uInitFlags)
{
   HAL_MDSS_ErrorType   eStatus        = HAL_MDSS_STATUS_SUCCESS;
   ...
   MDP_OSAL_MEMZERO(&gsMdpHwInfo, sizeof(HAL_MDP_Hw_Private_Info));
   ...
   // HW recommended programming to avoid underflow recovery lockup when multiple operations are active
   out_dword(HWIO_MMSS_MDP_VIDEO_INTF_UNDERFLOW_CTL_ADDR, 0x00);

   if(0x00 != (uInitFlags & HAL_MDP_INIT_FLAGS_TRFCTRL_CONFIGURATION))
   {
      HAL_MDP_TrafficCtrl_Init(0);
   }

   // Disable auto recovery mechanism, workaround for underflow case hang
   HWIO_MMSS_MDP_VIDEO_INTF_UNDERFLOW_CTL_OUT(0);
...
#if 0
      // RGB SSPP
      uHWBlockIndex  = HAL_MDP_SOURCE_PIPE_RGB_0;
      for(uIndex = 0; uIndex < HAL_MDP_GET_RGB_LAYER_COUNT(); uIndex++, uHWBlockIndex++)
      {
         HAL_MDP_SSPP_Init((HAL_MDP_SourcePipeId)uHWBlockIndex, HAL_MDP_LAYER_TYPE_RGB, HAL_MDP_RGB_LAYER_DEFAULT_FEATURE_INIT, 0x00);
      }

      // DMA SSPP
      uHWBlockIndex  = HAL_MDP_SOURCE_PIPE_DMA_0;
      for(uIndex = 0; uIndex < HAL_MDP_GET_DMA_LAYER_COUNT(); uIndex++, uHWBlockIndex++)
      {
         HAL_MDP_SSPP_Init((HAL_MDP_SourcePipeId)uHWBlockIndex, HAL_MDP_LAYER_TYPE_DMA, HAL_MDP_DMA_LAYER_DEFAULT_FEATURE_INIT, 0x00);
      }

      // Layer Mixer
      uHWBlockIndex  = HAL_MDP_LAYER_MIXER_0;
      for(uIndex = 0; uIndex < HAL_MDP_GET_LM_COUNT(); uIndex++, uHWBlockIndex++)
      {
         HAL_MDP_LayerMixer_Init((HAL_MDP_LayerMixerId)uHWBlockIndex, HAL_MDP_LM_DEFAULT_FEATURE_INIT, 0x00);
      }

      // DSPP
      uHWBlockIndex  = HAL_MDP_DESTINATION_PIPE_0;
      for(uIndex = 0; uIndex < HAL_MDP_GET_DSPP_COUNT(); uIndex++, uHWBlockIndex++)
      {
         HAL_MDP_DSPP_Init((HAL_MDP_DestinationPipeId)uHWBlockIndex, HAL_MDP_DSPP_DEFAULT_FEATURE_INIT, 0x00);
      }
#endif
   }
   return eStatus;
                                            ...
                                            uint32                        uEnableFlags)
{
   ...
   HWIO_MMSS_MDP_INTR_EN_OUT(uMDPIntrBits);
   HWIO_MMSS_MDP_HIST_INTR_EN_OUT(uMDPHistIntrBits);
...
*
* \param [in/out] puIntrStatusValue             - Interrupt status value
* \param [in]     uFlags                        - Reserved
*
* \retval HAL_MDSS_ErrorType
*
