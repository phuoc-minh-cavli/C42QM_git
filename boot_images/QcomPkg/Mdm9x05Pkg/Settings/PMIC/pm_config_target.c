...
--------   ---     ----------------------------------------------------------
05/04/18   sv      Added support for SMB1351.
...
uint32 num_of_lpg_chan[]   = {2, 0};
uint32 num_of_pwron[]      = {1, 0};
uint32 num_of_misc[]       = {1, 0};
uint32 num_of_rgb[]        = {0, 0}; /* TRI_LED */
uint32 num_of_wled[]       = {0, 0};
uint32 num_of_schg[]       = {0, 0};
uint32 num_of_coincell[]   = {0, 0};
uint32 num_of_fg[]         = {0, 0};
uint32 num_of_ibb[]        = {0, 0};
uint32 num_of_lab[]        = {0, 0};
uint32 num_of_vib[]        = {0, 0};
uint32 num_of_pbs_client[] = {2, 0};
uint32 num_of_haptics[]    = {0, 0};
...
{
   {
      ...
      {
         32,                              // s3 timer value  (max value: 128 sec)
...
sbl_smb231_specific_data[1] =
{
   {
      //Configuration Value,                             Enable config
      {SMB_LOW_BATT_THRESHOLD_2P9V,                      PM_ENABLE_CONFIG },  //Vlowbatt Threshold
      {70,                                               PM_DISABLE_CONFIG },  //Fg skin hot threshold: valid range is -30 to 97 degree C
      {80,                                               PM_DISABLE_CONFIG },  //Fg skin too hot threshold: valid range is -30 to 97 degree C
      {80,                                               PM_DISABLE_CONFIG },  //Fg charge hot threshold: valid range is -30 to 97 degree C
      {90,                                               PM_DISABLE_CONFIG },  //Fg charge too hot threshold: valid range is -30 to 97 degree C
      {TRUE,                                             PM_DISABLE_CONFIG },  //Use BATID and/or THERM pin for battery missing detection
      {1000,                                             PM_DISABLE_CONFIG },  //FAST Charging Current
      {250,                                              PM_DISABLE_CONFIG },  //PRE Charge Current
      {4200,                                             PM_DISABLE_CONFIG },  //Float Voltage
...
pm_pbs_info_data_type pm_pbs_info_data_a =  //Need to review with systems somu
{
    0x0,                /* PBS OTP MEMORY Start Address */
    256,                /* PBS OTP MEMORY Size */
    0x800,              /* PBS RAM MEMORY Start Address */
    1024,                /* PBS RAM MEMORY Size */
    PM_PBS_INFO_IN_OTP, /* Place where PBS Info stored */

};

pm_pbs_info_data_type pm_pbs_info_data_b =  //Need to review with systems somu
{
    0x0,                /* PBS OTP MEMORY Start Address */
    0,                /* PBS OTP MEMORY Size */
    0x0,              /* PBS RAM MEMORY Start Address */
...
#ifdef KEY_PRESS_DBNC
pm_pwrkey_dbnc_chk_type pm_pon_pwrkey_dbnc_chk[] = {{PM_PON_PWRKEY_DBNC_CHK_AT_LOADER, 5000}} ; //check power key press at, time in milli sec
#else
pm_pwrkey_dbnc_chk_type pm_pon_pwrkey_dbnc_chk[] = {{PM_PON_PWRKEY_DBNC_CHK_AT_CORE, 500}} ;
#endif
