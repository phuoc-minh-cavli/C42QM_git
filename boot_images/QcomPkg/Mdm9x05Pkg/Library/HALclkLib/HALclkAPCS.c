...
)
{
  ...
  nSel = (nVal &  HWIO_FMSK(APCS_ALIAS1_CFG_RCGR, SRC_SEL))
               >> HWIO_SHFT(APCS_ALIAS1_CFG_RCGR, SRC_SEL);
  ...
  pmConfig->nDiv2x = ((nVal &  HWIO_FMSK(APCS_ALIAS1_CFG_RCGR, SRC_DIV))
                            >> HWIO_SHFT(APCS_ALIAS1_CFG_RCGR, SRC_DIV)) + 1;
  pmConfig->nM = 0;
  pmConfig->nN = 0;
  pmConfig->n2D = 0;
...
)
{
  ...
  nCfgCGRVal &= ~(HAL_CLK_CFG_CGR_SRC_SEL_FMSK |
                  ...
                  //HAL_CLK_CFG_CGR_MODE_FMSK); //this field is not present in the APCS_ALIAS1_CFG_RCGR ?
