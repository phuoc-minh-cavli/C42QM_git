/*=============================================================================
  ...
  Copyright (c) 2011-2018, 2019 by Qualcomm Technologies, Inc. All Rights Reserved.
=============================================================================*/
...
*/
#define CDP_DISP_RESET_GPIO               39      //GPIO number for LCD0_RESET_N
#define CDP_DISP_TE_GPIO                  40      //GPIO number for TE
#define CDP_DISP_BACKLIGHT_GPIO           48      //GPIO number for Backlight //BU: TODO: Check how to enable backlight
...
/******************************************************************
 *           hx8394d video 720p (720 x 1280) Video mode                   *
 *                                                                *
 ******************************************************************/
const static int8 HX8394D_720P_video_xmldata[] =
"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
"<PanelName>HX8394D</PanelName>"
"<PanelDescription>hx8394d 720p video mode dsi panel</PanelDescription>"
"<Group id=\"Active Timing\">"
" <HorizontalActive units=\"Dot Clocks\">720</HorizontalActive>"
" <HorizontalFrontPorch units=\"Dot Clocks\">52</HorizontalFrontPorch>"
" <HorizontalBackPorch units=\"Dot Clocks\">100</HorizontalBackPorch>"
" <HorizontalSyncPulse units=\"Dot Clocks\">24</HorizontalSyncPulse>"
...
" 39 b9 ff 83 94\n"
" 39 ba 33 83\n"
" 39 b1 6c 12 12 37 04 11 f1 80 ec 94 23 80 c0 d2 18\n"
" 39 b2 00 64 0e 0d 32 23 08 08 1c 4d 00\n"
" 39 b4 00 ff 03 50 03 50 03 50 01 6a 01 6a\n"
" 39 bc 07\n"
" 39 bf 41 0e 01\n"
" 39 d3 00 07 00 00 00 10 00 32 10 05 00 00 32 10 00 00 00 32 10 00 00 00 36 03 09 09 37 00 00 37\n"
" 39 d5 02 03 00 01 06 07 04 05 20 21 22 23 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 24 25 18 18 19 19\n"
" 39 d6 05 04 07 06 01 00 03 02 23 22 21 20 18 18 18 18 18 18 58 58 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 18 25 24 19 19 18 18\n"
" 39 cc 09\n"
" 39 c0 30 14\n"
" 39 c7 00 c0 40 c0\n"
" 39 b6 43 43\n"
...
const PanelInfoType gPanelList[] =
{
  ...
  /* Vipertooth HX8394D 720P video panel */
  PANEL_CREATE_ENTRY("hx8394d_720P_video",         MDPPLATFORM_PANEL_HX8394D_720P_VIDEO,          "qcom,mdss_dsi_hx8394d_720p_video",            DISP_INTF_DSI, HX8394D_720P_video_xmldata,            DISP_TOPOLOGY_CONFIG_0,    PLL_OVERRIDE_NONE, DISP_MODE_SINGLE_DSI,                              DISP_MODE_SINGLE_DSI,                            DISP_MODE_SINGLE_DSI),
...
static MDP_Status Panel_CDP_PeripheralPower(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams, bool32 bPowerUp)
{
  ...
  if (EFI_SUCCESS != gBS->LocateProtocol(&gEfiTLMMProtocolGuid, NULL, (void **)&TLMMProtocol))
  {
    ...
    DEBUG((EFI_D_ERROR, "Sravank DisplayDxe: Configure Backlight GPIO %d !\n", CDP_DISP_BACKLIGHT_GPIO));
    if (EFI_SUCCESS != TLMMProtocol->ConfigGpio((UINT32)EFI_GPIO_CFG(CDP_DISP_BACKLIGHT_GPIO, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA), TLMM_GPIO_ENABLE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Configure GPIO %d for Reset_N line Failed!\n", CDP_DISP_BACKLIGHT_GPIO));
	  Status = MDP_STATUS_FAILED;
    }
    ...
    /* Set Reset_N line HIGH */
    else if (EFI_SUCCESS != TLMMProtocol->GpioOut((UINT32)EFI_GPIO_CFG(CDP_DISP_BACKLIGHT_GPIO, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), GPIO_HIGH_VALUE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Reset_N line HIGH failed!\n"));
	  Status = MDP_STATUS_FAILED;
    }
...
/*   // QCS405: DSIDriver_Write not available, to be removed
  ...
  else
  {
    ...
    case MDPPLATFORM_CONFIG_SW_RENDERER:
      ...
      else
      {
		...
		if (EFI_PLATFORMINFO_TYPE_IOT == sPlatformInfo.sEFIPlatformType.platform) 
		{
		  uPlatformSubtype      = sPlatformInfo.sEFIPlatformType.subtype         & 0xFF;
          uHardwareVersionMajor = (sPlatformInfo.sEFIPlatformType.version >> 16) & 0xFF;
          if ( (uPlatformSubtype == 2 && uHardwareVersionMajor == 1) || 
            (uPlatformSubtype == 0 && uHardwareVersionMajor == 1) ||
            (uPlatformSubtype == 1 && uHardwareVersionMajor == 2) ) 
          {
            /* For these variants DSI Display HW is available. */
        ...
        else
        {
          ...
          case MDP_DISPLAY_PRIMARY:
            ...
            else
            {
			  ...
			  case EFI_PLATFORMINFO_TYPE_QRD:
              case EFI_PLATFORMINFO_TYPE_IOT:
                /* Truly 1080p with NT35695B - Video Mode */
                pPlatformParams->sPlatformPanel.pPanelXMLConfig = (int8 *)HX8394D_720P_video_xmldata;
                pPlatformParams->sPlatformPanel.uConfigSize     = sizeof(HX8394D_720P_video_xmldata);
                pPlatformParams->sPlatformPanel.eSelectedPanel  = MDPPLATFORM_PANEL_HX8394D_720P_VIDEO;
                break;
              default:
                pPlatformParams->sPlatformPanel.pPanelXMLConfig    = (int8 *)dummy_xmldata;
                pPlatformParams->sPlatformPanel.uConfigSize        = sizeof(dummy_xmldata);
		  ...
		  case EFI_PLATFORMINFO_TYPE_IOT:
            ...
            else
            {
              ...
              if ((0 == PmicDeviceInfo.PmicVariantRevision) ||
				  ((2 == PmicDeviceInfo.PmicVariantRevision) &&
    			   (2 == PmicDeviceInfo.PmicAllLayerRevision)))
                {
