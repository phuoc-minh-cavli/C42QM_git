  ...
  )
{
  ...
  switch (CoreType)
  {
    ...
    default:
      ...
      *CoreNum = (QCOM_USB_CORE_NUM) USB_CORE_MAX_NUM_QCS_405;
    break;
  }
  ...
  )
{
  ...
  if (CoreNum >= USB_CORE_MAX_NUM_QCS_405) {
    Status = EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  if (gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].PeripheralHandle != NULL)
  {
  ...
  gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].UsbConfig = gUsbConfigTemplate;
  gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].UsbConfig.coreNum = (QCOM_USB_CORE_NUM)USB_CORE_MAX_NUM_QCS_405;
  gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].UsbConfig.modeType = USB_INVALID_MODE;
  gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].HostHandle = NULL;
  gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].PeripheralHandle = NULL;
  // Install the protocol on PeripheralHandle, this is used as an indicator that
  // the UsbConfigInit has been called.
  Status = gBS->InstallMultipleProtocolInterfaces(&gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].PeripheralHandle,
    &gQcomUsbConfigProtocolGuid, &gUsbConfigDevice[USB_CORE_MAX_NUM_QCS_405].UsbConfig,
    NULL);
  ...
  for (Index = 0; Index < USB_CORE_MAX_NUM_QCS_405; Index++) {
  ...
  if (!FeaturePcdGet(HostModeSupported)) {
    gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.Enabled = TRUE;
    gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.UsbMode = USB_DEVICE_MODE_SS;
  }
  else {
    ...
    Status = UsbPortGetRtConnectionMode(gUsbConfigDevice[USB_CORE_0_QCS_405].UsbPortHandle, &PortMode);
    ERR_CHK(" Cannot Get Connection Mode for Core %d: %r", USB_CORE_0_QCS_405, Status);
    if (PortMode == USB_TYPEC_PORT_MODE_DFP) {
      DBG(EFI_D_WARN, "Enable Host Mode");
      gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.Enabled = TRUE;
      gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.UsbMode = USB_HOST_MODE_XHCI;
      gUsbConfigDevice[USB_CORE_0_QCS_405].UsbCoreInterface->ModeType = USB_HOST_MODE;
    }
    else {
  ...
  )
{
  ...
  UsbCoreIfc = gUsbConfigDevice[USB_CORE_0_QCS_405].UsbCoreInterface;
  ...
  if (!FeaturePcdGet(HostModeSupported)) {
	...
	ERR_CHK("NOT EXPECTED TO REACH HERE IN STARLORD: %r", Status);
    if (gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.Enabled == TRUE &&
        gUsbConfigDevice[USB_CORE_0_QCS_405].EnableStaticCfg.UsbMode == USB_HOST_MODE_XHCI) {
  ...
  if (Message == UsbCfgPortMsgPartnerAttach) { //Attach
    ...
    if (IS_HARDWARE_CONFIUGRED(gUsbConfigDevice[CoreNum].UsbConfig.modeType)) {
      ...
      { 
        ...
        //}
        if(FeaturePcdGet(UsbNoPmic) || EFI_ERROR(Status)) { //Either SMB is not present or could not get charger type
          ChargerType = EFI_PM_USB_CHG_PORT_SDP;
        }
