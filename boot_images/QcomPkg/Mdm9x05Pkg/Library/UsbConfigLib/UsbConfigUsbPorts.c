  ...
  )
{
  ...
  EFI_STATUS  				    Status     = EFI_SUCCESS;
  EFI_PM_USB_CONNECTOR_TYPE    UsbConnectorType = EFI_PM_USB_CONNECTOR_MICRO_USB;
  ...
  *Message = UsbCfgPortMsgNone;
  ...
  Status = gPmicUsbProtocol->GetUSBConnectorType(&UsbConnectorType);
  //ERR_CHK("Failed to get UsbCGetPortState with error: %r", Status);
  DBG(EFI_D_INFO, "USB Connector Type: %d", UsbConnectorType);
  
  if(EFI_PM_USB_CONNECTOR_TYPEC == UsbConnectorType)
  {
    ERR_CHK("Should not be Type-C in VT");
#if 0
  ...
  else if(EFI_PM_USB_CONNECTOR_MICRO_USB == UsbConnectorType)
  {
  ...
  DBG(EFI_D_WARN, "ConnectState change event: %d", UsbPort->ConnectState);
