...
/*
 * USB 2.0 Clock definitions
 */

#define USB2_SYSTEM_CLK_MIN_FREQ_HZ           FREQ_MHZ(80)

// Mock CLK needs to be set at 19.2 Mhz for Gandalf
#define USB2_MOCK_CLK_MIN_FREQ_HZ             FREQ_KHZ(19200)
#define USB2_HS_PHY_MIN_FREQ_HZ               FREQ_MHZ(100)
...
QCOM_USB_CLOCK Usb30EnCoreClocks[] =
{
  ...
  {"gcc_usb_hs_system_clk",              USB2_SYSTEM_CLK_MIN_FREQ_HZ,   1 },
  {"gcc_usb_hs_inactivity_timers_clk",   0,                             1 },
  {"gcc_usb20_mock_utmi_clk",            USB2_MOCK_CLK_MIN_FREQ_HZ,     1 },
  {"gcc_usb_hs_phy_cfg_ahb_clk",         USB2_HS_PHY_MIN_FREQ_HZ,       1 }
};
...
)
{
  ...
  {
    HAL_USB_OR (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_USB_HS_BCR_OFFS, HWIO_GCC_USB_HS_BCR_BLK_ARES_BMSK);
    HAL_USB_OR (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_QUSB2_PHY_BCR_OFFS, HWIO_GCC_QUSB2_PHY_BCR_BLK_ARES_BMSK);
    ...
    HAL_USB_AND (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_QUSB2_PHY_BCR_OFFS, ~HWIO_GCC_QUSB2_PHY_BCR_BLK_ARES_BMSK);
    HAL_USB_AND (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_USB_HS_BCR_OFFS, ~HWIO_GCC_USB_HS_BCR_BLK_ARES_BMSK);
    gBS->Stall (100);
  }
  ...
  )
{
  ...
  // QCS405 RUMI PHY init sequence. 
  if (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_RUMI) {
    if (FALSE == usb_shared_hs_phy_rumi_init(0, 0)) {
      Status = EFI_INVALID_PARAMETER;
    }
  ...
  )
{
  ...
  */
  HAL_USB_OR (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_MDSS_GDSCR_OFFS, HWIO_GCC_MDSS_GDSCR_SW_COLLAPSE_BMSK);
  gBS->Stall (100);
  HAL_USB_AND (GCC_CLK_CTL_REG_REG_BASE, HWIO_GCC_MDSS_GDSCR_OFFS, ~HWIO_GCC_MDSS_GDSCR_SW_COLLAPSE_BMSK);
  gBS->Stall (100);
#endif
  ...
  //HAL_USB_WRITE (TOP_AHB2PHY_WEST_BASE, HWIO_TOP_AHB2PHY_TOP_CFG_ADDR(TOP_AHB2PHY_SWMAN_TOP_REG_BASE_OFFS), 0x11);
  ...
  */
  if ((((UsbCore->CoreType == USB_CONFIG_SSUSB1) && bSsPhyFailed)) || (FeaturePcdGet(HighSpeedOnlySupported)))
  {
    ...
    HAL_USB_OR (BaseAddr, HWIO_USB2_GENERAL_CFG_ADDR(USB2_USB30_QSCRATCH_REG_BASE_OFFS), HWIO_USB2_GENERAL_CFG_PIPE_UTMI_CLK_DIS_BMSK);
    gBS->Stall (MICRO_STALL_UNITS(10));
    HAL_USB_OR (BaseAddr, HWIO_USB2_GENERAL_CFG_ADDR(USB2_USB30_QSCRATCH_REG_BASE_OFFS), HWIO_USB2_GENERAL_CFG_PIPE_UTMI_CLK_SEL_BMSK);
    HAL_USB_OR (BaseAddr, HWIO_USB2_GENERAL_CFG_ADDR(USB2_USB30_QSCRATCH_REG_BASE_OFFS), HWIO_USB2_GENERAL_CFG_PIPE3_PHYSTATUS_SW_BMSK);
    ...
    HAL_USB_AND (BaseAddr, HWIO_USB2_GENERAL_CFG_ADDR(USB2_USB30_QSCRATCH_REG_BASE_OFFS), ~HWIO_USB2_GENERAL_CFG_PIPE3_SET_PHYSTATUS_SW_BMSK);
    gBS->Stall (MICRO_STALL_UNITS(10));
    HAL_USB_AND (BaseAddr, HWIO_USB2_GENERAL_CFG_ADDR(USB2_USB30_QSCRATCH_REG_BASE_OFFS), ~HWIO_USB2_GENERAL_CFG_PIPE_UTMI_CLK_DIS_BMSK);
  }
  else
  {
    /* Set DELAYP1TRANS to 0 */
    HAL_USB_AND (BaseAddr, HWIO_USB2_GUSB3PIPECTL_REGS_p_GUSB3PIPECTL_OFFS(BaseAddr, 0), ~HWIO_USB2_GUSB3PIPECTL_REGS_p_GUSB3PIPECTL_DELAYP1TRANS_BMSK);
  }
  ...
  */
  HAL_USB_OR (BaseAddr, HWIO_USB2_CGCTL_REG_OFFS, HWIO_USB2_CGCTL_REG_DBM_FSM_EN_BMSK);
  ...
  )
{
  ...
  if (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_RUMI) {
    // Reset Device Core.
    HAL_USB_OR(BaseAddr, HWIO_USB2_DCTL_OFFS, HWIO_USB2_DCTL_CSFTRST_BMSK);
    while (HAL_USB_READ(BaseAddr, HWIO_USB2_DCTL_OFFS) & HWIO_USB2_DCTL_CSFTRST_BMSK) { ; }
  ...
  */
  HAL_USB_OR( BaseAddr, HWIO_USB2_GUSB2PHYCFG_REGS_p_GUSB2PHYCFG_OFFS(BaseAddr, 0), HWIO_USB2_GUSB2PHYCFG_REGS_p_GUSB2PHYCFG_ENBLSLPM_BMSK);
  ...
  )
{
  ...
  if (CoreNum == USB_CORE_0_QCS_405) {
    ...
    *UsbCoreInterface = &UsbCore->CoreInterface;
    ...
    UsbCore->BaseAddr = USB2_USB20S_BASE;
