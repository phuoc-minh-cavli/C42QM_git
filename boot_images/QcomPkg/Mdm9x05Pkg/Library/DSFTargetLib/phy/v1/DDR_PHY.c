...
$Header: //components/rel/boot.xf/0.2/QcomPkg/Mdm9x05Pkg/Library/DSFTargetLib/phy/v1/DDR_PHY.c#3 $
$DateTime: 2018/12/27 04:45:05 $
$Author: pwbldsvc $
================================================================================
...
void HAL_SDRAM_PHY_Init(uint32 base, SDRAM_INTERFACE interface, uint32 clk_speed)
{
  ...
  /* Enable broadcast mode for 2 only DQ PHYs
     Because Twizy has x16 PHY */
  ...
  HAL_SDRAM_PHY_Update_CDC_Config(base, interface, DDR_PHY_INIT_CDC_DELAY, ddr_type);

  /* Wait for 10us */
  HAL_SDRAM_DDR_Wait(10, 0);

  /* Issue manual IOCAL */
  HAL_SDRAM_PHY_Manual_IO_Cal(base, interface, HAL_SDRAM_DDR_Wait, ddr_type);
...
*   None
*/
void HAL_SDRAM_PHY_Manual_IO_Cal(uint32 base, SDRAM_INTERFACE interface, ddr_wait wait, uint32 ddr_type)
{
  uint32 ca_master_offset;
...
*   None
*/
void HAL_SDRAM_PHY_Manual_CDC_Cal(uint32 base, SDRAM_INTERFACE interface, uint32 ddr_type)
{
  uint32 phy_offset;
...
*   None
*/
void HAL_SDRAM_PHY_Enable_Rapid_CDC_Cal(uint32 base, SDRAM_INTERFACE interface, uint32 ddr_type)
{
  uint32 phy_offset;
  uint32 hw_autocal_ena, timer_ena;
  uint32 dq;
...
*   None
*/
void HAL_SDRAM_PHY_Disable_Rapid_CDC_Cal(uint32 base, SDRAM_INTERFACE interface, uint32 ddr_type)
{
  uint32 phy_offset;
  uint32 dq;
  uint32 offset;
...
*   None
*/
void HAL_SDRAM_PHY_Update_CDC_Config(uint32 base, SDRAM_INTERFACE interface, uint32 config, uint32 ddr_type)
{
  uint32 phy_offset;
  uint32 dq;
  uint32 offset;
...
*   None
*/
void HAL_SDRAM_PHY_Update_Drive_Strength(uint32 base, SDRAM_INTERFACE interface, uint32 rout, uint32 pslew, uint32 nslew, uint32 ddr_type)
{
  uint32 phy_offset;
  uint32 dq;
  uint32 offset;
