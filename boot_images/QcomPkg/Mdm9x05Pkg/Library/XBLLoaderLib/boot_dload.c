...
/*=============================================================================
  ...
  $Header: //components/rel/boot.xf/0.2/QcomPkg/Mdm9x05Pkg/Library/XBLLoaderLib/boot_dload.c#2 $
...
boolean boot_dload_entry( void )
{
  ...
  do
  {
    dload_flag = HWIO_TCSR_BOOT_MISC_DETECT_INM(SBL_DLOAD_MODE_BIT_MASK | SBL_MINIDUMP_MODE_BIT_MASK);
    ...
    if (dload_flag)
    {
      ...
      HWIO_TCSR_BOOT_MISC_DETECT_OUTM(SBL_DLOAD_MODE_BIT_MASK | SBL_MINIDUMP_MODE_BIT_MASK,0);      
      status = TRUE;
      break;
    }
  }while(0);
...
boolean boot_dload_is_dload_mode_set( void )
{
  ...
  do
  {
    ...
    if (HWIO_TCSR_BOOT_MISC_DETECT_INM(SBL_DLOAD_MODE_BIT_MASK | SBL_MINIDUMP_MODE_BIT_MASK))
    {
      status=TRUE;
      break;
    }
...
void boot_dload_transition_pbl_forced_dload( void )
{
   ...
   /* EDL does not support UART, hence force device to re-enter SBL */
  ...
  register_value =HWIO_TCSR_BOOT_MISC_DETECT_INM(HWIO_TCSR_BOOT_MISC_DETECT_RMSK);
