...
/*=============================================================================
  ...
  $Header: //components/rel/boot.xf/0.2/QcomPkg/Mdm9x05Pkg/Library/XBLLoaderLib/sbl1_target.c#3 $
...
extern uint8 sbl1_partition_name[];
static boot_boolean ramdump_loaded = FALSE;
...
whitelst_tbl_entry_type xbl_ramdump_table[] =
...
static void xbl_ramdump_configure_header(bl_shared_data_type *bl_shared_data )
{
  ...
  boot_set_elf_and_prog_header(
      ((sec_img_auth_elf_info_type *)((pbl_elf_img_share_info_type *)bl_shared_data->sbl_shared_data->pbl_shared_data->pbl_elf_shared_data)->elf_img_meta_info)->elf_hdr,
      ((sec_img_auth_elf_info_type *)((pbl_elf_img_share_info_type *)bl_shared_data->sbl_shared_data->pbl_shared_data->pbl_elf_shared_data)->elf_img_meta_info)->prog_hdr
    );
  boot_set_hash_segment_buffer(
      (void *)((sec_img_auth_elf_info_type *)((pbl_elf_img_share_info_type *)bl_shared_data->sbl_shared_data->pbl_shared_data->pbl_elf_shared_data)->elf_img_meta_info)->hash_seg_hdr
  );

  /* Configure the target image using custom partition ID information */ 
  boot_flash_configure_target_image(sbl1_partition_name);
}

static void boot_set_ramdump_flag(bl_shared_data_type *bl_shared_data)
{
  ramdump_loaded = TRUE;
}

boot_boolean boot_get_ramdump_flag(void)
{
  return ramdump_loaded;
}

static void boot_reset_ramdump_flag(bl_shared_data_type *bl_shared_data)
{
  ramdump_loaded = FALSE;
}
...
boot_procedure_func_type load_xbl_ramdump_post_procs[] = 
{
  boot_reset_ramdump_flag,
...
boot_configuration_table_entry xbl_ramdump_config_entry =
  { SBL1_IMG,                                   /* host_img_id */
    ...
    load_xbl_ramdump_post_procs,                                       /* post_procs */
    NULL,                                       /* load_cancel */
    sbl1_partition_name,                          /* target_img_partition_id */
    (uint8 *)"XBLRamDump Image Loaded, Delta",  /* target_img_str */
    xbl_ramdump_table,                          /* whitelist_ptr */
...
struct mem_block xbl_ramdump_mem_block =
  { SCL_RAMDUMP_CODE_BASE,
    ...
    MMU_L2_PAGETABLE_MEM_WRITE_BACK_CACHE,
...
void sbl1_dload_entry ()
{
  ...
  result =
    boot_mmu_page_table_map_single_mem_block(
     (uintnt *)bl_shared_data.sbl_shared_data->mmu_l1_pagetable_base,
  ...
  */
  ...
  xbl_ramdump_mem_block.access = MMU_L2_PAGETABLE_MEM_READ_WRITE;
  xbl_ramdump_mem_block.executable = MMU_L2_PAGETABLE_NON_EXECUTABLE_REGION;
  xbl_ramdump_mem_block.cachetype = MMU_L2_PAGETABLE_MEM_NON_CACHEABLE;
