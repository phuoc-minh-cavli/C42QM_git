...
===========================================================================
  ...
  $Header: //components/rel/boot.xf/0.2/QcomPkg/Mdm9x05Pkg/Library/ClockTargetLib/ClockCommon.c#2 $
  $DateTime: 2018/12/05 04:16:18 $
  $Author: pwbldsvc $
...
#define CLOCK_CMD_CFG_UPDATE_FMSK   HWIO_FMSK(GCC_BLSP1_UART1_APPS_CMD_RCGR, UPDATE)

#define CLOCK_CFG_REG_OFFSET        (HWIO_ADDR(GCC_BLSP1_UART1_APPS_CFG_RCGR)-HWIO_ADDR(GCC_BLSP1_UART1_APPS_CMD_RCGR))
#define CLOCK_CFG_CGR_SRC_SEL_FMSK  HWIO_FMSK(GCC_BLSP1_UART1_APPS_CFG_RCGR, SRC_SEL)
#define CLOCK_CFG_CGR_SRC_SEL_SHFT  HWIO_SHFT(GCC_BLSP1_UART1_APPS_CFG_RCGR, SRC_SEL)
#define CLOCK_CFG_CGR_SRC_DIV_FMSK  HWIO_FMSK(GCC_BLSP1_UART1_APPS_CFG_RCGR, SRC_DIV)
#define CLOCK_CFG_CGR_SRC_DIV_SHFT  HWIO_SHFT(GCC_BLSP1_UART1_APPS_CFG_RCGR, SRC_DIV)

#define CLOCK_CFG_CGR_MODE_FMSK     HWIO_FMSK(GCC_BLSP1_UART1_APPS_CFG_RCGR, MODE)
#define CLOCK_CFG_CGR_MODE_SHFT     HWIO_SHFT(GCC_BLSP1_UART1_APPS_CFG_RCGR, MODE)
#define CLOCK_CFG_CGR_MODE_DUAL_EDGE_VAL  0x2
#define CLOCK_CFG_CGR_MODE_BYPASS_VAL     0x0

#define CLOCK_M_REG_OFFSET         (HWIO_ADDR(GCC_BLSP1_UART1_APPS_M)-HWIO_ADDR(GCC_BLSP1_UART1_APPS_CMD_RCGR))
#define CLOCK_N_REG_OFFSET         (HWIO_ADDR(GCC_BLSP1_UART1_APPS_N)-HWIO_ADDR(GCC_BLSP1_UART1_APPS_CMD_RCGR))
#define CLOCK_D_REG_OFFSET         (HWIO_ADDR(GCC_BLSP1_UART1_APPS_D)-HWIO_ADDR(GCC_BLSP1_UART1_APPS_CMD_RCGR))
...
boolean Clock_EnablePLL(const ClockConfigPLLType *pConfig)
{
       ...
       (nModeVal & HWIO_FMSK(GCC_STROMER_PLL_MODE, PLL_VOTE_FSM_ENA)) )
  {
    ...
    {
       active_flag = (inpdw(nStatusAddr) & nStatusMask);
       if((pConfig->ePLL != CLK_SOURCE_PLL_STROMER) &&  (active_flag== 0) )
       {
         nModeVal &= ~HWIO_FMSK(GCC_STROMER_PLL_MODE, PLL_BIAS_COUNT);
         nModeVal |= (0x18 << HWIO_SHFT(GCC_STROMER_PLL_MODE, PLL_BIAS_COUNT));
         outpdw(nModeAddr, nModeVal);
         outpdw(nModeAddr + HWIO_PLL_CONFIG_CTL_OFFS_SR, 0x30031800);
       }
      outpdw(pConfig->nVoteAddr, inpdw(pConfig->nVoteAddr) | pConfig->nVoteMask);
      if((pConfig->ePLL != CLK_SOURCE_PLL_STROMER) &&  (active_flag== 0) )
      {
         busywait(30);
         outpdw(nModeAddr + HWIO_PLL_CONFIG_CTL_OFFS_SR , 0x31800);
      }
...
boolean Clock_ConfigurePLL(const ClockConfigPLLType *pConfig)
{
  ...
  {
    if((pConfig->nVoteAddr)&&(inpdw(pConfig->nVoteAddr) & pConfig->nVoteMask))
    {
      return FALSE;
    }
  }
...
boolean Clock_ToggleClock(uintnt CBCR_addr, Clock_CBCRtoggleType toggle_clk)
{
  ...
  {
    CBCR_value = CBCR_value | HWIO_FMSK(GCC_BLSP1_UART1_APPS_CBCR, CLK_ENABLE);
    outpdw(CBCR_addr, CBCR_value);  
    do
    {
      CBCR_value = inpdw(CBCR_addr);
    }while((CBCR_value & HWIO_FMSK(GCC_BLSP1_UART1_APPS_CBCR, CLK_OFF)) != 0);
...
boolean Clock_DetectMuxCfg(const ClockConfigMuxType *pMuxCfg,ClockConfigMuxType *pCurrentCfg)
{
  ...
  nSource = 0;
  nCfgCGRAddr = pMuxCfg->nCMDRCGRAddr + CLOCK_CFG_REG_OFFSET; 
  nCfgCGRVal  = inpdw(nCfgCGRAddr);
