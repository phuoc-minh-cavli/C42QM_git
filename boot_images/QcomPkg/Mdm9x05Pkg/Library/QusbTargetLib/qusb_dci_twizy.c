...
//MDM gpio to be enabled for turning on 3P3v on BOB
#define USB_GPIO_3P3_EN                                      (29)

// TODO: Update the index after confirming with PMIC team
...
{
  (void)qusb_dci_tx_fifo;
#if 0 //Ramdumps not working on VT with these settings
  uint32 base_addr;
  uint32 depth;
  uint32* addr;
  int i;

  addr = (uint32*)HWIO_USB2_GTXFIFOSIZ_REGS_p_GTXFIFOSIZ0_ADDR(0);
  
  for(i=0; i<QUSB_DCI_MAX_TX_FIFO_NUM; i++)
  {
    base_addr = (qusb_dci_tx_fifo[i].base_addr << HWIO_USB2_GTXFIFOSIZ_REGS_p_GTXFIFOSIZ0_TXFSTADDR_N_SHFT) 
                 & HWIO_USB2_GTXFIFOSIZ_REGS_p_GTXFIFOSIZ0_TXFSTADDR_N_BMSK;
    depth = (qusb_dci_tx_fifo[i].depth << HWIO_USB2_GTXFIFOSIZ_REGS_p_GTXFIFOSIZ0_TXFDEP_N_SHFT) 
            & HWIO_USB2_GTXFIFOSIZ_REGS_p_GTXFIFOSIZ0_TXFDEP_N_BMSK;
...
void qusb_dci_typec_lane_detect(void)
{
  ...
  pmapp_chg_usb_connector_type  connector_type  = PM_APP_CHG_USB_CONNECTOR_INVALID;
  uint32                        loop_count    = 0;
  DALResult                     res;
  DalDeviceHandle               *tlmm = NULL;
  DALGpioSignalType             pin_config;

  res = DAL_DeviceAttach(DALDEVICEID_TLMM, &tlmm);
  if(res != DAL_SUCCESS)
  {
    return;
  }

  res = DalDevice_Open(tlmm, DAL_OPEN_SHARED);
  if(res != DAL_SUCCESS)
  {
    return;
  }

  pin_config = DAL_GPIO_CFG(
    (uint32)USB_GPIO_3P3_EN, 
    0, 
    DAL_GPIO_OUTPUT, 
    DAL_GPIO_NO_PULL, 
    DAL_GPIO_8MA);
  ...
  {
    if (PM_ERR_FLAG__SUCCESS != pmapp_chg_usb_get_usb_connector_type(&connector_type))
    {
      qusb_uart_log("vbus_pm_err",0);
      return;
    }

	  if(PM_APP_CHG_USB_CONNECTOR_MICRO_USB == connector_type)
	  {
		  return;
	  }
    qusb_dci_delay_us(QUSB_DCI_VBUS_HIGH_TIMEOUT_DELAY_US_PER_LOOP);
  }
  if (connector_type == PM_APP_CHG_USB_CONNECTOR_INVALID)
  {
    qusb_uart_log("connector type invalid",0);
    return;
  }
#if 0
