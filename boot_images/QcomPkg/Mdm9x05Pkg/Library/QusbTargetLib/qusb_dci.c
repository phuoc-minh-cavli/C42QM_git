...
// The below are not QSRAM buffers, they are IMEM addresses taken from PBL
#define QUSB_QSRAM_BASE                         (0x08C07C4C)
#define QUSB_COOKIE_SIZE                        (0x4)

#define QUSB_EVT_BUF_CUR_POS_COOKIE             (QUSB_QSRAM_BASE + (0*QUSB_COOKIE_SIZE))
#define QUSB_SINGLE_BOOT_ENUM_ADDR              (QUSB_QSRAM_BASE + (1*QUSB_COOKIE_SIZE))
#define QUSB_REMOTE_WAKEUP_COOKIE               (QUSB_QSRAM_BASE + (2*QUSB_COOKIE_SIZE))
#define QUSB_QSRAM_EVENT_BUFFER                 (0x08C07400)
...
{
  boolean is_vbus_high = TRUE;
  pm_err_flag_type pm_status = PM_ERR_FLAG__SUCCESS;
  // Read Vbus state using PMIC CHGPTH API.
  // Need to update once PMIC fixes the VBUS issue -- Akash --
  //pm_status = pmapp_chg_usb_get_vbus_presence(&is_vbus_high);
