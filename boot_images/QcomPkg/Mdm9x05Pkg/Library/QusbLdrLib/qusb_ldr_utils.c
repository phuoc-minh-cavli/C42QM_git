...
#include "pm_app_chg.h"        //charger detection api#include "pm_smbchg_usb_chgpth.h"  //vbus detection api
 ...
 */
// ===========================================================================
char* qusb_fedl_get_chgr_str(pmapp_chg_usb_apsd_result_status_type usb_charger_type)
{
  switch (usb_charger_type)
  {
    case PM_APP_CHG_SDP_CHARGER:         return " SDP_CHARGER";
    case PM_APP_CHG_OCP_CHARGER:         return " OCP_CHARGER";
    case PM_APP_CHG_CDP_CHARGER:         return " CDP_CHARGER";
    case PM_APP_CHG_DCP_CHARGER:         return " DCP_CHARGER";
    case PM_APP_CHG_FLOAT_CHARGER:       return " FLOAT_CHARGER";
    case PM_APP_CHG_QC_2P0:              return " QC_2P0";
    case PM_APP_CHG_QC_3P0:              return " QC_3P0";
    case PM_APP_CHG_ICL_OVERRIDE_LATCH:  return " ICL_OVERRIDE_LATCH";
    case PM_APP_CHG_ACA_DOCK:            return " ACA_DOCK";
    case PM_APP_CHG_ACA_C_CHARGER:       return " ACA_C_CHG";
    case PM_APP_CHG_ACA_B_CHARGER:       return " ACA_B_CHG";
    case PM_APP_CHG_ACA_A_CHARGER:       return " ACA_A_CHG";
    case PM_APP_CHG_INVALID:             return " INVALID";
...
static boolean qusb_fedl_is_chgr_sdp(void)
{
  ...
  //pmapp_chg_usb_apsd_result_status_type usb_charger_type = PM_APP_CHG_INVALID;
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    pm_err = pmapp_chg_usb_get_vbus_presence(&vbus_valid);
    ...
    if (FALSE == vbus_valid)
    {
      ...
      qusb_error_log(DCI_SKIP__CHGR_PORT_VBUS_LOW_LOG, 0, vbus_valid);
      return FALSE;
    }
    else
    {
      qusb_uart_log("fedl, vbus_high", 0);
      return TRUE;
	}
		
    //Commnented out because as per hardware design, pmic is not able to detect the charger type.
    /* pm_err = pmapp_chg_usb_get_charger_type(&usb_charger_type);
    if (PM_ERR_FLAG__SUCCESS != pm_err)
    {
      if(pm_err == PM_ERR_FLAG__FEATURE_NOT_SUPPORTED)
      {
        qusb_uart_log("fedl, pmic_not_present_assume_sdp", 0);
        return TRUE;
      }
    ...
    if (PM_APP_CHG_INVALID != usb_charger_type)
    {
      break;
    } */
  }
 ...
 /*  qusb_uart_w_str_log("usb: chgr", qusb_fedl_get_chgr_str(usb_charger_type));
  ...
  if (PM_APP_CHG_SDP_CHARGER != usb_charger_type)
  {
...
boolean qusb_ldr_utils_fedl_check(void)
{
  ...
  {
    qusb_uart_log("enter forced EDL", 0);
