...
$Header: //components/rel/boot.xf/0.2/QcomPkg/Mdm9x05Pkg/Library/XBLRamDumpLib/boot_dload_debug_target.c#5 $
...
**  Function :  dload_add_dcc_sram_region
...
* 
*/
static void dload_add_dcc_sram_region(void)
{
  dload_debug_add_region(OPTIONAL_DEF_SAVE,
                          (uint32)(SCL_DCC_SRAM_BASE),
                           (uint32)SCL_DCC_SRAM_SIZE,
                          "DCC SRAM region", "DCCSRAM.BIN");

  dload_debug_add_restricted_region((uint32)SCL_DCC_SRAM_BASE,
                                    (uint32)SCL_DCC_SRAM_SIZE);
}
